...
async def run_step1(categories, model_name, language, jsonl_path, force_regen):
    if force_regen:
    ...
    if os.path.exists(jsonl_path):
        logger.info(f"{jsonl_path} exists. Skipping Step 1 (LLM generation).")
        return
    ...
    logger.info(f"Step 1 complete. Raw data saved to {jsonl_path}")
...
def run_step2(
    ...
    if not os.path.exists(jsonl_path):
        logger.error(f"{jsonl_path} not found. Please run Step 1 first.")
        return
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        choices=[1, 2],
        help="1: Fetch and LLM, 2: Embed and Render. If omitted, runs both sequentially.",
    )
    ...
    if args.step in [1, None]:
        await run_step1(categories, model_name, language, jsonl_path, force_regen)
