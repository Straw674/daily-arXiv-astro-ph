...
def get_summary_prompt(language: str = "中文") -> str:
    ...
    return f"""你是一位严谨专业的天文领域学者。请基于论文标题与摘要，结合领域专业知识，输出结构化的 {language} JSON 总结：

字段要求：
1. `background_knowledge`：跳出论文具体细节，从宏观视角介绍该子领域的基础物理图像、研究范式或背景科普，为读者提供前置知识。
2. `contribution`：忠实于摘要，清晰陈述论文的核心工作与主要科学发现。

注意：字段内容请直接输出正文段落或无序列表，不要包含 Markdown 标题（如 `#`、`##`）。"""
