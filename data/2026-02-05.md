...
### [1] [pop-cosmos: Redshifts and physical properties of KiDS-1000 galaxies](https://arxiv.org/abs/2602.03930)
...
**TL;DR**: 首次将贝叶斯推断方法应用于百万量级的宽视场弱引力透镜巡天（KiDS-1000），通过pop-cosmos生成模型结合GPU加速MCMC采样，高效获取了400万星系的光谱能量分布（SED）拟合结果，验证了红移（z）和物理参数的联合约束精度（亮星系巡天偏差3×10⁻³、离散度0.04；红序星系偏差3×10⁻²、离散度0.05），并发现红序星系样本中存在z≈0.4的尘埃恒星形成污染体，同时基于恒星形成率识别出10%的静止星系（保守颜色切割为37%），为弱透镜样本的物理属性筛选提供了可扩展方案。


<details>
  <summary>Details</summary>

Background: 研究主题为基于贝叶斯推断的大规模弱引力透镜巡天中星系物理性质（如红移、恒星质量、恒星形成率）的精确测量及其在星系演化与宇宙学分析中的应用。

Data: 数据来源于KiDS-1000巡天的400万个星系的光谱能量分布（SED）拟合，并基于深度COSMOS2020测光数据进行校准。

Method: 采用基于贝叶斯推断的pop-cosmos生成模型，结合GPU加速的马尔可夫链蒙特卡洛（MCMC）采样和光谱能量分布（SED）拟合方法，在恒星种群合成（SPS）参数空间中对星系红移及物理性质进行联合约束。

Result: 首次将贝叶斯推断方法应用于百万量级宽视场弱引力透镜巡天（KiDS-1000），通过pop-cosmos生成模型对400万星系进行光谱能量分布（SED）拟合，在6.5 GPU秒/星系效率下获得红移与物理参数的联合后验分布。
...
### [2] [Dynamical Systematics on Time Delay Lenses and the Impact on the Hubble Constant](https://arxiv.org/abs/2602.03934)
*R. Forés-Toribio,C. S. Kochanek,J. A. Muñoz*

**TL;DR**: 该研究通过分析八个时间延迟透镜（time-delay lenses）系统，量化了速度弥散（velocity dispersion）测量中多种系统性误差（如点扩散函数表征、各向异性模型选择、恒星质量分布差异等）对哈勃常数（H_0）估算的影响，发现这些因素可导致Δσ²/σ²偏差达1-40%，进而限制H_0的测量精度至2%以内。


<details>
  <summary>Details</summary>

Background: 该研究探讨了利用时间延迟透镜（time-delay lenses）和速度弥散（velocity dispersion）测量哈勃常数（H0）时，如何通过分析潜在系统误差（如点扩散函数表征、速度弥散测量差异、各向异性模型选择以及恒星质量分布与光剖面模型偏差）来突破透镜收敛性（convergence）导致的简并性问题。

Data: 研究使用了八个用于宇宙学分析的时延透镜（time-delay lenses）系统的观测数据，重点考察了速度弥散（velocity dispersion）测量中的系统误差来源。

Method: 采用速度弥散(velocity dispersion)测量结合各向异性模型(anisotropy models)和Jeans方程(Jeans equations)分析，评估了PSF表征、狭缝对准误差、质量分布模型差异及径向颜色梯度等系统误差对哈勃常数(Hubble constant)估算的影响。

Result: 速度弥散（velocity dispersion）测量中的系统误差对哈勃常数（H_0）估计精度有显著影响，其中点扩散函数（PSF）表征偏差可导致Δσ²/σ²达1-6%，速度弥散与均方速度差异贡献3-8%，各向异性模型选择影响达5-18%，而恒星质量分布与光剖面模型差异影响高达5-40%。
...
### [3] [Low-mass stars dominate the hot (0.7 keV) Galactic X-ray emission](https://arxiv.org/abs/2602.03941)
...
**TL;DR**: 通过分析SRG/eROSITA观测数据发现银河系西半球0.7 keV X射线辐射主要源自未分辨的低质量恒星（M型矮星及F、G、K型恒星），其光度与局部恒星质量分布高度相关，并约束了假设的超位力（super-virial）热晕在10 kpc处的电子密度上限为4×10⁻⁴ cm⁻³。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系环星系介质（circumgalactic medium, CGM）中可能存在的高温超位力（super-virial）等离子体成分的物理本质及其观测特征。

Data: 数据来源于SRG/eROSITA（Spectrum-Roentgen-Gamma/eROSITA）对整个银河系西半球X射线背景的观测。

Method: 采用SRG/eROSITA观测数据分析了银河系西半球X射线背景，通过建立0.7 keV表面亮度与恒星质量分布的关联性，结合球对称β模型（β=0.4）约束超位力（super-virial）成分的电子密度上限。

Result: 银河系环星系介质(CGM)中观测到的0.7 keV发射主要来源于未分辨的低质量恒星（M型矮星及F、G、K型恒星），其单位质量X射线光度与太阳附近10 pc内同类恒星相当。
...
### [4] [On the coupled origin of the stellar IMF and multiplicity](https://arxiv.org/abs/2602.03976)
*B. Thomasson,I. Joncour,E. Moraux,F. Motte,T. Yoo,A. Ginsburg*

**TL;DR**: 研究发现恒星初始质量函数（IMF）的萨尔皮特斜率（Salpeter slope）和转折峰值（turn-over peak）源于气体团块（gas clumps）的层级碎裂（hierarchical fragmentation）过程，其中大质量核心（massive cores）碎裂效率较低且碎片数量较少，而低质量恒星（low-mass stars）因动力学不稳定导致伴星数量是高质量恒星（massive stars）的五倍。


<details>
  <summary>Details</summary>

Background: 研究探讨了初始质量函数（IMF）与核质量函数（CMF）之间的继承关系，重点关注气体团块的分级碎裂过程如何影响IMF的形成及恒星系统的多重性。

Data: 数据来源于W43-MM2&MM3恒星形成区观测到的具有顶部重（top-heavy）幂律分布的核质量函数（CMF）。

Method: 采用尺度无关（scale-free）的层级碎裂（hierarchical fragmentation）模型分析气体团块碎裂对初始质量函数（IMF）的影响，并探讨碎裂过程在质量依赖和质量无关两阶段的差异性。

Result: 分层碎裂（hierarchical fragmentation）将初始质量函数（IMF）的转折峰归因于至少四次碎裂层级，并揭示大质量恒星（>10 Msun）在0.2-2.5 kAU范围内平均拥有0.9个伴星，而低质量恒星（<0.1 Msun）伴星数量是其五倍。
...
### [5] [A gas-phase "top-down" chemical link between aldehydes and alcohols](https://arxiv.org/abs/2602.03989)
*Christopher N. Shingledecker,Germán Molpeceres,A. Mackenzie Flowers,Deaton Warren,Emma Stanley,Anthony Remijan*

**TL;DR**: 通过密度泛函理论（DFT）计算和天体化学模型揭示了在恒星形成区Sgr B2 (N)中，乙醇（CH3CH2OH）与卤素（氟、氯）的无势垒反应可生成乙醛（CH3CHO），并发现乙醇自由基CH3CHOH在星际介质中可能具有可观测丰度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星形成区人马座B2（Sgr B2 (N)）中星际复杂有机分子（COMs）的化学关联，特别是醇类（alcohols）与醛类（aldehydes）之间的气相反应路径。

Data: 研究采用密度泛函理论（DFT）计算分析了乙醇（CH3CH2OH）与卤素（氟和氯）的气相反应路径，并将这些反应纳入人马座B2（N）恒星形成区的天体化学模型中。

Method: 采用密度泛函理论（DFT）计算研究了乙醇与卤素（氟和氯）的气相反应路径，并通过天体化学模型评估了这些反应在真实星际环境中的重要性。

Result: 乙醇（CH3CH2OH）与卤素（氟和氯）的气相反应可通过无势垒氢原子抽提及后续氢原子加成反应生成乙醛（CH3CHO），并在热核（hot cores）条件下有效提升乙醛丰度。
...
### [6] [Intermediate-Mass Black Hole Formation from Hierarchical Mergers in Galactic Nuclei](https://arxiv.org/abs/2602.04176)
*Amanda Newton,Sanaea C Rose,Fulya Kiroglu,Bao-Minh Hoang,Frederic Rasio*

**TL;DR**: 研究发现核星团(NSCs)中单恒星质量黑洞(BHs)通过直接碰撞和引力波(GW)捕获可形成约500 M$_{\odot}$的中等质量黑洞(IMBHs)，其最终特性及GW源产生受初始BH质量和自旋分布显著影响，且原始双星会使二代及以上合并事件增加一个数量级。


<details>
  <summary>Details</summary>

Background: 研究聚焦于核星团（NSCs）等致密恒星环境中单恒星质量黑洞（BHs）与超大质量黑洞（SMBH）相互作用对引力波（GW）源形成的动力学影响。

Data: 数据来源于半解析模型（semianalytic model）模拟的核星团（NSCs）内单恒星质量黑洞（BHs）群体，考虑了黑洞与恒星直接碰撞及黑洞间引力波捕获（GW capture）过程，并探索了初始黑洞质量和自旋分布对其最终性质及引力波源产生的影响。

Method: 采用半解析模型（semianalytic model）分析黑洞（BHs）与恒星的直接碰撞及黑洞间的引力波（GW）捕获过程，并考察初始黑洞质量与自旋分布对最终性质及引力波源产生的影响。

Result: 核星团(NSCs)中单恒星质量黑洞(BHs)的初始质量分布上限可形成约500 M$_{\odot}$的中等质量黑洞(IMBHs)，而下限分布则无法形成。
...
### [7] [Kinematic-Distance Biases in the Inner Milky Way from a Stellar-Dynamically Constrained Bar](https://arxiv.org/abs/2602.04177)
*Junichi Baba*

**TL;DR**: 该论文量化了棒旋结构（bar）驱动的非圆周运动对基于运动学距离（KD）方法重建的银河系气体分布图的偏差影响，发现KD方法在棒旋主导区域（R~0.5-3kpc）因非圆周速度场产生系统性误差，导致各向异性伪影和径向混合效应，但在外区（R≳5kpc）仍能较好还原气体分布。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系棒状结构（bar）驱动的非圆周运动如何影响基于运动学距离（kinematic-distance, KD）方法推断的气体分布图。

Data: 数据来源于高分辨率流体动力学模拟（high-resolution hydrodynamical simulations），模拟了观测约束下的棒旋银河系势场（barred Milky Way potential）中的气体流动，并从中提取准稳态快照生成了合成经度-速度数据。

Method: 采用高分辨率流体动力学模拟（hydrodynamical simulations）结合运动学距离（KD）反演方法，基于观测约束的棒状银河势场（barred Milky Way potential）生成合成经度-速度数据，并通过几何敏感度（S）量化非圆周运动（non-circular motions）对距离误差的影响。

Result: 在棒状结构主导区域（$R \sim 0.5$--3~kpc），运动学距离（KD）方法重构的气体分布呈现各向异性、象限依赖的伪影，包括弧状过密度和视线延伸的低密度腔体，导致径向混合效应并掩盖真实的棒诱导凹陷（bar gap）。
...
### [8] [A high rate of foreground contaminants toward high-redshift galaxies resolved by JWST](https://arxiv.org/abs/2602.04179)
*Weiyang Liu,Linhua Jiang*

**TL;DR**: 基于哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）观测，研究发现高红移莱曼阿尔法发射体（LAEs）中约50%具有多组分结构，其中68%的组分是真实关联的年轻低质量星暴星系，且其比例随投影距离增大而显著降低，同时指出地面观测中2角秒孔径测光可能因前景污染高估关键参数如恒星质量。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）观测高红移莱曼阿尔法发射体（LAEs）的多重结构特征，以揭示其物理性质及前景污染对传统地面观测的影响。

Data: 数据来源于哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）的观测图像，并收集了840个光谱确认的莱曼阿尔法发射体（LAEs）样本，其中248个具有两个或更多相对孤立的成分。

Method: 采用哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）的高分辨率图像分析技术，结合光谱学确认和测光红移（photometric redshift）估计方法，对高红移莱曼α发射体（LAEs）的多重成分进行详细研究，并通过光谱能量分布（SED）建模评估其恒星形成活动和物理性质。

Result: 在JWST（James Webb Space Telescope）图像中，约50%的高红移莱曼α发射体（LAEs）呈现多组分结构，其中68%的组分是红移与光谱红移一致的“真实组分”，且其比例随投影距离增加从80%（0.2-0.4角秒）快速降至30%（0.8-1.0角秒）。
...
### [9] [Variable Modified Newtonian Mechanics VIII: Ultra faint dwarfs in Milky Way](https://arxiv.org/abs/2602.04273)
*James Chi Chung Wong*

**TL;DR**: 论文提出可变修正牛顿动力学模型（Variable Modified Newtonian dynamics）解释了超暗弱矮星系（Ultra faint dwarf galaxies）速度弥散度显著偏离重子塔尔-费希尔关系（Baryon Tully Fisher Relation）的现象。


<details>
  <summary>Details</summary>

Background: 超暗弱矮星系（UFD）的速度弥散度显著偏离重子塔尔-费希尔关系（BTFR），需通过可变修正牛顿动力学模型解释这一异常。

Data: 数据来源于对超暗弱矮星系（Ultra faint dwarf galaxies, UFD）速度弥散（velocity dispersion）的观测。

Method: 采用可变修正牛顿动力学模型（Variable Modified Newtonian dynamics model）解释超暗弱矮星系（UFD）与重子塔尔-费希尔关系（BTFR）的偏离。

Result: 可变修正牛顿动力学模型（Variable Modified Newtonian dynamics）可以解释超暗弱矮星系（Ultra faint dwarf galaxies）速度弥散度与重子塔尔-费希尔关系（Baryon Tully Fisher Relation）的显著偏离。
...
### [10] [BEACON: JWST NIRCam Pure-parallel Imaging Survey. IV. A Systematic Search for Galaxy Overdensities and Evidence for Gas Accretion Mode Transition](https://arxiv.org/abs/2602.04333)
...
**TL;DR**: 基于JWST（James Webb Space Telescope）BEACON巡天数据，在红移1.5到5范围内发现207个显著星系过密度区，揭示星系淬灭存在两条路径：大质量晕（halo）内过密度区淬灭比例更高且特定恒星形成率（sSFR）更低（红移≳2时减弱），以及红移<2时局域密度越高淬灭比例越高且sSFR越低（红移>3时可能反转）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（James Webb Space Telescope）的BEACON巡天数据，系统探索宇宙正午（cosmic noon）时期星系致密区（galaxy overdensities）的环境效应对星系淬灭（galaxy quenching）的双重路径影响。

Data: 数据来源于BEACON（JWST Cycle 2 NIRCam纯并行成像巡天）的20个独立视场，每个视场至少包含6个滤光片（F090W、F115W、F150W、F277W、F356W和F444W）的观测数据。

Method: 采用自适应核密度估计方法（adaptive kernel-density estimation），结合每个星系的全光变红移概率分布函数（photometric redshift probability distribution function）绘制星系过密度分布图。

Result: 在宇宙正午时期（cosmic noon），星系淬灭（quenching）呈现两条路径：（i）质量更大的晕（halo）内过密区（overdensities）具有更高的淬灭比例和更低的中位特定恒星形成率（sSFR），该趋势在红移（redshift）$z\gtrsim2$时减弱；（ii）红移$z<2$时淬灭比例随局部密度（local density）增加而上升且sSFR下降，但该环境趋势在$z\sim2$--$3$减弱并在$z>3$可能出现反转。
...
### [11] [Atom Addition Formation of Thionylimide (HNSO) on Interstellar Dust Grains: Chemical routes requiring oxygen and nitrogen atom surface diffusion](https://arxiv.org/abs/2602.04374)
...
**TL;DR**: 研究发现星际冰粒表面通过原子氧(O)和氮(N)与自由基NS、SO反应高效生成关键中间体NSO并进一步氢化为低能顺式构型HNSO，其固态丰度与OCS相当，表明冰粒扩散化学对致密云硫化学有重要贡献。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际冰尘表面化学反应（surface chemistry on interstellar icy grains）中HNSO分子的形成机制及其在天体化学（astrochemistry）中的意义。

Data: 数据来源于量子化学计算（quantum chemical calculations）和天体化学模型（astrochemical models）的结合使用，其中量子化学计算针对冰表面反应，而天体化学模型用于模拟星际环境中的化学反应过程。

Method: 采用量子化学计算（quantum chemical calculations）和天体化学模型（astrochemical models）相结合的方法，重点研究了冰颗粒表面通过原子氧（O）和氮（N）与自由基NS、SO反应形成关键中间体NSO，并进一步氢化生成HNSO的过程。

Result: HNSO主要通过冰粒表面原子氧和氮与自由基NS、SO反应形成，其中NSO是关键中间体，随后氢化生成最低能量的顺式(cis)构象体，而反式(trans)构象体在典型星际条件下可能短暂存在。
...
### [12] [Nonlinear Saturation of the Acoustic Resonant Drag Instability](https://arxiv.org/abs/2602.04710)
*Ben Y. Israeli,Jonathan Squire,Eric Moseley,Amitava Bhattacharjee*

**TL;DR**: 论文揭示了声学共振拖曳不稳定性（acoustic RDI）的非线性演化机制，发现其增长和饱和状态由不稳定性增长时间尺度与湍流涡旋翻转时间尺度的平衡决定，并建立了各向异性外强迫范围与各向同性湍流惯性范围共存的饱和态模型。


<details>
  <summary>Details</summary>

Background: 共振拖曳不稳定性（RDI）是一类新型的尘埃/流体不稳定性，广泛存在于多种天体物理环境中，由尘埃流与背景介质中波的共振相互作用驱动。

Data: 数据来源于对声学共振拖曳不稳定性（acoustic RDI）的数值模拟（simulation），重点关注超音速流动尘埃与气体中声波的相互作用。

Method: 采用流体动力学模拟（hydrodynamic simulation）研究声学共振拖曳不稳定性（acoustic RDI）的非线性增长与饱和状态，通过分析不稳定性增长时间尺度与湍流涡旋翻转时间尺度的平衡关系建立模型。

Result: 非线性增长和饱和状态由不稳定性增长时间尺度与湍流涡旋翻转时间尺度的平衡所表征。
...
### [13] [Evolution of dust attenuation in star-forming galaxies with UV slope, stellar mass, and redshift out to $z \sim 5$](https://arxiv.org/abs/2602.04765)
*J. V. Wijesekera,M. P. Koprowski,J. S. Dunlop,K. Lisiecki,D. J. McLeod,R. J. McLure,M. J. Michałowski,M. Solar*

**TL;DR**: 通过分析近红外波段（K-band）选出的约10^5个恒星形成星系样本，建立了红外超额（IRX）与紫外斜率（β）、恒星质量（M*）及红移（redshift）的定量关系，发现低质量星系中尘埃消光规律（attenuation law）斜率随质量增加而变平，且高红移（z≲2-3）大质量系统中IRX因冷气体吸积（cold-gas accretion）抑制而出现转折。


<details>
  <summary>Details</summary>

Background: 研究探讨了红外超量（IRX）与紫外斜率（β）、恒星质量（M*）及红移（redshift）之间的依赖关系，旨在建立可用于修正紫外/光学选择星系样本中尘埃吸收效应的统一函数关系。

Data: 数据来源于UDS和COSMOS天区的K波段选源样本，包含约10^5个恒星形成星系，排除了静止源和已知星暴星系，并通过在赫歇尔(Herschel)和JCMT远红外(FIR)图像中的叠加测量红外光度。

Method: 采用K波段选择样本，通过SED拟合获取紫外斜率(UV slope)β，利用中位数叠加法在赫歇尔(Herschel)和JCMT远红外图中计算红外超量(IRX)，并建立IRX与β、恒星质量(M*)及红移的分段函数关系。

Result: 在红移z≲2-3时，红外超量（IRX）随恒星质量（stellar mass）单调上升但在高质量端出现转折，与大质量系统中冷气体吸积和尘埃增长受抑制一致。
...
### [14] [The Atacama Cosmology Telescope: Constraints on Local Non-Gaussianity from the ACT Cluster Catalog](https://arxiv.org/abs/2602.03917)
...
**TL;DR**: 利用ACT DR6苏尼亚耶夫-泽尔多维奇(Sunyaev-Zel'dovich)星系团样本，通过建模1201个星系团的红移和质量依赖数量分布，结合Log-Edgeworth晕质量函数，在ΛCDM框架下测得局部型原初非高斯性参数fNL=55±125（68%置信水平），与高斯初始条件一致，并发现16.4%残余质量偏差对fNL约束影响可忽略。


<details>
  <summary>Details</summary>

Background: 研究主题为利用ACT DR6 SZ（Sunyaev-Zel'dovich）星系团样本约束局域型原初非高斯性（local-type primordial non-Gaussianity），以检验宇宙初始条件的性质。

Data: 数据来源于ACT DR6（Atacama Cosmology Telescope Data Release 6）的Sunyaev-Zel'dovich（SZ）星系团目录，包含Legacy区域10,347平方度内1,201个星系团的红移和质量依赖的计数数据，并考虑了巡天完备性、SZ固有散射和弱透镜校准的质量偏差。

Method: 采用对数-埃奇沃斯晕质量函数(Log-Edgeworth halo mass function)计算理论丰度，并考虑巡天完备性、固有SZ散射和弱透镜校准的质量偏差。

Result: result：基于ACT DR6 SZ（Sunyaev-Zel'dovich）星系团目录分析，在ΛCDM框架下测得局部型原初非高斯性参数f_NL = 55 ± 125（68%置信水平），与高斯初始条件一致，并发现16.4%残余质量偏差对f_NL约束影响可忽略。
...
### [15] [An emulator for the ionizing photon mean free path in ultra-high resolution simulations: the implications of mean free path measurements for the reionization history](https://arxiv.org/abs/2602.03923)
*Hurum Maksora Tohfa,Christopher Cain,Matthew McQuinn,Anson D'Aloisio*

**TL;DR**: 基于高分辨率宇宙学辐射传输模拟开发深度学习模拟器（emulator），精确预测再电离（reionization）过程中电离光子平均自由程（MFP），发现再电离中点（midpoint）为$z_{\rm re} = 6.8\pm 1.2$且$z \lesssim 6$时仍存在显著中性氢，支持晚再电离模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用深度学习模拟器预测高红移（$z \sim 5$-$6$）类星体光谱中电离光子的平均自由程（MFP），以约束宇宙再电离（reionization）历史并解决大尺度模拟无法解析的千秒差距级团簇效应问题。

Data: 数据来源于高分辨率宇宙学辐射传输模拟（high-resolution cosmological radiative transfer simulations），模拟了电离前沿在小尺度2共动Mpc/h体积中的传播过程。

Method: 采用残差多层感知器（residual multi-layer perceptron）神经网络构建深度学习模拟器，基于高分辨率宇宙学辐射传输模拟（cosmological radiative transfer simulations）预测电离光子平均自由程（mean free path）。

Result: result：研究发现再电离中点（midpoint of reionization）为$z_{\rm re} = 6.8\pm 1.2$，且宇宙在$z_{\rm re} < 5.8 ~(6.3)$仍保持10%中性（neutral fraction），同时全球电离发射率（global ionizing emissivity）在$z = 6$至$4.8$间下降2-3倍。
...
### [16] [The local void model for the Hubble and BAO tensions](https://arxiv.org/abs/2602.03928)
*Indranil Banik,Harry Desmond,Vasileios Kalaitzidis,Sergij Mazurenko*

**TL;DR**: 研究发现低红移(z < 0.15)宇宙中观测到的哈勃常数(Hubble tension)矛盾可能由银河系所处的局部大尺度低密度区(KBC void)引力效应导致，该模型能同时解释低红移重子声波振荡(BAO)距离异常(α_iso < 1)并预测红移z > 0.2时膨胀率会快速收敛至宇宙微波背景(CMB)预期值H_0^CMB。


<details>
  <summary>Details</summary>

Background: 研究聚焦于哈勃常数（Hubble constant）在局部低红移（low-redshift）宇宙与宇宙微波背景（CMB）基于ΛCDM模型推断值之间的不一致性（Hubble tension），探讨局部大尺度低密度区（local void）对其的影响机制。

Data: 数据来源包括多波段星系计数（multi-wavelength galaxy number counts）、低红移BAO距离异常（low-redshift BAO distance anomaly）、未校准的Ia型超新星（uncalibrated Type Ia supernovae）、运动学Sunyaev-Zel'dovich效应（kinematic Sunyaev-Zel'dovich effect）、快速射电暴（fast radio bursts）以及红移漂移测量（redshift drift measurements）。

Method: 采用半解析空洞模型（semi-analytic void models）分析局部低密度区域（如KBC空洞）对哈勃张力的影响，并结合多波段星系计数、体流（bulk flows）动力学要求及低红移BAO距离异常（α_iso < 1）进行验证。

Result: 哈勃张力（Hubble tension）主要由极低红移（z < 0.15）的数据驱动，而中等红移测量结果与ΛCDM模型基于宇宙微波背景（CMB）预期的H₀^CMB一致。
...
### [17] [pop-cosmos: Forward modeling KiDS-1000 redshift distributions using realistic galaxy populations](https://arxiv.org/abs/2602.03935)
...
**TL;DR**: 开发了结合生成模型(pop-cosmos)和数据模型(SKiLLS)的前向建模框架，首次实现了不依赖光谱重加权方法直接推断Kilo-Degree巡天(KiDS)宇宙剪切样本的断层扫描红移分布，在极端红移区间(0.1<z<0.3和0.9<z<1.2)发现与半解析模型(shark)存在Δz~0.05-0.1的系统差异。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升第四阶段星系巡天（Stage IV galaxy surveys）宇宙学参数测量精度，通过开发前向建模框架解决星系红移分布推断这一关键瓶颈问题。

Data: 数据来源包括用于校准星系演化生成模型（pop-cosmos）的COSMOS2020中Spitzer IRAC Ch.1<26星系样本，以及从基于SURFS的KiDS-Legacy-Like模拟（SKiLLS）中机器学习得到的数据噪声和选择模型。

Method: 采用前向建模框架（forward-modeling framework），结合基于COSMOS2020校准的星系种群生成模型（pop-cosmos）和从SKiLLS模拟中机器学习得到的数据噪声与选择模型。

Result: 通过开发包含\texttt{pop-cosmos}星系演化生成模型和基于SKiLLS的数据模型的前向建模框架，首次实现了不依赖光谱重加权方法直接推断KiDS宇宙剪切样本的红移分布，在五个红移层析区间中前两个和后两个区间发现平均红移存在$Δz\sim0.05$-$0.1$的系统差异。
...
### [18] [A simple mechanism for the enhancement of the inflationary power spectrum](https://arxiv.org/abs/2602.04055)
*I. Dalianis,A. Katsis,N. Tetradis*

**TL;DR**: 研究发现双场暴胀（two-field inflation）背景演化存在两个不同阶段，其中第二阶段在更低能标下发生的急剧场空间转向会显著放大穿越哈勃视界的涨落，从而在标量功率谱中产生峰值并导致显著的原初黑洞（primordial black holes）和次级引力波（secondary gravitational waves）丰度。


<details>
  <summary>Details</summary>

Background: 该研究探讨了双场暴胀（two-field inflation）背景下宇宙早期演化过程中两个不同能量尺度阶段的场空间轨迹转变及其对原初扰动放大的影响。

Data: 数据来源于两场暴胀（two-field inflation）模型的数值模拟（simulation），重点关注场空间（field space）中急剧转变阶段的演化特征。

Method: 采用两场暴胀（two-field inflation）模型分析背景演化，重点研究场空间快速转向（rapid turns）阶段对原初扰动的放大效应。

Result: 两场暴胀（two-field inflation）背景演化包含两个不同阶段，分别对应沿两个连续场方向的运动，其中第二阶段发生在显著较低能标时会导致场空间轨迹的急剧转变和快速转向。
...
### [19] [From Nonparametric Distance Reconstruction to Testing the Etherington Relation and Cosmic Curvature Using 2D and 3D BAO Measurements](https://arxiv.org/abs/2602.04203)
*Darshan Kumar,Jie Zheng,Zhi-Qiang You,Da-Chun Qiang*

**TL;DR**: 结合重子声学振荡（BAO）和宇宙计时器（Cosmic Chronometers）数据，通过高斯过程（Gaussian Process）非参数重建方法，验证了宇宙距离对偶关系（CDDR）在99%置信水平下成立，并发现宇宙曲率（Ω_{k0}）虽轻微偏向非平坦但仍与平坦宇宙在95%置信水平兼容。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过结合重子声学振荡（BAO）观测和宇宙计时器（Cosmic Chronometers）数据，联合测试宇宙曲率（$Ω_{k0}$）和宇宙距离对偶关系（CDDR）的有效性。

Data: 数据来源包括重子声学振荡（BAO）观测的角直径距离测量值（2D BAO、3D BAO和3D DESI BAO），以及通过高斯过程（Gaussian Process）非参数技术从宇宙计时器（Cosmic Chronometers）的哈勃参数$H(z)$数据重建的光度距离。

Method: 采用高斯过程（Gaussian Process）非参数技术重建哈勃参数$H(z)$的光度距离，并结合重子声学振荡（BAO）观测的角直径距离测量，系统比较不同BAO数据集（2D BAO、3D BAO和3D DESI BAO）对距离对偶参数$η(z)$和宇宙曲率$Ω_{k0}$的约束影响。

Result: 所有重子声学振荡（BAO）数据集和参数化方案下，距离对偶参数η(z)在99%置信水平上与1一致，未发现宇宙距离对偶关系（CDDR）的违反证据。
...
### [20] [Parametric Resonance and Backreaction Effects in Magnetogenesis from Ultralight Dark Matter](https://arxiv.org/abs/2602.04285)
*Nirmalya Brahma,Robert Brandenberger*

**TL;DR**: 研究发现超轻暗物质（ultralight dark matter）与电磁场耦合时，除原有速调共振（tachyonic resonance）外还存在窄带参量共振（narrow band parametric resonance）通道，能在极小耦合常数下主导红外模式指数放大，并将部分暗物质密度有效转化为可观测宇宙尺度磁场，且该机制对反作用效应具有鲁棒性。


<details>
  <summary>Details</summary>

Background: 研究探讨了超轻暗物质（ultralight dark matter）与电磁场耦合触发的新型磁生成机制（magnetogenesis mechanism）及其在宇宙学尺度上的可行性。

Data: data：研究基于理论模拟（simulation）数据，分析了超轻暗物质（ultralight dark matter）与电磁场耦合触发的磁生成机制，重点关注窄带参数共振（narrow band parametric resonance）和逆反应（back-reaction）效应的影响。

Method: 采用窄带参数共振（narrow band parametric resonance）和Floquet指数分析，研究了超轻暗物质（ultralight dark matter）与电磁场耦合下的光子生成机制。

Result: 发现存在一个窄带参数共振通道（narrow band parametric resonance channel），其特征弗洛凯指数（Floquet exponent）略低于快子共振（tachyonic resonance），但在极小耦合常数参数区域内该窄带共振仍足以产生宇宙学尺度观测磁场。
...
### [21] [Large-scale halo velocity correlations and the impact of finite simulation volumes](https://arxiv.org/abs/2602.04485)
*Yao-Tsung Chuang,Teppei Okumura,Takahiro Nishimichi*

**TL;DR**: 通过分析不同尺寸N体模拟中暗物质晕（dark-matter halos）的速度关联函数，发现有限模拟盒尺寸会导致大尺度速度关联振幅被抑制（尤其对高质量晕更显著），提出采用汉克尔变换（Hankel transform）下限$k_{\rm min}$作为自由参数可消除对生长率参数$f(z)σ_8(z)$的估计偏差。


<details>
  <summary>Details</summary>

Background: 研究探讨了数值模拟中暗物质（dark matter）和暗物质晕（halo）的速度关联函数（velocity correlation functions）在大尺度上与理论预测的差异，重点关注有限模拟盒子尺寸（box size）对测量结果的影响及其在宇宙学参数约束中的重要性。

Data: 数据来源于边长为1Gpc/h和2Gpc/h的N体数值模拟（N-body simulation），并通过将大盒子模拟细分为1Gpc/h的子盒子进行对比分析。

Method: 采用不同尺寸的N体数值模拟（N-body simulations）分析有限模拟盒尺寸对晕（halo）速度相关函数的影响，并通过引入汉克尔变换（Hankel transform）积分下限$k_{\rm min}$作为自由参数进行边际化处理。

Result: 速度相关函数在大尺度上因模拟盒尺寸限制导致的振幅抑制程度随晕（halo）质量增加而系统性增强。
...
### [22] [A model-independent measurement of the Hubble constant from gravitational-wave standard sirens and electromagnetic observations](https://arxiv.org/abs/2602.04497)
*Gaurav N. Gadbail,Kazuharu Bamba*

**TL;DR**: 通过结合引力波标准汽笛（standard siren）数据和电磁观测，采用高斯过程（Gaussian process）模型独立方法测得哈勃常数（Hubble constant）与SH0ES结果高度一致（差异小于2σ），量化了引力波数据对哈勃常数测定的贡献并探讨其在解决哈勃张力（Hubble tension）中的潜在作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用引力波(GW)和电磁观测数据，通过模型无关方法解决哈勃常数(Hubble constant)测量中的哈勃张力(Hubble tension)问题。

Data: 数据来源包括电磁波观测和引力波标准汽笛（standard siren）数据（来自GWTC3）。

Method: 采用高斯过程（Gaussian process）这一完全独立于模型的统计方法，结合电磁观测和引力波标准汽笛（standard siren）数据（GWTC3）进行哈勃常数估计。

Result: 通过结合引力波(GWTC3)和电磁波观测数据，采用高斯过程(Gaussian process)这一无模型方法测得的哈勃常数(Hubble constant)与SH0ES结果高度一致（差异小于2σ），表明引力波数据对解决哈勃张力(Hubble tension)具有潜在贡献。
...
### [23] [Weak Lensing Low Multipoles](https://arxiv.org/abs/2602.04504)
*Albert Bonnefous,Roya Mohayaee*

**TL;DR**: 通过分析FLRW宇宙中弱引力透镜（weak-lensing）低阶多极矩（low-multipole）收敛场（convergence field），发现观测到的低阶多极矩信号高于ΛCDM模型均值预测但与模拟中“银河系类似”（Milky Way-like）观测者结果一致，且局域结构（local structure）的弱透镜效应对宇宙偶极异常（cosmic dipole anomaly）的贡献不超过百分之几。


<details>
  <summary>Details</summary>

Background: 研究聚焦于弗里德曼-勒梅特-罗伯逊-沃尔克(FLRW)宇宙中弱引力透镜(weak-lensing)低阶多极矩(low-multipole)收敛场的特征及其与ΛCDM模型的对比。

Data: 数据来源包括全天空视界尺度N体模拟（Quijote）用于研究观测者环境依赖性，以及2MASS红移巡天（2MRS）用于重建本地宇宙的弱引力透镜收敛场。

Method: 采用解析计算、数值模拟（Quijote）和观测重建（2MRS）相结合的方法，重点分析弱引力透镜（weak-lensing）低阶多极矩（low-multipole）信号在FLRW宇宙中的演化规律及其与局域环境的关系。

Result: 观测到的低多极矩（low-multipole）弱透镜收敛场（weak-lensing convergence field）信号高于ΛCDM模型平均预测值，但与模拟中“银河系类似”（Milky Way-like）观测者环境完全吻合。
...
### [24] [Separation of polarized dust emission in Planck observations with Scattering Transforms](https://arxiv.org/abs/2602.04528)
*Alexandros Tsouros,Elisa Russier,Erwan Allys,Constant Auclair,François Boulanger,Jacques Delabrouille*

**TL;DR**: 该论文通过散射变换(scattering transforms)方法利用偏振与总强度的依赖关系及尘埃发射的非高斯丝状结构，从普朗克(Planck)卫星数据中重建了更精确的高银纬区域偏振尘埃图，显著提升了小尺度偏振尘埃发射的恢复能力，其重建功率谱与交叉谱与现有偏振尘埃图高度吻合。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过改进普朗克卫星数据中的偏振尘埃辐射（polarized dust emission）映射，以更精确地消除其对宇宙微波背景偏振（CMB）的干扰，从而探测原初B模偏振信号。

Data: 数据来源于普朗克（Planck）卫星任务观测的高银纬区域噪声主导的尘埃偏振（polarized dust emission）数据，通过局部笛卡尔天区（Cartesian patches of sky）处理斯托克斯（Stokes）线性偏振参数，并利用散射变换（scattering transforms）分析非高斯丝状结构（non-Gaussian filamentary structure）。

Method: 采用散射变换（scattering transforms）技术对非高斯结构的尘埃发射进行数据驱动分析，无需显式尘埃先验。

Result: 利用散射变换(scattering transforms)和局部参考系建模方法，通过最小化复合目标函数重建的偏振尘埃辐射图能更准确地恢复小尺度偏振尘埃发射特征，其功率谱和交叉谱与普朗克数据吻合。
...
### [25] [The Standard siren tests of viable $f(R)$ cosmologies](https://arxiv.org/abs/2602.04559)
*Yi Zhang,Xuanjun Niu,Xianfu Su,Dong-Ze He*

**TL;DR**: 结合电磁(PP+CC、Planck和DESI2)与模拟标准汽笛(SS)数据，研究发现标准汽笛观测能有效区分胡-萨维基(Hu-Sawicki)和斯塔罗宾斯基(Starobinsky)两类f(R)引力模型与ΛCDM模型，但无法解决哈勃张力问题，且胡-萨维基模型中FRR0<0可能引发理论不稳定性，而斯塔罗宾斯基模型在加入SS约束后显示出对曲率依赖的敏感性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用电磁波数据集和模拟标准汽笛(siren)数据约束Hu-Sawicki和Starobinsky两类修正引力理论($f(R)$ gravity)，以增强其与标准宇宙学模型($Λ$CDM)的区分能力。

Data: 研究中使用的数据来源包括当前电磁数据集（PP+CC、Planck和DESI2）以及基于最佳拟合参数模拟的标准汽笛（standard siren）目录。

Method: 采用电磁数据集(PP+CC、Planck和DESI2)约束Hu-Sawicki和Starobinsky的f(R)引力模型参数，并基于最佳拟合参数模拟标准汽笛(standard siren)目录。

Result: 标准汽笛（standard siren）数据增强了对$f(R)$引力与$Λ$CDM模型的区分能力，但无法解决哈勃张力问题。
...
### [26] [Unclustered tracers remain unclustered: the lack of primordial non-Gaussianity response of bias-zero tracers](https://arxiv.org/abs/2602.04698)
*Celia Merino,Santiago Avila,A. G. Adame,A. Anguren,V. Gonzalez-Perez,J. Meneses-Rizo*

**TL;DR**: 通过数值模拟验证了基于局部密度（δ_{t,R}）选择的零偏置（b_1=0）星系样本的原初非高斯性（PNG）响应（b_φ）普遍趋近于零，且其聚类特性偏离普适性关系和泊松噪声假设，从而限制了这类样本对PNG的约束能力。


<details>
  <summary>Details</summary>

Background: 研究主题为利用新一代大规模星系巡天约束原初非高斯性（primordial non-Gaussianities, PNG），并验证通过局域密度分选获得无偏置（bias-zero）星系作为最优探针的假设。

Data: 数据来源于PNG-UNITsim套件中的先进模拟，包括局部$f_{\rm NL}=100$和$f_{\rm NL}=-20$的设定，并考虑了不同父示踪剂目录：所有晕、大质量区间内的晕、以及LRGs和QSO的HOD模型。

Method: 采用局部密度（δ_{t,R}）分类方法测量线性偏差（b_1）和原初非高斯性响应（b_φ），并验证其与普适性关系（universality relation）的偏离程度。

Result: 高偏置（bias-zero）示踪体的原始非高斯性（PNG）响应（b_φ）近似为零，且其聚类行为偏离普适性关系和泊松散粒噪声假设，限制了其对PNG的约束能力。
...
### [27] [Measurement of the Hubble constant using the Dark Energy Survey Year 6 Gold galaxy catalog and the fourth Gravitational-Wave Transient Catalog](https://arxiv.org/abs/2602.04766)
...
**TL;DR**: 通过分析引力波（GW）第四瞬变目录（GWTC-4.0）中142个致密双星并合事件并结合暗能量巡天（DES）第六年黄金星系目录，利用gwcosmo流程联合推断宇宙学和引力波群体参数，发现限制红移范围（0.05<z<0.35）可减少星系红移分布偏差，测得暗汽笛（dark siren）的哈勃常数（H₀）为70.9^{+22.3}_{-18.6} km s⁻¹ Mpc⁻¹，结合亮汽笛（bright siren）GW170817后提升至73.1^{+11.7}_{-8.6} km s⁻¹ Mpc⁻¹，为下一代星系巡天最大化引力波宇宙学潜力提供了关键方法。


<details>
  <summary>Details</summary>

Background: 引力波标准汽笛（gravitational wave standard sirens）为哈勃常数（H_0）的独立测量提供了新方法，尤其关注无电磁对应体时通过暗汽笛（dark siren）统计关联引力波事件与宿主星系的技术路径。

Data: 数据来源为第四版引力波瞬变目录（GWTC-4.0）中的142个致密双星并合事件和暗能量巡天第六年黄金测光星系目录（Dark Energy Survey Year 6 Gold photometric galaxy catalog）。

Method: 采用gwcosmo流程联合推断宇宙学参数和引力波(GW)群体参数，并通过限制星系红移范围(0.05<z<0.35)确保与共动体积均匀分布的一致性。

Result: 通过暗汽笛（dark siren）方法结合引力波事件和星系红移分布，测得哈勃常数（Hubble constant）$H_0 = 70.9^{+22.3}_{-18.6}\;\text{km}\;\text{s}^{-1}\;\text{Mpc}^{-1}$，与亮汽笛（bright siren）GW170817联合后结果为$H_0=73.1^{+11.7}_{-8.6}\;\text{km}\;\text{s}^{-1}\;\text{Mpc}^{-1}$。
...
### [28] [Thermal conductivity of various CFRPs from 100 mK to 20 K](https://arxiv.org/abs/2602.04020)
*Valentin Sauvage*

**TL;DR**: 研究发现碳纤维增强聚合物（CFRPs）中T300、T700、HS40、M55J和IMA五种材料在100 mK至20 K极低温环境下不同纤维排布和密度的热导率特性，并开发新分析方法量化其低温导热行为。


<details>
  <summary>Details</summary>

Background: 碳纤维增强聚合物(CFRP)在航天器和航空工具的极端低温(cryogenic)环境中具有重要应用价值，因其具备高强度、轻量化和低热传导特性。

Data: 数据来源于对碳纤维样品（T300、T700、HS40、M55J和IMA）在不同纤维排布和密度下的热导率测量，测量温度范围为100毫开（mK）至20开（K）。

Method: 采用新开发的分析方法计算不同碳纤维样品在超低温下的热导率（thermal conductivity）。

Result: 在100 mK至20 K的极低温范围内测量了T300、T700、HS40、M55J和IMA五种碳纤维样品在不同纤维排布和密度下的热导率（thermal conductivity）表现。
...
### [29] [WFC3/UVIS Geometric Distortion -- Time Evolution of Linear Terms w.r.t Gaia](https://arxiv.org/abs/2602.04041)
*Anne O'Connor,Varun Bajaj,Jennifer Mack,Annalisa Calamida*

**TL;DR**: 通过分析2009至2022年WFC3/UVIS（宽视场相机3/紫外可见光通道）的7400多幅图像与盖亚（Gaia）DR3星表的对齐数据，发现其几何畸变线性项（平移、旋转、缩放和倾斜）中缩放和倾斜项存在微小时间演化（13年小于0.2像素），且受滤光片依赖性影响（F275W与F814W图像间最大差异达0.3像素），同时2017年后旋转偏移增至0.01度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于哈勃太空望远镜（HST）广角相机3（WFC3/UVIS）在2009至2022年间几何畸变线性项（位移、旋转、尺度和倾斜）的时间演化特征及其对高精度天体测量的影响。

Data: 数据来源于超过7400个WFC3/UVIS（Wide Field Camera 3/UVIS）曝光图像与盖亚（Gaia）DR3星表的对齐结果，时间跨度为2009年至2022年。

Method: 采用几何畸变解（geometric distortion solution）的线性项（位移、旋转、缩放和倾斜）时间演化分析方法，结合Gaia DR3星表对齐7400多幅WFC3/UVIS曝光图像。

Result: result：研究发现WFC3/UVIS几何畸变解的线性项（位移、旋转、缩放和倾斜）在2009至2022年间存在微小时间演化，其中缩放和倾斜项变化小于0.2像素（13年），但受固有散射（±0.3像素）主导，同时观测到滤镜依赖的缩放项偏移（F275W与F814W图像间最大差异0.3像素），且2017年后旋转偏移增至0.01度（散射较大）。
...
### [30] [GASV: A New VLBI analysis software for Geodesy and Astrometry](https://arxiv.org/abs/2602.04183)
*Dang Yao,Yuan-wei Wu,Xu-hai Yang*

**TL;DR**: GASV是一款新型Python软件包，专为处理甚长基线干涉测量（VLBI）数据设计，能高精度估算测地学和天体测量参数（如台站坐标、地球定向参数），其性能与国际VLBI分析中心相当，并支持单会话处理和长期SINEX文件的全球分析。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发用于甚长基线干涉测量(VLBI)数据分析的新型Python软件包GASV，旨在提升大地测量学和天体测量学参数估计的精度与效率。

Data: 研究中使用的数据来源为甚长基线干涉测量（VLBI）标准格式数据，包括HOPS输出文件和NGS卡文件。

Method: 采用基于Python的软件包GASV处理甚长基线干涉测量(VLBI)数据，支持标准格式（如HOPS输出和NGS卡文件）的基线延迟和速率分析，估算大地测量和天体测量参数（如测站坐标、地球定向参数、源坐标、钟参数和大气模型）。

Result: GASV是一款新型基于Python的软件包，专门用于处理甚长基线干涉测量(VLBI)数据，支持管道和交互式处理模式，能估算关键大地测量和天体测量参数。
...
### [31] [A Quantum Computing Framework for VLBI Data Correlation](https://arxiv.org/abs/2602.04269)
*Lei Liu*

**TL;DR**: 提出了一种基于量子计算（quantum computing）的甚长基线干涉测量（VLBI）数据处理框架，通过振幅编码（amplitude encoding）和量子算法显著降低了VLBI相关处理（包括条纹旋转、傅里叶变换、延迟补偿和互相关）的计算复杂度，验证了量子计算在VLBI系统中的可行性。


<details>
  <summary>Details</summary>

Background: 研究探讨了量子计算在甚长基线干涉测量（VLBI）数据相关处理中的应用潜力。

Data: 数据来源于经典基带时间序列数据（classical baseband time series data），通过振幅编码（amplitude encoding）嵌入量子叠加态。

Method: 采用量子计算框架实现VLBI（Very Long Baseline Interferometry）数据相关处理，核心方法包括基于振幅编码（amplitude encoding）的量子态制备、量子算法执行条纹旋转（fringe rotation）、傅里叶变换（Fourier transform）、延迟补偿（delay compensation）和互相关（cross correlation）操作，并通过构建完整量子处理流程验证其可行性。

Result: 提出了一种用于甚长基线干涉测量(VLBI)数据相关的量子计算框架，通过振幅编码(amplitude encoding)将经典基带时间序列数据嵌入量子叠加态，显著降低了VLBI相关和条纹拟合(fringe fitting)操作的计算复杂度。
...
### [32] [Expanding the Horizon of Black Hole Imaging with AtLAST](https://arxiv.org/abs/2602.04666)
...
**TL;DR**: 未来EHT（Event Horizon Telescope）升级结合AtLAST（Atacama Large Aperture Submillimeter Telescope）将显著提升灵敏度和多频能力，实现对更广泛质量、环境和吸积状态下黑洞增长、吸积物理及喷流发射的群体研究。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过升级事件视界望远镜(EHT)阵列（特别是结合阿塔卡马大口径亚毫米波望远镜(AtLAST)）来拓展超大质量黑洞(SMBH)的观测范围，从而深化对黑洞生长、吸积物理和喷流产生机制的统计研究。

Data: 数据来源包括事件视界望远镜(EHT)的观测数据以及阿塔卡马大孔径亚毫米波望远镜(AtLAST)的未来升级观测数据。

Method: method：通过大幅提升灵敏度与多频段观测能力，结合事件视界望远镜（EHT）与阿塔卡马大孔径亚毫米望远镜（AtLAST）的协同观测技术。

Result: 未来EHT升级（以AtLAST为核心）将显著提升灵敏度和多频段能力，实现对不同质量、环境和吸积状态下黑洞增长、吸积物理及喷流发射的群体性研究。
...
### [33] [The EXoplanet Climate Infrared TElescope (EXCITE): A balloon-borne mission to measure spectroscopic phase curves of transiting hot Jupiters](https://arxiv.org/abs/2602.04840)
...
**TL;DR**: EXCITE（EXoplanet Climate Infrared TElescope）通过0.5米主镜和0.8–3.5 um中分辨率光谱仪测量热木星（hot Jupiter）的光谱相位曲线，揭示其经度依赖的大气成分和能量循环模式。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用平流层气球搭载的红外望远镜（EXCITE）测量热木星型系外行星（hot Jupiter）的光谱相位曲线，以揭示其大气成分经度分布和能量循环模式。

Data: 数据来源于搭载0.5米主镜的中等分辨率衍射极限光谱仪（spectrograph）在0.8-3.5微米波段对热木星型系外行星（hot Jupiter-type exoplanets）的相位曲线（phase curve）观测，以及2024年从新墨西哥州Fort Sumner进行的工程试飞（engineering flight）性能数据。

Method: 采用0.5米主镜和中等分辨率衍射极限光谱仪（spectrograph）在0.8-3.5微米波段进行光谱相位曲线测量。

Result: EXCITE（EXoplanet Climate Infrared TElescope）通过气球搭载的0.5米主镜和中分辨率衍射极限光谱仪（0.8-3.5 um）测量热木星型系外行星的光谱相位曲线，以表征其经度依赖的大气成分和能量循环模式。
...
### [34] [Impact of Higher-Order Modes on Eccentricity Measurement in Binary Black Hole Gravitational Waves](https://arxiv.org/abs/2602.04642)
*Honglue Tang,Jinzhao Yang,Baoxiang Wang,Tao Yang*

**TL;DR**: 研究发现忽略引力波高阶模式（HOMs）仅对总质量超过120倍太阳质量、质量比大于4、倾角超过30度且信噪比高的双黑洞（BBH）并合事件产生显著偏心测量偏差（Δe/σ>1），而对现有候选事件（如GW190521）无统计显著影响，但未来对高质量、不对称或侧向系统的偏心测量需包含高阶模式以避免系统性误差。


<details>
  <summary>Details</summary>

Background: 研究探讨了引力波波形建模中忽略高阶模式（higher-order modes, HOMs）对双黑洞（binary black hole, BBH）并合轨道偏心度测量产生的系统性偏差。

Data: 数据来源包括六例疑似偏心双黑洞（BBH）并合引力波事件（GW190521、GW190620、GW190701、GW191109、GW200129和GW200208\_222617）的观测数据，以及为零噪声注入实验（zero-noise injection campaign）生成的模拟数据。

Method: 采用贝叶斯推断（Bayesian inference）结合先进的自旋对齐、含高阶模式（HOMs）的偏心有效单体模型SEOBNRv5EHM进行分析。

Result: 忽略高阶模式（HOMs）会导致高总质量（$M^{\rm det}\gtrsim120M_\odot$）、高不对称质量比（$q \gtrsim 4$）、近侧向（$θ_\textrm{JN} \gtrsim 30^\circ$）及高信噪比（$ρ^N_\textrm{mf}\approx50$）的双黑洞（BBH）系统出现显著偏心度测量偏差（$Δ_e/σ> 1$），而对准圆轨道且$M^{\rm det}\gtrsim140M_\odot$的系统可能产生非零偏心率的假阳性证据。
...
### [35] [Electromagnetic Couplings of Dark Domain Walls](https://arxiv.org/abs/2602.03933)
*Nemanja Kaloper*

**TL;DR**: 研究通过引入Chern-Simons项与暗区拓扑形式（dark sector top form）的耦合，发现穿越畴壁（domain walls）的电磁场会产生光学活性（optical activity），并在满足观测限制的参数空间内预测宇宙微波背景偏振旋转角Δϑ∼10⁻³弧度。


<details>
  <summary>Details</summary>

Background: 研究探讨了将麦克斯韦电动力学与暗区拓扑形式（dark sector top form）通过Chern-Simons耦合扩展的理论框架，重点关注畴壁（domain walls）对电磁场光学活性的影响及其在宇宙微波背景（cosmic microwave background）偏振旋转中的潜在效应。

Data: 数据来源于对宇宙微波背景（cosmic microwave background）各向异性的观测以及对畴壁（domain walls）成核率的观测约束。

Method: 采用扩展的麦克斯韦电动力学（Maxwell electrodynamics）结合Chern-Simons项与暗区拓扑形式（dark sector top form）的耦合模型。

Result: 在满足畴壁成核率和宇宙微波背景各向异性观测限制的参数空间内，离散渐逝的暗能量（dark energy）畴壁可导致宇宙微波背景产生约10^-3弧度的偏振旋转（polarization rotation）。
...
### [36] [Propulsion Trades for a 2035-2040 Solar Gravitational Lens Mission](https://arxiv.org/abs/2602.04198)
*Slava G. Turyshev*

**TL;DR**: 该论文通过对比近近日点太阳帆、裂变动力核电力推进（NEP）和奥伯特效应混合推进方案，量化分析了利用太阳引力透镜（SGL）在20年内抵达650-900天文单位（AU）目标区域的可行性，发现太阳帆需超低面密度（2.3-4.9 g/m²）与深近日点耐受性，而NEP需系统比功率低于3 kg/kWₑ才能实现20年内抵达，混合方案在初始速度超过50-70 km/s时更具现实性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用太阳引力透镜（Solar Gravitational Lens, SGL）实现近地系外行星的多像素成像与空间分辨光谱观测，并探讨了在日心距离650-900天文单位（AU）处实现这一目标所需的推进技术挑战。

Data: 研究中使用的数据来源于理论建模（simulation），包括太阳帆推进（solar sailing）、核电力推进（NEP）和混合架构（hybrid architectures）的性能参数计算。

Method: 采用简单时间-距离模型比较了近近日点太阳帆推进(solar sailing)、裂变动力核电力推进(NEP)和利用奥伯特效应(Oberth-enabled)的混合注入方案。

Result: 太阳引力透镜（SGL）可在日心距离650-900天文单位（AU）处实现邻近类地系外行星的多像素成像和空间分辨光谱观测。
...
### [37] [A Two-Dimensional Analytic Solution for the Generation of Hyperbolic Trajectories Via A Single Close Encounter with Applications To Interstellar Objects](https://arxiv.org/abs/2602.04806)
*Hayden Monk,Darryl Z. Seligman*

**TL;DR**: 通过限制性三体问题模型推导出星子被大质量摄动体散射至双曲轨道的解析判据，发现初始轨道离心率(eccentricity)超过0.4时喷射效率最高，并将该方法应用于太阳系、β绘架座(β Pictoris)和HR 8799系统以识别高效喷射轨道。


<details>
  <summary>Details</summary>

Background: 研究主题为探索行星系统中小天体通过动力学路径被抛射至双曲线轨道（hyperbolic trajectories）的机制及其影响因素。

Data: 研究中使用的数据来源于对太阳系、系外行星系统（exoplanetary systems）、β Pictoris和HR 8799的数值模拟（numerical simulations）结果。

Method: 采用限制性三体问题（restricted three-body problem）框架下的近似解析准则，通过映射散射前后轨道根数来预测小天体被大质量摄动体抛射到双曲轨道的条件。

Result: 发现星际闯入者（如1I/`Oumuamua、2I/Borisov和3I/ATLAS）后，需理解将小星体从行星系统抛射至双曲线轨道的动力学路径。
...
### [38] [The Landscape of Undergraduate Astronomy and Astrophysics Degree Requirements](https://arxiv.org/abs/2602.03959)
*Kate Follette,Carl Ferkinhoff,Michael Foley,Meridith MacGregor,Melissa Morris,Karen Masters,Tom Rice,Colin Wallace*

**TL;DR**: 2024-2025年美国天文学会教育委员会本科与研究生教育分会(SURGE)调查发现，当前天文学和天体物理学(Astronomy and Astrophysics)本科学位在知识体系和能力培养方面缺乏共识，削弱了学位证书对雇主和研究生院的可解释性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于美国天文学与天体物理学本科教育现状，调查了学位授予项目的课程设置与学习目标，揭示了当前缺乏统一标准的行业问题。

Data: 数据来源于78所院校完成的问卷调查，覆盖约1000名本科生/年，收集了学位名称、类型、必修与选修课程要求及学习目标等信息。

Method: 采用问卷调查（survey）方法收集和分析本科天文与天体物理学（Astronomy and Astrophysics）学位项目的数据。

Result: 目前天文学界对本科天文学或天体物理学学位应培养的知识与能力缺乏共识（community consensus）。
...
### [39] [Lessons Learned from Integrating Generative AI into an Introductory Undergraduate Astronomy Course at Harvard](https://arxiv.org/abs/2602.04389)
*Christopher W. Stubbs,Dongpeng Huang,Jungyoon Koh,Madeleine Woods,Andrés A. Plazas Malagón*

**TL;DR**: 该研究将生成式人工智能（GAI）整合到本科天文学入门课程中，发现学生在编程练习、作业准备和课堂讨论中使用GAI后课程评价未下降。


<details>
  <summary>Details</summary>

Background: 研究探讨了在本科天文学导论课程中全面整合生成式人工智能（GAI）的教学实践效果。

Data: 数据来源包括教学用Python笔记本、部分作业、学生演示准备材料以及结合RAG编码教材的课程Slack频道交互记录。

Method: 采用生成式人工智能（GAI）结合检索增强生成（RAG）编码教材，并分阶段应用于教学Python笔记本、作业辅助、学生展示准备及课程Slack频道互动。

Result: 生成式人工智能（GAI）被成功整合至本科天文入门课程，应用于教学Python笔记本、部分作业、学生展示准备及课程Slack频道互动（结合RAG编码教材），且未导致学生评价下降。
...
### [40] [Discovery of crested quasi-periodic eruptions following the most luminous SRG/eROSITA tidal disruption event](https://arxiv.org/abs/2602.03932)
...
**TL;DR**: 在银河核区潮汐瓦解事件（tidal disruption event）eRASSt J234402.9-352640中首次发现了伴随准周期喷发（QPEs）的高温短时耀斑，揭示了现有QPE模型无法解释的新现象。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于银河系核区潮汐瓦解事件(tidal disruption event) eRASSt J234402.9-352640 (J2344)中发现的复杂耀发活动及其与准周期喷发(quasi-periodic eruptions, QPEs)的关联现象。

Data: 数据来源于爱因斯坦探针（Einstein Probe）和XMM-牛顿（XMM-Newton）的观测数据。

Method: 通过XMM-牛顿望远镜光变曲线的时间分解(time decomposition)和时间分辨光谱分析(time-resolved spectral analysis)识别出周期性热耀斑。

Result: 在银河核J2344中首次观测到伴随准周期喷发(QPEs)的高温短时耀斑(5-30分钟)新现象，挑战了现有QPE模型。
...
### [41] [$γ$-Ray Lines -- Signatures of Nucleosynthesis, Cosmic Rays, Positron Annihilation, and Fundamental Physics](https://arxiv.org/abs/2602.04592)
...
**TL;DR**: 该论文概述了核γ射线（nuclear γ-ray）在MeV能段的天文观测价值，揭示了其作为核合成（nucleosynthesis）、宇宙射线（cosmic-ray）谱、星际介质（interstellar medium）相态、正电子湮灭（positron annihilation）及新物理（beyond standard model physics）探针的多重科学意义，并指出未来MeV望远镜对解决开放问题的必要性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于MeV能段核γ射线（nuclear γ-ray lines）在天体物理、粒子物理和基础物理中的关键作用，包括追踪核合成（nucleosynthesis）、宇宙射线（cosmic-ray）谱、星际介质（interstellar medium）相态以及探索超出标准模型（beyond standard model）的新物理现象。

Data: 数据来源于历史γ射线（gamma-ray）线测量结果和最新观测数据，结合新一代MeV望远镜的预期观测能力。

Method: 采用核γ射线谱线（nuclear γ-ray lines）的MeV能段观测技术分析天体核合成过程、宇宙射线谱及星际介质特性。

Result: MeV γ射线谱线（MeV γ-ray lines）为研究天体物理、粒子物理和基础物理提供了其他手段难以获取的关键信息。
...
### [42] [SN 2017ati: A luminous type IIb explosion from a massive progenitor](https://arxiv.org/abs/2602.04474)
...
**TL;DR**: 观测发现IIb型超新星(SN)2017ati的光变曲线晚期亮度超出典型范围1-2星等，结合磁星(magnetar)自转能量注入和放射性镍沉积(~0.21太阳质量)的混合模型能更好解释其演化，并推断其前身星初始质量≥17太阳质量。


<details>
  <summary>Details</summary>

Background: 研究聚焦于IIb型超新星（Type IIb supernova）SN 2017ati的光度演化异常及其可能的能源机制（如磁星magnetar贡献）。

Data: 数据来源于对IIb型超新星(SN)2017ati的光学测光和光谱观测。

Method: 采用标准放射性镍衰变模型（$^{56}$Ni decay scenario）和磁星（magnetar）自转减速能量输入相结合的方法拟合光变曲线。

Result: SN 2017ati的光度演化最佳解释为中子星自转减速能量(neutron star spin-down energy)和放射性镍沉积(radioactive nickel deposition)的共同作用。
...
### [43] [The incidence of LBV variability in the LMC](https://arxiv.org/abs/2602.04645)
*V. M. Kalari,J. S. Vink,C. Furey,R. Salinas,A. Udalski,M. Pawlak*

**TL;DR**: 通过分析大麦哲伦云（LMC）中87颗蓝超巨星（BSgs）的光变曲线，发现仅有[ST92] 4-13表现出S Dor型光变但不符合经典明亮蓝变星（LBV）标准，结合小麦哲伦云（SMC）数据证实S Dor相持续时间至多约10^3年，远短于文献假设，表明S Dor型变光可能由与爱丁顿极限相关的物理机制而非特定演化阶段引起。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探讨明亮蓝变星（LBVs）的独特变异性特征及其在大质量恒星演化中的普遍性，特别关注大麦哲伦云（LMC）中蓝超巨星（BSgs）的LBV类似变异性发生率。

Data: 数据来源于OGLE巡天项目对大型麦哲伦云（LMC）中254颗已知蓝超巨星（BSgs）中的87颗进行了长达二十年的光变曲线观测，并对其中37颗对象进行了为期三年的非周期性变化监测。

Method: 采用光学引力透镜实验（OGLE）巡天数据，对87颗大麦哲伦云（LMC）蓝超巨星（BSgs）进行长达二十年的光变曲线分析，并结合37个目标的三年度时标非周期性变化检测，筛选类似剑鱼座S型变星（S Dor variables）的光谱与测光特征。

Result: 在LMC（大麦哲伦星云）的87颗蓝超巨星（BSgs）中仅发现[ST92] 4-13表现出S Dor型光变特征，但其光谱特征、低光度和质量表明它并非真正的LBV（明亮蓝变星）。
...
### [44] [Wide binaries without viable bound Newtonian orbits](https://arxiv.org/abs/2602.04661)
*L. Pasquini,R. Saglia,F. Patat,L. Berni,D. Bossini,L. Magrini,H. Ludwig,M. T. Murphy,J. R. de Medeiros,J. Chanamé*

**TL;DR**: 通过分析26个投影间距超过13,000 AU的宽双星系统（wide binaries），发现其中3个系统的速度差异无法用牛顿引力束缚轨道解释，表明它们可能已被恒星相遇或银河潮汐扰动而不再引力束缚。


<details>
  <summary>Details</summary>

Background: 研究主题为利用宽双星系统在低加速度（low acceleration）条件下检验牛顿引力理论是否存在偏差。

Data: 数据来源于高分辨率VLT-ESPRESSO存档光谱（radial velocity）结合盖亚（Gaia）自行运动、视差和位置数据。

Method: 采用高分辨率VLT-ESPRESSO光谱仪测量径向速度（radial velocity），结合盖亚（Gaia）自行运动、视差和位置数据，通过剔除未解析三重系统、偶然对准或年轻系统，对剩余双星进行牛顿引力束缚轨道拟合分析。

Result: 在低加速度（low acceleration）区域，三个宽双星系统的速度差异过大，无法与任何束缚牛顿轨道相符，可能已被恒星相遇或银河潮汐扰动或解离。
...
### [45] [EPISODE III: The Nested Jet/Outflow Morphology of EC 53 Revealed by JWST and ALMA](https://arxiv.org/abs/2602.04688)
...
**TL;DR**: 结合詹姆斯·韦伯太空望远镜(JWST)和阿塔卡马大型毫米波/亚毫米波阵列(ALMA)观测，首次揭示了原恒星EC 53的喷流(jet)和外流(outflow)中存在热原子与分子成分的空间和运动学分层结构，支持磁流体动力学(MHD)盘风模型预测。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST和ALMA联合观测探究原恒星EC 53的喷流（jet）和外流（outflow）结构与运动学特征，以揭示恒星形成过程中物质抛射的物理机制。

Data: 研究使用了詹姆斯·韦伯空间望远镜(JWST)的近红外相机(NIRCam)和近红外光谱仪(NIRSpec)观测数据，以及阿塔卡马大型毫米波/亚毫米波阵列(ALMA)的连续谱和分子谱线(CO、C₂H)观测数据。

Method: 采用詹姆斯·韦伯太空望远镜(JWST)和阿尔玛望远镜(ALMA)的多波段观测数据，结合磁流体动力学(MHD)盘风模型(disk wind model)分析喷流和外流结构的热原子、分子组分与冷分子气体的空间及运动学分层特征。

Result: result：在EC 53（原恒星）中探测到高速电离喷流（position angle 142°、opening angle 1.4°、最大几何喷发半径约40 au），27条H₂发射线勾勒出窄锥形结构，中红外CO振转谱线（P支更强）与H₂同分布，ALMA观测的CO和C₂H示踪低速冷成分，空间与运动学分层符合MHD盘风模型预测。
...
### [46] [Ultralight time-oscillating scalars from magnetized compact stars: electrophilic radiation and photon propagation effects](https://arxiv.org/abs/2602.03953)
*Tanmay kumar Poddar,Gaetano Lambiase*

**TL;DR**: 研究发现超轻标量场（ultralight scalars）通过与致密星（compact stars）磁层中电荷密度和时变电磁场的耦合影响脉冲星自转减速（pulsar spin-down）、双星轨道动力学及光子传播特性，并基于蟹状星云脉冲星（Crab pulsar）等观测数据首次给出了标量-光子耦合（scalar-photon coupling）迄今最强的天体物理约束。


<details>
  <summary>Details</summary>

Background: 研究探讨了超轻标量场（ultralight scalars）与磁化致密星（magnetized compact stars）的电荷密度及电磁场相互作用对脉冲星自转减速、双星轨道动力学以及光子传播效应的潜在影响。

Data: 数据来源于对蟹状星云脉冲星（Crab pulsar）、SGR 1806-20和GRB 080805A的光谱与计时观测。

Method: 采用谱分析和时域观测技术约束标量-电子（scalar-electron）与标量-光子（scalar-photon）耦合参数。

Result: 超轻标量粒子（ultralight scalars）与磁化致密星（magnetized compact stars）的时变戈德赖希-朱利安电荷密度（Goldreich-Julian charge density）的电动耦合可导致其从磁层辐射，并影响脉冲星自转减速。
...
### [47] [Near-Resonant Thermal Leptogenesis](https://arxiv.org/abs/2602.04772)
*Angus Spalding*

**TL;DR**: 研究了准简并但非共振区间的轻子生成（leptogenesis），发现右手中微子质量低至电弱尺度（约100 GeV）时仍可实现成功的轻子生成，且再加热温度可低至10 GeV，无需依赖共振或非热产生机制。


<details>
  <summary>Details</summary>

Background: 研究探讨了在准简并但非共振（quasi-degenerate but non-resonant）条件下轻子生成（leptogenesis）的机制及其参数空间扩展至电弱能标（electroweak scale）的可能性。

Data: data来源包括数值模拟（simulation）和理论推导，未提及具体观测数据。

Method: 采用准简并但非共振（quasi-degenerate but non-resonant）条件下的CP不对称参数展开分析，并施加质量分裂远大于右手中微子衰变率（ΔM > 100Γ_i）的保守非共振条件。

Result: 在近共振但非共振的准简并区域中，轻子生成（leptogenesis）可实现右手中微子质量低至电弱尺度（M ≳ 100 GeV）且不依赖共振调节器或非热产生机制。
...
### [48] [Primordial black holes as cosmic accelerators of light dark matter: Novel direct detection constraints](https://arxiv.org/abs/2602.04858)
*Sk Jeesun,Anirban Majumdar,Rahul Srivastava*

**TL;DR**: 该论文提出通过原初黑洞（PBH）蒸发产生的轻暗物质（DM）粒子可在地球实验中被探测，研究了其与电子的反冲特征及不同洛伦兹结构相互作用的能谱差异，并利用XENONnT等实验数据对轻暗物质参数进行了约束。


<details>
  <summary>Details</summary>

Background: 当前多吨级暗物质(DM)探测器因反冲测量能量阈值限制难以探测来自银河系晕的轻暗物质。

Data: 研究中使用的数据来源于XENONnT、LZ和PandaX-4T的电子反冲数据以及未来可能利用的Super Kamiokande和Hyper Kamiokande等中微子探测器数据。

Method: 采用PBH（primordial black holes）蒸发产生的费米子暗物质（fermionic DM）电子反冲信号分析方法，结合能量无关（常数）和能量相关（标量和矢量介导）的DM-电子相互作用模型，并考虑地壳穿透过程中的动能损失衰减效应。

Result: 通过原初黑洞（PBH）蒸发产生的弱相互作用轻暗物质（或暗辐射）粒子能够以足够通量和动能抵达地球，为地面实验探测轻暗物质开辟了新途径。
