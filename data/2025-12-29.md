...
**TL;DR**: 提出了一种基于径向加速度与切向速度成正比的“速度耦合加速度”（velocity-coupled acceleration, VCA）模型，通过拟合171个SPARC样本星系旋转曲线发现其表现与NFW暗晕模型相当，部分优于Burkert模型，但参数间存在显著退化关系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过盘星系（disk galaxies）的观测旋转曲线（rotation curves）探索重子物质（baryons）与动力学（dynamics）之间的关系，并提出了一种基于径向加速度（radial acceleration）与切向速度（tangential speed）耦合的唯象模型（phenomenological model）作为标准暗晕参数化（halo parameterizations）的替代方案。

Data: 数据来源于SPARC样本中的171个星系旋转曲线（rotation curves），并使用了已发表的重子分解（baryonic decompositions）结果（气体、盘面、核球）。

Method: 采用速度耦合加速度（velocity-coupled acceleration, VCA）模型，通过假设径向加速度与局部切向速度成正比（$a_{\vca}(r)=γ(r)\,v(r)$）且耦合系数$γ(r)=\vinf/(r+\rzero)$饱和，结合圆周运动条件求解速度分布的二次方程。

Result: 提出的速度耦合加速度（VCA）模型通过引入与局部切向速度成正比的径向加速度，在拟合171个SPARC样本星系旋转曲线时表现出与NFW晕模型相当的性能，并在信息准则比较中与Burkert晕模型相当（但非全面更优）。
...
**TL;DR**: 研究发现旋转暗晕（spinning dark matter halo）能显著促进运动学热厚盘（kinematically hot and thick disk）形成棒状结构（bar formation），其角动量传输效率是非旋转暗晕的8倍。


<details>
  <summary>Details</summary>

Background: 高红移宇宙中运动学较热（kinematically hot）且较厚的星系盘存在大量棒状结构（bars），挑战了当前对类似银河系星系盘稳定性的理解。

Data: data：采用受控N体模拟(N-body simulation)方法，模拟了运动学上高温且厚的盘，该盘位于非旋转和自旋的暗物质晕(dark matter halo)中。

Method: 采用受控N体模拟(N-body simulations)方法研究运动学热且厚的盘在非旋转和旋转暗晕(spinning dark matter halo)环境下的棒不稳定性。

Result: 在旋转暗晕（spinning dark matter halo）环境中，经典稳定的运动学热厚盘（kinematically hot and thick disk）会形成棒状结构（bar formation），其角动量转移效率是非旋转暗晕的8倍。
...
**TL;DR**: 通过分析AKARI/IRC观测的243个近红外光谱，发现星系中脂肪族与芳香族烃类（PAHs）的发光强度比（L_ali/L_aro）随总红外光度（L_IR）和Brα线光度（L_Brα）增加而系统性降低，且蓝端连续谱颜色的星系具有更高比值，而部分红端ULIRGs（极亮红外星系）因核区遮蔽导致极低比值。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际碳氢尘埃（含芳香烃和脂肪烃）在银河系环境中受紫外辐射场和机械冲击等因素影响的演化过程。

Data: 使用AKARI/IRC观测的3.3微米和3.4-3.6微米波段近红外光谱数据，包括243个240个恒星形成(U)LIRGs（总红外光度$L_\rm{IR}>10^{11}\ L_\odot$）、119个105个恒星形成IRGs（$10^{10}\ L_\odot<L_\rm{IR}<10^{11}\ L_\odot$）和94个65个亚IRGs（$L_\rm{IR}<10^{10}\ L_\odot$）的光谱，以及作为参考样本的232个36个银河系HII区域的光谱。

Method: 采用近红外（IR）光谱模型拟合方法估算芳香烃和脂肪烃特征及氢重组线Brα的发光度。

Result: 在星系样本中，脂肪族与芳香族烃类发光特征的光度比（$L_\rm{ali}/L_\rm{aro}$）随总红外光度（$L_\rm{IR}$）和氢重组线Brα光度（$L_\rm{Brα}$）增加而系统性降低，且4微米连续谱颜色较蓝的（亚）红外星系倾向于具有更高的$L_\rm{ali}/L_\rm{aro}$。
...
**TL;DR**: 基于903颗经典造父变星（classical Cepheids）的精确运动数据构建了银河系6-18千秒差距范围内的旋转曲线（rotation curve），发现其在10-11千秒差距处存在凹陷（dip）并在13-14千秒差距处出现凸起（bump），结合历史数据测得太阳位置处圆周速度为236.8±0.8 km/s，并估算出18千秒差距内暗物质晕（dark matter halo）质量为1.19-1.45×10¹¹ M⊙。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用经典造父变星(Cepheids)精确测量银河系旋转曲线(rotation curve, RC)，以揭示银河系质量分布和暗物质晕特性。

Data: 数据来源为《盖亚》（Gaia）卫星第三次数据释放（DR3）中903颗精选经典造父变星（classical Cepheids）的精确自行运动（proper motions）和高质量径向速度（radial velocities），其距离通过已知的周期-威森赫特关系（Period-Wesenheit relations）准确测量。

Method: 基于903颗精选经典造父变星（classical Cepheids）的三维速度分量计算银河系旋转曲线（rotation curve），并结合前人测量构建平均旋转曲线以约束银河系质量模型。

Result: 银河系旋转曲线(RC)在6-18千秒差距范围内呈现随银心距离增加而轻微下降的趋势，并在R~10-11千秒差距处出现凹陷、R~13-14千秒差距处出现隆起特征。
...
**TL;DR**: 结合盖亚(Gaia)和LAMOST数据发现银河系最大超级气泡(Giant Oval Cavity)外围的O-B2型年轻恒星群具有25.8 km/s横向速度和6.2 km/s膨胀速度，证实其通过超新星反馈能量维持准稳态结构，与JWST观测的幻影气泡(Phantom Bubble)类似。


<details>
  <summary>Details</summary>

Background: 恒星反馈（stellar feedback）是星系演化中的关键机制，银河系中观测到的超级气泡（superbubble）现象为研究这一机制提供了重要案例。

Data: 数据来自盖亚(Gaia)和LAMOST的观测数据，获取了具有完整三维速度信息的年轻O-B2型恒星样本。

Method: 采用三维速度测量结合动力学时标计算（t_cross、t_expansion、t_survival、t_SN、t_shear）分析恒星反馈（stellar feedback）与银河系盘面动力学的平衡机制。

Result: 在银河系最大已知超级气泡(Giant Oval Cavity)中，重复超新星爆发提供的能量和动量足以维持气泡对抗环境压力的持续膨胀，其能量补充速率超过银河系剪切和湍流对空腔的破坏作用，表明该结构是一个由恒星反馈(Galactic disk dynamics)与星系盘动力学共同稳定的准静态大型超级气泡。
...
### [6] [Sensitivity of Weak Lensing Surveys to Gravitational Waves from Inspiraling Supermassive Black Hole Binaries](https://arxiv.org/abs/2512.21392)
*Tal Adi,Kris Pardo,Olivier Doré*

**TL;DR**: 该研究通过建立包含巡天特性的信噪比框架，发现理想化弱引力透镜巡天(weak lensing survey)可探测纳赫兹至微赫兹频段超大质量黑洞双星(SMBHBs)的连续引力波信号，填补脉冲星计时阵(PTA)与空间干涉仪(LISA)间的灵敏度间隙，其中角分辨率是主要限制因素。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用弱引力透镜（weak lensing）探测纳赫兹至微赫兹频段超大质量黑洞双星（SMBHBs）引力波（GWs）的观测潜力，填补脉冲星计时阵列（PTA）与空间干涉仪（LISA）之间的敏感度空白。

Data: 数据来源包括LSST类巡天和理想化巡天配置的模拟数据，以及基于红移和光度依赖分布的星系群模型生成的噪声功率谱密度。

Method: 开发了包含巡天特性（观测频率、角分辨率和深度）的信噪比框架，并采用红移和光度依赖的星系分布模型计算噪声功率谱密度。

Result: 弱引力透镜巡天在毫角秒级角分辨率和快速观测节奏下可探测宇宙学距离上的连续引力波信号，部分填补脉冲星计时阵列（PTA）与空间干涉仪（LISA）间的灵敏度空白。
...
### [7] [A Modal Approach to Constrain Inflation through Numerical Bispectra](https://arxiv.org/abs/2512.21432)
*Bowei Zhang,E. P. S. Shellard,James R. Fergusson*

**TL;DR**: 提出了一种结合\texttt{Primodal}代码和\texttt{Modal}分解的暴胀原初双谱（bispectrum）分析流程，通过数值计算暴胀拉格朗日参数空间下的树级双谱并与普朗克2018年数据对比，排除了不支持的暴胀场景，首次在IR DBI暴胀模型中得出声速（sound speed）$c_s \geq 0.073$和参数空间$β\leq 0.39$的观测约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过精确计算原初暴胀模型（inflationary models）的双谱（bispectrum）来约束暴胀参数空间，以提升对早期宇宙物理的理解。

Data: data部分：研究使用了WMAP和Planck卫星的CMB（宇宙微波背景辐射）观测数据，并通过\texttt{Modal}和\texttt{Primodal}代码处理这些数据以提取双谱（bispectrum）信息。

Method: 采用分离模态展开技术（separable mode-expansion technique）结合树阶内禀形式（tree-level in-in formalism）的数值计算方法，通过\texttt{Primodal}代码高效计算暴胀模型的原始双谱（primordial bispectrum）。

Result: 提出的双谱分析流程通过数值计算暴胀拉格朗日参数空间中的树级双谱（和功率谱），并与普朗克2018数据的模态双谱分解对比，排除了不符合观测的暴胀场景，直接约束了存活可行场景中的参数空间、声速（sound speed）等量。
...
### [8] [Particle production and Higgs reheating](https://arxiv.org/abs/2512.21658)
*Aarav Shah,Kanabar Jay,Maxim Khlopov,Oem Trivedi,Maxim Krasnov*

**TL;DR**: 论文揭示了在无直接耦合的希格斯暴胀（Higgs inflation）模型中，曲率标量（curvature scalar）的快速振荡通过引力泵浦效应（gravitational pump）高效产生标量旁观粒子（scalar spectator particles），为最小耦合早期宇宙模型提供了纯引力驱动的再加热（reheating）机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙再加热（reheating）过程中引力效应如何在没有直接耦合的情况下通过几何再加热机制实现粒子产生和能量转移。

Data: 数据来源于基于非最小曲率耦合（non-minimal curvature coupling）$ξφ^2 R$的希格斯暴胀（Higgs inflationary）情景下的后暴胀动力学模拟（simulation），采用博戈留波夫（Bogoliubov）形式主义计算粒子产生。

Method: 采用几何再加热（geometric reheating）框架，通过博戈留波夫（Bogoliubov）形式计算曲率标量振荡驱动的引力泵效应产生的标量旁观粒子（scalar spectator particles）。

Result: 曲率驱动的粒子产生机制为无直接耦合的暴胀模型提供了一种高效再加热途径。
...
### [9] [Golden and Silver Dark Sirens for precise H0 measurement with HETDEX](https://arxiv.org/abs/2512.21729)
...
**TL;DR**: 通过结合HETDEX巡天的VIRUS光谱数据和LIGO-A#网络探测的暗警报（dark sirens，包括黄金暗警报和白银暗警报），可在红移z<0.2范围内实现哈勃常数（H0）的百分比级精度测量。


<details>
  <summary>Details</summary>

Background: 引力波(GW)标准汽笛(siren)为测量哈勃常数(H0)提供了独立路径，可解决当前哈勃张力问题。

Data: 数据来源于Hobby-Eberly Telescope Dark Energy Experiment (HETDEX)第五次内部数据发布，并利用Visible Integral-field Replicable Unit Spectrograph (VIRUS)进行暗警报（dark sirens）的光谱后续观测。

Method: 采用基于可见光积分场可复制单元光谱仪（VIRUS）的光谱跟踪观测技术，结合霍比-埃伯利望远镜暗能量实验（HETDEX）第五次内部数据发布，对暗警报（dark sirens）进行宿主星系红移测量。

Result: 紧凑双星并合产生的引力波（GWs）作为标准汽笛（standard sirens）可直接测量源的光度距离，为独立测定哈勃常数（H0）提供了新途径。
...
### [10] [Implications of Cosmic Birefringence for Multi-Field ALP Dark Matter](https://arxiv.org/abs/2512.21888)
*Jialiang Shao,Ippei Obata,Dongdong Zhang*

**TL;DR**: 研究发现双轴子样粒子（ALP）场的叠加效应可缓解单场模型因清洗效应（washout effect）对宇宙微波背景（CMB）偏振功率谱的抑制，从而与观测数据相符。


<details>
  <summary>Details</summary>

Background: 宇宙双折射（cosmic birefringence）作为超出标准宇宙学模型的新物理探针，其研究背景聚焦于轴子类粒子（ALPs）作为暗物质候选者对宇宙微波背景辐射偏振面旋转的潜在解释。

Data: 数据来源为宇宙微波背景辐射(CMB)的偏振观测数据。

Method: 采用双轴子样粒子（ALP）场叠加的理论框架，结合解析近似和数值计算，以缓解单场模型中由标量场振荡动力学引起的洗脱效应（washout effect）限制。

Result: 双ALP场（Axion-like particles）叠加模型能缓解传统单场模型因washout效应导致的限制，与观测结果更吻合。
...
### [11] [TITAN DR1: An Improved, Validated, and Systematically-Controlled Recalibration of ATLAS Photometry toward Type Ia Supernova Cosmology](https://arxiv.org/abs/2512.21903)
...
**TL;DR**: 该研究通过交叉校准ATLAS测光数据与DES Y6释放的三级星，将像素级和芯片级零点偏移分别降低至约4和3毫米星等均方根，并识别出色散函数颜色依赖性这一主要系统误差，最终将整体校准相关系统误差控制在5-10毫米星等水平，为TITAN超新星数据集提供可靠宇宙学约束奠定了基础。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于提升ATLAS巡天系统对Ia型超新星(SNe Ia)大规模低红移数据集的光度测量精度，以支持宇宙学约束。

Data: 数据来源于ATLAS巡天系统的四台望远镜观测的超过10,000颗光谱确认的Ia型超新星（SNe Ia）和数千条宇宙学级光变曲线（即将作为TITAN DR1发布），并利用DES（Dark Energy Survey）Y6版本的三级恒星进行交叉定标，同时参考PS1（Pan-STARRS DR1）覆盖区和非覆盖区的重叠区域验证ATLAS Refcat2星表的主要定标器。

Method: 采用DES Y6释放的三级星对ATLAS强制测光进行交叉校准，通过确定像素间和芯片间的零点偏移（Δ zeropoint offsets）以及修正透射函数颜色依赖性（transmission-function color dependence）来提升校准精度。

Result: 通过交叉校准ATLAS测光数据与DES Y6三级星，将CCD内部像素间偏移从约8毫米星等降低至约4毫米星等，芯片间偏移从约17毫米星等降低至约3毫米星等，并识别出滤光片透射函数色依赖性是最大的系统误差来源。
...
### [12] [Primordial black holes and smooth coarse-graining in excursion set theory](https://arxiv.org/abs/2512.22075)
*Daiki Saito,Koki Tokeshi*

**TL;DR**: 采用高斯窗函数（Gaussian window function）平滑粗粒化时，原初黑洞（PBHs）质量函数的峰值及邻近区域主要由各质量尺度上密度对比超过给定阈值的概率决定，阐明了噪声关联对预测丰度的重要性。


<details>
  <summary>Details</summary>

Background: 研究探讨了平滑粗粒化（smooth coarse-graining）过程对原初黑洞（PBHs）质量函数的影响及其在随机过程中的作用。

Data: 研究中采用高斯窗函数（Gaussian window function）进行平滑粗粒化处理，其随机噪声在尺度间完全相关。

Method: 采用高斯窗函数（Gaussian window function）进行平滑粗粒化处理，分析相关噪声对原初黑洞（PBHs）质量函数的影响。

Result: 平滑粗粒化（smooth coarse-graining）引入的噪声相关性主导了原初黑洞（PBHs）质量函数的峰值及其邻近区域分布。
...
### [13] [Searching for Cosmological Collider in the Planck CMB Data II: collider templates and Modal analysis](https://arxiv.org/abs/2512.22085)
*Petar Suman,Dong-Gang Wang,Wuhyun Sohn,James R. Fergusson,E. P. S. Shellard*

**TL;DR**: 利用改进的普朗克(Planck)宇宙微波背景(CMB)数据分析方法，通过正交化方案消除单场原初非高斯性(PNG)的简并性，首次在2.35σ置信度上探测到暴胀期间大质量自旋-0粒子交换的宇宙学对撞机信号。


<details>
  <summary>Details</summary>

Background: 研究主题为利用宇宙微波背景辐射（CMB）数据探测暴胀时期大质量粒子产生的原初非高斯性（PNG）信号，以检验宇宙对撞机（cosmological collider）理论模型并约束单场暴胀机制。

Data: 使用普朗克卫星(Planck)的宇宙微波背景(CMB)观测数据进行分析。

Method: 采用模态双谱估计器（Modal bispectrum estimator）和正交化方案（orthogonalization scheme）分析普朗克CMB数据，以系统扫描参数并搜索显著的原初非高斯性（PNG）信号。

Result: 在普朗克(Planck)宇宙微波背景(CMB)数据分析中，发现自旋为0的大质量粒子交换信号的信噪比(S/N)最高达2.35σ（已考虑多重检验效应）。
...
### [14] [ESO Expanding Horizons White Paper: Electromagnetic counterparts of massive BH mergers with LISA](https://arxiv.org/abs/2512.21359)
...
**TL;DR**: 论文核心贡献是提出了最大化探测LISA（激光干涉空间天线）观测的中等质量黑洞双星（MBHB）电磁对应体数量的最优策略。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用激光干涉空间天线（LISA）探索毫赫兹引力波（gravitational-wave）并研究大质量黑洞双星（MBHB）合并的电磁对应体（EM counterparts）探测策略。

Data: 数据来源为激光干涉空间天线（Laser Interferometer Space Antenna, LISA）的观测数据。

Method: 采用激光干涉空间天线(LISA)的引力波观测技术探测毫赫兹频段信号，并结合电磁波段互补观测策略。

Result: LISA（激光干涉空间天线）将把引力波观测扩展到尚未探索的毫赫兹频段，推动引力波天文学的新革命。
...
**TL;DR**: 研究发现70颗位于M型恒星(M-stars)宜居带的系外行星中，恒星色球活动(Chromospheric activity)产生的紫外线和X射线辐射可能影响复杂生命存在，需通过大样本耀发活动研究评估其宜居性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于M型恒星（M-stars）的色球层活动（chromospheric activity）对宜居带（habitable zone）内系外行星（exoplanets）复杂生命存在可能性的影响。

Data: 数据来源包括盖亚卫星(Gaia)和柏拉图卫星(PLATO)的观测数据，以及拟议的宽视场巡天望远镜(Wide Field Survey telescope)的预期观测数据。

Method: 采用宽视场巡天望远镜（Wide Field Survey telescope）对M型恒星（M-stars）的耀斑活动及其紫外与X射线辐射特性进行大尺度统计分析。

Result: 截至2025年底，约50颗位于宜居带（habitable zone）的系外行星围绕以强色球活动（chromospheric activity）著称的M型恒星运行，其行星-恒星质量比和光度比更利于后续详细研究。
...
### [16] [Detection of Lensed Gravitational Waves in the Millihertz Band Using Frequency-Domain Lensing Feature Extraction Network](https://arxiv.org/abs/2512.21370)
*Tianlong Wang,Tianyu Zhao,Minghui Du,Ziren Luo,Peng Dong,Peng Xu*

**TL;DR**: 提出双通道透镜特征提取扩展长短期记忆网络（DCL-xLSTM），通过矩阵值记忆结构和记忆混合机制高效识别毫赫兹频段引力波（GW）透镜事件中的振幅衍射特征，在点质量（PM）和奇异等温球体（SIS）透镜模型下实现AUC超过0.99、真阳性率（TPR）98%且假阳性率（FPR）低于1%的稳定性能。


<details>
  <summary>Details</summary>

Background: 空间引力波(GW)探测器有望观测到透镜化引力波事件，为宇宙学和基础物理学研究提供新机遇。

Data: 数据来源为基于点质量模型（Point Mass, PM）和奇异等温球体模型（Singular Isothermal Sphere, SIS）生成的模拟数据，考虑了从波动光学到几何光学的过渡效应。

Method: 采用双通道透镜特征提取扩展长短期记忆网络（DCL-xLSTM）结合矩阵值记忆结构和记忆混合机制，有效捕捉毫赫兹频段振幅衍射模式。

Result: 提出的DCL-xLSTM网络通过矩阵值记忆结构和记忆混合机制有效捕捉毫赫兹频段振幅衍射模式，在PM和SIS模型数据上实现AUC超过0.99、FPR低于1%时TPR高于98%的性能。
...
### [17] [Neural Network for Simulating Radio Emission from Extensive Air Showers](https://arxiv.org/abs/2512.21407)
*Pranav Sampathkumar,Tim Huege,Andreas Haungs,Ralph Engel*

**TL;DR**: 研究发现神经网络可有效模拟宇宙射线(cosmic ray)空气簇射的无线电脉冲，并在保持与蒙特卡洛(Monte-Carlo)模拟相当精度的同时实现Xmax重建。


<details>
  <summary>Details</summary>

Background: 近年来利用大型射电阵列探测宇宙线簇射（cosmic ray shower）成为重要研究方向，但现有微观模拟方法计算成本过高限制了数据分析效率。

Data: 使用大型射电阵列（radio arrays）的观测数据和蒙特卡洛模拟（Monte-Carlo simulations）生成的射电脉冲信号作为数据来源。

Method: 采用神经网络模拟宇宙射线（cosmic ray）产生的射电脉冲（radio pulses），并用于Xmax重建（Xmax reconstruction）的物理应用。

Result: 神经网络可用于模拟宇宙射线(cosmic ray)引发的无线电脉冲，并成功应用于Xmax重建，其分辨率与完整蒙特卡洛(Monte-Carlo)模拟相当。
...
### [18] [WST spectroscopic variability alerts: discovery space, data flow system requirements](https://arxiv.org/abs/2512.21949)
*Valenitn D. Ivanov*

**TL;DR**: 论文核心贡献是提出世界巡天望远镜(WST)通过光谱线轮廓和强度变化监测实现天体变源警报系统，突破了传统仅限成像巡天的局限，可探测发射线恒星(如Be星、沃尔夫-拉叶星、高光度蓝变星)到活动星系核(包括1型与2型间转换的变脸类星体)的物理参数变化。


<details>
  <summary>Details</summary>

Background: 研究探讨了WST（宽视场光谱望远镜）通过光谱线轮廓和强度变化检测可变天体（如发射线星、活动星系和类星体）的新能力，突破了传统成像巡天的局限。

Data: 数据来源包括WST自身观测的光谱数据、其他光谱巡天（spectroscopic surveys）的参考光谱，以及用户提交的自定义参考光谱。

Method: 采用光谱线轮廓和强度变化检测技术，结合自动搜索WST自身档案及其他光谱数据库或用户提交的参考光谱（reference spectra）进行比对分析，并通过建模观测与参考光谱来推导拟合参数（fitting parameters）以实现基于物理参数的变异性检测。

Result: WST（Wide-field Spectroscopic Telescope）的数据流系统（DFS）通过检测光谱线轮廓和线强度变化，为发射线恒星（如Be星、WR星和LBV星）、活动星系、类星体（包括在1型和2型之间转换的变脸天体）等天体开辟了全新的变异性研究空间。
...
### [19] [From emission to absorption: the FAST observation of the OH 18-cm lines from the Comet C2025/A6](https://arxiv.org/abs/2512.21969)
...
**TL;DR**: 首次利用FAST望远镜超宽带接收器观测彗星C/2025 A6并检测到OH 18厘米谱线，发现其从发射到吸收的反转现象主要由日心速度变化引起，并通过梯形拟合推导出水分子膨胀速度随日心距离减小而增大，同时OH产生率呈现显著上升趋势。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用FAST望远镜（Five-hundred-meter Aperture Spherical radio Telescope）超宽带接收器观测彗星C/2025 A6的OH 18厘米谱线，揭示其与日心距离变化相关的动力学特征。

Data: 使用FAST望远镜（Five-hundred-meter Aperture Spherical radio Telescope）的超宽带接收器在2025年10月23日至11月8日期间对彗星C/2025 A6进行了观测，首次探测到OH 18厘米谱线。

Method: 采用FAST望远镜的超宽带接收器观测OH 18厘米谱线，并通过梯形拟合（trapezoidal fitting）分析谱线轮廓以推导水分子膨胀速度。

Result: 首次在彗星C/2025 A6中探测到OH 18厘米谱线，并发现其从发射到吸收的反转现象主要由日心速度变化引起。
...
### [20] [Numerical simulation of lunar response to gravitational waves and its 3D topographic effect using the spectral-element method](https://arxiv.org/abs/2512.21667)
*Lei Zhang,Han Yan,Jinhai Zhang,Xian Chen*

**TL;DR**: 开发了高精度三维谱元法（SEM）模拟月球对20 mHz以下引力波（GWs）的响应，发现表面地形对特定频率成分的放大效应不可忽略。


<details>
  <summary>Details</summary>

Background: 研究主题为利用三维数值模拟方法评估月球作为引力波(gravitational waves, GWs)天然放大器的响应特性及其地形效应。

Data: 采用高阶三维有限元法（谱元法，SEM）对月球在20 mHz以下引力波（GWs）的响应进行数值模拟，并通过与半解析解对比验证了方法精度。

Method: 采用高阶三维有限元法（谱元法，SEM）数值模拟月球对20 mHz以下引力波的响应。

Result: 月球表面地形对引力波（GWs）的响应具有不可忽视的影响，会在特定频率成分上产生信号增强效应。
...
### [21] [$Sp(4,\mathbb{Z})$ modular inflation](https://arxiv.org/abs/2512.21597)
*Si-Yi Jiang,Wenbin Zhao,Gui-Jun Ding*

**TL;DR**: 论文核心贡献是扩展了西格尔模群(Sp(4,ℤ))框架下的宇宙学α吸引子模型，利用二阶绝对不变量在模空间特定子空间中构建了与普朗克2018观测相符的类高原势能，并提出了能容纳ACT和SPT数据偏好的更大谱指数(nₛ)的修正多项式α吸引子模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于基于西格尔模群（Siegel modular group）$Sp(4,\mathbb{Z})$的暴胀模型构建，扩展了$SL(2,\mathbb{Z})$框架并保留凯勒势（Kähler potential）的双曲几何特性。

Data: 使用西格尔模空间（Siegel modular space）中由西格尔模群（Siegel modular group）$Sp(4,\mathbb{Z})$定义的二维复子空间和一维复子空间进行模拟分析。

Method: 采用基于西格尔模群（Siegel modular group）$Sp(4,\mathbb{Z})$的几何框架，结合双曲卡勒势（Kähler potential）和模量（moduli）虚部构建宇宙学α吸引子模型，并在西格尔模空间特定子空间内利用$g=2$绝对不变量构造暴胀势。

Result: result：基于西格尔模群(Sp(4,ℤ))构建的膨胀模型在特定子空间中利用亏格g=2绝对不变量构造的势能函数，其模量τ的虚部自然产生与普朗克2018观测相符的高原型势能结构。
...
**TL;DR**: 研究发现下一代引力波探测器（CE和ET）能探测到恒星质量双黑洞（BBH）合并时受第三体（如超大质量黑洞）引起的质心（CoM）加速度效应，该效应会导致双黑洞参数推断出现显著系统偏差，尤其对不对称双星影响更大，且CE和ET分别能约束低至10⁻⁹ s⁻¹和10⁻¹¹ s⁻¹的CoM加速度参数α。


<details>
  <summary>Details</summary>

Background: 研究聚焦于双黑洞（BBH）并合过程中受第三体引力波（GWs）相位调制效应的探测与参数推断偏差问题，特别关注下一代引力波探测器（3G）对质量中心（CoM）加速效应的敏感性提升。

Data: 研究中使用的数据来源于引力波（GW）波形模拟，重点关注恒星质量双黑洞（BBH）在第三代（3G）探测器（如Cosmic Explorer和Einstein Telescope）中的信号特征。

Method: 采用波形相位分析（waveform phase analysis）方法探究了恒星质量双黑洞（BBH）在第三体引力作用下产生的质心加速度（CoM acceleration）对引力波信号的影响。

Result: result：下一代引力波探测器（如CE和ET）能探测到总质量为5M⊙的低质量双黑洞（BBH）因第三体存在导致的质心（CoM）加速度效应，当α低至∼10−9 s−1（CE）和10−10 s−1（ET）时，未考虑的CoM加速度会导致啁啾质量和对称质量比估计的系统误差超过统计误差。
...
### [23] [GraviBERT: Transformer-based inference for gravitational-wave time series](https://arxiv.org/abs/2512.21390)
*Martin Benedikt,Ippocratis D. Saltas*

**TL;DR**: 提出GraviBERT深度学习框架，通过多尺度卷积特征提取器结合Transformer编码器和回归头，采用BERT式预训练和监督微调两阶段训练，显著提升引力波时间序列参数估计精度（平均绝对误差降低30%）和训练效率（收敛速度加快6倍），并在低信噪比下实现质量、距离参数误差达百分位级、有效自旋误差约10^-3，同时展示出色跨噪声分布和波形近似方法的迁移能力（误差降低44%，训练加速15倍）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发一种新型深度学习框架（GraviBERT），用于引力波（gravitational-wave）时间序列推断，旨在提升参数估计精度和训练效率。

Data: 数据来源于引力波时间序列的噪声波形（noisy waveforms）。

Method: 采用基于Inception启发的多尺度卷积特征提取器结合transformer编码器和回归头的深度学习框架GraviBERT，通过BERT式预训练（预测特征空间掩蔽段以学习通用模式和物理规律）和监督微调两阶段训练进行引力波时间序列推断。

Result: GraviBERT通过两阶段训练（BERT式预训练和监督微调）显著提升了引力波时间序列推断性能，在域内数据上将点估计参数推断的平均绝对误差降低达30%，训练收敛速度加快达6倍。
...
### [24] [Mass-to-Horizon Entropic Cosmology: A Unified Thermodynamic Pathway to Cosmic Acceleration](https://arxiv.org/abs/2512.22103)
*Tomasz Denkiewicz,Hussain Gohar*

**TL;DR**: 该研究通过结合大尺度结构增长数据和几何探针，发现基于广义质量-视界熵关系（M ∝ L^n）的熵宇宙学模型在统计上优于标准ΛCDM模型，支持了宇宙晚期加速膨胀的熵起源解释。


<details>
  <summary>Details</summary>

Background: 研究探讨了广义质量-视界熵宇宙学（generalized mass-to-horizon entropic cosmology）的理论框架及其观测验证，聚焦于熵力驱动宇宙加速膨胀的涌现现象。

Data: 数据来源包括Pantheon+ Ia型超新星数据（SH0ES校准）、DESI DR2重子声波振荡测量、宇宙微波背景（CMB）距离先验以及一系列宇宙结构增长观测数据。

Method: 采用贝叶斯模型比较方法分析广义质量-视界熵宇宙学模型与标准ΛCDM模型的统计显著性。

Result: 广义质量-视界熵宇宙学模型在联合几何与结构增长观测数据下比标准ΛCDM模型更优，支持晚期宇宙加速膨胀的熵力驱动机制。
...
### [25] [Methods for Detecting Gravitational Waves from mini-Extreme-Mass-Ratio Inspirals I: Statistics Based on Time-Frequency Signal Tracks](https://arxiv.org/abs/2512.21738)
*Zi-Xuan Wang,Gong Cheng,Ju Chen,Huai-Ke Guo,Andrew L. Miller*

**TL;DR**: 该论文提出了一种名为$Σ$Track的新方法，用于探测由恒星质量致密天体与更轻伴星组成的迷你极端质量比旋进系统（mini-EMRIs）产生的连续引力波信号，并建立了处理频谱泄漏的统计框架，为探测亚太阳质量致密天体提供了理论基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用地面引力波探测器（如LIGO-Virgo-KAGRA）探测由恒星质量致密天体与更轻伴星组成的微型极端质量比旋近系统（mini-EMRIs），以探索原始黑洞等奇异致密天体的存在。

Data: 使用LIGO-Virgo-KAGRA等地面干涉仪采集的连续引力波频段观测数据，并基于模拟的mini-EMRI（极端质量比旋进）信号进行方法验证。

Method: 采用$Σ$Track方法对时频平面上的信号轨迹进行求和，并建立统计框架处理频谱泄漏问题。

Result: 开发了名为$Σ$Track的新方法，通过沿时频平面轨迹求和处理频谱泄漏，为分析长时程瞬变信号（如mini-EMRIs）建立了通用统计框架。
...
### [26] [Methods for Detecting Gravitational Waves from mini-Extreme-Mass-Ratio Inspirals II: A Spectral-Leakage-Aware Framework](https://arxiv.org/abs/2512.21739)
*Zi-Xuan Wang,Xing-Yu Chen,Ju Chen,Gong Cheng,Huai-Ke Guo,Andrew L. Miller*

**TL;DR**: 提出了一种改进的$Σ$Track方法，通过建立频谱泄漏分析模型、引入$ΣR$统计量和动态优化相干时间的分层搜索策略，显著提升了地面引力波探测器对由亚太阳级奇异致密天体（如原初黑洞或玻色子星）与更大质量天体组成的mini-EMRIs（极低质量比旋进系统）的探测灵敏度，有效探测体积提高了一个数量级。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用地面引力波探测器（如LIGO、Virgo和KAGRA）探测微型极端质量比旋近系统（mini-EMRIs）以探索早期宇宙和暗物质本质的科学目标。

Data: 使用LIGO、Virgo和KAGRA地面引力波探测器采集的观测数据，并基于基准质量参数为$(1.5, 10^{-5})\,M_\odot$的微型极端质量比旋进（mini-EMRI）系统模拟信号进行方法验证。

Method: 采用$Σ$Track方法扩展准单色近似限制，通过建立频谱泄漏解析模型和提出加权功率比统计量$ΣR$，并引入动态优化相干时间的频率分层搜索策略。

Result: 提出了ΣR统计量（一种基于功率比加权求和的新型检测指标）和频率分层搜索策略，显著提高了地面引力波探测器对mini-EMRIs（极低质量比旋进系统）的探测灵敏度。
...
### [27] [Early Planet Formation in Embedded Disks (eDisk). XIX. Structures of molecular outflows](https://arxiv.org/abs/2512.21454)
...
**TL;DR**: 通过ALMA观测19颗近邻低质量原恒星(protostars)的12CO(2-1)分子外流(outflow)，发现15个源存在外流并将其分为三类（11个慢盘风(slow disk wind)、7个风驱动壳层(wind-driven shell)和1个弓形激波(bow shock)），揭示了外流与尘埃连续辐射小轴的不对齐现象及外流方向随时间变化的证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用ALMA（阿塔卡马大型毫米波/亚毫米波阵列）观测19颗邻近低质量原恒星（low-mass protostars）的12CO（2-1）分子外流（molecular outflow），旨在揭示早期行星形成阶段中外流形态与动力学特征的多样性及其与周围介质的相互作用。

Data: 数据来源于阿塔卡马大型毫米波/亚毫米波阵列(ALMA)大型项目"早期行星形成于嵌入盘"(eDisk)对19颗近邻低质量原恒星进行的12CO(2-1)谱线观测。

Method: 基于形态和运动学结构将CO外流(CO outflow)分为风驱动壳层(wind-driven shell)、弓形激波(bow shock)和慢盘风(slow disk wind)三类，并对风驱动壳层采用参数化模型推导其动力学年龄(dynamical age)等物理特性。

Result: 在19个邻近低质量原恒星中观测到15个存在分子外流（molecular outflow），并根据形态和运动学特征将其分为三类：11个慢盘风（slow disk wind）、7个风驱动壳（wind-driven shell）和1个弓形激波（bow shock），其中4个外流同时表现出慢盘风和风驱动壳特征。
...
### [28] [Cosmic-Ray-Constrained LSTM Model for Geomagnetic Storm Prediction](https://arxiv.org/abs/2512.22003)
*Zongyuan Ge,Chenwaner Zhang,Wei Zhou,Hongyu Zeng,Guiping Zhou*

**TL;DR**: 开发了一种融合宇宙线通量调制信息的物理知识驱动长短期记忆（LSTM）模型，通过整合宇宙线福布希下降（Forbush decreases）等19维特征向量，将地磁暴（GSTs）48小时预报准确率提升25.84%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于太阳风-磁层耦合驱动的地磁暴（GSTs）对技术系统的破坏性影响，旨在通过整合宇宙射线（GCR）前兆信号提升预测精度和预警时间。

Data: 数据来源包括1995-2020年间的多源空间天气数据，涵盖宇宙射线（GCR）观测、太阳风等离子体参数、行星际磁场数据以及地磁指数。

Method: 采用物理信息长短期记忆网络（LSTM）模型，整合宇宙射线通量调制作为先兆信号，构建19维特征向量输入50单元LSTM架构进行预测。

Result: 基于物理信息的长短期记忆(LSTM)模型通过整合宇宙线(cosmic-ray)前兆信号，将48小时地磁暴(GST)预报能力提升25.84%。
...
### [29] [Numerical Simulations of the Circularized Accretion Flow in Population III Star Tidal Disruption Events. I. The Accretion Flow and the Wind](https://arxiv.org/abs/2512.21500)
*Yu-Heng Sheng,De-Fu Bu,Xiao-Hong Yang,Yi-Ren Chang,Liang Chen*

**TL;DR**: 通过辐射流体动力学模拟发现，第三族星（Pop III）潮汐瓦解事件（TDE）系统中超大质量黑洞（$10^6$太阳质量）吸积率远超爱丁顿极限，辐射压驱动的高动能（达$10^{46}$ erg/s）强风带走了65%–75%回落物质，仅25%–35%被吸积，该风特性对理解第三族星TDE辐射的"风再处理"模型至关重要。


<details>
  <summary>Details</summary>

Background: 潮汐瓦解事件(TDEs)近期被提出作为探测第一代恒星(Population III stars)的潜在探针。

Data: 通过辐射流体动力学模拟（radiative hydrodynamic simulations）研究了质量为$10^6$和$300$太阳质量的黑洞（BH）与恒星组成的第三族星（Pop III）潮汐瓦解事件（TDE）系统的"环流"吸积流（'circularized' accretion flow）。

Method: 采用辐射流体动力学模拟（radiative hydrodynamic simulations）研究第三族恒星（Pop III star）潮汐瓦解事件（TDE）系统的环流吸积流（'circularized' accretion flow）特性，重点关注辐射压驱动的强风（wind）特性。

Result: 通过辐射流体动力学模拟发现，第一代恒星（Pop III）潮汐瓦解事件（TDE）系统中辐射压力驱动的强风带走了约65%–75%的回流物质，其动能功率高达10^46 erg/s。
...
### [30] [Multiband optical variability on diverse timescales of the blazar Ton 599 from 2011 to 2023](https://arxiv.org/abs/2512.22028)
...
**TL;DR**: 通过分析Ton 599（FSRQ）2011-2023年的多波段光变数据，发现其光学辐射呈现对数正态分布和红噪声功率谱，在低、中、高流量态分别表现出红变亮、无色和蓝变亮趋势，并基于光变时标估算出发射区尺度、磁场强度及中心黑洞质量（约10^8太阳质量）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核(AGN)中平谱射电类星体(FSRQ) Ton 599的光学变异性特征及其与中心引擎物理参数(如超大质量黑洞质量)和辐射过程(热辐射、同步辐射)的关联。

Data: 数据来源为世界望远镜网络（WEBT）合作项目2011-2023年的BVRI测光数据，以及斯图尔德天文台（Steward Observatory）的测光和光谱数据。

Method: 采用对数正态分布检验、均方根-流量关系分析、功率谱密度(PSD)估计、卡方检验($χ^2$ test)和分数变率(fractional variability)量化、光谱斜率计算、红移校正通量、单色光度计算以及颜色-星等(color-magnitude)和颜色-时间(color-time)图生成等方法。

Result: Ton 599的光学流量分布更符合对数正态分布(log-normal)，所有波段均呈现RMS-通量正相关且功率谱密度(PSD)表现为红噪声特征。
