...
**TL;DR**: 基于盖亚(Gaia)数据分析了双星团NGC 869和NGC 884的结构、运动学及银河系轨道特性，确定二者具有共同起源并可能在1100万年内发生动力学相互作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用盖亚(Gaia)时代数据探究双星团NGC 869和NGC 884的结构、天体物理特性、运动学及银河系轨道演化，以揭示其共同起源和动力学相互作用机制。

Data: 研究使用了盖亚(Gaia)天体测量数据，并应用UPMASK算法估算星团成员概率，其中NGC 869和NGC 884分别确定了808颗和707颗高概率成员星。

Method: 采用UPMASK算法处理盖亚(Gaia)天体测量数据估算星团成员概率，并基于贝叶斯框架结合PARSEC恒星演化等龄线和马尔可夫链蒙特卡洛(MCMC)方法推断基本天体物理参数。

Result: 通过应用UPMASK算法分析盖亚(Gaia)天体测量数据，确定了NGC 869和NGC 884分别包含808颗和707颗具有最高成员概率的恒星。
...
**TL;DR**: 首次利用JWST/NIRCam在78个放大星系中发现222个高红移(z>0.5)星团，通过SED拟合揭示其年轻年龄(<100 Myr)、高恒星密度(10^2-10^6 M/pc^2)及幂律质量函数(斜率β=-1.89)，表明宇宙正午时期已存在类似核星团的致密结构，可能是中等质量黑洞的形成通道。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST/NIRCam观测高红移(z>0.5)星团(star clusters)，探索宇宙正午(cosmic noon)时期恒星结构的形成与演化。

Data: 数据来源于JWST/NIRCam对78个不同星系团场中放大星系的观测，其中约60%的星团样本来自AbellS1063（GLIMPSE项目）的极深NIRCam观测。

Method: 采用JWST/NIRCam多波段同步尺寸-流量估计和光谱能量分布(SED)拟合分析，结合贝叶斯推断(Bayesian inference)测量星团质量函数。

Result: 首次在78个放大星系中探测到222个高红移（z>0.5）星团，其中约60%位于AbellS1063深场观测中，揭示了引力透镜与深度观测结合对探测原始恒星结构的强大能力。
...
**TL;DR**: 通过测量红移3-9星系中氢离子（Hα）和氧离子（[OIII]）的线紫外比（rline）及等效宽度（EW），发现其散射幅度不随红移显著变化，表明恒星形成爆发性在红移3-7期间保持稳定，且上升型、长持续时间、大幅度的恒星形成历史模型能解释高红移（z>10）亮星系的丰度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星形成时标（star formation timescales）对理解反馈机制（feedback mechanisms）和高红移（$z>10$）明亮星系丰度的影响。

Data: 数据来源于UNCOVER/MegaScience的光谱测光（spectro-photometry）观测数据，直接测量了Hα（\hanii）和[OIII]（\oiiihb）的线紫外比（\rline）和线等效宽度（EW），并通过与光谱数据的对比验证了系统精度优于10%。

Method: 采用UNCOVER/MegaScience光谱测光数据直接测量Hα（\hanii）和[OIII]（\oiiihb）的线紫外比（\rline）及等效宽度（EW），不依赖特定恒星形成历史（SFH）或星云线模型，并通过\texttt{fsps}构建种群级玩具模型解释观测结果。

Result: 在$z\sim3$到$z\sim7$期间，恒星形成爆发性（burstiness）的相似性导致谱线-紫外光度比（\rline）的固有散射未显著演化，但可能在$z\gtrsim8$时增加。
...
**TL;DR**: 提出了一种基于几何优先的模型，将薄盘水脉泽（water megamaser）观测数据（如天空角度、频移及其长期漂移、角红移率）映射到施瓦西规范（Schwarzschild gauge）下静态球对称（SSS）时空中的黑洞参数，通过局域点积关系和全局高斯-博内定理（Gauss-Bonnet theorem）验证，实现了黑洞质量、地球距离及脉泽轨道半径的完全约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于构建几何优先模型，用于在静态球对称（SSS）时空中将薄盘水脉泽（megamaser）观测数据映射至黑洞参数，并探索局部与全局几何关系的相互验证。

Data: 使用甚长基线干涉测量（VLBI）获得的薄盘水脉泽（water megamaser）观测数据，包括天空角度、频移、其长期漂移和角红移率（angular redshift rate）。

Method: 采用几何优先模型（geometry-first model）结合局域点积关系和赤道面弯曲几何，将观测数据与光子运动常数关联，并应用高斯-博内定理（Gauss-Bonnet theorem）进行全局自洽验证。

Result: 该研究构建了一个几何优先的模型，通过将观测到的薄盘水脉泽（water megamaser）参数（包括天空角度、频移、长期漂移和角红移率）映射到施瓦西规范（Schwarzschild gauge）下的静态球对称（SSS）时空中的黑洞参数，建立了光子运动常数与天空位置之间的局部关联。
...
**TL;DR**: 通过量化欧几里得(Euclid)望远镜观测条件对TNG50模拟中棒旋星系(Barred Galaxy)可探测性的影响，发现观测效应导致棒结构探测率显著降低（44%降至12-33%），揭示了模拟与观测间棒比例差异源于探测偏差。


<details>
  <summary>Details</summary>

Background: 研究探讨了欧几里得(Euclid)等现代巡天观测与TNG50模拟在棒旋星系(barred galaxies)比例上存在的差异，重点关注观测偏差对棒结构探测的影响。

Data: 数据来源为TNG50（The Next Generation IllustrisTNG-50）模拟数据集，包括$z=0.5$处完整星系样本的恒星质量分布图及通过蒙特卡洛辐射传输与后处理生成的合成图像（覆盖Euclid VIS $I_{\rm E}$、$Y_{\rm E}$及VIS-NISP RGB复合波段）。

Method: 采用蒙特卡洛辐射传输（Monte Carlo radiative transfer）和真实后处理生成合成图像，结合椭圆拟合（ellipse fitting）、傅里叶分解（Fourier decomposition）及Zoobot分类器进行分析。

Result: 在欧几里得(Euclid)观测条件下，TNG50模拟中棒旋星系(barred galaxy)的探测率显著下降，导致棒旋星系比例从44%降至12-33%，突显观测效应对棒结构探测的强烈影响。
...
**TL;DR**: 研究了质量$M$天体在垂直分层介质（scaleheight $H$）中诱导的尾迹密度和速度分布，发现当$H$等于引力半径$ξ_{0}$时尾迹最密最慢，停滞点位置和吸积率随$H/ξ_{0}$变化，并给出了无限薄层的解析解。


<details>
  <summary>Details</summary>

Background: 研究探讨了质量体在垂直分层介质（vertically-stratified media）中运动时诱导产生的尾迹（tail）密度与速度分布，聚焦于尺度高度（scaleheight）与引力半径（gravitational radius）比值对流体动力学过程的影响。

Data: 采用一维模型计算质量体$M$在垂直分层介质（scaleheight $H$）中平面诱导的尾迹密度和速度剖面，数据来源于对邦迪-霍伊尔-利特尔顿（Bondi-Hoyle-Lyttleton）问题的类比求解。

Method: 采用一维邦迪-霍伊尔-利特尔顿(Bondi-Hoyle-Lyttleton)模型结合打靶法(shooting method)求解满足特定物理数学约束的密度和速度剖面解族。

Result: 当尺度高度（scaleheight）H等于引力半径（gravitational radius）ξ0时，尾迹密度最大且速度最慢。
...
**TL;DR**: 研究发现，尽管JO201和JW100的冲压剥离尾、NGC 5291的碰撞环及NGC 7252的潮汐尾形成机制不同，但它们的尘埃含量和恒星形成活动显著相似。


<details>
  <summary>Details</summary>

Background: 研究探讨了不同扰动机制（引力相互作用与流体动力剥离）对星系外结构（如环和尾）中尘埃含量与恒星形成活动的影响。

Data: 数据来源包括AstroSat搭载的紫外成像望远镜（Ultraviolet Imaging Telescope）获取的高分辨率观测数据，以及先前研究中关于NGC 5291和NGC 7252的数据。

Method: 采用紫外连续谱斜率（β）通过FUV-NUV颜色计算尘埃衰减（dust attenuation），并据此校正星形成结（star-forming knots）的恒星形成率（SFR）。

Result: JO201和JW100的冲压剥离尾、NGC 5291的碰撞环及NGC 7252的潮汐尾中的尘埃衰减和尘埃校正恒星形成率密度具有显著相似性。
...
**TL;DR**: 首次通过叠加SRG/eROSITA全天巡天的X射线图像和光谱，在邻近（<50 Mpc）L*星系周围探测到显著延伸至约50千秒差距的弥散X射线辐射，其光谱特征支持热气体起源且与IllustrisTNG模拟预测的径向分布和总量一致，为校准下一代宇宙学模拟关键物理过程提供了约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系周介质（CGM）的热相（hot phase）在星系演化中的关键作用及其观测证据的缺失问题。

Data: 数据来源于SRG/eROSITA全天巡天（all-sky survey）的X射线图像和光谱叠加分析。

Method: 采用X射线图像和光谱叠加（stacking）技术分析SRG/eROSITA全天巡天数据，结合IllustrisTNG数值模拟验证热气体径向分布。

Result: 首次在近邻（<50 Mpc）L*星系周围50千秒差距范围内探测到显著弥散X射线辐射，其光谱特征支持热气体起源且排除了非热主导机制。
...
**TL;DR**: 基于公共数据集构建了包含约30万颗天琴RR变星(RRLs)的光度学星表和最大最均匀的RRLs与蓝水平支(BHB)恒星光谱数据集，揭示了银河系不同结构（晕、厚盘、薄盘）及主要恒星流中RRLs的金属丰度([Fe/H])和α元素丰度([α/Fe])分布特征及其径向梯度变化规律。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系内天琴RR变星（RR Lyrae variables, RRLs）和蓝水平分支（Blue Horizontal Branch, BHB）恒星的光谱与化学丰度特征，旨在揭示不同银河成分（晕、厚盘、薄盘）及恒星流（如盖亚-香肠-恩克拉多斯[GSE]）的金属丰度分布、α元素增强模式及径向梯度演化规律。

Data: 数据主要来自公开数据集的光度测量星表（约30万颗天琴RR变星）和最大最均匀的天琴RR变星与蓝水平分支（BHB）恒星光谱数据集（含约1.6万条视向速度测量、8140颗天琴RR变星的ΔS法铁丰度及547条文献铁丰度数据），并提供487颗天琴RR变星和64颗BHB恒星的高分辨率光谱元素丰度。

Method: 基于公共数据集的光度测量（photometric）和光谱测量（spectroscopic）数据，结合径向速度测量、铁丰度（ΔS方法）及高分辨率光谱元素丰度分析。

Result: result：研究发现天琴座RR型变星（RRLs）的金属丰度分布从晕（[Fe/H]=-1.56）到厚盘（TCD；[Fe/H]=-1.47）和薄盘（TND；[Fe/H]=-0.73）逐渐富集，且晕RRLs和逆行轨道RRLs呈现α元素增强（[α/Fe]=0.27），而厚盘RRLs在[Fe/H]≤-1.0时α增强、[Fe/H]>-1.0时α贫乏，薄盘RRLs主要α贫乏（[α/Fe]=-0.01）。
...
**TL;DR**: 该综述全面总结了星系内外多相气体（multiphase gas）数值模拟的研究进展，重点分析了从理想化小尺度（湍流混合层、云风相互作用等）到非理想化大尺度（超新星气泡、孤立星系模型、宇宙学放大模拟等）的模拟方法、关键物理量及跨尺度关联，并指出了当前挑战与未来方向。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于星系内外多相气体（multiphase gas）的数值模拟及其在理解星系形成与演化中的关键作用。

Data: 研究使用了理想化湍流混合层、云-风相互作用、热不稳定性及湍流盒子等数值模拟（simulation）数据，并涵盖辐射超新星气泡、高盒子模拟、孤立星系模型（包括矮星系和银河系质量系统）以及宇宙学放大（zoom-in）模拟等非理想化或更大尺度模拟数据。

Method: 采用数值模拟方法研究多相气体（multiphase gas）动力学，包括理想化湍流混合层、云风相互作用、热不稳定性及湍流盒子等模型，并扩展到辐射超新星气泡、高盒子模拟、孤立星系模型（含矮星系和银河系质量系统）以及宇宙学放大模拟（cosmological zoom-in simulations）。

Result: 多相气体（multiphase gas）的数值模拟对理解星系形成和演化至关重要，但因其多尺度和多物理特性而极具挑战性。
...
**TL;DR**: 基于JWST观测的31个平均红移(redshift)为7.3的星系样本分析表明，这些具有巴尔默跳变(Balmer break)和莱曼跳变(Lyman break)特征的"小红点"星系平均年龄为0.61±0.31 Gyr，形成于宇宙年龄0.70 Gyr之前（红移z>11.2），其中部分最红星系年龄与ΛCDM宇宙年龄存在>4.7σ显著性矛盾。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用詹姆斯·韦伯太空望远镜（JWST）观测数据探究高红移（redshift 7.3）星系中巴尔默跳变（Balmer break）和莱曼跳变（Lyman-$α$ break）特征，以揭示早期宇宙星系形成与演化的物理机制。

Data: 数据来源包括哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）的近红外相机（NIRCam）测光数据、13个星系的近红外光谱仪（NIRSpec）光谱数据以及15个星系的中红外测光数据（主要为JWST的中红外仪器MIRI）。

Method: 采用光谱能量分布（SED）拟合方法，结合巴耳末发射线、巴耳末+4000埃断裂或钙离子线光谱分析，并考虑真实消光曲线、V型SED和静止近红外低发射特性。

Result: 在31个红移平均值为7.3的星系中，最红星系呈现最大年龄，其中一个极红星系年龄与宇宙年龄存在>4.7σ的不兼容性，且TP-AGB效应无法解释该矛盾。
...
**TL;DR**: 通过ALMA观测红外暗云G14.225-0.506核心尺度磁场（B-field）发现，尽管两个枢纽（hub）磁场强度和碎裂程度相似，但三个区域的磁场形态存在显著差异：北部枢纽（northern hub）大尺度垂直磁场的南部维持原状而北部因致密凝聚体（condensations）扭曲，超临界质量-磁通比（mass-to-flux ratio）预示其即将坍缩；南部枢纽（southern hub）磁场不均匀且与凝聚体密度无直接关联；孤立尘埃团块（dust clump）磁场转为平行排列，表明磁场几何形态随空间尺度演化，北部枢纽最致密凝聚体正经历引力坍缩。


<details>
  <summary>Details</summary>

Background: 研究探讨了分子云碎裂过程中磁场（B-field）与湍流、引力之间的相互作用及其调控作用。

Data: 数据来源为ALMA（Atacama Large Millimeter/submillimeter Array）在红外暗云G14.225-0.506核心尺度上观测到的磁场（B-field）数据，并与先前在枢纽-纤维系统（Hub-filament system）尺度上的观测结果进行对比。

Method: 采用极化辐射观测结合质量-磁通量比（mass-to-flux ratio）分析，通过ALMA在核心尺度上探测磁场（B-field）形态与强度，并与丝状结构尺度观测结果对比。

Result: 在红外暗云G14.225-0.506的北部枢纽中检测到的最致密凝聚体正在经历引力坍缩，这由磁场与引力势的相对重要性以及质量-磁通比所揭示。
...
**TL;DR**: 通过分析小麦哲伦云(SMC)东北部外围星团的3D空间速度，发现其残余速度显著偏离主盘旋转模型，证实该区域存在由大麦哲伦云(LMC)潮汐作用导致的持续运动学扰动，且扰动存在于不同年龄星族中。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系相互作用（galaxy interaction）对小麦哲伦云（SMC）外围区域运动学扰动（kinematic disturbance）的影响及其与星系旋转盘（rotation disk）的关系。

Data: 数据来源于GEMINI GMOS光谱仪获取的星团平均径向速度（mean radial velocities），并结合推导的平均自行运动（mean proper motions）和日心距离（heliocentric distances）计算三维空间速度分量（3D space velocity components）。

Method: 通过分析GEMINI GMOS光谱获取的星团平均径向速度（radial velocity），结合自行运动（proper motion）和日心距离（heliocentric distance）数据，计算了其三维空间速度分量（3D space velocity components）。

Result: 小麦哲伦星云(SMC)外东北区域存在由与大麦哲伦星云(LMC)潮汐作用产生的残余速度模式，其星团三维空间速度分量与星系有序旋转预期值存在显著差异。
...
**TL;DR**: 行星状星云光度函数（PNLF）可作为独立于Ia型超新星校准的本地宇宙精确距离指示器，用于测量早期型星系（ETGs）距离至100 Mpc，从而为解决哈勃常数（H0）张力问题提供新方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过行星状星云光度函数（PNLF）等新型距离指标解决哈勃常数（H0）测量中宇宙微波背景与传统距离阶梯法之间的显著差异问题。

Data: 数据来源于MUSE存档数据（MUSE archival data）和新宽视场光谱设施（new wide-field spectroscopic facilities）的观测数据。

Method: 采用行星状星云光度函数（PNLF）作为距离指示器，结合多单元光谱探测仪（MUSE）存档数据和新一代宽视场光谱设施进行单次观测，实现对早期型星系（ETGs）的距离测量。

Result: 行星状星云光度函数（PNLF）可作为独立于Ia型超新星校准的局域宇宙精确距离指示器，其测量范围可达100 Mpc且仅需单次观测即可实现对哈勃常数（H0）的高精度估算。
...
**TL;DR**: 利用XRISM/Resolve高分辨率光谱首次绘制M87多温度X射线气体动力学图，发现东/西南臂较冷气体存在相反视线速度梯度（达数百km/s）且速度弥散高于环境热相，支持活动星系核（AGN）喷流抬升场景，同时揭示热相气体动力学扰动有限且AGN驱动运动在热环境星系际介质（ICM）中可能短暂。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高分辨率X射线光谱解析M87中活动星系核（AGN）喷流与周围多温度气体（multi-temperature X-ray gas）的复杂相互作用，特别是与历史AGN爆发相关的东侧和西南侧臂状结构的气体动力学特征。

Data: 使用XRISM/Resolve高分辨率光谱仪对M87核心区域进行马赛克观测，获取多温度X射线气体速度结构数据，重点分析东部和西南部与历史活动星系核(AGN)喷发相关的臂状结构。

Method: 采用高分辨率X射线光谱仪XRISM/Resolve对M87核心区域进行多温度模型拟合，分析不同区域和能段光谱以获取热环境气体与冷却抬升气体的视线速度及速度弥散。

Result: 通过XRISM/Resolve高分辨率光谱分析发现，M87中较冷气体相（cooler gas phase）沿视线方向呈现高达数百公里/秒的速度梯度及高于周围热相（hot phase）的速度弥散，支持东侧和西南侧X射线臂的气体抬升（uplift）模型。
...
**TL;DR**: 基于WINGS和MANGA巡天数据及Illustris模拟(TNG50和TNG100)，分析了不同形态星系的多重标度关系(ScRs)分布差异成因，并探讨了观测与模拟数据的正确统计比较方法。


<details>
  <summary>Details</summary>

Background: 研究主题为探索不同形态星系的多重标度关系（Scaling relations, ScRs）及其在两套大型巡天数据（WINGS和MANGA）中的分布差异成因。

Data: 数据来源于两个大型巡天项目（WINGS和MANGA）以及Illustris大规模模拟的两个子集（TNG50和TNG100）。

Method: 采用诊断平面分析（diagnostic plane analysis）方法，通过参数化星系分布比例来研究不同形态星系的标度关系（ScRs），并结合观测数据与理论模拟（Illustris的TNG50和TNG100子集）进行统计比较。

Result: result：通过分析WINGS和MANGA巡天数据，发现不同形态星系的多重标度关系（ScRs）存在显著差异，并与Illustris模拟（TNG50和TNG100）的理论数据进行了统计验证。
...
**TL;DR**: 利用JWST高分辨率红外光谱首次系统研究了低光度活动星系核(LLAGN)，发现其在爱丁顿比(log($L_{bol}/L_{Edd}$) ~ -3.5)处存在紫外光子不足的能谱转变，并观测到核区硅酸盐(~10 μm)发射及异常高温(~500 K)的H$_2$转动激发。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低光度活动星系核(LLAGN)的红外光谱特征及其与吸积率、黑洞质量的关系。

Data: 数据来源于JWST/NIRSpec和MIRI获取的8个低光度活动星系核(LLAGN)及半人马座A(Cen A)的存档近红外和中红外光谱数据。

Method: 采用JWST/NIRSpec和MIRI获取近红外至中红外光谱数据，结合高空间分辨率技术分离活动星系核(AGN)与宿主星系辐射，并运用发射线诊断法分析电离势能线及分子氢(H$_2$)转动激发温度。

Result: 在低光度活动星系核(LLAGN)中观测到当对数爱丁顿比(log($L_{bol}/L_{Edd}$))低于-3.5时，光谱能量分布出现紫外光子显著减少的转变特征。
...
### [18] [Calibration-independent consistency test of BAO and SNIa data: update](https://arxiv.org/abs/2601.16229)
*Bikash R. Dinda,Roy Maartens,Chris Clarkson*

**TL;DR**: 提出了一种基于阿尔科克-帕琴斯基变量（Alcock-Paczynski variable）和GPR（Gaussian Processes Regression）的新方法，验证了DESI DR2重子声波振荡（BAO）与Union3、Pantheon+和DES-Dovekie三种超新星数据集在1σ水平内的一致性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过阿尔科克-帕琴斯基变量（Alcock-Paczynski variable）这一共同参数检验未校准的重子声波振荡（BAO）与Ia型超新星（SNIa）数据间的一致性，独立于暗能量或修正引力模型以及声波视界和SNIa峰值亮度的影响。

Data: 数据来源于DESI DR2 BAO（重子声波振荡）和三个超新星数据集（Union3、Pantheon+和DES-Dovekie）。

Method: 采用高斯过程（Gaussian Processes）分析未校准的重子声波振荡（BAO）和Ia型超新星（SNIa）数据的一致性，通过阿尔科克-帕琴斯基变量（Alcock-Paczynski variable）作为共同参数。

Result: DES-Y5 SNIa数据与其他数据集存在不可忽略的张力，但更新后的DES-Dovekie消除了这一张力。
...
**TL;DR**: 利用双曲脐形（HU）透镜构型在RX J0437.1+0043星系团中发现疑似暗物质子晕（$m_{\rm halo} = 2.25 \pm 0.94 \times 10^9 M_\odot$），为研究致密环境中暗物质子晕特性提供了新方法。


<details>
  <summary>Details</summary>

Background: 研究主题为利用双曲脐型（Hyperbolic-Umbilic）引力透镜构型探测低质量（≤10^9 M⊙）暗物质（dark matter）子结构以约束暗物质本质。

Data: 数据来源包括高分辨率成像（high resolution imaging）、积分视场单元光谱（IFU spectroscopy）和引力透镜建模（gravitational lensing modelling）。

Method: 采用高分辨率成像、积分视场单元（IFU）光谱和引力透镜建模相结合的方法。

Result: 在RX J0437.1+0043星系团中通过双曲脐（Hyperbolic-Umbilic）透镜构型初步探测到一个暗物质子晕（subhalo）（$m_{\rm halo} = 2.25 \pm 0.94 \times 10^9 M_\odot$）。
...
### [20] [TORRCH: Tomographic reconstruction of the reionization of cosmic hydrogen with Ly$α$ emitters and non-Ly$α$-selected galaxies](https://arxiv.org/abs/2601.16263)
*Soumak Maitra,Girish Kulkarni,Vipul Arora,Matteo Viel,Shikhar Asthana,James S. Bolton,Martin G. Haehnelt,Laura Keating*

**TL;DR**: 提出了深度学习框架TORRCH（宇宙氢再电离层析重建），通过Ly$α$发射体（LAEs）和非Ly$α$选择星系（NLSGs）的空间分布重建再电离时期中性氢分数场，能高保真恢复大尺度电离形态并准确预测中性分数的一阶分布和二维功率谱。


<details>
  <summary>Details</summary>

Background: 研究主题为利用深度学习框架（TORRCH）从莱曼阿尔法发射体（LAEs）和非莱曼阿尔法选择星系（NLSGs）的空间分布中重建宇宙再电离时期的中性氢分数场（neutral-hydrogen fraction field），以突破传统全局统计方法的局限。

Data: 使用经过辐射传输后处理的水动力学模拟（hydrodynamical simulations post-processed with radiative transfer）生成覆盖多种再电离场景的模拟巡天数据，训练确定性3D U-Net模型。

Method: 采用深度学习框架TORRCH（TOmographic Reconstruction of the Reionization of Cosmic Hydrogen）中的确定性3D U-Net模型，基于流体动力学模拟结合辐射传输后处理生成的多种再电离情景模拟数据，从莱曼α发射体（LAEs）和非莱曼α选择星系（NLSGs）的空间分布中重建宇宙再电离时期的中性氢比例场。

Result: 开发了深度学习框架TORRCH（宇宙氢再电离层析重建），能够从莱曼α发射体（LAEs）和非莱曼α选择星系（NLSGs）的空间分布中高保真重建再电离时期中性氢分数场。
...
### [21] [Relaxing DESI DR2 BAO Constraints on $\sum m_ν$ with Planck and SPT-3G 2018 in the Context of SPT D1](https://arxiv.org/abs/2601.16277)
*Dmitry Gorbunov,Nikita Nedelko*

**TL;DR**: 结合SPT-3G 2018、Planck PR4透镜及PR3低$\ell$数据，测得中微子质量总和($\sum m_ν$)的95%上限为0.11 eV，缓解了$\rm Λ$CDM与振荡实验下限的张力；加入DES Y1弱透镜和Pantheon+超新星数据后上限放宽至0.138 eV且倾向于$\sum m_ν>0$，而采用SPT-3G 2019-2020(D1)数据则约束收紧至<0.082 eV并支持准负质量。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于通过宇宙微波背景（CMB）和重子声波振荡（BAO）等观测数据约束中微子质量总和（$\sum m_ν$），以调和Λ冷暗物质（ΛCDM）模型与中微子振荡实验间的矛盾。

Data: 数据来源包括SPT-3G 2018 TT/TE/EE+透镜谱、Planck PR4透镜数据、Planck PR3低$\ell$部分谱、DESI DR2重子声波振荡（BAO）测量、DES Y1弱透镜信息及Pantheon+超新星（SNIa）星表。

Method: 采用SPT-3G 2018 TT/TE/EE+透镜谱、Planck PR4透镜数据及Planck PR3低$\ell$谱作为基线数据集，结合DESI DR2重子声波振荡（BAO）测量、DES Y1弱透镜信息和Pantheon+超新星（SNIa）目录进行联合分析。

Result: result：结合DESI DR2重子声波振荡（BAO）测量数据，得出中微子质量总和（$\sum m_ν$）的95%上限为0.11电子伏特（eV），缓解了Λ冷暗物质（ΛCDM）模型与基于中微子振荡实验的下限之间的张力。
...
### [22] [MeerKAT observations of Abell 1775 and Abell 1795: the discovery of a hadronic radio halo?](https://arxiv.org/abs/2601.16288)
...
**TL;DR**: 通过MeerKAT和LOFAR观测发现Abell 1775和Abell 1795两个冷核星系团中存在由内外两部分组成的巨型射电晕（radio haloes），其中Abell 1795的平坦频谱（$\alpha=-1.08\pm0.06$）且延伸至$\sim$1 Mpc的射电晕挑战了动态弛豫星系团无法维持粒子再加速的传统认知，表明次级电子（secondary electrons）的强子作用或残留湍流可能驱动此类辐射。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于星系团中巨射电晕（giant radio haloes）和迷你射电晕（mini-haloes）的起源与演化机制，特别关注它们在冷却核心（cool-core）和动态弛豫（dynamically relaxed）星系团中的非典型存在及其对宇宙射线电子（cosmic-ray electrons）加速过程的启示。

Data: 数据来源包括903-1655 MHz频段的MeerKAT观测数据以及重新处理的120-168 MHz频段LOFAR两米巡天（LOFAR Two-metre Sky Survey）数据。

Method: 采用903-1655 MHz MeerKAT观测和120-168 MHz LOFAR再处理数据进行成像与频谱分析（spectral analysis），结合冷锋（cold fronts）和冷核（cool cores）特征研究射电晕结构。

Result: 在Abell 1795中发现了一个延伸至约1 Mpc、具有平坦谱（谱指数α=-1.08±0.06）的大型射电晕（radio halo），表明即使在动力学松弛的星系团中仍可能存在粒子再加速或强子相互作用产生次级电子的机制。
...
### [23] [Unifying cluster and galaxy cosmology analyses using the galaxy-halo connection](https://arxiv.org/abs/2601.16353)
*Shulei Cao,Hao-Yi Wu,Andrés N. Salcedo,David H. Weinberg,Matthieu Schaller,Joop Schaye*

**TL;DR**: 提出了一种基于星系-晕关联（galaxy-halo connection）的统一模拟框架，通过联合分析星系团引力透镜（cluster lensing）、星系成团性（galaxy clustering）和星系-星系团互相关（galaxy-cluster cross-correlation），将σ8-Ωm参数约束精度提升15倍。


<details>
  <summary>Details</summary>

Background: 研究主题为通过星系-晕（galaxy-halo）关联统一建模星系和星系团的宇宙学统计特性。

Data: 数据来源于FLAMINGO流体动力学模拟（hydrodynamic simulation）。

Method: 采用基于星系-晕连接（galaxy-halo connection）的统一模拟框架，结合星系团引力透镜（cluster lensing）、星系成团性（galaxy clustering）和星系-星系团互相关（galaxy-cluster cross-correlation）进行联合分析。

Result: 开发了一种基于星系-晕关联（galaxy-halo connection）的统一模拟框架，联合建模星系和星系团的统计特性，将σ8-Ωm指标（figure of merit）的精度提升了15倍。
...
### [24] [XMM-Newton observations of ten high-redshift CAMIRA clusters of galaxies](https://arxiv.org/abs/2601.16386)
...
**TL;DR**: 基于XMM-Newton对10个高红移（0.81 < z < 1.17）富星系团（CAMIRA catalog）的观测发现，其ICM（intracluster medium）动态松弛比例低（约10%），AGN（active galactic nucleus）活动显著增强且外围更活跃，标度关系无显著红移演化，支持星系团主要性质在z∼1时已基本确立的观点。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（0.81 < z < 1.17）富星系团（richness N > 40）内星系际介质（ICM）动力学状态及其与星系团形成演化的关联。

Data: 数据来源于XMM-Newton对十个高红移（0.81 < z < 1.17）星系团的观测，这些目标选自基于高富度（N > 40）的CAMIRA星表，并在Hyper Suprime-Cam Subaru Strategic Program（HSC-SSP）天区中被识别。

Method: 采用X射线成像与光谱分析（X-ray imaging and spectral analyses）测量ICM（intracluster medium）温度与辐射光度，并通过BCG（brightest cluster galaxy）与X射线峰值的偏移研究形态学特征。

Result: 高红移（0.81 < z < 1.17）富星系团样本中仅约10%表现为动力学弛豫状态，且活动星系核（AGN）比例显著高于低红移样本，尤其在团外围区域更为活跃。
...
### [25] [Unveiling a Thin Filament of the Cosmic Web in the Ursa Major Supergroup](https://arxiv.org/abs/2601.16408)
*Jin-Long Xu,Ming Zhu,Peng Jiang,Nai-Ping Yu,Chuan-Peng Zhang,Xiao-Lan Liu,Mei Ai,Yin-Jie Jing,Jie Wang*

**TL;DR**: 利用五百米口径球面射电望远镜（FAST）在大熊座超星系群中探测到一个由16个不同类型星系和5个无星气体团块组成的极薄原子氢（HI）纤维结构（长约0.9 Mpc），其厚度与星系直径相当，并观测到星系-纤维体自旋共向性和内部速度梯度，表明存在冷吸积流促进星系形成与生长，该纤维体可能通过等级结构形成并入大熊座超星系群。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙网（cosmic web）中丝状结构（filaments）作为星系和气体大尺度排列的关键载体及其在等级结构形成（hierarchical structure formation）中的作用。

Data: 使用五百米口径球面射电望远镜（FAST）对大熊座超星系团进行原子氢（HI）观测获取数据。

Method: 采用原子氢（HI）观测技术结合五百米口径球面射电望远镜（FAST）探测宇宙细丝结构。

Result: 利用五百米口径球面射电望远镜(FAST)在Ursa Major超星系团中探测到一个由16个不同类型星系和5个无星气体团块组成的极细氢原子(HI)纤维结构，其长度约0.9百万秒差距(Mpc)，厚度仅与星系直径相当。
...
### [26] [Cosmological analysis of a viable $f(R)$ gravity model](https://arxiv.org/abs/2601.16641)
*Siqi He,Weiqiang Yang*

**TL;DR**: 研究通过求解场方程并分析哈勃参数(Hubble parameter)和距离模数(distance modulus)，证明f(R)引力模型能解释宇宙从物质主导到暗能量主导的转变阶段，且与超新星(SNe Ia)观测数据及ΛCDM模型一致，同时通过多种诊断工具(r, s)、(r, q)、wD-w'D平面等系统比较，发现该模型演化图像可明显区别于ΛCDM。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索f(R)引力理论在物质主导时期向暗能量主导时期过渡阶段的动力学行为，以协调早期宇宙暴胀与晚期加速膨胀之间的矛盾。

Data: 数据来源包括SN Ia（超新星）观测数据以及哈勃参数（Hubble parameter）的测量值。

Method: 采用场方程求解结合哈勃参数和$y_{H}(z)$函数分析，并通过状态诊断器（statefinder hierarchy）、$(r, s)$平面、$(r, q)$平面、$w_{D}-w'_{D}$平面、增长率分析及$Om(z)$诊断工具系统对比$\Lambda$CDM模型。

Result: f(R)引力模型与SN Ia观测数据一致，并能解释宇宙晚期加速膨胀现象。
...
### [27] [One H2 molecule per ten million H-atoms reveals sub-pc scale cold overdensities at z~4](https://arxiv.org/abs/2601.16710)
...
**TL;DR**: 在类星体J0007-5705的z=4.24吸收系统中首次探测到迄今最高红移的分子氢(H2)，揭示出中性介质中存在微小(~0.01 pc)冷致密结构和两种不同物理条件(窄成分：log nH/cm^-3~2.8，T~40K；宽成分：log nH/cm^-3~1.4，T~600K)的云团。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高分辨率光谱技术探测和分析高红移（z = 4.24）类星体吸收系统中的分子氢（H2），以揭示早期宇宙中冷中性介质（cold neutral medium）的微观结构和物理条件。

Data: 数据来源于甚大望远镜(Very Large Telescope)的ESPRESSO类星体吸收线巡天(EQUALS)对类星体J0007-5705的高分辨率(R~120000)光谱观测。

Method: 采用极高分辨率光谱（R~120000）和分子氢（H2）转动能级建模技术，结合紫外辐射场条件分析，精确解析了两种速度成分的物理参数。

Result: 在z=4.24的类星体J0007-5705方向探测到迄今最高红移的分子氢(H2)吸收系统，总柱密度为N(H2)~2x10^14 cm^-2。
...
### [28] [When Primordial Black Holes Absorb During the Early Universe](https://arxiv.org/abs/2601.16717)
*Md Riajul Haque,Rajesh Karmakar,Yann Mambrini*

**TL;DR**: 研究发现早期宇宙中原始黑洞（PBHs）在热浴环境下的热吸收效应可显著增加其质量并延长寿命，临界塌缩效率（γ_c≈0.395）以上时质量无限增长，进而抑制再加热温度并修正暗物质（DM）参数空间达O(10)-O(10^4)量级，首次全面量化了热吸收对宇宙学可观测量影响。


<details>
  <summary>Details</summary>

Background: 该研究探讨了早期宇宙中形成的原初黑洞（PBHs）在周围热浴环境下的演化过程及其对宇宙再加热和暗物质（DM）产生的潜在影响。

Data: 数据来源于理论模拟（simulation）研究，通过构建包含热吸收效应的原初黑洞（PBHs）演化模型进行计算分析。

Method: 采用热吸收效应修正的霍金蒸发模型（Hawking evaporation scenario）分析原初黑洞（PBHs）演化动力学，重点计算临界塌缩效率（γ_c）对质量增长的约束条件。

Result: 在热吸收效应影响下，原初黑洞（PBHs）质量显著增长导致寿命延长，并偏离标准霍金蒸发模型，临界塌缩效率（γ_c≈0.395）决定质量是否无限增长。
...
**TL;DR**: 通过分析17个四重像尖点透镜（cusp lens）的微引力透镜（microlensing）无关流量比并结合冷暗物质（CDM）、自相互作用暗物质（SIDM）和模糊暗物质（FDM）模拟，发现J1042+1641在次要轴和窄主轴尖点透镜区域与CDM和SIDM存在>3σ不兼容，且贝叶斯因子超过100，强烈支持FDM模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用强引力透镜（strongly lensed）类星体的尖点（cusp）构型探测小尺度暗物质结构（dark matter structure），特别是比较冷暗物质（CDM）、自相互作用暗物质（SIDM）和模糊暗物质（FDM）模型的差异。

Data: 数据来源于17个四重像尖点构型（cusp configuration）引力透镜的微引力透镜（microlensing）无关流量比观测数据，并结合冷暗物质（CDM）、自相互作用暗物质（SIDM）和模糊暗物质（FDM）场景下的大量蒙特卡洛模拟（Monte Carlo simulations）生成的模拟透镜实现（mock lens realizations）。

Method: 采用蒙特卡洛模拟（Monte Carlo simulations）比较冷暗物质（CDM）、自相互作用暗物质（SIDM）和模糊暗物质（FDM）模型下的微引力透镜自由流量比异常。

Result: 在17个四重像尖点透镜（cusp lenses）的微引力透镜效应自由流量比分析中，J1042+1641的数据与冷暗物质（CDM）和自相互作用暗物质（SIDM）模型的差异超过3σ，且贝叶斯因子超过100，为模糊暗物质（FDM）模型提供了极强证据。
...
### [30] [Cosmography with $Λ$-Szekeres Models](https://arxiv.org/abs/2601.16844)
*Morag Hills,Asta Heinesen*

**TL;DR**: 该论文通过推导红移四阶光度距离的协变宇宙学描记法（covariant cosmography），并在轴对称塞凯赖斯模型（Szekeres models）中验证其数值结果与距离-红移关系的预测一致性，为模型无关的宇宙学数据分析及局部宇宙结构映射提供了新工具。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于宇宙学张力（cosmological tensions）推动的模型无关方法发展，特别是协变宇宙学（covariant cosmography）作为不依赖引力场方程或物质成分假设的工具，用于分析宇宙学数据并映射局部宇宙邻域的各向异性和不均匀性。

Data: 研究中使用的数据来源于Szekeres模型（Szekeres models）的数值模拟结果和合成观测者（synthetic observers）的距离-红移关系数据。

Method: 采用协变宇宙学（covariant cosmography）方法，对红移的四阶光度距离进行宇宙学分析，并在轴对称Szekeres模型中验证其适用性。

Result: 协变宇宙学描记法（covariant cosmography）是一种无需假设引力场方程或宇宙物质成分的高度通用性工具，可分析宇宙学数据并映射局部宇宙邻域的各向异性和不均匀性。
...
### [31] [A more inclusive effective dark fluid equation of state parameter: constraints from SKA and Euclid like surveys](https://arxiv.org/abs/2601.16899)
*Ziad Sakr*

**TL;DR**: 通过结合SKA和欧几里得(Euclid)巡天数据，研究发现广义参数化修正引力模型与ΛCDM的差异在1-2σ水平可被探测，但需更先进巡天数据才能显著区分。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过有效暗流体状态方程参数（weff）约束修改引力理论，该参数统一描述了修改引力对宇宙背景膨胀和大尺度结构增长的共同影响。

Data: 数据来源包括下一代SKA巡天（Square Kilometre Array）的观测规范，以及与之结合的欧几里得（Euclid）DR3巡天数据，并采用两种模拟数据模型：一种参数偏差为10%（但得出有效暗流体状态方程参数$w_{\rm eff}=-1.03$），另一种亚百分比接近$Λ$CDM模型。

Method: 通过将弗里德曼方程中的暗流体压力与密度关联，利用牛顿势方程参数μ₀、引力滑移参数η₀和红移依赖的哈勃参数H₀,bck，将有效暗流体状态方程参数w_eff与参数化修正引力模型联系起来。

Result: 通过将弗里德曼方程中的暗流体压力、密度与修正引力参数化模型相关联，利用牛顿势方程参数μ₀、引力滑移参数η₀和红移依赖的哈勃参数H₀,bck，构建了能同时反映宇宙背景膨胀和大尺度结构增长的有效暗流体状态方程参数w_eff的约束框架。
...
### [32] [The shape function of the observed growth index](https://arxiv.org/abs/2601.16921)
*Ziad Sakr,Jinglan Zheng*

**TL;DR**: 提出了一种基于红移分箱中可观测量的增长指数（growth index）无参数化建模方法，通过形状函数法构建其红移依赖关系，并验证了该方法在区分不同引力模型和扩展参数化模型中的有效性。


<details>
  <summary>Details</summary>

Background: 研究探讨了增长指数(growth index)在探测偏离ΛCDM模型时的作用，并提出了基于可观测量的无参数化方法构建其红移依赖关系。

Data: 数据来源为下一代巡天（next generation surveys）对红移区间内三个可观测量的预报测量值，即$fσ_8(z_i)$、$f(z_i)$和$H(z_i)$。

Method: 采用形状函数法（shape function method）构建增长指数γ的红移依赖关系，无需假设特定参数化形式，并基于可观测量fσ₈(zᵢ)、f(zᵢ)和H(zᵢ)直接确定其在红移分箱节点处的值及导数。

Result: 通过将增长指数(growth index)γ构建为与红移区间内可观测量fσ8(zi)、f(zi)和H(zi)直接相关的函数，并利用形状函数方法在不假设特定参数化形式的情况下重构其红移依赖关系，该方法能有效检测不同测量组分间的差异并区分修改引力模型或扩展参数化模型，特别适用于未来阶段-IV巡天提供的更高精度数据下的模型无关宇宙学研究。
...
### [33] [Forecast on the generalised dark matter properties from a Euclid-like survey](https://arxiv.org/abs/2601.16943)
*Ziad Sakr,Jessica N. López-Sánchez*

**TL;DR**: 基于欧几里得(Euclid)任务数据，采用光谱星系成团(GCsp)、光测星系成团(GCph)、弱引力透镜(WL)及其互相关(XC)方法，首次精确预测了广义暗物质(GDM)参数\(w_{\rm gdm}\)和\(c^{2}_{s,{\rm gdm}}\)的约束误差，并证明该任务能有效区分不同物质涨落归一化(\(σ_8\))情景。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用欧几里得(Euclid)任务的光谱和光度数据，通过广义暗物质(GDM)模型参数\(w_{\rm gdm}\)和\(c^{2}_{s,{\rm gdm}}\)的约束，探索宇宙学中暗物质性质及其对宇宙结构形成的影响。

Data: 数据来源包括欧几里得(Euclid)任务提供的星系光谱红移、测光位置和形状数据，以及用于计算非线性广义暗物质(GDM)功率谱的专用模拟数据。

Method: 采用半解析非线性红移空间畸变(RSD)模型处理GCsp数据，利用专用模拟计算非线性GDM物质功率谱并修改透镜效应、成团性窗口及内禀排列模型处理GCph+WL+XC数据。

Result: 在乐观设置下，当σ8基准值约为0.81和0.77时，所有探测手段联合对广义暗物质(GDM)参数c²s,gdm和wgdm的相对误差分别可达1.96%和1.85%；当σ8基准值降至0.67时，联合探测对这两个参数的约束精度仍能保持在2.45%和2.04%，表明欧几里得(Euclid)类巡天能有效区分不同物质涨落归一化情景并约束GDM参数。
...
### [34] [Improving constraints on primordial non-Gaussianity from Quaia with a new cosmological observable: angular redshift fluctuations](https://arxiv.org/abs/2601.16948)
...
**TL;DR**: 通过结合角向红移涨落（ARF）、类星体角密度和宇宙微波背景（CMB）透镜效应，在Quaia类星体样本中测得局部非高斯性参数$f_{\rm NL} = -3 \pm 14$，这是目前基于大尺度结构两点统计的第二严格约束，并比先前结果提升了约25%。


<details>
  <summary>Details</summary>

Background: 角向红移涨落（ARF）是一种新兴的宇宙学观测量，用于通过物质示踪体的二维红移分布偏差约束原初非高斯性参数$f_{\rm NL}$。

Data: 数据来源于基于盖亚卫星(Gaia)和unWISE数据的类星体样本Quaia，并结合了普朗克卫星(Planck) PR4宇宙微波背景(CMB)透镜图。

Method: 采用角向红移涨落（ARF）功率谱分析技术，结合二维（投影）星系密度及其与普朗克PR4 CMB透镜图的互相关信息，联合约束局部非高斯性参数$f_{\rm NL}$。

Result: 角红移涨落(ARF)是一种新的宇宙学可观测量，可用于通过尺度依赖偏置效应约束原初非高斯性参数$f_{\rm NL}$。
...
### [35] [Calibrating redshift distributions at $z>2$ with Lyman-$α$ forest cross-correlations](https://arxiv.org/abs/2601.16962)
*Qianjun Hang,Laura Casas,William d'Assignies,Wynne Turner,Andreu Font-Ribera,Benjamin Joachimi*

**TL;DR**: 开发了基于莱曼阿尔法森林(Ly$α$ forest)与高红移($2<z<3$)测光星系角互相关的新理论框架，在10角分尺度和$Δz=0.1$条件下实现了24σ的互相关信号测量，可将星系样本平均红移精度提升至$σ_z/(1+\bar{z}) = 0.006$。


<details>
  <summary>Details</summary>

Background: 研究探讨利用莱曼阿尔法森林（Lyman-α forest）校准高红移端（2<z<3）测光星系（photometric galaxies）整体红移分布的可行性。

Data: 数据来源于\texttt{CoLoRe}模拟生成的DESI五年期莱曼α森林(Ly$α$ forest)和鲁宾天文台LSST十年期光度测量星系(photometric galaxies)的模拟数据，覆盖红移范围至$z=3$。

Method: 开发了直接建模角向互相关（angular cross-correlation）的理论框架，以解决莱曼阿尔法森林（Ly$α$ forest）中红移空间畸变（redshift-space distortions）导致的传统$n(z)$估计器不适用问题。

Result: 开发了一种直接建模莱曼阿尔法森林（Lyα forest）与高红移（2<z<3）测光星系角互相关的新理论框架，克服了传统红移分布估计方法因大尺度红移空间畸变而不适用的问题。
...
### [36] [proto-Lightspeed: a high-speed, ultra-low read noise imager on the Magellan Clay Telescope](https://arxiv.org/abs/2601.16268)
...
**TL;DR**: 研制了proto-Lightspeed高速光学成像仪（工作波段g'、r'、i'，视场直径1'，最高帧频200Hz），通过商用镜头和ORCA-Quest 2相机实现亚电子级读出噪声，适用于致密双星、系外行星凌日等快速变源观测。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发新型高速光学成像仪器proto-Lightspeed，旨在通过低噪声、高时间分辨率观测支持致密双星系统、系外行星凌日、吸积快速耀发、脉冲星周期性光学辐射及海外天体掩星等瞬变天文现象的研究。

Data: 数据来源于麦哲伦克莱望远镜（Magellan Clay Telescope）Nasmyth East端口新安装的proto-Lightspeed仪器，该仪器利用商业再成像镜头和滨松ORCA-Quest 2相机进行高速光学成像，视场直径1角分，最高帧率200赫兹（或更高帧率的窗口模式，如1.6角秒×1角分视场下可达6600赫兹），在g'、r'和i'波段提供视宁度极限的图像质量，像素尺度可调范围为0.017角秒至0.050角秒。

Method: 采用商业再成像透镜和滨松ORCA-Quest 2相机实现高速光学成像，支持200 Hz全视场或6600 Hz窗口模式观测，具备g'、r'和i'波段视宁极限成像质量及0.017''-0.050''可调像素尺度。

Result: proto-Lightspeed（原型光速仪）是一种新型高速光学成像仪器，能在g'、r'和i'波段提供视宁度（seeing-limited）限制的图像质量，并支持0.017''-0.050''可调像素尺度。
...
### [37] [Characterization of the commercial spectrograph system for astronomical observations: PIXIS 1300BX Camera and IsoPlane 320A Spectrograph](https://arxiv.org/abs/2601.16371)
*Jiwon Jang,Changsu Choi,Ho Seong Hwang,Haeun Chung,Hyeonguk Bahk,Dongkok Kim,Jae-Woo Kim*

**TL;DR**: 对PIXIS 1300BX CCD相机和IsoPlane 320A光谱仪(spectrograph)的系统测试表明，该设备在400-800 nm波段量子效率(quantum efficiency)超过80%、450-750 nm达90%以上，具备亚秒级余辉衰减和R≈600-2600的中等分辨率，适用于快速多目标光谱观测。


<details>
  <summary>Details</summary>

Background: 研究聚焦于商业光谱仪系统（PIXIS 1300BX CCD相机和IsoPlane 320A光谱仪）的实验室及实测性能表征，为邻近星系全天空光谱巡天（A-SPEC）项目做准备。

Data: 数据来源包括实验室测试（量化读出噪声、暗电流、增益和满阱容量）和首尔国立大学天文台（SAO）1米望远镜的实测观测。

Method: 采用梯度校正技术（gradient correction technique）解决CCD相机无快门条件下图像行依赖信号梯度问题，并通过偏置、暗场和光子转移曲线分析量化读出噪声、暗电流、增益和满阱容量。

Result: 量子效率在400-800纳米波段超过80%，在450-750纳米波段达到90%以上，且亚秒级余辉衰减特性使其适合快速多目标光谱观测。
...
### [38] [ALMA publication statistics](https://arxiv.org/abs/2601.16687)
...
**TL;DR**: 研究分析了阿塔卡马大型毫米波/亚毫米波阵列（ALMA）2012-2024年的4190篇同行评审论文，发现ALMA是平均每篇论文被引41次的高影响力天文台站，其论文影响因子超过其他主要天文设施，并在甚长基线干涉测量（VLBI）领域发挥关键作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于评估天文观测设施（ALMA）的科学影响力及其在毫米波/亚毫米波天文学领域的贡献。

Data: 数据来源于阿塔卡马大型毫米波/亚毫米波阵列（ALMA）2012至2024年间2670个观测项目（总计19265小时12米阵列等效观测时长）生成的4190篇同行评审论文。

Method: 采用文献计量学方法（bibliometric analysis）对ALMA（Atacama Large Millimeter/Submillimeter Array）的4190篇同行评审论文进行多维度统计分析，包括科学分类、地域分布、档案数据使用、发表时滞及引用情况等指标。

Result: ALMA（Atacama Large Millimeter/Submillimeter Array）是一个高影响力天文台，其论文平均引用次数为41次，约70%的观测项目已发表成果，2024年约40%的论文利用了存档数据，拥有超过9400位独特作者，且其论文影响力因子目前超过所有其他主要天文设施。
...
### [39] [Illuminating the Physics of Cosmic Origin and Evolution: A UK Space Frontiers 2035 White Paper](https://arxiv.org/abs/2601.16761)
...
**TL;DR**: 提出SIRMOS任务概念，通过高吞吐量光谱观测精确测量1<z<4范围内超1亿星系的红移，以研究宇宙暴胀（cosmic inflation）、加速膨胀和中微子质量总和等关键宇宙学问题。


<details>
  <summary>Details</summary>

Background: 探索宇宙起源和演化是现代宇宙学最根本的科学挑战之一。

Data: 数据来源为利用现有最深的宽视场测光星表（photometric catalogues）进行目标选择，并通过SIRMOS任务概念获取1.25-2.5微米波长范围内的高通量、高复用光谱数据，测量超过1亿个星系的红移。

Method: 采用高吞吐量、高度多路复用的光谱技术（highly multiplexed spectroscopic capability），通过SIRMOS任务概念获取超过1亿个星系的精确红移数据，光谱覆盖1.25-2.5微米波长范围，分辨率适中。

Result: 提出了SIRMOS（Space Infrared Multi-Object Spectrometer）任务概念，通过高吞吐量、多目标光谱能力获取超过1亿个星系的高精度红移数据，以研究宇宙暴胀（cosmic inflation）、加速膨胀及中微子质量总和等关键科学问题。
...
### [40] [The Astrosky Ecosystem: An independent online platform for science communication and social networking](https://arxiv.org/abs/2601.16838)
*Emily L. Hunt,Vincent S. Carpenter,Kyle W. Cook,Douglas G. Hilton,Mehnaaz Asad,Janine Jochum,Kelly Lepo,Jamie Zvirzdin*

**TL;DR**: 这篇论文的核心贡献是提出了Astrosky生态系统（The Astrosky Ecosystem），通过开发基于AT协议（AT Protocol）的开源工具和集成（主要为Bluesky社交网络），使天文学界能够独立运营不受营利性公司控制的社交媒体基础设施。


<details>
  <summary>Details</summary>

Background: 研究聚焦于天文学领域科学传播（science communication）的社会化媒体工具依赖性与平台脆弱性问题。

Data: 数据来源为自主开发的开放源代码工具Astrosky Ecosystem及其集成的Bluesky社交网络平台，采用AT协议（AT Protocol）作为底层技术框架。

Method: method：开发基于AT协议（AT Protocol）的开源工具和集成方案，主要面向Bluesky社交网络。

Result: 提出了Astrosky生态系统，旨在为天文学社区开发开源工具（特别是基于Bluesky社交网络）以实现独立于营利性企业的社交媒体基础设施。
...
### [41] [CosmoSlider: An educational tool for cosmology](https://arxiv.org/abs/2601.16919)
*Andreas Nygaard,Steen Hannestad,Thomas Tram*

**TL;DR**: 开发了跨平台轻量级工具CosmoSlider，通过神经网络模拟器(neural-network emulator)实现宇宙微波背景(CMB)功率谱随多宇宙学参数变化的实时可视化交互，辅助教学和自学。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙学教育中宇宙微波背景(CMB)功率谱与宇宙学参数关系的交互式探索工具开发。

Data: 数据来源于神经网络模拟器（neural-network emulator）实现的快速计算，无需依赖预先计算的大规模模型网格或实时运行爱因斯坦-玻尔兹曼求解器（Einstein-Boltzmann solvers）。

Method: 采用基于TensorFlow Lite的神经网络模拟器(neural-network emulator)实现快速光谱计算。

Result: 开发了CosmoSlider工具，通过神经网络模拟器（neural-network emulator）实现宇宙微波背景（CMB）功率谱随多个宇宙学参数变化的实时可视化交互探索。
...
### [42] [Probabilistic Graphical Models in Astronomy](https://arxiv.org/abs/2601.16959)
*Abigail Sheerin,Giuseppe Vinci*

**TL;DR**: 概率图模型（probabilistic graphical models）通过节点网络揭示系外行星与宿主恒星间复杂依赖关系，为现代天文数据集分析提供了新方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用概率图模型（probabilistic graphical models）解决现代天文数据爆炸背景下传统方法难以处理的复杂依赖关系问题。

Data: 数据来源于现代天文观测仪器（observational instrumentation）获取的复杂数据集。

Method: 采用概率图模型（probabilistic graphical models）分析宇宙变量间的依赖结构和数据生成过程。

Result: 概率图模型（probabilistic graphical models）通过将变量表示为网络节点，能够揭示宇宙变量间的依赖结构和数据生成过程，特别适用于研究系外行星和宿主恒星。
...
### [43] [Probing dark matter interactions with a RES-NOVA prototype cryogenic detector](https://arxiv.org/abs/2601.16251)
...
**TL;DR**: 首次利用考古铅(archaeological Pb)生长的PbWO$_4$晶体作为低温量热器靶材，在地下环境中实现了对暗物质(dark matter)与中子自旋相关及自旋无关相互作用的排除极限测量，验证了RES-NOVA探测方案的技术可行性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用考古铅（archaeological Pb）生长的PbWO$_4$晶体作为低温量热器（cryogenic calorimeter）原型，验证其在暗物质（dark matter）探测中的可行性和低本底（low-background）运行能力。

Data: 数据来源于使用13克考古铅(Pb)生长的PbWO$_4$晶体作为低温量热器的地下实验原型机，通过锗(Ge)热敏电阻读取信号。

Method: 采用考古铅（archaeological Pb）生长的PbWO$_4$晶体作为低温量热计（cryogenic calorimeter），结合锗热敏电阻（Ge thermistor）读出技术，在暗物质（dark matter）搜寻中实现了低能量阈值和低背景噪声控制。

Result: 首次利用考古铅（archaeological Pb）生长的PbWO$_4$晶体作为目标材料，在低温量热计中实现了对暗物质（dark matter）自旋相关（spin-dependent）和自旋无关（spin-independent）相互作用的排除限设定。
...
### [44] [Mercury-Ar$χ$es: a high-performance n-body code for planet formation studies](https://arxiv.org/abs/2601.16791)
*Diego Turrini,Sergio Fonte,Romolo Politi,Danae Polychroni,Scigé J. Liu,Paolo Matteo Simonetti,Simona Pirani*

**TL;DR**: 开发了并行n体代码Mercury-Ar$χ$es，通过OpenMP并行化实现了对原行星盘（protoplanetary disk）中行星形成过程（包括行星生长、迁移、与盘气体相互作用及星子撞击通量）的高效模拟。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原行星盘（protoplanetary disk）中行星形成过程的动力学演化及其与盘气体的相互作用机制。

Data: 研究中使用的数据来源为数值模拟（simulation）数据，通过并行n体代码Mercury-Ar$χ$es进行高精度计算，该代码基于OpenMP指令的共享内存环境并行化实现，能够模拟行星形成过程中的多体动力学、气体盘相互作用以及星子撞击通量演化。

Method: 采用基于OpenMP指令的共享内存并行化高性能实现方法，结合改进的并行n体（n-body）算法，对行星形成过程进行多线程和向量化计算模拟。

Result: Mercury-Ar$χ$es是一个基于OpenMP指令级并行的高性能n体代码，能够模拟行星形成过程中的生长、迁移、与盘气体的相互作用以及星子撞击通量演化。
...
### [45] [The Origins of Planets for ArieL (OPAL) Key Science Project: the end-to-end planet formation campaign for the ESA space mission Ariel](https://arxiv.org/abs/2601.16841)
...
**TL;DR**: 通过建立包含数十种元素和数百种分子的系外行星大气合成光谱库(OPAL项目)，揭示了行星形成初始条件参数空间的高度简并性导致的大气成分多样性，为阿里尔(Ariel)任务奠定了建模基础。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于系外行星大气成分多样性及其与原生原行星盘（protoplanetary disk）和宿主恒星的遗传关联机制。

Data: data来源为ESA（European Space Agency）的Ariel空间任务观测数据及OPAL关键科学项目生成的真实合成大气（synthetic atmospheres）库。

Method: 采用高维参数空间下的跨学科建模方法（interdisciplinary modeling）结合高性能计算（high-performance computing）技术，通过追踪行星与原行星盘（protoplanetary disk）及宿主恒星的遗传关联（genetic link）构建合成大气光谱库。

Result: OPAL项目旨在通过高性能计算方法构建涵盖数十种元素和数百种分子的系外行星大气合成光谱库，以揭示行星形成初始条件参数空间的高度简并性导致的多样化大气组成结果。
...
### [46] [Post-Perihelion Integral Field Spectroscopy of the Interstellar Comet 3I/ATLAS](https://arxiv.org/abs/2601.16983)
...
**TL;DR**: 首次对星际天体3I/ATLAS进行近日点后蓝敏积分视场光谱观测，确认了CN、Fe、Ni及碳链分子C2、C3的释放，测得Fe、Ni产生率及径向分布特征，发现其活动特性与太阳系彗星一致且近日点后元素分布半径增大约6.5-7倍。


<details>
  <summary>Details</summary>

Background: 通过分析星际天体（interstellar objects）的彗星活动直接探测其他恒星系统的周围环境。

Data: 数据来源于2025年11月16日使用凯克宇宙网成像仪（Keck Cosmic Web Imager）对星际天体3I/ATLAS进行的后近日点蓝色敏感积分视场单元光谱观测。

Method: 采用凯克宇宙网成像仪（Keck Cosmic Web Imager）进行蓝敏感积分视场单元光谱（blue-sensitive integral-field unit spectroscopy）观测，并计算了CN、Fe、Ni、C₂和C₃等物种的产生率（production rates）及其径向分布特征。

Result: 首次在星际天体3I/ATLAS的近日点后观测中检测到碳链分子$\mathrm{C}_2$和$\mathrm{C}_3$，并测得Ni与Fe的产率比$\log(\mathrm{Q_{Ni}} / \mathrm{Q_{Fe}}) = -0.16\pm0.03$，与太阳系彗星一致。
...
### [47] [Searching for Ultralight Scalar Dark Matter with Clocks in Low Earth Orbit](https://arxiv.org/abs/2601.16259)
*Dawid Brzeminski,Aaron Pierce*

**TL;DR**: 研究发现超轻暗物质（ultralight dark matter）与标准模型的二次耦合会在地球附近形成密度调制，大气层屏蔽效应限制地面实验灵敏度，而基于国际空间站（International Space Station）的量子时钟可探测质量大于$10^{-9}$ eV的暗物质，其轨道调制信号和偶极分布特征（dipole feature）能增强实验灵敏度，光学时钟和核时钟可突破地面实验参数空间限制。


<details>
  <summary>Details</summary>

Background: 研究探讨了超轻暗物质（ultralight dark matter）在宏观物体附近密度变化的机制及其对标准模型（Standard Model）的二次耦合效应。

Data: data：研究使用国际空间站（International Space Station）上的量子钟（quantum clocks）和轨道核钟（orbiting nuclear clocks）观测数据，探测暗物质（dark matter）质量范围$m_{\rm DM}\gtrsim 10^{-9} \text{\, eV}$及$m_{\rm DM} \gtrsim 10^{-10}$ eV时的信号调制效应。

Method: 采用量子时钟（quantum clocks）探测暗物质背景引起的基本参数变化，并结合国际空间站（International Space Station）实验环境分析轨道平均场振幅和偶极特征（dipole feature）的时间调制效应。

Result: 量子钟（quantum clocks）在国际空间站上可有效探测质量大于$10^{-9}$ eV的超轻暗物质（ultralight dark matter），并约束其对基本参数的影响。
...
### [48] [Cosmic ray electron boosted light dark matter: Implications of LZ 2025 data](https://arxiv.org/abs/2601.16903)
*Sk Jeesun,Anirban Majumdar*

**TL;DR**: 通过分析LZ 2025（WS2024运行）最新数据，利用宇宙射线（cosmic rays）提升亚MeV电亲和暗物质（sub-MeV electrophilic dark matter）动能的方法，将MeV尺度暗物质（MeV scale dark matter）的探测限制比XENONnT先前结果提高了约一个数量级（$\sim\mathcal{O}(1)$），并在能量依赖截面（energy-dependent cross-sections）下通过中介粒子质量（mediator mass）关键作用排除了此前未探索的参数区域。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用宇宙射线（cosmic rays）提升非相对论性晕暗物质（non-relativistic halo dark matter）的动能，以探测轻暗物质（light dark matter）在直接探测实验中的信号。

Data: 数据来源为LZ 2025实验（WS2024运行）的最新观测数据。

Method: 采用宇宙射线(cosmic rays)加速机制结合LZ 2025实验数据，通过能量无关(energy-independent)和能量依赖(energy-dependent)两种截面模型分析亚MeV电亲和暗物质(electrophilic dark matter)，重点考察媒介粒子(mediator)质量对事件率的影响。

Result: 利用LZ 2025最新数据（WS2024运行）对宇宙线加速的亚MeV亲电暗物质（electrophilic dark matter）进行探测，在能量无关截面情况下将MeV尺度暗物质限制比先前XENONnT极限提高了约$\sim\mathcal{O}(1)$量级。
...
### [49] [Anisotropy Strikes Back: Modified Gravity and Dark Matter Halos](https://arxiv.org/abs/2601.16958)
*Paolo M Bassani*

**TL;DR**: 研究了球对称LTB（Lemaitre Tolman Bondi）迷你超空间中暗物质类流体（dark matter like fluids）在广义相对论（GR）和Horava Lifshitz（HL）引力下的对称性对有效暗源（effective dark sources）的调控机制，发现GR中势能形变导致各向异性应力能（anisotropic stress energy）贡献但无法产生平坦旋转曲线，而HL引力中形变狄拉克代数（Dirac algebra）使特定LTB背景产生符合无鬼条件（ghost-freedom）且红外回归GR的正比例暗物质密度。


<details>
  <summary>Details</summary>

Background: 研究探讨了广义相对论（GR）和Horava-Lifshitz（HL）引力中哈密顿约束对称性如何控制有效暗物质源的涌现，聚焦于球对称LTB（Lemaitre-Tolman-Bondi）迷你超空间内的类暗物质流体行为。

Data: 研究中使用了理论模拟（simulation）数据，基于Lemaitre Tolman Bondi（LTB）迷你超空间模型，在广义相对论（GR）和Horava Lifshitz（HL）引力框架下分析暗物质类流体的对称性特性。

Method: 采用哈密顿约束（Hamiltonian constraint）对称性分析和狄拉克代数（Dirac algebra）变形方法，在广义相对论（GR）和Horava Lifshitz（HL）引力理论中研究有效暗物质源的涌现机制。

Result: 在霍拉瓦-利夫希茨（Horava-Lifshitz）引力框架下，通过变形狄拉克代数（Dirac algebra）诱导出受控的非守恒定律，识别出一类受限的勒梅特-托尔曼-邦迪（LTB）背景，其源项产生符合无鬼条件且在红外极限恢复广义相对论的正标度暗物质密度。
...
### [50] [Analysis of $M1$ capture in the $α(d,γ)^6$Li reaction](https://arxiv.org/abs/2601.16551)
*Ergash M. Tursunov,Daniel Baye*

**TL;DR**: 研究发现有效算符（effective operator）与长波长$M1$算符在跃迁矩阵元中完全等效，揭示了$α(d,γ)^6$Li反应中$M1$贡献主要源于初始$S$波向$^6$Li基态同位旋1分量的跃迁，且忽略初始态畸变时同位旋标量（isoscalar）$M1$跃迁被禁戒。


<details>
  <summary>Details</summary>

Background: 研究聚焦于磁偶极矩（M1）算符在α(d,γ)^6Li反应中的等效性及其对长波长形式矩阵元的解析与数值分析。

Data: 数据来源于三体模型（three-body model）中质子、中子和无结构α粒子通过有效力（effective forces）相互作用的数值模拟。

Method: 采用有效算符（effective operator）方法分析$M1$跃迁矩阵元，并在三体模型（proton、neutron和无结构$α$粒子）中通过有效力（effective forces）进行数值计算。

Result: 在忽略初始态畸变时，同位旋标量（isoscalar）$M1$跃迁在辐射俘获反应中从初始$S$波出发是被禁戒的。
...
### [51] [A Binary-Based Reassessment of the Age and Stellar Properties of NGC 7789 Using Twelve Binary Components](https://arxiv.org/abs/2601.16265)
*Kadri Yakut,Belinda Kalomeni,Saul Rappaport*

**TL;DR**: 基于双星系统（SB2）的统一建模框架结合多种观测数据，精确测定NGC 7789年龄为1.26±0.09 Gyr，并验证了12颗成员星的物理参数与现代恒星演化模型高度一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用双星系统重新评估中等年龄疏散星团NGC 7789的年龄，并探索其在疏散星团研究中作为多观测手段基准的潜力。

Data: 数据来源包括径向速度轨道数据、TESS光变曲线数据、蓝外至红外光谱能量分布（SEDs）数据以及作为外部先验的Gaia DR3天体测量数据。

Method: 采用统一建模框架（unified modelling framework）结合视向速度轨道（radial-velocity orbits）、TESS光变曲线（TESS light curves）和蓝光到红外光谱能量分布（blue-to-IR SEDs）进行综合分析。

Result: 通过结合光谱能量分布(SED)、食双星和视向速度分析，得出NGC 7789的精确年龄为1.26±0.09 Gyr，消光值A_V=0.90±0.05 mag，与盖亚DR3距离2.06 kpc一致。
...
### [52] [Gaia20dsk: A new MNor discovered by the GLORIOUS collaboration](https://arxiv.org/abs/2601.16826)
...
**TL;DR**: 通过光学和近红外(NIR)光谱分析确认Gaia20dsk为具有MNor型特征的爆发式年轻恒星体(YSO)，其质量吸积率介于(0.5-1.8)*10^{-6}太阳质量/年，并观测到两次主要和一次短暂增亮事件（最大幅度约1.8星等）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于爆发式年轻恒星体(eruptive young stellar objects)的物理特性和吸积过程，特别是通过分析Gaia20dsk的光变曲线和光谱特征来确认其爆发式年轻恒星体的分类。

Data: 数据来源包括公开可用的光学和近红外(NIR)测光数据以及X-shooter光学/NIR光谱仪获取的光谱。

Method: 结合公开光学与近红外(NIR)测光数据及X-shooter光谱仪获取的光学/NIR光谱，通过分析爆发期光变曲线、多波段色指数图、发射线特征并估算吸积率。

Result: Gaia20dsk是一个爆发式年轻恒星天体（YSO），具有与MNors型天体相似的光变特征（包括亮度增幅和爆发持续时间）、光谱特征及吸积率。
