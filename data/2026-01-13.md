...
**TL;DR**: 提出了一种基于固定尺度滤波器的真实空间分析方法，用于区分星系团内介质(ICM)中大尺度体运动和湍流，发现大质量星系团并合期间160千秒差距以下尺度的湍流压力占比峰值仅5%，且该方法比流行的多尺度迭代滤波法更可靠。


<details>
  <summary>Details</summary>

Background: 研究主题为星系团内介质(ICM)中湍流与整体运动的分离方法及其在宇宙学模拟与观测中的应用。

Data: 数据来源于PICO-Clusters套件中的星系团合并模拟，该模拟使用移动网格代码Arepo和IllustrisTNG星系形成模型进行。

Method: 采用真实空间滤波算子分离星系团内介质（ICM）中不同尺度的体运动（bulk motion）与湍流，并定义体分量与湍流分量的一致动能和磁能。

Result: 在星系团（galaxy cluster）并合过程中，物理尺度≲160千秒差距（kpc）内的湍流压力占比在核心穿越时达到5%峰值，随后在约13亿年（Gyr）后降至2%，与XRISM观测结果一致，表明除非近期经历大并合，星系团内湍流水平通常较低。
...
**TL;DR**: 研究发现高红移（z~3-9）大质量恒星形成星系（log10(M⋆/M⊙)=9.5-11）中存在电离外流（ionized outflows），其气体具有更高尘埃消光（AV~0.59 mag）和金属丰度（+0.13 dex），但速度不足以逃逸星系，主要起金属和尘埃再分布作用，且外流速度（vout=170-600 km/s）与恒星形成率显著相关，质量加载因子（η≤1）表明其对恒星形成的抑制效率较低。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙早期（红移z~3-9）大质量恒星形成星系中电离气体外流（ionized outflows）的搜寻与表征，填补了以往对高红移低质量星系外流研究的空白。

Data: 数据来源于JWST/NIRSpec IFU观测的15个无活动星系核(AGN)证据的恒星形成系统（红移z~3-9），属于GA-NIFS项目，从中分离出40个星系/区域样本。

Method: 采用近红外积分场光谱仪（NIRSpec IFU）观测[OIII]5007和Hα发射线，通过识别宽运动学成分分析电离外流（ionized outflows）的动力学特性。

Result: 在宇宙最初20亿年内，大质量、高光度恒星形成星系中电离外流(ejective feedback)的反馈效率较低。
...
**TL;DR**: 研究发现异常微波辐射(AME)的频谱特征主要由尘埃颗粒尺寸、形状和相态依赖(phase-dependent)环境参数决定，分子云(MC)观测结果与模型完全吻合，暗云(DC)存在轻微偏差，而HII区在峰值频率上表现出显著偏移，表明需要同时考虑颗粒尺寸和环境参数的集合变化才能解释观测到的频谱展宽。


<details>
  <summary>Details</summary>

Background: 异常微波辐射（AME）的观测光谱特征与旋转尘埃模型不符，促使研究者深入探讨尘埃颗粒分布和环境参数如何影响AME光谱能量分布（SED）的峰值频率和谱宽。

Data: 研究中使用的数据来源包括蒙特卡洛采样（Monte Carlo sampling）生成的模拟数据集和全球敏感性分析（global sensitivity analysis）所依赖的尘埃颗粒分布参数（grain size, shape）及环境参数（phase-dependent environmental parameter）。

Method: 采用蒙特卡洛采样(Monte Carlo sampling)和全局敏感性分析(global sensitivity analysis)研究尘埃颗粒分布与环境参数对异常微波辐射(AME)谱能量分布(SED)峰值频率和谱宽的影响。

Result: result：异常微波辐射（AME）谱特征主要受颗粒尺寸、形状和相依赖环境参数控制，分子云（MC）与观测完全吻合而HII区存在显著峰值频率偏移，需同时考虑颗粒尺寸和环境参数的集合变化才能重现观测到的峰值频率和谱宽分布。
...
**TL;DR**: 通过多组宇宙学模拟（CAMELS）比较发现，卫星星系的气体比例和恒星形成率（SFR）在高密度环境中显著受抑制，而中心星系的环境敏感性随质量变化（低质量时受抑制、高质量时增强），且不同模型（SIMBA、Swift-EAGLE等）对晕内重子比例（f_B）和环星系介质质量比例（f_CGM）的环境响应呈现相反趋势，表明环境影响依赖于星系形成模型、质量及红移。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系演化中环境效应（environmental effects）与重子物理（baryonic physics）模型依赖性的相互作用。

Data: 研究使用了来自Cosmology and Astrophysics with MachinE Learning Simulations (CAMELS)的多套模拟数据，涵盖SIMBA、IllustrisTNG、ASTRID和Swift-EAGLE星系形成模型中不同恒星与活动星系核(AGN)反馈机制的实现方案。

Method: 采用宇宙学和天体物理学机器学习模拟（CAMELS）框架，结合SIMBA、IllustrisTNG、ASTRID和Swift-EAGLE等星系形成模型，分析不同重子物理实现（baryonic physics implementation）下环境对星系演化的影响。

Result: 卫星星系在所有模拟模型中均显著受环境影响，其气体分数和恒星形成率在高密度区域相比低密度环境下的同质量卫星星系更低。
...
**TL;DR**: 研究分析了COSMOS-Web中25个红移(z)为0.18-3.65的富星系群（通过AMICO算法识别），发现星系群环境是恒星形成抑制先于形态转变的过渡阶段，其中大质量星系（M_star > 10^10.5 M_sun）经历快速淬灭（约1 Gyr）并主要转变为椭圆型星系（ETGs），而中低质量星系受环境影响呈现缓慢淬灭或保持恒星形成，且活动星系核（AGN）反馈可能驱动大质量星系的快速淬灭。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系群（galaxy groups）环境中星系演化过程，特别是恒星形成抑制（quenching）与形态转变的时序关系及其与质量、环境因素的关联。

Data: 数据来源于COSMOS-Web巡天中通过AMICO算法识别的25个最富星系群（z = 0.18-3.65），包含20-30个成员概率高于75%的星系。

Method: 采用AMICO算法识别COSMOS-Web中红移z=0.18-3.65的25个最富星系群，分析其被动-密度与活动-密度关系，并基于成员概率>75%的20-30个星系样本，通过红序(RS)定位和形态-质量-环境关联研究星系演化动力学。

Result: 星系群环境代表了一个过渡阶段，其中恒星形成停止（quenching）先于形态转变发生，这与富星系团中的经典形态-密度关系形成对比。
...
**TL;DR**: 通过模拟云-云碰撞（cloud-cloud collisions）中恒星形成的全过程，发现碰撞速度、磁场取向（magnetic field orientation）和辐射反馈（radiative feedback）显著影响恒星形成的速率、形态及质量分布，但最终恒星形成效率（SFE）保持稳定（10-15%），且恒星质量函数（stellar mass function）对碰撞速度不敏感。


<details>
  <summary>Details</summary>

Background: 研究探讨了云-云碰撞（cloud-cloud collisions）如何通过速度、磁场取向和辐射反馈（radiative feedback）影响恒星形成过程。

Data: 使用首次模拟单个恒星形成和吸积过程并包含所有恒星反馈机制的云-云碰撞数值模拟数据。

Method: 采用三维磁流体力学(MHD)模拟结合恒星形成与吸积的个体建模，整合辐射压、电离反馈等所有恒星反馈机制，分析不同碰撞速度、磁场取向条件下的形态演化、恒星形成效率(SFE)及碎片化特征。

Result: 云碰撞显著提升了恒星形成速率和时机，但最终恒星形成效率(SFE)在不同条件下保持相似。
...
**TL;DR**: 研究了旋转多方气流（rotating polytropic flows）在宽参数空间内的吸积特性，发现邦迪半径（Bondi radius）大小决定了气体角动量（gas angular momentum）对吸积率（mass accretion rate）影响的显著程度（小邦迪半径时陡降、大邦迪半径时缓降），并揭示外流（outflows）会大幅减少黑洞实际吸积质量但弱化吸积率对角动量的依赖。


<details>
  <summary>Details</summary>

Background: 研究探讨了吸积流（accretion flow）性质与气体角动量（angular momentum）的关联及其对黑洞增长和能量输出的影响。

Data: 数据来源包括基于细盘近似（slim disk approximation）和适用于低高角动量流（low- and high-angular-momentum flows）的黏度描述的旋转多方流（rotating polytropic flows）全局解，以及采用ADIOS模型（ADIOS model）研究外流（outflows）的全局解。

Method: 采用全局解法结合旋转多方流模型，在细盘近似（slim disk approximation）框架下分析不同邦迪半径（Bondi radius）和粘滞参数α对吸积率的影响，并引入ADIOS模型（ADIOS model）研究外流效应。

Result: 质量吸积率（mass accretion rate）随气体角动量（gas angular momentum）增加的下降程度取决于邦迪半径（Bondi radius），小邦迪半径时下降陡峭而大邦迪半径时下降平缓。
...
**TL;DR**: 在红移(z=0.67)的大质量孤立螺旋星系中首次探测到延伸紫外(XUV)盘，其FUV辐射范围达光学半径两倍并包含低表面亮度(LSB)区域和紫外亮团块，表明存在冷气体吸积和近期恒星形成，支持中间红移星系盘生长理论。


<details>
  <summary>Details</summary>

Background: 研究主题为利用深紫外成像探测中红移(z=0.67)大质量孤立螺旋星系的延展紫外辐射(XUV)盘，验证星系由内向外的生长机制及外围低效恒星形成活动。

Data: 数据来源于AstroSat/UVIT对红移z=0.67的大质量孤立螺旋星系进行的深场紫外成像观测，并基于仪器点扩散函数(PSF)校正后的本征静止坐标系FUV表面亮度剖面进行分析。

Method: 采用AstroSat/UVIT进行深紫外成像观测，结合点扩散函数(PSF)校正和表面亮度剖面分析，检测并分类了XUV盘（Type I和Type II）的紫外亮星团及低表面亮度(LSB)区域。

Result: 在z=0.67的大质量孤立螺旋星系中首次探测到延展紫外（XUV）盘，其FUV亮度分布比光学和红外波段更延展，符合II型XUV定义，并存在无光学对应体的紫外团块（I型特征），表明外盘存在引力不稳定性和近期恒星形成，支持冷气体吸积驱动盘增长的观点。
...
**TL;DR**: 通过巴特沃斯空间滤波(Butterworth spatial filtering)和理查森-露西反卷积(Richardson-Lucy deconvolution)处理NGC 3115和NGC 4699星系的数据立方体(data cube)，使中心超大质量黑洞(supermassive black hole)质量参数的不确定性分别降低41%和45%，显著提升了恒星和气体动力学分析结果的质量。


<details>
  <summary>Details</summary>

Background: 数据立方体（data cube）在天文学中的应用日益广泛，但常受仪器效应和高频噪声影响。

Data: 使用了双子座多目标光谱仪(Gemini Multi-Object Spectrograph)积分视场单元(Integral Field Unit)获取的NGC 3115和NGC 4699星系中心区域的数据立方体(data cube)。

Method: 采用Butterworth空间滤波和Richardson-Lucy反卷积方法处理数据立方体，结合Jeans各向异性模型（Jeans Anisotropic Models）进行动力学建模。

Result: 提出的数据处理方法（Butterworth空间滤波和Richardson-Lucy反卷积）显著降低了超大质量黑洞（supermassive black hole）质量参数的不确定性，在NGC 3115和NGC 4699中分别减少了41%和45%。
...
**TL;DR**: 该论文提出了一种创建光学-氢原子(HI)联合模拟目录的新方法，用于即将开展的SKA前驱巡天项目（MIGHTEE-HI、LADUMA和WALLABY），通过结合光学特性和HI数据预测巡天结果，并展示了利用叠加观测约束低质量星系HI质量函数(HIMF)的应用。


<details>
  <summary>Details</summary>

Background: 原子氢（HI）作为冷气体调控恒星形成，是涉及吸积、反馈、外流和气体循环的重子物质循环（baryon cycle）中的关键物质相。

Data: 数据来源为即将开展的SKA前驱巡天项目MIGHTEE-HI和LADUMA（使用MeerKAT望远镜）以及WALLABY（使用ASKAP望远镜）的射电观测数据。

Method: 利用红移21厘米谱线发射（redshifted 21 cm line emission）作为示踪剂，结合光学-HI联合模拟星表（optical-HI joint mock catalogs）技术，为MIGHTEE-HI、LADUMA和WALLABY巡天预测HI质量函数（HIMF）及光学-HI标度关系（optical-to-HI scaling relations）。

Result: 中性氢（HI）通过作为冷气体燃料调节恒星形成，是涉及吸积、反馈、外流和气体循环的重子物质循环中的关键相态。
...
**TL;DR**: 基于2.5米艾萨克·牛顿望远镜(INT)宽视场相机(WFC)的测光时序数据，在仙女座星系(Andromeda galaxy)17个矮卫星星系中发现了约2800颗长周期变星(LPV)候选体，并发布了包含完整恒星种群和LPV候选体的公开星表，更新了结构参数（包括半光半径）并利用红巨星分支顶端(TRGB)方法测定了距离模数。


<details>
  <summary>Details</summary>

Background: 研究聚焦于仙女座星系（Andromeda galaxy）矮椭球卫星星系中长周期变星（LPV）的普查及其对矮星系恒星演化、质量损失和尘埃产生的影响。

Data: 数据来源于2.5米艾萨克牛顿望远镜（INT）上的广角相机（WFC）获取的光度时间序列数据。

Method: 基于宽视场相机(WFC)的光度时间序列数据，采用索贝尔滤波(Sobel-filter)边缘检测的红巨星分支顶端(TRGB)方法测定距离模数。

Result: 在仙女座星系（Andromeda）的17个矮卫星星系中发现了约2800颗长周期变星（LPV）候选体，覆盖了广泛的光度和变幅范围。
...
**TL;DR**: 基于JWST中分辨率光谱对红移$z \sim 3$典型恒星形成星系（莱曼断裂星系和莱曼α发射体）的电离气体丰度测量，发现氮氧比（N/O）在12+log(O/H)=7.5-8.44金属丰度范围内与低红移星系无显著差异，表明典型星系的氮氧比在宇宙时间尺度上未发生演化。


<details>
  <summary>Details</summary>

Background: 研究主题为利用JWST中分辨率光谱分析高红移（$z \sim 3$）典型恒星形成星系（莱曼断裂星系和莱曼α发射体）的电离气体氮（N）、氧（O）和氖（Ne）丰度，并探究其氮氧比（N/O）随金属丰度及宇宙时间演化的规律。

Data: 数据来源于JWST（James Webb Space Telescope）中分辨率光谱，取自LyC22巡天项目中典型恒星形成星系（莱曼断裂星系和莱曼α发射体）样本，红移z~3。

Method: 基于极光发射线[OIII]4363的直接测量法(direct method)测定25个典型恒星形成星系(莱曼断裂星系和莱曼α发射体)的电离气体元素丰度(N、O、Ne相对于H)。

Result: 在典型恒星形成星系中，氮氧比（N/O）在红移z~2-4与低红移星系及HII区相同金属丰度范围内无显著演化，平均值为log(N/O)=-1.29^{+0.25}_{-0.21}。
...
**TL;DR**: 基于盖亚(Gaia) DR3数据对疏散星团NGC 752应用分层聚类算法，发现其存在四个对应不同解体阶段的子结构，揭示了质量分层现象、外缘成员以0.25 km/s速度膨胀、约0.03 rad/Myr的投影旋转模式，并证实银河系潮汐力通过盘面穿越加速星团瓦解。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系疏散星团（open clusters）的结构演化及其动力学特征，重点探讨质量分层（mass segregation）和潮汐力（tidal forces）对星团解体的影响。

Data: 数据来源为盖亚(Gaia)卫星第三次数据释放(Gaia DR3)。

Method: 基于运动学信息采用层次聚类算法（hierarchical clustering algorithm）分析NGC 752疏散星团内部结构。

Result: 基于运动学信息应用层次聚类算法（hierarchical clustering algorithm）在NGC 752中识别出四个对应于不同解体阶段的子结构。
...
**TL;DR**: 首次在Dome~A成功观测两个典型触发大质量恒星形成区的CO(4-3)和[CI](^3P_1-^3P_0)谱线，结合存档[CII]数据完整表征了碳的三种相态，发现高消光区存在由大质量恒星强辐射场穿透团块状星际介质(ISM)导致升高的C^0/CO丰度比。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际介质（ISM）中碳元素在电离态、原子态和分子态之间的循环过程及其对化学组成和物理条件的影响。

Data: 数据来源于南极太赫兹探测器（Antarctic Terahertz Explorer）60厘米望远镜对Dome~A两个典型触发大质量恒星形成区的CO（4-3）和[CI]（^3P_1 - ^3P_0）首次成功测绘观测，并结合存档的[CII]数据。

Method: 采用南极太赫兹探测器（ATE）进行CO（4-3）和[CI]（3P1-3P0）的亚毫米波段成图观测，并结合存档的[CII]数据，完整表征了碳的三种相态（离子态、原子态和分子态）。

Result: 首次在Dome~A成功观测到两个典型触发大质量恒星形成区的CO(4-3)和[CI](3P1-3P0)谱线分布，并结合存档[CII]数据首次完整表征了这些环境中碳元素的三种相态。
...
**TL;DR**: 通过多波段射电数据分析首次确认超新星遗迹G321.3-3.9具有椭圆壳层形态(1.3度×1.7度)和平坦的非热谱指数(α=-0.40±0.03)，并结合Σ-D关系和HI结构提出了2.5-3.3千秒差距的近距离距离解。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超新星遗迹（supernova remnant）G321.3-3.9的多波段射电（radio）特性及其距离估算。

Data: 数据来源于存档的多波段射电观测数据（88-2304 MHz）。

Method: 采用射电（radio）多波段（88-2304 MHz）数据分析和Sigma-D关系（Sigma-D relation）进行距离估算。

Result: 超新星遗迹G321.3-3.9呈现出椭圆壳状形态（1.3度×1.7度）且具有较平坦的非热谱指数（alpha = -0.40 ± 0.03）。
...
**TL;DR**: 基于盖亚(Gaia) DR3数据开发算法，首次测定了33个邻近年轻移动星群(NYMGs)的初始质量函数(IMF)，发现其参数化结果($m_c=0.25\pm0.17~M_{\odot}$, $σ_c=0.45\pm0.17$, $α=-2.26\pm0.09$)与太阳邻域及年轻恒星关联的IMF一致，证实NYMGs是分散星团遗迹且在$0.1<m/M_{\odot}<10$范围内无系统偏差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系盘附近年轻移动星群(NYMGs)的初始质量函数(IMF)，探讨其作为分散星团或星协遗迹的恒星群体特征。

Data: 数据来源为盖亚卫星第三次数据释放（Gaia DR3）的光度测量（photometry）和天体测量（astrometry）数据。

Method: 基于盖亚(Gaia) DR3的光度测量(photometry)和天体测量(astrometry)数据开发了运动学空间检测算法，并采用对数正态分布(log-normal)描述低质量段($m<1~M_\odot$)和幂律(power-law)描述高质量段($m>1~M_\odot$)的参数化方法。

Result: 基于盖亚(Gaia) DR3数据，对33个邻近年轻移动星群(NYMGs)的初始质量函数(IMF)进行了参数化，发现其低质量段($m<1~M_{\odot}$)符合对数正态分布($m_c=0.25\pm0.17~M_{\odot}$, $σ_c=0.45\pm0.17$)，高质量段($m>1~M_{\odot}$)符合幂律分布($α=-2.26\pm0.09$)，与太阳邻域及年轻星协的IMF参数化结果一致。
...
**TL;DR**: 通过耦合三维辐射流体动力学模拟和辐射传输计算发现，NGC 3783的Brγ谱线源自旋转薄盘表面的电离气体，其观测轮廓由周围弥散电离气体中的电子散射显著展宽和平滑。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核(AGN)中宽线区(BLR)的起源、结构和动力学特征，特别是通过三维辐射流体力学(RHD)模拟结合辐射传输计算来揭示其物理机制。

Data: 数据来源为VLTI/GRAVITY对NGC 3783的红外干涉观测数据以及SINFONI获取的Br$γ$谱线轮廓。

Method: 采用三维辐射流体力学（RHD）模拟结合Cloudy辐射传输计算，并与NGC 3783的SINFONI Br$γ$谱线轮廓进行对比分析。

Result: Br$γ$谱线源自旋转薄盘表面的电离气体区域，电子温度约$10^4$ K，数密度为$10^8-10^{11}$ cm$^{-3}$，但辐射流体动力学模拟产生的本征谱线轮廓比观测更窄且存在子结构。
...
**TL;DR**: 提出了一种基于集成机器学习（ML）的光度红移（Pz）预测框架，通过整合多种算法显著提高了对暗弱星系和高红移（z ~ 4）范围的光度红移估计精度，性能优于单独模型并满足LSST科学要求文档的基准。


<details>
  <summary>Details</summary>

Background: 现代先进望远镜系统（如SDSS、Euclid和LSST）的发展推动了宇宙学模型的精细化研究，为解决天体物理学中关于宇宙演化的关键挑战提供了新机遇。

Data: 数据来源于斯巴鲁望远镜的Hyper Suprime-Cam战略巡天计划(Hyper Suprime-Cam Strategic Survey Program)公开的光学(grizy)测光数据。

Method: 采用集成机器学习框架（包括梯度提升机、极端梯度提升、k近邻算法和人工神经网络）结合袋装输入数据，基于光学测光数据（grizy波段）预测光度红移（Pz）。

Result: 提出了一种基于集成机器学习（ML）的新框架，仅利用光学（grizy）测光数据即可准确预测暗弱星系和高红移（z ~ 4）范围的光度红移（Pz），其性能优于独立模型并达到或超越LSST科学需求文档的基准。
...
**TL;DR**: 利用WISE（广域红外巡天探测器）数据对2MRS（2MASS红移巡天）星系的中红外星系尺度（戴森式）废热进行搜索，通过W3/W4测光数据设定保守的3σ上限，发现邻近星系中废热光度中值上限为(5-9)×10^8 L_sun（150-600K温度范围），且在300K温度下不超过0.0161%的星系能存在将≥21%银河系恒星光度转化为废热的KIII级文明系统。


<details>
  <summary>Details</summary>

Background: 研究主题为利用WISE(Wide-field Infrared Survey Explorer)中红外数据探测星系尺度(Dysonian)废热信号，以评估地外技术文明存在的可能性。

Data: 数据来源于2MASS红移巡天(2MRS)与CatWISE2020、AllWISE的交叉匹配，并利用WISE的W1-W4波段进行中红外(AGN/starburst)筛选和恒星基线处理。

Method: 采用WISE (Wide-field Infrared Survey Explorer)中波红外数据，通过交叉匹配2MRS (2MASS Redshift Survey)、CatWISE2020和AllWISE目录，结合标准中红外活动星系核(AGN)/星暴剔除方法，利用W1/W2波段作为恒星基线、W3/W4波段作为约束带，基于黑体废热温度模型(T=150-600K)将W3/W4测光数据转换为保守的3σ每星系废热光度上限，并应用WISE波段响应(RSR)色校正进行量化分析。

Result: result：在150-600 K温度范围内，附近星系的中值废热光度上限为~(5-9) x 10^8 L_sun，典型废热占比alpha ≤ 1.7-2.9%（如300 K时≤1.8%），且≤0.0161%的星系可能拥有将≥21%类银河系恒星光度转化为~300 K废热的KIII级系统。
...
**TL;DR**: 通过扩展HACC代码并引入高精度仿真器框架，发现AGN（活动星系核）动能反馈存在低反馈模式（与X射线星系团径向气体密度剖面吻合）和高反馈模式（更匹配星系团气体分数但低估内区密度）两种机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙大尺度结构形成中重力和重子物理(baryonic physics)的耦合作用，特别是在兆秒差距(Mpc)尺度上重子过程对结构演化的关键影响。

Data: 数据来源于64个边长为128h⁻¹Mpc和16个边长为256h⁻¹Mpc的模拟盒子，分别包含2×512³和2×1024³粒子。

Method: 采用硬件/混合加速宇宙学代码(HACC)结合平滑粒子流体动力学(SPH)方法，并整合了化学增丰、黑洞与恒星形成、活动星系核(AGN)动力学与热反馈、超新星反馈、星系风及金属线冷却等亚分辨率模型。

Result: 通过高保真仿真器分析发现，AGN（活动星系核）动力学反馈存在两种模式：低反馈模式与X射线星系团径向气体密度观测数据高度吻合，而高反馈模式更符合星系团气体分数数据但低估了内部区域气体密度。
...
**TL;DR**: 在银河系中心分子云G+0.693-0.027中首次探测到乳醛（lactaldehyde）和甲氧基乙醛（methoxyacetaldehyde），并确认了乙酸甲酯（methyl acetate）和羟基丙酮（hydroxyacetone）的存在，同时发现所有C$_3$H$_6$O$_2$异构体可能通过尘埃表面自由基反应由CO氢化形成。


<details>
  <summary>Details</summary>

Background: 研究主题为银河系中心分子云G+0.693-0.027中C₃H₆O₂异构体的星际探测及其形成机制。

Data: 数据来源于使用Yebes 40米和IRAM 30米望远镜对G+0.693-0.027进行的超深宽带光谱巡天观测。

Method: 采用耶贝斯40米和IRAM 30米望远镜对G+0.693-0.027进行了超深宽带光谱巡天观测。

Result: 首次在星际介质中检测到乳醛（lactaldehyde）和甲氧基乙醛（methoxyacetaldehyde），并确认了乙酸甲酯（methyl acetate）和羟基丙酮（hydroxyacetone）的存在，同时新检测到甲酸乙酯（ethyl formate）的反式（anti）和顺式（gauche）构象异构体。
...
**TL;DR**: 论文发现高红移(z~15)变源Capotauro的光度变化特征更符合大质量(250-260M⊙)零金属丰度恒星产生的对不稳定超新星(PISN)模型，而非原初星系，为研究第三星族星(Population III stars)提供了新线索。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（James Webb Space Telescope）观测到的超高红移（$z > 15$）异常明亮天体挑战早期星系形成模型，并探讨其可能源自第三星族（Population III）恒星产生的对不稳定性超新星（PISN）的假设。

Data: 数据来源于JWST（James Webb Space Telescope）的CEERS巡天观测数据，包括F356W-dropout源的测光数据、$\sim 800$天时间基线的光变监测、以及NIRSpec光谱数据，同时结合了最新的PISN（pair-instability supernova）理论光变曲线、光谱能量分布和合成光谱进行比对分析。

Method: 采用最先进的配对不稳定性超新星(PISN)光变曲线、光谱能量分布和合成光谱进行建模分析。

Result: Capotauro的观测特征（亮度、光变、测光及光谱）与红移约15的配对不稳定性超新星（PISN）模型高度吻合，可能源自一颗大质量（250-260倍太阳质量）无金属恒星。
...
**TL;DR**: 该研究通过高分辨率观测发现G28.37+0.07红外暗云(IRDC)中东西向会聚气流(质量流率约10^-3M_sun/yr)主导纵向运动，其速率是视线方向吸积流的25倍，揭示了会聚气流界面处定向流动向多向吸积运动的转化机制。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于恒星形成区G28.37+0.07（一个典型的红外暗云）在汇聚气体流界面处的动力学特性及其对恒星形成过程的影响。

Data: 数据来源为北延毫米波阵列（NOEMA）和IRAM30m天文台的单天线数据，覆盖3毫米波段的光谱线和连续谱发射。

Method: 结合北方扩展毫米波阵列(NOEMA)和IRAM30m单天线望远镜数据，在3毫米波段通过谱线及连续谱观测，以高角分辨率(7.0''x2.6'')绘制了大尺度(~81pc²)至核心尺度的气体分布。

Result: 在汇聚气体流界面处，原本定向的气体流可转化为多向的塌缩运动，活跃恒星形成主要发生在这些界面区域。
...
**TL;DR**: 提出了一种基于掩膜引导的去噪扩散概率模型（DDPM），通过无条件生成方法在FIRST、Radio Galaxies Zoo和MGCLS目录数据上训练，能生成形态学（morphologically）逼真的射电星系（radio galaxy）图像，为补充射电数据集和开发机器学习应用提供了数据驱动方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用去噪扩散概率模型（DDPM）生成和修复形态学真实的射电星系（radio galaxy）图像，以补充现有射电数据集并支持射电天文学中的机器学习应用发展。

Data: 数据来源于FIRST射电星系目录（FIRST radio galaxy catalog）、射电星系动物园（Radio Galaxies Zoo）和MGCLS目录切图（cutouts of the MGCLS catalog）。

Method: 采用掩模引导的去噪扩散概率模型（DDPM）生成和修复射电星系图像，并通过比较不同统计分布确保无条件方法产生形态真实的星系。

Result: 采用掩码引导的去噪扩散概率模型（DDPM）生成并修复了形态学上真实的射电星系图像，为补充现有射电数据集和支持机器学习应用提供了数据驱动方法。
...
**TL;DR**: 通过结合流体动力学模拟和观测数据，利用星系标度关系（galaxy scaling relations）约束了宇宙学参数（$Ω_m$、$σ_8$）和超新星反馈强度，并发现温暗物质（WDM）粒子质量在低恒星质量（log10(M_*/Msun) <~ 8）下对暗物质质量和总质量-恒星质量比有微弱影响，同时星系恒星质量函数在相同质量范围内对WDM粒子质量表现出可分离于反馈的依赖性。


<details>
  <summary>Details</summary>

Background: 研究探索了温暗物质（WDM）作为标准Λ冷暗物质（ΛCDM）范式中小尺度差异的替代模型，旨在通过星系尺度关系联合限制宇宙学、天体物理及WDM参数。

Data: 数据来源包括DREAMS项目的大体积均匀盒模拟（large-volume uniform-box simulations）和高分辨率银河系放大模拟（high-resolution Milky Way zoom-in runs），以及观测数据SPARC近邻螺旋星系目录和LVDB本地体积矮星系目录。

Method: 采用联合分析流体动力学模拟（hydrodynamic simulations）与观测数据的方法，结合大体积均匀盒模拟（uniform-box simulations）和高分辨率银河系放大模拟（zoom-in runs），并应用分辨率效应校准以确保不同模拟集间的一致性。

Result: 通过结合流体动力学模拟和观测数据的联合分析，发现暖暗物质(WDM)粒子质量虽未被直接约束，但高分辨率缩进模拟揭示了低恒星质量下暗物质质量和总质量-恒星质量比的细微WDM诱导趋势，且星系恒星质量函数在log10(M_*/Msun) <~ 8时表现出可测量的WDM粒子质量依赖性，这一效应与反馈影响可分离，表明其可作为补充探测手段。
...
**TL;DR**: 通过拟合多个球状星团（globular clusters）的测光数据并更新关键参数，研究发现这些星团的水平分支形态差异主要由金属丰度（metallicity）、年龄、质量损失效率及演化过程中低质量成员的流失共同决定，且多数星团表现出较高的质量损失效率（η>0.3）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系球状星团（Galactic globular clusters）的关键参数测定及其水平分支形态差异的成因分析。

Data: 数据来源包括哈勃太空望远镜（HST）、盖亚卫星（Gaia）、斯凯马特南方巡天（SMSS）、Stetson的大规模汇编数据集以及其他来源的多波段观测数据。

Method: 采用DSED和BaSTI等时线(isochrone)拟合多种颜色-星等图(CMD)，并结合盖亚(Gaia)和哈勃(HST)的天体测量数据精确筛选星团成员。

Result: 通过分析多个球状星团(Globular Clusters)的色-星等图(CMDs)并拟合等龄线，确定了NGC288、NGC362、NGC1904、NGC4372、NGC5904、NGC6205和NGC6218的金属丰度([Fe/H])、年龄、距离、红化值(E(B-V))、消光值(Av)和消光-红化比(Rv)等关键参数。
...
### [27] [Galaxy Mergers in UNIONS -- II: Predicting Timescales in the Post-Merger Regime](https://arxiv.org/abs/2601.07604)
*Leonardo Ferreira,Sara L. Ellison,David R. Patton,Shoshannah Byrne-Mamahit,Scott Wilkinson,Robert W. Bickley*

**TL;DR**: 研究扩展了MUlti-Model Merger Identifier（Mummi）框架，利用基于IllustrisTNG模拟的机器学习模型，将并合后星系分类到四个时间区间（0-1.76 Gyr），准确率超70%，并在UNIONS巡天中识别出8,716个高质量并合后星系样本，为研究并合后物理过程与时间尺度关联提供了新方法。


<details>
  <summary>Details</summary>

Background: 星系并合（galaxy mergers）通过触发恒星形成增强、淬灭（quenching）和活动星系核（AGN）活动等过程显著影响星系演化，但精确约束并合后阶段（post-merger phase）这些过程发生的时间尺度仍具挑战性。

Data: 数据来源包括基于IllustrisTNG模拟生成的现实增强模拟观测数据（realism-enhanced mock observations）和紫外近红外光学北方巡天（Ultraviolet Near Infrared Optical Northern Survey, UNIONS）的观测数据。

Method: 基于机器学习模型分析来自IllustrisTNG模拟的真实性增强模拟观测数据，扩展了MUlti-Model Merger Identifier（Mummi）框架以预测并后时间尺度（T_PM）。

Result: 基于机器学习增强的MUlti-Model Merger Identifier (Mummi)框架，对合并后星系实现了超过70%的时间分类准确率，将后并合时标(T_{PM})划分为四个时间区间（0至1.76 Gyr）。
...
### [28] [Core Scouring Dynamics and Gravitational Wave Consequences: Constraints on Supermassive Black Hole Binary Hardening](https://arxiv.org/abs/2601.07762)
*C. J. Harris,Kayhan Gültekin,Laura Blecha*

**TL;DR**: 通过多信使研究（multi-messenger）发现，恒星散射（stellar scattering）对超大质量黑洞双星（supermassive black hole binaries）的硬化效率需比N体实验预测值高1.6倍才能匹配观测数据，且仅靠恒星散射不足以解释引力波背景（gravitational wave background）低频转折现象，表明气体动力学（gas-dynamics）在硬化过程中起关键作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超大质量黑洞双星（supermassive black hole binaries）通过恒星散射（stellar scattering）机制实现轨道硬化的效率问题，并探讨其与星系核区恒星质量缺失及纳赫兹引力波观测的关联性。

Data: 数据来源于宇宙学模拟套件IllustrisTNG的合并树（merger trees）以及观测到的星系恒星核心质量亏损（core mass deficit）与星系恒星质量的关系。

Method: 采用基于IllustrisTNG宇宙学模拟的合并树（merger trees）方法，结合动力学摩擦（dynamical friction）、恒星散射（stellar scattering）和引力波辐射（gravitational wave emission）的物理模型，预测星系核区恒星质量亏损（stellar mass deficits）。

Result: 恒星散射机制对超大质量黑洞双星(SMBHB)的硬化效率比N体实验预测值高约1.6倍才能匹配观测数据。
...
### [29] [Testing subhalo abundance matching with galaxy kinematics](https://arxiv.org/abs/2601.07799)
*Fedir Boreiko,Tariq Yasin,Harry Desmond,Richard Stiskalek,Matt J. Jarvis*

**TL;DR**: 通过贝叶斯前向模型结合ΛCDM暗物质晕预测与SPARC星系观测数据，发现SPARC星系倾向于占据固定维里质量下低最大旋转速度(Vmax)的暗晕（仅占16%），且需引入极端选择阈值才能缓解与独立成团性约束的张力，暗示当前数据可能存在矛盾。


<details>
  <summary>Details</summary>

Background: 研究探讨了盘星系(disk galaxies)旋转速度与暗物质晕(dark matter halo)结构的关系，旨在通过运动学观测约束星系-晕连接(galaxy-halo connection)理论。

Data: 数据来源于盘星系(SPARC)样本的最大旋转速度观测结果和ΛCDM模型预测的暗物质晕(Dark Matter Halo)群体。

Method: 采用贝叶斯前向模型（Bayesian forward model）结合子晕丰度匹配方案（subhalo abundance matching scheme）和参数化晕响应（parameterised halo response）来分析盘星系（SPARC）的最大旋转速度与ΛCDM暗物质晕分布的关系。

Result: 盘星系(disk galaxies)的旋转速度可追踪暗物质晕(dark matter halo)结构，为星系-晕关联提供直接约束。
...
### [30] [Cosmological back-reaction of baryons on dark matter in the CAMELS simulations](https://arxiv.org/abs/2601.06258)
...
**TL;DR**: 通过分析CAMELS项目的数千次宇宙流体动力学模拟，发现重子过程（如辐射冷却和反馈）会显著改变暗物质晕（dark matter halo）的密度分布（中心增密、外围减密）和大尺度结构聚类（功率谱变化达20%），且这种"反向作用"（back-reaction）强度与宇宙学参数（Ω_m和σ_8）及反馈模型细节密切相关。


<details>
  <summary>Details</summary>

Background: 研究探讨了重子过程（如辐射冷却和大质量恒星及活动星系核(AGN)的反馈）通过改变引力势对暗物质分布产生的"反向作用(back-reaction)"及其对宇宙学参数和反馈模型的依赖性。

Data: 数据来源于宇宙学和天体物理学与机器学习模拟（CAMELS）项目的数千个宇宙流体动力学模拟，包括SIMBA、IllustrisTNG、ASTRID和Swift-EAGLE星系形成模型的参数变化，并与对应的纯暗物质（N-body）模拟进行匹配。

Method: 采用宇宙流体动力学模拟（cosmological hydrodynamic simulations）结合多组星系形成模型（SIMBA、IllustrisTNG、ASTRID、Swift-EAGLE）的参数变化分析，通过匹配暗物质专用N体模拟（N-body simulations）的晕结构，量化重子过程对暗物质分布的反向作用（back-reaction）。

Result: 重子过程（如辐射冷却和大质量恒星及活动星系核(AGN)的反馈）通过改变引力势间接影响暗物质分布，导致晕（halo）的维里质量因重子反馈而减少。
...
### [31] [Exploring the internal structure of a neutron star and the associated magnetic fields aided by the mass-radius relationship](https://arxiv.org/abs/2601.06380)
*Abriana Lyda,Prajwal MohanMurthy*

**TL;DR**: 通过构建整合中子滴线（neutron-drip line）和核物质饱和态的分段模型，发现毫秒脉冲星（millisecond pulsars）的观测磁场可完全由壳层中子极化（crustal neutron polarization）维持（需对齐约≲5.5%中子），为非磁星（non-magnetar）中子星的壳层约束磁场起源（crust-confined magnetic-field origin）提供了定量依据。


<details>
  <summary>Details</summary>

Background: 中子星（neutron star）的极端磁场和密度为研究强相互作用物质在极端条件下的行为提供了天然实验室。

Data: 数据来源包括观测约束的质量-半径关系（observationally anchored mass-radius relations）和符合已知状态方程（equations of state）的密度剖面（density profile）。

Method: 构建了分段模型（piecewise model），整合中子滴线（neutron-drip line）、核饱和（nuclear-saturation）、电子主导晕（electron-dominated halo）和壳核界面（core-crust interfaces），并验证了壳核过渡区的刚度与曲率行为。

Result: 观测到的毫秒脉冲星磁场可完全由壳层中子极化维持，仅需壳层中约≲5.5%（99%置信水平）的中子排列对齐。
...
### [32] [Constraining Inflation Models with Spinning Voids](https://arxiv.org/abs/2601.06589)
*Geonwoo Kang,Jounghun Lee*

**TL;DR**: 提出一种利用空洞（void）星系红移不对称性诊断原初密度扰动标量谱指数（scalar spectral index）跑动的新方法，发现其分布模型在绝大多数宇宙学中具有普适性，仅当标量谱指数跑动非零时失效，为研究暴胀机制提供了新途径。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过原初密度扰动的标量谱指数（scalar spectral index）随尺度变化特性来探索宇宙早期暴胀（inflation）机制。

Data: 研究中使用的数据来源于宇宙学模拟（cosmological simulations）AbacusSummit的数值结果。

Method: 利用空洞星系红移不对称性分布的非参数模型（non-parametric model）约束原初密度扰动标量谱指数（scalar spectral index）的跑动（running）。

Result: 通过分析空洞（void）星系红移不对称性分布，发现标量谱指数跑动（running of scalar spectral index）偏离零值时该普适性模型失效，且跑动越正（负）红移不对称性越低（高）于模型预测值。
...
### [33] [Impact of a negative cosmological constant on the reconstruction of dark energy in light of DESI BAO data](https://arxiv.org/abs/2601.06656)
*Hao Wang,Yun-Song Piao*

**TL;DR**: 研究发现负宇宙学常数（NCC）可能与低红移演化暗能量共存，并通过两种重建方法（红移分箱和高斯过程）表明NCC在1σ置信度下更受支持，且使幻影分界线w=-1更符合w(z)的后验分布。


<details>
  <summary>Details</summary>

Background: 研究探讨了负宇宙学常数(NCC)与低红移下演化的正暗能量组分可能共存的现象及其对暗能量状态方程(EOS)的影响。

Data: 数据来源于DESI（Dark Energy Spectroscopic Instrument）最新观测数据，并结合红移分箱（redshift-binned）和高斯过程（Gaussian Process-based）两种重构方法进行分析。

Method: 采用红移分箱（redshift-binned）和高斯过程（Gaussian Process-based）重构方法分析负宇宙学常数（NCC）对演化暗能量状态方程（EOS）$w(z)$的影响。

Result: 负宇宙学常数（NCC）在两种重建方法中均被略微偏好（约1σ水平），且其与演化暗能量状态方程的简并使幻影分界线w=-1更符合1σ后验分布。
...
### [34] [A New Consistency Test for the $Λ$CDM Model using Radial and Transverse BAO Measurements](https://arxiv.org/abs/2601.07075)
*Xing-Han A. Zhao,Zheng Cai*

**TL;DR**: 提出了一种基于重子声波振荡（BAO）距离测量的无校准一致性检验方法，通过构建距离比消除声视界尺度影响，验证了平坦ΛCDM模型中物质密度参数ΩM在红移和距离比选择上的独立性，并利用DESI数据证实了当前误差范围内ΩM的红移不变性。


<details>
  <summary>Details</summary>

Background: 研究主题为基于重子声波振荡（BAO）距离测量对空间平坦的ΛCDM模型进行无校准一致性检验。

Data: 数据来源于暗能量光谱仪（DESI）数据发布1（Data Release 1）和数据发布2（Data Release 2）的重子声波振荡（BAO）测量结果。

Method: 采用基于重子声波振荡（BAO）距离测量比值的校准无关一致性检验方法，通过构建$Ð(z)$、$\DM(z)$和$\DV(z)$的比值消除声视界尺度影响，并将观测比值映射为有效平坦-$Λ$CDM物质密度参数$\OmL$。

Result: 基于重子声波振荡（BAO）距离测量比值导出的有效平坦-$Λ$CDM物质密度参数$\OmL$在现有误差范围内与红移无关的常数一致，验证了平坦$Λ$CDM模型的内部自洽性。
...
### [35] [More power on large scales](https://arxiv.org/abs/2601.07106)
*Jeremy Mould*

**TL;DR**: 宇宙微波偶极子的高值表明暗物质可能是宏观的原初黑洞（PBH）而非基本粒子，其早期形成和霍金辐射导致的物质密度参数（Omega_m'）动态变化能解释星系团大尺度流动速度增大并缓解哈勃张力。


<details>
  <summary>Details</summary>

Background: 宇宙微波背景偶极子的高值可能暗示暗物质是宏观物体而非基本粒子，这为研究暗物质本质和早期宇宙结构形成提供了新视角。

Data: 数据来源于N体数值模拟（N-body simulation），包括在红移z>100时启动的暗晕形成模拟以及考虑原初黑洞（PBH）质量损失效应的动态物质密度参数（Omega_m'）演化。

Method: 采用N体模拟（N-body simulations）方法，在红移（redshift）z > 100的辐射主导时期（radiation-dominated era）开始模拟，并引入因霍金辐射（Hawking radiation）导致质量损失的原始黑洞（PBHs）动态物质密度参数Ω_m'。

Result: 原初黑洞（PBH）作为暗物质组分通过早期引力势阱形成和霍金辐射（Hawking radiation）导致的物质密度参数动态变化，能够解释宇宙微波偶极子高值并缓解哈勃张力（Hubble tension）。
...
### [36] [New $H(z)$ measurement at Redshift = 0.12 with DESI Data Release 1](https://arxiv.org/abs/2601.07345)
*Ze-fan Wang,Lei Lei,Yi-zhong Fan*

**TL;DR**: 基于DESI发布的四千多个大质量被动演化星系(Passively Evolving Galaxies)的全光谱拟合，测得红移z=0.12处的哈勃参数(Hubble Parameter)为71.33±4.20 km s⁻¹ Mpc⁻¹，与其他方法结果一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过哈勃参数（Hubble parameter, H(z)）测量来理解宇宙膨胀历史。

Data: 数据来源于DESI合作项目发布的四千多个大质量被动演化星系（massive, passively evolving galaxies）的全光谱拟合（full-spectrum fitting）。

Method: 采用BAGPIPES对DESI发布的四千多个大质量被动演化星系进行全光谱拟合，估算其独立于宇宙学的恒星年龄和恒星形成历史。

Result: 通过DESI合作发布的四千多个大质量被动演化星系的全光谱拟合，测得哈勃参数（Hubble parameter）在红移z=0.12处的值为$H(z=0.12)=71.33 \pm 4.20~{\rm km~s^{-1}~Mpc^{-1}}$，与其他方法结果一致。
...
### [37] [Strong Evidence for Dark Sector Interactions](https://arxiv.org/abs/2601.07361)
*Tian-Nuo Li,William Giarè,Guo-Hong Du,Yun-He Li,Eleonora Di Valentino,Jing-Fei Zhang,Xin Zhang*

**TL;DR**: 最新DESI重子声波振荡数据与ΛCDM模型的偏离可通过暗物质与暗能量（IDE）的相互作用解释，两种IDE模型（耦合精质和耦合流体）在3-5σ水平上显示出显著的非零相互作用证据，其拟合效果与动态暗能量参数化相当甚至更优，表明IDE是动态暗能量的可行替代方案。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗物质与暗能量（IDE）相互作用作为解释DESI重子声学振荡数据偏离ΛCDM宇宙学模型的替代机制，挑战了传统上归因于动力学暗能量（DE）的观点。

Data: 数据来源包括DESI重子声学振荡（BAO）数据、CMB（Planck、ACT、SPT）数据、超新星（SN）数据（含DES-Dovekie重新校准数据）。

Method: 采用耦合精质（coupled quintessence）和耦合流体（coupled fluid）两种暗物质-暗能量相互作用（IDE）模型，在统一框架下分析最新宇宙微波背景（CMB）、重子声学振荡（BAO）和超新星（SN）数据。

Result: 暗物质与暗能量（IDE）的相互作用模型在3-5σ水平上显示出显著的非零耦合证据，其拟合效果与或优于动态暗能量（CPL）参数化。
...
### [38] [Modified Gravity and the Origin of the Excess Radio Galaxy Number-Count Dipole](https://arxiv.org/abs/2601.07487)
*John. W. Moffat*

**TL;DR**: 研究发现标量-张量-矢量引力理论（STVG-MOG）能通过尺度选择性增强效应解释射电星系数量偶极子超出标准ΛCDM模型预期的现象，为大尺度引力物理提供了新探测手段。


<details>
  <summary>Details</summary>

Background: 该研究探讨了标准$Λ$CDM模型中宇宙数数偶极子（cosmic number-count dipole）显著超出纯运动学预期的异常现象，并探索标量-张量-矢量引力理论（STVG-MOG）作为解释这一大尺度各向异性增强现象的潜在物理框架。

Data: 数据来源于对广域射电星系巡天（wide-area radio-galaxy surveys）的统计分析。

Method: 采用标量-张量-矢量引力理论（STVG-MOG）分析大尺度各向异性偶极增强现象，通过尺度与时间依赖的有效引力耦合保持早期宇宙和小尺度标准演化，同时放大千兆秒差距尺度的引力响应。

Result: 在STVG-MOG理论框架下，尺度与时间依赖的有效引力耦合能放大千兆秒差距尺度上的引力响应，从而增强大尺度结构对射电偶极的贡献而不在小尺度上过度产生功率。
...
### [39] [CHEX-MATE: Relationship between X-ray and millimetre inferences of galaxy cluster temperature profiles](https://arxiv.org/abs/2601.07653)
...
**TL;DR**: 通过联合XMM-Newton和Planck对116个CHEX-MATE星系团的观测分析，发现X射线温度与SZ效应推算温度比η_T的平均值为1.01±0.03，其分布受团形态显著影响（弛豫团更接近1且离散较小），而与红移、质量或温度基本无关。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系团（galaxy clusters）X射线和毫米波观测中热力学剖面（thermodynamic profiles）的比较问题，关注简化假设（如平滑、球对称的团内介质）导致的推断差异及其对理解星系团结构或宇宙学的意义。

Data: 数据来源于XMM-Newton和Planck对116个CHEX-MATE星系团的联合观测，包括X射线光谱温度（spectroscopic X-ray temperatures）和由苏尼亚耶夫-泽尔多维奇效应（Sunyaev-Zel'dovich effect）压力与X射线密度导出的温度代理（temperature proxy）。

Method: 采用X射线和毫米波联合分析方法，结合XMM-Newton和Planck数据，测量了X射线光谱温度与SZ压力及X射线密度导出的温度代理之比η_T，并考虑了热SZ信号的相对论修正和银河系分子氢的X射线吸收效应。

Result: 通过XMM-Newton和Planck对116个CHEX-MATE星系团的联合分析发现，X射线温度与SZ压力及X射线密度导出的温度代理之比η_T（η_T = T_X/T_{SZ,X}）的分布平均值为1.01±0.03，且形态松弛的星系团比值更接近1且离散度更小。
...
### [40] [Tachyonic gravitational dark matter production after inflation](https://arxiv.org/abs/2601.07670)
*Giorgio Laverda,Tomás Mendes,Javier Rubio*

**TL;DR**: 提出了一种基于曲率诱导的快子不稳定性（tachyonic instabilities）的非热暗物质（dark matter）产生机制，通过将实标量场（real spectator scalar field）与高斯-博内拓扑不变量（Gauss-Bonnet topological invariant）耦合，在暴胀结束后触发对称性自发破缺和粒子爆发式产生，计算表明该纯引力机制能在广泛质量范围和暴胀能标下重现观测到的暗物质丰度。


<details>
  <summary>Details</summary>

Background: 研究探讨了暴胀后由曲率诱导的快子不稳定性（tachyonic instabilities）驱动暗物质非热产生的引力机制。

Data: 数据来源于结合解析估计与完全非线性3+1经典格点模拟（classical lattice simulations）的混合方法。

Method: 采用基于高斯-博内(Gauss-Bonnet)拓扑不变量的曲率诱导快子(tachyonic)不稳定性机制，结合解析估计与完全非线性的3+1维经典晶格模拟追踪系统非平衡演化。

Result: 提出了一种基于曲率诱导的快子不稳定性（tachyonic instabilities）的引力机制，通过暴胀后时空曲率快速重组触发暗物质场的非热产生。
...
### [41] [Induced gravitational waves - beyond linear cosmological perturbation theory](https://arxiv.org/abs/2601.07716)
*Raphael Picard*

**TL;DR**: 研究扩展了宇宙微扰理论中二阶引力波（GWs）的来源形式，首次引入线性张量涨落作为新源项，发现标量-张量耦合诱导的引力波（scalar-tensor induced GWs）在张量模式增强时贡献显著，并揭示了其紫外（UV）谱在原始标量功率谱非尖锐峰情形下存在非物理增强现象。


<details>
  <summary>Details</summary>

Background: 研究主题为宇宙学扰动理论中超出线性阶的引力波（GWs）生成机制，特别是标量诱导引力波（scalar-induced GWs）及其与早期宇宙原初黑洞（PBHs）形成的关联。

Data: 研究使用了基于宇宙学扰动理论（cosmological perturbation theory）的模拟数据（simulation data），包括线性张量涨落（linear tensor fluctuations）作为额外源项，并考察了标量-张量耦合（scalar-tensor couplings）和张量-张量耦合（tensor-tensor couplings）产生的二阶引力波（GWs）。

Method: 采用二阶宇宙微扰理论，扩展现有形式主义(formalism)纳入线性张量涨落作为新源项，研究标量-张量耦合(scalar-tensor coupling)和张量-张量二次项(tensor-tensor quadratic terms)两类新型引力波。

Result: 在现有理论框架基础上引入线性张量涨落作为额外源项，发现标量-张量耦合诱导的引力波（scalar-tensor induced GWs）在张量模式增强时贡献显著，而张量-张量诱导的引力波（tensor-tensor induced GWs）贡献始终次要。
...
### [42] [Popcorn in the sky: Identifying primordial black holes in the gravitational-wave background](https://arxiv.org/abs/2601.07774)
*Eleni Bagui,Sébastien Clesse,Federico De Lillo,Alexander C. Jenkins,Mairi Sakellariadou*

**TL;DR**: 首次提出利用占空比（duty cycle）区分原初黑洞（PBH）双星与其他引力波背景（GWB）源的方法，并通过量化其与频率、持续时间和振幅的依赖关系为约束PBH模型提供关键指标。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用引力波背景（GWB）中的占空比（duty cycle）区分原初黑洞（PBHs）与其他潜在源，以约束PBH模型并探索宇宙学意义。

Data: 研究使用了基于原始黑洞(PBHs)双星实际种群的理论模拟(simulation)数据来计算引力波背景(GWB)中的占空比(duty cycle)。

Method: 采用基于时间域信号中源数量统计的占空比（duty cycle）分析方法，区分原初黑洞（PBH）双星对引力波背景（GWB）的散粒噪声（shot-noise）、爆米花（popcorn）和连续（continuous）贡献。

Result: 利用占空比（duty cycle）对原初黑洞（PBH）双星引力波背景（GWB）的贡献进行量化，可区分其与其它源的特性并约束PBH模型。
...
### [43] [Euclid preparation. Testing analytic models of galaxy intrinsic alignments in the Euclid Flagship simulation](https://arxiv.org/abs/2601.07784)
...
**TL;DR**: 通过欧几里得旗舰模拟（Euclid's Flagship simulation）研究星系本征排列（intrinsic alignments, IA）对弱引力透镜信号的影响，发现非线性排列模型（NLA）和潮汐排列与潮汐扭矩模型（TATT）在6-7 h⁻¹Mpc尺度内均能准确描述IA信号，且红星系排列振幅与观测一致，而蓝星系在0.1<z<0.3红移区间排列信号为零但在更高红移出现非零信号，同时揭示了常用红移幂律模型在z>1.1时失效而引入光度依赖性可显著改善模拟与观测的一致性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于欧几里得(Euclid)旗舰模拟中本征排列(intrinsic alignments, IA)的建模及其对弱引力透镜信号的潜在影响。

Data: 数据来源于欧几里得旗舰模拟（Euclid's Flagship simulation），考虑了星系的光度学性质及其暗物质宿主晕（dark matter host halos）。

Method: 采用非线性排列(NLA)和潮汐排列与潮汐扭矩(TATT)模型分析本征排列(IA)信号，并测量其振幅随星系星等和颜色的变化关系。

Result: 在欧几里得旗舰模拟中，非线性对齐（NLA）和潮汐对齐与潮汐扭矩（TATT）模型均能准确描述尺度低至6-7 h⁻¹ Mpc的星系固有排列（IA）信号。
...
### [44] [Euclid preparation. Calibrated intrinsic galaxy alignments in the Euclid Flagship simulation](https://arxiv.org/abs/2601.07785)
...
**TL;DR**: 基于欧几里得旗舰模拟（Euclid Flagship simulation）构建了半解析模型预测星系本征排列（intrinsic alignments）对弱引力透镜观测的污染，校准后模型包含12个排列参数，预测欧几里得任务末期层析弱透镜两点统计量将因此产生约10%的修正。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于星系内禀排列(intrinsic alignments)对弱引力透镜(weak gravitational lensing)宇宙学分析的潜在污染效应。

Data: 研究中使用的数据来源包括欧几里得旗舰模拟（Euclid Flagship simulation）、COSMOS巡天（COSMOS Survey）的星系椭率统计、斯隆数字巡天（Sloan Digital Sky Survey）样本的选定对准测量结果以及Horizon-AGN流体动力学模拟（Horizon-AGN hydrodynamic simulation）在红移$z=1$处提取的星系取向数据。

Method: 基于半解析模型（semi-analytic model）结合N体模拟（N-body simulation）数据，通过随机错位（stochastic misalignments）调节中心与卫星星系（central and satellite galaxies）的排列强度，并依据红移、光度和静止帧颜色（rest-frame colour）进行条件约束。

Result: 构建的半解析模型通过引入12个对齐参数，成功校准了星系椭圆度和排列特性，预测欧几里得(Euclid)弱引力透镜观测中本征排列(intrinsic alignment)对两点统计量的影响可达10%。
...
### [45] [Cosmoglobe DR2. V. Spatial correlations between thermal dust and ionized carbon emission in Planck HFI and COBE-DIRBE](https://arxiv.org/abs/2601.07818)
...
**TL;DR**: 通过拟合五种热尘埃发射示踪物（tracer）在353 GHz至25 THz频段的天空分布，发现高于1 THz时离子碳（C ii）发射是主导成分（10 THz以上占比近100%），其有效温度约25 K为最高，且所有成分在1-5 THz范围内均符合修正黑体谱能量分布（SED），模型对1 THz以下信号均方根的解释率达98%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过多频段热尘埃辐射示踪剂（thermal dust tracers）分析不同物理宿主环境在天空中的相对贡献及其随频率变化的规律。

Data: 数据来源包括普朗克卫星高频仪器（Planck HFI）和宇宙背景探测器-漫红外背景实验（COBE-DIRBE）的10个频率在353 GHz至25 THz之间的星图，以及HI4PI的中性氢（H i）、Dame等人（2001a）的一氧化碳（CO）、WHAM的Hα（Haffner等人2003a, 2016）、盖亚（Gaia）的尘埃消光（Edenhofer等人2024）和COBE-FIRAS的电离碳（C ii）发射。

Method: 采用标准多元线性回归方法对五个热尘埃发射示踪物（H i、CO、Hα、尘埃消光和C ii）与十个普朗克HFI和COBE-DIRBE频率图进行联合拟合。

Result: 在1 THz以上频率范围内，C ii（离子化碳）是最主要的示踪物，且在10 THz以上几乎贡献了全部拟合信号；在1 THz以下频率，其重要性仅次于H i（中性氢）。
...
### [46] [Cosmoglobe DR2. VII. Towards a concordance model of large-scale thermal dust emission for microwave and infrared frequencies](https://arxiv.org/abs/2601.07822)
*E. Gjerløw,R. M. Sullivan,R. Aurvik,A. Basyrov,L. A. Bianchi,A. Bonato,M. Brilenkov,H. K. Eriksen,U. Fuskeland,M. Galloway,K. A. Glasscock,L. T. Hergt,D. Herman,J. G. S. Lunde,M. San,A. I. Silva Martins,D. Sponseller,N. -O. Stutzer,H. Thommesen,V. Vikenes,D. J. Watts,I. K. Wehus,L. Zapelli*

**TL;DR**: 该论文通过四组分热尘埃模型拟合COBE-DIRBE数据，发现热尘埃（hot dust）在所有波段（尤其是3.5微米）主导信号，模型在低频和高频捕获超过95%方差，为建立微波与红外联合热尘埃发射模型迈出重要一步。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过多组分热尘埃模型（thermal dust model）分析COBE-DIRBE数据，旨在建立适用于微波和红外波段的联合热尘埃辐射一致性模型。

Data: 数据来源于COBE-DIRBE在3.5至240微米波段的观测数据。

Method: 采用全局贝叶斯端到端Cosmoglobe DR2再分析方法，将四组分热尘埃模型（包括“热尘埃”、“冷尘埃”、“邻近尘埃”和“Hα相关尘埃”）拟合到COBE-DIRBE的3.5至240微米数据，每个组分通过固定空间模板和各DIRBE通道自由幅度的空间各向同性光谱能量密度（SED）建模。

Result: result：在12至240微米波段，除仅在240微米波段显著检测到的冷尘埃（cold dust）振幅外，所有尘埃成分均表现出统计显著的模板振幅，其中3.5微米波段的热尘埃（hot dust）贡献至少80%的总信号，且该成分在所有DIRBE频段具有最高绝对振幅。
...
### [47] [Cosmoglobe DR2. IV. Modelling starlight in DIRBE with Gaia and WISE](https://arxiv.org/abs/2601.07831)
...
**TL;DR**: 基于盖亚(Gaia)和WISE测量数据，建立了1.25-25微米波段DIRBE(漫红外背景探测器)星光发射模型，发现恒星发射贡献在2.2微米、4.9微米和25微米波段分别占观测通量密度的91%、54%和1%。


<details>
  <summary>Details</summary>

Background: 研究背景为基于DIRBE（Diffuse Infrared Background Explorer）数据在1.25至25微米波段建立星光发射模型，结合盖亚（Gaia）和WISE（Wide-field Infrared Survey Explorer）观测数据量化恒星对弥散红外背景的贡献。

Data: 数据来源包括DIRBE（1.25-25微米波段）、Gaia（亮星物理参数和SED测量）和WISE（暗弱点源及部分亮星流量密度）。

Method: 基于盖亚(Gaia)和WISE测量数据，采用贝叶斯端到端Cosmoglobe DR2框架，结合PHOENIX恒星模型库拟合单个恒星的物理参数和光谱能量分布(SED)，并考虑暗弱点源的叠加贡献。

Result: 基于DIRBE数据在1.25至25微米波段的恒星发射模型显示，恒星总发射贡献了观测到的2.2微米波段91%、4.9微米波段54%和25微米波段1%的辐射通量密度。
...
### [48] [Axion misalignment as a synchronization phenomenon](https://arxiv.org/abs/2601.07836)
*Veronica Sanz*

**TL;DR**: 该研究将轴子（axion）错位角重新解释为膨胀宇宙中梯度驱动有序化产生的集体动力学现象，揭示了轴子相位从初始非相干构型动态演化为宏观相干态的过程，为传统错位机制提供了动力学基础。


<details>
  <summary>Details</summary>

Background: 该研究探讨了轴子（axion）错位作为涌现集体现象的动力学重新解释，将其与耦合振荡器系统中的同步现象进行类比，揭示了宏观轴子相位如何在宇宙膨胀背景下通过梯度驱动有序化从初始非相干构型中动态产生。

Data: 数据来源于简单晶格模型（simple lattice model）的数值模拟。

Method: 采用耦合振荡器系统同步理论（synchronization in coupled oscillator systems）与晶格模型（lattice model）分析轴子场（axion field）梯度驱动有序化（gradient-driven ordering）的集体动力学过程。

Result: 轴物质（axion）错位角（misalignment angle）被重新解释为膨胀宇宙中梯度驱动有序化产生的涌现集体变量，而非基本初始条件。
...
### [49] [Exoplanet characterization with NASA's Habitable Worlds Observatory](https://arxiv.org/abs/2601.06233)
*Joanna K. Barstow,Beth Biller,Mei Ting Mak,Sarah Rugheimer,Amaury Triaud,Hannah R. Wakeford*

**TL;DR**: 该论文指出NASA计划于2040年代中期发射的宜居世界观测站（HWO）将通过日冕仪实现10$^{-10}$对比度，在近紫外到近红外波段探测类太阳恒星周围温带岩石行星和冷气体巨星的分子特征（包括生物标志物和宜居性指标），并强调了英国天文界需通过仪器贡献和持续资助保持在该领域的国际竞争力。


<details>
  <summary>Details</summary>

Background: 系外行星大气表征（exoplanet atmosphere characterization）的突破性进展推动了对大气化学、动力学及行星形成机制的深入理解，而探测类太阳恒星周围温带岩石行星（temperate rocky planets）和冷气体巨行星（cool gas giants）以寻找宜居性或生命迹象成为未来几十年的核心科学目标。

Data: 研究数据来源于NASA计划于2040年代中期发射的旗舰任务"宜居世界观测站"(Habitable Worlds Observatory, HWO)，该平台搭载日冕仪设备，覆盖近紫外至近红外波段，可进行长时间积分观测。

Method: 采用直接光谱观测技术结合日冕仪（coronagraph）实现10$^{-10}$超高对比度，在近紫外至近红外波段探测分子物种及生物标志物。

Result: 未来几十年系外行星科学的主要目标是观测并表征围绕类太阳恒星运行的温带岩石行星和冷气体巨行星，以探测宜居性或生命迹象。
...
### [50] [Transiting exoplanets as the immediate future for population-level atmospheric science](https://arxiv.org/abs/2601.06236)
*Joanna K. Barstow,Hannah R. Wakeford,Sarah L. Casewell,Vatsal Panwar*

**TL;DR**: 凌星法（transit method）通过观测行星穿越恒星时的大气透射光谱和轨道相位变化，为研究系外行星大气成分、结构及动力学提供了大规模样本，并揭示了其与恒星类型、行星参数及形成机制间的关联。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用凌星法（transit method）探测和表征系外行星大气成分、结构及动力学特性，为理解行星系统演化及宜居性提供关键科学依据。

Data: 数据来源于凌星法（transit method）观测的6000多颗系外行星的光谱数据，包括凌星和食相期间恒星和行星的光变曲线（light curve）及透过光谱（transmission spectrum）与发射光谱（emission spectrum）。

Method: 采用凌星法（transit method）结合光谱观测技术分析系外行星大气成分、结构及动力学特征。

Result: 凌日法（transit method）通过测量行星经过恒星前方时的光通量下降来推断行星存在，并利用透射光谱和掩食观测研究其大气成分、结构及动力学特征。
...
### [51] [Incorporating Wavefront Error, Wavefront Sensing and Control, and Sensitivities into Exposure Time Calculations for Future Space Missions with the Error Budget Software (EBS)](https://arxiv.org/abs/2601.06342)
*Sarah Steiger,Pin Chen,Laurent Pueyo*

**TL;DR**: 开发了开源工具误差预算软件（EBS），用于整合日冕仪灵敏度、波前误差（WFE）和波前传感与控制（WFS&C）信息，并与EXOSIMS代码直接对接以计算宜居带观测站（HWO）任务中系外类地行星成像的曝光时间。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是开发用于NASA宜居世界天文台（HWO）任务的误差预算软件（EBS），以优化日冕仪（coronagraph）性能并解决望远镜不稳定性和波前误差（WFE）对系外类地行星成像的影响。

Data: 数据来源于NASA的宜居世界天文台（HWO）任务概念中约100个恒星系统的宜居带（HZ）观测，以及约25颗类地行星（行星/恒星通量比约为1E-10）的光谱采集，同时结合了罗马日冕仪仪器（Roman Coronagraph Instrument）等任务和仪器的误差预算研究中关于日冕仪灵敏度、波前误差（WFE）和波前传感与控制（WFS&C）的已有成果。

Method: 开发了开源工具误差预算软件（EBS），整合了日冕仪灵敏度、波前误差（WFE）和波前传感与控制（WFS&C）信息，并与开源代码EXOSIMS直接对接以计算曝光时间。

Result: 开发了开源工具误差预算软件（EBS），用于综合不同时空尺度的日冕仪灵敏度、波前误差（WFE）和波前传感与控制（WFS&C）信息，并与EXOSIMS代码直接对接以计算曝光时间。
...
### [52] [NASA Decadal Astrobiology Research and Exploration Strategy (NASA-DARES 2025) White Paper -- Habitable Worlds Observatory Living Worlds Science Cases: Research Gaps and Needs](https://arxiv.org/abs/2601.06386)
...
**TL;DR**: 这篇论文的核心贡献是提出了宜居世界天文台(Habitable Worlds Observatory, HWO)的跨学科天体生物学科学目标，重点研究了通过紫外-可见光-近红外(UV-Vis-NIR)波段光谱特征识别系外岩石行星生命迹象(biosignatures)的方法框架，并确定了宿主恒星系统关键参数对生命探测的影响。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是探索太阳系外岩石行星（exoplanets）的潜在宜居性并寻找生命迹象（biosignatures），重点关注紫外-可见-近红外（UV-Vis-NIR）波段的光谱特征及其测量需求。

Data: 数据来源包括对太阳系附近恒星周围潜在宜居系外行星的紫外-可见-近红外（UV-Vis-NIR）波段光谱特征观测，以及基于现有知识构建的宿主恒星和行星系统属性数据库。

Method: 采用紫外-可见-近红外（UV-Vis-NIR）波段光谱特征分析技术识别潜在生物标志物（biosignatures），并结合假阳性与假阴性评估框架进行解释。

Result: HWO（Habitable Worlds Observatory）的核心科学目标是利用紫外-可见-近红外（UV-Vis-NIR）波段光谱特征探测太阳系外岩质行星的生命迹象（biosignatures），并通过消除假阳性与假阴性框架进行生命信号验证。
...
### [53] [Brahe: A Modern Astrodynamics Library for Research and Engineering Applications](https://arxiv.org/abs/2601.06452)
*Duncan Eddy,Mykel J. Kochenderfer*

**TL;DR**: 开发了开源卫星动力学库Brahe，为空间态势感知和任务规划提供高效、易用的轨道建模与传播工具。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发现代开源卫星动力学库（Brahe），以解决天体动力学（astrodynamics）中卫星运动表示与预测这一基础问题，满足空间态势感知（space situational awareness）、卫星任务规划和空间任务操作等领域对高精度高效数值工具的需求。

Data: 数据来源于开源卫星动力学库Brahe（Brahe satellite dynamics library）的数值模拟（simulation）和算法实现。

Method: 采用现代开源卫星动力学库（Brahe）实现坐标变换、摄动建模和轨道传播等核心算法。

Result: Brahe是一个现代开源卫星动力学库，旨在为研究和工程应用提供快速部署、可组合、可扩展且易学的轨道建模与预测工具。
...
### [54] [Computing Natural Magnitudes in the Photometric Systems of Astronomical Plates using Gaia DR3 SEDs](https://arxiv.org/abs/2601.06859)
*Maryam Raouph,Andreas Schrimpf*

**TL;DR**: 利用盖亚(Gaia) DR3的蓝光(BP)和红光(RP)测光仪低分辨率光谱数据，通过合成测光法计算天文照相底片固有光度系统中的自然星等，相比传统色指数方法能更精确地校正大气红化(air mass)并识别出APPLAUSE档案中±0.3星等以上的系统误差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决天文摄影底片（photographic plates）与现代观测数据间因光谱灵敏度（spectral sensitivities）差异导致的测光校准（photometric calibration）难题，旨在建立基于盖亚卫星（Gaia）DR3数据的自然星等（natural magnitudes）体系以提升历史数据的精度和一致性。

Data: 数据来源为盖亚卫星第三次数据释放（Gaia DR3）的蓝色光度计（BP）和红色光度计（RP）低分辨率光谱数据，以及波长范围330 nm至1050 nm的盖亚光谱能量分布（SEDs，即XP光谱）星表。

Method: 采用合成测光法（synthetic photometry）结合盖亚（Gaia）DR3蓝光光度计（BP）和红光光度计（RP）低分辨率光谱数据，计算天文照相底片固有光系统中的自然星等和流量。

Result: 利用盖亚(Gaia) DR3的BP/RP光谱数据通过合成测光法计算的自然星等，相比传统色指数法能揭示APPLAUSE档案中±0.3星等以上的系统误差，并有效校正高大气质量下约0.2星等的大气红化效应。
...
### [55] [The Potential Impact of Neuromorphic Computing on Radio Telescope Observatories](https://arxiv.org/abs/2601.07130)
*Nicholas J. Pritchard,Richard Dodson,Andreas Wicenec*

**TL;DR**: 该论文提出神经形态计算（neuromorphic computing）和脉冲神经网络（SNNs）可显著降低射电天文处理流程（如射频干扰RFI检测）的功耗（最高达三个数量级），并通过从现场可编程门阵列（FPGA）到专用集成电路（ASIC）的部署路径，为现有及未来望远镜（如SKA和ngVLA）提供实时高效的数据驱动解决方案。


<details>
  <summary>Details</summary>

Background: 射电天文学面临下一代望远镜（如SKA和ngVLA）在功耗限制与复杂电磁环境下传统冯·诺依曼架构难以满足实时高效计算需求的挑战。

Data: 数据来源包括现有和近期射电望远镜（如SKA和ngVLA）的观测数据流，以及基于FPGA和神经形态ASIC（Application-Specific Integrated Circuit）的仿真测试平台。

Method: 采用脉冲神经网络（SNNs）和神经形态计算技术（neuromorphic computing）进行实时高效能射频干扰（RFI）检测及射电天文数据处理。

Result: 神经形态计算（neuromorphic computing）结合脉冲神经网络（SNNs）可将射电天文关键处理环节的功耗降低高达三个数量级，显著改善天文台运行预算。
...
### [56] [A free-fall-based switching criterion for P^3 T N-body methods in collisional stellar systems](https://arxiv.org/abs/2601.07425)
*Long Wang,David M. Hernandez,Zepeng Zheng,Wanhao Huang*

**TL;DR**: 论文提出了一种基于自由落体时间（free-fall-based）的PT-PP切换准则，用于混合引力N体模拟（P$^3$T scheme），发现在维里平衡（virial equilibrium）系统中该准则对低速度弥散（low-$σ$）或含双星的松散星团更准确，而速度弥散准则（$σ$-based）更适合高速度弥散系统。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过混合粒子-树算法（PT）和粒子-粒子直接求解器（PP）的P³T方案，优化引力N体系统中长程与短程相互作用的模拟精度与效率。

Data: 数据来源为使用\textsc{petar}代码结合P$^3$T方案和慢速算法正则化（slow-down algorithmic regularization）对星团进行的大规模数值模拟（simulation）。

Method: 采用混合粒子-树（PT）算法处理长程力与高精度粒子-粒子（PP）直接求解短程相互作用的P$^3$T方案，并引入基于自由落体（free-fall-based）和速度弥散（$σ$-based）的切换准则。

Result: 在维里平衡系统中，自由落体基准（free-fall-based）切换准则对低速度弥散（low-σ）或含双星的松散星团更精确，而速度弥散基准（σ-based）准则更适合高速度弥散（high-σ）系统。
...
### [57] [Advanced computing for reproducibility of astronomy Big Data Science, with a showcase of AMIGA and the SKA Science prototype](https://arxiv.org/abs/2601.07439)
*Julián Garrido,Susana Sánchez,Edgar Ribeiro João,Roger Ianjamasimanana,Manuel Parra,Lourdes Verdes-Montenegro*

**TL;DR**: 该论文介绍了AMIGA团队为应对平方公里阵列天文台(SKAO)大数据挑战提出的解决方案，包括语义数据模型、联邦基础设施中的分析服务及提升研究透明度的技术应用，并强调必须将可重复性要求嵌入SKA区域中心网络(SRCNet)的基础架构设计以实现全球可验证研究。


<details>
  <summary>Details</summary>

Background: 研究聚焦于平方公里阵列天文台（SKAO）在大数据时代面临的科学计算与可重复性挑战。

Data: 数据来源于平方公里阵列天文台（SKAO）的观测数据以及AMIGA组开发的语义数据模型和联邦基础设施中的分析服务。

Method: 采用语义数据模型（semantic data models）、集成到联邦基础设施中的分析服务（analysis services integrated into federated infrastructures）以及增强研究透明度的技术应用（techniques that enhance research transparency）作为核心分析方法。

Result: AMIGA小组通过开发语义数据模型（semantic data models）、集成联邦基础设施的分析服务以及提升研究透明度的技术应用，证明了在大数据时代实现可重复科学（reproducible science）的可行性。
...
### [58] [Brightness Characterization and Modeling for Amazon Leo Satellites](https://arxiv.org/abs/2601.07708)
*Anthony Mallama,Richard E. Cole,Hui Zhi,Brad Young,Jay Respler,Olga Zamora,Michelle Dadighat*

**TL;DR**: 基于1938次观测发现亚马逊狮子座卫星（Amazon Leo satellites）平均视星等为6.28，92%运行中的卫星亮度超过国际天文学联合会（IAU）推荐的干扰研究限值，25%影响夜空美学观赏，其反射特性与星链（Starlink）V1版卫星相似。


<details>
  <summary>Details</summary>

Background: 研究聚焦于亚马逊(Amazon)低轨(LEO)卫星群的亮度特征及其对天文观测和夜空美学的影响。

Data: 数据来源于1938次观测（observation）和航天器运行模式（operational mode）下的反射特性分析。

Method: 采用光度测量(photometry)分析卫星反射特性，并与星链(Starlink)卫星V1版本进行对比。

Result: 基于1,938次观测，亚马逊狮子座卫星的平均视星等(apparent magnitude)为6.28。
...
### [59] [First Multi-Constellation Observations of Navigation Satellite Signals in the Lunar Domain by Post-Processing L1/L5 IQ Snapshots](https://arxiv.org/abs/2601.06081)
...
**TL;DR**: 首次实验证实月球距离可接收多星座全球导航卫星系统（GNSS）信号（包括GPS、伽利略、区域导航卫星系统RNSS和星基增强系统SBAS），通过混合相干/非相干捕获技术从低采样率IQ数据提取有效观测值，仿真表明增加星座使四星可见时段占比从11%提升至46%，显著增强地月任务自主导航能力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用全球导航卫星系统（GNSS）增强地月空间航天器自主定轨能力，特别是通过实验验证多星座信号在月球距离的可探测性及其对导航可用性的提升作用。

Data: 数据来源于LuGRE接收器在近月空间和月球表面采集的同相正交（IQ）快照，并额外利用了GPS、Galileo以外的RNSS和SBAS星座信号。

Method: 采用多频多星座（multi-frequency, multi-constellation）信号可用性评估方法，通过处理月球GNSS接收器实验（LuGRE）采集的同相正交（IQ）快照数据，结合混合相干/非相干捕获阶段补偿码多普勒（code Doppler）的技术手段。

Result: 首次实验证实包括GPS和伽利略(Galileo)之外的多个星座（如RNSS和SBAS）信号在地月空间可被捕获，且额外星座的加入使卫星可见性（26 dB-Hz阈值下）从11%提升至46%。
...
### [60] [Coordinate Systems and Transforms in Space Physics: Terms, Definitions, Implementations, and Recommendations for Reproducibility](https://arxiv.org/abs/2601.07605)
*R. S. Weigel,A. Y. Shih,R. Ringuette,I. Christopher,S. M. Petrinec,S. Turner,R. M. Candey,G. K. Stephens,B. Cecconi*

**TL;DR**: 研究发现空间物理中常用坐标系（如GEI、GSM）的定义和实现差异会导致同名坐标系转换和星历表数据不一致，建议制定标准定义、建立可引用参考数据库、维护SPICE内核并要求软件开发者提供实现对比文档以确保可复现性。


<details>
  <summary>Details</summary>

Background: 空间物理学中坐标系（如GEI、GSM）定义与实现差异导致可重复性问题。

Data: 数据来源包括在线资源、软件包、高频引用期刊文章中的坐标系统定义，以及不同数据提供商提供的星历(ephemerides)值，并建议维护SPICE (Spacecraft, Planet, Instrument, C-matrix, Events)内核作为空间物理任务生成坐标系统数据产品的中央权威数据源。

Method: 采用文献比对和坐标变换差异分析的方法评估不同数据源对同名坐标系（如GEI、GSM）的实现差异。

Result: 不同定义和实现方式导致同名坐标系（如GEI、GSM）转换及星历表(ephemerides)值存在显著差异，需建立标准定义、可引用参考数据库、统一维护SPICE内核并要求软件实现与标准结果明确比对。
...
### [61] [In-ice Radio Signatures of Cosmic Ray Particle Cascades](https://arxiv.org/abs/2601.06417)
*Simon Chiche,Simona Toscano,Krijn D. de Vries*

**TL;DR**: 首次通过FAERIE框架模拟宇宙射线（cosmic ray）在空气和冰中的联合辐射特征，量化了其能量、时序、偏振和频谱等信号特性与簇射（shower）性质的关系，为冰中射电实验中宇宙射线识别和中微子（neutrino）甄别提供了关键依据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用冰中无线电探测器（in-ice radio detectors）表征宇宙射线（cosmic-ray）诱发的粒子簇射（particle showers）信号，以支持超高能中微子（ultra-high-energy neutrinos）探测实验的背景分析与校准。

Data: 数据来源于蒙特卡洛框架FAERIE的模拟（simulation），同时结合了宇宙射线在空气和冰中的辐射。

Method: 采用蒙特卡洛框架FAERIE模拟宇宙射线信号，首次结合其大气层内和冰内辐射特征，分析辐射能量、时序、偏振和频谱等关键参数与簇射特性的依赖关系。

Result: 首次通过结合大气层和冰层内辐射的模拟，量化了宇宙射线信号（cosmic ray signals）的辐射能量、时间特性、极化和频谱特征及其对簇射特性的依赖性。
...
### [62] [Radio Morphing: Fast computation of inclined air shower radio emission](https://arxiv.org/abs/2601.06418)
*Simon Chiche,Olivier Martineau-Huynh,Matias Tueros,Krijn D. de Vries*

**TL;DR**: 开发了新版本Radio Morphing工具，能够基于少量参考簇射模拟数据快速生成大倾角（θ>60°）宇宙射线诱导空气簇射的无线电信号，在原始波形峰值振幅上平均相对差异低于17%，计算效率比蒙特卡洛模拟提升超四个数量级。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于开发高效模拟工具以支持下一代大型射电实验（radio experiments）的探测器配置优化和空气簇射（air shower）参数重建方法创新。

Data: 研究中使用的数据来源于少量参考簇射（reference showers）在给定位置的模拟数据，通过半解析工具Radio Morphing生成任意天线位置和天顶角θ>60°的宇宙射线诱导空气簇射的无线电发射模拟。

Method: 采用半解析工具Radio Morphing，通过少量参考簇射的模拟数据快速模拟任意宇宙线诱导大倾角（θ>60°）空气簇射的射电信号。

Result: Radio Morphing新版本可在原始迹线上实现峰值振幅平均相对差异低于17%，在3σ触发阈值下低于15%，在[50-200] MHz频段低于13%，在[30-80] MHz频段低于约10%。
...
### [63] [Masers and Broad-Line Mapping Favor Magnetically-Dominated AGN Accretion Disks](https://arxiv.org/abs/2601.06253)
*Philip F. Hopkins,Dalya Baron,Joanna M. Piotrowska*

**TL;DR**: 研究发现超大质量黑洞（supermassive black hole）吸积盘外区（半径$R \gtrsim 0.01\,$pc）必须由磁压（magnetic pressure）主导，观测数据排除了热压（thermal pressure）或辐射压（radiation pressure）主导的经典α盘模型（α-disk model），支持"超磁化"（hyper-magnetized）状态假说。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于超大质量黑洞（supermassive black hole, BH）吸积盘（accretion disk）外区（outer disk）物理机制（尤其是压力来源）的理论约束与观测验证。

Data: 数据来源包括现有脉泽（maser）和宽线区（BLR）运动学观测数据。

Method: 采用湍流（turbulence）和磁压（magnetic pressure）主导的盘模型分析超大质量黑洞吸积盘物理特性。

Result: 超大质量黑洞（supermassive black hole, BH）吸积盘的外区（半径$R \gtrsim 0.01\,$pc或$\gtrsim 1000\,R_{G}$）必须由磁压主导（'hyper-magnetized' state），因为观测排除了热压、辐射压或宇宙射线压主导的经典吸积盘模型（如Shakura-Sunyaev $α$-disk），而纯湍流压力模型需要不现实的极高湍流Toomre $Q$值（$Q \gtrsim 100$）。
...
### [64] [Ruling Out Compact Jets as the Dominant Source of Radio Emission in Radio-quiet, High Eddington-ratio Active Galactic Nuclei](https://arxiv.org/abs/2601.06480)
*Jeremiah D. Paul,Richard M. Plotkin*

**TL;DR**: 研究发现，在69个超大质量黑洞（$M_{\rm BH} \sim 10^{5}-10^{9}~M_\odot$）的高/超爱丁顿率（super-Eddington）射电宁静活动星系核（AGNs）中，射电/X射线光度比不支持喷流主导模型，更符合冕区主导射电辐射并伴随高吸积率下外流贡献的起源机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探讨射电宁静活动星系核（AGNs）核心射电辐射的起源问题，特别是针对高/超爱丁顿吸积率（super-Eddington）AGNs中射电宁静现象与广义相对论磁流体力学（GRMHD）模拟预测的大尺度喷流形成之间的观测矛盾。

Data: 研究中使用的数据来源于69个射电宁静（radio-quiet）、高/超爱丁顿（high/super-Eddington）活动星系核（AGNs）样本的多波段观测数据，包括射电和X射线光度比（radio to X-ray luminosity ratios），样本中黑洞质量（black-hole masses）范围在$10^{5}-10^{9}~M_\odot$之间。

Method: 采用多波段光度比（radio/X-ray luminosity ratios）分析方法，结合喷流主导模型（jet-dominated model）和冕区主导模型（corona-dominated model）的标度关系，扩展到超爱丁顿吸积（super-Eddington）状态进行验证。

Result: 在超大质量黑洞（supermassive black hole）质量范围$10^{5}-10^{9}~M_\odot$内，观测到的射电/X射线光度比与射电辐射由喷流主导的模型不符，更可能符合冕区主导的射电辐射起源模型，并在高吸积率下存在外流贡献。
...
### [65] [Multi-wavelength properties of changing-state active galactic nuclei: I. the evolution of soft excess and X-ray continuum](https://arxiv.org/abs/2601.07337)
...
**TL;DR**: 通过对五个变状态活动星系核（CSAGNs）的宽频X射线光谱分析，发现软超量和主连续谱辐射高度正相关且与爱丁顿比（Eddington ratio）强相关，表明其共同物理起源为温康普顿化（warm Comptonization），并在低爱丁顿比（$\log λ_{\rm Edd} \sim -2.5$）时软超量消失且吸积流几何结构改变。


<details>
  <summary>Details</summary>

Background: 研究聚焦于变化态活动星系核（CSAGNs）的快速光度变化及其吸积流演化，为实时观测内区吸积过程提供了独特窗口。

Data: 使用了超过1000次观测数据，研究样本为五个变化态活动星系核（CSAGNs）的宽带X射线光谱，覆盖爱丁顿比（Eddington ratio）三个数量级范围。

Method: 采用现象学模型（phenomenological models）分析宽频X射线光谱，追踪关键光谱成分演化，并推导光子指数、软超量强度、反射强度等基本参数。

Result: 软超和主连续辐射呈现极强正相关（$p \ll 10^{-10}$），表明二者具有共同物理起源。
...
### [66] [Systematic Biases in Gravitational-Wave Parameter Estimation from Neglecting Orbital Eccentricity in Space-Based Detectors](https://arxiv.org/abs/2601.07739)
*Jin-Zhao Yang,Jia-Hao Zhong,Tao Yang*

**TL;DR**: 研究发现忽略轨道偏心率（orbital eccentricity）会导致空间引力波探测器（B-DECIGO和LISA）在分析致密双星并合时产生显著参数估计偏差，其中B-DECIGO对微小偏心率（$e_0\sim 10^{-4}-10^{-3}$）敏感，而LISA在较大偏心率（$e_0\sim 10^{-2}-10^{-1}$）下才出现类似偏差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于未来空间引力波探测器（如B-DECIGO和LISA）对致密双星并合引力波信号参数估计中轨道偏心率（orbital eccentricity）忽略所引发的系统偏差问题。

Data: 数据来源包括基于频率域波形模型生成的模拟（simulation）数据，该模型整合了轨道偏心率（orbital eccentricity）诱导的高次谐波以及空间探测器（B-DECIGO和LISA）的时间依赖性响应，并通过构建覆盖恒星质量双星和大质量黑洞双星的模拟引力波目录进行系统分析。

Method: 采用频域波形模型（frequency-domain waveform）结合轨道偏心率（orbital eccentricity）诱导的高次谐波和空间探测器时变响应（time-dependent response of spaceborne detectors）进行引力波信号建模。

Result: 忽略轨道偏心率（orbital eccentricity）会导致双致密天体并合（compact-binary coalescences）参数估计的系统性偏差，在B-DECIGO中初始偏心率（initial eccentricity）低至$e_0\sim 10^{-4}-10^{-3}$（0.1 Hz）时偏差显著，而LISA中通常在$e_0\sim 10^{-2}-10^{-1}$（$10^{-4}$ Hz）时出现类似偏差。
...
### [67] [Cosmological Dynamics on a Novel $f(Q)$ Gravity Model with Recent DESI DR2 Observation](https://arxiv.org/abs/2601.06438)
*S. A. Kadam,D. Revanth Kumar,Santosh Kumar Yadav*

**TL;DR**: 通过马尔可夫链蒙特卡洛分析结合多种观测数据，在修正对称远平行引力模型(f(Q))框架下确定了模型参数(m=-0.386±0.090, n=-1.055±0.047)和当前宇宙学参数(H0=73.19±0.25, q0=-0.51±0.6, ω0=-0.73±0.3)，发现该模型在红移ztr=0.573处经历从早期减速到晚期加速的相变，且om(z)诊断的正斜率支持状态方程参数处于精质(quintessence)区域的行为特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于修正对称远平行引力模型（modified symmetric teleparallel gravity model）在$f(Q)$框架下的宇宙学可行性。

Data: 数据来源为宇宙计时器（cosmic chronometers）、PantheonPlus SH0ES和DESI BAO DR2的组合数据集。

Method: 采用马尔可夫链蒙特卡洛（Markov Chain Monte Carlo, MCMC）方法对修正对称远平行引力模型（modified symmetric teleparallel gravity model）的$f(Q)$框架进行参数约束分析。

Result: result：在修正对称远平行引力模型（modified symmetric teleparallel gravity）的$f(Q)$框架下，通过观测数据约束得到最佳拟合参数$m=-0.386 \pm 0.090$、$n=-1.055 \pm 0.047$及当前宇宙学参数$H_0 = 73.19 \pm 0.25$、$q_0 = -0.51 \pm 0.6$、$ω_{0} = -0.73 \pm 0.3$（68\%置信水平），模型表现出从早期减速到晚期加速的相变（$z_{tr} = 0.573$），且$om(z)$诊断的正斜率支持状态方程参数处于精质（quintessence）区域的行为特征。
...
### [68] [Strong Einstein-Hilbert Gravity Inflation and ACT Phenomenology](https://arxiv.org/abs/2601.06946)
*V. K. Oikonomou,Eleni I. Manouri,Georgios Konstantellos*

**TL;DR**: 研究发现在强曲率（strong curvature）下重标度（rescaled）的规范标量场（canonical scalar field）理论中，当重标度参数α小于1时，更强的引力（stronger gravity）仍能与ACT对原初标量扰动谱指数（spectral index）及BICEP/Planck对张标量比（tensor-to-scalar ratio）的约束兼容。


<details>
  <summary>Details</summary>

Background: 研究探讨了在强曲率（strong curvature）区域由有效$f(R,φ)$引力理论导出的重标度（rescaled）单标量场理论及其与宇宙微波背景（CMB）观测数据（ACT和BICEP/Planck）的相容性。

Data: 数据来源于ACT（Atacama Cosmology Telescope）对原初标量扰动谱指数（spectral index）的约束以及BICEP/Planck对张标比（tensor-to-scalar ratio）的最新约束。

Method: 采用重标度（rescaling）参数α的单标量场有效理论（effective single scalar field theory）分析重标度爱因斯坦-希尔伯特项（Einstein-Hilbert term）对原初扰动谱指数（spectral index）和张标比（tensor-to-scalar ratio）的观测约束影响。

Result: 重标度参数α小于1的典型标量场理论（canonical scalar field theories）在强引力条件下与ACT及更新后的BICEP/Planck观测约束相容。
...
### [69] [Viable f(R) Scenarios Unifying Inflation with Realistic Dynamical Dark Energy](https://arxiv.org/abs/2601.06949)
*S. D. Odintsov,V. K. Oikonomou,G. S. Sharov*

**TL;DR**: 研究测试了两个F(R)引力模型在所有宇宙演化阶段的适用性，发现它们能同时描述早期暴胀时期和晚期暗能量时期，并在拟合最新观测数据时优于ΛCDM模型，成功模拟了动态暗能量行为。


<details>
  <summary>Details</summary>

Background: 研究主题为验证两种F(R)引力模型在宇宙演化各阶段（包括早期暴胀时期和暗能量时期）的可行性及其对最新观测数据的解释能力。

Data: 数据来源包括Pantheon+超新星（Type Ia supernovae）目录、重子声波振荡（baryon acoustic oscillations）的暗能量光谱仪（Dark Energy Spectroscopic Instrument）测量、哈勃参数（Hubble parameter）估计以及宇宙微波背景辐射（cosmic microwave background radiation）数据。

Method: 采用F(R)引力模型（$F(R)$ gravity models）进行理论建模，通过分析$\frac{dF}{dR}>0$等可行性条件并结合参数空间限制域来验证模型在早期暴胀时期和晚期暗能量时期的宇宙学演化描述能力。

Result: 两个F(R)引力模型在早期暴胀时期和晚期暗能量时期均能描述宇宙演化，且与最新观测数据（包括Pantheon+超新星、DESI重子声学振荡、哈勃参数估计和宇宙微波背景辐射）拟合时，其最佳拟合参数远离参数空间中的禁区，在χ²统计量、Akaike和贝叶斯信息准则上均显著优于Λ-CDM模型。
...
### [70] [Oscillatory Freeze from Inertial Holographic Dark Energy](https://arxiv.org/abs/2601.07007)
*Swapnil Kumar Singh*

**TL;DR**: 该论文通过引入含哈勃(Hubble)参数及其前两阶时间导数的红外截止(infrared cutoff)构建广义全息暗能量(holographic dark energy)模型，发现宇宙膨胀会逐渐停止并渐进趋于闵可夫斯基(Minkowski)时空，其非线性背景演化与当前晚期膨胀观测数据高度吻合（χ²/ν≈0.52）。


<details>
  <summary>Details</summary>

Background: 研究探讨了广义全息暗能量（holographic dark energy）模型，其中红外截止（infrared cutoff）依赖于哈勃参数及其前两阶时间导数，并引入有效熵惯性（effective entropic inertia）解释视界自由度的有限弛豫时间尺度。

Data: 数据来源为宇宙计时器（cosmic chronometer）测量的哈勃参数（Hubble parameter）观测数据。

Method: 采用依赖于哈勃参数（Hubble parameter）及其前两阶时间导数的广义全息暗能量（holographic dark energy）模型，通过引入$\ddot H$项引入视界自由度的有限弛豫时间尺度，解释为全息真空能量的有效熵惯性。

Result: 在广义全息暗能量（holographic dark energy）模型中引入$\ddot H$项会导致哈勃参数（Hubble parameter）呈现指数衰减振荡并渐近趋于零，宇宙膨胀逐渐停止，最终形成渐近平直的闵可夫斯基时空（Minkowski spacetime）。
...
### [72] [Bringing Computation to the data: Interoperable serverless function execution for astrophysical data analysis in the SRCNet](https://arxiv.org/abs/2601.07308)
...
**TL;DR**: 研究发现无服务器计算（Serverless computing）中的函数即服务（FaaS）模型可有效支持平方公里阵列天文台（SKAO）的海量射电天文数据处理，通过将计算嵌入数据存储站点（SRCNet）实现低延迟、高效率的分布式分析。


<details>
  <summary>Details</summary>

Background: 研究聚焦于无服务器计算（serverless computing）和函数即服务（FaaS）在应对平方公里阵列天文台（SKAO）海量数据处理挑战中的应用潜力，探索其在射电天文工作流中实现分布式、近数据（data-proximate）计算的可能性。

Data: 研究中使用的数据来源于SKA区域中心网络（SKA Regional Centre Network, SRCNet）中存储的分布式射电天文数据产品，包括通过代表性天体物理数据分析函数（如高斯卷积函数）处理的观测数据。

Method: 采用函数即服务（FaaS）模型实现轻量级函数的按需部署与触发，并通过开发代表性天体物理数据分析函数（如高斯卷积函数）并将其集成至SKA区域中心网络（SRCNet）生态系统，验证了无服务器计算在射电天文工作流中的应用。

Result: 无服务器计算（Serverless computing）为应对平方公里阵列天文台（SKAO）的海量数据处理挑战提供了可扩展且高效的解决方案，通过将计算嵌入数据存储地点的现有服务生态系统（如SRCNet）来降低延迟、减少传输并提升效率。
...
### [73] [Ultraviolet observations of atmospheric escape in exoplanets with the Habitable Worlds Observatory](https://arxiv.org/abs/2601.06263)
*Leonardo A. Dos Santos,Eric D. Lopez*

**TL;DR**: 提出利用宜居世界观测站（Habitable Worlds Observatory）的紫外（UV）光谱能力，通过凌星光谱观测研究系外行星大气逃逸机制、评估小型系外行星大气保留能力并探测类地大气，主张开发具备中高分辨率、覆盖1000-3000埃关键谱段且耐高计数率的紫外光谱仪。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过大气逃逸（atmospheric escape）机制理解行星系统演化，并开发识别类地行星（Earth-like worlds）的技术，以支持寻找和表征宜居世界（habitable worlds）的优先科学目标。

Data: data：研究使用了宜居世界天文台（Habitable Worlds Observatory）的紫外（UV）波段观测能力，通过凌星光谱观测获取数据，重点关注1000至3000埃范围内的关键光谱特征。

Method: 采用紫外（UV）光谱仪进行中高分辨率点源光谱分析，覆盖1000至3000埃的关键光谱特征，并利用耐高计数率的UV探测器开展凌星光谱观测。

Result: 利用宜居世界观测站（Habitable Worlds Observatory）的紫外（UV）能力，通过凌星光谱观测可确定系外行星演化驱动机制、小型系外行星大气保留能力及搜寻类地大气特征。
...
### [74] [Functionalization of Benzene Ices by Atomic Oxygen](https://arxiv.org/abs/2601.07457)
...
**TL;DR**: 研究发现单重态氧原子(O($^1$D))在星际冰尘中与苯(C$_6$H$_6$)发生无势垒插入反应高效生成苯酚(C$_6$H$_5$OH)，其27-44%的转化率上限与彗星67P实测的20$\pm$6%苯酚/苯比值吻合，证实了冷星际冰中氧原子介导的芳香烃官能化是行星形成初期生物相关含氧芳香化合物的关键来源。


<details>
  <summary>Details</summary>

Background: 研究探讨了星际冰粒中单线态氧原子（O($^1$D)）与苯反应生成含氧芳香族化合物的机制及其在恒星和行星形成过程中的作用。

Data: 实验数据来源于实验室模拟冰相反应，使用臭氧作为单线态氧原子（O($^1$D)）的前体，在H$_2$O和CO$_2$冰基质中研究其与苯的反应产物。

Method: 采用臭氧光解产生单重态氧原子（O($^1$D)）与苯在冰相中反应的实验方法，结合温度依赖性和基质稀释效应分析反应路径及产物分布。

Result: 单重态氧原子（O($^1$D)）与苯在冰相中高效反应生成苯酚（phenol）、苯氧化物（benzene oxide）和氧杂环庚三烯（oxepine），其中苯酚形成无温度依赖性而苯氧化物/氧杂环庚三烯形成存在轻微温度依赖性。
...
### [75] [SPHEREx Re-Observation of Interstellar Object 3I/ATLAS in December 2025: Detection of Increased Post-Perihelion Activity, Refractory Coma Dust, and New Coma Gas Species](https://arxiv.org/abs/2601.06759)
...
**TL;DR**: NASA SPHEREx航天器在2025年12月观测发现彗星ISO 3I/ATLAS过近日点后活动显著增强，表现为冰完全升华、尘埃散射光与热辐射主导光谱特征，并首次检测到CN和C-H释放，表明这些物质可能被水冰包裹或埋藏。


<details>
  <summary>Details</summary>

Background: 研究聚焦于彗星ISO 3I/ATLAS在近日点前后由NASA SPHEREx航天器观测到的挥发性物质（volatiles）升华及尘埃活动演化特征。

Data: 数据来源于NASA SPHEREx航天器在2025年12月对ISO 3I/ATLAS彗星近日点后的观测，包括尘埃散射光、热辐射及CN（0.93微米）、H2O（2.7微米）、有机C-H（3.2至3.6微米）、CO2（4.25至4.27微米）和CO（4.6至4.8微米）的气体发射光谱数据。

Method: 采用成像分光光度法（imaging spectrophotometry）分析尘埃散射光、热辐射及CN、H2O、有机C-H、CO2和CO气体发射特征。

Result: 2025年12月NASA SPHEREx航天器对ISO 3I/ATLAS的近日点后观测显示其活动性显著增强，冰物质完全升华并发展为典型彗星体，CN、H2O、有机C-H、CO2和CO气体发射主导了新的成像分光光度测量特征。
...
### [76] [Spec-o3: A Tool-Augmented Vision-Language Agent for Rare Celestial Object Candidate Vetting via Automated Spectral Inspection](https://arxiv.org/abs/2601.06498)
*Minghui Jia,Qichao Zhang,Ali Luo,Linjing Li,Shuo Ye,Hailing Lu,Wen Hou,Dongbin Zhao*

**TL;DR**: 提出了一种名为Spec-o3的工具增强型视觉语言代理（vision-language agent），通过多模态链式推理实现与天文学家思维对齐的光谱检测，在LAMOST（郭守敬望远镜）的稀有天体识别任务中将宏F1分数从28.3提升至76.5，并展现出跨巡天项目（如SDSS/DESI）的强泛化能力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决深度学习分类器在罕见天体候选体最终筛选时泛化性和可解释性不足的问题，通过开发智能代理工具提升光谱数据（spectra）分析的效率和可靠性。

Data: 研究中使用的数据来源于LAMOST（郭守敬望远镜）的光谱巡天数据，并在泛化性评估中扩展到SDSS（斯隆数字化巡天）和DESI（暗能量光谱仪）的数据集。

Method: 采用了两阶段后训练方法：基于专家检查轨迹的冷启动监督微调，随后在稀有类型验证任务上进行基于结果的强化学习。

Result: 提出的Spec-o3工具增强型视觉语言代理通过多模态思维链推理实现了天文学家对齐的光谱检查，在LAMOST五个稀有天体识别任务中宏F1分数从28.3提升至76.5，并展现出对未见过检查任务的强泛化能力（从LAMOST到SDSS/DESI）。
...
### [79] [Impact of Nuclear Reaction Rates on Calcium Production in Population III Stars: A Global Analysis](https://arxiv.org/abs/2601.07623)
...
### [80] [Matière noire et (ou) gravitation modifiée : une approche historique et épistémologique](https://arxiv.org/abs/2601.06592)
*Benoit Famaey,Jonathan Freundlich*

**TL;DR**: 该论文综述了暗物质（dark matter）概念的历史及其对星系形成和演化的影响，探讨了现有模型的局限性和替代理论（如修改引力定律）。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙学标准模型中暗物质（dark matter）存在的理论基础及其对星系形成演化的解释作用，同时审视了该模型的局限性和替代理论（如修改引力定律）的可能性。

Data: 数据来源包括星系旋转曲线（galaxy rotation curves）、星系团引力透镜效应（gravitational lensing from galaxy clusters）和宇宙微波背景辐射（cosmic microwave background）的观测数据。

Method: 采用文献综述方法评估暗物质（dark matter）模型的证据与局限性，并探讨修正引力理论（modified gravitational law）等替代方案。

Result: 暗物质(dark matter)的存在性仍存疑，存在通过修改引力定律(gravitational law)替代暗物质理论的可能性。

Abstract: The current standard model of cosmology assumes that the majority of matter in the Universe is made of dark matter, and that the latter is fundamentally different from ordinary matter. Dark matter can in principle explain the rotation of galaxies, the gravitational lensing from galaxy clusters or the appearance of the cosmic microwave background, the oldest light in the Universe. But does dark matter really exist? Here, we review the history of this concept and its implications for the formation and evolution of galaxies. We also consider the questions that remain, the limitations of the model, and present alternative theories, in particular modifications to the gravitional law that would -- perhaps -- make it possible to do without it.
...
### [81] [Temperature-Dependent CPT Violation: Constraints from Big Bang Nucleosynthesis](https://arxiv.org/abs/2601.06259)
*Gabriela Barenboim,Anne-Katherine Burns*

**TL;DR**: 通过电子-正电子质量不对称性（参数化为$b_0(T) = αT^2$）研究大爆炸核合成（BBN）期间的温度依赖CPT破坏，发现$α$必须大于或约等于$10^{-6}$ GeV$^{-1}$才能在BBN时期产生keV量级质量差异，并首次给出了该机制下早期宇宙CPT破坏的最严格约束。


<details>
  <summary>Details</summary>

Background: 研究探讨了早期宇宙大爆炸核合成（BBN）过程中温度依赖的CPT（电荷-宇称-时间反演对称性）破坏现象，重点关注电子-正电子质量不对称性及其对原初核素丰度的影响。

Data: 研究中使用的数据来源于修改版的BBN代码PRyMordial（通过GitHub获取），结合了动态求解化学势和适当有限质量修正，并利用观测到的氦-4、氘和有效中微子数（$N_{\rm eff}$）丰度进行约束。

Method: 采用参数化电子-正电子质量不对称性$b_0(T) = αT^2$，通过改进版BBN代码PRyMordial（动态求解化学势并引入有限质量修正）约束早期宇宙CPT破坏效应。

Result: 电子-正电子质量不对称性参数$α$在BBN（Big Bang Nucleosynthesis）期间必须大于或约等于$10^{-6}$ GeV$^{-1}$才能在keV尺度产生显著CPT破坏。
