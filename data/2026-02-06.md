...
**TL;DR**: 通过分析旋转支撑与湍流在暗物质晕（dark matter halo）和星系自身引力势中的竞争关系，提出星系盘（disk）形成源于重子物质积累导致的势能陡化，并解释了盘形成前后星系尺寸变化及临界晕质量随红移增大的观测现象。


<details>
  <summary>Details</summary>

Background: 研究探讨星系盘形态（disk morphology）随质量增加和红移降低而更普遍的物理机制，聚焦于旋转支撑与湍流在暗物质晕（dark matter halo）和星系自身引力势中竞争导致盘形成的条件。

Data: data来源包括TNG50宇宙学模拟（cosmological simulation）和观测数据（observed data）。

Method: 采用了一个简单的解析模型，通过分析旋转支撑与湍流在暗物质晕（dark matter halo）和星系自身引力势中的竞争关系，解释星系形态转变的物理机制。

Result: 星系形态由暗物质晕和星系自身引力势中旋转支撑与湍流的竞争决定，盘状结构形成源于重子物质在晕中心积累导致的势能陡化。
...
**TL;DR**: 通过比较NFW（尖峰）、Einasto（中间）、Burkert（核式）和伪等温（pISO）（核式）四种暗物质晕（dark matter halo）模型对GARCIA样本中11个星系的拟合，发现所有模型均能提供统计上良好的拟合，其中NFW模型最匹配内区密度分布，而外区各模型结果趋于一致。


<details>
  <summary>Details</summary>

Background: 暗物质在星系内区的分布对ΛCDM宇宙学的小尺度结构提出了关键挑战。

Data: 数据来源为GMRT档案原子气体巡天(GARCIA)中的11个星系样本，结合3.6μm或r波段测光数据通过多高斯展开(MGE)与各向异性Jeans模型(JAM)获取恒星运动学信息，以及直接由气体表面密度(HI + He)计算的气体贡献。

Method: 采用马尔可夫链蒙特卡洛（MCMC）技术进行质量建模，结合基于3.6微米或r波段光度测量的多高斯展开（MGE）与各向异性琼斯模型（JAM）的恒星运动学，以及直接从气体表面密度（HI + He）计算的气体贡献。

Result: 通过比较四种暗物质晕轮廓（NFW、Einasto、Burkert和pseudo-isothermal）对GARCIA样本中11个星系的质量建模，发现所有晕轮廓均能提供统计上良好的拟合，且对晕质量和恒星质量-光比给出了一致估计。
...
**TL;DR**: 在MACS J1354.6+7715星系团中首次观测到主导射电星系（AGN）存在300千秒差距的锐弯射电尾（tail），其谱指数从核心区-0.46陡化至外围-2.43，同步辐射模型揭示该星系以1956 km/s速度首次坠入星系团（ICM），为研究活动星系核（AGN）与星系团介质相互作用提供了理想案例。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用射电尾迹星系（tailed radio galaxies）作为探针，揭示活动星系核（AGN）与星系团内介质（ICM）相互作用的动力学过程及其对星系演化的影响。

Data: 数据来源于LOw Frequency ARray (LOFAR) 144 MHz和uGMRT 400 MHz对MACS J1354.6+7715（红移z = 0.3967）的射电观测，以及钱德拉(Chandra)的X射线形态数据。

Method: 采用低频阵列(LOFAR)144 MHz和升级巨米波射电望远镜(uGMRT)400 MHz多频段射电观测数据，结合同步辐射模型(synchrotron modelling)分析射电尾的光谱指数分布和辐射年龄。

Result: result: 主导的带尾射电星系（tailed radio galaxy）显示出约300 kpc的急剧弯曲尾部，其谱指数从核心附近的-0.46±0.21陡化至最外区域的-2.43±0.30，同步加速器模型得出辐射年龄为150±10 Myr，表明星系速度为1956±130 km/s，约为逃逸速度的0.9倍。
...
### [4] [Abundant hydrocarbons in a buried galactic nucleus with signs of carbonaceous grain and polycyclic aromatic hydrocarbon processing](https://arxiv.org/abs/2602.04967)
*I. García-Bernete,M. Pereira-Santaella,E. González-Alfonso,M. Agúndez,D. Rigopoulou,F. R. Donnan,G. Speranza,N. Thatte*

**TL;DR**: 利用JWST首次在超亮红外星系(ULIRG)IRAS 07251-0248中探测到多种气相小分子碳氢化合物(C6H6、C6H2、C4H2、C2H2、CH4、CH3)和固相C-H吸收特征，其异常丰度表明碳质颗粒和多环芳烃(PAHs)的侵蚀碎裂是主要来源，且与宇宙射线(CR)电离率相关，这些高速外流(160 km/s)的碳氢化合物可能是氢化非晶颗粒的形成途径。


<details>
  <summary>Details</summary>

Background: 星际介质（ISM）中碳氢化合物（hydrocarbons）的化学演化及其与碳质颗粒和多环芳烃（PAHs）的相互作用机制尚缺乏明确的观测约束。

Data: 数据来源于JWST（James Webb Space Telescope）的NIRSpec（近红外光谱仪）和MIRI/MRS（中红外仪器/中分辨率光谱仪）对本地超亮红外星系（ULIRG）IRAS 07251-0248的红外波段（3-28微米）观测。

Method: 采用JWST（James Webb Space Telescope）的NIRSpec（近红外光谱仪）和MIRI/MRS（中红外仪器/中分辨率光谱仪）在3-28微米波段进行红外观测，结合气体相烃类分子丰度与宇宙射线（CR）电离速率的相关性分析。

Result: 在本地超亮红外星系（ULIRG）IRAS 07251-0248中首次探测到多种小分子气相碳氢化合物（如苯、三乙炔等）及固态相深非晶C-H吸收，其异常高丰度表明碳质颗粒和多环芳烃（PAHs）的侵蚀和碎裂是主要来源。
...
### [5] [Multi-spin stellar velocity maps of the most massive galaxies](https://arxiv.org/abs/2602.04968)
*Davor Krajnovic,Eric Emsellem,Roland Bacon,Leindert A. Boogaard,Peter M. Weilbacher,Lutz Wisotzki*

**TL;DR**: MUSE M3G巡天研究发现，最亮星系团星系（BCGs）通常呈现多自转反转和主轴旋转等复杂运动学结构，而非BCGs中存在快速旋转体，表明星系质量、亮度及局部环境位置共同影响其自转特性，并揭示了BCGs与非BCGs演化历史的差异。


<details>
  <summary>Details</summary>

Background: 研究聚焦于Shapley超星系团核心区域三个富星系团中最亮星系（BCGs）与非最亮星系（non-BCGs）的恒星运动学特征及其与星系演化历史的关联。

Data: 数据来源于MUSE（Multi Unit Spectroscopic Explorer）最巨大星系巡天（M3G Survey）的25个Ks波段亮度高于-25.7星等、恒星质量超过约6×10^11太阳质量的星系，这些星系位于Shapley超星系团核心区域的三个富星系团中，分为最亮团星系（BCGs）和亮度排名较低的星系（non-BCGs）。

Method: 采用积分场光谱仪(MUSE)获取恒星运动学数据，结合高斯-埃尔米特矩(h3)分析轨道分布，通过多自转速度图识别非轴对称形态和正/逆行轨道族混合结构。

Result: 在M3G巡天样本中，最亮星系团星系（BCGs）普遍呈现主轴旋转和多重自旋反转的复杂运动结构，而非BCGs中存在六例快速旋转体（fast rotators），其轨道分布以顺行短轴管（prograde short-axis tubes）为主。
...
### [6] [Clustering of z~6.6 Quasars and [O III] Emitters Constrains Host Halo Masses and Duty Cycles in 25 ASPIRE Fields](https://arxiv.org/abs/2602.04974)
...
**TL;DR**: 利用JWST ASPIRE数据测量了5.3<z<7.0的[O III]发射体自相关和6.51<z<6.82类星体-[O III]交叉相关，发现[O III]发射体最小晕质量(log(M_h,min^[O III]/M⊙)=10.5±0.1)和类星体(log(M_h,min^QSO/M⊙)=12.1±0.3)，表明紫外明亮阶段对超大质量黑洞(SMBH)早期生长的贡献有限。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST ASPIRE宽场无缝光谱(WFSS)数据，结合数值模拟方法，探究高红移(5.3<z<7.0)宇宙中[O III]发射体与类星体的空间分布特性及其对超大质量黑洞(SMBH)早期形成机制的约束。

Data: 数据来源于JWST ASPIRE宽场无缝光谱（WFSS）观测项目（测量[O III]发射体自相关和类星体-[O III]发射体互相关）和FLAMINGO-10k大体积模拟（2.8 cGpc立方体）。

Method: 采用合成源注入（synthetic source injection）校准选择函数，并结合FLAMINGO-10k大体积模拟（2.8 cGpc盒）构建真实模拟观测，捕捉非线性结构增长和小尺度依赖的偏置，并推导包含宇宙方差（cosmic variance）的协方差矩阵。

Result: [O,{\sc iii}]发射体的自相关长度为$r_0^{\rm GG}=4.7^{+0.4}_{-0.5},h^{-1}$cMpc（固定斜率$\gamma_{\rm GG}=1.8$），类星体与[O,{\sc iii}]发射体的互相关长度为$r_0^{\rm QG}=8.7^{+0.8}_{-0.9},h^{-1}$cMpc（斜率$\gamma_{\rm QG}=2.0$）。
...
### [7] [ASPIRE: The Environments and Dark Matter Halos of Luminous Quasars in the Epoch of Reionization](https://arxiv.org/abs/2602.04979)
...
**TL;DR**: 研究发现红移（redshift）大于6.5的25个明亮类星体（quasar）周围存在显著星系（galaxy）过密度（overdensity），其中7个类星体位于原星系团（protocluster）中心，证实早期明亮类星体是星系过密区的有效示踪体，但不同区域密度差异显著。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST和ALMA观测数据揭示红移（redshift）z > 6.5的早期明亮类星体（quasar）周围星系环境的多样性与高密度特征。

Data: 数据来源于JWST/NIRCam WFSS观测获取的487个红移5.3≲z≲7.0的[OIII]发射线星系，以及ALMA马赛克观测识别的16个与类星体同红移的[CII]发射线星系。

Method: 采用JWST/NIRCam WFSS（宽场狭缝光谱仪）和ALMA（阿塔卡马大型毫米波/亚毫米波阵列）观测数据，结合[OIII]和[CII]发射线星系的空间分布与红移匹配分析，并运用互相关函数（CCF）和自相关函数方法量化类星体与星系的关联性。

Result: 在红移(redshift)z∼7的类星体(quasar)周围发现了显著的星系超密度(protocluster)现象，其中7个类星体所在区域的星系密度超过平均值的5倍以上。
...
### [8] [The CON-quasar stage of IRAS 07251-0248 E](https://arxiv.org/abs/2602.04997)
*Eduardo González-Alfonso,Miguel Pereira-Santaella,Ismael García-Bernete,Jacqueline Fischer,Giovanna Speranza*

**TL;DR**: 研究发现IRAS 07251-0248 E拥有一个极紧凑（半径<27 pc）、高亮度（亮度温度>200 K）且被尘埃严重遮蔽（氢柱密度$N_H > 10^{25}$ cm^-2）的活跃星系核（AGN），其亚毫米与中红外连续辐射分别探测到核区深层与光球层，并通过温室效应模型确认该核区（半径~13 pc）的准星级别光度（~10$^{12}$ Lsun）和超高表面亮度（$Σ_{bol}$~5e8 Lsun pc^-2），同时观测到分子吸收线揭示出速度~160 km/s的外流气体及周围光学厚度$τ_{100 μm}$~10的厚盘/环结构。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用多波段观测揭示极亮红外星系（ULIRG）IRAS 07251-0248 E中高度遮蔽的致密活动星系核（AGN）及其周围结构。

Data: 数据来源于ALMA（Atacama Large Millimeter/submillimeter Array）在667微米波段的连续谱测量、JWST（James Webb Space Telescope）的中红外（mid-IR）观测以及Herschel/PACS（Photodetector Array Camera and Spectrometer）的远红外（far-IR）分子吸收线观测。

Method: 采用连续谱模型（continuum model）结合光子囚禁效应（"greenhouse" effect）分析ALMA亚毫米波和JWST中红外连续谱，并基于尘埃温度剖面（$T_{dust}$ profile）模拟分子吸收带。

Result: ALMA观测揭示IRAS 07251-0248 E的667微米连续谱显示了一个极其致密（半径<27 pc）、明亮（亮度温度>200 K）的核区，其前方存在吸收包层，周围环绕着近面朝上的盘或环状结构（半径~75 pc）。
...
### [9] [Modeling the recent interactions between the Magellanic Clouds and Milky Way](https://arxiv.org/abs/2602.05021)
*Bethany Garver,David Nidever,Victor Debattista,Nathan Deg*

**TL;DR**: 通过数值模拟（N-body simulations）结合遗传算法（genetic algorithm）重建大麦哲伦云（LMC）和小麦哲伦云（SMC）过去25亿年的相互作用历史，发现两者在9.4亿年前和1.4亿年前发生两次近距离交会，导致LMC盘面翘曲（振幅1.3千秒差距）、SMC潮汐膨胀等观测特征。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于大麦哲伦云（LMC）和小麦哲伦云（SMC）与银河系（MW）的相互作用及其产生的恒星子结构和气体流（Magellanic Stream），旨在通过数值模拟揭示其轨道演化历史。

Data: 数据来源于N-body模拟（N-body simulations）和遗传算法（genetic algorithm）结合生成的约8,000次模拟结果。

Method: 采用N体模拟（N-body simulations）结合遗传算法（genetic algorithm）分析大麦哲伦云（LMC）和小麦哲伦云（SMC）过去25亿年的轨道演化。

Result: 通过数值模拟（N-body simulations）结合遗传算法（genetic algorithm）重建了大麦哲伦云（LMC）和小麦哲伦云（SMC）过去25亿年的相互作用历史，发现二者在9.4亿年前和1.4亿年前发生过两次近距离交会，并成功复现了LMC盘面翘曲（振幅1.3千秒差距）、LMC环状密度增强、SMC潮汐膨胀以及SMC东侧距离弥散增大等观测特征。
...
### [10] [Shocks, Winds, and a Torus: The Large Binocular Telescope Interferometer (LBTI) Resolves the Active Nucleus of NGC 4151](https://arxiv.org/abs/2602.05094)
...
**TL;DR**: 利用大型双筒望远镜干涉仪(LBTI)对赛弗特1星系NGC 4151进行中红外(MIR)观测，首次在N波段(8.7和10.5μm)以28.8米等效口径的分辨率(5.8-9.1 pc)解析出32 pc直径的尘埃环(P.A.=125°)和窄线区内的尘埃云，通过四波段匹配分辨率测得中心100 pc空间分辨光谱能量分布(SED)，发现尘埃温度与消光剖面同时受吸积盘加热和射电喷流激波加热影响，并通过与NGC 1068的比较验证了活动星系核统一模型(Unified Model of AGN)中环尺度与光度(r∝L^0.5)的标度关系，表明现有中红外辐射转移模型需加入喷流加热机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用大型双筒望远镜干涉仪（LBTI）的中红外（MIR）观测，探索塞弗特1星系NGC 4151的中心区域结构及其尘埃环（dusty torus）和窄线区（narrow line region）内尘埃云的物理特性，并检验活动星系核统一模型（Unified Model of AGN）的普适性。

Data: 数据来源于大型双筒望远镜干涉仪（LBTI）在N波段（8.7和10.5微米）获取的66-104毫角秒（5.8-9.1秒差距）分辨率的开环斐索（Fizeau）图像，以及LM波段（3.7和4.8微米）50-62毫角秒（4.4-5.4秒差距）分辨率的自适应光学（AO）成像数据。

Method: 采用大型双筒望远镜干涉仪（LBTI）获取中红外（MIR）开放环路斐索（Fizeau）图像（分辨率66-104 mas）和自适应光学（AO）成像（分辨率50-62 mas），结合四波段匹配分辨率测量空间分辨光谱能量分布（SED），并通过修正黑体拟合分析尘埃温度与消光剖面。

Result: result：在NGC 4151中解析出一个直径32 pc、方位角125°的尘埃环（dusty torus），并探测到窄线区内的尘埃云，通过匹配四个波段的分辨率测量了中心约100 pc的空间分辨光谱能量分布（SED），修正的黑体拟合揭示了尘埃温度和消光轮廓，表明吸积盘加热和射电喷流（radio jet）引起的激波加热共同作用，与NGC 1068的比较支持活动星系核统一模型（Unified Model of AGN），但尘埃环尺寸遵循r∝L^0.5的比例关系，观测结果强调需修正中红外辐射转移模型以纳入喷流相关加热。
...
**TL;DR**: 研究发现早期宇宙中$z>6$的类星体（quasars）超大质量黑洞（$\gtrsim 10^{9}\,M_\odot$）的初始质量（$M_0 \sim 10^{6}\,M_\odot$）和形成环境（重子密度因子$f_ρ\gtrsim 50$）支持量子简并费米子暗物质（quantum-degenerate fermion dark matter）坍缩形成的大质量种子黑洞（massive seed black holes）模型。


<details>
  <summary>Details</summary>

Background: 研究探讨了高红移（$z>6$）超大质量黑洞（SMBH）的早期形成机制，特别是如何通过大质量种子黑洞（massive seed black holes）和原初气体吸积来解释类星体（quasars）中观测到的$\gtrsim 10^{9}\,M_\odot$黑洞。

Data: 数据来源于詹姆斯·韦伯太空望远镜（JWST）对小红点（Little Red Dots, LRDs）的观测，以及类星体J0313--1806（红移z=7.64）和J0100+2802（红移z=6.30）的质量和光度观测数据。

Method: 采用基于邦迪吸积率(Bondi rate)和爱丁顿限制率(Eddington rate)的最小宇宙学框架，通过贝叶斯推断拟合观测数据以推断初始条件。

Result: 早期超大质量黑洞（SMBH）的生长需要初始质量约$10^{6}\,M_\odot$的大质量种子（massive seed），形成于红移$z\sim20{-}30$且重子物质密度超出宇宙平均值50倍以上的环境中。
...
### [12] [IFU Spectroscopic Study of the Planetary Nebula Abell 30: Mapping the Ionisation and Kinematic Structure of the Inner Complex](https://arxiv.org/abs/2602.05277)
*Kam Ling Chan,Andreas Ritter,Quentin Andrew Parker,Katrina Exter*

**TL;DR**: 通过积分流量和速度通道图揭示了行星状星云Abell 30（A30）内结系统的电离结构、化学不均匀性及极端湍流运动，发现极区结（J1、J3）以复合线（ORLs）为主而赤道结（J2、J4）以碰撞激发线（CELs）为主，电子温度分布显示15,000 K冷核被20,000 K热层包围，速度结构差异和离子湍流（σ_vrad≈140 km/s）表明A30演化过程中存在激波加热和多次喷发历史。


<details>
  <summary>Details</summary>

Background: 研究聚焦于行星状星云Abell 30（A30）内结系统的电离结构、化学不均匀性及动力学特征，以揭示其演化过程中可能的激波加热和多阶段抛射历史。

Data: 数据来源于威廉赫歇尔望远镜（WHT）的INTEGRAL摄谱仪观测，部分覆盖了A30行星状星云内结系统（knots）的J1、J2，完整覆盖了J3、J4，并检测到C II、He I、He II、N III、O II的光学复合线（ORLs）以及[Ar IV]、[Ar V]、[N II]、[Ne III]、[Ne IV]、[O III]的碰撞激发线（CELs）。

Method: 采用积分视场单元（IFU）数据立方体分析技术，结合光学复合线（ORLs）和碰撞激发线（CELs）的流量比图及速度通道图，揭示了A30内结系统的电离结构、化学不均匀性、电子温度分布和离子速度结构。

Result: 光学复合线(ORLs)集中在极区(J1、J3)，而碰撞激发线(CELs)主导赤道区(J2、J4)。
...
### [13] [Galactic seismology: can a disc-crossing impulse explain the large-scale perturbations in the Milky Way's disc?](https://arxiv.org/abs/2602.05296)
*Pavadol Yamsiri,Joss Bland-Hawthorn,Thor Tepper-Garcia*

**TL;DR**: 研究发现人马座矮星系（Sgr）约10亿年前最后一次穿越银河系（MW）盘面可能是引发大规模盘面扰动（如螺旋臂、盘面波纹、相位螺旋和$L_{z}-\bar{V}_{R}$波）的主要原因，但无法完全解释$L_{z}-\bar{V}_{R}$波的形成。


<details>
  <summary>Details</summary>

Background: 研究探讨人马座（Sgr）矮星系穿越银河系（MW）盘面时产生的单次冲击是否能够同时解释当前观测到的多种盘面扰动现象（如螺旋臂、盘面波纹、相位螺旋和角动量-径向速度波）。

Data: 研究采用数值模拟（simulation）方法，数据来源于模拟人马座（Sgr）矮星系类似扰动体单次穿越银河系（MW）盘面的事件。

Method: 采用数值模拟方法分析人马座矮星系（Sgr）单次穿越银河系（MW）盘面时产生的动力学扰动，重点考察其对螺旋臂、盘面波纹（corrugations）、相位螺旋（phase spiral）以及角动量-径向速度波（$L_{z}-\bar{V}_{R}$ wave）的综合影响。

Result: 人马座矮星系（Sgr）约10亿年前最后一次显著穿越银河系（MW）盘面可能是引发大规模盘扰动（如螺旋臂、盘面波纹和相位螺旋）的主要原因，但无法完全解释Lz-VR波的形成。
...
### [14] [B-field Orion Protostellar Survey (BOPS). IV: The Relative Orientation Between Magnetic Fields and Density Structures in Young Protostellar Envelopes](https://arxiv.org/abs/2602.05316)
...
**TL;DR**: 通过分析猎户座8颗年轻原恒星（protostars）的磁场-密度结构相对排列关系，发现弱磁化（weakly magnetized）包层（envelopes）主要呈现平行或随机排列，而强磁化（strongly magnetized）包层在中等密度下即出现垂直构型，表明密度与磁化水平共同调控原恒星包层形态及早期恒星形成中重力与磁场的耦合作用。


<details>
  <summary>Details</summary>

Background: 研究探讨了年轻原恒星（protostars）中密度结构与磁场相对排列的关系及其在恒星形成早期阶段对重力与磁场耦合的影响。

Data: 数据来源于ALMA B-field Orion Protostellar Survey的870微米（μm）尘埃连续谱辐射观测数据，通过柱密度（column density）图分析原恒星包层（envelope）尺度的磁场与密度结构相关性。

Method: 采用柱密度（column density）图和相对方向直方图（HRO）方法量化包层尺度（~10^3 au）上磁场取向与密度结构的关联性。

Result: 在猎户座原恒星ALMA磁场巡测中，密度结构与磁场（B-field）的相对排列形态由柱密度和磁化强度共同决定，弱磁化包层（envelope）呈现平行或随机排列，而强磁化包层在中等密度下仍保持垂直构型。
...
### [15] [The sub-arcsecond ILT view of the Boötes Deep Field: A link between low-frequency kiloparsec radio morphology and AGN driven ionised outflows](https://arxiv.org/abs/2602.05318)
...
**TL;DR**: 研究发现活动星系核（AGN）的[O III]外流（outflow）与千秒差距尺度（kiloparsec-scale）射电辐射存在关联，且后者主要由AGN驱动，而无[O III]外流的AGN则以恒星形成相关的弥散射电辐射为主。


<details>
  <summary>Details</summary>

Background: 研究探讨了活动星系核（AGN）外流（outflows）通过AGN反馈（feedback）调控宿主星系演化的作用及其与射电辐射的关联。

Data: 数据来源于国际低频阵列射电望远镜（ILT）在144 MHz波段获取的宽视场、亚角秒分辨率成像数据，检测到4074个源，中心灵敏度为33.8微焦耳每波束。

Method: 采用国际低频射电望远镜（ILT）在144 MHz频段获取亚角秒分辨率图像，进行射电源形态学分析，并利用千秒差距尺度射电辐射与[O III]外流相关性研究。

Result: 在射电大尺度（6角秒）和小尺度（0.3角秒）均被探测到的活动星系核（AGN）中，90±7%存在[O III]外流，而仅在大尺度被探测到的AGN中这一比例为63±9%。
...
### [16] [A Chemistry-First Centered Icy Chemical Inventory of Protostellar Sources with JWST](https://arxiv.org/abs/2602.05383)
*Andrew M. Turner,Yao-Lun Yang,Rachel Gross,Nami Sakai,Ralf I. Kaiser*

**TL;DR**: 利用詹姆斯·韦伯太空望远镜(JWST)对4颗0级原恒星(IRAS 15398-3359、Ser-emb7、L483和B335)的5-28μm冰层光谱分析发现，其冰层主要由H2O、CO2、CH3OH等简单分子构成，并检测到NH2OH等复杂有机分子(COMs)，揭示了基于自由基-自由基反应的COMs形成路径，同时强调了冰层中COMs识别的证据严谨性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星形成早期阶段冰幔（ice mantles）化学组成的识别及其对化学演化过程的约束作用。

Data: 数据来源于詹姆斯·韦伯太空望远镜(JWST)的中红外仪器(MIRI)中分辨率光谱仪(MRS)对四个0级原恒星(IRAS 15398-3359、Ser-emb7、L483和B335)的观测光谱。

Method: 采用詹姆斯·韦伯太空望远镜(JWST)的中红外仪器(MIRI)中分辨率光谱仪(MRS)获取数据，并通过同时拟合连续谱和硅酸盐吸收来产生5-28微米(360-2000 cm$^{-1}$)波段的光学深度中红外冰光谱。

Result: 在四个0级原恒星（IRAS 15398-3359、Ser-emb7、L483和B335）的冰层中检测到水（H2O）、二氧化碳（CO2）、甲醇（CH3OH）、甲酸/甲酸盐（HCOOH/HCOO-）、氨/铵（NH3/NH4+）和甲醛（H2CO）等简单分子为主要成分，而复杂有机分子（COMs）如羟胺（NH2OH）、甲胺（CH3NH2）和乙醇（CH3CH2OH）含量较少。
...
### [17] [ELFO: A Python package for emission line fitting optimization in integral field spectroscopy data](https://arxiv.org/abs/2602.05396)
*Hui Guo,Guilin Liu,Jianghui Xu,Chao Geng,Zhicheng He,Shiyin Shen,Lei Hao*

**TL;DR**: 开发了用于积分视场光谱(IFS)数据拟合的Python工具ELFO，通过结合邻近光谱信息优化初始猜测并确保空间连续性，成功修正异常拟合、揭示未解析子结构并凸显大尺度运动学特征。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用积分视场光谱（IFS）技术提升星际介质物理和运动学特性分析的精度，重点解决传统独立谱线拟合方法导致的空间不连续性问题。

Data: 数据来源于多单元光谱探测器（Multi-Unit Spectroscopic Explorer）获取的两颗类星体的积分视场光谱（IFS）数据。

Method: 采用基于邻近光谱结果确定多组初始猜测并选择空间平滑性最优解的Emission Line Fitting Optimization (ELFO)方法进行积分视场光谱(IFS)发射线拟合。

Result: ELFO通过利用邻近光谱结果确定多个初始猜测并选择空间平滑的拟合结果，成功校正了异常拟合、揭示了未解析的子结构并使大尺度运动学结构更明显。
...
### [18] [Mergers Drive Structural Complexity but Not Starbursts in Lyman-$α$ Emitters at $3 < z < 4$: A JWST Spatially Resolved View](https://arxiv.org/abs/2602.05411)
*Qi Song,F. S. Liu,Jian Ren,Tianfu Gao,Pinsong Zhao,Qifan Cui,Yubin Li,Hao Mo,Guanghuan Wang*

**TL;DR**: 研究发现莱曼阿尔法发射体（LAEs）在红移3-4的并合系统中存在显著结构扰动和尘埃分布变化，但未引发剧烈星暴活动，这些形态变化可能促进莱曼阿尔法光子逃逸。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（$z > 3$）莱曼阿尔法发射体（LAEs）的并合（merger）动力学及其对星系结构、尘埃分布和莱曼阿尔法光子逃逸的影响。

Data: 数据来源包括JWST/NIRCam宽带和中波段成像数据、VLT/MUSE-Deep巡天在GOODS-S天区的观测数据以及HST/ACS存档数据。

Method: 采用空间分辨的光谱能量分布(SED)拟合方法，结合JWST/NIRCam宽波段和中波段成像与HST/ACS存档数据，使用Bagpipes软件包进行物理建模。

Result: 在红移3 < z < 4的莱曼阿尔法发射体(LAEs)中，观测到两个次要合并(minor merger)和一个主要合并(major merger)，且这些合并尚未触发显著星暴活动，但已引起显著内部结构扰动和尘埃分布紊乱。
...
### [19] [XGAP with uGMRT I: Old AGN plasma in merging galaxy groups](https://arxiv.org/abs/2602.05609)
...
**TL;DR**: 通过多波段分析三个动态活跃星系群（SDSSTG8102、SDSSTG16393和SDSSTG28674），发现其最亮群星系（BGGs）的射电形态不对称扭曲、温度变化及合并特征，揭示了活动星系核（AGN）喷流与星系群内介质（IGrM）相互作用的演化多样性。


<details>
  <summary>Details</summary>

Background: 星系群（galaxy groups）因其较弱的引力势能更易受活动星系核（AGN）喷流影响，成为研究AGN反馈与星系群共同演化的重要对象。

Data: 数据来源包括uGMRT 400 MHz射电观测、144 MHz LOFAR低频阵列观测以及XMM-Newton X射线观测，研究对象为X-GAP样本中三个动力学活跃星系群（SDSSTG8102、SDSSTG16393和SDSSTG28674）的射电源与最亮群星系（BGGs）的关联特征。

Method: 采用多波段（multi-wavelength）分析方法，结合uGMRT 400 MHz、LOFAR 144 MHz和XMM-Newton观测数据，研究三个活跃星系群（SDSSTG8102、SDSSTG16393、SDSSTG28674）中与最亮群星系（BGGs）相关的射电源。

Result: 星系群中活动星系核(AGN)喷流与星系际介质(IGrM)的相互作用导致射电形态不对称扭曲、遗留等离子体或复活射电凤凰(radio phoenix)等现象，其光谱多样性反映了AGN活动因喷发周期、源年龄和热介质约束而处于不同衰减阶段。
...
### [20] [MIGHTEE/COSMOS-3D: The discovery of three spectroscopically confirmed radio-selected star-forming galaxies at z=4.9-5.6](https://arxiv.org/abs/2602.05808)
...
**TL;DR**: 发现了三个红移（redshift）$z=4.9$-$5.6$的高红移射电源（HzRSs），其1.3 GHz射电光度为$L_{\rm 1.3 \, GHz}\approx2$-$5\times10^{24} \, \rm W \, Hz^{-1}$，通过多波段数据确认其射电辐射主要由恒星形成活动驱动而非活动星系核（AGN），为研究早期宇宙中不受尘埃影响的恒星形成提供了新途径。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用射电观测（radio observations）作为独立于尘埃的探针，探索高红移（$z>4.5$）宇宙中恒星形成与活动星系核（AGN）活动的交叉光度特征。

Data: 数据来源于哈勃（Hubble）、JWST NIRCam+MIRI和地面测光档案的光谱能量分布（SED）拟合，以及JWST COSMOS-3D的宽场无缝光谱（wide-field slitless spectroscopy）观测。

Method: 采用光谱能量分布（SED）拟合和宽视场无缝光谱（wide-field slitless spectroscopy）技术，结合哈勃、JWST NIRCam+MIRI及地面测光数据，通过Hα发射线确认高红移射电源（HzRSs）。

Result: 在红移（redshift）$z=4.9$-$5.6$处探测到三个高红移射电源（HzRSs），其射电光度（radio luminosity）$L_{\rm 1.3 \, GHz}\approx2$-$5\times10^{24} \, \rm W \, Hz^{-1}$，表明这些源位于或高于红移$z=4$-6的恒星形成主序带（star-forming main sequence）0.5-1.0 dex，并经历了近期恒星形成爆发。
...
### [21] [The JWST EXCELS survey: The ages and abundances of $3<z<5$ massive quiescent galaxies show that downsizing was already in place by $z\simeq4$](https://arxiv.org/abs/2602.05934)
...
**TL;DR**: 通过JWST/NIRSpec光谱分析发现3<z<5静止星系（quiescent galaxies）存在恒星年龄与质量的正相关关系（更高质量星系更早形成恒星），证实了考古学"降序演化"（downsizing）趋势在z≈4时已确立，并揭示该时期低质量（log10(M*/M⊙)≲10.5）星系缺乏古老恒星群体。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST/NIRSpec中分辨率光谱观测高红移（$3<z<5$）静止星系（quiescent galaxies）的恒星形成历史、金属丰度及化学演化特征，以揭示宇宙早期大质量星系形成与演化的关键物理过程。

Data: 数据来源于JWST/NIRSpect（詹姆斯·韦伯太空望远镜近红外光谱仪）在波长1-5微米范围内获取的14个红移3到5、恒星质量大于10^10太阳质量的静止星系的中等分辨率光谱数据，这些数据是EXCELS巡天的一部分。

Method: 采用全光谱拟合(full-spectral fitting)方法结合自定义最优提取(custom optimal-extraction)技术分析静止星系(quiescent galaxies)的恒星形成历史和金属丰度。

Result: 发现恒星年龄与恒星质量存在明确相关性，质量更大的星系在更早时期完成恒星质量积累，证实了考古学"降序演化"(downsizing)趋势在红移$z\simeq4$时已确立。
...
### [22] [Tracing AGN Feedback Power with Cool/Warm Outflow Densities: Predictions and Observational Implications](https://arxiv.org/abs/2602.05954)
*Ivan Almeida,Tiago Costa,Chris M. Harrison,Samuel R. Ward*

**TL;DR**: 研究发现活动星系核(AGN)驱动的冷气体外流密度与AGN光度正相关，揭示了其作为热激波风间接诊断的潜力，并表明观测到的外流速率可能被高估数个量级。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）驱动的能量守恒外流（energy-conserving outflows）如何通过热激波风（hot, shocked wind）与冷/暖气体（cool/warm gas）的相互作用影响星系演化，并探索冷外流气体密度与AGN光度间的关联作为间接探测热激波风的关键诊断手段。

Data: 研究使用了基于移动网格代码AREPO的流体动力学模拟（hydrodynamic simulation）数据，针对小尺度活动星系核（AGN）风（速度≈10^4 km/s）与理想化、团块状星际介质（ISM）的相互作用进行建模，并通过新的细化方案在快速冷却、高速运动气体中实现了≤0.1 pc的分辨率。

Method: 采用流体动力学模拟（hydrodynamic simulations）结合移动网格代码AREPO，并引入针对快速冷却高速运动气体的新型细化方案（refinement scheme），分辨率达到≤0.1 pc。

Result: 冷态外流气体密度与活动星系核(AGN)光度呈正相关，可作为热态激波风(hot shocked wind)的间接诊断指标。
...
### [23] [Water absorption confirms cool atmospheres in two little red dots](https://arxiv.org/abs/2602.06024)
...
**TL;DR**: 通过JWST光谱在2个高红移小红点(LRDs)中首次观测到1.4微米水吸收带，证实其存在贡献20-30%连续谱的低温(~2000-4000K)致密气体成分，排除了尘埃红化模型并提供了黑洞质量新诊断依据。


<details>
  <summary>Details</summary>

Background: 詹姆斯·韦伯太空望远镜（JWST）发现的高红移致密天体“小红点”（LRDs）的红色光学连续谱成因存在争议，涉及尘埃红移标准热吸积盘或超大质量黑洞周围致密氢包层固有冷热辐射两种解释。

Data: 数据来源于詹姆斯·韦伯太空望远镜（JWST）对四个红移（redshift）约为2的小红点（LRDs）样本的高信噪比光谱观测，其中两个样本显示出显著的1.4微米静止波长吸收特征。

Method: 采用JWST光谱分析结合大气模型（atmosphere models）和复合模型（composite model）来识别水吸收带并约束发射气体温度范围。

Result: 在部分LRDs（Little red dots）中观测到1.4微米水吸收带，证实存在温度低于3000K的冷密气体成分贡献20-30%连续谱，排除了单纯尘埃红化模型，表明其红色连续谱本质源于低温热辐射。
...
### [24] [Oort Cloud Bombardment by Dark Matter](https://arxiv.org/abs/2602.04922)
*Jeremy Mould*

**TL;DR**: 论文发现奥尔特云(Oort cloud)大小的太阳系中原始黑洞(PBHs)对彗星的扰动频率与观测相符，暗示PBHs可能是暗物质(dark matter)的组成部分。


<details>
  <summary>Details</summary>

Background: 研究探讨了原初黑洞（PBHs）作为暗物质（dark matter）候选者对太阳系（solar system）及奥尔特云（Oort cloud）的潜在影响及其与彗星观测频率的关联。

Data: 数据来源于对奥尔特云(Oort cloud)遭受暗物质(dark matter)轰击的数值模拟(simulation)结果。

Method: 采用数值模拟（numerical simulation）方法分析奥尔特云（Oort cloud）受原始黑洞（PBHs）轰击后原彗星（protocomets）的扰动与内太阳系进入频率。

Result: 原初黑洞（PBHs）可能是暗物质的一部分，其进入太阳系的频率取决于太阳系边界的大小，奥尔特云（Oort cloud）尺度下的模拟表明，若PBHs占比足够高，其撞击可释放原彗星（protocomets）并导致其进入内太阳系，频率与观测到的彗星出现频率一致。
...
### [25] [Establishing a relationship between the cosmological 21 cm power spectrum and interferometric closure phases](https://arxiv.org/abs/2602.04965)
*Pascal M. Keller,Bojan Nikolic,Nithyanandan Thyagarajan*

**TL;DR**: 通过建立闭包相位（closure phase）与宇宙学21厘米功率谱的数学关系，发现闭包相位功率谱估计在信号-前景比（signal-to-foreground ratio）较小时近似等于宇宙学功率谱与前景相关窗函数的卷积，但会导致模式混合（mode-mixing）增加和前景功率沿视线维度（line-of-sight dimension）的显著扩散。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过校准无关的闭合相位（closure phase）方法探测宇宙21厘米（21 cm）信号的功率谱，以解决传统校准误差对动态范围和谱线信号检测的限制问题。

Data: 数据来源为模拟HERA（Hydrogen Epoch of Reionization Array）观测数据。

Method: 采用延迟谱(delay spectrum)方法将宇宙信号分量作为闭合相位(closure phase)的微扰处理，并利用标准傅里叶变换技术建立其功率谱估计与宇宙学功率谱之间的数学关系。

Result: 闭相（closure phase）功率谱估计在信噪比（signal-to-foreground ratio）较小时近似等于宇宙学功率谱与前景依赖窗函数的卷积，导致模式混合（mode-mixing）增加和前景功率沿视线维度（line-of-sight dimension）的显著扩散。
...
### [26] [Differentiable Stochastic Halo Occupation Distribution with Galaxy Intrinsic Alignments](https://arxiv.org/abs/2602.04977)
*Sneh Pandya,Jonathan Blazek*

**TL;DR**: 开发了可微分晕族占据分布模型（diffHOD-IA），通过整合星系本征排列（IA）实现了从参数到星系场的端到端自动微分，验证了其在玻尔兹曼-普朗克模拟中与参考模型的一致性，并展示了在哈密顿蒙特卡洛分析中的优越性能。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发可微分晕族占位分布（HOD）模型diffHOD-IA，整合星系本征排列（IA）效应以支持弱引力透镜场级分析。

Data: 数据来源于Bolshoi-Planck模拟和TNG300模拟。

Method: 采用可微分晕族占据分布（HOD）模型结合星系本征排列（IA）建模，并扩展至可微分两点相关函数计算，通过自动微分实现从参数到星系场的端到端梯度计算。

Result: 开发了可微分晕族占据分布模型diffHOD-IA，整合了星系本征排列（intrinsic alignment）并支持端到端自动微分，适用于弱引力透镜分析。
...
### [27] [The Impact of Galaxy Formation on Galaxy Biasing, and Implications for Primordial non-Gaussianity Constraints](https://arxiv.org/abs/2602.04987)
*Lucia A. Perez,Shy Genel,Elisabeth Krause,Rachel S. Somerville*

**TL;DR**: 通过CAMELS-SAM管道和分离宇宙N体模拟，揭示了不同星系形成参数（如恒星和活动星系核反馈）对星系偏差参数b₁和bφ的影响，并发现sSFR（特定恒星形成率）选择对星系建模变化具有鲁棒性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过星系偏置参数（galaxy bias parameters）$b_1$和$b_φ$的精确建模来约束原初非高斯性（primordial non-Gaussianity）参数$f_{\textrm{NL}}$，以解决其与星系形成物理的退耦问题。

Data: 使用CAMELS-SAM管道生成的55组独特参数化半解析模型(SAM)数据，以及通过独立宇宙N体模拟(separate-universe N-body simulations)在205 h^{-1} cMpc盒子和1280^3粒子分辨率下测量的b_φ数据。

Method: 采用分离宇宙N体模拟（separate-universe N-body simulations）和55种不同参数的圣克鲁兹半解析模型（Santa Cruz semi-analytic model）来测量星系偏置参数$b_φ$和$b_1$，并探究星系形成参数变化对其影响。

Result: 通过CAMELS-SAM管道研究发现，不同星系形成参数变化会影响星系偏置参数$b_φ$和$b_1$，且SC-SAM模型的恒星质量、恒星形成率及比恒星形成率与晕质量关系的变化趋势能有效追踪$b_1$和$b_φ(b_1)$随红移和选择的变化，同时sSFR选择对星系建模变化具有较强鲁棒性。
...
### [28] [Wising up to CatWISE: using simulation-based inference to interpret the ecliptic bias and confirm the cosmic dipole excess](https://arxiv.org/abs/2602.05070)
*Oliver T. Oayda,Geraint F. Lewis*

**TL;DR**: 首次应用基于模拟的推断（SBI）研究宇宙偶极问题，发现CatWISE2020类星体样本的偶极强度是宇宙微波背景（CMB）预期的两倍但方向显著偏离（≈3σ），并揭示四极各向异性由扫描规律与光度误差（W1/W2星等）的关联导致的爱丁顿偏差（Eddington bias）引起，证实样本存在低估光度误差或未解决的系统误差。


<details>
  <summary>Details</summary>

Background: 首次应用基于模拟的推断（SBI）研究宇宙偶极问题，聚焦于CatWISE2020样本中类星体数量分布的测量及其与ΛCDM模型的潜在冲突。

Data: 数据来源于CatWISE2020（CatWISE）样本中类星体计数在天空中的分布测量。

Method: 采用基于模拟的推断（SBI）和神经网络似然估计器（neural likelihood estimator）分析CatWISE2020样本中类星体计数的偶极分布，并建模由扫描规律（scanning law）与光度误差（photometric uncertainty）引起的爱丁顿偏差（Eddington bias）。

Result: 首次应用基于模拟的推断（SBI）研究宇宙偶极问题，发现CatWISE2020样本的四极各向异性可归因于WISE扫描规律与W1/W2星等光度误差的关联性导致的随天区变化的爱丁顿偏差（Eddington bias）。
...
### [29] [The Galaxy Bias Profile of Cosmic Voids:A Comparison of Void Finders](https://arxiv.org/abs/2602.05117)
...
**TL;DR**: 通过比较五种不同空洞（cosmic voids）识别算法对同一星系样本的分析，发现星系偏差（b_i）在空洞内从中心负值向边界高值的径向梯度变化具有普适性，但基于密度阈值的方法更倾向选择b_i<0的星系，而无密度约束的分水岭（watershed）方法会混入高偏差边界星系，其中自由形式积分密度法（popcorn）在隔离反偏差星系群体时纯度最高。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙空洞（cosmic voids）作为研究星系形成和宇宙学探针的独特价值，特别关注不同空洞识别算法对星系偏置（galaxy bias）环境调制效应的影响。

Data: 数据来源于IllustrisTNG模拟（TNG300-1在$z=0$时）的同一星系样本。

Method: 采用五种互补的虚空识别算法（sparkling、zobov、revolver两种模式及popcorn）对同一星系样本进行对比分析，量化不同定义下个体星系偏差（b_i）与虚空中心距离的函数关系。

Result: 不同空洞定义方法下，个体星系偏置（b_i）的径向梯度（从空洞中心的负值向边界递增）具有普遍性，但密度阈值方法（如popcorn）优先选择b_i<0的星系，而无密度约束的分水岭方法（如zobov和revolver）会混入高偏置边界星系，且负偏置与空洞成员的相关性随积分低密度阈值放宽而系统性增强。
...
### [30] [Fractal dimension of the cosmic web with different galaxy types](https://arxiv.org/abs/2602.05309)
*Ana Elisa Lima,Julianne C. Soares,Ana Carolina S. Tavares,Mariana V. Taveira,Sharon Teles,Amanda R. Lopes,Marcelo B. Ribeiro*

**TL;DR**: 通过分析618,952个红移(z)达4的星系样本，发现不同颜色星系（蓝、绿、红）的分形维数(D)在z<1时呈D_蓝(1.40-2.03)>D_红>D_绿梯度，而在1<z≤4时变为D_蓝>D_绿>D_红且D值更低(0.03-0.44)，证实分形维数能有效诊断星系群对宇宙网演化的追踪差异。


<details>
  <summary>Details</summary>

Background: 研究利用分形维数（fractal dimension）分析宇宙大尺度结构中不同颜色类型星系（蓝、绿、红）的分布模式，探索星系群如何随红移演化追踪宇宙网（cosmic web）的结构变化。

Data: 数据来源于COSMOS2020子样本的618,952个星系（galaxy）的$NUVrK$完整观测数据，覆盖红移（redshift）范围$z=4$。

Method: 采用分形维数（fractal dimension）分析方法，基于NUVrK完备的COSMOS2020子样本（618,952个星系）计算不同颜色类型星系在两种宇宙学距离（光度距离和共动距离）下的数密度分布，以估计红移区间z≶1内蓝、绿、红色星系的分形维数。

Result: 在红移$z<1$时发现分形维数梯度为$D_{\mathrm{blue}}> D_{\mathrm{red}}>D_{\mathrm{green}}$（$D=1.40-2.03$），而在$1<z\leq4$时梯度变为$D_{\mathrm{blue}}>D_{\mathrm{green}}>D_{\mathrm{red}}$且分形维数值更低（$D=0.03-0.44$）。
...
### [31] [Dark Energy After DESI DR2: Observational Status, Reconstructions, and Physical Models](https://arxiv.org/abs/2602.05368)
*Slava G. Turyshev*

**TL;DR**: 该论文基于DESI DR2数据，综合分析了Ia型超新星（SNe Ia）、各向异性重子声波振荡（BAO）、宇宙微波背景（CMB）校准及扰动敏感探针（RSD和弱引力透镜）的相互作用，发现平坦ΛCDM模型存在轻微参数失配，而允许暗能量演化（如CPL参数w0-wa）可改善拟合，并提出两种诊断方法（rd无关的BAO形状观测量FAP(z)和超新星系统误差对w0-wa偏差的线性响应映射）以优化暗能量演化模型约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于晚期宇宙加速膨胀（late-time cosmic acceleration）背景下暗能量（dark energy）性质与修正引力理论（modified-gravity models）的观测约束，结合DESI DR2多探针数据探讨ΛCDM模型参数一致性及动态暗能量（evolving dark energy）的拟合优化空间。

Data: 数据来源包括DESI第二次数据发布（DR2）提供的各向异性重子声波振荡（BAO）距离比、Ia型超新星（SNe Ia）、宇宙微波背景（CMB）校准数据以及扰动敏感探针（红移空间畸变RSD和弱引力透镜）。

Method: 采用各向异性重子声波振荡（anisotropic BAO）距离比、CMB校准、扰动敏感探针（RSD和弱引力透镜）以及超新星（SNe Ia）数据相结合的方法，构建了独立于声波视界尺度（r_d）的BAO形状观测量F_AP(z)≡D_M(z)/D_H(z)，并开发了超新星哈勃图系统误差δμ(z)对暗能量参数(w_0,w_a)偏差的线性响应映射。

Result: DESI DR2提供了红移范围0≲z≲2.5内精度达百分比的各向异性重子声波振荡（BAO）距离比测量，包括在zeff=2.33处的莱曼α森林锚定点。
...
### [32] [Forecast on $f(R)$ Gravity with HI 21cm Intensity Mapping Surveys](https://arxiv.org/abs/2602.05575)
*Yanling Song,Yu Sang,Linfeng Xiao,Boyu Zhang,Bin Wang*

**TL;DR**: 通过中性氢（HI）21厘米强度映射（IM）和宇宙微波背景（CMB）观测联合分析，研究发现BINGO和SKA1-MID等实验能有效约束$f(R)$引力理论中的康普顿波长参数$B_0$，其中SKA Band 2对$B_0$的约束最为严格（$σ(B_0)\simeq 6.37\times 10^{-8}$），并显著降低与宇宙学背景参数的简并性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过修正引力理论（modified gravity theories）解释宇宙晚期加速膨胀现象，特别是探讨$f(R)$引力理论中康普顿波长参数$B_0$对广义相对论偏离的量化能力。

Data: 研究中使用的数据来源包括BINGO和SKA1-MID（Band 1和Band 2）的中性氢（HI）21厘米强度映射（IM）观测数据，以及Planck宇宙微波背景（CMB）先验数据。

Method: 采用费希尔矩阵(Fisher matrix)方法对$f(R)$引力理论中的康普顿波长参数$B_0$进行约束预测，结合中性氢(HI)21厘米强度映射(IM)和宇宙微波背景(CMB)观测数据。

Result: 未来中性氢（HI）21厘米强度映射（IM）观测与宇宙微波背景（CMB）数据结合可显著提升对$f(R)$引力理论中康普顿波长参数$B_0$的约束精度，其中SKA1-MID Band 2配置的灵敏度最高（$σ(B_0)\simeq 6.37\times 10^{-8}$）。
...
### [33] [The kinematic cosmic dipole beyond Ellis and Baldwin](https://arxiv.org/abs/2602.05700)
*Albert Bonnefous*

**TL;DR**: 研究将Ellis-Baldwin公式推广至任意光度分布和光谱轮廓（spectral profiles），基于CatWISE巡天类星体W1波段数据证实宇宙偶极异常（cosmic dipole anomaly）超越幂律假设持续存在。


<details>
  <summary>Details</summary>

Background: 宇宙偶极异常（cosmic dipole anomaly）对标准宇宙学模型构成重大挑战，其科学背景涉及河外光源分布与观测者速度关系的理论拓展。

Data: 数据来源于CatWISE巡天项目W1波段观测的类星体样本。

Method: 采用广义化埃利斯-鲍德温公式(Ellis & Baldwin formula)处理任意光度分布和光谱轮廓，推导有效光谱指数表达式并应用于CatWISE巡天W1波段类星体样本。

Result: 宇宙偶极异常（cosmic dipole anomaly）在非幂律假设下仍然存在，为未来大尺度巡天测量提供了更普适和稳健的理论框架。
...
### [34] [Microlensing constraint on Primordial Black Hole abundance with Subaru Hyper Suprime-Cam observations of Andromeda](https://arxiv.org/abs/2602.05840)
*Sunao Sugiyama,Masahiro Takada,Naoki Yasuda,Nozomu Tominaga*

**TL;DR**: 基于仙女座星系(M31)的高频次观测数据，通过微引力透镜效应分析首次在10^-7--10^-6太阳质量范围内探测到4个显著性原始黑洞(PBH)候选体，并约束其占暗物质比例约为10^-2--10^-1量级。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用微引力透镜效应（microlensing）探测仙女座星系（M31）中的原始黑洞（PBH）暗物质，以约束其丰度和质量尺度。

Data: 数据来源于2014年、2017年和2020年对仙女座星系（M31）进行的共计39.3小时的高频次（约2分钟间隔）Subaru Hyper Suprime-Cam（HSC）观测。

Method: 采用点透镜有限源模型(point-lens finite-source model)对微引力透镜光变曲线建模，并结合多阶段筛选流程识别候选体。

Result: 基于高频率观测数据，在原始黑洞(PBH)假设下推断其质量尺度为$10^{-7}$--$10^{-6}M_\odot$，占暗物质总量的比例约为$\mathcal{O}(10^{-2}{\rm -}10^{-1})$。
...
### [35] [Predicting galaxy bias using machine learning](https://arxiv.org/abs/2602.05881)
...
**TL;DR**: 基于IllustrisTNG300模拟数据，结合随机森林回归器、神经网络和标准化流（Normalizing Flows）三种机器学习方法，揭示了星系偏差（galaxy bias）主要受局域密度（δ₈）、宇宙网结构距离和内部晕属性（如z₁/₂）影响，并证明标准化流在预测星系偏差及其联合分布方面优于确定性方法。


<details>
  <summary>Details</summary>

Background: 研究星系如何示踪宇宙物质密度场（large-scale structure）是理解大尺度结构对星系形成影响的关键，为观测宇宙学提供重要基础。

Data: 数据来源于IllustrisTNG300模拟（simulation），包括使用DisPerSE计算的不同宇宙网状结构（cosmic-web structures）的距离数据，并补充了逐个天体的线性偏差（large-scale linear bias, $b_i$）估计值。

Method: 采用随机森林回归器（Random Forest Regressor）、神经网络（Neural Network）和概率方法（归一化流，Normalizing Flows）三种机器学习模型预测个体星系的大尺度线性偏差（$b_i$）。

Result: result：归一化流（Normalizing Flows）在预测星系偏差（galaxy bias）及其与星系特性的联合分布方面明显优于确定性方法，因其能捕捉物质-晕-星系关联随机性相关的固有方差。
...
### [36] [Early growth of massive black holes in dynamical dark energy models with negative cosmological constant](https://arxiv.org/abs/2602.05921)
*N. Menci,M. Castellano,P. Mukherjee,D. Roberts,P. Santini,A. A. Sen,F. Shankar*

**TL;DR**: 研究发现负宇宙学常数（Ω_Λ≈-1）的动力学暗能量（Dark Energy）模型能同时解释JWST观测到的高红移（z>10）亮星系过量、早期（z≳8）大质量黑洞（M_BH≳10^7M_⊙）形成及活动星系核（AGN）超丰度现象。


<details>
  <summary>Details</summary>

Background: 该研究探讨了宇宙学中动态暗能量（Dark Energy）的可能性，特别是负宇宙常数（Ω_Λ）对早期宇宙中明亮星系、超大质量黑洞（BHs）和活动星系核（AGN）异常丰度的潜在解释作用。

Data: 研究使用了基于暗能量（Dark Energy）动力学模型的数值模拟（simulation）数据，假设初始轻质量黑洞种子（Black Hole seeds）质量为$10^2\,M_{\odot}$并源自第三星族恒星（PopIII stars），通过爱丁顿极限吸积（Eddington-limited accretion）计算最大黑洞增长。

Method: 采用解析框架（analytic framework）将暗物质晕（dark matter halos）增长与爱丁顿极限吸积（Eddington-limited accretion）下的黑洞（BHs）最大增长关联，基于负宇宙学常数（Ω_Λ≈-1）的动力学暗能量（dynamical Dark Energy）模型。

Result: 负宇宙学常数（Ω_Λ≈−1）的动力学暗能量（Dark Energy）模型可同时解释JWST观测到的高红移（z≳8）大质量黑洞（M_BH≳10^7M_⊙）早期形成及活动星系核（AGN）超预期丰度现象。
...
### [37] [Cosmographic parameters from current and next-generation gravitational wave detectors](https://arxiv.org/abs/2602.05969)
*Jonathan Morais,Rodrigo Gonçalves,Jailson Alcaniz*

**TL;DR**: 通过分析先进LIGO、爱因斯坦望远镜和DECIGO对电磁明亮标准汽笛(siren)的观测能力，发现DECIGO能以前所未有的精度（哈勃常数$H_0$亚百分之一、减速参数$q_0$优于10%、急动度参数$j_0$数十百分比）约束宇宙学参数。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用引力波探测器（如Advanced LIGO、Einstein Telescope和DECIGO）通过电磁明亮标准汽笛（standard sirens）约束宇宙学参数（如哈勃常数$H_0$、减速参数$q_0$和急动度参数$j_0$）的能力评估。

Data: 研究使用了来自先进激光干涉引力波天文台（Advanced LIGO）、爱因斯坦望远镜（Einstein Telescope）和分赫兹干涉引力波天文台（DECIGO）的引力波探测数据，通过电磁明亮标准汽笛（standard sirens）进行宇宙学参数约束。

Method: 采用三阶泰勒展开（third-order Taylor expansion）分析信噪比（signal-to-noise ratio）和事件数量对哈勃常数（$H_0$）、减速参数（$q_0$）和急动参数（$j_0$）估计的影响。

Result: Advanced LIGO可提供无校准的哈勃常数(Hubble constant, $H_0$)测量精度达百分之几，但对高阶参数不敏感。
...
### [38] [Does Cosmology require Hermiticity in Quantum Mechanics?](https://arxiv.org/abs/2602.05973)
*Oem Trivedi,Alfredo Gurrola*

**TL;DR**: 论文通过将惠勒-德维特(Wheeler-DeWitt)框架扩展到非厄米(non-Hermitian)动力学，揭示了反厄米(anti-Hermitian)项在宇宙早期涨落和晚期结构增长中表现为有效阻尼或增益效应，并通过红外约束证明厄米性可能在描述宇宙的半经典分支中动态涌现。


<details>
  <summary>Details</summary>

Background: 研究探讨了在量子宇宙学中引入非厄米(Non-Hermitian)结构对宇宙演化的影响，扩展了传统惠勒-德维特(Wheeler-DeWitt)框架的厄米性限制。

Data: 数据来源于对早期宇宙原初涨落（primordial fluctuations）和晚期结构增长（structure growth）的观测，以及宇宙近平坦性（near flatness）的观测结果。

Method: 采用受控半经典约化方法（controlled semiclassical reduction）分析非厄米（non-Hermitian）结构在量子宇宙学中的动力学效应。

Result: 在量子宇宙学中引入非厄米（non-Hermitian）结构会导致早期宇宙原初涨落和晚期结构增长中出现有效阻尼或增益项，但宇宙学观测数据施加的红外约束强烈抑制了宇宙历史中的非厄米性。
...
### [39] [Testing cosmic anisotropy with cluster scaling relations](https://arxiv.org/abs/2602.06007)
*Tariq Yasin,Richard Stiskalek,Harry Desmond,Sebastian von Hausegger,Pedro G. Ferreira*

**TL;DR**: 通过贝叶斯前向建模联合分析312个低红移(z≲0.2)星系团的X射线和SZ(scaling relation)关系，在自洽处理局域速度场各向异性影响后，发现各向异性膨胀(δH₀/H₀<3.2%)和大尺度体流(bulk flow<1300 km/s)的证据均不显著，推翻了此前同源数据得出的显著各向异性结论。


<details>
  <summary>Details</summary>

Background: 研究探讨了利用星系团标度关系作为距离示踪剂来检验局域膨胀率大尺度各向异性（large-scale anisotropy）的科学问题。

Data: 使用312个红移$z \lesssim 0.2$的星系团样本，结合X射线光度-温度(LT)和热苏尼亚耶夫-泽尔多维奇效应-温度(YT)关系作为距离指标，并基于贝叶斯前向模型对局部速度场重建数据进行建模。

Method: 采用贝叶斯前向模型（Bayesian forward model）联合拟合X射线光度-温度（LT）和热苏尼亚耶夫-泽尔多维奇-温度（YT）关系，同时边缘化潜在星系团距离、建模选择效应及局域 peculiar velocities（peculiar velocities），并通过近似方案自洽处理各向异性红移-距离关系。

Result: 通过贝叶斯模型选择和312个星系团(z≲0.2)数据分析，未发现显著支持局部膨胀率存在偶极、四极或像素化各向异性模型的证据。
...
### [40] [ACT DR6+Planck impact on inflation with non-zero vacuum expectation value and the post-inflationary behavior](https://arxiv.org/abs/2602.06027)
*F. B. M. dos Santos,J. G. Rodrigues,G. Rodrigues,C. Siqueira,J. S. Alcaniz*

**TL;DR**: 该研究基于ACT+Planck联合分析的最新宇宙微波背景（CMB）数据，结合BICEP/Keck阵列偏振模式观测，首次在暴胀模型中通过晶格模拟发现大场区真空期望值（VEV）$M$与谱指数$n_s$的兼容性（$\log_{10}M/M_{Pl}=-2.5^{+1.1}_{-1.3}$），并证实该模型可产生准球形振荡子（oscillon）结构，虽引力波幅值在探测器灵敏度内但频率处于GHz范围，同时耦合项$yφσ^2$对再加热过程的扰动直接影响模型预测。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大场区宇宙暴胀模型中非零真空期望值（VEV）$M$对最新阿塔卡马宇宙学望远镜（ACT）宇宙微波背景（CMB）数据的影响及其与后暴胀过程（如预加热）的关联。

Data: 数据来源于阿塔卡马宇宙学望远镜（ACT）和普朗克卫星（Planck）联合分析的最新宇宙微波背景（CMB）数据，以及最新的BICEP/Keck阵列发布的CMB极化模式数据。

Method: 采用宇宙微波背景(CMB)数据联合分析(ACT+Planck+BICEP/Keck Array)约束暴胀模型参数，并首次对暴胀场进行晶格模拟以研究振荡子(oscillon)形成机制。

Result: result：研究发现$\log_{10}M/M_{Pl}=-2.5^{+1.1}_{-1.3}$（68%置信水平），与$M/M_{Pl}\simeq 0.003$相符，该值对再加热（preheating）等暴胀后过程具有重要意义，且首次通过晶格模拟确认暴胀场在前几倍e-fold期间产生符合振荡子（oscillons）特征的局域准球形结构，可能产生引力波和原初黑洞信号，但引力波频率处于GHz范围超出预期探测频段，同时耦合项$yφσ^2$对扰动再加热（perturbative reheating）的影响分析表明较低$M$值与BICEP/Keck Array+Planck+ACT联合数据限制及全温度范围一致。
...
### [41] [Nano-particle Transition Matrix code implementation](https://arxiv.org/abs/2602.05520)
*G. La Mura,G. Mulas,G. Aresu,M. A. Iatì,C. Cecchi-Pestellini,S. Rezaei,R. Saija*

**TL;DR**: 开发了基于高性能并行硬件的纳米粒子过渡矩阵代码（NP_TMcode），显著提升了处理任意形态球对称粒子聚集体电磁散射和吸收问题的计算效率。


<details>
  <summary>Details</summary>

Background: 电磁散射和吸收（electromagnetic scattering and absorption）是涉及实验室实验、生物学、材料科学、环境研究及天体物理等广泛领域的基础物理问题。

Data: 数据来源为数值模拟（simulation）方法，采用过渡矩阵（T-matrix）形式主义进行电磁散射和吸收计算，并通过高性能并行硬件架构优化算法实现复杂模型求解。

Method: 采用过渡矩阵方法（T-matrix）并优化算法以利用高性能并行硬件架构，显著提升计算效率。

Result: NP_TMcode项目通过利用高性能并行硬件架构显著降低了T矩阵方法（T-matrix）的计算时间，使其能够解决更复杂的球形对称粒子（spherically symmetric particles）聚集模型。
...
### [42] [Momentum-conserving self-gravity in the phantom smoothed particle hydrodynamics code. Parallel dual tree traversal for the symmetric fast multipole method](https://arxiv.org/abs/2602.05804)
*Yann Bernard,Timothée David-Cléris,Daniel J. Price,Mike Y. M. Lau*

**TL;DR**: 提出了一种并行化动量守恒笛卡尔快速多极方法（FMM），用于自适应引力软化长度下的平滑粒子流体动力学（SPH）重力计算，实现了线性动量的机器精度守恒，并改进了角动量和轨道相位的保持。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发一种适用于平滑粒子流体动力学（SPH）中自适应引力软化（adaptive gravitational force softening）的并行化动量守恒笛卡尔快速多极子方法（momentum-conserving Cartesian fast multipole method），以改进粒子自引力计算中的动量守恒性能。

Data: 数据来源于并行化的德亨（Dehnen）动量守恒笛卡尔快速多极方法（Cartesian fast multipole method），通过复制树结构中每个叶节点父节点的节点-节点交互实现双树遍历（dual tree walk），并针对自适应软化长度（adaptive softening lengths）调整了树开放准则（tree opening criterion）。

Method: 采用动量守恒的笛卡尔快速多极子方法（Cartesian fast multipole method）并改进双树遍历算法，通过复制叶节点父级的节点-节点交互实现并行化，同时针对自适应软化长度调整树开放准则。

Result: 提出了一种并行化的动量守恒笛卡尔快速多极子方法（Cartesian fast multipole method），用于自适应引力软化长度下的平滑粒子流体动力学（SPH）重力计算。
...
### [43] [Bayesian imaging inverse problem with scattering transform](https://arxiv.org/abs/2602.05816)
*Sébastien Pierre,Erwan Allys,Pablo Richard,Roman Soletskyi,Alexandros Tsouros*

**TL;DR**: 提出一种基于散射变换统计量（Scattering Transform statistics）的低维表示贝叶斯方法，在缺乏先验分布的低数据量条件下，成功实现了对魁乔特模拟（Quijote simulations）中大尺度结构柱密度场的精确统计推断和确定性信号重建。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决天体物理学和宇宙学中低数据条件下非高斯信号（non-Gaussian signals）的贝叶斯成像反演问题（Bayesian imaging inverse problems）的挑战。

Data: 数据来源于Quijote模拟（Quijote simulations）生成的大尺度结构（large-scale structure）柱密度场（column density field），并使用了真实的仪器前向算子（instrumental forward operator）。

Method: 基于散射变换统计量（Scattering Transform statistics）构建物理场的低维表示，采用迭代自适应算法高效逼近后验分布。

Result: 基于散射变换统计量(Scattering Transform statistics)的低维表示方法实现了在缺乏先验信息条件下对非高斯天体物理和宇宙学信号的准确贝叶斯反演和确定性重建。
...
### [44] [Direction-dependent calibration with image-domain gridding](https://arxiv.org/abs/2602.06002)
*Sebastiaan van der Tol,Sarod Yatawatta,Bram Veenboer,David Rafferty*

**TL;DR**: 提出了一种基于图像域网格化（image-domain gridding）的校准方法，通过傅里叶空间子网格（sub-grids）代替传统离散方向校准（discrete directional gains），消除了宽视场系统误差模型的不连续性，显著提升了LOFAR数据成像质量。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决射电干涉仪宽视场成像中方向依赖性系统误差（direction-dependent systematic effects）的校准问题。

Data: 数据来源于LOFAR（Low-Frequency Array）的射电干涉观测数据。

Method: 采用图像域网格化（image-domain gridding）作为校准模型，通过在傅里叶空间使用一组子网格（sub-grids）作为校准基础，取代传统离散方向校准，消除宽视场系统误差模型的不连续性。

Result: 提出了一种基于图像域网格化的校准方法，通过傅里叶空间子网格基代替离散方向校准，消除了传统分面校准（facet-based calibration）的不连续性，提高了图像质量。
...
### [45] [Detecting gravitational wave background with equivalent configurations in the network of space based optical lattice clocks](https://arxiv.org/abs/2602.05714)
*Mingzhi Lou,Hong Su,Tao Yang,Yun-Long Zhang*

**TL;DR**: 论文核心贡献是发现并分析了一种保持重叠减少函数(ORF)不变的变换方法，优化了双路光学晶格钟(OLC)探测器网络配置，使其在增大间距降低噪声相关性的同时保持高交叉相关响应，并对比了其与空间激光干涉引力波探测器的灵敏度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用空间光学晶格钟（OLCs）网络探测随机引力波背景（stochastic gravitational wave background）的科学主题。

Data: 数据来源于基于空间的光晶格钟（optical lattice clocks, OLCs）网络，采用单向链路配置的双探测器系统。

Method: 采用重叠减少函数（ORF）分析光学晶格钟（OLC）探测器网络，并解析推导出保持ORF不变的变换方法。

Result: 发现了一种保持重叠缩减函数（ORF）不变的变换方法，适用于配备单向链路的两台光学晶格钟（OLC）探测器配置。
...
### [46] [deci-Hz Gravitational Wave Observations on the Moon and Beyond](https://arxiv.org/abs/2602.05923)
...
**TL;DR**: 该研讨会总结了在月球及更远空间开展0.1-1赫兹(deci-Hz)频段引力波观测的实验方案（包括月球探测器、空间激光干涉仪和原子干涉仪）、该频段引力波源特性以及由此开启的多信使与多波段天文学研究前景。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于月面及深空探测环境下0.1-10赫兹频段(deci-Hz)引力波的实验方案与多信使天文学应用。

Data: 数据来源包括月球探测器（lunar detectors）、空间激光干涉仪（laser interferometers in space）和原子干涉测量（atom interferometry）。

Method: 采用月球探测器（lunar detectors）、空间激光干涉仪（laser interferometers in space）和原子干涉测量（atom interferometry）技术观测分赫兹频段（deci-Hz band）引力波。

Result: 月球和空间探测器（lunar and space-based detectors）可实现0.1-10 Hz频段引力波（gravitational waves）观测，并推动多信使（multi-messenger）与多波段（multi-band）天文学研究。
...
### [47] [Suppression of Gravitational-Wave Echoes in Diffeomorphism-Invariant Nonlocal Gravity](https://arxiv.org/abs/2602.04996)
*J. W. Moffat*

**TL;DR**: 研究发现，基于解析全函数非定域性（diffeomorphism--invariant, analytic, entire--function nonlocality）的紫外有限量子引力和修正引力理论会普遍抑制致密双星并合后引力波回波（gravitational--wave echoes）信号，其缺失与视界存在与否无关，且反映了量子引力在经典极限中强加的解析结构。


<details>
  <summary>Details</summary>

Background: 研究探讨了引力波回波（gravitational-wave echoes）在紧致双星合并后的观测特性及其对量子引力（quantum gravity）和修正引力（modified gravity）理论的约束作用。

Data: 研究使用了基于广义相对论引力波观测数据的理论模拟（simulation）分析，结合量子引力理论中满足解析性（analyticity）和佩利-维纳（Paley-Wiener）约束的非局部核（nonlocal kernel）模型。

Method: 采用基于解析非局域性(diffeomorphism-invariant, analytic, entire-function nonlocality)的紫外有限量子引力和修正引力理论作为核心物理模型，通过Paley-Wiener定理约束高频反射幅度的指数衰减效应。

Result: 基于微分同胚不变、解析且整函数的非局域性紫外有限量子引力和修正引力理论普遍抑制了可观测的回波信号。
...
### [48] [Tidal disruption event rates across cosmic time: forecasts for LSST, Roman, and JWST and their constraints on the supermassive black hole mass function](https://arxiv.org/abs/2602.04947)
*Mitchell Karmen,Suvi Gezari,Colin Norman,Muryel Guolo*

**TL;DR**: 通过构建潮汐瓦解事件(TDE)率的半经验模型，发现TDE率对超大质量黑洞(SMBH)质量函数及其红移演化高度敏感，并预测了Rubin LSST等巡天项目的观测率，为利用TDE直接约束SMBH质量函数演化提供了方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超大质量黑洞（SMBH）质量函数（mass function）在宇宙时间尺度上的演化及其对潮汐瓦解事件（TDE）率的影响。

Data: 研究中使用的数据来源于半经验模型（semi-empirical model）构建的潮汐瓦解事件（TDE）率红移演化模拟，并结合了星系尺度过程（如核区恒星密度、星系并合率、尘埃消光及早期宇宙可能存在的重元素丰度初始质量函数（IMF））的红移演化影响。

Method: 采用半经验模型（semi-empirical model）结合多种超大质量黑洞（SMBH）质量函数假设，并整合星系尺度演化过程（如核区恒星密度、星系合并率、尘埃消光及早期宇宙可能存在的顶部重初始质量函数（IMF））来量化其对潮汐瓦解事件（TDE）率的影响。

Result: 潮汐瓦解事件（TDE）的体发生率随红移增加直至在宇宙正午（cosmic noon）附近达到峰值，随后因可瓦解恒星的超大质量黑洞（SMBH）数量减少而下降。
...
### [49] [A gamma-ray-emitting blazar B3 1239+376 at z = 3.82 identified in a multi-wavelength context](https://arxiv.org/abs/2602.05222)
*Wei Zhang,Neng-Hui Liao,Hao-Yi Huang,Hai Lei,Xiong Jiang,Ning Jiang,Zhen-Feng Sheng,Lu-Lu Fan,Ting-Gui Wang*

**TL;DR**: 发现了红移(redshift) z=3.82的射电类星体B3 1239+376在2025年半年的显著γ射线辐射（7.7σ显著性），通过多波段观测证实其成为迄今第三遥远的γ射线耀变体(blazar)，并揭示了γ射线与红外光变曲线的同步性及喷流(jet)特性的单区轻子辐射模型解释。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（$z >$ 3）河外伽马射线源（$γ$-ray emitters）的探测及其对活动星系核（active galactic nuclei）喷流宇宙演化和超大质量黑洞（supermassive black holes）质量增长初始阶段的科学意义。

Data: 数据来源于费米大面积望远镜(Fermi-LAT)的观测数据和多波段(包括红外)的光变曲线观测。

Method: 采用费米大面积望远镜(Fermi-LAT)数据分析、多波段光变曲线相关性分析以及经典单区轻子辐射模型(one-zone leptonic radiation model)进行宽波段能谱拟合。

Result: 在2025年半年观测期内，费米-LAT(Fermi-LAT)数据检测到与射电类星体B3 1239+376（红移z=3.82）方向一致的显著性7.7σ伽马射线源，关联概率达0.91，并发现早期存在共位伽马射线残余信号，其伽马射线与红外光变曲线呈现同步峰值特征，证实该源为迄今第三遥远的伽马射线耀变体(blazar)。
...
### [50] [Classifying white dwarfs from multi-object spectroscopy surveys with machine learning](https://arxiv.org/abs/2602.04964)
...
**TL;DR**: 该研究设计了一种神经网络，结合暗能量光谱仪（DESI）和泛星计划（Pan-STARRS）的光谱与测光数据，实现了对白矮星（white dwarfs）光谱类型的自动化高精度分类（DA和DB型接近100%，其他主要类型85-95%），并通过UMAP降维发现了3颗表面成分不均匀的双面白矮星（'double-faced' white dwarfs），同时展示了机器学习在大规模数据中区分单白矮星与双白矮星联星系统的潜力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用机器学习提升多目标光谱巡天中白矮星（white dwarfs）光谱分类效率的自动化方法开发。

Data: 数据来源为暗能量光谱仪（DESI）数据发布1的光谱和泛星计划（Pan-STARRS）测光成像数据的组合。

Method: 采用神经网络结合暗能量光谱仪（DESI）和泛星计划（Pan-STARRS）测光数据对白矮星光谱类型进行分类，并利用均匀流形近似与投影（UMAP）降维技术分析光谱或测光特征。

Result: 设计的神经网络对DA和DB型白矮星光谱分类准确率接近100%，对其他主要类型（包括金属污染）分类准确率达85-95%。
...
### [51] [MESS: Multi-Epoch Spectroscopic Solver for Detecting Double-Lined Systems](https://arxiv.org/abs/2602.05610)
*Gil Nachmani,Simchon Faigler,Tsevi Mazeh*

**TL;DR**: 开发了自动化算法MESS，用于在大规模多历元光谱数据库中识别和表征双线光谱双星（SB2），通过全局多历元方法优化模板并联合推导双星径向速度（RVs），在模拟LAMOST MRS系统中分类准确率达95%。


<details>
  <summary>Details</summary>

Background: 开发自动化算法（MESS）用于大规模光谱数据库中双线光谱双星（SB2）的识别与表征。

Data: data部分：研究中使用的数据来源包括1500个模拟的LAMOST中分辨率光谱巡天（MRS）系统（信噪比SNR=50）以及实际观测的LAMOST数据发布11（DR11）中的光谱数据。

Method: 采用全局多历元形式扩展二维TODCOR方法，联合优化所有观测的模板，在八维参数空间（有效温度、表面重力、每颗星的旋转展宽、共同金属丰度和通量比）中搜索连续合成光谱流形。

Result: MESS算法通过全局多历元形式扩展了二维TODCOR方法，能自动识别和表征大样本多历元光谱中的双线光谱双星（SB2），同时联合优化所有观测的模板，在信噪比为50的1500个模拟LAMOST MRS系统中实现了约95%的整体分类准确率。
...
### [52] [Complex hydrogen chemical equilibrium and Gaia low mass problem in cool white dwarfs](https://arxiv.org/abs/2602.04995)
*Piotr M. Kowalski*

**TL;DR**: 研究发现当前模型低估有效温度（Teff）低于6000K的冷白矮星质量，通过抑制氢三正离子（H3+）形成可修正模型与盖亚（Gaia）DR3观测数据的偏差，表明需重新考虑氢三正离子丰度及未计入电荷物种对冷氢大气白矮星化学平衡的影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于冷白矮星（cool white dwarfs）大气模型中化学平衡的偏差问题，特别是氢离子（$\rm H_3^+$）的形成对自由电子和氢负离子（$\rm H^-$）丰度的显著影响。

Data: 使用盖亚(Gaia) DR3白矮星样本数据并结合量子力学计算量化了氢物种形成与稳定性。

Method: 采用最先进的大气模型分析盖亚(Gaia) DR3白矮星样本，并结合量子力学计算量化氢分子离子(H₃⁺)等不同氢物种在这些恒星大气中的形成和稳定性。

Result: 当前模型低估了有效温度（Teff）低于6000 K的冷白矮星质量，原因在于H3+物种的形成显著改变了化学平衡，导致自由电子和H-的过度丰度以及H-束缚-自由吸收增强。
...
### [53] [Simulating first-order phase transition during inflation](https://arxiv.org/abs/2602.05501)
*Jintao Zou,Ligong Bian,Shao-Jiang Wang*

**TL;DR**: 提出了一种在斯塔罗宾斯基暴胀（Starobinsky inflation）中通过大统一理论（GUT）尺度的一阶相变（FoPT）实现暴胀终止的机制，利用暴胀子滚动动态控制的指数演化势垒，使暴胀末期产生大量气泡成核，并通过数值模拟验证了该机制产生的引力波能谱具有高频振荡特征。


<details>
  <summary>Details</summary>

Background: 研究主题为在斯塔罗宾斯基暴胀（Starobinsky inflation）模型中通过大统一理论（GUT）能级的一阶相变（FoPT）解决暴胀优雅退出问题。

Data: 数据来源于格点数值模拟（lattice numerical simulations）对暴胀（inflation）末期大统一理论（GUT）尺度下一阶相变（FoPT）的测试结果。

Method: 采用指数演化势垒动态控制暴胀子滚动，并通过格点数值模拟验证了在大统一理论尺度下的一阶相变模型。

Result: 在斯塔罗宾斯基暴胀模型中，通过滚动暴胀子动态控制的指数演化势垒实现了大统一理论（GUT）能级的一阶相变（FoPT），早期暴胀阶段几乎无气泡成核，而在暴胀末期出现大量气泡成核。
...
### [54] [Induced-Gravity Higgs Inflation in Palatini Supergravity Confronts ACT DR6](https://arxiv.org/abs/2602.05623)
*C. Pallis*

**TL;DR**: 在帕拉蒂尼超引力(Palatini Supergravity)框架下构建了一个与ACT DR6高度一致的诱导引力暴胀模型，发现暴胀子(inflaton)在U(1)B-L对称性自发破缺时产生，能量尺度低于有效理论截断能标，并成功实现了非热轻子生成(non-thermal leptogenesis)和μ参数生成，同时预测了(40-60)PeV范围的引力微子(gravitino)质量与LHC希格斯玻色子质量结果一致。


<details>
  <summary>Details</summary>

Background: 研究在帕拉蒂尼超引力（Palatini Supergravity）框架下构建了一个与ACT DR6高度吻合的诱导引力暴胀（induced-gravity inflation）模型，重点探讨了暴胀子（inflaton）在U(1)B-L对称性自发破缺中的作用及其与有效理论能标的关联。

Data: 数据来源于Palatini超引力框架下的理论模型构建，结合ACT DR6（Atacama Cosmology Telescope Data Release 6）观测数据进行一致性验证。

Method: 采用帕拉蒂尼超引力（Palatini Supergravity）框架构建诱导引力暴胀模型，通过引入具有实数和位移对称性的卡勒势（Kaehler potential）贡献实现暴胀场正则归一化，并包含分别的对数全纯与反全纯项以耦合暴胀场与里奇标量（Ricci scalar）。

Result: 在Palatini超引力框架下构建的诱导引力暴胀模型与ACT DR6数据高度吻合，暴胀子（inflaton）通过U(1)B-L对称性自发破缺产生，破缺能标接近(0.102-5.85)×10^16 GeV。
...
### [55] [Early-universe constraints on the electron mass](https://arxiv.org/abs/2602.05720)
*Michela Garramone,Stefano Gariazzo,Nicolao Fornengo*

**TL;DR**: 研究通过分析非标准电子质量(me)对早期宇宙中微子退耦和大爆炸核合成(BBN)的影响，结合轻元素丰度观测和有效相对论性粒子数(Neff)限制，得出早期宇宙电子质量为0.504+0.007−0.006 MeV或0.510±0.007 MeV（1σ），与当前实验室测量值偏差仅1.4%，证实了电子质量在宇宙时间尺度上的恒定性。


<details>
  <summary>Details</summary>

Background: 探讨非标准电子质量（electron mass）对早期宇宙热历史的影响，特别是中微子退耦（neutrino decoupling）和大爆炸核合成（Big Bang Nucleosynthesis）过程的作用。

Data: 数据来源包括标准宇宙学模型中的中微子-电子相互作用理论、大爆炸核合成（BBN）的核反应网络（NACRE II或PRIMAT）以及观测得到的原初轻元素丰度和有效相对论性粒子数（$N_{\mathrm{eff}}$）的宇宙学约束。

Method: 采用非标准电子质量（nonstandard electron mass）对早期宇宙热历史的影响进行分析，重点研究其对中微子退耦（neutrino decoupling）和大爆炸核合成（Big Bang Nucleosynthesis, BBN）的作用机制。

Result: 非标准电子质量（electron mass）在早期宇宙中改变了中微子退耦（neutrino decoupling）和大爆炸核合成（BBN）过程，导致中微子能量密度和有效相对论性粒子数（$N_{\mathrm{eff}}$）的变化。
...
### [56] [An approximate Kappa generator for particle simulations](https://arxiv.org/abs/2602.05606)
*Seiji Zenitani,Takayuki Umeda*

**TL;DR**: 提出了一种基于q-指数函数(q-exponential function)近似累积分布函数的随机数生成方法，适用于卡帕速度分布(Kappa velocity distribution)粒子模拟，在k<4时精度高且GPU运算速度快。


<details>
  <summary>Details</summary>

Background: 研究主题为基于卡帕速度分布（Kappa velocity distribution）的粒子模拟随机数生成方法开发。

Data: 数据来源于粒子模拟（particle simulations）中生成的卡帕速度分布（Kappa velocity distribution）随机数。

Method: 基于q-指数函数(q-exponential function)逼近累积分布函数并构建逆变换过程。

Result: 提出的方法基于q-指数函数(q-exponential function)近似累积分布函数，构建了逆变换过程，能高效生成卡帕速度分布(Kappa velocity distribution)的随机数。
...
### [57] [Intereses convergentes: los años previos a la visita de Einstein a la Argentina en 1925](https://arxiv.org/abs/2602.05063)
*Alejandro Gangui*

**TL;DR**: 研究揭示了爱因斯坦（Albert Einstein）1925年阿根廷之行的促成因素，包括1919年日食观测后其声名鹊起、国际与当地社会政治环境及布宜诺斯艾利斯（Buenos Aires）个人与机构的推动，并分析了该国在相对论（theory of relativity）领域的早期认知基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于爱因斯坦（Albert Einstein）1919年日食观测成名后至1925年访问阿根廷（Argentina）期间的国际与本土社会政治环境，探讨其访阿事件在20世纪初科学边缘国家的文化背景与促成因素。

Data: 研究主要基于历史文献分析，包括1919年日食观测结果公布后爱因斯坦声名鹊起至1925年抵达布宜诺斯艾利斯期间的档案记录、当地报刊对相对论（theory of relativity）的讨论文本及机构往来信函等一手资料。

Method: 采用历史文献分析法（historical document analysis）结合社会文化语境考察（socio-cultural contextual examination），聚焦爱因斯坦访阿前后的国际与本土互动因素。

Result: 爱因斯坦(Albert Einstein)1925年阿根廷之行的促成因素是国际声誉(1919年日食观测后确立)、当地个人/机构推动以及特定社会政治环境的共同作用。
