...
**TL;DR**: 通过费米悖论分析表明，银河系中技术文明（technologically intelligent civilizations）的寿命上限在最优情景下不超过5×10³年，且电磁通信（electromagnetic communication）未检测到信号对长寿命文明构成挑战。


<details>
  <summary>Details</summary>

Background: 该研究基于费米悖论（Fermi paradox）探讨银河系中技术文明（technologically intelligent civilizations）的出现概率与存续时间的约束条件。

Data: 数据来源于对银河系内电磁通信信号（electromagnetic communication）的观测分析，并结合当前光锥（light cone）覆盖的约10^5年银河系历史范围。

Method: 采用德雷克方程(Drake equation)框架结合费米悖论(Fermi paradox)约束条件，通过概率分析和电磁信号可探测性评估推算技术文明寿命上限。

Result: 在乐观假设下，技术文明（technologically intelligent civilizations）的寿命上限被费米悖论（Fermi paradox）限制为不超过约5000年。
...
**TL;DR**: 基于盖亚(Gaia) DR3数据精确测定了银心反方向4个疏散星团(COIN-Gaia 24、Czernik 24、FSR 0893、UBC 74)的成员星、结构参数、天体物理参数及轨道特征，证实它们是具有薄盘典型近圆轨道的动态演化系统，可作为银河系薄盘化学和动力学演化的可靠示踪体。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系薄盘（thin disc）化学与动力学演化的示踪，通过分析四个反向银心方向的疏散星团（open clusters）来揭示其结构、天体物理及运动学特征。

Data: 数据来源于盖亚卫星第三次数据发布（Gaia Data Release 3）的天体测量、测光和光谱数据。

Method: 采用UPMASK算法在五维参数空间进行成员判定，并基于King模型拟合径向密度分布获取结构参数，结合PARSEC演化模型的贝叶斯马尔可夫链蒙特卡洛等龄线拟合和ARIADNE光谱能量分布分析估算天体物理参数。

Result: 通过贝叶斯马尔可夫链蒙特卡洛等时线拟合(PARSEC演化模型)和光谱能量分布分析(ARIADNE)确定这些银心反方向疏散星团为中度红化、中等年龄(年龄超过动力学弛豫时间数倍)、距离太阳1-3.5千秒差距的薄盘族天体，其近圆形低偏心率轨道和化学动力学特征证实它们是银河系薄盘化学与动力学演化的可靠示踪体。
...
**TL;DR**: 通过联合分析JWST发现的高红移宽线活动星系核(AGN)的宽发射线和钱德拉深场非探测现象，结合超爱丁顿吸积(self-shadowed super-Eddington accretion flow)和冕区过冷(coronal over-cooling)模型，揭示了这些天体实际由低质量(约10⁶-10⁷太阳质量)、超爱丁顿(f_Edd≫1)黑洞组成，其被抑制的冕区辐射和自遮蔽宽线区(BLR)几何结构导致传统方法高估了黑洞质量。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(high-redshift)宽线活动星系核(AGN)中黑洞质量估算的修正，特别关注超爱丁顿(super-Eddington)吸积流框架下冕区辐射冷却对X射线弱化现象的影响。

Data: 数据来源于Lupi等人（2024）和Maiolino等人（2025）分析的样本，结合了JWST观测的高红移宽线活动星系核（AGN）的宽发射线数据以及钱德拉（Chandra）深场成像中的系统性未探测结果。

Method: 采用自遮蔽超爱丁顿吸积流(self-shadowed, super-Eddington accretion flow)框架与冕区过冷机制(coronal over-cooling prescription)，结合漏斗依赖的康普顿化物理(funnel-dependent Comptonization physics)与细盘光谱(slim-disc spectra)，通过全马尔可夫链蒙特卡洛(MCMC)推断探索参数空间。

Result: 结合宽发射线和钱德拉深场非探测的联合分析表明，高红移宽线活动星系核(AGN)中存在低质量(约10^6-10^7 M⊙)、超爱丁顿(f_Edd ≫ 1)吸积的黑洞群体，其被约束且辐射过冷的冕区导致X射线弱化，质量估计显著低于单历元维里尔方法结果。
...
**TL;DR**: 通过拟合CEERS NIRCam成像中53885个F356W<28.5星系的形态参数，发现晚型星系(n<2.5)颜色梯度强烈依赖质量但无红移演化，早型星系(n>2.5)颜色梯度平坦且与质量无关，结构测量精度对F356W<27.0星系达20%以内。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用CEERS近红外相机(NIRCam)多波段观测数据，通过形态学参数拟合揭示高红移(z~9)星系的结构演化规律及其与质量、恒星种群分布的关系。

Data: 数据来源于CEERS（Cosmic Evolution Early Release Science）项目的NIRCam（Near-Infrared Camera）成像观测，包含F115W、F150W、F200W、F277W、F356W和F444W六个宽带滤光片下探测到的53,885个星系（星等极限F356W<28.5），并通过模拟星系（simulated galaxies）估计测量参数的不确定性。

Method: 采用galfit软件对CEERS NIRCam六波段（F115W、F150W、F200W、F277W、F356W、F444W）观测到的53,885个星系进行形态参数（Sérsic指数、有效半长轴、轴比、积分星等、位置角）拟合，并通过模拟星系估计参数测量误差。

Result: 在1<z<5的晚型星系(n<2.5)中，颜色梯度(color gradient)与质量显著相关但无明显红移演化，表明大质量星系中恒星种群(stellar population)和尘埃消光(dust attenuation)随半径变化显著；而早型星系(n>2.5)的颜色梯度几乎平坦且与质量无关，表明其恒星种群分布均匀。
...
### [5] [Filamentary Hierarchies and Superbubbles II: Impact of superbubbles and galactic dynamics on filament formation and fragmentation](https://arxiv.org/abs/2602.22326)
*Rachel Pillsworth,Ralph E. Pudritz,Eric W. Koch,Theo J. O'Neill*

**TL;DR**: 研究发现银河系类似星系中丝状结构（filament）的稳定性主要受磁场影响且与磁场方向平行，局部剪切（shear）对其影响较小，而自引力主导的丝状结构（$χ_f \le 1$）易形成引力超临界状态，星际介质压力（ISM pressure）随星系半径变化显著影响丝状结构稳定性和恒星形成率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于旋涡星系（spiral galaxies）中剪切流（shear）、超新星（supernovae）和磁场（magnetic fields）等大尺度物理过程对丝状结构（filamentary structure）形成、演化及恒星形成的影响机制。

Data: 数据来源于对赵等人（Zhao et al. 2024）模拟的银河系类似星系（Milky Way-like galaxy）中丝状结构（filamentary structure）的分析。

Method: 采用丝状结构（filament）和超级气泡（superbubble）分析代码，结合表面压力与中心压力比值（$χ_f=P_{surf}/P_{central}$）的测量方法，研究星系剪切（shear）、超新星（supernovae）、超级气泡及磁场对丝状结构稳定性和碎裂的影响。

Result: 局部剪切力对丝状结构稳定性影响微弱，盘外区最大结构更可能被剪切力而非超新星耗散。
...
### [6] [It's a matter of time: Empirical Constraints on Supernova Yields and Delay Times from Dwarf Spheroidal Galaxies](https://arxiv.org/abs/2602.22333)
...
**TL;DR**: 通过DLEIY化学演化模型对矮椭球星系（dwarf spheroidal galaxies）Sculptor和Fornax的联合分析，首次在不固定核合成产额绝对尺度下，测得核心坍缩超新星（core collapse supernovae）的铁产额与理论模型一致而镁产额高出2-4倍，并发现Ia型超新星（Type Ia supernovae）发生率比场星系高5倍且延迟时间分布遵循$\sim t^{-2}$的陡峭关系，暗示了金属丰度对Ia型超新星率和延迟时间分布的影响。


<details>
  <summary>Details</summary>

Background: 恒星化学丰度研究旨在通过核合成（nucleosynthesis）及其天体物理场所的信息揭示星系演化历史，但宿主星系特定恒星形成历史（star formation history）的干扰使得超新星产额（supernova yields）和时间约束面临挑战。

Data: 数据来源于对矮椭球星系（dwarf spheroidal galaxies）Sculptor和Fornax的观测，包括恒星形成历史（star formation history）和金属丰度分布（metallicity distribution）。

Method: 采用观测的恒星形成历史和金属丰度分布构建星系化学演化模型DLEIY，通过联合统计建模约束种群平均产额和星系外流。

Result: DLEIY模型通过观测的恒星形成历史和金属丰度分布，成功约束了矮椭球星系（dwarf spheroidal galaxies）Sculptor和Fornax中核心坍缩超新星（core collapse supernovae）和Ia型超新星（Type Ia supernovae）的产额及星系外流。
...
**TL;DR**: 首次在行星状星云NGC 6302的尘埃环面(dusty torus)中探测到结晶CO$_2$冰和冷气相CO$_2$，表明尘埃屏蔽使冰化学能在强紫外辐射环境中存在，且其气冰比显著高于年轻恒星天体。


<details>
  <summary>Details</summary>

Background: 研究揭示了行星状星云NGC 6302尘埃环（dusty torus）中CO$_2$冰的存在，挑战了极端紫外辐射环境下分子冰难以存活的传统认知。

Data: 数据来源于JWST/MIRI（James Webb Space Telescope/Mid-Infrared Instrument）对行星状星云NGC 6302尘埃环（dusty torus）的观测。

Method: 采用JWST/MIRI（James Webb Space Telescope/Mid-Infrared Instrument）观测数据结合纯结晶CO$_2$冰双峰吸收谱特征分析。

Result: 在行星状星云NGC 6302的尘埃环面(torus)中首次探测到结晶态CO$_2$冰及其气相分子，表明尘埃遮蔽使该高辐射环境仍能维持冰化学过程。
...
**TL;DR**: 基于MALT90巡天数据结合机器学习（ML）方法，利用五种分子谱线（HCO$^+$、HNC、N$_2$H$^+$、HCN、C$_2$H）成功实现了分子云团（clumps）的自动分类，揭示了与演化阶段对应的稳定集群（如原恒星和无活跃恒星形成区），并通过监督学习对522个未分类天体进行了精确归类。


<details>
  <summary>Details</summary>

Background: 分子云团（molecular clumps）的演化分类对理解恒星形成至关重要，但传统基于红外辐射（IR emission）和形态学标准的人工分类方法常因模糊特征、距离不确定性或强遮蔽而无法有效归类部分源。

Data: 数据来源为MALT90巡天观测的分子谱线强度数据，并辅以斯皮策(Spitzer)红外测光数据。

Method: 采用无监督聚类（HDBSCAN）和监督学习（supervised learning）分析分子谱线强度（HCO$^+$、HNC、N$_2$H$^+$、HCN、C$_2$H）和红外（IR）光度数据，揭示分子团块的演化阶段。

Result: 利用无监督聚类（HDBSCAN）和监督学习对MALT90分子云核（clump）进行分类，揭示了由C$_2$H和N$_2$H$^+$主导的稳定原恒星（protostar）与非恒星形成区（region without active star formation）群组，并发现H$^{13}$CO$^+$能区分紫外主导（UV-dominant）的演化后期区域，红外特征对分类贡献不显著。
...
**TL;DR**: 通过结合赫歇尔远红外(FIR)成像与CANDELS/3D-HST数据构建3D-Herschel星表，研究发现对于在至少两个赫歇尔波段有>3σ探测的3.2%星系，仅使用紫外-中红外(UV-MIR)数据即可准确获取恒星年龄、恒星形成率(SFR)和恒星质量，但固定红外(IR)模板会导致中红外-红外(MIR-IR)光度比不变和冷尘埃温度低估，而加入赫歇尔数据后尘埃温度升高约7K且低质量端(log(M⋆)∼9.6 M⊙)的IR-to-7.7μm光度比增加约0.2 dex，表明中红外-红外转换依赖恒星质量且需谨慎使用无远红外约束的固定模板。


<details>
  <summary>Details</summary>

Background: 研究聚焦于结合赫歇尔远红外（FIR）成像与CANDELS/3D-HST深场数据，通过多波段光测（0.3-350μm）探索尘埃遮蔽宇宙中星系恒星形成与尘埃特性的演化规律。

Data: 数据来源于结合了去混合处理的赫歇尔远红外（FIR）成像和CANDELS/3D-HST遗产巡天场的光度测量目录3D-Herschel（0.3-350μm），用于研究尘埃遮蔽的宇宙。

Method: 采用贝叶斯推断框架Prospector-$β$（17参数拟合代码）对41,387个星系进行建模，测量恒星和尘埃特性并计算真实误差范围。

Result: result：结合赫歇尔远红外(FIR)数据的拟合结果显示，在所有红移下尘埃温度比仅用紫外-中红外(UV-MIR)模型高约7K，且在赫歇尔探测样本的低质量端(log($M_{\star}$)~9.6 $M_{\odot}$)红外与7.7$μ$m光度比高出约0.2 dex。
...
**TL;DR**: 研究发现"小红点"(LRDs)与"小蓝点"(LBDs)本质相同，均为超大爱丁顿吸积活动星系核(AGN)，其观测差异源于视线倾角不同导致的自遮蔽效应和各向异性辐射。


<details>
  <summary>Details</summary>

Background: 该研究探讨了"小红点"(LRDs)是否作为由超爱丁顿吸积驱动的致密蓝色宽线活动星系核("小蓝点"，LBDs)的尘埃红化、高倾角对应体，通过几何厚吸积流模型统一解释其观测特性。

Data: 数据来源于使用Cloudy模拟软件生成的依赖于倾角的光谱能量分布（SEDs）作为输入，并结合几何厚、辐射压支撑的吸积流模型及尘埃屏幕假设。

Method: 采用几何厚、辐射压支撑的吸积流（geometrically thick, radiation-pressure supported accretion flow）模型作为中心引擎，结合各向异性电离连续谱、赤道集中的宽线区（BLR）及中等覆盖率的尘埃屏，通过Cloudy模拟倾角依赖的SED（spectral energy distribution）来验证观测特征。

Result: LRD（Little Red Dots）和LBD（Little Blue Dots）的观测特征可通过吸积流几何取向统一解释，其中LRD为高倾角下尘埃遮蔽的类星体，其极端宽Hα等效宽度（EW）由自阴影效应和各向异性电离连续谱导致。
...
### [11] [Cepheid-based Distances to NGC 4303 and NGC 1068](https://arxiv.org/abs/2602.22407)
*Madison Markham,Misty C. Bentz,Laura Ferrarese,Christopher A. Onken,Marianne Vestergaard*

**TL;DR**: 基于哈勃太空望远镜观测数据，利用造父变星(Cepheid)周期-光度关系测定了活动星系核(AGN) NGC 4303和NGC 1068的距离模数(distance modulus)，分别得到距离为16.47±0.27百万秒差距(Mpc)和10.72±0.52百万秒差距(Mpc)。


<details>
  <summary>Details</summary>

Background: 研究主题为利用造父变星(Cepheid)测量活动星系核(AGN)寄主星系NGC 4303和NGC 1068的几何距离。

Data: 数据来源于哈勃空间望远镜（Hubble Space Telescope）对NGC 4303和NGC 1068各12次非冗余时间间隔的观测，使用F555W和F814W滤光片，分别检测到32,694和20,207个点源，其中130个和51个被确定为强候选造父变星（Cepheid candidates）。

Method: 基于哈勃空间望远镜（Hubble Space Telescope）的观测数据，采用周期-光度关系（period-luminosity relationship）拟合并结合大麦哲伦星云（LMC）的几何距离校准，同时修正了金属丰度（metallicity）的潜在影响。

Result: NGC 4303和NGC 1068的距离模数（distance modulus）分别为31.083±0.035星等和30.150±0.106星等，对应距离为16.47±0.27百万秒差距（Mpc）和10.72±0.52百万秒差距（Mpc）。
...
### [12] [AGN Feedback and the Development of Dusty Multiphase Gas in X-ray Emitting Elliptical Galaxies](https://arxiv.org/abs/2602.22415)
...
**TL;DR**: 研究揭示了群中心椭圆星系（group-centered elliptical galaxies）中富尘埃区域与X射线热气体及尘埃颗粒特性的关联，发现含分子气体（CO示踪）的尘埃优先位于AGN反馈（AGN feedback）产生的X射线空腔边缘，且尘埃总选择消光比（total-to-selective extinction ratio Rv）的空间变化表明混合相环境（mixed-phase environments）中存在更小或演化程度更低的尘埃颗粒。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系群中心椭圆星系中富尘埃区的物理与运动学特性，探讨其与X射线热气体（hot X-ray emitting gas）及尘埃颗粒特征的关联。

Data: 数据来源包括MUSE观测的H-alpha发射线、ALMA探测的CO发射线以及X射线成像数据。

Method: 采用多波段数据（MUSE和ALMA观测的H-alpha与CO发射线）结合X射线成像分析，研究尘埃富集区的物理与运动学特性，重点解析尘埃颗粒特征（总选择消光比Rv）与热X射线气体（AGN反馈形成的空腔边缘）的空间关联。

Result: dust-rich regions containing molecular gas traced by CO are preferentially located at the rims of AGN-driven X-ray cavities, indicating enhanced condensation of cold dusty gas at compressive interfaces.
...
**TL;DR**: 基于数值模拟揭示了卫星星系（satellite galaxies）分布和丰度主要受环境主导，宿主恒星质量和宇宙时期起次要调节作用，在空洞、星系群和星系团中呈现差异化演化路径。


<details>
  <summary>Details</summary>

Background: 研究聚焦于在ΛCDM框架下，卫星矮星系（satellite dwarf galaxies）的丰度与空间分布如何受宿主星系（host galaxies）的恒星质量、形态、恒星形成活动及大尺度环境的影响，并追踪其从红移z=2到z=0的演化规律。

Data: 数据来源于Millennium数值模拟（Millennium simulation）结合半解析模型（semi-analytic model），构建了绝对星等Mr < -16的主星系及其在维里半径（virial radius）内卫星星系（恒星质量M_* ≥ 3×10^5 M_sun、绝对星等Mr < -9）的一致性样本。

Method: 采用半解析模型（semi-analytic model）结合Millennium数值模拟，在LCDM框架下构建宿主星系及其卫星样本，分析卫星丰度与径向分布随环境、宿主恒星质量及红移的演化。

Result: 卫星星系（satellite galaxies）的分布和丰度主要受环境主导，宿主恒星质量和宇宙时期起次要调节作用。
...
**TL;DR**: 提出ViT-MDNz深度学习模型，通过结合视觉变换器(ViT)和混合密度网络(MDN)，利用单波段星系图像的形态学特征实现了高精度红移(redshift)估计（$σ_{\rm NMAD}=0.034$，$f_{\rm out}=2.6\%$），验证了仅凭形态特征即可获得具有良好校准性的概率密度函数预测，为有限波段覆盖的星系样本红移估算提供了实用方案。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用单波段星系图像通过深度学习模型（ViT-MDNz）解决红移（redshift）估计的挑战，旨在提升未来大规模巡天（如DESI和LSST）中红移星表的完整性和可用性。

Data: 数据来源于DESI Legacy Imaging Surveys (DESI-LS)的约30万张单波段星系图像。

Method: 采用视觉变换器(Vision Transformer, ViT)提取深度形态学特征并结合混合密度网络(Mixture Density Network, MDN)预测完整红移概率密度函数。

Result: ViT-MDNz通过整合视觉变换器（Vision Transformer, ViT）和混合密度网络（Mixture Density Network, MDN），仅利用单波段星系图像的形态学特征即可准确预测红移及其概率密度函数，在DESI-LS数据上实现了σ_NMAD=0.034和f_out=2.6%的优异性能。
...
### [15] [VLBI Detections of Compact Nuclei in Spiral-hosted Double-lobed Radio-loud Active Galactic Nuclei (DRAGNs): Evidence for Weak Parsec-Scale Jet Activity](https://arxiv.org/abs/2602.22668)
*Mingyu Ryu,Jae-Young Kim*

**TL;DR**: 通过VLBI观测发现螺旋星系寄主的射电噪活动星系核（spiral DRAGNs）具有弱相对论性（β≲0.6）的亚秒差距尺度喷流和低亮度温度（约10⁹ K）核心，其低射电爱丁顿比（log(L_R,1.4GHz/L_Edd)≈-5至-8）与薄盘喷流磁化启动阶段的模拟结果一致，表明此类晚型星系中存在周期性或长时标喷流活动。


<details>
  <summary>Details</summary>

Background: 研究主题为探索罕见螺旋星系宿主（spiral DRAGNs）中射电噪活动星系核（AGN）的喷流形成机制及其与椭圆星系宿主传统关联的背离现象。

Data: 数据来源于公开的甚长基线干涉测量（VLBI）档案数据。

Method: 采用甚长基线干涉测量（VLBI）技术对螺旋星系宿主双瓣射电噪活动星系核（spiral DRAGNs）进行毫角秒尺度观测，结合射电爱丁顿比（radio-Eddington ratio）分析和基本平面（fundamental plane of black hole activity）比对，并参照薄盘喷流（thin-disk jets）的广义相对论磁流体力学（GRMHD）模拟结果。

Result: 在螺旋星系寄主的双瓣射电噪活动星系核（spiral DRAGNs）中探测到毫角秒尺度VLBI致密射电核，其核心亮度温度（$T_{\rm b} \approx 10^9$ K）和喷流-反向喷流流量比表明存在弱相对论性（$β\lesssim 0.6$）的pc尺度外流。
...
**TL;DR**: 基于紫外成像望远镜（UVIT）的远紫外（FUV）和g波段数据，开发了一种利用FUV-g颜色表征年轻恒星形成团块（SFCs）年龄的半新方法，并首次量化了NGC 1313的恒星形成层级参数。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用紫外成像（UV）探测年轻恒星形成团块（SFCs），并通过其空间分布揭示恒星形成的层级结构（hierarchy of star formation）。

Data: 数据来源于AstroSat卫星搭载的紫外成像望远镜（UVIT）的远紫外（FUV）波段观测数据，以及暗能量相机遗产巡天（DECaLS）的g波段数据。

Method: 采用紫外成像望远镜（UVIT）的远紫外（FUV）数据和暗能量相机遗产巡天（DECaLS）的g波段数据，结合两点相关函数（TPCF）分析，优化了基于FUV-g颜色的年轻恒星形成团块（SFCs）年龄表征方法。

Result: FUV-g颜色可作为与FUV-NUV颜色同等可靠的年轻恒星形成团块（SFC）年龄指标。
...
### [17] [The Environmental Switch in Black Hole Feeding: Bar-Driven vs. Merger-Driven Growth in IllustrisTNG50](https://arxiv.org/abs/2602.22875)
*Harsh Uttam,Sandeep Kumar Kataria*

**TL;DR**: 基于高分辨率宇宙学模拟（IllustrisTNG50）揭示了超大质量黑洞（SMBH）质量增长的环境分异机制：孤立棒旋星系中早期形成的恒星棒（stellar bar）通过持续吸积主导晚期SMBH增长，而非孤立环境中早期并合事件触发快速生长而后期棒形成贡献微弱。


<details>
  <summary>Details</summary>

Background: 研究主题为探讨星系环境（孤立与非孤立）中盘动力学过程（如棒旋结构形成）与星系相互作用（如并合事件）对超大质量黑洞（SMBH）质量增长的相对调控作用。

Data: 研究使用了高分辨率宇宙学模拟IllustrisTNG50的数据，通过时间分辨的逐星系分析追踪了棒旋结构（bar formation）、并合事件（merger events）以及主祖先分支上的环境演化。

Method: 采用高分辨率宇宙学模拟IllustrisTNG50进行时间分辨的单星系分析，明确追踪棒结构（bar formation）、并合事件（merger events）及主 progenitor 分支上的环境演化，并基于物理驱动的边界定义对比孤立与非孤立环境中的棒旋星系（barred disc galaxies）与非棒旋星系（unbarred disc galaxies）。

Result: 超大质量黑洞（SMBH）的吸积路径受环境调控，表现为孤立棒旋星系中早期形成的恒星棒（stellar bar）驱动持续吸积，而非孤立星系中早期并合主导吸积而后期形成的棒贡献微弱。
...
### [18] [Group Pre-processing in J1611+4026: Minor Rejuvenation of a Massive ETG fueled by Interaction-driven Gas Transfer](https://arxiv.org/abs/2602.22904)
*Yaosong Yu,Qihang Chen,Zizhao He,Limeng Deng,Liang Jing,Jianhui Lian*

**TL;DR**: 研究发现J1611+4026三重系统中两个富气体伴星系（A和B）的并合通过气体剥离触发了大质量早型主星系（C）的微弱恒星形成复苏（minor rejuvenation），为稠密环境中相互作用驱动的星系演化提供了定量证据。


<details>
  <summary>Details</summary>

Background: 星系群（galaxy group）内相互作用是驱动星系演化的关键机制，研究聚焦于卫星星系动力学过程与大质量主星系（host galaxy）恒星形成复苏（rejuvenation）之间的直接观测联系。

Data: 数据来源包括DESI-LS的深场光学成像数据、DESI和P200的光谱数据。

Method: 采用二维形态分解（2D morphological decomposition）工具GALIGHT和联合分光光度合成建模（joint spectrophotometric synthesis modelling）工具BAGPIPES与CIGALE，解构成员星系的结构特性和恒星形成历史。

Result: 在J1611+4026系统中，主星系Component C虽呈现静止形态和光谱特征，但通过显著内部尘埃消光（$E(B-V) \sim 0.53$）和紫外超量检测到微弱但可靠的"轻微复苏"信号，其恒星形成历史重建显示近期（约100 Myr）存在对总恒星质量贡献可忽略（$f_{\rm burst} < 0.1\%$）的星暴活动。
...
### [19] [Measurements of the HI intensity mapping power spectrum at low redshifts with MIGHTEE data: comparison with detected HI galaxies](https://arxiv.org/abs/2602.23055)
...
**TL;DR**: 利用MeerKAT射电望远镜的MIGHTEE巡天数据，通过中性氢(HI)强度映射技术在0.02≲z≲0.07红移范围内首次实现了干涉仪数据与星系目录功率谱的一致性验证，证实了亚百万秒差距尺度上两种方法测量结果在3≲k≲20 Mpc⁻¹范围内的统计显著性（信噪比∼13）和振幅匹配（10⁻²-10⁻¹ mK² Mpc³）。


<details>
  <summary>Details</summary>

Background: 线强度映射（line intensity mapping）为统计性探测宇宙大尺度物质分布提供了新方法。

Data: 数据来源于MeerKAT国际GHz级分层河外巡天（MIGHTEE Survey）的干涉测量数据，分析了COSMOS天区单点观测17.5小时、频率范围1332-1392 MHz（对应红移0.02≲z≲0.07）的60 MHz子带。

Method: 采用基于延迟谱(delay-spectrum-based estimator)的功率谱估计方法分析干涉仪数据，并通过时间分割可见度(visibilities)的互相关检测HI功率谱。

Result: 通过延迟谱（delay-spectrum）估计器在亚百万秒差距尺度上测量了中性氢（HI）功率谱，并与同一视场中单独探测到的HI星系目录推断的功率谱直接比较，两者在$3 \lesssim k \lesssim 20 \, \mathrm{Mpc}^{-1}$范围内具有一致性且振幅相当（约$10^{-2}$ - $10^{-1}$ $\mathrm{mK}^2 \mathrm{Mpc}^3$）。
...
### [20] [Stochastic Evolution of Galactic Star Formation with Halo Coupling, AGN Quenching and Hopf Bifurcation Dynamics](https://arxiv.org/abs/2602.23077)
*Sanjeev Kumar,A. K. Awasthi,Mahesh Kumar*

**TL;DR**: 该研究通过耦合随机非线性振荡器（Stochastic Hopf Engine）框架，揭示了气体密度（G）与恒星形成率（S）通过超临界霍普夫分岔（supercritical Hopf bifurcation）从静止稳定态向并合驱动星暴转变的共演化机制，并发现暗物质晕（dark matter halo）散射特性通过乘性噪声（multiplicative noise）拓宽分岔区域，在低于确定性阈值时诱发噪声驱动星暴，同时模拟中出现的确定性极限环（Galactic Heartbeat）与恒星形成光谱的data3共振峰验证了周期性特征，而引入径向剪切Ω(r)和空间变化分岔场可再现螺旋形态与活动星系核（AGN）驱动的淬灭现象，最终建立了一个连接局部反馈与全局势场的可解释性星系演化最小框架。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系演化（galactic evolution）的随机非线性动力学机制，通过耦合随机振荡器（stochastic nonlinear oscillator）建模气体密度（gas density）与恒星形成率（star formation rate）的协同演化，揭示从静态稳定到并合触发星暴（merger-driven starbursts）的相变过程。

Data: 数据来源于基于随机Hopf引擎（Stochastic Hopf Engine）的数值模拟，并采用欧拉-丸山方法（Euler--Maruyama method）对暗物质晕（dark matter halo）特性进行建模，同时验证了数据3（data3）共振峰在恒星形成光谱中的确定性极限环。

Method: 采用耦合随机非线性振荡器（Stochastic Hopf Engine）和欧拉-丸山方法（Euler--Maruyama method）模拟气体密度（G）与恒星形成率（S）的协同演化，并通过福克-普朗克方程（Fokker--Planck equation）解析爆发振幅的统计分布。

Result: 基于耦合随机非线性振荡器（Stochastic Hopf Engine）的星系演化计算框架揭示了气体密度（G）与恒星形成率（S）通过超临界霍普夫分岔（supercritical Hopf bifurcation）共同演化，捕捉了从静止稳定态到并合驱动星暴的转变过程。
...
### [21] [Distribution functions for spheroids](https://arxiv.org/abs/2602.23127)
*James Binney*

**TL;DR**: 该论文通过构建由作用积分(J)解析分布函数(DFs)自洽生成引力场的多组分(含暗物质)星系模型，揭示了热组分DFs在J_φ趋近零时需满足特定条件以避免非物理速度分布，证明了径向偏向球对称系统普遍存在四极扰动不稳定性，并指出混沌在绝热盘增长过程中维持这些约束的关键作用。


<details>
  <summary>Details</summary>

Background: 研究探讨了由分布函数（DFs）构建自洽引力场约束的多组分（包括暗物质）星系模型，并分析了其速度分布的非物理性条件及稳定性问题。

Data: 数据来源于通过解析作用积分（action integrals J）的分布函数（DFs）自洽构建的多组分（包括暗物质）星系模型。

Method: 采用基于分布函数（DFs）的自洽引力场构建方法，通过分析作用积分（J）的解析函数来构造包含暗物质等多组分的星系模型。

Result: 由分布函数（DFs）构建的自洽星系模型中，径向偏向的球状系统通常对四极扰动不稳定。
...
### [22] [Extreme Emission Line Galaxies in CEERS Are Powered by Star Formation, not AGN](https://arxiv.org/abs/2602.23310)
...
**TL;DR**: 利用JWST NIRSpec光谱在CEERS深场中发现了6个3.5<z<7的宽线活动星系核(AGN)，证实光度选极端发射线星系(EELGs)中约10%含AGN（深光谱达35%），但窄线Hα主导高等效宽度(EW)源的极端发射，表明AGN不主导此类辐射。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（詹姆斯·韦伯太空望远镜）NIRSpec（近红外光谱仪）光谱数据探究早期宇宙中产生过量电离辐射的极端发射线星系（EELGs）的光谱特征及其来源（活动星系核或年轻恒星形成）。

Data: 数据来源于JWST（James Webb Space Telescope）NIRSpec（Near-Infrared Spectrograph）光谱观测，包括CAPERS、RUBIES、THRILS和CEERS巡天项目，其中THRILS巡天进行了深度约8小时的G395M光谱观测。

Method: 采用JWST（James Webb Space Telescope）NIRSpec（近红外光谱仪）光谱技术分析极端发射线星系（EELGs）的光谱特征，结合G395M THRILS深场巡天数据识别宽线活动星系核（AGN），并通过颜色-颜色图筛选非极端发射线的AGN。

Result: 在3.5<z<7范围内通过G395M THRILS深场巡天新发现6个宽线活动星系核（AGN），且窄线Hα成分在高等效宽度（EW）源中主导发射。
...
### [23] [Revisiting the Perseus Cluster III: Role of Aspherical Explosions on its Chemical Composition and Extension to Metal-Poor Stars and Galaxies](https://arxiv.org/abs/2602.23356)
*Shing-Chi Leung,Henry Yerdon,Seth Walther,Ken'ichi Nomoto,Aurora Simionescu*

**TL;DR**: 通过喷流诱导的非对称爆炸模型（jet-induced explosion mechanism）研究了塌缩星（collapsar）对银河系化学演化的贡献，发现其对铁族元素（如Zn）丰度的显著影响是解释观测到的多种元素趋势的必要因素。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过超新星元素丰度比（如Si族和Fe族）探究恒星爆炸机制（如喷流诱导爆炸）及其对银河系化学演化（如Zn富集）的影响。

Data: 数据来源于日瞳(Hitomi)望远镜对英仙座星系团(Perseus Cluster)中硅族(Si-group)和铁族(Fe-group)元素的精确测量，以及通过大尺度后处理网络计算的喷流诱导爆炸(jet-induced explosion)机制下的详细化学丰度剖面。

Method: 采用喷流诱导爆炸机制（jet-induced explosion mechanism）进行非球对称爆炸模拟，并结合大型后处理网络计算爆炸前后的详细化学丰度分布。

Result: result：采用喷流诱导爆炸机制的非球状爆炸模型改进了镍铁比（Ni/Fe）等特征拟合，并证明塌缩星（collapsar）是解释银河系多元素趋势的必要成分。
...
**TL;DR**: 通过分析类星体B0218+357的毫米波吸收光谱（HCN、HNC及其同位素分子），首次在红移z=0.68处测得宇宙微波背景（CMB）温度为4.50±0.17 K，验证了标准宇宙学模型的预测。


<details>
  <summary>Details</summary>

Background: 该研究旨在通过分析类星体B0218+357的毫米波吸收谱线，测量红移z=0.68处的宇宙微波背景(CMB)温度，验证标准宇宙学模型。

Data: 数据来源于使用阿塔卡马大型毫米波/亚毫米波阵列(Atacama Large Millimeter/submillimeter Array, ALMA)观测获得的类星体B0218+357的毫米波数据，包括HCN、HCO$^{+}$、HNC、H$^{13}$CN和H$^{13}$CO$^{+}$在宇宙学红移$z\!=\!0.68$处的$J$=2-1和$J$=3-2转动跃迁吸收谱线。

Method: 采用蒙特卡洛采样（Monte Carlo sampling）传播连续谱覆盖因子的不确定性，并基于对数正态柱密度分布（lognormal column-density distribution）修正观测光学深度偏差，在辐射平衡假设下推导了光学薄区激发温度（excitation temperature）剖面。

Result: 在红移(z=0.68)处通过HCN和HNC的吸收线系统首次精确测定了宇宙微波背景(CMB)温度为4.50±0.17 K，与标准大爆炸宇宙学模型高度一致。
...
**TL;DR**: 通过分析超大质量天体黑洞（SLABs）在宇宙微波背景上的阴影效应，排除了质量超过$10^{17}\ M_{\odot}$的SLABs存在于最后散射面内的可能性，并得出质量范围$10^{15}$--$10^{18}\ M_{\odot}$的黑洞密度参数$Ω_{BH}$上限为$10^{-5}$。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索万亿太阳质量级别的超大质量天体黑洞（SLABs）的观测约束及其宇宙学意义。

Data: 使用宇宙微波背景（cosmic microwave background, CMB）数据探测超大质量黑洞（SLABs）的阴影作为负源信号。

Method: 通过分析超大质量黑洞（SLABs）在宇宙微波背景（CMB）上产生的负源阴影效应，结合固定质量下随红移增加（超过1.6）的探测优势，建立了对其丰度和质量上限（$Ω_{BH} \lesssim 10^{-5}$）的约束模型。

Result: 超大质量天体黑洞(SLABs)的阴影可在宇宙微波背景上形成可探测的负源信号，其固定质量阴影在红移超过1.6时更易被观测。
...
**TL;DR**: 澳大利亚SKA探路者（ASKAP）的“变源与慢瞬变（VAST）”巡天首次在南天外区域系统探测到117个天体变源（包括27颗脉冲星、40颗射电星、44个活动星系核等），为研究长期射电变源提供了大规模、高频率、均匀的观测数据基础。


<details>
  <summary>Details</summary>

Background: 探索动态射电天空（dynamic radio sky）中分钟至数年时间尺度变化的射电源。

Data: 数据来源于澳大利亚SKA探路者（ASKAP）望远镜在2023年6月至2025年5月期间对276个天区进行的2945次888 MHz波段观测，覆盖约12300平方度，典型灵敏度为0.24 mJy/beam，分辨率12-20角秒，每个天区平均每两个月重访一次，共获得10或11次观测数据。

Method: 采用澳大利亚SKA探路者射电望远镜(ASKAP)进行888 MHz频段观测，通过VAST数据处理管道提取源光变曲线并应用额外筛选以提高可靠性。

Result: 在VAST河外巡天数据发布1中，通过888 MHz观测发现了117个天体物理变源，包括27颗脉冲星、40颗射电恒星（其中10颗为首次在射电波段探测）、44个活动星系核、2颗光学证认超新星、1个超新星候选体、1颗褐矮星以及2个尚未识别的无多波段对应体源。
...
**TL;DR**: 通过一维薛定谔-泊松方程模拟发现，接触相互作用（contact interactions）会改变模糊暗物质（FDM）稳态解的密度轮廓并影响引力坍缩特征阶段（如壳交叉事件）的发生时机。


<details>
  <summary>Details</summary>

Background: 研究探讨了接触相互作用（contact interactions）在一维薛定谔-泊松方程（Schrödinger-Poisson equation）框架下对模糊暗物质（FDM）动力学的调控作用，旨在解决ΛCDM模型在小尺度结构形成预测与观测间的张力问题。

Data: 数据来源为数值模拟(numerical simulations)。

Method: 采用数值模拟方法研究一维薛定谔-泊松方程（Schrödinger-Poisson equation）中接触相互作用对模糊暗物质（FDM）动力学的影响。

Result: 接触相互作用（contact interactions）会改变模糊暗物质（FDM）稳态解的密度轮廓并影响引力坍缩特征阶段（如壳交叉事件）的发生时机。
...
**TL;DR**: 通过修正的相互作用暗能量（IDE）模型发现哈勃常数（H0）随红移呈幂律递减趋势，为缓解哈勃张力提供了理论一致且观测可行的机制。


<details>
  <summary>Details</summary>

Background: 研究主题为通过修正的相互作用暗能量（IDE）模型探讨哈勃常数（H0）随红移的演化，以解决哈勃张力问题。

Data: 数据来源包括DESI DR2和SDSS的重子声波振荡（BAO）、宇宙计时器、Pantheon样本的Ia型超新星以及普朗克CMB距离先验。

Method: 采用修正的相互作用暗能量（interacting dark energy, IDE）模型研究哈勃常数（H_0）红移演化，通过暗能量与暗物质间的能量交换机制实现H_0的红移依赖性。

Result: result：改进的相互作用暗能量（IDE）模型揭示了哈勃常数（H0）随红移呈幂律递减趋势（最佳拟合值α≈10^-2），结合CMB数据后约束增强至10^-5量级，表明该框架为缓解哈勃张力提供了理论自洽且观测可行的机制。
...
**TL;DR**: 研究通过模拟发现超轻轴子（axion）作为暗物质（DM）模型会抑制小质量晕的形成，导致小空洞（半径<25 Mpc/h）合并为更少的大空洞（半径>25 Mpc/h），并降低空洞-晕相关函数，且这些效应随轴子质量（m_a）减小而增强；基于空洞尺寸函数（VSF）的观测数据可显著提升对轴子能量密度的约束，例如欧几里得（Euclid）类巡天能将10^{-25} eV轴子的能量密度限制在暗物质的4.6%以下（95%置信度），比现有限制强约两倍。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙空洞（voids）作为探测暗物质（dark matter, DM）性质（特别是超轻轴子模型）的理想天体，因其受非线性和重子反馈影响最小。

Data: 使用质量峰斑模拟（mass-peak patch simulations）模拟轴子（axion）对空洞（void）的影响，并基于DESI（Dark Energy Spectroscopic Instrument）和欧几里得（Euclid）星系巡天的观测设置生成数据。

Method: 采用质量峰斑块模拟（mass-peak patch simulations）分析超轻轴子（ultra-light axions）对宇宙空洞（voids）形成和演化的影响，并基于空洞尺寸函数（void size function）和空洞-晕相关函数（void-halo correlation function）量化轴子暗物质（DM）的约束条件。

Result: 超轻轴子（ultra-light axions）通过抑制低质量晕的形成导致小空洞（半径<25 Mpc/h）合并为更少的大空洞（半径>25 Mpc/h），并降低空洞-晕相关函数，且这些效应随轴子质量减小而增强。
...
**TL;DR**: 基于Millennium-TNG流体动力学模拟，研究发现Tully-Fisher关系(TF)和基本平面关系(FP)在非线性尺度上因星系结构特性、恒星形成历史和局域环境的相关性而产生显著系统偏差，通过引入恒星形成率、气体质量和恒星质量等附加参数可有效缓解这些偏差。


<details>
  <summary>Details</summary>

Background: 研究探讨了塔利-费舍尔(Tully-Fisher, TF)关系和基本平面(Fundamental Plane, FP)关系在宇宙学参数测量中的应用及其受星系形成物理过程影响的系统偏差问题。

Data: 数据来源于Millennium-TNG（MTNG）流体动力学模拟（hydrodynamical simulation）。

Method: 采用千禧年-TNG（Millennium-TNG）流体动力学模拟评估Tully-Fisher（TF）和基本平面（FP）关系在包含真实星系形成物理条件下的准确性。

Result: 在非线性尺度上，星系形成物理（galaxy formation physics）会导致本动速度（peculiar velocity）测量出现显著系统性偏差，忽略这些效应可能使宇宙学分析产生偏差。
...
**TL;DR**: 首次在1 GHz以下以亚角秒分辨率研究高红移（z=0.822）星系团PSZ2G091.83+26.11中的射电遗迹（radio relic），发现其与射电星系相连且下游区域符合对数正态磁场分布，并揭示了射电功率与星系团质量关系的红移演化迹象。


<details>
  <summary>Details</summary>

Background: 高红移下增强的逆康普顿（IC）能量损失使星系团中的弥散射电谱变陡，使得低频（约100 MHz）观测更具优势。

Data: 数据来源包括欧洲低频阵列（LOFAR）在145 MHz的（亚）角秒分辨率观测数据，以及甚大天线阵（VLA）在更高频率的角秒分辨率补充数据。

Method: 采用欧洲低频阵列(LOFAR)在145 MHz进行亚角秒级分辨率观测，并结合甚大阵(VLA)在更高频率的角秒级分辨率数据，首次在1 GHz以下实现了对射电遗迹(radio relic)的高分辨率成像。

Result: result：在PSZ2G091.83+26.11星系团中首次以亚角秒分辨率（145 MHz）确认了射电遗迹（radio relic）的弥散发射特性，揭示了激波前沿的尖锐不连续性、与射电星系（radio galaxy）的连接关系，以及下游区域对数正态磁场分布的符合性，并发现了射电功率与星系团质量相关性可能存在红移演化的迹象。
...
**TL;DR**: 通过高分辨率数值模拟揭示了原初磁场（PMFs）在引力坍缩过程中受湍流和小尺度发电机（small-scale dynamo）作用的演化机制，发现高雷诺数下湍流触发发电机效应会显著改变金斯尺度（Jeans scale）以下的磁场能谱，其主导性取决于发电机增长时间与自由落体时间的竞争，并指出宇宙磁流体力学模拟需解析金斯尺度及湍流惯性区才能准确捕捉原初磁场记忆的保留尺度。


<details>
  <summary>Details</summary>

Background: 研究探讨了早期宇宙产生的原初磁场（PMFs）在现今大尺度结构中可能留下的可观测印记及其在引力坍缩过程中的演化。

Data: 通过高分辨率直接数值模拟（direct numerical simulations）研究了自引力磁化晕（self-gravitating, magnetized halos）的演化，并采用不同黏度参数探索了不同雷诺数（Reynolds-number）状态下的引力坍缩与磁流体湍流（magnetohydrodynamic turbulence）耦合过程。

Method: 采用高分辨率直接数值模拟（direct numerical simulations）研究自引力磁化晕（self-gravitating, magnetized halos）的演化，通过改变粘度探索不同雷诺数（Reynolds-number）状态下引力坍缩与磁流体动力学湍流（magnetohydrodynamic turbulence）的耦合过程。

Result: 早期宇宙产生的原初磁场（PMFs）在非线性结构形成过程中，其原始特征可在低于金斯尺度的湍流惯性范围内通过小尺度发电机（dynamo）放大而保留。
...
### [33] [STILTS-NLI: A Natural Language Interface for STILTS](https://arxiv.org/abs/2602.22357)
*R. A. Shaw,S. Fotopoulou,M. Taylor,M. Bremer*

**TL;DR**: 开发了STILTS-NLI自然语言接口（natural language interface），通过微调开源大语言模型（LLM）生成准确的天文数据分析工具STILTS命令，降低了用户学习门槛。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过自然语言界面（STILTS-NLI）降低天文数据分析工具（STILTS）的使用门槛，提升用户体验。

Data: 数据来源于人工合成的数据集（synthetically generated dataset），该数据集经过精心筛选和验证以确保覆盖STILTS关键功能并保证生成命令的语法正确性。

Method: 基于合成生成数据集微调紧凑型开源大语言模型（LLM），开发自然语言接口STILTS-NLI以生成STILTS命令。

Result: STILTS-NLI通过微调紧凑型开源大语言模型(LLM)生成准确命令，在降低STILTS使用门槛的同时其性能可媲美甚至超越大型专有模型。
...
### [34] [WFST Astrometric Calibration -- I. Modeling Global Geometric Distortion with Zernike Polynomials](https://arxiv.org/abs/2602.22569)
...
**TL;DR**: 提出基于泽尼克多项式(Zernike polynomials)的自校准方法，应用于宽视场巡天望远镜(WFST)成像数据，发现其全局几何畸变主要由彗差主导，校正后单次曝光系统不确定度中值低于10毫角秒，并将WFST天体测量系统与盖亚(Gaia)DR3坐标系对齐。


<details>
  <summary>Details</summary>

Background: 宽视场成像巡天中几何畸变（geometric distortion）的精确建模对天体测量定标至关重要。

Data: 数据来源于WFST望远镜r波段成像数据与盖亚(Gaia)DR3星表的位置偏移量，以及20250218当晚的82次曝光观测。

Method: 采用基于泽尼克多项式（Zernike polynomials）的自校准方法构建了广域巡天望远镜（WFST）的全局几何畸变（GD）模型。

Result: 基于泽尼克多项式（Zernike polynomials）的自校准方法揭示了WFST望远镜的全局几何畸变主要由光学设计固有的彗差（coma）主导，快速变化则可能源于大气色散校正器（atmospheric dispersion corrector）。
...
### [35] [Demonstrating Single Photon Counting with Kinetic Inductance Detectors from 3.8 to 25 $μ$m](https://arxiv.org/abs/2602.22970)
...
**TL;DR**: 该研究展示了超导微波动态电感探测器（Microwave Kinetic Inductance Detectors）在3.8、8.5、18.5和25微米波长实现单光子计数，测得能量分辨率（$E/δE$）分别为9.9、5.9、3.2和3.3，暗计数率低至4、8、34和48毫赫兹，其中3.8微米波长的膜基器件性能达到声子损失极限，比同类固体基底器件提升一倍以上。


<details>
  <summary>Details</summary>

Background: 现代天文学的主要目标之一是在可见光和红外波段对类地系外行星进行大气表征。

Data: 使用超导微波动态电感探测器（Microwave Kinetic Inductance Detectors）在3.8、8.5、18.5和25 μm波长下进行单光子计数实验，并测量了对应的能量分辨率（E/δE）和暗计数率。

Method: 采用基于超导微波动态电感探测器（Microwave Kinetic Inductance Detectors）的单光子计数技术，结合薄膜基底设计实现中红外波段（3.8、8.5、18.5和25微米）的高分辨率探测。

Result: 在3.8、8.5、18.5和25微米波长下实现了超导微波动态电感探测器（Microwave Kinetic Inductance Detectors）的单光子计数，并分别测得能量分辨率（$E/δE$）为9.9、5.9、3.2和3.3，对应暗计数率为4、8、34和48毫赫兹。
...
### [36] [Phase-Induced Amplitude Apodization Complex Mask Coronagraph (PIAACMC) on-sky demonstration with MagAO-X](https://arxiv.org/abs/2602.23016)
...
**TL;DR**: 首次在亚微米波长下对相位诱导振幅切趾（PIAACMC）日冕仪进行了天基表征和技术验证，实现了0.74λ/D（875nm）和0.76λ/D（z'波段）的小内工作角（IWA），并在1-5λ/D范围内分别达到1.6e-3（875nm）和1.3e-3（z'波段）的实验室平均原始对比度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于推进小内工作角（inner working angle, IWA）日冕仪的技术发展，以实现未来极大望远镜对温带系外行星的高对比度成像。

Data: 数据来源于MagAOX（极端自适应光学仪器）在Las Campanas天文台Magellan Clay 6.5米望远镜上的内部光源和实际天光观测，以及针对875nm窄带（3%带宽）和z'宽带（908nm，14%带宽）优化的相位偏移焦平面掩模。

Method: 采用相位诱导振幅切趾（PIAACMC）日冕仪技术，结合针对875nm窄带和z'宽带滤光片优化的相移焦平面掩模设计，通过内部光源和MagAOX极端自适应光学系统进行地面测试。

Result: 首次在亚微米波长下实现了PIAACMC的0.74λ/D（875nm）和0.76λ/D（z'波段）的小内工作角（IWA）性能验证。
...
### [37] [A High-Throughput AES-GCM Implementation on GPUs for Secure, Policy-Based Access to Massive Astronomical Catalogs](https://arxiv.org/abs/2602.23067)
*Samuel Lemes-Perera,Miguel R. Alarcon,Pino Caballero-Gil,Miquel Serra-Ricart*

**TL;DR**: 提出了一种整合细粒度访问控制策略引擎与GPU加速AES-GCM认证加密协议的框架，通过并行树归约策略将顺序GHASH计算转化为对数时间过程，实现了适用于PB级天文数据的高通量安全加密。


<details>
  <summary>Details</summary>

Background: 大规模天文巡天时代对海量图像数据的高效安全访问需求，尤其关注预发布阶段的数据保密性（confidentiality）和完整性（integrity）保障。

Data: 数据来源于大规模天文巡天（survey）生成的图像目录（image catalogs），包括预发布阶段的保密数据。

Method: 采用基于GPU加速的AES-GCM认证加密协议（authenticated encryption protocol）和并行树归约策略（parallel tree-reduction strategy）优化了原本串行的GHASH计算过程。

Result: 提出了一种集成细粒度访问控制策略引擎与GPU加速AES-GCM认证加密协议的框架，首次将并行树归约策略应用于天文大数据加密以突破GHASH计算瓶颈。
...
### [38] [Deep ensemble graph neural networks for probabilistic cosmic-ray direction and energy reconstruction in autonomous radio arrays](https://arxiv.org/abs/2602.23321)
*Arsène Ferrière,Aurélien Benoit-Lévy,Olivier Martineau-Huynh,Matías Tueros*

**TL;DR**: 提出了一种基于图神经网络(GNN)的无线电探测器阵列电压信号分析方法，通过融合物理知识改进网络架构，实现了超高能宇宙射线(ultra-high-energy cosmic rays)到达方向(0.092°精度)和能量(16.4%分辨率)的高精度重建，并引入不确定性估计增强预测可靠性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用机器学习技术精确重建超高能宇宙射线（ultra-high-energy cosmic rays）的到达方向与能量。

Data: 数据来源于地面射电探测器阵列（ground-based radio detector arrays）记录的电压信号轨迹（voltage traces），并采用包含真实噪声条件的模拟数据（simulated data with realistic noise conditions）进行训练和测试。

Method: 采用图神经网络（GNN）结合物理知识建模的方法，通过将触发天线表示为图结构输入并整合物理约束，实现了超高能宇宙线到达方向与能量的高精度重建。

Result: 通过将物理知识融入图神经网络(GNN)架构和输入数据，该方法在模拟数据中实现了0.092°的角度分辨率和16.4%的电磁能量重建分辨率。
...
### [39] [Lepton asymmetry leading to baryogenesis by primordial black holes](https://arxiv.org/abs/2602.21296)
*Mriganka Dutta,Banibrata Mukhopadhyay,Abhishek Kumar Jha,Mayank Pathak,Siba Prasad Das*

**TL;DR**: 研究发现原初黑洞(PBHs)通过引力效应导致中微子(neutrinos)和反中微子(antineutrinos)能量分裂，产生轻子不对称性(lepton asymmetry)并最终通过电弱时代的sphaleron过程形成观测到的重子不对称性(baryon asymmetry)。


<details>
  <summary>Details</summary>

Background: 该研究探讨了原初黑洞（PBHs）对中微子的引力效应如何通过产生轻子不对称性（lepton asymmetry）进而解决宇宙学中重子产生（baryogenesis）这一未解难题。

Data: 研究中使用的数据来源为理论模拟（simulation）数据，基于弯曲时空中的狄拉克拉格朗日量（Dirac Lagrangian in curved spacetime）和原初黑洞（PBHs）引力效应下的中微子行为计算。

Method: 采用弯曲时空中的狄拉克拉格朗日量（Dirac Lagrangian in curved spacetime）分析原初黑洞（PBHs）对中微子的非厄米矢量（non-Hermitian vector）和厄米赝矢量（Hermitian pseudo-vector）耦合效应。

Result: 原初黑洞（PBHs）对中微子的引力效应通过狄拉克拉格朗日量中的非厄米矢量项导致中微子与反中微子数量密度不对称，从而在130 GeV温度下产生观测到的重子不对称性（baryon asymmetry）。
...
### [40] [Current-carrying string network evolution in an external magnetic field](https://arxiv.org/abs/2602.22396)
*P. B. Barbosa,C. J. A. P. Martins*

**TL;DR**: 该论文扩展了电荷-速度依赖单尺度（CVOS）模型，通过引入外部磁场与载流宇宙弦（cosmic strings）相互作用的能量源机制，发现十种物理可行的标度解中一种可使网络通过该机制获得能量。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙弦（cosmic strings）作为早期宇宙对称性破缺相变产生的拓扑缺陷，及其携带电荷和电流时与外部磁场相互作用的动力学演化机制。

Data: 数据来源为基于速度依赖单尺度模型（VOS model）及其扩展版本电荷-速度依赖单尺度模型（CVOS model）的理论模拟（simulation）数据。

Method: 采用并扩展了电荷-速度依赖单尺度模型（Charge-Velocity dependent One-Scale model, CVOS），通过唯象方法引入外部磁场与载流宇宙弦相互作用的能量源机制。

Result: 扩展的电荷-速度依赖单尺度（CVOS）模型通过引入外部磁场与载流弦的相互作用机制，在十个物理允许的标度解中识别出一个能使弦网络通过该机制获取能量的解。
...
**TL;DR**: 论文提出了一种具有R对称性和规范B-L对称性的最小超对称标准模型（MSSM）扩展，通过四次势驱动的混沌暴胀（chaotic inflation）实现U(1)B-L自发对称性破缺，并利用分数阶平移对称性卡勒势（shift-symmetric Kaehler potential）满足ACT观测数据约束，同时解释了MSSM中的μ项并通过暴胀子（inflaton）衰变到最轻或次轻右手中微子实现非热轻子数生成（non-thermal leptogenesis）。


<details>
  <summary>Details</summary>

Background: 该研究探讨了在最小超对称标准模型（MSSM）基础上扩展具有R对称性和规范B-L对称性的可重整化理论框架，重点研究由四次势驱动的混沌暴胀（chaotic inflation）及其与U(1)B-L对称性自发破缺的关联。

Data: 研究中使用的数据来源于ACT（Atacama Cosmology Telescope）观测数据，并结合了基于R对称性和规范B-L对称性的最小超对称标准模型（MSSM）扩展的理论模拟。

Method: 采用分数阶位移对称卡勒势（fractional shift-symmetric Kaehler potential）分析模型参数（p,N）对暴胀（inflation）和重子生成（baryogenesis）的影响。

Result: result：在满足参数范围1.355<p<6.7和6x10^-5<N<0.7的条件下，分数位移对称卡勒势（fractional shift-symmetric Kaehler potential）实现了与ACT数据一致的混沌暴胀（chaotic inflation），并通过暴胀子（inflaton）衰变到最轻或次轻右手中微子（right-handed neutrinos）实现了非热轻子生成（non-thermal leptogenesis）。
...
### [42] [Ruling Out Spiky WIMP Dark Matter using Indirect Searches](https://arxiv.org/abs/2602.23348)
*Dibya S. Chattopadhyay,P. S. Bhupal Dev,Yago Porto*

**TL;DR**: 通过重分析费米-拉特(Fermi-LAT)、魔幻(MAGIC)和冰立方(IceCube)的间接探测数据，排除了银河系中心(GC)存在暗物质(DM)密度尖峰(spike)的可能性，对弱相互作用大质量粒子(WIMP)质量范围10 GeV至100 TeV的暗物质热冻结产生机制提供了新约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中心超大质量黑洞Sgr A*周围暗物质(DM)密度分布特征（尤其是"尖峰"spike存在性）的间接探测约束问题。

Data: 数据来源于费米伽马射线空间望远镜(Fermi-LAT)、大气切伦科夫成像望远镜(MAGIC)和中微子观测站(IceCube)的间接探测数据。

Method: 采用暗物质(DM)间接探测方法，通过重铸现有光子线和中微子线观测数据对热平均湮灭截面(thermally-averaged annihilation cross section)的约束，结合热冻结机制(thermal freeze-out)确定的湮灭截面值，推导出对银河系中心暗物质尖峰(spike)斜率γsp的新限制。

Result: 通过费米-拉特(Fermi-LAT)和魔幻(MAGIC)对光子通道的观测，排除了银河系中心存在暗物质密度尖峰(spike)的广泛弱相互作用大质量粒子(WIMP)质量范围（10 GeV至100 TeV）。
...
### [43] [The global structure of the time delay likelihood](https://arxiv.org/abs/2602.22307)
*Namu Kroupa,Will Handley*

**TL;DR**: 该论文揭示了高斯过程光变曲线模型下时间延迟（time delay）推断中似然函数普遍存在的边界驱动“W”形结构及其对全局采样方法的误导，提出了增加有效采样点等具体措施以确保贝叶斯推断的稳健性，并指出局部优化方法会因偏好较小延迟而高估哈勃常数（H_0）。


<details>
  <summary>Details</summary>

Background: 研究揭示了时间延迟(time delay)推断中似然函数存在的根本性病理特征及其对标准推断方法的挑战。

Data: 研究中使用的数据来源为模拟数据（simulation data）。

Method: 采用高斯过程(Gaussian process)光变曲线模型分析时间延迟推断的似然性，揭示了其边界驱动的“W”形特征及全局最优解位于真实延迟处的特性。

Result: 在时间延迟（time delay）推断中，似然函数普遍呈现边界驱动的“W”形，其全局最大值位于真实延迟处，并在观测窗口边缘逐渐上升，导致全局采样器（如嵌套采样）易受虚假边缘模式干扰。
...
### [44] [Trial dispersion measure spacing in fast radio burst searches with HEIMDALL](https://arxiv.org/abs/2602.22753)
*E. F. Keane,D. J. McKenna*

**TL;DR**: 研究发现快速射电暴（FRB）搜索工具海姆达尔（heimdall）的色散量容差参数与最小探测深度存在关联，据此可修正多个FRB样本数据以更准确评估巡天完备性。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于快速射电暴（FRB）探测样本与真实天体群体关系的校准问题，旨在通过分析搜索工具的色散量容差参数提升宇宙学应用的统计可靠性。

Data: 数据来源于快速射电暴(FRB)搜索工具\textsc{heimdall}使用的色散测量容忍度参数(DM tolerance)分析结果及多个可进行回溯拟合的FRB样本数据。

Method: 采用FRB搜索工具\textsc{heimdall}中的色散量（dispersion measure）容差参数分析方法，建立了该参数与最小搜索深度的关联关系。

Result: 快速射电暴（FRB）搜索工具\textsc{heimdall}的色散测量（dispersion measure）容差参数与最小搜索深度存在关联性，可用于重新校准多个FRB样本以更准确评估巡天完备性。
...
### [45] [Accretion-modified Stars in Accretion Disks of Active Galactic Nuclei: Contribution to AGN disk viscosity](https://arxiv.org/abs/2602.22478)
*Jun-Rong Liu,Jian-Min Wang,Hua Feng*

**TL;DR**: 研究发现活动星系核（AGN）吸积盘中的恒星质量黑洞（sMBH）通过两种吸积模式（间歇性邦迪爆炸超爱丁顿吸积和稳态爱丁顿吸积）产生的外流驱动湍流，有效促进了吸积盘的角动量传输，其黏度参数α_AMS与中心超大质量黑洞（SMBH）质量和吸积率呈幂律关系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）吸积盘中恒星质量黑洞（sMBH）作为“吸积修饰恒星”（AMS）的动力学反馈及其对盘内角动量传输的影响机制。

Data: 数据来源为理论模拟（simulation）研究，基于活动星系核（AGN）吸积盘中恒星质量黑洞（sMBHs）的两种吸积情景（情景A和情景B）构建参数空间进行计算。

Method: 采用两种吸积情景（情景A：间歇性邦迪爆炸与超爱丁顿吸积；情景B：稳态爱丁顿吸积）计算有效黏滞参数α_AMS，并分析其与中心超大质量黑洞(SMBH)质量(M_p)、无量纲吸积率(ℳ ̇_p)、恒星质量黑洞(sMBH)质量函数及空间分布的标度关系。

Result: AMS反馈通过两种吸积情景（情景A的间歇性邦迪爆炸和情景B的稳定爱丁顿吸积）产生的喷流与盘气体相互作用，有效驱动了AGN盘中的角动量传输，其有效黏滞参数α_AMS分别与ζM_p^2ℳ̇_p和ζM_p^1.5ℳ̇_p^0.1成比例关系。
...
### [46] [Variability of the X-ray obscuring wind in Mrk 335 with XMM-Newton/RGS](https://arxiv.org/abs/2602.23255)
...
**TL;DR**: 通过XMM-Newton/NuSTAR联合观测赛弗特1星系Mrk 335，发现其宽线区(BLR)附近存在三层电离态(log ξ≈3.63/3.10/2.01)的团块状吸积盘风，以不同速度(5820/3210/2140 km/s)外流，且柱密度与电离态在日际尺度上剧烈变化，证实了多相遮蔽现象在十年时间尺度上的重现性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于赛弗特1型星系(Seyfert 1)中由吸积盘风(accretion-disk winds)引发的瞬态X射线遮蔽现象及其多相结构在宽线区(BLR)尺度的动力学演化特征。

Data: 数据来源于2021年6月XMM-Newton/NuSTAR联合观测活动获取的Mrk 335中间流量状态数据，并辅以Swift长期监测数据，其中XMM-Newton的RGS（反射光栅光谱仪）保留了强吸收特征。

Method: 采用自洽光致电离模型(self-consistent photoionisation modelling)分析RGS光谱，结合宽带光谱能量分布确定电离连续谱，追踪三类光致电离吸收体的演化。

Result: 塞弗特1星系中观测到的三个光致电离吸收体（log xi约3.63、3.10和2.01）具有约5820、3210和2140 km/s的流出速度，其性质与2009年中间态报告的三相遮蔽体一致，表明在十年时间尺度上存在反复出现的多相遮蔽现象。
...
### [47] [PICASO 4.0: Clouds and Photochemistry in Climate Models of Brown Dwarfs and Exoplanets](https://arxiv.org/abs/2602.22468)
...
**TL;DR**: 开源大气建模工具PICASO 4.0通过整合自洽云模型（Virga）、挥发物沉降处理、光化学模块和参数化能量注入方案，显著提升了褐矮星（brown dwarfs）和系外行星富氢大气热结构与光谱模拟能力。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于褐矮星（brown dwarfs）和系外行星（exoplanets）富氢大气层的热结构与光谱模拟工具开发，旨在解决非平衡化学（non-equilibrium chemistry）和云层效应等关键科学问题。

Data: 数据来源包括JWST（James Webb Space Telescope）观测数据、Sonora Diamondback网格模型、能量注入大气模型、斑块云模型以及WASP-39b的光化学模型。

Method: 采用辐射-对流平衡模型框架，整合了自洽云模型（Virga）、挥发物雨出与冷阱新机制、光化学模块以及参数化能量注入方案。

Result: \texttt{PICASO 4.0}通过整合自洽云模型\texttt{Virga}、挥发性物种雨出和冷阱新处理方法、光化学支持以及参数化能量注入方案，显著扩展了褐矮星和系外行星富氢大气热结构与光谱模拟能力。
...
### [48] [TEPCat: The Transiting Extrasolar Planet Catalogue](https://arxiv.org/abs/2602.23102)
*John Southworth*

**TL;DR**: 该论文介绍了TEPCat（Transiting Exoplanet Catalogue）数据库，它是一个收录已知凌星行星系统物理和观测特性的关键汇编，为科研社区提供免费在线服务。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过凌星系外行星（transiting exoplanets）的高精度观测数据理解行星物理特性、轨道参数及形成机制，并建立开放数据库支持科学社区共享此类关键信息。

Data: 数据来源为公开的已知系外行星凌星系统观测数据，收录于TEPCat数据库。

Method: 采用关键汇编方法整理已知凌星行星系统的物理和观测属性，并在线维护更新数据库。

Result: TEPCat（Transiting Extrasolar Planets Catalogue）是一个收录已知凌星行星系统物理和观测特性的关键数据库，旨在为科学界提供高精度数据支持。
...
### [49] [KMT-2024-BLG-3237: Another Free-Floating Planet Candidate with Angular Einstein Radius Measurement](https://arxiv.org/abs/2602.22709)
...
**TL;DR**: 通过微引力透镜事件KMT-2024-BLG-3237发现一颗质量约为67.3倍地球质量、距离约7.34千秒差距的孤立自由漂浮行星（FFP）候选体，其爱因斯坦半径（θ_E）为6.30微角秒，可能属于海王星或土星质量范围。


<details>
  <summary>Details</summary>

Background: 该研究探讨了行星形成理论预测的自由漂浮行星（FFPs）通过引力微透镜效应被探测的可能性。

Data: 数据来源于微引力透镜事件KMT-2024-BLG-3237的观测光变曲线，该曲线表现出显著的有限源效应（finite-source effects），振幅较小（≲0.9星等）且时标较短（≲3天）。

Method: 采用引力微透镜（gravitational microlensing）技术分析光变曲线，结合有限源效应（finite-source effects）和贝叶斯（Bayesian）统计方法估算透镜质量和距离。

Result: 通过引力微透镜事件KMT-2024-BLG-3237发现了一个自由漂浮行星（FFP）候选体，其透镜质量估计为67.3_{-42.5}^{+103.2}倍地球质量，距离为7.34_{-2.11}^{+0.96}千秒差距，且未检测到显著宿主恒星特征。
...
### [50] [Confinement transition to gravitational waves in the one-flavor $SU(4)$ Hyper Stealth Dark Matter theory](https://arxiv.org/abs/2602.23002)
...
**TL;DR**: 论文通过格点动力学费米子模拟研究了$SU(4)$规范理论（含单味基本夸克）的热力学特性，发现暗海夸克通过降低Polyakov圈有效势的界面张力减弱了早期宇宙一级禁闭相变产生的引力波振幅。


<details>
  <summary>Details</summary>

Background: 研究聚焦于强相互作用暗物质（Hyper Stealth Dark Matter）模型的低能区段，探索$SU(4)$规范场论中单味基本夸克的热力学性质及其宇宙学意义。

Data: 数据来源于格点量子色动力学（lattice QCD）模拟，采用含单味基本夸克（fundamental quark）的$SU(4)$规范理论进行动力学费米子（dynamical fermion）计算。

Method: 采用格点量子色动力学（Lattice QCD）模拟结合动力学费米子（dynamical fermion）方法分析SU(4)规范场的热力学性质，并通过有效势中的Polyakov loop（Polyakov loop）计算界面张力对引力波振幅的影响。

Result: 在早期宇宙的一阶禁闭相变中，暗海夸克（dark sea quarks）通过降低Polyakov圈有效势的界面张力，数值计算表明会减小引力波振幅。
...
### [51] [Automated Identification of the Tip of the Red Giant Branch in Globular Clusters with Gaia Data](https://arxiv.org/abs/2602.22682)
*Yi Yang,Zhenzhen Shao,Xiaofeng Wang,Xiaochen Zheng*

**TL;DR**: 该研究通过结合天文数据与算法方法，开发了一种自动识别球状星团中红巨星分支顶端(TRGB)的方法，将样本扩大至91个并发现I波段光度在金属丰度大于−1时呈下降趋势，极低金属丰度下TRGB的绝对星等(MI)约为−4.02。


<details>
  <summary>Details</summary>

Background: 研究聚焦于球状星团中红巨星分支顶端（TRGB）的自动化识别方法及其在天文数据分析中的应用。

Data: 数据来源于160个球状星团（globular clusters）的观测数据，并与盖亚（Gaia）数据进行交叉匹配。

Method: 采用局部离群因子(LOF)算法剔除异常值，并用二阶多项式拟合红巨星支(RGB)以确定红巨星支顶端(TRGB)位置。

Result: result：研究发现I波段光度在金属丰度大于−1时呈下降趋势，与先前研究一致，且极低金属丰度下TRGB的绝对星等（$\rm M_{I}$）约为−4.02。
...
### [52] [Using Lithium and Beryllium to Study Structure and Evolution of Rotating Stars: Spite Plateau of Halo Stars](https://arxiv.org/abs/2602.22516)
*Wuming Yang,Shuya Dou,Xiangcun Meng,Yaqian Wu,Shaolan Bi*

**TL;DR**: 通过结合引力沉降、扩散、旋转和磁场效应的模型解释了银河系晕族恒星锂丰度(Li)的Spite高原现象，发现初始锂丰度与宇宙大爆炸核合成(BBN)理论预测一致，表明锂问题源于恒星内部的锂耗损过程。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系晕族恒星中锂（Li）丰度观测值与标准大爆炸核合成（BBN）理论预测值之间的显著差异（即宇宙学锂问题），探讨其可能源于新物理机制或恒星内部锂耗散过程。

Data: 数据来源于银河系晕族恒星（Galactic halo stars）的观测锂（Li）丰度，覆盖有效温度（effective temperature）在6400至5900 K范围内的恒星年龄约8-13 Gyr，以及红巨星分支（red giant branch）恒星（$T_{\mathrm{eff}} \lesssim$ 5200 K）的观测数据。

Method: 采用旋转恒星模型（rotating models）结合引力沉降（gravitational settling）、扩散（diffusion）、旋转（rotation）和磁场（magnetic fields）等物理过程来解释锂丰度观测特征。

Result: 旋转模型预测恒星锂（Li）丰度在8-13 Gyr年龄和6400-5900 K有效温度范围内形成2.0-2.4 dex的Spite高原，随后在5200 K以下急剧下降至约1.0 dex的红巨星分支高原，与观测一致。
...
### [53] [Identify ~20,000 Li-rich Giants in the LAMOST Low-Resolution Survey](https://arxiv.org/abs/2602.22572)
...
**TL;DR**: 基于LAMOST低分辨率光谱数据，通过模板匹配法系统识别出20,000余颗富锂巨星（Li-rich giants），发现其占比约2.5%，并分析了不同演化阶段的富锂巨星出现率。


<details>
  <summary>Details</summary>

Background: 富锂巨星（Li-rich giants）是研究恒星演化和表面增丰过程的重要示踪天体，建立统计上大且均匀的样本至关重要。

Data: 数据来源于大天区多目标光纤光谱望远镜（LAMOST）巡天获取的大规模低分辨率（R~1800）光谱数据集。

Method: 通过模板匹配Li I 6708 Å吸收线与ATLAS9模型大气生成的合成光谱网格来确定恒星锂丰度。

Result: 通过LAMOST低分辨率巡天数据系统性地识别出超过20,000颗富锂巨星(Li-rich giants)，占LAMOST DR9中所有巨星的约2.5%。
...
### [54] [Dyson spheres on H-R diagram](https://arxiv.org/abs/2602.23270)
*Amirnezam Amiri*

**TL;DR**: 研究通过辐射平衡分析发现，白矮星和M型矮星周围的戴森球（Dyson spheres）分别呈现近中红外和更长波段的特征辐射，为基于红外巡天的技术特征搜索提供了观测约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于戴森球（Dyson spheres）作为先进外星文明技术特征（techno-signature）的潜在探测目标，重点分析白矮星和红矮星（M-dwarfs）两类宿主恒星周围戴森球的热辐射特性及其在赫罗图（H-R diagram）上的观测特征。

Data: 研究中使用的数据来源为理论计算（theoretical calculations）和代表性恒星参数（representative stellar parameters），通过辐射平衡论证（radiative balance arguments）推导戴森球（Dyson spheres）的温度-半径关系。

Method: 采用辐射平衡（radiative balance）分析和赫罗图（Hertzsprung-Russell diagram）定位技术，结合典型恒星参数计算温度-半径关系，评估戴森球（Dyson sphere）的观测特征。

Result: 戴森球(Dyson sphere)在白矮星周围呈现更冷、更暗的黑体辐射（峰值在近红外至中红外波段），而在M型红矮星周围辐射更强但波长更长。
...
### [55] [Revisiting the Perseus Cluster II: Metallicity-Dependence of Massive Stars and Chemical Enrichment History](https://arxiv.org/abs/2602.23354)
*Shing-Chi Leung,Seth Walther,Henry Yerdon,Ken'ichi Nomoto,Aurora Simionescu*

**TL;DR**: 这篇论文通过扩展恒星演化和核心坍缩超新星（CCSN）模型（初始质量15-60倍太阳质量，金属丰度0-1倍太阳金属丰度），结合大尺度后处理网络计算爆炸前后化学丰度剖面，研究了CCSN对α链元素（如Si、S、Ar）、奇数元素（如P、K、Cl）和铁族元素（如Mn、Ni）的贡献，并以英仙座星系团为基准优化了化学演化模型参数。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用核心坍缩超新星（CCSN）模型和恒星演化模型探讨不同初始质量（15-60倍太阳质量）和金属丰度（Z=0至太阳金属丰度Z⊙）条件下α链元素、奇数元素及铁族元素的核合成过程及其对宇宙化学增丰的贡献。

Data: 数据来源于日瞳望远镜（Hitomi）对英仙座星系团（Perseus Cluster）中Si族元素（Si、S、Ar、Ca）和Fe族元素（Cr、Mn、Ni）化学丰度的精确测量结果。

Method: 采用大后处理网络（large post-processing network）计算爆炸前后详细化学丰度剖面，并基于初始质量15-60M⊙和金属丰度Z=0-Z⊙的恒星演化模型与CCSN模型进行参数扫描，寻找最佳拟合构型。

Result: 核心结论是通过大质量恒星（15-60M⊙）和不同金属丰度（Z=0-Z⊙）的CCSN模型计算，结合详细核合成网络，确定了CCSN对α链元素、奇数元素和铁族元素的产生贡献，并通过参数扫描找到了与英仙座星系团观测丰度最佳匹配的模型配置。
