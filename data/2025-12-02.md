...
**TL;DR**: 通过高分辨率N体模拟揭示了自相互作用暗物质(SIDM)晕中原始尖峰(prompt cusps)通过改变内区速度弥散梯度延迟核心形成约2倍，并在核心坍缩后期导致约5%偏离NFW参考轨迹的复杂动力学过程。


<details>
  <summary>Details</summary>

Background: 研究聚焦于早期宇宙中由密度扰动直接坍缩形成的原初尖峰（prompt cusps, PCs）在自相互作用暗物质（self-interacting dark matter, SIDM）模型中对暗物质晕中心区域动力学演化的影响。

Data: 数据来源于一套高分辨率N体模拟（N-body simulation），用于研究自相互作用暗物质（SIDM）中嵌入大质量原始尖峰（PCs）的孤立晕（halo）演化。

Method: 采用高分辨率N体模拟(N-body simulations)研究自相互作用暗物质(SIDM)中嵌入大质量原初尖峰(PCs)的孤立晕演化过程。

Result: 在自相互作用暗物质（SIDM）模型中，中心存在大质量原始尖峰（PCs）的孤立晕演化过程中，早期核心形成阶段因内区速度弥散梯度较小而延迟约2倍，而核心坍缩阶段在物理时间上基本一致，但晚期坍缩阶段与NFW晕参考轨迹存在约5%的偏差，这种偏差非平凡地取决于PCs导致的浓度增加及外晕区速度弥散对热传递过程的抑制或增强作用。
...
**TL;DR**: 研究发现，在早期高质量恒星形成阶段，引力束缚的预恒星核（prestellar cores）的质量函数（CMF）斜率（-2.32±0.30）与萨尔皮特初始质量函数（IMF）斜率（-2.35）无显著差异，表明存在普遍的核形成机制，而原恒星核（protostellar cores）的CMF斜率较平缓（-2.05±0.04）且最大核质量更大，可通过考虑外部气体吸入的预恒星CMF重现。


<details>
  <summary>Details</summary>

Background: 研究聚焦于前恒星核(CMF)的质量分布及其与初始质量函数(IMF)的关系，旨在揭示恒星和星团形成的初始条件及其普适性机制。

Data: 数据来源于阿塔卡马大型毫米波/亚毫米波阵列(ALMA)对70微米暗高质量团块的早期阶段调查(ASHES)，包括461个前恒星核心候选体和254个原恒星核心。

Method: 采用ALMA（阿塔卡马大型毫米波/亚毫米波阵列）对461个前恒星核候选体和254个原恒星核进行观测，结合引力束缚条件分析，通过幂律拟合得到核心质量函数（CMF）斜率。

Result: result：前恒星核（prestellar core）候选体的质量通常低于原恒星核（protostellar core），且前恒星核寿命数倍于自由落体时间（freefall time），但随着核质量增加而趋近自由落体时间；包含原恒星核和前恒星核的核心质量函数（CMF）幂律斜率为-2.05±0.04，比Salpeter初始质量函数（IMF）斜率更平缓，而仅含引力束缚前恒星核的CMF斜率（-2.32±0.30）与Salpeter斜率无显著差异，表明存在普适性核心形成机制；考虑外部气体吸积后，原恒星CMF的最大核质量可由前恒星CMF重现，且推断的质量吸积率高于邦迪-霍伊尔-利特尔顿（Bondi-Hoyle-Lyttleton）吸积率，其质量依赖性更平缓（幂律指数更小），与潮汐瓣吸积更一致。
...
**TL;DR**: 通过1024个银河系质量暗晕（Milky Way-mass halos）的流体动力学模拟（DREAMS Project），结合新型权重方案量化晕间差异（halo-to-halo variance）和反馈效应，发现具有盖亚香肠-恩克拉多斯（GSE）类似并合历史的星系呈现更低恒星形成率、更致密盘结构和更球状晕，且银河系当前性质需匹配多次并合事件而非仅重大事件。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过宇宙学流体动力学模拟（cosmological and hydrodynamical simulations）量化银河系质量暗晕（Milky Way-mass halos）形成过程中的理论不确定性，包括晕间差异（halo-to-halo variance）及恒星与黑洞反馈（stellar and black hole feedback）的影响。

Data: 数据来源于DREAMS项目中的1,024个宇宙学和流体动力学放大模拟（zoom-in simulations），这些模拟采用冷暗物质（Cold Dark Matter）模型，每个模拟具有独特的初始条件以及宇宙学和天体物理参数组合。

Method: 采用了一套包含1,024个宇宙学和流体动力学缩聚模拟（zoom-in simulations）的新套件，结合冷暗物质（Cold Dark Matter）模型，并开发了新颖的参数空间加权方案以匹配观测到的恒星质量-晕质量关系。

Result: 发现了具有盖亚香肠-恩克拉多斯（GSE）类似并合历史的星系表现出更低的恒星形成率、更致密的盘结构及更球状的恒星晕。
...
**TL;DR**: 通过TNG100模拟追踪红移(z)=0的恒星形成主序星系历史，发现恒星形成活动呈现周期性双分支模式（中心富金属区和外围低金属区），其时间波动（~0.2 dex）与星系间固有差异（~0.15 dex）共同解释了观测到的恒星形成率(SFR)离散度（~0.25 dex）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(high-redshift)和低红移(low-redshift)星系中间歇性恒星形成周期的物理机制及其对恒星形成主序星系中恒星形成率(star-formation rate, SFR)离散度的解释。

Data: 数据来源于TNG100模拟（TNG100 simulation）中追踪z=0恒星形成中心星系过去7-8吉年的历史。

Method: 采用TNG100数值模拟追踪红移z=0处恒星形成中心星系在过去7-8吉年（Gyr）内的恒星形成历史，重点分析金属丰度空间梯度与恒星形成率（SFR）时空演化的关联性。

Result: 恒星形成活动呈现两个分支：一个发生在星系中心金属富集气体中，另一个始于外围低金属丰度区域并逐渐向内推进。
...
**TL;DR**: 通过JWST/NIRSpec光谱分析7颗低质量恒星形成星系（$\log(M_\star / M_\odot)\sim7.2-9.7$），发现$z\sim2-3$质量-金属丰度关系（MZR）斜率陡峭（$γ=0.48\pm0.11$），表明金属滞留效率随质量快速提升，且$z\sim2$与再电离时期（Epoch of Reionization）间MZR无明显演化。


<details>
  <summary>Details</summary>

Background: 研究星系金属丰度（metallicity）与恒星质量的关系，以揭示宇宙生态系统中的气体吸积、恒星形成和外流过程的历史演化。

Data: 数据来源于JWST/NIRSpec对CECILIA Faint样本中7个连续谱微弱星系（包含Lyα选择和其他低光度恒星形成星系）的超深光谱观测，覆盖恒星质量范围log(M⋆/M⊙)∼7.2-9.7和中等暗弱静止紫外星等（−20.7≲MUV≲−17.3）。

Method: 采用超深JWST/NIRSpec光谱技术对CECILIA Faint样本中的七颗连续谱微弱星系进行观测，结合[O III]/Hβ和[N II]/Hα的经验校准计算气相氧丰度。

Result: 在$z\sim2-3$的低质量星系中，质量-金属丰度关系（MZR）的斜率较陡（$γ= 0.48 \pm 0.11$），表明金属滞留效率随质量快速增加，与能量驱动外流机制一致。
...
**TL;DR**: 通过MaNGA巡天首次系统性地发现了14个空间偏移的宽发射线（FWHM>1000 km/s）候选活动星系核（AGN），其中42%对应星系并合前的恒星核、58%表现出异常高的宽线光度，偏移速度（10-600 km/s）和弱窄线特征支持存在引力波反冲或动力学弹射机制产生的超大质量黑洞（MBH），其面密度上限符合双黑洞随机自旋取向理论预测。


<details>
  <summary>Details</summary>

Background: 研究主题为通过积分视场光谱巡天系统搜寻并研究因超大质量黑洞（MBH）并合或动力学“弹弓”（slingshot）机制而偏离星系核的超大质量黑洞（MBH）及其对共演化和引力波辐射的示踪作用。

Data: 数据来源于Mapping Nearby Galaxies at Apache Point Observatory (MaNGA)巡天项目，通过积分视场光谱(integral field spectroscopy)观测识别出14个核外宽发射线(FWHM>1000 km/s)源。

Method: 采用积分视场光谱（integral field spectroscopy）技术对马普近邻星系巡天（MaNGA）数据进行系统搜索，识别宽发射线（FWHM>1000 km/s）的空间偏移源，结合黑洞双星并合动力学模型和引力波反冲（recoil）或弹弓（slingshot）机制理论框架，分析候选体的运动学特征与寄主星系势场关系。

Result: 通过积分视场光谱巡天首次系统搜寻并约束了抛射大质量黑洞（MBH）的面密度上限，发现42%候选体具有光学对应体符合双黑洞阶段前吸积MBH的恒星核特征，58%表现出相对于恒星核质量上限更高的宽线光度（约为中心活动星系核的2倍），且反冲/弹弓候选体的窄发射线特征弱化68%，与抛射MBH情景一致。
...
**TL;DR**: 研究发现湍流主导的分子云中致密核（dense cores）间距与声速尺度（sonic scale）或驱动尺度（driving scale）无统计相关性，表明云内碎裂过程不受湍流级联固有尺度支配。


<details>
  <summary>Details</summary>

Background: 研究探讨了湍流主导的分子云中致密核间距与声速尺度(sonic scale)的关系及其在恒星形成理论中的适用性。

Data: 数据来源于使用马赫数（Mach number）$\mathcal{M}=4$和8以及三种湍流驱动波数（turbulent forcing wavenumber）$k_{\rm for}=2,4$和8进行的一系列湍流模拟（simulation）。

Method: 采用湍流模拟（Mach数$\mathcal{M}=4$和8）和三种湍流驱动波数（$k_{\rm for}=2,4$和8），通过柱密度图的树状图分析（dendrogram analysis）识别致密核（dense cores），并利用速度结构函数测量声速尺度（sonic scale）。

Result: 核心结论是湍流主导分子云中的致密核间距与声速尺度（sonic scale）或驱动尺度（driving scale）无统计相关性，表明云内碎裂过程不受湍流级联中固有尺度的支配。
...
**TL;DR**: VPHAS+巡天（VST Photometric Ha Survey）通过高分辨率光学观测绘制了南银河系平面，结合Gaia DR3等数据揭示了恒星分布与银河系翘曲（warping）一致，并发现位于外旋臂（Outer Arm）的B型星（B stars）聚集区（(l,b) = (212.0, -0.6)）。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用VPHAS+光学巡天数据对南天银河系平面（Galactic plane）进行高分辨率测光观测，重点分析第三象限（210°至260°银经）内r星等<19的点源的光度特性及其在银河系结构研究中的应用。

Data: 数据来源于VST Photometric Ha Survey of the Galactic plane and bulge (VPHAS+)的光学波段观测，并辅以Pan-STARRS、DECam Plane Survey (DECaPS-2)、Skymapper的测光数据比对，同时结合盖亚(Gaia)数据释放3、SIMBAD及专业星表进行交叉验证。

Method: 采用VST望远镜进行光学波段观测，结合泛星计划(Pan-STARRS)、DECam平面巡天(DECaPS-2)和Skymapper的测光数据比对，通过合成测光法校正Hα和u波段星等标度，并利用盖亚(Gaia)数据发布3、SIMBAD及专业星表筛选特定类型恒星样本。

Result: VPHAS+巡天数据与Pan-STARRS、DECaPS-2和Skymapper的gri波段临时校准对比发现了微小光度偏移，并通过合成光度法确定了Hα和u波段的光度校正。
...
**TL;DR**: 论文利用JWST UNCOVER场数据，通过深度约束完备性模型和考虑多重成像的源平面选择方法，首次在红移$z=6-8$的紫外波段光度函数(UV LF)中发现了微弱端拐点(turnover)的显著证据（$\alpha\simeq -1$，$\beta\simeq 1.6$），表明再电离时期极暗弱星系对电离贡献的早期抑制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（James Webb Space Telescope）观测数据探究高红移（$z=6-8$）紫外光度函数（UV LF）的演化特征及其对宇宙再电离时期（reionization）中超暗弱星系电离贡献的约束。

Data: 数据来源为詹姆斯·韦伯太空望远镜(JWST)在阿贝尔2744(Abell 2744)背景下的UNCOVER巡天场观测数据，结合了两种引力透镜模型(CATS和GLAFIC)计算的有效体积(V_eff,l)。

Method: 采用深度关联的完备性模型（depth-tied completeness model）和考虑多重成像的源平面选择（source-plane selection）方法，结合透镜依赖的有效体积计算（lens-dependent effective volumes）和统计误差分箱处理（binned LFs with Gehrels errors），独立拟合透镜模型并验证暗端转折（faint-end turnover）的显著性。

Result: result：研究发现在红移$z=6-8$的紫外光度函数（UV LF）中存在暗端转折（faint-end turnover），其峰值后验参数为$\alpha\simeq -1$、$\beta\simeq 1.6$，且$M_T$倾向于先验分布的高值端（$\sim -18$）。
...
**TL;DR**: 首次利用俄罗斯6米望远镜和SCORPIO-2多模式仪器对绿豌豆星系（Green Bean）SDSSJ095100.54+051026.7进行长缝光谱和扫描法布里-珀罗干涉观测，揭示了其延展气体区域的电离条件。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于绿豌豆星系（Green Bean）这一罕见星系类型，其作为活动星系核（AGN）生命周期中的短暂阶段，以星系周介质中存在大尺度强电离云为特征，为理解AGN寿命、反馈机制及辐射与动能活动模式转换提供了独特视角。

Data: 数据来源于俄罗斯6米望远镜使用SCORPIO-2多模式仪器对Green Bean星系（绿豌豆星系）进行的长缝光谱和扫描法布里-珀罗干涉仪观测。

Method: 采用长缝光谱（long-slit spectroscopy）和扫描法布里-珀罗干涉（scanning Fabry-Perot interferometry）技术，结合SCORPIO-2多模式仪器观测数据，通过光谱拟合分析延展气体区域的电离条件。

Result: SDSSJ095100.54+051026.7的扩展气体区域电离条件通过俄罗斯6米望远镜SCORPIO-2多模式仪器的长缝光谱和扫描法布里-珀罗干涉观测首次被表征。
...
**TL;DR**: 通过分析红赫歇尔源（red-Herschel sources）样本的ALMA观测数据，发现474个多重星系系统（包括420个双星、51个三星和3个四星系统），其中双星系统约32%可能物理关联，而多重系统（三星和四星）中约70%包含至少一个关联对，表明亚毫米波星系（submillimetre galaxies）的恒星形成率增强主要由内部过程驱动而非大尺度相互作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于红赫歇尔源（red-Herschel sources）的多重性（multiplicity），探讨亚毫米星系（submillimetre galaxies）中恒星形成率增强的驱动机制及其与大规模相互作用的关联。

Data: 数据来源于ALMA（Atacama Large Millimeter/submillimeter Array）档案观测数据，共分析了3089个观测区域中的2416个有ALMA探测的场区。

Method: 采用非参数检验(non-parametric tests)和模拟分析(mock catalogue simulations)评估多星系系统的物理关联性(physical association)。

Result: 在红赫歇尔源（red-Herschel sources）样本中发现了474个多重系统（420个双星、51个三星和3个四星系统），其中双星系统最亮源平均贡献总流量的64%，三星和四星系统分别贡献48%和42%。
...
**TL;DR**: 基于16个邻近旋涡星系样本，研究发现星系年龄与暗物质质量、密度及质光比(M/L)呈显著正相关(r≈0.91)，证实了星系演化历史与暗物质积累的紧密联系，为暗物质随时间累积提供了观测证据。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系年龄与暗物质含量(dark matter content)之间的关联，为理解星系演化过程中暗物质积累的时间依赖性提供了观测依据。

Data: 数据来源于IOA Tokyo公开数据库的原始旋转曲线(rotation curve)数据，并结合通过文献调研获取的恒星族(stellar population)研究资料推算星系年龄。

Method: 采用三组分分解模型（Hernquist核球、指数盘和NFW暗物质晕）结合蒙特卡洛模拟拟合旋转曲线数据，并通过NFW尺度半径确定暗物质主导起始点，利用开普勒动力学计算总包围质量。

Result: 银河年龄与暗物质质量（dark matter mass）、密度及质量光度比（M/L ratios）呈显著正相关（皮尔逊r≈0.91，斯皮尔曼ρ≈0.93），表明演化历史与暗物质积累存在稳健关联。
...
### [13] [OzDES Reverberation Mapping of Active Galactic Nuclei: Final Data Release, Black-Hole Mass Results, & Scaling Relations](https://arxiv.org/abs/2512.01261)
...
**TL;DR**: 澳大利亚暗能量合作项目（OzDES）通过6-7年的反响映射（Reverberation Mapping）观测735个活动星系核（AGN），测量了62个超大质量黑洞（supermassive black holes）的质量，建立了Hβ、MgII和CIV发射线最精确的延迟-光度关系（lag-luminosity relation），并首次发现MgII发射区可能比Hβ更远离中心。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用反响映射（Reverberation Mapping）技术测量高红移活动星系核（AGN）中超大质量黑洞（supermassive black holes）的质量及其宽线区（broad line region）物理尺度。

Data: 数据来源于澳大利亚暗能量巡天（OzDES）合作项目对735个活动星系核（AGN）在可见光波段进行的6-7年光度和光谱观测，覆盖红移范围$z\in [0.13, 3.85]$和光度范围$\log_{10}(L_{\mathrm{bol}})\in [44.3, 47.5] \mathrm{erg/s}$。

Method: 采用反响映射（Reverberation Mapping）技术分析活动星系核（AGN）的Hβ、MgII和CIV发射线反响，以估算宽线区（Broad Line Region）物理尺度并约束超大质量黑洞（Supermassive Black Hole）质量。

Result: 基于6-7年对735个活动星系核(AGN)的观测，成功利用反响映射(Reverberation Mapping)测定了62个超大质量黑洞(SMBH)的质量，并拟合出Hβ和MgII线延迟-光度关系的幂律，其离散度仅为0.25 dex，是目前最精确可靠的结果。
...
### [14] [Possible quasi-periodic optical oscillations of ZTF blazars](https://arxiv.org/abs/2512.00871)
*Na Wang,Guowei Ren,Shun Zhang,Tingfeng Yi,Tong Liu,Mouyuan Sun*

**TL;DR**: 基于ZTF数据在10个耀变体(blazar)样本中发现4个(J 0923.5+4125、J 1221.3+3010、J 1503.5+4759和J 1652.7+4024)存在准周期振荡(QPOs)迹象，其光变曲线均方根-流量(RMS-Flux)呈线性关系且流量服从对数正态分布，并通过多种周期分析方法确认J 1503.5+4759(周期~38.5天)和J 1652.7+4024(~48天)的全局显著性达2.5-2.7σ。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用ZTF（Zwicky Transient Facility）数据探究耀变体（blazar）光学光变特性及其准周期振荡（QPOs）现象。

Data: 数据来源于兹威基瞬变设施（ZTF）过去五年观测的10个耀变体的光学光变曲线（g波段和r波段）。

Method: 采用均方根-流量关系(RMS-Flux relation)、流量分布分析和准周期振荡(QPOs)检测方法（包括加权小波Z变换(WWZ)、Lomb-Scargle周期图(LSP)和自回归积分滑动平均(ARIMA)模型）对光学光变曲线进行详细分析。

Result: 在10个耀变体(blazar)样本中，发现J 0923.5+4125、J 1221.3+3010、J 1503.5+4759和J 1652.7+4024四个源存在准周期振荡(QPOs)候选信号，其中J 1503.5+4759和J 1652.7+4024的全局显著性分别达到~2.7σ和~2.5σ。
...
**TL;DR**: 该研究在1.9<z<3.5红移范围内测量了29个活动星系核(AGN)的CIV发射线时延和25个源的CIV谱线速度弥散，据此估算出0.8至13亿太阳质量的黑洞质量，并发现观测时长导致的筛选效应会影响半径-光度关系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(1.9<z<3.5)活动星系核(AGN)的CIV发射线时延测量及其在超大质量黑洞(SMBH)质量估算中的应用。

Data: 数据来源于暗能量巡天(Dark Energy Survey)反响映射项目中完整观测的29个活动星系核(Active Galactic Nuclei)的CIV发射线时延测量，样本红移范围1.9<z<3.5。

Method: 采用反响映射(reverberation mapping)技术测量CIV发射线的时间延迟和速度弥散(velocity dispersion)，并基于此计算高红移活动星系核(AGN)中心黑洞质量。

Result: 成功测量了29个活动星系核(AGN)的CIV发射线时间延迟(time lags)，其中25个源获得了CIV谱线宽度(spectral linewidth)的流速弥散(velocity dispersion)数据，并据此计算出黑洞质量介于8亿至13亿太阳质量之间。
...
**TL;DR**: 通过模拟早期团块状恒星形成阶段或逆行合并（retrograde merger）发现，这两种机制能显著降低盘状星系（disk galaxies）的恒星形成率（SFR），产生与银河系（MW）类似的化学双峰性（chemical bimodality），而顺行径向合并（prograde radial merger）则无此效果，且内区（$R_{form}<4$ kpc）恒星仅增强而非主导双峰性，同时早期团块相模型能生成与观测相符的古老（>110亿年）贫α（$α-$poor）盘状轨道恒星，支持高红移（$z\approx 1-2$）团块盘星系（clumpy disky galaxies）作为银河系前身星系的假说。


<details>
  <summary>Details</summary>

Background: 研究探讨了高密度恒星形成团块区域和单一并合事件对盘星系中α元素丰度富集（α-rich）和贫乏（α-poor）星族形成的潜在影响，特别针对类似银河系（Milky Way）中约100亿年前发生的盖亚-香肠/恩克拉多斯（Gaia-Sausage/Enceladus, GSE）吸积事件的模拟条件。

Data: 数据来源于GASTRO库中的多个平滑粒子流体动力学（smoothed particle hydrodynamics, SPH）和N体模型（N-body models），这些模型专门用于模拟约100亿年前在银河系（Milky Way, MW）中发生的盖亚-香肠/恩克拉多斯（Gaia-Sausage/Enceladus, GSE）吸积事件。

Method: 采用平滑粒子流体动力学(SPH)+N体模型(GASTRO库)模拟早期高密度恒星形成团块区域和单一合并事件对盘星系α元素丰度双峰分布的影响。

Result: 早期团块状恒星形成阶段或逆行合并显著降低盘状星系恒星形成率（SFR），导致化学双峰性与银河系类似。
...
**TL;DR**: 基于DESI（Dark Energy Spectroscopic Instrument）DR2数据，研究发现银河系（MW）恒星晕在8-200千秒差距范围内呈三轴椭球（轴比10:8:7）且相对银盘倾斜43度，存在16和76千秒差距两处断裂半径，分别与盖亚香肠/恩克拉多斯（GSE）和大麦哲伦云（LMC）相关，内晕（<30千秒差距）呈扁球状且与银盘对齐，外晕转为长球状且垂直于银盘，同时探测到LMC瞬态密度尾迹（断裂半径60千秒差距）和集体密度尾迹（断裂半径100千秒差距），并证实贫金属晕星径向延伸更广。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用DESI（Dark Energy Spectroscopic Instrument）银河系巡天数据揭示银河系恒星晕（stellar halo）的三维形态、径向结构及密度各向异性特征，以探索其在大尺度宇宙结构形成框架下的组装与演化机制。

Data: 数据来源于暗能量光谱仪（DESI）银河系（MW）巡天第二次数据发布（DR2）中的K巨星样本。

Method: 基于DESI（Dark Energy Spectroscopic Instrument）银河系巡天第二次数据发布（DR2）的K巨星样本，采用三维密度建模与径向分布断点分析技术，结合各向异性密度分布测量方法，量化了恒星晕（stellar halo）的形态、取向与径向结构。

Result: 银河系（MW）恒星晕的形状为三轴椭球（轴比10:8:7），相对银盘倾斜43度，并存在约16千秒差距和76千秒差距的两个断裂半径，分别与盖亚-香肠/恩克拉多斯（GSE）和大麦哲伦云（LMC）相关。
...
**TL;DR**: 在W33复合体的红外暗云(IRDCs)中发现45个致密枢纽-纤维系统(HFSs)，揭示了这些系统位于细长子结构交汇处、与致密原恒星群相关且缺乏射电连续谱辐射，表明云/纤维碎片形成团簇并孕育致密HFSs，促进了高效成团恒星形成（常形成大质量恒星）。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于W33复合体中红外暗云（IRDCs）内45个致密枢纽-丝状系统（HFSs）的发现及其在螺旋臂交界处（盾牌臂和矩尺臂）对恒星形成的影响。

Data: 数据来源于斯皮策（Spitzer）望远镜的8和24微米图像、unWISE的12微米图像以及ALMAGAL项目的1.38毫米连续谱核观测。

Method: 采用最小生成树（Minimum Spanning Tree, MST）分析结合斯皮策（Spitzer）8和24微米、unWISE 12微米图像识别红外暗云（IRDCs）中紧凑枢纽-丝状系统（HFSs）的分布及结构特征。

Result: 在W33复合体中发现了45个致密的枢纽-纤维系统（HFSs），这些系统位于盾牌座和矩尺座旋臂交界处的红外暗云（IRDCs）中，主要分布在细长子结构的交叉处，与无射电连续谱的原恒星群相关联。
...
**TL;DR**: 通过结合欧几里得(Euclid)和斯皮策(Spitzer)数据在英仙座区域发现42个高红移(z=3.5-5.5)极端红天体(HIEROs)，证实了尘埃遮蔽的大质量星系在高红移端的丰度可能被低估，凸显了欧几里得望远镜在揭示早期宇宙尘埃遮蔽恒星形成中的重要作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（$z>3$）恒星形成历史的观测局限性，特别是尘埃遮蔽（dust-obscured）和大质量星系在紫外波段（rest-frame UV）的遗漏问题。

Data: 数据来源包括欧几里得早期发布观测（Euclid ERO）的VIS和NISP测光数据、斯皮策（Spitzer）四个波段的红外成像数据以及地面MegaCam的u、g、r、Hα、i、z波段观测数据。

Method: 采用SED拟合（spectral energy distribution fitting）代码Bagpipes估算光测红移（photometric redshift）和物理参数。

Result: 在3.5<z<5.5红移范围内，即使排除可能的活动星系核(AGN)或恒星质量高估的系统，高质量端星系恒星质量函数(GSMF)仍与先前测定结果相当，表明真实丰度可能更高。
...
**TL;DR**: 通过结合哈勃太空望远镜(HST)和盖亚(Gaia)数据，首次精确测定了NGC 6749球状星团的绝对自行运动(proper motion)、成员星认证及内部运动学特征，发现其内部区域呈各向同性而外围存在轻微径向异性，并开发了公开工具GAIAHUB提升自行运动测量精度10倍。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用哈勃太空望远镜(HST)和盖亚(Gaia)卫星的联合观测数据，首次对34个未充分研究的球状星团(globular clusters)进行高精度自行运动(bulk motion)和内部动力学(internal kinematics)研究。

Data: 数据来源包括哈勃太空望远镜（HST）高质量双波段测光数据、盖亚（Gaia）位置测量数据以及公开代码GAIAHUB处理的8年时间跨度的数据集。

Method: 采用公开代码GAIAHUB结合哈勃太空望远镜（HST）和盖亚（Gaia）数据测定自行运动（proper motion），时间基线为8年，将暗弱天体自行运动测量精度提升10倍，并首次实现662颗盖亚仅能定位的恒星的自行运动测量。

Result: 结合盖亚(Gaia)和哈勃太空望远镜(HST)的自行运动数据，首次精确测定了NGC 6749的绝对自行运动、恒星成员身份及内部各向同性运动学特征，其精度比单独使用盖亚数据提高10倍。
...
### [21] [Searching for EeV photons with Telescope Array Surface Detector and neural networks](https://arxiv.org/abs/2512.01638)
...
**TL;DR**: 利用望远镜阵列(Telescope Array)14年数据，通过神经网络分类器区分质子与光子事件，测得超高能光子(能量>10^19 eV和>10^20 eV)的流量上限分别为2.3×10^-3和3.0×10^-4 (km²·sr·yr)^-1，与强子背景一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用超高能光子（ultra-high-energy photons）探索天体物理模型及超出标准模型（beyond-Standard-Model）的新物理场景。

Data: 数据来源于望远镜阵列(Telescope Array)表面探测器(Surface Detector)14年运行期间采集的原始时间分辨信号和重建的成分敏感参数。

Method: 采用神经网络分类器结合重建的成分敏感参数和表面探测器站记录的原始时间分辨信号，并通过实验数据子集微调网络以减少蒙特卡洛模拟偏差。

Result: 观测到的光子候选事件数量与预期强子背景一致，得出光子通量上限$Φ_γ(E_γ> 10^{19} \text{eV}) < 2.3 \cdot 10^{-3}$和$Φ_γ(E_γ> 10^{20} \text{eV}) < 3.0 \cdot 10^{-4} (\text{km}^2 \cdot \text{sr} \cdot \text{yr})^{-1}$。
...
### [22] [Exploring the ultra-faint dwarf Bootes I using JWST and HST: Metallicity distribution and binaries](https://arxiv.org/abs/2512.01547)
...
**TL;DR**: 通过哈勃太空望远镜(Hubble Space Telescope)和詹姆斯·韦伯太空望远镜(James Webb Space Telescope)对超暗弱矮星系(Ultra-faint dwarf galaxies)Bootes I的多波段观测，首次精确测量了其双星比例(30%)和金属丰度分布([Fe/H]<-2的恒星占85%，[Fe/H]<-3的占17%)，揭示了其与球状星团相似的双星性质及极端贫金属特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超暗弱矮星系（UFDs）作为宇宙中最古老、金属丰度最低的恒星系统，其金属丰度分布记录了早期恒星形成、反馈和化学增丰的化石记录，为检验第一代恒星、星系组装和暗物质晕（dark matter halos）模型提供了关键测试。

Data: 数据来源于哈勃空间望远镜（Hubble Space Telescope）和詹姆斯·韦伯空间望远镜（James Webb Space Telescope）对Bootes I的多波段（F606W和F322W2）多历元观测，结合亚巨星分支至M型矮星的深色星等图及恒星自行运动筛选成员星。

Method: 利用哈勃太空望远镜(Hubble Space Telescope)和詹姆斯·韦伯太空望远镜(James Webb Space Telescope)的多波段(F606W和F322W2)深度测光数据，结合恒星自行运动筛选成员星，构建了从次巨星分支到M型矮星的清洁颜色-星等图(color-magnitude diagram)，并通过M型矮星颜色对金属丰度的敏感性约束了金属丰度分布函数(metallicity distribution function)。

Result: 在牧夫座I（Bootes I）中约20±2%的恒星系统是质量比大于0.4的双星，总双星比例约为30%。
...
### [23] [Transition from Outside-in to Inside-Out at $z\sim 2$: Evidence from Radial Profiles of Specific Star Formation Rate based on JWST/HST](https://arxiv.org/abs/2512.01684)
*Jie Song,Enci Wang,Cheng Jia,Cheqiu Lyu,Yangyao Chen,Jinyang Wang,Fujia Li,Weiyu Ding,Guanwen Fang,Xu Kong*

**TL;DR**: 结合JWST和HST高分辨率观测数据，首次对46,313个红移0<z<4、质量log(M*/M⊙)>8的星系进行了空间分辨的恒星质量(Σ*)和恒星形成率(ΣSFR)剖面测量，发现高红移(z>2.5)星系中ΣSFR剖面比Σ*更陡导致sSFR呈负梯度，挑战了仅靠原位恒星形成驱动星系尺寸演化的理论，而低红移(z<2.0)则符合由内向外的生长模式。


<details>
  <summary>Details</summary>

Background: 研究主题为利用JWST和HST高分辨率观测数据，分析CANDELS天区星系在红移0<z<4范围内的恒星质量、恒星形成率(SFR)及多波段形态特征，并探讨星系尺寸演化机制。

Data: 数据来源于JWST（James Webb Space Telescope）和HST（Hubble Space Telescope）的高分辨率观测，覆盖CANDELS天区的46,313个红移0<z<4且恒星质量log(M*/M⊙)>8的星系，并基于静止波长1μm形态学测量了其空间分辨的恒星质量和恒星形成率面密度（Σ*和ΣSFR）分布。

Method: 基于静止帧1微米形态学分析，采用高分辨率JWST和HST观测数据，对46,313个红移0<z<4、恒星质量log(M*/M⊙)>8的星系进行了空间分辨的恒星质量和恒星形成率表面密度（Σ*和ΣSFR）剖面测量。

Result: 在$z>2.5$时，恒星形成率表面密度($Σ_{\rm SFR}$)的径向轮廓与恒星质量表面密度($Σ_*$)近乎平行但略陡，导致特定恒星形成率(sSFR)呈现轻微负梯度，表明该红移段星系无法仅通过原位恒星形成实现尺寸增长。
...
### [24] [First detections of methanol maser lines from a rare transition family](https://arxiv.org/abs/2512.01875)
*Bradley R. Johnson,Simon P. Ellingsen,Shari L. Breen,Maxim A. Voronkov,Tiege P. McCarthy,Lucas J. Hyland*

**TL;DR**: 首次在南部三个大质量恒星形成区（G358.93-0.03、NGC6334I和G345.01+1.79）观测到罕见的II类甲醇（CH3OH）和同位素甲醇（13CH3OH）脉泽（maser）跃迁，并在G358.93-0.03首次探测到41.9 GHz的13CH3OH脉泽线以及在NGC6334I首次确认28.9 GHz的CH3OH脉泽线。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高质量恒星形成区中罕见的II类甲醇脉泽（class II methanol maser）及其同位素（$^{13}$CH$_3$OH）的观测。

Data: 观测数据来自三个南部大质量恒星形成区（G358.93-0.03、NGC6334I和G345.01+1.79）的CH$_3$OH和$^{13}$CH$_3$OH分子脉泽（maser）跃迁（$8_2 \rightarrow 9_1 A^{-}$分别在28.9 GHz和41.9 GHz频段）。

Method: 采用射电天文观测技术对CH$_3$OH和$^{13}$CH$_3$OH的$8_2 \rightarrow 9_1 A^{-}$甲醇脉泽跃迁（ masing transition）进行频谱分析，重点检测28.9 GHz和41.9 GHz频段的同位素脉泽信号。

Result: 首次在G358.93-0.03中探测到41.9 GHz的$^{13}$CH$_3$OH脉泽（maser）线，并在NGC6334I中首次确认了28.9 GHz的CH$_3$OH脉泽线。
...
### [25] [Weak, extended water vapor emission in the Horsehead nebula](https://arxiv.org/abs/2512.01901)
*Dariusz C. Lis,Vincent Maillard,Emeric Bron,Franck Le Petit,Javier R. Goicoechea,Ducheng Lu,David Teyssier*

**TL;DR**: 通过分析马头光致离解区（PDR）的水汽发射及辅助观测数据，发现水汽发射微弱且超出其他PDR示踪剂（如$^{12}$CO或[CI]）范围，模型显示水汽丰度在$A_V \sim 3$星等处达$3.6 \times 10^{-7}$，但基态$o$-H$_2$O 557 GHz线强度被高估至少7倍，需改进水表面化学模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低紫外辐射光子主导区（PDR）中水蒸气分布与丰度的测定及其与分子和原子谱线的关系。

Data: 数据来源包括赫歇尔（Herschel）档案观测的水蒸气发射数据、地面和机载观测的CO同位素分子以及电离碳和原子碳的精细结构谱线数据。

Method: 采用新开发的PDR包装器（PDR wrapper）结合区域几何结构建模，分析赫歇尔（Herschel）存档水汽发射数据及地面/机载CO同位素谱线与碳离子/原子精细结构线观测。

Result: 水蒸气在马头光子主导区(PDR)中的丰度在视星等(A_V)约3等时达到3.6×10^-7，但模型对基态正水分子(o-H2O)557 GHz谱线的预测强度至少高估了7倍。
...
### [26] [JWST & the Waz Arc I: Spatially Resolving the Physical Conditions within a Post-Starburst Galaxy at Redshift 5 with NIRSpec IFS](https://arxiv.org/abs/2512.02000)
...
**TL;DR**: 利用JWST观测发现了一个红移(z=5.04)的强引力透镜放大(μ∼90)的紫外亮但恒星形成显著熄灭的星系，其具有后星暴(post-starburst)特征、空间异质的电离气体金属丰度分布，并首次在z>5探测到弥散电离气体(DIG)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙再电离时期（epoch of reionization）结束后（红移z=5.04）一个受引力透镜（gravitational-lensing）高度放大、静止紫外波段明亮（rest-frame UV-bright）的大质量星系（$M_* \sim 10^{10}$ M$_\odot$）的恒星形成抑制（quenched）特性及其空间化学丰度分布。

Data: 数据来源于NIRSpec/IFS（近红外光谱仪/积分场光谱仪）对一颗受引力透镜（gravitational-lensing）高度放大（$\barμ\sim90$）、再电离时期（epoch of reionization）结束后（$z=5.04$）的静止紫外波段明亮（rest-frame UV-bright）大质量星系（$M_* \sim 10^{10}$ M$_\odot$，$z_{AB}=20.5$）的观测，结合JWST（詹姆斯·韦伯太空望远镜）获取的高保真静止紫外和光学光谱数据。

Method: 采用近红外光谱仪积分场单元（NIRSpec/IFS）观测并结合引力透镜放大效应，对高红移（z=5.04）星系进行空间分辨光谱分析，重点检测巴尔默线、镁离子双线（MgII doublet）及强发射线分布，并利用消光值E(B-V)和电离参数log10U量化恒星形成活动与气体金属丰度。

Result: result：在再电离时期（reionization）结束后观测到一个受引力透镜（gravitational-lensing）高度放大的静止紫外亮星系（UV-bright galaxy），其性质为后星暴（post-starburst），具有弱/缺失发射线和强吸收特征，且存在由A型星（可能包含B型星）主导的较老恒星种群。
...
### [27] [Spatial Phonons: A Phenomenological Viscous Dark Energy Model for DESI](https://arxiv.org/abs/2512.00056)
*Muhammad Ghulam Khuwajah Khan*

**TL;DR**: 该研究提出了一种将空间视为弹性膜（brane）并引入纵向声子流体（phonon fluid）的暗能量唯象模型，通过两个无量纲参数ε和κ确定了声子声速（c_s^2 = κ/ε）和粘性修正导致的瞬态幻影偏差（phantom deviation），并在参数空间扫描中发现当κ≈ε≈1/3且H⋆/H0≈2.1时，该模型能紧密追踪DESI重子声学振荡测量所关注的Chevallier-Polarski-Linder参数化暗能量状态方程w_eff(z)。


<details>
  <summary>Details</summary>

Background: 探索暗能量（dark energy）的唯象模型，将空间视为具有均匀张力（tension $T_s$）的弹性膜（brane），并支持纵向声子流体（phonon fluid）。

Data: 研究采用弹性膜（brane）模型，其中膜张力（tension）$T_s$和三个标量场$φ^I$通过不变式$b = \sqrt{\det B_{IJ}}$构建有效作用量$F(b)$，背景演化由无量纲参数$\varepsilon$和$κ$决定声速$c_s^2 = κ/\varepsilon$，粘性效应通过依赖于哈勃参数$H$的弛豫时间$τ(H)$建模，并假设$Hτ(H)$满足玻尔兹曼抑制散射率形式，最终基于平坦$Λ$CDM背景$H(z)$扫描参数空间。

Method: 采用弹性膜（brane）模型描述空间，引入纵向声子流体（phonon fluid）和三个标量场φ^I，通过有效作用量F(b)（其中b=√det B_IJ）构建背景完美流体，并引入与哈勃率相关的麦克斯韦型弛豫定律（Maxwell type relaxation law）描述体粘滞压力（bulk viscous pressure）的耗散效应。

Result: result：该模型通过弹性膜（brane）张力与声子流体（phonon fluid）的相互作用，在背景水平上重现了具有能量密度、压力和体积模量的理想流体，并发现当参数κ≈ε≈1/3且H⋆/H0≈2.1时，超轻声子的声速接近因果极限，其粘性暗能量状态方程weff(z)在DESI重子声学振荡测量相关红移范围内与DESI驱动的Chevallier-Polarski-Linder参数化高度吻合。
...
### [28] [General-Relativistic Path Effects and the Local Hubble Expansion](https://arxiv.org/abs/2512.00067)
*Said Laaroua*

**TL;DR**: 提出了一种低红移(redshift)光子传播的有效映射方法，通过引入z_eff(z) = z - αf(z)的最小变形关系，在不改变背景膨胀的情况下捕捉了非均匀潮汐场对累积萨克斯红移(Sachs redshift)的主要贡献，为局部结构相关的方向依赖性提供了广义相对论描述。


<details>
  <summary>Details</summary>

Background: 研究主题为探索低红移光子传播的有效映射方法，以捕捉偏离标准FLRW红移的主要路径依赖性效应。

Data: 数据来源于低红移（low-redshift）光子传播的观测数据，通过引入一个最小变形红移关系z_eff(z) = z - αf(z)来捕捉与标准FLRW红移的主要路径依赖性偏差，其中α为小振幅，f(z)在z大于约0.1时抑制修正。

Method: 采用核心分析方法为引入最小变形红移关系z_eff(z) = z - αf(z)，其中α为小振幅参数，f(z)在z > 0.1时抑制修正项，以捕获非均匀潮汐场对累积萨克斯红移（Sachs redshift）的领先贡献。

Result: 提出了一种低红移光子传播的有效映射方法，通过引入最小变形关系z_eff(z) = z - αf(z)捕捉了标准FLRW红移的主要路径依赖偏差。
...
### [29] [Dynamical Heating from Dark Compact Objects and Axion Minihalos: Implications for the 21-cm Signal](https://arxiv.org/abs/2512.00169)
*Badal Bhalla,Aurora Ireland,Hongwan Liu,Huangyu Xiao,Tao Xu*

**TL;DR**: 通过21厘米氢原子超精细跃迁观测，探测到质量在$10^2 M_{\odot}$至$10^5 M_{\odot}$范围内的暗致密天体（dark compact objects）和轴子微晕（axion minihalos）通过动力学摩擦（dynamical friction）加热重子气体，可发现占暗物质总量约10%的暗致密天体，并将对质量在$10^{-18}$ eV至$10^{-9}$ eV范围的类轴子粒子（axion-like particles）的探测灵敏度显著提升。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用21厘米氢原子超精细跃迁观测探测宇宙黑暗时代末期重子温度，以揭示暗物质（dark matter）及其亚结构（如暗致密天体（dark compact objects）和轴子微晕（axion minihalos））通过动力学摩擦（dynamical friction）加热重子的潜在信号。

Data: 数据来源于即将开展的21厘米（21-cm）实验对中性氢（neutral hydrogen）21厘米超精细跃迁（21-cm hyperfine transition）的观测。

Method: 采用21厘米超精细跃迁观测数据，结合暗致密天体（dark compact objects）和轴子微晕（axion minihalos）通过动力学摩擦（dynamical friction）加热重子气体的物理模型，评估其对宇宙黑暗时代末期重子温度的影响。

Result: 通过21厘米全球信号和功率谱测量可探测到占暗物质约10%的暗致密天体（dark compact objects），并显著提升对质量在$10^{-18}$ eV至$10^{-9}$ eV范围的类轴子粒子（axion-like particles）的探测灵敏度。
...
### [30] [Probing Fuzzy Dark Matter in the 21 cm Signal via Wavelet Scattering Transform](https://arxiv.org/abs/2512.00402)
*Hayato Shimabukuro,Shihang Liu,Bohua Li*

**TL;DR**: 通过小波散射变换（WST）分析宇宙黎明和再电离时期的21厘米信号，首次发现该方法能有效区分模糊暗物质（FDM）与冷暗物质（CDM），尤其对FDM粒子质量（$m_{\mathrm{FDM}}=10^{-22}\,\mathrm{eV}$）敏感且抗噪性强。


<details>
  <summary>Details</summary>

Background: 研究探索模糊暗物质（FDM）对宇宙黎明和再电离时期红移21厘米信号的印记。

Data: 数据来源于修改后的\texttt{21cmFAST}模拟，该模拟结合了模糊暗物质（FDM）在线性和非线性结构形成中的效应。

Method: 采用小波散射变换（WST）分析21厘米亮度温度场的一阶和二阶系数，量化尺度依赖的方差和非高斯跨尺度耦合。

Result: 小波散射变换（WST）框架能有效区分模糊暗物质（FDM）与冷暗物质（CDM）情景，其低阶耦合系数对FDM粒子质量高度敏感且能抵御热噪声干扰。
...
### [31] [The SRG/eROSITA All-Sky Survey: A comprehensive X-ray analysis of the Hydra I galaxy cluster](https://arxiv.org/abs/2512.00518)
*Alpish Srivastava,Thomas H. Reiprich,Angie Veronica,Florian Pacaud,Jakob Dietl,Fiona Knies,Manami Sasaki*

**TL;DR**: 通过多波段观测揭示了长蛇座I星系团(Abell 1060)具有介于冷核与非冷核之间的中间态特性，在$R_{500}$内探测到两个弱激波(M≈1.5)与视线方向射电辐射相关，并在$R_{200}$外发现软X射线超($5.8σ$)与光学星系分布高度空间相关，表明外围存在活跃重子物质吸积。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近邻低温星系团长蛇座I星系团（Hydra I galaxy cluster，Abell 1060）的团内介质（ICM）多波段性质及其与光学/红外星系分布的关联，特别关注介于冷核与非冷核之间的过渡特征。

Data: 数据来源包括SRG/eROSITA全天巡天前四次观测数据、钱德拉(Chandra)存档观测数据、TGSS射电数据、2MASS红外数据以及NED光学数据。

Method: 采用SRG/eROSITA全天巡天数据、钱德拉(Chandra)存档观测以及TGSS(射电)、2MASS(红外)、NED(光学)等多波段数据，结合表面亮度分析、光谱分析和宇宙X射线背景(CXB)校正技术，研究了星系团内介质(ICM)的X射线发射与非热辐射、光学/红外星系分布的关联性。

Result: 在长蛇座I星系团（Abell 1060）外围（$R_{200}$以外）探测到多个软X射线超辐射区域，其中北部区域的显著性达$5.8σ$，表明该星系团外围正在活跃吸积重子物质。
...
### [32] [Constraints on Chiral-Quintom dark energy after DESI DR2 and impact on unifying dark energy with inflation](https://arxiv.org/abs/2512.00558)
*Andronikos Paliathanasis,Tommaso Mengoni,Genly Leon,Orlando Luongo*

**TL;DR**: 提出了一种双标量场宇宙学模型（Chiral-Quintom范式），统一描述了暴胀和暗能量（dark energy）驱动的宇宙加速膨胀，并实现了穿越幻影界限（phantom divide）的稳定机制，同时通过低红移观测数据验证了该模型与标准ΛCDM和w₀wₐCDM模型的兼容性。


<details>
  <summary>Details</summary>

Background: 探索暗能量（dark energy）动力学性质及早期（暴胀，inflation）与晚期（暗能量）宇宙加速膨胀的统一描述。

Data: 数据来源包括Pantheon+和Union3.0超新星数据集、观测哈勃参数测量值以及最新DESI发布的重子声波振荡数据。

Method: 采用双标量场宇宙学模型（Chiral-Quintom paradigm），通过指数势和非线性势结合动力学混合，分析早期和晚期宇宙加速膨胀的统一描述机制。

Result: 提出的Chiral-Quintom（手性-Quintom）双标量场宇宙学模型能够统一描述早期（暴胀）和晚期（暗能量）宇宙加速膨胀，并实现穿越幻影（phantom divide）的稳定机制。
...
### [33] [Controlling weak-lensing shear biases from undetected galaxies in the era of Stage IV Surveys](https://arxiv.org/abs/2512.00666)
*Lisa M Voigt*

**TL;DR**: 采用无噪声偏差的机器学习方法量化了类似欧几里得(Euclid)巡天中暗弱星系对剪切(shear)测量的影响，发现暗弱混合星系会导致-0.008的乘性剪切偏差，并确定了影响剪切偏差的关键暗弱星系属性（如视星等分布、空间排列）及其所需约束精度。


<details>
  <summary>Details</summary>

Background: 引力透镜（gravitational lensing）效应作为探测宇宙学模型的关键工具，其背景星系受前景物质扭曲的研究是当前宇宙学观测的核心主题。

Data: 研究中使用的数据来源于基线模拟（baseline simulations），包括对欧几里得（Euclid）类似巡天的微弱混合星系（faint blends）的模拟，要求校准模拟必须包含视星等（apparent magnitude）暗至27.0（+2.1, -0.9）且在明亮样本星系（BSG）约1.0（+0.2, -0.2）角秒范围内的邻近星系。

Method: 采用无噪声偏差的机器学习方法估计剪切（shear），并通过模拟量化微弱星系对类欧几里得（Euclid-like）巡天的影响。

Result: 暗弱星系（faint galaxies）的投影密度、视星等分布及其随亮样本星系（BSG）观测亮度的变化，以及相对于BSG的径向和切向排列与位置各向异性，显著影响剪切偏差（shear bias）。
...
### [34] [Joint Constraints on Fuzzy and Warm Dark Matter from Satellite Populations of the Milky Way and Andromeda](https://arxiv.org/abs/2512.01361)
*Jianxiang Liu,Yan Gong,Kai Liao*

**TL;DR**: 通过联合分析银河系(MW)和仙女座星系(M31)卫星群，对模糊暗物质(FDM)和热遗迹温暗物质(WDM)性质给出迄今最强限制：FDM质量下限为1.75×10⁻²⁰eV(95%置信水平)，WDM质量下限为6.22keV(95%置信水平)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用银河系(MW)和仙女座星系(M31)的卫星星系群联合分析，约束模糊暗物质(FDM)和热遗迹温暗物质(WDM)的性质。

Data: 数据来源包括暗能量巡天(Dark Energy Survey, DES)和泛星计划1(Pan-STARRS1, PS1)对银河系(Milky Way, MW)卫星的观测数据，以及泛仙女座考古巡天(Pan-Andromeda Archaeological Survey, PAndAS)对仙女座星系(Andromeda, M31)卫星的观测数据。

Method: 采用星系-晕连接模型（galaxy--halo connection model）结合相应选择函数（selection functions）对银河系（MW）和仙女座星系（M31）卫星群进行联合建模分析，并通过线性缩放相关模型参数的主星质量先验（host-mass priors）纳入宿主星系质量不确定性。

Result: 模糊暗物质（FDM）的质量下限为1.75×10^-20 eV（95%置信水平）和1.41×10^-20 eV（后验概率比20:1），热遗迹温暗物质（WDM）的质量下限为6.22 keV（95%置信水平）和5.75 keV（后验概率比20:1）。
...
### [35] [Accurate cosmological emulator for the probability distribution function of gravitational lensing of point sources](https://arxiv.org/abs/2512.01607)
*Tunç Türker,Valerio Marra,Tiago Castro,Miguel Quartin,Stefano Borgani*

**TL;DR**: 开发了一种高效精确的引力透镜放大率概率分布函数(PDF)仿真器(ace_lensing)，通过主成分分析(PCA)降维和XGBoost机器学习模型，在宽范围宇宙学参数($Ω_m$, $σ_8$, $w$, $h$)和红移($0.2 \le z \le 6$)空间内实现了透镜PDF的高保真建模，中位KL散度达0.007。


<details>
  <summary>Details</summary>

Background: 研究主题为开发高效精确的引力透镜放大率概率分布函数（PDF）仿真器，用于点源（如超新星和引力波观测）的宇宙学参数推断。

Data: 数据来源于宇宙学N体模拟（cosmological N-body simulations），通过构建过去光锥（past light cones）计算收敛（convergence）和剪切（shear）图。

Method: 采用主成分分析(PCA)降维和极端梯度提升(XGBoost)机器学习模型构建了引力透镜放大率概率分布函数(PDF)的快速仿真器(emulator)。

Result: 开发了一种精确且计算高效的模拟器（emulator），用于建模引力透镜放大率概率分布函数（PDF），能够对超新星和引力波观测等点源进行稳健的宇宙学推断。
...
### [36] [Analytical Emulator for the Baryon EoM inside the Fuzzy Dark Matter Soliton from Machine Learning](https://arxiv.org/abs/2512.01622)
*Ke Wang,Jianbo Lu,Man Ho Chan*

**TL;DR**: 通过机器学习构建了模糊暗物质(FDM)孤子内重子运动方程(EoM)，验证其在FDM孤子形成中与经验性重子密度分布效果相当（误差≲0.04），并适用于其他简单FDM孤子演化。


<details>
  <summary>Details</summary>

Background: 研究探讨了在施温格-泊松方程（SP）中引入经验重子密度分布对模糊暗物质（FDM）孤子形成的影响及其动力学演化。

Data: 研究中使用的数据来源于通过机器学习从获得的FDM密度和总势能构建的解析重子运动方程(EoM)。

Method: 采用机器学习从获得的模糊暗物质(FDM)密度和总势中构建解析重子运动方程(EoM)。

Result: 通过机器学习构建的解析重子运动方程（EoM）在模糊暗物质（FDM）孤子形成中表现与经验重子密度分布相当（分数误差≲0.04），并适用于其他简单FDM孤子演化。
...
### [37] [Constraining Dark Acoustic Oscillations with the High-Redshift UV Luminosity Function](https://arxiv.org/abs/2512.01998)
*Jared Barron,David Curtin,Hongwan Liu,Julian Munoz,Sandip Roy*

**TL;DR**: 通过高红移星系紫外光度函数（UVLF）观测数据，以模型无关方式约束暗声学振荡（DAO）特征，首次将首个DAO峰波数限制为k≳50 h/Mpc（除非经历DAO的暗物质占比低于7%）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于暗物质（dark matter）模型中暗声学振荡（dark acoustic oscillations, DAOs）对宇宙学观测量的潜在影响及其在小尺度上的特征约束。

Data: 数据来源于哈勃太空望远镜（Hubble Space Telescope）、詹姆斯·韦伯太空望远镜（James Webb Space Telescope）、斯巴鲁望远镜（Subaru Telescope）和加拿大-法国-夏威夷望远镜（Canada-France-Hawaii Telescope）的观测数据。

Method: 采用现象学转移函数模型（phenomenological transfer function model）描述暗区物种的暗声学振荡（DAOs），并结合扩展Press-Schechter形式（Extended Press-Schechter formalism）校准N体模拟（N-body simulations）以预测紫外光度函数（UVLF）。

Result: 利用高红移星系紫外光度函数(UVLF)测量数据，以模型无关方式约束小尺度暗声学振荡(DAOs)特征，得出首个DAO峰波数需满足$k \gtrsim 50\ h/\mathrm{Mpc}$，除非经历DAO的暗物质占比低于0.07。
...
### [38] [Comment on 'Color astrophotography with a 100 mm-diameter f/2 polymer flat lens', Appl. Phys. Lett. 126, 051701 (2025)](https://arxiv.org/abs/2512.00054)
*G. K. Skinner,J. F. Krizmanic*

**TL;DR**: 论文指出所描述的透镜（lens）聚焦效率低于0.03%，宽带辐射角分辨率比衍射极限（diffraction limit）差一个数量级，且未聚焦辐射会导致像面背景模糊。


<details>
  <summary>Details</summary>

Background: 研究探讨了透镜（lens）在聚焦效率（focussing efficiency）和宽带辐射（broadband radiation）角分辨率（angular resolution）方面的性能缺陷及其对成像背景噪声（background fog）的影响。

Data: 研究中使用的数据来源未在摘要中明确提及。

Method: 采用衍射极限（diffraction limit）分析透镜的聚焦效率和角分辨率。

Result: 所描述的透镜对宽带辐射的角分辨率比衍射极限(diffraction limit)差一个数量级，且聚焦效率低于0.03%。
...
### [39] [Gravity-darkening exponents for MESA stellar evolution models](https://arxiv.org/abs/2512.00116)
*A. Claret,G. Torres*

**TL;DR**: 基于MESA代码计算了1.0-20.0太阳质量恒星从主序到巨星阶段的演化模型，更新了重力昏暗指数(gravity-darkening exponents, β)值。


<details>
  <summary>Details</summary>

Background: 研究恒星演化过程中重力昏暗指数（gravity-darkening exponent, β）随恒星质量（1.0至20.0太阳质量）和演化阶段（从零龄主序星到巨星阶段）的变化规律。

Data: 数据来源于使用MESA代码（版本7385）计算的恒星演化模型，初始质量覆盖1.0至20.0太阳质量，采用A09不透明度表和混合长参数α_MLT=1.84，成分设定为X=0.70和Z=0.02。

Method: 采用MESA代码（版本7385）计算恒星演化模型，设定初始成分为$X = 0.70$和$Z = 0.02$，选用A09不透明度及混合长参数$α_{\rm MLT} = 1.84$，通过三角策略（triangle strategy）分析不同质量恒星的引力昏暗指数（gravity-darkening exponents）。

Result: 利用MESA代码（版本7385）计算了初始质量1.0-20.0太阳质量恒星从主序到巨星阶段的演化模型，更新了重力昏暗指数（β）值。
...
### [40] [Analysis of Angular-Differential Post-Processing Algorithms for Exoplanet Direct Detection with a Photonic Lantern Nuller](https://arxiv.org/abs/2512.00442)
*Suvinay Goyal,Yinzi Xin,Nemanja Jovanovic,Dimitri Mawet,Michael P. Fitzgerald*

**TL;DR**: 该论文提出了一种基于主成分分析改进的角差分成像（ADI）方法，用于从光子灯笼消零器（PLN）产生的一维非旋转不变数据中提取近距离系外行星的空间参数，并通过模拟数据验证了在恒星估计前注入反行星信号有助于定位低分离度下的行星。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用光子灯笼消零器（PLN）直接探测近轨道系外行星（exoplanet），以克服传统日冕仪（coronagraph）在内工作角（inner working angle）内的局限性，并改进角差分成像（ADI）技术以处理非旋转不变的行星信号。

Data: 数据来源于模拟的一维光子灯笼消零器（PLN）仪器生成的仿真数据。

Method: 采用主成分分析（PCA）重新构建角差分成像（ADI）技术，以从模拟的一维光子灯笼消零器（PLN）数据中提取系外行星的空间参数。

Result: 基于主成分分析重构角差分成像（ADI）方法，通过注入反行星信号优化恒星残余光扣除，有效定位近距系外行星空间参数。
...
### [41] [Intercepting Interstellar Objects](https://arxiv.org/abs/2512.00492)
*Colin Snodgrass,Marina Galand,Arnaud Beth,Charlotte Goetz,Abbie Donaldson,Cyrielle Opitom*

**TL;DR**: 欧空局彗星拦截器任务（ESA Comet Interceptor mission）为未来探测星际天体（Interstellar Object）提供了概念基础，通过快速响应机制实现对太阳系内短暂停留目标的飞越探测。


<details>
  <summary>Details</summary>

Background: 研究主题为利用欧空局彗星拦截器任务（ESA Comet Interceptor）的概念框架设计未来对星际天体（Interstellar Object）的原位探测任务。

Data: 数据来源为欧空局（ESA）彗星拦截器任务（Comet Interceptor）的快速响应观测数据，针对尚未发现的长周期彗星（long period comet）或星际天体（Interstellar Object）进行飞越探测。

Method: 采用快速飞越(fast flyby)技术对星际天体(Interstellar Object)进行原位探测。

Result: 欧空局彗星拦截器任务（ESA Comet Interceptor）的概念框架可扩展用于未来对星际天体（Interstellar Object）的原位探测。
...
### [42] [Quiet Skies Report: A Primer on Protecting Radio Astronomy in the Age of Satellite Mega-Constellations](https://arxiv.org/abs/2512.00941)
*Gregory Hellbourg*

**TL;DR**: 论文揭示了卫星星座（satellite constellations）扩张对射电天文（radio astronomy）的干扰风险，提出了协调频谱使用和管理的共存策略。


<details>
  <summary>Details</summary>

Background: 卫星星座（satellite constellation）快速扩张正在改变地球周边的射频环境，而射电天文（radio astronomy）正进入高灵敏度和巡天能力的新时代，亟需前所未有的干扰控制。

Data: data：研究基于卫星星座（satellite constellation）扩张对地球周围射电环境的影响及射电天文学（radio astronomy）面临的干扰问题。

Method: method：采用概念解释、脆弱性分析和共存管理工具概述的方法研究射电天文（radio astronomy）与卫星星座（satellite constellations）的频谱共存问题。

Result: 卫星星座的快速扩张正在改变地球周围的射频环境，而射电天文学（radio astronomy）正处于高灵敏度和巡天能力的新时代，需要前所未有的干扰控制。
...
### [43] [Euclid Structural-Thermal-Optical Performance](https://arxiv.org/abs/2512.01075)
...
**TL;DR**: 通过结构-热-光学性能（STOP）分析验证了欧几里得（Euclid）卫星的望远镜系统在轨稳态和瞬态条件下满足弱引力透镜（WL）探测对点扩散函数（PSF）形状稳定性的严苛要求，温度引起的光学扰动低于允许限值，但未预料的微小温度变化（<300 mK）对PSF稳定性产生可检测影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于验证欧几里得（Euclid）卫星VIS仪器在弱引力透镜（WL）宇宙学探测任务中满足点扩散函数（PSF）形状与稳定性要求的系统性能。

Data: 研究中使用的数据来源于在轨稳态和瞬态条件下通过结构-热-光学性能（STOP）分析建立的详细有限元数学模型和标准测试案例集。

Method: 采用结构-热-光学性能（STOP）分析方法，通过建立详细的有限元数学模型并定义稳态和瞬态测试案例集来验证望远镜在轨性能。

Result: 温度引起的光学扰动在所有允许观测条件下均远低于允许限值，但轨道温度变化虽小（<300 mK）却对弱引力透镜（WL）科学所需的点扩散函数（PSF）稳定性预算具有不可忽略影响。
...
### [44] [Egent: An Autonomous Agent for Equivalent Width Measurement](https://arxiv.org/abs/2512.01270)
*Yuan-Sen Ting,Serat Mahmud Saad,Fan Liu,Yuting Shen*

**TL;DR**: 开发了结合多沃伊特(Voigt)谱线拟合与大语言模型(LLM)视觉检测的自主代理Egent，能在未预归一化连续谱条件下实现5-7毫埃精度的谱线测量，将传统数月人工分析压缩至数天自动化处理。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发结合经典多Voigt轮廓拟合与大语言模型（LLM）视觉检测的自主代理（Egent），旨在实现光谱分析的自动化迭代优化与质量管控。

Data: 数据来源于84个马盖伦望远镜/MIKE光谱仪（Magellan/MIKE spectra）的信噪比（SNR）约50-250的原始流量光谱（raw flux spectra），包含C3PO项目中人类专家手动测量的18,615条谱线作为验证基准。

Method: 采用经典多Voigt轮廓拟合（multi-Voigt profile fitting）结合大语言模型（LLM）视觉检查与迭代优化的自主代理（Egent）方法。

Result: Egent通过结合经典多Voigt轮廓拟合与大语言模型（LLM）视觉检查及迭代优化，实现了无需预归一化连续谱的原始流量光谱自主分析，其与人工专家测量结果的系统偏移经校正后吻合度达5-7毫埃（milliangstrom）。
...
### [45] [Kratos-polrad: Novel GPU system for Monte-Carlo simulations with consistent polarization calculations](https://arxiv.org/abs/2512.01283)
*Haifeng Yang,Lile Wang*

**TL;DR**: 开发了基于GPU加速的蒙特卡洛辐射传输代码Kratos-polrad，通过斯托克斯参数(Stokes parameters)全传播处理、四元数代数框架和两步偏振成像技术，实现了对尘埃散射(align dust grains)和复杂磁场结构的高效偏振模拟，计算速度比CPU方法提升约100倍。


<details>
  <summary>Details</summary>

Background: 偏振辐射是天体物理学中研究磁场几何结构、散射过程和多维空间构型的关键诊断手段。

Data: data部分：研究基于自行开发的Kratos-polrad代码进行模拟（simulation），该GPU加速蒙特卡洛辐射传输代码采用四元数代数处理颗粒实验室坐标系（grain-lab frame）转换，并实现了非线偏振消光一致性计算。

Method: 采用GPU加速的蒙特卡洛辐射传输方法（Monte Carlo Radiative Transfer），结合四元数代数处理斯托克斯参数（Stokes parameters）的晶粒实验室坐标系变换，并实现非线性偏振消光一致性计算。

Result: 开发了基于GPU加速的蒙特卡洛辐射传输代码Kratos-polrad，通过四元数代数处理斯托克斯(Stokes)参数和颗粒-实验室坐标系转换，实现了自洽高效的偏振辐射传输模拟。
...
### [46] [Revisiting wideband pulsar timing measurement](https://arxiv.org/abs/2512.01288)
...
**TL;DR**: 提出了一种严格考虑测量噪声的宽频带（wideband）脉冲星计时新方法，应用于印度脉冲星计时阵（Indian Pulsar Timing Array）对PSR J2124$-$3358的观测数据，证明其比现有方法能提供更真实的测量不确定度估计。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宽频带（wideband）脉冲星计时中脉冲到达时间与对应色散量（dispersion measure）的同步测量方法优化。

Data: 数据来源于印度脉冲星计时阵列实验（Indian Pulsar Timing Array）使用升级后的巨型米波射电望远镜（upgraded Giant Metre-wave Radio Telescope）对PSR J2124$-$3358的观测。

Method: 提出了一种严格考虑测量噪声的宽频带测量新方法。

Result: 提出了一种严格考虑测量噪声的宽频带（wideband）脉冲星计时新方法，相比现有方法能提供更真实的测量不确定性估计。
...
### [47] [Enhanced detection limits in the SHINE F150 survey through the Regime Switching Model Optimizing thresholds and investigating environmental noise](https://arxiv.org/abs/2512.01511)
*Mariam Sabalbal,Olivier Absil,Carl-Henrik Dahlqvist,Philippe Delorme*

**TL;DR**: 提出基于状态转换模型（RSM）的角差分成像（ADI）检测算法，应用于VLT/SPHERE的SHINE巡天F150样本，通过环境参数聚类优化噪声分布分析和检测阈值设定，将1角秒处探测极限平均提升2倍并发现30余个新信号（含1个待确认候选体）。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于高对比度成像（high-contrast imaging）领域，旨在通过应用新型检测算法——状态切换模型（Regime Switching Model, RSM）提升系外行星探测极限并识别新候选体。

Data: 数据来源于VLT/SPHERE仪器进行的SHINE高对比度成像巡天中的F150样本，共分析了213次观测数据。

Method: 采用状态转换模型（RSM）算法分析SHINE巡天F150样本，并基于环境参数聚类观测数据以优化检测阈值。

Result: RSM算法将F150样本的探测极限在1角秒处平均提高了两倍，在内工作角处提高了五倍。
...
### [48] [Nested Sampling for ARIMA Model Selection in Astronomical Time-Series Analysis](https://arxiv.org/abs/2512.01929)
*Ajinkya Naik,Will Handley*

**TL;DR**: 提出结合ARIMA模型与嵌套采样(Nested Sampling)算法的贝叶斯框架，有效解决了天文时间序列分析中自回归(AR)和移动平均(MA)阶数选择难题，并在太阳黑子、开普勒(Kepler)红巨星及TESS系外行星宿主星观测数据中验证了其准确性和参数推断能力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决大规模、高采样率天文巡天时代下时间序列分析中自回归积分滑动平均（ARIMA）模型阶数选择与过拟合问题。

Data: 数据来源包括模拟数据、太阳黑子年记录（yearly sunspots number record）、开普勒（Kepler）红巨星KIC 12008916的光变曲线数据（Lightcurves data）以及TESS对系外行星宿主星Ross 176的光度测量数据（photometry）。

Method: 采用贝叶斯框架结合自回归积分滑动平均模型（ARIMA）和嵌套采样算法（Nested Sampling），通过向量化实现多阶数网格搜索，并引入奥卡姆惩罚机制抑制过拟合。

Result: 提出了一种结合自回归积分滑动平均(ARIMA)模型与嵌套采样(Nested Sampling)算法的贝叶斯框架，能有效解决天文时间序列分析中模型阶数选择与过拟合问题。
...
### [49] [Pixel-Based Non-Linearity Correction for the WFC3 IR Detector](https://arxiv.org/abs/2512.01951)
*Sachindev S. Shenoy,Ky Huynh,Varun Bajaj,Jennifer Mack*

**TL;DR**: 基于2011-2013年飞行校准数据，为哈勃望远镜广角相机3红外通道(WFC3/IR)建立了像素级非线性校正系数（采用三次多项式拟合），显著提升了接近满阱(80,000电子)像素的测光精度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于改进哈勃太空望远镜广角相机3红外通道(WFC3/IR)的非线性校正(non-linearity correction)方法，以提升其科学数据的测光精度。

Data: 数据来源于2011年至2013年期间利用内部钨灯平场（internal tungsten lamp flats）获取的在轨校准观测数据。

Method: 采用基于飞行校准观测数据的逐像素非线性校正方法，通过拟合每个像素累积信号（up-the-ramp）的三阶多项式来推导新校正系数。

Result: 基于2011-2013年飞行校准观测数据，采用逐像素三次多项式拟合方法建立了新的非线性校正系数（non-linearity correction coefficients），显著提升了接近满阱（full well limit）约80,000电子像素的光度测量精度。
...
### [50] [Real-time RFI Excision Techniques and Their Limitations](https://arxiv.org/abs/2512.01954)
*Kaushal D. Buch,Thushara Gunaratne,Gregory Hellbourg,Cedric Viou,Benjamin Winkel*

**TL;DR**: 该论文系统分析了射电天文观测中面临的射频干扰（RFI）问题，总结了实时抑制技术（如滤波、数字处理和优化调度）并提出了创新解决方案（如智能表面屏蔽和高级算法），强调需加强天文学家与频谱监管机构的合作以保护射电频段。


<details>
  <summary>Details</summary>

Background: 人类活动导致的射频频谱信号密度持续增加，使射电天文面临无线电频率干扰(RFI)日益严重的挑战。

Data: 数据来源包括射电望远镜观测数据（radio telescope observations）和数字处理后的信号数据（digitally processed signal data）。

Method: 采用实时射频干扰（RFI）抑制技术，包括滤波、数字处理、最优调度（optimal scheduling）、卫星规避调度、智能表面屏蔽干扰以及高级计算机算法数据清理。

Result: 人为信号密度增加导致射电天文面临射频干扰(RFI)挑战，需结合实时滤波、数字处理、智能调度等技术手段与政策协调来保障观测质量。
...
### [51] [Astrometric view of companions in the inner dust cavities of protoplanetary disks](https://arxiv.org/abs/2512.00157)
...
**TL;DR**: 通过盖亚天体测量在98个过渡盘（transition disks）中发现32%存在伴星（质量多大于30倍木星质量），其中7个为行星质量伴星（小于13倍木星质量），并指出半数伴星与尘埃空腔形成无关，推测若空腔由伴星造成则其轨道距离应更远且质量更小。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原行星盘（protoplanetary disks）内尘埃空腔（transition disks）作为行星形成标志的潜在机制，探索伴星（companions）对空腔形成的可能影响。

Data: 数据来源为盖亚(Gaia)天体测量数据，用于分析98个过渡盘(transition disks)样本中的行星和恒星伴星。

Method: 采用盖亚天体测量学（Gaia astrometry）计算年轻恒星天体（YSOs）的自行运动异常（proper motion anomalies），结合RUWE参数识别质量比q≳0.01、轨道间距约0.1-30 au的伴星，并评估盘引力、吸积、盘散射光等因素对测量结果的影响。

Result: 在98个过渡盘（transition disks）样本中，32%的源（31个）检测到显著自行运动异常（proper motion anomalies），表明存在伴星（companions），其中7个伴星质量处于行星质量范围（M < 13 M$\rm{_{J}}$）。
...
### [52] [Deep Neural Network-Based High-Precision Identification of Weak Stability Boundary Structures](https://arxiv.org/abs/2512.00457)
*Shuyue Fu,Ziqi Xu,Di Wu,Shengping Gong*

**TL;DR**: 提出基于深度神经网络的高效精确方法识别弱稳定边界结构（weak stability boundary structures），识别精度达97.26-99.91%，并应用于结构构建。


<details>
  <summary>Details</summary>

Background: 研究聚焦于弱稳定边界结构（weak stability boundary structures）在弹道捕获（ballistic capture）和低能转移轨道（low-energy transfers）分析中的应用，旨在解决传统数值与解析方法在计算效率与识别精度上的不足。

Data: 数据来源于通过深度神经网络模型训练和验证的代表性测试数据集（representative test datasets）。

Method: 采用基于深度神经网络的高效精确方法识别弱稳定边界结构（weak stability boundary structures），并通过分析其几何和动力学特性优化模型训练及超参数组合。

Result: 提出了一种基于深度神经网络的高效精确方法，用于识别弱稳定边界结构（weak stability boundary structures），模型在代表性测试数据集上的识别精度达到97.26-99.91%。
...
### [53] [A First Order Filter for the Detection of Potentially Habitable Exoplanets](https://arxiv.org/abs/2512.00899)
*Raka Dabhade,Jebraan Mudholkar,Siddhesh Durgude,Arpit Kottur*

**TL;DR**: 提出基于轨道半长轴与恒星直径比值（d/D_s）的分层筛选框架（G型星≈108，K型≈54，M型≈27），揭示该比值与地球相似指数（ESI）高度相关，为系外行星宜居性快速筛选提供新工具。


<details>
  <summary>Details</summary>

Background: 现代天体物理学的主要目标是搜寻潜在宜居的系外行星（exoplanet），但开普勒（Kepler）和TESS等任务发现的大量候选目标给后续详细表征带来了巨大挑战。

Data: 数据来源于NASA系外行星档案库（NASA Exoplanet Archive）。

Method: 采用基于轨道半长轴与恒星直径比值（d/Ds）的几何筛选方法，结合不同光谱型恒星（G、K、M型）的分级理想比值框架（分别约为108、54、27）建立"宜居性主序带"模型。

Result: 基于轨道半长轴与恒星直径比值（d/D_s）的分层框架（G型星≈108，K型星≈54，M型星≈27）与地球相似指数（ESI）高度相关，可作为筛选潜在宜居系外行星的"宜居主序带"工具。
...
### [54] [Machine Learning for Exoplanet Discovery: Validating TESS Candidates and Identifying Planets in the Habitable Zone](https://arxiv.org/abs/2512.00967)
*Sarah Huang,Chen Jiang*

**TL;DR**: 基于开普勒（Kepler）和凌日系外行星巡天卫星（TESS）数据开发的机器学习框架，通过分析凌日特征、行星参数及宿主恒星性质，以83.9%的交叉验证准确率从3987个TESS候选体中识别出1595个高置信度系外行星（exoplanets），包括100个新发现的多行星系统（multi-planet systems）和15颗位于宜居带（habitable zone）的行星，显著提升了大规模系外行星验证效率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用机器学习方法解决开普勒(Kepler)和TESS任务中海量系外行星候选体验证效率低下的问题。

Data: 数据来源于NASA的开普勒(Kepler)和凌日系外行星巡天卫星(TESS)任务的高精度测光数据，并基于开普勒确认的系外行星和假阳性样本目录进行训练。

Method: 采用机器学习框架，以凌星特性（transit properties）、行星特征（planetary characteristics）及宿主恒星参数（host stellar parameters）为训练特征，通过交叉验证优化模型精度达83.9%。

Result: 开发了一种机器学习框架，利用凌星属性、行星特征和宿主恒星参数作为训练特征，能高效识别真实行星候选体，交叉验证准确率达83.9%。
...
### [55] [Inferring planet occurrence rates from radial velocities](https://arxiv.org/abs/2512.01804)
*J. P. Faria,J. -B. Delisle,D. Ségransan*

**TL;DR**: 提出了一种基于重要性采样（importance sampling）整合多颗恒星径向速度（RV）观测数据后验样本的新方法，无需注入-恢复测试（injection-recovery tests）即可无偏估计行星出现率（planet occurrence rates），且精度随样本量增加而提升。


<details>
  <summary>Details</summary>

Background: 研究主题为基于径向速度（RV）观测的行星出现率后验分布推断方法。

Data: 数据来源于对多颗恒星径向速度（radial-velocity, RV）观测的独立数据集的后验样本，并通过重要性采样（importance sampling）进行重新加权。

Method: 采用重要性采样（importance sampling）方法对多颗恒星的径向速度（RV）数据集后验样本进行重新加权，无需注入-恢复测试或显式定义检测阈值。

Result: 提出了一种基于重要性采样（importance sampling）重新加权径向速度（RV）观测后验样本的新方法，无需注入-恢复测试即可无偏估计行星出现率。
...
### [56] [The Astrometric Resoeccentric Degeneracy: Eccentric Single Planets Mimic 2:1 Resonant Planet Pairs in Astrometry](https://arxiv.org/abs/2512.02007)
*Daniel A. Yahalomi,Tiger Lu,Philip J. Armitage,Megan Bedell,Andrew R. Casey,Adrian M. Price-Whelan,Malena Rice*

**TL;DR**: 该研究揭示了天体测量学（astrometry）中单颗偏心行星与一对2:1共振的共面圆轨道行星在观测信号上的等效性（即偏心-共振简并性），并建立了通过有效偏心率（$e_\mathrm{eff}$）和行星质量比定量描述该简并性的映射关系，同时指出相互轨道倾角可打破此简并性，这对盖亚（Gaia）数据解读和巨行星动力学演化历史推断至关重要。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用盖亚(Gaia) DR4数据解析长周期巨系外行星信号时面临的偏心轨道与双行星2:1轨道共振之间的天体测量学混淆问题（astrometric eccentricity degeneracy）。

Data: 数据来源为盖亚卫星(Gaia)第四批数据(DR4)和模拟数据(simulated Gaia data)。

Method: 采用天体测量学（astrometry）方法分析长周期巨行星的轨道运动，通过谐波分解技术将单颗偏心行星的投影运动分解为基频和一次谐波模式，并建立与双行星2:1轨道共振系统的等效映射关系。

Result: 单颗偏心率行星的星象运动可分解为基频和一次谐波，其振幅与偏心率成正比，从而能模拟2:1轨道共振的双行星系统结构。
...
### [57] [Interstellar Ices as Carriers of Supernova Material to the Early Solar System](https://arxiv.org/abs/2512.00522)
...
**TL;DR**: 该研究发现碳质和非碳质球粒陨石的水蚀变矿物中存在超新星核合成（supernova nucleosynthetic）指纹，揭示了太阳系锆（zirconium）同位素变异性源于星际冰（interstellar ices）与无冰岩石组分的混合，支持原行星盘物质热加工和行星吸积过程中挥发性载体传递核合成差异的模型。


<details>
  <summary>Details</summary>

Background: 研究探讨了太阳系行星物质核合成同位素组成随轨道距离变化的系统差异及其起源，重点关注星际冰携带超新星产生的核素对类地行星前体物质同位素特征的贡献。

Data: 数据来源于碳质和非碳质球粒陨石（carbonaceous and non-carbonaceous chondrite meteorites）中水蚀变矿物的锆石（zirconium）同位素分析，通过浸出实验（leaching experiments）揭示其同位素组成。

Method: 采用锆石（zirconium）同位素分析结合浸出实验（leaching experiments）检测含水蚀变矿物中的超新星核合成指纹。

Result: 水蚀变矿物中发现的极端$^{96}$Zr富集表明其携带了超新星物质，支持太阳系核合成变异性源于原行星盘物质热处理的模型。
...
### [58] [The chemical diversity of giant-planet nurseries as revealed by ALMA](https://arxiv.org/abs/2512.01731)
...
**TL;DR**: 通过ALMA观测六颗年轻F、A和B型恒星周围的过渡盘（transition disks），首次全面揭示了赫比格盘（Herbig disk）化学特征，发现与盘质量相关的C/O>1环境分子示踪物（如CS、C2H）和C/O<1环境分子示踪物（如SO、CH3OH）分布差异，并证实赫比格盘中10天文单位尺度普遍存在低C/O气体，与直接成像系外行星测量结果吻合。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过观测赫比格盘（Herbig disk）的化学组成来揭示巨系外行星（exoplanet）形成条件与恒星质量（1.5-2太阳质量）之间的关联。

Data: 数据来源于阿塔卡马大型毫米波/亚毫米波阵列（ALMA）对六颗年轻F型、A型和B型恒星周围过渡盘（transition disks）的观测，并结合已有文献数据。

Method: 采用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)对六颗年轻F型、A型和B型恒星周围的过渡盘(transition disks)进行分子谱线观测，检测到13C18O、CS、SO和H2CO等分子，并结合文献数据构建了赫比格盘(Herbig disk)化学特征的综合图像。

Result: 在赫比格盘（Herbig disk）中，C/O>1环境的分子示踪物（如CS、C2H）与盘质量（由C18O线通量指示）呈明显正相关，而C/O<1环境的示踪物（如SO、CH3OH）则与盘质量无显著关联且更易在低质量盘中富集。
...
### [59] [Surveys on the Existence of Extraterrestrial Intelligent Life and Effects of Revealing Expert Consensus](https://arxiv.org/abs/2512.00364)
*Omer Eldadi,Gershon Tenenbaum,Abraham Loeb*

**TL;DR**: 研究发现高科学素养群体普遍坚信地外智能生命(extraterrestrial intelligent life)存在（95.01%），但存在双重认知偏差：显著低估周围人认同度（46.07个百分点差异）并高估专家确信强度（实际21.10% vs 感知值），且专家共识信息对修正个人信念效果微弱（效应量d=-0.11）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于公众对地外智慧生命存在性的信念与专家共识之间的认知偏差及其社会心理机制。

Data: 数据来源于对6,114名高学历且科学参与度高的个体进行的问卷调查。

Method: 采用问卷调查法（survey method）评估受访者对地外智慧生命存在性的个人信念、社交圈信念感知、专家信念感知及对专家共识信息的反应。

Result: 95.01%的受访者相信存在地外智慧生命（extraterrestrial intelligent life），其中62.59%持有确定性而非概率性信念。
...
### [60] [Large/small eddy simulations: A posteriori analysis in high Reynolds number isotropic turbulence](https://arxiv.org/abs/2512.00554)
*Chang Hsin Chen,Arnab Moitro,Alexei Y. Poludnenko*

**TL;DR**: 提出大/小涡模拟（L/SES）方法，在泰勒尺度雷诺数约600的均匀各向同性湍流（HIT）中验证其精度和性能，计算成本比直接数值模拟（DNS）低约3个数量级，同时保持与文献一致的全局特性和小尺度特性。


<details>
  <summary>Details</summary>

Background: 研究主题为开发高效高精度湍流模拟方法（大/小涡模拟，L/SES）以突破传统直接数值模拟（DNS）在计算成本和高雷诺数（Reynolds number）应用上的局限性。

Data: 研究中使用的数据来源为直接数值模拟（DNS）和大/小涡模拟（L/SES）生成的仿真数据（simulation data），包括高雷诺数均匀各向同性湍流（HIT）的模拟结果，泰勒尺度雷诺数约600。

Method: 采用大/小涡模拟（Large/Small Eddy Simulation, L/SES）方法，通过耦合低精度大涡模拟（LES）和高精度小涡模拟（SES）来研究高雷诺数均匀各向同性湍流（HIT）。

Result: 大雷诺数（Reynolds number）均匀各向同性湍流（HIT）中，L/SES方法在全局特性（如动能谱、耗散异常）和小尺度特性（如速度梯度高阶矩和间歇量概率密度函数）方面均与文献结果高度吻合。
...
### [61] [Bayesian inference on Calabi--Yau moduli spaces and the axiverse: experimental data meets string theory](https://arxiv.org/abs/2512.00144)
*Mudit Jain,Elijah Sheridan,David J. E. Marsh,Elli Heyes,Keir K. Rogers,Andreas Schachner*

**TL;DR**: 开发了基于贝叶斯推断的工具研究卡拉比-丘流形（Calabi-Yau manifolds）模空间，通过马尔可夫链蒙特卡洛和归一化流采样威滕-彼得松测度（Weil-Petersson measure），揭示了$h^{1,1}\leq30$维度下CY体积谱和除数性质，并结合轴子物理先验与观测约束（如ADMX探测、宇宙微波背景和莱曼α森林数据）构建了弦现象学统计分析的完整框架。


<details>
  <summary>Details</summary>

Background: 研究主题为基于贝叶斯推断的卡拉比-丘流形（Calabi-Yau manifolds）模空间统计分析与轴子（axion）物理性质的关联研究。

Data: 数据来源包括马尔可夫链蒙特卡洛（Markov Chain Monte Carlo）和归一化流（normalising flows）在维度达$h^{1,1}=30$的Kähler模空间上的采样，以及克鲁泽-斯卡克数据库（Kreuzer--Skarke database）中$h^{1,1}\leq 5$的所有不等价卡拉比-丘流形（Calabi--Yau manifolds）的韦尔-彼得松测度（Weil--Petersson measure）边缘化先验。

Method: 采用贝叶斯推断（Bayesian inference）结合马尔可夫链蒙特卡洛（Markov Chain Monte Carlo）和归一化流（normalising flows）技术，在维度达$h^{1,1}=30$的卡勒模空间（Kähler moduli space）上对不变Weil-Petersson测度（invariant Weil--Petersson measure）进行采样分析。

Result: 基于贝叶斯推断在卡拉比-丘流形模空间采样，揭示了受限WP测度下CY体积谱和除数性质。
...
### [62] [Cosmology of Gravi-Axions](https://arxiv.org/abs/2512.00154)
*Stephon Alexander,Gregory Gabadadze,Leah Jenks,Nicolás Yunes*

**TL;DR**: 研究发现引力轴子(gravi-axion)通过欧几里得虫洞非微扰效应获得质量项，可在合理对称性破缺尺度下成为暗物质或暗能量候选者，并通过错位机制或引力粒子产生机制主导宇宙组分，同时晚期天体致密环境中表现为最小耦合的标量场。


<details>
  <summary>Details</summary>

Background: 研究探讨了引力轴子（gravi-axions）在非微扰欧几里得虫洞贡献下如何自然产生质量项并破坏标准理论的连续平移对称性，及其作为宇宙暗物质或暗能量的可能性。

Data: 数据来源于理论推导的欧几里得虫洞（Euclidean wormholes）非微扰贡献和宇宙学相关参数范围内的对称性破缺尺度假设。

Method: 采用非微扰(non-perturbative)欧几里得虫洞(Euclidean wormholes)贡献诱导引力轴子(gravi-axions)质量项，并讨论其作为暗物质或暗能量的宇宙学行为。

Result: 引力轴子（gravi-axion）可通过欧几里得虫洞（Euclidean wormhole）的非微扰贡献自然获得质量项，并打破标准理论的连续平移对称性。
...
### [63] [Widen the Resonance at Ultra-High Energies: Novel Probes of Neutrino Self-interactions in the High-Mass Regime](https://arxiv.org/abs/2512.00165)
*Pedro A. N. Machado,Isaac R. Wang,Xun-Jie Xu,Bei Zhou*

**TL;DR**: 提出利用超高能中微子与宇宙中微子背景（CνB）的共振散射效应探测超出标准模型（BSM）的中微子自相互作用（νSI），通过"拓宽共振"方法将可探测媒介粒子（mediator）质量范围扩展至MeV-GeV量级，耦合强度下限达g∼10^-3，比现有界限提升两个数量级，为高能区中微子自相互作用与暗物质（dark sector）关联研究开辟新途径。


<details>
  <summary>Details</summary>

Background: 探索超越标准模型（Standard Model）的中微子自相互作用（neutrino self-interaction）及其在宇宙学中的新物理意义。

Data: 数据来源包括宇宙中微子背景（cosmic neutrino background）和未来超高能中微子望远镜（如GRAND）的观测数据。

Method: 采用半解析框架（semi-analytic framework）分析宇宙源超高能中微子（UHE neutrino）产生过程，并通过“拓宽共振”（Widen the Resonance）技术增强中微子自相互作用（νSI）在MeV至GeV媒介粒子质量区间的探测灵敏度。

Result: 提出了一种利用超高能中微子与宇宙中微子背景散射来探测超出标准模型的中微子自相互作用（νSI）的新方法，通过"拓宽共振"效应将可探测的媒介粒子质量范围扩展到MeV至GeV量级。
...
### [64] [Probing a minimal dark gauge sector via microlensing of compact dark objects](https://arxiv.org/abs/2512.00701)
*Juan Barranco,Argelia Bernal,Víctor Jaramillo,Darío Núñez,Milton Ruiz*

**TL;DR**: 提出了一种仅含暗U(1)规范对称性的最小暗标准模型(DSM)，发现其标量场孤子(暗"迷你MACHO")的质量半径关系存在临界电荷限制，并结合微引力透镜观测约束标量质量μ≳10 eV。


<details>
  <summary>Details</summary>

Background: 提出了一种最小暗标准模型（DSM），包含一个具有暗$U(1)$规范对称性且与可见区完全解耦的标量粒子，用于探索暗物质（dark matter）的基本性质及其引力效应。

Data: 数据来源于数值模拟（simulation）构建和演化具有暗规范对称性的标量场孤子（gauged scalar-field solitons），并结合微引力透镜巡天（microlensing surveys）观测结果。

Method: 采用数值方法构建并演化具有暗U(1)规范对称性的标量场孤子（gauged scalar-field solitons），推导其质量-半径关系并识别引力束缚态存在的临界电荷阈值。

Result: result：暗标准模型（DSM）中稳定孤子构型的临界电荷和质量-半径关系被确定，结合微透镜观测约束得出标量质量μ≳10 eV（依赖q），表明纯引力观测可限制暗规范场内部参数。
...
### [65] [AI Agent for Source Finding by SoFiA-2 for SKA-SDC2](https://arxiv.org/abs/2512.00769)
*Xingchen Zhou,Nan Li,Peng Jia,Yingfeng Liu,Furen Deng,Shuanghao Shu,Ying Li,Liang Cao,Huanyuan Shan,Ayodeji Ibitoye*

**TL;DR**: 提出基于强化学习（Soft Actor-Critic）的AI代理框架，自动优化射电巡天（如SKA）源提取程序（如SoFiA）的关键参数配置，在SDC2数据集上以更少评估步骤和时间消耗超越基准性能。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用强化学习（Reinforcement Learning, RL）算法自动优化射电波段（radio band）巡天数据中的源提取（source extraction）程序参数配置，以应对下一代大规模巡天项目（如平方公里阵列SKA）数据处理中的关键挑战。

Data: 研究中使用的数据来源于SKA科学数据挑战2（SDC2）数据集。

Method: 采用基于强化学习（Reinforcement Learning, RL）的智能代理框架，结合最先进的软演员-评论家（Soft Actor-Critic, SAC）算法，自动优化射电波段源提取程序（如SoFiA）的关键参数配置。

Result: 提出的基于强化学习（RL）的AI代理框架能自动优化射电波段巡天（如SKA）源提取程序（如SoFiA）的关键参数，在SDC2数据集上仅用100次评估步骤即超越基准性能且耗时更少。
...
### [66] [Probing ALP-photon couplings in Neutron Stars: Scalar versus pseudoscalar cases](https://arxiv.org/abs/2508.09141)
*D. Suárez-Fontanella,C. Albertus,M. Ángeles Pérez-García*

**TL;DR**: 研究了磁化中子星内相互作用轴子场（axion-like field）的分布，发现轴子与电磁场的耦合及其质量共同决定了其空间分布特征，并可能在外层（特别是壳层）形成凝聚态，磁通量管的存在可能增强局部光子转化。


<details>
  <summary>Details</summary>

Background: 研究探讨了磁化中子星内相互作用的轴子类（axion-like）大质量场的分布及其与电磁场的耦合效应。

Data: 研究中使用的数据来源为理论模拟（simulation）数据，基于中子星内部预设的轴对称磁场${\bf B}(r,θ)$和弱电场构建的电磁场不变量$F_0, G_0$，并采用线性形式$Q$求解静态轴子场方程$a(r,θ)$。

Method: 采用线性形式的Q(F_0,G_0)函数耦合轴子场与电磁场，并通过半解析方法求解静态轴子场方程a(r,θ)，同时利用微扰方案处理恒星结构中的压力分量。

Result: 轴子场（axion field）的空间分布由其与磁场、电场的耦合及质量共同决定，并可能在星体外层（特别是壳层）形成凝聚态（condensate）。
...
### [67] [Normal or transitional? The evolution and properties of two type Ia supernovae in the Virgo cluster](https://arxiv.org/abs/2512.00555)
...
**TL;DR**: 研究发现SN 2020nlb的异常红色源于较低的光球温度而非星际红化，证实正常Ia型超新星(SNe Ia)颜色多样性具有物理起源，而SN 2020ue的光度特征类似过渡型SNe Ia但光谱特征接近正常SNe Ia，表明峰值亮度附近光谱分析对构建精确宇宙学样本至关重要。


<details>
  <summary>Details</summary>

Background: Ia型超新星（SNe Ia）作为研究宇宙膨胀历史的精密宇宙学距离指示器，其多样化的子类（如过渡型和快速衰减型）因光度和光谱特性与正常SNe Ia存在差异，导致其作为宇宙学工具的适用性面临挑战。

Data: 数据来源于对室女座星系团（Virgo cluster）中两颗Ia型超新星（SNe 2020ue和2020nlb）的高时间分辨率测光和光谱观测。

Method: 采用高时间分辨率的光变曲线（light-curve）和光谱（spectroscopic）观测数据，结合伪等效宽度（pseudo-equivalent width）等光谱指标分析。

Result: SN 2020nlb的异常红色源于较低的光球温度而非星际红化，表明正常Ia型超新星（SNe Ia）的颜色多样性具有物理起源；而SN 2020ue的光度特性类似过渡型SNe Ia，但光谱特征更接近正常SNe Ia，提示此类边缘样本可能被遗漏。
...
### [68] [Can the Efron-Petrosian Method Recover the Inverse-Square Distance Law for Simulated Radio Pulsar Fluxes?](https://arxiv.org/abs/2512.00784)
*Sanjith A.,Shantanu Desai*

**TL;DR**: 研究发现埃夫隆-彼得罗相（E-P）方法无法准确重现射电脉冲星（radio pulsar）通量的平方反比律（inverse-square law），主要原因是基于信噪比（SNR）截断的合成星表存在非线性依赖性和散射，而改用通量截断的合成星表则能恢复真实距离指数。


<details>
  <summary>Details</summary>

Background: 研究背景是检验埃夫隆-彼得罗相（Efron-Petrosian, E-P）方法能否从合成射电脉冲星（radio pulsar）目录中恢复其流量与距离平方反比律的依赖关系。

Data: 数据来源于使用PsrPopPy软件根据Parkes多波束巡天（Parkes multi-beam survey）规范生成的合成射电脉冲星（radio pulsar）目录。

Method: 采用埃夫隆-彼得罗相（Efron-Petrosian, E-P）方法分析合成射电脉冲星（radio pulsar）目录，检验其能否恢复通量的平方反比律（inverse-square law）关系。

Result: Efron-Petrosian (E-P)方法无法准确重现射电脉冲星通量的平方反比律依赖关系，仅在狭窄的通量阈值范围内部分适用且仍存在偏差。
...
**TL;DR**: 首次通过半经典分子收敛紧耦合方法计算了质子撞击H$_2$的截面，发现质子诱导的H$_2$激发与电子相当，并提出了次级电子谱的新解析参数化方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低能宇宙射线（$E\lesssim 1$ GeV）对分子云的电离和加热作用，特别关注此前被忽视的初级宇宙射线核子（质子和重核）在激发分子氢（H$_2$）过程中的贡献。

Data: 数据来源为基于半经典分子收敛紧耦合方法（semi-classical molecular convergent close-coupling method）计算的质子撞击H$_2$的截面数据。

Method: 采用半经典分子收敛紧耦合方法（semi-classical molecular convergent close-coupling method）计算质子与H$_2$碰撞的截面。

Result: 首次引入半经典分子收敛紧耦合方法计算质子撞击H$_2$的截面，发现质子诱导的H$_2$激发与电子相当。
...
### [70] [Broadband Spectral Analysis of 1ES2344+514: A Multi-epoch study](https://arxiv.org/abs/2512.00750)
*Kishor Chaudhury,Abhradeep Roy,Varsha R. Chitnis,Prajval Shastri,Rajat K. Dey*

**TL;DR**: 通过多波段观测发现1ES 2344+514在2017年7月处于最高耀发态，同步辐射峰(Synchrotron peak)移动证实其属于高同步峰(HSP)BL Lac天体，X射线呈现"越亮越硬"而γ射线呈现"越亮越软"趋势，并利用单区轻子同步辐射+逆康普顿(SSC)模型拟合能谱分布(SED)，发现峰值光度与磁场强度(B)及洛伦兹因子(γ_break)存在弱相关性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于1ES 2344+514的多波段（光学、紫外、X射线和γ射线）光谱特性及其物理参数演化，旨在揭示高同步峰值（HSP）BL Lac天体的辐射机制和磁化特性。

Data: 数据来源于2017年6月6日至2022年8月6日（MJD 57910-59797）期间从Swift-UVOT、Swift-XRT、NuSTAR、AstroSat（SXT和LAXPC）、Fermi-LAT以及MAGIC获取的光学、紫外、X射线和γ波段的近同步观测数据。

Method: 采用稳态轻子单区同步辐射+同步自康普顿（SSC）模型拟合宽带光谱能量分布（SED）。

Result: 观测到同步辐射峰（synchrotron peak）的明显偏移，将1ES 2344+514归类为HSP（high-synchrotron-peaked）型BL Lac天体。
...
### [71] [From Black Hole to Galaxy: Neural Operator: Framework for Accretion and Feedback Dynamics](https://arxiv.org/abs/2512.01576)
*Nihaal Bhojwani,Chuwei Wang,Hai-Yang Wang,Chang Sun,Elias R. Most,Anima Anandkumar*

**TL;DR**: 提出了一种基于神经算子（neural operator）的"亚网格黑洞"模型，通过从小尺度广义相对论磁流体动力学数据学习局部动态，首次实现了吸积驱动反馈（accretion-driven feedback）内在变异性与星系尺度气体的动态耦合。


<details>
  <summary>Details</summary>

Background: 研究主题为超大质量黑洞（supermassive black hole）与宿主星系（host galaxy）共同演化过程中跨尺度物理耦合的建模挑战及数据驱动的亚网格（subgrid）解决方案。

Data: 数据来源于小尺度（毫秒差距）范围内的高分辨率广义相对论磁流体动力学（general relativistic magnetohydrodynamic）模拟数据。

Method: 采用神经算子（neural operator）构建"亚网格黑洞"模型，基于小尺度域广义相对论磁流体动力学数据训练，预测粗网格边界条件和通量，实现无手工闭合项的多尺度动态耦合。

Result: result：基于神经算子（neural operator）的"亚网格黑洞"模型首次实现了吸积驱动反馈（accretion-driven feedback）内在变异性（intrinsic variability）的动态捕捉，实现了中心黑洞与星系尺度气体的动态耦合。
...
### [72] [An outflow from the X-ray corona as the origin of millimeter emission from radio-quiet AGN](https://arxiv.org/abs/2512.01662)
*A. Hankla,A. Philippov,R. Mbarek,R. Mushotzky,G. Musoke,D. Grošelj,M. Liska*

**TL;DR**: 论文提出毫米波发射源自与X射线冕（corona）磁连接的扩展锥形外流（conical outflow）中的非平衡电子，其100 GHz发射位于黑洞$\lesssim10^4$引力半径（$r_g$）内，谱形平坦且毫米波与X射线光度比约$10^{-4}$，与观测一致。


<details>
  <summary>Details</summary>

Background: 毫米波发射（mm emission）的起源与射电宁静活动星系核（RQAGN）中心黑洞附近（小于10^4引力半径）的磁化外流（magnetically connected outflow）相关，类似于太阳风与日冕的关系。

Data: data部分：研究使用了基于广义相对论磁流体动力学（GRMHD）模拟的薄吸积盘案例数据，并结合了射电宁静活动星系核（RQAGN）的多高度延展锥形外流解析模型。

Method: 采用解析模型（analytic model）结合广义相对论磁流体动力学（GRMHD）模拟，假设非平衡电子沿锥形外流分布塑造毫米辐射。

Result: 毫米波发射源自与致密X射线冕区（X-ray corona）磁连接的扩展锥形外流区域，非平衡电子沿不同高度分布塑造了其发射特性。
...
### [73] [Interferometry of Massive Stars: Multiplicity, Magnetism, and Stellar Winds](https://arxiv.org/abs/2512.00444)
*H. Sana E. Bordier,K. Deshmukh,A. J. Frost,A. Keskar,C. Lanthermann,R. R. Lefever,L. Mahy,A. A. C. Sander,T. Shenar,F. Tramper*

**TL;DR**: 光学长基线干涉测量（optical long-baseline interferometry）实现了对大质量恒星（massive stars）的亚天文单位（sub-au）尺度分辨，揭示了其轨道参数、星风结构（wind structures）和磁活动（magnetic phenomena）的直接测量结果。


<details>
  <summary>Details</summary>

Background: 光学长基线干涉测量（optical long-baseline interferometry）已成为研究大质量恒星（massive stars）多星性（multiplicity）、磁场（magnetism）和星风（stellar winds）的高分辨率观测主流技术。

Data: 数据来源为光学长基线干涉测量（optical long-baseline interferometry）的观测数据。

Method: 采用光学长基线干涉测量技术（optical long-baseline interferometry）对恒星的多重性、磁性和星风进行高分辨率观测。

Result: 光学长基线干涉测量（optical long-baseline interferometry）实现了对大质量恒星的多重性、磁性和星风（stellar winds）的高分辨率（亚天文单位尺度）直接观测。
...
### [74] [A tale of three tails: A misaligned streamer and mysterious structures around [BHB2007]1](https://arxiv.org/abs/2512.00295)
...
**TL;DR**: 研究发现原恒星源[BHB2007]1周围存在三条非对称延伸结构（streamer），其中西侧准直结构显示引力束缚下落特征，可显著增加行星形成物质供给并导致盘面倾斜，表明恒星形成过程受环境相互作用持续影响。


<details>
  <summary>Details</summary>

Background: 研究挑战了孤立轴对称恒星形成的传统观点，揭示了原恒星（protostellar）周围流状结构（streamer-like structures）的发现暗示了恒星形成过程中环境相互作用的动态影响。

Data: 数据来源于ALMA（阿塔卡马大型毫米波/亚毫米波阵列）对[BHB2007]1原恒星源的观测。

Method: 采用ALMA（Atacama Large Millimeter/submillimeter Array）观测数据结合运动学分析（kinematic analysis）和形态学建模（morphological modeling）方法研究了[BHB2007]1周围流状结构（streamer-like structures）的动力学特性与起源。

Result: 在[BHB2007]1原恒星源周围发现的三个细长结构中，西侧新识别的准直结构（collimated structure）显示出引力束缚（gravitationally bound）的明确证据，其质量、质量吸积率和角动量估算表明该吸积流（infalling streamer）可能使行星形成物质预算翻倍并使盘面倾斜数十度。
...
### [75] [Ubiquity of Methanol and its related Chemical Segregation in Orion Starless Cores: the ALMASOP Sample](https://arxiv.org/abs/2512.00498)
...
**TL;DR**: 在猎户座A和B分子云的16个无星核（包括5个前恒星核）中，通过高分辨率观测揭示了甲醇（CH3OH）、CCS和c-C3HD的空间分布差异及其与环境辐射场的关联，证实了甲醇在无星核中的普遍存在及其与碳链分子的分层结构。


<details>
  <summary>Details</summary>

Background: 研究聚焦于无星核（starless cores）中复杂有机分子（COMs）的化学特征及其对恒星形成早期阶段和生命前化学（prebiotic chemistry）的启示。

Data: 数据来源于阿塔卡马紧凑阵列(Atacama Compact Array, ACA)和耶贝斯40米望远镜(Yebes 40-m telescope)对猎户座A和B分子云中16个无星核(包括5个前恒星核)的化学观测，目标分子为CH3OH、N2H+、CCS和c-C3HD。

Method: 采用阿塔卡马紧凑阵列(Atacama Compact Array)和Yebes 40米射电望远镜对猎户座A和B分子云中16个无星核(starless cores)进行CH3OH、N2H+、CCS和c-C3HD的化学组分成像观测。

Result: 在猎户座A和B分子云中，对16个无星核（含5个前恒星核）的化学观测揭示了甲醇（CH3OH）、N2H+、CCS和c-C3HD的空间分布差异，其中CH3OH普遍存在且呈现延展扁平结构，CCS和c-C3HD的共现性及缺失模式表明星际辐射场等环境因素影响其分布。
...
### [76] [Substellar Initial Mass Function of Trumpler 14](https://arxiv.org/abs/2512.01606)
*Tamara Rom,Estelle Moraux,Koraljka Mužić,Morten Andersen,Mischa Schirmer,Víctor Almendros-Abad*

**TL;DR**: 在富含O型星的年轻大质量星团Trumpler 14（Tr 14）中，测得初始质量函数（IMF）在0.01-0.2太阳质量（MSun）区间斜率为α=0.14±0.19、0.2-4.5 MSun区间为α=1.72±0.04，并发现高紫外辐射环境可能抑制质量<0.03 MSun的褐矮星（BD）形成。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于年轻大质量星团（如Trumpler 14）中褐矮星（BD）在高密度和强紫外辐射环境下的形成机制及其初始质量函数（IMF）特征。

Data: 数据来源包括GeMS/GSAOI成像观测数据、Besancon银河模型（Besancon Galactic model）用于背景修正以及VISTA巡天的观测对照场（observational control field）。

Method: 采用GeMS/GSAOI成像技术构建初始质量函数(IMF)，结合Besancon银河模型和VISTA观测控制场进行背景校正，并通过人工星测试评估完整性。

Result: 在0.03-1太阳质量范围内，恒星与褐矮星(BD)的比例中位数为4.0（95%置信区间2.8-5.8），表明质量>0.03太阳质量的褐矮星在不同环境中形成效率相似，但高恒星密度和远紫外(FUV)辐射环境可能抑制质量<0.03太阳质量的褐矮星形成。
...
### [77] [Identification of new Galactic symbiotic stars with SALT - II. New discoveries and characterization of the sample](https://arxiv.org/abs/2512.01744)
*J. Merc,J. Mikołajewska,K. Iłkiewicz,B. Monard,A. Udalski*

**TL;DR**: 通过光谱和光度分析确认了14个新的南天银河共生星(symbiotic stars)并发现6个候选体，揭示了其中19个系统的可能轨道周期和8个爆发活动案例，特别发现一个系统具有类似FN Sgr的磁白矮星(magnetic white dwarf)特征，另一个系统存在尘埃遮蔽(dust-obscuration)现象。


<details>
  <summary>Details</summary>

Background: 研究主题为银河系共生星（symbiotic stars）的系统性搜寻与多波段特征分析。

Data: 数据来源于SuperCOSMOS Hα巡天和2MASS的候选体筛选，后续光谱观测使用南非大望远镜(Southern African Large Telescope, SALT)完成，并整合了多源全天巡天档案光变曲线数据。

Method: 采用南天大型望远镜(SALT)进行后续光谱观测，结合超COSMOS Hα巡天和2MASS数据，通过多波段测光变率分析及轨道运动/脉动周期性验证来确认共生星(symbiotic stars)性质。

Result: 发现了14个新确认的共生星（symbiotic stars）并识别出6个强候选体，其中19个确认系统和3个候选系统可能存在轨道周期光变，8个目标显示出爆发活动迹象。
...
### [78] [Search for Peak Structures in the Stochastic Gravitational-Wave Background in LIGO-Virgo-KAGRA O1-O4a Datasets](https://arxiv.org/abs/2512.01776)
*Catalina-Ana Miritescu,Mario Martinez,Oriol Pujolas*

**TL;DR**: 利用LIGO-Virgo-KAGRA网络前三轮及第四轮初期观测数据，首次对具有双峰结构的引力波背景（gravitational-wave backgrounds）进行模型无关的贝叶斯分析，虽未发现显著证据但量化了峰间斜率与振幅的关联约束，为未来探测非单峰谱形奠定了基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用LIGO-Virgo-KAGRA网络数据探索具有非平凡峰结构（nontrivial peak structures）的引力波背景（gravitational-wave backgrounds），以验证多种预测多峰信号的早期宇宙模型（early-Universe models）。

Data: 数据来源于LIGO-Virgo-KAGRA网络的前三次及第四次观测运行初期的引力波各向同性互相关数据。

Method: 采用基于两个归一化分段幂律叠加的模型无关参数化方法，对双峰谱进行贝叶斯推断分析。

Result: 基于LIGO-Virgo-KAGRA网络前三轮及第四轮初期观测数据，未发现具有统计显著性的多峰引力波背景证据，但对信号幅度与峰间斜率相关性给出了约束。
...
### [79] [On extremal black holes](https://arxiv.org/abs/2512.01898)
*Chiara Coviello,Ruth Gregory*

**TL;DR**: 通过考虑前置因子和体积效应，发现真实物理环境中极端带电黑洞（extremal electrically charged black hole）的质量下限需超过10^14太阳质量，且极端磁黑洞（magnetic black hole）的稳定所需电荷在宇宙学上不可行，结合极端克尔黑洞（extremal Kerr black hole）可能通过超辐射散射损失角动量，表明任何类型的极端黑洞（extremal black holes）在宇宙中持续存在的可能性极低。


<details>
  <summary>Details</summary>

Background: 研究探讨了非超对称低能物理框架下物理上真实的极端黑洞（extremal black holes）存在的可能性。

Data: 数据来源包括电荷中性环境中的施温格放电（Schwinger discharge）模拟、中性氢的电离估计、早期宇宙粒子对产生率计算以及奇异瞬子（singular instantons）对磁黑洞稳定性的影响分析。

Method: 采用施温格放电(Schwinger discharge)机制结合体积效应和电离估计，重新评估了极端带电黑洞在电荷中性环境中的稳定性，并通过计算李-奈尔-温伯格(Lee-Nair-Weinberg)不稳定性及奇异瞬子(singular instantons)增强的早期宇宙粒子对产生率，分析了磁单极黑洞的宇宙学可行性。

Result: 极端黑洞（extremal black holes）在低能物理环境中极不可能持续存在。
...
### [80] [Quantum Recoherence in Presence of Excited States in the Early Universe](https://arxiv.org/abs/2512.01932)
*Mattia Cielo,Simone Scarlatella,Gianpiero Mangano,Ofelia Pisanti,Louis Hamaide*

**TL;DR**: 研究揭示了在双场暴胀模型中，初始处于激发态（excited state）的绝热模（adiabatic mode）与有质量熵环境（entropic environment）相互作用会导致纯度冻结（purity-freezing）现象，而邦奇-戴维斯真空态（Bunch-Davies vacuum）是唯一能实现完全再相干（full recoherence）的初始态。


<details>
  <summary>Details</summary>

Background: 研究探讨了原始扰动在双场暴胀框架下从量子态到经典态的转变过程，重点关注绝热模与熵环境（entropic environment）相互作用中的退相干动力学。

Data: 研究采用理论模拟(simulation)方法，基于两场暴胀框架中绝热模与熵环境相互作用的量子-经典转变模型，通过参数化高斯初始态的博戈留波夫(Bogoliubov)系数来生成数据。

Method: 采用量子信息理论方法，通过参数化高斯初始态的博戈留波夫系数（Bogoliubov coefficients）并计算约化绝热态的纯度（purity）和Rényi-2熵（Rényi-2 entropy）来表征退相干动力学。

Result: 在双场暴胀框架中，激发态初始条件会导致绝热模的纯度冻结（purity-freezing）于非零平台，与邦奇-戴维斯真空（Bunch--Davies vacuum）的完全重相干形成鲜明对比。
