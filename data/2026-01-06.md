...
**TL;DR**: 研究发现大麦哲伦云(LMC)和小麦哲伦云(SMC)在1:10质量比相互作用下，LMC暗物质晕会产生约20千秒差距的动力学摩擦尾迹，并在每次SMC近心点移动密度中心，形成两个过密度区(约60和100千秒差距)，同时SMC在落入银河系(MW)时仅保留初始暗物质质量的三分之一，潮汐半径缩小至约4千秒差距，表明1:10卫星-宿主相互作用会在所有质量尺度上引发特征性暗物质晕形变。


<details>
  <summary>Details</summary>

Background: 研究主题为探讨大麦哲伦云（LMC）与小麦哲伦云（SMC）在1:10质量比相互作用下对彼此暗物质（DM）分布的长期扰动效应及其对暗物质模型检验的潜在影响。

Data: 数据来源于高分辨率N体模拟（N-body simulation）中孤立环境下大麦哲伦云（LMC）与小麦哲伦云（SMC）相互作用的模拟结果，并分析了它们在接近落入银河系（MW）时期的暗物质（DM）分布。

Method: 采用高分辨率N体模拟(N-body simulation)构建基函数展开(basis function expansion)，分析大麦哲伦云(LMC)和小麦哲伦云(SMC)孤立相互作用下的暗物质分布。

Result: 大麦哲伦云（LMC）与小麦哲伦云（SMC）的1:10质量比相互作用导致LMC暗物质晕产生约20千秒差距的动力学摩擦尾迹，并在每次SMC近心点移动时形成两个密度超区（约60和100千秒差距）。
...
**TL;DR**: 通过分析高红移(5 ≤ z ≤ 6)FIRE-2模拟数据并结合三维尘埃辐射转移和光电离模型，发现星系爆发性恒星形成驱动的外流与延展[CII]晕的形成密切相关，同时气体流入和湍流运动也有显著贡献。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高红移（$4 < z < 6$）星系的[CII]晕（[CII] halos）观测数据，探索星系生态系统演化及金属元素在星系周介质（circumgalactic medium）尺度上的富集机制。

Data: 使用了FIRE-2高红移模拟数据（stellar mass range of $10^{9}$--$10^{10.5}\,M_{\odot}$ at $5 \leq z \leq 6$），并通过三维尘埃辐射转移和光电离模型进行后处理。

Method: 采用高红移FIRE-2模拟结合三维尘埃辐射转移和光电离建模生成合成图像，并从中提取一维表面亮度分布。

Result: [CII]光晕（[CII] halos）的空间延伸程度与恒星形成率在近期星暴事件后的下降密切相关，表明星暴驱动的外流在产生[CII]光晕中起关键作用。
...
**TL;DR**: 研究发现红移（redshift）z=3.1的静止星系GS10578周围存在多黑洞（multi-black-hole）活动和数十千秒差距（tens-of-kpc）尺度的气体结构，揭示了反馈和卫星相互作用对早期大质量星系淬灭（quenching）晚期演化的影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于早期宇宙中大质量淬灭星系（massive quenched galaxies）的形成与演化机制，特别是活动星系核（AGN）反馈在星系快速形成和骤停过程中的关键作用。

Data: 研究中使用的数据来源包括JWST/NIRSpec IFU观测获取的LAE1和LAE2光学星云谱线新数据，以及VLT/MUSE光谱数据作为补充。

Method: 采用近红外光谱仪积分视场单元（NIRSpec IFU）观测结合MUSE数据，通过提取积分及空间分辨光谱、构建发射线图并运用标准诊断图分析电离源与运动学特征。

Result: LAE2的光学谱线比例完全符合内嵌活动星系核(AGN)特征，其亚千秒差距尺度上呈现团块状形态和不规则运动学特征。
...
**TL;DR**: 研究发现银河系盘（Galactic disk）引力势最可能是驱动18个本地年轻恒星群（LYSA）在银河系Z方向径向运动相关性的主要因素，这些相关性揭示了恒星形成过程中未被现有理论充分解释的内部运动学特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探讨本地年轻恒星群（LYSA）内部运动学特征及其与年龄的关联，以揭示银河系局部区域恒星形成过程的动力学机制。

Data: 数据来源于公开数据集，包含18个本地年轻恒星关联体（LYSA）的线性速度场参数和膨胀年龄。

Method: 采用频率学派和贝叶斯方法分析18个本地年轻恒星协（LYSA）的线性速度场参数和膨胀年龄数据，识别出与银河系Z方向径向运动相关的四个稳健统计相关性。

Result: 在银河系盘（Galactic disk）引力势驱动下，LYSA（local young stellar associations）的径向运动（radial motions）沿银河系Z方向呈现显著相关性，且这些运动在恒星形成后被短于LYSA年龄的时间尺度阻尼。
...
**TL;DR**: 基于DESI巡天数据系统分析28个可能源自外银河系的球状星团（globular clusters），首次发现Terzan 7存在潮汐结构，揭示星团内部动力学特性（如质量、逃逸速度、轨道参数）与外部银河系环境共同塑造了多样化的潮汐形态（tidal morphologies），且吸积历史与外部结构无直接关联。


<details>
  <summary>Details</summary>

Background: 球状星团周围的潮汐结构为研究星团演化及银河系等级成团过程提供了关键线索。

Data: 使用DESI Legacy Survey（DESI遗产巡天）的宽视场成像数据结合颜色-星等匹配滤波技术(color-magnitude matched-filter technique)进行系统性分析。

Method: 采用DESI Legacy Survey宽视场成像数据结合颜色-星等匹配滤波技术(color-magnitude matched-filter technique)，系统分析了28个可能源自银河系外的球状星团(globular clusters)的潮汐结构。

Result: 球状星团潮汐结构形态多样性由内部动力学演化与银河系外部环境共同作用驱动。
...
**TL;DR**: 利用217 ks XRISM高分辨率光谱数据精确测定蛇夫座(Ophiuchus)冷核心星系团中Si、S、Ar、Ca、Cr、Mn、Fe和Ni的丰度，发现其中心区域X/Fe丰度模式与太阳值高度一致，并通过核坍缩超新星（含大质量前身星）与Ia型超新星的线性组合模型成功重现观测丰度模式，同时指出富钙间隙暂现源可能弥补核合成模型对Ca/Fe的低估。


<details>
  <summary>Details</summary>

Background: 星系团（galaxy clusters）为研究宇宙化学增丰历史提供了理想实验室，其热 intracluster medium（ICM）包含多种化学元素。

Data: 使用217 ks的XRISM高分辨率光谱数据集准确测定了蛇夫座(Ophiuchus)冷核星系团中Si、S、Ar、Ca、Cr、Mn、Fe和Ni的丰度。

Method: 采用高分辨率光谱学（high-resolution spectroscopy）技术分析XRISM数据集，精确测定Ophiuchus星系团中心区域Si、S、Ar、Ca、Cr、Mn、Fe和Ni的丰度，并通过线性组合模型（linear combination models）模拟核心坍缩超新星（core-collapse supernovae）与Ia型超新星（Type Ia supernovae）对化学增丰的贡献。

Result: Ophiuchus星系团中心区域的X/Fe丰度模式与太阳高度一致，可通过大质量前身星的核坍缩超新星和Ia型超新星的线性组合模型全局复现。
...
**TL;DR**: 通过高分辨率三维磁流体动力学模拟发现，在高度可压缩的超音速湍流（均方根马赫数$\mathcal{M}_{\rm rms} \approx 11$）中，磁普朗特数（Pm）从1增至10时，波动发电机（fluctuation dynamo）的磁能增长机制从压缩主导转变为涡旋拉伸主导，导致更快增长、更高饱和水平及更弱的密度-磁场相关性，且速度与磁场的积分尺度比保持约3.4不变，而法拉第旋转测量（Faraday rotation measures）相干长度约为驱动尺度的1/4至1/3。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于理解高度可压缩星际介质(ISM)中波动发电机(fluctuation dynamo)放大弱种子磁场的机制及其行为特征。

Data: 数据来源于高分辨率三维磁流体动力学(MHD)模拟，模拟了均方根马赫数约11的超音速湍流，覆盖磁普朗特数(Pm)1至10的范围。

Method: 采用高分辨率三维磁流体动力学(MHD)模拟研究超音速湍流（均方根马赫数$\mathcal{M}_{\rm rms} \approx 11$）中磁普朗特数(Pm)在1-10范围内的涨落发电机效应。

Result: 在高度可压缩的超音速湍流中，磁普朗特数（Pm）的变化显著影响涨落发电机（fluctuation dynamo）的放大机制和饱和特性：Pm=1时压缩主导磁能增长且饱和能级较低，而Pm=10时涡旋拉伸成为主导机制，导致更快增长、更高饱和能级及磁压对密度-磁场相关性的更强抑制。
...
**TL;DR**: 通过模拟研究银河系类似星系中星际介质（ISM）的湍流驱动模式（参数b），发现湍流驱动参数b在0.4至1之间波动，且高恒星形成率出现在高度压缩驱动（b > 0.5）约10 Myr（一个湍流周转时间）后，而峰值恒星形成约20 Myr后，II型超新星（type-B SN）反馈使b降至0.4-0.5。


<details>
  <summary>Details</summary>

Background: 研究探讨了星际介质(ISM)中湍流驱动模式（包括超新星爆发、剪切力和引力坍缩等机制）如何影响湍流参数b（表征湍流驱动场中螺线模与压缩模的比值）及其与恒星形成的动态关联。

Data: 数据来源于TIGRESS剪切盒模拟（shearing-box simulation）中对一个类似银河系的kpc尺度区域的磁化暖中性介质（WNM）进行的100 Myr（约半个轨道周期）时间演化模拟。

Method: 采用TIGRESS剪切盒模拟（shearing-box simulations）量化湍流驱动参数b（turbulence driving parameter）的时间演化，并结合湍流马赫数（turbulent Mach number）和等离子体β（plasma beta）分析其与恒星形成的相关性。

Result: 湍流驱动参数（b）在0.4至1之间随时间波动，且与恒星形成活动存在时间相关性，高恒星形成率出现在高度压缩驱动（b > 0.5）后约一个湍流翻转时间（~10 Myr），而峰值恒星形成后约20 Myr，B型超新星反馈导致暖中性介质（WNM）比例和湍流马赫数上升，同时降低等离子体β和驱动参数至b ~ 0.4-0.5。
...
**TL;DR**: 开发了一种分析工具，用于计算长波长（λ≳8μm）下任意轴比、具有不同包层（confocal、co-axial或constant depth）的椭球核/包层（core/mantle）粒子的光学特性（消光、吸收、散射和偏振效率），该方法比数值计算更简便快速且与DDSCAT结果吻合，但指出有效介质理论（EMT）不适用于此类粒子光学特性近似。


<details>
  <summary>Details</summary>

Background: 研究聚焦于非球形和非均匀粒子光学特性（消光、吸收、散射和偏振效率）计算方法的优化，旨在解决传统数值模拟耗时问题，并发展适用于长波长（lambda >~ 8微米）条件下任意轴比覆层椭球粒子的解析计算工具。

Data: 研究中使用的数据来源为DDSCAT数值计算和布鲁格曼有效介质理论（Bruggemann effective medium theory, EMT）的有限情况计算结果。

Method: 采用解析方法计算任意轴比、长波长极限（a << λ）下具有包层（confocal、co-axial或恒定深度）的椭球核/包层粒子的光学特性（消光、吸收、散射和偏振效率）。

Result: 开发了一种分析工具，可在长波长极限（a << λ）下计算任意轴比、具有不同包层形态（共焦、同轴或恒定深度）的椭球核/包层粒子的光学特性（消光、吸收、散射和偏振效率），该方法比数值计算更简便快速且与DDSCAT结果吻合良好，但指出有效介质理论（EMT）不适用于此类粒子光学特性近似。
...
**TL;DR**: 研究发现恒星外流与周围气体相互作用产生的负动力摩擦（NDF）会加速星团膨胀并促使恒星向外迁移，同时减少中子星-主序星双星数量，保留更大轨道半长轴和更低偏心率的双星系统。


<details>
  <summary>Details</summary>

Background: 该研究探讨恒星-气体相互作用（star-gas interactions）通过动力学摩擦（DF）及其非耗散对应机制负动力学摩擦（NDF）对星团动力学演化的引力反馈影响。

Data: data部分：使用\texttt{PeTar}代码对初始包含$10^4$颗恒星的疏散星团在多种环境气体密度下进行直接$N$-体模拟。

Method: 采用直接N体模拟（direct N-body simulation）结合\texttt{PeTar}代码，研究开放星团在气体介质中的动力学演化，重点分析恒星外流与周围气体相互作用产生的负动力学摩擦（NDF）效应。

Result: 负动力摩擦（NDF）通过恒星外流与周围气体的相互作用加速星团膨胀，并优先将恒星输送到星团外围，同时导致中子星与主序星组成的双星数量更快减少。
...
**TL;DR**: 基于JWST的JADES巡天数据，首次揭示了宇宙黎明时期（红移5≤z<11）星系与暗物质共演化关系，发现红移10.6的星系寄主暗物质晕质量（~10^10.12 M⊙）比红移5.5（~10^11.45 M⊙）低一个量级且有效偏置（b_eff）更高（8.13对5.64），卫星星系比例随红移降低呈下降趋势。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙黎明时期（5 ≤ z < 11）星系与暗物质（dark matter）的共演化关系，利用JWST的深度观测数据揭示早期星系形成与暗物质晕（halo）质量增长的关联。

Data: 数据来源于JWST（James Webb Space Telescope）先进深空星系巡天（JADES）项目中通过光度选择（photometrically-selected）的约6500个莱曼断裂星系（Lyman Break Galaxies）样本。

Method: 采用晕占据分布模型（HOD）对两点角相关函数进行建模，追踪星系-暗物质晕关系。

Result: 在红移（redshift）z=10.6时，星系（galaxy）位于暗物质晕（dark matter halo）中的质量（Mh~10^10.12 M⊙）比z=5.5时（Mh~10^11.45 M⊙）低一个数量级，同时表现出更高的有效偏差值（bg^eff=8.13^+0.04_-0.02对比5.64^+0.10_-0.13）。
...
### [12] [Dynamical properties and star formation history of a low-mass quenched galaxy at Cosmic Noon](https://arxiv.org/abs/2601.01722)
...
**TL;DR**: 发现了红移(z=2.0834)低质量静止星系AURORA-LQG1（质量log(M⋆/M⊙)=9.6），其具有迄今红移z∼2静止星系中最小的恒星速度弥散(σ⋆=95−33+38 km/s)和紧凑尺寸(0.41±0.03 kpc)，证实其为真实静止星系且停熄时间仅∼0.2 Gyr，为研究低质量星系停熄机制提供了新见解。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙正午（Cosmic Noon）时期低质量静止星系（quiescent galaxy）的淬灭（quenching）机制及其演化特征。

Data: 数据来源于JWST/NIRSpec（詹姆斯·韦伯空间望远镜近红外光谱仪）的中等分辨率光谱观测。

Method: 采用中分辨率JWST/NIRSpec光谱技术测量恒星速度弥散度（σ⋆）和动力学质量（Mdyn），并结合恒星形成历史分析确定淬灭时间。

Result: AURORA-LQG1是一个低质量静止星系（quiescent galaxy），其恒星速度弥散（σ⋆ = 95_{-33}^{+38} km s^{-1}）是迄今在z∼2光谱确认的静止星系中测得的最小值。
...
### [13] [Exploring the interplay between molecular and ionized gas in HII regions](https://arxiv.org/abs/2601.01748)
...
**TL;DR**: 通过多组分对位甲醛（para-H2CO）跃迁观测结合非局域热动平衡分析，首次量化了61个HII区大质量恒星形成团块的动力学温度（33.7–265 K）和氢分子密度（0.8×10^4–1.05×10^7 cm^-3），并发现57%源存在与超音速非热运动相关的二次成分，揭示了中心大质量恒星通过辐射反馈主导分子气体与电离气体相互作用的机制。


<details>
  <summary>Details</summary>

Background: 研究主题为利用对位甲醛（para-H2CO）多谱线观测约束大质量恒星形成团块（clump）的动力学温度与氢分子（H2）体积密度，并探究电离气体、分子气体与尘埃的相互作用机制以揭示大质量恒星对其母团块的影响。

Data: 使用阿塔卡马探路者实验（APEX）12米亚毫米波望远镜的nFLASH230和SEPIA345接收器对61个HII区样本进行了para-H2CO（J = 3-2和4-3态）的JK_aK_c跃迁观测。

Method: 采用非局部热动平衡（non-LTE）辐射传输代码PyRADEX结合马尔可夫链蒙特卡洛（MCMC）分析方法，通过多组分高斯拟合para-H2CO谱线参数约束动力学温度（Tkin）和H2体积密度（n(H2)）。

Result: 利用多组分对位甲醛（para-H2CO）跃迁观测结合非局域热力学平衡（non-LTE）分析，测得大质量恒星形成团块的动力学温度（Tkin）范围为33.7 K至265 K，H2体积密度（n(H2)）介于0.8×10^4至1.05×10^7 cm^-3。
...
### [14] [When the Shadow Meets Its Measure: Assessing the Feasibility of Submillimeter Black Hole Shadow Imaging in Megamaser Disk AGN](https://arxiv.org/abs/2601.01763)
*Roman Burridge,Geoffrey C. Bower*

**TL;DR**: 该研究通过分析21个水脉泽（water megamaser）盘活动星系核（AGN），确定了其黑洞阴影（BHS）成像的亚毫米-毫米（submm-mm）干涉基线需求、核心流量密度及探测自旋相关位置偏移所需的天体测量精度，发现NGC~4258是唯一可在地球-L2基线上探测的目标，并揭示其亚毫米波过剩现象，表明吸积盘在小于100倍史瓦西半径（Schwarzschild radii）的过渡半径内保持薄盘状态。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用寄主活动星系核(AGN)的水脉泽(megamaser)盘实现黑洞(BH)质量、距离、倾角及动力学中心的极高精度几何测量，为超越银河系中心(Sgr A*)和M87*的黑洞阴影(BHS)成像提供独特强引力场探针。

Data: 数据来源于二十一个精确表征的水脉泽盘（water megamaser disk）活动星系核（AGN）的亚毫米-毫米（submm-mm）波段干涉测量基线需求、核心流量密度估计（考虑热尘埃污染、延展喷流辐射和本征变率）以及黑洞阴影（BHS）与脉泽盘动力学中心位置偏移的探测所需天体测量精度。

Method: 采用亚毫米-毫米(submm-mm)波段干涉测量技术结合黑洞阴影(BHS)成像方法，并整合热尘埃污染、喷流延伸辐射和本征变率校正模型来估算活动星系核(AGN)核心流量密度。

Result: 水脉泽盘（water megamaser disk）活动星系核（AGN）为黑洞（BH）质量、距离、倾角和动力学中心提供了极高精度的几何测量手段。
...
**TL;DR**: 首次利用UKIRT的UWIFE巡天系统研究银河系第一象限中[Fe II] 1.644微米发射线的激波气体，发现204个扩展电离铁天体（IFOs），其中多数为新发现，并确认其与超新星遗迹（25）、年轻恒星天体（100）、H II区（33）、行星状星云（17）和明亮蓝变星（4）存在关联。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用UKIRT宽视场红外巡天（UWIFE）系统探索银河系第一象限中的激波气体（shocked gas）及其[Fe II] 1.644微米发射线特征。

Data: 数据来源于英国红外望远镜(UKIRT)进行的UWIFE巡天观测，该巡天首次在[Fe II] 1.644微米发射线对银河系第一象限(7°<l<62°, |b|<1.5°)进行了成像观测。

Method: 采用UKIRT望远镜的UWIFE巡天数据，结合手动和自动搜索方法识别扩展电离铁天体(IFOs)，并通过位置交叉匹配与已知天体关联。

Result: 首次在[Fe II] 1.644微米发射线中系统探测到银河系第一象限204个扩展电离铁天体(IFOs)，其中多数为新发现，并与超新星遗迹(25)、年轻恒星天体(100)、H II区(33)、行星状星云(17)和明亮蓝变星(4)存在位置关联。
...
**TL;DR**: 通过球状星团(GC)吸积模型模拟发现，轨道偏心率为0.4-0.5且垂直于银道面的致密GC(r_hm=1pc)在早期(前10亿年)对原核星团(proto-NSC)质量贡献最大(平均下限6%)，但单独机制不足以解释当前核星团(NSC)总质量。


<details>
  <summary>Details</summary>

Background: 研究探讨了球状星团（globular cluster, GC）恒星在早期宇宙时间尺度上的吸积过程及其对银河系（Milky Way, MW）核星团（nuclear star cluster, NSC）质量形成的贡献。

Data: 数据来源于理论球状星团(GC)模型的详细N体(N-body)模拟，包括三组不同半质量半径(r_hm)的模型（每组50个完整N体模型），并在从宇宙学数据库IllustrisTNG-100提取的时间可变银河系(MW)类似势场中进行积分，模拟时间跨度为-10 Gyr至-5 Gyr。

Method: 采用更新的并行N体代码phi-GPU（含恒星演化模块）进行理论球状星团（GC）模型的详细N体模拟，并在取自宇宙学数据库IllustrisTNG-100的时变银河系（MW）势场中积分。

Result: 球状星团(GC)模型在轨道偏心率为0.4-0.5且轨道垂直于银盘时对原核星团(proto-NSC)质量贡献最大，其中半质量半径(r_hm)为1 pc的致密模型在最初十亿年内吸积效率最高。
...
**TL;DR**: 通过高分辨率毫米波观测发现DR21南纤维状云(DR21 South Filament)内部存在速度相干纤维结构(velocity-coherent fibers)和引力束缚致密核(dense cores)，揭示了其分层碎裂特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高分辨率毫米波观测揭示DR21南纤维状结构（DR21 South Filament）的内部碎裂特征及其与恒星形成活动的关联。

Data: 数据来源于北方扩展毫米波阵列（NOrthern Extended Millimeter Array）的高分辨率（约1000 AU）3毫米波段观测，包括连续谱和分子谱线（CS (2-1)、HCO+ (1-0)、H13CO+ (1-0)及NH2D (111-101)）的发射数据，并结合低分辨率（约18角秒）的柱密度图进行比对。

Method: 采用高分辨率（约1000 AU）3毫米连续谱和分子谱线（CS (2-1)、HCO+ (1-0)、H13CO+ (1-0)、NH2D (111-101)）观测，结合朋友的朋友算法（friend-of-friend algorithm）识别速度相干纤维（velocity-coherent fibers），并通过维里分析（virial analysis）评估致密核（dense cores）的引力束缚状态。

Result: 在DR21南纤维状云（DR21 South Filament）中检测到13个NH2D（1_{11}-1_{01}）致密核，其中3个与连续谱峰值重合，且大多数核受引力束缚；通过朋友的朋友算法（friend-of-friend algorithm）从CS、HCO+和H13CO+数据中分别识别出32、34和22条速度相干纤维（velocity-coherent fibers），其中H13CO+纤维更频繁地与NH2D冷核相关联且具有更高的单位长度质量平均值。
...
**TL;DR**: 通过ODIN巡天在红移2.4和3.1处对103和112个莱曼α斑块(LABs)的聚类分析，首次精确测量了其宿主暗物质晕(Dark Matter Halo)质量（最小质量分别为2.8×10¹²M⊙和0.7×10¹²M⊙），证实LABs是标记原星系群(proto-group)并演化至现今大质量星系团（~10¹³M⊙）的示踪体。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（$z > 2$）莱曼α斑块（Ly$α$ blobs, LABs）的暗物质晕（dark matter halo）性质及其在宇宙结构形成中的作用，旨在揭示这些罕见天体如何作为原星系群（proto-group）或现代大质量星系前身的示踪体。

Data: 数据来源于ODIN巡天项目在扩展COSMOS视场（约9平方度）中发现的103个（红移z~2.4）和112个（红移z~3.1）莱曼α斑块（LABs），并通过角自相关函数（ACFs）分析其空间分布。

Method: 采用角自相关函数（ACF）分析并结合星系偏置（galaxy bias）测量来估算莱曼α斑块（LABs）的宿主暗物质晕质量。

Result: 莱曼α斑块（LABs）寄居于质量下限为2.8^{+3.0}_{-1.8}×10^{12}M⊙（z~2.4）和0.7^{+0.8}_{-0.5}×10^{12}M⊙（z~3.1）的暗物质晕中，中位质量分别为4.2^{+3.8}_{-2.5}×10^{12}M⊙和1.1^{+1.1}_{-0.7}×10^{12}M⊙，暗示其可能标志着将演化成现今大质量星系或星系团（~10^{13}M⊙）的原星系群环境。
...
**TL;DR**: 研究发现超大质量黑洞双星（MBHBs）的初始轨道偏心率（e0）与形成后的双星偏心率（eb）存在普适关系，并给出了N体模拟分辨率要求及eb(e0)拟合公式，用于半解析模型计算双星并合时标和引力波背景（GWB）特征。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于超大质量黑洞双星（MBHBs）作为脉冲星计时阵列（PTAs）可探测引力波背景（GWB）的主要来源，其形成时的轨道偏心率（eccentricity）是决定从配对到并合演化时标的关键参数。

Data: 数据来源于对等质量星系合并的数值模拟，通过不同初始轨道条件下的N体模拟跟踪超大质量黑洞双星（MBHBs）从形成到硬化阶段的动力学演化。

Method: 采用高分辨率N体数值模拟（N-body simulation）研究超大质量黑洞双星（MBHBs）的形成与演化，重点分析初始轨道偏心率（e0）对双星偏心率（eb）的影响，并通过拟合公式量化模拟分辨率与初始偏心率的关系。

Result: 超大质量黑洞双星（MBHBs）的初始轨道偏心率（e0）与形成后的双星偏心率（eb）之间存在普遍关系，当初始偏心率超过临界值（约0.97）时双星偏心率会显著降低。
...
**TL;DR**: PASSAGE巡天项目利用JWST的NIRISS仪器在COSMOS天区获得了2183个0.08<z<4.7的发射线(emission line)源光谱红移(spectroscopic redshift)目录，与COSMOS-Web测光红移(photometric redshift)在强信号(S/N>5)多线发射源中高度一致，但单线发射源存在约18%误认率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用詹姆斯·韦伯太空望远镜（JWST）的无缝光谱技术（WFSS）在COSMOS天区开展星系演化（galaxy evolution）研究，填补近红外波段光谱红移（spectroscopic redshift）数据的空白。

Data: 数据来源于JWST（James Webb Space Telescope）Cycle 1的NIRISS（Near Infrared Imager and Slitless Spectrograph）宽场无缝光谱（WFSS）纯并行巡天观测，以及HST（Hubble Space Telescope）COSMOS天区和JWST COSMOS-Web巡天提供的额外空间测光数据。

Method: 采用定制线检测算法（custom line-finding algorithm）结合人工目视检查（visual inspection）识别发射线并推导光谱红移。

Result: PASSAGE巡天在0.08<z<4.7范围内识别了2183个发射线源（emission line sources），其中强信噪比（S/N>5）多线发射源的光谱红移（spectroscopic redshifts）与COSMOS测光红移（photometric redshifts）高度一致，而单线发射源（single-line emitters）存在约18%的误认率。
...
**TL;DR**: 通过JWST NIRSpec光谱观测发现，红移1.8-4.6的13个后星暴(PSB)和静止星系中3个存在高速中性气体外流(outflows)、2个存在气体流入(inflows)，外流速度达300-1200 km/s且仅出现在熄灭时间<600 Myr的星系中，其质量流失率远超当前恒星形成率，表明这些化石外流由先前活跃的AGN驱动，并与EAGLE模拟提出的周期性AGN活动-外流循环模型（约40 Myr周期含5 Myr活跃期和10 Myr可观测外流）相吻合。


<details>
  <summary>Details</summary>

Background: 研究聚焦于红移1.8至4.6的后星暴(PSB)和静止星系中中性气体外流(outflow)与内流(inflow)的起源及其对星系静止状态的维持机制。

Data: 数据来源于JWST（James Webb Space Telescope）NIRSpec（Near-Infrared Spectrograph）光谱观测，来自EXCELS（Extragalactic Excursions in the Local Universe Survey）巡天项目，并补充了近期文献中可比天体的数据。

Method: 采用JWST（詹姆斯·韦伯太空望远镜）NIRSpec（近红外光谱仪）光谱分析技术，结合NaD（钠原子D线）吸收谱线特征测量中性气体外流与内流速度，并通过EAGLE（宇宙学流体动力学模拟）数值模拟验证"外流循环"（outflow cycle）模型。

Result: 在13个后星暴星系（PSB）和静止星系中观测到中性气体外流（outflow）和内流（inflow），其中3个存在外流特征（速度300-1200 km/s），2个显示内流迹象，且气体流动仅出现在熄灭时间短于600 Myr的天体中。
...
**TL;DR**: 通过ALMA 100 GHz观测首次发现银河系中心射电弧(Radio Arc)非热丝状体(NTFs)中存在均匀旋转且沿不同丝状体系统变化的磁场结构，排除了显著法拉第效应干扰，并更新了对视线方向局部结构的理解。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中心非热辐射丝状结构（NTFs）的磁场特性及其形成机制。

Data: 数据来源于甚大天线阵（VLA）在10 GHz的射电偏振观测和阿塔卡马大型毫米波/亚毫米波阵列（ALMA）在100 GHz的偏振观测。

Method: 采用阿塔卡马大型毫米波/亚毫米波阵列（ALMA）在100 GHz频段进行偏振观测，避免了显著的法拉第旋转效应影响。

Result: 在射电弧(Radio Arc)不同丝状结构中观测到系统差异的磁场取向，表明其可能源于多个磁场系统的混淆或偏振在非热丝状体(NTFs)内部的中心集中。
...
**TL;DR**: 通过高分辨率宇宙学模拟(FOGGIE)发现，Mg II和O VI离子气体在银河系类似星系周围分别呈现中心聚集和弥散分布特征，且二者虽存在共动速度关系但多数空间上不相关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过模拟星系晕介质（CGM）中镁离子（Mg II）和氧离子（O VI）的空间与运动学关系，揭示星系重子循环的热力学特性。

Data: data部分：研究使用的数据来源于宇宙学放大模拟套件FOGGIE（Focusing On Galaxy Growth and Intergalactic Environments），该套件针对类似银河系的星系进行了高分辨率CGM（circumgalactic medium）模拟。

Method: 采用两种分析方法：通过二维投影模拟构建集成视线（mock sightlines），以及使用SALSA代码创建一维射线对象（mock absorbers）分析主导视线柱密度的气体结构。

Result: O VI气体以弥散晕形式分布于星系盘外围，而Mg II气体更集中于中心区域。
...
### [24] [Testing supermassive primordial black holes with lensing signals of binary black hole merges](https://arxiv.org/abs/2601.01034)
*Huan Zhou,Bin Liu,Zheng-Xiang Li,Xi-Jing Wang,Kai Liao*

**TL;DR**: 该研究通过分析强引力透镜效应下引力波（GW）事件的发生率和时间延迟分布，提出了一种约束超大质量原初黑洞（SMPBHs）丰度的新方法，发现在$Λ$CDM宇宙学模型中质量超过$10^8~M_{\odot}$的SMPBHs丰度$f_{\rm PBH}$在95%置信水平下被限制为$\sim10^{-4}$。


<details>
  <summary>Details</summary>

Background: 研究探讨了下一代地基引力波探测器观测透镜化双黑洞并合事件对限制早期宇宙中超大质量原初黑洞（SMPBHs）丰度的作用，该机制与暗物质晕形成及高红移星系观测相关。

Data: 数据来源包括下一代地基引力波（GW）探测器观测的透镜化双黑洞并合事件，以及詹姆斯·韦伯太空望远镜（James Webb Space Telescope）对高红移星系的观测数据。

Method: 采用统计分析方法测量强透镜引力波事件的发生率和时间延迟分布，以约束超大质量原初黑洞（SMPBHs）的丰度。

Result: 强透镜引力波（GW）事件的发生率和时间延迟分布可作为直接约束超大质量原初黑洞（SMPBHs）丰度的有效探针。
...
### [25] [Time-delay and lens galaxy redshift in the doubly imaged quasar PS J2305+3714](https://arxiv.org/abs/2601.01230)
*O. A. Burkhonov,V. N. Shalyapin,A. V. Sergeyev,Sh. E. Nurmamatov,Sh. A. Ehgamberdiev,T. A. Akhunov,F. Dux,F. Courbin,M. M. Muminov*

**TL;DR**: 通过7年光学R波段监测发现透镜类星体（gravitationally lensed quasar）PS J2305+3714的微引力透镜（microlensing）变幅小于10毫星等，测得52.2天时延并首次确定透镜红移（lens redshift）为0.473，其光谱域与时域微引力透镜信号微弱，构建的两种质量模型与观测数据高度吻合，验证了该系统作为时延宇宙学（time-delay cosmography）研究目标的可靠性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用引力透镜效应（gravitational lensing）和微引力透镜（microlensing）监测类星体系统PS J2305+3714，以验证宇宙学模型（ΛCDM cosmology）并探索时间延迟宇宙学（time-delay cosmography）的应用潜力。

Data: 数据来源于2018-2024年期间使用1.5米Maidanak望远镜在光学R波段对引力透镜类星体系统PS J2305+3714进行的七个季节的监测数据，以及长缝光谱仪（WHT spectroscopy）获取的光谱数据和哈勃太空望远镜（HST）成像提供的精确天体测量数据。

Method: 采用光学R波段监测和长缝光谱（long-slit spectroscopy）技术，结合哈勃太空望远镜（HST）精确天体测量数据，构建了两种简单透镜质量模型（mass model）。

Result: 在7年时间尺度上，PS J2305+3714引力透镜类星体系统的微引力透镜(microlensing)变幅不超过10毫星等(mmag)。
...
### [26] [Effective dark matter component presents a robust signature of negative pressure by the DESI observations](https://arxiv.org/abs/2601.01340)
*Hao Xu,Xinhe Meng*

**TL;DR**: 通过综合分析三种暗能量模型（解冻标量场、修正涌现暗能量模型和常数w模型）与新型非标准暗物质（NSDM）模型的耦合效应，发现所有数据集组合均一致偏好负值的状态方程参数w₂，表明暗物质流体具有微弱但非零的负压（非冷特性），为暗物质领域的新物理提供了有力证据。


<details>
  <summary>Details</summary>

Background: 研究探讨了非标准暗物质（NSDM）模型在三种不同单参数动态暗能量模型（解冻标量场、修正涌现暗能量模型MEDE和常数w模型）下的宇宙学效应，重点关注暗物质可能具有微弱负压的非冷特性。

Data: 数据来源包括普朗克2018年宇宙微波背景(CMB)距离先验、暗能量光谱仪(DESI)第二版数据发布的重子声波振荡(BAO)测量结果，以及三类Ia型超新星(SN Ia)数据集（暗能量巡天第五年(DESY5)汇编、Union3汇编和PantheonPlus(PP)样本）。

Method: 采用有效非标准暗物质(NSDM)模型的状态方程$w_{\mathrm{dm}} = w_2 a^2$结合三种单参数动态暗能量模型（解冻标量场、修正涌现暗能量(MEDE)模型和常$w$模型），并利用普朗克2018宇宙微波背景(CMB)距离先验、暗能量光谱仪(DESI)第二版重子声振荡(BAO)数据及三类Ia型超新星(SN Ia)数据集进行约束分析。

Result: 在所有暗能量参数化方案和数据集组合中，均一致检测到非标准暗物质(NSDM)状态方程参数w₂呈现负值，表明暗物质流体具有微弱但非零的负压（非冷特性），这一结果独立于暗能量模型选择，揭示了暗物质可能存在超越标准冷暗物质的新物理机制。
...
### [27] [Cracks in the Standard Cosmological Model: Anomalies, Tensions, and Hints of New Physics](https://arxiv.org/abs/2601.01525)
*Eleonora Di Valentino*

**TL;DR**: 这篇论文的核心贡献是批判性评估了标准宇宙学模型（ΛCDM）在多数据集联合分析中暴露的多个参数（如哈勃常数、宇宙微波背景透镜效应、曲率、中微子质量和暗能量性质）之间的系统性矛盾，强调了模型假设、参数简并性和数据集一致性的影响，并探讨了早期和晚期解决方案的可行性。


<details>
  <summary>Details</summary>

Background: 标准宇宙学模型（ΛCDM）在精度提升背景下显现出哈勃常数（Hubble constant）、宇宙微波背景辐射透镜效应（CMB lensing）、曲率、中微子质量及暗能量本质等多重观测矛盾。

Data: 数据来源包括联合分析中的哈勃常数（Hubble constant）、宇宙微波背景透镜效应（CMB lensing）、曲率（curvature）、中微子质量（neutrino masses）和暗能量（dark energy）相关数据集，以及近期DESI重子声波振荡（BAO）观测结果。

Method: 采用联合分析（joint analyses）方法评估标准ΛCDM模型与多数据集间的矛盾，并探讨早期和晚期解决方案及暗物质-暗能量相互作用（interacting dark-sector scenarios）情景。

Result: 标准ΛCDM范式在联合数据集分析中显示出涉及哈勃常数(Hubble constant)、宇宙微波背景透镜效应(CMB lensing)、曲率(curvature)、中微子质量(neutrino masses)和暗能量本质的多重矛盾。
...
### [28] [The Optical Origin of the Mass-Sheet Transformation](https://arxiv.org/abs/2601.01614)
*Marc V. Gorenstein*

**TL;DR**: 该研究揭示了引力透镜（gravitational lensing）中质量薄层变换（Mass-Sheet Transformation, MST）源于图像选择关系（Image-Selection Relation, ISR）的尺度对称性，该对称性保持像位不变但缩放放大率和时间延迟。


<details>
  <summary>Details</summary>

Background: 该研究探讨了引力透镜（gravitational lensing）中质量薄层变换（Mass-Sheet Transformation, MST）的物理本质，揭示了其源于射线追踪关系在固有距离坐标下的标度对称性。

Data: 数据来源为理论推导（theoretical derivation），未使用具体观测或模拟数据。

Method: 采用适当距离坐标（proper-distance coordinates）下的光线追踪关系（ray-trace relation）揭示缩放对称性（scaling symmetry），并通过分离几何聚焦项（geometric focusing term）定义图像选择关系（ISR）来分析质量薄层变换（MST）。

Result: 质量薄层变换（MST）源于将光线追踪关系改写为固有距离坐标时显现的尺度对称性，该对称性保持像位置不变但缩放放大率和时间延迟。
...
### [29] [The Dilaton: A Natural Resolution to the Hubble Tension via Spontaneous Scale Symmetry Breaking](https://arxiv.org/abs/2601.01938)
*Arpit Kottur,Jui Mahajan,Raka Dabhade*

**TL;DR**: 提出标度不变性自发破缺引入的膨胀子(Dilaton)通过共形引力耦合产生指数势($λ≈0.056$)，其非最小耦合强度($ξ≈7.8×10^{-4$)和状态方程($w_0≈-0.85$)可缓解哈勃常数($H_0$)早期与晚期测量差异。


<details>
  <summary>Details</summary>

Background: 研究探讨了哈勃常数（Hubble constant）早期与晚期测量值之间的统计差异，提出暗能量（dark energy）的动态特性可能源于标度不变性（Scale Invariance）的自发破缺机制。

Data: 数据来源包括普朗克卫星（Planck）、Pantheon+超新星数据集和SH0ES（哈勃太空望远镜关键项目）观测数据。

Method: method：通过引入与膨胀对称性破缺相关的赝南布-戈德斯通玻色子（Dilaton, χ），将基础理论中的简单二次质量项通过引力共形耦合转化为爱因斯坦框架下的"解冻"指数势$V(φ) \propto e^{-λφ}$，并利用贝叶斯方法重建暗能量动力学。

Result: 通过引入与标度对称性自发破缺相关的赝南布-戈德斯通玻色子（Dilaton, χ），并利用普朗克、Pantheon+和SH0ES数据的贝叶斯重建，约束出势斜率λ≈0.056，对应基本非最小耦合强度ξ≈7.8×10⁻⁴，该机制自然产生缓解哈勃张力所需的晚期状态方程演化（w₀≈-0.85），同时通过近似平移对称性保护场质量m∼H₀。
...
### [30] [Euclid: Improving redshift distribution reconstruction using a deep-to-wide transfer function](https://arxiv.org/abs/2601.02005)
...
**TL;DR**: 开发了一种通过降级深场测光数据匹配浅场巡天多波段测光特性的方法，有效降低了红移分布重构中的系统误差，使大部分层析区间满足欧几里得(Euclid)任务精度要求。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过改进光度测量匹配方法提升红移分布估计精度，以支持欧几里得(Euclid)任务探索宇宙膨胀历史和暗能量(dark energy)本质的科学目标。

Data: 研究中使用的数据来源包括欧几里得旗舰模拟（Euclid Flagship Simulation）的光度数据以及具有深场测光（deep photometry）的参考样本的光谱红移（spectroscopic redshifts）数据。

Method: 开发了一种在测光空间（photometric space）中通过降级深场测光数据以匹配浅场巡天特性的方法，保留了通量及其误差间的所有相关性。

Result: 开发的光度转移方法能有效降低红移分布重构中的系统偏差，使大部分层析箱（tomographic bins）满足欧几里得任务精度要求。
...
### [31] [Nonlinear Scales in Luminal Horndeski -- I. Halo mass function and power spectrum boost in models with Vainshtein screening](https://arxiv.org/abs/2601.02074)
*Dani de Boe,Mattia Pantiri,Gen Ye,Alessandra Silvestri*

**TL;DR**: 该论文在霍恩德斯基引力（Horndeski gravity）框架下，基于球对称塌缩模型和瓦因施泰因屏蔽（Vainshtein screening）机制，建立了计算非线性物质功率谱的通用方法，并推导了临界和维里（virial）密度阈值，为未来大规模结构巡天研究提供了理论工具。


<details>
  <summary>Details</summary>

Background: 研究聚焦于具有光速引力波传播速度（$c_T = 1$）的霍恩德斯基引力（Horndeski gravity）理论中非线性结构形成的机制及其对宇宙大尺度结构的影响。

Data: 研究中使用的数据来源于基于球状坍缩模型（spherical collapse model）的数值模拟（simulation），结合了Vainshtein屏蔽机制和EFTCAMB接口，适用于协变Horndeski模型和暗能量有效场论描述。

Method: 采用球对称塌缩模型（spherical collapse model）结合Vainshtein屏蔽机制（Vainshtein screening）分析Horndeski引力理论中非线性结构形成，并基于反应方法（reaction method）开发了计算非线性物质功率谱的通用框架。

Result: 在霍恩德斯基引力（Horndeski gravity）理论框架下，结合Vainshtein屏蔽机制和球对称坍缩模型，建立了计算非线性物质功率谱的灵活精确方法，适用于光速引力波（$c_T = 1$）条件下的广泛修正引力模型和暗能量有效场论描述。
...
### [32] [Joint Constraints on Neutrinos and Dynamical Dark Energy in Minimally Modified Gravity](https://arxiv.org/abs/2601.02077)
*Artur Ladeira,Rafael C. Nunes,Supriya Pan,Weiqiang Yang*

**TL;DR**: \(w_{\dagger}\)VCDM框架作为最小修正引力理论（minimally modified gravity theories）的扩展，在保持理论稳定性的同时，通过引入暗能量晚期相变（late-time dark-energy transition）和额外相对论自由度（relativistic degrees of freedom），显著缓解了哈勃常数（\(H_0\) tension）矛盾，并给出了中微子质量总和（\(\sum m_ν< 0.11~\mathrm{eV}\)）与有效自由度（\(N_{\rm eff} = 2.98^{+0.13}_{-0.14}\)）的严格限制。


<details>
  <summary>Details</summary>

Background: 研究探讨了在最小修正引力理论框架下扩展ΛCDM模型的\(w_{\dagger}\)VCDM方案，旨在解决标准宇宙学模型中的持续张力问题。

Data: 数据来源包括普朗克卫星（Planck）的宇宙微波背景（CMB）数据、DESI DR2的重子声波振荡（BAO）数据以及DESY5的超新星数据。

Method: 采用\(w_{\dagger}\)VCDM框架作为最小修正引力理论（minimally modified gravity theories）的扩展，结合普朗克（Planck）宇宙微波背景（CMB）数据、DESI DR2重子声波振荡（BAO）数据和DESY5超新星数据，通过约束中微子物理参数（包括\(\sum m_ν\)和\(N_{\rm eff}\)）并分析暗能量晚期相变（late-time dark-energy transition）的统计显著性，验证了该模型的稳定性与一致性。

Result: result：数据显著倾向于暗能量在晚期的状态转变，表现为稳定的精质-幻影穿越（quintessence-phantom crossing），且这一现象在所有数据集组合和中微子部分扩展中保持稳定。
...
### [33] [Bondi-Hoyle-Lyttleton accretion onto ultra dense dark matter halos and direct collapse black holes](https://arxiv.org/abs/2601.02104)
*Kandaswamy Subramanian,Bikram Phookun*

**TL;DR**: 该研究提出通过复合期后邦迪-霍伊尔-利特尔顿吸积（Bondi-Hoyle-Lyttleton accretion）进入超密暗物质晕（UDMH）的形成机制，解释了中等质量黑洞（$\sim 10^3 M_\odot$）在高红移（$z>200-400$）时期的形成过程，其丰度与星系相当，可为早期超大质量黑洞提供种子来源。


<details>
  <summary>Details</summary>

Background: 探索中等质量黑洞（intermediate mass black holes, IMBHs）的早期形成机制及其作为高红移超大质量黑洞（supermassive black holes, SMBHs）种子的潜在作用。

Data: 数据来源于对再复合期后邦迪-霍伊尔-利特尔顿吸积(Bondi-Hoyle-Lyttleton accretion)进入超致密暗物质晕(UDMH)的模拟分析，结合宇宙微波背景(CMB)光谱畸变限制下小尺度罕见曲率涨落的理论推导。

Method: 采用邦迪-霍伊尔-利特尔顿吸积（Bondi-Hoyle-Lyttleton accretion）模型研究超密暗物质晕（UDMH）中气体吸积过程，结合原子冷却效率、分子冷却抑制及旋转支撑机制分析中间质量黑洞形成条件。

Result: 提出了一种中等质量黑洞（intermediate mass black holes, $\sim 10^3 M_\odot$）的形成机制，即通过再复合后邦迪-霍伊尔-利特尔顿吸积（Bondi-Hoyle-Lyttleton accretion）进入超密暗物质晕（ultra dense dark matter halos, UDMH, $\sim 10^5 M_\odot$），并在某些情况下因旋转支撑的气体盘发生自引力不稳定性导致快速向内质量转移，最终形成$\sim 10^3 M_\odot$的超大质量恒星或黑洞。
...
### [34] [Quintom Dark Energy: Future Attractor and Phantom Crossing in Light of DESI DR2 Observation](https://arxiv.org/abs/2601.02284)
*Phusuda Thanankullaphong,Prasanta Sahoo,Prajwal Hassan Puttasiddappa,Nandan Roy*

**TL;DR**: 研究在平坦FLRW宇宙中由正标量场（quintessence）和幻标量场（phantom）构成的双场暗能量（dark energy）模型，发现其存在幻主导的稳定加速膨胀吸引子，观测约束支持当前暗能量状态方程（equation of state）渐进穿越幻界限（phantom divide）的动态行为。


<details>
  <summary>Details</summary>

Background: 研究探讨了在平坦FLRW宇宙中由标准精质场（quintessence）和幽灵场（phantom）组成的双场暗能量（dark energy）模型的晚期宇宙动力学行为。

Data: 数据来源包括Pantheon+超新星数据集、压缩宇宙微波背景距离先验（compressed CMB distance priors）、DESI DR2重子声学振荡（BAO）测量数据和DES Year-5超新星数据。

Method: 采用五维自治动力系统分析固定点及其稳定性，结合指数势（exponential potential）和逆幂律势（inverse power-law potential）的双场（quintessence和phantom）暗能量模型动力学。

Result: 稳定晚期吸引子对应于由幻象场主导的加速膨胀状态。
...
### [35] [An information-theoretic bound on cosmic coherence in finite-volume simulations](https://arxiv.org/abs/2601.02351)
*Biswajit Pandey,Anindita Nandi*

**TL;DR**: 通过互信息量化宇宙密度场（cosmic density field）的物理记忆效应，发现其最大相干保留模式尺度约为模拟盒尺寸（simulation box size）的1/8，为有限体积内宇宙结构相干性研究确立了边界。


<details>
  <summary>Details</summary>

Background: 研究通过互信息方法量化宇宙密度场（cosmic density field）的物理记忆特性，揭示引力关联在有限体积内对结构相干性的尺度依赖性。

Data: 数据来源于N体模拟（N-body simulation）在不同红移下的快照（snapshot）。

Method: 采用互信息(mutual information)量化宇宙密度场在不同红移下的物理记忆，通过去除随机基线来分离引力相关性。

Result: 宇宙密度场的物理记忆（physical memory）通过去除随机基线后剩余的重力关联互信息（mutual information）量化，其共享互信息随尺度增大而上升，在约L/8处达到峰值后下降。
...
### [36] [Inflight performance and future improvements for the INtegral Field Ultraviolet Spectrographic Experiment, the first far ultraviolet integral field spectrograph](https://arxiv.org/abs/2601.00956)
...
**TL;DR**: 首次在远紫外波段实现静态配置积分视场光谱仪(INFUSE)，成功探测到天鹅座圈(Cygnus Loop)XA区域电离气体发射，为未来宜居世界天文台(HWO)的积分视场单元(IFU)技术提供验证路径。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发紫外波段积分视场光谱仪（IFU）技术，旨在为未来宜居世界天文台（HWO）实现扩展源（如超新星遗迹或星系）的高效三维光谱测绘提供技术验证。

Data: 数据来源于INFUSE（INtegral Field Ultraviolet Spectrographic Experiment）探空火箭项目首次发射（2023年10月29日）获取的远紫外积分视场光谱数据，以及计划2025年秋季第二次发射对NGC 2366的观测数据，使用包含26元图像切片器和复制全息光栅的静态配置光谱仪，搭载最大空间飞行交叉条纹微通道板探测器。

Method: 采用静态构型远紫外积分视场光谱仪(INFUSE)技术，结合26单元像切分器与复制全息光栅实现光谱复用，并搭载最大型空间飞行交叉条纹微通道板探测器。

Result: INFUSE首次成功演示了紫外波段静态构型积分视场光谱仪（IFU）的光谱复用能力，在Cygnus Loop的XA区域探测到了电离气体发射。
...
### [37] [Website with interactive visualization of multivariate astronomical time series](https://arxiv.org/abs/2601.00975)
*M. Volkov,M. Demianenko,D. Matveev,K. A. Grishin,I. V. Chilingarian*

**TL;DR**: 开发了Fulu和VALC两个基于网络的交互式光变曲线（light curve）可视化平台，分别用于研究瞬变现象（transient event）和低质量活动星系核（low-mass AGNs），支持大规模天文光变曲线的便捷分析与科学发现。


<details>
  <summary>Details</summary>

Background: 现代天体物理学研究中，时域天文学（time-domain astronomy）因大规模巡天数据的快速增长而成为关键领域。

Data: 研究中使用的数据来源为基于大规模巡天（large-scale sky surveys）获取的光变曲线（light curves）实测数据。

Method: 采用基于网页的交互式可视化平台（Fulu和VALC）技术手段，支持用户友好地检查、比较和解释大量光变曲线（light curves）数据。

Result: 开发了两个基于网络的交互式光变曲线（light curve）可视化平台（Fulu和VALC），分别用于研究瞬变事件（transient event）和低质量活动星系核（AGN）。
...
### [38] [thornado+Flash-X: A Hybrid DG-IMEX and Finite-Volume Framework for Neutrino-Radiation Hydrodynamics in Core-Collapse Supernovae](https://arxiv.org/abs/2601.00976)
...
**TL;DR**: 论文核心贡献是开发了thornado中的中微子输运算法（neutrino-transport algorithms），通过嵌套定点迭代（nested fixed-point iteration）实现非线性中微子-物质耦合（neutrino-matter coupling），并与Flash-X中的自适应网格（AMR）流体动力学耦合，验证了该方法在核心坍缩超新星（CCSNe）模拟中的准确性和多维建模可行性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发高精度中微子输运算法及其与自适应网格（AMR）多物理场模拟框架的耦合，以提升核心坍缩型超新星（CCSNe）模拟的物理保真度和计算性能。

Data: 数据来源包括使用理想化不透明度(opacity)的基本输运测试、包含列表化微观物理(tabulated microphysics)的弛豫和去轻子化(deleptonization)问题、球对称核心坍缩超新星(CCSN)模拟以及与轴对称CCSN模拟的验证数据。

Method: 采用谱方法（spectral method）、六种中微子（six-species）双矩形式（two-moment formulation）结合代数闭合（algebraic closure）和特殊相对论观测修正（special-relativistic observer corrections），并利用间断伽辽金法（discontinuous Galerkin methods）进行相空间离散化与隐式-显式时间步进（implicit-explicit time stepping）。

Result: 在Flash-X框架中实现了基于非连续伽辽金（DG）方法的高阶中微子输运算法，并通过嵌套定点迭代与安德森加速的非线性中微子-物质耦合算法，实现了碰撞过程的完全隐式处理。
...
### [39] [Real-Time Multi-Mode Post-Merger Gravitational Wave Detection using Convolutional Neural Networks: Methodology Development for Third-Generation Detectors](https://arxiv.org/abs/2601.00985)
*Roo Weerasinghe*

**TL;DR**: 提出了一种卷积神经网络框架，用于实时检测双中子星并合（binary neutron star mergers）后引力波信号并提取多模频率（multi-mode frequency），在LIGO O4探测器噪声环境下实现了3.0毫秒延迟、48.6赫兹频率精度（53赫兹综合测试集）及99.998%检测效率（1%误报率），为第三代探测器（Einstein Telescope, Cosmic Explorer）奠定了关键技术基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升地面引力波探测器对双中子星并合后(post-merger)引力波信号的实时探测与多模态频率提取能力。

Data: 数据来源于包含真实LIGO O4探测器噪声及GravitySpy瞬态干扰形态的模拟数据集，并采用合成O4探测器噪声模型进行训练增强。

Method: 采用卷积神经网络框架进行实时检测和多模频率提取，并通过激进训练增强策略提升泛化能力。

Result: result：开发了一种卷积神经网络框架，可实现双中子星并合后引力波信号的实时检测（延迟3.0毫秒）和多模式频率提取（精度48.6-53赫兹），在包含真实LIGO O4噪声的验证中达到ROC AUC 0.999999和99.998%检测效率（1%误报率）。
...
### [40] [Accelerating cosmological simulations on GPUs: a step towards sustainability and green-awareness](https://arxiv.org/abs/2601.01935)
...
**TL;DR**: 通过GPU加速的pinocchio（暗物质晕快速生成工具）实现了8倍能耗降低和8倍速度提升，显著提高了宇宙学模拟的能效和性能。


<details>
  <summary>Details</summary>

Background: 宇宙学N体模拟(N-body simulation)的复杂性和规模日益增长，受欧几里得(Euclid)等天文巡天项目驱动，亟需向更可持续、高能效的高性能计算(HPC)范式转变。

Data: 数据来源为基于GPU加速的暗物质晕目录生成工具pinocchio，并在不同高性能计算平台上使用新开发的功耗测量工具包（Power Measurement Toolkit, PMT）进行性能分析。

Method: 采用新开发的功耗测量工具包(Power Measurement Toolkit, PMT)对GPU加速的暗物质晕快速生成工具pinocchio进行性能剖析，对比了不同高性能计算平台的能耗与计算时间。

Result: 基于GPU加速的pinocchio工具实现了8倍能耗降低（energy-to-solution）和8倍速度提升（time-to-solution），综合效率提升达64倍，显著减少宇宙学模拟的能源足迹。
...
### [41] [Prospects for identifying pulsar candidates in radio surveys using scintillation](https://arxiv.org/abs/2601.02097)
*Jitendra Salal,Shriharsh P. Tendulkar,Visweshwar Ram Marthi*

**TL;DR**: 基于脉冲星群模拟，研究发现利用闪烁特征在1420 MHz观测频率和10 kHz通道宽度下可高效探测传统时域巡天无法发现的脉冲星，其中50%的脉冲星占空比超过时域探测平均值0.09，未来应用该技术可使DSA-2000探测到56%普通脉冲星和84%毫秒脉冲星（MSPs）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用射电干涉图像中脉冲星的闪烁特征（scintillation signatures）进行候选体识别的方法优化及其在下一代射电望远镜（如DSA-2000）中的应用潜力。

Data: 数据来源为基于PsrPopPy模块的脉冲星群体模拟数据（simulation data）。

Method: 采用PsrPopPy模块进行脉冲星群体模拟，结合干涉射电图像中的闪烁特征识别技术，优化观测参数配置（1420 MHz频率和10 kHz通道宽度）。

Result: 基于闪烁特征的技术可额外探测56%的正常脉冲星和84%毫秒脉冲星(MSP)，且这些脉冲星无法通过时域搜索验证。
...
### [42] [Uncovering Hidden Systematics in Extreme-Precision Radial Velocity Measurements](https://arxiv.org/abs/2601.02296)
*Lily L. Zhao,Debra A. Fischer,Andrew E. Szymkowiak,John M. Brewer,Joe Llama*

**TL;DR**: 通过开发多维度回归方法校正EXPRES（EXtreme PREcision Spectrograph）径向速度数据中的仪器漂移（instrumental drifts），将太阳径向速度（solar RVs）的均方根误差（RMS）从1.32米/秒降至0.43米/秒，并消除ρ Coronae Borealis（ρCrB）恒星数据中的虚假行星信号（spurious planet d signal），显著提升下一代极精密径向速度仪（EPRV）对低振幅行星信号的探测灵敏度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于极端精度光谱仪（EXPRES）径向速度数据中系统性漂移的识别与校正，旨在提升下一代极端精度径向速度（EPRV）仪器的厘米级测量精度。

Data: data：研究使用来自极端精度光谱仪（EXPRES）的七年径向速度数据，并开发了基于二维阶梯光栅图位移、缩放和旋转的诊断方法，以及激光频率梳（LFC）谱线衍生的线双极跨度（LBS）指标。

Method: 采用多维回归结合二维阶梯光栅（échellogram）位移、缩放和旋转诊断以及激光频率梳（LFC）谱线衍生的线双极跨度（LBS）来隔离和校正仪器漂移。

Result: 通过多维回归结合仪器遥测数据，开发了一套诊断方法校正EXPRES（极端精密光谱仪）的径向速度系统漂移，使仪器趋势的均方根误差从1.32米/秒降至0.43米/秒，对低振幅行星信号的探测灵敏度提升了一倍。
...
### [43] [Finite parts of inflationary loops II: A streamlined UV in-in algorithm and distinguishable signatures](https://arxiv.org/abs/2512.20467)
*Guillermo Ballesteros,Jesús Gambín Egea,Flavio Riccardi*

**TL;DR**: 提出了一种简化维度正规化（dimensional regularization）计算任意外腿和顶点数目的in-in圈积分方法，发现哈密顿反项（Hamiltonian counterterms）在in-in形式中重整化存在困难，并识别出有效场论暴胀（effective field theory of inflation）下单圈原初三点谱（one-loop primordial bispectrum）中区别于树图的贡献。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用维度正规化（dimensional regularization）方法简化含任意外腿和顶点的圈图计算，以补充现有工作并促进紫外（ultraviolet）贡献的提取。

Data: 数据来源为理论模拟（simulation）数据，基于有效场论（effective field theory）框架下的暴胀模型（inflation）特定相互作用集合进行计算。

Method: 采用维度正规化(dimensional regularization)方法分析任意外腿和顶点数的in-in圈积分，重点提取紫外贡献并探讨哈密顿反项( Hamiltonian counterterms)重整化困难。

Result: 在有效场论框架下，研究识别了一类区别于树图贡献的原初三点关联函数（primordial bispectrum）单圈修正项。
...
### [44] [On Cosmological Correlators at One Loop](https://arxiv.org/abs/2601.00952)
*Guilherme L. Pimentel,Tom Westerdijk*

**TL;DR**: 论文通过时间排序分解和宇宙学Baikov表示法，系统构建了平坦时空中无质量标量场单圈等时关联函数的简化积分，揭示了其比波函数系数更简单的特性，分析了泡图和三角图的紫外发散与有限结果，利用朗道分析分类运动学奇点并验证，最终推导出奇异运动学下单圈宇宙学关联因子与平坦时空圈振幅及低点树级关联的分解关系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于平坦时空中无质量标量场（massless scalar fields）的单圈（one-loop）等时关联函数（equal-time in-in correlators）的计算与性质分析。

Data: 研究使用平直时空中无质量标量场（massless scalar fields）的等时关联函数（equal-time in-in correlators）作为数据来源，通过时间顺序分解关联函数和宇宙学Baikov表示（cosmological analogue of the Baikov representation）构建简单环路积分。

Method: 采用时间顺序分解关联函数和宇宙学Baikov表示（Baikov representation）的系统方法构建简化环路积分，并利用拉普拉斯格林函数（Laplacian Green's function）的积分变换显式计算三角形图解的有限结果。

Result: 在平坦时空中，无质量标量场的等时关联函数（equal-time in-in correlators）的单圈修正比波函数系数（wavefunction coefficients）的单圈修正更简单，且可通过巴伊科夫表示（Baikov representation）的宇宙学类比系统构造简单圈积分。
...
### [45] [From the Wavefunction of the Universe to In-In-Correlators: A Perturbative Map to All Orders](https://arxiv.org/abs/2601.00992)
*Gonzalo A. Palma*

**TL;DR**: 构建了宇宙波函数(Wavefunction of the Universe)与Schwinger-Keldysh实时形式主义在微扰论所有阶数下的显式图解对应关系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙波函数(Wavefunction of the Universe)与Schwinger-Keldysh实时(in-in)形式主义在原始宇宙学观测量分析中的理论关联。

Data: 研究采用理论推导（simulation）方法，基于波函数宇宙论（Wavefunction of the Universe）和施温格-凯尔迪什（Schwinger-Keldysh）形式体系的图展开对应关系构建映射。

Method: 通过分析波函数宇宙学（Wavefunction of the Universe）与Schwinger-Keldysh实时形式主义（Schwinger-Keldysh in-in formalism）在微扰展开层面的关系，建立了二者图展开之间的系统性对应映射。

Result: 在波函数宇宙论（Wavefunction of the Universe）方法中贡献于关联函数的图可唯一重组为Schwinger-Keldysh图。
...
### [46] [Quantum dynamics of cosmological particle production: interacting quantum field theories with matrix product states](https://arxiv.org/abs/2601.02331)
*Evan Budd,Adrien Florio,David Frenklakh,Swagato Mukherjee*

**TL;DR**: 采用张量网络方法研究了1+1维膨胀引力背景下相互作用的标量场（λφ⁴理论）和规范场（Schwinger模型）的实时动力学，发现自相互作用会抑制引力粒子产生并影响纠缠行为：标量场中抑制纠缠增长，而Schwinger模型中粒子产生抑制与关联增强导致更复杂的纠缠特性。


<details>
  <summary>Details</summary>

Background: 研究背景为探索弯曲时空（curved spacetime）中相互作用量子场（interacting quantum fields）的实时动力学行为及其非微扰特性。

Data: 数据来源为基于张量网络方法对1+1维时空中标量λφ⁴理论和施温格模型（Schwinger model）的数值模拟（simulation）。

Method: 采用张量网络方法（tensor network methods）研究1+1维时空中的相互作用标量场（$λφ^4$理论）和规范场（Schwinger模型）在模拟均匀膨胀引力背景的量子淬火（quench）下的实时动力学。

Result: 自相互作用导致引力粒子产生（gravitational particle production）相比自由场情形受到抑制。
...
### [47] [High Frequency Spectrum of Primordial Gravitational Waves](https://arxiv.org/abs/2601.00378)
*Kamil Mudrunka,Kazunori Nakayama*

**TL;DR**: 该论文详细计算了暴胀期间和之后产生的高频引力波（gravitational waves）频谱，发现考虑暴胀子（inflaton）振荡后频谱可延伸至更高频段，且详细频谱特征能区分不同暴胀模型。


<details>
  <summary>Details</summary>

Background: 研究探讨了暴胀（inflation）期间及之后产生的原初引力波（primordial gravitational waves）频谱特性及其对暴胀模型的区分能力。

Data: 数据来源于对暴胀期间和暴胀后原初引力波（primordial gravitational waves）的理论模拟（simulation），重点关注低频与高频区域的衔接。

Method: 采用暴胀子（inflaton）动力学模型分析暴胀期间及之后产生的引力波频谱，重点研究高低频段的衔接特性。

Result: 原初引力波（primordial gravitational waves）频谱可延伸至比以往预期更高的频率范围，且其精细结构能有效区分不同暴胀（inflation）模型。
...
### [48] [Asymmetric Dark Matter from Low-Scale Spontaneous Leptogenesis](https://arxiv.org/abs/2601.01849)
*Hiroki Takahashi,Juntaro Wada*

**TL;DR**: 提出了一种新型非对称暗物质(ADM)模型，通过低能标自发轻子生成机制同时产生宇宙中的暗物质不对称性和重子不对称性(BAU)，并预测暗物质质量在0.1 GeV至100 GeV（平衡态）或10 TeV（非平衡态）范围内。


<details>
  <summary>Details</summary>

Background: 研究探索了一种新型非对称暗物质（ADM）模型，其核心科学背景是通过低能标自发轻子生成机制同时解释宇宙中的暗物质不对称性和重子不对称性（BAU）。

Data: 研究采用理论模拟（simulation）方法，通过低能标自发轻子生成机制（low-scale spontaneous leptogenesis）构建不对称暗物质（ADM）模型，其中右手中微子质量标度远低于Davidson-Ibarra界限（$M_1 \ll 10^{9}~\rm{GeV}$），并利用动力学$CP$相位（majoron）的动力学错位（kinetic misalignment）作为时间依赖的有效$CP$相位源。

Method: 采用低尺度自发轻子生成机制（low-scale spontaneous leptogenesis）和动力学CP相位（dynamical CP phase）即majoron的动力学错位（kinetic misalignment）作为核心分析方法。

Result: 暗物质（ADM）质量在达到平衡值时典型预测范围为0.1 GeV至100 GeV，未达平衡时则扩展至0.1 GeV至10 TeV。
...
### [49] [A statistical framework for quantitative spectroscopy of luminous blue stars](https://arxiv.org/abs/2601.01491)
*Miguel A. Urbaneja*

**TL;DR**: 提出MAUI框架（Machine-learning Assisted Uncertainty Inference），利用高斯过程模拟器（Gaussian-process-based emulators）结合马尔可夫链蒙特卡洛（MCMC）采样，在保持非局部热动平衡（non-LTE）模型精度的前提下，将大质量恒星（O型矮星和B型矮星/亚巨星）光谱分析的计算成本降低数个量级。


<details>
  <summary>Details</summary>

Background: 研究背景为利用机器学习辅助贝叶斯推断（MAUI）提升大质量恒星（luminous blue stars）非局部热动平衡（non-LTE）光谱建模效率，以应对高精度模型计算量激增的挑战。

Data: 数据来源为有限数量的非局部热动平衡(non-LTE)模拟数据集，用于训练高斯过程(Gaussian-process)仿真器。

Method: 采用高斯过程(Gaussian process)模拟器结合马尔可夫链蒙特卡洛(MCMC)采样进行贝叶斯推断，通过有限非局部热动平衡(non-LTE)模拟训练实现高效后验分布探索。

Result: MAUI框架通过高斯过程(Gaussian-process)模拟器和马尔可夫链蒙特卡洛(MCMC)采样实现了高效贝叶斯推断，在保持非局部热动平衡(non-LTE)模型精度的同时将计算成本降低数个数量级。
...
### [50] [Can instrumental effects obscure the true photospheric wave spectrum?](https://arxiv.org/abs/2601.02155)
*M. Berretti,M. Stangalini,D. B. Jess,S. Jafarzadeh,S. D. T. Grant,G. Verth,V. Fedun,G. Chambers,F. Berrilli*

**TL;DR**: 利用高分辨率观测数据首次揭示了仪器分辨率降低会导致太阳磁孔（magnetic pore）多普勒速度振荡主频带从5毫赫兹偏移至3毫赫兹，证实低分辨率仪器会因邻近平静太阳区域的杂散光污染而低估本影振荡（umbral oscillations）的真实波能贡献。


<details>
  <summary>Details</summary>

Background: 研究太阳大气中光学像差（optical aberrations）和仪器分辨率对动力学过程观测的影响。

Data: 数据来源于邓恩太阳望远镜（Dunn Solar Telescope）上搭载的干涉二维光谱仪（Interferometric BIdimensional Spectrometer）获取的高分辨率磁孔（magnetic pore）观测数据。

Method: 利用干涉测量二维光谱仪(Interferometric BIdimensional Spectrometer)的高分辨率观测数据，通过分析衍射极限和仪器采样对视线多普勒速度振荡的影响。

Result: 衍射极限和仪器采样会显著改变太阳大气磁孔（magnetic pore）视线多普勒速度振荡（line-of-sight Doppler velocity oscillations）的主导频带，使其从5 mHz偏移至3 mHz。
...
### [51] [The SALT survey of helium-rich hot subdwarfs: unsupervised classification and kinematic analysis](https://arxiv.org/abs/2601.02054)
*Asish Philip Monai,C. Simon Jeffery*

**TL;DR**: 基于南非大望远镜(SALT)对697颗富氦热亚矮星的光谱分析，采用主成分分析(PCA)和谱聚类(SC)方法定义了6个主要光谱簇，揭示了传统光谱分类中未识别的He-sdB/OB和iHe-sdB/OB亚群，并发现He-sdO和Hot-sdO类包含较高比例的晕族(halo)恒星。


<details>
  <summary>Details</summary>

Background: 研究聚焦于贫氢恒星（hydrogen-deficient stars）在赫罗图（HR diagram）上的演化序列，涵盖从冷R CrB星到富氦热亚矮星（helium-rich hot subdwarfs）及PG1159星的多种演化路径。

Data: 数据来源于南非大望远镜（SALT）对697颗富氦热亚矮星及相关恒星的中等分辨率光谱巡测，并从中选取587颗恒星作为完整样本。

Method: 采用主成分分析（PCA）和谱聚类（SC）的无监督数据驱动分类系统对587颗氦富热亚矮星光谱进行分析，识别出6个主要光谱簇及子簇。

Result: 基于主成分分析(PCA)和谱聚类(SC)的无监督分类系统识别出6个主要热亚矮星光谱簇，包括传统光谱类型的sdB、sdO、He-sdO和极热亚矮星，并在He-sdB/OB和iHe-sdB/OB簇中发现子结构。
...
### [52] [Ground State and Collective Modes of Bose-Einstein Condensates in Newtonian and MOND-inspired gravitational potentials](https://arxiv.org/abs/2601.01039)
*Ning Liu*

**TL;DR**: 论文核心贡献是揭示了在受修正牛顿动力学（MOND）启发的对数势中玻色-爱因斯坦凝聚体（Bose-Einstein condensates）的基态和集体动力学特性，发现其尺寸随排斥耦合参数β呈β^(1/3)标度律增大且振荡频率在强相互作用极限下按β^(-1/3)标度降低，而牛顿势中不存在简单标度关系。


<details>
  <summary>Details</summary>

Background: 该研究探讨了玻色-爱因斯坦凝聚体(BEC)在牛顿势和受修正牛顿动力学(MOND)启发的对数势中的基态和集体动力学行为。

Data: 数据来源于对牛顿势和受修正牛顿动力学(MOND)启发的对数势中玻色-爱因斯坦凝聚体基态和集体动力学的解析与数值研究。

Method: 采用解析和数值方法研究玻色-爱因斯坦凝聚体在牛顿势和修正牛顿动力学(MOND)启发的对数势中的基态和集体动力学行为。

Result: 在基态研究中，修正牛顿动力学(MOND)势仅在深MOND区域支持束缚态，且凝聚体尺寸显著大于牛顿势情况。
...
### [53] [Compensating Star-Trackers Misalignments with Adaptive Multi-Model Estimation](https://arxiv.org/abs/2601.01130)
*Ridma Ganganath,Simone Servadio,David Daeyoung Lee*

**TL;DR**: 提出了一种自适应多模型框架（MEKF-MMAE），用于在无GPS的深空立方体卫星（CubeSat）任务中联合估计航天器姿态和星跟踪器（star-tracker）对准偏差，通过蒙特卡洛模拟验证了该框架在弧秒级对准偏差估计和亚度级姿态误差方面的鲁棒性，并首次实现了双星跟踪器对准偏差的六维网格高效估计。


<details>
  <summary>Details</summary>

Background: 研究聚焦于在无GPS信号的深空立方体卫星任务中，开发自适应多模型框架以联合估计航天器姿态和星敏感器（star-tracker）对准偏差。

Data: 数据来源于陀螺仪测量值、基于TRIAD（Tri-Axial Attitude Determination）的姿态观测数据以及两台星敏感器（star tracker）的堆叠视线测量数据，并通过蒙特卡洛（Monte Carlo）仿真验证了算法性能。

Method: 采用乘性扩展卡尔曼滤波(MEKF)估计姿态、角速度和陀螺偏差，同时基于贝叶斯多模型自适应估计(MMAE)在离散的星敏感器安装偏差假设网格上进行更新。

Result: 提出的MEKF-MMAE架构通过自适应多模型框架实现了GPS拒止深空立方星任务中航天器姿态和星敏感器失准角的联合高精度估计，蒙特卡洛仿真验证了该框架在单/双星敏感器配置下均能保持弧秒级失准角估计误差和亚度级姿态误差的鲁棒性。
...
### [54] [Probabilistic modeling of Cherenkov emission from particle showers](https://arxiv.org/abs/2601.00944)
*Ian Crawshaw,Tianlu Yuan,Emre Yildici,Lu Lu,Anatoli Fedynitch*

**TL;DR**: 该论文构建了能够描述冰中切伦科夫光（Cherenkov light）产额分布的参数化模型，显著提高了中微子望远镜对簇射（shower）事件幅度和形状波动的模拟精度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高能粒子簇射（particle shower）在多种介质中的普适性行为及其切伦科夫光（Cherenkov light）产额参数化模拟的优化。

Data: 研究中使用的数据来源是基于蒙特卡洛模拟（Monte Carlo simulation）生成的切伦科夫光（Cherenkov light）产额参数分布，并适用于冰介质及其他类似介质。

Method: 采用蒙特卡洛（Monte Carlo）模拟和参数化近似方法构建切伦科夫光（Cherenkov light）产额分布模型，用于描述冰中粒子簇射（particle showers）的幅度和形状涨落。

Result: 构建的参数分布能够描述冰中粒子簇射（particle shower）产生的切伦科夫光（Cherenkov light）产额，并可扩展至其他类似介质。
...
### [55] [SN 2024abvb: A Type Icn Supernova in the Outskirts of its Host Galaxy](https://arxiv.org/abs/2601.01333)
...
**TL;DR**: 论文发现超新星（supernova）2024abvb具有早期显著的碳离子窄发射线和蓝色连续谱，缺乏氢和氦特征，将其归类为Icn型超新星（SN Icn），并通过混合模型估算出其周围物质（CSM）、放射性镍和抛射物质量分别为0.28、3.54×10⁻³和0.12太阳质量，表明前身星经历了显著的质量剥离过程。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超新星（SN）2024abvb的观测与分类，探讨其作为罕见Icn型超新星（SN Icn）的物理起源及前身星演化特征。

Data: 数据来源包括超新星（SN）2024abvb的多波段测光（multiband photometric）和光谱（spectroscopic）观测数据。

Method: 采用混合模型（hybrid model）同时考虑抛射物-星周物质（ejecta-CSM）相互作用释放的能量和抛射物中镍放射性衰变来拟合光变曲线。

Result: SN 2024abvb被暂定为Icn型超新星(SN Icn)，其r波段峰值绝对星等为-19.7，属于Icn型超新星中的明亮成员。
...
### [56] [Looking for observational signatures of early binary black hole systems](https://arxiv.org/abs/2601.02288)
*Peggy Varniere,Fabien Casse,Fabrice Dodu*

**TL;DR**: 研究发现早期双黑洞（BBH）系统中原初吸积盘的截断（truncation）可作为识别此类系统的必要观测特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于早期双黑洞（BBH）系统中环绕主黑洞的原有吸积盘（pre-existing disk）受次级黑洞扰动产生的可观测特征及其热辐射信号。

Data: 研究中使用的数据来源于对早期双黑洞（BBH）系统中主黑洞周围原有吸积盘（pre-existing disk）的流体动力学模拟（hydrodynamical simulations），重点关注次级黑洞对吸积盘结构的影响。

Method: 采用广义相对论光线追踪（general relativistic ray-tracing）技术生成不同质量比和间距的早期双黑洞系统热辐射合成观测数据。

Result: 早期双黑洞（BBH）系统中预存盘（pre-existing disk）的截断（truncation）是其必要（非唯一）的观测特征。
...
### [57] [Calorimetric Measurement of the Surface Energy of Enstatite, MgSiO$_3$](https://arxiv.org/abs/2601.01314)
*Megan A. Householder,Tamilarasan Subramani,Kristina Lilova,James R. Lyons,Rhonda M. Stroud,Alexandra Navrotsky*

**TL;DR**: 首次直接测量了顽辉石（enstatite, MgSiO3）的表面能为4.79±0.45 J m−2，发现其高表面能可能阻碍行星大气等环境中的成核过程，同时界面能接近零且块体正交/单斜相变焓为0.34±0.93 kJ mol−1。


<details>
  <summary>Details</summary>

Background: 研究聚焦矿物表面热力学（surface thermodynamics）对地球和行星系统中矿物性质及分布的影响。

Data: 采用溶胶-凝胶法（sol-gel method）合成不同尺寸的顽火辉石（enstatite）纳米颗粒，并通过X射线衍射（X-ray diffraction）、热分析（thermal analysis）、红外光谱（infrared spectroscopy）、比表面积测量（surface area measurements）和电子显微镜（electron microscopy）进行表征。

Method: 采用高温氧化物熔融溶液量热法（high-temperature oxide melt solution calorimetry）结合X射线衍射、热分析、红外光谱、比表面积测量和电子显微镜表征技术，通过溶胶-凝胶法（sol-gel method）合成不同尺寸的顽辉石（enstatite）纳米颗粒，并基于量热学数据与计算得到的表面及界面面积进行分析。

Result: 首次直接测量得到顽辉石（enstatite）的表面焓为4.79±0.45 J m$^{-2}$，该值接近镁橄榄石（forsterite）且高于多数非硅酸盐氧化物材料。
...
### [58] [Hunting for "Oddballs" with Machine Learning: Detecting Anomalous Exoplanets Using a Deep-Learned Low-Dimensional Representation of Transit Spectra with Autoencoders](https://arxiv.org/abs/2601.02324)
*Alexander Roman,Emilie Panek,Roy T. Forestano,Eyup B. Unlu,Katia Matcheva,Konstantin T. Matchev*

**TL;DR**: 提出基于自编码器降维的异常检测方法，在低维潜在空间（latent space）中有效识别系外行星大气非常规化学特征（以CO2富集为异常指标），其中潜在空间的K均值聚类（K-means clustering）在30ppm高斯噪声下保持稳定性能。


<details>
  <summary>Details</summary>

Background: 研究主题为利用自编码器(autoencoder)的机器学习技术检测具有非常规化学特征(anomalous chemical signatures)的系外行星大气。

Data: 数据来源于公开的大气大数据挑战（Atmospheric Big Challenge, ABC）数据库，该数据库包含超过10万条模拟系外行星光谱数据。

Method: 采用基于自编码器(autoencoder)的机器学习技术进行异常检测，通过低维数据表征识别非常规化学特征系外行星大气，并对比了自编码器重构损失、单类支持向量机(1 class-SVM)、K均值聚类(K-means Clustering)和局部离群因子(Local Outlier Factor)四种方法在原始光谱空间和潜在空间(latent space)的性能。

Result: 在潜在空间（latent space）中进行异常检测比原始光谱空间更有效，尤其K均值聚类（K-means clustering）在潜在空间表现稳定且高效，且该方法在噪声水平达30 ppm时仍保持稳健性，50 ppm时仍可行。
...
**TL;DR**: 通过研究活动星系核（active galactic nuclei）中吸积盘（accretion disk）测试物体的运动学，首次应用考虑宇宙膨胀的史瓦西-德西特（Schwarzschild-de Sitter）黑洞模型，结合巨脉泽星系（megamaser galaxies）观测数据，利用贝叶斯推断（Bayesian inference）同时估计了黑洞质量、距离、发射体角位置和哈勃常数（Hubble constant），建立了不同于标准哈勃定律（Hubble law）的广义相对论框架。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用活动星系核（active galactic nuclei）中吸积盘（accretion disk）测试物体运动学，结合史瓦西-德西特黑洞（Schwarzschild-de Sitter black hole）模型与宇宙膨胀效应，建立广义相对论框架以关联黑洞参数与天体物理观测量的理论关系。

Data: 数据来源于哈勃流（Hubble flow）中的巨型脉泽星系（megamaser galaxies）的观测数据。

Method: 基于史瓦西-德西特（Schwarzschild-de Sitter）度规推导光子红移表达式，结合贝叶斯推断（Bayesian inference）和马尔可夫链蒙特卡洛（Markov Chain Monte Carlo）方法估计黑洞质量、距离、发射体角位置及哈勃常数。

Result: 推导了史瓦西-德西特(Schwarzschild-de Sitter)黑洞周围轨道粒子发射光子的红移表达式，该表达式将黑洞质量、距离、轨道半径和哈勃常数与可观测的红移及角位置直接关联。
...
### [60] [The Sequential Monte Carlo goes NUTS: Boosting Gravitational-Wave Inference](https://arxiv.org/abs/2601.02336)
...
**TL;DR**: 提出SHARPy框架，结合序贯蒙特卡洛（SMC）的并行性与证据估计能力及无调谐采样器（NUTS）的高效采样性能，利用后验局部几何结构提升效率，在十分钟内完成双黑洞引力波事件推断，结果与嵌套采样一致。


<details>
  <summary>Details</summary>

Background: 引力波推断中基于似然的方法面临高维参数空间探索和计算效率的挑战。

Data: 研究中使用的数据来源为引力波（gravitational-wave）事件中的双黑洞（binary black-hole）观测数据。

Method: 采用顺序蒙特卡洛(SMC)方法和基于梯度的马尔可夫链蒙特卡洛算法(No-U-Turn Sampler, NUTS)相结合，并利用后验分布的局部几何结构提升效率。

Result: SHARPy框架通过结合序贯蒙特卡洛(SMC)方法的并行性和证据估计能力与无调头采样器(NUTS)的高效采样性能，显著提升了引力波(gravitational-wave)推断的速度和效率。
...
### [61] [On the Properties of the Power-Law Cosmological Solutions in Lovelock Gravity](https://arxiv.org/abs/2601.01275)
*Sergey Pavluchenko*

**TL;DR**: 研究发现，在四阶及以上阶次的洛弗洛克引力（Lovelock gravity）中，高阶卡什纳解（Kasner solution）无法作为宇宙演化的过去渐近态，从而阻碍了高低阶解之间的平滑过渡并可能影响可行紧致化的实现。


<details>
  <summary>Details</summary>

Background: 研究探讨了洛弗洛克引力（Lovelock gravity）中卡什纳宇宙解（Kasner cosmological solutions）的特性及其在更高阶理论中作为渐近态的可能性差异。

Data: 数据来源于理论模拟（simulation）分析，基于洛夫洛克引力（Lovelock gravity）中的卡什纳宇宙学解（Kasner cosmological solutions）的数学推导。

Method: 采用洛弗洛克引力（Lovelock gravity）理论框架，重点分析高阶卡什纳解（Kasner solution）在四阶及以上洛弗洛克引力模型中的渐近行为及其对宇宙演化的限制。

Result: 在四次及更高阶洛夫洛克引力（Lovelock gravity）中，高阶卡什纳解（Kasner solution）无法作为过去渐近态，阻碍了平滑过渡并可能影响可行紧致化。
...
### [62] [Comparison of MOND and Verlinde's emergent gravity in dwarf spheroidals](https://arxiv.org/abs/2601.01715)
*Youngsub Yoon,Sanghyeon Han,Ho Seong Hwang*

**TL;DR**: 通过比较23个矮球状星系(dwarf spheroidals)的径向加速度观测值与修正牛顿动力学(MOND)和韦尔林德涌现引力(Verlinde's emergent gravity)理论预测值，发现后者在21个样本中更符合观测趋势且整体统计显著性达5.2σ，支持其优于MOND。


<details>
  <summary>Details</summary>

Background: 研究聚焦于比较修正牛顿动力学（MOND）和韦尔林德涌现引力（Verlinde's emergent gravity）在解释矮椭球星系（dwarf spheroidals）径向加速度观测数据上的表现差异。

Data: 数据来源为23个矮球状星系(dwarf spheroidals)的观测径向加速度数据。

Method: 采用修正牛顿动力学（MOND）和Verlinde涌现引力（emergent gravity）分别计算23个矮球状星系（dwarf spheroidals）的径向加速度并与观测值对比。

Result: 在23个矮球状星系(dwarf spheroidals)中的21个样本中，韦尔林德涌现引力(Verlinde's emergent gravity)比修正牛顿动力学(MOND)更接近观测径向加速度的趋势。
...
### [63] [FFCz: Fast Fourier Correction for Spectrum-Preserving Lossy Compression of Scientific Data](https://arxiv.org/abs/2601.01596)
*Congrong Ren,Robert Underwood,Sheng Di,Emrecan Kutay,Zarija Lukic,Aylin Yener,Franck Cappello,Hanqi Guo*

**TL;DR**: 提出了一种基于快速傅里叶校正算法（fast Fourier correction algorithm）的新型有损压缩技术，通过联合约束空间和频域误差，有效保留了宇宙学模拟、X射线衍射等科学数据的空间和频域关键特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于在科学数据有损压缩中同时保留空间域和频域（frequency domain）关键特征的技术挑战。

Data: 数据来源于宇宙学模拟（cosmology simulations）、X射线衍射（X-ray diffraction）、燃烧模拟（combustion simulation）和脑电图（electroencephalography）数据集。

Method: 采用基于快速傅里叶校正算法（fast Fourier correction algorithm）的迭代投影方法，通过将空间域误差向量投影到由空间和频率约束定义的区域来联合约束两域误差。

Result: 提出了一种基于快速傅里叶校正算法的新型技术，可在有损压缩中同时保留空间域和频率域的数据特征。
