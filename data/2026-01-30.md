...
**TL;DR**: 研究发现通过蒙特卡洛实验量化了投影效应和分辨率对分子云核（dense cores）最近邻（NN）间距测量的影响，并建立了依赖样本量和分辨率的经验校正因子，表明三维（3D）间距通常比二维（2D）测量值大20%至2倍，校正后形态驱动的不确定性约为30-40%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于分子云碎裂成致密核（dense cores）、纤维结构（filaments）和子星团（subclusters）过程中核心间距的二维投影与三维真实值之间的系统偏差及其校正方法。

Data: 研究使用球状和分形(fractal)玩具模型(toy models)生成数据，并通过蒙特卡洛(Monte Carlo)实验量化样本形态、大小和有效分辨率的偏差效应。

Method: 采用蒙特卡洛（Monte Carlo）实验量化投影效应和分辨率偏差，并通过球状和分形（fractal）玩具模型建立经验校正因子，将二维最近邻（NN）间距转换为三维估计值。

Result: 标准几何反投影因子（$4/π\simeq1.27$）不适用于核心间距测量，因为投影不仅缩短间距还重构最近邻网络，而有限角分辨率会合并邻近核心并放大表观间距。
...
### [2] [Little Red Dot $-$ Host Galaxy $=$ Black Hole Star: A Gas-Enshrouded Heart at the Center of Every Little Red Dot](https://arxiv.org/abs/2601.20929)
...
**TL;DR**: 这篇论文提出了一种基于[OIII]5008Å线区分小黑洞星体（BH*s）与宿主星系的新方法，通过分析98个小红点（LRDs）的光谱数据，发现其宿主扣除的中值叠加谱显示出强巴尔末跳变（>2倍大质量静止星系）和类黑体光谱特征（有效温度4050K、光度43.9 erg/s、半径1300天文单位），结合高巴尔末减幅（Hα/Hβ>10）和致密气体包层特征（如FeII、HeI、OI线），首次在群体层面证实了LRDs由BH*s主导（贡献率从紫外20%到Hα以远90%），并揭示BH*s普遍存在于经历近期星暴的低质量星系（恒星质量约1亿太阳质量），其短暂寿命（约1000万年）暗示所有大质量黑洞可能都经历过BH*阶段。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于探索小红色点源（LRDs）中心引擎的本质，提出其可能由具有致密气体包层的黑洞恒星（BH*s）驱动，并揭示这类天体在黑洞早期生长阶段的关键作用。

Data: 研究使用了98个LRDs（红移范围$z$~$2-9$）的高质量NIRSpec/PRISM光谱数据样本。

Method: 基于[OIII]5008Å线空间分离假设，采用宿主星系光谱扣除法构建中值叠加谱，结合黑体辐射谱拟合（SED）和发射线强度比（巴尔末减幅）分析，通过密度敏感谱线（FeII、HeI、OI）验证致密气体包层特征。

Result: 在LRDs（Little Red Dots）中观测到的强巴尔默跳变（Balmer break）、高密度敏感谱线（如FeII、HeI、OI）以及黑体状光谱能量分布（SED）等特征，为BH*s（black hole stars）作为其中心引擎提供了群体水平的证据。
...
### [3] [It's More Complicated Than You Think: A Forward Model to Infer the Recent Star Formation History, Bursty or Not, of Galaxy Populations](https://arxiv.org/abs/2601.20930)
...
**TL;DR**: 通过基于模拟的群体推断方法，首次在红移z~4的星系群中量化了恒星形成率(SFR)波动功率谱（power spectral density）及其时间尺度（1 Myr-10 Gyr），揭示了爆发式恒星形成历史（SFH）与长期平均斜率共同建模的必要性，并克服了传统诊断中爆发性（burstiness）被低估的局限。


<details>
  <summary>Details</summary>

Background: 研究主题为利用詹姆斯·韦伯太空望远镜（JWST）观测高红移（z > 4）星系群，通过基于模拟的群体推断方法解析恒星形成率（SFR）波动特性，以揭示早期宇宙恒星形成反馈物理机制。

Data: 数据来源于詹姆斯·韦伯太空望远镜（JWST）对红移（redshift）大于4的早期宇宙观测，以及基于功率谱密度（power spectral density）形式主义建立的随机恒星形成历史（SFH）模型对N=500个z~4星系的模拟样本。

Method: 采用基于功率谱密度（power spectral density）形式主义的随机恒星形成历史（SFH）模型，通过群体层面的模拟推断框架（simulation-based inference）正向建模星系群体及静止紫外至静止光学光谱特征分布，以恢复恒星形成率（SFR）波动的功率和时标。

Result: 基于群体水平模拟推断框架，成功测量了恒星形成率(SFR)波动的功率和时标，能有效区分不同模拟星系群体（如FIRE-2与Illustris类群体在<100 Myr时标上置信度>99%）。
...
### [4] [How supermassive black holes shape central entropies in galaxy clusters](https://arxiv.org/abs/2601.20932)
*Rainer Weinberger,Christoph Pfrommer*

**TL;DR**: 研究发现超大质量黑洞（SMBH）与星系团质量共同决定的平衡熵（$K\propto M_\bullet^{4/3}M_{500\mathrm{c}}^{-1}$）解释了冷核（cool core）系统中低中心熵现象，并揭示了超大质量黑洞质量在冷核星系团中存在上限（$1.4\times10^{10}\,\mathrm{M}_\odot$）。


<details>
  <summary>Details</summary>

Background: 研究探讨了冷核星系团（cool core clusters）中低中心熵值（central entropy）与超大质量黑洞（SMBH）吸积活动之间的平衡关系。

Data: 数据来源于三维流体动力学模拟（three-dimensional hydrodynamical simulations）中理想化的类英仙座星系团（idealized Perseus-like cluster）并包含活动星系核喷流（AGN jets）的模拟数据。

Method: 采用时间平均平衡假设结合邦迪吸积(Bondi accretion)和活动星系核(AGN)喷流加热机制，推导出与超大质量黑洞(SMBH)及星系团质量相关的平衡熵标度关系$K\propto M_\bullet^{4/3}M_{500\mathrm{c}}^{-1}$，并通过三维流体力学模拟验证该解析模型。

Result: 超大质量黑洞（SMBH）和星系团质量共同决定了冷却核心（cool core）系统的中心熵值（entropy），其关系为$K\propto M_\bullet^{4/3}M_{500\mathrm{c}}^{-1}$。
...
### [5] [What drives bar rotation? The effect of internal properties and galaxy interactions on bar pattern speeds](https://arxiv.org/abs/2601.20941)
*Alex Merrow,Francesca Fragkoudi,Rob J. J. Grand,Marie Martig*

**TL;DR**: 基于Auriga宇宙学流体动力学模拟，研究发现重子物质主导（baryon-dominated）星系在z=0时形成更早、转速更高且保持稳定的棒结构（bar），其转速与恒星质量正相关，而暗物质主导（dark matter-dominated）星系棒转速较慢，外部相互作用（如并合和飞掠）仅微弱降低棒转速。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙学背景下星系棒（galactic bar）旋转速度（pattern speed）的内外部驱动因素，特别是重子物质（baryon）主导与暗物质（dark matter）主导星系对棒旋转特性的差异化影响。

Data: 使用了Auriga宇宙学流体动力学放大模拟（Auriga suite of cosmological hydrodynamical zoom-in simulations）的数据。

Method: 采用宇宙学流体动力学放大模拟（Auriga suite）计算棒旋转速度（pattern speed）和棒旋转率（rotation rate），并分析其与棒年龄、强度、重子主导度（baryon dominance）、星系恒星质量及外部相互作用历史的关联。

Result: 星系中棒（bar）的旋转速度主要受内部重子（baryon）主导程度驱动，重子主导的星系形成更早且棒旋转更快，而暗物质（dark matter）主导的星系棒旋转较慢；此外，外部相互作用（如并合和飞掠）与较低的棒旋转率弱相关，但内部重子主导仍是快速旋转的主要驱动因素。
...
### [6] [Radiation hydrodynamic simulation of the Haro 11 galaxy: the escape of LyC and Ly$α$ in a dwarf galaxy merger](https://arxiv.org/abs/2601.20943)
*Timmy Ejdetjärn,Göran Östlin,Joakim Rosdahl,Jérémy Blaizot,Oscar Agertz*

**TL;DR**: 通过辐射流体动力学模拟Haro 11星系合并过程，发现莱曼连续谱(LyC)和莱曼α(Lyα)逃逸分数(f_esc)在近心点通过时分别显著增加和适度增加，且恒星形成爆发期间LyC逃逸增强，其中致密恒星团块C是LyC辐射的主要来源。


<details>
  <summary>Details</summary>

Background: 研究聚焦于Haro 11星系合并作为近邻莱曼连续谱(LyC)泄漏体和强莱曼-α(Lyα)发射源，为理解高红移星系再电离早期宇宙的物理机制提供了重要类比对象。

Data: 研究采用辐射流体动力学模拟（radiation hydrodynamics simulation）生成哈罗11（Haro 11）星系合并过程的模拟数据，并基于此创建了莱曼连续谱（LyC）、莱曼α（Lyα）和Hα的模拟观测数据。

Method: 采用辐射流体动力学模拟（radiation hydrodynamics simulation）追踪合并过程中莱曼连续谱（LyC）、莱曼α（Lyα）和Hα的辐射特性，并通过构建模拟观测计算其光度（L）和逃逸分数（f_esc）。

Result: result：Haro 11星系合并过程中莱曼连续谱(LyC)和莱曼α(Lyα)的逃逸分数(f_esc)在视线方向上变化1-2个数量级，且在两次近心点通过时总f_esc^LyC增加约一个数量级，而f_esc^Lyα仅适度增加。
...
**TL;DR**: 通过结合欧几里得(Euclid)早期释放观测(ERO)、GALEX和AstroSat/UVIT的紫外数据以及CFHT/MegaCam的多波段观测，重建了英仙座星系团中59个早型星系(ETGs)的恒星形成历史(SFH)，发现大多数ETGs需要紫外上升(UV upturn)机制解释其远紫外辐射，且这些星系在红移z~5时已形成超过30%的恒星质量，其中当前质量$\log_{10}(M_{\ast}/M_{\odot})\gtrsim 10.8$的ETGs在z~5时的恒星质量与JWST观测的高红移(z>4.6)静止红星系相当。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用多波段观测数据（包括紫外至近红外）分析英仙座星系团（Perseus galaxy cluster）中早型星系（ETGs）的恒星形成历史（SFH）和恒星种群（SP）特性，并探讨邻近大质量早型星系与高红移（z>4.6）静止星系（red quiescent galaxies）之间的演化关联。

Data: 数据来源包括欧几里得早期释放观测（ERO）对英仙座星系团中心区域0.7平方度的观测数据，结合GALEX和AstroSat/UVIT的远紫外（FUV）和近紫外（NUV）观测数据，以及加拿大-法国-夏威夷望远镜（CFHT）上MegaCam的$ugrizHα$波段数据。

Method: 采用光谱能量分布(SED)拟合代码CIGALE结合先进恒星族(SP)模型重建了59个早型星系(ETGs)的恒星形成历史(SFH)，并通过径向SED拟合分析了6个最巨大ETGs的空间分辨恒星族。

Result: 多数早型星系（ETGs）需要紫外上升（UV upturn）来解释其远紫外（FUV）辐射，其平均温度约为33800 K。
...
### [8] [Anomalous narrow line Seyfert I galaxies from SDSS DR17](https://arxiv.org/abs/2601.20957)
*Arihant Tiwari,Rachana,Vivek M,Suvendu Rakshit*

**TL;DR**: 通过分析22,656个窄线赛弗特1星系(NLSy1s)，发现620个异常源并分为三类：246个红NLSy1s（宿主星系主导光谱且AGN核心低光度）、374个蓝NLSy1s（AGN活动增强且连续谱更蓝）及257个中间型赛弗特（复合发射线轮廓且无连续谱），证实红蓝NLSy1s差异源于AGN活动本征差异而非尘埃遮蔽或取向效应。


<details>
  <summary>Details</summary>

Background: 研究聚焦于窄线赛弗特1型星系（NLSy1s）的光谱异常特征及其与活动星系核（AGN）内在物理机制的关联。

Data: 数据来源于斯隆数字巡天(SDSS) DR17发布的22,656个窄线赛弗特1星系(NLSy1s)（红移范围0.1≤z≤0.9），并辅以广域红外巡天探测器(WISE)颜色分析数据。

Method: 采用主成分分析（PCA）和分层k均值聚类（hierarchical k-means clustering）的光谱类星体异常检测算法（SQuAD），结合颜色余量（$E_{(B-V)}$）计算、广域红外巡天探测器（WISE）颜色分析、O III探测、等效宽度作为倾角指标、BPT图和特征向量1图等多种物理诊断分析。

Result: 在SDSS DR17的22,656个窄线赛弗特1星系(NLSy1s)中检测到620个光谱异常源，分为红NLSy1s(246个)、蓝NLSy1s(374个)和中间赛弗特(257个)三类，其差异主要源于活动星系核(AGN)活动的本质区别而非尘埃遮蔽或取向效应。
...
### [9] [Tidal features as tracers of galaxy merger histories: the colours of tidal features in HSC-SSP](https://arxiv.org/abs/2601.20974)
*A. Desmons,S. Brough,L. Canepa,A. Khalid*

**TL;DR**: 通过分析3.2万颗星系（包括1415颗具有潮汐特征（tidal features）的星系）的径向$g-i$颜色分布，发现潮汐特征宿主星系（红色序（red sequence））外围更红且质量比（mass ratios）随宿主恒星质量增加而降低，证实其外围恒星主要源自贫气体小质量并合（gas-poor minor mergers），且壳层（shells）比流状结构（streams）对应更高并合质量比。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过潮汐特征（tidal features）的径向颜色梯度分析星系并合（galaxy mergers）对星系演化的影响机制。

Data: 数据来源于Hyper Suprime-Cam Subaru Strategic Program光学成像巡天观测的约32,000个星系样本，其中包含1,415个具有潮汐特征（tidal features）的星系。

Method: 采用径向$g-i$颜色剖面分析方法比较潮汐特征（tidal features）与非潮汐特征宿主星系，结合质量比（mass ratio）统计揭示并合（mergers）性质差异。

Result: 潮汐特征宿主红序星系的外围区域比非潮汐特征宿主星系更红，表明其外围主要由贫气小质量并合（gas-poor minor mergers）吸积的恒星主导。
...
### [10] [Disk Wind Feedback from High-mass Protostars. V. Application of Multi-Modal Machine Learning to Characterize Outflow Properties](https://arxiv.org/abs/2601.21100)
*Duo Xu,Ioana A. Stelea,Joshua S. Speagle,Yichen Zhang,Jonathan C. Tan*

**TL;DR**: 提出了一种结合空间和光谱信息的深度学习框架，通过交叉注意力机制整合形态学和运动学特征，显著提升了从CO观测数据中推断原恒星外流（protostellar outflows）质量、倾角和位置角（PA）的精度，有效克服了传统方法的投影偏差问题。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用多模态深度学习克服原恒星外流(protostellar outflows)投影效应，以提升恒星形成反馈机制的理解精度。

Data: 数据来源为合成ALMA（Atacama Large Millimeter/submillimeter Array）观测数据，这些数据通过3D磁流体动力学（magnetohydrodynamic）模拟生成。

Method: 采用多模态深度学习框架，结合空间和光谱信息，通过交叉注意力融合机制整合形态学和运动学特征，并基于合成ALMA观测数据进行训练，实现概率不确定性估计。

Result: 多模态深度学习框架通过联合利用CO观测的空间和光谱信息，显著提升了原恒星外流(outflow)质量、倾角(inclination)和位置角(PA)的推断精度，并克服了传统方法的投影偏差(projection bias)。
...
### [11] [Properties of Polarized Radio Sources in the Wide Chandra Deep Field South from 2 to 4GHz](https://arxiv.org/abs/2601.20982)
...
**TL;DR**: 在宽钱德拉深场南天区(W-CDFS) S波段(2-4 GHz)研究中，发现总强度高于≈10mJy的射电源平均极化分数为≈3%，而低于1mJy的源(主要为恒星形成星系)极化分数上限约为3%，且极化分数与带内谱指数相关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宽钱德拉深场南天区（Wide Chandra Deep Field South, W-CDFS）S波段（2-4 GHz）射电源的线性偏振特性及其与总强度、法拉第旋转（Faraday Rotation）和频谱指数（spectral index）的关联。

Data: 数据来源于对宽钱德拉深场南天区(W-CDFS)的S波段(2-4 GHz)射电观测，角分辨率为15角秒，斯托克斯I的1σ均方根噪声约为50微焦耳/波束。

Method: 采用线性偏振测量、法拉第旋转（Faraday Rotation）测量和偏振源计数建模方法分析S波段（2-4 GHz）射电源的偏振特性，并通过叠加（stacking）技术估算弱源的偏振上限。

Result: 在S波段（2-4 GHz）对W-CDFS（Wide Chandra Deep Field South）区域内射电源的线性偏振特性研究中，发现总强度高于≈10mJy的源平均分数偏振（fractional polarization）值约为3%，而总强度低于1mJy的源（主要为恒星形成星系）的分数偏振上限约为≲3%。
...
### [12] [Radio-Near Infrared Imaging of Dual Active Galactic Nuclei Candidates](https://arxiv.org/abs/2601.20984)
...
**TL;DR**: 通过甚长基线干涉阵(VLBA)和哈勃太空望远镜观测，在J0948+6848和J1223+5409两个近邻(z<0.25)双峰窄线星系中发现了相距约90和56秒差距的高亮度温度(>10^8 K)双活动星系核(AGN)候选体或射电喷流。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用甚长基线干涉阵列(VLBA)和哈勃空间望远镜(Hubble Space Telescope)观测近邻($z<$0.25)具有双峰窄发射线特征星系中的双活动星系核(dual AGN)候选体。

Data: 数据来源包括甚长基线干涉阵列（VLBA）的高分辨率L波段（1.5 GHz或18 cm）连续谱图像和哈勃空间望远镜（Hubble Space Telescope）WFC3/IR F160W图像。

Method: 采用甚长基线干涉阵列（VLBA）的高分辨率L波段（1.5 GHz）连续谱成像和哈勃太空望远镜（HST）WFC3/IR F160W近红外成像技术，结合高亮度温度（>10^8 K）射电源的探测，分析候选双活动星系核（AGN）系统的亚秒级射电结构与近红外对应体空间分布。

Result: 在J0948+6848和J1223+5409两个候选双活动星系核(AGN)系统中，探测到与近红外AGN近似共位的致密非热射电辐射，存在两个高亮度温度(>10^8 K)射电源，表明可能存在秒差距级分离的双AGN(d_sep~90 pc和~56 pc)或射电喷流。
...
### [13] [The Initial Mass Function as the Equilibrium State of a Variational Process: why the IMF cannot be sampled stochastically](https://arxiv.org/abs/2601.20998)
*Eda Gjergo,Zhiyu Zhang,Pavel Kroupa*

**TL;DR**: 该研究通过最大熵原理( Maximum Entropy principle )证明了恒星初始质量函数( sIMF )是分子云薄光学厚度( optically thin )状态下依赖密度的冷却过程导致的碎裂过程中最大无偏分布，为恒星形成的确定性本质提供了理论基础。


<details>
  <summary>Details</summary>

Background: 恒星初始质量函数（sIMF）的确定性物理起源及其与最大熵原理的关系是研究的核心科学主题。

Data: data来源为基于最大熵原理（Maximum Entropy principle）对恒星形成团块（star-forming clumps）在光学薄（optically thin）状态下密度依赖冷却过程的分裂（fragmentation）进行理论推导和模拟分析。

Method: 采用变分法最小化基于碎裂假设获得的熵泛函，并结合最大熵原理（Maximum Entropy principle）推导出恒星初始质量函数（sIMF）的幂律形式。

Result: 恒星初始质量函数（sIMF）是应用最大熵原理于光学薄区密度依赖冷却所设定结构的恒星形成团块碎裂过程时产生的无偏分布。
...
### [14] [The LEGARE Project. I. Chemical evolution model of the Nuclear Stellar Disc in a Bayesian framework](https://arxiv.org/abs/2601.21032)
*E. Spitoni,M. Schultheis,F. Matteucci,N. Ryde,G. Cescutti,A. Saro,M. C. Sormani,B. Thorsbro*

**TL;DR**: 研究发现银河系核恒星盘（NSD）的低金属丰度尾部特征无法用内盘气体流入模型解释，需引入额外贫金属气体稀释或考虑核球恒星污染的影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系核恒星盘（NSD）的化学演化过程，探讨其形成机制与内盘棒驱动气体流入（bar-driven gas inflows）的关联性。

Data: 数据来源于银河系核恒星盘（NSD）的观测数据，包括金属丰度分布函数（MDFs）和Mg、Si、Ca相对于Fe的丰度比，以及LEGARE项目构建的化学演化模型所采用的贝叶斯MCMC框架拟合的不同NSD恒星样本的MDFs。

Method: 采用贝叶斯马尔可夫链蒙特卡洛(MCMC)框架拟合核恒星盘(NSD)不同恒星样本的金属丰度分布函数(MDFs)，并基于银河系棒驱动气体流入假设构建化学演化模型。

Result: 银河系核恒星盘（NSD）的化学演化模型表明，若忽略核球（bulge）恒星污染，仅靠内盘气体流入无法解释观测到的低金属丰度尾部，需引入额外贫金属气体稀释；若考虑核球污染并限制[Fe/H] > -0.3，则无需气体稀释且符合低恒星形成效率模型。
...
### [15] [Galaxy-Galaxy Blending in SPHEREx Survey Data](https://arxiv.org/abs/2601.21627)
...
**TL;DR**: 通过模拟和观测数据研究发现，SPHEREx（宇宙历史、再电离时期和冰探测器光谱-光度计）全天空巡天数据中星系-星系重叠（galaxy-galaxy blending）在极限星等19 AB mag时仅影响约0.7%的星系，但在黄极附近深场区域（极限星等22 AB mag）重叠比例升至7-9%，导致深场星系计数被高估10-20%，且对红移范围内光度函数的影响可达50%。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用SPHEREx（宇宙历史、再电离时期和冰探测器光谱-光度计）全天空光谱巡天数据研究星系形成与演化过程中的星系-星系混合（galaxy-galaxy blending）现象及其对星系计数和光度函数分析的潜在影响。

Data: 数据来源于宇宙学模拟生成的模拟星系目录和观测数据。

Method: 采用宇宙学模拟和观测数据生成的模拟星系目录(mock galaxy catalogs)分析SPHEREx数据中的星系-星系混合(galaxy-galaxy blending)效应。

Result: SPHEREx（宇宙历史光谱-光度计、再电离时期和冰探测器）的全天巡天数据中仅有约0.7%的星系会在19 AB星等极限星等下与其他星系发生混合（blending），但在黄道极附近深度达22 AB星等的深场区域该比例显著增至7-9%。
...
### [16] [A Low-mass Model of The Milky Way: The Disk Warp Resulting from A Galaxy Merger](https://arxiv.org/abs/2601.21638)
*Mingji Deng,Cuihua Du,Jian Zhang,Haoyang Liu,Zhongbcheng Li*

**TL;DR**: 构建了新的盖亚-香肠-恩克拉多斯(GSE)并合模型，揭示了星系并合通过暗物质晕(DM halo)不对称引力势导致盘翘曲(disk warp)的双重机制：短期内晕倾角(tilt angle)与翘曲幅度呈反相关（跷跷板角动量交换），长期则因动力学摩擦(dynamical friction)导致整体对齐衰减，并发现高倾角(high-inclination)并合可维持长期顺行进动(prograde precession)。


<details>
  <summary>Details</summary>

Background: 银河系(GSE)并合模型研究揭示了星系并合通过暗物质晕(DM halo)不对称引力势驱动盘翘曲(disk warp)演化的机制。

Data: 使用GIZMO代码模拟了不同轨道参数下的星系合并，以研究盘翘曲（disk warp）在不同条件下的演化。

Method: 采用GIZMO代码模拟不同轨道参数的星系合并，研究盘翘曲（disk warp）演化机制，揭示暗物质晕（DM halo）不对称引力势驱动的双区相互作用机制。

Result: 银河系(GSE)合并模型揭示了暗物质晕(DM halo)不对称引力势普遍导致星系盘翘曲(disk warp)的演化机制，并发现短期存在晕倾角(tilt angle)与盘翘曲幅度的"跷跷板"角动量交换机制，而长期动力学摩擦则使二者同步衰减。
...
### [17] [Little Red Dots and Supermassive Black Hole Seed Formation in Ultralight Dark Matter Halos](https://arxiv.org/abs/2601.21676)
*Dongsu Bak,Jae-Weon Lee*

**TL;DR**: 研究发现超轻暗物质(ULDM)晕中心孤子核通过限制高红移重子气体并驱动快速流入和激波加热，使其在无外部紫外背景条件下突破原子冷却和碎裂抑制阈值(~3×10^4K)，形成特征质量约10^5M⊙的超大质量黑洞(SMBH)种子，同时预测最大质量上限约10^10M⊙，所需ULDM粒子质量m≃10^-22eV与星系尺度观测一致。


<details>
  <summary>Details</summary>

Background: 研究探讨了超大质量黑洞（SMBH）种子如何在早期宇宙中超轻暗物质（ULDM）晕中心形成的关键条件及其与暗物质粒子质量（$m \simeq 10^{-22}{\rm eV}$）的统一关联。

Data: 数据来源于对超轻暗物质（ULDM）晕中心孤子核（solitonic cores）约束下的高红移重子气体直接单块坍缩（direct and monolithic collapse）条件的半解析推导，包括晕质量、孤子质量、重子核心半径及气体热力学状态的半解析关系式，并考虑了重子收缩效应。

Method: 基于超轻暗物质(ULDM)晕的中心孤子核(solitonic core)势场，采用半解析方法推导了晕质量、孤子质量、重子核心半径与气体热力学状态的关系，并考虑了重子收缩效应。

Result: 超大质量黑洞（SMBH）种子在超轻暗物质（ULDM）晕中心通过满足临界温度阈值（~3×10^4 K）的单体坍缩形成，特征质量约为~10^5M⊙，且最大质量上限为~10^10M⊙。
...
### [18] [Detection of Gravitational Anomaly at Low Acceleration from a Highest-quality Sample of 36 Wide Binaries with Accurate 3D Velocities](https://arxiv.org/abs/2601.21728)
*K. -H. Chae,B. -C. Lee,X. Hernandez,V. G. Orlov,D. Lim,D. A. Turnshek,Y. -W. Lee*

**TL;DR**: 利用36颗邻近宽双星（距离<150pc）在低加速度区（10^-11至10^-9 m/s²）的三维运动数据，测得重力增强因子γ=1.600_{-0.141}^{+0.171}（4.9σ偏离牛顿引力），首次以精确三维速度独立证伪牛顿引力在低加速度极限的适用性，支持类MOND（Milgromian dynamics）非标准动力学范式。


<details>
  <summary>Details</summary>

Background: 研究主题为在低加速度范围（$10^{-11},10^{-9}$ m\,s$^{-2}$）内通过孤立宽双星的三维运动精确测量引力（gravity），并检验牛顿引力（Newtonian gravity）在低加速度极限下的适用性。

Data: data部分：数据来源包括盖亚卫星第三次数据释放（Gaia DR3）的宽双星三维运动参数、新观测的径向速度数据以及公开数据库/文献中筛选的36个邻近（距离<150秒差距）低加速度宽双星样本。

Method: 采用三维运动学分析结合盖亚(Gaia) DR3数据和新观测数据，通过精确测量宽双星系统的相对位移($\mathbf{r}$)和速度($\mathbf{v}$)，并控制运动学污染源（如未探测伴星）来检验低加速度区($10^{-11},10^{-9}$ m/s²)的引力理论。

Result: 在低加速度区间（$10^{-11},10^{-9}$ m\,s$^{-2}$）下，通过36个邻近（距离$<150$pc）宽双星系统的三维运动测量，发现重力增强因子$γ=1.600_{-0.141}^{+0.171}$（$Γ=0.102_{-0.021}^{+0.023}$），与标准牛顿引力理论存在$4.9σ$偏离，支持非标准动力学（如MOND理论）的预测。
...
### [19] [AGN Variability with Rubin Observatory in the 2030s](https://arxiv.org/abs/2601.21769)
...
**TL;DR**: 基于ZTF（Zwicky Transient Facility）数据发现活动星系核（AGN）光学变幅和时标主要受吸积状态调控，并证实宽线区（BLR）弥散连续谱辐射导致光学连续谱辐射区尺寸常超出薄盘理论预测，为LSST（Legacy Survey of Space and Time）开展大规模反响映射研究提供了理论基础。


<details>
  <summary>Details</summary>

Background: 活动星系核(AGN)光变研究为吸积物理、盘结构及黑洞增长机制提供了直接探测手段。

Data: 数据来源包括维拉·C·鲁宾天文台时空遗产巡天（LSST）的多波段光变曲线和兹威基瞬变设施（ZTF）的光学变幅与时间尺度测量结果。

Method: 采用多波段光变曲线分析（multi-band light curve analysis）、连续反响映射（continuum reverberation mapping）和标准化变率指标流水线（standardized variability metric pipelines）技术手段。

Result: 光学变幅和时标主要受吸积状态调控，黑洞质量和红移为次要因素，且观测到的连续谱发射区尺寸常超出标准薄盘模型预测，表明宽线区(BLR)的弥散连续辐射对波段间延迟有显著贡献。
...
### [20] [A UV-Luminous Galaxy at z=11 with Surprisingly Weak Star Formation Activity](https://arxiv.org/abs/2601.21833)
...
**TL;DR**: JWST/MIRI观测发现红移(redshift)11.04的星系CEERS2-588具有显著巴尔默跳变(Balmer break)、延展至红移15的恒星形成历史、近期快速淬灭(quenching)及接近太阳的气相金属丰度(gas-phase metallicity)，挑战了早期星系形成理论模型。


<details>
  <summary>Details</summary>

Background: 詹姆斯·韦伯太空望远镜(JWST)在红移$z>10$处发现大量明亮星系，挑战了早期星系形成的理论模型，引发了对极端红移星系物理性质差异的激烈讨论。

Data: 数据来源于詹姆斯·韦伯太空望远镜(JWST)的中红外仪器(MIRI)对高红移(z=11.04)紫外亮星系CEERS2-588的深度观测，包括F560W和F770W波段的成像数据以及中分辨率光谱仪(MRS)的光谱数据。

Method: 采用JWST/MIRI多波段成像和光谱观测（F560W、F770W及MRS光谱）结合光谱能量分布（SED）拟合方法，首次在z>10星系中检测到巴尔默跳变（Balmer break），并约束恒星形成历史与金属丰度。

Result: 在红移(z)11.04的紫外明亮星系CEERS2-588中首次探测到显著的巴尔默跳变(Balmer break)，其恒星形成历史可能延伸至z>15并在最近约10百万年内快速停止，与其他z>10星系形成鲜明对比。
...
### [21] [Orbital decomposition of the nuclear regions in the early-type galaxy FCC 47: Unveiling the nuclear cluster origin](https://arxiv.org/abs/2601.21889)
...
**TL;DR**: 研究发现FCC 47（NGC 1336）星系中心的超大核星团（NSC）具有由热压支撑成分、反向旋转的暖结构和中心反向旋转冷盘组成的复杂轨道结构，支持其通过早期星团吸积和后期原位恒星形成的混合形成机制。


<details>
  <summary>Details</summary>

Background: 核星团(NSCs)作为宇宙中密度最高的恒星系统之一，常与星系中心的超大质量黑洞(SMBHs)共存，其恒星种群和运动学保留了星系演化过程中并合、吸积及内部动力学过程的累积印记。

Data: 数据来源为VLT/MUSE积分场光谱仪（integral-field spectroscopy）提取的恒星运动学数据，特别是视线速度分布（LOSVDs）。

Method: 采用非参数贝叶斯视线速度分布（Bayes-LOSVD）方法从VLT/MUSE积分场光谱中提取恒星运动学数据，并应用DYNAMITE软件进行三轴史瓦西轨道叠加（triaxial Schwarzschild orbit-superposition）建模，将轨道库分解为热、温、冷和反向旋转组分。

Result: 在FCC 47（NGC 1336）的核星团（NSC）中发现了由反向旋转、动力学解耦成分组成的复杂轨道结构，支持其通过早期星团吸积和后期原位恒星形成的混合形成机制。
...
### [22] [Stellar Populations in the Extreme Outer Halo of the Spiral Galaxy M96](https://arxiv.org/abs/2601.21987)
*J. Christopher Mihos,Patrick R. Durrell,Brian Malkan,Aaron E. Watkins*

**TL;DR**: 基于哈勃太空望远镜对M96星系外晕（halo）的观测，发现其金属丰度（[M/H] = -1.36）显著低于同质量螺旋星系预期值，且具有较高恒星晕质量占比（15%），暗示该星系可能经历了独特的吸积历史。


<details>
  <summary>Details</summary>

Background: 研究利用哈勃太空望远镜(Hubble Space Telescope)深度成像探索了位于动力学活跃的狮子座I星系群(Leo I galaxy group)中旋涡星系M96外晕的恒星种群特性。

Data: 数据来源于哈勃空间望远镜（Hubble Space Telescope）对M96星系外晕恒星群的两处50千秒差距投影距离区域的深度成像观测，测光完备性极限为F814W波段28.0星等。

Method: 采用哈勃空间望远镜(Hubble Space Telescope)深度成像技术分析M96星系外晕恒星种群，结合等龄线匹配法(isochrone matching)测定红巨星金属丰度，并运用幂律径向密度分布模型(power-law radial density profile)估算总晕质量。

Result: result：M96星系外晕的红巨星中位金属丰度（metallicity）为[M/H] = -1.36，四分位距为±0.75 dex，其总晕质量（halo mass）为7.8×10⁹ M☉（误差范围+17.4/-4.9），且其金属丰度显著低于相同晕质量旋涡星系的预期值。
...
### [23] [Mapping the Extended Lyman-Alpha Emission within the Circumgalactic Medium of Quasars Hosted by Dusty Starbursts with CubeCarve](https://arxiv.org/abs/2601.22063)
*Kevin Hall,Hai Fu*

**TL;DR**: 研究对象为尘埃星暴宿主类星体(quasars hosted by dusty starbursts)，主要发现是其莱曼α(Lyα)发射比同类光度类星体更暗且分布更平缓，且总莱曼α光度与宿主星系远红外亮度呈负相关，表明这类过渡阶段类星体的星系周介质(CGM)环境存在特殊性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于类星体（quasar）演化过渡阶段中尘埃星暴（dusty starburst）宿主星系周围延展莱曼阿尔法（Lyα）发射的星际介质（CGM）特性差异。

Data: 数据来源于对四颗类星体周围扩展莱曼α（Lyα）发射的观测，使用了双通道去卷积算法CubeCarve处理积分视场单元（IFU）数据集以分离未解析的类星体发射和空间扩展结构。

Method: 采用双通道反卷积算法CubeCarve分离未分辨的类星体发射与空间延展结构，可靠提取延展莱曼α(Lyα)发射。

Result: result：研究发现复合系统（composite systems）的莱曼阿尔法（Lyα）表面亮度轮廓平均比类似热光度（bolometric luminosities）类星体更暗且更平缓，且星云总莱曼阿尔法光度在宿主星系远红外（far-infrared）辐射更强的系统中更低。
...
### [24] [First constraints on causal sources of primordial gravitational waves from BICEP/Keck, SPTpol, SPT-3G, Planck and WMAP $B$-mode data](https://arxiv.org/abs/2601.20958)
*Jessica A. Zebrowski,Aurora Ireland,Christian L. Reichardt,Kylar Greene,Gordan Krnjaic,Yuhsin Tsai,François R. Bouchet*

**TL;DR**: 首次利用宇宙微波背景（CMB）B模偏振测量数据，对早期宇宙中因果性张量（ECT）功率谱振幅参数r_ect（k=0.01 Mpc^-1处张量与标量功率比）设定了95%置信水平上限r_ect<0.0077，约束了包括宇宙相变、小尺度密度扰动增强和拓扑缺陷在内的非暴涨引力波源，并推导出超低频段引力波能量密度上限。


<details>
  <summary>Details</summary>

Background: 研究聚焦于早期宇宙中非暴胀机制产生的引力波（gravitational waves）及其因果性限制的张量功率谱（tensor power spectrum）在低频段的观测约束。

Data: 数据来源于BICEP/Keck、SPTpol、SPT-3G、Planck和WMAP的宇宙微波背景（CMB）B模偏振测量。

Method: 采用宇宙微波背景(CMB) B模偏振测量数据，结合贝叶斯似然分析框架约束早期因果张量(ECT)功率谱振幅参数$r_{ect}$。

Result: 首次利用宇宙微波背景（CMB）B模偏振测量数据对早期因果性张量（ECT）功率谱振幅参数r_{ect}给出了95%置信水平上限r_{ect}<0.0077。
...
### [25] [A Universal CMB $B$-Mode Spectrum from Early Causal Tensor Sources](https://arxiv.org/abs/2601.20967)
*Kylar Greene,Aurora Ireland,Gordan Krnjaic,Yuhsin Tsai*

**TL;DR**: 研究发现早期因果张量源（ECTs）产生的张量扰动在小波数下具有普适的$k^3$标度行为，导致宇宙微波背景（CMB）$B$模角功率谱在小尺度上增强而大尺度上抑制，从而与暴胀$B$模的标度不变分布形成区分。


<details>
  <summary>Details</summary>

Background: 研究探讨了早期宇宙因果性张量扰动（ECTs）在暴胀后产生的普遍标度行为及其在宇宙微波背景（CMB）和随机引力波谱密度中的观测特征。

Data: 数据来源于宇宙微波背景（CMB）的B模式角功率谱观测和随机引力波谱密度（stochastic gravitational wave spectral densities）的低频观测。

Method: 采用统一框架分析早期因果张量源（ECTs）的普适红外标度行为，通过一阶相变、拓扑缺陷和微扰理论二阶张量模增强标量扰动等典型案例验证其在小波数下的k³幂律谱特征。

Result: 早期因果张量源(ECTs)产生的张量扰动在小波数下呈现普适的$k^3$标度行为，导致宇宙微波背景(CMB)中$B$模式角功率谱在小尺度增强、大尺度抑制的独特分布。
...
### [26] [The Lambert $W$ equation of state in light of DESI BAO](https://arxiv.org/abs/2601.20972)
*Vipin Chandra Dubey,Subhajit Saha,Abdulla Al Mamon*

**TL;DR**: 通过结合重子声学振荡（BAO）、Ia型超新星和哈勃参数观测数据，验证了基于兰伯特W函数（Lambert W function）的双参数暗流体状态方程（EoS）模型能一致描述宇宙晚期加速膨胀，其最佳拟合哈勃常数H0=67.4±1.2 km s−1 Mpc−1且与ΛCDM模型存在显著偏差。


<details>
  <summary>Details</summary>

Background: 探索宇宙演化是否可由单一暗流体（dark fluid）的有效状态方程（EoS）描述，该方程结合了对数项和幂律项并涉及朗伯W函数（Lambert W function）。

Data: 数据来源包括重子声学振荡（BAO）数据（含暗能量光谱仪DESI的测量结果）、Pantheon+汇编的Ia型超新星观测数据以及哈勃参数的直接测定值。

Method: 采用包含对数项和幂律项的线性组合形式（均涉及朗伯W函数）的有效状态方程（EoS）模型，通过观测数据拟合确定参数θ₁和θ₂。

Result: 通过结合重子声学振荡（BAO）、Pantheon+超新星数据及哈勃参数直接测量，确定描述宇宙演化的暗流体有效状态方程（EoS）参数θ₁和θ₂的最佳拟合值，得出哈勃常数H₀=67.4±1.2 km s⁻¹ Mpc⁻¹及重子拖曳时期声视界r_d=146±2.5 Mpc。
...
### [27] [Euclid preparation. Decomposing components of the extragalactic background light using multi-band intensity mapping cross-correlations](https://arxiv.org/abs/2601.21111)
...
**TL;DR**: 通过结合多波段强度映射（intensity mapping）、宇宙剪切（cosmic shear）和星系成团性（galaxy clustering）的联合晕模型（halo-model）框架，成功分解了河外背景光（EBL）中星系累积光（IGL）、晕内弥散光（IHL）和再电离时期（EoR）高红移源的贡献，并显著提高了IHL质量依赖比例和EoR恒星形成效率参数的测量精度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用多波段强度映射(intensity mapping)结合宇宙剪切(cosmic shear)和星系成团性(galaxy clustering)分解河外背景光(EBL)中的累积积分星系光(IGL)、弥散晕内光(IHL)以及再电离时期(EoR)的高红移源。

Data: 数据来源包括在平坦ΛCDM宇宙学模型中模拟的10个光谱波段（0.75-5.0μm）的NEP深场巡天数据，覆盖约100平方度，探测极限为AB=20.5用于掩模，并划分了6个红移区间至z=2.5。

Method: 采用联合晕模型（halo-model）框架，结合质量与红移依赖的IHL光度标度、演化Schechter光度函数的IGL以及分档恒星形成效率的EoR发射模型，通过MCMC方法拟合多波段强度映射（intensity mapping）、宇宙剪切（cosmic shear）与星系成团性（galaxy clustering）的自功率谱和交叉功率谱。

Result: 通过联合SPHEREx和Euclid数据分析，显著提高了对星系内晕光（IHL）参数（10-35%）、再电离时期（EoR）恒星形成效率参数（20-35%）以及晕质量函数中IHL比例（5-30%）的约束精度，并将恒星形成率密度限制扩展至$z\sim 11$（不确定性降低22-31%）。
...
### [28] [Realizing the phantom-divide crossing with vector and scalar fields](https://arxiv.org/abs/2601.21274)
*Shinji Tsujikawa*

**TL;DR**: 在广义普罗卡理论(Generalized Proca theories)中引入标量场后，实现了暗能量状态方程(equation of state)在低红移时从$w_{\rm DE} < -1$到$w_{\rm DE} > -1$的穿越，同时避免了理论不一致性，并通过调节矢量场横向模式影响下的纵向标量扰动声速$c_ψ$，使物质扰动增长参数$μ$和光线偏折参数$Σ$接近观测值1。


<details>
  <summary>Details</summary>

Background: 研究探讨了广义普罗卡理论（generalized Proca theories）中通过引入标量场实现暗能量状态方程（dark energy equation of state）穿越幻影界限（phantom-divide crossing）的理论可行性及其宇宙学观测一致性。

Data: 研究中使用的数据来源于广义普罗卡理论（generalized Proca theories）中的矢量场和标量场动力学模拟，并结合了低红移条件下的线性宇宙学扰动演化分析。

Method: 采用广义普罗卡理论（generalized Proca theories）结合规范势破缺的矢量场和带势的正则标量场，构建最小模型实现暗能量状态方程从$w_{\rm DE} < -1$到$w_{\rm DE} > -1$的穿越，并通过准静态近似分析哈勃半径内线性宇宙学扰动演化。

Result: 在广义普罗卡理论（generalized Proca theories）中引入标量场后，模型可实现暗能量状态方程（dark energy equation of state）从$w_{\rm DE} < -1$到$w_{\rm DE} > -1$的幻影界限穿越（phantom-divide crossing），且参数空间存在无鬼场（ghost）和拉普拉斯不稳定性（Laplacian instabilities）的可行区域。
...
### [29] [A redshift survey of the nearby galaxy cluster Abell 2199 : No upturn of the faint-end slope of galaxy luminosity function](https://arxiv.org/abs/2601.21329)
*Jong-In Park,Hyunmi Song,Ho Seong Hwang*

**TL;DR**: 通过增强光谱数据测定了近邻星系团Abell 2199（A2199）的星系光度函数，发现其暗端斜率（α= -1.23 ± 0.05）与Coma、Virgo星系团及TNG50模拟结果一致但比场星系略平缓，不支持此前提出的暗端陡峭上升（α∼ -2）观点，表明致密星系团核心环境不会促进低质量星系形成或存活。


<details>
  <summary>Details</summary>

Background: 研究聚焦于邻近星系团阿贝尔2199（Abell 2199）中暗弱端星系光度函数（luminosity function）的斜率特征，旨在揭示稠密星系团核心环境下低质量星系的形成或存活机制。

Data: 数据来源包括深度MMT/Hectospec巡天和暗能量光谱仪（DESI）的红移数据，显著提高了中心30角分区域内r波段测光星等（r_petro,0）至20.8的光谱完备性。

Method: 采用深度MMT/Hectospec巡天和暗能量光谱仪(DESI)的光谱数据，显著提升了中心30角分区域内至$r_{\mathrm{petro},0} = 20.8$的光谱完备性，并基于Schechter函数拟合得到光度函数。

Result: 在阿贝尔2199星系团（Abell 2199）中测得的暗端斜率（α = -1.23 ± 0.05）与邻近的Coma和Virgo星系团以及TNG50宇宙学模拟中的星系团一致，但比场星系略平缓，表明密集星系团核心的环境过程并未促进低质量星系形成或存活，从而不支持此前声称的陡峭暗端上升（α ∼ -2）。
...
### [30] [Cross-correlating galaxies and cosmic dispersion measures: Constraints on the gas-to-halo mass relation from 2MASS galaxies and 133 localized fast radio bursts](https://arxiv.org/abs/2601.21336)
*Masato Shirasaki,Ryuichi Takahashi,Ken Osato,Kunihito Ioka*

**TL;DR**: 通过2MASS星系与快速射电暴(FRB)的交叉相关分析发现，质量为10^12-13 M⊙的星系晕中热气体质量占比低于TNG300模拟预测值（需低于宇宙重子占比的10%），表明该质量区间需要更强的反馈机制。


<details>
  <summary>Details</summary>

Background: 该研究探讨了宇宙大尺度结构（large-scale structure）与快速射电暴（fast radio bursts, FRBs）色散测量之间的关联，旨在通过交叉相关分析约束星系晕（halo）中热气体质量分数（hot-gas mass fraction）及其对星系形成反馈过程（feedback processes）的影响。

Data: 数据来源于2MASS星系巡天（Two Micron All Sky Survey）的星系目录和133个已定位快速射电暴（fast radio bursts, FRBs）的宇宙色散测量（cosmic dispersion measures），并利用了两组数据的红移信息。

Method: 采用基于晕模型（halo-based model）的分析方法，利用流体动力学模拟IllustrisTNG-300（TNG300）构建自由电子密度分布模型。

Result: 在质量范围为$10^{12-13}\, M_\odot$的暗物质晕（halo）中，热气体质量分数（hot-gas mass fraction）必须低于TNG300模拟预测值，且不超过宇宙重子分数（global baryon fraction）的约10%。
...
### [31] [Cosmological analysis of the DESI DR1 Lyman alpha 1D power spectrum](https://arxiv.org/abs/2601.21432)
...
**TL;DR**: 基于暗能量光谱仪（DESI）一维莱曼α（Lyman-α）通量功率谱的宇宙学分析，结合流体动力学模拟仿真器，精确约束了红移z=3处线性物质功率谱的振幅（Δ²⋆=0.379±0.032）和对数斜率（n⋆=-2.309±0.019），并通过联合其他观测数据显著提升了相对论性粒子有效种类数（Neff=3.02±0.10）、谱指数跑动（αs=0.0014±0.0041）及其二阶跑动（βs=−0.0006±0.0048）的测量精度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用暗能量光谱仪（DESI）的首批数据，通过莱曼-α（Lyman-α）通量一维功率谱的宇宙学分析，探索宇宙大尺度结构形成、星系际介质（IGM）物理及ΛCDM（Lambda Cold Dark Matter）模型的扩展参数。

Data: 数据来源于暗能量光谱仪（DESI）首次数据发布的一维莱曼-α（Lyman-$α$）流量功率谱，并结合了基于流体动力学模拟（hydrodynamical simulations）的仿真器（emulator）进行建模。

Method: 采用基于流体动力学模拟(cosmological suite of hydrodynamical simulations)训练的模拟器(emulator)分析莱曼α(Lyman-α)通量功率谱的宇宙学信号，并修正天体物理污染物和系统误差的影响。

Result: 通过结合DESI BAO和普朗克（Planck）、ACT及SPT-3G的温度、偏振和透镜测量数据，对ΛCDM扩展模型的有效相对论性粒子数（N_eff=3.02±0.10）、谱指数运行（α_s=0.0014±0.0041）及其二阶运行（β_s=-0.0006±0.0048）的约束分别提高了1.18、1.27和1.90倍。
...
### [32] [Forecasting Constraints on Cosmology and Modified Gravitational-wave Propagation by Strongly Lensed Gravitational Waves Associating with Galaxy Surveys](https://arxiv.org/abs/2601.21820)
*Anson Chen,Jun Zhang*

**TL;DR**: 首次利用双透镜引力波事件开展时间延迟宇宙学研究，预测不同探测器网络（LVK O5、LVK升级后、ET+CE）对哈勃常数（$H_0$）的测量精度分别为14%、10%和0.42%，并联合标准汽笛法约束了动态暗能量模型参数（$w_0=-1.02^{+0.31}_{-0.22}$，$w_a=0.48^{+0.99}_{-1.54}$）及修正引力波传播效应。


<details>
  <summary>Details</summary>

Background: 引力波（GW）的引力透镜效应将成为研究宇宙学和引力理论的新前沿。

Data: 数据来源包括模拟的双透镜引力波事件，分别基于LIGO-Virgo-KAGRA（LVK）O5网络、LIGO O5升级后网络以及爱因斯坦望远镜（ET）+宇宙探索者（CE）的预期灵敏度生成，并筛选出能与未来星系巡天关联的事件。

Method: 采用奇异等温球(Singular Isothermal Sphere, SIS)透镜模型联合估计双透镜引力波信号的波形参数和冲击参数，并结合时间延迟宇宙学与标准汽笛法预测宇宙学参数和修正引力波传播效应的约束。

Result: 首次分析双透镜引力波（GW）事件的时间延迟宇宙学应用，预测LVK O5、LVK升级后及ET+CE网络分别可观测到0.2、2.4和73.2个符合条件的双透镜事件。
...
### [33] [A null test of the Hubble tension](https://arxiv.org/abs/2601.22043)
*Gerasimos Kouniatalis,Emmanuel N. Saridakis*

**TL;DR**: 提出了一种结合强透镜时间延迟距离和引力波标准汽笛光度距离的几何零检验方法，通过构建仅依赖于膨胀率红移演化的无量纲比值，可明确区分哈勃张力（Hubble tension）的早期或晚期宇宙起源。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于哈勃张力（Hubble tension）这一现代宇宙学核心问题，通过构建独立于哈勃常数（H_0）的几何零检验（geometric null test）来探究宇宙晚期膨胀历史的可能修正机制。

Data: 数据来源包括强引力透镜时间延迟距离（strong-lensing time-delay distances）和引力波标准汽笛光度距离（gravitational-wave standard-siren luminosity distances）。

Method: 采用强引力透镜时间延迟距离与引力波标准汽笛光度距离构建的无量纲比值作为纯几何零检验（null test），独立于哈勃常数（Hubble constant）且仅依赖于膨胀率（expansion rate）的红移演化。

Result: 通过结合强透镜时间延迟距离和引力波标准汽笛光度距离构建的无量纲比值，可直接测试晚期宇宙膨胀历史与早期宇宙预测的一致性，从而明确区分哈勃张力（Hubble tension）的早期或晚期起源。
...
### [34] [Probing the Sound Speed of Dark Energy with a Lunar Laser Interferometer](https://arxiv.org/abs/2601.22084)
*Alfredo Gurrola,Robert J. Scherrer,Oem Trivedi*

**TL;DR**: 提出利用月球激光干涉仪（LILA）首次直接探测暗能量（dark energy）声速，通过测量地平线尺度引力势实时演化，以空前灵敏度约束或排除暗能量模型类别。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用月基激光干涉仪（如LILA）通过测量暗能量（dark energy）声速来揭示宇宙加速膨胀的微观物理机制。

Data: 数据来源于月球激光干涉仪（LILA）对超低频引力波带的观测，通过测量由暗能量动力学引起的标量度规扰动来获取。

Method: 采用流体力学和有效场论（effective field theory）描述框架，将暗能量声速（sound speed of dark energy）与可观测应变信号关联，并构建似然分析流程和费希尔预报（Fisher forecasts）。

Result: 月球激光干涉仪（LILA）可通过测量地平线尺度引力势的实时演化直接探测暗能量（dark energy）的声速（sound speed），从而以前所未有的灵敏度检测或排除广泛类别的暗能量模型。
...
### [35] [The power of polarimetry for characterising exoplanet atmospheres, clouds, and surfaces with NASA's Habitable Worlds Observatory](https://arxiv.org/abs/2601.20902)
*Katy L. Chubb,Mei Ting Mak,Joanna K. Barstow,Beth Biller,Sarah Rugheimer,Daphne M. Stam,Victor Trees*

**TL;DR**: 论文核心贡献是提出在宜居世界观测站(Habitable Worlds Observatory, HWO)中集成偏振测量(spectropolarimetry)能力，通过分析反射光偏振特性破解云层和表面特征的反演难题，显著提升对类地系外行星大气成分和宜居性的评估能力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用宜居世界观测站(Habitable Worlds Observatory, HWO)通过高对比度反射光谱和光谱偏振测量技术，突破性地探测类地系外行星大气、表面特征及云层性质，以评估行星宜居性。

Data: 研究计划使用HWO（Habitable Worlds Observatory）搭载的日冕仪、高分辨率成像仪和高分辨率分光偏振仪（high-resolution spectropolarimeter）进行多波段（紫外、光学和近红外）高对比度反射光谱与偏振测量。

Method: 采用高对比度反射光谱（high-contrast reflection spectroscopy）和光谱偏振测量（spectropolarimetry）技术，结合日冕仪（coronagraph）、高分辨率成像仪和高分辨率光谱偏振仪（high-resolution spectropolarimeter）进行多波段（紫外、光学、近红外）观测。

Result: HWO（Habitable Worlds Observatory）将首次通过紫外、光学和近红外波段的高对比度反射光谱（reflection spectroscopy）实现对类地系外行星大气和表面的详细研究，为评估行星宜居性提供关键分子吸收特征（O2、O3、H2O、CO2、CH4）及潜在表面生物特征（如植被红边或海洋闪光）的观测能力。
...
### [36] [Teareduce: a Python package with utilities for teaching reduction techniques in Astronomy](https://arxiv.org/abs/2601.20914)
...
**TL;DR**: 开发了Python包teareduce用于支持天文数据减除（reduction）教学，提供处理多种仪器和望远镜天文图像的实践课程材料。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发用于天文数据减除（data reduction）教学的开源工具（teareduce），以支持天体物理学硕士课程中的实验技术教学实践。

Data: 数据来源于使用不同仪器和望远镜获取的天文图像（astronomical images）。

Method: 开发了基于Python的teareduce软件包，用于天文数据（astronomical data）处理教学，提供各类仪器和望远镜获取图像的实践课程支持。

Result: 开发了Python包teareduce以支持天文数据处理的实践教学。
...
### [37] [DESHIMA 2.0: A 200-400 GHz Ultra-wideband Integrated Superconducting Spectrometer](https://arxiv.org/abs/2601.21603)
...
**TL;DR**: DESHIMA 2.0（基于超导微带滤波器和漏波天线）在实验室环境下实现了200-400 GHz瞬时频率覆盖、平均滤波器Q值340±50、8%仪器效率（比前代提升4倍带宽和灵敏度），并验证了新型绝对频率校准方法的有效性。


<details>
  <summary>Details</summary>

Background: DESHIMA 2.0的研究聚焦于开发基于动态电感探测器（KIDs）的宽带集成超导光谱仪（ISS），用于毫米波/亚毫米波天文观测。

Data: 数据来源于实验室环境下设计的实验装置，包括容纳低温光学元件和集成超导光谱仪（ISS）芯片的低温恒温器、339个连接至微带（MS）滤波器的动力学电感探测器（KIDs）、安装在六足平台上的室温反射镜以及电机控制平台上用于精确位置和对准的天空位置斩波器。

Method: 采用基于动力学电感探测器(KIDs)的超宽带集成超导光谱仪(ISS)技术，结合低损耗氢化非晶碳化硅(a-SiC:H)介质铌钛氮(NbTiN)超导微带(MS)滤波器和超宽带漏波天线，在实验室环境下对DESHIMA 2.0进行性能表征。

Result: DESHIMA 2.0在实验室环境下实现了200-400 GHz的瞬时频率覆盖范围，平均滤波器品质因数(Q_filter)为340±50，仪器效率达到约8%，频带覆盖范围和灵敏度分别比前代提升了4倍。
...
### [38] [Critical Evaluation of Studies Alleging Evidence for Technosignatures in the POSS1-E Photographic Plates](https://arxiv.org/abs/2601.21946)
*Wesley Andrés Watters,Laura Dominé,Sarah Little,Cameron Pratt,Kevin H. Knuth*

**TL;DR**: 该研究通过分析POSS1（Palomar Observatory Sky Survey）底片扫描数据，质疑了此前关于地球阴影内特征缺失、线性星群存在及特征观测时间与核试验/UAP（Unidentified Aerial Phenomena）相关性的三项主张，指出数据集中包含恒星目录、扫描伪影和底片缺陷，且特征空间分布呈现边缘密度递增、几何形状集群等系统性模式，未证实存在光学瞬变体（optical transients）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于分析前苏联卫星时代帕洛玛天文台第一期巡天(POSS1)照相底片数字扫描数据中未识别特征的起源及其与核试验、不明空中现象(UAP)的潜在关联。

Data: 数据来源于对第一代帕洛玛天文台巡天（POSS1）在苏联卫星发射前时代拍摄的照相底片数字化扫描中未识别特征的测量和分析。

Method: 采用数字扫描技术分析第一代帕洛玛天文台巡天(POSS1)照相底片，并运用统计方法检验特征分布与核试验及不明空中现象(UAP)报告的时间相关性。

Result: 在POSS1（Palomar Observatory Sky Survey）数字化扫描图像中，未识别特征的空间分布呈现整体向底片边缘和角落逐渐增加的密度趋势，并存在跨越多底片的南北向空白条带、几何形状的聚集与空洞区以及无定形聚集等模式。
...
### [39] [Progress towards a 3D Monte Carlo radiative transfer code for outflow wind modelling: II. 3-D applications](https://arxiv.org/abs/2601.21782)
*Jakub Fišák,Jiří Kubát,Nicolas Moens,Brankica Kubátová*

**TL;DR**: 开发了蒙特卡洛辐射传输代码Andy Antares的新版本，通过改进任意几何结构下速度场的处理（velocity field）实现了三维辐射传输（radiative transfer）的精确计算，适用于非对称恒星风（stellar winds）等复杂流体动力学模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量热星（massive hot stars）的恒星风（stellar winds）非对称现象及其对星系动力学的关键影响，强调超越一维模型的理论研究需求。

Data: 研究中使用的数据来源于预计算的流体动力学模型（hydrodynamical model）生成的输入模型，并在笛卡尔网格（Cartesian grid）中进行处理。

Method: 采用改进的蒙特卡洛辐射传输方法（Monte Carlo radiative transfer）处理任意几何结构的速度场，基于离散点定义通用速度场并通过三线性插值（trilinear interpolation）实现数值积分求解光学深度。

Result: 开发了可处理任意几何速度场的蒙特卡洛辐射传输代码Andy Antares新版本，适用于三维任意速度场下的辐射传输数值求解。
...
### [40] [A rotation-based census of blue lurker candidates in open clusters](https://arxiv.org/abs/2601.21059)
*Vikrant V. Jadhav,Khushboo K. Rao,Elisabetta Reggiani*

**TL;DR**: 该研究通过分析开普勒（Kepler）、TESS及光谱数据在35个疏散星团中发现了97个新的蓝潜伏者（blue lurkers, BLs）候选体，结合自转速度和伴星特征表明其可能通过吸积或质量转移形成，并推测星团中可能存在数千个此类天体。


<details>
  <summary>Details</summary>

Background: 研究主题为星团中通过质量转移或合并形成的蓝潜伏者（Blue lurkers, BLs）及其在颜色-星等图上的分布特征。

Data: 数据来源包括开普勒(Kepler)、TESS卫星观测数据以及光谱学自转指标（如自转周期和$v\sin i$测量值）。

Method: 通过分析开普勒(Kepler)、TESS及光谱学旋转指标（如自转周期和$v\sin i$测量值）识别主序带快速旋转恒星。

Result: 在35个疏散星团中发现了97个新的蓝潜伏者（BL）候选体，使已知样本数量几乎增加了两倍，基于约3%的完备性估计，推测星团中可能隐藏着数千个BL。
...
### [41] [Evaluating Classifications of Extremely Metal-poor Candidates Selected from Gaia XP Spectra](https://arxiv.org/abs/2601.21292)
*Riley Thai,Andrew R. Casey,Alexander Ji,Vedant Chandra,Hans-Walter Rix*

**TL;DR**: 通过高分辨率光谱验证了盖亚(Gaia) XP低分辨率光谱在准确分类极贫金属(extremely metal-poor)和甚贫金属(very metal-poor)恒星方面的有效性，并新发现2颗极贫金属星和20颗甚贫金属星，其化学动力学性质与晕族恒星预期基本一致，但发现了一颗镁增强的碳增强贫金属星(CEMP)和一颗来自吸积超暗弱矮星系的贫镁星。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用盖亚(Gaia)卫星XP低分辨率光谱准确分类和验证极端贫金属(extremely metal-poor)与非常贫金属(very metal-poor)恒星的可行性及其在银河系化学演化研究中的应用。

Data: 数据来源于盖亚(Gaia)卫星XP低分辨率光谱和后续高分辨率光谱观测，其中75颗极贫金属(metal-poor)候选体选自R. Andrae、V. Chandra和H. W. Rix的星表。

Method: 采用高分辨率光谱分析（high-resolution spectroscopy）结合一维局部热力学平衡（1D local thermodynamic equilibrium）模型测定元素丰度，并整合盖亚天体测量（Gaia astrometry）与光谱径向速度（spectroscopic radial velocities）计算运动学参数。

Result: 发现了2颗新的极贫金属星（[Fe/H]<-3）和20颗新的甚贫金属星（[Fe/H]<-2），并证实盖亚XP光谱在识别银河系最贫金属星方面的有效性。
...
### [42] [MgAl burning chain in M 54: the globular cluster-like properties of a nuclear star cluster](https://arxiv.org/abs/2601.21440)
*D. A. Alvarez Garay,A. Mucciarelli,P. Ventura,M. Bellazzini,S. Covella*

**TL;DR**: 通过分析M 54球状星团233颗成员星的化学丰度，发现其存在显著金属丰度([Fe/H])弥散(-1.8至-1.0 dex)和与MgAl燃烧循环相关的反相关模式，表明该星团可能由两个不同金属丰度(贫金属和富金属)且化学异常程度不同的球状星团并合形成，其中富金属成分可能更年轻并具有更强氢燃烧特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于球状星团M 54的恒星化学丰度特征及其对核星团化学演化的启示。

Data: 数据来源于使用VLT（甚大望远镜）上安装的FLAMES（光纤大阵列多元素光谱仪）高分辨率多目标光谱仪观测的233颗M 54可能成员星的化学丰度数据。

Method: 采用FLAMES高分辨率多目标光谱仪观测并结合马尔可夫链蒙特卡洛（Markov Chain Monte Carlo）分析，确认了M 54中存在的非零本征铁弥散（intrinsic iron dispersion）及与MgAl燃烧循环相关的反相关模式。

Result: result：M 54的化学丰度特征支持其由两个球状星团（一个贫金属且具有标准特性，另一个富金属且化学异常更显著且可能更年轻）合并形成的假说。
...
### [43] [Detection of hot subdwarf binaries and sdB stars using machine learning methods and a large sample of Gaia XP spectra](https://arxiv.org/abs/2601.21727)
...
**TL;DR**: 基于盖亚(Gaia) XP光谱对约20000颗热亚矮星(hot sds)候选体进行降维和机器学习分析，发现双星比例超过60%且变异性与双星性高度相关，证实双星性和环境密度显著影响热亚矮星演化路径，同时识别出主序星和激变变星(cataclysmic variable stars)对样本的污染。


<details>
  <summary>Details</summary>

Background: 研究聚焦于热亚矮星（hot sds）作为极端水平分支（EHB）附近致密演化恒星的关键角色，及其对理解恒星演化和星系紫外超量的重要意义。

Data: 数据来源包括盖亚(Gaia) XP光谱数据和文献中已发表的热亚矮星候选体参数，其中约20000个候选体通过虚拟天文台光谱能量分布分析器(VOSA)进行双星识别。

Method: 采用均匀流形逼近与投影（UMAP）、自组织映射（SOMs）、卷积神经网络（CNNs）及非对称最小二乘基线拟合对盖亚XP光谱系数进行降维、分类和归一化处理。

Result: 热亚矮星（hot sds）的演化路径和紫外超量现象显著受双星性和环境密度影响，并在基础样本中发现可能存在主序星和激变变星的污染。
...
### [44] [BE Lyncis is not a Black Hole Binary: Lessons From Gaia and Hipparcos Astrometry](https://arxiv.org/abs/2601.22071)
*Pranav Nagarajan,Kareem El-Badry,Thomas J. Maccarone,Giuliano Iorio,Sara Rastello,Johanna Müller-Horn*

**TL;DR**: 通过分析依巴谷(Hipparcos)和盖亚(Gaia)天体测量数据，发现BE Lyn（天琴座δ型变星）不可能是拥有$\gtrsim 17.5\,M_{\odot}$黑洞(BH)的高偏心率($e \approx 0.9989$)双星系统，因为观测到的自行异常(PMA)和盖亚DR3 RUWE值均与黑洞伴星预测值严重不符，且该轨道构型会导致主星在近星点溢出洛希瓣(Roche lobe)，表明先前报道的脉冲到达时间(LTTE)模型存在误判，实际时序变化更可能源于红噪声或脉动相位演化。


<details>
  <summary>Details</summary>

Background: 该研究探讨了高振幅盾牌座δ型变星（HADS）BE Lyncis的轨道动力学特征及其潜在黑洞伴星存在的可能性。

Data: 数据来源为依巴谷卫星（Hipparcos）和盖亚任务（Gaia）的天体测量数据。

Method: 基于Hipparcos和盖亚(Gaia)天体测量数据，通过分析自行运动异常(PMA)和盖亚DR3 RUWE值，结合罗希瓣(Roche lobe)动力学约束，验证了双星系统中存在大质量黑洞(BH)的物理可能性。

Result: 基于天体测量学分析，BE Lyncis的观测数据（包括自行运动异常值1.7±0.8 mas/yr、盖亚DR3 RUWE值1.073以及未出现在非单星目录中）与存在≥17.5倍太阳质量黑洞伴星的预测结果存在数量级差异，表明其实际为低质量伴星或单星系统。
...
### [45] [Data-Driven Generation of Neutron Star Equations of State Using Variational Autoencoders](https://arxiv.org/abs/2601.21231)
*Alex Ross,Tianqi Zhao,Sanjay Reddy*

**TL;DR**: 基于结构化变分自编码器（VAE）框架开发机器学习模型，实现了中子星（NS）状态方程（EOS）的高保真重建与生成，通过包含最大质量（M_max）和典型半径（R_1.4）的二维潜在空间及一个随机变量控制壳核转变区EOS特征，解码重建误差低至0.15%。


<details>
  <summary>Details</summary>

Background: 研究主题为利用结构化变分自编码器（VAE）框架重建和生成中子星（NS）状态方程（EOS），以解决EOS建模中的高维数据降维和生成问题。

Data: 数据来源为基于Skyrme状态方程(EOS)数据集训练的变分自编码器(VAE)模型。

Method: 采用基于结构化变分自编码器（VAE）框架的机器学习模型，结合编码器网络降维映射和解码器网络重构中子星状态方程（EOS），并在潜在空间中嵌入监督学习中子星可观测量及随机变量以学习未指定EOS特征。

Result: 基于结构化变分自编码器(VAE)框架开发的机器学习模型能够通过包含最大质量$(M_{\max})$和典型半径$(R_{1.4})$两个监督中子星(NS)可观测量及一个控制壳核转变区域状态方程(EOS)的潜变量，高精度重建Skyrme状态方程，解码器重建EOS导出的$(M_{\max})$和$(R_{1.4})$平均绝对百分比误差约为$(0.15\%)$。
...
### [46] [Towards precision astrometry of scattered images of compact radio sources: scintillometry theory and prospects](https://arxiv.org/abs/2601.21041)
*Dylan L. Jow,Delon Shen*

**TL;DR**: 这篇论文的核心贡献是提出了一种基于瞬时空间波场（instantaneous spatial wavefield）的新理论框架，用于从快速射电暴（FRB）和脉冲星的星际介质（ISM）闪烁观测中提取完整的天体测量信息（astrometric information），并探讨了该框架在解决双屏幕散射测量关键简并性和揭示星系际介质（CGM）小尺度物理特性方面的潜力。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用星际介质(ISM)和星系外电离介质（如环星系介质CGM）对致密射电源（如脉冲星和快速射电暴FRB）的闪烁现象进行天体测量学分析，旨在通过闪烁观测提取天体位置、散射屏距离等关键参数。

Data: 数据来源为射电望远镜观测的脉冲星和快速射电暴（FRB）闪烁动态谱（dynamic spectrum）以及模拟的瞬时空间波场（instantaneous spatial wavefield）。

Method: 采用的核心分析方法为基于瞬时空间波场（instantaneous spatial wavefield）的理论框架，将闪烁观测的完整天体测量信息与动态谱（dynamic spectrum）等传统概念关联。

Result: result：研究发现瞬时空间波场（instantaneous spatial wavefield）包含了闪烁观测的全部天体测量信息，并建立了其与动态谱（dynamic spectrum）等传统概念的关联，为快速射电暴（FRB）闪烁测量学实现完整透镜几何重构提供了理论框架。
...
### [47] [NOCTURNE. I. The radio spectrum of narrow-line Seyfert 1 galaxies](https://arxiv.org/abs/2601.21099)
...
**TL;DR**: 研究发现窄线赛弗特1型星系(NLS1)的射电谱通常由低功率外流或核周恒星形成主导的光学薄辐射为主，相对论性喷流贡献有限，并新发现两个候选喷流源。


<details>
  <summary>Details</summary>

Background: 活动星系核(AGN)射电辐射起源机制存在争议，涉及相对论性喷流(relativistic jets)、喷流基部、外流、恒星形成和热冕同步辐射等多种物理过程。

Data: 数据来源于使用卡尔·G·詹斯基甚大天线阵（JVLA）对50个窄线塞弗特1型星系（NLS1）进行的观测，其中20个目标进行了两次观测。

Method: 采用卡尔·G·扬斯基甚大阵（JVLA）对50个窄线赛弗特1型星系（NLS1）进行射电观测，其中20个源进行了两次观测，并通过幂律模型拟合陡峭谱特征。

Result: 在窄线塞弗特1型星系（NLS1）中，射电谱通常由光学薄辐射主导，可能源于低功率外流或核周恒星形成，而相对论性喷流的贡献有限。
...
### [48] [TeV Gamma-Rays from the Low-Luminosity Active Galactic Nucleus NGC 4278: Implications for the Diffuse Neutrino Background](https://arxiv.org/abs/2601.21411)
*Chengchao Yuan,Ruo-Yu Liu*

**TL;DR**: 通过建模NGC 4278低光度活动星系核(LLAGN)在TeV高低态下的多波段辐射，发现其能谱可由中等相对论性喷流(AGN jets)的单区轻子过程或亚相对论性风(winds)的轻子-强子混合过程解释，且状态转变可能与吸积率增强及喷流/风膨胀共同作用相关，同时提出未来MeV和甚高能γ射线观测可区分这两种机制，并指出修正本地密度后轻子-强子风模型可解释PeV中微子背景。


<details>
  <summary>Details</summary>

Background: 研究探讨了LHAASO探测到的来自低光度活动星系核(LLAGN)宿主星系NGC 4278的TeV辐射起源问题。

Data: 数据来源于LHAASO（Large High Altitude Air Shower Observatory）对NGC 4278的TeV辐射观测数据，并结合X射线和GeV波段数据，用于分析低光度活动星系核（LLAGN）在TeV低态（准静态）和高态（活跃态）的辐射特性。

Method: 采用单区轻子（leptonic）喷流模型或轻子-强子（lepto-hadronic）风模型对X射线、GeV和TeV波段的光谱能量分布进行拟合。

Result: NGC 4278的TeV辐射可能源于中等相对论性喷流的单区轻子辐射或亚相对论性风的轻子-强子辐射，状态转变由吸积率增强及喷流或风扩张共同驱动。
...
### [49] [Balancing Variety and Sample Size: Optimal Parameter Sampling for Ariel Target Selection](https://arxiv.org/abs/2601.21020)
*Emilie Panek,Alexander Roman,Katia Matcheva,Konstantin T. Matchev,Nicolas B. Cowan*

**TL;DR**: 该论文针对阿里尔(Ariel)任务中系外行星(Exoplanet)观测目标选择问题，提出了基于杠杆值(leverage)的优化策略，在样本多样性与观测数量之间实现了最佳平衡。


<details>
  <summary>Details</summary>

Background: 研究聚焦于在有限观测时间约束下优化系外行星(Exoplanet)目标选择策略，以最大化科学产出，特别针对欧空局(ESA)即将开展的Ariel任务中Tier 2凌星光谱观测(Tier 2 transit spectroscopy)的选源问题。

Data: 研究使用了基于欧洲空间局（ESA）即将发射的Ariel任务（Ariel mission）的Tier 2凌星光谱（transit spectroscopy）观测数据，通过定义样本量（sample size）、方差（variance）和杠杆值（leverage）三个选择标准，并转化为与优化算法兼容的目标函数，测试了五种启发式方法（leverage greedy、simulated annealing、K-means clustering、regular classes和quantile classes）在多样性参数下的性能。

Method: 采用杠杆贪婪（leverage greedy）、模拟退火（simulated annealing）、K均值聚类（K-means clustering）、常规分类（regular classes）和分位数分类（quantile classes）五种启发式算法优化样本杠杆（sample leverage），并通过单参数、双参数和三参数多样性场景验证其性能。

Result: 基于杠杆(leverage)的系外行星目标选择策略在任务限制下最有效地平衡了样本多样性与样本规模。
...
### [50] [A Comprehensive Network for the Discovery and Characterization of Interstellar Objects](https://arxiv.org/abs/2601.21184)
*Oem Trivedi,Abraham Loeb*

**TL;DR**: 提出综合星际天体网络（CISON）框架，通过协同观测策略和洛布标度（Loeb Scale）分类方法，将星际天体（ISO）研究转变为可预测、可扩展的成熟学科，提升科学回报并支持行星防御。


<details>
  <summary>Details</summary>

Background: 星际天体（ISO）天文学在过去十年中迅速崛起成为行星天体物理学的新前沿领域。

Data: data来源包括双半球广域巡天发现数据、高分辨率快速表征数据以及拦截任务选择性升级数据。

Method: 采用双半球广域发现（wide-field discovery）、快速高分辨率表征（high resolution characterization）及选择性升级拦截任务（interceptor missions）的协同架构，结合洛布尺度（Loeb Scale）微分公式实现预测性分类与风险评估。

Result: 提出的综合星际天体网络（CISON）通过双半球广域发现与快速高分辨率表征相结合，将星际天体（ISO）研究转变为可预测分类和风险评估的成熟学科。
...
### [51] [PERTURB-c: Correlation Aware Perturbation Explainability for Regression Techniques to Understand Retrieval Black-boxes](https://arxiv.org/abs/2601.21685)
*Jools D. Clarke,Gordon Yip,Nikolaos Nikolaou*

**TL;DR**: 提出了PERTURB-c框架（一种考虑相关性的黑盒回归模型解释方法），通过利用已知光谱相关性解决传统方法（如SHAP）计算复杂度高且难以处理输入间交互作用的问题，并在系外行星WASP-107b透射光谱反演案例中验证了其有效性。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用机器学习方法解析系外行星（exoplanet）大气特征时面临的模型可解释性挑战，旨在通过开发相关性感知框架提升黑盒回归模型的物理合理性验证效率。

Data: 研究中使用的数据来源为基于机器学习系外行星WASP-107b透射光谱反演的模拟案例研究数据。

Method: 采用基于已知光谱相关性（spectral correlation）的PERTURB-c框架，通过热图可视化分析一维结构化输入的黑盒回归模型预测机制。

Result: 提出了一种基于光谱相关性认知的PERTURB-c框架，用于解释一维结构化输入的黑盒回归模型预测机制，并通过热力图可视化增强可解释性。
...
### [52] [Nucleus and Postperihelion Activity of Interstellar Object 3I/ATLAS Observed by Hubble Space Telescope](https://arxiv.org/abs/2601.21569)
*Man-To Hui,David Jewitt,Max J. Mutchler,Jessica Agarwal,Yoonyoung Kim*

**TL;DR**: 通过哈勃太空望远镜(HST)观测首次成功提取星际天体3I/ATLAS核(interstellar object nucleus)，测得其V波段几何反照率(geometric albedo)与物理截面积乘积为0.22±0.07 km²，推定有效半径1.3±0.2 km，并发现其具有显著的对冲效应(opposition surge)和活动不对称性(activity asymmetry)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际天体3I/ATLAS的核探测及其物理特性分析，拓展了太阳系外小天体的观测认知。

Data: 数据来源于哈勃太空望远镜（HST）在2025年12月至2026年1月期间的观测。

Method: 采用哈勃太空望远镜（HST）观测数据，应用核提取技术（nucleus extraction technique）分析星际天体3I/ATLAS的核。

Result: 成功探测到星际天体3I/ATLAS的核，通过哈勃太空望远镜(Hubble Space Telescope)观测并应用核提取技术获得其V波段几何反照率(geometric albedo)与物理截面积乘积为$0.22 \pm 0.07$ km$^{2}$。
...
### [53] [The Volatile Inventory of 3I/ATLAS as seen with JWST/MIRI](https://arxiv.org/abs/2601.22034)
*Matthew Belyakov,Ian Wong,Bryce T. Bolin,M. Ryleigh Davis,Steven J. Bromley,Carey M. Lisse,Michael E. Brown*

**TL;DR**: 首次利用JWST/MIRI中分辨率光谱仪在5-28微米波段对星际天体3I/ATLAS进行中红外光谱观测，检测到水、二氧化碳、镍原子和首次直接发现的甲烷等气体荧光特征，揭示其挥发性物质产生率显著下降且具有异常高的CO₂:H₂O比例和富集CH₄特征。


<details>
  <summary>Details</summary>

Background: 首次利用中红外光谱技术对星际天体（interstellar object）3I/ATLAS进行挥发性物质成分及活动特征研究。

Data: 数据来源于2025年12月15-16日和27日使用JWST/MIRI中分辨率光谱仪对3I/ATLAS进行的后近日点观测，当时目标分别位于2.20和2.54天文单位的日心距离。

Method: 采用JWST/MIRI中分辨率光谱仪获取3I/ATLAS的中红外(5--28微米)光谱数据，通过分析水(H2O)、二氧化碳(CO2)、甲烷(CH4)和镍(Ni)的荧光特征及挥发物生成速率变化，结合近核彗发(coma)映射技术，揭示挥发物分布及丰度比。

Result: 首次在星际天体(interstellar object)中直接检测到甲烷，且甲烷释放相对于水的延迟表明其曾在外层经历消耗，观测到的甲烷来自未受处理的次表层物质。
...
### [54] [Leveraging rapid parameter estimates for efficient gravitational-wave Bayesian inference via posterior repartitioning](https://arxiv.org/abs/2601.21630)
*Metha Prathaban,Charlie Hoy,Michael J. Williams*

**TL;DR**: 提出了一种结合快速算法（simple-pe）和后验重分区（posterior repartitioning）的贝叶斯推断方法，在信噪比（SNR）大于20时能高效引导嵌套采样器定位参数空间高概率区域，并保证结果与传统分析一致，验证表明该方法对信噪比低于150的双星系统可提速2.2倍且性能随信噪比提升而增强。


<details>
  <summary>Details</summary>

Background: 引力波天文学（gravitational wave astronomy）通常依赖于计算密集的贝叶斯分析，亟需开发快速且统计严谨的推断方法以提升分析效率。

Data: 数据来源为引力波信号（gravitational wave）的快速约束算法（simple-pe algorithm）和嵌套采样（nested sampling）加速技术（posterior repartitioning）的结合应用。

Method: 采用简单峰值算法（simple-pe）的快速约束与后验重分区（posterior repartitioning）嵌套采样加速技术相结合的方法。

Result: 提出了一种结合快速约束算法和后验重分配技术的新方法，可在信噪比(SNR)大于20时有效引导嵌套采样器定位参数空间高概率区域，并确保最终推断结果与标准无信息分析完全一致。
...
### [55] [Optimal cross-correlation technique to search for strongly lensed gravitational waves](https://arxiv.org/abs/2601.22138)
*Anirban Kopty,Sanjit Mitra,Anupreeta More*

**TL;DR**: 提出了一种基于最优互相关分析（OCCAM）的低计算成本方法，用于高效识别引力波（GW）探测器网络中强透镜化致密双星并合（CBC）事件候选体，在单探测器条件下达到97%（80%）的检测率且显著优于现有低计算量方法。


<details>
  <summary>Details</summary>

Background: 研究背景是随着引力波(GW)观测站灵敏度提升，探测强透镜化事件对的可能性增加，但现有方法在计算成本和依赖先验参数知识方面存在挑战。

Data: 数据来源于一个或多个探测器采集的应变数据（strain data），用于识别引力波（GW）搜索中发现的致密双星并合（CBC）事件。

Method: 采用最优多重互相关分析（OCCAM）技术对紧凑双星并合（CBC）事件的应变数据进行分析，通过最优、轻度模型依赖且低计算成本的方法识别强引力透镜候选体。

Result: 提出了一种名为OCCAM（Optimal Cross-Correlation Analysis for Multiplets）的新型方法，能够以较低计算成本和轻度模型依赖的方式，从引力波（GW）观测数据中高效识别强透镜候选体（strongly lensed candidates）。
...
### [56] [Is Phantom Divide Crossing in General Relativity Completely Impossible? Shortcomings and Possible Solutions](https://arxiv.org/abs/2601.21356)
*Shin'ichi Nojiri,S. D. Odintsov,V. K. Oikonomou*

**TL;DR**: 研究发现k-本质(k-essence)理论可实现宇宙晚期幻影(phantom)向精质(quintessence)的转变，而标准标量场理论无法实现这一过程，且相比修正引力理论需要更多精细调节。


<details>
  <summary>Details</summary>

Background: 研究探讨了广义相对论（general relativity）在大尺度宇宙描述中的局限性，聚焦于暗能量（Dark Energy）场景下实现幻影（phantom）向精质（quintessence）转变的理论可能性。

Data: 研究采用理论模拟(simulation)方法，基于广义相对论框架下的k-本质(k-essence)场论模型和修正引力理论进行数值分析。

Method: 采用k-本质理论（k-essence theories）分析幻影-精质（phantom-to-quintessence）转变的可能性，并探讨消除幽灵不稳定性（ghost instabilities）的方法。

Result: 在晚期宇宙中，k-本质理论(k-essence theories)可能实现幻影(phantom)向精质(quintessence)的转变，而修正引力理论无需精细调节即可自然实现这种转变。
...
### [57] [$\mathcal{R}^2$-corrected Tachyon Scalar Field Inflation, the ACT Data, and Phantom Transition](https://arxiv.org/abs/2601.21364)
*S. D. Odintsov,V. K. Oikonomou*

**TL;DR**: 研究发现，在包含$\mathcal{R}^2$修正项和重标度爱因斯坦-希尔伯特项的标量张量理论框架下，快子（tachyon）标量场模型可实现幻影（phantom）能态分界线跨越，且与ACT观测数据兼容，条件是暴胀期间有效引力常数变为$\frac{G}λ$。


<details>
  <summary>Details</summary>

Background: 该研究探讨了在包含$\mathcal{R}^2$修正项和重标度爱因斯坦-希尔伯特项（$\sim λ\frac{\mathcal{R}}{16πG}$）的框架下，单标量场（tachyonic minimally coupled single scalar field）理论中幻影能态分界线（phantom divide line）跨越的可能性及其与ACT观测数据的兼容性。

Data: 研究中使用的数据来源于ACT（Atacama Cosmology Telescope）观测数据。

Method: 采用包含$\mathcal{R}^2$修正项和重标度爱因斯坦-希尔伯特项（$\sim λ\frac{\mathcal{R}}{16πG}$）的框架，结合快子（tachyonic）最小耦合单标量场理论，分析幻影（phantom）分界线穿越现象。

Result: 在存在$\mathcal{R}^2$修正项和重标度爱因斯坦-希尔伯特项($\sim λ\frac{\mathcal{R}}{16πG}$)的框架下，标量场理论可实现幻影分界线(phantom divide line)穿越，且该模型与ACT数据兼容的条件是暴胀期间有效引力常数变为$\frac{G}λ$。
...
### [58] [Primordial black holes and Scalar-Induced Gravitational Waves formed by inflation potential with non-trivial characteristics](https://arxiv.org/abs/2601.21538)
*Ruifeng Zheng,Yanqing Xu*

**TL;DR**: 通过引入线性洛伦兹型耦合局部破坏慢滚条件，发现正负耦合均可形成可观丰度的原初黑洞（PBH），并研究了该模型产生的标量诱导引力波（SIGW）。


<details>
  <summary>Details</summary>

Background: 研究主题为探讨原初黑洞（PBH）形成机制中Starobinsky势和KKLT势的局部耦合特性及其对慢滚条件的破坏作用。

Data: 使用线性洛伦兹型耦合（linear Lorentzian-type coupling）方法分析Starobinsky势和KKLT势的局部耦合特性。

Method: 引入线性洛伦兹型耦合（linear Lorentzian-type coupling）局部破坏慢滚条件（slow roll conditions），分析Starobinsky势和KKLT势的局域耦合特性。

Result: 引入线性洛伦兹型耦合（linear Lorentzian-type coupling）局部破坏慢滚条件（slow roll conditions）后，Starobinsky势和KKLT势的正负耦合均可形成显著丰度的原初黑洞（PBHs）。
...
### [59] [Perturbative analysis of singularity-free cosmological solutions in unimodular Kaluza-Klein theory](https://arxiv.org/abs/2601.21661)
*J. C. Fabris,S. Faller,R. Kerner*

**TL;DR**: 通过将五维卡鲁扎-克莱因(Kaluza-Klein)理论引入幺模(unimodular)条件并投影到四维时空，揭示了该条件等效于在四维爱因斯坦方程中引入宇宙学项和布兰斯-迪克(Brans-Dicke)型标量场，并构建了含标量场和物质源的无奇点宇宙学解，通过微扰分析证明了这些解的稳定性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于五维卡鲁扎-克莱因理论（Kaluza-Klein theory）的单模（unimodular）版本及其四维时空投影，探讨引入单模条件如何等效于四维爱因斯坦方程中的宇宙学项和布兰斯-迪克（Brans-Dicke）型标量场。

Data: 数据来源于对五维卡鲁扎-克莱因理论（Kaluza-Klein theory）的投影构造及微扰分析。

Method: 采用的核心分析方法是对宇宙学解进行微扰分析（perturbative analysis），重点研究小涨落下的稳定性。

Result: 在五维Kaluza-Klein理论中施加幺模(unimodular)条件det$g_{AB}=1$等价于在四维时空中引入宇宙学项和Brans-Dicke型标量场。
...
### [60] [The reason peculiar velocities grow faster in general relativity than in Newtonian gravity](https://arxiv.org/abs/2601.21741)
*Erick Pastén,Christos Tsagas*

**TL;DR**: 研究发现相对论性处理（relativistic treatment）比牛顿理论（Newtonian theory）更能解释大尺度体流（bulk flows）的高速增长，因其考虑了物质运动动能（kinetic energy）对引力场的贡献。


<details>
  <summary>Details</summary>

Background: 该研究探讨了宇宙学中观测到的大尺度异常运动（bulk flows）与ΛCDM（Lambda冷暗物质）模型预测之间的不一致性，重点分析了牛顿力学与广义相对论框架下对异常速度场（peculiar-velocity field）增长速率的不同理论解释。

Data: 研究中使用的数据来源于文献中已有的完全相对论性（fully relativistic）和牛顿力学（Newtonian）理论分析结果，未涉及实际观测或数值模拟（simulation）数据。

Method: 采用完全相对论性处理（fully relativistic treatment）和牛顿理论对比分析，通过引入被牛顿理论忽略的无源项（source-free terms）到泊松方程（Poisson equation）中，统一比较了牛顿、准牛顿和完全相对论性研究对奇特速度场（peculiar-velocity field）线性增长的驱动机制差异。

Result: 相对论处理揭示了由物质运动引发的动能通量（peculiar flux）对引力场的直接贡献，显著增强了线性扰动下本动速度场（peculiar-velocity field）的增长速率，从而解释了观测到的大尺度高速体流（bulk flows）现象。
...
### [61] [Numerical simulations of primordial black hole formation via delayed first-order phase transitions](https://arxiv.org/abs/2601.21878)
*Zhuan Ning,Xiang-Xi Zeng,Rong-Gen Cai,Shao-Jiang Wang*

**TL;DR**: 通过数值模拟研究延迟一级相变（FOPTs）中超视界假真空域（FVD）坍缩，发现时间尺度比t_H/t_V能更可靠预测两类原初黑洞（PBH）形成（B型需t_H/t_V≳1，A型需0.35-0.7）及无PBH的弥散情况，而密度对比δ(t_H)虽能区分结果但对模型细节敏感。


<details>
  <summary>Details</summary>

Background: 研究探讨了延迟一阶相变（FOPTs）过程中超视界假真空域（FVD）坍缩引发原初黑洞（PBH）形成的非线性动力学机制。

Data: 使用自适应网格细化技术对耦合重力-标量-流体系统中的超视界假真空域（FVD）坍缩进行完全非线性、球对称数值模拟，数据来源于自主生成的仿真结果。

Method: 采用自适应网格加密技术（adaptive mesh refinement）对耦合引力-标量-流体系统中的超视界假真空域（FVD）坍缩进行完全非线性球对称数值模拟，并通过评估时间尺度比t_H/t_V（视界穿越时间与真空能主导时间之比）和视界穿越时的局域密度扰动δ(t_H)作为原始黑洞（PBH）形成的判别标准。

Result: 通过数值模拟发现时间尺度比$t_\mathrm{H}/t_\mathrm{V}$是更可靠的原始黑洞（PBH）形成判据：当$t_\mathrm{H}/t_\mathrm{V} \gtrsim 1$时形成B型（超临界）PBH，处于中间值时形成A型（亚临界）PBH，低于下限时无PBH形成。
