...
Background: 研究聚焦于利用JWST（詹姆斯·韦伯太空望远镜）观测到的“小红点”（LRDs）探索高红移（z∼3-10）宇宙中通过直接坍缩形成的超大质量黑洞（supermassive black holes）的早期形成机制，特别是准恒星（quasi-stars）作为其前身天体的理论模型与观测特征。

Data: 数据来源为JWST（James Webb Space Telescope）观测到的红移（redshift）在z∼3-10范围内的致密红色天体（Little Red Dots）光谱特征数据。

Method: 采用MESA-QUEST建模框架构建准恒星(quasi-star)演化模型，计算不同黑洞质量下的光度、温度、表面重力及寿命参数，并通过合成光谱与观测数据对比验证模型。

Result: 准恒星（quasi-star）模型能够再现观测到的“小红点”（LRDs）连续光谱特征，表明它们可能是早期宇宙中通过直接坍缩形成超大质量黑洞的前身天体。
...
Background: 研究利用银河系（MW）或本星系群（LG）中星系的恒星形成历史（通过解析测光获得）来推断高红移（$z \gtrsim 6$）星系的低质量/暗端恒星质量函数（SMFs）或紫外光度函数（UVLFs），以突破当前JWST探测极限（$M_{\mathrm{UV}} \gtrsim -15$）。

Data: 数据来源于FIRE-2模拟（FIRE-2 simulations），用于研究原初银河系（proto-MW/LG）在红移$z=6-9$时期的恒星质量分布和紫外光度函数（UVLFs）。

Method: 采用FIRE-2模拟（FIRE-2 simulations）分析原银河系/本星系群（proto-MW/LG）中低质量星系（包括被瓦解的恒星流和相位混合天体）对恒星质量函数（SMFs）和紫外光度函数（UVLFs）的贡献。

Result: 通过FIRE-2模拟发现，被瓦解星系（如恒星流和相位混合天体）贡献了原初银河系/本星系群在红移6-9时期总恒星质量预算的约50%。
...
Background: 研究探讨恒星反馈（stellar feedback）和爆发性恒星形成（bursty star-formation）如何通过气体流出-流入循环（gas outflow and inflow cycles）在低质量星系中产生暗物质核心（dark matter cores）并引发“呼吸模式”（breathing modes）。

Data: 研究使用了103颗恒星形成低质量星系样本的观测数据，包括恒星质量在7.9 < log M*/M⊙ < 9.6、红移0.02 < z < 0.19范围内的Hα发射线测量气体速度弥散度，并与FIRE-2模拟的模拟观测数据进行对比。

Method: 采用流体动力学模拟（FIRE-2）分析反馈驱动的气体流出-流入循环对低质量星系动力学的影响，并通过Hα发射线测量电离气体速度弥散（σ_gas）与模拟结果进行对比。

Result: 观测数据与FIRE-2模拟均发现气体速度弥散残差（Δσ_gas）与特定恒星形成率（sSFR）呈正相关，支持低质量星系中气体外流与内流循环驱动的短时标动力学效应（breathing mode）的存在。
...
Background: 研究聚焦于高红移（$3.0\lesssim z\lesssim4.5$）下三颗明亮亚毫米星系（SMGs）的静止光学和远红外连续辐射特性，探讨其形态多样性与恒星形成触发机制的关联。

Data: 数据来源于詹姆斯·韦伯太空望远镜（JWST）和阿塔卡马大型毫米波/亚毫米波阵列（ALMA）的观测，分辨率达到400-500 pc（约0.05角秒）。

Method: 采用詹姆斯·韦伯太空望远镜(JWST)和阿塔卡马大型毫米波/亚毫米波阵列(ALMA)的高分辨率(400-500 pc)观测数据，结合光学和远红外连续谱形态分析及Sérsic指数拟合，研究了亚毫米波星系(SMGs)的异质性结构及其恒星形成触发机制。

Result: 三个高红移亚毫米星系(SMG)表现出异质性形态和多种恒星爆发触发机制，包括气体盘不稳定性(AzTEC-4)、小质量并合(AzTEC-8)以及气体丰富大质量并合诱导的气体盘重组(AzTEC-1)。
...
Background: 研究聚焦于利用高分辨率X射线光谱分析窄线赛弗特1型星系（Narrow-Line Seyfert 1 galaxy）NGC 4051中由中心黑洞外流的多层光致电离气体（photoionized gas）的动力学特性与物理状态。

Data: 数据来源于XMM-Newton高分辨率反射光栅光谱仪（RGS）对窄线塞弗特1型星系NGC 4051的两次观测。

Method: 采用时间依赖的光电离模型（TEPID）分析高分辨率X射线光谱数据，约束了高电离相（HIP）气体的密度和距离。

Result: 在窄线塞弗特1型星系(Narrow-Line Seyfert 1 galaxy)NGC 4051中发现了三个不同电离程度的气体外流层：低电离相(LIP)、高电离相(HIP)和高速高电离相(HVIP)，其中HIP显示出显著的电离变化($3.8\sigma$)，表明其处于非平衡态。
...
Background: 研究孤立星系（isolated galaxies）的长期独立演化过程及其内部动力学机制，特别关注其物理性质如何主要由内部演化过程而非外部环境影响所塑造。

Data: 数据来源包括来自Catalogue of Isolated Galaxies (CIG)的样本、Fabry-Perot观测获得的H$\alpha$运动学数据、HI数据以及UV、光学和FIR波段的多波段观测数据。

Method: 采用法布里-珀罗(Fabry-Perot)干涉仪获取Hα运动学数据，并结合紫外(UV)、光学和远红外(FIR)多波段分析，计算样本星系旋转曲线和动力学质量，通过B波段和K波段Tully-Fisher关系验证运动学结果准确性。

Result: 在近边缘（$i\geq80^{\circ}$）晚型孤立星系（CIG）样本中，观测到半数星系（14个中的7个）存在沿$z$轴旋转滞后（$\Delta V/\Delta z=32.0\pm10.6\,{\rm km\,s^{-1}\,pc^{-1}}$），且形态学上未必伴随盘外成分，表明旋转滞后不能作为盘外弥散电离气体（eDIG）的确定性标志。
...
Background: 研究主题为探索尘埃恒星形成星系（DSFGs）是否可作为大尺度致密环境（large-scale overdensities）的可靠示踪天体。

Data: 数据来源于双子座天文台南站Flamingos-2 (F2)的$K_s$波段成像、斯巴鲁Hyper Suprime-Cam的辅助多波段测光数据以及哈勃太空望远镜(Hubble Space Telescope)的多波段测光数据。

Method: 采用Gemini South Flamingos-2 (F2) $K_s$波段成像结合Subaru Hyper Suprime-Cam和哈勃太空望远镜多波段测光数据，获取伴星系的测光红移($z_\text{phot}$)、恒星形成率和恒星质量估计。

Result: 两个尘埃恒星形成星系（DSFGs）的光度分析表明它们可能标记原星系团外围或相关大尺度结构，但小样本量无法确定其能否精确定位致密星系团核心。
...
**TL;DR**: 利用高速度K巨星（K giants）的6D相空间数据首次直接测量了银河系（Milky Way）逃逸速度曲线（escape velocity curve）延伸至约50千秒差距（kpc），并结合本地圆周速度估算出银河系总质量为$0.90_{-0.07}^{+0.06}\times 10^{12}\;M_{\odot}$，支持暗物质晕（DM halo）质量低于以往研究。


<details>
  <summary>Details</summary>

Background: 利用逃逸速度(escape velocity)约束银河系(Milky Way)暗物质晕(DM halo)质量是研究暗物质分布和星系动力学的核心课题。

Data: 数据来源为LAMOST DR8巡天数据和Gaia DR3数据的交叉匹配样本，选取了具有完整6维相空间信息且质量较好的高速K巨星（K giants）作为研究对象。

Method: 采用基于总速度的半径依赖标准（$v_{\rm GC}>300\;\rm{km s^{-1}}$或$v_{\rm GC}>v_{\rm min}\sim0.6\times v_{\rm esc}(r_{\rm GC})$）筛选高速K巨星样本，并结合$v_{\phi}-{\rm [Fe/H]}$信息选择各向同性晕星，使用传统幂律模型（power-law models）拟合速度分布以测定各径向分区的逃逸速度。

Result: 首次直接测量了延伸至银河系中心半径约50千秒差距的较连续逃逸速度曲线，发现其下降趋势与先前研究一致，并基于太阳位置逃逸速度测量值523.74^{+12.83}_{-13.47} km s^{-1}及局部圆周速度，采用纳维罗-弗伦克-怀特暗物质晕模型估算银河系总质量为0.90_{-0.07}^{+0.06}×10^{12} M_{\odot}，浓度参数c_{200}=13.47_{-1.70}^{+1.85}。
...
Background: 研究聚焦于高红移(z=4.4-5.7)恒星形成主序星系中尘埃衰减(attenuation)特性的表征及其对星系本征物理性质的影响。

Data: 数据来源于JWST/NIRSpec IFU和NIRCam对18个红移(z=4.4-5.7)恒星形成主序星系的观测，以及ALPINE和CRISTAL ALMA大型项目的选源。

Method: 采用Prospector工具对JWST/NIRSpec IFU和NIRCam获取的发射线流量与宽带测光数据进行空间积分和像素级（~0.6 kpc）光谱能量分布（SED）拟合，并结合巴尔默减幅（Balmer decrement）测量恒星与星云尘埃消光比（f=E(B-V)_{\mathrm{star}}/E(B-V)_{\mathrm{neb}}）。

Result: 在$z=4.4-5.7$的18个恒星形成主序星系中，恒星与星云尘埃消光比($f=E(B-V)_{\mathrm{star}}/E(B-V)_{\mathrm{neb}}$)的最佳拟合值为$0.51^{+0.04}_{-0.03}$，略高于本地星暴星系测量值，且与更高比恒星形成率、更年轻恒星年龄及更近期恒星形成活动呈弱正相关。
...
Background: 活动星系核(AGN)反馈在星系形成的宇宙学模拟中至关重要，但受限于分辨率其实现需依赖亚网格(subgrid)模型。

Data: 数据来源于三种流体动力学代码（SWIFT、AREPO和PLUTO）的理想化喷流模拟，比较了均匀和分层介质中不同分辨率和喷流参数下的喷流演化及其残余物。

Method: 采用了一种新型亚网格(subgrid)喷流发射方法，并在三种流体动力学代码（SWIFT、AREPO和PLUTO）中实现了该方法，通过比较理想化喷流在不同介质中的演化来评估流体动力学求解器的影响。

Result: 在均匀介质中，不同流体力学代码（SWIFT、AREPO、PLUTO）模拟的喷流演化表现出自相似性但形成不同形态的瓣状结构（lobes），其中SWIFT的瓣更短、更宽且更热，AREPO的瓣更长、更薄且更冷，而PLUTO的瓣呈现具有中间特性的复杂流动。
...
Background: 研究聚焦于热尘埃（thermal dust）作为主要偏振前景对宇宙微波背景（CMB）B模式探测的干扰，探讨其信号在频率空间中的复杂行为与银河系磁场方向及尘埃颗粒光谱特性变化的关联。

Data: 使用了普朗克卫星（Planck）的Sroll2版本数据和PR4官方发布数据，覆盖了排除银河系内平面97%天区的偏振微波观测数据。

Method: 采用基于残差图（residual maps）的复杂量表示方法，通过分析局部偏振光谱能量分布（SED）与天空掩模平均值的偏差，建立统计相关性及协方差与尘埃颗粒物理性质的关联模型。

Result: 提出了一种基于残差图（residual maps）的新分析框架，通过统计相关性和协方差分析揭示银河系尘埃（thermal dust）偏振信号的物理特性，发现Sroll2版本数据协方差谱依赖性与仅考虑温度变化的尘埃模型相符，而PR4版本数据与温度或谱指数变化模型均不符。
...
Background: 该研究聚焦于银河系疏散星团（open clusters）普查的完整性评估及其选择函数（selection function）的建立，旨在提升对银河系结构示踪工具的可靠性理解。

Data: 数据来源于盖亚卫星第三次数据释放（Gaia DR3）中的Hunt & Reffert星团表，并进行了80,590次模拟开放星团的注入和检索测试。

Method: 采用模拟星团注入与检索（injection and retrieval）方法，结合逻辑回归模型（logistic model）分析星团可探测性，该模型基于星团恒星数量、中位视差误差（median parallax error）、盖亚（Gaia）数据密度和用户指定显著性阈值等参数。

Result: 首次建立了盖亚(Gaia) DR3中Hunt & Reffert星团表的完整选择函数，开发了开源Python工具包并通过80,590次模拟星团注入与检索验证了该星团表的灵敏度。
...
Background: 双峰莱曼-α(Lyα)谱线为研究从中性氢(HI)的星际介质(ISM)到星系际介质(IGM)的气体运动学及分布提供了关键信息，并可作为电离莱曼连续谱(LyC)光子逃逸的重要诊断工具。

Data: 数据来源于VLT/MUSE（甚大望远镜/多单元光谱探测器）在MAGPI巡天项目中获取的观测数据。

Method: 采用VLT/MUSE观测数据和自动化峰分类技术（automated peak classification technique）分析双峰莱曼α（Lyα）发射体的全局和空间分辨特性。

Result: 双峰莱曼α(Lyα)发射体的蓝峰占比随光度变化：暗线通常蓝峰更强，但部分亮源也显示显著蓝峰。
...
Background: 研究揭示了银河系中可能存在高速下落的一氧化碳(CO)分子云，为理解星际介质(ISM)演化提供了新线索。

Data: 数据来源于Mopra CO $J=$1-0巡天观测数据（Mopra CO $J=$1-0 survey data）和盖亚卫星第三次数据释放（Gaia Data Release 3）。

Method: 采用Mopra望远镜CO J=1-0巡天数据分析和Gaia DR3距离测量，结合$^{13}$CO J=2-1、$^{12}$CO J=1-0及$^{13}$CO J=1-0谱线强度反演动力学温度。

Result: 在银河系盘面附近发现了两颗以超过35 km s$^{-1}$速度下落的CO分子云（carbon monoxide clouds），其头部温度显著高于典型分子云的10 K，表明它们与盘面HI气体（atomic hydrogen gas）的强激波相互作用导致了显著加热。
...
Background: 该研究聚焦于利用默奇森宽场阵列(MWA)开展300 MHz宽场射电巡天，填补南半球全天区高频(300 MHz)观测空白，补充银河系和河外全天MWA巡天(GLEAM)多频段(72-231 MHz)数据，为未来平方公里阵列低频阵(SKA-Low)巡天提供前瞻性光谱数据点。

Data: 数据来源于使用默奇森宽场阵列（MWA）在2015-2016年期间进行的覆盖$-90^\circ \leq \delta_\text{J2000} \lesssim +40^\circ$天区的两分钟快照观测，通过叠加重叠的快照图像生成全灵敏度拼接图像。

Method: 采用两分钟快照观测（two-minute snapshot observations）结合重叠图像拼接技术生成全灵敏度马赛克图像。

Result: 该300 MHz宽场射电巡天（radio survey）首次利用默奇森宽场阵列（MWA）覆盖了整个南半球，填补了GLEAM巡天在高频段的数据空白。
...
Background: 研究主题为重复变脸活动星系核（RCL AGNs）中宽发射线（BELs）反复出现与消失现象的物理机制及其对吸积过程的约束作用。

Data: 数据来源于斯隆数字巡天(SDSS)、大天区多目标光纤光谱望远镜(LAMOST)和暗能量光谱仪(DESI)的多期光谱数据，并辅以中红外(MIR)光变曲线。

Method: 采用多波段光谱数据（SDSS、LAMOST、DESI）和中红外（MIR）光变曲线筛选重复变脸活动星系核（RCL AGNs），并通过目视检查确认样本。

Result: 在331个变脸活动星系核(CL AGN)中发现了25个重复变脸活动星系核(RCL AGN)，其中19个为新发现样本，表明重复转变现象在CL AGN中并不罕见。
...
**TL;DR**: 发现了一颗33倍太阳质量的黑洞（Gaia BH3）与贫金属红巨星组成的双星系统，并通过对其伴星（bhstar）的全面化学分析（测得29种元素丰度）确认其为α元素富集（[α/Fe]=0.41）的r-I型中子俘获星（[Eu/Fe]=0.57），未发现化学异常，支持动力学捕获或孤立双星演化两种形成机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用金属贫乏红巨星（metal-poor red giant）的化学组成约束大质量恒星黑洞双星系统（如Gaia BH3）的形成机制。

Data: 数据来源于盖亚任务(Gaia)第四期数据发布(DR4)的初步天体测量数据，以及麦克唐纳天文台2.7米哈伦·J·史密斯望远镜上的Tull Coudé摄谱仪获取的高分辨率光谱。

Method: 采用高分辨率光谱（Tull Coudé Spectrograph）对伴星（\bhstar）进行29种元素丰度测定（从锂到钍），并利用Th/Eu计时器（chronometer）约束系统宇宙年代学年龄。

Result: 发现Gaia BH3系统的伴星（bhstar）具有α元素富集（[α/Fe] = 0.41）和r-I型中子俘获元素特征（[Eu/Fe] = 0.57），且未表现出任何化学异常，与动力学捕获或孤立双星演化形成机制均吻合。
...
Background: 高红移(high-redshift)星系中的星际介质(ISM)电子密度显著高于本地宇宙，研究其起源是理解星系演化的关键。

Data: 数据来源于暗能量光谱仪(DESI)数据发布1(Data Release 1)中选取的9590个恒星质量大于$10^9\,M_\odot$、红移范围$0.01 < z < 0.04$的中心恒星形成星系样本。

Method: 采用[S~II]~$\lambda\lambda6716,6731$双线测量电子密度($n_{\rm e}$)并建立其与恒星形成率面密度($\Sigma_{\rm SFR}$)的分段幂律关系。

Result: 高红移星系中星际介质(ISM)的电子密度($n_{\rm e}$)与恒星形成率面密度($\Sigma_{\rm SFR}$)存在分段幂律关系，当$\log(\Sigma_{\rm SFR}) \ge -1.46$时遵循$n_{\rm e} = (233 \pm 13)\,\Sigma_{\rm SFR}^{0.49 \pm 0.02}$，低于该阈值则稳定在$44 \pm 3~{\rm cm^{-3}}$。
...
Background: 研究聚焦于暗物质微观物理（dark matter microphysics）对亚晕（subhalo）内区密度分布的调控效应及其在强引力透镜观测中的可探测性差异。

Data: 数据来源于模拟的哈勃太空望远镜（HST）、欧几里得（Euclid）和詹姆斯·韦伯太空望远镜（JWST）类强引力透镜观测。

Method: 采用强引力透镜模拟观测技术，重点分析子晕内密度分布幂律斜率（β）对透镜信号的影响，并对比不同β值（如1和2.2）下可探测子晕的最小质量差异。

Result: 内幂律斜率（inner power-law slope）β约为2.2的子晕（subhalo）可探测质量下限比β约为1的NFW（Navarro-Frenk-White）子晕低一个数量级。
...
Background: 研究探讨了多组分波(和粒子)暗物质(dark matter)中结构增长的宏观科学背景，重点关注不同暗物质组分(如标量场和多自旋场)的密度涨落演化及其对自由流尺度(Jeans scales)的影响。

Data: 数据来源于理论推导框架下的多组分暗物质（包括波暗物质和粒子暗物质）的功率谱演化模拟（simulation）分析。

Method: 采用多组分密度扰动功率谱（power spectrum）演化框架分析波暗物质（wave dark matter）和粒子暗物质（particle dark matter）的结构增长。

Result: 建立了多组分波暗物质（wave dark matter）和粒子暗物质（particle dark matter）中任意数量、密度分数及初始场功率谱下总密度对比功率谱演化的统一理论框架。
...
Background: 星系群和星系团的燃气质量分数（gas mass fraction）是限制反馈过程对大规模结构影响的关键物理量。

Data: 研究使用了FLAMINGO大体积流体动力学模拟（hydrodynamical simulations）数据，重点分析了中心及卫星超大质量黑洞（BHs）对气体质量分数-晕质量关系（gas fraction-halo mass relation）散射的影响。

Method: 采用大体积FLAMINGO流体动力学模拟(FLAMINGO hydrodynamical simulations)分析超大质量黑洞(BHs)对气体质量分数-晕质量关系(gas fraction-halo mass relation)散射的影响，并通过追踪黑洞和晕的演化历史揭示其驱动机制。

Result: 超大质量黑洞（BHs）的早期增长历史及其宿主晕的演化显著影响了现今气体质量分数-晕质量关系的散射特征。
...
**TL;DR**: 提出了一种简化版运动学透镜（KL）方法（one-component KL），通过结合运动学信息和成像数据降低弱引力透镜（WL）的形状噪声，模拟分析表明该方法在深光谱巡天中约束力可能超越传统WL。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发一种简化版运动学透镜（KL）方法（即单分量KL），用于测量宇宙剪切（cosmic shear），旨在降低弱引力透镜（WL）的形状噪声并减少观测工作量。

Data: 使用模拟数据（simulation）评估一成分运动学透镜（one-component KL）方法的性能，并预测其在未来射电巡天和更深光谱巡天中的应用潜力。

Method: 采用单分量运动学透镜（KL）方法，结合运动学信息与成像数据以减少弱引力透镜（WL）的形状噪声。

Result: 单分量运动学透镜（KL）方法通过结合运动学信息和成像数据，在减少弱引力透镜（WL）形状噪声的同时降低了观测难度。
...
Background: 研究探讨了原初宇宙微波背景（CMB）各向异性三谱对大规模结构（LSS）中宇称破坏（PV）的敏感性。

Data: 研究使用理论模型探讨原初宇宙微波背景（CMB）各向异性三谱对大规模结构（LSS）中宇称破坏（PV）的敏感性，未涉及实际观测或模拟数据。

Method: 采用萨克斯-沃尔夫(Sachs-Wolfe)项主导的宇宙微波背景(CMB)各向异性三谱(trispectrum)理论模型，分析大尺度结构(LSS)中宇称破坏(parity violation, PV)的敏感性，并引入最大尺度$R$约束PV效应范围。

Result: 原初宇宙微波背景（CMB）各向异性三谱对大规模结构（LSS）中宇称破缺（PV）的敏感性受限于因子R/χ*≈1%的抑制效应。
...
Background: 研究主题为利用宇宙微波背景(CMB)观测数据通过多频率多仪器方法测量星系团和宇宙纤维丝的物理性质，包括热苏尼亚耶夫-泽尔多维奇效应(tSZ)和动力学苏尼亚耶夫-泽尔多维奇效应(kSZ)。

Data: 数据来源于普朗克卫星(Planck)的存档数据和阿塔卡马宇宙学望远镜(Atacama Cosmology Telescope)新获取的深度观测数据，覆盖了30 GHz至545 GHz范围内11个不同频率的14张地图。

Method: 采用多频率多仪器联合拟合方法，同时测量热（tSZ）和运动学（kSZ）苏尼亚耶夫-泽尔多维奇效应，并整合相对论修正及热尘埃发射等天体物理前景模型。

Result: 通过联合拟合多频段多仪器数据，成功测量了Abell 399-Abell 401星系团-纤维系统（filament system）的视线方向本动速度（peculiar velocities）和光学深度（optical depth），其中纤维结构的光学深度信噪比达8.5σ。
...
Background: 该研究聚焦于优化黄道光（zodiacal light）模型，特别是在光学波段（0.3-1.6微米）的精度，以弥补现有模型（如Kelsall等1998年基于DIRBE红外数据的模型）在短波长和特定太阳角度下的不足。

Data: 数据来源于哈勃太空望远镜（HST）的SKYSURF项目存档成像数据，以及基于超过5000次HST天空表面亮度（sky-SB）测量得出的0.3-1.6微米光学波段经验散射相函数和反照率分析形式。

Method: 采用基于哈勃太空望远镜（HST）SKYSURF项目的5000余次天空表面亮度（sky-SB）测量数据，通过经验确定散射相位函数和反照率随波长（0.3-1.6微米）变化的解析形式，对Kelsall等人1998年模型进行修正（称为ZodiSURF）。

Result: 基于哈勃空间望远镜（HST）SKYSURF项目的5000余次天空亮度（sky-SB）测量数据，开发了改进的黄道光模型（ZodiSURF），通过经验确定0.3-1.6微米波段散射相函数和反照率的解析形式，显著提升了该波长范围及太阳角大于80度时的黄道光发射预测精度，模型不确定性约为4.5%。
...
Background: 研究聚焦于利用[CII]谱线强度映射（LIM）技术探索再电离时期（EoR）的宇宙学信号，并评估其对原始间歇性非高斯性（PING）等新型暴胀模型的约束潜力。

Data: 数据来源于使用Peak Patch算法生成的暗物质(DM)晕光锥目录和WebSky框架前向建模的宇宙学[CII]信号模拟数据。

Method: 采用峰值补丁算法（Peak Patch algorithm）生成光锥暗物质晕（DM halo）目录，并利用WebSky框架对宇宙学[CII]信号进行前向建模，构建与CCAT天文台匹配的层析成像模拟巡天。

Result: result：通过[CII]体素强度分布(VID)和相对熵研究，发现即将开展的线强度测绘(LIM)巡天能改进宇宙线发射模型并对原始间歇非高斯性(PING)提供竞争性约束。
...
**TL;DR**: 提出了一种非微扰指数型f(R)暴胀模型，在ΛCDM框架下结合多源观测数据验证其优于Starobinsky R²暴胀模型，尤其在早期暗能量（EDE）框架下能更好兼容SH0ES对H₀的约束。


<details>
  <summary>Details</summary>

Background: 研究探讨了在ΛCDM框架下参数化的慢滚暴胀（slow-roll inflationary）模型及其在早期暗能量（early dark energy, EDE）框架下的修正。

Data: 数据来源于Planck 2018、ACT DR6、DESI DR2和BICEP/Keck 2018（P-ACT-LB-BK18）的联合观测数据，并在早期暗能量（EDE）框架下结合了SH0ES对$H_0$的先验（P-ACT-LB-BK18-$H_0$）。

Method: 采用数值计算和马尔可夫链蒙特卡洛（MCMC）分析结合P-ACT-LB-BK18数据集，验证了非微扰指数型f(R)暴胀模型的可行性。

Result: 在早期暗能量(EDE)框架下结合SH0ES对H0的先验，提出的非微扰指数f(R)暴胀模型能更好地适应ACT偏好的标量谱指数值，并允许对原始Starobinsky R²暴胀模型的更大偏离。
...
Background: 研究主题为在自相似结构形成范式下，通过归一化星系团质量和红移依赖性，重建热团内气体的普适压力分布（universal pressure profile）。

Data: 研究使用了来自“Cluster HEritage project with XMM-Newton”（CHEX-MATE）项目的观测数据，样本包含约25个星系团，质量范围在2至14×10^14太阳质量（M_{500}）之间，红移范围在0.07至0.6之间。

Method: 采用联合拟合方法同时确定普适压力分布（universal pressure profile）和单个星系团质量（$M_{500}$），并考虑分布形状、振幅与质量标度间的相关性。

Result: 首次提出了一种联合拟合通用压力分布（universal pressure profile）和单个星系团$M_{500}$质量的方法，能正确处理分布形状、振幅与质量标度之间的相关性。
...
Background: 研究主题为利用J-PAS巡天数据通过非参数贝叶斯方法约束原初功率谱（primordial power spectrum），重点关注非标准暴胀模型激发的局部振荡原初特征（oscillatory primordial feature）。

Data: 数据来源于模拟生成的功率谱（simulated power spectra），基于非标准暴胀理论激发的局部振荡原初特征模板（local oscillatory primordial feature template），并限定在波数范围$k \in [0.02,0.2] \text{ h} \text{ Mpc}^{-1}$内以最大化信噪比。

Method: 采用非参数贝叶斯方法（non-parametric Bayesian method）结合线性插值重构原初功率谱（primordial power spectrum），并使用PolyChord联合采样宇宙学参数与节点参数。

Result: 结合红移分箱和示踪类型可检测振幅小至2%的振荡特征（oscillatory features）。
...
Background: 研究探讨了在广义相对论框架下，具有符号切换特性的宇宙学常数（$\Lambda_{\rm s}$CDM）模型对非线性物质功率谱的影响，并与标准$\Lambda$CDM模型进行对比。

Data: 数据来源于相对论性N体模拟（relativistic N-body simulations），比较了ΛsCDM（sign-switching cosmological constant）和ΛCDM模型在红移z=15、2、1、0时的非线性物质功率谱，使用了Planck-only和组合“full”数据集的最佳拟合参数。

Method: 采用相对论性N体模拟（relativistic N-body simulations）分析ΛsCDM（sign-switching cosmological constant）模型与ΛCDM在非线性物质功率谱上的差异，基于普朗克（Planck-only）和联合数据集（full dataset）的最佳拟合参数。

Result: 在$\Lambda_{\rm s}$CDM模型中，符号切换宇宙学常数（CC）导致物质功率谱比率$P_{\Lambda_{\rm s}\rm CDM}/P_{\Lambda\rm CDM}$在过渡红移附近出现峰值（约20-25%），并在$z=0$时保持约15-20%的增强，特征尺度对应于星系群/贫团环境且位于弱透镜等观测的敏感范围内。
...
Background: 宇宙学中符号模拟器（symbolic emulators）通过快速准确预测复杂物理模型，为高效探索高维参数空间提供了关键工具。

Data: 数据来源于对ΛCDM模型共动距离和线性增长因子中使用的超几何函数（hypergeometric functions）的近似计算，覆盖了Ωm ∈ [0.1, 0.5]和所有红移范围。

Method: 采用符号仿真器（symbolic emulators）结合超几何函数近似方法，对ΛCDM模型共动距离和线性增长因子进行高精度建模。

Result: 符号模拟器（symbolic emulators）将超几何函数近似用于ΛCDM共动距离和线性增长因子，在Ωm∈[0.1,0.5]和所有红移下分别达到优于0.001%和0.05%的精度。
...
Background: 该研究聚焦于利用明亮警报（bright sirens）和电磁对应体（electromagnetic counterparts）的引力波探测，结合Ia型超新星样本，作为宇宙膨胀的高精度探针。

Data: 数据来源为CosmoDC2_BCO星表（CosmoDC2_BCO catalog）和罗马望远镜类似（Roman-like）的Ia型超新星样本。

Method: 采用引力波（gravitational-wave）探测与电磁对应体（electromagnetic counterparts）联合分析的方法，结合CosmoDC2_BCO模拟星表，通过当前（LVK）和下一代（ET、CE）探测器网络与罗马望远镜（Roman-like）Ia型超新星样本的协同观测，进行宇宙学参数约束预测。

Result: 第三代引力波探测器网络（CE+ET+LVK）结合罗马型Ia超新星样本可在十年内将哈勃常数（H_0）测量精度提升至0.2%，显著优于当前LVK的6%水平。
...
Method: 采用马尔可夫链蒙特卡洛（MCMC）方法高效扫描多项式模型的六维参数空间，并针对非最小耦合多项式暴胀模型在拐点处采用二阶慢滚近似以确保精确的宇宙微波背景（CMB）谱预测。

Result: result：阿塔卡马宇宙学望远镜（ACT）观测到的标量谱指数（n_s）正跑动（α_s）偏好显著限制了超慢滚（USR）模型的参数空间，导致大质量原初黑洞（PBHs）更难被探测到，但小行星质量PBHs仍可能构成全部暗物质（DM）；非最小耦合多项式暴胀模型中，精确宇宙微波背景（CMB）谱预测需在拐点处采用二阶慢滚近似。
...
### [34] [N-body Simulations of cosmologies with Light Massive Relics](https://arxiv.org/abs/2510.18797)
*Vikhyat Sharma,Arka Banerjee*

**TL;DR**: 首次开发了能追踪轻质量相对论性粒子（LiMRs）在非线性尺度上影响的宇宙学N体模拟框架，揭示了其质量、温度和初始分布对物质功率谱、晕质量函数等观测量的系统效应。


<details>
  <summary>Details</summary>

Background: 研究聚焦于轻质量相对论性粒子（LiMRs）作为暗物质热亚组分对宇宙结构形成的非线性尺度影响及其与标准模型中微子的相互作用。

Data: 使用自主研发的宇宙学N体模拟（N-body simulation）框架，首次追踪了轻质量相对论性粒子（LiMRs）及其与标准模型大质量中微子（massive Standard Model neutrinos）的相互作用对非线性尺度结构形成的影响。

Method: 采用宇宙学N体模拟(N-body simulations)框架，结合标准模型大质量中微子(massive Standard Model neutrinos)和轻质量遗迹粒子(Light Massive Relics, LiMRs)的建模，系统研究LiMRs的质量、温度和初始分布对非线性尺度宇宙学观测量的影响。

Result: 首次开发了能够追踪轻质量相对论性粒子（LiMRs）在非线性尺度上影响的宇宙学N体模拟框架，并系统研究了LiMRs质量、温度和初始分布对物质功率谱、晕质量函数等观测量的效应。
...
### [35] [$B$-sure I: Minkowski functionals as robustness test for tensor-to-scalar ratio detection from CMB observations](https://arxiv.org/abs/2510.18868)
*Claudio Ranucci,Alessandro Carones,Léo Vacher,Nicoletta Krachmalnicoff,Carlo Baccigalupi*

**TL;DR**: 该研究通过模拟LiteBIRD类宇宙微波背景(CMB)实验数据，发现闵可夫斯基泛函(MFs)无法可靠检测前景残留导致的非高斯性，在74%情况下无法警示张标比(tensor-to-scalar ratio, r)估计偏差，表明需开发更先进的统计方法进行CMB任务中的可靠性检验。


<details>
  <summary>Details</summary>

Background: 探测宇宙微波背景（CMB）的原初B模偏振是现代宇宙学的重要观测目标，为通过测量张量标量比（tensor-to-scalar ratio）r研究暴胀物理提供了潜在窗口。

Data: 数据来源于模拟的类LiteBIRD实验观测的天图，包含真实的仪器和前景（foregrounds）建模。

Method: 采用闵可夫斯基泛函（Minkowski functionals, MFs）作为非高斯性检验手段，通过比较高斯模拟信号与前景残留污染图之间的统计差异来验证张量标量比（tensor-to-scalar ratio, r）检测的可靠性。

Result: 在模拟的类LiteBIRD实验中，闵可夫斯基泛函（Minkowski functionals）无法可靠地作为检验原初B模偏振（primordial $B$-mode）张标比（tensor-to-scalar ratio $r$）探测稳健性的工具，因其仅在约26%的模拟案例中能有效标记前景残留导致的非高斯性（non-Gaussianity）。
...
### [36] [A Cost-Effective Search for Extraterrestrial Probes in the Solar System](https://arxiv.org/abs/2510.17907)
*Beatriz Villarroel,Wesley A. Watters,Alina Streblyanska,Enrique Solano,Stefan Geier,Lars Mattsson*

**TL;DR**: 该论文提出了四种探测太阳系内外星技术痕迹（technosignatures）的方法，重点论证了利用地球阴影作为滤光器搜寻近地空间光学发光物体的新方法，并通过ZTF（Zwicky Transient Facility）观测数据验证了其可行性。


<details>
  <summary>Details</summary>

Background: 该研究探讨了在太阳系内搜寻地外文明技术痕迹（technosignatures）的可能性，聚焦于利用地球阴影作为滤光器探测近地空间光学发光物体的新方法。

Data: 研究中使用了兹威基瞬变设施（ZTF）获取的多次重复30秒曝光图像数据。

Method: method：利用地球阴影作为滤光片搜索近地空间的光学发光物体，并通过兹威基瞬变设施（ZTF）获取的重复曝光图像进行瞬变事件验证。

Result: 利用地球阴影作为过滤器在近地空间搜寻光学发光物体是一种新颖且有前景的搜寻地外技术特征（SETI）方法。
...
### [37] [Charged liquid surface instabilities and detection of Solar neutrino by XENONnT](https://arxiv.org/abs/2510.17911)
*Sergey Pereverzev*

**TL;DR**: XENONnT双相探测器发现太阳8B中微子（Solar 8B neutrinos）信号时伴随未解释的探测器效应，其中导线交叉区域（wire crossing regions）存在最大表面电荷密度和形变，可能对小信号探测产生显著系统误差。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于氙时间投影室(XENONnT)探测太阳中微子(solar 8B neutrinos)时遇到的未解探测器效应，特别是表面电荷积累与带电表面不稳定性导致的系统性误差问题。

Data: 数据来自XENONnT双相探测器的观测数据，但排除了导线交叉区域的异常数据。

Method: 采用电荷密度分布模型和表面形变模拟分析探测器表面电荷积累及不稳定性效应。

Result: 在XENONnT双相探测器中观测到的太阳8B中微子检测伴随一系列未解释的探测器效应，包括导线交叉区域出现的最大表面隆起和电荷密度异常。
...
### [38] [Exploring Giant Planet Atmospheres with Habitable Worlds Observatory](https://arxiv.org/abs/2510.17926)
*Leigh N. Fletcher,Amy Simon,Michael H. Wong,Jonathan D. Nichols,Nick A. Teanby,Conor A. Nixon,Marina Galand*

**TL;DR**: 该论文主张未来大型空间光学/紫外观测站（如HWO）需具备追踪巨行星及其卫星、观测极光和大气的高动态范围能力，并实现80-900纳米空间分辨光谱和秒至年尺度的时间分辨成像，以研究巨行星三维大气动力学及其对系外行星研究的范式意义。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用太阳系巨行星（giant planets）的可见光和紫外波段成像与光谱观测，为系外巨行星（exoplanets）、褐矮星（brown dwarfs）及其与宿主恒星相互作用的大气、电离层和磁层过程提供范式。

Data: 数据来源于未来大型空间光学/紫外观测站（如拟议的宜居世界天文台HWO）对太阳系巨行星（giant planets）及其卫星的可见光与紫外波段成像和光谱观测，包括分子吸收、气溶胶散射、气辉（airglow）和极光发射（auroral emissions）的测量。

Method: 采用可见光与紫外波段成像和光谱技术（80-900 nm），结合非恒星目标追踪、高动态范围探测及时间分辨观测手段，重点分析分子吸收、气溶胶散射、气辉和极光发射特征。

Result: 可见光和紫外成像及光谱技术对太阳系巨行星的研究可为系外巨行星、褐矮星及其与宿主恒星相互作用的大气、电离层和磁层过程多样性提供范式。
...
### [39] [Comission Femmes et Astronomie de la SF2A : Women participation in french astronomy 2025](https://arxiv.org/abs/2510.17927)
...
**TL;DR**: 该研究通过调查法国11个天文机构的1525名员工（占法国天文界约57%），发现女性比例随职业稳定性（job security）、资历、职称和收入提高而持续下降，尤其在获得永久职位（permanent positions）上存在明显瓶颈，印证了“泄漏管道”（leaky pipeline）现象。


<details>
  <summary>Details</summary>

Background: 研究聚焦于法国专业天文学领域女性从业者的现状及其随时间演变的趋势分析。

Data: 数据来源于对法国11个天文与天体物理研究所的1525名员工（包括博士生、博士后、学者及技术行政人员）的统计调查，覆盖了约57%的法国天文界从业人员。

Method: 采用问卷调查和统计分析（statistical analysis）方法评估法国天文学界女性从业者在职业阶段、责任级别、工作稳定性和收入等方面的比例变化。

Result: 法国专业天文学领域女性比例随职业稳定性、资历、资质水平和薪资提升而持续下降。
...
### [40] [Universal Spectral Tokenization via Self-Supervised Panchromatic Representation Learning](https://arxiv.org/abs/2510.17959)
...
**TL;DR**: 提出了一种自监督深度学习模型（universal spectral tokenizer），能够统一处理不同分辨率、波段和天体类型的异构光谱数据，生成对齐、同质且物理意义明确的表征，为天文学基础模型构建提供了关键组件。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决跨分辨率、跨领域科学数据（如天文光谱）的统一表征难题，旨在构建适用于天文学及其他科学领域（如气候、医疗）异质序列数据的通用基础模型。

Data: 研究中使用的数据来源于大规模天文光谱巡天项目收集的跨多种波长范围和分辨率的数百万条光谱，涵盖不同天体类型（如恒星和星系）和光谱域（如光学与红外）。

Method: 采用自监督深度学习模型（universal spectral tokenizer）处理异构光谱数据，直接在原生波长网格上学习生成对齐、同质且物理意义明确的表征。

Result: 提出了一种自监督深度学习模型（universal spectral tokenizer），能够直接在原生波长网格上处理不同天体类型和分辨率的异构光谱数据，生成内在对齐、同质且具有物理意义的表征，首次实现了跨分辨率和跨光谱域的光谱数据统一。
...
### [41] [AION-1: Omnimodal Foundation Model for Astronomical Sciences](https://arxiv.org/abs/2510.17960)
...
**TL;DR**: 提出了AION-1天文多模态基础模型（foundation model），通过两阶段架构整合成像、光谱和标量数据，在五大巡天数据集上预训练后，能在多种下游任务中取得优异性能。


<details>
  <summary>Details</summary>

Background: 天文学领域缺乏统一框架来联合建模高度多样化的数据模态（data modalities），亟需构建多模态基础模型（foundation models）以实现跨模态整合。

Data: 数据来源于五个大规模巡天项目：Legacy Survey、Hyper Suprime-Cam (HSC)、Sloan Digital Sky Survey (SDSS)、Dark Energy Spectroscopic Instrument (DESI)和Gaia。

Method: 采用两阶段架构（modality-specific tokenization和transformer-based masked modeling）整合多模态天文数据（成像、光谱和标量数据），并基于五大巡天数据集（Legacy Survey、HSC、SDSS、DESI和Gaia）进行预训练。

Result: AION-1通过两阶段架构（模态特定标记化与基于Transformer的跨模态掩码建模）整合了成像、光谱和标量数据，在星系与恒星属性估计、形态分类等下游任务中表现优异。
...
### [42] [Close-in faint companions mimicking interferometric hot exozodiacal dust observations](https://arxiv.org/abs/2510.18047)
*Katsiaryna Tsishchankava,Florian Kirchschlager,Anton Krieger,Thomas A. Stuber,Sebastian Wolf*

**TL;DR**: 通过模拟和观测验证发现，微弱伴星（faint companion）在闭合相位（closure phase）中可能未被检测但仍能导致可见度缺失，挑战了热系外黄道尘（hot exozodiacal dust）与伴星的传统区分标准，并提出了新的鉴别方法。


<details>
  <summary>Details</summary>

Background: 邻近主序星的干涉观测中意外红外超的起源问题，涉及热系外黄道尘（hot exozodiacal dust）与暗弱伴星的竞争性解释。

Data: 使用甚大望远镜干涉仪（VLTI）的PIONIER、GRAVITY和MATISSE三种仪器及四种望远镜配置对2天文单位（au）×2天文单位视场内的恒星和暗弱伴星系统进行建模，并计算可见度与闭合相位数据。

Method: 采用恒星与微弱伴星系统模型，结合三种VLTI仪器（PIONIER、GRAVITY和MATISSE）及四种望远镜配置，计算了视场2au×2au内的可见度与闭合相位。

Result: 研究结果表明，即使闭合相位（closure phase）接近零，伴星仍可能因显著的非探测概率（non-detection probability）未被发现，但可产生可测量的可见度亏损（visibility deficit），这挑战了以往基于近零闭合相位和高斯分布尘埃密度（Gaussian-distributed dust densities）的伴星排除标准。
...
### [43] [Spectroscopic study of the light-polluted night sky in Hong Kong](https://arxiv.org/abs/2510.18200)
*Chu Wing So,Chun Shing Jason Pun,Shengjie Liu*

**TL;DR**: 首次通过城市和郊区12,000多组天顶光谱数据证实城市夜空辉光(skyglow)主要由紧凑型荧光灯和高压钠灯等人造光源主导，并发现商业LED和金属卤化物灯在午夜前对城区贡献更显著，同时揭示了单个霓虹灯广告牌对周边环境的显著影响，为制定光污染治理策略提供了关键光谱依据。


<details>
  <summary>Details</summary>

Background: 城市夜空光谱特性研究填补了人工光源对夜空辉光（skyglow）影响的知识空白，聚焦于光谱特征强度变化揭示的户外照明演变模式及其光污染效应。

Data: 使用紧凑型光谱仪在2021至2023年间于城市和郊区站点采集了超过12,000条天顶光谱（zenith sky spectra）数据。

Method: 采用紧凑型光谱仪在2021至2023年间采集超过12,000条城市及郊区天顶光谱数据，通过分析人工光源特征发射谱线强度变化揭示天空辉光（skyglow）组成。

Result: 城市夜空辉光（skyglow）主要由紧凑型荧光灯（compact fluorescent lamps）和高压钠灯（high-pressure sodium lamps）的人工光源排放主导，商业控制的照明（如采用发光二极管（light-emitting diode）和金卤灯（metal halide）技术的泛光和广告照明）在午夜前的城市区域贡献更为显著。
...
### [44] [Unveiling the nature of HE 0107-5240](https://arxiv.org/abs/2510.18498)
...
**TL;DR**: 研究发现极贫铁星（iron-poor star）HE 0107-5240（[Fe/H]=-5.56）是一个轨道周期约29年的长周期双星系统，其化学丰度特征符合碳增强贫金属且无重元素增强（CEMP-no）的分类，并质疑了该恒星铁元素局部热力学平衡（LTE）假设的偏离修正。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系极贫金属星（metal-poor stars）的化学丰度特征及其对早期宇宙恒星形成和超新星核合成过程的示踪作用。

Data: 数据来源于ESPRESSO光谱仪对HE 0107-5240的径向速度监测以及补充的UVES蓝光波段光谱。

Method: 采用ESPRESSO光谱仪进行视向速度监测，结合G波段特征的模板互相关方法确定轨道参数，并通过叠加UVES蓝光波段光谱补充波长覆盖范围。

Result: HE 0107-5240的轨道周期经修订约为29年，确认其为长周期双星系统。
...
### [45] [Constraining a de Broglie-Bohm quantum bounce cosmology with Planck data](https://arxiv.org/abs/2510.18022)
*Micol Benetti,Rudnei O. Ramos,Renato Silva,Gustavo S. Vicente*

**TL;DR**: 基于德布罗意-玻姆(de Broglie-Bohm)量子力学解释构建了无奇点反弹宇宙模型，发现原初功率谱因反弹区量子引力动力学产生特征调制，与普朗克2018数据高度吻合，并通过反弹尺度$k_B$上限约束了参数空间，其谱指数与扰动幅度的尺度依赖反相关性为缓解$H_0$-$\sigma_8$张力提供了可检验机制。


<details>
  <summary>Details</summary>

Background: 研究探讨了量子宇宙学（quantum cosmology）框架下无奇点的早期宇宙模型，重点解决经典大爆炸理论中的初始奇点问题。

Data: 研究使用了普朗克2018年宇宙微波背景辐射（cosmic microwave background）数据来验证模型。

Method: 采用德布罗意-玻姆解释（de Broglie-Bohm interpretation）构建量子反弹模型，通过确定性的标度因子轨迹消除奇点，并引入反弹附近量子引力动力学导致的特征调制（畸变函数）推导标量扰动的原初功率谱。

Result: 基于德布罗意-玻姆诠释(de Broglie-Bohm interpretation)构建的量子反弹模型消除了早期宇宙奇点，并通过原初功率谱中的特征调制（畸变函数）反映了反弹附近的非标准量子引力动力学效应。
...
### [46] [Comprehensive analysis of dissipative effects in the induced gravitational waves](https://arxiv.org/abs/2510.18663)
*Yan-Heng Yu,Zhe Chang,Sai Wang*

**TL;DR**: 研究发现宇宙流体耗散效应在波数$k \sim k_{\mathcal{H},\mathrm{dec}}$（对应弱相互作用粒子退耦时的视界尺度）对曲率扰动诱导的引力波（IGWs）产生显著影响，表现为谱上"双谷"结构抑制和红外区对数跑动消失等特征，并揭示中微子耗散在纳赫兹频段对脉冲星计时阵列分析的重要性，同时提出耗散特征可作为探测极高能标新物理的探针。


<details>
  <summary>Details</summary>

Background: 研究主题为宇宙流体耗散（dissipation）对曲率扰动（curvature perturbations）诱导的引力波（induced gravitational waves, IGWs）的影响及其在早期宇宙和基本物理研究中的意义。

Data: 研究采用理论模型（simulation）分析耗散效应，重点关注标准模型（Standard Model）中微子及潜在新弱相互作用粒子对诱导引力波（IGWs）的影响。

Method: 采用相对论性流体力学（relativistic hydrodynamics）模型分析宇宙流体耗散对诱导引力波（IGWs）的影响，重点研究弱相互作用粒子退耦时的特征尺度（k_H,dec）附近耗散效应的动力学特征。

Result: 在曲率扰动诱导的引力波（IGWs）中，耗散效应会在波数$k \sim k_{\mathcal{H},\mathrm{dec}}$（对应弱相互作用粒子退耦时的视界尺度）处产生显著特征，包括“双谷”结构的抑制和红外区域对数跑动的缺失。
...
### [47] [A Physical Model for the Ice Coma of 3I/ATLAS](https://arxiv.org/abs/2510.18157)
*Eric Keto,Abraham Loeb*

**TL;DR**: 利用哈勃太空望远镜观测星际彗星3I/ATLAS发现其逆向彗尾（anti-tail）由CO₂升华喷射的H₂O冰晶主导散射形成，并建立模型揭示水冰晶散射截面在3-4天文单位（AU）处达到峰值，随着近日距离减小逐渐转变为由长寿命耐火物质和大颗粒挥发物主导的传统彗尾。


<details>
  <summary>Details</summary>

Background: 研究主题为星际彗星3I/ATLAS在接近太阳过程中由二氧化碳升华驱动的水冰颗粒散射物理机制及其亮度演化规律。

Data: 数据来源于哈勃太空望远镜(Hubble Space Telescope)2025年7月21日对星际彗星3I/ATLAS的高分辨率成像观测，以及后续凯克(Keck)和双子南座(Gemini South)天文台的观测数据。

Method: 基于彗星日心距离变化，建立了冰彗发中散射表观光亮度的物理模型，重点分析了水冰颗粒散射截面峰值在3-4天文单位(AU)出现的机制。

Result: 随着彗星接近太阳，冰晶(H$_2$O ice grains)升华率的指数温度依赖性导致其产生率持续增加而驻留时间急剧缩短，在3-4天文单位(AU)处形成散射截面峰值，更近时散射由长寿命的耐火物质(refractory)和大颗粒挥发物(volatile grains)主导形成常规彗尾。
...
### [48] [Pre-perihelion Development of Interstellar Comet 3I/ATLAS](https://arxiv.org/abs/2510.18769)
*David Jewitt,Jane Luu*

**TL;DR**: 通过观测星际彗星3I/ATLAS发现其尘埃产生率与日心距离呈rH**(-1.8+/-0.3)关系，主要由二氧化碳驱动活动，且主导尘埃颗粒有效半径为0.1毫米、喷射速度为5米/秒。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际彗星3I/ATLAS在近日点前的光学观测，分析其尘埃产生率与日心距离的幂律关系及活动驱动机制。

Data: 使用北欧光学望远镜(Nordic Optical Telescope)在2025年7月至9月期间对星际彗星3I/ATLAS进行了近日点前的光学观测。

Method: 采用固定孔径测光法（fixed aperture photometry）结合幂律函数拟合分析尘埃产率随日心距离的变化规律。

Result: 星际彗星3I/ATLAS的尘埃产生率与日心距离(heliocentric distance)呈rH**(-1.8+/-0.3)的比例关系变化，表明其活动主要由二氧化碳驱动。
...
### [49] [Deep Synoptic Array Science: Searching for Long Duration Radio Transients with the DSA-110](https://arxiv.org/abs/2510.18136)
...
**TL;DR**: 设计并测试了DSA-110非快速射电暴(NSFRB)实时搜索系统，将脉冲宽度探测范围扩展3个量级至134毫秒-160.8秒，以290毫 Jy灵敏度排除了95%探测区域内周期10-70秒的白矮星-M矮星双星长周期射电暂现源(LPRT)模型。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于扩展快速射电暴(FRB)搜索的脉冲宽度范围，以探测银河系长周期射电暂现源(LPRTs)并验证其理论模型。

Data: 数据来源于DSA-110射电望远镜对银河系平面（$-3.5^\circ<b<5.7^\circ$和$141^\circ<l<225^\circ$）的离线观测（offline Galactic Plane Survey），以及实时处理中使用连续谱源（continuum sources）和脉冲星B0329+54的灵敏度测试数据。

Method: 采用基于Python与JAX框架的定制GPU加速搜索代码\texttt{cerberus}实现实时图像平面单脉冲搜索，灵敏度覆盖134毫秒至160.8秒的射电暴。

Result: DSA-110 NSFRB搜索系统将快速射电暴(FRB)搜索的脉冲宽度范围扩展了3个数量级，能够探测134毫秒至160.8秒的射电暴，包括新发现的银河系长周期射电暂现源(LPRTs)。
...
### [50] [Assessing the performance of future space-based detectors: astrophysical foregrounds and individual sources](https://arxiv.org/abs/2510.18695)
*Alice Perego,Matteo Bonetti,Alberto Sesana,Silvia Toonen,Valeriya Korol*

**TL;DR**: 该研究评估了μAres、AMIGO和Decihertz Observatory三种后LISA引力波探测任务设计的观测潜力，发现它们能有效探测大质量黑洞双星、极端质量比旋进和致密双星等天体物理源产生的未解析引力波背景（GWB）及信号特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索后LISA时代空间引力波探测器（如μAres、AMIGO和Decihertz Observatory）在毫赫兹至分赫兹频段对天体物理引力波源（如大质量黑洞双星、极端质量比旋近系统和致密双星系统）的观测潜力。

Data: 数据来源包括文献中提出的三个任务设计（μAres、AMIGO和Decihertz Observatory）的模拟数据，以及通过迭代减法算法计算未解析引力波背景（GWB）时使用的天体物理引力波源（如大质量黑洞双星、极端质量比旋进和致密双星系统）的预期群体数据。

Method: 采用迭代减法算法（iterative subtraction algorithm）计算了未解析引力波背景（GWB），并评估了不同探测器对致密双星、极端质量比旋近（EMRI）和大质量黑洞双星等天体物理引力波源的探测潜力。

Result: LISA（Laser Interferometer Space Antenna）计划于2035年发射，旨在探测毫赫兹频段的引力波（GW）信号。
...
### [51] [Degeneracy-Aware Pulsar Parameter Estimation from Light Curves via Deep Learning and Test-Time Optimization](https://arxiv.org/abs/2510.18807)
*Abu Bucker Siddik,Diane Oyen,Soumi De,Greg Olmschenk,Constantinos Kalapotharakos*

**TL;DR**: 提出一种结合定制损失函数和测试时优化的深度学习框架，通过脉冲星（pulsar）光变曲线反演其多组简并参数（degenerate parameters），解决了传统方法无法捕捉观测数据中参数简并性的问题。


<details>
  <summary>Details</summary>

Background: 探索极端环境中最高密度物质行为及中子星（neutron star）磁场的产生与演化机制。

Data: 使用了一套最先进的模拟脉冲星（pulsar）光变曲线数据。

Method: 采用深度学习框架，结合自定义损失函数（包含光变曲线模拟器作为前向模型）和差异损失函数，以推断脉冲星参数并捕捉多组简并解。

Result: 提出了一种结合定制损失函数和测试时优化的深度学习框架，能够从观测到的脉冲星（pulsar）光变曲线中推断出多种退化的参数集。
...
### [52] [Discovery of Powerful Multi-Velocity Ultra-Fast Outflows in the Starburst Merger Galaxy IRAS 05189$-$2524 with XRISM](https://arxiv.org/abs/2510.18027)
...
**TL;DR**: 利用XRISM首次在IRAS 05189-2524中探测到多组分超快外流(UFO)，其具有0.076c、0.101c和0.143c三种体速度，并发现P Cygni轮廓特征，表明该星系合并后短暂演化阶段已形成强大多速度外流，将抑制恒星形成并演化为类星体。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超亮红外星系(ultra-luminous infrared galaxy)IRAS 05189-2524在星系合并后短暂演化阶段产生的多速度超快外流(ultra-fast outflow)及其对星系演化的影响。

Data: 数据来源于XRISM（X射线成像和光谱任务）在性能验证阶段对IRAS 05189-2524的观测，包括搭载的X射线微量热计（X-ray microcalorimeter）和X射线CCD相机（X-ray CCD camera）获取的高分辨率光谱和宽带能谱数据。

Method: 采用X射线微量热计（X-ray microcalorimeter）和X射线CCD相机进行高能谱分析，结合多普勒频移（blueshifted）吸收线和P Cygni轮廓（P Cygni profile）建模，识别出超快外流（UFO）的多重速度分量。

Result: 在IRAS 05189$-$2524中发现了多组分超快外流（UFO），其体速度分别为$\sim0.076c$、$\sim0.101c$和$\sim0.143c$，并检测到与吸收线共同形成P Cygni轮廓的宽Fe-K发射线。
...
### [53] [Line-force driven wind from a thin disk in tidal disruption event](https://arxiv.org/abs/2510.18657)
*De-Fu Bu,Xiao-Hong Yang,Liang Chen,Chenwei Yang,Guobin Mou*

**TL;DR**: 研究发现潮汐瓦解事件(TDEs)中围绕百万太阳质量黑洞的亚爱丁顿薄吸积盘(accretion disk)可通过谱线力(line force)驱动速度达0.3倍光速的强风(wind)，其动能功率为爱丁顿光度(Eddington luminosity)的1%-6%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于潮汐瓦解事件(TDEs)中次爱丁顿吸积盘(Sub-Eddington accretion disk)产生的风(wind)特性及其对TDE辐射的影响机制。

Data: 采用PLUTO代码对潮汐瓦解事件(TDEs)中环绕10^6太阳质量黑洞的环化亚爱丁顿吸积盘进行二维流体力学模拟，研究线力驱动盘风。

Method: 采用二维流体力学模拟（PLUTO代码）研究潮汐瓦解事件（TDEs）中环化亚爱丁顿吸积盘的线力驱动（line force driven）盘风。

Result: 潮汐瓦解事件(TDEs)中薄吸积盘产生的线力驱动风速度可达光速的0.3倍，其动能功率可达爱丁顿光度(Eddington luminosity)的1%-6%。
...
### [54] [Gravitational-wave and electromagnetic detections in the context of the CosmoDC2 LSST synthetic catalog](https://arxiv.org/abs/2510.18727)
...
**TL;DR**: 发布了CosmoDC2_BCO合成目录（包含引力波事件和CosmoDC2星系电磁对应体），发现第三代引力波探测器网络（CE+ET+LVK）与第二代结合可显著提升双中子星并合（BNS）的定位精度，预计10年内LSST类巡天能探测约5000例千新星（kilonovae），但仅占第三代网络预期探测总量的极小部分。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用第三代引力波探测器网络（如CE+ET+LVK）与光学巡天（如LSST）协同观测双中子星并合（BNS mergers）及其电磁对应体（kilonovae），以提升事件探测率、参数估计精度及天空定位能力。

Data: 数据来源于CosmoDC2合成的引力波事件和电磁对应体目录CosmoDC2_BCO，包含本征和外源参数、信噪比、参数不确定性、天区定位面积及LSST滤波器下的千新星视星等。

Method: 采用引力波事件与电磁对应体（electromagnetic counterparts）的合成目录（CosmoDC2_BCO）进行参数估计，结合第三代探测器网络（CE+ET+LVK）与第二代探测器（LVK）的协同观测，通过信噪比（signal-to-noise ratios）、天区定位（sky localization areas）和千新星（kilonova）视星等分析提升双中子星并合（BNS mergers）的探测效率。

Result: 第三代引力波探测器网络显著提高了探测率并改善了参数估计精度。
...
### [55] [Ultra-Strongly Self-Interacting Dark Matter: From Phenomenology to Astrophysical Observables](https://arxiv.org/abs/2510.18142)
*M. Grant Roberts,Wolfgang Altmannshofer,Pierce Giffin,Stefano Profumo*

**TL;DR**: 提出了一个由主导的中等自相互作用暗物质(SIDM)和超强自相互作用子成分(uSIDM)构成的双组分模型，通过轻矢量媒介实现速度依赖的自散射，并发现该模型能在满足星系旋转曲线和星系团透镜观测约束的同时，解释矮星系速度尺度下有效截面(σ_eff/m=20-40 cm²/g)与星系团尺度上限(σ_eff/m<0.13 cm²/g)的差异，且uSIDM亚组分可驱动早期晕的快速引力热坍缩，形成高红移类星体与"小红点"的种子，其线性功率谱截断特征与现有Lyα森林和卫星计数数据兼容但存在可观测的非标准特征。


<details>
  <summary>Details</summary>

Background: 研究探讨了双组分自相互作用暗物质（SIDM）的简化可测试框架，其中主导的中等自相互作用组分与超强自相互作用子组分（uSIDM）共存，旨在解决暗物质微观物理与宇宙学及天体物理观测间的关联问题。

Data: 数据来源包括矮星系和低表面亮度星系（low surface brightness galaxy）的旋转曲线观测数据，以及强星系团（strong cluster）的引力透镜观测数据。

Method: 采用双组分自相互作用暗物质（SIDM）框架，结合轻矢量媒介诱导的速度依赖自散射和早期宇宙动力学（包括标准$2 \to 2$湮灭及$\chi_1\chi_1 \to \chi_2\chi_2$相互转换）解析计算遗迹丰度，并通过\texttt{CLASS}演化线性功率谱。

Result: 提出了一种双组分自相互作用暗物质（SIDM）模型，其中主导组分具有中等自相互作用而次要组分（uSIDM）具有极强自相互作用，并通过轻矢量媒介实现速度依赖的散射过程。
...
### [56] [The Muonic Portal to Vector Dark Matter:connecting precision muon physics, cosmology, and colliders](https://arxiv.org/abs/2510.18564)
*Alexander Belyaev,Luca Panizzi,Nakorn Thongyoi,Franz Wilhelm*

**TL;DR**: 该论文提出了一种通过重μ子（vector-like muons）连接暗物质与标准模型的最小扩展模型（MPVDM），发现其能同时解释暗物质残留丰度和μ子反常磁矩（a_mu），并揭示了轻质量（<1 GeV）矢量暗物质（vector dark matter）在非共振速度抑制机制下避开宇宙微波背景（CMB）限制的关键特性。


<details>
  <summary>Details</summary>

Background: 研究探讨了标准模型扩展框架下通过重味轻子（vector-like muons）连接暗物质与μ子物理的μ子门户矢量暗物质（MPVDM）模型。

Data: 数据来源于对ATLAS和CMS探测器μ+μ-+E_T^miss（缺失横动量）信号的重新分析，以及五维参数扫描（five dimensional parameter scan）的数值模拟结果。

Method: 采用SU(2)_D规范对称性扩展标准模型，引入矢量态μ子(vector-like muons)作为暗区与标准模型唯一耦合通道，通过五维参数扫描分析近共振区速度抑制机制。

Result: MPVDM（Muonic Portal to Vector Dark Matter）模型通过新的SU(2)_D规范对称性和矢量缪子（vector-like muons）将暗物质与缪子物理联系起来，能同时解释暗物质残留丰度和缪子反常磁矩（a_mu）的观测结果。
...
### [57] [Constraints on Attractor Models of Inflation and Reheating from Planck, BICEP/Keck, ACT DR6, and SPT-3G Data](https://arxiv.org/abs/2510.18656)
*John Ellis,Marcos A. G. Garcia,Keith A. Olive,Sarunas Verner*

**TL;DR**: 通过分析普朗克2018、BICEP/Keck 2018、ACT DR6和SPT-3G数据对宇宙微波背景(CMB)的约束，发现E模型(α-Starobinsky)在α≲25时与观测相符，典型Starobinsky模型(α=1)预测的标量谱指数(ns=0.958-0.963)与普朗克2018数据吻合，而T模型预测的ns值(0.956-0.961)与观测存在一定张力且α上限为11，并证明广义α吸引子模型中k≥6的单极势可自然预测ns≈0.965-0.968，与ACT DR6数据更吻合。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用宇宙微波背景辐射(CMB)观测数据检验暴胀理论中的吸引子模型(E-model和T-model)，探讨其与不同再加热温度下理论预测的标量谱指数(n_s)和张标比(r)的兼容性。

Data: 数据来源包括普朗克卫星2018年数据（Planck 2018）、BICEP/Keck 2018年数据、阿塔卡马宇宙学望远镜第六次数据发布（ACT DR6）和南极望远镜（SPT-3G）数据。

Method: 采用宇宙微波背景(CMB)观测数据约束E模型($\alpha$-Starobinsky)和T模型势能的理论预测，并扩展到广义$\alpha$-吸引子单极势模型和变形E/T模型的分析方法。

Result: 观测约束支持E模型（E-model）参数α≲25，标准Starobinsky模型（α=1）预测的标量谱指数（scalar spectral index）ns=0.958-0.963与Planck 2018数据吻合，且在Planck-ACT-SPT组合的95%置信区间内。
...
### [58] [Ultraviolet Completion of the Big Bang in Quadratic Gravity](https://arxiv.org/abs/2510.18733)
*Ruolin Liu,Jerome Quintin,Niayesh Afshordi*

**TL;DR**: 提出了一种紫外渐进自由的量子二次引力（quantum quadratic gravity）暴胀模型，通过1圈跑动动态产生红外慢滚暴胀，在大量物质场贡献下获得符合观测的谱指数和张标比，预测最小张标比为0.01，并揭示暴胀结束时理论进入强耦合区而必须退化为有效场论形式的一般相对论以实现再加热。


<details>
  <summary>Details</summary>

Background: 研究探讨了量子二次引力（quantum quadratic gravity）框架下的暴胀场景，该理论在紫外区渐进自由且通过1圈跑动动态引发慢滚暴胀，旨在解决Starobinsky暴胀与新宇宙学观测的冲突。

Data: 数据来源于量子二次引力（quantum quadratic gravity）理论的1-loop动态运行模拟（simulation），通过大量物质场对β函数的贡献来验证光谱指数和张量-标量比的可行性。

Method: 采用量子二次引力理论（quantum quadratic gravity）构建渐近自由紫外完备模型，通过1圈跑动（1-loop running）动态产生红外慢滚暴胀（slow-roll inflation），并预测张标比（tensor-to-scalar ratio）下限为0.01以避免强耦合。

Result: 提出的量子二次引力暴胀（quantum quadratic gravity inflationary）模型在紫外区渐进自由，但单圈跑动会动态导致红外区的慢滚暴胀（slow-roll inflation），当大量物质场参与贝塔函数（beta functions）时，谱指数和张标比（tensor-to-scalar ratio）可达观测要求。
