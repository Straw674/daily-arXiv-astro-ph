...
### [1] [Persephone's Torch: A 15th Magnitude Quadruply-Lensed Quasar From the Couch Discovered with SPHEREx and the LBT](https://arxiv.org/abs/2604.13152)
*Frederick B. Davies,Eduardo Bañados,Sarah E. I. Bosman,Arpita Ganguly,Silvia Belladitta,Jennifer Power,Jon Rees*

**TL;DR**: 该研究通过SPHEREx任务的光谱测量和LBT/LUCI的自适应光学成像，确认了红移为2.22、极为明亮（$i=14.77$）且紧凑（爱因斯坦半径约0.45角秒）的四重透镜类星体（quasar）J1330-0905（Persephone's Torch），发现其图像亮度存在高度异常的通量比，是迄今发现的最亮引力透镜类星体系，为未来微引力透镜（microlensing）研究提供了理想候选目标。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用先进观测技术（如SPHEREx任务的红外分光测光）发现和确认宇宙中极端明亮的引力透镜天体，以探索高红移类星体（quasar）的物理特性及透镜质量分布。

Data: 研究使用了来自SPHEREx任务的公共分光光度测量数据（"from the couch"）以及LBT/LUCI的自适应光学成像数据。

Method: 采用自适应光学成像（adaptive optics imaging）和椭圆幂律质量分布加外部剪切（elliptical power-law mass distribution plus external shear）的透镜模型进行几何构型分析和光度异常（anomalous flux ratios）研究。

Result: 该研究通过SPHEREx任务的光谱测定和LBT/LUCI的自适应光学成像，确认了J1330−0905（Persephone's Torch）是一个红移为2.22、亮度极高（i=14.77）、结构紧凑（爱因斯坦半径约0.45角秒）的四重成像透镜类星体（quadruply-lensed quasar），其图像呈“圆形风筝”构型，并且是该类系统中迄今发现的最亮系统。
...
### [2] [From Clumps to Sheets: Geometry Controls the Temperature PDF of Multi-Phase Gas](https://arxiv.org/abs/2604.13154)
*Zirui Chen,S. Peng Oh*

**TL;DR**: 本文通过对比星际介质（ISM）和星系周介质（CGM）条件下的模拟，揭示了温度概率分布函数（PDF）的差异主要由几何形态（如片状混合层与团块状湍流介质）而非微观物理过程决定，并提出了温度PDF可分解为等温面面积与层厚度的乘积的理论框架。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是探讨多相湍流气体（如星际介质和星系周介质）中温度概率分布函数（PDFs）的形成机制及其与几何形态的关联。

Data: 研究中使用的数据来源于在星际介质（ISM）和环星系介质（CGM）条件下进行的3D流体动力学模拟（simulation）。

Method: 研究采用三维流体动力学模拟（3D hydrodynamic simulations）比较了平面混合层（planar mixing layers）与湍流盒模拟（turbulent-box simulations）在相同微观物理条件下的温度概率分布函数差异。

Result: 温度概率分布函数（PDF）的差异主要由几何形态决定，其中等温面面积由形态控制，而层厚度由微观物理过程控制。
...
### [3] [A Dynamical Test for Cooling-Induced Entrainment in a Runaway Supermassive Black Hole Tail](https://arxiv.org/abs/2604.13155)
*Ish Kaul,S. Peng Oh*

**TL;DR**: 通过对RBH-1（首个确认的失控超大质量黑洞）尾迹的模拟，该研究首次为辐射湍流混合层（radiative turbulent mixing layers）理论提供了定量的动力学验证，证实其能解释冷气体在热流中形成相干尾迹并减速的现象。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是检验辐射湍流混合层（radiative turbulent mixing layers）理论在解释热天体物理流中冷气体（cold gas）存活、增长和卷吸过程方面的有效性。

Data: 使用三维流体动力学模拟（3D hydrodynamical simulations）结合湍流混合层理论（turbulent mixing-layer theory）对下游尾迹进行建模。

Method: 采用三维流体动力学模拟结合湍流混合层理论对下游尾迹进行建模。

Result: 通过模拟和理论分析发现，观测到的下游减速可由辐射混合层（radiative mixing layer）的吸积诱导阻力很好再现，且若无辐射冷却则无法形成连贯的冷尾流。
...
### [4] [Resolving circumgalactic gas flows around a z$\approx$3.6 quasar using MUSE and ALMA](https://arxiv.org/abs/2604.13159)
...
**TL;DR**: 该研究通过对高红移类星体MUSE Quasar Nebula 04（MQN04）的多波段观测，发现其周围存在延展约100千秒差距、速度蓝移约0-800 km/s的高度不对称电离气体晕，并揭示该区域是红移z≈3.66时期已知最密集的星系形成环境之一。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是星系形成与演化过程中气体与周围大尺度结构（circum- and intergalactic scales）的交换。

Data: 研究中使用的观测数据来源于高分辨率MUSE（Multi Unit Spectroscopic Explorer）对非共振HeII发射的探测，以及ALMA（Atacama Large Millimeter/submillimeter Array）对CO(4-3)谱线的精确测量。

Method: 采用高分辨率MUSE观测非共振HeII发射线并结合ALMA CO(4-3)线精确测定类星体系统红移，以研究类星体附近约1千秒差距内的冷却电离气体运动学。

Result: 该研究通过结合MUSE对非共振HeII发射的高分辨率探测和ALMA对CO(4-3)谱线的精确测量，发现围绕类星体MQN04的冷电离气体存在延伸约100千秒差距、相对类星体系统红移蓝移约0-800公里/秒的复杂团块结构和弥散发射，表明其环星系介质（circumgalactic medium）高度不对称。
...
### [5] [Investigating the Circumgalactic Medium through Mg II absorption coincidence](https://arxiv.org/abs/2604.13162)
*Paryag Sharma,Raghunathan Srianand,Hum Chand,Labanya Kumar Guha*

**TL;DR**: 基于对9204个类星体Mg II吸收体对（Mg II absorber）的统计分析，该研究发现其横向相干性在约100千秒差距（kpc）以下由单晕（single-halo）模型主导，而在更大尺度上则转为星系成团（galaxy clustering）主导，揭示了富金属冷环星系介质（CGM）的相干尺度约为100-200千秒差距。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是探索星系晕（halo）和星系际介质中金属富集冷气体（cool, metal-enriched CGM）的空间分布与横向相干性。

Data: 研究使用的数据来源于斯隆数字巡天（Sloan Digital Sky Survey）中的9204个以吸收体为中心的类星体视线对样本。

Method: 采用统计测量方法分析吸收体横向相干性，包括巧合概率曲线拟合、单晕（single-halo）几何模型与星系聚类贡献的对比，并通过叠加（stacking）分析验证金属富集冷气体晕（CGM）的相干尺度。

Result: 该研究发现Mg II吸收体横向相干性随投影距离呈现双区结构：在小于约100千秒差距（kpc）的尺度上相干概率显著升高至约5-8%，而在更大尺度上则迅速下降并维持在约1-2%的低平台水平。
...
### [6] [Euclid: Quick Data Release (Q1) -- Dual AGN in low-mass galaxies](https://arxiv.org/abs/2604.13170)
...
**TL;DR**: 该研究利用多波段数据首次在低质量（stellar mass < 10¹⁰ M☉）星系中发现了9个光谱证实的双活动星系核（dual AGN）候选体，其投影距离为20-51千秒差距，并得出低质量星系的双活动星系核比例为0.1%，这为黑洞由低质量星系中的小黑洞合并增长以及低频引力波预测提供了依据。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是探索低质量星系中双活动星系核（dual AGNs）的存在及其在星系演化、超大质量黑洞（supermassive black holes）形成和引力波（gravitational waves）源预测中的意义。

Data: 研究使用了来自欧几里得望远镜（Euclid）快速数据发布的光度测量和光谱数据，并结合了暗能量光谱仪（DESI）、低频阵列高波段天线（LOFAR）以及X射线和中红外星表中的多波长数据。

Method: 研究结合了欧几里得（Euclid）快速数据发布的光度与光谱数据、暗能量光谱仪（DESI）的多波段数据、低频阵列（LOFAR）高波段天线数据以及X射线和中红外目录的对应数据，以识别红移$z \lesssim 1$的双活动星系核（AGNs）。

Result: 该研究首次在低质量星系中发现了九个光谱证实的双活动星系核（dual AGN）候选体，其投影分离范围约为20至51千秒差距。
...
### [7] [Targeted search for eccentric supermassive binary black holes in OJ 287 and nearby galaxy clusters with PPTA DR3](https://arxiv.org/abs/2604.13173)
...
**TL;DR**: 本研究通过对OJ 287等六个目标方向进行贝叶斯搜索，未发现显著连续引力波信号，并首次利用脉冲星计时阵列数据结合轨道偏心率，对超大质量双黑洞（SMBBHs）的总质量（如OJ 287处小于5.25×10¹⁰ M⊙）和质量比（如排除M87和NGC 4889中q≳10⁻²的等质量系统）给出了新的观测约束。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是利用脉冲星计时阵列（PTA）对超大质量双黑洞（SMBBH）产生的连续引力波进行探测，以研究宇宙中的超大质量黑洞并合现象与星系演化。

Data: 研究使用的数据来自帕克斯脉冲星计时阵列第三次数据发布（PPTA DR3）。

Method: 采用贝叶斯定向搜索（Bayesian targeted searches）方法，并显式纳入轨道偏心率以稳健捕获分布在多个谐波上的信号功率。

Result: 基于PPTA DR3数据对OJ 287等目标进行贝叶斯搜索，未发现显著连续引力波信号，但对OJ 287的总质量给出了严格上限（$M_{\rm tot} < 5.25 \times 10^{10} M_{\odot}$，95%置信水平），并对M87等星系中潜在超大质量双黑洞（SMBBH）的质量比（$q$）设定了新约束，排除了参数空间中质量比$q \gtrsim 10^{-2}$（约10 nHz处）的等质量双黑洞并合。
...
### [8] [4MOST ChANGES: Catalog of high-redshift quasar candidates (4.5 < $z$ < 7) selected with SED fitting](https://arxiv.org/abs/2604.13178)
...
**TL;DR**: 本研究通过结合多波段测光数据与光谱能量分布（SED）拟合方法，首次在南方天区构建了一个包含6104个高红移（$4.5 < z < 7$）类星体候选者的星表，并通过光谱观测成功确认了其中3个红移大于5的类星体。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是探索早期宇宙、研究超大质量黑洞（Supermassive Black Hole）的增长以及宇宙再电离（cosmic reionization）过程。

Data: 研究使用的数据来源包括：光学数据来自DELVE DR2和DECaLS DR10，近红外数据来自VHS DR5及额外的VIKING天区，中红外数据来自AllWISE，光学天体测量数据来自Gaia DR3。

Method: 本研究采用的核心分析方法是结合类星体和褐矮星模板进行能谱分布（SED）拟合，并利用包括卡方（$χ^2$）、贝叶斯信息准则（BIC）和F检验在内的统计输出来对候选体进行排序与筛选。

Result: 通过多波段测光与光谱能量分布拟合，在4.5 < z < 7范围内筛选出6104个高红移（high-redshift）类星体候选体，其中已观测的6个候选体中有3个在红移大于5处得到光谱确认。
...
### [9] [Galactic Rain: Cool Gas Inflows in Red Geyser Galaxies and Their Connection to AGN Activity and Interactions](https://arxiv.org/abs/2604.13195)
*Arian Moghni,Namrata Roy,Timothy M. Heckman,Kevin Bundy,Kyle B. Westfall,Kate H. R. Rubin*

**TL;DR**: 该研究通过对红喷流星系（red geysers）中性钠吸收（Na I D）的观测，发现其中约70%的冷气体以约47 km/s的速度流入，且流入气体规模在射电探测或受环境影响的星系中显著更大，表明星系相互作用通过补充冷气体、激发活动星系核（AGN）反馈来维持长期宁静状态。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于红喷流星系（red geysers）中低水平活动星系核（AGN）反馈对冷气体吸积与流出的调控作用及其与星系环境和射电活动的关联。

Data: 研究使用的数据来源于斯隆数字巡天第四期（Sloan Digital Sky Survey-IV）的“阿帕奇点天文台附近星系测绘”（Mapping Nearby Galaxies at Apache Point Observatory，MaNGA）项目，具体为其中的140个红色间歇泉（red geyser）星系样本。

Method: 研究采用双高斯拟合（double-Gaussian fits）方法对Na I D双线进行空间分辨的速度和弥散测量。

Result: 研究发现红色间歇泉星系（red geysers）中约70%的冷中性气体（通过Na I D吸收示踪）以约47 km/s的中位速度流入，且其吸收比匹配对照样本更普遍（探测比例63%对40%，储层面积大1.6倍）。
...
### [10] [Sparks: The Magellan/FIRE survey from starburst to post-starburst](https://arxiv.org/abs/2604.13211)
*Dalya Baron,David J. Setton,Yilun Ma,J. X. Prochaska,Gabriela Canalizo,Ric Davies,Jenny E. Greene,Dieter Lutz*

**TL;DR**: 这项研究通过Sparks红外巡天对93个本地大质量星系进行全波段分析，发现具有强巴尔默吸收线和活动星系核（AGN）的星系主要处于第二次主要星暴阶段，这解释了其远红外辐射增强现象，并暗示了星暴与黑洞吸积之间存在短暂延迟。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是星系形成中从星暴（starburst）到静止状态的快速转变过程，特别是对后星暴星系（post-starburst galaxies）的观测研究。

Data: 本研究使用的数据来源于Sparks巡天项目，该项目利用麦哲伦望远镜上的FIRE光谱仪，获取了93个本地大质量星系（涵盖从星暴到静止后星暴星系，包括活动星系核宿主）的0.82-2.51微米近红外光谱，并综合了从远紫外到远红外波段的测光数据。

Method: 采用覆盖远紫外至远红外的测光数据进行恒星种群合成拟合（stellar population synthesis fitting），以测量星暴历史（star formation history）和星暴后淬灭状态。

Result: 基于全波段测光数据的新星形成率（SFR）和恒星形成历史测量将样本分为三组：正在进行首次主要星暴的星系、正在进行第二次主要星暴的星系以及爆发后正在熄灭的星系。
...
### [11] [Sparks II: Panchromatic SED modeling and galaxy physical properties across the starburst to post-starburst sequence](https://arxiv.org/abs/2604.13216)
*Dalya Baron,David J. Setton,Yilun Ma,J. X. Prochaska,Ric Davies,Jenny E. Greene,Dieter Lutz*

**TL;DR**: 该研究通过对比光学连续谱与全波段SED拟合，发现对93个处于星暴至后星暴过渡阶段的本地大质量星系（包括巴耳末强星系和AGN），全波段SED能更可靠地获取其恒星形成率（SFR）和活动星系核（AGN）活动信息，并揭示许多此类星系的实际恒星形成水平高于先前基于光学谱的推断。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过多波段光谱能量分布（SED）拟合，评估并优化对处于星暴（starburst）至后星暴（post-starburst）快速过渡阶段的本地大质量星系中恒星形成率（SFRs）、恒星形成历史（SFHs）及活动星系核（AGN）活动等模型依赖性物理参数的可靠提取方法。

Data: 该研究使用的数据包括Sparks样本的远紫外至远红外多波段测光数据（panchromatic SED）以及光学连续谱光谱数据。

Method: 采用的核心分析方法包括利用从远紫外到远红外的全波段光谱能量分布（SED）进行拟合，并基于Prospector代码测试了AGN环面模型（torus model）。

Result: 基于全波段SED拟合的恒星形成率（SFR）比仅基于光学连续谱拟合的结果更可靠，且揭示了许多复合星系与活动星系核（AGN）寄主星系的恒星形成水平高于先前推断。
...
### [12] [A spatial and kinematical reinterpretation of Gould's Belt in light of Gaia](https://arxiv.org/abs/2604.13225)
*Michelangelo Pantaleoni González,João Alves,Cameren Swiggum,Isak Niederbrunner*

**TL;DR**: 基于盖亚（Gaia）DR3数据对年轻大质量恒星和邻近年轻星团的分析表明，古尔德带（Gould's Belt）并非一个独立的、连贯的动力学结构，而是由数个星团家族（如$α$Per、Cr135、M6和$γ$Vel）的短暂空间排列、拉德克利夫波（Radcliffe Wave）的振荡模式、太阳相对运动以及局部静止标准（LSR）的历史假设共同投影形成的三维星群（3D asterism）。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于重新评估古尔德带（Gould's Belt）这一长期存在的天文学概念，探讨其作为太阳系附近恒星与年轻星团分布结构的物理本质。

Data: 研究结合了盖亚卫星第三次数据发布（Gaia DR3）的观测数据、ALS III星表中OB型恒星样本以及年龄小于70 Myr的邻近年轻星团同质样本。

Method: 研究结合ALS III星表与年龄小于70 Myr的年轻星团样本，在现实的银河系势场中追踪其运动轨迹，以分析其空间分布与运动学特征。

Result: 古尔德带（Gould's Belt）并非一个独立的物理结构，而是由多个年轻星团家族（如αPer、Cr135、M6和γVel）的叠加、太阳反射运动、观测偏差以及投影效应共同形成的一个三维星群（3D asterism）。
...
### [13] [A Unified HI Rotation Curve Corpus for Computational Astrophysics: 438 Galaxies from SPARC, THINGS, LITTLE THINGS, and WALLABY DR2](https://arxiv.org/abs/2604.13489)
*David C. Flynn*

**TL;DR**: 该研究整合了来自SPARC、THINGS、LITTLE THINGS和WALLABY DR2四大巡天的423个星系（共438个条目）的8,963个空间分辨中性氢（HI）旋转曲线（rotation curve）测量数据，构建了一个统一、结构化且公开可用的数据集，旨在支持传统数值分析和大型语言模型检索增强生成（RAG）流程。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于构建一个用于星系旋转曲线（rotation curve）分析的综合数据资源。

Data: 数据来源于四个主要巡天项目：SPARC（175个星系）、THINGS（34个星系）、LITTLE THINGS（26个星系）和WALLABY DR2（203个星系）。

Method: 研究采用了两级质量系统区分带逐点不确定度的手动整理旋转曲线（Tier 1）与自动化流程产品（Tier 2），并以结构化JSON文件与扁平CSV文件分发数据，支持传统数值分析与大型语言模型检索增强生成（RAG）流程。

Result: 该研究构建并发布了一个包含8,963个空间分辨HI旋转曲线（HI rotation curve）测量值的统一星表，涵盖来自四个主要巡天的423个星系（共438个条目）。
...
**TL;DR**: 本研究通过AstroSat和HST的深紫外成像，首次直接探测到红移5.9-6.0莱曼阿尔法发射体（Lyα emitters）在静止波长350Å、392Å和485Å的电离光子，发现其星际和星系际介质对电离辐射的透明度高于现有模型预测，且其硬紫外谱斜率、高电离光子产生效率（log₁₀ξ_ion^true = 25.86 ± 0.02 Hz erg⁻¹）及高逃逸分数（f_esc ≃ 0.8）表明这些拥有年轻年龄、低金属丰度及极蓝紫外连续谱的微弱星系可能含有能产生足量电离光子的炽热大质量恒星，为宇宙再电离（reionization）来源提供了新约束，同时>24.6 eV光子的探测表明该时期氦I（HeI）再电离已开始。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是宇宙再电离（reionization）时期，特别是由首批星系产生的极端紫外光子所驱动的宇宙黑暗时代结束过程。

Data: 研究使用了来自两个独立空间天文台（AstroSat和HST）的深紫外成像数据，以及基于VLT/MUSE和JWST/NIRSpec在哈勃超深场（Hubble Ultra Deep Field）中光谱确认的Lyα发射体（Lyα emitters）的叠加样本。

Method: 采用深紫外成像（AstroSat和HST）对光谱证认的莱曼阿尔法发射体（Lyα emitters）进行叠加分析，并利用叠加光谱测量紫外连续谱斜率、电离光子产生效率（ξ_ion）和逃逸分数（f_esc）等物理参数。

Result: 通过叠加HUDF中红移5.9-6.0的莱曼阿尔法发射体（Lyα emitters），首次直接探测到了350Å、392Å和485Å的静止波长电离光子，表明星际和星系际介质对电离辐射的透明度高于当前模型预测。
...
### [15] [CII fine-structure line observations of the Sagittarius C Region in the Galaxy's Central Molecular Zone](https://arxiv.org/abs/2604.13336)
...
**TL;DR**: 该研究利用高分辨率[CII]谱线观测人马座C（Sgr C），发现其核心的巨型HII区存在一个由膨胀壳层（光致离解区）驱动的、年龄约0.13 Myr的环状结构，其膨胀速度达23 km/s且可能由隐藏的超新星驱动，并与非热射电细丝空间关联，表明壳层膨胀触发了其边缘的大质量恒星形成。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是银河系中央分子区（CMZ）西部边缘一个相对宁静但质量巨大的复合体人马座C（Sgr C）的星际介质物理与运动学特性。

Data: 研究使用了SOFIA观测的[CII] 158微米发射线数据（空间分辨率0.55 pc，速度分辨率1 km/s）以及APEX望远镜的CO(2-1)及其同位素谱线的辅助数据。

Method: 该研究采用的核心分析方法是对人马座C（Sgr C）区域进行空间与速度分辨的[CII] 158微米谱线观测（使用SOFIA平台），并结合APEX望远镜的CO(2-1)及其同位素谱线辅助数据，通过运动学建模（kinematic modelling）分析气体运动与物理状态。

Result: 结果（result）部分：在Sgr C的巨型HII区中，[CII]发射揭示了一个膨胀的环状壳层，其膨胀速度约为23 km/s，动力学年龄约0.13 Myr，且该壳层与一条非热射电细丝在空间上显著关联，表明其膨胀触发了边缘的高质量恒星形成。
...
**TL;DR**: 该研究利用DECam窄带测光金属丰度结合Gaia DR3自行数据，在MAGIC巡天中识别出213颗Jet星流候选成员星，并发现该星流在远离银河系棒（bar）的末端呈现明显的扇形展宽（fanning）形态。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是利用恒星流（stellar streams）作为示踪物，探测银河系的引力势、吸积历史以及暗物质子晕（dark matter subhalos）的存在与相互作用。

Data: 该研究利用暗能量相机（DECam）上以约3950A的Ca II H&K线为中心窄带滤光片获取的光度测量金属丰度数据，这些数据来自“利用CaHK绘制古老星系”（MAGIC）巡天。

Method: 研究采用基于暗能量相机（DECam）窄带滤光片（中心波长约3950A，针对Ca II H&K线）的光度学金属丰度测量技术，结合盖亚（Gaia）DR3自行数据，对候选成员星进行识别。

Result: 利用CaHK测光金属丰度与Gaia DR3自行数据识别出213颗候选成员星，并发现该星流在远离银河系棒（bar）的末端存在明显的扇形展宽（fanning）。
...
### [17] [ALESS--JWST: Dust-driven Morphologies and Hidden Stellar Mass in $z\sim3$ Sub-millimeter Galaxies](https://arxiv.org/abs/2604.13408)
...
**TL;DR**: 该研究通过高分辨率多波段观测发现，亚毫米星系（SMG）中空间变化的尘埃衰减导致集成光谱拟合产生系统性恒星质量低估（即“尘埃遮蔽偏差”），且尘埃几何分布是造成其形态波长依赖性和结构多样性的主要驱动力，支持了其致密遮蔽性恒星形成构建致密恒星核并演化为大质量宁静星系的图像。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用高分辨率多波段观测揭示高红移亚毫米星系（SMG）中尘埃遮蔽对恒星质量测量和形态分析的偏差效应及其对星系结构演化的影响。

Data: 研究使用了JWST/NIRCam和MIRI对十二个$z\sim3$亚毫米波星系（SMG）的观测数据，并结合了高分辨率（$0.08''-0.16''$）的ALMA 870$μ$m成像数据。

Method: 研究采用了结合JWST/NIRCam与MIRI观测数据、高分辨率ALMA 870μm成像以及空间分辨（~kpc尺度）的SED拟合（Spectral Energy Distribution fitting，光谱能量分布拟合）方法。

Result: 通过高分辨率多波段观测和空间分辨SED拟合，发现亚毫米星系（SMG）中恒星形成与局部质量分布紧密耦合，且尘埃几何结构（而非经典“光淹效应”）是导致集成拟合中恒星质量被低估（即“尘埃遮蔽偏差”）以及形态波长依赖性的主要驱动因素，其本质致密恒星结构与尘埃连续谱尺寸一致，支持了SMG演化为大质量宁静星系的图像。
...
### [18] [Substructures of the Milky Way's Retrograde Halo: Evidence for Multiple Accretion Events](https://arxiv.org/abs/2604.13482)
*Young Kwang Kim,Young Sun Lee,Timothy C. Beers*

**TL;DR**: 该研究通过结合化学丰度与动力学信息，发现银河系晕中低倾角逆行子结构源于多个矮星系吸积事件而非单一前身星系，其中主要贡献者（primary progenitor）的碎片呈现一致的金属丰度-能量序列，并识别出与不同吸积时期相关的双起源（dual-origin）星族。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于银河系（Milky Way）晕中低倾角逆行子结构的起源，这些结构是逆行轨道上被吸积的矮星系遗迹。

Data: 研究结合了光谱数据与盖亚（Gaia）天体测量数据来构建样本。

Method: 研究采用的核心分析方法包括结合光谱数据与盖亚（Gaia）天体测量数据构建样本，并在远银心距离-轨道相空间中利用金属丰度分布函数（MDFs）识别子结构，同时整合化学与动力学信息以区分不同起源。

Result: 研究结果表明，银河系（Milky Way）的逆行晕（retrograde halo）具有复杂的多前身起源，主要由一个主要的前身星系通过多次吸积事件组装而成，而非单一前身。
...
### [19] [Massive star clusters and clumps in the collisional ring galaxy Arp 147](https://arxiv.org/abs/2604.13655)
*Z. Randriamanakoto,M. Rakototafika,B. Mongwane,P. Väisänen,M. Rakotomanga*

**TL;DR**: 对碰撞环星系（CRG）Arp 147的星团（或星结）测光研究发现，其环上年龄在10-200 Myr的星团质量函数偏离幂律并符合特征截断质量为6.2×10⁵ M⊙的Schechter函数，且碰撞触发的星暴事件促进了大量第二代年轻蓝色星结的形成。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是探究碰撞环星系（CRG）中星团（或星结）的恒星形成历史，以理解星系碰撞触发的星暴事件对第二代年轻星团形成的影响。

Data: 研究使用了哈勃空间望远镜（HST）的F450W、F606W和F814W波段图像数据。

Method: 采用哈勃空间望远镜（HST）的F450W、F606W和F814W图像进行测光研究，并使用星团质量函数（CMF）的Schechter函数拟合及星团年龄函数分析，以推导星团瓦解率和形成效率（CFE）。

Result: 在10-200 Myr年龄段的星团（或结）的质量函数（CMF）偏离幂律分布，遵循特征截断质量为${\rm M}_{c} = 6.2 \times 10^{5} \, {\rm M}_{\odot}$的Schechter函数。
...
### [20] [The ALMA-QUARKS Survey: Multipolar episodic molecular outflow associated with W49N, the most energetic water maser source in the Milky Way](https://arxiv.org/abs/2604.13671)
...
**TL;DR**: 通过对W49N区域的高分辨率ALMA观测，该研究在已知中心双极喷流外新识别出四个呈喷流状、带有节点链和S形扭动（表明为间歇性抛射和进动喷流）的外流瓣（outflow lobes），并提供了更可靠的物理参数估计，证实其为银河系内能量最高的外流之一，这些在极端大质量恒星形成区发现通常与低质量原恒星外流相关的特征，为外流产生的某些基本物理机制在不同恒星质量范围内具有保守性提供了有力证据。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于大质量恒星形成区W49N中多极、间歇性分子外流（multipolar episodic molecular outflow）的观测与性质分析。

Data: 研究使用了阿塔卡马大型毫米波/亚毫米波阵列（ALMA）的高分辨率（约0.3角秒）观测数据，这些数据来自ALMA-QUARKS巡天项目，具体观测了12CO和13CO（2-1）谱线发射。

Method: 该研究采用高分辨率（约0.3角秒）的阿塔卡马大型毫米波/亚毫米波阵列（ALMA）对一氧化碳（12CO）发射的观测数据，分析了分子外流的形态和运动学。

Result: 基于高分辨率ALMA观测，该研究在W49N区域新识别出四个具有链状结（knots）和S形摆动特征的喷流状外流瓣，为极端大质量恒星形成环境中存在与低质量原恒星类似的间歇性、进动喷流机制提供了有力证据。
...
### [21] [CHILES XII: The H I evolution of Luminous Compact Blue Galaxies](https://arxiv.org/abs/2604.13679)
*H. Arlow,D. J. Pisano,M. A. Bershady,L. R. Hunt,N. Luber,J. Donovan Meyer,E. Momjian,J. Blue Bird,H. B. Gim*

**TL;DR**: 基于CHILES巡天数据，通过叠加分析发现红移z<0.5范围内明亮致密蓝星系（LCBGs）的平均中性氢（H I）质量未呈现显著演化，其气体储量保持稳定，但气体消耗时标比正常恒星形成星系短近一个数量级，与其数量密度急剧下降的时期相符。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是探究发光致密蓝星系（LCBGs）在宇宙学时间尺度上的中性氢（H I）气体质量演化及其与星系演化关键阶段（如恒星形成活动骤减期）的关联。

Data: 研究使用了来自完整856小时COSMOS H I大尺度河外巡天（CHILES）的H I发射线数据，该数据覆盖了COSMOS天区内0 ≤ z ≤ 0.48的红移范围。

Method: 采用立方体堆叠分析（cubelet stacking analysis）方法估算平均中性氢（H I）质量演化，并对未探测样本给出平均中性氢质量上限估计。

Result: 在红移z=0.26、z=0.35和z=0.45处，亮致密蓝星系（LCBGs）的平均中性氢（H I）质量分别为<4.89×10^9 M⊙、(2.49±0.75)×10^9 M⊙和(6.44±2.71)×10^9 M⊙，且在此红移范围内未发现平均中性氢质量发生显著演化的强证据。
...
### [22] [Witnessing the onset of stellar winds in Super-Luminous Supernova Hosts: implications for star-formation-driven outflows in low and high-redshift galaxies](https://arxiv.org/abs/2604.13865)
*A. Saldana-Lopez,A. Gkini,M. J. Hayes,R. Lunnan,C. A. Carr,M. Huberty,C. Scarlata,S. Schulze,J. Sollerman*

**TL;DR**: 该研究通过分析六个I型极亮超新星（SLSN-I）宿主星系（z=0.15-0.51）的光谱，首次直接观测到在恒星形成早期（数Myr内）、由恒星风主导的星系外流（outflow），发现这些低质量、极贫金属（小于9%太阳丰度）星系的外流速度（37-104 km s$^{-1}$）和质量加载因子（$η<1$）均低于具有相似恒星形成率的更成熟星系，表明在超新星反馈主导之前，恒星风和辐射压即可驱动缓慢而微弱的外流。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或主题是探索低质量星系在恒星形成早期阶段（即超新星主导反馈启动前）由恒星风和辐射压驱动的、缓慢且微弱的外流（outflow），以提供对高红移星系早期反馈阶段及星系形成模拟中时变恒星反馈的实证约束。

Data: 研究使用了甚大望远镜（VLT）上的X-shooter摄谱仪对六个红移在0.15至0.51之间的I型极亮超新星（SLSN-I）宿主星系进行了中分辨率光谱观测，并利用这些超新星的明亮连续谱作为单视线方向上的探针来研究宿主星系的星际介质。

Method: 采用中分辨率VLT/X-shooter光谱技术，利用SLSN的连续谱作为单视线方向探针，并对Mg II吸收线进行沃伊特（Voigt）剖面建模。

Result: 通过利用SLSN-I作为单视线探针，在低质量系统中首次观测到由恒星风和辐射压驱动的低速弱外流，其最大速度（37-104 km s⁻¹）低于类似SFR的更演化星系的经验关系。
...
### [23] [Bayesian Analysis of Gravitational Wave Microlensing Effects from Galactic Double White Dwarfs](https://arxiv.org/abs/2604.13930)
*Yan Sun,Yong Yuan,Minghui Du,Wen-Fan Feng,Xilong Fan,Peng Xu*

**TL;DR**: 该研究通过模拟太极探测器对银河系双白矮星（DWD）系统引力波（GW）的四年观测，评估了透镜质量（$M_\mathrm{L}$）、有效速度（$v_\mathrm{eff}$）和初始分离（$L$）等参数对探测引力波微透镜效应的影响，并发现当透镜质量低于$10^5$\,M$_\odot$或初始分离大于等于3倍爱因斯坦半径（$R_\mathrm{E}$）时，无法区分透镜与非透镜模型。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或主题是探索利用空间引力波探测器通过微引力透镜效应探测银河系内致密天体（如双白矮星系统）的潜力。

Data: 本研究使用模拟的太极（Taiji）探测器四年观测数据，数据形式为第二代时延干涉（Time Delay Interferometry, TDI）数据流。

Method: 采用贝叶斯推断框架（Bayesian inference framework）和贝叶斯模型选择框架（Bayesian model selection framework）对透镜化引力波进行参数估计和模型区分。

Result: 通过贝叶斯模型选择框架分析透镜系统关键物理参数（透镜质量$M_\mathrm{L}$、有效速度$v_\mathrm{eff}$和初始分离$L$）对区分透镜事件的影响，发现当$M_\mathrm{L}$低于$10^5$\,M$_\odot$或$L\geq3R_\mathrm{E}$时无法区分透镜与非透镜模型，而在参数范围内有效速度$v_\mathrm{eff}$的降低虽会减小贝叶斯因子（Bayes factor），但仍可区分两类模型。
...
### [24] [Radial Distribution of Star Formation and Gas-phase Metallicity in Spiral-Elliptical Galaxy Pairs](https://arxiv.org/abs/2604.14092)
*Cailu Shi,Shuai Feng,Shiyin Shen,Linlin Li,Wenyuan Cui,Guozhen Hu*

**TL;DR**: 基于SDSS-IV MaNGA积分视场光谱，本研究发现螺旋-椭圆（S+E）对中的螺旋星系（研究对象）中心恒星形成率（SFR）受抑制且金属丰度升高，其程度取决于伴星系的质星以及气体速度场的对称性，并推断该现象源于螺旋星系进入早型伴星的热环星系介质后气体吸积受抑制以及近距离遭遇时的潮汐扰动触发恒星形成与化学增丰。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于旋涡星系与椭圆星系配对（S+E pairs）中星系的星形成率（SFR）和气体金属丰度的径向分布特征及其与伴星质量和气体动力学的关系。

Data: 研究使用的观测数据来源于SDSS-IV MaNGA（Mapping Nearby Galaxies at Apache Point Observatory）巡天的积分视场光谱（integral field spectroscopy）。

Method: 采用积分视场光谱（integral field spectroscopy）技术分析径向分布。

Result: 螺旋-椭圆星系对中的螺旋星系进入早型伴星的热环星系介质后气体吸积受到抑制，导致其中心恒星形成率下降。
...
### [25] [A Salpeter-like filament linear density function across nearby molecular clouds](https://arxiv.org/abs/2604.14093)
*Guo-Yin Zhang,Alexander Men'shchikov,Jin-Zeng Li*

**TL;DR**: 该研究通过多尺度提取方法系统分析了七个邻近分子云中丝状结构（filament）的线密度函数（FLDF），发现其整体线密度分布呈现幂律斜率（α≈1.30–1.34），与恒星初始质量函数（IMF）的Salpeter斜率（1.35）一致，表明恒星质量谱已预先编码在分子云的丝状结构中。


<details>
  <summary>Details</summary>

Background: 该研究探讨了分子云中丝状结构（filamentary structures）在恒星形成中的核心作用及其与恒星初始质量函数（IMF）的潜在联系。

Data: 研究使用多尺度提取方法$getsf$系统分析了七个距离约100–1000 pc、覆盖从宁静到活跃大质量恒星形成等多种环境的邻近分子云中的丝状体线性密度函数（FLDF）。

Method: 采用多尺度提取方法 $getsf$ 对七个不同距离和恒星形成环境的邻近分子云进行了系统分析，以研究其丝状结构线性密度函数（FLDF）。

Result: 对七个近邻分子云（距离约100-1000 pc）中丝状体线性密度函数（FLDF）的系统分析表明，当整合所有空间尺度时，所有云的综合线性密度产生的FLDF遵循幂律分布 ${\rm d}N/{\rm d}\logΛ\propto Λ^{-α}$，其指数 $α\approx 1.30$--$1.34$，与恒星初始质量函数（IMF）的Salpeter斜率（1.35）相符。
...
### [26] [The Radial and Vertical Structure of Molecular Gas in the Edge-On Galaxy NGC 4565](https://arxiv.org/abs/2604.14136)
...
**TL;DR**: 基于高分辨率ALMA观测，该研究发现NGC 4565星系的分子气体盘（molecular disk）在垂直方向上保持薄盘结构（半高全宽为79.1±1.6 pc）且无明显翘曲，其分子云特性与PHANGS-ALMA星系及M31相似，并识别出一个被称为“东环堆积”（East Ring Pileup）的显著恒星形成复合体。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或主题是借助高分辨率毫米波观测（ALMA）研究高度侧向（edge-on）星系（NGC 4565）的分子气体分布、物理性质及其与星系结构（如分子盘厚度、分子云特性）和恒星形成活动的关系。

Data: 本研究使用了阿塔卡马大型毫米波/亚毫米波阵列（ALMA）观测的高分辨率（0.94角秒，约55秒差距）CO(2-1)和13CO(2-1)谱线数据，覆盖了高度倾斜（倾角约87.5度）的星系NGC 4565，其观测范围延伸至星系中心半径大于正负17千秒差距的外围区域。

Method: 研究采用了高分辨率（0.94角秒）的阿塔卡马大型毫米波/亚毫米波阵列（ALMA）对CO(2-1)和13CO(2-1)谱线进行观测，并结合对位置-速度切片（position-velocity slices）的拟合以及垂直强度剖面分析等核心分析方法。

Result: 分子盘保持薄层结构，其垂直强度剖面测得的半高全宽（FWHM）尺度高度为79.1±1.6 pc，且几乎没有垂直耀斑（vertical flaring）的证据。
...
### [27] [Into the Gompverse: A robust Gompertzian reionization model for CMB analyses](https://arxiv.org/abs/2604.13423)
*Paulo Montero-Camacho,Yin Li,Marco Barquero-Hernández,Pablo Renard,Yang Wang,Xiao-Dong Li*

**TL;DR**: 本文提出了一种物理驱动的戈珀兹（Gompertz）再电离模型，通过将光学深度（$τ_{\rm reio}$）降为导出参数并将其不确定性降低约三倍，从而在联合分析宇宙微波背景（CMB）与再电离数据时，能更精确地约束中微子质量总和并揭示其与振荡实验的潜在张力，同时为21厘米实验提供物理信息参数范围。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是宇宙再电离（cosmic reionization）的物理建模及其与宇宙微波背景（CMB）观测的联合分析。

Data: 本研究使用宇宙微波背景（CMB）观测数据与再电离（reionization）数据（包括当前及未来的21厘米实验数据）进行联合分析。

Method: 该研究引入了一种基于物理动机的戈珀兹（Gompertzian）再电离模型，其包含三个天体物理（冗余）参数，旨在支持宇宙微波背景（CMB）与再电离数据的联合分析，并适用于广泛的数据集和宇宙学模型。

Result: 该研究引入的戈姆珀茨（Gompertz）再电离模型将光学深度（τ_reio）降格为导出参数，使其不确定性相比传统双曲正切（tanh）模型降低约三倍，从而能更严格地约束中微子质量总和，并揭示其与中微子振荡实验间的潜在张力。
...
### [28] [Double the axions, half the tension: multi-field early dark energy eases the Hubble tension](https://arxiv.org/abs/2604.13535)
*Marco Bella,Vivian Poulin,Sunny Vagnozzi,Lloyd Knox*

**TL;DR**: 多场模型（multiple fields）显著缓解了普朗克NPIPE宇宙微波背景（CMB）数据对类轴子早期暗能量（EDE）的强约束，其中两场模型将哈勃常数（H_0）的局域测量值残余张力降至1.5σ，并通过改善高角多极矩（high-ℓ）CMB数据拟合度，表明前复合期历史在更宽红移范围内存在修正。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是探索多场模型（multiple fields）对缓解早期暗能量（axion-like early dark energy, EDE）与宇宙微波背景（CMB）观测之间紧张关系的潜在作用。

Data: 研究使用了普朗克卫星NPIPE处理发布的宇宙微波背景（CMB）观测数据。

Method: 采用多场模型分析早期暗能量（EDE）对宇宙微波背景（CMB）数据的影响。

Result: 多场模型显著缓解了普朗克NPIPE宇宙微波背景（CMB）数据对类轴子早期暗能量（EDE）的强约束。
...
### [29] [Reconstructing inflationary features on large scales using genetic algorithm](https://arxiv.org/abs/2604.13547)
*Alipriyo Hoory,Dhiraj Kumar Hazra,L. Sriramkumar*

**TL;DR**: 该研究通过基于遗传算法（GA）的机器学习流程，在单一场正则标量场（canonical scalar field）驱动暴胀模型中系统重构了与普朗克（Planck）2018宇宙微波背景（CMB）数据一致的原初标量功率谱局部特征，发现这些特征可将角功率谱拟合优度提升Δχ²≲-10，并指出替代的背景参数与功率谱特征组合可能有助于缓解现有宇宙学张力。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过修改慢滚参数（slow roll parameter）在单一场暴胀（inflationary）模型中生成原初标量功率谱（primordial scalar power spectrum）的局域化特征（localized features），以更好地拟合宇宙微波背景（cosmic microwave background, CMB）观测数据。

Data: 该研究使用了普朗克卫星2018年发布的宇宙微波背景（CMB）各向异性观测数据。

Method: 该研究采用基于遗传算法（genetic algorithm，GA）的机器学习流程，通过系统性地重构与数据一致的标量功率谱特征（即对第一慢滚参数行为的修改）来实现分析。

Result: 基于遗传算法（GA）重建的原初标量功率谱（primordial scalar power spectrum）特征能显著改善与普朗克（Planck）2018年宇宙微波背景（CMB）观测数据的拟合度（Δχ² ≲ -10），并可能为缓解现有宇宙学张力提供途径。
...
### [30] [Reanalysing large-scale structure using an updated gamma-ray burst spatial density approach](https://arxiv.org/abs/2604.13712)
...
**TL;DR**: 本研究通过新方法在伽马射线暴（GRB）空间分布中检测到南北半球各一个偏离各向同性的区域，其中北半球巨大区域（约125个GRB）对应武仙-北冕座长城，而南半球小区域（4-5个GRB）的紧密聚集未显著违背宇宙学原理。


<details>
  <summary>Details</summary>

Background: 该研究探讨了宇宙大尺度结构（large universal structures）对标准宇宙学模型所预期的均匀性和各向同性（homogeneity and isotropy）的挑战。

Data: 本研究使用伽马射线暴（GRB）的观测数据，通过分析其在南北半球天区的空间分布来检验各向同性。

Method: 本研究采用在三维空间中使用球体测试规律性的新方法（a recently developed methodology using spheres in 3D space for testing regularities）来分析伽马射线暴（GRBs）的空间分布。

Result: 使用新方法在南北半球仅发现各一处偏离各向同性的伽马暴（GRB）聚类，其中北半球巨大聚类（约125个伽马暴）对应武仙-北冕座长城，南半球微小聚类（4-5个伽马暴）中GRB050822与GRB050318在红移、角位置及观测时间（相隔5个月）上均极为接近。
...
### [31] [Optical depth to reionization in a Universe with multiple inhomogeneous domains](https://arxiv.org/abs/2604.13718)
*Shashank Shekhar Pandey,Ruchika,Subhadeep Mukherjee,A. S. Majumdar*

**TL;DR**: 基于PantheonPlus+SH0ES数据对包含物质不均匀性反作用（backreaction）的宇宙学模型进行参数约束后，计算得出的再电离光学深度（τ_reion）为0.0581^{+0.0105}_{-0.0096}，该结果比标准宇宙学模型的预测值更符合观测估计，同时能在无需引入非标准物理的情况下适度缓解哈勃张力。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是宇宙再电离（reionization）的光深（optical depth）在包含物质不均匀性反作用（backreaction）的宇宙学框架下的计算与约束。

Data: 研究基于PantheonPlus+SH0ES Ia型超新星样本进行了马尔可夫链蒙特卡洛（MCMC）分析以推断模型参数。

Method: 采用Buchert平均形式主义构建包含多重非均匀域的后反应模型，并基于PantheonPlus+SH0ES Ia型超新星样本进行马尔可夫链蒙特卡洛（MCMC）分析以推断模型参数。

Result: 基于PantheonPlus+SH0ES数据对模型参数进行约束后，该反作用模型得出的再电离光学深度（τ_reion）值（0.0581^{+0.0105}_{-0.0096}）比标准宇宙学模型的预测值更接近观测估计值。
...
### [32] [Denoising clustering covariance matrices with Rotational Invariant Estimators](https://arxiv.org/abs/2604.13851)
*Antonio Farina,Massimo Guidi,Alfonso Veropalumbo,Claudio Guida*

**TL;DR**: 该研究首次将旋转不变估计量（RIE）应用于大尺度星系聚类，发现在傅里叶空间中RIE能显著提升最佳拟合稳定性且约束偏差较小，优于样本协方差和非线性收缩估计量NERCOME。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是宇宙学参数推断中星系成团性（galaxy clustering）的协方差矩阵与精度矩阵的准确估计问题。

Data: 研究中使用了受控合成数据集（controlled synthetic data sets），其协方差矩阵（covariance matrices）为解析已知（analytically known）。

Method: 研究采用的核心分析方法包括样本协方差（sample covariance）、非线性收缩估计器NERCOME和旋转不变估计器（Rotational Invariant Estimator, RIE），并基于EFT模型进行贝叶斯推断，通过偏差图（Figure of Bias, FoB）和优值图（Figure of Merit, FoM）量化评估。

Result: 对于星系大尺度聚类的协方差矩阵估计，旋转不变估计量（RIE）在傅里叶空间能显著提升最佳拟合值的稳定性，且仅引入轻微的优值图（FoM）偏差。
...
### [33] [Dark energy, spatial curvature, and star formation efficiency from JWST photometric and spectroscopic high-redshift galaxies](https://arxiv.org/abs/2604.13866)
*Leonardo Comini,Sunny Vagnozzi,Abraham Loeb*

**TL;DR**: 基于对CEERS和FRESCO样本中极端星系（high-redshift galaxies）的贝叶斯分析，该研究在允许暗能量状态方程（dark energy equation of state）和空间曲率（spatial curvature）变化的不同宇宙学模型中，发现对重子物质转恒星效率（baryon-to-star conversion efficiency）的约束结果与平坦ΛCDM模型一致，且无偏离标准宇宙学参数的证据，从而表明JWST观测到的大量高红移大质量星系所引发的“张力”更可能源于星系形成（galaxy formation）的天体物理过程而非新宇宙学。


<details>
  <summary>Details</summary>

Background: 该研究旨在探究詹姆斯·韦伯空间望远镜（JWST）早期观测到的高红移大质量星系（massive high-redshift galaxies）数量超出预期的现象是否源于ΛCDM模型之外的新物理（new physics）或大质量恒星形成效率的提升。

Data: 研究使用了来自詹姆斯·韦伯空间望远镜（JWST）的CEERS成像样本和FRESCO光谱样本的观测数据。

Method: 该研究采用贝叶斯分析（Bayesian analysis）方法，在允许暗能量状态方程（dark energy equation of state）$w$ 和空间曲率参数（spatial curvature parameter）$Ω_K$ 变化的不同模型中，联合约束宇宙学参数和重子转恒星效率（baryon-to-star conversion efficiency）$ε$。

Result: 在允许暗能量状态方程（w）和/或空间曲率参数（Ω_K）变化的情况下，分析未发现偏离w=-1或Ω_K=0的证据，表明“JWST张力”的根源很可能在于星系形成的天体物理学而非宇宙学。
...
### [34] [BROOM: a python package for model-independent analysis of microwave astronomical data](https://arxiv.org/abs/2604.14088)
*Alessandro Carones,Sijil Jose,Aliza Mustafa,Nicoletta Krachmalnicoff,Carlo Baccigalupi*

**TL;DR**: 该研究开发了名为BROOM的Python软件包，通过内部线性组合（ILC）和广义ILC（GILC）方法，实现了对宇宙微波背景（CMB）等已知光谱信号及未知协方差相干成分的盲分离，并提供了诊断前景复杂性和估计残余污染的工具。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是开发用于微波观测中盲最小方差成分分离（component separation）的Python软件包，以支持宇宙微波背景（CMB）等已知谱能量分布信号的提取。

Data: 该研究使用模拟数据（simulation）进行验证，具体通过软件包内嵌工具生成适用于任意宇宙微波背景（CMB）实验的逼真微波模拟数据。

Method: 该研究采用了一套内部线性组合（Internal Linear Combination, ILC）实现方法，包括用于重建具有未知协方差特性的相干发射成分的广义ILC（Generalized ILC, GILC）框架。

Result: BROOM是一个用于微波观测中盲最小方差成分分离（component separation）的新Python软件包，能通过内部线性组合（Internal Linear Combination, ILC）方法重建已知光谱能量分布的信号（如宇宙微波背景（Cosmic Microwave Background, CMB）），并支持通过广义内部线性组合（Generalized ILC, GILC）框架盲重建协方差属性未知的相干发射成分。
...
### [35] [TDCOSMO XXV: A "soup-to-nuts" 6.5% $H_0$ measurement $-$ strong lensing and dynamics with a maximally flexible mass sheet](https://arxiv.org/abs/2604.14145)
...
**TL;DR**: 该研究通过对四重像类星体SDSSJ1433+6007进行盲时间延迟宇宙学测量，结合强透镜建模、运动学观测及视线环境约束，在平坦ΛCDM宇宙学模型下推断出哈勃常数（H_0）为73.2^{+4.8}_{-4.7} km s^{-1} Mpc^{-1}，并发现内部质量薄片参数（λ_int）显著偏离1，强调了将其作为自由参数处理的重要性。


<details>
  <summary>Details</summary>

Background: 该研究旨在通过强引力透镜（strong gravitational lensing）时间延迟法精确测量哈勃常数（Hubble constant, H0），以约束宇宙学参数。

Data: 研究使用了哈勃空间望远镜（Hubble Space Telescope）的深度成像、温德尔施泰因（Wendelstein）和迈丹纳克（Maidanak）天文台的长期时延监测数据、凯克宇宙网成像仪与再电离测绘仪（Keck Cosmic Web Imager and Reionization Mapper）的空间分辨恒星运动学数据，以及暗能量光谱仪（Dark Energy Spectroscopic Instrument, DESI）遗产巡天数据发布10的宽视场测光数据。

Method: 研究采用的核心分析方法包括构建稳健的引力透镜模型以重建质量分布，并结合高信噪比运动学地图以打破质量片简并（MSD），同时利用暗能量光谱仪（DESI）Legacy Survey数据对视线方向环境进行表征以约束外部收敛（κ_ext），并将这些约束纳入联合透镜与动力学模型中进行多次迭代以估计随机和系统不确定性。

Result: 基于对SDSSJ1433+6007的联合透镜和动力学建模，并假设平坦ΛCDM宇宙学模型，本研究推断出哈勃常数（H_0）为73.2^{+4.8}_{-4.7} km s^{-1} Mpc^{-1}，内部质量片（mass-sheet）参数λ_{int}=1.12^{+0.05}_{-0.06}。
...
### [36] [SN 2022riv in RX J2129: Discovery, Spectroscopic Classification, and Microlensing of a Strongly Lensed Type Ia Supernova from JWST and HST Observations](https://arxiv.org/abs/2604.11882)
...
**TL;DR**: 该研究通过哈勃和韦伯太空望远镜观测，在星系团RX J2129.7+0005引力透镜背景下，对红移$z=1.522$处多重成像的Ia型超新星SN 2022riv的最后一幅到达图像进行了测量，获得了宇宙学无关的$5.35\pm1.01$放大率，并发现微引力透镜效应（microlensing）预期会引入20-50%的调制，而多个透镜模型预测的精度显著高于随机预期，其中HoliGRALE模型预测值在纳入微透镜效应后与测量值存在$1σ$张力。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或主题是利用引力透镜效应（gravitational lensing）探测高红移（high-redshift）宇宙中的高放大率超新星（highly magnified supernovae），特别是通过哈勃空间望远镜（Hubble Space Telescope, HST）和詹姆斯·韦伯空间望远镜（James Webb Space Telescope, JWST）对星系团（galaxy cluster）引力透镜场（lensing fields）的观测来研究宇宙学和微引力透镜（microlensing）效应。

Data: 研究中使用的数据来源于哈勃空间望远镜（HST）的SNAP项目观测以及詹姆斯·韦伯空间望远镜（JWST）的NIRSpec G140M和PRISM光谱仪后续观测。

Method: 采用SALT3-NIR光变曲线拟合器（SALT3-NIR light-curve fitter）进行宇宙学无关的放大倍数测量，并结合多个SALT超新星光谱时间序列模型进行一致性约束。

Result: 通过SALT3-NIR光变曲线拟合，对超新星最后到达图像的宇宙学独立放大率测量值为$5.35\pm1.01$，且多个SALT光谱时序模型给出了一致的约束。
...
### [37] [Secular Brightness Trends of Starlink and OneWeb Satellites](https://arxiv.org/abs/2604.13145)
*Anthony Mallama*

**TL;DR**: 基于MMT9天文台对星链VisorSat和OneWeb卫星的160万次光度测量，研究发现前者在2021至2026年间增亮0.6星等，后者则变暗0.4星等，两者趋势均具有3西格玛（3 sigma）显著性。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是监测低地球轨道（Low Earth Orbit, LEO）卫星星座（如Starlink和OneWeb）的长期亮度变化及其对天文观测的影响。

Data: 本研究基于MMT9机器人天文台记录的160万条星等（magnitude）数据。

Method: 基于MMT9机器人天文台记录的160万条星等数据进行分析。

Result: 基于160万次MMT9天文台观测数据，星链（Starlink）VisorSat卫星在2021至2026年间增亮0.6星等，而一网（OneWeb）卫星则变暗0.4星等，两者趋势均具有3西格玛（3 sigma）显著性。
...
### [38] [An Information-Theoretic Metric for Transient Classification and Novelty Detection](https://arxiv.org/abs/2604.13207)
*Yu-Qian,Ouyang,Alex I. Malz,Ming Lian,Shar Daniels,Federica Bianco,Mathilda Nilsson*

**TL;DR**: 本文提出了一种基于信息论交叉熵的新指标（metric），用于优化LSST（Vera C. Rubin Observatory Legacy Survey of Space and Time）的瞬变源（transient）观测策略，并展示了其在区分天体种群、优化观测与探测流程以及分配后续观测资源方面的应用价值。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是优化薇拉·C·鲁宾天文台时空遗产巡天（LSST）的观测策略以兼顾时域天文学内外的多种科学目标。

Data: 该研究使用了基于信息论交叉熵的新型度量方法，并讨论了其在观测策略优化、检测流程优化以及新奇天体发现和后续资源分配中的应用。

Method: 本研究采用了一种基于信息论交叉熵（cross-entropy）的新型度量方法。

Result: 该研究引入了一种基于信息论交叉熵（cross-entropy）的新颖度量方法，用于优化LSST瞬变科学（transient science）的观测策略和检测流程。
...
### [39] [Site Quality Analysis for an Indian Submillimeter Telescope: A Reanalysis-Based Approach](https://arxiv.org/abs/2604.13487)
*Tanmay Singh,Mayuri Sathyanarayana Rao,Ritoban Basu Thakur*

**TL;DR**: 基于再分析数据（ERA5）对印度拉达克地区可沉降水汽（PWV）的分析，识别出两个亚毫米波观测潜力优于现有台址的新候选区域（Site A与Site B），其PWV ≤ 1毫米的时间占比分别达约23%和19%。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是评估印度拉达克喜马拉雅高原地区作为亚毫米波天文台候选站址的潜力，这属于天文台址选址与表征（site characterization）的科学主题。

Data: 研究使用了欧洲中期天气预报中心第五代再分析（ERA5）数据，分析了从2010年1月至2025年4月共184个月、月分辨率的可降水量（PWV）。

Method: 采用欧洲中期天气预报中心第五代再分析数据（ERA5）分析可降水量（PWV），并利用大气辐射传输模型（am）计算大气透射率和相应的大气光子噪声。

Result: 基于ERA5再分析数据，在拉达克（Ladakh）识别出两个特别有前景的候选区域（Site A和Site B），其可沉降水汽（PWV）≤1毫米的时间占比分别约为23%和19%，显著高于现有天文台址汉勒（Hanle）和梅拉克（Merak）的约5%和8%。
...
### [40] [Simulation of non X-ray background for the DIffuse X-ray Explorer (DIXE) mission](https://arxiv.org/abs/2604.13569)
*Ruixuan Tian,Junjie Mao,Jiejia Liu,Hai Jin,Wei Cui*

**TL;DR**: 该研究通过Geant4模拟明确了中国空间站拟搭载的弥散X射线探测仪（DIXE）在低地球轨道（LEO）的非X射线背景（NXB）主要源于初级宇宙线质子诱发的次级粒子，其在0.1–10 keV能段的平均强度为$4.46 \times 10^{-2} ~\mathrm{counts~s^{-1}~cm^{-2}~keV^{-1}}$，并随纬度升高而增强，而南大西洋异常区（SAA）诱发的延迟背景在离开异常区约5分钟后可忽略。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或主题是评估低地球轨道（LEO）中DIffuse X-ray Explorer (DIXE)项目的非X射线背景（NXB），以支持其利用过渡边缘传感器（TES）微热量计对银河系热气体进行高分辨率光谱巡天的科学目标。

Data: 研究使用\textsc{Geant4}软件模拟了低地球轨道（LEO）环境下的非X射线背景（NXB），并构建了有效载荷的详细质量模型，主要背景源包括宇宙射线、反照中子和反照光子，同时评估了南大西洋异常区（SAA）捕获质子产生的延迟背景。

Method: 采用Geant4软件模拟了低地球轨道（LEO）环境下的非X射线背景（NXB），并构建了有效载荷的详细质量模型，同时评估了南大西洋异常区（SAA）俘获质子产生的延迟本底。

Result: 模拟结果表明，在0.1-10 keV能段，地磁赤道处非X射线背景（NXB）平均强度为$4.46 \times 10^{-2} ~\mathrm{counts~s^{-1}~cm^{-2}~keV^{-1}}$，其主要来源是初级宇宙线质子产生的次级粒子。
...
### [41] [The 256-antenna Coherent All-Sky Monitor](https://arxiv.org/abs/2604.13903)
...
**TL;DR**: 该研究介绍了正在部署的256天线相干全天监测器（CASM-256），这是一个工作于375-500 MHz的密集孔径阵列，其大视场和高灵敏度旨在探测邻近宇宙的快速射电暴（FRBs）及银河系内的快速暂现源，并展示了基于GPU的实时探测流水线，为未来扩展到数万天线规模奠定了基础。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是数字信号处理技术进步推动射电天文学（radio astronomy）向大数量天线（large-$N$ aperture arrays）和实时宽视场观测发展，以探测宇宙中的快速射电暴（fast radio bursts, FRBs）等快速瞬变现象。

Data: 该研究使用的观测数据来自部署于加州大松镇欧文斯谷射电天文台（Owens Valley Radio Observatory, OVRO）的256天线相干全天监测器（Coherent All-Sky Monitor, CASM-256），并已利用首批部署的二十余个天线获取了实测数据。

Method: 核心分析方法、物理模型或技术手段包括使用图形处理器（GPU）进行实时相干处理，并部署基于GPU的快速射电暴（FRB）实时搜索管线。

Result: 该研究描述了一种正在部署的256天线相干全天空监测器（CASM-256），这是一种工作在375-500 MHz频段的密集孔径阵列，其大视场和高点源灵敏度将使其能够探测到本地宇宙的快速射电暴（FRBs）。
...
### [42] [Visplot: A visibility plot and observation scheduling tool for astronomical observatories](https://arxiv.org/abs/2604.14151)
*Emanuel Gafton,Illa R. Losada*

**TL;DR**: 该研究开发了Visplot这一开源、基于Web的天文观测可视性分析与调度工具，通过整合天体、机械及自定义约束，结合确定性预分配与多目标启发式优化，为包括非恒星目标在内的观测提供了灵活、轻量级的调度支持，显著降低了观测规划负担并确保符合望远镜硬件限制。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是开发用于天文观测的硬件感知可见性分析与启发式调度工具。

Data: 该工具的数据来源于用户自定义的约束条件（如天顶距、月距、晨昏蒙影等天体参数，望远镜的机械边界，以及自定义的UTC或本地恒星时限制），这些约束的交集被用于计算可见性窗口。

Method: 该工具采用基于用户定义约束的交集计算可见性窗口（visibility windows）作为有限个不相交区间的并集，并结合确定性预分配与多目标启发式优化（multi-objective heuristic optimization）的调度引擎。

Result: Visplot是一款免费、开源、基于Web的工具，用于进行硬件感知的可见性分析及对恒星与非恒星（sidereal and non-sidereal）天文观测的启发式调度。
...
### [43] [The Material Point Method (MPM) for simulating hypervelocity impact on asteroids](https://arxiv.org/abs/2604.13136)
*Xiaoran Yan,Patrick Michel,Ruichen Ni,Yifei Jiao,Junfeng Li*

**TL;DR**: 本研究通过引入并验证物质点法（Material Point Method, MPM）来模拟小行星上的超高速撞击，其改进的材料模型成功再现了类似(433)Eros的大型连贯碎片的形成，为处理复杂界面和边界条件提供了新方案。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是模拟小行星（asteroid）遭受超高速撞击（hyper-velocity impact）的物理过程，以更真实地研究小行星演化、家族形成及行星防御等天体物理学问题。

Data: 本研究通过结合实验室冲击实验数据和与光滑粒子流体动力学（SPH）模拟的基准对比，对提出的物质点法（MPM）框架进行了严格验证。

Method: 本研究采用并验证了物质点法（Material Point Method, MPM）作为核心数值模拟技术，其实现包含了对材料模型的关键改进，如引入了压力相关的C^1连续屈服准则与可量化塑性应变，以及分辨率无关的Grady-Kipp碎裂模型，以捕捉极端条件下地质材料的复杂物理。

Result: 本研究验证了物质点法（MPM）能够精确模拟小行星高速撞击过程，成功再现了类似(433) Eros的大型连贯碎片的形成。
...
### [44] [VIGILant: an automatic classification pipeline for glitches in the Virgo detector](https://arxiv.org/abs/2604.13687)
*Tiago Fernandes,Francesco Di Renzo,Antonio Onofre,Alejandro Torres-Forné,José A. Font*

**TL;DR**: 本文针对Virgo引力波探测器中的瞬态噪声（glitches），开发并部署了自动分类可视化管线VIGILant，发现基于ResNet34的卷积神经网络模型在测试集上取得了0.9833的准确率，性能优于基于结构化参数的树模型。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于引力波探测器中常见的数据污染问题，即突发噪声（glitches）的自动分类与可视化，以提升天体物理信号观测与分析的有效性。

Data: 该研究使用来自Virgo探测器O3b运行阶段的精选故障（glitches）数据集。

Method: 研究采用机器学习方法，包括基于结构化Omicron参数的树模型（决策树、随机森林和XGBoost）以及基于频谱图训练的卷积神经网络（ResNet）。

Result: VIGILant管道利用ResNet34模型在测试集上实现了0.9772的F1分数和0.9833的准确率，对引力波探测器中的瞬态干扰（glitch）分类性能优越。
...
### [45] [Robust parameter inference for Taiji via time-frequency contrastive learning and normalizing flows](https://arxiv.org/abs/2604.13867)
*Tian-Yang Sun,Bo Liang,Ji-Yu Song,Song-Tao Liu,Shang-Jie Jin,He Wang,Ming-Hui Du,Jing-Fei Zhang,Xin Zhang*

**TL;DR**: 针对太极（Taiji）探测器配置中的大质量黑洞双星，本研究结合条件归一化流、时频多模态融合编码器和对比学习，提出了一种抗瞬态噪声干扰的摊销推断框架，并引入神经瞬态噪声生成器以低成本合成高保真噪声数据，实验表明该方法在噪声污染下能产生比传统马尔可夫链蒙特卡洛基线更准确、校准更好的后验分布，且时频全模型结合对比学习的整体性能最优，同时通过连续排序概率评分补充了标准覆盖诊断，为未来空间引力波观测提供了快速、稳健的贝叶斯参数估计框架。


<details>
  <summary>Details</summary>

Background: 该研究致力于提升未来空间引力波（gravitational wave）观测中针对大质量黑洞双星（massive black hole binary）参数推断的鲁棒性，以应对瞬态噪声伪影（glitch）带来的重大挑战。

Data: 该研究使用了一个神经瞬态噪声生成器（neural glitch generator）来高效产生高保真合成瞬态噪声，以支持大规模训练，并采用了Taiji探测器配置下的受污染引力波数据。

Method: 研究结合了条件归一化流（conditional normalizing flows）、时频多模态融合编码器（time-frequency multimodal fusion encoder）和对比学习（contrastive learning），并引入了神经瞬态噪声生成器（neural glitch generator）来高效合成受污染数据以进行大规模训练。

Result: 针对引力波观测中瞬态噪声（glitch）干扰的挑战，本研究提出了一种结合条件归一化流、时频多模态融合编码器和对比学习的鲁棒摊销推断框架，并通过系统实验证实，该方法在噪声污染下能比传统马尔可夫链蒙特卡洛基线产生更准确、校准更好的后验分布。
...
### [46] [Solar photospheric spectrum microvariability III. Radial velocities and line profiles in magnetic active-region granulation](https://arxiv.org/abs/2604.13166)
*Dainis Dravins,Hans-Günter Ludwig,Matthias Steffen,Carlos Allende Prieto,Lars Koesterke*

**TL;DR**: 该研究通过磁流体动力学三维建模揭示了强磁场（240 mT）下太阳光球层磁化米粒组织（magnetic granulation）的光谱线特征，发现其抑制对流运动、产生更对称的谱线并意外地呈现对流红移（convective redshift），这为区分系外行星引起的径向速度信号与恒星活动噪声提供了关键依据。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是理解恒星活动（如磁活动区）对光谱线轮廓和视向速度的影响，以区分系外行星信号与恒星大气噪声。

Data: 研究使用了合成超高分辨率光谱（R~900,000），该光谱通过对非磁化区域和强磁化（240 mT = 2400 G）的米粒组织进行流体动力学三维建模生成。

Method: 研究采用三维磁流体动力学模型模拟太阳磁化区和非磁化区的对流运动，并基于此合成超高分辨率（R~900,000）光谱，通过对比分析不同谱线（包括可见光和近红外波段）的轮廓、对流速度偏移及双极子形态，以提取磁化对流对谱线形成的影响。

Result: 强磁场抑制了对流运动、减少了能量流动、使谱线更对称并消除了常见的蓝移及其典型的C形双极轮廓。
...
### [47] [SJET: An Interactive Solar Jet Extraction Tool](https://arxiv.org/abs/2604.13590)
*Song Tan,Alexander Warmuth,Frédéric Schuller,Yuandeng Shen,Yue Fang,Jake A. J. Mitchell,Zedong Liu*

**TL;DR**: 该研究开发了用于太阳喷流（solar jet）特征提取的交互式工具SJET，它通过整合多种阈值算法与形态学操作、利用形态不对称性客观确定传播方向并结合二次贝塞尔曲线建模轴线，实现了对喷流几何参数的准确提取，为大规模样本统计研究建立了技术基础。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是太阳大气中动态准直等离子体流（solar jets）的自动识别与特征提取方法研究。

Data: 该研究使用了太阳轨道器（Solar Orbiter）的极紫外成像仪（EUI）高分辨率图像和太阳动力学天文台（SDO）的大气成像组件（AIA）的观测数据。

Method: 该研究采用了一种集成五种阈值算法与形态学操作的方法，并结合了基于圆形区域识别起止点以利用形态不对称性客观确定喷流传播方向的新方法，以及使用二次贝塞尔曲线（quadratic Bézier curves）建模喷流轴线以精确提取几何参数。

Result: SJET工具通过集成多种阈值算法与形态学操作，并结合基于圆形区域的起止点识别与二次贝塞尔曲线轴建模方法，实现了对太阳喷流（solar jet）几何参数（如长度、宽度、曲率、偏转角）的准确提取。
...
### [48] [Probing Coronal Activity Using Radio Signals Based on the 2021 superior conjunction of Mars: the Downlink Data from Tianwen-1](https://arxiv.org/abs/2604.13670)
*Yu-Chen Liu,De-Qing Kong,Song Tan,Zi-Han Zhao,Zan Wang,Dong-Hao Liu,Xin-Ying Zhu,Yan Su,Hong-Bo Zhang*

**TL;DR**: 基于天问一号（Tianwen-1）下行信号在10个太阳半径（solar radii）内受太阳风扰动的分析，该研究通过提取频率闪烁（frequency scintillation）参数，首次利用深空探测器信号有效识别了包括冕流、高速太阳风和日冕物质抛射（CMEs）在内的多种日冕活动，并验证了该参数与太阳风速度变化间存在明确的时间相关性和延迟，从而证实了空间定位太阳活动的可行性。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是利用行星际探测器下行信号受太阳风扰动（频率闪烁）的现象，探测日冕活动（coronal activity）并验证其空间定位的可行性。

Data: 研究使用的数据来源是天问一号（Tianwen-1）火星探测器在2021年10月第一次上合期间，由武清70米射电望远镜接收的下行链路信号，以及用于对比的SOHO和SDO卫星数据。

Method: 研究采用的核心分析方法是对太阳风引起的多普勒频率闪烁（frequency scintillation）谱进行分析以提取特征参数，并结合SOHO和SDO数据进行时空相关性比较与定量分析。

Result: 基于天问一号（Tianwen-1）下行信号的多普勒频率闪烁（frequency scintillation）参数分析，研究发现该参数能有效识别日冕流、高速太阳风及日冕物质抛射（CMEs）等太阳活动，并与SOHO和SDO数据在时空上存在强相关性。
...
### [49] [JOYS$+$: A JWST/MIRI survey of the evolution of H$_2$ winds and jets from low-mass protostars](https://arxiv.org/abs/2604.13773)
...
**TL;DR**: 基于JWST对33颗原恒星（Class 0和I）的观测，该研究发现H₂外流的质量损失率从Class 0到Class II阶段下降两个数量级，且其增大的喷流张角和温度结构与磁流体力学（MHD）盘风模型预测一致。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用JWST（詹姆斯·韦伯空间望远镜）的MIRI/MRS观测，探究原恒星（Class 0和I阶段）分子氢（H$_2$）外流的性质与演化，包括其形态、质量损失率以及速度与温度结构。

Data: 研究使用了JWST MIRI/MRS对13个单星和20个多重原恒星（Class 0和I）的观测数据，以及ALMA对亚毫米波CO的示踪数据。

Method: 研究采用JWST MIRI/MRS观测数据，结合ALMA亚毫米波CO数据，通过构建H₂ S(1)和S(7)谱线的流量与速度图，并利用旋转图（rotation diagram）拟合H₂谱线，结合外流宽度和线速度测量质量损失率。

Result: H$_2$外流质量损失率从0类到II类阶段下降两个数量级，并与光度相关。
...
### [50] [Interferometer observations of pulse pairs in an interstellar communication experiment](https://arxiv.org/abs/2604.13886)
*William J. Crilly*

**TL;DR**: 基于对参宿四（Rigel）方向3.7 Hz带宽脉冲对的相位测量，该研究提供了该脉冲源具有天体起源的新证据。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是探测并验证来自参宿七（Rigel）方向、带宽为3.7 Hz的异常脉冲信号的天体物理起源。

Data: 该研究使用了一个南向双单元相位测量干涉仪（two-element phase-measuring interferometer）对目标方向进行监测，并同步记录了0.27秒脉冲对测量期间的954赫兹和50兆赫兹带宽的噪声功率数据。

Method: 该研究采用了两单元相位测量干涉仪（two-element phase-measuring interferometer）对假设脉冲源进行监测。

Result: 基于3.7 Hz带宽脉冲对的相位测量，该研究进一步证实了脉冲信号源具有天体起源。
...
### [51] [Potentials of axisymmetric razor-thin disks](https://arxiv.org/abs/2604.13091)
*J. An*

**TL;DR**: 该研究针对轴对称无限薄盘（razor-thin disks）的引力势，证明了其盘一侧的势等价于垂直于盘的轴线上线质量分布的势，并系统给出了与特定区间上β分布族等价的盘面密度模型，这些模型的势至多只需对坐标初等函数进行一次实积分且常可解析表达，为构建现实盘势提供了基础。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于轴对称薄盘（axisymmetric, razor-thin disks）引力势的理论构建与等价性分析。

Data: 本研究采用理论建模与数学推导方法，未使用观测或仿真数据。

Method: 采用的核心分析方法是将轴对称薄盘（razor-thin disks）的引力势等效于垂直于盘面的轴上线性质量分布所产生的势，并基于此等价关系，研究由初等β分布及其相关分布在特定区间定义的表面密度剖面。

Result: 主要科学发现是：轴对称薄盘（razor-thin disks）在特定条件下，其一侧势场等价于垂直于盘面的轴上线性质量分布所产生的势场，并建立了产生相同势场的两种质量分布之间的联系。
...
### [52] [Searching for axions with quantum interferometry](https://arxiv.org/abs/2604.13181)
*Tanmay Kumar Poddar,Michael Spannowsky*

**TL;DR**: 该论文提出利用量子相位（包括Aharonov-Bohm相位和Berry相位）作为探测轴子（axion）的新框架，通过在射频超导量子干涉器件（rf-SQUID）中测量振荡轴子暗物质背景诱导的电压信号，可在轴子质量约$10^{-10}~\mathrm{eV}$处将最小轴子-光子耦合（$g_{aγγ}^{\mathrm{min}}$）灵敏度提升至约$7.8\times10^{-14}~\mathrm{GeV}^{-1}$，比现有极限提高一至两个数量级，并为非暗物质轴子及拓扑磁绝缘体中的轴子-光子准粒子（AQP）系统提供了原理性相位探测方案。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是探索利用量子相位测量（包括Aharonov-Bohm相位和Berry相位）作为探测轴子（axion）及其与光子相互作用的新实验框架。

Data: 该研究使用了基于射频超导量子干涉仪（rf-SQUID）的量子相位测量实验装置以及马赫-曾德尔干涉仪（Mach-Zehnder interferometer）的理论模型，并分析了在拓扑磁绝缘体中实现的光子-轴子准粒子（AQP）-轴子三能级系统的理论框架。

Method: 核心分析方法、物理模型或技术手段包括：利用射频超导量子干涉器件（rf-SQUID）测量阿哈罗诺夫-玻姆（Aharonov-Bohm, AB）相位，通过马赫-曾德尔干涉仪（Mach-Zehnder interferometer）观测几何（Berry）相位，并在拓扑磁绝缘体中构建光子-轴子准粒子（axion quasiparticle, AQP）-轴子三能级系统以分析太赫兹（THz）贝里相位。

Result: 该研究提出利用量子相位测量（包括阿哈罗诺夫-玻姆（Aharonov-Bohm, AB）相位和贝里（Berry）相位）作为探测轴子（axion）的新框架，在射频超导量子干涉器件（rf-SQUID）中，振荡轴子暗物质背景产生的有效电流可诱导随时间变化的磁通，从而通过约瑟夫森相位产生可测电压信号，对代表性基准参数（轴子质量约10⁻¹⁰ eV）的探测灵敏度可达最小轴子-光子耦合g_{aγγ}ᵐⁱⁿ ∼ 7.8×10⁻¹⁴ GeV⁻¹，比现有极限提升约一至两个数量级。
...
### [53] [Axion Inflation from Heavy-Fermion One-Loop Effects](https://arxiv.org/abs/2604.13674)
*Kai-Ge Zhang,Jian-Feng He,Chengjie Fu,Zong-Kuan Guo*

**TL;DR**: 这篇论文通过积分掉经历平滑局域阈值转变的具有暴胀子依赖复质量的狄拉克费米子，推导了轴子暴胀的单圈有效描述，发现阈值转变诱导的暴胀子与规范场修正会瞬态增强局域规范场产生，从而在BBO和DECIGO探测灵敏度的分赫兹频段内产生一个手征随机引力波背景，同时保持在典型原初黑洞界限以下。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是探讨在轴子暴胀（axion inflation）框架下，通过整合重狄拉克费米子（heavy Dirac fermion）的阈值跃迁效应，研究暴胀子与规范场（gauge field）的耦合修正及其对原初引力波（primordial gravitational-wave）产生的瞬态增强。

Data: 该研究通过积分掉一个具有暴胀子依赖的复质量并经历平滑局部阈值跃迁的重狄拉克费米子，推导了轴子暴胀的单圈有效描述。

Method: 研究采用的核心分析方法是通过积分掉具有暴胀子依赖复质量且经历平滑局域阈值转变的重狄拉克费米子，推导出轴子暴胀的单圈有效描述。

Result: 该研究通过积分出经历平滑局域阈值转变的具有暴胀子依赖复质量的狄拉克费米子，推导了轴子暴胀（axion inflation）的单圈有效描述，其中阈值转变引发了对暴胀子和规范场（gauge sector）的关联修正，包括科尔曼-温伯格项（Coleman-Weinberg term）、真空极化修正和反常诱导的Chern-Simons耦合，这些效应共同瞬时增强并局域化了规范场产生，从而在BBO和DECIGO探测灵敏度范围内的分赫兹频段（deci-hertz band）产生了一个手征随机引力波背景（chiral stochastic gravitational-wave background），同时保持在典型原初黑洞（primordial-black-hole）界限以下。
...
### [54] [Radiation hydrodynamic simulations for the origin of quasi-periodic oscillations for accretion onto supermassive black holes](https://arxiv.org/abs/2604.13199)
*Yiyang Lin,Erlin Qiao,Jifeng Liu,Meng Guo,Zikun Lin*

**TL;DR**: 通过辐射流体动力学模拟发现，在超大质量黑洞（SMBH）吸积流中，观测到的准周期振荡（QPO）频率与理论计算的最大径向本轮频率（$ν_{\rm r,max}$）在临界半径（3.8倍史瓦西半径）处相符，且该关系可推广解释不同质量黑洞的活动星系核（AGN）和潮汐瓦解事件（TDE）样本。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是探索吸积超大质量黑洞（SMBH）系统中准周期振荡（QPO）的物理起源。

Data: 本研究使用了基于特定初始条件（在10倍史瓦西半径处以固定质量吸积率注入物质，并设定黑洞质量为$10^7M_{\odot}$）进行辐射流体动力学模拟（radiation hydrodynamic simulations）所产生的数据。

Method: 采用辐射流体动力学（radiation hydrodynamic）模拟，通过在不同吸积率下于固定半径（10倍史瓦西半径）注入质量，并设置黑洞质量为$10^7M_{\odot}$，分析了不同半径处质量流入率随时间的变化。

Result: 研究发现，模拟得到的准周期振荡（QPO）频率在大于临界半径（3.8倍史瓦西半径）时与解析计算得到的径向本轮频率（radial epicyclic frequency）高度一致，并认为最大径向本轮频率（$ν_{\rm r,max}$）可作为观测到的$ν_{\rm QPO}$的良好代理。
...
### [55] [Projections of Earth's Technosphere: Civilization Collapse-Recovery Dynamics and Detectability](https://arxiv.org/abs/2604.13774)
*Celia Blanco,Jacob Haqq-Misra,George Profitiliotis*

**TL;DR**: 该研究通过模拟地球起源文明的十种未来情景，发现文明的技术活跃度（duty cycle）在约0.38至1.00之间变化，其长期轨迹由治理结构、资源压力和灾害暴露共同塑造，且降低资源消耗率与提升灾后恢复能力是避免崩溃、提升可探测性的关键杠杆。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景或科学主题是评估技术文明（technological civilization）的长期存续可能性及其对地外技术特征（technosignatures）搜寻的启示。

Data: 研究使用混合确定性-随机模拟方法，在十个合理的地球起源文明未来情景中，对千年时间窗口内的崩溃-恢复动态进行了建模。

Method: 研究采用混合确定性-随机模拟方法，在千年时间窗口内对十个地球起源文明的未来情景进行崩溃-恢复动态建模。

Result: 模拟表明，文明的技术活跃期占比（duty cycle）在约0.38至1.00之间，其长期轨迹由治理结构、资源压力和灾害暴露的相互作用塑造。
