...
**TL;DR**: 实验测量了星际无定形固态水冰(ASW)中氧气(O2)的表面扩散系数(1E-16至1E-15 cm^2 s^-1)和扩散能垒(10±3 meV)，发现其具有高迁移性且约20%的超挥发物会被捕获在冰基质中。


<details>
  <summary>Details</summary>

Background: 星际冰（interstellar ices）主要由含少量超挥发物（hypervolatiles）如O2的非晶态固态水（amorphous solid water, ASW）构成，其扩散限制反应在空间化学中起关键作用。

Data: 数据来源于在超高真空室中进行的实验，通过红外光谱（IR spectroscopy）和四极杆质谱（quadrupole mass spectrometry）监测多孔无定形固态水（ASW）中O2的表面扩散和捕获效率。

Method: 采用超高压真空腔结合红外光谱(IR spectroscopy)和四极杆质谱(quadrupole mass spectrometry)技术，通过拟合等温阶段质谱信号的菲克模型(Fickian model)提取扩散系数，并分析程序升温脱附(TPD)相测量捕获效率。

Result: 通过实验测量得出O2在非晶态固态水(ASW)中的扩散能垒为10±3 meV(116±35 K)，表明其在星际水冰中具有高迁移性。
...
**TL;DR**: 发现星系团XLSSC 122（红移z=1.98）具有高浓度（$c_{200 \rm c}=6.3 \pm 0.3$）和质量（$M_{200\rm c}=1.6 \pm 0.3\times 10^{14}~M_{\odot}$）特征，多波段数据揭示其NE-SW轴向延展与SZ/X射线峰偏移，表明存在显著并合活动，为JWST研究高红移透镜效应提供了范例。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（詹姆斯·韦伯太空望远镜）等先进设备探索高红移（$z\sim2$）星系团XLSSC 122的质量分布及动力学特征，以揭示宇宙早期（仅为当前年龄三分之一时）星系团演化与并合活动的物理机制。

Data: 数据来源于JWST/NIRCam深场成像、钱德拉(Chandra)X射线数据、MeerKAT射电成像、ALMA+ACA/ACT的苏尼亚耶夫-泽尔多维奇(Sunyaev-Zel'dovich, SZ)效应测绘以及新的JWST星系团内光(intracluster light)测量。

Method: 采用弱引力透镜(weak-lensing)分析结合JWST/NIRCam成像重建星系团质量分布，并通过多波段数据（X射线、射电、SZ效应）验证结构一致性。

Result: XLSSC 122星系团在红移(redshift) $z = 1.98$处展现出高浓度参数(concentration parameter) $c_{200 \rm c}=6.3 \pm 0.3$，与模拟和观测的质量-浓度关系存在显著差异，表明其经历了非典型的形成演化过程。
...
### [3] [Chemical composition and enrichment of the Centaurus cluster core seen by XRISM/Resolve](https://arxiv.org/abs/2512.11028)
...
**TL;DR**: 利用XRISM/Resolve和XMM-Newton/RGS数据首次精确测量了半人马座星系团核心区11种元素丰度比，发现氮/铁（N/Fe）和镁/铁（Mg/Fe）显著偏离太阳系组成，并证实渐近巨星分支（AGB）恒星、核坍缩超新星（SNcc）和Ia型超新星（SNIa）共同贡献了金属富集。


<details>
  <summary>Details</summary>

Background: 星系团内高温X射线辐射介质(ICM)的金属丰度研究为理解宇宙大尺度金属产生和富集过程提供了关键约束。

Data: 数据来源于XRISM/Resolve（5 eV分辨率）对半人马座星系团核心区域的287 ks观测，以及XMM-Newton/RGS存档数据获取的N/Fe、O/Fe、Ne/Fe和Mg/Fe比值。

Method: 采用XRISM/Resolve（~5 eV分辨率）和XMM-Newton/RGS（补充N/Fe、O/Fe、Ne/Fe、Mg/Fe）的高分辨率X射线光谱技术测量金属丰度及其比值。

Result: 利用XRISM/Resolve和XMM-Newton/RGS数据测量半人马座星系团核心的化学组成，发现11种元素丰度比中有9种与太阳系一致，但N/Fe（超太阳）和Mg/Fe（半太阳）显著偏离，表明并非所有系统都具有完全太阳系成分的普遍性。
...
### [4] [Dynamical disequilibrium in dwarf galaxies: rethinking gas dynamics, rotation curves, and dark matter inference](https://arxiv.org/abs/2512.11033)
...
**TL;DR**: 研究发现低质量星系（$10^{10.75}<M_\mathrm{200c}/\mathrm{M}_\odot<10^{11}$）气体盘普遍偏离流体动力学和离心平衡（hydrodynamical and centrifugal equilibrium），内区（inner few kiloparsecs）误差最大，非平衡运动主要由恒星反馈（stellar feedback）、自引力气体团块（self-gravitating gas clumps）和活动星系核（AGN）能量注入驱动，且气体盘动力学状态与星系质量、形态等性质显著相关。


<details>
  <summary>Details</summary>

Background: 研究探讨了低质量星系气体盘（gas discs）中流体动力学和离心平衡（hydrodynamical and centrifugal equilibrium）偏离的量化及其对暗物质密度分布约束的影响。

Data: 数据来源于COLIBRE宇宙学流体动力学模拟（COLIBRE cosmological hydrodynamical simulations）中质量范围在$10^{10.75}<M_\mathrm{200c}/\mathrm{M}_\odot<10^{11}$的低质量星系气体盘。

Method: 采用欧拉加速度平衡分析（Eulerian acceleration balance）评估气体盘的非平衡状态，并结合恒星反馈（stellar feedback）、自引力气体团块（self-gravitating gas clumps）和活动星系核（AGN）能量注入等扰动源进行分类研究。

Result: 低质量星系气体盘普遍偏离流体静力学和离心平衡（hydrodynamical and centrifugal equilibrium），内区（inner few kiloparsecs）失衡最显著且速度估计误差最大。
...
### [5] [The abundance of thin dwarf galaxies: a challenge for cosmological hydrodynamical simulations](https://arxiv.org/abs/2512.11035)
*José A. Benavides,Laura V. Sales,Julio F. Navarro,Simon D. M. White,Carlos S. Frenk,Kyle A. Oman,Shaun Cole*

**TL;DR**: 通过分析GAMA等观测数据发现，恒星质量在$10^9-10^{10} ~ \rm{M_\odot}$范围内40%的星系本征扁平度（intrinsic flatness）低于1:5（$c/a<0.2$），且矮星系（dwarf galaxies）中该比例达30%，但现有宇宙流体力学模拟（cosmological hydrodynamical simulations）无法重现观测到的薄盘矮星系比例，揭示了当前数值模型在低质量盘星系形成机制上的不足。


<details>
  <summary>Details</summary>

Background: 研究探讨了不同恒星质量范围内薄盘星系（thin galaxies）的普遍性及其与数值模拟的差异。

Data: 数据来源于GAMA、DESI、ALFALFA和Nearby Galaxy星表以及TNG50、FIREbox、Romulus25等宇宙学流体动力学数值模拟。

Method: 采用投影轴比（projected axis ratio, q）分布推断本征扁平星系（intrinsically flat galaxies）丰度的方法，结合GAMA、DESI、ALFALFA和Nearby Galaxy星表数据进行分析。

Result: 在恒星质量范围$10^9<M_{\star}/\rm{M_\odot}<10^{10}$内，多达$40\%$的星系本征扁平度（intrinsic flatness）低于1:5（$c/a<0.2$），而$c/a<0.3$的比例升至$\sim 80\%$。
...
**TL;DR**: 通过高分辨率N体模拟发现，核星团(NSC)中恒星质量黑洞(BH)显著加速中等质量黑洞(IMBH)双星的下沉速率并缩短其并合时间，同时揭示BH通过能量提取-转移机制持续主导IMBH双星演化，促进其通过并合增长并可能增强径向轨道中等质量比旋近(IMRI)事件。


<details>
  <summary>Details</summary>

Background: 研究聚焦于核星团（NSCs）中中等质量黑洞（IMBH）双星与恒星质量黑洞（BHs）及恒星群体间的动力学相互作用及其对IMBH双星演化路径的影响。

Data: 数据来源于基于NGC205和NGC404两个本地矮星系观测参数构建的高分辨率N体模拟（N-body simulation），实现了1太阳质量（$1\rm\;M_{\odot}$）的恒星粒子质量分辨率和10太阳质量（$10\rm\;M_{\odot}$）的黑洞质量分辨率。

Method: 采用高分辨率N体模拟（N-body simulation）方法，基于NGC205和NGC404的观测参数构建核星团（NSCs）模型，首次实现1太阳质量（M⊙）的恒星粒子分辨率和10太阳质量（M⊙）的黑洞（BH）分辨率。

Result: 在核星团(Nuclear Star Cluster)中，恒星质量黑洞(stellar-mass black holes)显著加速了中等质量黑洞双星(IMBH binary)的轨道衰减速率，使其下沉率(sinking rate)提高近一倍并缩短了一半的并合时间。
...
### [7] [MIRACLE III. JWST/MIRI expose the hidden role of the AGN outflow in NGC 1068](https://arxiv.org/abs/2512.11042)
...
**TL;DR**: 结合JWST的MIRI和MUSE数据揭示了NGC 1068活动星系核(AGN)周围多相气体性质，发现[OIV]示踪的尘埃丰富离子化外流比[OIII]快300 km/s，且携带质量远超光学观测结果，支持AGN驱动两阶段加速风模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（James Webb Space Telescope）的中红外（Mid-IR）和光学IFS（积分场光谱）数据，揭示活动星系NGC 1068核周多相气体特性及其与活动星系核（AGN）外流和射电喷流的相互作用机制。

Data: 数据来源为JWST（James Webb Space Telescope）的中红外积分场光谱仪（MIRI）和光学积分场光谱仪（MUSE）对活动星系NGC 1068的观测数据。

Method: 采用中红外积分视场光谱仪（MIRI）和光学积分视场光谱仪（MUSE）数据，结合多云运动学模型（MOKA）和光致电离模型（HOMERUN）分析电离气体特性，揭示活动星系核（AGN）驱动的外流与周围星际介质（ISM）的相互作用。

Result: result：JWST观测揭示NGC 1068存在AGN驱动的双组分外流，其中中红外示踪的尘埃富集组分携带离子化气体质量（达数百万太阳质量）远超光学示踪结果，支持能量驱动风模型，外流速度达~2000 km/s并有效耦合周围ISM。
...
### [8] [Dynamical constraints on the S2 (S0-2) star possible companions](https://arxiv.org/abs/2512.11046)
*Rodrigo P. Silva,Alexandre C. M. Correia,Tjarda C. N. Boekholt,Paulo J. V. Garcia*

**TL;DR**: 通过数值模拟发现S2恒星（Sgr A*周围S星团中的B型主序星）在轨道周期短于100天、偏心率低于0.8且轨道共面性较高时可存在未被观测到的伴星（恒星或行星），且伴星存在仅使S2的天体测量信号改变不超过5微角秒。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中心超大质量黑洞（Sgr A*）周围S星团中B型主序星S2（S0-2）是否存在未被探测到的恒星或行星伴星，探讨其缺失可能源于观测偏差或动态恶劣环境（如潮汐瓦解或并合）。

Data: 研究采用了一阶后牛顿修正的N体数值模拟方法，通过10^5次模拟实验并假设伴星轨道参数服从均匀分布来探索S2恒星可能存在的伴星系统。

Method: 采用一阶后牛顿修正的N体数值模拟方法，对S2恒星可能存在的伴星轨道参数进行均匀分布采样的大规模数值实验（10^5次模拟）。

Result: 银河系中心超大质量黑洞Sgr A*附近的S2恒星（S0-2）可能拥有轨道周期短于100天、偏心率低于0.8且相互倾角范围不限的恒星或行星伴星，但现有观测仅能探测到约3.0%的未受干扰的恒星双星系统。
...
### [9] [Balmer Transition Signatures from Gas-Enshrouded, Dust-Poor Active Galactic Nuclei](https://arxiv.org/abs/2512.11050)
*Zu Yan,Kohei Inayoshi,Kejian Chen,Jingsong Guo*

**TL;DR**: 研究发现JWST观测到的小红点（LRDs）中显著的巴尔末线特征（如大巴尔末减幅和强巴尔末跃变）源于无尘埃高密度气体（n_H≳10^8-10 cm^-3）的共振散射而非尘埃消光，并揭示其宽线区气体质量仅约10 M⊙，表明核区恒星形成活动极弱。


<details>
  <summary>Details</summary>

Background: 研究聚焦于詹姆斯·韦伯太空望远镜（JWST）新发现的"小红点"（LRDs）类星体（AGNs）中独特的巴尔末跃迁特征（Balmer-transition features），特别是其大巴尔末减幅（Balmer decrements）的物理起源，挑战了传统尘埃消光（dust extinction）解释并揭示无尘埃高密度气体（dust-free, dense gas）中巴尔末共振散射（Balmer resonance scattering）的关键作用。

Data: 数据来源于JWST（James Webb Space Telescope）对LRDs（Little red dots）的观测，包括MIRI（Mid-Infrared Instrument）获取的近至中红外数据以及Balmer跃迁特征（如Balmer吸收、Balmer跳变和宽$\mathrm{H}α$发射线）的测量。

Method: 采用辐射传输计算（radiation transfer calculations）分析无尘埃高密度气体（dust-free, dense gas）中的巴尔末共振散射（Balmer resonance scattering）效应。

Result: LRDs（Little red dots）的大巴尔末减幅（Balmer decrement）和巴尔末跃迁特征源于无尘埃高密度气体中的共振散射（resonance scattering），而非传统认为的尘埃消光（dust extinction）。
...
### [10] [Milky Way Globular Clusters: Nurseries for Dynamically-Formed Binary Black Holes](https://arxiv.org/abs/2512.11059)
...
**TL;DR**: 提出了一种耦合星系形成模型GAMESH与星团种群合成代码的自洽理论框架，揭示了球状星团中动态形成的黑洞双星（black hole binaries）的演化机制，证实其层级合并（hierarchical black hole mergers）对跨越对不稳定性质量间隙（pair-instability mass gap）的大质量黑洞形成有重要贡献，并量化了双黑洞并合率密度随红移（redshift）增至z=5的宇宙演化特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于动态形成的黑洞双星（black hole binaries）的形成、演化和并合位点，旨在解释LIGO-Virgo-KAGRA合作组观测到的超大质量事件，并耦合星系形成模型GAMESH与星团种群合成代码以追踪球状星团（globular clusters）的宇宙演化和大质量黑洞并合。

Data: 数据来源包括星系形成模型GAMESH和球状星团群体合成代码，用于追踪球状星团的宇宙演化和大质量黑洞合并。

Method: 采用耦合星系形成模型GAMESH与星团种群合成代码的自洽理论框架，追踪球状星团宇宙演化和大质量黑洞并合过程。

Result: 发现了约30%的银河系晕中球状星团可能源自其卫星星系。
...
### [11] [Effects of Varied Cosmic Ray Feedback from AGN on Massive Galaxy Properties](https://arxiv.org/abs/2512.11062)
*Charvi Goyal,Sam B. Ponnada,Philip F. Hopkins,Sarah Wellons,Jose A. Benavides,Kung-Yi Su*

**TL;DR**: 研究发现活动星系核(AGN)通过宇宙线(CRs)反馈机制(注入效率变化约1.5个数量级且局部传输可变)能够自调节地抑制大质量星系(晕质量M$_{\rm halo} \geq$ 10$^{12.5}$ M$_{\odot}$，恒星质量$10.4 \leq \log(\frac{M_*}{M_\odot}) \leq 11$)的恒星形成，但其星系周介质(CGM)气体性质存在数量级差异。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）通过宇宙线（CRs）反馈机制调控大质量星系（M$_{\rm halo} \geq $ 10$^{12.5}$ M$_{\odot}$，$10.4 \leq \log(\frac{M_*}{M_\odot}) \leq 11$）中恒星形成停止的物理过程及其不确定性。

Data: 数据来源于反馈真实环境(FIRE-3)项目的高分辨率数值模拟，包含多通道活动星系核(AGN)反馈机制，显式演化中心黑洞产生的动能/机械能、辐射及能谱分辨的宇宙线(CRs)。

Method: 采用高分辨率数值模拟（FIRE-3）结合多通道活动星系核（AGN）反馈模型，显式演化中心黑洞产生的动能/机械能、辐射及光谱分辨宇宙线（CRs），并探索不同宇宙线反馈与传输假设（基于银河系局部星际介质约束校准）。

Result: 在多种宇宙射线（CR）反馈和传输假设下，即使注入效率差异达约1.5个数量级且局部传输可变，仍能自调节产生符合观测星系比例关系的淬灭星系，但其环星系介质（CGM）气体性质存在数量级差异。
...
### [12] [Connection Between Dwarf Galaxies and Globular Clusters: Insights from the Perseus Cluster Using Subaru Imaging and Keck Spectroscopy](https://arxiv.org/abs/2512.11070)
...
**TL;DR**: 通过对189个英仙座星系团(Perseus cluster)矮星系及其球状星团(GC)系统的多波段观测分析，首次在统一数据集中发现矮星系GC比质量(GC specific mass)与恒星质量呈反相关，且低表面亮度或大有效半径的矮星系具有更高GC丰度，揭示了GC形成效率与星系早期致密状态或GC破坏效率的潜在联系。


<details>
  <summary>Details</summary>

Background: 研究主题为利用宽视场成像和光谱观测探讨英仙座星系团（Perseus cluster）中矮星系（dwarf galaxies）及其球状星团系统（globular cluster systems）的演化规律与环境依赖性。

Data: 数据来源于昴星团望远镜（Subaru）Hyper Suprime-Cam的深度成像观测、凯克望远镜（Keck）的光谱观测以及补充的文献数据。

Method: 采用斯巴鲁（Subaru）Hyper Suprime-Cam深场成像和凯克（Keck）光谱观测技术，结合文献数据，对189个矮星系及其球状星团（GC）系统进行系统性分析。

Result: 在英仙座（Perseus）星系团中，矮星系的球状星团（GC）比质量（specific mass）与恒星质量呈反相关关系，且表面亮度较低或有效半径较大的矮星系往往拥有更多球状星团。
...
### [13] [The Interplay of Stellar Evolution and Collisions in Galactic Nuclei](https://arxiv.org/abs/2512.11175)
*Jaden S. Sidhu,Sanaea C. Rose,Frederic A. Rasio*

**TL;DR**: 研究发现核星团(NSC)中恒星碰撞类型（主序星或红巨星）取决于恒星空间分布陡度（stellar cusp），且银河系中心S星(S-stars)部分成员在主序期经历碰撞的概率较高。


<details>
  <summary>Details</summary>

Background: 研究聚焦于核星团(NSCs)中超大质量黑洞(SMBHs)周围极端致密环境中恒星碰撞与恒星演化的耦合机制及其对恒星质量函数和特殊恒星种群形成的影响。

Data: 数据来源于对1000颗1太阳质量（M⊙）恒星在均匀分布的动态弛豫0.5 M⊙恒星团中的模拟（simulation），使用COSMIC（Close Orbital Stellar Merger and Collision）工具跟踪碰撞过程中恒星的质量、半径和演化状态。

Method: 采用COMSIC工具模拟恒星属性随时间演化，并追踪恒星质量、半径和演化状态在核星团（NSC）碰撞过程中的变化。

Result: 在核星团(NSC)中，恒星碰撞类型（主序星或红巨星）取决于恒星尖峰(stellar cusp)陡峭程度，后者决定了恒星的空间分布。
...
### [14] [Synthetic polarization observations of magnetized pillars in HII regions: The limitations of the Davis-Chandrasekhar-Fermi method](https://arxiv.org/abs/2512.11207)
...
**TL;DR**: 结合三维辐射磁流体动力学(R-MHD)模拟与合成偏振观测，发现HII区边界柱状结构的磁场形态与强度受外部电离气体压力主导，导致戴维斯-钱德拉塞卡-费米(DCF)方法高估实际磁场强度至少2倍。


<details>
  <summary>Details</summary>

Background: 研究聚焦于HII区边界柱状结构中磁场形态与强度的形成机制及其与外部压缩作用的关联。

Data: data：研究采用三维辐射磁流体动力学（R-MHD）模拟生成的自洽形成的第一根柱状结构数据，并结合850微米合成尘埃偏振观测数据。

Method: 采用三维辐射磁流体动力学（R-MHD）模拟结合合成偏振观测技术，重点测试了戴维斯-钱德拉塞卡-费米（DCF）方法在外部主导条件下的适用性。

Result: 合成尘埃偏振图在850微米波段再现了模拟柱状结构的大尺度磁场形态，证实偏振测量是磁场几何结构的可靠示踪剂。
...
### [15] [Kennicutt-Schmidt relation of galaxies over 13 billion years in the COLIBRE hydrodynamical simulations](https://arxiv.org/abs/2512.11309)
...
**TL;DR**: 通过COLIBRE宇宙学流体动力学模拟揭示了红移0至8期间恒星形成率(SFR)面密度与气体(HI和H₂)面密度(Kennicutt-Schmidt关系)的演化规律，发现H₂消耗时间随红移增加而缩短20倍且与特定恒星形成率(sSFR)显著相关。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙时间尺度（0 ≤ z ≤ 8）上千秒差距（kpc）尺度上恒星形成率（SFR）面密度与气体面密度（即肯尼卡特-施密特关系，KS关系）的关联。

Data: 研究使用了COLIBRE最先进的宇宙学流体动力学模拟（cosmological hydrodynamical simulations）数据，该模拟包含实时非平衡化学过程、尘埃颗粒演化以及低至约10K的详细辐射冷却机制，可直接预测原子氢（HI）和分子氢（H$_2$）的KS关系。

Method: 采用COLIBRE最先进的宇宙学流体动力学模拟，结合实时非平衡化学、尘埃颗粒演化及低至≈10 K的详细辐射冷却过程，直接预测原子氢(HI)和分子氢(H2)的Kennicutt-Schmidt关系。

Result: COLIBRE模拟成功再现了红移$z\approx 0$时观测到的原子氢(HI)和分子氢(H$_2$)的肯尼卡特-施密特(KS)关系，并预测其弥散与恒星面密度、局部特定恒星形成率(sSFR)及气体金属丰度显著相关。
...
### [16] [The Influential Roles of Gravity, Turbulence, and Magnetic Fields in Shaping the Physical Evolution of Dense Massive Clumps](https://arxiv.org/abs/2512.11409)
*Moses Onyemaechi Asogwa,Seblu Humne Negu,Gemechu Muleta Kumssa,Innocent Okwudili Eya*

**TL;DR**: 通过分析ATLASGAL巡天中致密大质量团块(clumps)的动力学状态，发现重力(gravity)在小尺度上相对湍流(turbulence)的支配作用增强，表现为速度弥散-尺寸关系(sigma-L^0.11)比经典拉尔森关系更平缓、湍流能谱(E(k)-k^-1.21)较预期更平坦以及维里参数(virial parameter)随质量增加而下降(alpha_vir-M^-0.37)，表明大质量团块的结构演化主要由引力主导。


<details>
  <summary>Details</summary>

Background: 探讨重力、湍流和磁场在ATLASGAL巡天中致密大质量团块内恒星形成（SF）调控中的竞争作用。

Data: 数据来源于ATLASGAL巡天（APEX Telescope Large Area Survey of the Galaxy）中识别的致密、大质量团块（clump）的观测数据。

Method: 通过分析标度关系（scaling relations）、维里参数（virial parameters）和湍流能谱（turbulent energy spectra）评估了致密团块的动力学状态。

Result: 在致密、大质量云块（clump）中，引力相对于湍流的相对主导性随尺度减小而增强，符合多尺度坍缩（multiscale collapse）情景，但绝对能量平衡无法通过现有数据量化。
...
### [17] [Laboratory rotational spectroscopy and interstellar search for the protein precursor 4-oxobutanenitrile (HCOCH$_2$CH$_2$CN)](https://arxiv.org/abs/2512.11500)
*V. M. Rivilla,E. R. Alonso,W. Song,A. Insausti,A. Maris,F. J. Basterretxea,S. Melandri,I. Jiménez-Serra,E. J. Cocinero*

**TL;DR**: 首次测量了星际介质(ISM)中潜在生命前体分子4-氧代丁腈(4-oxobutanenitrile)的气相转动光谱并确定了其TC构象体，在银河中心分子云G+0.693-0.027中未检测到该分子并给出了其柱密度上限(<4×10¹² cm⁻²)。


<details>
  <summary>Details</summary>

Background: 探索星际介质(ISM)中生命前驱分子(prebiotic precursors)的存在与分布是揭示生命化学起源的关键科学主题。

Data: 使用两种互补技术测量气相旋转光谱：啁啾脉冲傅里叶变换微波（CP-FTMW）光谱（2-18 GHz）和自由喷射毫米波（FJ-AMMW）吸收光谱（59.6-80 GHz），并结合量子化学计算分析低能构象体。

Method: 采用互补的啁啾脉冲傅里叶变换微波谱(CP-FTMW)和自由喷射毫米波吸收谱(FJ-AMMW)技术测量气相旋转谱，结合量子化学计算确认低能构象体。

Result: result：在G+0.693-0.027分子云的超深谱线巡测中未检测到4-氧代丁腈（4-oxobutanenitrile），其柱密度上限为$N<$ 4 $\times$10$^{12}$ cm$^{-2}$（相对于H$_{2}$的丰度$<$ 2.9 $\times$10$^{-11}$），显著低于结构相关的简单分子（含-HCO和-CN基团）的观测丰度。
...
### [18] [The multiphase interstellar medium as a common origin for magnetic misalignment and TB parity violation](https://arxiv.org/abs/2512.11656)
*Andrea Bracco,Ari J. Cukierman,Raphael Skalidis,François Boulanger*

**TL;DR**: 研究发现银河系中高纬度尘埃偏振观测的两个现象（HI纤维结构与磁场错位及普朗克数据中正TB相关性）源于太阳邻域冷、暖气体相比例分布差异导致的大尺度投影效应，并提出了支持该几何模型的多波段观测证据。


<details>
  <summary>Details</summary>

Background: 研究主题为探讨银河系中尘埃偏振观测的两个现象（中性氢（HI）丝状结构与磁场错位及星际介质（ISM）宇称破缺导致的TB正相关）的物理起源及其关联性。

Data: 数据来源于普朗克卫星（Planck）在30 GHz（以同步辐射为主）、217 GHz和353 GHz（以尘埃辐射为主）的偏振数据，并结合了星光偏振测量结果。

Method: 采用多相几何模型（multiphase geometrical model）结合普朗克（Planck）30 GHz、217 GHz和353 GHz偏振数据及星光偏振测量分析大尺度磁化星际介质（ISM）投影效应。

Result: 在中间和高纬度区域，尘埃偏振银河系的两个可观测现象（HI丝状结构与磁场错位及普朗克数据中显示星际介质宇称违反的正TB相关性）之间存在观测联系，且主要由大角度尺度（≥10°）产生，北半球信号更强。
...
### [19] [GLIMPSE-D: An Exotic Balmer-Jump Object at z=6.20? Revisiting Photometric Selection and the Cosmic Abundance of Pop III Galaxies](https://arxiv.org/abs/2512.11790)
...
**TL;DR**: 通过JWST/NIRSpec光谱观测排除了GLIMPSE-16043为零金属丰度（Pop III）天体的可能性，发现其异常巴尔末跳变（Balmer jump）和Hα等效宽度特征需用热单黑体（$T_{\rm eff}\!\simeq\!10^{4.7}$ K）嵌入低电子温度（$T_{\rm e}$）星云环境的模型解释，并据此优化了Pop III候选体的光度筛选标准，估算出$z\simeq6$--7时Pop III宇宙恒星形成率密度为$\approx[10^{-6}$--$10^{-4}]$~$M_{\odot}$~yr$^{-1}$~cMpc$^{-3}$。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST/NIRSpec G395M光谱数据验证GLIMPSE-16043是否为第一代恒星（Pop III）候选体，并探讨其异常巴尔末跃变（Balmer jump）和Hα等效宽度的物理起源。

Data: 数据来源为JWST/NIRSpec G395M光谱观测数据和NIRCam测光数据。

Method: 采用JWST/NIRSpec G395M光谱技术对GLIMPSE-16043进行深度观测，结合测得的谱线流量和测光数据，通过分析其光谱能量分布（SED）中异常强的巴尔末跳跃（Balmer jump）和Hα等效宽度（equivalent width），并利用单黑体（single blackbody）模型嵌入低电子温度（low-Te）星云环境的物理模型进行拟合。

Result: GLIMPSE-16043的光谱能量分布需要极强的巴尔末跳跃（Balmer jump）（$-1.66 \pm 0.47$ 星等）和H$α$等效宽度（equivalent width）（$3750\pm1800$ Å），这些特征无法被现有恒星+星云或纯星云光致电离模型重现。
...
### [20] [Rapid sinking and efficient mergers of supermassive black holes in compact high-redshift galaxies](https://arxiv.org/abs/2512.11665)
...
**TL;DR**: 通过高红移宇宙学放大模拟（cosmological zoom-in simulation）结合改进版KETJU代码，揭示了质量达$8.5\times 10^{10} \ M_{\odot}$的大质量星系在$z=5$时经历早期致密化（early compaction）阶段，其中心超大质量黑洞（SMBH）在极高恒星密度（$ρ_{\star}\gtrsim 10^{13}\,\mathrm{M_\odot/kpc^3}$）下快速合并（$\sim 4-35\ \mathrm{Myr}$），且致密系统特性与JWST观测到的"小红点"（Little Red Dots）相符。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(z)环境下大质量星系的致密前身阶段演化及其与超大质量黑洞(SMBH)动力学过程的耦合机制。

Data: 数据来源于高红移致密前身星系的宇宙学放大模拟（cosmological zoom-in simulation），采用新版KETJU代码对超大质量黑洞（SMBHs）从播种到引力波（GW）驱动下亚秒差距尺度并合的全过程进行建模，并整合了PhenomD模型以解析引力波辐射的完整演化过程。

Method: 采用KETJU代码结合正则化积分和动力学摩擦亚网格模型，模拟了超大质量黑洞（SMBH）从种子形成到引力波（GW）辐射导致亚秒差距尺度并合的动力学过程。

Result: 所有九个大质量星系在红移范围z∼7-9经历了一个气体主导的早期致密化阶段，起始于恒星质量M⋆≳10⁸ M⊙，结束于几倍于M⋆∼10⁹ M⊙。
...
### [21] [A Spatially Resolved Evolutionary Sequence of Multi-wavelength AGN Host Galaxies](https://arxiv.org/abs/2512.11694)
*Gaoxiang Jin,Guinevere Kauffmann,Y. Sophia Dai,Martin J. Hardcastle,Bohan Yue*

**TL;DR**: 基于MaNGA巡天的1813个活动星系核（AGN）多波段分类研究，发现不同AGN类型（红外、宽线、窄线、射电）在恒星形成、气体电离和外流性质上存在显著差异，并提出了AGN与宿主星系从恒星形成到逐渐熄灭的演化序列。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）及其宿主星系的演化关系，探讨不同类型AGN（红外、宽线、窄线和射电）在恒星形成、气体电离和外流特性上的差异及其与星系质量、演化阶段的关联。

Data: 数据来源于MaNGA巡天（Mapping Nearby Galaxies at Apache Point Observatory survey）的1813个活动星系核（AGNs）的空间分辨观测数据，包括恒星形成、气体电离和外流特性，并基于中红外颜色、光学光谱和/或射电测光将这些AGNs分类为红外（IR）、宽线（BL）、窄线（NL）和射电（RD）AGNs。

Method: 采用积分视场光谱（IFU）数据结合多波段（中红外、光学、射电）分类方法，对活动星系核（AGN）进行空间分辨的恒星形成、气体电离和外流特性分析，并基于[OIII]发射线测量外流特征。

Result: 活动星系核（AGN）的星形成、气体电离和外流特性随类型呈现显著差异，其中红外（IR）、宽线（BL）和窄线（NL）AGN中心区域具有更高的特定星形成率、更年轻恒星种群和更硬电离状态，而射电（RD）AGN则与静止星系类似。
...
### [22] [Looking into the faintEst WIth MUSE (LEWIS): on the nature of ultra-diffuse galaxies in the Hydra I cluster. V. Integrated stellar population properties](https://arxiv.org/abs/2512.11710)
...
**TL;DR**: 基于MUSE光谱仪对长蛇座I星系团内24个低表面亮度(LSB)和超弥散星系(UDGs)的积分视场光谱分析，发现其平均金属丰度[M/H]为-0.9 dex、年龄10 Gyr，并根据相空间分布划分为早期和晚期落入星系两类，其中早期落入星系存在两种不同金属丰度及恒星质量聚集时标的亚群，表明存在潮汐或环境效应影响的不同形成机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低表面亮度星系（LSB）和超弥散星系（UDG）在长蛇座I星系团（Hydra I cluster）内部环境中的恒星种群特性与环境相关性。

Data: 数据来源于欧洲南方天文台（ESO）大型项目LEWIS，使用甚大望远镜（VLT）上的MUSE光谱仪获取了位于长蛇座I星系团0.4倍维里半径（R$_{vir}$）内的24个低表面亮度（LSB）和超弥散星系（UDGs）的积分场光谱数据。

Method: 采用积分视场光谱（integral field spectroscopy）和全光谱拟合技术（full spectral fitting technique）分析恒星年龄、金属丰度（metallicity）和镁铁比（[Mg/Fe]）。

Result: 在长蛇座I星系团（Hydra I cluster）中发现了不同类别的超弥散星系（UDGs），表明存在多种形成机制，其中几乎所有UDGs和低表面亮度星系（LSBs）都符合膨胀矮星系形成情景，具有类似矮星系的金属丰度并与矮星系质量-金属丰度关系一致，而位于星系团最内部区域的金属丰度较高的UDGs可能受到潮汐或环境效应的影响。
...
### [23] [The low-mass and structured stellar halo of M83 argues against a merger origin for its starburst and extended neutral hydrogen disk](https://arxiv.org/abs/2512.11744)
...
**TL;DR**: 通过观测M83的恒星晕（stellar halo）发现其具有延伸、低密度且贫金属（[M/H]∼−1.15）的古老红巨星分支（RGB）恒星成分，并探测到南北两条潮汐流（stream），结合数值模拟排除了近期并合（merger）对M83核区星暴（starburst）和延伸HI盘的成因贡献，指出其特殊性质更可能源自非并合机制（如长期演化或星际介质气体吸积）。


<details>
  <summary>Details</summary>

Background: 研究探讨M83星系（Messier 83）的恒星晕（stellar halo）结构及其形成机制，以验证其延展HI盘（extended HI disk）和核区星暴（nuclear starburst）是否源于并合事件（merger origin）。

Data: 使用斯巴鲁望远镜(Subaru)的宽视场成像数据和哈勃太空望远镜(Hubble Space Telescope)的观测数据进行邻近星-星系分离训练，以解析M83星系晕中的恒星分布。

Method: 采用最近邻星-星系分离方法（nearest-neighbor star-galaxy separation）结合宽视场斯巴鲁（Subaru）成像和哈勃空间望远镜（Hubble Space Telescope）数据，对M83星系晕的恒星进行解析。

Result: M83拥有一个由古老且贫金属（[M/H]$\sim -1.15$）的红巨星分支（RGB）恒星组成的低密度平滑恒星晕，其15至40千秒差距范围内的质量为$\log_{10}M_{*,15-40,maj}/M_{\odot}=8.02\pm0.10$。
...
### [24] [The Effect of a Non-universal Extinction Curve on the Wesenheit Function and Cepheid Distances](https://arxiv.org/abs/2512.11758)
*D. M. Skowron,M. L. Fouesneau,R. Drimmel,S. Khanna*

**TL;DR**: 研究发现韦森特函数（Wesenheit function）的R系数随总选择消光比（Rv）显著变化，银河系盘内经典造父变星（classical Cepheids）的典型Rv变化范围（2.6-3.6）可导致基于盖亚（Gaia）的韦森特指数WG出现±0.7星等的差异及近40%的距离误差，而近红外韦森特指数对Rv变化不敏感。


<details>
  <summary>Details</summary>

Background: 研究探讨了星际消光（interstellar extinction）中总选择消光比（Rv）的非普适性对韦森海特函数（Wesenheit function）的影响及其在距离测量中引入的系统偏差。

Data: 数据来源为结合Rv依赖的消光曲线与恒星大气模型网格（grid of stellar atmosphere models）计算光学和近红外波段的综合消光。

Method: 采用恒星大气模型网格与Rv依赖的消光曲线相结合的方法，计算光学和近红外波段积分消光并推导多组Wesenheit指数的Rv依赖系数。

Result: Wesenheit函数中的R系数随总选择消光比(Rv)变化显著，银河系盘内经典造父变星在典型Rv范围(2.6-3.6)内变化可导致Gaia波段W_G指数产生±0.7星等差，造成近40%距离误差。
...
### [25] [Geometry of non-Gaussianity in transient non-attractor inflation](https://arxiv.org/abs/2512.11020)
*Mohammad Hossein Namjoo,Bahar Nikbakht*

**TL;DR**: 研究了原初黑洞（PBHs）和标量诱导引力波（SIGWs）生成模型中双谱（bispectrum）的几何特性，发现相位跃迁和粒子产生主导的尺度上双谱在等边构型最强，而非吸引子相主导的尺度上则在压缩构型最强，并提出了简化双谱估计器，表明非线性效应会显著改变PBHs丰度、SIGWs振幅及其质量和频率谱。


<details>
  <summary>Details</summary>

Background: 研究探讨了原初黑洞（PBHs）和标量诱导引力波（SIGWs）形成过程中非线性效应（特别是双谱）的作用及其几何特征。

Data: 研究中使用的数据来源为理论模拟（simulation）数据，主要基于暴胀模型（inflationary models）中相位跃迁（phase transitions）、粒子产生（particle production）或慢滚演化偏离（departures from slow-roll evolution）驱动的放大涨落。

Method: 采用双谱分析（bispectrum analysis）作为核心方法，重点研究其尺度与形状依赖性（geometry），并引入尺度相关形状关联器（scale-dependent shape correlator）以区分两种依赖性。

Result: 在相变和粒子产生（包括功率谱峰值）影响最显著的尺度上，双谱（bispectrum）在等边构型附近最强，而非吸引子相位倾向于在挤压构型附近产生相关性。
...
### [26] [KiDS-Legacy: Constraints on Horndeski gravity from weak lensing combined with galaxy clustering and cosmic microwave background anisotropies](https://arxiv.org/abs/2512.11039)
...
**TL;DR**: 结合KiDS-Legacy宇宙剪切（cosmic shear）数据与多种观测，首次在满足稳定性条件的有效场论框架下约束了Horndeski修正引力模型参数，发现标量场扰动动力学项（$D_{\rm kin}$）和普朗克质量偏差（$ΔM_*^2$）分别以1.9σ和1.5σ偏离广义相对论，结构增长参数$S_8$与ΛCDM模型一致，有效牛顿耦合常数偏差（$Δμ_{\infty,{\rm eff}}$）达2.9σ显著性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用多源天文观测数据（包括Kilo-Degree Survey宇宙剪切、DESI重子声学振荡、eBOSS红移空间畸变及Planck宇宙微波背景各向异性）在有效场论框架下检验霍恩德斯基（Horndeski）修正引力理论，并首次在固有稳定参数基中开展宇宙学分析。

Data: 数据来源包括Kilo-Degree Survey（KiDS-Legacy）最终数据发布的宇宙剪切（cosmic shear）分析、DESI的重子声学振荡（baryon acoustic oscillations）测量、eBOSS的红移空间畸变（redshift space distortions）观测以及Planck的宇宙微波背景（cosmic microwave background）各向异性数据。

Method: 采用有效场论框架下的霍恩德斯基修正引力模型（Horndeski modified gravity models），通过构造满足稳定性条件的参数化方法，首次在固有稳定参数基上进行宇宙学分析。

Result: 首次在固有稳定参数基中开展宇宙学分析，通过宇宙剪切（cosmic shear）显著约束了霍恩德斯基（Horndeski）修正引力模型参数空间，其约束力与宇宙微波背景（CMB）相当或更强。
...
### [27] [KiDS-Legacy: Constraining dark energy, neutrino mass, and curvature](https://arxiv.org/abs/2512.11041)
...
**TL;DR**: 结合千度巡天(KiDS-Legacy)宇宙剪切数据与宇宙微波背景(CMB)等外部探测，验证了平坦ΛCDM模型的适用性，测得中微子质量总和上限1.5eV、暗能量状态方程参数w0=-1.0±0.7和wa=-1.3^{+1.9}_{-2.0}，空间曲率ΩK=0.08^{+0.16}_{-0.17}，并发现w0waCDM模型相较ΛCDM改进不显著（贝叶斯因子B=0.07），但多数据集联合分析时S8参数在ΛCDM下为0.816±0.006、w0waCDM下为0.837±0.008。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用宇宙剪切（cosmic shear）和外部观测数据约束最小扩展宇宙学模型，包括空间曲率、中微子质量变化和动态暗能量（dark energy）演化对宇宙学参数的影响。

Data: 数据来源包括Kilo-Degree Survey (KiDS-Legacy)的最终数据发布、宇宙微波背景(CMB)透镜效应、星系红移空间畸变以及重子声学振荡(BAO)。

Method: 采用宇宙剪切（cosmic shear）分析结合宇宙微波背景（CMB）透镜、星系红移空间畸变（redshift-space distortions）和重子声学振荡（baryon acoustic oscillations）等多重外部探测手段。

Result: KiDS-Legacy（Kilo-Degree Survey）宇宙剪切分析结合外部探测数据支持平坦的ΛCDM模型，中微子质量总和上限为1.5 eV，暗能量状态方程参数w0 = -1.0±0.7且wa = -1.3^{+1.9}_{-2.0}，空间曲率Ω_K = 0.08^{+0.16}_{-0.17}（1σ置信区间）。
...
### [28] [Insights for Early Dark Energy with Big Bang Nucleosynthesis](https://arxiv.org/abs/2512.11163)
*Christopher Cook,Joel Meyers*

**TL;DR**: 利用现代数据和主成分分析方法，建立了对标准宇宙膨胀历史在太初核合成（BBN）时期偏离的最普适约束，并探讨了膨胀率修改对锂问题的潜在影响。


<details>
  <summary>Details</summary>

Background: 大爆炸核合成(BBN)作为宇宙早期可直接观测的关键过程，为研究宇宙膨胀历史提供了独立而强大的探测手段。

Data: 数据来源包括高效灵活的BBN代码、原初氘和氦-4丰度的百分比级测量、核反应速率的精确测量以及宇宙微波背景对重子密度的精确测定。

Method: 采用主成分分析（principal component analysis）方法识别膨胀历史变化的最受约束且物理意义明确的模式。

Result: 利用现代数据和统计技术，通过主成分分析（principal component analysis）识别出膨胀历史变化的最受约束且物理意义明确的模式，为BBN时期的早期暗能量（early dark energy）提供了最普遍的约束。
...
### [29] [Cosmological Implication of Cross-correlation between Galaxy Clustering and 21-cm Line Intensity Mapping](https://arxiv.org/abs/2512.11353)
*Yong-Seon Song,Minji Oh,Kyungjin Ahn,Feng Shi*

**TL;DR**: 通过星系聚类与21厘米线强度映射的互相关分析，显著提升了对结构增长（growth of structure）和中性氢含量（Omega_HI）的测量精度，其中结构增长约束提高两倍，Omega_HI精度达亚百分之一水平，并突破线性功率谱分析中Omega_HI与HI偏差（b_HI）的简并性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用星系聚集和21厘米线强度映射（21-cm line intensity mapping）在红移空间中的各向异性特征，通过阿尔科克-帕琴斯基效应（Alcock-Paczynski effect）和红移空间畸变（redshift-space distortions）联合探测宇宙膨胀和大尺度引力效应。

Data: 数据来源包括DESI光谱巡天（Dark Energy Spectroscopic Instrument）和21厘米线强度映射巡天（21-cm line-intensity mapping surveys）的观测数据。

Method: 采用星系成团性（galaxy clustering）与21厘米线强度映射（21-cm line intensity mapping）的统计互相关（cross-correlation）方法，结合非线性红移空间畸变（RSD）的费希尔矩阵（Fisher matrix）分析。

Result: 通过星系聚类与21厘米线强度映射的互相关分析，将结构增长（growth of structure）的测量精度提升至自相关方法的两倍，并将中性氢含量（Omega_HI）约束至亚百分之一水平。
...
### [30] [Observational evidence of tidal torque in the primordial Universe](https://arxiv.org/abs/2512.11383)
...
**TL;DR**: 研究证实了潮汐扭矩理论（TTT）对中心大质量椭圆星系气体角动量的预测，首次通过7σ显著性的方向相关性观测验证了原初密度场对星系角动量的印记，为宇宙学参数（如中微子质量）测量提供了新方法。


<details>
  <summary>Details</summary>

Background: 研究背景为验证潮汐力矩理论(TTT)关于星系角动量源于原初密度场并可用于精确测量早期宇宙的预测。

Data: 数据来源为ELUCID重建的邻近宇宙原初密度场（primordial density field）和中心大质量椭圆星系气体角动量的观测数据。

Method: 采用潮汐力矩理论(TTT)结合ELUCID重构的原始密度场，通过比较观测气体角动量矢量与理论预测值进行相关性分析。

Result: 首次通过观测证实了潮汐力矩理论（TTT）对星系角动量的预测，检测到气体角动量矢量与重构原始密度场之间存在显著方向相关性（约7σ）。
...
### [31] [The magnitude of the dark ages 21-cm signal in the context of existing early and late time constraints on $Λ$CDM](https://arxiv.org/abs/2512.11568)
*H. T. J. Bevins*

**TL;DR**: 研究通过结合早期（普朗克和WMAP）与晚期（DES星系透镜和重子声学振荡）宇宙学探测数据，在ΛCDM框架内将黑暗时代21厘米信号（dark ages 21-cm signal）的深度约束至优于1毫开尔文、中心频率精度达0.05兆赫兹。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用黑暗时代(dark ages)21厘米信号探测宇宙早期演化，以揭示暗物质(dark matter)、暗能量(dark energy)本质及检验标准宇宙学模型(ΛCDM model)的潜在扩展。

Data: 数据来源包括早期宇宙探测数据（Planck和WMAP）以及晚期宇宙探测数据（DES星系透镜和聚类及重子声波振荡（Baryon Acoustic Oscillations））。

Method: 采用普朗克(Planck)、WMAP、DES星系透镜与聚类(DES galaxy lensing and clustering)及重子声学振荡(Baryon Acoustic Oscillations)的观测数据，通过ΛCDM宇宙学模型参数不确定性系统传播方法约束黑暗时代21厘米信号(dark ages 21-cm signal)的幅度与中心频率。

Result: 暗纪元21厘米信号（dark ages 21-cm signal）的深度已知优于1毫开尔文（mK），中心频率已知在0.05兆赫兹（MHz）以内。
...
### [32] [Dark sector interactions in the $w \rightarrow -1$ limit: velocity locking in pure momentum exchange models](https://arxiv.org/abs/2512.11639)
*Nathan Cruickshank,Robert Crittenden,Kazuya Koyama,Marco Bruni*

**TL;DR**: 该研究揭示了暗能量（DE）与暗物质（DM）纯动量交换模型中存在一个依赖于状态方程参数$w$的速度锁定机制，导致物质功率谱抑制起始尺度随$w$趋近-1而系统性地向小尺度偏移，并指出忽略DE扰动的常见近似方法在此极限下失效。


<details>
  <summary>Details</summary>

Background: 研究聚焦于纯动量交换的暗能量（DE）与暗物质（DM）相互作用模型在DE状态方程参数$w$趋近-1时的理论行为及其对物质功率谱抑制的影响机制。

Data: 数据来源于理论模型模拟（simulation）的暗能量（DE）和暗物质（DM）相互作用动力学演化结果，重点关注纯动量交换情形下物质功率谱（matter power spectrum）的抑制特征及其尺度依赖性。

Method: 采用纯动量交换的暗能量（DE）和暗物质（DM）相互作用模型，通过分析速度锁定机制（velocity-locking mechanism）及其对物质功率谱抑制尺度的影响，研究了DE状态方程$w$趋近于$-1$时的理论极限行为。

Result: 在暗能量（DE）状态方程参数$w$趋近于-1的极限情况下，模型表现出速度锁定机制（velocity-locking mechanism），导致物质功率谱抑制的起始尺度随$w$减小而向更小尺度移动。
...
### [33] [One-loop power spectrum corrections in interacting dark energy cosmologies](https://arxiv.org/abs/2512.11678)
*Emanuelly Silva,Gabriel Hartmann,Rafael C. Nunes*

**TL;DR**: 研究了相互作用暗能量（IDE）模型对物质功率谱（matter power spectrum）的一环修正，通过EFTofLSS框架分析非线性尺度特征，并基于BOSS DR12数据约束新型耦合项Q=ΓρmρDEθm的相互作用率Γ（结果为0.0039±0.0082），表明与ΛCDM模型高度一致。


<details>
  <summary>Details</summary>

Background: 研究主题为探索暗物质和暗能量（Dark Energy）之间能量动量交换的相互作用暗能量（Interacting Dark Energy）模型，作为标准ΛCDM范式的动力学扩展。

Data: 研究使用了BOSS DR12数据中全形状（Full-Shape, FS）测量的星系功率谱（galaxy power spectrum）来约束相互作用暗能量（Interacting Dark Energy, IDE）模型的耦合率Γ、宇宙学参数和偏差参数。

Method: 采用有效场论大尺度结构（EFTofLSS）框架计算暗能量相互作用（IDE）模型中物质功率谱的单圈修正，并引入非线性耦合项Q = ΓρmρDEθm以隔离IDE模型在弱非线性尺度上的效应。

Result: 暗能量相互作用（IDE）模型在弱非线性尺度上通过引入与暗物质速度散度相关的非线性耦合常数Γ（Γ=0.0039±0.0082）的新相互作用项Q=ΓρmρDEθm，其约束结果与ΛCDM模型高度一致。
...
### [34] [The Domain Adaptation problem in photometric redshift estimation: a solution applied to the HSC Survey](https://arxiv.org/abs/2512.11700)
*M. Treyer,R. Ait-Ouahmed,S. Arnouts,J. Pasquet,E. Bertin,G. Desprez,V. Picouet,M. Sawicki*

**TL;DR**: 利用对抗性域适应网络改进卷积神经网络(CNN)，将在COSMOS超深场(UDF)训练的光度红移预测模型成功迁移至HSC-CLAUDS其他观测区域，显著提升了i<25星系的预测精度并公开了约400万源的红移星表。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用卷积神经网络(CNN)结合无监督对抗域适应网络提升多波段HSC-CLAUDS巡天中非COSMOS超深场(UDF)区域的测光红移(photometric redshift)估算精度。

Data: 数据来源于HSC-CLAUDS巡天（Hyper Suprime-Cam Subaru Strategic Program-CFHT Large Area U-band Deep Survey）的多波段观测，其中COSMOS Ultra Deep Field（UDF）区域提供了完备的红移覆盖，其他区域（XMM、DEEP2、ELAIS及剩余COSMOS区域）的观测条件各异。

Method: 采用卷积神经网络(CNN)结合无监督对抗域适应网络(unsupervised adversarial domain adaptation network)进行光度红移(photometric redshift)预测。

Result: 利用无监督对抗域适应网络(unsupervised adversarial domain adaptation network)改进的卷积神经网络(CNN)成功将COSMOS UDF训练的光度红移(photometric redshifts)预测能力迁移至HSC-CLAUDS其他天区，精度远超现有SED拟合方法。
...
### [35] [Solving the Cosmological Vlasov-Poisson Equations with Physics-Informed Kolmogorov-Arnold Networks](https://arxiv.org/abs/2512.11795)
*Nicolas Cerardi,Emma Tolley,Ashutosh Mishra*

**TL;DR**: 提出了一种基于物理信息神经网络（KAN）的无N体方法，通过连续位移场精确模拟一维晕坍缩中的冷暗物质（CDM）演化，误差低于1%且不随时间增长，同时支持逆向重建初始条件。


<details>
  <summary>Details</summary>

Background: 研究聚焦于冷暗物质（CDM）的连续场演化问题，旨在通过物理信息神经网络替代传统N体模拟的粒子离散化方法。

Data: 数据来源于基于Vlasov-Poisson方程的物理约束条件，通过物理信息神经网络（KAN）直接建模一维晕坍缩的连续位移场，不使用任何N体模拟数据或方法。

Method: 采用基于Kolmogorov-Arnold网络（KAN）的物理信息神经网络方法，通过将Vlasov-Poisson方程导出的物理约束直接嵌入损失函数来演化冷暗物质（CDM）连续位移场。

Result: 提出了一种基于物理信息神经网络（KAN）的无N体方法，可高精度模拟冷暗物质（CDM）一维晕坍缩的连续位移场演化，误差低于1%且不随时间累积。
...
### [36] [Stellar physics at sub-nanoradian angular resolution](https://arxiv.org/abs/2512.11005)
...
**TL;DR**: 论文核心贡献是探讨利用20公里级合成孔径（synthetic aperture）干涉仪观测白矮星（white dwarf）表面细节的潜力，包括亮度变化、周期性特征和瞬变现象。


<details>
  <summary>Details</summary>

Background: 研究主题为高角分辨率天文成像技术对恒星表面特征（如亮度变化、周期性现象和瞬变事件）的观测潜力及其科学意义。

Data: 研究中使用的数据来源于当前光学干涉仪（optical interferometers）提供的合成孔径观测数据，以及未来计划扩展至公里级合成孔径的观测项目。

Method: 采用光学干涉仪（optical interferometer）技术构建合成孔径（synthetic aperture），实现纳米弧度级角分辨率以解析恒星表面特征。

Result: 高角分辨率观测可揭示恒星表面亮度变化（limb darkening）、周期性特征（如振荡、对流胞、星斑）及瞬变现象（如耀斑）等理论预测但光谱无法提取的特征。
...
### [37] [libyt: an In Situ Interface Connecting Simulations with yt, Python, and Jupyter Workflows](https://arxiv.org/abs/2512.11142)
*Shin-Rong Tsai,Hsi-Yu Schive,Matthew J. Turk*

**TL;DR**: 开发了开源C库libyt，实现了天体物理模拟（astrophysical simulations）与Python工具（如yt）的并行计算集成，支持原位分析（in situ analysis）和交互式数据处理，显著降低了高性能计算环境下的I/O开销。


<details>
  <summary>Details</summary>

Background: 研究主题为面向天体物理模拟的高性能原位(in situ)数据分析与可视化方法开发。

Data: data部分：数据来源于自适应网格细化（AMR）模拟代码GAMER和Enzo，包括核心坍缩超新星、孤立矮星系、模糊暗物质（fuzzy dark matter）、Sod激波管测试、Kelvin-Helmholtz不稳定性以及AGORA星系模拟等案例。

Method: 开发了libyt开源C库，支持在并行计算中通过yt或其他Python包进行原位分析（in situ analysis），提供与Python的双向连接及Jupyter集成，并实现高效内存数据处理和AMR（自适应网格细化）模拟读取。

Result: libyt是一个开源C库，可在高性能计算环境中实现天体物理模拟数据与Python（如yt）的并行分析和可视化，支持自动调用Python例程或通过Jupyter交互操作，且对用户工作流干预极小。
...
### [38] [Asteroid Occultations with VERITAS: Observations and Considerations](https://arxiv.org/abs/2512.11144)
*Joshua Bartkoske,Anne Duerr,Dave Kieda*

**TL;DR**: 利用VERITAS望远镜成功观测20次小行星掩星（asteroid occultations）并测量9至13星等恒星的最小角直径（angular diameter），为研究太阳系形成模型提供新方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用掩星现象（occultations）探测小行星（asteroids）和海外天体（trans-Neptunian objects）以探索太阳系形成模型。

Data: 数据来源于VERITAS望远镜自2018年起对超过100次预测小行星掩星（asteroid occultations）的观测尝试，其中成功观测到20次掩星事件，并计划在FADC升级完成后开展对偶然掩星（serendipitous occultations）的搜索。

Method: 采用衍射图样拟合技术分析掩食事件中VERITAS中心像素的观测数据，以测量恒星角直径。

Result: VERITAS通过观测小行星掩星（asteroid occultations）成功测量了9至13星等恒星的最小角直径（angular diameter），精度超过现有光学仪器或技术。
...
### [39] [Classifying High-Energy Celestial Objects with Machine Learning Methods](https://arxiv.org/abs/2512.11162)
*Alexis Mathis,Daniel Yu,Nolan Faught,Tyrian Hobbs.*

**TL;DR**: 该论文通过比较树模型（tree-based models）和循环神经网络（RNN）在区分脉冲星（pulsars）和黑洞（black holes）光变信号（photometric data）中的性能，发现RNN在实时天体分类任务中具有潜力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于机器学习（machine learning）在天文学领域分类天体（celestial objects）中的应用潜力，特别是针对光变（photometric）信号相似的天体（如脉冲星pulsars和黑洞black holes）的分类问题。

Data: 研究中使用的数据来源为原始信号数据（raw signal data）经过下采样和归一化处理后的版本。

Method: 采用基于树（tree-based）的模型和循环神经网络（RNN）对天体光变（photometric）数据进行分类和实时判别分析。

Result: 基于光变（photometric）数据，树模型和循环神经网络（RNN）能有效区分脉冲星（pulsars）和黑洞（black holes）等光谱相似的天体。
...
### [40] [amc: The Automated Mission Classifier for Telescope Bibliographies](https://arxiv.org/abs/2512.11202)
*John F. Wu,Joshua E. G. Peek,Sophie J. Miller,Jenny Novacescu,Achu J. Usha,Christopher A. Wilkinson*

**TL;DR**: 开发了基于大语言模型（LLMs）的自动化任务分类器（Automated Mission Classifier, amc），能高效识别和分类天文文献中的望远镜引用，并在TRACS Kaggle挑战中取得0.84的宏观F1分数，为图书馆科学提供了可扩展的解决方案。


<details>
  <summary>Details</summary>

Background: 天文文献计量学（bibliometrics）通过追踪望远镜设施的出版物统计和引用指标来监测天文学研究的动态发展。

Data: data：研究使用了自动化任务分类器（Automated Mission Classifier, amc）处理的大量论文文本数据，并基于TRACS Kaggle挑战赛的测试集（held-out test set）和历史数据集进行验证。

Method: 采用大型语言模型（LLMs）处理大量论文文本以识别和分类望远镜引用，并开发了自动化任务分类器（Automated Mission Classifier, amc）。

Result: 开发了基于大语言模型(LLMs)的自动任务分类器(amc)，能有效识别和分类望远镜文献，在TRACS Kaggle挑战中取得0.84的宏观F1分数。
...
### [41] [The Simons Observatory: Development of a Pipeline to Detect Rapid Transients in Time-Ordered Data](https://arxiv.org/abs/2512.11313)
...
**TL;DR**: 提出了一种利用宇宙微波背景（CMB）巡天望远镜直接处理原始时序数据以探测毫秒至分钟级天体物理瞬变（astrophysical transients）的方法，在模拟观测中实现了对0.5秒级耀斑90%探测率（93GHz波段800mJy）且定位精度接近望远镜分辨率。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用宇宙微波背景(CMB)巡天望远镜探测毫秒至分钟时间尺度演化的天体物理暂现源(astrophysical transients)。

Data: data：研究使用了模拟观测数据，其中注入了类似恒星耀斑的光变曲线（light curves），并在93、145、225和280 GHz波段进行了测试。

Method: 采用直接处理原始时间序列数据（time-ordered data）的检测方法，整合至西蒙斯天文台（Simons Observatory）时域分析流程，并通过注入类恒星耀发光变曲线的模拟观测评估性能。

Result: 该方法通过直接处理原始时序数据，在93、145、225和280 GHz波段分别实现了对0.5秒时标耀发事件800、1150、1650和4250 mJy通量密度下≥90%的探测率。
...
### [42] [UK White Paper on Magnetic Reconnection](https://arxiv.org/abs/2512.11631)
...
**TL;DR**: 该论文明确了磁重联（magnetic reconnection）在太阳物理和空间科学中的核心作用，提出了未来十年磁重联研究的九个优先科学目标、理论模拟与基础设施投资建议以及确保英国保持国际领先地位的任务优先事项和测量需求。


<details>
  <summary>Details</summary>

Background: 磁重联（magnetic reconnection）是驱动等离子体宇宙中磁能爆发性释放、加热和粒子加速的普适过程，对理解日球层（heliosphere）及太阳活动与行星磁层相互作用至关重要。

Data: 数据来源包括地球磁层和太阳风的原位(in situ)测量、太阳日冕和色球层的观测数据以及世界一流的数值模拟(numerical simulation)结果。

Method: 采用磁重联（magnetic reconnection）理论模型结合实地测量、日冕和色球层观测以及数值模拟进行多尺度分析。

Result: 磁重联（magnetic reconnection）研究在未来十年的九个优先科学目标包括理解其在日球层关键现象中的作用及支撑未来空间任务的多重科学目标。
...
### [43] [Transformer Embeddings for Fast Microlensing Inference](https://arxiv.org/abs/2512.11687)
*Nolan Smyth,Laurence Perreault-Levasseur,Yashar Hezaveh*

**TL;DR**: 该论文提出了一种基于模拟推理的流水线方法，利用Transformer编码器学习原始时间序列数据的压缩摘要表示，并结合神经后验估计器，以比传统方法快三个数量级的速度准确校准后验分布，成功应用于短时自由漂浮行星（FFPs）候选事件KMT-BLG-2019-2073的分析。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用微引力透镜效应（microlensing）搜寻自由漂浮行星（free-floating planets, FFPs），这是南希·格雷斯·罗曼银河系系外行星巡天等未来观测计划的核心科学目标。

Data: 研究中使用的数据来源包括模拟生成的时间序列数据以及KMTNet（Korea Microlensing Telescope Network）观测到的KMT-BLG-2019-2073事件（短周期自由漂浮行星候选体）的实际观测数据。

Method: 采用基于模拟推理（simulation-based inference）的流程，结合Transformer编码器学习时间序列数据的压缩摘要表示，并利用神经后验估计器（neural posterior estimator）生成后验分布。

Result: 开发了一种基于Transformer编码器和神经后验估计器的模拟推理流程，能够快速准确地分析自由漂浮行星（FFPs）微透镜事件中的噪声和不规则采样数据。
...
### [44] [Temporal Substepping Scheme for Magnetohydrodynamics with Cell-based Adaptive Mesh Refinement on Staggered Grid](https://arxiv.org/abs/2512.11365)
*Ilja Honkonen,Riku Jarvinen,David Phillips*

**TL;DR**: 提出了一种基于约束输运方法、支持单元自适应网格细化（cell-based adaptive mesh refinement）和时间子步进（temporal substepping）的数值磁流体力学新算法，直接在逻辑笛卡尔网格上处理分辨率变化，无需插值或投影即可保持磁场散度为零（$\nabla \cdot B = 0$）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发一种保持磁场无散度（$\nabla \cdot B = 0）的数值磁流体力学（magnetohydrodynamics）算法，用于交错网格（staggered meshes）上的高精度模拟。

Data: data：基于逻辑笛卡尔网格（logically Cartesian grid）的单元自适应网格加密（cell-based adaptive mesh refinement）和时间子步进（temporal substepping）数据。

Method: 采用基于约束输运方法（constrained transport method）的算法，支持基于单元的自适应网格加密（cell-based adaptive mesh refinement）和时间子步进（temporal substepping），直接在逻辑笛卡尔网格上处理分辨率变化。

Result: 提出了一种基于约束输运方法（constrained transport method）的新算法，可在交错网格上保持磁场散度为零（∇·B=0），并支持基于单元的自适应网格细化（cell-based adaptive mesh refinement）和时间子步进（temporal substepping）。
...
### [45] [Seeking Spectroscopic Binaries with Data-Driven Models](https://arxiv.org/abs/2512.11043)
*Isabel Angelo,Erik Petigura,Megan Bedell*

**TL;DR**: 该研究利用The Cannon算法在Keck高分辨率阶梯光谱仪数据上训练数据驱动模型，发现结合新型小波预处理后能有效预测恒星参数（Teff、半径、[Fe/H]、vsin(i)和仪器点扩散函数），但约3%的像素级通量预测精度限制了其对未分辨双星系统的探测能力。


<details>
  <summary>Details</summary>

Background: 该研究的科学主题是数据驱动的恒星分类（data-driven stellar classification）及其在未分辨双星（unresolved stellar binaries）检测中的应用。

Data: 数据来源于凯克高分辨率阶梯光谱仪（Keck High Resolution Echelle Spectrometer）的光谱数据。

Method: 采用数据驱动方法（The Cannon）结合小波预处理技术对凯克高分辨率阶梯光谱仪（Keck HIRES）的光谱进行建模，重点提升恒星参数预测精度并探索未分辨双星系统的检测能力。

Result: 基于数据驱动的光谱模型在预测恒星有效温度（Teff）、半径、金属丰度（[Fe/H]）、自转速度（vsin(i)）和仪器点扩散函数（PSF）方面与现有模型性能相当，但受限于约3%的像素级通量预测精度，其未分辨双星探测能力存在明显局限。
...
### [46] [Type II and Type III Solar Radio Burst Classification Using Transfer Learning](https://arxiv.org/abs/2512.11487)
...
**TL;DR**: 基于e-Callisto网络数据构建了包含空白频谱图、II型（Type II）和III型（Type III）太阳射电暴（SRBs）的数据集，并通过微调预训练深度学习模型实现自动分类，其中YOLOv8以92%的F1分数表现最优。


<details>
  <summary>Details</summary>

Background: 太阳射电暴（SRBs）的自动分类对空间天气监测和地球技术基础设施保护具有重要科学意义。

Data: 数据来源于复合天文低频低成本光谱与可移动观测站网络（e-Callisto）记录的射电频谱图像，包含空白频谱图、II型太阳射电暴（Type II SRBs）频谱图和III型太阳射电暴（Type III SRBs）频谱图三类。

Method: 采用预训练的深度学习模型（包括VGGnet-19、MobileNet、ResNet-152、DenseNet-201和YOLOv8）对太阳射电暴（SRBs）光谱图进行微调分类。

Result: 基于e-Callisto网络数据构建的太阳射电暴（SRB）图像数据集，通过微调预训练深度学习模型实现了对II型和III型SRB的自动分类，其中YOLOv8模型表现最佳（F1分数87%-92%）。
...
### [47] [The Crab Nebula Revisited Using HST/WFC3](https://arxiv.org/abs/2512.11103)
*William P. Blair,Ravi Sankrit,Dan Milisavljevic,Tea Temim,J. Martin Laming,Patrick Slane,Ziwei Ding,Thomas Martin*

**TL;DR**: 通过哈勃太空望远镜（HST）的WFC3相机重新观测蟹状星云（Crab Nebula），对比1999-2000年WFPC2数据及詹姆斯·韦伯太空望远镜（JWST）近红外/中红外图像，发现了两组此前未被识别的、与脉冲星几乎对称但起源未知的具有相似发射特征的丝状结构。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用哈勃太空望远镜（HST）高分辨率观测数据，分析蟹状星云（Crab Nebula）24年来的动态演化特征及其多波段辐射性质。

Data: 数据来源于哈勃太空望远镜（HST）第31周期观测项目中的WFC3相机（使用F487N、F547M和F763M滤光片）以及1999-2000年WFPC2相机拍摄的旧拼接图像，并对比了詹姆斯·韦伯太空望远镜（JWST）的近红外/中红外（NIR/MIR）图像。

Method: 采用哈勃太空望远镜（HST）的第三代广域相机（WFC3）和与1999-2000年WFPC2相机相似的滤光片进行观测，包括F487N滤光片获取纯净氢波段数据以及F547M和F763M滤光片研究光学同步辐射（synchrotron）成分，并与WFPC2早期数据及JWST近红外/中红外（NIR/MIR）图像对比。

Result: 通过对比哈勃太空望远镜（HST）新旧观测数据，发现蟹状星云（Crab Nebula）中存在两组此前未被识别的、具有相似发射特征且与脉冲星几乎呈对角分布的丝状结构群，但其起源尚不明确。
...
### [48] [An Open Benchmark of One Million High-Fidelity Cislunar Trajectories](https://arxiv.org/abs/2512.11064)
*Travis Yeager,Denvir Higgins,Peter Mcgill,Kerianne Pruett,Alexx Perloff,Tara Grice,Michael Schneider*

**TL;DR**: 构建并开源了包含一百万条高精度数值传播的地月空间（cislunar space）轨道数据集，为比较研究和数据驱动方法提供了标准化基准。


<details>
  <summary>Details</summary>

Background: 研究聚焦于地月空间（cislunar space）的轨道动力学特性及其对未来探索、科学和安全任务的基础支撑作用。

Data: 数据来源于使用开源空间态势感知Python包（SSAPy）数值传播生成的100万条地月空间轨迹开放数据集，模型包含高精度地球/月球引力、太阳引力和地球/太阳辐射压力，初始条件均匀采样常用瞬时轨道根数范围，每条轨迹在固定起始历元下传播长达六年。

Method: 采用开源空间态势感知Python包（SSAPy）进行数值传播，模型包含高精度地球/月球引力、太阳引力及地球/太阳辐射压力，初始条件均匀采样常用瞬时轨道根数范围，每条轨迹在固定初始历元下传播最长六年。

Result: 开发并公开了一个包含100万条数值传播的地月空间（cislunar）轨迹的开源数据集，采用高精度动力学模型（含地球/月球高阶引力、太阳引力和地球/太阳辐射压），数据格式为CSV/HDF5并附带完整状态时间序列和元数据，旨在为地月空间领域的方法开发、统计研究和社区扩展提供可复用的基准资源。
...
### [49] [CPI-C: Cool Planet Imaging Coronagraph on Chinese Space Station Survey Telescope](https://arxiv.org/abs/2512.11292)
...
**TL;DR**: 提出中国空间站巡天望远镜(CSST)上的冷行星成像日冕仪(CPI-C)，首次在可见光波段(600-900 nm)通过直接成像探测40秒差距内类太阳恒星周围0.5-5 AU距离范围内的海王星到木星大小的系外行星，并利用阶梯透射变迹技术和精密相位校正技术实现优于10^-8的对比度(3-4λ/D内工作角)，为研究行星形成演化机制奠定基础。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用中国空间站巡天望远镜(CSST)上的冷行星成像日冕仪(CPI-C)对40秒差距内的类太阳恒星周围冷行星进行直接成像和光谱分析，以揭示行星形成与演化机制并为寻找类地行星奠定基础。

Data: 数据来源为CSST（Chinese Space Station Survey Telescope）搭载的CPI-C（Cool Planet Imaging Coronagraph）在可见光波段（600-900 nm）对距离太阳系40 pc内的类太阳恒星周围0.5-5 AU分离的类海王星至类木星系外行星进行高对比度直接成像观测。

Method: 采用阶梯透射变迹技术（step-transmission apodization）抑制望远镜光瞳衍射噪声，结合精密相位校正技术消除光学表面缺陷导致的散斑噪声（speckle noises）。

Result: CPI-C（Cool Planet Imaging Coronagraph）将首次在可见光波段（600-900 nm）直接成像冷系外行星的反射光，并测量其有效温度、表面重力、半径和质量等关键物理参数。
...
### [50] [Dynamical evolution of massless particles in star clusters with NBODY6++GPU-MASSLESS: II. The long-term evolution of free-floating comets](https://arxiv.org/abs/2512.11501)
*Francesco Flammini Dotti,M. B. N. Kouwenhoven,Kai Wu,Abbas Askar,Peter Berczik,Mirek Giersz,Rainer Spurzem,Ian Dobbs-Dixon*

**TL;DR**: 研究发现自由漂浮彗星（ffcs）在星团中不发生质量分层（mass segregation），其初始能量分布仅轻微影响早期动力学演化，且外部区域彗星最终会被星团核心引力拉回。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星团环境中自由漂浮彗星（ffcs）等低质量天体的动力学演化，探讨其初始能量分布对质量分层（mass segregation）的影响及运动学特征。

Data: 使用N-body代码NBODY6++GPU-MASSLESS（Flammini Dotti等人2025年）进行模拟，该代码可快速集成包含大量无质量粒子的星团，以表征具有亚维里（sub-virial）和超维里（super-virial）分布的低质量粒子群体的动力学特性。

Method: 采用N体代码NBODY6++GPU-MASSLESS（Flammini Dotti等人2025年）进行模拟，快速集成包含大量无质量粒子的星团，以表征具有亚维里（sub-virial）和超维里（super-virial）分布的低质量粒子群体的动力学特性。

Result: 彗星（ffcs）不参与质量分层（mass segregation）过程，与行星尺度天体类似，且不受初始能量分布影响。
...
### [51] [Vibration Isolation for the Laser Interferometer Lunar Antenna](https://arxiv.org/abs/2512.11268)
*Brett N. Shapiro*

**TL;DR**: 提出激光干涉仪月球天线（LILA）概念，利用月球极低地震噪声环境填补亚赫兹频段引力波探测空白，并针对不同地震背景假设为LILA Pioneer和LILA Horizon两种设计建立悬架系统基线方案。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用月球低地震噪声环境填补亚赫兹频段引力波探测空白，连接空间探测器（毫赫兹）与地面探测器（10赫兹至数千赫兹）的观测频段。

Data: 数据来源于月球的地震环境（seismic environment）和激光干涉仪月球天线（LILA）的两种设计方案（LILA Pioneer和LILA Horizon）。

Method: 采用基于月球共振模式（resonant modes）的非悬挂光学系统（non-suspended optics）和后续设计的悬挂系统（suspensions）实现带内自由浮动测试质量（in-band free floating test masses）并过滤残余地震背景噪声（residual seismic background）。

Result: LILA（Laser Interferometer Lunar Antenna）提出了一种利用月球低地震环境观测亚赫兹频段引力波的新方法，填补了空间探测器（毫赫兹）和地面探测器（10赫兹至几千赫兹）之间的空白。
...
### [52] [Gravitational Foundations and Exact Solutions in $n$--Dimensional Fractional Cosmology](https://arxiv.org/abs/2512.11583)
*S. M. M. Rasouli,J. Marto,D. Oliveira,P. Moniz*

**TL;DR**: 这篇论文的核心贡献是：基于时间依赖核加权作用量（time-dependent kernel weighted action）和广义Sáez-Ballester（SB）理论，在n维FLRW度规下建立了分数阶标量场宇宙学模型，分析了场方程动力学行为、守恒定律、Noether定理应用等基本引力结构特征，研究了关键宇宙学量的精确解析解及其演化规律，并与标准模型和观测数据对比，最后提出了构建有效势和探索反弹解条件的模型推广思路。


<details>
  <summary>Details</summary>

Background: 研究主题为在n维FLRW度规下基于分数阶标量场（fractional scalar field）和广义Sáez-Ballester理论构建宇宙学模型，并分析其引力结构特征、动力学行为及与标准模型的差异。

Data: 研究中使用的数据来源为理论推导（theoretical derivation）和数值模拟（numerical simulation）。

Method: 采用时间依赖核加权作用量(time-dependent kernel weighted action)方法，结合FLRW度规和广义Sáez-Ballester(SB)理论，在n维时空中建立分数阶宇宙学模型，重点分析场方程动力学行为、Bianchi恒等式、守恒定律及二阶Noether定理在背景和一阶扰动层面的应用。

Result: 基于分数阶标量场（fractional scalar field）构建了n维FLRW度规下的宇宙学模型，扩展了Sáez-Ballester理论，分析了场方程动力学行为、比安基恒等式满足性及守恒定律，并应用了二阶诺特定理。
...
### [53] [Constraints on the intergalactic magnetic field from Fermi-LAT observations of GRB 221009A](https://arxiv.org/abs/2512.11128)
...
**TL;DR**: 通过费米大面积望远镜观测伽马暴（GRB）221009A未发现级联信号，首次排除了相干长度大于1百万秒差距（Mpc）时宇宙空洞中磁场强度低于2.5×10⁻¹⁷高斯（G）的可能性（95%置信度），为宇宙大尺度结构磁场的宇宙学起源提供了迄今最严格的观测限制。


<details>
  <summary>Details</summary>

Background: 该研究探讨了宇宙大尺度结构中磁场的宇宙学起源问题，重点关注利用伽马射线暴（GRB）间接探测未被发现的宇宙空洞区域中可能存在但尚未观测到的星系际磁场（IGMF）。

Data: 数据来源于费米大面积望远镜（Fermi Large Area Telescope）对伽马射线暴（GRB）221009A的观测。

Method: 利用费米大面积望远镜（Fermi Large Area Telescope）观测伽马暴（GRB）221009A的GeV能级延迟信号，通过分析电磁级联（electromagnetic cascade）中电子-正电子对在星系际磁场（IGMF）中的偏转，建立间接探测方法。

Result: 通过费米大面积望远镜（Fermi Large Area Telescope）对伽马暴（GRB）221009A的观测，首次利用伽马暴数据排除了相干长度大于1百万秒差距（Mpc）时低于2.5×10⁻¹⁷高斯的宇宙大尺度结构磁场（IGMF）存在（95%置信水平）。
...
### [54] [Intergalactic magnetic field lower limits up to the redshift $z\approx3$](https://arxiv.org/abs/2512.11397)
*Ievgen Vovk*

**TL;DR**: 通过费米/LAT望远镜对高红移活动星系核的17年全天观测，在红移z=0.5-3范围内以≈9σ置信度排除了零磁场假设，得出相关长度超过1 Mpc的星系际磁场（intergalactic magnetic field）下限强度B≳1×10⁻¹⁸ cG及体积填充比例f≳90%。


<details>
  <summary>Details</summary>

Background: 该研究探讨了宇宙大尺度星系际磁场（intergalactic magnetic fields）的起源及其演化，重点关注高红移（$z\gtrsim 1$）区域的磁场约束。

Data: data部分：使用费米大面积望远镜(Fermi/LAT)对高红移活动星系核进行的17年全天观测数据，结合0.1 GeV至1 TeV能段的测量结果。

Method: 采用蒙特卡洛(Monte Carlo)模拟结合费米(Fermi)/LAT伽马射线望远镜观测数据，分析0.1 GeV至1 TeV能段的高红移活动星系核(AGN)伽马射线"回波"(γ-ray "echo")。

Result: 通过费米/LAT伽马射线望远镜对高红移活动星系核的17年全天观测，在红移z = [0.5; 3]范围内以≈9σ显著性水平排除了零场强假设，得出磁场相关长度超过1 Mpc时的下限B ≳ 1×10⁻¹⁸ cG。
...
### [55] [Globular Clusters GMRT Pulsar Search (GCGPS) II: Discovery of five MSPs in M69 and M70](https://arxiv.org/abs/2512.11058)
...
**TL;DR**: 利用升级的巨型米波射电望远镜(uGMRT)在球状星团(GC)NGC 6637和NGC 6681中新发现5颗毫秒脉冲星(MSP)，其中两颗精确定位至角秒级精度。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用升级后的巨型米波射电望远镜(uGMRT)在银河系球状星团(GCs)中搜寻脉冲星(PSRs)，特别是填补未被FAST望远镜覆盖的赤纬区域(-53°<δ<-17°)且缺乏高灵敏度射电观测的球状星团。

Data: 数据来源于升级后的巨型米波射电望远镜（uGMRT）的Band-4（550-750 MHz）和Band-3（300-500 MHz）接收器，观测目标为赤纬介于$-53^\circ$至$-17^\circ$之间且未被五百米口径球面射电望远镜（FAST）覆盖的银河系球状星团（GCs）。

Method: 采用升级后的巨型米波射电望远镜（uGMRT）的Band-4（550-750 MHz）和Band-3（300-500 MHz）接收器进行观测，结合干涉成像技术实现毫秒脉冲星（MSPs）的亚角秒级精确定位，并通过波束成形和成像分析提供初步位置估计。

Result: 在球状星团NGC 6637（M69）和NGC 6681（M70）中首次发现了五个毫秒脉冲星（MSP），其中M69A和M69B通过干涉成像精确定位，M70A、M70B和M70C的位置通过uGMRT波束成形和成像分析初步估计。
...
### [56] [Impacts of hydrogen envelope on supernova fallback and the resulting compact remnant masses](https://arxiv.org/abs/2512.11404)
*Kengo Shinoda,Yudai Suwa,Ryosuke Hirai,Ryo Sawada,Kengo Tomida,Kazunari Iwasaki,Takeru K. Suzuki*

**TL;DR**: 研究发现，在初始质量18-28倍太阳质量的贫金属恒星中，当爆炸能量达到氢包层结合能的2-3倍时，反向激波（reverse shock）会返回中心并显著增加致密遗迹（compact-remnant）质量（≥2倍太阳质量），而超过该阈值后氢包层和剥离包层前身星产生的遗迹质量几乎相同，并建立了爆炸能量、氢包层结合能与最终遗迹质量的普适解析关系。


<details>
  <summary>Details</summary>

Background: 研究探讨了核心坍缩超新星(core-collapse supernovae)中回落物质(fallback)对致密残骸质量的影响机制及其与氢包层反向激波(reverse shock)的相互作用。

Data: 使用一维流体动力学模拟（one-dimensional hydrodynamic simulations）研究了初始金属丰度低（metal-poor）、零龄主序质量（ZAMS）为18-28太阳质量的恒星模型，包含有氢包层和无氢包层两种情况，爆炸能量覆盖10^48至10^52尔格范围。

Method: 采用一维流体动力学模拟（one-dimensional hydrodynamic simulations）分析不同爆炸能量（$10^{48}$-$10^{52}\,{\rm erg}$）下贫金属（metal-poor）前身星（$M_{\rm ZAMS}=18$-$28\,M_\odot$）的回落物质（fallback）行为，并建立爆炸能量、氢包层结合能（hydrogen-envelope binding energy）与致密遗迹质量（compact-remnant mass）的解析关系。

Result: 当爆炸能量达到氢包层结合能的2-3倍时，反向激波（reverse shock）返回中心并显著增加致密遗迹质量约2倍太阳质量以上。
