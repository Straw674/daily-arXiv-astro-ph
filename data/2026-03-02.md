...
**TL;DR**: 通过高分辨率宇宙磁流体动力学模拟研究了银河系类似星系环星系介质（CGM）中千秒差距尺度的冷云（cool clouds），发现辐射冷却是云团存活的主导因素，磁场作用较小，且云团运动受复杂背景速度场动量交换加速影响显著偏离弹道轨迹。


<details>
  <summary>Details</summary>

Background: 研究围绕星系周介质(CGM)中千秒差距(kpc)尺度的冷云(cool clouds)的演化动力学及其与复杂宇宙学初始条件的相互作用展开。

Data: 数据来源于一个高度解析的宇宙磁流体动力学放大模拟（cosmological magneto-hydrodynamical zoom-in）中类似银河系的星系，从中选取了十个质量在$10^{4.5-5}$ M$_\odot$范围内的冷云（cold clouds），初始气体质量分辨率（mass resolution）为$200$ M$_\odot$，并通过局部细化方案（targeted refinement scheme）将分辨率提升至$0.2$ M$_\odot$（空间分辨率$r_{\rm{gas,cloud}}$ $\sim$ $O(\rm{pc})$）。

Method: 采用目标细化方案（targeted refinement scheme）在共动球区域内将局部质量分辨率提升1000倍至0.2 M$_\odot$，并结合宇宙学磁流体动力学（magneto-hydrodynamical）模拟初始条件来研究kpc尺度冷云（cool clouds）的演化。

Result: 辐射冷却是云团（cloud）存活的主导物理过程，而磁场作用相对较小。
...
**TL;DR**: 研究发现主序星系中具有星暴特征的特殊群体（SBMSs），其成因是更早的恒星质量聚集（由频繁合并和相互作用触发的高效星暴事件驱动），表现为间歇性短期（约30 Myr）演化模式且主要存在于大质量星系中。


<details>
  <summary>Details</summary>

Background: 研究主题为探讨星系演化中处于恒星形成主序（star-forming main sequence）上但表现出星暴（starburst）特征的“主序星暴”（SBMSs）的物理起源及其对星系演化模式多样性的影响。

Data: data来源为宇宙学模拟（cosmological simulation）NewHorizon。

Method: 采用宇宙学模拟（cosmological simulation）NewHorizon追踪星暴主序星系（SBMSs）的演化，通过分析恒星质量早期聚集、频繁合并事件及由此触发的星暴活动揭示其物理起源。

Result: 主序星系中的星暴现象（SBMSs）由更早的恒星质量聚集驱动，尤其源于更频繁的重复并合及由此触发的异常高效星暴事件，且间歇性出现于大质量星系中，每次持续约30 Myr。
...
**TL;DR**: 通过分析红移（redshift）z > 3的38个星系（包括热尘埃遮蔽星系Hot DOGs、亚毫米亮星系SMGs和光学选类星体）的高转动能级（high-J）CO谱线能量分布（SLEDs），发现高激发态CO发射主要由X射线主导区（XDR）加热或致密激波加热气体驱动，且热尘埃遮蔽星系的高激发可能源于遮蔽活动星系核（AGN）的XDR加热而非激波。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（high-redshift）星系中高转动能级（high-J）CO发射线的群体统计特征，以揭示分子气体激发机制的普遍规律。

Data: 数据来源为阿塔卡马大型毫米波/亚毫米波阵列（ALMA）的存档数据，并结合已发表的测量结果。

Method: 基于阿塔卡马大型毫米波/亚毫米波阵列(ALMA)的存档数据，结合已发表测量结果，对38个红移z~3.1-6.9的天体系统(包括热尘埃遮蔽星系、亚毫米波亮星系和光学选类星体)的CO谱线能量分布(SLEDs)进行编目分析，并采用X射线主导区(XDR)加热或激波加热气体的简单激发模型解释观测结果。

Result: 高红移星系中高转动能级（high-J）CO发射的类群中值谱线能量分布（SLEDs）在Jup=9处急剧上升并在Jup~11-12保持近似平坦，其中尘埃遮蔽星系（Hot DOGs）可能因活动星系核（AGN）遮蔽活动产生的X射线主导区（XDR）加热而非激波驱动表现出更高激发。
...
**TL;DR**: 通过快速射电暴(FRB)色散测量(DM)分析仙女座星系(M31)的环星系介质(CGM)，发现其内晕(0-151kpc)和外晕(151-302kpc)分别存在5.9-59.6 pc cm⁻³和25.7-64.6 pc cm⁻³的DM超额，并推断最佳闭合半径(r_close)为9.2⁺⁹·⁹₋₄.₉ r_vir，对应CGM总质量为18.6⁺⁷·⁹₋₈.₄×10¹⁰ M⊙，表明M31可能含有大量弥散电离气体形式的宇宙重子物质。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用快速射电暴(FRBs)探测仙女座星系(M31)的环星系介质(CGM)，以揭示其作为重子物质重要储库的分布特征和质量占比。

Data: 数据来源于加拿大氢强度测绘实验（CHIME）的快速射电暴（FRB）目录第二版（CHIME/FRB Catalog 2），包括171个视线穿过仙女座星系（M31）晕（$r_{\rm vir}= 302\,\mathrm{kpc}$范围内）的FRB样本和684个对照样本的色散测量（DM）。

Method: 采用快速射电暴（FRB）色散测量（DM）对比分析法，结合参数化闭合半径（closure radius）的广义晕模型（generalized halo model）约束重子分布。

Result: 在仙女座星系(M31)的晕内(0-151 kpc)和晕外(151-302 kpc)分别探测到5.9-59.6 pc cm⁻³和25.7-64.6 pc cm⁻³的色散量(DM)超额，并推断其星系周介质(CGM)总质量为(18.6±8.4)×10¹⁰ M⊙。
...
**TL;DR**: 基于MaNGA巡天的积分视场光谱数据，建立了利用局部恒星形成率面密度（Σ_SFR）预测近邻恒星形成星系（SFGs）中空间分辨尘埃衰减（A_V）的经验模型，在千秒差距尺度上精度达1.3倍，并能迭代恢复尘埃校正后的Σ_SFR。


<details>
  <summary>Details</summary>

Background: 研究主题为利用积分视场光谱（integral field spectroscopy）建立经验模型预测恒星形成星系（SFGs）中空间分辨的尘埃衰减（dust attenuation），以准确校正尘埃对星系结构和演化研究的影响。

Data: 数据来源于MaNGA巡天（Mapping Nearby Galaxies at Apache Point Observatory）的积分场光谱观测，样本包含5,155个星系（$7.20<M_\ast<11.14$，$0.0002 < z < 0.1444$），基于巴尔默减幅（Balmer decrement）从1,898,954个恒星形成像素（spaxels）中提取$A_V$分布图。

Method: 基于积分视场光谱（integral field spectroscopy）数据，采用巴尔默减量（Balmer decrement）方法构建经验模型，以局部恒星形成率面密度（Σ_{\text{SFR}}）为预测因子实现空间分辨尘埃衰减（A_V）的精确估算。
...
**TL;DR**: 研究发现，低质量无恒星的子晕（subhalo）通过引力捕获宿主（host）物质形成可见恒星结构的效率极低（最多约10^-4捕获质量），且捕获物质的空间和速度分布与背景场星（field stars）或晕内恒星流（stellar streams）无法区分。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗物质晕（dark matter halo）在落入更大质量天体后成为次晕（subhalo）时通过引力捕获宿主物质的可能性及其对次晕可见性的潜在影响。

Data: 使用高分辨率N体模拟(N-body simulation)数据。

Method: 采用高分辨率N体模拟(N-body simulation)估算子晕(subhalo)对宿主物质的捕获效率。

Result: 通过高分辨率N体模拟发现，子晕（subhalo）轨道运动期间捕获的物质质量最多仅占其剩余质量的约10^-4，且捕获物质的空间集中度低于原有物质。
...
**TL;DR**: 首次测量了年轻银晕脉冲星（PSR J1740+1000）的自行运动（总自行56.9±8.0 mas/yr，横向速度329±80 km/s）并发现其闪烁弧结构主要由脉冲星风云（PWN）主导的散射造成，其中双相邻层弧揭示了散射区存在AU尺度的折射结构。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系晕（Galactic halo）中年轻脉冲星PSR J1740+1000的自行（proper motion）和闪烁（scintillation）特性，旨在揭示其运动学特征及星际介质散射机制。

Data: 数据来源于南射电望远镜（Nanshan）、五百米口径球面射电望远镜（FAST）和帕克斯射电望远镜（Parkes）的观测数据。

Method: 采用计时分析（timing analysis）和闪烁研究（scintillation study）方法，结合各向同性与各向异性散射假设，测量了脉冲星（PSR J1740+1000）的自行运动并解析了次级谱中的闪烁弧（scintillation arcs）、小弧（arclets）及双层相邻弧（double-layered adjacent arcs）结构。

Result: 首次测量出PSR J1740+1000的自行运动（proper motion），显示其以56.9 +/- 8.0 mas/yr的总自行和329 +/- 80 km/s的横向速度远离银道面（Galactic plane），表明该脉冲星是典型速度的年轻银晕（Galactic halo）脉冲星。
...
**TL;DR**: 通过分析银河系核星团（NSC）距人马座A*约3 pc区域的恒星形成历史（SFH）和金属丰度，发现其恒星主要由古老（≥10 Gyr，占比75.6±9.5%）且富金属（[M/H]中值∼+0.35）的星群组成，存在显著的中龄（2-3 Gyr，20.8±8.7%）和少量年轻（20 Myr，3.6±1.4%）星群，揭示了早期主导形成、中期显著爆发及近期微弱活动的演化模式。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于银河系核星团（NSC）作为研究超大质量黑洞周围致密恒星系统形成与演化的独特实验室，旨在揭示其恒星形成历史（SFH）和金属丰度分布。

Data: 数据来源为甚大望远镜（VLT）NACO成像仪在2.24μm中波段滤光片观测数据，辅以H波段数据，并结合文献中的光谱测量结果。

Method: 采用甚大望远镜(VLT) NACO成像的2.24μm中波段滤波数据，结合H波段数据，构建完备性校正的K波段光度函数(LFs)，并利用MIST、PARSEC和BaSTI恒星演化模型通过蒙特卡洛采样拟合累积光度函数来推导恒星形成历史(SFH)，同时结合文献光谱测量数据优化金属丰度约束。

Result: 银河系核星团(NSC)中75.6±9.5%的恒星质量形成于≥10 Gyr前(金属丰度中位值[M/H]~+0.35)，20.8±8.7%形成于2-3 Gyr的中龄期，另有少量年轻成分(~400 Myr占0.9±0.8%，20 Myr占3.6±1.4%)。
...
**TL;DR**: 研究发现M31核周电离气体（LINERs）的观测特征（如Hα、[NII]径向强度分布）可由年老低质量恒星的光电离解释，但标准模型无法匹配[OIII]发射的高能光子不足现象，暗示传统光致电离机制存在局限性。


<details>
  <summary>Details</summary>

Background: 低电离核区（LINERs）的电离机制研究是理解近邻宇宙中普遍存在的低活动性星系核区电离来源的关键科学问题。

Data: 数据来源于加拿大-法国-夏威夷望远镜（CFHT）的SITELLE仪器获取的空间分辨观测数据。

Method: 采用详细的光电离模型（photoionization modeling）结合CFHT/SITELLE空间分辨观测数据，并针对M31中心区域无恒星形成和活动星系核（AGN）的物理条件进行定制化分析。

Result: 低质量热演化恒星（hot, evolved low-mass stars）的光致电离能大致复现M31核周电离气体的Hα、Hβ和[NII]径向强度分布，但无法解释观测到的较平坦[OIII]辐射分布及较高[O III]/Hβ比值，表明标准恒星光致电离模型存在高能电离光子不足的问题。
...
**TL;DR**: 该研究通过构建严格框架分析盖亚(Gaia) DR3数据中的宽双星(WBs)，发现其速度分布与牛顿引力一致且未显示MOND理论预测的20%增强，从而在a0/10至200 a0加速度范围内为修正引力理论提供了强观测约束。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用宽双星（WBs）在低加速度区域检验引力理论，特别是探索牛顿引力与修正牛顿动力学（MOND）在过渡加速度尺度（a0）附近的预测差异。

Data: 数据来源为盖亚卫星第三次数据释放（Gaia DR3），筛选了130秒差距（pc）内投影间距1-30千天文单位（kAU）的高纯度宽双星（WBs）样本。

Method: 采用高精度天体测量数据筛选、污染源排除、投影效应校正及不确定性传播分析等严格质量控制方法构建宽双星测试框架。

Result: result：在严格质量控制下，宽双星样本的相对速度分布未检测到MOND预测的20%增强效应，所有分离距离均与牛顿引力相符，对a0/10至200a0（MOND加速度尺度）范围内的修正引力理论构成强观测约束。
...
**TL;DR**: 通过JWST观测将HUDFJ0332.4-2746.6（HUDF46）原星系团（proto-cluster）确认成员从18个增至37个，首次直接揭示了$z\gtrsim1$时低恒星质量（$M_\bigstar \approx 10^{7.5\text{--}10.5}\,M_\odot$）年轻星系团特征，发现其虽处于热晕态（hot-halo regime）大质量暗晕（$M_{200} \approx 1.2\times 10^{14}\,M_\odot$）中，但星系仍以恒星形成主导且缺乏红序（red sequence），处于环境淬灭（environmental quenching）前的过渡阶段。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（z≈1.84）原星系团（proto-cluster）HUDF46的早期演化特征及其对理解大质量星系团（massive clusters）后期组装过程的启示。

Data: 数据来源包括哈勃太空望远镜（HST）的ACS相机、詹姆斯·韦伯太空望远镜（JWST）的NIRISS光谱仪、NIRCam和MIRI成像仪以及钱德拉（Chandra）X射线观测数据。

Method: 采用哈勃空间望远镜（HST）、詹姆斯·韦伯空间望远镜（JWST）的NIRISS光谱仪、NIRCam和MIRI成像仪进行多波段观测，结合光谱确认和恒星质量（stellar mass）分析，并通过速度弥散（velocity dispersion）和晕质量（halo mass）计算评估动力学状态。

Result: JWST/NIRISS光谱将HUDFJ0332.4-2746.6（HUDF46）原星系团的确认成员从18个增加到37个，并结合HST/ACS、JWST/NIRCam和JWST/MIRI成像提供了从紫外到中红外的多波段观测数据。
...
**TL;DR**: TIM与Euclid的Fornax深场(EDF-F)交叉相关(TIM×EDF-F)能在红移1.1处以≥7σ置信度检测[CII]-星系互功率谱，并在0.5<z<1.7范围内以≥3σ检测0.1≤k≤10 hMpc⁻¹的泊松分量(交叉散粒噪声)，从而约束宇宙历史一半时期的平均[CII]比强度并评估Euclid选定星系对TIM观测[CII]强度的贡献程度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用太赫兹强度测绘仪（TIM）与欧几里得望远镜（Euclid）的Fornax深场（EDF-F）的交叉相关（TIM×EDF-F）探测宇宙学红移1.1附近的[CII]（碳离子）发射线与星系的交叉功率谱（cross-power spectrum），以约束宇宙历史中期的[CII]平均比强度（specific intensity）并验证欧几里得星系对TIM观测[CII]强度的贡献比例。

Data: 数据来源为太赫兹强度测绘仪（TIM）与欧几里得望远镜（Euclid）福纳克斯深场（EDF-F）的交叉相关观测数据（TIM×EDF-F）。

Method: 采用太赫兹强度映射仪(TIM)与欧几里得望远镜(Euclid)福纳克斯深场(EDF-F)的交叉相关分析(TIM×EDF-F)来测量[CII]发射线与星系在红移0.5-1.7范围内的交叉功率谱。

Result: TIM与Euclid的Fornax深场（EDF-F）交叉相关（TIM×EDF-F）将以≥7σ置信度在红移中值1.1处探测到[CII]-星系交叉功率谱。
...
**TL;DR**: 该论文提出仅基于发光物质（luminous matter）和一个自由参数的新模型，成功拟合175个星系的旋转曲线（rotation curve），其拟合优度优于暗物质（dark matter）模型，并通过两个变换项将暗物质贡献编码到发光物质中，同时发现自由参数与星系光度量存在幂律关系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过仅基于发光物质（luminous matter）估计和一个自由参数的模型解决星系旋转曲线（rotation curve）与暗物质（dark matter）理论预测间的矛盾问题。

Data: 数据来源于SPARC样本库中的175个星系旋转曲线观测数据。

Method: 采用基于发光物质估计和单一自由参数的模型预测多普勒频移光谱（Doppler-shifted spectra），并通过拟合175个星系的SPARC样本旋转曲线获取质量-光比、拟合优度及自由参数值。

Result: 提出的新模型仅基于发光物质估计和一个自由参数即可预测观测到的多普勒频移光谱，其平均约化卡方优于暗物质模型且能拟合更多星系数据。
...
**TL;DR**: 首次通过红外光谱探测到Wd1星团的主序星，确定其赫罗图(Hertzsprung-Russell diagram)并利用转折点(turn-off)得出5.5±1.0 Myr的年龄，证实其为单次恒星形成事件，为研究大质量恒星演化提供了可靠基准。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用韦斯特伦德1（Westerlund 1, Wd1）这一银河系内已知质量最大的年轻星团，作为理解大质量恒星演化的理想基准。

Data: 数据来源为欧洲南方天文台甚大望远镜（VLT）的近红外多目标光谱仪（KMOS）获取的多历元近红外光谱观测数据。

Method: 采用近红外VLT/KMOS光谱观测和CMFGEN模型分析恒星参数，构建赫罗图（Hertzsprung-Russell diagram）并与演化模型等时线（isochrones）对比以确定年龄。

Result: 首次通过红外光谱探测到Westerlund 1星团的主序星，并基于赫罗图转折点确定其年龄为5.5±1.0百万年（距离4.23+0.23-0.21千秒差距），支持单次恒星形成事件。
...
**TL;DR**: 首次利用地基甚长基线干涉测量（VLBI）在活动星系核（AGN）中明确探测到折射亚结构（refractive substructure），发现TXS 2005+403在1-5 GHz波段观测中表现出湍流诱导的稳定散射特征，为研究银河系湍流提供了独特实验室。


<details>
  <summary>Details</summary>

Background: 首次利用地基甚长基线干涉测量（VLBI）在活动星系核（AGN）中明确探测到折射亚结构（refractive substructure），为研究银河系湍流（Galactic turbulence）提供了新途径。

Data: 使用地面甚长基线干涉测量（VLBI）在1-5 GHz频段对TXS 2005+403进行观测，数据来源于2010-2019年间多次观测的长期基线数据。

Method: 采用地面甚长基线干涉测量（VLBI）技术分析活动星系核（AGN）折射亚结构，结合1-5 GHz频段观测数据识别湍流诱导特征，排除单纯衍射效应平滑散射轮廓的干扰。

Result: 首次利用地基甚长基线干涉测量（VLBI）在活动星系核（AGN）中明确探测到折射亚结构（refractive substructure）。
...
**TL;DR**: 开发了nanoCMB这一精简但精确的未透镜化宇宙微波背景（CMB）温度与偏振角功率谱计算器，采用Python实现全视线积分方法，在$2 \le \ell \le 2500$范围内与CAMB误差低于1%，计算耗时约10秒。


<details>
  <summary>Details</summary>

Background: 开发精确计算平坦ΛCDM宇宙学模型下未透镜化宇宙微波背景（CMB）温度与偏振角功率谱的轻量化工具。

Data: 数据来源于基于同步规范（synchronous gauge）的爱因斯坦-玻尔兹曼扰动方程、RECFAST复合过程、预计算的球贝塞尔函数表以及波数和共形时间上的非均匀网格的模拟计算。

Method: 采用视线积分法(line-of-sight integration method)结合RECFAST复合模型、同步规范下的耦合爱因斯坦-玻尔兹曼扰动方程、预计算球贝塞尔函数表以及优化的非均匀波数和共形时间网格。

Result: 开发了nanoCMB（一种基于Python的简洁计算器），通过视线积分法（line-of-sight integration）精确计算平坦ΛCDM宇宙学模型下未透镜化的CMB温度与极化角功率谱（$C_\ell^{TT}$、$C_\ell^{EE}$、$C_\ell^{TE}$），与CAMB结果在$2 \le \ell \le 2500$范围内误差低于1%。
...
**TL;DR**: 通过暗辐射-物质解耦（DRMD）模型解决哈勃张力（Hubble tension）并预测与重子声波振荡（BAO）尺度相似的暗声波振荡（DAO），其拖曳视界尺度（drag-horizon scale）为54-65 Mpc/h、振幅为0.02-0.05，与DESI异常数据吻合，为验证该模型提供了观测目标。


<details>
  <summary>Details</summary>

Background: 当前宇宙学标准模型（ΛCDM）面临哈勃常数危机（Hubble tension）和暗能量光谱仪（DESI）数据异常的严峻挑战。

Data: 数据来源为普朗克卫星(Planck)对宇宙微波背景(cosmic microwave background)的测量和SH0ES项目校准的超新星(supernova)数据。

Method: 采用普朗克(Planck)宇宙微波背景测量和SH0ES校准超新星数据，通过独立于大尺度结构数据的推断方法分析暗辐射-物质解耦(DRMD)场景预测的暗声学振荡(DAO)信号。

Result: 通过普朗克宇宙微波背景和SH0ES校准的超新星数据独立推断，发现暗声学振荡（DAO）信号的存在，其拖曳视界尺度（drag-horizon scale）为54至65 Mpc/h（68%置信区间），振幅为0.02至0.05（68%置信区间）。
...
**TL;DR**: 利用星系紫外光度函数在红移6-9的观测数据约束标准暴胀功率谱的修正（引入凸起或振荡特征），发现其可探测波数范围0.5≲k≲20 Mpc⁻¹并给出振幅上限，比再电离光学深度测量更直接。


<details>
  <summary>Details</summary>

Background: 研究通过星系紫外光度函数（galaxy ultraviolet luminosity function）在红移$z=6-9$的观测数据，检验标准暴胀（standard inflationary）功率谱的修正模型。

Data: 研究使用了星系紫外光度函数（galaxy ultraviolet luminosity function）在红移$z=6-9$的观测数据来限制对标准暴胀功率谱的修正。

Method: 采用星系紫外光度函数测量数据，结合标准暴胀功率谱修正模型（引入凸起或振荡特征），在波数范围0.5≲k≲20 Mpc⁻¹内约束初始条件对物质功率谱的影响。

Result: 利用星系紫外光度函数测量在$z=6-9$处探测到波数$0.5\lesssim k \lesssim 20$ Mpc$^{-1}$范围内对标准暴胀功率谱的修正（如凸起或振荡特征）具有敏感性。
...
**TL;DR**: 研究发现行星质量至太阳质量范围内的原初黑洞（PBHs）和纳赫兹随机引力波（GW）背景可由振幅约$O(10^{-2})$的平坦曲率功率谱增强统一解释，且与现有观测约束一致。


<details>
  <summary>Details</summary>

Background: 该研究探讨了原初黑洞（PBHs）作为暗物质候选体的可能性及其与原初曲率功率谱增强的关联。

Data: 数据来源为Subaru-HSC（Hyper Suprime-Cam）的高频观测（high-cadence observations）和脉冲星计时阵（pulsar timing arrays, PTAs）报告的纳赫兹随机引力波背景（nanohertz stochastic gravitational-wave background）。

Method: 采用宽幅近平坦曲率功率谱增强模型（amplitude of $O(10^{-2})$）统一解释微透镜事件和纳赫兹随机引力波背景（stochastic gravitational-wave background）。

Result: 原初曲率功率谱（primordial curvature power spectrum）的宽幅近平坦增强（幅度约10^-2）统一解释了行星质量原初黑洞（PBHs）和纳赫兹随机引力波背景（stochastic GW background）的共同起源。
...
**TL;DR**: 研究发现小尺度功率谱（small-scale power spectrum）增强通过影响电离光子源（矮星系）和汇（迷你暗晕）的竞争显著改变电离场形态和21厘米信号，为探测小尺度功率谱提供了新途径。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙再电离（reionization）过程中矮星系（dwarf galaxies）和迷你晕（minihalos）对电离光子源和汇的贡献差异及其对小尺度功率谱（small-scale power spectrum）的依赖性。

Data: 数据来源于基于Cielo等人（2025）提出的功率谱（C25）进行的模拟研究，并结合高红移星系观测的紫外光度函数（UV luminosity function）和再电离历史的约束条件。

Method: 采用数值模拟方法分析小尺度功率谱增强对电离场和21厘米信号的影响，重点考察C25模型与标准ΛCDM模型的差异。

Result: 21厘米信号功率谱和电离气泡尺寸分布在C25小尺度功率谱增强模型下与标准ΛCDM模型存在显著差异。
...
**TL;DR**: 这篇论文的核心贡献是提出了一种基于模拟的线强度映射（LIM）预测框架，通过联合分析原子氢（HI）、电离碳（[CII]）和一氧化碳（CO）在红移$z\sim0.5-1.7$的自功率谱和互功率谱，发现当前一代巡天设备中星系互相关将成为主要探测手段，并预测未来仪器改进可将灵敏度提升数个量级，从而对宇宙学、恒星形成及重子循环（baryon cycle）的红移演化施加更严格约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙恒星形成高峰期（$z\sim1-2$）通过线强度映射（LIM）技术对星系及周围重子（baryon）循环中原子氢、电离碳和一氧化碳的空间分布与化学组成进行断层扫描。

Data: 数据来源为磁流体动力学模拟（magnetohydrodynamic simulations）生成的四种示踪剂（TIM、EoRSpec/FYST、MeerKAT和Euclid）在红移$z\sim0.5-1.7$范围内的三维分布。

Method: 采用磁流体动力学(MHD)模拟前向建模三维分布，直接捕获双晕(two-halo)、单晕(one-halo)和散粒噪声(shot noise)统计量，并开发了针对各向异性几何和傅里叶空间覆盖的线强度映射(LIM)信噪比形式化方法。

Result: 星系交叉关联将成为当前一代巡天的主要发现渠道，能够以S/N∼1-10探测CO和HI 21 cm的自功率谱及CO×21 cm交叉功率谱，同时对[CII]-158μm信号给出上限约束。
...
Background: 研究探讨了弱引力透镜（WL）高阶统计量（HOS）在区分广义相对论（GR）和修正引力理论（MG）以及考虑大质量中微子存在时的潜力。

Data: 数据来源于广义相对论（GR）和修正引力（f(R) gravity）的N体模拟生成的模拟收敛（convergence）图。

Method: 采用高阶弱引力透镜统计量（HOS）结合N体模拟生成的GR和f(R)引力理论收敛图，通过多维度度量指标量化不同探测方案的区分能力。

Result: 弱引力透镜高阶统计量（HOS）能够区分广义相对论（GR）和修正引力（MG），即使存在大质量中微子。
...
**TL;DR**: 在低质量星系团Abell 4067（$z=0.099$）中发现了一个与弱合并激波相关的射电遗迹（radio relic），其1.48±0.02 Mpc的尺寸和3.10±0.03×10²⁴ W Hz⁻¹的射电功率表明低质量星系团中的遗迹可能由低能合并事件形成，且未探测到中心射电晕（radio halo）进一步支持了明亮射电晕并非所有合并的普遍结果。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用星系团合并产生的激波（shock waves）作为探针，揭示宇宙大尺度结构（large-scale structure）增长与演化的机制。

Data: 数据来源于MeerKAT UHF波段（0.58-1.09 GHz）射电观测和XMM-Newton X射线数据。

Method: 采用MeerKAT UHF波段(0.58-1.09 GHz)射电观测和XMM-Newton X射线数据分析，结合射电遗迹功率($P_{150\,\mathrm{MHz}}$)与团质量($M_{500}$)的标度关系，验证弱合并激波模型。

Result: 在Abell 4067星系团（红移$z=0.099$）中发现了一个弧状射电遗迹（radio relic），其最大线性尺寸为$\sim 1.48 \pm 0.02$ Mpc，位于距团中心0.95 Mpc处，射电功率为$3.10 \pm 0.03 \times 10^{24}$ W Hz$^{-1}$（150 MHz），表明其形成与弱并合激波（weak merger shocks）相关。
...
**TL;DR**: 开发了OmegaNeuron机器学习工具，通过结合图像相似性方法和瞬态分析自动识别引力波探测器中的辅助通道噪声（glitches），提升了噪声识别的效率和观测可靠性。


<details>
  <summary>Details</summary>

Background: 引力波（GW）天文学通过激光干涉引力波天文台（LIGO）等仪器推进了对致密天体并合的理解。

Data: 数据来源于激光干涉引力波天文台（LIGO）的引力波（GW）观测数据及辅助通道（auxiliary channels）记录的瞬态噪声（glitches）。

Method: 采用机器学习工具OmegaNeuron，结合GravitySpy的图像相似性方法和Omega Scan的瞬态分析技术，自动识别辅助信道(auxiliary channels)中的瞬态噪声(glitches)。

Result: OmegaNeuron通过结合GravitySpy的图像相似性方法和Omega Scan的瞬态分析，实现了辅助信道中瞬态噪声（glitches）的自动化识别，提升了引力波探测器灵敏度和观测可靠性。
...
**TL;DR**: 该研究开发了兼容超高真空的惯性隔离和位置传感技术，将主动平台稳定频率扩展至10 mHz，其激光位置传感器在10 mHz以上实现亚皮米/√Hz灵敏度（比LIGO现有传感器提升100倍），惯性传感器低频灵敏度比商用地震仪提升5倍，集成到类LIGO干涉仪模型后预测10 Hz频率下灵敏度提升一个数量级，并将中等质量黑洞双星（10³ M⊙）的探测距离扩大3倍。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于突破地面引力波探测器在20 Hz以下频段的灵敏度限制，通过开发新型惯性隔离与位置传感技术推动下一代引力波天文台（如Cosmic Explorer和Einstein Telescope）的发展。

Data: data：研究使用了激光位置传感器（laser position sensors）和惯性传感器（inertial sensors）的实验数据，并与当前LIGO位置传感器及商用地震仪（commercial seismometers）进行了对比。

Method: 采用超高压兼容惯性隔离技术和激光位置传感技术，实现10 mHz频段主动平台稳定，其中激光位置传感器在10 mHz以上频段灵敏度达亚皮米每根号赫兹（sub-pm/√Hz），惯性传感器低频灵敏度比商用地震仪提升至少5倍。

Result: 开发出兼容超高真空的惯性隔离和位置传感技术，实现了低至10 mHz的主动平台稳定，激光位置传感器在10 mHz以上达到亚皮米/√Hz灵敏度且与输入光偏振无关，比现有LIGO传感器灵敏度提升100倍。
...
**TL;DR**: 通过分析2015-2021年哈勃ACS/WFC（Advanced Camera for Surveys Wide Field Channel）相机的短暗场数据，发现探测器中的下沉像素（sink pixels, SPs）以约2.2像素/天的速率持续产生，至2021年累计达44,068个（占科学帧0.25%），其y位置分布呈现靠近芯片间隙的反弹效应（bounce-back effect）局部峰值且受电荷转移效率（CTE）损失影响导致梯度随时间变陡。


<details>
  <summary>Details</summary>

Background: 研究聚焦于哈勃太空望远镜高级巡天相机宽视场通道(ACS/WFC)中电荷陷阱导致的异常像素(SPs)的长期演化特征及其与电荷转移效率(CTE)退化的关联机制。

Data: 数据来源于哈勃太空望远镜高级巡天相机广域通道(ACS/WFC)2015-2021年间的短曝光(0.5秒)暗场(dark)和沉陷像元(SP)参考文件，通过分析LED闪光校准数据识别出电子值低于局部背景10个电子以上的异常像元。

Method: 采用暗场和SP参考文件分析高级巡天相机宽视场通道(ACS/WFC)中的下沉像素(SPs)，通过比较LED闪光短曝光(0.5秒)暗场局部背景值(≤-10电子)来识别电荷收集和捕获异常像素。

Result: 在2015-2021年间，ACS/WFC（Advanced Camera for Surveys Wide Field Channel）的WFC1和WFC2分别新增了5,430和5,649个下沉像素（sink pixels, SPs），日均生成率约为2.15和2.23像素/天。
...
**TL;DR**: 研究发现太阳盲区通道(Solar Blind Channel)在25.5°C以上时暗计数率升高但异常升高与南大西洋异常区(South Atlantic Anomaly)无关，且SBC-LODARK孔径区域始终保持稳定低暗计数率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于太阳盲通道(Solar Blind Channel, SBC)在高温环境下暗计数率升高的异常现象及其与南大西洋异常区(South Atlantic Anomaly, SAA)的潜在关联。

Data: 数据来源于哈勃太空望远镜（HST）的太阳盲通道（SBC）观测数据，并结合了轨道参数（SPT products）以分析南大西洋异常区（SAA）对暗电流的影响。

Method: 采用哈勃空间望远镜(HST)轨道参数(SPT产品)定位暗场曝光位置，并对比南大西洋异常区(SAA)边界距离与暗计数率升高的相关性，同时分析探测器内SBC-LODARK孔径区域的暗计数率稳定性。

Result: 太阳盲区通道(Solar Blind Channel)在探测器温度超过25.5°C时通常表现出升高的暗计数率，但在达到该阈值前也观察到暗计数率快速上升的异常现象。
...
**TL;DR**: 该论文通过比较锥形三波导耦合器（tapered tri-coupler）和多模干涉耦合器（MMI）在1.5-1.8 μm波段的性能，发现二者在系外行星光通量（>85%）和恒星光衰减方面优于标准三波导耦合器，其中MMI对制造误差容忍度最高，为集成化宽带消零干涉测量（nulling interferometry）提供了实用化解决方案。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于天文光子学（astrophotonics）在抑制恒星光以揭示系外行星和伴星方面的关键应用，特别是基于光子灯笼（photonic lantern）的长基线零干涉测量（nulling interferometry）和紧凑型宽带光子合束器（photonic beam combiner）的研发。

Data: 研究中使用的数据来源于对两种倏逝波三耦合器（tri-coupler）和多模干涉耦合器（MMI）在1.5-1.8 μm波段的仿真模拟结果。

Method: 采用三种波导配置（三耦合器）和一种多模干涉耦合器（MMI）在1.5-1.8微米波段进行仿真比较，评估系外行星通量、星光衰减、传感特性和制造容差。

Result: 三波导结构（tri-coupler）和MMI耦合器在1.5-1.8 μm波段实现了更高的系外行星透射率（>85%）和稳定的消色差零位，其中锥形三波导设计进一步优化为非简并传感状态，而MMI耦合器展现出最佳的抗制造误差鲁棒性。
...
**TL;DR**: 研究发现，在流体剪切界面、哈里斯电流片(Harris current sheet)和等离子体剪切界面的模拟中，用包含最不稳定模态的线性本征模叠加初始化可大幅缩短线性演化阶段计算时间，并影响非线性演化进程。


<details>
  <summary>Details</summary>

Background: 研究聚焦于不稳定流体动力学（hydrodynamic）和磁流体动力学（magnetohydrodynamic）平衡态的早期演化中主导线性本征模（linear eigenmodes）的作用及其对非线性阶段的影响。

Data: 数据来源于使用MPI-AMRVAC非线性（磁）流体动力学模拟代码对流动剪切流体界面、哈里斯电流片（Harris current sheet）和流动剪切等离子体界面进行的模拟，初始扰动采用Legolas代码计算的线性本征模或其叠加。

Method: 采用线性本征模叠加（包含最不稳定模）作为初始扰动，结合非线性（磁）流体动力学模拟代码MPI-AMRVAC和线性本征模计算工具Legolas进行演化分析。

Result: 使用包含最不稳定模式的线性本征模叠加扰动不稳定平衡可显著减少线性演化阶段的计算时间，在最佳情况下达到非线性阶段所需的模拟时间缩短一个数量级。
...
**TL;DR**: 该研究通过系统比较四种符号回归(SR)方法在SDSS DR17数据中恒星、星系和类星体分类的应用，发现低复杂度解析函数（如MvSR和PhySO）不仅能达到高预测可靠性（Cohen's Kappa 0.8948）和参数稳定性（σ<0.002），还提供了天体物理界限的透明数学描述。


<details>
  <summary>Details</summary>

Background: 研究探索符号回归(SR)作为可解释机器学习方法，用于斯隆数字巡天(SDSS DR17)中恒星、星系和类星体的分类问题。

Data: 数据来源于斯隆数字巡天数据发布17（SDSS DR17）中10万个光谱确认天体的代表性训练子集，并通过5折分层交叉验证协议评估。

Method: 采用四种前沿符号回归（symbolic regression）框架（PySR、ESR、PhySO、MvSR）推导复杂度≤10的解析函数，通过5折分层交叉验证将光谱红移（z）映射为连续分类分数。

Result: 符号回归(SR)方法在斯隆数字巡天数据发布17(SDSS DR17)中实现了对恒星、星系和类星体的高可靠性分类，其中多视图符号回归(MvSR)的科恩卡帕值达0.8948，物理符号优化(PhySO)表现出卓越的参数稳定性(σ<0.002)。
...
**TL;DR**: 研究哈勃太空望远镜(HST)先进巡天相机宽视场通道(ACS/WFC)的超偏置(Superbias)校准帧长期行为，发现维修任务4(SM4)后15年间超偏置结构保持稳定，但受不稳定热列和读出暗信号增加影响，仍需比宽视场相机3紫外可见通道(WFC3/UVIS)年度校准更高的更新频率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于哈勃太空望远镜(HST)搭载的先进巡天相机宽视场通道(ACS/WFC)超级偏置(superbias)校准帧的长期稳定性评估，旨在优化其生成频率以平衡校准质量与操作效率。

Data: 数据来源于哈勃太空望远镜（HST）上高级巡天相机宽视场通道（ACS/WFC）2007年至2024年期间生成的超级偏置（superbias）校准帧。

Method: 采用主成分分析（PCA）评估超偏置（superbias）帧中是否存在长期全局结构，并结合视觉检查和像素级比较分析其稳定性。

Result: 哈勃太空望远镜（HST）搭载的高级巡天相机宽视场通道（ACS/WFC）的超偏置（superbias）校准帧结构在第四次维修任务（SM4）后保持稳定，未检测到显著全局趋势或系统性偏移。
...
**TL;DR**: 提出基于池式主动学习（pool-based active learning）的二元分类方法，显著提高了在观测资源有限、标签不平衡条件下系外行星宜居性评估的效率，并通过集成模型从原非宜居行星中识别出一个需优先跟进研究的候选目标。


<details>
  <summary>Details</summary>

Background: 系外行星（exoplanet）目录规模与异质性增长导致系统性宜居性评估面临挑战，尤其受限于潜在宜居行星的极端稀缺性及其标签的动态变化特性。

Data: 数据来源于宜居世界目录（Habitable World Catalog）和美国宇航局系外行星档案（NASA Exoplanet Archive）构建的统一数据集。

Method: 采用梯度提升决策树（gradient-boosted decision trees）建立监督基线模型，并嵌入基于不确定性边缘采样（uncertainty-based margin sampling）的主动学习框架进行效率优化。

Result: active learning（主动学习）显著减少了达到监督学习性能所需的标记样本数量，在标签效率上展现出明显优势。
...
### [33] [Contemporaneous Optical and Near-Infrared Observations of the Interstellar Comet 3I/ATLAS Pre- and Post-Perihelion](https://arxiv.org/abs/2602.23586)
...
**TL;DR**: 通过对星际天体3I/ATLAS的光谱观测，发现其在光学波段呈正光谱斜率（21-27%蓝光区，6-10%红光区），近红外波段呈负斜率（-0.9%和-2.3%），并在1.1微米处出现由尘埃彗发散射光导致的谱形转折，同时检测到Ni、CN、Fe和微弱C₃发射，但未发现水吸收特征增强。


<details>
  <summary>Details</summary>

Background: 星际天体为研究其他恒星系统的形成提供了独特视角。

Data: 数据来源于凯克一号望远镜的低分辨率成像光谱仪（Keck-I/LRIS）、凯克二号望远镜的近红外阶梯光栅光谱仪（Keck-II/NIRES）、双子座望远镜的多目标光谱仪（Gemini/GMOS）和夏威夷大学88英寸望远镜的超新星积分场光谱仪（UH88/SNIFS），覆盖波长范围0.3至2.5微米。

Method: 采用凯克望远镜（Keck-I/LRIS、Keck-II/NIRES）、双子座望远镜（Gemini/GMOS）和UH88/SNIFS光谱仪获取0.3-2.5μm波长范围的光谱数据，分析光谱斜率及尘埃彗发（dusty coma）散射光特征。
...
### [34] [Linear Polarization Variations and Circular Polarization are Common Among Airless Bodies](https://arxiv.org/abs/2602.24196)
*Sloane J. Wiktorowicz,Amanda J. Bayless,Larissa A. Nofi*

**TL;DR**: 利用POLISH2偏振仪发现小行星和近地天体(65803 Didymos)在383-720 nm波段普遍存在与自转相位锁定的线偏振变化，揭示表面反照率和/或成分不均匀性是主因，并提出光学圆偏振可作为金属富集天体的新识别方法。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于小行星和近地天体（NEO）表面线性偏振（linear polarization）与旋转相位锁定的变化规律及其与表面反照率（albedo）或成分异质性的关联。

Data: 数据来源于利克天文台(Lick Observatory)的Shane 3米望远镜和Nickel 1米望远镜上搭载的POLISH2偏振仪，在383至720 nm清晰波段对12颗小行星和近地天体(NEO)进行了为期八年的偏振观测。

Method: 采用POLISH2偏振仪（POLarimeter for In the Study of Heliophysics and Astrophysics）在Lick天文台Shane 3米和Nickel 1米望远镜上获取383至720 nm波段线性偏振数据，通过差分技术分析表面反照率或成分异质性引起的偏振变化。

Result: 在383至720 nm波段内发现小行星和近地天体（NEO）普遍存在与自转相位锁定的线偏振变化，且表面反照率和/或成分不均匀性是其主要成因。
...
### [35] [In-flight calibration of RADEM, the JUICE mission radiation monitor](https://arxiv.org/abs/2602.24246)
...
**TL;DR**: 这篇论文通过利用银河宇宙射线（galactic cosmic rays）对木星冰卫星探测器（JUICE）搭载的辐射硬化电子监测仪（RADEM）进行在轨校准，建立了新的校准系数和基于领结法（bow-tie method）的流量重建算法，验证了其在行星际空间准确测量质子通量的能力，与独立观测结果误差在两倍以内，但电子通量重建仍需进一步分析。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于木星冰卫星探测器(JUICE)搭载的辐射硬化电子监测仪(RADEM)在巡航阶段和任务期间对高能质子(proton)和电子(electron)的测量及其在轨校准。

Data: 数据来源于RADEM（RADiation-hard Electron Monitor）在JUICE（Jupiter Icy Moons Explorer）任务巡航阶段和标称任务期间测量的高能质子与电子数据，并结合基于Badhwar-O'Neill 2020银河宇宙线模型的理论预期进行校准。

Method: 采用基于巴德瓦-奥尼尔(Badhwar-O'Neill)2020年银河宇宙线模型的理论预期与弓形结(bow-tie)方法相结合的阈值依赖在轨校准技术，通过逐步提高探测器阈值来修正高能粒子响应函数。

Result: RADEM（RADiation-hard Electron Monitor）通过调整探测器阈值利用银河宇宙射线（galactic cosmic rays）进行在轨校准，校准斜率与地面校准值差异可达一个数量级，重建的质子通量与独立观测结果误差在两倍以内，证实其在行星际空间质子通量测量的准确性。
...
### [36] [New possible way to determine stellar wind terminal velocity from analysis of Lyman-$α$ absorption spectra](https://arxiv.org/abs/2602.23781)
*Sergey D. Korolkov,Vladislav V. Izmodenov*

**TL;DR**: 研究发现低速恒星风（terminal velocities $V_0 \lesssim 200$ km/s）与部分电离星际介质（partially ionized ISM）相互作用时，超音速区电荷交换效率显著提高，可在莱曼α（Lyman-$α$）光谱中形成可观测的中性风吸收特征，为直接测量恒星风速度提供了新方法。


<details>
  <summary>Details</summary>

Background: 研究探讨恒星风（stellar wind）与部分电离星际介质（partially ionized ISM）相互作用产生的物理现象及其莱曼α（Lyman-α）光谱特征。

Data: 数据来源于哈勃太空望远镜（Hubble Space Telescope）对太阳和其他恒星周围莱曼-α（Lyman-α）吸收光谱的观测，以及基于二维动力学-流体力学（2D kinetic-hydrodynamic）模型的参数化数值模拟分析。

Method: 采用二维动理学-流体力学（kinetic-hydrodynamic）模型进行参数化数值分析，通过改变终端风速（terminal wind velocity）并保持恒定动压来维持星体层（astrosphere）尺寸一致性，重点研究低速恒星风（V0≲200 km/s）中超音速区域电荷交换效率增强产生的可观测中性风吸收特征。

Result: 低速恒星风（终端速度V0≲200 km/s）在超音速区域电荷交换效率显著提高，会产生可观测的中性风吸收特征。
...
### [37] [The PLUTO Code on GPUs: Offloading Lagrangian Particle Methods](https://arxiv.org/abs/2602.23434)
*Alessio Suriano,Stefano Truzzi,Agnese Costa,Marco Rossazza,Nitin Shukla,Andrea Mignone,Vittoria Berta,Claudio Zanni*

**TL;DR**: 开发了基于OpenACC和MPI的GPU兼容C++版PLUTO代码拉格朗日粒子模块（LP module），显著提升了大尺度相对论磁化天体物理流中激波加速粒子非热辐射模拟的计算效率，在128个GPU节点上相比同等规模CPU节点实现了6倍加速。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发用于模拟大尺度相对论性磁化天体物理流中激波加速粒子产生的非热辐射的GPU兼容计算工具。

Data: 数据来源于使用PLUTO代码中的拉格朗日粒子(Lagrangian Particles, LP)模块进行的大规模相对论磁化天体物理流动模拟，该模块通过求解相对论宇宙射线输运方程更新粒子能量分布，并利用OpenACC编程模型和消息传递接口(Message Passing Interface)库实现对商用GPU及多节点(pre-)exascale计算设施的兼容。

Method: 采用GPU兼容的C++重构拉格朗日粒子(LP)模块，结合OpenACC编程模型和消息传递接口(MPI)库，支持商用单GPU及多节点(预)百亿亿级计算设施。

Result: 通过OpenACC编程模型和消息传递接口（MPI）库重新设计的LP模块可兼容单块商用GPU及多节点（准）百亿亿次计算设施，在128个全GPU节点（每节点4块GPU）上相比同等数量高端CPU节点（每节点112核）实现了6倍加速。
...
### [38] [Superposition model for energy reconstruction and mass identification in cosmic ray spectra](https://arxiv.org/abs/2602.23637)
*Hu Liu,Fanping Li,J. Zhao,L. Y. Wang,Zhe Li,S. Z. Chen*

**TL;DR**: 该论文提出了一种基于叠加模型的新方法，利用大型高海拔空气簇射观测站（LHAASO）电磁和缪子探测器在固定距离测量的粒子密度（而非环形带积分粒子数），以两条普适的、与成分和能量无关的校准线重建宇宙线能量和对数质量（lnA），在40度天顶角内实现了能量偏差±5%、lnA偏差±0.3的精度，能量分辨率在1 PeV以上为5%-15%，铁核在10 PeV以上的质量分辨率优于25%，且强子模型依赖性仅与lg(E/A)相关而与初始成分无关。


<details>
  <summary>Details</summary>

Background: 宇宙线能谱"膝区"(knee)可能反映银河宇宙线源的最大加速能量或银河系束缚宇宙线的能力极限。

Data: 数据来源于大型高海拔空气簇射观测站（LHAASO）的电磁探测器和缪子探测器测量的固定距离处粒子密度。

Method: 采用基于叠加模型（superposition model）的新方法，利用两条普适的、与成分和能量无关的校准线重建能量和对数质量（lnA），并通过固定距离处的粒子密度（而非环形带积分粒子数）提高分辨能力。

Result: 基于叠加模型的新方法利用LHAASO电磁和缪子探测器在固定距离测量的粒子密度（而非环形带积分粒子数），将能量和lnA（对数质量）重建偏差分别控制在±5%和±0.3以内，能量分辨率在1PeV以上为5%-15%，10PeV以上铁核质量分辨率最优达25%以下。
...
### [39] [Search for Gamma-ray emission from Abell 119 galaxy cluster using INTEGRAL/ISGRI, COMPTEL, and DAMPE data](https://arxiv.org/abs/2602.23686)
*Siddhant Manna,Shantanu Desai*

**TL;DR**: 通过对邻近合并星系团Abell 119 ($z=0.044$)进行跨七个数量级光子能量的多波段（硬X射线、MeV $γ$射线、GeV--TeV）观测分析，未在任何波段检测到显著的非热高能$γ$射线辐射，为Fermi-LAT报道的GeV超出现象提供了独立的多波段上限约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于邻近合并星系团Abell 119（红移z=0.044）中非热高能γ射线（gamma-ray）辐射的多波段搜寻及其天体物理意义。

Data: 数据来源于INTEGRAL/ISGRI（30-100 keV）、COMPTEL（0.75-30 MeV）和DAMPE（3 GeV-1 TeV）的存档观测数据。

Method: 采用多波段（multi-band）数据分析方法，结合INTEGRAL/ISGRI（30-100 keV）、COMPTEL（0.75-30 MeV）和DAMPE（3 GeV-1 TeV）的观测数据，通过统计显著性检验和置信水平上限计算来约束非热辐射（non-thermal emission）通量。

Result: 在邻近合并星系团Abell 119（红移z=0.044）的七个能量量程观测中，未在任何波段检测到统计显著的非热高能γ射线辐射。
...
### [40] [Survival Analysis of Intermediate-Mass Black Holes in Dense Star Clusters](https://arxiv.org/abs/2602.23431)
*Miguel A. S. Martinez,Elena González Prieto,Frederic A. Rasio*

**TL;DR**: 通过牛顿和后牛顿双星-单星散射实验，发现中等质量黑洞（IMBH）的动力学抛射概率强烈依赖于其与环境中其他黑洞的质量比及环境速度弥散度，并估算典型银河系球状星团中质量≲10³M⊙的IMBH难以保留至今。


<details>
  <summary>Details</summary>

Background: 研究探讨了中等质量黑洞（IMBH）在致密星团环境中通过恒星碰撞形成后，因动力学反冲或引力波反冲而被抛射的机制及其保留概率。

Data: 研究使用了牛顿和后牛顿（post-Newtonian）双星-单星散射实验数据，并结合了生存分析（survival analysis）技术来建模。

Method: 采用牛顿和后牛顿(NR/PN)双星-单星散射实验结合生存分析(survival analysis)技术，量化研究了中等质量黑洞(IMBH)因动力学反冲(dynamical recoil)或引力波反冲(gravitational-wave recoil)被抛射的概率。

Result: 中等质量黑洞（IMBH）通过动力学反冲或引力波反冲从致密星团环境中被抛射的概率强烈依赖于其与环境中其他黑洞的质量比以及环境的速度弥散。
...
### [41] [The environmental dependence of mid-IR luminous dusty Supernovae](https://arxiv.org/abs/2602.23673)
...
**TL;DR**: 通过斯皮策（Spitzer）和广域红外巡天探测器（WISE）图像发现42颗中红外高光度尘埃超新星（dusty supernovae），分析其宿主环境与尘埃特性，揭示尘埃超新星倾向位于高Hα等效宽度（EW(Hα)）、低金属丰度及高宿主消光区域，且尘埃质量与恒星形成率呈负相关，但与金属丰度和宿主消光无关，表明尘埃形成过程对后两者不敏感。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用中红外(IR)数据揭示尘埃超新星(SN)的尘埃形成机制及其与局部环境参数(如Hα等效宽度(EW)、金属丰度、宿主消光)的关联性，以探索不同前身星系统的多样性。

Data: 数据来源为斯皮策（Spitzer）和广域红外巡天探测器（WISE）的中红外图像及局部积分场光谱数据。

Method: 利用斯皮策（Spitzer）和广域红外巡天探测器（WISE）图像结合局域积分场光谱数据，对42个中红外高光度尘埃超新星（mid-IR luminous dusty supernovae）进行系统性分析，研究其宿主环境与尘埃特性，重点关注新形成尘埃或辐射加热的既有尘埃。

Result: 发现尘埃超新星(dusty supernovae)更倾向于出现在具有较高Hα等效宽度(EW(Hα))、较低金属丰度(metallicity)和较重宿主消光(host extinction)的区域，且不同尘埃超新星类型的环境特性差异表明其前身星(progenitors)具有多样性。
...
### [42] [Weak emission line quasar SDSS J101353.45+492758.1 I. Continuum fitting](https://arxiv.org/abs/2602.24217)
*Laetitia Gibaud,Marek Nikołajuk,Piotr Życki,Agata Różańska,Krzysztof Hryniewicz,Rafał Wojaczyński*

**TL;DR**: 论文核心贡献是通过多组分活动星系核（AGN）模型relagn和薄盘模型kerrbb对弱线类星体（WLQ）SDSS J101353.45+492758.1的宽波段光谱分析，发现其具有超大质量黑洞（M_{BH}≈2×10^9 M_\odot）、标准吸积盘（Eddington accretion rate≈0.1）、扩展暖冕区（34 R_g）和高度抑制的致密热冕（log(L_X/L_bol)≈-4.29），揭示了其极弱X射线辐射和高电离发射线缺失源于冕区几何结构与辐射效率的协同作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宽线弱发射类星体(WLQ) SDSS J101353.45+492758.1的极端光谱特性及其与热/温冕层物理机制的关联。

Data: 数据来源于宽波段观测，包括紫外-光学光谱（显示近乎无特征的连续谱和微弱Mg II线）和极低X射线流量数据。

Method: 采用相对论性薄盘模型kerrbb结合幂律谱及多成分活动星系核(AGN)模型relagn（包含暖、热康普顿化区域）进行光谱能量分布拟合。

Result: 黑洞质量约为2×10^9太阳质量，爱丁顿吸积率约为0.1，且热冕区能量抑制导致X射线本征微弱（log(L_X/L_bol)≈-4.29）。
...
### [43] [Shaping the Digital Future of ErUM Research: Sustainability & Ethics](https://arxiv.org/abs/2602.24087)
...
**TL;DR**: 该报告总结了2025年研讨会关于数据密集型基础研究（ErUM-Data）可持续性措施的进展，重点评估了减少碳排放、提升数据软件FAIR性（可查找、可访问、可互操作、可重用）、优化工作流程与计算基础设施的短期、中期和长期行动方案，并强调了系统性培训、专项资助机制以及将伦理考量（如人工智能应用的透明度与责任归属）融入科研实践的必要性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于数据密集型基础研究（ErUM-Data）中可持续性措施的实施与伦理挑战，特别关注资源节约型研究、碳排放监控、FAIR（可查找、可访问、可互操作、可重用）数据原则及人工智能伦理框架。

Data: 数据来源于对资源感知研究（resource-aware research）的监测和优化工作流程，包括二氧化碳排放监测、数据和软件FAIRness改进、计算基础设施优化以及与低碳能源可用性对齐的操作概念。

Method: 采用资源感知(resource-aware)方法评估并优化数据密集型研究的可持续性措施，包括碳排放监测、FAIR数据管理、工作流优化及低碳计算基础设施设计。

Result: 强调了将可持续性和伦理融入日常科研实践需结合技术措施、针对性意识培养、沟通策略、激励机制及社区驱动举措。
...
### [44] [High-Precision Mass Measurements of Proton-Rich Rh, Pd, Cd isotopes in the vicinity of 100Sn and Impact on X-Ray Burst and Supernova Nucleosynthesis](https://arxiv.org/abs/2602.23776)
...
**TL;DR**: 利用多反射飞行时间质谱仪(ZeroDegree)首次高精度测量了100Sn附近丰质子核91Rh、92Pd和96Cd的质量（不确定度约10 keV），显著改善了X射线暴模拟中A=90-100区域的核合成不确定性，揭示99Rh在νp过程中起关键作用，为丰质子核合成模型提供了关键约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高精度质量测量技术探索近双幻核100Sn的质子富集核区，以约束质子俘获核合成（proton-rich nucleosynthesis）模型并揭示X射线暴中的核过程反应路径。

Data: 使用RIKEN放射性同位素束流工厂(RIBF)的CRISMASS项目零度角多反射飞行时间质谱仪(ZeroDegree multi-reflection time-of-flight mass spectrograph)进行高精度质量测量。

Method: 采用零度角多反射飞行时间质谱仪（ZeroDegree multi-reflection time-of-flight mass spectrograph）对100Sn附近丰质子核进行高精度质量测量，精度达10 keV量级。

Result: 首次精确测定91Rh、92Pd和96Cd的质量，显著提升若干核素质量精度，为X射线暴模拟提供关键约束。
...
### [45] [Impact of non-standard neutrino-electron interactions on Big Bang Nucleosynthesis](https://arxiv.org/abs/2602.23915)
*Stefano Gariazzo,Jaume Moncho,Sergio Pastor,Ofelia Pisanti*

**TL;DR**: 该论文首次全面研究了电子中微子非标准相互作用（NSI）对宇宙等离子体中微子退耦和大爆炸核合成（BBN）轻元素产生的影响，发现BBN对非普适性和味改变NSI参数的约束虽弱于实验限制，但能互补地检验不同温度尺度和时期的中微子物理。


<details>
  <summary>Details</summary>

Background: 研究探讨了中微子非标准相互作用（NSI）对宇宙等离子体中微子退耦及大爆炸核合成（BBN）期间轻元素产生的潜在影响。

Data: 研究中使用的数据来源为数值模拟（simulation）数据，基于更新后的中微子非标准相互作用（NSI）参数空间和早期宇宙物理条件。

Method: 采用非标准相互作用（NSI）模型结合大爆炸核合成（BBN）数值模拟，更新了中微子退耦过程的分析方法。

Result: 首次全面研究了非标准相互作用（NSI）对太初核合成（BBN）期间轻元素产生的影响，发现BBN对非普适性和味改变NSI参数的约束明显弱于实验限制，但二者具有互补性并能测试不同温度尺度和时期的中微子物理。
...
### [46] [Fermion Mass Hierarchy and a High Quality Axion From Gauged U(1) Flavor Symmetry](https://arxiv.org/abs/2602.24253)
*K. S. Babu,Sai Charan Chandrasekar,Zurab Tavartkiladze*

**TL;DR**: 提出了一类基于规范U(1)_F味对称性的模型，通过Froggatt-Nielsen机制解释费米子质量层级结构，同时利用Peccei-Quinn机制解决强CP问题，并意外涌现出具有QCD反常的全局U(1)_PQ对称性，产生高质量轴子（axion），其作为味场（flavon）和暗物质候选者，在保持宇宙学域壁无问题的前提下实现了可计算的轻子生成机制和正确量级的重子不对称性。


<details>
  <summary>Details</summary>

Background: 研究通过引入规范U(1)_F味对称性（gauged U(1)_F flavor symmetry），结合Froggatt-Nielsen机制（FN mechanism）和Peccei-Quinn机制（PQ mechanism），统一解释费米子质量等级结构（hierarchical fermion masses）和强CP问题（strong CP problem），并构建了三种DFSZ轴子模型（DFSZ axion model）的推广形式。

Data: 研究中使用的数据来源于理论模型构建（simulation），包括基于规范$U(1)_F$味对称性和Peccei-Quinn（PQ）机制的三个DFSZ轴子模型推广方案，以及通过Froggatt-Nielsen（FN）机制解释费米子质量层级结构的紫外完备理论框架。

Method: 采用基于规范U(1)_F味对称性的Froggatt-Nielsen机制和Peccei-Quinn机制相结合的物理模型，通过三个DFSZ轴子模型的推广实现轴子作为flavon场的可测试预言。

Result: 基于规范U(1)_F味对称性构建的模型通过Froggatt-Nielsen机制解释费米子质量层级结构，同时利用Peccei-Quinn机制解决强CP问题，并意外涌现出具有非零QCD反常的全局U(1)_PQ对称性，产生高质量轴子（axion）。
...
### [47] [Polarisation Singularities of Gravitational Waves](https://arxiv.org/abs/2602.23425)
*Claire Rigouzzo,Sebastian Golat,Alex J. Vernon,Kyan Louisia,Eugene Lim,Francisco J. Rodriguez-Fortuno*

**TL;DR**: 该研究将偏振奇点（polarisation singularity）理论扩展到引力波和高自旋场，证明它们是引力波的普遍特征且维度取决于场自旋。


<details>
  <summary>Details</summary>

Background: 研究聚焦于波场（wave fields）中非理想平面波导致的几何结构（如偏振奇点（polarisation singularities）），并将其理论扩展至引力波（gravitational waves）和高自旋场（higher spin fields）。

Data: 数据来源于平面波干涉的数值模拟（simulation）结果。

Method: 基于电磁学描述构建引力波极化奇点（polarisation singularities）理论框架，并通过平面波干涉模拟验证其普遍性。

Result: 极化奇点（polarisation singularities）是引力波和高自旋场中普遍存在的特征，其维度取决于场的自旋属性。
...
### [48] [Photon rings, gravitational lensing, and ISCOs of exotic compact objects in Einstein-scalar-Maxwell theories](https://arxiv.org/abs/2602.23657)
*Antonio De Felice,Shinji Tsujikawa*

**TL;DR**: 在爱因斯坦-标量-麦克斯韦理论中，通过研究标量场$φ$与电磁场强度$F$的耦合形式$μ(φ)F$，首次证明了存在具有壳层结构且密度在中间半径处达到峰值的带电奇异致密天体（ECOs），并计算了其质量、半径及场分布，分析了光子环缺失条件下的参数约束、引力透镜偏转角最大值以及大质量粒子最内稳定圆轨道的参数空间。


<details>
  <summary>Details</summary>

Background: 研究探讨了爱因斯坦-标量-麦克斯韦理论（Einstein-scalar-Maxwell theories）中具有标量场（scalar field）与电磁场强度（electromagnetic field strength）耦合形式μ(φ)F的奇异致密天体（exotic compact objects, ECOs）的存在性及其在光子和有质量粒子动力学中的观测特征。

Data: 研究中使用的数据来源为静态球对称背景下的标量场和矢量场分布、光子环（photon rings）的观测约束条件以及大质量粒子最内稳定圆轨道（innermost stable circular orbits）的参数空间分析。

Method: 采用静态球对称背景下的标量场（scalar field）和矢量场（vector field）剖面分析方法，结合光子环（photon ring）稳定性约束条件，研究了带电奇异致密天体（ECOs）的壳层结构及其观测特征。

Result: 在标量场$φ$与电磁场强度$F$以$μ(φ)F$形式耦合的爱因斯坦-标量-麦克斯韦理论中，证明了存在电荷密度峰值位于中间半径的壳层结构带电奇异致密天体（ECOs）。
