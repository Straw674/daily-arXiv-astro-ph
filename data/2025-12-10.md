...
**TL;DR**: 通过三维非理想磁流体动力学模拟发现，分子云质量-磁通量比（mass-to-flux ratio）的观测值受投影效应显著影响，塞曼（Zeeman）测量会高估真实值一个量级以上，而偏振观测无法有效反映真实物理关系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于分子云质量-磁通量比（mass-to-magnetic flux ratio）的测量准确性及其在评估磁场与引力相对重要性中的核心作用。

Data: 数据来源于三维非理想磁流体动力学（MHD）化学动力学模拟，模拟对象为湍流坍缩分子云。

Method: 采用三维非理想磁流体动力学（MHD）化学动力学模拟分析湍流坍缩分子云，通过比较不同投影角度和动力学阶段的磁场-气体柱密度关系及观测值与真实值的偏差来量化质量-磁通比测量精度。

Result: 塞曼(Zeeman)观测会高估分子云的真实质量-磁通量比(mass-to-flux ratio)，尤其在磁场主要位于天空平面时偏差可达一个数量级以上，因此应将其视为上限值。
...
**TL;DR**: 研究发现超早期蓝怪物星系（Blue Monsters）中尘埃含量极低的原因是星团尺度上超新星爆发驱动的机械性喷流（mechanical venting）有效移除了大部分尘埃，其保留的尘埃-恒星质量比仅为超新星净尘埃产量的1/2至两个数量级，与观测结果一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(z≳10)极年轻星系"蓝色怪物"(Blue Monsters)中尘埃含量异常低的现象，探讨超新星驱动机械喷流在分层自引力云中清除尘埃的物理机制。

Data: 数据来源包括三维流体力学模拟（3-D hydrodynamical simulation）中年轻星团（young stellar clusters）的超新星（supernovae）尘埃存活率数据，以及三维薄壳标度（3-D thin-shell scaling）确定的喷流半径（blowout radius）与气体浓度的函数关系。

Method: 采用三维流体力学尘埃存活产额与三维薄壳标度相结合的方法，分析分层自引力云中成团超新星驱动的机械喷流机制。

Result: 通过结合三维流体力学尘埃存活产额与三维薄壳标度分析，发现星团尺度上超新星机械性喷流导致尘埃与恒星质量比降低0.5-2个数量级，主要取决于气体浓度而非金属丰度，从而解释了蓝怪物星系（Blue Monsters）极低尘埃含量的观测特征。
...
### [3] [How is cold, star-forming gas in galaxies affected by magnetic fields?](https://arxiv.org/abs/2512.07948)
...
**TL;DR**: 通过对比含磁流体动力学（MHD）与纯流体动力学（HD）的孤立恒星形成星系模拟，发现磁场通过发电机效应放大后会产生更致密的气体盘（半径约5.1千秒差距）和弥散原子包层，显著提高恒星形成所需的气体表面密度阈值，使KS关系向更高气体表面密度偏移，并降低恒星形成率（MHD模型约4.8太阳质量/年，HD模型约8.4太阳质量/年）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过数值模拟探究磁场在星系星际介质和恒星形成中的作用，特别关注未预设Kennicutt-Schmidt (KS)恒星形成定律的孤立恒星形成星系模型。

Data: 使用Arepo代码进行了两个孤立恒星形成星系的数值模拟（含磁场和无磁场），通过sink粒子模拟恒星形成过程且未强加Kennicutt-Schmidt关系。

Method: 采用Arepo数值模拟方法，结合非平衡化学网络、热演化方案和超新星反馈，通过发电机作用从小初始种子场放大磁场，并使用sink粒子模拟恒星形成而不强加Kennicutt-Schmidt关系。

Result: 磁流体动力学(MHD)模拟显示，磁场通过抑制引力坍缩提高了恒星形成所需的气体表面密度阈值，导致KS关系向更高气体表面密度偏移，与观测更一致。
...
### [4] [The dynamic central environment of NGC 3516 revealed by XRISM](https://arxiv.org/abs/2512.07950)
...
**TL;DR**: 利用XRISM观测赛弗特1.5型星系NGC 3516的铁K波段，发现其核区存在六种电离态各异、速度跨度大（+4300至-9800 km/s）的吸收成分，呈现快速演化的多相气体流动态特征，揭示了亚秒差距尺度上吸积、抛射和电离过程的紧密耦合。


<details>
  <summary>Details</summary>

Background: 研究聚焦于赛弗特1.5型星系（Seyfert 1.5 galaxy）NGC 3516核区复杂、快速演化的多相气体流动（multi-phase gas flows）及其与吸积、抛射和电离过程的耦合机制。

Data: 数据来源于XRISM对赛弗特1.5型星系（Seyfert 1.5 galaxy）NGC 3516进行的249千秒（ks）观测，时间跨度约310千秒（ks），获得了高时间分辨率的铁K波段（Fe K band）吸收光谱数据。

Method: 采用时间分辨光谱分析(time-resolved spectroscopy)技术研究Fe K波段吸收特征及其时变特性。

Result: 在赛弗特1.5型星系NGC 3516的核区环境中，XRISM观测揭示了由快速演化的多相气体流主导的复杂结构，其吸积、抛射和电离过程在亚秒差距尺度上紧密耦合。
...
### [5] [LEGA-C stellar populations scaling relations. I: Chemo-archaeological downsizing trends at z~0.7](https://arxiv.org/abs/2512.07952)
...
**TL;DR**: 基于LEGA-C光谱巡天分析了552个红移0.6<z<0.77星系的恒星族性质，发现大质量星系在60亿年前已存在局域观测到的"降序演化"趋势，并揭示了恒星年龄随质量呈双峰分布（过渡质量约10^11Msun）而金属丰度-质量关系无双峰性（在10^10.8Msun处由陡变平）的关键观测证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于中等红移(0.6<z<0.77)大质量星系(>10^10Msun)的恒星族群体性质(如光加权平均年龄和金属丰度)及其与恒星质量、速度弥散度的标度关系。

Data: 数据来源于LEGA-C（Large Early Galaxy Astrophysics Census）光谱巡天的第三次数据发布（DR3），包含552个红移范围0.6<z<0.77的星系样本，并结合了rizYJ波段测光数据。

Method: 采用贝叶斯框架结合随机恒星形成和金属丰度历史及尘埃衰减的模型光谱库，通过关键恒星吸收特征和rizYJ测光数据反演光加权平均年龄和恒星金属丰度。

Result: 在红移<z>=0.7且质量>10^10Msun的星系群体中，光加权平均年龄和恒星金属丰度与恒星质量呈现体积加权的标度关系，且局域观测到的"downsizing"趋势在60亿年前已形成。
...
### [6] [Tracing Nitrogen Enrichment across Cosmic Time with JWST](https://arxiv.org/abs/2512.07955)
...
**TL;DR**: 基于JWST/NIRSpec光谱对红移z~1-6的76个恒星形成星系氮氧比(N/O)的直接测量，首次建立了高红移强线诊断标定，发现z>1星系在固定氧丰度(O/H)下氮氧比(N/O)比本地关系高约0.18 dex（低金属丰度时达0.3-0.4 dex），其化学特征类似第一代球状星团(globular cluster)恒星，暗示可能反映了早期宇宙星团驱动富集模式的晚期阶段。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(z~1-6)恒星形成星系中氮氧丰度比(N/O)的演化及其与金属丰度(O/H)的关系，旨在揭示早期宇宙中氮元素增丰的化学演化机制及其与球状星团第一代恒星形成过程的潜在联系。

Data: 数据来源于JWST/NIRSpec光谱观测，结合MARTA巡天和高红移文献样本中76个z>1星系的微弱极光发射线（auroral emission lines）探测，以及额外430个星系的强线（strong-line）测量数据。

Method: 采用JWST/NIRSpec光谱技术分析高红移(z~1-6)恒星形成星系中氮氧丰度比(N/O)，基于76个星系的光谱数据通过静止波段光学谱线直接测量电子温度并推导氮氧丰度。

Result: 基于JWST/NIRSpec光谱数据，研究发现高红移(z>1)星系在固定氧丰度(O/H)下氮氧比(N/O)比本地关系高约0.18 dex(中位偏移)，低金属丰度(12+log(O/H)<8.1)时偏移达0.3-0.4 dex。
...
### [7] [Forecasting Dark Matter Subhalo Constraints from Stellar Streams using Implicit Likelihood Inference](https://arxiv.org/abs/2512.07960)
...
**TL;DR**: 该研究通过神经后验估计（NPE）方法，利用ATLAS-Aliqa Uma星流运动学数据约束暗物质（DM）子晕参数（质量、尺度半径、速度和相遇几何），在理想6D坐标下实现$10^7 \, \mathrm{M_\odot}$子晕质量15-20%的测量精度，并发现速度双峰性在相空间采样不足时出现，结合测光与光谱观测可有效消除该简并。


<details>
  <summary>Details</summary>

Background: 研究主题为利用恒星流运动学探测暗物质（DM）子晕（subhalos）以揭示暗物质粒子性质。

Data: 数据来源包括基于拉格朗日云剥离技术（Lagrange Cloud stripping technique）生成的粒子喷射模拟（particle spray simulations），并以ATLAS-Aliqa Uma星流作为测试案例，同时结合未来巡天数据（LSST、4MOST和10年盖亚数据）进行预测分析。

Method: 采用神经后验估计（NPE）方法结合拉格朗日云剥离技术生成的粒子喷雾模拟，约束暗物质子晕（subhalo）的质量、尺度半径、速度和相遇几何参数。

Result: 通过神经后验估计(NPE)方法分析恒星流运动学数据，能够准确约束暗物质(DM)子晕(subhalo)的质量、尺度半径、速度和相遇几何参数，在理想6D坐标下对$10^7 \, \mathrm{M_\odot}$子晕的质量不确定度达15-20%，实际观测条件下不确定度从当前50%改善至未来20-40%，并发现相位空间采样不足会导致速度双峰性(bimodality)现象。
...
### [8] [Probing Infrared eXcess to Investigate Early-Universe Dust (PIXIEDust)](https://arxiv.org/abs/2512.07964)
...
**TL;DR**: 通过对10个红移(z)大于8的星系进行ALMA和NOEMA观测，未检测到显著亚毫米波(rest-frame 158, 88 μm)尘埃发射，将平均尘埃质量(dust mass)限制在<10^5 M⊙、尘埃与恒星质量比(dust-to-stellar mass ratio)限制在<3.7×10^-4，表明高红移(z>8)宇宙中尘埃积累效率低下。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(z>8)星系中尘埃(dust)形成效率低下的宇宙学背景，探讨其可能成因包括超新星(supernova)产量不足、星际尘埃颗粒(interstellar grain)生长受限或外流(outflow)导致的尘埃抛射。

Data: 数据来源于约两百小时的阿塔卡马大型毫米波/亚毫米波阵列(ALMA)和北方扩展毫米波阵列(NOEMA)对十个红移(z)大于8的星系的观测。

Method: 采用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)和北方扩展毫米波阵列(NOEMA)的观测数据，结合尘埃温度(T_dust=50K)和尘埃发射谱指数(β_dust=2.0)的物理模型，对高红移(z>8)星系进行尘埃质量上限约束分析。

Result: 在红移(z>8)星系中未检测到显著尘埃发射(emission)，约束平均尘埃质量(dust mass)低于10^5 M⊙且尘埃-恒星质量比(dust-to-stellar mass ratio)低于3.7×10^-4，表明高红移宇宙中尘埃积累效率低下。
...
### [9] [Danger Zone: Establishing Buffers for Enhanced Classification in BPT Diagrams](https://arxiv.org/abs/2512.07975)
*Changhyun Cho,Ahmad Nemer,Ivan Yu. Katkov,Joseph D. Gelfand*

**TL;DR**: 采用无监督机器学习（UMAP算法）对MaNGA巡天中143.9万条恒星形成区、赛弗特星系和低电离（核）发射线区（LI(N)ERs）光谱进行精确分类，解决了传统诊断图10%光谱分类模糊问题，并识别出模糊光谱中的物理相关子集。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用无监督机器学习方法（UMAP算法）改进基于发射线比（emission line ratios）的星系电离源（ionization source）分类，解决传统诊断图中光谱分类边界模糊（$\sim10$\%光谱类别不一致）的问题。

Data: 数据来源于MaNGA巡天项目观测的6439个星系中约130万条光学光谱。

Method: 采用无监督机器学习中的均匀流形逼近与投影算法（UMAP）对光谱线比（line ratios）进行分类分析。

Result: 基于无监督机器学习（UMAP）对MaNGA巡天中约130万条光谱的分类，明确了传统诊断图边界模糊区域的光谱类别，并识别出其中具有物理意义的子集。
...
### [10] [SAGAN-VI: When Jets Meet Filaments -- Environmental Imprints on the Growth of Giant Radio Galaxies](https://arxiv.org/abs/2512.07985)
*Mousumi Mahato,Elmo Tempel,Shishir Sankhyayan,Pratik Dabhade,Kshitij Chavan*

**TL;DR**: 巨型射电星系（GRG）的成长与其射电喷流相对于宇宙细丝（filament）的排列角度相关，而非与细丝距离相关，且GRG比小型射电星系（SRG）表现出更强的形态不对称性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于巨型射电星系(GRGs)在宇宙大尺度环境（特别是丝状结构）中的形成机制及其形态演化。

Data: 数据来源于SDSS（Sloan Digital Sky Survey）的纤维结构目录和当前可用的最大巨型射电星系（GRG）目录。

Method: 采用斯隆数字巡天(SDSS)的纤维结构(filament)目录结合最大巨型射电星系(GRG)样本，测量邻近纤维结构脊线的共动距离、射电喷流投影取向角及通过臂长比(ALR)量化形态不对称性，并通过自助重采样(bootstrap resampling)和非参数统计检验验证结果。

Result: 巨型射电星系(GRG)的巨型尺度形成与其喷流相对于宇宙细丝(filament)的排列角度相关，而非与细丝邻近度有关。
...
### [11] [The Drivers of the Decline in Supermassive Black Hole Growth at $z<2$](https://arxiv.org/abs/2512.08004)
*Zhibo Yu,W. N. Brandt,Fan Zou,Bin Luo,Qingling Ni,D. P. Schneider,Fabio Vito*

**TL;DR**: 研究发现宇宙超大质量黑洞（SMBH）增长在红移$z\approx1.5-2$达到峰值后急剧下降的主要驱动因素是活动星系核（AGN）的爱丁顿比（$λ_\mathrm{Edd}$）普遍降低（下降约1.35 dex），而非吸积活动向低质量黑洞转移。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙超大质量黑洞（SMBH）吸积活动的演化规律及其主导机制，特别是红移（redshift）下降过程中爱丁顿比（Eddington ratio, λ_Edd）变化对SMBH增长整体下降的关键作用。

Data: 数据来源于九个特征良好的外场（extragalactic fields）采用“婚礼蛋糕”（"wedding-cake"）设计的观测数据。

Method: 基于九层"婚礼蛋糕"（wedding-cake）结构设计的河外星系场数据，采用爱丁顿比（Eddington ratio）演化分析量化超大质量黑洞（SMBH）吸积率下降的主导因素。

Result: 爱丁顿比（Eddington ratio, λ_Edd）的显著下降（从z≈1.5-2到z≈0.2减少约1.35 dex）是宇宙超大质量黑洞（SMBH）增长广泛衰退的主要驱动因素。
...
### [12] [The GLEAM 4-Jy (G4Jy) Sample: IV. Multiwavelength data and analysis](https://arxiv.org/abs/2512.08008)
...
**TL;DR**: 该论文更新了G4Jy星表的多波段版本，新增127个宿主星系（host-galaxy）识别，并利用DESI Legacy Surveys的$griz$测光数据补充红移信息（0.0 < z < 3.6），首次在文献中提出基于射电谱曲率指数（SCI）的射电功率-尺寸-年龄关系图，揭示了候选残余射电星系（SCI > 0.15）在D < 200 kpc的优势分布以及候选重启射电星系（SCI < -0.15）的广泛线尺寸范围。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用多波段（multiwavelength）数据探究南天射电亮星系（southern radio-bright galaxies）的物理特性，包括射电谱曲率（radio spectral-curvature）与射电源年龄的关系及其宿主星系性质。

Data: 数据来源包括G4Jy星表（https://github.com/svw26/G4jy、https://zenodo.org/communities/g4jy/records及VizieR平台）、DESI Legacy Surveys的DR10发布的$griz$测光数据以及151MHz流量密度大于4 Jy的完整样本的射电频谱曲率分析。

Method: 采用多波段（multiwavelength）分析方法，结合DESI Legacy Surveys的$griz$测光数据和射电谱曲率（radio spectral-curvature）指标（SCI），构建了射电功率-尺寸-年龄（P--D--SCI）关系图。

Result: 首次在文献中提出了射电功率-尺寸-年龄图[P--D](SCI)，发现候选残余射电星系(SCI > 0.15)主要分布在D < 200 kpc区域，而候选重启射电星系(SCI < -0.15)则表现出广泛的线性尺寸分布。
...
**TL;DR**: 通过JWST观测发现高红移（z=6.2）强透镜星系MACS0308-zD1中存在三个极端致密（有效半径约5 pc）、年轻（5-9 Myr）但动力学稳定的星团，其恒星质量（10^6-10^7 M⊙）和表面密度（Σ* > 10^4 M⊙ pc^-2）远超本地星团，且最亮星暴区中心存在电离光子产生效率（ξ_ion）局部抑制现象。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（$z>6$）强引力透镜星系MACS0308$-$zD1（“宇宙之矛”）中极端致密星团（star clusters）的恒星形成活动及其物理特性。

Data: 数据来源为JWST（James Webb Space Telescope）的NIRCam（近红外相机）成像和NIRSpec IFU（近红外光谱仪积分视场单元）高分辨率光谱观测，覆盖了静止紫外至光学波段。

Method: 采用JWST近红外相机（NIRCam）成像和近红外光谱仪（NIRSpec）积分视场单元（IFU）光谱技术，对引力透镜放大的高红移星系MACS0308-zD1进行空间分辨的恒星族分析。

Result: 在MACS0308-zD1（Cosmic Spear）中发现了三个极端致密的年轻星团（star clusters），其有效半径（Reff）约5 pc、恒星质量（M*）约10^6-10^7 M⊙、恒星质量面密度（Σ*）超过10^4 M⊙ pc^-2，动力学年龄超过1且引力束缚，显著高于本地星团的质量和密度。
...
### [14] [The Two-Infall Model Revisited: Constraints on Milky Way Bulge Assembly from >30,000 Galactic Chemical Evolution Models and Machine Learning](https://arxiv.org/abs/2512.08090)
*Niall Miller,Meridith Joyce,Christian I. Johnson,Jamie Tayar,Thomas Trueman,R Michael Rich*

**TL;DR**: 基于双吸积（two-infall）银河化学演化（GCE）模型，研究发现银河系核球（bulge）主要由早期快速恒星形成（t1~0.1Gyr，tau1~0.09Gyr，恒星形成效率SFE~3Gyr^-1）和后期低质量延迟吸积（t2~5.1Gyr，tau2~1.7Gyr，sigma2~0.69）共同形成，支持复合核球起源（composite bulge origin）并再现了金属丰度分布函数（MDF）和[α/Fe]-[Fe/H]趋势。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用双吸积星系化学演化(GCE)模型约束银河系核球(bulge)的形成历史。

Data: 数据来源于现代恒星网格（modern stellar grids）的质量和金属丰度依赖的核合成产额（nucleosynthetic yields），并通过混合遗传算法（hybrid genetic algorithm）和马尔可夫链蒙特卡洛（MCMC）优化方法探索了广泛的银河化学演化（GCE）参数空间。

Method: 采用双吸积星系化学演化(GCE)框架结合OMEGA++代码，通过混合遗传算法和马尔可夫链蒙特卡洛(MCMC)优化，探索吸积时序、恒星形成效率(SFE)、质量分配、初始质量函数(IMF)上限质量和Ia型超新星(SN Ia)归一化等参数空间。

Result: 银河系核球（bulge）的形成历史符合两阶段吸积模型，早期快速恒星形成暴（t1 ~ 0.1Gyr，tau1 ~ 0.09Gyr，恒星形成效率SFE ~ 3Gyr^-1）主导了大部分质量，而后期延迟的低质量二次吸积（t2 ~ 5.1Gyr，tau2 ~ 1.7Gyr，sigma2 ~ 0.69）解释了金属丰度分布函数（MDF）的富金属峰和[α/Fe]随[Fe/H]的下降趋势。
...
### [15] [The MAGPI Survey: forward modelled gas-phase metallicity gradients in galaxies at $z\sim 0.3$](https://arxiv.org/abs/2512.08112)
...
**TL;DR**: 研究测量了70个红移约0.3的MAGPI星系样本的视宁解卷积气体金属丰度梯度（∇[O/H]），发现其与恒星质量（M*）和气体速度弥散的关系支持理论预测，即恒星反馈、气体输运和吸积共同塑造金属丰度分布，且主导过程随星系质量变化。


<details>
  <summary>Details</summary>

Background: 研究主题为探索近宇宙（z~0.3）中恒星形成星系（star-forming galaxies）的气相金属丰度梯度（gas-phase metallicity gradients）与星系性质的关系，以揭示金属元素分布机制及其对星系演化的影响。

Data: 数据来源于MAGPI巡天（MAGPI survey）观测的70个红移$z\sim 0.3$的恒星形成星系的气相金属丰度梯度测量。

Method: 采用贝叶斯建模技术Blobby3D，该方法考虑了视宁效应（beam smearing）并能对通量分布的亚结构进行建模。

Result: 样本中70个恒星形成星系的中值金属丰度梯度（metallicity gradient）为$\nabla \mathrm{[O/H]}=-0.013^{+0.059}_{-0.033}$ dex/kpc，其中32.9%呈显著负梯度，10.0%呈正梯度，57.1%呈平坦梯度。
...
### [16] [Supermassive Black Holes with High Accretion Rates in Active Galactic Nuclei. XV. Reverberation Mapping of Mg II Emission Lines](https://arxiv.org/abs/2512.08192)
...
**TL;DR**: 通过监测18个超爱丁顿吸积活动星系核(AGN)中Mg II线的反响映射(RM)数据，成功测定了其中8个目标的Mg II时间延迟，更新了Mg II半径与3000 $\mathring{\mathrm{A}}$单色光度关系($R_{\rm MgII}-L_{3000}$)的斜率(0.24±0.03)，并发现超爱丁顿吸积AGN的Mg II延迟时间短于正常吸积率AGN，且偏离关系程度与吸积率显著相关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超大质量黑洞（SMBH）在超爱丁顿吸积（SEAMBH）状态下的宽线区（BLR）结构特征及其与吸积率的关系，特别关注Mg II发射线的反响映射（RM）测量对中心黑洞质量估算和半径-光度（$R_{\rm MgII}-L_{3000}$）关系的影响。

Data: 数据来源于2017至2024年间对18个超爱丁顿吸积率超大质量黑洞（SEAMBHs）进行的光谱监测，成功测定了其中8个对象的Mg II发射线时间延迟。

Method: 采用光谱学监测的回响映射（reverberation mapping）技术测量Mg II线的时间延迟，并结合线宽测定中心超大质量黑洞（supermassive black holes）质量。

Result: 通过测量Mg II发射线的时间延迟（time lag）和线宽，确定了8个超爱丁顿吸积率活动星系核（SEAMBHs）中心超大质量黑洞（supermassive black holes）的质量，并更新了Mg II半径与3000 $\mathring{\mathrm{A}}$单色光度关系（$R_{\rm MgII}-L_{3000}$ relation）的斜率为$0.24 \pm 0.03$。
...
### [17] [The Milky Way Imaging Scroll Painting Survey: Data Release 1](https://arxiv.org/abs/2512.08260)
...
**TL;DR**: 首次发布银河北部区域9.75°≤l≤229.75°、|b|≤5.25°范围内12CO、13CO和C18O分子谱线的高灵敏度巡天数据（MWISP DR1），基于DBSCAN算法识别出103,517个分子云并测量了总12CO流量。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用银道面成像卷轴巡天(MWISP)首次数据发布(DR1)，绘制北银道面12CO、13CO和C18O的J=(1-0)跃迁线分布图，以揭示银河系北部分子气体的分布与性质。

Data: 数据来源于2011-2022年使用PMO 13.7米望远镜对银纬|b|≤5.25度、银经9.75度≤l≤229.75度范围内2310平方度区域进行的MWISP巡天观测，覆盖了12CO、13CO和C18O的J=(1-0)跃迁谱线，采用OTF（On-The-Fly）扫描模式将观测区域划分为30'×30'的单元网格，最终数据重采样为像素尺寸30"×30"×0.16 km/s的l-b-V_LSR三维数据立方体。

Method: 采用PMO 13.7米射电望远镜进行J=(1-0)跃迁谱线（12CO、13CO、C18O）的On-The-Fly（OTF）扫描观测，数据经校准、清洗和拼接后重网格化为像素尺寸30"×30"×0.16 km/s的l-b-V_LSR三维数据立方体。

Result: 首次发布覆盖银经9.75至229.75度、银纬±5.25度范围内2310平方度区域的12CO、13CO和C18O分子谱线高灵敏度巡天数据，空间分辨率约50角秒，速度分辨率0.16 km/s。
...
### [18] [Chemical analysis of the Milky Way's Nuclear Star Cluster: Evidence for a metallicity gradient](https://arxiv.org/abs/2512.08387)
*M. Schultheis,L. Serrano,B. Thorsbro,F. Nogueras-Lara,A. Feldmeier-Krause,G. Nandakumar,K. Fiteni,M. C. Sormani*

**TL;DR**: 通过改进光谱分析技术对银河系核星团（MWNSC）中1140颗M巨星进行精确测量，发现其包含贫金属（[M/H]≈-0.7 dex）和富金属（[M/H]≈0.26 dex）两个主要群体，并基于-0.1±0.02 dex/pc的金属丰度梯度支持核星团由内向外形成（inside-out formation）的演化模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系核星团（MWNSC）及其周围核星盘（MWNSD）的形成演化机制及其潜在关联性，旨在揭示银河系中心300秒差距内引力势主导结构的起源。

Data: 数据来源为低分辨率KMOS光谱仪（KMOS spectra）和高分辨率红外光谱仪IGRINS（IGRINS spectra）。

Method: 采用改进的谱线列表（line-list）分析低分辨率KMOS光谱，重点优化冷M巨星（cool M giants）的恒星参数（$\rm T_{eff}$、$\rm \log\,g$、$\rm [M/H]$）并获取全局α元素（$\rm α$-elements）丰度。

Result: 银河系核星团（MWNSC）中存在显著贫金属（[M/H]≈-0.7 dex）和富金属（[M/H]≈0.26 dex）两个恒星群体，其中贫金属占比约20%，并发现金属丰度梯度为-0.1±0.02 dex/pc，支持核星团由内向外形成（inside-out formation）的演化模型。
...
**TL;DR**: 这篇论文的核心贡献是综述了星际尘埃（interstellar dust）及其冰幔（ice mantles）的光化学和热过程研究，揭示了这些过程如何促成复杂有机分子（COMs）的形成及其与彗星、小行星中分子的关联，进而可能影响地球早期生命前化学反应。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是星际介质中尘埃颗粒及其冰幔的光化学（photochemistry）和热力学过程对复杂有机分子（COMs）形成的影响及其在生命前化学（prebiotic chemistry）中的潜在作用。

Data: 数据来源于实验室模拟星际尘埃（dust particles）和冰幔（ice mantles）中的辐射与热驱动过程，以及观测到的星际冰（interstellar ice）中简单分子和复杂有机分子（COMs）的组成。

Method: method：采用实验室模拟辐射和热驱动过程（radiation and thermally-driven processes）研究星际尘埃（dust particles）及冰幔（ice mantles）中的光化学（photochemistry）和热处理（thermal processing）机制。

Result: 星际尘埃（interstellar dust）颗粒表面冰幔的光化学和热过程促进了复杂有机分子（COMs）的形成。
...
### [20] [Collisional rate coefficients for OH-H$_2$ at high temperatures](https://arxiv.org/abs/2512.08420)
*Zeno van den Heuvel,Benoît Tabone,Ewine F. van Dishoeck,Gerrit C. Groenenboom,Ad van der Avoird*

**TL;DR**: 该研究通过量子散射计算获得了OH分子与H$_2$（包括正氢和仲氢）在更高能级（最高$j=15/2$）和温度（最高750 K）下的碰撞速率系数，发现低$j_{\rm OH}$能级布居在恒星形成区典型环境中主要由碰撞主导。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际和星周介质化学中的关键分子羟基(OH)的激发过程，旨在完善其与氢分子(H2)碰撞的速率系数数据。

Data: 数据来源于通过收敛良好的闭耦合量子散射计算（close-coupling quantum scattering calculations）获得的OH与正氢（para-H$_2$）和仲氢（ortho-H$_2$）碰撞的态-态碰撞截面，能量范围覆盖至1700 cm$^{-1}$（约2450 K）。

Method: 采用基于量子散射理论（quantum scattering theory）的收敛闭合耦合（close-coupling）计算方法，计算了OH与正氢（ortho-H$_2$）和仲氢（para-H$_2$）碰撞的态-态截面（state-to-state cross sections），并推导出温度达750 K、OH转动能级（rotational levels）至$j=15/2$的碰撞速率系数（collisional rate coefficients）。

Result: 通过高精度量子散射计算扩展了OH与H$_2$的碰撞速率系数至j=15/2能级和750 K温度，发现低j态OH在恒星形成区暖密气体中的布居主要由碰撞主导。
...
### [21] [Euclid Quick Data Release (Q1): Euclid spectroscopy of QSOs. 1. Identification and redshift determination of 3500 bright QSOs](https://arxiv.org/abs/2512.08803)
...
**TL;DR**: 基于欧几里得(Euclid)卫星NISP无缝光谱数据，结合盖亚(Gaia)和WISE高纯度候选体筛选，首次构建了约3500个可靠红移(0<z≲4.8)的明亮类星体样本，并创建了无大气吸收线的紫外至近红外(NUV-NIR)复合光谱，揭示了不同红移区间宿主星系形态特征，为未来活动星系核(AGN)普查提供了关键基准。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用欧几里得(Euclid)卫星的无狭缝光谱(NISP)模式高效获取大视场(FoV)内天体的光谱数据，旨在通过结合盖亚(Gaia)和广域红外巡天探测器(WISE)的高纯度候选体筛选与NISP光谱，构建高质量类星体样本以推进活动星系核(AGN)普查和光谱分析。

Data: 数据来源为欧几里得卫星(Euclid)快速数据发布(Q1)中的无缝光谱(NISP slitless spectroscopy)数据，结合了盖亚(Gaia)和广域红外巡天探测器(WISE)的高纯度候选体筛选结果。

Method: 通过结合盖亚(Gaia)和广域红外巡天探测器(WISE)的高纯度候选体筛选与欧几里得(Euclid)近红外光谱仪(NISP)的无缝光谱数据，并辅以人工目视检查，实现了高可靠性类星体红移测量。

Result: 通过结合盖亚(Gaia)和广域红外巡天探测器(WISE)的高纯度候选体筛选与近红外成像光谱仪(NISP)光谱，从欧几里得(Euclid)快速数据发布(Q1)中识别出约3500个红移可靠($0<z\lesssim 4.8$)的类星体，并首次生成了无大气吸收线的静止坐标系下近紫外至近红外波段类星体复合光谱。
...
### [22] [Globular Cluster Systems in Dwarf Galaxies: Catalogs and Comparisons](https://arxiv.org/abs/2512.08453)
*Veronika Dornan,William E. Harris*

**TL;DR**: 研究发现矮星系（dwarf galaxies）中非经典类型（如超弥散星系和极低表面亮度星系）的球状星团系统质量（GCS mass）与晕质量（halo mass）关系存在系统性正偏移，主要由其较高的球状星团比频（specific frequency）导致。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系总球状星团系统(GCS)质量与晕质量之间的线性标度关系，特别关注低质量星系中该关系的偏离现象。

Data: 研究使用的数据来源为已发表文献中矮星系（dwarf galaxies）的恒星质量和球状星团（globular cluster）数量估计值，经标准化转换得到球状星团系统（GCS）质量和峰值晕质量（peak halo mass）。

Method: 采用标准转换方法将矮星系（dwarf galaxies）的球状星团系统（GCS）质量和峰值晕质量（peak halo mass）估计值统一化，实现一致比较并研究$M_{GCS}-M_h$关系行为。

Result: 非经典矮星系（ultra diffuse galaxies和extremely low-surface brightness galaxies）的球状星团系统（GCS）比经典矮星系具有更高的特定频率，导致其在$M_{GCS}-M_h$关系中呈现系统性正向偏移。
...
### [23] [J-PAS: First Identification, Physical Properties and Ionization Efficiency of Extreme Emission Line Galaxies](https://arxiv.org/abs/2512.08484)
...
**TL;DR**: 论文提出了一种在J-PAS巡天中通过窄波段等效宽度结合机器学习技术光度学筛选极端发射线星系(EELGs)的高效方法，在红移z≤0.8范围内识别出917个样本，证实其电离效率(ξ_ion)与[OIII]等效宽度强相关且多数满足宇宙再电离所需阈值，为研究早期星系提供了可靠的本地对照样本。


<details>
  <summary>Details</summary>

Background: 极端发射线星系（EELGs）是研究剧烈恒星形成和早期宇宙再电离源的关键示踪体。

Data: 数据来源于J-PAS巡天提供的56个光学波段覆盖8500平方度的观测数据，并利用已完全观测的30平方度区域结合窄波段等值宽度和机器学习技术筛选发射线超过300埃的星系。

Method: 采用窄波段等效宽度测量结合机器学习技术，在J-PAS巡天的56个光学波段数据中筛选发射线等效宽度超过300埃的极端发射线星系(EELGs)。

Result: 极端发射线星系（EELGs）的强[OIII]等效宽度与ξ_ion显著相关，且多数源超过再电离所需阈值，支持其作为早期宇宙星系的本地类似体。
...
### [24] [Measuring the diffuse Galactic synchrotron spectral index and curvature between 45 and 2300 MHz](https://arxiv.org/abs/2512.08522)
*Melis O. Irfan,Giuseppe Puglisi*

**TL;DR**: 通过结合多源部分天区实测数据，构建了45-2300 MHz全天空同步辐射谱指数和曲率（synchrotron spectral index and curvature）的1度分辨率地图，采用最小二乘参数拟合和组分分离技术，新模型在验证中展现出20%的平均精度优势（相比其他模型的10-70%），显著提升了银河系同步辐射建模的可靠性。


<details>
  <summary>Details</summary>

Background: 研究银河系同步辐射(synchrotron emission)的频谱特性及其在全天分布。

Data: 数据来源于多个部分天区的经验测量（empirical measurements）组合，覆盖45至2300 MHz频段，分辨率为1度。

Method: 采用最小二乘参数拟合结合自由-自由辐射模板去除及同步辐射与自由-自由辐射成分分离技术。

Result: 基于最小二乘参数拟合和组分分离技术构建的全天同步辐射(synchrotron)谱指数和曲率地图在45-2300 MHz频段展现出约20%的平均精度，显著优于现有模型10-70%的精度范围。
...
### [25] [Investigating ionising sources and the complex interstellar medium of GHZ2 at $z=12.3$](https://arxiv.org/abs/2512.08490)
...
**TL;DR**: 通过深度NIRSpec PRISM观测和多重电离区建模，发现高红移(z=12.3)星系GHZ2具有超太阳丰度氮氧比(N/O≈2倍太阳值)、分层星际介质(电子密度log(ne/cm−3)≳4的高密区与中低密区共存)以及存在活动星系核(AGN)或硬电离源的混合电离机制，挑战了纯恒星形成辐射边界模型的解释能力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙黎明（cosmic dawn）时期星系物理性质的精确表征，以揭示红移（redshift, z）≥10时紫外（UV）明亮星系高丰度的起源机制。

Data: data来源包括NIRSpec PRISM对GHZ2的深度观测数据、MIRI和ALMA提供的远红外及光学约束数据。

Method: 采用HOMERUN代码进行多区光致电离（photoionisation）建模，结合NIRSpec PRISM观测数据与MIRI、ALMA的光学及远红外约束，揭示了GHZ2星系中低/中密度气体与高密度区域（log($n_e$/cm$^{-3}) \gtrsim 4）共存的强分层星际介质（ISM）结构。

Result: GHZ2星系在红移12.3处展现出高度电离特征，其氮氧比（N/O ratio）约为太阳值的2倍，且存在活跃核（active nucleus）导致的发射线变异性。
...
### [26] [High-Redshift Galactic Outflows: Orientation Effects, Kinematics, and Metallicity in TNG50 and SERRA](https://arxiv.org/abs/2512.08543)
*Ivan Kostyuk,Stefano Carniani,Mahsa Kohandel,Andrea Pallottini*

**TL;DR**: 通过对比TNG50和SERRA宇宙学模拟与JWST/JADES观测数据，发现模拟预测的星系外流(Outflow)质量与观测基本吻合（相差约0.5dex），但外流速度比观测值低一个数量级，且面朝(Face-on)星系的外流探测概率比侧向(Edge-on)系统高15%（盘状星系达40%）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(z>3)低质量星系中暖电离外流(warm ionised outflows)的观测与模拟差异问题，旨在揭示早期宇宙恒星形成星系普遍存在高速外流气体的理论预期与JWST/NIRSpec实测发生率(25-40%)之间的矛盾。

Data: 数据来源于TNG50宇宙学模拟（约60000个星系，红移范围z=3-5，恒星质量范围Mstar=10^7.5-10^11Msun）和SERRA放大模拟（约3000个星系，红移范围z=4-5）。

Method: 采用高斯混合模型算法（Gaussian mixture model algorithm）结合气体速度、恒星形成率和位置作为输入参数，识别并分析星系附近的流出气体。

Result: 模拟结果显示TNG50和SERRA中的外流质量与JWST/JADES观测值大致吻合（误差约0.5dex），但模拟预测值略高，表明光学发射线仅捕获了多相外流的一部分。
...
### [27] [HI-detected Dwarf Galaxies in the FASHI Survey: Insights from Single- and Double-Peaked Emission-Line Samples](https://arxiv.org/abs/2512.08644)
*Cheng Cheng,Jia-Sheng Huang,Wei Du,Hong-Xin Zhang,Chuan-Peng Zhang,Ming Zhu,Gustavo Orellana*

**TL;DR**: 通过FAST全天空中性氢巡天(FASHI)项目发现低中性氢质量矮星系($M_{\rm HI} < 10^8 M_\odot$)具有与矮椭圆星系(dE)相近的恒星质量密度，且单峰谱线源表现出与椭圆星系相似的动力学状态，揭示了不同矮星系群体间可能的演化联系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低中性氢质量矮星系（low HI mass dwarf galaxies）的动力学特性及其与不同矮星族群（如矮椭圆星系dE、超弥散星系ultradiffuse galaxies）的结构演化关联。

Data: 数据来源于FAST全天空中性氢巡天项目(FASHI)和暗能量相机遗产巡天(DECaLS)。

Method: 利用DECaLS巡天数据对低氢质量矮星系进行精确测光分析，并基于氢谱线轮廓（HI spectral-line profiles）分类研究其动力学状态。

Result: 低氢质量矮星系（low HI mass dwarf galaxies）的恒星质量密度与矮椭圆星系（dE）相似，但低于氢选择低表面亮度星系（HI-selected low-surface-brightness galaxies），表明这些星系群体间可能存在演化联系。
...
### [28] [Predicting Quasar Counts Detectable in the LSST Survey](https://arxiv.org/abs/2512.08654)
...
**TL;DR**: 基于LSST（Legacy Survey of Space and Time）模拟数据预测了不同波段（特别是i波段和u波段）可探测的类星体（quasar）和低光度活动星系核（AGN）数量，发现u波段策略变化对类星体探测数量影响最大（达15%），而其他策略变化影响低于2%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用维拉·鲁宾天文台的时空遗产巡天（LSST）项目，通过多波段宽视场巡天提升对河外源（extragalactic sources）的理解。

Data: 数据来源于鲁宾天文台（Vera C. Rubin Observatory）使用LSST Operations Simulator生成的最新模拟数据（baseline strategy v4.3.1）。

Method: 采用LSST操作模拟器(LSST Operations Simulator)生成的最新模拟数据，评估不同巡天策略对类星体探测数量的影响。

Result: 在红移范围z=0.3-6.7内，基线策略(v4.3.1)探测到的类星体和低光度活动星系核(AGN)数量在i波段最高、u波段最低。
...
### [29] [Predicting Infall Time of Milky-Way Satellites via Machine Learning](https://arxiv.org/abs/2512.08792)
*Seungyeon Kim,Myoungwon Jeon,Seongjun Hyung*

**TL;DR**: 提出了一种基于LightGBM的快速可解释方法，利用τ90、[Fe/H]和M⋆特征预测矮星系(dwarf galaxies)的宿主星系(Milky Way-like)吸积时间(infall time)，在A-SLOTH模拟数据中取得5.04均方误差，并发现首次吸积时间(earliest infall)对卫星星系熄灭过程(quenching process)具有关键影响。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于矮星系（dwarf galaxies）的吸积时间（infall time）对理解等级宇宙（hierarchical universe）中星系形成与演化的关键作用。

Data: 数据来源于A-SLOTH（一个基于观测约束校准的半解析模型）生成的30个类似银河系（MW）宿主星系的卫星星系数据，观测约束包括银河系及其卫星的观测数据。

Method: 采用LightGBM（梯度提升决策树算法）训练模型，以τ90、[Fe/H]和M⋆为输入特征预测矮卫星星系的落入时间（infall time）。

Result: 基于LightGBM算法开发了一种快速且可解释的方法，用于预测矮星系(dwarf galaxies)的宿主星系(infall)时间，在A-SLOTH数据集上实现了5.04的平均均方误差(MSE)。
...
### [30] [FIRESTORM I: Stellar Feedback and Gas Kinematics in the Evolved W40 Hub-Filament System](https://arxiv.org/abs/2512.08811)
...
**TL;DR**: 通过分析W40区域的多波段观测数据，发现六个速度相干纤维（filament）形成枢纽-纤维系统（HFS），其中五个纤维（\vlsr $\sim$\,7.5\kms）和冷前景云相互作用产生吸收特征，亚声速团块（clump）与超声速纤维的湍动差异表明能量耗散，且位置-速度空间的桥状特征与稠密凝聚体及射电连续谱峰的空间重合暗示云-云碰撞触发了HFS和大质量星团形成。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于恒星反馈（stellar feedback）对恒星形成的影响，通过分析分子气体示踪剂（dense-gas tracers）揭示恒星形成区（star-forming region）W40的动力学和结构特征。

Data: 数据来源包括致密气体示踪剂C$^{18}$O(1--0)和H$^{13}$CO$^+$(1--0)、N$_2$H$^+$(1--0)追踪的稠密团块、HCN(1--0)谱线以及射电连续谱峰值。

Method: 采用致密气体示踪剂C$^{18}$O(1--0)和H$^{13}$CO$^+$(1--0)进行速度相干纤维结构识别，并结合N$_2$H$^+$(1--0)追踪致密团块，通过多分子谱线分析（包括HCN(1--0)吸收特征）和位置-速度空间桥接特征解析，量化恒星反馈对星形成的影响。

Result: 在W40区域发现了一个由四条速度相干纤维（velocity-coherent filaments）汇聚形成的枢纽-纤维系统（hub-filament system, HFS），其与中心大质量星团（IRS 1A South, O9.5V）的形成可能由过去的云-云碰撞（cloud-cloud collision）触发。
...
### [31] [Evidence for evolving Dark Energy from a new cosmic probe](https://arxiv.org/abs/2512.07931)
*Isaque Dutra,Colin J. Burke,Priyamvada Natarajan,Weixiang Yu*

**TL;DR**: 基于6992个活动星系核(AGN)的多波段光变曲线分析，发现其变幅与特征时标同本征光度存在紧密关系，据此构建的AGN哈勃图在红移z~3.5范围内与超新星数据联合推断，以3.8-4.8σ置信度揭示了暗能量状态方程(equation of state)演化证据，为检验ΛCDM模型偏离提供了新方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于哈勃张力（Hubble tension）引发的标准宇宙学模型（ΛCDM）与新物理（如暗能量状态方程演化）之间的潜在冲突。

Data: 数据来源于6992个活动星系核（AGN）的20年多波段光学光变曲线（multi-band optical light curves）观测数据。

Method: 基于6992个活动星系核（AGN）的20年多波段光学光变曲线，建立了变幅与特征时标和本征光度之间的紧密关系，并据此构建了红移至3.5的AGN哈勃图。

Result: 基于6992个活动星系核(AGN)的多波段光学光变曲线分析，发现其变幅与特征时标和本征光度之间存在紧密关系，据此构建的AGN哈勃图在3.8-3.9σ水平支持暗能量状态方程(equation of state)存在演化，显著偏离标准ΛCDM模型(4.4-4.8σ)。
...
### [32] [Probing the Warm Dark Matter mass with [C II] intensity mapping](https://arxiv.org/abs/2512.07933)
*Elena Marcuzzo,Cristiano Porciani,Emilio Romano-Díaz,Azadeh Moradinezhad Dizgah,Prachi Khatri,Matteo Viel*

**TL;DR**: 通过[C II]谱线强度成图(LIM)分析，研究发现深空光谱巡天(DSS)在冷暗物质(CDM)宇宙中对温暗物质(WDM)粒子质量($m_\mathrm{WDM}$)的95%置信下限可达1.10 keV（乐观光度函数）和0.58 keV（悲观光度函数），且未来更高灵敏度或光谱分辨率的观测可将其约束提升至5.82 keV和1.90 keV。


<details>
  <summary>Details</summary>

Background: 暗物质（DM）的本质仍存在争议，研究聚焦于通过[C II]谱线强度映射（LIM）探索温暗物质（WDM）粒子质量（m_WDM）对ΛCDM框架下小尺度结构问题的潜在影响。

Data: 研究中使用的数据来源包括模拟生成的[C II]功率谱（PS）数据，以及基于深空光谱巡天（DSS）在红移$z\simeq3.6$的观测数据，并扩展至更大天区覆盖、更高灵敏度和/或更高光谱分辨率的模拟数据。

Method: 基于晕模型（halo-model）方法构建了[C II]功率谱（PS）的公式化表达，并采用两种光度函数（LF）参数化方案处理其不确定性。

Result: 基于[C II]功率谱（PS）的线强度映射（LIM）巡天可有效约束温暗物质（WDM）粒子质量（m_WDM），在冷暗物质（CDM）宇宙中，深度光谱巡天（DSS）在95%置信水平下对m_WDM的下限达1.10 keV（乐观光度函数）或0.58 keV（悲观光度函数α=-1.1），而更先进的巡天结合更陡的光度函数（α=-1.9）可将约束提升至5.82 keV和1.90 keV。
...
### [33] [Estimation of the Hubble parameter from unedited compact object merger catalogues](https://arxiv.org/abs/2512.08164)
*Reiko Harada,Heather Fong,Kipp Cannon*

**TL;DR**: 该研究提出了一种基于引力波探测级信息（detection-level information）的宇宙学推断框架，无需逐事件参数估计即可利用边缘候选体（marginal candidates）进行红移分布推断，特别适用于探索遥远宇宙。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用引力波暗警报（dark sirens）进行哈勃参数（Hubble parameter）约束的宇宙学推断方法。

Data: 数据来源于引力波搜索管线直接生成的候选列表，无需额外筛选。

Method: 采用基于检测级信息（detection-level information）的群体推断（population inference）框架，绕过逐个候选体的参数估计（per-candidate parameter estimation）。

Result: 提出的框架利用仅检测级信息（detection-level information）实现了无需逐候选参数估计的宇宙学推断，特别适用于从边缘候选体（marginal candidates）中提取信息以探测遥远宇宙。
...
### [34] [Isocurvature Induced Gravitational Waves at Pulsar Timing Arrays](https://arxiv.org/abs/2512.08301)
*Yi-Fu Cai,Peizhi Du,Jiahang Zhong*

**TL;DR**: 该论文首次系统研究了冷暗物质（CDM）、重子、中微子和自由流动暗辐射（DR）四类初始等曲率扰动诱导的引力波，揭示了DR等曲率扰动产生的引力波谱与CDM存在本质差异，并通过NANOGrav数据在$10^{6}\,\textrm{Mpc}^{-1}$尺度上给出了对初始等曲率扰动的新约束。


<details>
  <summary>Details</summary>

Background: 引力波（GWs）是探测早期宇宙新物理的有力工具，尤其由原始等曲率扰动（primordial isocurvature perturbations）诱导的引力波携带着超越标准ΛCDM模型的新动力学信息。

Data: 数据来源包括脉冲星计时阵（Pulsar Timing Arrays）观测数据和NANOGrav最新数据集。

Method: 采用初始条件重构、各向异性应力分析及等曲率功率谱（isocurvature power spectrum）参数化（delta函数和分段幂律）方法研究原初等曲率扰动（primordial isocurvature perturbations）诱导的引力波。

Result: 自由流动暗辐射（DR）等曲率扰动诱导的引力波谱因各向异性应力存在而与冷暗物质（CDM）诱导的谱存在本质差异，且在高频段与DR密度占总辐射的恒定比例成正比。
...
### [35] [Stochastic gravitational-wave background search using data from five pulsar timing arrays](https://arxiv.org/abs/2512.08666)
*Wang-Wei Yu,Bruce Allen*

**TL;DR**: 基于五个脉冲星计时阵（PTA）的公开数据，采用新方法合并多PTA观测的121颗脉冲星数据，首次联合分析纳赫兹随机引力波背景（SGWB），得到其能量密度谱振幅（$A_{gw}$）和幂律指数（$γ_{gw}$）的后验概率分布，虽与HD曲线预测的脉冲星计时残差角相关函数一致，但统计显著性未达5σ置信阈值。


<details>
  <summary>Details</summary>

Background: 研究利用五台脉冲星计时阵（PTA）公开数据，探索纳赫兹频段平稳、各向同性且非偏振的随机引力波背景（SGWB）的存在性及其特性。

Data: 数据来源于五个脉冲星计时阵（PTAs）公开的121颗脉冲星到达时间数据，并对多PTA观测的脉冲星采用新“直接组合”方法合并其天体物理模型和数据。

Method: 采用“直接组合”（direct combination）方法合并多脉冲星计时阵（PTA）的天体物理模型与数据，并基于贝叶斯后验概率分布分析随机引力波背景（SGWB）的能量密度谱振幅$A_{gw}$和指数$γ_{gw}$。

Result: result：分析结果显示可能存在非零振幅（amplitude）$A_{gw}$的随机引力波背景（SGWB），但统计显著性未达到传统5σ置信检测阈值，且脉冲星间时序残差相关性符合Hellings和 Downs（HD）曲线预测。
...
### [36] [The frame-dragging vector potential on galaxy scales from DM-only Newtonian $N$-body simulations](https://arxiv.org/abs/2512.08703)
*William Beordo,Marco Bruni,Cristian Barrera-Hinojosa,Mariateresa Crosta*

**TL;DR**: 研究发现，在宇宙结构非线性演化中，后弗里德曼（post-Friedmann）框架下由动量流产生的引磁（gravito-magnetic）矢量势强度比微扰理论预测高两个数量级，但相对于非线性牛顿标量引力势仅为0.1%~1%的效应。


<details>
  <summary>Details</summary>

Background: 研究探讨了广义相对论（General Relativity）在宇宙结构非线性演化中的作用，特别是在后弗里德曼（post-Friedmann）展开框架下弱场相对论近似对宇宙学的适用性。

Data: 研究使用了IllustrisTNG模拟（simulation）数据，并通过Delaunay Tessellation Field Estimator代码计算了引力磁（gravito-magnetic）矢量势的功率谱。

Method: 采用后弗里德曼展开框架（post-Friedmann expansion framework）和Delaunay曲面细分场估计器（Delaunay Tessellation Field Estimator）代码，基于IllustrisTNG模拟数据计算引力磁矢量势（gravito-magnetic vector potential）的功率谱。

Result: 在IllustrisTNG模拟中，利用Delaunay Tessellation Field Estimator（DTFE）方法计算了引力磁（gravito-magnetic）矢量势的功率谱，发现其幅度比微扰理论预测高两个数量级，相当于非线性牛顿标量引力势的1%～0.1%。
...
### [37] [Neutrino mass constraints in the context of 4-parameter dark energy equation of state and DESI DR2 observations](https://arxiv.org/abs/2512.08752)
*Gowri S Nair,Amlan Chakraborty,Luca Amendola,Subinoy Das*

**TL;DR**: 通过采用四参数暗能量状态方程(4pDE)模型并结合DESI DR2数据，研究发现中微子总质量上限为0.101 eV（95%置信水平），比ΛCDM模型约束更宽松但比$w_0w_a$CDM框架更严格，且未发现红移0.5处存在显著幻影穿越现象。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过改进暗能量状态方程（4pDE）模型来缓解中微子总质量（∑mν）与晚期宇宙加速之间的参数简并性，从而优化宇宙学约束。

Data: 数据来源包括普朗克卫星(Planck)数据、DESI DR2重子声波振荡(BAO)数据和Pantheon+超新星数据。

Method: 采用四参数暗能量状态方程（4pDE）模型，在修改版CLASS中实现并结合马尔可夫链蒙特卡洛（MCMC）方法分析普朗克、DESI DR2重子声波振荡（BAO）和Pantheon+数据。

Result: 采用四参数暗能量状态方程（4pDE）模型结合DESI DR2数据得到的中微子总质量上限为$\sum m_ν< 0.101$ eV（95%置信水平），比$Λ$CDM框架下的约束更严格且比$w_0w_a$CDM模型的DESI DR2结果更精确。
...
### [38] [Probing the environment around GW170817 with DESI: insights on galaxy group peculiar velocities for standard siren measurements](https://arxiv.org/abs/2512.08818)
...
**TL;DR**: 基于DESI观测和GW170817引力波事件，通过测量NGC 4993所在星系群（halo质量约10¹³M⊙）的39个成员星系的本动速度（peculiar velocity），得出哈勃常数H₀为70.9⁺⁶·⁴₋₈·₅或73.4⁺³·³₋₃·⁹ km s⁻¹ Mpc⁻¹，强调了多光谱观测对提升标准汽笛（standard siren）测量精度的关键作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用引力波事件GW170817和暗能量光谱仪（DESI）观测数据精确测量哈勃常数（Hubble constant, H0），并探讨宿主星系NGC 4993所在星系群的局部动力学对标准汽笛（standard siren）测量的影响。

Data: 数据来源包括引力波事件GW170817的观测数据、暗能量光谱仪（DESI）对NGC 4993附近数千个星系的观测数据（其中39个星系属于同一星系群）、基本平面（Fundamental Plane）星系观测的视向速度测量数据以及宇宙流-4（Cosmicflows-4）中表面亮度涨落法对NGC 4993的视向速度测量数据。

Method: 采用标准汽笛法（standard siren）结合暗能量光谱仪（DESI）观测数据，通过测量宿主星系NGC 4993所在星系群中39个成员星系的本动速度（peculiar velocity），并利用基本平面（Fundamental Plane）和宇宙流-4（Cosmicflows-4）的表面亮度涨落（Surface Brightness Fluctuations）技术进行本动速度修正。

Result: 基于DESI观测和NGC 4993宿主星系群动力学分析，测得哈勃常数（Hubble constant）$H_0$为$70.9^{+6.4}_{-8.5}$ km s$^{-1}$ Mpc$^{-1}$（使用本征平面星系）或$73.4^{+3.3}_{-3.9}$ km s$^{-1}$ Mpc$^{-1}$（使用宇宙流-4表面亮度涨落）。
...
### [39] [Foreground Mitigation for CMB Lensing with the Global Minimum Variance Quadratic Estimator](https://arxiv.org/abs/2512.08908)
*Yuka Nakato,W. L. Kimmy Wu,Ana Carolina Silva Oliveira,Yuuki Omori,Abhishek S. Maniyar*

**TL;DR**: 通过将全局最小方差（GMV）框架与tSZ-deproj和cross-ILC前景抑制技术结合，显著降低了宇宙微波背景（CMB）弱引力透镜（weak gravitational lensing）重建偏差（SPT-3G Ext-10k和Simons Observatory在L<1000时从~4%降至<1%），提升了透镜图在交叉相关分析中的适用性。


<details>
  <summary>Details</summary>

Background: 宇宙微波背景辐射（CMB）的弱引力透镜效应是研究宇宙结构形成和演化的重要宇宙学探针。

Data: 数据来源于SPT-3G和Simons Observatory (SO)提供的高分辨率、低噪声的温度和偏振（polarization）地图。

Method: 采用全局最小方差（GMV）二次估计器结合tSZ-deproj和cross-ILC前景抑制技术，提升宇宙微波背景（CMB）弱引力透镜重建的鲁棒性。

Result: 采用tSZ-deproj和cross-ILC技术将CMB弱引力透镜(weak gravitational lensing)重建偏差在L<1000范围内从标准GMV的~4%分别降至2%和<1%。
...
### [40] [Core-halo scaling relations in self-interacting scalar field dark matter](https://arxiv.org/abs/2512.07020)
*Jessica N. López-Sánchez,Erick Munive-Villa,Tanja Rindler-Daller*

**TL;DR**: 通过三维格罗斯-皮塔耶夫斯基-泊松模拟研究标量场暗物质（SFDM）晕中自相互作用对孤子核结构演化的影响，发现排斥性相互作用导致更大质量、更延展且中心密度较低的核，而吸引性相互作用会增强中心密度并在超过临界质量时引发坍缩，同时证实孤子核质量-半径关系符合解析预测并扩展了核-晕质量关系至模糊暗物质以外情形，揭示了核质量、尺寸和总能量间的标度关系依赖于自相互作用的强度、符号及晕演化阶段。


<details>
  <summary>Details</summary>

Background: 研究探讨了标量场暗物质（SFDM）中自相互作用对暗物质晕结构和演化的影响。

Data: 使用三维Gross-Pitaevskii-Poisson模拟（simulation）研究了多种孤子（soliton）合并过程，覆盖了从排斥到吸引的广泛散射长度范围。

Method: 采用三维格罗斯-皮塔耶夫斯基-泊松(Gross-Pitaevskii-Poisson)模拟方法研究了标量场暗物质(SFDM)孤立子合并过程中自相互作用的排斥和吸引机制对不同散射长度下晕结构演化的影响。

Result: 排斥性自相互作用导致更大质量、更延展且中心密度更低的核，而吸引性相互作用增强中心密度并在超过临界质量时促使核坍缩。
...
### [41] [SATMO: a Multi-Planet Thermal Analysis Tool for CubeSat Missions](https://arxiv.org/abs/2512.07896)
*Alexander Chipps,Daniel Forgette,Kerri Cahoy*

**TL;DR**: 开发了一款开源、基于MATLAB的六节点卫星热模型（SATMO），用于低高度圆轨道立方星（CubeSat）的早期热分析，其精度与行业标准软件Thermal Desktop相当（温差在1.17°C以内），支持地球、金星和火星等天体的任务规划。


<details>
  <summary>Details</summary>

Background: 商业发射能力扩张推动行星际小卫星(SmallSat)任务需求增长，亟需易用工具预测地球及其他天体轨道环境对航天器的热效应。

Data: data：SATMO基于MATLAB开发，验证数据来自与Thermal Desktop软件的比较结果，适用于地球、金星和火星等天体的低高度圆轨道立方星（CubeSat）热分析。

Method: 开发了一个基于MATLAB的开源六节点热分析程序（SATMO），用于低高度圆轨道卫星的热效应建模，并通过与Thermal Desktop软件的对比验证了其准确性。

Result: 开发了一款开源、基于MATLAB的六节点卫星热模型(SATMO)，适用于低高度圆轨道卫星，其温度预测与行业标准Thermal Desktop软件相比误差在1.17°C以内。
...
### [42] [The interstellar signature: A computational framework for open source interstellar tracking](https://arxiv.org/abs/2512.07910)
*Pancha Narayan Sahu*

**TL;DR**: 开发了一个名为“星际特征（The Interstellar Signature）”的开源计算框架，通过集成实时天文数据和基于物理的模拟方法，为星际（interstellar）和太阳系天体提供实时建模、可视化及轨道分析工具，降低了专业天文研究与公众科学参与的壁垒。


<details>
  <summary>Details</summary>

Background: 星际物体（如1I/'Oumuamua和2I/Borisov）为研究其他恒星系统的形成与演化提供了独特窗口。

Data: 数据来源包括公共存储库和API提供的实时天文数据流，并结合基于物理的模拟方法。

Method: 采用基于物理的实时模拟方法（physics-based simulation methods）整合公开数据流，构建交互式可视化轨道分析框架。

Result: 开发了一个名为“星际特征”（The Interstellar Signature）的开源计算框架，通过基于网络的平台实现实时星际追踪和轨道分析。
...
### [43] [Microlensing Signatures of Dyson Sphere-like Structures around Primordial Black Holes as Technosignatures of Extraterrestrial Advanced Civilizations](https://arxiv.org/abs/2512.07924)
*Shant Baghram*

**TL;DR**: 该论文通过建立随机透射模型和概率框架，首次提出利用微引力透镜（microlensing）光变曲线中的动态调制特征（如光学深度变化和随机间隙分布）探测原始黑洞（PBHs）周围戴森群（Dyson Swarms）等外星技术特征（technosignatures），并分析了红外超额辐射和有效光学深度等互补观测指标，为搜寻先进外星文明提供了新方法。


<details>
  <summary>Details</summary>

Background: 研究主题为利用微引力透镜效应探测戴森球（Dyson sphere）等外星技术特征（technosignatures），特别是围绕原初黑洞（PBHs）的戴森群（Dyson Swarms）产生的动态调制信号。

Data: 数据来源包括模拟生成的戴森群（Dyson Swarms）随机透射模型参数扫描结果、光学透射持续时间热图、红外过量辐射及峰值发射波长观测数据。

Method: 采用概率框架结合随机传输模型（stochastic transmission model），分析可变光学深度（optical depth）和随机间隙分布（gap distributions）对微透镜光变曲线的动态调制效应。

Result: 通过引入随机透射模型和参数扫描，研究发现戴森群（Dyson Swarms）等人工动态结构可通过微引力透镜光变曲线的时间依赖性调制效应被探测到。
...
### [44] [The HIP 54515 b Superjovian Planet as an Early, Critical Look at the Roman Coronagraph's Performance in the Faint Target Star, Small IWA Limit](https://arxiv.org/abs/2512.07942)
*Thayne Currie,Yiting Li,Brianna Lacy,Mona El Morsy,Masayuki Kuzuhara,Naoshi Murakami,Danielle Bovie*

**TL;DR**: 论文核心贡献是首次利用罗马日冕仪（Roman Coronagraph）在HLC/575nm模式下对超级木星HIP 54515 b进行严格测试，验证其在微弱恒星、小内工作角（IWA）极限下的性能，为未来宜居世界观测站（Habitable Worlds Observatory）观测邻近K型和M型恒星周围岩质行星提供技术参考。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用罗曼日冕仪（Roman Coronagraph）在微弱恒星和小内工作角（IWA）极限下的性能测试，为未来宜居世界天文台（Habitable Worlds Observatory）观测邻近K型和M型恒星周围岩质行星的技术挑战提供前瞻性评估。

Data: 使用罗马日冕仪(Roman Coronagraph)的HLC/575nm模式观测新发现的超级木星HIP 54515 b及其V=6.8母星，并选取多颗参考星测试日冕仪在不同Δ俯仰角(Δ pitch angle)下的性能。

Method: 采用高对比度日冕仪（HLC/575nm）对超级木星HIP 54515 b进行严格测试，通过多参考星评估望远镜热环境对对比度的影响。

Result: 罗马日冕仪(Roman Coronagraph)在微弱恒星、小内工作角(IWA)极限下的性能测试为未来宜居世界天文台(Habitable Worlds Observatory)观测近邻K型和M型恒星周围岩质行星提供了首次挑战预演。
...
### [45] [Stellar Multiplicity via Speckle Interferometry with the 3.6 m Devasthal Optical Telescope](https://arxiv.org/abs/2512.07971)
*Km Nitu Rai,Neelam Panwar,Jeewan C Pandey,T S Kumar,Subrata Sarangi,Prasenjit Saha*

**TL;DR**: 利用3.6米DOT望远镜的散斑干涉测量(Speckle Interferometry)技术成功解析双星系统轨道间距和位置角，并通过贝叶斯推断精确测量暗弱伴星轨道参数，验证了中等口径望远镜实现高分辨率光学干涉的可行性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于克服地面光学望远镜的衍射极限（Diffraction Limit）和大气湍流限制，通过斑点干涉技术（Speckle Interferometry）提升对双星系统轨道参数的高分辨率测量能力。

Data: 数据来源于3.6米德文德拉光学望远镜（DOT）获取的散斑干涉成像（Speckle Interferometry）数据。

Method: 采用斑点干涉测量法（Speckle Interferometry）结合贝叶斯推断（Bayesian inference）建模斑点图案，精确测量双星系统轨道参数。

Result: 基于斑点干涉测量法（Speckle Interferometry）的观测和分析方法能够有效克服大气湍流和衍射极限限制，利用3.6米DOT望远镜实现双星系统的高分辨率解析，精确测量其轨道间距和位置角，并通过贝叶斯推断提升暗弱伴星系统的轨道参数估计精度。
...
### [46] [PolySwyft: sequential simulation-based nested sampling](https://arxiv.org/abs/2512.08316)
*Kilian H. Scheutwinkel,Will Handley,Christoph Weniger,Eloy de Lera Acedo*

**TL;DR**: 提出了一种结合嵌套采样（NS）和神经比率估计（NRE）的非摊销模拟推理框架PolySwyft，通过嵌套NRE轮次和自适应KL散度终止准则，在复杂多模态后验分布推断中实现了更高效率且保持贝叶斯有效性，并在宇宙学参数推断中验证了其优越性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发新型非摊销模拟推理框架（PolySwyft），用于解决似然函数不可解析但具备前向模拟器的复杂后验分布推断问题。

Data: 数据来源包括：在玩具问题中使用具有解析后验的5维多元高斯和多元相关高斯混合模型生成仿真数据，在真实应用中使用CosmoPower从CMB（宇宙微波背景辐射）功率谱生成的6维宇宙学参数仿真数据。

Method: 采用嵌套采样（NS）与神经比率估计（NRE）协同优化的非摊销模拟推理框架（PolySwyft），通过KL散度准则自适应终止采样，高效处理复杂多峰后验分布。

Result: PolySwyft框架通过嵌套神经比率估计(NRE)与嵌套采样(NS)方法，在保持贝叶斯有效性的前提下，对复杂多峰后验分布实现了比TNRE更快的收敛速度和更少的模拟器调用次数。
...
### [47] [Thermal design of focal plane assembly of wavefront camera for exoplanet imaging corona module](https://arxiv.org/abs/2512.08708)
*Chengcheng Wen,Shu Jiang,Mingming Xu,Lingyi Kong,Jinning He*

**TL;DR**: 采用柔性石墨导热电缆和槽道热管（grooved heat pipes）的热设计方案使波前相机焦平面组件（focal plane assembly）的CCD芯片工作温度稳定在-12.8至-10.9摄氏度，满足系外行星成像日冕仪模块（exoplanet imaging coronagraph module）的散热需求。


<details>
  <summary>Details</summary>

Background: 研究聚焦于系外行星成像日冕仪模块中波前相机焦平面组件（focal plane assembly）的热管理设计，以确保其成像质量和可靠性。

Data: 采用热分析软件建立有限元模型并进行热仿真（thermal simulation），数据来源于稳态下CCD芯片及其他组件的温度测量结果。

Method: 采用柔性石墨烯导热电缆和沟槽热管进行散热路径设计，并通过热分析软件建立有限元模型进行热仿真。

Result: 采用柔性石墨烯导热电缆和槽道热管的热设计方案使CCD芯片在稳态下工作温度范围为-12.8至-10.9摄氏度，满足温控指标。
...
### [48] [The impact of lunar topography on the 21-cm power spectrum for grid-based arrays : Insights for the Dark-ages EXplorer (DEX)](https://arxiv.org/abs/2512.08906)
...
**TL;DR**: 提出SPADE-21cm端到端模拟管道，首次结合月面地形数据研究月球背面DEX阵列天线位移（横向σxy/λ≲0.027、纵向）对黑暗时代（DA）21厘米信号功率谱的影响，发现横向偏移在32.5MHz下可使强污染傅里叶模式占比控制在1%以内。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用21厘米信号探测宇宙黑暗时期（Dark Ages）的物质三维分布，并评估月球背面部署的DEX阵列天线位置偏移对测量精度的影响。

Data: 数据来源包括整合了月球地形数据的宇宙黎明（Dark Ages）21厘米信号模型、天空模型以及用于模拟天线位移效应的端到端仿真流程SPADE-21cm。

Method: 采用端到端模拟流程SPADE-21cm，整合了月球坐标系下的21厘米信号模型与月球地形数据，并研究了天线位置偏移对二维功率谱的影响。

Result: 首次开发了SPADE-21cm端到端模拟管线，整合了月球坐标系下的黑暗时代(Dark Ages)21厘米信号模型与月球地形数据，量化了天线位置偏移对功率谱的影响。
...
### [49] [Machine learning classification of baseband data of CHIME FRBs](https://arxiv.org/abs/2512.08308)
...
**TL;DR**: 基于CHIME/FRB基带目录（baseband catalog）对快速射电暴（FRBs）进行分类研究，发现122个非重复暴中有15个可能是重复暴候选体（repeater candidates），其中1个为新发现且1个已被确认为重复暴，同时排除了31个先前被归类为候选体的非重复暴。


<details>
  <summary>Details</summary>

Background: 快速射电暴（FRB）是天文领域中起源未知的明亮毫秒级射电脉冲现象。

Data: 数据来源于CHIME/FRB基带目录（baseband catalog），其时间分辨率比强度目录（intensity catalog）高三个数量级，且提供了测量流量（fluence），而强度目录仅报告了上限值。

Method: 采用机器学习技术对CHIME/FRB基带目录（baseband catalog）中的快速射电暴（FRBs）进行分类分析。

Result: 重复快速射电暴（FRBs）比非重复事件具有更宽的时间宽度和更窄的频谱带宽，暗示它们可能源于不同的物理机制。
...
### [50] [Backwards Gamma-Ray Bursts: Searching for Exploding Primordial Black Holes in Short-Duration GRB Catalogs](https://arxiv.org/abs/2512.08778)
*Stefano Profumo,Kally Wen*

**TL;DR**: 通过系统分析39个短伽马暴（sGRB）样本并建立前向建模框架，排除了原始黑洞（PBH）蒸发信号的存在，将本地PBH爆炸率密度上限约束为$R_{\mathrm{PBH}} \lesssim 10^5~\mathrm{pc}^{-3}~\mathrm{yr}^{-1}$，同时建立了可扩展的模板匹配方法用于未来量子引力研究。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过短伽马暴(sGRB)数据系统搜寻原初黑洞(PBH)霍金蒸发末期的特征信号，以探索量子引力效应和早期宇宙物理。

Data: 数据来源于对39个经过充分表征的Swift短伽马射线暴（sGRB）的分析，这些暴发具有未检测到或极其微弱的余辉。

Method: 采用正向建模框架（forward-modeling framework）直接比较理论原初黑洞（PBH）蒸发光变曲线与经验伽马暴脉冲模板，结合标准模型粒子含量和探测器响应折叠计算。

Result: 未在短伽马暴（sGRB）样本中发现符合原初黑洞（PBH）蒸发特征的候选体，所有事件的时间轮廓均与传统FRED或ERCA模板更吻合。
...
### [51] [A search for successful and choked jets in nearby broad-lined Type Ic supernovae](https://arxiv.org/abs/2512.08822)
...
**TL;DR**: 通过多波段观测近邻宽线Ic型超新星（SNe Ic-BL），发现其与伽马暴（GRB）存在连续谱关联，确认SN 2024rjw为强星周物质（CSM）相互作用驱动的射电明亮事件，并首次提出SN 2022xxf可能是扼制喷流（choked jets）产生的茧状结构（cocoon）主导候选体，从而完善了普通SNe Ic-BL、引擎驱动爆发与GRB之间的统一演化图像。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于长伽马射线暴(GRB)与宽线剥离包层核心坍缩超新星(SNe Ic-BL)的关联性，旨在揭示大质量恒星演化与爆炸物理背景下喷流速度连续谱的多样性。

Data: data：研究使用了针对邻近（红移z≤0.1）宽线Ic型超新星（SNe Ic-BL）的协调射电至X射线观测数据，包括八个新射电监测事件和一个先前观测超新星的更新数据。

Method: 采用多波段（radio-to-X-ray）协同观测手段分析邻近（z≤0.1）宽线Ic型超新星（SNe Ic-BL）的射电至X射线辐射特征，结合动力学速度分布和喷流阻塞（choked jets）产生的包层（cocoons）模型约束相对论性喷流占比。

Result: 长伽马射线暴（GRBs）与宽线剥离包层核心坍缩超新星（SNe Ic-BL）之间存在观测联系，但对其在恒星演化和爆炸物理中的完整机制仍不完全清楚。
...
### [52] [Self-lensing of moving gravitational-wave sources can break the microlensing crossing timescale degeneracy](https://arxiv.org/abs/2512.08898)
*Helena Ubach*

**TL;DR**: 首次研究了移动双星合并产生的频变引力波(GW)信号被大质量黑洞(MBH)透镜化的特征，发现自透镜几何结构可打破微透镜在爱因斯坦半径穿越时标(t_E)上的简并性，通过测量调制周期(T)、GW频率(f)和t_E可同时获得透镜距离(d_LS)和红移质量(M_MBH,z)，为约束GW源天体物理环境提供了新方法。


<details>
  <summary>Details</summary>

Background: 研究探讨了运动中的引力波（GW）源经过大质量天体（如大质量黑洞MBH）时产生的引力透镜效应及其波形畸变特征。

Data: 数据来源于围绕大质量黑洞（MBH）做开普勒圆轨道运动的引力波（GW）双星合并信号和光信号的引力透镜效应观测。

Method: 采用自透镜(self-lensing)几何模型分析引力波(GW)信号在绕大质量黑洞(MBH)开普勒圆轨道运动时的干涉图样，通过测量调制周期T、GW频率f和爱因斯坦半径穿越时标t_E，直接获取红移质量M_{MBH,z}和距离d_LS。

Result: 引力波(GW)信号的自透镜(self-lensing)几何结构打破了微透镜(microlensing)在爱因斯坦半径穿越时标$t_{\rm E}$上的简并性，使得曲线持续时间($2 t_{\rm E}$)独立于大质量黑洞(MBH)质量$M_{\rm MBH}$，可直接测量到大质量黑洞的距离$d_{\rm LS}$。
...
### [53] [Self-lensing flares from black hole binaries V: systematic searches in LSST](https://arxiv.org/abs/2512.08427)
*Kevin Park,Zoltan Haiman,Chengcheng Xin,Tzuken Shen,Ashley Villar,Jordy Davelaar*

**TL;DR**: 研究发现匹配滤波器（MFs）能有效从LSST（大型综合巡天望远镜）观测的类星体光变曲线中识别出亚秒差距超大质量双黑洞（SMBHBs）产生的引力自透镜耀斑（SLFs），并开发出可高效恢复双星参数的三步法。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用薇拉·鲁宾天文台(Vera C. Rubin Observatory)的LSST项目在未来十年内探测数千万类星体中可能存在的亚秒差距超大质量双黑洞(SMBHBs)及其引力自透镜耀斑(SLFs)。

Data: 数据来源于维拉·C·鲁宾天文台（Vera C. Rubin Observatory）的LSST（Legacy Survey of Space and Time）项目，通过10年观测获取的数千万类星体（quasars）光变曲线，并利用流体动力学（hydrodynamical）、多普勒（Doppler）和自引力透镜（self-lensing）效应的玩具模型生成仿真数据，同时将自引力透镜耀发（SLFs）注入阻尼随机游走（DRW）光变曲线以模拟随机类星体变异性。

Method: 采用伦伯格-斯卡格尔周期图（Lomb-Scargle periodogram）和匹配滤波器（matched filters）对等质量偏心超大质量黑洞双星（SMBHBs）产生的流体动力学、多普勒和自引力透镜光变（SLFs）进行系统搜索，并通过注入阻尼随机游走（DRW）光变曲线评估其有效性。

Result: 匹配滤波器（matched filters）的信噪比（SNR）能有效区分自引力透镜耀斑（SLFs）与噪声，并可恢复双星参数，适用于LSST海量类星体光变曲线的计算高效分析。
...
### [54] [Pulsars identified in the LOFAR Two-metre Sky Survey at 144 MHz](https://arxiv.org/abs/2512.08663)
*G. A. C. Rijkers,C. G. Bassa,J. R. Callingham,T. Shimwell*

**TL;DR**: 在LOFAR两米巡天(LoTSS)第二版数据中识别出80颗已知射电脉冲星(radio pulsars)为144 MHz连续源，发现偏振特性是筛选脉冲星候选体的有效标准，且与54 MHz的LoLSS交叉匹配可补充光谱指数信息。


<details>
  <summary>Details</summary>

Background: 研究主题为利用低频阵列射电望远镜（LOFAR）在144 MHz波段对已知射电脉冲星（radio pulsars）进行天体测量识别，并探索基于偏振特性和频谱指数筛选脉冲星候选体的方法。

Data: 数据来源于低频阵列（LOFAR）两米巡天第二版（LoTSS DR2）在144 MHz探测到的未分辨连续谱源，以及即将发布的覆盖北天区（δ>0°）的完整LoTSS数据和低频阵列低频巡天（LoLSS）在54 MHz（覆盖δ>24°）的观测数据。

Method: method：基于LOFAR低频阵列（LOFAR）144 MHz连续谱源的天文测量识别，结合线性或圆偏振特性进行盲检测，并通过与54 MHz LoLSS源交叉匹配补充谱指数筛选。

Result: 在144 MHz波段成功识别出80颗已知射电脉冲星（radio pulsars）的连续谱源，其中35颗（44%）通过线偏振或圆偏振特性被盲检重测，表明基于偏振特性的候选体筛选方法具有可行性。
...
### [55] [Inflationary Particle Production and the Swampland](https://arxiv.org/abs/2512.07930)
*Dieter Lüst,Joaquin Masias,Mauro Pieroni,Marco Scalisi*

**TL;DR**: 研究了无限态塔（infinite tower of states）在质量尺度随暴胀轨迹指数下降时对粒子产生的影响，发现引力弱耦合宇宙学有效理论中态塔诱导的修正项按$(H/Λ_{\text{sp}})^{2+p}$比例被抑制，暴胀现象学基本不受影响。


<details>
  <summary>Details</summary>

Background: 研究探讨了暴胀期间粒子产生的影响，重点关注无限态塔（infinite tower of states）质量尺度沿暴胀轨迹指数下降的情景，这一现象与沼泽地距离猜想（Swampland Distance Conjecture, SDC）密切相关。

Data: 数据来源于理论模拟（simulation）和观测数据（包括阿塔卡马宇宙学望远镜（Atacama Cosmology Telescope）的最新观测约束）。

Method: 采用膨胀期间粒子产生的修正分析方法，基于Swampland距离猜想(SDC)中质量标度指数下降的无限态塔模型，计算了哈勃尺度(H)与物种标度(Λ_sp)之比的高阶幂次修正项(H/Λ_sp)^(2+p)。

Result: 在引力弱耦合的宇宙学有效理论中，无限态塔（infinite tower of states）对暴胀观测量的修正贡献被抑制，暴胀现象学基本不受影响。
...
### [56] [Open Effective Field Theory and the Physics of Cosmological Collider Signals](https://arxiv.org/abs/2512.07941)
*Thomas Colas,Zhehan Qin,Xi Tong*

**TL;DR**: 该研究通过开放有效场论（open effective field theory）框架分析宇宙学对撞机信号起源，发现大质量标量场单交换产生的四点谱（trispectrum）在Keldysh基下可清晰分离为局域与非局域分量，其中局域信号源于有效场论中的幺正部分，非局域信号则与随机涨落（stochastic noise）本质相关，并揭示了该信号与量子态混合度等内禀性质及熵产生之间的关联。


<details>
  <summary>Details</summary>

Background: 该研究探讨了宇宙学对撞机信号（cosmological collider signal）的起源，基于开放有效场论（open effective field theories）框架分析大质量标量场单交换过程对原初非高斯性的影响。

Data: 研究使用了基于开有效场论（open effective field theory）框架的理论模拟数据，重点分析了重标量场（massive scalar field）单交换情形下的四谱（trispectrum）分解特性。

Method: 采用开放有效场论（open effective field theory）框架，结合Keldysh基矢分解重场传播子，分析局域与非局域三谱分量，并通过重场积分导出包含幺正算符与非幺正耗散-噪声项的单场有效理论。

Result: 在宇称守恒理论中，局域信号主要来源于有效场论（effective field theory）的幺正部分，而非局域信号则与随机涨落（stochastic noise）部分本质相关。
...
### [57] [The Strong-$CP$ Problem and its Gauge Axion solution as Evidence for Fundamental Strings](https://arxiv.org/abs/2512.08834)
*Gia Dvali,Lucy Komisel,Otari Sakhelashvili,Anja Wachowitz*

**TL;DR**: 该论文通过将QCD真空的拓扑敏感性（topological susceptibility）与陈-西蒙斯规范理论（Chern-Simons gauge theory）的θ-真空态联系起来，提出具有精确品质的规范轴子（gauge axion）解决了强CP难题（strong-$CP$ puzzle），并证明其弦结构必须嵌入基本引力理论，从而为轴子与引力的关联提供了新证据。


<details>
  <summary>Details</summary>

Background: 研究探讨了量子色动力学(QCD)真空的拓扑敏感性(topological susceptibility)如何通过陈-西蒙斯规范理论(Chern-Simons gauge theory)理解θ真空(θ-vacua)的本质，并揭示了规范轴子(gauge axion)作为解决强CP问题(strong-CP puzzle)的内在机制。

Data: 数据来源为基于量子色动力学（QCD）规范理论的数值模拟（simulation）和理论推导。

Method: 采用拓扑不变量（topological susceptibility）分析QCD真空的θ-真空态（θ-vacua），通过规范轴子（gauge axion）作为监测工具研究不同物理区域特性，并建立其与引力基本理论的直接联系。

Result: QCD真空的拓扑敏感性（topological susceptibility）揭示了规范轴子（gauge axion）作为强CP问题的解决方案，其本质是QCD规范冗余的固有部分且不存在质量（quality）问题。
...
### [58] [Astrometric Reconnaissance of Exoplanetary Systems (ARES). I. Methodology validation with HST point-source images of Proxima Centauri](https://arxiv.org/abs/2512.08533)
*M. Libralato,L. Bedin,A. Burgasser*

**TL;DR**: 利用哈勃太空望远镜(HST)和盖亚(Gaia)数据，首次对半人马座比邻星(Proxima Centauri)进行了高精度天体测量，测得位置、自行(proper motion)和视差(parallax)的精度分别达0.4毫角秒、50微角秒/年和0.2毫角秒，并推断候选系外行星比邻星c(Proxima c)的质量为3.4(+5.2/-3.4)倍地球质量(M⊕)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用哈勃空间望远镜（HST）多历元观测数据验证和表征邻近恒星系统候选系外行星（exoplanets）的天体测量学（astrometry）特征。

Data: 数据来源于哈勃空间望远镜（HST）的存档和近期点源成像模式观测数据，并结合盖亚（Gaia）DR3星表进行联合分析。

Method: 结合哈勃空间望远镜(HST)点源成像模式观测数据与盖亚(Gaia)DR3星表，开发了高精度天体测量参数提取方法，并改进了HST探测器的几何畸变校准。

Result: result：结合哈勃太空望远镜（HST）长期观测与盖亚（Gaia）短期天体测量数据，在假设近邻星（Proxima Centauri）候选系外行星Proxima c为圆形正面轨道前提下，估算其质量为$m_c = 3.4^{+5.2}_{-3.4}$ $M_\odot$，与径向速度观测约束基本一致但受当前误差限制。
...
### [59] [Improvements to the NSO Farside Mapping Pipeline: Noise Reduction Updates](https://arxiv.org/abs/2512.08133)
*Mitchell Creelman,Kiran Jain,Niles Oien,John Britanik,Thomas M. Wentzel*

**TL;DR**: 美国国家太阳观测台（NSO）的远侧管道（Farside Pipeline）通过日震全息技术（helioseismic holography）分析正面连续多普勒图像，提升了太阳远侧活动区（solar active regions）的探测精度和一致性，增强了其在空间天气预报和科研中的应用价值。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用日震全息技术（helioseismic holography）提升太阳活动区（solar active regions）在不可见半球（farside）的监测能力，以支持空间天气（space weather）预警和关键基础设施防护。

Data: 数据来源于美国国家太阳观测台（NSO）的GONG网络（Global Oscillation Network Group）提供的连续多普勒图像。

Method: 采用日震全息成像技术（helioseismic holography）分析GONG网络提供的连续多普勒图像（Doppler images），推断太阳背面活动区（solar active regions）的尺寸和位置。

Result: 通过日震全息技术（helioseismic holography）分析连续多普勒图像，推断太阳背面活动区（active regions）的尺寸和位置。
...
### [60] [Magnetic activity of ultracool dwarfs in the LAMOST DR11](https://arxiv.org/abs/2512.08305)
*Yue Xiang,Shenghong Gu,Dongtao Cao*

**TL;DR**: 该研究通过分析LAMOST DR11数据识别出962颗极冷矮星(ultracool dwarfs)，利用半监督机器学习方法验证了CSST光谱的探测能力，基于Hα发射线研究了其磁活动性，并结合Kepler/K2光变曲线得出82颗样本的旋转周期，最终发现极冷矮星的活动-自转关系在罗斯贝数(Rossby number)为0.12时达到饱和。


<details>
  <summary>Details</summary>

Background: 研究聚焦于极冷矮星（ultracool dwarfs）的磁场生成机制及其活动特性，填补了这类完全对流天体与部分对流类太阳恒星之间的知识空白。

Data: 数据来源包括LAMOST（大天区面积多目标光纤光谱天文望远镜）数据发布DR11中的962颗极冷矮星（ultracool dwarfs）观测数据，以及通过降阶LAMOST光谱模拟的中国空间站巡天望远镜（CSST）低分辨率无缝光谱数据。

Method: 采用半监督机器学习方法和自编码器(autoencoder)模型识别超冷矮星(ultracool dwarfs)，并利用模拟的中国空间站巡天望远镜(CSST)无缝光谱数据进行验证。

Result: result：在超冷矮星（ultracool dwarfs）中，活动-自转关系在罗斯贝数（Rossby number）为0.12时达到饱和。
...
### [61] [Chemical Signatures of AGB Mass Transfer in Gaia White Dwarf Companions](https://arxiv.org/abs/2512.07972)
*Natsuko Yamaguchi,Kareem El-Badry,Henrique Reggiani,René Andrae,Sahar Shahaf*

**TL;DR**: 基于盖亚(Gaia) DR3数据的160颗白矮星+主序星(WD+MS)双星系统均质化学丰度分析，发现40颗钡星(其中36颗为新发现)，其s-过程元素丰度变化与组分质量、金属丰度相关，证实渐近巨星分支(AGB)质量转移是低金属丰度下碳增强贫金属星(CEMP-s)形成的关键机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过分析白矮星(WD)+主序星(MS)双星系统中主序星的化学丰度，探讨渐近巨星分支(AGB)阶段物质转移对s-过程元素富集的影响及其与双星系统参数的关系。

Data: 数据来源为盖亚卫星第三次数据释放（Gaia DR3）提供的160颗主序星（MS）与白矮星（WD）组成的双星系统轨道数据，并结合高分辨率光谱测量主序星的化学丰度（特别是s-过程元素）。

Method: 采用高分辨率光谱技术测量主序(MS)恒星化学丰度，重点分析s-过程元素，并结合渐近巨星分支(AGB)核合成模型解释观测到的丰度模式多样性。

Result: 在160颗白矮星+主序星(WD+MS)双星系统中发现了40颗钡星(barium dwarfs)，其中36颗为新发现，其s-过程元素丰度与伴星质量、金属丰度相关但与轨道间距无关，且最低金属丰度下观测到碳增强现象。
...
### [62] [Analytical Solutions for the Dynamics of Planetary Nebulae with and without Common Envelope Evolution](https://arxiv.org/abs/2512.07987)
*G. Garcia-Segura*

**TL;DR**: 该研究通过新解析方程分析行星状星云(planetary nebulae)动力学，发现大部分星云尤其是快慢速星云更符合共包层演化(common envelope evolution)模型，而单颗AGB恒星模型仅能解释膨胀速度20-30 km/s的星云。


<details>
  <summary>Details</summary>

Background: 研究聚焦于行星状星云(planetary nebulae)动力学的新解析解，特别关注机械光度(mechanical luminosity)时间变化和共有包层演化(common envelope evolution)情景。

Data: 研究中使用的数据来源于行星状星云(planetary nebulae)观测数据库与单渐近巨星支(AGB)恒星模型的模拟数据对比。

Method: 采用考虑机械光度（mechanical luminosity）时间变化和共有包层演化（common envelope evolution）情景的动力学新解析解进行分析。

Result: 行星状星云(planetary nebulae)动力学的新解析解表明，共包层演化(common envelope evolution)情景能更好地解释大部分星云，尤其是快速和慢速膨胀的星云。
...
### [63] [The SOFIA Massive(SOMA)Radio Survey. III. Radio Emission from Intermediate-Mass Protostars](https://arxiv.org/abs/2512.08105)
...
**TL;DR**: 通过甚大天线阵(VLA)对12颗中等质量(IM)原恒星进行射电连续谱观测，发现其射电光度低于低质量原恒星简单幂律外推预期，与恒星半径扩张导致光电离和激波电离辐射减弱的理论模型一致，为研究大质量原恒星演化与电离气体结构关系提供了新约束。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于中等质量（IM）原恒星的射电连续谱观测，旨在揭示其形态、多重性及射电谱能量分布（SEDs），并探讨大质量原恒星演化模型中电离气体结构的理论联系。

Data: 数据来源于甚大天线阵（VLA）对12颗中等质量（IM）原恒星的射电连续谱观测，这些观测是索菲亚（SOFIA）大质量恒星形成巡天的一部分。

Method: 采用甚大天线阵（VLA）射电连续谱观测技术分析中间质量（IM）原恒星形态、多重性和射电谱能量分布（SEDs），并结合SOMA Radio I和II数据扩展样本至29颗原恒星。

Result: 在中等质量（IM）原恒星样本中，无线电光度低于简单幂律从低质量原恒星外推的预期值，但与理论模型预测一致，反映了该演化阶段恒星半径扩大导致的低光电离和减少的激波电离发射。
...
### [64] [Time-Averaged Template for Stochastic Gravitational-Wave Background Detection in Space-Based Interferometers](https://arxiv.org/abs/2512.08521)
*Jing-yi Wu,Yong Tang*

**TL;DR**: 研究了时变臂长条件下模板构建对随机引力波背景（SGWB）参数估计的影响，发现时间平均模板能提高参数估计精度，而将有效臂长作为自由参数会增加不确定性。


<details>
  <summary>Details</summary>

Background: 研究探讨了在时变臂长（time-varying arm-lengths）轨道配置下模板构建对随机引力波背景（SGWB）参数估计的影响。

Data: 使用模拟的随机引力波背景（SGWB）信号和主要仪器噪声源作为数据来源。

Method: 采用三种模板策略（时间平均模板、等臂长模板和自由臂长参数模板）比较其对随机引力波背景（SGWB）参数估计的影响。

Result: 在真实轨道配置下，采用时间平均模板（time-averaged template）能提高随机引力波背景（SGWB）参数估计精度，而将有效臂长（effective arm-length）作为自由参数会增加估计不确定性。
...
### [65] [Probing Cosmic Strings via Black Hole Quasinormal Modes in Gravitational Wave Astronomy](https://arxiv.org/abs/2512.08368)
*Ishan Swamy,Deobrat Singh*

**TL;DR**: 通过将宇宙弦（cosmic strings）建模为静态黑洞时空中的微扰并分析准正规模（QNMs）频谱特征，首次确定了宇宙弦在引力波信号中可观测的扰动强度下限（不带电弦λ∼10⁻¹⁰，带电弦λ∼10⁻⁷），并建立了通过准正规模反推宇宙弦能量或电荷的新方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过引力波（GWs）探测宇宙弦（cosmic strings）这一早期宇宙遗留的一维拓扑缺陷，并探索其在准正则模（QNMs）谱中的特征印记。

Data: 数据来源于数值模拟（numerical simulation）对非旋转黑洞时空中宇宙弦（cosmic strings）作为微扰的建模及其对准正则模（QNMs）频谱的影响分析。

Method: 采用准正规模（QNMs）分析技术，通过数值模拟研究宇宙弦（cosmic strings）在非旋转黑洞时空中作为微扰的频谱特征。

Result: 宇宙弦（cosmic strings）可通过准正则模（QNMs）谱扰动在引力波信号中产生可观测特征，其扰动强度下限为不带电弦λ∼10⁻¹⁰和带电弦λ∼10⁻⁷。
...
### [66] [Single-field D-type inflation in the minimal supergravity in light of Planck-ACT-SPT data](https://arxiv.org/abs/2512.08760)
*Yermek Aldabergenov,Sergei V. Ketov*

**TL;DR**: 论文提出在最小超引力框架（minimal supergravity framework）下构建新型D型单场暴胀模型，以e折叠数（e-folds）为变量简化了暴胀势、标量扰动功率谱、宇宙学可观测量及重建过程，与多组精密宇宙微波背景辐射观测数据相符。


<details>
  <summary>Details</summary>

Background: 研究基于最小超引力框架（minimal supergravity framework）构建新型D型单场暴胀模型（D-type single-field inflation models），旨在与普朗克合作组等机构对宇宙微波背景辐射（cosmic microwave background radiation）的高精度观测结果相吻合。

Data: 数据来源于普朗克合作组（Planck Collaboration）、BICEP/Keck合作组（BICEP/Keck Collaboration）、阿塔卡马宇宙学望远镜（Atacama Cosmology Telescope）和南极望远镜（South Pole Telescope）对宇宙微波背景辐射（cosmic microwave background radiation）的精确测量。

Method: 采用最小超引力框架（minimal supergravity framework）构建新的D型单场暴胀模型，并以e-folds作为运行变量简化暴胀子势能（inflaton potential）、标量扰动功率谱（power spectrum of scalar perturbations）和宇宙学可观测量（cosmological observables）的计算。

Result: 使用以e-folds（e-折叠数）作为运行变量时，暴胀子势（inflaton potential）、标量扰动功率谱（power spectrum of scalar perturbations）、宇宙学可观测量（cosmological observables）和重建过程（reconstruction procedure）可以非常简化。
...
### [67] [Noble gravitational atoms: Self-gravitating black hole scalar wigs with angular momentum number](https://arxiv.org/abs/2512.08095)
...
**TL;DR**: 提出了描述黑洞周围自引力标量场构型的准静态近似解（noble gravitational atoms），发现其性质与角动量数（$\ell$）相关，在特定条件下可呈现星系尺度、暗物质密度及宇宙寿命量级的显著特征。


<details>
  <summary>Details</summary>

Background: 研究探索了黑洞周围自引力标量场（self-gravitating scalar field）构型的准静态近似解，类比于具有闭合电子壳层的惰性原子（noble atoms），提出了“惰性引力原子（noble gravitational atoms）”的新概念。

Data: 研究中使用的数据来源于对爱因斯坦-克莱因-戈登方程（Einstein-Klein-Gordon equations）的数值模拟（simulation）求解。

Method: 采用球对称的爱因斯坦-克莱因-戈尔登方程(Einstein-Klein-Gordon equations)在准稳态近似下构建自引力标量场构型，类比ℓ-玻色星(ℓ-boson stars)方法构造ℓ≥0的"贵族引力原子"(noble gravitational atoms)。

Result: 发现了描述黑洞周围自引力标量场构型的准静态近似解，称为高贵引力原子（noble gravitational atoms），其性质类似于具有闭合电子壳层的惰性原子。
...
### [68] [Infrared Freeze-In of Magnetic Dipole Dark Matter](https://arxiv.org/abs/2512.07927)
*Asher Berlin,Jae Hyeok Chang,Tanner Trickle*

**TL;DR**: 提出了一种通过有效磁偶极矩（effective magnetic dipole moment）与标准模型相互作用的中等质量暗物质（keV-GeV）产生机制，发现当带电重粒子质量与暗物质相近且在动力学混合U(1)'对称性下时，暗物质主要通过红外冻结（infrared freeze-in）产生，并确定了符合宇宙学和天体物理约束的参数空间区域。


<details>
  <summary>Details</summary>

Background: 研究探讨了通过有效磁偶极矩（effective magnetic dipole moment）与标准模型相互作用的keV-GeV质量暗物质（dark matter）的宇宙学产生机制。

Data: 数据来源于理论模拟(simulation)研究，通过假设暗物质与标准模型通过有效磁偶极矩相互作用并耦合到质量相近的重带电粒子，结合新的动力学混合$U(1)'$规范群进行计算。

Method: 采用红外冻结（infrared freeze-in）机制分析暗物质（dark matter）与带电重态（heavy charged states）通过动力学混合$U(1)'$相互作用的热化过程。

Result: 提出了一种通过新的动力学混合$U(1)'$下重带电态的红外冻结机制主导暗物质（dark matter）产生的新机制，其参数空间符合宇宙学和天体物理约束。
...
### [69] [Dark Photons in the Early Universe: From Thermal Production to Cosmological Constraints](https://arxiv.org/abs/2512.08528)
*Xun-Jie Xu,Boting Zhou*

**TL;DR**: 该论文通过分析早期宇宙中暗光子（dark photon）的热产生过程，计算了0.1 keV至100 MeV质量范围内暗光子的冻结丰度（freeze-in yield），发现轻于1 MeV的暗光子共振产生主导且解析估计准确，而较重暗光子的共振与非共振贡献比约为0.14，并推导出0.1-6 MeV质量区间最严格的宇宙学约束（kinetic mixing水平为10^-12~10^-10）。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗光子（dark photon）作为标准模型（Standard Model）通过动力学混合（kinetic mixing）相互作用的一类轻规范玻色子（light gauge bosons）在早期宇宙中的热产生过程及其宇宙学约束。

Data: 研究中使用的数据来源于对暗光子（dark photon）在早期宇宙中热产生过程的数值模拟（simulation），覆盖了0.1 keV至100 MeV的质量范围，并考虑了逆衰变、湮灭和半康普顿等过程。

Method: 采用热产生（thermal production）分析方法，涵盖逆衰变（inverse decay）、湮灭（annihilation）和半康普顿（semi-Compton）过程，并通过解析估计和数值验证计算暗光子（dark photon）产生率与产额。

Result: result：暗光子（dark photon）质量在0.1 MeV至6 MeV范围内时，宇宙学约束对动能混合（kinetic mixing）的探测灵敏度达到$10^{-12}\sim10^{-10}$水平，比恒星冷却和超新星限制更为严格。
