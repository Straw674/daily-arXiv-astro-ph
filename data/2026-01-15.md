...
**TL;DR**: 通过高分辨率观测发现，高质量星团形成区（high-mass cluster-forming regions）中的低质量多重系统（low-mass multiple systems）伴星间距分布峰值（~1200 au）显著低于邻近低质量区域（~4000 au），表明周围介质更高压力导致湍流核心碎裂（turbulent core fragmentation），且恒星密度增加时多重性分数（multiplicity fraction）保持恒定，说明极稠密环境中动力学相互作用会瓦解弱束缚系统。


<details>
  <summary>Details</summary>

Background: 恒星形成过程中的多星系统（multiple stellar systems）形成机制研究，聚焦于高密度星团环境下与低质量恒星形成区的差异。

Data: 数据来源于ALMA（阿塔卡马大型毫米波/亚毫米波阵列）1.33毫米尘埃连续谱观测，空间分辨率约160天文单位（au），覆盖23个高质量星团形成区，共探测到72个低质量多重系统。

Method: 采用阿塔卡马大型毫米波/亚毫米波阵列(ALMA) 1.33毫米尘埃连续谱观测技术，空间分辨率达~160天文单位(au)，结合湍动核心碎裂(turbulent core fragmentation)物理模型分析伴星间距分布。

Result: 在高质量恒星形成区观测到的伴星间距分布峰值(~1200 au)显著低于低质量区域(~4000 au)，表明周围介质更高压力(湍流和密度)导致了更短的分裂尺度。
...
### [2] [The miniJPAS survey: Dissecting galaxy properties across environments with spatially resolved photometry](https://arxiv.org/abs/2601.08911)
...
**TL;DR**: 利用J-PAS（Javalambre-Physics of the Accelerating Universe Astrophysical Survey）先导项目miniJPAS的窄带滤波数据，分析了51个按光谱类型（红/蓝）和环境（星系群/场）分类的星系的解析恒星种群和发射线特性，发现颜色-质量密度关系呈现红区年老、金属丰度高、恒星形成率低而蓝区发射线强、恒星形成率高的明显趋势，径向分布支持由内向外形成模型且红蓝星系差异显著，但环境依赖性较弱可能与样本星系群恒星质量较低有关。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用J-PAS（Javalambre-Physics of the Accelerating Universe Astrophysical Survey）窄带滤波系统观测数据，探究星系演化中环境因素对恒星种群和发射线特性的影响。

Data: 数据来源于J-PAS（Javalambre-Physics of the Accelerating Universe Astrophysical Survey）的窄带滤光片观测，以及其前驱项目miniJPAS对1平方度天区使用相同滤光片系统的观测数据。

Method: 采用椭圆环（elliptical annuli）按0.7倍有效半径（R_EFF）间距划分径向轮廓，结合由内向外分割法分析恒星形成历史，并利用贝叶斯SED拟合代码BaySeAGal获取恒星种群参数，同时通过人工神经网络（artificial neural networks）估算Hα、Hβ、[NII]和[OIII]发射线的等效宽度。

Result: result：研究发现质量密度-颜色图中存在明显趋势，即密度更高、更红的区域年龄更大、金属丰度更高且具有较低的比恒星形成率(sSFR)，而更蓝、密度较低的区域则表现出更强的发射线和更高的sSFR。
...
### [3] [Measuring the Vertical Structure of Active Galactic Nuclei Disks with Transformer Models and the Vera C. Rubin Observatory](https://arxiv.org/abs/2601.08926)
*Amy Secunda,Sebastian Wagner-Carena,Helen Qu,Shirley Ho*

**TL;DR**: 提出基于Transformer的机器学习模型，显著提升了对活动星系核(AGN)吸积盘垂直结构长负时滞(long negative lag)的检测精度（准确率98%），优于传统插值互相关函数(54%)和javelin方法(21%)。


<details>
  <summary>Details</summary>

Background: 研究主题为利用反响映射（reverberation mapping）技术探测活动星系核（AGN）吸积盘的径向和垂直结构，特别是针对长负时滞（long negative lag）的检测挑战。

Data: 研究中使用的数据来源于模拟的维拉·鲁宾天文台(Vera Rubin Observatory)活动星系核(AGN)光变曲线(mock Rubin AGN light curves)。

Method: 采用基于Transformer的机器学习模型检测活动星系核（AGN）光变曲线中的长负时滞（long negative lag）和短时滞（short lag）。

Result: 基于Transformer的机器学习模型在模拟Rubin AGN光变曲线中检测长负延迟（long negative lag）的准确率达98%，显著优于传统插值互相关函数（54%）和javelin方法（21%）。
...
**TL;DR**: 论文核心贡献是揭示了超大质量黑洞（BH）自旋与核星团（NSC）多极质量分布共同作用下通过显著重塑相空间（放大混沌散射或稳定特定轨道族）调控星系中心动力学架构的机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超大质量黑洞（BH）与核星团（NSC）、分子气体及非对称物质分布（如盘或晕）共同引力作用下星系中心强非线性动力学与混沌轨道行为的耦合机制。

Data: 数据来源于采用多极展开势的数值模拟（simulation），其中中心致密天体使用Artemova-Bjornsson-Novikov伪牛顿势描述，周围晕（halo）则建模为轴对称壳状质量分布并展开至三阶多极项。

Method: 采用多极展开势（multipolar expansion potential）结合Artemova-Bjornsson-Novikov伪牛顿势（pseudo-Newtonian potential）描述克尔黑洞（Kerr-like BH）自旋特性，并将周围晕（halo）建模为轴对称壳状质量分布（axisymmetric, shell-like mass distribution）并展开至三阶多极项以捕捉非对称性。

Result: 黑洞自旋（BH spin）通过改变相位空间结构显著影响星系中心动力学，既可放大核晕（halo）不对称性导致的混沌散射，也能稳定特定轨道族。
...
### [5] [One cloud is not enough: extreme conditions bias chemical abundances in high-redshift galaxies](https://arxiv.org/abs/2601.08939)
...
**TL;DR**: 通过多云光致电离模型（HOMERUN）分析高红移（$z=2 - 6$）星系案例，发现未解析高密度云团会显著影响极光（auroral）线流量并导致电子温度法（$T_{\mathrm{e}}$-method）低估金属丰度0.15-0.3 dex，揭示了不同诊断手段的丰度差异可能源于电离/密度结构而非化学不均匀性，同时证实氮富集区与正常组分共存现象，并指出极高电离线可仅由恒星形成产生。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST观测数据，结合多云光致电离模型（HOMERUN）分析高红移星系（$z=2 - 6$）电离星际介质（ISM）的物理条件与化学丰度，揭示经典诊断方法在极端环境中的局限性及其导致的金属丰度低估问题。

Data: 数据来源于JWST（James Webb Space Telescope）对三个高红移星系（MARTA 4327、Sunburst Arc和RXCJ2248-ID）的观测，结合多云光致电离模型框架（HOMERUN）对静止紫外和光学数据进行分析。

Method: 采用多云团光致电离模型（HOMERUN）框架对高红移星系电离星际介质（ISM）进行自洽建模，结合静止紫外和光学波段数据诊断电离状态与化学丰度分布。

Result: 高密度团块（high-density clumps）对极光线的贡献超过观测总通量的一半，但对标准光学密度示踪剂的影响可忽略不计。
...
### [6] [Polarization echoes from past nuclear activity in the quasi-periodic eruption source GSN 069](https://arxiv.org/abs/2601.08940)
...
**TL;DR**: 通过光学偏振测量发现GSN 069的径向递增偏振模式及其与延伸发射线区(EELR)的几何关联，支持其过去活动源于褪色活动星系核(AGN)而非潮汐瓦解事件(TDE)率升高。


<details>
  <summary>Details</summary>

Background: X射线准周期性喷发(QPEs)是观测自邻近低质量星系核区的重复性、高振幅软X射线爆发，其起源仍是吸积变率物理学的未解之谜。

Data: 数据来源于2019年9月6日使用VLT（甚大望远镜）的FORS2（焦面减光和低色散光谱仪）设备对GSN 069进行的成像偏振测量和光谱偏振测量观测。

Method: 采用光学偏振测量（imaging polarimetry）和光谱偏振测量（spectropolarimetry）技术分析GSN 069的偏振特性。

Result: 径向偏振的增加证实了GSN 069存在已关闭的活动星系核（AGN），其偏振角度与延伸的[OIII]、[NII]和Hα气体分布轴对齐，表明过去存在类似环面（torus-like）的结构。
...
### [7] [Resolved ISM properties and scaling relations in the barred galaxy NGC 3627: constraints from NIKA2 observations](https://arxiv.org/abs/2601.08941)
...
**TL;DR**: 基于NIKA2观测数据，研究发现NGC 3627星系中尘埃发射、自由-自由辐射和同步辐射的分解揭示了棒驱动气体流和潮汐相互作用对星际介质（ISM）特性、恒星形成效率及尘埃分布的显著影响，其中2毫米波段18%的射电发射峰值出现在南部棒端高恒星形成区，而孤立恒星形成区表现出更高的尘埃产生效率和有效产率。


<details>
  <summary>Details</summary>

Background: 研究NGC 3627中恒星形成、星际介质（ISM）组分与尘埃特性的相互作用及其对星系结构和化学演化的影响。

Data: 数据来源于IMEGIN大型项目提供的NIKA2 1.15和2毫米新观测数据，并结合3.4微米至6厘米波段的补充数据用于光谱能量分布拟合。

Method: 采用THEMIS尘埃模型在HerBIE代码中进行光谱能量分布拟合（从3.4微米到6厘米），并分解毫米-厘米波段的尘埃、自由-自由和同步辐射成分。

Result: 在NGC 3627中，动力学过程（如棒驱动气体流和潮汐相互作用）对星系结构形成、恒星形成效率及尘埃分布具有关键影响。
...
### [8] [Spatially Resolved Star Formation relations in local LIRGs along the complete merger sequence](https://arxiv.org/abs/2601.08980)
...
**TL;DR**: 通过分析27个近邻亮红外星系(LIRGs)的星际介质(ISM)特性，发现星系合并过程中分子气体团块的自引力调控恒星形成效率的演化规律：早期孤立星系气体和恒星形成较少，而晚期合并中心区域气体积累导致恒星形成效率提升，且肯尼卡特-施密特(KS)关系斜率变陡。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近邻亮红外星系（LIRGs）中星际介质（ISM）在~100 pc尺度上的性质及其与恒星形成（SF）的关系，特别关注分子气体表面密度随星系并合（merger）进程的变化规律。

Data: 数据来源为ALMA（阿塔卡马大型毫米波/亚毫米波阵列）的CO(2-1)观测数据以及HST（哈勃太空望远镜）的Pa-alpha或Pa-beta图像。

Method: 采用两种互补方法（光束尺度未分辨视线区域和物理识别的分子气体团块）研究恒星形成（SF）与分子气体面密度关系，并结合ALMA CO(2-1)观测和HST Pa-alpha/Pa-beta图像绘制分子气体分布及恒星形成示踪。

Result: 在晚期合并阶段，分子气体团块（clumps）的恒星形成效率（SF efficiency）与束缚参数（boundedness parameter）的关系从早期阶段的近似平坦转变为正相关，表明团块自引力仅在合并中后期开始调控恒星形成过程。
...
### [9] [Wind-fed Supermassive Black Hole Accretion in the Ultracompact Dwarf Galaxy M60-UCD1](https://arxiv.org/abs/2601.09199)
*Zhao Su,Zhiyuan Li,Meicun Hou*

**TL;DR**: 研究发现M60-UCD1中超大质量黑洞（SMBH）通过吸积约1500颗渐近巨星分支（AGB）恒星产生的星风形成冷气体盘和热冕，产生与观测一致的X射线辐射，揭示了无气体环境下恒星风供能的弱吸积机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超致密矮星系（UCDs）中由恒星风供能的超大质量黑洞（SMBH）吸积机制，特别是M60-UCD1中观测到的X射线辐射与弱吸积过程的关联性。

Data: 数据来源于三维流体动力学模拟（three-dimensional hydrodynamical simulations），模拟了来自约1500颗渐近巨星分支（asymptotic giant branch, AGB）恒星的星风（stellar winds）被超大质量黑洞（SMBH）吸积的过程，并考虑了周围星际或星系团介质（interstellar or intracluster medium）的冲压压力（ram pressure）作用。

Method: 采用三维流体动力学模拟研究恒星风供积（wind-fed accretion）机制，并探讨环境介质（interstellar/intracluster medium）的冲压压力（ram pressure）作用。

Result: M60-UCD1中的超大质量黑洞（SMBH）通过恒星风吸积形成冷气体盘和热冕，产生与观测一致的X射线辐射，且环境介质的冲压剥离使吸积率和光度降低约一半。
...
### [10] [The Quasar Feedback Survey: Revealing the importance of sensitive radio imaging for AGN identification deeper into the radio-quiet regime](https://arxiv.org/abs/2601.09218)
...
**TL;DR**: 研究发现低功率射电宁静（radio-quiet）类星体（$z<0.2$，$L_{\rm [O III]} > 10^{42.1}$ erg s$^{-1}$）普遍存在活动星系核（AGN）驱动的同步辐射活动，其中38%的样本显示出无法用恒星形成解释的AGN特征（如致密喷流或激波）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索低红移（z<0.2）光学选择类星体（quasar）在射电宁静（radio quiet）状态下普遍存在的活动星系核（AGN）驱动同步辐射活动及其反馈机制。

Data: 数据来源于甚大天线阵(VLA)在1.4 GHz和6 GHz波段对29个光学选[O III]发射线光度(L[O III] > 10^42.1 erg s^-1)、红移z<0.2的类星体进行亚角秒级(~0.3-1角秒；~1-3 kpc)射电成像观测，这些样本选自扩展的类星体反馈巡天(QFeedS)项目，射电光度(L_1.4 GHz)范围为10^22.6-10^26.3 W Hz^-1。

Method: 采用甚大天线阵(VLA)在1.4GHz和6GHz波段对29个光学选类星体进行亚角秒级(∼0.3-1角秒)高分辨率成像观测，结合形态学、光谱指数和亮度温度诊断分析射电活动特征。

Result: 在射电宁静（radio-quiet）类星体中普遍存在活动星系核（AGN）驱动的同步辐射活动，包括紧凑喷流或风驱动外流结构以及陡峭谱指数特征。
...
### [11] [The ALMaQUEST Survey XVII: Unveiling Multiple Quenching Pathways in Green Valley Galaxies via Molecular Gas and Quenching Timescale Analyses](https://arxiv.org/abs/2601.09225)
...
**TL;DR**: 通过分析28个绿谷(GV)星系中分子气体含量($f_{gas}$)和恒星形成效率(SFE)的空间分布，发现绿谷星系的淬灭存在三种路径：35.7%为$f_{gas}$主导、39.3%为SFE主导、25%为混合模式，且随着sSFR降低逐渐转向SFE主导淬灭，75%的绿谷星系淬灭时标($τ_{decay}$)短于1吉年。


<details>
  <summary>Details</summary>

Background: 研究探讨了绿谷星系(GV)中恒星形成活动停止(quenching)的驱动机制多样性及其与分子气体含量(f_gas)和恒星形成效率(SFE)的关系。

Data: 数据来源于ALMaQUEST巡天和额外文献样本中的28个绿谷(GV)星系的空间分布观测数据。

Method: 通过测量特定恒星形成率(sSFR)受抑制区域的分子气体分数偏移量(Δf_{gas})和恒星形成效率偏移量(ΔSFE)，并与恒星形成主序星系的分辨率标度关系进行比较，从而分类GV星系的淬灭驱动模式。

Result: 绿谷星系(GV)的淬灭由多种机制共同驱动，包括分子气体比例($f_{gas}$)主导(35.7±13.2%)、恒星形成效率(SFE)主导(39.3±14.0%)和混合模式(25.0±10.6%)，且随着特定恒星形成率(sSFR)降低，SFE主导的淬灭作用增强。
...
### [12] [New Estimate for the Cosmic Ray-Induced $\rm H_2$ Photodissociation Rate in the Interstellar Medium](https://arxiv.org/abs/2601.09387)
*O. Sipilä,P. Caselli,M. Padovani,D. Galli,T. Grassi,H. R. Hrodmarsson,S. S. Jensen,E. Roueff*

**TL;DR**: 研究发现宇宙射线(CR)诱导的氢分子(H2)解离速率新值k_diss,CR(H2)=0.831ζ显著低于莱顿数据库原有估计，该修正使化学模型中甲醇丰度预测值降低超过一个数量级。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际介质中宇宙射线（CRs）通过激发和辐射衰变产生的紫外（UV）光子场对分子云内部化学过程的影响，特别是对H₂分子解离速率及其在复杂有机分子形成中的作用。

Data: 研究中使用的数据来源于对宇宙射线（CRs）诱导的氢分子（H2）解离过程的模拟计算，并结合了化学模型（chemical models）的系列运行结果。

Method: 采用蒙特卡罗模拟方法重新计算了宇宙射线(CR)诱导的H₂分子解离速率，并基于更新的H₂丰度和光解离截面数据推导出新解离速率公式k_{diss, CR}(H₂)=0.831ζ。

Result: 宇宙射线（CRs）诱导的氢分子（H₂）解离速率新测定值为k_{\rm diss, CR}(\mbox{$\rm H_2$})=0.831ζ，显著低于莱顿数据库原有值，且对甲醇丰度等化学模拟结果产生量级影响。
...
### [13] [Discovery of a new open cluster as a companion to Czernik 38 cluster and its associated complex tide, using Gaia DR3](https://arxiv.org/abs/2601.09670)
*Nasser M. Ahmed*

**TL;DR**: 基于盖亚(Gaia)DR3数据发现新疏散星团Nasser 1，其与Czernik 38构成受较差旋转潮汐撕裂的原始双星团系统，位于船底-人马旋臂。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用盖亚(Gaia)卫星数据在船底-人马旋臂发现并表征一个与Czernik 38存在物理关联的新疏散星团Nasser 1，探讨其作为年轻原始双星系统的动力学演化历史。

Data: 数据来源为盖亚(Gaia)卫星第三次数据释放(DR3)。

Method: 采用可变成员概率阈值技术（variable membership probability threshold technique）结合pyUPMASK算法进行星团成员确认，并运用金斯轮廓（King profile）和颜色-星等图（CMD）分析物理参数。

Result: 在船底-人马座旋臂中发现了一个新的疏散星团Nasser 1，与Czernik 38构成年轻原始双星系统，两者物理参数一致且显示曾被较差旋转撕裂的迹象。
...
**TL;DR**: 通过谐波空间计算验证了脉冲星计时阵列重叠缩减函数（ORFs）受我们相对于随机引力波背景静止参考系的特殊速度影响，并扩展了该方法到自旋1引力波（spin-1 GWs）和天体测量相关性分析。


<details>
  <summary>Details</summary>

Background: 研究聚焦于脉冲星计时阵（pulsar timing array）重叠减少函数（ORFs）中由我们相对于随机引力波背景（stochastic gravitational-wave background）静止参考系的特殊速度（peculiar velocity）所引起的影响。

Data: 数据来源于对脉冲星计时阵列（pulsar timing array）重叠减少函数（ORFs）的谐波空间（harmonic-space）计算。

Method: 采用谐波空间（harmonic-space）计算方法分析脉冲星计时阵重叠减少函数（ORFs），并验证了其与随机引力波背景相对运动的影响。

Result: 谐波空间计算证实了最近关于脉冲星计时阵列重叠减少函数（ORFs）受我们相对于随机引力波背景静止参考系的特殊速度影响的结果。
...
**TL;DR**: 利用SZ效应数据测试四种星系团电子压力分布模型（gNFW、β模型、多方、指数）后发现，所有模型均能拟合观测但无显著优势，且存在质量-红移依赖性残余趋势，表明基于SZ的群体研究难以精确约束压力模型，同时非gNFW模型具有同等效果且物理意义更明确。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用SZ（Sunyaev-Zel'dovich）效应数据检验星系团电子压力剖面（electron pressure profile）的四种函数形式（gNFW、β-model、polytropic和exponential）的普适性。

Data: 数据来源于ACT-DR4星系团样本（3496个）和ACT-DR6康普顿参数y图（覆盖约13000平方度）。

Method: 采用多阶段马尔可夫链蒙特卡洛（MCMC）方法对四种理论压力模型（广义纳瓦罗-弗伦克-怀特模型（gNFW）、β模型、多方模型和指数模型）的自由参数进行约束，并通过堆叠3496个ACT-DR4星系团在ACT-DR6康普顿参数y图上的数据提取角康普顿轮廓进行建模分析。

Result: 所有函数形式都能在误差范围内有效再现测量到的康普顿参数y轮廓，没有明显更优的模型。
...
**TL;DR**: 在红移$z\gtrsim 10$的早期宇宙中，原初黑洞（PBHs）作为暗物质（DM）组分的质量函数通过小尺度功率增强促使暗物质晕提前坍缩，并通过动力学摩擦驱动大质量PBHs向晕中心迁移，从而形成类似UHZ1系统的恒星与中心黑洞质量匹配的高红移天体。


<details>
  <summary>Details</summary>

Background: 研究探讨了高红移（$z\gtrsim 10$）天体（如UHZ1系统）的形成机制，重点关注原初黑洞（PBHs）作为暗物质（DM）的重要组成部分如何通过小尺度功率谱增强和动力学摩擦促成早期暗物质晕的坍缩及中心黑洞的形成。

Data: 数据来源于空间观测（space-based observations）发现的UHZ1系统（红移z=10.1）及其他类似性质的高红移（z≳10）天体，并结合原始黑洞（PBHs）作为暗物质（DM）组分的理论模型进行模拟分析。

Method: 采用原初黑洞（PBHs）作为暗物质（DM）组分的动力学摩擦模型，量化了不同质量PBHs在晕核（halo）中心聚集的演化过程。

Result: 在原始黑洞(PBHs)作为暗物质(DM)重要组成部分的假设下，通过小尺度功率增强导致暗物质晕早期坍缩，并经由二体弛豫过程使大质量PBHs向晕中心迁移，可形成与UHZ1系统参数相似的早期天体。
...
**TL;DR**: 通过结合DESI DR2重子声学振荡、普朗克CMB数据及Ia型超新星观测，验证了束缚暗能量冷暗物质模型(BDE-CDM)的理论预言，发现其无自由参数的暗能量状态方程演化($w_0=-0.9298\pm0.0003$)显著优于ΛCDM模型，并在$k≈4.3h\,\mathrm{Mpc}^{-1}$处预测物质功率谱增强25%，为粒子物理与宇宙学观测提供了自洽的理论框架。


<details>
  <summary>Details</summary>

Background: 研究探索了基于超对称SU(3)暗规范群（含6种味）中最轻介子场的束缚暗能量(BDE)模型，其暗能量由逆幂律势能$V(φ) = Λ_{c}^{4+2/3} φ^{-2/3}$驱动，无需额外自由参数，通过规范耦合统一条件确定凝聚尺度$Λ_c$和转变时期$a_c$，为粒子物理与宇宙学观测建立理论联系。

Data: 数据来源包括DESI DR2重子声波振荡(BAO)测量、普朗克(Planck)宇宙微波背景(CMB)数据以及三类Ia型超新星数据集（PantheonPlus、Union3和DESY5）。

Method: 采用基于超对称SU(3)暗规范群（N_f=6味）的逆幂律势能V(φ)=Λ_c^(4+2/3)φ^(-2/3)模型，其中凝聚标度Λ_c和转变时期a_c由规范耦合统一条件确定。

Result: BDE-CDM模型通过暗能量状态方程从早期相对论性（w=1/3）经刚度主导相（w≈1）演化至当前值w0=-0.9298±0.0003，始终维持w>-1且不含自由参数，其凝聚标度Λc=43.93±0.13 eV和转变时期ac=(2.489±0.007)×10^-6与规范耦合统一理论一致。
...
**TL;DR**: 该研究基于潮汐排列和潮汐扭矩（TATT）形式建立了剪切三点相关函数和质量孔径统计偏度的本征排列污染模型，发现TATT模型显著影响3PCF的倾斜三角形构型，且其高阶效应会抑制两点函数信号但增强质量孔径偏度信号，联合2PCF+3PCF分析有助于打破参数简并并提供更稳健的宇宙学和本征排列振幅约束。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用潮汐排列和潮汐扭矩（TATT）形式主义建立剪切三点相关函数和质量孔径统计偏度的本征排列污染模型，旨在解决宇宙学观测中本征排列效应对弱引力透镜信号的影响。

Data: 数据来源于暗能量巡天（Dark Energy Survey）第三年的最佳拟合参数和层析红移分布（tomographic redshift distributions）。

Method: 采用潮汐排列和潮汐扭矩（TATT）形式体系计算了本征排列双谱分量，并扩展了包含速度-剪切（VS）参数的TATT模型。

Result: TATT模型显著影响了三点相关函数(3PCF)的倾斜三角形构型，且其高阶效应会对两点函数和质量孔径偏度产生相反影响（前者信号减弱，后者信号增强）。
...
**TL;DR**: 开发了贝叶斯代码Owl-z，通过光度和位置数据有效识别红移(z)≥7的类星体（quasars），并能区分褐矮星（brown dwarfs）和中红移早型星系（early-type galaxies），在欧几里得宽视场巡天中验证了对亮源（H_E≲22）的稳定分类性能。


<details>
  <summary>Details</summary>

Background: 开发贝叶斯（Bayesian）代码Owl-z以识别宽视场光学和近红外巡天中的高红移（z ≥ 7）类星体（quasars），并区分其污染源（褐矮星（brown dwarfs）和中红移早型星系（early-type galaxies））。

Data: 数据来源为宽视场光学和近红外巡天（wide field optical and near-infrared surveys）中目标的光度测量数据及其银经（galactic longitude）和银纬（galactic latitude）。

Method: 采用贝叶斯(Bayesian)方法构建的光谱分类模型，通过处理目标天体的测光数据及银道坐标，计算高红移类星体、褐矮星(brown dwarfs)和中红移早型星系(early-type galaxies)的概率分布。

Result: Owl-z是一种贝叶斯(Bayesian)代码，能有效识别宽视场光学和近红外巡天中红移(z)≥7的类星体(quasar)，并区分污染源（褐矮星(brown dwarf)和中红移早型星系(early-type galaxy)）。
...
**TL;DR**: 通过对比断裂幂律（BPL）和椭圆幂律（EPL）透镜质量模型，发现BPL模型因径向自由度更高导致哈勃常数（H0）估计值偏移（75 vs 61 km/s/Mpc），凸显透镜质量分布假设和点扩散函数（PSF）建模对时间延迟宇宙测量精度的关键影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于强引力透镜（strong gravitational lensing）宇宙学中透镜质量分布假设对哈勃常数（Hubble constant）测量的影响。

Data: 数据来源包括模拟透镜（simulated lenses）和四重成像类星体WGD~2038-4008的成像与运动学数据（imaging and kinematic data）。

Method: 采用断裂幂律（Broken Power Law, BPL）质量模型结合数值方法计算图像位置的时间延迟，并在Lenstronomy框架内实现。

Result: 采用断裂幂律(BPL)透镜质量模型推算的哈勃常数($H_0$)为$75^{+23.1}_{-16.3} \ \mathrm{km \ s^{-1} \ Mpc^{-1}}$，比椭圆幂律(EPL)模型结果($61^{+19.2}_{-13.2}$)更高，差异主要源于内区质量分布($θ<0.2''$)和点扩散函数(PSF)重建的不确定性。
...
**TL;DR**: 研究发现超轻暗物质（ULDM）与致密恒星系统的相互作用呈现两种演化模式：极紧凑系统以恒星弛豫主导保持高密度，而较扩展系统则因ULDM加热效应瓦解，并揭示了类似Segue 1的超暗弱矮星特征，提出了量化两者竞争关系的相图参数。


<details>
  <summary>Details</summary>

Background: 研究聚焦超轻暗物质（ULDM）场与致密恒星系统相互作用的动力学机制及其对暗物质约束的影响。

Data: 使用数值模拟(numerical simulations)研究了超轻暗物质(ULDM)晕中致密恒星系统的演化。

Method: 采用数值模拟方法研究超轻暗物质（ULDM）晕中致密恒星系统的演化，重点分析内部恒星弛豫与ULDM诱导加热的相互作用机制。

Result: 超轻暗物质（ULDM）场干扰产生时变引力势涨落，通过数值模拟发现致密恒星系统演化由内部恒星弛豫与ULDM加热共同主导，并引入无量纲参数量化两者相对重要性，最终构建了演化相图。
...
**TL;DR**: 通过结合快速射电暴（FRB）与引力波（GW）标准汽笛、强引力透镜（SGL）时间延迟或21厘米强度映射（IM）三种多信使方法，有效打破了哈勃常数（H0）与重子密度（Ωb）的简并性，在ΛCDM、wCDM和CPL模型中分别实现了优于（1%、1%）、（1.5%、2%）和（2%、3.5%）的联合测量精度，为解决哈勃张力和缺失重子问题提供了新途径。


<details>
  <summary>Details</summary>

Background: 宇宙学研究面临哈勃常数测量差异（Hubble tension）和本地宇宙重子物质缺失（missing baryon problem）两大核心挑战。

Data: 研究中使用的数据来源包括快速射电暴（FRBs）、引力波（GW）标准汽笛、强引力透镜（SGL）时间延迟以及21厘米强度映射（IM）。

Method: 采用快速射电暴（FRB）与引力波（GW）标准汽笛、强引力透镜（SGL）时间延迟及21厘米强度映射（IM）三种多信使方法相结合，打破哈勃常数（H0）与重子密度（Ωb）之间的简并性。

Result: 结合快速射电暴（FRB）与引力波（GW）标准汽笛、强引力透镜（SGL）时间延迟或21厘米强度映射（IM）能有效打破哈勃常数（H0）与重子密度（Ωb）的简并性，在ΛCDM、wCDM和CPL模型中分别实现优于（1%、1%）、（1.5%、2%）和（2%、3.5%）的联合约束精度。
...
**TL;DR**: 通过中性氢分数（$x_\mathrm{HI}(z)$）和普朗克宇宙微波背景（CMB）数据独立测定了光学深度（$τ=0.0552^{+0.0019}_{-0.0026}$），验证了重子声波振荡（BAO）与CMB在$Λ$CDM框架下的轻微差异（$2.4σ$），并给出中微子质量总和上限（$Σm_ν<0.0550$ eV）及其与振荡实验的$2.2σ$差异，暗示可能存在超出$Λ$CDM的物理机制。


<details>
  <summary>Details</summary>

Background: 研究探讨了重子声学振荡（BAO）与宇宙微波背景（CMB）在ΛCDM框架下的轻微差异，聚焦于光学深度（τ）可能被低估的问题及其对宇宙学模型的潜在影响。

Data: 数据来源包括基于地面光学望远镜和JWST（James Webb Space Telescope）观测的莱曼α森林（Lyman-α forest）及阻尼翼吸收（damping-wing absorption）测量数据，以及普朗克（Planck）CMB功率谱（不包括大尺度E模偏振数据）。

Method: 采用中性氢分数（neutral hydrogen fraction, x_HI）红移演化测量结合普朗克宇宙微波背景（CMB）功率谱（排除大尺度E模偏振数据）的方法独立约束光学深度（τ）。

Result: 基于中性氢分数演化测量和普朗克CMB数据（排除大尺度E模偏振）得出光学深度（τ）为0.0552^{+0.0019}_{-0.0026}，与包含E模偏振的CMB结果一致，但吸收模型潜在系统误差导致τ=0.0552^{+0.0075}_{-0.0049}。
...
### [24] [Explainable Galaxy Interaction Prediction with Hybrid Attention Mechanisms](https://arxiv.org/abs/2601.08872)
*Sathwik Narkedimilli,Satvik Raghav,Om Mishra,Mohan Kumar,Aswath Babu H,Tereza Jerabkova,Manish M,Sai Prashanth Mallellu*

**TL;DR**: 提出了一种结合AG-XCaps、H-SNN和ResNet-GRU架构的可解释注意力神经集成模型，在Galaxy Zoo DESI数据集上以高精度（F1=0.97）和低误报（23次）实现了星系相互作用分类，为欧几里得(Euclid)和LSST等大规模巡天提供了轻量级解决方案。


<details>
  <summary>Details</summary>

Background: 星系相互作用分类面临形态模式复杂和深度学习模型可解释性有限的挑战。

Data: 数据来源于Galaxy Zoo DESI数据集。

Method: 采用注意力神经集成（attentive neural ensemble）方法结合AG-XCaps、H-SNN和ResNet-GRU架构，并利用LIME增强可解释性。

Result: 提出的注意力神经集成模型在星系相互作用分类任务中实现了精确度0.95、召回率1.00、F1分数0.97和准确率96%，显著优于随机森林基线（假阳性23比70）。
...
### [25] [Habitable Worlds Observatory Living Worlds Working Group: Surface Biosignatures on Potentially Habitable Exoplanets](https://arxiv.org/abs/2601.08883)
...
**TL;DR**: 这篇论文的核心贡献是确定了宜居世界天文台（HWO）在探测太阳系外岩石宜居行星表面生物色素（surface biosignatures）所需的关键信噪比（SNR）和日冕仪（coronagraph）配置，发现500-1100纳米波段内信噪比20-40可有效识别不同地质时期（太古宙、元古宙和现代）的生物特征，并强调多通道无间隙日冕仪对区分生物与非生物特征至关重要。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用宜居世界天文台（HWO）探测太阳系外岩石宜居行星表面生物标志物（surface biosignatures）以寻找生命迹象，并探讨其在假阳性和假阴性框架下的解释方法。

Data: 数据来源包括基于不同地质时期（太古宙、元古宙和现代）大气成分的模拟观测数据，以及覆盖500-1100纳米波长范围的信噪比（SNR）为20-40的测量需求分析。

Method: 采用“地球随时间演变”情景评估表面生物标志物（surface biosignatures）的探测需求，通过分析光合与非光合色素（photosynthetic and non-photosynthetic pigments）引起的行星尺度光谱特征，结合多波段日冕仪（coronagraph）并行通道设计提升探测能力。

Result: 在古太古代(Archean)、元古代(Proterozoic)和现代大气成分条件下(15%覆盖率、50%云量)，检测行星表面生物色素(biopigments)需要在500-1100纳米波段达到20-40的信噪比(SNR)。
...
### [26] [Planet-Host Stars Across the Galaxy in the 2040s](https://arxiv.org/abs/2601.08890)
*M. Tsantaki,K. Biazzo,F. Zahra Majidi,G. Tautvaisiene,I. Busa*

**TL;DR**: 论文核心贡献是提出需开展大规模高分辨率光谱巡天（spectroscopic survey）以研究不同银河系环境（Galactic environments）中行星宿主恒星（planet-host stars）的形成、演化和多样性。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于2040年代系外行星（exoplanet）领域将从发现转向理解行星系统形成、演化及多样性机制，需通过大规模高分辨率光谱巡天（spectroscopic survey）解析不同恒星与银河系环境（Galactic environments）的影响。

Data: 数据来源包括盖亚(Gaia) DR5、TESS、PLATO、罗曼(Roman)及其后续任务的大规模高分辨率光谱巡天数据，覆盖薄盘、厚盘、核球、晕、星团和星协等不同银河系环境，并包含非宿主星的对照样本。

Method: 采用大规模高分辨率光谱巡天（large-scale, high-resolution spectroscopic survey）技术，覆盖多种银河系环境（thin and thick disks, bulge, halo, clusters, associations）并包含非宿主恒星对照样本。

Result: 到2040年代，行星科学的关键瓶颈将转向理解行星系统在不同恒星和银河环境中的形成、演化及多样化机制。
...
### [27] [LUMOS : Linear programming Utility for Multi-messenger Optical Scheduling](https://arxiv.org/abs/2601.08912)
*Yogesh P. Wagh,Michael W. Coughlin,Leo P. Singer,Varun Bhalerao*

**TL;DR**: 提出基于混合整数线性规划（MILP）的LUMOS调度算法，显著提升对引力波事件（GW events）光学对应体的探测效率，相比gwemopt平均累积概率提高84.7%，适用于地面（ZTF）和空间（M4OPT）多信使观测。


<details>
  <summary>Details</summary>

Background: 引力波(LIGO-Virgo-KAGRA)探测开启了多信使(multi-messenger)天体物理研究新范式，但电磁对应体因定位误差大而难以捕获。

Data: 数据来源于LIGO-Virgo-KAGRA合作组发布的1199个引力波事件（GW events）O4观测数据以及Zwicky Transient Facility（ZTF）的广域光学巡天观测数据。

Method: 采用混合整数线性规划（Mixed Integer Linear Programming, MILP）方法构建LUMOS调度器，通过最大覆盖问题选择观测区域并优化观测时序以提升累积探测概率。

Result: LUMOS调度器采用混合整数线性规划（MILP）方法，通过最大覆盖问题选择观测区域并优化时序安排，在满足观测约束条件下使累积概率最大化。
...
### [28] [Using 23 Years of ACS/SBC Data to Understand Backgrounds:Explaining & Predicting Background Variations](https://arxiv.org/abs/2601.09682)
*Christopher J. R. Clark,Roberto J. Avila,Alyssa Guzman,Norman A. Grogin*

**TL;DR**: 通过分析8000多组哈勃太空望远镜ACS/SBC（Advanced Camera for Surveys/Solar Blind Channel）档案数据并建立机器学习模型，发现SBC背景噪声主要受太阳高度角（Solar elevation）、太阳分离角（Solar separation angle）、地球临边角（Earth limb angle）、SBC温度和目标银纬（Galactic latitude）五个参数共同驱动。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于哈勃太空望远镜（Hubble Space Telescope）ACS/SBC（Advanced Camera for Surveys/Solar Blind Channel）23年观测数据中背景辐射（background）显著变化的成因分析，特别是短波滤光片（shorter-wavelength filters）中气辉（airglow）主导但驱动机制未明的问题。

Data: 数据来源于哈勃太空望远镜（Hubble Space Telescope）高级巡天相机（ACS）的太阳盲通道（Solar Blind Channel, SBC）23年存档观测数据，共分析了超过8000次观测。

Method: 采用机器学习模型分析23个观测参数（包括太阳高度角、太阳分离角、地球临边角、SBC温度和目标银纬）以预测哈勃太空望远镜ACS/SBC的背景变化。

Result: 背景变化主要由太阳高度角（Solar elevation）、太阳分离角（Solar separation angle）、观测地球临边角（Earth limb angle of observation）、SBC温度（SBC temperature）和目标银纬（target Galactic latitude）决定。
...
### [29] [HOPS-288: A Laboratory for Complex Organics in Proto-multiple Systems](https://arxiv.org/abs/2601.08948)
...
**TL;DR**: 通过ALMA观测原三星系统HOPS-288（视为原双星系统），发现两个成员均存在富含复杂有机分子（COMs）的热核（hot corino）以及中间可能存在激波或桥接结构的组分，且两者COM柱密度相似，暗示原双星/多星系统间可能存在复杂有机化学相似性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于年轻恒星天体（YSOs）中复杂有机分子（COMs）与生命前化学的潜在关联及其作为暖或受激气体示踪剂的作用。

Data: 数据来源于ALMA（阿塔卡马大型毫米波/亚毫米波阵列）对原三合星系统HOPS-288的观测，重点关注其物理结构、运动学和复杂有机分子（COMs）组成。

Method: 采用ALMA（阿塔卡马大型毫米波/亚毫米波阵列）观测技术分析HOPS-288原三合星系统的物理结构、运动学和复杂有机分子（COMs）组成。

Result: 在HOPS-288原双星系统中发现两个富含复杂有机分子（COMs）的热核（hot corino）和一个由CH$_3$OH和CH$_3$CHO示踪的中间成分，可能暗示原双星/多星系统间存在复杂有机化学相似性。
...
### [30] [The Structure and Kinematics of Three Class 0 Protostellar Jets from JWST](https://arxiv.org/abs/2601.09587)
...
**TL;DR**: 利用JWST观测发现三颗深埋原恒星（B335、HOPS 153、HOPS 370）在2000天文单位内的喷流形态和运动学特征，其中[Fe II]发射线可追踪喷流内部1000-2000天文单位范围，喷流呈现摆动、非单调增宽（2.1°-<10.1°）及红蓝移瓣不对称性，表明 circumstellar disks（原行星盘）与磁场存在复杂相互作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（James Webb Space Telescope）观测三颗深埋原恒星（protostars）2000天文单位（au）范围内的喷流（jets），揭示其形态、运动学特征及与周围环境的相互作用。

Data: 数据来源于JWST（James Webb Space Telescope）的NIRSpec和MIRI IFUs对三个原恒星（B335、HOPS 153和HOPS 370）在2.9-27微米波段的观测，捕获了激波电离的精细结构线发射。

Method: 采用JWST（James Webb Space Telescope）的近红外光谱仪（NIRSpec）和中红外成像光谱仪（MIRI IFUs）观测激波电离的精细结构线发射，分析[Fe II]等离子的空间分布和运动学特征。

Result: [Fe II]发射能追踪原恒星1000至2000天文单位范围内的喷流整体结构，而其他离子主要标记孤立激波结；喷流存在平面摆动运动及红蓝移瓣不对称性，宽度随距离非单调增加，张开角介于2.1度至小于10.1度之间，校正盘倾角后总速度范围为147至184公里/秒；B335喷流倾角与流出腔不同，爆发期间激波结出现更频繁；红蓝移瓣不对称性强烈暗示星周盘与磁场的复杂相互作用。
...
### [31] [Chiellini-Integrable Cosmologies with Phantom Divide Crossing](https://arxiv.org/abs/2601.09271)
*Soumya Chakrabarti,Nandan Roy*

**TL;DR**: 研究发现，在广义相对论中与爱因斯坦-希尔伯特作用量最小耦合的具有类希格斯(Higgs-like)自相互作用标量场模型下，基于Chiellini可积条件构造的解析解能实现暗能量状态方程(equation of state)平滑穿越幻影界限(phantom divide)并描述晚期宇宙加速膨胀，其重建的现今哈勃参数(Hubble parameter) H₀≳70 km·s⁻¹·Mpc⁻¹可缓解与ΛCDM模型的张力。


<details>
  <summary>Details</summary>

Background: 研究主题为在广义相对论框架下探索具有质量标量场（massive scalar field）与爱因斯坦-希尔伯特作用量（Einstein-Hilbert action）最小耦合的精确宇宙学解，重点分析标量自相互作用（scalar self-interaction）对晚期宇宙加速膨胀及暗能量状态方程（dark energy equation of state）幻影分界（phantom divide crossing）的影响。

Data: 数据来源包括重子声学振荡（BAO）、宇宙微波背景（CMB）、宇宙计时器（cosmic chronometer）以及Pantheon+SHOES超新星数据集。

Method: 采用Chiellini可积性条件分析阻尼Ermakov-Painlevé II类场方程，构建了扩展希格斯型标量自相互作用下的解析解。

Result: 在广义相对论框架下，通过Chiellini可积条件构建的含质量标量场宇宙学模型解析地实现了暗能量状态方程（equation of state）平滑穿越幻影界限（phantom divide crossing），且不引发病态不稳定性。
...
### [32] [Sub-Leading Logarithms for Scalar Potential Models on de Sitter](https://arxiv.org/abs/2601.09309)
*S. P. Miao,N. C. Tsamis,R. P. Woodard*

**TL;DR**: 论文证明了随机形式主义（stochastic formalism）可应用于1圈有效势（1-loop effective potential）的特定部分以捕捉第一亚主导对数项（first sub-leading logarithm），并在德西特（de Sitter）背景下通过无质量最小耦合四次自相互作用标量场的两圈验证。


<details>
  <summary>Details</summary>

Background: 研究背景是暴胀（inflation）期间长波长标量（long wavelength scalars）和引力子（gravitons）持续产生导致圈修正（loop corrections）出现长期增长效应，以及如何利用随机形式（stochastic formalism）捕捉主导对数（leading logarithms）和次主导对数（sub-leading logarithms）。

Data: 数据来源于德西特（de Sitter）背景上无质量、最小耦合且具有四次自相互作用的标量场的1圈和2圈有效势的模拟（simulation）计算。

Method: 采用随机形式主义（stochastic formalism）分析1圈有效势（1-loop effective potential）以捕获第一亚主导对数（first sub-leading logarithm）。

Result: 首次亚领先对数（first sub-leading logarithm）可通过将随机形式主义应用于1圈有效势的特定部分来捕获。
...
### [33] [Constant-roll $β$-exponential inflation: Palatini formalism](https://arxiv.org/abs/2601.09664)
*Ozan Sargın*

**TL;DR**: 研究了β-指数势模型中标量场耦合二次曲率引力(R + R²)的暴胀动力学，发现在帕拉蒂尼形式下满足常滚条件时，模型参数可产生与ACT DR6和普朗克观测一致的谱指数(n_s)和张标比(r)。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于暴胀（inflation）动力学，探讨标量场耦合到二次（$R + R^2$）引力的$β$-指数势模型（$β$-exponential potential model），其中暴胀子（inflaton）决定额外维度尺度。

Data: 数据来源于阿塔卡马宇宙学望远镜（ACT）第六次数据发布（DR6）和普朗克（Planck）卫星任务的最新观测结果。

Method: 采用帕拉蒂尼形式(Palatini formalism)推导爱因斯坦框架下的广义k-暴胀(k-inflation)有效理论，并在恒定滚动(constant-roll)条件下分析β-指数势(β-exponential potential)模型。

Result: 在满足恒定滚动条件（constant-roll condition）的假设下，通过帕拉蒂尼形式（Palatini formalism）推导的爱因斯坦框架广义k-暴胀（k-inflation）有效理论显示，模型参数在合适范围内时，暴胀可观测量（如谱指数$n_s$和张量标量比$r$）的预测值与ACT DR6及普朗克任务（Planck mission）的最新观测结果一致。
...
### [34] [True Masses using RV data with Hipparcos and Gaia Astrometry](https://arxiv.org/abs/2601.09401)
*G. Piccinini,A. Petralia,A. Sozzetti,S. Benatti,D. Gandolfi,G. Micela*

**TL;DR**: 通过结合依巴谷(Hipparcos)和盖亚(Gaia)天体测量数据与径向速度测量，该模型约束了长周期伴星(HD 5388 b、HD 6718 b等)的轨道倾角和真实质量，并重新确认部分目标(如HD 5388 b、HD 6718 b)具有行星性质而非褐矮星或小质量恒星。


<details>
  <summary>Details</summary>

Background: 研究主题为利用长基线视向速度（radial velocity）巡天结合天体测量（astrometry）技术（特别是自行异常（proper motion anomalies）方法）限制长周期伴星的轨道倾角和真实质量。

Data: 数据来源结合了依巴谷卫星（Hipparcos）和盖亚卫星（Gaia）的天体测量数据以及径向速度（radial velocity）测量结果。

Method: 采用径向速度测量与自行运动异常（proper motion anomalies）技术相结合的方法，同时建模自行运动异常和径向速度数据并分析灵敏度曲线。

Result: result：HD 5388 b和HD 6718 b很可能是行星，HD 141937 b可能为行星但质量尚不确定，HD 16760 b属于褐矮星且可能存在第二伴星，30 Ari B b属于恒星范畴但额外伴星可能影响结果可靠性，HD 148427 b、HD 96127 b和HIP 65891 b的轨道倾角范围已确定。
...
### [35] [The Second CHIME/FRB Catalog of Fast Radio Bursts](https://arxiv.org/abs/2601.09399)
...
**TL;DR**: 基于CHIME望远镜2018-2025年观测数据，发布了包含4539个快速射电暴（FRBs）的编目（其中83个重复源贡献981次爆发），提供了爆发形态、色散量（DM）等关键参数测量，并验证了先前研究结论。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用加拿大氢强度测绘实验（CHIME）望远镜观测数据构建快速射电暴（FRB）综合目录，以探索其天体物理和宇宙学应用价值。

Data: 数据来源于加拿大氢强度测绘实验（CHIME）望远镜在2018年7月25日至2023年9月15日期间观测到的4539个快速射电暴（FRB），包括来自3641个独立源的爆发（含83个已知重复源的981次爆发），并提供了400-800 MHz范围内0.983 ms分辨率的总强度动态频谱。

Method: 采用加拿大氢强度测绘实验（CHIME）望远镜观测数据，通过统一改进的分析框架对快速射电暴（FRB）动态频谱进行建模，约束爆发形态并测量到达时间、本征时间宽度、色散量（DM）、散射时间和流量密度等关键参数。

Result: 发现了4539个快速射电暴（FRBs），其中3641个为独立源，包括83个已知重复源的981次爆发。
...
### [36] [The NANOGrav 15 yr Data Set: Piecewise Power-Law Reconstruction of the Gravitational-Wave Background](https://arxiv.org/abs/2601.09481)
...
**TL;DR**: 基于分段幂律(PPL)模型对NANOGrav 15年数据集中纳赫兹引力波背景(GWB)信号进行频谱重建，发现该信号更符合宇宙学起源而非超大质量黑洞双星系统的物理特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用NANOGrav 15年数据集（NG15）探索纳赫兹频段引力波背景（GWB）信号的物理起源，包括超大质量黑洞双星并合或早期宇宙新粒子物理过程。

Data: 数据来源于NANOGrav 15年（NG15）数据集，该数据集提供了纳赫兹频段引力波背景（GWB）信号的证据。

Method: 采用分段幂律（PPL）假设结合贝叶斯模型平均方法，通过固定内部节点数量的个体PPL模型（如恒定幂律、断裂幂律、双重断裂幂律等）对NG15信号进行频谱重建。

Result: 基于分段幂律（PPL）假设的NG15信号频谱重建比恒定幂律模型更灵活，比自由频谱模型更接近物理现实的引力波背景（GWB）频谱。
