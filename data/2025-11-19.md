...
**TL;DR**: 采用CREST代码对银河系质量星系进行时间依赖的宇宙射线电子(CR)建模，发现500 GeV以下能谱与稳态模型吻合，但高能区因冷却时间短于注入时标呈现更陡峭谱形，且CREST模拟的电子空间分布更集中于恒星形成盘区。


<details>
  <summary>Details</summary>

Background: 宇宙射线（CR）电子是研究星系中非热过程的关键示踪物，但其能谱建模通常基于未经检验的注入与冷却达到稳态的假设。

Data: 使用AREPO对孤立银河系质量星系进行磁流体动力学模拟，并通过CREST代码在拉格朗日示踪粒子上注入超新星位点的宇宙线电子（CR electrons），同时考虑绝热变化及同步辐射、逆康普顿、轫致辐射和库仑相互作用的冷却过程。

Method: 采用CREST代码进行时间依赖的宇宙线电子（CR electron）建模，结合AREPO磁流体动力学模拟，通过拉格朗日示踪粒子追踪超新星注入及同步辐射、逆康普顿、韧致辐射和库仑相互作用等冷却过程。

Result: 银河系质量星系中宇宙线电子(CR electrons)在500 GeV以下能谱与稳态解高度吻合，仅在更高能量段因冷却时间短于注入时标出现偏离。
...
**TL;DR**: 通过钱德拉X射线、GMRT射电和SINFONI近红外观测发现，Abell 2597星系团核心存在活动星系核(AGN)反馈驱动的7个X射线空腔和1-3个弱激波(M~1.05-1.14)，其能量注入(~10^44 erg/s)与冷却光度相当但未能完全抑制冷却流(~15 M⊙/yr)，ALMA数据支持湍流驱动的混沌冷吸积(chaotic cold accretion)机制以超过邦迪吸积(Bondi accretion)的速率持续为AGN供能，维持了该星系团核心的自我调节反馈循环。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核(AGN)反馈如何影响阿贝尔2597星系团的核心区域，特别是其对星系团内介质(ICM)的塑造作用以及黑洞吸积过程的燃料供给机制。

Data: 数据来源包括深度钱德拉X射线观测（约600 ks）、存档GMRT射电数据和SINFONI近红外数据。

Method: 采用钱德拉X射线观测（Chandra X-ray observations）、GMRT射电数据（GMRT radio data）和SINFONI近红外数据（SINFONI near-infrared data）进行多波段分析，结合等离子体耗尽层（plasma depletion layer）和邦迪吸积（Bondi accretion）模型评估活动星系核反馈（AGN feedback）对星系团核心的影响。

Result: result：活动星系核（AGN）反馈在阿贝尔2597星系团核心区域通过X射线空腔和弱激波（M~1.05-1.14）释放~10^44 erg s^-1能量，与冷却光度相当，但辐射冷却仍以~15 M⊙ yr^-1速率持续补充冷气体，且邦迪吸积（Bondi accretion）功率（~2×10^43 erg s^-1）不足以维持观测到的空腔功率，支持湍流驱动冷凝（chaotic cold accretion）主导AGN燃料供给的自调节反馈循环。
...
**TL;DR**: 该论文通过分析16个近邻星系样本，发现空间分辨率不足会导致对Sérsic指数的系统性高估，从而错误判定经典核球（classical bulges）的存在，这可能是近邻与遥远星系观测结果差异的根本原因。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系二维分解技术中因局部极小值陷阱和主观选择导致的近邻与遥远宇宙中经典核球（classical bulges）比例观测差异，挑战了星系形成的并合驱动理论。

Data: 数据来源于TIMER项目中的16个邻近星系样本，并使用了原始图像和人工红移图像。

Method: 采用差分进化算法（Differential Evolution algorithm）进行二维分解分析，减少主观干预并准确测量结构特性。

Result: 二维分解方法在测量星系结构物理性质时存在因算法陷入局部极小值或主观选择导致的误差，且远距离样本中经典核球（classical bulges）比例的观测结果与近邻星系研究存在矛盾。
...
### [4] [The observed total star formation rate function up to z \sim 6: complementary UV and IR contributions and comparison with state-of-the-art galaxy formation models](https://arxiv.org/abs/2511.13828)
...
**TL;DR**: 通过结合红外(IR)和紫外(UV)恒星形成率函数(SFRF)首次获得了z~6范围内的总恒星形成率函数，发现二者在恒星形成率(SFR)范围上互补(UV校正覆盖SFR<100 M⊙yr⁻¹，IR覆盖SFR>100 M⊙yr⁻¹)，且与理论模型在z<2.5时吻合较好但在高红移下存在差异。


<details>
  <summary>Details</summary>

Background: 研究主题为比较红外(IR)和紫外(UV)波段测定的恒星形成率函数(SFRF)及其与星系形成演化理论模型的预测一致性。

Data: IR数据来自ALMA A$^3$COSMOS巡天和赫歇尔(Herschel)的IR光度函数(IR-LFs)，UV数据来自文献中的UV光度函数(UV-LFs)并进行了尘埃消光(dust-extinction)校正。

Method: 采用马尔可夫链蒙特卡洛（MCMC）方法独立拟合红外（IR）和紫外（UV）恒星形成率函数（SFRF），并通过结合两者首次估计了总恒星形成率函数（SFRF）。

Result: 通过比较总恒星形成率函数（SFRF）与理论模型预测，发现红移$z < 2.5$时整体吻合良好，但在更高红移下差异增大，所有模型均未能预测最高恒星形成率（SFR）星系的形成。
...
### [5] [How accurately can obscured galaxy luminosities be measured using spectral energy distribution fitting of near- through far-infrared observations?](https://arxiv.org/abs/2511.13849)
...
**TL;DR**: 通过模拟观测分析发现，红外波段覆盖范围（特别是涵盖光谱能量分布峰值且采样密集）对准确估算星暴(starburst)和活动星系核(AGN)光度至关重要，其中星暴光度需远红外数据而AGN光度需近/中红外数据，且高角分辨率能提升估算精度，同时红移增加会增强远红外的估算能力而削弱中红外能力。


<details>
  <summary>Details</summary>

Background: 红外明亮星系(Infrared-luminous galaxies)是研究恒星和黑洞质量聚集的关键天体，其光度特性受观测数据选择的影响尚未完全明确。

Data: 数据来源于基于已充分研究的本地样本模拟的红外波段观测数据，覆盖近红外至远红外波长范围，并考虑了信噪比(S/N)、流量定标、角分辨率和红移等参数的影响。

Method: 采用光谱能量分布（SED）模型拟合近红外至远红外数据，重点分析波长覆盖范围、信噪比（S/N）、流量定标、角分辨率和红移对星暴、活动星系核（AGN）及宿主星系光度恢复的影响。

Result: 红外明亮星系(infrared-luminous galaxies)的恒星形成暴(starburst)和活动星系核(AGN)光度恢复精度主要取决于光谱能量分布(SED)峰值附近的密集波长覆盖，系统偏差可控制在20%以下。
...
### [6] [Mapping the Cosmic-Ray Ionization Rate in the Local Galaxy with H$_3^+$](https://arxiv.org/abs/2511.13915)
*Nick Indriolo,Alexei V. Ivlev,T. Pellegrin,M. Obolentseva,Paola Caselli,A. M. Jacob,D. A. Neufeld,Kedron Silsbee,M. G. Wolfire*

**TL;DR**: 通过观测弥散分子云（diffuse molecular clouds）中H$_3^+$的吸收线并结合三维光致解离区（3D-PDR）模型，发现太阳系1千秒差距内宇宙线电离率（cosmic-ray ionization rate）存在显著空间差异（最高达5倍），且小尺度（数十秒差距）上相对均匀，表明其分布受粒子加速源距离调控。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际弥散分子云（diffuse molecular clouds）中宇宙射线电离率（cosmic-ray ionization rate）的空间分布及其对化学反应的调控作用。

Data: 数据来源于针对弥散分子云视线（sight lines）的H$_3^+$吸收线观测，结合紫外波段直接测量的H$_2$柱密度（column density）数据，以及Edenhofer等人（2024）的差分消光图（differential extinction maps）导出的气体密度峰值（gas density peaks）。

Method: 采用3D-PDR（光致解离区）建模方法推断宇宙线电离率，并结合H$_3^+$吸收谱线观测与气体密度峰值关联构建稀疏采样的三维分布图。

Result: 在约1千秒差距内的目标区域中，宇宙射线电离率(cosmic-ray ionization rate)呈现出小尺度（数十秒差距）相对均匀但不同区域间存在显著差异（最高达5倍）的空间分布特征，其变化可能与粒子加速源的距离相关。
...
### [7] [Unveiling the Sources of X-ray Luminosity in DESI Galaxy Groups: Insights from the SRG/eROSITA All-Sky Survey](https://arxiv.org/abs/2511.14216)
...
**TL;DR**: 基于eROSITA全天巡天数据(eRASS1)分析不同晕质量(Mh)星系群中活动星系核(AGN)和延展气体对X射线光度(LX)的贡献，发现低质量群(Mh≲10¹³h⁻¹M⊙)的LX主要由中心AGN主导，而高质量群(Mh≳10¹³h⁻¹M⊙)则更多来自延展气体，且两者相关性随位置偏移减弱。


<details>
  <summary>Details</summary>

Background: 研究探讨了不同红移和晕质量（halo mass）下活动星系核（AGN）和扩展气体对星系群总X射线光度（$L_X$）的贡献差异及其在星系群组装历史中的反馈机制。

Data: 数据来源于首次eROSITA全天巡天（eRASS1），并利用多波段目录（包括X射线对应体、ASKAP射电目录和DESI光谱测量）识别中心活动星系核（AGN）。

Method: 采用堆叠法(stacking method)结合多波段(包括X射线、射电ASKAP和光谱DESI)数据识别中心活动星系核(AGN)性质，分析不同晕质量(M_h)星系群X射线光度(L_X)的贡献来源。

Result: X射线星系群的总X射线光度（$L_X$）在不同中心活动星系核（AGN）性质下存在差异，其中X射线AGN群最高，其次是类星体（QSO）、射电、BPT-AGN和非AGN中心群。
...
### [8] [Enhanced UV emission knot in the giant radio galaxy NGC 315: Hint of patchy star formation?](https://arxiv.org/abs/2511.14252)
*Bannanje Ananthamoorthy,Debbijoy Bhattacharya,Dipanjan Mukherjee,P. Sreekumar*

**TL;DR**: 在椭圆星系NGC 315中发现一个距离中心约1.7千秒差距(kpc)的紫外(UV)辐射结(knot)，其较高的恒星形成率(SFR)(0.23±0.10 M⊙ yr−1)可能由活动星系核(AGN)反馈或小合并(minor merger)触发，且哈勃太空望远镜(HST)观测显示该区域存在可能与气体云相关的尘埃丝状结构(dust filaments)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于椭圆星系NGC 315中远离中心的紫外辐射结（UV knot）及其与活动星系核（AGN）反馈或小质量并（minor merger）触发的恒星形成（SF）过程的关联。

Data: 数据来源于高分辨率AstroSat-紫外成像望远镜（UVIT）观测和哈勃太空望远镜（HST）观测。

Method: 采用高分辨率AstroSat-紫外成像望远镜(UVIT)和哈勃太空望远镜(HST)观测数据，结合恒星形成率(SFR)估算和尘埃丝状结构分析。

Result: 在NGC 315椭圆星系中发现一个距离中心约1.7千秒差距(kpc)的紫外(UV)辐射结，其斑块状且空间延展的UV辐射可能源于星系内持续的恒星形成(SF)，且该区域的恒星形成率(SFR)（0.23±0.10 M⊙ yr−1）显著高于典型椭圆星系。
...
### [9] [Positive AGN feedback in the outskirts of nearby barred spiral galaxies?](https://arxiv.org/abs/2511.14257)
*Bannanje Ananthamoorthy,Debbijoy Bhattacharya,P. Sreekumar*

**TL;DR**: 基于高分辨率紫外观测发现，活动星系核(AGN)宿主星系外围(>0.5R25)的恒星形成率密度(ΣSFR)和FUV衰减(AFUV)下降速度均慢于非AGN星系，表明低光度邻近AGN可能存在辐射压或风模式的正反馈效应。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核(AGN)反馈对非喷流星系中恒星形成(SF)的影响及其与尘埃耦合的机制。

Data: 数据来源于AstroSat-UVIT的高分辨率紫外（UV）观测数据，并辅以GALEX（Galaxy Evolution Explorer）的观测数据。

Method: 采用高分辨率紫外观测（AstroSat-UVIT和GALEX）分析星系外围（>0.5R25）的恒星形成率密度（ΣSFR）和FUV衰减（AFUV），结合辐射压驱动和/或风模式活动星系核（AGN）反馈模型。

Result: 在活动星系核(AGN)宿主星系的外围区域(>0.5R25)观测到更高的恒星形成率密度(ΣSFR)和更缓慢的下降趋势，表明存在与尘埃耦合的正反馈效应。
...
### [10] [Chemical evolution in high-mass star-forming regions](https://arxiv.org/abs/2511.14326)
*Francesco Fontani,Maria Teresa Beltrán,Anton Vasyunin*

**TL;DR**: 该论文综述了新一代望远镜和计算机技术推动下关于银河系高质量恒星形成区（high-mass star-forming regions）化学演化的主要观测与理论成果，重点探讨了其化学演化指标及与太阳系原始化学组成的关联。


<details>
  <summary>Details</summary>

Background: 银河系中大多数恒星（包括太阳）形成于大质量恒星形成区（high-mass star-forming regions），但其化学演化因观测和理论挑战而远不如低质量恒星形成区清晰。

Data: 研究中使用的数据主要来源于新一代望远镜（如ALMA、VLA等）的观测数据和计算机模拟（simulation）结果。

Method: 采用新一代望远镜观测和计算机模拟技术，重点分析高质量恒星形成区（high-mass star-forming regions）的化学演化指标（chemical evolutionary indicators），并与低质量恒星形成核（low-mass star-forming cores）及河外星系核（extragalactic cores）进行对比研究。

Result: 银河系中大多数恒星（包括太阳）形成于大质量恒星形成区（high-mass star-forming regions），但其化学演化过程因观测和理论挑战而远不如低质量恒星形成区清晰。
...
### [11] [Hubble Space Telescope proper motions of Large Magellanic Cloud star clusters -- II. Kinematic structure of young and intermediate-age clusters](https://arxiv.org/abs/2511.14351)
...
**TL;DR**: 基于哈勃太空望远镜(HST)观测数据，首次对大麦哲伦云(LMC)中19个年轻(<1 Gyr)和中年(1-2.5 Gyr)大质量星团的自行运动、距离、年龄和金属丰度进行系统性测量，发现NGC 1850星团存在高倾角逆行轨道或沿棒状结构的偏心轨道，中年星团运动学特征支持小麦哲伦云(SMC)在约300和900 Myr前两次穿越LMC盘面的相互作用历史。


<details>
  <summary>Details</summary>

Background: 研究主题为基于大麦哲伦云（LMC）内19个年轻（<1 Gyr）和中等年龄（1-2.5 Gyr）大质量星团的运动学特性，探索其在LMC框架下的三维动力学行为及其与麦哲伦系统相互作用的可能历史。

Data: 数据来源于多历元哈勃空间望远镜（HST）观测的自行运动测量，并结合文献中的视线速度（line-of-sight velocities）信息。

Method: 基于多历元哈勃太空望远镜（HST）观测数据测量星团自行运动（proper motion），并结合文献视线速度（line-of-sight velocity）自洽研究大麦哲伦云（LMC）内星团三维动力学。

Result: 大多数年轻星团轨道接近大麦哲伦云（LMC）恒星盘面，而NGC 1850呈现特殊的高倾角逆行轨道或沿棒状结构的偏心轨道。
...
### [12] [QHSC: The Quasar Candidate Catalog for the Hyper Suprime-Cam Subaru Strategic Program](https://arxiv.org/abs/2511.14369)
*Rui Zhu,Xue-Bing Wu,Yuxuan Pang,Yuming Fu*

**TL;DR**: 基于HSC-SSP巡天宽层数据，利用XGBoost分类器构建了包含118万颗候选类星体（quasar）的QHSC星表，并通过集成学习方法实现了高完备性（最高92.7%）和较低红移（photometric redshift）估计异常率（最低9.5%）。


<details>
  <summary>Details</summary>

Background: 利用机器学习方法从HSC-SSP宽层巡天数据中筛选类星体候选体并估算其光度红移。

Data: 数据来源于HSC-SSP巡天（Hyper Suprime-Cam Subaru Strategic Program）的Wide层（覆盖1470平方度、深度达i波段26星等）公开数据第三版（Public Data Release 3），并分别结合WISE、UKIDSS和VISTA的近红外数据构成四个子样本（master、HSC+WISE、GoldenU和GoldenV），同时使用HETDEX、VVDS和zCOSMOS-bright的光谱测试集进行独立验证。

Method: 采用XGBoost分类器（XGBoost classifier）进行类星体候选样本筛选，并基于自助聚合（bagging）的XGBoost回归器（XGBoost regressors）集成方法估计光度红移（photometric redshift）。

Result: QHSC星表通过XGBoost分类器从HSC-SSP宽层数据中筛选出类星体候选体，包含四个子样本（master、HSC+WISE、GoldenU和GoldenV），分别包含1,184,574、371,777、87,460和120,572个候选体，完整性达85.3%至92.7%。
...
### [13] [Deep Andromeda JCMT-SCUBA2 Observations. The Submillimeter Maps and Giant Molecular Clouds](https://arxiv.org/abs/2511.14360)
...
**TL;DR**: 通过深度JCMT-SCUBA2观测在M31星系中发现383个旋臂巨分子云(GMCs)和189个旋臂间GMCs，揭示后者质量更低、弥散度更高且恒星形成效率(SFE)更低，并重建了尘埃不透明度谱指数(β)的径向分布。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用亚毫米波观测揭示M31（仙女座星系）中巨分子云（GMCs）的物理特性及其与旋臂/臂间环境的关联，为理解星系尺度下恒星形成效率（SFE）抑制机制提供线索。

Data: 数据来源于JCMT-SCUBA2对M31（仙女座星系）的深度850微米观测（分辨率约50 pc），并结合了Herschel和Planck的存档数据。

Method: 采用JCMT-SCUBA2进行深度、接近混淆极限的850微米成像观测，分辨率约50 pc，并结合赫歇尔(Herschel)和普朗克(Planck)存档数据重新约束尘埃不透明度谱指数β。

Result: 在M31星系中，旋臂间巨分子云（GMCs）比旋臂上的GMCs质量更小、更弥散、温度更低且恒星形成效率（SFE）更低。
...
### [14] [VENUS: A Strongly Lensed Clumpy Galaxy at $z\sim11-12$ behind the Galaxy Cluster MACS J0257.1-2325](https://arxiv.org/abs/2511.14483)
...
**TL;DR**: 发现并解析了红移约11-12的强引力透镜星系Misty Moons，其由星系团MACS J0257.1-2325透镜放大，呈现多个恒星团块主导的亚L*星系特征，揭示了早期宇宙中高度聚集的恒星形成模式。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（詹姆斯·韦伯空间望远镜）观测数据探索宇宙早期（红移$z\gtrsim10$）星系形成过程，特别是通过强引力透镜效应揭示高红移星系内部结构及其恒星形成模式。

Data: 数据来源于JWST（James Webb Space Telescope）财政巡天项目VENUS（Vast Exploration for Nascent, Unexplored Sources）的观测数据，并利用星系团MACS J0257.1-2325的引力透镜效应放大目标星系Misty Moons的五个多重像，其中ID1和ID2的图像亮度较高（F200W∼26 AB星等），且通过两个独立透镜模型验证了其可靠性。

Method: 采用引力透镜（gravitational lensing）模型和JWST/NIRCam点扩散函数（point-spread function）卷积技术分析星系内部结构和表面亮度分布。

Result: result：在Misty Moons星系中观测到多个恒星团块（stellar clumps），每个团块有效半径约10-70 pc、恒星质量约10^7 M⊙，这些团块贡献了该星系总恒星质量的80%以上，表明早期宇宙存在高度聚集的恒星形成模式。
...
### [15] [Feedback and Star Formation Efficiency in High-Mass Star-Forming Regions](https://arxiv.org/abs/2511.14557)
*Birka Zimmermann,Stefanie Walch,Seamus D. Clarke,Richard Wünsch,Andre Klepitko*

**TL;DR**: 通过模拟孤立湍流核心（turbulent cores）的引力坍缩过程，结合电离辐射（ionizing radiation）、辐射压力（radiation pressure）和非电离辐射（non-ionizing radiation）等多物理过程，揭示了初始密度分布、维里参数（virial parameter）和金属丰度（metallicity）对恒星形成效率（SFE）的影响，发现电离辐射终止吸积而辐射压力扩张HII区，非电离辐射加热抑制碎裂，高分辨率模拟中电离辐射滞留更久导致SFE提升至35%-57%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量恒星形成过程中恒星反馈（stellar feedback）与初始条件对恒星形成效率（SFE）的影响机制。

Data: 数据来源为ALMAGAL观测（ALMA Large Program to Investigate C+ at Early Stages of Star Formation）和基于FLASH代码的孤立湍流核（parsec-scale turbulent cores）数值模拟。

Method: 采用FLASH代码模拟孤立湍流核（turbulent cores）的引力坍缩，结合恒星作为汇粒子（sink particles）的建模，并自洽地纳入电离辐射、辐射压力、非电离辐射及其尘埃加热效应以及化学过程。

Result: 电离辐射（ionizing radiation）最终阻止了物质向星体吸积粒子的吸积，而非电离辐射（non-ionizing radiation）的加热作用抑制了碎片化过程。
...
### [16] [The Eddington Ratio Distribution of Narrow Line Active Galactic Nuclei](https://arxiv.org/abs/2511.14575)
*Michael R. Blanton,Arjun Suresh,Kyle B. Westfall,Dou Liu,John Moustakas*

**TL;DR**: 该研究基于MaNGA积分场光谱数据，测量了本地窄线活动星系核(AGN)的爱丁顿比率(Eddington ratio)分布与宿主星系特性的关系，发现AGN发生率(F_AGN)在恒星形成星系中随恒星质量(log10 M* > 10.25)恒定或增加，而在静止星系中随恒星质量显著下降，揭示了AGN活动对星系特性的复杂依赖模式。


<details>
  <summary>Details</summary>

Background: 研究探讨了本地光学窄线活动星系核（AGN）的爱丁顿比（Eddington ratio）分布与宿主星系特性的关系，旨在检验星系形成理论中超大质量黑洞（supermassive black hole）增长和反馈模型。

Data: 数据来源为SDSS-IV的DR17中MaNGA（Mapping Nearby Galaxies at Apache Point Observatory）数据的积分场光谱（integral field spectroscopy）。

Method: 基于积分视场光谱（integral field spectroscopy）和点扩散函数（PSF）优化技术重构数据立方体，结合Ji & Yan (2020)的发射线比例诊断方法筛选活动星系核（AGN）并测定Hβ和[O III]线光度。

Result: 测量发现活动星系核(AGN)发生率(F_AGN)在恒星形成星系中随恒星质量(log10 M* > 10.25)恒定或增加，而在静止星系中随恒星质量显著下降，且在高恒星质量下随特定恒星形成率(sSFR)单调增加。
...
### [17] [RIOJA. Dusty outflows and density-complex ISM in the N-enhanced lensed galaxy RXCJ2248-ID at z=6.1](https://arxiv.org/abs/2511.14658)
...
**TL;DR**: 基于JWST和ALMA观测数据，发现透镜星系RXCJ2248-ID（红移z=6.1）的电离气体具有高温（约30000K）、宽范围电子密度（log(ne[cm−3])∼2.7-3.8）、低填充因子（0.2%-10%）的团块状结构，并观测到包含两个宽成分（FWHM∼210和∼1000 km/s）的复杂ISM运动学特征，部分外流气体（0.16-2.1×10^7M⊙）可能逃逸至IGM，其余将回落至中心区域支持后续恒星形成。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(z=6.1)引力透镜星系RXCJ2248-ID中电离气体运动学与物理性质，旨在揭示其复杂星际介质(ISM)结构与反馈机制对星系演化的影响。

Data: 数据来源于JWST（James Webb Space Telescope）的NIRSpec/IFU（近红外光谱仪/积分场单元）高分辨率观测数据以及ALMA（Atacama Large Millimeter/submillimeter Array）的观测数据。

Method: 采用高分辨率JWST NIRSpec/IFU数据结合ALMA观测，通过电离气体运动学和物理性质分析（包括电子温度、电子密度、填充因子及ISM运动学结构）验证了无衰减（Attenuation-Free）和反馈无关星暴（Feedback-Free Starburst）两种理论模型。

Result: 在透镜星系RXCJ2248-ID（红移z=6.1）的电离气体中发现了高电子温度（Te~30000K），且该温度与电离水平无关。
...
### [18] [Aletheia: Emulating the non-linear matter power spectrum in the context of evolution mapping](https://arxiv.org/abs/2511.13826)
...
**TL;DR**: 开发了基于演化映射框架的非线性物质功率谱（P(k)）模拟器Aletheia，通过分离形状参数（Θ_s）和演化参数（Θ_e）并压缩为单一振幅参数（σ_12），采用两阶段高斯过程模拟，在0.006-2 Mpc⁻¹波数范围和动态暗能量模型下实现亚百分之一精度，预测方差（0.2%）比EuclidEmulator2（1%）低五倍。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发基于演化映射框架的非线性物质功率谱（P(k)）模拟器Aletheia，旨在克服传统模拟方法在h无关宇宙学参数分离和压缩方面的局限性。

Data: 数据来源于训练在形状参数（shape parameters）跨越普朗克（Planck）约束的±5σ范围及宽泛的聚类范围0.2 < σ12 < 1.0的模拟数据，并通过N体模拟（N-body simulations）验证。

Method: 采用基于演化映射框架的两阶段高斯过程(Gaussian Process)模拟方法，首先预测非线性增强因子与形状参数(Θₛ)及压缩振幅参数(σ₁₂)的关系，再通过次级模拟器施加基于积分增长历史的线性修正。

Result: Aletheia采用基于演化映射框架的两阶段高斯过程模拟方法，通过分离线性功率谱形状参数（$\mathbfΘ_{\mathrm{s}}$）和仅影响振幅演化的参数（$\mathbfΘ_{\mathrm{e}}$），将演化参数和红移的综合影响压缩为单一振幅参数$σ_{12}$，实现了对非线性物质功率谱$P(k)$的高精度模拟。
...
### [19] [Exploring the Model Dependence of MCMC-Based 21 cm Power Spectrum Parameter Constraints](https://arxiv.org/abs/2511.13854)
*August Berklas,Jonathan Pober*

**TL;DR**: 研究发现21CMMC（马尔可夫链蒙特卡洛采样器）对宇宙再电离时期(EoR)的21厘米信号分析结果高度依赖于半数值模拟模型（semi-numerical model）中电离区域识别算法（bubble-finding algorithm）与模拟数据天体物理模型的一致性。


<details>
  <summary>Details</summary>

Background: 探索宇宙再电离时期（Epoch of Reionization, EoR）的中性氢21厘米信号检测与分析。

Data: 数据来源为使用21cmFAST模拟生成的两组21厘米光锥数据集，仅电离区域识别算法（"bubble-finding"算法）不同。

Method: 采用半数值模拟代码21cmFAST结合马尔可夫链蒙特卡洛采样器21CMMC，通过比较不同电离区域识别算法（"bubble-finding"算法）生成的21 cm光锥数据集来评估模型依赖性。

Result: 21CMMC的性能高度依赖于模拟数据的天体物理模型与采样模型之间的一致性。
...
### [20] [Large Scale White Noise and Cosmology](https://arxiv.org/abs/2511.13866)
*Gabriela Barenboim,Aurora Ireland,Albert Stebbins*

**TL;DR**: 研究发现局部非线性偏微分方程描述的物理系统动力学普遍存在大尺度白噪声（white noise），其源于未解析小尺度动力学的无关联贡献，即使在线性主导的宇宙学标准模型中也能探测早期宇宙极小尺度现象。


<details>
  <summary>Details</summary>

Background: 研究探讨了由局部非线性偏微分方程描述的物理系统在大尺度上产生白噪声（white noise）的普遍动力学机制及其在宇宙学模型中的关键作用。

Data: 研究中使用的数据来源为理论推导和数值模拟（simulation），基于标准宇宙学模型（standard model of cosmology）中的原初密度功率谱（primordial density power spectrum）和哈勃尺度（Hubble scale）观测限制。

Method: 采用局部非线性偏微分方程（local non-linear partial differential equations）描述物理系统动力学，通过分析未解析小尺度动力学对大规模白噪声（white noise）的贡献来研究宇宙早期小尺度现象。

Result: 局部非线性偏微分方程描述的物理系统动力学在大尺度上普遍产生白噪声（white noise）。
...
### [21] [Thermodynamic Origin of the Tully-Fisher Relation in Dark Matter Dominated Galaxies: A Theoretical-Empirical Derivation](https://arxiv.org/abs/2511.13929)
*V. K. Oikonomou*

**TL;DR**: 提出了状态方程（equation of state）随半径变化的自我相互作用暗物质（self-interacting dark matter）模型，完美拟合了SPARC数据中100个暗物质主导星系的旋转曲线，解决了尖点-核心问题（cusp-core issue），并半理论半经验地推导出标准Tully-Fisher关系和重子Tully-Fisher关系。


<details>
  <summary>Details</summary>

Background: 研究探讨了自相互作用暗物质（self-interacting dark matter, SIDM）在状态方程（equation of state）随尺度变化条件下的性质及其对暗物质主导星系动力学的影响。

Data: 数据来源于SPARC数据库中的174个星系样本，重点关注其中100个暗物质主导的矮星系、低亮度和低表面亮度旋涡星系的旋转曲线数据。

Method: 采用尺度依赖状态方程的自相互作用暗物质（self-interacting dark matter with scale-dependent equation of state）模型，其状态方程形式为$P(r)=K(r)\left(\frac{ρ(r)}{ρ_{\star}}\right)^{γ(r)}$，并假设熵函数$K(r)=K_0\times\left(1+\frac{r}{r_c}\right)^{-p}$。

Result: 尺度依赖的自相互作用暗物质（scale-dependent self-interacting dark matter）模型通过半径依赖的状态方程$P(r)=K(r)\left(\frac{ρ(r)}{ρ_{\star}}\right)^{γ(r)}$完美拟合了SPARC数据中100个暗物质主导星系的旋转曲线，解决了尖峰-核心问题（cusp-core issue）。
...
### [22] [Accessing the homogeneity scale with 21 cm intensity mapping surveys](https://arxiv.org/abs/2511.13931)
*Bruno B. Bizarria,Camila P. Novaes,Felipe Avila,Rahima Mokeddem,Helissa H. da Costa,Carlos A. Wuensche,Gabriel A. S. Silva*

**TL;DR**: 通过量化射电望远镜波束平滑效应对21厘米强度映射（21cm IM）测量宇宙均匀性尺度（homogeneity scale）的影响，首次确定了无法恢复均匀性尺度的最大波束宽度σ_max(z)及其对应的参数空间禁区，为未来观测提供了理论依据和仪器要求预测。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙学原理（Cosmological Principle）的基本检验——均匀性尺度（homogeneity scale, $R_{\rm H}$）在21厘米强度映射（21cm intensity mapping）测量中受望远镜波束（beam）平滑效应影响的量化问题。

Data: 研究使用21厘米强度映射（21cm intensity mapping）观测数据，并通过两点相关函数（two-point correlation function）和相关维度（correlation dimension, $\mathcal{D}_2(r)$）量化望远镜波束（telescope beam）对信号平滑化的影响。

Method: 采用两点相关函数和相关维度（correlation dimension）$\mathcal{D}_2(r)$量化望远镜波束对21厘米强度映射（21cm intensity mapping）信号中本征成团性的平滑效应。

Result: 通过两点相关函数和相关维度（correlation dimension）量化了望远镜波束（beam）对21厘米强度映射（21cm intensity mapping）中宇宙均匀性尺度（homogeneity scale）测量的影响，确定了无法恢复均匀性尺度的最大波束宽度（σ_max(z)）及其对应的不可访问区域。
...
### [23] [Photon rest mass from localized fast radio bursts with improved distribution of dispersion measure from extragalactic gas](https://arxiv.org/abs/2511.14163)
*Yuchen Zhang,Yang Liu,Hongwei Yu,Puxun Wu*

**TL;DR**: 利用快速射电暴(FRBs)改进河外气体色散量分布函数，在三种宇宙学模型下将光子静质量上限约束至约4.8×10⁻⁵¹kg，为光子无质量性提供了迄今最严格的观测证据。


<details>
  <summary>Details</summary>

Background: 光子质量假设是现代物理学的基本假设之一，但需通过实验验证，快速射电暴(FRBs)因其宇宙学距离和精确测量的色散为检验该假设提供了理想实验室。

Data: 研究使用了基于快速射电暴（FRBs）的模拟数据（mock data）来验证改进的色散量分布函数。

Method: 采用改进的河外气体色散量（dispersion measure）分布函数并结合ΛCDM、wCDM和w0waCDM宇宙学模型约束光子静质量。

Result: 基于快速射电暴(FRBs)色散测量数据，在ΛCDM、wCDM和w0waCDM三种宇宙学模型下分别获得光子静质量上限为4.83×10⁻⁵¹kg、4.71×10⁻⁵¹kg和4.86×10⁻⁵¹kg，为光子无质量性提供了迄今最严格的观测约束。
...
### [24] [Numerical simulations of magnetic monopole evolution in an expanding universe](https://arxiv.org/abs/2511.14204)
*Mark Hindmarsh,Asier Lopez-Eiguren,Riikka Seppä,David J. Weir*

**TL;DR**: 通过数值模拟研究大统一理论（GUT）相变后由Kibble机制产生的't Hooft-Polyakov磁单极子在辐射主导时期的动力学行为，发现其湮灭效率足以维持密度分数恒定，支持了Zel'dovich、Khlopov和Preskill在忽略等离子体相互作用时期的理论模型。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是探索大统一理论（grand unified theories）中暴胀后对称性破缺相变产生的磁单极子（magnetic monopoles）的动力学行为及其与标准宇宙学模型的兼容性。

Data: 数据来源于通过数值模拟（simulation）产生的孤立子（'t Hooft-Polyakov monopoles）气体，这些孤立子由Kibble机制在相变后形成，并在不考虑辐射相互作用的辐射背景下进行模拟。

Method: 采用数值模拟方法研究由Kibble机制产生的't Hooft-Polyakov单极子（monopole）在辐射背景下的动力学行为，忽略与辐射的相互作用以验证解析模型差异。

Result: 在辐射主导时期，单极子（magnetic monopoles）通过高效相互寻找和湮灭维持密度分数恒定，支持了忽略等离子体相互作用时Zel'dovich、Khlopov和Preskill的模型；而在物质主导时期，密度分数呈对数下降。
...
### [25] [Spontaneous Symmetry Breaking as a Late-Time Trigger for Interacting Dark Energy](https://arxiv.org/abs/2511.14235)
*Pradosh Keshav MV,NS Kavya,Kenath Arun*

**TL;DR**: 研究提出了一种由Z2对称四次势标量暗能量场与暗物质通过汤川和门户耦合相互作用的暗区动力学模型，通过宇宙学自发对称破缺机制在临界尺度因子后激活有效耦合η(a)，将能量从暗物质转移至暗能量，抑制线性结构生长而不显著改变ΛCDM背景膨胀历史，缓解了S8张力但保持哈勃常数(H0)基本不变。


<details>
  <summary>Details</summary>

Background: 研究探讨了哈勃常数（H0）和物质聚集参数（S8）的持续张力，提出了一种在保持ΛCDM模型背景膨胀历史不变的前提下通过暗区动力学抑制结构增长的晚期新物理机制。

Data: 数据来源包括红移空间畸变（RSD）、重子声学振荡（BAO）、宇宙计时器（cosmic chronometers）、Pantheon+SH0ES超新星数据集以及压缩的普朗克距离先验（compressed Planck distance priors）。

Method: 采用Z2对称四次势标量暗能量场与暗物质通过Yukawa和portal耦合相互作用的模型，结合自发对称破缺机制生成随时间变化的真空期望值v(a)和有效耦合η(a)，分析对称相(a ≤ ac)和破缺相(a > ac)下能量转移对线性结构增长的抑制效应。

Result: 暗能量场（dark energy field）与暗物质通过汤川耦合（Yukawa coupling）和门户耦合（portal coupling）相互作用，在物质密度低于临界阈值时触发宇宙学自发对称性破缺（cosmological spontaneous symmetry breaking），产生随时间变化的真空期望值v(a)和有效耦合η(a)，从而抑制线性结构增长（linear structure growth）而不显著改变ΛCDM模型的背景膨胀历史。
...
### [26] [On the impact of the supernova subsamples in reducing the Hubble tension](https://arxiv.org/abs/2511.14332)
*Gonçalo Martins,Santiago González-Gaitán,João Duarte,Ana M. Mourão*

**TL;DR**: 研究发现校准样本与哈勃流（Hubble Flow）样本在宿主星系特性（恒星质量和特定恒星形成率）上的差异影响Ia型超新星（SN Ia）光度标准化和哈勃常数（H0）估计，揭示不同拉伸（stretch）子群间存在显著H0差异（低拉伸75.27±1.18 km s−1 Mpc−1，高拉伸71.25±1.59 km s−1 Mpc−1），表明单一β参数无法捕捉SN Ia子群的尘埃和本征颜色变化，导致质量阶跃（mass step）可能是对不同环境关联子群的过度校正。


<details>
  <summary>Details</summary>

Background: 研究聚焦于哈勃常数（Hubble constant, H₀）早期与晚期测量值间4-6σ差异（即"哈勃张力"）的成因，探讨Ia型超新星（SN Ia）样本在颜色（c）、拉伸（x₁）及宿主星系特性（恒星质量M、特定恒星形成率sSFR）差异对光度标准化及H₀估算的影响。

Data: 数据来源于SH0ES项目使用的校准样本和哈勃流（Hubble Flow, HF）样本中的Ia型超新星（SN Ia）数据，包括颜色（c）、拉伸（x₁）、宿主星系恒星质量（M）和特定恒星形成率（sSFR）等参数。

Method: 采用科尔莫戈罗夫-斯米尔诺夫检验（Kolmogorov-Smirnov test）评估子样本一致性，并通过改进样本匹配揭示参数估计变化。

Result: 低伸展和高伸展超新星（SNe Ia）子群之间存在持续的哈勃常数（H0）差异（低伸展H0=75.27±1.18 km s−1 Mpc−1，高伸展H0=71.25±1.59 km s−1 Mpc−1），导致哈勃张力分别为6.07σ和2.52σ，表明单一β参数无法捕捉不同尘埃和本征颜色的SNe Ia子群对宇宙学的影响。
...
### [27] [The SRG/eROSITA all-sky survey: X-ray scaling relations of galaxy groups and clusters in the western Galactic hemisphere](https://arxiv.org/abs/2511.14356)
...
**TL;DR**: 基于eROSITA首次全天巡天(eRASS1)数据构建了迄今最大的3061个星系群和星系团样本，精确测量了X射线光度-温度、X射线光度-气体质量等标度关系参数，发现其斜率显著偏离自相似预期但红移演化与之相符。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用eROSITA（extended ROentgen Survey with an Imaging Telescope Array）首次全天巡天数据（eRASS1）揭示星系群和星系团内禀性质（如X射线光度、温度、气体质量和低散射质量代理$Y_{\rm X}$）之间的标度关系（scaling relations），以探索其形成和演化机制。

Data: 数据来源于eROSITA（extended ROentgen Survey with an Imaging Telescope Array）软X射线望远镜在首次全天巡天（eRASS1）中探测到的星系群和星系团样本，以及通过最先进的eROSITA天空模拟（state-of-the-art simulations of the eROSITA sky）导出的选择函数。

Method: 采用基于eROSITA全天巡天(eRASS1)数据的统计建模方法，严格考虑选择函数(selection function)和红移演化效应，拟合了X射线光度(LX)-温度(T)、LX-气体质量(Mgas)、LX-质量代理(YX)以及Mgas-T的标度关系(scaling relations)。

Result: 发现了星系群和团（galaxy groups and clusters）的标度关系（scaling relations）斜率显著偏离自相似预期，而红移演化与自相似模型一致。
...
### [28] [Magnetic Fields in the Shapley Supercluster Core with POSSUM: Challenging Model Predictions](https://arxiv.org/abs/2511.14377)
...
**TL;DR**: 通过结合法拉第旋转测量（Faraday Rotation Measure, RM）网格和热苏尼亚耶夫-泽尔多维奇效应（thermal Sunyaev-Zeldovich effect）数据，研究发现Shapley超星系团核心区（SSC）的星系团和星系群间存在1-3 μG的平均磁场强度，且RM散射剖面比模型预期更平坦（η<0.5），表明在相互作用星系团外围区域观测结果与宇宙磁流体力学模拟预测存在显著差异。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用法拉第旋转测量网格（Faraday Rotation Measure Grids）追踪宇宙环境中磁化等离子体，以约束Shapley超星系团核心区（SSC）气体的磁场特性。

Data: 数据来源包括POSSUM试点巡天获取的法拉第旋转测量（Faraday Rotation Measure, RM）网格数据和普朗克（Planck）卫星提供的热苏尼亚耶夫-泽尔多维奇效应（thermal Sunyaev-Zeldovich effect）数据。

Method: 采用法拉第旋转测量(RM)网格数据结合热苏尼亚耶夫-泽尔多维奇效应(Sunyaev-Zeldovich effect)数据，通过半解析高斯随机场模型和宇宙学磁流体动力学(MHD)模拟分析星系团/群间磁化气体特性。

Result: result：在Shapley超星系团核心区（SSC）探测到30.5±4.6 rad/m²的额外法拉第旋转量（RM）散射，平均磁场强度为1-3 μG，且RM散射剖面比模型预期更平坦（η<0.5），表明在相互作用星系团外围区域观测结果与宇宙磁流体动力学（MHD）模拟预测存在显著差异。
...
### [29] [Early Universe Constraints on Variations in Fundamental Constants Induced by Ultralight Scalar Dark Matter](https://arxiv.org/abs/2511.14532)
*Subhajit Ghosh,Kimberly K. Boddy,Tien-Tien Yu*

**TL;DR**: 研究了超轻暗物质（ULDM）与标准模型粒子的二次耦合对精细结构常数和电子质量的时变影响，发现该耦合通过改变大爆炸核合成（BBN）和复合时期的轻元素丰度进一步限制了ULDM质量低于10^-26 eV时的暗物质占比，且对哈勃常数无显著影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超轻暗物质（ULDM）与标准模型粒子二次耦合对宇宙学的影响，特别是其对精细结构常数和电子质量的时变调制效应及其在宇宙微波背景（CMB）各向异性中的观测约束。

Data: 数据来源包括宇宙微波背景（CMB）各向异性数据和重子声学振荡（baryon acoustic oscillation）测量数据。

Method: 采用宇宙微波背景（CMB）各向异性数据和重子声波振荡（BAO）测量，结合自洽计算原初氦丰度对CMB功率谱的影响，约束超轻暗物质（ULDM）与标准模型粒子二次耦合引起的精细结构常数和电子质量的时间依赖性变化。

Result: 超轻暗物质（ULDM）与标准模型粒子的二次耦合会导致精细结构常数和电子质量的时变，进而影响大爆炸核合成（BBN）期间轻元素的丰度和复合过程，从而改变宇宙微波背景（CMB）的各向异性。
...
### [30] [Forecasting Synchrotron Spectral Parameters with QUIJOTE-MFI2 in combination with Planck and WMAP](https://arxiv.org/abs/2511.14572)
*Ana Almeida,José Alberto Rubiño-Martín,Roke Cepeda-Arroita,Ricardo Tanausu Génova-Santos,Debabrata Adak*

**TL;DR**: 通过模拟分析发现，QUIJOTE-MFI2（10-20 GHz）与WMAP+Planck+MFI数据结合能显著提升对偏振同步辐射（polarised synchrotron emission）谱指数（β_s）、曲率参数（C_s）和亮区偏振振幅的约束精度，并在低信噪比区域实现谱指数测量，同时将100 GHz同步辐射残余中值降低2.2倍至0.022 μK_CMB。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用QUIJOTE-MFI2仪器（10-20 GHz）提升低频偏振同步辐射（polarised synchrotron emission）的测量精度，为未来宇宙微波背景（CMB）巡天提供关键银河系前景辐射约束。

Data: 使用了基于幂律和弯曲同步辐射谱（power-law and curved synchrotron spectra）的模拟天图（simulated sky maps）数据。

Method: 采用参数化分量分离（parametric component separation）方法分析QUIJOTE-MFI2仪器数据，结合幂律（power-law）和弯曲同步辐射谱（curved synchrotron spectra）模拟天空图。

Result: 在深空宇宙学区域，结合QUIJOTE-MFI2与WMAP和Planck将100 GHz同步辐射中值残差降低2.2倍（至0.022 μK_CMB），总残差降低1.8倍（至0.030 μK_CMB）。
...
**TL;DR**: 2017年NASA冰巨星研究（Ice Giants Study）通过评估科学目标、技术和任务方案，证实了在预算限制内可实现天王星（Uranus）或海王星（Neptune）系统的高价值旗舰级（flagship-class）探测任务。


<details>
  <summary>Details</summary>

Background: 研究背景为NASA委托开展的冰巨星（Ice Giants）研究，旨在为第三次行星科学十年调查（Planetary Science Decadal Survey）重新评估天王星（Uranus）和海王星（Neptune）系统的科学目标与任务概念。

Data: 数据来源包括NASA下属喷气推进实验室(JPL)、兰利研究中心、艾姆斯研究中心、欧洲航天局(ESA)以及普渡大学等机构提供的多源观测数据和模拟分析结果。

Method: 采用科学目标评估与优先级排序方法，结合天体力学分析、任务架构设计及仪器技术评估，并通过JPL团队X流程和独立成本核算验证可行性。

Result: 高价值旗舰级任务概念（flagship-class mission concepts）在预算限制内可实现针对天王星（Uranus）或海王星（Neptune）的科学探测。
...
### [32] [The CHASM-SWPC Dataset for Coronal Hole Detection & Analysis](https://arxiv.org/abs/2511.14044)
*Cutter Beck,Evan Smith,Khagendra Katuwal,Rudra Kafle,Jacob Whitehill*

**TL;DR**: 开发了半自动标注工具CHASM和高质量日冕洞（CH）数据集CHASM-SWPC，用于训练CHRONNOS神经网络模型，其检测精度（0.9805）优于原预训练模型（0.9708）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于日冕洞（Coronal holes, CHs）的自动化检测方法开发与数据集构建。

Data: 数据来源于美国空间天气预报中心（SWPC）提供的日冕洞（CHs）手工绘制地图，并通过半自动化流程将其数字化为二值分割掩模，构建了CHASM-SWPC数据集，其中包含1111个标注样本的CHASM-SWPC-1111子集。

Method: 采用半自动化流程将SWPC手绘日冕洞(CHs)地图数字化为二值分割掩膜，并开发了CHASM软件工具进行快速准确标注，构建了CHASM-SWPC数据集用于训练CHRONNOS神经网络架构。

Result: result：基于CHASM-SWPC数据集训练的CHRONNOS神经网络模型在测试集上达到了0.9805准确率、0.6807 TSS和0.5668 IoU，性能优于原预训练模型（准确率0.9708、TSS 0.6749、IoU 0.4805）。
...
### [33] [SPECTRUMMATE: A Low-cost Spectrometer for Small Telescopes](https://arxiv.org/abs/2511.14094)
*Hien Phan-Thanh,Nguyen Nguyen-Duc,Thuy Le-Quang,Tobias C. Hinse,Quang Nguyen-Luong*

**TL;DR**: 研制了专为小型望远镜设计的紧凑型光谱仪SPECTRUMMATE，能在可见光谱范围内（4000-6600埃）实现高分辨率（R=2546@5500埃）光谱观测，尤其适用于太阳等明亮天体的专项光谱研究。


<details>
  <summary>Details</summary>

Background: 研究聚焦于为小型望远镜开发紧凑型光谱仪（SPECTRUMMATE），以支持资源有限的业余天文学家和教育机构开展可见光波段（4000-6600埃）的高分辨率光谱观测。

Data: 数据来源于越南河内科学技术大学空间与应用实验室（SpaceLAB）提供的可用光学元件，基于Sol'EX设计构建了SPECTRUMMATE光谱仪。

Method: 基于Sol'EX设计，采用光学元件（包括准直镜、衍射光栅和物镜）配置构建紧凑光谱仪SPECTRUMMATE，通过实验测试优化光谱色散并确保系统紧凑性。

Result: SPECTRUMMATE是一款专为小型望远镜设计的紧凑型光谱仪，基于Sol'EX方案构建，可在可见光谱范围内（4000至6600埃）实现高分辨率（R=2546@5500埃）光谱分析，并能灵活调整组件以满足不同观测需求。
...
### [34] [Detection of Faint Sources by the UltraViolet Imaging Telescope Onboard AstroSat Using Poisson Distribution of Background](https://arxiv.org/abs/2511.14254)
*B. Ananthamoorthy,Debbijoy Bhattacharya,P. Sreekumar,Swathi B*

**TL;DR**: 提出了一种基于泊松分布处理紫外背景的改进方法，利用AstroSat卫星的紫外成像望远镜(UVIT)观测数据构建紫外源(UV source)目录，在非拥挤天区（如小麦哲伦云(SMC)和M31的13号场）中成功探测到更多暗弱源（增15%-92%），多数新发现源集中在星等大于22的暗端。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用搭载在AstroSat卫星上的紫外成像望远镜（UltraViolet Imaging Telescope, UVIT）观测数据，通过考虑紫外背景（UV background）的泊松分布改进紫外源（UV source）编目方法。

Data: 数据来源于AstroSat卫星搭载的紫外成像望远镜（UltraViolet Imaging Telescope, UVIT）对小麦哲伦云（Small Magellanic Cloud, SMC）和M31（Field 13）非拥挤天区的观测。

Method: 采用基于泊松分布（Poisson distribution）的紫外背景处理方法改进紫外源（UV source）编目技术，结合紫外成像望远镜（UVIT）观测数据。

Result: 通过考虑紫外背景的泊松分布，改进后的方法能更高效地探测暗弱紫外源（UV sources），并发现大量新源（比以往多约15%至92%）。
...
### [35] [Estimating differential pistons for the Extremely Large Telescope using focal plane imaging and a residual network](https://arxiv.org/abs/2511.14577)
*P. Janin-Potiron,M. Gray,B. Neichel,M. Dumont,J. -F. Sauvage,C. T. Heritier,P. Jouve,R. Fetick,T. Fusco*

**TL;DR**: 提出基于2x2夏克-哈特曼波前传感器(SH-WFS)图像和残差网络(ResNet)的方法，有效估计极大望远镜(ELT)M4镜花瓣间差分活塞(differential piston)，在湍流斑点场平均后精度显著提升，且多色性和探测器噪声影响有限。


<details>
  <summary>Details</summary>

Background: 研究聚焦于优化即将投入使用的极大望远镜（ELT）的成像性能，重点解决由自适应光学（AO）控制回路、热机械效应等因素引起的差分活塞（differential piston）对图像质量的显著劣化问题。

Data: 数据来源于基于2x2夏克-哈特曼波前传感器(SH-WFS)的模拟数据集，用于训练基于ResNet架构的深度学习模型来估计M4镜花瓣间的差分活塞(differential piston)。

Method: 采用基于残差网络（ResNet）架构的深度学习模型，通过训练神经网络从模拟的2x2夏克-哈特曼波前传感器（SH-WFS）图像中估计M4镜花瓣间的差分活塞（differential piston）。

Result: 基于ResNet架构的深度学习模型能够从2x2夏克-哈特曼波前传感器(SH-WFS)图像中提取极大型望远镜(ELT)M4镜面花瓣间的差分活塞(differential piston)信息，且在时间平均帧处理后显著降低了均方根误差(RMSE)。
...
### [36] [A socio-demographic study of the exoplanet direct imaging community II](https://arxiv.org/abs/2511.14645)
...
**TL;DR**: 通过2022年莱顿Lyot会议调查揭示了系外行星和盘成像（exoplanet and disk imaging）领域在职位、外派、性别、少数群体身份及育儿状况维度上存在的代表性不足（under-representation）、不当行为经历及盟友认同的演变趋势。


<details>
  <summary>Details</summary>

Background: 研究聚焦于天文学与天体物理学领域内代表性不足（under-representation）、排斥（exclusion）及不当行为（problematic behavior）的识别与应对问题。

Data: 数据来源于2022年莱顿Spirit of Lyot会议期间向所有参会者发送的问卷调查，共收集96份有效回复。

Method: 采用问卷调查方法分析参会者的社会人口学特征、职业经历及少数群体相关行为体验。

Result: 天文领域需重视并解决代表性不足（under-representation）、排斥和不当行为问题。
...
### [37] [A non-local origin for massive gravity and late-time acceleration](https://arxiv.org/abs/2511.14055)
*Susobhan Mandal,S. Shankaranarayanan*

**TL;DR**: 研究提出了一种包含$R\Box^{-2}R$等非局域修正项的引力理论，证明了该理论能动态产生有质量引力子（massive graviton），在背景宇宙学中具有稳定的德西特（de Sitter）吸引子且无鬼模（ghost instabilities）扰动，为暗能量问题提供了区别于标量-张量模型的替代方案。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过非局域修正引力理论（non-local modified gravity）解释宇宙加速膨胀现象，挑战广义相对论在暗能量（dark energy）问题上的局限性。

Data: 研究中使用的数据来源为理论推导（simulation）数据，基于非局部修正引力作用量（non-local modified gravity action）的数学分析。

Method: 研究了包含非局域修正项（如$R\Box^{-2}R$、$R^{μν}\Box^{-2}R_{μν}$、$R^{μνσδ}\Box^{-2}R_{μνσδ}$）的修正引力作用量，并通过线性化分析将其约化为标准及扩展的Fierz-Pauli作用量。

Result: 提出的非局域修正引力理论通过引入$R\Box^{-2}R$等项，在背景宇宙学中展现出稳定的德西特吸引子（de Sitter attractor），能自然驱动加速膨胀。
...
### [38] [Cosmological dynamics of interacting dark matter-dark energy in generalized Rastall gravity](https://arxiv.org/abs/2511.14089)
*Manuel Gonzalez-Espinoza,Ramón Herrera,Giovanni Otalora,Carlos Ríos,Carlos Rodriguez-Benites*

**TL;DR**: 该研究在广义Rastall引力框架下探究了暗能量（dark energy）与暗物质（dark matter）的晚期相互作用，通过构建自治动力系统分析了三种相互作用模型（f∝ρ_m、f∝ρ_de、f∝ρ_m+ρ_de）的相空间稳定性和宇宙学演化，并结合观测数据给出了参数约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于广义拉斯托引力（generalized Rastall gravity）框架下暗能量与暗物质晚期相互作用的宇宙学动力学行为。

Data: 数据来源于宇宙计时器（Cosmic Chronometers）、PantheonPlus超新星数据集和DESI（Dark Energy Spectroscopic Instrument）观测数据。

Method: 采用自主动力系统方法分析暗能量（dark energy）与暗物质（dark matter）的三种相互作用形式（$f \propto ρ_m$、$f \propto ρ_{de}$、$f \propto ρ_m + ρ_{de}$），通过固定点（fixed points）稳定性分析揭示相空间演化特征。

Result: 在广义拉斯托引力（generalized Rastall gravity）框架下，暗能量（dark energy）和暗物质（dark matter）之间的相互作用通过能量-动量张量的非守恒性自然产生，并表现为三种形式（$f \propto \rho_m$、$f \propto \rho_{de}$、$f \propto \rho_m + \rho_{de}$）的自主动力系统演化。
...
### [39] [Exotic compact objects in Einstein-Scalar-Maxwell theories](https://arxiv.org/abs/2511.14207)
*Antonio De Felice,Shinji Tsujikawa*

**TL;DR**: 在广义相对论的k-本质(k-essence)理论中引入标量场φ与U(1)规范场强F的耦合项μ(φ)F，首次构建了无鬼场(ghost-free)且能量密度中心为零的渐近平坦带电致密星，其致密度(compactness)介于0.01至0.1量级，且电性致密星在μ(φ)>0时全域稳定。


<details>
  <summary>Details</summary>

Background: 研究探讨了广义相对论框架下k-essence（k-本质）理论中引入U(1)规范场场强F依赖后如何规避静态球对称致密天体无法同时满足正能量密度和无鬼场（ghost）存在的禁闭定理。

Data: 数据来源于理论模拟（simulation），基于爱因斯坦-标量-麦克斯韦理论框架下的标量-矢量耦合函数μ(φ)构建的渐近平坦带电致密星体模型。

Method: 采用爱因斯坦-标量-麦克斯韦理论（Einstein-scalar-Maxwell theories）结合标量-矢量耦合函数μ(φ)F，构建了渐近平坦的带电致密星解析解。

Result: 在爱因斯坦-标量-麦克斯韦理论中引入标量-矢量耦合μ(φ)F后，成功构建了能量密度和压力在中心处为零的渐近平坦带电致密星体，其致密度范围在O(0.01)到O(0.1)之间。
...
### [40] [Estimation of Magnetic Fields from Synchrotron Emission: Numerical Tests](https://arxiv.org/abs/2511.13834)
*Nora B. Linzer,Lucia Armillotta,Eve C. Ostriker,Eliot Quataert*

**TL;DR**: 该研究通过TIGRESS磁流体动力学模拟结合宇宙线(CR)传输模型，发现传统等分磁能与宇宙线能量密度方法在千秒差距(kpc)尺度有效但在更小尺度失效，提出采用恒定宇宙线能量密度可改进小尺度磁场强度预测，并验证了合成偏振辐射能准确还原磁场方向。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙线(CR)在局部星际介质(ISM)中的光谱分辨传输模型，重点检验了基于磁能与总宇宙线能量密度均衡假设的传统磁场强度估算方法在不同尺度(从kpc到~10 pc)的适用性。

Data: 数据来源于TIGRESS磁流体动力学(MHD)模拟生成的局部星际介质(ISM)光谱分辨宇宙线(CR)传输模型，并基于该模型合成了同步辐射发射数据。

Method: method：采用光谱分辨宇宙线（CR）传输模型结合TIGRESS磁流体力学（MHD）模拟，通过合成同步辐射发射检验传统等分磁能与总CR能量密度假设的磁场强度估算方法。

Result: 传统能量均分假设在千秒差距(kpc)尺度上有效，但在更小尺度(约10 pc)下失效。
...
### [41] [Multi-layered Fast Wind observed in XMM-Newton snapshot of Seyfert 1 Markarian 877](https://arxiv.org/abs/2511.13921)
*Xin Xiang,Jon M. Miller,Ehud Behar,W. N. Brandt,Luigi Gallo,Doyee Byun,Elena Gallo*

**TL;DR**: 通过XMM观测在赛弗特1型活动星系核(AGN)Mrk 877中首次发现三个速度分别为0.10c、0.04c和0.05c的超快外流(UFO)组分，其辐射驱动与磁驱动共同作用产生的能量超过爱丁顿光度5%，能有效驱动星系尺度反馈。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）中超快外流（UFOs）的物理特性及其在星系尺度反馈中的关键作用。

Data: 数据来源于XMM-Newton望远镜对Seyfert 1 AGN Mrk 877进行的18 ks快照观测。

Method: 采用光致电离分析(photoionization analysis)结合吸收测量分布(AMD)技术，通过XMM-Newton卫星观测数据解析了Mrk 877中三个不同速度与电离态的UFO组分，并基于辐射与磁驱动模型推断其动力学机制。

Result: 在Mrk 877中发现了三个速度分别为0.10c、0.04c和0.05c的超快外流（UFO）成分，其电离参数和柱密度范围广泛，并在软硬X射线波段产生吸收特征。
...
### [42] [The Scoured Spike: Suppression of Indirect Dark Matter Signals by a Hidden Companion](https://arxiv.org/abs/2511.13858)
*Jaden Lopez,Stefano Profumo*

**TL;DR**: 研究发现银河系中心超大质量黑洞周围可能存在的中等质量黑洞等暗伴星（dark companion）会通过引力加热和角动量交换重塑暗物质尖峰（dark-matter spike），显著抑制暗物质湮灭信号（annihilation signal），尤其对浅层尖峰（γ_sp≲1.8）的抑制可达1-2个数量级。


<details>
  <summary>Details</summary>

Background: 研究探讨了银河系中心超大质量黑洞周围可能存在的中等质量黑洞或致密暗天体（dark companion）对暗物质尖峰（dark-matter spike）动力学重塑的影响及其对暗物质湮灭信号（annihilation signal）的抑制效应。

Data: 数据来源为结合解析冲刷半径模型与修正密度剖面的完整数值积分计算结果。

Method: 采用解析冲刷半径模型与修正密度剖面的全数值积分相结合的方法，量化了暗物质湮灭J因子（J-factor）的抑制效应。

Result: 银河系中心超大质量黑洞周围的暗物质尖峰（dark-matter spike）会被其轨道上的大质量暗伴天体（如中等质量黑洞或致密暗天体）通过引力加热和角动量交换重塑，形成密度降低的"冲刷"区域并抑制湮灭信号（annihilation signal）。
...
### [43] [Stripped-Envelope Supernovae for QCD Axion Detection](https://arxiv.org/abs/2511.13815)
*Francisco R. Candón,Damiano F. G. Fiorillo,Ángel Gil Muyor,Hans-Thomas Janka,Georg G. Raffelt,Edoardo Vitagliano*

**TL;DR**: 研究发现Ibc型超新星（SN）因其剥离的致密前身星具有更强磁场，是探测QCD轴子（axion）转化为伽马射线的最佳目标，费米卫星（Fermi-LAT）可探测质量小至10⁻⁴ eV的轴子。


<details>
  <summary>Details</summary>

Background: 研究探讨了QCD轴子（QCD axion）在核心坍缩超新星（core-collapse supernova）原中子星（proto-neutron star）中的大量产生及其在银河系磁场和母星磁场中转化为伽马射线的过程。

Data: 研究使用伽马射线卫星费米-拉特（Fermi-LAT）或类似设备观测Ibc型超新星（Type Ibc SNe）产生的伽马射线信号。

Method: 采用伽马射线卫星（如Fermi-LAT）探测QCD轴子（axion）在银河系或前身星磁场中转换的伽马射线信号。

Result: Ibc型超新星（Type Ibc SNe）因其剥离（stripped）前身星的致密性和强磁场特性，成为探测QCD轴子（QCD axion）转化为伽马射线的最优目标。
...
### [44] [Impact of conversion-driven processes on singlet-doublet Majorana dark matter relic](https://arxiv.org/abs/2511.14571)
*Partha Kumar Paul,Sujit Kumar Sahoo,Narendra Sahu*

**TL;DR**: 研究单重态-双重态暗物质(DM)模型并假设暗物质为马约拉纳(Majorana)型时，发现其遗迹密度和直接探测允许的参数空间范围更大，具体为暗物质质量1 GeV≲M_DM≲1750 GeV、混合角2×10^(-7)≲sinθ≲0.16且质量分裂ΔM>1 GeV。


<details>
  <summary>Details</summary>

Background: 研究探讨了单重态-双重态暗物质(DM)模型在Majorana费米子假设下的参数空间，扩展了Dirac情形下的暗物质质量与混合角范围。

Data: 研究中未明确提及观测或模拟数据来源。

Method: 采用马约拉纳型（Majorana type）单重态-双重态暗物质（singlet-doublet DM）模型，通过湮灭、共湮灭和转换驱动过程分析参数空间。

Result: 假设单重态-双重态暗物质(DM)为马约拉纳(Majorana)类型时，可满足遗迹密度和直接探测的更大参数空间范围：DM质量$1~{\rm GeV}\lesssim M_{\rm DM}\lesssim1750$ GeV、混合角$2\times10^{-7}\lesssim\sinθ\lesssim0.16$（对所有$Δ{M}>1$ GeV成立）。
...
### [45] [PyEMILI: A New Generation Computer-aided Spectral Line Identifier -- II. Emission-line Identification and Plasma Diagnostics of a Sample of Gaseous Nebulae](https://arxiv.org/abs/2511.14451)
*Zhijun Tu,Xuan Fang,Jorge García-Rojas,Robert Williams,Jifeng Liu*

**TL;DR**: 通过测试PyEMILI光谱线识别工具在行星状星云(PNe)、电离氢区(HII regions)和赫比格-哈罗天体(HH objects)样本中的表现并扩展其原子跃迁数据库和重组线处理功能，证实该工具在线识别完整性和准确性上显著优于人工方法，成为深空光谱分析和等离子体诊断的可靠工具。


<details>
  <summary>Details</summary>

Background: 该研究旨在验证新一代谱线识别工具PyEMILI在发射线星云（包括行星状星云、HII区和赫比格-哈罗天体）光谱分析中的鲁棒性和可靠性，并建立未来光谱研究的基准数据集。

Data: 数据来源为过去二十年发表的深高色散光谱观测的行星状星云（PNe）、HII区和赫比格-哈罗天体（HH objects）的发射线列表，并交叉匹配了Kurucz线表数据库，同时为辅助识别微弱光学复合线（ORLs）编译了新的有效复合系数数据集。

Method: 采用PyEMILI光谱线自动识别代码，结合扩展的原子跃迁数据库（Kurucz Line Lists）和新编制的有效复合系数数据集，并开发了基于马尔可夫链蒙特卡洛（MCMC）方法的重合线理论光谱拟合子程序。

Result: PyEMILI在发射线星云光谱线自动识别中展现出比人工识别更高的完整性和准确性。
...
### [46] [Infrared Quantum Electrodynamics and the Rayleigh-Jeans Physics](https://arxiv.org/abs/2511.13943)
*Jorge Gamboa,Natalia Tapia Arellano*

**TL;DR**: 该研究通过将软光子（soft photons）描述为绝热输运的电子-光子云，为红外量子电动力学（IR-QED）提供了几何解释，发现宇宙微波背景（CMB）能量密度存在与ARCADE 2数据相符的幂律标度贝里相（Berry phase）修正，导致射电波段出现频率依赖性温度超量。


<details>
  <summary>Details</summary>

Background: 研究探讨了红外量子电动力学（IR-QED）中软光子作为绝热输运电子-光子云的几何描述及其对宇宙微波背景（CMB）能谱的伯里相位修正效应。

Data: 研究中使用的数据来源为宇宙微波背景辐射（cosmic microwave background, CMB）的观测数据，特别是ARCADE 2实验提供的低频段温度异常数据。

Method: 采用几何化框架下的功能贝里相位（functional Berry phase）分析软光子（soft photons）的绝热输运过程，通过重整化群方程求解有效因子$F_{\rm eff}(ω)$的频率依赖性。

Result: 红外量子电动力学（IR-QED）中软光子被描述为绝热输运的电子-光子云时，其红外结构由规范联络空间的功能性贝里相位编码，对应的贝里修正改变了瑞利-琼斯谱。
...
### [47] [Inferring Planet and Disk Parameters from Protoplanetary Disk Images Using a Variational Autoencoder](https://arxiv.org/abs/2511.13840)
*Sayed Shafaat Mahmud,Sayantan Auddy,Neal Turner,Jeffrey S. Bary*

**TL;DR**: 开发了基于变分自编码器（VAE）的VADER框架，首次实现从原行星盘（protoplanetary disks）尘埃连续谱图像中高效推断嵌入行星质量和盘参数（粘滞系数α、尘埃-气体比、斯托克斯数、膨胀指数），并验证了其高精度（结构相似性指数＞0.99、行星质量R²＞0.9）和快速推断（分钟级）特性。


<details>
  <summary>Details</summary>

Background: 研究主题为利用变分自编码器（VAE）的生成机器学习方法从原行星盘（protoplanetary disks）尘埃连续谱图像中推断嵌入行星质量及盘参数。

Data: 数据来源于对尘埃连续谱（dust-continuum）的合成图像，这些图像由经过蒙特卡洛辐射传输计算后处理的\texttt{FARGO3D}流体动力学模拟生成。

Method: 采用基于变分自编码器（VAE）的生成式机器学习框架VADER（Variational Autoencoder for Disks with Embedded Rings），结合\texttt{FARGO3D}流体动力学模拟和蒙特卡洛辐射传输计算生成的合成尘埃连续谱图像进行训练，推断嵌入行星质量及盘参数（粘滞系数α、尘埃-气体比、斯托克斯数、膨胀指数）。

Result: 开发了基于变分自编码器（VAE）的VADER框架，能通过原行星盘（protoplanetary disk）尘埃连续谱图像精确推断嵌入行星质量和盘参数（粘滞系数α、尘埃-气体比、斯托克斯数、膨胀指数），并输出完整后验分布。
...
### [48] [ExoClock Project IV: A homogeneous catalogue of 620 updated exoplanet ephemerides](https://arxiv.org/abs/2511.14407)
...
**TL;DR**: ExoClock项目通过整合地面和空间望远镜的30000次观测数据更新了620颗系外行星的星历表（ephemerides），显著提升了45%行星的预测精度（不确定性降低一个数量级），并发现了一些存在凌星时间变化（TTVs）的新行星，为未来大型望远镜的高效观测调度提供了支持。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过整合空间和地面望远镜观测数据更新系外行星（exoplanet）历表（ephemerides），以提升未来观测效率并支持开放科学（open science）框架下的协作研究。

Data: 数据来源包括地基望远镜（ExoClock网络）、文献资料以及空间望远镜（开普勒、K2和TESS）的30000次测量数据。

Method: 采用地面和空间望远镜（开普勒、K2和TESS）的同步观测新方法，整合了3万次测量数据，通过集体分析更新了系外行星星历表（ephemerides），并识别出行星凌星时间变化（TTVs）。

Result: ExoClock项目通过整合地面和空间望远镜的观测数据，更新了包含620颗系外行星的星历表（ephemerides），其中45%的行星需要更新且预测不确定性降低了一个数量级。
...
### [49] [From Flash to Crater: Morphological and Spectral Analysis of the Brightest Lunar Impact on 11 September 2013 using LRO Data](https://arxiv.org/abs/2511.14442)
*J. L. Rizos,L. M. Lara,J. L. Ortiz,J. M. Madiedo*

**TL;DR**: 基于月球勘测轨道飞行器(LRO)观测数据，首次发现2013年9月11日月球撞击事件形成的直径35±0.7米陨石坑导致中央溅射物区域光谱斜率增加16.54%，证实该事件由偶发流星体(sporadic meteoroid)引发并验证了Gault撞击标度律的准确性。


<details>
  <summary>Details</summary>

Background: 基于月球勘测轨道飞行器(LRO)观测数据对2013年9月11日月球撞击事件进行形态学与分光光度分析。

Data: 数据来源于月球勘测轨道飞行器(LRO)在2013年9月11日月球撞击事件前后的观测数据，包括广角相机(WAC)图像和形态测量数据。

Method: 采用形态测量、光谱分析（spectrophotometric analysis）和弹坑尺度定律（scaling laws）评估撞击事件特征。

Result: 首次报道了月球撞击导致喷射物区域颜色变化（平均光谱斜率在321-643 nm间增加16.54%）。
...
### [50] [MINDS: The molecule-rich disc of the Herbig star HD 35929 revealed with JWST/MIRI](https://arxiv.org/abs/2511.14418)
...
**TL;DR**: 利用JWST的MIRI/MRS数据首次在中质量年轻恒星（Herbig star）HD 35929的内盘（inner disc）约0.2天文单位（au）区域探测到气相SiO（v=1-0振动带）及H2O、CO、CO2、OH等分子，其柱密度显著高于低质量T Tauri星盘，表明该区域存在化学非平衡态、岩石形成元素不完全冷凝或气体动力学温度被低估。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用詹姆斯·韦伯太空望远镜（JWST）提升对中等质量年轻恒星（如赫比格星HD 35929）内盘气体化学组成的探测能力，填补该领域因仪器限制导致的知识空白。

Data: 数据来源于JWST（James Webb Space Telescope）的MIRI/MRS（Mid-Infrared Instrument/Medium Resolution Spectrometer）观测数据。

Method: 采用iSLAT分析观测分子谱线并结合DuCKLiNG进行分子发射检测、拟合与分析。

Result: 在HD 35929年轻中质量赫比格（Herbig）星的内盘（inner disc）中检测到了气相H2O、CO、CO2、OH和SiO，分子柱密度显著高于低质量T Tauri星盘，表明存在一个致密且高温的O-rich气体环境，硅酸盐凝结不完全。
