...
**TL;DR**: 通过流体动力学模拟和轨道分析，揭示了棒旋星系中心1千秒差距(kpc)区域气体形态（如核环和核螺旋）的形成主要受声速(20 km/s)等流体力学过程驱动，而非仅由引力势参数决定。


<details>
  <summary>Details</summary>

Background: 研究聚焦于棒旋星系中心1千秒差距(kpc)区域气体成分特定形态特征（如核环和核螺旋）的形成机制，探讨引力势参数与气体性质的相互作用对结构塑造的影响。

Data: 使用欧拉流体动力学代码RAMSES进行数值模拟，基于二维费勒斯棒(Ferrers bar)引力势模型，并假设气体为等温状态。

Method: 采用欧拉型RAMSES代码进行流体动力学模拟，结合二维费勒斯棒(Ferrers bar)引力势模型和等温气体假设，分析气体流动响应并开展轨道研究，评估周期性轨道形态及稳定性，以及系统内规则与混沌区域分布。

Result: 引力势参数不足以单独准确预测气体动力学，气体形态显著受声速变化影响，流体动力学过程对中心区域气体结构起决定性作用。
...
**TL;DR**: 结合ALMA（Atacama Large Millimeter/submillimeter Array）和JWST（James Webb Space Telescope）的观测能力，揭示了早期宇宙（红移星系）中气体、尘埃、恒星、星际介质、运动学、形态、活动星系核及恒星形成活动的空间分辨细节，推动了星系形成与演化研究的突破性进展。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用阿塔卡马大型毫米波/亚毫米波阵列（ALMA）和詹姆斯·韦伯太空望远镜（JWST）揭示早期宇宙（high-redshift）中星系形成与演化的物理机制。

Data: 数据来源为阿塔卡马大型毫米波/亚毫米波阵列(ALMA)和詹姆斯·韦伯太空望远镜(JWST)的观测数据。

Method: 结合阿塔卡马大型毫米波/亚毫米波阵列(ALMA)和詹姆斯·韦伯太空望远镜(JWST)的观测能力，在空间分辨尺度上分析高红移(high-redshift)星系的星际介质、运动学、形态学、活动星系核(AGN)及恒星形成活动。

Result: ALMA（Atacama Large Millimeter/submillimeter Array）和JWST（James Webb Space Telescope）的协同观测揭示了高红移（high-redshift）星系的气体、尘埃、恒星、星际介质（interstellar medium）、运动学特征、形态、活动星系核（active galactic nuclei）及恒星形成活动的空间分辨细节。
...
**TL;DR**: 研究发现棒旋星系（barred-galaxy）模型中气体流动受稳定周期轨道（x1 stable periodic orbits）大环结构影响，激波轨迹偏离环区并形成与直线激波成角度的延伸结构，同时在环区起始距离处观测到密集气体"尾流"（tails）流向直线激波。


<details>
  <summary>Details</summary>

Background: 研究聚焦于棒旋星系（barred-galaxy）模型中气体流动的动力学特征，重点关注4:1共振区与共转（corotation）区域之间混沌主导环境下激波形态的形成机制。

Data: 研究中使用的数据来源于两种不同的流体动力学计算方案：无网格拉格朗日SPH方法和自适应网格细化代码RAMSES。

Method: 采用两种概念上不同的流体动力学方案进行计算，即无网格拉格朗日SPH（smoothed particle hydrodynamics）方法和自适应网格细化代码RAMSES（adaptive mesh refinement code）。

Result: 棒旋星系（barred-galaxy）模型中气体流动的主要形态特征表现为激波轨迹偏离大环轨道（x1稳定周期轨道）区域并形成与直线激波成角度的延伸结构。
...
**TL;DR**: 基于积分视场光谱数据，研究发现NGC 7331星系盘中136个电离氢区(HII regions)的Hα光度与电离气体速度弥散(σ)存在相关性，并确定其速度弥散与恒星形成率面密度(Σ_SFR)的关系符合σ∝80Σ_SFR^0.285，表明恒星反馈驱动湍流。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近邻星系NGC 7331盘面HII区（HII region）的电离气体运动学特征及其与恒星形成活动的关联。

Data: 数据来源于环星系Hα光谱仪(CHαS)的积分视场单元(IFU)数据和SIRTF邻近星系巡天(SINGS)的窄带Hα图像。

Method: 采用积分视场单元（IFU）光谱观测技术结合窄带Hα测光（aperture photometry），基于CHαS和SINGS数据提取电离气体速度弥散（σ）和Hα光度（L(Hα)），并通过高信噪比像素筛选构建星系盘高分辨率速度场与弥散图。

Result: NGC 7331的HII区电离气体运动学研究表明，其速度弥散($σ$)与恒星形成率面密度($\rm Σ_{SFR}$)存在相关性，最佳拟合关系为$\rm σ\propto 80Σ_{SFR}^{0.285}$。
...
### [5] [Nitrogen enhancement of GN-z11 by metal pollution from supermassive stars](https://arxiv.org/abs/2601.04344)
*Sho Ebihara,Michiko S. Fujii,Takayuki R. Saitoh,Yutaka Hirai,Yuki Isobe,Chris Nagele*

**TL;DR**: 通过星系形成模拟验证了超大质量恒星（SMS）污染模型能解释高红移（high-z）星系GN-z11的高氮氧比（N/O）及其他丰度特征，并表明污染质量分数需在10-30%范围内。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用詹姆斯·韦伯太空望远镜（JWST）观测到的高红移（high-redshift）星系中异常高的氮氧比（N/O ratio）现象，探讨超巨星（supermassive stars, SMSs）通过恒星风污染（pollution）对星系化学丰度模式的潜在影响。

Data: 数据来源于詹姆斯·韦伯太空望远镜（JWST）对高红移（high-redshift）星系GN-z11（z=10.6）的光谱观测，以及包含化学演化的宇宙学放大模拟（cosmological zoom-in simulation），该模拟考虑了旋转大质量恒星（Wolf-Rayet stars）、超新星和渐近巨星分支（asymptotic giant branch stars）的贡献，并后处理假设了质量为$10^3$-$10^5$ $M_\odot$的超大质量恒星（supermassive stars, SMSs）形成及其抛射物的影响。

Method: 采用宇宙学放大模拟（cosmological zoom-in simulation）结合化学演化模型，并假设存在质量为$10^3$-$10^5$ $M_\odot$的超大质量恒星（SMS），通过其后处理计算其抛射物对丰度模式的贡献。

Result: 高红移(z)星系GN-z11的高氮氧比(N/O)可通过质量介于$10^3$-$10^5$ $M_\odot$的超大质量恒星(SMS)污染模型解释，其污染质量分数为10%-30%时能复现包括碳氧比和氧氢比在内的丰度模式。
...
### [6] [A catalogue of Type 2 active galactic nuclei in the 6dF Galaxy Survey](https://arxiv.org/abs/2601.04457)
*Sruthi Suresh,Wei Jeat Hon,Rachel L Webster,Christian Wolf,Christopher A Onken*

**TL;DR**: 基于6dF星系巡天(6dFGS)数据构建了包含5000个低红移(z~0.032)Ⅱ型活动星系核(Type 2 AGN)的新样本，其[OIII]发射线光度中位数为10^40.04 erg/s。


<details>
  <summary>Details</summary>

Background: 活动星系核（AGN）是由超大质量黑洞吸积周围气体和尘埃驱动的星系中心致密高能区域，其光学光谱可通过强发射线特征（宽线和/或窄线）识别。

Data: 数据来源于6dF星系巡天（6dFGS）的136,304条光谱，覆盖约17,000平方度的南天区低红移星系。

Method: 采用改进的Python类星体拟合工具（PyQSOFit）对6dFGS光谱进行拟合，并通过连续谱拟合后的中值绝对偏差截断筛选发射线星系，结合鲍德温-菲利普斯-特列维奇图（BPT diagram）分类出5000个II型活动星系核（Type 2 AGN）。

Result: result：基于6dFGS数据新发现了5000个通过BPT图分类的II型活动星系核(Type 2 AGN)，其典型红移为z~0.032且[O III]发射线光度中值为10^40.04 erg/s。
...
### [7] [J-PLUS: The planetary nebula population of M 33](https://arxiv.org/abs/2601.04533)
...
**TL;DR**: 利用J-PLUS多波段测光数据首次在M33星系中识别出行星状星云（PNe）候选体，发现16个具有PNe特征颜色的源（其中1个可能为晕族PNe），并验证了该方法在缺乏已知PNe星表的外星系研究中的可行性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用多波段测光数据（J-PLUS）探测近邻螺旋星系M~33中的行星状星云（planetary nebulae, PNe）种群，探索其在河外星系中的识别方法及科学价值。

Data: 数据来源于Javalambre-Photometric Local Universe Survey (J-PLUS)的DR3版本12波段测光数据集，并利用SExtractor软件从J-PLUS图像中提取源以获取文献中已知行星状星云(planetary nebulae, PNe)的测光数据，尽可能进行点扩散函数(PSF)测光。

Method: 采用12波段测光数据（J-PLUS DR3）和SExtractor软件进行源提取，结合点扩散函数（PSF）测光技术，并利用基于银河系晕行星状星云（PNe）标准开发的诊断性颜色-颜色图（DCCDs）进行候选体筛选。

Result: 利用J-PLUS多波段测光数据结合DCCDs（diagnostic color-color diagrams）分析，成功识别出16个具有行星状星云（PNe）特征颜色的源，但其中仅1个可能为晕族PNe（halo PN），其余多为H II区（H II regions），凸显了仅依赖测光数据区分PNe与污染源的挑战性。
...
### [8] [Dual AGNs on 100 kpc Scales from the Million Quasar Catalog](https://arxiv.org/abs/2601.04627)
*Zhuojun Deng,Cheng Xiang,Qihang Chen,Liang Jing,Xingyu Zhu,Jianghua Wu*

**TL;DR**: 从百万类星体目录中新筛选出173个低红移(z<0.5)双活动星系核(dual AGNs)、4个AGN三重系统和1个AGN四重系统，补充了现有低红移双AGN样本，并发现约16%的成对系统具有潮汐特征。


<details>
  <summary>Details</summary>

Background: 研究双活动星系核（dual AGNs）对理解星系与超大质量黑洞（supermassive black holes）的协同演化至关重要。

Data: 数据来源于百万类星体目录（Million Quasars Catalog）和暗能量光谱仪遗产巡天（Dark Energy Spectroscopic Instrument Legacy Survey）的光学图像。

Method: 采用光学图像目视检查（visual inspection）和统计分析（statistical analyses）方法，结合暗能量光谱仪（DESI）巡天数据筛选低红移（$z < 0.5$）双活动星系核（dual AGNs）样本，并统计其潮汐特征（tidal features）和速度差（$|Δv|$）分布。

Result: 从百万类星体目录中新筛选出173个双活动星系核（AGN）、4个AGN三重系统和1个AGN四重系统，补充了现有低红移（z < 0.5）双活动星系核的统计样本。
...
### [9] [Optimization of Deep Learning Models for Radio Galaxy Classification](https://arxiv.org/abs/2601.04773)
*Philipp Denzel,Manuel Weiss,Elena Gavagnin,Frank-Peter Schilling*

**TL;DR**: 研究发现现有预训练深度学习架构（如卷积神经网络和Transformer）通过数据适配和集成学习可在不增加模型复杂度的情况下实现射电星系（radio galaxies）定位与分类的高精度（超90%），性能媲美定制模型，适用于SKAO等未来射电巡天数据。


<details>
  <summary>Details</summary>

Background: 现代射电望远镜巡天（wide-field surveys）探测数十亿星系的能力使人工形态分类不可行，亟需解决再电离时期（Epoch of Reionization）研究中21厘米信号前景污染源的识别与扣除问题。

Data: 数据来源于公开的射电星系数据集（Radio Galaxy Zoo）以及未来可转移应用的默奇森宽场阵列（MWA）和平方公里阵列观测站（SKAO）的巡天数据。

Method: 采用预训练深度学习（DL）模型（包括卷积神经网络CNN和基于transformer的算法）进行微调，并通过集成分析和数据增强（如图像通道复制与变换）优化性能。

Result: 通过微调预训练深度学习(DL)架构并采用数据增强策略，可在不增加模型复杂度的前提下实现与定制模型相当的射电星系分类性能（准确率超90%）。
...
### [10] [Rapid emergence of overmassive black holes in the early Universe](https://arxiv.org/abs/2601.04955)
*Sunmyon Chon,Shingo Hirano,Tomoaki Ishiyama,Seok-Jun Chang,Volker Springel*

**TL;DR**: 首次通过全宇宙学辐射流体动力学模拟揭示，原星团环境中超大质量黑洞（SMBHs）可自然形成质量达10^6太阳量级的重种子（heavy seeds），并通过持续超爱丁顿吸积（super-Eddington accretion）快速成长为z~8时3×10^7太阳质量，解释了JWST观测到的过质量黑洞（overmassive black holes）与小红点（LRDs）的演化关联。


<details>
  <summary>Details</summary>

Background: 超大质量黑洞（SMBHs）的起源是天体物理学中长期存在的难题，尤其关注高红移（z>4-6）环境中超出局部标度关系的超大质量黑洞（overmassive black holes）如何形成和快速生长。

Data: 数据来源于完全宇宙学辐射流体动力学模拟（fully cosmological radiation-hydrodynamic simulations），自洽地追踪了超大质量黑洞（SMBHs）在原始星系团环境中的诞生、早期生长及可观测特征。

Method: 采用完全宇宙学辐射流体动力学模拟（fully cosmological radiation-hydrodynamic simulations）自洽追踪超大质量黑洞（SMBHs）在原始星系团环境中的形成、早期生长及可观测特征。

Result: result：研究发现超大质量黑洞（SMBHs）的重种子（heavy seeds）质量可达$10^6 M_\text{sun}$，并通过持续的超爱丁顿吸积（super-Eddington accretion）快速增长至$z \sim 8$时的$\sim 3 \times 10^7 ~M_\text{sun}$，为JWST观测到的过质量类星体（overmassive quasars）和现今SMBHs的前身提供了统一物理解释。
...
### [11] [Through Thick and Thin: The Cosmic Evolution of Disk Scale Height](https://arxiv.org/abs/2601.04988)
...
**TL;DR**: 基于JWST COSMOS-Web巡天对2631个红移0<z<3.5的大质量(M*>10^10 M⊙)侧向盘星系的垂直结构研究，发现高红移(z>1.5)单成分盘具有中等厚度(0.56±0.03 kpc)且长度-高度比恒定(2.7±0.2)，通过表面质量密度降低或引力不稳定性增厚，而薄盘(z≲1)从z≈2开始生长并主导低红移系统，最终形成现今厚薄双成分盘结构。


<details>
  <summary>Details</summary>

Background: 研究聚焦于盘星系（disk galaxies）垂直结构的形成与演化机制，特别关注高红移（high-redshift）单成分盘（single-component disks）如何通过表面质量密度降低或引力不稳定性（gravitational instabilities）演化为现今厚薄双成分盘（thick thin disk systems）的连续过程。

Data: 数据来源于JWST COSMOS-Web巡天项目中的2631个侧向盘星系（edge-on disk galaxies），样本恒星质量$M_*>10^{10} M_\odot$且红移范围$0< z < 3.5$。

Method: 采用全局$\operatorname{sech}^2$尺度高度测量方法，并开发新的投影偏差校正技术，在固定1微米静止波长下分析星系垂直结构演化。

Result: 高红移（high-redshift）单成分盘（single-component disks）通过表面质量密度降低和/或剧烈引力不稳定性增加垂直尺度高度，最终形成厚盘（thick disks），而薄盘（thin-disk）生长始于红移$z\approx2$并在$z\lesssim1$时占主导，导致垂直尺度高度从$z\approx1$到$0$逐渐减小。
...
### [12] [Detection of time delay between UV and X-ray variability in Mrk 1044 using AstroSat observations](https://arxiv.org/abs/2601.05135)
*M. Reshma,C. S. Stalin,Amit Kumar Mandal,Abhijit Kayal,S. B. Gudennavar,Prajwel Joseph*

**TL;DR**: 通过分析窄线赛弗特1型星系(NLS1) Mrk 1044的远紫外(FUV)和X射线波段同步观测数据，首次发现X射线变化领先FUV变化2.25-2.35天，支持了日冕X射线辐射激发吸积盘再处理产生紫外光变的物理模型。


<details>
  <summary>Details</summary>

Background: 活动星系核(AGN)在电磁波谱全波段表现出通量变化，其中紫外/光学与X射线波段通量变化的关联性为理解吸积盘与冕区(corona)的物理联系提供了关键诊断依据。

Data: 数据来源于2018年8月31日至9月8日期间使用搭载在AstroSat卫星上的紫外成像望远镜（Ultraviolet Imaging Telescope）和软X射线望远镜（Soft X-ray Telescope）对窄线塞弗特1型星系Mrk 1044进行的远紫外波段（FUV: 1300–1800 Å）和X射线波段（0.5–7 keV）同步观测。

Method: 采用插值互相关函数（ICCF）和JAVELIN方法进行交叉相关分析，以探索紫外（UV）和X射线波段之间的潜在时间延迟。

Result: X射线变化领先远紫外(FUV)变化2.25±0.05天(ICCF方法)和2.35+0.02−0.01天(JAVELIN方法)，支持冕区X射线辐射吸积盘(accretion disk)再处理导致紫外变化的物理模型。
...
### [13] [Unveiling the 3D structure of the central molecular zone from stellar kinematics and photometry: The 50 and 20 km/s clouds](https://arxiv.org/abs/2601.05252)
...
**TL;DR**: 通过分析恒星运动学和密度分布，发现银河系中心分子区（CMZ）中的50 km/s和20 km/s分子云位于人马座A*（Sgr A*）近侧（距离分别为43±8 pc和56±11 pc），并通过脊（ridge）物理连接。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中心分子区(CMZ)的三维结构解析，特别是针对50和20 km/s分子云视线距离的精确测定及其物理关联性探索。

Data: 数据来源为恒星运动学（stellar kinematics）、恒星密度（stellar densities）和恒星族（stellar populations）的观测数据，以及对照区（control field）的测光数据（photometry）。

Method: 通过分析恒星运动学（stellar kinematics）、恒星密度（stellar density）和恒星族（stellar population）并结合理论恒星分布模型（theoretical model of stellar distribution）来约束分子云的视线距离（line-of-sight distance）。

Result: 通过恒星运动学和密度分布分析，发现50 km/s云和20 km/s云位于人马座A*（Sgr A*）近侧，距离分别为43±8 pc和56±11 pc，且通过脊（ridge）物理连接。
...
### [14] [Neutrino decays as a natural explanation of the neutrino mass tension](https://arxiv.org/abs/2601.04312)
*Guillermo Franco Abellán*

**TL;DR**: 通过分析DESI DR2和CMB数据，发现中微子衰变为无质量的新物理粒子（massless BSM particles）可将宇宙学总中微子质量上限放宽至0.23 eV（95%置信度），从而与振荡实验数据达成一致，而衰变为更轻中微子的场景对质量界限的缓解效果有限且依赖于质量排序（mass ordering）。


<details>
  <summary>Details</summary>

Background: 宇宙学观测对中微子总质量（$\sum m_ν$）的严格上限与振荡实验给出的下限之间存在新张力，可能对宇宙学和粒子物理学产生深远影响。

Data: 数据来源于DESI DR2（暗能量光谱仪第二阶段数据）和CMB（宇宙微波背景辐射）数据集。

Method: 采用宇宙微波背景(CMB)和暗能量光谱仪(DESI DR2)数据约束中微子衰变模型，分析中微子衰变为无质量BSM粒子或更轻中微子对总质量限制的影响。

Result: 中微子（neutrino）衰变为无质量的新物理（BSM）粒子可将总中微子质量上限放宽至0.23电子伏特（eV），从而与振荡实验数据达成一致。
...
### [15] [Diluting the Dark Sector: A Common Origin for the PTA Signal and Inelastic SIDM](https://arxiv.org/abs/2601.04340)
*Zihan Wang*

**TL;DR**: 提出辐射性自相互作用暗物质（SIDM）稀释框架，通过暗扇区超冷相变产生的宇宙学峰值信号（成核温度T*≈1.24 MeV，持续时间倒数β/H*≈150）与受SIDM核晕抑制的天体物理背景混合，解释了纳赫兹随机引力波背景（SGWB）观测谱，同时解决了热过量SIDM残留密度和哈勃张力问题。


<details>
  <summary>Details</summary>

Background: 该研究探讨了纳赫兹随机引力波背景（SGWB）观测信号超出标准超大质量黑洞双星（SMBHB）模型预期的科学挑战，提出暗物质自相互作用（SIDM）和暗区超冷相变（supercooled phase transition）的混合机制解释信号谱特征。

Data: 数据来源包括纳赫兹随机引力波背景（SGWB）的观测信号、自相互作用暗物质（SIDM）的核晕动力学抑制效应以及暗区超冷相变产生的宇宙学峰值信号。

Method: 采用辐射性自相互作用暗物质稀释框架（Radiative SIDM Dilution），结合自由谱重构和贝叶斯模型比较方法，通过暗区超冷相变（supercooled phase transition）产生的成核温度$T_* \approx 1.24$ MeV和逆持续时间$β/H_* \approx 150$定义相变参数。

Result: 观测到的纳赫兹随机引力波背景（SGWB）信号幅度超出标准超大质量黑洞双星（SMBHB）模型的预测，可通过辐射性自相互作用暗物质（SIDM）稀释框架解释为天体物理背景与暗区超冷相变产生的宇宙学峰值的混合信号。
...
### [16] [Probing Trans-Planckian Signatures in the Early Universe: A Bayesian Analysis of the Generalized Sasaki-Mukhanov Equation](https://arxiv.org/abs/2601.04760)
*Mahdieh Eskandari Merajin*

**TL;DR**: 提出含时间相关修正项的广义Sasaki-Mukhanov方程，精确求解Whittaker函数形式的模函数，发现原初标量功率谱存在尺度依赖振荡调制及低多极矩功率抑制，约束耦合常数|f|≤10^-4，并缓解了低四极矩异常。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过广义暴胀扰动理论解决宇宙微波背景辐射（CMB）大尺度异常问题，探索早期宇宙中可能存在的跨普朗克（Trans-Planckian）问题和非正则动力学。

Data: 数据来源于普朗克卫星2018年发布的温度与偏振似然函数（Planck 2018 temperature and polarization likelihoods）及阿塔卡马宇宙学望远镜第六次数据发布（ACT DR6）的高分辨率数据。

Method: 采用广义Sasaki-Mukhanov方程（含时间依赖性修正项）的精确解析解（Whittaker函数）结合Cobaya贝叶斯推断引擎的蒙特卡洛马尔可夫链（MCMC）数值分析。

Result: 引入含时间相关修正项的广义佐佐木-牟田方程（generalized Sasaki-Mukhanov equation）后，原初标量功率谱（primordial scalar power spectrum）在低多极子（low multipoles）区域呈现尺度依赖的振荡调制和显著功率抑制，以95%置信度将修正参数|f|约束在10^-4以内，且该模型比标准LambdaCDM模型能更好地拟合低-l CMB谱，缓解了低四极矩异常（low-quadrupole anomaly）。
...
### [17] [First measurement of the Hubble constant from a combined weak lensing and gravitational-wave standard siren analysis](https://arxiv.org/abs/2601.04774)
*Felipe Andrade-Oliveira,David Sanchez-Cid,Danny Laghi,Marcelle Soares-Santos*

**TL;DR**: 首次联合标准汽笛（standard siren）与弱引力透镜及星系成团性（3×2pt）观测数据测得哈勃常数（H0）为67.9+4.4−4.3 km s−1 Mpc−1（精度6.4%），并将物质总量（Ωm）约束提升22%，证明将标准汽笛融入宇宙学大尺度巡天框架是解决H0测量分歧的有效途径。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过结合标准汽笛(standard sirens)、弱引力透镜和星系成团性观测来精确测量哈勃常数(Hubble constant, H0)，以解决当前宇宙学中H0测量值显著不一致的核心问题。

Data: 数据来源包括暗能量巡天（Dark Energy Survey, DES）第三年发布的数据、LIGO-Virgo-KAGRA（LVK）合作组发布的引力波瞬变源目录4.0（Gravitational-Wave Transient Catalog 4.0）数据，以及GW170817宿主星系红移和由残余物超光速喷流观测导出的倾角估计值。

Method: 采用标准汽笛（standard sirens）与弱引力透镜及星系成团性观测（3×2pt）的联合分析方法，并结合宿主星系红移和相对论喷流倾角测量。

Result: 通过联合分析标准汽笛（standard siren）与弱引力透镜及星系成团性观测数据（3×2pt），测得哈勃常数（Hubble constant）$H_0 = 67.9^{+4.4}_{-4.3}$~km~s$^{-1}$~Mpc$^{-1}$，精度达6.4%，并利用GW170817喷流信息将$H_0$测量精度提升至9.9%。
...
### [18] [Euclid preparation. Galaxy 2-point correlation function modelling in redshift space](https://arxiv.org/abs/2601.04780)
...
**TL;DR**: 通过比较星系两点相关函数（2PCF）多极矩的不同模型，发现VDG$_{\infty}$和CLEFT模型在低红移（$z=0.9$）下分别能在模板拟合和全形状分析中达到最小拟合尺度$20\,h^{-1}\,{\rm Mpc}$且无偏，而高红移（$z=1.8$）时CLPT模型已足够精确，为欧几里得（Euclid）卫星数据配置空间分析提供了最优模型选择基准。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用欧几里得卫星(Euclid)观测数据，通过测试多种先进模型（如有效场论(EFT)、速度差生成函数(VDG$_{\infty}$)和拉格朗日扰动理论(LPT)变体）来优化星系两点相关函数(2PCF)多极矩分析，以支持第四阶段巡天(Stage-IV)对宇宙学参数的精确约束。

Data: 研究使用了来自Flagship 1模拟（Flagship 1 simulation）的数据，该模拟包含红移（redshift）为$z\in\{0.9,1.2,1.5,1.8\}$的四个快照（snapshot）。

Method: 采用多极矩模型比较方法，测试了有效场论（EFT）、速度差生成函数模型（VDG$_{\infty}$）以及拉格朗日微扰理论（LPT）的不同变体（如CLPT和CLEFT），并分析了模板拟合和全形状两种分析策略。

Result: 在模板拟合方法中，仅速度差生成函数模型（VDG$_{\infty}$）能在红移0.9时达到最小拟合尺度20$h^{-1}$Mpc且不引入参数偏差，而有效场论模型（EFT）在30$h^{-1}$Mpc时已不准确；在全形状分析中，卷积拉格朗日微扰理论扩展模型（CLEFT）和VDG$_{\infty}$表现相近，但仅CLEFT能达到20$h^{-1}$Mpc，而VDG$_{\infty}$在最低红移下无偏地拟合至25$h^{-1}$Mpc；为满足欧几里得卫星精度要求，需采用VDG$_{\infty}$或CLEFT等非微扰模型；在红移1.8时，卷积拉格朗日微扰理论模型（CLPT）足以高精度描述数据。
...
### [19] [Constraining the Primoridal Black Hole Abundance with Space-Based Detectors](https://arxiv.org/abs/2601.05069)
*Wencong Hong,Shi Pi,Ao Wang,Zhenyu Zhang*

**TL;DR**: 这篇论文通过分析未来空间引力波探测器（LISA、Taiji和TianQin）对原初曲率扰动产生的标量诱导引力波（GWs）的探测能力，发现它们能完全覆盖小行星质量窗口的原初黑洞（PBHs）丰度约束，并证实若PBHs构成暗物质主体，诱导GWs将在mHz频段产生信噪比达$10^3\sim10^4$的强信号。


<details>
  <summary>Details</summary>

Background: 该研究探讨了原初黑洞（PBH）作为暗物质候选者在早期宇宙中通过密度过高区域坍缩形成的机制，重点关注其与标量诱导引力波（GWs）的关联性。

Data: 研究使用的数据来源于未来空间引力波探测器LISA、Taiji和TianQin对原初曲率扰动产生的标量诱导引力波（scalar-induced gravitational waves）的模拟观测。

Method: 采用标量诱导引力波（scalar-induced gravitational waves）分析方法，结合未来空间引力波探测器（LISA、Taiji、TianQin）的灵敏度模拟，系统研究了原初曲率功率谱宽度对PBH丰度的约束能力。

Result: 未来空间引力波探测器（LISA、Taiji和TianQin）可完全探测小行星质量窗口的原初黑洞（PBH）丰度，若PBH构成暗物质主体，诱导引力波在mHz波段将产生信噪比达$10^3\sim10^4$的强信号。
...
### [20] [How deep can a cosmic void be? Voids-informed theoretical bounds in Galileon gravity](https://arxiv.org/abs/2601.05145)
*Tommaso Moretti,Noemi Frusciante,Giovanni Verza,Francesco Pace*

**TL;DR**: 该论文提出了一种基于空洞（void）的普适性检验方法，揭示了伽利略标量-张量理论（Galileon scalar-tensor theories）中牛顿力预测失效与非线性空洞动力学及宇宙膨胀历史间的单条件关联，由此建立了红移依赖的空洞深度上限作为新理论判据，并排除了60%参数空间内$z\lesssim 10$失效的模型，从而将空洞发展为筛选修正引力理论的有效工具。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用宇宙空洞（void）作为一致性测试工具，检验伽利略标量-张量理论（Galileon scalar-tensor theories）的物理合理性。

Data: 研究中使用的数据来源为理论约束和观测限制下的尺度因子线性参数化（linear parameterization in the scale factor）。

Method: 采用非线性空洞（void）动力学与宇宙膨胀历史关联的单条件约束方法，检验伽利略标量-张量理论（Galileon scalar-tensor theories）的自洽性。

Result: 在伽利略标量-张量理论（Galileon scalar-tensor theories）中，空洞（void）动力学与宇宙膨胀历史的关联导出了一个红移依赖的空洞深度上限，该条件作为新的理论可行性判据排除了约60%的参数空间，其中多数问题模型在红移小于10时失效。
...
### [21] [Symbolically regressing dark matter halo profiles using weak lensing](https://arxiv.org/abs/2601.05203)
*Alicia Martín,Tariq Yasin,Deaglan J. Bartlett,Harry Desmond,Pedro G. Ferreira*

**TL;DR**: 提出一种基于符号回归算法（ESR）直接从观测数据约束暗物质晕（dark matter halo）密度剖面的方法，发现其最优剖面在弱引力透镜探测半径内（0.3-3 h$^{-1}$ Mpc）具有较浅内缘和密度峰值特征，统计性能优于NFW模型，且拟合质量估计普遍更高。


<details>
  <summary>Details</summary>

Background: 研究主题为通过观测数据直接约束暗物质晕（dark matter haloes）密度分布，避免依赖存在不确定性的模拟模型。

Data: 数据来源于HSC-XXL巡天观测的149个星系团样本，通过弱引力透镜（weak-lensing）数据在半径范围$R \sim 0.3 - 3$ h$^{-1}$ Mpc内约束暗物质晕密度分布。

Method: 采用符号回归算法（Exhaustive Symbolic Regression, ESR）直接拟合观测数据，寻找最优解析表达式以描述暗物质晕密度分布。

Result: 通过符号回归算法（ESR）在HSC-XXL巡天的149个星系团样本中识别出统计上优于NFW的密度分布模型，其在内区呈现较浅行为并在密度分布中存在峰值。
...
### [22] [Mitigating Simulator Dependence in AI Parameter Inference for the Epoch of Reionization: The Importance of Simulation Diversity](https://arxiv.org/abs/2601.05229)
*Jasper Solt,Jonathan C. Pober,Stephen H. Bach*

**TL;DR**: 提出通过增加训练数据集多样性（使用多种模拟器数据）提升AI模型泛化能力，有效减少再电离时期（EoR）21厘米信号参数推断中的模拟器特异性偏差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用21厘米中性氢信号探索宇宙再电离时期（Epoch of Reionization, EoR）这一观测约束较弱的宇宙学历史阶段。

Data: 研究中使用的数据来源于四种不同模拟器（simulator）生成的21厘米信号仿真数据。

Method: 采用多模拟器（multiple simulators）数据训练AI模型以提高泛化能力。

Result: 增加训练数据集的多样性（即结合多个模拟器的数据）能提升AI模型在再电离时期（EoR）21厘米信号参数推断中的泛化能力，减少模拟器特定偏差。
...
### [23] [Mimicking Phantom Dark Energy with Evolving Dark Matter Mass](https://arxiv.org/abs/2601.05235)
*Lorenzo La Penna,Alessio Notari,Michele Redi*

**TL;DR**: 提出了一种通过暗能量（DE）与暗物质（DM）能量交换再现给定宇宙学背景的通用方法，利用标准quintessence标量场控制DM质量，可在不引入鬼态或其他病态情况下实现幻影穿越（phantom crossing）背景，并精确复现DESI+CMB+DESY5数据最佳拟合的CPL参数化背景，同时因扰动差异导致结构增长改变，若DM质量在晚期变化则仅影响晚期可观测量（如物质功率谱、CMB透镜和ISW效应），通常产生约10%偏差，但特定参数选择可减小物质功率谱影响。


<details>
  <summary>Details</summary>

Background: 研究主题为通过暗能量（DE）和暗物质（DM）之间的能量交换机制再现给定宇宙学背景演化。

Data: 数据来源于对暗能量（DE）和暗物质（DM）能量交换的模拟（simulation）研究，基于标准quintessence标量场控制DM质量的理论框架，并参考了DESI+CMB+DESY5观测数据的最佳拟合结果。

Method: 采用标准quintessence标量场控制暗物质（DM）质量的方法，通过暗能量（DE）与暗物质之间的能量交换重现给定宇宙学背景演化。

Result: 通过暗能量（DE）与暗物质（DM）间的能量交换机制，可精确复现给定宇宙学背景演化（包括幻影穿越现象），同时保持微扰行为与目标模型差异并导致结构增长改变。
...
### [24] [The dynamical and thermodynamic effects of turbulence on the cosmic baryonic fluid](https://arxiv.org/abs/2503.06593)
*Yun Wang,Minxing Li,Ping He*

**TL;DR**: 利用IllustrisTNG50-1模拟揭示了湍流（turbulence）对宇宙重子流体中缺失重子（missing baryons）在温热星系际介质（WHIM）中的分布和物理起源起关键作用，挑战了传统激波加热观点并证实湍流在塑造大尺度结构中比激波更重要。


<details>
  <summary>Details</summary>

Background: 该研究探讨了宇宙中缺失重子（missing baryons）在温热星系际介质（WHIM）中的分布及其物理起源，强调了宇宙重子流体中的湍流（turbulence）对理解大尺度结构形成和星系演化的重要性。

Data: 使用了IllustrisTNG50-1模拟数据，并验证了TNG50-2、TNG100-1、WIGEON和EAGLE模拟数据以确保结果不受空间分辨率、模拟盒尺寸和亚网格物理变化的影响。

Method: 采用IllustrisTNG50-1模拟分析宇宙重子流体湍流（turbulence）对缺失重子（missing baryons）空间分布和物理起源的影响，并通过TNG50-2、TNG100-1、WIGEON和EAGLE模拟验证关键发现。

Result: 湍流（turbulence）在宇宙重子流体中的动力学和热力学效应是理解缺失重子（missing baryons）在温热星系际介质（WHIM）中分布和起源的关键因素。
...
### [25] [Sustainable, Local Socio-Economic Development Through Astronomy](https://arxiv.org/abs/2601.04224)
*Joyful E. Mdhluli*

**TL;DR**: 论文通过南非、智利、印尼和印度的案例研究，提出以参与式天文教育、旅游、技术转移和能力建设推动当地社会经济可持续发展的实践框架，证实天文投资能转化为促进就业、中小企业发展和STEM参与的具体效益，同时识别了利益分配不均等挑战。


<details>
  <summary>Details</summary>

Background: 研究探讨了天文学作为促进当地社会经济可持续发展的潜在驱动力（astronomy for sustainable development），聚焦其通过教育、旅游、技术转移和能力建设创造实际效益的宏观科学主题。

Data: 数据来源于南非、智利、印度尼西亚和印度的天文设施及社区倡议案例研究（case studies）。

Method: 采用案例研究（case study）方法分析南非、智利、印尼和印度的天文设施及倡议对当地社会经济的影响，并基于参与式（participatory）原则提出与联合国可持续发展目标（SDGs）对接的实践框架。

Result: 天文学通过教育、旅游、技术转移和能力建设为社区创造实际效益，成为包容性增长的催化剂。
...
### [26] [Exploring Metal Additive Manufacturing in Martian Atmospheric Environments](https://arxiv.org/abs/2601.04232)
*Zane Mebruer,Wan Shou*

**TL;DR**: 研究发现二氧化碳(CO2)可替代氩气(Ar)作为选择性激光熔化(SLM)保护气体，在火星大气环境下实现316L不锈钢的可行制造，虽表面质量和抗氧化性略逊于氩气但显著优于空气。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用火星大气主要成分二氧化碳（CO2）替代氩气（argon）作为选择性激光熔化（SLM）工艺的保护气体，以支持地外原位制造（in-space manufacturing）的可持续发展需求。

Data: 数据来源于在氩气（argon）、二氧化碳（CO2）和常压空气环境下，通过宽范围激光参数制备的316L不锈钢单道和二维试样。

Method: 采用选择性激光熔化（SLM）技术，在氩气、二氧化碳（CO2）和常压空气环境下通过单道和二维316L不锈钢试样实验，评估大气成分对表面形貌、微观结构内聚力和氧化行为的影响。

Result: 在火星大气主要成分二氧化碳(CO2)环境下，选择性激光熔化(SLM)技术可生产性能介于氩气和空气之间的316L不锈钢部件，表明其具备火星原位制造的可行性。
...
### [27] [Charge Migration and Residual Non-Linearity in NIRSpec BOTS Observations](https://arxiv.org/abs/2601.04255)
*Munazza K. Alam,Leonardo Ubeda,Qinyan,Lu,Nestor Espinoza,Nikolay Nikolov*

**TL;DR**: 通过分析WASP-121b的全轨道相位曲线观测数据，发现JWST/NIRSpec G395H的NRS1和NRS2探测器存在光谱迹线中心电荷损失和边缘电荷过剩现象，其中NRS1的非线性偏差约为NRS2的3倍，且NRS1的电荷分布随行星轨道相位变化而改变光谱迹线形态，而NRS2未表现出这种影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用韦伯太空望远镜(JWST)近红外光谱仪(NIRSpec)G395H模式下NRS1和NRS2探测器在超热木星WASP-121b观测中电荷迁移(charge migration)和残余非线性(residual non-linearity)效应的表征。

Data: 数据来源于詹姆斯·韦伯太空望远镜(JWST)近红外光谱仪(NIRSpec) G395H模式下NRS1和NRS2探测器的亮源时间序列(Bright Object Time Series, BOTS)观测数据，针对超热木星WASP-121b进行了持续37.8小时(1.57天)的全轨道相位曲线观测。

Method: 采用明亮天体时间序列（BOTS）观测数据，结合电荷迁移和残余非线性效应分析，对比研究了JWST/NIRSpec G395H中NRS1和NRS2探测器的非线性响应特性。

Result: NRS1探测器显示出比NRS2约3倍大的线性偏差（non-linearity deviation）。
...
### [28] [Search for Precessing Binary Black Holes in Advanced LIGO's Third Observing Run using Harmonic Decomposition](https://arxiv.org/abs/2601.04276)
*Rahul Dhurkunde,Ian Harry*

**TL;DR**: 采用谐波分解法开发了一种高效搜索自旋诱导轨道进动（spin-induced orbital precession）双黑洞（BBH）的新方法，将模板数量减少5倍并提升28%灵敏度，但未在LIGO第三观测期（O3）中发现新显著事件。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用自旋诱导轨道进动（spin-induced orbital precession）的双黑洞（BBH）系统来探索致密双星形成机制、宇宙学及广义相对论检验。

Data: 数据来源于高级LIGO（Advanced LIGO）第三次观测运行（O3）的引力波观测数据。

Method: 采用谐波分解法（harmonic decomposition method）分析自旋诱导轨道进动（spin-induced orbital precession）的波形，并引入新型方案减少谐波搜索中的滤波器数量。

Result: 提出了一种新型谐波搜索方案，将模板库所需滤波器数量降低至原有方案的1/5，并在灵敏度上实现最高28%的提升，同时计算成本降低至最高5倍。
...
### [29] [The Impact of Robotic Telescopes on Time-Domain Astronomy](https://arxiv.org/abs/2601.04347)
*Yakubu Mu'allim,J. O. Vwavware,A. Ohwofosirai*

**TL;DR**: 论文揭示了机器人望远镜（robotic telescopes）通过自主全天候监测和实时分类技术，显著提升了超新星、伽马射线暴等暂现源（transient phenomena）的探测效率与科学产出。


<details>
  <summary>Details</summary>

Background: 时间域天文(time-domain astronomy)因能够自主全天候监测天空的机器人望远镜(robotic telescopes)的广泛部署而经历了前所未有的发展。

Data: 数据来源包括ZTF（Zwicky Transient Facility）、ATLAS（Asteroid Terrestrial-impact Last Alert System）和LCOGT（Las Cumbres Observatory Global Telescope）等机器人望远镜（robotic telescopes）的自主全天候监测数据。

Method: 采用自主调度算法（scheduling algorithms）和机器学习流水线（machine learning pipelines）实现瞬变事件（transient events）的实时分类与观测。

Result: 机器人望远镜（robotic telescopes）通过自主全天候监测和机器学习实时分类，彻底改变了超新星、伽马暴等瞬变现象（transient phenomena）的探测与表征能力。
...
### [30] [Studies in Astronomical Time Series Analysis: The Double Lomb-Scargle Periodogram and Super Resolution](https://arxiv.org/abs/2601.04552)
*Jeffrey D. Scargle,Sarah Wagner*

**TL;DR**: 提出了一种实用、易用的多频周期图（omnigram）计算框架，实现了超分辨率（super resolution）频谱分析，适用于任意采样时间序列，并在白矮星J0135+5722等实例中验证了其应用价值。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发一种实用且易用的多频周期图（multiple-frequency periodograms）计算框架，用于分析包含两个或更多独立正弦波的时序数据，并突破瑞利准则（Rayleigh criterion）实现超分辨率（super resolution）。

Data: 研究中使用的数据来源包括合成数据（synthetic data）、太阳黑子数（sunspot numbers）和白矮星J0135+5722的脉动观测数据（pulsations of white dwarf J0135+5722）。

Method: 基于多频周期图（multiple-frequency periodograms）的时间序列模型分析方法，采用两个或更多独立正弦波构建计算框架，具有超越瑞利准则（Rayleigh criterion）的超分辨率能力，支持统计加权和锥化处理。

Result: 提出了一种实用、易用的多频率周期图计算框架，实现了超分辨率（super resolution）分析并支持统计加权和锥化处理。
...
### [31] [Explainable machine learning classification of \textit{Chandra} X-ray sources: SHAP analysis of multi-wavelength features](https://arxiv.org/abs/2601.04650)
*Shivam Kumaran,Samir Mandal,Sudip Bhattacharyya*

**TL;DR**: 利用LightGBM模型和可解释人工智能(XAI)技术对54,770个钱德拉(Chandra)X射线源进行可靠分类(置信度>3σ)，揭示了活动星系核(AGN)、恒星(Stars)和年轻恒星体(YSO)的多波段特征边界，并公开了首个结合XAI分析的大规模天文分类目录。


<details>
  <summary>Details</summary>

Background: 天文大视场巡天（如钱德拉X射线天文台）探测到大量未识别宇宙源，机器学习方法为高效概率分类提供了新途径。

Data: 数据来源于钱德拉X射线天文台(Chandra X-ray Observatory)的277,069个点源观测数据，并辅以多波段（红外、光学和X射线）特征信息。

Method: 采用可解释人工智能（XAI）技术中的SHapley Additive exPlanations（SHAP）方法量化多波段特征及其交互作用对分类概率的贡献。

Result: 利用可解释人工智能（XAI）技术揭示了红外-光学与X射线特征对区分活动星系核（AGN）和恒星的决策边界，以及红外-X射线特征对年轻恒星体（YSO）的分类边界。
...
### [32] [sidmkit: A Reproducible Toolkit for SIDM Phenomenology and Galaxy Rotation-Curve Modeling](https://arxiv.org/abs/2601.04735)
*Nalin Dhiman*

**TL;DR**: 开发了Python工具包\texttt{sidmkit}，支持自相互作用暗物质(SIDM)从微观到宏观的计算，并通过拟合SPARC数据中的旋转曲线发现Burkert模型在65.4%的星系中优于NFW模型。


<details>
  <summary>Details</summary>

Background: 暗物质自相互作用（SIDM）是冷暗物质的一种有理论依据的扩展模型，能在保持与宇宙大尺度结构一致的同时改变星系和星系群尺度的晕结构。

Data: 数据来源于SPARC数据库的191个\texttt{rotmod}星系（包含LTG和ETG类型）的旋转曲线数据，并采用NFW和Burkert暗晕模型进行拟合。

Method: 采用标准解析近似（Born、经典和Hulthén）及数值分波选项实现Yukawa相互作用的依赖速度的动量转移截面计算，并包含麦克斯韦相对速度的动量平均、散射率工具及文献约束汇总。

Result: 开发了\texttt{sidmkit}工具包，支持自相互作用暗物质（SIDM）从微观到宏观的计算，并实现对SPARC数据中旋转曲线的批量拟合。
...
### [33] [Multi-Messenger Studies with High-Energy Neutrinos and Gamma Rays: The WST Opportunity](https://arxiv.org/abs/2601.04939)
...
**TL;DR**: 该研究提出通过部署大视场高复用多目标光谱仪（MOS）和大口径望远镜快速覆盖超高能宇宙射线（UHECRs）源的中微子定位误差区域，并结合全景积分视场光谱仪（IFS）实现候选体运动学和环境特征分析，从而高效建立中微子望远镜与伽马射线天文台（CTAO）的多信使协同观测体系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高能中微子探测超高能宇宙射线（UHECRs）源这一高能天体物理学前沿问题。

Data: 数据来源于下一代中微子望远镜（IceCube-Gen2、KM3NeT）、伽马射线天文台（CTAO）以及Vera C. Rubin Observatory（LSST）的光学瞬变观测。

Method: 采用大视场高复用多目标光谱仪(MOS)结合大口径望远镜实现瞬时覆盖，并利用全景中央积分视场光谱仪(IFS)进行候选体运动学与环境特征分析。

Result: 通过部署大视场高复用多目标光谱仪（MOS）和大口径望远镜，可瞬时覆盖中微子误差范围，以前所未有的效率提供潜在对应体的关键光谱分类。
...
### [34] [Three-dimensional scene reconstruction using Roman slitless spectra](https://arxiv.org/abs/2601.05233)
...
**TL;DR**: 开发了一种三维场景重建算法（scene reconstruction），通过利用宿主星系（host galaxy）在所有可用观测角度的光谱数据来准确去除宽场无缝光谱巡天中Ia型超新星（Type Ia Supernovae）光谱的宿主星系污染，从而降低系统误差和随机噪声，提升暗能量（dark energy）参数测量的精度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用南希·格雷斯·罗曼空间望远镜（Nancy Grace Roman Space Telescope）的宽视场无缝光谱巡天数据，解决Ia型超新星（Type Ia Supernovae）光谱中宿主星系（host galaxy）光污染对暗能量（dark energy）参数测量精度的影响问题。

Data: data部分：研究使用了南希·格雷斯·罗曼空间望远镜（Nancy Grace Roman Space Telescope）模拟观测数据（simulated Roman data）进行算法验证。

Method: 开发了一种利用多角度光谱观测重建宿主星系三维（2维空间、1维光谱）表示的“场景重建（scene reconstruction）”算法。

Result: 开发了一种利用多角度光谱观测重建宿主星系三维（3d）表示（场景重建）的算法，能有效消除超新星光谱中的宿主星系污染，系统误差极小且随机噪声显著低于单次对齐光谱扣除方法。
...
### [35] [The effect of metallicity on the Leavitt Law using phase-dependent properties of classical Cepheids](https://arxiv.org/abs/2601.04769)
*Gautam Bhuyan,Shashi Kanbur,Sukanta Deb,Louise Breuval,Anupam Bhardwaj,Mami Deka,Earl P. Bellinger,Kerdaris Kurbah*

**TL;DR**: 通过多相位分析银河系、大麦哲伦云(LMC)和小麦哲伦云(SMC)中经典造父变星的光变曲线，发现周期-光度-金属丰度(PLZ)关系的系数随造父变星脉动相位动态变化，且短周期($0.4 \leq \log{P} < 1$)与长周期($1 \leq \log{P} < 2$)造父变星在$G_{\rm RP}$和$W_{G}$波段的金属丰度系数($γ_λ$)存在显著差异。


<details>
  <summary>Details</summary>

Background: 探讨造父变星(Classical Cepheid)的周光关系(PL relation)普适性及其金属丰度(metallicity)效应，旨在完善河外星系距离测量和宇宙局部膨胀率(H0)的校准。

Data: 数据来源于银河系（MW）、大麦哲伦云（LMC）和小麦哲伦云（SMC）中经典造父变星的光变曲线多相位分析。

Method: 采用多相位（multiphase）分析方法，结合辐射流体动力学（radiation hydrodynamic）模型，在五个测光波段（$V$、$I$、$G$、$G_{\rm BP}$、$G_{\rm RP}$）和两个韦森（Wesenheit）指数（$W_{VI}$、$W_{G}$）中推导了相位依赖的金属丰度系数（$γ$）。

Result: 多相位分析揭示了造父变星周期-光度-金属丰度（PLZ）关系的系数随脉动相位动态变化，且短周期与长周期造父变星在$G_{\rm RP}$和$W_{G}$波段的金属丰度系数（$γ_λ$）存在显著差异。
...
### [36] [Mutual Orbit Alignment in Resolved Triple Systems](https://arxiv.org/abs/2601.05006)
*Andrei Tokovinin*

**TL;DR**: 研究了278个外部分离距（outer separation）小于300天文单位（au）且内对（inner pair）已解析的三重系统，发现内外轨道对齐程度随外部分离距减小、内外分离距比值减小、内主星（inner primary）质量减小和内质量比（inner mass ratio）增大而提高，与外质量比（outer mass ratio）无关；全样本平均互倾角（mutual inclination）约40度，而38个内主星质量小于1太阳质量且外部分离距小于50 au的三重系统平均互倾角约10度；对齐三重系统的内轨道偏心率（inner eccentricity）小于未对齐系统；另一含371个已知外轨道和内食子系统的样本中仅22%互倾角小于20度，其余随机分布，这些发现与等级系统形成机制中N体动力学主导大尺度、吸积和迁移塑造小于100 au系统、孤立核碎裂产生低质量对齐系统的现有认知定性吻合。


<details>
  <summary>Details</summary>

Background: 研究聚焦于层级三合星系统（hierarchical triple systems）的形成动力学，探讨内、外轨道共面性与系统参数（如间距、质量比）的关联及其对碎片化（fragmentation）和吸积-迁移（accretion-migration）等形成机制的指示意义。

Data: 研究中使用的数据来源为278个外部分离（outer separation）小于300天文单位（au）且内对（inner pairs）已解析的三重系统样本，以及371个已知外轨道和内食子系统（inner eclipsing subsystems）的层次结构样本。

Method: 采用轨道构型统计分析（orbital configuration statistics）结合N体动力学（N-body dynamics）模拟，量化研究了三重系统内外轨道共面性与分离距离、质量比等参数的关联性。

Result: 内轨道与外轨道的对齐程度随以下因素增强：(i)外部分离距离减小，(ii)内外分离距离比值减小，(iii)内主星质量减小，(iv)内质量比增大。
...
### [37] [Machine learning for radiative hydrodynamics in astrophysics](https://arxiv.org/abs/2601.05155)
*Gonzague Radureau*

**TL;DR**: 开发了两种人工智能方法（多层感知机近似M1-多群闭合关系和物理信息神经网络求解辐射流体力学方程）以显著降低HADES代码模拟高温等离子体与辐射耦合过程的计算成本，其中前者将M1-多群闭合关系计算效率提升3000倍并成功应用于辐射激波高保真模拟，后者在纯流体激波中表现良好但辐射激波应用仍需改进。


<details>
  <summary>Details</summary>

Background: 辐射流体力学（radiation hydrodynamics）研究高温高超声速等离子体与其发射或吸收的辐射之间的相互作用，这一耦合机制在吸积（accretion）和抛射（ejection）等天体物理过程中起核心作用。

Data: 数据来源为HADES代码耦合的M1-gray或M1-multigroup辐射传输模型生成的仿真数据（simulation data）。

Method: 采用M1-多群(M1-multigroup)辐射传输模型与流体动力学耦合的HADES代码，并开发了多层感知机(MLP)和物理信息神经网络(PINN)两种人工智能策略来优化计算效率。

Result: 开发了两种基于人工智能的策略来克服辐射流体动力学模拟的高计算成本问题。
...
### [38] [A Relativistic MOND](https://arxiv.org/abs/2601.04290)
*Tejinder P. Singh*

**TL;DR**: 提出了一种最小相对论性MOND（修正牛顿动力学）理论框架，通过SU(2)_R规范场连接和红外（IR）度规变形实现广义相对论（GR）在高加速度区精确恢复、Bekenstein-Milgrom（AQUAL）方程在低加速度区自然涌现，且MOND加速度标度由动态选择的德西特半径确定。


<details>
  <summary>Details</summary>

Background: 研究探讨了修正牛顿动力学（MOND）的最小相对论性扩展，旨在统一高加速度区域下的广义相对论（General Relativity）与低加速度区域下的Bekenstein-Milgrom（AQUAL）方程，同时避免引入额外传播场。

Data: 数据来源于理论构建（simulation）中的$E_6\times E_6$框架和$SU(2)_R$连接机制，通过Plebanski/MacDowell--Mansouri机制生成涌现四元组（emergent tetrad）和爱因斯坦--希尔伯特作用量（Einstein--Hilbert action），并引入红外（IR）度量变形$ΔS_{\rm IR}[g]$实现MOND效应。

Method: 采用红外（IR）度规变形（metric deformation）$ΔS_{\rm IR}[g]$实现MOND（Modified Newtonian Dynamics），其紫外（UV）消失以恢复广义相对论（GR），而深MOND/静态极限由三维空间中的共形不变性（conformal invariance）和10参数群（同构于$SO(4,1)$）对称性原理固定。

Result: 该模型在低加速度区域精确恢复贝肯斯坦-米尔格罗姆（AQUAL）方程，同时在高加速度区域完全回归广义相对论（GR），且不引入右手规范场之外的额外传播场。
...
### [39] [Future Rip Scenarios in Fractional Holographic Dark Energy](https://arxiv.org/abs/2601.04414)
*Ayush Bidlan,Paulo Moniz,Oem Trivedi*

**TL;DR**: 研究在相互作用分数全息暗能量(FHDE)框架下发现，大撕裂(big rip)发生在莱维指数(α)1<α≤2范围内，小撕裂(little rip)和伪撕裂(pseudo-rip)分别需要特定红外截断形式和α>2条件，且在哈勃截断(Hubble cutoff)下非线性Q项中可实现小撕裂和伪撕裂。


<details>
  <summary>Details</summary>

Background: 研究探讨了在相互作用的分数全息暗能量(FHDE)框架下晚期宇宙奇点（如撕裂情景）的出现条件及其与莱维指数(α)的关系。

Data: 数据来源包括基于Granda-Oliveros (GO)截止尺度$L=(γH^{2}+δ\dot{H})^{-\frac{1}{2}}$的理论模拟数据，以及采用哈勃截止尺度$L=H^{-1}$时对哈勃参数$H(t)$的数值计算数据。

Method: 采用基于大尺度红外截断(Granda-Oliveros cutoff)和哈勃截断(Hubble cutoff)的分数全息暗能量(Fractional Holographic Dark Energy)模型，结合非线性相互作用项Q的数值模拟方法。

Result: 在分数阶全息暗能量（FHDE）框架下，使用Granda-Oliveros（GO）截止时，利维指数（Lévy's index）在1<α≤2范围内允许大撕裂（big rip）发生，而伪撕裂（pseudo-rip）要求α>2且在该截止下不可能出现。
...
### [40] [Towards a unified quantum field theory of dark energy and inflation: unstable de Sitter vacuum and running vacuum](https://arxiv.org/abs/2601.05218)
*Joan Solà Peracaula,Àlex González-Fuentes,Cristian Moreno-Pulido*

**TL;DR**: 这篇论文提出在运行真空模型（RVM）框架下，早期宇宙通过量子场论（QFT）效应驱动的动态真空能量密度（VED）产生暴胀（RVM-inflation），无需引入暴胀子（inflaton）场，并统一解释了暴胀和暗能量（作为动态真空能量）现象，其预测的当前宇宙可观测效应可能缓解宇宙学张力问题。


<details>
  <summary>Details</summary>

Background: 研究探讨了暴胀（inflation）作为解决标准宇宙学模型基本不一致性的必要机制，并提出了基于动态真空能量密度（VED）的运行真空模型（RVM）框架作为无需引入暴胀子（inflaton）的替代方案。

Data: 研究采用理论模拟（simulation）方法，基于量子场论（QFT）框架下的动态真空能量密度（VED）模型，通过引入哈勃参数（H）及其时间导数构建演化方程，模拟早期宇宙暴胀（RVM-inflation）和后期辐射主导时期的真空衰变过程。

Method: 采用运行真空模型(RVM)框架，将真空能量密度(VED)表达为哈勃率(H)及其时间导数的函数ρvac=ρvac(H, ̇H, ̈H,...)，通过量子场论(QFT)效应在早期宇宙产生H^4主导的暴胀机制。

Result: 在运行真空模型（RVM）框架下，早期宇宙中真空能量密度（VED）通过高阶项H^4驱动暴胀，无需引入暴胀子场（inflaton），并平滑过渡到辐射主导时期，最终在晚期宇宙形成缓慢演化的VED。
...
### [41] [Simulating the Formation of the Young "Fermi Bubbles" in the Circinus Galaxy](https://arxiv.org/abs/2601.04317)
*Shaokun Xie,Fulai Guo,Ruiyu Zhang,B. Mingo,Fangzheng Shi,Jiang-Tao Li*

**TL;DR**: 通过流体动力学模拟证实近邻Circinus星系中观测到的费米气泡（Fermi bubbles）由活动星系核（AGN）喷流驱动激波形成，支持AGN喷流激波模型是盘星系中延展气体气泡的普遍成因。


<details>
  <summary>Details</summary>

Background: 研究主题为银河系及近邻星系（如Circinus星系）中由活动星系核（AGN）喷流激波（jet-shock）驱动的千秒差距（kpc）尺度气泡（bubbles）的起源及其对星系演化的反馈作用。

Data: 使用流体动力学模拟（hydrodynamic simulations）研究邻近的Circinus星系中"费米气泡（Fermi bubbles）"的形成过程。

Method: 采用流体动力学模拟（hydrodynamic simulations）研究活动星系核（AGN）喷流驱动的激波在星系周介质中的传播过程，并通过合成X射线图像和光谱与观测数据对比验证模型。

Result: 活动星系核（AGN）喷流驱动的激波在环星系介质中演化约0.95 Myr后形成的泡状结构在尺寸和形态上与观测到的Circinus星系气泡相符，支持AGN喷流激波模型是Circinus气泡和费米气泡（Fermi bubbles）的共同起源。
...
### [42] [Can tidal disruption event models reliably measure black hole masses?](https://arxiv.org/abs/2601.04406)
...
**TL;DR**: 通过分析三个重复部分潮汐瓦解事件(rpTDEs)验证了现有TDE模型在估算超大质量黑洞质量($M_{\rm{BH}}$)时的可靠性，发现不同模型得出的$M_{\rm{BH}}$值自洽且与宿主星系代理值相符(误差0.3-0.5 dex)，但揭示了回落模型在恒星质量和撞击参数方面的局限性，并指出近紫外波段光变曲线覆盖对参数约束的关键作用。


<details>
  <summary>Details</summary>

Background: 潮汐瓦解事件（TDE）光变曲线被广泛用于推断静止超大质量黑洞（supermassive black hole）的质量，为独立于宿主星系标度关系的低质量黑洞人口统计提供了有力工具。

Data: 研究中使用的数据来源包括AT 2023adr、2020vdq和2022dbl三个重复部分潮汐瓦解事件(rpTDEs)的光谱观测数据，以及来自鲁宾天文台Legacy Survey of Space and Time模拟的预期光变曲线数据。

Method: 采用回落吸积拟合(fallback-accretion fits)、流-流碰撞标度关系(stream-stream collision scaling relations)、基于光度的经验关系(luminosity-based empirical relations)和冷却包层拟合(cooling-envelope fits)独立建模三个重复部分潮汐瓦解事件(rpTDEs)的耀发。

Result: 现有潮汐瓦解事件(TDE)模型在不同耀发间能自洽地得出超大质量黑洞质量($M_{\rm{BH}}$)，与宿主星系质量代理指标基本吻合，误差在0.3-0.5 dex范围内。
...
### [43] [The X-ray properties and structure of A3571 up to $R_{500}$](https://arxiv.org/abs/2601.04619)
...
**TL;DR**: 通过EP-FXT(爱因斯坦探针后续X射线望远镜)观测发现Abell 3571星系团虽形态松弛但存在南北两侧表面亮度异常，温度/压力/熵分布差异表明其正处于由偏心小质量合并触发的震荡后弛豫阶段(post-merger phase)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近邻X射线明亮星系团Abell 3571在夏普利超星系团（Shapley Supercluster）中的动力学演化状态，特别是通过多波段观测揭示其看似弛豫形态下潜在的残余并合活动特征。

Data: 数据来源于爱因斯坦探针后续X射线望远镜(EP-FXT)的宽视场($1^{\circ} \times 1^{\circ}$)观测数据。

Method: 采用爱因斯坦探针后续X射线望远镜(EP-FXT)的宽视场($1^{\circ} \times 1^{\circ}$)观测数据，通过表面亮度、温度、压力和熵的径向分布分析，检测到超出$R_{500}$半径的南北两侧表面亮度异常区域。

Result: Abell 3571尽管在X射线图像中呈现形态学松弛，但多波段观测揭示了与过去并合事件相关的残余动力学活动迹象。
...
### [44] [Self-consistent Dynamical and Chaotic Tides in the REBOUNDx framework](https://arxiv.org/abs/2601.04315)
*Donald J. Liveoak,Sarah C. Millholland,Michelle Vick,Daniel Tamayo*

**TL;DR**: 在REBOUNDx框架中实现了高偏心率轨道与动力学潮汐（dynamical tides）振模的耦合演化，为系外行星动力学提供了快速精确的N体模拟工具。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高偏心率轨道下潮汐力激发的动力学潮汐（dynamical tides）现象及其与轨道演化的耦合机制。

Data: 研究采用数值模拟（simulation）数据，基于\texttt{REBOUNDx}框架扩展的\texttt{REBOUND} $N$-体积分器实现轨道与振动模式耦合演化。

Method: 采用REBOUNDx框架（REBOUNDx framework）实现轨道与动力学潮汐（dynamical tides）振动模式的耦合演化分析。

Result: 在REBOUNDx框架中实现了高偏心率轨道与星体内部振动模式（dynamical tides）的耦合演化，为系外行星动力学提供了快速精确的N体模拟工具。
...
### [45] [Investigating the High-energy Radiation Environment of Planets in Sun-like Binary Systems](https://arxiv.org/abs/2601.04593)
...
**TL;DR**: 首次同时观测并构建了类太阳双星系统半人马座α AB的远紫外(FUV)光谱能量分布(SED)，结合光化学模型分析表明双星轨道运动导致的通量变化不会显著影响类地行星大气质量损失。


<details>
  <summary>Details</summary>

Background: 远紫外(FUV)辐射是行星大气光化学过程的关键驱动源，理解行星所处辐射环境对解释大气观测数据至关重要。

Data: 数据来源包括SISTINE火箭载光谱仪观测的α Centauri AB双星FUV光谱（980-1570 Å）、档案观测数据、模型光谱及新型恒星活动模型。

Method: 采用SISTINE火箭光谱仪观测数据、归档观测数据、模型光谱及新型恒星活动模型构建了5 Å--1 mm波段的α Centauri AB双星系统光谱能量分布(SED)，并利用VULCAN光化学动力学代码模拟极端通量条件下的行星大气化学丰度。

Result: result：观测和分析表明，恒星双星性导致的增强大气质量损失不太可能对在类太阳恒星周围寻找类地行星的未来系外行星探测任务（如宜居世界天文台）构成问题。
...
### [46] [Spectator Composes a Gravitational Canon: Spectator-field-triggered Phase Transition During Inflation and its Anisotropic Gravitational Wave Signals](https://arxiv.org/abs/2601.04307)
*Yunjia Bao,Keisuke Harigaya*

**TL;DR**: 提出了一种宇宙暴胀（inflation）期间由旁观场（spectator field）慢滚（slow-roll）动力学触发相变（phase transition）的通用框架，发现该过程产生的拓扑缺陷（topological defects）能形成各向异性引力波（GW）背景信号。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙暴胀（cosmic inflation）期间由旁观者场（spectator field）慢滚动力学触发的相变及其产生的各向异性引力波（gravitational-wave）信号。

Data: 数据来源于宇宙暴胀（cosmic inflation）期间由旁观者场（spectator field）慢滚动力学触发的相变过程，以及该相变形成的拓扑缺陷重新进入视界后产生的引力波（GW）信号。

Method: 采用慢滚（slow-roll）动力学分析观众场（spectator field）触发宇宙暴胀（cosmic inflation）期间的相变（phase transition），并研究拓扑缺陷（topological defects）和引力波（GW）信号的产生机制。

Result: 相变（phase transition）由旁观者场（spectator field）的慢滚（slow-roll）动力学触发，产生的拓扑缺陷（topological defects）在暴胀（inflation）后重新进入视界并产生各向异性引力波（GW）信号。
...
### [47] [Metastable cosmic strings are broken at the start](https://arxiv.org/abs/2601.04320)
*Lorenzo Tranchedone,Ethan Carragher,Edward Hardy,Natálie Koscelanská van IJcken*

**TL;DR**: 研究发现亚稳态宇宙弦（metastable cosmic strings）在早期因有限温度效应或与单极子（monopoles）结合而断裂，其残余弦段可长期存在，且网络破坏主要源于早期断裂而非晚期量子隧穿，同时修正了单极子质量与弦张力比值的下限（$m_M^2/μ\gtrsim 10^3$）并揭示量子隧穿断裂主要发生在高张力点。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙弦（cosmic strings）的早期断裂机制及其对网络演化的影响，包括温度效应和单极子（monopoles）附着作用。

Data: 研究中使用的数据来源于数值模拟（numerical simulation）结果。

Method: 采用有限温度效应和单极子(monopole)附着机制分析宇宙弦(cosmic strings)的早期断裂过程，并利用数值模拟研究量子隧穿(quantum tunnelling)诱导断裂的罕见高张力点机制。

Result: 宇宙弦（cosmic strings）在早期通过有限温度效应或附着于已有单极子（monopoles）而断裂，若不再重新渗透，其最终破坏主要由早期断裂而非晚期量子隧穿导致。
...
### [48] [Search for Ultralight Axion Dark Matter with a Levitated Ferromagnetic Torsional Oscillator](https://arxiv.org/abs/2601.04576)
*Chunlong Li,Yiwei Huang,Shien Yang,Yichong Ren,Yu Zhang,Peiran Yin,Pu Huang,Fei Xue*

**TL;DR**: 利用悬浮铁磁扭转振荡器（FMTO）探测超轻轴子（axion）暗物质与电子自旋的耦合，未在2e-14 eV附近观测到轴子诱导的赝磁场信号，并给出了轴子-电子耦合常数的90%置信水平上限（未校正g_aee < 1e-7，校正后g_aee < 6e-5）。


<details>
  <summary>Details</summary>

Background: 探索超轻轴子（axion）暗物质与电子自旋耦合的宏观量子效应。

Data: 数据来源于悬浮铁磁扭转振荡器（FMTO）在室温高真空条件下通过光学读出和三重磁屏蔽获得的18000秒可分析数据。

Method: 采用基于似然统计框架（likelihood-based statistical framework）的分析方法，结合轴子场振幅的随机涨落模型评估数据。

Result: 利用悬浮铁磁扭转振荡器（FMTO）在2e-14 eV附近未观测到轴子诱导赝磁场的显著信号，对轴子-电子耦合常数的90%置信水平限制分别为未校正（g_aee < 1e-7）和校正后（g_aee < 6e-5）。
...
### [49] [Braneworld Baryogenesis and QCD-Era Magnetogenesis: A Predictive Link](https://arxiv.org/abs/2601.04828)
*Michaël Sarrazin*

**TL;DR**: 研究发现原初磁场（PMF）在膜世界重子产生模型中起关键作用，其强度需达10^10 T才能解释观测到的重子-反重子不对称性，且磁场涨落导致大尺度上重子密度谱呈白噪声，与宇宙微波背景（CMB）等曲率扰动限制一致。


<details>
  <summary>Details</summary>

Background: 研究探讨了原初磁场（PMF）在膜世界重子生成（braneworld baryogenesis）机制中的关键作用及其与早期宇宙磁场的预测性关联。

Data: data：研究采用半解析框架整合了真实的随机原初磁场（PMF）数据，其强度在夸克-强子相变后约为10^10 T，并与因果性QCD-era磁生成机制一致。

Method: 采用半解析框架结合随机原初磁场（PMF）分析，研究了膜世界重子生成模型中磁场对重子-反重子不对称性的决定性影响。

Result: 原初磁场（PMF）强度在夸克-强子相变后需达到约10^10 T才能与观测到的重子-反重子不对称性匹配。
