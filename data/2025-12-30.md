...
**TL;DR**: 比较低红移(z<0.1)SDSS DR7中BPT选择的纯光学活动星系核(AGN)宿主与IllustrisTNG100和EAGLE Ref-L0100N1504模拟中颜色选择的"绿谷"(green-valley)中心星系，发现TNG模拟通过动力学反馈模式实现高效、近乎二元的恒星形成关闭，而EAGLE模拟的热反馈支持更缓慢的下降过程，与本地AGN宿主更一致。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于比较低红移（$z<0.1$）BPT（Baldwin-Phillips-Terlevich）选择的纯光学活动星系核（AGN）宿主星系与宇宙学模拟（IllustrisTNG100和EAGLE Ref-L0100N1504）中颜色选择的“绿谷（green-valley）”类似中心星系，探讨不同反馈机制对星系演化的影响。

Data: 数据来源包括SDSS DR7的光学活动星系核（AGN）宿主样本（观测数据）、MPA-JHU星表（用于获取恒星质量和孔径校正后的总恒星形成率）、以及数值模拟IllustrisTNG100和EAGLE Ref-L0100N1504中颜色选择的"绿谷"中心星系样本（模拟数据）。

Method: 采用BPT（Baldwin-Phillips-Terlevich）图选择低红移（z<0.1）纯光学活动星系核（AGN）宿主星系，并与IllustrisTNG100和EAGLE Ref-L0100N1504模拟中基于颜色选择的"绿谷（green-valley）"中心星系进行对比分析，通过定义内部（g-r）百分位数（75-95百分位）来减少跨数据集系统误差。

Result: 在相同质量下，TNG模拟的绿谷（green-valley）星系占比显著高于EAGLE（在$M_\star\sim10^{11}M_\odot$附近达到$\gtrsim60\%$，而EAGLE仅为20-40%）。
...
**TL;DR**: 研究发现双双射电星系（DDRG）中喷流间歇期与重启喷流寿命存在相关性，表明黑洞持续吸积过程中自旋归零导致喷流暂停，且喷流形态始终为FRII型源于吸积盘无倾斜。


<details>
  <summary>Details</summary>

Background: 双峰射电星系(DDRGs)的间歇性喷流活动与持续吸积过程相关，其喷流间歇期与重启喷流寿命的关联性揭示了黑洞自旋在喷流形成中的关键作用。

Data: 数据来源于对双射电星系（DDRGs）的射电波段观测，包括内、外喷流或瓣结构的形态学分析及黑洞自旋状态变化的模拟（simulation）数据。

Method: 采用连续吸积模型（continuous accretion scenario）和黑洞自旋演化机制（black hole spin evolution）解释双射电星系（DDRGs）的间歇性喷流和FRII形态。

Result: DDRGs（双双重射电星系）中喷流重启时间与静止期的相关性表明黑洞持续吸积但喷流缺失源于吸积过程中自旋降为零，而喷流形态保持FRII型则归因于吸积盘在自旋过零时无倾斜。
...
**TL;DR**: 通过多波段光谱分析发现活动星系核（AGN）中存在分层吸积盘风（stratified accretion disk wind），高电离线（如Civ λ1549）显示高速内风特征，低电离线（如Hβ、Mgii λ2800）反映低速致密外区辐射，中间电离线（如Aliii λ1860）呈现过渡特性，支持辐射驱动风模型的径向分层结构。


<details>
  <summary>Details</summary>

Background: 研究聚焦活动星系核(AGN)中分层结构吸积盘风(accretion disk wind)的观测特征及其辐射驱动机制。

Data: 数据来源为基于宽窄发射线的多波段光谱观测数据（multi-wavelength spectroscopic analysis of broad and narrow emission lines）。

Method: 基于多波段光谱分析（multi-wavelength spectroscopic analysis）和初步光致电离模型（photoionization modeling），采用辐射驱动风模型（radiatively driven wind models）研究活动星系核（AGN）中风的分层结构。

Result: 基于多波段光谱分析，观测证据支持活动星系核（AGN）中存在分层（stratified）吸积盘风，不同电离态发射线轮廓、运动学特征差异表明该外流具有径向分层结构。
...
**TL;DR**: 通过运动学和测光分析证实UGC 694与IC 412星系对（galaxy pair）的视近距是视线投影效应（line-of-sight projection effect），其123.5 Mpc的实际空间分离排除了引力相互作用（gravitational encounter）可能，并发现UGC 694的扰动形态（disturbed morphology）源于内部演化或小质量并合（minor mergers）。


<details>
  <summary>Details</summary>

Background: 研究主题为通过运动学(kinematical)和测光(photometric)方法重新评估星系对UGC 694与IC 412之间先前假设的相互作用关系，强调多波段验证对避免投影效应(projection effect)导致误判的重要性。

Data: 数据来源于SDSS DR16（Sloan Digital Sky Survey Data Release 16）的运动学数据和差分测光分析。

Method: 采用运动学（kinematical）和测光（photometric）相结合的方法，定量分析SDSS DR16运动学数据并运用差分测光技术研究结构不对称性和颜色分布。

Result: 通过SDSS DR16运动学数据分析发现UGC 694和IC 412存在约8372 km/s的径向速度差，证实两者约123.5 Mpc的实际空间分离，其视觉邻近性仅为视线投影效应。
...
**TL;DR**: 研究发现极低金属丰度（[Fe/H] < -3.0）的锂增丰（Li-rich）巨星极为罕见，在金属丰度 > -2.5的恒星中探测到水平支（HB）和渐近巨星支（AGB）阶段存在锂丰度高达6.15 dex的巨星，其红外超揭示质量损失与卡梅伦-福勒（Cameron-Fowler）机制导致的新7Li形成有关，并基于红外超确立了RGB、HB和AGB恒星锂丰度的三个阈值（1.5、0.5和-0.5 dex），同时发现金属贫乏巨星自转速度在40-90 km/s区间存在锂丰度平台（1.02-1.82 dex），且自转速度随金属丰度降低而增加，40 km/s可能成为关键临界值。


<details>
  <summary>Details</summary>

Background: 研究聚焦于极低金属丰度（[Fe/H] < -1.0）下锂富集（Li-rich）巨星恒星演化的未解之谜，特别是银河系晕和厚盘中锂增丰条件与恒星自转分布的关联。

Data: 数据来源于LAMOST巡天项目发布的两个近期大型星表，包含金属丰度（[Fe/H]）从-4.0到-1.0的贫锂（Li-poor）和富锂（Li-rich）低质量场巨星样本。

Method: 采用LAMOST巡天数据结合红外超（IR excesses）分析，通过卡梅伦-福勒机制（Cameron-Fowler mechanism）研究贫金属恒星中锂丰度阈值（RGB约1.5 dex、HB约0.5 dex、AGB约-0.5 dex）与自转速度（40-90 km/s）的关联性。

Result: 在金属丰度（metallicity）>-2.5的恒星中，观测到水平支（HB）和渐近巨星支（AGB）阶段的锂丰度高达6.15 dex的巨星，并检测到红外超（IR excesses）表明存在与卡梅隆-福勒机制（Cameron-Fowler mechanism）相关的近期锂增丰过程。
...
**TL;DR**: 基于CARMA的CO J=1-0数据，利用树状图方法在M31中识别出453个分子云（molecular clouds），发现其平均速度弥散（velocity dispersion）、半径和质量分别为2.8 km/s、22.1 pc和10^5.2 M⊙，且速度弥散与星系中心距离呈弱反相关，多数云处于引力束缚状态，其尺寸-速度弥散关系与银河系（Milky Way）一致但尺寸-质量关系斜率更平缓，并支持KS（Kennicutt-Schmidt）定律在秒差距尺度上受单个源演化状态影响的假说。


<details>
  <summary>Details</summary>

Background: 研究聚焦于M31（仙女座星系）中分子云（molecular clouds）的物理特性及其与恒星形成率（star formation rate）的关系。

Data: 数据来源于CARMA观测的M31中CO J=1-0谱线数据，通过树状图算法(dendrogram)提取了453个分子云。

Method: 采用树状图（dendrogram）方法从CARMA观测的CO J=1-0数据中提取分子云并分析其速度弥散、半径和质量关系。

Result: M31中453个分子云的平均速度弥散(velocity dispersion)、半径和质量分别为2.8 km s$^{-1}$、22.1 pc和10$^{5.2}$ M$_\odot$。
...
**TL;DR**: 通过恒星演化模拟发现，大质量恒星捕获的超重暗物质（DM）在未排除参数空间内可实现热化并达到捕获-湮灭平衡，非湮灭型暗物质甚至可能因自引力坍缩形成黑洞而提前终结恒星寿命，揭示了暗物质捕获效率对恒星演化阶段、成分及晕中心位置的依赖性。


<details>
  <summary>Details</summary>

Background: 研究探讨了重暗物质（DM）通过与大质量恒星成分的散射被捕获后对恒星演化的影响，特别关注了第一代恒星从主序前到晚期富金属阶段的暗物质捕获过程。

Data: 使用恒星演化模拟（stellar evolution simulations）追踪暗物质（DM）捕获过程，并采用爱丁顿反演方法（Eddington inversion method）获取暗物质速度分布。

Method: 采用恒星演化模拟（stellar evolution simulations）和埃丁顿反演法（Eddington inversion method）分析暗物质（DM）捕获过程，并引入多散射（multiple-scattering）模型描述金属核形成后的捕获增强效应。

Result: 在恒星演化过程中，超重暗物质(DM)的捕获效率因金属核形成而显著增强，需要三种不同核素准确描述多重散射捕获过程。
...
**TL;DR**: 通过ALMA CMZ巡测(ACES)分子谱线数据模拟银河系中心(CND)气体盘运动，发现中心20秒差距内引力势近似球形且质量密度呈尖峰分布(ρ_mass∝R^-1.9)，临界半径14秒差距内气体盘因潮汐作用抑制自引力坍缩，导致核周区恒星形成受抑并形成重质量主导的初始质量函数(IMF)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中心引力势和质量分布的探索，通过分析环绕核气体盘（CND）的形态和运动学特征揭示其动力学结构。

Data: 数据来源于ALMA CMZ探索巡天(ACES)的分子谱线数据，用于揭示银河系中心核周气体盘(CND)的形态和运动学特征。

Method: 采用测试粒子(test particle)运动模拟结合终端速度法(terminal velocity method)分析分子线数据，重构银河系中心引力势与质量分布。

Result: 银河中心引力势近似球形，质量密度分布呈尖峰型（cusp type），近似为ρmass ∼ 1.56×10^5(R/1 pc)^−1.9 M⊙ pc^−3。
...
**TL;DR**: 研究发现星系旋转曲线（rotation curves）外缘平坦现象可通过引入由重子势（baryonic potential）$Φ_b$产生的额外能量密度$ρ_s = μ_s Φ_b^2 / c^4$来精确拟合，无需暗物质（dark matter）假设，且参数$K$在91个SPARC星系样本中呈现窄范围分布，表明暗物质效应可能与重子引力势直接相关。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系旋转曲线（rotation curves）在远离中心区域时保持平坦的现象，提出了一种不依赖暗物质（dark matter）的新解释框架，即通过引入由重子物质（baryonic matter）引力势（gravitational potential）产生的额外能量密度（energy density）来修正泊松方程（Poisson equation）。

Data: 数据来源于SPARC数据库中的91个星系观测数据。

Method: 引入了一个经验定律，认为重子物质（baryonic matter）产生的引力势（gravitational potential）$Φ_b$会通过额外能量密度$ρ_s = μ_s Φ_b^2 / c^4$影响时空结构，并求解包含该修正项的泊松方程（Poisson equation）$\nabla^2Φ_{\rm tot} = 4πG\,ρ_b /c^2 + 4πG\,μ_s\,Φ_b^2 / c^6$来模拟星系旋转曲线（rotation curves）。

Result: 星系旋转曲线(Rotation curves)的平坦性可通过引入由重子势能(Baryonic potential)产生的额外能量密度(Energy density)来解释，无需依赖暗物质(Dark matter)假设。
...
**TL;DR**: 通过结合钱德拉源目录（CSC 2.1）的X射线数据、盖亚（Gaia）DR3的光学数据和CatWISE2020的中红外数据，并应用随机森林分类器，在银河系平面（|b|<20°）新发现了863个类星体候选体（其中514个高置信度），显著扩展了低流量银河系平面类星体（GPQ）样本，其X射线光谱比全天类星体样本更硬，符合银河系平面增强吸收的特征。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于解决低银纬（Galactic latitude）区域类星体（quasar）样本严重不完整的问题，通过多波段数据融合提升银盘后方类星体（Galactic Plane Quasars, GPQs）的探测能力。

Data: 数据来源包括钱德拉源表（Chandra Source Catalog, CSC 2.1）的X射线数据、盖亚（Gaia DR3）的光学数据、CatWISE2020的中红外数据，以及DESI、SDSS和LAMOST的光谱确认类星体和恒星数据。

Method: 采用随机森林分类器（Random Forest classifier）结合盖亚（Gaia）自行约束抑制恒星污染，并利用随机森林回归模型（Random Forest regression model）估计候选体的测光红移。

Result: 通过结合钱德拉源目录（CSC 2.1）的X射线数据、盖亚（Gaia）DR3的光学数据和CatWISE2020的中红外数据，并应用随机森林分类器与回归模型，在银河系平面（|b|<20°）区域新发现了863个类星体候选体（GPQ），其中514个为高置信度候选体，显著提升了低流量GPQ的探测灵敏度。
...
**TL;DR**: 利用JWST NIRISS（近红外成像仪和无缝光谱仪）光栅数据首次在红移z∼1.3和z∼2.0处测得Hα（氢阿尔法）光度函数(LF)的暗端斜率分别为-1.50^{+0.14}_{-0.08}和-1.60^{+0.17}_{-0.09}，探测极限达LHα∼10^{40.5} erg s^{-1}和10^{40.9} erg s^{-1}，对应恒星形成率0.4和1.0 M⊙ yr^{-1}。


<details>
  <summary>Details</summary>

Background: 利用JWST（James Webb Space Telescope）NIRISS（Near-Infrared Imager and Slitless Spectrograph）观测数据研究红移$z\sim1.3$和$z\sim2.0$处H$α$发射线光度函数（Luminosity Function）的演化特征。

Data: 数据来源于JWST（James Webb Space Telescope）NIRISS（Near-Infrared Imager and Slitless Spectrograph）棱镜数据，来自GLASS-JWST巡天，使用F115W、F150W和F200W滤光片光谱识别发射线，并借助引力透镜放大效应提升探测深度。

Method: 基于JWST NIRISS（近红外成像仪和无缝光谱仪）无缝光谱数据，采用有效体积和完备性分析方法构建Hα光度函数（LF）。

Result: 在红移$z\sim1.3$和$z\sim2.0$处测得H$α$光度函数（LF）的暗端斜率分别为$-1.50^{+0.14}_{-0.08}$和$-1.60^{+0.17}_{-0.09}$，并估算出相应的宇宙恒星形成率密度为$0.097^{+0.015}_{-0.016}~M_{\odot}~\rm yr^{-1}~Mpc^{-3}$和$0.129^{+0.025}_{-0.030}~M_{\odot}~\rm yr^{-1}~Mpc^{-3}$。
...
**TL;DR**: 在红移(redshift)2.39的星系合并系统J1244-LyC1中首次观测到空间分辨的多点位莱曼连续谱(Lyman continuum, LyC)泄漏，其绝对逃逸分数(escape fraction)达36%±4%，为研究大质量合并驱动宇宙再电离(cosmic reionization)提供了直接证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用哈勃太空望远镜(HST)的MAMMOTH-LyC巡天项目，探索高红移(z~2.2)大质量原星系团中莱曼连续谱(LyC)泄漏现象与星系合并过程的关联性，为理解宇宙再电离时期的主要物理机制提供新证据。

Data: 数据来源于哈勃太空望远镜（HST）第30周期的中等深度观测项目MAMMOTH-LyC survey，使用WFC3/UVIS F225W滤光片对两个红移约2.2的大质量原星系团进行了18轨深度成像。

Method: 采用哈勃空间望远镜(HST)的WFC3/UVIS F225W滤光片进行18轨道深度成像观测，结合莱曼连续谱(LyC)泄漏检测和空间分辨分析技术。

Result: 在z=2.39处发现了一个具有显著合并特征的强莱曼连续谱(LyC)泄漏源J1244-LyC1，其绝对逃逸分数为36%±4%（1σ），这是首个确认的高红移LyC泄漏合并系统，也是迄今已知质量最大的LyC发射体。
...
**TL;DR**: 基于盖亚(Gaia)数据发现银河系造父变星(Cepheids)的G波段消光系数(R_G)与红化存在显著负相关，揭示了固定R_G会导致高红化造父变星距离低估，并证实红外测距是目前最可靠方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于造父变星(Cepheids)作为宇宙距离阶梯的关键示距天体，探讨其在银河系结构、运动学和消光特性研究中的核心作用，尤其关注光学波段消光系数非线性效应和星际介质多样性对距离测量的影响。

Data: 数据来源为$Gaia$卫星提供的高精度测光（photometry）和视差（parallax）数据。

Method: 采用高精度测光（photometry）和盖亚（Gaia）视差数据，结合非线性效应分析和星际介质（ISM）引起的$R_V$变化建模，量化了$G$波段消光系数与红化的反相关关系。

Result: 发现银河系造父变星（Cepheids）的G波段消光系数（$R_G$）与红化（reddening）存在显著负相关关系，具体表现为$R_G = 1.921 \pm 0.060 - (0.107 \pm 0.022)\,E(G_{\mathrm{BP}} - G_{\mathrm{RP}})$。
...
**TL;DR**: 通过ALMA观测发现红外暗云G035.39-00.33中存在由超新星遗迹(SNR)和电离氢区(HII region)相互作用产生的大尺度激波压缩形成的弧状SiO结构，揭示了其丝状结构形成及恒星形成可能被触发的机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于红外暗云（IRDC）G035.39-00.33的形成机制及其与超新星遗迹（SNR）和电离氢区（HII region）相互作用的关联，探讨大尺度激波压缩在恒星形成初始条件中的作用。

Data: 数据来源于ALMA（阿塔卡马大型毫米波/亚毫米波阵列）对IRDC G035.39-00.33在3.5角秒分辨率（0.05秒差距）下观测的SiO、H13CO+、CH3OH和CS发射线数据，以及大尺度射电连续谱数据。

Method: 采用ALMA在3.5角秒分辨率下观测SiO、H13CO+、CH3OH和CS发射线，通过分析宽窄SiO发射特征追踪激波活动（shock activity）和分子外向流（outflows），并结合CS和CH3OH发射的空间分布揭示大尺度激波压缩（large-scale shock compression）对丝状结构（filamentary structure）的塑造作用。

Result: 在G035.39-00.33红外暗云(IRDC)中观测到的窄线宽拱形SiO发射结构表明，大尺度激波可能压缩了云周围气体并塑造了其丝状形态。
...
**TL;DR**: 通过甚大阵(VLA)在C和K波段观测，首次在四个大质量年轻恒星体(G23.389+00.185等)中探测到热喷流(thermal jets)，并发现6.7 GHz甲醇脉泽(methanol maser)环状结构可能源自这些喷流。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用厘米波段的脉泽（maser）辐射探测高质量原恒星（high-mass protostars）周围气体运动学特征及其环状结构起源。

Data: 数据来源于使用卡尔·G·扬斯基甚大天线阵（Karl G. Jansky Very Large Array）在A配置下对C波段和K波段的射电连续谱、6.7 GHz甲醇脉泽（methanol maser）及22 GHz水脉泽（water maser）辐射进行的成像观测。

Method: 采用卡尔·G·扬斯基甚大阵（Karl G. Jansky Very Large Array）的A配置在C波段和K波段对射电连续谱、6.7 GHz甲醇脉泽（methanol maser）及22 GHz水脉泽（water maser）辐射进行成像分析。

Result: 在六个高质量年轻恒星天体（high-mass young stellar objects）中首次观测到G23.389+00.185、G23.657-00.127、G28.817+00.365和G30.400-00.296的热喷流（thermal jets），并发现G23.207-00.377的K波段连续谱发射可能来自单个原恒星的喷流结或多颗致密年轻原恒星，而G31.047+00.356的射电连续谱发射与一个演化的H II区相关。
...
### [16] [Clustering Confuses Spectro-photometry: An Investigation of 2D and 3D Forced Profile Matching for Stacking Line-intensity Mapping Data on Source Catalogues](https://arxiv.org/abs/2512.22356)
*Ella M. Mansfield,Delaney A. Dunne,Dongwoo T. Chung*

**TL;DR**: 论文提出了两种针对线强度测绘（LIM）叠加分析方法的改进方案，通过在二维空间或三维空间-光谱轴上假设特定信号形状并使用更高精度的源坐标进行叠加，在模拟的CO测绘阵列项目（COMAP）探路者观测数据中实现了比原始方法高达25%的检测显著性提升，并发现最优拟合轮廓（Lorentzian光谱形状）大于COMAP波束宽度且依赖于红移空间聚类和指状效应等LIM信号特征。


<details>
  <summary>Details</summary>

Background: 线强度映射（LIM）是一种通过谱线发射在大尺度低分辨率下观测宇宙的新兴观测技术。

Data: 数据来源于模拟观测，包括一系列模拟CO Mapping Array Project (COMAP) Pathfinder观测的简单和复杂仿真数据。

Method: 采用基于二维空间或三维空间-光谱轴特定信号形态假设的拟合方法，结合比线强度映射（LIM）数据立方体分辨率更精确的源坐标叠加技术。

Result: 在现实COMAP模拟中，新拟合方法比原始叠加方法检测显著性提升达25%，且最优拟合轮廓（Lorentzian谱形）大于COMAP波束宽度（5'），其尺寸和形状微妙依赖于LIM信号特性（如红移空间成团性和指状效应）。
...
### [17] [A Generative Reconstruction of Low-$\ell$ CMB B-Mode Signal using Reverse Diffusion in Deep Learning](https://arxiv.org/abs/2512.22683)
*Anumanchi Agastya Sai Ram Likhit,Rajib Saha*

**TL;DR**: 提出基于方差爆炸随机微分方程（VE-SDEs）的分数扩散方法，通过训练固定张标比（tensor-to-scalar ratio）r=0.001的原初低阶B模角功率谱生成模型，有效实现含透镜效应、前景辐射和仪器噪声的CMB观测数据去噪与去透镜，为原初引力波探测提供新框架。


<details>
  <summary>Details</summary>

Background: 研究主题为利用基于分数扩散的方法从受污染的宇宙微波背景（CMB）观测中重建原始B模角功率谱，以探测暴胀引力波信号。

Data: 数据来源为模拟观测谱（simulated observational spectra），包含引力透镜效应、复杂偏振前景组合以及代表拟议ECHO任务特征的仪器噪声。

Method: 采用基于方差爆炸随机微分方程（VE-SDEs）的分数扩散方法，通过训练固定张量标量比（tensor-to-scalar ratio）r=0.001的原初低ℓ B模角功率谱随机实现来指导逆向SDE去噪和去透镜化。

Result: 基于分数扩散模型（score-based diffusion model）的逆向随机微分方程（reverse SDE）方法能有效从受污染观测中重建原初B模角功率谱（primordial B-mode angular power spectrum），通过固定张标比（tensor-to-scalar ratio）$r=0.001$下的统计先验实现去噪和去透镜（delensing）。
...
### [18] [Primordial black holes within Higgs hybrid metric-Palatini approach](https://arxiv.org/abs/2512.22924)
*Brahim Asfour,Farida Bargach,Yahya Ladghami,Ahmed Errahmani,Taoufik Ouali*

**TL;DR**: 采用希格斯混合度规-帕拉蒂尼模型（Higgs hybrid metric-Palatini model）研究辐射主导时期原初黑洞（PBHs）的形成，发现其丰度可作为暗物质候选者，具体占比取决于耦合常数和电子折叠数（e-folds number）取值。


<details>
  <summary>Details</summary>

Background: 研究主题为暴胀时期产生的大尺度原初标量涨落引发的显著密度扰动坍缩形成原初黑洞（PBHs），并探讨其作为暗物质候选者的丰度。

Data: 数据来源于对原初曲率功率谱（primordial curvature power spectrum）的分析，包括小尺度（对应大波数k）的显著增强特征，以及通过研究质量方差σ(M_PBH)和坍缩为PBHs的总能量密度质量分数β(M_PBH)来评估PBHs形成概率。

Method: 采用希格斯混合度规-帕拉蒂尼模型（Higgs hybrid metric-Palatini model）分析原初曲率功率谱（primordial curvature power spectrum）在小尺度上的增强行为，并通过研究质量方差σ(M_PBH)和能量密度塌缩质量分数β(M_PBH)来考察原初黑洞（PBHs）的形成概率。

Result: 在希格斯混合度规-帕拉蒂尼（Higgs hybrid metric-Palatini）模型中，原初黑洞（PBHs）可以解释当前暗物质全部或部分含量，具体取决于耦合常数和电子折叠数（e-folds number）的取值。
...
### [19] [A new path to constrain the expansion history of the Universe in future spectroscopic galaxy surveys](https://arxiv.org/abs/2512.22964)
...
**TL;DR**: 提出利用下一代光谱设施优化宇宙计时器（cosmic chronometers, CC）方法，将哈勃参数（Hubble parameter, H(z)）测量精度提升至可显著约束关键宇宙学参数并缓解当前宇宙膨胀历史观测矛盾的水平。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用宇宙计时器（cosmic chronometers, CC）方法在哈勃常数（H0）早期与晚期测量值间存在张力及暗物质、暗能量本质未明的背景下，为宇宙膨胀历史提供独立于模型的探测途径。

Data: 数据来源于下一代光谱观测设施（next-generation spectroscopic facility）的模拟（simulation）分析。

Method: 采用宇宙计时器（cosmic chronometers, CC）方法直接测量哈勃参数H(z)，不依赖任何宇宙学模型假设或积分距离测量。

Result: 宇宙计时器（CC）方法能够在不依赖任何宇宙学模型假设或积分距离测量的情况下直接测量哈勃参数H(z)。
...
### [20] [Three channel dissipative warm Higgs inflation with global inference via genetic algorithms](https://arxiv.org/abs/2512.23423)
*Wei Cheng*

**TL;DR**: 构建了温希格斯暴胀（Warm Higgs Inflation）的三通道耗散框架，通过混合先验（mixing prior）和熵先验（entropy prior）克服单通道主导问题，发现多通道共存解在观测约束下仍存在，表明暴胀历史中耗散的微观起源不必唯一。


<details>
  <summary>Details</summary>

Background: 研究聚焦于构建和分析温希格斯暴胀（Warm Higgs Inflation）的三通道耗散框架，探索总耗散系数Υ(h,T)在低温、高温和阈值激活贡献下的分解及其对暴胀动力学的多通道共存影响。

Data: 数据来源于14维唯象模型（phenomenological model）的全局数值解和统计推断，采用遗传算法（genetic algorithm）进行参数扫描，并通过两类结构先验（structural priors）克服高维参数空间中的单通道主导简并性。

Method: 采用遗传算法（genetic algorithm）进行全局数值求解和背景场动力学统计推断，并引入混合先验（mixing prior）和熵先验（entropy prior）两类结构先验克服高维参数扫描中的单通道主导简并性。

Result: 构建了一个三通道耗散框架用于温希格斯暴胀（Warm Higgs Inflation），将总耗散系数分解为低温、高温和阈值激活贡献。
...
### [21] [Space AI: Leveraging Artificial Intelligence for Space to Improve Life on Earth](https://arxiv.org/abs/2512.22399)
*Ziyang Wang*

**TL;DR**: 论文提出空间人工智能（Space AI）作为人工智能与空间科技交叉的统一学科框架，将其划分为地球应用（任务规划、航天器优化等）、轨道应用（卫星自主、空间机器人等）、深空应用（自主导航、资源测绘等）和多行星生存（原位资源利用、生态管理等）四大任务场景，系统阐述了该领域如何推动太空探索并反哺地球技术发展。


<details>
  <summary>Details</summary>

Background: 研究聚焦人工智能（AI）与空间科技交叉形成的统一学科领域——空间人工智能（Space AI），旨在推动极端不确定性和有限人工监督下的太空自主化与弹性化发展。

Data: 研究中使用的数据来源包括地面观测数据、航天器在轨采集数据、深空探测器数据以及多行星任务模拟数据。

Method: 提出系统化框架将空间人工智能（Space AI）划分为四大任务场景（地球应用、轨道应用、深空应用和多行星生存应用），整合历史发展与当代进展。

Result: 该论文提出将空间人工智能(Space AI)构建为人工智能与空间科学技术交叉的统一学科领域，并系统化地将其划分为地球端、轨道端、深空端和多行星生活四大任务场景框架。
...
### [22] [Starkindler: An Uncertainty Aware Objective for Photometric Redshift Estimation](https://arxiv.org/abs/2512.22566)
*Raahul Singh,Ashutosh Pandey*

**TL;DR**: 提出Starkindler训练目标，通过将观测误差（observational errors）显式纳入目标函数来建模偶然不确定性（aleatoric uncertainty），在SDSS数据上训练的卷积神经网络（CNN）相比传统方法提高了测光红移（Photometric Redshift）的准确性、校准性并降低异常预测率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过改进机器学习方法处理天文观测数据中的偶然不确定性（aleatoric uncertainty）以提高测光红移（Photometric Redshift）估计的准确性和可解释性。

Data: 数据来源于斯隆数字巡天（Sloan Digital Sky Survey, SDSS）的观测数据。

Method: 引入了一种名为Starkindler的新型训练目标，通过将观测误差显式纳入模型目标函数来直接处理偶然不确定性（aleatoric uncertainty）。

Result: Starkindler通过将观测误差显式纳入目标函数直接处理偶然不确定性(alearoric uncertainty)，相比传统仅关注认知不确定性(epistemic uncertainty)的概率模型，在SDSS数据上实现了更高的测光红移(photometric redshift)预测精度、校准性和异常率降低。
...
### [23] [Dark matter direct detection: status, results and future plans](https://arxiv.org/abs/2512.23039)
*Laura Baudis*

**TL;DR**: 这篇论文综述了直接探测（direct detection）暗物质（dark matter）的实验原理、预期信号与背景噪声、主要技术手段，并评估了当前探测keV-TeV质量范围新粒子的实验进展及未来探测器的灵敏度。


<details>
  <summary>Details</summary>

Background: 研究主题为直接探测（direct detection）银河系暗物质（dark matter）粒子及其在keV至TeV质量范围内的实验探索。

Data: 数据来源包括地下低本底探测器（low-background detectors）对银河系暗物质粒子（dark matter particles）的观测信号和本底噪声。

Method: 采用低本底地下探测器探测银河系暗物质（dark matter）粒子诱发稀有信号的核心实验技术。

Result: 暗物质直接探测实验通过地下低本底探测器搜寻银河系假设暗物质粒子引发的稀有信号。
...
### [24] [Two birds with one stone: simultaneous realization of both Lunar Coordinate Time and lunar geoid time by a single orbital clock](https://arxiv.org/abs/2512.23098)
*Tian-Ning Yang,Ren-Fang Geng,Jing Zhang,Chong Yang,Yong Huang,Yi Xie*

**TL;DR**: 提出了一种“时间对齐轨道”（time aligned orbit），其理想时钟读数可等于月球大地水准面（selenoid）原时（O2）并通过已知线性变换转换为月球坐标时（O1），从而在单时钟部署下同时实现两种时间定义方案。


<details>
  <summary>Details</summary>

Background: 月球参考时间定义方案中，两种候选方案（O1基于月球坐标时、O2基于月球大地水准面（selenoid）原时）各自存在实现难度或坐标系重构问题。

Data: 数据来源于数值模拟（numerical simulations），用于评估时钟在更真实月球环境中实现月球大地水准面（selenoid）固有时间（O2）的程度。

Method: 采用数值模拟方法评估时间对齐轨道（time aligned orbit）上时钟在更真实月球环境中实现月面大地水准面固有时间（selenoid proper time）的可行性。

Result: 提出了一种“时间对齐轨道”（time aligned orbit），其理想时钟读数可等于月面大地水准面（selenoid）固有时间（O2），并通过已知线性变换转换为月球坐标时（O1）。
...
### [25] [Why Machine Learning Models Systematically Underestimate Extreme Values II: How to Fix It with LatentNN](https://arxiv.org/abs/2512.23138)
*Yuan-Sen Ting*

**TL;DR**: 该论文提出了LatentNN方法，通过联合优化神经网络参数和潜在输入值（latent input values）来校正天文数据驱动模型中因测量误差导致的衰减偏差（attenuation bias），在低信噪比（signal-to-noise ratio）条件下显著提升了神经网络推断的准确性。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于天文数据驱动模型中因输入变量测量误差导致的回归系数系统性低估问题（attenuation bias）。

Data: 研究中使用的数据来源包括一维回归数据、具有相关特征的多元输入数据以及恒星光谱数据。

Method: 采用联合优化网络参数和潜在输入值的LatentNN方法，通过最大化输入输出联合似然来校正神经网络中的衰减偏差（attenuation bias）。

Result: 神经网络（neural networks）存在衰减偏差（attenuation bias），而潜在变量（latent variable）方法可直接推广至神经网络以纠正该偏差。
...
### [26] [Lossless compression of simulated radio interferometric visibilities](https://arxiv.org/abs/2512.23490)
*A. R. Offringa,R. J. van Weeren*

**TL;DR**: 提出了一种名为Sisco的无损压缩方法，可将无噪声前向预测模型数据平均压缩至原始体积的24%，并支持与基线相关平均结合进一步减小数据量。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于射电干涉测量数据处理中无噪声前向预测模型数据（forward-predicted model data）的高效无损压缩问题。

Data: data来源包括LOFAR、MeerKAT和MWA的多样化数据集。

Method: 采用多项式外推法（polynomial extrapolation）在时间和频率维度预测值，将复数浮点可见度值分解并分组字节进行高效编码，使用Deflate算法压缩残差。

Result: 提出的Sisco无损压缩方法将无噪声前向预测模型数据平均压缩至原始体积的24%，最佳压缩效果通过线性和二次预测组合实现。
...
### [27] [Sidelobe Modification for an Offset Gregorian Reflector System using a Reconfigurable Intelligent Surface-Equipped Subreflector](https://arxiv.org/abs/2512.23530)
*S. W. Ellingson,A. J. Yip*

**TL;DR**: 该论文通过在偏置格里高利（offset Gregorian）反射面系统的副反射镜边缘安装可重构智能表面（RIS），实现了在第二旁瓣峰值处产生深度零点，同时主瓣方向性仅降低0.2分贝，且所需RIS面积更小、可直接替换现有系统的副反射镜。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用可重构智能表面(RIS)抑制偏置格里高利反射器(offset Gregorian reflector)系统中的近旁瓣干扰，以提升射电天文观测的抗干扰能力。

Data: 使用安装在副反射镜边缘的可重构智能表面（RIS）替代副反射镜外三分之一区域，并采用具有1位相位控制的被动元件。

Method: 采用可重构智能表面（RIS）替换副反射镜外三分之一区域，通过无约束优化算法设置RIS单元的1比特相位控制状态。

Result: 在偏移格里高利(offset Gregorian)反射系统中，副反射镜边缘安装可重构智能表面(RIS)能有效在静态方向图第二旁瓣峰值处形成深度零点。
...
### [28] [Galaxy Zoo Evo: 1 million human-annotated images of galaxies](https://arxiv.org/abs/2512.23691)
...
**TL;DR**: 这篇论文的核心贡献是发布了Galaxy Zoo Evo数据集，包含823k张星系图像的104M众包精细标注（如螺旋臂数量、缠绕程度等）和167k个针对天文学家特定需求（如强引力透镜识别）的下游任务标注，旨在为天文领域基础模型（foundation models）的构建和评估提供基准。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于构建和评估天文图像基础模型（foundation models）所需的标注数据集，以支持天文学领域计算机视觉技术的发展和应用。

Data: 数据来源于四个望远镜的823k张图像，包含104M个众包标签和四个小型标签集（总计167k个星系），用于特定天文下游任务。

Method: 采用众包标注（crowdsourcing labels）构建细粒度问答标签数据集，并用于预训练或微调基础模型（foundation models）。

Result: Galaxy Zoo Evo（GZ Evo）是一个包含823k星系图像和104M众包标签的数据集，旨在为构建和评估天文学基础模型提供支持。
...
### [29] [A Linearized Approach to Radial-Velocity Extraction. II: Shot-Noise-Limited Precision via Spectral Factorization](https://arxiv.org/abs/2512.23040)
*Sahar Shahaf,Barak Zackay*

**TL;DR**: 该论文提出了一种针对未知光谱成分的短时傅里叶变换(STFT)径向速度提取方法，通过将光谱时间序列分解为主成分谱和时变核函数，在傅里叶空间利用奇异值分解(SVD)和相位差推断速度变化，验证表明该方法在HD 34411和τ Ceti观测中达到了约30 cm/s的仪器精度极限，并能检测半振幅低至约50 cm/s的信号，为太阳型恒星周围类地行星探测提供了新途径。


<details>
  <summary>Details</summary>

Background: 该研究致力于在未知光谱成分条件下开发一种广义化的短时傅里叶变换（STFT）方法，以解决恒星光谱变异性对极端精密视向速度（RV）测量的干扰，推动类地行星探测技术的发展。

Data: 数据来源包括合成数据、SOAP（Spectrum-Orbit Alignment Program）模拟数据以及EXPRES（Extreme Precision Spectrometer）对HD 34411和τ Ceti的观测数据。

Method: 采用短时傅里叶变换（STFT）形式体系，通过奇异值分解（SVD）将光谱时间序列分解为主光谱和时变核，并在傅里叶空间利用相位差推断视向速度（radial velocity）变化。

Result: 提出了一种基于短时傅里叶变换（STFT）的径向速度提取方法，通过光谱时间序列分解为基光谱和时变核函数，在未知光谱成分情况下实现高精度测量。
...
### [30] [Predicting the detection yields of giant planets and brown dwarfs with CSST astrometry](https://arxiv.org/abs/2512.23378)
*Yifan Xuan,Fabo Feng,Zhensen Fu,Shilong Liao,Zhaoxiang Qi,Yang Chen*

**TL;DR**: 中国空间站望远镜(CSST)通过高精度天体测量预计在10年任务期内可探测到20-170颗M型矮星和褐矮星周围的巨行星及低质量褐矮星伴星，以及300-570对600秒差距内的褐矮星双星系统，显著扩充低质量恒星周围亚恒星伴星样本。


<details>
  <summary>Details</summary>

Background: 研究主题为利用中国空间站望远镜(CSST)的高精度天体测量能力探测M型矮星和褐矮星周围的巨行星与褐矮星伴星，以增进对低质量恒星和褐矮星周围行星形成与演化的理解。

Data: 数据来源为CSST巡天相机（Survey Camera）对中高银纬和中高黄纬天区进行光学巡天观测所获得的高精度天体测量数据，以及通过不同发生率生成的FGK矮星、M矮星和褐矮星（brown dwarfs）的合成样本和伴星注入-回收模拟数据。

Method: 采用伴星注入-恢复模拟(injection-recovery simulations)结合不同恒星类型(FGK矮星、M矮星和褐矮星)的伴星发生率(occurrence rates)来预测CSST可探测的巨行星和褐矮星数量。

Result: CSST（中国空间站望远镜）的10年任务期间预计可探测到20-170颗M型矮星周围300秒差距内的巨行星和低质量褐矮星（brown dwarfs），以及600秒差距内300-570对褐矮星双星系统。
...
### [31] [Water Production of Interstellar Comet 3I/ATLAS from SOHO/SWAN Observations after Perihelion](https://arxiv.org/abs/2512.22354)
*M. R. Combi,T. Mâkinen,J. -L. Bertaux,E. Quemerais,S. Ferron,R. Lallement,W. Schmidt*

**TL;DR**: 利用SOHO卫星的SWAN全天空氢莱曼-α相机观测星际彗星3I/ATLAS（C/2025 N1），首次在近日点后9天（1.40天文单位处）测得水生成率为3.17×10^29 s^-1，并发现该值在40天后降至1-2×10^28 s^-1。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用SOHO卫星的太阳风各向异性(SWAN)全天氢莱曼α相机观测星际彗星3I/ATLAS的氢彗发，并基于莱曼α荧光模型推算其水生成率动态变化。

Data: 数据来源于SOHO卫星搭载的SWAN全天氢莱曼-α（Lyman-alpha）相机对星际彗星3I/ATLAS的观测图像，以及LASP数据库中经太阳自转校正的每日太阳莱曼-α通量数据。

Method: 基于SOHO卫星搭载的太阳风各向异性(SWAN)全天氢莱曼α相机观测数据，采用Mäkinen & Combi (2005)的水生成率计算方法和经太阳自转校正的LASP数据库日射莱曼α通量荧光率计算技术。

Result: 在11月6日（近日点后9天）观测到星际彗星3I/ATLAS（C/2025 N1）的水分子产生率（water production rate）为3.17×10^29 s^-1，随后逐渐下降至近日点后40天（12月8日）的1-2×10^28 s^-1。
...
### [32] [Interstellar Interloper 3I/ATLAS: Nucleus Size, Photometry in RGB, Af(rho) and Antitail Structure Analysis](https://arxiv.org/abs/2512.22365)
*Toni Scarmato*

**TL;DR**: 首次结合高分辨率成像与非引力动力学约束星际彗星3I/ATLAS（C/2025 N1）的核半径，并通过地面RGB成像证实其太阳方向存在异常紧密准直的尘埃反尾（anti-tail）结构。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际彗星3I/ATLAS（C/2025 N1）的异常尘埃反尾（anti-tail）特征及其核尺寸精确测定。

Data: 数据来源包括高分辨率成像、非引力动力学分析以及托尼·斯卡马托天文台（MPC L92）0.25米望远镜获取的新地面RGB反尾（anti-tail）尘埃图像。

Method: 采用高分辨率成像与非引力动力学约束核半径，并结合地基RGB成像分析尘埃反尾（anti-tail）的统一分析方法。

Result: 核心结论是结合现有核半径约束和新地面RGB成像数据，对星际彗星3I/ATLAS（C/2025 N1）的逆向尘埃尾（anti-tail）进行了统一分析。
...
### [33] [Search for Annual Modulation in Combined ANAIS 112 and COSINE 100 Data using Bayesian Model Comparison](https://arxiv.org/abs/2512.23191)
*Om Godse,Shantanu Desai*

**TL;DR**: 基于COSINE-100和ANAIS-112三年联合数据，采用贝叶斯模型比较发现1-6 keV和2-6 keV能区内暗物质(dark matter)相互作用余弦信号不存在显著证据（贝叶斯因子自然对数小于1.15）。


<details>
  <summary>Details</summary>

Background: 研究背景为利用贝叶斯模型比较检验暗物质(dark matter)相互作用产生的余弦信号是否存在。

Data: 数据来源为COSINE-100和ANAIS-112实验约三年的联合观测数据。

Method: 采用贝叶斯模型比较检验（Bayesian model comparison test）分析正弦调制模型与常数值模型，并针对余弦信号的角频率和相位分别使用均匀先验和基于DAMA最佳拟合值的正态先验。

Result: 在1-6 keV和2-6 keV能区联合分析中，贝叶斯因子自然对数小于1.15表明ANIAS-112/COSINE-100数据不支持暗物质(Dark Matter)相互作用产生的余弦信号。
...
### [34] [On Systematic Errors in Cross-Identification of Binary Stars in Gaia](https://arxiv.org/abs/2512.23451)
*E. Malik,P. Kaigorodov,D. Kovaleva,O. Malkov*

**TL;DR**: 基于盖亚(Gaia) DR3数据创建了包含260多万对双星的星表，将已知特征的双星样本规模扩大了一个数量级以上，并分析了交叉识别结果的系统误差特征及其改进方法。


<details>
  <summary>Details</summary>

Background: 基于盖亚(Gaia) DR3数据大幅扩展双星系统样本规模，建立包含260万对双星的星表，显著提升已知特征双星系统的统计基础。

Data: 数据来源为盖亚(Gaia)第三次数据释放(DR3)和预盖亚时期最完整的双星编目出版物ILB。

Method: 采用坐标交叉匹配(cross-identification by coordinates)方法对盖亚(Gaia) DR3数据与ILB星表进行系统比对，并分析识别结果的特性依赖关系。

Result: 基于盖亚(Gaia)DR3数据构建的双星目录将已知特征的双星系统数量从约14万对大幅提升至超过260万对。
...
### [35] [Cosmological long-wavelength solutions in non-adiabatic multi-fluid systems](https://arxiv.org/abs/2512.22531)
*Hayami Iizuka,Tomohiro Harada*

**TL;DR**: 基于ADM（Arnowitt-Deser-Misner）形式体系和空间梯度展开，在多流体系统中构建了超视界尺度非线性宇宙学扰动解，揭示了非线性主导阶下同时存在绝热模和熵模（entropy modes），并分析了不同纯熵初始条件下测地切片中曲率扰动和密度扰动的演化规律。


<details>
  <summary>Details</summary>

Background: 研究主题为基于梯度展开方法构建多流体（multi-fluid）系统中超视界（superhorizon）尺度非线性宇宙学扰动的统一理论框架。

Data: 数据来源于基于空间梯度展开（spatial gradient expansion）的数值模拟（simulation），其中小展开参数定义为共动波数与哈勃尺度之比。

Method: 采用基于阿诺维特-德塞-米斯纳(Arnowitt-Deser-Misner)形式的空间梯度展开方法，结合以共动波数与哈勃尺度之比为小参数的非线性扰动理论框架。

Result: 基于ADM（Arnowitt-Deser-Misner）形式结合空间梯度展开，构建了多流体系统中超视界尺度非线性宇宙学扰动的解析解。
...
### [36] [Gravitational Noether-Ward identities for scalar field](https://arxiv.org/abs/2512.22958)
*Tomislav Prokopec*

**TL;DR**: 研究在一般弯曲背景下爱因斯坦引力与有质量、非最小耦合量子标量场协变耦合时，发现引力扰动运动方程中各项均满足独立的诺特-沃德恒等式（Noether-Ward identities），且尽管各项非横截，整体方程保持横截性，同时推导了重正化引力子自能所需各抵消项的显式形式，并证明不同度规扰动定义下均存在对应恒等式。


<details>
  <summary>Details</summary>

Background: 研究探讨了量子物质背景下一般度规扰动演化的引力诺特-沃德恒等式（Noether-Ward identities），重点关注爱因斯坦引力与弯曲背景中非最小耦合的量子标量场的协变耦合。

Data: 研究中使用的数据来源于量子物质背景下广义度规扰动的理论模拟（simulation）分析。

Method: 采用爱因斯坦引力与量子标量场协变耦合模型，通过诺特-沃德恒等式（Noether-Ward identities）分析广义弯曲背景下引力扰动的演化规律，并推导了不同度规扰动定义下的恒等式形式。

Result: 引力扰动方程中的每一项均满足各自的诺特-沃德恒等式（Noether-Ward identity），尽管各项本身不具备横性（non-transverse），但整体方程仍保持横性。
...
### [37] [Matter environments around black holes: geodesics, light rings and ultracompact configurations](https://arxiv.org/abs/2512.22267)
*Dylan S. Fonseca,Caio F. B. Macedo,Mateus Malato Corrêa,Diego Rubiera-Garcia*

**TL;DR**: 研究了球对称暗物质分布对黑洞几何结构、测地线特性和引力波铃荡现象的影响，发现低致密性环境下暗物质会使最内稳定圆轨道（ISCO）向内移动而主光环向外偏移，高致密性下则可能产生额外光环和次级视界，并在铃荡信号中留下长寿命俘获模和回声状调制特征。


<details>
  <summary>Details</summary>

Background: 研究探讨了球对称暗物质（dark-matter）分布对黑洞几何结构、测地线动力学及铃荡（ringdown）现象的影响。

Data: data部分：研究采用爱因斯坦星团（Einstein clusters）模拟三种常见暗物质密度分布（Hernquist、Navarro-Frenk-White (NFW)和Jaffe模型）构建自洽时空，并通过标量扰动时域演化（time-domain evolutions）获取环形振荡信号数据。

Method: 采用爱因斯坦星团（Einstein clusters）构建自洽时空模型，结合Hernquist、NFW和Jaffe三种密度分布，分析暗物质对黑洞几何结构、测地线及环降现象的影响。

Result: 暗物质分布（dark-matter distributions）通过爱因斯坦星团（Einstein clusters）建模，揭示了低致密性（low-compactness）下内移最内稳定圆轨道（ISCO）和外移主光环（principal light ring）的解析关系，并发现高致密性（higher compactness）时会出现额外光环、边际稳定轨道及次级视界（secondary horizons）。
...
### [38] [The new generation lunar gravitational wave detectors: sky map resolution and joint analysis](https://arxiv.org/abs/2512.23556)
*Xiaolin Zhang,Chengye Yu,Haoran Li,Sobhan Kazempour,Mingqiu Li,Sichun Sun*

**TL;DR**: 研究发现，在0.1-10 Hz频段内，月坑边缘的引力波干涉仪CIGO（Crater Interferometry Gravitational-wave Observatory）在0.1 Hz以上定位精度优于天琴（TianQin）和LISA，且在0.1-2.87 Hz抑制月球噪声后主导联合探测器网络性能；升级版四面体构型TCIGO（Tetrahedron configuration）通过增加第四站点形成规则四面体星座，定位能力提升五倍且天空覆盖更优。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用月球基地的引力波干涉测量技术（lunar-based gravitational-wave interferometry）填补空间与地面探测器之间的观测空白。

Data: 采用费希尔矩阵(Fisher-matrix)方法分析月球北极附近环形山边缘的Crater Interferometry Gravitational-wave Observatory (CIGO)与天琴(TianQin)、LISA对0.1-10 Hz频段单色源的角分辨率性能。

Method: 采用费希尔矩阵法(Fisher-matrix method)分析月球环形山引力波天文台(CIGO)及其升级版四面体构型(TCIGO)在0.1-10赫兹频段对单色源的角分辨率性能。

Result: 在0.1-10 Hz频段内，月坑干涉引力波天文台(CIGO)在0.1 Hz以上频率的定位精度优于天琴(TianQin)和LISA，且在0.1-2.87 Hz范围内实现月球噪声抑制时主导联合探测器网络的性能；升级版四面体构型(TCIGO)相比CIGO将角分辨率能力提升五倍并在目标频段获得更好的天空覆盖。
...
### [39] [Double to tenfold M-shell photoionization of singly charged lanthanum ions](https://arxiv.org/abs/2512.22574)
...
**TL;DR**: 采用光子-离子并束技术测量了La$^+$离子在820-1400 eV能区的单光子多电离截面（从双电离到十重电离），发现理论计算与实验结果基本吻合但电荷态分布略窄，为千新星等非平衡等离子体建模提供了关键原子数据基准。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用光子-离子合并束技术测量La$^{+}$离子在820--1400 eV能区的多重光电离截面，为发展量子理论方法提供实验基准数据，以支持非平衡等离子体（如千新星）建模。

Data: 数据来源于在PETRA III同步辐射光源上使用光子-离子合并束技术（photon-ion merged-beams technique）测量的La$^{+}$离子在820-1400 eV能量范围内的单光子多重电离截面（cross sections for multiple photoionization）。

Method: 采用光子-离子合并束技术（photon-ion merged-beams technique）结合升级版Jena原子计算器（JAC）进行大规模量子计算，重点分析了La$^+$的光吸收截面和3d空穴形成后的退激发级联过程。

Result: result：实验测得La$^{+}$离子在820-1400 eV能量范围内的单光子多重电离（double至tenfold）截面数据，与升级后的JAC（Jena Atomic Calculator）理论计算结果基本吻合，但理论预测的产物离子电荷态分布比实验观测略窄。
...
### [40] [Multi-Messenger Predictions for T Coronae Borealis: Probing Particle Acceleration in Novae](https://arxiv.org/abs/2512.22338)
*Prantik Sarmah,Sovan Chakraborty,Xilu Wang*

**TL;DR**: 论文首次基于模型估计了再发新星（recurrent nova）T CrB在即将爆发时由强子（hadronic）过程产生的次级辐射通量，并评估了其在伽马射线（LHAASO等）和中微子（IceCube等）望远镜中的可探测性，发现磁重联（magnetic reconnection）机制可能产生早于激波（external shock）数小时的中微子信号且更易被探测。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索再发新星（recurrent nova）RS Ophiuchi和T Coronae Borealis在爆发期间产生高能伽马射线和中微子的机制，以区分强子（hadronic）和轻子（leptonic）过程在粒子加速中的作用。

Data: 数据来源包括伽马射线望远镜（LHAASO、Fermi-LAT、MAGIC、H.E.S.S.、MACE和HERD）和中微子望远镜（IceCube和KM3NeT）。

Method: 采用两种质子加速机制：外部激波（ES）驱动机制（作用于新星喷出物与红巨星风的相互作用区域，尺度为10^13厘米）和磁重联（MR）机制（发生于白矮星表面附近，尺度为10^9厘米）。

Result: MAGIC探测到复发性新星（recurrent nova）RS Ophiuchi在2021年爆发期间产生的近TeV伽马射线，证实其是TeV级粒子加速器。
...
### [41] [Schwinger-Keldysh Cosmological Cutting Rules](https://arxiv.org/abs/2512.22652)
*Francisco Colipí-Marchant,Gabriel Marin,Gonzalo A. Palma,Francisco Rojas*

**TL;DR**: 该论文通过施温格-克登（Schwinger-Keldysh）路径积分形式，首次在宇宙学可观测量层面建立了与平坦空间散射振幅相似但需引入特殊图组合的切割规则（cutting rules），并在包含各类相互作用的树图和圈图关联函数中验证了其有效性。


<details>
  <summary>Details</summary>

Background: 研究探讨了在施温格-凯尔迪什（Schwinger-Keldysh）路径积分形式下计算的原初宇宙学关联函数中基于幺正性（unitarity）的切割规则的实现。

Data: 数据来源于利用Schwinger-Keldysh路径积分形式计算的原始宇宙学关联函数（cosmological correlators）的树图和圈图修正。

Method: 采用基于Schwinger-Keldysh路径积分形式主义的单位性切割规则（unitarity-based cutting rules）对宇宙学关联函数进行图表示分析。

Result: 在施温格-克迪什（Schwinger-Keldysh）路径积分框架下，宇宙学原初关联函数的切割规则（cutting rules）实现需引入特定组合的费曼图，这些图本身不参与可观测量计算，但能将关联函数的不连续性表达为低阶关联函数的乘积。
...
### [42] [Gravitational waves from seesaw assisted collapsing domain walls](https://arxiv.org/abs/2512.22339)
*Debasish Borah,Indrajit Saha*

**TL;DR**: 研究发现重右手中微子（RHNs）通过量子修正产生Z2破缺偏置项，既能消除宇宙学观测中不稳定的拓扑缺陷（如畴壁），又能在I型跷跷板机制下关联中微子质量尺度、引力波峰值振幅与频率，并为共振轻子生成解释宇宙重子不对称性提供微小质量分裂。


<details>
  <summary>Details</summary>

Background: 研究探讨了离散对称性（如$Z_2$）自发破缺导致的拓扑缺陷（畴壁）及其通过显式$Z_2$破缺偏置项的消除机制，同时关联了I型跷跷板机制中的重右手中微子（RHNs）在量子修正下产生偏置项的作用。

Data: 研究中使用的数据来源包括数值模拟（simulation）和现有实验数据，如宇宙微波背景（CMB）的精确测量结果以及当前和未来的引力波（GW）实验数据。

Method: 采用量子修正方法研究重右手中微子（RHNs）在I型跷跷板机制中生成Z2破缺偏置项的作用。

Result: 在I型跷跷板机制中，重右手中微子（RHNs）通过量子修正产生显式Z_2破缺偏置项，导致畴壁湮灭并产生随机引力波（GW），其峰值振幅和频率与跷跷板能标存在可观测关联。
...
### [43] [Toward the Effective Light and Heavy QCD Axion Scenarios](https://arxiv.org/abs/2512.22829)
*Hai-Jun Li*

**TL;DR**: 研究揭示了轻、重QCD轴子（axion）场景中轴子质量（axion mass）与衰变常数（axion decay constant）参数空间的相似质量比与相反的衰变常数比特征，并将结论推广至多轴子体系。


<details>
  <summary>Details</summary>

Background: 研究探讨了QCD轴子（axion）在轻质量和重质量情景下，轴子质量与衰变常数（decay constant）的有效参数空间特征及其普适性规律。

Data: 数据来源于对两种QCD轴子（axion）情景（轻质量轴子和重质量轴子）的理论模拟（simulation）分析。

Method: 采用双轴子（two axions）模型定量分析轻、重QCD轴子（QCD axion）场景下的轴子质量（axion mass）与衰变常数（axion decay constant）参数空间。

Result: 在轻和重QCD轴子（axion）情景下，轴子质量比高度相似而轴子衰变常数（axion decay constant）比完全相反，该结论可推广至多轴子情形。
...
### [44] [Mini-review on the production of Primordial Black Holes from First-Order Phase Transitions in the Early Universe](https://arxiv.org/abs/2512.22989)
*Indra Kumar Banerjee,Ujjal Kumar Dey,Shaaban Khalil*

**TL;DR**: 该论文综述了一阶相变（first order phase transitions）产生原初黑洞（primordial black holes）的机制，探讨了模型依赖和独立机制及其与原初黑洞特性的关联，并提供了具体模型示例。


<details>
  <summary>Details</summary>

Background: 研究主题为探讨一阶相变（first order phase transition）在原始黑洞（primordial black holes）形成机制中的作用及其多样性。

Data: 数据来源包括理论模型（theoretical models）和文献综述（literature review）中的已有研究结果。

Method: 采用模型依赖（model-dependent）和模型无关（model-independent）机制分析一阶相变（first order phase transitions）产生原初黑洞（primordial black holes）的生成机制。

Result: 综述了一阶相变（first order phase transitions）产生原初黑洞（primordial black holes）的多种模型依赖性和独立性机制，并将这些机制的特性与原初黑洞的特性相关联。
...
### [45] [Defect Formation in NaI Crystals: A Novel Pathway to Dark Matter Detection](https://arxiv.org/abs/2512.23553)
...
**TL;DR**: 该研究通过分子动力学模拟和密度泛函理论技术揭示了碘化钠(NaI)晶体中暗物质(DM)粒子碰撞引发的缺陷（包括间隙原子和空位）形成机制及其对电子态和位移能阈值的影响，提出利用带隙新态将暗物质诱导缺陷作为新型探测通道的可能性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于碘化钠（NaI）晶体中缺陷形成及其对暗物质（dark matter）探测信号的影响，旨在探索暗物质诱导缺陷作为新型探测通道的潜力。

Data: 使用分子动力学模拟（molecular dynamics simulations）和密度泛函理论（density functional theory）技术模拟了暗物质粒子与碘化钠（NaI）晶体的碰撞过程，重点关注缺陷形成及其结构和电子特性。

Method: 采用分子动力学模拟（molecular dynamics simulations）和密度泛函理论（density functional theory）技术研究NaI晶体中缺陷形成及其结构、电子特性。

Result: 通过分子动力学模拟和密度泛函理论技术，发现暗物质粒子碰撞可在碘化钠晶体中形成间隙原子和空位缺陷，其电子态会引入带隙新态，为暗物质探测提供新通道。
