...
**TL;DR**: ELVES-Field（探索本地体积巡天-场区）调查在约3000平方度范围内探测并确认了95个本地体积（LV）矮星系（dwarf galaxies），其中44个为孤立场矮星系，通过质量函数分析发现其高质量端与红移巡天及IllustrisTNG模拟预测一致，为研究小尺度结构及环境对矮星系演化的影响提供了关键数据集。


<details>
  <summary>Details</summary>

Background: 研究聚焦于本地体积（Local Volume）内场（field）矮星系的普查及其演化环境效应。

Data: 数据来源为基于ELVES-Field巡天项目（覆盖约3000平方度天区）的半自动化算法检测的候选矮星系样本，并通过红移、红巨星分支尖端（TRGB）和表面亮度波动（SBF）等距离测量方法确认其真实性。

Method: 采用半自动化算法检测低表面亮度矮星系，并通过红移、红巨星分支顶端测距和表面亮度波动等方法确认候选体为邻近矮星系。

Result: result：ELVES-Field（探索局部体积巡天-场区）在约3000平方度天区内探测并确认了95个局部体积（LV）矮星系，其中44个为孤立场矮星系，其质量函数在高质量端与以往红移巡天及IllustrisTNG模拟预测相符。
...
### [2] [Causal Reversal in the $M_\unicode{x25CF}\unicode{x2013}σ_0$ Relation: Implications for High-Redshift Supermassive Black Hole Mass Estimates](https://arxiv.org/abs/2602.16775)
*Benjamin L. Davis,Saakshi More,Zehao Jin,Mario Pasquato,Andrea Valerio Macciò,Feng Yuan*

**TL;DR**: 研究发现超大质量黑洞（SMBH）与宿主星系的因果演化方向在恒星形成峰值前后发生反转，并基于因果关系的修正标度关系预测高红移黑洞质量比现有估计低约两个量级。


<details>
  <summary>Details</summary>

Background: 研究主题为应用因果发现（causal discovery）原理探索超大质量黑洞（SMBHs）与宿主星系共演化的因果本质，重点关注不同星系形态中因果方向的差异性及其随时间的变化。

Data: 数据来源于NIHAO宇宙学放大流体动力学模拟套件（NIHAO suite of cosmological zoom-in hydrodynamical simulations），用于追踪单个星系及其中心超大质量黑洞（SMBH）质量（$M_\unicode{x25CF}$）和中心恒星速度弥散（$σ_0$）等属性的演化过程。

Method: 采用因果发现（causal discovery）方法分析NIHAO宇宙学流体动力学模拟（cosmological zoom-in hydrodynamical simulations）数据，追踪超大质量黑洞（SMBH）质量（$M_\unicode{x25CF}$）与宿主星系中心恒星速度弥散（$σ_0$）的时序演化关系。

Result: 超大质量黑洞（SMBHs）与宿主星系的因果方向在恒星形成峰值前后发生明显反转。
...
**TL;DR**: 利用ELVES-Field巡天新样本研究了本地宇宙（LV）低质量（$M_* < 10^9$ $M_\odot$）孤立矮星系的恒星形成特性和尺寸，首次发现$M_* \lesssim 10^7$ $M_\odot$的孤立矮星系中约30%处于静止状态（quenched），且无论恒星形成状态如何，孤立矮星系比同质量卫星矮星系尺寸小约20%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于本地宇宙（Local Volume）中低质量孤立矮星系的恒星形成特性和尺寸分布，特别关注质量低于$10^9 M_\odot$的场矮星系（field dwarf galaxies）在局部环境下的演化差异。

Data: 数据来源于ELVES-Field（Exploration of Local VolumE Survey - Field）巡天项目，通过表面亮度涨落（surface brightness fluctuations）测距方法筛选出近3000平方度成像中的2300余个候选本地体积（Local Volume）矮星系，最终确认95个真实成员并排除1600余个背景天体，其中46个为新发现或首次测距确认的矮星系。

Method: 采用表面亮度波动法（surface brightness fluctuations）测定距离，并基于投影距离大于2倍维里半径（$>2\times R_{\mathrm{vir}}$）的环境标准筛选孤立矮星系。

Result: 在低质量（$M_* \lesssim 10^7$ $M_\odot$）孤立矮星系中，约30%的场矮星系（field dwarfs）处于静止状态（quenched），而高质量（$M_* \gtrsim 10^7$ $M_\odot$）场矮星系几乎全部处于恒星形成状态。
...
### [4] [The CGM with local universe FRBs: evidence of strong AGN feedback in a massive elliptical galaxy](https://arxiv.org/abs/2602.16781)
*Samuel McCarty,Liam Connor,Ralf M. Konietzka*

**TL;DR**: 利用快速射电暴(FRB)首次测量了宿主星系晕(CGM)的气体质量分数(f_gas)，发现大质量椭圆星系因活动星系核(AGN)反馈损失了90%重子物质，而L*星系保留了更多重子，并证明FRB可探测10^11-10^14 M⊙质量范围内其他手段无法触及的晕气体分布。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用快速射电暴（FRBs）探测宇宙重子物质分布，特别是晕（halo）内弥散气体对星系形成和演化的影响。

Data: 数据来源于五个低红移快速射电暴（FRBs）样本，其宿主星系星际介质（ISM）的色散量（DM）可忽略，用于约束各晕（halo）中环星系介质（CGM）的质量。

Method: 利用快速射电暴（FRB）的色散量（DM）测量约束星系晕（halo）内环星系介质（CGM）的气体质量分数（$f_\mathrm{gas}$），并分析不同质量星系晕（$10^{11-13}M_\odot$至$10^{14}M_\odot$）的反馈效应差异。

Result: 通过快速射电暴(FRB)测量发现，大质量椭圆宿主星系的气体质量显著缺失（$M_\mathrm{gas}=0.02^{+0.02}_{-0.02}M_\mathrm{h}$），这与活动星系核(AGN)反馈机制一致，而$L_*$星系则显示出更高的重子保留率。
...
**TL;DR**: 通过哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）对三个四重透镜类星体的观测，发现其透镜星系为具有典型德沃库勒（de Vaucouleurs）光剖面的早型星系（early-type galaxies），处于中等至高红移（intermediate to high redshift），代表了尚未充分探索的亚角秒分离透镜（subarcsecond-separation lenses）群体。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）观测三个四重透镜类星体（quadruply lensed quasars），探索中高红移（intermediate to high redshift）早型星系（early-type galaxies）作为强引力透镜（strong lensing galaxies）的几何构型与质量分布特性。

Data: 数据来源于哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）对三个红移分别为$z = 2.550$、2.975和1.500的四重透镜类星体的成像观测。

Method: 采用奇异等温椭球体(singular isothermal ellipsoid)质量分布模型和椭圆塞尔西奇(Sérsic)光度轮廓对透镜星系建模，重建了各系统的几何构型并测量了爱因斯坦半径。

Result: 通过哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）观测三个四重透镜类星体（redshift分别为2.550、2.975和1.500），发现其透镜星系（lensing galaxies）均为中高红移（intermediate to high redshift）的早型星系（early-type galaxies），具有典型的德沃库勒（de Vaucouleurs）光剖面（$n_{\rm S\acute{e}rsic} \sim 4$）和较小有效半径（$R_e \sim 1.5 - 3.5$ kpc），代表爱因斯坦半径（Einstein radii）较小（0.44$'$-0.58$'$）而透镜红移（lens redshifts）较高的未充分探索的亚角秒分离透镜（subarcsecond-separation lenses）种群。
...
### [6] [Under Pressure: UV Emission Line Ratios as Barometers of AGN Feedback Mechanisms](https://arxiv.org/abs/2602.17039)
...
**TL;DR**: 通过分析五个低红移(redshift)遮蔽类星体(obscured quasars)的远紫外(FUV)光谱，发现其中三个目标的窄线区(narrow-line region)云动力学由辐射压(radiation pressure)主导，一个目标同时受辐射压和热风压(hot wind)影响，表明活动星系核(AGN)反馈主要由辐射压驱动或热风短暂相变介导。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）反馈机制中辐射压（radiation pressure）与热风压（hot wind pressure）对星系核周窄线区（narrow-line region）电离气体动力学影响的相对贡献。

Data: 数据来源于宇宙起源摄谱仪（Cosmic Origins Spectrograph）对五个低红移目标（obscured quasars）的远紫外（FUV）光谱观测。

Method: 通过比较窄线区(NLR)紫外发射线比率与两种压力源(辐射压和热风压)重要性不同的理论模型，约束主导驱动机制。

Result: 在五个低红移目标中，三个目标的光电离窄线区（narrow-line region）云动力学由辐射压（radiation pressure）主导，一个目标显示辐射压与热风压（hot wind）共同作用，最低光度目标可能受热风压影响但证据不足。
...
### [7] [PRODIGE - envelope to disk with NOEMA: VII. (Complex) organic molecules in the NGC1333 IRAS4B1 outflow: A new laboratory for shock chemistry](https://arxiv.org/abs/2602.17177)
...
**TL;DR**: 首次在NGC1333 IRAS4B1原恒星喷流中可靠探测到CH3CN、CH3CHO和CH2DOH等复杂有机分子（COMs），其分子丰度梯度与喷流结构差异表明该区域是研究激波（shock）化学的新实验室。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用激波化学（shock chemistry）探究复杂有机分子（COMs）的形成与破坏机制，并将NGC1333 IRAS4B1原恒星外流作为研究激波对COM化学影响的新实验室。

Data: 数据来源于PRODIGE（PROtostars & DIsks: Global Evolution）大型观测项目对NGC1333 IRAS4B1原恒星外流的观测数据。

Method: 采用积分强度比映射（integrated intensity ratio mapping）和旋转温度（rotational temperature）分析方法，结合典型外流示踪分子（SiO、CO）和复杂有机分子（COMs）的观测数据，研究了激波对分子丰度分布的影响。

Result: 首次在IRAS 4B1外流中可靠探测到复杂有机分子（COMs）CH3CN、CH3CHO和CH2DOH，表明该外流是研究激波化学（shock chemistry）的新实验室。
...
### [8] [MIDIS: The identification of deep MIRI-red sources as candidates for extreme Balmer-break and line emitting galaxies at high-z](https://arxiv.org/abs/2602.17329)
...
**TL;DR**: 论文在哈勃超深场（Hubble Ultra Deep Field）的MIRI深度成像巡天（MIDIS）中发现了7个5.6微米中红外红（MIRI-red）星系候选体，包括一个高红移星系候选体，并证实这些天体可能由发射线星系（EW_0(Hα)≥750Å或EW_0(Hβ+[OIII])≥600Å）或高红移巴尔末跳变（Balmer breaks≥1.6）引起，同时发现一个极端MIRI-red星系可能对应Hα等效宽度达6000Å或巴尔末跳变达6.3的高红移小红点（LRD）类似体。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用JWST/MIRI在哈勃超深场（Hubble Ultra Deep Field）进行的MIDIS深度成像巡天中探测并分析5.6微米中红外（MIRI-red）源的性质及其物理起源。

Data: 数据来源于MIRI Deep Imaging Survey (MIDIS)，该巡天覆盖哈勃超深场（Hubble Ultra Deep Field）2.4平方角分区域，是迄今最深的JWST/MIRI观测，探测极限达28.75星等（5σ）。

Method: 采用经验性方法（empirical methodology）评估MIRI探测的纯度（purity）和完整性（completeness），并结合多波段颜色筛选（$m_{\rm F444W} - m_{\rm F560W} \ge 0.5$）识别MIRI-red源。

Result: 在MIDIS（中红外深场成像巡天）中发现的5.6微米MIRI红源（MIRI-red）可能源于发射线星系（Hα等效宽度≥750 Å或Hβ+[OIII]≥600 Å）、高红移巴尔默间断（≥1.6）或类似小红点（LRDs）的天体，其中极端红源（F444W未探测）的Hα和Hβ+[OIII]等效宽度分别达6000 Å和4000 Å，或具有6.3巴尔默间断的高红移LRD类似体。
...
### [9] [Dynamical Modelling of Galactic Kinematics using Neural Networks](https://arxiv.org/abs/2602.17371)
*David A. Simon,Michele Cappellari,Shude Mao,Jiani Chu,Dandan Xu*

**TL;DR**: 该论文通过构建基于Jeans各向异性模型（JAM）的早型星系训练集并训练神经网络，实现了用简单架构精确模拟JAM星系动力学，显著提升了建模速度。


<details>
  <summary>Details</summary>

Background: 积分视场数据（integral field data）的兴起革新了星系演化研究，而动力学建模方法（dynamical modelling methods）作为其关键组成部分，虽能通过运动学数据（kinematic data）提供重要见解，却常依赖与真实星系不符的几何（axisymmetry or triaxiality）、速度椭球形态或恒星分布等假设。

Data: 使用Jeans各向异性建模（JAM）生成的早期型星系动力学模型训练集和模拟测光数据作为输入数据。

Method: 采用Jeans各向异性建模（JAM）构建早型星系动力学模型训练集，并基于JAM参数和模拟测光数据输入训练神经网络。

Result: 积分视场数据（integral field data）的出现革新了星系演化研究，其中动力学建模方法（dynamical modelling methods）为运动学数据（kinematic data）提供了关键见解。
...
### [10] [UV slopes of Starforming Galaxies in Strong Lensing fields at the Epoch of Reionization with JWST](https://arxiv.org/abs/2602.17499)
...
**TL;DR**: 利用强引力透镜观测7.5<z<12.5的暗弱星系（M_UV~-16），发现紫外连续谱斜率（β）在z>7.5时趋于平坦（β=-2.3±0.4），且与M_UV仅呈弱负相关，表明高红移星系星际介质（ISM）成分相似，14个星系平均电离光子逃逸分数（f_esc）为0.26±0.22。


<details>
  <summary>Details</summary>

Background: 紫外光谱斜率（UV slope, β）是研究再电离时期（Epoch of Reionization）星系的重要探针，用于追踪恒星形成、星际介质（ISM）电离及电离光子逃逸率（f_esc）。

Data: 数据来源于CANUCS、Technicolor、JUMPS、Silver Bullet、UNCOVER和MEGASCIENCE项目的7个强引力透镜场中的NIRCam和NIRSpec观测数据。

Method: 采用正向建模（forward-modelling）程序测量紫外斜率（β），并利用基于低红移样本校准的紫外斜率、星系大小和Hβ等效宽度关系估算电离光子逃逸分数（f_{esc}）。

Result: 在7.5<z<12.5的高红移星系中观测到紫外连续谱斜率(β)平均值为-2.3±0.4，且未发现β随红移的显著演化，表明在z>7.5时β-z关系趋于平坦。
...
### [11] [Interstellar Formation of Thioethanal (CH$_{3}$CHS). Gas-Phase and Ice-Surface Mechanisms involving Secondary Sulfur Products](https://arxiv.org/abs/2602.17551)
*N. Rani,S. Vogt-Geisse,S. Bovino*

**TL;DR**: 研究发现硫乙醇（CH$_{3}$CH$_{2}$SH）在低温星际气相中可通过无势垒机制生成硫乙醛（CH$_{3}$CHS），并在冰颗粒表面因位点特异性调制形成竞争路径生成乙烷-1,1-二硫醇（CH$_{3}$CH(SH)SH），同时首次精确测定了硫乙醇在水冰上的结合能。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际介质中硫化物（sulfur-bearing species）的形成路径及其对冷分子云（cold molecular clouds）中硫预算（sulfur budget）的启示。

Data: 研究采用电子结构方法和密度泛函理论（density functional theory）模拟气相和冰粒表面环境，并使用中等（W6）和大尺寸（W22）非晶化水簇模拟星际冰幔，数据来源于结合能评估协议（Binding Energy Evaluation protocol）中的实现。

Method: 采用电子结构方法和密度泛函理论（density functional theory）研究了气相和冰粒表面环境下的反应机制，并利用W6和W22非晶水簇模拟星际冰幔，通过结合能评估协议（Binding Energy Evaluation protocol）分析表面反应路径（Langmuir-Hinshelwood和Eley-Rideal机制）。

Result: 在气相条件下发现了低温星际环境中硫代乙醛（CH$_{3}$CHS）的无势垒形成机制，而冰粒表面环境通过位点特异性方式调节活化能垒，并揭示了硫化学因S-H键较弱而存在替代路径，导致表面反应可能形成乙烷-1,1-二硫醇（CH$_{3}$CH(SH)SH）从而降低CH$_{3}$CHS产率。
...
### [12] [MUSEQuBES: Probing Anisotropies in Gas and Metal Distributions in the Circumgalactic Medium](https://arxiv.org/abs/2602.17593)
...
**TL;DR**: 研究发现低质量星系（7 < log10(M*/Msun) < 9）的环星系介质（CGM）中H I和O VI气体覆盖分数（covering fraction）在盘面方向（方位角phi < 20度）和极向（phi > 70度）呈现双峰分布，而高质量星系（9 < log10(M*/Msun) < 11.3）中未观测到此现象，且O VI运动学特征保留其起源位置信息。


<details>
  <summary>Details</summary>

Background: 研究聚焦于红移范围0.12 < z < 0.75的113个孤立星系周围介质（CGM）中氢原子（H I）和六价氧离子（O VI）气体的方位角依赖性分布特征及其与星系质量的关系。

Data: 数据来源包括MUSE Quasar-fields Blind Emitters Survey（MUSEQuBES）的91个新测量结果以及113个孤立星系（红移范围0.12 < z < 0.75）的H I和O VI气体观测数据。

Method: 采用晕环速度归一化像素速度两点相关函数（TPCF）分析O VI吸收体的运动学特性。

Result: 低质量星系(7 < log10(M*/Msun) < 9)的H I覆盖分数(k_HI)在盘面方向(方位角phi < 20度)和极向(phi > 70度)均呈现增强，而高质量星系(9 < log10(M*/Msun) < 11.3)未观测到这种双峰分布。
...
### [13] [A Chemodynamical Census of the Milky Way's Ultra-Faint Compact Satellites. I. A First Population-Level Look at the Internal Kinematics and Metallicities of 19 Extremely-Low-Mass Halo Stellar Systems](https://arxiv.org/abs/2602.17652)
...
**TL;DR**: 首次对19个"超暗致密卫星"(UFCSs)进行光谱普查，发现其运动学比超暗弱矮星系(UFDs)更冷、化学多样性显著（[Fe/H]范围达300倍），且约50%可能是迄今发现的最小最轻星系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系晕中发现的超致密（Ultra-Faint Compact Satellites, UFCSs）恒星系统的性质与起源，填补了超暗弱矮星系（UFDs）与经典球状星团（GCs）之间的观测空白。

Data: 数据来源于麦哲伦望远镜（Magellan/IMACS）和凯克望远镜（Keck/DEIMOS）对19个超暗致密卫星（UFCSs）的光谱观测，以及盖亚卫星（Gaia）提供的18个系统的平均自行运动数据。

Method: 采用Magellan/IMACS和Keck/DEIMOS光谱观测技术测量径向速度(radial velocity)、速度弥散(velocity dispersion)、金属丰度(metallicity)及其弥散度，并结合Gaia卫星的自行(proper motion)数据进行分析。

Result: UFCSs（Ultra-Faint Compact Satellites）在运动学上比UFDs（Ultra-Faint Dwarf galaxies）更冷，排除了大多数情况下存在非常密集的暗物质晕的可能性。
...
### [14] [C3NN-SBI: Learning Hierarchies of $N$-Point Statistics from Cosmological Fields with Physics-Informed Neural Networks](https://arxiv.org/abs/2602.16768)
*Kai Lehman,Zhengyangguang Gong,David Gebauer,Stella Seitz,Jochen Weller*

**TL;DR**: 该研究设计了一种结合机器学习效率与N点相关函数（NPCF）物理直觉的模拟推断流程，通过约束性宇宙学相关卷积神经网络（C3NN）从模拟透镜收敛图中提取可关联到特定阶NPCF的统计量，为物理信息驱动的模拟推断提供了新途径。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过超越传统两点统计量的高阶N点相关函数（NPCFs）和机器学习优化方法，提升宇宙学场（cosmological fields）信息提取效率与物理可解释性之间的平衡。

Data: 数据来源为模拟的透镜收敛图（simulated lensing convergence maps）。

Method: 采用基于模拟的推断（simulation-based inference）流程，结合受严格约束的宇宙学相关卷积神经网络（C3NN）提取与给定阶数N点相关函数（NPCF）直接关联的摘要统计量。

Result: 通过约束性强的宇宙学相关卷积神经网络（C3NN）提取可直接关联到特定阶数N点相关函数（NPCF）的摘要统计量，在理想化弱透镜收敛图示例中验证了该方法能有效保留NPCF形式并优化信息提取效率。
...
### [15] [The Sign-Switching of the Cosmological Constant](https://arxiv.org/abs/2602.16795)
*Mariam Bouhmadi-López,Beñat Ibarra-Uriondo*

**TL;DR**: 提出并研究了一类动态暗能量（dynamical dark energy）模型，其中宇宙学常数（cosmological constant）从早期宇宙的负值演化为低红移的正值，通过阶梯式或平滑过渡的统一框架描述了变号宇宙学常数现象，并基于宇宙学诊断（cosmographic diagnostics）和线性扰动分析揭示了其对物质密度对比度（matter density contrast）、引力势（gravitational potential）和$fσ_8$可观测量的独特影响，证实了此类模型在观测数据中的现象学可行性及其与标准$Λ$CDM模型在宇宙结构形成中的差异特征。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙学常数（cosmological constant）从早期宇宙负值演化至低红移正值的动态暗能量（dynamical dark energy）模型，为符号反转的宇宙学常数提供了统一描述框架。

Data: 数据来源于对宇宙学常数（cosmological constant）动态演化模型的数值模拟（simulation）分析，包括背景动力学和线性扰动方程的求解，并与标准ΛCDM模型及当前观测数据进行对比。

Method: 采用宇宙学诊断学（cosmographic diagnostics）分析理论构建和背景动力学，并在线性扰动体系中求解辐射主导时期具有绝热初始条件的扰动方程。

Result: 提出并研究了一类动力学暗能量（dynamical dark energy）模型，其中宇宙学常数（cosmological constant）从早期宇宙的负值演化至低红移的正值，通过广义阶梯式或平滑过渡情景统一描述了符号变化的宇宙学常数。
...
### [16] [Analytical Estimates of Gravitational Wave Background Anisotropies from Shot Noise and Large-Scale Structure in Pulsar Timing Arrays](https://arxiv.org/abs/2602.16808)
*Meng-Xiang Lin,Adam Lidz,Chung-Pei Ma*

**TL;DR**: 基于经验校准的超大质量黑洞双星(SMBHB)并合率模型，计算了纳赫兹频段引力波背景(GWB)的各向异性，发现离散SMBHB群体产生的散粒噪声(shot-noise)各向异性通常接近或超过现有NANOGrav上限，而大尺度结构(LSS)诱导的各向异性小2-3个量级。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用脉冲星计时阵（PTA）探测纳赫兹频段引力波背景（GWB）各向异性的科学意义，重点分析超大质量黑洞双星（SMBHB）并合率模型对GWB各向异性特征的预测及其与宇宙大尺度结构（LSS）的关联。

Data: 研究中使用的数据来源于经验校准的超大质量黑洞双星(SMBHBs)并合率模型。

Method: 采用经验校准的超大质量黑洞双星(SMBHBs)并合率模型计算引力波背景(GWB)各向异性，重点分析离散SMBHB群体的散粒噪声(shot-noise)项及其频率依赖性，同时考虑宇宙大尺度结构(LSS)的微弱贡献。

Result: 基于经验校准的超大质量黑洞双星(SMBHB)并合率模型，预测了脉冲星计时阵(PTAs)在纳赫兹频段探测到的引力波背景(GWB)各向异性主要由离散SMBHB群体的散粒噪声(shot-noise)主导，该噪声对SMBHB质量函数的高质量端敏感且强烈依赖于观测频率。
...
### [17] [The Epoch of Reionization 21 cm Bispectrum at $z=8.2$ from MWA data II: Smooth Component Filtering](https://arxiv.org/abs/2602.17304)
*Sukhdeep Singh Gill,Khandakar Md Asif Elahi,Somnath Bharadwaj,Shiv K. Sethi,Akash Kumar Patwa*

**TL;DR**: 利用平滑分量滤波(SCF)方法处理MWA观测数据，在再电离时期(EoR)首次测量了21厘米双谱(BS)，并给出了亮度温度波动Δ³的上下限约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用21厘米双谱（BS）探测再电离时期（EoR），并通过平滑分量滤波（SCF）技术抑制主导的天体物理前景干扰。

Data: 使用默奇森宽场阵列（MWA）在154.2 MHz（对应红移z=8.2）的观测数据，并通过模拟21厘米信号验证数据处理流程。

Method: 采用平滑分量滤波（SCF）技术抑制前景干扰并估计21厘米双谱（BS），通过模拟信号验证了在$k_{\parallel} \ge 0.135~{\rm Mpc}^{-1}$模式下能恢复输入双谱。

Result: 采用平滑分量滤波(SCF)方法显著抑制了前景干扰，使圆柱形双谱(BS)振幅降低了3-4个数量级，并有效消除了数据中缺失频率通道导致的伪影。
...
### [18] [Testing the cosmic distance-duality relation with localized fast radio bursts: a cosmological model-independent study](https://arxiv.org/abs/2602.16869)
*Jéferson A. S. Fortunato,Surajit Kalita,Amanda Weltman*

**TL;DR**: 通过结合Pantheon+超新星(SNIa)光度距离和快速射电暴(FRBs)重建的角直径距离，采用人工神经网络(ANNs)数据驱动方法验证了Etherington宇宙距离对偶关系(CDDR)，未发现当前精度下显著偏离CDDR的证据，同时约束了快速射电暴宿主星系平均色散量(DM_host)为128.8±34.1 pc cm⁻³。


<details>
  <summary>Details</summary>

Background: 验证宇宙距离对偶关系（Etherington cosmic distance-duality relation, CDDR）在Ia型超新星（SNIa）光度距离与快速射电暴（FRB）角直径距离比较中的适用性。

Data: 数据来源包括Pantheon+汇编的Ia型超新星（SNIa）光度距离数据和通过人工神经网络（ANN）从局域化快速射电暴（FRBs）重建的角直径距离（angular-diameter-distance）。

Method: 采用人工神经网络（ANNs）从快速射电暴（FRBs）数据中重建平滑的河外色散-红移关系，并通过其红移导数推算哈勃参数（H(z)）和角直径距离（D_A^FRB(z)），无需假设宇宙膨胀历史的参数化形式。

Result: 通过比较Pantheon+数据集中的Ia型超新星（SNIa）光度距离与快速射电暴（FRB）重建的角直径距离，未发现当前精度下宇宙距离对偶关系（CDDR）存在统计学显著偏离的证据。
...
### [19] [Inflationary Reheating to Preheating - A Personal Account](https://arxiv.org/abs/2602.16963)
*Robert Brandenberger,Jennie Traschen*

**TL;DR**: 该论文回顾了暴胀宇宙学"预热阶段"(preheating stage)的早期研究，并指出其不稳定性机制在宇宙学中的广泛适用性。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于暴胀宇宙学（inflationary cosmology）中“预加热阶段”（preheating stage）早期工作的理论基础及其机制在宇宙学中的广泛适用性。

Data: 数据来源于理论推导和数值模拟（numerical simulation）。

Method: 采用核心分析方法为暴胀宇宙学（inflationary cosmology）中的预热阶段（preheating stage）不稳定性机制研究。

Result: 发现了宇宙暴胀（inflationary cosmology）早期阶段“预加热（preheating stage）”现象背后的不稳定性机制具有更广泛的宇宙学适用性。
...
### [20] [Cosmic voids evolution in modified gravity via hydrodynamics](https://arxiv.org/abs/2602.17644)
*Tommaso Moretti,Noemi Frusciante,Giovanni Verza,Francesco Pace*

**TL;DR**: 通过构建修正引力（MG）下球状空洞演化的流体动力学框架，揭示了有效引力耦合强度对欧拉密度对比的非线性控制机制，并在发光伽利略子（luminal Galileon）模型中定量分析了引力修正对空洞演化、拉格朗日-欧拉映射及壳层交叉阈值的层级化影响（10%量级的引力耦合修正、百分比水平的密度演化偏移、亚百分比的映射与阈值偏差），同时证明空洞始终处于物理分支的非屏蔽区域。


<details>
  <summary>Details</summary>

Background: 研究探讨了修正引力（modified gravity, MG）理论下球形空洞（void）演化的流体动力学描述，通过将引力修正编码为进入欧拉方程和泊松方程的有效耦合项，扩展了广义相对论（General Relativity, GR）和动态暗能量的标准处理方法。

Data: 数据来源于对球状空洞（spherical void）在修正引力（modified gravity）理论下的流体动力学演化模拟，重点分析了发光伽利略子（luminal Galileon）模型中的参数空间约束和空洞深度与红移关系。

Method: 采用流体动力学方法描述修正引力（MG）中球形空洞演化，通过将引力修正编码为进入欧拉和泊松方程的有效耦合项，构建了受时间与密度依赖的有效引力强度控制的非线性演化方程。

Result: 在修正引力（MG）模型中，球形空洞演化受时间与密度依赖的有效引力强度控制，导致空洞密度演化出现百分比级别的偏移，而拉格朗日-欧拉映射与壳层交叉阈值的偏差低于百分之一。
...
### [21] [Fast pre-merger detection of massive black-hole binaries in LISA based on time-frequency excess power](https://arxiv.org/abs/2602.16792)
*Francesco Nobili,Malvina Bellotti,Riccardo Buscicchio,Massimo Dotti,Alessandro Lupi*

**TL;DR**: 提出了一种快速算法，用于在LISA（Laser Interferometer Space Antenna）数据中通过短时傅里叶变换谱图搜索啁啾（chirping）时频形态的过量功率，实现对超大质量黑洞双星（MBHBs）并合前的实时探测和初步参数（啁啾质量和并合时间）估计，验证显示对15个注入信号均成功检测且参数误差低于3%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用激光干涉空间天线（LISA）探测宇宙时间尺度上大质量黑洞双星（MBHB）合并前的引力波信号。

Data: data：研究使用Sangria LISA Data Challenge数据集中的引力波数据验证算法性能。

Method: 基于短时傅里叶变换谱图中啁啾(chirping)时频形态的功率超量搜索方法，结合四极矩频率演化定义的时频平面切片划分及背景噪声拟合。

Result: 提出了一种快速算法，通过短时傅里叶变换谱图中啁啾（chirping）时频形态的功率过剩搜索，实现对大质量黑洞双星（MBHBs）并合前的探测和初步表征。
...
### [22] [Status of the STIS Auto-wavecal Exposures](https://arxiv.org/abs/2602.16946)
*D. Welty,S. Lockwood*

**TL;DR**: 研究发现STIS光谱仪使用的铂铬氖灯(Pt/Cr-Ne lamps)在短波段的衰减导致波长校准需增加最短波段默认曝光时间以确保偏移量(SHIFTA)测量精度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于哈勃太空望远镜成像摄谱仪(STIS)波长校准(wavecal)光谱的可靠性问题，特别是针对铂铬氖灯(Pt/Cr-Ne lamps)衰减导致的短波区校准精度下降现象。

Data: 数据来源于STIS（Space Telescope Imaging Spectrograph）光谱观测中默认的波长校准（wavecal）光谱，以及用于模拟的Pt/Cr-Ne灯（Pt/Cr-Ne lamps）衰减特性数据。

Method: 采用波长校准谱(SHIFTA1/SHIFTA2)偏移量统计分析、铂铬氖灯(Pt/Cr-Ne)波长依赖性衰减测量及曝光时间需求模拟的方法。

Result: 当前波标（wavecal）光谱在最短波长设置下需增加默认曝光时间以确保波长零点（zero points）的可靠性。
...
### [23] [Kolmogorov analysis of pulsar TOA](https://arxiv.org/abs/2602.17188)
*N. Galikyan,A. A. Kocharyan,V. G. Gurzadyan*

**TL;DR**: 利用柯尔莫哥洛夫随机性参数（Kolmogorov stochasticity parameter, KSP）分析NANOGrav毫秒脉冲星计时数据中白噪声的随机性程度，发现不同脉冲星的白噪声随机性存在非均匀分布且随时间变化，表明存在影响计时精度的非平稳物理过程。


<details>
  <summary>Details</summary>

Background: 研究利用柯尔莫哥洛夫随机性参数（Kolmogorov stochasticity parameter, KSP）分析NANOGrav脉冲星计时数据中随机引力波背景（stochastic gravitational wave background）信号的非平稳特性及其物理过程影响。

Data: 研究中使用的数据来源为北美纳赫兹引力波天文台（NANOGrav）提供的68颗脉冲星的计时到达时间（TOA）白噪声数据。

Method: 采用Kolmogorov随机性参数（KSP）分析NANOGrav脉冲星计时数据中白噪声的随机性程度及其非均匀性。

Result: 利用柯尔莫哥洛夫随机性参数（Kolmogorov stochasticity parameter, KSP）分析NANOGrav毫秒脉冲星计时数据中的白噪声随机性程度，揭示了不同脉冲星白噪声随机性的非均匀分布特征及随时间变化的非平稳物理过程影响。
...
### [24] [Deeper detection limits in astronomical imaging using self-supervised spatiotemporal denoising](https://arxiv.org/abs/2602.17205)
...
**TL;DR**: 提出了一种基于自监督变换器（ASTERIS）的天文图像去噪算法，通过整合多曝光时空信息将检测极限提升1.0星等（90%完整度和纯度），并在詹姆斯·韦伯太空望远镜（JWST）和斯巴鲁望远镜数据中发现了低表面亮度星系结构和引力透镜弧等新特征，使红移>9的星系候选体数量增加三倍且紫外光度暗1.0星等。


<details>
  <summary>Details</summary>

Background: 天文成像观测的探测极限受多种相关噪声（correlated noise）影响，这些噪声在相邻像素和曝光间存在时空关联性。

Data: 数据来源包括模拟数据(mock data)、詹姆斯·韦伯太空望远镜(JWST)和斯巴鲁望远镜(Subaru telescope)的观测数据。

Method: 采用基于自监督学习的变换器（transformer）去噪算法（ASTERIS），整合多曝光图像的时空信息进行噪声建模与校正。

Result: 开发的天文自监督去噪算法（ASTERIS）通过整合多曝光的时空信息，在保持点扩散函数和测光精度的同时，将90%完整度和纯度下的探测极限提升了1.0星等。
...
### [25] [Nested Sampling with Slice-within-Gibbs: Efficient Evidence Calculation for Hierarchical Bayesian Models](https://arxiv.org/abs/2602.17414)
*David Yallup*

**TL;DR**: 该论文提出了一种用于高维分层贝叶斯模型（hierarchical Bayesian models）贝叶斯推断和证据估计的嵌套切片-吉布斯采样算法（NS-SwiG），通过分块更新和似然预算分解将每次替换成本从二次降为线性、整体算法复杂度从三次降为二次，并在马尔可夫结构潜变量和数千维问题上验证了其优于梯度采样器的可扩展性和准确性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高维模型（如层次贝叶斯模型）中贝叶斯推断（Bayesian inference）与证据估计（evidence estimation）的算法优化问题。

Data: 数据来源于对高维模型（如层次贝叶斯模型）进行贝叶斯推断和证据估计的算法NS-SwiG在具有挑战性基准测试中的评估结果。

Method: 采用切片-吉布斯嵌套采样（NS-SwiG）算法，通过外层超参数更新和内层局部参数块更新的切片-吉布斯核（Slice-within-Gibbs kernel）对可分解似然的高维模型进行贝叶斯推断与证据估计，并利用似然预算分解（likelihood-budget decomposition）实现常数时间可行性检查以降低计算复杂度。

Result: 提出了一种基于切片-吉布斯嵌套采样(NS-SwiG)的高效贝叶斯推断算法，通过分层更新超参数和局部参数并利用似然预算分解技术，将计算复杂度从三次方降至二次方，实现了在数千维模型中的可扩展精确推理。
...
### [26] [Schwarzian Theory and Cosmological Constant Problem](https://arxiv.org/abs/2602.17476)
*Jun Nian*

**TL;DR**: 论文通过基于施瓦西理论（Schwarzian theory）的量子引力框架和系综平均，揭示了宇宙学常数（cosmological constant）源于时间重参数化模式的系综平均，并推导出暗能量密度（dark energy density）的现象学值及其状态方程。


<details>
  <summary>Details</summary>

Background: 宇宙学常数（cosmological constant）的量子起源问题及其与观测值之间120个数量级的理论差异构成了当前宇宙学能量组分研究的核心挑战。

Data: 数据来源于宇宙学观测数据（observational data in cosmology）。

Method: 采用基于Schwarzian理论（Schwarzian theory）及其系综平均（ensemble averaging）的量子引力框架分析宇宙常数问题。

Result: 宇宙学常数（cosmological constant）源自时间重参数化模式（time-reparametrization modes）的系综平均（ensemble average）。
...
### [27] [Rescuing Overabundant Dark Matter with a Strongly First Order Phase Transition in the Dark Sector](https://arxiv.org/abs/2602.16822)
*Peisi Huang,Anibal D. Medina,Carlos E. M. Wagner*

**TL;DR**: 提出暗物质（DM）通过暗希格斯（dark Higgs）强一级相变（first order phase transition）后熵注入降低残留密度，其产生的随机引力波（stochastic gravitational wave）可被未来实验探测，并为NANOGrav信号提供解释。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗物质（DM）在破缺暗U(1)_D规范对称性下的费米子性质及其通过动力学混合与标准模型的相互作用，重点关注暗希格斯场的强一级相变对暗物质遗迹丰度的调节作用。

Data: 数据来源于对暗物质（DM）在暗U(1)_D规范对称性破缺下的费米子暗物质与标准模型通过动力学混合相互作用的模拟（simulation）研究。

Method: 采用暗希格斯（dark Higgs）强一级相变（first order phase transition）后熵注入机制来调节暗物质（DM）残留丰度，并通过超冷相变（supercooled phase transition）产生随机引力波信号作为探测手段。

Result: 暗物质（DM）丰度可通过对称相中热冻结产生后，由暗希格斯（dark Higgs）强一级相变注入的熵来降低。
...
### [28] [Stochastic galactic supernova flux of semi-relativistic particles](https://arxiv.org/abs/2602.17597)
*David Alonso-González,David Cerdeño,Marina Cermeño,Andres D. Perez*

**TL;DR**: 研究发现银河系核心坍缩超新星（SNe）发射的MeV级奇异粒子（如类轴子粒子或轻暗物质）到达地球时形成的能量通量具有显著随机性（而非文献假设的平滑稳态通量），并开发了计算半相对论性粒子随机银河通量的数值工具，修正了现有对类轴子粒子和费米子暗物质的约束限值。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探讨兆电子伏特(MeV)质量级新奇异粒子（如类轴子粒子或轻暗物质）从核心坍缩超新星(SNe)半相对论性喷发后形成非平稳弥散通量的物理机制及其对地面实验观测的影响。

Data: data部分：研究通过数值模拟（numerical simulation）重建了银河系超新星（SN）爆发历史，并开发了计算半相对论性粒子随机银河通量的数值工具。

Method: 采用数值模拟方法重建银河系超新星（SNe）爆发历史，并开发通用计算工具分析半相对论性粒子的随机性银河系通量。

Result: 核心结论：数值模拟显示，半相对论性粒子（semi-relativistic particles）在银河系超新星（SNe）历史中产生的通量具有显著随机性，其能量依赖特征远非平滑近似所描述，导致地面实验预期信号的光谱形状与先前平滑假设不符。
...
### [29] [Light dark sector via thermal decays of Dark Matter: the case of a 17 MeV particle coupled to electrons](https://arxiv.org/abs/2602.17620)
*Marco Graziani*

**TL;DR**: 在8Be、4He和12C核跃迁及正电子湮灭实验中观测到17 MeV质量的新共振态X17，可能作为标准模型与暗物质（dark matter）等隐藏扇区（hidden sectors）的新型媒介子。


<details>
  <summary>Details</summary>

Background: 探索标准模型（Standard Model）与潜在隐藏扇区（hidden sectors）之间的新型玻色子（$X_{17}$）作为暗物质（dark matter）候选者的可能媒介作用。

Data: 数据来源包括ATOMKI合作组报告的核跃迁实验数据（涉及${}^{8}\mathrm{Be}$、${}^{4}\mathrm{He}$和${}^{12}\mathrm{C}$）以及PADME合作组在Laboratori Nazionali di Frascati通过正电子湮灭固定靶碳原子电子产生的$e^{+}e^{-}$末态事例数据。

Method: 采用核跃迁实验（ATOMKI）和正电子湮没暗物质实验（PADME）的数据分析技术，结合标准模型与隐藏扇区（hidden sectors）的耦合假设模型，探究17 MeV粒子（X_{17}）作为潜在媒介粒子的可能性。

Result: ATOMKI和PADME合作组的实验观测暗示可能存在质量约17 MeV的新共振态（X_{17}），可能与核子和电子相互作用。
...
### [30] [ComptonUNet: A Deep Learning Model for GRB Localization with Compton Cameras under Noisy and Low-Statistic Conditions](https://arxiv.org/abs/2602.17085)
*Shogo Sato,Kazuo Tanaka,Shojun Ogasawara,Kazuki Yamamoto,Kazuhiko Murasaki,Ryuichi Tanida,Jun Kataoka*

**TL;DR**: 提出了一种结合直接重建模型统计效率与图像架构去噪能力的混合深度学习框架ComptonUNet，在低光子统计量和高背景噪声条件下显著提高了伽马射线暴(GRB)的定位精度。


<details>
  <summary>Details</summary>

Background: 伽马射线暴(GRB)是宇宙中最剧烈的瞬变现象之一，可作为研究高能天体物理过程的重要探针。

Data: 数据来源于模拟低地球轨道任务中具有代表性背景环境下嵌入的类伽马暴(GRB-like)事件的真实仿真数据。

Method: 采用混合深度学习框架ComptonUNet，结合直接重建模型的统计效率与基于图像架构的去噪能力，在低光子统计和高背景噪声条件下实现鲁棒的伽马暴(GRB)定位。

Result: ComptonUNet通过结合直接重建模型的统计效率与图像架构的去噪能力，在低光子统计量和高背景噪声条件下显著提升了伽马暴（GRB）定位精度。
...
### [31] [Low-Thrust Trajectory Optimization for Cubesat Lunar Mission: HORYU-VI](https://arxiv.org/abs/2602.16831)
*Omer Burak Iskender,Keck Voon Ling,Mengu Cho,Sangkyun Kim,Necmi Cihan Orger*

**TL;DR**: 提出了一种低推力轨迹优化策略，使立方星（CubeSat）HORYU-VI从月球飞越轨道进入近圆形（near-circular）100公里科学轨道，总速度增量（Delta-V）为710米/秒，耗时450天。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用低推力轨迹优化策略实现立方星（CubeSat）从月球借力轨道到近圆形科学轨道的精确转移。

Data: 数据来源于NASA太空发射系统（Space Launch System）搭载的12U立方星HORYU-VI，其配备四台霍尔效应推进器（Hall-effect thrusters），并采用高精度动力学模型整合地月日木引力相互作用及月球重力场数据，所有轨迹均通过NASA通用任务分析工具（General Mission Analysis Tool, GMAT）独立验证。

Method: 采用序列二次规划(SQP)算法对分段脉冲燃烧解进行有限推力优化，并整合地月日木四体引力场与高精度月球重力场模型进行动力学建模。

Result: 采用低推力轨迹优化策略，结合分解三阶段（预飞越减速、月球引力捕获和轨道圆化）和序列二次规划（SQP）算法，使HORYU-VI立方星在710 m/s总速度增量内450天实现100公里近圆形月球科学轨道。
...
### [32] [Design of low-energy transfers in cislunar space using sequences of lobe dynamics](https://arxiv.org/abs/2602.17444)
*Naoki Hiraiwa,Mai Bando,Yuzuru Sato,Shinji Hokamoto*

**TL;DR**: 该研究提出了一种基于瓣状动力学（lobe dynamics）和图框架的系统性方法，用于设计地月圆型限制性三体问题（CR3BP）中的低能量转移轨道，并通过多打靶法将其转化为双圆限制性四体问题中的最优转移轨道。


<details>
  <summary>Details</summary>

Background: 研究聚焦于圆型限制性三体问题（CR3BP）中的动力学结构，为设计低能量转移轨道提供理论基础。

Data: 数据来源于圆形限制性三体问题（CR3BP）中的混沌轨道（chaotic orbits）和瓣结构（lobes）动力学分析，并通过双圆形限制性四体问题（bicircular restricted four-body problem）的多重打靶法（multiple shooting）进行验证。

Method: 采用基于图论(graph theory)的框架结合多瓣动力学(lobe dynamics)方法，通过连接瓣内混沌轨道系统设计低能量转移轨迹。

Result: 提出了一种结合多瓣动力学(lobe dynamics)的系统性低能量转移轨道设计方法，通过构建图论框架连接瓣内混沌轨道实现燃料高效转移。
...
**TL;DR**: 首次系统研究了潮汐瓦解事件(TDE)X射线辐射的分钟至小时级随机变异性，发现含冕(TDE coronae)的源比纯热源变率更高且功率谱密度(PSDs)更陡，新生TDE冕的PSDs比活动星系核(AGN)更平且能谱更软，揭示了TDE冕与AGN冕的本质差异及新形成吸积流的微观物理特性。


<details>
  <summary>Details</summary>

Background: 首次系统研究潮汐瓦解事件(TDEs)X射线辐射中分钟至小时量级的随机时变特性，揭示新形成吸积流的微观物理机制。

Data: 数据来源于XMM-Newton卫星对18个潮汐瓦解事件（TDEs）的54次观测，其中27次处于热吸积盘主导状态，27次显示存在非热硬X射线冕区（corona）。

Method: 采用XMM-牛顿(XMM-Newton)数据和傅里叶分析(Fourier analysis)方法对潮汐瓦解事件(TDEs)的X射线光度分钟至小时尺度随机变率进行首次系统性研究。

Result: 潮汐瓦解事件(TDEs)中具有冕区(corona)的X射线辐射比纯热源表现出更强的变异性及更陡的功率谱密度(PSDs)，表明其具有更长的相关时标。
...
### [34] [The spectral state transition of Mkn 590, a potential link between AGNs and X-ray binaries?](https://arxiv.org/abs/2602.17572)
*B. Palit,A. Rozanska,A. G. Markowitz,D. Lawther,M. Vestergaard,J. J. Ruan,T. Saha,G. Walsh,A. Borkar,M. Sniegowska,K. X. Lu*

**TL;DR**: 首次在单个活动星系核(AGN)中实时观测到塞弗特星系Markarian 590经历了类似X射线双星的完整光谱态转变，其X射线响度参数(α_ox)与爱丁顿比(λ_Edd)呈V型关系并在λ_Edd=0.021处出现转折，同时发现其吸积状态转变时间尺度(~10年)远低于经典薄盘粘滞时标但符合内盘热波传播模型，为跨越黑洞质量谱的尺度不变吸积物理提供了有力证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）中黑洞吸积（black hole accretion）的动态生命周期及其与X射线双星（X-ray binaries）的尺度不变性（scale-invariant）物理过程。

Data: 数据来源于Swift天文台长达十年的多波段观测数据，涵盖1975至2025年期间对塞弗特星系Markarian 590的多波长动态监测。

Method: 采用多波段（multi-band）光谱状态转换分析和硬度强度图（HID）追踪技术，结合爱丁顿比（Eddington ratio）与X射线响度参数（α_{ox}）的V型相关性建模，验证了热波前（thermal wavefronts）在薄吸积盘内传播的动力学过程。

Result: 首次在单个活动星系核(AGN)中实时观测到完整的谱态转变，从暗淡的硬X射线态转变为明亮的紫外和软X射线主导态。
...
### [35] [Stellar Paternity Tests: Matching High-Latitude B Stars to the Open Clusters of their Birth](https://arxiv.org/abs/2602.16925)
*Brandon Schweers,M. Virginia McSwain*

**TL;DR**: 通过盖亚(Gaia) DR3数据回溯39颗高银纬B型星和447个银河系疏散星团的运动轨迹，结合年龄匹配和核心环境分析，确定了5颗B型星的起源、抛射机制及运动参数，并发现一颗非盘起源的特殊天体。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高银纬B型星（OB stars）的起源及其与银河系薄盘中开放星团（open clusters）的动力学关联，探讨这些恒星被抛射的机制及历史。

Data: 数据来源为盖亚卫星第三次数据释放（Gaia Data Release 3, DR3）的高精度天体测量数据，包括39颗高银纬B型恒星和447个银河系疏散星团的运动学轨迹。

Method: method：基于盖亚卫星第三批数据（Gaia DR3）的高精度天体测量数据，采用运动学轨迹回溯方法并结合疏散星团的赫罗图（HR diagram）年龄匹配分析，通过核心环境分析推断抛射机制。

Result: 通过回溯39颗高银纬B型星（high-latitude B-type stars）和447个银河系疏散星团（Galactic open clusters）的运动轨迹并结合HR图年龄匹配，确定了其中5颗恒星可能的母星团来源，并推算了各自的传播时间、抛射速度及逃逸机制；另发现一颗恒星在分析时段内未接近银盘。
...
### [36] [Lithium as a probe of stellar and galactic physics](https://arxiv.org/abs/2602.17470)
*C. Charbonnel,N. Prantzos*

**TL;DR**: 该论文系统综述了锂元素(Li)在天体物理学中的独特作用，指出考虑化学元素和角动量传输机制的恒星演化模型能统一解释银河系各恒星族群的锂丰度模式（包括凹陷区和平台区），观测证据支持新星(novae)是银河系锂的主要来源，而银河系盘面恒星径向迁移是理解锂演化历史的关键。


<details>
  <summary>Details</summary>

Background: 锂（Lithium）在天体物理学中作为低质量恒星物理与演化、银河系考古学及宇宙学的关键诊断元素，其观测与理论研究构成了该研究的核心科学主题。

Data: 数据来源包括恒星演化模型（stellar evolution models）中化学元素和角动量的基本传输过程、新星（novae）的观测数据以及银河系盘内恒星径向迁移（radial migration）的观测证据。

Method: 采用恒星演化模型（stellar evolution models）结合化学元素和角动量的基本传输过程来解释银河系各恒星族群的锂丰度模式。

Result: 锂（Li）丰度模式可通过考虑化学元素和角动量基本传输过程的恒星演化模型统一解释所有银河系恒星族群的观测特征。
