...
**TL;DR**: 通过测试粒子模拟和N体模拟对比研究银河系垂直相空间螺旋（phase spirals）的演化特性，发现自洽N体模拟中螺旋的缠绕起始时间延迟、缠绕速率减慢且随时间振荡，这些效应与方位角作用量（Jφ）相关，表明银河系盘势演化会显著改变相空间螺旋的缠绕动力学，实际缠绕时间应视为理论下限值且外盘区域的测量结果更可靠。


<details>
  <summary>Details</summary>

Background: 银河系垂直相空间螺旋结构（vertical phase-space spirals）是动力学非平衡态（disequilibrium）的直接证据，但受多种动力学过程和非平衡源影响导致其演化机制复杂。

Data: 使用了两种模拟数据来源：一种是测试粒子（test particle）模拟，另一种是自洽（N-body）模拟。

Method: 采用基函数展开(basis function expansion)方法分析两个模拟（测试粒子模拟和N体模拟）中的相位螺旋，并推导缠绕时间($T_{\rm fit}$)。

Result: 在N体模拟（N-body simulation）中发现相螺旋（phase spiral）的缠绕过程存在延迟、减速及时变振荡现象，且这些效应受方位角作用量（azimuthal action）$J_φ$影响。
...
**TL;DR**: 通过化学丰度和轨道能量分析，发现半人马座ω星团（ω Cen）及其关联结构（Sequoia、Thamnos）源自同一被吸积的矮星系（ω Dwarf），其内区富集年轻Al-N-He族元素且α增强，外区呈现r过程主导的Eu富集，支持核区通过球状星团并合形成，而Sequoia和Thamnos符合由外至内的剥离序列。


<details>
  <summary>Details</summary>

Background: 研究探讨了半人马座ω星团（ω Cen）作为被吸积矮星系核星团的起源假说，并验证其与Sequoia、Thamnos和盖亚-恩克拉多斯（Gaia-Enceladus, GE）等结构共同来源于单一瓦解母体（ω Dwarf）的可能性。

Data: 数据来源包括APOGEE和GALAH丰度数据以及盖亚(Gaia)天体测量数据。

Method: 采用APOGEE和GALAH丰度数据结合盖亚天体测量学（Gaia astrometry），以轨道能量作为前并合半径的代理变量，重建了ω矮星系（ω Dwarf）的化学结构。

Result: ω Cen及其关联结构（Sequoia和Thamnos）源自同一被瓦解的矮星系（ω Dwarf），其化学演化群体集中于内部区域，支持了共同矮星系起源假说。
...
**TL;DR**: 研究发现星系团、子团和群的中心星系（central galaxies）大多表现出加速度增强现象，偏离孤立和非中心星系遵循的径向加速度关系（RAR），且偏离起始半径随系统质量增加而减小，暗示未探测物质需具备动态冷且无碰撞特性（如非重子冷暗物质或冷气体致密云团）。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系径向加速度关系（RAR）在星系团环境中的适用性差异，重点关注不同星系类型（孤立、非中心、中心及子团中心星系）对标准RAR的偏离现象及其潜在物理机制。

Data: 数据来源于对17个早型星系(early-type galaxies)的球状星团系统(globular cluster systems)进行Jeans建模得到的加速度剖面，样本包括星系团和星系群的中心星系、非中心星系、孤立星系以及正在被更大星系团吸积的星系亚团(subclusters)中心星系。

Method: 采用Jeans建模（Jeans modeling）分析球状星团系统（globular cluster systems）的加速度剖面（acceleration profiles），并将其与标准径向加速度关系（RAR）及其星系团对应关系进行对比。

Result: 中心星系（central galaxies）在星系团、子团（subclusters）和群组中大多表现出加速度增强现象，其偏离标准径向加速度关系（RAR）的起始半径随群组质量增加而减小。
...
**TL;DR**: 研究发现银河系疏散星团中恒星初始质量函数（IMF）并非普适，而是随分子云平均条件演化而变化，证实了环境依赖性理论预测。


<details>
  <summary>Details</summary>

Background: 研究主题为恒星初始质量函数（IMF）的非普适性及其与分子云环境演化的关联性。

Data: 使用近期来自盖亚卫星（Gaia）的观测数据，通过解析疏散星团中的恒星群并采用分离初始质量函数（IMF）特性与后续动力学演化的参数化方法。

Method: 采用基于盖亚(Gaia)观测数据的参数化方法，分离初始质量函数(IMF)特性与后续动力学演化效应，结合开放星团解析恒星种群分析。

Result: 银河系恒星初始质量函数（IMF）并非普适，而是随不同恒星群体变化，反映了分子云平均条件随宇宙时间的演化。
...
**TL;DR**: 通过N体模拟和玩具模型研究发现，银河系(MW)盘面破坏(disruption)程度显著影响卫星星系数量及分布：高抗破坏性导致卫星数量少且径向集中，低抗破坏性则卫星数量多且分布分散，且大质量子晕(subhalo)因径向轨道更易被破坏。


<details>
  <summary>Details</summary>

Background: 研究探讨了银河系（MW）卫星星系数量估计中盘状结构（stellar disc）对卫星星系动力学扰动（disruption）的影响及其对卫星星系空间分布和宿主暗晕（subhalo）质量分布的预测差异。

Data: 研究中使用的数据来源包括虚构的全天巡天（fictional all-sky survey）以及Pan-STARRS和DES巡天（Pan-STARRS and DES surveys）的实际观测数据。

Method: 采用N体模拟（N-body simulation）结合银河系盘破坏（disc disruption）的简化模型（toy model）分析不同破坏程度对卫星数量推断的影响。

Result: 高弹性（high resilience）盘面扰动模型预测卫星数量少且径向分布高度集中，而低弹性（low resilience）模型预测卫星数量多且径向分布分散。
...
**TL;DR**: CLONE项目通过350 pc高分辨率的流体力学模拟复现了室女座星系团（Virgo cluster），发现模拟星系在恒星形成密度、金属丰度（metallicity）和淬灭比例（quenched fraction）等观测关系上与实际数据高度吻合，证实气体耗竭是低质量星系淬灭的主因，且该模拟能有效研究从水母星系（jellyfish galaxies）到星系团核心气体动力学的演化过程。


<details>
  <summary>Details</summary>

Background: 星系团为研究致密环境中星系演化提供了强大实验室，聚焦于环境对星系形成与演化的影响。

Data: data部分：数据来源于CLONE项目提供的针对Virgo星系团的zoom-in流体动力学模拟（zoom-in hydrodynamical simulation），分辨率达350 pc，包含活动星系核（AGN）和超新星反馈机制。

Method: 采用高分辨率(350 pc)的流体动力学(zoom-in hydrodynamical)模拟技术，包含活动星系核(AGN)和超新星反馈机制，构建了与室女座星系团(Virgo cluster)观测属性匹配的数值模型(CLONE项目)。

Result: 模拟星系群在恒星形成密度、金属丰度及淬灭比例等方面与观测趋势一致，证实了气体耗竭是低质量星系淬灭的主要机制。
...
**TL;DR**: 研究发现，在红移(redshift)z=2.41的致密环境(HATLAS J084933.4+021443)中，至少42%的气体丰富大质量星暴星系(starburst galaxies)为旋转主导的盘状星系(rotation-dominated discs)，其旋转速度超过400 km/s，且与z=0的巨椭圆星系(giant ellipticals)具有相似的晕质量(halo mass)，表明早期质量聚集主要发生在致密环境中。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（$z\gtrsim2$）致密环境中星暴星系（starburst galaxies）的起源及其与现今大质量椭圆星系（massive ellipticals）的演化关联。

Data: 使用0.15角秒分辨率的CO(4-3)、[C I] 1-0和尘埃连续谱观测数据（ALMA观测），分析了HATLAS J084933.4+021443星系团中五个富气体星系（红移z=2.41）的冷气体形态和运动学特征。

Method: 采用0.15角秒分辨率的CO(4-3)、[C I] 1-0和尘埃连续谱观测，表征了冷气体形态和运动学特征。

Result: 在$z\gtrsim2$高密度环境中，$\gtrsim42\%$的气体丰富、大质量星暴星系是旋转主导的盘状星系（rotation-dominated discs），其旋转速度超过400 km s$^{-1}$，且几乎仅存在于高密度区域，表明早期质量聚集发生在致密环境中。
...
**TL;DR**: 基于SPHEREx望远镜初步数据绘制了银河系多环芳烃（PAHs）和电离氢（H II）大尺度分布图，发现PAHs丰度与电离氢区域呈显著负相关，证实电离辐射是PAHs丰度变化的主要驱动因素。


<details>
  <summary>Details</summary>

Background: 研究主题为利用SPHEREx空间望远镜绘制银河系多环芳烃（PAHs）和电离氢气的弥散发射分布图并分析其相互关系。

Data: 数据来源于SPHEREx空间望远镜2025年发射后进行的全天空近红外光谱巡天（0.75-5.0微米波段），包括3.3微米多环芳烃（PAHs）发射特征和4.05微米Brackett-alpha发射的大尺度分布图。

Method: 采用近红外光谱巡天（0.75-5.0微米）数据，通过3.3微米多环芳烃（PAHs）发射特征和4.05微米布拉克特-α（Brackett-alpha）发射线绘制大尺度分布图，结合普朗克热尘埃辐射测量，分析多环芳烃与电离氢气的相关性及丰度变化。

Result: SPHEREx初步观测发现多环芳烃(PAH)丰度与电离氢气体呈显著负相关，表明电离辐射是银河系平面内PAH丰度变化的主要驱动因素。
...
**TL;DR**: 首次在M33中测得光谱紫外(UV)消光曲线，发现其UV形状参数与气尘比(gas-to-dust ratio)的相关性强于整体金属丰度，表明辐射场密度和激波等局部条件比星系整体性质更能决定尘埃消光的波长依赖性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过尘埃消光曲线(dust extinction curves)理解尘埃颗粒的微观性质及其对天体物理观测的影响，并探索局域环境因素（如辐射场密度和激波）相较于星系整体属性（如金属丰度）对尘埃消光波长依赖性的主导作用。

Data: 数据来源包括哈勃空间望远镜（Hubble Space Telescope）/空间望远镜成像摄谱仪（Space Telescope Imaging Spectrograph）的光谱数据以及从紫外到近红外的文献测光数据。

Method: 基于哈勃太空望远镜/空间望远镜成像摄谱仪（HST/STIS）光谱和紫外至近红外波段文献测光数据，采用光谱紫外消光曲线测量技术分析M33星系尘埃特性。

Result: 首次在M33中测量了光谱紫外（UV）消光曲线，将此类测量的本星系群星系样本扩展到五个。
...
### [10] [MICONIC: JWST/MIRI-MRS reveals heavily reprocessed PAH emission in the circum-nuclear disc of Centaurus A](https://arxiv.org/abs/2603.23674)
...
**TL;DR**: 基于JWST/MIRI-MRS观测发现半人马座A中心区域存在环状分布的多环芳烃（PAHs）结构，其11.3/7.7 μm和6.2/7.7 μm比值表明PAHs具有开放结构，且在喷流轴垂直方向的PAHs缺失区因激波侵蚀导致部分脱氢和破坏。


<details>
  <summary>Details</summary>

Background: 多环芳烃（PAHs）作为星系尘埃的关键组分，其与活动星系核（AGN）反馈的相互作用机制是星际介质（ISM）物理研究的核心争议问题。

Data: 数据来源于JWST/MIRI-MRS在5-28 μm波段的观测，来自MIRI欧洲联盟GTO项目MICONIC，角分辨率为0.35′′-1′′（约6-17 pc）。

Method: 采用JWST/MIRI-MRS观测数据（5-28 μm）进行局部连续谱扣除生成PAH矩-0图，并从五个感兴趣区域提取一维光谱，通过分解连续谱、发射线和PAHs测量特征强度和等效宽度（EWs）。

Result: PAHs（多环芳烃）在距核约40 pc处形成环状分布，西北侧存在与喷流轴垂直的PAH缺失区，该区域与电离气体速度弥散增强及分子流入流重合，且PAH结构比模型预测更开放（11.3/7.7 μm和6.2/7.7 μm比值偏高），11.3/12.7 μm比值表明氢位点以孤立态为主并存在部分脱氢，缺失区的PAH可能受激波侵蚀而部分破坏，核区低等效宽度主要由连续谱稀释导致。
...
### [11] [Benchmarking Astrochemistry Paradigms: Relative Absence of $\rm{C_6H_5CN^+}$ in the Diffuse ISM](https://arxiv.org/abs/2603.23739)
*Daniel Majaess,Cercis Morera-Boado,Tina A. Harriott,Quazi Rahi,Halis Seuret,Lou Massa,Cherif F. Matta*

**TL;DR**: 重新评估了星际弥散介质（diffuse ISM）中苯甲腈阳离子（$\rm{C_6H_5CN^+}$）的可探测性，发现其因分子内振动能量再分配（IVR）倾向碎裂、缺乏有效荧光机制防止解离、观测与实验谱线不匹配及类似尺寸阳离子未对应任何弥散星际带（DIBs）而基本不存在。


<details>
  <summary>Details</summary>

Background: 研究重新评估了苯甲腈阳离子（$\rm{C_6H_5CN^+}$）在弥散星际介质（diffuse ISM）中的可探测性，并探讨了其在星际化学合成中的缺失原因及对星际化学范式的潜在影响。

Data: 研究中使用的数据来源包括Daly等人的新实验光谱、扩展的观测DIB分析（APO目录）以及互补的$ω$B97X-D/cc-pVTZ计算。

Method: 采用Daly等人实验光谱、扩展的观测DIB（弥漫星际带）分析（APO目录）及ωB97X-D/cc-pVTZ计算作为核心分析手段。

Result: 在弥散星际介质（diffuse ISM）中，苯甲腈阳离子（$\rm{C_6H_5CN^+}$）的探测可能性被重新评估，其相对缺失归因于分子内振动能量再分配（IVR）倾向碎裂、复现荧光机制难以阻止解离、观测到的弥散星际吸收带（DIBs）与实验结果不匹配，以及迄今未发现类似尺寸阳离子对应的DIBs。
...
### [12] [Estimation of the magnetic field strength from ALMA dust polarization in the protocluster G327.29](https://arxiv.org/abs/2603.23747)
*A. Koley,P. Sanhueza,A. M. Stutz,P. Saha,F. A. Olguin,A. Ginsburg,N. Sandoval-Garrido,N. Castro-Toledo*

**TL;DR**: 利用ALMA高分辨率观测发现，大质量恒星形成区G327.29的磁场（B_tot）强度（1.4-2.0 mG）动态重要性超过湍流（能量比~0.25），且磁场方向呈全角度（-90至+90度）均匀分布，其与气体密度（ρ）的标度关系决定了湍流和磁场对致密核动力学的相对影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于分子云演化中磁场（magnetic fields）和湍流（turbulence）对高质量恒星形成（high-mass star formation）的调控作用及其相对重要性。

Data: 数据来源于阿塔卡马大型毫米波/亚毫米波阵列(ALMA)的12米望远镜对高质量恒星形成区G327.29的高分辨率面内磁场(B_pos)形态观测。

Method: 采用角分散函数法（ADF）分析高分辨率ALMA观测数据，结合湍流与磁能比值评估磁场动力学重要性。

Result: 在高分辨率ALMA观测下，G327.29区域的磁场(B_tot)强度在丝状结构和致密团块中分别为1.4±0.7 mG和2.0±0.8 mG，且湍流与磁能比(~0.25)表明磁场动力学作用强于湍流。
...
### [13] [The shortest detected intra-day variability of active galactic nuclei in TESS survey](https://arxiv.org/abs/2603.23752)
*Heechan Yuk,Xinyu Dai,Natalie Kovacevic*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016422485e41e59681f633b1b8d63fc66f6f148474ff1b', 'param': '', 'type': 'Forbidden'}}
...
### [14] [Rigorous Formulation of Finite-Sample and Finite-Window Effects in Galaxy Clustering](https://arxiv.org/abs/2603.23872)
*Tsutomu T. Takeuchi,Satoshi Kuriki,Keisuke Yano*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016450205e41e59681f633b1b8d63fc66f6f14844d102a', 'param': '', 'type': 'Forbidden'}}
...
### [15] [A Long Stellar Stream in M83: Possible Connection Between XUV Disks and Minor Mergers?](https://arxiv.org/abs/2603.23981)
*Itsuki Ogami,Sakurako Okamoto,Annette M. N. Ferguson,Yutaka Komiyama,Masashi Chiba,Jin Koda,Kohei Hayashi,Yoshihisa Suzuki*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016464245e41e59681f633b1b8d63fc66f6f1484799e07', 'param': '', 'type': 'Forbidden'}}
...
### [16] [GalSyn I: A Forward-Modeling Framework for Synthetic Galaxy Observations from Hydrodynamical Simulations and First Data Release from IllustrisTNG](https://arxiv.org/abs/2603.23986)
...
**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016484535e41e59681f633b1b8d63fc66f6f1484351706', 'param': '', 'type': 'Forbidden'}}
...
### [17] [Imprints of tidal interactions on the stellar distribution of satellite galaxies: implications for dark matter deficient galaxies](https://arxiv.org/abs/2603.24020)
*Zhihao Yin,Go Ogiya,Frank C. van den Bosch*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016491045e41e59681f633b1b8d63fc66f6f14845fa959', 'param': '', 'type': 'Forbidden'}}
...
### [18] [Machine Learning-Based Classification of Active Galaxies and Estimation of Supermassive Black Hole Masses](https://arxiv.org/abs/2603.24435)
*Farideh Mazoochi,Reihaneh Karimi,Mohammad Hossein Zhoolideh Haghighi,Fatemeh Tabatabaei*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016497645e41e59681f633b1b8d63fc66f6f148412b2fd', 'param': '', 'type': 'Forbidden'}}
...
### [19] [Massive star clusters detected by JWST as natural birth places to form intermediate-mass black holes](https://arxiv.org/abs/2603.24550)
...
**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016508805e41e59681f633b1b8d63fc66f6f1484b217fe', 'param': '', 'type': 'Forbidden'}}
...
### [20] [Derivation of the Kompaneets equation using the boost operator approach](https://arxiv.org/abs/2603.23572)
*Alex Hoey,Jacob Long,Jens Chluba*

**TL;DR**: 采用升压算子方法透明推导了各向同性及各向异性介质中光子与热电子散射的广义Kompaneets方程，验证了高阶温度修正表达式并拓展了该形式体系在辐射转移问题中的应用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低温下光子与热电子重复散射的动力学过程及其在辐射传输问题中的应用。

Data: 研究中使用的数据来源为电子静止坐标系（electron rest frame）中的散射算子表达式，考虑了电子反冲的一阶项O(hν/m_e c^2)并在所有电子动量p阶数下进行计算。

Method: 采用升算符方法（boost operator approach）推导各向异性光子场中的康帕涅茨方程（Kompaneets equation）及其高阶温度修正项，并在电子静止坐标系中考虑一级反冲效应（O(hν/m_e c^2)）的同时保留电子动量（p）的全阶贡献。

Result: 采用升算符方法（boost operator approach）透明地推导了各向同性介质中康帕涅茨方程（Kompaneets equation）及其含各向异性和高阶温度修正的广义形式，验证了该形式主义在辐射传输问题中的普适性。
...
### [21] [Investigating the radio emission in the Perseus cluster with LOFAR sub-80 MHz LBA](https://arxiv.org/abs/2603.23587)
...
**TL;DR**: 利用LOFAR低频天线(LBA)在30.0-57.7 MHz观测英仙座星系团(Perseus cluster)，首次同时探测到1.1 Mpc巨型射电晕(giant radio halo)和中央迷你射电晕(mini-halo)，测得二者在44-144 MHz频谱指数分别为-1.01±0.11和-1.34±0.10，并发现X射线"幽灵空腔"(ghost cavities)内老化射电等离子体频谱指数(-1.86±0.12和-1.90±0.12)及尾部射电星系NGC 1265和IC 310沿尾部频谱变陡现象。


<details>
  <summary>Details</summary>

Background: 研究聚焦于英仙座星系团（Perseus cluster）中射电小晕（mini-halo）和巨型射电晕（giant radio halo）的频谱特性及其起源，同时探讨X射线“幽灵空腔”（ghost cavities）内老化射电等离子体的演化特征。

Data: 数据来源于低频阵列（LOFAR）低波段天线（LBA）在30.0-57.7 MHz的观测图像以及甚大天线阵（VLA）352 MHz的补充数据。

Method: 采用低频阵列（LOFAR）低波段天线（LBA）在30.0-57.7 MHz频段对英仙座星系团进行成像观测，分辨率达19.2'' x 15.0''，噪声水平为3.7 mJy/beam，并结合甚大阵（VLA）352 MHz数据开展频谱分析。

Result: 在英仙座星系团（Perseus cluster）中探测到迷你射电晕（mini-halo）和巨型射电晕（giant radio halo）的频谱指数分别为-1.34 ± 0.10和-1.01 ± 0.11，而直接测量巨型射电晕的频谱指数为-1.28 ± 0.15，差异由电离层条件不佳导致。
...
### [22] [Reducing cosmological degeneracies by combining multiple classes of LISA gravitational-wave standard sirens](https://arxiv.org/abs/2603.23612)
...
**TL;DR**: 首次联合LISA探测的极低质量比旋近体（extreme mass-ratio inspirals）和超大质量双黑洞（massive black hole binaries）作为标准汽笛（standard sirens），通过互补红移范围降低了宇宙学参数简并性，显著提升了哈勃常数（Hubble constant）和暗能量状态方程参数（dark-energy equation-of-state parameter）的约束精度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用LISA（激光干涉空间天线）探测极端质量比旋近体（EMRIs）和大质量黑洞双星（MBHBs）作为标准汽笛（standard sirens），通过联合分析不同红移范围的引力波信号来约束哈勃常数（H_0）和暗能量状态方程参数（w_0），以解决宇宙学参数简并性问题并验证LISA在晚期宇宙学中的独特系统学优势。

Data: 数据来源为LISA（Laser Interferometer Space Antenna）探测的极端质量比旋进源（extreme mass-ratio inspirals）在红移$z\lesssim1$的引力波信号和大质量黑洞双星（massive black hole binaries）在红移$z\gtrsim1$的电磁对应体（electromagnetic counterparts）信号。

Method: 采用联合引力波（gravitational-wave）标准汽笛（standard sirens）方法，结合LISA（Laser Interferometer Space Antenna）对极端质量比旋近（extreme mass-ratio inspirals）和大质量黑洞双星（massive black hole binaries）的观测数据。

Result: 首次联合利用LISA（激光干涉空间天线）探测的极端质量比旋进源（EMRI）和中等红移大质量黑洞双星作为标准汽笛（standard siren），在降低宇宙学参数简并性的同时，对哈勃常数（H0）和暗能量状态方程参数（w0）给出了具有竞争力的约束。
...
### [23] [A velocity-dependent two-scale model for cosmic string networks with small-scale structure](https://arxiv.org/abs/2603.23619)
*Teresa O. Miranda,Lara Sousa*

**TL;DR**: 通过扩展速度依赖单尺度模型引入描述典型扭结间距的额外长度尺度，建立了包含小尺度结构的宇宙弦网络半解析模型，发现小尺度结构虽不阻碍线性标度态达成但会显著降低能量密度和均方根速度，且引力反作用主导时网络会经历类似无小尺度结构的准标度态过渡阶段。


<details>
  <summary>Details</summary>

Background: 研究宇宙弦（cosmic strings）网络中具有小尺度结构的宇宙学演化及其对线性标度（linear scaling）机制的影响。

Data: 研究采用半解析模型（semi-analytical model）扩展速度依赖单尺度模型（velocity-dependent one-scale model），引入描述典型扭结间距（interkink density）的额外长度尺度，通过理论模拟分析宇宙弦网络（cosmic strings）的小尺度结构演化。

Method: 采用速度依赖双尺度模型（velocity-dependent two-scale model）扩展速度依赖单尺度模型（velocity-dependent one-scale model），引入描述典型扭结间距（interkink density）的额外长度尺度。

Result: 通过引入描述典型扭结间距密度的额外长度尺度，扩展速度依赖单尺度模型，建立了描述具有小尺度结构的宇宙弦网络宇宙学演化的半解析模型。
...
### [24] [Brightest Cluster Galaxy ellipticity as proxy for halo shape: Orientation bias, assembly bias, and potential selection effects in SZ-selected clusters](https://arxiv.org/abs/2603.23689)
...
**TL;DR**: 利用中央最亮星系（BCG）形状作为三轴星系团视线方向（line-of-sight）的代理，首次观测到圆形BCG星系团的光学富度（richness）比椭圆BCG星系团高约10%，但弱透镜和星系密度剖面的1晕（1-halo）项在两组间无差异，而2晕（2-halo）项中椭圆BCG星系团剖面显著高于圆形BCG星系团，与数值模拟预期相反，表明BCG形状可能同时反映团簇取向和影响SZ信号及2晕项幅度的固有属性。


<details>
  <summary>Details</summary>

Background: 研究探讨了三轴星系团（triaxial galaxy clusters）沿视线方向（line-of-sight）的取向对光学观测量（如富度richness和弱引力透镜信号weak-lensing signal）的散射和潜在偏差的影响。

Data: 数据来源于南极望远镜（SPT）和阿塔卡马宇宙学望远镜（ACT）巡天的SZ效应选星系团，并与暗能量巡天第三年（DES）光学选星系团匹配。

Method: 利用中央最亮团星系（BCG）的观测形状作为视线方向（line-of-sight）取向的代理指标，构建了具有相同SZ质量估计和红移但BCG形状最圆与最椭圆的样本进行对比分析。

Result: 中央亮星系（BCG）的形状取向与视线方向的关系导致圆形BCG星系团的光学丰度比椭圆BCG星系团高约10%，但一晕区密度轮廓无差异，而二晕区椭圆BCG星系团轮廓显著高于圆形BCG星系团，与数值模拟预期相反。
...
### [25] [Perturbative and numerical study of nonlinear relativistic effects in weak lensing](https://arxiv.org/abs/2603.24179)
*Matteo Magi,Francesca Lepori,Julian Adamek*

**TL;DR**: 研究发现标准弱引力透镜（weak lensing）理论在二阶相对论性微扰下不完整，通过雅可比映射（Jacobi map）形式推导了旋转场、剪切B模（shear B-modes）及其角功率谱，证明萨克斯基（Sachs basis）平行传输会打破谱线简并，数值计算显示红移$z_\mathrm{s}=0.5$源在大尺度$\ell\sim5$处存在约$5\%$差异，并首次解析推导了参考系拖拽（frame-dragging）对剪切B模功率谱的显著影响（$\ell\lesssim10$为主导项），但二者对星系椭圆率观测影响仅约$1\%$。


<details>
  <summary>Details</summary>

Background: 研究探讨了标准弱引力透镜（weak lensing）理论在超出线性扰动理论时的局限性，重点关注了二阶相对论性扰动下旋转场（rotation field）、剪切B模式（shear B-modes）及其角功率谱（angular power spectra）的修正效应。

Data: data部分：研究使用了相对论性弱引力透镜模拟（relativistic simulations of weak lensing observables）数据，包括首次对拖曳效应（frame dragging）在透镜收敛（lensing convergence）和宇宙剪切（cosmic shear）功率谱中的数值研究。

Method: 采用雅可比映射（Jacobi map）形式体系，在相对论性二阶微扰理论中推导了旋转场、剪切B模及其角功率谱的表达式，并数值量化了萨克斯基（Sachs basis）平行传输导致的谱差异。

Result: 在二阶相对论微扰理论中，标准弱引力透镜（weak lensing）形式体系的不完整性被揭示，Sachs基的平行输运导致旋转场与剪切B模（shear B-modes）的谱退化被打破，数值计算显示在红移$z_\mathrm{s} = 0.5$、大角尺度$\ell \sim 5$上存在约$5\%$的差异。
...
### [26] [Impact of Antenna Structure and Orientation on Forward-Modelled Global 21 cm Signal Recovery](https://arxiv.org/abs/2603.24378)
*Joe H. N. Pattison,John M. Cumner,Dominic J. Anstey,Saurabh Pegwal,Wessel Croukamp,Dirk I. L. de Villiers,Eloy de Lera Acedo*

**TL;DR**: 研究发现天线建模误差（beam mismodelling）会导致宇宙氢原子21厘米吸收信号（21 cm absorption）参数推断偏差，其中0.25度的指向误差（orientation mismatches）即可造成显著偏差，但贝叶斯证据（Bayesian evidence）可在此精度内校正指向误差，而结构建模误差（structural mismodelling）中信号频率和宽度在有限频带内仍可稳健恢复，但信号深度对波束误差高度敏感。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于宇宙早期中性氢红移21厘米吸收线探测中天线建模误差对信号恢复的影响及其参数推断偏差问题。

Data: 研究中使用的数据来源于模拟（simulation）的21厘米吸收信号和天线波束模型，通过对比模拟与建模天线波束的差异来评估参数恢复的准确性。

Method: 采用基于贝叶斯统计的前向建模（forward-modelled）数据分析流程，通过扫描天线方向角（orientation）和结构（structural）的模型实现来量化天线建模误差对信号恢复的影响。

Result: 天线方向偏差（orientation mismatches）达到0.25度时会在典型观测场景中显著偏置信号参数恢复，但贝叶斯证据可通过扫描模型实现该精度内的方向推断。
...
### [27] [Probing Interacting Dark Sectors with upcoming Post-Reionization and Galaxy Surveys](https://arxiv.org/abs/2603.24554)
*Rahul Shah,Antara Dey,Purba Mukherjee,Supratik Pal*

**TL;DR**: 研究了未来再电离后时期和星系巡天对动态暗能量（dynamical dark energy）与暗物质（dark matter）相互作用的约束能力，发现SKA-mid和Euclid观测能显著提升对相互作用强度和暗能量状态方程参数的约束精度，其中SKA2对相互作用强度的约束最优，Euclid精度与SKA1相当。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用后再电离时期（post-reionization）和星系巡天数据约束动力学暗能量（dynamical dark energy）与暗物质（dark matter）之间可能存在的相互作用。

Data: 数据来源包括SKA-mid的21厘米强度映射、星系成团和宇宙剪切观测模拟数据，以及欧几里得(Euclid)任务的大尺度巡天模拟数据，均基于普朗克(Planck)2018、DESI DR2和Pantheon+的当前观测界限基准值生成。

Method: 采用马尔可夫链蒙特卡洛（Markov chain Monte Carlo）方法结合当前观测数据与模拟数据集（SKA-mid 21厘米强度映射、星系成团性、宇宙剪切及Euclid大尺度巡天）进行参数约束预测。

Result: 结果表明，SKA-mid和Euclid观测能显著提升对暗物质与动力学暗能量（dynamical dark energy）相互作用参数的约束精度，特别是相互作用强度和暗能量状态方程参数（dark energy equation of state parameters）的约束将明显优于当前Planck 2018、DESI DR2和Pantheon+的联合约束。
...
### [28] [Dynamical Systems in Cosmology: Reviewing An Alternative Approach](https://arxiv.org/abs/2603.24568)
*Nandan Roy,L. Arturo Ureña-López*

**TL;DR**: 这篇论文的核心贡献是通过引入极坐标和双曲变量变换的动力学系统方法，系统分析了标量场暗能量（scalar field dark energy）模型的演化行为、初始条件约束及各类势能下的追踪特性（tracking behavior）。


<details>
  <summary>Details</summary>

Background: 暗能量（dark energy）是现代宇宙学中最深刻的谜题之一，其本质可能超越宇宙学常数（cosmological constant）而具有动力学特性。

Data: 数据来源为理论推导的常微分方程系统（systems of ordinary differential equations）及基于极坐标和双曲变量变换的替代公式。

Method: 采用核心分析方法为动力系统理论（dynamical systems theory）结合极坐标和双曲变量变换（polar and hyperbolic variable transformations）来研究标量场暗能量模型（scalar field dark energy models）的演化行为。

Result: 动力系统方法为揭示宇宙演化规律提供了关键工具，通过极坐标和双曲变量变换等新方法可有效研究标量场暗能量（scalar field dark energy）模型并分析势能函数的追踪行为。
...
### [29] [From friction scaling to an efficient method for estimating bubble wall velocity](https://arxiv.org/abs/2603.24583)
*Tomasz Krajewski,Marek Lewicki,Marco Merchand,Ignacy Nałęcz,Mateusz Zych*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016441525e41e59681f633b1b8d63fc66f6f14848febee', 'param': '', 'type': 'Forbidden'}}
...
### [30] [Orbital Debris in Earth Orbit: Operations, Stability, Control, and Market Formation](https://arxiv.org/abs/2603.23552)
*Slava G. Turyshev*

**TL;DR**: 研究揭示了近地轨道(LEO)碎片问题本质上是受壳体占用率(shell occupancy)、碰撞核(collision kernel)、解体严重性(breakup severity)和轨道驻留时间(orbital residence time)共同调控的动态系统，提出通过提升新发射航天器的处置可靠性、降低高风险交会(conjunction)状态不确定性及清理遗留高质量失效物体三大杠杆实现轨道可持续性管理，并指出碎片治理将分化为合规减缓、寿命末期服务和遗留物修复三个关联市场。


<details>
  <summary>Details</summary>

Background: 地球轨道上的轨道碎片（orbital debris）问题是一个涉及壳体占用率（shell occupancy）、碰撞核（collision kernel）、解体严重性（breakup severity）和轨道驻留时间（orbital residence time）的耦合操作-稳定性问题。

Data: 数据来源包括截至2026年的公开ESA、NASA、FCC、NOAA、JAXA和OECD数据。

Method: 采用降阶控制框架（reduced-order control framework）进行干预排序和市场形成分析。

Result: 近地轨道(LEO)环境存在交通密度峰(500-600公里)与风险峰(850公里)的空间分离，其中96%的轨道物体为失效对象，长期危害主要由高轨道寿命/大质量失效物体主导。
...
### [31] [The Vera C. Rubin Observatory Data Preview 1](https://arxiv.org/abs/2603.23786)
...
**TL;DR**: 首次发布鲁宾天文台(Rubin Observatory)的DP1数据集，包含2024年底在智利帕琼山(Summit Facility)使用西蒙尼巡天望远镜(Simonyi Survey Telescope)上的鲁宾调试相机(LSSTComCam)获取的1792幅光学-近红外曝光数据，覆盖7个非连续天区约15平方度，提供六波段(ugrizy)原始与校准图像、叠加图像、差分图像、探测星表及辅助数据产品，其中最深天区(Extended Chandra Deep Field South)的5σ点源极限深度达u=24.55至y=23.1等，共包含230万天体(含160万延展源和431个太阳系天体)，支持2026年正式运行前的多领域早期科学研究。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用鲁宾天文台(Rubin Observatory)的首批数据(DP1)开展多波段光学-近红外巡天观测，为2026年正式运行的时空遗产巡天(LSST)提供早期科学验证平台。

Data: 数据来源于鲁宾天文台（Rubin Observatory）调试相机LSSTComCam在2024年底于智利帕琼山（Cerro Pachón）峰顶设施西蒙尼巡天望远镜（Simonyi Survey Telescope）上48个独立夜晚获取的1792次光学近红外（optical near-infrared）曝光。

Method: 采用鲁宾天文台(Rubin Observatory)的调试相机(LSSTComCam)获取的光学-近红外波段(u,g,r,i,z,y)单次曝光图像，通过叠加(coadds)、差分(difference images)处理和检测目录生成技术构建多波段天体物理对象数据集。

Result: 鲁宾数据预览1（DP1）首次提供了来自维拉·C·鲁宾天文台（Vera C Rubin Observatory）的高质量多样化数据，包括原始和校准的单历元图像、叠加图像、差异图像、检测目录及辅助数据产品，支持2026年全面运行前的广泛早期科学研究。
...
### [32] [A transdimensional sampling framework for pulsar timing noise modelling](https://arxiv.org/abs/2603.23817)
*Valentina Di Marco,Nir Guttman,Matthew T. Miles,Andrew Zic,Ryan M. Shannon,Eric Thrane*

**TL;DR**: 开发了基于Bilby库的跨维度贝叶斯推断框架tPTABilby，能自适应识别脉冲星计时噪声源（如辐射计噪声、相位抖动等）并统一处理模型选择与参数估计，在PSR J1713+0747的MeerKAT数据验证中表现出与传统方法一致但更稳健的统计性能。


<details>
  <summary>Details</summary>

Background: 研究聚焦于脉冲星计时阵列（PTA）实验中单颗脉冲星计时数据噪声过程的精确表征，以支持引力波探测和天体物理研究。

Data: 数据来源包括模拟数据和来自MeerKAT脉冲星计时阵（MPTA）的PSR J1713+0747实测数据。

Method: 采用基于Bilby库的跨维度(transdimensional)贝叶斯推断框架tPTABilby，通过灵活建模射电噪声(radiometer noise)、脉冲相位抖动(pulse-phase jitter)、本征红噪声(intrinsic red noise)、色散量变化(dispersion measure variations)和星际介质色散效应(chromatic interstellar medium effects)等噪声过程，实现噪声源数量与类型的联合推断。

Result: 开发了基于Bilby库的跨维度贝叶斯推断框架tPTABilby，可灵活建模多种脉冲星噪声过程并实现模型选择与参数估计的统一处理。
...
### [33] [SM-Net: Learning a Continuous Spectral Manifold from Multiple Stellar Libraries](https://arxiv.org/abs/2603.23899)
*Omar Anwar,Aaron S. G. Robotham,Luca Cortese,Kevin Vinsen*

**TL;DR**: SM-Net通过融合多个高分辨率恒星光谱库构建了覆盖有效温度(Teff)2000-190000K、表面重力(log g)-1至9、金属丰度(log Z)-4至1的连续参数空间光谱流形，实现了基于基本恒星参数的快速光谱生成与平滑插值。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用机器学习模型（SM-Net）从多源高分辨率恒星光谱库中学习连续光谱流形，以基于基本恒星参数（有效温度、表面重力和金属丰度）直接生成恒星光谱。

Data: 数据来源于PHOENIX-Husser、C3K-Conroy、OB-PoWR和TMAP-Werner光谱库的组合网格，覆盖有效温度(Teff)2000-190000 K、表面重力(log g)-1至9、金属丰度(log Z)-4至1的连续参数空间，光谱波长范围3000-100000埃。

Method: 采用机器学习模型SM-Net从有效温度(Teff)、表面重力(log g)和金属丰度(log Z)等基本恒星参数直接生成恒星光谱，并在PHOENIX-Husser、C3K-Conroy、OB-PoWR和TMAP-Werner光谱库组合的数据网格上进行训练。

Result: SM-Net通过学习多个高分辨率恒星光谱库构建了一个连续的光谱流形(spectral manifold)，能够直接从基本恒星参数有效温度(Teff)、表面重力(log g)和金属丰度(log Z)生成恒星光谱。
...
### [34] [Ising noise filter: physics-informed filtering for particle detectors](https://arxiv.org/abs/2603.24075)
*I. Kharuk*

**TL;DR**: 提出基于伊辛模型(Ising model)的图论滤波算法，通过将探测器信号映射为自旋网络并最小化能量泛函，在贝加尔湖中微子望远镜(Baikal-GVD)和NICA对撞机SPD探测器上分别实现96.8%和97%的噪声抑制召回率，结合彼得森-霍普菲尔德网络(Peterson-Hopfield network)使TrackML评分从0.5提升至0.95。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于粒子加速器和天体物理探测器早期背景抑制中的高效噪声过滤问题。

Data: 数据来源于贝加尔湖中微子望远镜（Baikal-GVD）的实测数据以及NICA对撞机SPD探测器的玩具蒙特卡洛（toy Monte Carlo）样本。

Method: 采用基于图论的伊辛噪声滤波器（Ising noise filter），通过将探测器命中映射到二元自旋网络并最小化能量泛函，结合物理信息交互核（physics-informed interaction kernels）实现噪声抑制。

Result: 提出的伊辛噪声滤波器（Ising noise filter）通过将探测器命中映射到二元自旋网络并最小化能量泛函，有效规避了传统轨迹拟合方法中的组合爆炸问题。
...
### [35] [Refractive multi-conjugate adaptive optics for wide-field atmospheric turbulence correction](https://arxiv.org/abs/2603.24174)
*Tommaso Furieri,Stefano Bonora*

**TL;DR**: 提出了一种利用新型多驱动器可变形透镜(DL)的折射式多共轭自适应光学系统(MCAO)，在中等湍流条件下将等晕区扩大至未校正大气等晕角的三倍，并验证了其在紧凑型可移动自由空间光通信系统中的双通道单模光纤耦合效率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用折射式多共轭自适应光学系统（MCAO）提升天文成像和自由空间光通信的校正视场（FoV），尤其针对小型可移动系统。

Data: data：研究使用夏克-哈特曼波前传感器（SH-WFS）结合两个共轭于不同大气层的多驱动器可变形透镜（DL）进行波前校正，并在模拟自由空间光通信的紧凑可移动系统中测试了MCAO系统性能。

Method: 采用折射多共轭自适应光学（Refractive-MCAO）系统，结合新型多驱动器可变形透镜（DL）和沙克-哈特曼波前传感器（SH-WFS），实现波前校正。

Result: 采用折射式多共轭自适应光学系统（Refractive-MCAO）结合新型多驱动器可变形透镜（DL），在中等湍流条件下（D/r0=2）将等晕区扩展至未校正大气等晕角的三倍。
...
### [36] [Short-Term Turbulence Prediction for Seeing Using Machine Learning](https://arxiv.org/abs/2603.24466)
*Mary Joe Medlej,Rahul Srinivasan,Simon Prunet,Aziz Ziad,Christophe Giordano*

**TL;DR**: 利用机器学习模型（包括高斯过程、循环神经网络和新型归一化流时序模型FloTS）预测大气视宁度（seeing）参数，发现FloTS在预测精度和不确定性校准方面表现最优，可提前两小时进行短期湍流预报。


<details>
  <summary>Details</summary>

Background: 大气光学湍流（optical turbulence）引起的折射率波动是地基光学系统面临的核心挑战，其干扰光传播并影响天文观测和自由空间光通信（free-space optical communications）性能。

Data: 数据来源于历史视宁度（seeing）观测数据。

Method: 采用机器学习模型（包括高斯过程（GP）、循环神经网络（RNN）、长短期记忆网络（LSTM）和新型时间序列归一化流（FloTS））预测大气视宁度（seeing）参数，重点比较统计与深度学习方法并评估概率模型的不确定性量化能力。

Result: 利用归一化流时间序列模型（FloTS）在预测大气视宁度（seeing）参数时实现了预测准确性与校准不确定性之间的最佳平衡。
...
### [37] [The VLT/ERIS grating vector Apodizing Phase Plate coronagraph](https://arxiv.org/abs/2603.24469)
...
**TL;DR**: 该论文介绍了为甚大望远镜(VLT)增强分辨率成像仪和光谱仪(ERIS)设计的栅格矢量变迹相位板(gvAPP)日冕仪，通过实验室和实际观测验证其在K波段透光率达90%、L波段达60%，原始对比度达1×10^-5但未达到后处理对比度5×10^-5目标，推荐用于已知位置亚恒星伴星表征并指出探测器电子噪声是限制因素。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于为甚大望远镜（VLT）的高分辨率成像仪和光谱仪（ERIS）开发并测试光栅矢量变迹相位板（gvAPP）日冕仪，旨在提升对亚恒星伴星的观测能力。

Data: 数据来源于实验室测量和实际天光观测（on-sky observations），覆盖了从K波段到L波段的多个不同滤光片。

Method: 采用实验室测量和天空观测相结合的方法，在K波段至L波段的多组滤光片下对光栅矢量变迹相位板（gvAPP）进行性能表征。

Result: 光栅矢量变迹相位板（gvAPP）在K波段和L波段分别实现了90%和60%的光学传输效率，达到了设计指标。
...
### [38] [Lyman-$α$ Forest Constraint on Dark Matter from Dark Sector Decay](https://arxiv.org/abs/2603.24331)
*Si-Yuan Zhao,Yi-Cheng Dai,Wei Liao,Yi-Song Lu*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016517285e41e59681f633b1b8d63fc66f6f1484faa298', 'param': '', 'type': 'Forbidden'}}
...
### [39] [Connecting Meteorite Spectra to Lunar Surface Composition Using Hyperspectral Imaging and Machine Learning](https://arxiv.org/abs/2603.24323)
...
**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016459945e41e59681f633b1b8d63fc66f6f1484faf76b', 'param': '', 'type': 'Forbidden'}}
...
### [40] [Simulation of laser travel-time on Mercury for BELA](https://arxiv.org/abs/2603.24408)
*Jean Barron,Frédéric Schmidt,François Andrieu,Gaku Nishiyama,Alexander Stark,Hauke Hussmann*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016479655e41e59681f633b1b8d63fc66f6f1484de0094', 'param': '', 'type': 'Forbidden'}}
...
### [41] [Artificial precession and instability in solar system and planetary simulations: analytic and numerical results](https://arxiv.org/abs/2603.24456)
*David M. Hernandez*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016486745e41e59681f633b1b8d63fc66f6f1484128055', 'param': '', 'type': 'Forbidden'}}
...
### [42] [Exoplanet Search and Characterization with the Proposed POET Canadian Space Mission](https://arxiv.org/abs/2603.24485)
...
**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016495355e41e59681f633b1b8d63fc66f6f1484bde799', 'param': '', 'type': 'Forbidden'}}
...
### [43] [Thermally inflated accretors in post-mass transfer binaries: Abell 35 and its class revisited](https://arxiv.org/abs/2603.23756)
*Soumyadeep Bhattacharjee,Kareem El-Badry,Jim Fuller,Cheyanne Shariat,Natsuko Yamaguchi*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016534355e41e59681f633b1b8d63fc66f6f14848a54b1', 'param': '', 'type': 'Forbidden'}}
...
### [44] [A Catalog of 1,408 Carbon-Enhanced Metal-Poor Stars from LAMOST DR11](https://arxiv.org/abs/2603.24040)
*Xianqi Liu,Xiangru Li,Ziyu Fang*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016552015e41e59681f633b1b8d63fc66f6f1484e16b08', 'param': '', 'type': 'Forbidden'}}
...
### [45] [Stellar Variability and Distance Indicators in the Near-infrared in Nearby Galaxies. II. Pulsating Stars in the Carina Dwarf Spheroidal](https://arxiv.org/abs/2603.24249)
*Chow-Choong Ngeow,Anupam Bhardwaj,Prashant Nishad,Das Susmita*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016558505e41e59681f633b1b8d63fc66f6f1484173646', 'param': '', 'type': 'Forbidden'}}
...
### [46] [Stable black hole solutions with cosmological hair](https://arxiv.org/abs/2603.22398)
*Laurens Smulders,Johannes Noller*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016455085e41e59681f633b1b8d63fc66f6f1484d5e391', 'param': '', 'type': 'Forbidden'}}
...
### [47] [Cosmology with Logarithmic Corrected Horizon Entropy According to the Generalized Entropy and Variable-G Correspondence](https://arxiv.org/abs/2603.23551)
*Chen-Hao Wu,Yen Chin Ong*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016468535e41e59681f633b1b8d63fc66f6f148494b605', 'param': '', 'type': 'Forbidden'}}
...
### [48] [The read-out electronics for the FLASH experiment](https://arxiv.org/abs/2603.23846)
*Luigi Calligaris,Claudio Puglia,Gianluca Lamanna*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016499875e41e59681f633b1b8d63fc66f6f1484f95346', 'param': '', 'type': 'Forbidden'}}
...
### [49] [IXPE view of the Crab pulsar following the 17 July and 6 August 2025 glitches](https://arxiv.org/abs/2603.23808)
...
**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016445805e41e59681f633b1b8d63fc66f6f148498d4f7', 'param': '', 'type': 'Forbidden'}}
...
### [50] [The Environments of Luminous Fast Blue Optical Transients: Evidence for a Compact Object and Wolf-Rayet Star Merger Origin](https://arxiv.org/abs/2603.23597)
*Anya E. Nugent,V. Ashley Villar,Brian D. Metzger,Christopher L. Fryer,Eric Burns,Alexa Gordon,Danielle Frostig*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016521585e41e59681f633b1b8d63fc66f6f14847daebc', 'param': '', 'type': 'Forbidden'}}
...
### [51] [Radiation-ionization hydrodynamic simulations of AGN line-driven winds lead to transient shielding and BAL/UFO signatures](https://arxiv.org/abs/2603.23642)
*Nicolas Scepi,Christian Knigge,Amin Mosallanezhad,Knox S. Long,James H. Matthews,Stuart A. Sim,Austen Wallis*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016530095e41e59681f633b1b8d63fc66f6f14842fbf45', 'param': '', 'type': 'Forbidden'}}
...
### [52] [Near-Infrared and Optical Observations of SN 2024rbc: The First Early Detection of CO and Dust in a Type Ib Supernova](https://arxiv.org/abs/2603.23877)
...
**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016545065e41e59681f633b1b8d63fc66f6f148493fd52', 'param': '', 'type': 'Forbidden'}}
...
### [53] [Simultaneous Multi-band Optical Follow-up Observations of a Gamma-Ray Flare in BL Lacertae](https://arxiv.org/abs/2603.24159)
...
**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016554215e41e59681f633b1b8d63fc66f6f14845b0d88', 'param': '', 'type': 'Forbidden'}}
...
### [54] [Dichroic Dual-Angle Refractor: Multi-Cell Huygens' Metasurface-Based Circuit Approximation](https://arxiv.org/abs/2603.23542)
*Georgios Kyriakou,Giampaolo Pisano*

**TL;DR**: Error code: 403 - {'error': {'code': 'AccountOverdueError', 'message': 'The request failed because your account has an overdue balance. Request id: 0217745016420285e41e59681f633b1b8d63fc66f6f1484ebc802', 'param': '', 'type': 'Forbidden'}}
