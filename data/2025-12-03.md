...
### [1] [COSMOS-3D: Two obscured X-ray AGNs with hot dust and He I$λ$10830 absorption at z~3](https://arxiv.org/abs/2512.02093)
...
**TL;DR**: 在红移z~3处发现两个具有显著He Iλ10830+Paγ发射和吸收特征的宽线X射线活动星系核（AGN）cid_414和cid_947，其致密核周气体可能普遍存在于高红移并调控AGN遮蔽与黑洞-宿主共演化。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(z~3)活动星系核(AGN)的致密环核气体(dense circumnuclear gas)及其对AGN遮蔽效应与黑洞-宿主共演化的调控作用。

Data: 数据来源包括JWST Cycle 3大型GO项目COSMOS-3D的NIRCam F444W光栅光谱数据、COSMOS场补充光谱的UV/光学谱线测量（如Lyα、Si IV、C IV）以及MIRI F1000W和F2100W的中红外探测数据。

Method: 采用近红外相机(NIRCam)F444W光栅光谱和补充的COSMOS场光谱进行紫外/光学谱线测量，并结合中红外仪器(MIRI)F1000W和F2100W成像数据探测热尘埃辐射，通过宿主星系分解和多组分光谱能量分布(SED)拟合分析黑洞质量与恒星形成率，并基于简化光致电离模型推断致密吸收气体特征尺度与核区宽线区相当。

Result: 在红移z~3处发现两个具有显著He Iλ10830+Paγ发射和吸收特征的宽线X射线活动星系核（AGN）cid_414和cid_947，其致密环核气体可能普遍存在于高红移并调控AGN遮蔽与黑洞-宿主共演化。
...
**TL;DR**: 基于DREAMS项目的1024个银河系质量宿主星系模拟，研究发现卫星星系性质主要受晕间离散性影响，超新星风能（supernova wind energy）是重子物理模型中影响卫星恒星质量和半光半径的最主要因素，其他天体物理和宇宙学参数影响可忽略。


<details>
  <summary>Details</summary>

Background: 研究聚焦于在Λ冷暗物质(ΛCDM)宇宙学框架下，利用流体动力学模拟探讨星系形成物理过程对银河系质量宿主星系周围卫星星系特性的影响。

Data: 数据来源于DREAMS项目模拟的1,024个银河系质量宿主星系及其卫星星系，采用TNG星系形成模型，结合重子物理和宇宙学不确定性，并与SAGA巡天观测数据及DREAMS纯N体模拟结果进行对比。

Method: 采用TNG星系形成模型（TNG galaxy-formation model）分析重子物理（baryonic physics）和宇宙学不确定性对卫星星系特性的影响，重点关注超新星风能（supernova wind energy）等参数的作用。

Result: 超新星(Supernova)风能增加会抑制卫星星系恒星质量并导致其半光半径(Stellar half-light radius)更延展。
...
**TL;DR**: JWST发现两个处于向典型活动星系核（AGN）过渡阶段的"小红点"（LRD）天体（红移z=2.868和2.925），其强烈的X射线、射电和中红外辐射表明中央黑洞周围致密气体包层正在消散，证实部分LRD会演化为AGN或类星体。


<details>
  <summary>Details</summary>

Background: 研究聚焦于詹姆斯·韦伯太空望远镜（JWST）发现的具有独特V形光谱能量分布（SED）的致密天体"小红点"（LRDs），探讨其作为活动星系核（AGNs）前身或过渡阶段的天体物理本质。

Data: 数据来源于James Webb Space Telescope (JWST)对两个红移z = 2.868和2.925的"little red dots" (LRDs)目标的观测，包括紫外和光学波段的V形光谱能量分布(SED)、X射线、射电和中红外辐射数据。

Method: 采用核心分析方法包括光谱能量分布（SED）建模、宽发射线（broad Balmer emission lines）分析以及多波段（X射线、射电和中红外）辐射观测。

Result: 发现两个处于向典型活动星系核（AGN）过渡阶段的"小红点"（LRDs），其强烈X射线、射电和中红外辐射表明中央黑洞周围稠密气体包层正在消散，同时尘埃环形成。
...
**TL;DR**: 开发了基于Python的强透镜高红移团块状恒星形成星系模拟器ClumPyLen，能通过用户提供的偏转角(deflection angle)地图模拟引力透镜效应，并生成包含盘(disks)、核球(bulges)和旋臂(spiral arms)等星系成分的逼真观测图像，尤其适用于研究低质量区恒星团块(stellar clumps)的可探测性和混合效应。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发模拟高红移（high-redshift）团块状（clumpy）恒星形成星系在强引力透镜（strongly lensed）条件下观测特征的Python工具，旨在探索恒星团块（stellar clumps）的可探测性及其在星系演化中的作用。

Data: 数据来源于用户提供的引力透镜偏转角地图（deflection angle maps）以及模拟的观测条件，包括仪器效应、点扩散函数卷积（Point-Spread-Function convolution）、天空背景和光子噪声，支持HST/ACS、HST/WFC3-IR和JWST/NIRCam等多种滤光片和仪器。

Method: 采用基于Sérsic轮廓（Sérsic profiles）的物理模型模拟星系盘、核球和旋臂，并通过物理驱动分布采样恒星团块（stellar clumps）属性，结合用户提供的引力透镜偏转角图（deflection angle maps）及仪器效应、点扩散函数卷积（Point-Spread-Function convolution）、天空背景和光子噪声等观测条件模拟。

Result: 开发了基于Python的模拟器ClumPyLen，可生成高红移团块状恒星形成星系在强引力透镜作用下的逼真模拟观测图像。
...
**TL;DR**: 提出了一种不依赖空间选择函数建模的"条件深度势能"方法，通过模拟条件速度分布$p\left(\vec{v}\mid\vec{x}\right)$同时推断银河系引力势$Φ\left(\vec{x}\right)$和示踪星群空间密度$n\left(\vec{x}\right)$，有效克服了星际消光等选择效应对恒星运动学研究的干扰。


<details>
  <summary>Details</summary>

Background: 研究旨在克服星际消光（interstellar extinction）和不同巡天深度（survey depth）等选择效应，开发一种无需建模空间选择函数（spatial selection function）的新方法，以更准确地测定银河系引力势（gravitational potential）及物质分布。

Data: 数据来源为盖亚(Gaia)观测数据及模拟生成的包含复杂三维尘埃分布的虚拟数据集。

Method: 采用条件速度分布建模方法（conditional velocity distribution modeling）和基于稳态假设的无碰撞玻尔兹曼方程（collisionless Boltzmann equation），同时学习引力势（gravitational potential）和示踪星群空间密度（spatial density of tracer population）。

Result: 提出了一种不依赖空间选择函数建模的"条件深度势能"方法（conditional Deep Potential method），通过建模条件速度分布$p\left(\vec{v}\mid\vec{x}\right)$并联合求解引力势$Φ\left(\vec{x}\right)$和示踪星群空间密度$n\left(\vec{x}\right)$，有效克服了星际消光等选择效应的影响。
...
**TL;DR**: 在阿贝尔383星系团（Abell 383）后方发现了一个双成像小红点（Little Red Dot，LRD）候选体A383-LRD1，其由符合LRD选择标准的红色点源和邻近的蓝色伴星组成，并通过引力透镜放大揭示了LRD的绝对星等、伴星性质及两者光谱能量分布（SED）特征，成为研究LRD与宿主星系及环境关系的独特案例。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用引力透镜效应（gravitational lensing）发现并分析高红移（high-redshift）小红点（Little Red Dot, LRD）候选体A383-LRD1及其伴星，以探索早期宇宙中类星体（quasar）与宿主星系及星际环境的相互作用。

Data: 数据来源包括哈勃太空望远镜（HST）成像数据、地面望远镜的发射线探测数据、JWST/NIRCam（近红外相机）的VENUS巡天周期4观测数据以及ALMA（阿塔卡马大型毫米波/亚毫米波阵列）对[C II]λ158 μm发射的探测数据。

Method: 采用JWST/NIRCam观测数据结合SED（光谱能量分布）拟合分析，对A383-LRD1的两个组成成分分别应用黑洞恒星（BH*）模型和年轻星形成星云模型进行物理建模。

Result: 在Abell 383星系团后方发现了一个双像小红点（Little Red Dot，LRD）候选体A383-LRD1，其由符合LRD选择标准的红色点源和邻近的致密蓝色伴星组成，并通过引力透镜放大和光谱能量分布（SED）拟合确认了其红移及物理性质。
...
**TL;DR**: 通过SEAMLESS（半自动机器学习搜索半解析星系）方法首次发现并表征了四个局域体积矮星系（Hydrus A、LEDA 486718、Cetus B和Sculptor 26），揭示了它们在不同环境和演化阶段的多样性特征，其中Hydrus A表现出宇宙再电离或内部反馈导致的淬灭特性，LEDA 486718为孤立恒星形成矮星系，Cetus B可能是低质量卫星或回溅候选体，Sculptor 26则处于过渡演化状态。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用半自动机器学习搜索系统(SEAMLESS)在近邻宇宙(Local Volume)中探测半解析矮星系(dwarf galaxies)，以揭示不同环境(孤立、卫星或回溅候选体)和演化阶段(恒星形成、淬灭或过渡态)下矮星系的多样性及其物理机制。

Data: 数据来源于半自动化机器学习搜索系统SEAMLESS（SEmI-Automated Machine LEarning Search for Semi-resolved galaxies）对本地体积（Local Volume）矮星系的观测。

Method: 采用半自动化机器学习搜索半解析星系（SEAMLESS）技术识别并分析本地体积（Local Volume）矮星系。

Result: 首次发现并报道了四个本地体积矮星系（Hydrus A、LEDA 486718、Cetus B和Sculptor 26），其中Hydrus A和LEDA 486718是5兆秒差距内已知最孤立的矮星系，而Cetus B和Sculptor 26位于NGC 253的2倍维里半径范围内。
...
**TL;DR**: 通过多波段近红外观测首次对NGC 2024星团中362颗年轻恒星天体（YSO）进行了高质量均匀自行运动测量，发现其呈现由内向外的年龄分层结构，支持快速（<1 Myr）星团坍缩模型，并观测到I类源相对于平谱源具有略高的速度弥散度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用年轻恒星天体（YSOs）的运动学特征揭示恒星形成区（如NGC 2024星团）的动力学演化过程，特别是恒星与气体的运动解耦时间尺度及星团快速坍缩模型。

Data: 数据来源于三个ESO公开巡天项目的多历元近红外观测：VISIONS、VHS和VISTA/VIRCAM科学验证计划。

Method: 采用多历元近红外观测数据（VISIONS、VHS和VISTA/VIRCAM科学验证计划）测量NGC 2024星团中6,769个天体的自行运动（proper motion），并通过与盖亚（Gaia）光学可见星的交叉验证确保测量精度。

Result: result：NGC 2024星团中年轻恒星天体（YSO）的自行运动测量支持快速（<1 Myr）星团坍缩模型，并证实了从内到外的年龄分层现象，同时发现I类源相对于平谱源具有略高的1D速度弥散度。
...
### [9] [A value-added catalogue of neural network-based europium abundances for GALAH DR4](https://arxiv.org/abs/2512.02125)
*Sarah G. Kane,Zofia Kaczmarek,Andrew Garner,Sven Buder,Stephanie Monty,Elana Kane*

**TL;DR**: 利用卷积神经网络（CNN）对GALAH DR4光谱数据进行标签迁移，构建了包含118,946颗恒星（其中54,068颗高质量巨星）的铕（Eu）丰度新星表，克服了低金属丰度区训练数据不足的问题，验证了吸积恒星铕丰度升高等已知化学演化规律。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用快中子俘获过程（$r$-process）元素铕（Eu）作为中子星合并等稀有核合成事件的示踪剂，通过恒星光谱数据提升其丰度测量精度。

Data: 数据来源为GALAH巡天项目（Galactic Archaeology with HERMES）的第四版数据（DR4）光谱和恒星参数，以及第三版数据（DR3）的[Eu/H]丰度值作为训练集，并针对低金属丰度（[Fe/H]<-1）恒星补充了铕吸收线合成计算的[Eu/H]丰度数据。

Method: 采用卷积神经网络（CNN）进行标签转移（label transfer），利用GALAH DR4光谱和恒星参数推断DR3的[Eu/H]丰度，并对低金属丰度（[Fe/H]<-1）恒星通过铕（Eu）特征合成提供额外丰度目录。

Result: 利用卷积神经网络（CNN）进行标签迁移，从GALAH DR4光谱和恒星参数推断DR3的[Eu/H]丰度，生成了包含118,946颗恒星（其中54,068颗巨星构成高置信度“黄金样本”）的公开[Eu/H]值目录，并针对低金属丰度（[Fe/H]<-1）恒星提供了基于铕（Eu）特征合成的补充目录。
...
### [10] [Ripples in the baryon to dark matter ratio in $Λ$CDM: implications for galaxy formation](https://arxiv.org/abs/2512.02127)
*Owen Jessop,Adrian Jenkins,Andrew Pontzen,Joop Schaye,Matthieu Schaller,John C. Helly*

**TL;DR**: 研究发现重子-冷暗物质等曲率扰动（baryon-CDM isocurvature perturbations）在红移8时使晕（halo）平均重子比例和恒星形成率分别降低5%和12%，且该效应随红移降低而减弱。


<details>
  <summary>Details</summary>

Background: 研究探讨了重子-冷暗物质（baryon-CDM）等曲率扰动（isocurvature perturbations）在ΛCDM框架下对星系形成的影响及其在宇宙学模拟中的重要性。

Data: 使用了FLAMINGO星系形成模型进行的三组流体动力学模拟（hydrodynamical simulations）数据，分别包含原始等曲率扰动（isocurvature modes）、忽略扰动以及反转扰动振幅的初始条件。

Method: 采用FLAMINGO星系形成模型（FLAMINGO galaxy formation model）和球状坍缩模型（spherical collapse model）分析重子-冷暗物质等曲率扰动（baryon-CDM isocurvature perturbations）对星系形成的影响，并通过三组流体动力学模拟（hydrodynamical simulations）对比包含、忽略及反转扰动幅度的初始条件。

Result: 重子-冷暗物质（baryon-CDM）等曲率扰动（isocurvature perturbations）导致高红移（z=8）晕（halo）的平均重子比例和恒星形成率分别降低5%和12%，且该效应随红移降低而减弱。
...
### [11] [$S^5$: Tidal Disruption in Crater 2 and Formation of Diffuse Dwarf Galaxies in the Local Group](https://arxiv.org/abs/2512.02177)
...
**TL;DR**: 通过光谱观测发现Crater 2（Cra2）矮星系及其潮汐尾具有极低速度弥散（中心$2.51^{+0.33}_{-0.30}\,{\rm km\,s^{-1}}$）和显著速度梯度，结合N体模拟表明其动力学特征更符合核心暗晕（cored halo）或低浓度尖峰暗晕（low-concentration cuspy halo）模型，且金属丰度（$\langle \rm[Fe/H]\rangle=-2.16\pm0.04$）异常偏低，挑战了标准暗物质理论和星系形成模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索Crater 2（Cra2）这类具有极低运动学温度但超大尺寸的弥散矮星系（diffuse dwarf galaxy）在无碰撞冷暗物质（collisionless cold dark matter）框架下的形成机制及其与标准星系形成模型的矛盾。

Data: 数据来源于南天恒星流光谱巡天（Southern Stellar Stream Spectroscopic Survey, $S^5$）对Crater 2（Cra2）及其潮汐尾的光谱观测。

Method: 采用N体模拟（N-body simulations）结合尖峰（cuspy）和核心（cored）密度剖面模型，在真实轨道上对比S5观测数据。

Result: 在Crater 2（Cra2）及其潮汐尾的光谱观测中，发现其主星体速度弥散极低（$2.51^{+0.33}_{-0.30}\,{\rm km\,s^{-1}}$），而潮汐流速度弥散较高（$5.74^{+0.98}_{-0.83}\,{\rm km\,s^{-1}}$），两者比值（$2.30^{+0.41}_{-0.35}$）难以用标准冷暗物质(cold dark matter)模型中的尖峰晕(cuspy halo)解释，但可通过小核心半径的平坦晕(cored halo)或低浓度尖峰模型重现。
...
### [12] [How much gas and dust is in the $z=5.7$ Lyman Break Galaxy HZ10? An ALMA Band 10 to 4 and JWST/NIRSpec study of its interstellar medium](https://arxiv.org/abs/2512.02320)
...
**TL;DR**: 首次对红移(redshift)5.65的主序恒星形成星系HZ10进行了完整重子物质普查，结合ALMA和JWST数据发现常用[CII]气体质量转换因子会高估实际值，并揭示其偏离局域尘埃-气体比与金属丰度关系的特殊性质。


<details>
  <summary>Details</summary>

Background: 研究主题为探索高红移（high-redshift）星系HZ10在红移$z=5.65$时的完整重子物质（baryonic content）组成，包括恒星、气体和尘埃，以揭示早期星系组装机制。

Data: 数据来源包括：新获取的ALMA Band 10（λ_rest=50μm）和Band 4（300μm）观测数据、先前ALMA Band 6至9（70-200μm）数据、存档高分辨率[CII]观测数据、JVLA CO(2-1)分子气体质量数据、JWST金属丰度和恒星质量测量数据。

Method: 采用ALMA多波段（Band 4、6-10）连续谱观测结合[CII]谱线动力学质量约束，以及JWST金属丰度与恒星质量测量，构建了完整的重子物质（气体、尘埃、恒星）分布模型，并推导了[CII]-to-ISM质量转换因子（α_[CII]^ISM）。

Result: 在红移(redshift) z=5.65的主序恒星形成星系HZ10中，[CII]到总星际介质(ISM)质量的转换因子为α_[CII]^ISM = 39^{+50}_{-25}M⊙ L⊙^{-1}，且该星系偏离了尘埃-气体比与金属丰度的局域标度关系。
...
### [13] [Evolution and Mass Dependence of UV-to-near-IR Color Gradients up to z=2.5 from HST+JWST](https://arxiv.org/abs/2512.02440)
...
**TL;DR**: 研究揭示了红移范围0.5<z<2.5内星系径向颜色梯度（静止坐标系下的U-V和V-J）随恒星质量的变化规律，发现恒星形成星系的V-J负梯度主要由尘埃消光梯度主导，而静止星系群的微弱负梯度则主要源于恒星种群梯度。


<details>
  <summary>Details</summary>

Background: 研究星系在红移范围0.5 < z < 2.5内的径向颜色梯度（静止坐标系下的U-V和V-J）演化及其与恒星质量的关系。

Data: 数据来源于公开可用的JWST/NIRCam（James Webb Space Telescope/Near-Infrared Camera）选源目录，并选取了约10,200颗恒星质量$M_\star>10^{9.5}~{\text{M}}_\odot$的星系，同时结合HST（Hubble Space Telescope）和JWST/NIRCam测光数据进行空间分辨的SED（Spectral Energy Distribution）拟合。

Method: 采用二维Sérsic轮廓拟合（2D Sérsic profile fits）结合点扩散函数（PSF）修正，对哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST/NIRCam）测光数据进行空间分辨光谱能量分布（SED）拟合，获取星系在2倍有效半径（2$R_\text{e, F444W}$）内的静止框架${\textit{U}} - {\textit{V}}$和${\textit{V}} - {\textit{J}}$颜色梯度。

Result: 在静止星系（quiescent galaxy）中，${\textit{U}} - {\textit{V}}$和${\textit{V}} - {\textit{J}}$颜色梯度微弱负值（约-0.1至-0.2星等），主要归因于恒星种群梯度（stellar population gradients），且几乎不依赖恒星质量、红移或轴比。
...
### [14] [Silicate emission in a type-2 quasar: JWST/MIRI constraints on torus geometry and radiative feedback](https://arxiv.org/abs/2512.02629)
...
**TL;DR**: JWST/MIRI观测发现部分II型类星体(QSO2s)核区存在硅酸盐发射特征，结合贝叶斯模型拟合证实高爱丁顿比活动星系核(AGN)的环面覆盖因子较低，支持辐射压力驱动尘埃气体清除的演化场景。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST/MIRI的高角分辨率观测揭示II型类星体(QSO2s)核区尘埃云结构及其与爱丁顿比率(Eddington ratio)的关联，探讨活动星系核(AGN)辐射压力对尘埃环面(torus)覆盖因子的调控机制。

Data: 数据来源于JWST/MIRI的中红外光谱仪(MRS module)对红移z~0.1的五个2型类星体(QSO2s)核区0.5-1.1千秒差距范围的观测。

Method: 采用贝叶斯工具BayesClumpy的更新版本，结合CLUMPY和CAT3D-WIND两种环面(torus)模型对J1010的中红外核光谱进行拟合分析。

Result: JWST/MIRI的高分辨率观测揭示了两个z~0.1的II型类星体(QSO2s)核区存在9.7、18和23微米硅酸盐发射特征，表明其尘埃云被直接照亮。
...
### [15] [SDSS-V LVM: Resolving Physical Conditions in the Trifid Nebula](https://arxiv.org/abs/2512.02802)
...
**TL;DR**: 通过高分辨率（0.24 pc）空间测绘发现，三叶星云（Trifid Nebula）作为孤立H II区（H II region）在电子温度（electron temperature）和总氧丰度（total oxygen abundance）上呈现均匀性，且积分测量与空间分辨测量结果无显著差异。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际介质化学丰度对恒星形成初始条件的影响及其在星系化学演化模型中的验证作用。

Data: 数据来源于SDSS-V Local Volume Mapper对三叶星云（Trifid Nebula）的0.24 pc分辨率空间映射观测，利用氢复合线、碰撞激发线（包括微弱极光线）等多种发射线。

Method: 利用SDSS-V Local Volume Mapper在0.24 pc分辨率下通过多种发射谱线（如氢复合线、碰撞激发线及微弱极光线）计算了[N II]、[O II]、[S II]、[S III]电子温度以及离子氧丰度的空间分布图。

Result: result：在0.24 pc分辨率下观测到的三叶星云(M 20)的电子温度和总氧丰度未发现明显梯度或结构，表明该孤立H II区在观测尺度上相对均匀，且积分测量与空间分辨测量结果无显著差异。
...
### [16] [Direct observational evidence that higher-luminosity type 1 active galactic nuclei are most commonly triggered by galaxy mergers](https://arxiv.org/abs/2512.02805)
*Yongmin Yoon,Yongjung Kim,Dohyeong Kim,Kyungwon Chun,Woowon Byun*

**TL;DR**: 研究发现，在近邻宇宙中，星系并合（galaxy mergers）是高光度活动星系核（AGN）的主要触发机制，而对低光度AGN的作用较小，且并合触发概率随[O III]λ5007发射线光度（$L_{[\mathrm{O\,{\scriptstyle III}}]}$）和黑洞质量（$M_\mathrm{BH}$）增加而显著上升。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系并合（galaxy mergers）与活动星系核（AGN）触发机制之间的关联。

Data: 数据来源于DESI Legacy Imaging Survey的深度图像中检测到的潮汐特征（tidal features），用于识别近期星系并合事件。

Method: 基于DESI遗产成像巡天（DESI Legacy Imaging Survey）的深度图像检测潮汐特征（tidal features）作为近期并合的直接证据。

Result: 潮汐特征比例(f_T)随[O III]λ5007发射线光度(L_[O III])和黑洞质量(M_BH)增加而显著上升，表明星系合并是高光度活动星系核(AGN)的主要触发机制，而对低光度AGN作用较小。
...
### [17] [The Large Sky Area Multi-object Fiber Spectroscopic Telescope (LAMOST) Quasar Survey: Quasar Properties from Data Release 10 to 12](https://arxiv.org/abs/2512.02836)
...
**TL;DR**: 基于LAMOST（大天区多目标光纤光谱望远镜）巡天数据发布10至12的类星体（quasar）目录新增了11346个类星体（其中5386个为首次发现），总发现数达67521个（新发现29513个），并通过ZTF（兹威基瞬变设施）准同步测光数据重新校准光谱，提升了发射线流量、连续谱流量及黑洞质量测量的准确性，促进了对罕见类星体（如变脸类星体和宽吸收线类星体）及活动星系核（AGN）相关瞬变现象的探测。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用LAMOST（大天区多目标光纤光谱望远镜）巡天数据发布10至12的类星体（quasar）目录，扩展已知类星体样本并改进光谱定标方法。

Data: 数据来源于大天区多目标光纤光谱望远镜（LAMOST）类星体巡天2021年9月至2024年6月的数据发布10至12（Data Releases 10-12）以及兹威基瞬变设施（ZTF）自2018年3月起的高频次监测提供的（准）同步测光数据。

Method: 采用Zwicky瞬变设施(ZTF)的准同步测光数据对LAMOST光谱进行重新校准，并基于校准后的单历元光谱估算发射线流量、连续谱流量和基于动力学模型的黑洞质量(virial black hole masses)。

Result: 基于LAMOST（大天区多目标光纤光谱望远镜）数据发布10至12的类星体（quasar）目录新增了11,346个类星体，其中5,386个为百万类星体目录（Million Quasars catalog）中未收录的新发现目标。
...
### [18] [On Constraining the Proposed Hierarchical Triple Scenario for an Eccentric Milli-Second Pulsar Binary PSR J1618-3921](https://arxiv.org/abs/2512.02857)
*Adya Shukla,A. Gopakumar,Paramasivan Arumugam*

**TL;DR**: 研究发现偏心毫秒脉冲星（eMSP）PSR J1618-3921可能属于一个包含0.6倍太阳质量伴星、轨道周期约300年的层级三体（HT）系统，其轨道偏心率和近星点进动率变化特征为探测潜在第三天体提供了严格约束条件。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于探索偏心毫秒脉冲星（eMSP）双星系统PSR J1618-3921可能作为层级三重（HT）系统一部分的天体动力学特征及其观测约束条件。

Data: 研究使用了来自对PSR J1618-3921的高精度计时观测数据，并结合了模拟（simulation）数据来探讨其可能的三体系统（hierarchical triple system）动力学特性。

Method: 采用点质量层级三体（HT）模型，结合内外双星四极矩相互作用及内双星近心点进动的主导阶广义相对论效应。

Result: PSR J1618-3921若处于当前经历Kozai振荡的层级三体系统（hierarchical triple）中，其轨道偏心率（eccentricity）将减小而近日点进动率（$\dotω$）将增加，且偏心率变化幅度可达$10^{5}$量级。
...
### [19] [Distinguishing ram pressure from gravitational interactions: Applying the Size-Shape Difference method to real galaxies](https://arxiv.org/abs/2512.02923)
...
**TL;DR**: 首次通过观测验证了尺寸-形状差异（SSD）测量方法能有效区分强冲压压力剥离（RPS）与引力相互作用星系，发现极端RPS样本的SSD值（56(+24/-15)）是未受扰及引力相互作用样本（16(+6/-2)和16(+6/-3)）的3.5倍，证实年轻恒星因RPS产生不对称分布而中年恒星保持对称的特征差异。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过年轻与中年恒星群形态差异（SSD）区分致密环境中由冲压剥离（RPS）和引力作用导致的星系形态特征。

Data: 数据来源于GASP巡天项目中的67个星系，利用SINOPSIS方法获取了空间分辨的恒星形成历史（star formation histories）。

Method: 采用尺寸-形状差异（SSD）测量方法，通过比较年轻（t < 20 Myr）和中龄（20 Myr <= t < 570 Myr）恒星群体的形态差异来区分冲压压力剥离（RPS）和引力相互作用星系。

Result: 强RPS（ram pressure stripping）星系展现出显著高于未受扰和引力相互作用星系的SSD（Size-Shape Difference）值（56(+24/-15) vs. 16(+6/-2)和16(+6/-3)），证实了年轻恒星因气体盘位移产生不对称性而中年恒星保持未扰动的模拟预测。
...
### [20] [The Composite Spectrum of QSO Absorption Line Systems in DESI DR2](https://arxiv.org/abs/2512.02992)
...
**TL;DR**: 这篇论文通过叠加暗能量光谱仪（DESI）第二数据发布的238,838个类星体（QSO）吸收线光谱，构建了高信噪比的复合光谱，首次识别出70余条吸收线并显著增强了氧和氢发射特征，为研究吸收体（absorber）的化学成分和物理条件提供了完整谱线图谱。


<details>
  <summary>Details</summary>

Background: 研究聚焦于构建类星体(QSO)吸收线系统的复合光谱，以揭示遥远类星体光线穿过介入星系环星系介质(CGM)时产生的吸收线特征。

Data: 数据来源于暗能量光谱仪（DESI）第二次数据发布的238,838条具有吸收线特征的类星体（QSO）光谱。

Method: 采用叠加光谱（stacking）和计算中位数复合光谱（median composite spectrum）的方法增强吸收线信号。

Result: 在类星体(QSO)吸收线系统的合成光谱中识别出70余条吸收线，并以高于以往研究的信噪比观测到氧和氢发射特征。
...
### [21] [MIRI spectrophotometry of GN-z11: Detection and nature of an optical red continuum component](https://arxiv.org/abs/2512.02997)
...
**TL;DR**: 通过中红外成像(MIRI)和光谱分析发现GN-z11存在年轻(4±1 Myr)和成熟(63±23 Myr)混合星族，并在0.66μm和0.86μm波段观测到显著辐射超出，可能由尘埃环(AGN)热辐射或极端紫外辐射场下的光致发光尘埃过程(ERE)引起。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移星系GN-z11的恒星形成活动与潜在活动星系核(AGN)贡献，通过多波段光谱能量分布(SED)分析揭示其混合恒星种群及可能的尘埃辐射机制。

Data: 数据来源包括新的MIRI F560W、F770W和F1000W成像数据，以及存档的NIRSpec/Prism和MRS光谱数据、NIRCam成像数据。

Method: 采用中红外成像仪(MIRI) F560W、F770W和F1000W波段成像数据，结合近红外光谱仪(NIRSpec)/棱镜和中分辨率光谱仪(MRS)光谱数据以及近红外相机(NIRCam)成像数据，通过光谱能量分布(SED)拟合分析恒星种群年龄和尘埃辐射特征。

Result: 在0.66微米（μm）和0.86微米（μm）静止波长处观测到36±3%和91±28%的流量超出（flux excess），可能由尘埃环（dusty torus）中的热尘埃发射（hot dust emission）或极端紫外辐射场下的光致发光尘埃过程（Extended Red Emission, ERE）引起。
...
### [22] [How Dark is Dark Energy? A Lightcones Comparison Approach](https://arxiv.org/abs/2512.02067)
*Mauro Carfora,Francesca Familiari*

**TL;DR**: 提出了一种几何方法计算FLRW观测者理想光锥与实际物理光锥间的尺度依赖距离函数，发现结构形成产生的红移依赖项Ω_Λ(ẑ)在低红移时显著影响FLRW宇宙学常数Ω̂_Λ的常数暗能量解释，尤其在星系团尺度贡献量级相当。


<details>
  <summary>Details</summary>

Background: 研究探讨了在主流FLRW宇宙学框架下，结构形成对宇宙学常数（cosmological constant）的尺度依赖性贡献如何挑战暗能量（dark energy）恒定性的传统解释。

Data: 数据来源于标准FLRW（Friedmann-Lemaître-Robertson-Walker）观测者通过测量宇宙学红移$\widehat{z}$处光源的几何-结构-增长贡献${Ω_Λ(\widehat{z})}$来获取。

Method: 采用几何方法构建非微扰技术，通过比较理想FLRW（Friedmann-Lemaître-Robertson-Walker）过去光锥与实际物理光锥，建立可测量的尺度依赖距离泛函。

Result: 提出了一种几何方法，通过非微扰技术计算标准FLRW观测者理想化的FLRW过去光锥与实际物理过去光锥之间的可测量、尺度依赖的距离泛函，揭示了结构形成产生的尺度依赖有效场对FLRW宇宙学常数的贡献Ω_Λ(ẑ)，其量级在星系团尺度与假设值相当。
...
### [23] [Photon (Non)Conservation in the Reduced Speed of Light Approximation and How to (Almost) Fix It](https://arxiv.org/abs/2512.02094)
*Nickolay Y. Gnedin*

**TL;DR**: 研究发现，在宇宙学模拟中采用“降低光速”(RSL)近似会导致光子数不守恒，提出了一种能精确计算缺失光子数的数值方案，该方案在简单测试中精度达亚百分比级别，但在半现实的宇宙再电离模拟中表现较差，导致电离区域更快重叠且与全光速辐射传输参考模拟存在显著误差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙学模拟中"降低光速"(Reduced Speed of Light, RSL)近似方法在辐射传输计算中的应用及其光子守恒性问题。

Data: 研究中使用的数据来源包括简化光速（RSL）近似下的辐射传输计算结果、半现实宇宙再电离模拟数据以及保持完整光速的参考模拟辐射场数据。

Method: 采用“降低光速”（RSL）近似方法进行辐射传输计算，并通过精确计数缺失光子并重新加入模拟的方案来修正光子非守恒问题。

Result: 在简化光速（RSL）近似下，通过特定数值方案可精确计算缺失光子数，但将其重新加入模拟极为困难，且在半现实宇宙再电离模拟中该方案导致更快重叠及显著点对点误差。
...
### [24] [Cosmic chronometers with galaxy clusters: a new avenue for multi-probe cosmology](https://arxiv.org/abs/2512.02109)
...
**TL;DR**: 利用宇宙计时器(CC)方法对三个邻近红移星系团(SDSS J2222+2745、MACS J1149.5+2223、SDSS J1029+2623)的高质量VLT/MUSE光谱数据进行分析，首次在红移z=0.542处测得哈勃参数H(z)=66_{-29}^{+81}±13 km/s/Mpc，并发现CC样本具有超太阳金属丰度(Z/Z⊙=1.3±0.7)、低尘埃消光(AV=0.3±0.3 mag)和短时标形成(τ=0.6±0.2 Gyr)特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用宇宙计时器（cosmic chronometers, CC）方法测量红移$z=0.54$处的宇宙膨胀历史，旨在通过高质量光谱数据验证不同宇宙学探针的自洽性。

Data: 研究使用了甚大望远镜(VLT)多单元光谱探测器(MUSE)对三个邻近红移区间星系团(SDSS J2222+2745、MACS J1149.5+2223和SDSS J1029+2623)的高质量光谱观测数据。

Method: 采用宇宙计时器（cosmic chronometers, CC）方法结合全光谱拟合技术，利用改进版Bagpipes代码（移除了年龄的宇宙学先验）分析选定星系团成员的光谱数据。

Result: 利用宇宙计时器（CC）方法在红移z=0.542处测得哈勃参数H(z)=66^{+81}_{-29}(统计)±13(系统) km/s/Mpc。
...
### [25] ["SNe Ia twins" in the Hubble flow, and the determination of H0](https://arxiv.org/abs/2512.02391)
*Pilar Ruiz-Lapuente,Antonio Quintana-Estellés,Jonay I. González Hernández,Andrea Pastorello*

**TL;DR**: 通过分析哈勃流（Hubble flow）中的Ia型超新星（SNe Ia）并采用"双胞胎"方法，研究发现基于12颗SNe Ia的样本得出哈勃常数（H$_{0}$）均值为72.610 ± 1.289(统计) ± 1.085(系统) km s$^{-1}$ Mpc$^{-1}$，验证了经典三阶梯方法的有效性，同时指出泛星计划+(Pantheon+)和芝加哥-卡内基哈勃计划(CCHP)中部分距离数据因尘埃红化低估导致颜色参数不准确。


<details>
  <summary>Details</summary>

Background: 利用Ia型超新星（SNe Ia）双星法在哈勃流中测量高红移（z > 0.015）超新星距离并推导哈勃常数（H₀），验证经典三阶梯方法的有效性。

Data: 数据来源包括哈勃流中的12颗Ia型超新星（SNe Ia）、M66星系中的SN 1989B、NGC 7250和NGC 5643星系中的造父变星（Cepheids）和J-渐近巨星支恒星（JAGB stars），以及Pantheon+和卡内基-芝加哥哈勃计划（CCHP）的完整距离汇编数据。

Method: 采用“Ia型超新星双胞胎”（SNe Ia twins）方法分析哈勃流（Hubble flow）中的超新星距离，并通过单步跨越低红移域与哈勃流验证经典三阶梯方法。

Result: 利用Ia型超新星(SNe Ia)在哈勃流中的距离测量得出哈勃常数(H0)的加权平均值为73.556±2.084(统计误差) km s⁻¹ Mpc⁻¹，而基于12颗SNe Ia的样本在两种锚定方法下分别得到H0=72.833±1.306(统计误差)±1.151(系统误差) km s⁻¹ Mpc⁻¹和H0=72.388±1.272(统计误差)±1.015(系统误差) km s⁻¹ Mpc⁻¹，最终综合均值为H0=72.610±1.289(统计误差)±1.085(系统误差) km s⁻¹ Mpc⁻¹。
...
### [26] [Updates on dipolar anisotropy in local measurements of the Hubble constant from Cosmicflows-4](https://arxiv.org/abs/2512.02526)
*Vincenzo Salzano,J. Beltrán Jiménez,Dario Bettoni,Philippe Brax,Aurélien Valade*

**TL;DR**: 基于宇宙流-4（Cosmicflows-4）星表分析哈勃常数（$H_0$）的角向和径向变化，发现距离递减的偶极子（dipole）信号在未修正局域速度时与主要引力结构方向一致，修正后信号减弱但依然存在，表明宇宙可能存在差异膨胀率，但对大尺度哈勃张力的影响有限。


<details>
  <summary>Details</summary>

Background: 研究聚焦于哈勃常数（Hubble constant）$H_0$的测量张力问题，探索宇宙膨胀速率和局部动力学的潜在各向异性特征及其对哈勃张力（Hubble tension）的影响。

Data: 数据来源于Cosmicflows-4星表（Cosmicflows-4 catalogue），覆盖距离模数（distance modulus）μ∈[29,36]（约对应[20,100] h⁻¹ Mpc）的径向壳层和等面积天区。

Method: 采用自适应加权和球谐函数（spherical harmonics）拟合至八极项的方法分析宇宙流（Cosmicflows-4）目录中径向壳层和等面积天区的哈勃常数（$H_0$）角向及径向变化。

Result: 观测到的哈勃常数(Hubble constant) $H_0$ 各向异性偶极(dipole)信号在未修正局域速度场(peculiar velocity)时与主要引力结构方向一致，修正后信号显著减弱但仍有残留。
...
### [27] [The suppression of the matter power spectrum: strong feedback from X-ray gas mass fractions, kSZ effect profiles, and galaxy-galaxy lensing](https://arxiv.org/abs/2512.02954)
...
**TL;DR**: 通过联合分析eROSITA和HSC-XXL样本的X射线气体质量分数及SDSS/DESI+ACT动力学Sunyaev-Zel'dovich效应(kSZ)轮廓，研究发现重子反馈(baryon feedback)在波数$k=1~h~\mathrm{Mpc}^{-1}$处导致物质功率谱(matter power spectrum)抑制达$10 \pm 2\%$，其气体分布比流体动力学模拟(FLAMINGO、BAHAMAS)的基准模型预测更延展且抑制更强。


<details>
  <summary>Details</summary>

Background: 研究聚焦于重子反馈(baryon feedback)对物质功率谱(matter power spectrum)小尺度抑制效应的幅度和尺度依赖性的不确定性及其约束。

Data: 数据来源于X射线气体质量分数（eROSITA和HSC-XXL样本）、SDSS/DESI+ACT动力学Sunyaev-Zel'dovich效应（kSZ）轮廓、星系-星系透镜效应（galaxy-galaxy lensing）以及模拟的LSST第一年弱引力透镜（weak lensing）分析。

Method: 采用重子化(baryonification)框架联合分析eROSITA和kSZ数据，约束重子反馈对物质功率谱的抑制效应。

Result: 联合分析eROSITA和kSZ数据得出重子反馈对物质功率谱的抑制在$k=1~h~\mathrm{Mpc}^{-1}$处为$10 \pm 2\%$，比流体动力学模拟(FLAMINGO和BAHAMAS)的基准模型预测更强。
...
### [28] [A new constraint on the $y$-distortion with FIRAS: implications for feedback models in galaxy formation and cosmic shear measurements](https://arxiv.org/abs/2512.03038)
*Giulio Fabbian,Federico Bianchini,Alina Sabyr,J. Colin Hill,Christopher C. Lovell,Leander Thiele,David N. Spergel*

**TL;DR**: 通过改进天体物理前景扣除技术重新分析FIRAS存档数据，测得宇宙微波背景辐射黑体谱的y型畸变（y-distortion）均值⟨y⟩=(1.2±2.0)×10⁻⁶（95%置信水平上限⟨y⟩≲5.2×10⁻⁶），精度比原始FIRAS结果提高约3倍，直接排除了多数宇宙流体力学模拟中重子反馈（baryonic feedback）模型，主要基于质量M≲10¹⁴M⊙天体的信息。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙微波背景辐射（CMB）黑体谱的y型畸变（y-distortion），旨在通过其探测星系群和星系团中气体压力对宇宙学反馈模型的约束作用。

Data: 研究使用了FIRAS仪器的存档数据，并通过改进的天体物理前景清除技术进行重新分析。

Method: 采用改进的天体物理前景（astrophysical foreground）清洗技术重新分析FIRAS仪器存档数据，测量宇宙微波背景辐射黑体谱的$y$-型畸变（$y$-type distortion）。

Result: 通过改进天体物理前景扣除技术重新分析FIRAS存档数据，测得平均y畸变（y-distortion）为⟨y⟩=(1.2±2.0)×10⁻⁶（95%置信水平下⟨y⟩≲5.2×10⁻⁶），比原始FIRAS结果约束强约3倍。
...
### [29] [Evaluating the Prospects of Cyclic Deconvolution across 312 Pulsars](https://arxiv.org/abs/2512.02248)
*Jacob E. Turner*

**TL;DR**: 研究发现，针对312颗自转周期小于40毫秒的脉冲星，在14种望远镜-观测频率组合中，80-300 MHz是最佳循环解卷积（cyclic deconvolution）频段，其中uGMRT是目前最佳观测设备，LOFAR次之，未来SKA-Low有望成为理想替代设备。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用循环品质因数（cyclic figure of merit）评估312颗自转周期低于40毫秒的脉冲星在14种不同望远镜-观测频率组合下实现循环解卷积（cyclic deconvolution）的可能性。

Data: 数据来源于312颗自转周期低于40毫秒的脉冲星在14种不同望远镜-观测频率组合下的观测结果，重点分析了升级版巨米波射电望远镜(uGMRT)、低频阵列(LOFAR)和默奇森宽场阵列(MWA)等低频观测设备的数据。

Method: 采用循环品质因数（cyclic figure of merit）评估312颗自转周期低于40毫秒的脉冲星在14种望远镜-观测频率组合下实现循环解卷积（cyclic deconvolution）的可能性。

Result: result：在14种望远镜-观测频率组合中，对312颗自转周期小于40毫秒的脉冲星进行循环解卷积可行性分析，发现最佳频率范围为~80-300 MHz，其中uGMRT是目前最适合循环谱分析的设备，LOFAR次之，未来SKA-Low有望成为理想替代。
...
### [30] [High-Precision Amplitude-Modulated Continuous-Wave Lunar Laser Ranging](https://arxiv.org/abs/2512.02431)
*Slava G. Turyshev*

**TL;DR**: 开发了基于振幅调制连续波（AM-CW）的高功率月球激光测距（LLR）系统框架，实现了亚毫米级（~80微米）双向测距和微米/秒级（0.1-1微米/秒）视线距离变化率测量精度，为探测超慢潮汐耗散、长周期天平动和微赫兹频段引力波提供了新方法。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过开发调幅连续波（AM-CW）激光测月（LLR）技术框架，突破现有脉冲系统限制，实现亚0.1毫米精度以探测超慢潮汐耗散、长周期天平动和微赫兹频段引力波。

Data: 数据来源于基于1千瓦、1064纳米连续波（CW）发射器的1米望远镜对10厘米角立方反射器（corner-cube retroreflectors）的激光测距（LLR）实验，采用GHz级调制频率的振幅调制连续波（AM-CW）技术框架，并考虑了光子统计、大气残余和仪器误差对测量精度的影响。

Method: 采用调幅连续波（AM-CW）技术框架，包括测量模型、联合估计器、统一协方差描述及观测站操作概念（CONOPS），通过GHz级调制信号的相位及相位斜率获取双向测距和单向视线测速数据。

Result: 开发了基于调幅连续波（AM-CW）的高功率月球激光测距（LLR）完整框架，通过GHz级调制相位测量实现双向距离和单向视线距离变化率联合估计，在100秒积分时间内光子统计误差达数十微米量级。
...
### [31] [Flexible Gravitational-Wave Parameter Estimation with Transformers](https://arxiv.org/abs/2512.02968)
...
**TL;DR**: 提出了一种基于Transformer架构的灵活深度学习模型Dingo-T1，能适应多种引力波数据分析配置（包括探测器设置、频率范围调整等），在LIGO-Virgo-KAGRA第三次观测期的48个事件中实现了高效参数估计（采样效率从1.4%提升至4.2%），并支持探测器配置影响研究和广义相对论检验。


<details>
  <summary>Details</summary>

Background: 引力波数据分析面临从噪声探测器信号中高效提取物理信息的挑战，深度学习为传统推断方法提供了灵活替代方案。

Data: 数据来源于48个引力波事件（gravitational-wave events）的观测数据，这些事件来自第三次LIGO-Virgo-KAGRA观测运行（third LIGO-Virgo-KAGRA Observing Run），并覆盖了多种探测器配置和频率范围的分析设置。

Method: 采用基于Transformer的灵活架构结合自适应训练策略，支持推理时动态适应不同探测器配置、频率范围或局部数据切割等分析设置。

Result: 提出了一种基于变换器（transformer）的灵活架构及训练策略，能在推理时适应不同分析设置，并成功应用于引力波参数估计，单模型Dingo-T1可处理多种配置下的48个事件、研究探测器设置对后验的影响、检验广义相对论，同时将实际事件的采样效率中值从1.4%提升至4.2%。
...
### [32] [Inflationary assessment of $F(\mathcal{R},\tilde{\mathcal{R}})$ Einstein-Cartan models](https://arxiv.org/abs/2512.02847)
*Theodoros Katsoulas,Kyriakos Tamvakis*

**TL;DR**: 论文在含立方霍尔斯特不变曲率项（Holst-invariant curvature）的F(ℛ,ℛ̃)爱因斯坦-嘉当引力（Einstein-Cartan gravity）框架下，发现特定参数区间的立方项能修正二次模型与观测数据的不符，使暴胀预测与当前观测一致。


<details>
  <summary>Details</summary>

Background: 研究探讨了$F(\mathcal{R},\tilde{\mathcal{R}})$爱因斯坦-嘉当引力（Einstein-Cartan gravity）框架下含霍尔斯特不变量（Holst-invariant）曲率立方项的模型对暴胀行为的潜在影响。

Data: data部分：研究使用理论模拟（simulation）数据，基于$F(\mathcal{R},\tilde{\mathcal{R}})$爱因斯坦-嘉当引力框架，通过引入霍尔斯特不变量曲率（Holst-invariant curvature）的立方项构建模型参数。

Method: method：在$F(\mathcal{R},\tilde{\mathcal{R}})$爱因斯坦-嘉当(Einstein-Cartan)引力框架下，采用包含霍尔斯特不变量(Holst-invariant)曲率立方项的作用量模型分析暴胀行为。

Result: 在含立方项（cubic terms）的$F(\mathcal{R},\tilde{\mathcal{R}})$爱因斯坦-嘉当引力（Einstein-Cartan gravity）模型中，这些项能显著改善与观测数据的吻合度，弥补仅含二次项模型在特定参数区间的不足。
...
### [33] [Singular $α$-attractors](https://arxiv.org/abs/2512.02969)
*Renata Kallosh,Andrei Linde*

**TL;DR**: 论文提出了一类边界处可能奇异（S-models）的更一般势能模型，解决了α-吸引子（α-attractors）初始条件问题，并能解释ACT、SPT和DESI数据中广泛的原初密度扰动谱指数（n_s）值范围。


<details>
  <summary>Details</summary>

Background: 研究探讨了超引力理论中具有双曲几何的α吸引子模型（α-attractor models）及其在暴胀子场（inflaton field）势能结构中的多样化表现。

Data: 研究中使用的数据来源于理论模型构建（simulation），包括T-模型、E-模型、P-模型和S-模型的势能函数及其在模空间边界的行为分析。

Method: 采用具有模空间边界奇异性的S-模型势能（S-models potentials）分析暴胀子动力学，拓展了传统多项式（P-models）和指数型（T/E-models）α-吸引子的势能结构。

Result: S-模型（S-models）通过引入模空间边界处的奇异势能，为α-吸引子（α-attractors）初始条件问题提供了简单解，并能解释与ACT、SPT和DESI数据匹配的广泛$n_{s}$值范围。
...
### [34] [Padé Approximants for Fast Radio Bursts Diffuse Dispersion Measure](https://arxiv.org/abs/2512.02357)
*Marios Kalomenopoulos,Jiaming Zhuge*

**TL;DR**: 该论文推导了适用于平坦ΛCDM和wCDM宇宙模型的快速射电暴(FRB)弥散量(DM_diff)解析近似公式，在红移0.01≤z≤2、物质密度参数0.2≤Ω_m≤1.0和状态方程参数-3.0≤w≤-0.5范围内相对误差小于3.5%，计算速度比数值解快15倍(ΛCDM)或2倍(wCDM)。


<details>
  <summary>Details</summary>

Background: 快速射电暴（FRBs）是研究宇宙电离特性和宇宙学参数的重要工具。

Data: 研究中使用的数据来源于对快速射电暴（FRBs）的观测数据，通过分析其扩散色散量（DM_diff）随红移的变化关系。

Method: 推导了适用于平坦ΛCDM和wCDM宇宙的解析近似公式来计算弥散量（DM_diff）。

Result: 推导出适用于平坦ΛCDM和wCDM宇宙的解析近似公式，用于计算快速射电暴(FRB)的弥散量(DM_diff)，在0.01≤z≤2、0.2≤Ω_m≤1.0和-3.0≤w≤-0.5范围内相对数值解的误差小于3.5%，且计算速度比数值解快15倍(ΛCDM)或2倍(wCDM)。
...
### [35] [The role of the galaxy stellar mass function in determining the cosmological distribution of astrophysical transients with applications to fast radio bursts and merging binary black holes](https://arxiv.org/abs/2512.02642)
*Sandeep Kumar Acharya*

**TL;DR**: 研究发现考虑宿主星系质量函数（galaxy stellar mass function）及其红移演化后，快速射电暴（FRBs）的单位恒星质量形成效率需提高约3倍才能解释z=0处的观测体积率，并指出忽略宿主星系信息会导致宇宙学种群研究出现偏差。


<details>
  <summary>Details</summary>

Background: 研究探讨了快速射电暴(FRB)等致密天体宇宙学分布和形成率的宿主星系(host galaxy)恒星质量函数(galaxy stellar mass function)及其红移演化的重要性。

Data: 数据来源于观测到的快速射电暴（FRBs）的宿主星系信息及其红移演化，以及文献中常用的仅作为红移函数的恒星形成率模板。

Method: 采用星系恒星质量函数（galaxy stellar mass function）结合快速射电暴（FRBs）宿主星系信息和星系恒星质量的红移演化进行建模分析。

Result: 快速射电暴（FRB）的恒星质量形成效率需比先前计算值高约3倍才能重现z=0处的观测体积率。
...
### [36] [Search for persistent radio emission towards selected localised Fast Radio Burst positions using the MeerKAT Telescope](https://arxiv.org/abs/2512.02693)
*Thulo Letsele,Lebogang Mfulwane,Christo Venter,James O. Chibueze,Mechiel Christiaan Bezuidenhout*

**TL;DR**: 这篇论文通过MeerKAT射电望远镜数据发现2个可能的持续射电源（PRS）候选体并确认1个未探测案例（涉及25个FRB位置），指出FRB20221106和FRB20181112存在宿主星系但需高分辨率观测（如e-MERLIN）区分射电连续辐射来源是否为PRS，同时为FRB220190102提供了两个观测时段的流量上限。


<details>
  <summary>Details</summary>

Background: 研究主题为快速射电暴（FRB）及其可能的持久射电源（PRS）伴星，旨在通过PRS特征揭示FRB的前身星、局域环境及演化机制。

Data: 数据来源于MeerKAT射电望远镜对25个FRB位置的观测，包括FRB20221106和FRB20181112的两个候选PRS探测结果以及FRB220190102的两个未探测结果（提供了流量上限）。

Method: 采用MeerKAT射电望远镜数据检测候选持久射电源（PRS），并通过e-MERLIN高分辨率观测确定其尺寸、通量变化和光谱形态。

Result: 发现了两个候选持久射电源（PRS）和一个非探测结果（针对25个FRB位置研究的一部分）。
...
### [37] [Slow-Light Effect in the Jet-Launching Region of M87](https://arxiv.org/abs/2512.02113)
*Yuh Tsunetoe,Dominic W. Pesce,Ramesh Narayan*

**TL;DR**: 研究发现慢光（slow-light）辐射传输处理能更准确模拟M87喷流观测结构，揭示喷流形态从黑洞附近环状主导过渡到远距离边缘主导的双锥结构，且喷流摆动幅度与黑洞自旋（spin）呈负相关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于广义相对论磁流体动力学（GRMHD）模型中慢光辐射传输对M87喷流可观测结构的影响及其与黑洞自旋的关联。

Data: 数据来源于广义相对论磁流体动力学（GRMHD）模拟中对M87喷流的慢光（slow-light）辐射传输计算。

Method: 采用广义相对论辐射传输（GRRT）方法结合慢光（slow-light）处理技术，在广义相对论磁流体力学（GRMHD）框架下模拟M87喷流中流体演化与光线传播的耦合效应。

Result: 慢光（slow-light）辐射传输处理揭示了M87喷流中从亚相对论到相对论速度的等离子体转变半径会在图像上表现为由黑洞附近小尺度环状主导过渡到大尺度边缘主导形态，且环-边转变距离随黑洞自旋减小而增大。
...
### [38] [AT2022zod: An Unusual Tidal Disruption Event in an Elliptical Galaxy at Redshift 0.11](https://arxiv.org/abs/2512.02136)
...
**TL;DR**: 在椭圆星系中发现的极端短时光学耀变AT2022zod最可能由超致密矮星系(UCD)内的中等质量黑洞(MBH)潮汐瓦解恒星产生，为中等质量黑洞搜寻提供了关键案例。


<details>
  <summary>Details</summary>

Background: 潮汐瓦解事件(TDEs)作为探测黑洞(尤其是中等质量黑洞)的重要探针，为研究高红移下黑洞的分布与行为特征提供了关键窗口。

Data: 数据来源于对椭圆星系中AT2022zod极端短时光学耀变（optical flare）的观测，该星系位于红移z=0.11处，耀变发生在星系中心3kpc范围内。

Method: 采用光学测光(photometry)分析结合潮汐瓦解事件(TDE)已知辐射机制模型，通过光度演化时标和空间位置约束黑洞质量范围。

Result: AT2022zod最可能的起源是恒星被超致密矮星系（UCD）中的中等质量黑洞（MBH）潮汐瓦解。
...
### [39] [Linearly Polarized Gravitational Waves from Bubble Collisions](https://arxiv.org/abs/2512.02137)
*Katarina Trailović*

**TL;DR**: 研究发现LISA和爱因斯坦望远镜（Einstein Telescope）等三角干涉仪可探测仅由两个真空泡（vacuum bubbles）碰撞产生的线性偏振引力波（GWs），为揭示早期宇宙相变（first-order phase transitions）起源提供关键证据。


<details>
  <summary>Details</summary>

Background: 研究探讨了标准模型（Standard Model）之外的物理现象可能引发的真空泡成核导致的一阶相变（first-order phase transitions），以及由此产生的引力波（GWs）对早期宇宙研究的潜在意义。

Data: 数据来源为LISA（激光干涉空间天线）和爱因斯坦望远镜（Einstein Telescope）的三角干涉仪观测数据。

Method: 采用线性极化引力波信号分析方法，通过LISA和爱因斯坦望远镜（Einstein Telescope）等三角干涉仪测量引力波幅度并确定其线性极化（linear polarization）。

Result: 在仅两个真空泡（vacuum bubbles）成核和碰撞的慢速相变（phase transition）情况下，LISA和爱因斯坦望远镜（Einstein Telescope）等三角干涉仪可测量引力波（GWs）的线性偏振（linear polarization），从而为信号起源提供重要线索。
...
### [40] [Exploring Leptogenesis, WIMP Dark Matter, and Gravitational Waves in an extended Scalar Framework](https://arxiv.org/abs/2512.02672)
*Subhaditya Bhattacharya,Niloy Mondal,Arunansu Sil*

**TL;DR**: 该研究通过引入标量媒介子（Φ）和复杂标量暗场（S）扩展I型跷跷板机制，提出基于$\mathcal{Z}_4\times CP$对称性的模型框架，关联中微子质量生成、轻子生成和暗物质候选者（S的稳定分量），并预言对称性破缺产生的畴壁湮灭可探测引力波信号。


<details>
  <summary>Details</summary>

Background: 研究探讨了I型跷跷板机制（type I seesaw）的扩展框架，通过引入标量媒介子（$Φ$）连接复杂标量暗场（$S$）和右手中微子（$N_i$），旨在关联中微子质量生成、轻子生成（leptogenesis）和暗物质现象。

Data: 研究中使用的数据来源于理论模拟（simulation）框架下的对称性破缺和粒子相互作用模型。

Method: 采用扩展对称性$\mathcal{Z}_4\times CP$模型，通过标量媒介子($Φ$)与暗物质场($S$)及右手中微子($N_i$)耦合，构建五维有效相互作用项$\bar{l}_L^α\tilde{H}ΦN_i$，并自发破缺$\mathcal{Z}_4$对称性以影响中微子质量生成和轻子生成，同时利用$CP$对称性稳定$S$的一个分量作为暗物质候选者。

Result: $\mathcal{Z}_4\times CP$对称性扩展能同时实现中微子质量生成、轻子生成和暗物质候选者稳定化，并通过标量介子$\Phi$的真空期望值（VEV）自发破缺影响前两者，而$CP$对称性稳定了$S$的一个分量作为暗物质。
...
### [41] [Modeling dark matter halos with self-interacting fermions](https://arxiv.org/abs/2512.03006)
*Fabian Hernandez-Gutierrez,Juan Barranco*

**TL;DR**: 提出了一种由自引力、自相互作用费米子组成的核-晕（core-halo）暗物质（dark matter）模型，通过拟合低表面亮度星系（Low Surface Brightness galaxies）的旋转曲线确定了费米子质量（fermion mass）和相互作用强度的最佳范围，并验证了该模型在银河系和SPARC数据库中的适用性。


<details>
  <summary>Details</summary>

Background: 研究探索了将星系中的暗物质（dark matter）建模为由自引力、自相互作用费米子（self-interacting fermions）组成的核-晕（core-halo）模型的可能性。

Data: 数据来源包括低表面亮度星系（Low Surface Brightness galaxies）的旋转曲线数据、银河系（Milky Way）观测数据以及SPARC数据库（SPARC database）中的星系数据。

Method: 采用核心-晕模型（core-halo model）结合自引力自相互作用费米子（self-gravitating, self-interacting fermions），核心部分通过求解托尔曼-奥本海默-沃尔科夫方程（Tolman-Oppenheimer-Volkoff equations）获得密度分布，晕部分分别考虑热化理想气体（thermalized ideal gas）或标准冷暗物质纳瓦罗-弗伦克-怀特轮廓（Navarro-Frenk-White profile）。

Result: 暗物质费米子（fermion）质量在38.73 eV至42.11 eV之间且粒子间相互作用强度在269.69至348.48范围内时，能较好地拟合低表面亮度星系（Low Surface Brightness galaxies）的旋转曲线。
...
### [42] [Hidden in Plain Sight II: Characterizing the luminous companion to Kappa Velorum with VLTI/GRAVITY](https://arxiv.org/abs/2512.02116)
*D. M. Rowan,S. Kraus,Todd A. Thompson*

**TL;DR**: 通过VLTI GRAVITY直接观测证实南天亮星Kappa Velorum（HD 81188）的双星系统中伴星为B型主序星而非致密天体（compact object），并结合天体测量轨道与视向速度数据精确测定其轨道参数（周期116.795±0.002天、偏心率0.1764±0.0004、倾角74.04±0.01°）及双星质量（主星10^{+4}_{-2} M⊙、伴星6.9±1.0 M⊙）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用干涉测量技术揭示银河系内隐藏的恒星质量黑洞（stellar-mass black holes）候选体，以Kappa Velorum（HD 81188）双星系统为案例验证方法论可行性。

Data: 数据来源包括VLTI GRAVITY干涉仪观测数据、Hipparcos卫星视差测量数据（$\varpi = 6.05\pm0.48$ mas）以及存档的UVES光谱数据。

Method: 采用VLTI GRAVITY干涉观测直接解析双星系统，并结合相对天体测量轨道与历史径向速度数据，利用MIST演化轨迹和紫外光谱(UVES)优化质量估计。

Result: result：利用VLTI GRAVITY观测排除了Kappa Velorum存在致密天体伴星的可能性，确认其伴星为一颗主序B型星，并测得轨道周期P=116.795±0.002天、偏心率e=0.1764±0.0004、倾角i=74.04±0.01°，主星质量M₁=10⁺⁴₋₂ M☉、伴星质量M₂=6.9±1.0 M☉。
...
### [43] [Untangling the Sources of Abundance Dispersion in Low-metallicity Stars II: Neutron Capture Elements](https://arxiv.org/abs/2512.02122)
*Emily J. Griffith,Marissa Blum,David H. Weinberg,Jennifer A. Johnson,Tawny Sit,Ilya Ilyin,Klaus G. Strassmeier*

**TL;DR**: 该研究通过高精度光谱分析发现贫金属次巨星（-2 < [Fe/H] < -1）中s-和r-过程重元素（如Eu）的本征丰度弥散（0.11-0.27 dex）显著高于轻元素，表明超新星前身星质量、自转和磁场强度的随机取样以及核合成源比例差异共同导致了这种弥散特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于贫金属（metal-poor）次巨星中重元素丰度分布及其内禀散射（intrinsic scatter）的起源，探讨核合成源比例差异及超新星前身星性质（如质量、自转、磁场强度）随机采样对元素丰度模式的影响。

Data: 数据来源于使用大型双筒望远镜（Large Binocular Telescope）和波茨坦阶梯光栅偏振光谱仪（Potsdam Echelle Polarimetric and Spectroscopic Instrument）获取的高信噪比（high-SNR）和高分辨率光谱。

Method: 采用KORG（1D-LTE光谱合成软件包）对LBT望远镜PEPSI仪器获取的高信噪比高分辨率光谱进行元素丰度分析，并通过剔除s过程显著增强的两颗恒星后测量本征丰度弥散度。

Result: 重元素（s-和r-过程元素）的本征丰度弥散（0.11至0.27 dex）比轻元素更大，可能源于核合成源比例差异或前身超新星性质（质量、自转、磁场强度）的随机采样。
...
### [44] [SCExAO/CHARIS and Gaia Direct Imaging and Astrometric Discovery of a Superjovian Planet 3--4 lambda/D from the Accelerating Star HIP 54515](https://arxiv.org/abs/2512.02159)
...
**TL;DR**: 在年轻A5型恒星HIP 54515周围发现了一颗质量17.7^{+7.6}_{-4.9}倍木星质量、半长轴约25天文单位的中等偏心(e≈0.4)超木星行星(superjovian planet)HIP 54515 b，为研究大质量行星的光度演化和偏心分布提供了新样本。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高精度天体测量（precision astrometry）和高对比度成像技术（high-contrast imaging）探测年轻A5型恒星HIP 54515周围的超级木星（superjovian planet），以完善大质量行星的光度演化与轨道偏心率的经验约束。

Data: 数据来源包括《依巴谷-盖亚加速星表》（Hipparcos Gaia Catalogue of Accelerations）的高精度天体测量数据以及SCExAO/CHARIS高对比度成像仪在\surveyname\项目中获取的观测数据。

Method: 采用高精度天体测量（precision astrometry）和高对比度成像（high-contrast imaging）技术，结合SCExAO/CHARIS系统进行观测，并通过动力学模型（dynamical modeling）约束行星质量和轨道参数。

Result: HIP 54515 b是一颗质量约为17.7（误差范围-4.9至+7.6）倍木星质量、位于M/L型过渡区域、轨道半长轴约25天文单位且具有中等偏心率（e≈0.4）的超木星行星。
...
### [45] [Segmented-Polynomial-fitting Least Squares (SPLS): An optimized algorithm to find Earth twins](https://arxiv.org/abs/2512.02356)
*Shuyue Zheng,Fabo Feng,Yicheng Rui*

**TL;DR**: 提出分段多项式拟合最小二乘法（SPLS）算法，通过分段双多项式模型同步拟合行星凌星和背景趋势，在信噪比低于9的10-480天周期信号检测中比传统方法真阳性率提高至少22.6%，显著提升低信噪比长周期凌星行星（如地球双星Earth twins）的探测能力。


<details>
  <summary>Details</summary>

Background: 研究主题为开发新型算法（SPLS）以提高对低信噪比（SNR）长周期凌星信号（如类地行星）的检测能力。

Data: 数据来源于开普勒(Kepler)、TESS以及即将开展的PLATO和Earth 2.0任务的观测数据。

Method: 采用分段双多项式模型（segmented double polynomial model）的Segmented-Polynomial-fitting Least Squares (SPLS)算法，结合贝叶斯因子（Bayes factor）模型比较选择最优多项式阶数，并通过三阶近似和非线性参数采样加速计算。

Result: SPLS算法在信噪比低于9、轨道周期10-480天的信号检测中，相比传统方法在相同10%假阳性率下实现了至少22.6%的真阳性率提升。
...
### [46] [JCMT Constraints on the Early-Time HCN and CO Emission and HCN Temporal Evolution of 3I/ATLAS](https://arxiv.org/abs/2512.02106)
*Jason T. Hinkle,Bin Yang,Karen J. Meech,Andrew Hoffman,Benjamin J. Shappee,W. B. Hoogendam,James J. Wray*

**TL;DR**: 利用JCMT望远镜对星际天体(ISO)3I/ATLAS进行亚毫米波段观测，首次获得其HCN和CO释放量的早期上限约束，并结合后续数据测得HCN释放率随日心距离变化的陡峭斜率(n=12.7)，显著区别于典型太阳系彗星。


<details>
  <summary>Details</summary>

Background: 星际物体（ISOs）尤其是具有彗星活动的个体为研究其起源行星系统形成时期的原始物理化学条件提供了独特视角。

Data: 数据来源于使用詹姆斯·克拉克·麦克斯韦望远镜（JCMT）上的`Ū`ū外差接收器对星际天体（ISO）3I/ATLAS进行的亚毫米波段观测，目标为HCN（$J = 3 - 2$）和CO（$J = 2 - 1$）的发射线，观测时间为2025年7月16日至21日（世界时），当时3I/ATLAS的日心距离在4.01至3.84天文单位之间。

Method: 采用亚毫米波段外差接收器（heterodyne receiver）对HCN($J = 3 - 2$)和CO($J = 2 - 1$)发射线进行观测，并结合$Q(HCN) \propto r_h^{-n}$模型拟合HCN生成速率随日心距离的演化。

Result: 对星际天体3I/ATLAS的亚毫米观测未检测到HCN和CO释放，其HCN生成率随日心距离变化的指数(n=12.7^{+6.9}_{-2.5})比太阳系彗星更陡峭。
...
### [47] [Effect of the avoided crossing on the rovibrational energy levels, resonances, and predissociation lifetimes within the ground and first excited electronic states of lithium fluoride](https://arxiv.org/abs/2512.02085)
*V. G. Ushakov,A. Yu. Ermilov,E. S. Medvedev*

**TL;DR**: 通过从头算(ab initio)方法计算了LiF分子在1-17玻尔半径范围内的绝热势能面，结合双态相互作用模型精确预测了束缚态能量及两类共振态（隧穿共振和预解离共振）的位置与宽度，其计算的X-X振转跃迁谱线与实验数据高度吻合（标准偏差小于0.001 cm$^{-1}$）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于氟化锂(LiF)分子在第一解离限以上7800 cm$^{-1}$范围内的光谱特性及其电子态相互作用机制。

Data: 研究采用从头算（ab initio）方法计算了原子间距（interatomic separation）在1-17玻尔半径（bohr）范围内的绝热势能（adiabatic potentials）及其他分子函数，并基于两个相互作用的电子态模型（包括束缚态和两种共振态：隧穿共振（tunneling resonances）和预解离共振（predissociative ones））构建了由两个函数模拟的玻恩-奥本海默势（Born-Oppenheimer potentials），其中非绝热耦合矩阵（nonadiabatic coupling matrix）积分得到的绝热-非绝热基旋转角（angle of the adiabatic-to-diabatic basis rotation）定义了最终的非绝热项（diabatic terms）和耦合项（diabatic coupling）。

Method: 采用基于双电子态相互作用模型（包括束缚态和隧穿共振态、预解离共振态）的非绝热耦合计算方法，通过积分非绝热耦合矩阵元获得绝热-非绝热基旋转角，重构最终的二态耦合项。

Result: 通过非绝热耦合矩阵元积分获得的绝热-非绝热基旋转角度，结合绝热势能，定义了最终的电子态（diabatic terms）及其耦合（diabatic coupling）。
...
### [48] [Limiting Reduction and Modified Gravity](https://arxiv.org/abs/2512.02871)
*Antonis Antoniou,Lorenzo Lorenzetti*

**TL;DR**: 论文核心贡献是分析修正牛顿动力学（MOND）因缺乏基本理论框架和统一数学结构而无法合理还原为牛顿引力，从而揭示了标准极限还原理论的局限性，并提出了更精细的还原框架标准。


<details>
  <summary>Details</summary>

Background: 研究探讨了修正牛顿动力学（MOND）作为解释星系旋转异常等现象的替代理论，在跨尺度上与牛顿引力等已确立理论间的还原关系及其合理性。

Data: 研究中使用的数据来源为理论分析（theoretical analysis），未涉及具体观测或模拟数据。

Method: 采用理论间还原（inter-theoretic reduction）分析方法，重点评估修正牛顿动力学（MOND）与牛顿引力理论在数学结构和基本框架上的还原关系，并提出区分成功还原与病态还原的两个新标准。

Result: 修正牛顿动力学（MOND）因缺乏基本理论框架证明其插值函数且未建立独立于牛顿理论的统一数学结构，未能以非任意方式充分还原为牛顿引力，从而未能满足理论间还原（inter-theoretic reduction）的关键条件。
