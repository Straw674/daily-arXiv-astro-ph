...
**TL;DR**: 研究发现高红移（$z\gtrsim4$）小红点（LRDs）中观测到的丰富活动星系核（AGN）可由FIRE-2模拟中的引力扭矩驱动吸积（GTDA）模型或自由落体吸积模型解释，且最佳匹配观测数据的场景表明LRDs是寄居于恒星质量$M_\star \gtrsim 2\times10^7 \ \rm M_\odot$星系中、爱丁顿光度受限的超爱丁顿吸积黑洞（$M_{\rm BH}\gtrsim 2\times10^5 \ \rm M_\odot$）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（$z\gtrsim4$）低质量星系中活跃星系核（AGN）的形成机制与理论模型预测间的矛盾，特别关注恒星反馈导致的气体反复驱逐对超大质量黑洞持续吸积的阻碍作用。

Data: 研究使用了高红移FIRE-2模拟（FIRE-2 simulations）中的星系数据，结合引力扭矩驱动吸积（GTDA）模型和简单自由落体吸积模型来推导黑洞吸积率并构建预测的AGN热光度函数。

Method: 采用重力扭矩驱动吸积（GTDA）模型和简单自由落体吸积模型推导黑洞吸积率并构建红移$z=5-7$的AGN（活动星系核）热光度函数预测。

Result: 高红移（$z\gtrsim4$）小紅点（LRDs）观测到的丰富活动星系核（AGN）数量可通过引力扭矩驱动吸积（GTDA）模型或自由落体吸积模型（黑洞每自由落体时标吸积中心气体供应的$\lesssim 1\%$）得到合理解释，但基准模型对低光度AGN数量的预测高于观测值。
...
**TL;DR**: 基于詹姆斯·韦伯太空望远镜(JWST)新发现的"小红点"(LRDs)和"小蓝点"(LBDs)类活动星系核(AGN)的比较分析表明，它们具有与标准I型AGN相同的中心引擎，但在吸积盘周围致密气体的含量、几何结构或吸积特性上存在差异。


<details>
  <summary>Details</summary>

Background: 研究聚焦于詹姆斯·韦伯太空望远镜（JWST）新发现的具有特殊性质的活动星系核（AGN）群体，包括"小红点"（LRDs）和"小蓝点"（LBDs），旨在揭示它们与标准I型AGN的异同及其物理机制。

Data: 数据来源于詹姆斯·韦伯太空望远镜（JWST）新发现的活跃星系核（AGN）样本，包括红移（redshift）分别为2.26和5.55的原型目标GN-28074（Red Rosetta Stone）和GS-3073（Blue Rosetta Stone）。

Method: 采用比较分析（comparative analysis）方法研究两类原型的典型代表（Rosetta Stones），包括光谱拟合（如指数轮廓与高斯轮廓对比）和激发诊断（如HeII 4686线检测）。

Result: LRDs（Little Red Dots）和LBDs（Little Blue Dots）具有与标准I型活动星系核（AGN）相同的中心引擎，但在吸积盘周围致密气体的数量与几何结构或吸积特性上存在差异。
...
**TL;DR**: 研究发现早期型射电星系(early-type radio galaxies)中不规则尘埃带(dust lanes)无宿主星系对齐偏好但垂直于喷流(radio jets)，而规则尘埃盘/环(disks/rings)倾向于与宿主星系主轴对齐但与喷流取向多变，表明并合(dusty material from mergers)注入的尘埃物质可影响喷流角度并解释活动星系核(AGN)反馈在维持大质量星系静止状态(quiescence)中的作用。


<details>
  <summary>Details</summary>

Background: 研究主题聚焦于射电喷流（radio jets）与宿主星系（host galaxies）的相对取向关系，以此探索活动星系核（AGN）与周围环境的相互作用机制。

Data: data：研究使用哈勃太空望远镜（Hubble Space Telescope）对早期型射电星系（early-type radio galaxies）进行的新成像数据，并通过高度自动化的流程测量位置角（position angles）。

Method: 采用哈勃空间望远镜(Hubble Space Telescope)成像和自动化位置角测量技术，结合核区尘埃特征（尘带、盘状结构、环状结构）形态分类分析方法。

Result: 尘埃通道（dust lanes）与喷流（jets）呈优先垂直关系，但与宿主星系无特定对齐模式。
...
**TL;DR**: 该研究通过分析40个动力学确认的极环星系（PRGs）的中性氢（H I）数据，发现它们主要位于绿谷或静止区域，气体分数高于典型早型星系且H I轮廓不对称性显著，表明存在气体吸积，并揭示极环星系中气体分布假设对图利-费舍尔关系（TFR）离散度的重要影响。


<details>
  <summary>Details</summary>

Background: 极环星系（PRGs）作为极端不对齐系统的典型代表，为研究星系相互作用、气体吸积和特殊气体动力学提供了关键窗口。

Data: 数据来源包括文献资料和FAST望远镜的新观测数据。

Method: 结合文献数据与FAST新观测，采用H I（中性氢）谱线测量分析PRGs（极环星系）的气体动力学特性，重点研究H I发射、吸收特征及非探测案例。

Result: 极环星系（PRGs）的H I气体分数高于典型早型星系，表明存在气体吸积过程。
...
**TL;DR**: 分析了194个红移z<1.5的X射线明亮活动星系核(AGN)的光谱能量分布(SED)、星系光轮廓和视觉形态分类，发现94%的AGN具有紫外和/或中红外辐射减弱的遮蔽型SED，仅6%呈现类星体状非遮蔽SED，且约三分之一星系处于相互作用状态，同时识别出一类疑似合并后球状星系具有紫外和红外遮蔽SED特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于红移(z)小于1.5的X射线明亮活动星系核(AGN)的光谱能量分布(SEDs)、宿主星系形态及其与吸积活动的关系。

Data: 数据来源于GOODS天区（Great Observatories Origins Deep Survey）中红移z小于1.5的194个X射线明亮活动星系核（AGN）的光谱能量分布（SEDs）、星系光轮廓及视觉形态分类。

Method: 采用光谱能量分布(SED)分析、星系光剖面拟合及视觉形态分类方法，结合galight软件计算核球-总光比(bulge-to-total light ratio)。

Result: 大多数（94%）GOODS活动星系核（AGN）表现出被遮蔽的光谱能量分布（SED），其特征是紫外和/或中红外辐射减弱，而只有6%表现出未被遮蔽的类类星体SED。
...
### [6] [The LBT $Y_{\rm p}$ Project II: MODS Spectra, Physical Conditions, and Oxygen Abundances in Local Metal-Poor Nebulae](https://arxiv.org/abs/2601.22236)
...
**TL;DR**: 通过分析62个低金属丰度（O/H≤20%太阳丰度）星系的光谱数据，建立了电子温度（Te[S III]-Te[O III]）和密度（ne[Ar IV]）的新标度关系，为精确测量原初氦质量分数（Yp）和星际介质元素丰度提供了关键数据支持。


<details>
  <summary>Details</summary>

Background: 研究主题为通过测量低金属丰度星系的氦（He）和氧（O）丰度来实证确定原始氦质量分数（$Y_{\rm p}$）。

Data: 数据来源于使用大型双筒望远镜(LBT)上的多目标双光谱仪(MODS)获取的62个低金属丰度(低于20%太阳O/H丰度)星系的深度静止光学光谱，作为LBT $Y_{\rm p}$项目的一部分。

Method: 采用深静止光学光谱（deep rest-optical spectroscopy）结合多目标双光谱仪（MODS）对62个低金属丰度星系进行观测，并开发新拟合方法恢复61条氢和氦复合线强度。

Result: 在低金属丰度星际介质(ISM)中，[Ar IV]测得的电子密度(n_e)系统性高于[S II]或[O II]测得的数值。
...
### [7] [Rotational Spectroscopy as a Tool to Study Vibration-Rotation Interaction: Investigations of $^{13}$CH$_3$CN and CH$_3$$^{13}$CN up to $v_8 = 2$ and a Search for $v_8 = 2$ Transitions toward Sagittarius B2(N)](https://arxiv.org/abs/2601.22824)
*Holger S. P. Müller,Arnaud Belloche,Frank Lewen,Stephan Schlemmer*

**TL;DR**: 该研究通过分析人马座B2(N)中$^{13}$CH$_3$CN和CH$_3$$^{13}$CN在$v_8=1$和$v_8=2$振动态的转动跃迁，精确测定了同位素变体的能级间距（22.93和21.79 cm$^{-1}$），并提供了改进的基态转动跃迁频率及新同位素分子数据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星形成区中甲基氰（methyl cyanide, CH₃CN）及其同位素分子在振动激发态（v₈=1和v₈=2）的转动跃迁特性及其相互作用机制。

Data: 数据来源于35至1091 GHz范围内富集$^{13}$CH$_3$CN和CH$_3$$^{13}$CN样品的谱线记录、1085至1200 GHz范围内天然同位素组成CH$_3$CN的已有谱线重新分析以及对人马座B2(N)中$^{13}$CH$_3$CN和CH$_3$$^{13}$CN在$v_8 = 2$态跃迁的搜寻结果。

Method: 采用旋转光谱(rotational spectroscopy)技术分析$^{13}$CH$_3$CN和CH$_3$$^{13}$CN在$v_8=1$和$v_8=2$振动激发态下的能级结构，并通过近简并(near-degeneracy)扰动和费米共振(Fermi-type interaction)精确测定同位素变体的能级间距。

Result: 在射手座B2(N)区域观测到了$^{13}$CH$_3$CN和CH$_3$$^{13}$CN在$v_8 = 1$最低振动激发态($E_{\rm vib} \approx 520$ K)的转动跃迁信号。
...
### [8] [The LBT $Y_{\rm p}$ Project III: LUCI Spectra of Metal-Poor Nebulae](https://arxiv.org/abs/2601.22237)
...
**TL;DR**: 通过分析48个低金属丰度星系的近红外光谱并精确测量HeI λ10830线通量，显著提升了氦丰度测定精度（中位通量比不确定度降至0.08），为推算原初氦丰度（Y_p）提供了更可靠的数据基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过近红外(near-IR, NIR)氦线(HeI λ10830)精确测量低金属丰度(low metallicity)星云的元素丰度，以解决气体密度(n$_e$)和温度(T$_e$)的简并性问题并提升原始氦丰度(primordial helium abundance, $Y_{\rm p}$)的测定可靠性。

Data: 数据来源于大型双筒望远镜（LBT）的近红外（NIR）LUCI光谱仪观测的48个低金属丰度目标，并利用Pypeit进行均匀化处理，通过手动波长校准使用明亮的OH发射线，仔细应用大气校正，并在重采样网格上叠加LUCI1和LUCI2的光谱以确保准确性。

Method: 利用近红外(NIR) LUCI光谱仪观测数据，结合高斯轮廓拟合发射线并测量相对于帕邢γ(Pγ)的流量比，重点分析氦I λ10830线的密度敏感性特征。

Result: 通过近红外(NIR) LUCI光谱分析低金属丰度星系的HeI λ10830线，显著提升了氦丰度测定的精度（中位通量比不确定度σ̃=0.08），为推断原初氦丰度(Y_p)提供了更可靠的数据基础。
...
**TL;DR**: 通过扩展甲基丙二烯（methylallene）的转动谱线列表并精确测定其光谱参数（包括甲基内转动参数），为在冷暗云TMC-1等星际环境中探测该分子提供了可靠依据。


<details>
  <summary>Details</summary>

Background: 研究背景是探索冷暗云TMC-1中多环芳烃(PAHs)和氰基衍生物的形成机制，特别是甲基丙二烯(CH$_3$CHCCH$_2$)作为潜在前体分子的光谱特征及其在星际介质中的检测可能性。

Data: 数据来源于在36-501 GHz频段记录的甲基丙二烯（methylallene）旋转跃迁观测数据，并结合量子化学计算（quantum-chemical calculations）评估初始光谱参数。

Method: 采用量子化学计算（quantum-chemical calculations）和旋转光谱测量（rotational transitions measurement）技术，扩展了甲基丙二烯（methylallene）的谱线列表并确定了其内部旋转分裂（internal rotation splitting）及高阶畸变参数（distortion parameters）。

Result: 在冷暗云TMC-1中检测到小型多环芳烃(PAHs)和较大氰基衍生物的存在，但其形成机制尚不明确。
...
### [10] [Fast, dust-poor outflows in the local candidate dual AGN MCG-03-34-64 observed with VLT/ERIS](https://arxiv.org/abs/2601.22254)
...
**TL;DR**: 通过甚大望远镜(VLT)ERIS仪器观测发现近邻赛弗特星系MCG-03-34-64中存在两个相距约100秒差距的活动星系核(AGN)候选体，其电离气体外流速度达-1700 km/s，质量约4×10^5太阳质量，外流率20太阳质量/年，且[Fe VII]6087A日冕线特征表明外流区尘埃含量低于宿主星系星际介质。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近邻赛弗特星系（Seyfert galaxy）MCG-03-34-64中候选双活动星系核（dual AGN）系统的电离气体运动学特征及其高速外流现象。

Data: 数据来源为甚大望远镜(VLT)的增强分辨率成像和光谱仪(ERIS)在J波段对MCG-03-34-64的积分场单元(IFU)观测数据，以及存档的VLT/X-shooter光谱数据。

Method: 采用积分场光谱仪(IFU)观测和离子化气体运动学分析，结合[Fe VII]6087A光学冕线谱线轮廓比较研究。

Result: 在MCG-03-34-64星系中观测到两个空间位置与候选双活动星系核(AGN)重合的高速外流区域，其最大速度达-1700 km/s，离子相外流质量均为$(4\pm1)\times 10^5$ $M_{\odot}$，质量外流率为$20\pm5$ $M_{\odot}$/年。
...
**TL;DR**: 通过结合动力学质量约束与五种前主序恒星演化模型，研究发现天蝎座上部(Upper Sco) K型和M型前主序星的等龄线年龄更支持8-11 Myr的较老年龄，其中考虑全球磁场(global magnetic fields)的模型表现最佳（9-10 Myr），且质量约束显著降低了不同模型间的年龄离散度，证实原行星盘寿命可能比此前认知长2倍以上。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用恒星动力学质量约束等龄线年龄（isochronal age）的方法，重新评估上蝎座（Upper Scorpius）恒星形成区23颗K型和M型前主序星（pre-main sequence stars）的年龄，以解决不同恒星演化模型对年轻恒星年龄估算的差异问题。

Data: 数据来源于对Upper Scorpius恒星形成区23颗K型和M型前主序星（pre-main sequence）的观测，结合五组前主序恒星演化模型（stellar evolutionary models）的等龄线（isochronal ages）分析，并利用恒星动力学质量（stellar dynamical masses）作为独立约束条件。

Method: 采用恒星动力学质量约束五种前主序恒星演化模型（pre-main sequence stellar evolutionary models）的等龄线年龄，并重点评估了考虑全球磁场（global magnetic fields）的模型表现。

Result: 考虑全局磁场（global magnetic fields）的恒星演化模型与观测结果最吻合，在无约束和质量约束条件下均得出9-10 Myr年龄。
...
**TL;DR**: 利用MUSE观测发现，恒星形成率（SFR）≥1 M$_{\odot}$ yr$^{-1}$的9个类似银河系质量的侧向星系中均存在延伸2-8 kpc的盘外气体（extraplanar gas），高SFR（≥5 M$_{\odot}$ yr$^{-1}$）系统的H$α$辐射更亮且范围更大，其双锥形态的气体速度弥散（$σ_{\rm Hα}$）符合外流（outflows）特征，而[OIII]/H$β$和[NII]/H$α$等谱线诊断显示盘外气体电离机制存在差异，表明反馈机制（如星暴的大尺度风或主序星系的小尺度喷泉）及激波加热共同作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星形成星系（star-forming galaxies）中盘外气体（extraplanar gas）的分布与性质，探讨恒星形成率（SFR）对气体动力学和电离机制的影响。

Data: 数据来源于欧洲南方天文台甚大望远镜(VLT)的多单元光谱探测器(MUSE)对九颗恒星形成星系的观测，分辨率约50-200 pc，样本选自GECKOS项目针对与银河系恒星质量相近的侧向星系。

Method: 采用多单元光谱探测器（MUSE）观测数据，结合窄波段成像和电离线比（[OIII]5007/Hβ - [NII]/Hα及σHα - [SII]/Hα）诊断技术，对盘外气体进行空间分辨分析。

Result: 在恒星形成率（SFR）≥1 M$_{\odot}$ yr$^{-1}$的星系中观测到延伸至盘面中平面2-8 kpc的强外盘气体（extraplanar gas）发射，且高SFR（≥5 M$_{\odot}$ yr$^{-1}$）系统表现出更亮、更延展的H$α$发射和双锥形态的气体速度弥散（$σ_{\rm Hα}$），符合外流（outflows）特征。
...
**TL;DR**: 研究发现原星团IRAS 15596-5301中两条丝状结构边缘发生非正面对撞，触发或加速了该区域恒星形成，且碰撞区密集核化学演化程度更高。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过观测原星团（protocluster）IRAS 15596-5301的分子云结构及动力学特征，探索丝状结构（filamentary structures）碰撞对恒星形成（star formation）的触发或加速机制。

Data: 数据来源于ALMA（阿塔卡马大型毫米波/亚毫米波阵列）870微米和3毫米波段观测数据。

Method: 采用高分辨率ALMA（阿塔卡马大型毫米波/亚毫米波阵列）870微米和3毫米数据，结合速度分辨积分强度图、位置-速度图及三维位置-位置-速度立方体数据分析，揭示了两个丝状结构的非正面碰撞。

Result: 在IRAS 15596-5301中，两个丝状结构边缘发生了非迎面相撞，触发或加速了恒星形成过程。
...
**TL;DR**: 研究发现小型HII区（R < 20 pc）与巨HII区遵循相同的大小-速度弥散关系，并揭示了年轻（< 0.5 Myr）和年老（> 0.5 Myr）HII区的湍流机制存在由恒星风和电离主导向膨胀动力学主导的演化转变。


<details>
  <summary>Details</summary>

Background: 研究探讨了小型HII区（物理半径R < 20 pc）与巨HII区在尺寸-速度弥散（size-velocity dispersion）关系上的异同及其演化过程中湍流主导机制的转变。

Data: 数据来源于LAMOST中分辨率光谱巡天（LAMOST MRS-N）数据集，该数据集具有大天区覆盖范围和高空间/光谱分辨率，用于分析10个物理半径小于20 pc的孤立银河系HII区。

Method: 利用LAMOST中分辨率光谱巡天（MRS-N）数据的高空间/光谱分辨率和大天区覆盖优势，对10个孤立的小型（R < 20 pc）银河系HII区进行了尺寸-速度弥散（σ）关系分析。

Result: 小型HII区(R < 20 pc)与巨HII区遵循相同的尺寸-速度弥散(size-σ)关系，可作为银河系HII区的新型距离指示器。
...
**TL;DR**: 这篇论文通过直接N体模拟和动力学算子校准，揭示了气体嵌入星团中原始双星系统的早期动力学演化主要由结合能（binding energy）和轨道周期（orbital period）主导，其中硬双星加热星团并抑制宽双星形成，而高质量比（high-q）系统增强、低质量比（low-q）系统易瓦解。


<details>
  <summary>Details</summary>

Background: 研究聚焦于气体嵌入星团（gas-embedded cluster）中原始双星（primordial binaries）的早期动力学演化及其对银河系场星（Galactic field）双星统计特性的影响。

Data: 数据来源于一组直接N体模拟（direct N-body simulations），模拟初始条件为100%原初双星（primordial binaries）并置于随时间变化的星团气体势场（gas-embedded cluster）中。

Method: 采用直接N体模拟（direct N-body simulations）结合时变气体势场（time-varying gas potential）的方法，并定义并校准了轨道周期、结合能和质量比的动力学算子（dynamical operators）来描述双星轨道属性的演化。

Result: 结合能（binding energy）和轨道周期（orbital period）主导星团中双星系统的演化，硬双星加热星团并抑制宽双星形成，而少量软双星残余得以存活，同时高质量比（high-q）系统倾向于增强而低质量比（low-q）系统易被瓦解。
...
**TL;DR**: 通过ALMA Band 7尘埃连续谱观测结合光谱能量分布分析，确认了5个红移(z)>3的超大质量星系(UMGs)中4个为极低尘埃质量比(Mdust/M⋆≲10⁻⁴)的真实静止星系，排除了尘埃遮蔽的恒星形成可能性，为早期宇宙快速尘埃清除和淬灭过程提供了观测约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（$z>3$）宇宙中超大质量静止星系（UMGs）的识别及其与尘埃遮蔽的恒星形成星系的区分问题。

Data: 使用了ALMA Band 7对MAGAZ3NE巡天中五颗光谱确认的$z_{\rm spec} > 3$超巨星系（UMGs）进行尘埃连续谱观测。

Method: 采用ALMA Band 7尘埃连续谱观测结合光谱能量分布(SED)分析和修正黑体拟合方法，确认了样本的静止状态并估算尘埃质量。

Result: 通过ALMA（阿塔卡马大型毫米波阵列）观测和光谱能量分布分析，确认了五颗超质量星系（UMGs）在红移z>3时处于真正静止状态，其特定恒星形成率（sSFR）低于星形成序一个数量级以上，排除了尘埃遮蔽的恒星形成可能性。
...
**TL;DR**: 通过近红外光谱观测发现六个近邻（z$\sim$0.1）射电宁静型2类类星体（QSO2s）存在高速电离气体外流（FWHM$\sim$1200-2500 km $s^{-1}$），其质量外流率（$\dot{M}_{Hion}\sim0.03-6~M_{\odot}~yr^{-1}$）和动能（$\dot{E}_{Hion}\sim 10^{37.8-40.8}~erg~s^{-1}$）显著高于[Si VI]线测得值，但未检测到分子外流且暖分子气体质量与其他QSO2s相当。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近邻（红移z∼0.1）射电宁静型2类类星体（QSO2s）的近红外光谱观测，旨在探究其核区高速外流（outflow）的动力学特征与物质输运机制。

Data: 数据来源于使用10.4米加那利大型望远镜(Gran Telescopio Canarias)上的多目标近红外光谱仪(Espectrógrafo Multiobjeto Infra-Rojo, EMIR)对六个邻近(z~0.1)射电宁静型2类类星体(QSO2s)进行的狭缝近红外光谱观测。

Method: 采用长缝近红外光谱观测技术（long-slit near-infrared spectroscopy）结合EMIR仪器对六个邻近（z$\sim$0.1）射电宁静型2类类星体（QSO2s）进行核区K波段光谱分析，测量了Pa$α$、Br$γ$和[Si VI]谱线中的高速外流特征，并基于近红外氢复合线计算了电离气体外流质量、质量流失率和动能功率。

Result: 在六个近邻（红移z$\sim$0.1）射电宁静型2类类星体（QSO2s）中，近红外氢复合线和[Si VI]线追踪了相同的高速度外流（速度半高全宽FWHM约1200-2500 km $s^{-1}$），但携带的质量不同，离子化气体外流质量中位数比[Si VI]线测得的高5.9倍。
...
### [18] [G183: An outer galaxy filament feeding a massive protostar](https://arxiv.org/abs/2601.22972)
*Bhaswati Mookerjea,Saurav Sen,V. S. Veena,Carsten Kramer*

**TL;DR**: 首次详细观测了外银河系5 pc长丝状结构G183及其关联的大质量年轻恒星天体(YSO) IRAS 05480+2545，发现该速度相干丝状结构通过湍流级联形成，并向恒星形成中心持续输送物质，其大质量原恒星S1(156 Msun)的吸积和喷流速率与内银河系类似，揭示了不同银河环境下大质量恒星形成物理过程的普适性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系外缘长5秒差距的丝状结构G183及其关联的大质量年轻恒星天体（YSO）IRAS 05480+2545，探讨大质量恒星形成过程中丝状结构动力学与物质吸积的普遍物理机制。

Data: 数据来源于使用IRAM 30米望远镜在λ=1.4和3毫米波段进行的角分辨率约12"-28"（对应距离d=2.1 kpc时0.1-0.3 pc）的分子气体分布观测，包括速度分辨的C18O(1-0)数据以及探测到的碳氢化合物（如CH3CN和HC3N）。

Method: 使用IRAM 30米望远镜在λ=1.4和3毫米波段进行角分辨率约12"-28"的分子气体分布观测，结合速度分辨的C18O(1-0)谱线分析揭示丝状结构运动学特征。

Result: G183长丝是一个5秒差距长的速度相干结构，具有沿其长度延伸至恒星形成中心的连续且静止的速度场，大部分区域的气体运动由热运动和由外流及大质量年轻恒星天体(YSO)吸积物质引起的大尺度速度梯度主导。
...
### [19] [Contrastive Learning of Extragalactic Stellar Streams: Sculpting a Latent Space of Representations with DES DR2 Photometry](https://arxiv.org/abs/2601.23013)
*Ernesto Benitez-Walz,Jelle Mes,Juan Miró-Carretero,Koen Kuijken,Amina Helmi*

**TL;DR**: 提出基于最近邻对比学习（NNCLR）的自监督方法，通过动态调整图像对比度的分层S型缩放函数处理DES DR2数据，有效识别低表面亮度潮汐特征但需额外监督区分恒星流，并揭示网络注意力机制抑制中央星系高亮度特征的学习模式。


<details>
  <summary>Details</summary>

Background: 研究主题为利用自监督学习方法（NNCLR）从宽视场巡天数据（DES DR2）中检测低表面亮度潮汐特征（tidal features），并开发动态图像对比度调整技术以优化微弱子结构探测。

Data: 数据来源于暗能量巡天数据发布2（DES DR2）的精选子集，构建了38,334个g、r、i波段的高分辨率星系切图，并辅以366个标记星系的补充样本用于定性评估。

Method: 采用最近邻对比学习视觉表示（NNCLR）算法结合卷积神经网络，通过动态调整图像对比度的"分层S型缩放函数"处理DES DR2数据，并利用UMAP及其变体densMAP进行降维分析。

Result: 通过最近邻对比学习视觉表示(NNCLR)算法在暗能量巡天数据(DES DR2)中成功识别出低表面亮度潮汐特征，但需额外监督才能可靠区分恒星流。
...
### [20] [Too many or too massive? Investigating the high-$z$ demography of active SMBHs from JWST](https://arxiv.org/abs/2601.23250)
...
**TL;DR**: JWST观测发现大量低光度活动星系核(AGN)在红移4-10的密度比预期高一个量级且黑洞质量远超本地质量比例关系($M_{\rm BH}-M_{*}$)，通过数据驱动模型表明中等偏离本地关系的$M_{\rm BH}-M_{*}$比例与较高活动周期能更好解释红移5.5的超大质量黑洞(SMBH)人口统计特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（$4< z<10$）低光度活动星系核（AGN）的超大质量黑洞（SMBH）与宿主星系恒星质量（$M_{\rm BH}-M_{*}$）比例关系的演化及其对早期宇宙黑洞增长机制的约束。

Data: 数据来源为JWST（James Webb Space Telescope）观测到的低光度活动星系核（AGN）样本，覆盖红移范围$4< z<10$，并结合了星系恒星质量函数、超大质量黑洞（SMBH）质量函数和AGN光度函数的观测数据。

Method: 采用数据驱动框架，通过不同黑洞质量-恒星质量关系（$M_{\rm BH}-M_{*}$ relations）和观测到的爱丁顿比分布，将星系恒星质量函数与超大质量黑洞（SMBH）质量函数及活动星系核（AGN）光度函数相关联。

Result: 在红移$z \sim 5.5$时期，超大质量黑洞（SMBH）平均质量-恒星质量关系（$M_{\rm BH}-M_{*}$）适度高于本地活动星系核（AGN）的比例关系，同时具有较高但物理上合理的占空比（duty cycle）。
...
### [21] [Evolution of Supermassive Black Hole Pairs on Inclined Orbits in Post-Merger Galaxies](https://arxiv.org/abs/2601.23260)
*Sena Ghobadi,David Ballantyne,Tamara Bogdanovic*

**TL;DR**: 构建三维动力学模型研究星系并合后超大质量黑洞(SMBH)对轨道倾角对配对时间的影响，发现初始倾角大于20°显著延长配对时间，倾角≲20°的SMBH对更可能形成双活动星系核(AGN)和引力波源。


<details>
  <summary>Details</summary>

Background: 研究超大质量黑洞（SMBH）双星在并合后残余星系中的演化理论模型，旨在为观测搜寻双活动星系核（AGN）和引力波源提供理论依据。

Data: 数据来源于使用高斯-赛德尔泊松求解器（Gauss-Seidel Poisson solver）初始化的81种不同星系配置中演化的超大质量黑洞（SMBH）对，并在每种星系中对12种初始倾角（0至75度）进行了动力学计算。

Method: 采用三维动力学模型结合高斯-赛德尔泊松求解器(Gauss-Seidel Poisson solver)模拟超大质量黑洞(SMBH)对在星系盘不同初始倾角下的轨道演化。

Result: 初始倾角大于20度的轨道通常比无倾角轨道需要更长的配对时间。
...
### [22] [MARVELously Dark: the gravothermal evolution of dwarf halos in velocity-dependent SIDM](https://arxiv.org/abs/2601.23264)
...
**TL;DR**: 研究发现自相互作用暗物质(SIDM)模型通过速度依赖的散射截面(σ/m_max=50 cm²/g)能在矮星系晕中形成与观测一致的暗物质核心结构，并识别出9个处于核心坍缩阶段的低质量晕(M_vir<2×10⁹ M⊙)，证实内区密度分布斜率比中心密度更能清晰区分核心形成与坍缩状态。


<details>
  <summary>Details</summary>

Background: 研究探讨了自相互作用暗物质（SIDM）在宇宙时间尺度上对矮星系晕中心暗物质核心（core）形成及核心坍缩（core-collapse）的影响，旨在缓解观测与理论间的矛盾。

Data: 数据来源于新的暗物质唯象（DMO）宇宙学模拟（SIDM）Ms.Marvel DMO，以及对比的CDM模拟套件Storm（包括DMO和暗物质+流体动力学运行）。

Method: 采用速度依赖的自相互作用截面（velocity-dependent self-interaction cross section）的暗物质唯宇宙学模拟（DMO cosmological SIDM simulation），对比冷暗物质（CDM）模拟组（Storm simulations）测试核心形成与坍缩过程。

Result: velocity-dependent自相互作用暗物质(SIDM)模型在Ms.Marvel DMO模拟中再现了与孤立矮星系观测一致的暗物质核心斜率。
...
### [23] [PDRs4All: XVIII. The evolution of the PAH ionisation and PAH size distribution across the Orion Bar](https://arxiv.org/abs/2601.23282)
...
**TL;DR**: 通过PAHdb和pyPAHdb建模揭示了猎户座巴（Orion Bar）不同物理区域（包括HII区、原子PDR（APDR）和HI/H2解离前沿）中多环芳烃（PAH）电荷态、尺寸分布及形成机制的演化规律，发现阳离子PAH在APDR区主导发射，中性PAH在HII区和DF2后分子云区峰值出现，阴离子PAH存在于DF2/DF3深部区域，平均尺寸约60-74个碳原子，并首次在电离前沿和分子云区分别观测到自上而下（top-down）和自下而上（bottom-up）的PAH形成机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于猎户座巴（Orion Bar）中多环芳烃（PAH）电荷态和尺寸在不同物理区域（包括HII区、原子光解离区（APDR）及三个HI/H2解离前沿（DF1、DF2和DF3））的演化规律。

Data: 数据来源于NASA Ames多环芳烃红外光谱数据库(PAHdb)和"PDRs4All"早期发布科学计划(ERS Program)中MIRI-MRS对猎户座巴的观测数据。

Method: 采用NASA Ames多环芳烃红外光谱数据库(PAHdb)和pyPAHdb光谱建模工具分析猎户座巴星云的中红外成像光谱仪(MIRI-MRS)观测数据，量化不同电荷态和尺寸多环芳烃(PAH)对总发射的分数贡献。

Result: 在猎户座棒（Orion Bar）的不同物理区域中，阳离子多环芳烃（PAH）发射在原子光解离区（APDR）达到峰值，中性PAH发射在HII区和DF2以外的分子云区达到峰值，而阴离子PAH在DF2和DF3区域深处被观测到。
...
### [24] [Isotropic Equivalence of STVG--MOG and $Λ$CDM and Its Breakdown in Large--Scale Anisotropic Cosmological Observables](https://arxiv.org/abs/2601.22207)
*John W. Moffat*

**TL;DR**: 研究表明标量-张量-矢量引力理论（STVG-MOG）在描述各向同性线性引力动力学的所有宇宙学观测（包括星系旋转曲线、星系团透镜效应、线性物质功率谱P(k)、σ₈、重子声波振荡和宇宙微波背景）中与标准ΛCDM模型观测等效，其尺度依赖的有效引力耦合常数G_eff使得两种理论在背景演化、转移函数和线性增长上完全一致，从而无需引入非重子暗物质即可解释早期宇宙及中小尺度观测现象，但该等效性会被大尺度各向异性引力响应观测（如射电星系和类星体数密度偶极子增强效应）打破，这为区分修正引力理论和粒子暗物质提供了实证途径。


<details>
  <summary>Details</summary>

Background: 该研究探讨了标量-张量-矢量引力理论（STVG-MOG）与标准宇宙学模型ΛCDM在各向同性线性引力动力学观测上的等效性及其宏观宇宙学意义。

Data: 数据来源包括星系旋转曲线、星系团透镜效应、线性物质功率谱P(k)、σ8、重子声学振荡和宇宙微波背景（CMB）的各向同性观测数据，以及吉秒差距尺度上增强的射电星系和类星体数计数偶极子的大尺度各向异性观测数据。

Method: 采用标量-张量-矢量引力理论（STVG-MOG）的尺度依赖有效引力耦合（$G_{\mathrm{eff}}$）模型，通过匹配背景演化、转移函数及线性增长实现与$Λ$CDM在各项同性动力学观测上的等效性。

Result: STVG-MOG（标量-张量-矢量引力理论）在依赖各向同性线性引力动力学的所有观测探针中与标准模型ΛCDM（宇宙学常数-冷暗物质模型）观测等效，无需引入非重子暗物质即可同样描述早期宇宙及中小尺度宇宙学观测数据。
...
### [25] [Physically-motivated priors in the local distance ladder significantly reduce the Hubble tension](https://arxiv.org/abs/2601.22215)
*Marcus Högås,Edvard Mörtsell*

**TL;DR**: 通过采用物理驱动的统一先验对距离阶梯进行贝叶斯重新校准，将哈勃常数(Hubble constant)从73.0±1.0 km/s/Mpc修正至70.6±1.0 km/s/Mpc，使哈勃张力(Hubble tension)从5σ降至2σ，揭示了先验假设在哈勃张力中的关键作用。


<details>
  <summary>Details</summary>

Background: 哈勃常数（Hubble constant）的局域距离阶梯（local distance ladder）测量与宇宙微波背景（cosmic microwave background）的早期宇宙推断之间存在显著张力（Hubble tension），该研究探讨了模型参数先验假设在此张力中的潜在影响。

Data: 数据来源包括银河系造父变星(Cepheids)的直接观测数据以及盖亚(Gaia) EDR3的视差残差偏移(parallax offset)数据。

Method: 采用全面的贝叶斯重新标定方法（Bayesian recalibration），对所有距离（包括银河系造父变星）施加物理驱动的均匀先验分布，并直接整合到联合拟合中。

Result: 采用物理驱动的先验分布对距离阶梯进行贝叶斯重新校准后，哈勃常数（Hubble constant）从73.0±1.0 km/s/Mpc降至70.6±1.0 km/s/Mpc，将哈勃张力（Hubble tension）从5σ减小到2σ。
...
### [26] [The LBT $Y_{\rm p}$ Project I: An Improved Determination of the Primordial Helium Abundance -- Project Description, Sample Selection, Observations, and Methodology](https://arxiv.org/abs/2601.22232)
...
**TL;DR**: 通过结合大双筒望远镜（LBT）新观测数据和新分析方法，将极低金属丰度HII区的氦丰度（$Y_{\rm p}$）测量精度提升至约0.5%，为标准模型外的物理（如中微子家族数）提供独立约束。


<details>
  <summary>Details</summary>

Background: 研究旨在通过观测极低金属丰度（metallicity）HII区测定原始氦丰度（$Y_{\rm p}$），以结合标准大爆炸核合成（BBN）和中子半衰期为超越标准模型的物理提供精确约束。

Data: 数据来源于新的大双筒望远镜（LBT）观测及其MODS和LUCI仪器获取的光谱数据，以及四年期间LBT/MODS标准恒星档案光谱数据。

Method: 采用大双筒望远镜（LBT）的MODS和LUCI仪器获取光谱数据，结合新开发的分析方法（包括基于四年标准星观测数据的光谱响应波长依赖性不确定度校正），实现了单个HII区氦丰度测定误差≤2%的精度。

Result: 通过结合新的大型双筒望远镜（LBT）观测数据和改进的分析方法，能够将单个HII区（HII region）的氦丰度（He abundance）测量不确定度降低至约2%或更低。
...
### [27] [The LBT Y$_\mathrm{p}$ Project IV: A New Value of the Primordial Helium Abundance](https://arxiv.org/abs/2601.22238)
...
**TL;DR**: 基于大双筒望远镜(LBT)对54个贫金属H II区的高质量观测，通过加权平均15个低金属丰度(O/H < 4×10⁻⁵)目标，首次精确测定原初氦丰度(Yₚ)为0.2458±0.0013，精度达0.5%，与基于普朗克重子密度的BBN理论预测值0.2467±0.0002高度吻合。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过观测贫金属(metal-poor)H II区精确测定原初氦丰度(primordial helium abundance, Yₚ)，以验证大爆炸核合成(BBN)理论预测。

Data: 数据来源于对54个贫金属（metal-poor）H II区的高质量大双筒望远镜（Large Binocular Telescope, LBT）观测，经筛选后得到41个可靠目标，其中包括15个高信噪比（high SNR）且低金属丰度（O/H < 4 × 10⁻⁵）的样本。

Method: 采用加权平均法（weighted average）分析15个低金属丰度（O/H < 4×10$^{-5}$）目标的氦发射线数据，结合更新后的氦发射率（helium emissivities）和95%置信水平的χ$^2$筛选标准。

Result: 通过加权平均15个低金属丰度(O/H < 4 × 10$^{-5}$)目标，测得原始氦丰度(Y$_\mathrm{p}$)为0.2458 ± 0.0013，精度达0.5%，与基于普朗克重子密度测量的BBN理论值0.2467 ± 0.0002高度吻合。
...
### [28] [The LBT $Y_{\rm p}$ Project V: Cosmological Implications of a New Determination of Primordial $^4$He](https://arxiv.org/abs/2601.22239)
*Tsung-Han Yeh,Keith A. Olive,Brian D. Fields,Erik Aver,Richard W. Pogge,Noah S. J. Rogers,Evan D. Skillman,Miqaela K. Weller*

**TL;DR**: 通过最精确测定原始氦-4质量分数（$Y_{\rm p} =0.2458 \pm 0.0013$）并结合氘丰度、大爆炸核合成（BBN）和宇宙微波背景（CMB）数据，该研究确定了宇宙重子光子比（$η= (6.120 \pm 0.038) \times 10^{-10}$）、重子密度参数（$Ω_{\rm B} h^2 = 0.02236 \pm 0.00014$）及轻中微子种类数（$N_ν= 2.925 \pm 0.082$），验证了标准粒子物理模型和连接BBN与CMB的标准宇宙学框架。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过精确测定原初氦-4（$^4$He）质量分数（$Y_{\rm p}$）并结合氘（D/H）观测数据，检验大爆炸核合成（BBN）和宇宙微波背景（CMB）在标准宇宙学框架下的自洽性。

Data: 数据来源于LBT $Y_{\rm p}$ Project对原始$^4$He质量分数($Y_{\rm p}$)的最新测量结果以及最新的原始氘(D/H)测量数据。

Method: 采用标准大爆炸核合成（BBN）模型和宇宙微波背景（CMB）数据联合分析，结合氦-4（$^4$He）原始丰度与氘（D/H）测量值，通过改变轻中微子种类数（$N_ν$）和宇宙重子-光子比（$η$）来约束宇宙学参数。

Result: 新测定的原始氦4质量分数$Y_{\rm p} =0.2458 \pm 0.0013$是目前最精确的测定结果。
...
### [29] [Mapping dark matter in the Bullet Cluster using JWST imaging and spectroscopy](https://arxiv.org/abs/2601.22245)
...
**TL;DR**: 通过结合JWST近红外相机(NIRCam)成像和近红外光谱仪(NIRSpec)观测数据，构建了子弹星系团(1E 0657-56)的改进引力透镜模型，利用27个背景星系(红移0.9-6.7)的135个多重像和199个候选像，揭示了主团呈双峰延伸分布、子团与最亮团星系(BCG)对齐(4_{-2}^{+4}千秒差距)的精细质量分布，并通过引入群尺度子结构显著提升了多重像位置拟合精度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（James Webb Space Telescope）近红外相机（NIRCam）成像和近红外光谱仪（NIRSpec）光谱数据更新子弹星系团（Bullet cluster, 1E 0657-56）的引力透镜模型，以提升对复杂质量分布和星系团动力学的理解。

Data: data部分：研究结合了JWST的NIRCam成像数据和NIRSpec光谱数据，并利用27个背景星系（红移范围0.9-6.7）的135个可靠多重像及199个候选多重像构建了透镜模型约束目录。

Method: 采用JWST（James Webb Space Telescope）近红外相机（NIRCam）成像和近红外光谱仪（NIRSpec）光谱数据，结合Lenstool软件构建引力透镜模型，整合多个大尺度晕（large-scale haloes）、星系团成员、团内气体及核心周围群尺度晕（group-scale haloes），并通过成员运动学光谱研究优化子结构分布。

Result: 通过结合JWST NIRCam成像和NIRSpec光谱数据，构建了子弹星系团(1E 0657-56)的更新引力透镜模型，显著提高了主亚团与最亮星系(BCG)对齐精度（4_{-2}^{+4} kpc），并将对齐不确定性降低了三倍。
...
### [30] [Tensions in Cosmology: Interpreting Them Through Inhomogeneous Models](https://arxiv.org/abs/2601.22278)
*Valerio Marra*

**TL;DR**: 该论文通过引入球对称非均匀ΛLTB模型（spherically symmetric inhomogeneous ΛLTB model）作为统一框架，揭示了该模型中大尺度空间梯度导致的各向异性膨胀和位置依赖性观测效应，能够在不引入动态暗能量（dynamical dark energy）的情况下解释哈勃张力（Hubble tension）、偶极信号（dipole signature）和暗能量状态方程（dark-energy equation of state）的演化现象，但尚不确定单一ΛLTB构型能否同时解决所有主要异常问题。


<details>
  <summary>Details</summary>

Background: 研究探讨了标准宇宙学模型（ΛCDM）面临的若干关键性张力（tensions），聚焦于系统性误差（systematics）和显著性膨胀（significance inflation）对其解释的影响，并探索了非均匀宇宙模型（ΛLTB）作为统一框架的潜力。

Data: 数据来源于超新星（supernova）、宇宙微波背景辐射（CMB）和大尺度结构（large-scale-structure）的完整数据集。

Method: 采用球对称非均匀ΛLTB模型（spherically symmetric inhomogeneous ΛLTB model）作为统一框架，通过引入大尺度空间梯度实现各向异性膨胀和位置依赖观测量的理论重构。

Result: 标准$Λ$CDM宇宙学模型面临的多重张力（tensions）可能源于系统性误差（systematics）和显著性膨胀（significance inflation），而球对称不均匀$Λ$LTB模型通过引入大尺度空间梯度（spatial gradients）可局部改变哈勃常数（$H_{0}$）估计值、产生偶极信号（dipolar signatures）并模拟暗能量状态方程（dark-energy equation of state）的演化现象，但现有超新星、宇宙微波背景（CMB）和大尺度结构数据尚无法确认单一$Λ$LTB构型能否统一所有异常。
...
### [31] [How isotropic is dark energy?](https://arxiv.org/abs/2601.22351)
*Richard A. Battye,Adam Moss*

**TL;DR**: 这篇论文通过引入时间依赖的各向异性应力（anisotropic stress）参数化模型，在满足宇宙微波背景（CMB）ISW四极矩约束条件下，发现五区间约束模型能显著改善Pantheon+超新星和DESI重子声波振荡（BAO）数据的拟合度（Δ(−2lnL_iso)=15.4），并部分缓解SH0ES的哈勃常数（H_0）与DESI BAO距离间的矛盾。


<details>
  <summary>Details</summary>

Background: 研究探讨了超越标准宇宙学模型（ΛCDM）的各向异性暗能量（dark energy）模型，以解决晚期宇宙膨胀数据中的张力问题。

Data: 数据来源包括Pantheon+SH0ES超新星(SNe)数据和DESI重子声波振荡(BAO)距离测量。

Method: 采用比安基（Bianchi）I模型引入时间依赖的各向异性应力参数化方法，通过强制剪切（shear）沿视线积分归零来满足宇宙微波背景（CMB）ISW四极矩约束。

Result: 五格分箱约束模型在满足宇宙微波背景(CMB)各向异性四极矩限制的同时，显著改善了标准宇宙学模型($w$CDM)的拟合优度($Δ(-2\ln L_{\rm iso})=15.4$)。
...
### [32] [Distinguishing the nature of dark matter by mapping cosmic filaments from Lyman-alpha emission](https://arxiv.org/abs/2601.22677)
*Yizhou Liu,Liang Gao,Shihong Liao,Kai Zhu,Yingjie Jing,Huijie Hu*

**TL;DR**: 莱曼α(Lyα)发射线可用于追踪红移z=2.5和z=4的宇宙纤维结构(filaments)，其平滑度和表面亮度差异(z=4时)为区分冷暗物质(ΛCDM)和温暗物质(WDM)模型提供了观测特征。


<details>
  <summary>Details</summary>

Background: 研究主题是利用莱曼阿尔法(Lyα)辐射探测宇宙纤维结构（cosmic filaments）的形态差异，以区分标准冷暗物质(ΛCDM)模型与温暗物质（warm dark matter）模型对暗物质性质的预测。

Data: 数据来源于莱曼阿尔法(Lyα)发射线对红移z=2.5和z=4处宇宙纤维结构的观测，并利用下一代30米级望远镜进行测量。

Method: 采用莱曼α(Lyα)辐射追踪宇宙纤维结构并对比冷暗物质(CDM)与温暗物质(WDM)模型预测的纤维平滑度差异。

Result: 莱曼阿尔法(Lyα)发射线在红移z=4时能有效区分暗物质模型，其丝状结构的平滑度和表面亮度差异可作为标准ΛCDM与温暗物质(WDM)模型的判别特征。
...
### [33] [The Bondi universe: How negative mass drives the cosmological expansion](https://arxiv.org/abs/2601.22910)
*Giovanni Manfredi,Jean-Louis Rouet,Bruce Miller*

**TL;DR**: 研究发现宇宙当前正从弱引力耦合（无碰撞）向强引力耦合（有碰撞）状态转变，该转变与膨胀从匀速到加速的转换同步，且由正负邦迪质量（Bondi mass）配对的不稳定性驱动，表明宇宙加速膨胀可能源于引力中性混合质量体系的非线性动力学而非暗能量或宇宙常数。


<details>
  <summary>Details</summary>

Background: 该研究探讨了宇宙从弱耦合（无碰撞）到强耦合（有碰撞）引力状态转变的动力学机制，揭示了这一耦合转变与宇宙从匀速膨胀转向加速膨胀的同步性，并提出引力中性混合质量（正负邦迪质量）宇宙的非线性动力学可能是驱动宇宙加速膨胀的内在原因。

Data: data来源包括基于弱等效原理和动量守恒的宇宙学模型（含等量正负邦迪质量）、弗拉索夫-泊松系统（Vlasov-Poisson system）的线性响应分析以及长期精确一维N体模拟（N-body simulations）。

Method: 采用线性响应分析（linear response analysis）研究Vlasov-Poisson系统的动力学不稳定性，并通过精确一维N体模拟（one-dimensional N-body simulations）验证宇宙经历的三阶段膨胀演化过程。

Result: 宇宙加速膨胀源于正负邦迪质量（Bondi mass）混合体系的非线性引力动力学机制，无需引入暗能量或宇宙常数。
...
### [34] [Linear perturbation theory and structure formation in a Brans-Dicke theory of gravity without dark matter](https://arxiv.org/abs/2601.22937)
*Lorenzo Gervani,Antonaldo Diaferio,Francesco Pace,Andrea Pierfrancesco Sanna*

**TL;DR**: 研究了不含暗物质和暗能量的布兰斯-迪克(Brans-Dicke)类标量-张量引力理论中宇宙大尺度结构形成，发现该理论在弱场极限下可简化为折射引力(Refracted Gravity)解释星系动力学，但线性扰动理论中势函数形式导致早期引力抑制、结构形成延迟至z<1与观测矛盾，且标量场对质量粒子的引力增强是光子的两倍。


<details>
  <summary>Details</summary>

Background: 研究探讨了标量-张量引力理论（scalar-tensor theory of gravity）中大规模宇宙结构（large-scale cosmic structure）的形成，属于布兰斯-迪克理论（Brans--Dicke theories）范畴，旨在替代暗物质（dark matter）和暗能量（dark energy）解释宇宙动力学和加速膨胀现象。

Data: 数据来源于标量-张量引力理论（scalar-tensor theory of gravity）中的数值模拟（simulation），其中标量场（scalar field）的动能项和自相互作用势分别设定为$W(\varphi)=-1$和$V(\varphi) = -Ξ\varphi$，且未引入暗物质（dark matter）或暗能量（dark energy）。

Method: 采用标量-张量理论（scalar-tensor theory）中的布兰斯-迪克理论（Brans--Dicke theories）框架，设定标量场（scalar field）的动能项函数W(φ)=-1和自作用势函数V(φ)=-Ξφ，在弱场极限下退化为折射引力（Refracted Gravity），通过修改泊松方程引入标量场φ替代暗物质（dark matter）解释星系动力学。

Result: 在标量-张量引力理论（Brans-Dicke theories）框架下，仅含重子物质而无暗物质或暗能量的宇宙模型中，标量场$\varphi$的自相互作用势$V(\varphi) = -Ξ\varphi$导致线性扰动增长方程中引力场源项系数与$H^{-1}(z)$成正比，造成早期引力场强烈抑制，使结构形成延迟至红移$z<1$，与高红移星系观测结果矛盾。
...
### [35] [Seeds of supermassive black holes in general relativistic and alternative cosmologies: Implications of massive seeds](https://arxiv.org/abs/2601.22991)
*Nirmali Das,Sanjeev Kalita,Ankita Kakati*

**TL;DR**: 研究发现超大质量黑洞（SMBH）在红移z=10时可通过爱丁顿极限吸积（Eddington limited accretion）或超爱丁顿吸积（super-Eddington accretion）从大质量种子（$M\geq10^{4}M_{\odot}$）或数十倍太阳质量种子增长形成，且不同宇宙学模型（$Λ$CDM、$ω$CDM、DDE和膜宇宙论）对种子质量区分度有限，同时计算得出原始黑洞（PBH）对暗物质贡献比例$f_{PBH}$在种子效应中质量$\geq 10^{7} M_{\odot}$时不超过$10^{-2}$，并分析了暗物质晕内气体质量演化及黑洞与恒星质量比随恒星形成效率的变化范围。


<details>
  <summary>Details</summary>

Background: 研究探讨了高红移(z=10)超大质量黑洞(SMBHs)的种子形成及其在不同宇宙学模型(ΛCDM、ωCDM、动态暗能量DDE和膜宇宙学)中的生长机制。

Data: 研究中使用的数据来源于理论模拟（simulation），包括在不同背景宇宙学模型（$Λ$CDM、$ω$CDM、动态暗能量(DDE)和膜宇宙学）下生成超大质量黑洞（SMBHs）种子及其生长过程的模拟数据。

Method: 采用爱丁顿极限吸积（Eddington limited accretion）和超爱丁顿吸积（super-Eddington accretion）模型，结合三种广义相对论宇宙学模型（ΛCDM、ωCDM和动态暗能量DDE）以及膜宇宙学（braneworld cosmology），研究超大质量黑洞（SMBHs）从z=30到z=10的生长过程。

Result: 在ΛCDM、ωCDM、动态暗能量(DDE)和膜宇宙学模型中，通过爱丁顿受限吸积（Eddington limited accretion）可由大质量种子（$M\geq10^{4}M_{\odot}$）在z=10时形成超大质量黑洞（SMBHs），而超爱丁顿吸积（super-Eddington accretion）可使数十倍太阳质量的旋转黑洞在相同红移下形成SMBHs。
...
### [36] [The Simons Observatory: On-sky performance of radio-transparent multi-layer insulation (RT-MLI) using Styroace-II Styrofoam](https://arxiv.org/abs/2601.23168)
...
**TL;DR**: 研究证实，采用苯乙烯泡沫（Styroace-II）的无线电透明多层隔热滤光片（RT-MLI）能有效阻隔90%以上的环境红外辐射（IR），使传输至亚100毫开尔文（mK）测辐射热计阵列的功率低于12瓦，且在探测器通带内传输功率下限达95%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发一种采用Styroace-II泡沫材料的无线电透明多层隔热滤光片（RT-MLI），用于抑制环境热辐射对亚100毫开尔文极低温测辐射热计阵列的影响。

Data: 数据来源于使用直径0.42米孔径的实测（on-sky performance）和稀释制冷器冷却的亚100毫开尔文（sub-100 mK）测辐射热计阵列（bolometric detector array）的测量结果。

Method: 采用无线电透明多层隔热材料（RT-MLI）结合Styroace-II泡沫材料，通过热辐射屏蔽设计和毫米波传输优化，实现环境热辐射的高效抑制。

Result: RT-MLI（Radio-Transparent Multi-Layer Insulation）滤光片使用Styroace-II泡沫材料成功阻隔了90%以上的预期入射红外（IR）辐射，测得透射功率低于12 W。
...
### [37] [HEP digital micromirror devices for precision solar spectroscopy](https://arxiv.org/abs/2601.23176)
*Christian Robles,Suvrath Mahadevan*

**TL;DR**: 该论文提出了一种利用新型高效像素(HEP)数字微镜器件(DMD)作为可重构空间光调制器的太阳观测仪器设计，通过仿真和实验验证了其在实时动态视场配置、高精度太阳磁活动区及系外行星凌日观测中的潜力。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发一种新型太阳观测仪器，利用高效像素（HEP）数字微镜器件（DMD）作为可重构空间光调制器，以提升对太阳磁活动区域和全日面观测的动态光谱分析能力。

Data: 数据来源包括模拟仿真（simulation）和激光测试（laser testing）结果。

Method: 采用新型高效像素（HEP）数字微镜器件（DMD）作为可重构空间光调制器，通过光学效率仿真与激光测试验证设计，并利用破坏性窗口移除进行结构分析。

Result: result：新型HEP DMD（High Efficiency Pixel Digital Micromirror Device）实现了250:1的对比度，并成功模拟了从气态巨行星到40 ppm岩石行星凌星深度的系外行星观测，验证了其在太阳和系外行星高精度科学研究中的应用潜力。
...
### [38] [Denoising the Deep Sky: Physics-Based CCD Noise Formation for Astronomical Imaging](https://arxiv.org/abs/2601.23276)
*Shuhong Liu,Xining Ge,Ziying Gu,Lin Gu,Ziteng Cui,Xuangeng Chu,Jun Liu,Dong Li,Tatsuya Harada*

**TL;DR**: 提出了一种基于物理的CCD噪声合成框架，通过模拟光子散粒噪声、光电响应非均匀性、暗电流噪声、读出效应及宇宙射线和热像素异常，结合多帧平均生成高信噪比基底，构建了用于监督学习的配对数据集，并提供了包含原始帧、校准帧及高信噪比基底的真实多波段望远镜观测数据集。


<details>
  <summary>Details</summary>

Background: 天文成像在观测约束下仍受限于噪声，而基于学习的去噪方法因缺乏配对训练数据及科学工作流对物理可解释模型的需求面临挑战。

Data: 数据来源包括通过平均多幅未配准曝光生成的高信噪比基底（high-SNR bases）、基于噪声模型合成的真实噪声对应数据、以及由两台地面望远镜获取的多波段真实数据集（包含原始帧、仪器校准帧、校准数据和高信噪比基底）。

Method: 基于电荷耦合器件(CCD)噪声形成机制，开发了物理驱动的噪声合成框架，建模了光子散粒噪声、光响应非均匀性、暗电流噪声、读出效应及宇宙射线和热像素导致的局部异常。

Result: 基于物理的CCD噪声合成框架通过建模光子散粒噪声、光响应非均匀性、暗电流噪声、读出效应及宇宙射线撞击和热像素产生的局部异常值，为监督学习构建了丰富的配对数据集。
...
### [39] [Axions on a Hyperbolic Ride: Geometric Suppression of CMB Isocurvature and a Blue-Tilted Spectrum](https://arxiv.org/abs/2601.22221)
*Sai Chaitanya Tadepalli*

**TL;DR**: 研究发现通过利用场空间几何（field-space geometry）赋予轴子涨落随时间变化的几何质量，可在不破坏佩塞-奎恩（PQ）对称性和无需极端径向位移的情况下抑制等曲率扰动，从而允许高能标暴胀和大QCD轴子衰变常数同时实现，重新开放了通常被认为被排除的参数空间。


<details>
  <summary>Details</summary>

Background: 研究探讨了在预暴胀Peccei-Quinn（PQ）场景中，通过利用场空间几何（field-space geometry）来规避冷暗物质（cold-dark-matter）等曲率（isocurvature）限制，从而重新开放高尺度暴胀与大QCD轴子衰变常数（QCD axion decay constants）同时实现的参数空间。

Data: 研究中使用的数据来源于理论模拟（simulation），基于具有双曲度规（hyperbolic metric）的最小复Peccei-Quinn（PQ）标量场的非线性σ模型动力学项和U(1)对称势能。

Method: 采用非线性σ模型（nonlinear sigma-model）动力学项和双曲度量（hyperbolic metric）的场空间几何（field-space geometry）方法，通过赋予轴子涨落随时间变化的几何质量来抑制等曲率扰动。

Result: 通过利用场空间几何（field-space geometry）赋予轴子涨落随时间变化的几何质量，可以在不破坏佩塞-奎恩（PQ）对称性和不极端径向位移的情况下抑制等曲率扰动，从而使得高尺度暴胀和大QCD轴子衰变常数在预暴胀PQ情景中同时实现成为可能。
...
### [40] [When inflationary perturbations refuse to classicalise: the role of non-Gaussianity in Wigner negativity](https://arxiv.org/abs/2601.22219)
*Aurora Ireland,Vincent Vennin*

**TL;DR**: 该论文通过计算暴胀理论(EFT)中考虑原初非高斯性(primordial non-Gaussianities)的曲率扰动(curvature perturbations)维格纳函数(Wigner function)，发现其在超哈勃尺度(super-Hubble scales)上产生显著干涉条纹且负性随标度因子平方($a^2$)增长，证明量子效应在晚期仍显著存在且挤压(squeezing)不足以保证经典性，表明宇宙起源的量子特征在宇宙学观测中的探测前景可能优于预期。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙暴胀（inflation）过程中量子扰动的本质特征及其在后期宇宙学观测中的潜在量子印记（quantum birthmarks），挑战了传统认为量子涨落会经典化的观点。

Data: 使用有效场论（EFT of inflation）计算了含原初非高斯性的曲率扰动的维格纳函数（Wigner function）。

Method: 采用有效场论（EFT）方法计算原初非高斯性影响下的曲率扰动维格纳（Wigner）函数。

Result: 曲率扰动的维格纳函数（Wigner function）在超哈勃尺度上产生显著干涉条纹，且在超慢滚背景下其负性以a²增长，表明量子效应在晚期仍显著存在。
...
### [41] [Nonlocal Corrections to Scalar Field Effective Action in de Sitter spacetime](https://arxiv.org/abs/2601.22644)
*Will Cerne,Teruaki Suyama*

**TL;DR**: 该研究通过施温格-克登形式（Schwinger-Keldysh formalism）推导出弗里德曼-勒梅特-罗伯逊-沃克（FLRW）背景中测试标量场的单圈有效作用量，发现二阶相互作用强度下的量子修正不仅包含标准局域辐射修正，还产生了具有记忆项和随机噪声项的非局域特征，并证明这些修正会导致红外区场方差相比树级结果受到抑制。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于弗里德曼-勒梅特-罗伯逊-沃克(FLRW)宇宙背景下测试标量场的单圈有效作用量及其量子修正效应。

Data: 数据来源于对弗里德曼-勒梅特-罗伯逊-沃尔克（FLRW）背景中测试标量场（test scalar field）的单圈有效作用量（one-loop effective action）的理论模拟（simulation），重点关注相互作用强度二阶的量子修正。

Method: 采用施温格-凯尔迪什形式（Schwinger-Keldysh formalism）推导场期望值的运动方程，包含局域辐射修正和非局域特征（记忆项与随机噪声项），并通过局部近似分析缓慢变化场条件下记忆项的物理效应。

Result: 在弗里德曼-勒梅特-罗伯逊-沃克(FLRW)背景下，通过施温格-凯尔迪什形式体系推导出包含记忆项和随机噪声项的非局域量子修正场期望值运动方程。
...
### [42] [Comparison of MOND and Verlinde's emergent gravity in dwarf spheroidals](https://arxiv.org/abs/2601.01715)
*Youngsub Yoon,Sanghyeon Han,Ho Seong Hwang*

**TL;DR**: 通过比较23个矮球状星系(dwarf spheroidals)的径向加速度，发现维林德涌现引力(Verlinde's emergent gravity)比修正牛顿动力学(MOND)更符合观测值，统计显著性达5.2σ。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过修正牛顿动力学（MOND）和Verlinde涌现引力理论分别计算23个矮球状星系（dwarf spheroidals）的径向加速度，并与观测值对比，以检验两种理论在解释星系动力学上的适用性。

Data: 研究中使用的观测数据来源为23个矮椭球星系(dwarf spheroidals)的径向加速度观测值。

Method: 分别应用修正牛顿动力学（MOND）和Verlinde涌现引力理论计算23个矮球状星系的径向加速度，并与观测值进行对比。

Result: 在23个矮球状星系(dwarf spheroidals)样本中，Verlinde涌现引力(emergent gravity)比修正牛顿动力学(MOND)更符合观测径向加速度趋势，统计显著性达5.2σ。
...
### [43] [The SPIRou Legacy Survey: Detection of a nearby world orbiting in the habitable zone of Gl725B achieved by correcting strong telluric contamination in near-infrared radial velocities with WAPITI](https://arxiv.org/abs/2601.22815)
...
**TL;DR**: 通过SPIRou近红外径向速度(radial velocity)观测和加权主成分分析重建方法(WAPITI)校正地球大气吸收线干扰，在距离3.5秒差距的晚型M矮星Gl 725 B周围发现由周期4.765天的候选行星(Gl 725 Bb)和周期37.90天的确认行星(Gl 725 Bc)组成的双行星系统，后者位于宜居带且最小质量为3.5倍地球质量，同时测得恒星自转周期为105.1天。


<details>
  <summary>Details</summary>

Background: 研究主题为利用近红外（NIR）视向速度（RV）方法探测M型矮星（M dwarfs）周围的行星，并解决地球大气吸收线（telluric absorption lines）对测量精度的影响。

Data: 使用高精度SPIRou径向速度（RV）观测数据，包括合成数据和实际观测数据，评估了地球大气层吸收线（telluric lines）对Gl 725 B行星探测的影响。

Method: 采用加权主成分分析重建方法（WAPITI）校正逐行视向速度（RV）数据，并通过合成和观测SPIRou数据模拟不同地球视向速度（BERV）条件下的地球大气吸收线影响。

Result: 在Gl 725 B周围发现了一个由内行星候选体（周期4.765±0.004天，半振幅1.4±0.3 m/s，最小质量1.5±0.4 M⊕）和已确认外行星（周期37.90±0.17天，半振幅1.7±0.3 m/s，最小质量3.5±0.7 M⊕）组成的双行星系统，其中外行星位于宜居带内。
...
### [44] [Quantifying the C/O ratio in the planet-forming environments around very-low-mass stars](https://arxiv.org/abs/2601.23069)
*Javiera K. Díaz-Berríos,Catherine Walsh,Ewine F. van Dishoeck*

**TL;DR**: 通过化学动力学模型揭示了极低质量恒星(VLMS)周围行星形成盘内烃类物质丰度显著升高的原因是碳氧比(C/O)增强，且该现象对X射线光度(LX)存在依赖性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于行星形成盘中关键挥发物的物理化学过程及其丰度分布对行星组成的影响，特别关注极低质量恒星(VLMS)周围盘内碳氢化合物富集现象与碳氧比(C/O)的关联机制。

Data: 数据来源于詹姆斯·韦伯太空望远镜（James Webb Space Telescope）对极低质量恒星（VLMS）周围行星形成盘的观测，并结合化学动力学模型模拟了M型矮星周围内盘物理结构（X射线光度为$L_X\sim10^{29}~\mathrm{erg~s^{-1}}$）下的碳氧比（C/O）变化效应。

Method: 采用化学动力学模型（chemical kinetics models）结合M型矮星（M Dwarf）内盘物理结构，分析碳氧比（C/O ratio）变化对关键挥发物丰度及分布的影响。

Result: 碳氧比(C/O)显著影响行星形成盘中碳氢化合物和含氧物质的柱密度和分子数量，碳增加2倍或氧减少10倍时碳氢化合物增幅最大。
...
### [45] [High-bandwidth frequency domain multiplexed readout of transition-edge sensors for neutrinoless double beta decay searches](https://arxiv.org/abs/2601.23106)
...
**TL;DR**: 开发了基于数字频域复用（fMux）的超导转变边缘传感器（TES）高速读出系统，将采样率提升至156 kHz以满足下一代无中微子双β衰变（neutrinoless double-beta decay）实验对背景甄别的高带宽需求。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升下一代无中微子双贝塔衰变(cryogenic neutrinoless double-beta decay)实验中超导探测器(transition-edge sensors, TESs)的快速读出技术。

Data: 研究中使用的数据来源于基于McGill/Berkeley数字fMux读出系统的新读出系统，该系统以156 kHz的采样率对探测器进行采样，并通过包含十个MHz范围超导谐振器和一个超导量子干涉器件（SQUID）的复用读出模块获取数据。

Method: 采用基于麦吉尔/伯克利数字频分复用读出（fMux）的新系统，通过包含十个兆赫兹范围超导谐振器和一个超导量子干涉器件（SQUID）的模块，结合基于现场可编程门阵列（FPGA）的高带宽电子设备进行数字信号处理和低延迟反馈。

Result: 开发了一种基于数字频域复用（fMux）的新型读出系统，采样频率达156 kHz，比前代系统快三个数量级，满足下一代TES（transition-edge sensors）低温量热计对高带宽和低噪声的需求。
...
### [46] [Drifters on the edge of town: $λ$ Boötis stars in clusters](https://arxiv.org/abs/2601.22216)
*Richard J. Parker,Megan Allen*

**TL;DR**: 通过N体模拟发现，部分A型恒星（A-stars）在星团演化过程中会短暂脱离潮汐半径（tidal radius）并吸积贫金属星际介质（pristine ISM gas），随后返回星团内部时仍能保持λ Boötis恒星（λ Boötis stars）的化学丰度特征，且可能形成宽双星系统（>100 au），支持星际介质吸积假说（ISM accretion scenario）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解释星团环境中λ Boötis型恒星（λ Boötis stars）特殊化学丰度（铁峰元素缺乏而轻元素丰度接近太阳）的成因机制，挑战了传统星际介质（ISM）原始气体吸积理论在辐射场强烈环境下的适用性。

Data: 使用N体模拟(N-body simulation)追踪星团演化过程中A型恒星的动力学历史。

Method: 采用N体模拟（N-body simulations）追踪星团演化过程中A型恒星的动力学历史。

Result: 部分A型恒星在离开星团潮汐半径范围后可吸积原始星际介质气体，随后返回星团内部时接收的光致电离辐射通量不足以阻止其形成λ Boötis（λ Boo）型丰度模式。
...
### [47] [SED and Galactic kinematic diagnostics for dormant BH/NS binary candidates](https://arxiv.org/abs/2601.22490)
*Qian-Yu An,Wei-Min Gu*

**TL;DR**: 通过结合宽波段光谱能量分布(SED)拟合和银河系运动学分析，从1328个候选体中筛选出176个休眠黑洞(black hole)和中子星(neutron star)双星系统的高优先级目标，其中19个黑洞候选体质量≥3倍太阳质量。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用盖亚任务第三次数据释放(Gaia DR3)开展大尺度搜寻具有宽轨道分离恒星伴星的休眠黑洞和中子星双星系统。

Data: 数据来源包括盖亚任务第三次数据发布（Gaia DR3）的汇总统计量、GALEX紫外测光数据以及从光学到红外的宽带光谱能量分布（SED）拟合数据。

Method: 采用宽带光谱能量分布（SED）拟合结合GALEX紫外测光评估隐藏热伴星的存在，并通过比较观测与单星SED预测的近紫外通量量化紫外超量，同时利用银河系运动学分析识别受致密天体形成时natal kicks影响的系统。

Result: 通过紫外和运动学诊断结合，筛选出176个最高优先级候选体（含19个黑洞候选体，质量≥3倍太阳质量）。
...
### [48] [Constructing a gravitational wave analysis pipeline for extremely large mass ratio inspirals](https://arxiv.org/abs/2601.22464)
*Tian-Xiao Wang,Yan Wang,Alejandro Torres-Orjuela,Yi-Ren Lin,Hui-Min Fan,Verónica Vázquez-Aceves,Yi-Ming Hu*

**TL;DR**: 首次开发了针对天琴任务的空间引力波探测器专用分层半相干搜索流程，实现了对超大质量黑洞（supermassive black hole）与褐矮星（brown dwarf）构成的极大规模比旋进系统（XMRIs）的高精度参数估计（轨道频率误差<10⁻⁶、黑洞质量误差≲2×10⁻³、自旋误差≲10⁻³）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超大质量黑洞（supermassive black hole）与褐矮星（brown dwarf）构成的极高质量比旋近系统（XMRIs）作为毫赫兹波段引力波源，为强场引力与黑洞特性研究提供新途径。

Data: 使用模拟（simulation）的TianQin数据验证了针对银河系中心超大质量黑洞（Sgr A*）和褐矮星组成的极大型质量比旋近系统（XMRI）的搜索流程性能。

Method: 采用分层半相干搜索流程，结合半相干多谐波$\mathcal{F}$-统计量与粒子群优化算法，并引入基于谐波间相对功率分布的新型偏心率估计方法。

Result: 首次开发了针对天琴任务的空间引力波探测器专用分层半相干搜索流程，结合半相干多谐波F统计量与粒子群优化算法，并引入基于谐波相对功率分布的新颖偏心率估计方法。
...
### [49] [Approximation of the reception coefficients of cosmic rays neutron component for latitude measurement](https://arxiv.org/abs/2601.22810)
*Kobelev P. G.,Yanke V. G*

**TL;DR**: 开发了基于地面宇宙射线探测器网络（ground-based cosmic ray detectors network）的纬度校正方法，通过近似零次和一次谐波各向异性系数的刚度依赖性（rigidity dependence），实现了高精度海洋科考纬度测量。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过修正监测数据中的初级宇宙射线（primary cosmic ray）变化以提升纬度监测精度。

Data: 数据来源于地面宇宙射线探测器网络（ground-based cosmic ray detectors network）的观测数据。

Method: 采用基于地面宇宙射线探测器网络（ground-based cosmic ray detectors network）的零次和一次谐波各向异性系数（zeroth and first harmonic anisotropy coefficients）的刚度依赖性（rigidity dependence）近似方法。

Result: 开发的方法通过获取接收系数随纬度变化函数，实现了对地面宇宙射线探测器网络零次和一次谐波各向异性系数刚度依赖性的高精度近似。
...
### [50] [Observational Implications of Cosmic Ray-Inverse Compton 'Boosted' Cool Cores in Clusters](https://arxiv.org/abs/2601.22229)
*Philip F. Hopkins,Emily Silich,Jack Sayers,Sam B. Ponnada,Isabel Sands*

**TL;DR**: 研究发现X射线明亮的冷核（CC）星系团中存在宇宙射线（CR）驱动的逆康普顿（IC）辐射，其产生的热谱特征可解释冷却流问题中的表观冷却现象，并自然关联射电迷你晕等观测特征，同时预言了CR-IC会导致CC中心区域X射线压力高估和金属丰度低估。


<details>
  <summary>Details</summary>

Background: 研究探讨了X射线明亮冷核（CC）星系团中宇宙线（CR）通过逆康普顿（IC）散射CMB光子产生的热谱X射线辐射及其对冷却流问题的替代解释。

Data: 数据来源于对X射线明亮冷核（CC）星系团中宇宙线（CR）的逆康普顿（IC）散射辐射的观测和模拟分析。

Method: 采用逆康普顿散射(IC)模型分析宇宙线(CR)与宇宙微波背景(CMB)光子相互作用产生的X射线辐射。

Result: 高能宇宙线（CR）通过逆康普顿（IC）散射产生与热辐射相似的X射线光谱，可能显著贡献于冷核（CC）中心的辐射，从而为冷却流问题提供替代解释。
...
### [51] [X-Ray Analysis of an Off-Axis Merger Stage Binary Galaxy Cluster: PSZ2 G279.79+39.09](https://arxiv.org/abs/2601.22251)
*Sibel Döner,Turgay Caglar,Krista L. Smith,Serap Ak,Andrea Botteon,M. Kiyami Erdim,John A. ZuHone*

**TL;DR**: 通过XMM-Newton和钱德拉(Chandra)观测分析合并星系团PSZ2 G279.79+39.09（红移z=0.29），发现其具有东西双峰结构（核心间距1.35 Mpc）、存在热（7.3 keV）高压高熵桥，并通过激波马赫数(M=1.41)和气体动力学特征推断为偏心合并（可能处于核心通道前后≲5亿年），且更倾向于 pericenter（近心点）前阶段。


<details>
  <summary>Details</summary>

Background: 研究聚焦于合并星系团（merging galaxy cluster）系统的X射线分析，探讨其动力学和热力学演化过程。

Data: 数据来源于XMM-Newton和钱德拉(Chandra)的存档观测数据。

Method: 采用X射线表面亮度成像、等温（isothermal）状态方程假设下的流体静力学质量估算、浓度指数（concentration index）与质心偏移（centroid shift）形态分析、兰金-雨贡纽激波（Rankine-Hugoniot shock）温度跃变推导马赫数（Mach number）等多元方法。

Result: 合并星系团系统PSZ2 G279.79+39.09呈现双峰X射线表面亮度分布，东西延伸且投影核心间距约1.35 Mpc，子团PSZ-E和PSZ-W的等温气体温度分别为5.36 keV和5.44 keV，静水力学质量比约1:1.7，且两者均存在由斜向合并导致的ICM不对称性和激波特征（Mach数1.41），支持一个非零碰撞参数并在核心通道附近（≲0.5 Gyr）被观测到的合并场景。
...
### [52] [Unveiling BLR Structure in AGN with High Resolution X-Ray Spectra: An Analytic Approach to Wind Emission Line Profiles](https://arxiv.org/abs/2601.22392)
*Scott Hagen,Chris Done,Gabriele A. Matzeu,Hirofumi Noda*

**TL;DR**: 论文构建了针对宽线区（BLR）尺度物质的风模型xwind，为XRISM观测到的Fe-Kα发射线中间成分提供了物理合理且计算快速的解析解，成功解释了NGC 4151数据中平滑的谱线轮廓。


<details>
  <summary>Details</summary>

Background: 研究聚焦于X射线波段发射与吸收线谱的复杂结构建模，特别是针对活动星系核(AGN)中铁Kα线(Fe-Kα line)轮廓的多组分特征（包括内盘、中间宽线区(BLR)尺度物质和外环结构）带来的理论挑战。

Data: 数据来源于XRISM（X射线成像和光谱任务）对NGC 4151在性能验证阶段的观测数据。

Method: 构建了针对宽线区（BLR）尺度物质的风模型（wind model）xwind，采用全解析方法模拟发射线轮廓，基于先前回响研究提出的BLR内缘风假设。

Result: XRISM观测揭示了活动星系核(AGN)中铁Kα线轮廓的显著复杂性，至少存在三个发射结构：内盘、中间尺度宽线区(BLR)物质和外环面。
...
**TL;DR**: 数百万年前银河系中心的类爱丁顿吸积事件（Eddington-like accretion）及其伴随的激波和喷流中的粒子加速可解释观测到的1 PeV以上宇宙线能谱，宇宙线粒子被限制在数百千秒差距（kiloparsec）大小的银河系晕（Galactic halo）中，且晕磁场可低至2×10^-7 G以实现有效约束。


<details>
  <summary>Details</summary>

Background: 研究探讨了数百万年前银河系中心发生的类爱丁顿(Eddington-like)吸积事件及其伴随的激波和喷流中的粒子加速机制如何解释观测到的1 PeV以上宇宙射线能谱。

Data: 数据来源于银河系中心数百万年前发生的类爱丁顿吸积(Eddington-like accretion)事件及其伴随的激波(shocks)和喷流(jets)中的粒子加速过程，以及被限制在延伸(extended)至数百千秒差距(kiloparsec)尺度的银河晕(Galactic halo)中的宇宙线粒子。

Method: 采用粒子加速（particle acceleration）模型结合扩展银晕（Galactic halo）磁约束机制，通过数值模拟验证了PeV能区宇宙线谱的成因。

Result: 数百万年前银河系中心发生的类爱丁顿吸积事件及其伴随的激波和喷流中的粒子加速过程，可解释观测到的1 PeV以上宇宙线能谱。
...
### [54] [Physical origin of very-high-energy gamma rays from the low-luminosity active galactic nucleus NGC 4278 and implications for neutrino observations](https://arxiv.org/abs/2601.23242)
*Shilong Chen,Abhishek Das,B. Theodore Zhang,Shigeo S. Kimura,Kohta Murase,Yunfeng Liang*

**TL;DR**: 通过多波段和多信使分析发现低光度活动星系核（LLAGN）NGC 4278的甚高能（VHE）辐射物理起源更符合外部逆康普顿（EIC）模型而非标准单区同步自康普顿（SSC）模型，并预测了中微子产率。


<details>
  <summary>Details</summary>

Background: 低光度活动星系核(LL AGNs)中极高能(VHE)辐射的物理起源是当前高能天体物理领域的重要未解难题。

Data: 数据来源于Swift-XRT的长期监测数据、LHAASO对甚高能(VHE)辐射的探测结果以及基于AMES代码的宽带谱能分布(SED)建模。

Method: 采用多波段与多信使分析，结合轻子强子混合模型AMES代码，对比标准单区同步自康普顿(SSC)模型和外逆康普顿(EIC)情景（散射来自辐射无效吸积流RIAF的种子光子）的拟合效果。

Result: 标准单区同步自康普顿（SSC）模型因弱X射线辐射提供的靶光子密度不足而低估甚高能（VHE）通量约70%，除非采用高多普勒因子（δ≳5）。
