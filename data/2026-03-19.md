...
**TL;DR**: 通过高分辨率宇宙学模拟揭示了星系团前身从$z\sim4.4$到$2.7$的演化过程，发现大质量并合和活动星系核(AGN)反馈驱动了由内向外的气体重分布，导致低电离镁吸收体(MgII)快速消失而高电离氧吸收体(OVII)主导，同时莱曼阿尔法(Ly$α$)晕从纤维状转变为球状。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系晕（galactic haloes）和成熟星系团（galaxy clusters）之间从冷气体主导的环星系介质（CGM）向热X射线辐射主导的星系团内介质（ICM）转变的演化过程。

Data: 研究中使用的数据来源于一个高分辨率的宇宙学缩进模拟（cosmological zoom-in simulation），该模拟追踪了一个大质量星系团前身从$z\sim4.4$到$z=2.7$的演化过程，重点关注了星系晕（CGM）中的气体吸积、并合和反馈过程，并生成了模拟的MgII、OVII吸收线以及Ly$α$和H$α$发射线数据。

Method: 采用宇宙学放大模拟（cosmological zoom-in simulation）结合辐射传输后处理技术，重点追踪星系团前身从z~4.4到2.7演化过程中的气体吸积、并合及反馈事件，并通过模拟MgII、OVII吸收线及Lyα、Hα发射线多相气体示踪。

Result: 在$z\sim4.4$至$2.7$期间，主要合并事件和活动星系核(AGN)反馈驱动了由内向外的转变，将气体重新分布到更大半径并压平了密度、温度和金属丰度分布。
...
**TL;DR**: 基于MIGHTEE巡天20个星系的氢原子（HI）旋转曲线和质量模型，研究发现采用空间分辨的质量光度比（M/L）能更准确表征星系光度与暗物质晕（dark-matter haloes）的关系，而固定M/L假设会系统性偏离ΛCDM理论预测的恒星-晕质量关系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过星系旋转曲线测量推断暗物质晕（dark-matter haloes）性质，以验证Lambda冷暗物质（ΛCDM）理论框架。

Data: 数据来源于MIGHTEE巡天（MIGHTEE survey）的20个星系的HI旋转曲线（HI rotation curves）和3.6微米成像（3.6 μm imaging），并结合多波段光谱能量分布拟合（multi-wavelength SED fitting）获取空间分辨的质量-光比（spatially resolved M/L）。

Method: 采用扩展中性氢（HI）运动学数据构建包含恒星、气体和暗物质成分的分辨质量模型，并结合多波段光谱能量分布（SED）拟合获得空间分辨的质量-光度比（M/L）。

Result: 采用空间分辨的质量-光度比(M/L)分析能更准确地反映星系光度与暗物质晕性质的关系，并保持与ΛCDM理论基准的最佳一致性。
...
**TL;DR**: 研究发现后星暴星系（PSGs）在宿主星系团中倾向于高轨道角动量（angular momentum）的切向轨道，表明低速剥离的压致星暴（ram-pressure-induced starburst）在低质量星系中更高效。


<details>
  <summary>Details</summary>

Background: 研究聚焦于揭示低质量星系在星系团环境中因冲压剥离（ram-pressure stripping）引发的恒星形成爆发（starburst）与轨道角动量（angular momentum）的关联机制。

Data: 数据来源于DESI早期数据发布（EDR）的光谱数据，选取了后发座星系团（Coma cluster）中41个后星暴星系（PSGs）样本，光度范围延伸至$M_g < -14$，且在$M_g \approx -14.8$处样本完整性至少达到90%。

Method: 采用归一化轨道角动量（normalized orbital angular momentum）分析结合三维径向坐标（3D radial coordinate）约束，假设轨道能量为零，通过概率分布统计评估星系轨道特性。

Result: 低质量星系中冲压剥离(ram-pressure)诱发星暴的关键因素是其宿主星系团内的角动量(angular momentum)。
...
**TL;DR**: 通过SPHINX20模拟和观测验证，发现SiII 1526低电离态吸收线的尘埃校正剩余通量（$\tilde{R}$）与电离光子逃逸率（$f_{\rm{esc}}$）存在强相关性（$f_{\rm{esc}} \approx 1.041\tilde{R}^{1.887} - 0.002$），可作为其有效诊断指标。


<details>
  <summary>Details</summary>

Background: 低电离态（LIS）吸收线（如SiII 1526）被广泛用于追踪星系中星际介质（ISM）的特性，为研究星系演化（包括反馈机制、金属增丰和电离光子逃逸率）提供关键信息。

Data: 使用SPHINX20宇宙学辐射流体动力学模拟（SPHINX20 cosmological radiation-hydrodynamics simulation）生成了全面的低电离态吸收线（LIS absorption lines）合成数据集，并通过辐射传输代码RASCAS计算了合成SiII 1260和SiII 1526谱线，同时考虑了光子共振散射、荧光发射和尘埃颗粒相互作用。

Method: 采用SPHINX20宇宙学辐射流体动力学模拟结合辐射传输代码RASCAS（包含共振散射、荧光发射和尘埃相互作用）生成合成低电离态（LIS）吸收线数据集。

Result: SiII 1526的尘埃校正残余通量（$\tilde{R} \equiv \rm{R_{flux}^{1526}} \cdot 10^{-0.4A_{1500}}$）与电离光子逃逸分数（$f_{\rm{esc}}$）存在强相关性，关系式为$f_{\rm{esc}} \approx 1.041\tilde{R}^{1.887} - 0.002$，误差较小。
...
**TL;DR**: 这篇论文通过分析24个射电噪活动星系核(AGN)在远紫外(FUV)和近紫外(NUV)波段的观测数据，首次系统研究了其紫外光变特性，发现18个源存在小时量级显著光变，并在CTA 102和PKS 0447-439中观测到"越亮越蓝"(BWB)趋势，表明这些紫外光变可能由相对论性喷流内部的本征过程驱动。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于射电噪活动星系核（AGN）在紫外（UV）波段的流量和光谱变异性特征，填补了该波段相较于其他波段研究相对不足的空白。

Data: 数据来源于搭载在AstroSat卫星上的紫外成像望远镜（Ultra-Violet Imaging Telescope, UVIT）在其运行前十年获取的观测数据，覆盖远紫外（FUV; 1300–1800 Å）和近紫外（NUV; 2000–3000 Å）波段。

Method: 采用紫外成像望远镜(UVIT)观测数据，在远紫外(FUV)和近紫外(NUV)波段分析24个射电噪活动星系核(AGN)的流量和光谱变异性特征。

Result: 在射电噪活动星系核(AGN)样本中观测到18个源存在显著的小时量级紫外(UV)波段光变，并在CTA 102和PKS 0447-439两个源中检测到"越亮越蓝"(BWB)光谱趋势。
...
**TL;DR**: 通过UXClumpy和RXTorusD模型分析11个候选强遮蔽活动星系核(AGN)的X射线光谱，发现样本中遮蔽柱密度(NH,los)分布在22.7-23.96之间，并比较了两模型在光子指数(Gamma)和NH,los测定上的一致性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用机器学习算法筛选出的11个候选强遮蔽活动星系核（AGN），旨在通过宽频X射线光谱分析表征本地宇宙（z < 0.1）中具有最高视线方向（los）柱密度（NH）的康普顿厚（CT-）AGN。

Data: 数据来源于11个候选强遮蔽活动星系核(AGN)的红外和X射线特性，这些目标由近期发表的机器学习算法筛选。

Method: 采用两种物理驱动模型UXClumpy和RXTorusD对X射线光谱进行建模分析。

Result: 通过UXClumpy和RXTorusD模型分析，发现11个候选AGN中分别有3个、5个和3个的视线柱密度（NH,los）落在22.7 < LogNH,los <= 23.0、23.0 < LogNH,los <= 23.25和23.4 < LogNH,los <= 23.9范围内，而RXTorusD模型则给出3个、4个和4个AGN的NH,los分别位于22.7 < LogNH,los <= 23.0、23.0 < LogNH,los <= 23.4和23.85 < LogNH,los <= 23.96区间。
...
**TL;DR**: 基于对30个大质量恒星形成区的高分辨率观测和模拟研究，首次发现凝聚体（condensations）的伸长方向优先与局部磁场（B fields）平行，表明湍流（turbulence）在凝聚体形成过程中比磁场起更重要作用，并可能通过降低磁制动（magnetic braking）效率促进原恒星盘（protostellar disks）的形成。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量恒星形成过程中引力-磁力（gravo-magnetic）过程与湍流对原星团（protocluster）中小尺度凝聚体（condensation）形态及磁场（B-field）排列的影响，挑战了经典磁控模型的预测。

Data: 数据来源为阿塔卡马大型毫米波/亚毫米波阵列（ALMA）对30个大质量恒星形成区进行的最高分辨率尘埃偏振巡测数据。

Method: 基于高分辨率ALMA尘埃偏振观测数据，采用湍流主导初始条件的数值模拟方法分析了原恒星团中凝聚体（condensations）形态与局部磁场（B fields）的统计关联性。

Result: 基于ALMA高分辨率尘埃偏振观测数据，首次统计发现大质量恒星形成原星团中凝聚体（condensations）的伸长方向优先与局部磁场（B fields）平行，而非经典磁调控模型预测的垂直排列。
...
**TL;DR**: 基于165个变脸活动星系核（CL AGNs）的光学和中红外光变分析，发现其在1型活动星系核（Type-1 AGN）阶段的光变特性（短时标振幅与黑洞质量、光度无关，长时标振幅随二者缓慢增加且普遍呈现蓝随亮变化趋势）与标准吸积盘-宽线区-尘埃环模型一致，但更易在年量级时标上发生连续谱重组。


<details>
  <summary>Details</summary>

Background: 研究聚焦于变脸活动星系核（CL AGNs）在盘-宽线区（BLR）-尘埃环（torus）系统中随时间变化的吸积过程，旨在揭示其光学光度与光谱状态在数年时间尺度上的显著变化机制。

Data: 数据来源为兹威基瞬变设施（Zwicky Transient Facility）的g波段和r波段光变曲线（165个变脸活动星系核），以及NEOWISE卫星W1和W2波段光变曲线（34个对象的子样本）。

Method: 采用结构函数（structure function）和阻尼随机游走模型（damped random-walk model）分析光变幅度和时间尺度，并研究其与黑洞质量、光度及爱丁顿比（Eddington ratio）的关系。

Result: 在1型活动星系核(AGN)阶段，变脸活动星系核(CL AGN)的短时标（约30天）光学变幅与黑洞质量、光度或爱丁顿比无关，而长时标（150-300天）变幅及阻尼随机游走时标随黑洞质量和光度缓慢增加但仍与爱丁顿比无关。
...
**TL;DR**: 提出了一种基于棒旋结构运动学距离（BIKD）的方法，通过替换轴对称圆周运动假设为流体动力学模拟提取的非轴对称流场，在观测约束的银河系棒旋势场中重建了银河系内部分子气体面密度（Σ）分布图，显著抑制了视线方向伪影并恢复了棒旋对齐的象限不对称结构。


<details>
  <summary>Details</summary>

Background: 研究聚焦于改进银河系内部分子气体面分布（face-on molecular-gas maps）的动力学距离（kinematic-distance）重建方法，以解决传统轴对称圆周运动假设在棒状结构（barred regions）区域引入严重伪影的问题。

Data: 数据来源于银河系CO巡天观测数据（Galactic CO survey）和流体动力学模拟（hydrodynamical simulations）中提取的非轴对称流场（non-axisymmetric streaming field）。

Method: 采用基于棒旋结构运动学距离（BIKD）的方法，通过从流体动力学模拟中提取非轴对称流场替代传统轴对称旋转曲线，并结合观测约束的棒旋星系势场，利用视线速度高斯似然推断离散距离后验分布。

Result: 提出了一种基于棒结构运动学距离（BIKD）的方法，通过替换传统轴对称圆周运动假设为从流体动力学模拟中提取的非轴对称流场，有效抑制了棒区域中常见的视线方向拉长伪影（finger-of-God features），并重建出与棒结构对齐且象限不对称的银河系内部分子气体面密度（Σ）分布。
...
**TL;DR**: 提出了一种新型机械反馈机制（mechanical feedback scheme），通过计算气体元素静止参照系中的动能增量确保能量守恒，解决了超新星（SN）反馈模拟中动量注入导致的能量不守恒问题，并在矮星系（dwarf galaxies）模拟中实现了恒星形成历史的良好收敛性，但发现银河系质量星系（Milky Way-mass galaxy）需要额外反馈机制（如活动星系核反馈（AGN feedback））来抑制恒星质量高估。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决星系形成模拟中超新星(SN)反馈模型中因动量注入的矢量特性导致的能量守恒破坏问题。

Data: 研究使用了宇宙学放大模拟（cosmological zoom-in simulations）数据，包括晕质量（halo mass）约为$10^{11},\mathrm{M}_\odot$的矮星系和$1.8\times10^{12},\mathrm{M}_\odot$的银河系质量星系，分别在两种分辨率（相差八倍）下进行。

Method: 采用了一种新的机械反馈方案（mechanical feedback scheme），通过在气体元素静止参考系中计算动能增量来确保能量守恒，并针对低密度环境切换为纯热反馈（thermal feedback）。

Result: 提出了一种新的机械反馈方案，通过计算气体元素静止系中的动能增量确保能量守恒，同时避免实验室系中可能出现的动量增量反转问题。
...
**TL;DR**: 提出了一种基于射电（radio）恒星形成率（SFR）校准Hα光度函数尘埃消光（dust extinction）的新方法，发现高红移（z≳1）下需减弱消光与光度的相关性才能重现观测到的宇宙恒星形成率密度（SFRD），表明早期星系尘埃含量或分布与本地宇宙显著不同。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用射电(radio)与H$α$辐射的恒星形成率(SFR)关系改进尘埃消光(dust extinction)校正方法，以解决高红移(z>5)星系恒星形成率测量的关键问题。

Data: 数据来自进化宇宙地图巡天(EMU)和星系与质量组装巡天(GAMA)。

Method: 采用基于射电(radio)与Hα恒星形成率(SFR)经验关系的尘埃校正方法，并通过引入随红移增加而减弱的尘埃消光(obscuration)光度依赖性模型进行改进。

Result: 通过利用射电(radio)与Hα恒星形成率(SFR)的经验关系，提出了一种新的尘埃消光校正方法，该方法在传统巴尔末线光谱技术不可用时仍适用。
...
**TL;DR**: 基于SDSS光学和GALEX紫外数据对比研究恒星质量大于$10^{10.5} M_{\odot}$的NUV-r蓝/红旋涡星系，发现蓝旋涡星系外盘(1-3 $R_{\rm e}$)存在显著紫外-光学色差且光学盘尺寸比红旋涡星系大20%，表明其通过吸积或并获取气体维持外盘恒星形成而内盘已淬灭。


<details>
  <summary>Details</summary>

Background: 研究聚焦于理解盘星系（disk galaxies）的复杂形成过程，特别是通过比较近紫外-光学（NUV-r）蓝、红旋涡星系来探讨其演化机制。

Data: 光学数据来自斯隆数字巡天(SDSS)，紫外(UV)数据来自星系演化探测器(GALEX)。

Method: 基于斯隆数字巡天(SDSS)的光学数据和星系演化探测器(GALEX)的紫外(UV)数据，采用图像分析和表面亮度剖面分析方法比较NUV-r蓝、红旋涡星系的外盘(1-3 $R_{\rm e}$)差异。

Result: NUV-r蓝旋涡星系（NUV-r blue spirals）与红旋涡星系（NUV-r red spirals）的主要差异体现在外盘（1-3 $R_{\rm e}$）区域，且NUV波段的对比度远大于光学波段。
...
**TL;DR**: 该论文开发了一种几何薄相对论吸积盘（relativistic accretion disc）模型，首次实现了星系形成模拟中对吸积盘（accretion disc）尺寸和结构的约束，并通过解析建模稳态吸积盘结构自洽演化黑洞（black hole）质量和自旋矢量，克服了经典邦迪吸积（Bondi-type prescriptions）的随机性局限，能更精确捕捉类星体（quasars）能量输出并直接提取通量和温度径向剖面以生成光谱能量分布（spectral energy distributions）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发自洽演化黑洞质量和自旋向量的几何薄相对论性吸积盘（accretion disc）模型，以改进星系形成模拟中对超大质量双黑洞（supermassive black hole binaries）吸积过程的物理描述。

Data: 研究中使用的数据来源于自主研发的几何薄相对论性吸积盘（accretion disc）模型，该模型通过解析稳态吸积盘结构自洽地演化黑洞质量和自旋向量，并采用了一套以气体或辐射主导压力、电子散射或自由-自由吸收主导不透明度的局部相对论性解。

Method: 采用自洽演化的几何薄相对论吸积盘模型（geometrically thin relativistic accretion disc model），结合局部相对论性解析解（包括气体或辐射主导压力、电子散射或自由-自由吸收主导不透明度）分段构建全局盘结构，并首次在星系形成模拟中约束吸积盘尺寸与结构。

Result: 开发并实现了一种几何薄相对论吸积盘（accretion disc）模型，首次在星系形成模拟中实现了对吸积盘尺寸和结构的约束。
...
**TL;DR**: 这篇论文通过分析18个红移（redshift）$z\approx3-7$的宽H$α$线源，发现巴尔末线（Balmer lines）轮廓与巴尔末跳变（Balmer break）强度存在系统性关联，揭示了其由光学厚度、电离态和柱密度的变化导致的辐射过程，而非动力学展宽，从而解决了标准维里校准（virial calibrations）推断黑洞质量过高的问题，并展现了中央热引擎周围复杂的气体环境。


<details>
  <summary>Details</summary>

Background: 研究探讨了JWST观测到的紧凑宽巴尔末线源（Little Red Dots）的巴尔末线轮廓特征及其与光学深度、电离状态和柱密度变化的辐射过程关联。

Data: 数据来源于JWST（James Webb Space Telescope）对18个红移（redshift）z≈3-7的宽Hα线选源的高质量光谱观测。

Method: 采用经验性分析方法研究18个宽Hα线源的光谱，结合辐射过程模型解释线轮廓变化与光学深度、电离态和柱密度变化的关系。

Result: Hα线轮廓随巴尔末跃变(Balmer break)强度系统性变化：紫外-光学颜色较蓝的源呈现窄核轮廓，较红且具巴尔末跃变的源显示蓝移吸收(P Cygni型轮廓)，最红的源表现为吸收主导的核。
...
**TL;DR**: 研究发现扇形区（SR）作为窗口使较小星际尘埃（大于约0.1微米）能深入日球层，并显著降低尘埃密度随太阳周期的变化。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际尘埃（interstellar dust）在太阳风磁场（solar wind magnetic field）作用下的传输机制及其对局域星际介质（very local interstellar medium）中尘埃特性的影响。

Data: 数据来源于模拟（simulation）计算，模型考虑了包含扇形区（SR）的日球层磁场结构对星际尘埃传输的影响。

Method: 采用包含扇形区（SR）的尘埃输运模型模拟星际尘埃与太阳风磁场的相互作用过程。

Result: 扇区区域（SR）可作为窗口使较小颗粒深入日球层，并减少尘埃密度随太阳周期的变化。
...
**TL;DR**: 研究发现低加速度条件下星系、宽双星系统和实验室实验中观测到的牛顿引力偏离现象与MOND理论框架一致，并将星系中确立的Tully-Fisher关系扩展到了更小质量尺度的宽双星系统。


<details>
  <summary>Details</summary>

Background: 研究探讨了低加速度条件下牛顿动力学修正（MOND理论）作为暗物质替代解释的可行性，并试图统一星系旋转曲线、宽双星系统动力学和卡文迪什型实验室异常现象。

Data: 数据来源包括星系旋转曲线观测数据、卡文迪什型实验室实验数据以及宽双星系统动力学观测数据。

Method: 采用修正牛顿动力学（MOND-like）框架分析低加速度条件下星系旋转曲线、宽双星系统及卡文迪什型实验室实验中的引力异常现象。

Result: 在低加速度条件下观测到的星系、宽双星系统和实验室实验对牛顿引力预测的偏离可能指向共同的物理机制。
...
**TL;DR**: 基于星系动物园(Galaxy Zoo)分类的棒状星系样本，结合MaNGA巡天的空间分辨光谱数据，发现慢速且强壮的棒状结构(bar)通过触发整个棒状区域的活跃恒星形成对宿主星系淬灭(quenching)影响最大，其中强棒主要增强棒中心的恒星形成而慢棒则促进沿棒区域的恒星形成。


<details>
  <summary>Details</summary>

Background: 星系棒（galactic bars）的强度和转速对宿主星系演化（特别是恒星形成抑制）的影响是当前星系演化研究的重要课题。

Data: 数据来源于星系动物园（Galaxy Zoo）分类的棒旋星系样本，并结合了阿帕奇点天文台附近星系测绘（MaNGA）巡天提供的空间分辨光谱数据。

Method: 基于星系动物园（Galaxy Zoo）分类样本，利用MaNGA巡天的空间分辨光谱数据，通过分析EW[Hα]、HδA、Hβ和Dn4000等不同时标恒星形成示踪因子的径向分布，构建八类棒旋星系（恒星形成/熄灭、强/弱棒、快/慢棒组合）的诊断演化平面。

Result: 慢速强棒（slow-strong bars）通过触发棒状区域内的活跃恒星形成对宿主星系产生最强的淬灭效应。
...
**TL;DR**: 通过化学演化模型首次揭示了小麦哲伦星云（SMC）中多种中子俘获元素（包括r-过程Eu、弱s-过程Zr和主s-过程Ba/La）的演化特征，发现需采用延迟r-过程增强和更轻的初始质量函数（IMF）才能解释观测到的[Eu/Fe]高平台和[s-过程/Fe]上升趋势。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系考古学（galactic archaeology）背景下的小麦哲伦云（SMC）化学演化，填补了该邻近大质量矮星系理论研究的空白。

Data: 数据来源为通过颜色-星等图拟合(color-magnitude diagram fitting)获得的恒星形成历史(star formation histories)数据。

Method: 采用基于星族合成（stellar population synthesis）的化学演化模型，结合从颜色-星等图（color-magnitude diagram）拟合获得的恒星形成历史（star formation history），并详细追踪多种核合成源（nucleosynthetic sources）的化学反馈。

Result: 首次揭示了近邻大质量矮不规则星系中多种中子俘获元素（n-capture elements）的演化特征，表明需采用延迟r过程（delayed r-process）在低金属丰度下的增强贡献及比Kroupa (2001)初始质量函数（IMF）更轻的顶部质量分布才能同时重现观测到的[Eu/Fe]高平台和[s过程/Fe]上升趋势。
...
**TL;DR**: 在红移z~0.71和z~0.75处发现两颗致密星系Pelias和Neleus，其具有极强光学发射线、低金属丰度(Z~0.1-0.4 Zsun)、低尘埃消光(Av~0.2 mag)和低恒星质量(Mstar~10^7 Msun)，中红外超揭示存在热尘埃遮蔽的活动星系核(AGN)，黑洞质量比(MBH/Mstar)达6-60%，显著偏离本地标度关系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(z ~ 0.71-0.75)致密星系中极端恒星形成活动与低金属丰度环境下超大质量黑洞(AGN)的协同演化及其与寄主星系的异常质量比例关系。

Data: 数据来源于JWST/NIRISS和JWST/NIRSpec光谱观测以及JWST/MIRI测光数据，结合MACS J0416.1-2403和GOODS-North的观测结果。

Method: 采用光谱能量分布（SED）建模结合JWST/NIRISS、JWST/NIRSpec光谱分析和JWST/MIRI测光数据，识别了极端等效宽度发射线、低金属丰度恒星群及中红外超量的热尘埃成分，并基于爱丁顿极限吸积假设估算了黑洞质量。

Result: 在红移z ~ 0.71和z ~ 0.75处发现的两个致密星系Pelias和Neleus具有极低金属丰度（Z ~ 0.1-0.4 Zsun）、低尘埃衰减（Av ~ 0.2 mag）和年轻爆发主导的恒星群，并显示出中红外超量，表明存在与深埋活动星系核（AGN）相关的热尘埃成分。
...
**TL;DR**: 通过高分辨率ALMA观测发现，大质量团块(clump)中的延展绿天体(EGO)存在多个具有外流(outflow)活动的热核(core)，表明团块尺度观测到的大质量外流活动可能源于多个核心尺度的小质量外流贡献。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量恒星形成过程中分子外流（molecular outflows）的核心作用及其在团块尺度（clump scale）与核心尺度（core scale）观测差异的机制探讨。

Data: 数据来源为阿塔卡马大型毫米波/亚毫米波阵列（ALMA）的高角分辨率观测数据。

Method: 采用高角分辨率阿塔卡马大型毫米波/亚毫米波阵列(ALMA)数据对分子气体进行观测，并在核心尺度上表征了与分子团物理参数相关的流出活动。

Result: 在核心尺度（~亚秒差距）观测到多个具有外流活动的热核（hot cores），表明大质量团块（clump）尺度观测到的外流活动可能源于多个与单个分子核（core）相关的低质量外流贡献。
...
**TL;DR**: 提出了一种高效的对数分箱估计器，首次直接从普朗克卫星的温度和偏振观测数据中重构了原初曲率扰动（primordial curvature）的非高斯性形状函数S(x,y)，实现了接近最优、高分辨率的全(x,y)平面测量，并以此探测到峰值灵敏度约0.1h Mpc⁻¹处6个e-fold的尺度不变性（scale-invariant）非高斯演化，同时通过精确自举法首次对2万多个理论模板（包括不同质量、自旋和声速）进行了毫秒级比对，在自旋二分析中发现了2.6σ显著性的粒子交换信号。


<details>
  <summary>Details</summary>

Background: 该研究探讨了宇宙暴胀（inflation）期间非线性相互作用引发的原初曲率扰动（primordial curvature）非高斯性（non-Gaussianities）及其尺度不变性（scale-invariant）特征。

Data: 数据来源于普朗克卫星（Planck）的温度和偏振（polarization）地图。

Method: 采用对数分箱估计器（logarithmically-binned estimator）在原始空间（primordial-space）直接重构形状函数$S(x,y)$，并结合模态程序（modal program）高效分析普朗克温度与偏振数据。

Result: 提出了一种高效的对数分箱估计器，直接在原初空间重构形状函数S(x,y)，实现了接近最优、高解释性且保留标准模板信息内容的非高斯性测量。
...
**TL;DR**: 开发了专为维拉·鲁宾天文台设计的SLSim模拟工具，能高效生成符合实际观测条件的强引力透镜（strong gravitational lensing）合成数据，用于宇宙学研究和透镜搜索算法测试。


<details>
  <summary>Details</summary>

Background: 引力透镜（gravitational lensing）通过光线在致密天体周围的弯曲为宇宙学研究提供独特视角，尤其强引力透镜（strong gravitational lensing）产生的放大或多重像对精确宇宙学测量和暗物质（dark matter）分布研究至关重要。

Data: 数据来源于为Vera C. Rubin天文台定制的SLSim模拟工具生成的合成强透镜（strong lens）群体数据，该工具整合了先进的天体物理模型并模拟了静态和可变透镜场景。

Method: 采用先进天体物理模型与计算效率结合的合成强透镜（strong lens）群体模拟工具SLSim，针对真实观测条件生成静态和可变透镜场景。

Result: 强引力透镜（strong gravitational lensing）通过弯曲光线产生放大和多重像，为精确宇宙学测量和暗物质（dark matter）分布研究提供关键信息。
...
**TL;DR**: 提出了一种针对部分天区球谐变换（SHT）的高效新算法，在南极望远镜（SPT-3G）主视场和BICEP3深天区观测中实现了3至10倍的加速，解决了任意像素化方案相比等纬度像素化或平面天区近似方法的传统劣势。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发一种新型部分天区球谐变换（SHT）算法，旨在高效处理球面上大量任意位置的高分辨率计算需求，主要应用于宇宙微波背景透镜化（CMB lensing）和原初引力波探测等前沿宇宙学研究领域。

Data: data：研究使用了南極望遠鏡（South Pole Telescope）SPT-3G主觀測場和BICEP3深場觀測數據作為算法測試的實際數據來源。

Method: 采用基于傅里叶球方法（Fourier-sphere method）的新型算法实现部分天区球谐变换（partial-sky spherical harmonic transforms），高效处理球面上大量任意位置的高分辨率计算需求。

Result: 提出了一种新型部分天区球谐变换（SHT）算法，在南极望远镜主视场和BICEP3深天区观测中实现了3至10倍的加速效果。
...
**TL;DR**: 采用物理信息神经网络(PINNs)的两种变体重构了Ia型超新星绝对星等$M_B(z)$，发现埃瑟林顿距离对偶关系(DDR)比宇宙学模型先验更具约束力，并在$z\sim0.4-0.5$处发现跨模型和基准集的$2-3σ$残留信号。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用物理信息神经网络（PINNs）重构Ia型超新星绝对星等（Type Ia supernova absolute magnitude）$M_B(z)$，并基于重子声波振荡（BAO）和超新星联合数据在四种宇宙学模型下验证距离对偶关系（Etherington distance duality relation）的基本性。

Data: 数据来源于联合重子声波振荡（BAO）和超新星（SN）数据，并使用了DESI DR2两组基准集（fiducial sets）。

Method: 采用物理信息神经网络（Physics-Informed Neural Networks, PINNs）的两种变体（异方差单网络方法和基于费希尔信息加权的双网络方法）重建Ia型超新星绝对星等$M_B(z)$。

Result: 在四种宇宙学模型和两组DESI DR2基准集下，异方差单网络方法证实埃瑟林顿距离对偶关系（Etherington distance duality relation）是比宇宙学模型先验更基本的约束条件，物理约束下的DDR（distance duality relation）违反为30-52毫米星等，而无约束时为85-2330毫米星等。
...
**TL;DR**: 提出一种实现慢滚(slow-roll)到超慢滚(ultra-slow-roll)平滑过渡的单场暴胀模型，通过引入穆哈诺夫-萨斯基(Mukhanov-Sasaki)方程有效质量项的简单时间依赖性，首次获得背景演化和曲率扰动的全解析解，并对比了其与急剧过渡模型的差异。


<details>
  <summary>Details</summary>

Background: 研究主题为构建实现慢滚（slow-roll）到超慢滚（ultra-slow-roll）平滑过渡的单场暴胀（single-field inflation）模型。

Data: data：该研究基于理论推导构建的单场暴胀（single-field inflation）模型，通过引入穆哈诺夫-萨斯基方程（Mukhanov-Sasaki equation）有效质量项的简单时间依赖性获得解析解，未使用实际观测或模拟数据。

Method: 采用单场暴胀模型（single-field inflation model）并通过在Mukhanov-Sasaki方程的有效质量项中引入简单时间依赖性来实现慢滚到超慢滚（slow-roll-to-ultra-slow-roll）的平滑过渡。

Result: result：该模型首次实现了慢滚（slow-roll）到超慢滚（ultra-slow-roll）的平滑过渡，并获得了背景演化和曲率扰动的全解析解。
...
**TL;DR**: 通过结合Pantheon$+$超新星数据、人工神经网络(ANN)距离重建和两种伽马暴(GRB)相关性(Amati和Combo)，建立了一个从低红移到高红移的距离阶梯，解决了GRB宇宙学中的循环性问题，将距离测量扩展到红移$z \sim 9$并约束了$\Lambda\mathrm{CDM}$和$w_0 w_a \mathrm{CDM}$模型的参数。


<details>
  <summary>Details</summary>

Background: 伽马射线暴（GRB）可作为高红移宇宙学探针，但其作为距离指示器的应用受限于校准经验性光度关系时的循环性问题（circularity problem），本研究旨在通过结合多种观测数据构建距离阶梯（distance ladder）来克服这一限制。

Data: data部分：数据来源包括Pantheon$+$超新星数据集、费米伽马射线暴监测器(Fermi GBM)和雨燕卫星X射线望远镜(Swift-XRT)提供的低红移(z<1)伽马射线暴样本。

Method: 采用人工神经网络（ANN）重构光度距离（luminosity distance）和距离模数（distance modulus），结合近似贝叶斯计算（approximate Bayesian computation）拒绝法和风险函数优化超参数，并联合贝叶斯分析同时约束宇宙学参数和伽马暴（GRB）相关参数。

Result: 通过结合Pantheon$+$超新星数据、人工神经网络距离重建和两种伽马暴（GRB）相关性（Amati和Combo），建立了一个从低红移到高红移的距离阶梯，解决了伽马暴宇宙学中的循环性问题，并将距离测量扩展至红移$z \sim 9$，确立了伽马暴作为高红移探针的地位。
...
**TL;DR**: 基于脉冲星计时阵（pulsar timing array）观测数据，通过改进的张量模式色散关系分析，预测未来30年可探测到引力波相速度（phase velocity）与光速存在10%或-1%偏差的3σ证据。


<details>
  <summary>Details</summary>

Background: 探测纳赫兹频段随机引力波背景（stochastic gravitational wave background）以检验修正引力理论（modified gravity）中的张量模式（tensor modes）色散关系（dispersion relation）。

Data: 数据来源于脉冲星计时阵列（pulsar timing array, PTA）系统对银河系内纳赫兹波段随机引力波背景（stochastic gravitational wave background）的观测结果，并通过模拟数据（mock-data）研究验证了费希尔分析（Fisher analysis）的可靠性。

Method: 采用费希尔分析（Fisher analysis）结合模拟数据研究（mock-data study）来预测张量模式修正色散关系的相位速度不确定性（phase velocity uncertainty），并考虑样本方差效应（sample variance effect）。

Result: 探测到银河系中存在纳赫兹波段随机引力波背景（stochastic gravitational wave background），可用于检验修正引力理论并预测未来对特定偏差的探测灵敏度。
...
**TL;DR**: 研究发现帕拉蒂尼(Palatini)引力框架下高斯-博内(Gauss-Bonnet)项与暴胀子耦合时，不同于度规形式，该项并非总导数项，且在三种连接约束情况下均导致暴胀子动能项和引力波修正项与陈-西蒙斯(Chern-Simons)项形式相同但符号相反。


<details>
  <summary>Details</summary>

Background: 研究探讨了Palatini引力框架下高斯-博内项(Gauss-Bonnet term)与暴胀子(inflaton)耦合的理论特性及其与度量形式(metric formulation)的差异。

Data: 数据来源于理论模拟（simulation），基于空间平坦的FLRW时空精确求解连接项，并在一般时空中采用梯度近似和阶数约简方法，同时考虑了无约束连接、零非度量性（non-metricity）和零挠率（torsion）三种情况。

Method: 采用帕拉蒂尼(Palatini)形式下的梯度近似和降阶方法，精确求解空间平坦FLRW时空的连接项并代入作用量，分析无约束连接、零非度量性(non-metricity)和零挠率(torsion)三种情况。

Result: 在帕拉蒂尼(Palatini)引力框架下，与暴胀子耦合的高斯-博内(Gauss-Bonnet)项会以与陈-西蒙斯(Chern-Simons)项相同形式但符号相反的方式修正暴胀子动能项和引力波部分。
...
**TL;DR**: 提出了一种结合普朗克总强度和偏振频率图的混合小波内线性组合（Hybrid NILC）方法，有效抑制了热Sunyaev-Zeldovich（SZ）康普顿-y参数图中银河系前景（热尘埃和同步辐射）的残余污染，并通过约束混合NILC变体选择性去除宇宙红外背景（CIB）各向异性，在降低银河系残余污染40%的同时保持了非偏振SZ信号。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用极化信息改进热苏尼亚耶夫-泽尔多维奇效应(SZ)康普顿-y参数图(y-maps)中银河系前景辐射（热尘埃和同步辐射）的分离方法。

Data: 数据来源为普朗克卫星第四版数据发布（Planck PR4）的总强度和偏振频率图，以及用于模拟分析的未来高灵敏度巡天（如LiteBIRD）仿真数据。

Method: 采用混合小波内线性组合方法（Hybrid NILC），结合普朗克卫星的总强度和偏振频率图，利用热尘埃和同步辐射的固有TE/TB相关性抑制残余银河系发射。

Result: 提出的混合小波内部线性组合（Hybrid NILC）方法通过联合利用普朗克总强度和偏振频率图，在保留未偏振SZ信号的同时有效抑制了银河系残余辐射（热尘埃和同步辐射）的污染。
...
**TL;DR**: 通过改进传统体积测试方法并整合星系质量函数与演化历史，提出修订的演化体积托尔曼测试（revised evolutionary volume Tolman test），实现了对暗能量（dark energy）状态方程等宇宙学参数的高精度测量。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过整合星系质量函数观测数据与恒星形成及并合历史，重新构建基于星系体积数密度演化的经典宇宙学测试方法（volume test），以精确测量暗能量（dark energy）状态方程等关键宇宙学参数。

Data: 数据来源于深空河外巡天（deep extragalactic surveys）的最新观测结果，包括星系质量函数（galaxy mass functions）的测量数据以及星系演化历史中的恒星形成（star formation）和并合（mergers）信息。

Method: 采用改进的演化体积托尔曼测试（revised evolutionary volume Tolman test），结合星系质量函数和星系演化历史（包括恒星形成与并合过程）作为宇宙学参数计算的核心部分。

Result: 通过改进传统体积密度测试方法并结合星系质量函数及演化历史，提出了一种修订的演化体积托尔曼测试（revised evolutionary volume Tolman test），能够有效测量宇宙学参数特别是暗能量（dark energy）的动力学性质。
...
### [31] [A Minimal Four-Thruster System for Comet-Based Interstellar Navigation](https://arxiv.org/abs/2603.16981)
*Bo Pieter Johannes Andrée*

**TL;DR**: 研究发现仅需四个推进器（一个主喷口和三个120度间隔的副喷口）即可通过前锥转向实现对星际彗星（interstellar comet）轨迹的导航控制，利用其自然旋转特性合成平面内转向并通过相位调度主喷口实现离面转向。


<details>
  <summary>Details</summary>

Background: 研究主题为探讨如何利用星际彗星（interstellar comets）的固有特性（如挥发物库存、自然旋转和双曲线轨迹）实现对其轨迹的最小化控制改造，以支持深空探测平台构建。

Data: data来源为数值模拟（simulation）分析星际彗星（interstellar comets）的动力学特性与控制策略。

Method: 采用前向锥体转向（forward-cone steering）和四推进器（one primary jet and three secondary jets）架构实现旋转介导的轨迹控制，通过主推进器相位调度点火完成离面转向。

Result: 星际彗星（interstellar comets）仅需四个推进器（一个主喷口和三个120度间隔的副喷口）即可实现可控轨迹导航，通过前锥转向和旋转相位调度实现姿态控制。
...
### [32] [Trajectory Stability and Signature Diagnostics for Comet-Based Interstellar Navigation](https://arxiv.org/abs/2603.16982)
*Bo Pieter Johannes Andrée*

**TL;DR**: 该论文针对利用挥发性星际天体（ISOs）进行深空导航的任务设计问题，建立了基于喷射修正的轨迹跟踪稳定性理论框架，揭示了高速穿越几何约束下非引力扰动与主动稳定控制的四层稳定性条件（扰动能量稳定、外环收缩、执行器记忆稳定和旋转介导的Floquet稳定），并提出了可通过短时误差修正、事件条件持续性降低等残差特征区分自然动态与受控运动的实证检验指标。


<details>
  <summary>Details</summary>

Background: 研究探讨了利用星际天体（ISOs）的固有双曲运动和原位推进剂进行持续深空导航时，在非引力强迫下稳定主动控制与自然动力学区分的天体测量特征及稳定性要求。

Data: 数据来源于对星际物体（ISOs）的观测数据（observational data）和基于航天器动力学模型的仿真数据（simulation data）。

Method: 采用稳定性理论框架（stability-theoretic framework）分析喷流驱动修正（jet-actuated correction）下的轨迹跟踪问题，并通过四层稳定性条件（扰动能量稳定性、外环收缩性、执行器记忆稳定性及旋转介导的Floquet稳定性）建模跟踪残差（tracking residuals）与扰动-修正平衡关系。

Result: 高速穿越几何结构（包括碎片带规避和相遇相位）严格限制了可行轨迹，使得长期跟踪稳定性成为任务关键。
...
### [33] [Quantifying the Scientific Potential of Intermediate and Extreme Mass Ratio Inspirals with the Laser Interferometer Space Antenna](https://arxiv.org/abs/2603.17072)
...
**TL;DR**: 激光干涉空间天线(LISA)将实现对极端质量比旋近(EMRI)和中等质量比旋近(IMRI)的高精度观测，揭示星系核天体物理环境和强场引力特性，其中EMRI对仪器性能退化最敏感(红移视界z~0.01)，IMRI最不敏感(红移视界z~1-3)，所有顺行系统在三个月观测内可实现亚百分之一自旋测量精度，4.5年完整任务将显著提升探测视界和定位精度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用激光干涉空间天线（LISA）探测极端质量比旋近（EMRIs）和中等质量比旋近（IMRIs），以探索星系核天体物理环境和强场引力效应。

Data: 研究使用了激光干涉空间天线（Laser Interferometer Space Antenna, LISA）的仿真数据，覆盖主质量$m_1 \in [5\times10^4, 10^7]\,M_\odot$和次质量$m_2 \in [1, 10^4]\,M_\odot$的范围。

Method: 采用完全相对论性分析流程（fully relativistic pipeline）对主质量$m_1 \in [5\times10^4, 10^7]\,M_\odot$和次质量$m_2 \in [1, 10^4]\,M_\odot$范围内的极端质量比旋近（EMRIs）和中等质量比旋近（IMRIs）进行建模。

Result: LISA（激光干涉空间天线）将实现对极端质量比旋近(EMRIs)和中等质量比旋近(IMRIs)的高精度研究，为研究星系核天体物理环境和强场引力提供独特手段。
...
### [34] [An Analysis of Large Astronomical Detector Controller Systems and Implications for Future ESO Detector Systems](https://arxiv.org/abs/2603.17389)
...
**TL;DR**: 论文分析了使用10个以上探测器（detector）的地基天文仪器控制方法，提出了结合低温电子学（cryogenic electronics）、以太网接口（Ethernet based interfaces）和全数字探测器（fully-digital detectors）的高效控制方案，可作为ESO下一代通用控制器NGCII的补充。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大型天文仪器中多探测器（10个以上）控制系统的工程挑战与技术优化，涉及性能、可扩展性（scalability）、功耗及成本等关键因素。

Data: 研究中使用的数据来源于对现有地面天文仪器（ground based astronomical instruments）采用10个或更多探测器（detectors）的科学成像或光谱分析方法的分析。

Method: 采用基于以太网（Ethernet）的接口、低温电子学（cryogenic electronics）和全数字探测器（fully-digital detectors）等关键技术实现多探测器高效控制。

Result: 采用低温电子学(cryogenic electronics)、基于以太网(Ethernet)的接口和全数字探测器(fully-digital detectors)等关键技术可实现多探测器的高效控制。
...
### [35] [KMTNet Synoptic Survey of Southern Sky II: Data Reduction and Real-Time Transient Detection Pipeline](https://arxiv.org/abs/2603.17442)
...
**TL;DR**: 开发了一套用于处理KMTNet南天巡天（KS4）首期数据的综合流程，实现了亚像素级天体测量精度（<0.4角秒）和5σ深度达22-23 AB星等的BVRI波段测光校准，覆盖南天超4000平方度区域并支持实时瞬变源探测。


<details>
  <summary>Details</summary>

Background: 研究主题为开发用于KMTNet南天同步巡天（KS4）数据发布1的图像处理综合流程。

Data: 数据来源包括KMTNet Synoptic Survey of the Southern Sky (KS4) Data Release 1的观测数据，并交叉验证使用了Gaia EDR3星表、APASS DR9星表和SkyMapper DR3星表进行天体测量和测光校准，最终参考Gaia XP测光数据校正空间变化。

Method: 采用的核心分析方法包括数据质量保证、天体测量(astrometry)、光度零点(ZP)校准、坏像素掩膜、图像叠加和差分图像分析(DIA)，并通过与盖亚(Gaia) EDR3星表交叉验证确保亚像素级天体测量精度(<0.4角秒)。

Result: 开发了一套全面的KS4数据发布1图像处理流程，实现了亚像素级天体测量精度（<0.4角秒）和跨BVRI波段22-23 AB星等的5σ探测深度。
...
### [36] [Long-algorithm based quantum search for gravitational wave](https://arxiv.org/abs/2603.17698)
*Fangzhou Guo,Jibo He*

**TL;DR**: 该论文首次将基于Long算法的量子匹配滤波框架应用于引力波（gravitational wave）探测，数值模拟表明该方法在保持量子搜索二次加速的同时显著提高了鲁棒性，克服了基于Grover算法方法的关键限制。


<details>
  <summary>Details</summary>

Background: 引力波天文学（gravitational wave astronomy）的快速发展推动了对量子数据处理方法的探索。

Data: 数据来源于数值模拟（numerical simulation）。

Method: 采用基于龙算法(Long algorithm)的量子匹配滤波框架进行引力波探测。

Result: 基于长算法(Long algorithm)的量子匹配滤波框架首次应用于引力波数据分析，在保持量子搜索二次加速的同时显著提高了鲁棒性，克服了基于格罗弗算法(Grover-algorithm)方法的关键限制。
...
### [37] [Test of a 34 GHz EOM laser frequency comb at ESPRESSO](https://arxiv.org/abs/2603.17908)
...
**TL;DR**: 该研究测试了34 GHz电光调制频率梳（electro-optic modulation comb）在ESPRESSO光谱仪中的应用，发现其波长校准稳定性达17 cm/s，但局部精度差异达15 m/s，揭示了仪器光纤对准问题对校准的影响，并强调需采用非高斯线扩散函数（line-spread function）建模以实现高精度校准。


<details>
  <summary>Details</summary>

Background: 激光频率梳（LFC）技术为天文高分辨率光谱仪提供了基于国际单位制（SI）时间基准的高精度波长校准方案，但实际应用仍受限于系统复杂性和数据分析挑战。

Data: 数据来源于使用34 GHz电光调制梳（electro-optic modulation comb）与ESPRESSO光谱仪的详细测试，通过薄膜铌酸锂波导（thin-film lithium-niobate waveguides）进行展宽和谐波生成，覆盖了部分红外、可见光和近紫外光谱范围。

Method: 采用34 GHz电光调制频率梳（electro-optic modulation comb）结合薄片铌酸锂波导（lithium-niobate waveguides）进行光谱展宽和谐波生成，并通过非高斯线扩散函数（non-Gaussian line-spread function）建模分析光谱质量与波长校准精度。

Result: 基于34 GHz电光调制光梳（electro-optic modulation comb）与ESPRESSO光谱仪的测试，研究发现波长校准存在局部差异（达15m/s），与阶梯光栅结构相关，但光梳本身表现出极佳稳定性（散射仅17cm/s），需通过精确建模非高斯线扩散函数实现。
...
### [38] [Simulations of a 2 x 1.5D coded aperture camera for X-ray astronomy](https://arxiv.org/abs/2603.17938)
*J. J. M. in 't Zand,L. Kuiper,F. Ceraudo,Y. Evangelista,M. Hernanz,A. Patruno*

**TL;DR**: 论文提出了一种基于迭代源移除算法（IROS）和最大似然法（MLM）的解码方法，用于优化宽视场监测器（WFM）在探测瞬变X射线源（如X射线新星、伽马暴和致密天体并合引力波事件的电磁对应体）时的成像性能和源流量测定精度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宽视场监测器（WFM）的设计与应用，旨在通过一维编码孔径相机技术监测宇宙X射线瞬变现象，服务于空间X射线天文台任务。

Data: 数据来源为模拟的探测器对X射线天空的响应，包括前端和后端电子设备的信号处理。

Method: 采用迭代源移除法（IROS）结合互相关分析以及最大似然法（MLM）进行图像解码。

Result: WFM（Wide Field Monitor）采用两个垂直的一维编码孔径相机设计，具有精细（约5角分）和粗糙（约5度）的角分辨率，形成所谓的“1.5D”成像能力。
...
### [39] [Extended Theories of Electrodynamics in $f(R)$ Gravity](https://arxiv.org/abs/2603.16979)
*Francesco Bajardi,Micol Benetti,Salvatore Capozziello,Abedennour Dib*

**TL;DR**: 在$f(R)$引力框架下引入与电磁曲率不变量$f(\mathbb{F})$的最小耦合函数，推导出克莱因-戈登方程推广解和非线性无质量解，并在平直时空下还原为普莱班斯基模型和博普-波多尔斯基电动力学。


<details>
  <summary>Details</summary>

Background: 研究在$f(R)$引力理论框架下探索电磁曲率不变量$f(\mathbb{F})$与引力的最小耦合，旨在统一处理这类理论中的曲率函数。

Data: 数据来源于理论推导（simulation）和已有模型（Plebanski家族模型及Bopp-Podolsky电动力学）的对比分析。

Method: 在$f(R)$引力理论框架下引入与电磁曲率不变量$f(\mathbb{F})$最小耦合的函数，确保理论中曲率函数处理的一致性。

Result: 在$f(R)$引力理论框架下引入电磁曲率不变量$f(\mathbb{F})$的最小耦合函数，推导出两类解分别推广了克莱因-戈登方程和产生典型非线性无质量解。
...
### [40] [Modified Friedmann equations and non-singular cosmologies in $d=4$ non-polynomial quasi-topological gravities](https://arxiv.org/abs/2603.17654)
*Johanna Borissova,João Magueijo*

**TL;DR**: 研究发现在四维非多项式曲率准拓扑引力（quasi-topological gravity）框架下，纯辐射或标准物质填充的宇宙存在三种无大爆炸奇点的解：源自德西特相（密度发散但曲率有限）、反弹宇宙（需多值拉氏量）或渐近闵氏起源（能量密度非奇异且近似恒定）。


<details>
  <summary>Details</summary>

Background: 研究探讨了准拓扑引力理论（quasi-topological gravity）在解决黑洞奇点后，能否进一步消除宇宙学奇点（cosmological singularities），特别是大爆炸奇点（Big-Bang singularity）。

Data: 研究使用了非多项式曲率准拓扑引力理论（non-polynomial curvature quasi-topological gravities）在四维时空中的模拟数据（simulation data）。

Method: 采用非多项式曲率拟拓扑引力理论（non-polynomial curvature quasi-topological gravities）分析四维时空下宇宙学奇点的消除机制。

Result: 准拓扑引力理论（quasi-topological gravity）在四维时空中通过三种非奇异宇宙学解避免了大爆炸奇点：从德西特（de Sitter）相演化、反弹宇宙（bouncing universe）以及渐近闵可夫斯基（asymptotically Minkowski）起源。
...
### [41] [Stellar characterization with photometric colors from J-PLUS and 2MASS surveys](https://arxiv.org/abs/2603.17083)
...
**TL;DR**: 基于J-PLUS和2MASS测光数据，采用半监督机器学习方法（k-means结合改进k近邻算法）成功估算了560万颗恒星的有效温度（$T_{\rm eff}$）、表面重力（$\log{g}$）和金属丰度（[Fe/H]），并通过降维将计算参数所需颜色缩减至11种，验证了该无监督方法在数据质量不足时仍能可靠估算参数。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用J-PLUS（Javalambre Photometric Local Universe Survey）和2MASS（Two Micron All-Sky Survey）的光度数据，通过半监督机器学习方法估算恒星大气参数（有效温度、表面重力及金属丰度），以扩展大样本恒星参数的高效测定能力。

Data: 数据来源为Javalambre测光局部宇宙巡天（J-PLUS）的第三次数据发布（DR3）测光数据以及2微米全天巡天（2MASS）的近红外测光数据。

Method: 采用基于k均值（k-means）的半监督机器学习方法结合改进的k近邻（k-nearest neighbors）算法，通过将观测测光数据与参考数据集对比来估计恒星有效温度（$T_{\rm eff}$）、表面重力（$\log{g}$）和金属丰度（[Fe/H]），并应用降维方法提升计算效率。

Result: 基于J-PLUS和2MASS测光数据，采用半监督机器学习方法（k-means结合改进的k近邻算法）成功估算了约560万颗恒星的有效温度（$T_{\rm eff}$）、表面重力（$\log{g}$）和金属丰度（[Fe/H]），结果与LAMOST和APOGEE光谱数据一致，并通过降维方法将计算参数所需颜色缩减至11个，进一步扩展到约600万颗恒星。
...
### [42] [The Sunrise Chromospheric Infrared Spectro-Polarimeter SCIP: an instrument for SUNRISE III](https://arxiv.org/abs/2603.17929)
...
**TL;DR**: Sunrise III搭载的新型近红外光谱偏振仪（SCIP）通过0.21角秒空间分辨率和2×10^5光谱分辨率在770 nm和850 nm波段实现了0.03%偏振灵敏度，为研究光球层和色球层的磁场及动力学过程提供了独特观测手段。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用Sunrise III气球载太阳观测平台开展高精度太阳光球层和色球层磁场、速度场的多波段（紫外、可见光、近红外）光谱偏振测量，以揭示太阳能量传输和动力学过程。

Data: 数据来源于Sunrise III气球载太阳观测平台的SCIP（Spectro-Polarimeter for Infrared and Optical Regions）光谱偏振仪，通过旋转波片和偏振分束器在770 nm和850 nm近红外波段进行高精度偏振测量。

Method: 采用旋转波片（rotating waveplate）作为调制器和偏振分束器（polarizing beam splitters）进行近红外波段（770 nm和850 nm）的精密光谱偏振测量（spectro-polarimeter），空间分辨率为0.21"，光谱分辨率为2x10^5，偏振灵敏度达到连续谱强度的0.03%（1σ）。

Result: Sunrise III搭载的新型光谱偏振仪SCIP实现了0.21角秒空间分辨率和2×10^5光谱分辨率，在10秒积分时间内达到0.03%（1σ）的偏振测量灵敏度，用于探测光球层和色球层中与太阳磁场及速度相关的近红外（770 nm和850 nm）谱线。
...
### [43] [A 3D physico-chemical model of a pre-stellar core. II. Dynamic chemical evolution in a pre-stellar core model using tracer particles](https://arxiv.org/abs/2603.17696)
*S. S. Jensen,S. Spezzano,P. Caselli,T. Grassi,O. Sipilä,T. Haugbølle*

**TL;DR**: 通过对比静态与动态物理化学模型对前恒星核（pre-stellar core）L1544的模拟，发现动态模型能更准确再现CH$_3$OH和$c$-C$_3$H$_2$的空间分布，而静态模型在多数分子丰度和辐射强度预测上更接近观测值但无法重现形态差异。


<details>
  <summary>Details</summary>

Background: 研究比较了静态与动态演化物理化学模型在预恒星核（pre-stellar cores）化学形态和分子辐射特性上的差异。

Data: 数据来源于对L1544的观测结果，包括CH$_3$OH和$c$-C$_3$H$_2$的发射形态以及CH$_3$OH、$c$-C$_3$H$_2$、CS、SO、HCN、HCO$^+$和N$_2$H$^+$的强度。

Method: 采用三维磁流体力学（MHD）模型结合尘埃辐射传输、气-尘化学模型和非局域热动平衡（non-LTE）谱线辐射传输模型，沿约20,000条示踪粒子轨迹模拟化学演化过程。

Result: 动态模型能重现L1544中CH$_3$OH和$c$-C$_3$H$_2$的空间分布，而静态模型则无法匹配这种形态。
...
### [44] [PRODIGE -- envelope to disk with NOEMA VIII. Sulfur oxides trace a shock caused by a streamer in the inner envelope of a protostar](https://arxiv.org/abs/2603.17918)
...
**TL;DR**: 通过观测原恒星Per-emb 50周围SO₂和SO的分布及运动特征，发现两个分别位于180天文单位（AU）和400 AU的发射峰，证实了内流-旋转包层与星流（streamer）相互作用产生的低速激波（~3-4 km/s）会显著影响原行星盘和包层的物理化学结构。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原行星盘（protoplanetary disks）外缘激波与下落流（infalling streamers）引发的物理化学变化，特别是通过含硫分子（SO$_2$和SO）追踪流体的动力学过程及其对恒星形成早期阶段盘-包层结构的塑造作用。

Data: 数据来源为NOEMA A-array观测数据（来自大型项目PRODIGE）以及同一项目中补充的H_2CO和CO数据。

Method: 采用NOEMA A-阵列观测数据，结合局域热动平衡（LTE）与非局域热动平衡（non-LTE）分析，通过贝叶斯模型选择（Bayesian model selection）和下落旋转（infalling-rotating）模型比对，解析了SO₂和SO的形态与运动学特征。

Result: 在Per-emb 50周围观测到两个SO$_2$发射峰（peak 1和peak 2），peak 2的蓝移特征表明其由内包层与流（streamer）的激波引起，而peak 1的运动学特征与包层一致但可能受流影响，两者丰度比均符合低速激波（$\sim 3-4$ \kms）模型。
...
### [45] [Multi-Component Dark Matter as a Solution to the Galactic Center GeV Excess](https://arxiv.org/abs/2603.17095)
*Farinaldo S. Queiroz,Clarissa Siqueira,Carlos E. Yaguna*

**TL;DR**: 研究发现银河中心过剩辐射(GCE)数据更支持由两种暗物质粒子(N=2)组成的多组分模型，其中每种组分分别湮灭为单一末态，且具有轻重质量层级结构，该模型能更好解释观测特征并缓解现有约束矛盾。


<details>
  <summary>Details</summary>

Background: 银河中心过剩辐射（GCE）可能源自多组分暗物质（multi-component dark matter）湮灭，挑战了单一粒子物种的传统范式。

Data: 数据来源于银河系中心过剩(GCE)的观测数据，通过系统分析双组分(N=2)和三组分(N=3)暗物质粒子模型下的湮灭谱形态。

Method: 采用赤池信息量准则（AIC）对多组分暗物质（multi-component dark matter）模型进行统计评估，结合双粒子（N=2）和三粒子（N=3）的排他性或混合湮灭通道（exclusive/mixed annihilation channels）分析。

Result: 银河中心过剩(GCE)数据在统计上更支持包含两种暗物质组分(N=2)的模型，其中每种组分专一地湮灭为单一末态，且优选解呈现轻质量加重质量的层级结构。
...
### [46] [Scale-Aware Navigation of Astronomical Survey Imagery Data on High Resolution Immersive Displays](https://arxiv.org/abs/2603.17337)
*Ava Nederlander,Zainab Aamir,Arie E. Kaufman*

**TL;DR**: 提出了一种面向高分辨率沉浸式显示环境的天文巡天图像尺度感知导航框架，通过薇拉·鲁宾天文台(Vera C. Rubin Observatory)和银河系巡天图像的典型应用场景验证了该框架在整合全局结构与局部细节方面的有效性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于天文巡天影像在空间尺度上的多层级探索挑战，提出沉浸式显示环境下的尺度感知导航框架以解决传统桌面工作流中全局与局部信息割裂的问题。

Data: 数据来源于薇拉·鲁宾天文台(Vera C. Rubin Observatory)和银河系巡天(Milky Way survey)的观测图像，部署在包括平铺高分辨率显示器和曲面沉浸式系统在内的房间级沉浸式环境中。

Method: 采用基于沉浸式显示环境（immersive display environments）的尺度感知导航（scale-aware navigation）框架进行天文图像探索分析。

Result: 设计导向框架支持在天文调查图像中进行尺度感知导航（scale-aware navigation），适用于高分辨率沉浸式显示环境。
...
### [47] [Ultrahigh-Energy Gamma-Ray Sources Need Not Be Hadronic PeVatrons](https://arxiv.org/abs/2603.17012)
*Zachary Curtis-Ginsberg,Dan Hooper,Justin Vandenbroucke*

**TL;DR**: 研究发现超高能伽马射线（$E_γ>100 \, {\rm TeV}$）源（如微类星体SS 433、银河系中心和TeV晕）可通过纯轻子（leptonic）模型解释，无需假设它们是PeV能级质子（hadronic cosmic rays）加速器（PeVatrons）。


<details>
  <summary>Details</summary>

Background: 研究探讨了超高能伽马射线（$E_γ>100 \, {\rm TeV}$）的起源是否必须归因于PeV级质子加速器（PeVatrons），并提出了纯轻子（leptonic）模型可解释这些观测现象的科学背景。

Data: 研究中使用的数据来源包括对微类星体SS 433、银河系中心以及TeV晕（TeV halos）的超高能伽马射线（$E_γ>100 \, {\rm TeV}$）观测数据。

Method: 采用简单轻子模型（leptonic models）分析超高能伽马射线（$E_γ>100 \, {\rm TeV}$）的起源，并通过微类星体SS 433、银河系中心和TeV晕的观测数据验证模型预测。

Result: 观测到的100 TeV以上超高能伽马射线可通过纯轻子（leptonic）模型合理解释，无需假设存在PeV级质子加速器（PeVatron）。
...
### [48] [Diffuse X-ray Emission in the Sagittarius C Complex](https://arxiv.org/abs/2603.17210)
*Zhenlin Zhu,Mark R. Morris,Gabriele Ponti,Ping Zhou*

**TL;DR**: 研究发现人马座C（Sgr C）复合体存在两个X射线辐射成分（H II区和西南部弥散特征），其光谱分析揭示了软（≤1 keV）和硬（~8 keV）热等离子体，结合[C II]壳层膨胀特征表明该区域可能嵌有一个年轻核心坍缩超新星遗迹（SNR），年龄≥1.7 kyr，揭示了银河系中心区（CMZ）中大质量恒星反馈、磁场与分子气体的复杂相互作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于人馬座C（Sgr C）複合體在中央分子區（CMZ）邊緣的X射線輻射特性及其與大質量恒星反饋、磁場和分子氣體的相互作用。

Data: 数据来源于钱德拉(Chandra)和XMM-牛顿(XMM-Newton)的深度存档观测数据。

Method: 采用深度钱德拉（Chandra）和XMM-牛顿（XMM-Newton）观测数据进行空间分辨光谱分析，结合软（kT ≤ 1 keV）和硬（~8 keV）热等离子体组分模型，并利用金属丰度与中央分子区（CMZ）预期值的一致性约束，推断超新星遗迹（SNR）的激波速度（~800 km/s）和年龄（≥1.7 kyr）。

Result: Sgr C区域的弥散X射线辐射可分解为与射电峰值重合的H II区（H II region）和其西南侧更亮的弥散特征，光谱分析显示两区域均存在软（kT ≤ 1 keV）等离子体（plasma）及H II区内更硬（~8 keV）的热成分（thermal component），结合[C II]壳层（[C II] shell）膨胀特征表明热气体可能源自H II区内的年轻超新星遗迹（SNR），其激波速度（~800 km/s）和年龄（≥1.7 kyr）符合银河系中心大质量恒星核心坍缩（core-collapse）SNR的预期。
...
### [49] [On the Astrophysical Origin of Binary Black Hole Subpopulations: A Tale of Three Channels?](https://arxiv.org/abs/2603.17987)
*Anarya Ray,Shirsha Mukherjee,Michael Zevin,Vicky Kalogera*

**TL;DR**: 通过参数化混合模型发现LIGO-Virgo-KAGRA探测到的双黑洞(BBH)样本包含三个可能由孤立双星演化、球状星团动力学形成和多代并合主导的子群，其相对比例分别为$79.0^{+11.5}_{-10.9}\%$、$14.5^{+11.6}_{-8.0}\%$和$2.5^{+5.5}_{-1.8}\%$，且这些比例随宇宙时间显著演化。


<details>
  <summary>Details</summary>

Background: 引力波观测揭示双黑洞（BBH）群体存在多个子结构，其天体物理起源涉及双星演化、核坍缩和宿主环境等理论不确定性。

Data: 数据来源为LIGO-Virgo-KAGRA合作组积累的引力波目录中检测到的双黑洞（BBH）样本。

Method: 采用参数化混合模型(parametrized mixture models)分析双黑洞(BBH)质量谱中的质量比、自旋对齐、自旋进动和红移演化特征。

Result: 通过参数化混合模型分析发现，LIGO-Virgo-KAGRA探测到的双黑洞(BBH)样本包含三个可能由孤立双星演化、球状星团动力学形成及多代并合主导的天体物理子群，其相对比例分别为$79.0^{+11.5}_{-10.9}\%$、$14.5^{+11.6}_{-8.0}\%$和$2.5^{+5.5}_{-1.8}\%$，且这些比例随时间演化具有超过$1σ$置信度。
...
### [50] [Long-term outburst activity of comet 17P/Holmes and constraints on ejecta size distributions](https://arxiv.org/abs/2603.17130)
...
**TL;DR**: 通过分析17P/Holmes彗星1892至2021年爆发（特别是2007年特大爆发）的亮度变化，首次定量约束了多孔团聚体（porous agglomerates）喷射物的粒径分布（幂律指数q=2-4对应1.15微米至5毫米）与总质量，为尘埃尾演化模型和流星群起源研究提供了关键物理参数。


<details>
  <summary>Details</summary>

Background: 研究聚焦于彗星爆发（cometary outbursts）过程中喷射粒子的尺寸分布及其对尘埃尾迹（dust-trail）形成和流星雨产生的动力学影响。

Data: 数据来源于对17P/Holmes彗星1892年至2021年爆发期间亮度振幅变化的分析，重点研究了2007年特大爆发事件。

Method: 通过分析彗星17P/Holmes从1892年至2021年爆发期间的亮度振幅变化，特别是2007年特大爆发事件，采用幂律分布模型（power law）约束了多孔团聚体（porous agglomerates）的粒径分布和总质量。

Result: result：通过分析17P/Holmes彗星1892至2021年爆发期间的亮度振幅变化，发现喷发抛射物（ejecta）的粒径分布符合幂律指数q，有效粒径介于1.15×10^-6米（q=4）至5×10^-3米（q=2）之间，且抛射颗粒总数随q值和升华通量（sublimation flux）增加而上升。
...
### [51] [Comprehensive VLBI observations of Galileo satellites with the AuScope array](https://arxiv.org/abs/2603.16955)
*David Schunck,Lucia McCallum,Jamie McCallum,Tiege McCarthy*

**TL;DR**: 首次利用澳大利亚AuScope甚长基线干涉测量(VLBI)阵列观测伽利略(Galileo)导航卫星，实现了VLBI与全球导航卫星系统(GNSS)框架间的技术关联，为未来空间大地测量共址任务奠定了基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用甚长基线干涉测量（VLBI）技术观测地球轨道卫星以实现空间大地测量共址（geodetic co-location）与空间基准联结（space ties），尤其关注欧洲航天局（ESA）Genesis任务推动下的技术整合需求。

Data: 数据来源于澳大利亚AuScope VLBI阵列（包括霍巴特、凯瑟琳和亚拉加迪的12米天线，配备VGOS仪器）对伽利略导航卫星E1和E6信号的测试实验及四次24小时全规模观测。

Method: 采用甚长基线干涉测量(VLBI)技术对伽利略导航卫星进行观测，包括相关处理、条纹拟合、精度评估和卫星跟踪方法，并首次实现了VLBI与全球导航卫星系统(GNSS)框架间的技术关联。

Result: 首次通过甚长基线干涉测量(VLBI)观测导航卫星实现了VLBI与全球导航卫星系统(GNSS)参考架之间的技术间连接(inter-technique ties)。
