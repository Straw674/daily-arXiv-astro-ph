<div id=toc></div>

# Table of Contents

- [astro-ph.GA](#astro-ph.GA) [Total: 13]
- [astro-ph.CO](#astro-ph.CO) [Total: 5]
- [astro-ph.IM](#astro-ph.IM) [Total: 7]
- [hep-ph](#hep-ph) [Total: 4]
- [astro-ph.HE](#astro-ph.HE) [Total: 7]
- [gr-qc](#gr-qc) [Total: 2]
- [physics.plasm-ph](#physics.plasm-ph) [Total: 1]
- [astro-ph.SR](#astro-ph.SR) [Total: 2]
- [physics.chem-ph](#physics.chem-ph) [Total: 1]
- [astro-ph.EP](#astro-ph.EP) [Total: 2]
- [cs.GR](#cs.GR) [Total: 1]


<div id='astro-ph.GA'></div>

# astro-ph.GA [[Back]](#toc)

### [1] [Forged by Feedback: Stellar Properties of Brightest Group Galaxies in Cosmological Simulations](https://arxiv.org/abs/2602.10192)
*Ruxin Barré,Arif Babul,Ghassem Gozaliasl,Alexis Finoguenov,Romeel Davé,Aviv Padawer-Blatt,Douglas Rennehan,Vida Saeedzadeh,Renier T. Hough,Thomas R. Quinn*

**TL;DR**: 通过对比四种宇宙学模拟（ROMULUS、SIMBA、SIMBA-C和OBSIDIAN）与COSMOS观测数据，发现OBSIDIAN采用的三模式活动星系核（AGN）反馈模型能最准确再现亮群星系（BGGs）的恒星质量、特定恒星形成率和质量加权年龄分布，证实了物理驱动的亚网格反馈机制对模拟大质量星系演化路径的关键作用。


<details>
  <summary>Details</summary>

Background: 研究探讨不同星系形成模型（ROMULUS、SIMBA、SIMBA-C和OBSIDIAN）对最亮群星系（BGGs）恒星特性的影响，重点关注活动星系核（AGN）反馈机制在调控大质量星系演化中的关键作用。

Data: 数据来源于四个宇宙学模拟（ROMULUS、SIMBA、SIMBA-C和OBSIDIAN）以及COSMOS天区X射线选星系群观测的最亮群星系（BGGs）的恒星质量、特定恒星形成率和质量加权恒星年龄。

Method: 采用四种宇宙学模拟（ROMULUS、SIMBA、SIMBA-C和OBSIDIAN）对比不同星系形成模型对最亮群星系（BGGs）恒星特性的影响，重点分析其活跃星系核（AGN）反馈模型的强度与机制差异。

Result: 模拟中最亮群星系（BGG）的恒星特性及演化路径显著受其活动星系核（AGN）反馈模型的强度和机制影响，其中OBSIDIAN采用的三机制AGN反馈模型与COSMOS观测数据在恒星属性分布、淬灭比例及大质量系统中恒星形成演化方面吻合度最高。

Abstract: We investigate how different galaxy formation models impact the stellar properties of brightest group galaxies (BGGs) in four cosmological simulations: ROMULUS, SIMBA, SIMBA-C, and OBSIDIAN. The stellar masses, specific star formation rates, and mass-weighted stellar ages of the simulated BGGs are analysed alongside those of observed BGGs from X-ray-selected galaxy groups in the COSMOS field. We find that the global properties and underlying evolutionary pathways of simulated BGG populations are strongly impacted by the strength and mechanism of their respective active galactic nucleus (AGN) feedback models, which play a critical role in regulating the growth of massive galaxies. OBSIDIAN's sophisticated three-regime AGN feedback model achieves the highest overall agreement with COSMOS observations, matching stellar property distributions, quenched fractions, and the evolution of star formation in increasingly massive systems. We find evidence suggesting that BGG populations of OBSIDIAN and COSMOS undergo a gradual decline in star formation with stellar mass, in contrast to SIMBA and SIMBA-C, which display rapid quenching linked to the onset of powerful AGN jet feedback. By comparison, ROMULUS produces highly star-forming, under-quenched BGGs due to the inefficiency of its thermal AGN feedback in preventing cooling flows from fuelling BGG growth. The success of the OBSIDIAN simulation demonstrates the importance of physically motivated subgrid prescriptions for realistically capturing the processes that shape BGGs and their dynamic group environments.

</details>

### [2] [The stellar-to-halo mass relation of central galaxies across three orders of halo mass](https://arxiv.org/abs/2602.10193)
*Victoria Toptun,Paola Popesso,Ilaria Marini,Stephan Vladutescu-Zopp,Klaus Dolag,Peter Behroozi,Lorenzo Lovisari,Stefano Ettori,Veronica Biffi,Xiaohu Yang,Natanael de Isídio,Daudi T. Mazengo*

**TL;DR**: 通过eROSITA eRASS1 X射线数据和SDSS测光数据测量了暗物质晕质量（halo mass）在$10^{12}$-$10^{15}\,M_\odot$范围内最亮中心星系（BCG）的恒星质量-晕质量关系（SHMR），发现该关系在$M_{\rm halo} \sim 10^{12}\,M_\odot$处达到峰值，高晕质量下恒星比例下降，反映了活动星系核反馈（AGN feedback）、冷却效率降低和外源组装（ex-situ assembly）主导性增强等过程对恒星质量增长的抑制作用，与流体动力学模拟在低质量端吻合但在星系团尺度存在显著差异。


<details>
  <summary>Details</summary>

Background: 星系恒星含量与宿主暗物质晕（dark matter halo）的质量和增长密切相关，理解这一关系对揭示星系形成和反馈过程的作用至关重要。

Data: 数据来源于eROSITA eRASS1的X射线观测数据（用于暗物质晕质量）和SDSS测光数据（用于最亮中心星系恒星质量），通过光学选群叠加X射线光谱获得平均晕气体温度并转换为晕质量。

Method: 采用X射线光谱叠加技术（stacking X-ray spectra）结合光学选群方法，通过eROSITA eRASS1 X射线数据推导暗物质晕质量（halo mass）并基于SDSS测光数据获取最亮中心星系（BCG）恒星质量，利用M-TX关系将平均晕气体温度转换为晕质量。

Result: 最亮中心星系（BCG）的恒星质量与暗物质晕质量关系（SHMR）在$M_{\rm halo} \sim 10^{12}\,M_\odot$附近达到峰值，随后高晕质量下恒星比例下降，反映了活动星系核（AGN）反馈、冷却效率降低以及外源（ex-situ）组装主导性增强等过程对恒星质量增长的抑制效应。

Abstract: The stellar content of galaxies is tightly connected to the mass and growth of their host dark matter halos. Observational constraints on this relation remain limited, particularly for low-mass groups, leaving uncertainties in how galaxies assemble their stars across halo mass scales. Accurately measuring the brightest central galaxy (BCG) stellar-to-halo mass relation (SHMR) over a wide mass range is therefore crucial for understanding galaxy formation and the role of feedback processes. Here we present the SHMR spanning $M_{\rm halo} \sim 10^{12}$-$10^{15}\,M_\odot$, using halo masses derived from eROSITA eRASS1 X-ray data and BCG stellar masses based on SDSS photometry. By stacking X-ray spectra of optically selected groups, we recover robust average halo gas temperatures for each bin, which are then converted to halo masses via the $M$-$T_X$ relation. We find that the SHMR peaks near $M_{\rm halo} \sim 10^{12}\,M_\odot$, with a declining stellar fraction at higher masses. This trend reflects a combination of processes that reduce the efficiency of stellar mass growth in massive halos, such as AGN feedback, reduced cooling efficiency, and the increasing dominance of ex-situ assembly, while halos continue to grow through mergers and accretion. Our measurements are consistent over the full mass range with previous observational studies, including weak lensing, X-ray analyses of individual clusters, and kinematical and dynamical methods. Comparisons with hydrodynamical simulations show good agreement at low masses but reveal significant discrepancies in the normalization at cluster scales, highlighting the sensitivity of BCG stellar growth to feedback prescriptions and halo assembly history. These results provide the first X-ray-based observational SHMR covering three orders of magnitude in halo mass, establish a robust benchmark for testing galaxy formation models.

</details>

### [3] [The Keck/DEIMOS Stellar Archive: I. Uniform Velocities and Metallicities for 78 Milky Way Dwarf Galaxies and Globular Clusters](https://arxiv.org/abs/2602.10200)
*Marla Geha,Debora Pelliccia,J. Xavier Prochaska,William Cerny,Frederick B. Davies,Joseph Hennawi,Brad Holden Dusty Reichwein,Kyle B. Westfall*

**TL;DR**: 基于凯克II望远镜和DEIMOS光谱仪对78个银河系矮星系和球状星团中22,339颗恒星进行均匀光谱观测，建立了包含视向速度、[Fe/H]金属丰度和成员星概率的星表，其视向速度测量精度达1.1 km/s、金属丰度精度达0.1 dex，相比已有研究精度提升20-50%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系矮星系和球状星团（Milky Way dwarf galaxy satellites and globular clusters）的恒星光谱观测与参数测定。

Data: 数据来源于凯克二号望远镜（Keck II telescope）和深空外河外成像多目标光谱仪（Deep Extragalactic Imaging Multiobject Spectrograph, DEIMOS）使用1200G光栅（光谱分辨率R~6000）获取的411个DEIMOS掩模均匀处理后的22,339颗恒星光谱数据。

Method: 采用凯克II望远镜和深空外成像多目标光谱仪(DEIMOS)获取光谱数据，使用1200G光栅(光谱分辨率R~6000)，通过PypeIt进行一维光谱还原，并开发dmost前向建模方法结合合成大气和恒星模板测定视向速度。

Result: 基于411个DEIMOS掩模的统一数据处理，构建了包含22,339颗恒星的径向速度、等效宽度[Fe/H]金属丰度及成员星概率的星表，数据精度达速度误差下限1.1 km/s、CaII三重线金属丰度误差下限0.1 dex。

Abstract: We present a homogeneous spectroscopic dataset of 22,339 stars in 78 Milky Way dwarf galaxy satellites and globular clusters. All data were taken with the Keck II telescope and Deep Extragalactic Imaging Multiobject Spectrograph (DEIMOS) spectrograph using the 1200G grating (spectral resolution R~6000). Based on a uniform data reduction of 411 DEIMOS masks, we present a catalog of individual stellar radial velocities, equivalent width-based [Fe/H] metallicities, and membership estimates. The Milky Way satellites range from M_V = 2 to -14 (M* = 10^1.5 to 10^7.5 Msun); the majority of individual stars presented in these systems have magnitudes 17 > r > 22. The data were reduced to 1D spectra using PypeIt, which provides near Poisson statistics-level sky subtraction. Radial velocities were determined via dmost, a forward modeling method first presented here, which combines both synthetic telluric and stellar templates to determine stellar radial velocities. We assess the accuracy and precision our method via comparison to thousands of repeat measurements and literature values. We determine a velocity error floor of 1.1 km/s and a CaII triplet-based metallicity error floor of 0.1 dex. We calculate internal velocity dispersions and compare to literature values, demonstrating 20-50% improved precision over the literature in most cases. In a companion paper, we use our homogeneous catalogs to explore properties of these Milky Way satellites, including previously unpublished measurements in several systems including Bootes II and Draco II. We provide full access to the data catalogs to enable further studies.

</details>

### [4] [The Keck/DEIMOS Stellar Archive: II. Dynamical Masses and Metallicities for a Uniform Sample of Milky Way Satellites](https://arxiv.org/abs/2602.10202)
*Marla Geha*

**TL;DR**: 通过对67个银河系卫星星系和球状星团的均匀光谱重分析，首次发现卫星星系与球状星团在相同恒星质量下存在动力学质量和质光比差异，并观测到恒星质量-金属丰度关系在log M*/Msun=4附近出现转折点，为暗物质研究和星系形成阈值提供了更精确的观测约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过均匀化光谱分析解决银河系卫星星系和球状星团（globular clusters）的动力学质量、金属丰度（metallicity）等参数的系统性差异问题，以更精确约束暗物质（dark matter）和星系形成阈值。

Data: 数据来源于对Keck/DEIMOS光谱仪观测的单个恒星进行均一化重新分析，构建了银河系卫星星系和球状星团的最大自洽光谱样本。

Method: 采用均匀重分析方法对Keck/DEIMOS光谱仪观测的恒星进行自洽样本分析，测定67个系统的动力学质量、平均[Fe/H]金属丰度及金属丰度弥散度。

Result: 在给定恒星质量下，卫星星系（satellite galaxies）与球状星团（globular clusters）在动力学质量和质光比上存在明显区分。

Abstract: Population-level studies of Milky Way satellites used to constrain dark matter or the threshold of galaxy formation often rely on velocity dispersions and metallicities derived from heterogeneous spectroscopic analyses. Systematic differences between data reduction pipelines and membership criteria can masquerade as astrophysical signals, or obscure real trends. Here, we present the largest self-consistent sample of spectroscopically-derived quantities for Milky Way satellite galaxies and globular clusters based on a homogeneous re-analysis of individual stars observed with the Keck/DEIMOS spectrograph. We determine enclosed dynamical masses, mean [Fe/H] metallicities, and metallicity dispersions for 67 systems with 10 or more member stars. At a given stellar mass, systems classified as satellite galaxies are well separated from globular clusters in their dynamical mass and mass-to-light ratios. The average enclosed mass densities of satellite galaxies agree with semi-analytic CDM model predictions. For satellite galaxies, we observe a break in the stellar mass-metallicity relation near log M*/Msun = 4 (M_V ~ -4.5). Above this stellar mass, satellite galaxies show the well-known tight trend (0.16 dex scatter in [Fe/H]) of decreasing metallicity with stellar mass; below log M*/Msun = 4, the mass-metallicity relation flattens and/or increases in scatter. Satellite galaxies have internal metallicity scatter between 0.3-0.4 dex across our stellar mass range. These uniform measurements will enable tighter constraints on the stellar mass-halo mass relation, improved J-factor estimates for dark matter searches, and lay a foundation for interpreting the flood of new Milky Way satellites expected in the LSST/Roman/Euclid era.

</details>

### [5] [The S-PLUS Fornax Project (S+FP): An extragalactic catalog covering $\sim$ 5 virial radii around NGC 1399 with galaxy properties](https://arxiv.org/abs/2602.10244)
*R. F. Haack,A. V. Smith Castelli,L. Sodré,C. Mendes de Oliveira,A. R. Lopes,L. A. Gutiérrez-Soto,R. Demarco,D. E. Olave-Rojas,E. R. Carrasco,P. K. Humire,J. P. Calderón,F. de Almeida Fernandes,L. Lomelí-Núñez,G. Sepúlveda,C. Lima-Dias,S. Torres Flores,E. Telles,N. M. Cardoso,D. Palma,L. Doubrawa,D. Pallero,M. Marinello,W. Schoenell,T. Ribeiro,A. Kanaan*

**TL;DR**: 利用南半球光度局部宇宙巡天(S-PLUS)的12个光学波段数据，结合深度学习算法构建了包含119,580个天炉座星系团(Foranx cluster)方向星系的跨波段(紫外至中红外)测光星表，实现了0.0219的光度红移精度，并提供了恒星质量、恒星形成率(SFRs)和D4000_N指数等物理参数估计。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用南半球光度局域宇宙巡天(S-PLUS)的宽视场多波段观测数据，构建天炉座星系团(Fornax cluster)方向的星系样本目录，以支持宇宙大尺度结构研究。

Data: 数据来源于南半球光度局域巡天（S-PLUS）的12个光学波段宽视场图像，并补充了GALEX（紫外）、VHS-VISTA（近红外）和AllWISE（中红外）数据，结合为天炉座星系团方向构建的光度测量星表。

Method: 采用监督式深度学习算法（supervised deep learning algorithms）结合降维技术（dimensionality reduction techniques）对S-PLUS天炉座项目（S+FP）的光度测量星表进行天体分类，并利用SExtractor优化配置进行星系检测与特征提取。

Result: 利用S-PLUS多波段巡天数据结合机器学习方法构建了包含119,580个星系的光度学星表，覆盖天炉座星系团（Fornax）周围约5个维里半径（virial radius）区域，并整合了紫外（GALEX）、近红外（VHS-VISTA）和中红外（AllWISE）数据，测得光度红移精度σ_NMAD∼0.0219且下限z_lim∼0.03，同时通过匹配SDSS光谱数据获得了恒星质量、恒星形成率（SFR）和D4000_N指数估计值。

Abstract: Observational extragalactic catalogs over wide sky areas are essential for uncovering the large-scale structure of the Universe. They allow, among others, cosmological studies and density analyses that impose strong constraints on models of galaxy formation and evolution. By taking advantage of the wide field images and the 12 optical bands of the Southern Photometric Local Universe Survey (S-PLUS), we aim at providing a catalog of galaxies located, in projection, towards the Fornax galaxy cluster, within $\sim$ 5 virial radii in right ascension (R.A.) and $\sim$ 3 virial radius in declination (Dec) around NGC,1399, the dominant galaxy of the cluster. Such a catalog will allow unprecedented large-scale structure studies in that sky region. Supervised deep learning algorithms have been developed, utilizing neural networks complemented with dimensionality reduction techniques, to classify and separate spurious objects, stars and galaxies in a photometric catalog previously built for the S-PLUS Fornax Project (S+FP). That catalog was built using a combination of SExtractor configurations optimized for galaxy detection and characterization. A catalog of 119,580 galaxies was obtained in the direction of the Fornax cluster containing photometric information in the 12 optical bands of S-PLUS complemented with GALEX (UV), VHS-VISTA (NIR) and AllWISE (MIR) data. We estimate photometric redshifts (σ_ NMAD $\sim$ 0.0219) with a lower limit of z_ lim $\sim$ 0.03. Stellar masses, star formation rates (SFRs) and D4000_N index estimates were obtained through a machine learning approach, by matching S-PLUS photometric data to SDSS spectroscopic data. The completeness of the catalog (72%) was calculated by comparing with mock catalogs ...

</details>

### [6] [Carbon-Enhanced Metal-Poor Star Candidates in the Milky Way from J-PLUS and S-PLUS](https://arxiv.org/abs/2602.10325)
*Jihye Hong,Timothy C. Beers,Yang Huang,Jonathan Cabrera Garcia,Young Sun Lee,Vinicius M. Placco,Evan N. Kirby*

**TL;DR**: 基于J-PLUS和S-PLUS巡天数据，在6200平方度天区中识别出约10.49万颗碳增强贫金属星（CEMP），证实其碳绝对丰度A(C)可划分为CEMP-no和CEMP-s两类，并首次在光度数据中验证了Yoon-Beers图中Group IV的统计显著性，揭示了CEMP-no在极贫金属区的主导性。


<details>
  <summary>Details</summary>

Background: 研究背景为利用多波段测光巡天数据（如J-PLUS和S-PLUS）对银河系碳增丰贫金属星（CEMP）进行大规模普查，以揭示其化学演化特征及与星系形成的关系。

Data: 数据来源于Javalambre Photometric Local Universe Survey (J-PLUS) DR3和Southern Photometric Local Universe Survey (S-PLUS) DR4的光度测量数据，覆盖约6,200平方度的天区。

Method: 采用J-PLUS DR3和S-PLUS DR4的多波段测光数据，通过[Fe/H]和[C/Fe]估计结合数据质量筛选和碳丰度演化校正，构建了包含约640万颗恒星的样本并识别出约10.49万颗CEMP候选体。

Result: 利用J-PLUS和S-PLUS巡天数据识别出约10.49万颗碳增丰贫金属星（CEMP），证实绝对碳丰度A(C)可将CEMP星分为CEMP-no和CEMP-s两类，并首次在测光数据中验证了Yoon-Beers图中Group IV的统计独立性。

Abstract: Recent large-scale multi-band photometric surveys now enable elemental-abundance estimates for millions of stars with accuracies approaching those of low- to medium-resolution spectroscopy. Using [Fe/H] and [C/Fe] estimates derived from the Javalambre Photometric Local Universe Survey (J-PLUS) DR3 and the Southern Photometric Local Universe Survey (S-PLUS) DR4, which together cover $\sim$6,200 deg$^2$ of the sky, we identify large numbers of carbon-enhanced metal-poor (CEMP) stars in the Milky Way. After applying data-quality cuts and evolutionary corrections to the carbon-abundance estimates, we construct a combined J/S-PLUS sample of $\sim$6.40 million stars and identify $\sim$104,900 CEMP candidates, roughly twice the number of CEMP candidates identified from Gaia XP spectra by Lucey et al. We photometrically confirm that the absolute carbon abundance $A$(C) separates CEMP stars into two primary groups, CEMP-no and CEMP-$s$ stars, consistent with previous spectroscopic studies. We also recover CEMP morphological Groups I-III in the Yoon-Beers diagram, as well as the recently proposed Group IV, and show that it is statistically distinct even in photometric data. A cumulative frequency analysis confirms that the CEMP fraction increases toward lower metallicity and that CEMP-no stars dominate in the most metal-poor regime. By comparing frequencies with and without Group IV stars, we assess their relation to CEMP-no and CEMP-$s$ stars, and examine CEMP distributions across different Galactic components. The resulting catalog provides a substantial sample for future spectroscopic follow-up, in particular to constrain the likely origin(s) of the Group IV stars.

</details>

### [7] [MAGNUS III: Mild evolution of the total density slope in massive early-type galaxies since z$\sim$1 from dynamical modeling of MUSE integral-field stellar kinematics](https://arxiv.org/abs/2602.10347)
*Pritom Mozumdar,Michele Cappellari,Christopher D. Fassnacht,Tommaso Treu*

**TL;DR**: 研究发现大质量早型星系（ETGs）在65亿年间（0 < z < 0.75）总质量密度斜率（γ_T）存在显著演化，中间红移样本（<z>=0.44）的中值斜率（2.19±0.01）比本地样本（<z>=0.04，2.26±0.01）更平缓，表明耗散过程在ETGs后期组装中起重要作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量早型星系（ETGs）在过去65亿年（红移范围0 < z < 0.75）内总质量密度斜率（total mass density slope）的演化规律。

Data: 研究中使用的数据来源包括高信噪比（S/N）MUSE-DEEP光谱仪获取的空间分辨恒星运动学数据、高分辨率HST成像的表面亮度模型，以及来自MaNGA巡天的本地（z~0.05）早型星系（ETGs）样本。

Method: 采用各向异性琼斯模型(JAM)结合多高斯展开(MGE)方法约束质量分布，并保持与本地样本分析方法严格一致。

Result: 在红移$<z>=0.44$时，中等红移早型星系(ETGs)的总密度轮廓近似等温，质量加权总密度斜率中位数为$<γ_{\rm T}>=2.19 \pm 0.01$，比本地基准值$<γ_{\rm T}>=2.26 \pm 0.01$更平缓，对应斜率红移梯度为$\mathrm{d} γ_{\rm T}/\mathrm{d} z \approx -0.20 \pm 0.03$。

Abstract: We investigate the total mass density slope evolution in massive early-type galaxies (ETGs) over the last 6.5 billion years ($0 < z < 0.75$). We perform a detailed dynamical analysis of approximately 200 ETGs spanning the redshift range $0.24 < z < 0.75$, utilizing spatially resolved stellar kinematics derived from high signal-to-noise ratio (S/N) MUSE-DEEP spectroscopy and surface brightness models from high-resolution HST imaging. We constrain mass distributions using the Jeans Anisotropic Modeling (JAM) technique coupled with Multi-Gaussian Expansion (MGE) method. To rigorously constrain evolutionary trends, we combine this intermediate-redshift dataset with a local ETG sample ($z \sim 0.05$) from the MaNGA survey. We adopt dynamical constraints for the local sample derived using an identical homogeneous methodology, ensuring a strictly consistent comparison. We found that the total density profiles of the intermediate-redshift ETG sample are approximately isothermal and exhibit a median mass-weighted total density slope, $<γ_{\rm T}>=2.19 \pm 0.01$ at $<z>=0.44$, which is shallower than the local baseline of $<γ_{\rm T}> = 2.26 \pm 0.01$ at $<z>=0.04$. This structural shift corresponds to a redshift gradient of $\mathrm{d} γ_{\rm T}/\mathrm{d} z \approx -0.20 \pm 0.03$, detected at $\sim$5-$σ$ significance. We demonstrate that this trend is robust against model assumptions and persists even when restricting the analysis to high-velocity dispersion systems ($σ_e > 150$ km/s). Our findings are consistent with previous lensing-based studies and in tension with cosmological simulations. The observed steepening suggests that dissipative processes, such as gas-rich accretion and mergers, must play a non-negligible role in the late-stage assembly of massive ETGs.

</details>

### [8] [Cosmic-ray electron propagation in NGC 3044 from radio continuum observations](https://arxiv.org/abs/2602.10574)
*Shengtao Wang,Xiaohui Sun,George Heald,Jiang-Tao Li,Chao-Wei Tsai,Judith Irwin,Theresa Wiegert,Jun Xu*

**TL;DR**: 这篇论文通过高灵敏度ASKAP射电观测发现NGC 3044星系中存在由宇宙射线电子(CREs)驱动的幂律加速外流风（速度超过逃逸速度400 km/s），并识别出一个半径约3千秒差距的超气泡结构，证实射电连续谱是研究正常恒星形成旋涡星系中风活动的有效手段。


<details>
  <summary>Details</summary>

Background: 研究主题为利用边缘朝向的恒星形成星系（star-forming edge-on galaxies）的多波段辐射特征，探究宇宙射线电子（cosmic-ray electrons, CREs）传输及其驱动的星系风（galactic wind）对周围介质的动力学影响。

Data: 数据来源于澳大利亚平方公里阵列探路者（ASKAP）观测获得的NGC 3044在943 MHz的总强度图像，分辨率为16角秒，均方根噪声为20 μJy beam⁻¹。

Method: 采用一维平流(advection)和扩散(diffusion)模型联合拟合总强度和谱指数剖面。

Result: ASKAP观测发现NGC 3044的宇宙射线电子（CREs）以幂律形式随高度加速外流，在$\sim3$ kpc处速度超过逃逸速度$\sim400$ km s$^{-1}$，表明存在强风驱动。

Abstract: Star-forming edge-on galaxies often exhibit extended halo radiation in multiple bands, providing ideal laboratories for studying the transfer of matter from the disk to the halo. We investigate the transport of cosmic-ray electrons (CREs) and the associated galactic wind, and assess their impact on the surrounding medium in NGC 3044. We obtained the NGC 3044 total intensity image at 943 MHz from the Australian SKA Pathfinder (ASKAP) observations with a resolution of 16 arcsec and an rms noise of 20 $μ$Jy beam$^{-1}$. The sensitivity is higher than the previous observations at similar frequencies. We find that the ASKAP intensity profiles perpendicular to the disk can be fit with two exponential components. The scale heights of the thin and thick disks are $0.43 \pm 0.13$ kpc and $1.91 \pm 0.26$ kpc, respectively. By jointly fitting total intensity and spectral index profiles with one-dimensional advection and diffusion models, we find that CREs are advected outward from the disk with the velocity increasing with height in a power law. Beyond $\sim3$ kpc, the velocity exceeds the escape speed of $\sim400$ km s$^{-1}$, indicating a strong wind. We further identify a possible superbubble of radius $\sim3$ kpc filled with soft X-ray emitting hot gas and surrounded by an HI shell and a bright H$α$ rim. These results demonstrate that radio continuum observations provide a powerful probe of cosmic-ray-driven winds in normal star-forming spiral galaxies.

</details>

### [9] [JWST spectra are consistent with the edge-on star-forming galaxy scenario for the "runaway supermassive black hole"](https://arxiv.org/abs/2602.10682)
*Jorge Sanchez Almeida,Ignacio Trujillo,Sebastian F. Sanchez,Mireia Montes*

**TL;DR**: 研究发现线性结构（linear structure）前端光谱符合低金属度低消光HII区特征，支持其为无核球侧向恒星形成星系而非超大质量黑洞（SMBH）产生的星体尾迹。


<details>
  <summary>Details</summary>

Background: 该研究探讨了van Dokkum等人(2023)报道的线性结构是超大质量黑洞(SMBH)逃逸产生的恒星尾迹还是无核球侧向星系的科学争议。

Data: 数据来源为JWST/NIRSpec IFU（积分视场单元）对线性结构尖端的新观测数据。

Method: 采用标准BPT图([OIII]5007/Hb vs [NII]6583/Ha和[OIII]5007/Hb vs [OII]6716,6731/Ha)分析线比以区分低金属度低消光HII区与星系合并激波。

Result: result：观测到的线性结构尖端发射线比率符合低金属度低消光HII区特征，不支持超质量黑洞产生的弓激波假说，支持其为具有HII区的恒星形成星系。

Abstract: The linear structure reported by van Dokkum et al. (2023) has been proposed as either a massive stellar wake produced by a runaway supermassive black hole (SMBH) or a bulgeless edge-on galaxy. New JWST/NIRSpec IFU observations target the tip of the structure, where a SMBH would produce a bow shock, whereas a normal galaxy would host an HII region. Using standard BPT diagrams ([OIII]5007/Hb vs [NII]6583/Ha and [OIII]5007/Hb vs [OII]6716,6731/Ha), we find that the line ratios at the tip fall on the locus of low-metallicity low-extinction HII regions. This region does not overlap with loci typical of shocks in merging galaxies. Thus, these results are consistent with the interpretation that the linear structure is a star-forming galaxy, with the bright knot representing one of its HII regions.

</details>

### [10] [The metal-poor tail of the APOGEE survey I. Uncovering [Fe/H] < -2.5 stars from the inner Galaxy to the Magellanic Clouds](https://arxiv.org/abs/2602.10859)
*M. Montelius,E. Starkenburg,H. Woudenberg,A. Angrilli Muglia,A. Ardern-Arentsen,A. Viswanathan,A. Byström,A. Helmi,N. Martin,T. Matsuno,C. Navarrete,J. Navarro*

**TL;DR**: 通过分析APOGEE巡天数据发现289颗极贫金属红巨星（[Fe/H]<-2.5），包括16颗麦哲伦云候选成员、14颗轨道限于银河系内区的极贫金属星和13颗内区晕族闯入者，并验证了未校准金属丰度恒星多为极贫金属星的特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用大规模光谱巡天数据（如APOGEE）识别和分析极贫金属星（metal-poor stars），以探索其轨道分布特征并筛选后续光谱观测目标。

Data: 数据来源包括APOGEE巡天（APOGEE survey）的校准恒星参数、盖亚卫星第三次数据释放（Gaia DR3）的位置和天体测量数据，以及本研究计算的测光距离。

Method: 采用APOGEE巡天数据的光谱分析管道（pipeline）结合盖亚(Gaia)DR3天体测量数据计算轨道参数，并通过与文献金属丰度数据的对比验证极贫金属星（very metal-poor stars）的筛选标准。

Result: 校准后的APOGEE恒星参数通过未给出金属丰度值但有效温度和表面重力被确定的特征，明确识别了巡天中最贫金属的恒星。

Abstract: In the search for metal-poor stars, large spectroscopic surveys are an invaluable tool. However, the spectra of metal-poor stars can be difficult to analyse because of the relative lack of available lines, which can also lead to misclassification. We aim to identify the stars observed by the APOGEE survey that are below the metallicity limit of APOGEE's analysis. For the highest confidence candidates, we classify the orbital properties of the stars to investigate whether their orbital distribution matches what we would expect for stars that are this metal poor and to select especially interesting targets for spectroscopic follow-up purposes. We examined the properties derived by APOGEE for metal-poor stars from the literature to find signatures of stars with a metallicity below the range of the grid used for spectral analysis. The calibrated APOGEE stellar parameters provide a clear signature of the most metal-poor stars in the survey, indicated by null values for their metallicities while having effective temperatures and surface gravities determined by the pipeline. From comparison with the literature, we find that, within a temperature range of 3700 - 6700 K and above a threshold of S/N > 30, the vast majority of APOGEE stars without calibrated metallicities are very metal poor. The radial velocities provided by APOGEE, Gaia DR3 positions and astrometry along with spectrophotometric distances derived in this work allowed for the computation of their orbits. In this work, we select 289 very metal-poor red giant stars (likely below = -2.5) from the APOGEE results. Our sample contains 16 very metal-poor member candidates of the Magellanic Clouds, 14 very metal-poor stars with orbits confined to the inner Galaxy, and 13 inner Galaxy halo interlopers.

</details>

### [11] [The metal-poor tail of the APOGEE survey II. Spectral analysis of Mg and Si in very metal-poor APOGEE spectra](https://arxiv.org/abs/2602.10883)
*M. Montelius,A. Angrilli Muglia,E. Starkenburg,C. Kobayashi,A. Ardern-Arentsen,F. Gran,P. Jablonka,N. Martin,J. Navarro,F. Sestito,K. A. Venn,S. Vitali*

**TL;DR**: 基于APOGEE数据库的327颗极贫金属星（[Fe/H] < -2）光谱，利用镁（Mg）和硅（Si）元素丰度测量其总体金属丰度（[M/H]），将APOGEE结果的贫金属端延伸了0.6 dex至-3.1 ≤ [M/H] ≤ -2.25，并发现5颗[Si/Mg]异常高的恒星可能与球状星团恒星或特定超新星事件有关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用H波段光谱中的镁（Mg）和硅（Si）元素吸收线测定极贫金属恒星（[Fe/H] < -2.5）的金属丰度，以克服该波长范围内铁（Fe）线微弱导致的传统分析困难。

Data: 数据来源于APOGEE（Apache Point Observatory Galactic Evolution Experiment）数据库，包含327颗极贫金属恒星（[Fe/H] < -2）的H波段光谱，其中289颗位于红巨星分支。

Method: 采用专门设计的流程（specialised pipeline）测量APOGEE光谱中镁（Mg）和硅（Si）元素丰度，并通过测试三种不同的Mg和Si增强假设集来确保可靠性。

Result: result：通过测量327颗极贫金属恒星（[Fe/H] < -2）的Mg和Si元素丰度，发现其中5颗具有异常高[Si/Mg]（>0.5）的恒星可能与球状星团恒星或特定超新星事件相关，并将APOGEE结果的贫金属端向下扩展了0.6 dex至[M/H] = -3.1。

Abstract: H-band spectra contain very limited spectral information for stars at the most metal-poor tail ( Fe/H < -2.5) because the available Fe lines in FGK stars in this wavelength range are weak. The first paper in this series successfully identified a sample of 327 very metal-poor stars (with [Fe/H] < -2) from the APOGEE database, 289 of which are on the red giant branch. The spectra of these stars were not properly analysed by the APOGEE main pipeline because they are very metal poor. In this work, we measure metallicities for these stars using the abundances of the elements Mg and Si. We demonstrate that the absorption lines of the elements Mg and Si are of good quality despite the challenging combination of (low) metallicity, wavelength regime, spectral resolution, and signal-to-noise ratios available for these spectra. A specialised pipeline was designed to measure the abundance of Mg and Si in APOGEE spectra and yielded a robust estimate of the overall metallicity. In order to provide reliable measurements, we tested three different sets of assumptions for Mg and Si enhancement. We present Mg and Si abundances as well as overall metallicities for 327 stars, all of which had previously gotten null values from the main APOGEE pipeline for either the calibrated M/H or [Fe/H] . The typical uncertainties for our measurements are 0.2 dex. We found five stars in our sample with unusual [Si/Mg] abundances higher than 0.5, and we connect this signature to globular cluster stars, and this might be related to specific supernova events. Our data suggest a concentration of high [Si/Mg] stars in the Sextans dwarf galaxy. Other dwarf galaxies are found to agree well with results in the literature. Our derived metallicities range between -3.1 $\leq$ [M/H] $\leq$ -2.25, thereby pushing the metal-poor tail of APOGEE results down by 0.6 dex.

</details>

### [12] [Deep and Sparse Denoising Benchmarks for Spectral Data Cubes of High-z Galaxies: From Simulations to ALMA observations](https://arxiv.org/abs/2602.10893)
*Arnab Lahiry,Tanio Díaz-Santos,Jean-Luc Starck,Niranjan Chandra Roy,Daniel Anglés-Alcázar,Grigorios Tsagkatakis,Panagiotis Tsakalides*

**TL;DR**: 论文评估了多种降噪方法在ALMA观测数据中的应用，发现迭代软阈值法(IST)在中信噪比(SNR)数据中表现稳健，而基于合成数据训练的3D U-Net在真实数据中降噪效果优异但泛化能力受限于训练集物理特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用先进降噪技术提升高红移（high-redshift）星系中微弱星际介质（interstellar medium）信号的探测能力，以揭示宇宙正午（cosmic noon）后大质量星系（massive galaxy）的组装机制。

Data: 研究中使用的数据来源包括：(i) 旋转玩具盘状星系（toy disk galaxies）的合成光谱立方体、(ii) 来自FIRE模拟的合成[C II]积分视场单元（IFU）立方体、(iii) ALMA对CRISTAL星系和W2246-0526的[C II]观测数据。

Method: 采用主成分分析（PCA）、独立成分分析（ICA）、基于二维-一维小波的迭代软阈值（IST）稀疏无监督表示以及三维U-Net监督深度学习进行降噪性能评估与比较。

Result: 在ALMA-CRISTAL中等信噪比观测中，3D U-Net和迭代软阈值(IST)去噪性能相当，能保持91%以上通量并将信噪比提升6倍以上。

Abstract: Beyond cosmic noon, galaxies appear as faint whispers amid noise, yet this epoch is key to understanding massive galaxy assembly. ALMA's sensitivity to cold dust and [C II] emission allows us to probe their interstellar medium, but faint signals make robust denoising essential. We evaluate and benchmark denoising strategies including Principal Component Analysis, Independent Component Analysis, sparse unsupervised representations: iterative soft thresholding with 2D-1D wavelets, and supervised deep learning with a 3D U-Net, to identify techniques that suppress noise while preserving flux and morphology across peak SNRs of 2.5-8, applied to (i) synthetic spectral cubes of rotating toy disk galaxies, (ii) synthetic [C II] IFU cubes from FIRE simulations, and (iii) ALMA [C II] observations of CRISTAL galaxies and W2246-0526. Performance is assessed via RMSE, conservation of flux and spectra, noise reduction, and SNR improvement of the central galaxy. For synthetic cubes: PCA and ICA provide marginal improvement; IST reduces noise effectively at moderate SNRs but can suppress emission at low SNRs; and the U-Net outperforms IST, though it can produce quantifiable hallucinations at lower-SNRs. For moderate-SNR observations (ALMA-CRISTAL), U-Net and IST achieve comparable performance, conserving >91% flux and increasing SNR by >6. However, for observations with complex morphologies absent in the training set (W2246), the U-Net underperforms relative to IST, recovering ~80% flux, while IST robustly conserves flux and improves SNR by ~3, highlighting generalisation challenges and the need for physically-motivated training priors. We conclude that IST is a robust unsupervised denoiser for moderate-SNR data, and a synthetically trained U-Net generalises effectively to real data, dependent on training priors. This framework offers a pathway for transferable denoising for ALMA, VLT/MUSE, and JWST.

</details>

### [13] [Most Strong Lensing Deflectors in the AGEL Survey are in Group and Cluster Environments](https://arxiv.org/abs/2602.11068)
*William J. Gottemoller,Nandini Sahu,Rodrigo Cordova-Rosado,Leena Iwamoto,Courtney B. Watson,Kim-Vy H. Tran,A. Makai Baker,Tania M. Barone,Duncan J. Bowden,Karl Glazebrook,Anishya Harshan,Tucker Jones,Glenn G. Kacprzak,Camryn M. Neches*

**TL;DR**: 通过测量89个AGEL（ASTRO3D Galaxy Evolution With Lenses）强透镜系统的偏折器尺度（Einstein质量）和环境（晕质量），发现47.2%的偏折器位于星系团环境中但仅9.0%具有星系团尺度的爱因斯坦半径（质量），揭示了透镜模型中单一星系尺度偏折器的假设过于简化，并建立了偏折器环境与爱因斯坦质量间的弱相关性（r=0.38）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于强引力透镜系统中偏折体（deflector）环境对测试宇宙学模型和约束星系演化特性的影响。

Data: 数据来源于ASTRO3D Galaxy Evolution With Lenses (AGEL)巡天中89个光谱确认的强引力透镜系统，包括测量爱因斯坦半径($θ_{\rm{E}}$)确定的透镜质量($M(<θ_{\rm{E}})$)和利用测光红移($z_{\rm{phot}}$)计算的第五近邻星系面密度($Σ_5(z)$)来量化透镜环境。

Method: 通过测量爱因斯坦角半径（$θ_{\rm{E}}$）确定被爱因斯坦半径包围的质量（$M(<θ_{\rm{E}})$）来分类偏折器尺度，并利用测光红移（photometric redshifts）计算第五近邻星系面密度（$Σ_5(z)$）来量化偏折器环境。

Result: 47.2%的强透镜系统偏转体（deflector）位于星系团环境，但仅9.0%具有星系团尺度的爱因斯坦半径（质量）。

Abstract: The environments of deflectors in strong lensing systems affect our ability to test cosmological models and constrain evolutionary properties of galaxies. Here we measure the deflector scale (Einstein mass) and deflector environment (halo mass) of 89 spectroscopically confirmed strong lenses in the ASTRO3D Galaxy Evolution With Lenses (AGEL) survey. We classify deflector scale by measuring $θ_{\rm{E}}$ to determine the mass enclosed by the Einstein radius, $M(<θ_{\rm{E}})$. We quantify deflector environment by using photometric redshifts to determine the galaxy surface density to the fifth-nearest neighbor $Σ_5(z)$. We find that 47.2% of our deflectors are embedded in cluster environments, whereas only 9.0% have cluster-scale Einstein radii (masses). We measure a weak correlation ($r = 0.38$) between Einstein mass and $Σ_5(z)$, suggesting that the assumption of single galaxy-scale deflectors in lens modeling is overly-simplified. We hypothesize that the weak correlation results from galaxy-scale bias in the original AGEL selection and the observational challenge of detecting faint arcs with large Einstein radii. Comparing number densities, $N_{\rm{gal}}$, between AGEL and control fields, we find that AGEL deflectors are in systematically denser environments. Our study provides a method to identify strong lenses as a function of deflector environment and approximate the impact of large-scale environment in lens modeling. We provide the measured lensing parameters for our 89 AGEL systems as well as $z_{\rm{phot}}$ and $r$-mag (AB) maps of the line-of-sight.

</details>

<div id='astro-ph.CO'></div>

# astro-ph.CO [[Back]](#toc)

### [14] [Statistical isotropy of the universe and the look-elsewhere effect](https://arxiv.org/abs/2602.10178)
*Alan H. Guth,Mohammad Hossein Namjoo*

**TL;DR**: 通过分析宇宙微波背景(CMB)数据的四项异常测试，指出Jones等人声称宇宙统计各向异性(statistical anisotropy)的5σ显著性结论存在方法论缺陷，实际显著性仅2-3σ，且数据仍符合ΛCDM模型和统计各向同性(statistical isotropy)假设。


<details>
  <summary>Details</summary>

Background: 研究质疑宇宙统计各向异性(statistical anisotropy)的显著性证据，指出其分析存在方法学缺陷并与ΛCDM模型预测的统计各向同性(statistical isotropy)相矛盾。

Data: 研究使用了宇宙微波背景辐射（cosmic microwave background）的观测数据，并基于Jones等人分析的四个异常性检验（anomaly tests）进行联合统计评估。

Method: 采用联合分析（joint analysis）四个异常检验（anomaly tests）的$p$值并结合相关性校正来评估宇宙统计各向异性（statistical anisotropy）的显著性。

Result: Jones等人声称宇宙统计各向异性(statistical anisotropy)的显著性被高估，实际数据与ΛCDM模型及统计各向同性(statistical isotropy)一致。

Abstract: Recently, Jones et al. [arXiv:2310.12859] claimed strong evidence for the statistical anisotropy of the universe. The claim is based on a joint analysis of four different anomaly tests of the cosmic microwave background data, each of which is known to be anomalous, with a lower level of significance. They reported a combined $p$-value of about $3\times 10^{-8}$, which is more than a $5σ$ level of significance. We observe that statistical anisotropy is not even relevant for two of the four considered tests, which seems sufficient to invalidate the authors' claim. Furthermore, even if one reinterprets the claim as evidence against $Λ$CDM rather than statistical anisotropy, we argue that this result significantly suffers from the look-elsewhere effect. Assuming a set of independent (i.e., uncorrelated) tests, we show that if the four tests with the smallest $p$-values are cherry-picked from 10 independent tests, the $p$-value reported by Jones et al. corresponds to only $3σ$ significance. If there are 27 independent tests, the significance falls to $2σ$. These numbers, however, overstate our argument, since the four tests used by Jones et al. are slightly correlated. Determining the correlation of Jones et al.'s tests by comparing their joint $p$-value with the product of the four separate $p$-values, we find that about 16 or 50 tests are sufficient to reduce the significance of Jones et al.'s results to 3$σ$ or 2$σ$ significance, respectively. We also provide a list of anomaly tests discussed in the literature (and propose a few generalizations), suggesting that very plausibly 16 (or even 50) independent tests have been published, and possibly many more have been considered but not published. We conclude that the current data is consistent with the $Λ$CDM model and, in particular, with statistical isotropy.

</details>

### [15] [Massive stars as gravitationally lensed transients -- Insights on the high-mass initial mass function](https://arxiv.org/abs/2602.10909)
*Sung Kei Li*

**TL;DR**: 通过观测强引力透镜星系中瞬态透镜恒星（transient lensed stars）的探测率，提出了一种测量局部宇宙外恒星初始质量函数（IMF）的新方法，目前未发现红移$z \approx 1$星系存在顶部重（top-heavy）IMF的证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过引力透镜效应（gravitational lensing）和微透镜事件（microlensing）观测高红移（$z \approx 1$）星系中的瞬变透镜恒星（transient lensed stars），以突破恒星初始质量函数（IMF）与恒星形成历史（star formation history）在光谱能量分布拟合中的简并性（degeneracy），从而拓展IMF的测量范围至本地宇宙（local universe）之外。

Data: 研究中使用的观测数据来源于强引力透镜（strongly lensed）星系中暂时可探测的瞬态透镜恒星（transient lensed stars）。

Method: 利用瞬变透镜恒星（transient lensed stars）的观测检测率作为探针，通过引力微透镜（stellar microlensing）效应打破光谱能量分布拟合（spectral energy distribution fitting）中初始质量函数（IMF）与恒星形成历史的简并性。

Result: 观测到的瞬态透镜恒星（transient lensed stars）的检测率可作为探针，在利用光谱能量分布拟合的研究中打破初始质量函数（IMF）与恒星形成历史的简并性，从而为研究引力透镜星系子样本的IMF提供窗口。

Abstract: A robust stellar initial mass function (IMF) is crucial in any studies related to star formation. However, the direct measurement of the stellar IMF is confined to the local universe, limited by the resolving power of telescopes. Recently, a new method for accessing the stellar IMF beyond the local universe has been developed. The observed detection rate of transient lensed stars -- individual, massive, thus luminous stars in strongly lensed galaxies that are temporarily detectable upon stellar microlensing -- can serve as a probe to break the IMF-star formation history degeneracy in studies utilizing spectral energy distribution fitting, hence providing a window to look at the IMF at a subsample of gravitationally lensed galaxies. In this proceeding, I summarize the contributed talk given at IAUS402 entitled the same as this contribution and highlight some key results, which currently show no evidence for a top-heavy IMF in $z \approx 1$ galaxies.

</details>

### [16] [Beyond $Λ$CDM: fundamental constants as cosmological observables](https://arxiv.org/abs/2602.10987)
*Dinko Milaković,John K. Webb*

**TL;DR**: 提出通过精细结构常数（fine-structure constant, α）的大规模测量结合其他探测手段，为超越ΛCDM模型的物理机制提供前所未有的约束。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙学常数（Λ）冷暗物质（CDM）模型面临的张力问题，并提出了通过测量精细结构常数（α）的时空变化来检验非引力相互作用和区分竞争模型的新方法。

Data: 研究使用了类星体吸收系统和星系原子双线发射气体的光谱观测数据，通过多重组态方法（Many Multiplet method）测量精细结构常数（fine-structure constant, α）。

Method: 采用多线法（Many Multiplet method）和原子双线法测量精细结构常数（fine-structure constant, α），结合类星体吸收系统和星系发射线气体光谱数据。

Result: 通过测量精细结构常数（fine-structure constant, α）的时空变化，可为超越ΛCDM模型的物理理论提供前所未有的约束条件。

Abstract: Recent cosmological tensions pose difficulties for $Λ$CDM. Forthcoming facilities will be able to check whether these tensions result from systematic effects or indeed with the $Λ$CDM model itself. However, these new data will primarily probe gravitational interactions and provide only limited information about non-gravitational interactions. Distinguishing between competing models that make similar predictions yet rely on fundamentally different principles, therefore requires suitably diverse physical tests. Observational constraints on spacetime variations of fundamental constants fill this need. The fine-structure constant, $α= e^2/\hbar c$, can be measured using absorption systems towards bright quasars using the Many Multiplet method, and using atomic doublets from line emitting gas in galaxies. A spectroscopic facility such as the WST could produce more than 100,000 new measurements of $α$ from quasars together with a million measurements from galaxies. When combined with other probes, such a large and homogeneous dataset of $α$ measurements would provide unprecedented constraints on physics beyond $Λ$CDM.

</details>

### [17] [New constraints on cosmic anisotropy from galaxy clusters using an improved dipole fitting method](https://arxiv.org/abs/2602.11093)
*Jianping Hu,Chao Geng,Xuandong Jia,Zhaoyu Zuo,Taozhi Yang,Fayin Wang*

**TL;DR**: 通过分析313个星系团(Chandra+XMM-Newton)数据，发现宇宙膨胀存在两个优先方向(257.82°+58.01/-52.88, -31.30°+35.92/-39.46)和(80.89°+60.97/-52.46, 31.75°+35.19/-40.16)，分别对应较快和较慢膨胀速率，各向异性幅度为5.2~5.4×10^-4，统计显著性达1.0σ，且XMM-Newton数据和高红移(HR)样本显示出更强的各向异性信号。


<details>
  <summary>Details</summary>

Background: 探索宇宙大尺度各向异性（cosmic anisotropy）信号及其在星系团中的表现。

Data: 数据来源于313个星系团（Chandra + XMM-Newton）以及基于子样本（Chandra、XMM-Newton、低红移LR $z < 0.10$、高红移HR $z > 0.10$数据集）的重新分析。

Method: 采用偶极子拟合（dipole fitting, DF）方法分析313个星系团（钱德拉+XMM-牛顿数据）的空间分布，构建统计各向同性分析框架，并通过子样本（钱德拉、XMM-牛顿、低红移LR、高红移HR数据集）验证观测设备和红移对各向异性信号的影响。

Result: 基于313个星系团(钱德拉+XMM-牛顿)数据，发现两个宇宙膨胀速率各向异性方向：(l, b) = (257.82°+58.01−52.88, −31.30°+35.92−39.46)对应更快膨胀区域，(80.89°+60.97−52.46, 31.75°+35.19−40.16)对应更慢膨胀区域，各向异性幅度|A|为5.2∼5.4×10−4。

Abstract: In this work, we attempted to apply the dipole fitting (DF) method to galaxy clusters to search for cosmic anisotropic signals, and to construct a statistical isotropic analysis scheme for them. Compared to Type Ia supernova (SNe Ia), the galaxy clusters offer a significant advantage in terms of spatial distribution. This advantage makes the anisotropic signals obtained from them more reliable. From 313 galaxy clusters (Chandra + XMM-Newton), we find two preferred directions (l, b) = (${257.82^{\circ}}_{-52.88}^{+58.01}$, $-31.30{^{\circ}}_{-39.46}^{+35.92}$) and ($80.89{^{\circ}}_{-52.46}^{+60.97}$, $31.75{^{\circ}}_{-40.16}^{+35.19}$). The former to a direction where the universe is expanding at a faster rate than the surrounding area, while the latter to a slower rate of expansion. The corresponding magnitude of anisotropy is $|A|$ = 5.2 $\sim$ 5.4 $\times$ 10$^{-4}$. The results of statistical isotropy analyses give $\sim$1.0$σ$ confidence level. From the reanalyses based on the subsamples including Chandra, XMM-Newton, low reshift (LR, $z < 0.10$), high redshift (HR, $z > 0.10$) datasets, we find that the observation equipment and sample redshift can affect the preferred direction, anisotropic magnitude, and statistical significance of anisotropy. The XMM-Newton dataset gives a statistical significance of 2.26$σ$ (Mock) and 2.86$σ$ (Iso) which are much higher than that from Chandra and the total datasets. The magnitude of anisotropy $|A|$ from HR dataset is larger than that from LR dataset. Overall, our results indicate the presence of anisotropic signals in galaxy clusters, which must be taken seriously. Further test is still needed to better understand these signals.

</details>

### [18] [Beyond thermal approximations: Precise cosmological bounds on Axion-Like Particles](https://arxiv.org/abs/2602.11100)
*Nicola Barbieri,Luca Caloni,Martina Gerbino,Massimiliano Lattanzi,Luca Visinelli*

**TL;DR**: 通过求解动量依赖的玻尔兹曼方程获得轴子类粒子（ALPs）相空间分布并计算其对宇宙学观测量的影响，结合最新宇宙微波背景和原初核合成数据，给出了ALPs与轻子（电子、μ子、τ子）及光子耦合的严格上限（如电子耦合的衰变常数fa > 1.63×10^6 GeV），并预测了未来实验配置对提升约束精度的潜力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过宇宙学观测约束轻轴子类粒子（ALPs）与轻子或光子的耦合，探讨其在早期宇宙热等离子体中的产生机制及其对相对论性粒子有效数目（ΔN_eff）的影响。

Data: 数据来源包括普朗克卫星（Planck）、阿塔卡马宇宙学望远镜（Atacama Cosmology Telescope）、南极望远镜（South Pole Telescope）的宇宙微波背景（cosmic microwave background）测量数据，以及大爆炸核合成（Big Bang Nucleosynthesis）对原始氘和氦丰度的测定，并补充了暗能量光谱仪（Dark Energy Spectroscopic Instrument）的重子声学振荡（baryon acoustic oscillation）数据。

Method: 采用动量依赖的玻尔兹曼方程(Boltzmann equation)求解轴子类粒子(ALPs)的相空间分布，并将其一致性地传播到宇宙学可观测量计算中，以评估非热谱畸变对有效相对论性粒子数($ΔN_{\rm eff}$)的影响。

Result: 通过求解动量依赖的玻尔兹曼方程获得轴子类粒子（ALPs）的相空间分布，并一致地将其传播到宇宙学可观测量计算中，从而评估非热谱畸变对有效相对论物种数（ΔN_eff）的影响。

Abstract: We derive updated cosmological bounds on light axion-like particles (ALPs) coupled to leptons or photons, using a full phase-space treatment of their production from the primordial thermal plasma. The ALP phase-space distribution, obtained by solving the momentum-dependent Boltzmann equation for the relevant production processes, is consistently propagated into the computation of cosmological observables, allowing us to assess the impact of non-thermal spectral distortions on the effective number of relativistic species, $ΔN_{\rm eff}$. Using state-of-the-art measurements of the cosmic microwave background from Planck, the Atacama Cosmology Telescope, and the South Pole Telescope, complemented with Big Bang Nucleosynthesis determinations of primordial deuterium and helium abundances, we obtain the following 95\% credible limits on the ALP decay constant: $f_a > 1.63 \times 10^6 \, {\rm GeV}$, $9.41 \times 10^6 \, {\rm GeV}$ and $8.06 \times 10^4 \, {\rm GeV}$ for ALPs coupled to electrons, muons and taus, respectively. For the ALP-photon coupling we find $g_{aγ} < 1.98 \times 10^{-8} \, {\rm GeV}^{-1}$. Including baryon acoustic oscillation data from the Dark Energy Spectroscopic Instrument mildly relaxes the constraints, in line with previous analyses of extra relativistic degrees of freedom. Finally, we present forecasts for the LiteBIRD$+$Simons Observatory and LiteBIRD$+$CMB-HD configurations, discussing the importance of an exact phase-space treatment for robust cosmological bounds on ALP interactions.

</details>

<div id='astro-ph.IM'></div>

# astro-ph.IM [[Back]](#toc)

### [19] [The indiscriminate adoption of AI threatens the foundations of academia](https://arxiv.org/abs/2602.10165)
*Roberto Trotta*

**TL;DR**: 人工智能在科研中的应用需加以限制，以保障学术（academia）推动人类知识进步的核心目标。


<details>
  <summary>Details</summary>

Background: 人工智能在科学研究中的应用需谨慎以确保学术界的核心目标——为人类推进知识发展——得到保障。

Data: 研究中未明确提及具体数据来源。

Method: 采用人工智能（Artificial Intelligence）技术辅助科学研究，但强调需限制其应用以保障学术研究以人类知识进步为核心目标。

Result: 人工智能在科研中的应用需受到限制，以保障学术核心目标（即推动人类知识进步）。

Abstract: Artificial intelligence offers much promise, but its use in scientific research should be restrained so that the primary aim of academia -- advancing knowledge for humans -- is safeguarded.

</details>

### [20] [Observing solar vortices with existing and future instrumentation. Solar Physics International Network for Swirls (SPINS) white paper (Helio)](https://arxiv.org/abs/2602.10170)
*Suzana S. A. Silva,Viktor Fedun,Gary Verth,Istvan Ballai,Eamon Scullion,Malcolm Druett,Kostas Tziotziou,Alex Pietrow,Nitin Yadav,Ioannis Dakanalis,Elena Khomenko,Hidetaka Kuniyoshi,Shivdev Turkay,Matias Koll Pistarini,Robertus Erdelyi,Luiz Augusto Camargo Aranha Schiavo*

**TL;DR**: 该论文核心贡献是提出五个关于太阳涡旋（solar vortices）的高优先级科学问题，并规划结合多波段空间仪器和分阶段验证任务的技术路线，以维持英国在太阳涡旋动力学研究领域的领先地位。


<details>
  <summary>Details</summary>

Background: 太阳涡旋（solar vortices）是太阳大气动力学的核心组成部分，为磁场扭曲、能量聚集与传输、波导以及跨大气层等离子体耦合提供了天然实验室。

Data: 数据来源包括高分辨率观测（如DKIST、Solar Orbiter）、数值模拟（MHD湍流理论）以及参与MUSE和SOLAR-C EUVST任务获取的太阳大气层多波段光谱偏振数据。

Method: 采用下一代多波段空间认证系统，结合四个可调谐法布里-珀罗干涉仪(Fabry-Pérot Interferometers)实现从深光球层到低日冕的衍射极限高时间分辨率光谱偏振覆盖。

Result: 太阳涡旋（solar vortices）是太阳大气动力学的核心组成部分，与磁重联（magnetic reconnection）、大气加热、湍流及波产生等关键物理过程密切相关。

Abstract: Solar vortices are fundamental components of solar atmospheric dynamics, serving as natural laboratories for magnetic field twisting, energy concentration and transport, wave guidance, and plasma coupling across atmospheric layers. Numerical and observational studies show that solar vortices are intimately connected to key physical processes including magnetic reconnection, atmospheric heating, turbulence, and wave generation. This white paper, prepared for the UK Space Frontiers 2035 call, outline five high-priority scientific questions addressing vortex generation mechanisms, cross-layer coupling, magnetic restructuring, collective wave-guidance structures, and their role in triggering explosive events and modulating the solar wind. Key observations and capabilities required to make significant advancements over the coming decade are identified. The UK solar physics community has established world-leading expertise in vortex dynamics, combining strengths in high-resolution observations, MHD turbulence theory, numerical modelling, and space instrumentation. UK researchers have made foundational contributions to Solar Orbiter, delivered critical systems for DKIST, and maintain active involvement in MUSE and SOLAR-C EUVST missions. Our technical approach centres on developing next-generation instrumentation: a multi-band, space-qualified system employing four tunable Fabry-Pérot Interferometers providing diffraction-limited, high-cadence spectropolarimetric coverage from the deep photosphere to the low corona. This capability will be validated through a staged mission architecture beginning with balloon-borne demonstrators. Continuing this effort over the coming decade is vital to maintain UK leadership in this field and achieve the goals of roadmap for solar system research.

</details>

### [21] [Cosmo3DFlow: Wavelet Flow Matching for Spatial-to-Spectral Compression in Reconstructing the Early Universe](https://arxiv.org/abs/2602.10172)
*Md. Khairul Islam,Zeyu Xia,Ryan Goudjil,Jialu Wang,Arya Farahi,Judy Fox*

**TL;DR**: 提出Cosmo3DFlow框架，通过结合三维离散小波变换(DWT)和流匹配技术，在128³分辨率下实现比扩散模型快50倍的宇宙学初始条件采样，解决了高维稀疏性和计算效率问题。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过生成式建模解决现代天体物理学中从演化后的现今宇宙重建早期宇宙的高维稀疏性计算难题。

Data: data部分：研究采用分辨率为128³的大尺度宇宙学N体模拟（N-body simulation）数据作为训练和测试基础。

Method: 采用三维离散小波变换（DWT）与流匹配（flow matching）结合的生成框架Cosmo3DFlow，通过小波变换将空间稀疏性转化为谱域稀疏性，并利用小波空间速度场实现大步长稳定ODE求解。

Result: 开发了Cosmo3DFlow框架，通过结合3D离散小波变换(DWT)和流匹配技术，实现了比扩散模型快50倍的采样速度，解决了宇宙学推断中的维数灾难和稀疏性问题。

Abstract: Reconstructing the early Universe from the evolved present-day Universe is a challenging and computationally demanding problem in modern astrophysics. We devise a novel generative framework, Cosmo3DFlow, designed to address dimensionality and sparsity, the critical bottlenecks inherent in current state-of-the-art methods for cosmological inference. By integrating 3D Discrete Wavelet Transform (DWT) with flow matching, we effectively represent high-dimensional cosmological structures. The Wavelet Transform addresses the ``void problem'' by translating spatial emptiness into spectral sparsity. It decouples high-frequency details from low-frequency structures through spatial compression, and wavelet-space velocity fields facilitate stable ordinary differential equation (ODE) solvers with large step sizes. Using large-scale cosmological $N$-body simulations, at $128^3$ resolution, we achieve up to $50\times$ faster sampling than diffusion models, combining a $10\times$ reduction in integration steps with lower per-step computational cost from wavelet compression. Our results enable initial conditions to be sampled in seconds, compared to minutes for previous methods.

</details>

### [22] [Why do we do astrophysics?](https://arxiv.org/abs/2602.10181)
*David W. Hogg*

**TL;DR**: 该白皮书探讨了大型语言模型（LLMs）在天体物理学数据科学领域自主设计、执行、撰写和评审科研项目的潜力及其对专业的影响，分析了天体物理学的核心价值（如创新性、以人为本、信任）和社会效益（如热爱、武器研发、人才培养），并反对两种极端政策（放任使用和全面禁止），强调制定适度政策的挑战。


<details>
  <summary>Details</summary>

Background: 该研究探讨了大型语言模型（LLMs）在天体物理学数据科学领域逐渐具备自主设计、执行、撰写和评审科研项目能力时，对天体物理学学科本质（如新颖性、以人为本、信任及非临床价值）及其社会价值（如促进热爱、武器研发与个人发展）的潜在影响，并批判性分析了两种极端政策建议（"let-them-cook"和"ban-and-punish"）的不合理性。

Data: 数据来源未在摘要中明确提及。

Method: 采用核心分析方法为基于事实或“共识点”（points of agreement）的论证框架，结合对天体物理学本质及潜在影响的哲学讨论。

Result: 大型语言模型（LLMs）在数据科学领域的天体物理学项目中展现出设计、执行、撰写和评审的能力，这对专业实践提出了新的挑战和机遇。

Abstract: At time of writing, large language models (LLMs) are beginning to obtain the ability to design, execute, write up, and referee scientific projects on the data-science side of astrophysics. What implications does this have for our profession? In this white paper, I list - and argue for - a set of facts or "points of agreement" about what astrophysics is, or should be; these include considerations of novelty, people-centrism, trust, and (the lack of) clinical value. I then list and discuss every possible benefit that astrophysics can be seen as bringing to us, and to science, and to universities, and to the world; these include considerations of love, weaponry, and personal (and personnel) development. I conclude with a discussion of two possible (extreme and bad) policy recommendations related to the use of LLMs in astrophysics, dubbed "let-them-cook" and "ban-and-punish." I argue strongly against both of these; it is not going to be easy to develop or adopt good moderate policies.

</details>

### [23] [Optimizing Deep Learning Photometric Redshifts for the Roman Space Telescope with HST/CANDELS](https://arxiv.org/abs/2602.10207)
*Ashod Khederlarian,Brett H. Andrews,Jeffrey A. Newman,Tianqing Zhang,Biprateep Dey*

**TL;DR**: 提出新型半监督深度学习模型PITA（Photo-$z$ Inference with a Triple-loss Algorithm），通过结合标记与未标记数据的三重损失函数，在哈勃太空望远镜CANDELS数据中实现了优于全监督和自监督方法的红移（photo-$z$）估计性能，尤其适用于南希·格雷斯·罗曼太空望远镜的高分辨率图像。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用深度学习提升光度红移（photo-$z$）测量精度，以支持南希·格雷斯·罗曼空间望远镜（Nancy Grace Roman Space Telescope）在星系演化、大尺度结构和瞬变天体研究中的应用。

Data: 数据来源于哈勃空间望远镜（Hubble Space Telescope）的CANDELS（Cosmic Assembly Near-infrared Deep Extragalactic Legacy Survey）光学和近红外成像观测。

Method: 采用深度学习（deep learning）方法，包括全监督（fully-supervised）、自监督（self-supervised）和半监督（semi-supervised）算法，其中新型半监督模型PITA（Photo-$z$ Inference with a Triple-loss Algorithm）通过三部分损失函数（three-part loss function）结合图像、颜色和可用红移数据实现最优性能。

Result: 基于哈勃太空望远镜CANDELS光学和近红外成像数据，研究发现半监督深度学习模型PITA（Photo-$z$ Inference with a Triple-loss Algorithm）通过结合标记与未标记数据的三重损失函数，在红移估计中表现最优，即使训练集显著减少时仍能保持最佳性能。

Abstract: Photometric redshifts (photo-$z$'s) will be crucial for studies of galaxy evolution, large-scale structure, and transients with the Nancy Grace Roman Space Telescope. Deep learning methods leverage pixel-level information from ground-based images to achieve the best photo-$z$'s for low-redshift galaxies, but their efficacy at higher redshifts with deep, space-based imaging remains largely untested. We used Hubble Space Telescope CANDELS optical and near-infrared imaging to evaluate fully-supervised, self-supervised, and semi-supervised deep learning photo-$z$ algorithms out to $z\sim3$. Compared to template-based and classical machine learning photometry methods, the fully-supervised and semi-supervised models achieved better performance. Our new semi-supervised model, PITA (Photo-$z$ Inference with a Triple-loss Algorithm), outperformed all others by learning from unlabeled and labeled data through a three-part loss function that incorporates images and colors for all objects as well as redshifts when available. PITA produces a latent space that varies smoothly in magnitude, color, and redshift, resulting in the best photo-$z$ performance even when the redshift training set was significantly reduced. In contrast, the self-supervised approach produced a latent space with significant color and redshift fluctuations that hindered photo-$z$ inference. Looking forward to Roman, we recommend using semi supervised deep learning to take full advantage of the information contained in the hundreds of millions of high-resolution images and color measurements, together with the limited redshift measurements available, to achieve the most accurate photo-$z$ estimates for both faint and bright sources.

</details>

### [24] [Pollux test bench: from NUV to FUV polarimetric measurements](https://arxiv.org/abs/2602.10901)
*Adrien Girardot,Coralie Neiner,Jean-Michel Reess,Olivier Dupuis,Margarita Carret*

**TL;DR**: 该论文核心贡献是设计了一种真空紫外(VUV)测试平台，用于验证Pollux光谱偏振仪中基于镜面创新组件的远紫外(FUV)通道和基于MgF₂双折射材料的近紫外(NUV)/中紫外(MUV)通道的性能，首次实现了FUV光谱偏振测量方法并提升紫外光谱偏振仪技术成熟度(TRL)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于为哈勃后继宽视场天文台（HWO）开发的高分辨率光谱偏振仪Pollux，特别针对其远紫外（FUV）通道缺乏双折射材料的挑战，提出了一种基于反射镜的创新偏振仪设计方案。

Data: 数据来源于波鲁克斯(Pollux)高分辨率分光偏振计的三个紫外波段通道（近紫外236-472纳米、中紫外118-236纳米和远紫外100-123纳米）的偏振仪测试平台，其中远紫外通道采用纯反射镜创新组件，测试平台在洁净室真空腔中运行。

Method: 采用基于真空紫外(VUV)范围的测试平台架构，结合可调波长设计和精密测量方法，验证不同波段的偏振仪性能，重点测试远紫外(FUV)通道的镜面组装偏振仪能否达到10^-3精度要求。

Result: Pollux的远紫外(FUV)通道偏振仪采用仅由反射镜组成的创新结构，解决了该波段缺乏双折射材料的问题。

Abstract: Pollux is a high-resolution spectropolarimeter proposed by an European consortium for HWO. The current design of Pollux features four spectropolarimetric channels, three of which are in the UV range. For the near-UV (NUV) [236-472 nm] and mid-UV (MUV) [118-236 nm] channels, the polarimeters consist of waveplates and prisms made of MgF2, a birefringent material. However, no such birefringent material can be used for the far-UV (FUV) channel [100-123 nm]. Therefore, the polarimeter for this FUV channel is composed solely of mirrors in an innovative assembly. In this talk, we aim to detail the architecture of the test bench that will allow us to validate the performance of these different polarimeters, as part of the HWO GOMaP. Given that we are working in the vacuum ultraviolet (VUV) range, the test bench operates in a vacuum chamber in a clean room. We will discuss the adaptable architecture of the bench based on wavelength and the measurement methodology that we will implement to test if the polarimeters achieve the precision of $10^{-3}$ required for the Pollux instrument. With this test bench, we will successfully increase the Technology Readiness Level (TRL) of UV spectropolarimeters and, for the first time, develop a means to test FUV spectropolarimetry.

</details>

### [25] [Early Architecture Concepts for the Habitable Worlds Observatory -- System Design, Modeling, and Analysis](https://arxiv.org/abs/2602.11046)
*Alice,Liu,Marie Levine,Charley Noecker,Jon Lawrence,Joshua Abel,Michael Akkerman,Eric Aanstaat,Ruslan Belikov,Pin Chen,Kenneth Dziak,Jordan Effron,Lee Feinberg,Alan Gostin,James Govern,Cameron Haag,Joseph Howard,Brian Kern,Gary Kuan,Milan Mandic,Carson McDonald,Connor Mulrenin,Bijan Nemati,Jon Papa,Fang Shi,Samuel Sirlin,Breann Sitarski,Cory Smiley,J. Scott Smith,Philip Stahl,Christopher Stark,Gregory Walsh,John Ziemer*

**TL;DR**: 论文核心贡献是提出了哈勃世界天文台（Habitable Worlds Observatory, HWO）的前三个探索性分析案例（Exploratory Analytic Cases, EACs），通过观测策略、误差预算制定、设计公式化和多学科系统级分析能力（如集成建模管道）来评估该任务的科学目标和工程参数。


<details>
  <summary>Details</summary>

Background: 研究主题为利用宜居世界天文台（Habitable Worlds Observatory, HWO）直接成像并表征类地系外行星及其大气层，探测潜在生命特征（biosignatures），同时作为多功能天体物理观测平台推动星系演化、恒星天体物理及暗物质研究。

Data: 研究中使用的数据来源包括模拟数据（simulation data）和来自先驱任务（如Nancy Grace Roman Space Telescope）的观测数据。

Method: 采用探索性分析案例（EACs）架构概念设计和集成建模管道（integrated modeling pipeline）进行多学科系统级分析。

Result: HWO（Habitable Worlds Observatory）将直接成像并表征类地系外行星及其大气层，具备探测生物标志物（biosignatures）的潜力，可能解答人类是否孤独的问题。

Abstract: The Habitable Worlds Observatory (HWO), NASA's next flagship science mission, follows in the tradition of the Nancy Grace Roman Space Telescope and other preceding great observatories. HWO will directly image and characterize Earth-like exoplanet and their atmospheres, with the capability to detect biosignatures and potentially answer the question of whether we are we alone. HWO will also serve as a powerful general astrophysics observatory, enabling breakthroughs in galaxy evolution, stellar astrophysics, and dark matter studies. Currently in pre-formulation, the project has established Exploratory Analytic Cases (EACs), a series of architectural concept designs used to assess the mission's demanding science objectives while exploring challenging engineering parameters. This paper describes the first three EACs, starting with observing strategies and error budget formulation and then progressing to design formulations, trade studies and lessons learned; this paper also discusses the integrated modeling pipeline, a key multidisciplinary system-level analysis capability, and analysis findings as applied to the first EAC. These activities set the stage for the follow on EACs 4 and 5, which will further explore the trade space and prepare for the baseline design that will support the Mission Concept Review (MCR).

</details>

<div id='hep-ph'></div>

# hep-ph [[Back]](#toc)

### [26] [Pushing the Limits of Atomic Dark Matter: First-Principles Recombination Rates and Cosmological Constraints](https://arxiv.org/abs/2602.10197)
*Jared Barron,Rouven Essig,Megan H. McDuffie,Jesús Pérez-Ríos,Gregory Suczewski*

**TL;DR**: 提出了一种最小原子暗物质（atomic dark matter）模型，通过结合宇宙微波背景（CMB）、重子声波振荡（BAO）和超新星（Pantheon+）数据，首次在全参数空间约束了暗物质质量与耦合强度对宇宙小尺度结构及早期声学阻尼效应的可观测影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于最小原子暗物质（minimal atomic dark matter）模型及其独特冷却机制对宇宙小尺度结构形成和早期宇宙学的潜在影响。

Data: 数据来源包括普朗克（Planck）和ACT的宇宙微波背景（CMB）测量数据、重子声学振荡（BAO）数据以及Pantheon+数据集。

Method: 采用暗原子物质模型（atomic dark matter model），包含两个费米子（$m_{p_{D}}$和$m_{e_{D}}$）在隐藏阿贝尔规范对称性（$U(1)_{D}$）下相互作用，通过暗精细结构常数（$α_{D}$）结合为中性原子态，并利用宇宙微波背景（CMB）、重子声波振荡（BAO）和Pantheon+数据对参数空间进行约束分析。

Result: 暗原子暗物质（atomic dark matter）模型通过暗精细结构常数（αD）和费米子质量（mpD、meD）参数化，其独特的冷却机制对小尺度结构形成和早期宇宙学具有潜在影响。

Abstract: Minimal atomic dark matter with its distinctive cooling mechanisms offers an instructive framework for understanding the potential impact of dark matter on small-scale structure formation and early cosmology. The model consists of two fermions with opposite charges under a hidden Abelian gauge symmetry $U(1)_{D}$ and masses $m_{p_{D}}$ and $m_{e_{D}}$, respectively. Analogous to hydrogen in the Standard Model, these fermions interact via their own electromagnetic-like force, with a dark fine structure constant denoted by $α_{D}$, and can bind into neutral atomic (and molecular) dark states. Previous work has largely focused on the benchmark scenario where the dark sector mirrors ordinary matter, with $m_{e_{D}}$ near the electron mass, $m_{p_{D}}$ near the proton mass, and $α_{D}\sim 1/137$. We extend this analysis by investigating dark recombination and cooling physics across the full parameter space of masses and couplings. Combining Cosmic Microwave Background (CMB) measurements from Planck and ACT with BAO and Pantheon+ data, we place new constraints on the atomic dark matter parameter space, identifying regions where acoustic damping and recombination dynamics leave observable imprints on the CMB.

</details>

### [27] [Gravitational scalar production with a generic reheating scenario](https://arxiv.org/abs/2602.10215)
*Francesco Costa,Jinsu Kim*

**TL;DR**: 研究了暴胀和再加热阶段引力产生解耦标量粒子的效率及其对暗物质丰度的影响，发现再加热动力学敏感地决定了标量自耦合常数（λ_s）、质量（m_s）和量子引力效应系数的限制条件，并揭示k<4的低再加热温度下延长再加热阶段可稀释遗迹粒子，而k>4时遗迹丰度增强导致严格限制，多阶段再加热中增强/稀释效应具有可分解性。


<details>
  <summary>Details</summary>

Background: 研究探讨了暴胀（inflationary）和后暴胀（post-inflationary）阶段中解耦标量粒子（decoupled scalars）的引力产生（gravitational production）及其对暗物质（dark matter）非热产生（non-thermal production）场景的影响。

Data: 研究中使用的数据来源于对暴胀（inflationary）和暴胀后（post-inflationary）阶段引力产生解耦标量（decoupled scalars）的理论模拟（simulation），包括通用幂律暴胀势（power-law inflaton potential）$V_{\rm inf}\propto φ^k$和多阶段再加热（multi-stage reheating）情景的数值计算。

Method: 采用核心分析方法包括研究暴胀（inflationary）和后暴胀（post-inflationary）阶段的标量粒子引力产生机制，分析不同再加热（reheating）情景（如幂律暴胀势$V_{\rm inf}\propto φ^k$和多阶段再加热）对自相互作用耦合常数$λ_s$、质量$m_s$及量子引力诱导算子系数的约束影响。

Result: 引力产生解耦标量粒子（decoupled scalars）的效率取决于再加热（reheating）动力学，其中k<4时延长的再加热阶段会稀释遗迹丰度，而k>4时遗迹丰度增强导致对标量的严格限制。

Abstract: Gravitational production of decoupled scalars during inflationary and post-inflationary phases is efficient and can lead to over-production. We study this production with various reheating scenarios such as a generic power-law inflaton potential $V_{\rm inf}\propto φ^k$ as well as a multi-stage reheating scenario. We derive constraints on the scalar self-interaction coupling $λ_s$, the mass $m_s$, and coefficients of quantum gravity-induced operators. We find that the constraints depend sensitively on the reheating dynamics. Our analysis demonstrates that universal gravity effects do not necessarily spoil the predictivity of non-thermal dark matter scenarios with $k < 4$ and low reheating temperatures, as an extended reheating phase dilutes gravitationally-produced relics. For $k > 4$, on the other hand, the relic abundance is enhanced during the reheating phase, leading to stringent constraints on the scalar. In multi-stage reheating, we show that the enhancement/dilution effect of subsequent reheating phases factorises.

</details>

### [28] [Light to Heavy, Brief to Eternal: An Axion for Every Occasion (in the Early Universe)](https://arxiv.org/abs/2602.11121)
*Francesco D'Eramo*

**TL;DR**: 这篇论文的核心贡献是通过研究早期宇宙中四种不同寿命的轴子（axion）情景（从长寿命到短寿命），揭示了轴子现象学与早期宇宙学之间的相互作用，并证明了宇宙学数据在探测广泛质量和寿命范围内的轴子方面的潜力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于早期宇宙中轴子（axion）的性质与相互作用，探讨其作为暗辐射（dark radiation）、暗物质（dark matter）、能量注入源或新物理门户（portal）在不同寿命尺度下的宇宙学现象学。

Data: 数据来源包括宇宙微波背景辐射（CMB）、大尺度结构（LSS）观测、21厘米全球信号（global 21 cm signal）以及数值模拟（numerical simulation）生成的早期宇宙演化数据。

Method: 聚焦四类轴子（axion）情景，从长寿命到短寿命依次分析其与早期宇宙的相互作用机制。

Result: 早期宇宙为研究轴子（axion）在不同寿命范围内的性质与相互作用提供了独特条件，涵盖从构成暗辐射（dark radiation）的长寿命轴子到作为暗物质（dark matter）的非平衡产物，再到通过衰变在21厘米信号中留下可观测特征的亚稳态轴子，以及作为新自由度门户的极短寿命轴子。

Abstract: The early universe grants access to energy scales far beyond those achievable in terrestrial experiments and allows unstable Standard Model particles to play an active dynamical role. In this contribution, we focus on recent studies aimed at quantifying the potential of the early universe to probe the properties and interactions of axions. The discussion is organized around four classes of axion scenarios, ordered from long to short lifetimes: (i) stable or long-lived axions contributing to dark radiation; (ii) stable or long-lived axions produced out-of-equilibrium and constituting dark matter; (iii) metastable axions whose decays inject energy into the primordial plasma and leave observable signatures in the global 21 cm signal; and (iv) very short-lived axions that act only as portals to additional degrees of freedom. Together, these scenarios highlight the interplay between axion phenomenology and early universe cosmology and demonstrate the potential of cosmological data to probe axions over a broad range of masses and lifetimes.

</details>

### [29] [Unidentified falling objects in the LHC as dark matter signals](https://arxiv.org/abs/2602.10562)
*Xunyu Liang,Ariel Zhitnitsky*

**TL;DR**: 提出轴子夸克块（AQNs）可能是大型强子对撞机（LHC）中部分不明坠落物（UFOs）的成因，并论证LHC可作为探测AQNs的宽带声学探测器。


<details>
  <summary>Details</summary>

Background: 研究探讨了大型强子对撞机（LHC）运行中观测到的未识别坠落物体（UFOs）现象，提出部分UFOs可能由轴子夸克块（AQNs）这一宏观暗物质（DM）候选体引起，并关联了暗物质与可见物质的丰度关系及重子-反重子不对称性机制。

Data: 数据来源于大型强子对撞机（LHC）运行期间观测到的偶发束流损失事件（UFOs）以及相关声波信号。

Method: 提出轴子夸克块（AQN）模型作为解释部分不明坠落物（UFO）的机制，并通过声波触发事件分析其与大型强子对撞机（LHC）的相互作用。

Result: 部分UFO事件（Unidentified Falling Objects）可能由轴子夸克块（axion quark nuggets, AQNs）引发，该模型将暗物质（dark matter, DM）与可见物质丰度关联并解释重子-反重子不对称性。

Abstract: Unidentified Falling Objects (UFOs) refer to sporadic beam losses observed during LHC operation. The prevailing explanation is that micrometer-sized dust particles released from the beam screen produce beam losses through interactions with the protons. However, the release mechanism of these particles remains unknown. We propose that roughly $(1-10)$% of UFOs may be caused by axion quark nuggets (AQNs), macroscopic dark matter (DM) candidates with masses of order $(5-1000)\,$g. The AQN model naturally relates the dark- and visible-matter abundances ($Ω_\mathrm{DM}\simΩ_\mathrm{visible}$) and provides a mechanism for generating the baryon-antibaryon asymmetry, with DM composed of both matter and antimatter AQNs. When passing underground within approximately 100km of the LHC, an antimatter AQN generates acoustic waves strong enough to trigger multiple UFO events within $2\,$s. If three correlated UFOs (placed at different locations along the LHC ring) are detected, the signal-to-noise ratio can exceed 5 across the entire allowed AQN mass range for a measurement time of about 360 hours. Practically, the LHC can serve as a large broadband acoustic detector for AQNs.

</details>

<div id='astro-ph.HE'></div>

# astro-ph.HE [[Back]](#toc)

### [30] [Machine Learning Methods for Stellar Collisions. I. Predicting Outcomes of SPH Simulations](https://arxiv.org/abs/2602.10191)
*Elena González Prieto,James C. Lombardi,,Sanaea C. Rose,Charles F. A. Gibson,Christopher E. O'Connor,Tjitske Starkenburg,Fulya Kıroğlu,Kyle Kremer,Tristan C. Parmerlee,Frederic A. Rasio*

**TL;DR**: 通过构建包含27,720组平滑粒子流体动力学(SPH)模拟的主序星碰撞网格数据并训练机器学习模型，实现了对恒星碰撞结果（合并/瓦解/飞越）和残余质量的快速高精度预测（分类平衡准确率98.4%，回归相对误差最低0.11%），为致密星团动力学N体模拟提供了高效工具包collAIder。


<details>
  <summary>Details</summary>

Background: 恒星碰撞在致密星团环境中频繁发生，是产生从球状星团中的蓝离散星到星系核高能暂现源等奇异现象的关键机制，并可能通过大质量恒星的连续碰撞与并合形成早期宇宙超大质量黑洞的种子。

Data: 数据来源于27,720个平滑粒子流体动力学（SPH）模拟计算的主序星碰撞网格，覆盖了广泛的质量、年龄、相对速度和碰撞参数范围。

Method: 采用平滑粒子流体动力学(SPH)模拟构建了27,720个主序星碰撞网格数据，并训练机器学习模型(包括最近邻、支持向量机和神经网络)预测碰撞结果(合并/瓦解/飞掠)和最终残余质量。

Result: 通过训练机器学习模型（最近邻、支持向量机和神经网络）对主序星碰撞结果（合并、瓦解或飞掠）及最终残余质量进行预测，分类平衡准确率达98.4%，回归相对误差最低为0.11%（恒星1）和0.15%（恒星2）。

Abstract: Stellar collisions can occur frequently in dense cluster environments, and play a crucial role in producing exotic phenomena from blue stragglers in globular clusters to high-energy transients in galactic nuclei. Successive collisions and mergers of massive stars could also lead to the formation of massive black holes, serving as seeds for supermassive black hole in the early universe. While analytic fitting formulae exist for predicting collision outcomes, they do not generalize across different energy scales or stellar evolutionary phases. Smoothed particle hydrodynamics (SPH) simulations are often used to compute the outcomes of stellar collisions, but, even at low resolution, their computational cost makes running on-the-fly calculations during an $N$-body simulation quite challenging. Here we present a new grid of $27,720$ SPH calculations of main-sequence star collisions, spanning a wide range of masses, ages, relative velocities, and impact parameters. Using this grid, we train machine learning models to predict both collision outcomes (merger vs disruption, or flyby) and final remnant masses. We compare the performance of nearest neighbors, support vector machines, and neural networks, achieving classification balanced accuracy of $98.4\%$, and regression relative errors as low as $0.11\%$ and $0.15\%$ for the final stars $1$ and $2$, respectively. We make our trained models publicly available as part of the package collAIder, enabling rapid predictions of stellar collision outcomes in $N$-body models of dense star cluster dynamics.

</details>

### [31] [TDE 2025abcr: A Tidal Disruption Event in the Outskirts of a Massive Galaxy](https://arxiv.org/abs/2602.10180)
*Robert Stein,Jonathan Carney,Charlotte Ward,Raffaella Margutti,Xander J. Hall,Itai Sfaradi,Igor Andreoni,Ryan Chornock,Suvi Gezari,Geoffrey Mo,Yuhan Yao,Eric C. Bellm,Joshua S. Bloom,Malte Busmann,Ilaria Caiazzo,S. Bradley Cenko,Matthew J. Graham,Steven L. Groom,Daniel Gruen,Erica Hammerstein,Mansi M. Kasliwal,Brendan O'Connor,Antonella Palmese,Josiah Purdum,Jillian C. Rastinejad,Reed Riddle,Ben Rusholme,Jesper Sollerman,Jean J. Somalwar,Sylvain Veilleux*

**TL;DR**: 首次在星系外围发现光学潮汐瓦解事件(TDE) 2025abcr，其由游荡黑洞（wandering black hole）引发，距离宿主星系核10.3千秒差距，中心黑洞质量估计为$10^{6.09\pm0.53}\mathrm{M}_{\odot}$，表明未来维拉·鲁宾望远镜每年可探测数十例类似偏移事件。


<details>
  <summary>Details</summary>

Background: 潮汐瓦解事件(TDEs)传统上通过针对星系核瞬变的光学巡天被发现，但预期部分TDEs会由星系合并产生的游荡黑洞(Wandering black holes)在星系核外区域引发。

Data: 数据来源于ZTF（Zwicky Transient Facility）光变曲线观测和后续确认观测。

Method: 采用基于光变曲线（lightcurve）的机器学习分类器（ML classifier）$\texttt{tdescore}$定制化"非核区"（off-nuclear）实现来识别潮汐瓦解事件（TDE）。

Result: 首次在宿主星系外围发现的光学潮汐瓦解事件(TDE) 2025abcr由游荡黑洞（wandering black hole）引发，距离星系核9.5角秒（10.3千秒差距投影距离），其中心黑洞质量估计为$10^{6.09\pm0.53}\mathrm{M}_{\odot}$，显著低于宿主星系中心黑洞质量$10^{8.82 \pm 0.65}\mathrm{M}_{\odot}$。

Abstract: Tidal disruption events (TDEs) have traditionally been discovered in optical sky surveys through targeted searches of nuclear transients. However, it is expected that some TDEs will occur outside the galaxy nucleus, arising from wandering black holes originating in galaxy mergers. Here we present observations of TDE 2025abcr, the first optical TDE discovered in the outskirts of a host galaxy. The TDE was identified by a custom 'off-nuclear' implementation of the ML classifier $\texttt{tdescore}$, which classifies new ZTF transients based on their lightcurves. Follow-up observations confirm that TDE 2025abcr is a TDE-H+He, occurring 9.5$"$ (10.3 kpc projected distance) from the nucleus of a massive galaxy ($\mathrm{M}_{\star}$ = $10^{11.18 \pm 0.03}\mathrm{M}_{\odot}$) with a central black hole mass of $10^{8.82 \pm 0.65}\mathrm{M}_{\odot}$. TDE 2025abcr itself was likely disrupted by a much lighter black hole ($10^{6.09\pm0.53}\mathrm{M}_{\odot}$, as estimated with peak luminosity scaling relations). The black hole was either dynamically ejected from the nucleus or lies at the center of a very faint tidally-stripped dwarf galaxy undergoing a minor merger. Late-time observations of TDE 2025abcr could confirm the origin of this apparent 'orphan' black hole. The rate of highly offset ($\gtrsim$3 kpc) TDEs can be constrained to $<$10% of the nuclear TDE rate, but our discovery implies that many dozens of similar sources will be detected by the Vera C. Rubin each year with resolvable offsets.

</details>

### [32] [Future Perspectives on Black Hole Jet Mechanisms: Insights from Next-Generation Observatories and Theoretical Developments](https://arxiv.org/abs/2602.11094)
*Andre L. B. Ribeiro,Nathalia M. N. da Rocha*

**TL;DR**: 通过整合事件视界望远镜(EHT)等新一代观测设施与磁滞吸积盘(MADs)等理论模型，未来十年将在黑洞喷流(black hole jets)的动力学机制和能量耗散过程研究方面取得突破性进展。


<details>
  <summary>Details</summary>

Background: 黑洞喷流（black hole jets）研究涉及吸积物理、相对论磁流体动力学（relativistic magnetohydrodynamics）及星系与星系团中大尺度反馈等极端天体物理过程。

Data: 研究中使用的观测数据来源于事件视界望远镜（EHT）、切伦科夫望远镜阵列（CTA）、薇拉·鲁宾天文台（LSST）和全地球耀变体望远镜（WEBT）。

Method: 采用广义相对论磁流体动力学（GRMHD）模拟结合磁滞吸积盘（MADs）模型分析喷流形成机制。

Result: 黑洞喷流（black hole jets）的物理机制（包括启动、准直和能量耗散）仍是未解难题，未来观测设施和理论模拟将推动该领域取得突破性进展。

Abstract: Black hole jets represent one of the most extreme manifestations of astrophysical processes, linking accretion physics, relativistic magnetohydrodynamics, and large-scale feedback in galaxies and clusters. Despite decades of observational and theoretical work, the mechanisms governing jet launching, collimation, and energy dissipation remain open questions. In this article, we discuss how upcoming facilities such as the Event Horizon Telescope (EHT), the Cherenkov Telescope Array (CTA), the Vera C. Rubin Observatory (LSST), and the Whole Earth Blazar Telescope (WEBT) will provide unprecedented constraints on jet dynamics, variability, and multi-wavelength signatures. Furthermore, we highlight theoretical challenges, including the role of magnetically arrested disks (MADs), plasma microphysics, and general relativistic magnetohydrodynamic (GRMHD) simulations in shaping our understanding of jet formation. By combining high-resolution imaging, time-domain surveys, and advanced simulations, the next decade promises transformative progress in unveiling the physics of black hole jets.

</details>

### [33] [Evidence for neutrino emission from X-ray Bright Seyfert Galaxies in the Southern Hemisphere using Enhanced Starting Track Events with IceCube](https://arxiv.org/abs/2602.10208)
*The IceCube Collaboration*

**TL;DR**: 研究发现14个南天赛弗特星系(Seyfert galaxy)存在集体中微子信号(3σ显著性)，为高能中微子河外起源提供了新证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探究赛弗特星系(Seyfert galaxies)对高能中微子(high-energy neutrinos)河外通量的潜在贡献。

Data: 数据来源于2011至2021年间IceCube探测器记录的相互作用顶点位于探测器内部的μ子中微子（muon neutrinos）数据集，并结合了Swift Burst Alert Telescope (BAT) AGN Spectroscopic Survey选定的南天区14个塞弗特星系（Seyfert galaxies）样本。

Method: 采用基于盘冕模型（disk-corona model）的 keV X射线与 TeV 中微子相关性分析，并假设与 NGC~1068 类似的中微子产生特征。

Result: 在14个南天赛弗特星系(Seyfert galaxies)的集体中微子信号搜索中发现了6.7_{-3.2}^{+4.0}个事件超出背景预期的3σ显著性证据，为赛弗特星系对高能中微子河外通量的贡献提供了新独立证据。

Abstract: IceCube recently reported the observation of TeV neutrinos from the nearby Seyfert galaxy NGC~1068, and the corresponding neutrino flux is significantly higher than the upper limit implied by observations of GeV-TeV gamma rays. This suggests that neutrinos are produced near the supermassive black hole, where the radiation density is high enough to obscure gamma rays. We use a set of muon neutrinos with interaction vertices inside the detector, which have good sensitivity to sources in the Southern sky, from IceCube data recorded between 2011 and 2021. We then search for individual and collective neutrino signals from 14 Seyfert galaxies in the Southern Sky selected from the Swift Burst Alert Telescope (BAT) AGN Spectroscopic Survey. Using the correlations between keV X-rays and TeV neutrinos predicted by disk-corona models, and assuming production characteristics similar to NGC~1068, a collective neutrino signal search reveals an excess of $6.7_{-3.2}^{+4.0}$ events, which is inconsistent with background expectations at the 3$σ$ level of significance. In this paper, we present new independent evidence that Seyfert galaxies contribute to the extragalactic flux of high-energy neutrinos.

</details>

### [34] [Constraining the Evolution of the HI Spin Temperature with Fast Radio Bursts](https://arxiv.org/abs/2602.10328)
*Hugh Roxburgh,Marcin Glowacki,Apurba Bera,Clancy James*

**TL;DR**: 通过分析快速射电暴(FRB) 20211127I的原子氢(HI)吸收信号与发射图谱，首次提出了利用非重复性FRB探测宿主星系中性气体自旋温度(T_spin)下限(26 K)的方法，并论证了现有望远镜阵列对光学深度的高灵敏度探测潜力。


<details>
  <summary>Details</summary>

Background: 该研究探讨了快速射电暴（FRB）在穿越宿主星系星际介质时可能携带的原子氢（HI）吸收信号，为通过自旋温度（$T_{spin}$）约束中性气体相平衡及热过程以探测宇宙学距离中性气体动力学提供了独特机会。

Data: 数据来源于ASKAP探测到的FRB 20211127I（流量35 Jy ms，宽度0.2 ms）以及MeerKAT L波段3小时观测获得的HI发射谱图。

Method: 结合原子氢（HI）吸收信号与高分辨率HI发射图，通过约束HI激发温度（T_spin）来探测宇宙学距离上中性气体动力学。

Result: 通过分析FRB 20211127I的脉冲平均光谱，测得积分光学深度（integrated optical depth）的3σ上限为33 km s⁻¹，并结合MeerKAT观测的HI发射数据，推导出自旋温度（T_spin）下限为26 K。

Abstract: Fast radio bursts (FRBs) emit broad band radio wave radiation that may, in rare cases, encode atomic hydrogen (HI) absorption signals produced as they traverse the interstellar medium of their host galaxies. Combining such signals with high resolution HI emission maps offers a unique opportunity to probe the dynamics of neutral gas at cosmological distances through constraints of the HI excitation temperature $T_{spin}$, which characterises the balance of neutral gas phases and the underlying thermal processes within these galactic environments. While no absorption signal has been recorded in an FRB to date, we demonstrate a proof of concept with the bright (F = 35 Jy ms) and narrow (0.2 ms) FRB 20211127I detected by ASKAP. We find a 3$σ$ upper limit on the integrated optical depth in the pulse-averaged spectrum of 33 km s$^{-1}$, and, based on the HI emission observed in a 3 hr MeerKAT L-band observation, subsequently find a lower limit on $T_{spin}$ of 26 K. While this test case provides little constraining power, we find that narrow, non-repeating FRBs with fluences greater than 20/70/150 Jy ms observed with all dishes with the current MeerKAT/ASKAP/DSA telescopes can probe integrated optical depths below 5 km s$^{-1}$. Furthermore, we highlight that utilising FAST's incredible sensitivity to stack thousands of bursts from hyperactive repeaters also provides a plausible avenue through which HI absorption, and hence $T_{spin}$, can be measured. Finally, we discuss how HI absorption can address several modern challenges in FRB science, providing a physical anchor for locating bursts within their host galaxies and helping to disentangle the host contribution to dispersion and scattering.

</details>

### [35] [An Enhanced Formation Channel for Galactic Dual-Line Gravitational-Wave Sources: von Zeipel-Lidov-Kozai effect in Triples Involving Sgr A*](https://arxiv.org/abs/2602.10928)
*Wen-Fan Feng,Tan Liu,Yun Fang,Yacheng Kang,Bin Liu,Lijing Shao*

**TL;DR**: 研究发现银河系中心超大质量黑洞(Sgr A*)周围的三体系统中，冯·蔡佩尔-利多夫-科扎伊(ZLK)效应通过调制内双星轨道偏心率和倾角，将可同时探测低频旋进与高频自旋的双线引力波源数量提升约3倍至4年内约1例。


<details>
  <summary>Details</summary>

Background: 银河系中心致密环境可能通过未来空间引力波探测器（如LISA、天琴、太极）和下一代地基探测器（如宇宙探索者、爱因斯坦望远镜）观测到毫赫兹和百赫兹频段双线引力波源（低频旋近与高频自转），但其形成机制尚不明确。

Data: data：研究结合了未来空间引力波探测器（如LISA、TianQin、Taiji）的毫赫兹波段观测数据与下一代地面引力波探测器（如Cosmic Explorer、Einstein Telescope）的百赫兹波段数据。

Method: 采用von Zeipel-Lidov-Kozai（ZLK）效应分析分层三体系统中内双星轨道偏心率与倾角的振荡对引力波（GW）辐射的调制作用。

Result: von Zeipel-Lidov-Kozai (ZLK)效应可将银河系中心双线引力波源的预期数量提升约3倍，使其在4年内从稀少增至约1个。

Abstract: The dense Galactic Center environment is expected to host compact binary inspirals detectable by future space-borne gravitational wave (GW) observatories (e.g., LISA, TianQin, Taiji) in the millihertz band. Aided by information from these facilities, next-generation ground-based GW detectors (e.g., Cosmic Explorer, Einstein Telescope) can potentially capture gravitational radiation in the hectohertz band from rapidly spinning neutron star (NS) components in such binaries. These Galactic Center systems are thus anticipated to act as dual-line (i.e., low-frequency inspiral and high-frequency spin) GW sources. However, the formation channels of these systems remain largely unexplored. In this \textit{Letter}, we propose that the von Zeipel-Lidov-Kozai (ZLK) effect can enhance the formation of dual-line GW sources in hierarchical triples involving the Galactic supermassive black hole, Sgr A*. We show that ZLK-driven oscillations in the eccentricity and inclination of the inner binary can modulate the GW emission from both the binary inspiral and the individual NS spins. This effect boosts the expected dual-line source count by a factor of $\sim 3$, from rare to $\mathcal{O}(1)$ in 4 years, making dual-line observations substantially more probable. Our results demonstrate that the ZLK effect provides an important formation channel for Galactic dual-line GW sources.

</details>

### [36] [X-ray stellar feedback in low-metallicity starbursts: Insights from the template starburst galaxy ESO 338-IG04 and its halo](https://arxiv.org/abs/2602.10979)
*M. Chatzis,L. M. Oskinova,S. Reyero Serantes,B. D. Lehmer,G. Östlin,A. Bik,M. Hayes,J. M. Mas-Hesse,J. S. Gallagher,F. Fürst*

**TL;DR**: 研究发现低金属丰度（low-metallicity）星暴星系ESO 338-IG04中存在五个超亮X射线源（ULXs）和弥散热气体，其X射线辐射显著影响电离光子预算，其中ULX1可解释观测到的氦离子（He II λ4686）发射线光度。


<details>
  <summary>Details</summary>

Background: 低金属丰度（low-metallicity）星暴星系（starburst galaxies）的X射线辐射是恒星反馈（stellar feedback）的关键组成部分，涉及气体电离（gas ionization）和化学增丰（chemical enrichment）过程。

Data: 研究使用了新的钱德拉(Chandra)和XMM-牛顿(XMM-Newton)深空观测数据，包括X射线光谱、光变曲线和图像数据。

Method: 采用X射线光谱、光变曲线和成像分析结合光致电离模型（photoionization modeling）研究X射线源性质及其对电离光子预算的影响。

Result: 低金属丰度(metallicity)星暴星系ESO 338-IG04中存在五个超亮X射线源(ULXs)和弥散热气体，其中两个ULXs与星团空间相关，总X射线光度超过10^41 erg/s，最亮源ULX1在日尺度上表现出变异性且无对应星团。

Abstract: The X-ray output of low-metallicity starburst galaxies is a key component of stellar feedback, tracing the processes responsible for gas ionization and chemical enrichment. The integrated X-ray luminosity ($L_X$) from high-mass X-ray binaries in star-forming galaxies scales with both the star formation rate (SFR) and host-galaxy metallicity $Z$. Due to the inverse correlation between $L_X/\mathrm{SFR}$ and $Z$, the contribution of X-ray binaries to the ionizing photon budget is enhanced in metal-poor systems and may ionize He II in the surrounding interstellar medium, powering nebular He II $\lambda4686$ emission. The blue compact dwarf galaxy ESO 338-IG04 (ESO 338-4) provides a nearby laboratory for studying stellar feedback in a low-metallicity starburst, combining vigorous recent star formation, low metallicity ($12+\log(\mathrm{O/H})\approx7.9$), and a rich population of massive stellar clusters. We characterize the X-ray emission of ESO 338-4 and its halo using new deep Chandra and XMM-Newton observations. We analyze X-ray spectra, light curves, and images to constrain the nature of its X-ray sources. We identify five ultra-luminous X-ray sources (ULXs) and diffuse hot gas surrounding the galaxy. Two ULXs are spatially associated with stellar clusters. The total X-ray luminosity exceeds $10^{41}\,\mathrm{erg\,s^{-1}}$. The brightest source, ULX1, shows variability on day timescales and lacks a stellar-cluster counterpart. Photoionization modeling shows that X-ray sources significantly impact the ionizing photon budget; models with ULX1 as the ionizing source predict nebular He II $\lambda4686$ luminosities of $\sim10^{39}\,\mathrm{erg\,s^{-1}}$.

</details>

<div id='gr-qc'></div>

# gr-qc [[Back]](#toc)

### [37] [Accelerated expansion of the universe purely driven by scalar field fluctuations](https://arxiv.org/abs/2602.10223)
*Daniel Jiménez-Aguilar*

**TL;DR**: 研究发现，在空间闭合且标量场（scalar field）的康普顿波长超过曲率半径的宇宙中，仅标量场涨落即可驱动宇宙加速膨胀，为暴胀（inflation）和暗能量（dark energy）源于闭合宇宙中极轻玻色子气体提供了新解释。


<details>
  <summary>Details</summary>

Background: 宇宙空间闭合且标量场（scalar field）的康普顿波长超过曲率半径时，其涨落可独立驱动宇宙加速膨胀。

Data: 数据来源于理论模拟（simulation）分析，基于标量场（scalar field）波动在空间闭合宇宙中的动力学行为。

Method: 采用标量场（scalar field）波动分析，结合空间闭合宇宙模型和康普顿波长（Compton wavelength）与曲率半径的比较。

Result: 标量场（scalar field）涨落可在空间闭合且康普顿波长（Compton wavelength）超过曲率半径的宇宙中单独驱动宇宙加速膨胀。

Abstract: We show that scalar field fluctuations alone can drive cosmic acceleration, provided the universe is spatially closed and the Compton wavelength of the field exceeds the radius of curvature. This mechanism may open new perspectives on inflation and dark energy, which could arise from a gas of sufficiently light bosons in a closed universe.

</details>

### [38] [The measurable impact of the 2pN spin-dependent accelerations on the jet precession of M87$^\ast$](https://arxiv.org/abs/2602.10311)
*Lorenzo Iorio*

**TL;DR**: 基于克尔度规(Kerr metric)在谐和坐标系(harmonic coordinates)中解析计算了无电荷、无自旋物体的二阶后牛顿(post-Newtonian)加速度，发现与黑洞角动量偶次和奇次幂相关的新加速度项，其中轨道平面进动(precession)正比于黑洞角动量一次幂和光速四次方倒数，相当于对应兰斯-蒂林效应(Lense-Thirring effect)的20%。


<details>
  <summary>Details</summary>

Background: 该研究基于对星系超大质量黑洞（supermassive black hole）周围盘/喷流（disk/jet）共进动现象的精确测量，探讨了克尔度规（Kerr metric）下无电荷、无自旋物体在谐和坐标系（harmonic coordinates）中受到的二阶后牛顿（second post-Newtonian）加速度及其轨道效应。

Data: 研究中使用的数据来源为理论推导的克尔度规（Kerr metric）在谐和坐标系下的解析计算结果，未涉及实际观测或数值模拟数据。

Method: 采用谐和坐标系（harmonic coordinates）下的克尔度规（Kerr metric）解析计算无电荷、无自旋物体在第二阶后牛顿近似（second post-Newtonian order）下的加速度，并以坐标无关的矢量形式表达。

Result: 在克尔(Kerr)度规下，谐和坐标系中无电荷、无自旋物体受到的加速度被解析计算至二阶后牛顿阶，揭示了与黑洞角动量偶次和奇次幂成正比的新加速度项。

Abstract: Motivated by recent accurate measurements of disk/jet coprecessions around some galactic supermassive black holes, the accelerations experienced by an uncharged, spinless object in the Kerr metric, written in harmonic coordinates, are analytically calculated up to the formal second post-Newtonian order. To such a level, some new accelerations make their appearance. They are proportional to even and odd powers of the hole's angular momentum. Their counterparts are not known where the primary is a material body. After expressing them in a coordinate-independent, vector form valid for any orientations of the hole's spin axis in space, their orbital effects are perturbatively worked out in terms of the particle's Keplerian orbital elements. The resulting expressions, averaged over one orbital revolution, are valid for generic shapes and inclinations of the orbit. The orbital plane's precession proportional to the first power of the hole's angular momentum and to the reciprocal of the fourth power of the speed of light amounts to about twenty per cent of the corresponding Lense-Thirring effect. The latter is believed to be the cause of the accurately measured disk/jet precessional phenomenology, currently measured to a few per cent accuracy. Although at a lesser extent, also the precession proportional to the second power of the hole's spin and to the reciprocal of the fourth power of the speed of light is measurable. Allowed domains in the parameter space of the jet precession around M87$^\ast$ are displayed.

</details>

<div id='physics.plasm-ph'></div>

# physics.plasm-ph [[Back]](#toc)

### [39] [Bridging the Kinetic-Fluid Gap: Ion-Driven Magnetogenesis to Prime Cosmic Dynamos](https://arxiv.org/abs/2602.10447)
*X. Liu,D. Wu,J. Zhang*

**TL;DR**: 通过高分辨率动理学模拟发现，离子主导的丝状不稳定性（filamentation instability）能突破电子尺度饱和限制，将磁场能量提升数个量级并扩展至离子尺度，填补了微观等离子体不稳定性与宏观宇宙发电机（cosmic dynamos）间的关键空白。


<details>
  <summary>Details</summary>

Background: 宇宙磁场的起源被广泛归因于湍流发电机（turbulent dynamos）对弱种子场的放大，但微观种子场生成与宏观发电机启动之间存在关键理解空白。

Data: data部分：采用高分辨率动理学模拟（kinetic simulation）数据，设定真实质量比（realistic mass ratio），研究离子-动理学区域（ion-kinetic regime）的物理过程。

Method: 采用高分辨率动理学(kinetic)模拟结合真实质量比，揭示离子动理学(ion-kinetic)机制下广义连续剪切驱动引发的离子主导演化过程。

Result: 在广义连续剪切驱动下，离子主导的丝状不稳定性（filamentation instability）能够将磁能放大数个量级并扩展到离子尺度，填补了微观等离子体不稳定性与宏观宇宙发电机（cosmic dynamos）之间的关键空白。

Abstract: The origin of cosmic magnetic fields is widely attributed to the amplification of weak seed fields by turbulent dynamos. However, a critical understanding gap remains between the microscopic generation of these seeds and the macroscopic onset of the dynamo. Current kinetic models, often constrained to electron scales, predict premature saturation via magnetic trapping, leaving the generated fields potentially too weak and small-scale to effectively prime magnetohydrodynamic (MHD) processes. Here, using high-resolution kinetic simulations with a realistic mass ratio, we reveal the physics of this unexplored ion-kinetic regime. Under generalized continuous shear driving, used to simulate ubiquitous macroscopic flows, we demonstrate that the saturation of electron instabilities is not the endpoint but a precursor to a distinct, ion-dominated evolution. Massive ions, sustaining the velocity shear, trigger a subsequent filamentation instability that accesses the vast ion kinetic energy reservoir. This mechanism amplifies the magnetic energy by orders of magnitude beyond the electron-saturation limit, expanding the field coherence to ion scales. Our results establish ion kinetics as the essential ''missing link'' that bridges the divide between microscopic plasma instabilities and macroscopic cosmic dynamos.

</details>

<div id='astro-ph.SR'></div>

# astro-ph.SR [[Back]](#toc)

### [40] [A method for constructing the joint mass function of binary stars](https://arxiv.org/abs/2602.10186)
*Amery Gration,Robert G. Izzard,Payel Das*

**TL;DR**: 该论文提出了一种构建双星系统主星和伴星质量联合分布（joint distribution）的通用方法，解决了现有方法仅适用于随机配对（random pairing）的局限性，并通过实例验证了该方法在随机配对和均匀配对（uniform pairing）情况下的有效性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星初始质量函数（IMF）及其在双星系统中的扩展统计描述，探讨其对星系演化关键参数（如光度、金属产量、恒星形成效率和超新星产生率）的基础性影响。

Data: 研究中使用的数据来源为理论模拟（simulation）方法构建的初始质量函数（IMF）和双星系统质量比分布。

Method: 采用构建双星系统主星（primary-star）和伴星（secondary-star）质量联合分布的方法，突破随机配对（random pairing）的限制，适用于一般情况下的统计建模。

Result: 构建了一种普适方法，可推导双星系统主星（primary-star）和伴星（secondary-star）质量的联合分布（joint distribution），突破了仅适用于随机配对（random pairing）的现有局限。

Abstract: The initial mass function (IMF) describes the distribution of stellar masses in a population of newly born stars and is amongst the most fundamental concepts in astrophysics. It is not only the direct result of the star formation process but it also explains the evolution of galaxies' luminosities, metal yields, star-formation efficiencies, and supernova production rates. Because most stars exist in binary systems, however, a full statistical account of stellar mass requires not the IMF but rather the joint distribution of a binary population's primary- and secondary-star masses. This joint distribution must respect the IMF of the stars from which the population has been assembled as well as the distribution of mass ratios that results from the assembly mechanism. Despite its importance, this joint distribution is known only in the case of random pairing. Here we present a method for constructing it in the general case. We also illustrate the use of our method by recovering the known result for random pairing and by finding the previously unknown result for uniform pairing.

</details>

### [41] [Re-visiting the Canis Major star-forming region with Gaia data release 3 data](https://arxiv.org/abs/2602.10323)
*S. Nogueira-Silva,P. A. B. Galli,J. Olivares,H. Bouy,B. P. Popowicz,P. M. Merino,T. Santos-Silva,J. Gregorio-Hetem,N. Miret-Roig,J. Alves*

**TL;DR**: 基于盖亚(Gaia) DR3数据在CMa区域新发现1130个候选成员星，确认两个距离相近(约1150-1183 pc)、年龄约2-3 Myr的亚群(Cluster A和B)，其中Cluster A因盘星比例更高可能更年轻。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索大犬座(Canis Major)恒星形成区的恒星种群特征及其演化阶段，该区域位于新发现的拉德克利夫波(Radcliffe Wave)分子云复合体中且现有研究较少。

Data: 数据来源为盖亚空间任务第三次数据释放（Gaia DR3）的天体测量和测光数据，以及补充的光谱观测。

Method: 基于盖亚卫星第三次数据释放（Gaia DR3）的天体测量与测光数据，结合光谱观测，采用成员星分析（membership analysis）方法对CMa区域恒星进行普查，并利用等龄线（isochronal age）和红外测光（infrared photometry）技术评估年龄与演化阶段。

Result: 在CMa区域确认了1531个成员天体（包括401个已知成员和1130个新候选成员），识别出两个距离相近（约1150-1183 pc）、运动学特征相似且等时年龄约2-3 Myr的亚群（Cluster A和Cluster B），其中Cluster A因具有更高比例的盘星比例可能比Cluster B更年轻。

Abstract: Context: The Canis Major (CMa) star-forming region, a remote molecular cloud complex within the recently discovered Radcliffe Wave, remains under-explored in the literature. Aims: We revisit the stellar census in the CMa region, characterizing its stellar population, kinematics, and age using recent astrometric and photometric data from the third data release of the Gaia space mission (Gaia DR3). Methods: We conducted a membership analysis of Gaia DR3 sources across a 16 deg$^2$ field encompassing the youngest subgroups in CMa. This new stellar census, combined with spectroscopic observations, allowed us to investigate the structure, kinematics, and age of this region. Results: We identified 1531 objects as members of the CMa region, confirming 401 previously known members and introducing 1130 new candidate members. These objects have magnitudes ranging from 10 to 18 mag in the G band from Gaia DR3. We identified two subgroups of CMa stars in our sample labelled as Cluster A and Cluster B. They are located at roughly the same distance ($d_{A} = 1150^{+79}_{-88}$ pc and $d_{B} = 1183^{+103}_{-108}$ pc) and exhibit similar space motions that can be derived thanks to the precise radial velocities obtained in this study. The subgroups have a mean isochronal age of about 2-3 Myr. However, based on infrared photometry we show that Cluster A has a higher fraction of disc-bearing stars suggesting that it could be somewhat younger than Cluster B. Conclusions: Our analysis provides new insights into the stellar population of the Canis Major region, by identifying new members, characterizing their kinematics, and assessing their evolutionary stages. Future studies incorporating additional data from upcoming Gaia data releases, multi-wavelength and high-resolution spectroscopic observations will be essential to further advance our understanding of the history of star formation in this region.

</details>

<div id='physics.chem-ph'></div>

# physics.chem-ph [[Back]](#toc)

### [42] [Machine learning exploration of binding energy distributions of H2O at astrochemically relevant dust grain surfaces](https://arxiv.org/abs/2602.11050)
*Anant Vaishnav,Niels M. Mikkelsen,Mie Andersen*

**TL;DR**: 该研究通过机器学习原子间势能模型，系统揭示了星际尘埃颗粒表面部分和完全冰覆盖条件下水分子结合能（BEs）的分布特征，发现亚单层覆盖时基底化学性质主导结合能分布，单层以上覆盖时冰内氢键作用成为主导因素，且非晶冰结构比晶态冰产生更强的结合能分布偏移。


<details>
  <summary>Details</summary>

Background: 研究聚焦星际尘埃颗粒表面吸附物结合能（BEs）对星云和行星形成区域天体化学模型的关键影响，特别关注水在部分或完全冰覆盖尘粒表面的结合能分布特征。

Data: 数据来源为基于图神经网络（graph neural networks）的机器学习原子间势（machine-learning interatomic potentials, MLIPs）模拟的水分子在石墨烯和镁端接（010）面橄榄石表面的吸附行为，覆盖了亚单层、单层及双层水冰在不同生长条件（晶态与非晶态）下的吸附位点采样。

Method: 采用基于图神经网络（graph neural networks）的机器学习原子间势（MLIPs）模拟水在石墨烯和镁端接（010）面的镁橄榄石表面的吸附行为，覆盖亚单层到双层冰结构在不同生长条件（晶态与非晶态）下的广泛位点采样。

Result: 在亚单层覆盖时，基底（碳质或硅酸盐）的化学性质显著影响冰形态和结合能（binding energies），而单层以上覆盖后吸附主要由冰内氢键主导，削弱了基底材料的影响。

Abstract: Binding energies (BEs) of adsorbates on interstellar dust grains critically control adsorption, desorption, diffusion, and surface reactivity, and therefore strongly influence astrochemical models of star- and planet-forming regions. While recent computational studies increasingly report full distributions of BEs rather than single representative values, these distributions are typically derived for either bare grain surfaces or thick water-ice mantles. In this work, we bridge these regimes by systematically investigating the BE distributions of water on partially and fully ice-covered dust grain surfaces.
  We employ machine-learning interatomic potentials (MLIPs) based on graph neural networks to model water adsorption on graphene and on the Mg-terminated (010) surface of forsterite, representing carbonaceous and silicate grains, respectively. The models enable extensive sampling of adsorption sites on water clusters, monolayers, and bilayers generated under both crystalline (thermally processed) and amorphous (low-temperature) growth conditions. At submonolayer coverage, the chemical nature of the underlying grain strongly affects both ice morphology and binding energies, with Mg-O interactions on silicate surfaces producing particularly deep binding sites. From monolayer coverage onward, adsorption on both substrates is dominated by hydrogen bonding within the ice, reducing the influence of the grain material.
  Across all coverages, amorphous ice structures systematically shift the BE distributions toward stronger binding compared to crystalline ice, introducing highly stable defect and pocket sites. These results demonstrate that BE distributions in the submonolayer to few-layer ice regime are broad and highly surface dependent, and they provide physically motivated input for next-generation astrochemical models incorporating surface heterogeneity.

</details>

<div id='astro-ph.EP'></div>

# astro-ph.EP [[Back]](#toc)

### [43] [Efficient reduction of stellar contamination and noise in planetary transmission spectra using neural networks](https://arxiv.org/abs/2602.10330)
*David S. Duque-Castaño,Lauren Flor-Torres,Jorge I. Zuluaga*

**TL;DR**: 提出了一种基于去噪自编码器（denoising autoencoder）的无监督方法，能有效消除系外行星（TRAPPIST-1e和K2-18b类似天体）透射光谱中的恒星活动（spots和faculae）及仪器噪声，在保持分子特征的同时将大气参数反演计算效率提升3至6倍。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用去噪自编码器降低系外行星透射光谱中恒星异质性（stellar heterogeneities）污染和仪器噪声，以提高大气参数反演的可靠性。

Data: 数据来源于针对类地行星（TRAPPIST-1e类似天体）和亚海王星（K2-18b类似天体）生成的大规模合成数据集（synthetic datasets）。

Method: 采用去噪自编码器（denoising autoencoder）对大型合成数据集进行训练，通过χ²统计量评估重构质量，并在不同信噪比下优化重构性能。

Result: 降噪自编码器（denoising autoencoders）能高效消除恒星污染并保留关键分子特征，相比传统方法将计算时间缩短3至6倍，显著提升大气参数反演准确性。

Abstract: Context: JWST has enabled transmission spectroscopy at unprecedented precision, but stellar heterogeneities (spots and faculae) remain a dominant contamination source that can bias atmospheric retrievals if uncorrected. Aims: We present a fast, unsupervised methodology to reduce stellar contamination and instrument-specific noise in exoplanet transmission spectra using denoising autoencoders, improving the reliability of retrieved atmospheric parameters. Methods: We design and train denoising autoencoder architectures on large synthetic datasets of terrestrial (TRAPPIST-1e analogues) and sub-Neptune (K2-18b analogues) planets. Reconstruction quality is evaluated with the $χ^2$ statistic over a wide range of signal-to-noise ratios, and atmospheric retrieval experiments on contaminated spectra are used to compare against standard correction approaches in accuracy and computational cost. Results: The autoencoders reconstruct uncontaminated spectra while preserving key molecular features, even at low S/N. In retrieval tests, pre-processing with denoising autoencoders reduces bias in inferred abundances relative to uncorrected baselines and matches the accuracy of simultaneous stellar-contamination fitting while reducing computational time by a factor of three to six. Conclusions: Denoising autoencoders provide an efficient alternative to conventional correction strategies and are promising components of future atmospheric characterization pipelines for both rocky and gaseous exoplanets.

</details>

### [44] [Carbon from Interstellar Clouds to Habitable Worlds](https://arxiv.org/abs/2602.10308)
*Edwin A. Bergin,Marc M. Hirschmann,Andre Izidoro*

**TL;DR**: 该论文综述了碳从星际介质到行星形成过程中的迁移路径，揭示了原行星盘内碳含量梯度、碳损失机制及两种行星形成模式（卵石吸积和星子吸积）对行星碳含量的影响，指出缺乏巨行星的系统中碳质岩质行星和亚海王星可能普遍存在。


<details>
  <summary>Details</summary>

Background: 研究聚焦于碳元素从星际介质到行星形成过程中的迁移与演化机制，探讨其对类地行星和亚海王星核心宜居性的关键影响。

Data: 数据来源于星际介质（ISM）中演化恒星包层形成的有机物和稠密ISM中形成的芳香族分子，以及原行星盘中碳/硅含量梯度的矿物地球化学分析。

Method: 采用行星形成（pebble accretion）和星子吸积（planetesimal accretion）两种范式，结合压力隆起（pressure bump）早期形成对碳漂移的调控作用，分析原行星盘中碳含量梯度及其对类地行星和亚海王星核碳组成的影响。

Result: 碳架构在太阳系中的模式可能不适用于所有系统，缺乏巨行星时碳含量高的岩石行星和亚海王星可能普遍存在。

Abstract: Carbon is an essential element for a habitable world. Inner (r < 3 au) disk planetary carbon compositions are strongly influenced by supply and survival of carbonaceous solids. Here we trace the journey of carbon from the interstellar medium to the processes leading to planet formation. The review highlights the following central aspects:
  -Organics forming in evolved star envelopes are supplemented by aromatic molecules forming in the dense ISM to represent the seeds of (hydro)carbon supply through pervasive pebble drift to rocky planets and sub-Neptune cores.
  -Within the protoplanetary disk the sharp gradient in the C/Si content of Solar System bodies and mineral geochemistry outlines a tale of carbon loss from pebbles to within planetesimals and planets, and from planetary atmospheres.
  -Within two planet formation paradigms (pebble and planetesimal accretion) a range of planetary carbon content is possible that is strongly influenced by early (< 0.5 Myr) formation of a pressure bump that titrates drift.
  Overall, it is unlikely that the carbon architecture of our Solar System applies to all systems. In the absence of giant planets, carbon-rich rocky worlds and sub-Neptunes may be common. We outline observations that support their presence and discuss habitability of terrestrial worlds.

</details>

<div id='cs.GR'></div>

# cs.GR [[Back]](#toc)

### [45] [Photons x Force: Differentiable Radiation Pressure Modeling](https://arxiv.org/abs/2602.10712)
*Charles Constant,Elizabeth Bates,Santosh Bhattarai,Marek Ziebart,Tobias Ritschel*

**TL;DR**: 提出一种结合蒙特卡洛模拟（Monte-Carlo）、神经网络代理模型和优化方法的系统，用于高效优化受辐射压力（radiation pressure）影响的航天器参数化设计，显著降低计算成本并支持多目标逆向设计。


<details>
  <summary>Details</summary>

Background: 研究聚焦于航天器设计中辐射压力（radiation pressure）主导的非保守力机制优化问题，针对800公里以上轨道高度场景，解决高保真辐射压力建模计算成本过高制约大规模设计优化的瓶颈。

Data: 数据来源于高保真蒙特卡洛（Monte-Carlo）辐射压力模拟，通过并行计算、重要性采样和下一事件估计降低方差，并支持对整个设计家族而非单一航天器进行仿真。

Method: 采用计算机图形学启发的蒙特卡洛(Monte-Carlo)模拟、神经网络代理模型和可微分优化框架来高效计算辐射压力(radiation pressure)并优化航天器设计参数。

Result: 提出了一种结合蒙特卡洛模拟（Monte-Carlo simulation）、神经网络代理模型和优化方法的系统，用于高效处理辐射压力（radiation pressure）约束下的航天器参数化设计问题。

Abstract: We propose a system to optimize parametric designs subject to radiation pressure, \ie the effect of light on the motion of objects. This is most relevant in the design of spacecraft, where radiation pressure presents the dominant non-conservative forcing mechanism, which is the case beyond approximately 800 km altitude. Despite its importance, the high computational cost of high-fidelity radiation pressure modeling has limited its use in large-scale spacecraft design, optimization, and space situational awareness applications. We enable this by offering three innovations in the simulation, in representation and in optimization: First, a practical computer graphics-inspired Monte-Carlo (MC) simulation of radiation pressure. The simulation is highly parallel, uses importance sampling and next-event estimation to reduce variance and allows simulating an entire family of designs instead of a single spacecraft as in previous work. Second, we introduce neural networks as a representation of forces from design parameters. This neural proxy model, learned from simulations, is inherently differentiable and can query forces orders of magnitude faster than a full MC simulation. Third, and finally, we demonstrate optimizing inverse radiation pressure designs, such as finding geometry, material or operation parameters that minimizes travel time, maximizes proximity given a desired end-point, minimize thruster fuel, trains mission control policies or allocated compute budget in extraterrestrial compute.

</details>