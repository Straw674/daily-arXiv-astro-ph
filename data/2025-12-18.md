...
### [1] [Discovery of Isolated, Quenched, and Likely Backsplash Dwarf Galaxies near M101](https://arxiv.org/abs/2512.14808)
*Julian Shapiro*

**TL;DR**: 发现三个微弱半解析静止矮星系（Shapiro DG-I/II/III），其中两个可能是M101（距离约6.7兆秒差距）附近的后溅（backsplash）候选体，其静止恒星群和空间分布符合后溅起源特征，并发现卫星丰度与暗物质子晕（subhaloes）形成的环境密度存在潜在关联，支持ΛCDM宇宙学模型预期。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于在近邻宇宙中首次发现可能由宿主星系相互作用剥离并抛射出的回跳星系（backsplash galaxies）候选体，填补了这类天体在本地宇宙中未被明确观测到的空白。

Data: 数据来源于对三个微弱半解析静止矮星系（Shapiro DG-I、DG-II、DG-III）的光学观测，并结合了星系演化探测器（GALEX）的紫外数据缺失分析，以及利用流体动力学模拟（hydrodynamical simulations）对回溅星系（backsplash population）的理论预测。

Method: 采用流体动力学模拟（hydrodynamical simulations）分析星系环境剥离和弹射过程，并结合标准宇宙学模型（ΛCDM）检验星系群体分布。

Result: 发现了三个微弱、半分辨的静止矮星系（quiescent dwarf galaxies），其中两个可能是与M101相关的回弹星系（backsplash candidates）。
...
### [2] [HELM's deep: Highly Extincted Low-Mass galaxies seen by JWST](https://arxiv.org/abs/2512.14822)
...
**TL;DR**: 基于CEERS数据发现1361个高消光低质量星系（HELM），其异常高尘埃消光（中值Av=2mag）与投影效应无关，可能由星系相互作用等环境过程导致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星形成星系（star-forming galaxies）中尘埃含量与恒星质量（stellar mass）的正相关性，并探讨矮星系（dwarf galaxies）中异常高尘埃衰减（dust attenuation）现象的成因。

Data: 数据来源于宇宙演化早期发布科学调查（CEERS）的光学至近红外数据，通过全面的光谱能量分布拟合分析获得多参数分布用于样本选择。

Method: 基于光学至近红外数据，采用综合光谱能量分布（SED）拟合分析获得多参数分布进行样本筛选。

Result: 发现了一类具有异常高尘埃衰减（Av>1mag）的矮星系（HELM星系），其分布偏向于比非尘埃矮星系更密集的环境且恒星形成率略低，暗示环境过程（如星系相互作用）可能是其高尘埃衰减的原因。
...
### [3] [Void Galaxies and AGN Activity in ZOBOV-identified TNG300 Voids Out to z=3.0](https://arxiv.org/abs/2512.14825)
*Olivia Curtis,Bryanne McDonough,Tereasa Brainerd*

**TL;DR**: 基于TNG300模拟研究了红移z=3至z=0的宇宙空洞(void)星系，发现密度控制样本(underdensity contrasts <−0.8)的星系比非空洞星系更蓝、更小、恒星形成更活跃、质量更大且金属丰度更低，其活动星系核(AGN)比例显著更高(红移z=0时高79.8±0.4%)。


<details>
  <summary>Details</summary>

Background: 研究宇宙空洞（cosmic void）环境中星系的演化特性及其与非空洞星系的差异。

Data: 数据来源于TNG300数值模拟（simulation），通过分水岭（watershed）算法构建宇宙空洞（void）目录，并定义了四类场星系（field galaxies）样本进行分析。

Method: 采用基于分水岭（watershed）算法的空洞查找方法构建宇宙空洞目录，并定义了四类场星系（field galaxies）群体进行比较分析。

Result: 在红移$z\geq 1$时，由于分辨率限制和基于分水岭（watershed）算法的选择效应，密度控制样本与非空洞星系之间的差异变得不明显。
...
### [4] [Doubling NIRSpec/IFS capability to calibrate the single epoch black hole mass relation at high redshift](https://arxiv.org/abs/2512.14844)
...
**TL;DR**: 这篇论文通过开发JWST/NIRSpec IFU观测的数据处理技术并应用于5个z~2类星体样本，首次基于Hα和Hβ提供了高红移单epoch校准关系，发现Hβ与混响测量（RM）得出的黑洞质量（M$_{BH}$）最接近，而Hα估计值偏差较大，且高吸积率黑洞的M$_{BH}$可能被系统性高估一个数量级。


<details>
  <summary>Details</summary>

Background: 早期宇宙中大量超大质量黑洞（BHs）的发现对黑洞-宿主星系共演化框架的可靠性提出了挑战。

Data: 数据来源于JWST/NIRSpec IFU观测的5个z~2类星体样本，其黑洞质量(M$_{BH}$)通过反响映射(RM)独立测量，并利用新开发的数据处理技术扩展了有效波长覆盖范围以同时获取H$β$和H$α$发射线数据。

Method: 开发了一种JWST/NIRSpec IFU（积分场单元）观测数据处理技术，通过结合相邻色散器数据将有效波长覆盖范围扩大一倍，从而探测原本无法观测的发射线。

Result: Hβ（H beta）与基于反响映射（reverberation mapping）的黑洞质量估计最接近，而Hα（H alpha）估计值则表现出更大的离散性。
...
**TL;DR**: 发现并分析了迄今最遥远的碰撞环星系（collisional ring galaxy）GS18660（红移z=3.08），其径向膨胀速度约200 km/s的旋转盘结构和伴星系动力学证据支持碰撞成因，环上存在近期（约50 Myr内）剧烈恒星形成（SFR约100太阳质量/年）。


<details>
  <summary>Details</summary>

Background: 碰撞环星系(collisional ring galaxies)是局域宇宙中仅占约0.01%的罕见相互作用系统，其形成通常归因于大质量星系与致密卫星星系(intruder)的正面碰撞触发密度波向外传播产生特征环状形态。

Data: 数据来源于JWST/NIRSpec积分场光谱仪(IFS)观测，包括低分辨率(R~100)覆盖~0.2-1.3um静止波段的谱段和高分辨率(R~2700)覆盖0.4-0.8um静止波段的谱段，这些数据来自GA-NIFS观测计划。

Method: method：基于JWST/NIRSpec积分场光谱仪(IFS)数据，采用低分辨率(R~100)和高分辨率(R~2700)光谱分析技术，结合多波段方法推导电离气体条件和恒星族性质，并通过气体运动学分析约束碰撞时间尺度。

Result: 发现了迄今最遥远的碰撞环星系（collisional ring galaxy）GS18660（红移z=3.08），其环结构由伴星系（intruder）碰撞触发径向膨胀波（~200 km/s）形成，并伴随剧烈恒星形成活动（SFR ~100 Msun/yr）。
...
### [6] [Sparse by the River: Diverse Environments of z > 3 Massive Quiescent Galaxies](https://arxiv.org/abs/2512.14881)
...
**TL;DR**: 基于JWST的COSMOS-Web数据发现高红移(z>3)大质量静止星系(QGs)周围存在大量低质量恒星形成星系(SFGs)，且其环境密度与恒星形成星系相当，部分QGs可能通过冷气体吸积复苏，并报道了迄今最遥远低质量静止星系Saigon(z=4.55)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（$z > 3$）大质量静止星系（QGs）在早期宇宙星系形成中的关键作用及其环境特性。

Data: 数据来源于JWST（James Webb Space Telescope）在COSMOS-Web天区获取的光度红移星表（photometric-redshift catalogs），包括171个光度选出的高红移（$3\leq z\mathrm{_{phot}}<5$）大质量（$\geq10^{10}$ M$_\odot$）静止星系（QGs），以及7个光谱确认的静止星系（M$_*$$\sim$$10^9-10^{11}$ M$_\odot$）。

Method: 采用JWST（James Webb Space Telescope）的COSMOS-Web场测光红移（photometric-redshift）数据构建S$\mathrm{\hat{O}}$NG样本，并运用几何方法分析高红移（$z > 3$）静止星系（QGs）的邻居分布和丝状结构信号。

Result: 高红移（$z > 3$）大质量静止星系（QGs）周围存在大量恒星形成星系（SFGs），与低红移的星系一致性效应形成对比。
...
### [7] [Portrait of a Galaxy on FIRE: Is the $α$-bimodality a natural consequence of inside-out disc growth in a hierarchical formation scenario?](https://arxiv.org/abs/2512.14897)
...
**TL;DR**: 通过FIRE-2模拟发现，银河系质量星系中α元素双峰分布（α-bimodality）的形成无需经历重大并合或显著径向迁移，而是由早期混沌聚集阶段形成高α厚盘、小并合稀释日冕（corona）导致[Fe/H]增长停滞、以及后期小并合气体流入触发低α序列共同驱动。


<details>
  <summary>Details</summary>

Background: 研究探讨了盘星系（disc galaxies）化学二分性（chemical dichotomy）的形成机制及其与星系形成演化过程中气体流动、恒星形成、并合历史以及热冕（hot corona）化学演化的关联。

Data: 数据来源于FIRE-2模拟套件中的一个银河系质量星系的流体动力学模拟（zoom-in hydrodynamical simulation），分析了盘内气体流动、恒星形成与合并历史以及热日冕（hot corona）的化学演化。

Method: 采用FIRE-2模拟套件中的高分辨率流体动力学模拟，结合气体流动分析、恒星形成与合并历史追踪以及热日冕化学演化建模，探究无重大合并和径向迁移条件下的α双峰性驱动机制。

Result: result：模拟星系在没有经历重大并合或显著径向迁移的情况下，通过早期混沌聚集阶段形成高α厚盘，随后小并合稀释日冕（corona）导致[Fe/H]增长停滞，后期小并合气体吸积在外部形成低α序列，径向内流则供给内部低α盘，且新恒星化学丰度保留显著差异表明瞬时气体混合不成立。
...
### [8] [The diffusion coefficient in the Large Magellanic Cloud](https://arxiv.org/abs/2512.14906)
*Javier Reynoso-Cordova,Daniele Gaggero,Marco Regis,Marco Taoso*

**TL;DR**: 通过扩展DRAGON和HERMES代码构建数值模型，首次推算出大麦哲伦云(LMC)中宇宙射线电子在1 GeV能量下的平均扩散系数D0为(3-6)×10²⁸ cm²/s，略高于银河系典型值。


<details>
  <summary>Details</summary>

Background: 麦哲伦云（LMC）作为银河系最大的卫星星系，为高能天体物理和暗物质研究提供了独特实验室。

Data: 数据来源包括利用公共代码DRAGON和HERMES对LMC（Large Magellanic Cloud）进行数值模拟，并对比观测到的低频射电辐射图（low-frequency radio maps）。

Method: 采用扩展的DRAGON和HERMES代码构建了宇宙线（cosmic-ray）输运及其非热辐射（non-thermal emission）的端到端数值模型，通过同步辐射（synchrotron radiation）形态反演有效扩散系数（diffusion coefficient）。

Result: result：在1 GeV能量下测得大麦哲伦云（LMC）的有效平均扩散系数D0为(3-6)×10²⁸ cm²/s，略高于银河系的典型值。
...
**TL;DR**: 该论文通过GLOSTAR（GLObal view of STAR formation）巡天项目，利用埃费尔斯贝格（Effelsberg）100米射电望远镜在4.89GHz和6.82GHz频段对银河系平面（$-2^{\circ}< \ell <60^{\circ}$，$|b|<1.1^{\circ}$）及天鹅座X（Cygnus X）区域进行大尺度连续谱成像观测，填补了甚大阵（VLA）D阵列缺失的零间距数据，以145$^{\prime\prime}$和106$^{\prime\prime}$角分辨率提供了迄今最高质量的单天线连续谱数据，有效恢复了90%以上VLA遗漏的流量密度，显著提升了银河磁场结构测绘的可靠性，并降低了去偏振效应的影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用扩展射电连续谱辐射及其线性偏振探测银河系大尺度同步辐射和自由-自由辐射结构，填补现有高灵敏度、高角分辨率单天线巡天数据的空白。

Data: 数据来源于使用Effelsberg 100米射电望远镜对银道面$-2^{\circ}< \ell <60^{\circ}$和$|b|<1.1^{\circ}$区域以及天鹅座X（Cygnus X）区域进行的大规模连续谱成像观测，频率为4.89GHz和6.82GHz，并补充了VLA D阵列缺失的零间距数据。

Method: 采用Effelsberg 100米射电望远镜在4.89GHz和6.82GHz频段进行大尺度射电连续谱成像观测，并利用低角分辨率巡天数据校正缺失的大尺度辐射。

Result: Effelsberg连续谱巡天在4.89GHz和6.82GHz频率下有效恢复了VLA D-array遗漏的90%以上流量密度，提供了迄今该频段最高质量的单天线数据。
...
### [10] [Dark matter in ALFALFA galaxies: Investigating galaxy-halo connection](https://arxiv.org/abs/2512.15073)
*Meng Yang,Ling Zhu,Niankun Yu,Yu Lei,Runsheng Cai,Jie Wang,Zheng Zheng*

**TL;DR**: 通过对比ALFALFA巡天和TNG100模拟中2453个气体丰富的晚型星系（恒星质量范围10^8.7-10^11.4M⊙），发现ALFALFA样本在所有恒星质量区间均表现出比模拟预测更低的中位暗物质质量，且高恒星质量区间（M*>10^11M⊙）的晚型星系中有32%的暗物质质量低于10^11.5M⊙，比模拟比例（17%）更高，表明大质量晚型星系实际存在于比TNG100模拟预测更轻的暗物质晕（dark matter halo）中的比例更高。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系-暗晕关联（galaxy-halo connection），通过中性氢（H I）积分光谱分析晚型星系（late-type galaxies）的暗物质含量，探讨观测数据与TNG100数值模拟预测之间的差异。

Data: 数据来源包括ALFALFA巡天（Arecibo Legacy Fast ALFA survey）的观测样本和TNG100（IllustrisTNG-100）数值模拟的星系样本，通过恒星质量和中性氢（H I）半径的一一匹配构建了2453个富含气体的晚型星系样本。

Method: 采用基于中性氢（H I）谱线宽度测量（使用生长曲线法）和光学图像倾角校正的动力学方法推断暗物质含量。

Result: ALFALFA星系在所有恒星质量区间均表现出比TNG100模拟结果更低的中位暗物质质量，且高恒星质量区间（$M_* > 10^{11} M_\odot$）的晚型星系暗物质质量中位数比TNG100模拟低23%，表明实际观测中存在更多暗物质晕质量相对较低的大质量晚型星系。
...
### [11] [Stellar Morphology of Optically Dark or Faint Galaxies at $z>3$ with](https://arxiv.org/abs/2512.15127)
...
**TL;DR**: 研究发现JWST观测到的光学暗弱星系（OFGs）是高红移（z > 3）宇宙中因大质量和高尘埃消光（$A_{v}$）被HST遗漏的重要恒星形成星系（SFGs），其尘埃遮蔽主要源于高恒星质量而非结构差异，且演化状态与典型SFGs连续。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST观测揭示高红移(z > 3)宇宙中光学暗弱或不可见星系(OFGs)的物理特性及其对恒星形成率密度(SFR density)的重要贡献，探索其高尘埃消光(dust attenuation)成因及在早期星系演化中的作用。

Data: 数据来源于PRIMER和CEERS天区的NIRCAM/F444W成像数据，分析了65个光学暗或微弱星系（OFGs）的恒星形态。

Method: 采用近红外相机（NIRCAM）F444W波段成像数据，结合形态学参数（如有效半径$R_e$、轴比、Sérsic指数）和物理参数（如尘埃衰减$A_{v}$、恒星质量、恒星形成率面密度）的相关性分析，比较了暗弱星系（OFGs）与典型恒星形成星系（SFGs）的标度关系。

Result: OFGs（光学暗或微弱星系）主要是由于高恒星质量导致严重尘埃遮蔽的恒星形成星系（SFGs），其尘埃衰减程度（$A_{v}$）与恒星质量强相关，但与质量归一化有效半径、恒星质量面密度、Sérsic指数、轴比或恒星形成率面密度无显著依赖关系。
...
### [12] [Dual-coding contrastive learning based on ConvNeXt and ViT models for morphological classification of galaxies in COSMOS-Web](https://arxiv.org/abs/2512.15129)
*Shiwei Zhu,Guanwen Fang,Chichun Zhou,Jie Song,Zesen Lin,Yao Dai,Xu Kong*

**TL;DR**: 提出了一种基于对比学习（contrastive learning）的自监督方法升级\texttt{USmorph}框架的无监督机器学习（UML）部分，通过卷积自编码器去噪、自适应极坐标变换增强旋转不变性、双编码器卷积神经网络降维和Bagging聚类模型，在0.5 < z < 6.0红移范围内对COSMOS-Web场星系光学图像分类效率提升至73%，并与星系演化参数具有良好一致性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过自监督对比学习升级星系形态分类框架\texttt{USmorph}的无监督机器学习部分，以提升特征提取效率并适应大样本星系形态学研究需求。

Data: 数据来源于COSMOS-Web天区中红移范围在0.5到6.0之间的星系的静止波段光学图像。

Method: 采用对比学习（contrastive learning）升级了无监督机器学习框架，结合卷积自编码器（Convolutional Autoencoder）去噪、自适应极坐标变换（Adaptive Polar Coordinate Transformation）增强旋转不变性，并基于ConvNeXt和ViT的双编码器卷积神经网络进行特征提取与降维，最后采用基于Bagging的聚类模型对星系特征进行分组。

Result: 提出了一种基于对比学习（contrastive learning）的自监督方法升级了\texttt{USmorph}框架的无监督机器学习（UML）部分，通过卷积自编码器降噪、自适应极坐标变换增强旋转不变性、双编码器卷积神经网络特征降维以及基于Bagging的聚类模型，在0.5 < z < 6.0红移范围内对COSMOS-Web场星系图像实现了73%的分类成功率。
...
### [13] [An updated efficient galaxy morphology classification model based on ConvNeXt encoding with UMAP dimensionality reduction](https://arxiv.org/abs/2512.15137)
*Guanwen Fang,Shiwei Zhu,Jun Xu,Shiying Lu,Chichun Zhou,Yao Dai,Zesen Lin,Xu Kong*

**TL;DR**: 该研究通过结合预训练ConvNeXt卷积神经网络（CNN）和UMAP降维技术改进了无监督机器学习框架USmorph，在COSMOS巡天数据（0.2<z<1.2）中将99,806个星系高效分类为5种物理形态类型，分类效率提升且与星系演化理论一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用无监督机器学习（UML）提升星系形态分类效率，以支持中国空间站望远镜（CSST）等大规模巡天项目。

Data: 数据来源于红移范围0.2<z<1.2的99,806个COSMOS星系I波段图像（I_mag<25）。

Method: 采用预训练ConvNeXt卷积神经网络(CNN)进行层次特征提取，并结合UMAP(Uniform Manifold Approximation and Projection)进行非线性流形降维。

Result: 通过预训练ConvNeXt卷积神经网络(CNN)进行层次特征提取并结合UMAP非线性流形学习，实现了对星系形态的高效无监督分类。
...
### [14] [Robustness Analysis of USmorph: I. Generalization Efficiency of Unsupervised Strategies and Supervised Learning in Galaxy Morphological Classification](https://arxiv.org/abs/2512.15142)
*Shiwei Zhu,Guanwen Fang,Yao Dai,Chichun Zhou,Yirui Zheng,Jie Song,Shiying Lu,Xu Kong*

**TL;DR**: 通过系统评估混合机器学习框架USmorph（结合无监督和监督学习进行星系形态分类）的核心模块稳定性，发现其卷积自编码器（CAE）采用5×5卷积核和40维潜在表征时性能最优，自适应极坐标变换（APCT）增强旋转不变性，K=50的聚类数实现最佳分类粒度，且最终分类结果与星系演化理论预期一致，验证了该算法在中国空间站望远镜（CSST）任务中的应用潜力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于混合机器学习框架USmorph在星系形态分类中的鲁棒性系统分析。

Data: 数据来源于COSMOS天区近10万张红移范围0.2<z<1.2、I波段星等I_mag<25的星系图像。

Method: 采用卷积自编码器(CAE)结合自适应极坐标变换(APCT)的无监督特征提取方法，并在监督学习阶段使用GoogLeNet进行分类。

Result: 卷积自编码器(CAE)在采用中等网络深度、5×5卷积核和40维潜在表示时能最优保留结构信息。
...
### [15] [Spin-filament alignments to unravel galaxy evolution and model intrinsic alignments](https://arxiv.org/abs/2512.15276)
...
**TL;DR**: 这篇论文的核心贡献是提出需要新一代宽视场光谱设施来测量数百万个星系的自旋-纤维结构（spin-filament）排列关系，同时实现亚百万秒差距（sub-Mpc）分辨率的宇宙网（cosmic web）测绘和空间分辨运动学分析，以揭示星系角动量演化与宇宙网的关联并解决弱引力透镜（weak-lensing）测量中本征排列（intrinsic alignments）的系统误差问题。


<details>
  <summary>Details</summary>

Background: 研究聚焦于揭示星系角动量（angular momentum）获取与演化机制及其与宇宙网（cosmic web）的关联，同时探索星系自旋（spin）与宇宙纤维结构（cosmic filaments）的排列关系对星系演化的启示。

Data: 研究中使用的数据来源于2040年代多个全天巡天（all-sky surveys）项目，结合运动学（kinematic）信息和高完备性红移（high-completeness redshifts）数据（至少24-25星等），以及新一代宽视场光谱（wide-field spectroscopic）设施获取的百万星系自旋-纤维结构（spin-filament）排列测量数据。

Method: 采用多目标光谱（MOS）巡天技术结合运动学信息和深度红移测量，以研究星系自旋与宇宙纤维结构（cosmic filaments）的关联性。

Result: 理解星系角动量获取和演化及其与宇宙网（cosmic web）的联系是2040年代天体物理学的主要未解问题之一。
...
### [16] [First reverberation mapping of a Bowen-fluorescence line](https://arxiv.org/abs/2512.15497)
*Martin W. Ochmann,Edward M. Cackett,Lukas Diehl,Keith Horne,Malte A. Probst,Wolfram Kollatschny*

**TL;DR**: 首次在NGC 4593中探测到OI 8446（Bowen荧光）的日际尺度变化（均方根振幅约4%），确认其发射区与巴耳末线加权宽线区（BLR）距离相同，且Lyβ抽运是主要激发机制，实现了首个直接响应Lyβ通量变化的Bowen荧光线 reverberation mapping（回响映射）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用反响映射(RM)技术探索活动星系核(AGN)宽线区(BLR)的动力学特性，特别是首次针对鲍文荧光(Bowen fluorescence)线OI 8446的短时标变异性及其与莱曼β(Lyβ)泵浦机制的关联。

Data: 数据来源于2016年HST/STIS对NGC 4593的观测活动以及存档的近红外光谱。

Method: 采用回照映射（reverberation mapping）技术分析OI 8446发射线的变异性，通过计算均方根（rms）振幅并提取积分光变曲线，结合相关性分析比较其与紫外连续谱及其他发射线的响应关系，同时利用档案近红外光谱评估其激发机制。

Result: 首次探测到OI 8446在日尺度上的变化，其相对Lyα（作为Lyβ的替代）的延迟为~2.5天，且与Hα几乎同步（仅延迟~0.3天），证实其发射区与巴耳末线加权BLR距离相同，并确定Lyβ泵浦是其主导激发机制。
...
### [17] [The impact of bars on the properties of HII regions in the TIMER survey](https://arxiv.org/abs/2512.15637)
...
**TL;DR**: 通过分析17个棒旋星系（barred galaxies）中2200个电离氢区（HII regions）的物理特性，发现棒结构诱导的气体流动导致核周盘（circumnuclear regions）电离氢区的氧氮丰度（O/H、N/O）、Hα光度、尘埃消光等参数呈现极端值，且星系质量（而非棒强度）显著影响这些特性，证实棒结构通过物质重分布引起不同区域电离氢区性质的显著差异。


<details>
  <summary>Details</summary>

Background: 研究聚焦于棒旋星系（barred galaxies）中棒结构对电离气体物理特性的影响机制。

Data: 数据来源于17个棒旋星系（barred galaxies）的积分视场光谱（integral field spectroscopy）观测，覆盖中心约6×6千秒差距区域，共探测到2200个电离氢区（HII regions），其中331个位于核盘（nuclear disc）、661个位于棒区域（bar region）、1208个位于盘（disc）。

Method: 基于积分视场光谱(integral field spectroscopy)数据对17个棒旋星系(barred galaxies)中心6x6千秒差距区域内的2200个电离氢区(HII regions)进行物理参数分析，包括氧氮丰度(O/H、N/O)、Hα光度、尘埃消光、电子密度和Hα等效宽度。

Result: 棒状结构驱动的气体流动导致星系内部HII区（HII region）性质存在显著差异，核盘（nuclear disc）区域HII区表现出最极端的物理参数。
...
### [18] [The longest known tails of ram-pressure stripped star-forming galaxies are caused by an ICM shock in Abell 1367](https://arxiv.org/abs/2512.15660)
...
**TL;DR**: 研究发现邻近（92 Mpc）合并星系团Abell 1367中三个星系（UGC 6697、CGCG 097-073和CGCG 097-079）在团内介质(ICM)激波作用下产生了迄今最长的300 kpc射电剥离尾(RPS tail)，并首次探测到星系前缘存在可能由磁化丝状结构中宇宙线再加速产生的陡谱射电羽流，直接证实了星系团合并激波对成员星系演化的影响。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系团合并过程中环境（如射电剥离（RPS））对成员星系演化的影响，特别是与星系团内介质（ICM）激波的相互作用。

Data: 数据来源于LOFAR和MeerKAT在54、144、817和1270 MHz频率的射电观测，以及X射线观测追踪的星系团内介质(ICM)激波和射电遗迹(radio relic)。

Method: 采用低频阵列（LOFAR）和MeerKAT射电望远镜在54、144、817和1270 MHz频段观测数据，对星系尾迹进行详细频谱分析，并构建包含压缩和再加速效应的尾迹模型以解释极端冲压剥离（RPS）现象。

Result: 在合并星系团Abell 1367中发现了三个星系（UGC 6697、CGCG 097-073和CGCG 097-079）受到极端冲压剥离（RPS）作用的直接证据，其中UGC 6697的尾流长达300 kpc，是目前已知恒星形成星系中最长的RPS尾流，且其长度和光谱变化无法仅用剥离宇宙射线的光谱老化解释，而需结合团内介质（ICM）激波引发的压缩和再加速机制。
...
### [19] [The Milky Way in motion: gauging stellar trajectories that shape the Galactic thin disc](https://arxiv.org/abs/2512.15670)
*M. L. L. Dantas,R. Smiljanic,R. S. de Souza,P. B. Tissera,L. Magrini*

**TL;DR**: 通过精细化银河系化学演化模型和广义加性模型（GAM）分析盖亚-ESO（Gaia-ESO）恒星样本，发现75%的恒星表现出搅动（churning）主导的迁移特征（金属富集恒星优先向外迁移，贫金属恒星向内迁移），其余则主要受模糊（blurring）或未扰动影响，且迁移模式随化学丰度群组变化可能与银河系棒和旋臂相互作用有关。


<details>
  <summary>Details</summary>

Background: 研究探讨银河系恒星轨道演化机制（churning和blurring）对盖亚-ESO巡天样本动力学演化的主导作用，并揭示化学丰度分层下的径向迁移特征。

Data: 数据来源包括盖亚(Gaia)卫星的视差和自行数据，以及Galpy软件包计算的动力学参数。

Method: 采用广义加性模型(GAM)构建精细网格的银河系化学演化模型，结合盖亚(Gaia)视差和自行运动数据及Galpy动力学参数，通过分层聚类对21种化学丰度进行分组分析。

Result: 金属丰度分层样本显示，富金属恒星优先向外迁移而贫金属恒星向内迁移，约75%恒星呈现搅动(churning)主导的迁移特征，其余主要受模糊(blurring)影响或未受扰动。
...
### [20] [Structural Limitations on Constraining the Time Evolution of Dark Energy](https://arxiv.org/abs/2512.14747)
*Seokcheon Lee*

**TL;DR**: 研究发现暗能量状态方程（dark energy equation of state）ω(z)的观测约束受限于红移空间积分结构的低通滤波特性，仅能探测缓慢变化的模式，而快速或局域时间变化被不可逆抑制。


<details>
  <summary>Details</summary>

Background: 研究揭示了弗里德曼-勒梅特-罗伯逊-沃克宇宙学(FLRW cosmology)中基于距离探针的结构性限制，即暗能量状态方程(equation of state)的时间变化观测存在固有低通滤波效应。

Data: 研究中使用的数据来源为基于距离探针（distance-based probes）的宇宙学观测数据。

Method: 采用积分结构分析暗能量状态方程（equation of state）ω(z)对宇宙学可观测量的影响，揭示其作为红移空间内禀低通滤波器的特性。

Result: 暗能量状态方程（dark energy equation of state）的时间变化性观测受到积分结构的根本限制，导致仅能探测少量缓慢变化的模式。
...
### [21] [FrankenStat I: a New Approach to Pulsar Timing Array Data Combination](https://arxiv.org/abs/2512.14807)
*David Wright,Kalista Wayt,Jeffrey S. Hazboun,Xavier Siemens,Rutger van Haasteren,Levi Schult,Stephen R. Taylor*

**TL;DR**: 该研究提出了一种名为FrankenStat的高效普适性数据整合方法，用于脉冲星计时阵(PTA)引力波背景探测，在保持与传统方法相当灵敏度与参数约束能力的同时将完整数据处理时间缩短至几分钟。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是探索脉冲星计时阵列（PTA）在探测随机引力波背景（stochastic gravitational wave background）中的应用及其数据整合方法的优化。

Data: 研究中使用的数据来源包括国际脉冲星计时阵列（IPTA）各区域合作组织的独立数据集以及模拟数据。

Method: 采用核心分析方法为弗兰肯统计法（FrankenStat），通过非相干合并各PTA数据集并忽略相同脉冲星不同数据集间的互相关，实现高效全数据组合。

Result: 开发了一种名为FrankenStat的高效通用数据组合方法，其灵敏度与参数约束能力与传统方法相当但耗时仅需数分钟。
...
### [22] [Constraining Fifth Forces using the Local Distance Ladder: Implications for the Hubble Tension](https://arxiv.org/abs/2512.14814)
*Marcus Högås,Edvard Mörtsell,Harry Desmond,Adam Riess*

**TL;DR**: 这篇论文通过重新校准SH0ES团队的造父变星-超新星距离阶梯（Cepheid-supernova distance ladder）并引入第五力（fifth force）对造父变星周光关系（period-luminosity relation）的影响，发现第五力在屏蔽模型（screened models）中被强烈约束且后验集中于零结果，推断哈勃常数（Hubble constant）为73.1±1.0 km/s/Mpc，表明哈勃张力（Hubble tension）仍大于5σ。


<details>
  <summary>Details</summary>

Background: 研究探讨了在存在第五种力（fifth force）的长程相互作用修正引力模型中，对哈勃常数（Hubble constant）局域距离阶梯测量的影响。

Data: 数据来源于SH0ES团队的造父变星-超新星距离阶梯（Cepheid-supernova distance ladder）校准数据以及四个独立的红巨星分支顶端（TRGB）距离数据集。

Method: 采用三种环境依赖参量（外部引力势$Φ$、加速度$a$和曲率$K$）建模第五力（fifth force）对造父变星周光关系的影响，并重新校准SH0ES团队的造父变星-超新星距离阶梯（distance ladder）。

Result: 在考虑第五力（fifth force）的屏蔽模型后，对哈勃常数（Hubble constant）的局域距离阶梯测量结果与标准模型一致，未显著缓解哈勃张力（Hubble tension）。
...
### [23] [BASILISK IV. No $S_8$ Tension with Satellite Kinematics](https://arxiv.org/abs/2512.14889)
*Kaustav Mitra,Frank C. van den Bosch,Josephine Baggen,Johannes U. Lange*

**TL;DR**: 提出了一种利用星系红移巡天数据最小尺度信息探测$S_8$张力的新方法（Basilisk），通过精确约束星系-晕关系并自洽预测星系光度函数，在考虑重子效应且避免晕组装偏差的情况下，基于SDSS-DR7数据测得$Ω_{\rm m}=0.324±0.012$、$σ_8=0.775±0.063$和$S_8=0.81±0.05$，与普朗克宇宙微波背景观测结果一致，未发现$S_8$张力证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用星系红移巡天数据的最小尺度信息开发新技术，以探究宇宙学参数$S_8$张力问题。

Data: 数据来源于SDSS-DR7（Sloan Digital Sky Survey Data Release 7）的光谱数据以及用于验证方法的SDSS类模拟数据（SDSS-like mock data）。

Method: 采用贝叶斯分层工具Basilisk对光谱数据中卫星星系运动学和丰度进行正向建模，精确约束星系-晕（galaxy-halo）关系，并通过自洽性要求预测正确星系光度函数以限制晕质量函数和宇宙学参数，同时考虑重子效应并避免晕组装偏差（halo assembly bias）问题。

Result: 基于SDSS-DR7数据，利用贝叶斯分层工具Basilisk对卫星星系运动学和丰度进行正向建模，精确约束了星系-晕（galaxy-halo）关系，并自洽地预测了星系光度函数，进而推断出宇宙学参数Ω_m=0.324±0.012、σ_8=0.775±0.063及S_8=0.81±0.05，与普朗克（Planck）宇宙微波背景辐射观测结果一致，未发现S_8张力（S_8 tension）迹象。
...
### [24] [BBNet: accurate neural network emulator for primordial light element abundances](https://arxiv.org/abs/2512.15266)
*Fan Zhang,Hang Diao,Bohua Li,Joel Meyers,Paul R. Shapiro*

**TL;DR**: 开发了基于深度学习的快速精确模拟器BBNet，能高效预测原初氦-4和氘丰度，计算速度比传统方法提升万倍且无偏差。


<details>
  <summary>Details</summary>

Background: 研究主题为利用深度学习模拟器（BBNet）快速精确预测大爆炸核合成（Big-Bang Nucleosynthesis, BBN）产生的原初轻元素丰度，以解决传统数值计算在宇宙学大规模推断中的计算瓶颈问题。

Data: 训练数据来源于两个公开的BBN代码（PArthENoPE和AlterBBN）进行的完整数值计算，这些代码经过修改以支持包含暗辐射（dark radiation）和刚性状态方程（stiff equation of state）的扩展宇宙学模型。

Method: 采用残差多头架构（residual multi-head architecture）的深度学习模拟器BBNet，基于PArthENoPE和AlterBBN两种公开BBN代码生成的数据进行训练，以快速准确模拟原初氦-4和氘丰度。

Result: BBNet是一种快速准确的深度学习模拟器，能在毫秒级时间内以可忽略误差预测原始氦-4和氘丰度，相比第一性原理求解器加速达10^4倍且在大参数范围内无偏差。
...
### [25] [Ultralight Dark Matter Constraints from NanoHertz Gravitational Waves](https://arxiv.org/abs/2512.15292)
*Shreyas Tiruvaskar,Russell Boey,Richard Easther,Chris Gordon\*

**TL;DR**: 研究发现超轻暗物质（ULDM）通过增加超大质量黑洞双星（SMBH）的阻力缩短其并合时间并抑制低频引力辐射，基于脉冲星计时阵列（PTA）数据约束ULDM粒子质量为7×10⁻²²电子伏特且其占比与现有模型兼容。


<details>
  <summary>Details</summary>

Background: 研究探讨了超轻暗物质（ULDM）对超大质量黑洞（SMBH）并合及其产生的随机引力波背景的影响。

Data: 数据来源于当前脉冲星计时阵列（pulsar timing array, PTA）的测量结果。

Method: 采用半解析模型（semi-analytic models）分析超轻暗物质（ULDM）晕中双超大质量黑洞（SMBH）的轨道衰减，并结合脉冲星计时阵（PTA）数据约束ULDM粒子质量和宇宙暗物质占比。

Result: 超轻暗物质（ULDM）通过增加双超大质量黑洞（SMBH）的阻力缩短了并合时间，并可能抑制低频引力辐射。
...
### [26] [Beyond Stage IV: Quasar and Galaxy Clustering and the Fundamental Physics of the 2040s](https://arxiv.org/abs/2512.15294)
...
**TL;DR**: 下一代宽视场光谱巡天通过同时观测数千个星系、类星体和发射线星系（$1 < z < 6$），将实现绝对中微子质量（$Σm_ν\lesssim 0.015\,\mathrm{eV}$）、原初非高斯性（$f_{\mathrm{NL}} \sim 1$）和结构增长（$fσ_8(z)$）的突破性测量，解决中微子质量等级、多场暴胀和引力修正等关键问题。


<details>
  <summary>Details</summary>

Background: 下一代宽视场光谱巡天旨在通过观测红移范围1到6的数千个星系、类星体和发射线星系，解决当前第四阶段巡天无法触及的中微子质量绝对测量（Σm_ν）、原初非高斯性（f_NL）探测以及引力理论检验等基础物理问题。

Data: 研究中使用的数据来源于下一代宽视场光谱巡天（next-generation wide-field spectroscopic surveys）对红移范围1 < z < 6的数千个星系、类星体和发射线星系（emission-line galaxies）的同时观测。

Method: 采用下一代宽视场光谱巡天技术，结合革命性光谱复用（spectroscopic multiplexing）能力（$\mathcal{O}(1000)$同时观测）、亚$2\times10^{-4}(1+z)$红移精度及场级推断（field-level inference）技术，利用高阶聚类统计（higher-order clustering statistics）分析。

Result: 下一代宽视场光谱巡天（Wide-field Spectroscopic Telescope）将实现绝对中微子质量（Σm_ν）约束达0.015电子伏特、探测f_NL~1水平的原初非高斯性，并通过fσ_8(z)测量约束暗能量和引力理论。
...
### [27] [First measurement of the Hubble constant from gravitational wave-galaxy cross-correlations](https://arxiv.org/abs/2512.15380)
*Isabela Santiago de Matos,Charles Dalang,Tessa Baker,Raul Abramo,João Ferri,Miguel Quartin*

**TL;DR**: 首次利用星系与引力波（GW）的互相关（cross-correlation）通过峰值法（Peak Sirens method）测量哈勃常数（$H_0$），基于GWTC-3.0和GLADE+数据在5.9σ置信度下检测到互相关峰值，得出$H_0 = 67^{+18}_{-15}$ km s$^{-1}$Mpc$^{-1}$并首次观测约束引力波偏差（GW bias）$b_{\rm gw} < 4.3$。


<details>
  <summary>Details</summary>

Background: 研究首次通过星系与引力波（GW）的互相关测量哈勃常数（H0），并应用峰值警报（Peak Sirens）方法探索宇宙学参数。

Data: 数据来源于GWTC-3.0引力波事件目录中的GW190814和GW170817事件，以及GLADE+星系目录。

Method: 采用峰值警报（Peak Sirens）方法分析星系红移（z）与引力波（GW）光度距离（DL）的三维角交叉谱Cℓ(z,DL)峰值。

Result: 首次通过星系与引力波（GW）的互相关（cross-correlation）并应用峰值警报（Peak Sirens）方法测量哈勃常数（$H_0$），得到$H_0 = 67^{+18}_{-15}$ km s$^{-1}$Mpc$^{-1}$，并首次观测约束引力波偏差（GW bias）$b_{\rm gw} < 4.3$（95%置信区间）。
...
### [28] [Comparing cosmic shear nulling methods for Stage-IV surveys](https://arxiv.org/abs/2512.15604)
*Naomi Clare Robertson,Alex Hall*

**TL;DR**: 该研究比较了三种抑制重子反馈对宇宙剪切（cosmic shear）测量影响的归零（nulling）策略（BNT变换、LUnul方法和交叉相关法），发现它们能有效减少宇宙学参数（如$S_8$和暗能量参数）的偏差，其中LUnul方法偏差抑制效果最强但精度降低，BNT方法保留更多信息且理论更严谨，交叉相关法需额外星系成团信息但对$S_8$偏差抑制有效。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过空化策略（nulling strategies）降低重子反馈（baryon feedback）对宇宙剪切（cosmic shear）测量的影响，以提升宇宙学参数约束的准确性。

Data: data来源包括透镜场（lensing field）的Bernardeau-Nishimichi-Taruya (BNT)变换数据、透镜两点统计量（lensing two-point statistics）的LU分解（LUnul）离散化Limber积分数据，以及利用相关大尺度结构（LSS）示踪物（cross-correlation）抑制低红移贡献的星系聚类（galaxy clustering）附加信息。

Method: 采用三种空值化（nulling）策略：对透镜场进行Bernardeau-Nishimichi-Taruya（BNT）变换、对透镜两点统计量应用离散化Limber积分LU分解（LUnul）新方法、以及利用相关大尺度结构（LSS）示踪物抑制低红移贡献的交叉相关（cross-correlation）方法。

Result: nulling技术能显著减小$S_8$和暗能量参数的偏差，其中cross-correlation方法需结合星系成团信息，LUnul方法在高$k_{\rm max}$时偏差抑制最有效但精度降低，BNT变换保留了更多信息且理论基础更严谨。
...
### [29] [The art of simulating the early Universe. Part II](https://arxiv.org/abs/2512.15627)
...
**TL;DR**: 提出非正则场论（non-canonical field theory）的格点模拟方法，包括非最小耦合标量场、轴子（ALP）与规范场相互作用、特殊场构型设置及高维标量场离散化，并扩展了引力波（GW）格点动力学，为CosmoLattice v2.0提供理论基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于非规范场论（non-canonical field theory）的格点模拟技术，特别是非最小耦合标量场、轴子类粒子（ALP）与规范场的相互作用以及高维标量场动力学。

Data: 研究中使用的数据来源于数值模拟（simulation）方法，具体通过CosmoLattice v2.0软件实现非标准场论（non-canonical field theory）和引力波（gravitational wave, GW）动力学的格点（lattice）模拟。

Method: 采用格点场论（lattice field theory）技术模拟非正则场论（non-canonical field theory）情形，包括非最小耦合引力标量场（$φ^2R$）、非正则标量动能理论（$\mathcal{G}_{ab}(\lbraceφ_c\rbrace)\partial_μφ^a\partial^μφ^b$）及轴子样粒子（ALP）与阿贝尔规范场（$φF_{μν}\tilde F^{μν}$）的相互作用，并扩展至$d + 1$维标量场离散化及引力波（GW）格点动力学实现。

Result: 该论文提出了非正则场论（non-canonical field theory）情形下的格点模拟技术，包括非最小耦合标量场、轴子类粒子（ALP）与阿贝尔规范场的相互作用、特殊场构型设置方法以及d+1维标量场动力学离散化，并讨论了格点上引力波（GW）动力学的实现，为CosmoLattice v2.0的非正则场论和引力波动力学模块提供了理论基础。
...
### [30] [Note on bulk viscosity as an alternative to dark energy](https://arxiv.org/abs/2512.15633)
*P. P. Avelino,A. R. Gomes,D. A. Tamayo*

**TL;DR**: 研究发现，体黏度（bulk viscosity）作为解释宇宙早期和晚期加速膨胀的潜在机制，在两种物理场景（非最小相互作用流体和理想气体弹性碰撞）中均无法产生足够负的有效压力驱动宇宙加速而不损害模型物理可行性，且前者会违反所有经典能量条件而后者状态方程参数限于[0,1/3]区间。


<details>
  <summary>Details</summary>

Background: 研究探讨了体黏滞（bulk viscosity）作为解释宇宙早期和晚期加速膨胀潜在机制的物理可行性及其在广义相对论框架下的局限性。

Data: 研究中使用的数据来源为理论推导（simulation），未涉及实际观测数据。

Method: 采用非最小耦合（nonminimal interactions）双流体模型和理想气体弹性碰撞（elastic collisions）模型分析体黏滞（bulk viscosity）对宇宙加速膨胀的影响。

Result: 体黏滞（bulk viscosity）无法通过两种流体非最小耦合或理想气体弹性碰撞机制产生足够负的有效压力驱动宇宙加速膨胀，且前者需违背所有经典能量条件，后者则限制状态方程参数在[0,1/3]区间。
...
### [31] [The Galactic White Dwarf Population](https://arxiv.org/abs/2512.14763)
...
**TL;DR**: 欧洲航天局盖亚任务（ESA Gaia）显著扩展了对白矮星（white dwarf）群体的认知，揭示了赫罗图（Hertzsprung-Russell diagram）中新的结构特征，并强调未来光谱观测对解决白矮星大气成分、结晶化及磁化等未解问题的关键作用。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用盖亚(Gaia)任务数据及欧洲南方天文台(ESO)设施的光谱观测，解决白矮星(white dwarf)的大气成分、光谱演化、结晶化、磁性和并合驱动路径等天体物理问题。

Data: 数据来源包括ESA Gaia任务提供的观测数据，以及当前和未来ESO（欧洲南方天文台）设施（如多光纤摄谱仪）在10-15米级望远镜上获取的光谱数据。

Method: 采用高精度光谱分析（spectroscopic characterization）结合欧洲南方天文台（ESO）设施获取有效温度（effective temperature）和表面重力（surface gravity），以推导白矮星质量、冷却年龄和光度等基本参数。

Result: ESA Gaia任务揭示了白矮星(white dwarf)赫罗图(Hertzsprung-Russell diagram)中前所未有的结构，并提出了关于大气成分、光谱演化、结晶化、磁性和并合驱动路径等新问题。
...
### [32] [Serendipitous and targeted mm/sub-mm transient searches with wide-FOV telescope](https://arxiv.org/abs/2512.14768)
*Karri Koljonen,Claudio Ricci,Thomas Stanke,Doug Johnstone,Atul Mohan,Francisco Montenegro-Montes,John Orlowski-Scherer*

**TL;DR**: 研制大视场高灵敏度毫米/亚毫米波（mm/sub-mm）观测设备可系统性探索该波段暂现源（transient）和变源（variable phenomena），填补射电与光学/红外波段间的观测空白，揭示喷流（jet launching）、相对论性激波（relativistic shocks）和吸积流（accretion flows）等被遮蔽的高能过程。


<details>
  <summary>Details</summary>

Background: 毫米/亚毫米波段(mm/sub-mm)天空是研究瞬变和变源现象的丰富但尚未充分探索的前沿领域。

Data: 数据来源于毫米/亚毫米波（mm/sub-mm）波段的大视场、高灵敏度仪器观测，用于系统巡天监测和快速响应多信使警报（如引力波触发）的大定位区域跟踪。

Method: 采用毫米/亚毫米波（mm/sub-mm）宽视场高灵敏度仪器进行多波段快速巡天监测和瞬变源追踪观测。

Result: 毫米/亚毫米(mm/sub-mm)波段观测填补了射电与光学/红外波段之间的光谱间隙，可直接探测其他波段无法观测的遮蔽环境。
...
### [33] [White Dwarfs in Wide Binary Systems as Reliable Age Calibrators](https://arxiv.org/abs/2512.14769)
...
**TL;DR**: 论文提出利用大质量白矮星（massive white dwarfs，质量≥0.7太阳质量）的冷却年龄作为精确恒星年龄标尺，通过忽略其主序前身星寿命来规避初始-终结质量关系（initial-to-final mass relation）的不确定性，并阐述了观测这类天体的仪器需求。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用大质量白矮星（white dwarfs）的冷却特性解决恒星年龄测定中的不确定性，特别是针对初始-最终质量关系（initial-to-final mass relation）约束不足的问题。

Data: 研究中使用的数据来源于对银河系内大质量白矮星（massive white dwarfs）的后续观测，这些观测需要专门的设施以确保足够的数据质量。

Method: 采用大质量白矮星（massive white dwarfs）样本并忽略主序前身星寿命的影响，通过白矮星冷却时间（white dwarf cooling time）提供精确年龄估计。

Result: 白矮星（white dwarfs）与主序伴星组成的宽双星系统可提供精确年龄估计，尤其大质量白矮星（>~0.7 Msun）因主序前身星寿命可忽略而显著降低年龄测定不确定性。
...
### [34] [The Emergence of Prebiotic Chemistry in the ISM](https://arxiv.org/abs/2512.14772)
...
**TL;DR**: 研究发现星际介质(ISM)中复杂有机分子(如氨基酸、糖类和核酸碱基)可能通过彗星和陨石将生命前体物质输送到地球。


<details>
  <summary>Details</summary>

Background: 研究探讨星际介质(ISM)中复杂有机分子（如氨基酸、糖类和核酸碱基）的化学演化及其与地球生命起源的潜在关联。

Data: 数据来源于大视场单天线射电望远镜（single-dish telescope）的多波段高灵敏度观测。

Method: 采用大视场、高灵敏度单天线射电望远镜（single-dish telescope）结合多波段观测技术探测星际介质（ISM）中复杂有机分子的化学极限。

Result: 星际介质（ISM）中存在可能构成生命基本组分的复杂有机分子（如氨基酸、糖类或RNA/DNA碱基），可通过彗星和陨石输送至地球。
...
### [35] [The Future of Evolved Planetary Systems](https://arxiv.org/abs/2512.14774)
...
**TL;DR**: 该论文提出利用白矮星(white dwarfs)作为研究系外行星系统演化末期和组成成分的独特实验室，并规划了欧洲南方天文台(ESO)未来设施需具备的光谱覆盖、灵敏度等关键技术特征，以实现对数千个瓦解星子(planetesimals)成分的无偏普查及其与银河系族群关联的研究。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用白矮星（white dwarfs）作为独特实验室，探索系外行星系统演化末期阶段及其化学多样性，为理解行星形成和演化机制提供关键线索。

Data: 数据来源包括欧空局盖亚任务（ESA Gaia）和维拉·C·鲁宾天文台（Vera C. Rubin Observatory）的巡天数据，以及欧洲南方天文台（ESO）下一代光谱设施的观测数据。

Method: 结合欧洲南方天文台（ESO）下一代光谱设施与盖亚（Gaia）和薇拉·鲁宾天文台（Vera C. Rubin Observatory）的大规模巡天数据，采用宽光学至近红外覆盖、蓝光高灵敏度、多分辨率能力、大规模多目标观测及时域响应等关键技术手段。

Result: 白矮星（white dwarfs）为研究系外行星系统演化末期及行星物质体成分提供了独特的高灵敏度实验室。
...
### [36] [Sub-millimeter wavelength protostellar accretion rate monitoring with AtLAST](https://arxiv.org/abs/2512.14777)
...
**TL;DR**: 通过论证需要大型地基亚毫米波单天线望远镜（sub-millimeter single-dish facility）来监测大量原恒星（protostar）样本的吸积光度变化，揭示其吸积过程以爆发（major bursts）或准稳态（quasi-steady）方式发生。


<details>
  <summary>Details</summary>

Background: 恒星形成（star formation）是天体物理学中关于原恒星（protostar）吸积过程（accretion processes）及其时间演化的基础问题。

Data: 研究中使用的数据来源于地面大型亚毫米波单天线望远镜（sub-millimeter single-dish facility）的宽视场（wide FoV）观测。

Method: 采用地面大型亚毫米波单天线望远镜（sub-millimeter single-dish）结合宽视场（wide FoV）技术进行长期监测。

Result: 监测原恒星（protostar）样本的吸积光度（accretion luminosity）长期变化需依赖具备大视场（FoV）的地面亚毫米波单天线设备。
...
### [37] [Why the Northern Hemisphere Needs a 30-40m Telescope and the Science at Stake: Shaping Galaxies and Their Stars with Stellar Population Gradients, IMF Variations and Environmental Drivers in Cluster Early-Type Galaxies](https://arxiv.org/abs/2512.14780)
...
**TL;DR**: 论文核心贡献是提出利用早型星系(ETGs)在4倍有效半径($\sim4R_e$)内的恒星种群梯度、化学丰度模式、初始质量函数(IMF)变化和结构特征，结合30米级望远镜的深空光谱观测，揭示其早期耗散性恒星形成、后续吸积合并及内部反馈过程的演化历史。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用早型星系（ETGs）的恒星种群梯度、化学丰度模式、初始质量函数（IMF）变化及结构特征等观测特征，揭示其形成与演化历史。

Data: 数据来源包括对室女座(Virgo)、英仙座(Perseus)和后发座(Coma)星系团的观测，以及通过30米级望远镜获取的大半径高信噪比空间分辨U波段-光学-近红外光谱数据。

Method: 采用高信噪比、空间分辨的U波段-光学-近红外光谱技术（U-band--optical--near-IR spectroscopy）在大半径范围内（$\sim4R_e$）测量恒星初始质量函数（IMF）变化、化学丰度模式和结构特征。

Result: 在较大半径（$\sim4R_e$）处测量的恒星初始质量函数（IMF）变化、化学丰度模式和结构特征为研究早型星系（ETGs）的形成和演化历史提供了关键诊断依据。
...
### [38] [Why the Northern Hemisphere Needs a 30-40 m Telescope and the Science at Stake: Cosmology and High-z Universe](https://arxiv.org/abs/2512.14781)
...
**TL;DR**: 建设北半球30-40米级望远镜（ELT/TMT）能通过多目标观测能力、仪器多样性和快速响应机制，为研究宇宙组成、第一代恒星形成及超大质量黑洞（supermassive black holes）等关键问题提供不可替代的观测支持，并推动多信使（multi-messenger）和时域天文学（time-domain Astronomy）时代的基础物理突破。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用30-40米级望远镜实现全天覆盖，以解决天体物理学、宇宙学和物理学中关于宇宙组成、第一代恒星及超大质量黑洞（supermassive black holes）形成等核心问题。

Data: 研究中使用的数据来源为下一代30-40米级地基望远镜（如ELT/TMT）的观测数据，并结合全天空巡天设施（如JWST、Euclid和Roman空间望远镜）的多波段协同观测。

Method: 采用30-40米级望远镜（ELT/TMT-like）进行北天区全覆盖观测，结合多目标观测能力、多样化仪器配置、快速响应机制及统计效能提升等核心技术手段。

Result: 30-40米级望远镜的全天覆盖对解决天体物理学、宇宙学和物理学中的基本问题至关重要。
...
### [39] [Why the Northern Hemisphere Needs a 30-40 m Telescope and the Science at Stake. How do Planetary Systems Form?](https://arxiv.org/abs/2512.14782)
...
**TL;DR**: 论文主张在北半球建造30-40米望远镜以获取原行星(protoplanets)和原行星盘(protoplanetary disks)的衍射极限成像，弥补现有设施对部分关键恒星形成区观测不足的缺陷，从而建立行星形成过程的统计普查并保持欧洲在该领域的领先地位。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过观测年轻恒星周围的原行星盘（protoplanetary disks）来探测和表征原行星（protoplanets），以填补成熟系外行星与盘结构多样性之间的关键观测空白。

Data: 研究中使用的数据来源包括欧洲极大望远镜（ELT）、下一代甚大天线阵（ngVLA）、盖亚卫星（Gaia）等设施获取的衍射极限成像观测数据。

Method: 采用衍射极限成像（diffraction-limited imaging）技术结合北半球30-40米望远镜观测，对原行星（protoplanets）和原行星盘（protoplanetary disks）进行高分辨率成像分析。

Result: 在北半球建造一台30-40米望远镜对获取原行星(protoplanets)和原行星盘(protoplanetary disks)的衍射极限成像至关重要，可填补当前对行星形成过程观测的空白。
...
### [40] [Why the Northern Hemisphere Needs a 30-40m Telescope and the Science at Stake: Ultra-Low-Mass Dwarf Galaxies Across the Boreal Cosmic Web](https://arxiv.org/abs/2512.14783)
...
**TL;DR**: 这篇论文的核心贡献是提出利用30-40米级望远镜对矮星系(dwarf galaxies)进行系统性光谱普查，以揭示极低恒星质量($M_\star \sim 10^{5}-10^{7} M_\odot$)下暗物质(dark-matter)物理、早期宇宙反馈(early-Universe feedback)和环境淬灭(environmental quenching)的作用机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于矮星系（dwarf galaxies）在宇宙学中的关键作用，探讨其在反馈（feedback）、再电离（reionization）及环境（environment）影响下的形成机制，并强调其在暗物质（dark-matter）物理和早期宇宙反馈模型中的测试价值。

Data: 数据来源包括北半球30-40米级望远镜搭载的多路复用光学积分视场光谱仪（multiplexed optical integral-field spectrograph）对后发座星系团（Coma Cluster）的深度巡天观测，以及针对不同环境（星系团、星系群、纤维状结构、低密度区域）中矮星系的定向观测。

Method: 采用多路复用光学积分场光谱仪（multiplexed optical integral-field spectrograph）对矮星系进行空间分辨光谱普查，结合深场观测（deep survey）和针对性观测（targeted observations）分析恒星形成历史、化学增丰及内部运动学。

Result: 北部30-40米级望远镜配备多路复用光学积分视场光谱仪可对$M_\star \sim 10^{5}-10^{7} M_\odot$的矮星系（dwarf galaxies）进行系统性空间分辨光谱普查，跨越多种环境直接检验暗物质物理、早期宇宙反馈（early-Universe feedback）和环境淬灭（environmental quenching）模型。
...
### [41] [Why the Northern Hemisphere Needs a 30-40 m Telescope and the Science at Stake: A Low Surface Brightness Science Case](https://arxiv.org/abs/2512.14784)
...
**TL;DR**: 论文核心贡献是论证30-40米级望远镜（配备自适应光学系统）对探测低表面亮度星系（LSB galaxies, μV≳27 mag/arcsec²）的关键作用，填补星系演化、暗物质本质等理论空白。


<details>
  <summary>Details</summary>

Background: 研究聚焦于揭示低表面亮度（LSB）宇宙在星系演化、等级成团及暗物质本质等天体物理核心问题中的关键作用。

Data: 研究中使用的数据来源为下一代30至40米级望远镜（30 to 40m class telescope）在可见光波段（$μ_V\gtrsim 27$ mag/arcsec$^2$）通过自适应光学（adaptive optics）增强的观测数据，重点覆盖北半球（Northern Hemisphere, NH）邻近宇宙的关键结构及密集环境区域。

Method: 采用30至40米级望远镜结合自适应光学（adaptive optics）技术，通过前所未有的灵敏度和空间分辨率探测低表面亮度（LSB）天体。

Result: 低表面亮度星系(LSB)是理解星系演化、等级成团及暗物质本质的关键未探索领域。
...
### [42] [Why the Northern Hemisphere Needs a 30-40m Telescope and the Science at Stake: from Interstellar Visitors to Planetary Defence](https://arxiv.org/abs/2512.14785)
...
**TL;DR**: 论文核心贡献是论证北半球30-40米级望远镜对实现全天覆盖观测的必要性，以充分发掘2030-2050年间发现的小型太阳系天体（包括星际天体、潜在威胁小行星、海外天体及空间任务目标）的科学价值。


<details>
  <summary>Details</summary>

Background: 太阳系小天体（SSOs）承载着太阳系原行星盘的物理、化学和动力学特征信息。

Data: 研究中使用的数据来源为即将开展的巡天观测（upcoming surveys）发现的太阳系小天体（SSOs）以及后续利用极大望远镜（ELT）和北半球30-40米级望远镜对这些目标的观测数据。

Method: 采用北半球30-40米级望远镜（Northern 30-40m telescope）进行全天空覆盖观测，重点针对瞬时目标（targets of opportunity）、星际天体（interstellar objects）、潜在威胁小行星（potentially hazardous asteroids）、海外天体（trans-Neptunian objects）及空间任务目标实施高灵敏度与高分辨率跟踪观测。

Result: 太阳系小天体（SSOs）保存了太阳原行星盘的物理、化学和动力学特征，而北半球30-40米级望远镜对实现全天覆盖、充分利用2030-2050年代小天体发现（包括星际天体、潜在威胁小行星、远海外天体及空间任务目标）至关重要。
...
### [43] [Why the Northern Hemisphere Needs a 30-40 m Telescope and the Science at Stake: Mapping formation pathways of nuclear star clusters across galaxies](https://arxiv.org/abs/2512.14786)
...
**TL;DR**: 提出利用30米级北方望远镜首次实现核星团(NSC)空间分辨系统性观测，揭示其形成路径与中心大质量黑洞的未知关联。


<details>
  <summary>Details</summary>

Background: 核星团（NSCs）作为星系中心致密恒星系统，其形成演化与宿主星系动力学状态及中心大质量黑洞存在潜在关联，是研究星系演化和中等质量黑洞（IMBHs）形成的关键探针。

Data: 数据来源于当前最大望远镜（very large telescopes）上最高分辨率仪器对5 Mpc内星系的空间分辨观测，以及拟议中利用北半球30米级望远镜（30-m-class telescope）开展的首个系统性无偏普查。

Method: 利用30米级北半球望远镜开展首次全面空间分辨巡天，结合高分辨率光谱观测技术解析核星团(NSCs)动力学特性。

Result: 核星团（NSCs）是位于星系中心的致密恒星系统，其形成路径及与中心大质量黑洞的未知联系有待揭示。
...
### [44] [Science Enabled by a 30-Meter-Class Telescope in the Northern Hemisphere: Massive Stars at Low Metallicity](https://arxiv.org/abs/2512.14787)
...
**TL;DR**: 该论文指出大质量恒星（massive stars）是理解高红移星系和暂现现象的关键，但现有模型仅基于局部宇宙环境存在较大不确定性，强调需通过下一代望远镜将其研究扩展至早期宇宙代表性条件。


<details>
  <summary>Details</summary>

Background: 大质量恒星（massive stars）的演化及其对宇宙再电离时期（reionization epoch）和星系化学演化的影响构成当前天体物理研究的核心科学主题。

Data: 研究中使用的数据来源包括银河系及邻近星系的大质量恒星（massive stars）观测数据，以及通过下一代望远镜（next generation of telescopes）获取的代表早期宇宙环境条件的观测数据。

Method: 采用银河系及邻近星系大质量恒星（massive stars）种群特征进行外推建模，结合新一代望远镜观测技术拓展至早期宇宙环境。

Result: 大质量恒星（massive stars）的研究是理解高红移星系和瞬变现象的关键基础，但现有模型仅在局部环境中经过验证，外推至早期宇宙条件时存在显著不确定性。
...
### [45] [Skykatana: a scalable framework to construct sky masks for the Vera Rubin Observatory and large astronomical surveys](https://arxiv.org/abs/2512.14848)
*Claudio Lopez,Emilio Donoso,Mariano Javier de L. Dominguez Romero*

**TL;DR**: 开发了开源工具Skykatana，能高效构建、可视化多阶覆盖图并在有限资源下生成高分辨率天区掩模，应用于斯巴鲁HSC-WISE复合掩模和鲁宾天文台按需生成的恒星掩模。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于为现代广域巡天（wide-field surveys）开发高效、低内存消耗的开源空间掩模（spatial masks）构建工具，以应对大规模数据处理中剔除亮星晕（bright-star halos）、电荷拖尾（bleed trails）及低质量区域等挑战。

Data: 数据来源包括HATS/LSDB框架中的Gaia数据、Rubin DP1经验拟合数据以及Subaru HSC-WISE复合掩模数据。

Method: 采用基于HEALPix/HEALSparse的布尔运算构建多级覆盖图(multi-order coverage maps)，并结合HATS/LSDB框架实现高效低内存操作。

Result: 开发了开源工具Skykatana，能高效构建、可视化多阶覆盖图并在有限资源下生成高分辨率掩模，支持基于HATS/LSDB框架的分层数据处理。
...
### [46] [Why the Northern Hemisphere Needs a 30-40 m Telescope and the Science at Stake: Resolved Stellar Populations Studies in M31 and its Satellites](https://arxiv.org/abs/2512.14788)
...
**TL;DR**: 在北方部署一台30米级光学/近红外望远镜（optical/near-IR telescope），通过衍射极限成像和高分辨率多目标光谱观测M31（仙女座星系）及其卫星系统，可揭示其形成演化过程，并评估银河系（MW）在星系演化中的代表性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过高分辨率光学/近红外望远镜观测仙女座星系(M31)及其卫星系统，以揭示其形成与演化过程，并探讨银河系(MW)在星系演化中的代表性。

Data: 研究中使用的数据来源为北半球一台30米级光学/近红外望远镜（optical/near-IR telescope）进行的高分辨率衍射极限成像（diffraction-limited imaging）和多目标光谱观测（multi-object spectroscopy）所获取的暗弱恒星数据。

Method: 采用衍射极限成像（diffraction-limited imaging）和高分辨率多目标光谱（high-resolution, multi-object spectroscopy）技术对M31及其卫星系统进行观测。

Result: 在北半球部署一台30米级光学/近红外望远镜（optical/near-IR telescope），配备衍射极限成像和高分辨率多目标光谱仪，将实现对M31及其卫星系统形成演化的革命性研究，其科学价值与盖亚卫星（Gaia）、哈勃太空望远镜（HST）等设施对银河系（MW）的研究相当。
...
### [47] [Why the Northern Hemisphere Needs a 30-40 m Telescope and the Science at Stake: Galactic Archaeology from the Northern Sky](https://arxiv.org/abs/2512.14789)
...
**TL;DR**: 论文提出在2040-50年代需在北半球建设30米级望远镜，以高分辨率光谱观测暗弱主序星和转折星，解决银河系早期构建历史、暗物质（dark matter）颗粒性测试及外盘与反中心（anticenter）结构等关键问题。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用下一代天文设施解析银河系（Milky Way）早期构建历史及暗物质（dark matter）精细结构的科学主题。

Data: 数据来源包括盖亚(Gaia)、WEAVE、4MOST、鲁宾(Rubin)、欧几里得(Euclid)、罗曼(Roman)和ESO ELT等设施的观测数据，以及计划中北半球30米级望远镜对微弱主序星和拐点星的高分辨率光谱观测。

Method: 采用高分辨率光谱技术对暗弱主序星和转折星进行观测，结合宽视场、高复用度的30米级望远镜（northern 30m telescope）实现晕流、外盘及反中心子结构的化学动力学测绘（chemodynamical mapping）和断层扫描研究（tomographic studies）。

Result: 到2040-50年代，北方30米级望远镜结合高分辨率光谱能力将填补银河系考古的关键空白，包括测量暗物质子晕扰动、解析外盘与反中心（anticenter）结构，并为暗物质小尺度结构提供严格限制。
...
### [48] [Why the Northern Hemisphere Needs a 30-40 m Telescope and the Science at Stake: Northern Local Star-forming Dwarf Galaxies. Analogues of the First Galaxies and Probes of the Cosmic Metallicity Scale](https://arxiv.org/abs/2512.14790)
...
**TL;DR**: 研究发现本地宇宙中恒星形成矮星系（尤其是极贫金属星系）的高电离线（如He II、[Ne V]、C IV）起源挑战现有恒星模型，暗示可能存在第三星族恒星或非常规电离源，其深度光谱研究将改变现有星系化学演化认知。


<details>
  <summary>Details</summary>

Background: 研究聚焦于将本地宇宙中的恒星形成矮星系（尤其是极贫金属星系）作为再电离时期（EoR）早期星系的类比对象，以解决当前望远镜观测限制下电离气体温度密度不均匀性对化学组成测定的影响，以及高电离谱线起源对现有恒星模型提出的挑战。

Data: 数据来源于对本地宇宙中选定的恒星形成矮星系（star-forming dwarf galaxies）样本进行深度光谱观测。

Method: 采用深光谱观测技术分析局部宇宙中恒星形成矮星系（star-forming dwarf galaxies）的电离气体特性，结合重元素复合线（heavy element recombination lines）和高电离线（very high-ionization lines）测量，以探究其化学丰度及潜在非传统电离源（non-conventional ionizing sources）的影响。

Result: 本地宇宙中的恒星形成矮星系（尤其是极贫金属星系）可作为再电离时期（z >= 6）早期星系的类比对象。
...
### [49] [Why the Northern Hemisphere Needs a 30-40 m Telescope and the Science at Stake: Key Targets of Opportunity on Gas and Ice Giants and their satellites](https://arxiv.org/abs/2512.14791)
...
**TL;DR**: 研究发现北方30米级望远镜（30-m class size telescope）与极大望远镜（ELT）设备互补可确保观测外行星（outer planets）及其卫星系统中高影响力不可预测现象，弥补ELT对北天目标（northern objects）观测窗口不足的缺陷。


<details>
  <summary>Details</summary>

Background: 研究探讨了在北方建立一台30米级望远镜以弥补极大望远镜(ELT)对北半球外行星及其卫星系统观测能力不足的必要性。

Data: 研究中使用的数据来源为基于极大型望远镜（ELT）的观测数据和行星轨道周期（orbital cycles）的模拟数据。

Method: 采用30米级北半球望远镜（30-m class size telescope）与极大望远镜（ELT）互补的科学仪器进行时间关键观测（time-critical observations）。

Result: 在2055年前天王星(Uranus)的赤纬更利于北半球观测，海王星(Neptune)从2027年起未来90年也更适合北半球观测。
...
### [50] [Towards ALMA2040: An update from the European community and invitation to contribute](https://arxiv.org/abs/2512.15652)
...
**TL;DR**: ALMA2040计划旨在通过构建下一代亚毫米/毫米波（sub-mm/mm）观测设施，解决2040年代天体物理学的关键科学问题，超越现有ALMA（Atacama Large Millimeter/submillimeter Array）基础设施的能力限制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于规划下一代亚毫米/毫米波（sub-mm/mm）观测设施（ALMA2040），以应对2040年代天体物理学前沿需求并突破现有ALMA（Atacama Large Millimeter/submillimeter Array）设施的观测极限。

Data: 数据来源为阿塔卡马大型毫米波/亚毫米波阵列(ALMA)的观测数据及其即将进行的宽带灵敏度升级(Wideband Sensitivity Upgrade)项目。

Method: 采用社区驱动（community-driven）方法整合科学目标与技术愿景，通过2025年研讨会成果提炼下一代亚毫米波（sub-mm）设施的设计需求。

Result: ALMA2040倡议旨在为2040年代设计下一代亚毫米/毫米波（sub-mm/mm）观测设施，以解决远超ALMA现有能力的关键科学问题。
...
### [51] [Why the northern hemisphere needs a 30-40 m telescope and the science at stake: Massive stars in spiral galaxies](https://arxiv.org/abs/2512.14799)
*J. Maíz Apellániz,S. Simón-Díaz,A. Herrero,S. R. Berlanas,J. M. Mas Hesse,I. Negueruela,G. Holgado,M. García*

**TL;DR**: 论文核心贡献是探讨未来30米级望远镜对研究大质量恒星三大关键问题（金属丰度(metallicity)对恒星演化的影响、初始质量函数(initial mass function)范围、环境对恒星形成模式的作用）的潜在科学价值，研究对象包括银河系、本星系群螺旋星系及25兆秒差距内的邻近星系。


<details>
  <summary>Details</summary>

Background: 研究主题聚焦于大质量恒星演化中金属丰度（metallicity）的作用、初始质量函数（initial mass function）的分布以及不同星形成模式中环境因素的影响。

Data: 数据来源包括银河系、本星系群中的M31和M33以及25 Mpc范围内的其他星系（如M101、M51和NGC 6946），通过假设的北半球30米望远镜进行观测。

Method: 采用30米级望远镜（30~m type telescope）对北半球螺旋星系（包括银河系、M31、M33及25兆秒差距内的星系）进行观测，结合类似极大望远镜（ELT）的仪器配置开展恒星形成模式、初始质量函数（initial mass function）及金属丰度（metallicity）对恒星演化影响的研究。

Result: 2040年代大质量恒星研究的三个核心方向是：金属丰度（metallicity）对恒星演化的影响、大质量恒星到亚恒星天体的初始质量函数（initial mass function），以及环境对不同恒星形成模式的作用。
...
### [52] [White Dwarf Binaries: Probes of Future Astrophysics](https://arxiv.org/abs/2512.14800)
...
**TL;DR**: 白矮星双星(white dwarf binaries)是研究双星演化、引力波源、Ia型超新星(Type Ia Supernovae)及星系化学演化的关键天体，未来需要相位分辨光谱观测设备以突破其微弱特性的观测限制。


<details>
  <summary>Details</summary>

Background: 白矮星双星（white dwarf binaries）是研究双星演化模型、引力波源、Ia型超新星（Type Ia Supernovae）以及星系化学演化的关键天体物理探针。

Data: 数据来源于下一代大型综合巡天望远镜（LSST）在维拉·鲁宾天文台的观测数据以及未来能够进行相位分辨光谱观测的变革性设施。

Method: 采用相位分辨光谱观测(phase-resolved spectroscopic observations)技术对白矮星双星(white dwarf binaries)进行高精度测量。

Result: 白矮星双星（white dwarf binaries）是研究双星演化模型、引力波源、Ia型超新星（Type Ia Supernovae）前身系统以及星系化学演化的关键天体物理探针。
...
### [53] [Dynamical binary interactions in the 2040s](https://arxiv.org/abs/2512.14802)
...
**TL;DR**: 通过多波段观测、线性光谱偏振测量和干涉测量大量与动力学双星相互作用相关的明亮红新星（Luminous Red Novae），将揭示双星演化路径中动力学相互作用阶段的物理机制和发生频率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于双星系统动力学相互作用（如共有包层演化或恒星并合）在多种天体现象（如蓝离散星、超新星、伽马暴等）形成中的关键作用及其物理机制。

Data: 数据来源包括多波段观测（光谱和测光）、线性光谱偏振测量以及大量与动力学双星相互作用相关的明亮红新星（Luminous Red Novae）的干涉测量。

Method: 采用多波段观测（光谱学和测光）、线性光谱偏振测量和干涉测量技术对大量明亮红新星（Luminous Red Novae）进行研究。

Result: 多波段观测（光谱和测光）、线性光谱偏振测量和干涉测量将揭示动力学双星相互作用（dynamical binary interactions）的物理机制。
...
### [54] [Finding New Debris Discs at Sub-millimetre Wavelengths](https://arxiv.org/abs/2512.14803)
...
**TL;DR**: 研究发现阿塔卡马大口径亚毫米望远镜(AtLAST)能克服混淆极限，通过多波长观测揭示更暗弱的碎片盘(debris discs)，从而更全面研究行星系统结构和动力学历史。


<details>
  <summary>Details</summary>

Background: 研究主题为利用亚毫米波观测揭示行星系统结构及动力学演化历史，聚焦碎片盘（debris discs）的群体特性与探测技术突破。

Data: 数据来源于阿塔卡马大型亚毫米波望远镜（AtLAST）的多波长观测，用于克服混淆极限并探测更暗弱的碎片盘（debris discs）。

Method: 采用亚毫米波（sub-millimetre）观测技术结合阿塔卡马大孔径亚毫米波望远镜（AtLAST）的多波长探测能力，突破混淆极限（confusion limit），实现对暗弱碎屑盘（debris discs）的大视场巡天和大尺度辐射敏感探测。

Result: 亚毫米阿塔卡马大孔径望远镜（AtLAST）能够克服混淆极限，探测更暗弱的碎片盘（debris discs），并具备多波长表征能力、对邻近目标大尺度辐射的灵敏度以及对遥远星群的大视场巡天能力。
...
### [55] [Characterisation of the Bedretto Underground Site for Fundamental Physics Experiments](https://arxiv.org/abs/2512.14815)
...
**TL;DR**: 在瑞士提契诺州贝德雷托隧道（Bedretto tunnel）1400米覆盖层下测得宇宙缪子（muon）通量比地表低六个数量级（等效水深4000米），伽马射线、中子通量及氡浓度反映当地地质特征，磁噪声和地震噪声极低，满足下一代原子干涉引力波探测器要求，证实该地点是极具竞争力的深地基础物理实验场所。


<details>
  <summary>Details</summary>

Background: 地下实验室为稀有事件探测、引力波(GW)检测和量子传感技术提供了超低本底和低振动环境。

Data: 数据来源于对瑞士提契诺州Bedretto隧道（水平通道）的现场测量，包括宇宙缪子（cosmic-muon）通量、伽马射线（gamma-ray）通量、中子通量、氡浓度、磁场频谱及地震背景等参数的直接观测。

Method: 采用宇宙线缪子(cosmic-muon)、伽马射线(gamma-ray)、中子通量(neutron fluxes)、氡浓度(radon concentration)、磁场谱(magnetic-field spectrum)及地震背景(seismic backgrounds)测量技术进行环境特征综合表征。

Result: 在1400米覆盖层下测得宇宙缪子(cosmic-muon)通量比地表低六个数量级，等效水深约4000米，伽马射线和中子测量反映了当地地质特征，磁噪声和地震噪声水平极低，满足下一代原子干涉引力波探测器的要求。
...
### [56] [Exploiting tidal asteroseismology in binary populations from combined space photometry and time-resolved high-resolution spectroscopy](https://arxiv.org/abs/2512.14816)
*Ema Šipková,Alex Kemp,Dario Fritzewski,Andrew Tkachenko,Dominic M. Bowman,Conny Aerts,Jasmine Vrancken*

**TL;DR**: 提出利用高分辨率多光纤光谱仪（multi-fibre spectrograph）对双星系统中的脉动恒星（pulsators）进行相位解析观测，以建立不依赖模型的恒星参数约束并验证双星星震学（asteroseismic）建模技术。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高分辨率光谱仪提升对双星系统中脉动变星（pulsating stars）的观测能力，以深化对恒星内部过程、潮汐物理（tidal physics）及双星演化（binary stellar evolution）的理解。

Data: 数据来源为空间测光（space-based photometry）和高分辨率光谱（high-resolution spectroscopy）观测，并计划使用多光纤光谱仪（multi-fibre spectrograph）进行大规模相位分辨光谱测量。

Method: 采用高分辨率（R≥50000）、高信噪比（S/N≥300）的多光纤光谱仪（multi-fibre spectrograph）进行相位分辨观测，以分钟级精度解析脉动变星（pulsators）的周期性变化及轨道运动。

Result: 高分辨率多光纤光谱仪（multi-fibre spectrograph）能高效解析双星系统中脉动变星（pulsators）的分钟级脉动和天至年级轨道运动。
...
### [57] [Luminous Fast Blue Optical Transients in the 2040s](https://arxiv.org/abs/2512.14832)
*A. A. Chrimes,N. Sarin,D. Coppejans,P. J. Groot,A. Inkenhaag,P. G. Jonker,T. L. Killestein,D. A. Perley,M. Pursiainen*

**TL;DR**: 该论文提出2040年前提升观测能力以解决明亮快速蓝色光学暂现源（LFBOTs）起源之谜（可能涉及中等质量黑洞潮汐瓦解事件、失败超新星或恒星与黑洞并合），并揭示其与黑洞形成/增长、中心引擎物理及暂现源-宿主星系相互作用的关联。


<details>
  <summary>Details</summary>

Background: LFBOTs（Luminous Fast Blue Optical Transients）作为一类起源未解的河外暂现源，其研究将推动黑洞形成/增长、中心引擎物理及暂现源-宿主星系相互作用等未来暂现天文学核心课题的发展。

Data: 研究使用了现有光度测量巡天（photometric surveys）中遗漏的绝大多数明亮快速蓝色光学瞬变（Luminous Fast Blue Optical Transients, LFBOTs）事件数据。

Method: 采用多波段光度测量(photometric survey)和光谱跟踪观测(spectroscopic follow-up)技术提升LFBOTs探测效率。

Result: LFBOTs（Luminous Fast Blue Optical Transients）的深入研究将为黑洞形成/增长、中心引擎物理和暂现源-宿主星系相互作用提供独特见解。
...
### [58] [Development of a Custom kV-amplitude, pressure-tolerant Radio-Frequency transmitter](https://arxiv.org/abs/2512.14866)
...
**TL;DR**: 设计并验证了一种用于超高能中微子(UHEN)探测实验的高压射频校准发射器（含IDL脉冲发生器和SPUNK天线），在南极1740米深钻孔中实现5公里距离的高信噪比信号传输。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用极地冰层中部署的射频接收天线探测超高能中微子（UHEN）的首次观测实验，重点解决接收器响应校准以准确估计中微子能量和入射方向的关键问题。

Data: 数据来源于南极1740米深钻孔中部署的定制射频校准发射器（含`IDL'脉冲发生器和`SPUNK'天线），在5公里距离处获取高信噪比信号。

Method: 采用定制射频校准发射器（包括千伏级信号发生器和高压环境下工作的天线）进行接收器响应校准，并通过南极1740米深钻孔验证其性能。

Result: 设计并验证了一种可在200大气压下工作的高压射频校准发射器（SPUNK天线），在1740米深钻孔中测试时能在5公里距离外产生高信噪比信号。
...
### [59] [AMKID -- a large KID-based camera at the APEX telescope](https://arxiv.org/abs/2512.14905)
...
**TL;DR**: 研制了安装在APEX望远镜上的双频段（350GHz和850GHz）微波动态电感探测器（AMKID）相机，其大视场（15.3'x15.3'）、高灵敏度（低频阵列噪声温度2.2mK√s）和高角分辨率（17.0"）为亚毫米波天文观测开辟了新途径。


<details>
  <summary>Details</summary>

Background: 亚毫米波（sub-millimeter）热辐射探测在天文研究中具有独特价值，涵盖从年轻恒星周围盘与行星形成到宇宙学距离的星系演化等广泛领域。

Data: 数据来源于安装在智利海拔5100米的12米APEX望远镜上的AMKID相机，该相机采用动感电感探测器（kinetic inductance detectors），覆盖350 GHz和850 GHz两个大气窗口，高频波段包含13,952个探测器，低频波段包含3,520个探测器，视场为15.3'x15.3'。

Method: 采用基于动力学电感探测器（kinetic inductance detectors）的双色宽视场相机技术，同时覆盖350 GHz和850 GHz大气窗口。

Result: AMKID相机在APEX望远镜上实现了双色观测能力、高灵敏度、大视场和高角分辨率，有望开启APEX望远镜的新科学研究领域。
...
### [60] [The use of astronomical data for satellite tracking](https://arxiv.org/abs/2512.15122)
*Rositsa Miteva,Nikola Antonov,Adrian Sonka*

**TL;DR**: 该研究通过分析天文观测数据中的卫星轨迹（spacecraft streaks），评估了专用天文观测在卫星监测、识别及轨道参数推算中的有效性，并对比验证了其与现有卫星数据库的一致性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用天文观测数据解决人造卫星（artificial satellites）对地面光学和射电观测（ground-based optical and radio observations）的干扰问题。

Data: 使用天文观测数据来自保加利亚梅什蒂察天文台（Astronomical Observatory Meshtitsa）和https://www.tycho-tracker.com/软件进行条纹分析。

Method: 采用天文观测数据结合Tycho-tracker软件进行条纹分析，并与Celestrak等卫星数据库进行比对。

Result: 利用天文观测数据结合专用软件分析航天器轨迹（streak），可有效监测卫星并推算轨道参数。
...
### [61] [Expanding stellar horizons with polarized light](https://arxiv.org/abs/2512.15170)
...
**TL;DR**: 该论文提出利用高分辨率、高信噪比（S/N）的时域分光偏振测量（spectro-polarimetry）结合星震学（asteroseismology）研究恒星（从M型矮星到O型星）表面磁场特性与非径向振荡模式，以揭示恒星内部磁场结构、角动量传输及化学元素混合机制，推动恒星演化模型的发展。


<details>
  <summary>Details</summary>

Background: 恒星表面磁场（surface magnetism polarization）的偏振观测及其对恒星演化、角动量传输和化学元素混合的影响是研究的核心科学主题。

Data: 数据来源为时间分辨的高分辨率、高信噪比（S/N）光谱偏振测量（spectro-polarimetry）和光谱数据，并结合过去、现在及未来任务的长期空间测光数据。

Method: 采用高分辨率、高信噪比(S/N)的时间分辨光谱偏振测量(spectro-polarimetry)结合星震学(asteroseismology)分析恒星表面磁场与内部磁场的关联。

Result: 偏振测量（polarimetry）是研究恒星表面磁场（surface magnetism）及其对恒星演化、角动量传输和化学元素混合影响的关键工具，结合高分辨率时域光谱偏振观测与空间测光数据可显著提升对恒星内部结构的认识。
...
### [62] [A Characterization of JWST MIRI Detector Persistence and Implications for High-Contrast Imaging](https://arxiv.org/abs/2512.15477)
*Alisha Vasan,Mary Anne Limbach,Andrew Vanderburg,Rachel Bowens-Rubin,Kevin B. Stevenson*

**TL;DR**: 首次在21微米波段（F2100W）量化了JWST中红外仪器（MIRI）探测器因饱和事件导致的通量缺失持续性（flux deficit persistence）现象，测得初始持续水平为1.69±0.10%，并通过贝叶斯指数衰减模型预测其降至初始值十分之一需5.16小时，揭示了该效应可能在高对比度成像中产生系外行星假信号并影响点扩散函数（PSF）扣除。


<details>
  <summary>Details</summary>

Background: 研究聚焦于詹姆斯·韦伯太空望远镜(JWST)中红外仪器(MIRI)探测器在21微米波段(F2100W)因饱和事件引发的通量缺失持续性(flux deficit persistence)现象及其对高对比度成像的影响。

Data: 数据来源于JWST（James Webb Space Telescope）中红外仪器（MIRI）在21微米（F2100W）波段对邻近（8.12±0.04秒差距）中M型矮星IRAS 21500+5903（K=5.65星等）饱和事件后的全视场成像观测。

Method: 采用贝叶斯指数衰减模型（Bayesian exponential decay model）量化JWST中红外成像仪（MIRI）探测器在21微米波段饱和事件后的通量亏损持续性效应。

Result: 在21微米波段（F2100W）观测中，JWST的MIRI探测器在饱和事件后表现出通量亏损型持久效应（persistence），初始水平为1.69±0.10%，经贝叶斯指数衰减模型拟合，其强度降至初始值十分之一的时间为5.16^{+1.49}_{-0.94}小时。
...
### [63] [Spectroscopic Alerts for the Time-Domain Era](https://arxiv.org/abs/2512.15555)
...
**TL;DR**: 该论文提出"光谱警报"(spectroscopic alerts)概念，通过实时监测显著光谱演化(spectral evolution)将光谱学转变为发现渠道而非仅作为后续观测手段，以解决下一代时域巡天(time-domain surveys)产生的高达10^9个警报与全球仅10^4-10^5个瞬变光谱(transient spectra)年观测能力之间的关键缺口。


<details>
  <summary>Details</summary>

Background: 时域天文学（time-domain astronomy）正进入由LSST、Roman、Euclid、SKA和PLATO等大视场高采样率巡天推动的前所未有的发现时代。

Data: 研究中使用的数据来源包括宽视场高采样率巡天项目（如LSST、Roman、Euclid、SKA和PLATO）产生的光变警报流（photometric alert streams）以及全球光谱跟踪观测（spectroscopic follow-up）获取的瞬变光谱（transient spectra）。

Method: 采用宽视场高复用光谱设施（wide-field, highly multiplexed spectroscopic facility）实现实时光谱警报（spectroscopic alerts）触发机制，通过监测电离状态（ionisation state）、运动学（kinematics）和吸积（accretion）等光谱特征演化进行瞬变天体发现。

Result: 下一代时域天文巡天将产生每年约10^9个警报，而全球光谱跟踪能力仅能提供约10^4-10^5个瞬变光谱，凸显了实时光谱警报（spectroscopic alerts）作为独立发现通道的必要性。
...
### [64] [Benchmarking Dimensionality Reduction Methods for High-Dimensional ALMA Image Cubes](https://arxiv.org/abs/2512.15615)
*Haley N. Scolati,Ryan A. Loomis,Anthony J. Remijan,Kin Long Kelvin Lee*

**TL;DR**: 该研究通过评估多种降维方法（包括传统线性和现代非线性技术）在高维天文数据立方体（high-dimensional astronomical data cubes）分析中的性能差异，为ALMA（阿塔卡马大型毫米波阵列）观测数据处理提供了兼顾计算成本、重建精度和可扩展性的方法选择建议，并探讨了这些技术在数据分割、标记算法及未来ALMA宽带灵敏度升级（Wideband Sensitivity Upgrade）时代高级数据产品生成中的通用性。


<details>
  <summary>Details</summary>

Background: 高维天文数据立方体（high-dimensional astronomical data cubes）为研究天体物理和化学过程提供了丰富的光谱与结构信息，但其复杂性和庞大规模对高效分析、可视化和解读提出了显著挑战。

Data: 使用公开存档的ALMA（Atacama Large Millimeter/submillimeter Array）数据，涵盖不同源形态和观测设置的多样化样本。

Method: 采用线性（如主成分分析）和非线性（如卷积自编码器）降维技术评估高维天文数据立方体的特征提取能力。

Result: 高维天文数据立方体（high-dimensional astronomical data cubes）的分析需要综合评估线性（如主成分分析）和非线性（如卷积自编码器）降维方法在特征保留、计算可扩展性等方面的性能差异。
...
### [65] [Searching potential astronomical sites in Ethiopia](https://arxiv.org/abs/2512.15669)
*SS Akalu,N. Suleiman,GM. Kumssa,ST. Belay,E. Alemayehu,M. Getnet,M. Povic,SH. Negu,B. Belata,J. Tamrat*

**TL;DR**: 通过多标准统计分析（MCSA）技术从埃塞俄比亚367座山峰中筛选出21个具备光学天文台（optical observatory）建设潜力的候选地点，为保护暗夜天空和发展东非地区天文旅游（astrotourism）提供科学依据。


<details>
  <summary>Details</summary>

Background: 研究主题为埃塞俄比亚及东非地区新光学天文台（optical observatory）候选站点的选址评估与暗天空保护。

Data: 数据来源包括367座山脉的海拔、人工光污染、云覆盖率、湿度、风速和风向等六项基础指标。

Method: 采用多准则统计分析(MCSA)技术对367座山脉的6项基本选址标准（海拔、人工光污染、云量、湿度、风速和风向）进行综合评估。

Result: 基于多标准统计分析（MCSA）技术，从367座山脉中筛选出21个具备高潜力的候选天文台址，用于研究未来视宁度（astronomical seeing）效应。
...
### [66] [Extensive Observational Evidence for Massive Star Stellar Wind Variability at Low Metallicities: implications for mass-loss rate determination](https://arxiv.org/abs/2512.15539)
*Timothy N. Parsons,Raman K. Prinja,Derck L. Massa,Alex W. Fullerton*

**TL;DR**: 通过分析大麦哲伦云（LMC）和小麦哲伦云（SMC）20颗OB型恒星的紫外光谱，证实低金属丰度环境下恒星风（stellar wind）普遍存在大尺度光学厚结构（optically thick wind structure），其导致的光学深度（optical depth）观测不确定性（最高达45%）与银河系金属丰度环境相当，并揭示了现有质量损失率（mass-loss rate）模型难以准确处理风形成紫外谱线中窄吸收成分（narrow absorption components）变化的影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量恒星的质量流失（mass-loss）对恒星和星系演化及星际介质（interstellar medium）富集的关键作用，尤其关注恒星风（stellar wind）光学深度结构对质量流失率测定不确定性的影响。

Data: 研究中使用的数据来源于20颗大麦哲伦云（LMC）和小麦哲伦云（SMC）OB型恒星的紫外光谱观测数据。

Method: 采用紫外光谱测量大麦哲伦云（LMC）和小麦哲伦云（SMC）20颗OB型恒星风形成吸收剖面的平均光学深度标准差与均值比例，量化低金属丰度环境下恒星风结构引入的不确定性。

Result: 大质量恒星风（stellar wind）光学深度（optical depth）在低金属丰度环境下的不确定性幅度与银河系金属丰度下已确认的幅度相似（较冷OB星可达45%）。
...
### [67] [A Very Rich Bimodal Galaxy Cluster Merger: RXC J0032.1+1808](https://arxiv.org/abs/2512.14945)
*David Wittman,Rodrigo Stancioli,Faik Bouhrik,Reinout van Weeren,Andrea Botteon*

**TL;DR**: 研究发现星系团RXC J0032.1+1808具有双峰X射线形态和光学/引力透镜观测的南北延伸结构，结合射电遗迹（radio relic）证据表明该系统处于首次近心点（pericenter）后的并合过程，通过流体动力学模拟（hydrodynamic simulations）确定其为非正碰并合，观测时刻距近心点约395-560 Myr且视线方向使子团表观间距缩短约2倍。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于星系团（galaxy cluster）RXC J0032.1+1808的动力学状态，特别是通过多波段观测数据探讨其合并（merging）过程的物理特征和演化阶段。

Data: 数据来源于XMM-Newton存档数据（X-ray温度与光度）、成员星系红移调查（速度差）以及公开可用的流体动力学模拟（合并参数）。

Method: 采用X射线形态分析、XMM-Newton数据温度与光度提取、成员星系红移测量以及流体动力学模拟（hydrodynamic simulations）推断合并参数。

Result: X射线形态的双峰特征（bimodal）及射电遗迹（radio relic）表明RXC J0032.1+1808处于首次近心点（first pericenter）后的并合过程。
...
### [68] [Super-Eddington Chimneys: On the Cooling Evolution of Tidal Disruption Event Envelopes](https://arxiv.org/abs/2512.14810)
*Semih Tuna,Brian D. Metzger,Yan-Fei Jiang,Andrea Antoni*

**TL;DR**: 通过轴对称辐射流体动力学模拟发现，潮汐瓦解事件(TDE)后形成的准静态包层通过辐射扩散、湍流混合和极向外流快速冷却收缩，在环流半径附近形成致密旋转支撑环，其冷却时间尺度比光子扩散时间短约十倍，且包层冷却时间对其光学厚度依赖较弱，表明平流和风驱动能量输运主导了该过程。


<details>
  <summary>Details</summary>

Background: 研究聚焦于潮汐瓦解事件（TDE）后致密吸积盘形成的热力学冷却机制，探讨弱束缚星体碎屑如何通过辐射扩散、湍流混合和极向外流等过程快速损失热能并收缩至环流半径附近。

Data: 研究使用轴对称辐射流体动力学模拟（axisymmetric radiation-hydrodynamic simulations）对类静态的“潮汐瓦解事件包层（TDE envelopes）”进行建模，初始条件基于完整恒星瓦解预期的总质量、角动量和结合能。

Method: 采用轴对称辐射流体力学模拟（axisymmetric radiation-hydrodynamic simulations）研究准静态TDE包层（TDE envelopes）的冷却过程，结合辐射扩散、湍流混合和极向外流（polar outflows）分析能量输运机制。

Result: 潮汐瓦解事件(TDE)后形成的致密吸积盘需要激波后的恒星碎片在向黑洞沉降过程中高效冷却。
...
### [69] [Time Evolution of Optical Darkness in GRB Afterglow: The Case of GRB 240825A](https://arxiv.org/abs/2512.15162)
*Rui-Zhi Li,Jirong Mao,Yuan-Pei Yang,Bo-Ting Wang,Fei-Fan Song,Yu-Xin Xin,Jin-Ming Bai*

**TL;DR**: 通过多波段观测和时变光谱分析揭示了伽马射线暴(GRB) 240825A余辉的光学-X射线光谱特征演化规律，发现其发生在尘埃遮蔽环境中且光学暗度($β_\mathrm{OX}$)在触发后约1000秒达到最小值。


<details>
  <summary>Details</summary>

Background: 研究主题为长时伽马射线暴(GRB)的光学-X射线余辉光谱演化及其尘埃遮蔽环境的物理特性。

Data: 数据来源于GRB 240825A早期余辉的多波段（multi-wavelength）跟踪观测，包括X射线和光学波段的时变光谱分析。

Method: 采用时间分辨光谱分析(time-resolved spectral analysis)和光学暗度($β_\mathrm{OX}$)计算，结合X射线至光学能谱分布(SED)拟合推导消光曲线(extinction curves)。

Result: GRB 240825A的余辉光学暗度($β_\mathrm{OX}$)呈现先降后升趋势，在触发后约1000秒达到最小值，但11小时后未达到光学暗暴标准。
...
### [70] [Future Space-based Gamma-ray Pulsar Timing Arrays](https://arxiv.org/abs/2512.14981)
*Matthew Kerr,Zorawar Wadiasingh,Adrien Laviron,Constantinos Kalapotharakos,Thankful Cromartie,Tyler Cohen*

**TL;DR**: 通过模拟银河系毫秒脉冲星（MSP）群体并开发高保真伽马射线光谱预测方法，研究发现下一代伽马射线脉冲星计时阵（GPTA）在0.1-5 GeV能段可探测10^3-10^4个MSP，其引力波（GW）灵敏度与当前射电脉冲星计时阵（PTA）相当甚至更优，并能区分银河系核球区域MSP群体的形成机制。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用毫秒脉冲星（MSPs）的伽马射线脉冲星计时阵（GPTA）探测纳赫兹引力波（nHz GWs），并评估其在下一代伽马射线天文台中的性能潜力。

Data: 研究中使用的数据来源包括费米大面积望远镜（Fermi LAT）对约100颗毫秒脉冲星（MSPs）的全天监测数据，以及通过模拟银河系MSPs群体生成的高保真伽马射线光谱预测数据。

Method: 采用伽马射线能谱预测方法模拟银河系毫秒脉冲星（MSP）群体，并结合费米大面积望远镜（LAT）观测数据验证模型准确性。

Result: GPTA（伽马射线脉冲星计时阵）在0.1至5 GeV能段表现最优，可探测10^3至10^4颗毫秒脉冲星（MSPs），其引力波（GWs）灵敏度与当前射电PTA相当或更优，并能区分银河系核球区MSPs的不同形成机制。
...
### [71] [Strong lensing cosmography using binary-black-hole mergers: Prospects for the near future](https://arxiv.org/abs/2512.15168)
*Koustav N. Maity,Souvik Jana,Tejaswi Venumadhav,Ankur Barsode,Parameswaran Ajith*

**TL;DR**: 该论文提出利用升级版LIGO-Virgo-KAGRA（LVK）网络观测到的强透镜化（strongly lensed）双黑洞（BBH）引力波（GW）事件数量和时间延迟分布，探索对宇宙学参数进行约束的可能性，并证明即使透镜化事件数量较少也能实现这一目标。


<details>
  <summary>Details</summary>

Background: 研究探讨了利用引力波（GW）强透镜效应（strong lensing）探测宇宙学参数（cosmological parameters）的潜力，重点关注下一代探测器（XG detectors）和升级版LIGO-Virgo-KAGRA（LVK）网络的中等红移（intermediate redshifts）宇宙学信息获取。

Data: 研究中使用的数据来源于升级后的LIGO-Virgo-KAGRA（LVK）探测器网络对引力波（GW）信号的观测数据。

Method: 采用强引力透镜效应（strong lensing）分析技术，通过统计透镜化引力波（GW）事件的数量和时延分布来约束宇宙学参数。

Result: 利用升级版LIGO-Virgo-KAGRA（LVK）网络观测到的数十个引力波（GW）强透镜事件，能够对宇宙学参数进行适度约束。
...
### [72] [Achromatic, spin-odd Kerr EVPA as a null Frenet--Serret torsion integral on the photon ring](https://arxiv.org/abs/2512.14773)
*M. Baran Ökten*

**TL;DR**: 通过计算克尔(Kerr)时空在光子环上对线性偏振的无色引力印记，发现直接临界曲线上的克尔减史瓦西(Schwarzschild)偏振模式具有严格奇对称性，并构建了受对称性保护的奇宇称环估计器，为M87$^\ast$和Sgr~A$^\ast$等源的视界尺度环偏振测量提供了仅依赖自旋和倾角的两参数模板。


<details>
  <summary>Details</summary>

Background: 该研究探讨了克尔时空（Kerr spacetime）在光子环（photon ring）上对线性偏振产生的无色差引力印记（achromatic gravitational imprint），聚焦于强引力场中平行输运（parallel transport）和偏振演化规律。

Data: 数据来源为通过向后追踪（backward-shot）、采用卡特分离（Carter-separated）的测地线结合中点二阶龙格-库塔（midpoint RK2）传输方法生成的模拟数据。

Method: 采用核心分析方法包括在零Frenet-Serret框架中重新表述平行输运以得到电场矢量位置角的标量演化规律，以及使用后向追踪、Carter分离测地线结合中点RK2输运实现二阶收敛。

Result: result：克尔(Kerr)时空在光子环上对线性偏振产生的无色差引力印记表现为直接临界曲线上的克尔减史瓦西(Kerr-minus-Schwarzschild)模式非零、严格奇对称且局限于薄环状区域，其幅度随自旋和倾角单调增长（均方根值约0.5°–2°）。
...
### [73] [Physical Effects of Gravitational Waves at Second Order](https://arxiv.org/abs/2512.15704)
*Guillem Domènech,Shi Pi,Ao Wang*

**TL;DR**: 首次通过测地观测者测量证实了二阶宇宙学扰动理论中引力波应变（gravitational wave strain）的物理效应，并确定其与牛顿规范（Newton gauge）下的横向无迹分量一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙学扰动理论二阶情形下引力波应变（gravitational wave strain）的严格定义缺失问题，揭示了传统空间超曲面横向无迹度规涨落关联方法在二阶情形因时空切片依赖性导致的根本性模糊性。

Data: 数据来源于宇宙学微扰理论（cosmological perturbation theory）中的二阶引力波（gravitational wave）计算，通过测地观测者（geodesic observers）发射和接收电磁信号来测量物理效应。

Method: 采用测地线观测者发射和接收电磁信号的物理测量方法，首次计算了二阶引力波的物理效应。

Result: 在牛顿规范（Newton gauge）下测量的引力波应变（gravitational wave strain）与横向无迹（transverse-traceless）分量一致。
...
### [74] [Non-Thermal Production of Sexaquark Dark Matter](https://arxiv.org/abs/2512.14838)
*Marianne Moore,Stefano Profumo*

**TL;DR**: 研究发现非热产生机制（non-thermal production）能克服传统热冻结（thermal freeze-out）无法解释六夸克（sexaquark）暗物质丰度的关键障碍，其最终丰度由奇异夸克物质分支比和物质主导时期的六夸克聚结效率决定。


<details>
  <summary>Details</summary>

Background: 该研究探讨了在标准热冻结情景下六夸克态（sexaquark）作为暗物质候选者面临的丰度不足问题，并聚焦于非热产生机制如何克服这一障碍。

Data: 研究采用非热机制（non-thermal mechanism）数据，具体来源于重加热子（reheaton）晚衰变过程中产生的奇异夸克富集物质（strange-quark-rich matter）及物质主导或早期辐射主导时期的夸克聚结（coalescence）概率。

Method: 采用非热机制（non-thermal mechanisms）和重加热子（reheatons）衰变模型，结合奇异夸克富集物质分支比及物质主导或早期辐射主导时期的六夸克（sexaquark）聚结概率计算最终丰度。

Result: 非热机制通过晚衰变重热子（reheaton）克服了传统热冻结（thermal freeze-out）无法产生足够六夸克（sexaquark）丰度的障碍，其最终丰度由奇异夸克富集物质的支化比和物质主导或早期辐射主导时期的六夸克聚结概率决定。
...
### [75] [ALMA Band 7 Observations of Water Lines in the Protoplanetary Disk of V883 Ori](https://arxiv.org/abs/2512.15108)
...
**TL;DR**: 通过阿塔卡马大型毫米波/亚毫米波阵列（ALMA）观测原恒星（protostar）V883 Ori的水同位素分子（HDO和H₂¹⁸O），首次在距中心约80天文单位（au）的水雪线（water snowline）内侧测得水蒸气丰度（N_H₂O/N_H₂）为3×10⁻⁷-5×10⁻⁶及HDO/H₂O比值为(0.4-2.0)×10⁻³，证实其继承自原恒星包层（protostellar envelopes）的化学特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原行星盘（protoplanetary disk）中水雪线（water snowline）的探测及其对行星形成过程中尘埃增长和挥发性物质输送的关键作用。

Data: 数据来源于阿塔卡马大型毫米波/亚毫米波阵列（Atacama Large Millimeter/submillimeter Array, ALMA）的Band 7观测，检测了V883 Ori原行星盘中两种水同位素分子跃迁：322 GHz的para-H$_2$$^{18}$O $5_{1,5}$-$4_{2,2}$和335 GHz的HDO $3_{3,1}$-$4_{2,2}$。

Method: 采用阿塔卡马大型毫米波/亚毫米波阵列(ALMA) Band 7观测数据，结合开普勒旋转校正和马尔可夫链蒙特卡洛(MCMC)方法的转动能级图分析。

Result: 在V883 Ori的原行星盘中，水同位素分子（HDO和H₂¹⁸O）的检测揭示了水汽丰度（N_H₂O/N_H₂）约为3×10⁻⁷至5×10⁻⁶，且HDO/H₂O比率为(0.4-2.0)×10⁻³，与来自原恒星包层的继承机制基本一致。
...
### [76] [A Comparison of 1D and 3D Exoplanet Atmosphere Model Grids: ScCHIMERA and the SPARC/MITgcm](https://arxiv.org/abs/2512.14882)
*Lindsey S. Wiser,Alexander Roth,Vivien Parmentier,Michael R. Line*

**TL;DR**: 通过对比三维大气环流模型（3D-GCMs）和一维辐射对流平衡模型（1D-RCE），发现两者在系外行星昼侧热辐射光谱上存在显著差异，尤其当平衡温度为1400 K时3D-GCMs出现温度反转而1D-RCE未出现，且光谱差异可能超过JWST观测误差。


<details>
  <summary>Details</summary>

Background: 研究主题为比较一维辐射对流平衡（1D-RCE）和三维大气环流模型（3D-GCM）在预测系外行星昼侧热发射光谱时的差异及其对观测数据解释的影响。

Data: 研究使用了SPARC/MITgcm生成的三维大气环流模型（3D-GCMs）数据和ScCHIMERA生成的一维辐射对流平衡模型（1D-RCE）数据，基于相同的出射辐射热通量在平衡温度、重力、金属丰度和自转周期网格上进行一对一比较。

Method: 采用一维辐射对流平衡模型（1D-RCE, ScCHIMERA）和三维大气环流模型（3D-GCM, SPARC/MITgcm）进行参数化网格对比，重点分析行星昼侧平均压力-温度结构对光谱差异的影响。

Result: 3D-GCM（SPARC/MITgcm）和1D-RCE（ScCHIMERA）模型在相同参数下产生的昼侧热发射光谱差异主要源于四层垂直压力-温度（PT）结构差异，其中Teq=1400 K的行星在3D-GCM中表现出温度反转而1D-RCE模型未出现。
