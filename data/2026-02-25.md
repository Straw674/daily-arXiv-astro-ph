...
### [1] [ALMA Central Molecular Zone Exploration Survey (ACES) II: 3mm continuum images](https://arxiv.org/abs/2602.20240)
...
**TL;DR**: 首次通过ALMA中央分子区探测计划（ACES）以2.5角秒分辨率绘制了银河系中心约200秒差距区域的大尺度3毫米连续谱图像，结合GBT MUSTANG-2数据恢复了延展结构，并区分了尘埃主导与自由-自由或同步辐射主导的亮致密源。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)和绿岸望远镜(GBT)对银河系中心区域(central $\sim200$ parsecs)进行高分辨率(约2.5角秒)和大尺度(约1000平方角分)的毫米波连续谱观测，以揭示其分子气体分布和物理特性。

Data: 数据来源于ALMA（Atacama Large Millimeter/submillimeter Array）中央分子区探测巡天（ACES）的45个独立马赛克观测和GBT（Green Bank Telescope）MUSTANG-2三毫米扩展核区巡天（TENS）。

Method: 采用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)和绿岸望远镜(Green Bank Telescope) MUSTANG-2的3毫米波段观测数据，通过拼接(mosaic)45个独立ALMA观测区域并结合羽毛化(feathering)技术恢复大尺度结构。

Result: ACES首次以高分辨率（~2.5角秒）绘制了银河系中心约200秒差距区域的大尺度3毫米连续谱图像，典型RMS噪声为~0.1 mJy/beam，但在人马座A*和人马座B2等亮源区域噪声显著升高。
...
### [2] [The ALPINE-CRISTAL-JWST Survey: Chemical Abundance Comparison Between the ISM and CGM of Main-Sequence Galaxies at z=4-6](https://arxiv.org/abs/2602.20241)
...
**TL;DR**: 研究发现4<z<6质量大于10^9.5太阳质量的主序星系中星际介质(ISM)和环星系介质(CGM)具有相似的化学丰度，表明早期宇宙中CGM已富集至ISM水平，且电离程度差异和氮氧丰度比异常揭示了电离水平差异主导了氧线比观测差异，并存在氮再分布延迟及[CII]158μm/Hα比值增强暗示反馈或并合对金属混合的关键作用。


<details>
  <summary>Details</summary>

Background: 星系周围的气体晕（CGM）在星系演化中起重要作用，其化学组成受恒星形成、反馈及并合历史影响。

Data: 数据来源为JWST/NIRSpec积分场光谱仪对星系样本的ISM和CGM光学谱线观测，以及ALMA对[CII]158μm和Hα辐射的观测。

Method: 采用JWST/NIRSpec积分视场光谱技术获取ISM和CGM的光学发射线比例，结合\texttt{Cloudy}电离模型分析电离水平差异及氮氧丰度比异常。

Result: 星系际介质（ISM）和环星系介质（CGM）在早期宇宙中具有相似的化学丰度，表明CGM已被富集至与ISM相当的水平。
...
### [3] [Dynamical friction on binary stars in dark matter dominated environments](https://arxiv.org/abs/2602.20245)
*Nicolas Esser*

**TL;DR**: 研究了双星系统在均匀暗物质（dark matter）背景下的动力学摩擦效应，发现双星轨道参数（如偏心率和半长轴）会发生长期演化，且在暗物质主导的极暗弱矮星系（ultra-faint dwarf galaxies）中动力学摩擦会加剧未分辨双星对速度弥散测量的偏差。


<details>
  <summary>Details</summary>

Background: 研究探讨了在均匀暗物质（dark matter）背景下经历动力学摩擦（dynamical friction）的双星系统（binary stars）的轨道参数长期演化。

Data: 研究中使用了理论模拟（simulation）数据，通过构建均匀暗物质背景下的双星系统模型来研究动力学摩擦效应。

Method: 采用解析推导方法研究双星系统在均匀暗物质背景下的轨道参数长期演化（secular evolution），重点分析动力学摩擦（dynamical friction）对宽/密近双星半长轴（semi-major axis）和偏心率（eccentricity）的影响。

Result: 在暗物质主导的极暗弱矮星系（ultra-faint dwarf galaxies）中，部分双星系统会经历显著的轨道偏心率（eccentricity）振荡，同时其轨道半长轴（semi-major axes）缓慢减小。
...
### [4] [Little Red Dots: One Photometric Tag Concealing Diverse Spectroscopic Flavors of Massive Star Formation and Black Hole Activity](https://arxiv.org/abs/2602.20247)
...
**TL;DR**: 通过分析249个红点天体(LRDs)的JWST/NIRSpec和MIRI数据，发现其构成异质群体，包含超大质量黑洞(BHs)(质量约10^6.0-6.5 M⊙)和恒星(质量约10^8.3 M⊙)，黑洞-恒星质量比为1-2%，表现出高密度气体环境下亚爱丁顿吸积(λ_Edd=0.6±0.2)、年轻星暴(3-7 Myr)及多种辐射转移特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST/NIRSpec棱镜和MIRI数据对249个小红点（LRDs）进行光谱分析，探索其作为超大质量黑洞（BHs）与致密气体云及伴生恒星组成的异质群体特性。

Data: 数据来源为JWST（詹姆斯·韦伯太空望远镜）的NIRSpec（近红外光谱仪）棱镜模式和MIRI（中红外仪器）观测数据，以及NIRCam（近红外相机）选定的249个小红点（LRDs）的光谱子集。

Method: 采用JWST/NIRSpec棱镜和MIRI数据对249个小红点(LRDs)进行光谱分析，通过中位数叠加光谱技术覆盖静止波长0.09-1.2μm范围，并利用MIRI测光将光谱能量分布扩展至4μm。

Result: LRDs（Little Red Dots）是一类异质性天体群体，具有多样化的连续谱斜率和发射线特征，中心存在超大质量黑洞（BHs）和致密气体云，并伴随恒星形成活动。
...
**TL;DR**: 首次系统普查了169个高倾角中心星系（恒星质量7.3 ≤ log Mstar/Msun ≤ 11.0，红移z < 0.1）的恒星晕（stellar halos），发现其普遍遵循幂律径向分布，且更高质量星系具有更小幂律指数和更大晕质量分数，表明更活跃的并合历史；银河系质量星系的中位恒星晕比例为10% ± 5%，而近邻星系（<25 Mpc）普遍呈现约0.3 dex的晕比例不足，暗示其异常宁静的并合历史。


<details>
  <summary>Details</summary>

Background: 恒星晕（stellar halos）作为盘状星系中唯一缺乏系统性观测表征的主要恒星成分，其研究对揭示星系并合历史具有关键意义。

Data: 数据来源于HSC-SSP Deep光学图像，用于研究169个高倾角中心星系（恒星质量7.3 ≤ log Mstar/Msun ≤ 11.0，红移z < 0.1）的恒星晕（stellar halos）。

Method: 采用HSC-SSP深场光学图像，通过低等照椭圆率（isophotal ellipticity）方法在星系外围检测恒星晕（stellar halo），改进了传统识别技术。

Result: 首次系统普查发现169个高倾角中心星系中93个存在恒星晕（stellar halo），其检测率在log Mstar/Msun > 9.9时达50%，银河系质量星系中超过70%。
...
**TL;DR**: 论文揭示了银河系中央分子区(CMZ)中存在两类分子气体细丝结构：大尺度(10 pc)细丝(LFs)可能反映CMZ整体动力学轨道结构，而普遍存在的小尺度(约1 pc)细丝(SFs)可能源于湍流和剪切动力学作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦银河系中央分子区（Central Molecular Zone, CMZ）作为调控气体从星系盘流向核心区域的关键枢纽，探索其高分辨率下分子气体的整体性质与动力学特征。

Data: 数据来源于ALMA（阿塔卡马大型毫米波/亚毫米波阵列）大型项目ACES（ALMA CMZ探索巡天）的Band 3观测数据，提供了覆盖整个CMZ（中央分子区）的高分辨率（3角秒或0.1秒差距）分子气体图像。

Method: 采用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)的高分辨率(3角秒或0.1秒差距)观测数据，结合形态学和运动学分析，研究了人马座B2(Sgr B2)区域两类丝状结构(大尺度和小尺度)与磁场方向的关联性及其分子组成特征。

Result: 银河系中央分子区(CMZ)的高分辨率观测揭示了其分子气体具有高度纤维状结构，主要分为大尺度(10 pc)纤维结构(LFs)和小尺度(约1 pc)纤维结构(SFs)两类，其中LFs反映了CMZ整体动力学特征，SFs则可能源于湍流和剪切动力学过程。
...
### [7] [Willman 1 Revisited: The Kinematics, Chemistry, and Orbital Properties of a Potentially-Disrupting Dwarf Galaxy](https://arxiv.org/abs/2602.20272)
*Camille Chiu,Marla Geha,William Cerny,Nitya Kallivayalil,Hannah Richstein,Christopher T. Garling,Beth Willman*

**TL;DR**: 通过多手段观测和轨道建模确认威尔曼1（Willman 1）为具有金属丰度弥散（[Fe/H] = -2.45±0.13，σ_[Fe/H] = 0.30±0.11 dex）和内部动力学特征（σ_v = 4.7±1.5 km/s）的矮星系，但其潮汐扰动可能影响质量测量精度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系超暗弱卫星Willman 1（W1）的分类争议（矮星系或星团），通过多波段观测和动力学建模探讨其本质属性。

Data: 数据来源包括凯克望远镜(Keck)的DEIMOS光谱仪、哈勃太空望远镜(HST)的ACS相机测光数据、盖亚(Gaia)自行运动数据以及轨道建模。

Method: 采用凯克望远镜(Keck)的DEIMOS光谱仪进行光谱分析、哈勃太空望远镜(HST)的ACS相机进行测光观测，并结合轨道建模方法重新评估Willman 1的天体性质。

Result: 威尔曼1（Willman 1）具有内部运动学特征、金属丰度弥散且无质量分层，表明其本质为星系（galaxy）。
...
### [8] [ALMA Central molecular zone Exploration Survey (ACES) III: Molecular line data reduction and HNCO and HCO$^{+}$ data](https://arxiv.org/abs/2602.20276)
...
**TL;DR**: 利用ALMA对银河系中心分子区（CMZ）进行多尺度观测，首次揭示了HCO$^{+}$示踪的普遍存在的秒差距尺度线性吸收特征，并发布了包含45个视场的HNCO和HCO$^{+}$数据立方体及高级衍生数据产品。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用ALMA望远镜对银河系中央分子区（Central Molecular Zone, CMZ）进行多尺度连续观测，揭示其分子气体动力学和物理结构特征。

Data: 数据来源于ALMA的12米、7米和总功率阵列（Total Power arrays）对银河系中心约200秒差距区域进行的3毫米波段（Band 3）观测，覆盖尘埃连续谱和多种分子谱线，其中HNCO（4-3）和HCO$^{+}$（1-0）数据以0.2公里/秒的高光谱分辨率获取。

Method: 采用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)的12米、7米和总功率天线阵列进行3毫米(Band 3)观测，结合多尺度连续成像和分子谱线分析技术。

Result: ACES首次揭示了银河系中央分子区(CMZ)中普遍存在的由HCO⁺示踪的秒差距尺度线性吸收特征，其起源尚不明确。
...
### [9] [BASS LVI. Connecting X-ray variability with AGN physical properties and a new path to Cosmological distances](https://arxiv.org/abs/2602.20315)
...
**TL;DR**: 基于BAT AGN光谱巡天(BASS)的134个赛弗特1星系样本，研究发现X射线变率与光度(LX)的相关性同黑洞质量(MBH)一样强，且未发现与爱丁顿比(Eddington ratio)的反相关性，将log(L)-log(exvar)关系的离散度降至0.63 dex但仍不足以作为竞争性标准烛光。


<details>
  <summary>Details</summary>

Background: 活动星系核（AGN）的X射线变率（X-ray variability）与超大质量黑洞质量（MBH）和光度（LX）的反相关关系及其作为宇宙学标准烛光（standard candles）的潜力构成了研究的核心科学主题。

Data: 数据来源于BAT AGN Spectroscopic Survey (BASS)的134个赛弗特1星系(Seyfert 1 galaxies)样本，这是迄今为止同类研究中规模最大的样本，比先前研究使用的样本量大了三倍以上。

Method: 采用BAT AGN光谱巡天(BASS)的134个塞弗特1星系样本，结合能谱分布(SED)拟合改进的爱丁顿比计算，分析X射线变率与光度及黑洞质量的相关性。

Result: X射线变率与活动星系核（AGN）的光度相关性强度与超大质量黑洞质量（MBH）相当，且未发现与爱丁顿比（Eddington ratio）存在预期反相关。
...
### [10] [Quasars behind the disk of M31 galaxy](https://arxiv.org/abs/2602.20318)
*P. Nedialkov,B. F. Williams,V. D. Ivanov,A. Valcheva,Y. Solovyeva,A. Vinokurov,E. Malygin,D. Oparin,O. Sholukhova*

**TL;DR**: 该研究通过光谱观测确认了23个M31（仙女座星系）背景类星体（quasar），包括2个新发现目标，提供了34个类星体的均一化红移（redshift）测量，将可靠红移类星体总数增至124个，发现多数目标消光（reddening）较低且与M31消光图无显著关联，指出低分辨率光谱红移测量存在误差风险。


<details>
  <summary>Details</summary>

Background: 研究旨在增加M31（仙女座星系）后方类星体（quasar）的数量，以探测气体化学组成并获取精确自行运动参考，同时提供基于发射线的可靠且均匀的红移（redshift）测量数据。

Data: 数据来源包括对32个类星体候选者的光谱跟踪观测，以及从档案中补充的17个光谱数据。

Method: 采用发射线光谱分析技术进行可靠且均匀的红移测量，并通过颜色超出法（color excess）计算消光（reddening）。

Result: result：在M31背景中确认了124个具有可靠红移的类星体（quasar），其中两个为新发现（J004029.727+403705.68和J004215.489+412031.52），且大多数类星体显示出低消光（reddening），未探测到高消光区域。
...
### [11] [ALMA Central Molecular Zone Exploration Survey (ACES) I: Overview](https://arxiv.org/abs/2602.20340)
...
**TL;DR**: ACES（ALMA CMZ探测计划）利用高角分辨率（~1.5"）和宽谱段（85-102 GHz）观测银河系中心分子区（CMZ），揭示了从100 pc到0.05 pc尺度上驱动恒星形成与反馈的物理、化学和运动学机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系内部区域的质量流动和能量循环如何通过恒星形成和反馈机制影响星系演化，特别是银河系中心核区（Central Molecular Zone, CMZ）作为唯一可解析这些物理过程的星系核。

Data: 数据来源于阿塔卡马大型毫米波/亚毫米波阵列（ALMA）对银河系中心分子区（CMZ）的观测，包括角分辨率约1.5角秒、谱分辨率0.2-3 km/s的ALMA Band 3（85-102 GHz）巡天数据，覆盖了70多个谱线特征（如HCO+、HNCO、SiO、H40alpha等）以追踪气体的物理、化学和运动学条件。

Method: 采用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)进行高角分辨率(~1.5")、宽谱带(85-102 GHz)和多谱线(70余种分子示踪剂)观测，结合0.2-3 km/s的谱分辨率分析银河系中心分子区(CMZ)的物理化学性质及运动学特征。

Result: ACES（ALMA CMZ Exploration Survey）通过高角分辨率（~1.5"）和宽谱段（85-102 GHz）观测银河系中心分子区（CMZ），旨在揭示大尺度（100 pc）到致密结构（0.05 pc）中恒星形成与反馈过程的物理、化学及运动学条件。
...
### [12] [An automated activity classification tool for optical galaxy spectra](https://arxiv.org/abs/2602.20343)
*C. Daoutis,A. Zezas,E. Kyritsis,K. Kouroumpatzakis,P. Bonfini*

**TL;DR**: 开发了一种基于支持向量机(support vector machine)的自动化诊断工具，利用Hβ、[O III]λ5007和Hα+[N II]λ6548,84的等效宽度(equivalent widths)等特征，无需预处理即可高精度区分恒星形成(SF)、活动星系核(AGN)、低电离核发射线区(LINERs)、复合型和被动星系，整体准确率达83%，并能进一步区分宽线和窄线AGN。


<details>
  <summary>Details</summary>

Background: 开发可靠、通用的星系活动诊断工具对理解星系演化至关重要。

Data: 数据来源为光学发射线比率（optical emission-line ratios）和颜色选择标准（color selection criteria）的文献研究数据。

Method: 基于支持向量机（support vector machine）训练光学发射线比率和颜色选择标准数据开发的自动化诊断工具。

Result: 开发了一种基于支持向量机（SVM）的光学发射线比和颜色选择标准自动诊断工具，能高精度区分恒星形成星系（SF）、活动星系核（AGN）、低电离核发射线区（LINERs）、复合星系和被动星系，总体准确率达83%，且无需预处理（如星光扣除或谱线拟合）。
...
### [13] [First Observation of CO$_2$ Emission and foreground absorption Toward the Galactic Center with JWST](https://arxiv.org/abs/2602.20374)
*J. Qiu,A. Ciurlo,M. R. Morris,P. Vermot,J. L. Bourlot,D. Rouan,A. Togi,T. Do,A. M. Ghez,E. Bron,F. L. Petit,Y. Clénet,E. A. C. Mills,J. R. Lu*

**TL;DR**: 利用詹姆斯·韦伯太空望远镜(JWST)首次在银河系中心两个红外亮区观测到气相CO$_2$吸收，并测得激发温度20-50K、柱密度2$\times$10$^{15}$~cm$^{-2}$及冰气比90，同时检测到C$_2$H$_2$和HCN吸收带，推测吸收介质位于距地球0.15-4kpc的前景团块云中，还发现可能与共生双星相关的点状CO$_2$发射。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用詹姆斯·韦伯太空望远镜(JWST)的高灵敏度探索宇宙中难以探测的气相二氧化碳(CO$_2$)，特别是在银河系中心红外明亮区域的首次观测。

Data: 数据来源于詹姆斯·韦伯太空望远镜（JWST）对银河系中心两个红外明亮区域的高灵敏度观测，包括气态CO$_2$吸收、C$_2$H$_2$和HCN吸收带（分别位于13.7和14.0 $μ$m附近）以及CO$_2$冰吸收的探测。

Method: 采用局部热动平衡模型（LTE model）分析CO$_2$气体激发温度、柱密度和径向速度。

Result: 首次利用JWST高灵敏度观测到银河系中心两个红外明亮区域的气相CO$_2$吸收，通过LTE模型测得CO$_2$气体激发温度为20-50 K、柱密度约2×10$^{15}$ cm$^{-2}$且径向速度接近0。
...
### [14] [Glance: A Comprehensive Framework for Galactic Archaeology](https://arxiv.org/abs/2602.20389)
...
**TL;DR**: 开发了自动化管道Glance，整合多种先进技术分析积分视场光谱数据，应用于NGC 1566中心区域发现其恒星盘显著偏离传统指数模型，挑战了盘形态普适性假设。


<details>
  <summary>Details</summary>

Background: 研究聚焦于理解星系（galaxy）形成与演化历史这一河外天文学核心主题，尤其关注多组分星系结构（structural components）中内恒星盘（inner stellar disk）的真实构型。

Data: data部分：研究使用MUSE（多单元光谱探测器）积分场光谱数据对NGC 1566中心区域进行采样观测。

Method: 采用的核心分析方法为整合恒星种群合成（Fado、Starlight及RemoveYoung后处理）、运动学提取（pPXF、Bayes-LOSVD）和动力学建模（Dynamite）的自动化流程，并集成消光校正、去红移、Voronoi分箱及星云连续谱校正等定制化处理技术。

Result: result：NGC 1566的恒星盘显著偏离传统指数模型，挑战了盘形态普遍性的假设。
...
### [15] [Spatially-resolved interstellar dust properties in the face-on spiral galaxy M 99 as observed by NIKA2](https://arxiv.org/abs/2602.20393)
...
**TL;DR**: 研究发现M 99星系中尘埃（dust）的光谱指数（spectral index）β在弥散区域为1.6-1.7，在致密恒星形成区升至2.3-2.5，表明颗粒（grain）凝聚和硅酸盐与碳质比例变化导致尘埃质量可变β模型比固定β模型高约4倍，且小颗粒比例从中心10%增至盘区15%，与星际辐射场（interstellar radiation field）呈反相关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过多波段观测和建模分析近邻旋涡星系M 99中尘埃颗粒（dust grains）的物理特性及其空间分布规律，以揭示恒星形成环境中大质量尘埃的热平衡状态及其对远红外至毫米波连续谱的贡献机制。

Data: 数据来源包括IRAM 30米望远镜NIKA2设备在1.15和2毫米波段的新观测数据（IMEGIN Guaranteed Time Large Programme），以及覆盖紫外到射电波段的多波长数据。

Method: 采用分层贝叶斯代码HerBIE分解红外至射电SED（光谱能量分布）为尘埃、自由-自由辐射和同步辐射成分，并利用可变毫米波谱指数β的修正黑体（MBB）模型及固定β的THEMIS尘埃模型进行建模。

Result: result：M 99星系中尘埃谱指数（beta）在弥散区域为1.6-1.7，在致密恒星形成区升至2.3-2.5，与颗粒凝聚和硅酸盐-碳质比例变化相关，且可变beta模型得出的尘埃质量比固定beta模型高约4倍。
...
### [16] [Constraining the Sub-Galactic Relationship Between Star Formation and the Hot Interstellar Medium in NGC 4254](https://arxiv.org/abs/2602.20397)
...
**TL;DR**: 通过结合空间分辨的恒星形成历史（SFHs）和贝叶斯X射线光谱拟合，发现NGC 4254（M99）中约40-50%的核心坍缩超新星（CCSNe）能量在星际介质（ISM）中热化为热气体，且其质量加载率随恒星形成率密度（Σ_SFR）呈-1/3次方下降，同时热气体温度（kT）和X射线产生效率（η）分别与Σ_SFR呈0.34±0.10和0.34±0.18次方关系。


<details>
  <summary>Details</summary>

Background: 探讨星系NGC 4254中星族形成活动与热星际介质(ISM)X射线辐射在千秒差距尺度上的关联机制。

Data: 数据来源包括分光光度紫外-红外(UV-IR)数据用于测量亚星系恒星形成率(SFR)，以及钱德拉(Chandra)数据用于生成点源扣除后的弥散X射线辐射图。

Method: 采用贝叶斯X射线光谱拟合(Bayesian X-ray spectral fitting)和空间分辨恒星形成历史(SFHs)建模技术，结合钱德拉(Chandra)数据点源扣除方法分析热星际介质(ISM)发射。

Result: NGC 4254的X射线热气体发射与核心坍缩超新星(CCSNe)约40-50%能量在星际介质(ISM)中的热化一致，且CCSNe喷出物的质量加载率随恒星形成率密度(Σ_{\rm SFR})的-1/3次方递减。
...
### [17] [Highly Efficient Selection of High-Redshift Emission-Line Galaxies for future DESI-like surveys with Deep Multi-band Imaging](https://arxiv.org/abs/2602.20405)
...
**TL;DR**: 通过多波段成像（如Rubin Observatory）和概率随机森林分类器设计颜色筛选方法，显著提高了红移1.1-1.6发射线星系（ELGs）的选取效率（成功率89%、正确红移范围84%、面密度1372 deg⁻²），将重子声波振荡（BAO）尺度参数的不确定性降低约2倍。


<details>
  <summary>Details</summary>

Background: 发射线星系(ELG)作为重子声波振荡(BAO)和大尺度结构(LSS)的重要示踪体，在红移z>1宇宙学研究背景下具有重要意义。

Data: 数据来源于Hyper Supreme-Cam的$grizy$测光数据、COSMOS2020多波段测光红移数据以及暗能量光谱仪(DESI)获取的具有深光谱和宽颜色覆盖范围的两个星系样本。

Method: 采用基于概率随机森林分类器（probabilistic random forest classifier）引导的简单颜色截断法（color cuts）筛选高红移发射线星系（ELGs）。

Result: 通过多波段成像（如鲁宾天文台）结合概率随机森林分类器设计的颜色截断方法，可高效选择高红移发射线星系（ELGs），显著提升红移测量成功率（89%）、正确红移范围成功率（84%）和面密度（1372 deg⁻²），并将重子声波振荡（BAO）尺度参数的误差降低约2倍。
...
### [18] [Chandra X-ray Observations of Quasars with Velocity-Offset Broad Lines: Assessing the Binary Supermassive Black Hole Hypothesis](https://arxiv.org/abs/2602.20438)
*Peter Breiding,Michael Eracleous,Tamara Bogdanović,Sarah Burke-Spolaor,T. Joseph W. Lazio*

**TL;DR**: 研究发现星系合并后期超大质量黑洞（SMBH）形成引力束缚双星系统时，活动吸积黑洞产生的宽线区（BLR）速度偏移现象与X射线光子指数变硬相关，但未明确证实其双星起源。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于星系合并末期超大质量黑洞（SMBH）受动力学摩擦作用形成引力束缚双星系统时，若至少一个黑洞处于活跃吸积状态，其宽线区（BLR）相对宿主星系静止参考系存在运动学偏移的现象。

Data: 数据来源包括对五个邻近（0.1 < z < 0.2）斯隆数字巡天（Sloan Digital Sky Survey）类星体的深度钱德拉（Chandra）X射线观测，以及同一样本中另外11个源的存档钱德拉观测数据。

Method: 采用钱德拉X射线观测数据，结合活动星系核（AGN）基本平面（fundamental plane of black hole activity）和单历元维里质量（single-epoch virial mass）等多种独立技术手段约束超大质量黑洞（SMBH）质量。

Result: 在具有速度偏移宽线区(BLR)的类星体中，X射线光子指数比普通类星体更硬，但光学-X射线谱指数无显著差异。
...
### [19] [ALMA Central Molecular Zone Exploration Survey (ACES)-IV. Data of the two intermediate-width spectral windows](https://arxiv.org/abs/2602.20445)
...
**TL;DR**: 公开了ALMA中央分子区探测计划(ACES)的中等宽度谱窗数据，包含SiO(2-1)等六种分子谱线(~2角秒分辨率、1.7 km/s速度分辨率)，并提供了积分亮度等关键数据产品及形态相关性分析。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)对银河系中心分子区(Central Molecular Zone)进行分子谱线观测，旨在揭示该区域分子气体的物理和化学性质。

Data: 数据来源于ALMA中央分子区探测计划（ACES）大型项目释放的中等宽度谱窗数据，覆盖SiO(2-1)、SO(2_2-1_1)、H13CO+(1-0)、H13CN(1-0)、HN13C(1-0)和HC15N(1-0)等分子谱线跃迁，角分辨率约2角秒，速度分辨率1.7 km s-1。

Method: method：采用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)的中等宽度谱窗观测数据，通过角分辨率约2角秒、速度分辨率1.7 km s-1的分子谱线(如SiO(2-1)、SO(2_2-1_1)等)分析技术。

Result: 释放了ALMA银河中心分子区探测计划（ACES）的中等宽度谱窗数据，覆盖SiO(2-1)、SO(2_2-1_1)、H13CO+(1-0)、H13CN(1-0)、HN13C(1-0)和HC15N(1-0)等分子谱线跃迁，角分辨率约2角秒，速度分辨率1.7 km s-1。
...
### [20] [Detectability of Satellite Planes in Mock Observations of Isolated L* Galaxies](https://arxiv.org/abs/2602.20447)
*Ethan Crosby,Marcel S. Pawlowski,Oliver Müller,Helmut Jerjen*

**TL;DR**: 基于宇宙学模拟（Illustris TNG100-1）构建的孤立L*主星系（L* host galaxies）卫星系统模型表明，卫星平面（satellite planes）的探测率高度依赖于卫星数量（$N_{\mathrm{sat}}$>20）和观测角度（$θ_{\mathrm{rot}}$），其中近侧向或正向排列的平面最易被现有指标（$N_{\mathrm{cor}}$、$b/a$、$v_\mathrm{los}$）识别。


<details>
  <summary>Details</summary>

Background: 研究围绕本地宇宙中L*主星系周围平面共转卫星星系分布的存在性和普遍性展开探讨。

Data: 数据来源于基于宇宙学模拟Illustris TNG100-1构建的孤立L*主星系及其卫星系统的模拟观测样本，包括天球位置和视线速度（LOS velocity）数据。

Method: 基于Illustris TNG100-1宇宙学模拟构建受控模拟观测样本，采用三个关键参数（$N_{\mathrm{sat}}$、$f_{p}$、$θ_{\mathrm{rot}}$）定义卫星平面，并通过三个观测指标（$N_{\mathrm{cor}}$、$b/a$、$v_\mathrm{los}$）评估其可探测性。

Result: 在局部宇宙中，L*主星系周围共转卫星平面（satellite planes）的探测率高度依赖于卫星数量（N_sat）和观测角度（θ_rot），仅当N_sat>20且平面接近侧向或正向时才易被检测。
...
### [21] [Formation of dusty clumps in the torus of active galactic nuclei](https://arxiv.org/abs/2602.20484)
*Xinwu Cao,Renyue Cen,Qingwen Wu,Jiancheng Wu*

**TL;DR**: 提出了一种新模型解释活动星系核(AGN)中尘埃环(torus)的形成机制，认为其由热不稳定性形成的冷尘埃气体团块(clumps)在辐射力与引力平衡下垂直支撑构成，并阐明了尘埃环仅存在于吸积率高于爱丁顿比率1%且光度高于爱丁顿光度0.1%的AGN中的物理原因。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）统一模型中尘埃环（dusty torus）的形成机制，提出由热不稳定性产生的冷气体团块在辐射力支撑下构成动态厚环的新物理模型。

Data: 数据来源于对活动星系核(AGN)中心亚秒差距(sub-pc)尺度上旋转吸积盘中热气体热不稳定性形成冷气体团块(clumps)的数值模拟(simulation)研究。

Method: 基于热不稳定性形成的小型冷尘埃气体团块（cold clumps）在辐射力与引力准静态平衡下构成动态尘埃环（dusty torus）。

Result: 活动星系核(AGN)的尘埃环(dusty torus)由大量垂直方向上受辐射力支撑的小型冷尘埃气体团块(clumps)组成，其形成依赖于吸积率超过爱丁顿比率(Eddington rate)约1%的热气体因热不稳定性凝结。
...
### [22] [Formation of first star clusters under the supersonic gas flow -- III. Environmental trends and halo-to-halo scatter in the Pop III IMF](https://arxiv.org/abs/2602.20690)
*Shingo Hirano,Yusuke Sakai,Hideyuki Umeda*

**TL;DR**: 通过138个宇宙学流体动力学模拟发现，第一代恒星（Pop III）初始质量函数（IMF）随红移、晕质量和重子-暗物质相对速度（SV）系统性变化，高红移、大质量和高SV晕倾向于形成大质量恒星（$\gtrsim10^3$-$10^4\,{\rm M}_\odot$）和星团，否定了单一普适IMF假设。


<details>
  <summary>Details</summary>

Background: 研究聚焦于第一代恒星（Pop III）初始质量函数（IMF）的环境依赖性及其对星系早期电离、化学增丰和大质量黑洞种子形成的调控作用。

Data: 使用138个宇宙学放大流体动力学模拟（cosmological zoom-in hydrodynamics simulations）构建致密云合并树（dense-cloud merger tree），并通过径向气体吸积率剖面（radial gas accretion-rate profile）映射恒星质量来分配第一代恒星质量。

Method: 采用138个宇宙学放大流体动力学模拟（cosmological zoom-in hydrodynamics simulations）构建密集云合并树（dense-cloud merger tree），并通过将径向气体吸积率剖面映射到恒星质量来分配第一代恒星质量，从而获得每个晕（halo）的恒星质量函数而不预先设定任何初始质量函数（IMF）。

Result: 第一代恒星的质量初始函数（IMF）随红移、晕质量和重子-暗物质流速度（SV）变化，排除了单一普适IMF的假设。
...
### [23] [Galaxy evolution in the cosmic web: the relative impact of nodes and filaments in the EAGLE simulation](https://arxiv.org/abs/2602.20733)
*Suman Sarkar,Biswajit Pandey,Apashanka Das*

**TL;DR**: 通过EAGLE宇宙流体动力学模拟发现，星系颜色分布与宇宙网节点（nodes）和纤维结构（filaments）的距离存在双重关联：红星系集中在节点（~2.5Mpc）和纤维核心（~0.75Mpc）附近，而蓝星系分布在边缘，且低质量星系的环境依赖性比高质量系统更显著。


<details>
  <summary>Details</summary>

Background: 研究宇宙网（cosmic web）中节点（nodes）和纤维结构（filaments）对星系演化的差异化影响。

Data: 数据来源于EAGLE宇宙学流体动力学模拟（EAGLE cosmological hydrodynamical simulation），其中节点和纤维结构通过DisPerSE算法识别。

Method: 采用EAGLE宇宙学流体动力学模拟(EAGLE cosmological hydrodynamical simulation)结合DisPerSE结构识别算法和基于信息论(Information-theoretic)的归一化互信息量化方法。

Result: 星系颜色分布与宇宙网节点（nodes）和纤维结构（filaments）的距离存在显著关联，红色星系集中在纤维核心和节点附近，蓝色星系则主要分布于外围区域。
...
### [24] [A multi-wavelength study of nearby starburst galaxy M 82](https://arxiv.org/abs/2602.20786)
*Nilkanth Vagshette,Satish S. Sonkamble,Madhav Patil,Sachindra Naik,Ilani Loubser*

**TL;DR**: 结合多波段观测（远紫外、Hα、中红外和X射线）揭示了M 82星系中尘埃遮蔽的恒星形成活动、II型超新星金属增丰及多相外流驱动的演化过程。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过多波段观测（紫外、光学、红外、X射线）揭示近邻星暴星系M 82的恒星形成活动、尘埃遮蔽效应、金属增丰及多相外流驱动的星系演化过程。

Data: 数据来源于AstroSat卫星搭载的紫外成像望远镜（UVIT）获取的高分辨率远紫外（FUV）图像和钱德拉（Chandra）X射线天文台的存档观测数据。

Method: 采用多波段（FUV、Hα、中红外、X射线）成像与光谱分析（三温区VAPEC模型）相结合的方法。

Result: 结合紫外、光学、红外和X射线观测揭示了M 82星系中尘埃遮蔽与非遮蔽恒星形成、金属富集及多相外流驱动的星暴演化过程。
...
### [25] [A Salpeter IMF and an NFW halo: Disentangling the dark and stellar mass through precise lens modelling of a double-source-plane system reinforces the canonical model of elliptical galaxies](https://arxiv.org/abs/2602.20889)
...
**TL;DR**: 通过双源平面透镜J0946+1006（"Jackpot"透镜）的强透镜分析，发现其恒星质量与光比（mass-to-light ratio）近似恒定且初始质量函数（IMF）归一化符合Salpeter形式，暗物质晕（dark matter halo）与NFW模型一致，恒星质量$M_{\star} = 4.4^{+0.25}_{-0.39}\times 10^{11}\,M_{\odot}$，晕质量$M_{200}^{\rm halo} = 1.11^{+0.37}_{-0.32}\times 10^{13}\,M_{\odot}$，内晕斜率$γ_{\rm in}^{\rm halo} = 1.04^{+0.10}_{-0.14}$，其恒星质量在固定晕质量下比同类文献关系高约0.1 dex。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用双源平面透镜（double source plane lens）J0946+1006的强引力透镜效应，精确测量暗物质密度分布（dark matter density profile）、恒星与晕质量比（stellar-to-halo mass ratio）以及恒星初始质量函数（stellar initial mass function）的归一化参数，以探索星系形成与暗物质晕结构之间的关联。

Data: 数据来源于对双源平面透镜J0946+1006（俗称“Jackpot”透镜）的强引力透镜效应观测。

Method: 采用双源平面透镜（double-source-plane lens）几何分析结合多高斯展开（multi-Gaussian expansion）光模型和椭圆广义NFW（elliptical generalised NFW）暗物质晕模型，通过自由全局质量-光比（mass-to-light ratio）及允许径向M/L梯度约束径向质量分布。

Result: 双源平面透镜J0946+1006的分析支持标准模型：恒星质量-光比近似恒定且初始质量函数（IMF）归一化接近Salpeter型，暗物质晕与NFW轮廓一致。
...
### [26] [Constraints on dynamically-formed massive black holes in Little Red Dots from X-ray non-detections](https://arxiv.org/abs/2602.21002)
*M. Liempi,D. R. G. Schleicher,M. A. Latif,R. Schneider,F. Flammini Dotti,A. Escala,M. C. Vergara*

**TL;DR**: 论文发现高红移（$z \sim 2$）致密星系（Little Red Dots, LRDs）是超大质量黑洞（BH）的理想形成环境，其质量-半径关系（$R_{gal} \propto M_{gal}^{0.6}$）支持碰撞形成模型，且与高红移黑洞质量-半径关系一致，但需特定吸积或遮蔽参数以解释X射线弱辐射特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于红移（redshift）$z \sim 2$处大质量致密星系（Little Red Dots, LRDs）中黑洞（black hole, BH）快速形成的机制及其与早期结构形成模型的矛盾。

Data: 数据来源于X射线未探测到的小红点星系（LRDs）样本，结合模拟的黑洞种子形成和X射线辐射（0.3-7 keV）与观测上限的对比分析。

Method: 采用碰撞驱动（collision-based）的种子黑洞（BH）形成模型，结合X射线辐射（0.3-7 keV）模拟与观测上限对比，约束金属丰度（metallicity）和柱密度（column density）等物理参数。

Result: LRDs（Little Red Dots）是超大质量黑洞（BH）的理想诞生地，尤其符合质量-半径关系$R_{gal} \propto M_{gal}^{0.6}$时，碰撞形成模型支持比本地宇宙更大的种子质量，与高红移BH质量-半径关系一致。
...
### [27] [HINORA II: Testing the Existence of the Council of Giants in ΛCDM simulations](https://arxiv.org/abs/2602.21008)
*Edward Olex,Alexander Knebe,Noam I. Libeskind,Stefan Gottlöber,Dmitry I. Makarov*

**TL;DR**: 研究发现巨人之环（CoG）在冷暗物质（LCDM）模型中属于罕见现象（2.7σ异常），暗示其可能是偶然构型或标准暗物质模拟未捕捉的中尺度物理过程印记。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解释本地宇宙(Local Universe)中巨墙星系环(Council of Giants)等大尺度结构的形成机制及其在冷暗物质(LCDM)宇宙学模型中的出现概率。

Data: 数据来源于暗物质（dark matter, DM）纯模拟（DM-only simulations），包括随机初始条件和约束初始条件的模拟，并选取了分别再现本星系群（Local Group）和本星系区（Local Volume）特性的区域，同时通过半经验关系将K波段光度与晕质量相关联。

Method: 采用HINORA算法分析暗物质(DM)模拟数据，通过半经验关系将K波段光度与晕质量关联，并应用与观测相同的选择标准。

Result: 在LCDM（冷暗物质）模型中观测到的巨环结构（CoG）是一个超过2.7西格玛的异常现象，表明其可能是罕见随机构型或标准暗物质模拟未能捕捉的中等尺度物理过程的印记。
...
### [28] [MHD Simulations of Strongly Magnetized HII Region Evolution: Evidence for Ionized Gas Filamentation](https://arxiv.org/abs/2602.21084)
*Samuel Crowe,Yisheng Tu,Zhi-Yun Li,Jeong-Gyu Kim,John Bally*

**TL;DR**: JWST观测发现银河系中央分子区（Central Molecular Zone）的电离氢区（HII regions）具有高度纤维状形态，磁流体动力学（MHD）模拟表明强磁场（等离子体-β << 1）环境下电离气体从预置高密度团块剥离后沿磁力线形成长丝状结构，其长度和发射度量与观测结果一致。


<details>
  <summary>Details</summary>

Background: 研究探讨了银河系中央分子区（Central Molecular Zone）中高度纤维状电离气体（ionized gas filaments）的形成机制及其与强磁场（plasma-β << 1）的相互作用。

Data: 数据来源于在30 pc$^3$模拟盒中进行的磁流体动力学(MHD)模拟，包括预先在环境介质中放置的密度扰动和通过驱动湍流生成的密度扰动两种模型。

Method: 采用磁流体动力学（MHD）模拟研究强磁化（等离子体-β << 1）环境下HII区演化，通过预设环境介质中的密度扰动或湍流驱动生成密度扰动，分析光致电离等离子体沿磁场线形成长丝状结构的机制。

Result: 在强磁化（等离子体-β << 1）环境中，电离气体被剥离致密团块后沿磁场线形成长丝状结构，其长度和发射测量与中央分子区观测到的丝状结构相似。
...
### [29] [Experimental and computational studies of the hydrogenation of carbon disulfide (CS2) on ice analogues](https://arxiv.org/abs/2602.21126)
*Thanh Nguyen,Germán Molpeceres,Yasuhiro Oba,Marcelino Agúndez,Gisela Esplugues,José Cernicharo,Naoki Watanabe*

**TL;DR**: 实验发现固态二硫化碳（CS2）在无定形固态水（ASW）表面与氢（H）原子通过量子隧穿和连续加氢反应生成硫化氢（H2S）、甲硫醇（CH3SH）等硫化物，解释了星际冰中未检测到CS2的原因。


<details>
  <summary>Details</summary>

Background: 星际介质(ISM)中硫化物（如二硫化碳(CS$_2$)）的表面化学反应及其产物形成机制。

Data: 数据来源于实验室实验（laboratory experiments）和计算模拟（computational calculations），其中实验部分在非晶态固态水（amorphous solid water, ASW）表面研究了固态二硫化碳（CS$_2$）与氢（H）原子的反应。

Method: 采用实验室实验结合计算模拟方法研究了非晶态固态水(ASW)表面二硫化碳(CS$_2$)与氢(H)原子的反应机制，揭示了量子隧穿(quantum tunneling)引发的初始反应及后续加氢反应路径。

Result: 在星际冰表面，二硫化碳(CS$_2$)与氢(H)原子通过量子隧穿(quantum tunneling)引发反应，随后发生连续加氢反应，生成硫化氢(H$_2$S)、甲硫醇(CH$_3$SH)及少量二硫代甲酸(HC(S)SH)和甲二硫醇(CH$_2$(SH)$_2$)。
...
### [30] [Scattering, Migration, Re-circularization and Relaxation to Build Out Galaxy Disks with Exponential Profiles](https://arxiv.org/abs/2602.21171)
*Curtis Struck,Bruce G. Elmegreen,Elena DOnghia*

**TL;DR**: 星际云团（interstellar clouds）或大质量团块（massive clumps）对恒星的散射作用使恒星轨道偏心率（eccentricity）达到稳态分布，通过再环化（recircularization）限制速度弥散并维持指数型盘结构（exponential disk profile）的长期稳定。


<details>
  <summary>Details</summary>

Background: 星际云团或大质量团块对恒星的散射作用通过调节恒星轨道偏心率维持盘结构稳定性，揭示了星系盘中恒星速度弥散度受限和指数型径向分布的物理机制。

Data: 数据来源于对恒星与星际云团（interstellar clouds）或大质量团块（massive clumps）相互作用的简化盘系统（disk system）模拟计算，排除了螺旋波（spiral waves）、恒星形成（star formation）等过程的干扰。

Method: 采用轨道散射（orbital scattering）理论模型分析星际云团（interstellar clouds）与恒星相互作用对轨道偏心率和速度弥散（velocity dispersion）的稳态分布影响。

Result: 散射作用使恒星轨道偏心率（eccentricity）达到稳态分布，此时偏心率增减速率相等。
...
### [31] [The JWST Resolved Stellar Populations Early Release Science Program. IX. The RR Lyrae Population in WLM with HST and JWST](https://arxiv.org/abs/2602.21205)
...
**TL;DR**: 利用HST和JWST观测数据，首次在近红外波段（NIRCam F090W和F150W滤光片）校准了天琴RR变星(RR Lyrae)的周期-威西特-金属丰度(PWZ)关系，测得WLM矮星系的距离模数为24.85±0.05（0.93±0.02 Mpc），比先前测量更近且与盖亚(Gaia)尺度一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）的近红外观测数据，评估JWST在表征天琴RR型变星（RR Lyrae）方面的效能，并校准其周期-威森海特-金属丰度（PWZ）关系。

Data: 数据来源为哈勃太空望远镜（HST）的先进巡天相机（ACS）和詹姆斯·韦伯太空望远镜（JWST）的近红外相机（NIRCam）对邻近矮星系WLM的存档观测数据。

Method: 采用马尔可夫链蒙特卡洛（MCMC）模板拟合技术从RR Lyrae时变数据中提取周期、振幅和平均星等，并直接对比HST与JWST重叠观测数据以校准近红外相机（NIRCam）F090W和F150W滤光片的周期-威森海特-金属丰度（PWZ）关系。

Result: result部分：研究发现NIRCam数据可可靠识别天琴RR型变星(RR Lyrae)，但短基线观测下光变曲线模板拟合困难，HST数据拟合得出WLM距离模数为24.85±0.05（0.93±0.02 Mpc），比先前测量更近且与盖亚(Gaia)尺度一致，而NIRCam滤光片中的PWZ校准虽存在较大不确定性，但属首次实现。
...
### [32] [Cloaking Cosmic Light](https://arxiv.org/abs/2602.20248)
*Nemanja Kaloper*

**TL;DR**: 研究发现暗域壁（dark domain walls）上可见光子与暗光子（dark photons）的局域转化会影响宇宙微波背景功率谱并改变低频光偏振，进而影响哈勃常数的宇宙学测定。


<details>
  <summary>Details</summary>

Background: 研究探讨了可见光子与暗光子（dark photons）通过耦合规范陈-西蒙斯项（gauge Chern-Simons terms）的暗畴壁（dark domain walls）时的转换机制及其对宇宙学观测的影响。

Data: 研究中使用的数据来源为理论模拟（simulation）数据，基于暗畴壁（dark domain walls）与可见及暗$U(1)$规范场的规范陈-西蒙斯项（gauge Chern--Simons terms）耦合的拓扑形式（top form）模型。

Method: 采用核心分析方法研究暗光子（dark photons）转换效应，基于拓扑形式（top form）与规范陈-西蒙斯项（gauge Chern-Simons terms）耦合的物理模型，分析光子频率在壁静止框架下的转换率及偏振旋转（Δϑ ~ 10^-3弧度）。

Result: 当光子穿越暗畴壁（dark domain walls）时，会转化为暗光子（dark photons），转化率与光子频率呈锐利函数关系并在紫外截断（ultraviolet cutoff）处消失。
...
### [33] [Cosmological Constraints on Neutrino Masses in Quintessential Inflation](https://arxiv.org/abs/2602.20349)
*Jamerson Rodrigues,Gabriel Rodrigues,Felipe B. M. dos Santos,Simony Santos da Costa,Jailson Alcaniz*

**TL;DR**: 该研究通过α吸引子（α-attractor）模型将暴胀时期与当前暗能量主导时期统一描述，利用改进的CLASS程序结合CMB、BAO和Ia型超新星数据，得出中微子质量总和（∑mν）在平坦空间几何下的严格上限为0.067 eV，并预测未来观测将显著提升对αQI参数和中微子质量的测量精度。


<details>
  <summary>Details</summary>

Background: 研究探讨了精质暴胀（quintessential inflation）模型如何统一描述宇宙早期暴胀阶段和当前暗能量主导阶段，通过单一标量场将这两个加速膨胀时期联系起来。

Data: 数据来源包括宇宙微波背景辐射（CMB）、重子声学振荡（BAOs）和Ia型超新星观测数据。

Method: 采用改进版玻尔兹曼求解器CLASS（Cosmic Linear Anisotropy Solving System）计算宇宙学可观测量，并结合贝叶斯参数估计方法分析宇宙微波背景辐射（CMB）、重子声波振荡（BAO）和Ia型超新星数据。

Result: 在α吸引子（α-attractor）情景下，精粹暴胀（quintessential inflation）模型通过打破暗能量状态方程与中微子总质量之间的简并性，给出了严格上限：平直空间几何下∑m_ν<0.067 eV，包含曲率时∑m_ν<0.116 eV。
...
### [34] [IR-Safe and IR-Resummed Bispectra Before and After Reconstruction in Unified Lagrangian Perturbation Theory](https://arxiv.org/abs/2602.20431)
*Naonori Sugiyama*

**TL;DR**: 基于统一拉格朗日扰动理论(ULPT)建立了包含重构前后密度场自/互双谱的暗物质实空间双谱解析框架，揭示了非线性重子声学振荡(BAO)抑制的物理机制并完整解析了wiggle-wiggle项的阻尼结构。


<details>
  <summary>Details</summary>

Background: 研究主题为基于统一拉格朗日扰动理论(ULPT)构建暗物质双谱(bispectrum)的统一解析框架，重点解决非线性重子声学振荡(BAO)阻尼的物理机制与红外(IR)效应问题。

Data: 数据来源于基于统一拉格朗日扰动理论（ULPT）的数值模拟位移映射卷积积分。

Method: 基于统一拉格朗日扰动理论（ULPT）构建了分析暗物质双谱（bispectrum）的统一解析框架，通过分离雅可比偏差（Jacobian deviation）与位移映射效应（displacement-mapping effect）实现非线性源双谱建模和红外（IR）效应精确抵消。

Result: 基于统一拉格朗日微扰理论(ULPT)构建的解析框架揭示了非线性重子声波振荡(BAO)抑制的物理机制，并完整解析了wiggle-wiggle项对红外重整化双谱的贡献结构。
...
### [35] [Machine-learning cosmological parameters by eROSITA data](https://arxiv.org/abs/2602.20483)
...
**TL;DR**: 首次利用机器学习算法从eROSITA X射线观测的星系团数据中推断宇宙学参数（Ωm=0.30+0.03−0.02、σ8=0.81±0.01、h0=0.710±0.004），发现团内性质相关性蕴含独立于丰度的宇宙学信息，且h0与普朗克结果存在显著偏差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用机器学习算法从eROSITA X射线观测的星系团数据中推断宇宙学参数（cosmological parameters），探索星系团内禀性质与宇宙学信息之间的关联。

Data: 数据来源于eROSITA的eFEDS和eRASS1星表在不同红移处观测到的X射线特征（气体光度、质量和温度），以及Magneticum多宇宙学流体动力学模拟生成的模拟星系团目录。

Method: 采用随机森林（Random Forest）算法结合磁流体动力学多宇宙学模拟（Magneticum multi-cosmology hydrodynamical simulations）生成的模拟星团目录进行训练，并应用于eROSITA观测的X射线特征（气体光度、质量和温度）。

Result: 首次利用机器学习算法从eROSITA对星系团的X射线观测中推断出宇宙学参数（$Ω_{\rm m}=0.30^{+0.03}_{-0.02}$、$σ_8=0.81\pm0.01$和$h_0=0.710\pm0.004$），且团内性质间的相关性蕴含了超出单纯团丰度之外的宇宙学信息。
...
### [36] [Bypassing the Lyth Bound with Entangled Gravitons: Primordial Signatures and Late-Time Noise](https://arxiv.org/abs/2602.20734)
*Shingo Akama,Chunshan Lin*

**TL;DR**: 研究发现原初引力子（primordial gravitons）在动力学解耦引力扇区中的量子纠缠可通过参数化增强暴胀期间的张量功率谱（tensor power spectrum），并产生可观测的振荡特征、单场一致性关系破坏及压缩极限双谱增强，同时预言该纠缠效应可能导致引力波干涉仪中的随机噪声增强。


<details>
  <summary>Details</summary>

Background: 研究探讨了暴胀期间原始引力子（primordial gravitons）的量子纠缠（quantum entanglement）如何通过动态解耦的引力扇区参数化增强张量功率谱（tensor power spectrum）。

Data: 研究中使用的数据来源为理论模拟（simulation）数据，基于量子纠缠和原初引力波（primordial gravitons）的动力学解耦模型。

Method: 采用量子纠缠（quantum entanglement）和约化密度矩阵（reduced density matrix）分析原初引力子（primordial gravitons）在动态解耦引力扇区（dynamically decoupled gravitational sectors）中的参数化增强效应。

Result: 量子纠缠（quantum entanglement）引发的原初引力子（primordial gravitons）参数化增强效应可显著提升暴胀期间的张量功率谱（tensor power spectrum）。
...
### [37] [Cylindrical cosmological simulations with StePS](https://arxiv.org/abs/2602.20787)
*Gábor Rácz,Viola H. Varga,Balázs Pál,István Szapudi,István Csabai,Till Sawala*

**TL;DR**: 提出了一种采用S^1×R^2（"slab"）拓扑结构的紧凑化宇宙学模拟框架（StePS），通过沿单轴周期性边界和径向立体投影压缩技术，有效解决了传统立方体周期性边界导致的引力长程力偏差问题，特别适用于柱对称结构（如宇宙纤维）和各向异性宇宙模型的模拟。


<details>
  <summary>Details</summary>

Background: 研究关注宇宙全局拓扑结构（global topology）对长程引力（long-range gravitational forces）的影响，特别是非平凡拓扑（non-trivial topologies）在引力模拟中的关键作用。

Data: 数据来源于采用立体投影压缩的周期性圆柱体（periodic cylinder）中的共动空间（comoving space），通过牛顿动力学演化粒子，并利用平滑变化的空间和质量分辨率抑制自由外边界（free outer boundary）的边缘效应。

Method: 采用逆球面投影（inverse stereographic projection）沿周期性圆柱径向压缩共动空间，结合牛顿动力学演化粒子，并利用空间与质量分辨率随半径平滑变化抑制边界伪影。

Result: 引入了一种采用S1×R2（"slab"）拓扑结构的紧凑化模拟框架，该框架沿单一轴周期性而在垂直方向具有无限拓扑结构，适用于模拟具有柱对称性的系统或各向异性宇宙学模型。
...
### [38] [Constraints on CMBR flux due to high-z dust emission](https://arxiv.org/abs/2602.20916)
*Martin Lopez-Corredoira*

**TL;DR**: 研究发现高红移(z=15-20)尘埃对宇宙微波背景辐射(CMBR)单极子通量的污染在β≥0.5和σ_d≥0.8条件下应小于1.3%(95%置信水平)。


<details>
  <summary>Details</summary>

Background: 研究探讨了高红移(z=15-20)尘埃对宇宙微波背景辐射(CMBR)单极子通量的潜在污染效应及其观测限制。

Data: 使用COBE-FIRAS（Cosmic Background Explorer-Far Infrared Absolute Spectrophotometer）单极子数据拟合不同尘埃发射率谱指数（β）和前身云红移离散度（σ_d）的模型。

Method: 采用COBE-FIRAS单极子数据拟合不同尘埃发射率谱指数（β）和前身云红移弥散（σ_d）的模型，约束高红移尘埃对CMBR（宇宙微波背景辐射）通量的污染水平。

Result: 在合理的尘埃发射率谱指数（β）和前体云红移离散度（σ_d）参数范围内，高红移尘埃对宇宙微波背景辐射（CMBR）通量的污染应小于1.3%（95%置信水平）。
...
### [39] [Simulations of the 21cm emission line for upcoming large-scale HI galaxy surveys](https://arxiv.org/abs/2602.21058)
...
**TL;DR**: 开发了一种基于半解析模型（GAEA和L-Galaxies）的中性氢（HI）21厘米发射线模拟框架，验证了与本地宇宙观测的一致性，并预测了未来SKA巡天的HI星系红移分布。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发预测中性氢（HI）21cm发射线的模拟框架，以支持平方公里阵列（SKA）天文台未来大规模星系巡天的科学目标。

Data: 数据来源为基于Millennium Simulation（千年模拟）套件的合并树（merger trees），并结合了星系演化与组装（GAEA）和L-Galaxies半解析模型。

Method: 采用半解析模型（semi-analytical models）结合Millennium Simulation（千年模拟）的合并树（merger trees），以GAEA和L-Galaxies为基础框架模拟中性氢21厘米发射线（21cm emission line）。

Result: 开发了一种基于半解析模型（GAEA和L-Galaxies）的中性氢（HI）21厘米发射线模拟框架，能准确复现本地宇宙观测数据并预测未来SKA巡天的红移分布。
...
### [40] [Revisiting CPL with sign-switching density: to cross or not to cross the NECB](https://arxiv.org/abs/2602.21169)
*Mine Gökçen,Özgür Akarsu,Eleonora Di Valentino*

**TL;DR**: 基于DESI DR2 BAO、CMB和SNeIa数据，研究发现动态暗能量（DE）存在从早期幻影态（phantom-like）向晚期精质态（quintessence-like）转变的$3.2σ$-$3.4σ$显著偏好，并通过引入负能量密度相（negative DE phase）验证了穿越零能量条件边界（NECB）的物理意义。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙学中动态暗能量（DE）的演化行为，特别是其状态方程（CPL参数化）从早期类幻影（phantom-like）到晚期类精质（quintessence-like）的转变，并关注负能量密度条件下幻影分界线（PDL）与零能量条件边界（NECB）的物理意义差异。

Data: 数据来源于DESI DR2 BAO（重子声波振荡）测量数据，并结合了CMB（宇宙微波背景）和SNeIa（Ia型超新星）数据。

Method: 采用参数化暗能量状态方程（CPL）及其两种扩展模型（CPL→−Λ和sCPL），通过引入负能量密度相来检验穿越零能量条件边界（NECB）的数据驱动偏好。

Result: DESI DR2 BAO测量结合CMB和SNeIa数据显示出对CPL参数化状态方程描述的动力学暗能量(DE)存在$3.2σ$-$3.4σ$的偏好，其重建结果最显著特征是暗能量状态从早期类幻影(phantom-like)状态向晚期类精质(quintessence-like)行为的明显转变。
...
### [41] [Neural Bayesian updates to populations with growing gravitational-wave catalogs](https://arxiv.org/abs/2602.20277)
*Noah E. Wolfe,Matthew Mould,John Veitch,Salvatore Vitale*

**TL;DR**: 提出利用变分神经后验估计（variational neural posterior estimation）方法快速更新引力波探测器观测到的双黑洞（binary black holes）群体分布推断，验证了不同更新频率的可靠性并发现数据量越大更新效果越好，同时拓展了贝叶斯更新在天体物理群体模型中的科学应用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用贝叶斯更新方法解决引力波（gravitational-wave）数据分析中高维天体物理源群体模型计算效率瓶颈问题。

Data: 数据来源包括引力波探测器观测到的真实双黑洞（binary black holes）目录和模拟目录。

Method: 采用变分神经后验估计（variational neural posterior estimation）技术进行贝叶斯更新，以推断引力波探测器观测到的双黑洞（binary black holes）群体参数分布。

Result: 利用变分神经后验估计（variational neural posterior estimation）方法实现了引力波探测器数据中双黑洞（binary black holes）群体推断的快速贝叶斯更新，并验证了不同更新频率（按新源目录、按月或按信号）的可靠性。
...
### [42] [Compute System Organization for High Frequency High Order Wavefront Sensing and Control](https://arxiv.org/abs/2602.20298)
*Barry Lyu,Vaibhavi Manjarekar,Nathaniel Bleier*

**TL;DR**: 提出通过将高精度波前传感与控制（HOWFSC）任务卸载至日地L2点伴飞计算卫星，解决宜居世界天文台（HWO）因现有星载硬件算力不足导致的控制频率瓶颈，实现从亚赫兹到百赫兹级的性能提升。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决宜居世界天文台（HWO）在实现10^-10级超高对比度观测时面临的长期波前稳定性维持挑战，核心科学主题为通过协同飞行计算卫星架构突破现有星载硬件对高阶波前传感与控制（HOWFSC）计算性能的限制。

Data: 数据来源于地面模拟（simulation）分析及现有辐射硬化处理器性能测试。

Method: 采用高精度波前传感与控制（HOWFSC）算法并结合专用协同飞行计算卫星架构，通过辐射耐受高性能处理器提升控制频率。

Result: 将高精度波前传感与控制（HOWFSC）任务卸载至共轨计算卫星可显著提升控制频率至数十甚至数百赫兹，同时降低主观测平台风险。
...
### [43] [70 years of spectroscopy of the photosphere and the solar chromosphere at the Pic du Midi Observatory (1956-2026)](https://arxiv.org/abs/2602.20641)
*Jean-Marie Malherbe*

**TL;DR**: 比拉山天文台(Baillaud dome)在1965年左右通过更新光谱仪和观测台设备实现了对日冕(corona)观测的专业化转型，同时该台站基础设施在几年内经历了革命性升级。


<details>
  <summary>Details</summary>

Background: 研究聚焦于法国比利牛斯山天文台(Pic du Midi)太阳观测设施的历史发展及其专用仪器配置的演变过程。

Data: 数据来源于皮克杜米迪天文台（Pic du Midi）的多功能赤道仪巴约穹顶（Baillaud cupola）中的光谱日冕仪（spectro coronagraph）、1956年后专门用于光球层（photosphere）和色球层（chromosphere）观测的仪器、国际地球物理年期间建立的太阳光谱实验室配备的4米和9米焦距光谱仪（spectrographs）、1961年东部塔楼穹顶安装的8米光谱仪以及1965年后巴约穹顶更新的日冕观测专用设备。

Method: 采用光谱日冕仪（spectro coronagraph）和不同焦距（4米、9米、8米）的光谱仪（spectrograph）进行太阳光球层、色球层和日冕的光谱观测。

Result: 皮克杜米迪天文台在1965年前后通过更新设备实现了对日冕（corona）观测的专业化转型。
...
### [44] [F10.7 Index Prediction: A Multiscale Decomposition Strategy with Wavelet Transform for Performance Optimization](https://arxiv.org/abs/2602.20712)
...
**TL;DR**: 提出基于小波分解的F10.7指数预测新方法（Combination 6），通过整合F10.7及其多级近似与细节信号，显著优于现有方法（包括S. Yan et al. (2025)和三种业务模型），且证实国际太阳黑子数（ISN）对预测无增益。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用小波分解（wavelet decomposition）提升F10.7太阳射电流量指数的预测精度，并探索国际太阳黑子数（International Sunspot Number, ISN）对预测性能的影响。

Data: 数据来源包括用于训练、验证和测试的Dataset A以及用于评估泛化能力的Dataset B，同时使用了F10.7指数及其小波分解后的近似和细节信号、国际太阳黑子数(ISN)及其小波分解信号。

Method: 采用小波分解（wavelet decomposition）将F10.7指数及其分解的近似信号和细节信号输入iTransformer模型，并结合国际太阳黑子数（ISN）及其小波分解信号评估其对预测性能的影响。

Result: 基于小波分解的F10.7指数预测方法（Combination 6）通过整合F10.7及其一至五级近似和细节信号，显著优于仅使用F10.7、仅近似或细节信号的基线方法，且预测性能随信号层级增加而提升。
...
### [45] [Gaia Data Release 4: Modelling of drift-scan related effects in Gaia's point spread function](https://arxiv.org/abs/2602.20906)
...
**TL;DR**: 开发了考虑盖亚(Gaia)空间望远镜独特点扩散函数(PSF)特性的半解析模型，通过建模扫描方向运动模糊效应并校准CCD位置和光源颜色的依赖关系，显著提升了11-13星等范围内天体测量和测光精度，已应用于第四期数据发布。


<details>
  <summary>Details</summary>

Background: 研究聚焦于建立精确的点扩散函数（PSF）模型以解决盖亚（Gaia）空间望远镜因采用漂移扫描模式和时间延迟积分（TDI）导致的独特系统性和周期性畸变问题。

Data: 数据来源为盖亚(Gaia)空间望远镜在漂移扫描模式(drift-scan mode)和时间延迟积分(TDI)模式下获取的星体观测数据。

Method: 采用半解析模型（semi-analytic model）对点扩散函数（PSF）进行建模，其中沿扫描方向和跨扫描方向的恒星图像运动模糊效应通过解析方法建模，而PSF形状对源颜色和CCD位置的依赖性则通过经验校准确定。

Result: result：开发了一种半解析点扩散函数（PSF）模型，显著提升了11-13星等范围内观测数据的建模精度，降低了天体测量和测光数据产品的系统误差。
...
### [46] [Evidence of a non-equipartition energy regime in 1803+784 Core-shift and Faraday rotation measurements from simultaneous multi-frequency polarimetric VGOS observations](https://arxiv.org/abs/2602.20746)
*V. Pérez-Díez,I. Martí-Vidal,E. Albentosa-Ruiz,R. Bachiller*

**TL;DR**: 通过VGOS对耀变体(blazar) 1803+784的3-11 GHz同步宽带偏振观测，发现其核心位移幂律指数(k_r=0.73)显著偏离能量均分假设预测值(k_r=1)，并首次分离出核心区内部(RM_I=121±8 rad m^-2)与外部介质(RM_E=-44±9 rad m^-2)的法拉第旋转分量，证实厘米波核心区未满足能量均分条件或锥形喷流几何结构。


<details>
  <summary>Details</summary>

Background: 研究聚焦于验证活动星系核（AGN）喷流在厘米波主导区域是否满足粒子与磁场能量密度均分（equipartition）假设，这对喷流物理和甚长基线干涉测量（VLBI）天体测量学具有重要意义。

Data: 数据来源于甚长基线干涉测量全球观测系统（VLBI Global Observing System, VGOS）对耀变体1803+784在3-11 GHz频段同时进行的宽带全偏振观测。

Method: 采用VLBI全球观测系统(VGOS)对耀变体1803+784进行3-11 GHz同步宽带全偏振观测，通过专用校准流程处理数据后，进行模型拟合和多频段成像分析，测量了频率依赖性核心位移并绘制了谱指数和旋转量(RM)分布图。

Result: 在厘米波核心区域观测到的核心位移幂律指数(k_r = 0.73^{+0.12}_{-0.19)显著偏离理论平衡值(k_r = 1)，表明该区域未满足能量均分条件或锥形喷流几何假设。
...
### [47] [Galactic Center Gamma-Ray Excess from a Generic Triaxial Halo](https://arxiv.org/abs/2602.20252)
*Leo Qiyuan Hu,Ilias Cholis,Yi-Ming Zhong*

**TL;DR**: 通过拟合费米伽马射线卫星(Fermi-LAT)观测数据，发现银河系中心过剩辐射(GCE)的形态学特征更符合来自三轴且倾斜的暗物质晕(Dark Matter Halo)而非恒星晕的湮灭信号，证实GCE谱形和内尖峰特征对暗物质晕的三轴性和倾斜度变化具有鲁棒性。


<details>
  <summary>Details</summary>

Background: 研究探讨了银河系暗物质晕（dark matter halo）的三轴性（triaxiality）和倾斜（tilt）对银河系中心过剩（GCE）伽马射线形态的影响及其与暗物质湮灭信号的兼容性。

Data: 数据来源为费米伽马射线空间望远镜（Fermi-LAT）的伽马射线观测数据。

Method: 通过拟合费米伽马射线空间望远镜(Fermi-LAT)的伽马射线观测数据，测试银河系中心过剩(GCE)形态与三轴暗物质晕的兼容性。

Result: 银河中心过剩（GCE）的伽马射线数据形态可区分暗物质晕（dark matter halo）的三轴性和倾斜性选择，且GCE更可能源自三轴倾斜暗物质晕而非恒星晕。
...
### [48] [The Magnetic Filling in MAD Simulations and its Impact on the Jet in M87](https://arxiv.org/abs/2602.20893)
*Felix Glaser,Christian M. Fromm,Yosuke Mizuno,Matthias Kadler,Karl Mannheim*

**TL;DR**: 研究发现磁化受阻吸积盘（MAD）模拟中提升初始磁场填充因子（filling factor）能显著增强喷流的电磁能量贡献和角动量外输运，并产生与M87*观测相符的高线性偏振比例及喷流形态。


<details>
  <summary>Details</summary>

Background: 研究聚焦于磁滞吸积盘（MADs）在黑洞喷流形成模拟中的应用，特别针对低光度活动星系核（AGN）如M87*的建模。

Data: 研究中使用的数据来源于五组三维广义相对论磁流体动力学（GRMHD）模拟，这些模拟初始化采用大尺度环面（tori）并浸入弱极向磁场（poloidal magnetic fields）中。

Method: 采用三维广义相对论磁流体动力学(GRMHD)模拟，通过调节初始磁场空间范围（填充因子）来研究磁能含量对吸积盘和喷流的影响。

Result: 初始磁填充因子（filling factor）的增加显著提升了喷流中的电磁能量贡献和向外角动量传输，并导致下游喷流亮度明显增强。
...
### [49] [Cosmic Ray Spectra and Metal Budget Regulated by the Galactic Wind](https://arxiv.org/abs/2602.16214)
*Yusaku Fukumoto,Katsuaki Asano,Jiro Shimoda*

**TL;DR**: 研究发现银河风（Galactic wind）的平流效应通过最大速度约700 km/s的速度分布再现了宇宙线（cosmic ray）谱在几百GV处的硬化和几TV处的软化，且无需改变扩散系数的幂律依赖关系，同时揭示了银河盘上方3-5 kpc处存在指数约2的硬宇宙线谱（hard CR spectrum），该谱有利于解释费米气泡（Fermi bubbles）的伽马射线谱，并表明银河风对维持盘内金属丰度至关重要，但导致晕中铍氧比（Be/O）高于盘内气体。


<details>
  <summary>Details</summary>

Background: 研究聚焦银河系风（Galactic wind）对局域宇宙线（cosmic ray）能谱的平流效应（advection effect）及其对银河系物质循环（matter circulation）和金属丰度（metal abundance）维持的影响。

Data: 数据来源于银河风（Galactic wind）对局部宇宙射线（cosmic ray）能谱的平流效应（advection effect）观测，以及基于速度剖面（velocity profile）最大速度约700 km/s的模拟结果。

Method: 采用银河风(galactic wind)速度剖面模型，最大速度约700 km/s，在不改变扩散系数幂律依赖性的情况下重现了宇宙线(cosmic ray)谱在数百GV处的硬化(hardening)和数TV处的软化(softening)。

Result: 银河风(galactic wind)的最大速度约为700 km/s，无需在扩散系数的幂律依赖中引入间断即可重现宇宙线(cosmic ray)谱在几百GV处的硬化(hardening)和几TV处的软化(softening)。
...
### [50] [XRISM Discovery of Multiple Ionized Fe-K Emission and Absorption Components in Centaurus A](https://arxiv.org/abs/2602.20898)
...
**TL;DR**: 首次在近邻射电星系半人马座A(Centaurus A)中清晰探测到电离态铁K(Fe-K)发射和吸收成分，发现一个源自黑洞附近(D=0.02 pc)的宽发射成分(σ=3000 km/s，红移+3400 km/s)和两个源自较远区域(D=0.1 pc)的窄发射成分(σ=500 km/s，红移+2600 km/s和蓝移-1500 km/s)，并探测到两个蓝移吸收线(~7.1 keV和~10.6 keV)，为研究低光度射电星系核周环境提供了新基准。


<details>
  <summary>Details</summary>

Background: 首次在近邻射电星系半人马座A中清晰探测到电离铁K发射和吸收成分，揭示了活动星系核(AGN)统一模型下低光度射电星系核周环境的高能物理过程。

Data: 数据来源于邻近射电星系半人马座A的高分辨率XRISM/Resolve探测器观测到的X射线数据，重点分析了6.5-6.9 keV波段内电离铁（Fe-K）的发射和吸收成分。

Method: 采用高分辨率XRISM/Resolve探测器对电离态铁K线(Fe-K)发射与吸收成分进行光谱分析，结合光致电离(photo-ionization)和碰撞电离(collisional ionization)模型解释不同速度分量。

Result: 首次在近邻射电星系半人马座A中清晰探测到电离铁K(Fe-K)发射和吸收成分，其中6.5-6.9 keV能段存在三个Fe XXV和Fe XXVI发射成分：一个宽(σ=3000 km/s)且红移(+3400 km/s)的成分源自距黑洞0.02 pc处，两个窄(σ=500 km/s)且分别红移(+2600 km/s)和蓝移(-1500 km/s)的成分源自0.1 pc处。
...
### [51] [Likelihood-Based One-Class Scoring in CWT Latent Space for Confusion-Limited LISA Gravitational-Wave Detection](https://arxiv.org/abs/2602.20212)
*Jericho Cain*

**TL;DR**: 该论文通过对比几何风格评分与似然风格隐密度评分方法，发现在LISA混淆限制数据中基于自编码器（AE）隐变量的似然评分（ROC-AUC 0.8555±0.0181，PR-AUC 0.9219±0.0118）显著优于流形距离方法（ROC-AUC 0.7663±0.0450，PR-AUC 0.8667±0.0255），证实了显式隐密度建模在此类场景中的优越性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于在LISA（Laser Interferometer Space Antenna）混淆限制（confusion-limited）时域数据中，利用连续小波变换（CWT）尺度图进行可分辨源（resolvable-source）检测的单类（one-class）评分方法。

Data: 数据来源于LISA（Laser Interferometer Space Antenna）时间序列数据，通过连续小波变换（CWT）转换为时频谱图（scalograms）。

Method: 采用连续小波变换（CWT）尺度图表示LISA时序数据，并基于几何风格评分与似然风格隐密度评分进行对比分析，同时评估了形态增强和对比变体方法。

Result: 在混淆受限的LISA时域数据中，基于自动编码器(AE)潜在空间密度建模的似然评分法(ROC-AUC 0.8555±0.0181，PR-AUC 0.9219±0.0118)显著优于基于几何距离的流形外方法(ROC-AUC 0.7663±0.0450，PR-AUC 0.8667±0.0255)。
...
### [52] [Complex Inflaton Potentials with Nonminimal Coupling: Robust Inflation and Geometric Reheating](https://arxiv.org/abs/2602.20355)
*S. D. Campos*

**TL;DR**: 研究了由非最小耦合（nonminimally coupled）到引力的复标量场驱动的暴胀情景，发现其非厄米（non-Hermitian）虚部势能通过几何机制实现暴胀和再加热，且谱指数（spectral index）和张量标量比（tensor-to-scalar ratio）与普朗克2018数据一致。


<details>
  <summary>Details</summary>

Background: 研究探讨了由复标量场（complex scalar field）非最小耦合引力和非对称复势驱动的暴胀情景，重点关注非厄米（non-Hermitian）变形如何通过虚部势能编码耗散效应并影响宇宙学背景演化。

Data: 研究采用数值模拟（simulation）方法，基于非最小耦合（nonminimal coupling）复标量场和复势能（complex potential）的动力学方程进行求解，其中背景演化由实部势能控制，虚部势能作为非厄米（non-Hermitian）变形引入耗散效应。

Method: 采用非厄米标量-张量框架，通过复标量场与引力的非最小耦合（nonminimal coupling）及非对称复势（non-symmetric complex potential）实现暴胀，其中实部势能驱动与α吸引子T模型（α-attractor T-models）兼容的暴胀，虚部势能作为非厄米变形（non-Hermitian deformation）编码耗散效应。

Result: result：复标量场非厄米变形模型在慢滚阶段产生谱指数$n_s\simeq 0.968-0.971$和张量标量比$r<10^{-3}$，与普朗克2018数据吻合，并通过$\mathcal{PT}$对称通道实现暴胀退出和有效再加热。
...
### [53] [Thermodynamic Gravity with Non-Extensive Horizon Entropy and Topological Calibration](https://arxiv.org/abs/2602.20430)
*Marco Figliolia,Petr Jizba,Gaetano Lambiase*

**TL;DR**: 论文通过局部林德楔框架将克劳修斯关系表述为固定昂鲁温度下马休泛函的平稳条件，揭示了熵斜率控制有效引力耦合的机制，并针对幂律熵模型引入拓扑校准原理，推导出依赖于欧拉示性数的有效耦合常数及宇宙学中引力耦合的特征调制。


<details>
  <summary>Details</summary>

Background: 研究探讨了广义非广延（non-extensive）视界熵背景下，通过热力学方法推导引力动力学（gravitational dynamics）的理论框架，重点关注熵密度形式对有效引力耦合（effective gravitational coupling）和场方程的影响。

Data: data部分：研究基于局部Rindler楔形框架（local Rindler-wedge framework）的理论推导，采用幂律熵模型$S(A) = η(A/4G)^δ$描述视界截面熵，并通过高斯-博内定理（Gauss-Bonnet theorem）引入拓扑校准原理（Topological Calibration Principle）确定粗粒化尺度。

Method: 采用局部Rindler楔形框架下的克劳修斯关系（Clausius relation）作为Massieu泛函在固定Unruh温度下的平稳条件，通过熵斜率参数控制有效引力耦合强度，并引入拓扑校准原理（Topological Calibration Principle）基于高斯-博内定理（Gauss-Bonnet theorem）确定粗粒化尺度。

Result: 在广义非广延（non-extensive）视界熵框架下，通过局部林德勒楔（local Rindler-wedge）构建克劳修斯关系（Clausius relation），发现熵斜率参数控制有效引力耦合强度（effective gravitational coupling），其中面积型熵对应爱因斯坦场方程（$G_{eff} = 1/(4s_0)$），而曲率依赖熵密度则导出$f(R)$引力场方程。
...
### [54] [Physical Predictions in Closed Quantum Gravity](https://arxiv.org/abs/2602.13387)
*Yasunori Nomura,Tomonori Ugajin*

**TL;DR**: 该论文提出了一种在完全约束希尔伯特空间（fully constrained Hilbert space）一维性条件下通过观测数据条件化在扩大无约束希尔伯特空间中提取物理预测的框架，证明了部分可观测性抑制了引力路径积分（gravitational path integral）微观结构相关的系综涨落，从而以指数精度恢复了半经典可预测性，并明确构建了包含哈特尔-霍金无边界态（Hartle-Hawking no-boundary state）贡献的观测密度算符规范不变希尔伯特空间，将形式推广至历史条件化以阐明经典概率和有效时间箭头的涌现。


<details>
  <summary>Details</summary>

Background: 研究探讨了封闭宇宙非微扰物理希尔伯特空间（Hilbert space）在一维超选择扇区内的量子引力状态如何产生半经典物理、测量结果和经典概率的基本问题。

Data: 研究中使用的数据来源为理论推导（simulation），基于引力路径积分（gravitational path integral）和哈特尔-霍金无边界态（Hartle-Hawking no-boundary state）的贡献。

Method: 采用核心分析方法为在不受约束的希尔伯特空间（unconstrained Hilbert space）中通过观测数据条件化提取物理预测，并利用密度算符定义规范不变的观测希尔伯特空间。

Result: 通过条件化观测数据在非约束希尔伯特空间中提取物理预测，解决了全约束希尔伯特空间一维性导致的半经典物理与测量结果涌现问题。
...
### [55] [Linking Leptogenesis and Asymmetric Dark Matter: A Testable Framework for Neutrino Mass and the Matter-Antimatter Asymmetry](https://arxiv.org/abs/2602.20237)
*Henry G. F. McKenna,Juri Smirnov,Martin Gorbahn*

**TL;DR**: 该研究通过重马约拉纳中微子（heavy Majorana neutrino）衰变的最小扩展模型，同时解释了重子不对称和暗物质（DM）丰度，提出了两种机制（洗入和共生成）并预测了可观测的自旋无关截面，为TeV尺度轻子生成和直接探测实验提供了新基准框架。


<details>
  <summary>Details</summary>

Background: 研究探讨了通过重马约拉纳中微子（Majorana neutrino）衰变同时解释重子不对称性和暗物质（DM）丰度的轻子生成（Leptogenesis）框架最小扩展。

Data: 数据来源于重马约拉纳中微子（heavy Majorana neutrino）衰变过程中产生的标准模型（SM）和暗物质（DM）不对称性，通过复Yukawa耦合引入的CP破坏机制，并分析了两种机制（wash-in和co-genesis）下的不对称性转移和增强。

Method: 采用重马约拉纳中微子（heavy Majorana neutrino）衰变模型，通过复Yukawa耦合（complex Yukawa couplings）引入CP破坏，研究洗入（wash-in）和共生成（co-genesis）两种机制下的不对称性传递与低能标跷跷板（low-scale seesaw）现象。

Result: 在共起源（co-genesis）情景下，重暗物质（DM）的自旋无关截面可通过当前直接探测实验检验，而轻暗物质截面位于中微子雾（neutrino fog）区域，同时该框架建立了不对称轻子生成（asymmetric leptogenesis）作为直接探测实验的可预测基准模型，并提出了支持TeV尺度轻子生成的新层级耦合机制。
...
### [56] [Unitarity violation and restoration in radiative bound-state formation](https://arxiv.org/abs/2602.20243)
*Marcos M. Flores,Kalliopi Petraki*

**TL;DR**: 研究发现非相对论性辐射束缚态形成过程中部分波(partial-wave)幺正性严重破坏，通过重求和技术(inelastic contributions)恢复幺正性，并推导出类克拉默斯(Kramers-like)公式。


<details>
  <summary>Details</summary>

Background: 该研究探讨了暗物质（dark matter）研究中辐射性束缚态形成（radiative bound-state-formation）过程的非相对论区域部分波幺正性（partial-wave unitarity）的严重破坏及其恢复机制。

Data: 研究中使用的数据来源于理论推导（theoretical derivation）和模型独立（model-independent）的解析计算，未涉及具体观测或模拟（simulation）数据。

Method: 采用自能(self-energy)吸收贡献的复动量平面解析结构重求和(resummation)方法恢复分波幺正性(partial-wave unitarity)。

Result: 现有计算在非相对论性辐射束缚态形成过程中严重违反分波（partial-wave）幺正性，而通过适当重求和入射态自能的非弹性贡献可恢复幺正性。
...
### [57] [Electromagnetic Radiation from Cosmic-Ray Scatterings on Relic Neutrinos](https://arxiv.org/abs/2602.20255)
*Gonzalo Herrera,Abraham Loeb*

**TL;DR**: 首次估算宇宙射线与宇宙中微子背景（cosmic neutrino background）散射产生的伽马/X射线通量及其宇宙学贡献，并基于费米-LAT（Fermi-LAT）数据将宇宙中微子背景密度上限约束至$η\lesssim 2 \times 10^{4}$（$m_ν\gtrsim0.1$ eV），比实验室探测严格数个量级，与IceCube约束相当。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙线（cosmic-ray）与宇宙中微子背景（cosmic neutrino background）相互作用产生的伽马射线和X射线辐射及其宇宙学意义。

Data: 数据来源于费米大面积望远镜（Fermi-LAT）的弥散伽马射线观测数据以及后续CTA（切伦科夫望远镜阵列）的伽马射线预期数据。

Method: 采用蒙特卡洛模拟（Monte Carlo simulation）和费米-拉特（Fermi-LAT）伽马射线数据拟合方法计算宇宙射线与宇宙中微子背景（cosmic neutrino background）散射产生的伽马/X射线通量及其宇宙学累积贡献。

Result: 首次估计了宇宙线在宇宙中微子背景（cosmic neutrino background）上散射产生的伽马射线和X射线通量及其累积宇宙学贡献。
...
### [58] [X-Shooting ULLYSES: Massive stars at low metallicity XIV. Properties of SMC late-O and B supergiants reveal the metallicity dependence of winds in the Magellanic Clouds](https://arxiv.org/abs/2602.20239)
...
**TL;DR**: 通过分析小麦哲伦云(SMC)中20颗晚O型和B型超巨星的光谱数据，研究发现恒星风动量与金属含量显著相关，且未在21-25kK温度范围内发现双稳态跳跃(bi-stability jump)导致的间断，同时指出单星演化无法解释SMC中经典沃尔夫-拉叶星(WR)的形成。


<details>
  <summary>Details</summary>

Background: 研究大质量恒星辐射驱动星风(wind)的物理特性与恒星大气金属含量(metallicity)的依赖关系，验证演化计算中广泛使用的星风参数化方案的可靠性。

Data: 数据来源于对小麦哲伦云(SMC)中20颗晚O型和B型超巨星样本的紫外与光学光谱联合定量分析，使用CMFGEN进行建模，并与大麦哲伦云(LMC)对应研究中的类似样本和数据进行对比。

Method: 采用CMFGEN进行紫外和光学光谱联合定量分析，结合辐射转移模型推导恒星和星风参数。

Result: result：通过比较小麦哲伦云(SMC)和大麦哲伦云(LMC)样本的风特性，推导出了适用于5.4≤log(L_bol/L_⊙)≤6.1和14≤T_eff/kK≤32范围内的风动量与金属丰度相关的经验公式。
...
### [59] [Multiple protostellar outflows from a single protostar with a misaligned disk](https://arxiv.org/abs/2602.20691)
*Shingo Hirano,Yuri Aikawa,Masahiro N. Machida*

**TL;DR**: 研究发现原恒星(protostar)系统中核心角动量与大规模磁场(magnetic field)的错位(misalignment)会导致经典磁离心盘风(disk wind, DW)和螺旋流(spiralflow, SF)两种喷流成分共存，其中SF在错位角≥60°时质量、尺寸和持续时间均超过DW。


<details>
  <summary>Details</summary>

Background: 研究探讨了原恒星（protostellar）系统中核心角动量与大尺度磁场（large-scale magnetic field）的错位如何影响外流（outflow），以及单一系统能否驱动多组分外流。

Data: 数据来源于三维非理想磁流体动力学（MHD）模拟，模拟对象为磁化旋转核心（magnetized rotating cores），初始条件中核心角动量矢量与磁场之间的夹角从0°到90°系统变化。

Method: 采用三维非理想磁流体动力学（MHD）模拟方法，系统改变核心角动量矢量与磁场之间的初始夹角（0°至90°），研究原恒星外流的形成机制。

Result: 核心角动量与磁场大尺度方向间的错位角度（misalignment angle）越大（≥60°），螺旋流（SF）与经典磁离心盘风（DW）的质量比、尺度比及相对持续时间显著增加，且二者可间歇共存于同一原恒星系统。
...
### [60] [SN 2024abvb: a Type Ibn/Icn supernova with evidence of helium and an extreme offset from its host galaxy](https://arxiv.org/abs/2602.20775)
...
**TL;DR**: 研究发现SN 2024abvb是一种独特的过渡型Ibn/Icn超新星（Type Ibn/Icn supernova），其光谱和光度特征不同于典型Ibn或Icn事件，最可能起源于双星系统中的超剥离（ultra-stripped）前身星，揭示了相互作用超新星前身星通道的多样性。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于探究SN 2024abvb这一特殊过渡型Ibn/Icn超新星（Type Ibn/Icn supernova）的观测特征与起源机制，以揭示相互作用超新星（interacting supernovae）前身星（progenitor）演化路径的多样性。

Data: 数据来源包括SN 2024abvb的光谱观测（spectroscopic observations）和测光观测（photometric observations），涵盖$g$-波段和$o$-波段的测光数据以及光谱中的蓝连续谱、窄P-Cygni轮廓（P-Cygni profiles）、C {\sc ii} $λ$5890发射线和弱He {\sc i} $λ$5876特征。

Method: 采用光谱和测光观测技术，结合黑体温度模型和P-Cygni轮廓分析，通过比较不同超新星亚类的光变曲线和光谱特征，并利用二元系统超剥离（ultra-stripped）超新星模型评估前身星场景。

Result: SN 2024abvb最可能的起源是涉及双星系统的超剥离超新星(ultra-stripped supernova)场景，这最能解释其与宿主星系的大距离、低周星物质质量、快速光度演化及低镍产量，尽管模型与观测抛射物质量仍存在差异。
...
### [61] [A Catalog of 486 Barium Stars Identified in GALAH DR4](https://arxiv.org/abs/2602.21014)
*Guochao Yang,Jingkun Zhao,Nian Liu,Xiaokun Hou,Zhenxin Lei,Xianhao Ye,Gang Zhao*

**TL;DR**: 基于GALAH DR4巡天数据，首次构建了486颗钡星（Ba stars）的高精度丰度样本（含5颗新发现的钡矮星），通过运动学与化学丰度分析揭示其多数源于银河系薄盘或厚盘（2颗显示吸积起源），证实除6053735173729807872外均为含白矮星伴星的双星系统，并利用FRUITY模型反推前身AGB星质量，发现s-过程元素丰度随金属度下降反映中子俘获效率的负相关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中钡星（Ba stars）的化学丰度、运动学特征及双星系统性质，以揭示s-过程元素（s-process elements）的核合成起源及其在恒星演化中的传递机制。

Data: 数据来源于GALAH DR4（Galactic Archaeology with HERMES Data Release 4）的高分辨率光谱和精确丰度测量，并结合了Gaia DR3和StarHorse的天体测量与运动学参数。

Method: 采用高分辨率光谱和精确丰度测量，结合s-过程富集标准（Ba和La相对于Eu的丰度）筛选样本，并应用符号距离法（signed-distance method）进行精炼，构建了迄今最大的Ba星（Ba stars）样本。

Result: 在银河系薄盘和厚盘中发现了486颗钡星（Ba stars），其中18颗具有晕族恒星运动学特征，两颗（4077588766331013248和6692980582560946304）显示出吸积族群的化学丰度特征。
...
### [62] [Range Emulator: A Compact Paraxial Optical System to Emulate Long-Distance Monochromatic Laser Propagation](https://arxiv.org/abs/2602.20538)
*Subaru Shibai,Kiwamu Izumi*

**TL;DR**: 提出了一种名为距离模拟器（RE）的新型光学系统，通过三透镜最小配置在紧凑装置中模拟长距离光束的空间传播效应，量化了系统紧凑性与制造精度之间的基本权衡。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过实验室尺度模拟长距离光传播（long-distance light propagation），为星间光学系统的地面测试提供紧凑型光学测试平台设计框架。

Data: data：研究使用数值模拟（numerical exploration）方法分析距离模拟器（Range Emulator）的光学系统性能。

Method: 采用三透镜组（three-lens system）作为最小配置的区间模拟器（Range Emulator）来复现长距离光束空间传播效应，并通过数值模拟量化系统紧凑性与制造精度之间的基本权衡关系。

Result: 提出并分析了一种称为距离模拟器（RE）的新型光学系统，可在紧凑装置中复现长距离光束的空间传播效应。
...
### [63] [Low-Energy Radon Backgrounds from Electrode Grids in Dual-Phase Xenon TPCs](https://arxiv.org/abs/2602.21177)
...
**TL;DR**: 该论文建立了首个基于第一性原理的高压电极网格表面氡(222Rn)衰变链同位素沉积背景模型，其预测与LZ和LUX实验数据一致，并探讨了该背景在未来双相氙时间投影室(TPC)中的缓解策略及其在纯电离信号暗物质搜索中的应用潜力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于双相氙时间投影室（TPC）技术在探测暗物质粒子与原子核罕见散射中的应用，特别是针对低质量暗物质相互作用预期的低能信号灵敏度提升。

Data: 数据来源于LZ和LUX实验的观测数据。

Method: 采用双相氙时间投影室(TPC)技术，通过构建第一性原理模型(first-principle model)分析高压电极网格表面$^{222}$Rn衰变链同位素沉积导致的电子背景。

Result: 双相氙时间投影室（TPC）中高压电极网格表面沉积的$^{222}$Rn衰变链同位素是导致过量电子背景的主要来源。
