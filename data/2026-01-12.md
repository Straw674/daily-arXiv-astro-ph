...
**TL;DR**: 研究发现原恒星IRAS 04166的极高速度(EHV)气体喷流通过横向喷射物质并与慢速外流气体相互作用，加速了慢速气体成分。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原恒星IRAS 04166+2706（IRAS 04166）外流中极高速度（EHV）气体喷流状成分与低速外流气体之间的物理关联及其动力学机制。

Data: 数据来自ALMA的12m阵列和紧凑阵列(Compact Arrays)对IRAS 04166外流的CO(2-1)发射进行的马赛克观测。

Method: 采用ALMA的12m和紧凑阵列对CO(2-1)发射进行马赛克观测，并开发了描述气体从喷流横向弹道抛射的简化模型(toy model)来解释数据。

Result: IRAS 04166原恒星外流中的极高速度(EHV)气体通过横向喷射物质并与周围低速气流相互作用，加速了低速气体成分。
...
**TL;DR**: 通过半解析模型结合贝叶斯推断证实，银河系中心分子区超新星驱动的多相（multiphase）外流（含高温相和低温云）能合理解释观测到的HI高速云（HI high-velocity clouds）速度、空间分布及质量特征，并揭示低温云通过湍流混合冷却和热风拖拽加速但逐渐瓦解的物理机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中心多相高速气体（multiphase high-velocity gas）的物理特性及其与星系反馈（galactic feedback）和星风（galactic winds）机制的关联。

Data: 数据来源包括从射电到X射线的多波段观测数据（multi-wavelength observations）以及银河系中心区域1.5千秒差距（kpc）范围内探测到的HI高速云（HI high-velocity clouds）的观测数据。

Method: 采用半解析模型（semi-analytical models）结合贝叶斯推断（Bayesian inference）分析多相（multiphase）外流中热气体相（hot gas phase）与冷云（cold clouds）间的引力、拖曳力及质量-动量-能量交换。

Result: 银河系中心区域观测到的多相高速气体(HI high-velocity clouds)符合由中央分子区(central molecular zone)恒星形成驱动的超新星风(supernova-driven winds)模型预测，该模型能解释云团速度、空间分布和质量特征。
...
**TL;DR**: 采用更精细的AGN（活动星系核）光谱能量分布模型和暗弱AGN样本，发现其对宇宙再电离的贡献比先前估计高3~4倍，占比约20%。


<details>
  <summary>Details</summary>

Background: 宇宙再电离（reionization）是宇宙历史中最重要的相变之一，研究聚焦于活动星系核（AGNs）对再电离过程的贡献程度。

Data: 数据来源于整合了物理驱动的活动星系核（AGN）光谱能量分布（SED）模型与最先进的观测数据。

Method: 采用物理驱动的活动星系核（AGN）光谱能量分布（SED）模型并结合先进观测数据，重新评估活动星系核对宇宙再电离的贡献。

Result: 采用更精细的AGN光谱能量分布(SED)模型可将电离光子输出提高3~4倍，使AGN在维持再电离中的贡献提升至约20%，且大量暗弱AGN的纳入进一步放大了这一作用。
...
**TL;DR**: 研究发现高红移（z > 7）富气体星系周围环境中可通过细丝碎裂高效形成致密大质量星团（stellar surface densities > 10^5 M⊙ pc^-2），部分星团可能孕育中等质量黑洞（IMBHs）种子，为早期星团形成、现今球状星团及大质量黑洞种子间的演化联系提供了新路径。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高分辨率宇宙流体动力学模拟（MassiveBlackPS）探索高红移（$z > 7$）富气体星系周围环境中致密大质量星团（stellar clusters）的形成机制及其与中等质量黑洞（IMBHs）种子产生的潜在关联。

Data: 使用分辨率为2 pc的宇宙学流体动力学模拟MassiveBlackPS，研究了恒星质量介于$\sim$$10^{8}$ - $10^{11}$ M$_{\odot}$之间、红移$z > 7$的气体丰富星系周围环境中的星团形成。

Method: 采用宇宙学流体动力学模拟（MassiveBlackPS）结合2秒差距分辨率，研究星系外围环境中星团形成机制，重点分析热力学与引力不稳定性驱动的丝状结构碎裂过程。

Result: 在星系晕（circumgalactic medium）环境中，通过细丝（filament）碎裂机制可以高效形成大质量星团，其极高的恒星表面密度（>10^5 M⊙ pc^-2）可能触发恒星碰撞并形成中等质量黑洞（IMBHs）种子。
...
**TL;DR**: 通过分析天鹅座X（Cygnus-X）的柱密度概率分布函数（N-PDF）发现其呈现对数正态+幂律形态，揭示了湍流和引力共同塑造密度结构的证据，并表明幂律段随时间变平且过渡柱密度可作为分子云特有的稳定恒星形成阈值。


<details>
  <summary>Details</summary>

Background: 分子云(cloud-scale)的柱密度概率分布函数(N-PDF)形态演化研究为理解湍流与引力相互作用如何塑造恒星形成区的密度结构提供了关键统计诊断工具。

Data: 数据来源包括赫歇尔(Herschel)观测获得的柱密度分布图，并辅以氢原子(HI)和年轻恒星体(YSO)数据。

Method: 采用柱密度概率密度函数(N-PDF)作为统计工具，结合赫歇尔望远镜获取的柱密度图、HI数据和年轻恒星天体(YSO)数据，分析天鹅座-X及其子区域密度分布的湍流与引力共同作用特征。

Result: 天鹅座X及其四个子区域的柱密度概率分布函数(N-PDF)呈现对数正态+幂律形态，表明湍流和引力共同塑造了密度结构。
...
**TL;DR**: 通过贝叶斯建模分析仙女座星系(M31)内外球状星团(GC)的运动学特征，发现低金属丰度GC和与子结构相关的GC具有更快速且方向独特的共同旋转模式，而高金属丰度GC的旋转与星系盘一致，揭示了不同GC亚群是在不同时期被吸积形成的。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用球状星团（GCs）的运动学特征揭示仙女座星系（M31）的复杂吸积历史及其动力学演化过程。

Data: 数据来源包括仙女座星系(M31)内外区域球状星团(GC)的异质性观测数据，具体涵盖金属丰度(metallicity)和子结构关联(substructure)定义的子群。

Method: 采用贝叶斯建模（Bayesian modelling）分析M31内外球状星团（GCs）的运动学特征，基于金属丰度和子结构关联划分亚群。

Result: 低金属丰度球状星团（GCs）和与子结构关联的球状星团表现出更快速且方向独特的共同旋转模式，而高金属丰度球状星团的旋转方向与仙女座星系（M31）恒星盘一致。
...
**TL;DR**: 通过98个高银纬快速射电暴(FRB)数据约束银河系晕(Galactic halo)的色散量(DM)贡献为68^{+27}_{-24}pc/cm^3，证实其与文献结果一致且大样本统计比少量低DM暴更可靠。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用快速射电暴(FRB)探测银河系晕(Galactic halo)的电子柱密度(dispersion measure, DM)贡献，以揭示银河系晕的物质分布特征。

Data: 数据来源于ASKAP、Parkes、DSA和FAST探测到的98个高银纬（$|b| > 20^\circ$）快速射电暴（FRBs），其中32个具有红移测量结果。

Method: 采用各向同性模型（isotropic model）拟合快速射电暴（FRBs）的色散量（DM）数据，同时考虑未知的FRB群体参数相关性。

Result: 银河晕（Galactic halo）对色散量（DM）的平均贡献为68^{+27}_{-24}pc/cm$^3$，与已有研究结果一致。
...
### [8] [Backlighting young stellar objects in the Central Molecular Zone: an ensemble-averaged abundance structure of methanol ices](https://arxiv.org/abs/2601.05507)
*Yewon Kang,Deokkeun An,Jiwon Han,Sang-Il Han,Dayoung Pyo,A. C. Adwin Boogert,Kee-Tae Kim,Do-Young Byun*

**TL;DR**: 通过分析银河系中心分子区(CMZ)15个年轻恒星天体(YSO)的红外光谱，发现其包层中甲醇(CH3OH)冰丰度(2-5%)显著低于银河系盘区，且从包层内部(10%)到外部(30%)呈现梯度增加特征，可能源于大质量原恒星的加热导致内层甲醇冰升华。


<details>
  <summary>Details</summary>

Background: 研究聚焦银河系中央分子区(CMZ)中致密分子气体环境下年轻恒星天体(YSOs)和稠密云核的红外冰吸收特征及其化学演化特性。

Data: 数据来源包括使用双子座天文台（Gemini/GNIRS）获取的15个极红点源的L波段光谱、NASA/IRTF的K波段光谱以及斯皮策太空望远镜（Spitzer/IRS）的中红外光谱。

Method: 通过结合Gemini/GNIRS的L波段光谱、NASA/IRTF的K波段光谱和Spitzer/IRS的中红外光谱数据，测量了3.535微米处的CH3OH吸收特征，并利用前景消光（foreground extinction）作为投影距离的代理指标，分析了甲醇冰（CH3OH ice）在分子包层（envelope）中的丰度分布。

Result: 在银河系中心分子区(CMZ)中，甲醇(CH3OH)冰在CH3OH-CO2混合物中的丰度为2%-5%，显著低于银河系盘区典型值，且其相对固态CO2的丰度在包层内区保持约10%，在外区骤增至30%。
...
### [9] [Milliarcsecond-resolution Radio-imaging Survey of Blazar Candidates at 4 < z < 5.4](https://arxiv.org/abs/2601.05736)
...
**TL;DR**: 利用欧洲甚长基线干涉阵（VLBI）对17个高红移（4 < z < 5.4）耀变体候选体进行5 GHz毫角秒分辨率观测，确认其中11个具有多普勒增亮效应（Doppler boosting）的典型耀变体特征，5个缺乏增亮证据但X射线支持耀变体分类，2个为非耀变体活动星系核（AGN），将红移大于4的甚长基线干涉成像射电类星体样本扩大约10%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用甚长基线干涉测量(VLBI)技术探索高红移(4 < z < 5.4)活动星系核(AGN)中耀变体(blazar)的物理特性及其在早期宇宙中的分布。

Data: 数据来源包括欧洲甚长基线干涉测量网（VLBI）在5 GHz的观测数据、单天线和低分辨率干涉测量巡天的总射电流量密度档案数据、盖亚（Gaia）的光学天体测量位置数据以及X射线数据。

Method: 采用欧洲甚长基线干涉测量网络(VLBI)在5GHz频段进行毫角秒分辨率观测，结合多波段归档数据（包括单天线射电流量、低分辨率干涉测量数据、盖亚(Gaia)光学天体测量位置和X射线数据）进行耀变体(blazar)分类诊断。

Result: 在5 GHz频率下使用欧洲甚长基线干涉测量网络（VLBI）观测的17个高红移（4 < z < 5.4）耀变体候选体中，11个表现出多普勒增强辐射特征，5个缺乏多普勒增强证据但X射线数据支持耀变体分类，2个被归类为非耀变体。
...
### [10] [Methanol Formation via the Radical-radical Reaction of OH and CH3 Radicals Undergoing Transient Diffusion on Ice at 10 to 60 K](https://arxiv.org/abs/2601.05757)
*Arisa Iguchi,Hiroshi Hidaka,Atsuki Ishibashi,Masashi Tsuge,Yasuhiro Oba,Naoki Watanabe*

**TL;DR**: 通过实验证实星际冰尘表面在10-60 K温度下可经由CH4 + OH -> CH3 + H2O和CH3 + OH -> CH3OH的连续反应生成甲醇（CH3OH），其中CH3的瞬态扩散由第一反应释放的热量驱动。


<details>
  <summary>Details</summary>

Background: 星际尘埃冰面（interstellar ice dust）上甲醇（CH3OH）的形成机制研究，特别是通过CH3与OH自由基反应路径在低温条件下的可行性验证。

Data: 使用新开发的Cs+离子拾取技术（Cs+ ion pickup technique），在10至60 K温度范围内研究了非晶态固态水（amorphous solid water）表面CH3与OH自由基的反应，其中OH自由基通过紫外光解水分子产生，随后将甲烷（CH4）气体沉积在冰基底上。

Method: 采用新开发的铯离子拾取(Cs+ ion pickup)技术研究了无定形固态水表面CH3与OH自由基在10-60 K温度范围内的反应。

Result: 首次通过实验证实了在10-60 K温度范围内，CH3和OH自由基在无定形固态水（ASW）表面通过CH4 + OH -> CH3 + H2O和CH3 + OH -> CH3OH的连续反应生成甲醇（CH3OH），且第二反应由第一反应释放热量引发的CH3瞬态扩散驱动。
...
**TL;DR**: 通过光学连续区反响映射研究发现超爱丁顿（Eddington）比率活动星系核（AGN）I Zw 1的吸积盘（accretion disk）在薄盘（thin disk）模型下尺寸（4.23±0.24光日）比基准尺寸（1.07±0.15光日）大2-4倍，且高频时延数据支持实际盘尺寸接近基准值，同时多波段观测表明吸积盘未显著延伸至宽线区（BLR），初步否定了辐射加速尘埃驱动外流模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核(AGN)吸积盘(accretion disk)结构及其随爱丁顿比(Eddington ratio)变化的未解问题，特别关注超爱丁顿活动星系核I Zw 1的光学连续谱反响映射(reverberation mapping)观测。

Data: 数据来源于Las Cumbres Observatory三年的亚日采样光学监测（$uBgVriz_s$波段）以及光学干涉测量（optical interferometry）和反响映射（reverberation mapping）获取的光学至中红外波段直接测量数据。

Method: 采用光学连续谱反响映射(reverberation mapping)结合交叉相关方法和PyROA分析技术，研究了超爱丁顿(Eddington)活动星系核(AGN) I Zw 1的吸积盘结构。

Result: 在超爱丁顿比(Eddington ratio)活动星系核(AGN) I Zw 1中，通过光学连续谱的回响映射(reverberation mapping)发现吸积盘(accretion disk)的4495 Å处薄盘(thin disk)模型尺寸为$4.23\pm0.24$光日(ld)，而厚盘(slim disk)模型为$1.71\pm0.09$光日(ld)，均比基于爱丁顿比估算的标准尺寸(fiducial size)大2-4倍。
...
### [12] [Extragalactic archaeology through high-resolution integrated-light spectroscopy of globular clusters](https://arxiv.org/abs/2601.05849)
*M. A. Beasley,K. Fahrion,A. Gvozdenko,S. Larsen*

**TL;DR**: 提出利用河外星系球状星团（extragalactic globular clusters）进行星系考古的大规模光谱观测计划，通过化学标记重建星系形成历史、追踪暗物质晕（dark matter haloes）并测量河外距离。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用河外星系球状星团（extragalactic globular clusters）作为星系考古（extragalactic archaeology）工具，以揭示星系组装历史和暗物质晕（dark matter haloes）分布。

Data: 使用高分辨率（R~20,000）光谱仪对近邻宇宙中大量球状星团（globular clusters）进行大规模光谱观测。

Method: 采用高分辨率光谱（R~20,000）对近邻宇宙中大量球状星团进行化学标记（chemical tagging）分析。

Result: 利用高分辨率光谱（R~20,000）对近邻宇宙球状星团（globular clusters）进行大规模观测，可追溯星系组装历史、追踪暗物质晕（dark matter haloes）并测量河外距离。
...
### [13] [Evaluating star formation rates at z = 5](https://arxiv.org/abs/2601.05916)
*D. Ismail,K. Kraljic,M. Béthermin,A. U. Kapoor,F. Renaud,C. Accard,J. Freundlich,S. Han,J. K. Jang,S. Jeon,T. Kimm,J. Rhee,S. Yi*

**TL;DR**: 通过流体动力学模拟(NewHorizon和NewCluster)结合辐射传输处理(Skirt)，量化了z=5星系中Hα、[CII] 158微米、红外连续谱(IR)和混合(IR+UV)等恒星形成率(SFR)示踪剂的系统误差，发现混合方法最稳健且散射最小，[CII]-SFR关系斜率比前人更陡但受气体密度和金属丰度影响显著。


<details>
  <summary>Details</summary>

Background: 高红移(z=5)星系中恒星形成率(SFR)的推断存在挑战，主要源于观测限制或光度与SFR校准方法的不确定性。

Data: 研究中使用的数据来源于两个先进的水动力模拟（hydrodynamical simulations）NewHorizon和NewCluster，并利用辐射传输代码Skirt进行后处理，生成了合成观测数据，包括Halpha发射线、[CII] 158微米精细结构线、总红外（IR）连续谱光度以及混合（IR + UV）光度。

Method: 采用流体动力学模拟（NewHorizon和NewCluster）结合辐射传输代码Skirt进行后处理，生成合成观测数据（Hα发射线、[CII] 158微米精细结构线、总红外连续谱光度和混合IR+UV），分析不同示踪剂对恒星形成率（SFR）的系统偏差和不确定性。

Result: Hα推断的恒星形成率（SFR）对校准方法选择敏感，且受尘埃衰减（dust attenuation）、视角（viewing angle）和尘埃-金属比（dust-to-metal ratio）影响导致显著离散性。
...
### [14] [An Improved UV-Optical Diagnostic for Rejuvenating Galaxies in the Local Universe and Implications for Galaxy Evolution](https://arxiv.org/abs/2601.05992)
*Dylan Lazarus,Laura C. Parker*

**TL;DR**: 提出了一种基于紫外（UV）成像和光学光谱的局部宇宙星系复壮（rejuvenating galaxies）识别方法，通过分离O星和B/A星对近紫外（NUV）的贡献，在斯隆数字巡天（SDSS）样本中识别出约1万个（4.5%）具有中等恒星质量、低密度环境分布和低气体金属丰度的复壮星系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过紫外（UV）和光学光谱数据识别局部宇宙中的再活跃星系（rejuvenating galaxies），以揭示星系演化过程中恒星形成历史的动态特征。

Data: 数据来源于斯隆数字巡天（SDSS）的紫外（UV）成像和光学光谱数据。

Method: 基于紫外（UV）成像和光学光谱数据，通过建立近紫外（NUV）辐射与Hα示踪的电离O星（O-star）群体之间的关联模型，并扣除O星贡献以分离长寿命恒星（B/A型星）的NUV信号，从而可靠识别局部宇宙中的复苏星系（rejuvenating galaxies）。

Result: 通过紫外（UV）成像和光学光谱分析，建立了一种基于近紫外（NUV）与Hα辐射差异的局部宇宙星系复活动（rejuvenating galaxies）识别方法，并发现约4.5%的SDSS星系（约10^4个）符合复活动特征，其具有中等恒星质量、偏好低密度环境且气体金属丰度较低。
...
### [15] [Ultra Diffuse Galaxies in clusters : the peculiar gas loss of VCC 1964](https://arxiv.org/abs/2601.05948)
*Rhys Taylor,Vojtech Partík,Robert Minchin*

**TL;DR**: 研究发现超弥散星系（UDG）候选体VCC 1964的气体（HI）与光学成分存在9千秒差距偏移，动力学特征暗示显著暗物质（dark matter）缺失，可能因首次进入星系团时遭遇冲压剥离（ram pressure）导致气体位移。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超弥散星系（Ultra Diffuse Galaxies, UDGs）在星系团与场环境中的演化差异，特别是通过气体动力学特征探讨其暗物质缺失问题及团内与场UDGs的结构相似性。

Data: 数据来源于阿雷西博（Arecibo）的两个深度巡天项目——阿雷西博星系环境巡天（Arecibo Galaxy Environment Survey）和宽视场阿雷西博室女座环境巡天（Widefield Arecibo Virgo Environment Survey），以及斯隆数字巡天（Sloan Digital Sky Survey）和DESI传统巡天（DESI Legacy Surveys）的光学数据。

Method: 采用阿雷西博射电望远镜（Arecibo）的深场巡天数据（Arecibo Galaxy Environment Survey和Widefield Arecibo Virgo Environment Survey）测量候选超弥散星系（UDG）VCC 1964的气体含量，并结合斯隆数字巡天（SDSS）和DESI Legacy Surveys的光学数据量化其光学性质。

Result: VCC 1964符合首次进入星系团时因冲压压力（ram pressure）导致气体位移的超弥散星系（Ultra Diffuse Galaxy, UDG）特征，其动力学暗示显著暗物质缺失，但无法排除气体位移导致非平衡状态的可能性。
...
**TL;DR**: 通过流体动力学N体模拟和X射线分析，研究发现NGC 5098/5096双星系群碰撞(galaxy group collision)以接近视线方向(line-of-sight)发生，其暗物质分布与群内光(intragroup light)形态相关，模拟结果与钱德拉(Chandra)观测数据吻合。


<details>
  <summary>Details</summary>

Background: 星系群（galaxy groups）研究对理解宇宙大尺度结构的演化历史至关重要，其高密度环境显著影响星系的气体含量、形态和恒星形成活动。

Data: 研究使用了钱德拉(Chandra)X射线观测数据和基于流体动力学N体模拟(N-body simulation)生成的合成图像数据。

Method: 采用流体动力学N体模拟（hydrodynamical N-body simulations）结合钱德拉（Chandra）X射线观测数据生成模拟图像，并通过80度倾角模拟重现动力学约束条件。

Result: result：NGC 5098与NGC 5096的碰撞几乎沿视线方向发生，相对视线速度为700 km/s，投影距离为155 kpc，暗物质分布与群内光形态存在相关性。
...
### [17] [VENUS: Two Faint Little Red Dots Separated by $\sim70\,\mathrm{pc}$ Hidden in a Single Lensed Galaxy at $z\sim7$](https://arxiv.org/abs/2601.06015)
...
**TL;DR**: 在红移（redshift）$z\sim7$的强透镜星系中发现一对空间分离约70秒差距（parsec）的微弱小红点（LRDs）Red Eyes，其偏离星系中心约一个有效半径（effective radius），表明典型高红移恒星形成星系中可能普遍存在类似的中等质量黑洞（IMBHs），并通过并合促进中心超大质量黑洞（supermassive black hole）的生长。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(z~7)强引力透镜星系中发现的微弱红色点源对(Red Eyes)，探讨其作为典型恒星形成星系中隐藏的非中心多重红色点源(LRDs)的普遍性及其与中等质量黑洞(IMBHs)形成和中心超大质量黑洞(SMBH)增长的潜在关联。

Data: 数据来源于JWST（James Webb Space Telescope）财政支持项目VENUS（View of Extremely New and Unusual Star-forming regions）的观测数据。

Method: 采用强引力透镜效应（strong lensing）和空间分辨光谱分析技术，结合哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）的多波段观测数据，对红点对（Red Eyes）进行形态学与光谱能量分布（SED）建模。

Result: 在强引力透镜星系中发现了一对名为"红眼"的微弱小红点（LRDs），它们位于星系盘上且偏离中心约一个有效半径（Re）处，表明类似的中等质量黑洞（IMBHs）可能普遍隐藏在典型高红移恒星形成星系中。
...
### [18] [Probing General Relativity on Cosmological Scales in the 2040s](https://arxiv.org/abs/2601.05320)
...
**TL;DR**: 这篇论文提出利用下一代大尺度结构巡天（如ESO的Expanding Horizons计划）通过多示踪功率谱（multi-tracer power spectra）和高红移莱曼断裂星系（Lyman-break galaxies）的单示踪双谱（bispectrum）首次精确测量相对论效应（Doppler terms、gravitational redshift、lensing magnification、potential evolution），为2040年代在宇宙学尺度上检验广义相对论及其替代理论开辟新途径。


<details>
  <summary>Details</summary>

Background: 研究聚焦于广义相对论(General Relativity)在宇宙学尺度上的未充分验证性，探索下一代巡天设施通过大尺度结构(galaxy clustering)中的相对论性效应(Doppler terms, gravitational redshift等)开辟检验引力的新窗口。

Data: 数据来源包括下一代设施（如ESO的Expanding Horizons计划）通过多示踪功率谱（multi-tracer power spectra）和高红移莱曼断裂星系（Lyman-break galaxies）的单示踪双谱（single-tracer bispectrum）获取的大规模结构观测数据。

Method: 采用多示踪功率谱（multi-tracer power spectra）和单示踪双谱（single-tracer bispectrum）技术分析高红移莱曼断裂星系（Lyman-break galaxies）的大尺度结构数据。

Result: 下一代设施（如ESO的Expanding Horizons计划）将首次通过多示踪功率谱和高红移莱曼断裂星系单示踪双谱，实现对大尺度结构中相对论效应的稳健测量。
...
### [19] [Kinematic lensing with high-resolution spectroscopic surveys. A unique opportunity for transformative cosmology at high redshifts in the 2040s](https://arxiv.org/abs/2601.05322)
...
**TL;DR**: 提出利用下一代光谱仪（如MegaMapper和宽视场光谱望远镜）通过运动学透镜（KL）技术实现红移2至5的宇宙剪切测量，其信噪比是现有弱透镜（WL）巡天的两倍以上，为联合星系成团-宇宙剪切分析和暗能量主导时期结构增长研究开辟新途径。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用下一代光谱仪（如MegaMapper和宽视场光谱望远镜）通过运动学透镜（KL）技术开展高红移（2-5）宇宙剪切巡天，以探索物质主导时期至暗能量主导时期的结构增长及宇宙加速膨胀机制。

Data: 数据来源于下一代光谱仪（如MegaMapper和Wide-field Spectroscopic Telescope）通过运动学透镜（KL）技术获取的2至5红移范围内的剪切目录。

Method: 采用运动学透镜（KL）技术进行高红移（2至5）宇宙剪切测量。

Result: 通过运动学透镜（KL）技术可在红移2至5区间获得比现有弱透镜（WL）巡天高两倍的信噪比。
...
### [20] [Empirical study of the Pantheon SNeIa Catalog within power-law cosmology and varying speed of light](https://arxiv.org/abs/2601.05512)
*Hoang Ky Nguyen*

**TL;DR**: 研究结合Dolgov幂律宇宙学（a=(t/t0)^μ）和Barrow变光速（c=c0a^-ζ）模型，发现修正的Lemaitre红移公式1+z=a^-(1+ζ)能高质量拟合Ia型超新星数据，并揭示出(1+ζ)μ=1的简并关系，表明c=μ^-1c0t0(da/dt)这一ΛCDM模型不具备的特征。


<details>
  <summary>Details</summary>

Background: 研究探讨了Dolgov幂律宇宙学（power-law cosmology）和Barrow变光速（varying speed of light）模型在晚期加速膨胀（late-time acceleration）背景下的运动学特征及其对宇宙学原理（cosmological principle）的拓展意义。

Data: 数据来源为Pantheon Ia型超新星目录（Pantheon Catalog of Type Ia supernovae）。

Method: 采用Dolgov幂律宇宙学模型（power-law cosmology）和Barrow变光速模型（varying speed of light）分析运动学特性，结合修正的Lemaitre红移公式（modified Lemaitre redshift formula）拟合Ia型超新星数据。

Result: 该模型与Pantheon Ia型超新星目录高度吻合，并展现出沿轨迹$(1+ζ)\,μ=1$的显著简并性。
...
### [21] [A microphysically inspired approach to dark matter-dark energy interactions: first bounds on dark-sector scattering cross sections](https://arxiv.org/abs/2601.05646)
*A. A. Escobal,F. B. Abdalla,J. F. Jesus,E. Abdalla,C. Feng,J. A. S. Lima*

**TL;DR**: 提出了一种基于粒子物理碰撞过程（形式为$Q\proptoρ^2$）的相互作用暗能量（Interacting Dark Energy）模型，通过宇宙学观测数据约束得到哈勃常数（$H_0$）为$67.71\pm0.65$ km s$^{-1}$ Mpc$^{-1}$，并给出了暗物质（Dark Matter）和暗能量（Dark Energy）自湮灭耦合系数$A$和$B$的严格上限。


<details>
  <summary>Details</summary>

Background: 研究聚焦于哈勃常数（Hubble constant, H₀）观测值争议背景下，探索基于粒子物理碰撞过程构建的相互作用暗能量（Interacting Dark Energy, IDE）微观物理模型，以解决传统唯象模型缺乏微观基础和大尺度不稳定的问题。

Data: 数据来源包括背景宇宙学数据、Pantheon Plus、宇宙计时器（Cosmic Chronometers）、DESI DR2和普朗克18（Planck18）的CMB距离先验。

Method: 采用基于玻尔兹曼方程的粒子物理碰撞过程构建"自下而上"相互作用模型，将暗物质(Dark Matter)与暗能量(Dark Energy)的相互作用形式设定为$Q\proptoρ^2$，代表可逆湮灭/产生过程。

Result: result：基于粒子物理碰撞过程构建的相互作用暗能量（Interacting Dark Energy）模型与观测数据一致，得出哈勃常数（Hubble constant）$H_0=67.71\pm0.65$ km s$^{-1}$ Mpc$^{-1}$，暗物质（Dark Matter）自湮灭系数$A$上限为$7.586\times10^{-25}$，暗能量（Dark Energy）自湮灭系数$B$上限为0.048（95%置信水平）。
...
### [22] [Marked statistics across the cosmic web: Environmental dependent clustering in modified gravity simulations](https://arxiv.org/abs/2601.05934)
*Joaquin Armijo,Lucas Da Costa*

**TL;DR**: 研究通过标记相关函数分析Hu-Sawicki f(R)修正引力模拟，发现节点和纤维结构对修正引力的响应强度是其他宇宙环境的四倍以上，结合纤维结构的环境信息使DESI类模拟样本的信噪比显著提升，证明环境标记（尤其纤维结构）能将星系聚类对修正引力的约束能力提高约两倍。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用标记相关函数（marked correlation function）分析环境依赖性聚类，以探索胡-萨维茨基$f(R)$修正引力理论（Hu-Sawicki $f(R)$ modified gravity）对宇宙结构形成的尺度依赖性增强效应。

Data: 数据来源于胡-索维茨基（Hu-Sawicki）修正引力理论（f(R) modified gravity）的数值模拟（simulation）。

Method: 采用标记相关函数（marked correlation function）分析Hu-Sawicki f(R)修正引力模拟数据，并结合多尺度宇宙结构查找算法（multi-scale cosmic structure finder algorithm）划分节点（nodes）、纤维（filaments）、壁（walls）和空洞（voids）四种宇宙环境。

Result: 在Hu-Sawicki f(R)修正引力理论下，节点（nodes）和纤维结构（filaments）对修正引力的响应比其他环境强四倍以上，且结合纤维结构的环境信息能显著提升类CMASS和类DESI模拟样本的信噪比。
...
### [23] [The Linear Point Standard Ruler with DESI DR1 and DR2 Data](https://arxiv.org/abs/2601.05967)
...
**TL;DR**: 研究发现线性点（linear point）作为两点相关函数单极子中的几何特征，比相关函数峰值更能抵抗后期非线性效应（percent level），在DESI巡天DR1/DR2星系样本中通过后重构方法测量其转换参数$α_{iso,LP}$的精度提升15-60%，并提出样本依赖性校正以消除与各向同性BAO测量间的系统偏移。


<details>
  <summary>Details</summary>

Background: 研究探讨了线性点（linear point）作为两点相关函数单极矩中纯几何特征的标准尺（standard ruler）替代方案，及其在晚期非线性效应下的稳健性。

Data: 数据来源于DESI（暗能量光谱仪器）调查的第一和第二数据发布（DR1和DR2）的星系样本，以及第二代AbacusSummit模拟目录（mock catalogs）。

Method: 采用线性点（linear point）作为标准尺度的几何特征，结合后重构（post-reconstruction）技术降低非线性效应影响，并通过样本依赖性校正（sample-dependent correction）缓解晚期非线性效应导致的系统偏移。

Result: 线性点（linear point）作为两点相关函数单极子中的纯几何特征，比相关函数峰值更能抵抗百分之一水平的晚期非线性效应影响。
...
### [24] [Probing Cosmic Expansion and Early Universe with Einstein Telescope](https://arxiv.org/abs/2601.06017)
...
**TL;DR**: 这篇论文的核心贡献是提出下一代引力波(gravitational-wave)观测站爱因斯坦望远镜(Einstein Telescope)将通过与电磁观测互补的方式，以前所未有的灵敏度和频率范围探测高红移(redshift)双黑洞和双中子星系统，解决暗物质(dark matter)、宇宙加速膨胀(cosmic acceleration)和早期宇宙物理条件等关键宇宙学问题。


<details>
  <summary>Details</summary>

Background: 引力波（GW）观测正从发现驱动转向为天体物理学、宇宙学和基础物理学的精密工具，以探索暗物质（dark matter）、宇宙加速膨胀的物理起源、宇宙尺度上的引力性质及大爆炸（Big Bang）后极早期物理条件等电磁观测无法单独解决的重大问题。

Data: 数据来源于下一代引力波(gravitational-wave)观测站爱因斯坦望远镜(Einstein Telescope, ET)以及天基探测器(space-based detectors)的观测数据。

Method: 采用下一代引力波（GW）观测站如爱因斯坦望远镜（ET）的高灵敏度、宽频段探测技术，结合地基与空基探测器互补观测策略。

Result: 引力波（GW）观测将在未来二十年内从发现驱动转变为精确研究天体物理、宇宙学和基础物理的工具，下一代探测器如爱因斯坦望远镜（ET）将突破现有灵敏度、红移覆盖和频率范围的限制，通过探测不发光的物理过程和宇宙时期解决暗物质本质、宇宙加速膨胀起源等关键问题。
...
### [25] [Weak lensing of bright standard sirens: prospects for $σ_8$](https://arxiv.org/abs/2601.06023)
*Ville Vaskonen*

**TL;DR**: 通过结合引力波事件及其电磁对应体（electromagnetic counterparts）和弱引力透镜效应（weak lensing），实现了对哈勃常数、物质丰度及物质扰动标准差（σ₈）的联合测量。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用引力波事件（gravitational wave events）及其电磁对应体（electromagnetic counterparts）结合弱引力透镜效应（weak lensing）测量宇宙学参数，特别是物质扰动标准差（σ₈）、哈勃常数（Hubble constant）和物质丰度（matter abundance）。

Data: 数据来源包括利用爱因斯坦望远镜（ET）观测的300个带有电磁对应体的中子星双星系统，以及激光干涉空间天线（LISA）观测的12个带有电磁对应体的大质量黑洞双星系统。

Method: 采用引力波标准汽笛（standard siren）结合弱引力透镜（weak lensing）分析技术，测量宇宙学参数（包括物质扰动标准差σ₈、哈勃常数和物质丰度）。

Result: 在引力波事件中结合弱引力透镜效应（weak lensing）可测量物质扰动标准差（σ₈），预计ET观测300个中子星双星系统可实现10%精度，LISA观测12个大质量黑洞双星系统可达30%精度。
...
### [26] [Primordial black holes: constraints, potential evidence and prospects](https://arxiv.org/abs/2601.06024)
*Bernard Carr,Antonio J. Iovino,Gabriele Perna,Ville Vaskonen,Hardi Veermäe*

**TL;DR**: 这篇论文全面总结了原初黑洞（PBHs）作为暗物质候选者的现有观测限制、形成机制及其质量函数，并展望了引力波观测等未来探测前景。


<details>
  <summary>Details</summary>

Background: 研究主题为探讨原初黑洞（PBHs）作为早期宇宙形成产物及暗物质候选体的可能性及其观测约束。

Data: 数据来源包括当前观测限制（observational constraints）和引力波观测站（gravitational-wave observatories）的未来搜索数据。

Method: 采用多波段观测数据结合引力波探测技术，系统评估了不同形成机制下原初黑洞（PBHs）的质量函数分布。

Result: 原初黑洞（PBHs）可能形成于早期宇宙并构成全部或部分暗物质。
...
### [27] [GALFITools: A Python library to enhance GALFIT usage in galaxy image modeling](https://arxiv.org/abs/2601.05291)
*Christopher Añorve*

**TL;DR**: 开发了Python库GALFITools以自动化GALFIT（星系图像拟合工具）的预处理和结果解析流程，提升星系（galaxy）光分布建模的效率和可重复性。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于通过望远镜图像分析星系（galaxy）的光分布以理解其形成与演化机制。

Data: data：研究使用望远镜拍摄的星系图像数据，通过GALFIT拟合其光分布并提取物理参数。

Method: 采用Python库GALFITools自动化处理GALFIT输入文件生成、图像掩模创建、天空背景估计、点扩散函数(PSF)建模及参数提取等流程。

Result: GALFITools通过自动化图像掩模生成、天空背景估计、点扩散函数(PSF)建模及参数提取等任务，显著提升了GALFIT的工作流程效率和可重复性。
...
### [28] [The Nucleated Atomistic Grain Growth Simulator (NAGGS): application to the size-dependent structural and physical properties of nanosilicate dust](https://arxiv.org/abs/2601.05430)
*Joan Mariñoso Guiu,Antoni Macià Escatllar,Stefan T. Bromley*

**TL;DR**: 开发了核化原子级晶粒生长模拟器（NAGGS），用于模拟纳米级尘埃颗粒通过单体在核化种子上的渐进吸积生长过程，发现纳米硅酸盐颗粒表面粗糙度与Mg:Si比例相关且具有高偶极矩（dipole moments），为天体化学和微波辐射研究提供了新见解。


<details>
  <summary>Details</summary>

Background: 研究主题为开发新型模拟工具（NAGGS）以原子尺度建模纳米尘埃颗粒（nanograins）在天体物理环境中的生长过程及其物理化学特性。

Data: 研究中使用的数据来源于开源分子动力学代码（open source molecular dynamics codes）生成的模拟数据（simulation data），用于模拟不同化学成分和天体物理条件下纳米尘埃颗粒的生长过程。

Method: 采用核化原子级晶粒生长模拟器（NAGGS）结合开源分子动力学代码，模拟不同化学成分的纳米尘埃颗粒在多种天体物理条件下的生长过程。

Result: 开发了核化原子级晶粒生长模拟器（NAGGS），用于模拟纳米级尘埃颗粒通过单体在核化种子上逐步吸积的生长过程。
...
### [29] [A Large and Precise All-Sky Photometric Standard Star Dataset Across More Than 200 Passbands](https://arxiv.org/abs/2601.05486)
...
**TL;DR**: 构建了迄今最大（超2亿颗）、最精确（优于10毫星等）且最全面（超200个波段）的全天区标准星数据库BEST，为大规模巡天实现毫星等精度测光奠定了基础。


<details>
  <summary>Details</summary>

Background: 高精度测光标准星（photometric standard stars）对实现精确测光校准和推动天文学多领域发展具有关键作用。

Data: 数据来源于构建的包含超过2亿颗恒星的全天标准星数据库BEST（BEst STars Database），其覆盖200多个波段，精度优于10毫星等（mmag），并基于此校准了多个巡天数据集。

Method: 基于超过2亿颗恒星的高精度参考数据，采用多波段（超过200个）全天空校准方法构建了BEST数据库（BEst STars Database），实现了优于10毫星等（mmag）的测光精度。

Result: 构建了迄今最大（超2亿颗星）、最精确（优于10毫星等）且最全面（超200个波段）的全天区标准星数据库BEST，为大规模巡天实现毫星等级测光精度奠定基础。
...
### [30] [Data post-processing gain resulting from the patchy nature of speckles](https://arxiv.org/abs/2601.05598)
*Jean-Baptiste Ruffio,Laurent Pueyo*

**TL;DR**: 研究发现，在恒星衍射光斑（speckles）主导噪声时，结合两个不同观测滚动角（roll angles）的数据可使信噪比提升超过√2，当恒星光噪声超过其他噪声一个数量级时增益接近2倍。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用方位差分成像（ADI）技术提升Habitable Worlds Observatory（HWO）对系外地球探测的灵敏度，特别关注散斑（speckles）噪声的非对称性处理及其对信噪比优化的潜在影响。

Data: 研究中使用的数据来源于模拟（simulation）假设下的方位对称噪声特性及残余衍射星光（speckles）斑块状分布。

Method: 采用角差分成像（ADI）技术分析斑块状衍射星光（speckles）对信噪比的影响，并通过加权平均方法结合不同观测角度的数据以提高灵敏度。

Result: 结合两个观测滚动角的数据可使信噪比提升超过√2，当斑块状星光主导其他噪声源时增益可接近2倍。
...
### [31] [Featureless stars: Flux Calibration for Extremely Large Telescopes](https://arxiv.org/abs/2601.05343)
*Ryan Cooke,Nao Suzuki,J. Xavier Prochaska*

**TL;DR**: 研究发现29颗明亮无特征白矮星（featureless white dwarf stars）可作为高精度光谱能量分布标准源，用于校正多个天文测光系统（Gaia、SDSS等）的系统误差和AB星等偏移。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决下一代成像与光谱巡天及高精度宇宙学实验中因H I巴尔默吸收(H I Balmer absorption)导致的1-3%系统偏差的光谱测光(spectrophotometric)流量定标问题。

Data: 数据来源包括盖亚(Gaia)、斯隆数字巡天(SDSS)、天空监测巡天(SMSS)、泛星计划(PanSTARRS)、暗能量巡天(DES)、2微米全天巡天(2MASS)、星系演化探测器(GALEX)和广域红外巡天探测器(WISE)。

Method: 基于29颗无特征（featureless）白矮星样本，采用黑体（blackbody）光谱能量分布模型计算了多个巡天系统的系统不确定性和AB星等偏移量。

Result: 发现29颗明亮（G < 17.5）且无特征的白矮星（white dwarf stars），其光谱能量分布在整个光学和近红外波段几乎符合纯黑体（blackbody）形式，适合作为下一代大型望远镜和巡天观测的高精度分光光度标准星（spectrophotometric standard stars）。
...
### [32] [Paving the Road to the Habitable Worlds Observatory with High-Resolution Imaging I: New and Archival Speckle Observations of Potential HWO Target Stars](https://arxiv.org/abs/2601.05387)
...
**TL;DR**: 该研究通过光学散斑成像（speckle imaging）观测了NASA系外行星探测计划（ExEP）暂定HWO目标恒星列表中的80颗恒星，确认了HD 90089的候选伴星并模糊检测到HD 212330的已知伴星，模拟显示75%-85%质量低至0.1太阳质量（$M_{\odot}$）的伴星可被探测到，为HWO（Habitable Worlds Observatory）目标选择提供了关键的多重性（stellar multiplicity）数据支持。


<details>
  <summary>Details</summary>

Background: 研究聚焦于为哈勃世界天文台（HWO）直接成像潜在宜居系外行星任务筛选目标恒星，重点评估其恒星多重性（stellar multiplicity）以确保任务成功。

Data: 数据来源包括使用双子座天文台的`Alopeke和Zorro设备获取的新观测数据，以及从系外行星后续观测计划档案（Exoplanet Follow-up Observing Program Archive）中查询的存档观测数据，共覆盖了164颗恒星中的80颗。

Method: 采用光学散斑成像技术（optical speckle imaging）对NASA系外行星探索计划（ExEP）候选恒星进行观测，结合模拟恒星伴星（stellar companions）检测灵敏度分析。

Result: 通过光学散斑成像观测了NASA ExEP暂定HWO星表中的80颗恒星，确认了盖亚(Gaia)先前发现的一颗候选伴星(HD 90089)，并对另一颗已知伴星(HD 212330)获得模糊检测。
...
### [33] [Impact of newly measured 26Al(n, p)26Mg and 26Al(n, α)23Na reaction rates on the nucleosynthesis of 26Al in stars](https://arxiv.org/abs/2601.05710)
...
**TL;DR**: 重新评估了短寿命放射性核素铝-26（26Al）在0.01-10 GK温度区间的中子俘获反应截面（26Al(n, p)26Mg和26Al(n, p)23Na），发现新反应率使低质量AGB星（2-3 Msun）能复现陨石碳化硅（SiC）颗粒中测量的26Al/27Al比值范围，并使高质量恒星（15 Msun）的26Al丰度变化因子达2.4，显著改善了星尘观测与恒星核合成模型的匹配度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于短寿命放射性核素铝-26（26Al）的宇宙生成过程及其对恒星和星系演化的关键影响。

Data: 数据来源包括n TOF测量结果、理论预测和高能区先前测量数据，用于重新评估26Al(n, p)26Mg和26Al(n, α)23Na基态反应率；低质量恒星模型使用2-3太阳质量范围内金属丰度Z=0.01至0.02的样本，高质量恒星模型采用15太阳质量且Z=0.006和0.02的样本，并将低质量恒星模型预测的26Al/27Al比值与62颗陨石碳化硅(SiC)颗粒实测数据对比。

Method: 采用蒙特卡洛核合成代码（Monash nucleosynthesis code）、NuGrid并行粒子输运代码（mppnp code）和FUNS恒星演化代码，结合n_TOF实验测量数据与理论预测，重新评估了0.01-10吉开尔文温度范围内26Al(n,p)26Mg和26Al(n,α)23Na基态反应率。

Result: 新反应率使低质量渐近巨星支(AGB)恒星能够重现碳化硅(SiC)颗粒中测量的全部26Al/27Al比值范围。
...
### [34] [Fading into darkness: A weak mass ejection and low-efficiency fallback accompanying black hole formation in M31-2014-DS1](https://arxiv.org/abs/2601.05774)
...
**TL;DR**: 在仙女座星系中观测到恒星质量黑洞（stellar-mass black hole）候选体M31-2014-DS1的持续光度衰减（$\log(L/L_\odot)\approx3.88$），其尘埃壳层（dust shell）和分子气体（molecular gas）特征首次为低能爆炸（low-energy explosion）与长期回落吸积（fallback accretion）机制形成黑洞提供了完整证据链。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星质量黑洞（stellar-mass black holes）通过大质量恒星近完全坍缩直接形成的物理过程及其低能爆炸（low-energy explosion）与长期回落吸积（fallback accretion）的观测特征。

Data: 数据来源包括詹姆斯·韦伯太空望远镜（JWST）的中红外（MIR）观测（MIRI/NIRSpec）和钱德拉X射线天文台（Chandra X-ray Observatory）的X射线观测。

Method: 采用詹姆斯·韦伯太空望远镜(JWST)的中红外(MIR)观测和钱德拉X射线天文台的X射线观测数据，结合尘埃连续谱建模和分子气体动力学分析。

Result: M31-2014-DS1的观测结果支持其前身星（约12-13太阳质量的黄超巨星）通过低能量（约10^46尔格）爆炸形成约5太阳质量黑洞（BH）的假说，并揭示了长期回落物质低效吸积（约0.1%质量）驱动的中心黑洞光度持续衰减。
...
### [35] [GlueNN: gluing patchwise analytic solutions with neural networks](https://arxiv.org/abs/2601.05889)
*Doyoung Kim,Donghee Lee,Hye-Sung Lee,Jiheon Lee,Jaeok Yi*

**TL;DR**: 提出一种将渐近解析解的积分常数推广为尺度依赖函数的学习框架，通过约束这些系数函数在全局满足原微分方程，无需边界匹配即可获得在化学动力学和宇宙学中准确且优于传统方法的全局解。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决物理和工程中复杂微分方程（differential equations）因强尺度依赖性导致解析或数值解不可得的问题。

Data: 研究中使用的数据来源为理论模拟（simulation）数据，涉及化学动力学和宇宙学领域的代表性问题的数值解。

Method: 采用学习框架将渐近解析解的积分常数推广为尺度依赖函数，并通过约束这些系数函数来学习全局有效解。

Result: 提出了一种将渐近解析解的积分常数提升为尺度依赖函数的学习框架，通过约束这些系数函数使其满足原微分方程，从而获得全局有效解，无需边界匹配。
...
### [36] [On the orbital evolution of binaries with polar circumbinary disks](https://arxiv.org/abs/2601.05932)
*Cheng Chen,Philip J. Armitage,C. J. Nixon*

**TL;DR**: 研究发现，偏心双星（eccentric binary）周围的极对齐盘（polar-aligned disk）会导致双星轨道持续收缩，且收缩速率高于对齐或逆行盘（aligned/retrograde disk）情况，同时在小吸积半径（sink radii）下可能形成长期存在的极环绕主盘（polar circumprimary disk）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于双星系统（binary）周围环双星盘（circumbinary disk）的轨道演化动力学，特别是偏心双星导致盘面极向排列（polar-aligned）的机制及其对双星轨道衰变的影响。

Data: 研究中使用的数据来源于基于平滑粒子流体动力学（Smoothed Particle Hydrodynamics, SPH）模拟的数值计算结果，模拟在质量注入条件下演化至近似稳态状态。

Method: 采用平滑粒子流体动力学（Smoothed Particle Hydrodynamics, SPH）模拟方法，在质量注入条件下演化至近似稳态，研究不同双星-盘参数下极向对齐（polar-aligned）盘的双星轨道演化。

Result: 在模拟的所有参数条件下，极对齐（polar-aligned）盘环绕的双星轨道会随时间收缩，且收缩速率高于具有相同盘参数的对齐或逆行盘情况。
...
### [37] [Mimetic Dark Matter from Inflation](https://arxiv.org/abs/2601.05931)
*Ali H. Chamseddine,Viatcheslav Mukhanov*

**TL;DR**: 研究发现拟态暗物质（mimetic dark matter）与高斯-博内拓扑项（Gauss-Bonnet term）的耦合能在宇宙暴胀阶段自然产生拟态暗物质，其能量密度可由暴胀结束时的哈勃参数表示，且耦合形式会影响物质-辐射平衡后的演化行为。


<details>
  <summary>Details</summary>

Background: 研究探讨了在爱因斯坦-希尔伯特作用量基础上，模仿暗物质（mimetic dark matter）与高斯-博内拓扑项（Gauss-Bonnet topological term）的耦合机制及其在宇宙暴胀阶段自然产生暗物质的可能性。

Data: 数据来源于理论模拟（simulation）分析，基于爱因斯坦-希尔伯特作用量（Einstein-Hilbert action）与高斯-博内拓扑项（Gauss-Bonnet topological term）耦合的数值计算。

Method: 采用耦合拟态暗物质(mimetic dark matter)与高斯-博内拓扑项(Gauss-Bonnet topological term)的修正爱因斯坦-希尔伯特作用量(Einstein-Hilbert action)模型。

Result: 在宇宙暴胀阶段，拟态暗物质（mimetic dark matter）与高斯-博内拓扑项（Gauss-Bonnet topological term）的耦合能自然产生正确数量的类尘埃暗物质，其能量密度可由暴胀结束时的哈勃参数表示。
...
### [38] [Robust Bilinear-Noise-Optimal Control for Gravitational-Wave Detectors: A Mixed LQG/$H_\infty$ Approach](https://arxiv.org/abs/2601.05447)
*Ian A. O. MacMillan,Lee P. McCuller*

**TL;DR**: 该论文针对LIGO（激光干涉引力波天文台）低频噪声问题，提出了基于线性二次高斯（LQG）和混合H∞控制的优化方法，通过建立基准成本函数和帕累托前沿（Pareto front）计算，首次确定了双线性控制噪声的理论下限，并实现了快速全局最优的稳定鲁棒反馈控制，显著提升了现有和下一代引力波探测器的控制噪声性能。


<details>
  <summary>Details</summary>

Background: 研究聚焦于降低LIGO（激光干涉引力波天文台）低频噪声中的非线性相互作用（nonlinear interactions），特别是双线性噪声（bilinear noise），以提升引力波探测灵敏度。

Data: 数据来源于LIGO（Laser Interferometer Gravitational-Wave Observatory）的悬吊光学元件（suspended optics）反馈控制系统中的噪声测量和仿真分析。

Method: 采用线性-二次-高斯控制（Linear-quadratic-Gaussian control）与混合LQG/H∞方法直接计算稳定、鲁棒且最优的反馈控制。

Result: 非线性相互作用是低频噪声的主要来源，通过混合多个自由度（degrees of freedom）的噪声影响干涉仪性能。
...
### [39] [Probing the morphology of the Gum Nebula through pulsar observables and a novel distance estimation method](https://arxiv.org/abs/2601.05791)
*Ashish Kumar,Avinash A. Deshpande,Pankaj Jain*

**TL;DR**: 通过分析船帆座脉冲星(Vela pulsar)等数据并改进Gum Nebula电子密度模型(YMW16模型)，发现散射体距离占比为0.89±0.01（对应254±16 pc），证实船帆座脉冲星位于Gum Nebula壳层后方，并提出结合色散测量(DM)和时间展宽(temporal broadening)的新方法以更准确估算脉冲星距离。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过分析船帆座脉冲星（Vela pulsar）等天体的观测数据，改进对古姆星云（Gum Nebula）电子密度模型和形态结构的理解，以解决现有模型在参数和起源解释上的显著分歧。

Data: 数据来源包括船帆座脉冲星（Vela pulsar）的观测数据及十颗脉冲星的独立距离数据。

Method: 采用主导散射区假设分析船帆座脉冲星（Vela pulsar）数据，结合十颗脉冲星独立距离优化了Gum Nebula电子密度模型（YMW16模型形态基础），并开发了同时利用色散测量（dispersion measure）和时间展宽（temporal broadening）约束脉冲星距离的新技术。

Result: Vela脉冲星前方主导散射区域的分数距离为0.89±0.01，对应实际距离254±16秒差距（pc）。
...
### [40] [Superluminous supernovae: diverse rise times explain diverse spectra](https://arxiv.org/abs/2601.05363)
*Matt Nicholl*

**TL;DR**: 研究发现I型极亮超新星（SLSNe）的光谱多样性由极大光时抛射物温度（ejecta temperature at maximum light）驱动而非爆炸或前身星（progenitor）的本质差异，并通过构建亮度-时标-温度-半径（Brightness-Timescale-Temperature-Radius）诊断图和光球层速度（photospheric velocity）演化模型，证实光变曲线上升时间差异（源于抛射质量差异）导致宽光变曲线SLSNe具有更弱氧离子（O II）线、更低极大光后光球层速度和更慢速度演化，且已知SLSNe速度分布支持与中央引擎（central engine）加热气泡一致的平坦抛射物密度轮廓（flat ejecta density profile）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于I型极亮超新星(SLSNe)的光谱多样性机制及其与光变曲线特性的关联。

Data: 数据来源于对PTF12dam这一超亮超新星（SLSN）的详细观测。

Method: 采用亮度-时标-温度-半径(Brightness-Timescale-Temperature-Radius)图和光球层速度演化的简化玩具模型(toy model)作为核心分析方法。

Result: I型极亮超新星（SLSNe）的光谱多样性主要由最大光时抛射物温度驱动，而非爆炸或前身星的根本差异。
...
### [41] [A Minimalist Merger Interpretation of XRISM's Gas Velocity Measurements in the Coma Cluster](https://arxiv.org/abs/2601.05803)
*Congyao Zhang,Eugene Churazov,Ildar Khabibullin,Natalya Lyskova,Norbert Werner,Irina Zhuravleva*

**TL;DR**: 研究发现早期阶段离轴小合并（off-axis minor merger）能驱动后发座星系团（Coma cluster）核心区产生与XRISM观测结果一致的团内气体运动特征，包括显著速度梯度和约100-200 km/s的均匀速度弥散。


<details>
  <summary>Details</summary>

Background: 研究探讨了后发座星系团（Coma cluster）内气体速度特征的物理起源，特别是早期阶段离轴小质量并合（off-axis minor merger）对团内气体运动的影响。

Data: 数据来源于XRISM对后发座星系团（Coma cluster）的微热量X射线观测（microcalorimetric X-ray observations）以及Lyskova等人2019年研究中提出的合并情景（merger scenario）。

Method: 采用早期阶段离轴小合并（off-axis minor merger）模型解释后发座星系团（Coma cluster）核心区域气体运动特征。

Result: 早期阶段的离轴小质量并合（off-axis minor merger）能够驱动与XRISM观测结果基本一致的星系团内气体运动特征。
...
### [42] [Information-theoretic astrophysical uncertainties in the effective theory of dark matter direct detection](https://arxiv.org/abs/2601.05332)
*Gonzalo Herrera*

**TL;DR**: 研究利用Kullback-Leibler（KL）信息散度量化了天体物理不确定性对暗物质-核子相互作用的非相对论有效场论（non-relativistic effective field theory）中所有算符的影响，发现不同算符的不确定性范围从小于一个数量级到阈值附近三个数量级不等，并揭示了散射率与速度分布的不同速度加权矩（如均值、方差或偏度）的依赖关系，为识别特定暗物质模型的关键速度分布特征提供了新解析见解。


<details>
  <summary>Details</summary>

Background: 研究探讨了天体物理不确定性（astrophysical uncertainties）对直接探测（direct detection）不同暗物质（dark matter）模型和探测器靶标的影响差异，重点关注暗物质-核子相互作用的非相对论有效场论（non-relativistic effective field theory）中所有算符的散射截面速度与动量依赖性。

Data: 研究中使用的数据来源为暗物质速度分布的理论模型（包括麦克斯韦-玻尔兹曼分布及其熵驱动偏离形式），并通过Kullback-Leibler（KL）信息散度量化真实分布与理论模型间的偏差。

Method: 采用Kullback-Leibler（KL）信息散度测量暗物质速度分布与麦克斯韦-玻尔兹曼（Maxwell-Boltzmann）形式的偏差，量化天体物理不确定性对有效理论中各算符的影响。

Result: 利用Kullback-Leibler（KL）信息散度量化了暗物质速度分布偏离麦克斯韦-玻尔兹曼形式时对不同非相对论有效场论算符的影响，发现不确定性范围从小于一个数量级到阈值附近三个数量级不等。
...
### [43] [Dipole Radiation and Kinetic Mixing from Dark Photon Solitons](https://arxiv.org/abs/2601.05351)
*Enrico D. Schiappacasse,Moira Venegas*

**TL;DR**: 该论文发现自旋1粒子（暗光子）构成的波状暗物质会形成致密天体"矢量孤子"，并通过维度6耦合和动力学混合机制在外部电磁场或电荷密度作用下产生光子，为暗物质探测提供了新途径。


<details>
  <summary>Details</summary>

Background: 研究探讨了由自旋为1的暗光子（dark photons）构成的波状暗物质（wave-like dark matter）形成"矢量孤子（vector solitons）"的理论及其通过光子耦合产生光子的机制。

Data: 数据来源于理论模拟（simulation）和已有暗物质（dark matter）分布模型。

Method: 采用维度-6暗光子-光子耦合(dimension-6 dark photon-photon coupling)和动力学混合(kinetic mixing)分析暗光子构成的矢量孤子(vector solitons)在外部电磁场或电荷密度影响下产生光子的机制。

Result: 暗光子（dark photons）构成的波状暗物质可形成称为“矢量孤子（vector solitons）”的致密天体结构，其通过维度-6暗光子-光子耦合或动力学混合在外部电磁场或电荷密度作用下产生光子。
...
### [44] [Towards Accurate Gravitational Wave Predictions: Gauge-Invariant Nucleation in the Electroweak Phase Transition](https://arxiv.org/abs/2601.05793)
*Jie Liu,Renhui Qin,Ligong Bian*

**TL;DR**: 论文研究了标准模型有效场论（Standard model effective field theory）框架下早期宇宙中通过一级相变（first-order phase transition）发生的真空衰变（vacuum decay）及其引力波产生的规范依赖性（gauge dependence），发现根据新物理尺度（new physics scale）选择的幂计数方案（power-counting scheme），微扰计算方法可获得规范无关的成核率（nucleation rates）和相变参数，从而更精确预测引力波特征。


<details>
  <summary>Details</summary>

Background: 研究探讨了早期宇宙中规范不变性（gauge-invariant）的真空衰变及其伴随的引力波产生机制。

Data: 研究中使用的数据来源于三维标准模型有效场论框架下的微扰计算（perturbative computation）方法，结合幂计数（power-counting）方案和有限温度下的尼尔森恒等式（Nielsen identity）进行理论推导。

Method: 采用三维标准模型有效场论框架下的幂计数方案和有限温度下的尼尔森恒等式（Nielsen identity）进行微扰计算，获得规范无关的成核率和相变参数。

Result: 通过考虑幂次计数方案并利用有限温度下的尼尔森恒等式(Nielsen identity)，证明了基于新物理尺度的不同幂次计数方案下，微扰计算方法可获得规范无关的成核率与相变参数。
...
### [45] [Naturally small Dirac neutrino mass and $B-L$ dark matter](https://arxiv.org/abs/2601.05926)
*Ernest Ma,Partha Kumar Paul,Narendra Sahu*

**TL;DR**: 论文研究了$B-L$荷为3或4的单重态标量场$\chi$，发现其可导致中微子保持狄拉克性（Dirac nature），同时引入$B-L$荷为2的矢量费米子$S$作为暗物质候选者，并预言自发$B-L$对称性破缺会产生可探测的引力波（GW）信号及对有效相对论自由度$Δ{N}_{\rm eff}$的额外贡献。


<details>
  <summary>Details</summary>

Background: 研究探讨了标准模型的${B-L}$规范扩展中，单重态标量场$χ$携带不同${B-L}$电荷（3或4）时对中微子质量（Dirac fermions）、暗物质候选者（vector-like fermion $S$）以及宇宙学现象（如随机引力波和有效相对论自由度$Δ{N}_{\rm eff}$）的影响。

Data: 研究中使用的数据来源于理论模拟（simulation），主要基于标准模型的${B-L}$规范扩展框架下的不同电荷配置（$χ\sim 3$或4）及其对暗物质候选者（vector-like fermion $S$）和引力波（GW）产生的影响。

Method: 采用规范${B-L}$扩展标准模型，通过赋予单重态标量场$χ$不同的$B-L$电荷（3或4）并引入具有2单位$B-L$电荷的矢量费米子$S$，研究暗物质候选者及自发$B-L$对称性破缺导致的一阶相变产生的引力波（GW）。

Result: 在标准模型的传统规范${B-L}$扩展中，将单重态标量$χ$的$B-L$电荷改为3或4时，中微子保持狄拉克费米子特性，并引入具有2单位$B-L$电荷的矢量费米子$S$作为暗物质候选者。
...
### [46] [Gravitational Ionization by Schwarzschild Primordial Black Holes](https://arxiv.org/abs/2601.05935)
*Alexandra P. Klipfel,David I. Kaiser*

**TL;DR**: 研究发现原初黑洞(PBHs)通过潮汐力可电离中性氢、解离氘核并诱导重核裂变，为探测质量范围5×10²¹g至10²³g的PBHs提供了新方法，并确定了其在大爆炸核合成(BBN)期间对氘核解离的主导作用。


<details>
  <summary>Details</summary>

Background: 该研究探讨了原初黑洞（PBHs）的引力场梯度可能通过潮汐力破坏原子和原子核的现象，为探测这类暗物质候选体提供新方法。

Data: 数据来源于理论模拟（simulation）分析，包括原初黑洞（PBH）引力场对中性氢的电离、复合时期（recombination epoch）的能量沉积效应、以及潮汐力对氘核（deuteron）和重核的破坏作用。

Method: 采用引力电离（gravitational ionization）和潮汐力（tidal force）分析方法评估原初黑洞（PBH）对中性氢原子、核子及重核的破坏效应。

Result: 原初黑洞（PBHs）在典型质量范围$5\times10^{21}\,{\rm g}\lesssim M \lesssim 10^{23}\, {\rm g}$内可通过潮汐力主导重组时期（$z\simeq1090$）中性氢的电离和加热过程。
...
### [47] [Gravitational waves and small-field astrometry](https://arxiv.org/abs/2601.05754)
*Robin Geyer,Sven Zschocke,Michael Soffel,Sergei Klioner,Lennart Lindegren,Uwe Lammers*

**TL;DR**: 论文通过理论推导和数值模拟揭示了在小视场天体测量（small-field astrometry）中引力波（gravitational waves）的微分效应会被标准平板校准（plate calibrations）几乎完全吸收，从而显著限制了小视场任务的探测能力。


<details>
  <summary>Details</summary>

Background: 该研究探讨了利用天体测量观测（astrometric observations）检测引力波（gravitational waves）的可行性，重点分析了小视场（small-field）数据中引力波效应的特征及其局限性。

Data: 数据来源于数值模拟（numerical simulation）和小视场天体测量观测（small-field astrometric observations）。

Method: 采用数值模拟（numerical simulations）和理论推导（theoretical derivation）方法分析平面引力波（plane gravitational wave）对小视场天体测量（small-field astrometry）的微分效应（differential effect）。

Result: 小视场（small-field）天体测量任务因视场较小而仅能测量微小差分效应，这些效应几乎完全被标准底片校准（plate calibrations）吸收，导致其具有显著不利特性。
...
### [48] [Parity-violating Dark Photon Halos](https://arxiv.org/abs/2601.04145)
*Stephon Alexander,Lawrence Edmond,Cooper Niu*

**TL;DR**: 研究发现物质主导时期通过轴子（axion）相干振荡的速子不稳定性可产生具有净螺旋度的暗光子（dark photon），形成引力束缚的暗光子晕（halo），其质量范围为$10^5 \, M_{\odot}$至$10^{11} \, M_{\odot}$，并可能通过诱导涡度促进超大质量黑洞的早期形成。


<details>
  <summary>Details</summary>

Background: 研究探讨了物质主导时期通过轴子（axion）相干振荡的速子不稳定性（tachyonic instability）产生引力束缚暗光子晕（dark photon halos）的机制及其对超大质量黑洞（supermassive black holes）早期形成的潜在影响。

Data: 数据来源于理论模拟（simulation）中暗光子（dark photon）与超轻轴子（ultralight axion）通过宇称破坏的Chern-Simons项耦合产生的 tachyonic 不稳定性。

Method: 采用超轻轴子场（axion）与暗光子（dark photon）通过宇称破缺Chern-Simons项耦合的物理模型，利用轴子相干振荡的速子不稳定性（tachyonic instability）产生具有净螺旋度的暗光子，进而诱导度量涡度（metric vorticity）并形成手性亚结构（chiral substructures）。

Result: 在物质主导时期，通过与超轻轴子场的宇称破缺陈-西蒙斯项耦合，暗光子（dark photon）可通过轴子相干振荡的速子不稳定性产生，并形成具有净螺旋度的引力束缚暗光子晕。
