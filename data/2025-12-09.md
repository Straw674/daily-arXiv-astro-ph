...
### [1] [Do Low-Mass, Low-Luminosity AGNs Deviate from the Quasar Main Sequence?](https://arxiv.org/abs/2512.06068)
*Himanshu Sharma,Vivek Kumar Jha,Hum Chand,Swayamtrupta Panda*

**TL;DR**: 研究发现低光度活动星系核（AGN）中寄居的低质量黑洞样本（Hβ半高全宽<2200 km/s）在低爱丁顿比率（中位值log R_Edd≈-0.68）和弱FeII发射（中位值R_FeII≈0.61）方面与经典窄线赛弗特1星系（NLSy1）存在显著差异，揭示了4DE1参数空间中低R_FeII和低R_Edd区域存在物理性质独特的吸积态。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低光度活动星系核（AGN）中低质量黑洞的吸积和辐射特性，旨在揭示其与传统窄线赛弗特1型星系（NLSy1）的物理差异及其在低质量尺度黑洞生长中的意义。

Data: 数据来源包括基于$Hβ$半高全宽（FWHM）筛选的低光度活动星系核（AGNs）样本的光谱数据，以及来自兹威基瞬变设施（ZTF）光变曲线的光学变异性振幅数据。

Method: 采用光谱分析和光变特性（optical variability）结合的方法，基于Hβ半高全宽（FWHM）筛选样本，并运用4D特征向量1（4DE1）参数空间进行物理状态分类。

Result: 低光度活动星系核(AGN)样本在4D特征向量1(4DE1)参数空间中占据低$R_{FeII}$、低$R_{Edd}$区域的独特位置，表明其处于物理性质不同的吸积状态。
...
### [2] [The quenched fraction of satellites around simulated Milky Way-mass galaxies](https://arxiv.org/abs/2512.06071)
...
**TL;DR**: 通过比较三个宇宙学模拟套件（FIREbox、FIRE-2和IllustrisTNG50）与观测数据（SAGA、ELVES及银河系-M31卫星群），发现所有模拟均能复现低质量卫星更易被熄灭（quenching）的质量依赖性趋势，但径向分布差异揭示了宿主环境（host environment）和组装历史（assembly history）对卫星熄灭的重要影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于比较不同宇宙学模拟（FIREbox、FIRE-2 zoom-ins和IllustrisTNG50）与观测数据（SAGA、ELVES及银河系-M31卫星群）中卫星星系（satellite galaxy）的淬灭（quenching）特征，探讨星系形成模型中质量依赖淬灭的普适性及宿主环境（host environment）对淬灭径向趋势的影响。

Data: 数据来源包括三个宇宙学模拟套件（FIREbox、FIRE-2 zoom-ins和IllustrisTNG50）以及来自SAGA、ELVES和银河系与M31合并卫星群的观测数据集。

Method: 采用三种宇宙学模拟套件（FIREbox、FIRE-2 zoom-ins和IllustrisTNG50）与观测数据集（SAGA、ELVES及银河系与M31的并合卫星群）进行对比分析，通过统一投影孔径和淬灭（quenching）定义，选取暗晕质量（halo mass）为$10^{11.9}$ - $10^{12.2} \, M_{\odot}$的银河系质量宿主及恒星质量（stellar mass）为$10^{7}$ - $10^{10}\, M_{\odot}$的卫星星系。

Result: 不同宇宙学模拟（FIREbox、FIRE-2 zoom-ins和IllustrisTNG50）均能复现观测数据中低质量卫星星系更易发生恒星形成熄灭（quenching）的质量依赖性趋势，表明该现象是当代星系形成模型的稳健预测结果。
...
### [3] [Lyman-$α$ Visibility During the Epoch of Reionization: Combining JWST FRESCO Grism Data with Keck Archival Spectroscopy](https://arxiv.org/abs/2512.06072)
...
**TL;DR**: 基于JWST FRESCO数据在GOODS-N和GOODS-S天区发现8个新莱曼阿尔法发射体(LAEs)，结合文献样本共14个LAEs分析表明其环境密度与普通[OIII]发射体相当，莱曼阿尔法可见性主要受星形成率和[OIII]等效宽度等内在因素影响而非环境密度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用莱曼阿尔法(Lyman-α)发射线在红移$z>7$的可见性来探索宇宙再电离(reionization)过程及星系在塑造电离星系际介质中的作用。

Data: 数据来源包括JWST FRESCO巡天数据、凯克望远镜(KECK)/MOSFIRE存档观测数据以及六项文献中的莱曼阿尔法发射体(LAEs)样本。

Method: 利用JWST FRESCO数据识别[OIII]发射体并分析其大尺度分布，结合Keck/MOSFIRE观测数据恢复莱曼α发射体(LAEs)，通过定义以最亮LAEs为中心的[OIII]发射体群组并计算过密度参数来研究LAEs的环境特性。

Result: 在$z=7.0-7.7$红移范围内发现了8个新的莱曼-α发射体（LAEs），包括一个$z \sim 7.2$的潜在活动星系核（AGN）候选体，结合文献中的6个LAEs共组成14个LAEs样本。
...
### [4] [A Galactic Transformation -- Understanding the SMC's Structural and Kinematic Disequilibrium](https://arxiv.org/abs/2512.06075)
*Himansh Rathore,Gurtina Besla,Roeland P. van der Marel,Nitya Kallivayalil*

**TL;DR**: 通过数值模拟发现小麦哲伦云(SMC)的气体视线速度梯度源于近期与大麦哲伦云(LMC)碰撞(约1亿年前)导致的径向运动而非盘旋转，该碰撞还导致恒星运动转为弥散主导、气体旋转消失及恒星与气体中心偏移。


<details>
  <summary>Details</summary>

Background: 研究探讨了大麦哲伦云（LMC）与小麦哲伦云（SMC）近期碰撞对SMC内部结构和动力学的影响，揭示了星系碰撞在驱动不规则矮星系（dIrr）向椭圆/球状矮星系（dE/dSph）转变过程中的关键作用。

Data: 研究使用了N体流体动力学模拟（N-body hydrodynamical simulations）数据，初始条件为旋转的恒星和气体盘（rotating stellar and gaseous disks），模拟了近期（约100 Myr前）SMC与LMC的碰撞（impact parameter约2 kpc）及其对SMC内部结构和运动学的影响。

Method: 采用N体流体动力学模拟(N-body hydrodynamical simulations)分析SMC(小麦哲伦星云)与LMC(大麦哲伦星云)碰撞后的动力学演化过程。

Result: 小麦哲伦星系（SMC）与大麦哲伦星系（LMC）约1亿年前发生的碰撞（撞击参数约2千秒差距）解释了其内部结构和运动学特征，包括气体视向速度梯度由径向运动而非盘旋转引起、恒星与气体中心偏移以及恒星运动学转变为弥散主导。
...
### [5] [Globular clusters in \textsc{OrbIT}: complete dynamical characterisation of the globular cluster population of the Milky Way through updated orbital reconstruction](https://arxiv.org/abs/2512.06079)
*Michele De Leo,Manuela Zoccali,Julio Olivares-Carvajal,Belén Acosta-Tripailao,Felipe Gran,Rodrigo Contreras-Ramos*

**TL;DR**: 通过精确动力学分析银河系球状星团（globular clusters）并考虑时变势场（time-varying potential），更新了其原位形成（in-situ）与不同吸积事件（如Aleph、Antaeus等）的分类，深化了对银河系组装历史的理解。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系形成和演化过程中球状星团（globular clusters）的原位（in-situ）形成与吸积（accretion）过程的动力学特征及其对理解星系层级结构形成（hierarchical structure formation）的贡献。

Data: 使用定制构建的轨道积分器（orbit integrator）计算球状星团样本的精确轨道参数、运动积分和绝热不变量，并考虑旋转棒（rotating bar）导致的时间变化势场（time-varying potential）影响。

Method: 采用定制轨道积分器（custom built orbit integrator）并考虑旋转棒（rotating bar）引起的时变势场（time-varying potential），对球状星团（globular clusters）进行完整动力学表征。

Result: 通过精确计算球状星团轨道参数和动力学特征，更新了银河系内原位形成与不同吸积事件（如Aleph、Antaeus、Cetus、Elqui和Typhon）相关的球状星团分类目录。
...
### [6] [SHORES II: Multi-frequency Characterisation of the Sub-mJy Radio Population in FIR-selected Fields](https://arxiv.org/abs/2512.06089)
*Meriem Behiri,Marcella Massardi,Vincenzo Galluzzi,Marika Giulietti,Gayathri Gururajan,Isabella Prandoni,Andrea Lapi*

**TL;DR**: 利用澳大利亚望远镜致密阵列(ATCA)对两个河外天区进行多频段(2.1/5.5/9 GHz)深度射电巡天，探测到489个2.1 GHz源并分析其能谱分布(SED)，发现亚毫央斯基(sub-mJy)射电源谱指数(spectral index)集中在-0.7附近，证实其主导成分为恒星形成星系(SFG)，同时通过远红外-射电相关(FIRRC)分析识别出可能的高红移SFG候选体。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用多频段射电观测（2.1、5.5和9 GHz）探索河外星系暗弱射电源的光谱能量分布（SEDs）及亚毫央斯基（sub-mJy）射电源群体的物理性质。

Data: 数据来源于澳大利亚致密阵列望远镜(ATCA)在2.1、5.5和9 GHz三个频段对两个Herschel天体物理太赫兹大天区巡天(H-ATLAS)场中心0.5平方度区域的深度观测，灵敏度分别为9-17 μJy/beam(2.1 GHz)、28-39 μJy/beam(5.5 GHz)和38-61 μJy/beam(9 GHz)。

Method: 采用澳大利亚致密阵列望远镜(ATCA)在2.1、5.5和9 GHz频段进行深度多频射电观测，通过频谱能量分布(SED)分析和光谱指数测量研究亚毫央斯基射电源种群性质。

Result: 在2.1、5.5和9 GHz频段观测中，探测到489个2.1 GHz射电源（其中101个同时在5.5 GHz被检测到），其谱指数分布峰值位于α∼−0.7，符合微弱射电群体同步辐射特征。
...
### [7] [On the Origin of Intracluster Light based on the High-resolution Simulation, NewCluster](https://arxiv.org/abs/2512.06098)
...
**TL;DR**: 这篇论文通过高分辨率模拟(NewCluster)发现星系团内光(ICL)主要来源于卫星星系(包括存活和瓦解星系)，其中预处理成分更符合暗物质密度分布且具有年老、低金属丰度和高α元素丰度的特征，并提出通过ICL的轨道动力学、化学和人口特性定量追溯星系团组装历史的方法。


<details>
  <summary>Details</summary>

Background: 星系团内光（ICL）作为星系团的关键组分，其研究背景聚焦于通过ICL追踪星系团动力学组装历史及暗物质分布。

Data: 研究中使用的数据来源于正在进行的高分辨率星系团模拟NewCluster，通过严格追踪程序对星系团内及周围的数十亿粒子进行分类。

Method: 采用高分辨率星系团模拟(NewCluster)和严格粒子追踪程序分析ICL(Intracluster light)的起源及动力学特性。

Result: ICL（Intracluster light）主要来源于卫星星系（包括现存和瓦解的星系），其中预加工（preprocessed）组分与暗物质密度轮廓更吻合，具有年老、低金属丰度和高α元素丰度的特征。
...
### [8] [From the Intergalactic to the Interstellar Scales -- EQUALS: a High-resolution Legacy Survey of Gas in the Distant Universe Using ESPRESSO](https://arxiv.org/abs/2512.06159)
...
**TL;DR**: 利用ESPRESSO（Echelle SPectrograph for Rocky Exoplanet and Stable Spectroscopic Observations）高精度光谱仪开展的EQUALS（ESPRESSO Quasar Absorption Line Survey）巡天计划，通过类星体吸收线系统（quasar absorption lines）精确测量莱曼阿尔法森林（Lyman-alpha forest）最小尺度暗物质（dark matter）性质、z~4星系际介质（intergalactic medium）温度及早期宇宙恒星核合成产物（stellar nucleosynthetic yields），为宇宙演化模型提供关键观测约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过类星体吸收线（quasar absorption lines）探索宇宙从原始弥散气体演化至现今宇宙网（cosmic web）的过程，涉及暗物质性质、星系际介质（intergalactic medium）温度及早期宇宙恒星核合成贡献等核心天体物理问题。

Data: 数据来源于ESPRESSO（Echelle SPectrograph for Rocky Exoplanet and Stable Spectroscopic Observations）光谱仪对类星体吸收线（quasar absorption lines）的高精度观测。

Method: 采用高精度波长校准、高光谱分辨率和高灵敏度的ESPRESSO光谱仪（Echelle SPectrograph for Rocky Exoplanet and Stable Spectroscopic Observations）进行类星体吸收线（quasar absorption lines）观测，结合莱曼α森林（Lyman-alpha forest）分析和星系际介质（intergalactic medium）温度测量等核心方法。

Result: ESPRESSO（Echelle SPectrograph for Rocky Exoplanet and Stable Spectroscopic Observations）的高精度波长校准、高光谱分辨率和高灵敏度为通过类星体吸收线（quasar absorption lines）研究宇宙演化提供了前所未有的观测能力，能进一步约束星系演化的最先进宇宙学模拟和理论恒星核合成产物（stellar nucleosynthetic yields）。
...
### [9] [The Milky Way Project: Bridging Intermediate- and High-Mass Star Formation with the MIRION Catalog of Yellowballs](https://arxiv.org/abs/2512.06213)
...
**TL;DR**: 构建了中红外星际物体和星云（MIRION）目录，将候选中等质量恒星形成区（SFRs）数量提升近两个数量级，揭示了黄色球体（YBs）多为大质量年轻恒星天体产生的致密光解离区，并证实了红外颜色与恒星形成区物理特性及演化阶段的关联。


<details>
  <summary>Details</summary>

Background: 研究聚焦于中红外波段星际天体与星云（MIRION）目录的构建与应用，旨在探索从中低质量孤立恒星形成到高质量团簇恒星形成的过渡机制。

Data: 数据来源于银河系计划（Milky Way Project）参与者识别的6176个“黄球”（yellowballs, YBs）天体，并整合了中远红外测光数据、分子云速度、距离、星形成团块物理性质及与其他星表的交叉匹配信息。

Method: 采用基于志愿者识别的“黄球”（yellowballs）数据构建中红外星际物体与星云（MIRION）目录，整合中远红外测光、分子云速度、距离、恒星形成团块物理属性及跨目录匹配等多维数据表。

Result: MIRION星表将中等质量恒星形成区候选体数量提升了近两个数量级，为研究孤立低质量到成团高质量恒星形成的过渡提供了广泛数据库。
...
### [10] [Effects of helium sedimentation on late star formation in galaxy clusters](https://arxiv.org/abs/2512.06223)
*J. Racker,N. Padilla*

**TL;DR**: 研究发现氦沉积(helium sedimentation)通过增加星系团中心气体密度和改变原子核与电子分布，对中心星系恒星形成率的影响随晕质量(halo mass)变化：$M_{\rm halo} \lesssim 10^{14} M_{\rm sun}$时影响可忽略，$10^{14} M_{\rm sun} \lesssim M_{\rm halo} \lesssim 10^{15} M_{\rm sun}$时影响较弱，$M_{\rm halo} \gtrsim 10^{15} M_{\rm sun}$时可能显著重启中心星系恒星形成活动。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系团中氦元素沉降（helium sedimentation）如何通过改变气体密度和核子电子分布影响中心星系恒星形成历史。

Data: 采用一维数值模拟（1-dimensional numerical simulation）研究氦沉降（helium sedimentation）过程，并结合半解析恒星形成模型（semi-analytical model of star formation）进行分析。

Method: 采用一维数值模拟（1-dimensional numerical simulation）结合半解析恒星形成模型（semi-analytical model of star formation）分析氦元素沉降（helium sedimentation）对星系团中心恒星形成的影响。

Result: 氦元素沉降（helium sedimentation）对中心星系恒星形成率的影响在晕质量（halo mass）$M_{\rm halo} \gtrsim 10^{15} M_{\rm sun}$的星系团中可能非常显著，甚至能重新激发中心星系的恒星形成活动。
...
### [11] [Generating LSB-optimised synthetic images for simulated galaxies](https://arxiv.org/abs/2512.07320)
*Maarten Baes,Peter Camps,Andrea Gebek,Arno Lauwers,Joop Schaye,Paul Vauterin*

**TL;DR**: 这篇论文的核心贡献是在SKIRT辐射传输代码中引入了一种基于发射偏置的方案，通过按恒星粒子平滑长度（smoothing length）比例分配偏置因子来提升低密度区域光子发射效率，显著扩展了宇宙学模拟星系（如TNG50的银河系类似体）低表面亮度（LSB）区域可靠成像的深度。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于提升低表面亮度（LSB）天体合成图像生成效率的辐射传输（radiative transfer）方法优化。

Data: 研究中使用的数据来源于TNG50宇宙学模拟（cosmological simulation）中的一个类似银河系的星系。

Method: 采用基于平滑长度（smoothing length）的发射偏置（emission-biasing）方案优化低表面亮度（LSB）区域的合成图像生成效率。

Result: 通过引入与平滑长度(smoothing length)成比例的发射偏置因子，显著扩展了模拟星系低表面亮度(LSB)区域的可靠成像范围。
...
### [12] [A pan-galaxy study of synthetic giant molecular filaments: a turbulence-dominated life cycle](https://arxiv.org/abs/2512.06241)
*Zipeng Hu,Ke Wang,Mark R. Krumholz,Keyun Su*

**TL;DR**: 通过高分辨率磁流体动力学模拟分析700多条巨分子丝(GMF)，发现其形成主要由星系剪切和恒星反馈的湍流激波驱动，内部动力学受磁化湍流主导并遵循香肠不稳定性模型触发高效恒星形成，典型寿命约1400万年且70%以上经历云-云碰撞。


<details>
  <summary>Details</summary>

Background: 银河系巨分子丝(GMFs)的起源、动力学演化及其在恒星形成中的作用是当前星系分子云研究的前沿课题。

Data: 数据来源于对银河系类似星系的高分辨率磁流体动力学模拟生成的合成CO发射图。

Method: 采用高分辨率磁流体动力学(MHD)模拟结合合成CO发射图分析技术，重点考察湍流激波(galactic shear和stellar feedback)驱动机制及磁化湍流(magnetized turbulence)主导的内部动力学过程。

Result: 湍流激波（turbulent shock）和恒星反馈（stellar feedback）是巨分子纤维（GMFs）形成的主要驱动力。
...
### [13] [CLASSY XIV: The Nitrogen Exception -- Multi-Phase Enrichment and Feedback in High-$z$ Analogs](https://arxiv.org/abs/2512.06271)
...
**TL;DR**: 通过分析31个本地恒星形成星系中低电离（中性）与高电离（电离）气体的N、O、S、Fe丰度，发现O和S在相间分布均匀，Fe在中性气体中富集，N的电离相丰度比中性相高0.7 dex且与星风速度强相关，证实恒星反馈、外流及相依赖混合共同调控化学演化，解释了高红移星系极端N/O丰度的成因。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过分析31个本地恒星形成星系中星际介质（ISM）低电离（中性）与高电离（电离）两相金属含量，揭示恒星反馈、外流及相依赖混合对星系化学演化的调控机制，为理解宇宙黎明期星系极端氮氧丰度提供关键线索。

Data: 数据来源于COS Legacy Archive Spectroscopic SurveY (CLASSY)的31个本地恒星形成星系样本，结合共空间紫外吸收和光学发射线光谱测量了低电离（中性）和高电离（电离）气体中N、O、S和Fe的丰度。

Method: 采用共空间紫外吸收光谱和光学发射线光谱技术，对比分析低电离（中性）与高电离（电离）气体中N、O、S和Fe的丰度。

Result: O和S在低电离（中性）和高电离（电离）气体中混合良好，散射反映局部不均匀性。
...
### [14] [Unified models revisited I: modelling the effect of source geometry on radio galaxy/quasar unification](https://arxiv.org/abs/2512.06469)
*Siddhant Pinjarkar,Martin J. Hardcastle,Jonathon C. S. Pierce,Frits Sweijen*

**TL;DR**: 基于圆柱构型模拟射电噪类星体（radio-loud quasars）和射电星系（radio galaxies）的投影效应，提出验证统一模型需满足样本完全光学认证、可测尺寸且规模超过800个源。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于验证射电噪类星体（radio-loud quasars）与射电星系（radio galaxies）的取向统一模型（orientation-based unification model）中投影效应对观测尺寸的影响。

Data: 研究中使用的数据来源于基于圆柱构型（cylindrical configuration）的射电星系和类星体模拟，考虑了射电源的有限宽度和固有轴比分布（intrinsic axial ratio distribution），并模拟了不同样本量和临界角下类星体与射电星系线性尺寸比的分布。

Method: 采用圆柱构型（cylindrical configuration）模拟射电星系和类星体，并基于观测推导的固有物理尺寸和轴比分布预测观测长度比值。

Result: 基于圆柱构型模拟射电星系和类星体时，需考虑射电源有限宽度和固有轴比分布以准确预测观测长度比。
...
### [15] [The impact of surface acetylene cyclotrimerization on the abundance of aromatic hydrocarbons in carbon-rich asymptotic giant branch stars](https://arxiv.org/abs/2512.06510)
*M. S. Murga,I. V. Loginov,D. S. Wiebe,D. R. Fedotova,V. S. Krasnoukhov,I. O. Antonov*

**TL;DR**: 研究发现碳星渐近巨星分支(AGB)星拱星包层中尘埃表面催化乙炔环三聚反应是芳香烃形成的有效途径，能将芳香族物质丰度提升一个数量级，且气相化学与尘埃表面过程存在协同演化关系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于富碳渐近巨星分支（AGB）星周尘壳中尘埃颗粒表面催化乙炔环三聚形成芳香烃（aromatic hydrocarbons）的机制及其在天体化学演化中的作用。

Data: 数据来源于文献中已有的流体力学模型（hydrodynamical model）提供的IRC+10216星周包层物理条件。

Method: 采用综合计算天体化学模型（computational astrochemical model），耦合气相、气-固相和表面（环三聚化）反应以及尘埃颗粒（coagulation）的物理演化过程。

Result: 尘埃颗粒表面催化的乙炔环三聚反应是富碳渐近巨星分支(AGB)星周环境中芳香烃形成的可行途径，能将芳香族物种总丰度提升至多一个数量级。
...
### [16] [A New Torus Generator for AGAMA](https://arxiv.org/abs/2512.06512)
*James Binney,Thomas J Wright,Eugene Vasiliev*

**TL;DR**: 开发了AGAMAb代码库，可在任意轴对称引力势中计算并利用轨道环面（orbital tori），实现了高精度角作用量坐标转换，改进了潮汐流建模并应用于GD1星流分析。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发一种计算和利用轨道环面（orbital tori）的代码，以支持轴对称引力势（axisymmetric gravitational potential）下的星系建模和动力学分析。

Data: 数据来源于AGAMA软件包开发的AGAMAb代码库，支持从Python访问其大部分功能，并新增了处理天球坐标（sky coordinates）、视线方向（lines of sight）、距离和消光等观测数据的功能。

Method: 采用轨道环面（orbital tori）计算技术，结合角度-作用量（angle-action）坐标转换和插值方法，开发了AGAMAb代码库以支持任意轴对称引力势下的星系建模。

Result: 开发了能计算并利用任意轴对称引力势中轨道环面（orbital tori）的代码AGAMAb，其新增的环面生成器可高效创建高偏心率和近圆形轨道的环面，且支持通过插值低成本生成环面。
...
### [17] [Actions of highly eccentric orbits](https://arxiv.org/abs/2512.06519)
*Thomas J Wright,James Binney*

**TL;DR**: 论文证明了在非零角动量下仍可区分盒轨道（box orbit）和环轨道（loop orbit），并给出了确定临界积分I_{3crit}(E)和临界作用量Jzcrit的算法，同时改进了Staeckel Fudge方法以解决轨道作用量和频率计算困难的问题。


<details>
  <summary>Details</summary>

Background: 研究探讨了轴对称势场（axisymmetric potentials）中偏心轨道（eccentric orbits）动力学行为的计算挑战，重点关注轨道分类（box and loop orbits）与第三积分（I_3）的临界值关系。

Data: 研究中使用了基于Staeckel势（Staeckel potential）的模拟数据（simulation data）来分析轨道类型与第三积分（third integral I_3）的关系。

Method: 采用基于Staeckel势（Staeckel potential）的修正Fudge算法（modified Fudge）分析轨道类型（box or loop orbits）与第三积分I_3临界值I_{3crit}(E)的关系，并计算临界作用量Jzcrit。

Result: 在轴对称势场中，即使角动量非零，轨道仍可明确区分为盒轨道（box orbit）和环轨道（loop orbit），其分类取决于第三积分I_3与临界值I_{3crit}(E)的关系。
...
### [18] [Rotational Spectra and Search for Aromatic Imines: 9-Iminofluorene and Benzophenone imine](https://arxiv.org/abs/2512.06579)
*Huanyu Ren,D. Archie Stewart,Gabi Wenzel,Thomas H. Speak,Martin S. Holdren,Reace H. J. Willis,Brett A. McGuire*

**TL;DR**: 通过实验室测量9-亚氨基芴和苯甲酮亚胺的微波旋转光谱并尝试在TMC-1分子云中未探测到它们，结合量子化学计算提出了中间体形成存在能垒可能是其丰度低的原因。


<details>
  <summary>Details</summary>

Background: 研究聚焦于含氮不饱和多环芳烃（PAHs）如芳香亚胺（aromatic imines）的实验室光谱测量及其在星际空间中的搜寻，以探索复杂有机分子在星际介质中的形成机制和丰度限制。

Data: 数据来源于绿岸望远镜（Green Bank Telescope, GBT）对冷分子云TMC-1的观测以及实验室在6-18 GHz波段对9-亚氨基芴和苯甲酮亚胺的宽带光谱测量。

Method: 采用宽带微波旋转光谱测量技术（6-18 GHz）并结合高精度量子化学计算评估形成路径的势能面（potential energy surface）。

Result: result：在TMC-1中未检测到9-亚氨基芴和苯甲酮亚胺，其柱密度上限分别为5.1×10¹² cm⁻²和1.3×10¹³ cm⁻²，且未发现苯甲亚胺（E/Z异构体），量子化学计算表明中间体形成存在入口势垒可能是丰度低的原因。
...
### [19] [A XRISM/Resolve view of the dynamics in the hot gaseous atmosphere of M87](https://arxiv.org/abs/2512.06596)
...
**TL;DR**: 利用XRISM/Resolve微热量计直接测量了室女座星系团核心区气体速度，发现活动星系核(AGN)反馈在5千秒差距(kpc)尺度上产生高速度弥散($σ_v$=262 km/s)，向西北方向5-25 kpc迅速降至~60 km/s，表明该区域存在由激波驱动膨胀和各向同性湍流混合的超音速(三维马赫数0.69)动力学扰动。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用XRISM/Resolve微热量计直接测量室女座星系团核心区域气体速度，探索活动星系核(AGN)反馈在最近星系团中的运动学影响。

Data: 数据来源于XRISM/Resolve微热量计对室女座星系团核心区域气体速度的直接观测。

Method: 采用XRISM/Resolve微热量计（microcalorimeter）直接测量气体速度，并通过光谱分析揭示速度场特征。

Result: 在室女座星系团核心区观测到高速度弥散（$σ_v$=262 (+45 / -38) km/s），表明存在由激波驱动膨胀和各向同性湍流（isotropic turbulence）混合形成的跨声速（Mach数0.69 (+0.14 / -0.11)）热气体运动，非热压力占比达21 (+7 / -5)%。
...
### [20] [The influence of external environment at cosmic noon on the subsequent evolution of galaxy stellar mass](https://arxiv.org/abs/2512.06792)
*Tianmu Gao,J. Trevor Mendel,Lucas C. Kimmig,Claudia del P. Lagos,Rhea-Silvia Remus,Emily Wisnioski,Kathryn Grasha*

**TL;DR**: 通过EAGLE和MAGNETICUM模拟发现，$z\sim3$时1 cMpc球状过密度($δ_{1,\mathrm{sp}}$)是预测$z\sim0$星系恒星质量最重要的环境参数，高密度环境使后代星系质量可达低密度环境的8倍，且对$M_{*}\lesssim10^{10}M_{\odot}$星系主要源于更多并合增长，对更大质量星系则源于原位恒星形成和外部物质吸积共同作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过高红移（high-redshift）环境因素提升星系演化研究中高红移星系与低红移（low-redshift）后代匹配精度的科学问题。

Data: 研究使用了来自EAGLE和MAGNETICUM模拟的数据。

Method: 采用随机森林回归（random forest regression）评估高红移（$z\sim3$）环境指标对低红移（$z\sim0$）星系恒星质量的相对重要性，并识别1共动兆秒差距（cMpc）内的球状过密度（$δ_{1,\mathrm{sp}}$）为最关键的环境预测因子。

Result: 在红移(z)≳2时，1 cMpc球状过密度(δ1,sp)等环境因素对预测z∼0星系恒星质量的重要性与恒星质量相当，结合δ1,sp可使预测残差离散度在EAGLE和MAGNETICUM模拟中分别降低约20%和35%。
...
### [21] [The GLEAM 4-Jy (G4Jy) Sample: III. Further host-galaxy identification, and redshift assessment](https://arxiv.org/abs/2512.06947)
...
**TL;DR**: 通过分析MeerKAT、VLASS和RACS的射电数据，新识别了127个G4Jy样本（S_151MHz > 4 Jy）的宿主星系，并提供了红移范围0.0 < z < 3.6、151MHz光度及线性尺寸的初步测量结果。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过多波段射电观测数据（MeerKAT、VLASS和RACS）对G4Jy样本（S_151MHz > 4 Jy）进行宿主星系（host-galaxy）识别，并追求样本的100%光谱完备性（spectroscopic completeness）。

Data: 数据来源包括MeerKAT的射电图像、甚大阵列巡天（VLASS）和快速ASKAP连续谱巡天（RACS），并辅以K_s波段图像的射电等值线目视检查。

Method: 基于MeerKAT、VLASS和RACS的射电图像数据，结合Ks波段图像上的射电等值线目视检查（visual inspection）进行宿主星系（host-galaxy）识别。

Result: 基于多波段射电图像（MeerKAT、VLASS、RACS）和Ks波段光学图像目视检查，新识别出127个G4Jy样本（S_151MHz > 4 Jy）的宿主星系（host-galaxy），并通过数字光谱分析（digital optical spectroscopy）、测光红移（photometric redshifts）及NASA/IPAC数据库补充，使样本红移覆盖范围扩展至0.0 < z < 3.6，同时首次提供了151MHz光度（151-MHz luminosities）和线性尺度（linear sizes）的初步分析结果。
...
### [22] [JWST reveals extended stellar disks for ALMA-bright dusty star-forming galaxies in the Spiderweb protocluster](https://arxiv.org/abs/2512.07050)
...
**TL;DR**: 利用JWST/NIRCam数据首次系统分析了红移(redshift)z~2的Spiderweb原星系团中22个ALMA探测的尘埃遮蔽星暴星系(DSFGs)，发现其具有较大恒星盘和有效半径（中位质量log(M/Msun)=10.8±0.3）、晚型盘主导且形态多结块扭曲的动力学特征，支持致密环境中通过冷气体吸积驱动恒星盘加速增长的"由内而外"演化模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST/NIRCam数据系统分析蜘蛛网原星系团（Spiderweb protocluster）中红移z=2.16的尘埃遮蔽星暴星系（DSFGs）的形态学特征、结构演化及其在致密环境中的特殊性质。

Data: 数据来源为JWST/NIRCam成像数据和ALMA探测的尘埃恒星形成星系（DSFGs）样本，其中22个DSFGs被ALMA和JWST同时探测到，10个具有光谱证认的原星团成员身份。

Method: 采用JWST/NIRCam成像数据结合ALMA观测，通过Gini-M20形态分析、UVJ颜色诊断和有效半径测量等方法研究了原星团中尘埃遮蔽星暴星系(DSFGs)的形态、尘埃遮蔽程度及结构演化特征。

Result: 在蜘蛛网原星系团（Spiderweb protocluster）中，ALMA和JWST共同探测到的22个尘埃遮蔽的恒星形成星系（DSFGs）具有较大的恒星盘和有效半径（中位恒星质量log(M/Msun) = 10.8 ± 0.3），其尺寸-质量关系高于同期的场星系DSFGs和典型原星系团星系，表明致密环境中存在加速的结构演化。
...
### [23] [Statistics of the projected angles between the black-hole spin and the host-galaxy rotation axes from NewHorizon](https://arxiv.org/abs/2512.07129)
*Sebastien Peirani,Yasushi Suto,Clotilde Laigle,Yen-Ting Lin,Yohan Dubois,Sukyoung K. Yi*

**TL;DR**: 通过高分辨率宇宙学模拟(NewHorizon)发现低红移活动星系核(AGN)喷流方向与宿主星系结构存在显著对齐趋势，且运动学位置角(MaNGA-like)对齐更强。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）喷流与宿主星系结构之间的对齐关系，以揭示AGN统一模型、喷流反馈过程及星系与中心黑洞（BH）协同演化的关键机制。

Data: 数据来源于高分辨率宇宙学放大模拟（NewHorizon），通过合成DESI-LS和欧几里得（Euclid）图像的光学位置角估算星系方向，并采用蒙特卡洛方法对100个低红移黑洞-星系系统生成5000张模拟光学图像样本。

Method: 采用高分辨率宇宙学放大模拟（NewHorizon）自洽演化黑洞(BH)质量与自旋，结合合成DESI-LS和欧几里得(Euclid)图像的光学位置角估算星系取向，并以蒙特卡洛方法随机采样视角和红移生成5000幅模拟光学图像，通过黑洞自旋向量作为活动星系核(AGN)喷流方向代理进行统计分析。

Result: AGN喷流（jet）方向与宿主星系结构存在统计显著的共向性，且基于合成MaNGA-like恒星速度场获得的运动学位置角（kinematic position angle）显示出稍强的对齐趋势。
...
### [24] [FEASTS Compared with Simulations: Abnormally Irregular and Extended HI Morphologies at a Column Density of $10^{18}\,\text{cm}^{-2}$ in TNG50 and Auriga](https://arxiv.org/abs/2512.07223)
*Xuchen Lin,Jing Wang,Guinevere Kauffmann,Volker Springel,Rüdiger Pakmor*

**TL;DR**: 通过对比FAST观测与TNG50模拟的银河系类似中心星系低柱密度（$10^{18}\,\text{cm}^{-2}$）原子氢（HI）形态，发现超过三分之一的TNG50星系（尤其是大质量且HI贫乏的）呈现异常不规则和延展形态，且恒星反馈（stellar feedback）显著影响HI形态，而活动星系核（AGN）反馈和磁场的直接作用低于预期。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用中性氢（HI）观测数据与宇宙学模拟（cosmological simulations）首次统计比较银河系类似中心星系低柱密度（$10^{18}\,\text{cm}^{-2}$）区域的形态差异，以揭示恒星反馈（stellar feedback）和活动星系核（AGN）反馈对HI形态的影响机制。

Data: 数据来源包括FAST扩展目标巡天（FEASTS）的新原子氢（HI）观测数据，以及从IllustrisTNG50（TNG50）和Auriga宇宙学模拟中选取的匹配样本。

Method: 采用宇宙学模拟（IllustrisTNG50和Auriga）与FAST观测数据（FEASTS）的匹配样本，通过模拟观测（mock observation）在相同分辨率、深度、倾角和距离条件下进行统计比较，重点分析低柱密度（$10^{18}\,\text{cm}^{-2}$）原子氢（HI）形态差异及其与恒星反馈（stellar feedback）、活动星系核（AGN）反馈和磁场（magnetic field）的相关性。

Result: 超过三分之一的TNG50星系（尤其是大质量且氢贫乏的）表现出异常不规则和延展的原子氢（HI）形态，与FEASTS观测结果显著不同，且恒星反馈（stellar feedback）是导致这种差异的最显著相关因素，而活动星系核（AGN）反馈的作用不如预期直接。
...
### [25] [Relationship between Poincaré Sections and Spectral Characteristics of Orbits of Globular Clusters in the Central Region of the Galaxy](https://arxiv.org/abs/2512.07502)
*Anisa Bajkova,Anton Smirnov,Vadim Bobylev*

**TL;DR**: 研究了银河系中心区域45个球状星团(GCs)在旋转棒(bar)影响下的轨道动力学，发现庞加莱截面(Poincaré sections)与轨道频谱特征分类存在100%相关性，可用频谱分析替代庞加莱截面分类，并区分了规则与混沌轨道动力学的球状星团。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中心区域受旋转棒（rotating bar）影响显著的球状星团（GCs）的轨道动力学（orbital dynamics）特性，包括规则（regular）与混沌（chaotic）运动的区分与关联。

Data: 数据来源包括银河系中心区域半径3.5 kpc范围内的45个球状星团（GCs）样本，以及来自盖亚卫星（Gaia）迄今最精确的天体测量数据和新优化的平均距离，用于构建轨道积分所需的6维相空间。

Method: 采用庞加莱截面（Poincaré sections）和频谱方法（spectral methods）分析球状星团（GCs）轨道的规则或混沌动力学特性，并比较了两种方法的分类结果。

Result: Poincaré截面分类与轨道振幅谱分析之间存在100%相关性，可用后者替代前者进行更直观的轨道动力学分类。
...
### [26] [Chemical complexity in star formation induced by stellar feedback: cores shock-formed by the supernova remnant W44](https://arxiv.org/abs/2512.07562)
*G. Cosentino,I. Jiménez-Serra,F. Fontani,P. Gorai,C. -Y. Law,J. C. Tan,R. Fedriani,A. T. Barnes,P. Caselli,S. Viti,J. D. Henshaw*

**TL;DR**: 首次在超新星遗迹(SNR) W44与红外暗云G034.77-00.55交界处的Clump中探测到复杂有机分子(COMs)和氘代分子，其D/H比值(0.01-0.04)和COM丰度与典型低质量无星核及彗星值一致，表明SNR激波可能为低质量恒星形成设定了物理化学条件。


<details>
  <summary>Details</summary>

Background: 研究探讨超新星遗迹（SNR）低速激波如何影响分子云中恒星形成的物理与化学条件，特别是验证太阳可能通过此机制形成的假说。

Data: 数据来源于使用IRAM 30米天线和YEBES 40米天线在3毫米和7毫米波段获得的高灵敏度宽谱线巡测。

Method: 采用高灵敏度宽带光谱巡天观测（3毫米和7毫米波段），结合IRAM 30米和YEBES 40米射电望远镜，对目标分子云团（the Clump）进行氘代分子（D-bearing species）和复杂有机分子（COMs）的丰度测定与激发温度分析。

Result: 在超新星遗迹(SNR)W44与红外暗云G034.77-00.55交界处发现了一个具有星形成潜力的激波作用云核(Clump)，其氘代分子(DCO+、DNC、DCN、CH2DOH)和复杂有机分子(COMs)(CH3OH、CH3CHO、CH3CCH、CH3CN、CH3SH)的丰度与典型低质量无星核及彗星相当，表明该云核处于激波诱导的低质量星形成早期阶段。
...
### [27] [Interstellar Medium in Extremely High Star-Formation Regions: A Prospect of Observations on the Millimetron](https://arxiv.org/abs/2512.07565)
*E. O. Vasiliev,S. A. Drozdov,P. V. Baklanov,O. P. Vorobyov,S. Yu. Dedikov,M. S. Kirsanova,T. I. Larchenkova,N. N. Shakhvorostova*

**TL;DR**: 研究发现超亮红外星系（ultra-luminous infrared galaxies）在红移0-3区间内，其星际介质（interstellar medium）受高恒星形成率和活动星系核（active galactic nucleus）辐射影响，气体和尘埃发射特性发生显著变化，可通过远红外波段（100-500μm）的原子分子谱线和尘埃连续谱进行观测研究。


<details>
  <summary>Details</summary>

Background: 研究主题为极端条件下超亮红外星系（ultra-luminous infrared galaxies）中星际介质（interstellar medium）受高恒星形成率和活动星系核（active galactic nucleus）辐射影响的物理过程与观测方法。

Data: 数据来源于远红外波段（100-500μm）的原子分子谱线和尘埃连续谱观测，以及Millimetron空间天文台的仪器观测。

Method: 采用远红外波段（100-500μm）的原子分子谱线和尘埃连续谱分析极端条件下超亮红外星系（ultra-luminous infrared galaxies）星际介质的物理状态。

Result: 在超亮红外星系(ultra-luminous infrared galaxies)中，高恒星形成率和活动星系核(active galactic nucleus)辐射显著改变了星际介质的发射特性，使气体和尘埃在高激发态下产生远红外波段(100-500μm)的原子分子谱线和连续辐射。
...
### [28] [Diverse stages of star formation in the IRAS 18162-2048 region. Emergence of UV Feedback](https://arxiv.org/abs/2512.07604)
...
**TL;DR**: 通过近红外和射电观测发现IRS 7/SC是一颗B2/B3型零龄主序星（ZAMS），其开始电离周围环境并形成扩展光致离解区（PDR）和致密电离氢区（HII region），表明该区域存在多代恒星形成活动。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近红外和射电波段对原恒星形成区IRAS 18162-2048及其周边多代恒星形成环境的物理条件与动力学过程进行多波段联合观测。

Data: 数据来源包括VLT/SINFONI提供的近红外（IR）K波段（1.93-2.47 μm）自适应光学辅助积分场光谱、VLA X和C波段（3-6 cm）观测以及ALMA波段3（约3.3 mm）观测。

Method: 采用自适应光学辅助的近红外（IR）积分视场光谱技术（VLT/SINFONI）结合VLA X和C波段（3-6 cm）及ALMA 3波段（~3.3 mm）观测数据，并运用辐射传输模型Cloudy模拟H₂转动振动能级分布。

Result: 近红外和射电观测表明IRS 7/SC是一颗B2/B3零龄主序星(ZAMS)，已开始光致电离周围环境，形成了延展的光离解区(PDR)和致密的氢电离区(H ii region)。
...
### [29] [The shape-velocity alignment of satellites forged by tidal locking and dynamical friction](https://arxiv.org/abs/2512.07637)
...
**TL;DR**: 基于TNG50模拟发现卫星/次晕（subhalos）的径向排列（radial alignment）显著强于轨道排列（orbital alignment），且两者在次晕中更明显，其中轨道排列行为受径向排列、动力学摩擦（dynamical friction）轨道衰减和潮汐剥离（tidal stripping）影响，而Pace等人报道的银河系（MW）卫星排列实际是轨道平面与观测者共面导致的投影效应。


<details>
  <summary>Details</summary>

Background: 研究利用TNG50模拟探讨了卫星/亚晕（subhalos）的两种空间排列模式：主轴向与星系中心径向（radial alignment）及运动方向（orbital alignment）的对齐关系。

Data: 数据来源于TNG50（IllustrisTNG-50）模拟。

Method: 采用TNG50数值模拟分析卫星/次晕(subhalos)的主轴与银心径向方向(径向排列)及运动方向(轨道排列)的两种对齐模式。

Result: 径向排列（radial alignment）显著强于轨道排列（orbital alignment），且亚晕（subhalos）的两种排列信号均强于卫星（satellites）。
...
### [30] [The heartbeat of stellar halos: Insights from the stellar halo mass-metallicity relation](https://arxiv.org/abs/2512.07780)
...
**TL;DR**: 研究发现星系金属丰度-晕质量关系（MZhR）在红移z=3.5时已形成与现今相似的斜率，且通过稳定性时间（tst）可追溯恒星晕达到现今MZhR的时刻及最后一次主要并合事件。


<details>
  <summary>Details</summary>

Background: 研究探讨了质量-金属丰度关系（MZhR）从红移z=3.5到z=0的存在与演化过程，并确定了星系何时稳定在当前MZhR上。

Data: 数据来源于CIELO模拟（CIELO simulations）中log10(Mgal/Msun)=[9,11]的中心星系（central galaxies），使用AM-E方法（AM-E method）从z=3.5到z=0识别恒星晕（stellar halos），重点关注1.5倍光学半径（optical radius）与维里半径（virial radius）之间的区域。

Method: 采用AM-E方法（AM-E method）分析CIELO模拟中红移z=3.5至z=0的恒星晕（stellar halos），并引入晕轮心动图（halo cardiograms）和稳定性时间（tst）作为恒星晕组装历史的新研究手段。

Result: 在红移z=3.5时恒星晕已形成与现今斜率相似的金属丰度-质量关系（MZhR），其金属丰度随红移降低固定质量增加约0.21 dex，反映卫星吸积带来的渐进化学增丰。
...
### [31] [Hot, Photoionized X-ray Gas in Two Luminous Type 2 Quasars: Chandra-HST Evidence for a Wind-Driven Sequence](https://arxiv.org/abs/2512.07793)
...
**TL;DR**: 利用钱德拉X射线观测结合哈勃[O III]形态动力学分析，发现两个类星体(FIRST J120041和2MASX J130038)的X射线发射与[O III]存在空间关联差异，揭示了热气体质量远超[O III]外流物质，并构建了类星体反馈演化序列模型，表明二者分别处于核周气体清除过程中热风涌现与主导的过渡阶段。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核(AGN)反馈机制演化序列，探讨类星体如何通过辐射和星风逐步清除环核气体从而调控星系演化。

Data: 数据来源为钱德拉(Chandra)/ACIS-S成像光谱仪对两个类星体(FIRST J120041.4+314745和2MASX J13003807+5454367)的X射线观测数据，以及哈勃太空望远镜(Hubble Space Telescope)的[O III]λ5007形态学和运动学数据。

Method: 采用钱德拉X射线天文台（Chandra/ACIS-S）成像光谱技术结合哈勃太空望远镜（HST）[O III]λ5007形态与运动学分析，通过光谱建模（photoionization modeling）量化软X射线主导机制，并估算热气体储层质量（M$_{\rm x-ray}$）。

Result: 在2MASX J130038中探测到一个可能蓝移的Fe XXVI Lyα线（E$_{\rm rest}$ = 7.14 $\pm$ 0.06 keV，速度约7600 km s$^{-1}$），表明存在局限于内几百秒差距的新生热风。
...
### [32] [The THESAN project: Lyman-alpha intensity mapping of cosmic reionization](https://arxiv.org/abs/2512.06085)
...
**TL;DR**: 利用THESAN宇宙学模拟预测了再电离时期(EoR)莱曼阿尔法(Lyman-alpha)谱线强度成图(LIM)的空间涨落功率谱，发现含吸收效应的功率谱在小尺度(k > 10^(-2) 1/arcsec)上随红移降低而变陡，且仅考虑发射的信号高于SPHEREx灵敏度而含吸收的信号低4个量级，同时简单外流模型可使功率提升4个量级。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用莱曼阿尔法(Lyman-alpha)线强度映射(LIM)技术探测再电离时期(EoR)大尺度结构的演化，并探讨中性氢(HI)在弥散星系际介质(IGM)中的共振吸收对莱曼阿尔法辐射的影响。

Data: 研究使用了最先进的THESAN宇宙学模拟（cosmological simulation）数据，构建了连续光锥（light cones）以进行视线方向宇宙学积分，并评估了复合辐射、碰撞激发和未分辨HII区对总莱曼阿尔法（Lyman-alpha）光谱强度的贡献。

Method: 采用高分辨率THESAN宇宙学模拟构建连续光锥(light cone)进行视线方向积分，并利用阻尼翼(damping wing)分析评估不同红移下中性氢吸收贡献。

Result: 莱曼阿尔法(Lyman-alpha)强度映射(LIM)功率谱在小尺度(k > 10^(-2) 1/角秒)上的斜率随红移降低而变陡，且仅考虑发射的功率谱高于SPHEREx灵敏度，而包含吸收的信号低约4个数量级。
...
### [33] [Generalized tension metrics for multiple cosmological datasets](https://arxiv.org/abs/2512.06086)
*Matías Leizerovich,Susana J. Landau,Claudia G. Scóccola*

**TL;DR**: 提出了一种新型多数据集统计张力估计器（Q_DM），通过几何方法量化宇宙学数据集间主导张力方向，重新解释DESI和普朗克（Planck）数据间的张力从1.18σ^eff（DR1）增至1.45σ^eff（DR2）。


<details>
  <summary>Details</summary>

Background: 开发了一种新型估计量（estimator）用于同时量化多个宇宙学数据集之间的统计张力（statistical tensions）。

Data: 数据来源于DESI（暗能量光谱仪）和Planck（普朗克卫星）的观测数据，具体包括DESI数据发布1（DR1）和数据发布2（DR2）以及Planck的相应数据集。

Method: 采用了一种推广差异均值统计量(Q_DM)的新型估计器，用于同时量化多个宇宙学数据集间的统计张力。

Result: 提出了一种新型估计量(Q_DM)来同时量化多个宇宙学数据集间的统计张力，并将其推广至多数据集场景。
...
### [34] [Black hole merger rates for LISA and LGWA from semi-analytical modelling of light seeds](https://arxiv.org/abs/2512.06094)
...
**TL;DR**: 研究发现LISA和LGWA探测器每年可探测超过15个中等质量黑洞(IMBHs)合并事件（乐观情况），并揭示LGWA更适合高红移IMBHs探测而LISA对超大质量黑洞(SMBHs)更敏感，联合观测将覆盖完整黑洞质量谱。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用LISA和LGWA等空间与月球引力波探测器探索中等质量黑洞(IMBH)和超大质量黑洞(SMBH)并合事件，以开启引力波天文学新时代。

Data: 使用基于PINOCCHIO宇宙学模拟和基于GAEA的半解析模型生成合成黑洞（BH）双星群体数据，并采用"轻"种子方案（light seeding scheme）及钱德拉塞卡（Chandrasekhar）公式计算合并时标。

Method: 采用PINOCCHIO宇宙学模拟和基于GAEA的半解析模型，通过“轻量级”(light)种子方案构建黑洞(BH)双星种群，并利用钱德拉塞卡(Chandrasekhar)公式计算合并时标。

Result: LISA和LGWA在乐观情况下每年可探测超过15个中等质量黑洞（IMBHs）并合事件，悲观情况下整个探测器寿命期内探测数量低于约5次。
...
### [35] [The eROSITA Final Equatorial-Depth Survey (eFEDS): X-ray stacking analysis of Subaru's optically selected clusters spanning low richness regime](https://arxiv.org/abs/2512.06138)
...
**TL;DR**: 基于SRG/eROSITA和Subaru HSC数据，研究发现997个光学选星系团（CAMIRA clusters）的X射线光度-质量关系（$L-M$）斜率（1.56）略高于自相似预期，而富度-质量关系（$N-M$）斜率（0.766）符合理论预测，揭示了X射线与光学选星系团在X射线性质上的系统性差异。


<details>
  <summary>Details</summary>

Background: 研究星系团（galaxy cluster）的X射线可观测特性与质量之间的标度关系（scaling relation），并分析光学选星系团的径向X射线剖面特征。

Data: 数据来源于SRG/eROSITA最终赤道深度巡天(eFEDS)的X射线观测数据和Subaru Hyper Suprime-Cam (HSC)巡天的光学选星系团样本。

Method: 采用叠加分析（stacking analysis）结合弱引力透镜质量校准（weak-lensing mass calibration）研究光度-质量（$L-M$）和富度-质量（$N-M$）标度关系，并考虑选择效应和红移演化影响。

Result: 通过叠加分析发现，$L-M$关系斜率(1.56^{+0.14}_{-0.12})略陡于自相似(self-similar)预期但与先前结果一致，$N-M$斜率(0.766^{+0.070}_{-0.060})符合理论预测且无需额外红移演化修正。
...
### [36] [A Fully Photometric Approach to Type Ia Supernova Cosmology in the LSST Era: Host Galaxy Redshifts and Supernova Classification](https://arxiv.org/abs/2512.06319)
...
**TL;DR**: 通过模拟维拉·鲁宾天文台LSST巡天数据，结合光变曲线分类器(SCONE)和BEAMS with Bias Corrections (BBC)方法，利用约6000颗光测分类Ia型超新星(SNeIa)构建哈勃图，在wCDM和CPL暗能量模型下获得显著优于DESVYR的FoM值(约150)，但发现存在微小系统性偏差需进一步优化。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用维拉·鲁宾天文台(Vera C. Rubin Observatory)的时空遗产巡天(LSST)发现的大量Ia型超新星(SNeIa)，通过纯测光方法约束暗能量(dark energy)性质。

Data: 数据来源包括基于扩展LSST天文时间序列分类挑战（ELAsTiCC）的详细模拟数据集（包含多波段光变曲线、非Ia型超新星污染、宿主星系错误关联和瞬变-宿主相关性）、高红移深钻场（DDF）约50平方度天区的观测数据，以及基于宽快深（WFD）天区的光谱确认低红移样本。

Method: 采用联合超新星+宿主星系测光红移拟合、基于神经网络的光度分类器(SCONE)以及带偏差校正的BEAMS(BBBC)方法构建偏差校正后的哈勃图。

Result: 基于ELAsTiCC模拟数据，采用SCONE分类器和BEAMS+BBC方法对约6000颗光变曲线分类的Ia型超新星(SNeIa)进行宇宙学分析，在wCDM模型下获得的品质因子(FoM)达到约150，显著优于DESVYR的54。
...
### [37] [Testing the Distance Duality Relation with Cosmological Observations at high Redshift using Artificial Neural Network](https://arxiv.org/abs/2512.06454)
*Yukang Xie,Yang Liu,Puxun Wu,Xiangyun Fu,Nan Liang*

**TL;DR**: 利用Pantheon+超新星、Fermi伽马暴、DESI重子声波振荡和强引力透镜系统数据，通过人工神经网络方法验证了高红移(0.01 < z ≲ 8)下宇宙距离对偶关系(DDR)在2σ置信度内与标准理论一致。


<details>
  <summary>Details</summary>

Background: 宇宙距离对偶关系（DDR）是度量引力理论在光子数守恒条件下的基本预测。

Data: 数据来源包括Pantheon+ Ia型超新星(SN Ia)、Fermi伽马射线暴(GRBs)的FULL和GOLD样本、暗能量光谱仪(DESI)数据发布2(DR2)的重子声学振荡(BAO)测量结果，以及使用人工神经网络(ANN)方法处理的高红移($0.01 < z \lesssim 8$)星系尺度强引力透镜(SGL)系统样本。

Method: 采用人工神经网络（ANN）方法结合Pantheon+ Ia型超新星（SN Ia）、Fermi伽马射线暴（GRBs）的FULL和GOLD样本、DESI DR2重子声学振荡（BAO）测量数据以及高红移（0.01 < z ≲ 8）星系尺度强引力透镜（SGL）系统样本进行模型无关的宇宙距离对偶关系（DDR）检验。

Result: 标准宇宙距离对偶关系（DDR）在高红移（$0.01 < z \lesssim 8$）范围内与宇宙学观测结果在$\sim 2 σ$置信水平内一致。
...
### [38] [Angular BAO Forecasts for the IBIS Medium-Band Survey](https://arxiv.org/abs/2512.06568)
*Richard M. Feder,Martin White*

**TL;DR**: 论文核心贡献是评估了中波段成像巡天（IBIS）测量角向重子声波振荡（BAO）的可行性，发现5000平方度巡天能在红移2.8处以2.6%精度约束BAO膨胀参数α，并探讨了未来光谱仪（Spec-S5）实现高红移精确BAO测量的潜力。


<details>
  <summary>Details</summary>

Background: 研究探讨了利用成像巡天（如IBIS和Spec-S5）进行角向重子声波振荡（BAO）测量的可行性及其对高红移宇宙大尺度结构（large-scale structure）的约束潜力。

Data: 数据来源为中间波段成像巡天（Intermediate-Band Imaging Survey, IBIS）的试点巡天数据，覆盖红移范围2.2<z<3.5，并计划扩展至5000平方度的莱曼α发射体（LAEs）巡天。

Method: 采用费希尔预测(Fisher forecast)方法分析中波段成像巡天(IBIS)的角重子声波振荡(BAO)测量可行性，并评估不同样本特性对BAO膨胀参数α的约束精度。

Result: 通过费希尔(Fisher)预测分析，发现5000平方度中波段成像巡天(IBIS)在有效红移(zeff)=2.8处能以2.6%的精度约束重子声波振荡(BAO)膨胀参数α，其样本特性影响与散粒噪声主导的测量结果一致。
...
### [39] [Probing Cosmic Magnetism with Rotation Measure-Squared-Galaxy Cross-Correlations](https://arxiv.org/abs/2512.06584)
*Zekai Zhang,Adam Lidz*

**TL;DR**: 提出了一种通过河外法拉第旋转量(RM)与星系巡天的互相关提取宇宙磁场信息的新方法，利用RM平方与前景星系密度场的两点互相关统计量⟨RM²×g⟩实现无污染、无噪声偏差的测量，并结合红移分层重建宇宙大尺度磁场的演化历史，发现其有效场强主要由星系暗晕内部区域的磁场幅度决定并随红移降低而增强。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过河外法拉第旋转量(RM)目录与星系巡天的互相关提取宇宙磁场信息，旨在探索大尺度宇宙磁场的红移演化及其与星系分布的关联。

Data: 数据来源于外源法拉第旋转测量（Faraday rotation measure, RM）目录、星系巡天数据以及Illustris-TNG宇宙磁流体动力学模拟（cosmological magnetohydrodynamic simulations）。

Method: 采用两点互相关方法测量背景源的法拉第旋转量平方（RM²）与前景星系投影密度场（g）的横向分离函数⟨RM²×g⟩，并基于Illustris-TNG宇宙磁流体动力学模拟建模该统计量。

Result: 提出了一种通过河外法拉第旋转量(RM)平方与前景星系密度场的两点互相关$\langle {\rm RM}^2 \times {\rm g} \rangle$来提取宇宙磁场信息的新方法，该方法可避免噪声偏差并实现大尺度宇宙磁场的层析重建。
...
### [40] [What Galaxy Clusters Have to Say About Dynamical Dark Energy and $H_0$](https://arxiv.org/abs/2512.06701)
*Andrés N. Salcedo,Eduardo Rozo,Hao-Yi Wu,Shulei Cao,Enrique Paillas,Hanyu Zhang,Eli S. Rykoff*

**TL;DR**: 研究发现低红移（low-redshift）结构探测（包括星系团丰度、3×2点分析和全形状聚类）在平坦ΛCDM模型中相互一致，联合给出了与几何数据集（CMB+BAO+SN）相符的σ₈和Ωₘ精确约束，并在w₀wₐCDM模型中通过加入星系团数据降低了暗能量演化的证据，同时缓解了H₀张力，支持晚期宇宙遵循ΛCDM演化且声视界（sound horizon）偏离标准值。


<details>
  <summary>Details</summary>

Background: 研究探讨了在平坦ΛCDM（冷暗物质）和w0waCDM（动态暗能量）宇宙学模型中，低红移结构探针（如星系团丰度、3×2点分析和全形状聚类）对宇宙学参数（σ8和Ωm）的约束及其与几何数据集（CMB+BAO+SN）的一致性。

Data: 数据来源包括低红移结构探针（low-redshift structure probes）——星系团丰度（cluster abundances）、3×2点分析（3×2-point analyses）和全形状聚类（full-shape clustering），以及几何数据集（geometrical datasets）——宇宙微波背景（CMB）、重子声学振荡（BAO）和超新星（SN）。

Method: 采用低红移结构探针（cluster abundances、3×2-point analyses、full-shape clustering）联合分析平坦ΛCDM模型中的σ₈和Ωₘ参数。

Result: 在平坦ΛCDM模型中，低红移结构探针（cluster abundances、3×2点分析和全形状聚类）相互一致，共同提供了与几何数据集（CMB+BAO+SN）相符的σ₈和Ωₘ的精确约束。
...
### [41] [An Extended WZDR Model with Interacting Scalar Field Dark Matter and Stepped Dark Radiation](https://arxiv.org/abs/2512.06719)
*Gang Liu*

**TL;DR**: 研究将冷暗物质（cold dark matter）替换为标量场暗物质（scalar field dark matter）并与阶梯暗辐射（stepped dark radiation）通过纯动量耦合相互作用，构建的新模型仅略微改善了哈勃常数（H0）和S8参数的拟合值，但未能完全解决宇宙学张力问题。


<details>
  <summary>Details</summary>

Background: 研究聚焦于标量场暗物质（scalar field dark matter）与阶梯型暗辐射（stepped dark radiation）的相互作用，作为WZDR模型的扩展，旨在缓解哈勃张力（Hubble tension）和$S_8$张力。

Data: 数据来源于宇宙微波背景辐射(CMB)、重子声学振荡(BAO)、Ia型超新星、SH0ES的H0测量、暗能量巡天第三年(DES Y3)的S8数据以及阿塔卡马宇宙学望远镜(ACT)的数据。

Method: 采用纯动量耦合（pure momentum coupling）技术手段构建标量场暗物质（scalar field dark matter）与阶梯暗辐射（stepped dark radiation）相互作用的新模型。

Result: 新模型通过标量场暗物质（scalar field dark matter）与阶梯暗辐射（stepped dark radiation）的纯动量耦合，在缓解哈勃张力和$S_8$张力方面仅带来边际改进，最佳拟合值$H_0$为70.89 km/s/Mpc，$S_8$降至0.8113，但耦合信号较弱（$\log_{10}(ξ)<4.56$），未能完全解决宇宙学矛盾。
...
### [42] [Probing memory-burdened Primordial Black Holes with global 21 cm signal](https://arxiv.org/abs/2512.06941)
*Priyanka Sarmah,Kingman Cheung*

**TL;DR**: 研究了记忆负担效应(memory burden effect)影响的原初黑洞(PBH)对宇宙黎明时期21厘米信号的印记，发现质量在10^8-10^13克且慢过渡(δ=10^-2)的PBH作为暗物质的比例被限制在f_PBH≳10^-8，而快过渡(k=1)时质量≳10^7克的PBH不受21厘米信号限制。


<details>
  <summary>Details</summary>

Background: 研究探讨了记忆负担效应（memory burden effect）对宇宙黎明时期全球21厘米信号的潜在影响，聚焦于原初黑洞（PBH）作为暗物质候选者在低质量区间的重新评估及其量子引力修正下的蒸发行为。

Data: 研究中使用的数据来源包括基于记忆负担效应（memory burden effect）修正的原初黑洞（PBH）蒸发模型计算得出的能量注入率，以及中性氢的热和电离演化过程中对全球21厘米信号（global 21 cm signal）在红移z≈17处的吸收振幅的观测数据。

Method: 采用记忆负担效应（memory burden effect）修正的蒸发模型计算原初黑洞（PBH）能量注入率，并结合中性氢的热与电离演化分析21厘米信号吸收幅度的变化。

Result: 记忆负担效应（memory burden effect）显著延长了轻质量原初黑洞（PBH）的寿命，通过修改其晚期发射光谱影响了宇宙黎明时期的全球21厘米信号吸收幅度。
...
### [43] [Prospects for independent measurement of $\boldsymbol{\ell}$=1,2,3 CMB anisotropy multipoles using the anisotropic Sunyaev-Zel'dovich effect](https://arxiv.org/abs/2512.07441)
*D. I. Novikov,A. O. Mihalchenko,A. M. Osipova,K. O. Parfenov,S. V. Pilipenko*

**TL;DR**: 通过探测星系团热等离子体散射导致的宇宙微波背景各向异性谱畸变（各向异性Sunyaev-Zel'dovich效应），提出改进的最小响应法（Least Response Method）实现多极矩独立测量，并区分Sachs-Wolf效应与积分Sachs-Wolf效应（Rees-Sciama效应），同时部分规避低多极矩的宇宙方差问题。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙微波背景（cosmic microwave background）特定谱畸变的观测前景，该畸变由星系团热等离子体散射辐射各向异性引起。

Data: 研究中使用的数据来源于模拟信号（simulated signal），该信号包含多种光谱形状不明确的 foreground（前景）污染以及由 Sunyaev-Zel'dovich 效应及其相对论性修正引起的 relic blackbody spectrum（遗迹黑体谱）畸变。

Method: 采用改进的最小响应方法（modified Least Response Method）进行数据处理中的成分分离。

Result: result：探测各向异性苏尼亚耶夫-泽尔多维奇效应（anisotropic Sunyaev-Zel'dovich effect）可独立测量低阶多极矩（$\ell=1,2,3$），区分萨克斯-沃尔夫效应（Sachs-Wolf effect）与积分萨克斯-沃尔夫效应（Rees-Sciama effect），并部分规避低多极矩的宇宙方差（cosmic variance）问题。
...
### [44] [Core-halo scaling relations in self-interacting scalar field dark matter](https://arxiv.org/abs/2512.07020)
*Jessica N. López-Sánchez,Erick Munive-Villa,Tanja Rindler-Daller*

**TL;DR**: 通过三维格罗斯-皮塔耶夫斯基-泊松模拟研究标量场暗物质(SFDM)晕中自相互作用对孤子核结构演化的影响，发现排斥性相互作用形成质量更大、延展性更强且中心密度更低的核，而吸引性相互作用超过临界质量会增强中心密度并导致核坍缩，同时揭示了核质量-半径关系及核-晕质量关系对自相互作用强度和符号的非普适依赖性。


<details>
  <summary>Details</summary>

Background: 研究探讨了标量场暗物质(SFDM)晕中自相互作用对结构和演化的影响，聚焦于不同散射长度下排斥与吸引相互作用的效应及其对暗物质晕核心特性的调控机制。

Data: 使用三维Gross-Pitaevskii-Poisson模拟（simulation）研究了多种孤子（soliton）合并过程，覆盖了从排斥到吸引的广泛散射长度（scattering length）范围。

Method: 采用三维Gross-Pitaevskii-Poisson（格罗斯-皮塔耶夫斯基-泊松）模拟研究多孤子合并过程，分析不同散射长度下排斥性和吸引性自相互作用对SFDM（标量场暗物质）晕结构的影响。

Result: 排斥自相互作用导致更大质量、更延展且中心密度低于自由模糊暗物质情形的核，而吸引相互作用增强中心密度并在超过临界质量时促使核坍缩。
...
### [45] [A Multi-Probe ISW Study of Dark Energy Models with Negative Energy Density: Galaxy Correlations, Lensing Bispectrum, and Planck ISW-Lensing Likelihood](https://arxiv.org/abs/2512.07060)
*Payam Ghafari,Mahdi Najafi,Mina Ghodsi Yengejeh,Emre Özülker,Eleonora Di Valentino,Javad T. Firouzjaee*

**TL;DR**: 对比研究三种暗能量（DE）模型（CPL参数化、ΛsCDM和全能DE模型）在引力势演化晚期对宇宙微波背景（CMB）各向异性（特别是ISW效应和透镜-ISW双谱）的影响，发现它们虽在高阶多极子CMB谱上与标准ΛCDM难以区分，但在大尺度ISW平台和ISW相关高阶统计量中展现出独特特征，凸显了结合多探针方法对区分负或变号暗能量的关键作用。


<details>
  <summary>Details</summary>

Background: 研究探讨了三种暗能量（DE）模型（CPL参数化、$Λ_{\rm s}$CDM和全能DE模型）在宇宙学观测中对引力势时间演化的晚期印记。

Data: 数据来源于宇宙微波背景辐射（CMB）的双点和三点相关分析，重点关注集成萨克斯-沃尔夫效应（ISW）和透镜-ISW双谱。

Method: 采用基于集成萨克斯-沃尔夫效应（ISW）和透镜-ISW双谱的多探针框架，分析两点和三点宇宙微波背景（CMB）关联以评估暗能量模型的晚期动力学特征。

Result: 三种暗能量（DE）模型（CPL参数化、$Λ_{\rm s}$CDM和全能DE模型）在低$\ell$ ISW平台和与ISW相关的高阶统计量中产生显著特征，凸显了大尺度CMB各向异性和高阶CMB统计量在检验暗能量物理中的重要性。
...
### [46] [Dynamical Dark Energy in light of DESI BAO and Full-Shape Data](https://arxiv.org/abs/2512.07104)
*Quan Zhou,Sibo Zheng*

**TL;DR**: 该研究结合DESI Y1物质功率谱（matter power spectrum）和DR2重子声波振荡（BAO）数据，显著提升了对动态暗能量（DDE）参数$w_0$和$w_{a}$的限制精度，尤其在DESY5样本中表现突出。


<details>
  <summary>Details</summary>

Background: 研究探讨了动力学暗能量（DDE）相较于标准宇宙学模型（ΛCDM）的潜在优势，重点关注低红移物质功率谱测量对DDE模型的敏感性。

Data: 数据来源包括DESI Y1全形数据提取的物质功率谱（matter power spectrum）、DESI DR2重子声波振荡（BAO）数据以及DESY5样本。

Method: 采用马尔可夫链蒙特卡洛（Markov Chain Monte Carlo, MCMC）分析方法结合DESI Y1似然函数对动力学暗能量（DDE）模型参数进行约束。

Result: 结合DESI Y1物质功率谱数据显著提升了动态暗能量(DDE)模型参数w0和wa的约束精度，尤其在DESY5样本中效果最为明显。
...
### [47] [Vision Transformers for Cosmological Fields: Application to Weak Lensing Mass Maps](https://arxiv.org/abs/2512.07125)
*Jash Kakadia,Shubh Agrawal,Kunhao Zhong,Bhuvnesh Jain*

**TL;DR**: 研究发现基于注意力机制（attention-based）的变换器（transformers）架构（如Swin变换器）在弱引力透镜（weak gravitational lensing）质量图中约束宇宙学参数（$Ω_m$和$S_8$）时优于传统卷积神经网络（CNNs），尤其在训练数据有限时表现更优，但在实际噪声条件下两者宇宙学品质因子（Figure of Merit）相当。


<details>
  <summary>Details</summary>

Background: 弱引力透镜（weak gravitational lensing）是研究宇宙增长历史的有力工具，尤其关注从小尺度非线性结构中提取非高斯信息以约束宇宙学参数。

Data: 数据来源于N体模拟（N-body simulations）生成的弱引力透镜质量图（weak lensing mass maps），并采用模拟推断框架（simulation-based inference framework）进行分析。

Method: 采用基于注意力机制（attention-based）的架构，包括视觉变换器（ViTs）及其变体（如Swin变换器），结合模拟推断（SBI）框架，从弱引力透镜质量图中提取非高斯信息以约束宇宙学参数。

Result: 基于注意力机制（attention-based）的架构（如Swin变换器）在弱引力透镜（weak lensing）质量图中约束宇宙学参数$Ω_m$和$S_8$时表现优于传统视觉变换器（ViTs），尤其在训练数据有限时，但其在真实噪声条件下的宇宙学品质因子（Figure of Merit）与卷积神经网络（CNNs）相当。
...
### [48] [Inference of $B$-mode polarization in the presence of non-Gaussian foregrounds](https://arxiv.org/abs/2512.07235)
*Sen Li,Chang Feng,Filipe B. Abdalla*

**TL;DR**: 该论文针对宇宙微波背景（CMB）极化观测中原始引力波（PGWs）B模信号探测面临的银河系前景污染和引力透镜效应干扰，提出结合约束矩内部线性组合（cMILC）方法与基于功率谱的分析方法，有效处理空间变化光谱能量分布（SED）和非高斯前景，并通过模拟验证了未来高灵敏度多频段观测中不同B模信号的探测能力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过宇宙微波背景（CMB）偏振观测中的暴涨$B$-模信号探索原初引力波（PGWs）的潜在特征及其面临的银河系前景污染和引力透镜效应干扰。

Data: 数据来源为未来高灵敏度多频段宇宙微波背景（CMB）偏振观测的端到端模拟（end-to-end simulations）和模拟数据（mock data）。

Method: 采用约束矩内线性组合（cMILC）方法和基于功率谱的分析方法分离并提取原初引力波（PGW）B模信号。

Result: 采用约束矩内部线性组合（cMILC）方法和基于功率谱的分析方法，有效消除了空间变化谱能量分布（SED）的偏振前景和非高斯宇宙学B模信号，提升了原初引力波（PGW）B模信号的探测能力。
...
### [49] [Counting voids and filaments: Betti Curves as a Powerful Probe for Cosmology](https://arxiv.org/abs/2512.07236)
*Jiayi Li,Cheng Zhao*

**TL;DR**: 利用贝蒂曲线（Betti curves）量化宇宙大尺度结构（LSS）的多尺度拓扑特征（连通分量、环状结构和空洞），结合功率谱分析显著提升了对宇宙学参数（$n_{\mathrm{s}}$、$σ_8$、$Ω_{\mathrm{m}}$和$w$）的约束精度，验证了其作为红移空间畸变（RSD）敏感探针的互补价值。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用拓扑分析（topological analysis）揭示宇宙大尺度结构（LSS）的非高斯特征，以补充传统两点聚类统计的不足。

Data: 数据来源于\textsc{Quijote}模拟套件中的晕族（halo）目录。

Method: 采用持续同调（persistent homology）导出的贝蒂曲线（Betti curves）量化大尺度结构（LSS）的多尺度拓扑特征（连通分量、环状结构及空洞），并结合基于机器学习的模拟器建模其与宇宙学参数的依赖关系。

Result: 利用贝蒂曲线(Betti curves)结合功率谱分析可显著提升对宇宙学参数（如$n_{\mathrm{s}}$、$σ_8$和$w$）的约束精度，凸显其作为未来星系巡天补充探针的潜力。
...
### [50] [Dynamical Dark Energy and the Unresolved Hubble Tension: Multi-model Constraints from DESI 2025 and Other Probes](https://arxiv.org/abs/2512.07281)
*Zhuoming Zhang,Tengpeng Xu,Yun Chen*

**TL;DR**: 通过贝叶斯比较分析五种宇宙学模型（ΛCDM、wCDM、w0waCDM、φCDM和ξ模型），结合DESI DR2（重子声学振荡）、Pantheon+（Ia型超新星）和CMB（宇宙微波背景辐射）数据，发现哈勃常数（H0）测量值持续存在张力，暗能量呈现动态演化特征（早期宇宙支持幻影相而晚期支持精质相），并首次揭示暗能量与物质在晚期存在相互作用，直接挑战了宇宙学常数范式。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过贝叶斯比较分析探讨暗能量(dark energy)演化和哈勃常数(Hubble tension)问题。

Data: 数据来源包括暗能量光谱仪（DESI）DR2的重子声波振荡（BAO）数据、Pantheon+的Ia型超新星（SNIa）数据，以及来自普朗克卫星（Planck）和阿塔卡马宇宙学望远镜（ACT）的宇宙微波背景辐射（CMB）数据（含透镜效应）。

Method: 采用贝叶斯比较分析（Bayesian comparative analysis）结合暗能量状态方程参数（equation-of-state parameter）约束和暗能量-物质相互作用（dark energy-matter interaction）模型，评估五类宇宙学模型对哈勃张力（Hubble tension）的解释能力。

Result: 哈勃常数（H_0）的测量结果在所有模型中均与早期宇宙观测值一致，表明哈勃张力（Hubble tension）持续存在。
...
### [51] [Squeezed Limit non-Gaussianity Estimation with Cosmic Shear](https://arxiv.org/abs/2512.07295)
*Shi-Hui Zang,Moritz Münchmeyer*

**TL;DR**: 该研究提出了一种利用局部透镜功率谱（lensing power spectrum）的大尺度调制来约束本地原初非高斯性（primordial non-Gaussianity）的新方法，通过将π场（π-field）方法扩展到球坐标并应用于星系透镜效应，实现了对f<sub>NL</sub>参数的高效准确估计，在LSST类弱透镜巡天中的预测精度达到σ<sub>f<sub>NL</sub></sub>≈12。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用局部透镜功率谱的大尺度调制来约束局部原初非高斯性（primordial non-Gaussianity），旨在通过高效计算方法改进对宇宙早期物理过程的探测。

Data: 数据来源于一套N-body模拟（N-body simulations）和类似LSST的弱引力透镜巡天（LSST-like weak lensing survey）预测。

Method: 采用π场方法（π-field method）将大尺度透镜功率谱调制应用于球坐标下的局部原初非高斯性（local primordial non-Gaussianity）约束，并通过双极功率谱（multipole power spectra）及其协方差实现高效计算。

Result: 提出了一种利用局部透镜功率谱（lensing power spectrum）的大尺度调制来约束局部原初非高斯性（primordial non-Gaussianity）的新方法，该方法计算高效且仅需大尺度上的分档多极功率谱$C_\ell(z_1,z_2)$及其协方差。
...
### [52] [Phase-space perturbation theory for cosmic large-scale structure](https://arxiv.org/abs/2512.07396)
*Hannes Heisler,Marvin Sipp,Matthias Bartelmann*

**TL;DR**: 该论文提出了一种不依赖动量累积量截断的微扰方法求解宇宙结构形成的弗拉索夫-泊松(Vlasov-Poisson)系统，通过求解沃尔泰拉(Volterra)型积分方程获得线性解并递归计算高阶项，揭示了微小初始速度色散会动态生成所有高阶动量累积量。


<details>
  <summary>Details</summary>

Background: 研究主题为宇宙结构形成中基于弗拉索夫-泊松系统(Vlasov-Poisson system)的非截断动量累积层次微扰方法。

Data: 数据来源于对弗拉索夫-泊松系统（Vlasov-Poisson system）的数值模拟和解析近似计算，包括求解沃尔泰拉型积分方程（Volterra-type integral equation）生成的线性解及递归获得的高阶解，同时考虑了具有均匀各向同性初始速度弥散的微冷初始条件。

Method: 采用无动量累积层次截断的微扰方法处理Vlasov-Poisson（弗拉索夫-泊松）系统，通过求解Volterra型积分方程获得非平凡线性解，并递归推导高阶解。

Result: 采用无动量累积层次截断的微扰方法求解弗拉索夫-泊松(Vlasov-Poisson)系统，通过沃尔泰拉(Volterra)积分方程计算非平凡线性解并递归获得高阶解，证明偏离冷初始条件时所有高阶动量累积量会在任意微扰阶动态生成。
...
### [53] [ZTF-SEDm Type Ia supernova sample for Twins Embedding spectrophotometric standardisation](https://arxiv.org/abs/2512.07696)
...
**TL;DR**: 基于ZTF DR2的3069条光谱构建了大型均匀光度型Ia超新星（SN Ia）样本，改进了Twins Embedding（TE）分光光度标准化方法，使本征散射降至0.073星等，并通过Read Between The Lines（RBTL）颜色标准化实现比SALT方法少一个参数且宿主星系步长趋零的无偏标准化。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用ZTF DR2数据构建大型均匀光度型Ia超新星（SN Ia）样本，并改进其光谱能量分布（SED）的流量定标精度，以验证Twins Embedding（TE）分光光度标准化方法的有效性。

Data: 数据来源为兹威基瞬变设施第二次数据发布（ZTF DR2）的3069条光谱，并利用ZTF测光数据提高了SEDm超新星光谱样本的流量定标精度。

Method: 采用ZTF DR2的3069条光谱数据构建大型均匀Ia型超新星(SN Ia)样本，并应用Twins Embedding(TE)分光光度标准化方法的三步参数化流程（相位校正、Read Between The Lines(RBTL)颜色标准化、三个非线性参数最终标准化）。

Result: 基于3069颗Ia型超新星(SN Ia)光谱构建了同质化样本，并应用Twins Embedding(TE)光度标准化方法使本征散射降至0.073星等。
...
### [54] [DESI Strong Lens Foundry V: A Sample of HST-Observed Strong Lenses Modeled with GIGA-Lens](https://arxiv.org/abs/2512.07823)
...
**TL;DR**: 利用GIGA-Lens对6个星系尺度强引力透镜（galaxy-scale strong lenses）进行全正向建模（full forward modeling），首次实现所有透镜和源参数在单次推断中同时采样，并通过$\widehat{R}$和有效样本量（ESS）验证收敛性，为哈勃（HST）、欧几里得（Euclid）、JWST和罗曼（Roman）的大样本高分辨率强透镜研究奠定基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用哈勃太空望远镜（HST）成像和GIGA-Lens建模方法分析星系尺度强引力透镜（strong lens）系统，旨在通过收敛验证的建模推动对星系质量密度分布、低质量暗物质（子）晕探测以及精确宇宙学约束等关键科学目标的研究。

Data: 数据来源包括哈勃太空望远镜（HST）成像数据、DESI Legacy巡天成像数据、DESI光谱数据、Keck/NIRES光谱数据和VLT/MUSE光谱数据。

Method: 采用GIGA-Lens进行全正向建模（full forward modeling），同时采样所有透镜和源参数，并使用$\widehat{R}$和有效样本量（ESS）进行显式收敛验证。

Result: 发现了六个哈勃太空望远镜（HST）成像的星系尺度强引力透镜（galaxy-scale strong lenses），并首次采用全正向建模（full forward modeling）方法通过GIGA-Lens实现了统计稳健的参数推断。
...
### [55] [Solver-in-the-Loop Applications in Astrophysical (Magneto)hydrodynamics](https://arxiv.org/abs/2512.05999)
*Leonard Storcks,Tobias Buck*

**TL;DR**: 该论文通过在可微分天体物理（磁）流体力学模拟器中训练机器学习模型，实现了低分辨率模拟中高分辨率风驱气泡（wind-blown bubbles）辐射冷却动力学的恢复，并利用卷积神经网络（convolutional neural network）修正了特定爆炸波问题的二维磁流体力学（magnetohydrodynamics）模拟。


<details>
  <summary>Details</summary>

Background: 探索可微分天体物理（磁）流体力学模拟器中机器学习模型的训练应用，以提升传统模拟方法的效率和精度。

Data: 研究使用了可微分天体物理（磁）流体动力学模拟器（differentiable astrophysical (magneto)hydrodynamics simulator）内部训练的机器学习模型，包括针对辐射冷却风泡问题的低分辨率模拟数据和特定爆炸波问题的二维磁流体动力学（magnetohydrodynamics）模拟数据。

Method: 采用可微分天体物理(磁)流体力学模拟器训练机器学习模型，包括学习冷却函数和卷积神经网络校正二维磁流体动力学(MHD)模拟。

Result: 在可微分天体物理（磁）流体力学模拟器中训练机器学习模型能有效提升低分辨率风驱气泡模拟的收敛速度，并通过卷积神经网络修正特定爆炸波问题的二维磁流体力学模拟结果。
...
### [56] [yieldplotlib: A unified library for exoplanet yield code visualizations](https://arxiv.org/abs/2512.06149)
*Corey Spohn,Sarah Steiger,Alex R. Howe*

**TL;DR**: 这篇论文的核心贡献是开发了yieldplotlib这一统一的可视化工具库，用于完整、直观地展示Altruistic Yield Optimizer (AYO)和EXOSIMS等系外宜居带行星（habitable zone planets）探测预测工具（yield codes）的输入和输出数据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于下一代旗舰级天文观测设备宜居带行星搜寻任务（HWO）的系外类地行星探测效率（yield）预测与优化。

Data: 研究中使用的数据来源包括Altruistic Yield Optimizer (AYO)和EXOSIMS等工具生成的系外行星宜居带（habitable zone）探测模拟数据。

Method: 采用统一的可视化库yieldplotlib对宜居带行星探测工具Altruistic Yield Optimizer (AYO)和EXOSIMS的输入输出数据进行整合分析。

Result: yieldplotlib提供了一个统一的可视化库，能够完整、直观且易用地展示Altruistic Yield Optimizer（AYO）和EXOSIMS等代码的输入输出结果。
...
### [57] [FIP-TOI: Fast Imaging Pipeline for Pulsar Localisation with a Transient-Oriented Radio Astronomical Imager](https://arxiv.org/abs/2512.06254)
*X. Li,K. Adamek,M. Giles,W. Armour*

**TL;DR**: 开发了基于奇异值分解（SVD）和CUDA并行化的瞬变天体专用成像器（TOI），与FITrig探测器结合形成FIP-TOI系统，实现了对脉冲星候选体的实时高精度定位，相比标准成像器WSClean将定位速度提升约10倍。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过开发瞬态导向成像器（TOI）提升射电天文成像效率，以快速定位脉冲星等天体瞬变源。

Data: 研究中使用的数据来源包括具有多个脉冲星的视场、一个间歇性脉冲星以及一个表现出强度变化的脉冲星等多样化数据集。

Method: 基于奇异值分解（SVD）和CUDA并行化的瞬态导向成像器（TOI）结合先进瞬态探测器FITrig，实现实时高精度定位。

Result: 开发了基于奇异值分解（SVD）并在NVIDIA GPU上并行化的瞬态导向成像仪（TOI），与FITrig探测器集成形成FIP-TOI系统，实现了对脉冲星候选体的实时高精度定位。
...
### [58] [Back-End System of BURSTT](https://arxiv.org/abs/2512.07300)
*Kai-Yang Lin,Chih-Yi Wen,Homin Jiang,Jen-Hung Wang,Sujin Eie,Shih-Hao Wang,Yao-Huan Tseng,Hsien-Chun Tseng,Ue-Li Pen*

**TL;DR**: 台湾BURSTT望远镜通过高效多级处理架构实现了对快速射电暴（FRB）的大视场实时探测和亚角秒级精确定位，每年可探测约50个FRB。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用新一代宽视场射电望远镜BURSTT（Bustling Universe Radio Survey Telescope）探测和精确定位快速射电暴（FRBs）这一高能毫秒级未知河外起源信号。

Data: 数据来源于台湾忙碌宇宙射电巡天望远镜（BURSTT）的实时观测，包括对明亮射电源的波束形成测试和已知脉冲星的实时探测。

Method: 采用高效多级处理架构：在赛灵思ZCU216射频片上系统（RFSoC）平台执行初始波束成形，随后利用英特尔至强服务器的AVX-512和AMX指令集完成二级波束成形与信道化，并通过高度优化的bonsai消色散算法实现256波束实时脉冲搜索与触发。

Result: BURSTT（Bustling Universe Radio Survey Telescope）是一种专为探测快速射电暴（FRBs）设计的新一代广角射电望远镜，具备每年约50次FRB探测和亚角秒级定位能力。
...
### [59] [Limits on Unintended Radio Emission from Geostationary and Geosynchronous Satellites in the SKA-Low Frequency Range](https://arxiv.org/abs/2512.07341)
*S. J. Tingay,N. Hurley-Walker,K. Ross,T. J. Galvin,J. Morgan,B. Venville*

**TL;DR**: 利用MWA望远镜的GLEAM-X巡天数据，在72-231 MHz频段对162颗地球静止轨道（GEO）卫星进行叠加成像分析，首次将非故意射频辐射（unintended emissions）的探测灵敏度提升至1 mW EIRP（等效全向辐射功率）以下，未发现显著证据表明GEO卫星存在类似近地轨道（LEO）卫星的射频泄漏现象，为未来平方公里阵（SKA）低频干扰监测建立了基准。


<details>
  <summary>Details</summary>

Background: 探测地球同步轨道（GEO）卫星在低频段（72-231 MHz）是否存在类似近地轨道（LEO）卫星的无意射频辐射（unintended radio emission），以评估其对低频平方公里阵（Square Kilometre Array）的潜在干扰风险。

Data: 数据来源于2020年使用默奇森宽场阵列(MWA)进行的GLEAM-X巡天观测，具体使用了一晚的72-231 MHz频段数据，针对162颗地球静止轨道(GEO)卫星的预测坐标形成叠加图像。

Method: 采用MWA（Murchison Widefield Array）的GLEAM-X巡天数据，在72-231 MHz频段内对162颗地球静止轨道（GEO）卫星的预测坐标进行叠加成像，以搜索非预期射电辐射。

Result: 在72-231 MHz频段对162颗地球静止轨道（GEO）卫星的观测中，未发现显著的非预期无线电发射（unintended radio emission），最佳探测限值低于10微瓦等效全向辐射功率（EIRP）。
...
### [60] [Constraining meV Axion Dark Matter with ALMA Observations of the Galactic Center Magnetar SGR 1745-2900](https://arxiv.org/abs/2512.06441)
*Javier De Miguel,Evanthia Hatziminaoglou,Frédéric Poidevin,Nanda Rea,Daniel L. Walker,Davide De Grandis,Jaime Prieto-Polo*

**TL;DR**: 利用ALMA对磁星（magnetar）SGR 1745-2900的毫米波观测未发现轴子（axion）信号，首次在0.55-0.62 meV范围内将轴子-光子耦合常数（axion-photon coupling）约束至$g_γ\gtrsim 2\times10^{-13}$ GeV$^{-1}$，触及QCD轴子（QCD axion）在meV尺度参数空间。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用毫米波观测探测磁星（magnetar）SGR 1745-2900可能产生的轴子（axion），以探索轴子-光子耦合（axion-photon coupling）在QCD轴子参数空间（QCD axion parameter space）近meV能标（meV scale）的物理特性。

Data: 数据来源于4.8小时的ALMA（阿塔卡马大型毫米波/亚毫米波阵列）观测，覆盖133.99-135.78、135.91-137.70、145.99-147.78和147.99-149.78 GHz四个频段。

Method: 采用毫米波（mm-wave）观测技术结合ALMA（Atacama Large Millimeter/submillimeter Array）数据，通过轴子-光子（axion-photon）共振转换模型与最新恒星模型（star model）及银河系中心暗物质密度（dark-matter density）约束轴子耦合参数（axion-photon coupling）。

Result: 在133.99-135.78、135.91-137.70、145.99-147.78和147.99-149.78 GHz频段内未检测到统计显著信号，首次在meV尺度附近探索了QCD轴子（axion）参数空间，将轴子-光子耦合（axion-photon coupling）限制在$g_γ\gtrsim 2\times10^{-13}$ GeV$^{-1}$（0.55-0.62 meV范围内）。
...
### [61] [A few ideas to promote inclusion](https://arxiv.org/abs/2512.06011)
...
### [62] [SN 2024afav: A Superluminous Supernova with Multiple Light Curve Bumps and Spectroscopic Signatures of Circumstellar Interaction](https://arxiv.org/abs/2512.06067)
...
**TL;DR**: 研究发现SN 2024afav（氢贫乏超亮超新星SLSN-I）的光变曲线多峰和光谱异常特征（窄蓝移Hα吸收、持久He I线、早期[O III]发射等）源于喷出物与富氢壳层（circumstellar medium）的相互作用，并揭示了前身星外层和星周介质中存在氦，表明其属于SLSN-I中罕见的星周物质相互作用亚群。


<details>
  <summary>Details</summary>

Background: 研究聚焦于贫氢型极亮超新星（SLSN-I）SN 2024afav的光学及近红外光谱特征，探讨其多峰光变曲线及特殊光谱现象与星周介质（CSM）相互作用的关联性，揭示此类稀有SLSN-I亚群中能量输入机制的重要调制因素。

Data: 数据来源为光学和近红外光谱观测，覆盖了SN 2024afav从-14天到+160天的不同阶段。

Method: 采用光学和近红外光谱分析技术，结合多波段光谱观测数据（-14至+160天）对SN 2024afav的氢贫乏超亮超新星（SLSN-I）进行时序光谱特征解析，重点检测Hα窄吸收、He I双吸收结构及早期星云相[O III]和[O II]+[Ca II]发射特征。

Result: SN 2024afav的光谱特征（如窄蓝移Hα吸收、持久He I线、早期[O III]发射等）及其与光曲线隆起时间的关联，为SN抛射物与富氢壳层（circumstellar medium）的相互作用以及前身星外层和星周介质中存在氦提供了有力证据。

Abstract: We present a comprehensive optical and near-infrared spectroscopic study of SN 2024afav - a hydrogen-poor superluminous supernova (SLSN-I) that peaks at $\approx$ -20.7 mag and exhibits an unusual multi-bumped light curve. Our spectroscopic observations, spanning phases of -14 to +160 d, reveal several unusual features: (i) a narrow (1,800 km s$^{-1}$) and blueshifted (11,000 km s$^{-1}$) absorption from H$α$ starting at +20 d; (ii) persistent optical and NIR He I lines at all available phases, showing double absorption structure in NIR spectra at +23 d, with a high velocity component at a similar velocity to H$α$; (iii) early appearance of nebular [O III] emission starting at $\approx$ +50 d; and (iv) strong [O II] + [Ca II] 7300 Å emission complex starting at $\approx$ +110 d. These unusual features, and their onset at the time of the light curve bumps, provide compelling evidence of circumstellar interaction between the SN ejecta and a nearby hydrogen-rich shell, as well as the presence of helium in both the outer layers of the progenitor star and in the circumstellar medium. A comparison of SN 2024afav to other SLSNe-I showing bumpy light curves and similar spectral properties (PTF10hgi, SN 2017egm, SN 2019hge), points to a rare sub-group of SLSNe-I in which CSM interaction provides an important modulation to the energy input.

</details>

### [63] [Precise determination of circumstellar disk lifetimes: Disk evolution in a single star-forming region](https://arxiv.org/abs/2512.06873)
*Fabian A. Polnitzky,Sebastian Ratzenböck,Josefa E. Großschedl,João Alves*

**TL;DR**: 通过分析天蝎-半人马星协（Scorpius-Centaurus OB association）中33个年龄为3至2100万年的星团，发现原行星盘（circumstellar disks）寿命为5.8±0.3百万年，比以往估计长约两倍，且指数衰减模型最能描述盘消散过程。


<details>
  <summary>Details</summary>

Background: 研究围绕原行星盘（circumstellar disk）寿命这一关键问题展开，旨在通过精确测量其演化时间尺度来深化对行星形成过程的理解。

Data: 数据来源为盖亚卫星（Gaia）、2微米全天巡天（2MASS）和广域红外巡天探测器（WISE）。

Method: 通过分析盖亚(Gaia)、2MASS和WISE数据，利用颜色-颜色图和光谱能量分布(SED)一致筛选具有红外超(infrared-excess)的盘源。

Result: 盘寿命为5.8±0.3百万年，比多数先前估计长约两倍，表明行星形成时间可能更充裕。
...
### [64] [Revisiting the Evolutionary Status of Massive Stars at the central parsec of the Milky Way](https://arxiv.org/abs/2512.07432)
*A. C. Gormaz-Matamala,J. Cuadra,B. Kubátová,J. Kubát,S. Ekström*

**TL;DR**: 通过更新大质量恒星早期阶段的质量损失率（mass-loss rate）模型，研究发现银河系中心（GC）演化中的大质量恒星在红超巨星（RSG）阶段经历显著质量损失，导致产生的沃尔夫-拉叶星（WR stars）内部结构更不均匀且无氢游离WN星，与观测化学丰度更吻合。


<details>
  <summary>Details</summary>

Background: 大质量恒星及其星风对周围环境有重要影响，例如决定银河系中心(GC)超大质量黑洞Sgr A*的吸积率。

Data: 使用日内瓦演化代码（Geneva-evolution-code）模拟初始质量20至60倍太阳质量、金属丰度Z=0.020的恒星演化，采用新的O型星和B型超巨星线驱星风（line-driven winds）质量损失率公式及红超巨星（RSG）尘埃驱星风（dust-driven winds）公式，并设定初始旋转速率Ω/Ω_crit=0.4，采用勒杜（Ledoux）对流判据处理内层对流。

Method: 采用基于更新质量损失率（mass-loss rate）配方的日内瓦演化代码（Geneva-evolution-code），结合初始旋转速度（Ω/Ω_crit=0.4）和勒杜（Ledoux）对流判据，对20-60 M⊙恒星演化进行建模，包括O型星与B型超巨星（B-supergiants）的线驱动星风（line-driven winds）和红超巨星（RSG）的尘埃驱动星风（dust-driven winds）新配方。

Result: 采用新的质量损失率（mass-loss rate）模型预测恒星在初始阶段外层损失较少，而在红超巨星（RSG）阶段质量损失显著增加，导致形成的沃尔夫-拉叶星（WR stars）从核心到表面的内部结构径向不均匀性降低，且不产生无氢WN星。
...
### [65] [Quenching factors for Na recoils as a function of Tl dopant concentrations in NaI(Tl) crystals](https://arxiv.org/abs/2512.05980)
...
**TL;DR**: 通过测量不同铊（Tl）掺杂浓度的碘化钠（NaI(Tl))晶体在5-26keV$_{nr}$核反冲能量范围内的淬灭因子（quenching factor），发现铊浓度对淬灭因子的影响在校准方法（$^{241}$Am源与$^{133}$Ba低能校准线）间存在差异。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过精确测量铊掺杂碘化钠（NaI(Tl)）晶体中钠核反冲的淬灭因子（quenching factor），以解决直接暗物质（DM）探测中DAMA/LIBRA实验与其他实验结果矛盾的关键标定问题。

Data: 数据来源于上海硅酸盐研究所生产的五种超纯掺铊碘化钠（NaI(Tl)）晶体，并在美国北卡罗来纳州三角大学核实验室使用准单能中子束进行辐照。

Method: 采用准单能中子束（quasi-monoenergetic neutron beam）辐照和比例响应校准方案（proportional response calibration schema）测量了钠碘化铊（NaI(Tl)）晶体中钠核反冲的淬灭因子（quenching factor）。

Result: 钠(Na)反冲淬灭因子(quenching factor)在5-26keV$_{nr}$能区测得与铊(Tl)掺杂浓度相关，但该相关性依赖于校准方案（$^{241}$Am源显示而$^{133}$Ba低能线未显示）。
...
### [67] [Evaporation of Primordial Black Holes in a Thermal Universe: A Thermofield Dynamics Approach](https://arxiv.org/abs/2512.07284)
*Ayan Chatterjee,Jitumani Kalita,Debaprasad Maity*

**TL;DR**: 研究发现有限温度环境中克尔黑洞（Kerr BHs）的霍金辐射谱因粒子与热背景相互作用发生修正，且该热修正会加速原初黑洞（PBHs）蒸发并缩短其寿命。


<details>
  <summary>Details</summary>

Background: 研究探讨了有限温度环境对黑洞（BHs）霍金辐射的影响，重点关注浸没在宇宙学热浴中的克尔黑洞（Kerr BHs）。

Data: 数据来源于对克尔黑洞（Kerr BHs）在宇宙学热浴（cosmological thermal bath）环境中霍金辐射（Hawking radiation）的理论模拟，采用热场动力学（Thermofield Dynamics, TFD）方法计算修正后的辐射谱，并应用于早期宇宙再加热（reheating）背景下的原初黑洞（PBHs）蒸发过程。

Method: 采用热场动力学（Thermofield Dynamics, TFD）这一热量子场论的实时形式体系，推导了渐进平直时空（如史瓦西和克尔几何）中霍金辐射谱的修正占据数。

Result: 热环境通过粒子热化效应修正了霍金辐射谱（Hawking radiation spectrum），加速了原初黑洞（PBHs）的蒸发并缩短其寿命。
...
### [68] [Long-wavelength UV-LEDs and charge management in the detection of gravitational waves in space](https://arxiv.org/abs/2512.07546)
...
**TL;DR**: 该研究通过采用模拟LISA Pathfinder立方体测试质量块（TM）表面特性和功函数的复杂惯性传感器模型，发现275纳米紫外发光二极管（UV-LED）在双极电荷管理系统中表现最优，能维持TM电势接近零并兼容快速和连续放电策略。


<details>
  <summary>Details</summary>

Background: 研究聚焦于引力波探测任务中基于紫外发光二极管（UV-LED）的电荷管理系统优化，旨在通过连续放电策略平衡充电与放电速率并减少电荷测量频率。

Data: 数据来源于基于模拟立方体测试质量（TM）表面特性和功函数的复杂惯性传感器模型（类似LISA Pathfinder），并采用峰值波长为269纳米、275纳米、280纳米和295纳米的紫外发光二极管（UV-LED）进行双极电荷管理系统的实验研究。

Method: 采用模拟立方体测试质量（TM）表面特性和功函数的复杂惯性传感器模型（类似LISA Pathfinder），研究基于269 nm、275 nm、280 nm和295 nm峰值波长UV-LED的双极电荷管理系统性能。

Result: 采用275纳米紫外发光二极管（UV-LED）的电荷管理系统在维持测试质量块（TM）电势接近零的同时，兼顾了快速放电和连续放电策略的最优性能。
...
### [69] [Apparent Phantom Crossing in Gauss-Bonnet Gravity](https://arxiv.org/abs/2512.06279)
*Shin'ichi Nojiri,Sergei D. Odintsov,V. K Oikonomou*

**TL;DR**: 研究通过构建标量-爱因斯坦-高斯-博内引力（scalar-Einstein-Gauss-Bonnet gravity）和无鬼（ghost-free）f(𝒢)引力模型，证实了暗能量状态方程参数w在红移z∼0.5时从w<-1到w>-1的逆幻影穿越（inverse phantom crossing）可能性，并提出暗物质能量密度下降速度减缓的表观幻影穿越（apparent phantom crossing）新场景，其中标量场粒子可能因与高斯-博内不变量（Gauss-Bonnet invariant）耦合而质量增加，暗示逆幻影穿越或与宇宙从减速膨胀向加速膨胀的转变相关。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗能量状态参数（dark energy equation of state parameter）在红移（redshift）z∼0.5时可能发生从w<-1到w>-1的转变（即逆幻影穿越，inverse phantom crossing）及其在标量-爱因斯坦-高斯-博内引力（scalar--Einstein--Gauss-Bonnet gravity）和无鬼f(𝒢)引力（ghost-free f(𝒢) gravity）框架下的理论实现。

Data: 数据来源于暗能量光谱仪（DESI）的观测结果以及基于标量-爱因斯坦-高斯-博内引力（scalar–Einstein–Gauss-Bonnet gravity）和无鬼$f(\mathcal{G})$引力（ghost-free $f(\mathcal{G})$ gravity）框架构建的模型仿真数据。

Method: 采用标量-爱因斯坦-高斯-博内引力（scalar–Einstein–Gauss-Bonnet gravity）和无鬼$f(\mathcal{G})$引力（ghost-free $f(\mathcal{G})$ gravity）框架构建可实现幻影穿越（phantom crossing）的模型，并研究表观幻影穿越（apparent phantom crossing）场景中暗物质能量密度下降速率减缓的现象。

Result: 在标量-爱因斯坦-高斯-博内引力（scalar-Einstein-Gauss-Bonnet gravity）和无鬼$f(\mathcal{G})$引力框架下构建了可实现暗能量状态方程参数$w$从$w<-1$到$w>-1$转变（即逆幻影穿越）的物理模型。
...
### [70] [Extreme mass ratio inspirals in the cold vector dark matter environment](https://arxiv.org/abs/2512.06807)
*Rajesh Karmakar,Kaustubh Mukund Vispute,Debaprasad Maity*

**TL;DR**: 研究了由大质量矢量场（Proca场）描述的暗物质在黑洞附近的密度分布及其引力波探测可行性，发现矢量暗物质在粒子态和波动态均会导致引力波应变退相，并对比了标量暗物质情形。


<details>
  <summary>Details</summary>

Background: 研究探讨了通过引力波观测探测由大质量矢量场（Proca场）描述的暗物质（vector dark matter）在不同质量范围内的密度分布及其可探测性。

Data: 研究中使用的数据来源于对暗物质密度分布的数值模拟（simulation），包括粒子态和波态下暗物质质量范围$μ\sim 10^{-10}-10^{-15}{\mathrm eV}$的密度剖面，以及外围区域采用带中心尖峰的Navarro-Frenk-White (NFW)轮廓的解析模型。

Method: 采用数值计算和解析方法分析原初黑洞（Proca field）周围暗物质密度分布，结合费希尔信息矩阵（Fisher forecast）评估LISA观测中引力波应变（gravitational wave strain）的相位偏移特征。

Result: 在矢量暗物质（Proca field）环境下，通过引力波应变中的相位偏移分析和费希尔预报，研究发现矢量暗物质在粒子态和波动态均表现出可探测的独特特征，并与标量暗物质情形进行了对比。
...
### [71] [Revealing Hidden Repeaters in the CHIME/FRB Catalog: Semi-Supervised Insights into the Fast Radio Burst Population](https://arxiv.org/abs/2512.06316)
*N. Mankatwit,P. Thongkonsing,S. Loekkesee,P. Chainakun,W. Luangtip,S. Sanpa-arsa*

**TL;DR**: 开发半监督学习框架识别快速射电暴（FRB）重复暴与非重复暴的区分特征（色散量、峰值频率、流量密度），发现重复暴色散量更低、峰值频率和流量密度更高，并新鉴定出36个传统方法可能遗漏的重复暴候选体。


<details>
  <summary>Details</summary>

Background: 快速射电暴（FRB）是毫秒级持续时间的河外瞬变现象，观测上被分类为重复暴和非重复暴，但该分类可能存在偏差。

Data: 数据来源于Blinkverse数据库，该数据库整合了CHIME/FRB Catalogs的观测参数，包括已知重复暴、高置信度非重复暴以及具有重复暴特征但被标记为非重复暴的未标记源。

Method: 采用半监督学习框架结合均匀流形近似与投影（UMAP）和最近邻方案筛选候选体，并运用随机森林、支持向量机、逻辑回归、AdaBoost和梯度提升五种基分类器进行精细调优和投票决策。

Result: 快速射电暴（FRB）重复暴与非重复暴在色散量（dispersion measure）、峰值频率和能量通量上存在显著差异，重复暴通常表现出更低的色散量、更高的峰值频率和更高的能量通量。
...
### [72] [Possible $ν$ Source Class: 3-sigma Detection of High-Energy Neutrinos from Supermassive Black Hole Binary Candidates](https://arxiv.org/abs/2512.02099)
*Pugazhendhi A D,Subhadip Bouri,Bei Zhou,Rachana,Ranjan Laha*

**TL;DR**: 首次发现超大质量黑洞双星（SMBHBs）与高能中微子存在3.0σ显著性的正相关性，为该类天体作为高能中微子源提供了首个证据。


<details>
  <summary>Details</summary>

Background: 高能(TeV-PeV)天体物理中微子源的研究对天体物理学和粒子物理学具有重要意义。

Data: 数据来源于693个超大质量黑洞双星（SMBHBs）候选体目录和10年的IceCube公开数据。

Method: 采用无分箱最大似然比分析(unbinned maximum-likelihood-ratio analysis)对693个候选超大质量黑洞双星(SMBHBs)样本与10年IceCube公开数据进行关联性检验。

Result: 首次发现超大质量黑洞双星（SMBHBs）是高能中微子辐射源（3.0σ显著性）。
...
### [73] [Paucity of downward UHE neutrino tracks in IceCube versus unexpected huge KM3-230213A event: solving the puzzles?](https://arxiv.org/abs/2512.07042)
*D. Fargion*

**TL;DR**: 研究发现ARCA阵列探测器观测到的极高能中微子事件KM3-230213A（约200PeV至EeV能量）与IceCube和AUGER阵列的上限不符，可能源于深海探测器浮动导致的几何偏差，而非真实水平方向中微子，若该罕见事件被证实将开辟陶(tau)中微子天文学新领域。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于超高能（EeV量级）中微子（neutrino）的观测异常及其与现有理论模型的矛盾，特别是KM3-230213A事件能量远超冰立方（IceCube）和奥格（AUGER）阵列的统计上限，挑战了大气μ子束（atmospheric muon bundles）污染或粲单事例（charmed single event）的解释框架，并探讨了罕见τ中微子（Tau neutrino）天文观测的可能性及其对宇宙高能中微子散射（neutrino scattering）与轻质量遗迹中微子（relic light mass neutrino）共振模型的潜在验证意义。

Data: 数据来源于ARCA阵列探测器（ARCA array detector）观测到的KM3-230213A事件，并对比了IceCube和AUGER阵列（AUGER array）的历史上限数据。

Method: 采用蒙特卡洛模拟（Monte Carlo simulation）分析探测器几何形变对中微子（neutrino）到达角重建的影响，并结合大气μ子束（atmospheric muon bundle）背景模型评估事件误认概率。

Result: KM3-230213A事件的高能特性（约200 PeV至EeV）与IceCube和AUGER阵列的统计上限不一致，暗示其可能源于大气μ子束污染而非真实中微子信号。
...
### [74] [A Song of Lines and Winds: Tracing the Signatures of AGN Outflows in X-rays](https://arxiv.org/abs/2512.06077)
...
**TL;DR**: 通过分析122个超快外流（UFO）探测数据，发现吸收线宽度、等效宽度与流速呈正相关，证实最快外流对应最宽最强吸收特征，并首次揭示不同活动星系核（AGN）类型的UFO差异主要由黑洞质量和光度驱动，支持多相介质中团块状外流模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）中超快外流（UFOs）的观测特征及其对AGN反馈和星系演化的影响。

Data: data部分：研究数据来源于对已发表的122个显著性超过2σ的超快外流（UFO）探测结果的综合分析，覆盖57个活动星系核（AGN）样本，涉及红移、光度、黑洞质量及爱丁顿比率的广泛参数范围。

Method: 结合现象学模型和光致电离模型(photoionization modeling)分析吸收特征，并表征超快外流(UFO)性质间的经验相关性。

Result: 超快外流（UFO）的线宽、等效宽度和外流速度呈正相关，表明最宽最强的吸收线对应最快风，尽管速度-弥散趋势相对较弱。
...
### [75] [Revisiting PBH Accretion, Evaporation and Their Cosmological Consequences](https://arxiv.org/abs/2512.07291)
*Jitumani Kalita,Debaprasad Maity*

**TL;DR**: 该论文在完全相对论框架下研究了辐射主导时期克尔(Kerr)原初黑洞(PBHs)的质量吸积和霍金蒸发耦合演化，发现相对论性吸积显著增加PBH质量并抑制自旋，使所有PBH在蒸发前变为史瓦西(Schwarzschild)黑洞，从而将大爆炸核合成(BBN)对初始PBH质量的限制提高4-5倍，并将幸存至今的PBH质量阈值降至2.7×10¹⁴克，同时消除了高频随机引力波背景中自旋诱导特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原初黑洞（PBHs）作为早期宇宙独特探针的演化机制，特别是相对论性吸积与霍金蒸发竞争对其质量和自旋的影响。

Data: 研究采用完全相对论性框架分析辐射主导时期克尔原初黑洞（Kerr PBHs）的吸积过程，推导了相对论性吸积率并计算了自旋依赖效率λ_{\text{Kerr}}(a_*)。

Method: 采用完全相对论性框架分析克尔(Kerr)原初黑洞(PBHs)在辐射主导时期的吸积过程，推导了相对论性吸积率并计算了自旋依赖效率λ_{\text{Kerr}}(a_*)，构建了同时包含相对论性吸积和自旋依赖蒸发的PBH质量与自旋耦合演化方程。

Result: 相对论性吸积显著增加了原初黑洞（PBHs）的质量并抑制其自旋，导致所有原初黑洞在蒸发前均趋近于史瓦西黑洞。
...
### [76] [Fast X-ray Variability from the Coronae of Supermassive Black Holes](https://arxiv.org/abs/2512.06622)
*Xiurui Zhao,Luca Comisso,Stefano Marchesi,Marco Ajello,Elias Kammoun,Yue Shen,Qiaoya Wu*

**TL;DR**: 首次在射电宁静活动星系核(AGN)中系统研究了短时标X射线变异性，发现14个AGN存在快速(数万秒)且显著的X射线通量变化，其冕区可变区域上限尺度(小于10^-4秒差距)远小于微引力透镜推断值，表明冕区由局域瞬态结构组成，支持磁主导冕区模型。


<details>
  <summary>Details</summary>

Background: 首次系统研究射电宁静活动星系核（AGN）在短时标下的X射线快速变异性，聚焦于磁主导冕区的小尺度结构和动力学特征。

Data: 数据来源于钱德拉(Chandra)存档观测数据，约3000个射电宁静活动星系核(AGN)选自SDSS和DESI光谱巡天。

Method: 采用钱德拉(Chandra)X射线存档数据，通过统计显著性检验($p\le6\times10^{-4}$)识别快速X射线光变，并将最小光变时标转换为光行时以限制冕区尺寸上限。

Result: 在射电宁静活动星系核(AGN)中首次系统发现了快速(时间尺度约数万秒)X射线光度变化现象，其变化幅度达两倍以上且统计显著($p\le6\times10^{-4}$)，揭示了日冕存在快速变异的局部瞬态结构而非均匀等离子体。
...
### [77] [SS433 PeV neutron jet feeding the far TeV gamma beam](https://arxiv.org/abs/2512.07012)
*Daniele Fargion,Pier Giorgio De Sanctis Lucentini,Sara Turriziani,Danila Sopin,Maxim Yu. Khlopov*

**TL;DR**: 研究发现SS433双星系统中的黑洞在剧烈潮汐爆发期间喷射出数十PeV（petaelectronvolt）能量的中子束流，其飞行中的β衰变和逆康普顿散射可解释远处观测到的数十TeV（teraelectronvolt）伽马射线尾迹。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是探索SS433双星系统中黑洞吸积伴星物质并产生高能喷流（jet）的机制，以及这些喷流如何通过超高能中子（PeVatron neutrons）的β衰变和逆康普顿散射（Inverse Compton scattering）产生远距离且不连续的TeV级伽马射线束（gamma-ray beams）。

Data: 数据来源于H.E.S.S.、HAWC和LHAASO对SS433系统观测到的伽马射线尾迹以及该系统历史喷发活动的理论模拟。

Method: 采用超相对论性PeV中子喷流模型，结合飞行中β衰变和逆康普顿散射（Inverse Compton scattering）机制解释观测到的TeV级γ射线束。

Result: SS433系统在过去一个世纪可能喷射出数十PeV的中子爆发喷流（neutron burst jets），这些喷流通过飞行中的β衰变和逆康普顿散射（Inverse Compton scattering）产生了观测到的遥远且分离的数十TeV伽马射线束。
...
### [78] [Interplay between Escaping Cosmic Rays and Interstellar Medium: Driving of Galactic Winds and Shaping the Local Proton Spectrum](https://arxiv.org/abs/2512.07239)
*Jiro Shimoda,Katsuaki Asano,Shu-ichiro Inutsuka*

**TL;DR**: 研究发现逃逸宇宙线(CRs)通过加速和加热星际介质(ISM)影响其周围环境，其加热效应可解释超新星遗迹中异常Hα和[OIII]谱线观测，产生的气体外流与银河风需求相当，并提出了对局部宇宙线质子能谱的新解释。


<details>
  <summary>Details</summary>

Background: 研究探讨了逃逸宇宙线（cosmic rays, CRs）对星际介质（interstellar medium, ISM）的影响及其在星系风形成和金属污染晕气体中的作用。

Data: data：研究采用球对称宇宙射线流体动力学模拟（spherically symmetric CR-hydrodynamical simulations），考虑了宇宙射线能谱演化、辐射冷却和热传导效应。

Method: 采用球对称宇宙射线(CR)流体动力学模拟，结合CR能谱演化、辐射冷却和热传导过程，研究逃逸CR对星际介质(ISM)的影响。

Result: 逃逸宇宙线(CRs)通过加速和加热星际介质(ISM)流体影响其周围环境，加热效应可能解释超新星遗迹中Hα和[OIII]λ5007谱线的异常观测。
...
**TL;DR**: 提出基于掩码自编码器(MAE)的预训练策略，利用模拟强引力透镜(strong lensing)图像训练通用编码器，在暗物质模型分类(准确率88.65%)和超分辨率重建(PSNR ~33 dB)任务中均优于从头训练的模型。


<details>
  <summary>Details</summary>

Background: 研究主题为利用强引力透镜效应（strong gravitational lensing）探测星系中暗物质（dark matter）子结构的分布特性及其对透镜图像的影响。

Data: 数据来源于DeepLense ML4SCI基准测试中模拟的强引力透镜图像（simulated strong-lensing images）。

Method: 采用掩码自编码器（MAE）预训练策略，在模拟的强引力透镜图像上通过掩码图像建模目标预训练视觉变换器（Vision Transformer）编码器，并针对不同下游任务进行微调。

Result: 采用掩码自编码器(MAE)预训练策略在模拟强透镜(strong-lensing)图像上学习通用表征，可同时提升暗物质模型分类(冷暗物质、轴子类或无子结构)和超分辨率重建任务性能，其中90%掩码率下分类宏AUC达0.968、超分辨率PSNR约33分贝。
...
### [80] [ExoNAMD: Leveraging the spin-orbit angle to constrain the dynamics of multiplanetary systems](https://arxiv.org/abs/2512.06126)
*A. Bocchieri,J. Zak,D. Turrini*

**TL;DR**: 开发了开源工具ExoNAMD，通过归一化角动量亏损（NAMD）指标量化多行星系统（multiplanetary systems）动力学状态，发现加入自旋轨道角（spin-orbit angle）参数的A-NAMD比仅用相对倾角的R-NAMD更能准确描述系统动力学历史。


<details>
  <summary>Details</summary>

Background: 多行星系统（multiplanetary systems）是研究同一恒星环境下系外行星形成与演化的理想实验室，其轨道构型多样性（如自旋轨道角差异）为理解系统动力学历史提供了关键信息。

Data: 数据来源包括PLATO和Roman空间望远镜的观测数据，以及未来可能通过ELTs、JWST和Ariel获取的大气表征数据。

Method: 采用归一化角动量亏损（NAMD）指标评估多行星系统的动力学状态，并引入自旋轨道角参数（A-NAMD）以改进描述。

Result: 开发了开源工具ExoNAMD，通过归一化角动量亏损（NAMD）指标评估多行星系统的动力学状态，并发现加入自旋轨道角参数（A-NAMD）比仅用相对倾角（R-NAMD）能更准确描述系统动力学特性。
...
### [81] [Deciphering transmission spectra by exploring the solar paradigm](https://arxiv.org/abs/2512.06487)
*Nina-Elisabeth Nèmec,Òscar Porqueras- Léon,Ignasi Ribas,Alexander I. Shapiro*

**TL;DR**: 该研究以太阳为基准模型，揭示了恒星磁活动（黑子和光斑）分布通过连续谱变化（CLV）对色差视半径的显著影响，特别是光斑在近太阳活动水平下主导视半径变化，其活动诱导的色差效应在木星大小凌星时可达40 ppm（超过JWST探测限），而地球大小凌星时仅0.4 ppm。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星磁活动（stellar magnetic activity）对系外行星大气透射光谱（transmission spectroscopy）测量的影响，特别关注太阳作为基准模型下黑子（spots）和光斑（faculae）分布对表观恒星半径（apparent stellar radius）的波长依赖性效应。

Data: 数据来源为SDO/HMI（Solar Dynamics Observatory/Helioseismic and Magnetic Imager）的磁图（magnetograms）和强度图（intensitygrams），结合SATIRE（Spectral And Total Irradiance Reconstruction）模型计算太阳表观半径的色散依赖关系。

Method: 采用SDO/HMI磁图和强度图提取的太阳黑子（spot）和光斑（facular）掩模，结合SATIRE模型计算太阳视半径的色散依赖关系，并通过卷积表面覆盖与光谱的方法识别物理驱动因素。

Result: 活动引起的太阳最小至最大活动期间色差视半径变化对木星大小凌星可达约40 ppm，超过JWST的10 ppm噪声阈值，而对地球大小凌星仅约0.4 ppm。
...
### [82] [NEO and imminent impactor discoveries from Hungary: recent results and lessons learnt](https://arxiv.org/abs/2512.06594)
...
**TL;DR**: 该论文介绍了匈牙利Konkoly天文台Piszkéstető观测站通过升级设备和优化观测策略，成功发现三颗即将撞击地球的小天体（2022 EB5、2023 CX1和2024 BX1），并阐述了提高对米级撞击体（meter-sized impactors）观测灵敏度的具体方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升近地天体(NEO)监测能力，特别是针对米级尺寸的即时撞击体(imminent impactor)的发现效率，依托匈牙利Konkoly天文台Piszkéstető观测站的技术升级与观测策略优化。

Data: 数据来源于Konkoly天文台Piszkéstető Mountain Station的观测数据。

Method: 采用升级后的巡天仪器和优化的观测策略，强调实时数据分析与观测反馈的紧密结合以提升对米级撞击体（meter-sized impactors）的探测灵敏度。

Result: 在匈牙利皮什凯斯泰托山天文台（Piszkéstető Mountain Station）通过升级后的巡天设备和优化的观测策略，显著提升了对米级近地天体（NEO）撞击事件的即时探测能力。
...
### [83] [A Fast, Parallelized, GPU-Accelerated Photochemical Model, XODIAC, with Built-in Equilibrium Chemistry and Multiple Chemical Networks for Exoplanetary Atmospheres](https://arxiv.org/abs/2512.06729)
*Priyankush Ghosh,Sambit Mishra,Shubham Dey,Debayan Das,Paul B. Rimmer,Liton Majumdar*

**TL;DR**: 开发了GPU加速的一维光化学模型XODIAC（含平衡化学求解器、更新热化学数据库和三个反应网络），通过HD 189733 b验证表明该模型能有效比较不同化学网络（C-H-O-N-S、C-H-O-N-S-Metals和新型XODIAC-2025）下硫、碳、磷化学的差异（磷变化最大源于新增390个PHO/PN反应路径）。


<details>
  <summary>Details</summary>

Background: 詹姆斯韦伯太空望远镜（JWST）的发射提供了高质量系外行星大气观测数据，推动了大气化学建模领域的发展以解释这些新数据。

Data: 数据来源包括詹姆斯·韦伯太空望远镜（JWST）提供的高质量大气观测数据，以及基于热木星（hot Jupiter）HD 189733 b的公开可用化学网络（C-H-O-N-S和C-H-O-N-S-Metals）的基准测试数据。

Method: 开发了基于GPU加速的一维光化学模型XODIAC，包含平衡化学求解器、更新热化学数据库及三个化学反应网络（含新开发的XODIAC-2025网络），并通过与VULCAN和ARGO模型的基准测试验证了数值方案的可靠性。

Result: XODIAC模型通过整合594种化学物质和7720个反应的最新C-H-O-N-P-S-Metals网络（XODIAC-2025），在热木星HD 189733 b的基准测试中验证了其与VULCAN和ARGO模型的一致性，并揭示了硫化学（sulfur chemistry）跨网络差异最小、磷化学（phosphorus chemistry）因PHO和PN路径引入而变异最大的规律。
...
### [84] [Understanding the Planetary Formation and Evolution in Star Clusters(UPiC)-II: Catalog of planets/candidates in Open Clusters and Moving Groups](https://arxiv.org/abs/2512.07029)
*Yuan-Zhe Dai,Hui-Gen Liu,Xiaoying Pang,Yueyue Jiang,Jerome P. de Leon,Jing Zhong,Ji-Lin Zhou*

**TL;DR**: 该研究通过分析开放星团（OCs）和移动星群（MGs）中的106颗确认行星和168颗候选行星，发现移动星群的行星比例显著高于开放星团（尤其热木星），并揭示年轻亚木星（<100 Myr）是经历强烈光致蒸发的蓬松海王星质量行星，而木星质量行星能维持大气层，同时在两类星群中均观测到100 Myr时已存在热海王星沙漠现象。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过探测疏散星团（Open Clusters）和移动星群（Moving Groups）中的行星来检验星团环境中行星形成理论，并利用其精确年龄追踪行星系统早期演化特征。

Data: 数据来源于通过交叉匹配最新星团目录与凌日行星及候选体目录构建的迄今最全面目录，包含106颗确认行星和168颗候选体，并利用\texttt{HDBSCAN}和高斯混合模型(GMM)算法重新拟合了这些恒星群的结构参数。

Method: 采用雅可比半径（Jacobi radius）对恒星群进行分类，并使用\texttt{HDBSCAN}和高斯混合模型（GMM）算法识别子结构。

Result: 在年轻疏散星团（Open Clusters）和移动星群（Moving Groups）中，移动星群的行星比例显著高于疏散星团，尤其是热木星（Hot Jupiters），且系外行星半径在早期演化阶段呈现明显二分现象：亚木星（sub-Jupiters）在100 Myr内演化为海王星大小，而超级木星（super-Jupiters）仅经历轻微收缩。
...
### [85] [SPHEREx Pre-Perihelion Mapping of $\mathrm{H_2O}$, $\mathrm{CO_2}$, and $\mathrm{CO}$ in Interstellar Object 3I/ATLAS](https://arxiv.org/abs/2512.07318)
...
**TL;DR**: 通过SPHEREx航天器对星际天体3I/ATLAS的光谱观测首次发现了其延展至4.2×10^5 km的巨大CO₂大气层、微弱水冰彗发及CO₂主导的彗星活动特征，表明星际天体在进入星际介质前普遍经历高度热演化过程。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际天体3I/ATLAS的挥发性成分与物理特性，揭示了其与短周期彗星103P/Hartley 2的相似性，暗示星际天体（ISOs）在抛射至星际介质（ISM）前普遍经历显著热演化过程。

Data: 数据来源于SPHEREx航天器在2025年8月1日至15日期间对星际天体3I/ATLAS进行的观测，获取了波长范围0.7-5微米、分辨率40-130的光谱测量数据，包括光变曲线、光谱和成像。

Method: 采用分辨率（resolution）R=40-130的光谱光度测量技术，在0.7-5μm波段获取3I/ATLAS的光变曲线、光谱及成像数据。

Result: 在星际物体3I/ATLAS中检测到了水汽（H$_2$O）和二氧化碳（CO$_2$）的强烈发射信号，并初步发现了$^{13}$CO$_2$和一氧化碳（CO）气体，同时观测到略微扩展的水汽彗发和巨大的二氧化碳大气层延伸至至少$4.2\times10^{5}\,$公里。
