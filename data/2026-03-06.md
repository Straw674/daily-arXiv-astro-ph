...
**TL;DR**: 提出利用银河系棒（Galactic bar）共振俘获恒星探测暗物质子晕（subhalo）的新方法，发现冷暗物质（CDM）模型预测的子晕群会完全破坏共转共振（co-rotation resonance），而观测到的共振持续存在表明本地子晕密度需低于CDM预期的1/3。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系棒（Galactic bar）与暗物质晕（halo）的耦合机制，通过共振俘获恒星探测暗物质子晕（subhalo）分布及其对暗物质粒子性质的约束。

Data: 数据来源包括银河系棒（Galactic bar）共振捕获的恒星动力学模拟（test particle integration）和冷暗物质（CDM）子晕质量函数的理论模型。

Method: 采用共振恒星动力学模型和脉冲近似（impulse approximation）方法，结合作用量扩散（diffusion in action）理论分析暗物质子晕对银河系棒共振结构的影响。

Result: result：银河系棒状结构（Galactic bar）与暗物质亚晕（subhalo）相互作用导致共振恒星（resonant stars）扩散，冷暗物质（CDM）模型预期的亚晕群可完全消除共转共振（co-rotation resonance），而实际观测到的共振持续存在表明局部亚晕密度需低于CDM预期的1/3。
...
**TL;DR**: 研究发现多质量星团（multi-mass star clusters）中质量依赖的弛豫过程会导致特征相似尺度分离，使单尺度自相似演化（self-similar evolution）结构不稳定，且在速度各向异性（velocity anisotropy）影响下该不稳定性分裂为径向和切向模式，分别通过增强径向动能支撑或加速中心演化来调控质量分层（mass segregation）和多尺度准同源演化（near-homologous evolution）的形成。


<details>
  <summary>Details</summary>

Background: 研究探讨了自相似演化（self-similar evolution）理论在具有多恒星质量且速度分布各向异性（anisotropic velocity distributions）的碰撞星团（collisional star clusters）中的结构稳定性问题。

Data: 采用气体模型近似（gaseous-model approximation）构建理论框架，分析各向同性与各向异性速度分布下多质量恒星团的自相似背景对质量依赖性扰动的响应。

Method: 采用气体模型近似（gaseous-model approximation）理论框架分析多质量星团自相似演化对质量依赖扰动的响应，并研究各向同性与各向异性速度分布的影响。

Result: 质量依赖的弛豫过程导致特征相似标度分离，使单标度解在结构上不稳定。
...
**TL;DR**: 基于宇宙学星系演化模拟和尘埃处理，研究发现星系尘埃温度（dust temperature）随红移升高而增加，主要由恒星形成率面密度（star formation rate surface density）和尘埃气体比（dust-to-gas ratio）驱动，其中高红移下高恒星形成率面密度和低尘埃气体比的星系具有更暖的尘埃温度，并提供了由恒星形成率面密度、尘埃温度和红移估算尘埃气体比的简单关系。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系尘埃温度（dust temperature）随红移（redshift）演化的规律及其与恒星形成率面密度（star formation rate surface density）和尘埃-气体比（dust-to-gas ratio）等物理性质的关系。

Data: 数据来源于经过辐射传输后处理的半解析星系形成模型（semi-analytic model of galaxy formation），该模型包含显式尘埃处理，并通过修正黑体光谱能量分布（SED）拟合方法从模拟星系中导出尘埃温度（dust temperature）。

Method: 采用半解析星系形成模型结合尘埃处理及辐射传输后处理，通过修正黑体光谱能量分布拟合方法计算尘埃温度。

Result: 尘埃温度（dust temperature）随红移增加而升高，主要由恒星形成率面密度（star formation rate surface density）和尘埃-气体比（dust-to-gas ratio）驱动，前者反映星际辐射场强度，后者反映暖分子云光学厚度。
...
### [4] [Low-resolution spectroscopic characterisation of five poorly known Galactic stellar clusters](https://arxiv.org/abs/2603.04507)
*E. Ceccarelli,M. Bellazzini,D. Massari,A. Mucciarelli,M. De Leo,M. Libralato,E. Dodd*

**TL;DR**: 通过低分辨率光谱分析五个银河系星团（Koposov 1、Koposov 2、Muñoz 1、Pfleiderer 2和RLGC2）的系统视向速度（$V_{\rm sys}$）和金属丰度（$\mathrm{[Fe/H]}$），首次测定Pfleiderer 2和RLGC2的动力学参数，并结合轨道分析揭示RLGC2与盖亚-香肠-恩克拉多斯（Gaia-Sausage-Enceladus）并合事件相关，Koposov 1可能源自人马座矮球状星系（Sagittarius dwarf spheroidal），而Muñoz 1与该星系关联性较弱，Koposov 2未显示与任何已知前身系统的明确关联。


<details>
  <summary>Details</summary>

Background: 恒星团簇保存了塑造现今银河系(MW)形成和演化过程的关键信息。

Data: 数据来源于低分辨率MODS@LBT光谱仪对五个恒星系统（Koposov 1、Koposov 2、Muñoz 1、Pfleiderer 2和RLGC2）的观测，通过测量红巨星分支成员的红外Ca II三重线等效宽度获取视向速度（$V_{\rm sys}$）和金属丰度。

Method: 采用低分辨率MODS@LBT光谱技术测量五个星团的视向速度（$V_{\rm sys}$）和基于红外Ca II三重线等效宽度的金属丰度（$\mathrm{[Fe/H]}$），并利用轨道分析追溯其起源。

Result: 基于低分辨率光谱测量首次确定了Pfleiderer 2和RLGC2的系统性视向速度（$V_{\rm sys}$）和金属丰度（$\mathrm{[Fe/H]}$），分别为$3 \pm 3$ km/s和$-316 \pm 4$ km/s，$-0.76 \pm 0.09$ dex和$-2.33 \pm 0.04$ dex。
...
### [5] [The Far-Ultraviolet Extragalactic Legacy (FUEL) Survey: Hubble Far-UV Images and Catalogs of the Extragalactic Legacy Fields](https://arxiv.org/abs/2603.04511)
...
**TL;DR**: 利用哈勃太空望远镜（HST）的先进巡天相机/太阳盲通道（ACS/SBC）在远紫外波段（FUV，约1600埃）对GOODS-S、GOODS-N和COSMOS三个外场进行高分辨率成像观测，获得了1068个星系的光度测量数据，填补了红移（redshift）z≈0到z>1之间高分辨率远紫外成像的空白，为研究恒星形成区、尘埃特性、远紫外河外背景及红移z>1.2星系的莱曼连续谱发射提供了关键数据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用哈勃太空望远镜（HST）先进巡天相机/太阳盲通道（ACS/SBC）在远紫外波段（FUV）对三个河外星场（GOODS-S、GOODS-N和COSMOS）进行高分辨率成像，填补了红移（redshift）在0至1.2之间高分辨率远紫外观测的空白。

Data: 数据来源于哈勃太空望远镜（HST）高级巡天相机/太阳盲通道（ACS/SBC）F150LP（约1600埃）对三个河外场（GOODS-S、GOODS-N和COSMOS）的365轨道高分辨率成像观测，覆盖面积44.7平方角分，典型深度为FUV约28.7 AB（3σ，0.5角秒直径孔径）。

Method: 采用哈勃太空望远镜（HST）高级巡天相机/太阳盲通道（ACS/SBC）F150LP滤光片（约1600埃）获取远紫外（FUV）图像数据，并通过365轨高分辨率成像覆盖44.7平方角分区域，典型深度达FUV约28.7 AB（3σ，0.5角秒直径孔径）。

Result: 利用哈勃太空望远镜（HST）ACS/SBC F150LP滤光片对GOODS-S、GOODS-N和COSMOS三个外场进行了365轨高分辨率远紫外（FUV）成像观测，覆盖44.7平方角分区域，典型深度达28.7 AB星等（3σ，0.5角秒孔径）。
...
### [6] [MXDFz4.4: A LyC emitter 250Myr after the epoch of reionization and a first test of Ly-alpha morphology as a tracer of LyC escape at high redshift](https://arxiv.org/abs/2603.04517)
...
**TL;DR**: 发现了红移(z)4.442的莱曼连续谱(LyC)发射体MXDFz4.4，其光子逃逸率高达50-100%，表明早期宇宙中星暴活动对宇宙再电离的重要贡献。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙再电离（cosmic reionization）过程中电离源（ionizing sources）的贡献评估，特别是高红移（high-redshift）星系中电离光子逃逸（LyC escape）的量化挑战。

Data: 数据来源于MUSE eXtremely Deep Field（MXDF）中红移z=4.442的LyC（Lyman continuum）发射体MXDFz4.4的观测，使用F435W滤光片检测到10.3σ显著性的LyC流量（4.2±0.5nJy）。

Method: 采用已确立的低红移莱曼连续谱(LyC)逃逸示踪方法和首次应用于高红移的莱曼阿尔法(Ly-alpha)形态示踪指标（如光晕占比(halo fraction)），结合光谱能量分布(SED)拟合和紫外(UV)诊断（包括恒星形成率面密度和比恒星形成率(sSFR)）分析。

Result: 在z=4.442处发现的MXDFz4.4是目前最高红移的莱曼连续谱(LyC)发射体，其LyC逃逸分数高达50-100%，表明早期宇宙中星系的随机恒星形成对宇宙再电离有重要贡献。
...
### [7] [Detection of non-thermal radio emission components from the Orion Nebula: stellar jets, cloud collision or feedback from stellar winds?](https://arxiv.org/abs/2603.04521)
*Md Rashid,Nirupam Roy,Prasun Dutta,Jagadheep D. Pandian,Sarita Vig,Srijita Pal,Arnab Chakraborty,Samir Choudhuri*

**TL;DR**: 利用升级的巨米波射电望远镜(uGMRT)首次在扩展猎户座星云(EON)区域明确探测到非热辐射，发现其与年轻恒星天体喷流存在强相关性。


<details>
  <summary>Details</summary>

Background: 研究猎户座星云(Orion Nebula)这一邻近的高质量恒星形成区，旨在揭示复杂恒星形成环境中的物理过程（特别是热轫致辐射(thermal bremsstrahlung)和非热同步辐射(non-thermal synchrotron)的发射机制）及其与磁场、相对论粒子能量分布的关联。

Data: 使用升级后的巨型米波射电望远镜（uGMRT）的3和4波段进行宽频带干涉观测，覆盖了其他灵敏射电干涉仪未探测的频率范围。

Method: 采用升级后的巨米波射电望远镜(uGMRT)进行宽频带干涉观测，并利用模拟数据验证谱指数测量的可靠性。

Result: 在扩展猎户座星云(EON)区域首次明确探测到非热辐射(non-thermal emission)，其起源与年轻恒星天体(YSO)的外流存在强相关性。
...
**TL;DR**: 这篇论文的核心贡献是综述了星际多环芳烃（PAHs）的形成、演化及其在星系星际介质（ISM）中的关键作用，并总结了詹姆斯·韦伯太空望远镜（JWST）观测与实验室研究对该领域的新见解。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际多环芳烃（PAHs）在星系星际介质中的关键作用及其形成与演化机制。

Data: 数据来源为詹姆斯·韦伯太空望远镜（James Webb Space Telescope）提供的高质量光谱数据，并结合了实验室研究和量子化学研究的支持数据。

Method: 采用实验室研究、量子化学计算和分子物理模型（molecular physics models）分析星际多环芳烃（PAHs）的化学演化及光谱特性。

Result: 星际多环芳烃（PAHs）是星系星际介质的重要组成部分，约占据元素碳的10%，其振动辐射主导了银河系和河外星系天体的中红外光谱。
...
### [9] [The First Detection of Forbidden Emission Lines at the Outskirts of the AGN Broad Line Region?](https://arxiv.org/abs/2603.04577)
*Kelly F. Heckler,Rogemar A. Riffel,Daniel Marsango,Tiago V. Ricci,Angela C. Krabbe,Oli L. Dors*

**TL;DR**: 研究发现LINER星系IC 1459中氢复合线和禁戒跃迁均呈现双峰(Double-peaked)宽发射线轮廓，通过盘状宽线区(BLR)模型拟合表明其结构为倾角约35度、内部湍流速度约500 km/s、最大半径9.6 (+4.8, -1.1)光年的圆形吸积盘，证实禁戒发射线可在BLR外围低密度区产生。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）中双峰（DP）宽发射线轮廓的起源及其对宽线区（BLR）盘状结构的指示作用，特别关注低光度AGN中重组线和禁戒线的观测现象。

Data: 数据来源于对LINER星系IC 1459的光谱观测（optical spectroscopy），包括氢复合线和禁戒跃迁线的双峰宽发射线轮廓（double-peaked broad emission line profiles），但排除了临界密度最低的[S II]双线。

Method: 采用圆盘状宽线区（BLR）模型拟合双峰发射线轮廓，假设吸积盘倾角约35度、内部湍流速度约500 km/s，最大半径限制在9.6 (+4.8, -1.1) 光年范围内。

Result: 在LINER星系IC 1459中，除[S II]双线外所有强光学发射线均检测到双峰（DP）轮廓，表明禁戒发射线可在宽线区（BLR）外围低密度区域产生。
...
### [10] [3D Rotation of the Open Cluster NGC 2516](https://arxiv.org/abs/2603.04687)
*Nicholas J. Wright,R. D. Jeffries,M. A. Whalley*

**TL;DR**: 结合盖亚(Gaia)天体测量和盖亚-ESO巡天径向速度数据，首次精确测量了疏散星团NGC 2516的三维旋转（中值转速0.12±0.02 km/s），发现其最大旋转轴与银河系平面夹角为74±17度，且转速与星团年龄和质量的理论预期不符。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用盖亚(Gaia)天体测量和盖亚-ESO巡天径向速度数据测量疏散星团NGC 2516的三维旋转特性，并探讨其旋转轴与银河系盘面的夹角及旋转速度与星团年龄、质量预期关系的偏差。

Data: 数据来源为盖亚(Gaia)天体测量数据和盖亚-ESO巡天(Gaia-ESO Survey)的径向速度数据。

Method: 采用贝叶斯模型(Bayesian model)结合盖亚(Gaia)天体测量和盖亚-ESO巡天径向速度数据，推断疏散星团NGC 2516中所有恒星的三维位置和速度。

Result: 测得的NGC 2516星团中位旋转速度为0.12±0.02 km/s，其最大旋转轴与银河系平面夹角为74±17度，且该旋转速率与星团年龄和质量预期关系不符。
...
### [11] [Early Planet Formation in Embedded Disks (eDisk). XVIII. Indication of a possible spiral structure in the dust-continuum emission of the protostellar disk around IRAS 16544-1604 in CB 68](https://arxiv.org/abs/2603.04719)
...
**TL;DR**: 通过数值模拟和辐射传输计算发现，原恒星盘（protostellar disk）IRAS 16544-1604尘埃连续谱发射中的不对称肩部特征可由托尔敏参数（Toomre parameter）Q~1的盘内引力不稳定性（gravitational instability）解释，且观测中螺旋结构不明显并不代表内部子结构缺失。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原恒星盘（protostellar disk）演化理论与eDisk观测之间的关联，探讨引力不稳定性（gravitational instability）在盘结构形成中的作用及其观测表现。

Data: 研究采用数值模拟（numerical simulation）结合辐射传输计算（radiative transfer calculation）的数据来源。

Method: 采用流体动力学模拟（hydrodynamic simulations）结合辐射传输计算（radiative transfer calculations）分析原行星盘（protostellar disk）的引力不稳定性（gravitational instability）和螺旋结构（spiral structures）。

Result: 数值模拟和辐射传输计算表明，原行星盘（protostellar disk）中观测到的不对称肩部特征（asymmetric shoulder feature）可能源于质量足够大（托尔米参数Q~1）时产生的螺旋结构，即使这些结构因盘面倾斜和望远镜波束卷积而变得难以分辨。
...
### [12] [Accelerating massive galaxy formation with primordial black hole seed nuclei](https://arxiv.org/abs/2603.04784)
*Jeremy Mould*

**TL;DR**: 研究发现，大质量原初黑洞（PBHs）作为暗物质组分可通过充当高密度种子将星系形成时间缩短至100 Myr，并可能导致低表面亮度或弥散星系的形成。


<details>
  <summary>Details</summary>

Background: 该研究探讨了原始黑洞（PBHs）作为暗物质候选者通过其高密度种子效应加速星系形成（galaxy formation）的潜在机制及其对低表面亮度星系（low surface brightness galaxies）起源的影响。

Data: 研究中使用的数据来源为理论模拟（simulation）数据，基于假设的原初黑洞（PBHs）作为暗物质（dark matter）组分对星系形成的影响进行建模。

Method: 采用数值模拟（numerical simulation）和半解析模型（semi-analytic model）分析原始黑洞（PBHs）作为暗物质（dark matter）种子对星系形成的催化作用。

Result: 大质量原初黑洞（PBHs）作为暗物质组分可显著加速星系形成，将星系组装时间缩短至100 Myr以内，并可能导致低表面亮度或弥散星系的产生。
...
### [13] [The Age of the Universe with Globular Clusters IV: Multiple Stellar Populations](https://arxiv.org/abs/2603.04872)
*David Valcin,Raul Jimenez,Carmela Lardo,Uroš Seljak,Licia Verde*

**TL;DR**: 通过考虑球状星团(globular clusters)中多星族(multiple stellar populations)的影响并分析69个球状星团的高质量观测数据，发现多星族假设对年龄估算影响可忽略（差异仅0.6σ），测得最古老贫金属星族([Fe/H]<-1.5)平均年龄为13.61±0.25(统计)±0.23(系统)Gyr，推算宇宙年龄为13.81±0.25(统计)±0.23(系统)Gyr，证实球状星团宇宙计时法对星族复杂性具有鲁棒性。


<details>
  <summary>Details</summary>

Background: 研究主题为通过球状星团（globular clusters）的多重恒星族（multiple stellar populations）分析重新测定宇宙年龄，验证其作为宇宙计时器（cosmic chronometry）的可靠性。

Data: 数据来源于69个球状星团的高质量哈勃太空望远镜(Hubble Space Telescope)颜色-星等图(color-magnitude diagrams)。

Method: 采用基于哈勃太空望远镜(Hubble Space Telescope)高质量颜色-星等图的全形态建模方法，结合显式处理场污染和层次化框架传播非高斯年龄后验的统计推断技术，对球状星团中两个独立年龄、金属丰度、氦丰度及种群比例的恒星群体进行建模。

Result: 考虑多重恒星族群的球状星团年龄测定对宇宙年龄估算影响可忽略，金属贫乏亚样本中主导古老成分平均年龄为13.61±0.25(统计)±0.23(系统)Gyr，推算宇宙年龄为13.81±0.25(统计)±0.23(系统)Gyr。
...
### [14] [Euclid: A blue galaxy population and a brightest cluster galaxy in the making in a $z\sim1.74$ MaDCoWS2 galaxy cluster candidate](https://arxiv.org/abs/2603.04960)
...
**TL;DR**: 利用欧几里得(Euclid)望远镜观测发现红移(redshift)z~1.74的候选星系团"Puddle"中存在由6-7个星系正在合并形成未来最亮团星系(BCG)，其恒星质量为(5.7±0.3)×10¹¹M☉且300百万年前经历短暂星暴，表明多体合并可能是BCG形成的普遍机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（$z>1.5$）星系团（galaxy cluster）的发现与演化机制，特别是最亮团星系（BCG）通过多体并合（multiobject merger）的形成过程。

Data: 数据来源于欧几里得(Euclid) Quick Release 1 (Q1)的观测数据、Keck MOSFIRE光谱数据以及辅助测光数据。

Method: 采用欧几里得(Euclid)和辅助测光数据，结合光谱能量分布(SED)拟合方法分析星系群和最亮团星系(BCG)。

Result: 在2'半径范围内，BCG（Brightest Cluster Galaxy）周围存在110±14个HE≤22.25的星系过密度，其中18±4%的星系呈红色，与部分z>1.5的星系团一致但低于其他星系团。
...
### [15] [Accelerated size evolution in the FirstLight simulations from z=14 to z=5](https://arxiv.org/abs/2603.05045)
*Daniel Ceverino,Yurina Nakazato,Naoki Yoshida,Ralf Klessen,Simon Glover,Luca Costantin*

**TL;DR**: 基于430个宇宙学模拟和辐射传输计算，研究发现宇宙黎明期（红移z=14至z=5）星系尺寸-质量关系已形成且呈现多样性，尺寸随质量快速演化（600 Myr内增加0.5 dex），主要由红移≥5时星系效率（galaxy efficiency）提升驱动，且恒星形成率面密度（SFR surface density）随红移增加而升高。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙黎明时期（$z=14$至$z=5$）星系尺寸的演化及其驱动机制，特别是高红移（$z>5$）下星系效率（galaxy efficiency）与气体密度或致密分布的关联性。

Data: 使用FirstLight数据库中的430个放大宇宙学模拟（zoom-in cosmological simulations）和辐射传输计算生成七个JWST波段的合成图像，并添加受近期JWST深场河外巡天启发的观测效应。

Method: 采用430个放大宇宙学模拟(zoom-in cosmological simulations)和辐射传输计算(radiative transfer calculations)生成七个JWST波段的合成图像，并叠加受近期JWST深场河外巡天启发的观测效应。

Result: 星系尺寸-质量关系在红移约14时已形成，且固定质量下星系尺寸呈现高度多样性。
...
### [16] [Exploring the chemical evolution in hot molecular cores](https://arxiv.org/abs/2603.05124)
*N. C. Martinez,S. Paron,M. E. Ortega,L. Supán,A. Petriella*

**TL;DR**: 利用ALMA（Atacama Large Millimeter Array）数据首次揭示了热分子核（hot molecular cores, HMCs）内部存在温度梯度，并通过测量CH₃CN、CH₃CCH和CH₃OH的转动温度（T₃rot）与柱密度，结合Nautilus模型模拟验证了其化学演化示踪物的分子丰度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于热分子核（hot molecular cores, HMCs）的物理与化学条件表征及其在分子云中的演化过程。

Data: 数据来源于阿塔卡马大型毫米波阵列（ALMA）档案中的毫米波连续谱和光谱数据。

Method: 基于阿塔卡马大型毫米波阵列(ALMA)存档数据，采用毫米波连续谱和光谱分析技术，通过旋转温度($\rm T_{rot}$)和柱密度估计方法对热分子核(HMCs)进行了热力学表征。

Result: 在热分子核（HMCs）中发现了温度梯度，并测定了CH3CN、CH3CCH以及A-和E-CH3OH的转动温度（Trot）和柱密度。
...
### [17] [HC$_3$N, H$^{13}$CN, and HN$^{13}$C in molecular cores evolving towards star-forming regions](https://arxiv.org/abs/2603.05141)
*R. D. Taboada,S. Paron,M. E. Ortega,H. Saldaño*

**TL;DR**: 通过分析37个分子核（molecular cores）在330-350 GHz频段的谱线，发现H$^{13}$CN和HN$^{13}$C丰度与区域温度呈正相关，而HC$_3$N无此关联，揭示了恒星演化早期星际介质的初始化学条件。


<details>
  <summary>Details</summary>

Background: 研究聚焦恒星形成早期阶段分子核（molecular cores）的化学与物理特性，旨在揭示星际介质初始条件对恒星演化的影响。

Data: 使用阿塔卡马大型毫米波阵列(ALMA)的存档数据，分析了37个分子核中心位置在330-350 GHz频段的光谱。

Method: 采用高斯拟合(Gaussian fits)分析ALMA档案数据中HC$_3$N、H$^{13}$CN和HN$^{13}$C的谱线，获取峰值强度、流量和线宽，并估算柱密度与丰度。

Result: 在恒星形成早期阶段，分子核的化学和物理分析揭示了H$^{13}$CN和HN$^{13}$C丰度与区域温度呈正相关，而HC$_3$N无此相关性。
...
### [18] [EMU/GAMA: A statistical perspective on active galactic nuclei diagnostics](https://arxiv.org/abs/2603.05265)
...
**TL;DR**: 利用无监督机器学习聚类算法在多波段（光学、红外、射电）AGN（活动星系核）诊断空间中量化了恒星形成过程和核黑洞活动对射电源总能量预算的贡献比例，开发了可靠性达90%的新型红外-射电AGN三维诊断方案（基于W1-W2颜色、W2星等和1.4 GHz射电流量密度），并发布了包含光学活动概率的射电源星表。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用无监督机器学习聚类算法量化星系总能量预算中恒星形成过程与核黑洞活动（AGN）的相对贡献。

Data: 数据来源于星系与质量组装巡天（GAMA）的G09和G23天区、演化宇宙地图巡天（EMU）以及广域红外巡天探测器（WISE）的多波段（光学、红外和射电）观测数据。

Method: 采用无监督机器学习聚类算法分析多波段（光学、红外和射电）活动星系核（AGN）诊断空间数据，并定义新的红外-射电AGN诊断方案。

Result: 基于多波段（光学、红外、射电）数据和无监督机器学习聚类算法，开发了一种新型三维（$\rm W_1-W_2$颜色、$\rm W_2$星等和1.4 GHz射电流量密度）射电活动星系核（AGN）诊断方案，其选择可靠性接近90%且完备性达90%。
...
### [19] [The Local Tremaine-Weinberg Method for Galactic Pattern Speed: Theory and its Application to IllustrisTNG](https://arxiv.org/abs/2603.05287)
*Hangci Du,Yougang Wang,Junqiang Ge,Rui Guo*

**TL;DR**: 提出基于任意闭合环路积分形式的统一框架，可精确测量星系盘（包括棒旋结构和旋臂）的局部模式速度（pattern speed），并通过TNG50模拟验证其能区分刚性旋转体（棒）与旋臂，恢复恒定或径向变化的全局模式速度。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于建立测量星系棒（galactic bars）和螺旋结构模式速度（pattern speed）的统一积分框架，通过推导连续性方程在任意闭合环路上的积分形式，实现对星系盘中不同区域局部模式速度的精确测定。

Data: 数据来源于TNG50模拟（TNG50 simulation），包括正面观测的原型棒旋星系和高度约束的模拟银河系标准构型（Mock Milky Way standard configurations）。

Method: 通过推导连续性方程在任意闭合环路上的积分形式，建立了一个统一框架来定义星系盘（包括棒和旋臂）中任意区域的局部模式速度（pattern speed）。

Result: 通过推导连续性方程在任意闭合环路上的积分形式，建立了一个统一框架来测量星系盘（包括棒旋结构和旋臂）的局部模式速度（pattern speed），并验证了该框架在恢复恒定全局模式速度和径向变化轮廓方面的准确性。
...
### [20] [ULTIMATE deblending I. A 50-band UV-to-MIR photometric catalog combining space- and ground-based data in the JWST/PRIMER survey](https://arxiv.org/abs/2603.05289)
...
**TL;DR**: 通过整合地面望远镜和JWST（James Webb Space Telescope）多波段数据，ULTIMATE-deblending项目首次为高红移（z~8）星系提供了自洽的紫外至射电连续谱（UV-to-Radio）测光数据，将测光红移精度提升40%并减少60%异常值，为早期宇宙星系形成与演化研究建立了关键基准样本。


<details>
  <summary>Details</summary>

Background: 早期宇宙研究中因颜色选择偏差导致的星系样本局限性问题。

Data: 数据来源于JWST（NIRCam和MIRI）和HST的深场成像数据，以及地面望远镜（如CFHT）的低分辨率测光数据，覆盖了从CFHT/U波段到JWST/MIRI F1800W波段的50个波段，总面积为627.1平方角分。

Method: 采用多波段（从CFHT/U到JWST/MIRI F1800W）测光数据结合光谱能量分布拟合（SED-fitting）方法，并整合地面望远镜的低分辨率测光数据以提高精度。

Result: ULTIMATE-deblending项目通过整合地基望远镜的低分辨率测光数据与JWST（James Webb Space Telescope）观测，将光度红移（photometric redshift）精度提升约40%，同时将异常值比例降低约60%。
...
### [21] [GASTON-GP: Source catalogue and millimetre variability of massive protostellar objects](https://arxiv.org/abs/2603.05351)
...
**TL;DR**: 通过GASTON-GP项目对银河系平面区域进行多波段毫米波监测，首次在大质量原恒星样本中未检测到显著变源，表明观测期间未发生百倍级光度爆发（luminosity bursts），印证了孤立核坍缩模拟的预期，凸显了未来高分辨率高频次巡天的必要性。


<details>
  <summary>Details</summary>

Background: 研究主题为原恒星（protostellar）质量增长的物理过程，重点关注大质量原恒星（high-mass protostars）的间歇性吸积（episodic accretion）现象及其毫米波（millimetre）变率观测。

Data: 数据来源于GASTON-GP项目，该项目使用IRAM 30米望远镜上的NIKA2设备在1.15和2.00毫米波段对银道面（中心位置l = 24度）2.4平方度区域进行了11个历元（历时四年）的观测，构建了包含2925个1.15毫米和1713个2.00毫米致密源（compact sources）的星表，并从中筛选约200个高信噪比（high-signal-to-noise）源生成毫米波光变曲线。

Method: 采用NIKA2仪器在IRAM 30米望远镜上对银道面区域进行1.15和2.00毫米多历元观测，结合专用相对校准方案构建毫米波光变曲线。

Result: 在GASTON-GP观测区域内未发现可靠的变光原恒星源（protostellar sources），这与孤立核塌缩模拟（isolated core collapse simulations）推断的爆发频率一致。
...
### [22] [Resolving the sub-parsec circumnuclear density profiles of quiescent galaxies: Evidence for Bondi accretion flows in tidal disruption event hosts](https://arxiv.org/abs/2603.05412)
*Adelle J. Goodwin,Andrew Mummery*

**TL;DR**: 通过11个潮汐瓦解事件(TDE)宿主星系的射电观测，提出了一种新方法拟合其亚秒差距核周密度分布，发现这些星系核周密度分布与简单邦迪吸积流($n_e\propto R^{-3/2}$)预期高度一致，并据此约束了样本平均邦迪吸积率爱丁顿分数为$\log_{10}f_{\rm{Edd}} = -3.96^{+0.30}_{-0.38}$。


<details>
  <summary>Details</summary>

Background: 星系亚秒差距(parsec)尺度核周密度分布是理解超大质量黑洞(SMBH)吸积历史和燃料供给的关键因素。

Data: 研究使用了11个潮汐瓦解事件（TDE）宿主星系的射电观测数据，通过拟合观测到的TDE射电辐射来限制亚秒差距尺度下的核周密度分布。

Method: 采用射电观测数据拟合潮汐瓦解事件(TDE)的核周密度分布，基于邦迪吸积流(Bondi accretion flow)模型（$n_e\propto R^{-3/2}$）联合约束外流物质质量与环境密度。

Result: 利用潮汐瓦解事件(TDE)的射电观测数据，发现TDE宿主星系的亚秒差距(parsec)核周密度分布与简单邦迪(Bondi)吸积流($n_e\propto R^{-3/2}$)预期高度吻合。
...
### [23] [Surprising increase of electron temperature in metal-rich star-forming region](https://arxiv.org/abs/2603.05434)
*Ziming Peng,Renbin Yan,Zesen Lin,Xihan Ji,Man-Yin Leo Lee,Yuguang Chen*

**TL;DR**: 研究发现，在金属丰度（12+log(O/H)）≥8.7的星系中，通过O$^+$的极光-强线比测得的电子温度（electron temperature）呈现与理论预期相反的上升趋势，且该现象未在S$^+$和N$^+$等其他低电离离子中观测到，这一结果对直接$T_e$法测量金属丰度的基本原理提出了挑战。


<details>
  <summary>Details</summary>

Background: 电子温度是测定星系气相金属丰度的关键参数，理论上金属丰度高的星系应具有较低的电子温度。

Data: 数据来源于两个独立的数据集（two independent datasets），通过测量O$^+$的极光-强线比（auroral-to-strong line ratios）来获取电子温度（electron temperature）信息。

Method: 采用氧离子（O$^+$）的极光线与强线比值测量电子温度（electron temperature），并通过对比不同电离态离子（如S$^+$和N$^+$）的数据验证异常趋势。

Result: 在金属丰度(12+log(O/H))≥8.7的星系中，通过O$^+$的极光-强线比测得的电子温度呈现与理论预期相反的上升趋势。
...
### [24] [A FAST Survey of H I Absorption in Low-power Radio Sources](https://arxiv.org/abs/2603.05477)
*Yang Su,Qingzheng Yu,Taotao Fang,Junfeng Wang,Jianfeng Wu,Bo Zhang*

**TL;DR**: 通过对147个邻近(z < 0.1)低功率射电源(10 mJy < S<sub>1.4 GHz</sub> < 30 mJy)的HI 21厘米吸收线研究，发现低功率源中气体丰富且以恒星形成为主导，探测率为10.2%，其中多数吸收线轮廓窄且速度接近系统速度，符合旋转盘特征，但有四例显示扰动运动学特征(流入或流出)，且流出比例随射电功率增加而上升，同时紧凑源比延展源具有更高的HI探测率，而中红外(MIR)亮源未表现出更高探测率或更扰动运动学，且蓝移吸收仅出现在塞弗特星系和低电离核区(LINERs)中，表明原子流出与活动星系核(AGN)电离状态相关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低功率射电源（low-power radio sources）中中性氢（HI）吸收气体的起源与运动学特征，旨在揭示活动星系核（AGN）与周围星际介质的相互作用机制。

Data: 数据来源于使用五百米口径球面射电望远镜（Five-hundred-meter Aperture Spherical radio Telescope）对147个邻近（红移z < 0.1）低功率射电源（1.4 GHz流量密度10-30 mJy、对数功率20.5-23.7 W/Hz）进行的HI 21厘米吸收线观测，包括先导巡天（26个源中3个探测）和后续观测共发现12个新吸收体。

Method: 采用五百米口径球面射电望远镜（FAST）对147个近邻低功率射电源进行中性氢（HI）21厘米吸收线研究，结合先导巡天数据分析了吸收气体的起源与运动学特征。

Result: 在低功率射电源样本中探测到12个新的HI 21厘米吸收体，总探测率为~10.2^{+3.1}_{-2.0}\%，表明低功率源中存在气体丰富且以恒星形成为主导的群体。
...
### [25] [The dark fate of ultra-faint dwarfs: gravothermal collapse in action](https://arxiv.org/abs/2603.04508)
*Moritz S. Fischer,Hai-Bo Yu*

**TL;DR**: 研究发现自相互作用暗物质(SIDM)模型中银河系超暗弱矮星系(UFD)的暗物质晕经历了引力热演化，产生多样化的密度分布，多数处于坍缩阶段，支持大截面(80 cm²/g)的SIDM模型。


<details>
  <summary>Details</summary>

Background: 研究探讨了超暗弱矮星系（UFD）作为暗物质（DM）物理探针的潜力，重点关注暗物质自相互作用（SIDM）模型下暗物质晕的引力热演化如何解释银河系（MW）UFDs暗物质密度分布的多样性。

Data: 研究中使用了针对类似银河系系统的高分辨率自相互作用暗物质（SIDM）N体模拟数据。

Method: 采用理想化的高分辨率自相互作用暗物质（SIDM）N体模拟方法，针对类似银河系的系统进行研究。

Result: 自相互作用暗物质（SIDM）模型中晕的引力热演化产生了与观测相符的多样化暗物质分布，多数超暗弱矮星系（UFD）已进入核心坍缩阶段且中心密度可能随时间增加。
...
### [26] [Beyond21: A Global Framework for Cosmic Dawn and Reionization Within and Beyond the Standard Model](https://arxiv.org/abs/2603.04542)
*Omer Zvi Katz*

**TL;DR**: 开发了开源Python工具包Beyond21，通过统一框架模拟宇宙黎明（CD）和再电离时期（EoR）中第二星族（Pop II）与第三星族（Pop III）恒星形成对21厘米信号、紫外光度函数（UVLFs）、电离历史及X射线背景（CXB）的影响，并演示了毫电荷暗物质导致重子冷却的案例。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙黎明（Cosmic Dawn, CD）和再电离时期（Epoch of Reionization, EoR）的多波段观测数据一致性解释，开发了开源工具Beyond21以模拟早期星系形成及其对星际介质的辐射影响。

Data: 研究中使用的数据来源为模拟数据（simulation data），通过Beyond21这一开源Python软件包实现，该软件包采用灵活的Pop II和Pop III恒星形成模型，并计算由此产生的辐射背景及其对星系际介质的影响。

Method: 采用模块化架构的Python软件包Beyond21，通过自洽耦合演化计算实现恒星形成（Pop II和Pop III）的灵活参数化建模，并预测21厘米信号、紫外光度函数（UVLFs）、电离历史及X射线背景（CXB）贡献。

Result: 开发了开源Python工具包Beyond21，可自洽计算宇宙黎明（CD）和再电离时期（EoR）的21厘米信号、紫外光度函数（UVLFs）、电离历史及X射线背景（CXB）贡献。
...
### [27] [Probing Dark Energy on the Moon](https://arxiv.org/abs/2603.04841)
*Alfredo Gurrola,Robert J. Scherrer,Oem Trivedi*

**TL;DR**: 月球激光干涉仪通过测量视界尺度（horizon-scale）的度规涨落，首次直接探测了暗能量有效场论（EFT）的动力学部分，特别是约束了动能系数M₂⁴和声速cₛ²，为检验晚期宇宙加速模型的微观物理一致性提供了新方法。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用宇宙加速的有效场理论（EFT）框架独立于模型地描述暗能量和修正引力，并探索现有观测对其动力学算子的约束不足问题。

Data: data来源为月球激光干涉仪（lunar laser interferometer）对宇宙加速有效场论（EFT）中视界尺度（horizon-scale）度规涨落的测量数据。

Method: 采用月球激光干涉仪测量视界尺度度规涨落，直接探测暗能量有效场论（EFT）动力学部分，重点约束控制标量扰动动力学的算子（如动能系数M₂⁴和声速cₛ²）。

Result: 月球激光干涉仪测量可有效约束宇宙加速有效场论（EFT）的动力学系数M₂⁴和暗能量声速cₛ²，为微物理一致性条件提供新观测手段。
...
### [28] [STOchastic LAttice Simulation of hybrid inflation](https://arxiv.org/abs/2603.04850)
*Tomoaki Murata,Yuichiro Tada*

**TL;DR**: 通过晶格模拟和拓扑分析发现，多瀑布场混合暴胀模型（multi-waterfall hybrid inflation）中随机噪声会将拓扑缺陷（如畴壁、宇宙弦和单极子）重连为远小于哈勃尺度的精细结构，并揭示n=1时曲率扰动存在独特的全局结构特征。


<details>
  <summary>Details</summary>

Background: 研究主题为多瀑布场混合暴胀模型中曲率扰动的空间分布特征及其与拓扑缺陷（如畴壁、宇宙弦和磁单极子）的关联机制。

Data: 研究使用了基于随机暴胀形式论的晶格模拟代码STOLAS，通过改变瀑布场数量n和暴胀势函数形式（"Quadratic"和"Cubic"两种情况）对六种案例进行了分析。

Method: 采用基于随机暴胀形式论的晶格模拟代码STOLAS分析多瀑布场混合暴胀模型的空间曲率扰动分布。

Result: result：多瀑布混合暴胀模型中曲率扰动的空间分布分析表明，拓扑缺陷（如畴壁（n=1）、宇宙弦（n=2）和单极子（n=3））在暴胀期间被随机噪声重新连接为更精细结构，使其关联长度远小于瀑布相变临界点的哈勃尺度；欧拉示性数还揭示了n=1时曲率扰动的全局结构，这为早期宇宙物理提供了新探测手段。
...
### [29] [Sound Mode and Scale-Dependent Growth in Two-Fluid Dynamical Dark Energy](https://arxiv.org/abs/2603.05049)
*Frans van Die,Vincent Desjacques*

**TL;DR**: 通过双流体模型研究发现，动力学暗能量（DDE）产生的声波扰动会导致物质涨落和晕偏置（halo bias）的尺度依赖性，利用星系数密度功率谱（P）和双谱（B）的多示踪分析可探测该效应，其中双谱信息对探测DDE声速模式（sound mode）的尺度依赖性至关重要。


<details>
  <summary>Details</summary>

Background: 研究探索了动力学暗能量（DDE）对宇宙结构增长的扰动效应及其在星系巡天中的可观测特征。

Data: 数据来源于对星系数统计的功率谱（P）和双谱（B）进行多示踪分析（multi-tracer analysis）的费希尔预测（Fisher forecast），针对红移范围$z=0.5 - 1$、体积$V\sim 10\, h^{-3}{\rm Gpc}^3$的巡天观测。

Method: 采用双流体模型（two-fluid model）研究动力学暗能量（DDE）对宇宙结构增长的影响，通过允许暗能量状态方程平滑穿越幻影界限（phantom divide）并支持类似声波的传播扰动。

Result: 动态暗能量（DDE）的声波扰动会诱导物质涨落和晕偏置（halo bias）的尺度依赖性，可用于通过其涨落测试暗能量的动力学性质。
...
### [30] [Reassessing the SIGW Interpretation of PTA Signal: The Role of Third-Order Gravitational Waves and Implications for the PBH Overproduction](https://arxiv.org/abs/2603.05191)
*Zhi-Chao Zhao,Sai Wang,Qing-Hua Zhu,Xin Zhang*

**TL;DR**: 通过引入三阶引力波增强标量诱导引力波（SIGW）谱振幅，结合多源观测数据约束其参数，缓解了原初黑洞（PBH）过量产生问题，并验证了SIGW对脉冲星计时阵（PTA）信号解释的鲁棒性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于脉冲星计时阵（PTA）信号与早期宇宙线性曲率扰动诱导的二阶引力波关联性引发的原初黑洞（PBH）过量产生问题，探索高阶引力波对缓解该理论矛盾的潜在贡献。

Data: 数据来源包括宇宙微波背景（cosmic microwave background）和重子声学振荡（baryon acoustic oscillations）的联合数据集，以及脉冲星计时阵列（pulsar timing array）数据。

Method: 将标量诱导引力波（SIGW）框架扩展至三阶引力波以增强其频谱振幅，并结合宇宙微波背景和重子声学振荡数据推导宇宙学约束。

Result: 将标量诱导引力波（SIGW）框架扩展至三阶引力波可显著增强其频谱振幅，从而部分缓解原初黑洞（PBH）过量产生问题。
...
### [31] [The Bayesian view of DESI DR2: Evidence and tension in a combined analysis with CMB and supernovae across cosmological models](https://arxiv.org/abs/2603.05472)
*Dily Duan Yi Ong,David Yallup,Will Handley*

**TL;DR**: 采用贝叶斯重分析（Bayesian reanalysis）结合DESI DR2重子声波振荡（BAO）、普朗克宇宙微波背景（Planck CMB）及超新星数据，发现原DES-SN5YR校准误差导致ΛCDM与$w_0w_a$CDM间3.1σ差异，修正后动态暗能量（dynamical dark energy）证据消失。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用贝叶斯方法重新分析DESI DR2数据，评估ΛCDM（Lambda Cold Dark Matter）模型及其扩展版本在多种宇宙学数据集（包括重子声波振荡、宇宙微波背景、超新星和弱引力透镜）中的证据强度。

Data: 数据来源包括DESI DR1/DR2、Planck CMB、超新星数据（Pantheon+、Union3、DES-SN5YR、DES-Dovekie）以及DES-Y1弱引力透镜观测。

Method: 采用无阻碍框架(unimpeded framework)结合嵌套抽样(nested sampling)和PolyChord算法进行完全贝叶斯分析，计算ΛCDM及其七个扩展模型的证据值(evidence)。

Result: 贝叶斯奥卡姆剃刀（Bayesian Ockham's razor）对扩展模型施加惩罚，导致与基于Δχ²的分析相比偏好减弱或相反。
...
### [32] [Parameter estimation of eccentric massive black hole binaries with LISA and its cosmological implications](https://arxiv.org/abs/2603.04462)
*Jia-Hao Zhong,Jin-Zhao Yang,Tao Yang,Xu-Heng Ding,Xi-Long Fan,Kai Liao,Bei You*

**TL;DR**: 研究发现大质量黑洞双星（MBHBs）的轨道偏心率（eccentricity）能显著提升LISA对引力波（GWs）参数估计精度，通过增加谐波信息打破参数简并性，使三种模型（popIII、Q3d、Q3nod）下定位精度和距离推断提升约10倍，亮汽笛（bright sirens）事件数增加50%-85%，并将Q3d模型中哈勃常数（H0）相对不确定度从8.17%降至4.35%，暗能量状态方程和修正引力波传播的约束也同步增强。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用空间引力波(GW)探测器（如LISA）观测具有轨道偏心率(eccentricity)的大质量黑洞双星(MBHB)及其电磁对应体，以提升宇宙学参数测量精度。

Data: 使用EccentricFD波形并基于三种种群模型（popIII、Q3d和Q3nod）构建了LISA的5年引力波事件目录。

Method: 采用EccentricFD波形模型并构建三种不同种群模型（popIII、Q3d和Q3nod）下的5年LISA引力波事件目录。

Result: 初始轨道偏心率为0.4（10^-4 Hz）显著提升了LISA对MBHBs（massive black hole binaries）的定位精度和距离推断能力，使三种模型（popIII、Q3d、Q3nod）的明亮标准汽笛（bright standard sirens）平均数量分别增至11、12、24例，并将Q3d模型下H_0的相对不确定度从8.17%降至4.35%。
...
### [33] [SNAPPY CubeSat Control Script Generation and Data File Processing](https://arxiv.org/abs/2603.04467)
*Edward Bierens,Jonathan Folkerts,Brian Doty,Holger Meyer,Nickolas Solomey*

**TL;DR**: 开发了用于nuSOL项目中SNAPPY立方星数据自动检索、处理和存储的服务器系统，包含基于ROOT和PostgreSQL的数据处理架构及卫星指令图形界面(GUI)，并计划增加自动邮件通知功能。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于开发专用服务器系统以支持nuSOL（Neutrino Solar Orbiting Laboratory）项目中SNAPPY（Solar Neutrino and Astro-Particle PhYsics）立方星（CubeSat）生成数据的检索、处理和存储。

Data: 数据来源于太阳中微子和天体粒子物理立方星（SNAPPY）通过中微子太阳轨道实验室（nuSOL）项目生成的观测数据。

Method: 采用CERN的ROOT软件和PostgreSQL数据库构建数据处理系统，并开发图形用户界面(GUI)辅助生成与立方星(CubeSat)通信的指令。

Result: 开发了一个服务器系统，用于自动处理、存储来自SNAPPY CubeSat（太阳中微子与天体粒子物理立方星）的数据，并记录文件及指令到PostgreSQL数据库，同时提供了图形用户界面(GUI)辅助生成卫星指令。
...
**TL;DR**: 提出了一种用于高维贝叶斯推断的深度生成框架，在宇宙微波背景（CMB）去透镜（delensing）任务中比扩散基线方法快一个数量级地恢复真实功率谱，并展现出对宇宙学参数变化的鲁棒性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用深度生成框架解决高维贝叶斯推断问题，以应对天体物理数据规模和分辨率快速增长带来的高效后验采样需求。

Data: 数据来源于模拟观测（simulated observations）生成的宇宙微波背景（CMB）透镜化及去透镜化功率谱。

Method: 采用基于深度生成框架的高维贝叶斯推断方法，结合快速模拟推断技术实现高效后验采样。

Result: 提出了一种用于高维贝叶斯推断的深度生成框架，能够实现高效的后验采样（posterior sampling），其采样速度比扩散基线（diffusion baseline）快一个数量级。
...
### [35] [NEXUS: Quick Release Notes](https://arxiv.org/abs/2603.04586)
*Ming-Yang Zhuang,Yue Shen,Zhiwei Pan,Lei Hu,Adam J. Burgasser,David A. Coulter,Jenny E. Greene,Junyao Li,Feige Wang*

**TL;DR**: NEXUS巡天计划利用JWST在2024-2028年对北黄极(North Ecliptic Pole)区域开展多波段成像和光谱观测，通过宽区(Wide tier)和深区(Deep tier)两档覆盖，实现了NIRCam/WFSS 2.4-5μm光谱与多波段成像的宽视场监测，以及NIRSpec 0.54-5.5μm高复用度MOS/PRISM光谱对~10,000个目标的深度时域观测，并定期发布深区数据以支持后续研究。


<details>
  <summary>Details</summary>

Background: 该研究围绕詹姆斯·韦伯太空望远镜（JWST）在2024-2028年期间对北黄极（North Ecliptic Pole）区域开展的多周期（Cycles 3-5）深度成像与光谱巡天（NEXUS），旨在通过宽视场（Wide tier）与深场（Deep tier）两层次观测获取近红外至中红外波段的星系演化、恒星形成及宇宙学参数的高精度数据。

Data: 数据来源于JWST（James Webb Space Telescope）在2024-2028年期间对北黄极区域进行的NIRCam/WFSS（2.4-5μm）和NIRSpec（0.54-5.5μm）多波段成像与光谱观测，包括宽视场（Wide tier）的3年3次重复观测和深视场（Deep tier）的18次2个月间隔重复观测，并辅以MIRI并行成像和额外NIRCam滤光片观测。

Method: 采用近红外相机（NIRCam）宽场狭缝光谱（WFSS）和多目标光谱仪（NIRSpec）多天体狭缝（MOS）棱镜（PRISM）技术进行2.4-5微米和0.54-5.5微米光谱观测，结合多波段成像（F090W至F444W）和中红外仪器（MIRI）并行观测。

Result: NEXUS计划在2024-2028年期间利用JWST进行北黄极区域的多波段成像和光谱观测，包含宽视场（Wide）和深视场（Deep）两个重叠层级的观测。
...
### [36] [PulSKASim: A Pulsar Simulator for SKA-Scale Interferometric Observations](https://arxiv.org/abs/2603.04593)
*X. Li,V. Stolyarov*

**TL;DR**: 开发了PulSKASim模拟器，通过精确建模脉冲星（pulsar）流量随周期、最大流量、占空比和噪声的演化，并考虑积分时间、采样和观测时长等参数，填补了现有工具在平方公里阵列（SKA）干涉测量端到端测试中脉冲星流量变异性模拟的空白。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升平方公里阵列（SKA）干涉测量流程测试的准确性，通过开发新型脉冲星流量模拟器（PulSKASim）解决现有工具忽略积分时间等关键观测参数导致的模拟局限性。

Data: 研究中使用的数据来源为通过OSKAR和Pyuvsim等射电干涉仪模拟器生成的合成测量集（synthetic measurement sets），其中每个快照（snapshot）包含具有可控流量水平的脉冲星。

Method: 开发了PulSKASim模拟器，通过建模脉冲星周期、最大流量、占空比和噪声等参数，并考虑积分时间、采样和观测时长，生成合成测量集（measurement sets），以模拟SKA规模干涉观测中的脉冲星流量变化。

Result: 开发了PulSKASim模拟器，可精确模拟脉冲星（pulsar）在积分时间、采样和观测时长影响下的流量演化，填补了现有工具无法结合脉冲星变异性建模与干涉测量模拟的空白。
...
### [37] [AstroInspect: a web-based system to organize, assess, and visually inspect astronomical objects](https://arxiv.org/abs/2603.04713)
...
**TL;DR**: 开发了基于网页的天文数据可视化工具AstroInspect，通过整合SDSS、LS和S-PLUS等多源观测数据实现了实时交互式天体目标分类与验证，并以长蛇座I星系团方向981个候选体中确认80个Hα发射星系为例验证了其科学价值。


<details>
  <summary>Details</summary>

Background: 天文观测数据快速增长背景下亟需高效工具支持视觉检查（visual inspection）以完成分类、星表优化和自动化方法验证等任务。

Data: 数据来源包括斯隆数字巡天(SDSS)、遗产巡天(LS)和南天局域宇宙测光巡天(S-PLUS)的实时获取的成像、光谱和测光数据。

Method: 基于网络平台的AstroInspect系统，通过整合斯隆数字巡天(SDSS)、Legacy Surveys(LS)和南天光度局部宇宙巡天(S-PLUS)等多源数据，提供实时图像、光谱和测光数据的自动化检索与图形用户界面(GUI)交互式可视化分析。

Result: 开发了基于网页的天文数据可视化工具AstroInspect，可实时整合多源观测数据并辅助高效目视检查。
...
### [38] [A New Method for Identifying Contaminating Sources and Locating Target Sources through the Cross-Arm Features of Micro Pore Optics](https://arxiv.org/abs/2603.04830)
...
**TL;DR**: 提出利用微孔光学器件（MPOs）点扩散函数（PSF）十字臂结构增强卫星识别污染源和定位目标源能力的新方法，通过十字臂探测器相对计数变化推导源位置，模拟显示在1 Crab流量和200秒曝光下可识别8'离轴角污染源并实现6'定位精度，扩展至16x16硅漂移探测器（SDDs）阵列时1秒曝光下识别离轴角2.4'污染源且定位精度达1.8'。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于提升CATCH（Chasing All Transients Constellation Hunters）空间任务中Type-A卫星搭载的微孔光学元件（Micro-Pore Optics, MPOs）和硅漂移探测器（Silicon Drift Detectors, SDDs）对污染源的识别能力及目标源定位精度。

Data: 数据来源于模拟观测（simulation），包括使用微孔光学（MPOs）和硅漂移探测器（SDDs）的仿真实验，以及针对CATCH Type-A卫星计划中SDD阵列的扩展模拟研究。

Method: 采用基于微孔光学器件（MPOs）点扩散函数（PSF）十字臂的新型定位方法，通过水平与垂直十字臂上的探测器相对计数变化反演目标源位置。

Result: 提出了一种基于微孔光学器件（MPO）点扩散函数（PSF）十字臂的新方法，通过水平与垂直十字臂探测器相对计数变化定位目标源，模拟显示在1 Crab流量和200秒曝光下可识别8'离轴角污染源并将定位精度提升至6'。
...
### [39] [MWA tied-array processing V: Super-resolved localisation via amplitude-only maximum likelihood direction finding](https://arxiv.org/abs/2603.04961)
*Bradley W. Meyers,Arash Bahramian*

**TL;DR**: 通过利用望远镜主波束和绑定波束（tied-array beam）模式信息结合无线电测向技术，在默奇森宽场阵列（MWA）上实现了超越原生空间分辨率的瞬变源和脉冲星"超分辨"定位（super-resolved localisation），验证了该方法对南天快速两米脉冲星巡天（Southern-sky MWA Rapid Two-metre pulsar survey）候选体后续跟踪定位的有效性和不确定性评估的可靠性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用干涉仪（interferometer）和波束成形（tied-array beam）技术提升低频紧凑阵列对瞬变源和脉冲星（pulsar）的超分辨率定位（super-resolved localisation）能力。

Data: 数据来源于默奇森宽场阵列（MWA）的电压捕获和绑定阵列（tied-array）处理能力，并利用先前发现的MWA脉冲星通过高分辨率干涉成像定位的案例以及已知脉冲星样本的定位进行验证。

Method: 采用射电定向（radio direction finding）技术结合MWA（Murchison Widefield Array）的电压捕获和固定阵列（tied-array）处理能力，通过多相邻波束信号的信噪比与波束结构关系实现超分辨（super-resolved）定位。

Result: 利用望远镜主波束和固定阵列波束模式，通过无线电测向技术结合多波束探测实现瞬变体和脉冲星（pulsar）的超分辨率定位（super-resolved localisation），其精度远超仪器固有空间分辨率。
...
### [40] [Sensitivity of a closed dielectric haloscope to axion dark matter](https://arxiv.org/abs/2603.05006)
...
**TL;DR**: 提出了一种计算封闭介质光晕仪（dielectric haloscope）对轴子暗物质（axion dark matter）灵敏度的简化模型，应用于MADMAX原型机在1.6 T磁场中的实验数据，为未来暗物质探测奠定基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用介电晕镜（dielectric haloscope）探测轴子暗物质（axion dark matter）的理论可行性和实验方法，特别针对26-500微电子伏特（μeV）质量范围的轴子。

Data: 数据来源于欧洲核子研究中心（CERN）的1.6 T Morpurgo磁体中进行的磁化盘与镜面轴子实验（MADMAX）原型机采集的数据。

Method: 采用简化电磁响应模型分析封闭式介电晕镜（dielectric haloscope）的灵敏度，该模型可处理几何缺陷和接收系统噪声，并应用于MADMAX原型机在1.6 T Morpurgo磁体中的实验数据。

Result: 提出了一种计算封闭介质光晕仪（dielectric haloscope）对轴子暗物质（axion dark matter）灵敏度的简化模型，该模型能准确描述实际几何缺陷和接收系统噪声的电磁响应。
...
### [41] [Fusion of JWST data - Demonstrating practical feasibility](https://arxiv.org/abs/2603.05014)
*Landry Marquis,Claire Guilloteau,Thomas Oberlin,Nicolas Dobigeon,Olivier Berné*

**TL;DR**: 首次成功将JWST（James Webb Space Telescope）的NIRSpec积分场光谱与NIRCam多波段成像数据融合，生成兼具NIRCam空间分辨率和NIRSpec光谱分辨率的高光谱数据立方体（hyperspectral cube），为天文观测提供了前所未有的空间分辨率物理特性提取方法。


<details>
  <summary>Details</summary>

Background: 数据融合（data fusion）在天文学中用于提升观测数据质量，通过结合不同仪器的互补特性（如高空间分辨率与高光谱分辨率）来生成高分辨率高光谱数据立方体。

Data: 数据来源于JWST（James Webb Space Telescope）的积分视场光谱仪NIRSpec和29个滤光片成像仪NIRCam的观测数据。

Method: 采用数据融合（data fusion）技术结合JWST的NIRSpec积分场光谱仪和NIRCam多波段成像数据。

Result: 首次成功将JWST的NIRSpec积分场光谱与NIRCam 29个滤光片成像数据融合，生成了兼具NIRCam空间分辨率和NIRSpec光谱分辨率的超光谱立方体。
...
### [42] [Simultaneous Misalignment and Mode Mismatch Sensing in Optical Cavities Using Intensity-Only Measurements](https://arxiv.org/abs/2603.05101)
*Liu Tao,Eleonora Capocasa,Yuhang Zhao,Jacques Ding,Isander Ahrend,Matteo Barsuglia*

**TL;DR**: 提出了一种基于卷积神经网络(CNN)的两步深度学习框架，仅通过光束强度图像即可实现高精度模式分解和八自由度(DoFs)失调参数估计，为精密光学系统提供了硬件高效的实时波前传感解决方案。


<details>
  <summary>Details</summary>

Background: 精密光学系统（尤其是干涉型引力波探测器）中空间模式（spatial mode）的精确传感与控制对抑制光学损耗和量子噪声至关重要。

Data: 数据来源于标准CCD（电荷耦合器件）成像采集的单一光束强度图像。

Method: 采用基于多强度图像卷积神经网络(CNN)的两阶段深度学习流程，先通过CNN实现精确模式分解，再通过回归网络同时估计八个自由度(DoFs)的失准和模式失配参数。

Result: 提出了一种基于深度学习的两阶段方法，仅利用光束强度图像即可实现高精度波前传感和模式失配诊断，其中卷积神经网络（CNN）在模态分解阶段达到0.0034的平均绝对误差（MAE），最终阶段恢复的光束缺陷参数总MAE为0.0062，对应每自由度（DoF）平均残余光学损耗为39 ppm。
...
### [43] [Scientific performance of on-board analyses for the SVOM X-ray telescope MXT](https://arxiv.org/abs/2603.05342)
*F. Robinet,C. Van Hove,M. Moita,S. Crepaldi,C. Feldman,A. Fort,O. Frandon,D. Götz,P. Maggi,K. Mercier,A. Sauvageon*

**TL;DR**: 基于SVOM卫星搭载的微通道X射线望远镜(Microchannel X-ray Telescope)对15例伽马暴余辉的观测数据，证实其星载计算机能在数秒内实现优于2角分的定位精度（平均偏差40角秒），并通过高效甚高频天线网络快速下传数据，为多波段后续观测提供了关键支持。


<details>
  <summary>Details</summary>

Background: 研究聚焦于SVOM卫星搭载的微通道X射线望远镜（Microchannel X-ray Telescope）在探测和定位伽马射线暴（gamma-ray bursts）X射线余辉方面的在轨性能评估与科学应用。

Data: 数据来源于搭载在SVOM卫星上的微通道X射线望远镜（Microchannel X-ray Telescope）在2024年10月至2025年8月期间探测到的15个伽马射线暴余辉的观测数据。

Method: 采用微通道X射线望远镜（Microchannel X-ray Telescope）进行在轨实时分析，结合高效甚高频（very-high-frequency）天线网络实现低延迟定位。

Result: 在轨分析能在观测开始后数秒内提供伽马暴（gamma-ray burst）的精确天区位置。
...
### [44] [Resolving diffusion signatures in distant pulsar halos with current and future experiments](https://arxiv.org/abs/2603.04821)
*Yong-Jian Wei,En-Sheng Chen,Kun Fang,Xiao-Jun Bi*

**TL;DR**: 通过模拟LHAASO-KM2A和CTA实验，发现提升光子统计量和角分辨率能显著增强对脉冲星晕（pulsar halos）候选体的形态鉴别能力，有助于扩大样本并研究脉冲星周围宇宙射线传输机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用脉冲星晕（pulsar halos）作为宇宙射线在脉冲星附近传播的独特探针，及其对星际介质中粒子扩散机制理解的重要意义。

Data: 研究中使用的数据来源于模拟两个先进γ射线实验（LHAASO-KM2A和切伦科夫望远镜阵列CTA）的虚拟观测（mock observations），并采用了真实的仪器响应（realistic instrumental responses）。

Method: 采用模拟观测方法评估LHAASO-KM2A和切伦科夫望远镜阵列(CTA)通过形态学区分识别脉冲星晕候选体的能力，重点分析扩散模型与简化空间模型的区分效能。

Result: result：未来γ射线观测（LHAASO-KM2A和CTA）通过提升光子统计量和角分辨率可显著增强脉冲星晕（pulsar halos）形态鉴别能力，从而扩大样本并深化对脉冲星周围宇宙线传输的理解。
...
### [45] [Coexistence of Chromatic Flares and an Achromatic QPO in the Gamma-ray Blazar PG 1553+113](https://arxiv.org/abs/2603.04527)
*Elena Madero,Alberto Domínguez*

**TL;DR**: 通过对BL Lac天体PG 1553+113长达17年的伽马射线准周期振荡(QPO)研究，发现其存在与耀斑相关的"越亮越软"色变趋势，但QPO调制本身是非色变的，支持喷流进动等几何机制而非等离子体过程解释QPO起源。


<details>
  <summary>Details</summary>

Background: 研究聚焦于耀变体中准周期振荡（QPOs）的物理起源争议，主要探讨几何驱动与等离子体驱动两种竞争性解释机制的区分问题。

Data: 数据来源于费米大面积望远镜（Fermi-LAT）2008至2025年期间观测的伽马射线数据，采用30天时间分段处理。

Method: 采用奇异谱分析（SSA）去除慢基线趋势，并结合块自助法（Block Bootstrap）量化光子流量与光子指数间的相关性，同时保留时间依赖性。

Result: 在PG 1553+113中观测到的准周期振荡（QPO）具有非色散性特征，且与等离子体驱动的色散性耀变无关，支持其几何起源（如喷流进动）而非喷流内禀过程。
...
### [46] [Spectral Hardening Revealed by Geometric De-boosting in the Masked Jet of PKS 2155$-$304](https://arxiv.org/abs/2603.04529)
*Alberto Domínguez,Adithiya Dinesh,Elena Madero*

**TL;DR**: 研究发现PKS 2155-304耀变体(blazar)的1.7年伽马射线准周期振荡(QPO)与光谱硬化事件相位锁定，支持几何掩蔽模型(Geometric Masking)解释两成分喷流结构，表明低流量状态的光谱硬化可揭示被相对论放大掩盖的喷流物理机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于耀变体(blazar)伽马射线光变中准周期振荡(QPOs)的物理起源及其与能谱硬化的关联，旨在揭示喷流几何结构对辐射过程的调制机制。

Data: 数据来源于费米大面积望远镜(Fermi-LAT)对PKS 2155-304长达18年的观测数据，采用30天时间分箱处理。

Method: 采用奇异谱分析（Singular Spectrum Analysis）抑制红噪声效应，并结合移动区块自助法（Moving Block Bootstrap）量化光子通量与光子指数的相关性。

Result: 在PKS 2155-304中发现的1.7年伽马射线准周期振荡（QPO）与光谱硬化事件相位锁定，支持几何掩蔽模型（Geometric Masking）解释喷流结构对加速过程可见性的调控作用。
...
### [47] [Geometric masking in AGN jets and its implications for unification and blazar physics](https://arxiv.org/abs/2603.04533)
*Alberto Domínguez*

**TL;DR**: 研究发现活动星系核（AGN）统一模型（Unification Scheme）和耀变体序列（blazar sequence）中的极端GeV谱硬化事件与准周期振荡（quasi-periodic oscillation）相位锁定，提出几何掩蔽（Geometric Masking）导致多普勒增强（Doppler boosting）优先放大软成分，揭示低流量态对应几何透明窗口而非本征静止，表明AGN喷流极端加速的占空比（duty cycle）远高于流量选择观测的推断值。


<details>
  <summary>Details</summary>

Background: 该研究探讨了几何遮蔽（Geometric Masking）情景对活动星系核（AGN）统一模型、耀变体序列（blazar sequence）及更广泛耀变体现象学的影响，并评估其与近期观测结果的一致性。

Data: data来源包括PKS 2155$-$304的相位分辨分析数据、FSRQs S5 1027+74和3C 273在低通量状态下积分探测到的TeV辐射数据。

Method: 采用相位分辨分析(phase-resolved analysis)和几何掩蔽(Geometric Masking)框架分析耀变体序列(blazar sequence)中的几何调制层(geometric modulation layer)。

Result: 多普勒增强（Doppler boosting）优先放大软成分时喷流紧密对准，产生能见度偏差（visibility bias），从而扩展活动星系核（AGN）统一模型并在耀变体序列中引入几何调制层。
...
### [48] [On curvature corrections for field theory cosmic strings](https://arxiv.org/abs/2603.05243)
*Josu C. Aurrekoetxea,Jose J. Blanco-Pillado,Alberto García Martín-Caro,J. M. Queiruga*

**TL;DR**: 在平坦空间中基于阿贝尔-希格斯模型（Abelian-Higgs model）的系统性推导表明，仅保留平移戈德斯通模（Goldstone modes）时有效作用量无曲率修正项，且高阶涨落通过耦合世界面里奇标量（worldsheet Ricci scalar）引入主导曲率贡献。


<details>
  <summary>Details</summary>

Background: 研究聚焦于平坦空间中阿贝尔-希格斯模型（Abelian-Higgs model）场论宇宙弦（cosmic strings）有效作用量的解析与数值分析。

Data: 数据来源于基于自适应网格加密（Adaptive Mesh Refinement）技术的平直空间中完整场论运动方程格点模拟（lattice simulations）。

Method: 采用解析推导与数值模拟（Adaptive Mesh Refinement）相结合的方法，从孤子场论描述出发系统推导低能有效作用量，并通过晶格模拟验证其动力学行为。

Result: 在平坦空间中，阿贝尔-希格斯模型（Abelian-Higgs model）的场论宇宙弦（cosmic strings）有效作用量研究显示，当仅保留平移戈德斯通模（Goldstone modes）时不存在非平凡曲率修正项。
...
### [49] [Experiments towards a neutron target for measurements in inverse kinematics](https://arxiv.org/abs/2603.04471)
...
**TL;DR**: 在洛斯阿拉莫斯国家实验室的中子靶演示项目（Neutron Target Demonstrator）中，通过实验测量和模拟验证了1立方米石墨立方体（full cube）和半立方体（half cube）对1 keV至50 MeV能区中子源的慢化效果，为逆向运动学中子诱发反应测量提供了原理验证依据。


<details>
  <summary>Details</summary>

Background: 中子诱发反应（neutron-induced reactions）在基础核物理、核天体物理和应用领域具有重要作用。

Data: 数据来源于美国圣母大学和德州农工大学加速器产生的1 keV至50 MeV能区中子源，以及1立方米石墨立方体完整和半立方体构型的实验测量。

Method: 采用石墨立方体（graphite cube）作为慢化剂（moderator）测量中子通量分布（neutron flux distribution），并通过实验与模拟结果对比验证可行性。

Result: 在1~m$^3$石墨立方体（full cube moderator）中测量的中子通量分布与模拟结果一致，但半立方体（half cube moderator）配置在部分情况下仍存在差异。
...
### [50] [Neural blind deconvolution to reconstruct high-resolution ground-based solar observations](https://arxiv.org/abs/2603.05033)
*Christoph Schirninger,Robert Jarolim,Astrid M. Veronig,Matthias Rempel,Friedrich Wöger*

**TL;DR**: 基于物理信息神经网络（physics-informed neural networks）开发的新型图像重建方法，通过同时估计大气点扩散函数（PSF）和真实高分辨率强度分布，显著提升了地基太阳观测中对光球层至色球层（photosphere to chromosphere）微小结构的重建精度，超越了现有最优方法。


<details>
  <summary>Details</summary>

Background: 地基太阳观测虽能获取低层太阳大气前所未有的空间、光谱和时间分辨率，但地球湍流大气引入的像差限制了观测质量，需依赖先进的事后图像重建技术突破分辨率极限。

Data: 数据来源包括合成强度数据（源自辐射磁流体力学模拟（radiative MHD simulation））以及来自GREGOR和DKIST的高分辨率观测数据。

Method: 基于物理信息神经网络（physics-informed neural networks）的端到端图像重建方法，通过同步更新点扩散函数（PSF）参数并最小化合成图像与实际短曝光图像序列的损失函数，实现高分辨率太阳图像重建。

Result: 开发了一种基于物理信息神经网络的新型图像重建方法，能同时估计退化点扩散函数（PSF）和真实高分辨率强度分布，显著提升了对太阳光球层至色球层最小空间尺度特征的重建性能。
...
### [51] [TILARA: Template-Independent Line-by-line Algorithm for Radial velocity Analysis. I. Description of the code and application on a Sun-like star](https://arxiv.org/abs/2603.05445)
*C. San Nicolas Martinez,N. C. Santos,V. Adibekyan,K. Al Moulla,A. M. Silva,S. G. Sousa*

**TL;DR**: 该论文开发了一种不依赖模板的逐线径向速度（RV）提取方法TILARA，通过高斯拟合自动测量吸收线中心并计算RV，在太阳型恒星HD 102365的ESPRESSO观测数据中表现出与现有方法相当的精度和稳定性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发一种不依赖模板的逐线径向速度（RV）提取方法（TILARA），以克服传统基于参考光谱的交叉相关或模板匹配方法在数据可变、受污染或稀疏采样时引入的偏差问题。

Data: 数据来源于520次ESPRESSO（高分辨率光谱仪）对类太阳恒星HD 102365的观测。

Method: 采用高斯拟合（Gaussian fitting）自动测量吸收线中心，并通过可配置的离群值剔除方法（如sigma-clipping或down-weighting）计算视向速度（RVs）。

Result: 开发了一种不依赖模板（template-independent）的逐线（line-by-line）径向速度（RV）提取代码TILARA，能在不推荐构建光谱模板时有效运行，并通过高斯拟合自动测量线心，经测试其提取的RV时间序列与现有方法结果精度相当。
...
### [52] [Stellar contents and Star Formation in IRAS 18456-0223](https://arxiv.org/abs/2603.05224)
*Nilesh Pandey,U. S. Kamath*

**TL;DR**: 通过多波段数据分析在IRAS 18456-0223恒星形成区（距离约600 pc）发现89颗年轻恒星体（YSOs），其质量范围为0.1-7.2太阳质量、年龄不超过4 Myr，并揭示这些YSOs形成径向尺度约0.5 pc、平均表面密度约60 pc⁻²的星团，同时探测到多个柱密度峰值（N_H₂∼10²² cm⁻²）的冷丝状结构（尘埃温度10-13 K）及近红外消光峰。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用多波段观测数据解析IRAS 18456-0223恒星形成区中年轻恒星天体（YSOs）的物理特性及其空间分布。

Data: 数据来源包括档案光学数据（盖亚卫星DR3）和红外数据（2MASS、UKIDSS、斯皮策、WISE和赫歇尔），以及自行获取的三颗亮星的光学光谱数据。

Method: 基于红外特性识别年轻恒星天体（YSOs），采用多波段光谱能量分布（SED）拟合确定质量与年龄，利用最小生成树（MST）分析空间分布，并通过赫歇尔（Herschel）数据构建柱密度与温度分布图。

Result: 在IRAS 18456-0223恒星形成区约10'×10'区域内发现了89颗年轻恒星天体（YSOs），包括80颗II类和9颗I类天体。
...
### [53] [Augmenting representations with scientific papers](https://arxiv.org/abs/2603.04516)
...
**TL;DR**: 提出了一种对比学习框架，通过将X射线光谱与科学文献提取的领域知识对齐，建立了共享多模态表征，显著提升了罕见天体源（如候选脉动超亮X射线源PULX和引力透镜系统）的物理解释精度和20个物理变量的估计性能（比单模态基线提高16-18%）。


<details>
  <summary>Details</summary>

Background: 研究主题为通过对比学习框架整合X射线光谱与科学文献中的领域知识，建立多模态共享表征以加速天体物理源（astrophysical sources）的解读。

Data: 数据来源包括X射线光谱（X-ray spectra）和从科学文献中提取的领域知识（domain knowledge extracted from scientific literature）。

Method: 采用对比学习框架（contrastive learning framework）对齐X射线光谱与从科学文献中提取的领域知识，并开发共享多模态表征。

Result: 引入对比学习框架将X射线光谱与文献提取的领域知识对齐，建立共享多模态表征空间，提升罕见天体源解析效率。
...
### [54] [NASA's Pandora SmallSat Mission: Simulated Modeling and Retrieval of Near-Infrared Exoplanet Transmission Spectra](https://arxiv.org/abs/2603.04488)
...
**TL;DR**: 潘多拉(Pandora)小型卫星任务通过同步可见光光度监测和近红外(NIR)光谱观测，解决了恒星异质性对系外行星透射光谱的影响问题，并能将H2O和CH4等主要大气吸收成分的丰度约束精度提升至约1.0 dex，与JWST协同观测可进一步提高大气特征分析的可靠性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过解析恒星异质性（stellar heterogeneity）对系外行星透射光谱的影响来理解系外行星及其宿主恒星。

Data: 数据来源于潘多拉(Pandora)小卫星任务提供的可见光光度监测(0.4--0.7 μm)和近红外(NIR)光谱(0.9--1.6 μm)观测数据。

Method: 采用同步可见光光度监测(0.4-0.7μm)和低分辨率近红外光谱(0.9-1.6μm)技术，结合大气模型对系外行星(如HD 209458 b等)的主要吸收成分(H2O、CH4)进行丰度约束分析。

Result: 潘多拉(Pandora)任务通过分离恒星异质性对系外行星透射光谱的影响，将提供对主恒星变率的长期监测和系外行星大气特征的精确表征。
...
### [55] [NASA's $\textit{Pandora SmallSat Mission}$: Simulating the Impact of Stellar Photospheric Heterogeneity and Its Correction](https://arxiv.org/abs/2603.04519)
...
**TL;DR**: 潘多拉（Pandora）小卫星任务通过联合可见光测光和近红外光谱观测，量化了恒星光球（photosphere）活动（温度误差≈30 K）对系外行星透射光谱的污染（10²-10³ ppm降至≲10 ppm），并确定了单/双组分模型的适用条件（斑点填充因子检测阈值≈0.3%）及复杂斑点分布下的残余污染水平（10³ ppm）。


<details>
  <summary>Details</summary>

Background: 恒星光球层异质性（stellar photospheric heterogeneity）是影响系外行星透射光谱的主要天体物理系统误差来源。

Data: 研究中使用的数据来源为模拟生成的160组Pandora任务数据集，包含随时间变化的恒星光谱（time-dependent stellar spectra）、仪器响应（instrument response）和噪声（noise），基于八种典型恒星活动场景构建。

Method: 采用贝叶斯反演方法(Bayesian retrieval)联合可见光测光和近红外光谱数据，结合双组分光谱模型（宁静光球和黑子）量化恒星光球特性。

Result: 贝叶斯反演结合可见光测光和近红外光谱可恢复恒星光球温度，典型不确定度为约30 K且无显著偏差。
