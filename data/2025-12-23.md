...
**TL;DR**: 在宇宙流体动力学模拟中发现超大质量黑洞(SMBH)质量与固定晕质量下的暗物质晕浓度(dark matter halo concentration)呈正相关，且该相关性在采用邦迪吸积(Bondi accretion)模型的TNG和EAGLE模拟中显著存在，但在采用扭矩限制(torque-limited)模型的SIMBA中缺失，表明晕浓度可能是调控黑洞-星系协同演化的关键参数。


<details>
  <summary>Details</summary>

Background: 研究探讨了超大质量黑洞(Supermassive Black Hole)质量与暗物质晕(Dark Matter Halo)浓度在固定晕质量条件下的相关性及其对黑洞-星系协同演化(coevolution)的影响。

Data: 数据来源于宇宙学流体动力学模拟TNG100（N = 18,954）、EAGLE（N = 1,522）和CAMELS-TNG（N = 6,664），并对比了采用扭矩限制黑洞吸积模型的SIMBA模拟。

Method: 采用宇宙学流体动力学模拟（cosmological hydrodynamical simulations）分析TNG100、EAGLE和CAMELS-TNG数据集中的中心星系，并基于邦迪吸积模型（Bondi-based prescription）计算超大质量黑洞（supermassive black hole）质量与暗物质晕浓度（dark matter halo concentration）的相关性。

Result: 在固定晕质量条件下，超大质量黑洞(Supermassive Black Hole)质量与暗物质晕(Dark Matter Halo)浓度呈显著正相关，且该相关性在基于邦迪吸积(Bondi-based Accretion)的模拟(TNG100、EAGLE、CAMELS-TNG)中高度显著，但在采用扭矩限制模型(Torque-limited Model)的SIMBA中不存在。
...
**TL;DR**: 首次建立了短平均自由程（short-mean-free-path）自相互作用暗物质（self-interacting dark matter）微观粒子物理描述到宏观模拟粒子表征的第一性原理映射，揭示了模拟粒子间会产生依赖于基础粒子物理截面的有效作用力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于暗物质（dark matter）粒子间非引力自相互作用对星系形态的影响及其数值模拟中的相空间分布演化问题。

Data: 数据来源于数值模拟(numerical simulation)，用于研究自相互作用暗物质(self-interacting dark matter)对星系形态的非线性演化影响。

Method: 基于第一性原理构建微观粒子物理描述到宏观模拟粒子表示的映射，推导出模拟粒子间的有效作用力及其与基本粒子物理截面的依赖关系。

Result: 暗物质（dark matter）的自相互作用（self interactions）在短平均自由程（short-mean-free-path）条件下会导致模拟粒子间产生有效力（effective force），其相互作用截面（interaction cross section）依赖于基础粒子物理的截面。
...
**TL;DR**: 首次发现ORC4中心星系中存在由激波电离（shock ionization）产生的空间不对称、高速度梯度（>100 km/s）和高速度弥散（~200 km/s）的电离气体，其发射线比率与激波速度~200-300 km/s的模型高度吻合，表明大尺度射电环（radio ring）的形成事件与中心星系中的激波活动直接相关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于奇射电环（ORCs）这类新发现的河外天体，其起源未知，尤其关注ORC4中心星系中电离气体的空间分布、运动学特征及激发机制。

Data: 数据来源于Keck/KCWI+KCRM积分视场光谱仪（integral field spectroscopy）对ORC4中心星系的多条强光学发射谱线进行的观测。

Method: 采用积分视场光谱技术（integral field spectroscopy）测量ORC4中心星系中电离和中性气体的空间分布、形态、运动学及谱线比。

Result: ORC4中心星系中的电离气体是由速度约200-300 km/s的激波电离产生的，该激波事件很可能与形成ORC4大尺度射电环的事件相关。
...
**TL;DR**: 基于斯隆数字巡天第四期数据发布17（SDSS-IV DR17）的扩展重子振荡光谱巡天（eBOSS）项目开发的eBOSS数据分析管道（eBOSS-DAP）成功分析了1,899,553个红移低于1.12的高质量星系光谱，提供了包括发射线流量、恒星和气体运动学、连续谱指数及恒星族拟合在内的统一测量数据产品，为星系演化和宇宙学研究开辟了新途径。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用斯隆数字巡天第四期数据发布17（SDSS-IV DR17）的扩展重子振荡光谱巡天（eBOSS）海量星系光谱数据，探索星系演化（galaxy evolution）和大尺度结构（large-scale structure）的未开发潜力。

Data: 数据来源于斯隆数字巡天第四期数据发布17（Sloan Digital Sky Survey IV DR17）的扩展重子振荡光谱巡天（Extended Baryon Oscillation Spectroscopic Survey, eBOSS），包含2,233,939条高质量光学星系光谱，通过2角秒光纤获取，覆盖广泛红移范围，并包含197,521条重复观测光谱用于评估分光测光质量。

Method: 采用eBOSS数据分析管道（eBOSS-DAP）对星系光谱进行统一测量，包括发射线流量与等值宽度、恒星与气体运动学（kinematics）、连续谱指数及恒星种群拟合。

Result: 开发了eBOSS-DAP（eBOSS Data Analysis Pipeline）数据分析流程，成功分析了1,899,553个红移低于1.12的高质量星系光谱，生成了包含发射线流量、等值宽度、恒星与气体运动学、连续谱指数及恒星族群拟合的综合光谱目录。
...
### [5] [Discovery of a Luminosity-dependent Continuum Lag in NGC 4151 from Photometric and Spectroscopic Continuum Reverberation Mapping](https://arxiv.org/abs/2512.18276)
...
**TL;DR**: 通过对NGC 4151的多波段反响映射观测，首次发现连续谱延迟与波长呈τ∝λ^(4/3)关系且超出薄盘理论预测6.6倍，揭示了宽线区(BLR)弥散连续谱(DC)污染的存在，并发现延迟-光度非单调关系(>3σ)，为超大质量黑洞(SMBH)质量测量和活动星系核(AGN)内结构理论提供了新证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦超大质量黑洞（SMBH）吸积过程对活动星系核（AGN）的驱动机制及其通过反馈作用影响星系演化的科学问题。

Data: 数据来源于丽江2.4米望远镜的光谱观测和Swift卫星的紫外/X射线协同监测。

Method: 采用多波段光谱反响映射（reverberation mapping）技术结合斯威夫特（Swift）紫外/X射线协同监测，测量紫外至光学波段的连续谱延迟。

Result: 超大质量黑洞(SMBH)吸积驱动活动星系核(AGN)并影响星系演化，但吸积盘结构的直接观测约束仍很稀少。
...
### [6] [Measurements of quasar proximity zones with the Lyman-$α$ forest of DESI Y1 quasars](https://arxiv.org/abs/2512.18281)
...
**TL;DR**: 基于DESI（Dark Energy Spectroscopic Instrument）对10,000多个类星体对的观测，发现类星体周围星系际介质（IGM）的增强气体聚集（密度峰值Δ∼10）主导了邻近区域的莱曼阿尔法吸收，且吸收强度与前景类星体光度无关，暗示其电离辐射可能具有高度时间变异性或各向异性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于类星体周围星系际介质（IGM）受其致密环境和过量电离辐射影响形成的"类星体邻近区"（quasar proximity zone），探讨其尺寸和各向异性如何取决于类星体晕质量、光度、年龄及辐射几何结构。

Data: 数据来源于暗能量光谱仪（DESI）第一年观测数据中的超过10,000个类星体对，其投影共动距离$r_{\perp} < 2\,h^{-1}{\rm Mpc}$，并聚焦于红移$z\sim2{\rm-}3.5$的前景类星体周围。

Method: 采用暗能量光谱仪（DESI）第一年数据中超过10,000对类星体对，通过测量横向邻近效应（transverse proximity effect）分析前景类星体对背景类星体莱曼阿尔法吸收（Lyman-alpha absorption）的影响，并基于类星体电离光度各向同性且稳定的假设推断星系际介质（IGM）的密度轮廓。

Result: 类星体（quasar）周围星系际介质（IGM）的增强气体聚集效应主导了其电离效应，导致邻近视线上的吸收更强。
...
### [7] [The evolution of obscured AGN across cosmic time -- A large quasar survey for the 2040s](https://arxiv.org/abs/2512.18421)
...
**TL;DR**: 论文提出通过多目标光谱（MOS）巡天统一观测5000万活动星系核（AGN）候选体至红移（redshift）z~6.5，整合多种选择标准建立基于光谱和光变的统一框架以研究不同遮蔽类型活动星系核的完整人口统计特征。


<details>
  <summary>Details</summary>

Background: 研究主题为通过多目标光谱（MOS）巡天统一不同遮蔽活动星系核（AGN）的观测框架，突破传统分类方法的局限性。

Data: 数据来源于针对超过5000万个活动星系核(AGN)候选体的光学多目标光谱(MOS)巡天观测，包括重复观测，覆盖了光学波段可探测的最高红移(z~6.5)。

Method: 采用多目标光谱（MOS）技术结合多种活动星系核（AGN）选择标准，在变光和光谱统一框架下进行大样本统计分析。

Result: 提出了一种基于多种选择标准的光学多目标光谱（MOS）巡天方案，旨在通过重复观测统一不同遮蔽类活动星系核（AGN）的多样性，建立基于光谱和光变的统一框架。
...
### [8] [The Extended Baryonic Tully-Fisher Relation for MaNGA Galaxies](https://arxiv.org/abs/2512.18577)
*Nitya Ravi,Kelly A. Douglass,Regina Demina*

**TL;DR**: 基于5743个SDSS MaNGA DR17螺旋星系和椭圆星系样本，构建了重子塔尔-费希尔关系(BTFR)，测得斜率为3.54 (+0.65/-0.48)，与重子质量大于10^9太阳质量的TNG100模拟星系斜率3.57 (+0.48/-0.37)一致，在高质量区间符合Lambda CDM和MOND理论预测。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过重子塔尔-费希尔关系（BTFR）探究螺旋星系旋转速度与重子质量的关系，以检验Lambda CDM（冷暗物质）模型与修正牛顿动力学（MOND）的预测一致性。

Data: 数据来源包括5743个SDSS MaNGA DR17螺旋星系（H-alpha运动学数据建模旋转曲线）和椭圆星系（恒星速度弥散度估算总质量），以及IllustrisTNG 100-1 (TNG100)模拟星系数据。

Method: 采用H-alpha运动学(H-alpha kinematics)建模螺旋星系旋转曲线，并基于维里定理(virial theorem)估算椭圆星系总质量以定义等效旋转速度(effective rotational velocity)，结合恒星、HI、H2和He质量计算重子质量(baryonic mass)，构建并拟合重子塔尔-费希尔关系(BTFR)。

Result: BTFR（重子塔尔-费希尔关系）斜率在3.54（+0.65/-0.48）范围内，与重子质量大于10^9太阳质量的TNG100星系（斜率3.57（+0.48/-0.37））吻合，且在Lambda CDM模拟和MOND理论预测范围内。
...
### [9] [Linear Analyses of Thermal Instability in Stratified Medium](https://arxiv.org/abs/2512.18624)
*Izumi Seno,Shu-ichiro Inutsuka,Jiro Shimoda*

**TL;DR**: 研究发现引力分层介质（CGM）中的热不稳定性可驱动过稳定模式并产生两种不稳定模式，据此估算的星系盘气体吸积率与观测恒星形成率相当，为理解多相气体（特别是CGM）形成提供了理论框架。


<details>
  <summary>Details</summary>

Background: 研究探讨了引力分层介质（如星系周介质，CGM）中热不稳定性（thermal instability）对冷云（如高速云）形成的动力学影响及其在维持星系盘质量中的作用。

Data: data来源为线性扰动分析（linear perturbation analyses）和简化CGM模型（simplified model of the CGM）。

Method: 采用线性扰动分析（linear perturbation analysis）研究引力对热不稳定性（thermal instability）在分层介质（stratified medium）中行为的影响。

Result: 在引力分层介质（如星系周介质）中，热不稳定性可驱动过稳定模式（over-stable modes），并与其他不稳定模式共同影响气体吸积率和高速云（high-velocity clouds）的典型尺寸。
...
### [10] [Towards Understanding the Milky Way's Matter Field and Dynamical Accretion History based on AI-GS3 Hunter](https://arxiv.org/abs/2512.18693)
...
**TL;DR**: 提出了一种结合孪生神经网络(Siamese Neural Networks)和K均值聚类(K-means)的深度学习方法GS3 Hunter，通过分析盖亚(Gaia)和GALAH等星体运动学数据，不仅验证了已知星流结构（如Thamnos、Helmi等），还首次发现银河系(GSE)由四个独立成分组成，揭示了多事件吸积起源，为研究银河系晕的组装和潮汐历史提供了新工具。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用深度学习技术（GS3 Hunter）探测银河系（Milky Way）晕（halo）中的动力学子结构（substructures）和星流（streams），以揭示其吸积（accretion）历史和潮汐（tidal）演化过程。

Data: 数据来源包括盖亚卫星早期第三版数据发布(Gaia EDR3)、银河考古学与赫米斯高分辨率光谱巡天第三版数据发布(GALAH DR3)、暗能量光谱仪数据集(DESI)以及郭守敬望远镜K巨星样本(LAMOST K-giants)，并通过FIRE数值模拟进行验证。

Method: 采用孪生神经网络(Siamese Neural Networks)和K均值聚类(K-means clustering)结合的深度学习方法来识别恒星运动学数据中的子结构和星流。

Result: GS3 Hunter通过结合孪生神经网络(Siamese Neural Networks)和K均值聚类(K-means clustering)，在盖亚(Gaia)和GALAH数据中识别出已知星群（如Thamnos、Helmi、GSE、Sequoia），并在DESI数据中发现GSE由四个独立组分（GSH-GSH1至GSE-GSH4）构成，表明其为多次吸积事件形成。
...
### [11] [Towards Unveiling the Origins of the Milky Way Bulge through Multi-band-Messenger Sky Surveys](https://arxiv.org/abs/2512.18708)
*Hai-Feng Wang,Xiao Han,Giovanni Carraro,Martin Lopez-Corredoira,Yuan-Sen Ting,Guan-Yu Wang*

**TL;DR**: 通过分析OGLE-IV的RRab变星（RR Lyrae stars）及APOGEE和Gaia的巨星数据，发现银河系核球（Galactic bulge）存在中央核球、内核球和晕/盘污染物三个动力学群体，其中内核球具有棒状运动学特征且化学双峰分布，支持其通过盘不稳定性形成的伪核球起源，整体形态更接近盒状而非X形。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系核球（Galactic bulge）的结构与化学动力学特性，结合RRab变星（RR Lyrae stars）和巨星（giant stars）的多源数据，探索其形成机制与演化历史。

Data: data部分：研究使用了OGLE-IV的ab型天琴RR变星（RRabs）、APOGEE和Gaia的巨星数据，包括1,879颗RRab变量的轨道积分和28,188颗恒星的APOGEE化学动力学数据。

Method: 采用轨道积分(orbital integration)分析RRab变星的动力学特性，并结合APOGEE和盖亚(Gaia)数据验证化学动力学趋势。

Result: inner bulge RRab变星呈现棒状运动学特征，而central bulge恒星旋转较慢且弥散度较低。
...
### [12] [Spider-Webb: Spatially-Resolved Evidence of Inside-Out Quenching in the Spiderweb Protocluster at $z \sim 2$](https://arxiv.org/abs/2512.18805)
...
**TL;DR**: 通过JWST和HST对Spiderweb原星系团（z~2.16）的空间分辨分析，首次观测到大质量静止星系（log(M*/M⊙)≥10.5）呈现由内向外的质量依赖性淬灭（quenching），中心sSFR比外围低约一个量级，且中心恒星形成活动与Sérsic指数呈负相关，表明AGN反馈机制在z~2时已有效运作。


<details>
  <summary>Details</summary>

Background: 研究聚焦于红移（redshift）$z \sim 2.16$的Spiderweb原星系团（protocluster）内星系淬灭（quenching）的空间分辨特征及其与活动星系核（AGN）反馈机制的关联。

Data: 数据来源于詹姆斯·韦伯太空望远镜(JWST)和哈勃太空望远镜(HST)的深度成像观测。

Method: 采用逐像素光谱能量分布拟合（pixel-by-pixel spectral energy distribution fitting）方法，生成恒星质量、恒星形成率（SFR）、比恒星形成率（sSFR）及静止系UVJ颜色的空间分布图。

Result: 在蜘蛛网原星系团（Spiderweb Protocluster）中，质量较大的静止星系（$\log(M_*/M_{\odot}) \geq 10.5$）表现出由内向外的质量依赖性抑制（inside-out quenching），其中心区域的特定恒星形成率（sSFR）比外围区域低约一个数量级。
...
### [13] [Orbital classification in rotating bar potentials using an empirical proxy of the second integral of motion](https://arxiv.org/abs/2512.18870)
*Tian-ye Xia,Juntai Shen,John Magorrian,Yu-jing Qin*

**TL;DR**: 提出了一种基于校准角动量(CAM)和均方根半径($R_{RMS}$)参数空间的新方法，用于在旋转棒势场中对二维轨道进行分类，该方法能清晰区分不同轨道族并验证了与解析模型和数值模拟的一致性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于旋转棒势场中二维轨道分类的新方法，基于经验性第二运动积分（校准角动量，CAM）与轨道动力学特性的内在关联。

Data: 数据来源于测试粒子模拟（test-particle simulations）和代表性旋转棒势场（rotating bar potentials）中的轨道样本。

Method: 基于经验性第二运动积分代理量——标定角动量(CAM)，构建CAM与均方根半径($R_{RMS}$)的参数空间，通过分析轨道在CAM-$R_{RMS}$平面的分布模式实现分类。

Result: 基于校准角动量(CAM)与均方根半径(R_{RMS})构建的新参数空间能有效区分旋转棒势场中的不同轨道族，并验证了该方法在三维N体模拟中的适用性。
...
### [14] [Validating the CROCODILE model within the AGORA galaxy simulation framework](https://arxiv.org/abs/2512.18945)
*Pablo Granizo,Yuri Oku,Kentaro Nagamine*

**TL;DR**: 通过对比GADGET4-OSAKA与前代代码发现，机械动量注入（mechanical momentum injection）抑制气体碎片化并调控恒星形成，而随机热加热（stochastic thermal heating）驱动形成多相环星系介质（multiphase circumgalactic medium），最终校准的模拟符合AGORA合作组设定的恒星质量增长目标。


<details>
  <summary>Details</summary>

Background: 星系形成数值模拟（numerical galaxy formation simulations）的代码比较研究对于量化数值方法和亚网格物理模型（sub-grid physics models）的不确定性至关重要。

Data: 研究中使用的数据来源于孤立盘星系和宇宙学放大模拟（cosmological zoom-in run）的数值模拟结果，遵循AGORA合作项目的多步骤校准流程。

Method: 采用数值模拟方法（numerical simulation）结合更新的恒星反馈模型（stellar feedback model），包括机械动量注入（mechanical momentum injection）和随机热加热（stochastic thermal heating），并遵循AGORA多步骤校准流程（multi-step AGORA calibration procedure）。

Result: 机械动量注入（mechanical momentum injection）可抑制非物理气体碎裂并调节恒星形成，符合肯尼卡特-施密特关系（Kennicutt-Schmidt relation）；随机热加热（stochastic thermal heating）则对驱动富含金属的热气体晕及形成多相环星系介质（multiphase circumgalactic medium）至关重要。
...
### [15] [Effects of helium sedimentation on late star formation in galaxy clusters](https://arxiv.org/abs/2512.06223)
*J. Racker,N. Padilla*

**TL;DR**: 研究发现氦元素沉积(helium sedimentation)通过增加星系团中心气体密度和改变冷却速率显著影响中心星系恒星形成率，其效应在晕质量(halo mass)超过$10^{15} M_{\rm sun}$的星系团中尤为显著。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系团中氦元素沉降(helium sedimentation)如何通过改变气体密度和核子-电子分布影响中心星系恒星形成历史。

Data: 采用一维数值模拟（1-dimensional numerical simulation）研究氦沉积（helium sedimentation）过程，并将其应用于简单的半解析恒星形成模型（semi-analytical model of star formation）。

Method: 采用一维数值模拟（1-dimensional numerical simulation）结合半解析恒星形成模型（semi-analytical model of star formation）量化氦沉降（helium sedimentation）对星系团中心恒星形成历史的影响。

Result: 氦元素沉降(helium sedimentation)对中心星系恒星形成率的影响在晕质量(halo mass) $M_{\rm halo} \lesssim 10^{14} M_{\rm sun}$ 的星系团中可忽略不计，在 $10^{14} M_{\rm sun} \lesssim M_{\rm halo} \lesssim 10^{15} M_{\rm sun}$ 范围内影响较弱（低于约2倍），而在 $M_{\rm halo} \gtrsim 10^{15} M_{\rm sun}$ 的星系团中可能显著重启中心星系恒星形成活动。
...
### [16] [Fragmenting Filaments and Evolving Cores -- Insights from Dust Polarisation Study of a filament in Northern Orion B](https://arxiv.org/abs/2512.18992)
...
**TL;DR**: 通过分析猎户座B分子云北部一个质量线密度（~80太阳质量/秒差距）超过临界磁线密度（~37太阳质量/秒差距）的1秒差距长细丝的850微米偏振尘埃辐射，发现原恒星核心的磁场强度（~39-110微高斯）高于前恒星核心（~22-61微高斯）和无星核心（~6微高斯），且原恒星核心的磁场方向与局部细丝更一致，表明核心从前恒星阶段演化到原恒星阶段时磁场从弱场状态转变为强场状态。


<details>
  <summary>Details</summary>

Background: 研究聚焦于猎户座B分子云北部一个秒差距尺度长丝状结构的850微米偏振尘埃发射，探讨了星际介质中磁场与恒星形成过程的相互作用。

Data: 数据来源于JCMT（James Clerk Maxwell Telescope）SCUBA-2/POL-2偏振计对猎户座B分子云北部区域850微米偏振尘埃辐射的观测。

Method: 采用JCMT SCUBA-2/POL-2偏振计对猎户座B分子云北部区域的850微米偏振尘埃辐射进行观测，并运用Davis-Chandrasekhar-Fermi（DCF）方法计算平面天空磁场强度。

Result: 在猎户座B分子云北部的一个秒差距长细丝中，原恒星核心（protostellar cores）的磁场强度（39-110微高斯）显著高于前恒星核心（prestellar cores，22-61微高斯）和无恒星核心（starless core，6微高斯），且其磁场方向与局部细丝更一致，表明核心演化过程中磁场从弱场状态向强场状态转变。
...
### [17] [The Milky Way Bulge Extra-Tidal Star Survey: NGC 6569](https://arxiv.org/abs/2512.19074)
...
**TL;DR**: 基于光谱证据发现银河系核球（bulge）球状星团NGC 6569存在潮汐剥离（tidal stripping）产生的碎片，估算当前质量损失速率为1.0-1.6太阳质量/百万年，占星团质量的5.6±1.3%/十亿年。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系核球区（bulge）内大质量球状星团NGC 6569的潮汐剥离（tidal stripping）过程及其对核球场星（bulge field）的贡献。

Data: 数据来源为基于英澳望远镜（Anglo-Australian Telescope）的中等分辨率（R ~ 11,000）光谱观测的303颗恒星数据，目标选择结合了Blanco DECam Bulge Survey（BDBS）测光数据和盖亚（Gaia）DR3天体测量数据。

Method: 基于中等分辨率（R ~ 11,000）光谱数据，采用轨道建模（orbit-based modeling）分析潮汐半径（King-model tidal radius）和雅可比半径（Jacobi radius）随时间的变化，并结合运动学与化学丰度一致性筛选候选成员星。

Result: 基于中等分辨率光谱数据，在NGC 6569球状星团周围1-5倍潮汐半径范围内发现了40颗具有匹配运动学和丰度的潮汐剥离候选恒星，其中7颗高质量候选体平均金属丰度为[Fe/H] = -0.83 ± 0.14、α元素丰度为[α/Fe] = +0.38 ± 0.06 dex，与星团束缚成员一致。
...
### [18] [GW231123: A Case for Binary Microlensing in a Strong Lensing Field](https://arxiv.org/abs/2512.19118)
*Xikai Shan,Huan Yang,Shude Mao*

**TL;DR**: 研究发现GW231123引力波信号由嵌入强透镜星系的双透镜模型解释最佳，其推断的源帧双黑洞质量（80.0^{+21.3}_{-14.4}和62.0^{+19.8}_{-29.4}太阳质量）与自旋（0.37^{+0.51}_{-0.33}和0.40^{+0.52}_{-0.35}）与现有O1-O3观测群体一致。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于解释引力波事件GW231123中观测到的异常特性（如组分质量位于对不稳定性质量间隙内和接近克尔极限的极大自旋），挑战了标准形成机制，并探索了嵌入强透镜星系中的双透镜模型作为更合理的解释方案。

Data: 数据来源于NRSur7dq4波形模型（NRSur7dq4 waveform model）和基于Transformer的神经网络生成的透镜化波形。

Method: 采用基于Transformer的神经网络快速生成引力透镜波形，并结合NRSur7dq4波形模型进行二元透镜（binary lens）分析。

Result: GW231123的异常特性（包括处于对不稳定性质量间隙的组分质量及接近克尔极限的极大自旋）挑战了标准形成机制。
...
### [19] [ALMA Observations of Cold Methanol Gas in the Large Magellanic Cloud (LMC): N79 South GMC](https://arxiv.org/abs/2512.19185)
*Suman Kumar Mondal,Takashi Shimonishi,Soumen Mondal,Prasanta Gorai,Kei E. I. Tanaka,Kenji Furuya,Ankan Das*

**TL;DR**: 利用ALMA观测在大麦哲伦云（LMC）N79区域的超星团（SSC）候选体H72.97-69.39附近发现两个冷密分子云核（N79S-1和N79S-2），其甲醇（CH3OH）丰度与银河系类似源相当，但N79区域整体有机分子形成受到抑制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大麦哲伦云(LMC)中N79区域的超级星团(SSC)候选体H72.97-69.39周围冷分子气体的化学组成与物理特性。

Data: 数据来源于ALMA（阿塔卡马大型毫米波/亚毫米波阵列）对LMC（大麦哲伦云）N79区域中超星团候选体H72.97-69.39的连续谱和分子谱线观测，分辨率达0.1 pc。

Method: 采用ALMA（阿塔卡马大型毫米波/亚毫米波阵列）连续谱和分子谱线观测技术，结合旋转图分析法（rotation diagram analysis）测定气体温度及分子丰度。

Result: 在大麦哲伦云（LMC）N79区域的超星团候选体H72.97-69.39附近发现了两个与CH3OH发射相关的连续谱源N79S-1和N79S-2，其气体温度分别为13±0.4 K和15±0.9 K，且CH3OH丰度分别为(2.1±1.1)×10^-9和(4.5±2.5)×10^-10。
...
### [20] [Dynamically close galaxy pairs from the unWISE survey: Testing the merger-AGN-star formation connection](https://arxiv.org/abs/2512.19236)
*Josephine Chishala,Roberto De Propris,Mirjana Pović*

**TL;DR**: 研究发现恒星质量对数(log(M/Msun))约10.2和11.4、红移0.25以内的星系对样本中，仅在投影距离小于20千秒差距且速度差小于500公里/秒的极近距星系对中可能观测到微弱恒星形成率(SFR)提升，但未发现明显证据支持星系并合或相互作用会显著触发恒星形成或活动星系核(AGN)活动。


<details>
  <summary>Details</summary>

Background: 星系并合（galaxy mergers）对星系恒星形成历史和活动星系核（AGN）活动的潜在影响是理解星系演化中质量增长和反馈机制的关键理论问题。

Data: 数据来源于对红移0.25范围内不同投影距离和速度差的星系对样本（恒星质量log(M/Msun)约10.2和11.4）的观测，包括近紫外-光学颜色（NUV - r）测量恒星形成率（SFR），以及X射线、20cm射电辐射、WISE红外颜色和发射线比率检测活动星系核（AGN）活动程度。

Method: 采用星系对样本（galaxy pairs）的投影间距和速度差分析，结合恒星形成率（SFR）和活动星系核（AGN）活动的多波段观测（X射线、20厘米射电、WISE红外颜色和发射线比）测量方法。

Result: 星系并合（galaxy merger）对恒星形成率（SFR）和活动星系核（AGN）活动的触发作用较弱，仅在小于20千秒差距（kpc）和速度差小于500公里/秒（km/s）的极近距星系对中可能存在微弱影响。
...
### [21] [Galaxy And Mass Assembly (GAMA): Deconstructing the galaxy stellar mass function by star formation and environment](https://arxiv.org/abs/2512.19310)
*A. Sbaffoni,J. Liske,A. S. G. Robotham,L. J. M. Davies,S. P. Driver,E. N. Taylor*

**TL;DR**: 基于GAMA数据研究发现：低红移(redshift)星系恒星质量函数(GSMF)中，被动(passive)星系呈双谢克特(Schechter)分布而恒星形成(star-forming)星系呈单谢克特分布，中心(central)星系主导高质量端而卫星(satellite)和孤立星系主导中低质量端，且更高质量晕(halo)中被动星系比例更高但中间质量星系数量下降更快，观测到的被动与恒星形成星系差异大于唯象淬灭(phenomenological quenching)模型预测。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用GAMA数据集分析低红移星系恒星质量函数（GSMF）在不同星系群体和暗晕质量（halo mass）条件下的变化规律。

Data: 数据来源于赤道区域的星系与质量组装(GAMA)巡天数据集。

Method: 采用星系和物质组装巡天(GAMA)数据集，结合双分量谢克特函数(Schechter function)拟合方法分析不同星系群组的恒星质量函数(GSMF)分布特征。

Result: 被动星系和恒星形成星系的恒星质量函数（GSMF）分别由双Schechter函数和单Schechter函数较好描述，但恒星形成星系加入第二组分后描述更精确，且前者主导总GSMF低质量端而后者主导中高质量端。
...
### [22] [Impact of Galaxy Cluster Environment on the Stellar Mass Function of Galaxies](https://arxiv.org/abs/2512.19591)
*Sana Begum Murtuja Shaikh,Priyanka Singh*

**TL;DR**: 通过分析G9天区的多波段星系数据和eFEDS星系团目录，研究发现高质量星系团中心区域（0-0.5R500）低恒星质量（M*≲2×10¹⁰M⊙）星系的恒星质量函数（SMF）呈下降趋势，而低质量星系团SMF保持平坦，表明低质量星系在到达星系团中心前经历了质量增长过程。


<details>
  <summary>Details</summary>

Background: 研究星系团（galaxy clusters）这一宇宙极端环境对星系恒星质量函数（Stellar Mass Function, SMF）演化的影响。

Data: 数据来源于千度巡天（Kilo Degree Survey，光学波段）和VISTA千度红外星系巡天（VISTA Kilo-Degree Infrared Galaxy Survey，红外波段）的9波段测光星系数据，覆盖约370万个星系，并与eROSITA最终赤道深度巡天（eFEDS）提供的星系团目录重叠。

Method: 采用多波段测光数据（光学和红外）结合星系团中心径向分箱（radial bins）方法，分析不同环境（孤立场与星系团）对恒星质量函数（SMF）的影响，并考察红移、质量、X射线光度及椭圆度等参数的效应。

Result: 在0-0.5R500范围内，高质量星系团（cluster）的星系恒星质量函数（SMF）在低恒星质量端（M*≲2×10¹⁰M⊙）呈现下降趋势，而低质量星系团的SMF保持平坦，表明低质量星系在到达团中心前经历了增长过程。
...
### [23] [Deep Hα survey of the Coma cluster: The Catalog](https://arxiv.org/abs/2512.19626)
*Sarah E. Kay,Ehsan Kourkchi,A. Molaeinezhad,H. G. Khosroshahi,M. Mouhcine,P. A. James,D. Carter*

**TL;DR**: 利用深度窄带成像首次在室女座星系团（Coma cluster）不同环境区域中可靠测量了包括矮星系在内的124个Hα发射源（其中96个为新发现）的Hα+[NII]流量和等效宽度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用窄带成像技术对本地富集且动力学松弛的后发座星系团(Coma cluster)进行深度宽视场观测，以探索不同环境条件下星系电离氢区(Hα)发射特性。

Data: 数据来源于艾萨克牛顿望远镜（Isaac Newton Telescope）宽视场相机（Wide Field Camera）对后发座星系团（Coma cluster）进行的深度宽视场窄带成像巡天观测，覆盖约2.5平方度区域，使用R波段（6380Å）和[SII]（6725Å）滤光片测量Hα+[NII]流量及等效宽度。

Method: 采用宽视场相机（Wide Field Camera）的$R$波段（6380Å）和[$S\scriptstyle\rm II$]波段（6725Å）窄带成像技术，结合光谱学成员确认，精确测量了后发座星系团（Coma cluster）中星系的H$α$+[N{\sc ii}]流量及等效宽度。

Result: 首次在英仙座(Coma)星系团大范围环境中可靠测量了包括矮星系在内的124个Hα发射源（其中96个为新发现）的Hα+[N II]流量和等效宽度。
...
### [24] [Ionizing Photon Production Efficiencies and Chemical Abundances at Cosmic Dawn Revealed by Ultra-Deep Rest-Frame Optical Spectroscopy of JADES-GS-z14-0](https://arxiv.org/abs/2512.19695)
...
**TL;DR**: 通过JWST/MIRI对高红移(z>14)星系JADES-GS-z14-0的最深光谱观测，首次精确测定了其低尘埃衰减、高恒星形成率(10±2 M⊙/yr)及亚太阳级金属丰度([O/H]≈-0.3)等物理参数，挑战了现有宇宙早期星系形成理论。


<details>
  <summary>Details</summary>

Background: 研究探讨了詹姆斯·韦伯太空望远镜（JWST）发现的早期星系形成活动超出预期的现象，挑战了对早期宇宙演化的现有认知。

Data: 使用JWST/MIRI获取的迄今最长光谱积分数据，覆盖了高红移星系JADES-GS-z14-0（z>14）最亮的静止光学波段星云发射线。

Method: 采用JWST/MIRI进行超深光谱观测，结合标准强线诊断（standard strong-line diagnostics）和详细光电离模型（detailed photoionization modeling）分析[OIII]λλ4959,5007、Hα等发射线。

Result: 在红移（redshift）z > 14的明亮星系JADES-GS-z14-0中，通过JWST/MIRI的超深光谱观测检测到显著的电离氧（[OIII]）和氢阿尔法（Hα）发射线，揭示了其低尘埃衰减、近期恒星形成率（SFR）约为10±2 M⊙/年、高恒星形成率面密度（ΣSFR）和高效电离光子产生效率（ξion）等特性。
...
### [25] [The Clustering of Little Red Dots from Ultra-Strongly Self-Interacting Dark Matter](https://arxiv.org/abs/2512.18000)
*M. Grant Roberts,Aarna Garg,Tesla Jeltema,Stefano Profumo*

**TL;DR**: 研究发现超强自相互作用暗物质（uSIDM）种子的红矮点（LRDs）在红移5处的有效成团偏置参数（$b_{\rm{eff}}$）约为4.5，对应典型晕质量$\sim 8\times10^{10}~M_{\odot}$，表明LRDs与高红移类星体不同且成团性对uSIDM微观物理参数不敏感。


<details>
  <summary>Details</summary>

Background: 研究探讨了超强自相互作用暗物质（uSIDM）模型下小红点（LRDs）在红移z∼5处的有效成团偏置参数（beff）及其与暗物质微观物理参数无关的稳健性。

Data: 研究使用基于超强自相互作用暗物质（uSIDM）模型的理论模拟数据预测小红点（LRDs）的有效聚类偏置参数（$b_{\rm{eff}}$）。

Method: 采用超强自相互作用暗物质(uSIDM)模型预测小红点(LRDs)的有效成团偏置参数$b_{\rm{eff}}$，并通过理论计算得出该参数与暗物质微观物理参数(如截面$\sigma/m$和比例$f$)无关的稳健结论。

Result: result：超强自相互作用暗物质(uSIDM)模型预测小红点(LRDs)在红移5处的有效成团偏袒参数$b_{\rm{eff}}\sim4.5$，对应典型晕质量$\sim 8\times10^{10}~M_{\odot}$，且该参数对uSIDM微观物理参数(如截面$\sigma/m$和比例$f$)不敏感。
...
### [26] [Harmony of the Spheres: Extension to All Points of an Algorithm for Producing a Density Field with Given Two-, Three-, and Four-Point Correlation Functions](https://arxiv.org/abs/2512.18547)
*Zcahary Slepian,Alessandro Greco*

**TL;DR**: 该论文改进了Smith-Zaldarriaga算法，使其能在三维网格上生成具有任意指定N点相关函数(NPCFs)的密度场，且这些统计特性在网格所有点处均成立，从而将"约束实现"(constrained realization)方法推广至高阶统计量。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于扩展宇宙微波背景（CMB）谐波空间双谱算法至三维密度场，实现任意N点相关函数（NPCFs）的精确模拟，并完善高阶统计量约束实现（constrained realization）的理论框架。

Data: 研究中使用的数据来源于模拟生成的三维密度场（3D density field），该密度场通过算法在网格上实现，并确保网格上每一点的非线性点相关函数（NPCFs）符合预设要求。

Method: 采用广义化的Smith-Zaldarriaga算法生成具有任意指定N点相关函数（NPCFs）的三维密度场，并确保网格上每一点均满足预设统计特性。

Result: 该算法可生成在网格上每一点均具有正确、期望的N点相关函数（NPCFs）的三维密度场。
...
### [27] [Nanohertz Gravitational Waves](https://arxiv.org/abs/2512.18822)
*Alberto Sesana,Daniel G. Figueroa*

**TL;DR**: 这篇论文的核心贡献是通过脉冲星计时阵列（PTA）探测到纳赫兹引力波（GW）信号，研究了其可能的天体物理（如超大质量黑洞双星MBHBs）和宇宙学起源（如暴胀、一级相变或拓扑缺陷），并探讨了该发现对高能物理、早期宇宙状态方程、暗物质性质等领域的深远影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于纳赫兹引力波（GW）信号的物理机制及其对天体物理学和粒子物理学的深远影响，包括探索来自大质量黑洞双星（MBHBs）的天体物理背景和早期宇宙（inflation scenarios、first order phase transitions、topological defects等）的宇宙学起源。

Data: 数据来源为脉冲星计时阵列（PTA）观测数据。

Method: 采用脉冲星计时阵（PTA）数据分析技术，结合天体物理（如大质量黑洞双星MBHBs）和宇宙学（如暴胀、一级相变、拓扑缺陷）引力波背景（GWB）模型，通过环境耦合、轨道偏心率及稀疏采样等参数约束信号谱与统计特性。

Result: 脉冲星计时阵列（PTA）数据中出现的引力波（GW）信号为探索纳赫兹引力波宇宙提供了新途径，其潜在来源包括天体物理（如大质量黑洞双星并合）和宇宙学（如暴胀、一级相变或拓扑缺陷）过程，并对高能物理、暗物质性质及早期宇宙状态方程等研究具有深远意义。
...
### [28] [Constraints on the polarization angle oscillations of the Crab Nebula with the Simons Array and its applications to the search for axion-like particles](https://arxiv.org/abs/2512.18882)
...
**TL;DR**: 利用西蒙斯阵列(Simons Array)在毫米波段观测蟹状星云(Tau A)的偏振振荡，未检测到全局信号并给出偏振振荡振幅上限A<0.12°，同时将轴子类粒子(ALPs)与光子耦合常数约束至gaγγ<3.84×10−12×(ma/10−21 eV)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用毫米波偏振观测验证蟹状星云(Tau A)偏振角稳定性并探索轴子类粒子(ALPs)存在的可能性。

Data: 数据来源于西蒙斯阵列(Simons Array)在2023年观测季获取的90 GHz波段观测数据。

Method: 采用时间分辨偏振测量(time-resolved polarimetry)技术分析蟹状星云(Tau A)的偏振角振荡，并结合轴子类粒子(ALPs)模型约束其与光子的耦合强度。

Result: 在蟹状星云(Tau A)毫米波偏振振荡的搜索中，未检测到全局信号，并给出了偏振振荡振幅的中位数95%上限A<0.12°（振荡频率范围3.39年⁻¹至1.50天⁻¹），同时对轴子类粒子(ALP)与光子耦合常数gaγγ设定了上限约束。
...
### [29] [Weak Lensing Mass Calibration of the ACT DR5 Galaxy Clusters with the DES Year 3 Weak Lensing Data](https://arxiv.org/abs/2512.18935)
...
**TL;DR**: 利用弱引力透镜效应（weak gravitational lensing）校准443个由苏尼亚耶夫-泽尔多维奇效应（Sunyaev-Zel'dovich effect）选出的星系团质量，测得SZ信号估算质量与真实质量间的偏差（hydrostatic mass bias）为1-b=0.75^{+0.04}_{-0.06}，并发现该偏差随红移增加而减小（幂律指数-2.0^{+0.7}_{-0.4}）。


<details>
  <summary>Details</summary>

Background: 利用弱引力透镜效应校准星系团质量以研究宇宙学参数和天体物理过程。

Data: 数据来源包括暗能量巡天（Dark Energy Survey）第三年数据的弱引力透镜（weak gravitational lensing）测量结果，以及阿塔卡马宇宙学望远镜（Atacama Cosmology Telescope）第五次数据发布的宇宙微波背景（cosmic microwave background）图中通过苏尼亚耶夫-泽尔多维奇效应（Sunyaev-Zel'dovich effect）选出的443个星系团的红移和SZ测量数据。

Method: 采用弱引力透镜（weak gravitational lensing）测量结合分层模型（hierarchical model）进行贝叶斯分析（Bayesian analyses），以约束星系团的流体静力学质量偏差（hydrostatic mass bias），并系统处理弱透镜增强因子（weak lensing boost factor）、中心偏移效应（miscentering effect）、源星系光度红移分布（photometric redshift distributions）和剪切校准（shear calibration）的不确定性。

Result: 利用弱引力透镜(weak gravitational lensing)测量校准了443个通过苏尼亚耶夫-泽尔多维奇效应(Sunyaev-Zel'dovich effect)选出的星系团质量，得出平均弱透镜晕质量为$M_{\rm 500c} = 5.4 \times 10^{14} M_{\odot}$，并测得基于X射线标定的流体静力学平衡假设下SZ信号估算质量与真实质量间的偏差为$1-b = 0.75^{+0.04}_{-0.06}$。
...
### [30] [$Ω_1Ω_2$-$Λ$CDM: A promising natural extension of the standard model of cosmology](https://arxiv.org/abs/2512.19000)
*Suresh Kumar*

**TL;DR**: 提出并验证了$Ω_1Ω_2$-$Λ$CDM模型，通过引入两个可观测的暗能量组分$Ω_1(1+z)$和$Ω_2(1+z)^2$，有效缓解了哈勃常数（Hubble tension）张力，使普朗克（Planck）数据得出的$H_0$与SH0ES测量值一致，同时保持早期宇宙物理不变。


<details>
  <summary>Details</summary>

Background: 研究探讨了标准ΛCDM模型的自然扩展——Ω₁Ω₂-ΛCDM模型，通过引入两个可观测检验的暗能量组分Ω₁(1+z)和Ω₂(1+z)²来参数化宇宙总能量密度，旨在缓解哈勃张力并探索晚期宇宙动力学对暗能量状态方程的影响。

Data: 数据来源为普朗克宇宙微波背景辐射（Planck CMB）和暗能量光谱仪重子声波振荡（DESI BAO）观测数据。

Method: 采用扩展的$Ω_1Ω_2$-$Λ$CDM模型，在标准$Λ$CDM基础上引入两个可观测的暗能量组分$Ω_1(1+z)$和$Ω_2(1+z)^2$，通过普朗克(Planck)宇宙微波背景辐射(CMB)和DESI重子声波振荡(BAO)数据约束模型参数。

Result: 该模型通过引入两个可观测的暗能量贡献项（Ω₁(1+z)和Ω₂(1+z)²）扩展了标准ΛCDM框架，能有效缓解哈勃张力（Hubble tension），使普朗克CMB数据得出的H₀=75.4^{+3.9}_{-2.3} km s⁻¹ Mpc⁻¹与SH0ES距离阶梯测量结果一致。
...
### [31] [DESI DR2 Constraints on the $R_h=ct$ Universe: Model Viability and Comparison with $Λ$CDM](https://arxiv.org/abs/2512.19175)
*Amritansh Mehrotra,S. K. J. Pacif,A. F. Santos*

**TL;DR**: 基于宇宙计时器、Ia型超新星和重子声学振荡数据对比分析ΛCDM和Rh=ct模型，发现ΛCDM在统计指标（AIC/BIC/贝叶斯因子）和哈勃参数H(z)、减速因子q(z)演化方面表现更优，其预测的宇宙年龄与普朗克2018结果一致，而Rh=ct模型因线性膨胀导致宇宙年龄偏大。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过宇宙计时器(cosmic chronometers)、Ia型超新星(Type Ia supernova)和重子声学振荡(baryon acoustic oscillations)数据，对比标准ΛCDM宇宙学模型与替代性Rh=ct框架在描述宇宙膨胀历史方面的表现差异。

Data: 数据来源于宇宙计时器（cosmic chronometers）、Ia型超新星（Type Ia supernova）和重子声学振荡（baryon acoustic oscillations）的最新观测数据。

Method: 采用基于卡方统计量（chi-squared statistics）、赤池信息量准则（AIC）、贝叶斯信息准则（BIC）和贝叶斯因子（Bayes factor）的联合统计评估方法比较ΛCDM模型和Rh=ct框架对宇宙膨胀历史的拟合能力。

Result: ΛCDM模型在统计评估中表现更优，其预测的宇宙年龄与普朗克2018年结果一致，而Rh=ct框架则预测了更老的线性膨胀宇宙年龄。
...
### [32] [Illuminating the Dark Sector: Understanding Modified Gravity Signatures with Cross-Correlations of Gravitational Waves and Large-Scale Structure](https://arxiv.org/abs/2512.19186)
*Chiara De Leo,Guadalupe Cañas-Herrera,Anna Balaudo,Matteo Martinelli,Alessandra Silvestri,Tessa Baker*

**TL;DR**: 结合大尺度结构（LSS）观测与引力波（GW）事件的交叉关联信号，显著增强了对广义相对论偏离的约束能力，为多信使宇宙学开辟了新途径。


<details>
  <summary>Details</summary>

Background: 该研究探讨了利用大尺度结构（LSS）观测与引力波（GW）事件的协同效应来检验修正引力理论（modified gravity）。

Data: 数据来源包括欧几里得(Euclid)等第四阶段大尺度结构(LSS)巡天观测数据以及爱因斯坦望远镜(Einstein Telescope)未来的引力波(GW)探测数据。

Method: 采用大尺度结构（LSS）与引力波（GW）互相关信号（cross-correlation）的合成预测方法（synthetic forecast methodology）分析修正引力（modified gravity）场景。

Result: 结合大尺度结构（LSS）和引力波（GW）的交叉关联信号能显著增强对广义相对论偏离的约束，为多信使宇宙学开辟新窗口。
...
### [33] [Dynamics of clusters of galaxies in the presence of dark energy: Virgo cluster](https://arxiv.org/abs/2512.19436)
*H. A. Harutyunian,E. H. Nikogosyan*

**TL;DR**: 研究发现重子物质(baryonic matter)与暗能量载体相互作用导致所有重子结构获得能量并必然膨胀，通过分析室女座星系团(Virgo galaxy cluster)子结构证实星系平均星等与速度呈正相关，为星系团膨胀提供了证据。


<details>
  <summary>Details</summary>

Background: 研究探讨了重子物质（baryonic matter）与暗能量（dark energy）载体的相互作用导致重子物质获得能量并必然膨胀的机制，这一机制适用于重子宇宙（baryonic universe）的所有层级结构。

Data: 数据来源于对室女座星系团(Virgo galaxy cluster)的结构特征及其子结构的观测分析。

Method: 采用简单统计方法识别星系团内星系的径向运动，并通过分析室女座星系团(Virgo galaxy cluster)的子结构特征验证该方法。

Result: 暗能量载体与重子物质(baryonic matter)的相互作用导致所有重子物质构型获得能量并必然发生膨胀。
...
### [34] [Revealing the intricacies of radio galaxies and filaments in the merging galaxy cluster Abell 2255. II. Properties of filaments using multi-frequency radio data](https://arxiv.org/abs/2512.19471)
...
**TL;DR**: 结合LOFAR-VLBI、uGMRT和VLA数据首次在阿贝尔2255星系团（Abell 2255）中发现延伸250千秒差距(kpc)、谱指数(α)大于2的超陡谱细丝结构，并揭示原始射电星系(Original TRG)是驱动这些细丝形成的主要机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用多波段射电观测（LOFAR、uGMRT、VLA）解析Abell 2255星系团内射电纤维丝（filaments）的形态学特征与物理起源，特别关注其与原始射电星系（Original TRG）的动力学关联及宇宙线成分的分离。

Data: 数据来源于LOFAR（56小时观测生成的1.5角秒分辨率VLBI宽场图像）、uGMRT（1260 MHz）和VLA（1520 MHz）的校准成像数据，包括频谱指数图、偏振信息（通过旋转测量合成技术获取）以及辐射年龄和等分磁场的附加信息。

Method: 采用低频阵列甚长基线干涉测量（LOFAR-VLBI）宽场成像技术结合升级巨米波射电望远镜（uGMRT）和甚大天线阵（VLA）数据校准与成像，生成光谱指数图并应用旋转测量合成技术获取偏振信息。

Result: 在阿贝尔2255（Abell 2255）中发现了光谱指数（α）大于2的额外陡峭丝状结构，延伸约250千秒差距（kpc），并确认原始射电星系（Original TRG）是这些结构的主要驱动源。
...
### [35] [Low-redshift 3D Lyman-α Forest Correlations with China Space Station Telescope](https://arxiv.org/abs/2512.19474)
*Ting Tan,Huanyuan Shan,Eric Armengaud*

**TL;DR**: 利用中国空间站望远镜(CSST)的紫外无缝光谱仪首次测量了红移1.1-2.0范围内莱曼α森林(Lyα forest)的三维自相关函数及其与类星体(QSO)和发射线星系(ELG)的互相关函数，预计能以10-30%精度测定Lyα偏差参数，并通过相关函数组合实现2.5σ(3.7σ)显著性的重子声波振荡(BAO)特征探测。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用中国空间站望远镜(CSST)的紫外无缝光谱仪首次测量红移$z < 2$下莱曼阿尔法(Ly$α$)森林的三维(3D)聚类特性及其与类星体(QSO)和发射线星系(ELG)的交叉关联。

Data: 研究使用模拟数据（mock data），通过构建符合中国空间站望远镜（CSST）紫外无缝光谱仪预期观测深度、光谱分辨率和噪声特性的模拟类星体（quasar）光谱，来推导莱曼α（Lyα）自相关函数以及与类星体（QSO）和发射线星系（ELG）在红移范围1.1 < z < 2.0内的互相关函数。

Method: 采用标准各向异性红移空间聚类模型（standard anisotropic redshift-space clustering model）分析三维莱曼阿尔法（Lyα）自相关函数以及与类星体（QSO）和发射线星系（ELG）的互相关函数。

Result: 在有效红移$z_{\rm eff}=1.59$（ELG为1.58）下，莱曼阿尔法(Ly$α$)森林的偏置参数(bias parameters)将获得10-30%精度的测量，具体取决于其他示踪天体偏置的先验信息。
...
### [36] [Do low-redshift observations open the doors to an open universe?](https://arxiv.org/abs/2512.19565)
*Deng Wang,Olga Mena,Salvatore Capozziello,David Mota*

**TL;DR**: 结合DESI重子声波振荡(BAO)和超新星(SN Ia)的低红移观测数据，首次发现支持开放宇宙(open universe)的3σ以上显著证据，与普朗克卫星(Planck)高红移宇宙微波背景(CMB)测量结果形成新张力，对标准暴胀理论和最小ΛCDM模型提出挑战。


<details>
  <summary>Details</summary>

Background: 宇宙曲率（cosmic curvature）的测量对暴胀理论和基础物理学具有深远影响。

Data: 数据来源于DESI（暗能量光谱仪）的BAO（重子声波振荡）观测、SN Ia（Ia型超新星）样本以及声学尺度$θ_\star$测量值，其中SN Ia数据包括DESY5超新星样本。

Method: 采用重子声波振荡（BAO）和Ia型超新星（SN Ia）的低红移观测数据结合声学尺度（θ_\star）测量，通过宇宙曲率约束分析得出开放宇宙的显著证据。

Result: 低红移观测（DESI重子声波振荡和Ia型超新星）以超过3σ显著性支持开放宇宙（open universe），使用DESY5超新星数据时显著性达5σ。
...
### [37] [Deep Learning for Primordial $B$-mode Extraction](https://arxiv.org/abs/2512.19577)
*Eric Guzman,Joel Meyers*

**TL;DR**: 利用深度学习技术估计并去除宇宙微波背景（CMB）中引力透镜（gravitational lensing）等多种次级B模偏振信号，实现了对原初引力波（primordial gravitational waves）振幅的近乎最优无偏估计。


<details>
  <summary>Details</summary>

Background: 探测原初引力波（primordial gravitational waves）是宇宙微波背景辐射（CMB）研究的核心科学目标。

Data: 数据来源于低噪声、多频段宇宙微波背景辐射（CMB）观测。

Method: 采用深度学习网络ResUNet-CMB进行多源次级B模偏振（secondary B-mode polarization）的估计与去除。

Result: 引力透镜效应（gravitational lensing）是次级B模偏振（secondary B-mode polarization）的主要来源，而深度学习技术可近乎最优地估计并去除多种次级B模偏振源以提升对原初引力波（primordial gravitational waves）振幅的限制精度。
...
### [38] [Non-local-in-time structure formation and sixth-order galaxy bias](https://arxiv.org/abs/2512.19578)
*Alex Edison,Matthew Lewandowski,Leonardo Senatore*

**TL;DR**: 这篇论文通过构建大尺度结构有效场论（Effective Field Theory of Large-Scale Structure）中非局域时间（non-local-in-time）的星系偏置（galaxy bias）展开式，发现第六阶存在57个独立偏置项（相比局域时间情形的46个），揭示了高阶聚类统计量可区分星系形成的局域与非局域时间特性并敏感于其形成时间。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用大尺度结构有效场论（Effective Field Theory of Large-Scale Structure）系统构建非局域时间（non-local-in-time）星系偏置展开，以完整捕捉至六阶的时间非局域性效应。

Data: 研究中使用的数据来源于大尺度结构有效场论（Effective Field Theory of Large-Scale Structure）中的非局域时间星系偏置展开的模拟（simulation）数据。

Method: 采用有效场论（Effective Field Theory of Large-Scale Structure）框架下的非局域时间（non-local-in-time）星系偏置展开方法，构建了包含57个独立偏置算符的六阶完整基组。

Result: 在有效场论的大尺度结构（Effective Field Theory of Large-Scale Structure）中，非局域时间星系偏置展开（non-local-in-time galaxy bias expansion）的第六阶完整基包含57个独立偏置项，比局域时间展开的46个多11个。
...
### [39] [InterACTing dark radiation models after ACT](https://arxiv.org/abs/2512.19633)
*William Cvetko,Melissa Joseph,Gustavo Marques-Tavares*

**TL;DR**: 通过分析ACT DR6数据，发现两种相互作用暗辐射（WZDR和nuADaM）模型参数空间大幅收紧，基本排除了其作为哈勃常数（H_0）张力解决方案的可能性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过阿塔卡马宇宙学望远镜(ACT)DR6数据评估两种相互作用暗辐射(dark radiation)模型对缓解哈勃张力(Hubble tension)的影响。

Data: 数据来源于阿塔卡马宇宙学望远镜第六次数据发布（ACT DR6）的宇宙微波背景（CMB）测量结果，特别是高多极矩（high-$\ell$）阻尼尾区域的观测数据。

Method: 采用核心分析方法包括评估Wess-Zumino暗辐射（WZDR）模型的暗区质量阈值效应和新型原子暗物质（nuADaM）模型的暗辐射耦合机制，并结合ACT DR6数据对高$\ell$阻尼尾区进行精确约束。

Result: ACT DR6显著收紧了对两种相互作用暗辐射（interacting dark radiation）模型的限制，消除了先前允许更高哈勃常数（H_0）值的大部分参数空间。
...
### [40] [A cosmologist's take on Little Red Dots](https://arxiv.org/abs/2512.19666)
*Valerio De Luca,Loris Del Grosso,Gabriele Franciolini,Konstantinos Kritos,Emanuele Berti,Daniel D'Orazio,Joseph Silk*

**TL;DR**: 詹姆斯·韦伯太空望远镜（JWST）发现的"小红点"（LRDs）可能包含超大质量黑洞（BHs），其质量远超本地比例关系预测值，研究通过排除原始黑洞（PBHs）直接形成和并合组装的可能性，提出气体吸积中等质量原始黑洞可能是解释其观测特性的可行机制。


<details>
  <summary>Details</summary>

Background: 研究探讨了詹姆斯·韦伯太空望远镜（JWST）发现的致密高红移天体（LRDs）中超大质量黑洞（BHs）可能具有原初起源（primordial origin）的假说，挑战了早期星系形成的标准模型。

Data: 数据来源为詹姆斯·韦伯太空望远镜（JWST）观测到的高红移致密天体（Little Red Dots, LRDs）以及强引力透镜源QSO1的观测数据。

Method: 采用原初黑洞（PBHs）的等级合并模型和气体吸积模型，结合金属丰度演化追踪，检验早期超大质量黑洞（SMBHs）的形成机制。

Result: LRDs（Little Red Dots）中观测到的超大质量黑洞（supermassive BHs）可能源于轻质量原初黑洞（PBHs）通过等级合并或气体吸积演化而来，但受限于早期宇宙的暗物质晕稀有性和CMB μ畸变限制。
...
### [41] [Are Primordial Black Holes Truly Fine-Tuned?](https://arxiv.org/abs/2512.19668)
*A. J. Iovino,A. Riotto*

**TL;DR**: 研究发现采用威尔逊自然性准则（Wilson's naturalness criterion）量化精细调节程度时，原初黑洞（primordial black hole）生成模型在技术上并非不自然。


<details>
  <summary>Details</summary>

Background: 研究探讨了单场暴胀模型（single-field inflationary models）中通过小尺度曲率原初功率谱增强产生原初黑洞（primordial black holes）时参数微调（fine-tuning）问题的合理性及其量化标准。

Data: 研究中采用威尔逊自然性准则（Wilson's naturalness criterion）的修正版本量化精细调节（fine-tuning）和自然性，并证明原初黑洞（primordial black hole）模型在技术上并非不自然。

Method: 采用威尔逊自然性准则（Wilson's naturalness criterion）的修正版本量化精细调节和自然性。

Result: 通过采用改进版威尔逊自然性准则(Wilson's naturalness criterion)量化精细调节和自然性，发现原初黑洞(PBH)模型在技术上并非不自然。
...
### [42] [CoBiTS: Single-detector discrimination of binary black hole signals from glitches using deep learning](https://arxiv.org/abs/2512.17975)
*Matthew VanDyke,Kexuan Wu,Sukanta Bose*

**TL;DR**: 开发了基于Conformer神经网络（结合卷积层和Transformer架构）的CoBiTS模型，能有效区分LIGO-Virgo-KAGRA探测器数据中的双黑洞引力波信号与瞬态噪声伪迹（glitches），尤其擅长在高质量双黑洞信号与突发噪声或散射光噪声共存时进行准确鉴别。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用神经网络模型（Conformer Binary neTwork Search, CoBiTS）从LIGO-Virgo-KAGRA探测器数据中区分双黑洞（BBH）引力波（GW）信号与非高斯非平稳噪声伪迹（glitches）。

Data: 数据来源于当前一代LIGO-Virgo-KAGRA探测器的单探测器应变时间序列片段（single-detector strain timeseries snippets），这些片段由其他搜索流程标记为包含引力波候选信号。

Method: 采用Conformer神经网络（结合卷积层和自注意力层的混合架构）分析单探测器应变时间序列片段，以区分双黑洞引力波信号与瞬态噪声伪影。

Result: CoBiTS（Conformer Binary neTwork Search）能有效区分双黑洞（BBH）引力波信号与LIGO-Virgo-KAGRA探测器数据中的非高斯非平稳噪声伪影（glitches），尤其擅长从高频BBH信号中识别blips和散射光伪影。
...
### [43] [Bridging stellar evolution and planet formation: from birth, to survivors of the fittest, to the second generation of planets](https://arxiv.org/abs/2512.17976)
...
**TL;DR**: 该论文通过研究恒星从诞生到死亡过程中尘埃运动学（dust kinematics）和宏观结构形成及其对行星-宿主星相互作用的影响，首次系统性桥接了恒星演化与行星形成领域，提出未来需发展成像技术以解析行星生命周期中多样尘埃环境，从而全面验证从尘埃微观组装到行星-宿主星宏观互动的理论框架。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星与行星协同演化过程中尘埃动力学（dust kinematics）和宏观结构形成对行星-宿主星相互作用的跨尺度影响，涵盖从恒星诞生到死亡的各类尘埃环境。

Data: 研究中使用的数据来源包括对原行星盘（protoplanetary discs）、红巨星分支（RGB）或渐近巨星分支（AGB）恒星的星风（stellar winds）以及后RGB/后AGB盘的观测数据（observational data）和模拟数据（simulation data）。

Method: 采用尘埃运动学(dust kinematics)和宏观结构形成(macrostructure formation)分析方法研究行星-宿主相互作用的动力学过程。

Result: 恒星与行星的形成、生存及演化过程相互交织，尘埃运动学（dust kinematics）和宏观结构形成在从恒星诞生到死亡的尘埃环境中对行星-宿主相互作用产生关键影响。
...
### [44] [Monitoring and Evaluating Astronomy-for-Development Initiatives](https://arxiv.org/abs/2512.18015)
*Joyful E. Mdhluli*

**TL;DR**: 为天文学促进发展（astronomy-for-development）项目提供系统化的监测与评估（M&E）框架，通过明确理论变革（theory of change）、定义指标和识别风险，提升项目设计的科学性和影响力测量的有效性，同时兼顾国际天文学联合会发展办公室（IAU OAD）与可持续发展目标（SDGs）的协同实现。


<details>
  <summary>Details</summary>

Background: 研究主题为天文学促进发展（astronomy-for-development）项目的设计、实施与评估方法，旨在通过系统证据收集提升项目影响力并实现可持续发展目标（SDGs）。

Data: data部分：研究未涉及具体观测或模拟(simulation)数据，而是基于国际天文学联合会天文发展办公室(IAU OAD)年度提案征集的项目设计、实施与评估框架进行方法论探讨。

Method: 采用系统证据收集框架（systematic evidence collection framework）区分监测（monitoring）与评估（evaluation），并整合六种关键评估类型（过程、可行性、影响、结果、经济及总结性评估）以优化天文学促进发展项目的设计。

Result: 系统证据收集能强化天文学促进发展（astronomy-for-development）项目的设计、增强问责并提升可测量影响的可能性。
...
### [45] [Characterization of telecentric dual-etalon Fabry-Pérot systems from observational data. Properties of the CRISP2 instrument at the Swedish 1-m Solar Telescope](https://arxiv.org/abs/2512.18039)
*J. de la Cruz Rodríguez,G. B. Scharmer,P. Sütterlin,J. Leenaarts,M. G. Löfdahl,D. Kiselman,T. Hillberg,O. Andriienko*

**TL;DR**: 提出了一种无需改变光学设置或外部光源即可表征双标准具(etalon)法布里-珀罗干涉仪(FPI)空间参数变化的方法，发现CRISP2 FPI的腔间距变化极小(均方根<1.9 nm)，且需考虑次级透射峰和预滤光片曲线才能准确估算标准具反射率。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于太阳物理学中双标准具法布里-珀罗干涉仪(FPI)的光谱仪器特性空间分布表征方法优化，旨在提升光球层和色球层物理参数反演精度。

Data: 研究中使用的数据来源为CRISP2 FPI（Fabry-Pérot Interferometer）的两组观测数据和盘面中心平均宁静太阳光谱模板。

Method: 采用前向模型（forward model）结合双标准具（dual-etalon）FPI仪器退化效应和宁静太阳光谱模板，对CRISP2 FPI的视场依赖参数（包括标准具间距、反射率及预滤光片变化）进行空间表征。

Result: 提出了一种无需改变光学设置或使用外部光源的方法，通过结合FPI仪器退化前向模型和宁静太阳光谱模板，准确表征双标准具FPI仪器（CRISP2）的视场依赖参数（腔间距和反射率）及预滤光片变化。
...
### [46] [Designing the Community Infrastructure for ESO's Next Transformational Facility. Equitable Governance and Sustainable Teams for 2040s Astronomy](https://arxiv.org/abs/2512.18047)
...
**TL;DR**: 论文提出2040年代需将望远镜时间、数据、领导权、培训和职业发展的公平获取（equitable access）作为旗舰天文台（flagship observatory）的核心设计参数，以保障长期科学卓越性、可持续性和社会回报。


<details>
  <summary>Details</summary>

Background: 2040年代的科学目标需要跨大陆、跨机构和复杂政治与资金环境的大型多学科团队协作。

Data: 研究基于欧洲南方天文台(ESO)的Expanding Horizons倡议，结合现有天文台和新兴合作项目的经验数据。

Method: 采用跨学科协作框架（interdisciplinary collaboration frameworks）和全生命周期设计（lifecycle design）方法，将公平参与（equitable participation）作为核心参数整合到旗舰天文台（flagship observatory）的规划与运营中。

Result: 2040年代的核心科学挑战是运行一个旗舰天文台（flagship observatory），确保望远镜时间、数据、领导权、培训和职业发展的获取在机构、成员国及其他范围内实现公平。
...
### [47] [The Impact of the MAST Data Archive](https://arxiv.org/abs/2512.18101)
...
**TL;DR**: 研究发现MAST（Barbara A. Mikulski Archive for Space Telescopes）托管的数据支撑了大量前沿研究，其中哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）的档案文献产出率分别超过原始观测团队的3倍且可能更早占据主导，且观测项目文章产出可持续数十年，其中观测时间分配极端（极小或极大）的项目单位时间产出效率更高。


<details>
  <summary>Details</summary>

Background: 研究聚焦于评估NASA太空望远镜任务（如HST和JWST）的科学产出与长期影响力，通过分析MAST（Mikulski太空望远镜档案库）托管数据在学术出版物中的使用情况及其引用模式。

Data: 数据来源于Barbara A. Mikulski太空望远镜档案库(MAST)，该库汇集了20多个NASA任务的科学就绪数据产品、社区贡献的数据集以及其他精选巡天数据，覆盖紫外、光学和近红外波段。

Method: 采用文献计量学方法分析哈勃太空望远镜(HST)和詹姆斯·韦伯太空望远镜(JWST)任务数据的使用情况，重点考察论文发表数量、时间分布及引用模式。

Result: MAST（Barbara A. Mikulski Archive for Space Telescopes）托管的数据支持了紫外、光学和近红外波段的前沿研究，并已识别出超过37,000篇同行评审文章使用了其数据。
...
### [48] [Continuous Wide-Field Optical Monitoring for Very Early-Phase Transient Discovery](https://arxiv.org/abs/2512.18157)
...
**TL;DR**: 提出连续宽视场光学监测方法弥补传统巡天在时间分辨率上的不足，实现多信使（multimessenger）瞬变事件（如超新星、千新星、黑洞形成等）从物理起始阶段的电磁辐射与引力波、中微子信号的协同观测。


<details>
  <summary>Details</summary>

Background: 多信使（multimessenger）天文学背景下瞬变现象的早期物理过程研究。

Data: 数据来源为连续宽视场光学监测（continuous wide-area optical monitoring）和爱因斯坦望远镜（Einstein Telescope）的引力波观测。

Method: 采用连续宽视场光学监测（continuous wide-area optical monitoring）技术，结合多信使（multimessenger）观测策略，实时捕捉瞬变天体物理事件的电磁辐射与引力波（gravitational waves）、中微子（neutrinos）及高能辐射的关联信号。

Result: 连续宽视场光学监测填补了传统巡天在分钟至小时时间尺度上的观测空白，实现了对多信使（multimessenger）瞬变现象早期电磁辐射与引力波、中微子、高能辐射耦合机制的实时研究。
...
### [49] [Red noise-based false alarm thresholds for astrophysical periodograms via Whittle's approximation to the likelihood](https://arxiv.org/abs/2512.18205)
*Amna Ejaz,Sarah Dodson-Robinson,Charlotte Haley*

**TL;DR**: 提出了一种基于预白化（prewhitening）的频率依赖性错误警报水平（FAL）计算方法，通过拟合幂律和一阶自回归模型（AR1）优化Whittle近似负对数似然（NLL），有效识别了红噪声（red noise）背景下α Cen B、GJ 581等天体的真实周期信号，并验证了其在径向速度（RV）和开普勒（Kepler）数据中优于白噪声（white noise）模型的检测性能。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用预白化（prewhitening）技术改进基于红噪声（red noise）模型的Lomb-Scargle周期图（Lomb-Scargle periodogram）中虚假警报水平（false alarm level, FAL）的估计方法，以更准确地识别天文时间序列中的周期性信号。

Data: 数据来源包括α Cen B、GJ 581、HD 192310的观测数据，径向速度拟合挑战赛（RV Fitting Challenge）的合成数据，以及开普勒（Kepler）对差速自转天体的观测数据。

Method: 采用基于预白化（prewhitening）的方法计算频率依赖性错误警报水平（FAL），通过最小化负对数似然（NLL）的Whittle近似拟合幂律和1阶自回归模型到每个Lomb-Scargle周期图，并基于最佳拟合模型功率谱计算FAL。

Result: 提出了一种基于预白化（prewhitening）的方法来计算频率相关的错误警报水平（FAL），通过拟合幂律和自回归模型（AR(1)）到Lomb-Scargle周期图，并基于最佳拟合模型功率谱计算FAL。
...
### [50] [Unlocking the physics of dwarf galaxies in the 2040s: The case for a next-generation wide-field spectroscopic facility with fibres and IFUs](https://arxiv.org/abs/2512.18260)
...
**TL;DR**: 提出建造大口径（≥20米）、宽视场、高复用光谱设施以解决矮星系（dwarf galaxies）内部运动学、恒星种群及暗物质（dark matter）研究难题，推动对ΛCDM（Lambda-Cold Dark Matter）模型的检验并拓展低质量星系光谱观测至红移z∼1.5。


<details>
  <summary>Details</summary>

Background: 矮星系（dwarf galaxies）是宇宙中数量最多的星系，对研究暗物质（dark matter）、重子反馈（baryonic feedback）和星系形成至关重要。

Data: 研究中使用的数据来源包括宽视场成像巡天（wide-field imaging surveys）获取的矮星系候选体样本，以及未来可能通过专用大口径（≥20米）、宽视场、高复用光谱设施（highly multiplexed spectroscopic facility）对低表面亮度矮星系（low-surface-brightness dwarfs）进行的高信噪比光谱观测（high signal-to-noise observations）。

Method: 提出采用大口径（≥20米）、宽视场、高度复用光谱仪（highly multiplexed spectroscopic facility）结合可部署或单片式积分视场单元（IFUs）的技术手段，实现对低表面亮度矮星系（low-surface-brightness dwarfs）的高信噪比光谱观测（down to I_E≳22-23 mag）。

Result: 需要建设一台大口径（≥20米）、宽视场、高复用光谱仪（deployable or monolithic IFUs），以高效获取低表面亮度矮星系（I_E≳22-23星等）的高信噪比光谱数据。
...
### [51] [Robust and scalable simulation-based inference for gravitational wave signals with gaps](https://arxiv.org/abs/2512.18290)
*Ruiting Mao,Jeong Eun Lee,Matthew C. Edwards*

**TL;DR**: 该论文针对激光干涉仪空间天线（LISA）数据流中的间隙问题，提出了一种基于流匹配后验估计（FMPE）的可扩展仿真推理框架，通过双路径摘要架构（时域1D CNN和小波域2D CNN）直接处理含间隙时间序列数据，在模拟银河双星信号中实现了比传统方法更精确、无偏差的后验估计，并首次系统证明FMPE在严重数据伪影下比常规标准化流具有更优的稳定性和覆盖校准能力。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于解决激光干涉仪空间天线（LISA）数据流因维护和环境干扰导致的间隙问题及其引发的非平稳性和频谱泄漏对标准频域似然评估的负面影响。

Data: 数据来源为激光干涉空间天线（LISA）模拟数据流中的银河系双星类似信号（Galactic Binary-like signals）。

Method: 采用基于流匹配后验估计（FMPE）的模拟推理框架，结合双路径摘要架构（时间域1D卷积神经网络和小波域非对称膨胀核2D卷积神经网络）直接处理含间隙时间序列数据。

Result: 提出了一种基于流匹配后验估计（FMPE）和双路径摘要架构的可扩展仿真推理框架，能直接从含间隙时间序列数据中实现稳健参数估计，在存在严重数据伪影时比传统归一化流具有更优的稳定性和覆盖校准性。
...
### [52] [Standard Sirens in 2040s: Probing the Cosmic Expansion History with Gravitational Waves and Spectroscopic Galaxy Surveys](https://arxiv.org/abs/2512.18369)
...
**TL;DR**: 该论文核心贡献是论证了引力波（GWs）作为独立宇宙学探针（标准汽笛，standard sirens）的潜力，指出通过双中子星合并事件（如GW170817）可精确测量哈勃常数（Hubble constant），并强调宽视场高复用光谱设施对提升红移测量精度及宇宙学参数约束的关键作用。


<details>
  <summary>Details</summary>

Background: 引力波（GWs）作为独立于宇宙距离阶梯的自校准光度距离测量工具，已成为研究宇宙学的标准汽笛（standard sirens）。

Data: 研究中使用的数据来源包括引力波（GW）事件（如GW170817）的观测数据，以及通过电磁对应体（bright sirens）或统计关联潜在宿主星系（dark sirens）获取的红移信息，其中红移数据主要依赖宽视场高复用光谱设施的光谱测量。

Method: 采用引力波（GW）标准汽笛（standard sirens）方法，结合电磁对应体（bright sirens）或统计宿主星系关联（dark sirens）获取红移信息，并通过宽视场高复用光谱设施提升红移测量精度。

Result: 引力波（GWs）作为"标准汽笛"（standard sirens）通过自校准光度距离测量为宇宙学研究提供了独立于传统距离阶梯的新方法。
...
### [53] [EuroHPC SPACE CoE: Redesigning Scalable Parallel Astrophysical Codes for Exascale](https://arxiv.org/abs/2512.18883)
...
**TL;DR**: SPACE卓越中心（CoE）通过重构关键天体物理代码、采用创新编程范式及软件解决方案，解决了百亿亿次（exascale）系统的架构复杂性挑战，并利用机器学习（ML）和可视化工具提升高性能数据分析能力，促进欧洲天体物理社区围绕标准化软件和数据协议整合。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高性能计算（HPC）和百亿亿次（exascale）计算能力应对天体物理学与宇宙学（A&C）中复杂现象的模拟挑战，并通过跨学科合作优化代码重构与软件解决方案。

Data: 研究中使用的数据来源于高性能计算（HPC）模拟生成的大规模数据输出以及现代天文观测数据。

Method: 采用创新编程范式和软件解决方案重构关键天体物理代码，结合机器学习（ML）和可视化工具处理高性能数据分析。

Result: SPACE卓越中心（CoE）通过采用创新编程范式和软件（SW）解决方案重新设计关键天体物理代码，以应对百亿亿次级（exascale）系统带来的新计算挑战。
...
### [54] [Optical SETI at ESO in the 2040s](https://arxiv.org/abs/2512.18903)
*Valenitn D. Ivanov*

**TL;DR**: 论文提出利用ESO（欧洲南方天文台）现有及未来高复用光谱设备探测地外人工激光通信信号（technosignature），强调其单色光特征可作为明确技术标志物，并指出该计划具有巨大科普潜力。


<details>
  <summary>Details</summary>

Background: 探索地外生命和智慧存在是当代科学面临的核心问题。

Data: 数据来源为欧洲南方天文台（ESO）当前及未来2040年代可能开发的极端多路复用（extreme multiplexity）光谱设施。

Method: 采用极端多路复用（extreme multiplexity）光谱技术进行人工激光通信（optical laser communication）搜索。

Result: 探索人工激光通信（artificial laser communication）作为地外智慧生命（extraterrestrial intelligence）的可识别技术特征（technosignature）具有重要科学意义。
...
### [55] [The Simons Observatory: Detector Polarization Angle Calibration using Sparse Wire Grid with Initial Data Sets of the Small Aperture Telescope](https://arxiv.org/abs/2512.19102)
...
**TL;DR**: 通过开发基于稀疏线栅（sparse wire grid）的远程可控校准系统，实现了西蒙斯天文台小口径望远镜（Simons Observatory Small Aperture Telescopes）探测器偏振角（polarization angle）的高精度校准（93 GHz和145 GHz频段的统计不确定度分别为0.02°和0.03°，系统不确定度为0.08°），满足宇宙微波背景（cosmic microwave background）B模测量对全局偏振角校准精度（≲0.1°）的要求。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过精确校准探测器天线在天空投影的偏振角度，提升宇宙微波背景（cosmic microwave background）中B模（$B$-modes）的测量精度，以降低E模（$E$-modes）泄漏对B模探测的偏差。

Data: 数据来源于安装在智利阿塔卡马沙漠帕克天文台的西蒙斯天文台小口径望远镜（Small Aperture Telescope）上运行的稀疏线栅（sparse wire grid）校准系统，以及93 GHz和145 GHz频段的初始观测数据。

Method: 采用稀疏线栅（sparse wire grid）注入可旋转线偏振信号的全远程可控校准系统，结合探测器偏振角校准流程，实现全球偏振角校准精度优于0.1°。

Result: 开发了一种基于稀疏线栅（sparse wire grid）的全远程可控校准系统，用于精确校准探测器偏振角（polarization angle），其统计和系统误差的平方和小于0.1度。
...
### [56] [Towards DM-free search for Fast Radio Bursts with Machine Learning -- I. An implementation on multibeam data](https://arxiv.org/abs/2512.19249)
...
**TL;DR**: 提出了一种基于机器学习（ML）的无需消色散（dedispersion）处理的新方法，在多波束接收器数据环境下训练EfficientNet模型，实现了超过92%的准确率和精确度的快速射电暴（FRB）识别，显著提升了搜索效率并自然抑制了射频干扰（RFI）。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用机器学习（ML）技术高效识别快速射电暴（FRB）信号，以解决传统消色散（dedispersion）方法在宽视场射电望远镜海量数据处理中耗时巨大的挑战。

Data: 数据来源于多波束接收器环境下的原始观测数据及中国五百米口径球面射电望远镜（FAST）的实际巡天数据。

Method: 采用机器学习（ML）中的EfficientNet模型直接从原始数据中识别快速射电暴（FRB）信号，无需进行消色散（dedispersion）处理。

Result: result：机器学习（ML）方法在不进行去色散（dedispersion）的情况下识别快速射电暴（FRB）信号，准确率和精确率均超过92%，且显著提升了搜索效率并自然抑制了多波束环境下的射频干扰（RFI）。
...
### [57] [Observations of the 21 cm HI Line from the Milky Way galaxy using Pyramidal Horn Radio Telescope](https://arxiv.org/abs/2512.19262)
*Kaustav Bhattacharjee,Himanshu Grover,P. Arumugam*

**TL;DR**: 设计并验证了一种基于软件无线电(SDR)的低成本金字塔形喇叭射电望远镜系统，成功探测到银河系21厘米中性氢(HI)谱线，并绘制出HI天图、银河系旋转曲线和旋臂结构特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用低成本射电望远镜系统探测银河系21厘米中性氢(HI)谱线发射及其在银河系结构研究中的应用。

Data: 数据来源于使用SDR(软件定义无线电)管道获取的漂移扫描(drift-scan)观测数据。

Method: 采用基于软件无线电（SDR）的漂移扫描观测（drift-scan）数据处理流程，结合金字塔形喇叭天线（pyramidal horn）进行校准和成像分析。

Result: 该低成本系统能有效用于教育目的及射电波段银河系结构的科学探索。
...
### [58] [GALATEA: The 15-m Galactic Archaeology Spectroscopic Surveyor](https://arxiv.org/abs/2512.19444)
...
**TL;DR**: GALATEA（Galactic Archaeology and Local-group Astrophysics Telescope for Extended Areas）作为北半球专用15米口径、10,000光纤光谱巡天设施，通过大视场（约1平方度）中高分辨率（R=5,000-25,000）多目标光谱观测，首次实现对银河系外盘、翘曲、晕亚结构及M31、M33等近邻星系恒星群体的深度化学动力学普查，填补现有巡天在6D相空间和化学丰度信息获取能力的空白。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过下一代15米级宽视场多目标光谱巡天设施GALATEA（Galactic Archaeology and Local-group Astrophysics Telescope for Extended Areas）推动银河系（Milky Way）和本星系群（Local Group）的恒星考古学（Galactic archaeology）与化学动力学（chemodynamical）研究。

Data: 数据来源包括盖亚(Gaia)及其近红外后续任务盖亚近红外(GaiaNIR)提供的精确天体测量和测光数据，以及GALATEA自身通过15米口径望远镜、1平方度视场和10,000根光纤进行的大规模多目标光谱观测。

Method: 采用15米口径望远镜结合10,000根光纤的多目标光谱技术（multi-object spectroscopy），在1平方度视场下实现中分辨率（R≈5,000–10,000）和高分辨率（R≈20,000–25,000）光谱观测。

Result: GALATEA（Galactic Archaeology and Local-group Astrophysics Telescope for Extended Areas）是一种专用于北半球的15米宽视场、配备10,000根光纤的光谱巡天设施概念，旨在通过度级视场和多目标光谱技术开启银河系及本星系群天文学的新领域。
...
### [59] [Scalar-Mediated Inelastic Dark Matter as a Solution to Small-Scale Structure Anomalies](https://arxiv.org/abs/2512.18959)
*Zihan Wang*

**TL;DR**: 提出了一种由轻子性标量媒介子和质量分裂为100电子伏特的伪狄拉克暗物质候选者组成的标量媒介自相互作用暗物质（SIDM）模型，通过共振增强在矮星系中实现大自相互作用截面并抑制超暗弱卫星星系中的散射，同时引入五维磁偶极算子满足大爆炸核合成（BBN）要求并产生独特的直接探测信号。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过标量媒介的自相互作用暗物质（SIDM）模型解决小尺度结构异常问题（如核心-尖峰问题和多样性问题）。

Data: 数据来源于理论模型构建的数值模拟（simulation）和现有天体物理观测（如超微弱卫星星系、矮星系的动力学特征）以及宇宙学约束（如BBN、CMB）。

Method: 采用标量介导的自相互作用暗物质(SIDM)模型，结合轻子性标量媒介子和质量分裂为100 eV的赝狄拉克暗物质候选体，通过暗离散$\mathbb{Z}_2$对称性抑制树级弹性散射，并引入五维磁偶极算子实现激发态衰变($χ_2 \rightarrow χ_1 γ$)。

Result: 提出了一种标量媒介的自相互作用暗物质（SIDM）模型，通过轻子偏好标量媒介和100 eV质量分裂的赝狄拉克暗物质候选者，解决了核心-尖峰及多样性等小尺度结构异常问题。
...
### [60] [Gravitational Production of Massive Spin-2 Particles During Reheating](https://arxiv.org/abs/2512.19041)
*Sarunas Verner*

**TL;DR**: 研究了仅通过引力耦合的庞加莱质量（massive spin-2）暗物质候选者$X_{μν}$在扰动再加热（perturbative reheating）期间的最小引力产生机制，发现其纵向模式（helicity-0）主导产生且截面增强$\sim (m_φ/m_2)^4$，相比低自旋粒子更高效，再加热温度$T_{\rm RH}$或质量$m_2 \lesssim m_φ$决定避免过量产生的约束条件。


<details>
  <summary>Details</summary>

Background: 研究探索了在宇宙再加热（reheating）过程中通过最小引力门户（minimal gravitational portal）产生自旋-2暗物质候选者（massive spin-2 dark matter candidate）的机制。

Data: 数据来源于理论模拟（simulation），基于Fierz-Pauli框架下自由质量自旋-2场（massive spin-2 field）的引力子（graviton）交换过程，包括暴胀子凝聚态湮灭（inflaton condensate annihilation）和热散射（thermal scatterings）两个通道的振幅计算及螺旋度分解（helicity decomposition）。

Method: 采用费尔兹-泡利（Fierz-Pauli）框架分析自由质量自旋-2场（massive spin-2 field）的引力子介导振幅，并进行末态全螺旋度分解（helicity decomposition）。

Result: 在轻质量区间（$m_2 \ll m_\phi$），自旋-2暗物质（$X_{\mu\nu}$）主要通过纵向（螺旋度-0）模式产生，其截面参数增强为$\sim (m_\phi/m_2)^4$，导致引力耦合下高效暗物质生成。
...
### [61] [Probing Dark Sectors with Exploding Black Holes: Gamma Rays](https://arxiv.org/abs/2512.19603)
*Michael J. Baker,Joaquim Iguaz Juan,Aidan Symons,Andrea Thamm*

**TL;DR**: 通过改进实验响应函数和背景估计方法，量化了当前（HAWC、LHAASO）和未来（SWGO、CTA）伽马射线望远镜探测暗物质（dark sectors）中10-50个新狄拉克费米子（Dirac fermions）在10^5-10^6 GeV质量范围内的能力，其中CTA对0.1-0.6 pc距离的爆炸黑洞（black hole）最敏感。


<details>
  <summary>Details</summary>

Background: 探索黑洞爆炸产生的霍金辐射(Hawking radiation)如何揭示自然界粒子谱系，并评估伽马射线望远镜探测新暗物质区(dark sectors)的潜力。

Data: data：研究使用了当前实验（HAWC和LHAASO）和未来实验（SWGO及CTA North和South）的伽马射线望远镜响应函数，并优化了背景估计和统计分析。

Method: 采用实验响应函数（experimental response functions）优化统计分析，并改进背景估计方法，量化了当前和未来伽马射线望远镜（HAWC、LHAASO、SWGO、CTA）探测新暗区（dark sectors）的灵敏度。

Result: HAWC可探测10-20个新狄拉克费米子（Dirac fermions）至约10^5 GeV质量，CTA可探测2-15个新狄拉克费米子至10^6 GeV质量，CTA南北站分别对10个暗费米子（dark fermions）敏感至0.1 pc距离、对50个敏感至0.6 pc距离。
...
### [62] [Ultra-high precision high voltage system for PTOLEMY](https://arxiv.org/abs/2512.19437)
...
**TL;DR**: PTOLEMY项目通过开发超高精度电压控制系统（精度达0.2 ppm/kV）和场磨（field mill）监测装置，为基于石墨烯束缚氚电子捕获的宇宙中微子背景（cosmic neutrino background）探测提供了关键技术支撑，首次实现了在10 kV串联系统中总电压波动小于50 mV的突破性控制精度。


<details>
  <summary>Details</summary>

Background: 探测宇宙中微子背景（cosmic neutrino background）并通过结合石墨烯的氚电子捕获实现高精度β能谱测量。

Data: 使用商用超高精度电压参考链（commercial ultra-high-precision voltage references）、精密万用表（precision multimeters）和场磨装置（field mill device）进行实时监测。

Method: 采用商业超高精度电压参考链（commercial ultra-high-precision voltage references）结合精密万用表（precision multimeters）和场磨仪（field mill）实时监控电极电压。

Result: 首次实现了基于商用超高精度电压基准链的高压控制精度达0.2 ppm/kV（单板）和≤50 mV（10 kV串联），理论上通过高斯噪声外推可进一步降至0.05 ppm/20 kV。
...
### [63] [The First Model-Independent Upper Bound on Micro-lensing Signature of the Highest Mass Binary Black Hole Event GW231123](https://arxiv.org/abs/2512.19077)
*Aniruddha Chakraborty,Suvodip Mukherjee*

**TL;DR**: 论文核心贡献是采用无模型方法（μ-GLANCE）分析GW231123等高质量双黑洞（BBH）合并事件的引力透镜（gravitational lensing）特征，未发现强透镜证据但检测到潜在微透镜（micro-lensing）残留信号（振幅<0.8），指出当前波形系统误差阻碍了短时引力波（GW）的透镜探测，需更精确波形模型支持未来透镜事件发现。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用引力波（GW）事件GW231123探索高质量双黑洞（BBH）合并的起源机制，特别是针对恒星前身黑洞因对不稳定性超新星质量间隙而罕见存在的质量范围，提出了引力透镜效应作为可能解释其高质量推断值的替代假说。

Data: 数据来源于LIGO-Virgo-KAGRA合作组探测到的引力波事件GW231123及其它事件，采用\texttt{$\mu$-GLANCE}技术对残余应变进行模型无关分析。

Method: 采用模型无关方法\texttt{$μ$-GLANCE}技术，通过交叉相关分析探测器网络中的残余应变（residual strain）寻找共同特征，并在贝叶斯框架下推断引力透镜信号。

Result: GW231123事件的分析未发现支持引力透镜（gravitational lensing）的强证据，但检测到可能存在振幅小于0.8（95%置信区间）的微引力透镜（micro-lensing）残余信号。
...
### [64] [Parameter estimation for the GWTC-4.0 catalog with phenomenological waveform models that include orbital eccentricity and an updated description of spin precession](https://arxiv.org/abs/2512.19513)
...
**TL;DR**: GWTC-4.0目录扩展分析通过引入椭圆轨道和频域自旋进动（spin precession）改进模型，利用IMRPhenom系列波形（XPNR/TPHM/TEHM）重新评估84例双黑洞并合事件，更新后验样本并量化波形效应对源属性推断的影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用LIGO-Virgo-KAGRA（LVK）引力波探测器网络第四观测运行前半段数据，通过改进椭圆轨道和频域自旋进动（spin precession）建模来扩展GWTC-4.0暂现引力波信号目录的分析。

Data: 数据来源于LIGO-Virgo-KAGRA (LVK)引力波探测器网络在第四次观测运行第一阶段记录的GWTC-4.0目录中的84个确信探测到的双黑洞并合事件，并使用了IMRPhenom波形族（具体为XPNR、TPHM和TEHM）的最先进波形模型。

Method: 采用改进的IMRPhenom系列波形模型（XPNR、TPHM和TEHM）分析椭圆轨道和频域自旋进动效应。

Result: GWTC-4.0目录扩展分析表明，考虑椭圆轨道和频域自旋进动（spin precession）的改进描述会显著改变双黑洞（binary-black-hole）合并事件的推断源属性。
...
### [65] [Bose-Einstein Condensate dark matter with logarithmic nonlinearity](https://arxiv.org/abs/2512.19042)
*Zahra Haghani,Tiberiu Harko*

**TL;DR**: 该论文提出对数形式自相互作用势的玻色-爱因斯坦凝聚态(Bose-Einstein Condensation)暗物质模型，通过流体力学方法推导其状态方程和静态密度分布，并基于SPARC数据验证该模型能合理解释星系旋转曲线。


<details>
  <summary>Details</summary>

Background: 探索暗物质作为具有自相互作用势的对数形式玻色-爱因斯坦凝聚态(Bose-Einstein Condensate)的宇宙学演化及其性质。

Data: 研究使用了来自SPARC（Spitzer Photometry and Accurate Rotation Curves）数据库的星系旋转曲线观测数据。

Method: 采用对数非线性项的Gross-Pitaevskii方程（Gross-Pitaevskii equation）和托马斯-费米近似（Thomas-Fermi approximation）描述凝聚态暗物质，并通过流体力学表示推导状态方程。

Result: result：对数形式自相互作用的玻色-爱因斯坦凝聚暗物质模型能合理解释SPARC数据中的星系旋转曲线特征。
...
### [66] [High-Precision Differential Radial Velocities of C3PO Wide Binaries: A Test of Modified Newtonian Dynamics (MOND)](https://arxiv.org/abs/2512.19652)
*Serat Mahmud Saad,Yuan-Sen Ting*

**TL;DR**: 该研究通过开发高精度差分径向速度测量技术（精度达8-15 m/s）并结合盖亚天体测量数据，构建层次贝叶斯模型分析100对宽双星系统，发现其轨道参数与修正牛顿动力学（MOND）在公认加速度标度（a₀）下的预测存在显著差异（简单形式排斥度3.1σ，标准形式1.9σ）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用宽双星系统（wide-binary stars）在低加速度（low-acceleration）环境下检验修正牛顿动力学（MOND）理论对牛顿引力的偏离预测。

Data: 数据来源于C3PO巡天项目中的100组宽双星系统的高分辨率、高信噪比光谱测量，并结合了盖亚(Gaia)天体测量数据。

Method: 采用高分辨率、高信噪比光谱测量宽双星差分视向速度（differential RVs）技术，结合盖亚天体测量数据构建层次贝叶斯模型（hierarchical Bayesian model），推断宽双星轨道参数和全局MOND加速度标度（a0），并测试简单形式（b=1）和标准形式（b=2）的MOND插值函数。

Result: result：研究结果表明，修正牛顿动力学（MOND）在当前公认的加速度尺度（a₀）值下存在张力，其中简单形式（b=1）在3.1σ水平排除标准值，标准形式（b=2）在1.9σ水平排除标准值。
...
### [67] [Unveiling Long-Period Variables in M33's Central Region: Insights into Stellar Evolution and Star Formation via Near-Infrared Photometry](https://arxiv.org/abs/2512.18423)
*Mina Alizadeh,Yousefali Abedini,Hedieh Abdollahi*

**TL;DR**: 通过UKIRT观测分析了M33中心平方千秒差距内的演化恒星种群，利用点扩散函数（PSF）测光技术识别出约750颗长周期变星（LPVs），主要为渐近巨星分支（AGB）星，为研究M33中心区域的质量损失和动力学演化提供了关键数据基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于M33星系中心平方千秒差距区域内演化恒星（尤其是渐近巨星支(AGB)星）的分布与特性。

Data: 数据来源于2003至2007年UKIRT望远镜的观测，使用UIST和WFCAM仪器在$J$、$H$、$K$波段合并的观测数据，通过逐帧交叉匹配39个观测夜的三波段数据提取了211,179颗恒星。

Method: 采用点扩散函数（PSF）测光技术处理UKIRT望远镜在J、H和K波段的观测数据，通过逐帧交叉匹配39个观测夜的数据提取恒星信息，并利用孔径测光交叉验证以识别长周期变星（LPVs）。

Result: 通过点扩散函数（PSF）测光法在M33中心区域识别出约750颗长周期变星（LPVs），主要为渐近巨星分支（AGB）星，有效解析了密集星场中的混合源并探测到可能被遗漏的暗弱尘埃变星。
...
### [68] [Pulsational Instability of Quasi-Stars: Interpreting the Variability of Little Red Dots](https://arxiv.org/abs/2512.17997)
...
**TL;DR**: 发现JWST观测到的"小红点"(LRDs)中不稳定准恒星(quasi-star)包层的径向脉动周期为20-180年，其光变特性与κ机制驱动的氦氢电离区不稳定性一致，支持这类天体是嵌有快速增长黑洞种子的辐射压支撑包层结构。


<details>
  <summary>Details</summary>

Background: 研究探讨了詹姆斯·韦伯太空望远镜（JWST）发现的“小红点”（LRDs）中可能存在超大质量黑洞（SMBHs）的准恒星（quasi-star）结构及其包层稳定性。

Data: 数据来源于JWST（James Webb Space Telescope）对引力透镜作用下的LRD R2211-RX1的观测，包括世纪尺度的光度变化和光度-温度平面的滞后回线，以及使用恒星演化代码MESA和非绝热振荡代码GYRE进行的数值模拟。

Method: 采用恒星演化代码MESA耦合非绝热振荡代码GYRE分析准恒星包层稳定性，识别出理论上的"准恒星不稳定带"（Quasi-Star Instability Strip），其蓝边位于有效温度$T_{\mathrm{eff}} \approx 5000-5200$ K。

Result: 在有效温度（effective temperature）低于约5000-5200 K时，准恒星（quasi-star）包层会因氦和氢电离区的κ机制（κ-mechanism）驱动径向脉动而变得不稳定，形成理论上的"准恒星不稳定带"（Quasi-Star Instability Strip）。
...
### [69] [Time-resolved X-ray spectra of Proxima Centauri as seen by XMM-Newton](https://arxiv.org/abs/2512.18011)
*A. Damonte,I. Pillitteri,A. Maggio,A. García Muñoz,G. Micela*

**TL;DR**: 通过分析XMM-Newton观测数据，首次以高时间分辨率生成了比邻星（Prox Cen）1-920埃波段的时变XUV光谱，揭示其X射线通量瞬时变化幅度达平均值的20倍至五分之一，且波长依赖性显著，为研究其宜居带行星（Prox Cen b）大气演化提供了关键辐射变率参数。


<details>
  <summary>Details</summary>

Background: 恒星软X射线（1-100埃）和极紫外（EUV，100-920埃；合称XUV）辐射影响系外行星大气演化和化学过程。

Data: 数据来源于XMM-Newton望远镜对Proxima Centauri的全部存档观测数据，并基于已发表的EUV模板通过X射线与EUV通量关系进行缩放。

Method: 采用新的堆积效应(pile-up)校正方法、自适应时间分箱算法和时间依赖的等离子体模型选择来推导时间分辨X射线光谱，并基于X射线与EUV通量关系的已发表模板估算EUV光谱。

Result: Prox Cen的X射线瞬时通量在观测基线内波动范围达平均值的20倍至五分之一，且不同波长间存在显著差异。
...
### [70] [A Search for Radio Technosignatures from Interstellar Object 3I/ATLAS with the Allen Telescope Array](https://arxiv.org/abs/2512.18142)
...
**TL;DR**: 首次对星际天体3I/ATLAS开展1-9GHz射电技术信号（technosignatures）搜索，未发现可信信号并给出有效全向辐射功率（EIRP）上限为10-110W。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用射电天文观测技术搜寻星际天体3I/ATLAS可能携带的地外文明技术特征信号（technosignatures），以探索太阳系外智慧生命存在的可能性。

Data: 使用SETI研究所的艾伦望远镜阵列（Allen Telescope Array）在1-9 GHz频段对3I/ATLAS进行了观测，并通过新开发的搜索管道bliss处理了7.25小时的数据，检测到近7400万个窄带信号。

Method: 采用艾伦望远镜阵列(Allen Telescope Array)对3I/ATLAS进行1-9GHz观测，运用新开发的bliss搜索管道检测窄带信号，并通过频率和漂移率空白处理抑制射频干扰(RFI)，再使用NBeamAnalysis定位代码进行筛选，最终在时频域人工检查剩余信号。

Result: 使用艾伦望远镜阵列(Allen Telescope Array)在1-9 GHz范围内观测星际物体3I/ATLAS，未检测到值得进一步追踪的技术特征信号(technosignatures)，据此设定了10-110 W的有效全向辐射功率(EIRP)上限。
...
### [71] [The size of 3I/ATLAS from non-gravitational acceleration](https://arxiv.org/abs/2512.18341)
*John C. Forbes,Harvey Butler*

**TL;DR**: 研究发现星际天体3I/ATLAS的非引力加速度与质量损失一致，其核直径约1公里（时间滞后模型下为820-1050米），估算受喷发不对称因子（ζ）和密度（ρ）影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于太阳系中第三颗宏观星际天体（3I/ATLAS）的非引力加速度（non-gravitational acceleration）与质量损失（mass loss）之间的关联性。

Data: 数据来源于对3I/ATLAS彗星近日点附近轨迹的非引力加速度（non-gravitational acceleration）最佳拟合模型，并结合质量损失估算进行对比分析。

Method: 采用非引力加速度模型（non-gravitational acceleration）结合质量损失率估算，通过设定喷发不对称因子（outgassing asymmetry factor）ζ=0.5和彗核密度（comet nucleus density）ρ=0.5 g cm⁻³等参数约束彗核直径范围。

Result: 3I/ATLAS彗核直径约为1公里时，其质量损失与非引力加速度解一致。
...
### [72] [Imaging the LkCa 15 system in polarimetry and total intensity without self-subtraction artefacts](https://arxiv.org/abs/2512.18439)
...
**TL;DR**: 采用近同步参考星差分成像技术（star-hopping）获取了LkCa~15原行星盘（protoplanetary disk）无自减伪影的高分辨率Ks波段图像，结合ALMA数据通过多尺度（微米和毫米）橄榄石模型分析发现盘面散射特性更符合多孔聚集体（CAHP）而非致密球体（Mie spheres），并排除了200天文单位（au）外存在质量超过1.5倍木星质量（MJ）的行星。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用高分辨率成像技术克服传统角差分成像（angular differential imaging）的自减效应（self-subtraction artefacts），以精确解析原行星盘（protoplanetary disk）的小尺度结构及其尘埃特性，进而深入理解行星形成过程。

Data: 数据来源为SPHERE获取的LkCa~15系统高分辨率Ks波段总强度和偏振强度图像，以及ALMA亚毫米连续谱数据。

Method: 采用近红外散射光图像与ALMA亚毫米连续谱数据结合的双粒度（微米级和毫米级）致密橄榄石模型（RADMC-3D）及多孔聚集体散射模型（CAHP）分析尘埃特性。

Result: 采用多孔聚集体（CAHP）散射模型显著改善了Ks波段形态和偏振观测结果与模拟的一致性，同时保留毫米级致密颗粒以匹配ALMA连续谱数据。
...
### [73] [Detecting false positives with PLATO using double-aperture photometry and centroid shifts](https://arxiv.org/abs/2512.18844)
*F. Gutiérrez-Canales,R. Samadi,A. Birch,J. Cabrera,C. Damiani,P. Guterman,C. Paproth,M. Pertenais,A. Santerne*

**TL;DR**: 该论文提出了一种双孔径测光法（double-aperture photometry），通过扩展掩模（extended masks）和次级掩模（secondary masks）结合通量和质心偏移指标，高效检测由食双星（eclipsing binaries）引起的凌星观测假阳性信号（false positives），其中次级通量（secondary flux）检测效率最高（92%），为PLATO任务提供了一种计算和遥测成本较低的解决方案。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用双孔径测光法（double-aperture photometry）提升PLATO任务中凌日系外行星候选体的假阳性（false positives）识别效率。

Data: 数据来源为盖亚(Gaia) DR3星表数据，用于定义P5样本目标恒星及其背景恒星（假设为食双星），并结合观测分布模拟凌星深度和持续时间生成模拟测光(photometry)和质心偏移(centroid shift)数据。

Method: 采用双孔径测光法（double-aperture photometry）结合扩展掩模（extended masks）和次级掩模（secondary masks）检测由食双星（eclipsing binaries）引起的假阳性信号（false positives）。

Result: 双孔径测光法（double-aperture photometry）通过次级掩模通量（secondary flux）和扩展掩模质心偏移（extended centroid shifts）可高效检测由食双星（eclipsing binaries）引起的假阳性信号（false positives），其中次级通量的检测效率最高（92%）。
...
### [74] [Disentangling the hemispheres of Teegarden's Star b with LIFE](https://arxiv.org/abs/2512.19231)
*Ryan Boukrouche,Markus Janson*

**TL;DR**: 研究发现利用LIFEsim软件和三维全球气候模型(GCM)模拟，在宽带观测3天条件下可区分Teegarden's Star b（非凌日岩质行星）半球特征（1σ或3σ置信水平），并可能通过空间分辨率探测水云存在以推断活跃水文循环。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用热辐射地图和三维全球气候模型(GCM)模拟评估LIFE任务对非凌日类地行星Teegarden's Star b的观测几何敏感性及其潜在水文活动特征。

Data: 数据来源为LIFEsim软件及三维全球气候模型（GCM）模拟生成的热辐射图。

Method: 采用三维全球气候模型（GCM）模拟生成热辐射图，结合LIFEsim软件分析观测几何敏感性。

Result: 通过三维全球气候模型(GCM)模拟和LIFEsim软件分析，发现仅需3天宽带观测即可在1σ或3σ置信水平下区分Teegarden's Star b的半球特征。
