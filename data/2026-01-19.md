...
**TL;DR**: 通过磁流体动力学(MHD)模拟和宇宙射线电子谱演化，揭示了活动星系核(AGN)喷流在类似英仙座星系团环境中产生的射电同步辐射特征，建立了喷流动力学、电子能谱与观测射电特性之间的物理联系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核(AGN)喷流加速宇宙射线电子并产生射电同步辐射的物理机制及其与星系团环境的相互作用。

Data: 使用磁流体动力学（MHD）模拟了单个活动星系核（AGN）喷流在类似英仙座星系团中的爆发，并采用亚网格模型模拟未解析喷流内部激波中宇宙线质子和电子的加速过程。

Method: 采用磁流体动力学(MHD)模拟结合亚网格宇宙射线电子加速模型，并利用Fokker-Planck求解器Crest和辐射计算工具Crayon+进行电子谱演化和非热辐射计算。

Result: 射电同步辐射谱指数与电子注入年龄相关：特定频率下弱磁场区域由动量较高电子主导（其年龄由最近注入事件决定），而强磁场区域由动量较低电子主导（其年龄由过去最大能量注入事件决定）。
...
**TL;DR**: 通过高分辨率射电观测发现室女座星系NGC 4522在星系际介质(ICM)作用下，其射电连续谱、谱指数和磁场呈现不对称分布，外盘谱指数陡化由ICM激波电子再加速和热辐射减弱导致，而盘外云平坦谱指数源于热辐射增强与非热成分谱老化混合效应，偏振辐射分布进一步证实ICM风对星际介质(ISM)的显著影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于室女座星系团中受冲压剥离（ram-pressure-stripped）作用的星系NGC 4522，探索其射电连续谱、谱指数和磁场在星系团内介质（ICM）影响下的特征。

Data: 数据来源于甚大天线阵(VLA)在S波段(3 GHz)和X波段(10 GHz)的高分辨率连续谱观测。

Method: 采用甚大天线阵(VLA)在S波段(3 GHz)和X波段(10 GHz)进行高分辨率连续谱观测，结合谱指数分析和磁场测量，研究ICM(星系团内介质)与星际介质(ISM)相互作用下的射电连续谱特征。

Result: 外盘（outer disk）的谱指数（spectral index）陡化行为主要由星系团内介质（ICM）激波对电子的再加速和热辐射显著减少导致。
...
**TL;DR**: 该论文总结了星际介质(ISM)中尘埃颗粒热连续谱辐射的物理机制，提供了通过观测强度和通量密度计算尘埃质量、柱密度、光度、温度及不透明度谱指数等物理参数的方法，并更新了不同ISM环境下的平均分子量数值（每个H原子1.404、每个H2分子2.809、每个气体粒子2.351）。


<details>
  <summary>Details</summary>

Background: 研究主题为星际介质中尘埃颗粒的热连续辐射（thermal dust continuum emission）物理机制及其观测特性分析。

Data: 研究中使用的数据来源包括历史观测记录（如1965年发现的星际尘埃热辐射）、不同天体环境（如弥散星际介质、致密核、原行星盘）的尘埃不透明度特性表，以及过去、当前和未来测辐射热计相机波长的实测数据。

Method: 采用热尘埃连续谱辐射（thermal dust continuum emission）的物理模型，通过观测尘埃天体的强度和流量密度计算质量、柱密度、光度、尘埃温度和尘埃不透明度谱指数等物理参数。

Result: 星际尘埃颗粒的热连续谱发射（thermal dust continuum emission）观测可用于计算天体物理对象的物理性质，包括质量、柱密度、光度、尘埃温度（dust temperature）和尘埃不透明度谱指数（dust opacity spectral index）。
...
**TL;DR**: 利用CHILES观测首次在z=0.22-0.47红移范围内解析了四个星系的HI中性气体形态，发现高红移下H2/HI质量比均值(10.3±3.4)显著高于本地宇宙。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用CHILES巡天和LMT观测数据，结合JWST成像，探索红移范围0-0.5内星系中性氢（HI）和分子氢（H2）的形态、质量分布及演化规律。

Data: 数据来源于COSMOS HI Large Extragalactic Survey (CHILES)的HI观测、LMT的CO(1-0)观测以及JWST的NIRCam四波段组合成像。

Method: 采用COSMOS HI大尺度河外巡天(CHILES)的高分辨率中性氢(HI)成像技术，结合大型毫米波望远镜(LMT)的一氧化碳(CO(1-0))观测和詹姆斯·韦伯太空望远镜(JWST)近红外相机(NIRCam)四波段合成成像技术。

Result: 在最高红移处观测到的平均H2/HI比值（10.3 ± 3.4）显著高于本地宇宙中的平均值。
...
**TL;DR**: 采用深度学习（ResNet-18架构）从DESI Legacy巡天数据中筛选出485个高置信度星系团强引力透镜（galaxy cluster-scale strong gravitational lensing）候选体，其中247个为新发现，并公开了包含透镜弧显著程度分级（A、B、C级）的候选体目录。


<details>
  <summary>Details</summary>

Background: 研究星系团尺度强引力透镜系统（galaxy cluster-scale strong gravitational lensing）作为探索暗物质（dark matter）、暗能量（dark energy）及高红移宇宙的独特工具。

Data: 数据来源于DESI Legacy Imaging Surveys（DESI传统成像巡天）和Zou等人（2021）的星系团候选体目录，其中正样本采用模拟的星系团强引力透镜图像，负样本采用观测到的非透镜星系团图像。

Method: 采用基于深度学习（deep learning）的残差网络（ResNet-18）架构，结合模拟的星系团透镜（cluster lenses）正样本和观测的非透镜（non-lenses）负样本进行训练，并通过迭代操作（iterative operation）提升完整性，辅以人工检查（human inspection）对候选体按强引力弧（strongly lensed arcs）显著性分级。

Result: 采用深度学习（ResNet-18架构）结合迭代训练和人工检查方法，从540,432个星系团候选体中筛选出485个高置信度星系团强引力透镜候选体（含247个新发现候选体），其质量范围$M_{500}$为$10^{13.67\sim14.97}M_{\odot}$、中心亮星系（BCG）红移范围为0.04~0.89。
...
**TL;DR**: 分析JADES DR3中763个星系（红移1.4 < z < 7）的静止光学发射线比例，发现红移3.5时固定气体相金属丰度下电离谱比红移2时更硬，导致BPT图（[N II]/Hα）和O32-R23图中恒星形成星系轨迹显著偏移，而红移≥3.5时电离条件无明显演化，且与大质量恒星化学丰度模式（α/Fe）相关，揭示了JWST时代发射线图对再电离纪元星际介质（ISM）条件的诊断价值。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用静止光学发射线比率分析宇宙再电离时期(Epoch of Reionization)高红移(1.4 < z < 7)星系际介质(ISM)的电离条件及其与恒星化学丰度模式的关联。

Data: 数据来源于JADES DR3（JWST Advanced Deep Extragalactic Survey Data Release 3）的763个星系样本及其复合光谱。

Method: 采用多种静止光学发射线比（[O III]λ5008/Hβ、[N II]λ6585/Hα等）分析样本及JADES DR3复合光谱，结合BPT图（Baldwin-Phillips-Terlevich diagram）和O32-R23图探究电离条件演化。

Result: 在红移$z\gtrsim 3.5$时，恒星形成星系在固定气体相金属丰度下占据共同轨迹，表明此时星际介质(ISM)电离条件未发生显著演化。
...
**TL;DR**: 利用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)对20 km/s云团中两个大质量团块(Clump 1和Clump 4)的870微米偏振连续谱观测表明，磁场在云团尺度(0.1-1 pc)起主导作用，而重力在核心(0.01-0.1 pc)和凝聚体(≤0.01 pc)尺度控制结构形成，且磁场张力不足以阻止气体向致密核心坍缩。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)观测大质量分子云团(Clump 1和Clump 4)中磁场与重力在多尺度结构形成中的动态竞争关系。

Data: 数据来源于阿塔卡马大型毫米波/亚毫米波阵列（ALMA）对20 km s$^{-1}$云中两个大质量团块（Clump 1和Clump 4）的870微米线性偏振连续谱观测，分辨率约为0.2角秒（2000天文单位）。

Method: 采用角分散函数（ADF）方法分析ALMA观测的870微米线性偏振连续谱数据，推导出磁场强度。

Result: 利用角弥散函数（ADF）方法测得两个团块（Clump 1和Clump 4）的磁场强度范围为约0.3至3.1毫高斯（mG）。
...
**TL;DR**: 在再电离时期(EoR)的引力透镜星系A2744-z7DLA（红移z≈6.87）中首次发现同时存在莱曼阿尔法(Lyα)发射和阻尼莱曼阿尔法吸收(DLA)，揭示了其致密中性氢环境、潜在活动星系核(AGN)特征（黑洞质量2.90×10⁷太阳质量）以及以恒星形成主导的紫外-光学光谱能量分布。


<details>
  <summary>Details</summary>

Background: 莱曼阿尔法(Lyα)吸收轮廓作为再电离时期(EoR)恒星形成星系周围延展致密中性氢气体的示踪剂。

Data: 数据来源于JWST（James Webb Space Telescope）和HST（Hubble Space Telescope）的光谱与测光数据。

Method: 采用莱曼阿尔法(Lyα)吸收谱线分析、光谱能量分布(SED)拟合以及哈勃空间望远镜(HST)和詹姆斯·韦伯太空望远镜(JWST)的光谱与测光数据联合建模方法。

Result: A2744-z7DLA星系在再电离时期(EoR)展现出莱曼阿尔法(Lyα)发射和阻尼莱曼阿尔法吸收特征，表明其周围存在致密中性氢环境。
...
**TL;DR**: 研究发现邻近星系中心环（central rings）的分子气体质量和恒星形成率（SFR）与宿主星系整体性质（如棒旋结构形态）相关，其典型半径为400±250 pc、分子气体质量log(Mmol/M⊙)∼8.1±0.2、SFR为0.21±0.16 M⊙/yr，且银河系中央分子区（CMZ）在相对参数上与这些中心环相似，表明邻近星系中心环的气体流入和恒星形成过程具有共性。
...
Data: 数据来源于PHANGS-ALMA项目提供的1角秒分辨率的CO(2-1)观测数据，并从中视觉识别了20个中心环结构（central rings），其中14个环具有恒星形成率（SFR）表面密度图。

Method: 采用1角秒分辨率的PHANGS-ALMA CO(2-1)观测数据，通过目视识别20个中心环并测定其几何结构、分子气体质量（molecular gas mass）、恒星形成率（SFR）及消耗时间（depletion time），并与宿主星系及棒结构性质进行关联分析。

Result: 中心环（central rings）的分子气体质量约为$\log(M_\text{mol}/M_\odot){\sim}8.1_{-0.23}^{+0.17}$，恒星形成率为$0.21_{-0.16}^{+0.15}\,M_\odot/\text{yr}$，分别贡献宿主星系分子气体质量和恒星形成率的$5.6_{-2.1}^{+4.5}\,\%$和$13_{-5}^{+10}\,\%$。
...
**TL;DR**: 研究221个850微米选亚毫米星系(SMG)发现，其典型恒星质量为$10^{11}$ M$_\odot$，主要在大爆炸后1.68吉年开始形成，经历约1吉年演化进入SMG阶段，并在0.2吉年内转为静止，且质量越大的星系成熟越早，对宇宙恒星形成率密度贡献在红移$z=2.5-3.5$时达50-60%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于亚毫米波星系（submillimeter galaxies, SMGs）的恒星形成历史（star formation histories, SFHs）、光谱能量分布（spectral energy distributions, SEDs）及红外-射电相关性（infrared-radio correlation, IRRC），旨在揭示其宇宙学演化规律和质量分布特征。

Data: 研究数据来源于SSA22深场中221个850微米波段选出的亚毫米波星系（SMGs）的光谱能量分布（SEDs）、恒星形成历史（SFHs）和红外-射电相关性（IRRC）分析。

Method: 采用光谱能量分布(SED)拟合、恒星形成历史(SFH)重建和红外-射电相关性(IRRC)分析技术。
...
**TL;DR**: 在SSA22天区对850微米选亚毫米星系(SMG)的多波段分析发现，这些星系的中位红移为2.00±0.08、红外光度中值为(2.25±0.25)×10¹²L⊙，63%为超亮红外星系(ULIRG)，可作为高红移宇宙结构形成的可靠示踪体，并观测到质量下降(downsizing)现象。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用多波段分析方法探究SSA22天区中850微米选亚毫米星系（SMGs）的物理特性及其在宇宙结构形成中的示踪作用。

Data: 数据来源于JCMT/SCUBA-2在SSA22天区的最深观测（灵敏度σ850∼0.80mJy beam−1），通过多波长分析筛选出850μm波段流量（deblended S850≳1mJy）的192个SCUBA-2源对应的248个SMG候选体。

Method: 采用多波长分析（multiwavelength analysis）结合光谱能量分布拟合（SED fitting）方法，并利用JCMT/SCUBA-2在850微米波段的深度观测数据进行亚毫米星系（SMG）候选体的去混叠（deblending）和多重性统计。

Result: SSA22区域SMGs（亚毫米波星系）的中位红外光度为(2.25±0.25)×10¹² L⊙，其中约63%（约8%）属于ULIRGs（超亮红外星系）或HLIRGs（极亮红外星系）。
...
**TL;DR**: 河外星系行星状星云（xPNe）作为离散示踪天体，可在大至100兆秒差距范围内直接探测星系晕和星系际光的组装过程，并用于研究星系中心到外围的元素丰度、运动学特征及恒星种群年龄和金属丰度。
...
Data: 数据来源于河外星系行星状星云（xPNe）的观测，利用10米级望远镜的新型宽视场光谱仪和全景积分场光谱技术获取。

Method: 采用全景积分视场光谱技术（panoramic integral-field spectroscopy）结合10米级望远镜的新型宽视场光谱仪器，对河外星系行星状星云（xPNe）进行直接、单一探测和表征。

Result: 河外星系行星状星云（xPNe）是研究不同形态和质量星系从中心到晕（halo）的元素丰度与运动学的离散示踪体，也是识别晕和星系团内光（intracluster light）来源的直接恒星探针。
...
**TL;DR**: 研究发现恒星形成致密星系群（SFCGs）中的星系比场星系具有更高的特定恒星形成率（sSFR），约16%的SFCG星系呈现合并特征且不对称性增强，这些合并星系的恒星形成率（SFR）显著高于其他SFCG类型和场星系，表明SFCGs代表致密星系群（CGs）的早期演化阶段，其合并活动会促进而非抑制恒星形成。


<details>
  <summary>Details</summary>

Background: 研究聚焦于前处理（pre-processing）场景下环境对致密星系群（Compact Groups）中星系物理和形态特性的影响，特别是恒星形成致密星系群（SFCGs）作为蓝移入群（Blue Infalling Group）类似体的演化机制。

Data: 数据来源于DECaLS巡天图像，用于提取形态学参数（如塞尔西克指数n、有效半径Re、基尼系数、M20和不对称性），同时使用测光技术估算恒星质量和恒星形成率（SFR），并将所有测量结果与场星系对照样本进行比较。

Method: 采用光度测量技术获取恒星质量和恒星形成率（SFR），并通过DECaLS图像提取形态参数（包括使用GALFITM拟合的Sérsic指数$n$和有效半径$R_{\mathrm{e}}$，以及非参数指标如基尼系数、$M_{20}$和不对称性），以此对星系进行E/S0/Sa、Sb/Sc/Ir及并合类型的分类。

Result: 在恒星形成致密星系群(SFCGs)中，星系表现出比场星系更高的特定恒星形成率(sSFR)，且约16%的星系呈现合并特征并伴随不对称性增强和恒星形成率提升，表明该阶段星系合并会促进而非抑制恒星形成。
...
**TL;DR**: 通过分析四个邻近星系（NGC 628、NGC 4449、M51和M83）中年轻星团（YSCs）的角两点相关函数（TPCF），发现新形成星团（eYSCs）和年龄小于10 Myr的光学星团（oYSCs）具有显著层级结构，且其二维分形指数（D_2）与普适恒星形成过程的理论值（D_2~1.3）更吻合，而10-300 Myr的星团空间分布逐渐随机化。


<details>
  <summary>Details</summary>

Background: 研究探讨了利用角两点相关函数（TPCF）分析近邻星系中年轻星团（YSCs）的空间分布，以揭示其潜在的层次结构特征。

Data: 数据来源于JWST（James Webb Space Telescope）红外波段识别的新形成阶段年轻星团（eYSCs）目录和HST（Hubble Space Telescope）档案光学数据中检测的年轻星团（YSCs），覆盖了四个目标星系（NGC 628、NGC 4449、M51和M83）。

Method: 采用角两点相关函数（TPCF）分析年轻星团（YSCs）的空间分布，并计算二维分形指数（fractal index）D₂以量化其层次结构。

Result: 年轻星团（YSCs）在年龄小于10 Myr时表现出明显的层级结构（hierarchical structuring），且红外波段新形成星团（eYSCs）与光学波段年轻星团（oYSCs）的角两点相关函数（TPCF）相似，但NGC 628中二者TPCF差异显著表明随机化时标更短。
...
**TL;DR**: 通过奥瑞加(Auriga)宇宙流体动力学模拟发现，重子物质使银河系亚晕(subhalo)丰度降低1.9-2.4倍、最大圆周速度浓度降低约1.5倍，显著影响暗物质(DM)间接探测约束条件。


<details>
  <summary>Details</summary>

Background: 研究聚焦于重子物质（baryons）对银河系尺度系统中子晕（subhalo）群体的影响及其在暗物质（DM）间接探测中的关键作用。

Data: 研究使用了Auriga项目的高精度宇宙学流体动力学模拟（cosmological hydrodynamical simulations）数据，包括银河系尺度系统的模拟及其对应的纯暗物质（DM-only）对照模拟，并通过重新填充低质量子晕（subhalos）来扩展原始数据集。

Method: 采用奥瑞加(Auriga)宇宙学流体动力学模拟和暗物质(DM-only)对照模拟，结合低质量子晕(低于质量分辨率极限)的重新填充技术，并考虑两种子晕抗潮汐剥离能力("脆弱型"和"坚韧型")的假设情景。

Result: 重子物质使银河系亚晕（subhalo）总体丰度降低约2.4倍（脆弱型）和1.9倍（抗扰型），并使其最大圆周速度定义的浓度降低约1.5倍。
...
**TL;DR**: 在Abell 370透镜星系团后方发现了一个高放大率、低质量(z=5.96)的伪小红点星系(Pseudo-LRD-NOM)，其光谱特征表明存在一个中等尘埃消光(E(B-V)=0.18-0.45)、高气体密度(n_H≥10^6 cm^{-3})、极低金属丰度(Z=0.01-0.1 Zsun)的星暴环境，并含有一个质量2.9×10^6 M⊙的活跃黑洞，可能是真实小红点星系(LRD)的前身天体。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(z=5.96)低质量星系Pseudo-LRD-NOM的极端物理条件及其与早期宇宙中黑洞生长和化学增丰过程的关系。

Data: 数据来源于JWST（James Webb Space Telescope）的NIRSpec（Near-Infrared Spectrograph）光谱观测和Abell 370引力透镜团对Pseudo-LRD-NOM的高放大率成像。

Method: 采用JWST近红外光谱仪（NIRSpec）分析Hα和Hβ发射线特征，结合高气体密度（n_H ≥ 10^6 cm^{-3}）、极低金属丰度（Z = 0.01-0.1 Zsun）和适度色余（E(B-V)=0.18-0.45）的恒星爆发模型解释光谱特性。

Result: Pseudo-LRD-NOM（伪小红点无金属线星系）的光谱特征表明其存在一个质量为2.9x10^6太阳质量的活动黑洞，并揭示了该星系处于化学富集早期阶段的高密度、尘埃遮蔽的星暴环境中。
...
**TL;DR**: 首次将卡皮查势（Kapitza potential）引入玻色-爱因斯坦凝聚态（BEC）暗物质模型，通过结合薛定谔-泊松方程基态平衡解与卡皮查-BEC相互作用，成功模拟了具有中心核区与尾部分布的星系结构，其预测与SPARC星表星系旋转曲线观测数据吻合良好。


<details>
  <summary>Details</summary>

Background: 研究主题为探索玻色-爱因斯坦凝聚态(BEC)中的卡皮查势(Kapitza potential)与暗物质模型的关联性及其在星系旋转曲线中的适用性。

Data: data：使用SPARC星表（Spitzer Photometry and Accurate Rotation Curves）中的代表性星系旋转曲线数据。

Method: 采用薛定谔-泊松方程(Schroedinger-Poisson)基态平衡解构建星系核心区密度分布，并在外围区域引入类卡皮查势(Kapitza potential)的玻色-爱因斯坦凝聚体(BEC)相互作用模型。

Result: 在玻色-爱因斯坦凝聚态(BEC)暗物质模型中，结合薛定谔-泊松方程的基态平衡解与卡皮查(Kapitza)式相互作用，能够合理拟合SPARC星表中的典型星系旋转曲线。
...
**TL;DR**: 通过JWST/NIRSpec PRISM观测确认了红移(redshift)z=13.53的极亮星系PAN-z14-1，其大物理尺寸(233±10 pc)和缺乏强发射线的特征支持了z>10星系存在两种不同星族(star-formation rate surface density区分)的新观点。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（詹姆斯·韦伯太空望远镜）观测和分析极高红移（$z>10$）星系的性质多样性，特别是探索宇宙早期星系形成与演化的两种潜在不同星族。

Data: 数据来源为JWST（James Webb Space Telescope）的NIRSpec（Near-Infrared Spectrograph）PRISM（低分辨率棱镜模式）观测和NIRCam（Near-Infrared Camera）纯平行成像。

Method: 采用JWST/NIRSpec PRISM观测数据，通过建模莱曼断裂(Lyman-α break)确认红移，并分析紫外连续谱斜率(UV-continuum slope)、物理尺寸(physical size)及静止帧紫外发射线(rest-frame UV emission lines)的探测上限。

Result: PAN-z14-1是一个红移(redshift)为13.53的极亮（$M_{\rm UV}=-20.6\pm0.2$）、具有蓝色UV连续谱斜率（$β=-2.26\pm0.08$）和大物理尺寸（$r_{\rm c}=233\pm10\, \rm pc$）的星系，与红移14.18的星系GS-z14-0物理性质相似，代表了一类在$z>10$时扩展、明亮且缺乏强发射线的正常恒星形成星系。
...
**TL;DR**: 针对欧几里得(Euclid)任务中弱引力透镜测量对点扩散函数(PSF)校正的精度需求，提出了一种结合物理驱动和数据驱动的混合方法（模板通量校正与高斯过程），通过重构55个波长的星系光谱能量分布(SED)，验证其在特定红移区间满足PSF四极矩误差要求，并评估了斯巴鲁(Subaru)望远镜新增16个中等带宽滤波器对提升SED重建精度的潜力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升欧几里得(Euclid)任务中弱引力透镜(weak lensing)测量的精度，通过优化星系光谱能量分布(SED)重建方法以校正点扩散函数(PSF)的系统误差。

Data: 数据来源包括欧几里得(Euclid)任务中的宽波段和中波段辅助测光数据，以及斯巴鲁(Subaru)望远镜新增的16个均匀分布的中波段滤光片提供的准光谱覆盖数据。

Method: 采用基于物理模型和数据驱动相结合的方法，重点运用模板流量校正和高斯过程（Gaussian processes）的新方法，并引入光谱能量分布（SED）度量指标来评估其对点扩散函数（PSF）四极矩误差的偏差传递。

Result: 高斯过程（Gaussian processes）和模板拟合仅在特定但互补的红移区间满足要求，因此提出了一种结合两种方法的混合方案，该方案在大部分红移范围内满足欧几里得（Euclid）任务对点扩散函数（PSF）模型的精度要求。
...
**TL;DR**: 通过对高红移星系团SPT-CL J2215-3537（z=1.16）的全面再分析，首次在z>1时检测到核心晃动冷锋对（core-sloshing cold fronts）的温度各向异性，并发现其处于自调节冷却-反馈循环前的短暂过渡阶段，同时揭示了传统X射线形态参数在高红移动态分类中的偏差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（high-redshift）星系团SPT-CL J2215-3537（z=1.16）的动力学状态与星系团内介质（intracluster medium）的热力学及形态学特性。

Data: 数据来源于钱德拉(Chandra)望远镜对高红移星系团SPT-CL J2215-3537 (z = 1.16)的全套可用观测数据。

Method: 采用钱德拉（Chandra）X射线观测数据，结合热力学和形态学分析技术，检测了核心晃动冷锋（core-sloshing cold fronts）和幽灵空腔（ghost cavity），并重新计算了X射线形态学参数以评估星系团动力学状态。

Result: 首次在红移z>1的星系团SPT-CL J2215-3537中探测到核心晃动冷锋对，并发现其处于辐射冷却显著超过活动星系核反馈加热的短暂过渡阶段。
...
**TL;DR**: 研究发现早期暗能量（EDE）模型能解释JWST观测到的高红移（$z \sim 11-6$）星系过量现象，并在仅需中等莱曼连续谱（LyC）逃逸分数（$f_{\rm esc}\sim 0.05-0.1$）和紫外光度密度（$M_{\rm UV}\lesssim -17$至$-15$）条件下，使再电离历史符合当前观测约束。
...
Data: 数据来源于JWST（James Webb Space Telescope）对红移$z \sim 11-6$范围内莱曼α（Lyman-$α$）发射的观测以及基于早期暗能量（Early Dark Energy, EDE）范式的模拟建模。

Method: 采用早期暗能量（Early Dark Energy, EDE）模型模拟再电离历史，结合中等莱曼连续谱光子逃逸分数（LyC escape fraction, $f_{\rm esc}\sim 0.05-0.1$）和紫外光度密度（UV luminosity density, $M_{\rm UV}\lesssim -17$ 至 $-15$）约束。

Result: 早期暗能量（EDE）模型在仅需中等莱曼连续谱光子逃逸率（$f_{\rm esc}\sim 0.05-0.1$）和紫外光度密度（$M_{\rm UV}\lesssim -17$至$-15$）条件下，即可产生与当前观测约束一致的再电离历史。
...
**TL;DR**: 基于兹威基瞬变设施巡天(ZTF)的Ia型超新星(SNe Ia)观测数据，开发了一种测量哈勃常数(H0)各向异性（dipole）的无偏方法，并验证了该方法能准确恢复模拟中引入的3 km s−1 Mpc−1幅度各向异性，其振幅误差为0.33 km s−1 Mpc−1、方向误差为赤经3.4°和赤纬6.1°。


<details>
  <summary>Details</summary>

Background: 宇宙学原理（cosmological principle）假设宇宙在大尺度上是各向同性的，这是当前标准宇宙学模型ΛCDM的基础假设。

Data: 数据来源于兹威基瞬变设施巡天(ZTF)的第二批数据释放(DR2)中的Ia型超新星(SNe Ia)观测数据及其模拟数据。

Method: 采用基于ZTF Ia型超新星(SNe Ia)第二数据发布(DR2)模拟数据的偶极子(dipole)恢复方法，通过构建误差模型量化偶极子振幅(ΔH₀)及其方向(α₀, δ₀)的测量精度。

Result: 开发了一种测量哈勃常数（H0）各向异性（anisotropy）的方法，能够以0.33 km/s/Mpc的精度恢复初始振幅为3 km/s/Mpc的偶极（dipole），且方向误差为赤经3.4°、赤纬6.1°。
...
**TL;DR**: 提出基于高斯过程回归(GPR)的BOBE方法，通过贝叶斯优化构建快速替代似然函数，将宇宙学模型证据计算所需直接似然评估次数减少约10^3倍，特别适用于耗时超过1秒的"昂贵"似然函数场景。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用贝叶斯模型选择（Bayesian model selection）框架高效评估物理模型与观测数据的兼容性，特别是解决计算贝叶斯证据（Bayesian evidence）时因高维积分和慢速似然函数评估导致的效率瓶颈问题。

Data: 研究中使用的数据来源包括模拟数据（simulation data）和实际宇宙学似然函数（actual cosmological likelihoods）的观测数据。

Method: 采用基于高斯过程回归（Gaussian Process Regression, GPR）的贝叶斯优化算法构建快速模拟似然函数（emulated likelihood），以最小化直接计算贝叶斯证据（Bayesian evidence）所需的高成本似然函数评估次数。

Result: 提出的BOBE方法通过高斯过程回归(GPR)构建模拟器，将贝叶斯证据计算所需似然函数调用次数减少至直接嵌套采样方法的千分之一量级。
...
**TL;DR**: 通过分析费米卫星(Fermi-LAT)15年数据与KiDS-Legacy弱引力透镜剪切数据的交叉关联，未发现显著信号，据此给出了弱相互作用大质量粒子(WIMP)衰变率和湮灭截面的95%上限约束，并预测未来欧几里得(Euclid)透镜巡天与费米数据联合分析可将限制提高约2倍。


<details>
  <summary>Details</summary>

Background: 研究主题为通过未解析伽马射线背景（UGRB）与弱引力透镜的互相关分析，约束暗物质（WIMP）的衰变率和湮灭截面等性质。

Data: 数据来源于费米大面积望远镜(Fermi-LAT)15年观测数据构建的未解析伽马射线背景(UGRB)强度图（10个能量区间，0.5-1000 GeV）与KiDS-Legacy巡天弱引力透镜剪切数据的六层断层扫描分箱的交叉关联分析。

Method: 采用伪-$C_\ell$方法（pseudo-$C_\ell$ method）估计角功率谱来分析未解析伽马射线背景（UGRB）与弱引力透镜（KiDS-Legacy shear）的交叉关联。

Result: 未发现伽马射线背景（UGRB）与弱引力透镜（KiDS-Legacy）之间存在显著互相关信号。
...
**TL;DR**: 研究了动量传递耦合的暗能量(Dark Energy)和暗物质(Dark Matter)对星系团数量计数的影响，发现动量传递增强会抑制线性密度对比度(linear density contrast)并提高高红移区间的星系团数量，但统计结果显示该耦合模型与标准ΛCDM模型相比无显著差异。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗能量(dark energy)与暗物质(dark matter)间存在动量转移耦合作用对星系团数量统计的影响。

Data: 数据来源包括eROSITA巡天观测结果和基于球塌缩模型（spherical collapse model）及Sheth-Tormen质量函数（Sheth-Tormen mass function）的理论预测数据。

Method: 采用球塌缩模型（spherical collapse model）计算外推线性密度对比度，并结合Sheth-Tormen质量函数（Sheth-Tormen mass function）计算星系团数目统计。

Result: 动量耦合暗能量模型下星系团数量在最大红移区间增加，但统计上未显著优于标准ΛCDM模型。
...
**TL;DR**: 研究发现振荡子（oscillon）快速衰变可增强诱导引力波产生，其能量密度可达相对论性粒子有效数目的观测上限，从而为允许振荡子形成的暴胀子（inflaton）质量、三次和四次自耦合参数提供了互补性约束。


<details>
  <summary>Details</summary>

Background: 研究探讨了暴胀后宇宙中振荡暴胀子凝聚体的碎裂形成振荡子（oscillon）及其导致的早期物质主导阶段对引力波产生和相对论性粒子数目的影响。

Data: 数据来源于数值模拟（simulation）方法对振荡子（oscillon）形成和衰变过程的模拟。

Method: 采用数值模拟和解析计算相结合的方法分析振荡子（oscillon）动力学及其诱导的引力波产生机制。

Result: 振荡子（oscillon）的快速最终衰变导致诱导引力波（induced gravitational waves）产生增强，其能量密度可达到相对论性粒子有效数目的观测上限。
...
**TL;DR**: 加纳射电天文台（GRAO）通过将退役的32米卫星通信天线改造成具备甚长基线干涉测量（VLBI）能力的射电望远镜，实现了非洲射电天文学的重大突破，其早期科学观测验证了该设施在甲醇脉泽、脉冲星计时和VLBI联测中的高性能表现。


<details>
  <summary>Details</summary>

Background: 非洲射电天文发展背景下，加纳射电天文台（GRAO）通过改造退役卫星通信天线成为VLBI-ready（甚长基线干涉测量就绪）设备，填补了赤道附近全天空覆盖观测能力的空白，推动了非洲参与全球射电天文合作及平方公里阵列（SKA）前期准备。

Data: 数据来源包括加纳射电天文台（GRAO）的32米天线改造后的实测数据（如6.7 GHz甲醇脉泽谱线观测、Vela脉冲星计时、VLBI条纹检测）以及模拟（simulation）和台站测试（commissioning tests）结果。

Method: 采用双极化C波段接收机（5和6.7 GHz）、氢脉泽、宽带ROACH2系统及改进的控制与指向基础设施进行射电天文观测。

Result: 成功将退役的32米卫星通信天线改造为具备甚长基线干涉测量（VLBI-ready）能力的射电望远镜，验证了加纳射电天文台（GRAO）在独立运行和网络协作中的科学能力。
...
**TL;DR**: 研究通过系统评估不同训练集缺陷对四种光红移(photo-z)估计算法的影响，发现Kullback-Liebler散度、Wasserstein距离和概率积分变换是最能有效量化训练集缺陷影响的指标，并证实仅逆向红移不完整性不足以模拟真实训练数据的复杂性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于评估训练集不完整性（training set imperfection）对LSST（大型综合巡天望远镜）光变红移（photo-$z$）估计算法性能的影响及其诊断指标优化。

Data: 数据来源于模拟训练数据（mock training data），这些数据通过降质处理模拟了多个现有光谱巡天（spectroscopic sky surveys）以及高红移处观测到的不完整性模式（inverse redshift incompleteness）。

Method: 采用开源Python库Redshift Assessment Infrastructure Layers (RAIL)中的CMNN、GPz、FlexZBoost和PZFlow算法，结合TrainZ作为对照，在模拟训练数据上系统测试不同光谱巡天数据退化条件下的性能，并利用外部实现的Kullback-Liebler Divergence、Wasserstein Distance和Probability Integral Transform等统计指标量化算法表现。

Result: 通过系统测试多种光红移(photo-z)算法在不同训练集缺陷下的表现，确定Kullback-Liebler散度、Wasserstein距离和概率积分变换是最能有效评估训练集缺陷影响的指标。
...
**TL;DR**: 提出了一种通过共享相邻光谱像素信息来表征时变观测的新统计模型，能无偏且最优地提取地面近红外(NIR)探测器受大气变化影响的时变信号，当计数率变化超过3.5%时优于恒定通量模型，并实证验证了APOGEE光谱中存在幅度显著超过该阈值的波长无关时变效应。


<details>
  <summary>Details</summary>

Background: 近红外(NIR)探测器在现代天体物理学中发挥关键作用，但地面观测面临短时标大气变化导致光子数显著波动的挑战。

Data: 数据来源于模拟生成的多种通量和时间变异性条件下的合成数据（synthetic data）以及真实的APOGEE光谱（real APOGEE spectra）。

Method: 采用基于邻近光谱像素信息共享的统计模型（statistical model）来表征时变观测并提取无偏通量。

Result: 提出了一种利用相邻光谱像素共享信息的新统计模型，能有效表征时变观测并提取无偏通量（flux）和最优不确定度，当观测计数率变化超过3.5%时该模型优于恒定通量模型。
...
**TL;DR**: 提出了一种基于辐射波束方向图直接推导均匀相控阵有源阻抗（active impedance）的新方法，将扫描角度变化与波束固有角度变化直接关联，为相控阵设计提供了更直观的物理理解和计算简化。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大型均匀相控阵天线(phased array antennas)中主动阻抗(active impedance)的物理机制与计算方法优化。

Data: 数据来源于对原型阵列的全波仿真（full-wave simulation）。

Method: 采用基于辐射波束图（radiated beam pattern）的直接推导方法计算有源阻抗（active impedance），建立了扫描角度变化与波束固有角度变化的直观映射关系。

Result: 提出了一种基于辐射波束方向图直接推导有源阻抗(active impedance)的新方法，建立了更直观的扫描角度依赖性分析框架。
...
**TL;DR**: 该研究通过评估硅光电倍增管(Silicon Photo-Multipliers)在极端温度下的性能参数（击穿电压、淬灭电阻等），为POEMMA-Balloon任务中的切伦科夫相机(Cherenkov Camera)选定了Hamamatsu S13361-3050系列传感器，并建立了标准化测试协议。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发用于探测大气中粒子簇射产生的切伦科夫辐射（Cherenkov emission）和荧光辐射（fluorescence emission）的硅光电倍增管（Silicon Photo-Multipliers）探测器，以支持下一代气球载和空间望远镜实验。

Data: 数据来源包括对Hamamatsu S13161-3050AE-08传感器阵列在30°C至-40°C温度范围内的实验测量，以及对32个Hamamatsu S13361-3050系列硅光电倍增管（SiPM）的大规模测试验证。

Method: 采用硅光电倍增管(SiPM)性能表征协议，在-40°C至30°C温度范围内测量击穿电压、淬灭电阻、增益、暗计数率和串扰概率等关键参数。

Result: result：硅光电倍增管(SiPM)在-40°C至30°C温度范围内表现出关键参数(击穿电压、淬灭电阻、增益、暗计数率和串扰概率)随温度变化的特性，为POEMMA-Balloon任务中切伦科夫相机(Cherenkov Camera)的2048像素焦平面(Focal Surface)提供了传感器验证与校准基础。
...
**TL;DR**: 该研究设计了一种基于亚克力菲涅耳透镜（acrylic Fresnel lenses）的移动强度干涉仪（Mobile Intensity Interferometer for Stellar Observations），通过测量大角星（Arcturus）的空间相干曲线，在11小时内以毫角秒精度验证了其测角能力，为针对特定目标（如双星系统）的紫外（u-v）平面选择性覆盖提供了紧凑、经济且可扩展的解决方案。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用强度干涉仪（intensity interferometry）技术进行恒星观测，特别是在切伦科夫望远镜阵列（Imaging Atmospheric Cherenkov Telescope arrays）无法执行伽马射线观测的明亮月相期间，探索其作为专用设备的可行性及应用潜力。

Data: 数据来源于使用直径1米、焦距1.2米的丙烯酸菲涅耳透镜（acrylic Fresnel lenses）构建的移动强度干涉仪（Mobile Intensity Interferometer for Stellar Observations），通过调整z轴位置适应不同波长测量，并完整替换探测器组件，首次观测活动测量了大角星（Arcturus）的空间相干曲线（spatial coherence curve）。

Method: 采用直径1米、焦距1.2米的丙烯酸菲涅耳透镜(Fresnel lens)实现紧凑轻量化设计，通过z轴平移调节不同波长测量焦点，并支持快速更换探测器模块。

Result: 使用直径1米、焦距1.2米的丙烯酸菲涅耳透镜实现了紧凑、经济且轻量化的专用强度干涉仪（Mobile Intensity Interferometer for Stellar Observations）设计，首次观测活动在11小时内以毫角秒精度测量了大角星（Arcturus）的空间相干曲线，结果与文献值一致。
...
**TL;DR**: 该研究利用引力波干涉仪应变瞬态信号训练的神经网络编码器，在未经重新训练或微调的情况下，成功实现了对滚动轴承故障的零样本异常检测，并揭示了该方法对相干弹性瞬态结构的敏感性及其在电气主导振动信号中的物理边界。


<details>
  <summary>Details</summary>

Background: 研究探索了从引力波观测站的干涉仪应变瞬变信号中学习到的表征能否作为形态敏感的冻结算子（frozen operator）迁移至未见传感器，前提是目标信号保持相干弹性瞬变结构。

Data: 数据来源于引力波观测站的干涉仪应变瞬态信号（interferometric strain transients）、IMS-NASA轴承运行至失效数据集（run-to-failure dataset）、凯斯西储大学轴承数据集（CWRU）以及维也纳工业大学的电气主导振动信号数据集（VSB）。

Method: 采用神经网络编码器（neural encoder）对非高斯仪器故障（non-Gaussian instrumental glitches）进行训练，构建干涉应变瞬态（interferometric strain transients）的形态敏感算子，用于零样本异常分析。

Result: 在IMS-NASA运行至失效数据集中，该算子生成的单调健康指数HI(t) = s0.99(t)/tau实现了固定误报率监测（1-q = 1e-3），其中tau = Q0.999(P0)。
...
**TL;DR**: Mini-EUSO（Mini-Extreme Universe Space Observatory）作为首个运行于国际空间站(ISS)的JEM-EUSO计划探测器，通过五年观测积累了750小时数据，首次系统性实现了空间基流星体普查（检出22,000余颗流星及3个星际候选体）、全球紫外辐射地图绘制及闪电现象（如多层同心环状高空发光事件elves）动态研究，同时为超高能宇宙线(UHECRs)通量上限测定及未来空间观测奠定技术基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用国际空间站(ISS)搭载的Mini-EUSO探测器探索极端高能宇宙射线(UHECRs)及地球高层大气瞬态发光现象(TLEs)，旨在填补超高能宇宙射线通量上限的观测空白并推动未来空间观测站发展。

Data: 数据来源于国际空间站（ISS）上运行的Mini-EUSO探测器自2019年10月以来收集的约750小时观测数据，包括全球紫外（UV）辐射地图、闪电现象、瞬态发光事件（TLEs）、人造光源、流星以及超高能宇宙射线（UHECRs）候选事件。

Method: 采用紫外成像技术（UV imaging）和快速时间分辨率（fast time resolution）观测系统，结合国际空间站（ISS）的有利观测几何条件，对地球大气层中的瞬态发光事件（TLEs）、流星及人工光源进行系统性监测与分析。

Result: Mini-EUSO首次在空间站(ISS)上实现了对超高能宇宙射线(UHECRs)的探测，并绘制了全球首幅地球紫外辐射地图。
...
### [35] [Rescaling Transforms for Local Models of Spherical Flows](https://arxiv.org/abs/2601.11337)
*Elliot M. Lynch,Guillaume Laibe*

**TL;DR**: 将宇宙结构形成研究中的超共动变量(super-comoving variables)推广到局部球面流模型，通过映射笛卡尔流解获得广泛解析解，并揭示球面流中线性不稳定性(linear instabilities)具有时变增长率且小尺度各向同性收缩/膨胀流可映射为无粘不可压流体动力学。


<details>
  <summary>Details</summary>

Background: 研究聚焦于球对称收缩/膨胀气体云（spherically contracting/expanding gas cloud）的局部模型构建及其湍流与小尺度不稳定性分析，通过推广超共动变量（super-comoving variables）方法简化解析解推导与复杂流动背景相互作用研究。

Data: 研究中使用的数据来源于理论推导和数值模拟（simulation），未涉及实际观测数据。

Method: 采用超共动变量（super-comoving variables）将宇宙学结构形成研究中的方法推广到局部球对称流动模型，便于解析求解和分析复杂流动与背景的相互作用。

Result: 将超共动变量(super-comoving variables)推广到局域球对称流动模型中，显著简化了复杂流动与背景相互作用的解析求解和分析过程。
...
### [36] [Checking It Twice: Using [C/N]-Masses and Asteroseismic Masses as a Diagnostic of Mass Loss and Transfer on the RGB](https://arxiv.org/abs/2601.10800)
*John D. Roberts,Marc H. Pinsonneault,Jennifer A. Johnson,Madeline Howell*

**TL;DR**: 红巨星表面碳氮比([C/N])与诞生质量相关且不受质量损失直接影响，通过比较相同[C/N]但不同演化状态（红巨星分支初期与红团簇阶段）的星震学质量，发现净红巨星分支质量损失强度存在整体差异，其随金属丰度和质量增加而降低，与Reimers质量损失公式预期相矛盾，据此提出了质量与金属丰度依赖的Reimers η参数校准方案，并识别出207颗(3.33%)可能经历质量转移的异常星体。


<details>
  <summary>Details</summary>

Background: 研究主题为利用红巨星表面碳氮比([C/N])与初始质量的相关性，结合星震学质量比较不同演化阶段恒星的质量损失差异，并建立与金属丰度和质量相关的Reimers质量损失参数η校准关系。

Data: 数据来源于对红巨星（red giants）的星震学（asteroseismic）质量测量，并结合相同[C/N]值但不同演化状态（红巨星分支初期和红团簇阶段）的恒星进行比较分析。

Method: 利用红巨星表面碳氮比([C/N])与初始质量相关但不受质量损失直接影响的特点，通过比较相同[C/N]但不同演化阶段（红巨星分支和红团簇）的星震学质量差异，直接约束了场星中红巨星分支净质量损失的强度。

Result: 红巨星表面碳氮比([C/N])与诞生质量相关，但与质量损失无直接关联。
...
### [37] [Unveiling Ionized Jet Morphologies: Sub-arcsecond VLA Observations of Compact Radio Sources](https://arxiv.org/abs/2601.11130)
*Tatiana M. Rodriguez,Peter Hofner,Emmanuel Momjian,Esteban D. Araya,Ananay Sethi,Viviana Rosero*

**TL;DR**: 通过亚角秒分辨率观测发现，23个研究对象中7个被归类为热喷流（thermal jet），16个支持电离喷流（ionized jet）解释，主要证据包括H2O脉泽（maser）与射电连续谱峰的空间分布关联、4例准直性结构及10例平均投影速度约120 km/s的自行运动。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过高分辨率射电连续谱和水脉泽（H$_2$O maser）观测，探究致密射电源和细长射电结构的形态学特征及其激波电离机制。

Data: 数据来源于甚大天线阵（VLA）1.3厘米连续谱和22.2 GHz水脉泽（H$_2$O maser）观测，针对Rosero等人（2016, 2019）巡天中15个具有上升谱指数的致密射电连续谱源和8个弦状射电连续结构。

Method: 采用甚大天线阵(VLA)1.3厘米连续谱和22.2 GHz水脉泽(H$_2$O maser)观测技术，空间分辨率达亚角秒级($θ\sim0.1^{\prime\prime}$)。

Result: 在23个目标源中观测到三种形态：6个呈拉长或双峰细丝状结构，4个为离散连续谱峰集合，13个为单一致密源。
...
### [38] [Fundamental Properties of Novae in M31](https://arxiv.org/abs/2601.11476)
*Allen W. Shafter,Kamil Hornoch*

**TL;DR**: 通过分析M31（仙女座星系）新星样本的光度峰值和衰减率，发现其白矮星（WD）质量分布呈高斯分布，平均质量为1.16±0.14 M⊙（校正后为1.07 M⊙），与银河系新星相似但比Schaefer研究的系统高约0.15 M⊙，且再发新星的平均质量（1.33±0.08 M⊙）显著高于普通新星，同时表征了吸积率、抛射物速度和再发时间等参数的偏态分布特征，但未发现新星性质与M31位置存在明显关联。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用新星模型分析M31（仙女座星系）新星样本的白矮星（WD）质量分布及其相关参数，探讨其与银河系新星系统的异同及休眠机制对吸积率和重现周期的影响。

Data: 数据来源于Clark等人近期发表的大样本新星峰值光度及衰减率观测结果，结合Yaron等人的新星模型进行分析。

Method: 采用Yaron等人的新星模型（Yaron et al. nova models）分析M31新星样本的光度峰值和衰减率以估算其基本性质。

Result: M31新星样本中白矮星（WD）的视质量分布近似高斯分布，平均质量为1.16±0.14 M⊙，经复发时间偏差校正后降至1.07 M⊙。
...
### [39] [Light Dark Matter Search with 7.8 Tonne-Year of Ionization-Only Data in XENONnT](https://arxiv.org/abs/2601.11296)
...
**TL;DR**: 利用XENONnT探测器在579天内通过7.83吨·年的总曝光量对暗物质（DM）进行盲搜，首次建立了完整的仅S2信号背景模型，在[0.5, 5.0] keVnr核反冲和[0.04, 0.7] keVer电子反冲能区未观测到显著超出背景的信号，为3-8 GeV/c²质量区间的自旋无关DM-核子散射、自旋相关DM-中子散射、DM-电子散射、类轴子粒子及暗光子设定了更严格的90%置信水平上限，其中5 GeV/c²处自旋无关DM-核子散射截面排除限达6.0×10⁻⁴⁵ cm²，使XENONnT灵敏度接近相干弹性中微子-核散射（CEνNS）不可减背景区域。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用XENONnT探测器开展暗物质（DM）的盲搜实验，探索低质量暗物质与核子或电子的相互作用，并扩展对轴子类粒子（axion-like particles）和暗光子（dark photons）的探测限制。

Data: 数据来源于XENONnT实验在579天内完成的三次科学运行中收集的总曝光量为7.83吨×年的电离信号（S2-only）数据。

Method: 采用电离信号(S2-only)分析技术结合专用背景抑制方法和首个完整的S2-only背景模型，在XENONnT中开展暗物质(DM)盲搜。

Result: 在暗物质（DM）质量3至8 GeV/c²范围内，未观测到显著超出背景的信号，并给出了自旋无关DM-核子散射截面在5 GeV/c²处低于6.0×10⁻⁴⁵ cm²的90%置信水平上限。
...
### [40] [Gravitational perturbations of nonlocal black holes](https://arxiv.org/abs/2601.11101)
*Rocco D'Agostino,Vittorio De Falco*

**TL;DR**: 该论文在修正的Deser-Woodard非局部引力理论框架下，首次完整推导了静态球对称黑洞（black hole）时空的轴向和极向引力扰动（gravitational perturbations）主方程，并将其应用于作者近期提出的偏离史瓦西几何（Schwarzschild geometry）的单参数黑洞解族，为未来研究奠定了基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于修正的德塞尔-伍德非局域引力理论（revised Deser-Woodard nonlocal gravity theory）框架下静态球对称黑洞时空的轴向和极向引力扰动。

Data: 数据来源于作者最近获得的一族单参数黑洞解（one-parameter family of black hole solutions），该解表示对史瓦西几何（Schwarzschild geometry）的一阶微小偏离。

Method: 采用修订的德塞尔-伍德沃德非局部引力理论框架，推导了静态球对称黑洞时空轴向和极向引力扰动的主控方程。

Result: 在修正的Deser-Woodard非局域引力理论框架下，首次完整建立了静态球对称黑洞时空轴向和极向引力扰动的主控方程。
...
### [41] [Rotation Period of 3I/ATLAS After Perihelion from Jet Position Angle Wobble and Photometric Variability](https://arxiv.org/abs/2601.10860)
*Toni Scarmato,Abraham Loeb*

**TL;DR**: 通过喷流位置角（PA）调制和测光（photometry）分析，确定3I/ATLAS彗星近日点后自转周期为7.1小时，并发现其喷流结构绕自转轴存在20°进动（precession）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用喷流形态（jet morphology）和测光数据（photometry）测定星际彗星3I/ATLAS的近日点后自转周期（post-perihelion rotation period），并探讨其与喷流结构进动（precession）的关系。

Data: 数据来源包括哈勃太空望远镜（Hubble Space Telescope）在2025年11月20日至12月27日期间拍摄的图像，以及一台0.25米口径望远镜（MPC L92）采集的30分钟分箱光变曲线数据。

Method: 采用拉尔森-塞卡尼纳旋转梯度滤波（Larson-Sekanina Rotational Gradient filter）处理哈勃太空望远镜图像，并通过加权最小二乘傅里叶级数拟合相位折叠的位置角曲线，同时扫描试验周期以识别χ²最小值。

Result: 通过喷流位置角（PA）调制和测光光变曲线分析，确定3I/ATLAS彗星近日点后自转周期为7.1小时，伴随喷流结构绕自转轴20°进动。
...
### [42] [KMT-2025-BLG-1616Lb: First Microlensing Bound Planet From DREAMS](https://arxiv.org/abs/2601.11469)
...
**TL;DR**: 通过KMTNet和DREAMS对KMT-2025-BLG-1616微引力透镜事件的观测分析，发现一颗质量约40倍地球质量的行星绕一颗质量约0.3倍太阳质量的主星运行，轨道投影距离约1.6天文单位，并证实DREAMS能探测银河系核球中火星质量自由漂浮行星(FFP)和银盘中月球质量自由漂浮行星(FFP)。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用微引力透镜（microlensing）技术探测银河系内行星系统（包括束缚行星和自由漂浮行星（FFP））的动力学特征和统计分布。

Data: 数据来源于韩国微引力透镜望远镜网络（KMTNet）和DECam流浪地球与火星巡天（DREAMS）的观测数据。

Method: 采用贝叶斯分析(Bayesian analysis)推断宿主恒星质量、行星质量、投影行星-宿主间距及透镜距离。

Result: 行星系统具有约5×10⁻⁴的星-行质量比（planet-host mass ratio），贝叶斯分析显示宿主恒星质量约0.3倍太阳质量（M⊙）、行星质量约40倍地球质量（M⊕）、投影星-行间距约1.6天文单位（au）、透镜距离约7.5千秒差距（kpc）。
...
**TL;DR**: 通过毫米波（millimeter-band）和光学偏振监测发现，BL Lac天体（BL Lac objects）比平谱射电类星体（FSRQs）具有更高的偏振度（fractional polarization）和更低的位置角（EVPA）变异性，且偏振度随频率增加而升高，表明短波长下磁场有序性增强，同时位置角变异性与射电/伽马射线光度正相关、与同步辐射峰值频率负相关。
...
Data: 数据来源于IRAM 30米望远镜进行的1毫米和3毫米偏振监测（POLAMI计划）以及多台设施提供的长期光学偏振观测数据。

Method: 采用贝叶斯(Bayesian)框架分析毫米波和光学偏振数据，结合圆统计方法表征电场矢量角(EVPA)变异性，并计算平均通量密度、调制指数等参数。

Result: BL Lac天体在所有波段平均表现出比FSRQs更高的偏振度（fractional polarization）和更低的位置角（EVPA）变异性。
...
**TL;DR**: 研究发现等离子体透镜效应（plasma lensing）能解释快速射电暴（FRBs）的复杂观测特征，包括频率漂移、偏振角跳跃和色散活动窗口，并可通过透镜位置限制发射区尺度。


<details>
  <summary>Details</summary>

Background: 快速射电暴（FRB）在致密磁离子环境中的传播效应及其观测信号重塑机制。

Data: data部分：研究使用一维高斯等离子体透镜（1D Gaussian plasma-lens）框架模拟快速射电暴（FRB）波前入射角变化及多路径传播效应，并基于FRB 20180916B的色散活动特征验证模型。

Method: 采用一维高斯等离子体透镜（1D Gaussian plasma-lens）模型分析快速射电暴（FRB）波前入射角微小变化对子爆发频率漂移和偏振角跃变的影响。

Result: 等离子体透镜效应可解释快速射电暴（FRB）观测中频率漂移、偏振角（PA）跃变和色散活动窗口等复杂特征，并限制发射区横向尺度。
...
**TL;DR**: 利用罗曼空间望远镜（Roman Space Telescope）的微引力透镜（astrometric microlensing）技术，首次系统模拟了孤立中子星（NS）透镜事件，发现特征参数空间（$\log_{10} t_{\rm E}$ - $\log_{10} θ_{\rm E}$）可用于高效分类中子星，并预测将观测到约100例中子星透镜事件。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用引力微透镜（gravitational microlensing）技术探测孤立中子星（isolated neutron stars），以解决恒星演化和爆发物理中的基础问题。

Data: 使用针对致密天体微透镜效应优化的先进银河系模型，模拟了四种具有麦克斯韦分布初生速度（$\bar{v} = (150, \ 250, \ 350, \ 450)$ km/s）的中子星（NS）种群数据，并应用了罗曼空间望远镜（Roman Space Telescope）的预期精度、观测频率和可探测性标准。

Method: 采用最先进的银河系模型（Galactic models）模拟了四种具有麦克斯韦分布（Maxwellian natal kick distributions）的中子星（NS）群体，并应用了罗曼望远镜（Roman）的预期精度、观测频率和可探测性标准。

Result: 利用罗曼空间望远镜（Roman Space Telescope）的观测能力，在参数空间$\log_{10} t_{\rm E}$ - $\log_{10} θ_{\rm E}$中发现了一个中子星（NS）的特征性标志，可用于高效分类恒星残骸并构建最优中子星候选样本。
...
**TL;DR**: DWF（Deeper, Wider, Faster）项目利用DECam（Dark Energy Camera）首次发布的高时间分辨率（分钟级）光变数据（覆盖166小时观测和12000张图像）及其新型数据处理流程（dwf-postpipe），在g波段峰值星等<22 AB mag的暂现源检测效率达97.24%，并发现13颗未编目变星（10颗脉动变星、2颗食双星、1颗ZZ Ceti型白矮星）及2颗UV Ceti型耀星。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于快速时标（<1小时）下瞬变（transient）和可变光学天象的表征不足问题，利用暗能量相机（DECam）通过Deeper, Wider, Faster（DWF）项目填补深度（中位g波段约22.2星等）和分钟级连续成像的参数空白。

Data: 数据来源于Deeper, Wider, Faster (DWF)项目使用暗能量相机(DECam)获取的约12000张图像和166小时望远镜观测时间的高时间分辨率测光数据，并通过交叉匹配公共星表验证测光精度。

Method: 采用了一种新颖的数据处理流程（$\texttt{dwf-postpipe}$）来识别源并提取其光变曲线，并通过将合成伽马射线暴（GRB）余辉注入图像子集来评估测光精度。

Result: 开发的新型数据处理管道（dwf-postpipe）能高效识别源并提取光变曲线，对峰值星等g<22 AB mag的暂现源恢复效率达97.24^{+0.7}_{-1.0}%，但较难恢复出现在亮度相当或更亮星系中的暂现源。
...
**TL;DR**: 该论文全面分类了张力变化的标度宇宙弦网络（scaling cosmic string networks）引力波辐射现象学，计算了相应随机背景的谱指数（spectral indices），并通过IIB型弦理论中的D3膜和NS5膜缠绕内部循环的具体例子及沼泽地（Swampland）约束推导了FLRW背景下有效弦张力时间变化和谱指数的普适界限。


<details>
  <summary>Details</summary>

Background: 研究聚焦于弦论（string theory）中模场（moduli）动态变化导致的宇宙弦（cosmic string）张力时变及其引力波（gravitational wave）辐射现象。

Data: 数据来源于对弦紧致化（string compactifications）中D3-和NS5-膜（branes）包裹内部循环的动力学模拟（simulation），并结合FLRW背景下的沼泽地约束（Swampland constraints）推导有效弦张力（effective string tension）的时间变化。

Method: 基于弦紧致化（string compactifications）和沼泽地约束（Swampland constraints），采用有效弦张力（effective string tension）时变模型分析引力波（gravitational wave）随机背景的谱指数（spectral indices）。

Result: 引力波（gravitational wave）发射现象学在张力变化的宇宙弦网络（cosmic string networks）中被完全分类，并计算了相应随机背景的谱指数（spectral indices）。
