...
**TL;DR**: 构建了一个一维自洽稳态无碰撞自引力系统模型，发现具有相同能量的粒子轨道位置和速度可由正弦和余弦函数泰勒展开前两项精确描述，并提出了冷暗物质模拟中密度、势场和轨道行为的新猜想。


<details>
  <summary>Details</summary>

Background: 研究提出了一种一维自洽稳态无碰撞自引力系统模型，其中所有粒子具有相同能量，其粒子轨道运动可通过正弦和余弦函数泰勒展开前两项精确解析表达。

Data: 研究中使用的数据来源于理论模型构建的一维自洽稳态无碰撞自引力系统（one-dimensional self consistent steady state collisionless self-gravitating system），未涉及实际观测或模拟数据。

Method: 采用一维自洽稳态无碰撞自引力系统模型（one-dimensional self-consistent steady state collisionless self-gravitating system），所有粒子具有相同能量，其轨道位置和速度由正弦和余弦函数泰勒展开前两项精确描述。

Result: 构建了一个能量均等的一维自洽稳态无碰撞自引力系统模型，其粒子轨道位置和速度可精确表示为正弦和余弦函数泰勒展开的前两项截断。
...
**TL;DR**: 基于DESI数据发布2的49,959个低红移(z < 0.96)恒星形成星系直接电子温度测量，发现了宇宙近邻区域两个极贫金属星系(氧丰度1.2-1.3%太阳值)及139个低金属丰度高氮氧比异常星系，揭示了Ne/O、S/O和Ar/O比值随金属丰度的演化规律。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过直接测量电子温度（electron temperature）获取大样本星系（49,959个）的元素丰度（N、O、Ne、S、Ar），揭示低红移（z < 0.96）星系化学演化的规律及其与金属丰度（metallicity）等性质的关系。

Data: 数据来源于DESI（Dark Energy Spectroscopic Instrument）数据发布2（Data Release 2），包含49,959个红移$z < 0.96$的恒星形成星系的直接法丰度测量。

Method: 通过直接测量多个电离区的电子温度（electron temperature），构建电子温度关系并推导N、O、Ne、S和Ar的丰度。

Result: 发现了宇宙近邻区域中两个氧丰度最低的星系（12+log(O/H)分别为6.77和6.81），并识别出139个低金属丰度下具有高氮氧比（N/O）的稀有星系群。
...
**TL;DR**: 研究发现詹姆斯·韦伯太空望远镜（JWST）可能探测到红移（redshift）值约30的极端高能瞬变事件（如对不稳定性超新星PISNe），为研究宇宙早期（大爆炸后约1亿年）第一代恒星形成提供了直接证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用詹姆斯·韦伯太空望远镜（JWST）探测宇宙早期（红移z~30）可能存在的第一代恒星形成相关的超高能瞬变事件（如对不稳定性超新星PISNe），以挑战现有理论对极早期星系存在的预测极限。

Data: 数据来源于詹姆斯·韦伯太空望远镜（JWST）的光谱观测和宇宙学模拟（cosmological simulations）。

Method: 采用宇宙学模拟（cosmological simulations）评估高红移（high-redshift）区域中成对不稳定性超新星（PISNe）的发生概率及其观测通量。

Result: JWST可能在极高红移（$z\sim30$）探测到成对不稳定超新星（PISNe），为研究第一代恒星形成提供直接证据。
...
**TL;DR**: 通过分析SDSS DR16和DESI早期数据发布的约110万条类星体光谱，发现了1345个独特的类星体背景前景星系叠加系统（GOTOQs），覆盖红移范围0.0045≤z≤1.09，首次统计性探测到盘晕界面处λ4428弥散星际带（W_r=0.055±0.011埃），表明该区域存在富含气体、金属和尘埃的复杂有机分子环境。


<details>
  <summary>Details</summary>

Background: 研究聚焦于类星体背景光穿过前景星系（GOTOQs）时产生的吸收线系统，旨在揭示星系盘-晕界面（disk-halo interface）的气体、金属丰度及尘埃分布特征。

Data: 数据来源于SDSS DR16和DESI早期数据发布（DESI Early Data Release）中约110万条类星体光谱。

Method: 采用吸收无关（absorption agnostic）和吸收选择（absorption selected）相结合的方法对类星体背景下的前景星系进行搜索，并利用复合光谱（composite spectrum）分析技术检测星际介质特征。

Result: 发现了1345个独特的类星体背景前景星系系统（GOTOQs），覆盖红移范围0.0045 ≤ z ≤ 1.09，数量超过此前已知的四倍。
...
**TL;DR**: 该研究通过分析16个侧向星系(CHANG-ES)在144 MHz和3 GHz的射电连续谱数据，发现厚盘(halo)的平均尺度高度比为1.26±0.16，表明星系风系统中存在宇宙线(cosmic rays)的有效传输，且星系总质量与厚盘射电谱指数(spectral index)呈强负相关，而与恒星形成率-质量面密度(SFR-to-mass surface density)呈中等正相关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙射线（cosmic rays）和磁场对星系形成与演化的影响，特别是通过射电连续谱观测（radio continuum observations）探索边缘朝向星系（edge-on galaxies）晕中的相关物理过程。

Data: 数据来源于连续谱近邻星系晕巡天（CHANG-ES）的16个侧向星系射电连续谱数据，包括低频阵列（LOFAR）144 MHz和甚大阵列（VLA）3 GHz的7角秒分辨率图像。

Method: 采用射电连续谱观测数据，结合双指数模型拟合方法分析边缘星系垂直强度剖面，计算厚盘平均谱指数和144 MHz与3 GHz间的尺度高度比。

Result: 星系风（galactic winds）的存在导致低频射电连续谱平坦且尺度高度较大，表明宇宙线（cosmic rays）在这些系统中被有效传输。
...
**TL;DR**: 研究发现银河系盘面方位角金属丰度变化与V_phi-R平面速度空间脊状结构相关，并通过模拟证实棒旋臂相互作用是同时塑造化学和动力学子结构的主要机制。


<details>
  <summary>Details</summary>

Background: 银河系盘面存在由非轴对称动力学特征（如棒旋结构和瞬态旋臂）驱动的方位角金属丰度变化（azimuthal metallicity variations）与速度空间脊状结构（kinematic ridges）的关联性，构成当前星系化学动力学演化研究的核心议题。

Data: 数据来源包括盖亚卫星第三次数据释放（Gaia DR3）的盘星样本和一套银河系测试粒子模拟（test-particle simulations），后者评估了瞬态旋臂、棒状结构以及与人马座矮星系（Sagittarius-like dwarf galaxy）相互作用的影响。

Method: 采用银河系测试粒子模拟（Milky Way test-particle simulations）评估瞬态旋臂（transient spiral arms）、棒状结构（bar）及人马座矮星系（Sagittarius-like dwarf galaxy）相互作用对化学与运动学子结构的共同塑造作用。

Result: 银河系盘面方位角金属丰度变化与V_phi-R平面速度空间脊状结构相关，且棒（bar）和旋臂（spiral arm）相互作用是同时解释化学-运动学子结构及其对齐的主要物理机制。
...
**TL;DR**: 通过流体动力学模拟发现，星系碰撞中的恒星形成率变化主要受初始相对速度（relative impact velocity）、初始间距（initial distance）和气体含量（gas content）影响，高速碰撞导致单次剧烈峰值后骤降，低速碰撞则引发多次峰值并伴随星系合并。


<details>
  <summary>Details</summary>

Background: 研究探讨了初始速度与气体质量条件变化对星系碰撞过程中恒星形成率（star formation rate）的影响。

Data: 研究采用GADGET-4代码进行流体动力学模拟（hydrodynamic simulation），初始条件由Galstep和SnapshotJoiner程序生成，模拟了两星系以100 km/s至1000 km/s相对初速度迎头相撞的两种情景（相同大小星系和不同大小星系）。

Method: 采用GADGET-4代码进行流体动力学模拟（hydrodynamic simulations），结合Galstep和SnapshotJoiner程序生成初始条件，分析不同初始速度（100-1000 km/s）和气体质量下星系碰撞对恒星形成率的影响。

Result: 初始相对速度、星系间初始距离和气体含量是分析碰撞星系中恒星形成率的重要参数。
...
**TL;DR**: 基于FIRE-2模拟研究发现，约25%的银河系质量星系能在低气体比例(5-15%)时期通过恒星形成率快速下降（而非主要并合事件）形成与观测相似的α元素（如Mg）双峰分布，其中高α种群对应年老致密的厚盘，低α种群对应年轻延展的薄盘，转变过程历时0.3-1.2 Gyr且始于5.5-6.5 Gyr前。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系恒星丰度中α过程元素（如Mg）在固定[Fe/H]下的强双峰分布特征及其形成机制。

Data: 数据来源于FIRE-2（Feedback in Realistic Environments）模拟的16个银河系质量星系。

Method: 采用FIRE-2模拟（Feedback In Realistic Environments）分析16个银河系质量星系中[Mg/Fe]与[Fe/H]的分布模式，重点关注恒星形成率快速下降期间低气体分数（5-15%）条件下α元素双峰结构的形成机制。

Result: FIRE-2模拟中16个银河系质量星系中有4个能产生强双峰分布的[Mg/Fe]与[Fe/H]模式，高α（high-α）星族对应较老、径向致密的厚盘，低α（low-α）星族对应较年轻、径向延展的薄盘。
...
**TL;DR**: 利用VLT/MUSE数据对21个近邻星系（恒星质量≥10¹⁰M⊙、距离≤20Mpc）的电离气体运动学分析发现，52%的星系（11/21）在内千秒差距存在与棒驱动激波（bar-driven shocks）模型一致的连贯速度跃变特征，表明激波可能是星系中心区域气体流入的主导机制，且所有含内棒的星系均呈现LINER或赛弗特型核区发射，证实内棒能抑制核区恒星形成。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近邻星系中电离气体运动学（ionised-gas kinematics）及其与星系中心千秒差距（kiloparsec）尺度内激波（shocks）驱动的气体流入、核区恒星形成和活动星系核（AGN）活动之间的关联。

Data: 使用VLT/MUSE（甚大望远镜/多单元光谱探测器）观测数据，分析了一个质量与体积受限（恒星质量$M_* \geq 10^{10} M_\odot$、距离$D \leq 20$\,Mpc）的21个近邻星系样本的电离气体运动学。

Method: 采用VLT/MUSE数据，通过分析残余速度场（residual velocity field）并扣除旋转盘（rotating disc）模型，结合不同气体示踪剂（gas tracers）的速度差，识别扩展激波（extended shocks）特征。

Result: result：在21个近邻星系中，11个（约52%）显示出与棒驱动激波（bar-driven shocks）模型一致的扩展激波特征，且激波痕迹向内延伸至分辨率极限附近，表明激波可能是星系中心千秒差距内物质流入的主要机制。
...
### [10] [COUGS-DESI: A Catalog of Unusual Galaxies with Polar Structures in the DESI Legacy Imaging Surveys](https://arxiv.org/abs/2601.02579)
...
**TL;DR**: 通过DESI Legacy成像巡天数据构建了包含2,989个极结构星系(PSG)候选体的COUGS-DESI目录，将已知样本扩大了一个数量级，并发现极结构星系占本地非矮星系总数的2.2%，其中极环星系占0.7%。


<details>
  <summary>Details</summary>

Background: 极结构星系（PSGs）为研究星系形成与演化提供了独特视角，但其罕见性限制了统计研究。

Data: 数据来源于DESI Legacy Imaging Surveys（DR10）数据，通过目视检查、卷积神经网络分类和与已报道系统的交叉匹配，从Siena Galaxy Atlas（SGA）中识别出极结构星系（PSG）候选体。

Method: 采用视觉检查、卷积神经网络分类和与已报道系统的交叉匹配方法识别极结构星系（PSG）候选体。

Result: 构建了包含2,989个极结构星系（PSG）候选体的COUGS-DESI目录，将已知样本数量提升了一个数量级，其中极环（polar rings）占比0.7%，S0星系中约1%拥有极环结构。
...
### [11] [E-XQR-30: Evidence for an Increase in the Ionization State of Metal Absorbers from z~6 to z~2](https://arxiv.org/abs/2601.02634)
...
**TL;DR**: 基于E-XQR-30巡天在4.3<z<6.3红移范围内对488个金属吸收系统的分析，发现低电离(Low-ionization)系统比例从z∼6时的24%降至z∼4.3时的2%，而高电离(High-ionization)系统比例从52%增至82%，且log(N_CII/N_CIV)比值在z∼6至z∼2间下降约20倍，表明再电离末期紫外背景增强和宇宙平均密度降低共同驱动了电离态演化。


<details>
  <summary>Details</summary>

Background: 研究探索了再电离时期（epoch of reionization）附近星系及其周围金属富集气体电离状态的演化。

Data: 数据来源于E-XQR-30巡天项目中488个红移范围4.3<z<6.3的金属吸收系统样本，并结合了红移z∼2-4的文献测量数据。

Method: 采用光致电离模型（photoionization models）分析均匀紫外背景照射下固定密度、金属丰度和中性氢柱密度的气体薄层电离状态。

Result: 低电离态（low-ionization）金属吸收系统占比从红移（redshift）6时的24%降至红移4.3时的2%，而高电离态（high-ionization）系统占比从52%增至82%。
...
### [12] [The Origin of Spin-Alignment of Dark Matter Subhalos](https://arxiv.org/abs/2601.02678)
*Daiki Osafune,Keiichi Wada,Tomoaki Ishiyama,Takashi Okamoto*

**TL;DR**: 通过高分辨率宇宙学N体模拟(Shin-Uchuu)发现子晕(subhalo)自旋方向主要受宿主晕(host halo)潮汐场扭矩作用影响，其与宿主晕自旋的总体对齐源于轨道角动量方向的一致性，但内区极轨(polar)和逆行(retrograde)轨道的子晕自旋分别呈现垂直和反平行排列，且质量比和径向距离是主导对齐的关键参数。


<details>
  <summary>Details</summary>

Background: 研究主题为探索宿主晕（host halo）潮汐环境中子晕（subhalo）自旋获取的物理机制及其与宿主晕自旋的关联性。

Data: 使用高分辨率宇宙学N体模拟（cosmological N-body simulation）Shin-Uchuu作为数据来源。

Method: 采用高分辨率宇宙学N体模拟（Shin-Uchuu）分析子晕（subhalo）与主晕（host halo）自旋的关联性，重点考察轨道角动量与潮汐场扭矩的作用机制。

Result: 子晕（subhalo）自旋方向主要受宿主晕（host halo）潮汐场施加的扭矩影响，且与轨道角动量方向一致。
...
### [13] [A Tale of Two Dust Disks in Our Milky](https://arxiv.org/abs/2601.02724)
*Ruoyi Zhang,Haibo Yuan,Bingqiu Chen,Maosheng Xiang,Yang Huang,Xiaowei Liu,Jifeng Liu*

**TL;DR**: 基于超过500万颗恒星的观测数据，首次发现银河系宇宙尘埃（cosmic dust）由薄盘（60-200 pc）和厚盘（300-800 pc）两个空间分布分别与分子氢和原子氢盘相关的独立成分构成，并测量了其径向尺度长度（9.6 kpc和4.2 kpc）及气体-尘埃比（60-470）的指数梯度变化。


<details>
  <summary>Details</summary>

Background: 宇宙尘埃（cosmic dust）在恒星和星系形成演化中起关键作用，但银河系内其三维结构因缺乏精确消光（reddening）和距离测量而长期不明确。

Data: 数据来源包括超过500万颗恒星的光度测量（photometric）、光谱测量（spectroscopic）和天体测量（astrometric）数据。

Method: 基于超过500万颗恒星的光度、光谱和天体测量数据，采用双盘模型（thin/thick dust disk）分析银河系尘埃的三维结构。

Result: 在银河系5-14千秒差距（kpc）范围内首次发现薄尘埃盘（thin dust disk）和厚尘埃盘（thick dust disk）分别与分子氢盘和原子氢盘存在空间关联。
...
### [14] [A Galactic Supernova Remnant Candidate at l =25.8, b=+0.2 Revealed by Near-Infrared Imaging and Spectroscopy](https://arxiv.org/abs/2601.02761)
*Kim Yesol,Koo Bon-Chul,Lee Jae-Joon*

**TL;DR**: 通过高分辨率近红外光谱观测新发现的超新星遗迹(SNR)候选体G25.8+0.2，揭示了其[Fe II] 1.644微米发射中存在激波激发的高速成分和光致电离的低速成分，并结合射电壳层与X射线辐射形态证实了该区域存在近期超新星活动。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系内G26恒星形成区中新发现的超新星遗迹候选体G25.8+0.2的多波段观测与物理性质解析。

Data: 数据来源于使用浸没光栅红外光谱仪（IGRINS）在Harlan J. Smith望远镜上获得的高分辨率近红外光谱观测，以及UKIRT宽场红外巡天（UWIFE）中发现的[Fe II] 1.644微米发射源。

Method: 采用浸没光栅红外光谱仪(IGRINS)获取高分辨率近红外光谱数据，通过[Fe II] 1.644微米发射线的运动学组分分析、[Fe II]/Brγ比值诊断激波激发特征，并结合氢氦复合线分析光致电离气体。

Result: 在G25.8+0.2中观测到高速[Fe II]发射成分具有激波激发特征，低速成分则符合H II区光致电离气体特性，表明该区域存在近期超新星活动。
...
### [15] [Beyond Sgr A* and M87*: Sub-Microarcsecond Black Hole Shadow Detection via Lunar-based Extremely Long Baseline Interferometry](https://arxiv.org/abs/2601.02812)
*Shan-Shan Zhao,Ru-Sen Lu,Lei Liu,Zhiqiang Shen*

**TL;DR**: 提出利用月球望远镜扩展甚长基线干涉阵列(EHT)可提升分辨率至0.85微角秒，识别出6个优先观测的超大质量黑洞(SMBH)目标并确定所需望远镜尺寸，为未来月球望远镜研究黑洞阴影提供科学依据。


<details>
  <summary>Details</summary>

Background: 研究探讨了利用月球望远镜扩展甚长基线干涉测量（VLBI）阵列以提升超大质量黑洞（SMBH）阴影成像能力的科学前景。

Data: 数据来源包括地面甚长基线干涉阵列（VLBI）Event Horizon Telescope (EHT)的1.3毫米观测数据、假设的月球望远镜模拟数据、以及基于月球轨道VLBI实验（LOVEX）和国际月球科研站（ILRS）计划的扩展数据。

Method: 采用月球基地望远镜扩展甚长基线干涉阵列(VLBI)，通过模拟月球-地球(u,v)覆盖和几何环模型(geometric ring model)评估超大质量黑洞(SMBH)阴影可探测性。

Result: 在月球远端部署5米至100米口径望远镜可探测M104等六个超大质量黑洞（SMBH）的阴影，填补基线覆盖缺口后还能实现Sgr A*等天体的光子环探测。
...
### [16] [Investigating the Anisotropy of Dispersion Measure Contribution from the Galactic Halo by Using Fast Radio Bursts](https://arxiv.org/abs/2601.02849)
*Yang Liu,Bao Wang,Puxun Wu,Jun-Jie Wei,Xue-Feng Wu*

**TL;DR**: 基于快速射电暴(FRB)数据重建银河系晕色散量(DM_halo)全天空分布，首次发现其存在显著偶极各向异性（指向银经130°±28°、银纬+5°±28°方向，该方向DM_halo值63±9 pc cm^-3比全天平均值高2.6σ）。


<details>
  <summary>Details</summary>

Background: 研究主题为利用球谐展开方法重构银河系晕（Galactic halo）的色散量（DM_{halo}）全天空分布并探究其各向异性特征。

Data: 数据来源包括基于NE2001模型的92个定位和574个非定位非重复快速射电暴（FRB）样本（银河系纬度$|b|>15^\circ$），以及精炼后的62个定位FRB样本（排除CHIME探测、重复暴和非定位事件）。

Method: 采用球谐展开（spherical harmonic expansion）的数据驱动方法重建银河系晕的色散量（DM_halo）全天空分布。

Result: 银河晕(halo)的色散量(DM_halo)分布存在显著偶极各向异性，指向$(l=130^\circ,\, b=+5^\circ)$方向且该方向DM值$63\pm9~\mathrm{pc~cm^{-3}}$比全天均值高$2.6σ$。
...
### [17] [BEACON: JWST NIRCam Pure-parallel Imaging Survey. II. Physical Properties of $z=7-14$ Galaxies](https://arxiv.org/abs/2601.02861)
...
**TL;DR**: 基于BEACON项目第二期数据（DR2）的161个红移（redshift）7-14星系候选体分析，发现红移13.71的极亮（M_UV=-21.19）候选体具有极蓝紫外连续谱斜率（β_UV）、致密形态和高恒星形成率面密度（Σ_SFR），并在三个独立视场中识别出显著（>3σ）星系数过密度区，但不同环境下的星系性质无显著差异，表明红移7以上过密区域的加速演化尚未普遍或受样本量限制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（James Webb Space Telescope）的BEACON计划观测数据，探索红移（redshift）z=7-14范围内星系候选体的光度特性及其在不同宇宙环境中的演化规律。

Data: 数据来源于BEACON（JWST Cycle 2纯并行NIRCam成像计划）的第二批数据发布（DR2），包含36个独立指向区域（覆盖约350平方角分天区）的观测数据。

Method: 采用BEACON项目第二数据发布(DR2)的纯并行近红外相机(NIRCam)成像数据，通过光度测量分析星系候选体的紫外连续谱斜率(β_UV)、恒星质量(M_*)、恒星形成率(SFR)和尺寸等物理特性。

Result: 在BEACON项目第二数据发布(DR2)中发现的z=13.71±0.15候选星系具有M_UV=-21.19±0.08的超高紫外光度、极蓝紫外连续谱斜率(β_UV)、致密形态及高恒星形成率面密度(Σ_SFR)，可能具有极低金属丰度、高电离光子逃逸率或存在活动星系核(AGN)贡献。
...
### [18] [GATOS XI : Excess dust heating in the Narrow Line Regions of nearby AGN revealed with JWST/MIRI](https://arxiv.org/abs/2601.02865)
...
**TL;DR**: 利用JWST/MIRI对GATOS巡天中8个邻近活动星系核(AGN)的观测发现，窄线区(NLR)内延伸100-200 pc的尘埃结构形态与射电喷流及[Si VI]冕线发射密切相关，且其光谱能量分布斜率比恒星形成团更陡、温度达150-220 K，结合激波速度(v_shock~200-400 km/s)模型证实激波是AGN核区数百秒差距内尘埃加热的关键机制。


<details>
  <summary>Details</summary>

Background: 研究利用JWST/MIRI对GATOS巡天中八个邻近活动星系核(AGN)的观测，探讨了窄线区(NLR)内延伸尘埃的物理条件及其与激波(shocks)相互作用的机制。

Data: 数据来源于JWST/MIRI对GATOS巡天中8个邻近活动星系核(AGN)的成像观测，重点分析了其中4个星系(ESO 428-G14、NGC 4388、NGC 3081和NGC 5728)窄线区(NLR)延伸约100-200 pc的尘埃结构。

Method: 采用空间分辨光谱能量分布（SED）分析和简单辐射传输模型，结合激波（shock）加热机制解释中红外（MIR）超额辐射。

Result: 在四个活动星系核(AGN)的窄线区(NLR)中，观测到尘埃形态与射电喷流及[Si VI]冕线发射强烈关联，表明尘埃携带了塑造NLR过程的印记；NLR尘埃温度150-220K且发射谱斜率比恒星形成团更陡，其过剩中红外辐射无法仅用AGN照射解释，而需200-400 km/s激波在致密气体中的原位加热机制。
...
### [19] [A Long Tidal Feature Extending from a Low-Redshift Galaxy Revealed in Rubin Data Preview 1 Imaging](https://arxiv.org/abs/2601.03080)
*Virginia Johnson,Aaron J. Romanowsky*

**TL;DR**: 利用Vera C. Rubin天文台数据预览(DP1)在ECDFS区域发现LEDA 751050（红移z=0.1）存在一个与M31巨恒星流类似的恒星流，其延伸约130×15 kpc，恒星质量6×10^8 M_Sun，平均g波段表面亮度28.6 mag/arcsec^2，证实了Rubin探测低亮度子结构的能力。


<details>
  <summary>Details</summary>

Background: 研究主题为利用维拉·鲁宾天文台（Vera C. Rubin Observatory）数据预览1（DP1）探测并分析位于扩展钱德拉南深场（ECDFS）中红移z=0.1的大质量盘星系LEDA 751050小轴方向延伸的恒星流（stellar stream），类比M31巨恒星流（M31 Giant Stellar Stream），以验证鲁宾天文台在揭示低表面亮度（low-SB）子结构方面的能力。

Data: 数据来源于维拉·C·鲁宾天文台(Vera C. Rubin Observatory)的数据预览1(DP1)，针对扩展钱德拉深场南天区(ECDFS)中红移z=0.1的大质量盘星系LEDA 751050进行了观测。

Method: 基于维拉·C·鲁宾天文台数据预览1（DP1）进行光度测量分析，结合扩展钱德拉南深场（ECDFS）中LEDA 751050星系的红移（z=0.1）数据，检测并表征了沿其短轴延伸的恒星流。

Result: 在LEDA 751050（红移z=0.1）的短轴方向发现一个延伸约130千秒差距×15千秒差距、恒星质量约6×10^8太阳质量、平均g波段表面亮度为28.6星等/平方角秒的恒星流（stellar stream），类似于M31巨恒星流（M31 Giant Stellar Stream）。
...
**TL;DR**: 在最小I型跷跷板模型中，单重态标量驱动的一阶相变（FOPT）通过气泡碰撞产生右手中微子（RHNs），实现了探测超高能标的宇宙级对撞机，并产生新型低频引力波（GWs），同时稳定最轻RHN可解释暗物质（DM），不稳定RHN的CP破坏衰变可实现重子数生成或暗区费米子共生成，而UV完备的多Majoron模型则通过$U(1)_N$对称性破缺的FOPT留下可探测的独特GW信号并实现跷跷板尺度的轻子数生成。


<details>
  <summary>Details</summary>

Background: 研究探讨了通过单重态标量驱动的一阶相变（FOPT）产生右手中微子（RHNs）的最小I型跷跷板机制，构建宇宙尺度对撞机以探测超高能标，并关联暗物质（DM）残留丰度、重子不对称性及暗区费米子共生成等问题。

Data: 数据来源于数值模拟(simulation)和理论模型推导，包括单态标量驱动的第一类相变(FOPT)过程、右手中微子(RHNs)产生机制以及引力波(GW)信号预测，涉及LISA、ET、BBO和LVK等探测器的灵敏度范围。

Method: 采用单态标量驱动的一阶相变（FOPT）通过气泡碰撞产生右手中微子（RHNs），并基于全局$U(1)_N \times U(1)_{\rm B-L}$扩展的多马约拉子（Majoron）模型分析宇宙尺度对撞机效应。

Result: 在最小I型跷跷板模型中，单重态标量驱动的一阶相变（FOPT）通过气泡碰撞产生右手中微子（RHNs），实现了探测超高能标的宇宙级对撞机，并产生低频引力波（GWs）新信号。
...
### [21] [A possible challenge for Cold and Warm Dark Matter](https://arxiv.org/abs/2601.02466)
*S. Vegetti,S. D. M. White,J. P. McKean,D. M. Powell,C. Spingola,D. Massari,G. Despali,C. D. Fassnacht*

**TL;DR**: 通过引力透镜研究发现一个百万太阳质量的天体具有点质量（半径≤10 pc）和延伸质量分布（截断半径139 pc）的双组分结构，其特性与已知天体不符，若为暗物质主导则与冷暗物质模型矛盾但可能与自相互作用暗物质（self-interacting dark matter）形成中心黑洞的模型一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过引力透镜效应测量暗物质晕（dark-matter haloes）的密度分布和质量集中度，以检验冷暗物质（cold dark matter）标准模型的预测。

Data: 数据来源于对叠加在极薄引力弧上的百万太阳质量天体的引力透镜效应观测。

Method: 通过引力透镜效应（gravitational lensing）分析暗物质晕（dark-matter haloes）的质量密度分布和结构特征，并采用双组分模型（点质量与延展质量分布）拟合观测数据。

Result: 发现一个由未解析点质量（半径≤10 pc）和延伸质量分布（截断半径139 pc）组成的双组分结构，若为暗物质主导，其性质与冷暗物质模型不符，但可能与自相互作用暗物质（self-interacting dark matter）晕中心坍缩形成黑洞的情况一致。
...
### [22] [Gas rotation and turbulence in the galaxy cluster Abell 2029](https://arxiv.org/abs/2601.02495)
*T. Bartalesi,A. Simionescu,S. Ettori,C. Nipoti,V. Ghirardini,A. Sarkar,M. Sun*

**TL;DR**: 通过结合XRISM和XMM-Newton数据构建动力学平衡模型，发现Abell 2029星系团内介质(ICM)在0-650千秒差距范围内湍流压力占比约2%，200-600千秒差距处旋转速度与湍流速度比峰值为0.15，且在r2500半径处静水力平衡质量占总质量约97%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用动力学平衡模型和先进X射线数据约束星系团内介质（intracluster medium, ICM）的旋转和湍流支撑特性。

Data: 数据来源于XRISM/Resolve的X射线光谱分析、XMM-Newton和Planck的径向热力学剖面测量以及XRISM点源的非热速度弥散和红移观测。

Method: 采用旋转湍流内禀速度场模型结合马尔可夫链蒙特卡洛(Markov chain Monte Carlo)算法，通过拟合XMM-Newton和Planck热力学剖面及XRISM视线方向非热速度弥散度与红移数据，约束星系团内介质(ICM)动力学状态。

Result: 利用动力学平衡模型和先进X射线数据集，发现A2029星系团内介质（ICM）的湍流压力占比约2%（0-650 kpc径向范围），旋转速度与湍流速度比值在200-600 kpc处达到峰值0.15，且在r2500半径处静水力质量占总质量比约0.97。
...
### [23] [Indications of a Late-Time Transition to a Strongly Interacting Dark Sector](https://arxiv.org/abs/2601.02789)
*Andronikos Paliathanasis*

**TL;DR**: 提出了一种由红移阈值控制的暗能量（dark energy）与暗物质（dark matter）相互作用模型，发现低红移（$z_T<0.624$）时存在从暗能量向暗物质的显著能量转移，且不跨越幻影界限（phantom divide line）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索从标准宇宙学模型（ΛCDM）向暗能量（dark energy）与暗物质（dark matter）相互作用的暗区（dark sector）模型的转变，通过引入红移阈值控制能量转移的起始时机。

Data: 数据来源于晚期宇宙学观测，包括宇宙计时器（Cosmic Chronometers）、DESI DR2重子声学振荡（Baryonic Acoustic Oscillations）以及来自Pantheon Plus、Union3.0和DES-Dovekie目录的超新星数据。

Method: 采用具有红移阈值（redshift threshold）的相互作用暗能量（interacting dark energy）模型，通过引入能量转移（energy transfer）触发机制分析暗能量与暗物质（dark matter）的相互作用动力学。

Result: 在低红移（low redshift）下，暗能量（dark energy）向暗物质（dark matter）的能量转移存在强相互作用项，且不同观测样本给出的转移红移阈值（transition redshift）分别为PantheonPlus样本$z_{T}<0.624$、Union3.0样本$z_{T}=0.400_{-0.23}^{+0.021}$和DES-Dovekie样本$z_{T}=0.371_{-0.26}^{+0.028}$。
...
### [24] [Nonlinear Weak Lensing reconstruction for Galaxy Clusters](https://arxiv.org/abs/2601.02816)
*Yuan Shi,Li Cui*

**TL;DR**: 提出了一种改进的弱引力透镜质量重建框架，通过引入平滑掩膜和基于模型解析解的初始猜测，在非线性区域（强引力透镜区）实现了高精度质量分布重建（残差低于0.02σ）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用弱引力透镜质量映射（weak lensing mass mapping）在非线性区域提升星系团（cluster）质量重建的精度。

Data: 数据来源于模拟的星系团引力透镜数据（simulated cluster lensing data），包含已知质量分布和由弱引力透镜测量限制产生的真实掩模（realistic masks）。

Method: 采用平滑掩膜处理非线性区域并结合模型解析解作为初始猜测的弱引力透镜质量映射（weak lensing mass mapping）框架。

Result: 开发的重建框架通过平滑掩膜和模型解析初始猜测，在非线性区域实现了高保真质量重建，无形状噪声时未掩膜区域残差低于0.02σ。
...
### [25] [Measuring the homogeneity scale using the peculiar velocity field](https://arxiv.org/abs/2601.02886)
*Leonardo Giani,Cullan Howlett,Chris Blake,Ryan J. Turner,Tamara M. Davis*

**TL;DR**: 提出基于速度场（peculiar velocities）而非密度场定义宇宙均匀性尺度（homogeneity scale）的新方法，通过斯隆数字巡天（SDSS）速度数据测得该尺度为133 (+28/-52) Mpc/h，解决了传统密度定义中因星系偏差（galaxy bias）导致的尺度模糊问题。


<details>
  <summary>Details</summary>

Background: 研究提出了一种基于速度（peculiar velocities）而非密度场（matter density field）的创新方法来定义宇宙均匀性尺度（homogeneity scale），以解决传统密度方法中因任意阈值选择和星系偏差（galaxy bias）导致的退化问题。

Data: 数据来源于斯隆数字巡天（Sloan Digital Sky Survey）的视向速度（peculiar velocity）目录。

Method: 基于星系本动速度（peculiar velocity）相关函数分析，通过测量速度场从正相关到反相关的转变尺度来定义宇宙均匀性尺度。

Result: 基于速度场（peculiar velocities）而非密度场定义了宇宙均匀性尺度（homogeneity scale），其值为$R_H\approx 133\substack{+28 \\ -52}\, \rm{Mpc/h}$。
...
### [26] [Probing ionized bubbles around luminous sources during reionization with SKA 21-cm observations](https://arxiv.org/abs/2601.02889)
*Arnab Mishra,Kanan Kumar Datta,Chandra Shekhar Murmu,Samir Choudhuri,Iffat Nasreen,Snehasish Saha*

**TL;DR**: 开发了一种基于网格化可见度数据的高效电离气泡（ionized bubbles）检测方法，结合高斯过程回归（Gaussian process regression）去 foreground 技术和贝叶斯似然框架，实现了对再电离时期（EoR）红移 z ∼ 7−8 的电离气泡半径、位置及中性氢分数（neutral hydrogen fraction）的高信噪比（SNR ≳10）精确探测，并建立了信噪比与气泡参数的标度关系（scaling relation），为 SKA1-Low 观测策略提供了可靠工具。


<details>
  <summary>Details</summary>

Background: 探测和表征再电离时期(EoR)中电离气泡(HII regions)的21厘米红移信号，直接研究早期电离源和星系际介质(IGM)。

Data: 数据来源于SKA1-Low（Square Kilometre Array Phase 1 Low Frequency Component）的AA*和AA4阵列约100小时的观测，用于探测再电离时期（EoR）红移后的HI 21厘米信号。

Method: 采用基于高斯过程回归（Gaussian process regression）的非参数前景扣除方法，最小化中性氢（HI）21厘米信号损失并提升信噪比。

Result: 开发了一种基于网格化可见度数据的高效电离气泡（ionized bubbles）检测方法，相比传统方法显著降低了计算复杂度。
...
### [27] [Polarization of the CMB in the Standard Model Extension](https://arxiv.org/abs/2601.02961)
*Iman Motie,Jafar Khodagholizadeh,S. Mahmoudi,Brahim Lamine,Alain Blanchard*

**TL;DR**: 研究发现标准模型扩展（SME）框架下的背景洛伦兹破缺（LV）张量$K_{AF}$和$K_{F}$可分别诱导宇宙微波背景（CMB）辐射产生圆偏振和宇宙双折射（CB），并给出$K_{AF}$和$K_{F}$的约束上限分别为$10^{-41} \, \text{GeV}$和$10^{-32} \, \text{GeV}$。


<details>
  <summary>Details</summary>

Background: 研究探讨了标准宇宙学中洛伦兹对称性破缺（Lorentz violation, LV）对宇宙微波背景（CMB）辐射偏振特性的影响，特别是背景LV张量$K_{AF}$和$K_{F}$如何诱导CMB的圆偏振和宇宙双折射（cosmic birefringence, CB）。

Data: 研究基于标准宇宙学模型，利用玻尔兹曼方程在标准模型扩展（SME）框架下分析洛伦兹破坏（LV）背景张量$K_{AF}$对宇宙微波背景（CMB）辐射偏振特性的影响，并通过法拉第转换角计算线性偏振向圆偏振的转化过程。

Method: 采用标准模型扩展(SME)框架下的玻尔兹曼方程(Boltzmann equation)分析背景洛伦兹破缺(LV)张量$K_{AF}$对CMB辐射圆偏振的影响，并通过法拉第转换角(Faraday conversion angle)计算线性偏振向圆偏振的转化过程。

Result: 洛伦兹破坏（Lorentz violation, LV）背景张量 $ K_{AF} $ 可在宇宙微波背景辐射中诱导出圆偏振，其上限约为 $ 10^{-41} \, \text{GeV} $。
...
### [28] [Bounce Inflation with Dynamical Dark Energy in Light of DESI DR2](https://arxiv.org/abs/2601.03028)
*Xin-zhe Zhang,Hao-Hao Li,Taotao Qiu*

**TL;DR**: 研究发现非奇异反弹暴胀(BI)模型结合动态暗能量(CPL参数化)和DESI DR2数据可得到更高哈勃常数(H₀=65.2^{+1.8}_{-2.2} km/s/Mpc)，且比幂律(PL)模型更支持暗能量演化(w₀=-0.919±0.038，w_a=-0.37±0.12)，将哈勃张力缓解至3.1σ置信水平。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探讨动态暗能量（dynamical dark energy）对宇宙学模型的影响，特别是结合非奇异反弹暴胀（non-singular bounce inflation）场景和CPL参数化（Chevallier-Polarski-Linder parameterization）来缓解哈勃张力（Hubble tension）。

Data: 数据来源包括DESI DR2（Dark Energy Spectroscopic Instrument Data Release 2）、宇宙微波背景（cosmic microwave background, CMB）数据及Pantheon+数据集。

Method: 采用非奇异反弹暴胀（BI）模型结合动态暗能量的切瓦利埃-波拉斯基-林德（CPL）参数化方法分析宇宙学数据。

Result: 采用非奇异反弹暴胀(BI)模型结合动态暗能量(CPL参数化)和DESI DR2数据，得到哈勃常数$H_0 = 65.2^{ + 1.8}_{ - 2.2} \ \mathrm{km} \cdot \mathrm{s}^{ -1 } \cdot \mathrm{Mpc}^{ -1 }$，优于幂律(PL)模型的$H_0 = 64.0 \pm 2.1$结果。
...
### [29] [Late-time Background Constraints on Linear and Non-linear Interacting Dark Energy after DESI DR2](https://arxiv.org/abs/2601.03122)
*David Figueruelo,Marcel van der Westhuizen,Amare Abebe,Eleonora Di Valentino*

**TL;DR**: 通过分析八种相互作用暗能量(IDE)模型，发现所有模型都比ΛCDM更符合观测数据，且倾向于低红移时能量从暗能量流向暗物质、高红移时反向转移的符号切换相互作用，同时揭示有效暗能量状态方程存在幻影界限(phantom-divide)交叉现象。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过观测数据约束一类允许哈勃参数H(z)解析解的现象学相互作用暗能量(IDE)模型，探讨暗能量与暗物质(dark matter)之间的能量转移及其对宇宙演化的影响。

Data: 数据来源于观测性约束（observational constraints）对哈勃参数H(z)的分析解，包括五类线性和三类非线性相互作用暗能量（IDE）模型的数据集组合。

Method: 采用线性与非线性相互作用暗能量（IDE）模型分析哈勃参数H(z)的解析解，包括五种线性及三种非线性相互作用形式，涵盖暗物质和/或暗能量密度的比例组合。

Result: 所有八种相互作用暗能量（IDE）模型在Δχ²分析中均比ΛCDM模型拟合更好，且多数情况下AIC准则也支持这一结论，但Q=3Hδρde模型在一个数据集组合中例外。
...
### [30] [The Squeezed Bispectrum from CHIME HI Emission and Planck CMB Lensing: Current Sensitivity and Forecasts](https://arxiv.org/abs/2601.03240)
...
**TL;DR**: 通过测量加拿大氢强度测绘实验(CHIME)的小尺度21厘米功率与普朗克宇宙微波背景(CMB)透镜大尺度结构的非线性引力耦合（挤压积分双谱），发现当前噪声水平是预期信号的5倍，但利用已收集的额外观测数据可使信噪比达到3。


<details>
  <summary>Details</summary>

Background: 探索利用氢原子（HI）线强度映射技术分离强射电前景辐射以高效测绘宇宙大尺度结构的可行性。

Data: 数据来源于加拿大氢强度测绘实验（CHIME）在红移1.0至1.3范围内94个夜晚的观测数据，并结合普朗克（Planck）宇宙微波背景（CMB）透镜化数据。

Method: 采用非线性引力耦合方法测量加拿大氢强度测绘实验(CHIME)的小尺度21厘米功率与普朗克宇宙微波背景(CMB)透镜大尺度结构之间的挤压积分双谱(squeezed integrated bispectrum)。

Result: 通过测量加拿大氢强度测绘实验（CHIME）的小尺度21厘米功率与普朗克宇宙微波背景（CMB）透镜化大尺度结构的非线性引力耦合（挤压积分双谱），发现当前噪声水平比预期信号高五倍，但利用已采集的额外观测数据可使信噪比提升至3。
...
### [31] [Heavy Black-Holes Also Matter in Standard Siren Cosmology](https://arxiv.org/abs/2601.03257)
*Grégoire Pierra,Alexander Papadopoulos*

**TL;DR**: 通过分析GWTC-4.0中142例致密双星并合(CBCs)事件并采用谱警报(spectral siren)和暗警报(dark siren)方法，研究发现黑洞质量谱中63.3太阳质量的新质量尺度可将哈勃常数(Hubble constant)测量不确定度降低30%-36%，谱警报和暗警报结果分别为$H_0 = 78.8^{+19.0}_{-15.3}$和$82.5^{+16.8}_{-14.3}\,{\rm km s^{-1} Mpc^{-1}}$。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用引力波（GWs）探测数据（特别是致密双星并合（CBCs）事件）改进哈勃常数（Hubble constant）的宇宙学估计，并探索黑洞质量谱中重质量特征对宇宙学参数约束的影响。

Data: 数据来源于LIGO-Virgo-KAGRA (LVK)合作组发布的引力波瞬变目录GWTC-4.0中的142个紧凑双星并合(CBCs)候选体，筛选标准为年误报率低于0.25。

Method: 采用参数化模型（parametric model）对致密双星并合（CBCs）的全质量谱进行建模，并结合谱警报（spectral siren）和暗警报（dark siren）方法联合推断宇宙学及群体参数。

Result: 通过光谱警报法（spectral siren）测得哈勃常数（Hubble constant）为$H_0 = 78.8^{+19.0}_{-15.3}\,{\rm km s^{-1} Mpc^{-1}}$（68%置信区间），暗警报法（dark siren）测得$H_0 = 82.5^{+16.8}_{-14.3}\,{\rm km s^{-1} Mpc^{-1}}$（68%置信区间），精度分别提升30.4%和36.2%，主要归因于在$63.3^{+4.8}_{-4.8}\,M_{\odot}$处发现的新质量尺度对哈勃常数的额外约束。
...
### [32] [Predictability of bursts of a recurrent nova using topological data analysis and machine learning](https://arxiv.org/abs/2601.02403)
*Ignacio Morales-Gil*

**TL;DR**: 论文核心贡献是利用持久同调（persistent homology）技术对再发新星（recurrent nova）RS Oph的光变曲线进行特征提取，并通过监督学习模型基于持久景观（persistence landscapes）等特征化方法实现了高准确率的爆发预测。


<details>
  <summary>Details</summary>

Background: 研究背景为利用拓扑数据分析（topological data analysis）中的持续同调（persistent homology）技术分析再发新星（recurrent nova）RS Oph的光变曲线（lightcurve）特征，以预测其爆发状态。

Data: 研究中使用的数据来源于RS Oph（蛇夫座RS）的光变曲线（lightcurve）分段数据。

Method: 采用持久同调（persistent homology）技术分析RS Oph光变曲线数据，并基于持久图（persistence diagrams）的多种特征化方法（包括持久景观、Carlsson坐标、持久图像和模板函数）训练监督学习模型。

Result: 基于持久同调（persistent homology）特征化的监督学习模型能有效预测RS Oph是否在一年内爆发。
...
### [33] [Experiments in binary evolution](https://arxiv.org/abs/2601.02448)
*Stephan Geier,Thomas Kupfer,Pierre Maxted,Veronika Schaffenroth*

**TL;DR**: 这篇论文提出通过实时调度优化下一代时域光谱巡天观测策略，以高效测定大样本双星系统（binary systems）轨道参数，从而建立体积完备的前身星和物质转移后系统样本，为恒星演化中的相互作用提供关键观测约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量恒星在双星或多星系统中的演化过程，特别是质量转移（mass transfer）前后系统的关联性及其观测约束。

Data: 数据来源于下一代时域分辨光谱巡天（time-resolved spectroscopic surveys）结合实时调度器（scheduler）和多目标光谱仪（multi-object spectrograph）或望远镜网络（network of independent telescopes）。

Method: 采用实时调度器（scheduler）预测最佳观测时间，并结合可灵活调度的多目标光谱仪（multi-object spectrograph）或独立望远镜网络来高效测定双星系统轨道参数。

Result: 下一代时域分辨光谱巡天应优化为高效跟踪并解析整个双星系统群体，需结合实时预测最佳观测时间的调度器与灵活可调的多目标光谱仪或独立望远镜网络。
...
### [34] [Validation of Satellite Lifetime Predictions at Leonid Space](https://arxiv.org/abs/2601.02453)
*Scott Shambaugh*

**TL;DR**: 通过大规模回溯测试验证了Leonid Space卫星寿命预测工具链（结合弹道系数估计与概率轨道传播）的准确性，在完全预测条件下将低地球轨道（LEO）卫星1年寿命预测误差中位数提升至45.5天（12.4%），较现有工具链精度提高4倍，并揭示太阳活动周期预测是仅次于弹道系数估计的主要误差来源。


<details>
  <summary>Details</summary>

Background: 研究聚焦于验证低地球轨道（LEO）卫星寿命预测工具链的准确性，旨在为卫星任务规划和监管合规提供可靠依据。

Data: 数据来源包括1961至2024年间934颗非机动低地球轨道（LEO）卫星的两行轨道根数（TLE）数据、跨越六个太阳周期的空间天气记录以及用于弹道系数估计的在轨数据。

Method: 采用基于弹道系数（ballistic coefficient）估计与概率轨道传播（probabilistic orbit propagation）的三阶段验证方法，结合TLE数据和空间天气记录进行卫星寿命预测。

Result: 在完全预测条件下实现了45.5天（12.4%）的1年寿命预测中位连续排序概率得分（CRPS），相比ESA标准工具链（DRAMA & DISCOS）将最先进精度提升了4倍。
...
### [35] [Exposure-averaged Gaussian Processes for Combining Overlapping Datasets](https://arxiv.org/abs/2601.02462)
*Jacob K. Luhn,Ryan A. Rubenzahl,Samuel Halverson,Lily L. Zhao*

**TL;DR**: 提出了考虑曝光时间的集成高斯过程（GP）框架，用于预测恒星（如太阳）的真实潜在振荡信号和不同仪器因曝光时间差异观测到的分箱信号，并扩展至处理具有显著时间重叠的仪器数据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星变率（stellar variability）的高斯过程（Gaussian process）建模，特别针对曝光时间与内核时间尺度相当的极端精密视向速度（EPRV）光谱仪数据。

Data: 数据来源包括极端精密视向速度（EPRV）摄谱仪获取的快速读出恒星数据（如星震学目标）和白天观测的太阳变异性监测数据，以及不同设计导致曝光时间差异的各站点太阳EPRV设施数据。

Method: 采用高斯过程（Gaussian process, GP）框架，通过计算瞬时核函数的积分形式来考虑曝光时间影响，并扩展至处理具有显著时间重叠的仪器数据。

Result: 提出了一种考虑曝光时间的高斯过程（GP）框架，通过计算典型瞬时核函数的积分形式来预测真实潜在振荡信号和各仪器预期的曝光合并版本。
...
### [36] [Scalable Gaussian Processes for Integrated and Overlapping Measurements Via Augmented State Space Models](https://arxiv.org/abs/2601.02527)
*Ryan A. Rubenzahl,Soichiro Hattori,Simo Särkkä,Will M. Farr,Jacob K. Luhn,Megan Bedell*

**TL;DR**: 提出了一种基于线性高斯状态空间模型（SSM）的smolgp方法，通过引入积分状态（integral state）处理天文观测中的曝光积分（exposure integration）问题，在保持与高斯过程（GP）相同后验精度的同时将计算复杂度从O($N^3$)降至O($N$)，并支持GPU并行加速。


<details>
  <summary>Details</summary>

Background: 研究聚焦于天文观测中有限曝光时间导致的变异性模糊问题，提出基于高斯过程（Gaussian Processes, GPs）与线性高斯状态空间模型（Linear Gaussian State Space Models, SSMs）等效性的可扩展计算方法，以解决传统方法因协方差矩阵非准可分性带来的计算复杂度瓶颈。

Data: 研究中使用的数据来源为模拟（simulation）数据，通过高斯过程（Gaussian Processes, GPs）和线性高斯状态空间模型（Linear Gaussian state space models, SSMs）进行建模，并利用smolgp软件包实现高效计算。

Method: 采用线性高斯状态空间模型（SSMs）结合积分状态（integral state）的方法，通过卡尔曼滤波（Kalman filter）和RTS平滑器（RTS smoother）实现O(N)计算复杂度，并开发了smolgp开源工具支持标准及曝光感知（exposure-aware）的高斯过程（GPs）建模。

Result: 通过状态空间模型（SSM）增强积分状态的方法，实现了在O(N)时间复杂度下精确计算高斯过程（GP）对积分曝光的后验分布，并支持GPU并行加速至O(N/T + log T)。
...
### [37] [Using 23 Years of ACS/SBC Data to Understand Backgrounds: Significant Reductions in Expected Background Levels](https://arxiv.org/abs/2601.02551)
*Christopher. J. R. Clark,Roberto J. Avila,Alyssa Guzman,Norman A. Grogin*

**TL;DR**: 基于23年哈勃太空望远镜ACS/SBC数据，研究发现受气辉(airglow)影响的F115LP、F122M、F125LP、PR110L和PR130L滤光片背景值变化超一个数量级且显著低于ETC预测值，而暗流主导的F150LP和F165LP滤光片背景变化较小，表明短波SBC滤光片可更快完成背景限制观测。


<details>
  <summary>Details</summary>

Background: 研究利用哈勃太空望远镜(Hubble Space Telescope) ACS/SBC长达23年的数据，探究了实际观测中背景辐射水平及其变化规律，重点分析了气辉(airglow)和暗电流对不同滤光片背景辐射的影响。

Data: 研究使用了哈勃太空望远镜(Hubble Space Telescope) ACS/SBC(Advanced Camera for Surveys/Solar Blind Channel) 23年的观测数据。

Method: 采用哈勃太空望远镜ACS/SBC（Advanced Camera for Surveys/Solar Blind Channel）23年的观测数据，通过分析不同滤光片（F115LP、F122M等）的背景变化及其与气辉（airglow）的关系，并与曝光时间计算器（Exposure Time Calculator, ETC）预测值进行对比。

Result: 背景水平（background）在F115LP、F122M、F125LP、PR110L和PR130L滤光片中因气辉（airglow）变化呈现超过一个数量级的观测间差异，而F150LP和F165LP滤光片因暗流主导表现出更小的背景变化。
...
### [38] [The Lazuli Space Observatory: Architecture & Capabilities](https://arxiv.org/abs/2601.02556)
...
**TL;DR**: Lazuli空间天文台(Lazuli Space Observatory)通过3米口径离轴自由曲面望远镜在400-1700纳米波段实现衍射极限成像(斯特列尔比＞0.8@633纳米)，配备宽视场相机(WCC)、积分视场光谱仪(IFS)和系外行星日冕仪(ESC)，在月球共振轨道实现4小时内快速响应观测，支持时域天文学、系外行星研究和宇宙学三大科学目标。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用蓝宝石空间天文台（Lazuli Space Observatory）开展时间域与多信使天文学（time-domain and multi-messenger astronomy）、恒星与行星（stars and planets）以及宇宙学（cosmology）三大领域的科学探索。

Data: 数据来源于Lazuli空间天文台配备的三台仪器：宽视场环境相机（Wide-field Context Camera, WCC）提供35'×12'视场的多波段成像和高频测光数据；积分视场光谱仪（Integral Field Spectrograph, IFS）提供400-1700 nm连续光谱覆盖（分辨率R∼100-500）的稳定分光测光数据；系外行星日冕仪（ExtraSolar Coronagraph, ESC）提供原始对比度达10⁻⁸、后处理对比度接近10⁻⁹的高对比度成像数据。

Method: 采用离轴自由曲面望远镜（off-axis, freeform telescope）实现衍射极限成像质量（Strehl >0.8 @633 nm），并配备宽视场相机（WCC）、积分视场光谱仪（IFS）和外太阳系日冕仪（ESC）三种互补仪器，支持400-1700 nm波段的高精度测光与光谱分析（R~100-500）。

Result: Lazuli空间天文台（Lazuli Space Observatory）配备了3米口径主镜，采用离轴自由曲面望远镜设计，在400-1700纳米波段提供衍射极限成像质量（斯特列尔比Strehl＞0.8@633纳米），并通过三台仪器实现互补观测功能：宽视场环境相机（WCC）提供35'×12'视场的多波段高时间分辨率测光，积分视场光谱仪（IFS）实现400-1700纳米连续覆盖且分辨率R∼100-500的稳定分光测量，系外行星日冕仪（ESC）则能实现原始对比度10⁻⁸和后处理对比度10⁻⁹的高对比度成像。
...
### [39] [Optomechanical platform for high-frequency gravitational wave and vector dark matter detection](https://arxiv.org/abs/2601.02576)
*David Rousso,Moritz Bjoern Kristiansson Kunze,Christoph Reinhardt*

**TL;DR**: 提出了一种集成纳米机械膜谐振器与中等精细度光学腔的平台，可同时探测高频引力波（0.5-40 kHz）和矢量暗物质（vector dark matter），峰值应变灵敏度达2×10⁻²³/√Hz（40 kHz），对矢量暗物质的探测范围（2×10⁻¹²至2×10⁻¹⁰ eV/c²）超越现有极限。


<details>
  <summary>Details</summary>

Background: 探索高频引力波和矢量暗物质（vector dark matter）的统一探测平台。

Data: 数据来源于集成在中等精细度（$\mathcal{F}\sim 10$）光学腔中的纳米机械膜谐振器（nanomechanical membrane resonator）的辐射压力（radiation-pressure force）驱动运动及硅膜（silicon membrane）与砷化镓输入镜（gallium-arsenide input mirror）的差分加速度测量。

Method: 采用纳米机械膜谐振器（nanomechanical membrane resonator）集成到中等精细度（moderate-finesse）光学腔中的方法，通过辐射压力（radiation-pressure force）驱动膜运动并原位调谐其共振频率。

Result: 提出了一种集成纳米机械膜谐振器与中等精细度光学腔（$\mathcal{F}\sim 10$）的平台，可统一探测高频引力波和矢量暗物质（vector dark matter）。
...
### [40] [The Study of a Cosmic Ray Candidate Detected by the Askaryan Radio Array](https://arxiv.org/abs/2601.02718)
*Shoukat Ali,Dave Z. Besson*

**TL;DR**: 该研究通过ARA（Askaryan Radio Array）阵列的A2站点观测到一个向下传播的宇宙线（CR）诱发大气簇射候选事件，结合FAERIE和AraSim模拟框架分析其双脉冲信号特征（先后到达的地磁辐射与Askaryan辐射），优化了事件拓扑结构并比较了两种信号的顶点重建结果及时延差异。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用射电技术探测超高能(UHE)中微子的同时，对宇宙线(CR)诱发大气簇射产生的射电信号进行探测与校准。

Data: 数据来源于ARA（Askaryan Radio Array）阵列中2号站（A2）记录的候选事件信号，并结合了现代冰层影响宇宙射线（CR）簇射模拟框架FAERIE和探测器模拟软件AraSim的详细仿真结果。

Method: 结合FAERIE宇宙射线(CR)簇射模拟框架与AraSim探测器模拟包进行详细仿真分析。

Result: result：通过结合FAERIE框架和AraSim模拟，优化了事件拓扑结构，比较了地磁信号和Askaryan信号的顶点重建结果以及各天线中两个信号的观测时间延迟。
...
### [41] [SpaceWire-based Data Acquisition Network for the Solar Flare Sounding Rocket Experiment FOXSI-4 and FOXSI-5](https://arxiv.org/abs/2601.02788)
...
**TL;DR**: 为FOXSI-4和FOXSI-5探空火箭实验开发了基于SpaceWire的模块化数据采集(DAQ)系统，实现了高灵敏度、高动态范围的太阳耀斑直接X射线聚焦观测，并首次在FOXSI-4任务中成功观测到GOES M1.6级耀斑的软硬X射线能谱成像。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发基于SpaceWire的数据采集系统，以支持FOXSI-4和FOXSI-5探空火箭实验对太阳耀斑的高灵敏度、宽动态范围X射线直接聚焦观测。

Data: 数据来源于FOXSI-4和FOXSI-5探空火箭实验，包括两个CMOS传感器、四个CdTe双面条带探测器（CdTe-DSDs）和一个Quad-Timepix3探测器获取的直接X射线聚焦观测数据。

Method: 采用基于SpaceWire和远程内存访问协议(RMAP)的模块化数据采集(DAQ)网络架构，开发了支持高速数据传输和灵活仪器控制的FPGA读出板(SPMU-001和SPMU-002)。

Result: 为FOXSI-4和FOXSI-5探空火箭实验开发了基于SpaceWire的模块化数据采集(DAQ)系统，支持多种探测器的高速数据传输和分布式开发。
...
### [42] [A New Collision Avoidance Fiber Assignment Algorithm for Robotic Fiber Positioners in Multi-Object Spectroscopy](https://arxiv.org/abs/2601.02795)
...
**TL;DR**: 提出了一种用于多目标光谱机器人光纤定位系统的新分配算法，通过三步优化策略（初始分配、碰撞分组、组内求解）将目标分配完整度提升10%（150光纤场景下），适用于光纤定位区域高度重叠的系统如A-SPEC巡天项目。


<details>
  <summary>Details</summary>

Background: 研究聚焦于多目标光谱（multi-object spectroscopy）中机器人光纤定位系统（robotic fiber positioner system）的光纤分配算法优化。

Data: 数据来源于基于K-SPEC光谱仪系统的全天空邻近星系光谱巡天（A-SPEC）项目，并适用于具有高度重叠光纤定位器的类似光纤系统。

Method: 采用分步优化算法解决光纤定位器（fiber positioner）碰撞问题，先最大化分配目标再分组处理碰撞。

Result: 提出了一种新的光纤分配算法，可在150根光纤的视场中比简单算法提高10%的目标分配完整度。
...
### [43] [Primordial Black Hole Formation in $f(R)=R+αR^2$ Gravity: Perturbative and Non-Perturbative Analysis](https://arxiv.org/abs/2601.02416)
*G. G. L. Nashed*

**TL;DR**: 该论文通过解析和半解析方法研究了二次$f(R)$引力模型($f(R)=R+αR^2$)中的引力坍缩和原初黑洞(PBH)形成，发现微扰效应在辐射主导时期较弱但在高曲率背景下显著，并通过爱因斯坦框架下的数值计算获得了暴胀末期PBH形成的临界密度阈值$δ_c(k)$。


<details>
  <summary>Details</summary>

Background: 研究探讨了二次$f(R)$引力模型（$f(R)=R+αR^2$）中引力坍缩和原初黑洞（PBH）形成的解析与半解析解。

Data: 研究中使用的数据来源于对平坦FLRW（Friedmann-Lemaître-Robertson-Walker）尘埃内部坍缩和闭合FLRW（Friedmann-Lemaître-Robertson-Walker）超密斑块演化的半解析及数值模拟分析。

Method: 采用一阶微扰展开（perturbative expansion）和爱因斯坦框架（Einstein frame）转换结合数值求解常微分方程（ODE）系统的方法分析$f(R)=R+αR^2$模型中引力坍缩和原初黑洞（PBH）形成的临界条件。

Result: 在二次$f(R)$引力模型中，微扰分析表明标量场（scalaron）和背景曲率显著影响原初黑洞（PBH）形成阈值$δ_c$，尤其在早期高曲率时期效应更为突出。
...
### [44] [Utilizing anticoincidence veto in a search for gravitational-wave transients](https://arxiv.org/abs/2601.02713)
*Souradeep Pal*

**TL;DR**: 提出了一种基于时间反符合（temporal anticoincidence）的噪声瞬态抑制技术，通过地理分离探测器非同时触发的特性剔除虚假信号，显著提高了双黑洞（binary black holes）短时瞬态信号搜索的灵敏度和检测效率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过时间反符合（temporal anticoincidence）技术抑制引力波探测器噪声瞬变（noise transients）对双黑洞瞬变信号（transient signals from binary black holes）搜索的干扰。

Data: 数据来源于引力波探测器（gravitational-wave detectors）采集的时域非符合（temporal anticoincidence）观测数据及匹配滤波搜索中使用的模拟信号（simulated signals）。

Method: 采用基于时间反符合（temporal anticoincidence）的噪声瞬态抑制技术，通过地理分离探测器间非同时发生的异常触发信号进行剔除。

Result: 基于时间反符合（temporal anticoincidence）的噪声瞬态抑制技术能有效提升引力波探测器对短时瞬态信号的探测效率。
...
### [45] [On the exceptionality of exceptional gravitational-wave events](https://arxiv.org/abs/2601.02467)
*Rodrigo Tenorio,Davide Gerosa*

**TL;DR**: 论文指出引力波事件GW231123的总质量(total mass)和GW241110的自旋(spin)异常性(exceptionality)可能源于测量误差而非真实极端值，其中GW241110的自旋反排列(anti-aligned)概率被高估约70%。


<details>
  <summary>Details</summary>

Background: 研究探讨了引力波天文学中“异常”（exceptional）源的定义及其可能由测量误差而非真实极端值导致的科学问题。

Data: 数据来源于引力波事件GW231123和GW241110的观测数据。

Method: 采用贝叶斯推断（Bayesian inference）结合先验分布（agnostic priors）量化评估测量误差对“异常性”（exceptionality）声称的影响。

Result: 在引力波天文学中，基于不可知先验（agnostic priors）得出的“特殊性”结论需谨慎对待，特别是当测量不确定性与群体分布宽度相当时，GW241110的自旋反排列概率（约70%）实际与无自旋或排列构型一致。
...
### [46] [Resolving the Fe K$α$ Doublet of the Galactic Center Molecular Cloud G0.11-0.11 with XRISM](https://arxiv.org/abs/2601.02482)
*Stephen DiKerby,Shuo Zhang,Kumiko Nobukawa,Masayoshi Nobukawa,Yuma Aoki,Jack Uteg*

**TL;DR**: 通过XRISM-Resolve对银河中心分子云G0.11-0.11的高分辨率X射线光谱分析首次解析了其Fe Kα线复合体（Fe Kα line complex），发现其由能量分别为6.4040 keV（Fe Kα₁）和6.3910 keV（Fe Kα₂）的两条窄线组成且无次级线存在，排除了宇宙射线电离模型（cosmic ray ionization model）并支持X射线反射模型（X-ray reflection model），表明该分子云的Fe Kα辐射源自约200年前人马座A*（Sgr A*）的一次X射线爆发（X-ray outburst），其8 keV波段光度需达10³⁸ erg/s。


<details>
  <summary>Details</summary>

Background: 银河中心分子云(G0.11-0.11)的Fe Kα线发射机制存在X射线反射和宇宙线电离两种竞争性解释模型。

Data: 数据来源于XRISM-Resolve对银河系中心分子云G0.11-0.11进行的高分辨率X射线光谱观测，首次解析了其Fe Kα线复合体。

Method: 采用XRISM-Resolve高分辨率X射线光谱技术首次解析了G0.11-0.11分子云的Fe Kα线复合体，通过测量Fe Kα1（6.4040 keV）和Fe Kα2（6.3910 keV）的精细结构、非仪器展宽（≈3 eV）及径向速度（vLSR = 50±12fit±14scale km/s），并检测次级Fe Kα线的缺失，排除了宇宙射线电离模型，确立了X射线反射模型为G0.11-0.11的Fe Kα辐射机制。

Result: 银河中心分子云G0.11-0.11的Fe Kα线复合体被首次分解为Fe Kα₁（6.4040 keV）和Fe Kα₂（6.3910 keV），其非仪器展宽FWHM≈3 eV接近量子力学预测宽度，且未检测到次级Fe Kα线，排除了宇宙射线电离模型，支持X射线反射模型。
...
### [47] [Thick Disks, Thin Hopes: Suppressed Capture and Merger Rates in AGN](https://arxiv.org/abs/2601.02487)
*Yashvardhan Tomar,Philip F. Hopkins,Kyle Kremer*

**TL;DR**: 研究发现吸积盘（accretion disk）厚度（H/R）对超大质量黑洞周围天体相互作用事件（如引力波并合）的发生率具有极强的负相关依赖（可达(H/R)^(-8)），且不同压力主导模型下外盘厚度差异可达千倍以上，导致某些模型预测事件发生率比传统薄盘模型低数十个量级，凸显考虑盘厚度和压力来源对准确预测的关键影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超大质量黑洞（supermassive black hole）周围吸积盘（accretion disk）的结构与支撑机制，探讨不同压力主导模型（如热压、辐射压、磁压或湍流）对盘中天体动力学过程（如引力波并合、俘获事件）发生率的显著影响。

Data: 数据来源于对活动星系核（AGN）吸积盘（accretion disk）不同压力支撑模型（包括热压力主导的标准薄盘模型、几何厚度较大的辐射或磁压或湍流主导模型）的理论模拟（simulation）分析。

Method: 采用引力截面（gravitational cross-sections）分析方法，重点研究盘厚度（H/R）对过程速率的强烈逆依赖关系（如(H/R)^{-8}），并考虑不同压力支撑源（如磁压力）对盘参数的影响。

Result: 引力截面介导的过程（如捕获率）与盘厚度（H/R）呈强反比关系（可达(H/R)^{-8}），且不同压力支撑模型下H/R变化幅度超过1000倍，导致某些模型预测速率降低数十个数量级。
...
### [48] [Inflation in Extra-Dimensions with one or two branes](https://arxiv.org/abs/2601.02982)
*Nicolás Bernal,Catarina Cosme,Andrea Donini,Nuria Rius*

**TL;DR**: 研究单态暴胀（monomial inflation）和α吸引子暴胀（α-attractor inflation）在包含暗维度（Dark Dimension）和两种Randall-Sundrum（RS1、RS2）额外维框架下的表现，发现单态暴胀在所有场景中均被强烈排除，而α吸引子暴胀与观测数据高度吻合且额外维框架比标准四维情形更具灵活性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于在额外维度框架下探讨暴胀模型（monomial inflation和α-attractor inflation）与观测数据的兼容性，旨在揭示额外维度对早期宇宙膨胀动力学的影响。

Data: 数据来源于普朗克卫星（Planck）、BICEP实验和ACT观测项目的实验数据。

Method: 采用弗里德曼方程（Friedmann equations）推导和慢滚参数（slow-roll parameters）计算作为核心分析方法，结合普朗克（Planck）、BICEP和ACT实验数据对暴胀模型进行拟合。

Result: α-吸引子暴胀（α-attractor inflation）在所有额外维框架中均能极好拟合当前观测数据，而单项式暴胀（monomial inflation）则在所有情景下均被强烈排除。
...
### [49] [Standard Model Higgs Peaks: a Note on the Vacuum Instability during Inflation](https://arxiv.org/abs/2601.03231)
*G. Franciolini,A. Kehagias,A. Riotto*

**TL;DR**: 论文指出，在宇宙暴胀（inflation）期间，采用描述希格斯场（Higgs field）峰值（maxima）的极值统计（extreme-value statistics）可更准确地推导避免真空不稳定性（vacuum instability）的条件，所得哈勃率（Hubble rate）约束虽仅比文献常用方法严格√2倍，但具有定性差异。


<details>
  <summary>Details</summary>

Background: 研究探讨了标准模型（Standard Model）中希格斯势（Higgs potential）在高场值下的不稳定性及其在宇宙暴胀（cosmic inflation）期间由量子涨落驱动的真空衰变风险。

Data: 研究中使用的数据来源为理论模拟（simulation）数据，基于标准模型（Standard Model）中希格斯势（Higgs potential）在高场值下的不稳定性分析。

Method: 采用极值统计（extreme-value statistics）分析希格斯场峰值以避免真空不稳定性。

Result: 极端值统计（extreme-value statistics）是推断避免希格斯势真空不稳定性条件的正确统计方法。
...
### [50] [Catching 3I/ATLAS Using a Solar Oberth](https://arxiv.org/abs/2601.02533)
*Adam Hibberd,T. Marshall Eubanks*

**TL;DR**: 该论文通过"最佳星际轨道软件"(Optimum Interplanetary Trajectory Software)模拟验证了利用太阳奥伯斯机动(Solar Oberth Manoeuvre)实现化学推进航天器在2031-2037年间(最优为2035年)发射并拦截第三颗星际天体3I/ATLAS的可行性，发现该方案可支持约500公斤有效载荷但面临显著技术挑战。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用太阳奥伯斯效应（Solar Oberth Manoeuvre, SOM）探索化学推进系统对第三颗星际天体3I/ATLAS的间接探测任务可行性。

Data: 研究中使用的数据来源为通过'Optimum Interplanetary Trajectory Software'（最佳星际轨道软件）进行的数值模拟（simulation）数据。

Method: 采用“最佳星际轨道软件”（Optimum Interplanetary Trajectory Software）模拟化学推进的间接方案，即太阳奥伯斯机动（Solar Oberth Manoeuvre, SOM），通过低近日点燃烧利用奥伯斯效应（Oberth Effect）加速航天器。

Result: 利用太阳奥伯斯机动（SOM）模拟表明，2035年发射可实现最高效的星际物体3I/ATLAS拦截任务，需在距太阳中心3.2个太阳半径处实施机动，携带约500公斤有效载荷并配备热防护罩。
...
### [51] [Meteor observations as a tool to constrain cosmogonic models of the Solar System](https://arxiv.org/abs/2601.02599)
*Vitalii Kuksenko,Juraj Tóth*

**TL;DR**: 通过分析AMOS系统观测到的流星数据，发现奥尔特云(Oort cloud)中存在异常坚固的岩石流星体，为限制奥尔特云冰质与岩石成分比例提供了观测约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用奥尔特云(Oort cloud)中耐火物质(asteroidal material)的观测数据约束太阳系起源模型(cosmogonic models)，探讨不同模型对奥尔特云内岩石天体数量的预测差异。

Data: 数据来源于位于加那利群岛和智利的全天空流星轨道系统（AMOS）相机收集的两个流星数据库。

Method: 采用全天空流星轨道系统(AMOS)的观测数据，通过分析异常强岩石流星体的轨道特征和通量计算来约束奥尔特云(Oort cloud)中冰质与岩石成分的比例。

Result: 在奥尔特云(Oort cloud)中发现了异常坚固的岩石流星体，其轨道特征表明源自彗星，这为限制奥尔特云冰质与岩石成分比例提供了观测依据。
...
### [52] [Where does the simplified Stellar Contamination Model fail in Exoplanet Transmission Spectroscopy?](https://arxiv.org/abs/2601.02621)
*Viktor Y. D. Sumida,Raissa Estrela,Mark Swain,Adriana Valio*

**TL;DR**: 论文提出了像素级框架ECLIPSE-Xlambda，通过考虑临边昏暗（limb darkening）、活动区分布和凌日几何结构，改进了仅基于填充因子和光谱对比度的R-TLSE方法，发现光学波段对活跃恒星和非赤道凌日的修正差异可达400 ppm，并证明单纯恒星污染难以解释LHS 1140 b的短波斜率，需结合真实大气贡献。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星光球非均匀性（如黑子、光斑）对凌星光谱的扭曲效应及其对行星-恒星半径比（Transit Light Source Effect, TLSE）的波长依赖性偏差影响。

Data: 研究中使用的数据来源为JWST/NIRISS SOSS（近红外成像仪和无缝光谱仪）对LHS 1140 b的观测光谱以及理想化无噪声模型与R-TLSE（Rackham Transit Light Source Effect）的模型间比较。

Method: 采用像素级解析框架ECLIPSE-Xlambda，整合了临边昏暗效应、活动区分布及凌星几何构型，并与R-TLSE（Rackham Transit Light Source Effect）模型进行无噪声理想化对比。

Result: 像素级模型ECLIPSE-Xlambda揭示了R-TLSE（Rackham-TLSE）在光学波段（误差达400 ppm）和近红外波段（误差约10 ppm）的差异，并证实极端亮斑（delta Tfac约600 K，ffac约0.35）的物理不合理性，表明需结合大气信号与适度亮斑模型。
...
### [53] [Orchestral AI: A Framework for Agent Orchestration](https://arxiv.org/abs/2601.02577)
*Alexander Roman,Jacob Roman*

**TL;DR**: Orchestral框架为多LLM（Large Language Model）提供商提供了统一、类型安全的轻量级接口，通过通用消息表示和自动工具模式生成解决了API碎片化和可移植性问题，支持确定性执行和模块化扩展。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决大语言模型(LLM)代理框架在跨提供商集成时面临的API碎片化、消息格式不兼容等工程挑战。

Data: data部分：研究使用了多源LLM供应商的API接口数据，通过Orchestral框架统一处理不同供应商的消息格式、工具调用和流式传输行为差异。

Method: method：采用轻量级Python框架Orchestral，通过统一类型安全接口（type-safe interface）、自动工具模式生成（automatic tool schema generation）和同步执行模型（synchronous execution model）实现跨LLM供应商的无缝集成。

Result: Orchestral框架通过统一的类型安全接口解决了多LLM提供商工具调用的碎片化问题，实现了跨提供商的消息、工具和LLM使用的通用表示。
...
### [54] [Discovery of an Extremely Luminous Type II Cepheid in the Andromeda Giant Stellar Stream: Evidence for a Hierarchical Triple with an Inner Binary Merger](https://arxiv.org/abs/2601.02960)
*Pinjian Chen,Bingqiu Chen,Xiaodian Chen,Haibo Yuan,Jianrong Shi,Shu Wang,Chunqian Li,Jiyu Wang,Jianxing Zhang,Yi Ren*

**TL;DR**: 发现了仙女座恒星流中最亮的后渐近巨星支（post-AGB）II型造父变星（TIIC）LAMOST J0041+3948，其光谱能量分布（SED）显示近红外超和双星尘埃盘特征，结合演化轨迹和s-过程富集证据，提出该天体源自三重系统内双星合并的罕见形成机制。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于在仙女座巨恒星流中发现的一颗极亮后渐近巨星支（post-AGB）II型造父变星（TIIC），探讨其双星系统、尘埃盘及可能的形成机制（如恒星合并）对理解恒星演化及星系并合事件的影响。

Data: 数据来源包括LAMOST（郭守敬望远镜）的光谱数据、ZTF（兹威基瞬变设施）的光变曲线数据以及后续光谱观测数据。

Method: 采用光谱能量分布（SED）拟合和理论演化轨迹对比分析，结合ZTF光变曲线和后续光谱观测，检测到s-过程元素富集和吸积盘特征。

Result: 在仙女座巨型恒星流中发现了一颗目前已知最亮的后渐近巨星支(post-AGB) II型造父变星(TIIC) LAMOST J0041+3948，其光谱能量分布(SED)显示出强烈的近红外超，表明存在环双星尘埃盘和双星系统。
