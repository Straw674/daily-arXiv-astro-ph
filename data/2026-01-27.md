<div id=toc></div>

# Table of Contents

- [astro-ph.GA](#astro-ph.GA) [Total: 24]
- [astro-ph.CO](#astro-ph.CO) [Total: 13]
- [astro-ph.IM](#astro-ph.IM) [Total: 10]
- [astro-ph.HE](#astro-ph.HE) [Total: 5]
- [physics.flu-dyn](#physics.flu-dyn) [Total: 1]
- [gr-qc](#gr-qc) [Total: 1]
- [physics.class-ph](#physics.class-ph) [Total: 1]
- [eess.SY](#eess.SY) [Total: 2]
- [physics.optics](#physics.optics) [Total: 1]
- [stat.ME](#stat.ME) [Total: 1]
- [astro-ph.SR](#astro-ph.SR) [Total: 2]
- [astro-ph.EP](#astro-ph.EP) [Total: 3]


<div id='astro-ph.GA'></div>

# astro-ph.GA [[Back]](#toc)

### [1] [The Impact of Star Formation Histories on the Inner Dark Matter Density Slopes of Galaxies](https://arxiv.org/abs/2601.17113)
*Jorge Sarrato-Alós,James Bullock,Arianna Di Cintio,Christopher Brook,Fernando Valenciano,Andrea V. Macciò*

**TL;DR**: 研究发现恒星形成历史（SFH）的爆发性（burstiness）和时间分布影响模拟星系暗物质密度剖面内区（1-2% R$_{\rm vir}$）的核化（cored）或尖峰（cuspy）形态，其中高爆发性和延后恒星质量形成（$M_{\star, \rm post}$ / $M_{\star, \rm pre}$）更易形成核化剖面，而平滑SFH和早期恒星聚集则维持尖峰剖面。


<details>
  <summary>Details</summary>

Background: 研究探讨了模拟星系中恒星形成历史（SFH）与暗物质内密度轮廓（cored versus cuspy profiles）的关联性，聚焦于恒星形成的爆发性（burstiness）和时间分布对暗物质轮廓演化的影响。

Data: 数据来源于NIHAO和FIRE-2项目的模拟星系数据。

Method: 采用双时间尺度恒星形成率(SFR)对比标准量化恒星形成爆发性(burstiness)，并通过计算再电离时期前后恒星质量比($M_{\star, \rm post}$ / $M_{\star, \rm pre}$)量化恒星形成历史(SFHs)持续时间。

Result: 恒星形成历史（SFH）的爆发性（burstiness）和时间分布决定了暗物质密度剖面的核心（cored）或尖峰（cuspy）形态，爆发性更强且形成时间更延展的星系更易形成核心剖面。

Abstract: Aims. We aim to investigate the connection between star formation histories (SFHs) and the inner dark matter density profiles of simulated galaxies. In particular, we test whether the burstiness and temporal distribution of star formation influence the formation of cored versus cuspy dark matter profiles. Methods. We homogeneously analysed simulated galaxies from the NIHAO and FIRE-2 projects. For each galaxy, we derived dark matter density profiles and measured the logarithmic slope in the inner region of the dark matter halo (1-2% of R$_{\rm vir}$). To characterise star formation burstiness, we introduced a criterion based on comparing the star formation rate (SFR) averaged over two distinct timescales. We further quantified the duration of SFHs by computing $M_{\star, \rm post}$ / $M_{\star, \rm pre}$, the ratio of stellar mass formed after versus before the epoch of reionisation at redshift z $\sim$ 6.5. Results. Homogeneous analysis reveals that inner slope versus stellar-to-halo mass ratio trends for NIHAO and FIRE-2 galaxies are in much better agreement than reported in previous works. The burstiness and duration of the SFH explain the scatter in the inner slope versus stellar-to-halo mass ratio relation, revealing that galaxies with above average burstiness and more extended SFHs are more efficient at developing cored dark matter profiles. In contrast, galaxies with smoother SFHs and earlier stellar mass assembly tend to maintain cuspier dark matter profiles. We present an analytic expression that improves predictions for the inner slope using the parameter $M_{\star \rm,post}$ / $M_{\star \rm,pre}$, which reduces the mean squared error in both simulation suites relative to previous formulations based solely on the stellar-to-halo mass ratio.

</details>

### [2] [An Enhanced Isothermal Jeans Approach to Constraining Dark Matter Self-Interactions from Galactic Kinematics](https://arxiv.org/abs/2601.17118)
*Zixiang Jia,Fangzhou Jiang,Shubo Li,Ran Li,Jing Wang,Ling Zhu*

**TL;DR**: 基于自相互作用暗物质(SIDM)等温金斯模型改进的半解析方法拟合SPARC星系旋转曲线，发现速度依赖的散射截面参数呈现L型简并分布（σ0≈5 cm²/g、ω≈250 km/s），优于冷暗物质(CDM)模型且与反馈强度无关，为矮星系多样性提供了新解释。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过改进的自相互作用暗物质（SIDM）半解析模型约束其散射截面参数，以解释星系旋转曲线观测与冷暗物质（CDM）模型在小尺度结构上的差异。

Data: 数据来源于SPARC星系旋转曲线（SPARC galaxy rotation curves）。

Method: 采用改进的半解析模型（semi-analytical model）结合等温金斯（isothermal Jeans）方法，整合了速度依赖截面（velocity-dependent cross sections）、经验性核心坍缩（core collapse）处理及增强的鲁棒性求解机制。

Result: 基于SPARC星系旋转曲线约束自相互作用暗物质(SIDM)的散射截面，发现参数空间呈现L形简并性，支持两种可行模型：近似恒定的低截面($σ_0\sim2\,{\rm cm}^2$/g, $ω\gtrsim500\,$km/s)和强速度依赖模型($σ_0\sim100\,{\rm cm}^2$/g, $ω\sim60\,$km/s)，其中核增长解释的最佳拟合值为$σ_0\simeq5\,{\rm cm}^2$/g和$ω\simeq250\,$km/s。

Abstract: We present an improved semi-analytical model to predict density profiles of self-interacting dark matter (SIDM) halos and apply it to constrain the self-scattering cross section using SPARC galaxy rotation curves. Building on the isothermal Jeans approach, our model incorporates (i) velocity-dependent cross sections, (ii) an empirical treatment of core collapse, and (iii) enhanced robustness for identifying solutions. These advances allow us to fit a large sample of galaxies, including systems with baryon-dominated centers often excluded in earlier studies. We find that roughly 1/6 of galaxies admit both a core-growth and a core-collapse solution, while the rest favor a unique evolutionary state. Joint constraints across the sample reveal clear velocity dependence: the allowed parameter space forms an L-shaped degeneracy, where both nearly constant, low cross sections ($σ_0\sim2\,{\rm cm}^2$/g, $ω\gtrsim500\,$km/s) and strongly velocity-dependent models ($σ_0\sim100\,{\rm cm}^2$/g, $ω\sim60\,$km/s) are viable. Adopting the core-growth interpretation yields best-fit values $σ_0\simeq5\,{\rm cm}^2$/g and $ω\simeq250\,$km/s. Our constraints are remarkably consistent with previous results derived from a variety of independent probes. Compared to cold dark matter (CDM) models, SIDM outperforms simple adiabatic-contraction profiles and rivals empirical feedback-based CDM profiles, yet shows no correlation with stellar-to-halo mass ratio, a proxy for feedback strength, offering a distinct explanation for dwarf galaxy diversity. Moreover, SIDM does not affect galaxy-halo scaling relations significantly and makes concentration systematically lower. Our results highlight SIDM as a compelling framework for small-scale structure, while future low-mass kinematic data will be crucial for breaking degeneracies in velocity-dependent cross-section models.

</details>

### [3] [The mass distribution in and around the Local Group](https://arxiv.org/abs/2601.17125)
*Ewoud Wempe,Simon D. M. White,Amina Helmi,Guilhem Lavaux,Jens Jasche*

**TL;DR**: 通过ΛCDM（冷暗物质）模拟发现，局域星系群（Local Group）周围10 Mpc内质量高度集中于平面结构且存在深空区，这种扁平几何构型解释了局域星系群动力学质量估计与周围速度场的矛盾，验证了标准宇宙学模型的内部一致性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用ΛCDM（冷暗物质）模型模拟本地星系群（Local Group）及其周围物质分布，以解释观测到的哈勃流（Hubble flow）异常平静现象及其各向异性特征。

Data: 使用初始条件与两个主晕（halo）及周围流动观测动力学匹配的$Λ$CDM模拟（simulation）生成的本地群（Local Group）类比数据。

Method: 采用ΛCDM（冷暗物质）模拟对局域群（Local Group）类似物进行建模，初始条件约束为匹配两个主要晕（halo）及周围流动的观测动力学特征。

Result: 在ΛCDM（冷暗物质）模型中，局域群（Local Group）周围10 Mpc范围内质量高度集中于平面结构且表面密度向外递增，上下存在深空区，这种扁平几何结构解释了局域群动力学质量估计与周围速度场的一致性。

Abstract: Our Galaxy, Andromeda and their companion dwarf galaxies form the Local Group. Most of the mass in and around it is believed to be dark matter rather than gas or stars, so its distribution must be inferred from the effect of gravity on the motion of visible objects. Modelling efforts have long struggled to reproduce the quiet Hubble flow around the Local Group, as they require unrealistically little mass beyond the haloes of the two main galaxies. Here we revisit this using $Λ$CDM simulations of Local Group analogues with initial conditions constrained to match the observed dynamics of the two main haloes and the surrounding flow. The observations are reconcilable within $Λ$CDM, but only if mass is strongly concentrated in a plane out to 10 Mpc, with the surface density rising away from the Local Group and with deep voids above and below. This configuration, dynamically inferred, mirrors known structures in the nearby galaxy distribution. The resulting Hubble flow is quiet yet strongly anisotropic, a fact obscured by the paucity of tracers at high supergalactic latitude. This flattened geometry reconciles the dynamical mass estimates of the Local Group with the surrounding velocity field, thus demonstrating full consistency within the standard cosmological model.

</details>

### [4] [The multiple coherence scales of C IV at cosmic noon](https://arxiv.org/abs/2601.17127)
*H. Cortés-Muñoz,S. Lopez,N. Tejos,J. -K. Krogager,D. Zamora,R. Cuellar,P. Anshul,F. Urbina,A. Afruni*

**TL;DR**: 通过分析类星体对中CIV吸收系统的两点相关函数，发现CIV富集区域在红移z≈2时的典型尺度为654 kpc，而单个CIV云结构的相干长度为4.7 kpc，表明CIV能有效示踪宇宙正午时期星系周介质(CGM)的小尺度内部结构及星系团簇特征。


<details>
  <summary>Details</summary>

Background: 研究主题为利用类星体对（quasar pairs）观测宇宙正午时期（cosmic noon）的星系周介质（circumgalactic medium, CGM）中CIV吸收系统的空间和运动学结构。

Data: 数据来源于甚大望远镜(VLT)紫外及可见光阶梯光谱仪(UVES)和凯克望远镜(Keck)高分辨率阶梯光谱仪(HIRES)获取的8对投影类星体对和4对引力透镜类星体对的高分辨率光谱(R=45000)，覆盖横向分离尺度从亚千秒差距到数兆秒差距，红移范围1.6<z<3.3。

Method: 采用高分辨率光谱（R=45000）对类星体对进行观测，并利用沃伊特(Voigt)剖面拟合和两点相关函数ξ(Δv, Δr)分析CIV吸收系统的空间与运动学结构。

Result: CIV吸收系统的两点相关函数ξ(Δr)在Δr≈0.1 kpc处达到峰值，随后在Δr≈5 kpc内持续下降并在Δr≈500 kpc内保持平坦后再次下降，推断出CIV富集区域(z≈2)的代表性尺度r₁=654⁺¹⁰⁰₋₈₇ kpc以及单个CIV云团尺度r₂=4.70⁺¹.⁶⁰₋₁.₁₉ kpc。

Abstract: The spatial and kinematic structure of the circumgalactic medium (CGM) remains poorly constrained observationally. In this article we compute the clustering of CIV absorption systems at cosmic noon using quasar pairs. We analyze VLT/UVES and Keck/HIRES high-resolution spectra (R = 45000) of a sample of 8 projected and 4 lensed quasar pairs that probe transverse separations, $Δr$, from sub-kpc to a few Mpc, over the redshift range 1.6 < z < 3.3. We detect and fit Voigt profiles to a total of 141 CIV systems, corresponding to 620 velocity components across all quasar lines-of-sight. We compute the two-point correlation function of CIV, $ξ(Δv, Δr)$, where $Δv$ is the velocity difference between components across all available scales. We find a strong dependence of $ξ(Δr)$ with $Δr$ at all velocities. $ξ(Δr)$ reaches a sharp peak at the smallest scales analyzed here, $Δr\approx 0.1$ kpc, decreases steadily up to $Δr\approx 5$ kpc and remains flat up to $Δr\approx 500$ kpc, where it begins to decrease again. By fitting power-laws to the projected transverse correlation function $Ξ(Δr)$, we infer two coherence lengths: $r_1 = 654^{+100}_{-87}$ kpc, which we interpret as a representative size for the CIV enriched regions at $z\approx 2$, and $r_2 = 4.70^{+1.60}_{-1.19}$ kpc for the individual CIV-bearing "clouds". Projecting instead in $Δr$, we find consistent amplitudes of $ξ(Δv)$ with previous work using quasars and extended background sources. Our results suggest that CIV may be a good tracer of not only the small, internal structure of the circumgalactic medium, but also of the way in which galaxies cluster at cosmic noon.

</details>

### [5] [Direct Abundance Maps and Radial Metallicity Gradients of two Galaxies at z~4-5 in the GARDEN Survey](https://arxiv.org/abs/2601.17148)
*L. Stanghellini,S. A. Kassin,C. Pacifici,J. E. Morrison,M. E. Dickinson,E. Sukay,C. R. Mulcahey,L. E. Bergeron,M. W. Regan,C. N. A. Willmer,B. J. Weiner,N. Dencheva,D. Law,A. de la Vega,A. M. Koekemoer,C. Conselice,J. P. Gardner,Y. Guo,F. Hammer,A. Henry,B. W. Holwerda,J. Kartaltepe,R. A. Lucas,M. Puech,M. Rafelski,I. Shivaei,C. Welker,X. Xu,L. Y. A. Yung*

**TL;DR**: 利用JWST/NIRSpec MSA光谱对高红移(z=3.794和4.702)星系CANDELS 8005和7986进行空间分辨化学分析，首次通过直接法(direct method)测得氧丰度径向梯度(-0.111和-0.093 dex/kpc)，支持反馈调节化学增丰的"由内而外"(inside-out)星系形成模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用GARDEN巡天中具有极光发射线（auroral emission lines）的星系，通过空间分辨测量电子温度和直接氧丰度，探索高红移（high redshift）星系的化学演化及其与星系形成机制（如由内而外增长模型 inside-out growth）的关系。

Data: 数据来源于GARDEN巡天中两个星系（CANDELS 8005和CANDELS 7986）的JWST/NIRSpec MSA光谱观测，通过测量其极光发射线和关键星云发射线通量获取空间分辨的电子温度和直接氧丰度数据。

Method: 采用近红外光谱仪多缝光谱（NIRSpec MSA）进行空间分辨光谱观测，通过测量极光发射线（auroral emission lines）和关键星云发射线通量，结合直接法（direct-method）测定单个空间像素（spaxels）的氧丰度。

Result: 首次在红移z>0的星系中通过直接法探测到径向金属丰度梯度（CANDELS 8005为-0.111 (+0.026, -0.025) dex/kpc，CANDELS 7986为-0.093 ± 0.088 dex/kpc），支持反馈调节化学增丰的由内向外星系生长模型。

Abstract: We investigate galaxies in the GARDEN (Galaxies at All Redshifts Deciphered and Explained with the NIRSpec MSA) survey that show auroral emission lines, enabling spatially resolved measurements of electron temperature and direct oxygen abundances. Two galaxies have spectra suitable for this analysis: CANDELS 8005 at z=3.794 and CANDELS 7986 at z=4.702. For both, we measure auroral and key nebular emission-line fluxes across their full extent, allowing direct-method oxygen abundance determinations in individual spaxels. These observations demonstrate the viability of deep JWST/NIRSpec MSA spectroscopy for spatially resolved chemical analyses at high redshift, aided by weak nebular continua and low interstellar extinction. We derive global direct abundances of 12 + log(O/H) = 8.008 (+0.025, -0.027) for CANDELS 8005 and 7.89 (+0.027, -0.028) for CANDELS 7986. Emission-line diagnostics indicate neither galaxy hosts an active galactic nucleus. A first-order kinematic analysis suggests a potential merger in CANDELS 8005. The direct abundances agree with strong-line estimates from our data and recent high-redshift calibrations. We build emission line, radial velocity, strong-line abundance, electron temperature, and direct abundance maps for both galaxies. From these maps, we measure linear radial metallicity gradients of -0.111 (+0.026, -0.025) dex/kpc for CANDELS 8005 (statistically significant) and -0.093 +/- 0.088 dex/kpc for CANDELS 7986, where the large uncertainties limit significance. These results represent the first detection of a radial metallicity gradient from direct-method abundances with measurements taken in galaxies at z>0, supporting inside-out galaxy growth with feedback-regulated chemical enrichment.

</details>

### [6] [Galactic Dust Polarization in Turbulent Multiphase ISM: On the Origin of the $EE/BB$ Asymmetry](https://arxiv.org/abs/2601.17255)
*Yue Hu,Bao Truong,Thiem Hoang,Le Ngoc Tram*

**TL;DR**: 通过高分辨率三维磁流体动力学模拟结合合成偏振图，发现银河系中性介质中不稳定相(UNM)尘埃的偏振光谱指数和EE/BB功率比(约2)与普朗克观测最吻合，表明其可能是主导偏振前景的主要贡献源。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系尘埃热极化辐射（polarized thermal emission）作为宇宙微波背景（CMB）偏振测量的主要前景干扰，其统计特性由多相星际介质（ISM）中湍流与磁场的相互作用决定，尤其关注暖中性介质（WNM）、不稳定中性介质（UNM）和冷中性介质（CNM）中湍流状态及密度-磁场对齐差异对功率谱和$EE/BB$功率比的独特影响。

Data: 数据来源于高分辨率三维磁流体动力学（MHD）模拟的多相星际介质（ISM）湍流，并结合合成尘埃偏振图。

Method: 采用高分辨率三维磁流体动力学(MHD)模拟结合合成尘埃偏振图技术，量化了多相星际介质(ISM)中相依赖的湍流、各向异性及密度-磁场对齐特性。

Result: 暖中性介质（WNM）和不稳定中性介质（UNM）表现出强烈的各向异性，密度和速度结构与局部磁场紧密对齐，而冷中性介质（CNM）则显示出较低的各向异性和弱对齐。

Abstract: Polarized thermal emission from Galactic dust is the dominant foreground for CMB polarization measurements at high frequencies, with its statistical properties set by the interplay between turbulence and magnetic fields in the multiphase interstellar medium (ISM). Variations in turbulence regime and density-magnetic-field alignment across the warm (WNM), unstable (UNM), and cold (CNM) neutral media should imprint distinct signatures on the power spectra and $EE/BB$ power ratio, yet the relative phase contributions remain poorly constrained. Using high-resolution 3D magnetohydrodynamic simulations of a turbulent multiphase ISM coupled with synthetic dust polarization maps, we quantify phase-dependent turbulence, anisotropy, and alignment properties. We find that the trans-Alfvénic and transonic WNM and UNM are strongly anisotropic, exhibiting tight alignment of density and velocity structures with the local magnetic field. In contrast, the super-Alfvénic and supersonic CNM displays reduced anisotropy and weak alignment. These dynamical differences are reflected in the statistical scaling of fluctuations: the square root of the second-order velocity structure function exhibits a slope near $1/3$ in the WNM, near $1/2$ in the CNM, and intermediate in the UNM. Our synthetic observations reproduce the polarization power spectra measured by Planck. We find that polarization from UNM dust yields spectral indices most consistent with Planck, whereas WNM and CNM dust produce steeper and shallower spectra, respectively. The WNM yields $EE/BB>2$, the UNM gives $EE/BB\sim2$, and the CNM yields $EE/BB\approx1$. These results indicate that UNM dust could be the dominant contributor to the polarized foreground. We present predictions at 150 GHz to improve foreground separation.

</details>

### [7] [The rest ultraviolet to infrared spectral energy distributions of heavily reddened quasars are "V-shaped" and hot-dust poor](https://arxiv.org/abs/2601.17416)
*Matthew Stepney,Manda Banerji,Shenli Tang,Matthew J. Temple,Paul C. Hewett*

**TL;DR**: 研究发现红化类星体（HRQs）普遍具有红光学和蓝紫外连续谱特征（82%样本显示紫外超），其光谱能量分布（SED）可用红化类星体与无遮挡散射光成分组合解释，近红外SED显示热尘缺失与活动星系核（AGN）反馈导致内环尘抛射相关，且紫外散射比例与热尘发射正相关、与视线消光（E(B-V）负相关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于红移z=0.7-2.7、尘埃消光E(B-V)=0.4-1.8的63个严重红化类星体(HRQs)的静止紫外至红外光谱能量分布(SED)特征，探讨其与活动星系核(AGN)反馈及尘埃分布的关联。

Data: 数据来源为63个红移z=0.7-2.7且尘埃消光E(B-V)=0.4-1.8的强红化类星体(HRQs)的静止紫外到红外光谱能量分布(SED)观测数据。

Method: 采用光谱能量分布（SED）建模方法，结合红化类星体（reddened quasar）和未遮蔽散射光成分（unobscured scattered light component）分析紫外至红外波段数据。

Result: HRQs（高红化类星体）普遍呈现红光学和蓝紫外连续谱的SED（光谱能量分布），82%样本存在紫外超出现象。

Abstract: We present a rest-ultraviolet to infrared spectral energy distribution (SED) analysis of 63 heavily reddened quasars (HRQs) at redshifts z=0.7-2.7 and with dust extinctions E(B-V)=0.4-1.8. Our analysis demonstrates that SEDs with red optical and blue UV continua are very common in HRQs, with more than 82 per cent of the sample showing a UV-excess relative to the reddened quasar continuum. We model the SEDs by combining a reddened quasar and an unobscured scattered light component, though contributions from a star-forming host galaxy cannot be ruled out. The average scattering fraction is small (0.3 per cent). Higher scattering fractions are ruled out by the (i-K)=2.5 colour-cut used to select HRQs which pre-dates the discovery of the JWST "Little Red Dot" (LRD) population. Hence, LRDs generally have bluer UV continua. Nevertheless, four HRQs satisfy the LRD UV/optical continuum slope selections and are therefore massive, cosmic noon analogues of LRDs. Analysis of the near-infrared SEDs of HRQs reveals a deficit of hot dust relative to blue quasars, similar to what is observed in LRDs. This suggests HRQs trace a phase where strong AGN feedback processes eject dust from the inner torus. The UV scattering fraction of HRQs is weakly correlated with the amount of hot dust emission and anti-correlated with the line-of-sight extinction, E(B-V). This is consistent with the hot dust acting as the scattering medium, and the line-of-sight extinction being dominated by dust on interstellar medium scales in the host galaxy.

</details>

### [8] [The Supermassive Black Hole in the Nearby Spiral Galaxy M81: A Robust Mass from JWST/NIRSpec Stellar Dynamics](https://arxiv.org/abs/2601.17439)
*Dieu D. Nguyen,Tuan N. Le,Michele Cappellari,Hai N. Ngo,Tinh Q. T. Le,Tien H. T. Ho,Long Q. T. Nguyen,Elena Gallo,Fan Zou,Michele Perna,Niranjan Thatte,Miguel Pereira-Santaella*

**TL;DR**: 首次利用JWST/NIRSpec近红外二维运动学数据，通过贝叶斯框架下的JAM模型（Jeans Anisotropic Modelling）系统分析，确定旋涡星系M81中心超大质量黑洞（SMBH）质量为(4.78±0.09)×10⁷太阳质量，解决了其质量长期不确定性问题。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高分辨率近红外动力学观测首次精确测量旋涡星系M81中心超大质量黑洞（SMBH）的质量，以解决其长期存在的不确定性并为黑洞-星系标度关系提供可靠基准。

Data: 数据来源于JWST/NIRSpec对M81星系中心$3''\times3''$区域的高分辨率二维运动学观测，通过近红外波段追踪恒星运动并分离恒星光与非热活动星系核(AGN)连续谱。

Method: 采用JWST/NIRSpec获取的高分辨率二维运动学数据，结合贝叶斯框架下的JAM（Jeans各向异性模型）方法，系统建模点扩散函数、轨道各向异性和恒星质量-光比的不确定性，通过集成建模分析恒星运动学。

Result: 首次通过JWST/NIRSpec高分辨率二维运动学观测，结合贝叶斯框架下的JAM模型分析，确定M81星系中心超大质量黑洞(SMBH)质量为(4.78^{+0.07}_{-0.10})×10^7 M⊙，解决了长期存在的质量不确定性问题。

Abstract: Despite its proximity, the mass of the supermassive black hole (SMBH) in the spiral galaxy M81 (NGC~3031) has remained uncertain, with previous dynamical measurements being unreliable. We present the first robust stellar-dynamical measurement of its mass using high-resolution, two-dimensional kinematics from JWST/NIRSpec observations of the central $3''\times3''$. By tracing stellar motions in the near-infrared, our data penetrate the obscuring nuclear dust and allow for the separation of stellar light from the non-thermal AGN continuum. We modeled the kinematics using JAM within a Bayesian framework, exploring a comprehensive suite of models that systematically account for uncertainties in the point-spread function, orbital anisotropy, and stellar mass-to-light ratio. This ensemble modeling approach demonstrates that a central dark mass unambiguously drives the central rise in velocity dispersion. The models yield a robust SMBH mass of $M_{\rm BH} = (4.78^{+0.07}_{-0.10})\times10^7$ M$_\odot$. This result resolves a long-standing uncertainty in the mass of M81's black hole and provides a crucial, reliable anchor point for SMBH-galaxy scaling relations.

</details>

### [9] [Discovery of an Extremely Metal-Poor Galaxy at $z=3.654$ Using JWST Infrared Spectroscopy](https://arxiv.org/abs/2601.17498)
*Zijian Yu,Sijia Cai*

**TL;DR**: 发现了一个红移z=3.654的极贫金属星系（CAPERS-39810），其金属丰度12 + log(O/H)=6.73±0.13，恒星质量log(M*/M⊙)=8.02+0.22−0.34，为研究宇宙正午时期年轻星系的化学增丰和恒星形成过程提供了关键样本。


<details>
  <summary>Details</summary>

Background: 研究主题为利用詹姆斯·韦伯太空望远镜（JWST）发现高红移（z = 3.654）极贫金属（extremely metal-poor）星系CAPERS-39810，以探索宇宙正午（cosmic noon）时期年轻星系的化学增丰（chemical enrichment）和恒星形成过程。

Data: 数据来源为詹姆斯·韦伯太空望远镜（JWST）的NIRSpec/MSA光谱分析结合COSMOS2025目录的光度测量数据。

Method: 采用JWST NIRSpec/MSA进行光谱分析，结合R3强线诊断法（R3 strong-line diagnostic method）估算金属丰度，并利用COSMOS2025目录的光度数据进行补充。

Result: 在红移z=3.654处发现了一个极贫金属（extremely metal-poor）星系CAPERS-39810，其金属丰度12 + log(O/H) = 6.73±0.13，化学组成原始，类似于宇宙早期星系形成阶段。

Abstract: We report the discovery of an extremely metal-poor galaxy at a redshift of z = 3.654, identified through infrared spectroscopy using the James Webb Space Telescope (JWST). This galaxy, CAPERS-39810, exhibits a metallicity of 12 + log(O/H) = $6.73\pm0.13$, indicative of its primitive chemical composition, resembling the early stages of galaxy formation in the Universe. We use JWST NIRSpec/MSA for spectroscopic analysis, complemented by photometric data from the COSMOS2025 catalog. Our analysis employs the R3 strong-line diagnostic method to estimate metallicity, due to the lack of auroral lines in the spectrum. The galaxy's emission lines, including Hb, [O III], Ha and He I, are clearly detected. The rest-frame equivalent widths of the strong hydrogen recombination lines are EW_0(Hb) = $184\pm48$ Åand EW_0(Ha) = $1144\pm48$ Å. Furthermore, we perform detailed spectral energy distribution modeling to derive a galaxy logarithmic stellar mass of $8.02^{+0.22}_{-0.34}$ $M_\odot$. This discovery adds to the growing body of evidence for the existence of very low-metallicity galaxies existed at cosmic noon of $z\approx3$, which are crucial for understanding the processes of chemical enrichment and star formation in young galaxies at the cosmic noon.

</details>

### [10] [Disentangling Drivers of Disk Warps in Tilted and Tumbling TNG50 Halos](https://arxiv.org/abs/2601.17599)
*Saarthak Johri,Neil Ash,Monica Valluri*

**TL;DR**: 研究发现暗物质晕（DM halo）的三轴性和图像旋转（figure rotation）与恒星盘翘曲（warp）存在关联，特定情况下晕的倾斜、旋转轴与盘自旋轴的错位等因素会显著影响翘曲程度。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗物质晕（DM halos）的三轴性和自转如何影响孤立星系盘中翘曲（warps）的形成机制。

Data: 数据来源于TNG50模拟套件（TNG50 simulation suite）中40个孤立晕（isolated halos）及其恒星盘（stellar disks）的模拟数据，时间跨度为约40亿年（$\sim 4$~Gyr），使用了25个线性间隔的快照（snapshots）覆盖回溯时间（lookback time）$t_{lb}$从0到2.33 Gyr的范围。

Method: 采用改进的互相关分析（modified cross-correlation analysis）测量盘面翘曲角（warp angle）与图形旋转模式速度（figure rotation pattern speed）及盘自转轴与图形旋转轴、晕短轴、气体角动量轴之间错位（misalignment）的关系。

Result: 所有孤立星系盘都存在轻微翘曲（warp），最大翘曲角（ψ）均超过1.8°。

Abstract: Dark matter (DM) halos in $Λ$ Cold DM cosmological simulations are triaxial. Most exhibit figure rotation. We study 40 isolated halos with stellar disks from the TNG50 simulation suite across $\sim 4$~Gyr to understand whether and how a triaxial halo's tumbling and orientation relative to the disk can drive warps. We measure a warp angle $ψ$ and find even our isolated disks are all at least slightly warped, with each galaxy's maximum $ψ> 1.8^{\circ}$. We perform a modified cross-correlation analysis between $ψ$ and the figure rotation pattern speed, as well as the misalignment between the disk spin axis and (a) the figure rotation axis, (b) the halo minor axis, and (c) the gas angular momentum axis. We use snapshots spanning a lookback time $t_{lb} ~4$ Gyr with 25 linearly-spaced lags from $ 0 - 2.33$ Gyr. We do not find evidence for a consistent lag between the onset of a warp and any of the aforementioned factors on the population level. However, we find significant correlations between individual time-series at various lags. These maximum correlation coefficients were significantly offset from random chance at the population level, suggesting that several of these factors do correlate with disk warping in specific situations. By examining four case studies whose maximum correlation coefficients were significantly higher than random chance, we establish clear qualitative relationships between these factors and warps. While a non-warped galaxy typically shows minimal halo tilt and figure rotation, warped galaxies can have strong/weak tilts and/or strong/weak figure rotation.
  Keywords: Disk galaxies(391), Galaxy dynamics(591), Hydrodynamical simulations(767), Galaxy DM halos(1880)

</details>

### [11] [Tracing Galaxy Evolution in the Nearby Universe: The Role of Dark Matter](https://arxiv.org/abs/2601.17610)
*A. Nigoche-Netro,P. Lagos,R. J. Diaz,E. de la Fuente,M. P. Agüero,A. Ruelas-Mayorga,S. N. Kemp,R. A. Marquez-Lugo,R. Ibarra-Nuño*

**TL;DR**: 基于SDSS约12.6万晚型星系(LTGs)样本，研究发现动力学质量与恒星质量差异(Δlog M)随质量和红移呈连续变化：低质量低红移时呈鞍形分布，高质量高红移时转为线性趋势且与早型星系行为相似，表明暗物质占比最多等于Δlog M且形态演化呈连续过渡。


<details>
  <summary>Details</summary>

Background: 研究探讨了晚型星系（LTGs）中恒星质量与动力学质量的关系及其随红移和质量变化的演化模式，揭示了暗物质在宇宙结构和演化中的关键作用。

Data: 数据来源于SDSS（Sloan Digital Sky Survey）中约126,000个晚型星系（LTGs）的样本，恒星质量通过八种采用恒定初始质量函数（IMF）的恒星种群合成（SPS）模型估算，动力学质量则基于牛顿动力学和维里平衡的七种公式计算，同时考虑了恒星和气体速度弥散。

Method: 采用八种恒星种群合成（SPS）模型估算恒星质量，并基于牛顿动力学和位力平衡的七种公式计算动力学质量，同时考虑倾角、颜色、浓度和Sérsic指数等关键因素。

Result: 晚型星系(LTGs)中动力学质量与恒星质量的差异($Δ\log \mathbf{M}$)呈现从近零至约95%动力学质量的连续变化，其分布形态随质量和红移演化，且在高质量区与早型星系行为相似。

Abstract: Using a sample of $\sim$126,000 late-type galaxies (LTGs) from SDSS, we analyzed stellar mass as a function of dynamical mass. Stellar masses were estimated using eight SPS models with constant IMFs, while dynamical masses were derived from seven formulations based on Newtonian dynamics and virial equilibrium, incorporating both stellar and gas velocity dispersions. We account for key factors affecting dynamical mass estimation, including inclination, colour, concentration, and Sérsic index. We find that the difference between dynamical and stellar mass ($Δ\log \mathbf{M}$) ranges from nearly zero to $\sim$95% of the dynamical mass, depending on mass and redshift. $Δ\log \mathbf{M}$ appears to decreases with increasing redshift, but exhibits a saddle-like shape at low mass and low redshift-especially in disk-dominated LTGs-transitioning into a steep, linear trend at higher masses and redshifts. In the high-mass regime, the behavior resembles that of early-type galaxies. Moreover, our results indicate that this evolution is not discrete but follows a continuous transition between morphological regimes. Dark matter within LTGs is at most equal to $Δ\log \mathbf{M}$, depending on the impact of the IMF and SPS on stellar mass estimation. Although SPS-based stellar masses do not include the gas component, previous studies have shown that galaxies with log($\mathbf{M_{Stellar}/M_{Solar}}) > 10$ at $z \leq 0.3$ are predominantly stellar-mass dominated. Most galaxies in our sample fall within this regime, minimizing the impact of gas exclusion. Our findings go beyond the scope of single galaxies, providing insight into the nearby Universe and highlighting the influence of dark matter in determining the Universe's structure and evolution.

</details>

### [12] [Escaping ionizing photons from massive spiral galaxies at $z\sim 1$](https://arxiv.org/abs/2601.17746)
*Soumil Maulick,Kanak Saha*

**TL;DR**: 在红移约1处首次探测到三个大质量（$\text{M}_{*}>10^{10}\:\text{M}_{\odot}$）近正面（face-on）螺旋星系的莱曼连续谱（LyC）光子，表明星系取向可能增强盘状星系LyC逃逸概率，并揭示需结合几何因素和各向异性逃逸路径来理解LyC逃逸机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(redshift)螺旋星系中莱曼连续谱(LyC)光子逃逸机制及其与星系取向(orientation)和反馈过程(feedback processes)的关联。

Data: 数据来源于AstroSat UV Deep Field South的观测数据。

Method: 采用低倾角（low inclination）观测结合活动星系核（AGN）信号分离技术，重点分析面朝视角（face-on viewing angle）对莱曼连续谱（LyC）逃逸的各向异性影响。

Result: 在近1红移处首次探测到三个大质量（$\text{M}_{*}>10^{10}\:\text{M}_{\odot}$）旋涡星系的莱曼连续谱(LyC)光子，表明低倾角（近正面）观测可能增强盘星系中LyC逃逸概率。

Abstract: We report the detection of Lyman continuum (LyC) photons from three massive ($\text{M}_{*}>10^{10}\:\text{M}_{\odot}$) spiral galaxies at a redshift of nearly 1 in the AstroSat UV Deep Field South. Notably, all three systems are viewed at low inclination (i.e., nearly face-on), prompting an investigation into the role of galaxy orientation in the detectability of LyC emission from disk systems. Two of the three galaxies, however, host active galactic nuclei (AGNs), adding complexity to the interpretation of the LyC signal. We present a detailed analysis of the likely star-forming case, and report tentative evidence that a face-on viewing angle may enhance the likelihood of LyC detection in disk galaxies. This represents the first detection of LyC emission from well-characterized spiral galaxies at high redshift, offering a new window into LyC escape mechanisms in such systems. Our findings highlight the need to consider geometric factors and anisotropic escape pathways facilitated by feedback processes alongside more traditional density-bounded scenarios that imply isotropic escape.

</details>

### [13] [Evidence of Gas Depletion in Quasars with Moderate Radio Emission](https://arxiv.org/abs/2601.17951)
*Yuhan Wen,Ran Wang,Luis C. Ho,Jinyi Shangguan,Ezequiel Treister,Guodong Li,Franz E. Bauer*

**TL;DR**: 研究发现中等射电活动（radio-detected radio quiet）的类星体（quasar）宿主星系尘埃和气体质量比其他组低约0.3 dex，且电离气体外流比例和速度更高，表明弱/中等射电活动的活动星系核（AGN）反馈效率可能高于无射电探测或强射电活动（radio loud）的类星体。


<details>
  <summary>Details</summary>

Background: 研究探讨了活动星系核（AGN）释放的能量对宿主星系冷气体特性的潜在影响，特别是通过加热或移除气体进而抑制恒星形成的过程。

Data: 数据来源包括赫歇尔空间天文台（Herschel）的尘埃连续谱探测数据以及射电星表的交叉匹配结果。

Method: 通过光谱能量分布（SED）拟合分析不同射电活动水平类星体的冷气体储量差异。

Result: 射电活动中等水平的类星体（radio-detected RQ quasars）表现出气体匮乏特征，其寄主星系的尘埃和气体质量比其他两组低约0.3 dex，且电离气体外流比例和速度更高，表明弱/中等射电活动可能比无射电探测或强射电活动更高效地引发AGN反馈。

Abstract: The energy released by active galactic nuclei (AGNs) is considered to have a profound impact on the cold gas properties of their host galaxies, potentially heating or removing the gas and further suppressing star formation. To understand the feedback from AGN radio activity, we investigate its impacts on the cold gas reservoirs in AGNs with different radio activity levels. We construct a quasar sample with a mean $z\sim1.5$ and a mean $L_{\rm bol}\sim10^{45.8}\ \rm erg\ s^{-1}$, all with Herschel detections to enable estimates of the total gas mass through the galactic dust continuum emission. The sample is then cross-matched with radio catalogs and divided into radio loud (RL) quasars, radio-detected radio quiet (RQ) quasars and radio-undetected quasars based on their radio loudness. Through spectral energy distribution (SED) fitting, we find the radio-detected RQ quasars exhibit evidence of gas deficiency with host galaxies possessing $\sim 0.3$ dex lower dust and gas masses compared to the other two groups, despite being matched in $M_{\rm BH}$, $L_{\rm bol}$, $M_{*}$ and SFR. Furthermore, evidence from optical spectra shows that both the fraction and velocity of outflows are higher in the radio-detected RQ group, suggesting a connection between the ionized gas outflows and the moderate radio activity. These results suggest that the AGN feedback could be more efficient in AGNs with weak/moderate radio emission than in those without radio detection or those with strong radio emission. Further high-resolution observations are needed to understand the interaction between the interstellar medium and the weak/moderate AGN radio activity.

</details>

### [14] [JWST Spectroscopic Census of ALMA Faint Submillimeter Galaxies in the Hubble Ultra Deep Field](https://arxiv.org/abs/2601.18149)
*Tomokazu Kiyota,Masami Ouchi,Daisuke Iono,Seiji Fujimoto,Kotaro Kohno,Yoshihiro Ueda,Kimihiko Nakajima,Moka Nishigaki,Hidenobu Yajima*

**TL;DR**: 利用JWST/NIRSpec对哈勃超深场（UDF）中16个红移z~1-4的亚毫米星系（SMGs）进行光谱分析，发现其具有强长波光学发射线（Hα、[N II]、[S II]）和弱短波光学发射线（Hβ、[O III]），且80%大质量（M*>10^10.5M⊙）SMGs为活动星系核（AGNs），金属丰度（~0.4-2Z⊙）超过尘埃形成临界值（~0.1-0.2Z⊙），表明SMGs本质上是高红移典型大质量恒星形成星系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST/NIRSpec对哈勃超深场(UDF)中ALMA 1毫米连续源进行静止光学光谱普查，揭示高红移(z~1-4)亚毫米星系(SMGs)的物理特性及其与活动星系核(AGN)的关联。

Data: 数据来源于JWST/NIRSpec对哈勃超深场（Hubble Ultra Deep Field, UDF）中ALMA 1-mm连续谱源的静止光学光谱普查，结合ALMA UDF和ASPECS项目的深度观测，样本由ALMA流量限制（$S_{1\,\mathrm{mm}}\gtrsim 0.1\,\mathrm{mJy}$）的源组成，并利用JADES和SMILES的中分辨率NIRSpec光谱观测了16个微弱亚毫米星系（submillimeter galaxies, SMGs）。

Method: 采用JWST/NIRSpec中分辨率光谱观测和ALMA 1-mm连续谱数据，结合BPT诊断图和钱德拉X射线流量分析，对亚毫米星系(SMGs)进行活动星系核(AGN)分类和金属丰度测量。

Result: 在哈勃超深场（UDF）中发现的16个微弱亚毫米星系（SMGs）在红移$z\sim1$-$4$范围内表现出高活动星系核（AGN）比例（$\sim80\%$），且主要为遮蔽型AGN。

Abstract: We present a JWST/NIRSpec rest-frame optical spectroscopic census of ALMA 1-mm continuum sources in the Hubble Ultra Deep Field (UDF) identified by the deep ALMA UDF and ASPECS programs. Our sample is composed of the ALMA flux-limited ($S_{1\,\mathrm{mm}}\gtrsim 0.1\,\mathrm{mJy}$) sources observed with medium-resolution NIRSpec spectroscopy from JADES and SMILES, 16 faint submillimeter galaxies (SMGs) at spectroscopic redshifts of $z\sim 1$-$4$. These SMGs show bright longer-wavelength optical lines (H$α$, [N II]$λ\lambda6548,6583$, and [S II]$λ\lambda6717,6731$) and faint shorter-wavelength optical lines (H$β$ and [O III]$λ\lambda4959,5007$) with a large nebular attenuation, $E(B-V)\sim0.3$-$1.8$. We test the SMGs using BPT diagnostics and Chandra X-ray fluxes, and find that most SMGs are classified as AGNs; the AGN fraction is $\sim80\%$ for the SMGs at $M_*>10^{10.5} M_\odot$. We find only one SMG ($<10\%$) with a broad Balmer line, indicating that the SMGs are predominantly obscured AGNs. With the optical lines, we estimate the metallicities of the SMGs to be moderately high, $\sim0.4$-$2 Z_\odot$, exceeding the model-predicted dust-growth critical metallicity ($\sim0.1$-$0.2Z_\odot$), which naturally explains the dusty nature of the SMGs. Interestingly, the SMGs fall in the mass-metallicity relation and the star-formation main sequence, showing no significant differences from other high-$z$ galaxies. Similarly, we find electron densities of $n_e\sim10^2$-$10^3\,\mathrm{cm}^{-3}$ for the SMGs that are comparable with other high-$z$ galaxies. Together with the high SMG fraction ($\sim 100\%$) at the massive end ($M_*>10^{10.5} M_\odot$), these results indicate that the SMGs are mostly not special, but typical massive star-forming galaxies at high redshift.

</details>

### [15] [The AIDA-TNG project: gas distributions inside and around haloes](https://arxiv.org/abs/2601.18578)
*Chi Zhang,Enrico Garaldi,Giulia Despali,Matteo Viel,Lauro Moscardini,Mark Vogelsberger*

**TL;DR**: 通过AIDA-TNG宇宙学模拟比较冷暗物质(CDM)、自相互作用暗物质(SIDM)、速度依赖SIDM(vSIDM)和温暗物质(WDM)模型，发现暗物质模型对晕的气体和中性氢(HI)分布影响有限，但SIDM能增强中心气体冷却并在近中心保留更多HI，同时给出了区分vSIDM与CDM以及SIDM1与CDM的特定晕质量范围。


<details>
  <summary>Details</summary>

Background: 暗物质（Dark Matter, DM）的本质是现代天体物理学中最突出的未解之谜之一，标准冷暗物质（Cold DM, CDM）模型虽能解释大多数天体物理尺度的观测现象，但暗物质粒子尚未被直接探测，导致多种替代模型仍有存在空间。

Data: 使用AIDA-TNG宇宙学模拟套件（AIDA-TNG cosmological simulation suite）预测冷暗物质（CDM）、自相互作用暗物质（SIDM）、速度依赖自相互作用暗物质（vSIDM）和温暗物质（WDM）模型中的气体和中性氢（HI）分布。

Method: 采用AIDA-TNG宇宙学模拟套件预测冷暗物质(CDM)、自相互作用暗物质(SIDM)、速度依赖自相互作用暗物质(vSIDM)和温暗物质(WDM)模型中的气体和中性氢(HI)分布。

Result: 暗物质(DM)模型对晕的气体和中性氢(HI)分布影响有限，但自相互作用暗物质(SIDM)能通过减弱中心势阱增强气体冷却，导致中心区域保留更多HI。

Abstract: The nature of Dark Matter (DM) is one of the most outstanding mysteries of modern astrophysics. While the standard Cold DM (CDM) model successfully explains observations on most astrophysical scales, DM particles have not yet been detected, leaving room for a plethora of different models. In order to identify their observable signatures, we use the AIDA-TNG cosmological simulation suite to predict the distributions of gas and neutral hydrogen (HI) in the CDM, Self-Interacting DM (SIDM), velocity-dependent SIDM (vSIDM), and Warm DM (WDM) models.
  We find that the DM models investigated have very limited impact on the median gas and HI profile of haloes. In particular, for the most massive haloes ($M_{\rm vir}\sim10^{14}\,\mathrm{M}_\odot$), we find that DM self-interactions can shallow the central potential and thereby enhance gas cooling. We find that, in all models, the halo-to-halo variation in the HI profiles is explained by AGN feedback, and that the specific characteristics of DM model is largely subdominant. Nevertheless, we detect some systematic difference in the case of SIDM, with more HI surviving close to the centre with respect to other models. We provide fitting functions for the gas and HI profiles.
  We investigate the galaxy-Ly$α$ cross-correlation function (\galacc) for different halo masses, redshift and observation strategies. We find that at $z=0$ vSIDM can be distinguished from CDM in haloes with $10^{12}\lesssim M_{\rm vir}\lesssim10^{13}\,{\rm M}_\odot$, while SIDM1 can be distinguished from CDM in haloes with $M_{\rm vir}\gtrsim10^{13}\,{\rm M}_\odot$. We estimate that statistically-robust detection requires sampling $\sim160$ haloes with $\sim20$ sightlines each, a task that can be achieved with current and future facilities like WEAVE, 4MOST, PFS, ELT and WST.

</details>

### [16] [In-depth analysis of bar formation mechanisms of disk galaxies in halos of different concentrations](https://arxiv.org/abs/2601.18161)
*T. Worrakitpoonpon*

**TL;DR**: 高浓度暗晕(halo)中棒旋结构(bar)形成受早期多臂模式(multi-arm modes)抑制，随后通过粒子俘获(particle trapping)机械增长；低浓度暗晕中棒旋结构由单一频率线性不稳定模式主导并触发共转共振(corotation resonance)；中等浓度暗晕则同时存在线性模式、摆动放大(swing amplification)和粒子俘获三种演化机制。


<details>
  <summary>Details</summary>

Background: 研究探讨了不同浓度暗晕（halo concentration）中盘星系棒结构（bar formation）形成的不同动力学机制及其演化过程。

Data: data部分：研究采用N体模拟（N-body simulation）数据，针对不同浓度暗晕（halo concentration）中的盘星系（disk）棒结构（bar）形成过程进行数值实验。

Method: 采用N体模拟(N-body simulations)分析不同浓度晕中盘状星系的棒结构形成机制，结合运动学模态分析和共振图(resonance diagram)等工具研究线性不稳定性、摆动放大(swing amplification)及粒子俘获(particle trapping)的时序演化关系。

Result: 在高度集中（highly concentrated）的晕（halo）中，棒（bar）的形成早期受多臂模式（multi-arm modes）主导，后期通过粒子俘获（particle trapping）机械增长，不涉及棒模式的共转共振（corotation resonance）。

Abstract: We use $N$-body simulations to investigate the distinct bar formation processes in disks residing in halos of various concentrations. In a highly concentrated halo, the bar development is limited by the dominant multi-arm modes as a result of the swing amplification in the early stage. Only after the multi-arm modes decay, the bar growth proceeds mechanically owing to the particle trapping in continuation of that bar seed. In this scheme, the corotation resonance of the bar modes does not come into play at all, justified by a low amount of disk-halo angular momentum transfer and a modestly decreasing bar pattern speed. On the other hand, although reducing the halo concentration suggests the reduction of the preferred swing-amplified modes to be bi-symmetric, the bar formation in a lowly concentrated halo does not involve the swing amplification at all. Rather, the fast-growing linearly unstable bar modes of a single uniform frequency is solely the governing factor, attributed to a mild shearing. The bar modes trigger the corotation resonance since the beginning and such resonance is maintained until the end, which leads to a high amount of angular momentum transfer and a fast slowdown. For the intermediate halo concentration, the kinematical analyses of multiple non-axisymmetric modes suggests that the linear modes, the swing amplification, and the particle trapping are all present in the evolution chronology. To specify bars formed in the different halo concentrations, full analyses of the isophotal shape, the radial Fourier amplitude, and the resonance diagram can be of use.

</details>

### [17] [Intracluster light as a dark matter tracer: how their spatial and kinematic relationship is shaped by satellite demographics](https://arxiv.org/abs/2601.18693)
*G Martin,F R Pearce,N A Hatch,H J Brown,J Butler,Y M Bahe,W Cui,Y Dubois,A Knebe*

**TL;DR**: 研究发现星系团内光（ICL）相对于暗物质（DM）晕的相空间和径向分布差异主要由卫星星系质量函数特征质量决定，且ICL始终比DM更中心聚集。


<details>
  <summary>Details</summary>

Background: 研究探讨了卫星星系（satellite galaxies）的轨道演化与质量分布如何影响星系团内光（intracluster light, ICL）相对于暗物质（dark matter, DM）晕的相空间和径向分布。

Data: 使用N体模拟(N-body simulation)追踪卫星星系在落入活体星系团晕(live cluster halo)过程中的潮汐剥离(tidal stripping)和轨道演化，系统改变卫星与宿主质量比(satellite-to-host mass ratio)和轨道圆度(orbital circularity)。

Method: 采用N体模拟(N-body simulations)追踪卫星星系在落入活体星系团晕时的潮汐剥离(tidal stripping)和轨道演化过程，并系统改变卫星与宿主质量比(satellite-to-host mass ratio)及轨道圆度(orbital circularity)。

Result: 剥离的恒星始终占据比剥离暗物质(DM)更低能量和更低角动量的相空间区域，且这种差异随卫星-宿主质量比趋近于1而显著增大。

Abstract: We investigate how the orbital evolution and mass distribution of infalling satellite galaxies shape the phase-space and radial distributions of intracluster light (ICL) relative to the underlying cluster dark matter (DM) halo. Using N-body simulations, we follow the tidal stripping and orbital evolution of satellite galaxies as they are accreted into a live cluster halo, systematically varying satellite-to-host mass ratio and orbital circularity. We measure the specific orbital energy and angular momentum of stripped stellar and DM material, finding that the stripped stars consistently occupy lower-energy and lower-angular momentum regions of phase-space than the stripped DM. The magnitude of this difference increases strongly towards more equal satellite--to--host mass ratios, while the dependence on orbital circularity is weak. We construct a predictive model for the phase-space properties of stripped stars and DM from a whole infalling satellite population and find that the resulting phase-space difference between the components are driven primarily by the characteristic mass of the infalling satellite stellar mass function. We find that the ICL is always more centrally concentrated than the DM. The magnitude of this offset depends on the characteristic mass and increases towards higher characteristic masses. Comparisons with four independent cosmological hydrodynamical simulations show that, once the infalling satellite stellar mass function is matched, the model reproduces the radial stellar-to-DM density profile offsets to better than the inter-simulation scatter. This demonstrates that the radial relationship between the ICL and the DM distribution is largely governed by satellite demographics. With adequate constraints on the infalling satellite population, ICL density profiles can therefore be used as informative tracers of the underlying radial DM distribution in clusters.

</details>

### [18] [Chemical study of two starless cores in the B213/L1495 filament](https://arxiv.org/abs/2601.18259)
*L. Moral-Almansa,A. Fuente,M. Rodríguez-Baras,T. Alonso-Albi,G. Esplugues,D. Navarro-Almaida,P. Riviére-Marichalar,B. Tercero,A. Asensio Ramos,C. Westendorp Plaza*

**TL;DR**: 通过对金牛座分子云B213/L1495纤维中无星核C2和C16的7毫米波段光谱观测，发现两者化学组成差异源于演化阶段不同（C2更接近前恒星阶段），且早期化学模型（t~0.1 Myr）能较好解释多数分子丰度，但无法重现碳链CCS和C3O的观测值。


<details>
  <summary>Details</summary>

Background: 研究聚焦于金牛座分子云B213/L1495丝状结构中星前核（pre-stellar cores）C2和C16在向原恒星（protostellar）阶段转变过程中的化学演化机制及其与环境条件、演化状态的关联。

Data: 数据来源于使用Yebes 40米望远镜对金牛座分子云B213/L1495丝状体中C2和C16两个无星核进行的7毫米波段完整光谱巡测。

Method: method：利用Yebes 40米望远镜在7毫米波段对B213/L1495丝状结构中的C2和C16星核进行完整光谱巡测，通过化学模型计算分子丰度并与不同演化阶段源进行比较。

Result: 在金牛座分子云B213/L1495丝状结构中的无星核C2和C16的化学特征表明，C2处于更晚的演化阶段，具有更高的分子氢密度、硫耗损及更低的宇宙射线电离率。

Abstract: The chemical evolution of pre-stellar cores during their transition to a protostellar stage is not yet fully understood. Detailed chemical characterizations of these sources are needed to better define their chemistry during star formation. Our goal is to characterize the chemistry of the starless cores C2 and C16 in the B213/L1495 filament of the Taurus Molecular Cloud, and to understand how it relates to the environmental conditions and the evolutionary state of the cores. We made use of two complete spectral surveys at 7 mm of these sources, carried out using the Yebes 40-m telescope. Derived molecular abundances were compared with those of other sources in different evolutionary stages and with values computed by chemical models. Including isotopologs, 22 molecules were detected in B213-C2, and 25 in B213-C16. The derived rotational temperatures have values of between $\sim$ 5 K and $\sim$ 9 K. A comparison of the two sources shows lower abundances in C2, except for l-C$_{3}$H and HOCO$^{+}$, which have similar values in both cores. Model results indicate that both cores are best fit assuming early-time chemistry, and point to C2 being in a more advanced evolutionary stage, as it presents a higher molecular hydrogen density and sulfur depletion, and a lower cosmic-ray ionization rate. Our chemical modeling successfully accounts for the abundances of most molecules, including complex organic molecules and long cyanopolynes (HC$_{5}$N, HC$_{7}$N), but fails to reproduce those of the carbon chains CCS and C$_{3}$O. Chemical differences between C2 and C16 could stem from the evolutionary stage of the cores, with C2 being closer to the pre-stellar phase. Both cores are better fit assuming early-time chemistry of t $\sim$ 0.1 Myr. The more intense UV radiation in the northern region of B213 could account for the high abundances of l-C$_{3}$H and HOCO$^{+}$ in C2.

</details>

### [19] [The Most Luminous H$β$ Reverberation Mapping of E1821+643 Indicates the Lower Boundary of the Radius-Luminosity Relation](https://arxiv.org/abs/2601.18269)
*Sha-Sha Li,Hai-Cheng Feng,Jiancheng Wu,J. M. Bai,H. T. Liu,Kai-Xing Lu,Mouyuan Sun,Jian-Guo Wang*

**TL;DR**: 通过对高光度类星体E1821+643的混响映射(RM)观测，发现其宽线区(BLR)存在双组分结构（核心组分267天和红移尾组分-49天），导致整体时延（83.2天）比标准半径-光度关系预测值短5.6倍，揭示高吸积率活动星系核(AGN)的BLR结构复杂性会引入高达数十倍的黑洞质量估计系统误差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）中宽线区半径-光度（$R_{\rm BLR}$-$L_{5100}$）关系的校准问题，尤其针对高光度端因缺乏可靠反响映射（RM）数据导致的偏差。

Data: 数据来源包括丽江2.4米望远镜开展的四年反响映射（reveration mapping）观测以及补充的归档多波段数据。

Method: 采用丽江2.4米望远镜进行为期四年的反响映射（RM）观测，并结合多波段档案数据，通过光谱分解识别出两个不同的Hβ成分（核心成分和红移尾成分），揭示了多成分宽线区（BLR）结构对时延测量的影响。

Result: 在类星体E1821+643中观测到83.2天的Hβ发射线延迟时间比标准半径-光度关系预测值短5.6倍，揭示了高吸积率活动星系核(AGN)存在多组分宽线区(BLR)结构导致延迟时间系统性缩短的现象。

Abstract: The radius-luminosity ($R_{\rm BLR}$-$L_{5100}$) relation is fundamental to active galactic nucleus (AGN) studies, enabling supermassive black hole (SMBH) mass estimates and AGN-based cosmology applications. However, its high-luminosity end remains poorly calibrated due to insufficient reliable reverberation mapping (RM) data. We present a four-year RM campaign of the luminous quasar E1821+643 using the Lijiang 2.4-m telescope, supplemented by archival multi-wavelength data. E1821+643 is the most luminous AGN with an \hb\ RM measurement to date. The measured time lag of $83.2_{-18.7}^{+17.5}$ days is a factor of 5.6 shorter than predicted by the canonical $R_{\rm BLR}$-$L_{5100}$ relation. By compiling the full \hb\ RM sample, we find that such deviation defines a lower envelope ($0.2R_{\rm BLR}$) of measured lags across the entire luminosity range, while the upper envelope lies near $2R_{\rm BLR}$, implying that the scatter for individual AGNs can reach 1 dex. Spectral decomposition reveals two distinct \hb\ components: a core component with a lag of $267.0_{-17.6}^{+16.6}$ days closer to the $R_{\rm BLR}$-$L_{5100}$ relation, and a redshifted tail with a much shorter lag of $-49.0_{-34.5}^{+50.5}$ days. The short-lag component not only accounts for the significantly shortened overall lag, but also leads to an opposite interpretation of the intrinsic BLR kinematics. These effects can introduce systematic uncertainties in black hole mass estimates by factors of up to tens. Our findings demonstrate that shortened lags in high-accretion-rate AGNs arise from multi-component BLR structures, posing substantial challenges to single-epoch mass estimates and impacting SMBH demographics and cosmological applications.

</details>

### [20] [Galactic Large-scale Filaments Resident in Asymmetric Environments: Clues from Cross-filament Profiles of Density and Temperature](https://arxiv.org/abs/2601.18485)
*Keyun Su,Ke Wang,Fengwei Xu,N. K. Bhadari*

**TL;DR**: 通过对35条银河系大尺度丝状结构（large-scale filaments）的分子氢柱密度（N(H2)）和尘埃温度（Td）径向剖面的统计分析，发现其周围环境在N(H2)上呈现显著不对称且多变，而在Td上则呈现温和不对称但稳定，且与HII区（HII region）尺度相近的大丝状结构可能受其影响产生反向趋势的不对称剖面，但HII区不太可能独立触发整个大尺度丝状结构的形成。


<details>
  <summary>Details</summary>

Background: 银河系星际介质中大尺度丝状结构（large-scale filaments）的普遍存在及其径向剖面特征为研究其形成机制提供了重要线索。

Data: 数据来源于赫歇尔空间天文台（Herschel）的Hi-GAL巡天项目提供的分子氢柱密度（N(H₂)）和尘埃温度（T_d）二维分布图。

Method: 采用分子氢柱密度（N(H₂)）和尘埃温度（T_d）的径向剖面统计分析方法，基于Herschel Hi-GAL数据提取315个脊柱段的不对称度（α_asy）和不对称剖面长度比例（f_asy）。

Result: 银河大尺度丝状结构（large-scale filaments）的径向分布不对称性（αasy）和长度比例（fasy）分析表明，其周围环境在分子氢柱密度（N(H2)）上呈现显著不对称且多变，而在尘埃温度（Td）上表现为温和不对称且稳定。

Abstract: Large-scale filaments ubiquitously exist in the Galactic interstellar medium, and their radial profiles offer insights into their formation mechanisms. We present a statistical analysis of molecular hydrogen column density ($\rm N(H_2)$) and dust temperature ($\rm T_d$) radial profiles for 35 Galactic large-scale filaments. We divided their spines into 315 segments, extracted the radial profiles of each segment using $\rm N(H_2)$ and $\rm T_d$ maps derived from $Herschel$ Hi-GAL data, and estimated the asymmetry degree within the radial profiles ($α_{\rm asy}$), as well as the length proportion of segments with asymmetric profiles across the entire filament ($f_{\rm asy}$). We found that Galactic large-scale filaments reside in surroundings distinctly asymmetric and varied in $\rm N(H_2)$, and mild asymmetric yet stable in $\rm T_d$. Different filament morphology types do not show significant differences in $α_{\rm asy}$ or $f_{\rm asy}$. A bent filament shape does not necessarily correspond to an asymmetric radial profile, whereas a straight filament shape may be associated with a symmetric profile. Segments with asymmetric surroundings in $\rm N(H_2)$ may not simultaneously appear asymmetric in $\rm T_d$, and vice versa. We found three filaments with 4-44% of their spine show asymmetric $\rm N(H_2)$ and $\rm T_d$ radial profiles in inverse trends, likely caused by nearby HII region. HII regions of similar scale to large filaments can induce asymmetric radial profiles within them, indicating their influence on filament evolution. However, they are unlikely to independently trigger the formation of an entire Galactic large-scale filament, in contrast to their role in small-scale filament formation.

</details>

### [21] [Ancient relic moderately metal-rich bulge cluster Tonantzintla 2](https://arxiv.org/abs/2601.18567)
*Sergio Ortolani,Stefano O. Souza,Domenico Nardiello,Beatriz Barbuy,Eduardo Bica,Bernardo P. L. Ferreira,Cristina Chiappini,José Fernandez-Trincado,Heitor Ernandes*

**TL;DR**: 发现Tonantzintla 2（球状星团）是银河系原始核球（primordial bulge）的遗迹，其年龄（13.58 Gyr）和化学丰度特征表明核球形成始于宇宙大爆炸后约0.2 Gyr，为银河系早期化学增丰提供了关键约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系核球（bulge）的早期形成历史，特别是通过球状星团（globular clusters）的古老特征追溯原始构建块（primordial building blocks），以Tonantzintla 2为关键案例探讨内银河系（inner Milky Way）的初始化学增丰过程。

Data: 数据来源于哈勃空间望远镜（Hubble Space Telescope）的WFC3和ACS测光数据（proper motion-corrected）以及APOGEE高分辨率光谱仪（APOGEE high-resolution spectroscopy）对7颗成员星的化学丰度分析。

Method: 采用自行运动校正的哈勃太空望远镜WFC3和ACS测光数据，结合等龄线拟合（isochrone fitting）方法确定基本参数，并通过APOGEE高分辨率光谱对7颗成员星进行化学丰度分析。

Result: Tonantzintla 2是一个年龄为13.58 Gyr、金属丰度[M/H]=-0.68的球状星团，代表了银河系最早化学增丰阶段的遗迹，其原位起源特征为银河系核球形成起始时间提供了严格约束（约大爆炸后0.2 Gyr内）。

Abstract: The assembly history of the Galactic bulge is intimately tied to the formation of the proto-Milky Way, yet reconstructing this early phase is difficult because mergers and secular evolution have erased most of its original structure. Among present-day stellar systems, only globular clusters retain the ancient signatures needed to trace these primordial building blocks. Here we present the most detailed characterization to date of Tonantzintla 2, a prime candidate for a relic of the Milky Way's primordial bulge. It is a moderately metal-rich globular cluster projected onto the bulge that has remained largely unexplored despite its potential to constrain the early formation of the inner Milky Way. We derive its fundamental parameters using proper motion-corrected Hubble Space Telescope WFC3 and ACS photometry. By applying an isochrone fitting to very clean data, we obtain an age of 13.58 Gyr, a reddening E(B-V) = 1.44, a metallicity [M/H]=-0.68, and a heliocentric distance of d = 7.38 kpc. A complementary chemical-abundance analysis of seven member stars from APOGEE high-resolution spectroscopy reveals an enrichment pattern consistent with an in-situ origin. Tonantzintla 2 is among the oldest globular clusters studied in the literature, and the oldest so far analyzed in the Galactic bulge. Its age places a stringent constraint on the onset of the bulge formation, implying that star formation in the inner Galaxy began within ~0.2 Gyr of the Big Bang and that Tonantzintla 2 represents an exceptional relic of the Milky Way's earliest chemical enrichment.

</details>

### [22] [The ALMA-ATOMS survey: Methanol emission in a large sample of hot molecular cores](https://arxiv.org/abs/2601.18628)
*Jiahang Zou,Tie Liu,Sheng-Li Qin,Yaping Peng,Fengwei Xu,Xunchuan Liu,Li Chen,Xindi Tang,Sami Dib,Zi-Yang Li,Hong-Li Liu,Mika Juvela,Patricio Sanhueza,Pablo Garcia,Chang Won Lee,Guido Garay,Swagat R. Das,Yan-Kun Zhang,Kee-Tae Kim,Jeong-Eun Lee,Meizhu Liu,Leonardo Bronfman,Zihping Kou,Dongting Yang,Gang Wu,Jihye Hwang,Dezhao Meng,Mengyao Tang,James O. Chibueze*

**TL;DR**: 通过高分辨率ALMA观测发现甲醇(CH3OH)核自旋异构体(特别是E型v_t=1态)的激发特性和丰度与化学复杂度密切相关，其谱线强度与通道探测率(CDR)强相关，证实了E型v_t=1甲醇谱线可作为热分子核(HMCs)和化学复杂度的可靠示踪剂。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际介质中甲醇（CH$_{3}$OH）作为示踪剂在致密气体和热分子核（HMCs）中的激发态、丰度及其与化学复杂性的关联。

Data: 数据来源于ATOMS巡天的高分辨率ALMA观测数据。

Method: 采用高分辨率ALMA观测数据，在局域热动平衡(LTE)假设下通过转动温度分析研究甲醇核自旋异构体的激发态与丰度关系。

Result: 在94个热分子核候选体中，CH$_{3}$OH-E v$_{t}$=1谱线强度与通道探测率(CDR)呈强相关，表明该谱线是热分子核和化学复杂性的可靠示踪剂。

Abstract: Methanol (CH$_{3}$OH) is a key complex organic molecule (COM) in the interstellar medium, widely used as a tracer of dense gas and hot molecular cores (HMCs). Using high-resolution ALMA observations from the ATOMS survey, we investigate the excitation and abundance of methanol nuclear spin isomers and their relationship to chemical complexity in massive star-forming cores. We identify 20 methanol transitions, including A- and E-type lines in the v=0 state and E-type lines in the v$_{t}$=1 state, and detect 94 HMC candidates. Rotational temperature analysis under the LTE assumption yields average values of 194 $\pm$ 33 K for CH$_{3}$OH-E v$_{t}$=1, 178 $\pm$ 33 K for CH$_{3}$OH-A v=0, and 75 $\pm$ 33K for CH$_{3}$OH-E v=0. Emission from COMs other than methanol is detected in 87 of the 94 cores, with the CH$_{3}$OH-E v$_{t}$=1 line intensity showing a strong correlation with the channel detection ratio (CDR). These results demonstrate that CH$_{3}$OH-E v$_{t}$=1 lines are reliable tracers of HMCs and chemical complexity, and that the CDR provides a robust indicator of molecular richness. The temperature difference between A- and E-type methanol transitions is driven by anomalously strong J(2,J-2)$-$J(-1,J-1) lines, highlighting the importance of analyzing methanol symmetry types separately.

</details>

### [23] [New parameters for star cluster dynamics: observational results](https://arxiv.org/abs/2601.18679)
*Barbara Lanzoni,Francesco R. Ferraro,Enrico Vesperini*

**TL;DR**: 该论文通过哈勃太空望远镜(HST)对40个银河系球状星团(GCs)的观测，验证了A5、P5等新定义的经验参数能有效追踪恒星系统内部动力学演化，发现这些参数值与后核坍缩(post-core collapse)系统、短中心弛豫时间及蓝离散星(blue straggler stars)中心聚集程度显著相关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于球状星团（GCs）内部动力学演化，通过定义新的经验参数（A5、P5和S2.5）量化恒星径向累积分布在最内区随动力学老化（dynamical aging）因核心收缩而逐渐变陡的现象。

Data: 数据来源于哈勃太空望远镜（HST）对40个银河系球状星团（GCs）的光度观测。

Method: 采用蒙特卡洛模拟（Monte Carlo simulations）定义并验证了三个经验参数（A5、P5、S2.5），通过测量累积径向分布的陡峭程度量化球状星团（GCs）内部动力学演化状态。

Result: 提出了三个新的经验参数（A5、P5和S2.5）用于量化球状星团（GCs）内部动力学演化程度，这些参数与核心收缩导致的恒星径向分布陡化程度相关。

Abstract: We recently used a large set of Monte Carlo simulations of globular clusters (GCs) to define new fully empirical parameters (named A5, P5, and S2.5) able to trace the internal dynamical evolution of dense stellar systems. These parameters are specifically designed to quantify the steepness of the cumulative radial distribution of stars in the innermost region of the host system, which tends to progressively increase with dynamical aging due to core contraction. Following the original definitions, here we measure A5 and P5 in a sample of 40 Galactic GCs homogeneously surveyed through HST photometric observations. In agreement with the predictions of our simulations, the largest values of A5 and P5 are found for the most dynamically evolved GCs, i.e., those previously classified as post-core collapse systems based on the shape of their density profile, and those characterized by the shortest central relaxation times. Moreover, the new dynamical parameters here measured strongly correlate with A+rh, another fully empirical, independent parameter that traces the dynamical age of star clusters through the level of central segregation of blue straggler stars.

</details>

### [24] [Virgo Filaments VI: H$α$ clumps in the filaments around the Virgo galaxy cluster](https://arxiv.org/abs/2601.18762)
*G. Nagaraj,P. Jablonka,R. A. Finn,Y. M. Bahé,F. Combes,G. Castignani,B. Vulcani,G. Rudnick,D. Zakharova,R. A. Koopmann,D. Zaritsky,K. Conger*

**TL;DR**: 该研究通过分析685个室女座(Virgo)星系团内外星系的Hα图像，发现星系中团块(clump)的数量和平均尺寸与距离及角分辨率呈明确函数关系，且团块数量与距离的幂律关系符合恒星形成区的分形结构，同时提出消除观测差异后比较丝状结构(filament)与非丝状结构星系的方法，发现丝状结构星系团块分布略偏向外围。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙丝状结构（filaments）中环境过程对星系演化（galaxy evolution）的影响机制，旨在揭示丝状结构在星系团吸积过程中的作用。

Data: 研究中使用的数据来源为685个星系在室女座(Virgo)团周围丝状结构内外的高分辨率Hα成像数据以及综合物理性质测量结果。

Method: 创建了一个分解H$α$图像为独立团块（clumps）的流程，并开发了算法以消除观测差异后比较丝状结构（filament）与非丝状结构星系。

Result: result：星系丝状结构（filament）中的恒星形成团块（clump）数量与距离呈幂律关系，符合恒星形成区的分形结构特征，且丝状结构星系比非丝状结构星系具有更多外围团块。

Abstract: It is still not clear which environmental processes operate in filaments. Given the ubiquity of filaments and their importance in feeding clusters, a proper understanding of these mechanisms is crucial to a more complete picture of galaxy evolution. To investigate them, we need large galaxy samples with resolved imaging. For this study, we analyse resolved H$α$ maps of 685 galaxies inside and outside the filaments around the Virgo cluster in addition to extensive measurements of integrated physical properties. We create a pipeline to decompose the H$α$ images into individual clumps. We find that the number and average size of clumps in a galaxy are well-defined functions of distance and angular resolution. In particular, the power-law relation between the number of clumps and the distance of a galaxy is consistent with a fractal structure of star forming regions. We formulate an algorithm to compare filament and non-filament galaxies after removing observational differences. Although we do not have any conclusive evidence for a difference in clump size distributions between filament and non-filament galaxies, we do find that filament galaxies have slightly more peripheral clumps than their non-filament counterparts.

</details>

<div id='astro-ph.CO'></div>

# astro-ph.CO [[Back]](#toc)

### [25] [Non-Equilibrium Relativistic Core Collapse of Self-Interacting Dark Matter Halos -- Limits On Seed Black Hole Mass](https://arxiv.org/abs/2601.17117)
*Hua-Peng Gu,Fangzhou Jiang,Xian Chen,Ran Li*

**TL;DR**: 首次将Misner-Sharp形式引入自相互作用暗物质(SIDM)晕研究，发现晚期短平均自由程(SMFP)阶段外层快速膨胀显著延缓坍缩，并计算出视界形成时种子黑洞质量约为晕质量的3×10^-8倍，表明需要额外机制解释早期超大质量黑洞(SMBHs)的形成。


<details>
  <summary>Details</summary>

Background: 研究探讨了自相互作用暗物质（SIDM）晕的引力热坍缩机制如何解释高红移超大质量黑洞（SMBHs）的形成难题。

Data: 数据来源于对自相互作用暗物质晕（SIDM halos）的数值模拟（simulation），首次在SIDM背景下引入了Misner-Sharp形式体系并考虑了广义相对论效应。

Method: 采用Misner-Sharp形式主义（Misner-Sharp formalism）结合广义相对论效应，并放宽流体静力学平衡假设，对自相互作用暗物质晕（SIDM halos）的引力热坍缩进行建模。

Result: 自相互作用暗物质(SIDM)晕的引力热塌缩模型通过放宽流体静力学平衡假设并引入广义相对论效应，揭示了晚期短平均自由程(SMFP)阶段外包层快速膨胀显著减缓核心塌缩的现象。

Abstract: Recent observations of supermassive black holes (SMBHs) at high redshifts pose challenges to standard seeding mechanisms. Among competing models, the collapse of self-interacting dark matter (SIDM) halos provide a plausible explanation for early SMBH formation. While previous studies on modeling the gravothermal collapse of SIDM halos have primarily focused on non-relativistic evolution under the assumption of hydrostatic equilibrium, We advance this framework by relaxing the equilibrium assumption and additionally incorporating general-relativistic effects. To this end, we introduce the Misner-Sharp formalism to the SIDM context for the first time. Our model reproduces the standard hydrostatic models in the early long-mean-free-path (LMFP) regime, but displays interesting distinct behavior in the late short-mean-free-path (SMFP) regime, where intense outward heat flux drives a rapid expansion of the outer envelope, removing mass from the core and significantly decelerating the collapse. Our general relativistic treatment enables us to follow halo evolution to the final stage when the apparent horizon forms. Our simulation yields a seed black hole mass of approximately $3\times10^{-8}$ of the halo mass at horizon formation, suggesting that additional mechanisms such as baryonic effects are critical for seeding black holes that are sufficiently massive to account for SMBHs in the early Universe.

</details>

### [26] [An ultra-high-resolution map of (dark) matter](https://arxiv.org/abs/2601.17239)
*Diana Scognamiglio,Gavin Leroy,David Harvey,Richard Massey,Jason Rhodes,Hollis B. Akins,Malte Brinch,Edward Berman,Caitlin M. Casey,Nicole E. Drakos,Andreas L. Faisst,Maximilien Franco,Leo W. H. Fung,Ghassem Gozaliasl,Qiuhan He,Hossein Hatamnia,Eric Huff,Natalie B. Hogg,Olivier Ilbert,Jeyhan S. Kartaltepe,Anton M. Koekemoer,Shouwen Jin,Erini Lambrides,Alexie Leauthaud,Zane D. Lentz,Daizhong Liu,Guillaume Mahler,Claudia Maraston,Crystal L. Martin,Jacqueline McCleary,James Nightingale,Bahram Mobasher,Louise Paquereau,Sandrine Pires,Brant E. Robertson,David B. Sanders,Claudia Scarlata,Marko Shuntov,Greta Toni,Maximilian von Wietersheim-Kramsta,John R. Weaver*

**TL;DR**: 利用詹姆斯·韦伯太空望远镜(JWST)的高分辨率弱引力透镜(weak gravitational lensing)观测数据，首次绘制了覆盖0.77×0.70平方度、角分辨率达1.00±0.01角分的暗物质(dark matter)分布图，揭示了暗物质与发光物质在宇宙网(filaments)、星系团(clusters)及低密度区(under-densities)中的共演化关系，并探测到红移(z)~2的质量结构。


<details>
  <summary>Details</summary>

Background: 宇宙中普通物质（ordinary matter）仅占全部物质的六分之一，其余为通过引力影响星系和结构演化的暗物质（dark matter）。

Data: 数据来源于詹姆斯·韦伯太空望远镜（JWST）在COSMOS-Web巡天中获取的高分辨率成像数据，覆盖0.77度×0.70度天区，每平方角分钟测量129个星系形状（部分星系在F115W和F150W波段独立观测）。

Method: 采用弱引力透镜(weak gravitational lensing)技术，结合詹姆斯·韦伯太空望远镜(JWST)的F115W和F150W波段高分辨率成像数据，以每平方角分129个星系的形状测量为基础，实现1.00±0.01角分的角分辨率。

Result: 利用詹姆斯·韦伯太空望远镜(JWST)的COSMOS-Web巡天数据，构建了迄今分辨率最高(1.00±0.01角分)的弱引力透镜(weak gravitational lensing)质量分布图，揭示了暗物质(dark matter)与发光物质在宇宙细丝、星系团及低密度区域的共演化特征，并探测到红移(redshift)z~2范围内的质量结构。

Abstract: Ordinary matter-including particles such as protons and neutrons-accounts for only about one sixth of all matter in the Universe. The rest is dark matter, which does not emit or absorb light but plays a fundamental role in galaxy and structure evolution. Because it interacts only through gravity, one of the most direct probes is weak gravitational lensing: the deflection of light from distant galaxies by intervening mass. Here we present an extremely detailed, wide-area weak-lensing mass map, covering 0.77 deg x 0.70 deg, using high-resolution imaging from the James Webb Space Telescope (JWST) as part of the COSMOS-Web survey. By measuring the shapes of 129 galaxies per square arcminute-many independently in the F115W and F150W bands-we achieve an angular resolution of 1.00 +/- 0.01 arcmin. Our map has more than twice the resolution of earlier Hubble Space Telescope maps, revealing how dark and luminous matter co-evolve across filaments, clusters, and under-densities. It traces mass features out to z ~ 2, including the most distant structure at z ~ 1.1. The sensitivity to high-redshift lensing constrains galaxy environments at the peak of cosmic star formation and sets a high-resolution benchmark for testing theories about the nature of dark matter and the formation of large-scale cosmic structure

</details>

### [27] [Constraining Quintessence Models with ISW-tSZ Cross-Correlations: A Comparative Analysis of Thawing, Tracker, and Scaling-Freezing Dynamics](https://arxiv.org/abs/2601.17298)
*Ayodeji Ibitoye,Shiriny Akthar,Md. Wali Hossain,Amare Abebe,Prabhakar Tiwari,Xuelei Chen,Jackson Said,Jacob Oloketuyi*

**TL;DR**: 通过ISW-tSZ（集成萨克斯-沃尔夫效应-热苏尼亚耶夫-泽尔多维奇效应）互相关数据集分析，发现解冻型(Thawing)精质(quintessence)暗能量模型在χ²检验中表现最优，其指数势斜率λ=0.736^{+0.270}_{-0.227}，且所有模型与ΛCDM的偏差均在1σ内，凸显了该互相关方法对暗能量动力学研究的有效性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用ISW-tSZ交叉关联数据集约束五重态暗能量（quintessence dark energy）模型，探讨其与标准ΛCDM宇宙学模型的差异。

Data: 使用综合萨克斯-沃尔夫效应(ISW)与热苏尼亚耶夫-泽尔多维奇效应(tSZ)的观测交叉相关数据集。

Method: 采用综合似然分析（comprehensive likelihood analysis）方法比较三类精质暗能量（quintessence dark energy）动力学模型（解冻型、追踪型和标度冻结型）与标准ΛCDM宇宙学，并推导关键宇宙学参数的最佳拟合值和68%置信区间。

Result: 通过综合似然分析得出关键宇宙学参数的最佳拟合值和68%置信区间，其中ΛCDM模型的Ωm = 0.322+0.027−0.030和σ8 = 0.735+0.045−0.035，其他模型的偏差在1σ范围内一致。

Abstract: We present constraints on quintessence dark energy models using the observational detection of the Integrated Sachs-Wolfe (ISW)--thermal Sunyaev-Zeldovich (tSZ) cross-correlation dataset. Our analysis compares three classes of quintessence dynamics: thawing, tracker, and scaling-freezing with the standard $Λ$CDM cosmology. Through a comprehensive likelihood analysis, we derive best-fit values and 68\% confidence intervals for key cosmological parameters, finding $Ω_{\rm m} = 0.322^{+0.027}_{-0.030}$ and $σ_8 = 0.735^{+0.045}_{-0.035}$ for $Λ$CDM, with deviations in alternative models consistent within $1σ$. For the thawing model, we consider an exponential potential with slope $λ= 0.736^{+0.270}_{-0.227}$, while for the tracker and scaling-freezing models, we use inverse axion-like and double exponential potentials, respectively. Observationally, the tracker model yields $n = 5.651^{+1.625}_{-1.604}$ and $f = 0.258^{+0.149}_{-0.096}$, and the scaling-freezing model gives $λ_1 = 0.405^{+0.293}_{-0.322}$ and $λ_2 = 23.226^{+7.975}_{-7.258}$. The dimensionless tSZ amplitude ($\widetilde{W}^{\rm SZ}$) and cosmic infrared background (CIB) parameters are tightly constrained across all models, providing additional insights into astrophysical foregrounds. Our results demonstrate the effectiveness of ISW--tSZ cross-correlations as a probe of dark energy dynamics, with the Thawing quintessence model yielding the lowest $χ^2_{\rm min}$ among the tested scenarios, and highlight the need for future high-precision measurements to distinguish between quintessence models and $Λ$CDM.

</details>

### [28] [Stellar Disruption of Axion Minihalos and Consequences for Direct Axion Detection](https://arxiv.org/abs/2601.17478)
*Ian DSouza*

**TL;DR**: 研究发现考虑多次恒星相遇后能量注入累积效应时，微晕（minihalo）在太阳系位置的质量保留率从先前估计的60%降至30%，改变了轴子（axion）暗物质分布并可能提升局域探测概率。


<details>
  <summary>Details</summary>

Background: 研究探讨了QCD轴子（QCD axion）和暴胀后破缺的Peccei-Quinn对称性预测的亚秒差距尺度上增强的物质功率谱，及其形成的致密暗物质结构微晕（minihalos）对早期宇宙动力学和直接探测实验的影响。

Data: 研究中使用的数据来源包括N-body模拟（N-body simulation）结果和银河系环境下的迷你晕轨道模拟数据。

Method: 提出了考虑微晕（minihalo）结构在恒星穿越扰动后变化的新质量损失公式，并研究了多次恒星穿越对质量损失的影响。

Result: 提出了一种考虑微晕（minihalo）结构在恒星穿越后变化的新质量损失公式，并证明多次恒星相遇时需考虑每次相遇后结合能变化以避免显著低估质量损失。

Abstract: Scenarios such as the QCD axion with the Peccei-Quinn symmetry broken after inflation predict an enhanced matter power spectrum on sub-parsec scales. These theories lead to the formation of dense dark matter structures known as minihalos, which provide insights into early Universe dynamics and have implications for direct detection experiments. We examine the mass loss of minihalos during stellar encounters, building on previous studies that derived formulas for mass loss and performed N-body simulations. We propose a new formula for the mass loss that accounts for changes in the minihalo profile after disruption by a passing star. We also investigate the mass loss for multiple stellar encounters. We demonstrate that accurately assessing the mass loss in minihalos due to multiple stellar encounters necessitates considering the alterations in the minihalo's binding energy after each encounter, as overlooking this aspect results in a substantial underestimation of the mass loss.
  We further extend our analysis to the Galactic environment by more accurately incorporating multiple stellar encounters and dynamical relaxation timescales, simulating minihalo orbits in the Galactic potential. Our results show stellar interactions are more destructive than previously estimated, reducing minihalo mass retention at the solar system to ~30%, compared to earlier estimates of ~60%. This enhanced loss arises from cumulative energy injections when relaxation periods between stellar encounters are accounted for. The altered minihalo mass function implies a larger fraction of axion dark matter occupies inter-minihalo space, potentially increasing the local axion density and improving haloscope detection prospects. This thesis highlights the significance of detailed modeling of stellar disruptions in shaping the axion dark matter distribution.

</details>

### [29] [Forecasting the E_G measurements from the photometric and spectroscopic surveys of Chinese Space Station Survey Telescope (CSST)](https://arxiv.org/abs/2601.17874)
*Yu Song,Yi Zheng*

**TL;DR**: 基于中国空间站巡天望远镜(CSST)模拟星系样本，研究发现其能在红移0 < z < 1.2范围内将引力增长因子($E_G$)测量精度提升至3%-9%，并在μ-Σ修正引力框架下将韦尔势有效引力常数相关参数$Σ_0$约束至~5%精度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用中国空间站巡天望远镜(CSST)的模拟星系样本红移分布数据，通过$E_G$统计量检验引力理论并约束修正引力框架下的关键参数。

Data: 数据来源为即将投入使用的中国空间站巡天望远镜（CSST）提供的模拟星系样本（mock galaxy samples）红移分布数据。

Method: 采用红移空间畸变参数β和μ-Σ修正引力框架分析E_G统计量，结合中国空间站巡天望远镜(CSST)模拟星系样本的红移分布。

Result: CSST将实现红移范围0 < z < 1.2内$E_G$统计量3%-9%的精度约束，较现有观测提升数倍至一个量级。

Abstract: We present forecasts for the $E_G$ statistic using redshift distributions of realistic mock galaxy samples from the upcoming Chinese Space Station Survey Telescope (CSST). The dominant uncertainty in $E_G$ stems from the redshift space distortion parameter $β$, whose precision limits the overall constraining power. Our analysis shows that CSST will nevertheless achieve $E_G$ constraints at the few-percent level (3%-9%) over $0 < z < 1.2$, an improvement by a factor of several to an order of magnitude over current observations. Within the $μ-Σ$ modified gravity framework, the parameter $Σ_0$, associated with the effective gravitational constant of the Weyl potential, can be constrained to $\sim 5\%$ precision. In a plausible scenario where upcoming spectroscopic surveys determine $β$ to 1\% accuracy, $E_G$ constraints tighten to the percent level, and $Σ_0$ becomes measurable at $\sim 1\%$. These results demonstrate that CSST will serve as a powerful facility for testing gravity and underscore the essential synergy between photometric weak lensing and spectroscopic surveys in probing cosmic acceleration.

</details>

### [30] [Three-point intrinsic alignments of galaxies and haloes in the FLAMINGO simulations](https://arxiv.org/abs/2601.17914)
*Casper Vedder,Thomas Bakx,Nora Elisa Chisari,Henk Hoekstra,Matthieu Schaller*

**TL;DR**: 该论文通过FLAMINGO流体动力学模拟，首次测量并建模了星系和暗晕（halo）的三点相关函数及三阶孔径质量统计量（third-order aperture-mass statistics），发现大尺度上两者均能用树阶有效场论（EFT）准确描述，且与两点统计推断的排列（alignment）幅度一致，同时验证了简化EFT模型在减少自由参数时的优异性能。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用三阶统计量（third-order statistics）深入探究星系和暗晕（halo）的本征排列（intrinsic alignments, IA）特性，旨在超越传统两点统计的局限并验证有效场论（effective field theory, EFT）在大尺度上的适用性。

Data: 数据来源于FLAMINGO流体动力学模拟套件中$(2.8\,\mathrm{Gpc})^3$模拟体积内晕质量$M_{\rm halo} > 10^{13}\,{\rm M}_\odot$的星系和暗晕样本。

Method: 采用树阶有效场论（EFT）对星系和暗晕的三阶孔径质量统计量进行建模，并结合速度-剪切项忽略版本、非线性排列模型（NLA）以及基于拉格朗日空间线性排列假设的简化EFT进行对比分析。

Result: 在FLAMINGO流体动力学模拟中，星系和暗晕（halo）的三阶孔径质量统计（third-order aperture-mass statistics）在大尺度上可通过树阶有效场论（EFT）准确描述，且其排列幅度（alignment amplitude）与两点统计结果一致。

Abstract: Third-order statistics provide information beyond two-point measures, but extracting this information requires accurate and consistent modelling. We measure and detect the three-point correlation function and third-order aperture-mass statistics of intrinsic alignments (IA) for galaxies and for haloes with $M_{\rm halo} > 10^{13}\,{\rm M}_\odot$ in the $(2.8\,\mathrm{Gpc})^3$ simulation volume of the FLAMINGO hydrodynamical simulation suite. We model the third-order aperture-mass statistics and show that on large scales both the galaxy and halo samples are well described by the tree-level effective field theory (EFT) of IA across the three dark matter density-shape combinations and a wide range of triangle configurations, with the alignment amplitude consistent with that inferred from two-point statistics. We compare the full EFT to several other models: a version neglecting the velocity-shear term, the non-linear alignment model (NLA), and to a reduced EFT assuming co-evolution relations that follow from the assumption that alignment is linear in Lagrangian space. The first two models yield biased constraints on the alignment amplitude, but the reduced EFT performs remarkably well, achieving a low reduced chi-squared and minimal bias. We examine the redshift and mass dependence of the higher-order bias parameters, finding that the linear Lagrangian bias assumption is approximately satisfied across the explored halo mass and redshift ranges for both galaxies and haloes, suggesting that the galaxies broadly follow the alignment properties of their host haloes. These co-evolution relations can be valuable for photometric shear surveys, where limited constraining power on IA parameters favours models with fewer free parameters.

</details>

### [31] [Hubble Tension as an Effect of Horizon Entanglement Nonequilibrium](https://arxiv.org/abs/2601.17938)
*Alexander S. Sakharov,Rostislav Konoplich,Merab Gogberashvili,Jack Simoni*

**TL;DR**: 提出了一种基于宇宙学表观视界（cosmological apparent horizon）偏离纠缠平衡的红外机制，通过参数化纠缠熵相对于贝肯斯坦-霍金值（Bekenstein-Hawking value）的微小缺失δ(a)，发现其产生的能量密度正比于H²/G的平滑组分可在红移z≲1时激活，使晚期膨胀率提升几个百分点而不影响重组或声视界，为哈勃常数（Hubble constant）张力提供了一种晚期视界信息缺失的解释框架。


<details>
  <summary>Details</summary>

Background: 该研究探讨了通过红外机制（infrared mechanism）缓解哈勃常数（Hubble constant）张力的可能性，其科学主题聚焦于宇宙学表观视界（cosmological apparent horizon）的纠缠熵（entanglement entropy）偏离平衡态对晚期宇宙膨胀率的影响。

Data: 研究使用了包括SN Ia距离模数、重子声学振荡距离测量、宇宙计时器H(z)数据以及红移空间畸变约束在内的当前低红移数据集。

Method: 基于弗里德曼-勒梅特-罗伯逊-沃克(FLRW)尺度因子演化的纠缠熵赤字参数δ(a)和吉本斯-霍金(Gibbons-Hawking)温度下的能量均分赤字，构建了最小三参数激活模型ce²(a)来量化纠缠熵赤字对宇宙背景膨胀的影响。

Result: 提出了一种基于宇宙学表观视界（cosmological apparent horizon）纠缠平衡微小偏离的红外机制，通过参数化纠缠熵相对于贝肯斯坦-霍金值（Bekenstein-Hawking value）的分数亏缺δ(a)，其产生的均分亏缺在吉本斯-霍金温度（Gibbons-Hawking temperature）下会激发一个密度正比于H²/G且系数ce²(a)≈δ(a)的平滑组分，该组分在z≲1时激活并提升晚期膨胀率约几个百分点。

Abstract: We propose an infrared mechanism for alleviating the Hubble constant tension, based on a small departure from entanglement equilibrium at the cosmological apparent horizon. If the horizon entanglement entropy falls slightly below the Bekenstein-Hawking value, we parametrize the shortfall by a fractional deficit $δ(a)$ evolving with the FLRW scale factor $a$. The associated equipartition deficit at the Gibbons-Hawking temperature then sources a smooth, homogeneous component whose density scales as $H^{2}/G$, with a dimensionless coefficient $c_{e}^{2}(a)$ of order unity times $δ(a)$. Because this component tracks $H^{2}$, it is negligible at early times but can activate at redshifts $z\lesssim 1$, raising the late time expansion rate by a few percent without affecting recombination or the sound horizon. We present a minimal three parameter activation model for $c_{e}^{2}(a)$ and derive its impact on the background expansion, effective equation of state, and linear growth for a smooth entanglement sector. The framework predicts a small boost in $H(z)$, a mild suppression of $fσ_{8}(z)$, and a corresponding modification of the low-$z$ distance-redshift relation. We test these predictions against current low-redshift data sets, including SN~Ia distance moduli, baryon acoustic oscillation distance measurements, cosmic chronometer $H(z)$ data, and redshift space distortion constraints, and discuss whether the $H_0$ tension can be consistently interpreted as a late-time, horizon-scale information deficit rather than an early universe modification.

</details>

### [32] [First Observational Evidence for Split Infall Flow of Cosmic Filaments into Clusters](https://arxiv.org/abs/2601.18434)
*Ji Yao,Huanyuan Shan,Pengjie Zhang,Xiaohu Yang,Jiale Zhou,Jiaxin Han,Peng Wang*

**TL;DR**: 研究发现星系团间连接纤维（galaxy filaments）存在分裂式内流（split infall）速度场，其速度剖面在中点附近反转且最大内流振幅为$\sim30$ km/s，表明纤维结构动态响应竞争引力势而非被动传输物质。


<details>
  <summary>Details</summary>

Background: 宇宙大尺度结构（cosmic web）中速度场对结构形成至关重要但非线性区观测仍具挑战性。

Data: 数据来源于斯隆数字巡天(Sloan Digital Sky Survey)的光谱星系观测数据。

Method: 基于斯隆数字巡天(SDSS)光谱数据，采用背景刚性运动与哈勃流(Hubble flow)扣除方法分离出星系丝状结构内部速度场。

Result: 检测到星系丝状结构（filament）内部存在分裂式下落（split infall）速度场，其速度剖面在中点附近发生符号反转，最大下落幅度约为30 km/s（视线投影约20 km/s），显著低于平均宇宙环境预期值。

Abstract: Velocity fields in the cosmic web are fundamental to structure formation but remain difficult to observe directly beyond the linear regime. Here we present observational evidence that galaxy filaments connecting pairs of galaxy clusters undergo a split infall, with opposite velocity flows toward the two clusters. Using spectroscopic galaxies from the Sloan Digital Sky Survey, we isolate the internal filament velocity field by subtracting its rigid-body background motion and Hubble flow, and detect this effect at greater than $5σ$ significance across a wide range of cluster and filament selections. The measured velocity profile exhibits a sign reversal near the filament midpoint and a maximum infall amplitude of $\sim30$ km/s ($\sim20$ km/s projected onto the line-of-sight) for clusters of mass $\sim10^{14.3}M_\odot$, substantially lower than expected for infall from an average cosmic environment. Multiple results on density-velocity correlation, mass-dependency, and validation with simulation indicate that filaments dynamically respond to competing gravitational potentials rather than acting as passive mass transport channels. Our results establish a new observational window on quasi-linear velocity fields in the cosmic web and provide a promising probe of mass measurement, testing gravity and velocity reconstruction with upcoming wide-field spectroscopic surveys.

</details>

### [33] [Constraining Reionization Morphology and Source Properties with 21cm-Galaxy Cross-Correlation Surveys](https://arxiv.org/abs/2601.18627)
*Yannic Pietschke,Anne Hutter,Caroline Heneka*

**TL;DR**: 通过21厘米（21cm）与星系巡天的互相关分析，量化了互功率谱对中性氢分数（x_HI(z)）和平均密度（⟨1+δ_HI⟩(z)）的约束能力，发现其相比自功率谱能减小20-30%的后验体积（PV），并首次突破性地约束了电离源逃逸分数（f_esc）和恒星形成效率（f_*）等原本在自功率谱中高度简并的参数。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用21厘米（21cm）观测与星系巡天的互相关（cross-correlation）来探测再电离（reionization）时期的电离形态与星系关联，并降低前景干扰的敏感性。

Data: 数据来源包括100小时SKA-Low（Square Kilometre Array-Low）观测的热噪声模拟数据（采用前景避免策略）以及考虑了实际效应的星系巡天模拟数据。

Method: 采用基于模拟的推断框架EoRFlow进行无似然参数估计，结合21cm-星系互功率谱分析中性氢分数x_HI(z)和平均过密度⟨1+δ_HI⟩(z)的约束能力。

Result: 21cm-星系互功率谱（cross-power spectra）能够无偏地约束中性氢分数$x_\mathrm{HI}(z)$和平均密度涨落$\langle 1+δ_\mathrm{HI} \rangle(z)$，并将后验体积（PV）缩小至先验的10%。

Abstract: Cross-correlations between 21cm observations and galaxy surveys provide a powerful probe of reionization by reducing foreground sensitivity while linking ionization morphology to galaxies. We quantify the constraining power of 21cm-Galaxy cross-power spectra for inferring neutral hydrogen fraction $x_\mathrm{HI}(z)$ and mean overdensity $\langle 1+δ_\mathrm{HI} \rangle(z)$, exploring dependence on field of view, redshift precision $σ_z$, and minimum halo mass $M_\mathrm{h,min}$. We employ our simulation-based inference framework EoRFlow for likelihood-free parameter estimation. Mock observations include thermal noise for 100h SKA-Low with foreground avoidance and realistic galaxy survey effects. For a fiducial survey ($\mathrm{FOV}=100\,\mathrm{deg}^2$, $σ_z=0.001$, $M_\mathrm{h,min}=10^{11}\mathrm{M}_\odot$), cross-power spectra yield unbiased constraints with posterior volumes (PV) of $\sim$10% relative to priors. Cross-power measurements reduce PV by 20-30% versus 21cm auto-power alone. With foreground avoidance, spectroscopic redshift precision is essential; photometric redshifts render cross-correlations uninformative. Notably, cross-power spectra constrain ionizing source properties, the escape fraction $f_\mathrm{esc}$ and star formation efficiency $f_*$, which remain degenerate in auto-power (PV $>$60%). Tight constraints require either deep surveys detecting faint galaxies ($M_\mathrm{h,min} \sim 10^{10}\mathrm{M}_\odot$) with moderate foregrounds, or conservative mass limits with optimistic foreground removal (PV $<$15%). 21cm-Galaxy cross-correlations enhance morphology constraints beyond auto-power while enabling previously inaccessible source property constraints. Realizing full potential requires precise redshifts and either faint galaxy detection limits or improved 21cm foreground cleaning.

</details>

### [34] [Impact of Rastall gravity on hydrostatic mass of galaxy clusters](https://arxiv.org/abs/2601.18652)
*M. Lawrence Pattersons,Feri Apryandi,Freddy P. Zen*

**TL;DR**: 在拉斯托引力(Rastall gravity)框架下，无暗物质时星系团流体静力学质量与观测重子质量呈1:1对应关系，存在暗物质时则显著缓解流体静力学质量偏差(hydrostatic mass bias)，表明该理论能有效解释星系团质量差异问题。


<details>
  <summary>Details</summary>

Background: 该研究探讨了在Rastall引力框架下解决星系团中流体静力学质量偏差（hydrostatic mass bias）问题的可能性，并分析了暗物质存在与否两种情形对该理论解释力的影响。

Data: 研究中使用的数据来源于对星系团（galaxy clusters）的观测数据，包括流体静力学质量（hydrostatic mass）和引力透镜效应（gravitational lensing）测量得到的质量。

Method: 采用拉斯托引力（Rastall gravity）框架推导星系团的流体静力学质量，并分析其在无暗物质和存在暗物质两种情形下的影响。

Result: 在第一种情景（无暗物质）中，Rastall引力显著降低了流体静力学质量（hydrostatic mass），使其与观测到的重子质量（baryonic mass）接近一致，最佳线性拟合斜率M=1.07±0.11表明两者近乎一一对应。

Abstract: Galaxy clusters are the largest virialized structures in the Universe and are predominantly dominated by dark matter. The hydrostatic mass and the mass obtained from gravitational lensing measurements generally differ, a discrepancy known as the hydrostatic mass bias. In this work, we derive the hydrostatic mass of galaxy clusters within the framework of Rastall gravity and investigate its implications under two scenarios: (i) the absence of dark matter and (ii) the existence of dark matter. In the first scenario, Rastall gravity effectively reduces the hydrostatic mass, bringing it closer to the observed baryonic mass. The best linear fit yields a slope $\mathbf{M}=1.07\pm0.11$, indicating a near one-to-one correspondence between the two masses. In the second scenario, Rastall gravity helps to alleviate the hydrostatic mass bias. The linear fit between the Rastall hydrostatic mass and the observed lensing mass results in a best-fit slope $\mathbf{M}=1.01\pm0.16$, which is very close to unity. These results suggest that Rastall gravity provides a statistically favorable framework for addressing mass discrepancies in galaxy clusters.

</details>

### [35] [Ultra-fast growth of primordial black holes through radiative absorption](https://arxiv.org/abs/2601.18708)
*Dimitris S. Kallifatides,Theodoros Papanikolaou,Emmanuel N. Saridakis*

**TL;DR**: 研究发现辐射主导时期形成的史瓦西原初黑洞（PBHs）通过遵循斯特藩-玻尔兹曼定律的辐射吸收机制可极速增长质量，其临界质量标度律（critical collapse mass-scaling law）与相对论性粒子种类数共同导致连续质量谱，为恒星质量、中等质量及超大质量黑洞（SMBHs）的早期形成提供了统一标准模型解释。


<details>
  <summary>Details</summary>

Background: 该研究探讨了辐射主导时期形成的史瓦西原初黑洞（Schwarzschild PBHs）通过遵循完整斯特藩-玻尔兹曼定律的辐射吸收（radiative absorption）机制实现极速质量增长的物理过程及其对恒星质量、中等质量和超大质量黑洞（SMBHs）起源的统一解释。

Data: 研究使用了基于辐射主导时期形成的史瓦西原初黑洞(PBHs)通过完整斯特藩-玻尔兹曼定律(Stefan-Boltzmann law)控制的辐射吸收(radiative absorption)过程进行质量增长的模拟数据。

Method: 采用基于等律原则（isonomy）的全斯特藩-玻尔兹曼定律（full Stefan-Boltzmann law）辐射吸收模型分析原初黑洞（PBHs）质量增长机制。

Result: 辐射主导时期形成的史瓦西原初黑洞（PBHs）通过遵循完整斯特藩-玻尔兹曼定律的辐射吸收机制能极速增长质量，当环境温度高于黑洞视界温度时普遍呈现质量增加现象。

Abstract: We show that Schwarzschild primordial black holes (PBHs) formed in the radiation-dominated era can grow extremely rapidly through $\textit{radiative absorption}$ governed by the full Stefan-Boltzmann law. By introducing a principle of isonomy - ensuring identical particle species dependence for Hawking emission and absorption - we find that, whenever the temperature of the PBH environment is larger than the PBH horizon temperature, PBHs generically gain mass. In particular, for PBH masses following the critical collapse mass-scaling law with critical exponent $γ_\mathrm{crit}$, with $γ_\mathrm{crit} \in (0.33, 0.49)$, the aforementioned radiative absorption mass growth mechanism produces a striking effect: PBHs forming with a mass $10^6M_\odot$ during BBN can reach $\mathcal{O}(10^{10} M_\odot)$ within $\mathcal{O}(10^{6} \mathrm{s})$ ($\sim $ 58 days). Interestingly enough, small deviations from $γ_\mathrm{crit}$, depending itself on the number of relativistic species present in the primordial plasma, yield a continuous PBH mass spectrum providing us ultimately with a single, Standard-Model-based explanation for the origin of stellar-mass, intermediate-mass, and supermassive black holes (SMBHs), and naturally accounting for the early appearance of SMBHs. The Schwarzschild treatment presented here can be extended to spherically symmetric cosmological black holes, indicating that radiative absorption is a dominant and previously overlooked PBH growth channel in the early Universe.

</details>

### [36] [Baryonification III: An accurate analytical model for the dispersion measure probability density function of fast radio bursts](https://arxiv.org/abs/2601.18784)
*MohammadReza Torkamani,Robert Reischke,Michael Kovač,Andrina Nicola,Jozef Bucko,Alexandre Refregier,Sambit K. Giri,Aurel Schneider,Steffen Hagstotz*

**TL;DR**: 基于重子化模型（BFC）建立了快速射电暴（FRB）色散测量（DM）单点概率分布函数（PDF）的全解析预测框架，验证了该模型通过控制气体剖面的关键参数（$M_\mathrm{c}$、$μ$、$δ$）主导PDF形态，并证明对数正态分布在少量FRB样本下适用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用重子化模型（BFC）建立快速射电暴（FRBs）色散量（DM）单点概率分布函数（PDF）的解析预测框架，以解决宇宙尺度上重子效应建模的计算效率问题。

Data: 数据来源于一致性检验模拟（consistency-check simulations）和IllustrisTNG流体动力学模拟（IllustrisTNG hydrodynamical simulation），验证范围覆盖红移值（redshift）高达$z=5$。

Method: 采用重子化模型（BFC）结合晕质量函数和晕偏置，通过解析卷积不同质量与红移晕的贡献来预测快速射电暴（FRB）色散量（DM）的单点概率分布函数（PDF）。

Result: 基于重子化模型(BFC)构建了快速射电暴(FRB)色散测量(DM)单点概率分布函数(PDF)的全解析预测框架，验证了该模型在气体剖面拟合和PDF预测中的自洽性，并确定了控制气体剖面的BFC参数(特别是特征质量尺度$M_\mathrm{c}$、质量依赖斜率$μ$和外部截断$δ$)是PDF形状的主要决定因素。

Abstract: We develop a fully analytical framework for predicting the one-point probability distribution function (PDF) of dispersion measures (DM) for fast radio bursts (FRBs) using the baryonification (BFC) model. BFC provides a computationally efficient alternative to expensive hydrodynamical simulations for modelling baryonic effects on cosmological scales. By applying the halo mass function and halo bias, we convolve contributions from individual halos across a range of masses and redshifts to derive the large-scale structure contribution to the DM PDF. We validate our analytical predictions against consistency-check simulations and compare them with the IllustrisTNG hydrodynamical simulation across a range of redshifts up to $z = 5$, demonstrating excellent agreement. We demonstrate that our model produces consistent results when fitting gas profiles and predicting the PDF, and vice versa. We show that the BFC parameters controlling the gas profile, particularly the halo mass scale ($M_\mathrm{c}$), mass-dependent slope ($μ$), and outer truncation ($δ$), are the primary drivers of the PDF shape. Additionally, we investigate the validity of the log-normal approximation commonly used for DM distributions, finding that it provides a sufficient description for a few hundred FRBs. Our work provides a self-consistent model that links gas density profiles to integrated DM statistics, enabling future constraints on baryonic feedback processes from FRB observations.

</details>

### [37] [A Novel Lensed Point Source Modeling Pipeline using GIGA-Lens with Application to SN Zwicky and SN iPTF16geu](https://arxiv.org/abs/2601.18787)
*Saul Baltasar,Nicolas Ratier-Werbin,Xiaosheng Huang,W. Sheu,C. J. Storfer,Y. -M. Hsu,Sean Xu,David J. Schlegel*

**TL;DR**: 基于GIGA-Lens框架开发的新型强引力透镜点源建模流程，仅利用透镜点源图像即可精确恢复图像位置、流量和时间延迟，并推断复杂透镜质量分布（包括质量密度斜率γ）及哈勃常数（H0），在SN iPTF16geu案例中验证了与传统方法一致性，而在SN Zwicky案例中发现了新的最佳拟合模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用GIGA-Lens框架和JAX平台开发新型强引力透镜点源建模流程，旨在精确反演透镜质量分布（包括质量密度斜率γ）和哈勃常数（H0）。

Data: 数据来源于模拟（simulation）和两个已知的透镜超新星（lensed SNe Ia）Zwicky和iPTF16geu的观测数据。

Method: 采用基于GIGA-Lens框架的建模流程，通过JAX平台在四块A100 GPU上实现强引力透镜点源的完整前向建模（full forward modeling），同时推断所有透镜模型参数和哈勃常数（H0）。

Result: 基于GIGA-Lens框架的建模流程仅利用透镜点源图像即可精确恢复图像位置、流量和时间延迟，并推断复杂透镜质量分布（包括质量密度斜率γ）及哈勃常数（H0）。

Abstract: We introduce a novel modeling pipeline for strongly lensed point sources, using the GIGA-Lens framework, running on four A100 GPUs via the JAX platform. Using simulations, we demonstrate accurate and precise recovery of image positions, fluxes, and time delays, together with inference of complex lens mass distributions -- including the mass density slope, $γ$ -- from images of lensed point sources alone. We further show that we can achieve statistical uncertainty of $\sim 3.6\%$ ($\sim 2.5\, \mathrm{km\, s^{-1}/Mpc}$) on $H_0$ from a single system, with full forward modeling, i.e., simultaneous inference of all lens model parameters together with $H_0$. We apply our pipeline to two well-studied lensed SNe Ia, Zwicky and iPTF16geu. For SN iPTF16geu, unlike previous modeling efforts, we model only the images of the lensed point source (the SN) and do not use the lensed images of the extended host-galaxy. Nevertheless, we are able to infer all of the mass parameters modeled in earlier studies, and our best-fit values, including $γ$, are fully consistent with published results. In the case of SN Zwicky, taking the same approach, however, we obtain an alternative best-fit model compared to published results, underscoring the importance of fully exploring the model parameter space.

</details>

<div id='astro-ph.IM'></div>

# astro-ph.IM [[Back]](#toc)

### [38] [Extragalactic Planetary Nebulae (xPNe). Chemical evolution and assembly histories of nearby galaxies using Oxygen and Argon abundances. From the local universe to cosmic dawn](https://arxiv.org/abs/2601.17106)
*Magda Arnaboldi,Ortwin Gerhard,Martin Roth,Souradeep Bhattacharya,Johanna Hartke,Chiara Spiniello,Azlizan Soemitro,Claudia Pulsoni,Lucas Valenzuela*

**TL;DR**: 研究发现河外行星状星云（xPNe）通过示踪元素丰度、运动学和空间分布，可揭示宇宙早期星系形成与演化过程中合并与相互作用的特征，并将银河考古学研究拓展至本地宇宙最古老的恒星聚集体。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近场宇宙学（Near-Field Cosmology）中星系形成与演化的机制，特别是通过解析恒星成分追溯百亿年前恒星形成历史。

Data: 数据来源包括利用高角分辨率、大覆盖面积和高灵敏度的新设备观测的河外星系行星状星云（xPNe）的氧和氩元素丰度及其运动学数据。

Method: 采用外行星状星云（xPNe）的氧和氩元素丰度测量结合运动学分析，拓展银河系考古学研究至本星系群最古老恒星系统。

Result: 通过研究河外星系行星状星云（xPNe）的元素丰度、运动学和空间分布，可以揭示星系合并和相互作用的长期动力学特征。

Abstract: How galaxies formed and evolved in the expanding Universe is the main science goal of Near-Field Cosmology research. Studies of the properties of galaxies' resolved stars open a widow on their ancient galactic components, probing star formation during epochs more than 10 billion years ago. Extragalactic Planetary Nebulae (xPNe) can help decipher the signatures of mergers and interactions persisting over many dynamical times by tracing elemental abundances coupled with their kinematics and spatial distributions. With new facilities, reaching higher angular resolution, area coverage and sensitivity, one can use xPNe to map Oxygen and Argon element abundances, in addition to their kinematics, to extend the Galactic Archeology investigation to the oldest stellar aggregates in our Local Universe.

</details>

### [39] [Learning at the Edge: Tailed-Uniform Sampling for Robust Simulation-Based Inference](https://arxiv.org/abs/2601.17120)
*Chaipat Tirapongprasert,Matthew Ho*

**TL;DR**: 提出尾式均匀分布（Tailed-Uniform）作为模拟训练参数生成方法，通过高斯尾延伸消除先验边界处的突变，提升神经网络后验估计器在参数空间边界附近的性能表现。


<details>
  <summary>Details</summary>

Background: 研究聚焦于改进基于模拟的推断（simulation-based inference）中训练模拟生成的参数采样策略，通过引入带高斯尾部的均匀分布（Tailed-Uniform）解决先验边界处后验分布不连续导致的神经网络估计器失效问题。

Data: 研究中使用的数据来源包括合成高斯线性任务（synthetic Gaussian linear task）和来自物质功率谱（matter power spectrum）的宇宙学参数推断（cosmological parameter inference）模拟数据。

Method: 采用带尾均匀分布（Tailed-Uniform）作为训练模拟的提议分布，通过高斯尾部平滑扩展先验边界外的参数采样范围。

Result: 提出了一种带尾均匀分布（Tailed-Uniform）的提议分布方法，通过在高斯尾部平滑扩展先验边界外的参数采样区域，解决了后验分布与先验边界相交或超出时神经网络后验估计失效的问题。

Abstract: We introduce the \textsc{Tailed-Uniform} proposal distribution for generating training simulations in simulation-based inference. Instead of sampling parameters uniformly within bounded regions, we extend the distribution beyond prior boundaries with smooth Gaussian tails. This eliminates sharp discontinuities that cause neural posterior estimators to fail when the posterior distribution intersects or extends beyond the prior bounds. The method requires minimal hyperparameter tuning, with tail widths of 10--30\% of the prior width proving robust across problems. We demonstrate these benefits on a synthetic Gaussian linear task and cosmological parameter inference from the matter power spectrum. We also find that \tail-trained models outperform \textsc{Uniform} ones near the boundaries across various training set sizes and dimensions of the parameter space. This advantage grows in higher dimensions, where boundaries dominate parameter space volume. All code is publicly available on Github at https://github.com/chaipattira/tailed-uniform-sbi.

</details>

### [40] [STIS Cycle 30 Calibration Programs](https://arxiv.org/abs/2601.17159)
*D. Welty,R. Bohlin,J. Carlberg,M. Dallas,S. Hernandez,A. Jones,S. Lockwood,S. Medallon,E. Rickman,D. Stapleton,T. Wheeler*

**TL;DR**: 该论文总结了哈勃太空望远镜（HST）第30周期（2022年11月7日至2023年11月5日）期间执行的空间望远镜成像光谱仪（STIS）校准程序，通过19项常规校准任务验证了仪器性能稳定性，更新了参考文件并推进了通量校准（flux calibration）修订工作。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于哈勃太空望远镜(HST)第30周期(2022年11月7日至2023年11月5日)内空间望远镜成像光谱仪(STIS)校准程序组的执行情况与性能评估。

Data: 数据来源于哈勃太空望远镜（HST）第30周期（2022年11月7日至2023年11月5日）执行的19个常规STIS（Space Telescope Imaging Spectrograph）校准程序观测数据。

Method: 采用空间望远镜成像摄谱仪（STIS）的19个常规校准程序进行观测数据分析和参考文件更新。

Result: STIS（Space Telescope Imaging Spectrograph）校准程序在HST（Hubble Space Telescope）第30周期（2022年11月7日至2023年11月5日）执行，验证了现有参考文件的持续适用性并更新了部分模式的光度校准。

Abstract: We discuss the suite of STIS calibration programs executed during HST Cycle 30, covering the period 2022 Nov 07 through 2023 Nov 05. For each of the 19 current regular calibration programs, we provide brief descriptions of the objectives, observations, analysis procedures, and results - with comparisons to the results from previous cycles and to desired accuracies, as well as references to more detailed analyses of the calibration data. Many of these calibration programs produce routine reference file deliveries or demonstrate the continuing applicability of existing reference files for processing STIS observations. This ISR provides a brief snapshot of the current instrument performance, similar to those given in annual reports for Cycles 7-10 and 17-21. Two Appendices briefly discuss the state of the onboard calibration lamps and the ongoing major effort to revise the flux calibration for the many STIS spectroscopic and imaging modes.

</details>

### [41] [Detached eclipsing binary star science in the 2040s](https://arxiv.org/abs/2601.17179)
*Pierre F. L. Maxted,Dominic M. Bowman,Thomas G. Wilson,Sophie Rosu,Keivan G. Stassun,Simon J. Murphy,Ayush Moharana,Amaury H. M. J. Triaud,Axel Hahlin,John Southworth*

**TL;DR**: 论文核心贡献是提出长周期分离食双星（DEBS）对恒星结构和演化研究的关键作用，并指出当前高精度样本以短周期系统为主，需配备2-8米级望远镜的高分辨率阶梯光栅光谱仪来扩大长周期DEBS研究样本。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用分离食双星（DEBS）精确测定恒星基本参数，以约束物理过程对恒星结构和演化的影响，尤其关注长周期双星因其更接近单星特性而具有的特殊价值。

Data: 研究数据主要来自盖亚卫星第三次数据发布(Gaia DR3)中的食双星(DEBS)样本，以及未来可能用于观测长周期食双星的2-8米级望远镜配备的高分辨率阶梯光栅光谱仪(echelle spectrographs)获取的光谱数据。

Method: 采用高分辨率阶梯光栅光谱仪(echelle spectrographs)对长周期分离双星(DEBS)进行系统性光谱监测。

Result: 长周期分离食双星（DEBS）是研究恒星结构和演化的关键目标，因其组分间相互作用最小化而更接近单星特性。

Abstract: Detached eclipsing binary stars (DEBS) are currently the best source of accurate and precise fundamental stellar parameters. This makes DEBS crucial targets for constraining the impact of various physical processes on stellar structure and evolution. Long-period binaries are particularly interesting because their separation minimises interactions between the components. This makes long-period binaries more comparable to single stars. However, the current sample of DEBS with high precision stellar parameters are dominated by short-period systems (e.g. ~90% of the Gaia DR3 eclipsing binaries have periods < 5 days). Facilities capable of performing detailed studies of long-period DEBS will be essential to further improve our understanding of stellar structure and evolution. Such facilities would need to be able to obtain spectroscopic observations of more distant objects at high resolution and cadence. 2-8m class telescopes with echelle spectrographs and an ability to monitor a large sample of stars would be required.

</details>

### [42] [Improving Generalization and Uncertainty Quantification of Photometric Redshift Models](https://arxiv.org/abs/2601.17222)
*Jonathan Soriano,Tuan Do,Srinath Saikrishnan,Vikram Seenivasan,Bernie Boscoe,Jack Singal,Evan Jones*

**TL;DR**: 通过结合光谱红移(spectroscopic redshifts)和多波段测光红移数据训练复合数据集，神经网络(NN)模型在红移范围0.3<z<1.5内将测光红移(photo-z)偏差降低4.5倍、散射减少1.1倍、异常率下降1.4倍，贝叶斯神经网络(BNN)则能提供可靠的不确定性估计。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过结合不同来源的真实红移数据（spectroscopic redshifts和photometric redshifts）提升机器学习模型在更广泛星系类型中光变红移（photo-z）估计的适用性，以支持星系演化和精密宇宙学研究。

Data: 数据来源包括光谱红移数据集GalaxiesML和多波段（约35个滤光片）测光红移数据COSMOS2020（TransferZ），用于训练和评估神经网络模型。

Method: 采用确定性神经网络（NN）和贝叶斯神经网络（BNN）架构，并结合分裂保形预测（split conformal prediction）校准不确定性估计和生成预测区间。

Result: 在复合数据集上训练的神经网络（NN）将红移范围0.3<z<1.5内的测光红移（photo-z）偏差降低了4.5倍，散射减少了1.1倍，异常率降低了1.4倍。

Abstract: Accurate redshift estimates are a vital component in understanding galaxy evolution and precision cosmology. In this paper, we explore approaches to increase the applicability of machine learning models for photometric redshift estimation on a broader range of galaxy types. Typical models are trained with ground-truth redshifts from spectroscopy. We test the utility and effectiveness of two approaches for combining spectroscopic redshifts and redshifts derived from multiband ($\sim$35 filters) photometry, which sample different types of galaxies compared to spectroscopic surveys. The two approaches are (1) training on a composite dataset and (2) transfer learning from one dataset to another. We compile photometric redshifts from the COSMOS2020 catalog (TransferZ) to complement an established spectroscopic redshift dataset (GalaxiesML). We used two architectures, deterministic neural networks (NN) and Bayesian neural networks (BNN), to examine and evaluate their performance with respect to the Legacy Survey of Space and Time (LSST) photo-$z$ science requirements. We also use split conformal prediction for calibrating uncertainty estimates and producing prediction intervals for the BNN and NN, respectively. We find that a NN trained on a composite dataset predicts photo-$z$'s that are 4.5 times less biased within the redshift range $0.3<z<1.5$, 1.1 times less scattered, and has a 1.4 times lower outlier rate than a model trained on only spectroscopic ground truths. We also find that BNNs produce reliable uncertainty estimates, but are sensitive to the different ground truths. This investigation leverages different sources of ground truths to develop models that can accurately predict photo-$z$'s for a broader population of galaxies crucial for surveys such as Euclid and LSST.

</details>

### [43] [2MASS Re-processing I: The Search for Faint Objects](https://arxiv.org/abs/2601.17831)
*Zehao Liu,Xiyan Peng,Zhenghong Tang,Zhaoxiang Qi,Shilong Liao,Yong Yu*

**TL;DR**: 开发了一种基于DAOFind的自动化流程，通过优化检测参数和联合评估信噪比与中心锐度，将2MASS（Two Micron All Sky Survey）J波段巡天数据的探测极限从16.20星等提升至16.60星等，并比原有点源目录多检测出21.36%的暗弱点源（false-positive rate仅4.80%），显著提升了存档数据的科学价值。


<details>
  <summary>Details</summary>

Background: 开发自动化流程提升2微米全天巡天(2MASS)档案数据的微弱点源探测能力，以支持时域变源、银河系结构和低温低光度天体的研究。

Data: 数据来源于两微米全天巡天（2MASS）的J波段全天天图发布巡天图像（Atlas All Sky Release Survey Images）。

Method: 采用基于DAOFind的自动化流程，通过优化检测参数并联合评估信噪比（signal-to-noise ratio）和中心锐度（central sharpness）的筛选程序，有效识别先前未检测到的暗弱点源（point sources）。

Result: result：通过优化检测参数并联合评估信噪比和中心锐度，改进后的流程将2MASS（Two Micron All Sky Survey）探测极限从16.20星等提升至16.60星等，点源（point sources）探测数量增加21.36%，且假阳性率仅为4.80%。

Abstract: We present an automated, DAOFind-based pipeline developed to reprocess J-band Atlas All Sky Release Survey Images from the Two Micron All Sky Survey (2MASS). By optimizing the detection parameters and implementing a screening procedure that jointly evaluates the signal-to-noise ratio and central sharpness, the pipeline effectively identifies faint point sources that were previously undetected. Applying this method to eight representative sky regions improves the 2MASS detection limit from 16.20 to 16.60 mag and increases the number of detected point sources by approximately 21.36% relative to the 2MASS Point Source Catalog, with a false-positive rate of only 4.80%. These results demonstrate that the proposed reprocessing pipeline can substantially enhance the scientific yield of archival 2MASS data, providing valuable faint-source supplements for studies of time-domain variability, Galactic structure, and cold, low-luminosity objects.

</details>

### [44] [SiGMa-Net II: Distinguishing Binary Black Holes from Glitches](https://arxiv.org/abs/2601.17882)
*Soorya Narayan,Anupreeta More,Sunil Choudhary,Sudhagar Suyamprakasam,Sukanta Bose*

**TL;DR**: 该研究利用迁移学习结合InceptionNetV3和正弦高斯投影(Sine-Gaussian Projection, SGP)映射，在LIGO第三次观测运行数据中有效区分了双黑洞(Binary Black Hole, BBH)合并信号与六类常见噪声瞬变(glitches)，测试集准确率达87%，显著提升了引力波事件快速识别的效率。


<details>
  <summary>Details</summary>

Background: 引力波（GW）探测器灵敏度提升导致可探测事件显著增加，亟需快速准确处理和分析双黑洞（BBH）合并产生的大量信号。

Data: 数据来源包括LIGO第三次观测运行中通过GravitySpy识别的真实瞬态噪声（glitches），以及注入到两个LIGO探测器真实噪声中的模拟双黑洞（BBH）信号。

Method: 采用迁移学习（transfer learning）结合InceptionNetV3模型，并基于正弦高斯投影（Sine-Gaussian Projection, SGP）映射技术来区分双黑洞（BBH）信号与六种常见噪声瞬态（glitches）。

Result: 该研究通过使用基于InceptionNetV3的迁移学习和正弦高斯投影(SGP)映射方法，在区分双黑洞(BBH)信号与六种常见瞬态噪声(glitches)时实现了87%的准确率，证明了SGP映射能有效提升引力波事件快速识别的效率。

Abstract: With increasing sensitivity of the gravitational wave (GW) detectors, we expect a significant rise in the detectable GW events. To process, analyse and identify such large amounts of GW signals arising from mergers of Binary Black Holes (BBH), we need both speed and accuracy. In the search for (massive) BBH signals, the biggest hurdle is posed by the various non-gaussian noise transients called glitches. Compared to our previous work, which used a simple convolutional neural network to distinguish BBHs from Blip glitches, this work uses transfer learning with InceptionNetV3 to distinguish BBHs from six types of most popular glitches from the third observing run of LIGO. While the glitches are real and identified via GravitySpy, the BBH signals are simulated and then injected into the real detector noise for each of the two LIGO detectors. We generate Sine-Gaussian Projection (SGP) maps by cross-correlating data with Sine-Gaussian functions of varied quality factors ($Q$) and central frequencies ($f_0$) and projected on the $Q$ - $f_0$ plane. We find that SGP maps make it easier to distinguish BBHs from glitches that look very similar to BBHs in the Time-Frequency maps like the Blips, while also maintaining significant morphological differences between BBHs and the more frequent glitches - Scattered Light and Fast Scattering. Our network has an accuracy of $87%$, a TPR of 0.83 for an FPR of 0.1 on our test dataset. It is also robust, retaining its level of accuracy, when tested on real BBH events identified in the first three observing runs of LIGO. Our proposed method shows the viability of using the SGP maps and neural networks for fast identification of GW events improving the efficiency of standard search pipelines.

</details>

### [45] [150 years of ground-based solar instrumentation at Meudon observatory (1876-2026)](https://arxiv.org/abs/2601.18227)
*Jean-Marie Malherbe*

**TL;DR**: 该论文回顾了1876年至今默东天文台（Meudon）在地基或气球仪器方面的太阳观测技术发展历程，突出了默东天文学家在太阳物理学史上的先驱作用。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是回顾默东天文台自1876年成立至今在地面或气球太阳观测仪器发展中的关键贡献及其在太阳物理学史上的先驱作用。

Data: 数据来源包括默东（Meudon）设计并安装于当地或其他地点（如楠赛（Nançay）、比利牛斯山（Pic du Midi）、加那利群岛（Canary Islands））的地面或气球仪器观测数据。

Method: 采用摄影术(photography)、光谱学(spectroscopy)、日冕仪(coronagraph)、单色滤光片(monochromatic filters)及大型地基望远镜(ground-based telescopes)等创新技术手段开展太阳仪器研发。

Result: 太阳研究在19世纪末因摄影术和光谱学(spectroscopy)出现、20世纪初因日冕仪(coronagraph)和单色滤光片发明、20世纪后半叶因大型地基望远镜和空间探索而取得重大突破。

Abstract: The Sun has been observed through a telescope for four centuries. However, its study made a prodigious leap at the end of the nineteenth century with the appearance of photography and spectroscopy, then at the beginning of the following century with the invention of the coronagraph and monochromatic filters, and finally in the second half of the twentieth century with the advent of large ground-based telescopes and space exploration. This article retraces the main stages of solar instrumental developments in Meudon, from its foundation by Jules Janssen in 1876 to the present day, limited to ground-based or balloon instrumentation, designed in Meudon and installed there or in other places (Nan{\c c}ay, Pic du Midi, Canary Islands). The Meudon astronomers played a pioneering role in the history of solar physics through the experimentation of innovative techniques. After the golden age of inventions, came the time of large instruments, studied in Meudon but often installed in more favourable sites, and that of space, in a framework of international collaboration, but this is not discussed here.

</details>

### [46] [Photoexcitation spectroscopy of highly charged ions for application to astronomy using a compact electron beam ion trap (EBIT) at the synchrotron radiation facility SPring-8](https://arxiv.org/abs/2601.18311)
*Leo Hirata,Yuki Amano,Moto Togawa,Hiroyuki A. Sakaue,Nobuyuki Nakamura,Makoto Sawada,Hiromasa Suzuki,Masaki Oura,Hiroya Yamaguchi*

**TL;DR**: 通过电子束离子阱(EBIT)实验精确测量了类氖铁离子(Fe$^{16+}$)的L$α$共振跃迁(3C和3G)及类氦氧离子(O$^{6+}$)的K$α$共振跃迁光谱数据，为天体等离子体光谱建模提供了关键原子参数。


<details>
  <summary>Details</summary>

Background: 高分辨率X射线天文光谱观测对精确原子数据的需求推动了实验等离子体光谱学的发展。

Data: 数据来源于电子束离子阱（electron beam ion trap, EBIT）与同步辐射设施SPring-8结合实验获取的高分辨率光谱数据，包括Ne-like Fe$^{16+}$离子的L$α$共振跃迁（3C和3G）以及He-like O$^{6+}$离子的K$α$共振跃迁。

Method: 采用电子束离子阱（EBIT）结合同步辐射设施（synchrotron radiation facilities）进行实验测量，获取高精度原子数据。

Result: 成功利用JAXA-EBIT在SPring-8同步辐射设施中获得了高分辨率的类氖铁离子(Ne-like Fe$^{16+}$) L$α$共振跃迁(3C和3G)及类氦氧离子(He-like O$^{6+}$) K$α$共振跃迁光谱数据，并首次实验测定了3G与3C振子强度比的上限值。

Abstract: In the past few decades, X-ray astronomy satellites equipped with grating spectrometers and microcalorimeters have enabled high-resolution spectroscopic observations of astrophysical objects. The need for accurate atomic data has arose as we attempt detailed analysis of the high-resolution spectra they provide. This is because current spectral models, which heavily rely on theoretical calculations, entail non-negligible uncertainties. We employ a plasma spectroscopy device called electron beam ion trap (EBIT) to experimentally obtain precise atomic data. An EBIT with a design that allows combined operation with synchrotron radiation facilities was developed based on the Heidelberg Compact EBIT and installed at ISAS/JAXA for this purpose. We conducted a spectroscopic experiment using the JAXA-EBIT at the synchrotron radiation facility SPring-8, and successfully obtained high-resolution spectra of the L$α$ resonance transition of Ne-like Fe$^{16+}$ ions, 3C, as well as the K$α$ resonance transition of He-like O$^{6+}$ ions. We also measured another Ne-like Fe$^{16+}$ L$α$ resonance transition, 3G, and constrained an upper limit of the oscillator strength ratio of 3G to 3C, using our experimental results. The experimental values obtained in this study will be applied to observational studies of astrophysical objects as a part of the plasma spectral modeling.

</details>

### [47] [tilepy: A Flexible Open-Source Scheduling Engine for Time-Domain and Multi-Messenger Astronomy](https://arxiv.org/abs/2601.18668)
*Fabian Schüssler,H. Ashkar,W. Kiendrébéogo,M. Seglar-Arroyo,M. de Bony,A. Berti,E. Ruiz-Velasco,R. Le Montagner*

**TL;DR**: 开发了Python软件tilepy（集成于Astro-COLIBRI平台），通过模块化架构优化多信使（GW、GRB、高能中微子等）暂现源事件的实时观测调度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于多信使天体物理（multi-messenger astrophysics）时代对瞬变事件（transient events）快速高效跟测的需求优化。

Data: data来源未明确提及，但tilepy设计用于优化地面和空间观测台（ground- and space-based observatories）的观测计划。

Method: 开发了基于Python的模块化软件tilepy，采用高层调度逻辑与底层拼接和指向工具分离的架构设计。

Result: 开发了基于Python的模块化软件tilepy，用于优化多信使天体物理瞬变事件的观测调度。

Abstract: The era of multi-messenger astrophysics requires rapid and efficient follow-up of transient events, many of which, such as gravitational waves (GW), gamma-ray bursts (GRB), and high-energy neutrinos, suffer from poor sky localisation. We present tilepy, a Python-based software designed to optimize observation schedules for these events. We here detail the modular architecture of tilepy, which separates high-level scheduling logic from low-level tiling and pointing tools, enabling full adaptability for ground- and space-based observatories. Furthermore, we describe the integration of tilepy into the Astro-COLIBRI platform, providing the community with a user-friendly interface and API for triggering complex observation campaigns in real time.

</details>

<div id='astro-ph.HE'></div>

# astro-ph.HE [[Back]](#toc)

### [48] [Neuro-Parametric Spectral Classification of Black Hole and Neutron Star X-ray Binary Systems](https://arxiv.org/abs/2601.18139)
*Akash Garg,Aman Kumar,Ajit Kembhavi,Ranjeev Misra,Aniruddha Kembhavi,N. S. Philip,Rohan Pattnaik,Shreya Watwe*

**TL;DR**: 该研究利用深度神经网络对RXTE X射线光谱数据进行黑洞和中子星X射线双星系统分类，发现仅需少量关键物理参数（如黑体温度、幂律指数、黑体与幂律通量比、约化卡方和数据方差）即可达到与直接使用光谱数据相当的分类精度（90-94%），为当前和未来X射线巡天提供了一种可解释且不依赖特定任务的致密天体分类方法。


<details>
  <summary>Details</summary>

Background: 研究主题为利用深度神经网络对黑洞（black hole）和中子星（neutron star）X射线双星系统进行分类，并探索其物理可解释性。

Data: 数据来源于RXTE（Rossi X-ray Timing Explorer）卫星存档的X射线光谱数据。

Method: 采用深度神经网络对RXTE X射线光谱数据进行分类，并构建了仅使用光谱通量值和同时使用通量及其误差的两种模型。

Result: 基于深度神经网络对RXTE X射线光谱数据的分析，能够以90-94%的高准确率区分黑洞和中子星X射线双星系统，且关键物理参数（如黑体温度、幂律指数、黑体与幂律通量比、约化卡方和数据方差）的解释性分析验证了分类结果的物理意义。

Abstract: We perform the classification of black hole and neutron star X-ray binary systems using deep neural networks applied to archival RXTE X-ray spectral data. We first construct two neural network models: one trained using only spectral flux values and another trained using both fluxes and their associated errors. Both models achieve high classification accuracies of ~90-94 %. To gain physical interpretability of these networks, we fit all spectra with a simple phenomenological model consisting of a thermal disk component and a power-law. From this analysis, we identify the blackbody temperature, power-law index, the ratio of blackbody to power-law flux, the reduced $χ^2$, and the variance of the data as key parameters that likely contribute to the classification. We validate this inference by designing an additional neural network trained exclusively on this reduced parameter set, without using the spectral data directly. This parameter-based model achieves a classification accuracy comparable to that of the spectral models. Our results show that deep neural networks can not only classify compact objects in X-ray binaries with high accuracy but can also be interpreted in terms of physically meaningful spectral parameters derived from conventional X-ray spectral analysis. This framework offers a promising, mission-agnostic approach for compact object classification in current and future X-ray surveys.

</details>

### [49] [Revisiting a Quasar Microlensing Event Towards AGN~J1249+3449](https://arxiv.org/abs/2601.18262)
*Mario Cazzolla,Francesco De Paolis,Antonio Franco,Achille Nucita*

**TL;DR**: 研究发现GW190521引力波事件后约34天观测到的活动星系核（Active Galactic Nucleus, AGN）J1249+3449光学增亮现象，可通过宿主星系内质量约0.1倍太阳质量（$M_{\odot}$）的微引力透镜（microlensing）效应合理解释。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于引力波事件GW190521与活动星系核（AGN）J1249+3449电磁对应体的可能关联性，探讨其光学增亮现象是否源于宿主星系内微引力透镜效应。

Data: 数据来源于兹威基瞬变设施（Zwicky Transient Facility, ZTF）的观测数据。

Method: 采用三种拟合模型（点源点透镜、有限源点透镜、均匀源双透镜）对ZTF观测的光变曲线进行详细分析。

Result: GW190521的电磁对应体（electromagnetic counterpart）光学增亮现象可解释为由宿主星系中质量约0.1倍太阳质量（$M_{\odot}$）的透镜体（lens）产生的微引力透镜（microlensing）效应。

Abstract: The gravitational wave event GW190521 seems to be the only BH merger event possibly correlated with an electromagnetic counterpart, which appeared about 34 days after the GW event. This work aims to confirm that the electromagnetic bump towards the Active Galactic Nucleus (AGN) J1249+3449 can be explained within the framework of the gravitational microlensing phenomenon. In particular, considering the data of the Zwicky Transient Facility (ZTF), what emerges from a detailed analysis of the observed light curve using three fitting models (Point Source Point Lens, Finite Source Point Lens, Uniform Source Binary Lens) is that the optical bump can be explained as a microlensing event caused by a lens with mass {$\sim\,$0.1 $M_{\odot}$}, lying in the host galaxy of the AGN in question.} %MDPI: Please confirm if the bold formatting is necessary; if not, please remove it.

</details>

### [50] [XFit: Global Optimization and Degeneracy Mapping in X-ray Spectral Modeling](https://arxiv.org/abs/2601.18542)
*Austin MacMaster,Adam Rogers,Jason Fiege,Rebecca Man,Samar Safi-Harb*

**TL;DR**: 提出XFit全局优化方法（基于Ferret进化算法），用于X射线光谱拟合，能自动探索复杂参数空间、高效映射置信区间并识别局部方法可能忽略的简并解（degenerate solutions），在仙后座A中央致密天体（Central Compact Object）和超新星遗迹G41.1-0.3的测试中表现优于传统局部优化方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于改进X射线光谱数据建模方法，针对局部优化算法（如Levenberg-Marquardt）在高维或简并参数空间（degenerate parameter spaces）易陷入局部极小值且依赖人工干预的局限性。

Data: 研究中使用的数据来源于仙后座A中的中央致密天体（Central Compact Object in Cassiopeia A）和超新星遗迹G41.1-0.3（supernova remnant G41.1-0.3）的X射线观测数据。

Method: 采用基于Ferret进化算法(evolutionary algorithm)的全局优化方法XFit进行X射线光谱拟合，自动探索复杂参数空间并识别局部方法可能忽略的简并解(degenerate solutions)。

Result: XFit通过全局优化方法（Ferret evolutionary algorithm）实现了对X射线数据复杂参数空间的自动化探索，能有效识别局部优化方法可能忽略的简并解（degenerate solutions）并提升拟合精度。

Abstract: The standard approach to modeling X-ray spectral data relies on local optimization methods, such as the Levenberg-Marquardt algorithm. While effective for simple models and speedy spectral fitting, these local optimizers are prone to becoming trapped in local minima, particularly in high-dimensional or degenerate parameter spaces, and typically require extensive user intervention. In this work, we introduce XFit, a global optimization method for fitting X-ray data, which makes extensive use of the Ferret evolutionary algorithm. XFit enables automated exploration of complex parameter spaces, efficient mapping of confidence intervals, and identification of degenerate solutions that may be overlooked by local methods. We demonstrate the performance of XFit using two representative X-ray sources: the Central Compact Object in Cassiopeia A and the supernova remnant G41.1-0.3. These examples span both low- and high-dimensional models, allowing us to illustrate the advantages of global optimization. In both cases, XFit produces solutions that are consistent with or improve upon those found with traditional methods, while also revealing alternative fits or degenerate solutions within statistically acceptable confidence levels. The automated mapping of parameter space offered by XFit makes it a powerful complement to existing spectral fitting tools, particularly as models and data quality become increasingly complex. Future work will expand the application of XFit to broader datasets and more physically motivated models.

</details>

### [51] [Gas Kinematics and Cosmic-Ray Acceleration in the Gamma-ray SNRs W41 and G22.7-0.2](https://arxiv.org/abs/2601.18040)
*Takeru Murase,Hidetoshi Sano,Kohei Matsubara,Yasuo Fukui,Junya Nishi,Sabrina Einecke,Miroslav Filipovic,Rina Kasai,Ren Matsusaka,Gavin Rowell,Hiroshi Sodoh,Hiromasa Suzuki,Yosuke Shibata,Kisetsu Tsuge,Hiroshi Takaba,Toshihiro Handa*

**TL;DR**: 通过高分辨率分子和一氧化碳(CO)及氢原子(HI)数据，研究发现中老年超新星遗迹(SNRs) W41和G22.7-0.2与星际介质相互作用的气体云，并估算出其加速宇宙线质子(cosmic-ray protons)总能量分别为3×10⁴⁷ erg和1×10⁴⁸ erg，符合SNR年龄-Wₚ关系中Wₚ随年龄递减的趋势。


<details>
  <summary>Details</summary>

Background: 研究聚焦于中老年超新星遗迹（SNRs）W41和G22.7-0.2与星际介质（ISM）的相互作用及其对甚高能（TeV）伽马射线辐射的贡献。

Data: 数据来源为野边山45米望远镜的高角分辨率$^{12}$CO($J$ = 1-0)数据和甚大天线阵(VLA)的HI数据。

Method: 采用高角分辨率$^{12}$CO($J$ = 1-0)数据（来自野边山45米望远镜）和HI数据（来自VLA）分析分子与原子气体的空间及运动学特性。

Result: W41和G22.7-0.2两个中年超新星遗迹(SNRs)中加速的宇宙线质子总能量分别为~3×10⁴⁷ erg和~1×10⁴⁸ erg，与已报道的SNR年龄-质子能量关系(Wₚ)中中年SNRs的下降趋势一致。

Abstract: We present a study of the interstellar medium associated with the two middle-aged supernova remnants (SNRs) W41 and G22.7-0.2, both detected in TeV gamma-rays. Using high-angular-resolution $^{12}$CO($J$ = 1-0) data from the Nobeyama 45-m telescope and HI data from the VLA, we investigated the spatial and kinematic properties of molecular and atomic gas that interact with the SNRs. We identified associated clouds in the velocity ranges of +50-+80 km s$^{-1}$ for W41 and +76-+110 km s$^{-1}$ for G22.7-0.2. Column density analysis indicates that target protons are dominated by molecular hydrogen, while atomic hydrogen contributes less than $\sim$10-15% even after correction for self-absorption. The mean proton densities are $\sim$1.2$\times$10$^{3}$ cm$^{-3}$ for W41 and $\sim$5.3$\times$10$^{2}$ cm$^{-3}$ for G22.7-0.2. From the gamma-ray luminosities, we estimate the total energy of accelerated cosmic-ray protons as $W_\mathrm{p}$ $\sim$3$\times$10$^{47}$~erg for W41 and $\sim$1$\times$10$^{48}$ erg for G22.7-0.2, corresponding to 0.03-0.1% of the canonical supernova explosion energy. hese $W_\mathrm{p}$ values agree with the decreasing trend in $W_\mathrm{p}$ observed in the middle-aged SNRs within the previously reported SNR age-$W_\mathrm{p}$ relation.

</details>

### [52] [Multi-epoch VLBI observations of the blazar 3C 66A: Spatial twisting and temporal oscillation of the parsec-scale jet](https://arxiv.org/abs/2601.18551)
*Paloma Thevenet,Jeonguk Kim,Guang-Yao Zhao,Bong Won Sohn,Suk-Jin Yoon*

**TL;DR**: 通过高分辨率VLBI观测发现3C 66A喷流位置角(PA)存在振幅7.77±0.79度、周期10.94±0.22年的振荡叠加-0.83±0.07度/年的持续顺时针偏移，结合核心流量周期性变化与光学周期特征，支持其由超大质量双黑洞(SMBHB)系统中轨道运动与盘轨错位驱动的喷流进动模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用甚长基线干涉测量(VLBI)技术探索耀变体(blazar) 3C 66A喷流的运动学特征及其物理起源，特别是喷流摆动和核心流量变化的周期性现象与超大质量黑洞双星系统(SMBHB)中轨道运动导致的喷流进动之间的关联。

Data: 使用韩国VLBI网络（KVN）和VERA阵列（KaVA）在2014年三个时期进行的22/43 GHz观测数据，以及1996-2025年间收集的109组甚长基线阵列（VLBA）43 GHz存档数据。

Method: 采用甚长基线干涉测量(VLBI)技术获取高分辨率数据，并通过圆形高斯拟合(Circular Gaussian fittings)对紫外可见度(UV visibilities)进行参数化分析。

Result: 首次发现3C 66A喷流位置角（PA）以10.94±0.22年为周期、7.77±0.79度为振幅振荡，并叠加-0.83±0.07度/年的持续顺时针偏移。

Abstract: Previous VLBI kinematic studies of the blazar 3C 66A have unveiled complex jet kinematic behaviors. Using follow-up high-resolution VLBI observations and archival data, we investigate the morphology and the variations in orientation and core flux density of the 3C 66A jet to gain a deeper insights into its kinematic behavior and physical origins. We performed KVN and VERA array (KaVA) observations at 22/43 GHz over three epochs in 2014 and collected 109 sets of Very Long Baseline Array (VLBA) archival data at 43 GHz between 1996 - 2025. We imaged the parsec-scale jet and parameterized it using circular Gaussian fittings to the UV visibilities. Finally, we derived the inner jet PA and the core flux densities for the VLBA data. The jet presents a twisted morphology in the KaVA maps. The PA of the fitted Gaussian components is in the range between 170 deg and 195 deg. Our kinematic analysis using the VLBA data indicates that the PA oscillates with an amplitude of 7.77 pm 0.79 deg and a period of 10.94 pm 0.22 years, presented for the first time in this work. This oscillation is topped by a continuous clockwise shift of the PA by -0.83 pm 0.07 deg/year. We also identified a strong core flux variability with possible periodicity and a 2 sigma correlation between the core flux density and the inner jet PA change. We discuss possible physical models that could explain the observed features for this object; in particular, a supermassive black hole binary (SMBHB) system, Lense Thirring (LT) effect, and jet or disk instabilities. The oscillation and continuous shift of the PA and the possible radio flux periodicity, together with the optical flux periodicity of approximately 2 years that had previously been confirmed in several independent studies, favor a jet precession scenario driven by orbital motion and disk-orbit misalignment in a SMBHB system.

</details>

<div id='physics.flu-dyn'></div>

# physics.flu-dyn [[Back]](#toc)

### [53] [AI-based separation of turbulence from coherent background flows in decaying hydrodynamic turbulence](https://arxiv.org/abs/2601.18163)
*Ji-Hoon Ha,Elena S. Volnova*

**TL;DR**: 提出基于人工智能（AI）的湍流-背景分离方法，在二维不可压缩纳维-斯托克斯（Navier-Stokes）模拟中成功恢复了演化不同阶段的湍流涨落（turbulent fluctuations）并保持惯性区谱标度（inertial-range spectral scaling）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于从数值模拟和天文观测数据中分离湍流涨落（turbulent fluctuations）与相干大尺度背景流（coherent large-scale background flows）这一基础性难题。

Data: 数据来源于二维不可压缩纳维-斯托克斯(Navier-Stokes)模拟的衰减流体动力学湍流，初始条件包含相干背景流和满足类科尔莫戈罗夫(Kolmogorov-like)功率谱的无散湍流扰动，演化过程无外部驱动。

Method: 采用基于人工智能（AI）的湍流-背景分离方法，通过训练在静态合成图像上的神经网络分析二维不可压缩纳维-斯托克斯（Navier-Stokes）模拟数据。

Result: 基于AI的湍流-背景分离方法在早期和中期阶段能有效恢复湍流涨落，并在后期阶段仍能保持视觉和频谱上合理的湍流结构及惯性区谱标度。

Abstract: Separating turbulent fluctuations from coherent large-scale background flows is a fundamental challenge in the analysis of numerical simulations and astronomical observations. Traditional approaches to this problem commonly rely on decomposition-based techniques, including scale-based filtering methods such as Fourier or wavelet transforms, as well as adaptive methods like the Hilbert-Huang transformation. In realistic flows, however, coherent motions and turbulent fluctuations often overlap across a broad range of scales and interact nonlinearly, rendering any clear and unique separation inherently ambiguous, particularly in astrophysical settings where data are projected or sparsely sampled. In this work, we assess the robustness of AI-based turbulence-background separation using two-dimensional incompressible Navier-Stokes simulations of decaying hydrodynamic turbulence. The simulations are initialized with a coherent background flow and divergence-free turbulent perturbations with a Kolmogorov-like power spectrum, and evolve without external forcing, providing a conservative physical testbed. A neural network trained exclusively on static synthetic images is applied to simulation snapshots at different evolutionary stages. We find that the model successfully recovers turbulent fluctuations during early and intermediate stages, when partial scale separation is preserved. At later stages, despite the substantial decay of turbulent energy and the resulting reduction in fluctuation strength, the model continues to recover visually and spectrally plausible turbulent structures and preserves inertial-range spectral scaling, demonstrating robust separation under increasingly challenging conditions. These results demonstrate the potential of applying AI models trained on static data to time-evolving turbulent flows, with direct implications for astrophysical and cosmological applications.

</details>

<div id='gr-qc'></div>

# gr-qc [[Back]](#toc)

### [54] [Assessing astrophysical foreground subtraction in DECIGO using compact binary populations inferred from the first part of the LIGO-Virgo-KAGRA's fourth observation run](https://arxiv.org/abs/2601.18378)
*Takahiro S. Yamamoto*

**TL;DR**: 论文核心贡献是评估利用LIGO-Virgo-KAGRA合作组最新引力波事件目录推断的种群模型，验证了必须采用Cutler & Harms (2005)提出的投影方案才能将双星信号降至DECIGO可探测原初随机引力波背景(SGWB)的水平。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过空间引力波探测器DECIGO在0.1赫兹频段探测宇宙暴胀时期产生的随机引力波背景（SGWB），并解决该频段内致密双星并合引力波信号对原始背景探测的干扰问题。

Data: 数据来源于LIGO-Virgo-KAGRA合作组最新发布的引力波事件目录（gravitational wave event catalogue）中推断出的双星系统种群模型（population model）。

Method: 采用基于LIGO-Virgo-KAGRA合作组最新引力波事件目录推断的群体模型（population model）和Cutler & Harms (2005)提出的投影方案（projection scheme）来评估双星信号扣除的可行性。

Result: 通过采用LIGO-Virgo-KAGRA合作组最新引力波事件目录推断的种群模型，证实Cutler & Harms (2005)提出的投影方案对于将双星信号降至DECIGO可探测原初随机引力波背景(SGWB)的水平是必要的。

Abstract: Detecting the stochastic gravitational wave background (SGWB) from our Universe under the inflationary era is one of the primary scientific objectives of DECIGO, a space-borne gravitational wave detector sensitive in the 0.1 Hz frequency band. This frequency band is dominated by the gravitational waves from inspiraling compact object binaries. Subtracting these signals is necessary to search for the primordial SGWB. In this paper, we assess the feasibility of the subtraction of such binary signals by employing the population model inferred from the latest gravitational wave event catalogue of the LIGO-Virgo-KAGRA Collaboration. We find that the projection scheme, which was originally proposed by Cutler & Harms (2005), is necessary to reduce the binary signals to the level where DECIGO can detect the primordial background.

</details>

<div id='physics.class-ph'></div>

# physics.class-ph [[Back]](#toc)

### [55] [Visualisation of spherical harmonics in Peirce's quincuncial projection](https://arxiv.org/abs/2601.17045)
*Bjoern Malte Schaefer*

**TL;DR**: 该论文通过对比球谐函数（spherical harmonics）在三维空间与皮尔斯五分投影（Peirce's quincuncial projection）二维展开图中的可视化表现，揭示了后者能直观保留旋转对称性与角度关系，为理解球谐函数数学特性提供了更友好的教学工具。


<details>
  <summary>Details</summary>

Background: 研究聚焦于球谐函数（spherical harmonics）在物理学和天文学中的广泛应用及其在皮尔斯梅花投影（Peirce's quincuncial projection）中的二维可视化教学价值。

Data: 数据来源于理论推导的球谐函数（spherical harmonics）数学性质和皮尔斯四分投影（Peirce's quincuncial projection）的几何变换。

Method: 采用皮尔斯梅花投影法（Peirce's quincuncial projection）将球面谐函数（spherical harmonics）从三维球面保角映射到二维正方形面，直观展示其数学关系与对称性。

Result: 该论文展示了皮尔斯梅花投影（Peirce's quincuncial projection）为球谐函数（spherical harmonics）提供了保留角度和旋转对称性的二维直观可视化方法。

Abstract: The spherical harmonics $Y_{\ell m}(θ,\varphi)$ are complex-valued functions on the surface of a sphere, and have found widespread application in physics and astronomy. Every physics students knows them from quantum mechanics and electromagnetic theory, where they form the basis of hydrogen orbitals and of the multipole expansion, respectively. More advanced applications include the physics of the cosmic microwave background, gravitational lensing, and gravitational waves. In this paper I aim to contrast their usual $3d$ visualisation with Peirce's quincuncial projection, a conformal projection of the sphere onto a $2d$ unfolded square dihedron, where the projection respects the fundamental rotational symmetries and preserves angles. With this mapping, I guide the reader through the properties of the spherical harmonics in a pedagogical way and show that many of their mathematical relations have an intuitive visualisation on Peirce's $2d$ map, which might be useful for people challenged by processing $3d$ shapes, or which people might appreciate aesthetically.

</details>

<div id='eess.SY'></div>

# eess.SY [[Back]](#toc)

### [56] [Autonomous Mars Rover Module for Soil Sampling and Life Component Analysis](https://arxiv.org/abs/2601.17158)
*Bibek Adhikari,Rishab Rijal,Rakesh Yadav,Nikchey Khatri,Sandesh Dhakal*

**TL;DR**: 提出了一种集成于火星车(Mars rover)的自主生命检测模块(proof-of-life module)，通过机械钻探和生化分析实现火星土壤中生命成分的现场检测。


<details>
  <summary>Details</summary>

Background: 探索地外生命（extraterrestrial life）是科学研究的核心主题，技术快速进步和宇宙认知深化推动了对火星等行星生命存在可能性的研究。

Data: 研究中使用的数据来源于火星车搭载的生命探测模块（proof-of-life module）自主采集的火星土壤样本，并通过车载生化分析系统进行实时检测。

Method: 采用集成机械系统（钻探机构和真空系统）与生化分析相结合的自主生命探测模块（proof-of-life module），实现火星土壤样本的现场采集与检测。

Result: 提出了一种集成于火星车的自主生命检测模块（proof-of-life module），能通过机械钻探和真空系统采集土壤样本并进行生化分析，成功验证了地外环境中自主检测生命存在与否的可行性。

Abstract: The search for extraterrestrial life has long been a primary focus of scientific exploration, driven by rapid advancements in technology and our understanding of the universe. The discovery of water on Mars has sparked significant interest, raising the question of whether life could exist on the planet. This study proposes a novel approach to simulate and illustrate the detection of life using a proof-of-life module integrated into a Mars rover. The module is an autonomous system capable of traveling to designated regions, excavating soil, collecting samples, and performing biochemical testing onboard the rover itself. The project is inherently multidisciplinary, integrating mechanical systems such as a drill mechanism and a vacuum system, alongside biochemical analysis for soil testing. The module is capable of successfully detecting the presence or absence of living components of life from the collected soil particles. This proof-of-life module serves as a proof-of-concept for autonomous life detection in extraterrestrial environments and lays the foundation for future exploration missions.

</details>

### [57] [AstroTimer: Rethinking Non-Access Stratum Timers in LEO Constellations](https://arxiv.org/abs/2601.17195)
*Arshiya Rezaie Hezaveh,Peng Hu*

**TL;DR**: 提出了针对低地球轨道（LEO）星座动态特性的轻量级自适应计时框架AstroTimer，通过优化非接入层（NAS）计时器显著提升了5G注册效率并降低了用户设备（UE）能耗。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低地球轨道（LEO）星座在5G覆盖扩展中面临的动态拓扑、延迟波动和资源受限等独特挑战，以及现有3GPP非接入层（NAS）定时器机制对LEO环境的适应性不足问题。

Data: 数据来源于仿真模拟（simulation），基于低地球轨道（LEO）特有的链路变化性、处理延迟和网络功能部署等参数构建模型。

Method: 提出了基于低轨卫星（LEO）链路动态性、处理延迟和网络功能部署等参数的轻量级自适应框架AstroTimer，通过闭式解析模型优化5G注册过程中的看门狗定时器（watchdog timer）和退避定时器（backoff timer）。

Result: AstroTimer通过基于低地球轨道（LEO）特有参数（如链路可变性、处理延迟和网络功能部署）的轻量级自适应框架，显著降低了5G注册时间、重试频率和用户设备（UE）能耗，同时避免了信令过载。

Abstract: Low-Earth Orbit (LEO) constellations expand 5G coverage to remote regions but differ fundamentally from terrestrial networks due to rapidly changing topologies, fluctuating delays, and constrained onboard resources. Existing 3GPP Non-Access Stratum (NAS) timers, inherited from terrestrial and geostationary (GEO) or medium Earth orbit (MEO) systems, fail to accommodate these dynamics, leading to signaling storms and inefficiency. This paper introduces AstroTimer, a lightweight, adaptive framework for sizing NAS timers based on LEO-specific parameters such as link variability, processing delays, and network-function placement. AstroTimer derives a closed-form timer model with low computational cost and optimizes both watchdog and backoff timers for the 5G registration procedure. Simulation results demonstrate that AstroTimer significantly reduces registration time, retry frequency, and user equipment (UE) energy consumption compared to 3GPP defaults, while preventing signaling overloads. The proposed approach provides an operator-ready foundation for reliable, efficient, and scalable non-terrestrial 5G/6G deployments.

</details>

<div id='physics.optics'></div>

# physics.optics [[Back]](#toc)

### [58] [Gravitational wave detectors from an experimental perspective](https://arxiv.org/abs/2601.18501)
*Marina Trad-Nery,Margherita Turconi,Walid Chaibi*

**TL;DR**: 该论文以Virgo探测器为框架，系统分析了引力波探测器(gravitational wave detector)的主要噪声源及其耦合机制，通过简化模型计算了接近实际观测的灵敏度曲线(sensitivity curve)。


<details>
  <summary>Details</summary>

Background: 研究聚焦引力波探测器（gravitational wave detector）的基本原理及其高灵敏度实现过程中关键噪声源的识别、耦合机制与抑制策略。

Data: 数据来源于室女座干涉仪(Virgo detector)的噪声贡献计算和简化模型生成的灵敏度曲线。

Method: 采用基于Virgo探测器框架的噪声耦合分析和灵敏度曲线计算方法，通过简化模型量化主要噪声源及其抑制策略。

Result: 引力波探测器（gravitational wave detector）的简化模型考虑了主要噪声源及其耦合机制，计算得出的灵敏度曲线与实际观测结果接近。

Abstract: This chapter introduces the fundamental principles of gravitational wave detectors in a simple and comprehensive manner. Because these instruments aim for extremely high sensitivity, it is essential to understand their various noise sources, how such noise couples to the detector output, and the strategies used to mitigate them. These noises contributions are computed in the frame of the Virgo detector and a sensitivity curve is calculated. Although a simplified layout of a gravitational wave detector is considered, it takes into account the most dominant effects and yields in a sensitivity estimate close to the what is observed in real detectors.

</details>

<div id='stat.ME'></div>

# stat.ME [[Back]](#toc)

### [59] [Learned harmonic mean estimation of the marginal likelihood for multimodal posteriors with flow matching](https://arxiv.org/abs/2601.18683)
*Alicja Polanska,Jason D. McEwen*

**TL;DR**: 提出基于流匹配（flow matching）的连续归一化流（continuous normalizing flows）作为学习调和平均估计器（learned harmonic mean estimator）的内部密度估计架构，有效解决了多模态后验分布（multimodal posteriors）的估计难题，在20维参数空间中验证了该方法的鲁棒性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用流匹配（flow matching）的连续归一化流（continuous normalizing flows）改进学习调和平均估计器（learned harmonic mean estimator）的内部密度估计，以应对高度多峰后验分布（multimodal posteriors）的挑战。

Data: 研究中使用的数据来源为通过任意采样方法获取的后验样本（posterior samples）。

Method: 采用基于流匹配（flow matching）的连续归一化流（continuous normalizing flows）作为学习调和平均估计器（learned harmonic mean estimator）内部密度估计的强大架构。

Result: 基于流匹配（flow matching）的连续归一化流（continuous normalizing flows）架构显著提升了学习调和平均估计器（learned harmonic mean estimator）处理高维多峰后验分布的能力。

Abstract: The marginal likelihood, or Bayesian evidence, is a crucial quantity for Bayesian model comparison but its computation can be challenging for complex models, even in parameters space of moderate dimension. The learned harmonic mean estimator has been shown to provide accurate and robust estimates of the marginal likelihood simply using posterior samples. It is agnostic to the sampling strategy, meaning that the samples can be obtained using any method. This enables marginal likelihood calculation and model comparison with whatever sampling is most suitable for the task. However, the internal density estimators considered previously for the learned harmonic mean can struggle with highly multimodal posteriors. In this work we introduce flow matching-based continuous normalizing flows as a powerful architecture for the internal density estimation of the learned harmonic mean. We demonstrate the ability to handle challenging multimodal posteriors, including an example in 20 parameter dimensions, showcasing the method's ability to handle complex posteriors without the need for fine-tuning or heuristic modifications to the base distribution.

</details>

<div id='astro-ph.SR'></div>

# astro-ph.SR [[Back]](#toc)

### [60] [The r-Process: History, Required Conditions, Astrophysical Sites, and Observations](https://arxiv.org/abs/2601.17246)
*Friedrich-Karl Thielemann,John J. Cowan*

**TL;DR**: 该论文综述了快中子俘获(r-)过程的研究，通过分离慢中子俘获(s-)过程贡献确定太阳系r-过程丰度模式，评估了基于不稳定区核物理的模型重现该模式的尝试，并对比观测与模型预测以识别相关天体物理场所。


<details>
  <summary>Details</summary>

Background: 研究聚焦于快速中子俘获过程（r-process）的核合成机制及其在太阳系丰度模式中的贡献，并探讨其可能的宇宙学场所（astrophysical sites）与银河系化学演化中的观测约束。

Data: 数据来源包括太阳系丰度模式（Solar System r-abundance pattern）、历史独立位点方法（site-independent approaches）的核物理数据、低金属丰度恒星（low-metallicity stars）观测数据以及银河系演化模型（Galactic evolution models）的预测结果。

Method: 采用历史性独立于天体物理场所的方法，基于远离稳定线的核物理数据尝试重现太阳系r过程元素丰度模式。

Result: 综述探讨了快速中子俘获（r-process）过程，重点分析了基于远离稳定线的核物理历史性独立位点方法在重现太阳系r过程丰度模式方面的尝试。

Abstract: This review of the rapid-neutron-capture (i.e. r-) process starts with determining the Solar System r-abundance pattern via first obtaining (and subtracting) the contribution from the slow-neutron capture (s-) process. We emphasize the extensive work in this area by our late colleague Roberto Gallino and continue in an overview, concentrating on attempts to reproduce the solar r-process pattern with historical site-independent approaches, based on nuclear physics far from stability. In a second step we address the existing proposals for astrophysical sites. Among stellar observations we start with available observations of individual events before analyzing low-metallicity stars, which witness r-process contributions in the early Galaxy.We conclude with a comparison of observations and model predictions, focusing on our present ability to identify the responsible individual astrophysical sites by their imprint in Galactic evolution.

</details>

### [61] [JOYS: JWST MIRI/MRS spectra of the inner 500 au region of the L1527 IRS bipolar outflow](https://arxiv.org/abs/2601.17820)
*R. Devaraj,E. F. van Dishoeck,T. P. Ray,Ł. Tychoniec,A. Caratti o Garatti,L. Francis,C. Gieser,M. L. van Gelder,J. J. Tobin,H. Beuther,P. J. Kavanagh,K. Justtanont,W. B. Drechsler,M. G. Navarro,G. Perotti*

**TL;DR**: 利用JWST MIRI/MRS光谱在5-28微米波段对L1527双极外流（bipolar outflow）最内500 au区域的研究发现，该低质量原恒星（Class 0/I protostar）存在分层外流结构，包含一个弱准直高速电离喷流（high velocity ionized jet）嵌入宽角分子外流中，并检测到分子氢（H2）温度分为550 K和2500 K两个组分，以及多种原子和离子禁戒发射线，揭示了不同于早期演化阶段的独特喷流发射特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST中红外仪器（MIRI）光谱观测低质量原恒星（Class 0/I）L1527双极外流的500 au内区物理与运动学特性，揭示其分子、原子与电离发射线特征及分层外流结构。

Data: 数据来源于JWST（詹姆斯·韦伯太空望远镜）的MIRI/MRS（中红外仪器/中分辨率光谱仪）在5-28微米波长范围、0.2-1.0角秒分辨率下获取的光谱观测数据。

Method: 采用JWST中红外仪器（MIRI）的中分辨率光谱仪（MRS）在5-28微米波段以0.2-1.0角秒分辨率进行光谱观测，通过发射线积分强度图分析空间形态，并利用H2转动谱线的激发图解法推导气体温度和柱密度，同时与激波模型进行对比。

Result: 在L1527双极外流的500天文单位（au）内区域发现了分子氢（H2）发射呈现暖（约550 K）和热（约2500 K）组分共存，源于中等速度J型激波（J-type shocks）和部分紫外辐射，并检测到[Ni ii]、[Ar ii]、[Ne ii]、[Ne iii]、[S i]和[Fe ii]等禁线发射，其空间分布显示存在一个准直性差的高速电离喷流（high velocity ionized jet）嵌入更广角的分子外流中，支持分层外流结构且喷流启动特征在早期演化阶段非典型。

Abstract: This study characterizes the physical and kinematic properties within the innermost 500 au region of the L1527 bipolar outflow, a Class 0/I low-mass protostar using JWST MIRI/MRS spectroscopy across 5-28 micron at 0.2-1.0 arcsec resolution. We identify emission lines from molecular and ionized species and analyze their spatial morphology using line integrated intensity maps. We derive gas temperature and column density through excitation diagram analysis of H2 rotational lines and compared results with shock models. The observations reveal extended molecular hydrogen emission tracing the bipolar outflow, with the H2 gas temperatures distributed into warm (~550 K) and hot (~2500 K) components, likely originating from moderate velocity J-type shocks and some UV irradiation. We detect forbidden atomic and ionized emission lines of [Ni ii], [Ar ii], [Ne ii], [Ne iii], [S i], and [Fe ii] showing spatially extended morphology. Double peaked emission profiles were seen in [Ar ii], [Ne iii], and [Fe ii], in the eastern region, suggesting that the high velocity component traces a fast, highly ionized jet. Radial velocity map derived from [Ne ii] emission shows the eastern region to be redshifted and the western region blueshifted, contrary to earlier interpretations. The analysis of the MIRI/MRS observations reveals the presence of molecular, atomic, and ionized emission lines in this low-mass protostar connected with active outflow signatures. The most striking feature discovered is the presence of a poorly collimated high velocity ionized jet, embedded within a broader wide-angle molecular outflow likely driven by a disk wind. The co-existence of these components supports a stratified outflow structure and suggest L1527 exhibits unique jet-launching characteristics atypical for its early evolutionary stage.

</details>

<div id='astro-ph.EP'></div>

# astro-ph.EP [[Back]](#toc)

### [62] [Decameter-sized Earth Impactors -- II: A Bayesian Inference Approach to Meteoroid Ablation Modeling](https://arxiv.org/abs/2601.14371)
*Ian Chow,Peter G. Brown*

**TL;DR**: 基于动态嵌套采样（dynamic nested sampling）的贝叶斯推断方法分析了13颗十米级（decameter-size）地球撞击体的光变曲线，首次将其划分为三类结构差异显著的群体：弱均质体（<1.5 MPa灾难性解体）、渐进碎裂异质体（1-8 MPa）和强聚合体（9-10 MPa保持完整），并揭示其质量损失呈现0.04-0.09 MPa和1-4 MPa两阶段碎裂模式。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过分析火球（fireball）在大气中的烧蚀和碎裂行为来推断小行星和大型流星体（meteoroid）的物理与材料特性。

Data: 数据来源于2022年公布的美国政府部门（USG）卫星传感器记录的数百条新火球光变曲线（light curves）以及7个有独立地基观测验证的USG传感器观测火球案例。

Method: 采用基于动态嵌套采样（dynamic nested sampling）的贝叶斯推断方法，从非信息性平坦先验出发，通过火球光变曲线（light curves）稳健估计天体物理参数。

Result: 发现十米级（decameter-size）撞击体可分为三类结构：第一类是结构均质且脆弱的物体，在低于约1.5兆帕（MPa）时发生灾难性解体；第二类是异质物体，从约1兆帕开始渐进式碎裂直至约3-8兆帕；第三类是坚固聚集体，在9-10兆帕前基本保持完整。

Abstract: Small asteroids and large meteoroids frequently impact the Earth, though their physical and material properties remain poorly understood. When observed as fireballs in Earth's atmosphere, these properties can be inferred from their ablation and fragmentation behavior. The 2022 release of previously classified United States Government (USG) satellite sensor data has provided hundreds of new fireball light curves, allowing for more detailed analysis. Here we present a new Bayesian inference method based on dynamic nested sampling that can robustly estimate these objects' physical parameters from their observed light curves, starting from relatively uninformative, flat priors. We validate our method against seven USG sensor-observed fireballs with independent ground-based observations and demonstrate that our results are consistent with previous estimates. We then apply our technique to $13$ decameter-size Earth impactors to conduct the most detailed population-level study of their structure and material strength to date. We identify three structurally distinct groups within the decameter impactors. The first group are primarily structurally homogeneous, weak objects which catastrophically disrupt below $\sim1.5$ MPa. The second group are heterogeneous objects which progressively fragment starting from $\sim1$ MPa typically up to $\sim3-8$ MPa. The third group are strong aggregates which remain mostly intact until $9-10$ MPa. Our results also suggest that decameter-size asteroids fragment in two distinct phases: an initial phase at $\sim0.04-0.09$ MPa and a second at $\sim1-4$ MPa. While decimeter- to meter-size objects typically lose most of their mass in the initial phase, larger decameter-size objects instead lose most of their mass in the second phase.

</details>

### [63] [Higher-Order Gravitational Models: A Tutorial on Spherical Harmonics and the Newtonian Model](https://arxiv.org/abs/2601.17628)
*Felipe Arenas-Uribe*

**TL;DR**: 论文提出球谐函数重力模型（spherical harmonic gravity models）的系统理论框架，通过求解拉普拉斯方程（Laplace equation）推导高阶引力场项，以精确刻画非均匀质量分布对近地轨道（Low Earth Orbit）卫星和不规则小行星附近航天器轨道动力学的显著扰动效应。


<details>
  <summary>Details</summary>

Background: 研究聚焦于非球形天体（non-spherical celestial bodies）引力场建模对航天器轨道动力学的关键影响。

Data: 数据来源包括低地球轨道（Low Earth Orbit）卫星的实测轨道数据以及不规则形状小行星附近航天器的动力学仿真（simulation）数据。

Method: 采用球谐函数重力模型（spherical harmonic gravity models）作为核心分析方法，通过求解拉普拉斯方程（Laplace equation）推导高阶引力场以系统化表征非均匀质量分布效应。

Result: 非球形天体（non-spherical celestial bodies）的引力场高阶项（higher-order terms）对航天器轨道动力学产生显著扰动，需采用球谐函数模型（spherical harmonic models）精确建模。

Abstract: Accurate modeling of gravitational interactions is fundamental to the analysis, prediction, and control of space systems. While the Newtonian point-mass approximation suffices for many preliminary studies, real celestial bodies exhibit deviations from spherical symmetry, including oblateness, localized mass concentrations, and higher-order shape irregularities. These features can significantly perturb spacecraft trajectories, especially in low-altitude or long-duration missions, leading to cumulative orbit prediction errors and increased control demands. This article presents a tutorial introduction to spherical harmonic gravity models, outlining their theoretical foundations and underlying assumptions. Higher-order gravitational fields are derived as solutions to the Laplace equation, providing a systematic framework to capture the effects of non-uniform mass distributions. The impact of these higher-order terms on orbital dynamics is illustrated through examples involving Low Earth Orbit satellites and spacecraft near irregularly shaped asteroids, highlighting the practical significance of moving beyond the point-mass approximation.

</details>

### [64] [Effect of different Non-Gravitational accelerations on the trajectory of Interstellar Comet 3I/ATLAS](https://arxiv.org/abs/2601.17083)
*Goldy Ahuja,Shashikiran Ganesh*

**TL;DR**: 研究发现基于对称和非对称水(H2O)升华模型计算得出彗星C/2025 N1（3I/ATLAS）的近木点距离，并与早期结果进行了对比。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于星际彗星(interstellar object)C/2025 N1（3I/ATLAS）的非引力加速效应及其对近日点距离的影响。

Data: 数据来源包括NASA JPL Horizons系统提供的非引力参数（基于CO2升华模型）以及使用Find_Orb软件导出的对称与非对称H2O升华模型。

Method: 采用非引力加速度模型（Horizons）结合对称与非对称水冰（H2O）升华模型（Find_Orb软件）计算彗星轨道参数。

Result: 彗星C/2025 N1（3I/ATLAS）在木星附近（0.358天文单位）飞越时，基于对称与非对称水（H2O）升华模型计算的近木点距离与先前结果（历元JD 2460867.5）存在差异。

Abstract: Comet C/2025 N1 or 3I/ATLAS is the third confirmed interstellar object. It has passed perihelion on 2025 October 29, and is currently on a path to leave the solar system. During its outbound journey, it will pass close to Jupiter at a distance of 0.358 au. NASA JPL \textsc{Horizons} has updated the non-gravitational parameters of the comet based on the CO$_2$ sublimation model, where $g(r)= 1/r^2$. In this research note, we use the non-gravitational accelerations from \textsc{Horizons} together with symmetric and asymmetric H$_2$O sublimation models derived using \texttt{Find\_Orb} software. We calculate the resulting perijove distances and compare them with our earlier results at epoch JD 2460867.5.

</details>