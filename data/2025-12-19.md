...
### [1] [From "The Cliff" to "Virgil": Mapping the Spectral Diversity of Little Red Dots with JWST/NIRSpec](https://arxiv.org/abs/2512.15853)
...
**TL;DR**: 通过分析118个高信噪比光谱的“小红点”（LRDs）样本，发现其紫外-光学连续谱和发射线特性与颜色相关，揭示了由黑洞（BH）光度、遮蔽程度和宿主星系与黑洞光度比变化导致的AGN-宿主星系主导发射过渡现象。


<details>
  <summary>Details</summary>

Background: 研究聚焦于詹姆斯·韦伯太空望远镜（JWST）发现的“小红点”（LRDs）天体，这些高红移（$z \gtrsim 3$）致密源具有非常规光谱特征，挑战了标准活动星系核（AGN）模型，可能代表一类早期特殊AGN群体。

Data: 数据来源于六个JWST深场（deep fields）的公开NIRSpec/PRISM光谱观测和综合测光选择。

Method: 采用综合测光选择与公共NIRSpec/PRISM光谱技术，结合双组分模型（气体遮蔽黑洞与星系宿主）分析LRDs的紫外-光学连续谱与发射线特征。

Result: LRDs（Little Red Dots）的紫外-光学连续谱和发射线特征存在显著相关性：较蓝的LRDs具有较蓝的紫外斜率（$β_{ν,\mathrm{UV}} \sim 0.3$）和较红的可见光斜率，而较红的LRDs则表现出较红的紫外斜率（$β_{ν,\mathrm{UV}} \sim 1.1$）。
...
### [2] [Shedding the envelope: JWST reveals a kiloparsec-scale [OIII]-weak Balmer shell around a z=7.64 quasar](https://arxiv.org/abs/2512.15881)
...
**TL;DR**: 利用JWST/NIRSpec IFU观测红移(redshift) z=7.64的最遥远类星体J0313-1806，发现其周围存在缺乏[O III]发射的薄层电离气体壳（延伸至1.8千秒差距(kpc)），证实了早期类星体通过致密气体碰撞退激发产生间歇性反馈循环的证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（z>7）明亮类星体（quasar）中超大质量黑洞（SMBH）与宿主星系共生演化的物理条件及早期反馈机制。

Data: 数据来源于JWST（James Webb Space Telescope）的NIRSpec（Near-Infrared Spectrograph）IFU（Integral Field Unit）对红移z=7.64的类星体J0313-1806的观测。

Method: 采用JWST/NIRSpec IFU（积分视场单元）观测和光致电离模型（photoionization modelling）分析J0313-1806的宽线区（broad-line region）至星际介质（ISM）和环星系介质（CGM）的光谱特性。

Result: 在红移z=7.64的类星体J0313-1806中发现了一个延伸至1.8千秒差距(kpc)的电离气体壳层（ionized gas shell），其Hβ发射缺乏显著[O III]特征，表明存在由致密气体碰撞退激（collisionally de-excited）的薄而团块状外流壳层（outflowing shell），这被解释为近期喷发阶段（blowout phase）的化石遗迹，为最早类星体中的间歇性反馈循环（episodic feedback cycles）提供了证据。
...
### [3] [Cored galaxies in cuspy dark matter halos](https://arxiv.org/abs/2512.15886)
...
**TL;DR**: 研究发现弱三维核心（weak 3D cores）恒星密度轮廓可与尖峰暗物质晕（cuspy dark matter halos）共存，但强三维核心（strong 3D cores）则不能，且二者在投影观测中难以区分，表明仅凭光度数据无法排除暗物质尖峰的存在。


<details>
  <summary>Details</summary>

Background: 研究探讨了矮球状星系和超暗弱矮星系的光度观测数据对恒星内部密度轮廓的限制条件，聚焦于核心恒星轮廓（cored stellar profiles）与冷暗物质晕（cold dark matter halos）预期尖峰轮廓（cuspy profiles）偏离的关联性。

Data: 数据来源为矮球状星系和超暗弱矮星系的测光数据（photometric data），包括银河系伴星系中研究最充分的矮球状星系之一Fornax。

Method: 采用球对称恒星密度剖面（spherically symmetric stellar profiles）分类方法，依据中心斜率（$b_0$）和对数斜率（$γ_0$）行为将恒星剖面划分为“强/弱核（strong/weak cores）”和“尖峰（cusps）”，并分别在各向同性与各向异性运动学条件下检验尖峰暗物质晕（cuspy dark matter halos）中嵌入不同恒星剖面时分布函数的物理合理性。

Result: 弱三维核心（weak 3D cores）恒星密度分布可与尖峰暗物质晕（cuspy dark matter halos）共存，但强三维核心（strong 3D cores）则无法与之兼容。
...
### [4] [The Next Generation Fornax Survey (NGFS).VIII. A Support Vector Machine Approach for Disentangling Globular Clusters from other Sources](https://arxiv.org/abs/2512.15895)
...
**TL;DR**: 提出了一种基于颜色-颜色图和形态学参数的监督分类方法（SVM），利用近紫外/光学/近红外多波段数据在下一代巡天（如LSST、Euclid和Roman）中高效分离球状星团（GCs）、恒星和星系，其中包含近红外波段的全15特征模型分类准确率达97.3%，证明近紫外和近红外波段对提升GC样本纯度至关重要。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用宽视场多波段巡天数据解决邻近星系团中球状星团（GCs）与前景恒星和背景星系的高效分类难题，以支持星系组装过程的精确约束。

Data: 数据来源于下一代Fornax巡天（NGFS）对Fornax星系团中心3平方度区域在u'g'i'JKs波段的深度成像观测。

Method: 采用支持向量机（SVM；svm.SVC，scikit-learn）基于15个特征（所有颜色组合和基本形态参数）进行监督分类，并通过光谱确认源定义训练类别。

Result: 基于颜色-颜色图和形态学参数的支持向量机(SVM)方法在15个特征下实现了97.3%的分类准确率，其中包含近紫外(u')和近红外(NIR)波段对球状星团(GC)分类至关重要。
...
### [5] [Dust evolution across cosmic times as seen through DUSTY-GAEA](https://arxiv.org/abs/2512.15902)
*Omima Osman,Gabriella De Lucia,Fabio Fontanot,Lizhi Xie,Michaela Hirschmann*

**TL;DR**: 在GAEA星系形成模型中引入DUSTY-GAEA自洽尘埃模型，发现尘埃在稠密星际介质(ISM)中的生长主导了红移(z)~8之前的宇宙尘埃总量，并成功再现了红移(z)~6的尘埃累积、局域宇宙的尘埃-气体/尘埃-金属比例标度关系以及红移(z)~1的尘埃质量函数。


<details>
  <summary>Details</summary>

Background: 尘埃在星系形成和演化中扮演关键角色，其来源、破坏及增长机制是理解星系化学演化的核心问题。

Data: 数据来源于GAEA星系形成模型（galaxy formation model）的自洽模拟实现（self-consistent implementation），包括恒星形成尘埃、超新星激波和热气体破坏尘埃以及致密星际介质（dense ISM）中尘埃增长的模拟数据。

Method: 采用自洽方法模拟恒星形成尘埃、超新星激波和热气体破坏尘埃、以及致密星际介质（ISM）中尘埃增长过程，并整合到GAEA星系形成模型中。

Result: DUSTY-GAEA模型表明星际介质（ISM）中的尘埃增长主导了红移（redshift）z ∼ 8之前的宇宙尘埃总量，且高效尘埃增长是解释高红移观测约束的必要条件。
...
### [6] [The Era of Binary Supermassive Black Holes: Coordination of Nanohertz-Frequency Gravitational-Wave Follow-up](https://arxiv.org/abs/2512.15911)
...
**TL;DR**: 该会议总结了纳赫兹引力波探测中双超大质量黑洞（binary supermassive black holes）的多信使追踪策略，提出了知识缺口与未来路线图。


<details>
  <summary>Details</summary>

Background: 研究聚焦于协调纳赫兹引力波探测与电磁波多信使观测，以推动超大质量双黑洞（binary supermassive black holes）的联合研究。

Data: 数据来源包括脉冲星计时阵列（pulsar timing arrays）的观测数据以及相关电磁波段的观测数据。

Method: 采用多信使（multi-messenger）协同分析框架整合脉冲星计时阵列（pulsar timing arrays）与电磁波段观测数据。

Result: 会议提出了协调纳赫兹引力波（nanohertz-frequency gravitational-wave）后续观测的多信使策略开发指南，并总结了知识缺口及电磁对应体（electromagnetic counterparts）搜索的潜在路线图。
...
### [7] [MAGellanic Outflow and chemistry Survey (MAGOS): Hot cores in the LMC](https://arxiv.org/abs/2512.15983)
*Takashi Shimonishi,Kei E. I. Tanaka,Yichen Zhang,Kenji Furuya,Yu Cheng,Asako Sato*

**TL;DR**: 该研究通过ALMA对LMC（Large Magellanic Cloud）中30个大质量原恒星体的热核（hot core）观测，首次在LMC棒状结构外区域探测到复杂有机分子CH3OCH3，发现所有热核在高激发SO谱线发射更强，揭示了CH3OH丰度存在两个数量级差异且部分源缺乏COMs（complex organic molecules），而SO2在所有热核中均被检测且其丰度与旋转温度呈正相关，表明低金属丰度、周边活跃恒星形成和高原恒星光度共同导致了LMC中COM贫乏的热核化学特征。


<details>
  <summary>Details</summary>

Background: 探索大麦哲伦云（LMC）在亚太阳金属丰度条件下恒星形成和星际化学多样性的关键实验室。

Data: 数据来源于使用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)在350 GHz对大麦哲伦云(LMC)中30个大质量原恒星天体进行的热核(hot core)巡测观测。

Method: 采用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)在350 GHz频段对30个大麦哲伦云(LMC)中的大质量原恒星天体进行热核(hot core)巡测，通过连续谱成像和分子谱线分析识别热核候选体，并检测多种分子发射线（包括CO、HCO+、HC3N、SO2、CH3OH等）进行化学丰度与激发条件的相关性研究。

Result: 在低金属丰度条件下，大麦哲伦云(LMC)中观测到的热核(hot core)化学特征表现为复杂有机分子(COMs)丰度差异显著且部分源缺乏COMs，而SO2在所有热核中均被检测且其丰度与旋转温度呈正相关。
...
### [8] [ASKAP discovery of a 30 kpc bipolar outflow from the edge-on disk of the nearby spiral galaxy ESO 130-G012](https://arxiv.org/abs/2512.15991)
...
**TL;DR**: 在近缘侧向星系ESO 130-G012中发现了一个源自恒星形成盘、延伸至少30千秒差距的空心沙漏状大规模亮边外流（outflow），其高度异常且与星系较低引力势能相关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低质量星系ESO 130-G012中发现的超大尺度沙漏状外流（outflow）现象及其与恒星形成活动的关联。

Data: 研究中使用的数据来源于澳大利亚平方公里阵列探路者（ASKAP）在944 MHz射电连续谱波段获取的"宇宙演化图"（EMU）项目观测图像。

Method: 采用澳大利亚平方公里阵列探路者(ASKAP)的944 MHz射电连续谱成像技术，结合"宇宙演化图"(EMU)项目数据，发现了ESO 130-G012星系的双极(bipolar)空心钟形(hourglass-shaped)外流结构。

Result: 在边缘星系ESO 130-G012中发现了一个尺度至少达30千秒差距(kpc)、具有边缘增亮特征的空心沙漏状外流(outflow)，其高度是恒星盘直径(~10 kpc)的3倍，且膨胀速度接近逃逸速度。
...
### [9] [LSTM-MDNz: Estimating Quasar Photometric Redshifts with an LSTM-Augmented Mixture Density Network](https://arxiv.org/abs/2512.16010)
*Jianzhen Chen,Zhijian Luo,Liping Fu,Zhu Chen,Hubing Xiao,Shaohua Zhang,Chenggang Shu*

**TL;DR**: 提出LSTM-MDNz深度学习模型，通过融合14个紫外至红外波段的多波段测光数据（SDSS、DESI-LS、WISE、GALEX），将类星体（quasar）测光红移预测的归一化中位绝对偏差（$σ_{\mathrm{NMAD}}$）降至0.037、异常率（$f_{\mathrm{out}}$）降至3.5%，显著缓解红移-颜色简并问题（redshift-color degeneracy）。


<details>
  <summary>Details</summary>

Background: 类星体（quasar）测光红移研究对于宇宙学和大尺度结构研究至关重要。

Data: 数据来源整合了四大巡天项目-SDSS、DESI-LS、WISE和GALEX-的14个紫外至红外波段数据，构建了包含55万多个光谱确认类星体（$0 \leq z_{\mathrm{spec}} \leq 5$）的训练测试样本。

Method: 采用长短期记忆网络（LSTM）与混合密度网络（MDN）相结合的端到端深度学习模型LSTM-MDNz，直接输入多波段测光通量及其误差作为波长有序序列，无需人工特征工程即可同步实现类星体红移点估计和概率分布函数（PDF）预测。

Result: LSTM-MDNz模型通过结合长短期记忆网络(LSTM)和混合密度网络(MDN)，利用14个紫外至红外波段的光度数据，将类星体(quasar)红移预测的归一化中位绝对偏差($σ_{\mathrm{NMAD}}$)降至0.037，异常值率($f_{\mathrm{out}}$)降至3.5%，较传统SDSS+WISE波段组合分别降低29%和56%。
...
### [10] [Astrometric and Spectroscopic Analysis of IC 2714: An Open Cluster Hosting a Lithium-Rich Giant](https://arxiv.org/abs/2512.16042)
...
**TL;DR**: 通过对银河系星团IC 2714中8颗红巨星的详细化学分析，首次提供了该星团23种元素丰度及7颗星碳同位素比（12C/13C）的全面表征，发现其中一颗（#087）具有高锂丰度（log ε(Li)NLTE=+1.54 dex）和较快自转（v sin i=6.7 km s−1），并确认两颗（#028和#034）可能非星团成员。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用银河系疏散星团（open clusters）作为实验室，探讨恒星演化及银河系化学演化模型，尤其关注其中锂丰度异常红巨星（lithium-rich giants）等化学特征。

Data: 数据来源包括高分辨率光谱观测和盖亚(Gaia) DR3星表数据。

Method: 采用高分辨率光谱分析（high-resolution spectroscopy）结合盖亚（Gaia）DR3星表数据，对IC 2714星团8颗红巨星进行23种元素丰度测定，并首次获取7颗恒星的碳同位素比（12C/13C）。

Result: 在银河系星团IC 2714的八颗红巨星中，发现恒星#087具有显著高的锂丰度（非局部热动平衡下log ε(Li) = +1.54 dex）和略高的投影自转速度（v sin i = 6.7 km s⁻¹），而恒星#028和#034可能不属于该星团成员。
...
### [11] [Gas Accretion from a Neighbouring Galaxy Fuels the Low-luminosity AGN in NGC 4278](https://arxiv.org/abs/2512.16049)
*Jin-Long Xu,Nai-Ping Yu,Ming Zhu,Chuan-Peng Zhang,Xiao-Lan Liu,Mei Ai,Peng Jiang*

**TL;DR**: 研究发现邻近星系的潮汐作用导致外部气体吸积（gas accretion）形成旋转气体盘（rotating gas disk），为椭圆星系NGC 4278的低光度活动星系核（low-luminosity AGN）提供燃料并可能触发相对论性喷流（relativistic jets）及TeV伽马射线辐射。


<details>
  <summary>Details</summary>

Background: 研究探讨了看似“死亡”的宿主星系如何通过外部气体吸积（external gas accretion）为活动星系核（AGN）提供燃料这一未解之谜。

Data: 数据来源于五百米口径球面射电望远镜（FAST）对邻近椭圆星系NGC 4278及其周边区域的高灵敏度原子氢（HI）观测。

Method: 采用五百米口径球面射电望远镜(FAST)对NGC 4278及其邻近区域进行高灵敏度中性氢(HI)观测，结合潮汐相互作用模型分析外部气体吸积过程。

Result: 邻近星系的潮汐相互作用导致外部气体吸积，为NGC 4278中的低光度活动星系核（AGN）提供燃料。
...
### [12] [Exploring the magnetic field structure of the Milky Way with pulsars in the SKA era](https://arxiv.org/abs/2512.16159)
*Jun Xu,J. L. Han,Weicong Jing,The SKA Pulsar Science Working Group*

**TL;DR**: 该论文提出利用脉冲星（pulsars）的偏振观测研究银河系（Milky Way）磁场三维结构，并指出SKA1望远镜阵列（AA4和AA*）将大幅增加已知脉冲星数量和视线方向旋转测量（rotation measures），从而实现对银河系盘（Galactic disk）和晕（Galactic halo）磁场结构的精细探测。


<details>
  <summary>Details</summary>

Background: 银河系磁场结构研究为理解星系磁场起源提供了关键线索。

Data: 数据来源为利用SKA1设计基线（AA4）和分阶段交付阵列（AA*）对脉冲星（pulsar）进行的偏振观测，获取数千条视线方向的旋转量（rotation measure）。

Method: 利用脉冲星（pulsars）的偏振观测测量旋转量（rotation measure），结合SKA1（AA4和AA*）望远镜的高灵敏度探测能力，分析银河系磁场三维结构。

Result: 脉冲星（pulsar）是研究银河系三维磁场结构的最佳探针，因其高度偏振的短时射电脉冲、可忽略的本征法拉第旋转效应及广泛分布于银河系薄盘、旋臂和延展晕（halo）的特性。
...
### [13] [Unveiling the properties and origin of massive quenched galaxies at $z\ge2$ in the COLIBRE hydrodynamical simulations](https://arxiv.org/abs/2512.16208)
...
**TL;DR**: JWST在早期宇宙（红移z≳2）中发现的大量大质量（M⋆≳10¹⁰M⊙）静止星系（MQGs）挑战了现有星系形成模型，而基于COLIBRE宇宙流体动力学模拟的研究表明这些星系具有比活跃恒星形成星系低1个数量级的尘埃和H₂含量，其静止化主要源于活动星系核（AGN）反馈作用，且55%的z=3 MQGs能作为主 progenitor 延续至z=0。


<details>
  <summary>Details</summary>

Background: 研究聚焦于早期宇宙（红移z≳2）中JWST发现的大质量（M⋆≳10¹⁰M⊙）静止星系（MQGs）的成因及其对现有星系形成模型的挑战。

Data: 数据来源为COLIBRE宇宙学流体动力学模拟（cosmological hydrodynamical simulations）。

Method: 采用COLIBRE宇宙学流体动力学模拟（COLIBRE cosmological hydrodynamical simulations）结合活动星系核（AGN）反馈机制作为主要淬灭（quenching）驱动因素的分析方法。

Result: JWST在早期宇宙（红移z≳2）中发现的大量大质量（M⋆≳10¹⁰M⊙）静止星系（MQGs）挑战了现有星系形成模型。
...
### [14] [ALMA and JWST Identification of Faint Dusty Star-Forming Galaxies up to z~8](https://arxiv.org/abs/2512.16215)
...
**TL;DR**: 基于ALMA CHAMPS大项目和JWST多波段数据，提出利用新参数I_star（恒星质量对数与恒星形成率对数的乘积）结合F277W-F444W颜色筛选尘埃遮蔽恒星形成星系（DSFG）的方法，并发现了一批此前被遗漏的高红移（z=6-8）微弱DSFG样本，其平均1.2毫米流量密度为0.15微焦耳/秒、数密度约6×10^-6 Mpc^-3，推测可能与z>10紫外亮星系及z=3-5大质量静止星系存在演化关联。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用多波段观测数据开发新方法识别尘埃遮蔽的恒星形成星系（DSFGs），并探索高红移（z=6-8）暗弱尘埃星系与早期宇宙星系（z>10）及大质量静止星系（z=3-5）之间可能的演化联系。

Data: 数据来源于ALMA CHAMPS大型项目提供的约400个明亮尘埃星系样本及COSMOS区域丰富的JWST多波段数据产品。

Method: 基于新定义的参数I_star（I_star = log(M_star) x log(SFR)）并结合F277W-F444W颜色作为第二参数，建立了一种有效的经验选择标准来识别尘埃恒星形成星系（DSFGs）。

Result: 基于新定义的参数I_star（I_star = log(M_star) x log(SFR)）并结合F277W-F444W颜色作为第二参数，建立了一种有效筛选尘埃恒星形成星系（DSFGs）的实证标准，并通过叠加技术在COSMOS2025星表中发现了一批红移z=6-8、平均流量密度S_1.2mm=0.15uJy、空间密度约6E-6 Mpc^-3的暗弱DSFGs。
...
**TL;DR**: 研究发现暗晕质量（halo mass）是中心星系（central galaxies）恒星形成淬灭（quenching）的主导因素，存在$M_{h}\sim 10^{12.1}M_\odot$的明确阈值，且此前研究中中央属性（central properties）的主导性结论源于丰度匹配（abundance matching）的系统性偏差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于揭示中央星系恒星形成停止（quenching）的主导机制，核心科学背景是探讨暗物质晕质量（halo mass）与星系中心特性（如速度弥散、核球质量、黑洞质量）在星系演化中的相对作用争议。

Data: 数据来源于SDSS（Sloan Digital Sky Survey）巡天的星系群数据，并重新估计了晕质量（halo mass），同时结合弱引力透镜（weak lensing）测量得到的恒星-晕质量关系（SHMRs）进行校正。

Method: 采用改进的弱引力透镜(weak lensing)和卫星运动学(satellite kinematics)校准的晕质量函数(halo mass function)及恒星-晕质量关系(SHMRs)，重新估算SDSS星系群晕质量以消除丰度匹配(AM)的系统偏差。

Result: 暗物质晕质量（halo mass）是中心星系恒星形成淬灭的主导因素，存在明确阈值（$M_{h}\sim 10^{12.1}M_\odot$）。
...
### [16] [The AGN nature of strong CIII emitters in the Early Universe with JWST](https://arxiv.org/abs/2512.16365)
...
**TL;DR**: 基于JWST/NIRSpec棱镜数据对红移5-7星系的CIII]发射线研究发现，高红移星系中存在比红移3-4更强（中值等效宽度22.8埃）的CIII]发射线群体，其中29个星系具有活动星系核(AGN)特征，并首次报道了一个新的小红点(LRD)源。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用半禁戒CIII]λλ1907,1909双线作为高电离辐射示踪剂，探索早期宇宙（红移z=5-7）星系中恒星形成与活动星系核(AGN)的物理特征及其演化规律。

Data: 数据来源于公开的JWST/NIRSpec棱镜观测数据，包括CEERS、JADES、RUBIES和CAPERS等项目。

Method: 采用基于静止紫外波段CIII]/HeII λ1640和CIV/HeII λ1640等效宽度(EW)的诊断图以及光学波段[OIII] λ5007、Hβ、[NeIII] λ3869和[OII] λλ3727,3729的OHNO图进行星系分类，并结合宽巴尔末发射线特征区分恒星形成星系与活动星系核(AGN)。

Result: 在5<z<7的高红移星系中，半禁线CIII]发射体（semi-forbidden CIII] emitters）的中值等效宽度（EW）为22.8埃，显著高于红移3<z<4对照样本的4.7埃，且相同Muv范围内CIII] EW从3<z<4到5<z<7增加了约0.67 dex，表明该谱线强度存在显著的红移演化。
...
### [17] [MUSE Analysis of Gas around Galaxies (MAGG) - VII. Emission line galaxies near strong blended Ly$α$ absorption systems at $z\gtrsim3$](https://arxiv.org/abs/2512.16422)
*Marta Galbiati,Davide Tornotti,Michele Fumagalli,Matteo Fossati,Matthew Pieri*

**TL;DR**: 研究发现强混合莱曼α吸收系统（SBLAs）与红移z≳3的莱曼α发射星系（LAEs）在投影距离R≤300kpc和视线速度差|Δv|≤300km/s范围内存在显著关联，且关联率随吸收通量降低而增强，证实了SBLAs可作为研究星系周介质（CGM）的探针。


<details>
  <summary>Details</summary>

Background: 研究探讨了强混合莱曼α吸收系统（SBLAs）与高红移（z≳3）莱曼α发射星系（LAEs）之间的关联性及其在星系周介质（CGM）研究中的作用。

Data: 数据来源于MUSE Analysis of Gas around Galaxies (MAGG)巡天项目中28个类星体（quasar）视场的光谱观测数据，包括强混合莱曼α吸收系统（SBLAs）和约1000个红移z≳3的莱曼α发射星系（LAEs）。

Method: 采用二维互相关分析（two-dimensional cross-correlation analysis）研究强混合莱曼α吸收系统（SBLAs）与莱曼α发射星系（LAEs）的空间关联性，并通过调整光谱窗口宽度（spectral window width）验证信号强度变化。

Result: 强混合莱曼α吸收系统（SBLAs）与红移z≳3的莱曼α发射星系（LAEs）在300千秒差距投影距离和300公里/秒视线速度差内存在显著关联，且关联率随吸收通量降低而增强。
...
### [18] [Galaxy evolution in compact groups - III. Structural analysis of galaxies and dynamical state of non-isolated compact groups](https://arxiv.org/abs/2512.16542)
...
**TL;DR**: 研究发现非孤立致密星系群（non-isolated CGs）中星系（尤其质量$\log(M/M_\odot) > 10.2$的早型星系ETG）的淬灭比例和结构参数波长依赖性显著高于周围星系群，且过渡型星系呈现独特形态转化特征，表明致密构型在星系演化中具有独立于大尺度环境的关键作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于非孤立致密星系群（non-isolated CGs）在更大质量系统（如星系群和星系团）中的演化特征及其与宿主结构内星系的差异。

Data: 使用多波段S-PLUS巡天数据分析了122个嵌入更大质量系统（如更大星系群和星系团）的非孤立致密星系群（non-isolated CGs）中的星系。

Method: 采用多波段S-PLUS数据和MorphoPLUS流程进行多波长Sérsic轮廓拟合，获取星系结构参数并划分形态类型（早型ETG、过渡型、晚型LTG）。

Result: 非孤立致密星系群（non-isolated CGs）中的星系比周围群星系具有更高的淬灭比例和更多早型星系（ETG），尤其在恒星质量$\log(M/M_\odot) > 10.2$时更显著。
...
### [19] [N-emitters as possible sign-posts of GC formation](https://arxiv.org/abs/2512.16549)
*D. Schaerer,R. Marques-Chaves,H. Atek,N. Prantzos,C. Charbonnel,M. Talia,I. Morel,M. Dessauges-Zavadsky*

**TL;DR**: 研究发现氮发射体（N-emitters）可能是球状星团（GC）形成位置的标志，其预测的GC年龄分布和恒星质量密度与观测结果一致。


<details>
  <summary>Details</summary>

Background: 研究探讨了氮发射体(N-emitters)作为球状星团(GC)形成位点标志的可能性及其与宇宙恒星形成率密度演化的关联。

Data: 数据来源于对N发射体（N-emitters）的化学丰度比、致密性、高星际介质（ISM）密度等特性的观测结果，以及近期对N发射体群体频率$f_N$和红移演化的统计研究（Morel et al. 2025）。

Method: 采用统计分析和宇宙恒星形成率密度演化模型，结合N发射体（N-emitters）的观测频率fN及其红移演化数据，预测球状星团（GCs）的宇宙学演化、年龄分布及总恒星质量密度。

Result: N-emitters（氮发射体）的化学丰度比、致密性及高星际介质密度等特征与球状星团（GC）恒星相似，支持其可能是球状星团形成位点的假说。
...
### [20] [JWST MIRI color classification of mid-infrared selected galaxies: MIRI color classification toward cosmic noon](https://arxiv.org/abs/2512.16589)
...
**TL;DR**: 基于斯皮策（Spitzer）中红外（MIR）光谱数据库开发了詹姆斯·韦伯太空望远镜（JWST）中红外仪器（MIRI）的七红移窗口（z=0.25-2.10）颜色分类工具，通过多环芳烃（PAH）发射和硅酸盐（silicate）吸收特征区分活动星系核（AGN）、恒星形成星系（SFGs）和硅吸收主导星系，并应用于SMILES巡天数据验证其有效性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用詹姆斯·韦伯太空望远镜（JWST）的中红外（MIR）测光颜色分类方法，探索宇宙正午时期高红移星系的中红外光谱特征及其天体物理性质。

Data: 数据来源于斯皮策（Spitzer）中红外（MIR）光谱数据库，用于在詹姆斯·韦伯太空望远镜（JWST）中红外仪器（MIRI）的宽带滤光片中获取合成测光数据。

Method: 基于斯皮策(Spitzer)中红外光谱数据库合成JWST/MIRI滤波器测光数据，构建了追踪多环芳烃(PAH)发射特征和9.7微米硅酸盐吸收特征的七红移窗口滤波器组合方法。

Result: 基于JWST/MIRI滤光片组合构建了红移(z)依赖的中红外(MIR)颜色分类工具，有效区分了活动星系核(AGN)、恒星形成星系(SFGs)和硅酸盐吸收主导星系至z~2。
...
**TL;DR**: 首次在红移(z=1.8)的3C 294沙漏形态X射线周围探测到共位(cosmological)射电辐射，证实其由同步辐射和逆康普顿散射产生，揭示了高红移射电瓣(lobes)中极低电子洛伦兹因子(γ_max≪10^4, γ_break≲10^3)的关键特性，表明该系统是致密原星系团(protocluster)环境中已知最强的射电噪(radio-loud)活动星系核(AGN)之一，并通过频谱分析发现内热斑(hotspots)由重启活动导致，而外热斑仍保持活跃。


<details>
  <summary>Details</summary>

Background: 研究主题为高红移（z=1.8）射电噪活动星系核（AGN）3C 294周围特殊沙漏形态X射线与射电辐射的物理机制及其在致密原星系团环境中的能量学特征。

Data: 数据来源于国际低频阵列（LOFAR）在144 MHz的观测数据、钱德拉（Chandra）在0.3-6 keV的X射线数据以及最高达15 GHz的存档射电数据。

Method: method：利用国际低频阵列（LOFAR）144 MHz和钱德拉（Chandra）0.3-6 keV数据，通过同步辐射和逆康普顿过程建模分析共位弥散射电与X射线辐射，并结合最高15 GHz的射电档案数据进行分辨光谱分析。

Result: 首次在红移z=1.8的3C 294周围探测到具有沙漏形态的X射线共空间弥散射电辐射，其辐射机制由同一电子群体的同步辐射和逆康普顿散射共同解释。
...
### [22] [On the Origin of Kinematic Structure in the Young Association Serpens OB2](https://arxiv.org/abs/2512.16708)
*Michael A. Kuhn,Robert A. Benjamin,Simran S. Singh*

**TL;DR**: 研究发现蛇夫座OB2星协（Serpens OB2 association）呈现多尺度膨胀特征，其成员星（包括OB星和中低质量年轻恒星体YSOs）空间分布与运动学模式表明，恒星反馈驱动的垂直速度梯度（0.10±0.02 km/s/pc）与小尺度速度相干性、大尺度发散运动共同反映了湍流分子云初始速度场与持续恒星形成活动的动态耦合。


<details>
  <summary>Details</summary>

Background: 研究聚焦于蛇夫座OB2星协（Serpens OB2 association）这一银河系盘面上方约65 pc的大型恒星形成区，探讨其内部恒星分布、动力学特征及与分子云相互作用的演化过程。

Data: 数据来源包括SPICY星表中的OB星和低-中质量年轻恒星天体（YSOs）、13CO MWISP数据中探测到的分子云团块以及盖亚（Gaia）DR3自行运动数据。

Method: 基于盖亚卫星(Gaia) DR3自行数据研究内部运动学特征，包括小尺度(<2 pc)速度对齐、两点速度统计显示随距离增大速度差异增加且以发散运动为主，以及星团间速度差异。

Result: Serpens OB2星协（距离银道面约65 pc）表现出垂直于银道面的全局扩张（空间梯度为0.10±0.02 km/s/pc），其内部运动学特征包括小尺度（<2 pc）速度对齐、大尺度分离时速度差异增大且以发散运动为主，以及星团间速度差异，这些特征与继承自湍流分子云并受恒星反馈影响的初始速度场一致。
...
### [23] [A faint M$_{\rm UV} = -14.5$ Lyman continuum leaker in the reionization epoch: unprecedented Ly$α$ properties at z=5.725](https://arxiv.org/abs/2512.16728)
...
**TL;DR**: 首次在再电离时期（reionization epoch）发现了一个极贫金属（extremely metal-poor）、低质量、超致密的矮星系AMORE6，其莱曼α（Lyα）发射线具有前所未有的窄对称特征和极小速度偏移，表明中性氢柱密度低且可能存在电离光子逃逸，成为再电离时期最具潜力的莱曼连续谱（LyC）泄漏候选体之一。


<details>
  <summary>Details</summary>

Background: 研究主题为探索高红移(z=5.7253)极端贫金属(12+log(O/H)<6)、低质量(4.4×10^5 M⊙)超致密矮星系AMORE6的莱曼α(Lyα)发射线特性及其对再电离时期电离光子逃逸(LyC-leaking)的指示意义。

Data: 数据来源于哈勃空间望远镜（HST）的宽场相机3（WFC3）和詹姆斯·韦伯空间望远镜（JWST）的NIRCam无狭缝光谱观测，以及A2744星系团的引力透镜效应放大目标星系信号。

Method: 采用莱曼阿尔法(Lyα)发射线轮廓分析和辐射传输效应评估，结合哈勃空间望远镜(HST)和詹姆斯·韦伯太空望远镜(JWST/NIRCam)的无缝光谱观测数据。

Result: AMORE6展现出莱曼α（Lyα）发射线近乎对称且窄的轮廓特征，表明其辐射传输效应极小，中性氢柱密度与光学薄介质一致，暗示可能存在非零的电离光子逃逸分数。
...
### [24] [Connecting current and future dual AGN searches to LISA and PTA gravitational wave detections](https://arxiv.org/abs/2512.16844)
...
**TL;DR**: 基于ASTRID宇宙学模拟构建的双活动星系核(DAGN)目录揭示了当前观测中5千秒差距以下双星系的缺失源于选择效应而非物理匮乏，证实双活动星系核能有效追踪大质量黑洞(MBH)并合过程（30-70%在10亿年内并合），其中20-60%的并合事件可被LISA探测到，且AXIS和Roman可观测的双星系是10%低红移LISA事件和30%PTA波段随机背景的前身源，同时明确了大质量绿谷星系和中光度活动星系核以及红移大于1的恒星形成星系中的明亮类星体是最可能孕育即将并合的大质量黑洞双星的环境。


<details>
  <summary>Details</summary>

Background: 双活动星系核（DAGN）标志着星系并合中大质量黑洞（MBH）配对的观测阶段，是产生低频引力波的MBH双星前身。

Data: 数据来源于大体积宇宙学模拟ASTRID（Astrophysical Simulations of the Triply Imagined Domain），并构建了与当前（COSMOS-Web、DESI）及未来（AXIS、Roman）搜索相匹配的双活动星系核(DAGN)目录。

Method: 采用大体积宇宙学模拟(ASTRID)构建双活动星系核(DAGN)目录，并应用真实选择函数匹配当前及未来观测搜索。

Result: 预测了大量当前观测未能捕捉到的小间距（<5千秒差距）双活动星系核（DAGN），表明COSMOS-Web中亚千秒差距系统的明显稀缺主要由选择效应而非物理缺失驱动。
...
### [25] [The Simons Observatory: forecasted constraints on primordial gravitational waves with the expanded array of Small Aperture Telescopes](https://arxiv.org/abs/2512.15833)
...
**TL;DR**: 通过增加西蒙斯天文台（SO）的小孔径望远镜（SAT）数量、延长观测时间并优化前景和噪声模型，将张量标量比（tensor-to-scalar ratio）$r$的1σ测量精度提升至$1.2\times10^{-3}$（或更优条件下$7\times10^{-4}$），显著增强了对宇宙微波背景偏振中原始$B$模信号的探测能力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过升级西蒙斯天文台（Simons Observatory）的小孔径望远镜阵列和延长观测时间，提升对宇宙微波背景辐射偏振中B模信号的探测能力，以探索极早期宇宙产生的引力波特征。

Data: 数据来源为西蒙斯天文台（Simons Observatory）的小孔径望远镜（Small Aperture Telescope, SAT）在93 GHz和145 GHz频道的48,160个探测器采集的宇宙微波背景（cosmic microwave background, CMB）极化数据，并结合大孔径望远镜（Large Aperture Telescope, LAT）的高分辨率观测及重叠的大尺度结构巡天数据。

Method: 采用1/f噪声模型（knee multipole ℓ_knee=50）结合中等复杂度的银河系前景模型进行预测分析。

Result: 西蒙斯天文台（Simons Observatory）通过将小型孔径望远镜（SAT）数量从三台增至六台、探测器数量翻倍至48160个，并将观测时间延长至2035年，显著提升了探测宇宙微波背景偏振中B模信号（B-mode signal）的能力，预计在无原初B模信号情况下对张标比（tensor-to-scalar ratio）$r$的1σ约束可达$σ_r = 1.2\times10^{-3}$（假设70%的透镜化B模信号被移除）。
...
### [26] [The AIDA-TNG project: 3D halo shapes](https://arxiv.org/abs/2512.15856)
*C. Giocoli,G. Despali,L. Moscardini,M. Meneghetti,R. K. Sheth,A. Pillepich,M. Vogelsberger*

**TL;DR**: 通过高分辨率宇宙学模拟（AIDA-TNG）比较冷暗物质（CDM）、自相互作用暗物质（SIDM）和温暗物质（WDM）模型，发现SIDM导致更圆且各向同性的暗物质晕（尤其内区），WDM晕比CDM更球状且浓度更低，而重子物理使所有模型中心区域变圆但仍保持模型间差异。


<details>
  <summary>Details</summary>

Background: 研究主题为利用暗物质晕（dark matter halo）的三维形状约束暗物质基本性质，重点关注自相互作用暗物质（SIDM）和温暗物质（WDM）等非标准模型对晕结构的影响。

Data: 使用了AIDA-TNG（Alternative Interacting Dark Matter and Astrophysics - TNG）高分辨率宇宙学模拟（cosmological simulations）套件中的数据，该套件基于IllustrisTNG框架构建，包含暗物质（dark matter）、恒星（stars）和气体（gas）在不同半径处的三维形状测量。

Method: 采用高分辨率宇宙学模拟套件AIDA-TNG（基于IllustrisTNG框架），系统测量不同半径处暗物质、恒星和气体组分的三维形状，以研究自相互作用暗物质（SIDM）和温暗物质（WDM）模型对星系形成及暗物质晕结构的影响。

Result: 自相互作用暗物质（SIDM）模型倾向于产生更圆且各向同性的晕，特别是在内部区域，而温暗物质（WDM）晕比冷暗物质（CDM）晕稍显球形且浓度较低。
...
### [27] [The AIDA-TNG project: dark matter profiles and concentrations in alternative dark matter models](https://arxiv.org/abs/2512.15869)
*Giulia Despali,Carlo Giocoli,Lauro Moscardini,Annalisa Pillepich,Mark Vogelsberger,Massimo Meneghetti*

**TL;DR**: 通过AIDA-TNG模拟揭示了温暗物质(WDM)和自相互作用暗物质(SIDM)模型下暗物质晕内部结构（密度分布轮廓和中心斜率）与标准冷暗物质(CDM)的差异，并量化了重子过程对差异的削弱效应。


<details>
  <summary>Details</summary>

Background: 研究探讨了标准冷暗物质（CDM）模型与替代性暗物质理论（如温暗物质WDM和自相互作用暗物质SIDM）对暗物质晕（halo）内部结构影响的差异。

Data: 数据来源于AIDA-TNG模拟（Auriga-Infrared Dark Ages-TNG simulations），包括暗物质单独（dark-matter-only）和全物理过程（full-physics）两种运行模式，覆盖质量范围从10^9.5到10^14.5太阳质量（Msun）的暗物质晕（halo）。

Method: 采用AIDA-TNG数值模拟（包括暗物质单独模拟和全物理过程模拟）结合多种解析模型（如Einasto轮廓）分析暗物质晕密度分布，量化温暗物质（WDM）和自相互作用暗物质（SIDM）模型下浓度-质量关系及核心参数分布。

Result: 标准冷暗物质（CDM）模型中暗物质晕的密度轮廓可通过解析模型准确描述其浓度与质量关系，而温暗物质（WDM）和自相互作用暗物质（SIDM）模型会改变晕内部结构并预测不同的轮廓形态与中心斜率。
...
### [28] [Dark Acoustic Oscillations as an Early-Universe Explanation of the DESI Anomaly](https://arxiv.org/abs/2512.15870)
*Mathias Garny,Florian Niedermann,Martin S. Sloth*

**TL;DR**: 研究发现早期宇宙的暗声学振荡（DAO）与重子声学振荡（BAO）尺度相近时，会干扰BAO尺度的提取，导致距离推断偏差；考虑这种偏差后，振幅百分比的DAO特征能协调DESI DR2与普朗克2018及Pantheon+超新星数据，其拟合改进程度与演化暗能量相当，且该DAO特征与解决哈勃张力的暗物质-暗辐射提前解耦（DRMD）模型预测一致。


<details>
  <summary>Details</summary>

Background: 研究探讨了早期宇宙物理机制（如暗声学振荡DAO）对重子声学振荡BAO尺度提取的潜在影响及其对暗能量演化解释的替代可能性。

Data: 数据来源于DESI DR2（Dark Energy Spectroscopic Instrument Data Release 2）、Planck 2018宇宙微波背景辐射数据以及Pantheon+超新星数据集。

Method: 采用早期宇宙物理模型，通过分析暗声学振荡（DAO）与重子声学振荡（BAO）在星系相关函数峰值处的叠加效应来修正距离推断偏差。

Result: 早期宇宙物理中的暗声学振荡（DAO）与重子声学振荡（BAO）尺度相近时，会干扰星系相关函数峰值处BAO尺度的提取，导致距离推断出现偏差。
...
### [29] [The AIDA-TNG project: abundance, radial distribution, and clustering properties of halos in alternative dark matter models](https://arxiv.org/abs/2512.15883)
*Massimiliano Romanello,Giulia Despali,Federico Marulli,Carlo Giocoli,Lauro Moscardini*

**TL;DR**: 通过分析暖暗物质（Warm dark matter）和自相互作用暗物质（self-interacting dark matter）对晕族（halos）丰度、径向分布和聚类特性的宏观影响，发现暖暗物质模型呈现更尖锐的卫星分布，自相互作用暗物质则具有更平缓的密度轮廓，且小尺度晕族聚类能有效区分不同暗物质模型。


<details>
  <summary>Details</summary>

Background: 研究探讨了暖暗物质（warm dark matter）和自相互作用暗物质（self-interacting dark matter）作为非重子解决方案，用于缓解Λ冷暗物质（ΛCDM）模型在千秒差距尺度上与观测结果之间的张力。

Data: 数据来源于\textsc{aida-tng}项目的暗物质专用模拟运行（dark matter-only runs），包括不同尺寸和分辨率的宇宙学模拟。

Method: 采用暗物质晕占据分布（halo occupation distribution）形式主义分析其参数M₁和α随样本质量和红移选择的演化，并通过将晕群划分为中心晕和卫星晕研究其空间密度分布。

Result: 暖暗物质（WDM）和自相互作用暗物质（SIDM）模型下卫星子晕的径向分布呈现更陡或更浅的密度轮廓，且小尺度暗物质晕的聚类特性可有效区分不同暗物质模型。
...
### [30] [DESI-DR1 $3 \times 2$-pt analysis: consistent cosmology across weak lensing surveys](https://arxiv.org/abs/2512.15960)
...
**TL;DR**: 通过联合分析暗能量光谱仪首批数据（DESI-DR1）与三个弱引力透镜数据集（KiDS-1000、DES-Y3、HSC-Y3）的3×2点相关函数，首次实现了对宇宙学参数S₈（S₈ = σ₈√(Ωₘ/0.3)）的高精度测量（DESI-DR1+DES-Y3得S₈=0.786±0.021，KiDS-1000得S₈=0.760±0.019，HSC-Y3得S₈=0.771±0.027），其值较普朗克宇宙微波背景辐射数据低1.5-2σ。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过联合分析星系聚类（galaxy clustering）和弱引力透镜（weak gravitational lensing）观测数据来约束宇宙学参数，特别是打破单个观测手段在宇宙学拟合中的参数简并性（parameter degeneracies）。

Data: 数据来源于暗能量光谱仪数据发布1（DESI-DR1）的星系聚类观测，以及来自三个弱引力透镜数据集的重叠观测：千度巡天（KiDS-1000）、暗能量巡天（DES-Y3）和超广角相机巡天（HSC-Y3）。

Method: 采用联合分析3×2点相关函数（3×2-pt correlation functions）并结合解析协方差矩阵（analytical covariance matrix）和统一处理流程（unified pipeline）来测量宇宙学参数。

Result: 通过联合分析DESI-DR1星系成团性观测与KiDS-1000、DES-Y3、HSC-Y3弱引力透镜数据，测得S₈参数分别为0.786^{+0.022}_{-0.019}、0.760^{+0.020}_{-0.018}和0.771^{+0.026}_{-0.027}，与普朗克宇宙微波背景数据集拟合结果一致但S₈-Ωₘ平面数值低1.5-2σ。
...
### [31] [Joint cosmological fits to DESI-DR1 full-shape clustering and weak gravitational lensing in configuration space](https://arxiv.org/abs/2512.15961)
...
**TL;DR**: 该研究通过联合分析DESI-DR1的BGS和LRG样本与KiDS-1000、DES-Y3、HSC-Y3弱透镜测量的$3\times2$-pt相关函数，发现加入弱透镜信息使功率谱振幅参数$\ln(10^{10}A_{\rm{s}})$和$σ_{12}$的测量精度分别提升15%和36%，透镜星系线性偏差测量精度提升15-20%，且测得的弱透镜振幅$S_8$值比Planck结果低1.9σ-2.9σ。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用暗能量光谱仪（DESI）和弱引力透镜（weak lensing）数据的联合分析，探索宇宙学参数（如功率谱振幅和线性偏差）的精确测量及其与普朗克（Planck）卫星结果的差异。

Data: 数据来源于DESI-DR1 Bright Galaxy Survey (BGS)和Luminous Red Galaxy (LRG)样本的观测数据，以及KiDS-1000、DES-Y3和HSC-Y3弱引力透镜巡天的剪切测量数据。

Method: 采用联合3×2点相关分析（joint $3\times2$-pt analysis）方法，结合DESI-DR1 BGS和LRG样本与KiDS-1000、DES-Y3、HSC-Y3弱透镜测量的切向剪切（tangential shear）及两点相关函数多极矩（two-point correlation function multipoles）全形状拟合（full-shape analysis）。

Result: 联合弱引力透镜（weak lensing）信息将功率谱振幅参数$\ln(10^{10}A_{\rm{s}})$和$σ_{12}$的测量精度分别提高了15%和36%，透镜星系线性偏差（linear bias）测量精度提高了15-20%。
...
### [32] [Cosmological Constraints from Full-Scale Clustering and Galaxy-Galaxy Lensing with DESI DR1](https://arxiv.org/abs/2512.15962)
...
**TL;DR**: 通过分析暗能量光谱仪(DESI)数据发布1中的亮星系巡天(BGS)和亮红星系(LRG)样本的星系聚类和星系-星系透镜效应，结合暗能量巡天(DES)、千平方度巡天(KiDS)和超极形相机(HSC)的透镜数据，采用基于AbacusSummit模拟和复杂晕占据分布模型的框架，在非线性尺度上对宇宙结构增长参数S8和物质密度参数Ωm给出了严格限制，发现S8=0.794±0.023、Ωm=0.295±0.012(DES+KiDS)以及S8=0.793±0.017、Ωm=0.303±0.010(HSC)，与其他研究结果一致并凸显了非线性尺度的约束能力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用星系分布和引力透镜效应约束宇宙结构增长（cosmic structure growth），特别是通过暗能量光谱仪（DESI）数据结合多源透镜观测数据探讨宇宙学参数。

Data: 数据来源包括暗能量光谱仪(DESI)数据发布1中的亮星系巡天(BGS)和亮红星系(LRG)样本，以及来自暗能量巡天(DES)、千平方度巡天(KiDS)和超极深巡天(HSC)的透镜测量数据。

Method: 采用基于AbacusSummit模拟的仿真建模框架和包含组装偏差(assembly bias)的复杂晕占据分布模型(halo occupation distribution model)进行分析。

Result: 通过分析DESI（Dark Energy Spectroscopic Instrument）数据并结合DES（Dark Energy Survey）、KiDS（Kilo-Degree Survey）和HSC（Hyper Suprime-Cam）的弱引力透镜观测，在固定其他宇宙学参数为CMB（宇宙微波背景）优选值的情况下，测得S8=0.794±0.023和Ωm=0.295±0.012（DES和KiDS数据）或S8=0.793±0.017和Ωm=0.303±0.010（HSC数据），与现有研究一致且凸显了非线性尺度的约束能力。
...
### [33] [Clustering redshift distribution calibration of weak lensing surveys using the DESI-DR1 spectroscopic dataset](https://arxiv.org/abs/2512.15963)
...
**TL;DR**: 利用暗能量光谱仪（DESI-DR1）的光谱红移样本，通过聚类红移校准技术首次精确测定了弱引力透镜巡天（DES、KiDS、HSC）的源星系红移分布，并验证了该方法在消除放大效应和星系偏置演化影响后的可靠性，为弱透镜宇宙学提供了独立且互补的系统误差约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用暗能量光谱仪（DESI）数据改进弱引力透镜（weak lensing）宇宙学中的红移分布校准问题。

Data: 数据来源于暗能量光谱仪（DESI）数据发布1（DESI-DR1）提供的星系红移样本，包括亮星系巡天（BGS）、亮红星系（LRGs）和发射线星系（ELGs），并与暗能量巡天（DES）、千平方度巡天（KiDS）和超深主焦点相机巡天（HSC）的重叠层析源样本进行互相关分析。

Method: 采用基于聚类的红移校准技术（clustering-based redshift calibration），结合暗能量光谱仪数据发布1（DESI-DR1）提供的星系红移样本，通过交叉关联亮星系巡天（BGS）、发光红星系（LRGs）和发射线星系（ELGs）与弱透镜源样本，并利用模拟星表测试信号稳定性、显式建模和边缘化放大效应贡献。

Result: 基于DESI-DR1光谱样本的聚类红移校准技术显著降低了弱引力透镜宇宙学中红移分布的系统误差，与自组织映射(SOM)方法的结果在误差范围内一致。
...
### [34] [The Power of DESI for Photometric Redshift Calibration: A Case Study with KiDS-1000](https://arxiv.org/abs/2512.15964)
...
**TL;DR**: 利用暗能量光谱仪（DESI）观测数据校准KiDS-1000星系样本的光度红移（photo-z）分布，发现与先前结果总体一致但部分层析区间平均红移存在差异（Delta Mean(z) = -0.028），并指出自组织映射（SOM）单元的不完整性会影响红移分布。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用暗能量光谱仪(DESI)的光谱红移数据校准弱引力透镜巡天中的光测红移(photo-z)分布，以提升对暗能量和大尺度宇宙结构约束的精度。

Data: 数据来源于暗能量光谱仪（DESI）自2021年科学运行以来收集的超过5000万个红移数据（包括COSMOS和XMM及VVDS视场中256k个高质量光谱红移（spec-zs））以及KiDS-1000星系子集与DESI COSMOS覆盖区域重叠的自组织映射（SOM）单元中的星系。

Method: 采用自组织映射（self-organizing map, SOM）细胞匹配技术校准KiDS-1000星系的光度红移（photo-z）分布，并利用DESI在COSMOS区域获取的高质量光谱红移（spec-z）数据进行验证。

Result: 使用DESI（Dark Energy Spectroscopic Instrument）观测数据校准KiDS-1000星系的光度红移（photo-z）分布时，发现与先前发表结果总体一致，但在某些层析（tomographic）区间内平均红移存在差异，平均偏移Delta Mean(z) = -0.028，中位数偏移Delta Median(z) = +0.011。
...
### [35] [Uniform Rolling: An LSST Observing Cadence Offering Sufficient Survey Uniformity for Comprehensive Cosmological Analysis](https://arxiv.org/abs/2512.16478)
...
**TL;DR**: 研究发现针对时域科学优化的滚动观测策略（rolling survey strategy）会显著影响静态科学（static science）的中期数据深度均匀性，并提出了一种新策略在关键发布年份恢复均匀性，从而在不损失时域数据的前提下使静态科学可利用全部天区并恢复暗能量品质因子（Dark Energy figure of merit）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于优化维拉·鲁宾天文台(Vera C. Rubin Observatory)的LSST(Legacy Survey of Space and Time)巡天策略，以平衡静态科学(static science)和时间域科学(time-domain science)的需求。

Data: 数据来源为NSF-DOE Vera C. Rubin Observatory即将开展的Legacy Survey of Space and Time (LSST)观测数据。

Method: 采用滚动巡天策略（rolling survey strategy）优化观测区域覆盖频率，并通过量化巡天连续性和均匀性损失评估静态科学分析风险，最终提出在关键数据发布年份恢复均匀性的新策略。

Result: 针对静态科学（星系成团性和弱引力透镜）分析，基线滚动策略在中期数据发布时可能导致近半数观测区域因连续性和均匀性不足而无法使用，使暗能量品质因子下降约40%。
...
### [36] [How massive neutrinos reshape the cosmic web](https://arxiv.org/abs/2512.16517)
*Leonor N. L. Simões,Krishna Naidoo,Benjamin Joachimi,Willem Elbers,Carlos S. Frenk*

**TL;DR**: 通过FLAMINGO模拟发现大质量中微子(massive neutrinos)通过减少星系团和空洞(voids)体积改变宇宙网状结构(cosmic web)环境，并证实最小生成树(MST)边缘长度与不同尺度宇宙网环境存在强相关性，可作为星系巡天中大尺度结构的分类器。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量中微子（massive neutrinos）对宇宙网状结构（cosmic web）演化的影响及其在星系巡天中的结构分类潜力。

Data: 数据来源于FLAMINGO模拟（simulation）中的子晕（subhaloes）三维真实位置数据，并构建了最小生成树（MST）图，其数量密度选择与DESI类观测预期相匹配。

Method: 采用最小生成树（MST）图分析方法，基于子晕（subhaloes）构建三维空间网络结构，并关联宇宙网状环境（voids、sheets、filaments、clusters）分类。

Result: 中微子质量增加使体积加权密度分布向高密度偏移并变窄，表明中微子延迟了结构形成。
...
### [37] [Decoupling perturbations from background in $f(Q)$ gravity: the square-root correction and the alleviation of the $σ_8$ tension](https://arxiv.org/abs/2512.16551)
*Chunyu Li,Xin Ren,Yuhang Yang,Emmanuel N. Saridakis,Yi-Fu Cai*

**TL;DR**: 在对称远平行引力理论中引入平方根修正项（$\sqrt{Q}$）可抑制结构增长并缓解$\sigma_8$张力，尤其适用于$H_0$张力缓解的宇宙学背景，但参数$M$与$\sigma_8$仍存在残余简并。


<details>
  <summary>Details</summary>

Background: 研究探讨了对称远平行引力(symmetric teleparallel gravity)的微扰级修正形式$f(Q)=F(Q)+M\sqrt{Q}$及其缓解$\sigma_8$张力的能力。

Data: 使用最新红移空间畸变数据（redshift-space distortion data），包括DESI DR2全形状测量（DESI DR2 Full-Shape measurements），在三种代表性背景（ΛCDM、缓解H0张力的模型和DESI驱动的动态暗能量情景）下约束参数M和σ8。

Method: 采用扰动级修正的对称远平行引力模型（symmetric teleparallel gravity）$f(Q)=F(Q)+M\sqrt{Q}$，通过引入平方根项保持背景膨胀不变但修改有效引力耦合（effective gravitational coupling），实现背景宇宙学与结构增长演化的纯解耦。

Result: 在对称远平行引力（symmetric teleparallel gravity）的微扰级修正形式$f(Q)=F(Q)+M\sqrt{Q}$中，平方根项通过改变有效引力耦合（effective gravitational coupling）抑制结构增长，能在$1σ$水平上缓解$σ_8$张力（$σ_8$ tension），且在$H_0$张力（$H_0$ tension）优化背景下效果最显著。
...
### [38] [An effective $\boldsymbolΛ$-Szekeres modelling of the local Universe with Cosmicflows-4](https://arxiv.org/abs/2512.16591)
*Marco Galoppo,Leonardo Giani,Morag Hills,Aurélien Valade*

**TL;DR**: 通过叠加多结构Λ-Szekeres（Lambda-Szekeres）斑块模型校准HAMLET（HAMLET）重构数据，量化了红移z≲0.1局域宇宙结构对哈勃常数（H0）测量的各向异性影响，发现局域结构效应使哈勃张力加剧，导致最佳拟合值偏移约0.5 km s−1 Mpc−1。


<details>
  <summary>Details</summary>

Background: 研究聚焦于量化局部宇宙结构（local cosmic environment）对红移（redshift）$z \lesssim 0.1$范围内宇宙学观测量的偏差影响。

Data: 研究使用了来自Cosmicflows-4的HAMLET重建的局域速度场（peculiar velocity）和密度场（density field）数据，以及Pantheon+星表中的低红移（low-redshift）Ia型超新星（Type Ia supernovae）数据。

Method: 采用叠加多结构Λ-Szekeres（Λ-Szekeres）斑块模型校准HAMLET重构的局域速度-密度场（peculiar velocity and density field），计算非均匀各向异性准局域膨胀场（quasilocal expansion field）。

Result: 考虑局部结构效应会使哈勃常数（H0）的最佳拟合值增加约0.5 km s−1 Mpc−1，从而加剧哈勃张力。
...
### [39] [Current observations favour phantom-enhanced nature of dark energy](https://arxiv.org/abs/2512.16731)
*Md. Wali Hossain*

**TL;DR**: 研究发现，暗能量（dark energy）状态方程（equation of state）在红移演化中呈现更强类幻影（phantom-like）行为的模型更符合当前观测数据，且与ΛCDM模型存在显著统计差异（6.26σ），尤其幂律型参数化形式比限制性参数化更具优势。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗能量（dark energy）状态方程（equation of state）的幻影行为（phantom-like）演化对当前观测数据的拟合效果及其与标准宇宙学模型（$Λ$CDM）的统计张力。

Data: 数据来源包括DESY5（Dark Energy Survey Year 5）观测数据集。

Method: 采用幂律型（power-law）等具有更高红移演化灵活性的参数化方法分析暗能量状态方程（equation of state）的幽灵态（phantom-like）行为。

Result: 暗能量模型的状态方程（equation of state）越倾向于过去呈现幻影（phantom-like）行为，其与当前观测数据的拟合度越高，同时与ΛCDM模型的统计张力也越强。
...
### [40] [Dark energy and neutrinos along the cosmic expansion history](https://arxiv.org/abs/2512.16781)
*Pietro Ghedini,Rasmi Hajjar,Olga Mena*

**TL;DR**: 采用模型无关方法联合重构暗能量状态方程（$w_\mathrm{DE}$）和中微子质量总和（$\sum m_ν$），发现暗能量状态方程基本符合宇宙学常数假设，但结合DESI数据并允许幻影穿越时在红移$z\sim1.2$处出现95%置信水平偏离$w_\mathrm{DE}=-1$，且中微子质量约束在幻影暗能量或PCHIP重构时更宽松。


<details>
  <summary>Details</summary>

Background: 研究聚焦于暗能量状态方程（$w_\mathrm{DE}$）和中微子质量总和（$\sum m_ν$）的演化特性及其与宇宙学常数假设的一致性检验。

Data: 数据来源包括宇宙微波背景辐射（CMB）、重子声波振荡（Baryon Acoustic Oscillations）和Ia型超新星（Supernovae Ia）观测数据。

Method: 采用分段三次Hermite插值多项式（PCHIP）方法，在七个固定节点上联合重构暗能量状态方程（$w_\mathrm{DE}$）和中微子质量总和（$\sum m_ν$），并利用CMB、重子声学振荡和Ia型超新星数据进行约束。

Result: 暗能量状态方程（$w_\mathrm{DE}$）在包含DESI数据并允许幻影穿越（phantom crossing）时，在红移$z\sim1.2$附近出现95%置信水平偏离$w_\mathrm{DE}=-1$的现象。
...
### [41] [Future Dark Energy Constraints from Atomic Clocks](https://arxiv.org/abs/2512.16804)
*Oem Trivedi*

**TL;DR**: 原子钟测量揭示了标量张量暗能量（scalar tensor dark energy）必须处于缓慢滚动状态，排除了多数典型和非典型模型，仅支持接近ΛCDM行为或完全解耦场，从而将宇宙常数和最小耦合标量场模型确立为最一致的暗能量机制。


<details>
  <summary>Details</summary>

Background: 该研究探讨了利用原子钟测量作为探测标量张量暗能量（scalar tensor dark energy）的高灵敏度工具，揭示了其对牛顿常数变化和时钟漂移的关联性，从而为暗能量的动力学行为提供了新的约束条件。

Data: 数据来源于原子钟测量（atomic clock measurements）、月球激光测距（Lunar Laser Ranging）和光子轨迹（photon trajectories）观测结果。

Method: 通过原子钟测量（atomic clock measurements）和月球激光测距（Lunar Laser Ranging）数据，结合标量张量暗能量（scalar tensor dark energy）动力学模型，约束暗能量状态方程参数（equation of state parameter）。

Result: 原子钟测量为标量张量暗能量（scalar tensor dark energy）提供了极其敏感的太阳系探测手段，将牛顿常数变化和钟差漂移与单一暗能量标量的动力学关联，直接约束了当前状态方程（equation of state），迫使局域耦合标量暗能量进入极慢滚动状态，排除了广泛的正则与非正则模型，仅允许近ΛCDM行为或完全解耦场作为可行的晚期标量暗能量。
...
### [42] [Robust CMB B-mode analysis with Needlet-ILC and simulation-based inference](https://arxiv.org/abs/2512.16869)
*Adriaan J. Duivenvoorden,Kristen Surrao,Adrian E. Bayer,Alexandre E. Adler,Nadia Dachlythra,Susanna Azzoni,J. Colin Hill*

**TL;DR**: 提出了一种结合仿真推断、内波线性组合（NILC）算法和交叉相关统计的数据压缩方法，有效利用前景场的各向异性和非高斯信息，在复杂前景模型下显著提高了宇宙微波背景（CMB）极化信号分离精度和张标比（tensor-to-scalar ratio）$r$的测量稳健性。


<details>
  <summary>Details</summary>

Background: 研究探索了利用大规模宇宙微波背景（CMB）偏振数据进行参数推断的新型分析框架。

Data: 数据来源包括基于西蒙斯天文台小口径望远镜（Simons Observatory Small Aperture Telescopes）理想化地面实验设置的模拟数据，以及PySM银河前景模拟（包括高复杂度d12模型）和半解析前景模型生成的合成数据集。

Method: 采用基于模拟的推断方法结合小波内部线性组合（NILC）算法和基于互相关统计量的数据压缩技术，通过归一化流进行神经后验估计。

Result: 提出了一种结合小波内线性组合（NILC）算法与基于交叉相关统计量的模拟推断框架，通过数据压缩有效利用前景场的各向异性和非高斯信息，显著提升了张量标量比（tensor-to-scalar ratio）$r$的测量精度和对复杂前景的鲁棒性。
...
### [43] [Discovering gravitational waveform distortions from lensing: a deep dive into GW231123](https://arxiv.org/abs/2512.16916)
*Juno C. L. Chan,Jose María Ezquiaga,Rico K. L. Lo,Joey Bowman,Lorena Magaña Zertuche,Luka Vujeva*

**TL;DR**: 开发了DINGO-lensing深度学习算法，将引力波（GW）透镜效应推断时间从CPU天数缩短至分钟级，并通过分析候选事件GW231123发现其透镜效应统计显著性不超过4σ，同时证实该方法能有效提升未来透镜引力波的探测效率。


<details>
  <summary>Details</summary>

Background: 引力波（GWs）作为宇宙中不受干扰传播的唯一信使（除引力透镜效应外），其长波长特性使其易受宇宙结构衍射影响，为探测暗物质亚结构提供了独特机遇。

Data: 数据来源包括引力波（GW）观测数据和通过模拟生成的数千至数百万个透镜化（lensed）与非透镜化（nonlensed）事件数据集。

Method: 采用深度学习算法DINGO-lensing加速引力波（GW）透镜效应推断，将计算时间从CPU天数缩短至分钟级。

Result: 引力波（GWs）的长波长特性使其易受宇宙结构衍射影响，为绘制暗物质子结构提供了独特机会。
...
### [44] [Density reconstruction from biased tracers: Testing the equivalence principle through consistency relations](https://arxiv.org/abs/2510.13803)
*Lawrence Dam,Omar Darwish*

**TL;DR**: 通过提出基于局部交叉功率谱（local cross-power spectrum）中特定反对称调制的二次估计量（quadratic estimator），为DESI类巡天提供了检验弱等效原理（weak equivalence principle）的实用方法，其约束能力与直接测量双谱（bispectrum）相当。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用大尺度结构的一致性关系检验宇宙尺度上的弱等效原理（weak equivalence principle, EP），探索不同示踪物在均匀引力场中加速度差异导致的非普适性现象。

Data: 数据来源于DESI类巡天（DESI-like survey）的观测数据，并利用重构方法扩展到弱非线性尺度（$k_\mathrm{max}\simeq0.15\, h\,\mathrm{Mpc}^{-1}$）。

Method: 采用基于局部交叉功率谱（local cross-power spectrum）反对称调制特征的二次估计量（quadratic estimator）方法替代传统双谱（bispectrum）直接估计，通过重构包含弱非线性尺度（$k_\mathrm{max}\simeq0.15\, h\,\mathrm{Mpc}^{-1}$）的密度场来检测等效原理（EP）违反信号。

Result: 在弱等效原理（EP）被违反的情况下，不同示踪物（tracers）对均匀引力场的加速度响应差异会在局域交叉功率谱中表现为特定的反对称调制（anti-symmetric modulation），这为检验EP提供了一种比直接估计双谱（bispectrum）更实用的二次估计量（quadratic estimator）方法。
...
### [45] [An Improved Machine Learning Approach for RFI Mitigation in FAST-SETI Survey Archival Data](https://arxiv.org/abs/2512.15809)
*Li-Li Zhao,Xiao-Hang Luan,Xin Chao,Yu-Chen Wang,Jian-Kang Li,Zhen-Zhao Tao,Tong-Jie Zhang,Hong-Feng Wang,Dan Werthimer*

**TL;DR**: 提出并应用改进的密度聚类算法（DBSCAN）处理FAST-SETI巡天数据，高效剔除77.87%的残余射频干扰（RFI）且保留候选信号，比现有方法提升7.44%的剔除率并减少24.85%计算时间。


<details>
  <summary>Details</summary>

Background: 探索地外文明（SETI）的共视巡天旨在通过扫描天空探测地外生命的技术特征信号。

Data: 数据来源于2019年7月的FAST-SETI（五百米口径球面射电望远镜-搜寻地外文明计划）共视巡天存档数据。

Method: 采用基于密度的噪声应用空间聚类（DBSCAN）算法识别并消除FAST-SETI巡天数据中的残余射频干扰（RFI）。

Result: 采用改进的DBSCAN算法在FAST-SETI巡天数据中识别并去除了36977个残余射频干扰（RFI），去除率达77.87%，较先前方法提升7.44%且执行时间减少24.85%，同时保留了潜在候选信号。
...
### [46] [Milky Way disc & Bulge in situ populations: ESO white paper - Expanding horizons call](https://arxiv.org/abs/2512.15812)
...
**TL;DR**: 该论文指出当前缺乏银河系（Milky Way）全域高精度化学动力学图谱，提出建设新型宽视场、大通量光谱设施的必要性，以解决银河系盘（disc）、棒（bar）和核球（bulge）形成演化中的关键问题。


<details>
  <summary>Details</summary>

Background: 研究主题为银河系(Milky Way)盘、棒和核球(bulge)的形成与演化机制及其化学动力学(chemo-dynamical)特征。

Data: 数据来源包括现有光谱巡天（spectroscopic surveys）的小样本高分辨率数据和大范围低分辨率数据，但缺乏银河系中平面（midplane）、棒（bar）远侧及外盘（outer disc）等关键区域的均匀丰度、精确三维运动学和可靠年龄数据。

Method: 采用宽视场、大规模多目标、大口径光谱观测设施（wide-field, massively multiplexed, large-aperture spectroscopic facility）进行高分辨率和低分辨率光谱测量。

Result: 当前或计划中的设施无法全面解决银河系原始盘（primitive discs）存续、隆起（bulge）多组分结构起源、并合（mergers）与长期过程（secular processes）作用及恒星化学-动力学-引力势耦合等关键问题。
...
### [47] [Asteroseismology of white dwarfs in the 2040s](https://arxiv.org/abs/2512.15814)
...
**TL;DR**: 该论文核心贡献为利用白矮星（white dwarfs）脉动的星震学（asteroseismology）研究其内部化学分层、质量、自转和磁场，揭示了当前观测与理论模型在脉动激发机制和质量测定方面的差异。


<details>
  <summary>Details</summary>

Background: 白矮星（white dwarfs）作为绝大多数恒星演化的最终阶段，是宇宙年代学、行星系统演化研究以及非标准物理（包括奇异冷却通道、弱相互作用粒子和结晶过程）的关键实验室对象。

Data: 数据来源包括空间任务提供的高精度连续测光（photometry）数据以及地面光谱（spectroscopic）后续观测。

Method: 采用白矮星脉动周期观测与理论模型对比的星震学（asteroseismology）方法推断内部化学分层、总质量、自转剖面和磁场强度。

Result: 白矮星（white dwarfs）的脉动特性为研究其内部化学分层、总质量、旋转剖面和磁场强度提供了直接途径。
...
### [48] [Sailing to the next safe harbour in our trip to the early Universe: The massive star population of metal-poor galaxies](https://arxiv.org/abs/2512.15817)
*N. Castro,M. Garcia,A. Herrero,A. A. C. Sander,A. F. McLeod,M. M. Roth,I. Negueruela,J. S. Vink*

**TL;DR**: 论文提出需通过新一代全景积分视场光谱仪（panoramic integral-field spectrographs）和高通量多目标光谱仪（high multiplex multi-object spectrographs）系统研究本星系群（Local Group）极贫金属（very metal-poor）大质量恒星，以解决当前观测局限并全面揭示其对宇宙再电离（reionization）和早期星系化学演化的影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于极贫金属（very metal-poor）大质量恒星作为宇宙第一代恒星的最佳替代物，对理解再电离（reionization）和早期星系化学演化的关键作用。

Data: 研究中使用的数据主要来源于MUSE积分场光谱仪（MUSE integral field spectrograph）对银河系和麦哲伦云（Magellanic Clouds）的观测，以及针对选定区域的小规模专项观测。

Method: 采用全景积分视场光谱仪（panoramic integral-field spectrographs）和高复用多目标光谱仪（high multiplex multi-object spectrographs）结合大型光学望远镜进行系统性观测。

Result: 极贫金属（very metal-poor）大质量恒星是研究宇宙第一代恒星的关键对象，对建模再电离和早期星系化学演化至关重要。
...
### [49] [Nova Explosions in 2040](https://arxiv.org/abs/2512.15821)
...
**TL;DR**: 这篇论文的核心贡献是提出未来二十年新星（nova）研究的关键科学问题，聚焦喷出物的质量、成分、几何结构与动力学，以及双星系统的作用，并强调需通过快速响应、高频率多波段观测结合系统性高分辨率光谱技术推动新星物理学的突破性进展。


<details>
  <summary>Details</summary>

Background: 新星（nova）是吸积白矮星表面的热核爆炸现象，是研究爆发性核合成、粒子加速、激波物理和双星演化的重要实验室。

Data: 研究建议通过快速响应、高频率、多波段观测获取数据，并采用系统性高分辨率光学和近红外光谱（high-resolution optical and near-infrared spectroscopy）从爆发到静止期的完整覆盖。

Method: 采用快速响应、高频率的多波段观测技术，并结合从爆发到静息期系统性的高分辨率光学与近红外光谱(high-resolution optical and near-infrared spectroscopy)分析。

Result: 新星（nova）研究在2040年代的科学问题将聚焦于喷出物的质量、成分、几何结构与动力学特性，以及双星系统的作用和核燃烧、激波与全电磁波段辐射之间的联系。
...
### [50] [Accretion and Ejection Physics at High Time Resolution](https://arxiv.org/abs/2512.15832)
...
**TL;DR**: 这篇论文的核心贡献是提出通过配备光子计数探测器和紫外至中红外高光谱分辨率设备的专用设施，可推动对吸积（accretion）与喷流（ejection）物理机制的理解，解决当前关于致密天体（compact objects）吸积过程的关键开放性问题。


<details>
  <summary>Details</summary>

Background: 吸积（accretion）是驱动宇宙中最明亮天体并影响星系演化（通过外流（outflows）改变周围介质）的核心天体物理过程。

Data: 数据来源于亚秒级光学-红外波段对吸积X射线双星（accreting X-ray binaries）的观测。

Method: 采用光子计数探测器(photon-counting detectors)和紫外至中红外高光谱分辨率技术(high spectral resolution from the UV to the mid-infrared)的专用设施。

Result: 吸积（accretion）过程通过驱动强外流（outflows）显著影响宇宙演化，而亚秒级光学-红外观测为研究吸积-外流关联开辟了新途径。
...
### [51] [What Builds and Quenches the Most Massive Galaxies in the Early Universe?](https://arxiv.org/abs/2512.15836)
*Mengyuan Xiao,Longji Bing,Guilaine Lagache,Miroslava Dessauges-Zavadsky,Olivier Ilbert,Benjamin Magnelli,Pascal A. Oesch*

**TL;DR**: 论文核心贡献是提出需建设新一代大孔径宽视场远红外（FIR）观测设施，以解决大质量星系（massive galaxies）在宇宙早期的真实空间密度、重子物质（baryon）聚集路径及淬灭（quenching）机制等关键问题。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙早期（the first few billion years）大质量星系的快速形成及其空间密度（space density）、重子物质（baryon）聚集路径和早期淬灭（quenching）机制等未解问题。

Data: 数据来源于下一代具备超大孔径、宽视场和高测绘速度的观测设施开展的深场静止帧远红外（rest-frame FIR）巡天。

Method: 采用宽视场、高灵敏度静止帧远红外（FIR）巡天技术结合大孔径望远镜系统，重点开展深度度级尺度的多波段观测以约束重子物质分布与演化。

Result: 早期宇宙中最大质量星系的真实空间密度（space density）、重子物质（baryon）聚集路径及早期淬灭（quenching）机制仍缺乏明确约束。
...
### [52] [Galaxies as stochastic systems: why the next breakthrough in galaxy evolution requires one hundred million spectra](https://arxiv.org/abs/2512.15841)
*Sandro Tacchella,Vasily Belokurov,Harry T. J. Bevins,Roberto Maiolino,Hiranya V. Peiris,Lucia Pozzetti,Mark T. Sargent*

**TL;DR**: 该论文提出通过构建深度和规模兼具的大样本星系光谱观测（涵盖质量、红移、环境、结构和演化阶段等多维度参数），采用分层推断方法约束星系随机演化过程的高维超参数（如相关时标、爆发性、占空比等），从而突破现有观测限制，实现对恒星形成、淬灭、化学增丰和黑洞增长等物理机制的理论约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过统计反演方法从单次观测的星系集合中推断时间依赖的物理过程（如恒星形成、化学增丰和黑洞增长），以超越描述性标度关系并揭示其随机过程的调控机制。

Data: 研究数据来源于未来ESO宽视场光谱设施（ESO wide-field spectroscopic facility）计划获取的0≲z≲3范围内数千万至数亿条静止紫外-光学光谱（rest-UV-optical spectra）。

Method: 采用分层贝叶斯推断方法对星系演化进行随机过程建模，通过约束超参数（如相关时标、爆发性、占空比）来揭示恒星形成、淬灭、化学增丰和黑洞增长的物理调控机制。

Result: 星系演化研究需通过大样本（约10^8个星系）的单次观测数据，采用分层推断方法获取随机过程的超参数（如相关时标、爆发性、占空比），从而揭示恒星形成、淬灭、化学增丰和黑洞增长等物理调控机制。
...
### [53] [Hunting exomoons with a kilometric baseline interferometer](https://arxiv.org/abs/2512.15858)
*Thomas O. Winterhalder,Antoine Mérand,Sylvestre Lacour,Jens Kammerer,Guillaume Bourdarot,Frank Eisenhauer*

**TL;DR**: 提出利用新型光学干涉仪（精度达1微角秒）可在50-200秒差距范围内稳定探测木星类行星周围的地球质量级及次地球质量级系外卫星（exomoons）。


<details>
  <summary>Details</summary>

Background: 系外卫星（exomoons）的探测对研究行星形成与演化、宿主行星迁移历史及潜在宜居环境具有重要意义。

Data: 研究中使用的数据来源为模拟（simulation）数据，基于假设的干涉仪设备在光学波段（optical wavelength domain）的观测性能。

Method: 采用光学波段干涉测量技术（interferometry）并利用比甚大望远镜干涉仪（VLTI）更长的基线，以1微角秒（μas）的精度进行天体测量（astrometry）。

Result: 发现新型光学干涉仪可实现1微角秒天体测量精度，稳定探测50-200秒差距范围内类木行星周围的地球质量级及亚地球质量级系外卫星(exomoons)。
...
### [54] [Binarity Beyond Gaia: The case for a dedicated spectroscopic survey of binary stars](https://arxiv.org/abs/2512.15904)
*Borja Anguiano*

**TL;DR**: 基于盖亚(Gaia)任务在双星观测上的局限性，提出"超越盖亚的双星"光谱巡天计划，通过更深探测极限、灵活观测节奏和中等至高光谱分辨率，填补超短周期、超长周期、低振幅和致密双星等参数空白，实现银河系恒星多重性无偏普查。


<details>
  <summary>Details</summary>

Background: 恒星多重性（stellar multiplicity）是恒星天体物理学的基本要素，但银河系内的双星统计仍缺乏充分约束。

Data: 数据来源为盖亚(Gaia)任务发布的DR3数据，包括高精度天体测量、测光和全局径向速度数据，以及数十万非单星解。

Method: 采用简化的正向模型（forward model）模拟类似盖亚（Gaia）DR5性能，识别难以获得稳健轨道解的双星群体（超短周期、超长周期、低振幅和致密天体双星）。

Result: 盖亚(Gaia)任务虽革新了双星天体物理学，但其观测限制导致超短周期、极长周期、低振幅和致密天体双星等群体的轨道解算仍稀缺。
...
### [55] [Graph Neural Networks for Interferometer Simulations](https://arxiv.org/abs/2512.16051)
*Sidharth Kannan,Pooyan Goodarzi,Evangelos E. Papalexakis,Jonathan W. Richardson*

**TL;DR**: 首次将图神经网络(GNNs)应用于激光干涉引力波天文台(LIGO)仪器设计领域，证明其能准确模拟复杂光学物理过程且运行速度比现有仿真软件快815倍。


<details>
  <summary>Details</summary>

Background: 近年来图神经网络（GNNs）在解决高能物理、材料科学和流体动力学问题中展现出巨大潜力。

Data: 数据来源于激光干涉引力波天文台（LIGO）的高保真光学物理模拟数据集，包含三种干涉仪拓扑结构。

Method: 采用图神经网络（GNNs）模拟激光干涉引力波天文台（LIGO）的光学物理过程，并显著提升计算效率。

Result: 图神经网络(GNNs)被成功应用于激光干涉引力波天文台(LIGO)仪器设计模拟，能以815倍于现有模拟软件的速度精确捕捉复杂光学物理过程。
...
### [56] [Expanding Horizons - Transforming Astronomy in the 2040s Time-Domain Multi-Messenger Astronomy in the 2040s: EM Follow-up of LGWA Sources](https://arxiv.org/abs/2512.16264)
...
**TL;DR**: 月球引力波天线（LGWA）通过填补毫赫兹至赫兹频段的观测空白，实现了对合并前数月到数年致密双星系统的持续监测，并推动时域多信使天文学从被动响应转向可预测的电磁（EM）跟踪观测。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用月球引力波天线（LGWA）填补毫赫兹至赫兹频段引力波观测空白，推动时域多信使天文学从被动响应合并事件转向主动预测并规划电磁（EM）跟踪观测的变革性发展。

Data: data部分：研究使用月球引力波天线（Lunar Gravitational Wave Antenna, LGWA）的仿真数据（simulation data）来探测mHz-Hz频段的引力波信号。

Method: 采用月球引力波天线（Lunar Gravitational Wave Antenna, LGWA）技术进行连续数月到数年的引力波（GW）观测，重点探测空间基mHz探测器和地面基>10 Hz设施无法覆盖的致密双星系统。

Result: 月球引力波天线（LGWA）将开辟毫赫兹至赫兹频段的引力波（GW）观测新窗口，实现对合并前数月到数年持续可见的致密双星系统的探测，并填补空间和地面现有探测器无法覆盖的空白频段。
...
### [57] [Radio frequency interference identification using eigenvalue decomposition for multi-beam observations](https://arxiv.org/abs/2512.16278)
...
**TL;DR**: 开发了基于特征值分解算法的新型射频干扰(RFI)抑制工具mRAID，通过处理五百米口径球面射电望远镜(FAST)多波束互相关矩阵数据，显著提升了脉冲星和快速射电暴(FRB)搜索效率并具有并行计算优势。


<details>
  <summary>Details</summary>

Background: 研究聚焦于射电望远镜下一代相控阵馈源(PAF)接收机应用中大规模巡天所需的射频干扰(RFI)抑制方法开发。

Data: 数据来源于五百米口径球面射电望远镜（FAST）的高时间分辨率脉冲星搜索数据。

Method: 采用基于特征值分解(eigenvalue decomposition)算法分析多波束数据互相关矩阵(CCM)的方法识别射电干扰(RFI)。

Result: 开发的多波束射电干扰检测器(mRAID)通过特征值分解算法识别多波束数据互相关矩阵中的射电干扰(RFI)，显著提升了脉冲星和快速射电暴(FRB)搜索效率。
...
### [58] [New methods to improve the decontamination of slitless spectra](https://arxiv.org/abs/2512.16324)
*Mostafa Bella,Shahram Hosseini,Thierry Contini,Hicham Saylani*

**TL;DR**: 提出了四种基于局部瞬时（local instantaneous）和局部卷积（local convolutive）模型的新方法，有效实现了欧几里得（Euclid）等任务中无狭缝光谱（slitless spectroscopy）观测的恒星与星系光谱去污染（decontaminate）和反卷积（deconvolution）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发无狭缝光谱（slitless spectroscopy）观测中恒星与星系光谱的去污染（decontamination）方法，以应对欧几里得（Euclid）等空间任务的光谱数据混杂问题。

Data: 研究中使用的数据来源于欧几里得(Euclid)等空间任务中采用无缝摄谱仪(slitless spectroscopy)获取的恒星和星系光谱数据，并利用光度计(photometer)提供的直接图像进行去污染处理。

Method: 基于两种不同方法（局部瞬时法(local instantaneous approach)和局部卷积法(local convolutive approach)）开发了四种光谱去污染方法，分别采用空间域混合模型和傅里叶域混合模型。

Result: 提出了四种基于局部瞬时（local instantaneous）和局部卷积（local convolutive）两种新方法的光谱去污技术，能有效处理多色散方向的无缝光谱数据。
...
### [59] [Maximizing the sensitivity of ELT to habitable worlds with a space-based starshade](https://arxiv.org/abs/2512.16361)
*Markus Janson,Enric Palle,Thomas Henning,Sascha Quanz,Lars Buchhave,Alexis Brandeker*

**TL;DR**: 结合极大望远镜（ELT）和空间遮星仪可观测数十颗类地行星（Earth analogs），为研究宇宙中宜居性和生命分布提供科学基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过结合极大望远镜(ELT)和遮星板(starshade)技术突破现有观测极限，探索类太阳恒星宜居带内类地行星(earth analogs)的分布与适居性频率(hability frequency)问题。

Data: 数据来源为欧洲极大望远镜（ELT）与空间遮星板（starshade）的协同观测。

Method: 采用地面极端大口径望远镜（ELT）与空间遮星仪（starshade）协同观测技术，通过联合提升对比度突破衍射极限，实现类太阳恒星宜居带内地外行星（exoplanet）的直接成像探测。

Result: 结合ELT（极大望远镜）与空间遮星仪可观测数十颗类地行星（Earth analogs），为研究宇宙中宜居性和生命的分布频率提供科学基础。
...
### [60] [Transformational astrophysics and exoplanet science with Habitable Worlds Observatory's High Resolution Imager](https://arxiv.org/abs/2512.16416)
...
**TL;DR**: Habitable Worlds Observatory（HWO）是NASA计划于2040年代发射的旗舰空间望远镜，旨在搜寻系外行星生命并推动天体物理学多个领域的变革，其成像相机（imaging camera）可能由英国主导设计建造。


<details>
  <summary>Details</summary>

Background: 研究聚焦于NASA未来旗舰太空望远镜宜居世界天文台（HWO）的规划与建设，旨在探索系外行星生命迹象并推动天体物理学多领域突破。

Data: 数据来源为NASA主导的旗舰空间望远镜Habitable Worlds Observatory（HWO）的观测数据。

Method: method：设计并建造成像相机（imaging camera）以支持HWO（Habitable Worlds Observatory）任务。

Result: Habitable Worlds Observatory (HWO)将成为NASA在2040年代的旗舰空间望远镜，旨在搜寻其他行星上的生命并推动天体物理学的广泛领域发展。
...
### [61] [A direction-dependent framework for visibility plane mosaicing and primary beam correction](https://arxiv.org/abs/2512.16440)
*Keegan S. Trehaeven,Cyril Tasse,Oleg Smirnov,Tiziana Venturi*

**TL;DR**: 提出了一种基于相干可见度平面的方向依赖成像、校准和拼接框架，显著提升了MeerKAT（南非射电望远镜阵列）对Shapley超星系团核心的成像深度（灵敏度达3.6微焦耳/波束）和动态范围（提升约50%），在超出半功率波束宽度区域实现了通量密度（6%误差）和谱指数（20%误差）的高精度恢复，为SKA（平方公里阵列）等未来设备的校准流程提供了基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用现代射电干涉仪（radio interferometer）提升大视场（field of view）天体的成像灵敏度与保真度，特别是在超出半功率波束宽度（half-power beam width）区域的通量密度和谱指数（spectral index）恢复精度。

Data: 研究中使用的数据来源包括模拟数据和真实MeerKAT（英文原词：MeerKAT）的1283 MHz宽频带存档数据。

Method: 采用相干可见度平面方向依赖（direction-dependent）成像、校准和拼接框架，结合联合反卷积与主波束校正方法及方向依赖校准技术。

Result: result：提出的相干可见度平面方向依赖成像、校准和拼接框架显著提升了马赛克图像的深度和保真度，在成像区域内实现了6%的流量密度不确定度和20%的谱指数精度，且动态范围比传统图像平面方法提升约50%。
...
### [62] [Simulation-based inference with neural posterior estimation applied to X-ray spectral fitting - III Deriving exact posteriors with dimension reduction and importance sampling](https://arxiv.org/abs/2512.16709)
*Didier Barret,Simon Dupourqué*

**TL;DR**: 该论文提出了一种基于神经后验估计（NPE）和自动编码器压缩高分辨率X射线谱的方法，结合似然重要性采样修正后验分布，在保持统计准确性的同时实现了10倍加速，适用于多种X射线仪器和分辨率。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用基于模拟的推断（SBI）和神经后验估计（NPE）技术提升X射线光谱拟合效率，并探索自动编码器在压缩高分辨率X射线光谱中的应用，以应对新一代Athena X射线积分视场单元（X-IFU）的观测需求。

Data: 研究使用了基于模拟(simulation)的数据，包括新雅典娜X射线积分视场单元(X-IFU)的高分辨率X射线光谱模拟数据，以及XRISM/Resolve、NICER和XMM-Newton EPIC-pn的低分辨率仪器数据。

Method: 采用神经网络后验估计（NPE）进行基于模拟的推断（SBI），通过自动编码器压缩高分辨率X射线光谱并利用基于似然的重要性采样修正NPE输出，结合多轮训练策略提升效率。

Result: 基于神经后验估计（NPE）的模拟推断（SBI）通过从模拟中学习近似参数后验，实现了高斯和泊松体系下的快速X射线光谱拟合。
...
### [63] [The WINTER Observatory: A One-Degree InGaAs Survey Camera to study the Transient Infrared Sky](https://arxiv.org/abs/2512.16753)
...
**TL;DR**: WINTER（Wide-field Infrared Transient Explorer）是一台安装在帕洛玛天文台的1米专用机器人望远镜上的新型近红外时域巡天设备，利用大格式砷化铟镓（InGaAs）传感器和非制冷技术，实现了在Y、J和缩短H波段（0.9-1.7微米）覆盖超过1平方度视场的高效观测，用于探测千新星（kilonovae）、研究银河系和河外暂现源及变量，并构建近红外天空参考图像。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用近红外时域巡天技术（WINTER望远镜）探索引力波事件千新星（kilonovae）、银河系及河外暂现源与变源，并验证非制冷型砷化铟镓（InGaAs）传感器技术。

Data: 数据来自安装在帕洛玛天文台专用1米机器人望远镜上的宽视场红外瞬变探测器（WINTER），该仪器使用六个非制冷铟镓砷（InGaAs）传感器组成的定制相机，在Y、J和缩短H波段（0.9-1.7微米）以1角秒像素分辨率覆盖超过1平方度的视场，自2023年6月起每晚自动巡天至J_AB=18.5星等中值深度。

Method: 采用新型非制冷砷化镓铟（InGaAs）传感器阵列结合复眼式光学设计，在Y/J/短H波段（0.9-1.7微米）实现1角秒像素分辨率的大视场（>1平方度）近红外测光。

Result: WINTER（Wide-field Infrared Transient Explorer）是一台配备新型非制冷砷化铟镓（InGaAs）传感器、采用拼接复眼光学设计的近红外瞬变源巡天设备，可覆盖1平方度视场并在Y、J及缩窄H波段（0.9-1.7微米）实现1角秒像素分辨率。
...
### [64] [Strategies for Accurate Effective Point Spread Function (ePSF) Modelling on Undersampled Images](https://arxiv.org/abs/2512.16764)
*Emma Godden,Katherine M. Blundell*

**TL;DR**: 通过改进有效点扩散函数（ePSF）建模方法并系统评估关键参数影响，显著提升了真实天文图像（如Global Jet Watch观测数据）中测光和天体测量的精度，强调需根据仪器特性和数据特征定制建模策略。


<details>
  <summary>Details</summary>

Background: 高精度测光（photometry）和天体测量（astrometry）中精确建模有效点扩散函数（ePSF）的需求，尤其在欠采样（undersampled）成像条件下的关键作用。

Data: 数据来源包括合成有效点扩散函数（ePSF）生成的模拟恒星图像数据集和全球喷流观测站（Global Jet Watch observatories）的真实天文图像。

Method: 采用改进的有效点扩散函数（ePSF）建模框架，通过合成ePSF生成模拟恒星图像数据集，系统评估建模选择对精度的影响，包括过采样、插值、网格点估计、平滑、星样本分布和抖动等因素。

Result: 对现有有效点扩散函数(ePSF)建模方法进行简单有效的改进可显著提升模型精度。
...
### [65] [Introduction to Symbolic Regression in the Physical Sciences](https://arxiv.org/abs/2512.15920)
*Deaglan J. Bartlett,Harry Desmond,Pedro G. Ferreira,Gabriel Kronberger*

**TL;DR**: 这篇论文的核心贡献是综述了符号回归（symbolic regression, SR）在物理科学中的概念基础、应用场景（如自动方程发现和替代模型构建）及方法学挑战（如可扩展性和抗噪性），并探讨了整合对称性约束等新兴方向。


<details>
  <summary>Details</summary>

Background: 符号回归（Symbolic Regression，SR）作为一种揭示数据中可解释数学关系的强大方法，为科学发现和高效经验建模提供了新途径。

Data: 数据来源包括自动化方程发现、涌现现象建模以及为计算密集型仿真构建的紧凑仿真器（compact emulators）。

Method: 采用符号回归（symbolic regression, SR）方法，结合搜索空间设计、算子选择、复杂度控制、特征选择及与现代人工智能技术的融合，并引入对称性约束、渐近行为等理论信息。

Result: 符号回归（SR）已成为从数据中揭示可解释数学关系的强大方法，为科学发现和高效经验建模提供了新途径。
...
### [66] [Particle Production by Time-Varying Dark Energy and the End of Cosmic Expansion](https://arxiv.org/abs/2512.15860)
*Nicolas Patino,Paul J. Steinhardt*

**TL;DR**: 该研究探讨了五元场（quintessence scalar field）能量密度部分转化为粒子时产生的热摩擦（thermal friction）对暗能量（dark energy）动力学的影响，发现热摩擦通过减缓标量场加速度延长了宇宙加速膨胀期并可能产生可观测的热暗辐射（thermal dark radiation），同时掩盖了宇宙从膨胀转向收缩的临界点。


<details>
  <summary>Details</summary>

Background: 研究探讨了由五元标量场（quintessence scalar field）能量密度部分转化为粒子所导致的时变暗能量（time-varying dark energy）效应及其对宇宙膨胀和收缩相变的影响。

Data: 研究中使用的数据来源为理论模拟（simulation）数据，基于五元标量场（quintessence scalar field）模型及其与粒子产生的相互作用。

Method: 采用五元标量场（quintessence scalar field）模型分析暗能量时变效应，重点引入粒子产生导致的摩擦热（thermal friction）机制，量化其对场演化动力学和宇宙膨胀速率的影响。

Result: 热摩擦（thermal friction）延长了加速膨胀期并推迟了宇宙膨胀终结，使传统宇宙学测试难以检测向收缩的转变。
...
### [67] [GWTC-4.0: Searches for Gravitational-Wave Lensing Signatures](https://arxiv.org/abs/2512.16347)
*The LIGO Scientific Collaboration,The Virgo Collaboration,The KAGRA Collaboration*

**TL;DR**: 论文核心贡献为：基于LIGO-Virgo-KAGRA第四观测期(O4a)数据，通过三种方法未发现强透镜(strong lensing)引力波信号证据，并据此约束了可探测强透镜事件率及高红移双黑洞(binary black holes)并合率密度，同时发现一个异常事件GW231123_135430需进一步研究。


<details>
  <summary>Details</summary>

Background: 引力波在传播过程中可能被大质量天体引力透镜（gravitational lensing）效应扭曲或产生多重信号。

Data: 数据来源于第四轮LIGO-Virgo-KAGRA联合观测运行（O4a）的第一部分采集的引力波数据。

Method: 采用相位偏移检测（phase shift detection）、频率演化一致性配对（frequency evolution consistency matching）和亚阈值对应信号识别（sub-threshold counterpart identification）三种方法搜索强透镜（strong lensing）事件，并使用孤立点质量模型（isolated point-mass model）分析单次畸变信号。

Result: 在第四轮LIGO-Virgo-KAGRA联合观测(O4a)数据中未发现强引力透镜效应(strong lensing)的确凿证据，但识别出一个异常事件GW231123_135430需进一步研究。
...
**TL;DR**: 论文通过二阶泰勒展开处理F(T)引力（F(T) gravity）幂律模型的非线性问题，发现当偏离ΛCDM模型的额外自由度参数n≤0.05时，该方法在CLASS数值模拟中能保持可忽略的截断误差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于F(T)引力理论中幂律模型的非线性特性及其在宇宙学扰动计算中的局限性。

Data: 数据来源包括超新星（supernova）观测数据。

Method: 采用二阶泰勒展开(Taylor expansion)处理F(T)引力理论中非线性场方程，并假设额外自由度n（量化与ΛCDM模型偏差）足够小以保持ΛCDM关键特性。

Result: F(T)引力理论的幂律模型非线性特性导致CLASS（宇宙线性各向异性求解系统）无法直接处理非线性情况，需借助泰勒展开近似求解。
...
**TL;DR**: 基于非线性因果约束推导了正压（barotropic）Israel-Stewart流体的广义弛豫时间表达式，提出了正压流体中因果粘性耗散的新模型，并在弗里德曼宇宙中获得了线性与非线性区域的新解析解，同时数值模拟显示该模型支持从哈勃慢滚膨胀相平滑过渡到辐射主导宇宙的演化过程。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用非线性因果约束改进以色列-斯图尔特(Israel-Stewart)流体弛豫时间表达式，并建立描述正压流体中因果黏性耗散的新理论框架。

Data: 研究中使用的数据来源为理论推导（simulation）数据，基于广义非线性以色列-斯图尔特（Israel-Stewart）理论框架下的因果粘性耗散模型，并在空间平坦的弗里德曼宇宙中进行了数值求解验证。

Method: 采用非线性因果约束推导了正压以色列-斯图尔特(Israel-Stewart)流体的广义弛豫时间表达式，并提出了正压流体中因果粘性耗散的新建模方法。

Result: 基于非线性因果约束推导了正压以色列-斯图尔特(IS)流体的广义弛豫时间表达式，并提出了正压流体中因果粘性耗散的新建模方法。
...
### [70] [Exponential plateaus and inflation in metric-affine gravity](https://arxiv.org/abs/2512.16815)
*Antonio Racioppi*

**TL;DR**: 在度规-仿射引力框架下提出了一种涉及暴胀子与霍尔斯特不变量非最小耦合的新机制，当耦合函数在零点附近足够陡峭时，暴胀子势能总会形成指数型平台，其预言与斯塔罗宾斯基暴胀一致。


<details>
  <summary>Details</summary>

Background: 研究探讨了在度规-仿射引力（metric-affine gravity）框架下构建暴胀（inflationary）模型的新机制，重点关注暴胀子（inflaton）与霍尔斯不变量（Holst invariant）的非最小耦合效应。

Data: 数据来源于理论模拟（simulation）研究，基于度量仿射引力（metric-affine gravity）框架下的暴胀模型构建机制。

Method: 采用度量仿射引力（metric-affine gravity）框架，通过暴胀子（inflaton）与霍尔斯特不变量（Holst invariant）的非最小耦合构建模型。

Result: 在度量仿射引力框架下，通过引入与霍尔斯特不变量（Holst invariant）非最小耦合的暴胀子，当耦合函数在零点处非常陡峭时，正则化的暴胀子势能总会呈现指数型平台，其暴胀预测与斯塔罗宾斯基暴胀（Starobinsky inflation）一致。
...
### [71] [Observational constraints on the origin of the elements. X. Combining NLTE and machine learning for chemical diagnostics of 4 million stars in the 4MIDABLE-HR survey](https://arxiv.org/abs/2512.15888)
*Nicholas Storm,Maria Bergemann,Tomasz Różański,Victor F. Ksoll,Thomas Bensby,Guillaume Guiglion,Gražina Tautvaišienė*

**TL;DR**: 开发了基于非局部热平衡(NLTE)人工神经网络(ANN)的4MOST-HR光谱分析工具，可自动一致地测定恒星参数和18种元素丰度，精度偏差<0.13 dex、离散度<0.16 dex，并验证了其在银河系化学演化研究中的应用潜力。


<details>
  <summary>Details</summary>

Background: 开发用于高分辨率光谱分析的非局部热平衡(NLTE)人工神经网络(ANN)，以支持银河系化学演化研究。

Data: 数据来源为404,793条新计算的FGK光谱（Non-Local Thermal Equilibrium条件下16种元素）及121颗低质量FGKM型恒星（包括主序矮星、亚巨星和巨星）的观测光谱（分辨率降至4MOST-HR）。

Method: 采用非局部热平衡（NLTE）Payne人工神经网络（ANN）分析恒星光谱，训练数据包含404,793条FGK光谱和16种元素的NLTE计算结果。

Result: 开发了一种基于非局部热平衡（NLTE）Payne人工神经网络（ANN）的全自动拟合算法，可自洽地同时推导恒星参数和元素丰度，验证显示对18种元素丰度的恢复偏差小于0.13 dex且离散度小于0.16 dex。
...
### [72] [IPA. Accretion rate of a low-mass Class 0 protostar, measured via mid-infrared fluorescent OH emission](https://arxiv.org/abs/2512.15999)
...
**TL;DR**: 通过詹姆斯·韦伯太空望远镜观测低质量低光度原恒星（Class 0 protostar）IRAS 16253-2429，发现其中心致密源存在OH纯转动谱线和CO_2振转谱线但缺乏H_2O发射线，证实OH荧光辐射源自H_2O在114-145 nm波段的紫外光解，并利用OH光谱测得原恒星吸积率为3×10^(-10) M☉/年，表明系统光度主要来自恒星光球而非吸积过程。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星形成早期阶段中吸积（accretion）、外流（outflow）与辐射过程的复杂相互作用如何塑造原恒星（protostar）的化学与物理环境。

Data: 数据来源于詹姆斯·韦伯太空望远镜（James Webb Space Telescope）对低质量、低光度0级原恒星IRAS 16253-2429的中红外观测，包括OH纯转动谱线和CO_2转振谱线发射光谱。

Method: 采用紫外光解（UV photodissociation）和荧光级联（fluorescent cascade）分析方法，通过中红外OH纯转动谱线和CO2振转谱线测量中央原恒星的紫外辐射通量。

Result: 通过红外羟基(OH)光谱测量中心原恒星的紫外辐射通量，确定盘-星吸积率为3×10⁻¹⁰太阳质量每年，并证明系统光度主要来自原恒星光球而非吸积光度。
...
### [73] [The Preliminary Mauve Science Programme: Science themes identified for the first year of operations](https://arxiv.org/abs/2512.16675)
...
**TL;DR**: Mauve卫星搭载200-700纳米紫外-可见光(UV-Vis)光谱仪，填补银河系恒星紫外波段观测数据空白，首年执行10项科学主题的长期监测与快速采样观测。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用低成本小卫星Mauve填补银河系恒星天体紫外波段空间观测数据的空白。

Data: 数据来源于搭载13厘米望远镜和紫外-可见光谱仪(UV-Vis spectrometer)的Mauve小卫星，其探测器单次覆盖200-700纳米波长范围，分辨率R~20-65，运行于510公里太阳同步轨道。

Method: 采用13厘米望远镜与光纤连接的紫外-可见光谱仪(UV-Vis spectrometer)，在200-700纳米范围内单次拍摄获取低分辨率光谱(R~20-65)。

Result: Mauve卫星将填补银河系恒星紫外波段空间观测数据的空白。
...
### [74] [Infrared SED Modeling of Velocity-Excess Maser Sources: Identifying Incipient Water-Fountain Candidates](https://arxiv.org/abs/2512.16209)
*Jia-Yong Xie,Jun-ichi Nakashima,Yong Zhang*

**TL;DR**: 研究发现速度超出（velocity excess）现象可诊断水脉泽源（Water Fountains）最早演化阶段，其中IRAS 19229+1708和IRAS 19052+0922可能代表初始非对称喷流开始形成的极早期水脉泽源。


<details>
  <summary>Details</summary>

Background: 研究探讨了拱星脉泽线中的“速度超出(velocity excess)”现象能否用于诊断水喷泉天体(Water Fountains)的最早演化阶段。

Data: 数据来源包括17个显示速度超出（velocity excess）的天体样本的红外光谱能量分布（SEDs）拟合结果、IRAS变率指数以及NEOWISE-R光变曲线。

Method: 采用一维辐射传输代码DUSTY拟合红外光谱能量分布(SED)，并结合速度超出(velocity excess)分析来诊断喷泉源(WFs)的早期演化阶段。

Result: 速度超出（velocity excess）现象可用于诊断水脉泽源（Water Fountains, WFs）的最早演化阶段，并有效筛选处于喷流形成和早期形态转变初期的天体。
...
### [75] [SNR 1987A : Spitzer data from days 6000 to 8000 revisited](https://arxiv.org/abs/2512.16801)
*Patrice Bouchet,René Gastaud,Alain Coulais,Richard G. Arendt*

**TL;DR**: 研究发现超新星遗迹SNR 1987A在3-5微米波段的辐射超量可能源于自由-自由辐射（free-free emission）而非暖无定形碳尘（warm amorphous carbon dust），并通过特定条件下的气体碰撞加热模型成功再现了从近红外到射电（3微米至1.4 GHz）的光谱能量分布（SED），同时建立了暖碳尘与硅酸盐尘质量增长的线性关系。


<details>
  <summary>Details</summary>

Background: 该研究探讨了超新星遗迹SNR 1987A在近红外至射电波段（3微米至1.4 GHz）光谱能量分布（SED）中观测到的[3,5]微米范围辐射超的潜在物理机制。

Data: 数据来源于斯皮策太空望远镜（Spitzer）在近红外至射电波段（3微米至1.4 GHz）对超新星遗迹SNR 1987A爆发后6000至8000天的光谱能量分布（SED）观测。

Method: 采用自由-自由辐射（free-free emission）模型结合自吸收截止频率约束条件，通过分析光谱能量分布（SED）从近红外到射电波段（3微米至1.4 GHz）的演化特征，并建立暖碳尘（warm amorphous carbon dust）与硅酸盐尘（silicates dust）质量增长的线性关系。

Result: 超新星遗迹1987A在3至5微米波段的辐射超可能源于自由-自由辐射（free-free emission）而非暖无定形碳尘（warm amorphous carbon dust）。
...
### [76] [Cosmological Neutron Stars Produce Diffuse Axion X-Ray Signatures](https://arxiv.org/abs/2512.15849)
*Orion Ning,Kailash Raman,Benjamin R. Safdi*

**TL;DR**: 研究发现中子星（NSs）核心产生的轴子（axion-like particles）通过磁层转换或衰变产生X射线信号，基于宇宙X射线背景观测排除了轴子解释"七壮士"（Magnificent Seven）X射线超量的可能性，并对轴子-光子和轴子-核子耦合参数给出了新约束。


<details>
  <summary>Details</summary>

Background: 该研究探讨了轴子类粒子（axion-like particles）在中子星（NSs）核心散射过程中大量产生后，通过磁层转换或衰变产生可探测X射线信号的机制及其对宇宙X射线背景的潜在贡献。

Data: 研究中使用的数据来源于NuSTAR、HEAO、Swift和INTEGRAL等仪器测量的宇宙X射线背景（cosmic X-ray background）数据。

Method: 采用中子星冷却模拟和磁场演化模型对轴子诱导的X射线信号进行建模。

Result: result：未发现轴子（axion）存在的证据，对超轻和重轴子情景均给出了强约束，排除了轴子解释附近孤立中子星（NSs）X射线超出的可能性。
...
### [77] [Kination and the Inert Doublet Model](https://arxiv.org/abs/2512.15864)
*Geneviève Bélanger,Nicolás Bernal,Andreas Goudelis,Alexander Pukhov*

**TL;DR**: 惰性双峰模型（inert doublet model）在标准宇宙学下暗物质质量120-500 GeV区间因湮灭效率过高导致不足，但研究表明在状态方程较硬（w>1/3）的宇宙学场景（如kination）中该质量窗口可满足当前实验限制且具有可探测性。


<details>
  <summary>Details</summary>

Background: 惰性双峰模型（inert doublet model）作为标准模型的双希格斯二重态扩展，为冻结暗物质（frozen-out dark matter）提供了最小且通用的理论框架。

Data: 数据来源于惰性双希格斯模型（inert doublet model）的理论模拟（simulation），结合标准宇宙学假设和暗物质质量在120 GeV至500 GeV范围内的参数空间分析。

Method: 采用惰性双希格斯二重态模型（inert doublet model）并结合具有刚性状态方程（stiff equation of state）的宇宙学情景（如kination）分析暗物质冻结过程。

Result: 惰性双峰模型（inert doublet model）在标准宇宙学下暗物质质量约120-500 GeV时因湮灭效率过高导致丰度不足，但在状态方程较硬（w > 1/3）的宇宙学场景（如kination）中该质量窗口可被允许且满足当前实验约束。
...
### [78] [Fifty shades of grayness: parameterizations of spectral distortions and applications in cosmology](https://arxiv.org/abs/2512.15893)
*Gabriela Barenboim,Julien Froustey,Cyril Pitrou,Héctor Sanchis*

**TL;DR**: 该论文通过提出基于多项式调制的热化参考谱和正交多项式基分解的新方法，简化了宇宙中微子和光子背景谱畸变的参数化描述，并获得了对这些宇宙遗迹非标准畸变的模型无关约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于热分布函数（thermal distribution functions）的偏离（distorted spectra）参数化方法及其在宇宙学背景（cosmological backgrounds）中的应用。

Data: 数据来源包括宇宙背景中微子（cosmological neutrinos）和光子（photons）的观测数据。

Method: 采用基于正交多项式基展开的方法，通过多项式调制参考热化谱来描述畸变谱，其中畸变参数为多项式展开系数。

Result: 构建了一种基于正交多项式基展开的新方法，通过用多项式调制参考热化谱来描述扭曲谱，其参数为分解系数，计算更简便且能描述广泛扭曲类型。
...
### [79] [Chiral magnetic effect amplified baryogenesis at first-order phase transitions](https://arxiv.org/abs/2512.16537)
*Hui Liu,Ligong Bian*

**TL;DR**: 研究发现原初磁场背景下电弱一级相变期间CME效应（chiral magnetic effect）能显著增强泡壁附近CP破坏（CP violation）产生的手征化学势，使重子不对称性提高数个量级，从而用远超电子偶极矩限制的费米子区CPV解释宇宙重子不对称性。


<details>
  <summary>Details</summary>

Background: 研究探讨了原初磁场（primordial magnetic field）背景下CME效应（Chiral Magnetic Effect）对电弱一级相变（first-order electroweak phase transition）期间泡壁附近CP破坏（CP violation）产生的手征化学势（chiral chemical potential）的放大作用及其对宇宙重子不对称性（baryon asymmetry of the Universe）的贡献。

Data: 数据来源于对原初磁场背景下的宇宙微波背景辐射（CMB）效应和电弱一级相变期间泡壁附近CP破坏（CP violation）产生的手征化学势（chiral chemical potential）的模拟分析。

Method: 采用手征磁效应（CME）和电弱一级相变（first-order electroweak phase transition）期间的CP破坏（CP violation）机制分析原初磁场背景下重子不对称性的增强效应。

Result: 在原始磁场背景下，CME（手征磁效应）能显著放大电弱一级相变期间泡壁附近CP破坏（电荷-宇称破坏）产生的手征化学势，使重子不对称性增强数个量级，从而用远超电子偶极矩限制的费米子区CP破坏解释宇宙重子不对称性。
...
### [80] [On Non-Minimal Couplings to Gravity and Axion Isocurvature Bounds](https://arxiv.org/abs/2512.16754)
*Claire Rigouzzo,Sebastian Zell*

**TL;DR**: 研究发现，在度规和Palatini引力框架下，暴胀子与引力的非最小耦合($ξ_σ$)会加剧轴子（axions）的等曲率扰动限制，而径向Peccei-Quinn场的非最小耦合($ξ_ρ$)则能缓解该限制，但$ξ_σ$会缩小$ξ_ρ$的有效调节区间以避免对暴胀动力学的反作用。


<details>
  <summary>Details</summary>

Background: 研究探讨了暴胀时期轴子（axions）的非最小耦合（non-minimal coupling）对等曲率扰动（isocurvature bounds）的影响，重点分析了暴胀场（inflaton）和径向佩斯-奎恩场（radial Peccei-Quinn field）与引力的不同耦合形式在度量（metric）和帕拉蒂尼（Palatini）引力框架下的相互作用及其对宇宙学参数的约束。

Data: 数据来源为理论模拟（simulation）研究，基于度量（metric）和帕拉蒂尼（Palatini）引力形式下的暴胀模型参数分析。

Method: 采用非最小耦合（non-minimal coupling）分析轴子（axion）等曲率扰动，结合度量（metric）和帕拉蒂尼（Palatini）引力形式，量化$ξ_σ$与$ξ_ρ$对等曲率界限的竞争性影响。

Result: 在暴胀期间存在轴子（axions）的情况下，非最小耦合（non-minimal coupling）$ξ_σ$会加剧等曲率（isocurvature）限制，而径向Peccei-Quinn场的非最小耦合$ξ_ρ$可以缓解这些限制。
...
### [81] [Impact of Supercooling on Direct Searches for Dark Matter and Gravitational Wave Backgrounds](https://arxiv.org/abs/2512.16809)
*Davide Racco,Alfredo Stanzione*

**TL;DR**: 研究发现宇宙相变中的超冷（supercooling）阶段通过改变膨胀历史和再加热熵注入影响暗物质（dark matter）最终能量占比，首次揭示该效应使弱相互作用大质量粒子（WIMP）候选参数空间从排除区移至允许区并提升冻结产生（freeze-in）候选者的实验可探测性，同时伴随的引力波背景低频特征为暗物质产生机制提供了互补性检验。


<details>
  <summary>Details</summary>

Background: 宇宙相变（cosmological phase transition）中的指数膨胀阶段（supercooling）及其对暗物质（dark matter）能量分数的影响是该研究的宏观科学背景。

Data: 研究数据来源于理论模拟（simulation）分析，结合了宇宙学相变（cosmological phase transition）中超冷却（supercooling）阶段的膨胀历史修正和再加热（reheating）熵注入效应。

Method: 采用修正的冻结-冻结动力学（freeze-out and freeze-in dynamics）分析暗物质在过冷阶段（supercooling）产生的额外效应。

Result: 超冷阶段（supercooling）的指数膨胀历史及再加热熵注入会显著改变暗物质的最终能量分数，尤其当暗物质产生在超冷阶段完成时，其动力学过程会叠加标准稀释因子之外的额外效应。
...
### [82] [Delayed Scaling of Multi-Type Cosmic F- and D-strings in VOS Models](https://arxiv.org/abs/2512.16821)
*Kazuto Nakamura,Masaki Yamada*

**TL;DR**: 研究纯旋量群（Spin(4N)）规范理论中宇宙F弦和D弦（作为色流管）的VOS模型，解析计算了弦密度随重连概率的标度关系，发现特定重连概率下标度时间极长，导致高频引力波信号特征性抑制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于纯旋量规范理论（Spin($4N$) gauge theory）中宇宙弦（F-string和D-string）作为色通量管（color flux tubes）的动力学行为及其引力波特征。

Data: 研究使用了数值计算(numerical calculations)和解析计算(analytical calculations)相结合的方法，其中数值计算部分基于自行开发的模拟程序对弦密度(scaling string density)与重连概率(reconnection probabilities)的关系进行验证。

Method: 采用速度依赖单尺度（VOS）模型分析纯Spin($4N$)规范理论中宇宙F弦和两条D弦作为色通量管的行为。

Result: 在纯自旋（Spin($4N$)）规范理论中，发现弦密度达到标度态的时间尺度可因重连概率的特定取值而显著延长，导致高频引力波信号出现特征性抑制。
...
### [83] [Multiple Axions in Laboratory Experiments](https://arxiv.org/abs/2512.16837)
*Arturo de Giorgi,Joerg Jaeckel,Sebastian Monath,Volodymyr Takhistov*

**TL;DR**: 研究提出多轴子（multiple axions）模型下轴子-光子振荡的通用公式，揭示多轴子通过相干和干涉效应显著改变实验观测信号（增强或抑制），并证明通过调整实验参数可识别多轴子贡献及其特性。


<details>
  <summary>Details</summary>

Background: 研究探讨了标准模型扩展中普遍存在的轴子（axion）和类轴子粒子（axion-like particles）的多重场谱系及其对实验探测的影响。

Data: 数据来源于理论模拟（simulation）分析，基于多轴子（multiple axions）与光子振荡的通用公式推导。

Method: 采用多轴子（multiple axions）模型分析轴子-光子振荡（axion-photon oscillations）并研究其对相干和干涉效应的影响。

Result: 多轴子（axion）存在会通过相干和干涉效应显著改变观测特征，既可能增强也可能抑制信号。
...
### [84] [Reconstruction of the dark matter density profile from cosmic positron anomaly data](https://arxiv.org/abs/2512.15741)
*K. M. Belotsky,F. V. Kostromin,M. L. Solovyov*

**TL;DR**: 通过逆向求解暗物质（DM）空间分布的反问题，首次数学验证了调整暗物质源分布可解释宇宙线正电子异常（PA）的可行性，并识别出一类解集。


<details>
  <summary>Details</summary>

Background: 研究探讨了通过改变湮灭或衰变暗物质(DM)的空间分布来解释宇宙射线中正电子异常(PA)的可能性。

Data: 研究中使用的数据来源为宇宙射线（cosmic rays）观测数据。

Method: 采用线性代数和自适应网格方法（adaptive grid methods）的独特算法调整暗物质（DM）空间分布以匹配观测数据。

Result: 通过数学方法构建满足观测数据的暗物质（DM）空间分布，证明了调整暗物质源的空间分布原则上可以解释宇宙线中的正电子异常（PA）。
...
### [85] [Pulsar Science with the SKA Observatory](https://arxiv.org/abs/2512.16152)
*Bhal Chandra Joshi,Aris Karastergiou,Marta Burgay,The SKA pulsar science working group*

**TL;DR**: 平方公里阵列射电望远镜（SKA-Low和SKA-Mid）凭借其高灵敏度、宽频段覆盖和多波束灵活观测能力，将彻底革新射电天文学（特别是脉冲星研究领域）。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于平方公里阵列（SKA）天文台的两台望远镜（SKA-Low和SKA-Mid）在射电天文学（radio astronomy）特别是脉冲星天文学（pulsar astronomy）领域的革命性潜力及其科学应用。

Data: 数据来源于SKA-Low和SKA-Mid两台望远镜的大瞬时灵敏度、宽频率覆盖、灵活观测模式及多波束天空扫描能力。

Method: 采用平方公里阵列(SKA)低频(SKA-Low)和中频(SKA-Mid)望远镜的大瞬时灵敏度、宽频覆盖、多波束灵活观测模式进行脉冲星科学研究。

Result: SKAO望远镜（SKA-Low和SKA-Mid）凭借其高瞬时灵敏度、宽频率覆盖范围、灵活观测模式及多波束能力，将成为射电天文学（尤其是脉冲星研究）领域的革命性工具。
...
### [86] [A Square Kilometre Array Pulsar Census](https://arxiv.org/abs/2512.16153)
...
**TL;DR**: 论文提出利用平方公里阵列(SKA)第一阶段(AA*构型)开展全天盲巡时，通过组合SKA1-Low和SKA1-Mid(重点覆盖银道面5°区域)的优化策略，预计可探测约10,000颗慢速脉冲星和800颗毫秒脉冲星(MSPs)，为研究中子星物理性质、致密物质状态方程等重大科学问题提供关键样本。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用平方公里阵列（SKA）开展全天盲巡脉冲星（pulsar）普查，以解决中子星（neutron star）物理特性、致密物质状态方程（dense-matter equation of state）、强场引力检验（strong-field gravity tests）和引力波天文学（gravitational wave astronomy）等重大科学问题。

Data: 数据来源于平方公里阵列（SKA）第一阶段（SKA1）的低频（SKA1-Low）和中频（SKA1-Mid）波段1和2的组合观测，采用全天空盲扫策略，重点覆盖银道面5°以内区域（SKA1-Mid）及更高纬度区域（SKA1-Low）。

Method: 采用两种互补方法（基于脉冲星种群认知、SKA在建元件性能及规避射频干扰等实际约束）对多种示意性巡天设计进行脉冲星探测数量预测，并结合SKA1低频段（Low）和中频段（Mid）的波段1和2进行优化组合。

Result: 采用SKA1-Low和SKA1-Mid波段1及2的复合巡天策略最优，其中Mid波段2集中于银道面区域。
...
### [87] [The SKAO Pulsar Timing Array](https://arxiv.org/abs/2512.16163)
...
**TL;DR**: 论文提出利用高灵敏度的平方公里阵列射电望远镜（SKAO）构建脉冲星计时阵（PTA），以探测纳赫兹频段引力波背景信号并研究其源天体，同时分析了需克服的天体物理噪声及提升探测灵敏度的技术方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用脉冲星计时阵列（Pulsar timing arrays, PTAs）探测纳赫兹频段的引力波（gravitational waves），并探索其在基础物理和天文学中的潜在应用。

Data: 数据来源于使用平方公里阵列射电望远镜（SKAO）对毫秒脉冲星（millisecond pulsars）进行长期计时观测，并结合其他南半球射电望远镜设施的数据。

Method: 采用脉冲星计时阵列（Pulsar Timing Array, PTA）技术结合甚长基线干涉测量（Very Long Baseline Interferometry, VLBI）提升探测灵敏度。

Result: SKAO望远镜凭借其比南半球其他设施高三到四倍的灵敏度，将显著提升毫秒脉冲星（millisecond pulsars）观测数量和时间精度，从而加速纳赫兹频段引力波背景（gravitational-wave background）的确证性探测并深入研究引力波源。
...
### [88] [Understanding the Neutron Star Population with the SKAO telescopes](https://arxiv.org/abs/2512.16156)
...
**TL;DR**: 利用SKAO望远镜大幅增加已知射电脉冲星（radio pulsars）数量并发现新类型中子星（neutron stars），推动中子星物理、引力理论及核物质状态方程（nuclear equation of state）等研究。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过SKAO望远镜扩展非吸积中子星（non-accreting neutron stars）的已知样本，以深化中子星物理、演化机制、引力理论检验及核物质状态方程（nuclear equation of state）等核心科学问题的理解。

Data: 数据来源于SKAO（Square Kilometre Array Observatory）望远镜的脉冲星巡天观测，包括实时搜索的大视场、多波束同步观测数据以及多子阵列形成的宽频段观测数据。

Method: 采用射电巡天观测（radio pulsar surveys）结合实时多波束（tied-array beams）搜索技术，并利用SKAO望远镜的高灵敏度、宽视场及多子阵（sub-arrays）能力进行中子星群体研究。

Result: SKAO望远镜将通过大幅增加已知中子星（neutron star）数量（包括双星或三星系统中的脉冲星）来促进引力理论和广义相对论的更高精度检验，并探索中子星质量分布。
...
### [89] [Probing neutron star interiors and the properties of cold ultra-dense matter with the SKAO](https://arxiv.org/abs/2512.16162)
...
**TL;DR**: 该论文通过分析中子星（neutron star）的全局与非全局观测特性（如质量、转动惯量、脉冲星自转突变等），结合平方公里阵列射电望远镜（SKA）的高灵敏度观测能力，为研究极端致密物质（cold, ultra-dense matter）性质和超流性提供了新约束方法，并探讨了暗物质（dark matter）与修正引力模型对相关约束的潜在影响。


<details>
  <summary>Details</summary>

Background: 中子星内部物质在远超核饱和密度（nuclear saturation density）的极端条件下保持低温及高中子-质子不对称性，为研究地球无法复现的超密物质（ultra-dense matter）物理提供了独特实验室。

Data: 数据来源包括射电望远镜（如SKA）对孤立中子星和双星系统中子星的观测，以及X射线望远镜和下一代引力波天文台的协同观测数据。

Method: 采用射电观测技术（如SKA的超高灵敏度、大规模巡天和子阵能力）结合X射线望远镜和引力波观测站等多设施协同，研究孤立中子星和双星系统的全局（质量、转动惯量、最大自转频率）与非全局（脉冲星自转突变和自由进动）观测特征。

Result: 中子星内部物质在远高于核饱和密度（nuclear saturation density）的极端条件下仍保持低温及显著的中子-质子不对称性，为研究地球无法复现的超密物质（ultra-dense matter）提供了独特实验室。
...
### [90] [Fastest or Significant: A Systematic Framework for Validating Global Minimum Variability Timescale Measurements of Gamma-ray Bursts](https://arxiv.org/abs/2512.16204)
*S. Bala,P. Veres,A. Goldstein,R. Sonawane,R. Samanta,S. Iyyani*

**TL;DR**: 该研究通过模拟建立了基于Haar小波的最小变时标（MVT）定量验证框架，发现多成分光变曲线中MVT反映的是统计显著性最高的结构而非最快本征时标，并提出了结合信噪比（SNR_MVT）的验证曲线来区分可靠测量值与上限值。


<details>
  <summary>Details</summary>

Background: 伽马射线暴（GRB）中心引擎的探测依赖于最小变时标（MVT）观测，但该指标易受统计噪声和分析方法干扰，导致对真实源变与人为假象的区分存在困难。

Data: 研究中使用的数据来源于综合模拟套件（comprehensive suite of simulations），用于建立基于Haar变换的最小变异性时标（MVT）测量的定量验证框架。

Method: 采用基于哈尔小波变换（Haar-based）的模拟分析方法，结合信噪比（SNR_MVT）与最小变时标（MVT）的定量关系，构建了经验性MVT验证曲线（MVT Validation Curve）并建立了测量结果分类流程。

Result: 最小变异性时标（MVT）在多成分光变曲线中反映的是区间内统计显著性最高的结构而非最快本征时标，因此可能收敛至中间值。
...
### [91] [Pulsars in Globular Clusters With the SKAO](https://arxiv.org/abs/2512.16154)
...
**TL;DR**: 球状星团（globular clusters）因其极高恒星密度成为X射线双星和射电脉冲星（radio pulsars）的高效产生地，利用平方公里阵列（SKA）望远镜可大幅提升脉冲星探测数量并研究致密物质、恒星演化和星团动力学历史。


<details>
  <summary>Details</summary>

Background: 研究聚焦于球状星团（globular clusters）作为高效产生X射线双星和射电脉冲星（radio pulsars）的天然实验室，及其在探索致密物质物理、恒星演化和银河系球状星团系统动力学历史中的独特价值。

Data: 数据来源为SKA-MID和SKA-LOW的射电望远镜观测数据。

Method: 采用射电望远镜阵列（SKA-MID和SKA-LOW）的定向波束（tied-array beams）技术进行深空脉冲星（pulsar）搜寻，并利用核心配置（AA*）及升级配置（AA4）提升探测效率。

Result: 球状星团（globular clusters）因其极高的恒星密度，是X射线双星和射电脉冲星（radio pulsars）的高效工厂，单位恒星质量下这些奇异天体的数量约为普通环境的1000倍。
...
### [92] [Hydrodynamic Evolution and Detectability of Nova Remnants in the Galactic Center](https://arxiv.org/abs/2512.16316)
*Zhao Su,Zhiyuan Li*

**TL;DR**: 通过流体动力学模拟和时变多波段辐射计算，研究发现银河系中心(Galactic center, GC)环境中新星(nova)遗迹在X射线、射电和帕邢-α(Paschen-α)波段的可探测性分别为6、44和51个模型，预测峰值光度分别为~10³² erg s⁻¹、~10³¹ erg s⁻¹和~10³⁶ erg s⁻¹，探测概率分别为20%、8%和18%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中心（Galactic center）新星遗迹的探测及其对理解该区域老星族（特别是激变变星（CVs））和星际介质特性的科学意义。

Data: 研究使用了钱德拉X射线天文台(Chandra)、甚大天线阵(VLA)和哈勃太空望远镜(HST)对银河系中心(GC)的现有观测数据。

Method: 采用流体动力学模拟（hydrodynamical simulations）计算时变多波段辐射以评估可探测性。

Result: 银河中心(Galactic center, GC)环境中新星遗迹在X射线、射电和帕邢-α(Paschen-α)波段的峰值光度分别可达~10^{32} erg s^{-1}、~10^{31} erg s^{-1}和~10^{36} erg s^{-1}，现有观测设备对其的探测概率分别为20%、8%和18%。
...
### [93] [A Tidal Disruption Event from an Intermediate-mass Black Hole Revealed by Comprehensive Multi-wavelength Observations](https://arxiv.org/abs/2512.16568)
...
**TL;DR**: 在矮星系中心发现了一个由中等质量黑洞(IMBH,约$(1-6) \times 10^5$倍太阳质量)引发的潮汐瓦解事件(TDE)，其独特的多波段连续耀发、X射线高态平台期和长达550天的延展上升期为IMBH的存在提供了有力证据。


<details>
  <summary>Details</summary>

Background: 潮汐瓦解事件(TDEs)为探测宽质量范围内休眠黑洞(BH)提供了新颖而强大的手段。

Data: 数据来源包括对矮星系中心特殊潮汐瓦解事件(TDE)的晚期光学、X射线和射电波段多波长观测数据。

Method: 采用多波段（光学、X射线、射电）观测数据与宿主星系性质标度关系相结合的综合分析方法。

Result: 在矮星系中心发现了一个由中等质量黑洞(IMBH)引发的潮汐瓦解事件(TDE)，其X射线在峰值后出现意外的高态平台期，结合持续至少550天的长时间上升过程，表明黑洞质量约为(1-6)×10^5太阳质量。
