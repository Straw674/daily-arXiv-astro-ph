...
**TL;DR**: 通过磁流体动力学(MHD)模拟和宇宙射线电子谱演化，揭示了活动星系核(AGN)喷流在类似英仙座星系团中产生的射电同步辐射特征，建立了喷流动力学、电子能谱与观测射电特性间的物理联系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）喷流在星系团环境中加速宇宙射线电子并产生射电同步辐射的物理机制。

Data: 研究中使用的数据来源于对类似英仙座星系团（Perseus-like galaxy cluster）中单个活动星系核（AGN）喷流爆发的磁流体动力学（MHD）模拟，并采用亚网格模型处理未解析喷流内部激波中宇宙线质子与电子的加速过程。

Method: 采用磁流体动力学(MHD)模拟结合亚网格模型处理未解析激波加速宇宙射线电子/质子，并利用Fokker-Planck求解器Crest追踪拉格朗日轨迹上的电子能谱演化，通过Crayon+计算非热辐射。

Result: 活动星系核(AGN)喷流通过未解析内激波加速宇宙线电子，其高动量段谱达到稳态斜率且归一化随时间递减，而低动量段保持自由冷却谱特征。
...
**TL;DR**: 通过高分辨率射电观测发现，NGC 4522星系在团内介质(ICM)作用下表现出射电连续谱不对称分布、谱指数从主盘(α∼-0.6)到外盘(α∼-1.1)系统性陡峭以及超平面云具有异常平坦谱指数(α∼-0.2至0)的特征，揭示了ICM激波重加速电子、热辐射显著减少以及ICM与星际介质(ISM)混合增强热辐射的物理机制，同时偏振辐射分布表明ICM风强烈影响星系磁场的空间结构。


<details>
  <summary>Details</summary>

Background: 研究聚焦于室女座星系团中受星系团内介质(ICM)影响的射电剥离星系NGC 4522，通过多波段射电连续谱观测探究其磁场、谱指数及热与非热辐射组分在ICM作用下的演化特征。

Data: 数据来源于甚大阵(VLA)在S波段(3 GHz)和X波段(10 GHz)的高分辨率连续谱观测。

Method: 采用甚大天线阵(VLA)在S波段(3 GHz)和X波段(10 GHz)进行高分辨率连续谱观测，结合谱指数分析和偏振测量技术研究磁场特性。

Result: NGC 4522的射电连续谱、谱指数和磁场特性在星系团内介质(ICM)影响下呈现不对称分布，谱指数从主盘的$α\sim-0.6$系统性地陡化至外盘的$α\sim-1.1$。
...
**TL;DR**: 该论文系统总结了星际介质中尘埃颗粒热连续谱辐射的物理机制，提供了基于观测强度与流量密度计算尘埃质量、柱密度、温度等物理参数的实用方法，并更新了不同天体环境（如弥散ISM、致密核、原行星盘）的尘埃不透明度参数及平均分子量（1.404每H原子、2.809每H2分子、2.351每气体粒子）。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于星际介质(ISM)中尘埃颗粒的热连续谱辐射物理机制及其观测特性。

Data: 数据来源包括不同天体物理环境（如弥散星际介质、致密核、原行星盘）的尘埃不透明度特性，以及过去、当前和未来测辐射热计相机波长的观测数据。

Method: 基于热尘埃连续谱辐射（thermal dust continuum emission）物理模型，通过推导强度与流量密度观测值计算尘埃质量、柱密度、光度、温度及不透明度谱指数（dust opacity spectral index），并针对不同天体环境（如弥散星际介质、致密核、原行星盘）的尘埃不透明度特性进行讨论与列表。

Result: 推导了适用于不同天体物理环境（如弥散星际介质、致密核、原行星盘）的尘埃不透明度（dust opacity）特性，并针对过去、当前和未来测辐射热计（bolometer）相机的波长范围进行了列表汇总。
...
**TL;DR**: 利用COSMOS HI大尺度河外巡天(CHILES)观测了红移0.22-0.47的四个星系的中性氢(HI)形态，发现高红移下分子氢(H2)与中性氢的质量比(H2/HI ratio)均值(10.3±3.4)显著高于本地宇宙。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用CHILES巡天数据探究红移0.22至0.47范围内星系中性氢（HI）和分子氢（H2）的形态学特征及其质量比（H2/HI ratio）的宇宙学演化。

Data: 数据来源于COSMOS HI Large Extragalactic Survey (CHILES)的中性氢(HI)观测、LMT的CO(1-0)谱线观测以及JWST的NIRCam四波段组合成像。

Method: 采用COSMOS HI大尺度河外巡天（CHILES）的7.5角秒波束和43千秒差距线性尺度解析中性氢（HI）形态，并结合LMT的CO(1-0)观测确认红移并推导分子氢（H2）质量，同时利用JWST四波段NIRCam成像揭示恒星成分扰动。

Result: 在最高红移(z=0.47)处观测到的平均H2/HI比值(10.3 ± 3.4)显著高于本地宇宙的平均值。
...
**TL;DR**: 采用深度学习（ResNet-18）从DESI Legacy Imaging Surveys中筛选出485个高置信度星系团强引力透镜（galaxy cluster-scale strong gravitational lensing）候选体，其中247个为新发现，包括16个A级、90个B级和141个C级候选体。


<details>
  <summary>Details</summary>

Background: 星系团尺度强引力透镜系统（galaxy cluster-scale strong gravitational lensing systems）是研究暗物质（dark matter）和暗能量（dark energy）特性以及探索遥远宇宙的稀有但重要工具。

Data: 数据来源于DESI Legacy Imaging Surveys（DESI传统成像巡天）和Zou等人（2021年）的星系团候选体目录。

Method: 采用基于深度学习（deep learning）的ResNet-18架构，结合迭代训练策略和人工检查（human inspection）对候选样本进行分级筛选。

Result: 基于深度学习方法和迭代训练策略，从540,432个星系团候选体中筛选出485个高置信度强引力透镜候选体（含247个新发现样本），其质量范围$M_{500}$为$10^{13.67\sim14.97}M_{\odot}$，中心星系红移覆盖0.04至0.89。
...
**TL;DR**: 通过分析1.4 < z < 7红移范围内763个星系和JADES DR3复合光谱的关键静止光学发射线比，发现z∼3.5时恒星形成星系在固定气体相金属丰度下比z∼2具有更硬的电离光谱，导致BPT图中[N II]/Hα和O32-R23图上的显著偏移，且z≳3.5时星系电离条件趋于稳定，其演化模式与银河系恒星α/Fe化学丰度模式一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用静止光学发射线比率分析高红移(1.4 < z < 7)星系中星际介质(ISM)的电离条件演化及其与大质量恒星化学丰度模式的关联。

Data: 数据来源于JADES DR3（JWST Advanced Deep Extragalactic Survey Data Release 3）的763个星系样本及其复合光谱。

Method: 采用多种静止光学发射线比值（[O III]λ5008/Hβ、[N II]λ6585/Hα、[S II]λλ6718,6733/Hα、[O I]λ6302/Hα、O32、R23、Ne3O2和RO2Ne3）分析星系样本及JADES DR3复合光谱。
...
**TL;DR**: 利用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)对20 km/s云中两个大质量团块(Clump 1和Clump 4)的870微米偏振连续谱观测表明，磁场在云尺度上占主导地位，而重力在核心(0.01-0.1 pc)和凝聚体(≤0.01 pc)尺度上起主要作用，且磁场张力不足以阻止气体向致密核心下落。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)观测20 km/s云中两个大质量团块(Clump 1和Clump 4)的870微米偏振连续谱发射，探讨磁场与引力在分子云不同尺度结构形成中的相对作用。

Data: 数据来源于阿塔卡马大型毫米波/亚毫米波阵列(ALMA)对20 km/s云中两个大质量团块(Clump 1和Clump 4)的870微米线性偏振连续谱观测，空间分辨率约为0.2角秒(2000天文单位)。

Method: 采用角分散函数（ADF）方法分析ALMA观测到的870微米连续谱线偏振数据，以推导磁场强度。
...
**TL;DR**: 在再电离时期(EoR)的透镜星系A2744-z7DLA（红移z≈6.87）中首次发现同时存在莱曼阿尔法(Lyα)发射和阻尼莱曼阿尔法吸收(DLA)，揭示了其致密中性氢环境、2.90×10⁷太阳质量的黑洞活动以及恒星形成区主导的紫外-光学辐射特征。
...
Data: 研究使用了来自JWST（詹姆斯·韦伯太空望远镜）和HST（哈勃太空望远镜）的光谱与测光数据。

Method: 采用莱曼阿尔法(Lyα)吸收谱线分析和光谱能量分布(SED)拟合方法，结合JWST和HST的光谱与测光数据，研究了A2744-z7DLA的物理特性。

Result: 在再电离时期(EoR)的恒星形成星系中，莱曼阿尔法(Lyα)吸收剖面能有效示踪其周围延展致密中性氢环境。
...
**TL;DR**: 研究发现邻近星系中心环（central rings）的分子气体质量和恒星形成率与其宿主星系整体性质（如棒旋结构形态）相关，其中银河系中央分子区（MW CMZ）在相对参数上与PHANGS样本中心环相似，表明邻近星系中心环的气体流入和恒星形成过程具有共性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近邻星系中心环（central rings）的分子气体含量与恒星形成率，探讨其与宿主星系整体性质（如棒旋结构形态）的关联。

Data: 研究使用了分辨率达1角秒的PHANGS-ALMA CO(2-1)观测数据，并从中目视识别出20个中心环结构（central rings），其中14个环具有可用的恒星形成率（SFR）表面密度图。

Method: 采用1角秒分辨率的PHANGS-ALMA CO(2-1)观测数据，通过目视识别20个中心环并测定其几何参数、分子气体质量、恒星形成率及消耗时间等物理量。

Result: 中心环（central rings）的典型半径为$400_{-150}^{+250}\,$pc，分子气体质量为$\log(M_\text{mol}/M_\odot){\sim}8.1_{-0.23}^{+0.17}$，恒星形成率为$0.21_{-0.16}^{+0.15}\,M_\odot/\text{yr}$，分别贡献宿主星系分子气体质量和恒星形成率的$5.6_{-2.1}^{+4.5}\,\%$和$13_{-5}^{+10}\,\%$。
...
**TL;DR**: 通过对SSA22深场221个850微米选亚毫米星系(SMG)的光谱能量分布(SED)、恒星形成历史(SFH)和红外-射电相关性(IRRC)的分析，发现这类星系中位质量加权年龄为567 Myr，主要在宇宙大爆炸后约1.68 Gyr开始形成，经历约1 Gyr演化后进入SMG主导观测阶段，并在随后约0.2 Gyr内转变为静止星系，其中部分大质量星系表现出早期快速组装和高恒星形成效率(0.2-0.8)，且样本星系对宇宙恒星形成率密度和恒星质量密度的贡献在红移z=2.5-3.5时达到峰值(50-60%)，同时揭示850微米巡天可能遗漏红移z≥1-2的暗弱温暖星系群体，并测得中位红外-射电相关参数qIR为2.37且随红移呈(1+z)^-0.11演化。


<details>
  <summary>Details</summary>

Background: 研究聚焦于亚毫米波星系（SMGs）的光谱能量分布（SEDs）、恒星形成历史（SFHs）及红外-射电相关性（IRRC），探讨其在宇宙早期（大爆炸后约1.68 Gyr）的快速形成与演化机制，并揭示质量与恒星形成效率（SFE）对星系成熟时间的调控作用。

Data: 数据来源于SSA22深场中221个850微米波段选出的亚毫米星系（SMGs）的光谱能量分布（SEDs）、恒星形成历史（SFHs）及红外-射电相关性（IRRC）观测数据。

Method: 采用光谱能量分布(SED)拟合、恒星形成历史(SFH)重建和红外-射电相关性(IRRC)分析技术。

Result: 大多数亚毫米波星系(SMG)始于宇宙大爆炸后约1.68吉年，经历约1吉年演化后进入"SMG阶段"，并在约0.2吉年内转变为静止星系。
...
**TL;DR**: 在SSA22天区对850微米选亚毫米星系（SMGs）的多波段分析发现，这些高红移（中值z=2.00）星系具有极高红外光度（中值2.25×10¹²L⊙）和恒星形成率（166M⊙/年），是宇宙结构形成的可靠示踪体，并展现出质量依赖的“降级形成”演化特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用多波段分析SSA22天区850微米选亚毫米星系(SMG)的物理性质及其在大尺度结构中的分布特征，探讨高红移宇宙结构形成与星系演化规律。

Data: 数据来源于JCMT/SCUBA-2在SSA22天区的最深观测（灵敏度$σ_{850}\sim$ 0.80mJy beam$^{-1}$），通过多重识别方法从192个SCUBA-2源中确认了248个去混叠的亚毫米星系（SMG）候选体。

Method: 采用多波长分析方法，结合JCMT/SCUBA-2观测和SED（光谱能量分布）拟合技术对SSA22天区的亚毫米星系（SMG）进行解混和统计分析。

Result: SSA22区域中850微米选亚毫米星系(SMG)的中位红外光度为(2.25±0.25)×10¹² L⊙，其中约63%为超亮红外星系(ULIRG)，约8%为极亮红外星系(HLIRG)。
...
**TL;DR**: 河外星系行星状星云（xPNe）作为离散示踪天体，能直接探测100兆秒差距内星系晕和星系际光的元素丰度与运动学特征，并为改进渐近巨星分支后恒星演化模型提供年龄与金属丰度标定。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用河外星系行星状星云（xPNe）作为离散示踪体，探索不同形态和质量星系的元素丰度与运动学特征，并揭示星系晕及星系团内光的形成机制。

Data: 数据来源于河外星系行星状星云（xPNe）的观测，利用10米级望远镜的新型宽视场光谱仪和全景积分场光谱技术进行探测。

Method: 采用全景积分视场光谱（panoramic integral-field spectroscopy）技术对河外星系行星状星云（xPNe）进行直接探测和特征分析。

Result: 河外星系行星状星云（xPNe）是研究不同形态和质量星系从中心到晕（halo）的元素丰度与运动学的离散示踪剂，可直接探测星系晕和星系团内光（intracluster light）的前身星系。
...
**TL;DR**: 研究发现恒星形成致密星系群（SFCGs）中的星系相较于场星系具有更高的比恒星形成率（sSFR）和16%的合并特征比例，且合并活动促进恒星形成，表明SFCGs代表致密星系群演化早期阶段。


<details>
  <summary>Details</summary>

Background: 研究聚焦于前处理（pre-processing）场景下致密星系群（Compact Groups）中星系的环境效应，特别是恒星形成致密群（Star-Forming Compact Groups）作为蓝移群（Blue Infalling Group）类似体的物理与形态演化特征。

Data: 数据来源于DECaLS巡天图像，通过GALFITM提取形态参数（如Sérsic指数n和有效半径Re）以及非参数指标（基尼系数、M20、不对称性），并利用测光技术获取恒星质量和恒星形成率（SFRs）。

Method: 采用测光技术（photometric techniques）测量恒星质量和恒星形成率（SFRs），结合DECaLS图像提取形态参数（包括GALFITM拟合的Sérsic指数n和有效半径Re，以及非参数指标Gini系数、M20和不对称性），用于星系分类（E/S0/Sa、Sb/Sc/Ir和并合类型）。

Result: 在恒星形成致密星系群(SFCGs)中，星系表现出比场星系更高的特定恒星形成率(sSFR)，且约16%的星系呈现合并特征并伴随不对称性增强，合并过程促进了恒星形成而非抑制。
...
**TL;DR**: 通过分析四个邻近星系(NGC 628、NGC 4449、M51和M83)中年轻星团(YSCs)的角两点相关函数(TPCF)，发现新形成红外星团(eYSCs)和光学星团(oYSCs)在10 Myr内呈现显著层级结构，且NGC 628中eYSCs与oYSCs的TPCF差异表明随机化时间更短，而10-300 Myr星团空间分布逐渐随机化，最年轻星团的分形维数$D_2$为1.3，支持普适恒星形成过程。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用角两点相关函数（TPCF）分析近邻恒星形成星系中年轻星团（YSCs）的空间分布，以揭示其潜在的层级结构特征。

Data: 数据来源包括JWST红外数据识别出的新生阶段年轻星团(eYSCs)和HST档案光学数据探测到的年轻星团(YSCs)，涵盖NGC 628、NGC 4449、M51和M83四个邻近恒星形成星系。

Method: 采用角两点相关函数（TPCF）分析年轻星团（YSCs）的空间分布，并计算二维分形指数（fractal index）D₂以量化其层级结构特征。

Result: 年轻星团（YSCs）在空间分布上表现出明显的层级结构特征，尤其是红外波段新发现的eYSCs和年龄小于10 Myr的光学波段oYSCs，其两点相关函数（TPCF）在各星系内相似，而10至300 Myr的星团空间分布逐渐随机化，约100 Myr后完全随机；最年轻YSC群体的二维分形指数（D_2）拟合值为1.3，与普适恒星形成过程的理论预期更吻合。
...
**TL;DR**: 通过奥瑞加(Auriga)宇宙流体动力学模拟研究发现，重子物质使银河系亚晕(subhalo)数量减少1.9-2.4倍、最大圆周速度表征的浓度降低约1.5倍，显著影响暗物质(DM)间接探测约束（脆弱亚晕情形下湮灭截面限制放宽3.6倍），而亚晕抗潮汐剥离能力越强则约束改善越明显（相比纯暗物质模拟和流体动力学脆弱情形分别提升4.5倍和10倍）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于重子物质（baryons）对银河系尺度系统中子晕（subhalos）群体的影响及其在暗物质（DM）间接探测中的关键作用。

Data: 使用了Auriga——一套最先进的银河系尺度系统的宇宙学流体动力学模拟（cosmological hydrodynamical simulations）——及其对应的纯暗物质（DM-only）模拟对比数据，并基于Auriga数据在可解析子晕质量范围内的详细特征，补充了远低于质量分辨率极限的低质量子晕。

Method: 采用Auriga宇宙学流体动力学模拟（cosmological hydrodynamical simulations）对比纯暗物质（DM-only）模拟，通过重新填充低质量子晕（subhalos）并考虑两种潮汐剥离抗性（‘fragile’和‘resilient’）模型，量化重子物质对银河系子晕数量及浓度（以最大圆周速度定义）的影响。
...
**TL;DR**: 在Abell 370透镜星系团后方发现了一个高放大率、低质量伪小红点星系（Pseudo-LRD-NOM，z=5.96），其光谱特征表明存在一个活跃黑洞（M_BH=2.9×10^6 M☉）和极端低金属丰度（Z=0.01-0.1 Z☉）的高密度（n_H≥10^6 cm⁻³）尘埃星暴，可能是真实小红点星系（LRD）的前身。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(z=5.96)低质量星系Pseudo-LRD-NOM（伪无金属线小红点）的极端物理条件及其与早期宇宙中黑洞-星族共演化（co-evolution）的关系。

Data: 数据来源于JWST（James Webb Space Telescope）的NIRSpec（Near-Infrared Spectrograph）光谱观测和Abell 370透镜星系团的引力透镜效应放大后的低质量星系Pseudo-LRD-NOM。

Method: 采用JWST（James Webb Space Telescope）近红外光谱仪（NIRSpec）分析Halpha和Hbeta发射线特征，结合高气体密度（n_H >~ 10^6 cm^{-3}）、低金属丰度（Z = 0.01-0.1 Zsun）及适度色余（E(B-V)=0.18-0.45）的恒星形成爆发模型解释光谱特性。

Result: 在z=5.96处发现了一个高放大率低质量星系Pseudo-LRD-NOM，其红移光学颜色主要由JWST NIRSpec光谱中显著的Halpha线（EW0 >~ 800埃）驱动，并包含窄成分和宽成分，表明存在一个质量为2.9x10^6 Msun的活动黑洞。
...
**TL;DR**: 首次将卡皮查势（Kapitza potential）与玻色-爱因斯坦凝聚态（BEC）暗物质模型结合，通过薛定谔-泊松方程基态解构建星系内核与尾部结构，模型与SPARC星系旋转曲线观测数据吻合。


<details>
  <summary>Details</summary>

Background: 研究聚焦于玻色-爱因斯坦凝聚态(BEC)在卡皮查势(Kapitza potential)实验实现与暗物质(dark matter)天体物理模型之间的交叉领域。

Data: data：研究使用了SPARC星表（SPARC catalogue）中的代表性星系旋转曲线数据。

Method: 采用薛定谔-泊松方程(Schroedinger-Poisson)基态平衡解构建星系核心区密度分布，并在外围区域引入类卡皮查势(Kapitza potential)的玻色-爱因斯坦凝聚体(BEC)相互作用模型。

Result: 首次在玻色-爱因斯坦凝聚体(BEC)中实验实现卡皮查势(Kapitza potential)后，该研究通过结合薛定谔-泊松方程的基态平衡解与类卡皮查-BEC相互作用，构建了能同时描述星系核心区(inner cored profile)和外围区域(tail region)的暗物质模型。
...
**TL;DR**: 利用JWST/NIRSpec PRISM观测确认了红移(z=13.53)的极亮(M_UV=-20.6)星系PAN-z14-1，其大物理尺寸(r_c=233pc)和缺乏强发射线的特征支持了高红移(z>10)存在两类星系(高恒星形成率密度致密星系与普通延展星系)的观点。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（詹姆斯·韦伯太空望远镜）观测和确认高红移（$z>10$）星系候选体的物理特性，以探索宇宙早期星系的多样性和演化规律。

Data: 数据来源为JWST（James Webb Space Telescope）的NIRSpec（Near-Infrared Spectrograph）PRISM模式和纯并行（pure-parallel）NIRCam（Near-Infrared Camera）成像观测。

Method: 采用JWST/NIRSpec PRISM观测和莱曼断裂（Lyman-α break）建模分析技术确认红移（redshift）并测量物理参数。

Result: PAN-z14-1被确认为红移(redshift)z=13.53的极遥远星系，具有高光度(M_UV=-20.6±0.2)、蓝紫外连续谱斜率(β=-2.26±0.08)和大物理尺度(r_c=233±10 pc)，且未检测到显著紫外发射线，其性质与GS-z14-0相似，支持z>10星系存在两种不同星族(star-forming galaxy populations)的观点。
...
**TL;DR**: 提出混合高斯过程与模板拟合的方法，在欧几里得(Euclid)任务的光谱能量分布(SED)重建中满足大部分红移区间的点扩散函数(PSF)精度要求，并通过新型中等带宽滤波器提升重建准确性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升欧几里得(Euclid)任务中弱引力透镜测量精度，通过优化星系光谱能量分布(SED)重建方法以校正空间衍射极限点扩散函数(PSF)对星系形状测量的影响。

Data: 数据来源包括欧几里得(Euclid)任务中的宽波段和中波段辅助测光数据，以及斯巴鲁(Subaru)望远镜新增的16个均匀分布的中波段滤光片提供的准光谱覆盖数据。

Method: 采用基于模板的流量校正和高斯过程（Gaussian processes）的物理与数据驱动混合方法，并引入光谱能量分布（SED）度量来评估其对点扩散函数（PSF）四极矩误差的影响。

Result: 高斯过程（Gaussian processes）和模板拟合仅在特定但互补的红移区间满足要求，因此提出了一种结合两者的混合方法，有效覆盖了欧几里得（Euclid）巡天的大部分红移范围。
...
**TL;DR**: 通过对高红移星系团SPT-CL J2215-3537（z=1.16）的再分析，首次在z>1时检测到核心晃动冷锋（core-sloshing cold fronts）和幽灵空腔（ghost cavity）候选体，揭示其处于冷却反馈循环（cooling-feedback cycle）前的短暂动态活跃阶段，并发现传统X射线形态参数在高红移下会因表面亮度陡度（surface brightness cuspiness）的非自相似演化导致动力学分类偏差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移星系团SPT-CL J2215-3537（z=1.16）的动力学状态重分析，探讨其星系团内介质（intracluster medium）的热力学与形态学特征演化及其对高红移动态分类的影响。

Data: 使用钱德拉（Chandra）观测的全部可用数据对星系团SPT-CL J2215-3537的团内介质（intracluster medium）热力学和形态学特性进行了表征。

Method: 采用钱德拉（Chandra）X射线观测数据的热力学和形态学分析方法，结合核心晃动冷锋（core-sloshing cold fronts）和幽灵空腔（ghost cavity）候选体的识别技术，重新计算了X射线形态学参数并评估了星系团动力学状态。

Result: 在红移z=1.16的高红移星系团SPT-CL J2215-3537中首次探测到一对核心晃动冷锋（core-sloshing cold fronts），并发现其处于辐射冷却显著超过活动星系核反馈加热的短暂过渡阶段。
...
**TL;DR**: 研究发现早期暗能量（EDE）模型能解释JWST观测到的高红移（$z \sim 11-6$）星系过量现象，并在仅需中等莱曼连续谱光子逃逸率（$f_{\rm esc}\sim 0.05-0.1$）和紫外光度密度（$M_{\rm UV}\lesssim -17$至$-15$）条件下，使再电离历史与现有约束一致。


<details>
  <summary>Details</summary>

Background: 研究探讨了早期暗能量（EDE）范式下宇宙再电离（reionization）的历史及其与高红移星系观测的关联。

Data: 数据来源为JWST（James Webb Space Telescope）对红移$z \sim 11-6$的莱曼α（Lyman-$α$）发射线的观测结果。

Method: 采用早期暗能量（EDE）范式下的再电离历史建模方法，结合适度莱曼连续谱（LyC）逃逸分数和紫外光度密度约束。
...
**TL;DR**: 基于ZTF巡天Ia型超新星(SNe Ia)观测数据，开发了测量哈勃常数(H0)各向异性(anisotropy)的方法，能准确恢复模拟数据中3 km/s/Mpc的偶极振幅(dipole amplitude)，误差为0.33 km/s/Mpc，方向误差为3.4°(赤经)和6.1°(赤纬)。


<details>
  <summary>Details</summary>

Background: 当前宇宙学标准模型（ΛCDM模型）基于宇宙学原理（cosmological principle）假设大尺度宇宙各向同性，但近期观测张力促使研究者探索宇宙可能存在的各向异性（anisotropies）。

Data: 研究使用了兹威基瞬变设施巡天（ZTF）第二版数据发布（DR2）中的Ia型超新星（SNe Ia）观测数据，并在模拟中人为引入了偶极（dipole）结构以测试方法的有效性。

Method: 采用基于ZTF Ia型超新星(SNe Ia)第二期数据(DR2)的模拟分析方法，开发了无偏置的偶极子(dipole)引入与恢复技术，并构建了针对偶极子振幅(ΔH₀)及其方向(α₀, δ₀)的误差模型。

Result: 开发了一种测量哈勃常数（H0）各向异性的无偏方法，能够以0.33 km/s/Mpc的精度恢复初始振幅为3 km/s/Mpc的偶极信号，且结果与H0取值和天区覆盖无关。
...
**TL;DR**: 提出基于高斯过程回归(GPR)的BOBE方法，通过贝叶斯优化构建快速替代似然函数，将贝叶斯证据计算所需似然评估次数减少约10^3倍，特别适用于计算成本高昂的宇宙学似然分析。


<details>
  <summary>Details</summary>

Background: 研究主题为基于贝叶斯模型选择（Bayesian model selection）框架，通过构建高斯过程回归（Gaussian Process Regression）模拟器来高效计算贝叶斯证据（Bayesian evidence），以解决传统方法在计算高成本似然函数时的效率瓶颈问题。

Data: 研究中使用的数据来源包括模拟数据（simulation data）和实际宇宙学似然函数（actual cosmological likelihoods）。

Method: 采用高斯过程回归（GPR）构建快速模拟似然函数，结合贝叶斯优化算法以减少计算成本并评估模拟器不确定性对证据计算的影响。

Result: 提出了基于高斯过程回归（GPR）的BOBE方法，通过贝叶斯优化显著减少计算贝叶斯证据所需的似然函数评估次数，在保证精度的前提下将计算量降低至直接嵌套采样方法的千分之一。
...
**TL;DR**: 通过分析费米卫星(Fermi-LAT)15年数据与KiDS-Legacy弱引力透镜的交叉关联，首次在0.5-1000 GeV能段对弱相互作用大质量粒子(WIMP)的衰变率Γ_dec和湮灭截面〈σ_ann v〉给出了95%置信水平上限，并预测欧几里得(Euclid)透镜巡天将使限制精度提高约2倍。


<details>
  <summary>Details</summary>

Background: 通过分析未解析伽马射线背景(UGRB)与弱引力透镜的互相关来约束暗物质(dark matter)性质。

Data: 使用费米大面积望远镜（Fermi-LAT）15年观测数据构建了十个能量区间（0.5-1000 GeV）的未解析伽马射线背景（UGRB）强度图，并与六个层析区间的KiDS-Legacy弱引力透镜剪切数据交叉关联。

Method: 采用伪-$C_\ell$方法（pseudo-$C_\ell$ method）分析费米-LAT（Fermi-LAT）伽马射线强度图与KiDS-Legacy弱引力透镜切变（shear）的角功率谱（angular power spectra）。

Result: 通过费米-LAT（Fermi-LAT）与KiDS-Legacy（KiDS-Legacy）剪切数据的交叉关联分析，未发现暗物质（WIMP）衰变或湮灭的显著信号，并给出了95%置信水平的上限约束。
...
**TL;DR**: 研究了动量传递的暗能量（dark energy）和暗物质（dark matter）耦合对星系团数量统计的影响，发现动量传递增强会抑制线性密度外推值（extrapolated linear density contrast），导致高红移区星系团数量增加，但统计结果显示该模型与标准ΛCDM框架的拟合差异不显著（p=0.189）。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗能量（dark energy）与暗物质（dark matter）间存在动量转移耦合时对星系团数量统计的影响。

Data: 数据来源于eROSITA巡天观测结果和基于球塌缩模型（spherical collapse model）及Sheth-Tormen质量函数（Sheth-Tormen mass function）的理论预测。

Method: 采用球塌缩模型（spherical collapse model）计算外推线性密度对比度，并结合Sheth-Tormen质量函数（Sheth-Tormen mass function）计算星系团数量统计。

Result: 动量耦合暗能量模型下星系团数量在高红移区间达到峰值，且随动量和能量转移强度增加而增多，但统计上未显著优于标准ΛCDM模型。
...
**TL;DR**: 研究发现振荡子（oscillon）快速衰变会增强诱导引力波产生，其能量密度可达相对论性粒子有效数目的观测上限，从而为允许振荡子形成的暴胀子（inflaton）质量、三次和四次自耦合参数提供了宇宙微波背景观测无法单独约束的新限制。


<details>
  <summary>Details</summary>

Background: 研究探讨了暴胀后宇宙中振荡子（oscillon）的形成及其对早期物质主导阶段和引力波产生的潜在影响。

Data: 数据来源于数值模拟（simulation）分析振荡子（oscillon）形成和衰变过程中的引力波产生。

Method: 采用数值模拟和解析计算相结合的方法，重点分析振荡子（oscillon）衰变过程中诱导引力波的产生机制，并基于相对论性粒子有效种类数（effective number of relativistic species）的观测限制反推暴子（inflaton）质量、三次和四次自耦合参数。

Result: 振荡子（oscillon）的快速最终衰变导致诱导引力波（induced gravitational waves）的增强产生，其能量密度可达到相对论性粒子有效数目的观测上限。
...
**TL;DR**: 加纳射电天文台（GRAO）通过将退役的32米卫星通信天线改造成具备甚长基线干涉测量（VLBI）能力的射电望远镜，实现了非洲射电天文学的重大突破，其近全天空覆盖范围（-77度至+88度赤纬）、高孔径效率（>77%）及早期科学观测结果验证了该设施在时域天文学、瞬变源巡天和全球VLBI网络中的价值。


<details>
  <summary>Details</summary>

Background: 加纳射电天文台（GRAO）通过将退役的32米卫星通信天线改造为具备甚长基线干涉测量（VLBI）能力的射电望远镜，推动了非洲射电天文学发展并支持平方公里阵列（SKA）前期准备。

Data: 数据来源包括加纳射电天文台(GRAO)的32米天线实测数据、高分辨率6.7 GHz甲醇脉泽谱线观测数据、脉冲星PSR J0835-4510计时数据以及跨洲甚长基线干涉(VLBI)条纹探测数据。

Method: 采用双极化C波段接收机（5和6.7 GHz）、氢脉泽、宽带ROACH2系统及改进的控制与指向基础设施进行射电望远镜升级与性能验证。

Result: 加纳射电天文台（GRAO）通过将退役的32米卫星通信天线改造为具备科学观测能力且支持甚长基线干涉测量（VLBI）的射电望远镜，显著推动了非洲射电天文学发展。
...
**TL;DR**: 研究对象为LSST（大型综合巡天望远镜）星系测光红移（photo-z）估计方法，主要发现是训练集不完整性和非代表性会导致估计偏差，并确定了KL散度（Kullback-Liebler Divergence）、Wasserstein距离和概率积分变换（Probability Integral Transform）是评估训练集缺陷影响的最有效指标。


<details>
  <summary>Details</summary>

Background: 研究聚焦于评估训练集不完整性（training set imperfection）对LSST（大型综合巡天望远镜）星系测光红移（photo-$z$）估计方法性能的影响及其诊断指标。

Data: 数据来源于通过开源Python库Redshift Assessment Infrastructure Layers (RAIL)系统测试的模拟训练数据，这些数据根据多个现有光谱巡天（spectroscopic sky surveys）以及高红移处观测到的不完整性模式近似模拟的反向红移不完整性（inverse redshift incompleteness）条件进行了降质处理。

Method: 采用开源Python库Redshift Assessment Infrastructure Layers (RAIL)中的方法，系统测试了CMNN、GPz、FlexZBoost和PZFlow算法在不同光谱巡天模拟训练数据及高红移逆红移不完整性条件下的表现，并以TrainZ作为对照。

Result: 通过系统评估多种统计指标，发现Kullback-Liebler散度、Wasserstein距离和概率积分变换最能有效量化训练集缺陷对光红移(photo-z)算法性能的影响。
...
**TL;DR**: 提出了一种通过共享相邻光谱像素信息来表征时变观测的新统计模型，能无偏且最优地提取地面近红外（NIR）探测器在光子到达率变化超过3.5%时的真实通量和时变信号，解决了忽略时变性导致的通量依赖性偏差问题，并在APOGEE光谱数据中验证了该模型对云量变化等观测条件引起的大于3.5%幅度波长无关时变的有效校正能力。


<details>
  <summary>Details</summary>

Background: 近红外（NIR）探测器在现代天体物理学中至关重要，但地面望远镜观测中短时标大气变化导致传统恒定流量假设失效，需开发新方法处理时变信号。

Data: 数据来源包括使用多种通量和时间变异性幅度条件生成的合成数据（synthetic data）以及真实的APOGEE光谱（APOGEE spectra）。

Method: 采用基于邻近光谱像素信息共享的统计模型（statistical model）来表征时变观测并提取无偏通量（unbiased fluxes）。

Result: 提出了一种利用相邻光谱像素共享信息的新统计模型，能有效表征时变观测并提取无偏通量（flux）和最优不确定度。
...
**TL;DR**: 提出基于辐射波束图直接推导均匀相控阵有源阻抗（active impedance）的新方法，将阻抗随扫描角的变化与波束固有角度变化直接关联，为相控阵设计提供更直观的物理理解和计算框架。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于改进大型均匀相控阵天线(phased array antennas)中主动阻抗(active impedance)的计算方法，以解决传统方法物理直观性不足和计算量大的问题。

Data: 数据来源于对原型相控阵（phased array）的全波仿真（full-wave simulation）。

Method: 提出了一种基于辐射波束方向图直接推导有源阻抗(active impedance)的新方法，建立了扫描角度变化与波束固有角度变化之间的直接映射关系。

Result: 提出了一种基于辐射波束方向图直接推导有源阻抗(active impedance)的新方法，将阻抗随扫描角的变化映射到波束固有角度变化，为相控阵设计提供了更直观的物理解释框架。
...
**TL;DR**: 该论文核心贡献是为下一代气球和空间望远镜实验开发硅光电倍增管(Silicon Photo-Multipliers)探测器，通过建立硅光电倍增管(以Hamamatsu S13161-3050AE-08为例)在-40°C至30°C温度范围内的性能表征协议，并设计专用测试系统验证将用于POEMMA-Balloon任务切伦科夫相机(Cherenkov Camera)的32个硅光电倍增管阵列(2048像素)的性能。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发用于探测大气中粒子簇射产生的切伦科夫辐射（Cherenkov emission）和荧光辐射（fluorescence emission）快信号的硅光电倍增管（Silicon Photo-Multipliers）探测器，以支持下一代气球载和空间望远镜实验。

Data: 数据来源于对Hamamatsu S13161-3050AE-08传感器阵列（$8 \times 8$）在$30 \, \textrm{C}^{\circ}$至$-40 \, \textrm{C}^{\circ}$温度范围内的表征测试，以及即将对32个Hamamatsu S13361-3050系列硅光电倍增管（Silicon Photo-Multipliers，每片64通道）进行的大规模验证和校准实验。

Method: 采用硅光电倍增管(SiPM)性能表征协议，在-40°C至30°C温度范围内测量击穿电压、淬灭电阻、增益、暗计数率和串扰概率等关键参数。

Result: result：硅光电倍增管(SiPM)在-40°C至30°C温度范围内表现出关键参数（击穿电压、淬灭电阻、增益、暗计数率和串扰概率）随温度变化的特性，为POEMMA-Balloon任务中切伦科夫相机焦平面的大规模测试提供了校准依据。
...
**TL;DR**: 该研究设计了一种基于亚克力菲涅耳透镜（acrylic Fresnel lenses）的移动强度干涉仪（Mobile Intensity Interferometer），通过测量大角星（Arcturus）的空间相干曲线，在11小时内以毫角秒精度验证了其角直径测量能力，证实了该紧凑轻量化设计的可行性和精确性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用强度干涉仪（intensity interferometry）技术进行恒星观测，特别是在切伦科夫望远镜阵列（Imaging Atmospheric Cherenkov Telescope arrays）无法进行伽马射线观测的明亮月相期间，探索其作为专用设备的可行性和性能优化。
...
Method: 采用直径1米、焦距1.2米的丙烯酸菲涅耳透镜(Fresnel lens)实现紧凑轻量化设计，通过z轴平移调节波长测量焦点，并配备可整体更换的探测器模块。

Result: 使用直径1米、焦距1.2米的丙烯酸菲涅耳透镜实现了紧凑、经济且轻量化的专用强度干涉仪（Mobile Intensity Interferometer for Stellar Observations）设计，首次观测活动通过测量大角星（Arcturus）的空间相干曲线，在11小时内以毫角秒精度测得其角直径，与文献值一致。
...
**TL;DR**: 这篇论文的核心贡献是：利用引力波观测站干涉仪应变瞬态信号训练的神经网络编码器（neural encoder）作为形态敏感算子，在未经重新训练或微调的情况下，成功实现了对滚动轴承（rolling-element bearings）故障的零样本异常检测（zero-shot anomaly analysis），并在IMS-NASA和CWRU数据集中表现出优异的故障区分能力（AUC_win约0.90，AUC_file约0.99），同时揭示了该算子对相干弹性瞬态结构（coherent elastic transient structure）的敏感性而非通用CNN特征。


<details>
  <summary>Details</summary>

Background: 研究探讨了引力波观测站中干涉仪应变瞬态信号（interferometric strain transients）学习到的表征能否作为形态敏感的冻结算子（frozen morphology-sensitive operator）迁移至未见传感器，前提是目标信号保持相干弹性瞬态结构（coherent elastic transient structure）。

Data: data：数据来源于引力波观测站的干涉仪应变瞬态信号（interferometric strain transients）、IMS-NASA轴承运行至失效数据集（run-to-failure dataset）、凯斯西储大学轴承故障数据（CWRU）以及振动信号基准库（VSB）。

Method: 采用神经网络编码器（neural encoder）对非高斯仪器干扰（non-Gaussian instrumental glitches）进行训练，并基于干涉应变瞬变（interferometric strain transients）学习表征，用于零样本异常分析。

Result: 在IMS-NASA数据集上，基于引力波观测站干涉仪应变瞬态信号训练的神经编码器无需重新训练即可生成单调健康指数HI(t) = s0.99(t)/tau，实现固定虚警率（1e-3）的轴承状态监测。
...
**TL;DR**: Mini-EUSO（Mini Extreme Universe Space Observatory）作为首个部署在国际空间站(ISS)的JEM-EUSO项目探测器，通过五年观测积累了750小时数据，首次系统性实现了全球紫外辐射(UV)地图绘制、22,000余颗流星（含3颗星际候选体）监测及对瞬态发光事件(TLEs)中elves（90公里高度环状扩展结构）的动力学研究，同时验证了空间探测器区分超高能宇宙线(UHECRs)与大气簇射信号的能力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用国际空间站(ISS)搭载的Mini-EUSO探测器探索极端高能宇宙射线(UHECRs)及地球高层大气瞬态发光现象(TLEs)，旨在填补超高能粒子探测空白并揭示闪电相关物理过程。

Data: 数据来源于国际空间站（ISS）上运行的Mini-EUSO探测器自2019年10月起收集的约750小时观测数据，包括超过150次观测任务中记录的紫外线（UV）地球辐射图、闪电现象、瞬态发光事件（TLEs）、人造光源及流星事件。

Method: 采用紫外成像技术（UV imaging）结合快速时间分辨率（fast time resolution）观测地球大气瞬变发光现象（TLEs）和超高能宇宙线（UHECRs）。

Result: Mini-EUSO首次在空间站(ISS)上实现了对超高能宇宙射线(UHECRs)的系统性探测，并建立了首个全球紫外线地球辐射地图。
...
**TL;DR**: 通过分析3I/ATLAS彗星喷流位置角（PA）和光变曲线，确定其近日点后自转周期为7.1小时，并发现喷流结构绕自转轴存在20°进动。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过分析3I/ATLAS彗星近日点后的旋转周期（rotation period）和喷流结构（jet structure）的进动（precession），揭示其动力学特性与太阳方向（sunward direction）的几何关系。

Data: 数据来源包括哈勃空间望远镜（Hubble Space Telescope）在2025年11月20日至12月27日期间拍摄的图像，以及一台0.25米口径望远镜（MPC L92）采集的30分钟分箱光变曲线数据。

Method: 采用拉尔森-塞卡尼纳旋转梯度滤波器（Larson-Sekanina Rotational Gradient filter）分析哈勃太空望远镜图像，并通过加权最小二乘傅里叶级数拟合相位折叠的位置角曲线，同时扫描试验周期以识别χ²最小值。
...
**TL;DR**: 通过KMTNet和DREAMS对KMT-2025-BLG-1616微引力透镜事件的观测分析，发现该行星系统包含一颗质量约40倍地球质量的行星（~40M⊕），其宿主恒星质量约0.3倍太阳质量（~0.3M⊙），轨道投影距离约1.6天文单位（~1.6 au），并证实DREAMS具备探测银河系核球区火星质量（Mars-mass）和盘区月球质量（Moon-mass）自由漂浮行星（FFP）的灵敏度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用微引力透镜（microlensing）技术探测银河系内行星系统，特别是低质量行星和自由漂浮行星（FFP）的发现与表征。

Data: 数据来源于韩国微引力透镜望远镜网络（KMTNet）和DECam流浪地球与火星巡天（DREAMS）的观测数据。

Method: 采用贝叶斯分析（Bayesian analysis）推断宿主恒星质量、行星质量、投影行星-宿主间距及透镜距离，并结合DREAMS的高精度测光数据模拟自由漂浮行星（FFP）探测灵敏度。

Result: 行星系统具有约5×10^-4的行星-宿主质量比（planet-host mass ratio），贝叶斯分析得出宿主恒星质量约0.3倍太阳质量、行星质量约40倍地球质量、投影行星-宿主间距约1.6天文单位、透镜距离约7.5千秒差距。
...
### [37] [Rescaling Transforms for Local Models of Spherical Flows](https://arxiv.org/abs/2601.11337)
*Elliot M. Lynch,Guillaume Laibe*

**TL;DR**: 将超共动变量(super-comoving variables)推广至局部球面流模型，揭示了球面流中线性不稳定性具有时变增长率，并证明各向同性收缩/膨胀中的小尺度流动可直接映射到无粘不可压流体力学，预示存在重标度的柯尔莫哥洛夫湍流(Kolmogorov-turbulence)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于球对称收缩/膨胀气体云（spherically contracting/expanding gas cloud）的局部模型，旨在探索此类流动中的湍流和小尺度不稳定性（small scale instabilities）问题。

Data: 使用超共动变量（super-comoving variables）将宇宙学结构形成研究中的方法推广到局部球对称流动模型，通过映射常规笛卡尔流解获得解析解并分析复杂流动与背景的相互作用。

Method: 采用超共动变量（super-comoving variables）将局部球面流模型推广至更复杂流动与背景相互作用的分析，显著简化了解析解推导过程。

Result: 将超共形变量(super-comoving variables)推广到局部球面流模型中，显著简化了复杂流动与背景相互作用的解析求解和分析过程。
...
### [38] [Checking It Twice: Using [C/N]-Masses and Asteroseismic Masses as a Diagnostic of Mass Loss and Transfer on the RGB](https://arxiv.org/abs/2601.10800)
*John D. Roberts,Marc H. Pinsonneault,Jennifer A. Johnson,Madeline Howell*

**TL;DR**: 研究发现红巨星表面碳氮比（[C/N]）与诞生质量相关且不受质量损失直接影响，通过比较相同[C/N]但不同演化状态（红巨星分支与红团簇）的星震学质量，揭示了净红巨星分支质量损失强度随金属丰度和质量增加而减弱的经验规律，提出了修正的Reimers质量损失公式参数η的校准关系，并识别出207颗（3.33%）可能经历质量转移的异常恒星。


<details>
  <summary>Details</summary>

Background: 红巨星表面碳氮比([C/N])与诞生质量相关且不受质量损失直接影响，这为研究恒星演化过程中的质量损失提供了新途径。

Data: 数据来源于对红巨星（red giants）的星震学（asteroseismic）质量测量，以及相同[C/N]值但不同演化状态（如红巨星分支初期和红团簇阶段）的恒星样本比较。

Method: 利用红巨星表面碳氮比([C/N])与初始质量的相关性，通过比较相同[C/N]但不同演化阶段（红巨星分支和红团簇）的星震学质量差异，直接约束了场星在红巨星分支阶段的净质量损失强度。
...
### [39] [Unveiling Ionized Jet Morphologies: Sub-arcsecond VLA Observations of Compact Radio Sources](https://arxiv.org/abs/2601.11130)
*Tatiana M. Rodriguez,Peter Hofner,Emmanuel Momjian,Esteban D. Araya,Ananay Sethi,Viviana Rosero*

**TL;DR**: 通过亚角秒分辨率观测发现，23个致密射电源（包括15个上升谱指数源和8个弦状结构）中7个符合热喷流（thermal jet）特征、16个支持电离喷流（ionized jet）解释，并测得H$_2$O脉泽（maser）平均投影速度约120 km/s，表明电离主要源于激波（shocks）且仅在4例中观测到准直性（collimation）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高分辨率射电连续谱和水脉泽（H2O maser）观测，探究银河系内致密射电源和细丝状结构的物理性质与电离机制。

Data: 数据来源为甚大天线阵（VLA）1.3厘米连续谱和22.2 GHz水脉泽（H$_2$O maser）观测，目标为Rosero等人（2016, 2019）巡天中15个具有上升谱指数的致密射电连续源和8个弦状射电连续结构。

Method: 采用甚大天线阵(VLA)1.3厘米连续谱和22.2 GHz水脉泽(H$_2$O maser)观测技术，对15个具有上升谱指数的致密射电连续源和8个弦状射电连续结构进行亚角秒级($θ\sim0.1^{\prime\prime}$)分辨率观测。

Result: 在23个目标源中观测到三种形态：6个呈拉长或双峰弦状结构，4个为离散连续谱峰集合，13个为单一致密源。
...
### [40] [Fundamental Properties of Novae in M31](https://arxiv.org/abs/2601.11476)
*Allen W. Shafter,Kamil Hornoch*

**TL;DR**: 通过分析M31（仙女座星系）新星样本并应用Yaron等人模型，发现其白矮星（WD）质量分布呈高斯分布，平均质量为1.16±0.14 M⊙（校正后为1.07 M⊙），与银河系新星相似但比Schaefer研究结果高约0.15 M⊙，且再发新星平均质量显著更高（1.33±0.08 M⊙），同时揭示了吸积率、抛射物速度和再发周期等参数的分布特征及其与M31位置无关的结论。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用新星（nova）模型分析M31星系中新星样本的白矮星（white dwarf, WD）质量分布及其相关参数特征。

Data: 数据来源于Clark等人近期发表的M31星系新星大样本峰值光度及衰减率观测数据，结合Yaron等人的新星模型进行理论分析。

Method: 采用Yaron等人的新星模型（Yaron et al. nova models）分析M31新星样本的光度峰值和衰减率以估算其基本性质。

Result: M31新星样本中白矮星(WD)的质量分布近似高斯分布，平均质量为1.16±0.14太阳质量，经复发时间偏差校正后降至1.07太阳质量。
...
### [41] [POLAMI Multi-Wavelength Polarization Study of AGN Jets: A Millimeter-Optical Comparison](https://arxiv.org/abs/2601.10798)
...
**TL;DR**: 通过毫米波（millimeter-band）和光学偏振监测伽马射线耀变体（gamma-ray-bright blazars），发现BL Lac天体比平谱射电类星体（FSRQs）具有更高偏振度（fractional polarization）和更低偏振角（EVPA）变率，且偏振度随频率增加而升高，证实了短波长下磁场有序性增强的模型预测。


<details>
  <summary>Details</summary>

Background: 毫米波段偏振测量为研究活动星系核（AGN）喷流提供了有效手段，相比更长波长的射电波段更少受到不透明性和法拉第旋转效应的影响。

Data: 数据来源包括IRAM 30米望远镜进行的1毫米和3毫米偏振监测数据（POLAMI计划）以及多台设备长期积累的光学偏振观测数据。

Method: 采用贝叶斯(Bayesian)框架分析毫米波和光学偏振数据，结合圆统计方法表征偏振角(EVPA)变异性，并计算本征平均流量密度和调制指数。
...
### [42] [The Role of Plasma Lensing in Fast Radio Bursts](https://arxiv.org/abs/2601.11122)
*R. N. Li,Y. B. Wang,S. X. Yi,X. Zhou,F. Y. Wang*

**TL;DR**: 基于一维高斯等离子体透镜（plasma-lens）模型，揭示了快速射电暴（FRB）波前入射角微小单调变化可产生双向子暴频率漂移，不同透镜路径的旋转量（RM）差异导致千兆赫频段正交偏振角（PA）跳变，并解释了FRB 20180916B的频率依赖性活动窗口及磁层尺度发射区域约束，表明等离子体透镜可调制FRB多重复观测特征。


<details>
  <summary>Details</summary>

Background: 快速射电暴(FRB)的观测特征可能受到致密磁离子环境中等离子体透镜效应(plasma lensing)的显著影响。

Data: 使用一维高斯等离子体透镜（plasma lens）框架模拟数据，通过改变快速射电暴（FRB）波前入射角生成频率漂移和偏振角跃迁现象。

Method: 采用一维高斯等离子体透镜（1D Gaussian plasma-lens）模型分析快速射电暴（FRB）波前入射角变化和多重成像路径对频率漂移及偏振角跃变的影响。

Result: 等离子体透镜效应可解释快速射电暴（FRB）的复杂观测特征，包括频率漂移、偏振角（PA）跳变和色散活动窗口，并限制发射区横向尺度。
...
### [43] [Astrometric microlensing probes of the isolated neutron star population with Roman](https://arxiv.org/abs/2601.10789)
*Zofia Kaczmarek,Abby Halasi-Kun,Peter McGill,Scott E. Perkins,William A. Dawson*

**TL;DR**: 该论文通过模拟罗马空间望远镜（Roman Space Telescope）对银河系核球（Galactic Bulge）的微引力透镜（microlensing）观测，首次系统研究了孤立中子星（NS）透镜的探测特征，发现参数空间$\log_{10} t_{\rm E}$-$\log_{10} θ_{\rm E}$中存在中子星特有的分类特征，并预测罗马望远镜将观测到约100例中子星透镜事件，其事件产率受初始速度（natal kick）分布显著影响。


<details>
  <summary>Details</summary>

Background: 孤立中子星（NS）的探测与研究有助于解答恒星演化和爆发物理的基本问题。

Data: 使用专为致密天体微透镜效应优化的先进银河系模型，模拟了四个具有麦克斯韦分布初生速度（$\bar{v} = (150, \ 250, \ 350, \ 450)$ km/s）的中子星（NS）种群，并应用了罗曼望远镜（Roman）的预期精度、观测频率和可探测性标准。

Method: 采用最先进的适用于致密天体微透镜效应的银河系模型，模拟了四种具有麦克斯韦分布初生速度（$\bar{v} = (150, \ 250, \ 350, \ 450)$ km/s）的中子星（NS）群体，并应用了罗曼望远镜（Roman）的预期精度、观测频率和可探测性标准。

Result: 通过模拟分析发现，罗曼空间望远镜（Roman Space Telescope）可利用天体测量微引力透镜（astrometric microlensing）在参数空间$\log_{10} t_{\rm E}$ - $\log_{10} θ_{\rm E}$中识别中子星（NS）特征，并构建最优候选样本，预计将观测到约100例中子星透镜事件。
...
**TL;DR**: 利用DECam（Dark Energy Camera）开展DWF（Deeper, Wider, Faster）项目首次数据发布，开发了新型数据处理流程（dwf-postpipe），高效（97.24%恢复率）探测到峰值亮度g<22 AB星等的暂现源（transient），并发现13颗未编目变星（包括脉动变星、食双星和ZZ ceti）及2颗耀星（UV ceti）。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于快速时标（<1小时）下瞬变（transient）和可变光学天空的观测与表征，填补现有参数空间的空白。

Data: 数据来源于Deeper, Wider, Faster (DWF) 项目使用暗能量相机(DECam)获取的约12000张图像和166小时望远镜观测时间的高时间分辨率(分钟级)测光数据，以及注入到部分DWF DECam图像中的合成伽玛暴余辉(synthetic GRB afterglows)样本。

Method: 采用了一种新型数据处理流程（$\texttt{dwf-postpipe}$）进行源识别和光变曲线提取，并通过注入合成伽马射线暴余辉（GRB afterglows）评估其性能。

Result: 开发的新型数据处理管道（dwf-postpipe）能高效识别源并提取光变曲线，对峰值星等g<22 AB mag的暂现源恢复效率达97.24^{+0.7}_{-1.0}%，但较难恢复亮度与宿主星系相当或更亮的暂现源。
...
### [45] [Light Dark Matter Search with 7.8 Tonne-Year of Ionization-Only Data in XENONnT](https://arxiv.org/abs/2601.11296)
...
**TL;DR**: 在XENONnT实验中利用7.83吨·年的总曝光量对暗物质（DM）进行盲搜未发现显著超出背景的信号，为3-8 GeV/c²质量区间的自旋无关DM-核子散射、自旋相关DM-中子散射及DM-电子散射等过程设定了更严格的90%置信水平上限，其中自旋无关DM-核子散射在5 GeV/c²处的截面上限为6.0×10⁻⁴⁵ cm²。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用XENONnT探测器开展暗物质（DM）的盲搜实验，探索暗物质与核子或电子的相互作用，并扩展至轴子类粒子（ALPs）和暗光子等候选粒子。

Data: 数据来源于XENONnT探测器在579天内三个科学运行周期中收集的总曝光量为7.83吨·年的电离信号（S2-only）数据。

Method: 采用电离信号（S2-only）分析技术结合专用背景抑制方法和首个完整的S2-only背景模型，在XENONnT中开展暗物质（DM）盲搜。

Result: 首次在XENONnT中利用仅电离(S2-only)信号对暗物质(DM)进行盲搜，总曝光量为7.83吨×年，覆盖核反冲[0.5, 5.0] keVnr和电子反冲[0.04, 0.7] keVer能区。
...
### [46] [Gravitational perturbations of nonlocal black holes](https://arxiv.org/abs/2601.11101)
*Rocco D'Agostino,Vittorio De Falco*

**TL;DR**: 在修正的德瑟-伍德沃德非局部引力理论框架下，首次完整推导了静态球对称黑洞时空的轴向和极向引力扰动主方程，并应用于一类偏离施瓦西几何的单参数黑洞解，为后续研究奠定基础。


<details>
  <summary>Details</summary>

Background: 研究主题为在修正的德塞尔-伍德沃德非局部引力理论（revised Deser-Woodard nonlocal gravity theory）框架下，对静态球对称黑洞时空的轴向和极向引力扰动（axial and polar gravitational perturbations）进行理论建模。

Data: 数据来源于对一类静态球对称黑洞时空的引力扰动分析，基于修订版Deser-Woodard非局部引力理论框架下的一参数族黑洞解（one-parameter family of black hole solutions）。

Method: 采用修订的德塞尔-伍德沃德非局部引力理论（revised Deser-Woodard nonlocal gravity theory）框架下的主方程分析方法。

Result: 在修正的Deser-Woodard非局部引力理论框架下，首次完整建立了静态球对称黑洞时空轴向和极向引力扰动的主控方程。
...
**TL;DR**: 论文核心贡献为：分类了张力变化的标度宇宙弦网络（scaling cosmic string networks）引力波辐射现象学，计算了相应随机背景的谱指数（spectral indices），并通过IIB型弦论中D3和NS5膜缠绕内部循环的具体例子及沼泽地（Swampland）约束，给出了FLRW背景下有效弦张力时间变化和谱指数的普适界限。


<details>
  <summary>Details</summary>

Background: 研究聚焦于弦论（string theory）中模场（moduli）动态变化导致的宇宙弦（cosmic string）张力时变效应及其引力波（gravitational wave）随机背景谱指数（spectral indices）的理论计算与分类。

Data: 数据来源于数值模拟（simulation）和理论推导，包括基于IIB型弦论中D3-和NS5-膜包裹内部循环的具体模型，以及FLRW背景下的有效弦张力时间变化分析。

Method: 采用斯瓦兰（Swampland）约束推导FLRW背景中有效弦张力允许的时间变化及所得谱指数的普适界限。

Result: 引力波（gravitational wave）发射现象学在弦张力（string tension）变化的标度宇宙弦网络（scaling cosmic string networks）中被完全分类，并计算了由此产生的随机背景（stochastic backgrounds）的谱指数（spectral indices）。
