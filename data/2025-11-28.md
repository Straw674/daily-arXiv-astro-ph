...
**TL;DR**: 提出了一种利用神经网络将Nuker轮廓参数转换为可解析退投影的多高斯展开（Multi Gaussian Expansion）成分的方法，实现了无需光学观测即可构建物理真实的恒星质量模型，并将其集成到贝叶斯推断超大质量黑洞（supermassive black hole）质量的SuperMAGE流程中，在ALMA数据应用中与现有最优模型结果一致且扩展了尘埃遮蔽和活动星系的适用性。


<details>
  <summary>Details</summary>

Background: 研究主题为利用神经网络方法改进星系动力学建模，通过可微分映射替代传统基于成像的去投影（deprojection）技术，实现无需光学观测的物理真实恒星质量模型构建。

Data: 研究中使用的数据来源为ALMA（阿塔卡马大型毫米波/亚毫米波阵列）观测数据。

Method: 采用神经网络将Nuker轮廓参数转换为可解析反投影的多高斯展开（Multi Gaussian Expansion）分量，并集成到可微分动力学建模流程SuperMAGE中。

Result: 提出了一种基于神经网络的星系动力学建模方法，通过可微分映射将Nuker轮廓参数转换为可解析反投影的多高斯展开（Multi Gaussian Expansion）分量，无需光学观测即可构建物理上真实的恒星质量模型。
...
**TL;DR**: 利用ALMA透镜团巡天数据盲搜发现7个线发射体（包括1个[CII]发射体和4个CO发射体），通过引力透镜效应探测到分子气体质量下限降低1个数量级，首次在宇宙学距离上盲测到CH分子发射并发现其与活动星系核（AGN）相关的分子气体特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用ALMA透镜星系团巡天(ALMA Lensing Cluster Survey)数据开展线发射星系的盲搜寻，旨在通过引力透镜效应探测低分子气体质量的星系并分析其恒星形成与分子气体性质。

Data: 数据来源于ALMA透镜星系团巡天（ALMA Lensing Cluster Survey）的观测数据。

Method: 采用盲搜索（blind search）结合引力透镜（gravitational lensing）放大效应分析ALMA（Atacama Large Millimeter/submillimeter Array）数据，检测线发射星系（line-emitting galaxies）并测量分子气体质量（molecular gas mass）、恒星形成率（SFR）与主序距离的关系。

Result: 通过ALMA透镜星系团巡天数据的盲搜索，检测到7个线发射体，包括1个红移6.071的[C II]发射体、4个红移0.8-1.1的CO发射体（其中3个为同一星系的多重像）及2个可能为CO或[C I]的光度红移候选体，借助引力透镜效应（典型放大率μ∼4，极端达∼30）探测到分子气体质量比以往ALMA深场低约1个量级。
...
**TL;DR**: 基于ALMA（阿塔卡马大型毫米波/亚毫米波阵列）档案数据，整合JWST（詹姆斯·韦伯太空望远镜）和HST（哈勃太空望远镜）观测数据，建立了包含1288个光学/近红外对应源的连续谱图像和双源目录，扩展了A^3COSMOS和A^3GOODS-S的参数空间，并支持宇宙气体与尘埃质量演化等科学研究。


<details>
  <summary>Details</summary>

Background: 研究主题为利用ALMA（阿塔卡马大型毫米波/亚毫米波阵列）档案数据和遗产巡天项目进行星系生态（ECOGAL）研究，重点探索宇宙气体和尘埃质量演化、遮蔽恒星形成及光学暗系统等科学问题。

Data: 数据来源于ALMA（Atacama Large Millimeter/submillimeter Array）档案数据、詹姆斯·韦伯太空望远镜（JWST）和哈勃太空望远镜（HST）的观测足迹，并整合了CANDELS（COSMOS、GOODS-S和UDS）区域的公开JWST数据集（来自DAWN JWST Archive，DJA）。

Method: 采用ALMA（Atacama Large Millimeter/submillimeter Array）数据挖掘技术，结合JWST（James Webb Space Telescope）和HST（Hubble Space Telescope）的观测区域，统一重处理ALMA数据生成连续谱图像及两种互补源表（基于光学/近红外探测的先验源表和基于显著ALMA探测的盲源表），并通过峰值信噪比统计和反转图分析确定探测阈值。

Result: ECOGAL项目通过整合ALMA、JWST和HST数据，生成了两个互补的源目录（基于光学/近红外探测的先验目录和基于显著ALMA探测的盲目录），覆盖了约130,000个光学/近红外选星系（红移范围0.0 < z < 15.0），并识别出1288个探测（622个唯一源），其中307个具有0.12 < z < 6.85的光谱红移。
...
**TL;DR**: 研究发现银河系盘碳元素（C）主要来源于大质量恒星（CCSN）和渐近巨星分支（AGB）恒星，观测数据支持CCSN产额随金属丰度（metallicity）上升抵消AGB碳产量下降的趋势，表明AGB恒星在太阳金属丰度下贡献了10-40%的碳元素。


<details>
  <summary>Details</summary>

Background: 研究探讨了银河系盘中碳（Carbon, C）的天体物理来源，重点关注核心坍缩超新星（core collapse supernovae, CCSN）和渐近巨星分支（asymptotic giant branch, AGB）恒星对碳元素产生的相对贡献及其随金属丰度的演化。

Data: 研究中使用的观测数据来自APOGEE（Apache Point Observatory Galactic Evolution Experiment）对亚巨星（subgiant）的丰度测量。

Method: 采用银河系化学演化模型（GCE）结合APOGEE亚巨星丰度数据，通过分析[C/Mg]-[Mg/H]和[C/Mg]-[Mg/Fe]趋势约束碳元素来源比例。
...
**TL;DR**: 研究发现22个质量超过10^5太阳质量的河外星团（年龄3-100 Myr）在JWST-MIRI观测中呈现紧凑尘埃形态，其中19个已清除原生尘埃，10微米波段显著增强源于硅酸盐发射而非电离气体，最亮发射源来自三个超106太阳质量的星团，暗示极端质量下存在罕见短寿命尘埃产生机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星团（star cluster）演化不同阶段的尘埃（dust）产生机制及其在重子循环（baryonic cycle）中的作用。

Data: 数据来源于JWST-MIRI（James Webb Space Telescope Mid-Infrared Instrument）对22个质量大于10^5M$_{\odot}$的河外星系星团的观测，这些星团年龄介于3至100 Myr之间，并展现出紧凑的尘埃形态。

Method: 采用JWST-MIRI观测数据结合光谱分析（spectral analysis）和尘埃形态建模（dust morphology modeling）研究星团尘埃产生机制。

Result: 在22个质量超过10^5M$_{\odot}$的河外星团中观测到显著的10微米增强（silicate emission），最亮的10微米辐射源来自三个质量超过10^6M$_{\odot}$的星团，暗示极端质量和短寿命条件下的罕见机制。
...
**TL;DR**: 通过分析17个低质量星系（$3 \times 10^8 \, M_\odot \leq M_* \lesssim 2 \times 10^{10} \, M_\odot$）的金属丰度（[Fe/H]）和年龄，发现其恒星晕（stellar halos）的径向金属丰度梯度与星系固有属性无关，而质量-金属丰度关系的弥散可由主卫星（dominant satellite）的吸积时间解释，且约65%星系的径向年龄分布呈U型，主要由原位恒星物质（in situ stellar material）驱动，反映出低质量星系演化路径的复杂性和多样性。


<details>
  <summary>Details</summary>

Background: 研究旨在通过分析低质量星系及其恒星晕（stellar halos）的金属丰度（metallicity）和年龄来揭示它们的形成历史。

Data: 使用来自Auriga Project的17个模拟低质量星系（$\sim 3 \times 10^8 \, M_\odot \leq M_* \lesssim 2 \times 10^{10} \, M_\odot$）的数据。

Method: 采用17个Auriga项目模拟的低质量星系样本（$\sim 3 \times 10^8 \, M_\odot \leq M_* \lesssim 2 \times 10^{10} \, M_\odot$），通过分析其恒星晕的金属丰度（[Fe/H]）和年龄分布及其径向梯度，结合原位恒星物质与并合事件的贡献，探究低质量星系形成历史。

Result: 低质量星系及其星晕的金属丰度（[Fe/H]）和年龄呈现复杂多样性，其径向[Fe/H]梯度均为负值且与星系固有属性无关，而星晕质量-金属丰度关系的弥散可由主要卫星的吸积时间解释。
...
**TL;DR**: 该研究通过自组织映射（SOM）方法对COSMOS场230,638个星系的光学-近红外（NIR）光谱能量分布进行分箱叠加分析，建立了基于宽带光学-NIR颜色的恒星形成率（SFR）经验关系，并校准了低质量星系（i波段星等≤25.5）至红移z∼2.5的主序演化关系。


<details>
  <summary>Details</summary>

Background: 研究主题为利用自组织映射(Self-Organizing Map, SOM)方法从宽带光学-近红外(optical-to-NIR)颜色中学习恒星形成率(star formation rate, SFR)的经验关系，以解决大样本高红移暗弱星系缺乏光谱诊断或远红外(far-infrared, FIR)测光数据的问题。

Data: 数据来源为斯皮策（Spitzer）MIPS和赫歇尔（Herschel）PACS/SPIRE成像数据，以及COSMOS场（COSMOS field）中选取的230,638个星系的光学至近红外（optical-to-NIR）宽带图像。

Method: 采用自组织映射（Self-Organizing Map, SOM）对230,638个COSMOS场星系进行光学-近红外光谱能量分布分类，并通过堆叠分析（stacking analysis）测量中远红外（24-500微米）光度以校准恒星形成率。

Result: 通过自组织映射(SOM)方法分析光学-近红外颜色与远红外光度关系，校准了COSMOS场中近半数暗弱星系(i波段星等≤25.5)的恒星形成率(SFR)，并揭示了低质量星系主序演化至红移z∼2.5的规律。
...
**TL;DR**: 研究发现星系中可见物质与暗物质（dark matter）可能存在相对振荡，这种振荡可通过恒星速度异常（如密度波和逃逸恒星）观测到，并可能影响到达地球的暗物质风方向和强度。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系中重子物质（baryonic matter）与暗物质（dark matter）在引力相互作用下可能产生的相对振荡现象及其对恒星速度异常的影响。

Data: 研究采用理论建模方法，基于简化假设（忽略暗物质晕粒子内部动力学及星系恒星开普勒运动）构建双流体模型，类比原子核的"巨偶极共振"现象。
...
**TL;DR**: DEVILS（Deep Extragalactic VIsible Legacy Survey）通过高完整性多波段光谱观测在COSMOS（宇宙演化巡天）区域的D10区域提供了4824个新高质量红移数据，将Y波段21星等目标的红移完备性从50%提升至90%，并揭示了致密环境中恒星形成抑制与形态及恒星质量的关联。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用深空多波段巡天（DEVILS）提升高红移（0<z<1.2）星系样本的光谱完备性，以揭示星系形成与演化过程中环境效应（environmental quenching）的普遍性机制。

Data: 数据来源于英澳望远镜（Anglo-Australian Telescope）的AAOmega光谱仪进行的深空多波段光谱观测（DEVILS survey），覆盖了三个外银河系区域约4.5平方度天区，并整合了28个波段（从X射线到射电连续谱）的匹配成像数据以及来自其他来源的光谱红移（spectroscopic redshift）和光测红移（photometric redshift）信息。

Method: 基于英澳望远镜的AAOmega光谱仪进行光谱观测，采用多波段高完备性深度巡天方法。

Result: DEVILS（Deep Extragalactic VIsible Legacy Survey）显著提升了宇宙演化光谱巡天（COSMOS）区域Y波段21星等附近天体的光谱完备性（spectroscopic completeness），从其他样本的约50%提高到约90%，并提供了4824个暗弱天体的独特高质量光谱红移数据。
...
**TL;DR**: 研究发现矮椭圆星系(dEs)中带核(nucleated)与不带核(non-nucleated)成员在室女座星系团(Virgo cluster)内呈现显著空间分布差异：带核dEs更密集分布且与巨星系关联，表明其原位(in-situ)形成；而不带核dEs分布广泛且与星系团整体势阱一致，暗示其为外部吸积(ex-situ)起源。


<details>
  <summary>Details</summary>

Background: 矮椭圆星系(dwarf elliptical galaxies, dEs)作为星系团中主导星系群，是研究环境对星系演化影响的理想探针。

Data: data：使用最先进的机器学习框架系统扫描了室女座星系团区域（15°×20°，中心坐标为赤经187.2°、赤纬9.6°），构建了包含2123个矮椭圆星系（dEs）的最大同质样本，并进行了可靠的核分类。

Method: 采用最先进的机器学习框架对室女座星系团区域进行系统扫描，构建了包含2123个矮椭圆星系(dEs)的最大同质样本，并实现了可靠的核分类。

Result: 核化矮椭圆星系(dEs)更倾向于空间聚集且与巨星系关联更强，表明其主要在星系团内部原位形成(in-situ formation)。
...
**TL;DR**: 提出了一种基于深度强化学习(deep reinforcement learning)的自动化谱线拟合框架，显著提升了ALMA观测数据中甲醇(CH$_3$OH)等分子谱线物理参数(如激发温度、柱密度)的拟合效率。


<details>
  <summary>Details</summary>

Background: 星际分子谱线分析为研究星际介质(interstellar medium)的物理和化学条件提供了关键信息。

Data: 使用ALMA（Atacama Large Millimeter/submillimeter Array）的观测数据训练神经网络，并结合分子光谱数据（molecular spectroscopic data）和实测光谱（observed spectra）进行物理参数拟合。

Method: 采用深度强化学习框架自动拟合分子谱线，结合局部优化器提升初始参数估计精度。

Result: 提出了一种基于深度强化学习的自动化谱线拟合框架，显著减少了前向建模计算量并保持与全局优化一致的拟合精度。
...
**TL;DR**: 利用FAST望远镜对五个银河系球状星团（NGC 6171等）进行窄带技术信号搜索，首次采用MBPS策略并建立几何感知门控验证框架，未发现高于0.50 Jy的持续各向同性L波段信标信号，为稠密恒星系统中多历元观测提供了可扩展方案。


<details>
  <summary>Details</summary>

Background: 研究主题为利用FAST望远镜对银河系球状星团(globular clusters)进行窄带技术特征(technosignature)搜索。

Data: 数据来源于使用五百米口径球面射电望远镜（FAST）的19波束L波段接收器（1.05-1.45 GHz）对五个银河系球状星团（NGC 6171、NGC 6218、NGC 6254、NGC 6838和IC 1276）进行的窄带技术特征搜索观测。

Method: 采用多波束点源扫描（MBPS）策略，通过广义在目标窗口（gOTWs）门控检测和确定性多波束照明序列几何验证，结合turboSETI对7.5 Hz频率分辨率和10 s时间分辨率的动态频谱进行漂移率|ν_dot| ≤ 4 Hz s^-1、信噪比S/N ≥ 10的窄带技术特征搜索。

Result: 使用FAST望远镜对五个银河系球状星团（globular clusters）进行窄带技术特征（technosignature）搜索，未发现任何符合预设标准的信号，排除了在观测时段内存在亮度超过0.50 Jy的持续各向同性L波段信标的可能性。
...
**TL;DR**: 基于高分辨率CO数据对近邻矮星系（NGC 1035等6个目标）的分析表明，其分子气体与恒星形成率密度的幂律关系斜率（N=0.81±0.18）与质量更大旋涡星系一致，且托尔米Q参数（Toomre Q）径向分布接近1，证实矮星系恒星形成受与大质量星系相同物理过程调控。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近邻矮星系（dwarf galaxies）中分子气体与恒星形成率（star formation rate, SFR）的关联性，旨在验证分子恒星形成律（molecular star formation law, SFL）和引力不稳定性（gravitational instability）在低质量星系中的普适性。

Data: 数据来源于阿塔卡马大型毫米波/亚毫米波阵列(ALMA)的高分辨率$^{12}$CO ($J=1\rightarrow0$)观测数据，并结合了星系演化探测器(GALEX)的近紫外和广域红外巡天探测器(WISE)的12微米成像数据。

Method: 采用高分辨率$^{12}$CO ($J=1\rightarrow0$)数据（ALMA观测）结合近紫外（GALEX）与12μm（WISE）成像数据，在数百秒差距尺度上分析分子气体与恒星形成率（SFR）密度的关系，并计算托尔敏（Toomre）$Q$参数径向分布。

Result: 分子恒星形成定律（SFL）的幂律斜率在0.62至1.08之间，平均值为N=0.81±0.18，排除CO数据约束较差的星系后增至N=0.87±0.05，与大型旋涡星系观测值基本一致，表明在足够分辨率和灵敏度下分子SFL具有普适性。
...
**TL;DR**: 开发了可扩展的多波段光变曲线建模工具EzTaoX，显著提升处理维拉·鲁宾天文台LSST巡天数据效率（速度提高10²-10⁴倍），实现同时刻画活动星系核（AGN）紫外/光学随机光变特性和测量波段间时延，为研究AGN吸积流几何结构及识别矮星系中的低质量AGN提供新途径。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用维拉·鲁宾天文台（Vera C. Rubin Observatory）的时空遗产巡天（LSST）多波段观测数据，开发高效工具以表征活动星系核（AGN）紫外/光学随机变异性并测量波段间时延，从而探索AGN吸积流几何结构及识别低质量AGN。

Data: 数据来源为Vera C. Rubin天文台的时空遗产巡天（Legacy Survey of Space and Time, LSST）在六个宽波段（broad photometric bands）以平均3天间隔持续10年观测的数千万个活动星系核（active galactic nuclei, AGNs）的多波段光变曲线数据。

Method: 采用基于多波段观测的可扩展光变曲线建模工具EzTaoX，通过同时表征活动星系核(AGN)紫外/光学随机变异性并测量波段间时延，实现比现有工具快10^2-10^4倍的计算速度。

Result: EzTaoX工具通过利用LSST多波段观测数据实现了对活动星系核(AGN)紫外/光学随机变率和波段间时延的高效测量，计算速度比现有工具提升10^2-10^4倍且精度相当或更高。
...
**TL;DR**: 首次在154颗银河系盘区巨星中测定钼(Mo)和钌(Ru)丰度，发现[Mo/Fe]和[Ru/Fe]随[Fe/H]增加而下降，其分散性可能源于r-过程与s-过程的共同贡献，但尚需验证是否需额外核合成机制（如中微子风或i-过程）参与解释。


<details>
  <summary>Details</summary>

Background: 研究探讨了钼(Mo)和钌(Ru)等中子俘获元素的恒星起源及其在银河系化学演化中的作用。

Data: 数据来源为银河系盘内154颗巨星的新观测样本，金属丰度范围-1 < [Fe/H] < +0.3，首次测定了其中大部分巨星的钼(Mo)和钌(Ru)丰度。

Method: 采用局部热力学平衡假设下的合成光谱拟合方法，通过Mo I谱线（5506和5533 Å）和Ru I谱线（4584和4757 Å）测定钼（Mo）和钌（Ru）丰度。

Result: 钼（Mo）和钌（Ru）的丰度随金属丰度（[Fe/H]）增加呈下降趋势，与锆（Zr）类似，而锶（Sr）则呈现相对平坦的趋势。
...
**TL;DR**: 该研究通过分析约14,000个并合后星系样本，首次直接测量了并合引发的中心恒星质量增长（burst mass fraction），发现并合后星系中心区域（7千秒差距范围内）存在15-20%的恒星质量超额，且这种质量增长呈现延展分布而非高度集中。


<details>
  <summary>Details</summary>

Background: 星系并合（galaxy mergers）触发中心恒星形成爆发并导致内区恒星质量增长的观测研究。

Data: 数据来源包括一个约14,000个已完成并合诱导恒星形成的后并合星系样本，以及来自Mapping Galaxies at Apache Point (MaNGA)巡天的61个z<0.05星系的积分视场单元(integral field unit)数据。

Method: 采用固定角孔径（Delta M_{star,fibre）和星系核球（Delta M_{star,bulge）的恒星质量增强量化方法，并结合积分视场单元（IFU）数据进行千秒差距尺度评估。

Result: 合并后星系中心区域恒星质量增加约15-20%，且这种质量增长延伸至约7千秒差距（1.4倍有效半径）范围。
...
### [17] [A Super-Eddington, Lensing-Magnified Quasar at $z=5.07$ observed with JWST](https://arxiv.org/abs/2511.21618)
*Katherine Panebianco,Minghao Yue,Anna-Christina Eilers,Xiaohui Fan,Feige Wang,Jinyi Yang,Robert A. Simcoe*

**TL;DR**: 利用JWST/NIRCam成像确认红移(redshift)5.07类星体J0025-0145未被多重成像，并发现其具有已知红移大于5超大质量黑洞中最高的爱丁顿比(Eddington ratio)（λEdd>4.9），支持早期宇宙中超爱丁顿吸积的可行性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST和HST观测数据探究高红移(z=5.07)类星体J0025-0145的引力透镜效应及其宿主星系特性，以揭示早期宇宙中超大质量黑洞的超爱丁顿吸积(Super-Eddington accretion)生长机制。

Data: 数据来源于JWST/NIRCam的F070W和F480M波段成像以及哈勃空间望远镜（HST）的成像数据。

Method: 利用JWST/NIRCam的F070W和F480M波段成像数据，结合HST测光数据，通过光谱能量分布（SED）拟合和单点扩散函数（PSF）分析，排除了多重成像可能性并确定了前景透镜星系的质量和红移。

Result: 利用JWST/NIRCam F070W波段的高分辨率成像确认背景类星体J0025-0145未被多重成像，其宿主星系在F070W和F480M波段均未被探测到。
...
### [18] [Evidence For A Correlation Between Astrophysical Neutrinos and Radio Flares](https://arxiv.org/abs/2511.21627)
*Yjan A. Gordon,Peter S. Ferguson,Eric J. Hooper,Michael N. Martinez*

**TL;DR**: 研究发现射电耀变体（radio flares）与冰立方（IceCube）中微子存在显著关联（>2σ置信度），其中约13%的天体中微子可能源自射电耀变体，且80%以上关联源未在γ/X射线波段被探测，凸显射电观测对识别中微子电磁对应体的重要性。


<details>
  <summary>Details</summary>

Background: 该研究探讨了射电变源（radio variability）与天体物理中微子（astrophysical neutrinos）之间的潜在关联性，旨在通过多信使（multi-messenger）观测揭示中微子的电磁对应体（electromagnetic counterparts）。

Data: 研究中使用的数据来自甚大天线阵巡天项目（Very Large Array Sky Survey, VLASS）前两个观测时期和冰立方中微子天文台（IceCube Neutrino Observatory）的观测数据。

Method: 采用射电变源（radio flares）与中微子（neutrinos）关联分析方法，结合VLASS和IceCube数据的时间约束统计检验。

Result: 在VLASS和IceCube数据中发现，射电耀变体（radio flares）与中微子（neutrinos）的关联数量存在>2σ置信度的超出，表明射电耀变体贡献了IceCube观测到的约13%的天体物理中微子（astrophysical neutrinos）。
...
### [19] [Game Theory in Cosmology](https://arxiv.org/abs/2511.20739)
*Oem Trivedi,Venkat Venkatsubramanian*

**TL;DR**: 这篇论文提出了一种称为宇宙学目的动力学（Cosmological Teleodynamics）的博弈论统计框架，将暗物质（dark matter）、暗能量（dark energy）、宇宙加速膨胀和大尺度结构等观测现象重新解释为自引力宇宙中非局域记忆（nonlocal memory）和内在持续组织结构的涌现结果，并通过引入最大口径权重（maximum-caliber weight）和结构记忆偏置泛函（bias functional）推导出修正的弗里德曼方程（Friedmann equations）、玻尔兹曼方程（Boltzmann equations）和泊松方程（Poisson equations），揭示了宇宙加速膨胀的非平衡统计起源并预测了环境依赖的晕族特征（environment-dependent halo signatures），最终表明宇宙演化遵循一种连续形式的纳什均衡（Nash equilibrium）。


<details>
  <summary>Details</summary>

Background: 提出了一种基于博弈论统计框架的宇宙学新范式（cosmological teleodynamics），将暗物质、暗能量、宇宙加速膨胀和大尺度结构等观测现象重新解释为自引力宇宙中非局域记忆（nonlocal memory）和内在持续组织结构（persistent organization）的涌现结果。

Data: 数据来源于理论推导（simulation）构建的宇宙学博弈论统计框架，通过引入最大口径权重（maximum-caliber weight）和结构记忆偏置泛函（bias functional）重新推导了弗里德曼方程、玻尔兹曼方程和泊松方程。

Method: 采用博弈论统计框架（game-theoretic statistical framework）结合最大口径权重（maximum-caliber weight）和结构记忆偏置泛函（bias functional），推导了修正的弗里德曼方程、玻尔兹曼方程和泊松方程。

Result: 宇宙加速膨胀和大尺度结构是自引力宇宙中非局域记忆（nonlocal memory）和固有持续组织（intrinsically persistent organization）的涌现现象。
...
### [20] [Reanalyzing DESI DR1: 2. Constraints on Dark Energy, Spatial Curvature, and Neutrino Masses](https://arxiv.org/abs/2511.20757)
*Anton Chudaykin,Mikhail M. Ivanov,Oliver H. E. Philcox*

**TL;DR**: 通过重新分析暗能量光谱仪（DESI）数据并结合宇宙微波背景（CMB）和全形状（FS）测量，显著提升了非标准宇宙学模型（包括空间曲率、动态暗能量和大质量中微子）的参数约束精度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用DESI（暗能量光谱仪）公开数据集对标准宇宙学模型ΛCDM进行扩展验证，包括中微子质量（ΛCDM+Mν）、空间曲率（oΛCDM）和动态暗能量（w0waCDM）等非最小化模型。

Data: 数据来源于暗能量光谱仪（DESI）公开数据集，包括星系和类星体的功率谱（power spectrum）和双谱（bispectrum）测量结果，并结合宇宙微波背景（CMB）数据及超新星（SNe）数据。

Method: 采用基于有效场论（EFT）的管道分析DESI星系和类星体的功率谱和双谱，结合宇宙微波背景（CMB）和DESI全形状（FS）数据，通过精心选择的EFT参数先验约束非最小宇宙学模型。

Result: 结合DESI全形状（FS）功率谱和双谱测量显著提升了非最小宇宙学模型（如含中微子质量、空间曲率或动力学暗能量）的参数约束能力。
...
### [21] [Caustic crossings in giant arcs with extended dark matter objects](https://arxiv.org/abs/2511.20761)
*Djuna Croon,Benedict Crossey,Jose Maria Diego,Bradley J. Kavanagh,Jose Maria Palencia*

**TL;DR**: 该研究将微引力透镜（microlensing）框架扩展到扩展暗物质天体（extended dark objects, EDOs），通过分析巨弧背景下的焦散交叉（caustic-crossing）事件（如“伊卡洛斯”事件），发现EDOs能产生额外窄焦散并在光变曲线中形成新特征，从而约束半径达$10^7 R_\odot$的EDOs，证明该方法可探测更大物理尺寸的暗物质天体。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用巨弧背景下的焦散交叉（caustic-crossing）恒星探测暗物质子结构（dark matter substructure），特别是扩展暗物质天体（extended dark objects, EDOs）的分布与性质。

Data: 数据来源为哈勃太空望远镜（Hubble Space Telescope）对MACS J1149星系团后方巨弧中LS1“伊卡洛斯”（Icarus）事件的观测数据。

Method: 开发了分析微引力透镜（microlensing）的解析模型，用于研究嵌入宏透镜（macrolensing）星系团势场中的延展暗物质天体（EDOs），并推导了产生的焦散线（caustics）和光变曲线（light curves）。
...
### [22] [Euclid preparation. Controlling angular systematics in the Euclid spectroscopic galaxy sample](https://arxiv.org/abs/2511.20856)
...
**TL;DR**: 提出了一种识别和减轻欧几里得(Euclid)光谱星系巡天中角系统误差(angular systematics)的方法，通过构建代表巡天可视掩膜(visibility mask)的随机星表(random catalogue)，在理想情况下能以亚百分精度恢复受系统误差影响的星系功率谱(galaxy power spectrum)，并在近似已知可视掩膜时证实大尺度效应有限且宇宙学参数估计稳健。


<details>
  <summary>Details</summary>

Background: 研究聚焦于欧几里得(Euclid)光谱星系巡天中角向系统误差的识别、校正及其对星系成团性测量和宇宙学参数估计的影响。

Data: 研究中使用的数据来源于模拟的光谱巡天（simulated spectroscopic surveys），包括构建代表巡天可见性掩模（visibility mask）的随机星表（random catalogue）以及基于信号噪声比（S/N）的探测模型（detection model）。

Method: 采用随机目录（random catalogue）构建技术模拟观测掩膜（visibility mask），并结合基于信噪比（S/N）的探测模型（detection model）校正角向系统误差（angular systematics）。

Result: 通过构建代表巡天可见性掩模的随机星表，并利用检测模型校正系统误差，可在理想情况下以亚百分比的精度恢复星系功率谱（galaxy power spectrum），且在可见性掩模近似已知时，参数估计仍保持稳健。
...
### [23] [Testing Bump in the Cosmological Power Spectrum Using Dwarf Galaxies](https://arxiv.org/abs/2511.21134)
*Maxim Zabelkin,Sergey Drozdov,Oleg Skorikov,Sergey Pilipenko*

**TL;DR**: 通过分析邻近矮星系的光度函数和空间分布，研究发现1.3兆秒差距波长处宇宙学功率谱"凸起"(bump)的无量纲振幅需小于0.25（3σ置信水平），且该凸起显著影响绝对星等-13至-17范围内矮星系的光度函数。


<details>
  <summary>Details</summary>

Background: 研究旨在利用邻近矮星系的光度函数和空间分布观测数据，约束宇宙学功率谱（cosmological power spectrum）相对于标准模型的偏离。

Data: 数据来源为邻近矮星系的观测数据，包括其光度函数（luminosity function）和空间分布（spatial distribution）。

Method: 采用邻近矮星系的观测数据（光度函数和空间分布）分析宇宙学功率谱偏离标准模型的约束条件，重点考察波长1.3~Mpc处振幅$\mathcal{A}=2.0$的“凸起”（bump）模型。

Result: 在波长1.3兆秒差距（Mpc）处引入振幅𝒜=2.0的功率谱"凸起"（bump）会导致局域矮星系在-13>M_B>-17范围内的光度函数显著增加。
...
### [24] [Constraints on Features in the Cosmological Power Spectrum from Observations of the Epoch of Reionization](https://arxiv.org/abs/2511.21179)
*O. R. Skorikov,S. V. Pilipenko,M. V. Tkachev*

**TL;DR**: 研究发现，在微扰功率谱（power spectrum）于矮星系尺度存在适度增强（振幅$\mathcal{A}<1.5-2$）的宇宙学模型中，紫外光子逃逸比例（$f_{esc}$）和中性氢分布不均匀性导致的不确定性使得此类模型在$z \approx 8$的再电离（reionization）观测中未被排除。


<details>
  <summary>Details</summary>

Background: 研究探讨了具有矮星系尺度扰动增强（"隆起"或"倾斜"）功率谱的宇宙学模型对再电离时期（reionization epoch）提前的影响及其与观测的兼容性。

Data: 数据来源于对宇宙学模型（cosmological models）中具有增强振幅的扰动功率谱（power spectrum of perturbations）的观测和模拟（simulation），重点关注矮星系尺度（dwarf galaxy scales）上的“凸起”（bump）或“倾斜”（tilt）特征。

Method: 采用修正的功率谱扰动模型（power spectrum of perturbations）分析宇宙早期星系形成对再电离时期的影响。

Result: 在适度增强幅度（$\mathcal{A}<1.5-2$）下，考虑模型未被再电离（reionization）观测排除，主要归因于紫外光子逃逸率（$f_{esc}$）和中性氢分布不均匀性的现有不确定性。
...
### [25] [First Results from HERA Phase II](https://arxiv.org/abs/2511.21289)
...
**TL;DR**: 首次利用氢再电离纪元阵列（HERA）第二阶段观测数据，在宇宙黎明（Cosmic Dawn）和再电离纪元（EoR）期间获得了21厘米涨落功率谱的2σ上限（最高灵敏度分别为1.13×10⁶ mK²和1.78×10³ mK²），并发现互耦效应导致低k模（k=0.35-0.55）污染成为主要系统误差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用氢再电离纪元阵列(HERA)第二阶段观测数据，首次探测宇宙黎明(Epoch of Cosmic Dawn)和再电离时期(Epoch of Reionization)的21厘米辐射功率谱上限。

Data: 数据来源于氢再电离纪元阵列（HERA）实验第二阶段的两周观测子集，频率覆盖50-250 MHz。

Method: 采用改进的信号链分析技术（包括扩展带宽至50-250 MHz）并建立系统误差校正流程，重点识别仪器新系统特性。

Result: 首次在再电离时期(Epoch of Reionization)和宇宙黎明(Cosmic Dawn)阶段获得了21厘米涨落功率谱的上限值，其中宇宙黎明(z>12)最严格上限为1.13×10⁶ mK²(k=0.55 h Mpc⁻¹, z=16.78)，再电离时期(5.5<z<12)最严格上限为1.78×10³ mK²(k=0.70 h Mpc⁻¹, z=7.05)。
...
### [26] [Friction in Stochastic Inflation](https://arxiv.org/abs/2511.21388)
*Baptiste Blachier,Christophe Ringeval*

**TL;DR**: 在无边界平坦势能（flat potential）模型中引入恒定漂移项（drift term），通过时间反转随机暴胀（time-reversed stochastic inflation）方法精确求解曲率涨落概率分布，发现其呈现指数衰减尾部（exponential decaying tails），与无摩擦时的幂律行为（Levy-like power law）形成对比，并揭示了正向与反向时间方法在经典极限下尾部高斯化（Gaussian）的定量差异。


<details>
  <summary>Details</summary>

Background: 研究探讨了半无限平坦势场（semi-infinite flat potential）中具有恒定漂移项（constant drift term）的时间反转随机暴胀（time-reversed stochastic inflation）问题及其对曲率涨落概率分布的影响。

Data: 数据来源于半无限平坦势场（semi-infinite flat potential）中带有恒定漂移项（constant drift term）的随机暴胀（stochastic inflation）时间反演模拟（time-reversed simulation）。

Method: 采用时间反演随机暴胀（time-reversed stochastic inflation）方法在半无限平坦势场中求解含恒定漂移项的曲率涨落概率分布精确解。

Result: 在时间反演随机暴胀模型中，恒定漂移项导致曲率涨落概率分布呈现指数衰减尾部，与无摩擦时的幂律行为形成鲜明对比。
...
### [27] [Testing modified gravity with 3x2pt analyses in galaxy mocks](https://arxiv.org/abs/2511.21468)
*Marc Alemany-Gotor,Cristian Viglione,Pablo Fosalba,Isaac Tutusaus*

**TL;DR**: 通过全天空高保真模拟星系目录首次量化了在广义相对论(GR)假设下分析f(R)引力宇宙时3x2pt方法导致的严重参数偏差（Ωm和σ8偏差达12σ），证明Stage-IV巡天中错误引力假设会显著影响宇宙学推断。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用第四阶段巡天（Stage-IV surveys）在宇宙学尺度上检验引力理论，特别是探讨在广义相对论（GR）假设下分析大尺度结构时可能引入的系统偏差，以及修正引力理论（如Hu-Sawicki $f(R)$引力）作为宇宙加速膨胀的替代解释。

Data: 研究中使用的数据来源于一对孪生模拟（twin simulations）：一组采用广义相对论（GR），另一组采用Hu-Sawicki $f(R)$引力模型（$|f_{R0}| = 10^{-5}$），并通过HOD（halo occupation distribution）方法在暗物质晕中生成星系模拟星表，其低红移部分标定了SDSS观测数据。

Method: 采用高保真全天域星系模拟目录，通过3x2pt（星系成团和弱引力透镜联合）分析方法，结合嵌套采样（nested sampling）技术，在保守尺度截断下量化了广义相对论（GR）假设下分析$f(R)$引力模拟数据导致的宇宙学参数偏差。

Result: 在假设广义相对论(GR)而实际宇宙由$f(R)$引力(以Hu-Sawicki形式$|f_{R0}|=10^{-5}$为例)主导时，即使采用保守尺度截断和边缘化星系偏置与重子反馈效应，仍会导致宇宙学参数$\{Ω_{\rm m}, σ_8\}$和$S_8$出现高达$\sim12σ$的显著偏差。
...
### [28] [Large-Scale Galaxy Correlations from the DESI First Data Release](https://arxiv.org/abs/2511.21585)
*Francesco Sylos Labini,Tibor Antal*

**TL;DR**: 基于DESI首批数据中BGS和LRGS样本的星系相关性分析，发现条件平均密度呈现幂律衰减（$\langle n(r) \rangle \propto r^{-0.8}$），且在探测尺度内（$r \sim 400$ Mpc/$h$）未出现向空间均匀性的转变，密度涨落服从极值统计的Gumbel分布而非高斯分布。


<details>
  <summary>Details</summary>

Background: 该研究探讨了暗能量光谱仪（DESI）首批数据中亮星系样本（BGS）和亮红星系样本（LRGS）的星系关联性，重点分析了星系分布是否在观测尺度内呈现空间均匀性（homogeneity）转变。

Data: 数据来源于暗能量光谱仪（DESI）首次数据发布中的亮星系样本（BGS）和亮红星系样本（LRGS），并采用保守边界条件确保计算中仅包含中心球体完全位于观测范围内的星系。

Method: 采用条件平均密度（conditional average density）测量方法，通过限定球形区域完全包含在观测范围内的保守标准来最小化边界效应，并构建体积限制子样本消除光度依赖性偏差。

Result: 条件平均密度（conditional average density）遵循幂律衰减$\langle n(r) \rangle \propto r^{-0.8}$，且在探测尺度内（$r \sim 400$~\text{Mpc}/$h$）未表现出向均匀性（homogeneity）的转变。
...
### [29] [StarEstate: A Python Package for Galactic Population Synthesis](https://arxiv.org/abs/2511.20735)
*Amedeo Romagnolo*

**TL;DR**: 开发了开源Python工具StarEstate，通过优化预计算逆累积分布采样器快速生成银河系恒星种群合成模型，并基于年龄相关动力学温度实现恒星在旋臂上的概率分布，能高效模拟不同金属丰度环境下Wolf-Rayet星、O型星等特定演化阶段天体的空间分布。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发一种高效、统计稳健的银河系恒星种群合成建模工具（galactic population synthesis），旨在通过开源软件快速生成合成恒星群体并模拟其空间分布特征。

Data: 数据来源为预生成的恒星演化轨迹网格（stellar tracks）和MESA或快速SSE/BSE演化轨迹（evolutionary tracks）。

Method: 采用优化的预计算逆累积分布函数采样器（inverse-cumulative distribution function samplers）结合MESA或快速SSE/BSE演化轨迹，通过概率性分配恒星到螺旋臂并基于年龄相关动力学温度实现快速星系种群合成建模。

Result: StarEstate通过基于年龄依赖动力学温度的概率性旋臂分配机制，重现了年轻示踪体紧密约束于旋臂而年老群体逐渐弥散的观测特征。
...
### [30] [A Direct Calibration Algorithm for ADC Interleaving](https://arxiv.org/abs/2511.20763)
*Chi-kwan Chan,Hina Suzuki,David Forbes,Andrew Thomas West,Arash Roshanineshat,Daniel P. Marrone,Amy Lowitz*

**TL;DR**: 提出一种利用正弦参考信号和傅里叶分析直接校准模数转换器(ADC)时间交织系统中相位延迟、增益和偏移失配的高效算法，显著提升了高速数据采集系统的信号重建精度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决高速数据采集系统中模数转换器(ADC)时间交织技术存在的相位延迟、增益和偏移失配问题，以提升射电天文和甚长基线干涉测量(VLBI)等应用场景的信号保真度。

Data: 数据来源于广泛的数值模拟（numerical simulation）。

Method: 采用正弦参考信号和傅里叶分析直接校准算法（direct calibration algorithm）来隔离和校正模数转换器（ADC）时间交织系统中的相位延迟、增益和偏移失配。

Result: 提出了一种基于正弦参考信号和傅里叶分析的新型直接校准算法，能有效校正模数转换器(ADC)时间交织系统中的相位延迟、增益和偏移失配。
...
### [31] [TriPoDPy: 1D Tri-Population size distributions forDust evolution in protoplanetary disks](https://arxiv.org/abs/2511.20764)
*Nicolas Leo Kaufmann,Thomas Pfeil,Sebastian Stammler,Anna Penzlin,Sandro Christian Paetzold,Til Birnstiel*

**TL;DR**: TriPoDPy代码通过一维径向网格模拟原行星盘（protoplanetary disk）中尘埃（dust）演化（包括生长和动力学）及气体盘演化，并新增了尘埃与气体示踪剂（tracer）功能以实现成分追踪。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于原行星盘(protoplanetary disk)中尘埃颗粒演化（包括生长和动力学）的数值模拟。

Data: 数据来源于使用Python编写、Fortran90实现核心例程的TriPoDPy代码，该代码基于一维径向网格模拟原行星盘（protoplanetary disk）中尘埃（dust）和气体（gas）的演化，并采用参数化尘埃模型（parametric dust model）和标准alpha描述（canonical alpha-description）。

Method: 采用参数化尘埃模型（parametric dust model）结合一维径向网格模拟原行星盘中尘埃演化及气体盘演化，核心算法由Python和Fortran90实现，并扩展了尘埃与气体示踪剂描述功能。

Result: TriPoDPy是一个基于参数化尘埃模型（parametric dust model）的Python代码，用于模拟原行星盘（protoplanetary disks）中尘埃增长和动力学的演化过程。
...
### [32] [The WEAVE-TwiLight-Survey: Expanding WEAVE's Reach to Bright and Low-Surface-Density Targets with a Novel Observing Mode](https://arxiv.org/abs/2511.21184)
...
**TL;DR**: 开发了WEAVE望远镜的新型观测模式（WEAVE-Tumble-Less），通过叠加低目标密度视场并优化光纤分配，实现了对低表面密度目标（如系外行星宿主星）的高效观测，同时减少校准时间并扩展了黄昏时段的观测窗口。


<details>
  <summary>Details</summary>

Background: 研究背景为开发威廉·赫歇尔望远镜增强区域速度探测器（WEAVE）的新型观测模式，以解决多目标光谱巡天（multi-object spectroscopic surveys）对低表面密度亮星观测效率低的问题。

Data: data部分：观测数据来源于ESA PLATO长周期观测场（ESA PLATO long-duration-phase fields），通过叠加多个低目标密度场并分配光纤配置实现高效观测。

Method: 采用启发式方法定义以中心导星为参考的叠加低目标密度场，并通过定制算法选择适合合并配置的导星光纤。

Result: 开发了一种新型观测模式（WEAVE-Tumble-Less），可在不增加校准曝光开销的情况下高效观测低表面密度目标场，并成功通过测试观测验证了其可行性。
...
### [33] [Enhancing Galaxy Classification with U-Net Variational Autoencoders. II. JWST High Redshift Galaxy Sample](https://arxiv.org/abs/2511.21207)
*Sergey Mirzoyan*

**TL;DR**: 该研究利用U-Net变分自编码器(VAE)框架对詹姆斯·韦伯太空望远镜(JWST)拍摄的高红移(z≈8)星系图像进行去噪和形态分类增强，发现早期宇宙(红移>3)中盘状星系占比达70-80%，表明盘状结构在宇宙早期可能普遍存在。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用深度学习技术提升詹姆斯·韦伯太空望远镜(JWST)对高红移(redshift)星系图像的降噪和形态分类能力，以探索早期宇宙中星系结构的演化特征。

Data: 数据来源于詹姆斯·韦伯太空望远镜（JWST）观测到的292个红移值最高达7.69的星系图像。

Method: 采用U-Net变分自编码器(VAE)框架对詹姆斯·韦伯太空望远镜(JWST)星系图像进行去噪处理，通过标准图像质量指标评估去噪效果，并利用增强图像提升多模型形态分类精度。

Result: 基于U-Net变分自编码器(VAE)的去噪方法显著提升了詹姆斯·韦伯太空望远镜(JWST)高红移(redshift)星系图像的形态分类精度，尤其识别出早期宇宙中盘状(disk-like)结构的普遍存在。
...
**TL;DR**: 提出一种基于格里芬-利姆(Griffin-Lim)算法的深度学习模型，通过优先去噪的幅度谱图增强相位恢复，显著提升了总质量超过30太阳质量的恒星级质量双黑洞(BBH)并合引力波信号在幅度和相位对齐上的去噪精度。


<details>
  <summary>Details</summary>

Background: 引力波去噪是揭示宇宙中致密双星系统（compact binary objects）事件的重要研究课题。

Data: 数据来源于模拟的二元黑洞（BBH）合并事件产生的引力波信号，以及真实探测到的引力波事件数据。

Method: 采用深度学习模型结合格里芬-林算法(Griffin-Lim algorithm)从时频数据中恢复相位信息，并利用先验去噪幅度谱增强相位重建。

Result: 利用基于Griffin-Lim算法的深度学习模型，通过先验去噪幅度谱实现了对双黑洞并合引力波信号的高精度相位恢复，尤其在并合阶段与模拟模板波形高度吻合。
...
**TL;DR**: 通过热力学方法推导修正弗里德曼方程，发现扩展不确定性原理（EUP）修正能产生不依赖宇宙学常数的晚期加速膨胀，并证明该模型具有动力学稳定性及德西特吸引子解。
...
Data: 数据来源于理论推导（simulation）的修正弗里德曼方程（modified Friedmann equations）和切瓦列尔-波拉尔斯基-林德参数化（Chevallier-Polarski-Linder parametrization）分析的有效状态方程演化。

Method: 采用热力学论证推导修正弗里德曼方程，并利用Chevallier-Polarski-Linder (CPL)参数化分析扩展不确定性原理(EUP)诱导的有效状态方程演化。

Result: 基于热力学论证推导的扩展不确定性原理（EUP）修正弗里德曼方程表明，该宇宙学模型允许存在一个晚期德西特吸引子（de Sitter attractor）。
...
**TL;DR**: 提出双滤波模型（Double Filter Model）利用光变曲线数据提升年轻恒星体（YSOs）和渐近巨星分支（AGB）星的分类精度，纠正了红外测光中的误分类并识别出SPICY星表中的潜在AGB污染源。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用光变曲线数据提升年轻恒星体（YSOs）和渐近巨星分支（AGB）恒星分类精度，解决二者因相似红外光度特征导致的误分类问题。

Data: 研究中使用的数据来源为斯皮策太空望远镜（Spitzer/IRAC）的银心中平面候选年轻恒星天体目录（SPICY catalog）的光变曲线数据，并针对金牛座年轻恒星天体和光谱确认的渐近巨星分支恒星进行了验证。

Method: 采用双滤波模型(Double Filter Model)结合机器学习与深度学习方法，基于光变曲线数据区分年轻恒星体(YSOs)和渐近巨星分支(AGB)恒星。

Result: 利用光变曲线数据的双滤波模型（Double Filter Model）有效提升了年轻恒星天体（YSOs）和渐近巨星分支（AGB）恒星的分类准确性，并识别出红外测光中被误分类的样本。
...
**TL;DR**: 通过模拟和观测对比，研究发现原恒星天体L1157 B2属于非辐照C型激波（non-irradiated C-type shock），并证实甲醇（methanol）解吸率可作为非辐照条件下区分激波类型的有效示踪剂。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低速度分子外流（low-velocity molecular outflows）中激波类型（shock type）的约束问题，特别是通过天体化学（astrochemical）方法区分非辐照C型（non-irradiated C-type）与J型激波（J-type shocks）的分子丰度差异。

Data: 使用巴黎-杜罕激波代码(Paris-Durham Shock code)模拟了速度范围5-19 km/s、低密度范围10²-10⁵ cm⁻³的网格化激波模型集。

Method: 采用巴黎-杜罕激波代码（Paris-Durham Shock code）模拟了速度范围5-19 km/s、密度范围10²-10⁵ cm⁻³的网格化激波模型集，重点计算了甲醇（methanol）解吸百分比并外推至高速度情况。
...
### [38] [Axion Constraints from White Dwarf Cooling in 47 Tucanae](https://arxiv.org/abs/2511.21676)
*Leesa Fleury,Alysa Obertas,Harvey Richer,Jeremy Heyl*

**TL;DR**: 通过分析球状星团杜鹃座47（47 Tucanae）中白矮星（white dwarf）冷却过程并模拟轴子（axion）辐射影响，发现最优模型不支持轴子发射且轴子-电子耦合常数（axion-electron coupling）$g_{aee}$在95%置信水平下新上限为$0.81 \times 10^{-13}$，排除了银河系白矮星异常冷却暗示的轴子参数范围。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于通过分析球状星团杜鹃座47（47 Tucanae）中白矮星（white dwarf）的冷却过程，探寻轴子（axion）发射影响白矮星冷却速率的证据。

Data: 数据来源于哈勃太空望远镜（Hubble Space Telescope）对球状星团47 Tucanae中白矮星的观测。

Method: 采用恒星演化模拟（stellar evolution simulations）分析白矮星冷却过程，考虑轴子韧致辐射（axion bremsstrahlung）导致的额外能量损失，并针对不同轴子-电子耦合常数（axion-electron coupling constant）、白矮星质量和包层厚度生成冷却模型。

Result: 在球状杜鹃座47（47 Tucanae）白矮星冷却研究中未发现轴子（axion）发射证据，并得出轴子-电子耦合常数（axion-electron coupling constant）新上限$g_{aee} \leq 0.81 \times 10^{-13}$（95%置信水平），排除了银河系白矮星异常冷却暗示的轴子参数范围。
...
### [39] [Cosmological tensions in Proca-Nuevo theory](https://arxiv.org/abs/2511.21071)
*Hsu-Wen Chiang,Claudia de Rham,Sebastian Garcia-Saenz,Xue Zhou*

**TL;DR**: 研究扩展普罗卡-努埃沃理论(extended Proca-Nuevo theory)发现，该矢量-张量理论通过引入有效暗能量流体(范围从冻结精质到解冻类幽灵行为)，在1.5σ水平上优于ΛCDM模型(结合CMB和BAO数据)，将哈勃张力从5.8σ降至2.3σ，但需微调以抑制扰动对物质功率谱的增强效应。


<details>
  <summary>Details</summary>

Background: 研究探讨了扩展普罗卡-努沃(Proca-Nuevo)理论的宇宙学预测，该矢量-张量理论提供了稳定的均匀各向同性解，其有效暗能量流体行为涵盖冻结精质(quintessential)到解冻幽灵(phantom-like)模式，为审视标准ΛCDM模型面临的宇宙学张力问题提供了理论框架。

Data: 数据来源包括最新的宇宙微波背景辐射（CMB）、重子声波振荡（BAO）和低红移（low-redshift）观测数据集。
...
Result: Proca-Nuevo理论（extended Proca-Nuevo theory）作为矢量-张量理论（vector-tensor theory）能产生稳定的均匀各向同性解，其有效暗能量流体（effective dark energy fluid）行为涵盖冻结精质（freezing quintessential）到解冻幽灵态（thawing phantom-like），为缓解标准$Λ$CDM模型面临的宇宙学张力提供了理论框架。
...
### [40] [Weak gravity at micron scales from dark bubble cosmology and its cosmological consequences](https://arxiv.org/abs/2511.21362)
*Ulf Danielsson,Suvendu Giri*

**TL;DR**: 暗泡模型（dark bubble model）预测在小尺度（约10^-5米）重力比牛顿引力更弱，并通过桌面实验提供可测量偏差的明确预测，同时解释了高能量密度下引力减弱导致早期暴胀及宇宙物质含量的量子起源，提出宇宙正曲率参数Ω_c≈5×10^-4以解决宇宙常数（cosmological constant）的“为何现在”问题。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗泡模型（dark bubble model）如何将正宇宙常数自然地纳入弦理论框架，并预测了近期可观测的新物理现象。

Data: 使用桌面实验（table top experiments）测量$10^{-5}$米尺度下引力强度的偏差，并结合早期宇宙高能量密度条件下有效引力减弱导致的暴胀现象进行验证。

Method: 采用暗泡模型（dark bubble model）分析重力强度在$10^{-5}$米尺度的变化，并通过桌面实验测量与牛顿引力的偏差。

Result: 暗泡模型（dark bubble model）预测在小尺度（约$10^{-5}$米）下引力会比牛顿引力更弱而非更强，并可通过桌面实验测量其偏差。
...
**TL;DR**: 通过比较XMM-Newton观测的四个星系团（Virgo、Centaurus、Ophiuchus和A3266）与Illustris TNG-300模拟的速度图，发现最佳匹配的模拟晕（halo）再现了观测到的大尺度速度梯度和局部运动子结构，表明星系团内介质（ICM）运动源于气体晃动（gas sloshing）、活动星系核（AGN）反馈和小规模并合活动的共同作用。


<details>
  <summary>Details</summary>

Background: 研究主题为利用X射线观测与宇宙学模拟（cosmological simulations）对比，探索星系团内介质（intracluster medium, ICM）的物理条件、动力学状态及其演化机制。

Data: 数据来源于XMM-牛顿(XMM-Newton)对室女座(Virgo)、半人马座(Centaurus)、蛇夫座(Ophiuchus)和A3266星系团的观测速度图，以及Illustris TNG-300模拟生成的合成速度图。

Method: 采用孪生卷积神经网络(Siamese CNN)分析观测与模拟的速度图形态相似性，构建高维相似度度量以捕捉传统统计方法无法识别的运动学与结构特征。

Result: 基于形态学匹配的Siamese卷积神经网络（CNN）分析表明，观测到的星系团内介质（ICM）运动源于气体晃动（gas sloshing）、活动星系核（AGN）反馈及小规模并合活动的共同作用。
...
**TL;DR**: 通过JWST观测球状星团ω Centauri中心区域，未发现符合中等质量黑洞(IMBH)低吸积率模型的SED特征，但对IMBH质量和吸积效率组合给出了比现有射电观测更严格的限制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（詹姆斯·韦伯太空望远镜）观测球状星团ω Centauri（NGC 5139）中心区域，以验证基于HST（哈勃太空望远镜）多历元观测中快速移动恒星运动推断出的中等质量黑洞（IMBH）候选体的存在性及其吸积特性。

Data: 数据来源于JWST（James Webb Space Telescope）对球状星团ω Centauri中心区域的观测（NIRCam的F200W和F444W波段以及MIRI的F770W和F1500W波段），并结合HST（Hubble Space Telescope）的oMEGACat光测数据（Haberle et al. 2024b）构建了中心区域天体的紫外至红外光谱能量分布（SED）。

Method: 采用PSF拟合测光（PSF-fitting photometry）分析JWST的NIRCam（F200W和F444W）和MIRI（F770W和F1500W）数据，并结合HST测光数据构建中心区域的紫外至红外光谱能量分布（UV to IR SEDs）。

Result: 在ω Centauri中心区域可靠测量的源中，未发现任何光谱能量分布（SED）与Pesce等人2021年模型预测的低吸积率下中等质量黑洞（IMBH）吸积星团内介质的SED相符。
...
**TL;DR**: 在7 Ms钱德拉南深场中发现了一个异常X射线暂现源XID 935（红移0.251），其X射线光变曲线呈现非单调下降且存在短期剧烈增亮现象，排除了部分潮汐瓦解事件(TDE)模型，为超大质量黑洞(SMBH)双星TDE模型提供了关键观测约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超大质量黑洞(Supermassive Black Hole, SMBH)潮汐瓦解事件(Tidal Disruption Events, TDEs)的观测特征与理论模型验证，旨在通过极端深场X射线巡天数据揭示此类瞬变现象的物理机制。

Data: 数据来源于7 Ms钱德拉南深场(Chandra Deep Field-South)的X射线观测，并利用总曝光时间约9.5 Ms的X波段数据构建了长达20年的光变曲线。

Method: 采用部分潮汐瓦解事件(TDE)模型和超大质量黑洞(SMBH)双星TDE模型分析X射线光变曲线。

Result: 在7 Ms钱德拉南深场（Chandra Deep Field-South）中发现了一个异常X射线暂现源XID 935，其X射线光度在1999至2016年间整体衰减超过40倍，但在2014年10月至2015年3月期间出现短时增亮（>27倍），峰值0.5-7 keV光度达10^(42.31±0.20) erg s^(-1)，且光学光谱未显示活动星系核（active galactic nucleus）特征。
...
**TL;DR**: 构建并分析了一个仅与破缺的SU(5)规范方向（轻夸克矢量玻色子X、Y）耦合且标准模型规范群下中性的暗物质（dark matter）模型，证明了其色中性（color neutrality）并与冷暗物质（cold dark matter）现象学一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于构建和分析一个在未破缺标准模型（Standard Model）规范群下中性且仅耦合于破缺SU(5)规范方向（X,Y轻子夸克矢量）的暗物质（dark matter）领域。

Data: 数据来源为理论模拟（simulation）研究，未使用实际观测数据。

Method: method

Result: 构建并分析了一个在未破缺的标准模型（Standard Model）规范群下中性且仅与破缺的SU(5)规范方向（轻夸克矢量X,Y）耦合的暗物质（dark matter）模型，证明了其稳定性由精确的Z_2对称性保证。
...
### [45] [Cosmological Probes of Lepton Parity Freeze-in Dark Matter: $ΔN_{\rm eff}$ & Gravitational Waves](https://arxiv.org/abs/2511.21634)
*Ernest Ma,Partha Kumar Paul,Narendra Sahu*

**TL;DR**: 提出了一种基于轻子宇称（lepton parity）守恒的I型跷跷板机制扩展模型，引入马约拉纳费米子（Majorana fermion）S作为暗物质（DM）候选者，并通过右手中微子（N）衰变或希格斯玻色子（h）衰变产生DM遗迹，同时预测了可观测的引力波信号和额外相对论自由度（ΔN_eff）。


<details>
  <summary>Details</summary>

Background: 该研究探讨了标准I型跷跷板机制（type-I seesaw）中轻子宇称（lepton parity）守恒条件下引入马约拉纳费米子（Majorana fermion）S作为暗物质（DM）候选者及其与单态标量场σ的耦合如何影响暗物质遗迹丰度、电弱相变强度以及额外相对论自由度（ΔN_eff）的产生。

Data: 研究中使用的数据来源为理论模拟（simulation）数据。

Method: 采用标准I型跷跷板机制（type-I seesaw）并引入具有偶数轻子宇称（lepton parity）的马约拉纳费米子（Majorana fermion）S和奇数轻子宇称的单态标量σ，通过耦合NSσ和h→SS单圈过程研究暗物质（DM）产生机制及电弱相变（electroweak phase transition）特性。

Result: 在标准I型跷跷板机制（type-I seesaw）中引入具有偶轻子数（lepton parity）的马约拉纳费米子（Majorana fermion）S和奇轻子数的单态标量σ，使得S成为稳定的暗物质候选者，其质量范围可从MeV到TeV，并通过引力波和额外相对论自由度（ΔN_eff）留下可观测痕迹。
...
### [46] [Tracking the Activity of the Interstellar Object 3I/ATLAS through its Perihelion](https://arxiv.org/abs/2511.20810)
...
**TL;DR**: 通过地基和天基观测监测并预测了3I/ATLAS彗发（coma）的光学特性，利用日冕仪（solar coronagraphs）和PUNCH任务数据实现了对其2025年10月8日至30日不可见期间全近日点通过时光星等的连续追踪。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用地面和空间观测数据监测和预测星际彗星3I/ATLAS（3I/Borisov）彗发（coma）的光学特性，以支持行星际航天器观测任务。

Data: 数据来源于地基观测和天基太阳日冕仪（solar coronagraphs）以及PUNCH任务的观测数据。

Method: 采用地基与天基观测数据结合太阳日冕仪（solar coronagraph）和PUNCH任务的光学测光技术追踪3I/ATLAS彗发光学星等变化。

Result: 通过地基和天基观测数据监测并预测了3I/ATLAS彗发(coma)的光学特性，特别利用日冕仪(solar coronagraphs)和PUNCH任务数据实现了对其整个近日点期间（包括2025年10月8日至30日不可见时段）光度变化的连续追踪。
...
### [47] [CH$_3$OH and HCN in Interstellar Comet 3I/ATLAS Mapped with the ALMA Atacama Compact Array: Distinct Outgassing Behaviors and a Remarkably High CH$_3$OH/HCN Production Rate Ratio](https://arxiv.org/abs/2511.20845)
...
**TL;DR**: 在星际彗星3I/ATLAS中首次探测到甲醇(CH$_3$OH)和氰化氢(HCN)的独特释放模式，发现甲醇在向阳半球增强释放且存在彗发来源，而氰化氢仅来源于核直接升华，其甲醇丰度比显著高于多数彗星。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际彗星3I/ATLAS中甲醇(CH3OH)和氰化氢(HCN)的释放行为及其空间分布特征。

Data: 数据来源于阿塔卡马大型毫米波/亚毫米波阵列（ALMA）的阿塔卡马紧凑阵列（Atacama Compact Array），在2025年8月28日、9月18日、22日和10月1日对甲醇（CH$_3$OH）进行观测，并在9月12日和15日对氰化氢（HCN）进行观测。

Method: 采用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)的阿塔卡马紧凑阵列(ACA)对星际彗星3I/ATLAS进行亚毫米波谱线观测，通过统计分析法确定甲醇(CH3OH)的彗发源尺度长度(Lp>258 km)，并对比氰化氢(HCN)的直接核升华模式。

Result: 在星际彗星3I/ATLAS中检测到甲醇（CH3OH）和氰化氢（HCN）的释放模式存在显著差异，其中HCN在彗发向阳面释放减少而CH3OH在该区域增强。
