...
**TL;DR**: 在W40大质量恒星形成区中心观测到HCN和HCO$^+$ J = $3 - 2$发射，结合辐射转移模型(RADEX)和化学演化模型(NAUTILUS)发现其丰度分别为$1.3-1.7 \times 10^{-8}$和$1.3-3.1 \times 10^{-9}$，并确定发射源自密度为$5 \times 10^4 - 5 \times 10^5$ cm$^{-3}$的稠密气体，且紫外辐射在$A_V < 6$区域会降低分子丰度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量恒星形成区W40中心区域的致密气体化学特征，通过观测HCN（氰化氢）和HCO$^+$（甲酰离子）分子谱线探讨其丰度分布与辐射场相互作用的物理化学过程。

Data: 数据来源于JCMT望远镜（James Clerk Maxwell Telescope）MAJORS大型项目的试点观测，覆盖W40大质量恒星形成区中心424''×424''区域的HCN和HCO$^+$ J=3-2分子谱线观测。

Method: 采用RADEX辐射转移代码(RADEX radiative transfer code)结合先验N(H$_2$)和$T_K$数据，并假设恒定密度，同时使用NAUTILUS化学演化代码(NAUTILUS chemical evolution code)考虑H$_2$密度变化进行建模分析。

Result: 在W40大质量恒星形成区中心区域观测到HCN和HCO$^+$丰度分别为$X$(HCN)=$1.3-1.7 \times 10^{-8}$和$X$(HCO$^+$)=$1.3-3.1 \times 10^{-9}$，且当视觉消光(A_V)小于6时两者丰度因紫外辐射破坏反应而降低。
...
**TL;DR**: 通过测量54个GLIMPSE-D星系（红移z=6-9）的R3（[O III]5008/Hb）比值并建立其与紫外辐射演化的四种模型，发现紫外亮度Muv>-18的星系金属丰度显著降低，且Ha光度L(Ha)~1e41-1e42 erg/s的星系在8<z<9期间主导电离光子产生率，而7<z<8时各光度区间的贡献更均匀，支持贫金属暗弱星系对再电离的主要贡献。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(z=6-9)星系中氧丰度(oxygen abundance)与紫外光度(UV luminosity)的关系，及其对宇宙再电离(reionisation)的贡献。

Data: data：研究使用GLIMPSE-D巡天中54个星系的红外光谱数据，测量了[O III]5008/Hb线比（R3），样本包含z=6-9、紫外绝对星等（Muv）在-20至-14之间的引力透镜放大星系。

Method: 采用校准关系推导氧丰度，并基于新测量和前人结果探索了四种描述R3随紫外辐射演化的处方模型。

Result: 在紫外光度Muv>-18的较暗星系中，[O III]5008/Hβ比值（R3）显著下降，表明这些星系金属丰度降低。
...
**TL;DR**: 通过JWST/NIRCam观测极暗弱星系（M$_{\rm UV}$ > -16）的紫外连续谱斜率（β），发现其逃逸分数（f$_{\rm esc}$）在M$_{\rm UV}=-$16.5处达到峰值约20%后随亮度降低而下降，并证实M$_{\rm UV}$介于$-18$至$-14$的星系贡献了宇宙再电离约60%的电离光子。


<details>
  <summary>Details</summary>

Background: 研究聚焦于最暗弱星系（M$_{\rm UV}$ > -16）的电离特性及其对宇宙再电离（cosmic reionization）的贡献，通过紫外连续谱斜率（β）分析其恒星种群特征和电离光子逃逸分数（f$_{\rm esc}$）。

Data: 数据来源于利用JWST/NIRCam对强透镜场Abell S1063进行的超深度GLIMPSE成像观测，获取了555颗红移z>6的星系样本，其绝对星等覆盖至M$_{\rm UV}$≃−12.5。

Method: 利用JWST/NIRCam GLIMPSE强透镜场Abell S1063的超深成像数据，通过测量高红移（z>6）星系的紫外连续谱斜率（β）来推断其恒星种群特性及电离光子逃逸率（fesc）。

Result: 紫外连续谱斜率（β）与绝对星等（M$_{\rm UV}$）的关系在M$_{\rm UV}$ $\sim -$16.5处趋于平坦，表明更暗弱的星系不再呈现最蓝的紫外斜率。
...
**TL;DR**: 利用JWST对Abell S1063引力透镜场中16颗高红移(z~6)极暗弱星系(M_UV<-17星等)的光谱分析发现，这些低质量(M_*~10^6-10^5.6 M_⊙)星系表现出强[OIII]发射线、极端电离源特征及双金属增丰路径，并在M_*=10^6.6 M_⊙星系中首次同时探测到超新星驱动的尘埃衰减、金属外流和重子循环现象。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(high-$z$)极暗弱星系(ultra-faint galaxies)的光谱特征及其在早期星系演化中的作用。

Data: 数据来源于JWST（James Webb Space Telescope）的NIRCam（近红外相机）和NIRSpec（近红外光谱仪）对引力透镜场Abell S1063的超深观测，这些观测是GLIMPSE巡天的一部分。

Method: 利用JWST/NIRCam和NIRSpec对Abell S1063引力透镜场进行超深观测，结合光谱分析技术测量了16个极暗星系（$M_{\rm UV}<-17$星等）的金属丰度（metallicity）、尘埃消光（dust attenuation）和电离光子产生效率（ionizing photon production efficiency），并通过[OIII]发射线强度扩展了质量-金属丰度关系（mass-metallicity relation）的低质量端至$M_\star\sim10^6\ M_\odot$。

Result: 在$z\sim6$时，质量-金属丰度关系（mass-metallicity relation）的低质量端延伸至$M_\star\sim10^6\ M_\odot$，且样本中最低质量星系（$M_\star\sim10^{5.6}\ M_\odot$）的强[OIII]发射线检测与近期类似质量及红移的极贫金属星系候选体报告形成对比，暗示可能存在两种不同的早期金属增丰途径。
...
**TL;DR**: 在紫外辐射的马头星云边缘观测到多种复杂有机分子（COMs）丰度增强，表明光解驱动的尘埃表面化学反应可能是其形成机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于紫外辐射环境下复杂有机分子（COMs）的形成机制及其在星际介质（ISM）光解离区（PDRs）中的分布特征。

Data: 数据来源于ALMA和IRAM 30m单天线观测（分辨率约15角秒），并结合先前的C$^{17}$O、C$^{18}$O单天线数据以及PdBI的H$_2$CO和CH$_3$OH成图数据。

Method: 采用ALMA和IRAM 30m单天线观测数据结合C$^{17}$O、C$^{18}$O单天线数据及PdBI的H$_2$CO、CH$_3$OH成图，通过气体密度、温度、热压及柱密度剖面分析，探究UV辐射对有机分子空间分布的影响机制。

Result: 在紫外辐射边缘观测到H2CO、CH2CO、CH3CHO、HNCO和CH3CN等有机分子柱密度增强，表明CO和CH3OH光解产生的原子C与自由基通过尘埃表面扩散驱动了高效化学反应。
...
**TL;DR**: 通过银经-速度($\ell$-$v$)图分析银河系(MW)盘面金属丰度结构，发现其可有效追踪径向梯度且不依赖距离测量，但当前HII区氧丰度误差掩盖了方位角变化，同时FIRE-2模拟中异常运动(anomalous motions)导致的局域静止标准(LSR)速度尾分布比银河系实测大3倍。


<details>
  <summary>Details</summary>

Background: 研究银河系(MW)盘金属丰度结构的宏观科学背景在于揭示其形成、组装及长期演化过程中留下的化学印记。

Data: 数据来源包括银河系（MW）盘中HII区域的氧丰度观测数据，以及来自FIRE-2宇宙学模拟中银河系质量星系的模拟数据。

Method: 采用经度-速度($\ell$-$v$)图分析技术，结合HII区氧丰度观测数据和FIRE-2宇宙学模拟的银河系质量星系数据，研究银河系盘面金属丰度结构。

Result: 银河系( MW )盘金属丰度结构的经度-速度( $\ell$-$v$ )图分析揭示了径向梯度及非对称方位角变化，但当前HII区氧丰度误差过高导致无法观测到后者。
...
**TL;DR**: 在红移(redshift)z~3.25的MQN01结构中，发现了一个位于大量冷气体储层中心的大质量静止星系(passive galaxy)，其低恒星形成率可能由邻近活动星系核(AGN)喷流反馈导致的气体吸积效率降低和环星系介质(CGM)湍流维持所引起。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(z≳3)静止星系(quiescent galaxies)的淬灭机制及其与星系周介质(CGM)动力学、活动星系核(AGN)反馈的关联。

Data: 数据来源于JWST（James Webb Space Telescope）的观测数据、MQN01结构中z~3.25的被动星系的多波段深度信息（包括钱德拉X射线数据）以及Lyα和Hα发射线对冷气体储层的示踪。

Method: 采用深度多波段观测数据（包括钱德拉X射线数据）和荧光发射分析，结合活动星系核（AGN）喷流反馈模型解释气体吸积抑制机制。

Result: 在红移(z~3.25)的MQN01结构中观测到一个大质量被动星系（恒星质量约10^11太阳质量），其恒星形成率显著低于主序星系1个数量级且未探测到分子气体，却位于由Lyα和Hα发射示踪的富冷气体环境中，附近活动星系核（AGN）的喷流反馈可能通过维持星系周介质（CGM）高湍流状态抑制了气体吸积。
...
**TL;DR**: 通过多波段观测发现IRAS20100-4156星系存在由恒星形成驱动的多相（分子、电离、中性原子）外流（outflow），其中分子外流质量达8×10^9 M⊙（占系统总分子气体40%），呈双锥形态延伸至5 kpc，速度170 km/s，并伴随3.5×10^7 M⊙尘埃外流，且外流气体在千秒尺度上持续加速。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用多波段观测数据解析超亮红外星系(ULIRG) IRAS20100-4156中由恒星形成驱动的多相（分子、电离、中性原子气体及尘埃）外流现象及其动力学特征。

Data: 数据来源为ALMA CO(1-0)观测数据和VLT/MUSE静止光学波段数据。

Method: 采用ALMA CO(1-0)观测和VLT/MUSE静止光学数据，结合恒星速度场对应关系将复杂CO(1-0)发射线轮廓分解为引力与非引力运动分量。

Result: 发现一个质量达8×10^9 M⊙的双锥形分子外流（molecular outflow），占系统总分子气体质量的40%，特征速度为170 km/s，质量流失率为700 M⊙/yr，耗尽时间为16 Myr，其能量学特征表明恒星形成是主要驱动机制。
...
### [9] [An unexpected population of quenched galaxies harbouring under-massive SMBHs revealed by tidal disruption events](https://arxiv.org/abs/2601.20519)
*Paige Ramsden,Sean L. McGee,Matt Nicholl*

**TL;DR**: 研究发现低质量(10^9.6≲M_gal≲10^10.5 M⊙)静止星系中的超大质量黑洞(SMBH)质量(M_BH≲10^6.5 M⊙)低于同质量恒星形成星系，表明其静止机制不同于大质量星系且可能由环境过程驱动。


<details>
  <summary>Details</summary>

Background: 潮汐瓦解事件（TDE）为研究低质量端超大质量黑洞（SMBH）与恒星形成之间的联系提供了独特窗口，突破了事件视界的观测限制。

Data: 数据来源包括42个潮汐瓦解事件(TDE)宿主星系的紫外-中红外(UV-MIR)宽带光谱能量分布(SED)观测数据，以及文献中获取的超大质量黑洞(SMBH)质量估计值。

Method: 采用Prospector拟合紫外-中红外宽波段光谱能量分布(SED)获取宿主星系恒星质量和特定恒星形成率，并结合文献中的超大质量黑洞(SMBH)质量估计。

Result: 在低质量星系(10^9.6≲M_gal≲10^10.5 M⊙)中发现了一类反常的淬灭星系，其中心超大质量黑洞(SMBH)质量(M_BH≲10^6.5 M⊙)显著低于同等恒星质量下恒星形成星系中的黑洞质量。
...
### [10] [Analyzing PAHs as a Tracer of Anomalous Microwave Emission Near the Galactic Plane Using the COSMOGLOBE DIRBE Reduction](https://arxiv.org/abs/2601.20545)
*Dylan M. Pare,David T. Chuss,Danielle Sponseller,Brandon Hensley,Alan Kogut*

**TL;DR**: 通过分析DIRBE数据发现，银河系中心区域(|b|≤10°)的异常微波辐射(AME)与远红外尘埃辐射(ρ∼0.9)的相关性强于多环芳烃(PAHs)辐射(ρ∼0.7)，表明AME可能主要源自非PAH尘埃颗粒或不同激发条件下的PAHs。


<details>
  <summary>Details</summary>

Background: 异常微波辐射（AME）的物理机制是近30年来未解的谜题，其科学主题聚焦于探究星际环境中AME的起源机制，特别是多环芳烃（PAHs）旋转辐射与远红外尘埃辐射的潜在关联性。

Data: 研究中使用的数据来源于DIRBE（Diffuse Infrared Background Experiment）实验的band 3观测到的3.3 μm PAH发射特征以及COSMOGLOBE DIRBE数据集的再处理结果。

Method: 采用3.3微米多环芳烃(PAH)发射特征和远红外尘埃发射数据，基于宇宙球(COSMOGLOBE)DIRBE数据集的改进版本进行相关性分析。

Result: 异常微波辐射（AME）与远红外尘埃辐射（ρ∼0.9）的相关性强于多环芳烃（PAH）辐射（ρ∼0.7），表明银河系平面区域的AME可能主要由非PAH尘埃颗粒或替代物理发射机制主导，或PAH的中红外辐射与AME激发条件存在显著差异。
...
### [11] [Revealing the link between halo mass and radio jet activities in quasars](https://arxiv.org/abs/2601.20576)
*B. -H. Yue,P. N. Best,H. J. A. Röttgering,K. J. Duncan,C. L. Hale,L. K. Morabito,D. J. B. Smith*

**TL;DR**: 研究发现射电喷流（radio jet）主导的类星体（quasar）倾向于分布在质量更大的暗物质晕（dark matter halo）中（典型质量为$10^{13-14}\ h^{-1}M_\odot$），表明射电喷流功率与富星系团环境存在显著关联。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索类星体（quasar）射电喷流（radio jet）功率差异的成因，强调大尺度环境（large-scale environment）对喷流活动的潜在影响。

Data: 数据来源于扩展重子振荡光谱巡天（eBOSS）的225,382个类星体和低频阵列两米巡天（LoTSS）数据发布2的射电通量密度数据。

Method: 采用了两点相关函数（two-point correlation function）分析类星体空间聚类特性，并结合射电喷流贡献比例（jet fraction）进行分组比较。

Result: 类星体射电喷流功率与暗物质晕质量呈正相关，喷流主导的类星体通常位于质量更大的暗物质晕（dark matter halo）中。
...
### [12] [Encounters Between M33 and Present-Day M31 Satellites Hint at a Previous Group Accretion](https://arxiv.org/abs/2601.20837)
*Ekta Patel,Paul Bennet,Sangmo Tony Sohn,Mark Fardal,Roeland van der Marel*

**TL;DR**: 研究发现仙女座星系(M31)的两个已知卫星星系双鱼座矮星系(LGS 3)和仙女座XVI可能曾是M33的卫星星系，42%和60%的轨道模拟支持它们曾位于M33的维里半径内，并在过去1-20亿年内以超过M33逃逸速度的相对速度近距离飞越M33，为M31与M33之间的卫星交换或群体吸积提供了新证据。


<details>
  <summary>Details</summary>

Background: 研究探讨仙女座星系（M31）的两个已知卫星星系双鱼座矮星系（LGS 3）和仙女座XVI是否与M31质量最大的卫星星系M33发生过动力学相互作用。

Data: 研究中使用的数据来源包括仙女座星系（M31）卫星星系双鱼座（LGS 3）和仙女座XVI（And XVI）的自行运动数据（proper motions），并结合半解析势场（semi-analytic potential）进行轨道反向积分（backward orbit integration）。

Method: 采用半解析势场（semi-analytic potential）结合自行运动（proper motions）和逆向轨道积分（backward orbit integration）方法，约束了Pisces和And XVI与M33相互作用的可能情景。

Result: 42%（仙女座XVI）和60%（双鱼座）的矮星系轨道支持它们曾是M33的卫星（即曾位于其维里半径内）。
...
### [13] [Molecular Gas Detections in Eight Faint DSFGs with Red NIR Colors at z = 1.2-2.5](https://arxiv.org/abs/2601.20862)
*Michael J. Nicandro Rosenthal,Stephen J. McKay,Amy J. Barger,Lennox L. Cowie*

**TL;DR**: 利用NOEMA观测8个微弱（平均$S_{\rm 850 μm}=2.3$ mJy）尘埃恒星形成星系（DSFGs）的CO(3-2)、CO(4-3)和[C I]($^3$P$_1$-$^3$P$_0$)谱线，发现低质量（$M_\star \lesssim 10^{11}~{\rm M}_\odot$）星暴星系的分子气体质量分数和消耗时间是现有标度关系预测值的两倍，表明其$M_{\rm mol}$可能因$α_{\rm CO}$降低或CO激发增强而被系统性高估。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（$z = 1.2-2.5$）微弱尘埃恒星形成星系（DSFGs）的分子气体特性及其与恒星形成活动的关系。

Data: 数据来源于使用NOEMA（Northern Extended Millimeter Array）对八个微弱（平均$S_{\rm 850 μm} = 2.3$ mJy）的尘埃恒星形成星系（DSFGs）在$z = 1.2-2.5$红移范围内进行的CO(3-2)、CO(4-3)和[C I]($^3$P$_1$-$^3$P$_0$)谱线观测。

Method: 采用毫米波干涉仪（NOEMA）观测CO(3-2)、CO(4-3)和[C I]($^3$P$_1$-$^3$P$_0$)谱线，并结合紫外至毫米波段的SED拟合分析恒星形成率与恒星质量。

Result: 在低质量（$M_\star \lesssim 10^{11}~{\rm M}_\odot$）星暴星系中，气体分数和消耗时间比分子气体标度关系预测值高两倍，可能表明该群体中分子气体质量（$M_{\rm mol}$）被系统性高估，原因可能是其一氧化碳转换因子（$α_{\rm CO}$）降低或一氧化碳激发增强。
...
### [14] [Bottom-heavy initial mass functions reveal hidden mass in early galaxies](https://arxiv.org/abs/2601.20864)
...
**TL;DR**: 研究发现高红移（z~0.7）大质量静止星系（quiescent galaxies）具有比银河系（MW）更底部重（bottom-heavy）的初始质量函数（IMF），其中最早两个星系（JWST观测的"不可能早期"星系直接后代）的恒星质量因此需修正为原值的3-4倍，加剧了与星系形成模型的矛盾。


<details>
  <summary>Details</summary>

Background: 研究探讨了早期宇宙中巨大星系（massive galaxies）恒星初始质量函数（IMF）与银河系（MW）的差异及其对星系形成模型的挑战。

Data: 数据来源于JWST-IMFERNO项目对红移（redshift）$z\sim0.7$的9个大质量静止星系（quiescent galaxies）的超深光谱观测，并结合LEGA-C[8]的深光谱数据扩展至更短波长。

Method: 采用超深光谱（ultra-deep spectra）结合JWST-IMFERNO和LEGA-C数据，首次测量了红移$z\sim0.7$处静止星系（quiescent galaxies）的初始质量函数（IMF），发现其低质量恒星占比高于银河系（MW），呈现底部重（bottom-heavy）特征。

Result: 首次在遥远宇宙中测量到静止（quiescent）星系具有比银河系更底部重（bottom-heavy）的初始质量函数（IMF），其中两个最古老星系因IMF修正使恒星质量增加3-4倍。
...
### [15] [An X-ray-Emitting Proto-Cluster at $z\approx5.7$ Reveals Rapid Structure Growth](https://arxiv.org/abs/2601.19978)
*Akos Bogdan,Gerrit Schellenberger,Qiong Li,Christopher J. Conselice*

**TL;DR**: 通过钱德拉和JWST联合观测在红移(redshift)z≈5.68处发现了一个原星系团(proto-cluster)JADES-ID1，其X射线辐射和延伸热气体表明大质量晕(halo)中星系团内介质(intracluster medium)的加热过程在早期宇宙就已开始，且该大质量结构的发现暗示早期宇宙某些区域的大尺度结构(large-scale structure)形成速度比标准宇宙学模型预测更快。


<details>
  <summary>Details</summary>

Background: 研究星系团（galaxy clusters）及其前身原星系团（proto-clusters）的形成过程，以揭示宇宙大尺度结构（large-scale structure）的早期组装机制。

Data: 数据来源为钱德拉(Chandra)和詹姆斯·韦伯太空望远镜(JWST)的联合观测数据。

Method: 采用钱德拉(Chandra)X射线天文台和詹姆斯·韦伯太空望远镜(JWST)的联合观测数据，结合X射线光度测量和引力质量推断方法，检测了红移z≈5.68的原星系团JADES-ID1的扩展热气体(ICM)和总质量。

Result: 在红移(redshift)z≈5.68处探测到延伸的激波加热气体，表明早在宇宙诞生后约10亿年时大质量晕(massive halos)中即可发生显著的星系团内介质(intracluster medium, ICM)加热。
...
### [16] [A dynamical systems approach to studying the equivalence principle in dilaton gravity](https://arxiv.org/abs/2601.20156)
*A. M. Velásquez-Toribio*

**TL;DR**: 研究在达穆尔-波利亚科夫（Damour-Polyakov）机制下的弦论启发的膨胀子宇宙学，发现渐近稳定的最小耦合固定点通过宇宙学演化动态抑制等效原理偏离，其阻尼速率由固定点雅可比矩阵特征值决定。


<details>
  <summary>Details</summary>

Background: 研究探讨了弦理论启发的伸缩子(dilaton)宇宙学在Damour-Polyakov(DP)机制下的动力学行为，聚焦于宇宙演化如何调控等效原理(equivalence principle)偏离。

Data: 研究中使用的数据来源于基于达穆尔-波利亚科夫（Damour-Polyakov）机制下的动力学系统模拟（simulation），通过展开共形耦合函数和标量势能构建了一个封闭自洽的自主系统来研究晚期宇宙演化。

Method: 采用动力学系统方法（dynamical-systems methods）在爱因斯坦框架（Einstein frame）下构建封闭自洽的自治系统，通过线性化处理分析最小耦合点附近雅可比矩阵特征值对等效原理偏离的阻尼控制。

Result: 在达穆尔-波利亚科夫（Damour-Polyakov）机制下，宇宙演化通过稳定不动点渐近实现最小耦合，有限时期的偏离决定了等效原理偏差（equivalence principle violation）的幅度。
...
### [17] [Model independent test of the FLRW metric and the curvature in light of DESI DR2](https://arxiv.org/abs/2601.20293)
*Cléa Millard,Benjamin L'Huillier,Marian Douspis*

**TL;DR**: 基于Pantheon+、DESY5超新星数据和DESI DR2重子声波振荡(BAO)测量，通过迭代平滑算法重构距离模数、无量纲共动距离和哈勃参数，并应用$\mathcal{O}_k$诊断检验FLRW度规和宇宙平坦性，发现不同数据组合下宇宙曲率参数(Ω_{k,0}^\text{med})的中位值分别为0.035^{+0.046}_{-0.079}±0.037、0.092^{+0.055}_{-0.132}±0.064和-0.119^{+0.113}_{-0.047}±0.043，且结果不依赖暗能量模型。


<details>
  <summary>Details</summary>

Background: 该研究旨在独立于任何暗能量（Dark Energy）模型，基于最新DESI DR2数据，通过数据驱动方法检验FLRW度规和宇宙平坦性（flatness of the Universe），并验证标准宇宙学模型（concordance model）的基本假设。

Data: 使用了Pantheon+超新星数据、DESY5超新星数据和DESI DR2重子声波振荡（BAO）测量数据。

Method: 采用迭代平滑算法重构距离模数(distance modulus)、无量纲共动距离(dimensionless comoving distance)和哈勃参数(Hubble parameter)，并结合DESI DR2重子声波振荡(BAO)测量数据，运用$\mathcal{O}_k$诊断法检验FLRW度规和宇宙平坦性。

Result: 结合Pantheon+和DESI DR2数据时宇宙曲率参数（Ω_{k,0}^\text{med}）的中位值为0.035^{+0.046}_{-0.079}\pm 0.037，在红移z=1.13处截断Pantheon+数据时为0.092^{+0.055}_{-0.132}\pm 0.064，而DES Y5与DESI DR2组合时为-0.119^{+0.113}_{-0.047}\pm 0.043。
...
### [18] [SPT-3G D1: A Measurement of Secondary Cosmic Microwave Background Anisotropy Power](https://arxiv.org/abs/2601.20551)
...
**TL;DR**: 基于SPT-3G接收器毫米波温度功率谱测量，首次在$\ell=3000$处精确测定了143 GHz频段热SZ效应（thermal SZ）功率为$4.91\pm0.37\,\mu{\rm K}^2$和运动学SZ效应（kinematic SZ）功率为$1.75\pm0.86\,\mu{\rm K}^2$，并通过运动学SZ功率约束再电离（reionization）持续时间上限为$Δ^{50} z_{\rm re}<3.8$（半解析模型）或$Δ^{90} z_{\rm re}<6.1$（AMBER模拟）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过毫米波温度功率谱测量探索宇宙微波背景（CMB）中的热和动力学Sunyaev-Zel'dovich效应（SZ效应）、射电星系及宇宙红外背景（CIB），以约束再电离时期（reionization）的持续时间。

Data: 数据来源于南极望远镜（South Pole Telescope）SPT-3G接收器在95、150和220 GHz三个观测波段收集的两年数据，覆盖南天区1646平方度区域。

Method: 采用毫米波温度功率谱分析技术，结合热和运动学Sunyaev-Zel'dovich效应(SZ)、射电星系及宇宙红外背景(CIB)的多波段建模方法。

Result: 在$\ell=3000$处测得热苏尼亚耶夫-泽尔多维奇效应(Sunyaev-Zel'dovich effect)功率为$D_{3000}^{\rm tSZ} = 4.91\pm0.37\, μ{\rm K}^2$，运动苏尼亚耶夫-泽尔多维奇效应功率为$D_{3000}^{\rm kSZ} =1.75\pm0.86\, μ{\rm K}^2$。
...
### [19] [Apparent Dark-Energy Evolution from Cosmic Inhomogeneities](https://arxiv.org/abs/2601.20633)
*Yonadav Barry Ginat,Pedro G. Ferreira*

**TL;DR**: 研究发现宇宙中适度不均匀性（inhomogeneity）与宇宙学常数（cosmological constant）共同作用可模拟演化暗能量（dark energy）效应，三种不均匀性模型均使距离-红移关系更符合超新星和重子声波振荡观测数据。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙中存在轻微不均匀性（inhomogeneity）时可能被误认为暗能量（dark energy）演化的现象。

Data: 研究使用了来自暗能量巡天（Dark Energy Survey）的Ia型超新星数据和暗能量光谱仪（Dark Energy Spectroscopic Instrument）的重子声波振荡数据。

Method: 通过三种方式建模不均匀性及其影响：作为被空洞包围的团块物质、作为小尺度结构对宇宙整体膨胀的反作用（back-reaction）以及作为大尺度曲率不均匀性。

Result: 在三种不均匀性模型中，光传播效应导致宇宙学观测量（如角直径距离和光度距离）发生畸变，使距离-红移关系更符合暗能量巡天（Dark Energy Survey）的Ia型超新星数据和暗能量光谱仪（Dark Energy Spectroscopic Instrument）的重子声波振荡数据。
...
### [20] [Replicating weak-lensing summary-statistic covariances with normalizing flows](https://arxiv.org/abs/2601.20669)
*Joaquin Armijo,Leander Thiele,Jia Liu*

**TL;DR**: 研究验证了归一化流(NF)生成模型在训练于宇宙学模拟数据时能百分数精度复现弱引力透镜(weak-lensing)关键统计量(角功率谱$C_{\ell}$、概率密度函数和Minkowski泛函)的均值与方差，但协方差矩阵非对角元素低估达25%，而数据增强和含噪声训练可将其精度提升至10%量级。


<details>
  <summary>Details</summary>

Background: 研究探讨了归一化流（normalizing flow, NF）生成模型在宇宙学模拟训练下复现弱引力透镜（weak-lensing）关键统计量的能力。

Data: 数据来源于一组宇宙学模拟（cosmological simulations）生成的弱引力透镜（weak-lensing）收敛（convergence）图（κ-maps）。

Method: 采用归一化流（normalizing flow, NF）生成模型对弱引力透镜（weak-lensing）收敛（κ）图的角功率谱（$C_{\ell}$）、概率密度函数和Minkowski泛函等关键统计量的均值、标准差和协方差进行建模，并通过数据增强和含噪声场训练策略提升协方差矩阵非对角元素的恢复精度。

Result: 归一化流（NF）生成模型在训练于宇宙学模拟数据时能准确复现弱透镜（weak-lensing）统计量的均值与标准差（百分级精度），但对协方差矩阵非对角元素的低估可达25%，需通过数据增强或含噪声场训练等策略将误差降至10%量级。
...
### [21] [How well is the local Large Scale Structure of the Universe known? CosmicFlows vs. Biteau's Galaxy Catalog with Cloning](https://arxiv.org/abs/2601.20808)
*Yifei Li,Glennys R. Farrar*

**TL;DR**: 该研究比较了Biteau (2021)与CosmicFlows项目提出的本地宇宙物质密度场（density field），发现两者在银心遮挡区（ZoA）存在显著差异，并指出Biteau (2021)在11 Mpc内与Local Volume星表一致，但在更大距离上不适用于ZoA区域，而在其他区域优于CosmicFlows，但其径向质量分布因视线速度（line-of-sight peculiar velocities）影响可靠性较低。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过比较不同方法（Biteau 2021、CosmicFlows2和CosmicFlows4）重建的局部宇宙物质密度场（density field），评估其在银河系中心遮挡区（ZoA）及更大尺度上的可靠性差异，旨在为超高能宇宙射线源（ultrahigh energy cosmic ray sources）和暗物质（Dark Matter）湮灭信号预测提供更精确的局部质量密度基准模型。

Data: data部分：研究使用了Biteau (2021)提出的密度场、CosmicFlows2（Hoffman et al. 2018）的准线性密度场、CosmicFlows4（Valade 2026）的平均后验场以及Local Volume catalog（Karachentsev et al. 2018）的星系目录数据。

Method: 采用基于星系目录的补充方案（包括对不可见星系的“克隆”填充）与准线性密度场（quasi-linear density field）及平均后验场（mean posterior field）对比分析的方法。

Result: Biteau (2021)密度场在银河中心遮挡区(ZoA)的径向质量分布可靠性较低，但在其他区域优于CosmicFlows，尤其适用于结构方向和累积质量分析。
...
### [22] [Euclid preparation. Galaxy power spectrum modelling in redshift space](https://arxiv.org/abs/2601.20826)
...
**TL;DR**: 比较了三种红移空间畸变(RSD)建模方法（有效场论(EFT)、速度差生成器(VDG_∞)和BACCO模拟器）对Hα发射线星系功率谱多极子的适用性，发现VDG_∞和BACCO在0.9≤z≤1.8红移范围内优于EFT，其中VDG_∞在小尺度(k_max≳0.30 h Mpc^(-1))的宇宙学参数约束能力最强。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过精确建模红移空间畸变（RSD）来优化从大型星系红移巡天中提取宇宙学信息的能力，特别是为欧几里得（Euclid）光谱数据分析做准备。

Data: 使用了模拟Hα发射线星系（mock Hα emission line galaxies）的功率谱多极子数据，并基于三种方法建模红移空间畸变（RSD）效应：有效场论（EFT）、速度差生成器（VDG∞）和结合微扰论与高分辨率N体模拟的BACCO仿真器（BACCO emulator）。

Method: 采用三种方法建模红移空间畸变（RSD）效应：有效场论（EFT）、速度差生成器（VDG_∞）和结合微扰论与N体模拟的BACCO仿真器。

Result: 在红移区间0.9≤z≤1.8内，基于速度差生成器（VDG_∞）和BACCO混合方法的模型在波数k_max≲0.35 h Mpc^(-1)范围内比有效场论（EFT）更准确地恢复了关键宇宙学参数（h、ω_c、A_s），其中VDG_∞对小尺度红移空间畸变（RSD）的改进处理使参数约束精度最高提升两倍。
...
### [23] [POEMMA-Balloon with Radio: A multi-messenger, multi-detector balloon payload](https://arxiv.org/abs/2601.19997)
...
**TL;DR**: 论文提出POEMMA-Balloon with Radio (PBR)气球观测站方案，通过结合广角施密特望远镜(Schmidt telescope)和混合光学焦面与专用射电设备，首次实现亚轨道高度对超高能宇宙线(UHECR)荧光信号的验证观测，并同步获取膝区能量(E>3PeV)水平空气簇射的光学切伦科夫(Cherenkov)与射电联合测量数据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超高能宇宙射线（UHECR）领域的前沿科学问题及下一代探测器的必要性。

Data: data：POEMMA-Balloon with Radio (PBR)采用高空气球搭载的施密特望远镜(Schmidt telescope)和混合光学焦面(hybrid optical focal surface)结合专用射电仪器，获取广延空气簇射(extensive air showers)的同步互补观测数据。

Method: 采用双卫星观测站（POEMMA）概念设计的紧凑型气球载探测器（PBR），结合广视场施密特望远镜（Schmidt telescope）、混合光学焦面及专用射电设备，实现对广延空气簇射（EAS）的同步互补测量。

Result: 需要高精度仪器和最大化超高能暴露量的探测器来解决超高能宇宙线(UHECR)领域的开放性问题。
...
### [24] [The Sound of Noise: Leveraging the Inductive Bias of Pre-trained Audio Transformers for Glitch Identification in LIGO](https://arxiv.org/abs/2601.20034)
*Suyash Deshmukh,Chayan Chatterjee,Abigail Petulante,Tabata Aira Ferreira,Karan Jani*

**TL;DR**: 提出基于音频预训练模型（Audio Spectrogram Transformer）的跨域框架，通过迁移学习将自然声音表征应用于引力波（GW）干涉仪瞬态噪声（glitches）和中等质量黑洞（IMBH）合并信号的分类，显著提升特征提取效率并克服传统监督方法的标签瓶颈问题。


<details>
  <summary>Details</summary>

Background: 引力波(GW)干涉仪中瞬态噪声伪迹(glitch)的存在限制了探测灵敏度，并可能模仿真实的天体物理信号，尤其是短时标的中等质量黑洞(IMBH)并合事件。

Data: data：研究使用了来自LIGO探测器第三次（O3）和第四次（O4）观测运行的应变（strain）数据。

Method: 采用预训练的音频谱图变换器(Audio Spectrogram Transformer, AST)模型，通过跨域迁移学习方法将自然声音的表征迁移至引力波(GW)数据噪声和信号分析中，并结合t-SNE无监督聚类技术进行特征可视化。

Result: 提出了一种基于音频预训练模型（Audio Spectrogram Transformer）的跨域框架，通过迁移学习将自然声音的表征能力应用于引力波（GW）数据中的噪声伪影（glitches）和中间质量黑洞（IMBH）信号分类，相比传统监督方法展现出更优的特征提取性能。
...
### [25] [RM-Tools: Software for Analyzing Polarized Radio Spectra](https://arxiv.org/abs/2601.20092)
...
**TL;DR**: 开发了用于从线性偏振射电源（linearly polarized radio sources）的频谱偏振观测中提取法拉第旋转量（Faraday rotation measures）等偏振特性的分析软件RM-Tools，支持旋转量合成（rotation measure synthesis）和QU模型拟合（QU-model fitting）等技术，现被POSSUM和VLASS等大面积偏振巡天项目采用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于现代射电望远镜宽带多通道偏振观测数据的可靠科学结果提取技术。

Data: 数据来源于现代射电望远镜（radio telescopes）的宽频带多通道偏振观测，包括POSSUM和VLASS等大面积偏振巡天项目的数据。

Method: 采用旋转测量合成（rotation measure synthesis）和QU模型拟合（QU-model fitting）技术分析射电偏振数据。

Result: 开发了用于从线性偏振射电源的频谱偏振观测中提取法拉第旋转量等偏振特性的分析软件RM-Tools。
...
### [26] [Probabilistic Interpolation of Sagittarius A*'s Multi-Wavelength Light Curves Using Diffusion Models](https://arxiv.org/abs/2601.20863)
*Gabriel Sasseville,Julie Hlavacek-Larrondo,Daryl Haggard,Alexandre Adam,Hadrien Paugnat,Gunther Witzel*

**TL;DR**: 首次将基于分数扩散模型（score-based diffusion models）和变压器模型（transformer-based model）应用于人马座A*（Sgr A*）多波段光变曲线重建，显著提升了稀疏、含噪声天文时间序列的插值精度和不确定性校准能力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用多波段观测数据探究人马座A*（Sgr A*）黑洞事件视界附近的吸积与辐射物理过程。

Data: 数据来自钱德拉（Chandra，X射线2-8 keV）、GRAVITY（甚大望远镜近红外2.2微米）、斯皮策（Spitzer，红外4.5微米）和ALMA（亚毫米波340 GHz）四个关键观测台的多波段实测数据，并结合模拟光变曲线（模拟数据匹配了真实Sgr A*的统计与观测特征）。

Method: 采用基于扩散的生成模型（diffusion-based generative model）和基于Transformer的光变曲线重建模型，用于插值稀疏多变量天体物理时间序列，并首次将基于分数的扩散模型（score-based diffusion models）应用于天文时间序列。

Result: 首次将基于分数扩散模型（score-based diffusion models）应用于天文时间序列插值，并开发了首个包含校准不确定性估计的基于变压器（transformer）的光变曲线重建模型，扩散模型在模拟和真实数据上均实现了更高精度和竞争性校准能力。
...
### [27] [The CO snow line favours strong clumping by the streaming instability in protoplanetary discs with porous grains](https://arxiv.org/abs/2601.19985)
*Jean-François Gonzalez,Stéphane Michoulier*

**TL;DR**: 研究发现原行星盘（protoplanetary discs）中多孔颗粒（porous grains）的存在和CO雪线（CO snow line）位置共同促进了流不稳定性（streaming instability）导致强团簇（strong clumping）的条件，从而在十万年内大范围区域满足星子（planetesimals）形成的初始条件。


<details>
  <summary>Details</summary>

Background: 研究探讨了原行星盘（protoplanetary discs）中多孔尘埃颗粒（porous grains）如何通过流不稳定性（streaming instability）克服径向漂移和碎裂障碍，促进星子（planetesimals）形成的条件。

Data: data：基于前人模拟的原行星盘（protoplanetary discs）中多孔颗粒（porous grains）在生长、碎裂、压实和反弹作用下的演化结果。

Method: 采用包含外部湍流的流不稳定性（streaming instability, SI）模拟方法，结合多孔颗粒（porous grains）的生长、破碎、压实和弹跳演化模拟结果，分析原行星盘中满足强团簇条件的区域。

Result: 多孔颗粒（porous grains）在考虑CO雪线（CO snow line）的情况下，可在原行星盘（protoplanetary discs）大范围内于前十万年内满足强团簇（strong clumping）条件。
...
### [28] [Twenty-four thousand hours of GREENBURST observations with the GBT](https://arxiv.org/abs/2601.20143)
...
**TL;DR**: GREENBURST项目利用绿岸望远镜（Green Bank Telescope）的共享观测时间开展快速射电暴（FRB）搜索，在24,186小时观测中探测到50颗脉冲星（包括新发现的周期2.2秒、特征年龄200万年且具有高零脉冲比例（$\sim70-80\%$）的PSR J0039+5407）和3个已知重复FRB源，并指出单波束设计对区分射频干扰与新FRB的挑战。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用大型射电望远镜（如绿岸望远镜GBT）的伴生观测机会开展快速射电暴（FRB）和暂现源搜寻，并探讨单波束系统在区分真实天体信号与射频干扰方面的挑战。

Data: 数据来源于利用绿岸望远镜（Green Bank Telescope, GBT）进行的GREENBURST搜索项目，共分析了24,186小时的观测数据，并后续使用加拿大氢强度测绘实验（Canadian Hydrogen Intensity Mapping Experiment）进行跟踪观测。

Method: 采用共位观测（commensal observation）技术，结合绿岸望远镜（Green Bank Telescope）的GREENBURST系统进行色散脉冲搜索，并通过加拿大氢强度测绘实验（Canadian Hydrogen Intensity Mapping Experiment）开展后续计时观测。

Result: GREENBURST项目利用绿岸望远镜（Green Bank Telescope）在24,186小时的观测中发现了50颗脉冲星（pulsar）和3个快速射电暴（FRB），其中PSR J0039+5407是新发现的周期为2.2秒的脉冲星，特征年龄为200万年且具有高达70-80%的零脉冲比例（nulling fraction），而检测到的3个FRB均为已知重复暴源。
...
### [29] [The Need for Ultra High Resolution X-ray Imaging](https://arxiv.org/abs/2601.20823)
...
**TL;DR**: 论文提出在软至中能X射线波段（0.5-8 keV）实现毫角秒至微角秒成像分辨率对揭示最小天体物理尺度（如吸积过程）的关键科学价值，并探讨了当前技术（如正在研究的吸积探测器(Accretion Explorer)任务架构）的可行性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升软至中等能段X射线（soft to medium-energy X-ray band）成像分辨率至毫角秒（milliarcsecond）至微角秒（microarcsecond）级别的重要性，以弥补当前X射线观测在小尺度天体物理现象研究中的滞后性。

Data: 研究中使用的数据来源为当前技术或任务概念（如Accretion Explorer）的模拟（simulation）数据。

Method: 采用分散孔径X射线干涉仪（dispersed aperture X-ray interferometer）技术实现毫角秒至微角秒级软X射线至中能X射线（0.5-8 keV）高分辨率成像。

Result: 高分辨率软至中能X射线（soft to medium-energy X-ray）成像对揭示吸积（accretion）、喷流（jet）等关键天体物理小尺度过程至关重要。
...
### [30] [From cosmological simulations to binary black hole mergers: The impact of using analytical star formation history models on gravitational-wave source populations](https://arxiv.org/abs/2601.20202)
*Sasha Levina,Floor Broekgaarden,Lieke van Son,Emanuele Berti,Amedeo Romagnolo,Ruediger Pakmor,Ana Lam*

**TL;DR**: 基于宇宙学模拟（IllustrisTNG）评估了金属丰度-红移依赖的恒星形成率密度（S(Z, z)）解析近似对双黑洞（BBH）并合率和质量分布的影响，发现高红移（z≳6）下解析模型会高估并合率10-10⁴倍并引入虚假质量特征（如8M⊙附近人为峰），揭示了模拟S(Z, z)中高金属丰度隆起和低金属丰度平坦尾对准确建模BBH群体的关键作用。


<details>
  <summary>Details</summary>

Background: 研究探讨了双黑洞（BBH）并合与宇宙不同时期大质量恒星演化之间的联系，聚焦于精确模拟宇宙恒星形成和化学富集历史对理解BBH种群的关键作用。

Data: 研究使用了IllustrisTNG宇宙学模拟数据来评估金属度特异性宇宙恒星形成率密度S(Z, z)解析拟合的准确性，并考察模拟分辨率和体积对推断S(Z, z)的影响。

Method: 采用宇宙学模拟（IllustrisTNG）与种群合成代码（COMPAS）耦合的方法，评估了金属丰度特定恒星形成率密度（S(Z, z)）解析拟合的准确性，并分析了模拟分辨率与体积对S(Z, z)推断的影响。

Result: 解析的S(Z, z)模型在高红移(z≳6)会高估双黑洞(BBH)并合率10-10⁴倍，并在低红移(z≲2)产生8M⊙附近虚假的主质量峰，同时抑制高质量特征。
...
### [31] [Graviton energy spectra arising from the KSVZ axion model](https://arxiv.org/abs/2601.20767)
*Yonghua Wang,Lin-Yun He,Wei Chao,Yu Gao*

**TL;DR**: 论文核心贡献为：在KSVZ轴子模型中计算了重标量粒子（heavy scalar）或类矢量重夸克（VLQ）衰变产生的引力子轫致辐射能谱，发现这些重粒子可发射峰值频率依赖模型参数的超高频率引力波（GWs），且其谱在高频区区别于由这些粒子诱导的早期物质主导时期产生的热引力波背景，未来超高频率引力波测量可能为KSVZ轴子提供间接证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于轴子（axion）作为佩塞-奎恩（Peccei-Quinn）对称性自发破缺产生的戈德斯通玻色子（goldstone boson），其动力学机制解决了强CP问题并作为暗物质候选者，同时探索轴子模型中重粒子（如重标量s和类矢量重夸克）通过引力子轫致辐射（graviton bremsstrahlung）产生超高频率引力波的信号特征。

Data: 研究中使用的数据来源于KSVZ轴子模型中重标量粒子（heavy scalar s）和类矢量重夸克（VLQ）衰变产生的引力子轫致辐射能谱的理论计算。

Method: 计算了KSVZ模型中重标量粒子（heavy scalar）或类矢量重夸克（VLQ）衰变产生的引力子轫致辐射能谱。

Result: KSVZ轴子模型中的重标量粒子（heavy scalar）或类矢量重夸克（VLQ）衰变可产生超高频率引力波（GWs），其峰值频率取决于模型参数输入。
...
### [32] [Enhancing online estimation of CBC parameters with the low-latency MBTA analysis](https://arxiv.org/abs/2601.20512)
...
**TL;DR**: 提出基于信噪比优化技术（SNR optimizer）过滤密集局部模板库的方法，为致密双星并合（CBC）引力波参数在线生成后验分布，其性能经模拟和O4a观测数据验证与LVK结果一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升致密双星并合（CBC）引力波事件在线参数后验分布估计的实时性和准确性。

Data: 数据来源包括仿真模拟数据和LIGO-Virgo-KAGRA（LVK）合作组发布的第四次观测运行第一阶段（O4a）实测数据。

Method: 采用信噪比优化技术（SNR optimizer technique）过滤密集局部模板库（dense local template banks）来估计致密双星并合（CBC）引力波参数的后验分布。

Result: 开发了一种基于信噪比优化技术（SNR optimizer technique）的在线后验分布生成方法，用于紧凑双星并合（CBC）引力波参数估计。
...
### [33] [General Scalar Field Inflation ACT Attractors: Utilizing the $n_s(r)$ relation](https://arxiv.org/abs/2601.20053)
*V. K. Oikonomou*

**TL;DR**: 研究发现三类与ACT数据兼容的单标量场暴胀（inflation）模型，其势能形式为$n_s(r)=γ\pm βr \pm r^{1/2}$的变体，突破了传统高原势（plateau potentials）模型的限制。


<details>
  <summary>Details</summary>

Background: 研究旨在探索与ACT（阿塔卡马宇宙学望远镜）数据兼容的单标量场暴胀宇宙学模型，特别关注吸引子关系（attractor relation）$n_s(r)=f(r)$的构建及其对应的势能形式。

Data: 数据来源于阿塔卡马宇宙学望远镜（ACT）的观测数据。

Method: 采用微分方程求解方法，通过固定吸引子关系形式$n_s(r)=f(r)$反推得到满足ACT数据的单标量场暴胀势能（potential）变体。

Result: 发现三种与ACT数据兼容的势能（potential）变体，其形式为n_s(r)=γ±βr±r^(1/2)。
...
### [34] [A Remedy of the Trans-Planckian Censorship Problem with Smooth Slow-roll to Power-law Inflation Transitions in Scalar Field Theory](https://arxiv.org/abs/2601.20056)
*S. D. Odintsov,V. K. Oikonomou*

**TL;DR**: 通过推广单标量场（single scalar field）的动能项形式$\dotφ^2=β(φ)V(φ)$，实现了慢滚（slow-roll）与幂律（power-law）暴胀阶段的平滑过渡，且慢滚阶段仅需约30个$e$-折叠（$e$-foldings）即可解决平坦性和视界问题，与阿塔卡马宇宙学望远镜（Atacama Cosmology Telescope）数据相符。


<details>
  <summary>Details</summary>

Background: 研究探讨了在单标量场暴胀（single scalar field inflation）模型中实现慢滚（slow-roll）与幂律暴胀（power-law inflationary regime）平滑过渡的理论机制。

Data: 研究中使用的数据来源为理论推导（simulation），基于广义单标量场动能项$\dotφ^2=β(φ)V(φ)$的数学模型进行分析。

Method: 通过推广单标量场（single scalar field）的动能项为$\dotφ^2=β(φ)V(φ)$形式并选择适当的函数$β(φ)$，实现了慢滚（slow-roll）与幂律（power-law）暴胀机制间的平滑过渡。

Result: 通过推广单标量场的动能形式为$\dotφ^2=β(φ)V(φ)$，可实现慢滚暴胀（slow-roll inflation）与幂律暴胀（power-law inflation）之间的平滑过渡。
...
### [35] [An Analytic Scale-dependent Dark Matter Profile and the Baryonic Tully-Fisher Relation](https://arxiv.org/abs/2601.20070)
*V. K. Oikonomou*

**TL;DR**: 提出了一种基于状态方程（equation of state）随半径变化的自相互作用暗物质（self-interacting dark matter）解析模型，成功解释了116个低表面亮度星系、不规则星系、低光度旋涡星系和矮星系的旋转曲线，并推导出重子物质-塔尔-费舍尔关系（baryonic Tully-Fisher relation）为$M_b\sim V_{flat}^{4.026 \pm 0.371}$。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于探索自相互作用暗物质（self-interacting dark matter）模型在解释低表面亮度星系、不规则星系、低光度旋涡星系及矮星系旋转曲线时的适用性，以解决冷暗物质（cold dark matter）模型在这些天体中的理论挑战。

Data: 数据来源于SPARC星系数据库，共测试了175个星系。

Method: 采用尺度依赖状态方程的自相互作用暗物质(self-interacting dark matter)模型，其中有效状态方程形式为$P(r)=K(r)\left(\frac{ρ(r)}{ρ_{\star}}\right)$，能量密度分布为$ρ(r)=\frac{ρ_0}{\left( 1+\frac{r^2}{α^2}\right)^{5/2}}$，熵函数$K(r)=\frac{K_0}{\left( 1+\frac{r^2}{α^2}\right)^{1/2}}$。

Result: 提出的自相互作用暗物质（self-interacting dark matter）解析模型成功解释了116个低表面亮度星系、不规则星系、低光度旋涡星系和矮星系的旋转曲线。
...
### [36] [Quantum Cosmology as a Hydrogen atom: Discrete $Λ$ and cyclic Universes from Wheeler-DeWitt quantization](https://arxiv.org/abs/2601.20286)
*Dipayan Mukherjee,Harkirat Singh Sahota,S. Shankaranarayanan*

**TL;DR**: 研究发现负宇宙学常数（$Λ< 0$）的FLRW（Friedmann-Lemaître-Robertson-Walker）平直宇宙模型具有离散能谱，量子化了宇宙常数，并通过氢原子类比揭示了宇宙循环演化（cyclic universe）中量子反弹（quantum bounce）与经典转折点（classical turnaround）的交替机制，解析了大爆炸（Big Bang）与大挤压（Big Crunch）奇点。


<details>
  <summary>Details</summary>

Background: 研究探讨了负宇宙学常数（$Λ< 0$）在平坦FLRW宇宙尘埃模型中的量子宇宙学与氢原子对应关系。

Data: 使用负宇宙学常数（Λ < 0）的平坦FLRW（Friedmann-Lemaître-Robertson-Walker）宇宙尘埃模型进行理论模拟（simulation）研究。

Method: 采用量子宇宙学与氢原子类比的方法，在负宇宙学常数($Λ<0$)的平直FLRW宇宙中建立可解模型，通过波函数叠加构建波包，解析观测量的期望值。

Result: 负宇宙学常数（Λ<0）条件下，宇宙能量本征值呈现离散谱，导致宇宙学常数有效量子化，并通过波包构造证明经典大爆炸与大挤压奇点被量子反弹取代，形成由量子反弹与经典转折点构成的振荡循环宇宙。
...
### [37] [A constrained linear model for continuum normalization of stellar spectra](https://arxiv.org/abs/2601.19973)
*Andrew R. Casey,Adam Wheeler,Megan Bedell,David W. Hogg,Andrew Sayjdari,Lily Zhao*

**TL;DR**: 该论文提出了一种约束线性模型，通过非负矩阵分解（non-negative matrix factorization）同时拟合恒星吸收、大气透射（telluric transmission）和仪器-连续谱联合响应（joint continuum-instrument response），实现了无需初始猜测的高效、稳定且精确的光谱分析，在ESO/HARPS高分辨率光谱数据中达到了优于0.5%的跨时间一致性。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过正向建模（forward modeling）同时拟合恒星吸收谱、大气透射谱（telluric transmission）和连续谱-仪器响应（continuum-instrument response）的约束线性模型，以解决传统光谱归一化方法导致的参数偏差或信噪比不一致问题。

Data: 研究中使用的数据来源为ESO/HARPS（欧洲南方天文台高精度径向速度行星搜索器）获取的高分辨率阶梯光栅光谱（echelle spectra），涵盖BAFGKM型恒星（包括重复观测的α-半人马座A）。

Method: 采用约束线性模型（constrained linear model）将理论光谱网格分解为两个非负矩阵，以拟合恒星吸收、大气透射和连续谱-仪器响应，并通过非负性约束确保基向量严格可加。

Result: 提出了一种约束线性模型，可同时拟合恒星吸收、大气透射和连续谱-仪器联合响应，通过非负矩阵分解将理论光谱网格分解为少量基向量，实现高效、稳定且无需初始猜测的凸优化推断。
...
### [38] [The IACOB project: XVI. Surface helium abundances in Galactic O-type stars: indications for identifying binary interaction products](https://arxiv.org/abs/2601.20698)
...
**TL;DR**: 通过对318颗银河系O型星（O-type stars）的均匀光谱分析发现，22%的恒星表面存在明显氦（He）富集（YHe>0.13），且多数富氦星可能是双星相互作用（binary interaction）的产物，挑战了传统认为的孤立恒星内部旋转混合（internal rotational mixing）主导机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量O型星表面化学元素（特别是氦）富集现象的成因机制，挑战了传统认为自转混合（rotational mixing）主导的观点，强调双星相互作用（binary interaction）尤其是物质转移（mass-transfer）可能起关键作用。

Data: 数据来源于IACOB项目的高质量光谱数据，对318颗银河系O型恒星进行了均匀的光谱分析。

Method: 采用IACOB-BROAD和FASTWIND/IACOB-GBAT工具对318颗银河系O型星进行均质化光谱分析，同时考虑双星相互作用、逃逸状态及参数简并性（如微湍流、星风特性、诊断谱线和伴星污染）对丰度测定的影响。

Result: 通过分析318颗银河系O型恒星的光谱数据，发现22%的恒星表面氦丰度（YHe>0.13）显著富集，且多数富集恒星可能是双星相互作用（binary interaction）的产物。
