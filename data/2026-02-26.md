...
**TL;DR**: 通过CaHK窄带测光发现Crater II矮星系存在延伸至少7°（约95千秒差距）的潮汐尾（tidal tails）和首次探测到的金属丰度梯度（-0.34±0.17 dex/度），表明该星系已损失≥25%初始恒星质量，但其暗物质晕（dark matter halo）密度轮廓（"cored"或"cuspy"）仍不明确。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解释Crater II矮球状星系（CraII）在冷暗物质（cold dark matter）模拟中难以复现的低密度和大尺度特性，特别是通过潮汐剥离（tidal disruption）机制探讨其异常观测性质。

Data: 数据来源于暗能量相机（Dark Energy Camera）对Ca II H和K线的窄带测光观测，覆盖了Crater II中心及已识别潮汐尾的128平方度天区，作为MAGIC巡天的一部分。

Method: 采用基于暗能量相机(Dark Energy Camera)的Ca II H和 K线窄带测光技术，结合光度金属丰度、颜色-星等、自行运动和视差筛选方法，识别Crater II候选体并分析其潮汐尾特征。

Result: 在Crater II（CraII）星系中心首次检测到金属丰度梯度（-0.34±0.17 dex deg^-1），并推断潮汐尾宽度为w∼0.8°，比CraII半光半径大约50%。
...
**TL;DR**: 该论文通过积分视场光谱（integral field spectroscopy）和MUSE WFM观测，提出了一种联合建模和扣除1型活动星系核（Type 1 AGN）中类星体（QSO）光的方法，并在喷流和伽马射线源PKS 2153-68（z=0.028）中首次发现了一种未解析、未识别的高速度（~25000 km/s）短时标（≤1年）可变发射现象。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决传统方法在探究1型活动星系核（Type 1 AGN）中心区域时因宿主星系光谱与吸积盘及窄线区/宽线区（NLR/BLR）强发射混合而难以准确解析星系与活动星系核特性的问题。

Data: 数据来源于MUSE（Multi Unit Spectroscopic Explorer）宽场模式（WFM）的观测数据。

Method: 采用积分视场光谱（integral field spectroscopy）结合MUSE宽场模式（MUSE WFM）观测数据，开发了类星体（QSO）光建模与扣除方法以分离寄主星系与活动星系核（AGN）光谱。

Result: 发现了一种未解析且尚未识别的高速度（约25000 km/s）短时标（≤1年）可变发射现象，不同于其他可变活动星系核（AGN）中观测到的任何现象。
...
**TL;DR**: 通过分析约60,000个邻近活动星系核(AGN)和50万个宿主星系的新环境与晕质量数据，发现主流宇宙学模拟(SIMBA、TNG、EAGLE)在再现恒星形成、AGN光度、恒星质量与晕质量关系时存在显著偏差，尤其错误预测了大质量晕中低质量卫星星系及不同环境下的致密星系比例，凸显需改进AGN反馈与多相气体冷却模型。


<details>
  <summary>Details</summary>

Background: 星系中恒星形成受内部和环境过程共同调控，但二者相对作用机制尚不明确，尤其超大质量黑洞反馈(AGN feedback)在不同环境和时间尺度上的影响因观测与建模限制而存在显著认知缺口。

Data: 数据来源包括约60,000个邻近活动星系核(AGN)（红移z < 0.15）和约500,000个AGN与非AGN宿主星系的新环境与晕质量测量数据，并与SIMBA、TNG和EAGLE三个主流宇宙学模拟进行对比分析。

Method: 采用大样本统计分析（~60,000个邻近活动星系核(AGN)和~500,000个宿主星系）结合三种主流宇宙学模拟（SIMBA、TNG、EAGLE）的对比验证方法。

Result: 主要科学发现是：当前主流宇宙学模拟（SIMBA、TNG和EAGLE）无法准确再现观测到的恒星形成、静止状态、活动星系核（AGN）光度、恒星质量与晕质量之间的关联趋势，且在低质量卫星星系静止比例和大质量中心星系静止比例方面存在系统性偏差。
...
**TL;DR**: 通过帕邢(Paschen)发射线和巴耳末(Balmer)线测量红移z=1.43-2.65的77个星系，发现帕邢发射集中在星系外围且比连续谱覆盖区域小，表明当前恒星形成呈由内向外模式，同时帕邢线对高遮蔽OB星协的探测灵敏度更高，能更准确测量尘埃消光和恒星形成率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用帕邢(Paschen)和巴尔末(Balmer)发射线探测红移$z=1.43-2.65$星系的尘埃衰减和恒星形成率(SFRs)，揭示高红移星系内部恒星形成空间分布与尘埃遮蔽效应。

Data: 数据来源包括JWST FRESCO巡天的帕邢(Paschen)发射线探测和MOSDEF地面巡天的巴尔末(Balmer)线测量。

Method: 采用帕邢(Paschen)发射线和巴耳末(Balmer)发射线比率(Paα/Hα、Paβ/Hα)以及巴耳末减幅(Hα/Hβ)，结合银河系消光曲线，计算电离气体红化和尘埃校正后的恒星形成率(SFR)。

Result: 帕申线（Paschen lines）对高红移星系（$z\sim2$）中尘埃遮蔽最严重的OB星协（OB associations）具有更高的探测灵敏度，尤其在尘埃分布不均匀的星系中能提供更稳健的消光测量。
...
**TL;DR**: 对部分嵌入星团AFGL 6366S的近红外(NIR)和光学偏振观测发现，其中心区域存在由辐射扭矩破坏(RAT-D)和大颗粒消偏振及磁场纠缠导致的偏振空洞，局部磁场与银河系大尺度磁场及区域纤维长轴显著错位，并包裹着两个分别处于超临界和亚临界状态的致密分子团块，形成垂直于磁场的U型场曲率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近红外(NIR)和光学波段偏振测量在部分嵌入星团AFGL 6366S中的应用，探讨星际介质中尘埃颗粒、磁场结构与恒星形成过程的相互作用机制。

Data: 使用近红外(NIR)和光学波段线性偏振观测数据，偏振度范围分别为0.44-10.3%和0.16-11.22%，位置角覆盖1°-179°。

Method: 采用近红外(NIR)和光学波段线性偏振测量技术分析AFGL 6366S部分嵌入星团的偏振特性，并结合辐射扭矩破坏(RAT-D)模型和磁场纠缠效应解释中心区域偏振空洞现象。

Result: 在AFGL 6366S部分嵌入星团中心区域观测到极化空洞（polarization hole），主要由大颗粒辐射扭矩破坏（RAT-D）和磁场纠缠导致去极化效应引起。
...
**TL;DR**: 利用MeerKAT望远镜对LMC（大麦哲伦云）开展1.3 GHz射电连续谱巡天，首次发现新超新星遗迹候选体、未检测到射电信号的行星状星云和沃尔夫-拉叶星，并揭示30 Doradus恒星形成区及前景/背景特殊射电源的精细结构。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用MeerKAT射电望远镜对小麦哲伦云(LMC)进行全斯托克斯(Full-Stokes)连续谱巡天观测，旨在揭示其射电辐射特征并支持多类天体物理研究。

Data: 数据来源于使用MeerKAT望远镜在1.3 GHz中心频率、0.8 GHz带宽下对LMC（Large Magellanic Cloud）进行的射电连续谱巡天观测，成像产品包含6个视场（field of view），每个视场覆盖约5°×5°区域，图像分辨率达到8"，斯托克斯I（Stokes I）宽带图像的中值均方根噪声约为11 μJy beam⁻¹。

Method: 采用1.3 GHz中心频率、0.8 GHz带宽的MeerKAT射电望远镜全斯托克斯(Full-Stokes)成像技术，生成6个视场(各约5°×5°)、分辨率8角秒的连续谱图像。

Result: 首次利用MeerKAT望远镜对LMC（大麦哲伦云）开展1.3 GHz频段（带宽0.8 GHz）的射电连续谱巡天，成像覆盖6个视场（每个约5°×5°），分辨率达8"，斯托克斯I图像均方根噪声中值为∼11 μJy beam⁻¹。
...
**TL;DR**: 利用ALMA CO(J=2-1)观测在13pc分辨率下测量了NGC7793和NGC1313整个星系盘湍流驱动模式(b参数)的空间分布，发现两星系平均以螺线管驱动为主(b值分别为0.33±0.05和0.24±0.03)，但存在显著空间变化且NGC1313湍流马赫数更高，表明湍流驱动模式随星系环境系统性变化并与恒星形成调控相关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系尺度湍流驱动模式（turbulence driving modes）的空间分布及其对恒星形成调控的影响，通过高分辨率观测揭示湍流参数b（表征螺线型与压缩型驱动平衡）在星系盘中的系统性变化。

Data: 数据来源于阿塔卡马大型毫米波/亚毫米波阵列(ALMA)对NGC7793和NGC1313的CO(J=2-1)谱线观测，空间分辨率达到13秒差距。

Method: 采用基于核(kernel-based)的密度和速度涨落分析方法，量化湍流驱动参数b（表征螺线型(solenoidal)与压缩型(compressive)湍流驱动模式的平衡）。

Result: NGC7793和NGC1313的湍流驱动模式（turbulence driving modes）在空间上存在显著变化，平均以螺线型（solenoidal）驱动为主（NGC7793的b≥0.33±0.05+0.14−0.10，NGC1313的b≥0.24±0.03+0.10−0.07），但局部区域存在强烈压缩型（compressive）驱动。
...
**TL;DR**: 基于24,954个本地恒星形成星系的大样本，研究发现恒星形成主序(SFMS)的表观相关性完全由分子气体面密度介导，证实分子气体质量与恒星质量的相关性（MGMS）及分子气体与恒星形成的相关性（KS关系）更为直接和根本。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星形成主序（SFMS）的宏观驱动机制，探讨恒星形成率与恒星质量在全局尺度上的关联性及其与分子气体质量的内在联系。

Data: 数据来源于一个包含24,954个本地恒星形成星系的大体积限制样本，基于中红外12微米光度、恒星质量和g-r颜色估算分子气体质量。

Method: 采用多元回归分析和偏相关分析探究恒星形成率面密度与分子气体面密度及恒星质量面密度之间的直接关系。

Result: 在星系尺度上，恒星形成率面密度与恒星质量面密度的表观相关性完全由分子气体面密度介导，其最佳拟合参数可直接从肯尼卡特-施密特(KS)关系和分子气体主序(MGMS)关系中导出。
...
**TL;DR**: 通过多波段观测发现NGC 6447在2018年从静止星系转变为活动星系核(AGN)，其红外光变和X射线特性符合AGN启动过程中遮蔽物质逐渐消散的预期，并估算此类事件持续时间(duty cycle)为10^4-10^6年。


<details>
  <summary>Details</summary>

Background: 研究探讨了活动星系核(AGN)从静止星系(quiescent galaxy)到活跃阶段的初始开启(turning-on)过程，重点关注尘埃遮蔽(obscured accretion)阶段作为这一转变的关键环节。

Data: 研究中使用的数据来源包括WISE/NEOWISE的中红外3.5μm和4.6μm波段光变曲线、ASAS-SN的光学光变曲线、SPHEREx光谱以及NuSTAR和Swift的X射线观测数据。

Method: 采用多波段光变曲线分析（WISE/NEOWISE中红外、ASAS-SN光学）、SPHEREx光谱分析及NuSTAR X射线观测相结合的方法。

Result: NGC 6447的中红外光变曲线显示其在14年内亮度单调增加1.2星等，光学光变曲线保持稳定，表明该星系在2018年转变为活动星系核(AGN)。
...
### [10] [Probing Large-scale Structure and the Multi-Phase IGM at the Cosmic Noon -- Insights from a Joint Survey with Euclid, CSST, JPCam, and JUST](https://arxiv.org/abs/2602.22004)
*Jiang-Tao Li,Renato A. Dupke,Yan Gong,Zhijie Qu,Weichen Wang,Xiaohu Yang,Xiaodi Yu*

**TL;DR**: 提出了一种结合欧几里得(Euclid)、中国空间站望远镜(CSST)、JPCam和JUST的协同观测方案，通过多波段测光、窄带成像和吸收线光谱技术，在宇宙正午时期(~20平方度天区)建立了研究星系生态系统(星系-星系际介质(CGM/IGM)关联)的综合框架，显著提高了光红移(photo-z)精度并实现了延展[O II]发射CGM晕的探测，但发现现有设备无法探测弥散IGM的Mg II吸收。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是探索宇宙正午时期（cosmic noon）约20平方度天区内多相星系际介质（CGM/IGM）与星系生态系统的关联机制。

Data: 数据来源包括欧几里得（Euclid）和CSST（China Space Station Telescope）的宽波段测光数据、JPCam的窄波段成像数据、JUST（Jilin University Spectroscopic Telescope）的背景星系吸收线光谱数据，以及模拟构建的3D气体分布模型和真实星系目录。

Method: 采用宽窄带测光结合提升光变红移（photo-z）精度，通过窄带成像探测延展CGM发射，并利用背景星系吸收线光谱测绘前景IGM，构建了整合观测框架。

Result: 通过结合宽窄带测光显著提升红移测量精度，尤其改善了z~1.0-1.4恒星形成星系的光度红移（photo-z）准确性。
...
### [11] [Performance assessment of the gPLUTO code for the numerical modeling of radio galaxy evolution](https://arxiv.org/abs/2602.21560)
*Gourab Giri,Andrea Mignone,Alessio Suriano,Marco Rossazza,Stefano Truzzi*

**TL;DR**: 开发了GPU加速的PLUTO代码（gPLUTO），显著提升了天体物理喷流（astrophysical jets）模拟效率（提速10-30倍），并首次实现高分辨率三维模拟巨型射电星系喷流（GRGs），揭示了环境因素对其形成、快速外围扩张和非对称茧形态发展的影响。


<details>
  <summary>Details</summary>

Background: 现代计算天体物理研究中，高分辨率三轴模拟对精确建模天体物理喷流（astrophysical jets）的动力学特征和辐射特征至关重要。

Data: 数据来源为基于GPU加速的高分辨率三轴模拟（high-resolution tri-axial simulations），使用gPLUTO代码进行天体物理喷流（astrophysical jets）和巨射电星系喷流（giant radio galaxy jets, GRGs）的数值模拟。

Method: 采用GPU加速的三维高分辨率流体动力学模拟代码gPLUTO进行天体物理喷流动力学和辐射特征的高保真数值模拟。

Result: GPU加速的PLUTO代码（gPLUTO）相比仅使用CPU的配置实现了10倍至约30倍以上的运行速度提升。
...
### [12] [Uncovering the absorbed atomic Universe with the [OI]63um line](https://arxiv.org/abs/2602.22161)
...
**TL;DR**: 首次在4.2<z<5.8的12个SPT选DSFG（dusty star-forming galaxies）样本中系统探测到强吸收[OI]63μm精细结构线，发现其源于致密高光学厚度发射区与亚热激发氧分子云的混合，且高气体尘埃柱密度会削弱本征强发射，限制该谱线示踪高红移DSFG中大质量恒星形成区的能力。


<details>
  <summary>Details</summary>

Background: 首次系统性地研究红移（redshift）z>4的[OI]63微米精细结构线在尘埃遮蔽星系（DSFGs）中的吸收与发射特征。

Data: 数据来源于ALMA（Atacama Large Millimeter/submillimeter Array）Bands 9和10对12个4.2<z<5.8的SPT（South Pole Telescope）巡天选DSFGs（dusty star-forming galaxies）样本进行的空间和光谱分辨观测。

Method: 采用ALMA（阿塔卡马大型毫米波/亚毫米波阵列）Bands 9和10进行空间和光谱分辨观测，探测亚千秒差距尺度的星际介质（ISM）。

Result: 在4.2<z<5.8的12个SPT巡天选DSFG样本中首次系统探测到显著吸收的[OI]63微米（[OI]63um）精细结构线，并发现其源于致密高光学厚度发射区与亚热激发富氧分子云的混合。
...
**TL;DR**: 通过高分辨率观测和流体动力学模拟发现，室女座（Virgo）星系团外围的矮星系并合遗迹VCC 693源于两个恒星质量比3:1-4:1的矮星系近等质量并合（其中一个是贫气体前身星系），其光学形态呈现复杂潮汐结构但中性氢（HI）气体形成规则旋转盘，中心恒星形成和气体金属丰度适度增强，暗物质晕高度集中，表明矮星系并合可在星系团外围环境中产生与普通矮星系难以区分的遗迹结构。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系团外围环境中矮星系的相互作用与并合（merger）过程对星系演化的影响，特别是作为星系团吸积（cluster infall）前的预演化（pre-processing）机制。

Data: 数据来源于VLA和FAST的HI发射线观测、补充的光学成像与光谱数据，以及流体动力学模拟（hydrodynamical simulations）。

Method: 采用甚大天线阵(VLA)和五百米口径球面射电望远镜(FAST)的氢原子(HI)发射线观测数据，结合流体动力学模拟(hydrodynamical simulations)进行联合分析。

Result: VCC 693是由两个矮星系（恒星质量比3:1-4:1）近等并合形成的残骸，其中一个可能为贫气体前身星系（即阻尼并合），其光学形态呈现复杂潮汐结构而中性氢（HI）气体已沉降为规则旋转盘，中心恒星形成和气体金属丰度适度增强，整体恒星结构与普通矮星系难以区分，支持星系团外围环境促进阻尼或混合并合作为星系预处理的组成部分。
...
**TL;DR**: 该研究通过模拟ASKAP POSSUM巡天频段（800-1088 MHz和1296-1440 MHz）下五种偏振源模型（法拉第简单源、伯恩板、内部湍流、外部湍流及双成分模型），揭示了QU拟合（QU-fitting）在模型选择中的贝叶斯因子（Bayes factor）优于传统指标（BIC、AIC和卡方），并建立了参数空间区分边界的经验关系，评估了其在法拉第深度空间中对点源和延展源的参数恢复精度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用QU拟合（QU-fitting）方法解析射电连续谱偏振观测数据以估算磁场特性，并探讨其在ASKAP POSSUM巡天频段（800-1088 MHz和1296-1440 MHz）及其他射电偏振观测中的适用性与局限性。

Data: 研究中使用的数据来源于在ASKAP POSSUM调查频段（800-1088 MHz和1296-1440 MHz）内模拟的不同物理偏振源（法拉第简单源、Burn板、内部湍流、外部湍流和双组分模型）。

Method: 采用QU拟合（QU-fitting）方法结合贝叶斯因子（Bayes factor）进行模型选择，评估了法拉第简单（Faraday simple）、伯恩板（Burn slab）、内部湍流（internal turbulence）、外部湍流（external turbulence）及双组分（two-component）等极化源模型在POSSUM频段（800-1088 MHz和1296-1440 MHz）的观测退化和拟合精度。

Result: QU-fitting在ASKAP POSSUM频段（800-1088 MHz和1296-1440 MHz）中能有效估计法拉第旋转介质的物理参数，但模型可靠性高度依赖参数空间区域，且贝叶斯因子（Bayes factor）比传统指标（如BIC、AIC和卡方）更优用于模型选择。
...
### [15] [Formation timescales for stellar bars in diverse galactic discs](https://arxiv.org/abs/2602.21748)
*Matthew Frosst,Danail Obreschkow,Aaron Ludlow*

**TL;DR**: 通过145个盘星系模拟研究，发现恒星棒（stellar bar）形成时间尺度（τ_bar）与盘结构稳定性参数（ε_ELN、t_OP、f_disc）呈幂律关系，并建立了适用于活晕（live halo）模型的棒形成经验公式，指出静态晕（static halo）中棒形成速率减半且需要更高盘不稳定性。


<details>
  <summary>Details</summary>

Background: 研究恒星棒（stellar bar）在厚盘（thick disc）和高湍流（turbulent）盘星系中的形成机制及其与盘结构、运动学参数的关系。

Data: 研究中使用的数据来源于145个嵌入活动态和静态暗物质晕（live and static dark matter haloes）的盘星系（disc galaxies）模拟。

Method: 采用数值模拟（N-body simulations）结合指数棒增长时标（τ_bar）量化盘结构（disc structure）和运动学（kinematics）对棒形成（bar formation）的调控作用，并引入经验关系描述速度弥散（velocity dispersion）对棒增长的延迟效应。

Result: 在银河系类似盘状星系中，若盘内恒星质量分数（$f_{\rm disc}$）≥0.18、奥斯特里克-皮布尔斯比（$t_{\rm OP}$）≥0.27或埃夫斯塔西奥-莱克-尼格罗蓬特参数（$ε_{\rm ELN}$）≤1.44，棒状结构会在哈勃时间内形成。
...
### [16] [CALIMA: On-the-fly dust and PAH evolution for radiation-hydrodynamics galaxy formation simulations](https://arxiv.org/abs/2602.21790)
*Francisco Rodríguez Montero,Yohan Dubois,Harley Katz,Adrianne Slyz,Julien Devriendt*

**TL;DR**: 开发了CALIMA模块，在RAMSES辐射流体动力学模拟中自洽耦合尘埃（dust）和多环芳烃（PAH）的形成演化与辐射传输及非平衡热化学过程，揭示了尘埃和PAH演化对星际介质（ISM）热相图、碳化学及星系红外辐射、紫外透射率和H$_2$形成的非平庸影响。


<details>
  <summary>Details</summary>

Background: 星际介质(ISM)中尘埃颗粒和多环芳烃(PAHs)对热力学、化学和辐射状态的贡献是当前星系模拟的重要科学主题。

Data: 数据来源于辐射流体动力学模拟（radiation-hydrodynamics simulations）中自洽耦合的尘埃物理与辐射传输及非平衡热化学过程，采用双尺寸双成分尘埃框架（two-size, two-composition dust framework）及独立演化的多环芳烃（PAHs）组分，并整合了恒星尘埃注入、湍流信息的气相吸积、破碎、凝聚、热与非热溅射及激波破坏等物理过程。

Method: 采用双尺寸双成分尘埃框架（two-size, two-composition dust framework）结合对数正态分布的颗粒群（log-normal grain populations），显式演化恒星尘埃注入、湍流驱动的气相吸积（turbulence-informed gas-phase accretion）、碎裂（shattering）、凝聚（coagulation）、热与非热溅射（thermal and non-thermal sputtering）及激波破坏（shock destruction），并将多环芳烃（PAHs）作为独立组分处理。

Result: 开发了CALIMA模块，在辐射流体动力学模拟中自洽耦合尘埃（dust）和PAH（polycyclic aromatic hydrocarbons）的形成与演化过程，改进了高温下尘埃溅射（sputtering）速率和热冷却机制，揭示了尘埃和PAH演化对星际介质（ISM）热相图、碳化学及星系红外辐射、UV透明度、H$_2$形成的非线性影响。
...
### [17] [The supersonic nature of jellyfish galaxies](https://arxiv.org/abs/2602.21821)
...
**TL;DR**: 研究发现磁流体动力学激波（magnetohydrodynamic shock）压缩和超音速运动（supersonic motion）是触发星系团环境中“水母星系”（jellyfish galaxy）剥离尾部恒星形成的必要条件。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系团环境中气体丰富星系因冲压剥离（ram-pressure stripping）形成“水母星系”（jellyfish galaxies）的物理条件，重点关注磁场的差异对星盘外恒星形成（extraplanar star formation）的影响。

Data: data：研究使用了MeerKAT射电连续谱观测数据和GASP项目的17个水母星系样本的轨道动力学模型及真实星系团温度剖面数据。

Method: 采用射电连续谱偏振观测（MeerKAT）和动力学轨道模型结合真实星系团温度剖面推断马赫数（Mach number），探究磁力场拖曳（magnetic field draping）、气体激波压缩与盘外恒星形成活动的关联。

Result: 磁场的存在与星系尾部的超平面（extraplanar）恒星形成活动差异相关，且超音速运动是触发水母星系（jellyfish galaxy）剥离尾部恒星形成的必要条件。
...
### [18] [The Dawes Review: A Decade of Ultra-Diffuse Galaxies](https://arxiv.org/abs/2602.21875)
*Jonah S. Gannon,Anna Ferré-Mateu,Duncan A. Forbes*

**TL;DR**: 论文总结了超弥散星系（UDGs）发现十年来的研究进展，重点分析了静止（quiescent）UDGs的基本特性、形成机制、内部性质及球状星团系统，并指出其异常特征（如偏离标准恒星质量-晕质量关系、高α元素丰度、富球状星团）挑战了传统星系形成理论。


<details>
  <summary>Details</summary>

Background: 超弥散星系（UDGs）的发现挑战了传统矮星系认知，引发对其特殊性与形成机制的持续争议。

Data: data部分：研究使用了世界级天文台（world-class observatories）对本地宇宙（local Universe）全范围环境中大量超弥散星系（UDGs）的观测数据以及专门模拟（dedicated simulations）提出的多种形成路径数据。

Method: 采用数值模拟（numerical simulations）和观测数据分析（observational data analysis）方法研究超弥散星系（UDGs）的形成机制与物理特性。

Result: 超弥散星系(UDGs)的观测证据表明其存在多种异常特性，包括偏离标准恒星质量-晕质量关系、异常高的α元素丰度以及远超同恒星质量矮星系的球状星团数量。
...
### [19] [Optical Spectroscopy of Dwarf Galaxies at $z\sim 0.15$ in the COSMOS Field: Star Formation and Dust Properties](https://arxiv.org/abs/2602.22035)
*Zahra Sattari,Daniel D. Kelson,Bahram Mobasher,Nima Chartab,Vihang Mehta,Harry I. Teplitz,Shannon G. Patel*

**TL;DR**: 研究发现低质量星系（LMGs）与中等质量星系（IMGs）虽遵循相同的恒星形成率-质量（SFR-M_*）标度关系，但前者的尘埃含量更低且星际介质（ISM）受反馈维持，导致其消光行为与后者存在显著差异。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低质量星系（LMGs）和中等质量星系（IMGs）的恒星形成率（SFRs）、尘埃衰减特性以及星云与恒星红化关系的比较，旨在揭示不同质量星系间星际介质（ISM）和尘埃分布的差异。

Data: 数据来源为COSMOS场（Cosmic Evolution Survey field）中红移z~0.15的低质量星系（LMGs）和红移z~0.35的中等质量星系（IMGs）的光谱观测数据。

Method: 采用光谱分析方法比较低质量星系(LMGs)和中等质量星系(IMGs)的恒星形成率(SFRs)、尘埃衰减特性以及星云与恒星红化关系，并拟合联合SFR-质量关系。

Result: 低质量星系(LMGs)的星族合成历史(star formation histories)可用无标度自相似形式良好描述，其恒星形成率(SFR)与恒星质量(M*)的联合拟合斜率为1.01±0.03。
...
### [20] [Overlap Zoo Beta: A Catalogue of ~800 Occulting Pairs in the DESI Legacy Survey using Citizen Science](https://arxiv.org/abs/2602.22079)
...
**TL;DR**: 利用公众科学项目从Galaxy Zoo DECaLS和DESI Legacy Survey DR10中筛选重叠星系对（overlapping galaxies），显著扩充了现有样本量，为详细研究尘埃衰减（dust attenuation）和机器学习自动识别提供了更丰富的数据基础。


<details>
  <summary>Details</summary>

Background: 研究主题为利用重叠星系（overlapping galaxies）精确测量尘埃衰减（dust attenuation），并通过扩展重叠星系对样本提升尘埃研究的统计显著性和机器学习自动化识别能力。

Data: 数据来源于Galaxy Zoo DECaLS（GZD-1、-2和-5）中分类为“重叠”的星系以及DESI Legacy Survey数据发布10（DR10）的图像。

Method: 利用来自Galaxy Zoo DECaLS（GZD-1、-2和-5）分类为“重叠”的星系及DESI Legacy Survey数据发布10（DR10）的图像，通过公民科学项目直接由志愿者对这些重叠星系对进行分类。

Result: 重叠星系（overlapping galaxies）为通过建模前景与背景星系的光度并推断重叠区域缺失光强，提供了精确测量尘埃衰减（dust attenuation）的独特机会。
...
### [21] [Stellar associations powering HII regions $\unicode{x2013}$ II. Escape fraction of ionizing photons](https://arxiv.org/abs/2602.22151)
...
**TL;DR**: 研究发现19个邻近星系中电离辐射（ionizing radiation）从电离氢区（HII regions）向周围弥散电离气体（DIG）的逃逸比例高达82%，且部分区域存在电离光子预算（ionizing photon budget）由电离氢区外恒星贡献的现象。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星反馈（stellar feedback）过程中电离辐射从电离氢区（HII region）向周围弥散电离气体（DIG）泄漏的机制及其观测约束。

Data: 数据来源于PHANGS$\unicode{x2013}$MUSE和PHANGS$\unicode{x2013}$HST对19个邻近星系中HII区（HII region）及其电离恒星源的匹配观测。

Method: 采用HII区域与电离恒星源的匹配方法，结合PHANGS-MUSE和PHANGS-HST数据测量电离光子逃逸比例，并通过星系整体电离预算分析验证结果。

Result: 在19个邻近星系样本中，测得电离光子逃逸分数（photon escape fraction）为82^{+12}_{-24}%，且部分电离辐射由HII区域（HII region）外的恒星贡献。
...
### [22] [Recovering 3D Magnetic Turbulence from Single-Frequency Faraday Screens](https://arxiv.org/abs/2602.22204)
*Aliaksandr Melnichenka,Alex Lazarian,Dmitri Pogosyan*

**TL;DR**: 通过分析单频偏振方向谱（SPD）和法拉第旋转屏（Faraday screen）的湍流特性，研究发现SPD对法拉第屏湍流更敏感，并提供了区分发射区和法拉第屏湍流的观测标准，为单波段射电偏振测量提取湍流统计信息提供了实用方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用偏振同步辐射（polarized synchrotron radiation）统计特性解析发射区域与法拉第旋转屏（Faraday-rotation screen）中的湍流性质。

Data: 研究中使用的数据来源包括磁流体动力学（MHD）模拟和合成湍流谱（synthetic turbulence spectra）。

Method: 采用磁流体动力学（MHD）模拟和合成湍流谱（synthetic turbulence spectra）分析偏振方向谱（SPD）以诊断发射区和法拉第旋转屏（Faraday-rotation screen）的湍流统计特性。

Result: 偏振方向谱（SPD）对法拉第屏（Faraday screen）中的湍流表现出更高的敏感性。
...
### [23] [SIRIUS: The relation between the diversity of dwarf galaxies and their formation histories](https://arxiv.org/abs/2602.22206)
*Chi-Hong Lin,Michiko S Fujii,Takayuki R Saitoh,Yutaka Hirai*

**TL;DR**: 通过高分辨率宇宙学放大模拟发现，低质量矮星系（暗物质晕质量约10^9 M⊙）在再电离时期的晕质量存在一个数量级离散，其z=0恒星-晕质量比离散近两个数量级（5×10^-5至2×10^-3），且部分星系形成超致密矮星系（UCD）尺度（半质量半径~30 pc）的致密恒星分布，关键形成条件是中心气体面密度超过临界阈值（Σ_gas≳30 M⊙ pc^-2）。


<details>
  <summary>Details</summary>

Background: 研究探讨了低质量矮星系（low-mass dwarf galaxies）的结构多样性及其恒星形成效率与暗物质晕（dark matter halo）质量的关系，特别是关注恒星-晕质量比（stellar-to-halo mass ratio）的散射和超致密矮星系（ultra-compact dwarfs, UCDs）的形成条件。

Data: 使用了高分辨率宇宙学放大模拟（cosmological zoom-in simulations）对八个暗物质晕质量约为$10^9\rm\ M_{\odot}$的星系进行了模拟，模拟时间持续到1.2 Gyr时大部分气体被排出。

Method: 采用高分辨率宇宙学放大模拟(cosmological zoom-in simulations)对八个暗物质晕质量为$\sim 10^9\rm\ M_{\odot}$的星系进行建模，追踪至1.2 Gyr时大部分气体被驱逐的状态。

Result: 低质量矮星系（low-mass dwarf galaxies）的结构多样性主要由中心气体表面密度（central gas surface density）超过临界阈值（$Σ_{\rm gas} \gtrsim 30\rm\ M_\odot \rm\ {pc}^{-2}$）时的高效恒星形成驱动，导致致密恒星系统形成。
...
### [24] [CNN+FoF: application of deep learning to the identification of dark matter haloes](https://arxiv.org/abs/2602.21246)
*Soumadeep Maiti,Carlos M. Correa,Andrea Fiorilli,Andrés N. Ruiz,Dante J. Paz,Alejandro Pérez Fernández,Ariel G. Sánchez*

**TL;DR**: 提出了一种基于深度学习的方法，利用体积卷积神经网络（volumetric Convolutional Neural Network）和优化的Friends-of-Friends聚类算法，在宇宙学N体模拟中高效识别暗物质晕（dark matter haloes），相比传统方法（如ROCKSTAR）速度提升约一个数量级，同时保持高精度（分类准确率超98%，纯度超95%，完整性达93%），并能准确重建暗物质晕的质量、位置、速度及密度分布。


<details>
  <summary>Details</summary>

Background: 研究主题为利用深度学习技术改进宇宙学N体模拟中暗物质晕（dark matter haloes）的识别方法。

Data: 数据来源于使用GADGET-4生成的宇宙学N体模拟（cosmological N-body simulations），训练标签通过ROCKSTAR晕搜寻算法（ROCKSTAR halo finder）获取。

Method: 采用深度学习框架结合体积卷积神经网络（volumetric Convolutional Neural Network）进行暗物质晕（dark matter haloes）粒子分类，并优化并行化朋友间聚类算法（Friends-of-Friends clustering algorithm）构建完整晕目录。

Result: 基于深度学习的暗物质晕（dark matter haloes）识别方法在宇宙学N体模拟中实现了超过98%的粒子分类准确率，且优化的Friends-of-Friends算法生成的晕目录纯度普遍高于95%，同时质量函数与参考值偏差在5%以内。
...
### [25] [Evolution of Cosmic Voids: Structure, Galaxies, and Dynamics](https://arxiv.org/abs/2602.21292)
*Saeed Tavasoli*

**TL;DR**: 研究揭示了宇宙空洞（cosmic voids）从红移$z=2.09$至今的结构、光度及动力学演化特征，发现空洞随红移降低显著变空且内部星系亮度下降、成团性减弱，大空洞星系比小空洞更亮且演化更快，密度轮廓呈现普适标度形态，观测数据中发现的中心星系群与$Λ$CDM模拟预测存在差异。


<details>
  <summary>Details</summary>

Background: 研究宇宙空洞（cosmic voids）及其星系群从红移$z=2.09$至今的结构、光度及动力学演化，重点关注空洞尺寸作为关键演化参数。

Data: 数据来源包括四组Millennium Simulation（千禧年模拟）的快照数据和斯隆数字巡天（SDSS）在红移$z<0.04$的观测数据。

Method: 采用基于Millennium Simulation（模拟）和SDSS（斯隆数字化巡天）数据的统一分析方法，结合空洞（void）半径标度的堆叠密度剖面（stacked density profiles）和动力学（dynamical）流场分析。

Result: 宇宙空洞（cosmic voids）从红移$z=2.09$至今呈现出显著变空的演化趋势，其内部星系（galaxy populations）的亮度和成团性明显下降，且大空洞比小空洞拥有更亮、演化更快的星系。
...
### [26] [Implications for PBH Dark Matter from a single Sub-Solar$\unicode{x2013}$GW Detection in LVK O1$\unicode{x2013}$O4](https://arxiv.org/abs/2602.21295)
*Alberto Magaraggia,Nico Cappelluti*

**TL;DR**: 发现亚太阳质量（sub-solar mass）黑洞候选体S251112cm支持原初黑洞（Primordial Black Holes）模型，其合并率与量子色动力学（Quantum Chromodynamics）时期形成的宽质量函数原初黑洞种群预测一致，并给出原初黑洞丰度下限f_{PBH}>0.04。


<details>
  <summary>Details</summary>

Background: 探测亚太阳质量(sub-solar mass)黑洞是现代天体物理学的重要里程碑，可能揭示新恒星物理或暗物质(Dark Matter)作为原初黑洞(Primordial Black Holes)的本质。

Data: 数据来源于LIGO-Virgo-KAGRA (LVK)合作组在2025年11月12日报告的致密双星并合候选体S251112cm的观测数据，该事件没有明显电磁对应体，符合啁啾质量(chirp mass)在0.1-0.87太阳质量范围内的双黑洞并合特征。

Method: 采用量子色动力学(QCD)时期形成的宽质量函数原初黑洞(PBH)模型进行统计分析，预测与LIGO-Virgo-KAGRA观测的事件率相符。

Result: 探测到亚太阳质量(sub-solar mass)黑洞将为研究新恒星物理或揭示暗物质作为原初黑洞(Primordial Black Holes)的本质提供重要线索。
...
### [27] [Intracluster Medium Fluctuations on Scales up to 1 Mpc: A Combined eROSITA and SPT/Planck Analysis of Abell 3266](https://arxiv.org/abs/2602.21443)
...
**TL;DR**: 通过分析A3266星系团（Abell 3266）的SRG-eROSITA和Planck/SPT观测数据，发现其北部区域密度涨落强于南部且随半径增大，证实了与宇宙学模拟一致的丝状结构吸积过程，并测得压力与密度涨落幅度比为1.00±0.55、非热压力支撑占比0.068±0.050。


<details>
  <summary>Details</summary>

Background: 研究星系团（galaxy clusters）通过等级式并合（hierarchical assembly）形成过程中，由并合、活动星系核（AGN）反馈、纤维状吸积（filamentary accretion）等能量注入过程在星系团内介质（ICM）中产生的湍流和扰动。

Data: 研究使用了SRG-eROSITA和Planck/SPT对Abell 3266（A3266）的深度观测数据。

Method: 采用X射线（SRG-eROSITA）和SZ效应（Planck/SPT）观测数据，测量了A3266星系团内ICM（intra-cluster medium）的密度和压力涨落，并计算了这些扰动的功率谱和三维振幅。

Result: 通过分析A3266的SRG-eROSITA和Planck/SPT观测数据，测得压力与密度涨落幅度比值为1.00±0.55，非热压力支撑占比0.068±0.050，且密度涨落在北部区域更强，与eROSITA揭示的丝状结构吸积过程一致。
...
### [28] [Resurgence and Hyperasymptotics in Wave Optics Astronomy](https://arxiv.org/abs/2602.21493)
*Job Feldbrugge,Samuel Crew,Ue-Li Pen*

**TL;DR**: 研究了随机引力透镜和等离子体透镜（random gravitational and plasma lenses）的波动光学行为，通过构建包含高阶项的折射和衍射展开式（refractive and diffractive expansions）及跨级数（transseries），为近焦散线（caustics）区域和非几何光学近似下的波动光学透镜效应建模提供了系统框架，并揭示了离轴焦散线（off-axis caustics）的强波动光学抑制现象。


<details>
  <summary>Details</summary>

Background: 研究聚焦于引力波和快速射电暴（fast radio bursts）发现背景下波光学（wave optics）在天体物理学中的重要性，特别是随机引力透镜和等离子体透镜的行为及其在焦散（caustics）附近和非几何光学近似下的建模框架。

Data: 研究中使用了基于皮卡-莱夫谢茨(Picard-Lefschetz)技术的理论模拟数据，并通过波雷尔(Borel)和超渐近(hyperasymptotic)重求和系统近似透镜积分。

Method: 采用折射展开（refractive expansion）和衍射展开（diffractive expansion）结合皮卡-莱夫谢茨（Picard-Lefschetz）技术及渐进分析（asymptotic analysis），构建了跨频率干涉模式的统一框架，并通过波雷尔（Borel）与超渐进（hyperasymptotic）重整化处理奇点附近的光学透镜积分。

Result: 基于皮卡-莱夫谢茨(Picard-Lefschetz)技术推导出任意阶的全折射展开式，并将其组织为跨级数(transseries)，在焦散(caustic)附近通过均匀渐近展开补充标准跨级数。
...
### [29] [Bulk and turbulent gas motions in the interacting galaxy cluster Abell 3395 South observed with XRISM](https://arxiv.org/abs/2602.21580)
...
**TL;DR**: 通过XRISM/Resolve高分辨率X射线光谱分析Abell 3395南次团(A3395S)核心区铁XXV Heα发射线，首次直接测量到团内介质(ICM)存在显著视线方向整体速度(263±23 km/s)但湍流速度较低(124±21 km/s)，表明该星系团尚未达到动力学平衡状态且湍流粒子再加速效率低下。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高分辨率X射线光谱（XRISM/Resolve）探测Abell~3395南子星系团（A3395S）核心区气体动力学特征，揭示星系团合并过程中团内介质（intracluster medium）运动状态与动态弛豫程度的关系。

Data: 使用XRISM/Resolve进行高分辨率X射线光谱观测，直接测量了Abell 3395南子团核心区铁XXV Heα发射线的视线方向体速度和湍流速度。

Method: 采用高分辨率X射线光谱（XRISM/Resolve）分析铁XXV Heα发射线，直接测量了星系团内介质（ICM）的视线方向整体速度和湍流速度。

Result: 通过分析Fe~XXV Heα发射线，直接测量到阿贝尔3395南子团（A3395S）核心区星系际介质（ICM）的视线方向一维湍流速度为124±21 km/s，而视线方向整体运动速度为263±23 km/s。
...
### [30] [A sound horizon independent measurement of $H_0$ from BOSS, DESI and DES Y3](https://arxiv.org/abs/2602.21733)
*Zhiyu Lu,Théo Simon,Vivian Poulin,Yifu Cai*

**TL;DR**: 通过多探针大尺度结构分析（首次结合BOSS全形状功率谱和双谱、DESI DR9角功率谱、DES Y3 3×2点分析和Planck PR3 CMB引力透镜功率谱），实现了不依赖声学视界（sound horizon）的哈勃参数（h=0.702±0.023）和宇宙学参数（Ω_m=0.310±0.013，σ_8=0.799±0.020）的3-4%精度测量，并发现双谱小尺度导致BAO尺度参数与ΛCDM值存在1.8σ偏离。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过多探针大尺度结构分析实现不依赖声波视界（sound horizon）的哈勃参数测量，并探讨其与暗能量（dark energy）演化的潜在关联。

Data: 数据来源于BOSS全形状功率谱和双谱、DESI Legacy Imaging Survey DR9的自和交叉角功率谱、DES Y3的3×2点分析以及Planck PR3的CMB引力透镜功率谱。

Method: 采用有效场论（effective field theory of large-scale structure）分析BOSS全形状功率谱（full-shape power spectrum）和双谱（bispectrum），并结合DESI Legacy成像巡天DR9的自/交叉角功率谱（auto- and cross-angular power spectra）、DES Y3的$3 \times 2$点分析（$3 \times 2$pt analysis）以及Planck PR3的CMB引力透镜功率谱（CMB gravitational lensing power spectrum）。

Result: 通过多探针大尺度结构分析，采用不依赖声学视界（sound horizon）的重新标定方法，首次结合BOSS全形状功率谱和双谱（bispectrum）的一圈有效场论分析，测得哈勃参数h=0.702^{+0.022}_{-0.024}、物质密度参数Ω_m=0.310±0.013和涨落幅度σ_8=0.799±0.020，精度达3-4%。
...
### [31] [Linear Perturbations and Multi-Probe Diagnostics in Dark-Sector Selective $f(R,T_χ)$ Gravity](https://arxiv.org/abs/2602.21774)
*L. Yildiz,D. Kayki,E. Gudekli*

**TL;DR**: 提出了一种仅依赖暗物质(χ)能量-动量张量迹(Tχ)的选择性暗区耦合引力扩展模型，建立了具有微观物理基础的迹耦合机制，消除了f(R,T)类模型的物质拉格朗日量模糊性，并推导出该模型在亚哈勃尺度下产生与广义相对论相关的尺度及时间依赖的结构增长和光偏折修正。


<details>
  <summary>Details</summary>

Background: 研究探索了暗物质（dark matter）与引力耦合的选择性追踪扩展理论，聚焦于暗物质能量-动量张量（energy-momentum tensor）的迹（trace）对引力修正的微观物理机制及其在宇宙结构形成中的观测效应。

Data: 研究中使用的数据来源于理论推导和数值模拟（simulation），包括线性标量扰动系统、有效修正引力函数以及基于红移空间畸变、弱引力透镜和CMB透镜的多探针框架。

Method: 采用暗物质场χ的能量-动量张量迹Tχ选择性耦合的引力扩展模型，推导场方程、暗物质交换结构及线性标量扰动系统，构建基于红移空间畸变、弱透镜和CMB透镜的多探针扰动分析框架。

Result: 该模型通过暗物质场χ的能量-动量张量迹Tχ选择性耦合曲率，构建了微观物理明确的迹耦合引力扩展，消除了f(R,T)类模型中常见的物质拉氏量模糊性。
...
### [32] [Impact and interplay of $Λ$CDM analysis choices for LSST cosmic shear](https://arxiv.org/abs/2602.21863)
...
**TL;DR**: 研究通过分析鲁宾天文台LSST（Legacy Survey of Space and Time）宇宙剪切（cosmic shear）数据，发现重子反馈（baryon feedback）和光测红移（photometric redshifts）校准不确定性会显著削弱对宇宙学参数S₈（S₈=σ₈√(Ωₘ/0.3)）的约束精度，强调需改进重子反馈模型和深场红移校准方法以提升LSST最终约束能力至现有宇宙剪切巡天的五倍以上。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于鲁宾天文台LSST巡天项目的宇宙剪切（cosmic shear）分析对宇宙学参数（cosmological parameters）的约束预测，重点探讨重子反馈（baryon feedback）和红移校准（photometric redshift calibration）等系统效应对ΛCDM模型恢复精度的影响。

Data: 数据来源为鲁宾天文台时空遗产巡天（LSST）的宇宙剪切观测数据，并利用星系交叉相关技术校准的光度红移数据。

Method: 采用宇宙剪切（cosmic shear）分析框架，结合重子反馈（baryon feedback）对非线性物质功率谱（non-linear matter power spectrum）的抑制效应，并利用星系互相关技术（galaxy cross-correlation techniques）校准光测红移（photometric redshifts）。

Result: LSST宇宙剪切（cosmic shear）分析中，当考虑重子反馈（baryon feedback）对非线性物质功率谱抑制的不确定性时，复合参数S₈的误差比忽略该天体物理现象的分析结果增加近一倍。
...
### [33] [The impact of cosmic filaments on starburst galaxies across cosmic times](https://arxiv.org/abs/2602.21890)
...
**TL;DR**: 研究发现星系形成活动与宇宙网纤维结构（cosmic web filaments）的距离存在依赖关系，其中星暴星系（starburst galaxies）在z>1时更靠近纤维而在z<1时远离，主序星系（MS galaxies）处于中间环境且演化微弱，静止星系（quenched galaxies）向低红移逐渐靠近纤维，且星暴与主序星系在z~1时出现距离交叉。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙网（cosmic web）中星系位置对其恒星形成活动的依赖性及其随红移（redshift）的演化规律。

Data: 观测数据来源于COSMOS2020和COSMOS-Web（COSMOS-Web）巡天项目，通过精确测光红移（photometric redshifts）重建了z=2至z=0.5的投影宇宙网（cosmic web），并利用远红外光谱能量分布拟合（far-infrared spectral energy distribution fitting）识别星暴星系（starburst galaxies）。

Method: 采用宇宙学模拟（SIMBA）分析不同星系群到宇宙网丝状结构（filament）的平均距离随红移的演化，并结合COSMOS2020和COSMOS-Web数据通过测光红移重建投影宇宙网及远红外光谱能量分布拟合识别星暴星系。

Result: 星暴星系在z>1时更靠近宇宙网纤维结构（filament），在z<1时距离更远；主序星系（MS）占据中间环境且演化微弱；而淬灭星系（quenched）向低红移逐渐靠近纤维结构，并在z~1附近与星暴星系距离曲线交叉。
...
### [34] [How does a MOND cosmology fare on Gpc scales? - Collisionless $N$-body simulations of $ν$HDM](https://arxiv.org/abs/2602.21975)
*Alfie Russell,Indranil Banik,Oscar Cray,Hongsheng Zhao*

**TL;DR**: 该研究通过迄今最大规模的修正牛顿动力学（MOND）宇宙学无碰撞N体模拟，发现νHDM模型虽能匹配宇宙微波背景（CMB）功率谱和星系旋转曲线，但严重高估了大尺度结构形成（最巨星系团达5×10¹⁷M⊙/h）并产生过高本动速度（数千km/s），其局域空洞解无法缓解哈勃张力（减速参数<-1.5），且与观测的宇宙流（Cosmicflows-4）矛盾，以>5σ置信度排除了νHDM模型及热暗物质（HDM）替代冷暗物质（CDM）的可行性。


<details>
  <summary>Details</summary>

Background: 研究探讨了修正牛顿动力学（MOND）宇宙学模型在大尺度结构形成中的表现及其对哈勃张力（Hubble tension）的潜在解释能力。

Data: 研究使用了4组基于修改牛顿动力学（MOND）宇宙学的大规模无碰撞N体数值模拟（simulation）数据，包括ΛCDM基准模型、νHDM（MOND加热暗物质）模型以及两个非物理模型ΛHDM和νCDM，模拟采用256³粒子在800/h共动Mpc的盒子里进行。

Method: 采用无碰撞N体宇宙学模拟方法，在修正牛顿动力学（MOND）框架下对比ΛCDM、νHDM、ΛHDM和νCDM四种模型。

Result: MOND（Modified Newtonian Dynamics）模型在宇宙学尺度上会过度产生大尺度结构，其中νHDM模型在z=0时形成的最巨大星系团质量约为5×10¹⁷ M⊙/h，且典型本动速度达数千公里/秒。
...
### [35] [A Path to an All-Sky Survey with Roman](https://arxiv.org/abs/2602.21280)
...
**TL;DR**: 提出利用南希·格雷斯·罗曼太空望远镜(Roman Space Telescope)开展首个全天空近红外巡天(first-epoch survey)，实现0.1角秒分辨率、H波段25.5星等(5σ)深度探测，并通过首轮观测(Cycle 1)计划显著增强与LSST和盖亚(Gaia)等设施的协同观测能力，为后续全天空巡天奠定基础。


<details>
  <summary>Details</summary>

Background: 研究探讨利用南希·格雷斯·罗曼空间望远镜（Nancy Grace Roman Space Telescope）开展全天空近红外（near-infrared）深度巡天的科学价值与实施路径。

Data: 数据来源于南希·格雷斯·罗曼空间望远镜（Nancy Grace Roman Space Telescope）进行的全天空近红外巡天观测，包括高分辨率（~0.1角秒）成像数据、H波段深度达25.5 AB星等（5σ）的测光数据，以及与LSST和盖亚（Gaia）任务协同产生的强制测光数据、联合天体测量数据和稀有天体（如银河系子结构、强引力透镜等）星表。

Method: 采用空间近红外全天空巡天（all-sky near-infrared survey）技术，结合南希·格雷斯·罗曼太空望远镜（Nancy Grace Roman Space Telescope）实现约0.1角秒（arcsecond）分辨率的成像观测。

Result: 利用南希·格雷斯·罗曼空间望远镜（Nancy Grace Roman Space Telescope）开展的全天近红外深度巡天（all-sky near-infrared survey）将构建未来数十年天文学研究的基础设施，其首期观测（Cycle 1）可实现$\sim0.1''$角分辨率、$\mathrm{H}\sim25.5$ AB星等（5$σ$）的探测极限，并产出LSST强制测光、盖亚-罗曼联合天体测量（joint \textit{Gaia}-Roman astrometry）及银河系子结构等稀有天体目录的高价值数据产品。
...
### [36] [Conditional Image Diffusion with Interferometric Closure Invariants: Independent EHT Imaging of Centaurus~A and 3C~279](https://arxiv.org/abs/2602.21507)
*Samuel Lai,Nithyanandan Thyagarajan,O. Ivy Wong,Foivos Diakogiannis*

**TL;DR**: 采用GenDIReCT（基于条件扩散模型）对半人马座A射电星系和3C~279类星体的EHT观测数据进行独立成像分析，首次在亚秒差距尺度上精确测定了半人马座A喷流鞘层两条发射脊（各约80微角秒）的亮度比（1.4±0.1）与张角（12.3±0.3度），并发现3C~279南部喷流以表观超光速（约10±2倍光速）远离北部组分，验证了闭包不变量在VLBI稀疏阵列中能有效规避校准系统误差并保留稳健形态信息的优势。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用生成式深度学习框架（GenDIReCT）处理事件视界望远镜（EHT）对活动星系核（active galactic nuclei）的干涉观测数据，解决甚长基线干涉测量（VLBI）中极端稀疏阵列导致的病态反演问题，并验证闭包不变量（closure invariants）在保留形态信息与规避校准系统误差方面的有效性。

Data: 数据来源于事件视界望远镜（Event Horizon Telescope, EHT）对射电星系半人马座A（Centaurus~A）和类星体3C~279的活动星系核的观测数据。

Method: 采用基于条件扩散模型的生成深度学习图像重建方法（GenDIReCT），以干涉闭合不变量（closure invariants）为主要观测数据。

Result: 对半人马座A（Centaurus A）的成像分析揭示了沿喷流鞘的两个显著发射脊（$\simeq 80\,μ$as），亮度比为$1.4\pm 0.1$，张角为$12.3\pm 0.3$度；对3C~279的成像识别出三个独立成分，南部喷流抛射物在亚秒差距尺度上表现出$4.6\pm 1.0\,μ$as的自行运动，对应$\simeq 10\pm 2$倍光速的超光速视速度。
...
### [37] [pythonradex: a fast Python re-implementation of RADEX with extended functionality](https://arxiv.org/abs/2602.21837)
*Gianni Cataldi*

**TL;DR**: 开发了RADEX的Python重实现pythonradex，改进了用户界面和性能，新增了连续谱效应、谱线重叠及球对称几何等自洽总流量计算功能。


<details>
  <summary>Details</summary>

Background: 研究主题为利用观测谱线发射估算气体物理参数（温度、质量、密度等）及辐射传输（radiative transfer）计算在射电和红外天文学中的应用。

Data: 研究中使用的数据来源为RADEX（van der Tak et al., 2007）的Fortran代码，并基于此重新实现了pythonradex工具。

Method: 采用非局部热动平衡（non-LTE）辐射传输（radiative transfer）模型，通过Python重写的RADEX代码（pythonradex）计算均匀介质在简化几何结构中的辐射传输，并新增连续谱效应和谱线重叠功能。

Result: 开发了pythonradex工具，改进了RADEX代码的性能和功能，支持连续谱效应和谱线重叠计算，并能自洽计算所有几何结构（包括球对称结构）的总通量。
...
### [38] [Electrical coupling of a horizontal dipole antenna to a dielectric half-space: applications to radio astronomy from the lunar surface](https://arxiv.org/abs/2602.22194)
*Kaja M. Rotermund,Aritoki Suzuki,Stuart D. Bale,Anže Slosar*

**TL;DR**: 论文揭示了月球背面（far side）低高度部署的偶极天线（dipole antenna）与月壤（lunar regolith）的强耦合效应会显著改变阻抗和辐射特性，但适当增加天线高度可快速减弱这种影响。


<details>
  <summary>Details</summary>

Background: 月球背面（far side of the Moon）因其屏蔽地球射频干扰和电离层影响的独特环境，成为开展低频射电天文（radio astronomy）和宇宙学（cosmological）研究的理想场所。

Data: 数据来源为Ansys HFSS积分方程求解器（integral equation solver）的仿真结果（simulation results）。

Method: 采用安捷伦高频结构仿真器（Ansys HFSS）积分方程求解器对月壤（lunar regolith）介电半空间上方水平偶极子天线行为进行数值模拟，量化天线阻抗和波束方向图与天空的耦合效应。

Result: 月球背面（far side of the Moon）的无线电静默环境为低频射电天文观测提供了独特优势，但天线与月壤（lunar regolith）的强耦合会显著改变阻抗、辐射方向图和效率。
...
### [39] [Classical, large scale 3D MHD simulations of interacting pulsar wind nebulae](https://arxiv.org/abs/2602.21795)
*D. M. -A. Meyer,D. F. Torres*

**TL;DR**: 研究发现脉冲星（pulsar）风星云（pulsar wind nebula）的形态和非热射电特性主要受其自转轴与星际介质局部磁场方向夹角的影响，年轻星云可呈现矩形、圆形或不规则长条状结构，但老年星云（50至80 kyr）的物质混合对该参数不敏感。


<details>
  <summary>Details</summary>

Background: 研究探讨了磁化旋转中子星（脉冲星）在星际介质局部磁场方向与自转轴夹角对其风星云（pulsar wind nebula）形态和射电特性的影响。

Data: 使用PLUTO进行三维磁流体力学（MHD）模拟，并基于模拟结果开展辐射传输计算以生成脉冲星风云（pulsar wind nebula）的非热射电辐射图。

Method: 采用三维磁流体动力学（MHD）模拟（使用PLUTO代码）结合辐射转移计算，研究脉冲星风星云（pulsar wind nebula）形态与非热射电辐射特性。

Result: 当脉冲星（pulsar）旋转轴与局部环境磁化方向之间的夹角影响其风星云（pulsar wind nebula）形态时，该夹角是决定静态大质量恒星风星云形态和非热射电特性的关键参数。
...
### [40] [Whistler-Alfvén turbulence in a non-neutral ultrarelativistic pair plasma](https://arxiv.org/abs/2602.22165)
*Stanislav Boldyrev,Mikhail Medvedev*

**TL;DR**: 研究发现非中性（non-neutral）电子-正电子对等离子体（pair plasmas）中，大尺度动力学由混合哨声-阿尔芬（whistler-Alfvén）模式主导，而小尺度下转变为纯阿尔芬模式。


<details>
  <summary>Details</summary>

Background: 研究聚焦于非中性电子-正电子对等离子体（pair plasmas）中混合哨声-阿尔芬模（hybrid whistler-Alfvén modes）与纯阿尔芬模（pure Alfvén modes）在不同尺度下的动态转换机制及其在脉冲星、磁星、黑洞吸积盘与相对论性喷流等极端天体物理环境中的主导作用。

Data: 数据来源于对超相对论性非中性磁主导电子-正电子等离子体（ultrarelativistic non-neutral magnetically dominated pair plasma）的理论推导和非线性方程模拟。

Method: 采用非线性方程描述超相对论性非中性磁主导电子-正电子等离子体(electron-positron plasma)中混合哨声-阿尔芬模(hybrid whistler-Alfvén modes)的动力学行为。

Result: 在非中性对等离子体（non-neutral pair plasmas）中，大尺度动力学由混合哨声-阿尔芬模式（hybrid whistler-Alfvén modes）主导，并在小尺度下转变为纯阿尔芬模式。
...
### [41] [HATPI Pre-Perihelion Time-series Photometry of the Interstellar Comet 3I/ATLAS](https://arxiv.org/abs/2602.21586)
...
**TL;DR**: 利用HATPI（High cadence All-sky Transit and Parallax Instrument）设施对星际彗星3I/ATLAS进行时序测光，发现其亮度随日心距离（heliocentric distance）$r_{H}$呈幂律增长（指数$n = 5.167 \pm 0.095$），且相位函数（phase function）约束为$β= 0.0552 \pm 0.0032$ mag deg$^{-1}$。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用HATPI（高精度时间域光度成像仪）对星际彗星3I/ATLAS进行时间序列测光观测，分析其亮度随日心距离变化的幂律关系及相位函数特征。

Data: 数据来源于智利拉斯坎帕纳斯天文台（Las Campanas Observatory）的HATPI设施，该设施使用64个广角透镜（直径9.6厘米）和背照式CCD，以45秒的观测频率和19.7角秒/像素的空间分辨率获取7,100平方角度的拼接视场数据。

Method: 采用宽视场透镜阵列和背照式CCD（charge-coupled device）进行高时间分辨率（45秒）的光度观测，并通过叠加夜间观测数据提升暗弱目标的探测能力。

Result: HATPI观测发现星际彗星3I/ATLAS的亮度随日心距离（heliocentric distance）$r_{H}$减小而增大的速率（幂律指数$n = 5.167 \pm 0.095$）比文献报道的其他数据（$n = 3.94 \pm 0.10$）更陡，且相位函数（phase function）约束为$β= 0.0552 \pm 0.0032$ mag deg$^{-1}$。
...
### [42] [INFLAVON: CMB as cosmic tracer of Flavor physics](https://arxiv.org/abs/2602.20257)
*Mu-Chun Chen,Anish Ghoshal,V. Knapp-Perez,Xueqi Li,Xiang-Gan Liu,Cameron Moffett-Smith*

**TL;DR**: 将弗罗格特-尼尔森机制(Froggatt-Nielsen mechanism)中的味对称性破缺场(flavon)统一为驱动暴胀的暴胀子(inflavon)，建立了味物理与早期宇宙学的新联系，并通过α吸引子势计算了与普朗克和ACT数据相排斥或可被下一代CMB实验探测的味对称性破缺能标(Λ_FN)和FN荷参数空间。


<details>
  <summary>Details</summary>

Background: 研究将标准模型中解释味层级结构的弗罗格特-尼尔森机制(Froggatt-Nielsen mechanism)与宇宙暴胀(inflation)统一起来，提出驱动暴胀的复标量场(flavon)同时打破弗罗格特-尼尔森$U(1)$对称性并生成标准模型的汤川耦合(Yukawa couplings)。

Data: 数据来源于普朗克（Planck）、ACT（Atacama Cosmology Telescope）及下一代CMB实验（如CMB-S4、SO和LiteBIRD）的观测结果。

Method: 采用弗洛格特-尼尔森机制（Froggatt-Nielsen mechanism）中的复标量场（flavon）作为驱动暴胀的暴胀子（Inflavon），并构建α吸引子势（α-attractor potential）模型。

Result: 将标准模型中解释味层级结构的弗罗格特-尼尔森机制(Froggatt-Nielsen mechanism)与宇宙暴胀统一，提出驱动暴胀的复标量场(flavon)同时生成标准模型汤川耦合，称为暴胀味子(Inflavon)。
...
### [43] [ALP Dark Matter, Cosmological Magnetic Fields and the Direct Collapse Black Hole Formation Scenario](https://arxiv.org/abs/2602.22170)
*Ashu Kushwaha,Robert Brandenberger*

**TL;DR**: 研究发现，假设暗物质为类轴子（ALP）的超轻赝标量粒子，其振荡产生的宇宙磁场与原始暗物质涨落耦合可激发规范场涨落谱，产生足够莱曼-维尔纳（Lyman-Werner）光子通量以实现直接坍缩黑洞（Direct Collapse Black Hole）形成，且该通量与ARCADE2和EDGES测量的射电光子过剩通量上限一致。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗物质作为超轻赝标量粒子（ALP）与电磁学耦合时，其振荡产生的宇宙磁场如何通过耦合原初暗物质涨落影响莱曼-维尔纳光子通量，从而支持直接坍缩黑洞形成机制。

Data: 数据来源于ALP（轴子）场振荡产生的宇宙磁场与原始暗物质涨落的耦合作用，以及ARCADE2和EDGES测量对射电光子过剩通量的限制。

Method: 采用轴子（ALP）与电磁耦合的超轻赝标量暗物质模型，通过ALP场振荡产生的宇宙学磁场与原始暗物质涨落耦合来生成规范场涨落谱。

Result: 暗物质作为类轴子（ALP）的超轻赝标量粒子与宇宙学磁场的耦合，能够产生足够多的莱曼-维尔纳（Lyman-Werner）光子通量以支持直接坍缩黑洞（Direct Collapse Black Hole）的形成。
...
### [44] [A deep HST view of the open cluster NGC2158: binaries, mass functions, and M-dwarf discontinuity](https://arxiv.org/abs/2602.21882)
...
**TL;DR**: 通过哈勃太空望远镜对中等年龄疏散星团NGC 2158的观测，首次发现其双星比例（38%）和质量函数（MF）存在质量依赖性：双星比例从1.0太阳质量的52%降至0.2太阳质量的11%，质量函数斜率在高、低、极低质量区分别呈现-2.49、-1.11和-0.08的差异，并在约0.3太阳质量处检测到与3He不稳定性相关的主序带间断。


<details>
  <summary>Details</summary>

Background: 研究聚焦于理解银河系场星（field stars）和星团中双星系统的性质及其对恒星结构、演化和星团动力学的意义，特别关注低质量恒星的光度双星（photometric binaries）和星团中极低质量恒星的质量函数（mass function）约束问题。

Data: 研究使用了哈勃太空望远镜（Hubble Space Telescope）对中等年龄疏散星团NGC 2158的深度成像数据。

Method: 采用哈勃太空望远镜(Hubble Space Telescope)深度成像技术分析NGC 2158星团的双星系统，结合光度函数和质量函数(MF)拟合方法，量化双星比例随恒星质量的变化规律。

Result: result：NGC 2158星团的双星比例为38%，且呈现明显的质量依赖性（从1.0太阳质量的52%降至0.2太阳质量的11%），其质量函数（MF）在高、低和极低质量区分别呈现α=-2.49±0.19、-1.11±0.09和-0.08±0.07的斜率变化，并在约0.3太阳质量处检测到与3He不稳定性相关的主序带间断。
...
### [45] [MESA Isochrones and Stellar Tracks. II. Models with alpha-enhanced chemical composition](https://arxiv.org/abs/2602.22012)
*A Dotter,E Bauer,MJ Park,C Conroy,A Milone,M Joyce,M Cantiello*

**TL;DR**: 该研究更新并扩展了MESA等时线和恒星轨迹（MIST）数据库，新增了α捕获元素（alpha-capture elements）的丰度变化（[alpha/Fe]=-0.2至+0.6），并自洽地将其从恒星内部模型扩展到观测平面的合成光谱，同时验证了模型并提供了可复现的数据产品。


<details>
  <summary>Details</summary>

Background: 研究聚焦于更新和扩展MESA等龄线及恒星演化轨迹（MIST）数据库，纳入α捕获元素（alpha-capture elements）丰度变化对恒星模型的影响。

Data: 数据来源于MESA Isochrones and Stellar Tracks (MIST)数据库的更新版本，包括恒星演化轨迹、等龄线及测光改正表，可从MIST项目网站（https://mist.science）获取，相关模型复现文件托管于Zenodo平台。

Method: 采用自洽方法更新恒星内部模型和合成光谱，涵盖[alpha/Fe]从-0.2到+0.6的α捕获元素变化，并整合了多项物理模型更新。

Result: 更新后的MIST数据库扩展了α捕获元素（alpha-capture elements）的变体范围，涵盖[alpha/Fe]=-0.2至+0.6的五个数值，适用于[Fe/H]在-3到+0.5之间的恒星模型。
...
### [46] [Dynamical masses of YSOs with the VLBA: DYNAMO VLBA: Trigonometric parallaxes and proper motions of YSOs in Orion](https://arxiv.org/abs/2602.22048)
...
**TL;DR**: 通过多历元VLBA观测猎户座复合体（Orion complex）216个致密射电源（其中58个获得可靠天体测量解），首次以约0.05毫角秒和0.10毫角秒/年精度测定三角视差（2.26-2.65毫角秒）和自行运动，揭示该区域深度结构（380-440秒差距），并给出NGC 2068（405±16秒差距）、NGC 2024（403±5秒差距）、σ Orionis（407±12秒差距）、ONC（388.5±1.7秒差距）及L1641（438±12秒差距）的精确距离，同时验证与盖亚（Gaia）DR3数据在视差（偏移-0.02±0.01毫角秒）和自行（差异约0.07毫角秒/年）方面的高度一致性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用甚长基线干涉阵列（VLBA）对猎户座复合体中的致密射电源进行高精度天体测量，旨在揭示该恒星形成区的三维结构和运动学特征。

Data: 数据来源于多历元甚长基线干涉阵列(VLBA)观测，包括DYNAMO-VLBA和GOBELINS项目的观测数据，共探测到216个致密射电源，其中58个源获得可靠的多历元天体测量解。

Method: 采用甚长基线干涉阵(VLBA)进行多历元观测，结合三角视差(trigonometric parallax)和自行(proper motion)测量技术，实现了对猎户座恒星形成区致密射电源的高精度天体测量。

Result: 在猎户座复合体中测得NGC 2068平均距离为405±16 pc、NGC 2024为403±5 pc、σ Orionis区域为407±12 pc、猎户座星云团(ONC)为388.5±1.7 pc、L1641为438±12 pc，揭示了该区域380-440 pc的深度结构。
...
### [47] [Beam tube boundary effects in stray light modeling of long Fabry-Perot arm cavities for third-generation gravitational-wave detectors](https://arxiv.org/abs/2602.21303)
*M. Andrés-Carcasona,M. Evans*

**TL;DR**: 提出了结合波导管边界条件的光学场模态描述方法，量化了挡板偏心及管壁缺陷对引力波探测器（Cosmic Explorer和Einstein Telescope）的应变等效耦合影响，验证了在密集挡板腔体和小扰动下自由空间FFT工具的适用性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于下一代引力波探测器（如Cosmic Explorer和Einstein Telescope）中真空束管（vacuum beam tubes）内法布里-珀罗（Fabry-Perot）臂腔的光场传播边界条件效应及其对探测器设计的潜在影响。

Data: 研究中使用的数据来源为基于自由空间快速傅里叶变换（FFT）工具和引入波导管（waveguide-like）模式描述的仿真（simulation）数据。

Method: 采用波导模式描述（waveguide-like mode description）光学场并推导镜面和挡板的模态混合矩阵（modal-mixing matrices），包括轴对称圆形孔径的闭式级数解。

Result: 在密集挡板（baffle）腔体和小扰动相关机制下，光束管（beam tube）边界效应处于次要地位，支持继续使用基于快速傅里叶变换（FFT）的代码指导第三代（3G）探测器设计。
...
### [48] [Choice of Quantum Vacuum for Inflation Observables](https://arxiv.org/abs/2602.22116)
*Melo Wood-Saanaoui,Rudnei O. Ramos,Arjun Berera*

**TL;DR**: 在Starobinsky暴胀模型中，采用α-真空（α-vacuum）而非邦奇-戴维斯真空（Bunch-Davies vacuum）会修正原初扰动观测量（如标量谱指数及其跑动），但受普朗克（Planck）数据严格限制，且相关能量尺度可截断在哈勃尺度（~10¹³ GeV）而不违背现有卡文迪什型（Cavendish-type）亚毫米引力实验约束。


<details>
  <summary>Details</summary>

Background: 研究探讨了暴胀（inflation）过程中采用α-真空（α-vacuum）而非标准邦奇-戴维斯真空（Bunch-Davies vacuum）对量子涨落观测量的修正效应。

Data: 研究采用Starobinsky暴胀模型的理论模拟数据，通过计算标量谱指数及其跑动来比较不同初始真空态（α-vacuum与Bunch-Davies vacuum）对暴胀观测量的修正效应。

Method: 采用α-真空（α-vacuum）替代标准邦奇-戴维斯真空（Bunch-Davies vacuum）计算暴胀观测量修正，并在Starobinsky暴胀模型中比较不同初始真空态对标量谱指数及其跑动的贡献。

Result: 采用α-真空（α-vacuum）而非标准邦奇-戴维斯真空（Bunch-Davies vacuum）会导致暴胀观测量的修正，但该修正受到最新普朗克（Planck）数据的严格限制。
...
### [49] [On the Kalb-Ramond field with non-minimal coupling to gravity](https://arxiv.org/abs/2602.21675)
*Anamaria Hell,Ippei Obata*

**TL;DR**: 研究发现，带质量Kalb-Ramond场（massive Kalb-Ramond field）与里奇标量（Ricci scalar）的非最小耦合会引发两个横向赝矢量自由度（transverse pseudo-vector degrees of freedom）的强耦合，而与里奇张量（Ricci tensor）或黎曼张量（Riemann tensor）的耦合还会导致张量模式强耦合，且在均匀各向同性时空下会引发失控不稳定性（runaway instability）。


<details>
  <summary>Details</summary>

Background: 研究探讨了具有一般非最小引力耦合的大质量Kalb-Ramond场（massive Kalb-Ramond field）在平坦时空和均匀各向同性时空中的动力学行为及其不稳定性问题。

Data: 研究中使用的数据来源为理论推导（theory derivation），未涉及实际观测或模拟数据。

Method: 采用非最小耦合（non-minimal coupling）方法分析大质量Kalb-Ramond场与引力（Ricci标量、Ricci张量或Riemann张量）的相互作用，并探讨了共形耦合（disformal coupling）对不稳定性的潜在修正作用。

Result: result：在均匀各向同性时空下，Kalb-Ramond场与里奇张量（Ricci tensor）或黎曼张量（Riemann tensor）的耦合会引发逃逸不稳定性（runaway instability）。
