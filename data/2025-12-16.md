...
**TL;DR**: 首次系统研究了薄盘（thin disks）吸积对偏心双星系统轨道演化的影响，发现薄盘不仅加速双星螺旋靠近，还显著提高轨道偏心率至e≳0.6，且薄盘和低黏度（α viscosities）使双星仅在更窄的低偏心率范围内趋向圆形轨道。


<details>
  <summary>Details</summary>

Background: 环绕双星盘（circumbinary disks）对宇宙中各类双星系统的轨道和电磁特性具有关键影响，研究主题聚焦于薄盘吸积对双星轨道演化的作用机制及其天体物理意义。

Data: 研究使用数值模拟（simulation）方法生成薄盘（thin disks）和厚盘（thick disks）数据，重点分析不同厚度和黏度（α viscosities）条件下双星系统的轨道演化。

Method: 采用高精度流体动力学数值模拟（hydrodynamic numerical simulations）研究薄盘（thin disks）对偏心双星系统（eccentric binary systems）轨道演化的影响，重点分析压力支撑（pressure support）对吸积流内区（inner regions of the accretion flow）的吸积流（accretion streams）和激波（shocks）的作用机制。

Result: 薄盘（thin disk）不仅导致双星系统快速旋进，还会以更高速率激发轨道偏心率（eccentricity），且薄盘可将偏心率提升至e≳0.6，而厚盘（thick disk）仅能稳定在e≈0.425。
...
**TL;DR**: 研究发现银河系中心(Sgr A*)附近环核盘(CND)的最佳拟合模型是内半径1.0 pc、外半径2.9 pc、倾角60°、位置角35°的圆形盘结构。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中心超大质量黑洞（SMBH）人马座A*周围核星团（NSC）和环核盘/环（CND）的动力学结构建模。

Data: 数据来源为银河系中心区域HCN（J=1-0）分子谱线观测数据。

Method: 采用非开普勒玫瑰花结轨道（non-Keplerian rosette orbits）和由精细间隔同心环构成的圆盘模型，在Sgr A*与核星团（NSC）共同势场中测试不同内外半径、倾角及位置角参数。

Result: 最佳拟合模型是一个内半径1.0 pc、外半径2.9 pc、倾角60°、位置角35°的圆盘结构。
...
**TL;DR**: 首次利用自适应光学辅助的MUSE积分场光谱仪以0.1"-0.2"角分辨率观测亚角秒间距的双活动星系核(dual AGN)和引力透镜类星体(lensed quasar)候选体，通过盖亚多峰(GMP)技术筛选的30个目标中确认19个为活动星系核多重系统（含6个双活动星系核、10个双像透镜类星体和3个四重像系统），揭示了亮系统以透镜类星体为主而暗系统双活动星系核比例增加的红移演化规律。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高分辨率积分场光谱（integral-field spectroscopy）探索亚角秒间距的双活动星系核（dual AGN）和引力透镜类星体（lensed quasars）的未开发特性，以揭示星系演化、黑洞并合和强引力透镜建模的关键机制。

Data: 数据来源于盖亚(Gaia)目录中采用盖亚多峰(GMP)技术筛选出的角距小于0.8"的点源对，并通过自适应光学辅助的MUSE积分场光谱仪以0.1"-0.2"角分辨率观测30个红移z=0.5-3.5的目标。

Method: 采用自适应光学辅助的MUSE（Multi Unit Spectroscopic Explorer）积分场光谱技术，结合盖亚多峰（Gaia Multi-Peak, GMP）预选方法，对亚角秒分离的双活动星系核（AGN）和引力透镜候选体进行高分辨率（0.1"-0.2"）观测。

Result: 首次采用盖亚多峰（GMP）预选结合MUSE光谱观测，在0.5-3.5红移范围内确认了19个活动星系核（AGN）多重系统，包括6个双活动星系核、10个双成像类星体和3个四重成像系统。
...
**TL;DR**: 首次公开MeerKLASS UHF巡天干涉测量数据（DR1），验证了机载扫描（OTF）技术并发布覆盖南天800平方度的高保真连续谱图像（中心频率816 MHz），探测到95483个射电源，其亚角秒级天体测量精度和可靠流量密度尺度为未来SKA大天区巡天奠定基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用MeerKAT射电望远镜的干涉测量组件开展MeerKLASS UHF巡天项目，验证新型实时（OTF）测绘技术并推动大范围射电巡天科学。

Data: 数据来源于MeerKAT射电望远镜的12小时早期科学观测，覆盖南天约800平方度区域，采用816 MHz中心频率的快速扫描（on-the-fly mapping）技术获取原始干涉测量数据。

Method: 采用快速扫描观测的数据处理流程（data processing pipeline）进行校准和成像，生成中心频率816 MHz的高保真连续谱图像。

Result: 首次公开释放的MeerKLASS UHF巡天干涉测量数据（DR1）验证了实时（OTF）测绘技术的可行性，并提供了覆盖南天800平方度的高保真连续谱图像和95483个射电源的精确编目。
...
**TL;DR**: 发现了57个高红移（redshift）$z>6$的星系候选体，包括14个光谱确认（$z = 6.10$ -- 9.25）、2个小红点（$z = 4.77$, 5.81）和2个背景天体（$z = 3.23$, 3.72），其中最高红移确认（$z = 9.25$）的极蓝星系（Extremely Blue Galaxy）存在阻尼莱曼α（Lyman-$α$）吸收特征，并证实了$z \sim 6$星系团的密度过剩现象，同时通过高分辨率光谱揭示了一个星系中两个空间分辨成分的动力学质量约为$\sim 10^8\ M_\odot$。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（詹姆斯·韦伯太空望远镜）和HST（哈勃太空望远镜）观测数据探索高红移（$z>6$）星系候选体的性质及其在强引力透镜星系团MACS0647中的分布特征。

Data: 数据来源包括JWST（詹姆斯·韦伯太空望远镜）的NIRCam（近红外相机）成像（7个滤光片）和NIRSpec（近红外光谱仪）光谱（PRISM和G395H模式）以及HST（哈勃太空望远镜）存档成像（17个滤光片）对强引力透镜星系团MACS0647的观测。

Method: 采用近红外相机（NIRCam）多波段成像、近红外光谱仪（NIRSpec）棱镜（PRISM）和G395H模式光谱分析，结合哈勃太空望远镜（HST）多波段存档数据，通过光谱红移确认、阻尼莱曼α（Lyman-α）吸收特征识别及空间分辨动力学质量估算进行高红移星系候选体研究。

Result: 发现了57个高红移（redshift）$z>6$的星系候选体，包括14个光谱确认（$z = 6.10$ -- 9.25）、2个小红点（Little Red Dots，$z = 4.77$, 5.81）和2个背景干扰源（interlopers，$z = 3.23$, 3.72）。
...
**TL;DR**: 提出了一种非参数化方法量化局域恒星形成率(SFR)与尘埃衰减的空间关系，发现1.0<z<2.5的14个恒星形成星系中50%恒星形成发生在3.41星等或更高衰减(A^(SFR)_(50%))后，表明尘埃对恒星形成区有显著遮蔽效应。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙正午时期（cosmic noon）恒星形成星系（SF galaxies）中尘埃对局部恒星形成活动（local star-formation）的遮蔽效应及其空间分布特征。

Data: 数据来源于JWST/NIRISS无缝光谱观测（slitless spectroscopy），样本为NGDEEP巡天中14个红移范围1.0<z<2.5的恒星形成星系（star-forming galaxies）。

Method: 采用基于巴尔默减幅（Balmer decrement）的空间分辨映射技术，通过构建累积恒星形成率（SFR）随尘埃消光（attenuation）增加的分布曲线，定义并量化了$\mathrm{A}^{\mathrm{SFR}}_{10\%}$、$\mathrm{A}^{\mathrm{SFR}}_{50\%}$和$\mathrm{A}^{\mathrm{SFR}}_{90\%}$三个关键消光阈值指标。

Result: 50%的局部恒星形成发生在3.41星等或更高的尘埃消光（$\mathrm{A}^{\mathrm{SFR}}_{50\%}$）之后，表明这些星系中大部分恒星形成受到尘埃显著遮蔽。
...
**TL;DR**: 基于LoVoCCS宽视场观测和HST存档图像，测得Tucana Dwarf星系（Tucana Dwarf galaxy）的半光半径（half-light radius）为225.7±14.4 pc，较先前结果增大约10%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用宽视场巡天数据（LoVoCCS）和哈勃空间望远镜（HST）存档图像重新测定Tucana Dwarf星系的半光半径（half-light radius），以提升对本地宇宙矮星系结构参数的测量精度。

Data: 数据来源包括Local Volume Complete Cluster Survey（LoVoCCS）的宽视场深度成像和哈勃太空望远镜（HST）的存档图像。

Method: 基于椭圆拟合（elliptical fitting）和Sérsic轮廓拟合（Sérsic profile fitting）分析恒星密度分布。

Result: 基于LoVoCCS宽视场观测和HST存档图像，测得Tucana Dwarf星系半光半径（half-light radius）为$0.89_{-0.02}^{+0.06}$角分（对应$225.7_{-5.5}^{+14.4}$ pc），较先前结果增大约10%。
...
**TL;DR**: 通过基于模拟推理的神经网络后验估计（SBI-NPE）方法，利用NuSTAR光谱数据优化了MYTORUS解耦模型（分离视线与全局等效氢柱密度NH_Z和NH_S），显著提升了活动星系核（AGN）中铁Kα线（6.4 keV）相关参数（NH_Z、NH_S、光子指数Γ和相对归一化AS）的推断精度，联合准确率达70%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用X射线光谱学（X-ray spectroscopy）揭示活动星系核（AGN）周围核区几何结构特征。

Data: 数据来源包括34篇文献中的NuSTAR光谱拟合结果，以及基于均匀和高斯参数分布生成的34,000个合成光谱。

Method: 采用基于模拟推理的神经后验估计（SBI-NPE）技术，结合归一化流和自回归网络从模拟光谱中学习灵活的后验分布，并应用物理驱动的MYTORUS解耦模型（分离视线与全局等效氢柱密度NH_Z和NH_S）进行参数推断。

Result: 采用基于模拟推理与神经后验估计（SBI-NPE）的机器学习方法显著提升了MYTORUS解耦模型参数（NH_Z、NH_S、Gamma和AS）的推断精度，观测驱动网格比均匀采样表现更优，各参数在1σ内的预测准确率分别达到89%、90%、82%和90%，联合准确率为70%。
...
**TL;DR**: 首次在预恒星核（prestellar core）L1544中通过空间分辨观测发现甲亚胺（methanimine, CH2NH）呈延展分布，其丰度从南部碳链富集区向北部尘埃峰递减，表明在引力坍缩前的中性气相反应主导了前生命氮碳化学演化。


<details>
  <summary>Details</summary>

Background: 首次在预恒星核（prestellar core）L1544中通过空间分辨观测研究了甲亚胺（methanimine, CH2NH）的分布及其天体化学意义。

Data: 数据来源于使用IRAM 30米望远镜对L1544预恒星核的127 GHz CH2NH 2$_{0,2}$-1$_{0,1}$谱线进行的空间分辨观测，分辨率为20"（约2800天文单位）。

Method: 采用IRAM 30米望远镜在127 GHz频段对L1544前恒星核的CH2NH分子进行20角秒分辨率（约2800天文单位）的空间分布成图观测。

Result: 首次在L1544前恒星核中通过空间分辨观测绘制了甲亚胺（methanimine, CH2NH）分布图，其柱密度为(0.5-1.4)×10¹² cm⁻²，丰度梯度从南部碳链富集区向北部尘埃和异氰酸（HNCO）峰值递减。
...
**TL;DR**: 论文揭示了零基线汉伯里-布朗-特维斯(Hanbury Brown-Twiss)实验中二阶相干函数的时间分量可直接反映光源发射过程的光子统计特性，为区分热辐射、同步辐射、轫致辐射和受激辐射机制提供了新方法。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用汉伯里-布朗与特威斯效应（Hanbury Brown-Twiss effect, HBT）的二阶相干函数（second-order coherence function）分析天体辐射源的光子统计特性及其发射机制演化。

Data: 数据来源于HBT实验（Hanbury Brown-Twiss experiment）中通过空间分离探测器测量的强度关联数据，以及共置探测器（通常使用分束器实现）获取的零基线强度关联数据。

Method: 采用汉伯里-布朗-特维斯效应（HBT效应）测量强度关联函数（intensity correlation function），通过解卷积探测器响应函数获取二阶相干函数（second-order coherence function）的时间分量，并结合多波段观测分析光子统计特性（photon statistics）以约束辐射机制。

Result: 零基线强度相关测量经探测器响应函数解卷积后得到二阶相干函数的时间分量，该函数独立于光源亮度分布或大小，直接反映光子统计特性，可用于约束和区分热辐射、同步辐射、轫致辐射和受激辐射等发射机制。
...
**TL;DR**: THRILS（The High-(Redshift+Ionization) Line Search）项目通过JWST/NIRSpec G395M光谱仪对z>8星系进行深度观测，探测高电离谱线特征、搜寻z~4-9典型星系中的超大质量黑洞（supermassive black holes）并研究大质量星系恒星质量增长历史，同时提供了89个天体的光谱红移测量结果和探测阈值。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST/NIRSpec G395M光谱仪获取高红移(z>8)星系的高分辨率深场光谱数据，旨在探索高电离谱线特征、搜寻超大质量黑洞(Supermassive Black Hole)以及解析大质量星系的恒星质量增长历史。

Data: 数据来源于JWST/NIRSpec G395M光谱仪对两个天区进行的深度（>8小时）观测，以及THRILS项目获取的89个天体的光谱红移测量数据。

Method: 采用JWST/NIRSpec G395M光谱仪进行深度（>8小时）观测，重点分析高红移（z>8）星系的高电离（high ionization）谱线特征、z~4-9典型星系中宽巴尔末线（broad Balmer line）发射以及大质量星系恒星质量增长历史。

Result: THRILS项目通过JWST/NIRSpec G395M光谱仪对z>8星系进行深度观测，旨在探测高电离谱线特征以研究初始质量函数(top-heavy initial mass functions)或大质量黑洞生长，搜寻z~4-9典型星系中宽巴尔末线发射的吸积超大质量黑洞，并探究大质量星系的恒星质量增长历史。
...
**TL;DR**: 通过大规模三维粒子模拟发现，无碰撞磁重联（collisionless reconnection）产生的湍流在速度场二阶结构函数平方根呈现斜率约1/3的幂律分布（符合类科尔莫戈罗夫湍流），而磁场二阶结构函数平方根斜率更陡（0.6-0.8），且导向场（guide field）强度会增强磁场波动局部各向异性并加剧磁间歇性，首次系统揭示了无碰撞重联层的多尺度湍流特性。


<details>
  <summary>Details</summary>

Background: 磁重联（magnetic reconnection）是等离子体中磁能转化为流动动能、热能和非热粒子加速的基础物理过程。

Data: 研究使用了大规模三维粒子模拟（particle-in-cell simulation）方法生成的数据来研究无碰撞重联（collisionless reconnection）过程中产生的湍流特性。

Method: 采用三维粒子模拟（Particle-in-Cell, PIC）方法计算重联层内不同方向的结构函数，分析无碰撞重联产生的湍流速度与磁场涨落的统计特性。

Result: 速度涨落二阶结构函数的平方根在中等至大尺度上呈现斜率约1/3的幂律分布，符合类Kolmogorov湍流特征，且该行为在入流、出流和导向场方向均稳健。
...
**TL;DR**: 通过三维双流体模拟揭示了中性-离子碰撞阻尼（neutral-ion collisional damping）对可压缩磁流体湍流（compressible MHD turbulence）谱特性和能量分配的调控作用：随着耦合减弱，阿尔文（Alfvén）和慢（slow）模式谱变陡且慢模式能量占比增至70%，而快（fast）模式占比保持稳定。


<details>
  <summary>Details</summary>

Background: 研究探讨了中性-离子碰撞阻尼（neutral-ion collisional damping）如何影响可压缩磁流体力学（MHD）湍流的频谱特性及能量分配，聚焦于部分电离星际介质中的湍流驱动过程。

Data: 使用一套三维双流体模拟（3D two-fluid simulations）系统性地改变中性-离子耦合强度并分解湍流速度场来获取数据。

Method: 采用三维双流体模拟（3D two-fluid simulations）系统改变中性-离子耦合强度，并通过速度场分解为阿尔文（Alfvén）、慢（slow）和快（fast）模式来量化各模式对耦合强度变化的响应。

Result: 中性-离子碰撞阻尼通过使阿尔芬(Alfvén)模式和慢模式谱变陡并显著增加慢模式能量占比（在小尺度达70%），同时保持快模式能量占比对耦合强度不敏感，重塑了可压缩磁流体力学(MHD)湍流的能量分布。
...
**TL;DR**: 分析了银河系中心半径3.5千秒差距区域内45个受旋转棒（bar）显著影响的球状星团轨道规则性/混沌性，通过多种方法（包括新提出的振幅谱熵法）发现经典计算下正李雅普诺夫指数（Lyapunov exponents）分布的双峰性，为球状星团分类提供了概率性新方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中心半径3.5千秒差距区域内45个球状星团（globular clusters）在旋转棒（bar）引力作用下的轨道规则性/混沌性（regularity/chaoticity）分析。

Data: 数据来源包括盖亚卫星(Gaia)的EDR3（Early Data Release 3）高精度天体测量数据以及新优化的球状星团平均距离。

Method: 采用最大特征李雅普诺夫指数（MCLE）、MEGNO（邻近轨道平均指数增长因子）、庞加莱截面法、基于计算基频的频率分析法以及基于轨道振幅谱熵的新方法分析了轨道规则性/混沌性，并构建了棒状引力势模型（三轴椭球体）。

Result: 基于计算轨道振幅谱熵的新方法及多种分析技术，发现银河系中心区域45个球状星团（globular clusters）的轨道存在规则/混沌双峰分布，并提出了基于未重归一化影轨道（shadow orbit）的Lyapunov指数概率分类新方法。
...
**TL;DR**: EROS项目通过1996-2003年对麦哲伦云（大麦哲伦云LMC和小麦哲伦云SMC）、银河系核球和旋臂密集星场的光度监测，公开了超过8600万颗恒星的光变曲线（light curves）数据，为通过微引力透镜（gravitational microlensing）效应搜寻银河系暗物质（Dark Matter）候选体大质量致密晕族天体（machos）及瞬变源研究提供了独特历史记录。


<details>
  <summary>Details</summary>

Background: 探索银河系暗物质（Dark Matter）候选体——大质量致密天体（machos）的引力微透镜效应（gravitational microlensing effect）。

Data: 数据来源于EROS-1和EROS-2项目在1990-2003年间对麦哲伦云（LMC和SMC）、银河系核球（Galactic Bulge）和银河系旋臂（Galactic spiral arms）密集恒星场的光度测量，其中EROS-2在1996-2003年监测了超过8600万颗恒星的光变曲线，具体包括LMC的2870万条、SMC的400万条、银河系核球的4290万条和银河系旋臂的1040万条光变曲线，每条光变曲线包含数百次测量，数据通过斯特拉斯堡数据中心（CDS）公开。

Method: 采用引力微透镜效应（gravitational microlensing）技术监测恒星亮度变化，通过分析光变曲线（light curves）搜寻大质量致密天体（machos）。

Result: EROS项目通过引力微透镜效应（gravitational microlensing）搜寻以大质量致密天体（machos）形式存在的银河系暗物质（Galactic Dark Matter），并公开了1996至2003年间监测的超过8600万颗恒星的光变曲线数据。
...
**TL;DR**: 通过分析球状星团（globular cluster）NGC 6304的27颗巨星成员，发现其金属丰度（[Fe/H]）为-0.45±0.05，无显著内禀弥散，存在显著的氮丰度（[N/Fe]）弥散和碳氮反相关，并首次在富金属球状星团中观测到铈（Ce）与氮、铝（Al）的相关性，表明渐近巨星支（asymptotic giant branch）恒星可能驱动了这些元素的富集，且其化学丰度与相似金属丰度的银心场星一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用近红外光谱技术详细分析富金属球状星团（metal-rich globular clusters）NGC 6304的化学丰度特征，以揭示其多星族（multiple populations）现象及与银心场星（bulge field stars）的化学演化关联。

Data: 数据来源于APOGEE-2近红外光谱仪对27颗巨星成员星的观测，结合ASPCAP和测光数据（Gaia和2MASS）提供的大气参数。

Method: 采用BACCHUS代码分析APOGEE-2近红外光谱，结合ASPCAP和测光（Gaia与2MASS）提供的大气参数，测定了27颗巨星成员中17种元素（C、N、O、Na、Mg、Al、Si、S、K、Ca、Ti、V、Cr、Mn、Fe、Ni和Ce）的丰度。

Result: NGC 6304的化学丰度分析显示其金属丰度为$[{\rm Fe/H}] = -0.45\pm0.05$（ASPCAP参数）或$-0.45\pm0.08$（测光参数），且无内禀金属丰度弥散。
...
### [17] [An Extremely-red, UV-bright, and Extended Galaxy at z~6 in PRIMER/UDS: An Early Massive Galaxy Caught Quenching after an Obscured Starburst?](https://arxiv.org/abs/2512.12813)
*Nadara Hudson,Ryan Endsley,John Chisholm*

**TL;DR**: 在红移(redshift)z∼6的莱曼断裂星系(LBG)UDS_43065中发现了一个极红(β=−0.6)但紫外明亮的恒星形成系统，其经历了5-10 Myr前爆发性恒星形成(SFR≈500-1000 M⊙/yr)，形成了20-40%总恒星质量(≈1.5×10¹⁰ M⊙)，且具有极高恒星质量面密度(log(Σeff/(M⊙ kpc⁻²))=10.25±0.13)，可能代表了大质量尘埃恒星形成星系(DSFG)与早期被动星系之间罕见的过渡阶段。


<details>
  <summary>Details</summary>

Background: 研究主题为探索早期宇宙（$z>4$）中大量存在的静止星系（quiescent galaxies）与剧烈星暴活动（starburst episodes）之间的演化联系，特别是通过极端红移（$z\sim6$）紫外亮星系（UV-bright LBG）的观测揭示其作为尘埃遮蔽星暴星系（DSFGs）向静止系统过渡的关键阶段。

Data: 数据来源于约500平方角分的深度近红外相机（NIRCam）成像观测，从中筛选出一个极红（β=-0.6）但紫外明亮的（F115W=26.0星等）z∼6莱曼断裂星系（LBG）UDS_43065，并结合其近红外光谱仪（NIRSpec）光谱分析。

Method: 采用近红外相机（NIRCam）深度成像和光谱分析技术，结合恒星形成历史（SFH）模型推断恒星形成率（SFR）和恒星质量表面密度。

Result: 发现了一个极红（β=-0.6）但紫外明亮的z∼6恒星形成系统（UDS_43065），其光测颜色显示显著的巴尔末间断和强Hα发射，表明5-10 Myr前发生过剧烈星暴（SFR≈500-1000 M⊙/yr），形成了其总恒星质量（≈1.5×10¹⁰ M⊙）的20-40%，之后活动微弱。
...
### [18] [The Merian Survey: A Statistical Census of Bright Satellites of Milky Way Analogs](https://arxiv.org/abs/2512.12846)
...
**TL;DR**: 基于Merian巡天首次数据发布，对393个恒星质量$10^{10.5} < M_{\star, \rm host} < 10^{10.9} M_\odot$的银河系(MW)类似体周围793个明亮、恒星形成卫星星系进行统计普查，发现80%宿主拥有0-3个大质量卫星（类似麦哲伦云），其丰度与总恒星质量相关，径向分布比NFW轮廓更分散，且银河系卫星的空间集中度与Merian系统中50%最集中系统一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系(Milky Way)类似宿主星系周围明亮、恒星形成卫星星系(star-forming satellite galaxies)的统计普查及其在星系形成和演化中的角色。

Data: 数据来源于Merian Survey的第一批数据发布，包括393个恒星质量在$10^{10.5} < M_{\star, \rm host} < 10^{10.9} M_\odot$之间、红移范围$0.07 < z < 0.09$的银河系(Milky Way)类似体（MW analogs）及其周围793个卫星候选体，通过光度选择（包括星等、颜色、角大小、光度红移和大小-质量截断）并结合两个针对H$α$和[O \textsc{iii}]发射线的中波段滤光片进行识别。

Method: 采用光度选择（包括星等、颜色、角大小、光度红移和大小-质量截断）并结合两个针对Hα和[O III]发射线的中波段滤光片进行卫星候选体识别。

Result: result：约80%的银河系(MW)类似宿主拥有0-3个大质量卫星，13±4%拥有两个卫星（与银河系相似），卫星丰度与总恒星质量相关，且径向分布比NFW轮廓更不集中。
...
### [19] [Kinematics of H I and O VI Absorbers: Insights into the Turbulence Driver of the Multiphase Circumgalactic Medium](https://arxiv.org/abs/2512.12958)
*Zhijie Qu,Hsiao-Wen Chen,Eliana Schiller,Jing Wang,Max Gronke*

**TL;DR**: 研究发现H I和O VI吸收体分别呈现多普勒宽度与柱密度的负相关和正相关趋势，表明H I追踪恒定总柱密度系统而O VI反映恒定空间密度区域，共同揭示了从冷光致电离团块到暖高电离晕气体的连续湍流级联物理图像。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用H I和O VI吸收体的谱线宽度与柱密度相关性来探索多相星系周介质（CGM）的大尺度气体运动学。

Data: 数据来源于远紫外光谱探测器（FUSE）获取的背景类星体（QSO）远紫外光谱以及公开的星系巡天数据（$z\lesssim0.1$），并结合文献收集的样本。

Method: 基于H I和O VI吸收体线宽与柱密度的观测相关性，采用多相环星系介质(CGM)运动学模型框架，将H I解释为总柱密度($N_{\rm H}$)近似恒定系统，O VI则反映空间密度($n_{\rm H}$和$n_{\rm OVI}$)恒定区域，并通过$b_{\rm OVI}$-$N_{\rm OVI}$关系映射到符合Kolmogorov类湍流谱的速度-尺度关系。

Result: H I吸收体（H I absorbers）的多普勒宽度（Doppler width）与柱密度（column density）在$N_{\rm HI}\approx10^{13}$-$10^{18}\rm~cm^{-2}$范围内呈明显负相关，而O VI吸收体（O VI absorbers）在$N_{\rm OVI}\approx3\times10^{13}$-$10^{15}\rm~cm^{-2}$范围内呈正相关。
...
### [20] [DREAMS.II. Galaxy Demographics from Direct Te-Based Metallicities at z~2-10: Tracing the Evolution of the Mass-Metallicity and Fundamental Relations](https://arxiv.org/abs/2512.12983)
...
**TL;DR**: 基于JWST中分辨率光谱对292个z=2-10星系的分析发现，在固定恒星质量（M*）和恒星形成率（SFR）条件下，[OII]3727/Hb比值从z~3到8下降5倍，而[OIII]5007/Hb比值保持稳定；通过直接电子温度（Te）法测定金属丰度，揭示高红移星系位于电离参数-金属丰度反相关的低金属端，且气体流入（流出）在z>8时增强约5倍（1.7倍）可解释质量-金属丰度（MZ）和基本（MZ-SFR）关系的演化趋势。


<details>
  <summary>Details</summary>

Background: 研究探讨了红移（redshift）z=2-10的292个星系在固定恒星质量（stellar mass, M*）和恒星形成率（star formation rate, SFR）条件下的发射线比率和直接电子温度（Te）金属丰度（metallicity）的演化规律。

Data: 数据来源于JWST（James Webb Space Telescope）中分辨率光谱观测的292个红移z=2-10的星系，结合了DREAMS、JADES和CEERS项目的观测数据。

Method: 采用固定恒星质量（M*）和恒星形成率（SFR）区间内的红移分箱叠加光谱法消除系统误差，结合直接电子温度（Te）法通过[OIII]4363谱线测定金属丰度，并利用光致电离模型（photoionization modeling）解释观测到的谱线比演化。

Result: 在固定恒星质量（M*）和恒星形成率（SFR）条件下，[OII]3727/Hb比值从z~3到8下降了五倍，而[OIII]5007/Hb比值几乎保持不变。
...
### [21] [A New Catalog of Head Tail Radio Galaxies from the VLA FIRST Survey](https://arxiv.org/abs/2512.13048)
*Tapan K. Sasmal,Soumen Bera,Sabyasachi Pal,Soumen Mondal*

**TL;DR**: 基于甚大阵(VLA)巡天数据系统发现了717个新头尾(HT)射电星系（包括287个窄角尾NAT和430个广角尾WAT），测得其1.4 GHz光度范围10³⁸–10⁴⁵ erg/s、红移上限2.01，并分析了形态特征与物理属性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于射电星系头尾（HT）形态的成因及其与星系团内介质（intracluster medium）相互作用的关联。

Data: 数据来源于甚大天线阵(Very Large Array)的Faint Images of the Radio Sky at Twenty-Centimeters巡天数据库在1400兆赫兹(MHz)频段的观测数据。

Method: 采用甚大天线阵(Very Large Array)在1400兆赫兹频段对射电天空进行系统巡天观测，结合射电星系头尾形态(HT)的形态学分类标准（窄角尾NAT和广角尾WAT）开展统计分析。

Result: 从甚大阵（VLA）FIRST巡天数据库中发现717个新头尾（HT）射电星系，包括287个窄角尾（NAT）和430个宽角尾（WAT），其光度范围$10^{38} \lesssim L_{1.4\,\mathrm{GHz}} \lesssim 10^{45}\,\mathrm{erg\,s^{-1}}$，红移最高达2.01。
...
### [22] [Simulating the spatial distributions of gas- and ice-phase molecules in galaxies: a new method and preliminary results](https://arxiv.org/abs/2512.13117)
*K. Bekki,K. Furuya,T. Shimonishi*

**TL;DR**: 通过整合气体和尘埃相星际化学的星系尺度模拟，首次揭示了尘埃温度（T_dust）和气相元素丰度（如CHNOPS）对银河系类似盘状星系中H_2O、CO、CO_2和CH_3OH冰分子空间分布的关键控制作用。


<details>
  <summary>Details</summary>

Background: 研究探讨了不同光度与类型星系中气相和冰相分子丰度变化的物理成因，通过整合气相与尘埃相星际化学及多种重子物理过程（如尘埃形成、演化和破坏）到星系尺度模拟中。

Data: 数据来源为基于气体密度和温度、尘埃温度（T_dust）、元素丰度（如CHNOPS）、紫外辐射强度（F_UV）以及分子云内宇宙射线电离率（zeta_CR）的星系尺度模拟（simulation）数据。

Method: 采用星系尺度模拟结合气体和尘埃相星际化学（gas- and dust-phase interstellar chemistry），考虑重子物理过程（baryonic physics）如尘埃形成、演化和破坏，基于气体密度、温度、尘埃温度（T_dust）、元素丰度（CHNOPS）、紫外辐射强度（F_UV）和宇宙射线电离率（zeta_CR）预测分子云中气态和冰相分子（如H_2O和CO_2冰）丰度。

Result: 通过整合气体和尘埃相星际化学及多种重子物理过程，首次在星系尺度模拟中预测了单个分子云内H_2O和CO_2等冰相分子的丰度分布，并揭示了尘埃温度（T_dust）和气相元素丰度对上述分子空间分布的关键控制作用。
...
### [23] [Mapping the Ages of Stars with Chemistry](https://arxiv.org/abs/2512.13145)
...
**TL;DR**: 基于化学时钟（chemical clocks）的宽视场高分辨率光谱结合天体测量和星震学，可精确测定恒星年龄并揭示银河系形成历史，其中宽视场光谱望远镜（WST）能提供关键数据集以突破当前校准限制。


<details>
  <summary>Details</summary>

Background: 基于年龄敏感的恒星丰度比（chemical clocks）的化学时钟为重建银河系形成历史提供了强大且可扩展的方法。

Data: 数据来源为宽视场光谱望远镜（WST）提供的高分辨率光谱数据，并结合天体测量学（astrometry）和星震学（asteroseismology）数据。

Method: 基于宽视场高分辨率光谱（wide-field, high-resolution spectroscopy）结合天体测量（astrometry）和星震学（asteroseismology）技术，将化学时钟（chemical clocks）转化为精确且广泛适用的恒星年龄估算工具。

Result: 基于化学时钟（chemical clocks）的恒星年龄敏感丰度比可重构银河系形成历史，而宽视场高分辨率光谱结合天体测量和星震学能将其转化为精确且广泛适用的年龄估计工具。
...
### [24] [Dust emission and extinction in the Orion OMC-3 cloud](https://arxiv.org/abs/2512.13202)
*M. Juvela,N. Ysard*

**TL;DR**: 通过三维辐射传输模型拟合猎户座分子云3（OMC-3）的远红外（FIR）和近红外（NIR）观测数据，发现该区域尘埃主要由平均粒径0.1-0.3微米的多孔大颗粒（如THEMIS模型）组成，可能含冰幔，且NIR消光曲线排除了更大粒径的存在。


<details>
  <summary>Details</summary>

Background: 星际尘埃作为星际云结构的重要示踪物，同时影响云的熱平衡和化学过程，但当前对尘埃特性（尤其是致密恒星形成云中）的认识仍不完整。

Data: 使用远红外（FIR）波段160-250 μm的尘埃发射观测数据和近红外（NIR）消光测量作为约束条件。

Method: 采用三维辐射传输（RT）模型拟合猎户座分子云3（OMC-3）的远红外（FIR）尘埃发射观测数据，并结合近红外（NIR）消光测量作为额外约束，测试了包括THEMIS大孔隙颗粒模型在内的多种尘埃模型。

Result: 在OMC-3区域，尘埃颗粒平均尺寸为0.1-0.3微米，可能带有冰幔，且大孔隙颗粒模型（如THEMIS）与观测数据最为吻合。
...
### [25] [Non-detection of J1407b in ALMA Band 7 Observations](https://arxiv.org/abs/2512.13254)
*Pamela Klaassen,Matthew Kenworthy,Eric Mamajek,Nienke van der Marel,Michiel Min,Amaury Triaud,Antonion Hales*

**TL;DR**: 利用ALMA波段7连续谱观测未在预期位置探测到发射源，排除了尘埃天体导致J1407系统2007年掩食事件的可能性。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用ALMA（阿塔卡马大型毫米波/亚毫米波阵列）对恒星1SWASP J140747.93-394542.6的连续谱观测，旨在验证2017年意外探测到的发射源是否与2007年该恒星的掩食事件相关。

Data: 数据来源于2024年中期使用ALMA（阿塔卡马大型毫米波/亚毫米波阵列）Band 7对恒星1SWASP J140747.93-394542.6进行的连续谱观测。

Method: 采用阿塔卡马大型毫米波/亚毫米波阵列（ALMA）Band 7连续谱观测技术对1SWASP J140747.93-394542.6进行位置测量与自行运动（proper motion）分析。

Result: 2024年ALMA波段7连续谱观测未在预期位置（包括2017年探测位点和自行运动推算位点）检测到发射源（1σ上限17.5 μJy），排除了尘埃天体导致2007年J1407掩星事件的可能性。
...
### [26] [HALO I: Photometric continuum reverberation mapping of Fairall~9](https://arxiv.org/abs/2512.13296)
...
**TL;DR**: 通过塞弗特星系Fairall~9的多波段连续谱延迟测量，发现其延迟谱在u/U波段（巴尔默连续谱污染）和I波段（帕邢跃变/环面尘埃发射）存在异常延迟，整体偏离标准吸积盘模型预测，表明宽线区散射、再处理和长波尘埃贡献导致延迟偏离幂律关系τ_λ∝λ^β。


<details>
  <summary>Details</summary>

Background: 研究活动星系核(AGN)吸积盘(accretion disk)的结构与性质，特别是通过测量连续谱波段间时延(lag-spectrum)来探索其物理起源。

Data: 数据来源包括奥塞瓦托里奥塞罗墨菲（OCM）望远镜新采集的HALO项目监测数据、重新分析的Swift卫星历史光变曲线数据以及文献中的光谱数据。

Method: 采用多波段测光监测（photometric monitoring）结合斯威夫特（Swift）档案数据重分析，构建延迟光谱（lag-spectrum）并量化连续谱时延（continuum time delays）。

Result: 通过HALO和Swift光变曲线测量发现，Fairall~9的带间延迟谱在$u$和$U$波段（巴尔默连续谱污染）及$I$波段（帕邢跃迁/环面尘埃发射）存在异常延迟，整体显著偏离标准盘模型预测。
...
### [27] [APOGEE physical properties of globular cluster tidal tails](https://arxiv.org/abs/2512.13258)
*André E. Piatti*

**TL;DR**: 研究发现银河系球状星团(globular clusters)潮汐尾(tidal tails)的视向速度($σ$$_{\rm V_{LOS}}$)与切向速度($σ$$_{\rm V_{Tan}}$)高度相关，且其运动学特性与不同吸积群(accretion groups)的关联表明球状星团可能形成于具有广泛暗物质晕(dark matter halos)质量分布的星系中。


<details>
  <summary>Details</summary>

Background: 研究探讨球状星团（globular clusters）的形成机制（原位形成或经由暗物质晕吸积至银河系）与其潮汐尾（tidal tails）的动力学特性（宽度、角动量z分量弥散度、视线速度弥散度、切向速度弥散度）之间的潜在关联性。

Data: 数据来源为APOGEE DR17数据库，从中筛选了17个银河系球状星团（globular clusters）的高置信度潮汐尾（tidal tails）成员，并首次计算了潮汐尾宽度（$w$）、角动量z分量弥散度（$σ$$_{\rm L_z}$）、视线速度弥散度（$σ$$_{\rm V_{LOS}}$）和切向速度弥散度（$σ$$_{\rm V_{Tan}}$）四个属性。

Method: 利用APOGEE DR17数据库筛选17个银河系球状星团的高置信度潮汐尾成员，首次计算了潮汐尾宽度（$w$）、角动量z分量弥散度（$σ$$_{\rm L_z}$）、视线速度弥散度（$σ$$_{\rm V_{LOS}}$）和切向速度弥散度（$σ$$_{\rm V_{Tan}}$）四项特性。

Result: 球状星团潮汐尾的视线速度弥散（$σ$$_{\rm V_{LOS}}$）与切向速度弥散（$σ$$_{\rm V_{Tan}}$）呈现高度相关性，与模型预测一致。
...
### [28] [Standardizing reverberation-mapped H$β$ active galactic nuclei using time-averaged radius$-$luminosity relations with 5100\,Å\,, broad H$β$, and narrow \OIII\ luminosities](https://arxiv.org/abs/2512.13345)
*Shulei Cao,Amit Kumar Mandal,Michal Zajaček,Bharat Ratra*

**TL;DR**: 研究发现均质化处理的100个活动星系核(AGN)样本中窄线区[OⅢ]发射线可作为标准化指标，其宇宙学参数支持当前加速膨胀，而宽线区Hβ和5100Å连续谱则倾向于减速膨胀，首次证实[OⅢ]在AGN标准化中的适用性。


<details>
  <summary>Details</summary>

Background: 活动星系核（AGN）因其高且稳定的光度及宽广的红移范围被用作宇宙学研究的替代性探针。

Data: 数据来源于一个均匀的RM Hβ样本，包含约100个活动星系核（AGN），其Hβ时间延迟和三个光度示踪剂（5100Å、宽Hβ线和窄[O III]线）在多个时期进行了平均。

Method: 采用回响映射（reverberation-mapping, RM）技术对宽线Hβ和窄线[O III]进行时间延迟测量，并基于三个R-L关系（5100Å、宽Hβ线和窄[O III]线）进行标准化分析。

Result: 基于宽线区Hβ和窄线区[O III]的RM AGN样本标准化分析表明，[O III]光度能提供与加速膨胀一致的宇宙学约束，而Hβ和5100Å光度则倾向于减速膨胀。
...
**TL;DR**: 提出了一个三参数自洽平衡模型族，用于描述受外部潮汐和内部刚性旋转共同作用的准弛豫恒星系统（如球状星团），发现异步性参数增大时系统更偏离球对称并呈现朝向银河中心延伸、轨道面方向压扁的三轴结构。


<details>
  <summary>Details</summary>

Background: 研究主题为构建受外部潮汐和内部刚性旋转共同作用的准弛豫恒星系统（quasi-relaxed stellar systems）自洽平衡模型，特别针对与宿主星系轨道运动异步旋转的球状星团（globular clusters）。

Data: 数据来源于对截断金模型（truncated King models）的扩展，通过新定义的异步性参数（asynchronicity parameter）耦合潮汐和旋转扰动。

Method: 采用截断金模型(truncated King models)扩展方法，通过新定义的异步参数(asynchronicity parameter)耦合潮汐和旋转扰动，并运用匹配渐近展开法(method of matched asymptotic expansion)求解泊松-拉普拉斯方程组的自由边界问题。

Result: 提出了一个三参数自洽平衡模型族，用于描述受外部潮汐和内部刚性旋转共同作用的准弛豫恒星系统（quasi-relaxed stellar systems），其非同步性参数（asynchronicity parameter）越大，模型的三轴构型（triaxial configuration）偏离球对称程度越高。
...
**TL;DR**: 该研究开发了一种基于Transformer神经网络的快速深度学习超分辨率(SR)技术，将赫歇尔/SPIRE 500μm观测的分辨率提高4.5倍，在模拟测试中实现提取源流量精度优于5%(≥8 mJy)、天体测量误差小于1"，并成功应用于真实COSMOS场观测，与SCUBA-2测量结果吻合良好。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升尘埃恒星形成星系（DSFGs）在远红外和亚毫米波段的观测分辨率，以解决单天线望远镜（single-dish）因角分辨率不足导致的多波段对应体识别困难问题。

Data: 研究中使用的数据来源于SIDES和SHARK模拟数据，并在输入模拟图像中注入仪器噪声，同时保持目标图像无噪声以增强去噪能力。

Method: 采用基于transformer神经网络（transformer neural network）的超分辨率（super-resolution）技术，结合SIDES和SHARK模拟数据训练，提升赫歇尔（Herschel/SPIRE）500微米图像分辨率4.5倍。

Result: result：基于变换器（transformer）神经网络开发的超分辨率（SR）方法将赫歇尔（Herschel）/SPIRE 500μm观测分辨率提升4.5倍，在模拟测试中提取源的通量精度达5%（本征通量≳8 mJy），天体测量误差低于1"，可靠性≳90%（通量＞3 mJy），实际应用于500μm观测时超分辨图像提取源的通量与SCUBA-2测量结果高度吻合（≥10 mJy）。
...
**TL;DR**: 通过高分辨率ALMA观测发现大质量年轻恒星天体G25.65+1.05中的毫米波峰值源MM1是热分子核（hot molecular core），其周围SiO和CH3CN分子发射揭示了宽角度外流结构和可能的旋转盘，而VLA 1A（水脉泽超耀发区域）作为激波界面展现的湍流运动直接关联水脉泽超耀发的起源。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量年轻恒星天体（massive young stellar object）G25.65+1.05的高分辨率观测，旨在揭示其水脉泽（H2O maser）超耀发及周边致密连续谱源（compact continuum sources）的物理本质。

Data: 数据来源于高分辨率ALMA（Atacama Large Millimeter/submillimeter Array）观测的1.3毫米尘埃连续谱和分子谱线发射数据。

Method: 采用高分辨率ALMA（阿塔卡马大型毫米波/亚毫米波阵列）观测数据，结合1.3毫米尘埃连续谱和分子谱线发射分析，重点研究了SiO和CH3CN分子发射揭示的宽角度外流结构及潜在旋转盘动力学特征。

Result: 在G25.65+1.05区域，VLA 1A（水脉泽超耀斑发生位置）被解释为具有SiO分子线观测到的湍流运动的激波界面，这种湍流直接创造了水脉泽活动所需条件，从而将VLA 1A的性质与水脉泽超耀斑起源联系起来。
...
**TL;DR**: 提出了一种基于符号回归(symbolic regression)的新方法，仅需四个基本观测量（u和i波段星等、(g-r)颜色及红移）即可快速估算星系恒星质量，精度与传统光谱能量分布(SED)拟合相当且计算成本极低，适用于LSST和Euclid等下一代巡天项目。


<details>
  <summary>Details</summary>

Background: 研究主题为开发高效准确的方法以估算大规模星系巡天（如LSST和欧几里得）中数十亿星系的恒星质量（stellar mass），重点利用符号回归（symbolic regression）从基本观测数据（测光与红移）推导简洁显式数学表达式。

Data: 数据来源于GALFORM半解析模型生成的模拟目录，该目录复现了加速宇宙物理巡天（PAUS）的观测特征。

Method: 采用符号回归(symbolic regression)方法，基于GALFORM半解析模型模拟数据，通过四个基本观测量（u和i波段星等、g-r颜色、红移）的线性组合构建恒星质量估算的显式数学表达式。

Result: 基于符号回归（symbolic regression）的方法仅需四个基本观测参数（u和i波段星等、g-r颜色及红移）即可快速估算星系恒星质量，精度与传统光谱能量分布（SED）拟合相当，计算成本极低，适用于大规模巡天数据。
...
**TL;DR**: 研究发现维拉·鲁宾天文台（Vera C. Rubin）对潮汐瓦解事件（TDE）的观测可精确测量黑洞质量（误差0.26 dex），但难以准确判断恒星是否完全瓦解，且恒星质量恢复受吸积效率简并性限制。


<details>
  <summary>Details</summary>

Background: 潮汐瓦解事件（Tidal Disruption Events, TDEs）为研究原本处于静止状态的超大质量黑洞（supermassive black holes）提供了重要窗口。

Data: 数据来源于维拉·C·鲁宾天文台时空遗产巡天（Vera C. Rubin Legacy Survey of Space and Time）模拟的潮汐瓦解事件（TDE）光变曲线。

Method: 采用模拟潮汐瓦解事件(TDE)光变曲线并结合鲁宾天文台(Rubin Observatory)观测策略建模的方法，量化了黑洞质量等参数恢复的典型误差。

Result: 潮汐瓦解事件(TDE)的光变曲线可用于建模黑洞(质量恢复误差典型值为0.26 dex)和被瓦解恒星物理参数，但数据采样和测光不确定性会与模型简并性耦合限制参数恢复精度。
...
**TL;DR**: 研究发现简化化学网络（reduced chemistry network）计算非零金属丰度气体分子氢比例（$f_{\mathrm{H2}}$）时存在系统性高估，主要因忽略氧（oxygen）相关反应导致，建议在最小化学网络中纳入氢、氦和氧以准确获取分子氢丰度。


<details>
  <summary>Details</summary>

Background: 研究探讨了在星系形成模拟中使用简化化学网络（reduced chemistry network）计算分子氢分数（$f_{\mathrm{H2}}$）时出现的系统性偏高现象及其成因。

Data: 数据来源于对比研究简化化学网络（reduced chemistry network）和完整化学网络（full chemical network，含30种元素）计算的分子氢分数（$f_{\mathrm{H2}}$），重点关注不同金属丰度（metallicity）、温度（$T\approx1000~\mathrm{K}$）、辐射场强度及屏蔽柱密度（shielding column densities）条件下的HⅠ-$\mathrm{H}_2$转变密度偏移。

Method: 采用简化化学网络（reduced chemistry network）计算非零金属丰度气体中分子氢比例（$f_{\mathrm{H2}}$）并与完整化学网络（30种元素）对比，分析氢分子过剩现象。

Result: 使用简化化学网络计算含金属气体分子氢分数（$f_{\mathrm{H2}}$）时存在系统性高估，尤其在高温高金属丰度条件下，缺失氧相关反应是主要原因。
...
**TL;DR**: 该论文提出了一种基于高斯过程的通用周期核方法，能够有效识别非均匀采样光变曲线中任意复杂周期信号（如非正弦波形），相比传统方法显著提高了对超大质量黑洞双星（massive black hole binaries）复杂周期性特征的探测灵敏度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超大质量黑洞双星（massive black hole binaries）在时域测光巡天中作为周期性活动星系核（AGN）的可观测性及其复杂周期性光变曲线的识别方法。

Data: 研究中使用的数据来源于模拟生成的光变曲线（mock light curves），包括不同基线（baselines）的模拟PTF（Palomar Transient Factory）和LSST（Legacy Survey of Space and Time）数据。

Method: 采用高斯过程（Gaussian processes）结合通用周期性核函数（generic periodic kernel）分析非均匀采样光变曲线，识别任意复杂周期性信号。

Result: result：提出了一种基于高斯过程（Gaussian processes）的通用周期核方法，能有效识别非均匀采样光变曲线中的任意周期性（包括非正弦波形），在模拟数据中比周期图分析或灵活性较低的高斯过程核方法检测到更高比例的真实周期信号。
...
**TL;DR**: 提出$\mathcal{Z}$-$κ$变换（$\mathcal{Z}$-$κ$ transform）消除引力非线性效应，通过$κ=6$或对数变换使物质密度场高斯化并恢复线性功率谱，为利用两点统计量探测原初非高斯性（primordial non-Gaussianity）和早期宇宙物理提供了新方法。


<details>
  <summary>Details</summary>

Background: 研究主题为通过非线性变换消除引力非高斯性（gravitational non-Gaussianity）并恢复线性功率谱（linear power spectrum），以探索早期宇宙物理。

Data: 数据来源于N体模拟（N-body simulations）。

Method: 采用$\mathcal{Z}$-$κ$变换（一种模拟非线性引力演化逆过程的局部非线性密度场变换）结合N体模拟分析密度分布高斯化效果。

Result: 提出的$\mathcal{Z}$-$κ$变换（$κ=6$或$κ\to\infty$）能有效高斯化物质密度场并恢复线性功率谱（linear power spectrum），其功率谱可直接测量原初局部非高斯性（local primordial non-Gaussianity）。
...
**TL;DR**: 论文提出ηCDM模型（eta CDM model），认为标准冷暗物质（cold dark matter）的动力学可驱动宇宙加速膨胀，但该观点被质疑缺乏合理性。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙加速膨胀的可能机制，提出ηCDM模型（eta CDM model）试图通过标准冷暗物质（cold dark matter）动力学解释这一现象，无需引入新物理。

Data: 数据来源为理论模拟（simulation）框架下的标准冷暗物质（cold dark matter）动力学。

Method: method：采用随机框架（stochastic framework）分析密度场（density field）在数十兆秒差距（Mpc）尺度上的涨落对宇宙加速膨胀的影响。

Result: 提出的ηCDM模型声称仅通过标准冷暗物质（cold dark matter）的动力学即可驱动宇宙加速膨胀，无需引入新物理机制。
...
**TL;DR**: 该论文通过模拟星团成员星系引力红移的端到端分析，发现设计以最大化光谱认证成员数量、实现高纯度叠加并控制系统误差的宽视场光谱星团巡天，是获得修正引力理论竞争性约束的关键条件。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索广义相对论（GR）在宇宙学尺度上对引力描述的完备性，特别是在暗能量（dark energy）本质未解和ΛCDM框架下观测数据间张力加剧的背景下。

Data: data：研究使用模拟数据（mock catalogues）进行端到端管道分析，重点关注偏移中心（mis-centring）和光谱完整性（spectroscopic completeness）对修正引力约束的影响。

Method: 采用端到端管道（end-to-end pipeline）在模拟星表上分析红移精度和巡天设计对修正引力理论的约束影响，重点关注中心偏移（mis-centring）和光谱完整性（spectroscopic completeness）。

Result: competitive measurements require wide-field spectroscopic cluster surveys explicitly designed to maximise the number of spectroscopically confirmed members per cluster, to enable high-purity stacking, and to control systematic effects.
...
**TL;DR**: 轴-胀子宇宙学（axio-dilaton cosmology）通过将轴子（axion）作为暗物质（DM）、胀子（dilaton）作为暗能量（DE）的最小基准模型，联合普朗克（Planck）、SPT-3G、DESI DR2和Pantheon$+$数据拟合显示其比ΛCDM模型更优（χ²降低约7），将哈勃常数提升至$H_0 \simeq 69.2\,\mathrm{km\,s^{-1}\,Mpc^{-1}}$，使哈勃张力（Hubble tension）降至3σ以下，但胀子-物质耦合强度$|\mathbf{g}|\sim 10^{-2}$-$10^{-1}$与太阳系广义相对论实验存在矛盾。


<details>
  <summary>Details</summary>

Background: 研究探讨了轴子-膨胀子（axio-dilaton）宇宙学模型如何作为暗物质（DM）和暗能量（DE）的统一理论框架，通过基本物理原理预测粒子质量随膨胀子演化而变化的机制。

Data: 数据来源包括普朗克卫星2018年的温度、偏振和透镜数据，SPT-3G的高$\ell$测量数据，DESI DR2的重子声波振荡（BAO）数据，以及Pantheon$+$超新星数据集。

Method: 采用轴子-伸缩子（axio-dilaton）宇宙学模型，结合普朗克2018温度、偏振和透镜数据、SPT-3G高$\ell$测量、DESI DR2重子声波振荡（BAO）和Pantheon$+$超新星数据，通过$\chi^2$统计分析和参数拟合方法验证模型。

Result: 轴-胀子宇宙学模型（axio-dilaton cosmology）与ΛCDM模型相比能更好地拟合当前观测数据（χ²降低约7），并将哈勃常数（Hubble constant）提升至69.2 km s⁻¹ Mpc⁻¹，使哈勃张力（Hubble tension）降至小于3σ水平。
...
**TL;DR**: 该论文通过贝叶斯分析比较了费米子（FDM）和玻色子（BDM1、BDM2）暗物质混合中子星（DMANS），发现所有模型在核物质参数上一致且允许暗物质比例低于10%，当前观测数据无法区分两类暗物质模型。


<details>
  <summary>Details</summary>

Background: 该研究通过贝叶斯分析比较了中子星中费米子型暗物质（FDM）与玻色子型暗物质（BDM）的混合模型，旨在利用多源观测约束暗物质参数并探讨其对中子星物态方程（EoS）的影响。

Data: 数据来源包括低密度下的手征有效场论（χEFT）计算、中等密度下的有限核与重离子碰撞数据、高密度下的中子星（NS）观测结果。

Method: 采用贝叶斯推断(Bayesian inference)结合相对论平均场(relativistic mean-field)模型约束强子物质状态方程(EoS)，并通过暗物质质量、耦合强度和占比等参数对费米子(FDM)与玻色子(BDM1/BDM2)暗物质模型进行对比分析。

Result: 通过贝叶斯分析比较费米子（FDM）和玻色子（BDM1、BDM2）暗物质混合中子星（DMANS）模型，发现所有模型均允许中子星内暗物质占比低于10%，且暗物质轻微软化状态方程（EoS），导致中子星质量、半径和潮汐形变模量略微减小，但均与NICER及GW170817观测数据兼容。
...
**TL;DR**: 研究发现LISA（激光干涉空间天线）探测毫赫兹引力波背景的能力受天体物理前景和非高斯原初密度扰动影响，导致其对小行星质量原初黑洞（PBH）暗物质（DM）的探测窗口存在显著不确定性，且非线性参数$f_{\rm NL}$的误差会大幅影响PBH丰度估算。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用LISA探测毫赫兹(mHz)引力波(GW)背景来检验小行星质量原初黑洞(PBH)作为暗物质(DM)候选体的可行性，并探讨原初非高斯性对PBH丰度与引力波关联的影响。

Data: 研究中使用的数据来源为LISA（Laser Interferometer Space Antenna）观测的毫赫兹引力波背景信号以及模拟的非高斯原初密度扰动和天体物理前景噪声。

Method: 采用标量诱导引力波（SIGW）背景与原始黑洞（PBH）丰度的关联模型，结合LISA对毫赫兹引力波（GW）的探测能力，分析非高斯性（$f_{\rm NL}$）对PBH暗物质（DM）窗口的约束影响。

Result: LISA对毫赫兹(mHz)引力波(GW)背景的观测能力受天体物理前景和非高斯原初密度扰动的显著影响，导致其对小行星质量原初黑洞(PBH)暗物质(DM)的探测窗口存在较大不确定性。
...
**TL;DR**: 研究发现，在初始条件存在单位量级不均匀性（inhomogeneities）的情况下，当张量标量比（tensor-to-scalar ratio）临界值$r_{crit} \approx 10^{-6}$（对应特征尺度$\mu_{crit} \approx 0.02m_{P}$）时，α吸引子T模型（α-attractor $T$-model）仍可实现60个e-folds的暴胀（inflation），且该$r_{crit}$是均匀解（homogeneous solution）达到给定e-folds数时的下限。


<details>
  <summary>Details</summary>

Background: 研究探讨了在初始条件存在非均匀性（inhomogeneities）的情况下，暴胀（inflation）成功所需的张量-标量比（tensor-to-scalar ratio）临界值（$r_{crit}$）及其与特征尺度（characteristic scale $μ$）的关系。

Data: 数据来源于对α-吸引子T模型（α-attractor T-model）的数值模拟（simulation），通过改变其特征尺度μ并保持初始梯度场和动能场的不均匀性为暴胀能量尺度的单位量级。

Method: 采用α吸引子T模型（α-attractor T-model）并调整其特征尺度μ，同时保持初始梯度场和动能场的不均匀性为暴胀能量量级的单位量级。

Result: 在初始不均匀性（inhomogeneities）为通胀能量尺度量级时，若均匀解产生100个e-folds，则实现60个e-folds通胀的临界特征尺度（characteristic scale）为$μ_{crit} \approx 0.02m_{P}$，对应临界张标比（tensor-to-scalar ratio）$r_{crit} \approx 10^{-6}$。
...
**TL;DR**: 提出一种利用EB模式耦合的二次估计器，可无偏重建弱引力透镜剪切测量中空间变化的乘性（m-）偏差，在欧几里得类红移分布下能检测到5%均方根变化的m偏差（20σ置信度）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用EB模式耦合检测和重建弱引力透镜剪切测量中的空间变化乘性（m-）偏差，以支持第四阶段巡天（Stage IV surveys）的高精度宇宙学研究。

Data: 数据来源于未来第四阶段巡天（Stage IV surveys）的弱引力透镜剪切测量，包括欧几里得（Euclid-like）红移分布和重复在约1°×1°天区的m-偏置（m-bias）模式，考虑了不同偏置形态和特征尺度下的信号叠加（stacking）效果。

Method: 采用二次估计器（quadratic estimator）结合逆方差加权（inverse-variance weights）的$E$和$B$模式耦合分析方法，重建空间变化的乘性（$m-$）偏差。

Result: 提出了一种利用EB模式耦合检测和重建弱引力透镜剪切测量中空间变化乘性(m-)偏差的二次估计器。
...
**TL;DR**: 提出了一种结合非线性粒子位移（N-body simulation）和有效场论（EFT）的混合模型（HEFT），在红移空间中精确模拟莱曼阿尔法森林（Lyman-alpha forest），将模型与模拟功率谱的偏差控制在5%以内（尺度达k≤1 h/Mpc），为DESI及DESI-II的大尺度聚类分析提供了高效准确的模拟工具。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用莱曼阿尔法森林（Lyman-alpha forest）作为高红移和小尺度下宇宙学及星系际介质（intergalactic medium）的独特探测手段，并开发精确理论工具以支持暗能量光谱仪（DESI）的统计分析需求。

Data: 数据来源于非线性粒子位移计算使用的N-body模拟套件AbacusSummit与基于有效场论（EFT）框架的解析扰动偏差前向模型相结合。

Method: 采用混合有效场论（HEFT）模型结合N体模拟套件AbacusSummit的非线性粒子位移与有效场论（EFT）框架下的解析扰动偏置前向建模。

Result: 开发了一种混合有效场论（HEFT）前向模型，结合N体模拟（AbacusSummit）的非线性粒子位移精度与解析扰动偏置前向模型（EFT框架），将模拟Lyman-alpha（Lya）森林场与模型的残余噪声功率谱控制在准线性尺度上近似白噪声，使三维Lya森林随机性的建模误差降至5%以内（尺度k≤1 h/Mpc）。
...
**TL;DR**: 研究发现，在最小耦合的霍恩德斯基引力子类（KGB）中，存在一类可实现幻影穿越（phantom crossing）且与当前宇宙背景和线性大尺度结构观测一致的模型，无需引入共形耦合。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用暗能量光谱仪（DESI）的重子声学振荡测量结合宇宙微波背景（CMB）和超新星数据，探索晚期暗能量状态方程（equation of state）穿越$w=-1$的现象及其与霍恩德斯基引力（Horndeski gravity）中非最小共形耦合的关联。

Data: 数据来源包括暗能量光谱仪（DESI）的重子声波振荡测量、宇宙微波背景（CMB）温度与透镜功率谱、红移空间畸变、宇宙剪切以及星系与积分萨克斯-沃尔夫效应的互相关观测数据。

Method: 采用非参数化方法探索动能引力编织（KGB）模型，利用mochi_class工具及其基于稳定有效场论（EFT）的实现，在排除幽灵和梯度不稳定性的前提下自由扫描时变EFT函数模型。

Result: 在霍恩德斯基引力（Horndeski gravity）的动能引力编织（KGB）子类中，存在一组无需共形耦合（conformal coupling）即可实现幻影穿越（phantom crossing）且与当前宇宙学观测一致的模型。
...
**TL;DR**: 研究发现，在时域天文警报分类任务中，采用标准化架构（如预训练于星系图像）的卷积神经网络（CNN）在性能和效率上均优于传统定制CNN模型。


<details>
  <summary>Details</summary>

Background: 现代广域时域巡天通过图像差分和警报传递促进了对瞬变、变源和移动现象的研究。

Data: 研究中使用的数据来源包括现代广域时域巡天（wide-field time-domain surveys）的差分成像警报数据、ImageNet日常图像数据集以及Galaxy Zoo的星系图像数据集。

Method: 采用卷积神经网络（CNN）结合预训练（pre-training）和标准化架构（standardized architectures）的方法，比较了不同预训练策略（如Galaxy Zoo星系图像和ImageNet）对天体警报分类性能的影响。

Result: 采用预训练标准化架构的模型在警报分类中表现优于或等同于传统定制卷积神经网络（CNN），且基于Galaxy Zoo星系图像的预训练效果优于ImageNet或从头训练。
...
**TL;DR**: 论文核心贡献是综述了实时瞬变源(transient)工作流的现状，预测了鲁宾天文台(Rubin Observatory)时代的发展趋势，并提出了通过自动化加速瞬变源天文学研究的建议。


<details>
  <summary>Details</summary>

Background: 研究聚焦于自动化技术在光学时域天文学(optical time-domain astronomy)中应对大规模瞬变源(transient)发现的挑战与机遇。

Data: 数据来源包括兹威基瞬变设施（Zwicky Transient Facility）、小行星陆地撞击持续报警系统（Asteroid Terrestrial-impact Last Alert System）和即将投入运行的薇拉·鲁宾天文台（Vera C. Rubin Observatory）及其时空遗产巡天（Legacy Survey of Space and Time）。

Method: 采用机器学习（machine learning）和人工智能（artificial intelligence）工具实现端到端自动化发现与分类光学暂现源（optical transient），并支持自动化快速响应空间跟踪观测。

Result: 当前实时瞬变源（transient）工作流已通过机器学习与人工智能工具实现端到端自动发现与分类，并支持自动化快速响应空间后续观测。
...
**TL;DR**: MeerKLASS-OTF通过快速扫描和相位旋转校正方案实现了高效宽视场连续谱成像，在UHF和L波段分别达到23.3角秒和14角秒分辨率，灵敏度达35μJy/beam和33μJy/beam，为宇宙学、星系演化等研究提供了高质量数据。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用MeerKAT射电望远镜的单口径中性氢强度映射（HI intensity mapping, IM）技术开展南天区大尺度巡天，以支持宇宙学研究，同时兼顾高角分辨率干涉测量。

Data: 数据来源于MeerKAT望远镜的超高频波段（UHF-band）和试点L波段（pilot L-band）的快速恒高度扫描（constant-elevation scans）记录的可见度数据（visibilities），用于生成高质量的快照图像和深度拼接图像。

Method: 采用基于时间相关相位旋转、方向依赖点扩散函数(PSF)建模和宽频带分面去卷积(\texttt{DDFacet})的校正方案处理固定延迟相关观测模式导致的去相关(拖尾)效应。

Result: MeerKLASS-OTF通过时间相关相位旋转、方向依赖点扩散函数建模和宽带分块反卷积校正方案，在UHF和L波段分别实现了23.3角秒和14角秒分辨率，灵敏度达到35μJy/beam和33μJy/beam。
...
**TL;DR**: 开发了AION-Search系统，利用视觉语言模型(VLMs)生成星系图像描述并训练可扩展的语义搜索引擎，在未标注的1.4亿张星系图像中实现了零样本状态下稀有天文现象的先进检索性能。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用视觉语言模型（Vision-Language Models, VLMs）和对比学习技术，实现大规模未标注天文图像（如星系图像）的高效语义搜索，以突破传统人工标注方法在探索海量科学图像数据时的效率瓶颈。

Data: 数据来源为完全未标注的140百万个星系图像（galaxy images），通过望远镜观测获得。

Method: 利用视觉语言模型（VLM）生成星系图像描述，并通过对比对齐预训练多模态天文学基础模型与嵌入描述来构建可扩展的语义搜索嵌入。

Result: 基于完全无标注的星系图像数据开发的AION-Search语义搜索引擎，通过视觉语言模型(VLM)生成描述并对比对齐预训练多模态天文基础模型，实现了对1.4亿张星系图像的灵活语义搜索，且在零样本场景下对罕见天体现象的检索性能达到当前最优水平。
...
**TL;DR**: 该研究通过分析WFC3/UVIS CCD的串行电荷转移效率(x-CTE)效应，发现其对图像的影响远小于并行电荷转移效率(y-CTE)，并开发了基于像素的串行电荷捕获释放模型及校正工具。


<details>
  <summary>Details</summary>

Background: 该研究探讨了哈勃太空望远镜（HST）宽视场相机3（WFC3/UVIS）中电荷转移效率（CTE）损失的科学问题，重点关注串行电荷转移效率（x-CTE）对图像质量的影响及其校正方法。

Data: 数据来源包括暖像素（warm pixels）、宇宙射线（cosmic rays）、饱和恒星（saturated stars）和过扫描像素（overscan pixels）。

Method: 采用暖像素（warm pixels）、宇宙射线（cosmic rays）、饱和恒星（saturated stars）和过扫描像素（overscan pixels）的组合方法，以类似于并行电荷转移效率（parallel-CTE）模型的方式对串行电荷转移效率（serial CTE）进行建模。

Result: 串行电荷转移效率（x-CTE）对图像的影响远小于并行电荷转移效率（y-CTE），截至2023年，串行CTE对亮源的影像约为1%，对暗源的影响约为3%。
...
**TL;DR**: 该论文指出银河系致密双星（以双白矮星为主）在毫赫兹频段产生连续引力波信号，并强调需配套系统性电磁波观测以解算引力波数据中的双星组分性质（质量、温度、成分），但目前缺乏相应设施或协同巡天计划。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系致密双星系统（尤其是双白矮星(DWDs)）在毫赫兹频段产生的连续引力波信号及其电磁对应体的多信使研究。

Data: 数据来源包括LISA（Laser Interferometer Space Antenna）对银河系内致密双星系统的引力波观测数据，以及可能包含中子星和黑洞的系统贡献的引力波信号。

Method: 采用引力波（gravitational-wave）探测与电磁波（electromagnetic）系统表征相结合的方法，通过LISA等引力波探测器获取毫赫兹（millihertz）波段双白矮星（DWDs）及含中子星、黑洞系统的轨道参数，并利用电磁观测破解引力波简并性以确定天体质量、温度与成分。

Result: 银河系毫赫兹波段连续引力波信号主要来自双白矮星（DWDs），并包含中子星和黑洞系统，需通过系统性电磁表征破解引力波简并性以获取天体物理参数。
...
**TL;DR**: 对8种天文学期刊（包括《日本天文学会欧文研究报告》PASJ）的文献计量分析表明，PASJ整体影响力低于国际基线，但通过设施或任务驱动的特刊可阶段性提升影响力，建议通过策划高影响力特刊和加强国际合作改善表现。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于天文学期刊的文献计量学比较分析（comparative bibliometric analysis），探讨不同期刊的学术影响力动态及其与出版策略和国际合作的关系。

Data: 数据来源于Scopus和SciVal数据库。

Method: 采用文献计量学分析（bibliometric analysis）方法，基于Scopus和SciVal数据提取年度出版活动与学术影响力指标，分析时间序列、引用分布及引用年龄特征（citation age profiles）。

Result: 《日本天文学会欧文研究报告》（PASJ）通过设施或任务驱动的特刊和专题展示间歇性高于基准水平的影响力，鉴于其年度发文量较少，这些举措能显著改变年度指标。
...
**TL;DR**: 开发了通用贝叶斯成像工具包（UBIK），基于信息场理论（IFT）和NIFTy软件包，实现了多仪器（如Chandra、eROSITA、JWST和ALMA）数据的统一贝叶斯成像，支持仪器无关的天空模型、共享响应表示、开源协同分析，并能分离弥散辐射、点源和扩展辐射区域。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于天体物理测量数据的贝叶斯成像（Bayesian imaging）方法及其跨电磁波谱的普适性统一框架。

Data: 数据来源包括钱德拉（Chandra）、eROSITA、詹姆斯·韦伯太空望远镜（JWST）和阿塔卡马大型毫米波/亚毫米波阵列（ALMA）的观测数据。

Method: method：基于信息场理论（IFT）和数值信息场理论软件包NIFTy，采用通用贝叶斯成像工具包（UBIK）实现天体测量数据的统一贝叶斯成像，提供独立于仪器的天空模型和共享响应表示共性的仪器接口。

Result: 开发了基于信息场理论(IFT)的通用贝叶斯成像工具包(UBIK)，实现了跨仪器联合分析并分离不同类型天体辐射。
...
**TL;DR**: 这篇论文的核心贡献是建立了斯皮策太空望远镜红外光谱仪（Spitzer/IRS）的中红外光谱立方体数据库（SIMLA），包含数百个空间分辨和非分辨天体的5.2-38μm光谱图，并开发了去除天文前景、背景及探测器效应的新型数据处理流程。


<details>
  <summary>Details</summary>

Background: 研究主题为利用斯皮策太空望远镜（Spitzer）的中红外光谱仪（IRS）构建中红外光谱立方体数据库（SIMLA），涵盖多种天体（如星系、分子云、超新星遗迹、电离氢区）的空间分辨与非分辨光谱数据。

Data: 数据来源于斯皮策太空望远镜（Spitzer Space Telescope）的红外光谱仪（IRS）低分辨率映射模式固定目标观测（5.2-38μm，分辨率R~60-130），包括数百个空间分辨和非分辨目标（如星系、分子云、超新星遗迹、HII区等）的光谱图。

Method: method：采用斯皮策（Spitzer）/IRS低分辨率映射模式固定目标观测数据，结合CUBISM代码进行数据立方体（cube）组装，并利用新型处理流程去除天文前景/背景及探测器效应，实现坏像素自动检测与剔除。

Result: result：开发了SIMLA（Spitzer/IRS Mapping Legacy Archive）中红外光谱立方体数据集，包含数百个空间分辨和非分辨天体的光谱图，并通过新流程有效去除了天文前景、背景及探测器效应。
...
**TL;DR**: 这篇论文的核心贡献是提出需要长期高频率光谱监测年轻恒星体（YSOs）以研究其形成和演化过程中涉及的多尺度物理现象（如吸积、喷流和磁活动等）的时变特性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于年轻恒星天体（YSOs）作为宇宙重子物质基本组成单元的形成与演化机制，涉及从原行星盘演化到吸积动力学的多尺度天体物理过程。

Data: 研究中使用的数据来源于长期高频率的光谱监测（spectroscopic monitoring），时间跨度至少十年，覆盖从短时（小时至天）到长时（月至年）的多种变率（variability）现象。

Method: 采用长期高频率光谱监测（long-term, high-cadence spectroscopic monitoring）结合时域天文框架（time-domain astronomy framework），针对年轻恒星天体（YSOs）的多波段辐射特性进行重复观测，并适配不同科学目标的多节奏策略。

Result: 恒星和行星的形成涉及跨越多个数量级的尘埃颗粒增长以及时间尺度从数十到数百万年的复杂过程。
...
**TL;DR**: 研究发现低质量恒星（low-mass stars）的磁活动（如黑子、耀斑）显著影响宜居带行星的大气化学、辐射水平和长期逃逸，提出利用新一代12米级地基天文台的高分辨率光谱技术（integral-field spectroscopy和multi-object spectroscopy）精确表征恒星活动物理特性及其对行星宜居性的影响。


<details>
  <summary>Details</summary>

Background: 恒星磁活动（stellar magnetic activity）通过星斑（spots）和耀斑（flares）等现象塑造系外行星环境，尤其影响低质量恒星宜居带行星的大气化学、辐射水平和长期宜居性。

Data: 数据来源为下一代12米级地基天文台（next-generation 12-m class ground-based observatory）配备的积分视场光谱仪（integral-field spectroscopy, IFS）和多目标光谱仪（multi-object spectroscopy, MOS）。

Method: 采用积分视场光谱（IFS）和多目标光谱（MOS）技术，在R~4,000和~40,000分辨率下进行时域监测，解析恒星活动（如黑子、耀斑）的连续谱变化、色球层和日冕线诊断以及粒子加速耀斑特征。

Result: 恒星磁活动（stellar magnetic activity）通过黑子（spots）、谱斑（faculae）和耀斑（flares）显著影响系外行星环境，尤其对宜居带行星的大气化学、表面辐射水平和长期大气逃逸起主导作用。
...
**TL;DR**: 开发了基于Nyx代码的自适应射线追踪（adaptive ray tracing）新方法，通过源合并算法和几何重叠校正显著提升了宇宙再电离（reionization）模拟的计算效率，并在4096个GPU上验证了多物理宇宙学模拟的收敛性。


<details>
  <summary>Details</summary>

Background: 现代天体物理学和宇宙学中辐射传输（radiative transfer）数值方法的研究，特别是针对非均匀再电离（reionization）过程的模拟。

Data: 研究中使用的数据来源于多物理宇宙学数值模拟（multiphysics cosmological simulations），包括4096^3个欧拉气体网格（Eulerian gas cells）、4096^3个暗物质粒子（dark matter particles）以及1024^3粗网格上的射线追踪（ray tracing）。

Method: 采用自适应光线追踪（adaptive ray tracing）作为核心分析方法，通过一系列新颖的滤波和变换与AMReX粒子抽象结合，并开发了源合并算法及几何重叠校正方法以提升计算效率。

Result: 提出了一种基于自适应光线追踪（adaptive ray tracing）的新型算法框架，通过源合并（merging sources）和几何重叠校正（geometric overlap correction）显著提升了宇宙再电离（reionization）模拟的计算效率，并在4096个GPU上实现了多物理宇宙学模拟的扩展性验证。
...
**TL;DR**: 这篇论文的核心贡献是提出Mega-MUSE邻近星系偶然性巡天（Mega-MUSE Nearby Galaxy Serendipity Survey）计划，通过利用其他巡天项目在本地群（Local Group）和本地体积（Local Volume）星系中获得的数据，解决因银河系盘面消光和选择效应而无法研究的稀有恒星与气体星云统计特性问题，并探索未知天文现象。


<details>
  <summary>Details</summary>

Background: 该研究旨在利用新观测设施在邻近星系（Local Group and Local Volume galaxies）中获取稀有恒星和气体星云（gaseous nebulae）的可靠统计特性，并探索未知天文现象。

Data: 数据来源于其他巡天项目在本地星系群(Local Group)和本地体积(Local Volume)星系中获取的观测数据。

Method: 采用多单元光谱探测器（MUSE）对邻近星系进行意外观测（serendipitous observations），利用凌星系（Local Group）和邻近宇宙（Local Volume）其他巡天数据解决开放科学问题。

Result: Mega-MUSE邻近星系偶然性巡天（Mega-MUSE Nearby Galaxy Serendipity Survey）旨在利用其他巡天项目在本地群（Local Group）和本地体积（Local Volume）星系中的数据，解决因消光和选择效应无法在银河系盘面研究的稀有恒星与气体星云统计特性问题，并探索未知现象。
...
**TL;DR**: 提出了基于注意力机制（attention-based）的多探测器引力波（gravitational-wave）检测模型AttenGW，通过分层扩张卷积网络和跨探测器一致性注意力聚合模块替代传统图聚合方法，在LIGO真实O3数据测试中单模型误报率比图方法降低数倍，三模型集成性能媲美六模型图集成，且在非高斯噪声背景下表现稳定。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发一种基于注意力机制（attention-based）的多探测器引力波（gravitational-wave）检测模型AttenGW及其配套软件栈，旨在提升LIGO（Laser Interferometer Gravitational-Wave Observatory）实时数据分析的精度与效率。

Data: 数据来源包括LIGO观测数据（GWOSC时间序列）和模拟注入（simulated injections）数据，其中真实数据采用O3运行期间2020年2月时段的引力波事件段。

Method: 采用基于注意力机制（attention-based）的多探测器聚合模块和分层扩张卷积网络（hierarchical dilated convolutional network）进行引力波信号检测。

Result: AttenGW模型通过基于注意力机制（attention-based）的多探测器聚合模块替代了先前基于图（graph-based）的方案，在真实LIGO数据上实现了比单一图模型更低的误报率（false-positive rate），且三模型集成性能与六模型图集成相当，同时在非高斯噪声背景下表现稳定。
...
### [60] [Design and Performance of the Upgraded Prototype Schwarzschild-Couder Telescope Camera Module](https://arxiv.org/abs/2512.12529)
...
**TL;DR**: 原型施瓦西-库德尔望远镜(pSCT)通过升级配备11,328个硅光电倍增管(SiPM)像素的相机模块，显著提高了对甚高能(VHE)伽马射线的探测性能，包括降低噪声、减少电子串扰并优化电荷分辨率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升切伦科夫望远镜阵列(CTAO)的中型望远镜(MST)性能，特别是通过升级施瓦西-库德尔望远镜(SCT)原型机(pSCT)的相机系统来增强甚高能(VHE)伽马射线探测能力。

Data: 数据来源于切伦科夫望远镜阵列观测站（CTAO）的中型望远镜候选设计——施瓦西-库德望远镜（SCT）的原型机（pSCT），该原型机配备部分填充的相机在弗雷德·劳伦斯·惠普尔天文台（FLWO）观测到了蟹状星云（Crab Nebula）的显著伽马射线辐射，目前相机正在升级为包含11,328个硅光电倍增管（SiPM）像素的完整焦平面阵列，并采用降低电子串扰和噪声的升级电子学模块。

Method: 采用双镜设计（dual-mirror）的施瓦西-库德望远镜（Schwarzschild-Couder Telescope）原型机（pSCT）配备升级版电子元件和硅光电倍增管（SiPM）像素阵列，并通过预生产测试模块开发了校准流程以降低电子串扰（crosstalk）和噪声。

Result: 升级后的pSCT相机模块具有低噪声、最小电子串扰和优异的电荷分辨率。
...
### [61] [CUBE2: A Parallel $N$-Body Simulation Code for Scalability, Accuracy, and Memory Efficiency](https://arxiv.org/abs/2512.12629)
*Hao-Ran Yu,Bing-Hang Chen,Kun Xu,Ming-Jie Sheng,Jiaxin Han,Yipeng Jing,Huahua Cui*

**TL;DR**: 开发了名为CUBE2的宇宙学N体模拟代码，通过粒子网格（PM）方法结合优化的快速傅里叶变换（FFT）和多级空间分解，在6144³粒子规模下实现了高效能、线性可扩展性和高精度计算。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于宇宙学N体模拟（N-body simulation）这一高精度计算领域，旨在通过优化算法和数据结构提升大规模粒子系统演化模拟的效率和精度。

Data: 数据来源于在先进计算华东分中心（ACECS）上运行的两个粒子数为$6144^3$的宇宙学数值模拟（cosmological simulation）。

Method: 核心分析方法为采用粒子网格（Particle-Mesh, PM）方法求解物质分布的泊松方程，结合优化的快速傅里叶变换（Fast Fourier Transform, FFT）提升计算效率，并通过多级PM空间分解降低计算复杂度至近似线性，同时利用优化的格林函数（Green's function）确保多级PM与粒子-粒子（Particle-Particle, PP）计算间的引力相互作用精度。

Result: CUBE2（宇宙学N体代码）通过粒子网格（PM）方法结合快速傅里叶变换（FFT）高效求解泊松方程，并采用多级PM空间分解降低计算复杂度至近似线性。
...
**TL;DR**: 利用ZTF（Zwicky Transient Facility）多波段光变曲线数据和周期信息，开发了基于卷积神经网络和全连接层的分类算法，对五类周期性变星（经典造父变星、II型造父变星、盾牌座δ型变星、食双星和天琴座RR型变星）实现了最高99%的分类准确率。


<details>
  <summary>Details</summary>

Background: 开发并测试基于神经网络的、多波段分类算法以分类周期性变星（如脉动变星和食双星），旨在未来应用于LSST（大型综合巡天望远镜）数据。

Data: 数据来源于兹威基瞬变设施（Zwicky Transient Facility，ZTF）第17次数据发布中的g和r两个波段的光变曲线图像及周期信息，涵盖五类周期性变星（经典造父变星、II型造父变星、盾牌座δ型变星、食双星和天琴座RR型变星）。

Method: 采用基于卷积神经网络（Convolutional Neural Networks）与全连接层（Fully Connected Layers）串联的监督学习方法，利用相位折叠光变曲线图像和周期信息对五类周期性变星进行分类。

Result: 开发了一种基于卷积神经网络（CNN）和全连接层（Fully Connected Layers）的监督学习方法，用于多波段周期性变星分类，训练准确率达99%，验证准确率达95.6%。
...
**TL;DR**: 扩展了可微分流体力学代码diffhydro，通过集成混合物理-机器学习模型（hybrid physics-ML models）实现了可扩展的偏微分方程约束推断（PDE-constrained inference），新增辐射加热/冷却（radiative heating/cooling）、OU驱动湍流（OU-driven turbulence）和多网格泊松自引力（multigrid Poisson self-gravity）等物理模块，验证了与Athena++在标准测试中的一致性，并引入求解器内循环神经校正器（solver-in-the-loop neural corrector）减少粗网格误差，支持高效端到端梯度和多设备扩展，在分布式GPU系统上实现了1024^3规模的自引力辐射冷却湍流初始条件梯度重建。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发可扩展的偏微分方程约束推断（PDE-constrained inference）框架及混合物理-机器学习（hybrid physics-ML）模型，以拓展天体物理流体动力学模拟的应用范围。

Data: 数据来源于分布式GPU系统上运行的JAX编写的可微分流体力学代码diffhydro，支持高达1024^3网格规模的模拟，并利用XLA编译模块化有限体积组件生成融合加速器内核。

Method: 采用可微分流体力学代码(diffhydro)结合混合物理-机器学习(hybrid physics-ML)模型，通过模块化有限体积法(finite-volume)组件和自定义伴随(adjoints)实现高效端到端梯度计算，并引入求解器在环(solver-in-the-loop)神经校正器降低粗网格误差。

Result: 开发的可微分流体力学代码diffhydro通过集成辐射加热/冷却、OU驱动湍流(Ornstein-Uhlenbeck driven turbulence)和多重网格泊松自引力(self-gravity via multigrid Poisson)等新物理模块，在1024^3网格规模上实现了分布式GPU系统的高效梯度重建与混合物理-机器学习建模。
...
**TL;DR**: 首次利用H.E.S.S.强度干涉仪测量了Nunki（σ Sgr）和Dschubba（δ Sco）两个恒星系统的三阶相关函数（third-order correlation functions），并通过实验室伪热光源验证了从二阶贡献中分离三光子项以提取闭合相位（closure phase）余弦值（cos φ）的分析方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用闭合相位（closure phase）在强度干涉测量（intensity interferometry）中恢复传统方法无法获取的干涉相位信息，以拓展该新兴领域的观测能力。

Data: 使用H.E.S.S.强度干涉仪（H.E.S.S. intensity interferometer）在2023年观测了Nunki（σ Sgr）和Dschubba（δ Sco）两个恒星系统，并在实验室实验中采用伪热光源（pseudo-thermal light source）获取数据。

Method: 采用三阶相关函数（third-order correlation functions）分析技术，通过计算三台望远镜同时测量的三光子相关性来提取闭合相位（closure phase）的余弦值（cos φ）。

Result: 首次利用H.E.S.S.强度干涉仪测量了恒星系统Nunki（σ Sgr）和Dschubba（δ Sco）的三阶相关函数，并展示了从二阶贡献中分离三光子项以获取闭合相位（closure phase）余弦值（cos φ）的方法。
...
**TL;DR**: 论文核心贡献是提出利用新一代公里级光学/红外干涉仪（optical/IR interferometer）结合衍射极限积分场光谱（IFS）技术，研究致密恒星系统（compact stellar systems）等天体对星系演化和黑洞增长的驱动作用，并拓展至活动星系核（AGN）供能、恒星表面及双星相互作用、系外行星大气等科学领域。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于致密恒星系统（compact stellar systems）、星团、核星团（NSCs）、剥离核（stripped nuclei）和超致密矮星系（UCDs）作为星系演化和黑洞增长的驱动机制。

Data: 数据来源包括现有欧洲南方天文台（ESO）基础设施的观测数据以及下一代公里级光学/红外干涉仪（optical/IR interferometer）的衍射极限积分场光谱（IFS）数据。

Method: 采用下一代公里级光学/红外干涉仪（optical/IR interferometer）结合衍射极限积分场光谱（IFS）技术进行观测分析。

Result: 核心结论为下一代公里级光学/红外干涉仪（optical/IR interferometer）结合衍射极限积分场光谱（IFS）技术将推动致密恒星系统（compact stellar systems）与星系演化及黑洞增长研究取得突破性进展。
...
**TL;DR**: 研究发现球状星团（GCs）的中子俘获（n-capture）元素分布能有效追踪其形成环境、原位与吸积系统的化学特征，并完善对银河系（MW）晕早期化学演化的理解。


<details>
  <summary>Details</summary>

Background: 球状星团（GCs）是研究银河系（MW）早期组装过程的关键示踪物，其化学和动力学特征保留了形成环境及星系合并历史的原始信息。

Data: 数据来源为高分辨率、多目标且对蓝光敏感的光谱观测（high-resolution, multiplexing, and blue-sensitive spectroscopy），用于获取每个球状星团（GC）中数十颗恒星的中子俘获（n-capture）元素诊断信息。

Method: 采用高分辨率、多目标且对蓝光敏感的光谱技术（high-resolution, multiplexing, and blue-sensitive spectroscopy）对球状星团（GCs）中的中子俘获元素（n-capture elements）进行系统性分析。

Result: 球状星团（GCs）是研究银河系（MW）早期组装的基本示踪物，其化学和动力学特征保留了形成环境和星系合并历史的信息。
...
**TL;DR**: 提出了一种新型贝叶斯建模方法XID+stepwise，利用PRIMAger（PRobe far-Infrared Mission for Astrophysics）的高光谱成像能力，通过从短波到长波顺序传递约束，在45-235微米波段实现了超越混淆极限（confusion limit）3-7倍的流量测量精度，并证实该方法能有效探测红移z~2的IR明亮星系（IR-luminous galaxies）在多数波段的辐射。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用PRIMAger（PRobe far-Infrared Mission for Astrophysics）远红外空间望远镜的高光谱成像能力，突破传统远红外观测的混淆噪声限制，实现对高红移（z~2）红外明亮星系的高灵敏度探测与光谱能量分布（FIR SED）精确测量。

Data: 数据来源为合成的深场成像巡天数据，包含真实仪器噪声和混淆噪声（confusion noise），并基于最新PRIMAger仪器参数生成。

Method: 采用了一种新的贝叶斯建模方法XID+stepwise，利用PRIMAger的高光谱成像技术，通过从短波长到长波长顺序传播约束，推导出自洽且信息丰富的流量先验。

Result: 提出了一种新的贝叶斯建模方法XID+stepwise，利用PRIMAger的高光谱成像能力通过从短波到长波顺序传播约束来推导自洽且信息丰富的流量先验。
...
### [68] [Extending Weinberg's EFT: effective scalar-tensor theories up to sixth order](https://arxiv.org/abs/2512.13453)
*Eugeny Babichev,Sukŗti Bansal,Maria Mylova,Antonio Padilla*

**TL;DR**: 该研究系统构建了六阶导数（six-derivative）标量-张量有效理论，确定了包含5个宇称守恒和3个宇称破坏的独立相互作用项，为探索量子/弦论修正、宇称破坏作用及强曲率效应提供了高阶理论框架。


<details>
  <summary>Details</summary>

Background: 研究主题为构建六阶导数（six-derivative）标量-张量（scalar-tensor）有效场论，扩展了四阶导数框架以探索量子或弦论修正、宇称破坏（parity-violating）相互作用及强曲率效应。

Data: 研究基于四维平直时空（four-dimensional flat spacetime）的散射振幅形式（scattering amplitude formalism）和广义协变性（general covariance）约束下的独立六阶导数标量-张量相互作用理论推导数据。

Method: 采用六阶导数（six-derivative）有效场论方法构建标量-张量理论（scalar-tensor theories），包含五个宇称偶（parity-even）和三个宇称奇（parity-odd）独立相互作用项。

Result: 六导数标量-张量理论（six-derivative scalar-tensor theories）的系统构建包含五个宇称偶（parity-even）和三个宇称奇（parity-odd）独立相互作用项，代表了与广义协变性一致的所有不等价变形。
...
### [69] [Constraining the dark matter origin of the halo-like 20 GeV $γ$-ray excess with the AMS-02 antiproton data](https://arxiv.org/abs/2512.12176)
*Xiao Wang,Kai-Kai Duan*

**TL;DR**: 研究发现银河系晕中约20 GeV的伽马射线超量（gamma-ray excess）不太可能源自暗物质（dark matter）湮灭，因为预期的反质子（antiproton）通量在约100 GeV时已超出AMS-02观测值，且所需湮灭截面（annihilation cross section）比解释该信号所需值低约一个数量级。


<details>
  <summary>Details</summary>

Background: 研究探讨了银河系晕中约20 GeV伽马射线超出的暗物质(Dark Matter)起源假说及其与AMS-02反质子观测数据的矛盾。

Data: 数据来源为AMS-02（阿尔法磁谱仪）观测数据。

Method: 采用宇宙线传播模型（cosmic ray propagation model）计算反质子（antiproton）发射通量，并与AMS-02观测数据对比以约束暗物质（dark matter）湮灭截面（annihilation cross section）。

Result: result：银河系晕中约20 GeV的伽马射线超出现象不可行作为暗物质信号，因其预期的反质子通量在约100 GeV已超出AMS-02观测值，所需湮灭截面比解释该信号低约一个数量级。
...
### [70] [A Search for Soft $γ$-ray Emission from the Galaxy Cluster SPT-CL J2012-5649 (Abell 3667) Using INTEGRAL/ISGRI](https://arxiv.org/abs/2512.12616)
*Siddhant Manna,Shantanu Desai*

**TL;DR**: 利用INTEGRAL/ISGRI观测数据对合并星系团SPT-CL J2012-5649（Abell 3667）进行软伽马射线（soft γ-ray）搜索，未检测到显著信号，给出了30-300 keV能段最严格的逆康普顿（inverse-Compton）辐射上限（6.9×10⁻⁹ erg cm⁻² s⁻¹），排除了明亮辐射模型并约束了合并加速粒子效率。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用软γ射线探测合并星系团SPT-CL J2012-5649中由激波加速的相对论性电子可能产生的逆康普顿（IC）辐射，以揭示星系团合并过程中的粒子加速机制。

Data: 使用INTEGRAL/ISGRI的存档观测数据（archival INTEGRAL/ISGRI observations），总有效曝光时间为2506秒。

Method: 采用标准OSA 11.2成像和光谱处理流程（standard OSA 11.2 imaging and spectral pipelines）分析INTEGRAL/ISGRI存档数据，并通过马赛克方差图（mosaic variance maps）推导保守上限。

Result: 在30-300 keV能段未探测到SPT-CL J2012-5649的显著软γ射线辐射，其流量上限为$F_{30-300\,\mathrm{keV}} < 6.9 \times 10^{-9}\,\mathrm{erg\,cm^{-2}\,s^{-1}}$，排除了明亮逆康普顿(IC)辐射情景。
...
### [71] [Multimessenger Probes of the Supermassive Black Hole Binary Population: The Role of Pulsar Timing Arrays](https://arxiv.org/abs/2512.11981)
*Nima Laal,Stephen R. Taylor,Cayenne Matt,Kayhan Gultekin*

**TL;DR**: 通过贝叶斯推断模拟脉冲星计时阵（PTA）数据，量化了利用引力波背景（GWB）约束超大质量黑洞双星（SMBHB）硬化速率（hardening rate）、演化寿命及星系恒星质量函数（GSMF）特征质量变量的显著潜力（不确定性降低50%、25-75%及25-50%），但对其余参数（无红移演化的$M_{\text{BH}}$-$M_{\text{Bulge}}$模型参数和GSMF归一化参数）信息增益有限。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用脉冲星计时阵（PTAs）探测超大质量黑洞双星（SMBHBs）产生的引力波背景（GWB），以多信使天文手段揭示黑洞动力学与星系演化间的关联。

Data: 使用北美纳赫兹引力波天文台（NANOGrav）15年数据集的模拟数据，共分析了200组真实模拟的NANOGrav数据集。

Method: 采用贝叶斯推断(Bayesian inference)分析模拟脉冲星计时阵(PTA)数据，并应用已成功处理NANOGrav十五年数据的超大质量黑洞双星(SMBHB)演化模型。

Result: 通过分析200组模拟NANOGrav数据，发现脉冲星计时阵（PTA）有超过50%概率将超大质量黑洞双星（SMBHB）硬化率先验不确定性降低50%以上，并将其演化寿命不确定性降低25-75%。
...
### [72] [Probing jet-induced optical variability across timescales in radio-loud NLSy1 galaxies](https://arxiv.org/abs/2512.11961)
*Vivek Kumar Jha,Anshul Kumar Sharma,Madhu Sudan,Hum Chand*

**TL;DR**: 研究发现射电噪窄线赛弗特1星系（RL-NLSy1）中γ射线探测源与高偏振类星体（HPQs）呈现陡升结构函数（SFs）的长期相干性，非喷流系统则表现出高变率但平台化SFs的随机盘驱动波动，结构函数衍生的时间相干性可作为喷流主导和取向的有效诊断指标。


<details>
  <summary>Details</summary>

Background: 研究聚焦于射电噪窄线塞弗特1星系（RL-NLSy1）与高偏振类星体（HPQs）的多时间尺度光学变异性，旨在揭示相对论性喷流（jet）与吸积盘（disc）驱动机制对活动星系核（AGN）变异性特征的差异影响。

Data: 数据来源于兹威基瞬变设施（Zwicky Transient Facility）的光变曲线。

Method: 采用分光变幅（fractional variability, F_var）和静止坐标系结构函数（structure functions, SFs）分析光变特性，结合色-光变趋势区分喷流主导（jet-dominated）和吸积盘主导（disc-driven）的辐射机制。

Result: γ射线探测到的射电噪窄线塞弗特1星系（RL-NLSy1s）和高度偏振核心主导类星体（HPQs）展现出陡峭上升的结构函数（SFs），表明尽管变率幅度（F_var）适中，但存在由相对论性喷流多普勒增强同步辐射导致的强长期相干性。
...
### [73] [Flashing fast: characterising the 2025 outburst of MAXI J1957+032](https://arxiv.org/abs/2512.12998)
...
**TL;DR**: 研究发现MAXI J1957+032（吸积毫秒X射线脉冲星）在2025年爆发期间表现出313.6赫兹的相干脉动和长期自转减速（约-2×10^-14赫兹/秒），符合磁偶极制动特征，其表面热点辐射和光学再处理特征支持该天体处于轻度泄漏螺旋桨状态（磁场强度约0.5-3×10^8高斯），且光学延迟峰值反映了短时爆发中向外传播的加热波前和快速盘演化。


<details>
  <summary>Details</summary>

Background: 研究聚焦于吸积毫秒X射线脉冲星（accreting millisecond X-ray pulsar）MAXI J1957+032的爆发特性及其在超紧凑轨道（ultra-compact orbit）中的自旋演化与磁层-吸积盘相互作用。

Data: 数据来源于XMM-Newton和Swift的X射线时序与能谱观测（包括后期NuSTAR观测）以及LCO的同步光学测光数据。

Method: 采用X射线计时（X-ray timing）和光谱分析（spectroscopy）技术，结合XMM-Newton、Swift和NuSTAR观测数据，以及LCO的光学测光（optical photometry），通过热康普顿化（thermal Comptonisation）模型和冷黑体（cool blackbody）拟合0.5-10 keV能谱，并检测脉冲轮廓（pulse profile）的谐波特征。

Result: MAXI J1957+032在2025年爆发期间检测到~313.6 Hz的相干脉冲（coherent pulsations），且未测量到频率导数（frequency derivative），但相对于2022年爆发测量到长期自转减慢（long-term spin-down）~-2x10^-14 Hz s^-1，符合磁偶极制动（magnetic-dipole braking）在宁静期的预期。
...
### [74] [eROSITA-RU Tidal Disruption Events with Keck-I/LRIS: Sample Selection, Optical Properties, and Host Galaxy Demographics](https://arxiv.org/abs/2512.12480)
...
**TL;DR**: 基于eROSITA全天巡天数据筛选出70个潮汐瓦解事件(TDE)候选体，最终确认52个TDE（含41个高置信度样本），发现其X射线波段普遍比光学更亮（93%高置信度样本），首次在宿主星系中观测到绿谷星系和静止强巴尔默星系过表征程度显著低于以往样本，揭示了TDE宿主恒星形成历史的多样性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用eROSITA全天巡天数据发现并分析潮汐瓦解事件（TDE）候选体，探索其X射线与光学特性、宿主星系性质及恒星形成历史多样性。

Data: 数据来源于eROSITA全天巡天（eRASS1--5）在2020年12月至2022年2月期间发现的东银河半球X射线暂现源中筛选的70个潮汐瓦解事件（TDE）候选体，并通过贝叶斯似然比技术与存档光学巡天数据交叉匹配获得宿主星系，同时使用Keck/LRIS光谱仪对所有70个宿主星系进行了光谱观测。

Method: 采用贝叶斯似然比技术（Bayesian likelihood-ratio techniques）进行X射线源与宿主星系的交叉匹配，并通过Prospector进行光谱能量分布（SED）拟合及pPXF进行发射线分析。

Result: 通过X射线和光学波段光度、窄线电离诊断及光学变异性建立分类方案，最终确认52个潮汐瓦解事件（TDE），其中41个高置信度（gold）事件93%在X射线波段本征更亮（$L_{\rm X,peak} > L_{\rm opt,peak}$），且23个光学耀变体中普遍存在X射线峰值延迟现象。
...
### [75] [Probing Periodic and Aperiodic Variability of X-ray Sources in M31, M81 and Centaurus A with Chandra](https://arxiv.org/abs/2512.12651)
*Jiachang Zhang,Zhiyuan Li,Ziqian Hua,Tong Bao*

**TL;DR**: 基于钱德拉(Chandra)存档数据对M31、M81和半人马座A(Cen A)中数百个X射线源（主要是低质量X射线双星(LMXB)）的系统性时变分析，发现短时标耀发、偶发/重复性光变下降及星系依赖的光变-流量关系，为吸积物理提供了均匀的观测约束。


<details>
  <summary>Details</summary>

Background: 基于钱德拉(Chandra)存档数据对M31、M81和半人马座A(Cen A)中数百个X射线源（主要是低质量X射线双星(LMXB)）进行系统性时变分析，研究单次观测中的非周期性和周期性光变特征。

Data: 数据来源于钱德拉(Chandra)存档观测数据，对M31、M81和半人马座A(Cen A)中数百个X射线源（主要为低质量X射线双星LMXBs）进行了系统性时序分析。

Method: 采用钱德拉(Chandra)存档数据对M31、M81和半人马座A(Cen A)的X射线源进行系统性时序分析，重点检测并表征单次观测中的非周期性和周期性光变特征。

Result: 在M31、M81和半人马座A（Cen A）中分别检测到24、5和26个X射线源的耀发（flare）事件，持续时间从数十秒至数万秒，峰值光度为10^37-10^40 erg s^-1，耀发占空比（duty cycle）低至4.9×10^-6-3.5×10^-2。
...
### [76] [Disentangling AGN Feedback and Sloshing in the Perseus Cluster with XRISM: Insights from Simulations](https://arxiv.org/abs/2512.12754)
...
**TL;DR**: 利用高分辨率X射线光谱发现英仙座星系团（Perseus cluster）内存在由合并引发的液面振荡（sloshing）和活动星系核（AGN）反馈共同驱动的非单调速度弥散分布，揭示必须同时考虑星系团内介质（ICM）中大尺度相干运动和小尺度湍流才能解释观测数据。


<details>
  <summary>Details</summary>

Background: 研究主题为利用高分辨率X射线光谱学（X-ray spectroscopy）探究英仙座星系团（Perseus cluster）内由星系团内介质（ICM）动力学过程驱动的气体运动机制，重点关注合并引发的晃动（sloshing）与活动星系核（AGN）反馈的相互作用。

Data: data：研究采用高分辨率X射线光谱仪XRISM和Hitomi的观测数据，并结合理想化控制模拟（controlled simulations）生成的模拟数据，系统分析星系团内介质（ICM）速度弥散剖面和X射线谱线形态。

Method: 采用理想化控制模拟方法，系统分离并合并了合并诱导晃动（sloshing）和活动星系核（AGN）反馈机制，通过前向建模谱线轮廓预测可观测速度剖面和X射线线形。

Result: 仅靠冷流振荡（sloshing）或活动星系核（AGN）反馈均无法重现观测到的速度弥散剖面，二者共同作用才能匹配星系团核心和外围的高弥散值。
...
### [77] [Identifying and Characterizing Very Low Mass Spectral Blend Binaries with Machine Learning Methods](https://arxiv.org/abs/2512.12098)
...
**TL;DR**: 该研究利用机器学习方法开发了分层随机森林模型，能有效识别和分类由晚型M、L和T矮星(very low mass dwarfs)组成的未分辨光谱双星系统，在最优范围内实现了≥85%的识别准确率和≤0.1亚型的分类误差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用机器学习方法识别和表征由晚型M、L和T矮星（late-M, L, and T dwarfs）组成的未分辨极低质量光谱混合双星（spectral blend binaries）。

Data: 数据来源于经验光谱生成的单一和合成双星模板样本。

Method: 采用分层随机森林模型（hierarchical random forest models）对合成的晚型M、L和T矮星（late-M, L, and T dwarfs）双星光谱模板进行训练，实现了双星识别及主次成分分类。

Result: 基于机器学习的分层随机森林模型能有效识别和分类未分辨的极低质量光谱混合双星（late-M、L、T型矮星），在最优参数范围内二元识别召回率和精确度≥85%，组分分类误差中位数≤0.1亚型，系统性分类不确定度≤1亚型。
...
### [78] [HEL1OS -- A Hard X-ray Spectrometer on Board Aditya-L1](https://arxiv.org/abs/2512.12679)
...
**TL;DR**: HEL1OS（高能L1轨道X射线光谱仪）通过碲化镉（CdTe）和碲锌镉（CZT）探测器在8-150 keV能量范围内实现了高分辨率（1 keV@14 keV，7 keV@60 keV）的太阳耀斑时间分辨光谱测量，其6°×6°视场设计在满足质量限制的同时优化了离轴响应。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高能L1轨道X射线光谱仪(HEL1OS)监测太阳耀斑的时间分辨光谱，覆盖8至150 keV能量范围。

Data: 数据来源于搭载在Aditya-L1任务上的HEL1OS（High Energy L1 Orbiting X-ray Spectrometer）载荷，使用碲化镉（CdTe: 8 - 70 keV）和碲锌镉（CZT: 20 - 150 keV）复合半导体探测器分别覆盖8 keV至150 keV的宽能段，几何面积分别为0.5 cm²和32 cm²，并通过不锈钢准直器提供6°×6°视场以限制离轴响应。

Method: 采用碲化镉（CdTe）和碲锌镉（CZT）半导体探测器覆盖8-150 keV能段，结合不锈钢准直器限制视场，并利用自主研发的低噪声数字脉冲处理前端电子学实现高分辨率能谱测量。

Result: HEL1OS（高能L1轨道X射线光谱仪）通过碲化镉（CdTe）和碲锌镉（CZT）探测器实现了8-150 keV能段太阳耀斑时间分辨光谱的高精度测量，地面测试和在轨运行表明其性能符合预期。
...
### [79] [VLBI astrometry of radio stars to link radio and optical celestial reference frames III: 11 radio stars](https://arxiv.org/abs/2512.13136)
...
**TL;DR**: 通过新增11颗射电星（radio stars）的甚长基线干涉测量（VLBI）天体测量数据（其中10颗获得视差和自行），将光学亮端（G<13）的国际天球参考架（ICRF）与盖亚天球参考架（Gaia-CRF）的比对精度提升至最佳<1%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于射电国际天球参考架（ICRF）与光学盖亚天球参考架（Gaia-CRF）在亮光学端（G<13）的系统性偏差问题，旨在通过射电星（radio stars）的甚长基线干涉测量（VLBI）天体测量数据提升两参考架的对齐精度。

Data: 使用甚长基线干涉阵列（VLBA）对11颗射电星（radio stars）进行新观测，获得其位置、视差（parallaxes）和自行（proper motions）数据，其中10颗的视差和自行测量精度在最佳情况下达到<1%。

Method: 采用甚长基线干涉测量（VLBI）技术，利用甚长基线阵列（VLBA）对11颗射电星进行天体测量，获取位置、视差和自行运动数据。

Result: 通过甚长基线干涉测量（VLBA）对11颗射电星（radio stars）进行的新天体测量，其中10颗的视差（parallaxes）和自行（proper motions）测量精度在最佳情况下达到<1%，显著增强了国际天球参考架（ICRF）与盖亚天球参考架（Gaia-CRF）在光学亮端（G<13）的连接。
...
### [80] [Binarity at LOw Metallicity (BLOeM): Projected rotational velocities](https://arxiv.org/abs/2512.12102)
...
**TL;DR**: BLOeM巡天通过测量小麦哲伦云（SMC）929颗大质量星的投影自转速度（vsini），发现主序星样本存在30-60 km/s的低速峰且三分之一低速样本可能富氮，SB2系统多为同步自转短周期双星（平均vsini约140 km/s），超巨星普遍具有低vsini。


<details>
  <summary>Details</summary>

Background: 该研究旨在通过BLOeM（低金属丰度双星）巡天项目获取小麦哲伦云（SMC）中929颗大质量恒星的多历元光谱数据，以分析双星比例、轨道构型及搜寻休眠黑洞双星候选体（OB+BH）。

Data: 数据来源于欧洲南方天文台（ESO）大型项目BLOeM对小麦哲伦云（SMC）中929颗大质量恒星的多历元光谱观测。

Method: 采用多历元光谱观测技术（multi-epoch spectroscopy）测量大质量恒星的自转速度（vsini），并基于赫罗图（Hertzsprung-Russell diagram）分析自转速度分布特征，区分单星、单线光谱双星（SB1）和双线光谱双星（SB2）的旋转特性。

Result: 在赫罗图（Hertzsprung-Russell diagram）中识别出一个自转速度显著下降的区域，该特征大致对应于终端年龄主序（terminal-age main sequence）。
...
### [81] [Citizen CATE 2024: Extending Totality During the 8 April 2024 Total Solar Eclipse with a Distributed Network of Community Participants](https://arxiv.org/abs/2512.13407)
...
**TL;DR**: 利用43台相同望远镜设备在2024年日全食(TSE)期间获取60分钟高动态范围(HDR)偏振光日冕连续观测数据，首次实现长时间尺度日冕拓扑结构变化的物理过程研究。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用2024年日全食(TSE)期间的高动态范围(HDR)偏振光观测，揭示太阳日冕加热、结构及演化的物理机制。

Data: 使用43套相同望远镜和相机设备沿2024年4月8日全食带(Total Solar Eclipse, TSE)布设，获取了60分钟内冕(inner and middle solar corona)偏振可见光的高时间分辨率(high-cadence)、高动态范围(high-dynamic-range, HDR)观测数据。

Method: 采用高时间分辨率（high-cadence）、高动态范围（HDR）偏振光观测技术分析日冕加热、结构和演化的物理过程。

Result: 利用43套相同望远镜设备沿2024年日全食(TSE)路径进行高动态范围(HDR)偏振观测，首次获取了持续60分钟的内、中冕区偏振可见光动态影像。
...
### [82] [The stellar to sub-stellar masses transition in 47 Tuc](https://arxiv.org/abs/2512.13470)
*C. Ventura,M. Tailo,P. Ventura,F. D'Antona,A. P. Milone,A. F. Marino,C. Fiumi*

**TL;DR**: 通过模拟球状星团47 Tuc（47 Tucanae）中低质量恒星的演化并结合JWST观测数据，发现第一代恒星占星团总体的45%，第二代占55%，并确定了第一代和第二代恒星向亚恒星（sub-stellar）过渡的质量分别为0.074和0.07太阳质量，且两代恒星的质量函数在0.22太阳质量以下均符合Kroupa型分布。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用球状星团47 Tuc（47 Tucanae）探讨球状星团中多星族（multiple populations）的存在争议，结合HST（哈勃空间望远镜）测光和高分辨率光谱揭示的表面化学成分差异。

Data: 数据来源为JWST（James Webb Space Telescope）对球状星团47 Tuc低主序带的观测数据。

Method: 采用恒星演化模型（stellar evolution modelling）模拟金属丰度[Fe/H]=-0.78、氧含量[O/Fe]=+0.4和[O/Fe]=0的低质量恒星演化，以区分球状星团47 Tuc中不同星族并推断其质量函数。

Result: 47 Tuc第一代星族占星团总星数的45%，第二代占55%。
...
### [83] [astroCAMP: A Community Benchmark and Co-Design Framework for Sustainable SKA-Scale Radio Imaging](https://arxiv.org/abs/2512.13591)
...
**TL;DR**: 提出了astroCAMP框架，通过统一度量标准（fidelity metrics）、标准化数据集和多目标协同设计（co-design）方法，优化平方公里阵列（SKA）成像管道的科学产出与可持续性（sustainability）平衡。


<details>
  <summary>Details</summary>

Background: 研究聚焦于平方公里阵列(SKA)项目在严格能耗限制下实现高效能科学数据处理时面临的硬件性能利用率低、能效差及缺乏标准化评估体系等挑战。

Data: 数据来源包括标准化的SKA代表数据集（SKA-representative datasets）和参考输出（reference outputs），以及针对AMD EPYC 9334处理器和NVIDIA H100 GPU的WSClean与IDG基准测试结果。

Method: 采用统一可扩展的指标套件（astroCAMP）进行科学保真度、计算性能、可持续性和生命周期经济性的多目标协同设计（co-design），结合标准化SKA代表性数据集和参考输出，在CPU、GPU及新兴加速器上实现可复现基准测试。

Result: astroCAMP框架通过统一可扩展的指标套件、标准化数据集和多目标协同设计方法，为SKA项目在运行和环境限制内最大化科学产出提供了系统性解决方案。
...
### [84] [A Comprehensive Sulfur Chemistry Network with Excited S(1D) and SO(1Δ) for XODIAC Photochemical Model: Validation on Venus and Implications for Exo-Venus Atmospheres](https://arxiv.org/abs/2512.12270)
*Priyankush Ghosh,Namrata Rani,Jeehyun Yang,Karen Willacy,P. B. Rimmer,Liton Majumdar*

**TL;DR**: 计算了金星及类金星系外行星大气中基态S(3P)和激发态S(1D)与CO2反应的动力学参数，发现这些反应对60公里以上大气影响有限但能显著改变系外行星30公里以上硫物种垂直分布，并指出近表面1 ppm原子硫混合比可改善S3和S4观测吻合度。


<details>
  <summary>Details</summary>

Background: 金星及类金星系外行星大气中硫化学（sulfur chemistry）对理解其结构、云层形成及成分组成具有关键作用。

Data: 计算了基态S(3P)和激发态S(1D)与CO2在金星类似条件下的反应动力学参数，采用基于化学显著本征值方法的主方程方法获得温度依赖的速率系数，并推导了NASA 7项多项式系数用于光化学模型。

Method: 采用基于化学显著特征值方法（chemically significant eigenvalue method）的主方程（master-equation）计算温度依赖的速率系数，并结合势能面分析中间体。

Result: 计算获得了基态S(3P)和激发态S(1D)与CO2在金星类似条件下的反应动力学参数，揭示了SO(3Sigma)、SO(1Delta)和CO的生成路径及中间体。
...
### [85] [On the use of polarized thermal emission to constrain cloud grain size and temperature structure of sub-stellar objects](https://arxiv.org/abs/2512.12393)
*Fei Wang,Yuka Fujii,Ben Burningham,Jinping He*

**TL;DR**: 通过热发射光谱偏振分析（thermal emission spectropolarimetry）揭示了褐矮星和系外行星大气偏振光谱主要受温度梯度、云粒子尺寸和云光学厚度三因素调控，并证明未来低分辨率偏振仪可捕捉分子吸收带的窄偏振特征以解析光球层温度梯度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用热发射光谱偏振测量（thermal emission spectropolarimetry）解决褐矮星和系外行星大气温度结构、化学成分与云特性的反演简并性问题。

Data: 数据来源于使用三维蒙特卡洛辐射传输代码ARTES对一维大气模型模拟生成的偏振光谱。

Method: 采用三维蒙特卡洛辐射传输代码（3D Monte Carlo radiative transfer code）ARTES模拟一维大气模型的偏振光谱。

Result: 热偏振光谱主要受温度、温度梯度与波长的组合、云粒子大小及云光学厚度三因素共同调控。
...
### [86] [Pre-perihelion detection of a wobbling high-latitude jet in the interstellar comet 3I/ATLAS](https://arxiv.org/abs/2512.12819)
*M. Serra-Ricart,J. Licandro,M. R. Alarcon*

**TL;DR**: 首次在星际彗星3I/ATLAS内彗发中探测到高纬度喷流（high-latitude jet），其位置角（PA）呈现周期性调制（周期7.74±0.35小时），表明核自转周期可能为15.48±0.70小时，并推算出核自转轴投影方向为PA=280.7±0.2度。


<details>
  <summary>Details</summary>

Background: 首次在星际彗星3I/ATLAS的内彗发中探测到与可见光图像中宽羽流（broad plume）重合的高纬度微弱喷流（high-latitude jet），并发现其喷流角度存在周期性调制现象。

Data: 数据来源于2025年8月3日、5日、18日、19日、21日、24日和29日七个夜晚对彗星3I/ATLAS内彗发中微弱高纬度喷流（high-latitude jet）的观测。

Method: 采用高精度位置角（PA）测量和周期性调制分析技术，结合进动运动模型推断彗核自转轴投影方向及周期。

Result: 首次在星际彗星中探测到高纬度喷流（high-latitude jet）的周期性角度调制，其调制周期为7.74±0.35小时，暗示核自转周期可能为15.48±0.70小时。
...
### [87] [New Massless Spectra from Cosmic String Cusps](https://arxiv.org/abs/2512.12258)
*Amelia Drew,Ivan Rybak*

**TL;DR**: 研究发现南布-后藤（Nambu-Goto）宇宙弦环的尖点（cusp）存在非典型形态（如平滑形状），并推导出其引力波和轴子辐射谱的普适渐近形式$\mathop{\tilde{\!κ}}(ω) \propto ω^{-n/(2n-1)} ω^{-m/(2m-1)}$，同时分析了反向作用和有限弦宽等实际修正对高频截断的影响。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于宇宙弦（cosmic strings）标准模型中纽曼-高托（Nambu-Goto）弦环的尖点（cusps）特性及其引力波（GW）辐射谱的普适性描述与修正。

Data: 研究使用了基于Nambu-Goto弦环模型的模拟数据（simulation data），并考虑了后向反应（backreaction）和有限弦宽（finite string width）等实际修正效应引入的高频截止。

Method: 采用Nambu-Goto弦模型（Nambu-Goto string model）分析宇宙弦（cosmic strings）尖点（cusps）的引力波（gravitational-wave）和轴子（axion）辐射谱，并推导出渐进形式为$\mathop{\tilde{\!κ}}(ω) \propto ω^{-n/(2n-1)} ω^{-m/(2m-1)}$的普适表达式。

Result: 在标准宇宙弦（cosmic strings）模型中，Nambu-Goto弦环的尖点（cusps）会产生渐近形式为$\mathop{\tilde{\!κ}}(ω) \propto ω^{-n/(2n-1)} ω^{-m/(2m-1)}$（$n, m\geq 2$）的引力波（GW）和轴子（axion）辐射谱，且高频段存在由反作用（backreaction）和有限弦宽导致的截断。
...
### [88] [The post-inflationary cosmology of the VISH$ν$ axion-majoron model](https://arxiv.org/abs/2512.12627)
*Alexei H. Sopov,Carlos Tamarit,Raymond R. Volkas*

**TL;DR**: 提出VISH$ν$模型（DFSZ型单位畴壁数选项）的详细晶格分析，精确估计再加热温度和膨胀历史，改进暴胀和轻子生成预测，同时满足自然性条件和暗辐射限制，并表征高频引力波谱以区别于其他方案。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过引入可实验探测的QCD轴子（QCD axion）同时充当马约拉子（majoron）角色，并使其标量伙伴在暴胀期间动态演化，以统一解决标准模型（Standard Model）在暗物质起源、物质-反物质不对称性、微小中微子质量、强CP守恒及大尺度结构形成种子等方面的解释缺陷。

Data: data部分：研究使用了基于DFSZ型（DFSZ-type）VISH$ν$模型的晶格模拟（lattice simulation）数据来分析再加热（reheating）时期的宇宙学历史。

Method: 采用晶格模拟(lattice-informed analysis)结合重加热温度估算，精确化暴胀和轻子生成预测。

Result: VISH$ν$模型通过精确估计再加热温度（reheating temperature）和膨胀历史，使暴胀和轻子生成的预测更加准确，同时满足自然性（radiative stability）的严格条件并符合暗辐射（dark radiation）限制。
...
**TL;DR**: 该论文提出了一种由哈勃诱导相变触发的轴子（axion）动力学错位新机制，通过负里奇标量（Ricci scalar）翻转非最小耦合复场$Φ$的质量项符号，驱动其径向模式产生快子不稳定性而大幅振荡，并在大$|Φ|$时通过高维$U(1)$破缺算子产生角向冲击形成守恒电荷，最终转化为轴子丰度以补充标准错位机制未覆盖的暗物质参数空间。


<details>
  <summary>Details</summary>

Background: 该研究探讨了暴胀后刚性（kination）时期由哈勃诱导相变触发的轴子（axion）动力学错位新机制，涉及非最小耦合复标量场（Φ）的径向模式通过快子（tachyonic）不稳定性放大振幅，并通过高维U(1)破缺算子产生守恒电荷以维持旋转，最终形成多畴结构并转化为轴子丰度以解释暗物质。

Data: 研究使用了基于哈勃诱导相变（Hubble-induced phase transition）的线性域平均处理（linear, domain-averaged treatment）方法，结合非最小耦合复场$Φ$的径向模式动力学和更高维$U(1)$破缺算符的数值模拟（simulation）数据。

Method: 采用线性域平均处理（linear, domain-averaged treatment）分析动力学过程，并识别了两种热历史：通过萨克逊子（saxion）衰变到希格斯玻色子（Higgs bosons）的里奇再加热（Ricci reheating），以及通过希格斯/费米子散射（Higgs/fermion scatterings）高效阻尼萨克逊子能量的外部再加热（external reheating）。

Result: 在哈勃诱导的相变过程中，非最小耦合复场$Φ$的径向模通过快子不稳定性达到大振幅，高阶$U(1)$破缺算符赋予角向动量形成守恒电荷，最终转化为轴子丰度产生暗物质。
...
### [90] [Searching for axion dark matter with magnetic resonance force microscopy](https://arxiv.org/abs/2512.12120)
*Elham Kashi,Muhammad Hani Zaheer,Ryan Petery,Swati Singh*

**TL;DR**: 提出利用磁共振力显微镜（MRFM）在1 GHz频段探测轴子（axion）暗物质的新方法，通过轴子与电子的导数耦合在极化电子自旋样品中产生有效交流磁场，结合泵浦场增强信号和机械振荡器共振检测，可在现有技术条件下快速获得与实验室实验相当的约束限，并能通过调节泵浦场频率和直流磁场扫描轴子质量，同时适用于探索其他暗物质-标准模型耦合。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用磁共振力显微镜（MRFM）技术探测约1 GHz频段的轴子（axion）暗物质，探索其与电子自旋的衍生耦合效应。

Data: 数据来源于磁共振力显微镜（MRFM）实验，包括极化电子自旋的直流磁场（D.C. magnetic field）、微磁体（micromagnet）产生的磁场、附近频率的泵浦场（pump field）以及磁负载机械振荡器（magnet-loaded mechanical oscillator）的共振频率匹配。

Method: 采用磁共振力显微镜（MRFM）技术，结合直流磁场和微磁体极化电子自旋，通过邻近频率的泵浦场增强信号，并利用磁负载机械振荡器的共振频率匹配失谐，以光学干涉法高灵敏度检测自旋相关力。

Result: 提出了一种利用磁共振力显微镜（MRFM）在1 GHz附近搜索轴子（axion）暗物质的方法。
...
### [91] [Born-Infeld signatures in AdS black hole thermodynamics and gravitational lensing](https://arxiv.org/abs/2512.12015)
*Ekrem Aydıner,Tekin Dereli,İzzet Sakallı,Erdem Sucu,Ece Seyma Yörük*

**TL;DR**: 研究爱因斯坦-玻恩-英费尔德-反德西特（EBI-AdS）黑洞的热力学和光学特性，发现其霍金温度、量子修正增强热辐射并可能形成残余黑洞，引力红移分析分离了质量、宇宙常数、电磁电荷和玻恩-英费尔德（BI）修正的贡献，光偏转角计算显示等离子体环境可增强或抑制非线性电动力学特征，热力学相结构显示局部稳定性对参数敏感，热机效率达卡诺极限的30-61%，强场下BI修正显著，等离子体偏折分析提供了频率依赖的观测特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于爱因斯坦-玻恩-因费尔德-反德西特（EBI-AdS）黑洞的热力学与光学特性及其在扩展相空间中的行为。

Data: 研究使用了基于爱因斯坦-玻恩-因费尔德-反德西特（EBI-AdS）黑洞理论模型的模拟数据（simulation data），包括表面引力方法计算霍金温度、广义不确定性原理（GUP）和指数熵修正的量子修正分析，以及通过高斯-博内定理计算真空和等离子体环境中的光线偏转角。

Method: 采用标准表面引力方法计算霍金温度，结合广义不确定性原理（GUP）和指数熵修正分析量子效应，并运用高斯-博内定理计算真空与等离子体环境中的光线偏折角。

Result: 通过广义不确定性原理（GUP）和指数熵修正研究揭示了增强的热辐射及潜在残余黑洞形成。
...
### [92] [A parameterized equation of state for dark energy and Hubble Tension](https://arxiv.org/abs/2512.12697)
*Jing-Ya Zhao,Tong-Yu He,Jia-Jun Yin,Zhan-Wen Han,Rong-Jia Yang*

**TL;DR**: 提出暗能量（dark energy）参数化状态方程并通过多源观测数据验证，发现最佳拟合参数（H₀=73.96±0.16、Ωₘ=0.2434±0.0079、α=-0.00049±0.00092）高度吻合天文观测，为哈勃张力（Hubble tension）问题提供有效解决方案。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过参数化暗能量状态方程（equation of state for dark energy）探索哈勃张力（Hubble tension）问题。

Data: 数据来源包括哈勃参数测量、Pantheon超新星样本、重子声学振荡和DESI DR2数据。

Method: 采用参数化的暗能量状态方程（equation of state for dark energy）并结合哈勃参数测量、Pantheon超新星样本、重子声波振荡（baryon acoustic oscillations）和DESI DR2数据进行观测检验。

Result: result：暗能量参数化状态方程的最佳拟合参数为$H_0=73.96\pm 0.16$、$Ω_{\rm m}=0.2434\pm 0.0079$和$α=-0.00049\pm 0.00092$，与天文观测高度一致，为解决哈勃张力（Hubble tension）提供了有效的参数化方法。
...
### [93] [Hybrid algorithm combining matched filtering and convolutional neural networks for searching gravitational waves from binary black hole mergers](https://arxiv.org/abs/2512.12399)
*Takahiro S. Yamamoto,Kipp Cannon,Hayato Motohashi,Hiroaki W. H. Tahara*

**TL;DR**: 提出了一种基于神经网络分析信噪比（SNR）图的方法，用于高效探测恒星质量黑洞并合（stellar-mass black hole mergers）产生的引力波，其性能与标准匹配滤波流程及MLGWSC-1挑战赛的机器学习算法相当，且计算资源需求合理。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用神经网络提升致密双星并合（compact binary coalescence）引力波信号的搜索效率。

Data: 数据来源于将恒星质量黑洞合并的引力波信号注入到平稳高斯噪声中生成的模拟数据集。

Method: 采用神经网络分析匹配滤波器生成的信噪比（SNR）时间序列堆叠图（SNR map）来预测引力波存在与否。

Result: 提出的神经网络方法能够以与标准匹配滤波流程相当的灵敏度检测引力波信号，且计算资源需求合理。
...
### [94] [Pivot-Only Azimuthal Control and Attitude Estimation of Balloon-borne Payloads](https://arxiv.org/abs/2512.11987)
*Philippe Voyer,Simon Tartakovsky,Steven J. Benton,William C. Jones*

**TL;DR**: 提出了一种基于纯枢轴驱动（pivot-only actuation）的气球载载荷姿态估计与偏航率控制框架，通过多状态扩展卡尔曼滤波（MEKF）融合惯性和矢量相机数据实现高精度估计，并结合PI控制器在Taurus吊舱实验中验证了该模型在真实硬件约束下稳定高速跟踪的有效性。


<details>
  <summary>Details</summary>

Background: 研究背景为基于金牛座实验(Taurus experiment)开发仅通过枢轴驱动实现球载载荷姿态估计与偏航速率控制的方法。

Data: 数据来源包括数值模拟（numerical simulations）和金牛座（Taurus）实验平台的实测数据。

Method: 采用乘法扩展卡尔曼滤波(MEKF)融合惯性和矢量相机测量来估计姿态和陀螺仪偏差，并基于简化刚体模型实施仅枢轴驱动的偏航率PI控制。

Result: 基于Taurus实验开发的仅用枢轴驱动的姿态估计和偏航率控制框架，通过融合惯性测量和矢量相机数据，实现了高精度姿态和陀螺仪偏差估计，并在实际硬件约束下验证了稳定的高速跟踪性能。
