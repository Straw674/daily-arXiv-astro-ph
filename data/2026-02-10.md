<div id=toc></div>

# Table of Contents

- [astro-ph.GA](#astro-ph.GA) [Total: 17]
- [astro-ph.CO](#astro-ph.CO) [Total: 12]
- [astro-ph.IM](#astro-ph.IM) [Total: 10]
- [hep-ph](#hep-ph) [Total: 2]
- [astro-ph.SR](#astro-ph.SR) [Total: 7]
- [hep-ex](#hep-ex) [Total: 1]
- [physics.ins-det](#physics.ins-det) [Total: 1]
- [gr-qc](#gr-qc) [Total: 5]
- [stat.CO](#stat.CO) [Total: 1]
- [astro-ph.HE](#astro-ph.HE) [Total: 6]
- [astro-ph.EP](#astro-ph.EP) [Total: 4]


<div id='astro-ph.GA'></div>

# astro-ph.GA [[Back]](#toc)

### [1] [Scavenger hunt: Selection of obscured active galactic nuclei combining multiband optical variability and colors](https://arxiv.org/abs/2602.07109)
*Demetra De Cicco,Stefano Cavuoti,Maurizio Paolillo,Vincenzo Petrecca,Ylenia Maruccia,Paula Sánchez-Sáez*

**TL;DR**: 通过结合VST-COSMOS巡天的g+r+i三波段光变曲线和颜色特征，采用随机森林分类器显著提高了遮蔽活动星系核(AGN)的识别率（三波段确认下遮蔽AGN恢复率达69%，双波段确认达80%），并证明光变与颜色特征联用比单独使用颜色能有效降低污染率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用光学变率和颜色选择相结合的方法提升活动星系核（AGN）识别效率，特别是在LSST等宽视场光学巡天时代背景下优化对 obscured AGN 的探测能力。

Data: 数据来源于VST-COSMOS巡天项目提供的g+r+i波段光变曲线，时间跨度为3.3年。

Method: 采用随机森林（RF）分类器结合光学变异性特征、六种光学/红外颜色及形态指标，通过三波段（g+r+i）光变曲线独立识别活动星系核（AGN）候选体后合并样本以提高选择纯度，并优化了遮蔽型AGN的识别特征子集。

Result: 结合三波段光变特征与颜色特征的随机森林分类器能显著提升遮蔽活动星系核(AGN)的识别率，在三个波段均独立确认的情况下可恢复69±10%已知遮蔽AGN，仅需两个波段确认时提升至80±10%。

Abstract: As wide-field optical surveys such as Vera Rubin Observatory's Legacy Survey of Space and Time (LSST) begin operations, time-domain astronomy is facing a data revolution, paving the road for new, expanded variability studies. This work leverages the complementary power of optical variability and color selection to identify active galactic nuclei (AGN), focusing on optimizing the identification of obscured AGN, typically more challenging to distinguish from inactive galaxies based on optical variability alone. The analysis is designed to provide valuable insights in the context of performance preview for the LSST, albeit using a scaled-down version of the LSST dataset. We present the first combined AGN selection based on g+r+i band light curves from the VST-COSMOS survey, spanning 3.3 yr. We identify AGN candidates independently in each band using a random forest (RF) classifier trained on features mainly related to optical variability, along with six optical/infrared colors and a morphology indicator. We subsequently merge the three band-specific samples in order to enhance selection purity and reliability. We then focus on defining a subset of features that significantly improve the identification of obscured AGN. The RF classifiers yield a consistent performance across the three bands, highlighting the critical role of contamination. Using the combined three-band plus color selection we successfully recover $58^{+9}_{-8}\%$ of all AGN and $69^{+10}_{-8}\%$ of the known obscured AGN that have been independently confirmed in all three bands. When requiring confirmation in two out of the three bands, these fractions increase to $69^{+10}_{-8}\%$ and $80^{+10}_{-9}\%$, respectively. We also demonstrate that, while combining variability features with colors is crucial to improve obscured AGN selection, relying solely on color features returns a markedly higher contamination rate.

</details>

### [2] [Exploring the dynamical evolution of binary stars in multiple-population globular clusters](https://arxiv.org/abs/2602.07119)
*J. Bruce,E. Vesperini,A. Askar,E. Bortolan,M. Giersz,J. Hong,A. Hypki,A. P. Milone*

**TL;DR**: 通过蒙特卡洛模拟揭示了球状星团中双星（包括混合双星和主序星-白矮星双星）的长期动力学演化差异，发现第二星族（P2）双星比第一星族（P1）经历更强的硬化和瓦解，且双星空间混合时标长于单星，可作为星团初始条件和动力学演化的示踪剂。


<details>
  <summary>Details</summary>

Background: 球状星团中多星族（multiple stellar populations）的共存形成了复杂动力学环境，显著影响双星系统的演化并进而塑造星团自身的演化进程。

Data: 使用MOCCA代码进行了一系列蒙特卡洛模拟（Monte Carlo simulations）来研究球状星团中双星的长期动力学演化。

Method: 采用蒙特卡洛模拟（Monte Carlo simulations）结合MOCCA代码研究球状星团中双星系统的长期动力学演化。

Result: 球状星团中第二族恒星（P2）的双星系统相较于第一族恒星（P1）表现出更高的硬化和瓦解率，导致双星出现率和比例的径向分布存在显著差异。

Abstract: The presence of multiple stellar populations in globular clusters leads to a complex dynamical environment that significantly influences the evolution of binary stars, which in turn impacts the evolution of the cluster itself. For this study, we used a series of Monte Carlo simulations run with the MOCCA code to investigate the long-term dynamical evolution of binary stars in globular clusters hosting two distinct stellar populations. We explored how global binary properties such as incidence, fraction, and spatial distribution evolve over time due to the unique dynamical environment associated with each population. Our results show how binaries in the more centrally concentrated second population (P2) experience increased rates of hardening and disruption relative to the first population (P1), leading to distinct radial profiles in binary incidence and fraction. We also demonstrate the difference in spatial mixing timescales for binaries compared to single stars, where binary stars in each population retain some memory of their initial configurations even after complete single star mixing. Additionally, we investigated the formation and evolution of mixed binaries (binaries composed of a P1 component and a P2 component), which form primarily within the core through dynamical interactions. Finally, we studied main sequence--white dwarf binaries and find that they represent a larger fraction of binaries in P1 compared to P2. The results of this paper highlight the interplay between cluster dynamics and the evolution of binary stars and how binaries can act as tracers of the cluster's initial conditions and dynamical evolution.

</details>

### [3] [AGN versus Star-formation: A MUSE Analysis of NGC 1365](https://arxiv.org/abs/2602.07124)
*Kyla Mullaney,Kelly E. Whalen,Jenna Cann,Kimberly Weaver,Jeffrey McKaig,Sara Doan*

**TL;DR**: 通过高分辨率多波段观测分析NGC 1365（Seyfert星系），首次在约40千秒差距尺度上发现其恒星形成臂中存在激波加热导致的高电离区，并量化了活动星系核（AGN）与恒星形成反馈对气体电离状态的相对影响。


<details>
  <summary>Details</summary>

Background: 活动星系核（AGN）与恒星形成反馈通过加热和移除星系气体抑制恒星形成并塑造星系演化。

Data: 使用了甚大望远镜（VLT）的多单元光谱探测器（MUSE）仪器获取的积分视场单元（IFU）存档数据，并结合詹姆斯·韦伯太空望远镜（JWST）和钱德拉X射线天文台的观测数据。

Method: 采用积分视场单元（IFU）数据结合多波段观测（JWST和Chandra X-ray Observatory），通过分析发射线流量、电离状态、恒星形成及气体运动学的高分辨率映射，探究电离和运动学特性，并利用BPT诊断识别异常高电离区域。

Result: 在NGC 1365的恒星形成臂中发现了一个BPT（Baldwin-Phillips-Terlevich）识别的异常高电离区域，并确定激波加热可能在其中起重要作用。

Abstract: Active galactic nuclei (AGN) and star formation feedback may heat and remove gas from galaxies in a process that quenches ongoing star formation and shapes the evolution of galaxies. Potential impacts from these processes can be seen in the complex and interconnected signatures of AGN and star formation activity throughout a galaxy. Here, we analyze archival integral field unit (IFU) data for the nearby Seyfert galaxy, NGC 1365, as observed with the Multi Unit Spectroscopic Explorer (MUSE) instrument on the Very Large Telescope (VLT). Our analysis probes the ionization and kinematic properties of NGC 1365 at high spatial resolution over unprecedentedly large physical scales (approximately 40 kpc), allowing us to trace the effects of feedback throughout nearly an entire galaxy. We use these optical IFU data in conjunction with observations from the James Webb Space Telescope (JWST) and Chandra X-ray Observatory to analyze and compare maps of emission line flux, ionization state, star formation, and gas kinematics. In doing so, we identify a region of BPT-identified unexpectedly high ionization relative to surrounding areas in the star forming arms, and work to identify its source, finding that shock heating may play a significant role. Results from this analysis allow us to place constraints on the relative impact of AGN and star formation processes on the star forming gas in NGC 1365, as well as begin to inform our understanding on the global impacts of feedback in galaxy populations as a whole.

</details>

### [4] [The stellar velocity anisotropy of strong lensing massive elliptical galaxies and its role in the inference of the Hubble parameter $H_0$ using spatially resolved kinematics](https://arxiv.org/abs/2602.07159)
*Vishal Verma,Quinn Minor*

**TL;DR**: 基于TNG100模拟的10个椭圆星系(z=0.2)研究发现，联合强透镜和运动学建模时广义Osipkov-Merritt(gOM)模型能最准确恢复哈勃常数(H0)，平均偏差仅-0.9±1.6%，而传统Osipkov-Merritt(OM)模型会导致+11.5±1.3%的系统偏差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决宇宙学中的哈勃张力（Hubble Tension）问题，通过强引力透镜时间延迟宇宙学（strong lensing with time-delay cosmography）结合运动学数据改进哈勃常数（H₀）的独立测量精度。

Data: 数据来源于Illustris TNG100模拟中选取的十个红移$z=0.2$的大质量椭圆星系样本，并生成了模拟视线速度弥散图以模拟JWST NIRSpec观测。

Method: 采用四种各向异性模型（Osipkov-Merritt、Mamon-Lokas、恒定β和广义OM）进行运动学单独分析及运动学与强引力透镜联合建模，通过模拟JWST NIRSpec观测的视线速度弥散图来量化H₀偏差。

Result: 联合运动学与强透镜建模可将哈勃常数(H0)偏差控制在亚百分比水平（ML模型+0.2±1.6%、常数β模型-0.9±1.9%、gOM模型-0.9±1.6%），其中广义Osipkov-Merritt(gOM)模型在恢复星系参数和哈勃常数精度方面表现最优。

Abstract: One of the biggest challenges in cosmology, the Hubble Tension, requires independent measurements of $H_0$, and strong lensing with time-delay cosmography is a promising avenue. The inclusion of spatially resolved kinematic data helps break the mass--sheet degeneracy, a key limitation in strong lensing. Kinematics, however, suffers from its own degeneracy due to unknown stellar velocity anisotropy, which can bias galaxy mass profile inferences. We investigate the bias in $H_0$ using a sample of ten massive elliptical galaxies at $z=0.2$ from the Illustris $TNG100$ simulations. We generate mock line-of-sight velocity-dispersion maps resembling JWST NIRSpec observations and test four anisotropy models: Osipkov--Merritt (OM), Mamon--Lokas (ML), constant $β$, and a generalized--OM (gOM) profile, under both kinematics-only and joint kinematics plus strong lensing analyses. We find a sub-percent average bias in $H_{0}$ across ten galaxies with joint modeling for three models: $+0.2 \pm 1.6\%$ (ML), $-0.9 \pm 1.9\%$ (constant) and $-0.9 \pm 1.6\%$ (gOM), with $\sim 5\%$ scatter. Joint modeling reduces bias, improves precision, and mitigates outlier results. Overall, the gOM model best recovers galaxy parameters and delivers the most accurate $H_{0}$ relative to posterior uncertainties considering both analyses. However, the single-parameter OM model produces large systematic biases: with kinematics only data, $H_{0}$ errors can exceed $20\%$, and even with joint modeling, produces an overall bias of $+11.5 \pm 1.3\%$ (OM). The higher bias in OM is unlikely to average out across an ensemble of galaxies. Our findings highlight the impact of anisotropy assumptions on $H_{0}$ inference and, more broadly, in galaxy dynamics.

</details>

### [5] [Massive Star Population in the Sextans A Dwarf Galaxy from HST UV Photometry](https://arxiv.org/abs/2602.07177)
*Roberto Flores,Elena Fantino,Giuseppina Battaglia,Antonio Aparicio*

**TL;DR**: 这篇论文通过哈勃太空望远镜（HST）WFC3紫外测光和有限元（FE）插值方法，构建了贫金属（low-metallicity）矮不规则星系Sextans A中655颗大质量（M>8 M⊙）主序星的星表，最大质量恒星达58±11 M⊙，并与地面光谱观测结果对比验证了质量估计的一致性，为研究低金属丰度大质量恒星早期演化提供了候选样本。


<details>
  <summary>Details</summary>

Background: 研究聚焦于贫金属（low-metallicity）矮不规则星系Sextans A中大质量（M>8M⊙）主序星的编目及其性质分析。

Data: 数据来源为哈勃太空望远镜（HST）宽视场相机3（WFC3）在275和336纳米宽波段滤光片下的紫外测光数据。

Method: 采用有限元（FE）插值法对MIST恒星演化轨迹进行插值计算恒星属性（质量、年龄等）及其相对于测光数据的梯度。

Result: 构建了Sextans A贫金属（low-metallicity）矮不规则星系中655颗大质量（M>8 M⊙）主序星的星表，其中最重恒星质量为58±11 M⊙。

Abstract: We build a catalog of massive (M>$8~$M$_\odot$) main sequence stars in the \mbox{metal-poor} ($\sim0.1~$Z$_\odot$) dwarf irregular galaxy Sextans A. HST WFC3 UV photometry in the 275 and 336 nm wideband filters is arranged in a Color-Magnitude Diagram (CMD), and overlaid on top of stellar evolutionary tracks from the MIST library. The star properties (mass, age, etc.) are computed with a Finite Element (FE) interpolation of the stellar tracks. The FE method, originally developed for solid mechanics problems, provides a general framework for interpolating fields inside domains of complex geometry. Besides the interpolated properties, the algorithm computes their gradients with respect to the photometry. These sensitivities provide a direct an efficient estimate of the associated uncertainties. Our catalog contains 655 stars, with the most massive one estimated at $58\pm11~$M$_\odot$. A comparison with a ground-based spectroscopic census of OB stars yields only 8 matches, evidencing the minimal overlap between both datasets. The mass estimates derived from the UV CMD and the spectral classification are in good agreement for the majority of O-type stars found in both datasets. Our catalog provides an extensive list of candidates for followup spectroscopic observation, which could improve our understanding of the early evolutionary stages of massive \mbox{low-metallicity} stars.

</details>

### [6] [Clump-Scale Dust Attenuation in Epoch of Reionization Galaxies: Spatially Resolved Properties from FirstLight Simulations](https://arxiv.org/abs/2602.07347)
*Yurina Nakazato,Kosei Matsumoto,Akio K. Inoue,Daniel Ceverino,Takashi Hosokawa,Daisuke Toyouchi*

**TL;DR**: 通过宇宙学放大模拟和尘埃辐射传输计算，研究发现高红移（$z=6$-$9$）星系的系统综合衰减曲线比Calzetti曲线更平坦，星团（clumps）衰减曲线更平坦且尘埃柱密度高一个量级，而弥散区域（diffuse regions）因光学薄环境散射增强呈现更陡峭曲线，并通过IRX-$Δβ$模型揭示了尘埃-恒星几何分布差异。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（high-redshift）星系中尘埃衰减（dust attenuation）的空间变化特性及其对星系物理性质精确测定的影响。

Data: 研究使用了宇宙学放大模拟（cosmological zoom-in simulations）结合后处理的尘埃辐射传输计算，分析了376个红移在$z=6$-$9$之间、恒星质量$M_* \gtrsim 10^9 \, M_\odot$的团块星系（clumpy galaxies）数据。

Method: 采用宇宙学放大模拟（cosmological zoom-in simulations）结合后处理尘埃辐射传输计算（dust radiative transfer calculations），并引入基于IRX-$Δβ$平面的简化模型（toy model）分析尘埃衰减特性。

Result: 系统整体衰减曲线比Calzetti曲线更平坦（grayer），即使假设银河系（MW）或小麦哲伦云（SMC）型尘埃；单个团块（clumps）的衰减曲线更平坦，而弥散区域（diffuse regions）因光学薄环境中散射增强呈现更陡峭曲线。

Abstract: Understanding dust attenuation in galaxies at both integrated and spatially resolved scales is fundamental for accurately determining the physical properties of galaxies. Recent high-spatial-resolution observations with ALMA and JWST enable investigations of spatially resolved properties in high-redshift galaxies ($z \gtrsim 6$), but spatial variations in dust properties remain poorly constrained. We use cosmological zoom-in simulations combined with post-processing dust radiative transfer calculations for 376 clumpy galaxies at $z=6$-$9$ with stellar masses of $M_* \gtrsim 10^9 \, M_\odot$. For each system, we investigate dust attenuation and re-emission properties for three components: system-integrated, individual clumps, and diffuse regions. We find that system-integrated attenuation curves are grayer than the Calzetti curve, even when assuming MW- or SMC-type dust. Attenuation curves of individual clumps are even grayer, while diffuse regions exhibit steeper curves owing to enhanced scattering in optically thin environments. Since the effects of optical depth and dust-star geometry are intrinsically degenerate in attenuation curves, we introduce a toy model based on the IRX-$Δβ$ plane, where $Δβ$ denotes the difference between attenuated and intrinsic UV slopes. Applying this framework, we find that clumps have dust column densities approximately an order of magnitude higher than system-integrated values and exhibit co-spatial or dust-extended geometries. In contrast, system-integrated attenuation reflects star-extended geometries driven by contributions from optically thin diffuse regions. We apply this framework to REBELS-IFU galaxies at $z \sim 7$ and find good agreement with our simulation predictions.

</details>

### [7] [Active Galactic Nuclei and STaR fOrmation in Nearby Galaxies AGNSTRONG. III. A Study on Ionized and Warm Molecular Gas Outflows of 6 Type-2 AGNs](https://arxiv.org/abs/2602.07392)
*Ruisong Xia,Chen Qin,Huynh Anh N. Le,Yongquan Xue,Shifu Zhu,Mengqiu Huang,Hao Liu,Xiaozhi Lin*

**TL;DR**: 这篇论文通过研究六个低红移（$z<0.1$）II型活动星系核（AGN）的电离和暖分子气体外流（outflow），发现其中一例展现出近红外波段最强的运动学和光度特征，并揭示了外流运动学与AGN光度之间的正相关性，表明高光度AGN对气体外流有更强驱动作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）反馈机制中气体外流（outflows）对宿主星系星际介质（ISM）的影响及其在抑制或促进恒星形成中的作用。

Data: 数据来源于帕洛玛200英寸海尔望远镜的三重摄谱仪（Triple Spectrograph）和双摄谱仪（Double Spectrograph）对六个II型活动星系核（AGN）的离子化和暖分子气体（如[O III]、Paα、H2 1-0 S(1)等）进行空间分辨测量的观测数据。

Method: 采用帕洛玛(Palomar)200英寸海尔(Hale)望远镜的三重摄谱仪(Triple Spectrograph)和双摄谱仪(Double Spectrograph)对[O III]、Paα、H₂ 1-0 S(1)等电离与暖分子气体发射线进行空间分辨光谱测量。

Result: 在六个II型活动星系核(AGN)中，近红外波段观测到一个AGN同时表现出显著的离子化气体和暖分子气体外流，其运动学特征最强且光度最高。

Abstract: Active galactic nucleus (AGN)-driven gas outflows are one of the best tracers of AGN feedback in action, as these powerful outflows expel/heat or compress the surrounding interstellar medium (ISM), thus quenching or enhancing star-forming activity in their hosts. Studying the kinematics of outflows in different gas phases is crucial for comprehending how AGNs impact the ISM within their host galaxies. However, the differences in the physical natures of ionized and warm molecular gas outflows remain largely unexplored. To obtain a complete picture of AGN outflows and their feedback effects, we present a study of both ionized and warm molecular gas outflows in six type-2 AGNs ($z<0.1$) that exhibit strong ionized outflows in previous optical observations. Utilizing the Triple Spectrograph and Double Spectrograph instruments on the Palomar 200-inch Hale Telescope, we conduct spatially resolved measurements in the slit direction of strong emission lines from both ionized and warm molecular gas, such as $\rm [O\ III]$, $\rm Paα$, $\rm H_{2}$ 1-0 S(1), etc., allowing for a direct comparison of their outflow properties. One out of six AGNs shows significant ionized and warm molecular outflows in near-infrared bands, exhibiting the most powerful kinematics and highest luminosity. A positive correlation between the kinematics and AGN luminosity is shown, suggesting that more luminous AGNs, which reflect higher levels of AGN activity, tend to have a greater impact on the gases, probably driving the outflows.

</details>

### [8] [Properties of Galactic Outflows Driven by Starburst at Cosmic Noon: Insights from Hydrodynamical Simulations](https://arxiv.org/abs/2602.07592)
*Huan Chen,Weishan Zhu,Xue-Fu Li,Tian-Rui Wang,Antonios Katsianis,Long-Long Feng*

**TL;DR**: 通过高分辨率三维流体力学模拟研究了宇宙正午时期低质量星系（恒星质量$9.0 < \log(M_*/M_\odot) < 10.0$）中星暴驱动的星系外流，发现外流特性（速度50-1000 km/s、质量流失率0.3-20 M$_\odot$/yr、质量加载因子0.24-6.26）随时间和空间显著演化，其中冷相（8000 < T ≤ 2×10⁴ K）占主导，且质量加载因子随星系恒星质量增加而降低、随恒星形成率增加而升高，观测与模拟间的部分差异源于测量方法不一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙正午时期低质量星系（$9.0 < \log(M_*/M_\odot) < 10.0$）中星暴驱动（starburst-driven）的星系外流（galactic outflows）现象。

Data: 研究使用基于M82多相外流再现框架的高分辨率三维流体动力学模拟（high-resolution 3D hydrodynamical simulations）数据。

Method: 采用基于M82多相外流再现框架的高分辨率三维流体力学模拟（high-resolution 3D hydrodynamical simulations）方法。

Result: 低质量星系（$9.0 < \log(M_*/M_\odot) < 10.0$）在宇宙正午时期经历恒星爆发驱动的星系外流，其速度范围为50-1000 $\,\rm{km\,s}^{-1}$，质量外流率为0.3-20 M$_\odot \,\rm{yr}^{-1}$，质量加载因子（$η_\mathrm{M}$）为0.24-6.26。

Abstract: We investigate starburst-driven galactic outflows in low-mass galaxies ($9.0 < \log(M_*/M_\odot) < 10.0$) at cosmic noon using high-resolution 3D hydrodynamical simulations based on a framework that can reproduce the multiphase outflows in M82. The simulations produce starbursts lasting 20-30 Myr, with peak star formation rates of 2-68 M$_\odot \,\rm{yr}^{-1}$. Outflow properties vary strongly with time, radial distance to galaxy center, stellar mass, and gas fraction, exhibiting velocities of 50-1000 $\,\rm{km\,s}^{-1}$, mass outflow rates of 0.3-20 M$_\odot \,\rm{yr}^{-1}$, and mass loading factors, $η_\mathrm{M}$, of 0.24-6.26. The cool phase ($8000 < T \le 2 \times 10^4$ K) dominates the outflow, and properties of the cool and warm phases are broadly consistent with observations. At $M_*= 10^{9.5}\,M_\odot$, average $η_\mathrm{M}$ for the total, cool, and warm phases are $\sim$1.2, 0.75, and 0.25, respectively. The mass loading factor decreases with increasing galaxy stellar mass, but increases with star formation rate. Given strong temporal and spatial evolution, scaling slopes from limited samples should be treated with caution. Our total $η_\mathrm{M}$ values are higher than FIRE-2 by 0.06 dex but lower than EAGLE and TNG50 by 0.50 and 0.84 dex. Accounting for methodological differences in outflow measurement reduces these gaps to 0.2-0.4 dex, suggesting that part of the discrepancy between observations and simulations reported in the literature may arise from inconsistent definitions and measurement methods, though differences in individual phases persist. Larger observational and simulation samples, together with consistent methods for measuring outflow properties, are required to draw robust conclusions about the scaling relations of galactic outflows.

</details>

### [9] [K-DRIFT Science Theme: Galaxies in the Faint Universe](https://arxiv.org/abs/2602.08283)
*Woowon Byun,Yongmin Yoon,Jongwan Ko,Yun Hee Lee,Gain Lee,Ho Seong Hwang,Cristiano G. Sabiu,Kwang-il Seon,Kyungwon Chun,Jihye Shin,Jinsu Rhee,Jae-Woo Kim,Jaewon Yoo,Jaehyun Lee,Sang-Hyun Chun,Hong Soo Park,Soung-Chul Yang,Sungryong Hong,Jeehye Shin,Hyowon Kim*

**TL;DR**: 研制K-DRIFT望远镜突破光学波段超深测光极限，系统探测近邻星系低表面亮度（LSB）结构以揭示星系演化历史及低密度气体区域恒星形成（SF）特性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低表面亮度（LSB）结构在揭示星系质量组装历史中的作用及其对理解低表面气体密度区域恒星形成（SF）活动的意义。

Data: 数据来源于自主研发的新型望远镜K-DRIFT的观测数据，并结合其他巡天数据进行联合分析。

Method: 采用新型望远镜K-DRIFT进行超深光学波段测光观测，专注于探测近邻单个星系中的低表面亮度（LSB）结构。

Result: 研究开发新型望远镜K-DRIFT以突破现有观测极限，旨在系统探测近邻星系中的低表面亮度（LSB）结构及其恒星形成特性。

Abstract: Low-surface-brightness (LSB) structures serve as evidence of the intricate mass assembly of galaxies, and dedicatedly studying them promises to give us profound insights into the evolutionary history of galaxies. Furthermore, delving into the properties of star formation (SF) in the LSB regime can broaden our understanding of SF activity in regions characterized by low surface gas density, thereby shedding light on fundamental cosmic processes. However, systematic uncertainties may hamper the exploration of the LSB universe by limiting detectable SB levels. Indeed, despite dedicated advancements in telescope and observing techniques over decades, achieving ultra-deep photometric depths in optical wavelengths remains a formidable challenge. To overcome this challenge and explore the LSB universe that we have yet to see, we have been developing a novel telescope called K-DRIFT. This paper outlines the telescope's specification and describes various LSB features we aim for, explicitly focusing on nearby individual galaxies. To further advance the capabilities of the K-DRIFT survey, focused on LSB detection, we present several feasible research topics that utilize other survey data together and discuss the role of LSB observation in understanding the evolution of galaxies.

</details>

### [10] [CFHT MegaCam Two Deep Fields Imaging Survey (2DFIS) I: Overview](https://arxiv.org/abs/2602.08312)
*Binyang Liu,Wentao Luo,Martin Kilbinger,Shenming Fu,Ian Dell'Antonio,Liping Fu,Xian Zhong Zheng,Yi-fu Cai,Cheng Jia,Ning Jiang,Qinxun Li,Yicheng Li,Shurui Lin,Christopher J. Miller,Surhud S. More,Huiyuan Wang,Yibo Wang*

**TL;DR**: 2DFIS（Two Deep Fields Imaging Survey）利用CFHT望远镜对含重复快速射电暴（FRB）源和候选旋转星系团的两个天体物理区域进行深度成像（r~26星等），通过光学对应体搜索、环境特征分析和弱引力透镜质量分布测量，为FRB宿主环境、星系团质量重建及相关宇宙学研究提供高质量数据基础。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过宽视场成像巡天（2DFIS）探索快速射电暴（FRB）宿主环境特征和旋转星系团候选体的弱引力透镜质量分布。

Data: 数据来源于加拿大-法国-夏威夷望远镜(CFHT)进行的宽视场成像观测，使用MegaCam设备对包含重复快速射电暴(FRB)源和候选旋转星系团的两个天体物理区域进行深度达r~26星等的成像。

Method: 采用LSST科学流程（LSST Science Pipelines）并针对CFHT/MegaCam数据进行特定调整，实现原始曝光数据到科学就绪数据产品的完整处理流程，包括单历元校准图像、多波段叠加拼接图及详尽源表。

Result: 2DFIS（Two Deep Fields Imaging Survey）通过加拿大-法国-夏威夷望远镜（CFHT）对包含重复快速射电暴（FRB）源和候选旋转星系团的两个天体物理区域进行深度成像（r~26星等），为研究FRB光学对应体、环境特征及星系团弱透镜质量分布提供了高质量数据基础。

Abstract: We present the Two Deep Fields Imaging Survey (2DFIS), a wide-field imaging program conducted with the Canada-France-Hawaii Telescope (CFHT) targeting two astrophysically distinct regions: one containing a repeating fast radio burst (FRB) source and another hosting a candidate of a rotating galaxy cluster. Achieving a depth of r~26mag, the survey enables a search for faint optical counterparts and environmental signatures associated with the FRB, while high-quality photometric and galaxy shape measurements in the cluster field support a weak-lensing analysis of its mass distribution. This paper describes the observing strategy and data processing methodology adopted for 2DFIS, including the use of the LSST Science Pipelines with survey-specific adaptations for CFHT/MegaCam data. We outline a complete workflow for transforming raw CFHT exposures into science-ready data products, including calibrated single-epoch images, multi-band coadded mosaics, and extensive source catalogs. These data products provide the foundation for ongoing and future studies of FRB host environments, cluster mass reconstruction, and related cosmological applications.

</details>

### [11] [CFHT MegaCam Two Deep Fields Imaging Survey (2DFIS) II: Decoding the Lensing Profile of a "Rotating" Cluster with Deep CFHT Imaging](https://arxiv.org/abs/2602.08360)
*Yicheng Li,Liping Fu,Wentao Luo,Binyang Liu,Wei Du,Martin Kilbinger,Calum Murray,Christopher J. Miller,Ray Wang,David Turner,Lance Miller,Dezi Liu,Mario Radovich,Jean-Paul Kneib,Huanyuan Shan,Kaiwen Mai,Zicheng Wang,Haoran Zhao*

**TL;DR**: 通过对星系团RXCJ0110.0+1358（红移z=0.058）的多波段分析，发现其光学双峰分布实际由前景纤维结构投影造成，弱引力透镜测量揭示质量主要集中于东南区域（$\log M_{200}/M_\odot = 14.04_{-0.40}^{+0.24}$），排除了旋转系统的动力学可能性，并解释了光学与X射线观测的差异。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于星系团（galaxy cluster）RXCJ0110.0+1358的多波段观测数据与动力学特征之间的矛盾，探讨光学与X射线视角下结构差异的物理本质。

Data: 数据来源包括CFHT（加拿大-法国-夏威夷望远镜）深度成像、SDSS（斯隆数字巡天）测光数据、光谱红移数据以及XMM-牛顿X射线观测。

Method: 采用多波段（multi-wavelength）分析方法，结合CFHT成像、SDSS测光、光谱红移和XMM-牛顿X射线观测数据，并通过弱引力透镜（weak lensing）技术揭示质量分布。

Result: 弱引力透镜分析揭示质量主要集中于东南区域（$\log M_{200}/M_\odot = 14.04_{-0.40}^{+0.24}$），而西北子结构质量可忽略（$\sim 10^{13} M_\odot$），排除了该星系团为旋转系统的动力学可能性。

Abstract: We present a multi-wavelength analysis of the galaxy cluster RXCJ0110.0+1358 ($z=0.058$), a rotating cluster candidate, combining deep CFHT imaging, SDSS photometry, spectroscopic redshifts, and XMM-Newton X-ray observations. We find a notable discrepancy between the optical and X-ray views: while optical data reveal a pronounced bimodal galaxy distribution with significant kinematic substructure signatures, the X-ray emission exhibits a single, smoothly extended component centered on the BCG. Our weak lensing analysis resolves this discrepancy by revealing that the mass is predominantly concentrated in the southeast ($\log M_{200}/M_\odot = 14.04_{-0.40}^{+0.24}$), while the northwestern substructure has a negligible mass ($\sim 10^{13} M_\odot$). This immense mass disparity rules out the dynamical possibility of a rotating system. We demonstrate that the apparent optical bimodality arises from the projection of a filament, which led optical group-finding algorithms to misclassify these galaxies as cluster members. This contamination creates a spurious substructure that mimics a rotation signal and leads to an overestimation of the luminosity-based halo mass, resolving the observed inconsistencies.

</details>

### [12] [Stratification of the AGN-Driven multi-phase outflows in the dwarf Seyfert galaxy NGC 4395](https://arxiv.org/abs/2602.08447)
*Payel Nandi,Luis Colina,Rogemar A. Riffel,Miguel Pereira Santaella,C. S. Stalin,D. J. Saikia,Javier Alvarez-Marquez,Markus Kissler-Patig*

**TL;DR**: 通过多波段观测发现NGC 4395中心的中等质量黑洞(intermediate-mass black hole)驱动了分层双锥结构的冷/热分子气体与电离气体外流(outflow)，其中冷分子相的外流质量率比热相高1-2个量级，且低电离气体对星际介质(ISM)的影响最显著。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近邻矮赛弗特星系NGC~4395（含中等质量黑洞）中核区多相外流（电离气体、分子气体）的物理特性及其对星际介质（ISM）的反馈作用。

Data: 数据来源包括JWST/NIRSpec和MIRI IFU光谱仪(1.66--28.6μm)、ALMA CO(2--1)观测数据以及Gemini/GMOS数据。

Method: 采用多波段光谱分析（1.66-28.6 μm）结合H₂旋转/振动跃迁建模和精细结构线（fine-structure lines）探测，识别出580K、1480K、2900K三个温/热分子组分及ALMA CO(2-1)示踪的冷分子相（<50K）。

Result: 在NGC~4395中探测到冷分子气体（ALMA CO(2-1)）和暖/热分子气体（H$_2$旋转/振动跃迁）以及电离气体（H\,\textsc{i}和精细结构线）的多相外流，其中冷分子气体的质量外流率比暖/热分子和电离相高1-2个数量级。

Abstract: We present a multi-wavelength study of nuclear outflows in the nearby dwarf Seyfert galaxy NGC~4395, which hosts an intermediate-mass black hole. Using \textit{JWST}/NIRSpec and MIRI IFU spectroscopy (1.66--28.6~$μ$m), together with ALMA and Gemini/GMOS data, we probe the ionised and molecular gas on parsec scales. The JWST nuclear spectra reveal 134 emission lines, including H\,\textsc{i}, He, numerous fine-structure lines, H$_2$ rotational/ro-vibrational transitions, and several PAH bands. Modelling of the H$_2$ rotational lines reveals three warm/hot molecular components ($T\!\approx\!580$, 1480, and 2900~K), along with a cold ($<50$~K) phase traced by ALMA CO(2--1). Outflow signatures are detected in cold and warm/hot molecular gas, in H\,\textsc{i}, and in 36 fine-structure lines spanning ionisation potentials of 7.6--300~eV. Ionised outflow velocities range from 127 to 716~km\,s$^{-1}$, with blueshifted and redshifted components consistent with a stratified biconical geometry. The cold molecular gas shows a mass outflow rate nearly 1--2 orders of magnitude larger than that of the warm/hot molecular and ionised phases. The kinetic coupling efficiency is 0.003--0.12\% for the coronal-line gas and 0.4--1.4\% for the H\,\textsc{i} outflow, indicating that only the low-ionisation gas significantly impacts the surrounding ISM. Outflow velocity and the fraction of flux in the outflowing component increase with ionisation potential, implying that the most highly ionised gas originates closest to the AGN and is most efficiently accelerated.

</details>

### [13] [GaLactic and Extragalactic All-sky Murchison Widefield Array survey eXtended (GLEAM-X) III: Galactic Plane](https://arxiv.org/abs/2602.08475)
*S. Mantovanini,N. Hurley-Walker,K. Ross,S. W. Duchesne,G. Anderson,T. J. Galvin*

**TL;DR**: GLEAM-X第三次数据发布提供了南银河系平面（southern Galactic Plane）3800平方度、频率72-231 MHz的射电图像和98,207个射电源的星表，结合GLEAM观测实现了45角秒至15度的空间尺度灵敏度，在170-231 MHz频段噪声水平为3-6 mJy/束，位置精度达1角秒，星表在特定区域50 mJy和125 mJy流量限下分别具有90%完备性，整体可靠性达99.3%。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用扩展配置的默奇森宽场阵列(MWA)开展南天银河系平面(GP)低频(72-231 MHz)射电连续谱巡天(GLEAM-X)，旨在填补该区域高灵敏度、宽空间尺度覆盖的观测空白。

Data: 数据来源于GLEAM-X巡天第三次数据发布（GLEAM-X DR3）和先前GLEAM巡天的观测数据，使用MWA（Murchison Widefield Array）望远镜的扩展Phase-II配置（基线范围约12米至5公里），覆盖南天银河系平面（Galactic Plane）3800平方度区域（233° < l < 44°，|b| < 11°），频率范围72-231 MHz分为20个子波段。

Method: 采用扩展相位II配置的默奇森宽场阵列(MWA)进行观测，结合先前GLEAM巡天数据，在72-231 MHz频段内分成20个子波段，实现45''至15'空间尺度敏感度覆盖。

Result: GLEAM-X第三次数据发布覆盖南银河面（Galactic Plane）3800平方度区域（233° < l < 44°，|b| < 11°），在72-231 MHz频段内划分为20个子带，结合MWA扩展相位II配置（基线12米至5千米）与先前GLEAM巡天数据，实现了45角秒至15度空间尺度敏感度优化。

Abstract: We present the third data release for the Galactic and Extragalactic All-Sky Murchison Widefield Array eXtended (GLEAM-X) survey, covering = 3800 deg2 of the southern Galactic Plane (GP) with \ang{233} < l < \ang{44} and |b| < \ang{11} across a frequency range of 72 - 231 MHz divided into 20 sub-bands. GLEAM-X observations were taken using the "extended" Phase-II configuration of the Murchison Widefield Array (MWA), which features baselines ranging from approximately 12 m to 5 km. This configuration limits sensitivity to the diffuse structure of the GP, with an angular resolution range of about 45'' to 2'. To achieve lower noise levels while being sensitive to a wide range of spatial scales (45''- \ang{15}), we combined these observations with the previous Galactic and Extragalactic All-Sky Murchison Widefield Array (GLEAM) survey. For the area covered, we provide images spanning the whole frequency range. A wide-band image over 170 - 231 MHz, with RMS noise of = 3 - 6 mJy/beam and source position accuracy within 1 arcseconds, is then used to perform source-finding, which yields 98,207 elements measured across 20 x 7.68 MHz frequency bands. The catalogue is 90% complete at 50 mJy within \ang{233} < l < \ang{324} and at 125 mJy in \ang{290} < l < \ang{44}, while it is 99.3% reliable overall. All the images and the catalogue are available online for download.

</details>

### [14] [Modeling Tidal Disruption Events and Compact Object Plunges in Nuclear Star Clusters](https://arxiv.org/abs/2602.08574)
*Philip Cho,Kai Wu,Francesco Flammini Dotti,Taras Panamarev,Rainer Spurzem*

**TL;DR**: 该研究通过改进的STARDISK/NBODY6++GPU模型模拟核星团(NSCs)中潮汐瓦解事件(TDEs)和致密天体螺旋入轨过程，发现TDEs吸积在最初2 Myr达到峰值且随初始超大质量黑洞(SMBH)质量增加而更快衰减（约半数瓦解物质被吸积），而致密天体吸积主要发生在4-27倍史瓦西半径的近心轨道且应用螺旋入轨标准后吸积率降低一个量级。


<details>
  <summary>Details</summary>

Background: 研究探讨了核星团(NSCs)中潮汐瓦解事件(TDEs)和致密天体螺旋对中央超大质量黑洞(SMBH)质量增长的贡献机制。

Data: 使用直接N体代码NBODY6++GPU的STARDISK版本进行模拟，改进了潮汐瓦解事件（TDE）的质量回落模型和基于轨道衰减时标的致密天体（compact object）坠落模型。

Method: 采用改进的恒星潮汐瓦解事件（TDEs）质量回落模型和基于轨道衰减时标的致密天体（compact objects）坠入模型，利用NBODY6++GPU的直接N体模拟代码STARDISK版本进行数值模拟。

Result: 潮汐瓦解事件(TDEs)导致的吸积质量在前2 Myr达到峰值且随初始超大质量黑洞(SMBH)质量增加而更快下降，约半数瓦解恒星物质被吸积。

Abstract: We study tidal disruption events (TDEs) and compact object inspirals in nuclear star clusters (NSCs) hosting a central supermassive black hole (SMBH), focusing on their role in SMBH growth. Using the STARDISK version of the direct N-body code NBODY6++GPU, we perform pilot simulations with two improved models: one for mass fallback from TDEs and another for compact object plunges based on orbital decay timescales. Our results show that mass accretion via TDEs peaks within the first 2 Myr and decreases more rapidly for higher initial SMBH masses, with roughly half the disrupted stellar debris being accreted. Compact object accretion is confined mostly to orbits with pericenters between 4 and 27 Schwarzschild radii and is suppressed by an order of magnitude when inspiral criteria are applied.

</details>

### [15] [An unusual pair of interstellar HI features and a related white dwarf star inside the HI cavity surrounding the Upper Sco-Cen OB2 Association](https://arxiv.org/abs/2602.08756)
*Gerrit L. Verschuur*

**TL;DR**: 在针对一颗暗弱白矮星方向的高分辨率21厘米(HI)巡天数据中，发现了一对速度分别为+12和-6 km/s的神秘中性氢(HI)结构，推测其为约30万年前的行星状星云(planetary nebula)冷却后的中性残留物，因其在星际物质稀薄空腔中膨胀而保留了原始形态特征。


<details>
  <summary>Details</summary>

Background: 研究探讨了在星际氢原子（HI）中发现的未解结构及其与上蝎-半人马座OB2星协（Upper Sco-Cen OB2 Association）周围大空腔的潜在关联，推测这些结构可能是古老超新星或行星状星云（planetary nebula）的遗迹。

Data: 数据来源于高分辨率21厘米（21-cm）巡天观测数据。

Method: 采用高分辨率21厘米（21-cm）谱线观测数据并结合形态学分析，研究了星际氢原子（HI）结构的空间分布与演化特征。

Result: 在+12和-6 km/s速度处发现的两个未解析氢原子（HI）结构可能是约3×10^5年前的行星状星云（planetary nebula）冷却后的中性气体残余，其形态特征得以保留是因为膨胀进入了星际氢原子（interstellar HI）稀少的空腔区域。

Abstract: Two mysterious unresolved HI structures at velocities of +12 and -6 km/s were discovered in high resolution 21-cm survey data in the direction of a faint white dwarf star. Examination of the HI morphology in this area of sky shows that the star and HI features exists in a large cavity in interstellar HI surrounding the Upper Sco-Cen OB2 Association. The cavity may have been created by an ancient supernova. It is hypothesized that the pair of HI features and filamentary HI structure found in its immediate vicinity may be the remnants of a planetary nebula some 3 x 10^5 years old that have cooled to the point that the gas is neutral and emitting the 21-cm spectral line. This remnant has maintained the morphological characteristics of the original planetary nebula because it expanded into a volume of space relatively devoid of interstellar gas that would otherwise have absorbed any traces of the original nebula.

</details>

### [16] [CHIMPS2: The physical properties and star formation efficiency of molecular gas in the Central Molecular Zone](https://arxiv.org/abs/2602.08824)
*S. M. King,T. J. T. Moore,S. N. Longmore,D. J. Eden,J. D. Henshaw,A. J. Rigby,R. Rani*

**TL;DR**: 基于新的$^{12}$CO $J=2\to1$观测数据结合多波段分子谱线，发现银河系中心分子区(CMZ)的$^{13}$CO中位激发温度(T_ex)为11K，Sgr B1/B2复合体峰值超120K，H_2柱密度(N(H2))中位值为2×10^22 cm^-2，总气体质量(M_gas)为7×10^6 M_⊙，并揭示70-μm和160-500-μm波段显示的恒星形成效率(SFE)空间分布差异暗示了从冷密气体到原恒星的演化梯度，表明CMZ可能进入更活跃的恒星形成阶段。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中央分子区（Central Molecular Zone, CMZ）的分子气体物理特性及恒星形成效率（star formation efficiency, SFE），探讨其在局部热力学平衡（Local Thermodynamic Equilibrium, LTE）条件下的演化动态与未来恒星形成潜力。

Data: 数据来源包括詹姆斯·克拉克·麦克斯韦望远镜（James Clerk Maxwell Telescope）的新$^{12}$CO $J=2\to1$观测数据、CHIMPS2项目的$^{12}$CO和$^{13}$CO $J=3\to2$数据、SEDIGISM项目的$^{13}$CO $J=2\to1$数据，以及Hi-GAL项目在70-$μm$和160--500-$μm$波段探测到的致密源（compact sources）。

Method: 采用局部热力学平衡（LTE）模型结合$^{12}$CO和$^{13}$CO分子谱线观测数据估算中央分子区（CMZ）的激发温度（$T_{\rm ex}$）、H$_{2}$柱密度（$N(\mathrm{H}2)$）及气体质量（$M_{\rm gas}$）。

Result: 在银河系中心分子带（CMZ）中，70微米亮星形成效率（SFE）整体较低但在人马座B1/B2复合体、拱星团和人马座C区域较高，而160-500微米SFE在尘埃脊云和人马座C周围负经度区域更广泛增强，表明存在从冷密气体到原恒星的演化梯度。

Abstract: We present Local Thermodynamic Equilibrium (LTE) estimates of the physical properties and star formation efficiency (SFE) of molecular gas in the Central Molecular Zone (CMZ), using new $^{12}$CO $J=2\to1$ observations from the James Clerk Maxwell Telescope. Combined with CHIMPS2 $^{12}$CO and $^{13}$CO $J=3\to2$, and SEDIGISM $^{13}$CO $J=2\to1$ data, we estimate a median excitation temperature of $T_{\rm ex} = 11$K for $^{13}$CO throughout the CMZ, with peaks exceeding $120$K in the Sgr B1/B2 complex. Cooler gas dominates around Sgr A and nearby clouds. We derive a median H$_{2}$ column-density of $N(\mathrm{H}2) = 2 \times 10^{22}$ cm$^{-2}$ and a total $^{13}$CO-traced gas mass of $M_{\rm gas} = 7 \times 10^6$ M$_\odot$, consistent with previous estimates when accounting for spatial coverage. The instantaneous SFE is assessed using Hi-GAL compact sources detected at 70-$μm$ and 160--500-$μm$. The 70-$μm$-bright SFE, tracing current star formation, is modest overall but elevated in Sgr B1/B2, the Arches cluster, and Sgr C. In contrast, the 160--500-$μm$ SFE, tracing cold pre-stellar gas, is more broadly enhanced, particularly in the dust ridge clouds and towards negative longitudes surrounding Sgr C. The contrasting distributions suggest an evolutionary gradient in SFE, consistent with a transition from dense, cold gas to embedded protostars. Our results imply that the CMZ may be enter a more active phase of star formation, with large reservoirs of gas primed for future activity.

</details>

### [17] [The contribution of neutral gas to Faraday tomographic data at low frequencies. A first extensive comparison between real and synthetic data](https://arxiv.org/abs/2602.08839)
*Jack Berat,Marc-Antoine Miville-Deschênes,Andrea Bracco,Patrick Hennebelle,Jeremy Scholtys*

**TL;DR**: 研究发现低频偏振观测能有效探测太阳系附近多相星际介质（ISM）的磁场形态，揭示了冷中性介质（CNM）与法拉第层析结构的相关性受湍流水平、磁场方向和观测噪声影响，但与CNM丰度关系较弱。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用低频偏振观测探究多相（multi-phase）太阳邻域星际介质（ISM）中磁场形态与中性氢（HI）细丝结构的关联性。

Data: 研究使用了LOFAR在米波段的观测数据，并基于50 pc尺度、不同湍流和压缩水平的磁流体动力学（MHD）模拟生成合成21 cm和同步辐射观测数据，同时考虑了仪器噪声和波束效应。

Method: 采用磁流体力学（MHD）模拟热双稳态中性星际介质（ISM）并结合不同冷中性介质（CNM）比例，通过MOOSE代码生成合成同步辐射偏振和法拉第层析数据，并基于HOG算法开发量化指标评估冷/暖中性介质结构对法拉第层析数据的贡献。

Result: 在100-200 MHz频段，中性氢（HI）相相关的热电子对同步辐射偏振发射有显著贡献，且冷中性介质（CNM）与法拉第层析结构的关联程度受湍流水平、磁场方向和观测噪声影响，但与CNM比例关系较弱。

Abstract: LOFAR observations of diffuse interstellar polarization at meter wavelengths reveal intricate polarized intensity structures with an unexpected correlation with neutral HI filaments that could not be reproduced in simulations with low cold neutral medium (CNM) abundance. We investigate whether MHD simulations of thermally bi-stable neutral interstellar medium, with a range of CNM fraction, can reproduce the properties of the 3C196 field, the high Galactic latitude test field. Using 50 pc simulations with varying levels of turbulence and compressibility, we generated synthetic 21 cm and synchrotron observations, including instrumental noise and beam effects, for different line-of-sight orientations relative to the magnetic field. We developed MOOSE, a code to generate synthetic synchrotron polarization and Faraday tomography. We also developed a metric based on the HOG algorithm, to quantify the relative contribution of cold and warm neutral medium structures to the Faraday tomographic data. The synthetic observations show levels of polarization intensity and RM values comparable to the 3C196 field, indicating that thermal electrons associated with the neutral HI phase can account for a significant fraction of the synchrotron polarized emission at 100-200 MHz. The simulations consistently reveal a correlation between CNM and Faraday tomographic structures that depends on turbulence level, magnetic field orientation, and observational noise, but only weakly on CNM fraction. We found slightly weaker CNM-Synchrotron polarized emission correlation level than observed in the 3C196 field. These results suggest that low-frequency polarimetric observations provide a valuable probe of magnetic-field morphology in the multi-phase Solar-neighborhood ISM, while simultaneously underscoring the need for improved modeling of the turbulent, multi-phase, and partially ionized interstellar medium.

</details>

<div id='astro-ph.CO'></div>

# astro-ph.CO [[Back]](#toc)

### [18] [AMICO galaxy clusters in KiDS-1000: Splashback radius from weak lensing and cluster-galaxy correlation function](https://arxiv.org/abs/2602.07114)
*G. F. Lesci,C. Giocoli,F. Marulli,M. Romanello,L. Moscardini,M. Sereno,M. Maturi,M. Radovich,G. Castignani,H. Hildebrandt,L. Ingoglia,E. Puddu*

**TL;DR**: 基于Kilo Degree Survey（KiDS）第四期数据中9049个星系团的叠加弱引力透镜（$g_{\rm t}$）和星系-星系团互相关函数（$w_{\rm cg}$）测量，研究发现溅落半径（$r_{\rm sp}$）与暗物质晕质量吸积率（$Γ$）及峰值高度（$ν_{200\rm m}$）的关系符合Λ冷暗物质模型预测，且$w_{\rm cg}$结合质量-富度关系约束使溅落半径测量精度达10%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用AMICO星系团样本分析溅落半径（splashback radius）及其与暗物质晕分布的关系，以验证Λ-冷暗物质（Λ-cold dark matter）模型预测。

Data: 数据来源于千度巡天（Kilo Degree Survey，KiDS）第四版数据发布的AMICO（Adaptive Matched Identifier of Clustered Objects）星系团样本，包含9049个红移范围$z\in[0.1,0.8]$的富星系团，其中8730个具有剪切测量数据。

Method: 采用堆叠弱引力透镜（reduced shear）和星系-星系团互相关函数（cluster-galaxy correlation function）联合建模方法，在径向范围$R\in[0.4,5]$ $h^{-1}$Mpc内约束暗物质晕溅落半径（splashback radius）及其与质量吸积率（mass accretion rate）和峰值高度（peak height）的关系。

Result: 通过分别建模切变（shear）和星系-星系团相关函数（cluster-galaxy correlation function），在径向范围$R\in[0.4,5]$ $h^{-1}$Mpc内约束了溅落半径（splashback radius）、质量吸积率（mass accretion rate）以及$\mathcal{R}_{\rm sp}\equiv r_{\rm sp}/r_{200\rm m}$与峰高（peak height）的关系，结果与Λ冷暗物质（Λ-cold dark matter）模型预测一致。

Abstract: We present the splashback radius analysis of the Adaptive Matched Identifier of Clustered Objects (AMICO) galaxy cluster sample in the fourth data release of the Kilo Degree Survey (KiDS). The sample contains 9049 rich galaxy clusters within $z\in[0.1,0.8]$, with shear measurements available for 8730 of them. We measure and model the stacked reduced shear, $g_{\rm t}$, and the cluster-galaxy correlation function, $w_{\rm cg}$, in bins of observed intrinsic richness, $λ^*$, and redshift, $z$. Building on the methods employed in recent cosmological analyses, we model the average splashback radius, $r_{\rm sp}$, of the underlying dark matter halo distribution, accounting for the known systematic uncertainties affecting measurements and theoretical models. By modelling $g_{\rm t}$ and $w_{\rm cg}$ separately, in the cluster-centric radial range $R\in[0.4,5]$ $h^{-1}$Mpc, we constrain $r_{\rm sp}$, the mass accretion rate, $Γ$, and the relation between $\mathcal{R}_{\rm sp}\equiv r_{\rm sp}/r_{200\rm m}$ and the peak height, $ν_{200\rm m}$, over the mass range $M_{200\rm m}\in[0.4,20]$ $10^{14}h^{-1}$M$_\odot$. The two probes provide consistent results that also agree with $Λ$-cold dark matter model predictions. Our $\mathcal{R}_{\rm sp}$ constraints are consistent with those from previous observations. For $g_{\rm t}$ and $w_{\rm cg}$, we achieve a precision of 14% and 10% per cluster stack, respectively. The higher precision of $w_{\rm cg}$, enabled by its combination with weak-lensing constraints on the mass-richness relation, highlights the complementarity of lensing and clustering in measuring $r_{\rm sp}$ and constraining the properties of the infalling material region.

</details>

### [19] [A General Formulation of the Kinematic Dipole as a Functional of Selection and Source Properties: Beyond the Ellis--Baldwin Approximation](https://arxiv.org/abs/2602.07389)
*Tsutomu T. Takeuchi*

**TL;DR**: 论文提出了一种统一的理论框架，通过引入泛函$\mathcal{A}[\mathcal{W},f]$描述运动学偶极（kinematic dipole）振幅，修正了传统Ellis-Baldwin公式在非理想观测条件下的局限性，并阐明了理论系数与实测偶极向量的关系。


<details>
  <summary>Details</summary>

Background: 研究主题为探讨运动观测者引起的星系和类星体（QSO）数量分布的偶极各向异性（运动偶极子），以检验宇宙学各向同性并与宇宙微波背景（CMB）偶极子进行比较。

Data: data部分：研究使用了多波段测光（multi-band photometry）和光度红移（photo-z）选择等观测数据，考虑了非理想效应如多样化的光谱能量分布（SEDs）、有限仪器带通（instrumental bandpasses）和非幂律数计数（non-power-law number counts）。

Method: 采用广义多维度选择函数（multi-dimensional selection function）和母体（parent population）的多普勒响应（Doppler response）功能形式$\mathcal{A}[\mathcal{W},f]$作为核心分析方法，统一处理非理想效应（non-ideal effects）并推广经典Ellis-Baldwin公式。

Result: 运动观测者引起的星系和类星体（QSO）数量偶极各向异性（运动学偶极）的振幅由选择函数作用于底层群体的多普勒响应泛函$\mathcal{A}[\mathcal{W},f]$决定，而非单一指数描述。

Abstract: The dipole anisotropy in galaxy and QSO number counts induced by the motion of the observer (the kinematic dipole) provides an important test of cosmological isotropy and a comparison with the Cosmic Microwave Background (CMB) dipole. Traditionally, the Ellis \& Baldwin expression,$\mathcal{A}=2+x(1+α)$, has been widely adopted, assuming power-law number counts and a single power-law spectral energy distribution (SED). Realistic surveys, however, involve a range of non-ideal effects, including diverse SEDs, finite instrumental bandpasses, non-power-law number counts, multi-band photometry and photo-$z$ selections, and direction-dependent or stochastic detection limits. In this paper, we incorporate these effects explicitly at the theoretical level and present a unified formulation of the kinematic dipole for a general parent population and a general multi-dimensional selection function. We show that the dipole amplitude is not described by a single index, but is instead given by a functional, $\mathcal{A}[\mathcal{W},f]$, defined as the Doppler response of the selection function acting on the underlying population. We demonstrate that the classical Ellis--Baldwin result is recovered as a special limiting case of this formalism, and clarify the relation between the theoretical coefficient $\mathcal{A}$ and the dipole vector estimated from finite catalogs, separating theoretical response from statistical uncertainty. This framework provides a basis for reinterpreting reported discrepancies in kinematic dipole measurements across surveys and is directly applicable to future wide-area, multi-band observations.

</details>

### [20] [Prospective bounds on f(Q) gravity with pulsar timing arrays](https://arxiv.org/abs/2602.07557)
*Mohammadreza Davari,Alireza Allahyari*

**TL;DR**: 通过分析纳赫兹频段随机引力波背景（SGWB）数据，研究发现对称远平行$f(Q)$引力理论中张量模式振幅的阻尼参数$n$与广义相对论（GR）基本一致但存在微小偏差，未来平方公里阵列（SKA）观测将显著提高约束精度以区分二者。


<details>
  <summary>Details</summary>

Background: 研究探讨了纳赫兹频段随机引力波背景（SGWB）对对称远平行$f(Q)$引力理论的启示，该理论通过非度量标量$f(Q)$引入与广义相对论（GR）的偏差。

Data: 数据来源包括NANOGrav 15年数据集和国际脉冲星计时阵（IPTA）第二次数据发布，并预测未来平方公里阵列（SKA）观测站将显著提升约束精度。

Method: 采用模型无关的参数化方法并推导张量模式传递函数的解析近似，以获取原初暴胀引力波的谱能量密度。

Result: 对称远平行（symmetric teleparallel）$f(Q)$引力理论中张量模式传播速度为光速，但振幅演化过程中存在修正阻尼效应，与NANOGrav 15年和IPTA第二次数据对比显示阻尼参数$n$与广义相对论（GR）基本一致但仍存在微小可观测偏差。

Abstract: Pulsar timing arrays (PTAs) have recently provided compelling evidence for a stochastic gravitational wave background (SGWB) in the nanohertz frequency band, offering a unique window into fundamental physics. Here, we explore implications for symmetric teleparallel $f(Q)$ gravity, a theory in which deviations from General Relativity (GR) arise through the non-metricity scalar $f(Q)$. Crucially, tensor modes propagate at the speed of light in this framework. However, their amplitude undergoes a modified damping during their evolution. We adopt a model-independent parameterization and derive an analytic approximation to the tensor mode transfer function to obtain the spectral energy density of primordial inflationary gravitational waves. Comparison with the NANOGrav 15-year and IPTA second data releases show that the inferred damping parameter $n$ remains consistent with GR, yet allows small deviations that could be observable. We then conduct a Fisher information matrix forecasts which demonstrate that the Square Kilometre Array (SKA) observatory will improve these constraints by several orders of magnitude, offering the potential to distinguish $f(Q)$ gravity from GR with high precision. These results highlight PTAs as powerful probes of non-metricity-based modifications to gravity.

</details>

### [21] [Gravitational Wave Informed Inference of 21-cm Global Signal Parameters](https://arxiv.org/abs/2602.07631)
*Avinash Tiwari,Sajad A. Bhat,Tirthankar Roy Choudhury,Susmita Adhikari,Mukesh Kumar Singh,Shasvath J. Kapadia*

**TL;DR**: 该论文提出利用下一代引力波探测器观测到的双黑洞（BBH）并合率作为红移范围∼13-25内恒星形成率密度（SFRD）的示踪指标，通过多信使框架打破21厘米氢信号参数推断中的简并性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用下一代引力波探测器观测到的双黑洞并合事件作为独立观测探针，通过多信使框架解决宇宙黎明时期21厘米氢信号参数推断中的简并性问题。

Data: 数据来源包括下一代引力波探测器（如Cosmic Explorer）观测到的红移范围约13-25的双黑洞（BBH）并合事件，以及中性氢21厘米超精细跃迁吸收的宇宙微波背景光子信号。

Method: 采用下一代引力波探测器（如Cosmic Explorer）观测双黑洞（BBH）并合率作为红移范围∼13-25内恒星形成率密度（SFRD）的示踪，结合21厘米氢信号参数推断的多信使框架来打破参数简并。

Result: 下一代引力波探测器观测的双黑洞并合事件可作为红移范围~13-25内恒星形成率密度(SFRD)的示踪剂。

Abstract: Understanding how and when the first stars and galaxies formed remains one of the central challenges in modern cosmology. These structures emerged during the transition from the Dark Ages to the Cosmic Dawn, a period that remains observationally unconstrained despite strong theoretical progress. During this epoch, neutral hydrogen absorbed a fraction of cosmic microwave background photons through its 21-cm hyperfine transition, producing a 21-cm absorption signal whose evolution encodes the early Universe's thermal and ionization history. However, extracting the underlying astrophysical parameters from this signal is limited by severe parameter degeneracies, which cannot be resolved without independent observational probes. The next-generation gravitational wave (GW) detectors, such as Cosmic Explorer (CE), will observe binary black hole (BBH) mergers up to very large redshifts and hence will detect a fraction of them formed within the redshift range $\sim 13-25$. The merger rate of these BBHs will depend on the star formation rate density (SFRD) at these redshifts, together with the BBH formation efficiency and a time delay distribution. Therefore, the merger rate of these BBHs can work as a tracer of the SFRD in the redshift range $\sim 13-25$. In this Letter, we establish a novel multi-messenger framework and present a proof-of-principle concept of how the observations of BBH mergers form next-generation GW detectors can improve the inference of parameters generating the 21-cm cosmic hydrogen signal, and help break degeneracies between them.

</details>

### [22] [Cosmology with one galaxy: An analytic formula relating $Ω_{\rm m}$ with galaxy properties](https://arxiv.org/abs/2602.07651)
*Kito Liao,Francisco Villaescusa-Navarro,Romain Teysser,Natalí S. M. de Santi*

**TL;DR**: 首次提出了一种基于单个星系内禀观测量的解析宇宙学示踪方法，通过符号回归从流体动力学模拟中识别出由引力势深度调控的重子保留与富集效率的紧凑函数关系，直接关联物质密度参数（Ω_m）与星系尺度物理特性，为小尺度星系物理与大尺度宇宙学建立了可解释的联系。


<details>
  <summary>Details</summary>

Background: 该研究探讨了通过星系尺度观测（如重子保留和富集效率）直接提取宇宙学参数（如物质密度参数Ωm）的可能性，建立了小尺度星系物理与大尺度宇宙学之间的直接联系。

Data: 数据来源于CAMELS项目的多套先进流体力学模拟（IllustrisTNG、ASTRID、SIMBA和Swift-EAGLE）。

Method: 采用符号回归（symbolic regression）技术分析CAMELS项目的高精度流体动力学模拟数据，识别出将物质密度参数(Ω_m)与星系尺度可观测量直接关联的紧凑函数形式。

Result: 首次发现并验证了可直接从单个星系的内禀观测特征中提取宇宙学参数Ω_m的解析关系式，揭示了星系引力势深度调控的重子物质保留与金属增丰效率是Ω_m在星系尺度上局部编码的物理机制。

Abstract: Standard cosmological analyses typically treat galaxy formation and cosmological parameter inference as decoupled problems, relying on population-level statistics such as clustering, lensing, or halo abundances. However, classical studies of baryon fractions in massive galaxy clusters have long suggested that gravitationally bound systems may retain cosmological information through their baryonic content. Building on this insight, we present the first analytic and physically interpretable cosmological tracer that links the matter density parameter, $Ω_m$, directly to intrinsic galaxy-scale observables, demonstrating that cosmological information can be extracted from individual galaxies. Using symbolic regression applied to state-of-the-art hydrodynamical simulations from the CAMELS project, we identify a compact functional form that robustly recovers $Ω_m$ across multiple simulation suites (IllustrisTNG, ASTRID, SIMBA, and Swift-EAGLE), requiring only modest recalibration of a small number of coefficients. The resulting expression admits a transparent physical interpretation in terms of baryonic retention and enrichment efficiency regulated by gravitational potential depth, providing a clear explanation for why $Ω_m$ is locally encoded in galaxy properties. Our work establishes a direct, interpretable bridge between small-scale galaxy physics and large-scale cosmology, opening a complementary pathway to cosmological inference that bypasses traditional clustering-based statistics and enables new synergies between galaxy formation theory and precision cosmology.

</details>

### [23] [Scalar-Induced Gravitational Waves and Primordial Black Holes from a Localized Bump or Dip Feature in a Single-Field Inflationary Potential](https://arxiv.org/abs/2602.07951)
*Xiang Zhang,Zhao-Huan Yu*

**TL;DR**: 在单场暴胀模型中引入局域势能隆起或凹陷特征，通过减缓慢滚暴胀子运动显著增强原初曲率功率谱峰值，从而产生可观测的随机引力波背景并形成大量与当前观测限制相容的原初黑洞(Primordial Black Holes)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于单场暴胀（single-field inflation）模型中具有局域隆起或凹陷特征的势能所诱导的原初引力波（scalar-induced gravitational waves）和原初黑洞（primordial black holes）的生成机制。

Data: 数据来源于单场暴胀（single-field inflation）模型中具有局部隆起或凹陷特征的势能所驱动的数值模拟（simulation）。

Method: 采用单场暴胀（single-field inflation）模型，在势能中引入局域化凸起或凹陷特征，通过减缓慢滚（slow-roll）暴胀子运动放大原初曲率功率谱峰值。

Result: 在单场暴胀模型中引入局域势垒或势阱特征可显著增强原初曲率功率谱，从而产生可观测的随机引力波背景并促进原初黑洞（primordial black holes）的大量形成。

Abstract: We study the production of scalar-induced gravitational waves and primordial black holes in a single-field inflation model with a localized bump or dip feature in the potential. Introducing such a localized feature temporarily decelerates the slow-roll inflaton, amplifying the primordial curvature power spectrum into a sharp peak. Consequently, this enhancement sources a significant stochastic background of gravitational waves and leads to abundant formation of primordial black holes. Through eight benchmark cases, we show that the predicted abundances of primordial black holes can remain compatible with current observational limits, while the corresponding gravitational wave spectra peaking across a wide range of frequencies are accessible to future gravitational wave experiments in multiple observational bands.

</details>

### [24] [Self-resonance preheating in deformed attractor models: oscillon formation and evolution](https://arxiv.org/abs/2602.07972)
*Bao-Min Gu,Yu-Peng Zhang,Fu-Wen Shu,Yu-Xiao Liu*

**TL;DR**: 研究发现，在具有高斯特征（Gaussian feature）的变形α吸引子（α-attractor）T模型中，势能特征参数h显著改变了共振带和弗洛凯图（Floquet charts），导致产生更多更小且寿命更短的振荡子（oscillons），并抑制了引力波高频尾端发射。


<details>
  <summary>Details</summary>

Background: 研究聚焦于具有高斯特征变形的α-吸引子T-模型（α-attractor T-model）中自共振预加热阶段暴胀子凝聚体碎裂为振荡子（oscillon）的动力学及其宇宙学影响。

Data: 数据来源为非线性晶格模拟（nonlinear lattice simulations）和线性分析（linear analysis）。

Method: 采用线性分析和非线性晶格模拟（nonlinear lattice simulations）方法研究高斯特征（Gaussian feature）对自共振（self-resonance）和振荡子（oscillons）演化的影响。

Result: 具有高斯特征的变形α吸引子T模型中，势能特征参数h显著改变了共振带和弗洛凯图（Floquet chart），导致产生更多数量但体积更小的振荡子（oscillon），且其能量占比和寿命随h增大而系统性降低。

Abstract: It is well known that, in potentials that are quadratic near the minimum but shallower away, such as small-$α$ ($\ll M_P^2$) attractors, the inflaton condensate fragments into localized compact objects known as oscillons during self-resonance preheating. In this work we investigate the self-resonance in deformed $α$-attractor T-model with a Gaussian feature near the minimum, distant from inflation's end. Linear analysis reveals altered resonance bands and deformed Floquet charts dependent on feature parameters. In fully nonlinear lattice simulations, we find that the gradient energy transfer is largely independent of the potential feature parameter $h$. In contrast, after resonance terminates, the subsequent evolution of gradient energy becomes strongly dependent on $h$. Statistical analysis reveals that models with the potential feature produce larger number of smaller oscillons, with a reduced energy stored in these objects, increasingly suppressed as the magnitude of $h$ grows. By tracking the total energy and the gradient energy contained in oscillons, we find that in models with nonzero $h$ oscillons are systematically shorter-lived, with this effect strengthening for larger $h$. The gravitational wave emission is dominated by the resonance stage and is strongly suppressed once oscillons form. Potential features leave the low-frequency spectrum largely unchanged but significantly modify the high-frequency tail. Although a complete reheating description requires external couplings and higher-resolution simulations, clear qualitative differences of cosmic expansion history already emerge within our simulated time window. These results highlight the important role of potential features in shaping reheating dynamics and their cosmological implications, and provide a deeper understanding of preheating dynamics and the properties of oscillons.

</details>

### [25] [Complementary Roles of Distance and Growth Probes in Testing Time-Varying Dark Energy](https://arxiv.org/abs/2602.08207)
*Seokcheon Lee*

**TL;DR**: 该论文通过信息矩阵特征值分析揭示了距离和哈勃膨胀率观测数据对暗能量（dark energy）状态方程时间变化的敏感性存在固有局限，而结构增长（growth）测量能提供独立信息方向，并阐明了不同精度下增长数据对提升暗能量动力学约束的关键作用。


<details>
  <summary>Details</summary>

Background: 宇宙膨胀历史和暗能量（dark energy）性质的研究依赖于距离测量（distance measurements）对哈勃率（Hubble rate）累积视线积分的观测约束。

Data: 数据来源包括模拟生成的受控模拟数据(mock data)和代表下一代实验的类巡天配置(survey-like configurations)，其中后者参考了欧几里得卫星(Euclid-like)的观测方案。

Method: 采用基于费希尔信息矩阵(Fisher information matrix)特征值结构的信息论分析方法，通过构建距离、膨胀率和结构增长观测量的统计模型，量化暗能量状态方程时间演化特征的探测灵敏度限制。

Result: 距离和哈勃膨胀率数据产生高度层级化的费希尔谱（Fisher spectrum），主要受单一信息模式主导，反映了对暗能量状态方程（dark energy equation of state）时间变化的固有敏感性丧失。

Abstract: Distance measurements have long provided the primary observational constraints on the expansion history of the Universe and the properties of dark energy. However, because such observables depend on cumulative line-of-sight integrals over the Hubble rate, their sensitivity to time-dependent features of the dark energy equation of state is intrinsically limited. In this work, we examine this limitation from an information-based perspective using the eigenvalue structure of the Fisher information matrix constructed from distance, expansion rate, and growth observables. We show that distance and expansion-rate data generically produce a strongly hierarchical Fisher spectrum dominated by a single information mode, reflecting an irreducible loss of sensitivity to temporal variations in dark energy. This behavior can be traced directly to the integrated kernel structure of geometric observables. Growth measurements, by contrast, respond through differential dynamics and can introduce additional independent information directions. Using both controlled mock data and survey-like configurations representative of next-generation experiments, we find that the impact of growth information depends not only on its nominal precision but also on the structure of the data covariance. In simplified mock setups, growth measurements can partially activate a second information direction even at moderate precision. In Euclid-like configurations, however, the information remains effectively one-dimensional until growth precision reaches the percent level, below which a second mode emerges rapidly. These results clarify the complementary roles of distance and growth probes and provide a model-independent criterion for assessing the physical content of cosmological constraints on dynamical dark energy.

</details>

### [26] [Dark Matter from Eternity](https://arxiv.org/abs/2602.08338)
*G. Franciolini,M. Peloso,A. Riotto*

**TL;DR**: 提出宇宙暗物质可能源自永恒暴胀（eternal inflation）未再加热区域形成的原初黑洞（primordial black holes），其丰度高于辐射相密度扰动引力坍缩标准机制，并预测宽谱曲率扰动和毫赫兹频段平坦随机引力波背景（Ω_GW h^2 ≈ 10^-10）。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙中暗物质（dark matter）可能源自宇宙暴胀（cosmological inflation）的永恒性特征，即未经历再加热（reheating）而持续暴胀的区域在可观测宇宙中表现为原初黑洞（primordial black holes）。

Data: 数据来源于理论模拟（simulation）和宇宙学暴胀（cosmological inflation）模型的预测参数。

Method: 基于永恒暴胀（eternal inflation）理论，通过未再加热区域形成原初黑洞（primordial black holes）的机制分析暗物质起源。

Result: 宇宙暗物质可能源于永恒暴胀（eternal inflation）未再加热区域形成的原初黑洞（primordial black holes），其丰度高于辐射相密度扰动引力坍缩标准机制，并预测了宽谱曲率扰动和毫赫兹频段平坦随机引力波背景（$Ω_\text{GW} h^2 \simeq 10^{-10}$）。

Abstract: We propose that the totality of dark matter in the universe might ascribe its origin to one of the key properties of cosmological inflation, that it may be eternal: regions that at the end of the primordial accelerated expansion of the universe never reheated, but keep eternally inflating, manifest themselves as primordial black holes in our observable universe. This mechanism can provide a primordial black hole abundance which is larger than the standard one due to the gravitational collapse of sizeable overdensities in the radiation phase. It also predicts a broad spectrum for the curvature perturbation and a flat stochastic gravitational wave background at a level of $Ω_\text{GW} h^2 \simeq 10^{-10}$ up to the mHz.

</details>

### [27] [Kosmulator: A Python framework for cosmological inference with MCMC](https://arxiv.org/abs/2602.08424)
*Renier T. Hough,Robert Rugg,Shambel Sahlu,Amare Abebe*

**TL;DR**: 开发了Kosmulator这一模块化、向量化的Python框架，通过数组原生执行和高效集成切片采样（Zeus）加速宇宙学模型（如f(Q) "f1CDM"）的贝叶斯推断，在保持与Cobaya代码≤0.3σ（H0和Ωm）和<0.6%（χ²）统计一致性的同时，单CPU核心计算时间减少约4.5倍。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于加速宇宙学模型（cosmological models）的统计检验，以应对超越标准ΛCDM理论框架时传统爱因斯坦-玻尔兹曼求解器（Einstein-Boltzmann solvers）面临的计算瓶颈问题。

Data: 数据来源包括Ia型超新星（Type Ia Supernovae）、宇宙计时器（Cosmic Chronometers）和重子声波振荡（Baryon Acoustic Oscillation, BAO）数据。

Method: 采用基于数组原生执行(array-native execution)和高效集成切片采样(efficient ensemble slice sampling via Zeus)的快速贝叶斯推断方法。

Result: 开发了模块化、向量化的Python框架Kosmulator，通过数组原生执行和高效集成切片采样显著加速宇宙学模型的统计检验，在保持与Cobaya代码统计一致性（≤0.3σ）的同时将单核计算时间减少约4.5倍。

Abstract: We present Kosmulator, a modular and vectorised Python framework designed to accelerate the statistical testing of cosmological models. As the theoretical landscape expands beyond standard $Λ$CDM, implementing new expansion histories into traditional Einstein--Boltzmann solvers becomes a significant computational bottleneck. Kosmulator addresses this by leveraging array-native execution and efficient ensemble slice sampling (via Zeus) to perform rapid Bayesian inference. We validate the framework against the industry-standard Cobaya code using a combination of Type Ia Supernovae, Cosmic Chronometers, and Baryon Acoustic Oscillation (BAO) data. Our results demonstrate that Kosmulator reproduces Cobaya's posterior constraints to within $\leq0.3σ$ statistical agreement on $H_{0}$ and $Ω_{m}$ and $<0.6\%$ precision on $χ^{2}$, while achieving a $\sim 4.5\times$ reduction in wall-clock time on a single CPU core compared to a standard MPI-parallelised baseline. Furthermore, we showcase the framework's utility by constraining the implicit power-law $f(Q)$ "$f_1$CDM" model and demonstrating its automated model selection capabilities (AIC/BIC). Kosmulator is introduced as a "scientific sieve" for rapid hypothesis testing, allowing researchers to efficiently filter theoretical candidates before deploying high-precision resources.

</details>

### [28] [Synchrotron Self-Compton Process for Constraining sub-GeV Dark Matter in Omega Centauri via SKA](https://arxiv.org/abs/2602.08731)
*Guan-Sen Wang,Bing-Yu Su,Yang Yu,Bo Zhang,Lei Feng*

**TL;DR**: 通过研究球状星团半人马座ω中MeV级暗物质（DM）湮灭产生的同步自康普顿（SSC）辐射，发现该间接探测方法对亚GeV暗物质的湮灭截面约束可达$\langleσv\rangle \sim 10^{-30}\,\rm{cm}^{3}\,\rm{s}^{-1}$，极端参数下甚至低于$\langleσv\rangle \sim 10^{-32}\,\rm{cm}^{3}\,\rm{s}^{-1}$，优于现有间接探测极限。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过同步自康普顿辐射(SSC)探测亚吉电子伏特(GeV)质量区间的暗物质(DM)粒子特性，以填补现代物理学中暗物质间接探测的关键空白。

Data: 数据来源于对球状星团Omega Centauri的同步自康普顿(SSC)辐射观测，并结合平方公里阵列(Square Kilometre Array)的灵敏度分析。

Method: 采用同步自康普顿辐射(SSC)分析技术研究MeV能区暗物质(DM)湮灭产生的电子-正电子对。

Result: 通过研究亚GeV（sub-GeV）质量区间暗物质（DM）湮灭产生的同步自康普顿（SSC）辐射，在球状星团半人马座ω（Omega Centauri）和平方公里阵列（SKA）灵敏度条件下，将湮灭截面限制提升至$\langleσv\rangle \sim 10^{-30}\,\rm{cm}^{3}\,\rm{s}^{-1}$（数十MeV范围），极端参数下可达$\langleσv\rangle \sim 10^{-32}\,\rm{cm}^{3}\,\rm{s}^{-1}$，证明SSC辐射是探测亚GeV暗物质的强有力手段。

Abstract: The search for the particle identity of dark matter (DM) continues to be a primary objective in modern physics. In this field, the sub-GeV mass range of DM detection remains a crucial yet challenging window. We investigate synchrotron self-Compton (SSC) emission from electrons and positrons produced by MeV-scale DM annihilation as a novel indirect detection channel. Focusing on the globular cluster Omega Centauri and the sensitivity of the Square Kilometre Array, we derive constraints on the annihilation cross section reaching $\langleσv\rangle \sim 10^{-30}\,\rm{cm}^{3}\,\rm{s}^{-1}$ in the tens-of-MeV range. Furthermore, constraints could even reach below $\langleσv\rangle \sim 10^{-32}\,\rm{cm}^{3}\,\rm{s}^{-1}$ for extreme parameter choices. Remarkably, even under deliberately conservative astrophysical assumptions, this channel outperforms existing indirect limits, establishing SSC emission as a robust probe of sub-GeV DM.

</details>

### [29] [Hints of sign-changing scalar field energy density and a transient acceleration phase at $z\sim 2$ from model-agnostic reconstructions](https://arxiv.org/abs/2602.08928)
*Özgür Akarsu,Maria Caruana,Konstantinos F. Dialektopoulos,Luis A. Escamilla,Emre O. Kahya,Jackson Levi Said*

**TL;DR**: 基于多种观测数据重建晚期宇宙膨胀历史，发现暗能量密度（dark-energy density）随红移从负变正且单标量场模型无法解释此行为，而幻影场（phantom field）或双场quintom框架可容纳该现象，同时重建运动学暗示了红移1.7-2.3区间可能存在额外加速膨胀阶段。


<details>
  <summary>Details</summary>

Background: 研究主题为利用多种观测数据重建宇宙晚期膨胀历史并探讨暗能量（dark energy）动力学特性。

Data: 数据来源包括CC（宇宙计时法）、Pantheon+ SNIa（超新星）、SDSS和DESI的BAO（重子声波振荡）数据、横向BAO数据以及外部H0先验（SH0ES和H0DN）。

Method: 采用基于节点的高斯过程核插值法（Gaussian-process-kernel interpolant）对减速哈勃率（reduced Hubble rate）进行建模，并结合CC、Pantheon+ SNIa、SDSS和DESI的BAO数据、横向BAO数据以及外部H0先验（SH0ES和H0DN）约束重建过程。

Result: 暗能量密度（dark-energy density）在演化过程中会从高红移时的负值转变为当前正值，并定义了过渡红移（transition redshift）$z_\dagger$。

Abstract: We present a data-driven reconstruction of the late-time expansion history and its implications for dark-energy dynamics. Modeling the reduced Hubble rate with a node-based Gaussian-process-kernel interpolant, we constrain the reconstruction using CC, Pantheon+ SNIa, BAO data from SDSS and DESI, transversal BAO data, and external $H_0$ priors (SH0ES and H0DN). Assuming GR at the background level, we map the reconstructed kinematics onto a dark-energy fluid and a scalar-field description, yielding the total potential and kinetic contributions that reproduce the inferred $H(z)$. To interpret the reconstruction, we consider both a minimal single-field model (canonical or phantom) and a two-field (quintom) system consisting of one canonical and one phantom scalar field (or families). Within the GR-based effective-fluid mapping, the inferred dark-energy density changes sign for all dataset combinations explored, transitioning from $ρ_{\rm DE}<0$ at higher redshift to $ρ_{\rm DE}>0$ toward the present, and defining a transition redshift $z_\dagger$ by $ρ_{\rm DE}(z_\dagger)=0$. A single canonical scalar cannot realize such a smooth evolution during expansion, whereas a phantom field or a two-field quintom framework can accommodate the required behavior; in particular, the two-field system permits smooth phantom-divide crossings at finite $ρ_{\rm DE}>0$ and distinguishes them from the separate notion of a density zero crossing. The reconstructed kinematics admit intermediate-redshift structure in some combinations, including hints of an additional accelerated-expansion interval around $z\sim 1.7$--$2.3$. The present-day equation of state remains close to a cosmological constant: combinations including supernovae give $w_0\simeq -1$, while combinations without supernovae but with an external $H_0$ prior show only a mild preference for $w_0<-1$ at the $\sim1.5$--$1.7σ$ level.

</details>

<div id='astro-ph.IM'></div>

# astro-ph.IM [[Back]](#toc)

### [30] [An Open-Source High-Level Graphical Signal Processing Language with Simulation and HDL Generation](https://arxiv.org/abs/2602.07130)
*Wei Liu,Jonathon Kocz,Dan Werthimer*

**TL;DR**: 提出将开源软件Scilab作为CASPER工具流的新前端工具，成功替代了原有的MATLAB/Simulink，并在RFSoC4x2平台上开发了基于FPGA的光谱仪，验证了其设计CASPER模块、生成FPGA知识产权(IP)核心和模拟数字信号处理(DSP)系统的能力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于射电天文领域数字信号处理系统设计框架（CASPER toolflow）的开源化改进，旨在解决当前依赖商业软件（MATLAB/Simulink）的局限性。

Data: 数据来源为基于射频系统级芯片（RFSoC4x2）的现场可编程门阵列（FPGA）平台，该平台是适用于射电天文应用的常用CASPER兼容硬件。

Method: 采用开源软件Scilab作为CASPER工具流的新前端工具，用于设计CASPER模块、生成FPGA知识产权(IP)核心和模拟数字信号处理(DSP)系统。

Result: 提出将开源软件Scilab作为CASPER工具流的新前端工具，成功替代了原有的MATLAB/Simulink专有软件，并验证了其在开发基于FPGA的天文信号处理系统（如射电频谱仪）中的可行性。

Abstract: The CASPER (Collaboration for Astronomy Signal Processing and Electronic Research) toolflow is a widely used framework for designing and implementing digital signal processing systems, particularly in the field of radio astronomy. It provides a set of tools and libraries that enable researchers to create custom hardware and software solutions for processing astronomical data. The CASPER toolflow has been instrumental in the development of Field-Programmable Gate Array (FPGA) based digital instruments for various radio telescopes, enabling for real-time data processing and analysis. However, the current frontend tool that CASPER uses for high-level FPGA design is based on Model Composer integrated into MATLAB/Simulink, which is a proprietary software. In this paper, we introduce Scilab as a new frontend tool for the CASPER toolflow. Scilab is an open-source software platform for numerical computation and data visualization, which offers a similar environment to MATLAB/Simulink for designing CASPER blocks, generating FPGA Intellectual Property (IP) cores, and simulating Digital Signal Processing (DSP) systems. We present our implementation of Scilab in the CASPER toolflow and demonstrate its capabilities by developing an FPGA based spectrometer on a RFSoC4x2, a commonly used CASPER platform well suited to radio astronomy applications. We have also developed Scilab support for other CASPER compatible platforms. Our results show that Scilab can successfully be used as an alternate frontend for CASPER-based designs.

</details>

### [31] [WFC3/UVIS: External CTE Monitoring 2009-2024](https://arxiv.org/abs/2602.07133)
*Benjamin Kuhn*

**TL;DR**: 研究哈勃太空望远镜广角相机3号UVIS探测器电荷转移效率(CTE)通量损失，发现2009-2023年间FLT数据中远离读出端的500-2000电子源在1-3电子/像素背景下的通量损失率为每年每2051像素0.05星等，2024年FLT数据在20-25电子/像素背景下的通量损失达23%/2051像素，FLC数据可缓解但仍有12%损失，且背景超过40电子/像素时存在1-3%的过度校正。


<details>
  <summary>Details</summary>

Background: 研究聚焦哈勃太空望远镜广角相机3号紫外可见通道探测器(UVIS)的电荷转移效率(CTE)通量损失问题。

Data: 数据来源于哈勃太空望远镜搭载的广域相机3（Wide Field Camera 3）紫外可见光探测器（UVIS detector）在2009年10月至2024年2月约14年间的观测数据。

Method: 采用经验模型（empirical model）分析哈勃太空望远镜广角相机3号UVIS探测器的电荷转移效率（CTE）通量损失趋势，并更新了点源测光校正的非CTE校正（FLT）和CTE校正（FLC）数据系数。

Result: 哈勃太空望远镜广角相机3号(WFC3)紫外可见光(UVIS)探测器的电荷转移效率(CTE)通量损失在2009至2023年间以每年约0.05星等差(Δmag)/2051像素的速率递增。

Abstract: This report examines Charge Transfer Efficiency (CTE) flux losses in the Wide Field Camera 3 UVIS detector aboard the Hubble Space Telescope. Spanning approximately 14 years of observations from October 2009 to February 2024, the study analyzes CTE flux loss trends across various total background levels and source fluxes. In addition to analyzing the present state of CTE flux losses, we provide updated coefficients for the empirical model for point source photometry corrections in both non-CTE-corrected (FLT) and CTE-corrected (FLC) data. Between 2009 and 2023, the rate of CTE flux loss for a 500-2000 e$^-$ source, farthest from the readout, in FLT data with a 1-3 e$^-$/pix background, is measured to be $\sim$0.05 $Δ$mag/2051 pix/year. The recommended minimum total background level to mitigate CTE losses remains at 20-25 e$^-$/pix. At that level, we find that 500-2000 e$^-$ sources, farthest from the readout, in 2024 FLT data can suffer $\sim$23$\%$ flux loss/2051 pix. The FLC data provide some relief, but we measure a $\sim$12$\%$ flux loss/2051 pix in 2024. There continues to be a slight over-correction in some FLC results that contain backgrounds above 40 e$^-$/pix. In 2024, 8000-20000 e$^-$ sources farthest from the readout in a 40, 60, or 90 e$^-$/pix background are over-corrected by $\sim$1, 2, and 3$\%$, respectively.

</details>

### [32] [Imagining the Alien: Human Projections and Cognitive Limitations](https://arxiv.org/abs/2602.07284)
*S. G. Djorgovski*

**TL;DR**: 这篇论文的核心贡献是指出人类对外星生命（extraterrestrial life）的想象主要受限于地球生物形态和人类文化认知框架，而人工智能（Artificial Intelligence）的发展可能为理解外星高级智慧提供新视角。


<details>
  <summary>Details</summary>

Background: 该研究探讨了人类对外星生命（extraterrestrial life）特别是智慧生命的想象及其文化、哲学和认知根源。

Data: 研究中未明确提及具体观测或模拟（simulation）数据来源。

Method: 采用跨学科（interdisciplinary）方法结合文化人类学（cultural anthropology）与认知科学（cognitive science）框架，分析人类对地外智能（extraterrestrial intelligence）的想象模式及其认知根源。

Result: 人类对地外生命的想象主要映射了地球生命形式及人类文化、历史和心理学现象，反映了人类思维的内在认知局限和文化现象。

Abstract: Imagining what life on other planets, and intelligent life in particular, may be like is a long-running theme in human culture. It is a manifestation of the innate human curiosity about the Cosmos, and it has inspired numerous works of art and folklore, including whole literary and other media genres. It is a profound question, with philosophical and existential implications. There is also an obvious connection with religious beliefs, as gods and other superhuman beings were imagined in the heavens. Speculations about alien beings grew in time, and today, it is a scientific subject of astrobiology, and it is pursued through serious searches for life and intelligence in the universe. However, almost all imaginings of the alien map terrestrial life forms and human cultural, historical, and psychological phenomena to the putative aliens. This lack of individual and collective imagination may reflect our biological and cultural evolution, as our minds are formed through our experiences, perceptions of the world, and interactions with our terrestrial and human environments. As such, imagining aliens is mainly a cultural phenomenon and may reflect the intrinsic cognitive limitations of the human mind. Interestingly, we did create what is effectively an alien intelligence on this planet in the form of now rapidly evolving Artificial Intelligence (AI). As its capabilities grow, it may give us new insights into what extraterrestrial advanced intelligences may be like.

</details>

### [33] [Sunrise III: The Wavefront Correction System](https://arxiv.org/abs/2602.07448)
*Thomas Berkefeld,Alexander Bell,Reiner Volkmer,Frank Heidecke,Tobias Preis,Thomas Sonner,Eiji Nakai,Andreas Korpi-Lagg,Achim Gandorfer,Sami K. Solanki,Jose Carlos del Toro Iniesta,Yukio Katsukawa,Pietro Bernasconi,Alex Feller,Tino L. Riethmüller,Alberto Álvarez-Herrero,Masahito Kubo,Valentín Martínez Pillet,H. N. Smitha,David Orozco Suárez,Bianca Grauf,Michael Carpenter*

**TL;DR**: 论文开发了用于Sunrise III气球望远镜的波前校正和图像稳定系统（CWS），实测表明该系统能实现0.005角秒（均方根）的图像稳定精度和0.01波长的焦平面聚焦稳定性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于平流层气球望远镜（Sunrise III）的波前校正和图像稳定系统（CWS）开发，旨在解决高空观测中的波前畸变和图像抖动问题。

Data: data来源为Sunrise III气球载望远镜的集成测试和2024年科学飞行期间采集的实测数据。

Method: 采用相关跟踪器（CT）和快速倾斜镜实现图像稳定，通过六单元沙克-哈特曼波前传感器（WFS）测量离焦和彗差等低阶像差，并利用主动式望远镜副镜进行自动调焦和手动彗差校正。

Result: Sunrise III气球望远镜的波前校正和图像稳定系统（CWS）实现了0.005角秒（均方根值）的图像稳定精度和0.01波长的焦平面聚焦稳定性。

Abstract: This paper describes the wave-front correction and image stabilisation system (CWS) developed for the Sunrise III balloon-borne telescope, and provides information about its performance as measured during the integration into the telescope and during the 2024 science flight. The fast image stabilisation is done by a correlation tracker (CT) and a fast tip-tilt mirror, low order aberrations such as defocus and coma are measured by a six-element Shack-Hartmann wavefront sensor (WFS) and corrected by an active telescope secondary mirror for automated focus and manual coma correction. The CWS is specified to deliver a stabilised image with a precision of 0.005 arcsec (rms). The autofocus adjustment is specified to maintain a focus stability of 0.01 waves in the focal plane of the CWS.

</details>

### [34] [Toward Vision-Language Assistants for Radio Astronomical Source Analysis](https://arxiv.org/abs/2602.07469)
*S. Riggi*

**TL;DR**: 该论文评估了多种视觉语言模型(VLMs)在射电天文学任务中的表现，发现商用模型(GPT-4.1)在零样本模式下优于开源模型，而微调后的radio-llava模型显著提升了性能但仍不及专用视觉模型，同时发现微调会导致通用多模态任务性能下降(最高达40%)。


<details>
  <summary>Details</summary>

Background: 研究探讨了视觉语言模型（VLM）在射电天文学（radio astronomy）等专业科学领域中的适用性和性能优化问题。

Data: 使用了开源和商用视觉语言模型（VLMs）的预训练权重，并通过指令微调构建了专用模型radio-llava。

Method: 基于LLaVA架构并通过指令微调（instruction fine-tuning）构建了面向射电领域的多模态助手radio-llava。

Result: 在射电天文学任务中，微调后的多模态助手radio-llava显著优于基础LLaVA架构和商业模型，但仍逊于专用视觉模型。

Abstract: Vision-language models (VLMs) have recently shown promise in general-purpose reasoning tasks, yet their applicability to domain-specific scientific workflows remains largely unexplored. In this work, we evaluated a series of open-weight and commercial VLMs on six tasks relevant to radio astronomy, such as source morphology classification. We also introduced radio-llava, a fine-tuned multimodal assistant built on the LLaVA architecture and adapted for the radio domain through instruction fine-tuning.
  In zero-shot mode, commercial models like GPT-4.1 outperform open-weight VLMs on most radio benchmarks. However, radio-llava significantly improves upon both base LLaVA and commercial models across nearly all tasks. Despite these gains, specialized vision-only models still deliver substantially better performance across the board. Additionally, we observed that fine-tuning introduces catastrophic forgetting on general multimodal tasks, with performance drops up to 40% that can be partly mitigated with a more diverse training dataset or shallow fine-tuning.

</details>

### [35] [Scintillation muon telescope module with fiber-optic light collection](https://arxiv.org/abs/2602.07490)
*Belov S. M.,Yanke V. G*

**TL;DR**: 开发了一种采用硅光电倍增管光纤集光的闪烁缪子(muon)望远镜模块，测试和长期监测表明其稳定性优于0.1%/年且无需恒温控制，具备操作简便、成本低、结构紧凑等优势。


<details>
  <summary>Details</summary>

Background: 开发用于宇宙线(cosmic ray)变化研究的闪烁缪子(muon)望远镜模块。

Data: 数据来源于自主研发的闪烁体μ子望远镜模块（scintillation muon telescope module）的连续监测数据，并结合其他探测器的内部对照数据。

Method: 采用硅光电倍增管(silicon photomultipliers)的光纤光收集闪烁体缪子望远镜模块，结合内部控制技术（internal control technique）评估参数稳定性。

Result: 开发的光纤收集闪烁μ子望远镜模块（scintillation muon telescope module）在非恒温环境下长期稳定性优于0.1%/年。

Abstract: A scintillation muon telescope module with fiber-optic light collection using silicon photomultipliers was developed, tested, and installed for continuous monitoring to study cosmic ray variations. The aim of this study was to create a scintillation muon telescope module, continuously monitor the muon component in test mode, and study the long-term stability of the detector parameters. Methods for processing the obtained data were developed. To assess the stability of the module parameters, an internal control technique was used, involving data from other detectors. The results of testing and long-term continuous monitoring showed that the stability of the developed muon detector is better than 0.1%/year, without the need for its operation in a thermostatic chamber. The study concludes that ease of operation, cost, compactness, low power consumption and stability are factors that determine the advantages of the developed module, which is an essential element for constructing a multidirectional muon telescope.

</details>

### [36] [SOLO: wide-field asteroid light curve monitoring system for SPHEREx](https://arxiv.org/abs/2602.08037)
*Bumhoo Lim,Seungwon Choi,Yoonsoo P. Bach,Masateru Ishiguro,Sunho Jin,Carey M. Lisse,Max Mahlke,Jooyeon Geem,Jinguk Seo,Sihu Ahn,Hangbin Jo*

**TL;DR**: 研发了太阳系天体光变观测站（SOLO）系统，通过宽视场、高频率光学观测获取盖亚（Gaia）G波段绝对校准的小行星光曲线，支持SPHEREx太阳系天体目录（SSOC）构建，实测性能显示单次曝光180秒可达G波段极限星等17.5且视场稳定性良好。


<details>
  <summary>Details</summary>

Background: 研究主题为利用宽视场、高频率光学观测系统SOLO获取绝对校准的小行星光变曲线（light curve），以支持SPHEREx任务中的太阳系天体目录（SSOC）构建和科学应用。

Data: 数据来源于安装在美国加利福尼亚州Sierra Remote Observatories (SRO)的Solar system Objects Light curve Observatory (SOLO)系统获取的宽视场、高频率光学观测数据，包括绝对定标的小行星光变曲线（转换为Gaia G波段测光系统）以及性能验证阶段的调试数据。

Method: 采用宽视场、高时间分辨率光学巡天系统（SOLO）获取绝对定标的小行星光变曲线，并将其转换为盖亚（Gaia）G波段测光系统。

Result: SOLO（Solar system Objects Light curve Observatory）能够实现11.6平方度视场内稳定的光度校准，并在180秒曝光下达到G波段约17.5星等的10σ极限星等。

Abstract: We present the Solar system Objects Light curve Observatory (SOLO), a wide-field, high-cadence optical survey system designed to obtain absolutely calibrated asteroid light curves, converted to the Gaia G-band photometric system, in support of the SPHEREx Solar System Object Catalog (SSOC). SOLO was installed at the Sierra Remote Observatories (SRO) in California, USA, in July 2025 and is optimized for continuous, multi-night monitoring of asteroid brightness variations. We describe the system configuration, remote operation, and data reduction pipeline, and evaluate its optical and photometric performance using commissioning data. SOLO achieves stable photometric calibration across the 11.6 deg^2 field of view and reaches a 10-sigma limiting magnitude of G ~ 17.5 for a 180 sec exposure. Sample asteroid light curves obtained over multiple nights demonstrate consistent absolute photometry at the same rotational phase, validating the estimated performance. Finally, we outline the planned operational use of SOLO in connection with NASA's SPHEREx mission. Full science operations of SOLO are scheduled to begin in January 2026. Using these data, we aim to obtain on the order of 10^3 absolutely calibrated asteroid light curves per year in the Gaia G-band, which will be used to support the construction and scientific utilization of the SPHEREx SSOC.

</details>

### [37] [DerivKit: stable numerical derivatives bridging Fisher forecasts and MCMC](https://arxiv.org/abs/2602.08078)
*Nikolina Šarčević,Matthijs van der Wild,Cynthia Trendafilova*

**TL;DR**: DerivKit是一个Python工具包，通过实现稳定数值微分和导数组装功能，支持费希尔矩阵(Fisher-matrix)预测和高阶似然近似，为自动微分不可行或不可用的标量和矢量模型（包括黑盒或表格函数）提供了高斯近似之外的导数推断方法。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发用于基于导数（derivative-based）统计推断的Python工具包，旨在扩展科学应用中超越高斯近似的导数推断方法。

Data: data：DerivKit支持标量和向量值模型（scalar- and vector-valued models）的黑盒（black-box）或表格化函数（tabulated functions）的稳定数值微分（numerical differentiation）和导数组装（derivative assembly），适用于自动微分（automatic differentiation）不可行或不可用的情况。

Method: 基于数值微分（numerical differentiation）和导数组装（derivative assembly）技术，支持费希尔矩阵（Fisher-matrix）预测和高阶似然近似（DALI）。

Result: DerivKit通过实现稳定的数值微分和导数组装工具，为费希尔矩阵(Fisher-matrix)预测和高阶似然近似提供支持，扩展了基于导数的统计推断方法。

Abstract: DerivKit is a Python package for derivative-based statistical inference. It implements stable numerical differentiation and derivative assembly utilities for Fisher-matrix forecasting and higher-order likelihood approximations in scientific applications, supporting scalar- and vector-valued models including black-box or tabulated functions where automatic differentiation is impractical or unavailable. These derivatives are used to construct Fisher forecasts, Fisher bias estimates, and non-Gaussian likelihood expansions based on the Derivative Approximation for Likelihoods (DALI). By extending derivative-based inference beyond the Gaussian approximation, DerivKit forms a practical bridge between fast Fisher forecasts and more computationally intensive sampling-based methods such as Markov chain Monte Carlo (MCMC).

</details>

### [38] [Particle hydrodynamics with accurate gradients: a comparison of different formulations](https://arxiv.org/abs/2602.08091)
*S. Rosswog*

**TL;DR**: 论文比较了多种现代光滑粒子流体动力学(SPH)方法，发现采用精确梯度计算（特别是线性精确梯度(aLE)和线性再生核(RPKs)）的方法在不稳定性模拟中表现优异，其中RPKs效果最佳但aLE梯度计算更简单且性能接近，而基于黎曼求解器(Roe's approximate Riemann solver)的方法在激波测试中略有优势但在低分辨率下不稳定性增长较慢，最终推荐结合激波耗散和RPKs（版本V3和V5）或aLE梯度（版本V2）的方案。


<details>
  <summary>Details</summary>

Background: 研究聚焦于比较不同现代版本的平滑粒子流体动力学（SPH）方法，重点探讨梯度精度对流体模拟的影响。

Data: 数据来源于对多种现代SPH（光滑粒子流体动力学）版本的比较研究，包括标准SPH核梯度、线性精确梯度（aLE）近似、线性再现核（RPKs）以及结合Roe近似黎曼求解器的粒子流体动力学方法，重点关注梯度精度的影响。

Method: 采用平滑粒子流体动力学（SPH）方法，重点比较了标准SPH核梯度、线性精确梯度（aLE）近似和线性再现核（RPKs）的梯度精度影响，并测试了结合斜率限制重构的激波耗散（人工粘性和传导）参数以及使用RPKs和Roe近似黎曼求解器的粒子流体动力学方案。

Result: 在涉及不稳定性问题时，梯度精度（gradient accuracy）起关键作用，其中再现核（reproducing kernels, RPKs）梯度表现最佳，而近似线性精确（aLE）梯度紧随其后。

Abstract: We compare here several modern versions of SPH with a particular focus on the impact of gradient accuracy. We examine specifically an approximation to the "linearly exact" gradients (aLE) with standard SPH kernel gradients and with linearly reproducing kernels (RPKs) that fulfill the lowest order consistency relations exactly by construction. Most of the explored SPH formulations use shock dissipation (i.e. artificial viscosity and conductivity) with slope-limited reconstruction and parameters that trigger on both shocks and noise. We also compare with a recent particle hydrodynamics formulation that uses both RPKs and Roe's approximate Riemann solver instead of shock dissipation. Not too surprisingly, we find that the shock tests are rather insensitive to the gradient accuracy, but whenever instabilities are involved the gradient accuracy plays a crucial role. The reproducing kernel gradients perform best, but they are closely followed by the much simpler aLE gradients. The Riemann solver approach has some (minor) advantages in the shock tests, but shows some resistance against instability growth and at low resolution the corresponding Kelvin-Helmholtz simulations show substantially slower instability growth than the best shock dissipation approaches. Based on the battery of benchmark tests performed here, we consider a shock dissipation approach with reproducing kernels (our versions $V_3$ and $V_5$) as best, but closely followed by a similar version ($V_2$) that uses the simpler and computationally cheaper aLE gradients.

</details>

### [39] [Towards FAIR Astrophysical Simulations](https://arxiv.org/abs/2602.08416)
*Susanne Pfalzner,Stephan Hachinger,Jolanta Zjupa,Salvatore Cielo,Frank W. Wagner,Marcus Brüggen,Annika Hagemeier*

**TL;DR**: 该论文针对天体物理数值模拟领域，分析了高性能计算环境下实现FAIR（可发现、可访问、可互操作、可重用）数据标准面临的三大障碍（海量数据管理困难、共享激励不足、工作流程缺失），并提出了实现更FAIR化数据和开源标准的可行方案。


<details>
  <summary>Details</summary>

Background: 探讨天体物理数值模拟领域实现FAIR（可发现、可访问、可互操作、可重用）数据原则的现状与挑战。

Data: 数据来源主要基于高性能计算（HPC）设施生成的天体物理模拟（simulation）数据。

Method: 采用高性能计算（HPC）视角分析FAIR（可查找、可访问、可互操作、可重用）原则在天体物理模拟领域的实施障碍，并提出低门槛方法满足基本FAIR要求及基础工具建议。

Result: 在天体物理模拟领域实现FAIR（可查找、可访问、可互操作、可重用）数据的主要障碍包括：HPC设施产生的大规模数据集使FAIR数据管理复杂化、缺乏共享代码/结果/诊断数据的激励机制、以及缺少包含数据发布和技术支持的工作流程。

Abstract: Reproducibility is a cornerstone of science. FAIR (findable, accessible, interoperable, and reusable) data is often a vital step towards testing the reproducibility of results. The implementation of FAIR principles in the astrophysical simulation community is still varied. We approach the discussion of this topic mainly from a high-performance computing (HPC) point of view. We identify the main obstacles to FAIR astrophysics simulations: First, the vast datasets created in simulations on HPC facilities complicate FAIR data management. Second, missing incentives to fully share codes, results, and diagnostic data. Third, a lack of workflows that include data publication and technical support. Therefore, particularly smaller research groups struggle due to the unavailability of dedicated personnel and time in their efforts towards FAIR and open simulations. We propose actionable steps towards achieving ``FAIRer'' data and open source publication standards in numerical astrophysics. Our suggestions include low-threshold methods to fulfil the basic FAIR requirements as well as basic tools for FAIR (meta-)data generation and data/code publication. This work is a high-level overview intended to initiate discussions within the community, offering initial solutions to these challenges.

</details>

<div id='hep-ph'></div>

# hep-ph [[Back]](#toc)

### [40] [Can Dirac neutrinos destabilize $\mathcal{Z}_2$ domain wall network?](https://arxiv.org/abs/2602.07380)
*Debasish Borah,Partha Kumar Paul,Narendra Sahu*

**TL;DR**: 这篇论文揭示了离散$\mathcal{Z}_2$对称性自发破缺与狄拉克中微子质量生成机制的关联，发现通过辐射修正产生的显式破缺项与狄拉克中微子质量立方成反比，导致引力波谱与其六次方成正比，从而将狄拉克跷跷板标度（Dirac seesaw scale）、畴壁湮灭时期和随机引力波信号联系起来，并表明该机制部分参数空间可通过引力波和宇宙微波背景实验探测，同时解释观测到的重子不对称性（baryon asymmetry）。


<details>
  <summary>Details</summary>

Background: 研究探讨了早期宇宙中离散$\mathcal{Z}_2$对称性自发破缺导致的稳定畴壁网络（domain wall networks）形成及其对宇宙能量密度的潜在影响，同时提出通过狄拉克中微子质量生成机制自然引入显式对称性破缺项的理论框架。

Data: 数据来源于理论模型构建中引入的离散$\mathcal{Z}_2$对称性自发破缺机制及其在早期宇宙动态演化过程中产生的稳定畴壁网络（domain wall networks）和引力波信号。

Method: 采用狄拉克中微子质量生成机制中粒子辐射修正诱导的显式$\mathcal{Z}_2$对称性破缺项作为偏置项（bias term），推导出与狄拉克中微子质量立方成反比的标度关系。

Result: 在粒子物理模型构建中，若用于产生狄拉克中微子质量（Dirac neutrino mass）的离散$\mathcal{Z}_2$对称性自发破缺，其显式破缺项可通过狄拉克质量生成相关粒子的辐射修正自然产生，且偏置项与狄拉克中微子质量立方成反比，导致引力波谱正比于狄拉克中微子质量的六次方。

Abstract: In particle physics model building, a discrete $\mathcal{Z}_2$ symmetry is often spontaneously broken for phenomenological reasons. When this breaking occurs dynamically in the early Universe, stable domain wall networks are formed, which can eventually dominate the cosmic energy density. To avoid this problem, explicit $\mathcal{Z}_2$-breaking terms in the scalar potential are usually introduced in an ad hoc manner. In this Letter, we show that if the same $\mathcal{Z}_2$ symmetry is also responsible for generating light Dirac neutrino masses, such explicit breaking terms can instead arise radiatively from the particles involved in the Dirac mass generation. We find that the resulting bias term scales inversely with the cube of the Dirac neutrino mass, leading to a gravitational wave spectrum proportional to the sixth power of the Dirac neutrino mass. This establishes a nontrivial connection between the Dirac seesaw scale, the domain wall annihilation epoch, and the resulting stochastic gravitational wave signal. We further demonstrate that a wide range of Dirac seesaw scales can be probed by upcoming gravitational wave and cosmic microwave background experiments, while part of the parameter space simultaneously explains the observed baryon asymmetry via Dirac leptogenesis.

</details>

### [41] [Primordial features as probes of baryogenesis from supersymmetric flat directions](https://arxiv.org/abs/2602.08781)
*Yi-Peng Wu,Xingang Chen,Nino Ephremidze,Lingfeng Li*

**TL;DR**: 该研究通过宇宙微波背景（CMB）对重子密度等曲率扰动的约束更新了Affleck-Dine机制（Affleck-Dine mechanism）的可行参数空间，并证明原初暴胀特征可作为探测该机制中轻、重模式的独特工具，其相关信号（如尖锐特征和时钟信号）同时体现在曲率和重子密度等曲率扰动中。


<details>
  <summary>Details</summary>

Background: 该研究探讨了Affleck-Dine机制（Affleck-Dine mechanism）作为暴胀期间通过超对称平坦方向形成标量凝聚态（scalar condensates）的主要重子生成（baryogenesis）情景，并基于宇宙微波背景（Cosmic Microwave Background）对重子密度等曲率扰动（baryon-density isocurvature perturbations）的最新约束更新了可行参数空间。

Data: 数据来源于宇宙微波背景辐射（Cosmic Microwave Background）对重子密度等曲率扰动（baryon-density isocurvature perturbations）的最新观测约束，以及暴胀期间标量凝聚态（scalar condensate）量子涨落的初始条件。

Method: 采用量子涨落作为初始条件，结合宇宙微波背景辐射（CMB）对重子密度等曲率扰动（baryon-density isocurvature perturbations）的最新约束，更新了重子生成的可行参数空间，并通过暴胀子（inflaton）原初特征（primordial features）产生的相关印记（如尖锐特征信号和时钟信号）直接验证Affleck-Dine机制中轻、重模式的存在。

Result: Affleck-Dine机制（Affleck-Dine mechanism）中暴胀期间形成的标量凝聚态量子涨落与重子密度等曲率扰动（baryon-density isocurvature perturbations）的原始特征（primordial features）相关联，为机制中轻模式和重模式的存在提供了直接证据。

Abstract: The Affleck-Dine mechanism is a leading baryogenesis scenario in which scalar condensates form coherently during inflation along supersymmetric flat directions that are lifted by supersymmetry-breaking effects. We update the viable parameter space for baryogenesis using recent Cosmic Microwave Background constraints on baryon-density isocurvature perturbations, taking the quantum fluctuations of the scalar condensate generated during inflation as initial conditions. We then show that primordial features arising from the inflaton sector can serve as a unique probe of baryogenesis models, whose mechanisms are otherwise difficult to access directly due to their high energy scales. These primordial features leave correlated imprints, such as sharp feature signals and clock signals, on both the curvature and baryon-density isocurvature perturbations, providing direct evidence for the existence of both light and heavy modes involved in the Affleck-Dine mechanism.

</details>

<div id='astro-ph.SR'></div>

# astro-ph.SR [[Back]](#toc)

### [42] [SPHEREx as a frontier for infrared transients: Classification of new Galactic FU Ori outbursts and classical novae](https://arxiv.org/abs/2602.07129)
*Viraj Karambelkar,Kishalay De,Lynne A. Hillenbrand,J. L. Sokoloski,Danielle Frostig,Julianne Dalcanton*

**TL;DR**: 提出利用SPHEREx和NEOWISE空间任务的协同观测策略，通过分析八颗银河系暂现源（包括两颗已知FU Orionis型爆发源、三颗已知经典新星）的红外光谱，新发现了两颗FU Orionis型爆发源和一颗被遗漏的银河系新星，证明了SPHEREx在系统探索动态红外天空方面的潜力。


<details>
  <summary>Details</summary>

Background: 该研究探讨了利用SPHEREx和NEOWISE空间任务的协同效应探索动态红外天空（dynamic infrared sky）的新策略，填补了缓慢演化红外爆发（slowly evolving infrared outbursts）系统性研究的空白。

Data: 数据来源为SPHEREx空间任务和NEOWISE空间任务的红外观测数据，其中NEOWISE提供十五年基线全天中红外（mid-IR）覆盖数据，SPHEREx提供全天光谱测光覆盖和中红外灵敏度匹配数据。

Method: 采用NEOWISE和SPHEREx的协同观测策略，结合NEOWISE的全天巡天中红外（mid-IR）数据与SPHEREx的低分辨率红外光谱（low-resolution IR spectra）分析技术，通过模板匹配方法识别红外瞬变源（IR transients）的光谱特征。

Result: SPHEREx与NEOWISE协同观测策略为系统探索动态红外天空（dynamic infrared sky）提供了新途径，成功识别出两个新FU Orionis型爆发（FUOr）和一个被遗漏的银河系新星（nova）。

Abstract: We demonstrate proof-of-concept of a new strategy for studying infrared (IR) transients enabled by the newly launched SPHEREx space mission, by leveraging its synergy with the NEOWISE space mission. With its fifteen year baseline and all-sky mid-IR coverage, NEOWISE provides an excellent avenue to discover thousands of slowly evolving infrared outbursts. With its all-sky spectro-photometric coverage and mid-IR sensitivity matching NEOWISE, SPHEREx is uniquely positioned to provide low-resolution IR spectra for the vast majority of these outbursts, several of which are too obscured for ground-based spectroscopic classification. As a demonstration of this approach, we present SPHEREx spectra for eight Galactic transients identified in NEOWISE. This sample includes two previously known FU Orionis-type (FUOr) outbursts whose SPHEREx spectra exhibit clear signatures of cool molecular absorption and three known classical novae showing strong emission lines in SPHEREx. Using these sources as templates, we identify two new FUOrs and one previously missed Galactic nova. Our results highlight the potential of SPHEREx for systematic explorations of the relatively underexplored dynamic infrared sky.

</details>

### [43] [Emergence of a lithium dip in ~35 Myr "Snake" Open Clusters](https://arxiv.org/abs/2602.07367)
*Yun-Yi Zhang,Hai-Jun Tian,Jian-Rong Shi,Cheng-Cheng Xie,Xiang-Ming Yang*

**TL;DR**: 在年龄仅35±5 Myr的年轻星团"Snake"中首次发现锂凹陷(Li-dip)，其有效温度范围为6200-6800 K且锂耗损深度达0.40 dex，比传统认知提前了100 Myr以上，并揭示快速自转(v sin i >25 km/s)恒星比慢速自转恒星表现出更强的锂耗损，表明快速自转通过增强对流-辐射边界处的旋转剪切加速了锂破坏。


<details>
  <summary>Details</summary>

Background: 研究挑战了传统认知，首次在年龄仅35±5 Myr的年轻星团"Snake"中发现了锂凹陷(Li-dip)现象，将此类现象的出现时间提前了至少100 Myr。

Data: 数据来源于GALAH DR4（Galactic Archaeology with HERMES Data Release 4）的高分辨率光谱（分辨率R约28,000），共分析了211颗成员星。

Method: 采用高分辨率光谱（GALAH DR4，分辨率R∼28,000）分析211颗成员星的锂丰度（Li-dip）和有效温度（Teff）关系，并结合自转速度（v sin i）与锂耗损的相关性研究。

Result: 在蛇夫座恒星（年龄为35±5百万年）中发现锂凹陷（Li-dip），挑战了锂凹陷仅在年龄≳150百万年时出现的经典观点。

Abstract: We report the discovery of a lithium dip (Li-dip) in the stellar "Snake" (age = $35 \pm 5$ Myr), challenging the classical view that Li-dips emerge only at ages $\gtrsim 150$ Myr. Using high-resolution spectra from GALAH DR4 ($R \sim 28,000$) for 211 member stars, we identify a clear depletion feature in a $T_{\mathrm{eff}}$ range of 6200--6800 K with a depth of $ΔA(\mathrm{Li}) \approx 0.40$ dex. Our analysis reveals two key advances: the Li-dip appears $\gtrsim 100$ Myr earlier than the previous observations, and within the dip temperature range, a significant correlation is found between rotational velocity and lithium depletion. Specifically, fast rotators ($v \sin i > 25$ km s$^{-1}$) exhibit stronger lithium depletion than slow rotators ($v \sin i < 25$ km s$^{-1}$). This trend suggests that faster rotators develop stronger rotational shear at the convective-radiative boundary, which enhances turbulent mixing and accelerates lithium destruction. It is also found that the lower temperature edge of the lithium plateau can reach as low as 5500 K for the young open clusters.

</details>

### [44] [Variation of the sunspot area during the rising and declining phases of the solar cycle supports the toroidal flux loss due to flux emergence](https://arxiv.org/abs/2602.07194)
*Bidya Binay Karak,Soumya Mishra,Anu Sreedevi*

**TL;DR**: 研究发现太阳黑子群面积和双极磁区(BMR)磁通量在太阳活动周上升相普遍大于下降相，且上升相分布均值和中位数与周期强度相关，而下降相(尤其最后三年)呈弱相关或无关联，支持了磁通量涌现导致非线性损失的理论模型。


<details>
  <summary>Details</summary>

Background: 太阳黑子（sunspots）是太阳对流区发电机（dynamo）产生的环形磁场（toroidal magnetic field）的显著可观测表现，其面积分布广泛且与太阳活动周期（solar cycle）的动力学过程密切相关。

Data: 数据来源于过去13个太阳周期的太阳黑子群面积（sunspot group area）以及第23和24周期的双极磁区（Bipolar Magnetic Region, BMR）磁通量观测数据。

Method: 采用统计分析方法和非线性通量损失理论模型（nonlinear flux loss due to flux emergence）研究太阳黑子群面积和双极磁区（BMR）通量分布。

Result: 太阳黑子群面积和双极磁区(BMR)磁通量在太阳活动周上升相统计上大于下降相。

Abstract: Sunspots are obvious observable manifestations of the toroidal magnetic field generated through the dynamo in the convection zone. They appear in different sizes, having a wide distribution in their area. We analyse the sunspot group area of the past 13 cycles and the Bipolar Magnetic Region (BMR) flux for Cycles 23 and 24 to explore their area and flux distributions and connect with the theory. We find that, in general, the group area and BMR flux are statistically larger in the rising phase than in the declining phase of the solar cycle. This implies that the rising phase of the solar cycle is prone to drive more intense space weather. We further show that the mean and median of the area distribution during the rising phase are dependent on cycle strength. However, the distribution mean and median are cycle strength-independent or weakly dependent during the decline phases of the solar cycles, particularly during the last three years when the latitudinal bands of all cycles migrate towards the equator along the same trajectory. These results support the theoretical model of nonlinear flux loss due to flux emergence, which explains why solar cycles rise differently but decay similarly.

</details>

### [45] [Twisted Pseudodisk and Asymmetric Mass Accretion on the Circumstellar Disk](https://arxiv.org/abs/2602.07741)
*Masahiro N. Machida,Shingo Hirano,Shantanu Basu*

**TL;DR**: 通过三维电阻磁流体动力学（MHD）模拟发现，原恒星形成后即使无湍流或外部不对称流入，原行星盘周围磁场也会因旋转运动显著扭曲，导致假盘（pseudodisk）结构复杂化，并形成非对称、各向异性的窄通道吸积流和喷流，进而引起中央原恒星质量吸积率的时变特性。


<details>
  <summary>Details</summary>

Background: 研究探讨了无湍流且初始磁场与旋转轴未对准的预恒星核中，气体流入原行星盘（circumstellar disks）及伪盘（pseudodisk）演化的三维电阻磁流体动力学（resistive MHD）过程。

Data: 使用三维电阻磁流体力学（MHD）模拟方法，初始条件为无湍流且初始磁场与旋转轴存在错位的预恒星核（prestellar core），模拟了原恒星形成后约10^5年的气体流入模式和假盘（pseudodisk）演化过程。

Method: 采用三维电阻磁流体力学(MHD)模拟方法研究气体流入原行星盘的模式及假盘(pseudodisk)的演化过程。

Result: 磁场的显著扭曲导致伪盘（pseudodisk）结构演化为复杂形态，吸积和外流呈现不对称和各向异性特征。

Abstract: We model gas inflow patterns onto circumstellar disks and the evolution of the pseudodisk using three-dimensional resistive MHD simulations. Starting from a prestellar core without turbulence and with a misalignment between the initial magnetic field and rotation axis, the simulations are performed for $\sim10^5$ yr after protostar formation. After disk formation, the magnetic field around the disk becomes significantly distorted due to the disk rotational motion. Consequently, the structure of the pseudodisk also evolves into a complex morphology. As a result, both accretion onto the disk and outflow become asymmetric and anisotropic. Accretion to the disk occurs primarily through narrow-channel flows or streams. The time evolution of the infalling envelope leads to non-steady accretion onto the disk, which in turn causes variability in the mass accretion onto the central protostar. This study demonstrates that complex infalling envelope structures and channelized accretion flows onto the disk naturally arise even without assuming turbulence or external asymmetric inflows.

</details>

### [46] [A mapping method of age estimation for binary stars: Application to the $α$ Centauri system A and B](https://arxiv.org/abs/2602.08879)
*F. Thévenin,V. A. Baturin,A. V. Oreshina,P. Morel,S. V. Ayukov,L. Bigot,A. B. Gorshkov*

**TL;DR**: 通过逆向校准方法测定双星系统α Centauri A和B的年龄（7.2-7.8 Ga）、初始氦丰度（0.284±0.004）和金属丰度（0.0335±0.0015），并发现高太阳金属质量比（Z/X⊙=0.0245）的辐射核心模型与星震学观测更吻合。


<details>
  <summary>Details</summary>

Background: 利用盖亚(Gaia)和即将发射的柏拉图(PLATO)任务提供的大量数据改进恒星模型以获取精确恒星年龄。

Data: 数据来源为盖亚(Gaia)和即将发射的柏拉图(PLATO)任务提供的观测数据，以及α Centauri A和B的最新观测数据，包括质量(M)、半径(R)、光度(L)和金属丰度([Fe/H])。

Method: 采用逆向校准方法（inverse calibration method）对多恒星系统演化进行建模，假设成员星共享相同年龄和初始化学组成，并利用观测光度、半径及表面化学丰度确定年龄、初始氦质量分数（$Y_{ini}$）、重元素质量分数（$Z_{ini}$）及对流混合长参数（$α_A$和$α_B$）。

Result: 采用高太阳金属丰度比（$Z/X_\odot =0.0245$）时，得出半人马座αA和B的年龄为7.8±0.6 Ga、初始氦丰度（$Y_{ini}$）为0.284±0.004、初始金属丰度（$Z_{ini}$）为0.0335±0.0015，且该模型与观测到的星震频率更吻合。

Abstract: Given the wealth of data provided by Gaia and the upcoming PLATO mission, it is essential to improve stellar models to obtain accurate stellar ages. Our objective is to apply a mapping technique to estimate the age of a system and the initial chemical composition. We also evaluate the influence of observational uncertainties in mass and heavy-element mixtures on results. We applied an inverse calibration method to the evolution of a multiple stellar system, assuming that the stars share the same age and initial chemical composition. This approach determines age, the initial mass fractions of helium ($Y_{ini}$) and heavy elements ($Z_{ini}$), as well as the convective mixing-length parameters ($α_A $ and $α_B$). It uses the observed luminosities ($L_A$ and $L_B$), radii ($R_A$ and $R_B$), and surface chemical compositions ($Z/X_A$ and $Z/X_B$). We used the most recent observational data for $M$, $R$, $L$, and $[Fe/H]$ of $α$ Centauri A and B as input data for our method. We compared two assumptions for the $Z/X$ ratio, following the results for the solar composition. For an assumed high solar $Z/X_\odot =0.0245$, we obtain an age of $7.8 \pm 0.6$ Ga, $Y_{ini} = 0.284 \pm 0.004$, and $Z_{ini} = 0.0335 \pm 0.0015$. For a low solar $Z/X_\odot = 0.0181$, the derived age is $8.7 \pm 0.6$ Ga, $Y_{ini} = 0.267 \pm 0.008$, and $Z_{ini} = 0.025 \pm 0.002$. Observational errors in the stellar masses of $\pm$0.002 lead to an age error of 0.6 Ga. Overshooting of $0.05-0.20H_p$ at the boundary of the convective core increases the age by $0.6-2.1$ Ga. Models with higher $Z/X$ and radiative cores, with ages of $7.2-7.8$ Ga, appear preferable and show better agreement with the observed asteroseismic frequencies.

</details>

### [47] [Discovery of a double white dwarf in the Galactic globular cluster NGC 6397](https://arxiv.org/abs/2602.08898)
*Fabian Göttgens,Marilyn Latour,Ulrich Heber,Sebastian Kamann,Kyle Kremer,Sven Martens,Stefan Dreizler*

**TL;DR**: 在银河系球状星团NGC 6397中心发现了一个由两颗极低质量氦核白矮星(WD)组成的致密双星系统NF1，其轨道周期为0.54天且伴星可能为另一颗白矮星或中子星，同时存在三体系统的可能性。


<details>
  <summary>Details</summary>

Background: 球状星团核心区域的双星系统（binary system）通过动力学相互作用阻止星团引力坍缩，其中NGC 6397核心区域预测存在大量白矮星（white dwarf）组成的双星系统。

Data: 数据来源于MUSE径向速度巡天观测和哈勃太空望远镜（HST）多波段测光数据。

Method: 采用MUSE径向速度巡天观测和HST多波段测光数据的光谱能量分布拟合技术，结合双星轨道参数反演及三体模型假设检验。

Result: 在银河系球状星团NGC 6397中心发现了一个由两颗白矮星(WD)组成的致密双星系统NF1，其轨道周期为0.54天，径向速度半振幅达200 km/s，暗示不可见伴星最小质量为0.78 Msun（可能是另一颗白矮星或中子星）。

Abstract: Binaries in the cores of globular clusters are known to prevent the gravitational collapse of the cluster, and simulations predict that the core of NGC 6397 contains a large number of white dwarfs (WDs), of which many are expected to be part of a binary system. In this work, we report the discovery of a compact binary system consisting of two WDs in the centre of the Galactic globular cluster NGC 6397. The system, known in the literature as NF1, was observed as part of a MUSE radial-velocity survey aiming at characterizing the binary population in the centre of NGC 6397. The spectral analysis of NF1 provides an effective temperature of 16000 K and a surface gravity (log g) of 5.72 (cgs), which is consistent with an extremely low-mass He-core WD nature. This is further supported by the mass of 0.23 +/- 0.03 Msun obtained from fitting the star's spectral energy distribution using its HST magnitude in various filters. The system has a circular orbit with a period of 0.54 days. The radial velocities show a large semi-amplitude of 200 km/s, implying a minimum mass of 0.78 Msun for the invisible companion, which is likely another WD, or a neutron star if the inclination of the system is smaller than about 50 deg. Some significant residuals in radial velocity remain with our best orbital solution and we tested whether a model with a third body can explain these deviations. While this possibility seems promising, additional measurements are needed to confirm whether the star is actually part of a triple system.

</details>

### [48] [Is the nitrogen-rich source PN K4-47 a true planetary nebula?](https://arxiv.org/abs/2602.08959)
*Thomas Steinmetz,Tomasz Kaminski,David Jones,Marcin Hajduk,Denise R. Goncalves,Stavros Akras*

**TL;DR**: 首次空间分辨观测行星状星云(PN) K4-47的亚毫米环境，发现其具有快速光学双极外流(约350 km/s)和慢速分子外流(50-60 km/s)，推断前身星为4-6倍太阳质量的J型碳AGB星，并通过尘埃消光和激波加热机制证实其为非典型但真实的行星状星云。


<details>
  <summary>Details</summary>

Background: 研究聚焦于行星状星云（planetary nebula）K4-47的非典型特性及其分子与尘埃环境的形成机制，探讨其是否属于特殊类型或典型行星状星云。

Data: 数据来源包括光学成像与光谱观测、亚毫米波和射电干涉测量（sub-millimetre and radio interferometry）。

Method: 采用光学成像与光谱学、亚毫米波与射电干涉测量（sub-millimetre and radio interferometry）技术，首次空间解析了K4-47的亚毫米波环境。

Result: 首次在亚毫米波段空间解析了K4-47的分子环境，发现其CO(2-1)发射沿与光学双极外流相似的方位角延伸。

Abstract: PN K4-47 is a young planetary nebula that exhibits shock-excited bipolar lobes and a complex molecular environment, with the highest number of molecules detected within a planetary nebula. It has been questioned whether K4-47 is indeed a `typical' planetary nebula, or may be more exotic in nature. We examine this question using optical imaging and spectroscopy, and sub-millimetre and radio interferometry. Our observations spatially resolve the sub-millimetre environment of K4-47 for the first time. We find elongated CO (2-1) emission along a similar PA to the optical bipolar outflow. We derive a distance upper limit of 6 kpc to the source. The source hosts a fast ($\sim$350 km s$^{-1}$) bipolar optical outflow, and a slow (~50-60 km s$^{-1}$) molecular outflow along a similar PA. The outflow velocity indicates an age of 336 $\pm$ 119 yr. We also find that the excitation temperature and density of the atomic gas is~20 kK and 2800 cm$^{-3}$, respectively. The elemental and isotopic enrichment of K4-47 infers an AGB progenitor mass of 4-6 M$_{\odot}$, which corresponds to a white dwarf mass of ~1 M$_{\odot}$, following the initial-final mass relation for white dwarfs. We find that the core of K4-47 must contain 10$^{-2}$ M$_{\odot}$ of dust to explain the extinction, and that photoionisation alone cannot explain the excitation of the atomic gas. We instead require an additional heating mechanism, with shocks a likely scenario. It is likely that the progenitor star of K4-47 was a J-type carbon AGB star, which formed the molecular and dusty circumstellar environment. The bipolar outflow is later triggered, punching through the circumstellar environment, producing shocks, and shaping the environment into the elongated structure seen in the sub-millimetre. We therefore classify K4-47 as a genuine, if unusual, planetary nebula.

</details>

<div id='hep-ex'></div>

# hep-ex [[Back]](#toc)

### [49] [Fabrication and characterization of AlMn alloy superconducting films for 0vbb experiments](https://arxiv.org/abs/2510.00641)
*Zhouhui Liu,Yifei Zhang,Yu Xu,Mengxian Zhang,Qing Yu,Xufang Li,He Gao,Zhengwei Li,Daikang Yan,Shibo Shu,Yongjie Zhang,Xuefeng Lu,Yu Wang,Jianjie Zhang,Yuanyuan Liu,Congzhan Liu*

**TL;DR**: 该研究通过制备和分析铝锰（AlMn）合金超导薄膜，确定了临界温度（Tc）与退火温度的关系及磁场对Tc的影响，发现AlMn薄膜的Tc可通过退火和磁场调控在10-20 mK范围内，且退火温度越高锰离子分布越均匀，为AlMn过渡边缘传感器（TES）应用于无中微子双贝塔衰变（0vbb）实验提供了关键支持。


<details>
  <summary>Details</summary>

Background: 研究中微子无中微子双贝塔衰变（0vbb）以揭示中微子特性并探索新物理。

Data: 使用铝锰（AlMn）合金超导薄膜作为过渡边缘传感器（TES）的主要组件，通过实验建立了临界温度（Tc）与退火温度的关系并测试了磁场对Tc的影响。

Method: 采用铝锰（AlMn）合金超导薄膜制备技术，通过调控退火温度和磁场条件研究临界温度（Tc）的可调性及其与锰离子分布均匀性的关系。

Result: AlMn合金超导薄膜的临界温度（Tc）可通过退火温度和磁场调控在10-20 mK范围内，且对垂直磁场敏感但平行磁场不敏感。

Abstract: Neutrinoless double-beta decay (0vbb) experiments constitute a pivotal probe for elucidating the characteristics of neutrinos and further discovering new physics. Compared to the neutron transmutation doped germanium thermistors (NTD-Ge) used in 0vbb experiments such as CUORE, transition edge sensors (TES) theoretically have a relatively faster response time and higher energy resolution. These make TES detectors good choice for next generation 0vbb experiments. In this paper, AlMn alloy superconducting films, the main components of TES, were prepared and studied. The relationship between critical temperature (Tc) and annealing temperature was established, and the impact of magnetic field on Tc was tested. The experimental results demonstrate that the Tc of AlMn film can be tuned in the required range of 10 - 20 mK by using the above methods, which is a key step for the application of AlMn TES in 0vbb experiment. In the test range, the Tc of AlMn film is sensitive to out-of-plane magnetic field but not to the in-plane magnetic field. Furthermore, we find that a higher annealing temperature results in a more uniform distribution of Mn ions in depth, which opens a new avenue for elucidating the underlying mechanism for tuning Tc.

</details>

<div id='physics.ins-det'></div>

# physics.ins-det [[Back]](#toc)

### [50] [Fabrication and Characterization of X-ray TES Detectors Based on Annular AlMn Alloy Films](https://arxiv.org/abs/2510.00642)
*Yifei Zhang,Zhengwei Li,Mengxian Zhang,Guofu Liao,Zhouhui Liu,Yu Xu,Nan Li,Liangpeng Xie,Junjie Zhou,Xufang Li,He Gao,Shibo Shu,Yongping Li,Yudong Gu,Daikang Yan,Xuefeng Lu,Hua Feng,Yongjie Zhang,Congzhan Liu*

**TL;DR**: 该论文开发了基于环形铝锰（AlMn）合金薄膜的X射线超导转变边缘传感器（TES）探测器，实验表明三个环形TES探测器具有高度一致的电流-电压特性，其中最小吸收体的探测器在5.9 keV能量下实现了11.0 eV的最佳能量分辨率，但该值低于理论预期，主要归因于读出电子学噪声大于预期。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发基于环形铝锰合金薄膜（AlMn alloy films）的超导转变边缘传感器（TES）用于X射线探测，填补了该材料在X射线或伽马射线检测领域的应用空白。

Data: 研究中使用的数据来源于在稀释制冷机(dilution refrigerator)中评估的三个环形铝锰合金(AlMn)薄膜TES(transition edge sensor)样品的I-V特性测量结果。

Method: 采用环形铝锰（AlMn）合金薄膜制备超导转变边缘传感器（TES），并在稀释制冷机中评估其特性。

Result: 基于独特环形铝锰(AlMn)薄膜的X射线超导转变边缘传感器(TES)探测器实现了11.0 eV @ 5.9 keV的最佳能量分辨率，但低于理论预期值。

Abstract: AlMn alloy flms are widely fabricated into superconducting transition edge sensors (TESs) for the detection of cosmic microwave background radiation. However, the application in X-ray or gamma-ray detection based on AlMn TES is rarely reported. In this study, X-ray TES detectors based on unique annular AlMn flms are devel-oped. The fabrication processes of TES detectors are introduced in detail. The char-acteristics of three TES samples are evaluated in a dilution refrigerator. The results demonstrate that the I-V characteristics of the three annular TES detectors are highly consistent. The TES detector with the smallest absorber achieved the best energy resolution of 11.0 eV @ 5.9 keV, which is inferior to the theoretical value. The dis-crepancy is mainly attributed to the larger readout electronics noise than expected.

</details>

<div id='gr-qc'></div>

# gr-qc [[Back]](#toc)

### [51] [Accelerating Black Hole Image Generation via Latent Space Diffusion Models](https://arxiv.org/abs/2602.07786)
*Ao Liu,Xudong Zhang,Cuihong Wen,Wentao Liu,Jieci Wang*

**TL;DR**: 提出了一种基于物理条件约束的低维扩散模型，通过从磁化吸积流合成黑洞图像并捕捉其低维流形特征，在显著降低计算成本的同时高精度复现了广义相对论光线追踪（GRRT）模拟的关键观测特征。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用低维流形和物理条件扩散模型替代传统计算密集型广义相对论性光线追踪（GRRT）模拟，以高效生成高保真黑洞图像并推动强场引力高精度测试。

Data: 研究中使用的数据来源于基于磁化吸积流（magnetized accretion flows）的全广义相对论性光线追踪（GRRT）模拟生成的合成黑洞图像。

Method: 采用物理条件扩散模型（physics-conditioned diffusion model）在紧凑潜在空间中直接从物理参数生成高保真黑洞图像。

Result: 基于物理参数的低维流形结构，通过扩散模型在紧凑潜在空间中直接生成高保真黑洞图像，显著降低计算成本并保持关键观测特征精度。

Abstract: Interpreting horizon-scale black hole images currently relies on computationally intensive General Relativistic Ray Tracing (GRRT) simulations, which pose a significant bottleneck for rapid parameter exploration and high-precision tests of strong-field gravity. We demonstrate that physically accurate black hole images, synthesized from magnetized accretion flows, inherently reside on a low-dimensional manifold-encoding the essential features of spacetime geometry, plasma distribution, and relativistic emission. Leveraging this structure, we introduce a physics-conditioned diffusion model that operates in a compact latent space to generate high-fidelity black hole imagery directly from physical parameters. The model accurately reproduces critical observational signatures from full GRRT simulations-such as shadow diameter, photon-ring structure, and relativistic brightness asymmetry-while achieving over a fourfold reduction in computational expense. Compared with the previous generation of denoising diffusion models, the proposed approach achieves significant improvements in image quality, reconstruction fidelity, and parameter estimation accuracy, while reducing the average inference time per black hole image from 5.25 seconds to 1.15 seconds. Our work establishes diffusion-based latent models as efficient and scalable substitutes for traditional radiative transfer solvers, offering a practical framework toward real-time modeling and inference for next-generation black hole imaging.

</details>

### [52] [Can UV meet IR in the Swiss cheese?](https://arxiv.org/abs/2602.08487)
*Madina Abilmazhinova,Diana Kulubayeva,Hrishikesh Chakrabarty,Daniele Malafarina*

**TL;DR**: 论文核心贡献是研究了正则黑洞(regular black hole)在膨胀宇宙中的嵌入效应，发现其紫外修正(UV modifications)对宇宙膨胀速率的影响可区分不同几何模型，且最佳拟合参数对应于无视界致密天体(horizonless compact objects)。


<details>
  <summary>Details</summary>

Background: 研究探讨了将规则黑洞（regular black holes）嵌入膨胀宇宙中，并分析其紫外修正如何影响外部宇宙膨胀速率。

Data: 数据来源于对多个正则黑洞（regular black hole）几何模型的观测约束分析，包括对紫外截断参数（UV cutoff parameter）的观测拟合。

Method: 采用多种正则黑洞（regular black hole）几何模型，通过弗里德曼方程（Friedmann equations）分析紫外修正对宇宙膨胀速率的影响，并利用观测数据约束紫外截断参数（UV cutoff parameter）。

Result: 紫外修正的黑洞几何可能解释当前宇宙加速膨胀阶段，最佳拟合参数值对应于无规则视界的致密天体(horizonless compact objects)。

Abstract: We consider the embedding of regular black holes in an expanding universe and study how the ultraviolet modifications to the Schwarzschild geometry that regularize the black hole singularity affect the exterior universe's expansion rate. We consider several proposals for the regular black hole geometry and obtain the corresponding Friedmann equations for a universe filled only with dust and black holes. We show that different proposals have different implications which may be distinguished. We then test the hypothesis that the UV corrections to the black hole geometry may be responsible for the current phase of accelerated expansion. To this aim we constrain the value of the regular black hole UV cutoff parameter from observations. Interestingly we find that the best fit is obtained by values of the parameter corresponding to regular horizonless compact objects.

</details>

### [53] [Dynamic Black-hole Emission Tomography with Physics-informed Neural Fields](https://arxiv.org/abs/2602.08029)
*Berthy T. Feng,Andrew A. Chael,David Bromley,Aviad Levis,William T. Freeman,Katherine L. Bouman*

**TL;DR**: 提出了一种基于物理信息（PI-DEF）的可微分神经渲染方法，通过联合重建黑洞周围气体的4D（时间+3D）发射率场和3D速度场，显著提高了动态三维成像的精度，并能估计黑洞自旋等物理参数。


<details>
  <summary>Details</summary>

Background: 动态黑洞成像（dynamic black-hole imaging）是黑洞静态成像成功后的前沿研究方向，旨在揭示黑洞附近三维气体（3D gas）动力学过程以探索新物理模型。

Data: 研究中使用的数据来源为事件视界望远镜（EHT）的稀疏射电测量（sparse radio measurements）和模拟数据（simulated data）。

Method: 采用基于可微分神经渲染（differentiable neural rendering）的物理信息方法（PI-DEF）联合重建4D（时间+3D）发射率场和3D速度场，并将速度作为发射率动态的软约束。

Result: 提出的PI-DEF方法通过结合可微分神经渲染和物理约束，显著提升了黑洞周围动态3D气体（emissivity field）和速度场的重建精度，并能估计黑洞自旋等物理参数。

Abstract: With the success of static black-hole imaging, the next frontier is the dynamic and 3D imaging of black holes. Recovering the dynamic 3D gas near a black hole would reveal previously-unseen parts of the universe and inform new physics models. However, only sparse radio measurements from a single viewpoint are possible, making the dynamic 3D reconstruction problem significantly ill-posed. Previously, BH-NeRF addressed the ill-posed problem by assuming Keplerian dynamics of the gas, but this assumption breaks down near the black hole, where the strong gravitational pull of the black hole and increased electromagnetic activity complicate fluid dynamics. To overcome the restrictive assumptions of BH-NeRF, we propose PI-DEF, a physics-informed approach that uses differentiable neural rendering to fit a 4D (time + 3D) emissivity field given EHT measurements. Our approach jointly reconstructs the 3D velocity field with the 4D emissivity field and enforces the velocity as a soft constraint on the dynamics of the emissivity. In experiments on simulated data, we find significantly improved reconstruction accuracy over both BH-NeRF and a physics-agnostic approach. We demonstrate how our method may be used to estimate other physics parameters of the black hole, such as its spin.

</details>

### [54] [Thermal Vacuum Cosmology Explains Hubble Tension](https://arxiv.org/abs/2602.08522)
*Robert Alicki*

**TL;DR**: 提出用吉布斯-霍金温度(Gibbons-Hawking temperature)描述的膨胀真空热能量替代宇宙学常数，解释了哈勃张力(Hubble tension)的起源。


<details>
  <summary>Details</summary>

Background: 该研究探讨了用吉本斯-霍金温度(Gibbons-Hawking temperature)描述的膨胀真空热能量替代宇宙学常数(cosmological constant)的修正平坦暴胀ΛCDM模型，以解释著名的"哈勃张力"(Hubble tension)问题。

Data: 数据来源于标准（平坦）暴胀ΛCDM模型的修改版本，其中宇宙常数被具有吉本斯-霍金温度（Gibbons-Hawking temperature）的膨胀真空热能所替代。

Method: 采用修正的暴胀ΛCDM模型，将宇宙常数替换为具有吉本斯-霍金温度(Gibbons-Hawking temperature)的膨胀真空热能量。

Result: 热膨胀真空的吉本斯-霍金温度(Gibbons-Hawking temperature)取代宇宙学常数(cosmological constant)的修正模型解释了哈勃张力(Hubble tension)的起源。

Abstract: It is argued that the previously proposed modification of the standard (flat) inflationary $ΛCDM$ model in which cosmological constant is replaced by thermal energy of expanding vacum, characterized by the Gibbons-Hawking temperature, explains the origin of notorious ``Hubble tension''.

</details>

### [55] [Post-Newtonian accelerations of a Mercury orbiter](https://arxiv.org/abs/2602.08720)
*Miriam Falletta,Gabriel Rodríguez-Moris,Sergei A. Klioner*

**TL;DR**: 该论文建立了水星(Mercury)中心局部坐标系下的航天器相对论运动模型，量化分析了后牛顿(post-Newtonian)摄动项（包括史瓦西(Schwarzschild)项、兰斯-蒂林(Lense-Thirring)进动、四极矩加速度以及第三体引力效应）对贝皮科伦坡(BepiColombo)任务轨道的影响，提出了适用于水星轨道器的高精度相对论轨道建模方法。


<details>
  <summary>Details</summary>

Background: 研究水星（Mercury）后牛顿（post-Newtonian）局部坐标系中航天器运动的相对论建模。

Data: 数据来源于BepiColombo任务中两艘航天器的轨道轨迹数据。

Method: 采用后牛顿（post-Newtonian）框架下的水星中心局部坐标系，通过分析史瓦西项（Schwarzschild term）、兰斯-蒂林进动（Lense-Thirring precession）、四极矩加速度以及相对论性第三体摄动（包括引力电和引力磁加速度）构建高精度轨道模型。

Result: 水星（Mercury）的后牛顿局部坐标系中航天器运动模型需同时考虑局部相对论扰动（包括史瓦西项、兰斯-蒂林进动和四极矩加速度）与第三体相对论扰动（引力电和引力磁加速度及耦合项）。

Abstract: We investigate the relativistic modeling of spacecraft motion in Mercury's post-Newtonian local coordinates. This investigation is motivated by the fact that Mercury's post-Newtonian gravitational field (as well as that of any other planet) admits an expansion in terms of multipole moments, which are most appropriately defined in the local reference system. The equations of motion in the Mercury-centric local frame include relativistic local perturbations, given by the Schwarzschild term, Lense-Thirring precession, and the acceleration due to the quadrupole moment, and relativistic third-body perturbations, which are the gravito-electric and gravito-magnetic accelerations, along with a coupling term between Mercury and other solar system bodies. The relativistic third-body perturbations are usually neglected in all practical applications. In this study, we analyze the magnitude of the post-Newtonian terms of the equations of motion formulated in the Mercury-centric frame, evaluating them along the trajectories of the two BepiColombo spacecrafts. Based on this analysis, we provide a practical approach for constructing a high-accuracy relativistic orbital model suitable for a Mercury orbiter.

</details>

<div id='stat.CO'></div>

# stat.CO [[Back]](#toc)

### [56] [SunBURST: Deterministic GPU-Accelerated Bayesian Evidence via Mode-Centric Laplace Integration](https://arxiv.org/abs/2601.19957)
*Ira Wolfson*

**TL;DR**: 该论文提出了SunBURST算法（一种基于GPU的确定性贝叶斯证据计算方法），通过模式中心几何积分替代全局体积探索，在高达1024维的高斯或近高斯后验分布中实现了双精度容差的数值一致性，并展现出亚线性计算时间扩展特性。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于高维贝叶斯证据计算(Bayesian evidence evaluation)的计算效率问题，针对物理参数估计和反问题中后验分布呈现局部高斯结构的特点，提出了一种确定性GPU原生算法以突破传统采样方法的维度诅咒和序列计算限制。

Data: data：SunBURST算法使用GPU原生并行计算处理高维数据，测试中包括高斯和近高斯后验（Gaussian and near-Gaussian posteriors）以及多峰高斯混合（multimodal Gaussian mixtures）的仿真基准数据（benchmarks）。

Method: 采用确定性GPU原生算法SunBURST，结合径向模态发现、批量L-BFGS优化和基于拉普拉斯近似的解析积分，通过模态中心几何积分替代全局体积探索。

Result: SunBURST通过确定性GPU原生算法（结合径向模式发现、批量L-BFGS优化和拉普拉斯解析积分）在高斯或近高斯后验下实现了1024维度的双精度容差计算，并在多峰高斯混合中保持亚百分比精度和良好扩展性。

Abstract: Bayesian evidence evaluation becomes computationally prohibitive in high dimensions due to the curse of dimensionality and the sequential nature of sampling-based methods. We introduce SunBURST, a deterministic GPU-native algorithm for Bayesian evidence calculation that replaces global volume exploration with mode-centric geometric integration. The pipeline combines radial mode discovery, batched L-BFGS refinement, and Laplace-based analytic integration, treating modes independently and converting large batches of likelihood evaluations into massively parallel GPU workloads.
  For Gaussian and near-Gaussian posteriors, where the Laplace approximation is exact or highly accurate, SunBURST achieves numerical agreement at double-precision tolerance in dimensions up to 1024 in our benchmarks, with sub-linear wall-clock scaling across the tested range. In multimodal Gaussian mixtures, conservative configurations yield sub-percent accuracy while maintaining favorable scaling.
  SunBURST is not intended as a universal replacement for sampling-based inference. Its design targets regimes common in physical parameter estimation and inverse problems, where posterior mass is locally well approximated by Gaussian structure around a finite number of modes. In strongly non-Gaussian settings, the method can serve as a fast geometry-aware evidence estimator or as a preprocessing stage for hybrid workflows. These results show that high-precision Bayesian evidence evaluation can be made computationally tractable in very high dimensions through deterministic integration combined with massive parallelism.

</details>

<div id='astro-ph.HE'></div>

# astro-ph.HE [[Back]](#toc)

### [57] [Effects of Viscosity on Sloshing Cold Fronts in Galaxy Clusters](https://arxiv.org/abs/2602.07661)
*Ming-Hsueh Hsieh,H. -Y. Karen Yang,John ZuHone*

**TL;DR**: 通过模拟星系团合并中四种不同星系团内介质(ICM)黏度模型和磁场强度变化，发现各向同性斯必泽黏度(Spitzer viscosity)显著抑制开尔文-亥姆霍兹不稳定性(KHI)形成光滑冷锋面，而微不稳定性限制的布拉金斯基黏度(Braginskii viscosity)则产生明显波纹，表明黏度和磁场共同稳定星系团内冷锋面的关键作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系团内介质（intracluster medium, ICM）的粘滞特性及其与磁场共同作用对冷锋（cold fronts）稳定性的影响机制。

Data: 数据来源于数值模拟（simulation），包括四种星系团内介质（ICM）黏度模型（A无黏性、B各向同性Spitzer黏性、C各向异性Braginskii黏性、D受微观不稳定性限制的Braginskii黏性）和两种等离子体β参数（β≈100和1600）条件下的星系团并合过程。

Method: 采用四种ICM黏度模型（无黏性、各向同性斯必泽黏度(Spitzer viscosity)、各向异性布拉金斯基黏度(Braginskii viscosity)及受微观不稳定性限制的布拉金斯基黏度）进行星系团合并模拟，并考察等离子体β参数（β≈100和1600）对磁流体动力学稳定性的影响。

Result: 各向同性斯皮策黏度（Spitzer viscosity）模型显著抑制开尔文-亥姆霍兹不稳定性（KHI），形成光滑冷锋面，而无黏度和微不稳定性受限模型则产生明显波纹。

Abstract: The viscous properties of the intracluster medium (ICM) remain poorly constrained. Cold fronts-sharp discontinuities formed during cluster mergers-offer a potential avenue to probe the effective viscosity of the ICM. Velocity shear across these fronts should generate Kelvin-Helmholtz instabilities (KHI), unless viscosity or magnetic tension suppresses them. We perform cluster merger simulations incorporating four ICM viscosity models: (A) inviscid, (B) isotropic Spitzer viscosity, (C) anisotropic Braginskii viscosity, and (D) Braginskii viscosity limited by microinstabilities. The isotropic Spitzer viscosity (case B) strongly suppresses KHI, producing smooth cold front surfaces, while the inviscid (A) and microinstability-limited (D) cases show prominent ripples. The Braginskii case (C) yields intermediate suppression. We also vary the plasma $β$ parameter ($β\approx$ 100 and 1600) to examine how a changing magnetic field strength affects the results. Stronger magnetic fields further suppress KHI, leading to smoother fronts and reduced differences between different viscosity models, while also widening the range of permitted pressure anisotropies when microinstability-based limiters are present. These results indicate that both viscosity and magnetic fields play crucial roles in stabilising sloshing cold fronts in galaxy clusters.

</details>

### [58] [Identifying Host Galaxies of Binary Black Hole Mergers with Next-Generation Gravitational Wave Detector Networks](https://arxiv.org/abs/2602.08459)
*Sumedha Biswas,Andrew Levan,Peter G. Jonker,Kendall Ackley,Gregory Ashton,Nikhil Sarin*

**TL;DR**: 通过模拟未来引力波探测器网络（包括LIGO-India、爱因斯坦望远镜和宇宙探索者）对近邻（z<0.25）双黑洞并合事件的定位能力，提出了一套基于星系恒星质量函数、金属丰度阈值和偶然对齐概率的诊断框架，证明下一代网络可将双黑洞并合定位到理论阈值体积内（最远约1000 Mpc，年率约100次），为群体水平上限制双黑洞形成通道提供新方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用未来引力波（GW）探测器网络（包括LIGO-India、爱因斯坦望远镜（ET）和宇宙探索者（CE））识别双黑洞（BBH）并合宿主星系，以约束BBH形成通道并测量哈勃常数（H0）等宇宙学参数。

Data: 数据来源于模拟的双黑洞（BBH）并合事件，包括不同质量、距离和方向敏感性的变化，并利用基于费希尔信息矩阵（FIM）的参数估计方法通过BILBY推断定位体积。

Method: 采用基于费希尔信息矩阵（Fisher Information Matrix, FIM）的参数估计方法，通过BILBY软件实现双黑洞并合事件的定位体积推断。

Result: 未来包含爱因斯坦望远镜（ET）和宇宙探索者（CE）的引力波探测器网络能将双黑洞（BBH）并合事件定位到小于理论阈值的体积内，在约1000兆秒差距范围内以每年约100次的频率实现潜在唯一宿主星系认证。

Abstract: Identifying the host galaxy of a binary black hole (BBH) merger detected via gravitational waves (GWs) remains a challenge due to the absence of electromagnetic counterparts and the large localization volumes produced by current-generation detectors. A confident host association would provide stellar population properties to constrain BBH formation channels and enable measurements of cosmological parameters such as the Hubble constant, H0. We simulate BBH mergers in nearby (z<0.25) host galaxies to evaluate the feasibility of host identification with future GW detector networks, including configurations with the planned LIGO-India detector and third-generation detectors such as the Einstein Telescope (ET) and Cosmic Explorer (CE). We construct two injection grids to explore variations in BBH mass, distance, and directional sensitivity, and infer localization volumes using the Fisher Information Matrix (FIM)-based parameter estimation implemented through BILBY. To assess the prospects for unique host identification, we introduce a set of diagnostics: theoretical comoving volume thresholds for galaxies of a given stellar mass, derived from galaxy stellar mass functions, a metallicity-based volume threshold motivated by progenitor environment models, stellar mass fractions to quantify candidate host prominence, and the probability of chance alignment (p_c). These metrics provide ways to evaluate host associations and constrain BBH formation channels. We find that future networks that include ET and CE localize BBH mergers to volumes smaller than those theoretical thresholds, implying potentially unique host identification, out to ~1000 Mpc at a rate of ~100 yr^{-1}. While associations for individual events may remain uncertain, our framework is well-suited to population-level analyses, enabling constraints on BBH formation scenarios in the era of next-generation GW detector networks.

</details>

### [59] [Pulsars and Millisecond Pulsars I: Advancements, Open Questions and finding Gaps via statistical insights](https://arxiv.org/abs/2602.08065)
*Maria Rah,Areg Mickaelian,Francesco Flammini Dotti,Rainer Spurzem*

**TL;DR**: 该研究通过多波段观测分析银河系和球状星团中脉冲星（pulsars）和毫秒脉冲星（millisecond pulsars, MSPs）的辐射特性、计时行为和空间分布，结合数值模拟揭示了动力学相互作用和双星演化对致密天体性质的影响，并指出需改进建模框架以解决双星相互作用、磁场演化及脉冲星物理融入大规模模拟等开放问题。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系场和球状星团中脉冲星（pulsars）和毫秒脉冲星（millisecond pulsars, MSPs）的统计特性及其与磁场-自旋关系的理论模型。

Data: 数据来源包括银河系场和球状星团（Globular Clusters）中的多波段（multiwavelength）观测数据。

Method: 采用数值工具NBODY6++GPU、CMC和COMPASS研究动力学相互作用和双星演化对致密天体特性的影响。

Result: 需要改进建模框架以更好地理解脉冲星和毫秒脉冲星（MSPs）的形成路径和长期演化。

Abstract: We present a statistical study of pulsars and millisecond pulsars (MSPs) based on multiwavelength observations in the Galactic Field and Globular Clusters. We examine their emission properties, timing behavior, and spatial distributions, and discuss how theoretical models are required to interpret these observational trends. We focus on the magnetic field spin relation, including spin up through accretion in binaries and spin down driven by magnetic dipole radiation. Using numerical tools such as NBODY6++GPU, CMC, and COMPASS, we explore how dynamical interactions and binary evolution shape the properties of compact objects. Despite major progress, several open questions remain regarding binary interactions, magnetic field evolution, and the incorporation of pulsar physics into large scale simulations. Our analysis highlights the need for improved modeling frameworks to better understand the formation pathways and long term evolution of pulsars and MSPs.

</details>

### [60] [Pulsars and Millisecond Pulsars II: Deep diving into the Evolutionary Mechanisms](https://arxiv.org/abs/2602.08106)
*Maria Rah,Rainer Spurzem,Francesco Flammini Dotti,Areg Mickaelian*

**TL;DR**: 研究了不同天体环境中脉冲星和毫秒脉冲星（MSPs）的演化机制，发现吸积、磁制动和动力学相互作用通过不同途径主导银河系场和球状星团中致密天体的长期演化。


<details>
  <summary>Details</summary>

Background: 研究主题为探究不同天体物理环境中脉冲星和毫秒脉冲星（MSPs）的演化机制及其物理过程。

Data: 数据来源包括观测约束和理论模型，并整合了NBODY6++GPU、CMC和COMPASS等数值工具。

Method: 采用观测约束与理论模型相结合的方法，重点分析吸积、磁制动和动力学相互作用对致密天体长期演化的影响，并整合NBODY6++GPU、CMC和COMPASS等数值工具进行种群研究。

Result: 不同天体物理环境下脉冲星（pulsars）和毫秒脉冲星（MSPs）的演化主要由吸积、磁制动（magnetic braking）和动力学相遇（dynamical encounters）共同驱动，且在银河系场（Galactic Field）和球状星团（Globular Clusters）中因恒星密度和相互作用率差异呈现显著分化。

Abstract: This second paper in our series investigates the evolutionary mechanisms that shape pulsars and millisecond pulsars (MSPs) across different astrophysical environments. We focus on the physical processes that govern spin evolution, magnetic field decay, binary interactions, and recycling pathways. Using observational constraints together with theoretical models, we examine how accretion, magnetic braking, and dynamical encounters contribute to the long-term evolution of compact objects. We also explore how these mechanisms differ between the Galactic Field and Globular Clusters, where stellar densities and interaction rates vary significantly. By integrating insights from population studies and numerical tools such as NBODY6++GPU, CMC, and COMPASS, we aim to clarify the dominant channels that lead to the formation and transformation of pulsars and MSPs. Our results highlight key uncertainties in current models and outline the physical ingredients required for more accurate simulations of compact object evolution.

</details>

### [61] [A comprehensive catalogue of high-mass X-ray binaries in the Large Magellanic Cloud detected during the first eROSITA all-sky survey](https://arxiv.org/abs/2602.08152)
*D. Kaltenbrunner,C. Maitra,F. Haberl,J. Bodensteiner,D. Bogensberger,D. A. H. Buckley,M. R. L. Cioni,J. Greiner,I. Monageng,A. Udalski,G. Vasilopoulos,R. Willer*

**TL;DR**: 基于SRG/eROSITA全天巡天数据首次对大小麦哲伦云（LMC）中高质量X射线双星（HMXB）进行全覆盖普查，构建了包含53个天体（含28个确认HMXB和21个新发现）的流量限制星表，发现3颗可能具有白矮星（WD）伴星的Be星及候选超巨星快速X射线暂现源，并据此估算LMC恒星形成率为(0.22±0.07) M⊙/年。


<details>
  <summary>Details</summary>

Background: 研究主题为利用SRG/eROSITA全天巡天数据对大麦哲伦云（Large Magellanic Cloud）中的高质量X射线双星（HMXB）群体进行全面普查，填补其完整样本的空白。

Data: 数据来源于SRG/eROSITA全天巡天（eROSITA all-sky survey）的首次探测数据，并结合光学和红外波段星表进行交叉匹配。

Method: 基于SRG/eROSITA全天巡天数据，通过X射线位置与光学/红外星表的交叉匹配，结合多波段后续观测和已知大麦哲伦云(LMC)高质量X射线双星(HMXB)特性，采用置信度分类标志(flags)方法构建了流量限制样本目录。

Result: 基于SRG/eROSITA全天巡天数据构建的流量限制目录首次完整覆盖大麦哲伦云(LMC)，共识别53个候选体（含28个确认的高质量X射线双星(HMXB)和21个新发现源），并据此估算LMC恒星形成率为$(0.22^{+0.06}_{-0.07})$ M$_{\odot}$yr$^{-1}$。

Abstract: The Magellanic Clouds, the closest star-forming galaxies to the Milky Way, offer an excellent environment to study high-mass X-ray binaries. While the Small Magellanic Cloud has been thoroughly investigated with over 120 systems identified, the Large Magellanic Cloud has lacked a complete survey due to its large angular size. Most prior studies targeted central or high-star-formation regions. The SRG/eROSITA all-sky surveys now enable a comprehensive coverage of the LMC, particularly due to its close vicinity to the south ecliptic pole. This work aims to improve our understanding of the HMXB population in the LMC by building a flux-limited catalogue. This allows us to compare sample properties with those of HMXB populations in other nearby galaxies. Using detections during the first eROSITA all-sky survey, we cross-matched X-ray positions with optical and infrared catalogues to identify candidate HMXBs. We assigned flags based on multi-wavelength follow-up observations and archival data, using properties of known LMC HMXBs. These flags defined confidence classes for our candidates. We detect sources down to X-ray luminosities of a few $10^{34}$ erg s$^{-1}$, resulting in a catalogue of 53 objects, including 28 confirmed HMXBs and 21 new eROSITA detections. We identify several likely supergiant systems, including a candidate supergiant fast X-ray transient with phase-dependent flares. We find three Be stars with likely white dwarf companions. Two of the Be/WD candidates show steady luminosities across four eROSITA scans, unlike the post-nova states seen in the majority of previous Be/WD reports. Our catalogue is the first to cover the entire LMC since the ROSAT era, providing a basis for statistical population studies. Using the HMXB population, we estimate the LMC star-formation rate to be $(0.22^{+0.06}_{-0.07})$ M$_{\odot}$yr$^{-1}$, which is in agreement with other tracers.

</details>

### [62] [Seasonal Variation of Polar Ice: Implications for Ultrahigh Energy Neutrino Detectors](https://arxiv.org/abs/2602.08921)
*Alexander Kyriacou,Steven Prohira,Dave Besson*

**TL;DR**: 研究发现极地粒雪（firn）密度季节性变化导致冰中微子（neutrino）诱发的无线电信号振幅和传播时间在海拔约3000米处产生约10%的季节性波动，进而对基于冰介质探测器重建中微子能量和到达方向引入不可消除的背景不确定性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于极地冰盖表层100-150米厚度的粒雪层(firn)密度季节性变化对中微子(neutrino)探测中射频信号传播特性的影响。

Data: 数据来源为基于海拔约3000米处极地粒雪（firn）的射频（RF）模拟（simulation）数据。

Method: 采用射频（RF）模拟方法分析极地粒雪（firn）密度异常对中微子诱导射电信号传播特性的影响。

Result: 极地粒雪层(firn)密度异常导致冰中中微子诱发无线电信号在海拔约3000米处传播时幅度和传播时间出现季节性变化，其中上层约15米粒雪层折射信号接收功率的季节性变化约为10%。

Abstract: The upper $100 \, \mathrm{m}$ to $150 \, \mathrm{m}$ of the polar ice sheet, called the firn, has a time-dependent density due to seasonal variations in the surface temperature and snow accumulation. We present RF simulations of an in-ice neutrino-induced radio source that show that these density anomalies create variations in the amplitude and propagation times of radio signals propagating through polar firn at an altitude of ${\sim}3000 \, \mathrm{m}$ above sea level. The received power from signals generated in the ice that refract within the upper ${\sim} 15 \, \mathrm{m}$ firn are subject to a seasonal variation on the order of 10\%. These variations result in an irreducible background uncertainty on the reconstructed neutrino energy and arrival direction for detectors using ice as a detection medium.

</details>

<div id='astro-ph.EP'></div>

# astro-ph.EP [[Back]](#toc)

### [63] [A New Strategy for Using Spectroscopic Phase Curves to Characterize Non-Transiting Planets](https://arxiv.org/abs/2602.07127)
*Ted M. Johnson,Avi M. Mandell*

**TL;DR**: 提出了一种无需凌星或恒星光谱模型的系外行星光谱相位曲线分析方法VPIE（Variable Planetary Infrared Excess），通过线性组合少量光谱数据提取行星辐射信息，成功应用于三颗M型矮星周围行星（TOI-519 b、GJ 876 d、Proxima Centauri b）的模拟观测，证明其能区分不同大气环流模式并约束行星半径。


<details>
  <summary>Details</summary>

Background: 开发了一种无需凌星或恒星光谱模型即可从系外行星光谱相位曲线中提取行星辐射信息的新方法（VPIE）。

Data: 数据来源于模拟的詹姆斯·韦伯太空望远镜（JWST）对三颗已知系外行星（TOI-519 b、GJ 876 d和Proxima Centauri b）的观测。

Method: 采用可变行星红外超量（VPIE）方法，通过线性组合少量个体光谱构建恒星贡献的经验模型，归一化各时间步光谱以在残差数据中保留行星通量印记。

Result: 提出了一种无需凌星或恒星光谱模型即可提取系外行星（exoplanet）通量贡献的新方法——可变行星红外超量（VPIE）法，能有效区分不同大气环流模式并约束非均一日夜温度比下的行星半径。

Abstract: We introduce a new time-series analysis strategy for combined-light exoplanet spectroscopic phase curves called the Variable Planetary Infrared Excess (VPIE) method. VPIE can be used to extract information about the planetary flux contribution without the need for the planet to transit, or use of a stellar spectral model. VPIE utilizes a linear combination of a small set of individual spectra to produce an empirical model of the stellar contribution at each time step, thereby normalizing each spectrum and leaving only an imprint of the planet's flux in the residual data. We demonstrate the effectiveness of VPIE through simulated James Webb Space Telescope (JWST) observations of three known exoplanet orbiting late-type M stars: the warm giant TOI-519 b, the warm sub-Neptune GJ 876 d, and the temperate super-Earth Proxima Centauri b. Our results indicate that though VPIE loses sensitivity for very high redistribution values, it can successfully distinguish between various atmospheric circulation regimes (zero, moderate, or high heat redistribution) and constrain planetary radii for non-unity day-night temperature ratios. While performance for cooler targets may be limited by JWST spectroscopic capabilities at longer wavelengths, future VPIE improvements or new instrumentation could enable characterization of potentially habitable planets. VPIE offers a promising new framework for pulling back the veil on the population of non-transiting planets around nearby M-stars that are otherwise inaccessible to current techniques.

</details>

### [64] [Dynamical Mass Constraints on Transition Disk Perturbers with the G23H Catalog](https://arxiv.org/abs/2602.07731)
*Dori Blakely,William Thompson,Doug Johnstone,Jessica Speedie,Jerry W. Xuan,Simon Blouin,Jingwen Zhang,Jean-Baptiste Ruffio,Eric Nielsen,Brendan P. Bowler,Kyle Franson,William Roberson,Ryan Cloutier,Andre Fogal,Kaitlyn Hessel,Christian Marois,Alexandra Rochon*

**TL;DR**: 利用校准的依巴谷(Hipparcos)和盖亚(Gaia)绝对天体测量数据，在11个过渡盘(transition disk)系统中检测到7个伴星（其中3个显著），包括HD 142527 B、AB Aurigae的亚恒星(sub-stellar)伴星及MWC 758的可能亚恒星伴星，并排除了4个系统中特定质量与轨道范围的伴星存在。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用依巴谷(Hipparcos)和盖亚(Gaia)天体测量数据对过渡盘(transition disk)系统中的扰动天体进行动力学质量约束。

Data: 数据来源于校准后的依巴谷(Hipparcos)和盖亚(Gaia)绝对天体测量数据。

Method: 采用校准后的依巴谷(Hipparcos)和盖亚(Gaia)绝对天体测量数据结合的新型分析方法，对11个过渡盘系统中的扰动天体进行动力学质量约束。

Result: 在11个过渡盘（transition disk）系统中，通过结合依巴谷（Hipparcos）和盖亚（Gaia）绝对天体测量数据，明确检测到HD 142527 B、AB Aurigae和MWC 758三个系统的伴星，并在HD 97048和UX Tau A中发现了强有力证据，在HD 100546和CQ Tau中发现了中等证据。

Abstract: We present dynamical mass constraints on perturbers in 11 transition disk systems using a novel combination of calibrated Hipparcos and Gaia absolute astrometry data. Out of the sample of 11, we find support for companions in seven systems, with significant detections in three. These systems are: HD 142527, where we clearly detect the known low-mass stellar companion HD 142527 B; AB Aurigae, where we detect a low-mass stellar or sub-stellar companion; and MWC 758, where we detect a likely sub-stellar companion. We also find strong evidence of companions to HD 97048 and UX Tau A, and moderate evidence for companions to HD 100546 and CQ Tau. In the four systems with non-detections, we find no evidence for companions more massive than $\sim$6 $M_{\mathrm{Jup}}$ with a semi-major axis greater than 3 au for HD 100453, nor for companions more massive than $\sim$2 $M_{\mathrm{Jup}}$ with a semi-major axis greater than 2 au for TW Hya. We also find no evidence for stellar mass companions with semi-major axes between $\sim$3 and $\sim$25 au for both HD 34282 and RY Lup. In addition to our fiducial model, we perform cross validation between astrometry sources. By comparing results across models, we find tentative evidence of a short timescale excess astrometric noise that may impact some protoplanetary disk systems. We conclude with predictions for the prospects of making dynamical mass constraints on protoplanets in protoplanetary disk systems with Gaia data release 4 using detailed simulations of Gaia DR4 data of PDS 70 and WISPIT 2.

</details>

### [65] [JWST spectral retrieval of cold directly imaged planet WD0806 b and the first measurement of altitude-dependent K$_{zz}$ in exoplanet atmospheres](https://arxiv.org/abs/2602.08200)
*Ben W. P. Lew,Thomas Roellig,Natasha E. Batalha,Nicholas F. Wogan,Thomas Greene,Mark S. Marley,Jonathan J. Fortney,Jarron Leisenring,Doug Johnstone,Matthew De Furio,Klaus Hodapp,Charles Beichman,Marcia Rieke*

**TL;DR**: 发现围绕白矮星运行的冷系外行星WD0806 b（温度350K），利用JWST观测数据首次基于CO、CH4和CO2分子丰度测量了高度依赖的垂直混合（涡扩散系数从50巴的10^4 cm²/s降至20巴的10^2 cm²/s），并确定其具有高C/O比（0.76）、亚太阳金属丰度（[M/H]=-0.25）和近太阳C/S比（1.17倍太阳）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于围绕白矮星运行的罕见系外行星WD0806 b的大气成分与动力学特征。

Data: 数据来源于詹姆斯·韦伯太空望远镜(JWST)的近红外相机(NIRCam)的F150W2、F200W、F356W和F444W宽带测光数据，以及3-5微米波段的近红外光谱仪(NIRSpec)G395M光谱数据，并结合先前发表的中红外仪器(MIRI)数据。

Method: 基于开源PICASO软件开发的新型检索框架，包含加性和乘性系统参数，用于分析JWST获取的近红外光谱和测光数据。

Result: 首次基于CO、CH4和CO2分子丰度测量报告了随高度变化的垂直混合（即淬灭物种依赖的垂直混合）现象。

Abstract: WD0806 b is a rare exoplanet companion orbiting a white dwarf, currently with a projected orbital distance of 2500 au. The Spitzer mid-IR photometry suggests that the temperature is as cold as 350K, making it one of the coldest directly imaged exoplanets. In this paper, we present the Near-infrared Camera (NIRCam) F150W2, F200W, F356W, and F444W broadband photometry and a 3--5\um Near-Infrared spectroscopy (NIRSpec) G395M spectrum obtained with the James Webb Space Telescope (JWST). We develop a new retrieval framework based on the open-source PICASO software that includes additive and multiplicative systematic parameters. Our retrieval results reveal bounded abundances of H$_2$S, CO$_2$, CO, NH$_3$, H$_2$O, and CH$_4$. We present a new chemical analysis framework that utilizes retrieved abundances to measure altitude-dependent eddy diffusion coefficients (K$_{\mathrm zz}$) at multiple quenched pressures. We find that the eddy diffusion coefficients decrease from around $10^4$ to $10^2$ $\rm cm^2/s$ as the atmospheric pressure decreases from from 50 to 20 bars. To our knowledge, this is the first study to report altitude-dependent vertical mixing (or, equivalently, quenched-species-dependent vertical mixing) based on the measured molecular abundances of CO, CH$_4$, and CO$_2$. With the 1--21\um NIRCam, NIRSpec and the previously published MIRI data, we measure the bolometric luminosity to be log(L/L$_{\odot}$) = $-6.75\pm0.01$ and derive the mass to be $8\pm 1 \mathrm{M_J}$. The retrieval results suggest that \target has an elevated C/O ratio of 0.76, or 1.3$\times$ solar, sub-solar metallicity ([M/H ]= -0.25), and a nearly solar C/S ratio (1.17x solar).

</details>

### [66] [NovaMoon: A Strategic Lunar Reference Station for Positioning, Timing, and Largely Enhanced Science in the Earth-Moon System Serena](https://arxiv.org/abs/2602.08432)
*Serena Molli,Agnès Fienga,Pascale Defraigne,Krzysztof Sośnica,Luigi Cacciapuoti,Luca Porcelli,Lotfi Massarweh,Sara Bruni,Riccardo Pozzobon,Albert Roura,Francesco Vespe,Diego Blas,Ozgur Karatekin,Yoann Audet,Floor Melman,Richard Swinden,Javier Ventura-Traveset,Olivier Alibart,Marie-Christine Angonin,Daniel Arnold,Ruth Bamford,Emmanuele Battista,Marco Belloni,J. C. Berton,Orfeu Bertolami,Mathis Bloßfeld,Adrien Bourgoin,Giada Bargiacchi,Salvatore Buoninfante,Nicolò Burzillà,Roberto Campagnola,Paolo Cappuccio,Salvatore Capozziello,Giuseppe Cimò,Clément Courde,Rolf Dach,Mario Siciliani de Cumis,Simone Dell'Agnello,Fabrizio De Marchi,Valentina Galluzzi,Francesco Gini,Philipp Glaeser,Klaus Gwinner,Alex Guinard,Rüdiger Haas,Aurélien Hees,Hauke Hussmann,Luciano Iess,Alexander C. Jenkins,Siddarth K. Joshi,Maria Karbon,Sergei Klioner,Kaisa Laiho,Christophe Le Poncin-Lafitte,Marco Lucente,David Lucchesi,Riccardo March,Lucia McCallum,Jürgen Müller,Weijie Nie,Jillian Oduber,Roberto Peron,Francesco Picciariello,Théo Pichavant,Michael Plumaris,Eleonora Polini,Ana-Catalina Plesa,Dimitrios Psychas,Bernardino Quaranta,Nicolas Rambaux,Marc Rovira-Navarro,Francesco Santoli,Matthias Schartner,Florian Seitz,Ilaria Sesia,Yan Seyffert,Stefano Speretta,Tim Springer,Giuseppe Vallone,Paolo Villoresi,Sebastien Vincent-Bonnieu,Ben Wadsworth,Pierre Waller,Radosław Zajdel,Erik Schoenemann*

**TL;DR**: NovaMoon作为首个集成多种测距、跟踪和授时技术的月球站，通过协同使用激光反射器、甚长基线干涉仪发射器、导航信号接收器、高稳原子钟和直接对地无线电链路，实现了亚米至分米级定位精度，提升了月球参考系、自转参数和星历表精度，并为月球科学研究提供了首个长期稳定的时间参考基准。


<details>
  <summary>Details</summary>

Background: 月球探索的复兴及未来通信导航需求推动了建立精确、稳定、可互操作的月球大地测量（geodetic）与授时基础设施。

Data: 数据来源包括月球激光反射器（lunar laser retroreflector）、甚长基线干涉测量发射器（Very Long Baseline Interferometry transmitter）、兼容LunaNet标准的导航信号接收器、高稳定原子钟（high-stability atomic clocks）以及直接对地无线电链路（direct-to-Earth radio links）。

Method: 采用多技术协同定位（co-location）方法整合月球激光后向反射器（lunar laser retroreflector）、甚长基线干涉测量（VLBI）发射器、兼容LunaNet标准的导航信号接收器、高稳定原子钟（high-stability atomic clocks）及直接对地无线电链路（direct-to-Earth radio links）。

Result: NovaMoon（诺瓦月球）作为首个集成多种测距、跟踪和授时技术的月球站，通过协同使用激光反射器、甚长基线干涉测量发射器、导航信号接收器、高稳原子钟和直接对地无线电链路，实现了亚米至分米级定位精度，并为月球用户提供本地差分修正，确保位置和时间的精确稳定实现。

Abstract: The renewed interest in lunar exploration and the development of future lunar communication and navigation services highlight the need for a precise, stable, and interoperable geodetic and timing infrastructure on the Moon. NovaMoon, proposed as a scientific and navigation payload for ESA's Argonaut lander, is designed as a lunar-based local differential, geodetic, and timing station supporting both operational needs in the Moon's south polar region and a broad range of scientific investigations. The payload integrates a lunar laser retroreflector, a Very Long Baseline Interferometry transmitter, a receiver for navigation signals compatible with LunaNet standards, high-stability atomic clocks, and direct-to-Earth radio links -- making it the first lunar station to co-locate multiple ranging, tracking, and timing techniques. NovaMoon will enable sub-metre to decimetre positioning, provide local differential corrections for lunar users, and ensure an accurate and stable realisation of position and time. Preliminary simulation studies show that this multi-technique dataset improves the lunar reference frame, orientation and ephemerides, and estimates of interior parameters like tidal response and core properties. NovaMoon will also provide the first long-duration physical realisation of a lunar time reference. Beyond its primary goals, it supports improved cartography, precise surface geolocation, and higher-resolution topography, contributing to safer landings and operations. It also enables new tests of fundamental physics, including constraints on relativity and possible deviations from classical gravity.

</details>