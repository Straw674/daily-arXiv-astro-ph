...
**TL;DR**: 研究发现JWST观测到的致密红色天体（"Little Red Dots"）可能源于模糊暗物质（FDM）晕内快速重子流入导致的短暂遮蔽阶段，其核心尺度与粒子质量$m_{22}=2$（$10^{-22}$ eV量级）相符，且高柱密度（$N_H \ge 10^{24}$ cm$^{-2}$）环境暗示辐射损失主导的快速流入机制。


<details>
  <summary>Details</summary>

Background: 研究探讨了模糊暗物质（FDM）晕中深孤子核内快速重子流入导致的短暂遮蔽阶段与高红移（$z \ge 5$）致密红色源（"小红点"，LRDs）的潜在关联。

Data: 数据来源于JWST（James Webb Space Telescope）巡天观测发现的致密红色源（"Little Red Dots"）以及$512^3$伪谱薛定谔-泊松模拟（pseudo-spectral Schrödinger-Poisson simulations）的理想化孤子（soliton）合并过程。

Method: 采用伪谱（pseudo-spectral）薛定谔-泊松（Schrödinger-Poisson）模拟研究理想化孤子（soliton）合并过程，结合孤子尺寸标度关系与观测约束，推断模糊暗物质（FDM）粒子质量$m_{22}=2$作为基准值。

Result: LRDs可能反映了模糊暗物质(FDM)晕深层孤子核内快速重子流入相关的短暂遮蔽相。
...
**TL;DR**: 基于哈勃太空望远镜(HST)和詹姆斯·韦伯太空望远镜(JWST)的PHANGS观测数据，构建了包含97057颗质量≳10 M⊙红超巨星(RSG)的迄今最大样本，发现其空间分布与局域恒星形成率密度(FUV校正)呈强相关(rs∼0.82)，与近红外示踪的恒星质量密度呈弱相关(rs∼0.57)，并测得6-30 Myr恒星群体中每103.77±0.27 M⊙产生1颗RSG。


<details>
  <summary>Details</summary>

Background: 红超巨星(RSG)研究对理解超新星前身星、恒星族群、恒星演化、质量损失和尘埃产生具有重要意义。

Data: 数据来源于PHANGS HST+JWST Treasury项目中的19个星系，结合哈勃太空望远镜(HST)的F814W和詹姆斯·韦伯太空望远镜(JWST)的F200W测光数据，并参考PARSEC等时线预测的颜色和星等进行红超巨星(RSGs)筛选，同时利用GALEX+WISE的消光校正远紫外(FUV)数据以及WISE+Spitzer的近红外辐射数据作为辅助。

Method: 采用哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）的F814W和F200W测光数据，结合PARSEC等龄线模型预测的颜色和星等筛选红超巨星（RSGs）。

Result: 利用HST和JWST联合观测在19个星系中构建了包含97057颗红超巨星（RSG）的迄今最大单次巡天样本，其质量普遍超过10倍太阳质量。
...
**TL;DR**: 基于盖亚(Gaia) DR3数据对四个年轻疏散星团（NGC 663、NGC 2301、NGC 2384、NGC 7510）的综合分析揭示了其年龄（log t ~7.0-8.15）、消光（E(B-V)=0.093-1.24 mag）、质量函数斜率（α≈2.00-2.26）、总质量（486-3584 M⊙）及动力学状态（仅NGC 2301和NGC 2384b达到动力学弛豫），并证实NGC 2384a/b为光学双星对（相距0.55 kpc），所有星团均位于银河系薄盘内（轨道偏心度e≈0.003-0.014，垂直距离Zmax<0.142 kpc）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用盖亚(Gaia) DR3数据对四个年轻疏散星团（NGC 663、NGC 2301、NGC 2384和NGC 7510）进行天体测量和测光分析，以揭示其动力学状态、质量分布及银河系薄盘轨道特性。

Data: 数据来源于盖亚卫星第三次数据释放(Gaia DR3)的高精度天体测量和测光数据。

Method: 采用贝叶斯马尔可夫链蒙特卡洛（Bayesian MCMC）等时线拟合和UPMASK算法进行星团成员判定及参数估计。

Result: 基于贝叶斯MCMC等时线拟合（Bayesian MCMC isochrone fitting）得出四个年轻疏散星团的年龄范围为$\log t \sim 7.0$-$8.15$，不确定性为$\sim 0.11$-$0.18$。
...
### [4] [Little Red Dots: The Assembly of Early Supermassive Black Holes in the JWST Era](https://arxiv.org/abs/2601.00089)
*David D Vaida,Ryan Jeffrey Farber*

**TL;DR**: 詹姆斯·韦伯太空望远镜（JWST）发现的高红移"小红点"（LRD）可能是超大质量黑洞（SMBH）或致密恒星形成区，其观测特征为理解早期宇宙中异常大质量黑洞的形成机制提供了新线索。


<details>
  <summary>Details</summary>

Background: 詹姆斯韦伯太空望远镜（JWST）的观测挑战了现有宇宙学模型对高红移（high-redshift）天体（如大质量星系和超大质量黑洞）的预期，并发现了一类新型高红移天体“小红点”（LRD），其本质（恒星形成区或活动星系核）的争议为早期宇宙结构形成提供了关键线索。

Data: 数据来源包括詹姆斯·韦伯太空望远镜（JWST）的红外观测、亚毫米波段的尘埃质量探测、射电和X射线叠加分析的限制条件以及JWST提供的静止紫外和光学波段测量。

Method: 采用多波段观测数据（包括亚毫米波、射电、X射线及紫外光学波段）和叠加分析（stacking）技术对"小红点"（LRD）进行系统性研究。

Result: JWST发现的高红移天体“小红点”（LRD）可能代表超大质量黑洞（SMBH）或致密恒星形成区，其身份争议对理解早期宇宙中异常大质量黑洞的形成机制至关重要。
...
### [5] [Evolution of bar formation in galaxies over the last 6 giga-years](https://arxiv.org/abs/2601.00106)
*M. A. Chacón*

**TL;DR**: 通过分析棒旋星系比例与关键结构参数的关系，发现近域宇宙中棒结构比例随恒星质量和星系演化程度（红色）增加而升高，且盘主导结构（低B/T值）更易形成棒，而高红移星系因盘不稳定导致棒结构比例显著降低，揭示了盘稳定性对星系形态演化的关键作用。


<details>
  <summary>Details</summary>

Background: 研究探讨了在过去60亿年中盘星系（disk galaxies）棒结构（bar formation）的演化及其与恒星质量、颜色指数和核球-总质量比（B/T）的关系。

Data: 数据来源于对本地和遥远场星系（field galaxies）的代表性样本分析，结合基于椭圆等光轮廓（elliptical isophotes）和傅里叶分解（Fourier decomposition）的棒结构检测技术。

Method: 采用基于椭圆等照度线（elliptical isophotes）和傅里叶分解（Fourier decomposition）的棒结构检测技术分析棒旋星系比例随关键结构参数的变化。

Result: 棒状结构（bar）的形成与盘星系（disk galaxy）的稳定性密切相关，近域宇宙中棒状星系比例随恒星质量增加且在红移（redshift）较低、演化更成熟的星系中更高，而高红移星系棒状比例显著降低表明早期宇宙动力学不稳定性限制了棒的形成和维持。
...
### [6] [Comparison of Static and Evolving Potentials in the Orbital Dynamics of Globular Clusters in the Central Region of the Galaxy](https://arxiv.org/abs/2601.00280)
*Anisa Bajkova,Anton Smirnov,Vadim Bobylev*

**TL;DR**: 通过分析银河系中心区域45个球状星团(globular clusters)在静态和演化势场中的轨道运动，发现从静态势场过渡到演化势场时轨道动力学仅发生微小变化，证实了引力势成分质量和尺寸变化对轨道参数的影响在小银心距离处相互抵消，而远距离和大远银心点距离天体受影响最大。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中心区域（半径3.5千秒差距）内45个球状星团（globular clusters）轨道运动（规则或混沌）动力学的比较分析。

Data: 研究中使用的数据来源包括来自盖亚卫星（Gaia）的最精确天体测量数据（Vasiliev & Baumgardt, 2021）以及新的精化平均距离数据（Baumgardt & Vasiliev, 2021），用于构建45个球状星团（globular clusters）的6D相空间轨道积分所需数据。

Method: 采用频率分析法（frequency method）分析45个球状星团（GCs）在静态和演化势场中的轨道混沌/规则运动，并考虑中心旋转棒（rotating elongated bar）的影响。

Result: 在静态和演化势场中，球状星团（GCs）的轨道动力学仅发生微小变化，证实了引力势分量质量与尺寸变化对轨道参数的影响相互抵消，且在较小银心距时补偿效应最大，而远距离和大远银心距天体受影响最显著。
...
### [7] [Asymmetric torus variability in active galactic nuclei driven by global brightening and dimming](https://arxiv.org/abs/2601.00358)
*Suyeon Son,Minjin Kim,Luis C. Ho*

**TL;DR**: 研究发现活动星系核（AGN）中红外（MIR）连续谱的时间不对称性与光学-MIR颜色相关：蓝颜色AGN在增亮时MIR变幅更大（正不对称），红颜色AGN在衰减时变幅更大（负不对称），且该现象源于环面（torus）而非吸积盘紫外-光学变异的反射。


<details>
  <summary>Details</summary>

Background: 活动星系核(AGN)的通量变化时间不对称性为理解其变异的物理机制提供了关键线索。

Data: 使用光学波段监测数据来自兹威基瞬变设施（Zwicky Transient Facility），中红外（MIR）波段数据来自近地天体广域红外巡天探测器（Near-Earth Object Wide-field Infrared Survey Explorer）。

Method: 通过比较增光与减光阶段的集合结构函数（ensemble structure functions），分析了中红外（MIR）连续谱的时间不对称性。

Result: 在光学-中红外颜色偏蓝的活动星系核(AGN)中，中红外(MIR)连续谱呈现正时间不对称性(变亮时变化幅度更大)，而颜色偏红的AGN则呈现负不对称性(衰减时变化幅度更大)。
...
### [8] [Low Metallicity Gas on the Outskirts of the Local Group: the Circumgalactic Medium of Sextans B](https://arxiv.org/abs/2601.00437)
...
**TL;DR**: 通过HST/COS光谱分析发现，Sextans B矮不规则星系（dwarf irregular galaxy）的环星系介质（CGM）中硅和碳丰度极低（[Si/H]=-1.7±0.2，[C/H]=-2.1±0.2），且其冷CGM气体质量（~8×10^8 M⊙）远超中性氢和恒星质量，成为该星系主要重子物质储库。


<details>
  <summary>Details</summary>

Background: 研究聚焦于六分仪座B（Sextans B）这一本星系群外围的矮不规则星系（dwarf irregular galaxy）的环星系介质（circumgalactic medium, CGM）的紫外吸收线特征及其金属丰度与质量分布。

Data: 研究使用了哈勃太空望远镜（HST）的宇宙起源光谱仪（COS）对两个活动星系核（AGN）视线方向的光谱数据，以及氢原子（H I）柱密度的干涉测量数据。

Method: 采用哈勃太空望远镜（HST）的宇宙起源光谱仪（COS）对六分仪座B（Sextans B）星系周介质（CGM）进行紫外吸收线分析，结合Cloudy光致电离模型和氢原子（H I）柱密度的干涉测量数据。

Result: 在Sextans B星系周围介质(CGM)中检测到Si II、Si III、Si IV和C II吸收线，其柱密度随半径增加而下降，且低于邻近矮星系的平均水平(低0.3-0.6 dex)。
...
### [9] [A New Empirical Fit to Galaxy Rotation Curves](https://arxiv.org/abs/2601.00522)
*David C. Flynn,Jim Cannaliato*

**TL;DR**: 提出了一种基于观测恒星运动推导速度修正项ω(omega)的星系旋转曲线(Rotation Curve)经验模型，无需修改牛顿动力学或引入暗物质分布即可高精度拟合多种星系的观测数据，在均方根误差(RMSE)和决定系数(R-squared)指标上优于MOND理论和冷暗物质(CDM)晕模型。


<details>
  <summary>Details</summary>

Background: 研究主题为通过引入基于观测恒星运动的速度修正项ω（velocity correction term ω）构建星系旋转曲线（galaxy rotation curves）的非参数化经验模型，旨在不改变牛顿动力学或暗物质分布的前提下解释广泛星系中观测到的运动学偏移。

Data: 数据来源于SPARC数据库（Spitzer Photometry and Accurate Rotation Curves）。

Method: 采用基于观测恒星运动推导的速度修正项ω（velocity correction term ω）的非参数化经验模型，通过锚定开普勒基线（Keplerian baselines）分析星系旋转曲线，无需修改牛顿动力学或引入暗物质分布假设。

Result: 该模型通过引入基于恒星运动观测的速度修正项ω（velocity correction term ω），在不改变牛顿动力学或暗物质分布的情况下，实现了对广泛星系旋转曲线的高精度拟合。
...
### [10] [The impact of cosmic voids on AGN activity](https://arxiv.org/abs/2601.00594)
*Benedict Rouse,Patricia B. Tissera,Yetli Rosas-Guevara,Claudia del P. Lagos*

**TL;DR**: 研究发现宇宙空洞（cosmic voids）中活动星系核（AGN）比例（12%）高于致密环境（6.7%），且最质量大的空洞星系AGN比例更高，其超大质量黑洞（SMBH）质量、特定恒星形成率（sSFR）和宿主晕质量均高于非AGN星系，近期并合（尤其是主并合）更频繁驱动了空洞AGN活动。


<details>
  <summary>Details</summary>

Background: 研究探讨宇宙空洞(void)及其周围结构（如纤维状结构(filament)和壁状结构(wall)）中寄主活动星系核(AGN)的星系特性，并与相同环境中的非活动星系核(non-AGN)星系进行对比。

Data: 数据来源于Eagle项目（Eagle project）的模拟数据，用于研究宇宙空洞（cosmic voids）及其周围结构（如纤维结构和壁）中活动星系核（AGN）寄主星系的性质，并与类似环境中的非AGN星系进行比较。

Method: 采用数值模拟（Eagle项目）结合环境分类（空洞、纤维结构、壁结构）和星系物理参数（恒星质量、超大质量黑洞质量、特定恒星形成率、暗晕质量）的统计比较方法。

Result: 活动星系核（AGN）比例随空洞中心距离增加而降低，空洞内星系AGN比例最高（12%），致密环境中最低（6.7%）。
...
### [11] [One-dimensional and time-dependent modelling of complex organic molecules in protostars](https://arxiv.org/abs/2601.00731)
*Le Ngoc Tram,Serena Viti,Katarzyna M. Dutkowska,Gijs Vermariën,Tobias Dijkhuis,Audrey Coutens,Timea Csengeri,Thiem Hoang*

**TL;DR**: 该论文通过改进一维时变化学模型（UCLCHEM）并应用于原恒星（protostar）的预恒星（prestellar）和加热阶段，结合内外辐射场机制，成功模拟了不同条件下复杂有机分子（COMs）的观测结果，揭示部分分子分布于包层（envelope）高温区而另一些来自低温区，并发现人马座B2(N1)（SgrB2(N1)）的宇宙射线电离率显著高于星际介质标准值。


<details>
  <summary>Details</summary>

Background: 研究聚焦于复杂有机分子（COMs）作为生命基础组分在不同天体环境（从静态云到恒星形成区）中的形成与破坏机制建模，旨在通过改进的气相-颗粒化学模型（UCLCHEM）揭示其与局部物理化学特性的关联。

Data: 数据来源于对SgrB2(N1)热核、银河系大质量团块和猎户座热核的单天线（single-dish）和干涉测量（interferometry）观测。

Method: 采用一维时变气体-尘埃化学模型（UCLCHEM）并整合内外辐射场的简单辐射机制，针对原恒星的前恒星阶段和加热阶段进行模拟。

Result: 复杂有机分子（COMs）在恒星形成过程中可分布于不同温度区域，部分位于高温包层而另一些来自低温区，单点模型可能导致误读。
...
### [12] [Isotropic stochastic gravitational wave background reconstruction for Taiji constellation](https://arxiv.org/abs/2601.00169)
*Yang Jiang,Qing-Guo Huang*

**TL;DR**: 该研究针对太级(Taiji)空间引力波探测任务，开发了能有效分离随机引力波背景(SGWB)与仪器噪声的数据分析流程，验证了算法在已知谱密度情况下准确恢复参数的能力，并利用跨维度马尔可夫链蒙特卡洛方法实现了未知谱形态背景的重建。


<details>
  <summary>Details</summary>

Background: 研究聚焦于毫赫兹频段随机引力波背景（stochastic gravitational wave background, SGWB）的探测及其作为未来天基干涉仪（如Taiji计划）潜在观测目标的天体物理与宇宙学意义。

Data: 使用太极（Taiji）模拟数据集（simulation datasets）进行算法性能评估和参数恢复。

Method: 采用转维马尔可夫链蒙特卡洛方法（trans-dimensional Markov Chain Monte Carlo）重建未知谱形态的随机引力波背景（SGWB）。

Result: 开发的算法在排除银河双星前景干扰后能准确恢复已知谱密度参数的随机引力波背景（SGWB），并利用跨维度马尔可夫链蒙特卡洛方法重建未知谱形态的背景信号。
...
### [13] [Measurement of the extragalactic background light at 8600Å using dark cloud shadow and the CaII-triplet lines](https://arxiv.org/abs/2601.00308)
*Lauri K. Haikala,Kalevi Mattila,Petri Väisänen*

**TL;DR**: 利用暗云法测量近红外河外背景光（Extragalactic Background Light），通过差分光谱技术消除前景干扰并结合GAIA RVS数据库建模散射星光，在2.1σ置信度下测得强度为13.8±6.5 nW m⁻²sr⁻¹。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用暗云法（dark-cloud method）测量近红外波段的河外背景光（Extragalactic Background Light），以探索宇宙学背景辐射的强度与特性。

Data: 数据来源于长缝光谱测量技术（long-slit spectra）对中间纬度球状体DC303.8-14.2不透明核心及几乎无遮挡区域的观测，并利用GAIA RVS光谱数据库构建了主要前景成分（散射的集成星光）的模板。

Method: 采用暗云法（dark-cloud method）测量近红外河外背景光（Extragalactic Background Light），结合沿狭缝摆动测量技术（nodding-along-the-slit measuring technique）获取差分光谱，并利用GAIA RVS光谱数据库构建散射星光（scattered integrated starlight）模板以消除主要前景成分。

Result: result：利用暗云法测得近红外河外背景光（Extragalactic Background Light）强度为$1.62\pm 0.76(σ_{stat})\, 10^{-9} erg\, cm^{-2} s^{-1}Å^{-1} sr^{-1}$，置信度$2.1σ$。
...
### [14] [Constraining Primordial Black Holes via p-wave annihilation in light of CMB Spectral Distortion and 21-cm global signal](https://arxiv.org/abs/2601.00413)
*Shibsankar Si,Pravin Kumar Natwariya,Alekha C. Nayak*

**TL;DR**: 原初黑洞(PBHs)通过吸积弱相互作用大质量粒子(WIMPs)形成尖峰密度晕，其中p波主导的暗物质湮灭产生的能量注入会改变宇宙热历史和电离历史，基于宇宙微波背景(CMB)谱畸变和21厘米吸收信号的观测约束，首次推导出p波湮灭模型下PBHs在暗物质中占比的严格上限。


<details>
  <summary>Details</summary>

Background: 研究探讨了原初黑洞（PBHs）通过弱相互作用大质量粒子（WIMPs）吸积形成尖峰密度晕及其p波主导的湮灭过程对宇宙热历史和电离历史的潜在影响。

Data: 数据来源包括宇宙微波背景（CMB）谱畸变（$y$-type）的观测约束和宇宙黎明时期21厘米吸收信号的测量数据。

Method: 采用速度依赖的p波湮灭模型（velocity-dependent p-wave annihilation）分析原初黑洞（PBHs）周围暗物质（WIMPs）湮灭对宇宙热历史和电离历史的影响。

Result: 原初黑洞（PBHs）通过吸积弱相互作用大质量粒子（WIMPs）形成尖峰密度晕，其中增强的密度显著提高了WIMPs的p波主导湮灭率，其能量注入可改变宇宙热历史和电离历史，从而在宇宙微波背景（CMB）频谱和全局21厘米信号中留下可观测印记。
...
### [15] [Fisher Forecasts for Cosmological Yields from $3\!\times\!2$pt Analysis of the Roman Space Telescope High Latitude Imaging Survey](https://arxiv.org/abs/2601.00438)
*Kaili Cao,David H. Weinberg,Vivian Miranda,Nihar Dalal,Tim Eifler,Jiachuan Xu,Haley Bowden*

**TL;DR**: 通过高纬度成像巡天（HLIS）的费希尔预测分析，发现星系-星系透镜效应（GGL）与星系成团性联合测量对宇宙学参数σ₈和Ωₘ的约束显著强于宇宙剪切，三者联合（3×2点）可进一步提升精度，且外部先验能显著改善参数限制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用南希·格雷斯·罗曼空间望远镜(Nancy Grace Roman Space Telescope)的高纬度成像巡天(HLIS)数据，通过宇宙剪切(cosmic shear)、星系-星系透镜效应(GGL)和星系成团性(galaxy clustering)的精确测量来检验宇宙学模型。

Data: 数据来源于NASA南希·格蕾丝·罗曼空间望远镜的高纬度成像巡天(HLIS)项目基础设施团队的数据挑战1(DC1)基准数据集。

Method: 采用费希尔预测(Fisher forecast)和马尔可夫链蒙特卡洛(MCMC)分析对宇宙学参数约束进行量化，重点评估了弱引力透镜信号振幅参数$σ_8$和$Ω_{\rm{m}}$的逆方差品质因子(FoMs)。

Result: 在基准DC1先验条件下，星系-星系透镜效应(GGL)+星系成团性(galaxy clustering)的约束显著强于宇宙剪切(cosmic shear)，而三者联合(3×2点)分析可带来适度提升。
...
### [16] [Late Time Magnetogenesis from Ultralight Scalar Dark Matter](https://arxiv.org/abs/2601.00443)
*V. Kamali,R. Brandenberger*

**TL;DR**: 研究发现，假设暗物质（Dark Matter）为通过规范动力学函数与电磁耦合且在复合期（recombination）以哈勃尺度相干振荡的超轻标量场，会导致规范场模式出现快子不稳定性（tachyonic instability），从而在宇宙学尺度上产生足以解释观测的磁场。


<details>
  <summary>Details</summary>

Background: 该研究探讨了暗物质（Dark Matter）作为与电磁场通过规范动力学函数耦合的超轻标量场（ultralight scalar field），在复合时期（recombination）以哈勃尺度（Hubble patch）相干振荡时引发规范场模式（gauge field modes）的快子不稳定性（tachyonic instability），从而在宇宙学尺度上产生可解释观测现象的磁场。

Data: 数据来源于对暗物质（Dark Matter）作为超轻标量场（ultralight scalar field）与电磁场通过规范动力学函数（gauge-kinetic function）耦合的理论模拟（simulation）。

Method: 采用规范-动力学函数（gauge-kinetic function）耦合的超轻标量场（ultralight scalar field）模型，通过分析规范场模式的快子不稳定性（tachyonic instability）生成宇宙学尺度磁场的机制。

Result: 暗物质（Dark Matter）作为超轻标量场（ultralight scalar field）通过规范动力学函数（gauge-kinetic function）与电磁场耦合时，在复合期（recombination）哈勃尺度（Hubble patch）上相干振荡会引发规范场模式（gauge field modes）的快子不稳定性（tachyonic instability），从而在宇宙学尺度上产生足够强度的磁场。
...
### [17] [Late-Time Resolution of the Hubble Tension in CPL Cosmology with Massive Neutrinos via Bayesian Physics-Informed Neural Networks](https://arxiv.org/abs/2601.00495)
*Muhammad Yarahmadi,Amin Salehi*

**TL;DR**: 基于物理信息神经网络(PINNs)的贝叶斯分析表明，在含中微子质量的CPL参数化动态暗能量模型中，哈勃常数(H0)的后验分布向高值偏移，显著缓解了早期与晚期宇宙测量之间的张力（降至1-2σ水平）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用贝叶斯物理信息神经网络（PINNs）框架分析哈勃常数（Hubble constant）的宇宙学约束问题，重点探讨标准ΛCDM模型及其动态暗能量（CPL参数化）扩展模型（含或不含大质量中微子）对哈勃张力（Hubble tension）的影响机制。

Data: 数据来源包括宇宙计时器（Cosmic Chronometers）、重子声学振荡（BAO DESI DR2）和Pantheon超新星样本。

Method: 采用基于物理信息神经网络（PINNs）的贝叶斯框架，将宇宙学背景方程直接嵌入网络架构，以数据驱动且物理自洽的方式重建哈勃膨胀历史H(z)，并严格传播认知不确定性。

Result: 在标准$Λ$CDM模型中，包含重子声学振荡（BAO）的数据组合倾向于较低的哈勃常数（$H_0$）值，减轻了与普朗克（Planck）数据的张力但加剧了与SH0ES的差异。
...
### [18] [The SRG/eROSITA all-sky survey: The morphologies of clusters of galaxies: II. The intrinsic distributions of morphological parameters](https://arxiv.org/abs/2601.00636)
...
**TL;DR**: 基于eROSITA(eRASS1)X射线选星系团样本，通过贝叶斯框架校正选择效应后首次量化了形态参数(浓度、中心密度等)随红移(z)和光度(LX)的标度关系，发现X射线选样本比SZ效应(Sunyaev-Zel'dovich)选样本具有更高浓度，表明SZ选方法同样存在形态选择偏差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于X射线选星系团形态学参数（如浓度、中心密度、尖度等）的本征分布及其随红移(z)和光度(LX)的演化规律，特别关注X射线选择与SZ效应(Sunyaev-Zel'dovich effect)选择导致的形态学偏差问题。

Data: 数据来源于eROSITA（SRG搭载）首次巡天eRASS1发现的12000多个光学认证星系团，并通过模拟获取了选择函数。

Method: 采用贝叶斯框架分析形态参数（包括浓度、中心密度、尖度、椭圆度和晃动）的本征分布，并通过模拟获得选择函数进行校正。

Result: 在考虑选择函数后，eRASS1的X射线选星系团显示出比SZ效应选样本更高的浓度参数（concentration），表明SZ选样本也可能存在形态选择效应。
...
### [19] [Dissecting the Hubble tension: Insights from a diverse set of Sound Horizon-free H0 measurements](https://arxiv.org/abs/2601.00650)
*Ioannis Pantos,Leandros Perivolaropoulos*

**TL;DR**: 通过分析83项不依赖声波视界(Sound-Horizon)的哈勃常数(H0)测量数据，发现距离阶梯法(Distance Ladder)测得的H0=72.74±0.40 km s⁻¹ Mpc⁻¹与其他方法测得的68.67±0.46 km s⁻¹ Mpc⁻¹存在6.7σ差异，表明哈勃张力(Hubble tension)本质是测量方法而非早晚宇宙时期的分歧。


<details>
  <summary>Details</summary>

Background: 该研究探讨哈勃常数(Hubble constant, H₀)测量中存在的哈勃张力(Hubble tension)，挑战了将其简单归因于早期与晚期测量差异的传统观点，提出张力本质是距离阶梯法(Distance Ladder)与其他方法间的系统性分歧。

Data: 研究中使用的数据来源包括83个独立于声视界（Sound-Horizon）的哈勃常数（H0）测量值，分为四类：使用本地校准器的距离阶梯（Distance Ladder）测量、假设标准膨胀历史的本地一步（Local One-Step ΛCDM）测量、独立于H(z)形状的纯本地一步（Pure Local One-Step）测量以及不使用声视界尺度的CMB声视界无关（CMB Sound Horizon Free）测量。

Method: 采用四类独立于声视界（Sound-Horizon）的哈勃常数（H0）测量方法：距离阶梯（Distance Ladder）测量、局部单步（Local One-Step）ΛCDM测量、纯局部单步（Pure Local One-Step）测量及CMB声视界无关（CMB Sound Horizon Free）测量。

Result: 哈勃张力（Hubble tension）更准确的特征是距离阶梯法（Distance Ladder）与其他所有方法之间的差异，而非早期与晚期测量的分歧。
...
### [20] [Dynamical Dark Energy Imprints in the Lyman-Alpha Forest](https://arxiv.org/abs/2601.00767)
...
**TL;DR**: 结合重子声学振荡、超新星巡天和宇宙微波背景数据，研究发现暗能量状态方程可能随时间演化（DDE），并通过莱曼阿尔法森林（Lyman-Alpha forest）吸收特征的功率谱倾斜、星系际介质温度升高及莱曼阿尔法不透明度降低等特征提供了独立验证DDE的观测途径。


<details>
  <summary>Details</summary>

Background: 暗能量（Dark Energy, DE）的本质及其随时间演化的动力学特性（dynamical DE, DDE）是理解宇宙晚期加速膨胀的核心科学问题。

Data: 研究中使用的数据来源于宇宙流体动力学模拟（cosmological hydrodynamical simulations）中结构形成过程产生的莱曼阿尔法森林（Lyman-Alpha forest）吸收特征。

Method: 采用宇宙流体动力学模拟（cosmological hydrodynamical simulations）研究动力学暗能量（DDE）对莱曼阿尔法森林（Lyman-Alpha forest）吸收特征性质的影响。

Result: 暗能量状态方程（DE equation of state）可能随时间演化，与宇宙学常数模型存在显著偏离。
...
### [21] [Identifiability Limits of Low-Rank RFI Mitigation Under Single-Epoch Constraints: A Controlled Failure-Mode Study](https://arxiv.org/abs/2601.00046)
*Yujin Kim*

**TL;DR**: 研究发现低地球轨道（LEO）卫星群产生的非稳态射频干扰（RFI）在单次观测中会因低秩分解的激进截断导致弥散或低信噪比天体信号显著失真，并量化了干扰强度临界值后不可避免的信息损失。


<details>
  <summary>Details</summary>

Background: 低地球轨道（LEO）卫星星座的快速扩张对射电天文观测产生了新型射频干扰（RFI），其非稳态特性挑战了传统基于长期平均或多时期观测的抑制方法。

Data: 研究中使用的数据来源包括受控模拟（controlled simulations）和实证分析（empirical analysis）生成的合成数据集。

Method: 采用低秩分解方法（low rank decomposition）分析单次观测（single epoch）中卫星干扰的非平稳结构化特征与天文信号的分离条件。

Result: 卫星电子设备产生的非预期电磁辐射具有强时变性和弱频谱规律性，导致传统基于长期平均或多历元观测的射频干扰(RFI)抑制方法失效。
...
### [22] [Light-tight skipper-CCDs for X-ray detection in space](https://arxiv.org/abs/2601.00127)
...
**TL;DR**: 提出了一种用于Skipper电荷耦合器件（skipper-CCDs）的光密屏蔽层，通过电子束蒸发沉积铝薄膜（50和100纳米）实现了>99.6%的光学背景抑制，同时保持了对5.9和6.4千电子伏特X射线的无损探测效率。


<details>
  <summary>Details</summary>

Background: Skipper电荷耦合器件（skipper-CCDs）作为一种具有亚电子级分辨率的硅基探测器，其辐射硬度和高精度能量重建能力使其成为空间X射线天文探测的潜在关键技术。

Data: 数据来源包括使用电子束蒸发器（e-beam evaporator）在CCD表面沉积铝层的实验数据、单色仪（monochromator）在650至1000 nm波长范围内的光学性能测试、$^{55}$Fe源的X射线透射测量以及Geant4模拟扩展的能量范围分析。

Method: 采用电子束蒸发沉积（e-beam evaporator）在Skipper-CCD表面镀铝膜，结合单色仪和$^{55}$Fe源分别评估650-1000 nm波长光抑制及5.9/6.4 keV X射线透射性能，并通过Geant4模拟扩展能谱范围分析不同铝膜厚度的效率损失。

Result: 50和100纳米铝层对650至1000纳米波长光子的抑制率超过99.6%，且对5.9和6.4千电子伏特X射线无效率损失。
...
### [23] [Combining datasets with different ground truths using Low-Rank Adaptation to generalize image-based CNN models for photometric redshift prediction](https://arxiv.org/abs/2601.00146)
*Vikram Seenivasan,Srinath Saikrishnan,Andrew Lizarraga,Jonathan Soriano,Bernie Boscoe,Tuan Do*

**TL;DR**: 提出利用低秩自适应（LoRA）技术结合不同星系成像数据集改进卷积神经网络（CNN）红移（redshift）估计，相比传统迁移学习方法减少约2.5倍偏差和2.2倍离散度，在计算效率与模型性能间取得平衡。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用低秩自适应（LoRA）技术融合不同星系成像数据集以提升卷积神经网络（CNN）在宇宙学红移（redshift）估计中的性能。

Data: 使用了测光红移（photometric redshift）地面实况数据集训练基础模型，并通过光谱红移（spectroscopic redshift）地面实况数据集进行LoRA微调。

Method: 采用低秩自适应（Low-Rank Adaptation, LoRA）技术，通过添加适配器网络调整模型权重和偏置，高效微调大型基础卷积神经网络（CNN）模型。

Result: LoRA（Low-Rank Adaptation）通过结合不同星系成像数据集改进了CNN模型的宇宙学红移（redshift）估计性能，其偏差和离散度分别比传统迁移学习方法降低约2.5倍和2.2倍。
...
### [24] [Spectral performance of single-channel plastic and GAGG scintillator bars of the CUbesat Solar Polarimeter (CUSP)](https://arxiv.org/abs/2601.00485)
...
**TL;DR**: 开发了CUbesat Solar Polarimeter (CUSP)立方星任务，通过双相位康普顿偏振仪在25-100 keV能段测量太阳耀斑硬X射线偏振，以约束磁重联和粒子加速理论模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦太阳耀斑（solar flare）硬X射线偏振（hard X-ray polarization）测量，旨在通过偏振特征区分磁重联（magnetic reconnection）与粒子加速（particle acceleration）的竞争理论模型，从而深化对空间天气（space weather）物理机制的理解。

Data: 数据来源于CUbesat Solar Polarimeter（CUSP）任务开发的6U-XL平台上的双相位康普顿偏振仪（dual-phase Compton polarimeter），该偏振仪由四个4x4塑料闪烁体（plastic scintillators）阵列（耦合至多阳极光电倍增管）和四个由八个细长GAGG闪烁体（GAGG scintillator）条带（耦合至雪崩光电二极管）组成的中央组件构成，传感器分别由Hamamatsu提供，并由Weeroc的MAROC-3A和SKIROC-2A ASICs读取。

Method: 采用双相位康普顿偏振计（dual-phase Compton polarimeter）技术，结合塑料闪烁体（plastic scintillators）阵列与GAGG闪烁体（GAGG scintillator）条带，通过MAROC-3A和SKIROC-2A专用集成电路（ASIC）进行信号读取。

Result: 太阳耀斑的硬X射线偏振（hard X-ray polarization）测量对于区分竞争理论模型至关重要，但目前观测数据仍有限。
...
### [25] [ASCNet: Research on all-sky camera images classification at the Muztagh-ata site](https://arxiv.org/abs/2601.00563)
*Siqi Wang,Qi Fan,Wenbo Gu,Haozhi Wang,AYZADA Jumahali,Lixian Shen,Daiping Zhang,Liyong Liu,Ali Esamdin*

**TL;DR**: 提出ASCNet模型对慕士塔格站点2022-2024年夜间全天空相机(ASC)图像进行云量分类，通过融合ResNet34与深度可分离空洞卷积(Depthwise Dilated Convolution)的ASCModule模块，在测试集上达到92.7%的标注一致率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用全天空相机（All-sky camera, ASC）图像改进天文台址云量监测的自动化分类方法。

Data: 数据来源于2022年3月至2024年6月在慕士塔格站点采集的夜间全天空相机（All-sky camera）图像，按类别划分，70%作为训练集，30%作为测试集。

Method: 采用ResNet34结合ASCModule（含深度扩张卷积Depthwise Dilated Convolution和轻量级压缩-激励注意力Squeeze-and-Excitation）从亮度通道提取细粒度纹理信息。

Result: ASCNet在夜间全天空相机（ASC）图像分类中实现了92.7%的一致性率，其准确率（Accuracy）达92.66%、精确率（Precision）83.26%、召回率（Recall）84.25%、F1分数（F1-Score）83.67%，消融实验与对比实验均显著优于其他模型。
...
### [26] [Unlocking the Future of X-Ray Polarimetry with IXPE: Lessons Learned and Next Steps](https://arxiv.org/abs/2601.00764)
...
**TL;DR**: 该论文总结了成像X射线偏振探测卫星(IXPE)早期仪器开发中的问题，发现星载校准系统有效解决了探测器非理想行为，基于科学回报和载荷状态任务已延至2028年，其经验正指导新一代X射线偏振任务设计。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于X射线偏振测量探测器(Imaging X-ray Polarimetry Explorer)在早期研制阶段遇到的技术挑战及其解决方案。

Data: 数据来源于IXPE（Imaging X-ray Polarimetry Explorer）搭载的偏振和非偏振X射线源（on-board polarized and unpolarized X-ray sources）以及约100个观测目标的实测数据。

Method: 采用机载偏振与非偏振X射线源数据通过飞行管线实时校正仪器响应（instrument response）。

Result: IXPE（Imaging X-ray Polarimetry Explorer）任务通过搭载的偏振和非偏振X射线源数据持续校正仪器响应，实现了对约100个天体目标的有效偏振测量，并因其科学成果和载荷状态良好而延期至2028年。
...
### [27] [Terrestrial Test of Shapiro Time Delay: Forth test of Einstein General Theory of Relativity](https://arxiv.org/abs/2601.00468)
*Farhad Hakimi,Hosain Hakimi*

**TL;DR**: 提出基于光纤萨格纳克干涉仪(Sagnac interferometer)的地面测量新方法，首次在实验室尺度实现接近10^-9精度的PPN参数γ测定，为广义相对论提供了新测试范式。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用地面实验精确测量夏皮罗时延（Shapiro time delay），以独立验证广义相对论中参数化后牛顿（PPN）参数γ的预测值。

Data: 使用基于光纤的萨格纳克干涉仪（Sagnac interferometer）进行地面精密测量。

Method: 采用基于光纤的萨格纳克干涉仪(Sagnac interferometer)进行高精度地面测量。

Result: 提出了一种基于光纤萨格纳克干涉仪(Sagnac interferometer)的地面测量方法，可在实验室尺度上以接近10-9的灵敏度测定PPN参数γ。
...
### [28] [Eccentric Disks from Circumbinary Rings](https://arxiv.org/abs/2601.00741)
*Leonardo Betancourt,Andrew MacFadyen,Jonathan Zrake*

**TL;DR**: 通过高分辨率流体动力学模拟发现，冷态有限气体环双星盘（CBRs）会抑制双星吸积，产生显著低频（∼0.1Ω_b）光谱峰并形成偏心（e≃0.3）盘，为中等质量黑洞双星准周期喷发和类星体光变曲线不对称双峰发射线提供了解释机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于围绕双星系统的有限气体环（CBRs）粘性扩散形成盘结构的流体动力学过程及其对双星吸积抑制、准周期性喷发（QPEs）产生机制、以及活动星系核中盘发射线不对称性的影响。

Data: 采用高分辨率网格流体动力学模拟（grid-based hydrodynamics simulations）研究有限气体环双星盘（CBRs）粘性扩散过程，数据来源于自主数值模拟。

Method: 采用高分辨率网格流体动力学模拟（grid-based hydrodynamics simulations）研究有限气体环双星盘（CBRs）粘性扩散过程。

Result: 冷气体条件下所有系统均抑制双星吸积，环状结构在约0.1倍双星轨道频率处呈现显著且稳定的光谱峰值。
...
### [29] [Metric Reconstruction and Second Order Perturbation for Generic Spherically Symmetric spacetime](https://arxiv.org/abs/2601.00162)
*Rong-Zhen Guo,Qing-Guo Huang*

**TL;DR**: 提出适用于一般球对称时空的修正Teukolsky方程和无需赫兹势(Hertz potential)的度规重构方法，并建立了适用于此类时空的二阶微扰理论。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超越广义相对论（General Relativity）的微扰框架和时空度规重构方法，以检验引力理论中高阶微扰在环down阶段的作用。

Data: 研究基于渐进平坦、球对称时空的微扰计算引入修正的Teukolsky方程，并扩展了不依赖赫兹势(Hertz potential)的度规重构方法至$tr$对称时空。

Method: 采用改进的Teukolsky方程（modified Teukolsky equation）进行微扰计算，并扩展了不依赖赫兹势（Hertz potential）的度规重构方法至$tr$对称时空（$tr$-symmetric spacetime），同时提出适用于一般球对称时空的二阶微扰理论。

Result: 提出了适用于一般球对称时空的二阶微扰理论。
...
### [30] [Gravitational Wave Tails and Transient Behaviors of Quantum-Corrected Black Holes](https://arxiv.org/abs/2601.00164)
*Rong-Zhen Guo,Qing-Guo Huang*

**TL;DR**: 研究了有效圈量子引力（effective Loop Quantum Gravity）框架下黑洞引力波尾迹的量子修正效应，发现其振幅和中间行为受量子修正影响且对黑洞动力学细节敏感。


<details>
  <summary>Details</summary>

Background: 引力波天文学在强场（strong-field）区域检验引力动力学和探索黑洞本质方面具有关键作用。

Data: 数据来源于对有效圈量子引力（effective Loop Quantum Gravity）框架下带有量子修正的黑洞引力波尾迹（gravitational wave tails）的理论模拟（simulation）。

Method: 采用有效圈量子引力（effective Loop Quantum Gravity）框架分析黑洞引力波尾迹（gravitational wave tails）的量子修正效应。

Result: 量子修正显著影响了黑洞引力波尾波（gravitational wave tails）的振幅和中间行为。
...
### [31] [Exceptional Lines and Excitation of (Nearly) Double-Pole Quasinormal Modes: A Semi-Analytic Study in the Nariai Black Hole](https://arxiv.org/abs/2601.00704)
*Nao Nakamoto,Naritaka Oshita*

**TL;DR**: 研究发现克尔-德西特(Kerr-de Sitter)和迈尔斯-佩里(Myers-Perry)黑洞中具有质量的标量场准正规模(QNMs)在参数空间存在一条由连续例外点(EPs)构成的例外线(EL)，并在Nariai极限下通过解析方法揭示了该线附近准正规模的激发因子特征及早期衰变阶段的瞬态线性增长现象。


<details>
  <summary>Details</summary>

Background: 研究聚焦于克尔-德西特（Kerr-de Sitter）和迈尔斯-佩里（Myers-Perry）黑洞中具有质量标量场的准正规模（QNMs）在参数空间内表现出的异常线（EL）特性及其对模激发和铃荡稳定性的影响。

Data: 研究使用了克尔-德西特（Kerr-de Sitter）和迈尔斯-佩里（Myers-Perry）黑洞中大质量标量场的准正规模（QNMs）数据，并在Nariai极限（$r_{\rm c} - r_{\rm h} \to 0$）下通过扰动方程简化为具有Pöschl-Teller（PT）势的波动方程进行解析处理。

Method: 采用的核心分析方法包括解析研究拟正则模（QNMs）在异常线（EL）附近的振幅或激发因子，以及利用Nariai极限下扰动方程简化为具有Pöschl-Teller（PT）势的波动方程进行理论推导。

Result: 在克尔-德西特（Kerr-de Sitter）和迈尔斯-佩里（Myers-Perry）黑洞中，大质量标量场的准正规模（QNMs）在参数空间内存在一条由连续异常点（EPs）构成的异常线（EL），导致两个QNM频率及其解在此重合。
...
### [32] [Open Questions in Massive Star Research across Cosmic Scales](https://arxiv.org/abs/2601.00373)
*Andreas A. C. Sander*

**TL;DR**: 这篇论文的核心贡献是：通过分析大质量恒星（massive stars）演化模型与JWST观测的高金属丰度（metallicity）及早期氮富集（nitrogen enrichment）之间的矛盾，指出现有理论面临的根本性挑战，并以弱风问题（weak-wind problem）和沃尔夫-拉叶星（Wolf-Rayet stars）的多样性为例，提出了重新审视恒星演化模型的必要性。


<details>
  <summary>Details</summary>

Background: 大质量恒星（massive stars）作为宇宙演化的核心驱动力，其演化理论面临观测与模型间的显著矛盾，亟需重新审视现有认知框架。

Data: 研究中使用的数据来源包括JWST（James Webb Space Telescope）对高红移星系金属丰度和早期氮富集的观测数据，以及大尺度巡天（large surveys）和计算模型（computational modeling）的模拟数据。

Method: 采用计算建模（computational modeling）和大规模巡天（large surveys）技术重新评估现有大质量恒星演化模型。

Result: 大质量恒星（massive stars）是宇宙演化的关键驱动力，但JWST观测到的高红移星系高金属丰度和早期氮富集等新发现对现有恒星演化理论提出了挑战。
...
### [33] [The Solar Neighborhood LV: Spectral Characterization of an Equatorial Sample of 580 K Dwarfs](https://arxiv.org/abs/2601.00462)
...
### [34] [Covertly Active Comet (139359) 2001 ME1](https://arxiv.org/abs/2601.00060)
*Qicheng Zhang,Quanzhi Ye,Karl Battams,Matthew M. Knight,Worachate Boonplod,Rainer Kracht*

**TL;DR**: 首次确认小行星(139359) 2001 ME1在近日点附近存在周期性尘埃活动（recurrent dust activity），其前向散射（forward scattering）增亮达7星等，比尘埃贫乏的2P/Encke彗星增亮更强，轨道分析表明该天体近万年来保持近地轨道（near-Earth orbit）导致挥发性物质持续耗尽。


<details>
  <summary>Details</summary>

Background: 研究主题为利用太阳和日光层观测台(SOHO)及日地关系天文台(STEREO)的日冕仪观测数据，分析小行星(139359) 2001 ME1在近日点附近未被注意到的尘埃活动及其前向散射增亮现象。

Data: 数据来源于SOHO（Solar and Heliospheric Observatory）和STEREO（Solar Terrestrial Relations Observatory）的日冕仪观测数据，包括2018年11月18日SOHO捕获的彗星穿越视场事件、2017年类似几何条件下对2P/Encke彗星的观测数据，以及2014年STEREO对同一彗星活动的早期观测记录。

Method: 利用日地关系天文台(STEREO)和太阳和太阳风层探测器(SOHO)的日冕仪数据，通过前向散射(forward scattering)和后向散射(backscattering)光度测量技术量化彗星尘埃活动亮度变化。

Result: 2018年11月18日SOHO日冕仪观测到小行星(139359) 2001 ME1在近日点附近未被注意的尘埃活动，其前向散射（forward scattering）导致亮度增强7星等。
...
### [35] [A free-floating-planet microlensing event caused by a Saturn-mass object](https://arxiv.org/abs/2601.00057)
...
**TL;DR**: 首次通过微引力透镜效应直接测量了一颗质量为0.219倍木星质量的自由漂浮行星（free-floating planet），并推断其形成于原行星盘（protoplanetary disk）后因动力学过程被抛射。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过微引力透镜（gravitational microlensing）技术探测自由漂浮行星（free-floating planets）的统计特性和形成机制。

Data: 数据来源于地基望远镜（KMTNet和OGLE）和天基望远镜的联合观测。

Method: 采用引力微透镜（gravitational microlensing）技术结合地面和空间望远镜的多平台观测数据，通过质量-距离简并性（mass-distance degeneracy）破解方法精确测量天体质量。

Result: 通过地面和空间望远镜联合观测的微引力透镜事件KMT-2024-BLG-0792/OGLE-2024-BLG-0516，首次精确测量了一颗质量为0.219^{+0.075}_{-0.046}木星质量的自由漂浮天体（free-floating object），其形成于原行星盘（protoplanetary disk）而非孤立环境（如褐矮星），后经动力学过程被抛射出原始系统。
...
### [36] [KMT-2024-BLG-0816/OGLE-2024-BLG-0519 -- A Microlensing Event with Candidate Free-Floating Planet Lens and Blended Light](https://arxiv.org/abs/2601.00325)
...
**TL;DR**: 发现了自由漂浮行星（free-floating planet）微引力透镜事件KMT-2024-BLG-0816，其具有有限源效应（finite-source effect）和未解析的混合光（blending light），且未检测到潜在行星宿主信号，为目前唯一具有未解析混合光的自由漂浮行星事件。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用微引力透镜效应（microlensing）发现并分析自由漂浮行星（free-floating planet）KMT-2024-BLG-0816的观测特征及其潜在宿主恒星的关联性。

Data: 数据来源于韩国微引力透镜望远镜网络（KMTNet）观测到的微引力透镜事件KMT-2024-BLG-0816。

Method: 采用微引力透镜（microlensing）技术分析有限源效应（finite-source effect）和未解析混合光（unresolved blending light）。

Result: 发现了一例具有有限源效应（finite-source effect）、显著混合光且无宿主恒星微引力透镜信号的自由漂浮行星（free-floating planet）事件KMT-2024-BLG-0816。
...
### [37] [Dark Recipe for the First Giants: From Population III Stars to Early Supermassive Black Holes via Dark Matter Capture](https://arxiv.org/abs/2512.23789)
*Sulagna Bhattacharya,Debajit Bose,Basudeb Dasgupta,Jaya Doliya,Ranjan Laha*

**TL;DR**: 提出了一种通过非湮灭暗物质(DM)与标准模型(SM)粒子相互作用在第三族恒星(Pop III)内积累并促使其提前坍缩形成黑洞种子的机制，解释了詹姆斯·韦伯太空望远镜(JWST)观测到的高红移(z>5)超大质量黑洞(SMBH)的起源。


<details>
  <summary>Details</summary>

Background: 研究探讨了高红移(z>5)超大质量黑洞(SMBH)的形成机制，挑战了标准黑洞(BH)形成理论，提出暗物质(DM)与标准模型(SM)粒子的非引力相互作用导致第三星族(Pop III)恒星提前坍缩为黑洞种子的新机制。

Data: 数据来源包括詹姆斯·韦伯太空望远镜（JWST）对高红移（$z>5$）超大质量黑洞（SMBHs）的观测结果，以及未来直接探测实验（direct detection experiments）、激光干涉空间天线（LISA）和脉冲星计时阵列（PTA）对引力波（GW）信号的预期观测。

Method: 采用非湮灭暗物质(DM)与标准模型(SM)粒子的自旋依赖(SD)相互作用模型，通过参数空间分析解释高红移超大质量黑洞(SMBH)的观测特征。

Result: 非湮灭暗物质（DM）与标准模型（SM）粒子的非引力相互作用导致第三族恒星（Pop III）内部积累暗物质并提前坍缩为与母恒星质量相同的黑洞种子，从而解释了高红移超大质量黑洞（SMBH）的早期形成。
...
### [38] [Freeze-in leptogenesis without the need for right-handed neutrino oscillations](https://arxiv.org/abs/2601.00063)
*Martin A. Mojahed,Sascha Weber*

**TL;DR**: 该论文提出了一种基于标准I型跷跷板模型（standard type-I seesaw model）、可在吉电子伏特（GeV）能级右手中微子（RHN）质量下运行的实验可检验轻子生成机制（leptogenesis），发现该机制在仅存在单个GeV能级RHN且无需RHN间振荡或CP破坏的情况下，仍能通过高能温下右旋电子与左旋正电子不对称性实现成功重子生成（baryogenesis）。


<details>
  <summary>Details</summary>

Background: 研究提出了一种基于标准I型跷跷板模型（type-I seesaw）的实验可检验轻子生成（leptogenesis）机制，该机制在吉电子伏特（GeV）能标附近的有效右手中微子（RHN）质量下成功运行。

Data: 数据来源于标准I型跷跷板模型（standard type-I seesaw model）中约GeV能标的右手中微子（RHN）质量相关实验可测的轻子生成机制（leptogenesis）的模拟研究。

Method: 采用基于标准I型跷跷板模型（type-I seesaw）的实验可检验轻子生成机制，重点研究吉电子伏特（GeV）能标附近右手中微子（RHN）的质量效应，并在高温度下产生右旋电子与左旋正电子不对称的宇宙学背景下运行。

Result: 在GeV能级的右手中微子（RHN）质量下，通过高温度产生右旋电子与左旋正电子不对称性的宇宙学背景，实现了无需RHN振荡或CP破坏的标准I型跷跷板模型轻子生成机制。
...
### [39] [Anatomy of RHN DM relic in the vanilla scotogenic neutrino mass model](https://arxiv.org/abs/2601.00436)
*Sujit Kumar Sahoo,Narendra Sahu,Vicky Singh Thounaojam*

**TL;DR**: 研究最小暗原性(scotogenic)中微子质量模型，发现最轻右手中微子(RHN)可作为暗物质候选者，其质量范围在热暗物质情形下为$M_{h}/2\lesssim M_{\rm DM}\lesssim2000 {\rm GeV}$，非热暗物质情形下为$0.1 ~{\rm GeV}\lesssim M_{\rm DM}\lesssim 1000 {\rm GeV}$，并预测了未来对撞机可探测的长寿命粒子位移顶点特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于暗物质（DM）候选粒子的产生机制及其性质，特别是通过辐射机制（radiative mechanism）生成轻中微子质量的暗色生成模型（scotogeneic neutrino mass models）。

Data: 数据来源为理论模拟（simulation）研究，未涉及实际观测数据。

Method: 采用辐射机制（radiative mechanism）和最小暗原性（minimal scotogenic）模型分析右手中微子暗物质（RHN DM）遗迹，包括自湮灭（self-annihilation）、共湮灭（co-annihilation）、转换驱动过程（conversion-driven processes）以及冻结机制（freeze-in mechanism）的贡献。

Result: 最轻的右手中微子（RHN）在质量范围$M_{h}/2\lesssim M_{\rm DM}\lesssim2000 {\rm GeV}$（热暗物质）和$0.1 ~{\rm GeV}\lesssim M_{\rm DM}\lesssim 1000 {\rm GeV}$（非热暗物质）内可作为可行的暗物质候选者。
...
### [40] [Dark Dimension Right-handed Neutrinos Confronted with Long-Baseline Oscillation Experiments](https://arxiv.org/abs/2601.00790)
*Ai-Yu Bai,Auttakit Chatrabhuti,Yin-Yuan Huang,Hiroshi Isono,Jian Tang*

**TL;DR**: 研究发现T2K和NOvA实验的中微子振荡数据与标准模型一致，并对暗额外维度（dark extra dimension）模型参数设定了严格限制，其约束结果与对撞机实验和宇宙学观测互补。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗额外维度模型（dark extra dimension）自然诱导右手中微子（right-handed neutrinos）的现象及其在中微子振荡中的关键作用。

Data: 数据来源于T2K和NOvA实验的（neutrino oscillation）中微子振荡实验数据。

Method: 采用核心分析方法为计算并比较额外维度模型（extra dimension models）与标准模型下的中微子振荡（neutrino oscillations）能谱。

Result: 右手中微子（right-handed neutrinos）在暗额外维度模型中自然产生，并对中微子振荡起关键作用。
