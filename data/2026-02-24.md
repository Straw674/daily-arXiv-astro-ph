...
**TL;DR**: 利用SDSS-V DR19银河系测绘APOGEE数据，通过研究疏散星团中的单线光谱双星(SB1s)，发现具有显著紫外超的双星子集其Δ[C/N]比预期高0.2-0.5 dex，表明存在来自演化伴星的污染，这对基于[C/N]的银河系年龄研究具有影响，同时证实SB1s在群体水平上与单星化学同质且轨道间距与化学丰度变化无关。


<details>
  <summary>Details</summary>

Background: 研究探讨了密近双星（close binarity）对疏散星团（open clusters）中单线光谱双星（SB1s）表面化学丰度的影响及其对银河系[C/N]年龄研究的启示。

Data: 数据来源为SDSS-V DR19 Milky Way Mapper APOGEE（斯隆数字巡天第五期数据发布19中的银河系测绘者APOGEE项目）的径向速度观测数据。

Method: 采用单线光谱双星（SB1s）分析方法，结合The Joker和UltraNest对APOGEE视向速度数据进行轨道参数拟合，并利用BACCHUS进行14种化学元素（Si、Fe、C、N、O、Na、Mg、Al、Ca、Ti、Cr、Ni、Ce、Nd）的丰度分析。

Result: result：研究发现具有显著紫外超的特定双星子集其Δ[C/N]比预期高0.2-0.5 dex，导致基于[C/N]的年龄估计偏高，表明存在来自演化伴星的污染。
...
### [2] [Blue Monsters and Dusty Descendants: Reconciling UV and IR Emission from Galaxies from z=7, up to z= 14](https://arxiv.org/abs/2602.18556)
*Laura Sommovigo,Lachlan Lancaster,Shyam H. Menon,Joseph A. O'Leary,Rachel S. Somerville,Greg L. Bryan*

**TL;DR**: 通过引入多孔湍流星际介质(ISM)模型，解决了高红移(z≥7)星系紫外(UV)和红外(IR)光度函数(LFs)之间的矛盾，表明湍流通过增加UV逃逸分数同时保持IR光度不变，而早期星系(z>10)的明亮UV端可能还需高效尘埃清除或后期ISM尘埃增长来解释。


<details>
  <summary>Details</summary>

Background: 研究探讨了早期宇宙中恒星形成与尘埃产生的统一物理模型能否解释高红移（$z > 10$）紫外（UV）明亮星系与红移$z \simeq 7$尘埃富集红外（IR）明亮星系之间的观测差异。

Data: 数据来源包括JWST（James Webb Space Telescope）对红移$z > 10$的紫外明亮星系的观测数据，以及ALMA（Atacama Large Millimeter/submillimeter Array）对红移$z \simeq 7$的尘埃丰富、红外明亮星系的探测数据。

Method: 采用双参数（瞬时恒星形成效率ε⋆和II型超新星尘埃产率yd）最小框架，结合多孔湍流星际介质（ISM）模型及辐射传输效应，预测红移14至7的紫外和红外光度函数。

Result: 通过引入多孔湍流星际介质(ISM)模型，在考虑辐射传输导致的衰减曲线平坦化后，能够显著缓解高红移(z≥7)星系紫外(UV)和红外(IR)光度函数明亮端的观测矛盾。
...
### [3] [The ionised interstellar medium of DSFGs revealed by JWST/NIRSpec and ALMA: Super-solar metallicity, low ionisation parameters and, typical electron densities](https://arxiv.org/abs/2602.18558)
...
**TL;DR**: 基于JWST/NIRSpec和ALMA观测48个高红移(z=2.53)星系的近红外光谱，发现这些大质量(中位恒星质量log(M*/M⊙)=10.8)、尘埃遮蔽的远红外明亮星系具有较高气体金属丰度(12+log(O/H)=8.71)且偏离基本金属丰度关系，表明它们是化学演化成熟的系统。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST/NIRSpec和ALMA观测高红移(z=2.53)尘埃遮蔽恒星形成星系(DSFGs)的星际介质性质及其化学演化特征。

Data: 研究使用了JWST/NIRSpec的近红外(2-4μm)光谱数据和ALMA在>3σ水平上探测到的48个高红移星系的多波段SED(spectral energy distribution)分析数据。

Method: 采用多波段光谱能量分布(SED)分析和近红外光谱([NII]/Hα、[SII]双线比、[OII]与Hβ)测量技术。

Result: 样本中大多数星系未显示活动星系核(AGN)迹象，40%具有X射线对应体、升高的光学线比或宽Hα谱线特征，但受NIRSpec狭缝随机放置影响该比例为下限。
...
### [4] [Assessing Zeeman Measurements of Magnetic Fields in Synthetic HI Observations](https://arxiv.org/abs/2602.18713)
*Duo Xu,Peter Martin,Stella Offner,Robert Gutermuth,Michael Grudic,Joshua Speagle*

**TL;DR**: 通过合成HI塞曼(Zeeman)谱分析两种方法（整体估计和分量分解）验证了星际介质视线方向(LOS)磁场测量的可靠性，误差分别控制在16%和13%，并在湍流环境和L1544恒星形成区观测中证实了其有效性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用塞曼(Zeeman)效应直接探测星际介质中视线方向(LOS)磁场的精度评估与局限性分析。

Data: 数据来源于磁流体动力学模拟（magnetohydrodynamic simulations）和理想化云模型（idealized cloud models）生成的合成HI塞曼光谱（synthetic HI Zeeman spectra），以及FAST望远镜对L1544恒星形成区的观测数据（FAST observations of the L1544 star-forming region）。

Method: 采用两种互补方法分析合成HI塞曼(Zeeman)谱：方法I基于斯托克斯(Stokes) V与dI/dν的经典关系估算视线方向平均磁场；方法II通过高斯分解斯托克斯I和V获取组分级磁场信息。

Result: result：通过合成HI塞曼(Zeeman)谱分析，方法I获得视线平均磁场相对误差上限为16%，方法II获得组分级磁场相对误差为13%，两种方法在均匀场条件下能准确还原原始磁场且在湍流环境中保持稳健，联合拟合斯托克斯(Stokes)I和V谱优于顺序拟合，噪声增加八倍仅使相对误差升至26%，而显著光学深度对总体不确定性的额外贡献较小，FAST对L1544恒星形成区的观测验证了塞曼分析的可靠性。
...
**TL;DR**: 该研究通过半解析模型揭示了气体丰富的原恒星团（proto-stellar clusters）在形成初期（约10 Myr内）通过气体吸积导致恒星黑洞（BH）质量增长至跨越质量间隙（mass gap）并达到约10^3 M⊙，与引力波事件GW231123的组分质量范围一致。


<details>
  <summary>Details</summary>

Background: 研究探讨了气体富集的原恒星团（proto-stellar clusters）在形成初期（约前10 Myr）通过气体吸积导致恒星黑洞（stellar black hole）质量增长的半解析模型。

Data: 数据来源于詹姆斯·韦伯太空望远镜（JWST）对原恒星团（proto-stellar clusters）的观测结果以及基于气体吸积的半解析模型（semi-analytic model）模拟。

Method: 采用半解析模型(semi-analytic model)研究气体吸积对恒星黑洞(BH)质量增长的动力学影响。

Result: 在气体丰富的原恒星团（proto-stellar cluster）中，恒星黑洞（BH）通过气体吸积可在气体耗尽后达到并超过黑洞质量间隙（BH mass gap）的质量范围，最高可达约$10^3\,{\rm M}_\odot$，与引力波信号GW231123的组分质量范围一致。
...
### [6] [Characterization of Residual Morphological Substructure Using Supervised and Unsupervised Deep Learning](https://arxiv.org/abs/2602.18883)
...
**TL;DR**: 提出结合监督卷积神经网络(CNN)和无监督卷积变分自编码器(CvAE)的深度学习框架，成功实现了对CANDELS巡天中10,046个大质量星系(1<z<3)单Sérsic轮廓拟合残差图像中星系子结构的自动化表征，发现CNN潜在特征与显著像素通量(SPF)相关并能区分强弱残差子结构，而CvAE虽与视觉特征相关但区分能力有限。


<details>
  <summary>Details</summary>

Background: 利用深度学习框架自动化表征星系子结构（galactic substructure）以揭示驱动星系演化的物理过程。

Data: 数据来源于CANDELS巡天中10,046个明亮且大质量星系（H<24.5 mag且M_stellar≥10^9.5 M_⊙）在红移1<z<3范围内的单Sérsic轮廓拟合残差图像及其视觉分类标签。

Method: 采用监督式卷积神经网络（CNN）和无监督卷积变分自编码器（CvAE）分析单Sérsic轮廓拟合残差图像，并结合主成分分析（PCA）、高斯混合模型（GMM）聚类和支持向量分类（SVC）评估潜在空间特征。

Result: 深度学习框架（CNN和CvAE）能有效表征星系残余图像中的子结构特征，其中监督式CNN的潜在特征与显著像素通量（SPF）相关并能区分强弱残余子结构，而无监督CvAE虽与视觉和定量特征相关但缺乏明确区分能力。
...
### [7] [The Optical Properties of Host Galaxies of Radio Sources in the Coma Cluster](https://arxiv.org/abs/2602.19050)
*Xiaolan Hou,Heng Yu,Tong Pan,Hu Zou,Haoran Dou,Emily Moravec,Chengkui Li*

**TL;DR**: 该研究通过结合LOFAR低频射电观测与光学数据，发现后发座(Coma)星系团内射电源寄主星系形态与位置相关：致密射电源主要关联活动星系核(AGN)驱动的静止大质量星系，拖尾射电源多由受冲压剥离(RPS)影响的恒星形成星系主导，且延展源和拖尾源分别富集于团外围和中等落向区域，揭示了星系团内介质密度对射电形态分布的关键影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于后发座星系团（Coma cluster）内射电源寄主星系的射电与光学特性，探讨星系团环境对星系演化和活动星系核（AGN）活动的影响。

Data: 数据来源包括LOFAR双米波巡天（LoTSS-DR2）的144MHz射电观测数据、DESI的光谱数据以及SDSS的光度测量数据。

Method: 采用低频阵列（LOFAR）144MHz深场观测数据结合DESI和SDSS光谱测光数据，通过相位空间分析和投影下落时间指标（d_R）研究射电源形态与星系团动力学环境的关联。

Result: 紧凑射电源（compact radio sources）主要与由活动星系核（AGN）驱动的质量较大且静止的星系相关，而拖尾射电源（tailed sources）则主要寄生于正在经历星暴活动的星系中，反映了持续的冲压剥离（RPS）过程。
...
### [8] [Feedback shaped the galaxy morphological sequence in presence of mergers](https://arxiv.org/abs/2602.19227)
*Masafumi Noguchi*

**TL;DR**: 该论文揭示了暗物质晕（dark matter halo）中星系质量比例在$M_{\rm halo} \sim 10^{12} {\rm M}_\odot$处达到峰值的特征通过并合过程塑造了星系形态序列（morphological sequence），表明星系形态与活动星系核（AGN）和超新星（SN）反馈过程存在密切关联。


<details>
  <summary>Details</summary>

Background: 星系形态序列（morphological sequence）的形成机制与暗物质晕（dark matter halo）中星系质量分数（galaxy mass fraction）的峰值特征密切相关。

Data: data部分：研究采用基于暗物质晕合并的简单星系演化模型（galaxy evolution model including mergers），结合观测已知的星系质量分数（galaxy mass fraction）在暗物质晕质量$M_{\rm halo} \sim 10^{12} {\rm M}_\odot$处达到峰值的特征。

Method: 采用包含并合过程的简单星系演化模型（simple galaxy evolution model including mergers）分析星系形态序列的成因。

Result: 暗物质晕（dark matter halo）中星系质量比例在$M_{\rm halo} \sim 10^{12} {\rm M}_\odot$处达到峰值的特征是导致星系形态序列（morphological sequence）形成的根本原因。
...
### [9] [BayeSED-GALAXIES II. Bayesian full spectrum analysis of galaxies and application in the CSST wide-field slitless spectroscopy survey](https://arxiv.org/abs/2602.19451)
...
**TL;DR**: 开发了贝叶斯全光谱分析方法BayeSED3（含星云发射建模和模型缩放因子处理），显著提升了中国空间站望远镜（CSST）低信噪比无缝光谱数据的红移（redshift）测量精度（恒星形成星系σNMAD=0.0008，静止星系σNMAD=0.0015），并可靠恢复了恒星质量和恒星形成率参数。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用中国空间站望远镜(CSST)的低分辨率无缝光谱数据，通过贝叶斯全光谱分析方法提升星系红移和物理参数测量精度，以支持宇宙学和星系演化研究。

Data: 数据来源于中国空间站望远镜（CSST）的宽视场多波段测光成像和无缝缝光谱巡天观测，以及基于CESS模拟器（包括仪器效应和自混合效应）生成的中位信噪比（SNR）为1.65的模拟数据。

Method: 采用贝叶斯全光谱分析方法（BayeSED3），整合了星云发射模型（通过Cloudy模拟）和模型缩放因子的贝叶斯处理，提升了低信噪比（SNR）光谱分析的可靠性。

Result: BayeSED3通过贝叶斯全光谱分析（包括星云发射建模和模型缩放因子的贝叶斯处理）显著提高了低信噪比（SNR）光谱的可靠性，在模拟数据（中位SNR=1.65）中实现了优异红移精度（σNMAD=0.0008对恒星形成星系，σNMAD=0.0015对静止星系）。
...
### [10] [SMMAN: quasi-Simultaneous Multi-wavelength Monitoring of gamma-ray-loud AGNs with the Nanshan 26-m radio telescope](https://arxiv.org/abs/2602.19524)
*Lang Cui,Krishna Mohana A,Xin Wang,Ning Chang,Guiping Tan,Xiang Liu*

**TL;DR**: 通过南shan26米射电望远镜（SMMAN）对131个北天区伽马射线噪活动星系核（AGN）进行8年多波段（4.8和23.6 GHz）监测，发现BL Lac天体比平谱射电类星体（FSRQ）变异性更强且谱指数更平，而FSRQ具有更高射电光度和伽马射线噪度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核(AGN)在射电波段的流量密度变化特性及其与吸积和等离子体喷流等物理过程的关联。

Data: 数据来源于南山26米射电望远镜（Nanshan 26-m radio telescope）在2016至2024年间对131个北天区（δ>∼0°）伽马射线强活动星系核（gamma-ray-loud AGNs）进行的4.8和23.6 GHz双频段准同步多波段监测（quasi-Simultaneous Multiwavelength Monitoring），样本选自费米大面积望远镜第三源表（Fermi Large Area Telescope third source catalog）。

Method: 采用准同步多波段监测技术（quasi-Simultaneous Multiwavelength Monitoring）结合南山26米射电望远镜（Nanshan 26-m radio telescope）在4.8和23.6 GHz频段的长期观测数据。

Result: BL Lac天体在23.6 GHz的射电变率（variability）高于4.8 GHz且比平谱射电类星体（FSRQs）更剧烈，而FSRQs的射电光度（radio luminosity）高于BL Lac天体、不确定类型耀变体候选体（BCUs）和射电星系（RDGs）。
...
### [11] [Probing dark matter distributions with the pericentre precession of the stellar orbits near the Galactic Centre black hole](https://arxiv.org/abs/2602.19553)
*Debojit Paul,Sanjeev Kalita*

**TL;DR**: 该论文通过分析银河系中心黑洞附近13颗S星（S-stars）的近心点位移（pericentre shift），研究了不同暗物质（dark matter）质量分布对恒星轨道的影响，发现低偏心率（low eccentricity）和宽轨道恒星是探测暗物质诱导进动（dark matter induced precession）的有效探针，现有及未来天体测量设施（如Keck、GRAVITY和TMT）可区分不同暗物质分布并推断银河系中心区域形成历史。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用银河系中心黑洞（Galactic Centre black hole）周围恒星轨道探索未知物质分布和新引力物理现象。

Data: 研究中使用的数据来源包括13颗S星（S-stars）的低偏心率和高偏心率轨道观测数据，以及近期测量的S2轨道偏差参数（deviation parameter）$f_{sp}$，并结合凯克望远镜（Keck）、GRAVITY和三十米望远镜（TMT）等现有及未来天体测量设施的观测能力。

Method: 通过恒星轨道近心点位移（pericentre shift）分析不同暗物质质量分布（dark matter mass profiles），并利用现有及未来天体测量设施（astrometric facilities）检测暗物质引起的进动（precession）和区分暗物质分布模型。

Result: 银河中心黑洞附近低偏心率宽轨道恒星是探测暗物质诱导进动的有效手段，现有及未来天体测量设施（如Keck、GRAVITY和TMT）可区分不同暗物质密度分布（dark matter density profiles）并推测银河系中心区域形成历史。
...
### [12] [MEDUSA I. Tracing magnetic field structures in tidal arms of the dwarf-dwarf merger NGC 1487](https://arxiv.org/abs/2602.19638)
...
**TL;DR**: 研究发现矮星系（dwarf galaxy）合并事件NGC 1487能快速放大并形成跨越约3千秒差距（kpc）的有序大尺度磁场（large-scale magnetic field），揭示了低质量星系合并对早期宇宙星系磁化的重要贡献。


<details>
  <summary>Details</summary>

Background: 矮星系(dwarf galaxies)是研究宇宙磁场的理想对象，因其低质量和弱引力势仍能通过湍流发电机(turbulent dynamo)维持强磁场。

Data: 数据来源于MeerKAT L波段（1.28 GHz）和澳大利亚望远镜紧凑阵列（ATCA）的L/S波段（2.1 GHz）、C波段（5.5 GHz）及X波段（9 GHz）的高灵敏度多波段射电连续谱数据。

Method: 采用极化分析和旋转测量（RM）合成技术分析NGC 1487的磁场构型，结合多波段射电连续谱数据（MeerKAT L波段和ATCA L/S、C、X波段）测定磁场强度并区分大尺度与湍流分量。

Result: 在矮星系NGC 1487的并合过程中，即使低质量星系也能通过小尺度发电机（small-scale dynamo）快速放大并形成跨越约3千秒差距（kpc）的有序大尺度磁场结构，这对早期宇宙星系磁化过程具有关键贡献。
...
### [13] [Searching for and characterizing halo substructures with the GALAH DR4 survey](https://arxiv.org/abs/2602.19647)
...
**TL;DR**: 通过分析银河系(Galaxy)恒星晕的化学动力学特征，识别出五个不同起源的亚结构（包括GSE、Thamnos等），其独特的元素丰度模式支持它们源自星系吸积事件。


<details>
  <summary>Details</summary>

Background: 银河系恒星晕（stellar halo）由不同起源的星族组成，其形成受多次吸积事件（accretion events）影响，研究旨在通过化学和运动学特征揭示银河系及其他旋涡星系的形成机制。

Data: 研究使用了GALAH DR4（Galactic Archaeology with HERMES Data Release 4）和盖亚（Gaia）卫星的观测数据。

Method: 采用小波变换在sqrt(J_r)和方位角作用量(L_z)空间识别运动学超密度结构，并结合15种元素丰度进行无监督机器学习算法t-SNE化学标记。

Result: result：利用小波变换和t-SNE算法识别出银河系晕中的五个动力学结构（GSE、Thamnos1、Thamnos2等），其独特的化学动力学特征支持其为外银河系起源。
...
### [14] [Chemodynamical properties of gas-rich galaxies: a comparison of observations and simulations](https://arxiv.org/abs/2602.19707)
*Anna Velichko,Yves Revaz,Annalisa De Cia,Cédric Ledoux,Jens-Kristian Krogager,Céline Péroux,Benedetta Casavecchia*

**TL;DR**: 通过对比类星体阻尼莱曼α吸收体(DLA)星系与GEAR化学动力学模拟，发现DLA的[α/Fe]和[M/H]丰度与模拟星系气体丰度趋势一致，且Δv90速度宽度低于和高于100 km/s分别对应log M*为6-8和8-11的星系质量范围，同时指出观测数据在约束[α/Fe]与特定恒星形成率(sSFR)关系时需谨慎处理系统误差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于类星体阻尼莱曼α吸收体（DLA）星系的化学与动力学特性，并将其与GEAR化学动力学模拟进行对比，以探索星系化学演化与动力学过程。

Data: 数据来源于使用化学动力学树状平滑粒子流体动力学（Tree-SPH）代码GEAR模拟的五颗恒星质量在log(M*/Msol)介于6.1至10.8之间、红移介于0.33至4.12之间的星系。

Method: 采用化学动力学树状平滑粒子流体动力学（Tree-SPH）代码GEAR对类星体阻尼莱曼α（DLA）星系进行模拟，并对比观测数据以约束α元素增强与金属丰度、[α/Fe]与特定恒星形成率（sSFR）的关系以及速度宽度（Δv90）与恒星质量、金属丰度的关联。

Result: 观测到的DLA（damped Lyman-α）星系中星际介质（ISM）的丰度比[α/Fe]和[M/H]与模拟星系气体中的丰度趋势重叠。
...
### [15] [MIGHTEE: Discovery of a triple-double radio galaxy](https://arxiv.org/abs/2602.19729)
...
**TL;DR**: 发现并分析了红移（redshift）约0.94的星系宿主中的新三对双射电星系（TDRG）J022248-060934，通过谱龄分析揭示其核活动具有短休眠期和快速循环周期（90%），反驳了以往基于运动学年龄认为三对双结构源于长期静止期的观点。


<details>
  <summary>Details</summary>

Background: 研究主题为利用三对射电瓣（triple-double radio galaxies, TDRGs）约束射电星系（radio galaxies, RGs）的核活动周期（duty cycle），并探讨其核活动间歇期特征。

Data: 主要数据来源于MIGHTEE-DR1数据集和MIGHTEE子波段图像。

Method: 采用JP模型（JP model）对源各组分进行独立的光谱年龄拟合。

Result: 发现了一个新的三重双射电星系（TDRG）J022248\m060934，其宿主星系的光谱红移为$z \approx$ 0.94。
...
### [16] [The morphological stability of open clusters: a new 2D perspective](https://arxiv.org/abs/2602.19746)
*Yuting Li,Qingshun Hu,Yufei Cai,Yu Dai,Mingfeng Qin,Yangping Luo*

**TL;DR**: 首次定义核心与外围成员数比值(Ncore/Nouter)作为形态稳定性参数，发现其与总成员数(N)的正相关性(斜率1.140±0.039)比几何参数(Score/Souter)更显著，且银心方向潮汐力对疏散星团(OC)形态的影响大于银河盘较差转动产生的剪切力，同时年轻星团(<30 Myr)因内部加热和外部扰动比年老星团(>800 Myr)更不稳定。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系疏散星团（open clusters, OCs）形态稳定性随成员数量变化的规律及其空间位置依赖性。

Data: 数据来源于文献中的疏散星团（open clusters, OCs）编目。

Method: 首次定义形态稳定性参数Ncore/Nouter（核心与外围成员数之比），并分析其与N的相关性，发现核心恒星密度比几何特征更能敏感反映形态稳定性。

Result: 首次定义的核心与外围成员数比值（Ncore/Nouter）与总成员数（N）呈显著正相关（斜率1.140±0.039），表明核心恒星密度比几何形态（Score/Souter）更能敏感反映疏散星团（OC）的形态稳定性。
...
### [17] [Probing the Milky Way Halo with RR Lyrae Stars from Gaia Data Release 3](https://arxiv.org/abs/2602.19858)
*T. Muraveva,L. Monti,D. Massari,M. De Leo,A. Garofalo,G. Clementini,E. Ceccarelli,U. Michelucci*

**TL;DR**: 通过分析4933颗天琴座RR型变星(RRLs)的轨道参数和金属丰度，确定了银河系(Gaia Sausage/Enceladus、Sequoia、Helmi streams等)主要吸积遗迹的化学演化特征，发现Thamnos具有双峰金属丰度分布，并验证了Shiva和Shakti结构可能为原生成分。


<details>
  <summary>Details</summary>

Background: 银河系（Milky Way）恒星晕作为等级质量组装的化石记录，保存了早期吸积事件的遗迹。

Data: 数据来源为盖亚(Gaia)卫星观测的4933颗天琴座RR型变星(RRLs)样本，并计算了其运动积分和轨道参数。

Method: 采用域信息新颖性检测框架CLiMB（CLustering in Multiphase Boundaries）识别银河系子结构中的天琴RR变星（RRLs）成员。

Result: 通过分析RR Lyrae变星(RRLs)的金属丰度分布，发现银河系(Gaia Sausage/Enceladus、Sequoia和Helmi streams)主要吸积遗迹的化学演化状态在早期阶段具有显著差异，其中Thamnos的金属丰度分布呈双峰特征，贫金属峰([Fe/H]=-1.94±0.20 dex)代表真实的吸积成分。
...
### [18] [Bar Formation During a Gaia-Sausage-Enceladus-like Merger Event](https://arxiv.org/abs/2602.19995)
*Bin-Hui Chen,Juntai Shen,Paola Di Matteo*

**TL;DR**: 通过N体模拟研究卫星星系与中心盘星系合并对棒旋结构（bar）的影响，发现棒结构存活概率随质量比（m/M）增加而降低，小质量比（m/M≲1/10）合并通常保留棒结构而大质量比（m/M≥1/2）易破坏棒结构形成早型星系残余体，中等质量比（1/5≤m/M≤1/3）可能产生逐渐衰减的“弱化棒”，而撞击参数（b）和轨道倾角（θ_i）对棒存活仅产生次要随机影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系棒（bar）的形成机制，特别是探讨外部相互作用（如并合事件）对星系棒形成与存续的影响。

Data: 数据来源于一套N体(N-body)模拟数据集，模拟了类似盖亚香肠-恩克拉多斯(Gaia-Sausage-Enceladus)的径向偏向卫星盘星系与中心盘星系在棒状结构形成过程中的合并过程，其中中心星系固定不变，而卫星星系在质量比m/M、碰撞参数b和轨道倾角θi三个参数上变化。

Method: 采用N体数值模拟方法，通过构建不同质量比(m/M)、撞击参数(b)和轨道倾角(θi)的星系并合模型，研究卫星星系对中心星系棒状结构形成与存续的影响。

Result: 棒状结构（bar）的存活概率随卫星星系与中心星系质量比（$m/M$）增大而降低，质量比小于1/10时棒状结构通常保留，大于等于1/2时易被破坏，中等质量比（1/5至1/3）则可能形成逐渐衰减的"弱化棒状结构"（weakening bar）。
...
### [19] [Simulation of proton radiolysis of H2O and O2 ices with the Nautilus code](https://arxiv.org/abs/2602.19996)
*Tian-Yu Tu,Valentine Wakelam,Jean-Christophe Loison,Marin Chabot,Emmanuel Dartois,Yang Chen*

**TL;DR**: 通过调整化学模型参数（包括反应-扩散竞争、非扩散化学及超热物种解吸能），在Nautilus天体化学代码中引入宇宙射线(CRs)辐解过程，成功模拟了H2O和O2辐解实验中[H2O2]/[H2O]与[O3]/[O2]_0的稳态丰度比，但需进一步约束冰体内部离子化学及反应网络的活化能与分支比。


<details>
  <summary>Details</summary>

Background: 宇宙射线(CRs)的辐射分解效应在分子云化学中起重要作用，通过解吸尘埃颗粒上的分子产生超热物种和自由基，促进大分子形成。

Data: 数据来源于Nautilus天体化学模拟代码（Nautilus astrochemical code）中调整参数后的模拟结果，包括反应扩散竞争、非扩散化学及超热物种解吸能等敏感性分析，并与H2O和O2辐解实验中的稳态丰度比[H2O2]/[H2O]和[O3]/[O2]_0进行对比验证。

Method: 采用Nautilus天体化学代码（Nautilus astrochemical code）并引入辐射分解过程及相关反应，通过调整反应-扩散竞争、非扩散化学及超热物种解吸能等参数来模拟H2O冰的化学演化。

Result: 通过微调化学模型可以重现实验中[H2O2]/[H2O]和[O3]/[O2]_0的稳态丰度比，但仍需对冰体内部离子化学、反应活化能及分支比等中间路径施加更多约束。
...
### [20] [The major merger-active galactic nucleus connection up to the cosmic noon](https://arxiv.org/abs/2602.19997)
*A. La Marca,B. Margalef-Bentabol,L. Wang,S. C. Trager,V. Rodriguez-Gomez,G. Martin*

**TL;DR**: 研究发现星系大合并（major merger）在红移$z=0.5$-$2$期间能适度增强活动星系核（AGN）活动，且合并比例随吸积盘光度（L$_{disc}$）单调上升，对高光度AGN（L$_{disc} \gtrsim 10^{46}\,{erg\,s^{-1}}$）的触发起主导作用。


<details>
  <summary>Details</summary>

Background: 星系并合（galaxy major mergers）与活动星系核（AGN）触发机制的关系是当前星系演化研究的核心议题之一，尤其在宇宙红移$z=0.5$-$2$范围内存在显著争议。

Data: 数据来源于COSMOS（Cosmic Evolution Survey）天区的多波段数据集和詹姆斯·韦伯太空望远镜（JWST）观测数据。

Method: 利用卷积神经网络（CNN）分析模拟JWST观测数据来识别星系并合事件。

Result: 星系并合（major mergers）在红移z=0.5-2区间能触发活动星系核（AGN）活动，且并合对高光度AGN（L$_{disc} \gtrsim 10^{46}\,{erg\,s^{-1}}$）的触发作用尤为显著。
...
**TL;DR**: 通过引入基于模拟的先验（SBPs）分析DESI全形状聚类数据，显著提升了ΛCDM宇宙学参数约束精度，测得物质密度参数Ω_m=0.2987±0.0066、哈勃常数H_0=68.80±0.35 km s⁻¹Mpc⁻¹和质量涨落幅度σ_8=0.766±0.015，并首次在中微子质量约束中得出w_0w_aCDM模型下M_ν<0.090 eV的最强上限。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用基于模拟的先验（SBPs）增强宇宙学参数约束，特别是对ΛCDM模型、暗能量状态方程（w0waCDM）和中微子质量的精确测定。

Data: 数据来源于DESI公开的全形状聚类数据（DESI full-shape clustering data）和基于场级模拟（field-level simulations）生成的定制化晕占据分布模型（HOD）数据。

Method: 采用基于模拟的先验（SBPs）结合归一化流（normalizing flows）拟合有效场论（EFT）参数分布，并利用定制化的晕占据分布（HOD）模型生成场级模拟数据。

Result: 采用基于模拟的先验(SBPs)分析DESI全形状聚类数据显著提升了宇宙学参数限制，得到物质密度参数Ω_m=0.2987±0.0066、哈勃常数H_0=68.80±0.35 km s⁻¹Mpc⁻¹和质量涨落幅度σ_8=0.766±0.015（或透镜参数S_8=0.764±0.018），精度分别比基线结果提高1%、40%和50%。
...
**TL;DR**: 基于IllustrisTNG模拟对812个星系团质量晕的研究发现，暗物质溅落半径(splashback radius)与气体激波面(accretion shock)存在显著偏移(Rsh/Rsp~1.3-2)，该偏移主要沿空洞方向且源于并合过程中气体通过耗散能量抵抗收缩的机制。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系团外围暗物质（dark matter）与气体边界（分别由溅落半径（splashback radius）和吸积激波（accretion shock）界定）的理论预期一致性及其实际位移现象。

Data: 数据来源于IllustrisTNG模拟套件（IllustrisTNG simulation suite），分析了812个星系团质量晕（cluster-mass halos）样本中溅落半径（splashback radius）和激波（shock）表面的统计关系。

Method: 采用基于弥散度（dispersion-based）的溅落半径（splashback radius）测量方法和最大熵距离（maximum entropy distance）的激波位置定位技术，结合全角度分布统计分析与不同矩（moments）的关联性研究。

Result: 在星系团外围，无碰撞暗物质形成的溅落半径（splashback radius）与气体形成的吸积激波（accretion shock）边界存在显著偏移，其比值Rsh/Rsp约为1.3-2，且该偏移主要沿空洞方向发生。
...
**TL;DR**: 结合DESI（Dark Energy Spectroscopic Instrument）DR1的全形状（full-shape）聚类测量和DR2的重子声波振荡（baryon acoustic oscillation, BAO）数据，采用ShapeFit压缩方法首次在贝叶斯框架下仅依赖大爆炸核合成（Big Bang Nucleosynthesis）对重子密度的先验和宽谱指数先验，获得了ΛCDM扩展模型的可靠约束，测得平坦ΛCDM模型中Ω_m=0.3035±0.0085、h=0.6876±0.0059、σ_8=0.822±0.034，动态暗能量模型w_0w_aCDM中w_0=0.49±0.25、w_a=-1.52±0.77，并将与ΛCDM的差异缩小至1.4σ，同时在中微子质量和空间曲率方面取得竞争性限制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用暗能量光谱仪（DESI）数据结合重子声波振荡（BAO）和全形状（FS）聚类测量，在最小外部先验条件下探索超越ΛCDM（冷暗物质）宇宙学模型的扩展参数约束。

Data: 数据来源包括DESI DR1的全形状（full-shape, FS）聚类测量数据、DESI DR2的重子声波振荡（baryon acoustic oscillation, BAO）测量数据以及用于估计联合协方差的EZmocks模拟数据。

Method: 采用ShapeFit压缩方法和EZmocks估计联合协方差，结合DESI DR1的全形状（FS）聚类测量与DR2的重子声学振荡（BAO）测量，以解决数据发布间相关性并减少先验体积效应。

Result: 首次在贝叶斯框架内仅使用DESI数据并结合大爆炸核合成（Big Bang Nucleosynthesis）对重子密度的先验及谱指数的宽先验，可靠地约束了ΛCDM模型的扩展参数，测得平坦ΛCDM模型中Ω_m=0.3035±0.0085、h=0.6876±0.0059、σ_8=0.822±0.034，动态暗能量模型w0waCDM中w0=0.49±0.25、wa=-1.52±0.77，并将与ΛCDM的差异缩小至1.4σ。
...
**TL;DR**: 研究了两种形式的修正引力理论(f(R,T)=R+αT^n)，发现其参数(α和n)允许小幅偏离ΛCDM模型但观测上不可区分，能重现宇宙从减速到加速膨胀的转变并满足能量条件(NEC、DEC)而违反SEC，为解释晚期加速膨胀提供了可行替代方案。


<details>
  <summary>Details</summary>

Background: 研究探讨了两种形式的修正引力理论（$f(R,T)$ gravity）在宇宙学中的应用，重点关注其对宇宙加速膨胀的解释能力及其与标准宇宙学模型（$Λ$CDM）的兼容性。

Data: 数据来源于多种观测数据集，通过χ²最小化和MCMC分析（使用emcee算法）对模型参数进行约束。

Method: 采用修正弗里德曼方程和χ²最小化结合MCMC（emcee算法）分析约束f(R,T)=R+αTⁿ引力模型的自由参数α和n。

Result: 两种$f(R,T) = R + αT^n$引力模型均能重现晚期宇宙加速膨胀，其观测结果与$Λ$CDM模型不可区分，但允许通过参数$α$和$n$表征微小偏差。
...
**TL;DR**: 基于JWST早期数据发现高红移（high-$z$）星系和活动星系核（AGN）的红移分布存在对数周期性（log-periodic）结构（$\Delta x \simeq 0.34$），支持玻色-爱因斯坦凝聚体（BEC）宇宙学中循环凝聚体动力学作为离散尺度不变性（discrete scale invariance）的起源。


<details>
  <summary>Details</summary>

Background: 研究探讨了高红移(high-$z$)宇宙中星系和活动星系核(AGN)红移分布的周期性结构及其与玻色-爱因斯坦凝聚态(BEC)宇宙学中循环凝聚动力学的潜在联系。

Data: data：研究使用了JWST（James Webb Space Telescope）早期观测数据中高红移星系和活动星系核（active galactic nuclei）的红移分布数据。

Method: 采用基线校正的无分箱频率分析(baseline-corrected unbinned frequency analysis)检测变量$x=\ln(1+z)$中的周期性结构。

Result: 在早期JWST数据中发现高红移星系和活动星系核的红移分布存在统计显著的$\Delta x \simeq 0.34$对数周期结构，支持玻色-爱因斯坦凝聚体(BEC)宇宙学中循环凝聚动力学作为离散标度不变性的可能起源。
...
**TL;DR**: 研究了两种修正引力理论（Starobinsky模型和Odintsov-Oikonomou幂律模型）在爱因斯坦框架下通过引力重子生成（gravitational baryogenesis）产生宇宙重子-反重子不对称性，发现两种模型计算的重子不对称因子η值分别为(1.05-1.46)×10⁻¹¹和(1.06-1.53)×10⁻¹¹，接近观测值8.65×10⁻¹¹，且通过调整质量参数M∗可进一步提高吻合度。


<details>
  <summary>Details</summary>

Background: 研究探讨了在两种修正引力理论（f(R)引力）框架下通过引力重子生成（gravitational baryogenesis）机制解释宇宙中重子-反重子不对称性的产生问题。

Data: 研究使用了两种f(R)修正引力理论（Starobinsky模型和Odintsov-Oikonomou幂律模型）的解析近似解，通过标量场（scalaron）势能推导了慢滚暴胀时期的里奇标量（Ricci scalar）、时间导数、哈勃参数（Hubble parameter）和宇宙尺度因子的表达式。

Method: 采用爱因斯坦框架下的解析近似方法，通过标量场（scalaron）势能推导慢滚暴胀时期的运动方程，并由此获得里奇标量（Ricci scalar）、其时间导数、哈勃参数（Hubble parameter）和宇宙尺度因子的解析表达式。

Result: 在爱因斯坦框架下研究了两种修正引力理论（Starobinsky模型和Odintsov-Oikonomou幂律模型）通过引力重子生成机制产生的重子-反重子不对称性，计算得到的重子不对称因子η值分别为(1.05-1.46)×10⁻¹¹和(1.06-1.53)×10⁻¹¹，当质量参数M₊略低于普朗克质量（0.4M_{Pl}）时与观测值η=8.65×10⁻¹¹相符。
...
**TL;DR**: 研究提出基于指数势的精华场（quintessence field）模型，通过引入早期宇宙的动能主导（kination）阶段，将暗能量精细调节需求降低数十个数量级，并探讨了通过引力波背景观测检验该模型的方法。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用指数势五元场（quintessence field）解释DESI星系巡天提出的时变暗能量（time-varying dark energy）现象。

Data: 数据来源于DESI（Dark Energy Spectroscopic Instrument）星系巡天观测和早期宇宙动力学（kination）阶段的模拟分析。

Method: 采用指数势能的精质场（quintessence field）模型，结合早期宇宙的动能主导（kination）阶段，通过解析推导与宇宙大爆炸核合成（Big Bang Nucleosynthesis）及当前暗能量密度一致的初始条件约束。

Result: 与宇宙学常数相比，指数势五重态场（quintessence field）模型将暗能量的精细调节需求降低了数十个数量级。
...
**TL;DR**: 该研究通过第四阶段宇宙微波背景（CMB）和光学巡天的层析交叉关联（tomographic cross-correlations）方法，结合热苏尼亚耶夫-泽尔多维奇效应（tSZ）、星系成团性和弱引力透镜效应，在完整考虑系统误差和参数边际化后，实现了对星系团流体静力学质量偏差（$b_{\mathrm{HSE}}$）的亚百分之一级精度标定（最优达0.98%），并通过红移信息将$b_{\mathrm{HSE}}$与$σ_8$的简并性打破，使该偏差对宇宙学参数的限制误差降低约三倍。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用第四阶段宇宙微波背景（CMB）和光学巡天数据，通过层析交叉相关方法校准星系团宇宙学中的流体静力学质量偏差（hydrostatic mass bias, $b_{\mathrm{HSE}}$），以解决其与宇宙学参数$σ_8$和$Ω_m$的简并性问题。

Data: 研究使用了来自第四阶段宇宙微波背景（CMB）和光学巡天（Stage-IV CMB and optical surveys）的观测数据，包括热苏尼亚耶夫-泽尔多维奇效应（tSZ）、星系成团性和弱引力透镜的层析互相关数据，并考虑了巡天噪声、前景模型（如聚集宇宙红外背景和射电源）以及天体物理和宇宙学干扰参数的全边际化处理。

Method: 采用费希尔预测(Fisher forecast)方法结合优化的层析分箱(tomographic binning)技术，通过热苏尼亚耶夫-泽尔多维奇效应(tSZ)、星系成团性和弱引力透镜的层析互相关分析，在完整边缘化处理宇宙学和天体物理干扰参数条件下校准流体静力学质量偏差(b_HSE)。

Result: 利用第四阶段宇宙微波背景(CMB)和光学巡天的层析交叉关联分析，在考虑系统误差和天体物理干扰参数全边际化后，能够将流体静力学质量偏差(b_HSE)的约束精度提升至约1-2.4%，较非层析方法提高约三倍。
...
**TL;DR**: 研究发现在拉斯托引力（Rastall gravity）框架下，原始黑洞（primordial black holes）的形成过程因能量-动量张量非守恒性导致密度涨落增长、临界形成阈值和视界穿越涨落幅度的显著改变，使其成为探测修正引力的新工具。


<details>
  <summary>Details</summary>

Background: 早期宇宙形成的原初黑洞（primordial black holes）是暗物质（dark matter）的潜在候选者，研究其在引入物质与几何非最小耦合的拉斯托引力（Rastall gravity）中的生成机制。

Data: 数据来源于对辐射主导时期宇宙扰动的理论分析，结合了当前宇宙学约束条件（包括大爆炸核合成、宇宙微波背景辐射和大尺度结构观测）。

Method: 采用Rastall引力理论（Rastall gravity）分析辐射主导时期宇宙扰动，研究能量-动量张量非守恒性对密度涨落增长、黑洞形成临界阈值及视界穿越涨落幅度的动力学影响。

Result: Rastall引力（Rastall gravity）中的Rastall参数通过改变密度涨落的增长、黑洞形成的临界阈值和视界穿越时的涨落幅度，显著影响了原初黑洞（primordial black holes）的产生效率。
...
**TL;DR**: 首次联合使用普朗克2018年数据和BICEP/Keck 2018年B模式测量，对宇宙弦（cosmic strings）和畴壁（domain walls）的张力给出了更严格的限制，发现虽无统计显著证据支持缺陷存在，但观测到宇宙弦张力非零的微弱倾向，并将缺陷功率谱约束提升至多两倍，其中阿贝尔-希格斯弦（Abelian-Higgs strings）的改进尤为显著。


<details>
  <summary>Details</summary>

Background: 研究利用普朗克(Planck)2018和BICEP/Keck 2018 B模式数据，首次对宇宙弦(cosmic strings)和畴壁(domain walls)稳定网络的宇宙微波背景(CMB)限制进行综合分析。

Data: 数据来源包括普朗克卫星2018年全数据集（Planck 2018 data）和BICEP/Keck 2018年的B模偏振测量（BICEP/Keck 2018 $B$-mode measurements）。

Method: 采用非连接线段模型（Unconnected Segment Model）计算宇宙弦（Nambu-Goto和Abelian-Higgs）及稳定畴壁（domain walls）引起的各向异性，并结合马尔可夫链蒙特卡洛（Markov Chain Monte Carlo）方法联合分析所有ΛCDM参数、张量标量比及弦/畴壁张力。

Result: 首次联合使用普朗克（Planck）2018年完整数据和BICEP/Keck 2018年B模式测量数据，未发现宇宙弦（cosmic strings）和畴壁（domain walls）存在的统计显著证据，但对非零宇宙弦张力存在轻微偏好。
...
### [31] [The Impact of Degraded Charge Transfer Efficiency on Extended Sources in ACS/WFC](https://arxiv.org/abs/2602.18605)
*David V. Stark,M. Chiaberge,N. A. Grogin*

**TL;DR**: 基于哈勃太空望远镜ACS/WFC（高级巡天相机/广域通道）对星系团17年重复观测数据的分析，发现电荷转移效率(CTE)退化对大孔径测光（背景>20e^-/像素且单次曝光源亮度>300e^-时误差<0.05星等）影响较小，但对小尺度测光（误差常超0.1星等）和星系光分布对称性分析（需背景>20e^-/像素且目标距串行寄存器<1536像素）影响显著，并指出ACS数据处理流程的像素级CTE校正算法未能完全还原扩展源内电荷原始位置。


<details>
  <summary>Details</summary>

Background: 研究电荷转移效率（CTE）退化对哈勃太空望远镜（HST）ACS/WFC拍摄的延展源（extended sources）测光精度的影响。

Data: 数据来源于哈勃太空望远镜（HST）搭载的先进巡天相机（ACS）宽视场通道（WFC）在十七年基线内对同一星系团（galaxy cluster）的重复成像观测。

Method: 采用基于哈勃太空望远镜（HST）ACS/WFC（Advanced Camera for Surveys/Wide Field Channel）的长期重复成像数据，分析电荷转移效率（CTE）退化对延展源测光的影响，重点考察时间、探测器位置、源亮度和局部背景水平等因素。

Result: 大孔径测光在曝光背景高于20e^-/像素且单次曝光源亮度超过~300e^-时，跨WFC探测器的测量结果通常可靠（误差~0.05星等）。
...
### [32] [Variational views for self-supervised learning in radio astronomy](https://arxiv.org/abs/2602.18923)
*Johnny Joseph Alphonse,Anna M. M. Scaife*

**TL;DR**: 提出结合变分自编码器（VAE）生成视图与标准图像增强的自监督学习（SSL）方法，改善了射电星系形态分类性能，揭示了生成式与对比式方法的互补性。


<details>
  <summary>Details</summary>

Background: 现代天文巡天项目产生的数据集日益庞大复杂，传统依赖大量标注数据的监督学习方法面临挑战，促使自监督学习（SSL）成为从无标注图像中提取结构信息的可扩展解决方案。

Data: 数据来源于Radio Galaxy Zoo（RGZ）数据集，用于训练β-VAE（Variational Autoencoder）以生成增强视图，并用于基于视图的自监督学习（self-supervised learning）模型训练。

Method: 采用变分自编码器（VAE）生成视图并结合基于视图的自监督学习（SSL）算法进行预训练，通过β-VAE在RGZ数据集上平衡重构质量与生成因子解耦（如源多重性和瓣不对称性）。

Result: 使用变分自编码器(VAE)生成增强视图结合标准图像增强，可提升射电星系形态分类的下游任务性能。
...
### [33] [The ngVLA Band-4 Water Vapor Radiometry Concept](https://arxiv.org/abs/2602.18975)
*Tirupati Kumara Sridharan,Robert Lehmensiek,Yoshiharu Asaki*

**TL;DR**: 提出利用ngVLA标准波段4（Band-4）科学接收机和主天线替代独立水汽辐射计（WVR）的方案，该方案能更准确探测对流层低层水汽分布、降低接收机温度稳定性要求并减少独立单元部署维护成本。


<details>
  <summary>Details</summary>

Background: 研究探讨了利用标准Band-4科学接收器和主天线替代独立水汽辐射计（WVR）系统以优化高频段（5和6；30.5-116 GHz）对流层相位校正的可行性。

Data: 数据来源为标准Band-4科学接收器和主天线（main antennas），替代了独立1.5米天线（standalone 1.5 m antenna）方案。

Method: 采用标准Band-4科学接收器和主天线作为水汽辐射计（WVR）的替代方案，通过将Band-4馈源置于Bands 5和6馈源之间实现0.8度波束偏移。

Result: 利用标准Band-4科学接收机和主天线实现水汽辐射计(WVR)的方案能更准确探测对流层低层水汽分布，显著降低接收机温度稳定性要求，并减少独立单元部署维护成本。
...
### [34] [Astrometric properties of reference frame sources as a function of redshift](https://arxiv.org/abs/2602.19135)
*Zhiyun Zhang,N. Liu,Xiaxuan Zhang,I. Nurul Huda,Sufen Guo,Z. Zhu,J. -C. Liu,J. Yao,Z. -W. Wang,H. -F. Yu,D. -D. Zhang*

**TL;DR**: 通过分析约22,000个致密射电源（AGN）的射电（VLBI）和光学（Gaia）天体测量特性，发现射电天体测量特性与红移无关，而光学天体测量特性（包括射电-光学偏移量、自行运动和G星等）在红移0<z<5范围内呈现显著变化，主要受Gaia观测性能随G星等变化及G星等随红移演化影响。


<details>
  <summary>Details</summary>

Background: 研究探讨了活动星系核(AGN)的天体测量特性（如射电-光学偏移量）与红移(z)之间的相关性，扩展了基于国际天球参考架(ICRF3)的先前发现。

Data: 数据来源于射电基本星表（RFC）约22,000个致密射电源的全天样本，并结合暗能量光谱仪数据发布1（DESI DR1）、斯隆数字巡天数据发布17/19（SDSS DR17/19）及NASA/IPAC河外数据库（NED）补充红移数据，同时与盖亚数据发布3（Gaia DR3）交叉匹配获得4,068个具有可靠光谱红移和分类（星系与类星体）的RFC天体样本。

Method: 采用甚长基线干涉测量（VLBI）和盖亚（Gaia）光学天体测量数据，系统分析了4,068个致密射电源的射电-光学天体测量特性与红移的关系。

Result: VLBI（甚长基线干涉测量）天体测量特性在达到的精度水平内未显示出与红移的显著相关性。
...
### [35] [Safety Acquisitions: Redundancy for non-repeatable multi-orbit STIS visits](https://arxiv.org/abs/2602.19286)
*Matthew M. Dallas,Matthew R. Siebert*

**TL;DR**: 研究发现哈勃太空望远镜（HST）的STIS目标捕获（STIS ACQ）序列在2018-2023年间约5%的观测中因导星锁定失败导致指向精度不足，且多轨道观测中39%完全失败，建议通过冗余ACQ序列提升多轨道观测的指向准确性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升哈勃太空望远镜（HST）在观测超新星、千新星（kilonovae）、潮汐瓦解事件（tidal disruption events）等不可重复天文现象时，通过优化目标获取（STIS ACQ）序列的冗余设计以确保科学数据在特定时间窗口内精准指向的可靠性。

Data: 研究数据来源于2018年9月至2023年9月期间哈勃太空望远镜（HST）的113次STIS ACQ（目标捕获）序列观测记录，其中特别筛选了因导星定位问题导致失败的案例子集。

Method: 采用哈勃空间望远镜（HST）的STIS目标捕获（STIS ACQ）序列分析观测失败频率，并评估在多轨道观测中引入冗余ACQ序列以恢复指向精度的可行性。

Result: 哈勃空间望远镜（HST）的STIS目标捕获（STIS ACQ）序列在2018年9月至2023年9月期间约有5%出现导星锁定失败问题，且2023年3月至9月失败率上升至约9%。
...
### [36] [Photometry of Fireballs using High Frame Rate Cameras](https://arxiv.org/abs/2602.19447)
*Dale Giancono,Hadrien Devillepoix,Robert Howie,Denis Vida,David Rollinson*

**TL;DR**: 开发了每秒500帧的全天相机系统，采用检测局部自动亮度控制技术，实现了-3至-17星等的高动态范围火球（fireball）光度测量，支持半经验碎片分析并为全球火球观测站（Global Fireball Observatory）下一代仪器提供核心组件。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发高动态范围全天相机系统以精确表征火球（fireball）及其碎裂过程，为揭示流星体内部结构提供关键观测数据。

Data: 数据来源于自主研发的全天相机系统（最高500帧/秒）和传统30帧/秒相机的实地部署对比观测，通过瞬变检测和兴趣区域实时保存处理高吞吐量数据，并捕获了亮度达-15等的火球（fireball）样本。

Method: 采用每秒500帧的全天相机系统，结合新型检测局部自动亮度控制（Detection Localised Auto-brightness Control）技术，通过瞬态检测和感兴趣区域保存实现实时测光。

Result: 开发的全天相机系统通过局部自动亮度控制（Detection Localised Auto-brightness Control）实现了-3至-17星等的高动态范围，成功捕获了亮度达-15星等的火球并最小化饱和。
...
### [37] [Direct comparison of SiPMs and PMTs in operation with a bright background and prospects of using SPADs as truly digital sensors](https://arxiv.org/abs/2602.19814)
...
**TL;DR**: 研究发现硅光电倍增管(SiPM)与传统光电倍增管(PMT)在17米口径MAGIC望远镜成像相机中并行测试时，尽管SiPM具有更高光子探测效率(PDE)，但由于其对长波长夜天光(LoNS)噪声更敏感，最终信噪比(SNR)表现仅与PMT相当。


<details>
  <summary>Details</summary>

Background: 现代地面甚高能伽马射线天文观测中硅光电倍增管(SiPM)与传统光电倍增管(PMT)的协同应用是关键技术突破。

Data: 数据来源于在直径17米的MAGIC望远镜成像相机中并行安装和运行的硅光电倍增管（SiPM）模块与传统光电倍增管（PMT）的长期观测对比实验。

Method: 采用硅光电倍增管(SiPM)与传统光电倍增管(PMT)并行运行的模块化设计，通过信号放大、数字化和幅度校准进行性能对比分析。

Result: 硅光电倍增管(SiPM)尽管具有更高的光子探测效率(PDE)，但在实际观测中仅能达到与光电倍增管(PMT)相当的性能水平。
...
### [38] [Multivariate time-series forecasting of ASTRI-Horn monitoring data: A Normal Behavior Model](https://arxiv.org/abs/2602.19984)
...
**TL;DR**: 开发了基于多层感知机(MLP)的正常行为模型(NBM)，用于ASTRI-Horn切伦科夫望远镜15个物理变量的多变量时间序列预测，在5小时预测范围内实现了0.019±0.003的均方误差(MSE)和0.032±0.009的归一化中位绝对偏差(NMAD)，性能与长短期记忆网络(LSTM)相当但收敛更快。


<details>
  <summary>Details</summary>

Background: 开发用于预测ASTRI-Horn切伦科夫望远镜正常运行时监测时间序列数据的正常行为模型（Normal Behavior Model, NBM），以支持在线异常检测和预测性维护系统的开发。

Data: 数据来源于ASTRI-Horn切伦科夫望远镜（Cherenkov telescope）的望远镜控制单元（Telescope Control Unit）在2022年9月至2024年7月期间采集的15个物理变量，包括方位角（Azimuth）和仰角（Elevation）电机的传感器测量值。

Method: 采用多层感知机（MLP）进行多变量时间序列预测，并通过滑动窗口技术增强数据输入序列。

Result: 开发的多层感知机(MLP)模型在ASTRI-Horn望远镜监测时间序列预测中表现优异，其最佳配置(4隐藏层、每层720单元、300样本输入/预测长度)下测试集均方误差(MSE)为0.019±0.003、归一化中位绝对偏差(NMAD)为0.032±0.009，预测性能与长短期记忆网络(LSTM)相当但收敛更快，且6.5小时内预测效果稳定。
...
### [39] [Cosmological Constraints on Long-Lived Particles Using Dimension-Six Effective Operators](https://arxiv.org/abs/2602.18580)
*Mickael V. S. de Farias,Rodrigo Holanda,Matheus M. A. Paixao,Farinaldo S. Queiroz,Priscila V. dos Santos*

**TL;DR**: 研究了长寿命粒子（LLPs）衰变成暗物质对宇宙学的影响，发现其寿命超过$10^4$秒时会干扰早期宇宙的轻核合成并改变大爆炸核合成（BBN）预测，同时通过有效六维算子描述了参数空间如何满足BBN、结构形成、宇宙微波背景和重子声波振荡数据的宇宙学约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于长寿命粒子（LLPs）在超出标准模型（BSM）物理中的作用及其对早期宇宙核合成（BBN）和有效中微子种类数（$N_{eff}$）的影响。

Data: 使用维度六有效算子（dimension-six effective operators）描述长寿命粒子（LLPs）衰变，并基于大爆炸核合成（BBN）、结构形成、宇宙微波背景（CMB）和重子声学振荡（BAO）的观测数据划定符合宇宙学约束的参数空间。

Method: 采用维度六有效算子（dimension-six effective operators）描述长寿命粒子（LLPs）衰变过程。

Result: 长寿命粒子（LLPs）衰变进入暗物质（dark matter）会改变早期宇宙中轻核合成（BBN）的预测并影响有效中微子种类数（$N_{eff}$）。
...
### [40] [Scaling solutions for varying tension strings](https://arxiv.org/abs/2602.18586)
*C. S. C. M. Coelho,A. -L. Y. Gschrey,C. J. A. P. Martins*

**TL;DR**: 研究了张力随时间变化的宇宙弦（cosmic string）网络演化，发现张力变化中拉伸长度尺度（stretching length scale）比阻尼长度（damping length）对网络密度影响更大。


<details>
  <summary>Details</summary>

Background: 研究宇宙弦（cosmic string）网络在张力随时间变化条件下的动力学行为及其标度解（scaling solutions）的分类和演化特征。

Data: 数据来源于基于速度相关单尺度模型（Velocity-dependent One Scale Model）对张力随时间变化的宇宙弦网络演化进行的数值模拟（simulation），并考虑了宇宙学和非宇宙学背景下两种不同唯象假设对弦动力学中拉伸和阻尼贡献的影响。

Method: 采用速度依赖单尺度模型（Velocity-dependent One Scale Model）分析拓扑缺陷演化，并基于两种不同现象学假设（涉及弦动力学中拉伸和阻尼贡献的差异）对张力随时间变化的弦网络进行分类。

Result: 在时变张力条件下，弦网络（string network）的演化受拉伸长度尺度变化的影响比阻尼长度变化更显著。
...
### [41] [Dark Glueball Direct Detection](https://arxiv.org/abs/2602.18753)
*Ji-Wei Li,Roman Pasechnik,Wei Wang,Zhi-Wei Wang*

**TL;DR**: 研究提出在杨-米尔斯暗区（Yang-Mills dark sector）中胶子球（glueball）作为暗物质（DM）候选者的理论框架，发现当矢量费米子质量（m_ψ）超过5.5倍暗区禁闭能标（Λ_D）时，最轻的C-odd自旋1态（m_χ）可作为可行暗物质候选者，并通过有效场论预测了核子相干弹性散射截面（σ_SI）与Λ_D和m_ψ的定量关系（σ_SI∝Λ_D^2.15 m_ψ^-8），为直接探测实验提供了可测试的理论基础。


<details>
  <summary>Details</summary>

Background: 研究探讨了杨-米尔斯(Yang-Mills)暗区中胶球暗物质(glueball dark matter)通过带电磁荷和暗色荷的矢量费米子(vector-like fermion)与标准模型(Standard Model)耦合的物理机制。

Data: 数据来源于量子色动力学（QCD）现象学支持的非微扰信息构建的有效场论框架。

Method: 采用受QCD现象学支持的有效场论框架（effective field theory framework）并结合非微扰信息，定量预测胶子球（glueball）与原子核的相干弹性散射。

Result: 在标准模型通过带电磁荷和暗色荷的矢量费米子门户耦合的Yang-Mills暗区中，当$m_ψ\gtrsim 5.5 Λ_D$时，质量$m_χ$的最轻$C$-奇自旋-1态（oddball）可作为可行的暗物质候选者。
...
### [42] [Leptophilic scalar dark matter in U(1)$_{L_μ-L_τ}$: Evading direct detection and prospective neutron star heating](https://arxiv.org/abs/2602.19958)
*Chengfeng Cai,Hong-Hao Zhang*

**TL;DR**: 研究了标准模型$\mathrm{U(1)}_{L_μ-L_τ}$规范扩展中的轻子亲和暗物质（leptophilic dark matter），通过三种基准模型（孤立暗物质、SO(4)和SO(3)对称性下的赝南布-戈德斯通玻色子模型）在满足遗迹密度和规避直接探测约束的同时，提出中子星近红外观测可探测亚TeV至TeV能区的剩余参数空间。


<details>
  <summary>Details</summary>

Background: 研究聚焦于轻子亲和暗物质（leptophilic DM）的热WIMP框架，通过标准模型的$\mathrm{U(1)}_{L_μ-L_τ}$规范扩展探索其与中子星加热效应的关联，旨在解释观测到的遗迹丰度并规避直接探测信号。

Data: 研究中使用的数据来源于标准模型（Standard Model）的$\mathrm{U(1)}_{L_μ-L_τ}$规范扩展框架下的三种基准实现方案（secluded DM scenario、SO(4) pNGB model和SO(3) pNGB model）的系统参数扫描结果，并结合了遗迹密度（relic density）、直接和间接探测（direct- and indirect-detection）以及中微子三叉戟（neutrino trident）约束条件。

Method: 采用标准模型的$\mathrm{U(1)}_{L_μ-L_τ}$规范扩展框架，研究了三种基准实现：孤立暗物质（secluded DM）场景通过湮灭到规范玻色子设定遗迹密度，以及基于SO(4)和SO(3)对称性的两种赝南布-戈德斯通玻色子（pNGB）暗物质模型。

Result: 在标准模型的$\mathrm{U(1)}_{L_μ-L_τ}$规范扩展中，轻子偏爱暗物质（leptophilic DM）通过中子星（NS）加热效应可被探测，研究确定了三种基准模型（孤立暗物质、SO(4)和SO(3)对称性下的赝南布-戈德斯通玻色子模型），并通过系统参数扫描发现亚TeV至TeV能区的可行候选者，其剩余参数空间可通过中子星近红外观测进行检验。
...
### [43] [Improving calibration accuracy with torque coupled gravity field calibrator for sub-Hz gravitational wave observation in CHRONOS](https://arxiv.org/abs/2602.19436)
*Yuki Inoue,Daiki Tanabe,Vivek Kumar*

**TL;DR**: 通过优化扭矩耦合重力场校准器（GCal）的几何构型，将低温亚赫兹扭杆探测器（CHRONOS）在1赫兹处的校准信号信噪比提升超过一个量级，首次实现了高信噪比校准线直接注入亚赫兹频段，系统不确定度低至0.24%。


<details>
  <summary>Details</summary>

Background: 低频引力波探测器中校准线信噪比（SNR）不足是长期存在的核心挑战，尤其在受旋转动力学主导的扭杆系统中更为突出。

Data: 数据来源于低温亚赫兹交叉扭杆探测器（CHRONOS）在0.1-10赫兹频段内采集的单色校准信号，其中1赫兹处的应变等效校准幅度达到1.18×10^-14，对应信噪比密度为4.25×10^3。

Method: 优化扭矩耦合重力场校准器（GCal）的几何构型以提高校准线信噪比（SNR）。

Result: 通过优化扭矩耦合重力场校准器（GCal）的几何构型，将校准线信噪比（SNR）较传统布局提升了一个数量级以上。
...
### [44] [Chaotic imprints of dark matter in extreme mass-ratio inspirals](https://arxiv.org/abs/2602.19541)
*Mustapha Azreg-Aïnou,Mubasher Jamil,Emmanuel N. Saridakis*

**TL;DR**: 研究发现极端质量比旋近系统（EMRIs）在非真空时空中普遍存在由环境扰动引发的混沌运动，其产生、持续和终止过程敏感依赖于黑洞视界结构、核心正则化及物质分布，并通过引力波辐射的振幅不规则调制和相位相干性丧失等特征区别于规则运动。


<details>
  <summary>Details</summary>

Background: 极端质量比旋近(EMRIs)是研究强场引力(strong-field gravity)和超大质量致密天体周围环境的强有力探针。

Data: 数据来源包括数值模拟（numerical Kludge approach）和理论分析的非真空时空（non-vacuum spacetimes）中测试粒子运动数据，涵盖奇异黑洞（singular black holes）、规则黑洞（regular black holes）、裸奇点（naked singularities）和爱因斯坦星团构型（Einstein-cluster configurations）。

Method: 采用数值Kludge方法分析暗物质嵌入几何中的测试粒子运动，研究强场区域因环境扰动导致的混沌动力学及其引力波印记。

Result: 极端质量比旋近系统(EMRIs)在非真空时空中演化时，环境扰动普遍会破坏强场区域的可积性，产生对视界结构、核心正则化和物质分布敏感的混沌运动。
...
### [45] [Improved constraints on modified Newtonian gravity from Cassini radio tracking data](https://arxiv.org/abs/2602.17884)
*R. S. Park,A. Hees,B. Famaey,H. Desmond,A. Durakovic*

**TL;DR**: 利用DE440行星星历表数据改进对太阳系四极矩参数(Q2)的约束至(1.6±1.8)×10^-27 s^-2（精度提升40%），验证了理论计算中仅保留太阳贡献的近似合理性，并揭示该结果与星系旋转曲线及银河系内观测存在3-15σ显著性差异，表明太阳系测量对修正牛顿动力学(MOND)类引力理论提供了比宽双星数据更强的约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于修正牛顿动力学（MOND）理论框架下太阳系四极矩参数（quadrupole parameter $Q_2$）的约束及其对星系旋转曲线与银河系内径向加速度的观测限制。

Data: 使用DE440行星星历表数据集，同时估计了行星星历表中包含的其他参数。

Method: 采用DE440行星星历表数据集，同时估计四极矩参数（quadrupole parameter）$Q_2$与行星星历表中其他参数，获得$Q_2$的改进约束。

Result: 基于DE440行星星历表数据集并与其他参数同步估计，测得太阳系四极矩参数（quadrupole parameter）$Q_2 = (1.6 \pm 1.8) \times 10^{-27}\,\mathrm{s}^{-2}$（1-$σ$），精度较先前提升40%。
...
### [46] [Addressing leakage and mode suppression in angular power spectrum estimation for gravitational-wave backgrounds using pulsar timing arrays](https://arxiv.org/abs/2602.20075)
*Deepali Agarwal,Joseph D. Romano,Yacine Ali-Haïmoud,Tristan L. Smith*

**TL;DR**: 研究发现脉冲星计时阵（PTA）分析中球谐展开截断应选择最大信息角尺度对应的多极矩l_max^res而非传统l_max^N_pair，以避免小尺度功率泄漏并优化引力波背景（GWB）各向异性功率谱估计。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用脉冲星计时阵（PTA）探测引力波背景（GWB）各向异性时面临的球谐展开截断问题及其对功率谱估计的影响。

Data: 数据来源为脉冲星计时阵（PTA）的观测数据，通过分析不同脉冲星对的独立约束条件来确定最大信息角尺度对应的多极阶数l_max^res。

Method: 采用球谐函数展开（spherical harmonic expansion）并基于费雪信息矩阵（Fisher information matrix）的特征映射（eigenmaps）确定最大可解析多极矩（l_max^res），以分析脉冲星计时阵（PTA）对引力波背景（GWB）各向异性的响应。

Result: 脉冲星计时阵（PTA）分析中球谐展开应截断于最大信息角尺度对应的多极阶l_max^res，以避免小尺度角功率泄漏并确保覆盖可观测天空空间。
...
### [47] [Testing the Spacetime Geometry of Sgr A* with the Relativistic Orbit of S2 star](https://arxiv.org/abs/2602.18994)
*Parth Bambhaniya,Preet Dalal,Giovani H. Vicentin,Riccardo Della Monica,Elisabete M. de Gouveia Dal Pino,Bina Patel*

**TL;DR**: 通过分析S2恒星轨道并整合甚大望远镜(VLT)观测数据与事件视界望远镜(EHT)阴影尺寸约束，发现当前数据无法区分史瓦西(Schwarzschild)、赖斯纳-诺德斯特伦(Reissner-Nordström)和巴丁(Bardeen)正则黑洞等时空几何模型，同时约束了非史瓦西时空中的广义电荷参数q/M，并确定了可通过未来VLT和凯克(Keck)望远镜高精度观测进一步检验的特定黑洞与无视界几何模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过人马座A*（Sgr A*）周围S2恒星轨道运动，对广义相对论框架下不同时空几何模型（包括黑洞和裸奇点解）进行相对论性检验。

Data: 数据来源于甚大望远镜（VLT）对S2恒星的现有观测以及事件视界望远镜（EHT）对黑洞阴影尺寸的约束。

Method: 采用相对论性测地线（timelike geodesic）方程积分并结合Rømer时延（Rømer time delay）和相对论红移（relativistic redshift）效应的投影方法，测试多种黑洞（Schwarzschild、Reissner-Nordström、Bardeen、Hayward、Simpson-Visser）和裸奇点（Janis-Newman-Winicour naked singularity）时空模型的轨道动力学。

Result: 基于S2恒星轨道对Sgr A*时空几何的相对论性测试表明，施瓦西黑洞(Schwarzschild)、赖斯纳-诺德斯特伦黑洞(Reissner-Nordström)和巴丁规则黑洞(Bardeen regular)等时空模型在当前观测数据下仍无法被统计学区分。
...
### [48] [Dark Matter in Zwicky's Cosmology: Towards an Epistemological Reconstruction](https://arxiv.org/abs/2602.18475)
*Simon Beyne,Christian Marinoni*

**TL;DR**: 通过对弗里茨·兹威基(Fritz Zwicky)1933年关于后发座星系团(Coma cluster)速度弥散维里分析(virial analysis)论文的重新解读，揭示了暗物质(dark matter)并非源于"朴素"的天文观测，而是兹威基基于宇宙学动机预先寻找的理论预期，其质量估算与预期量级一致，并可能成为支持广义相对论(General Relativity)的早期宇宙学证据。


<details>
  <summary>Details</summary>

Background: 重新审视弗里茨·兹威基(Fritz Zwicky)1933年关于后发座(Coma)星系团速度弥散度维里(virial)分析的研究，揭示了暗物质(dark matter)概念的提出具有深刻的宇宙学动机而非单纯观测驱动。

Data: 数据来源于对弗里茨·兹威基（Fritz Zwicky）1933年关于后发座星系团（Coma cluster）中星系速度弥散进行维里分析（virial analysis）的原始论文《The redshift of extragalactic nebulae》的重新语境化解读。

Method: 采用文献重读（contextualised reading）和认识论分析（epistemic analysis）方法重新审视了兹威基（Zwicky）1933年对后发座星系团（Coma cluster）的维里分析（virial analysis）及其暗物质（dark matter）论述的宇宙学动机。

Result: 暗物质（dark matter）并非源于简单的天体物理观测，而是茨维基（Zwicky）基于宇宙学动机预先寻找的结果，且其丰度差异与预期一致，可能代表支持广义相对论（General Relativity）的最早宇宙学迹象之一。
...
### [49] [GPU-Native Compressed Neighbor Lists with a Space-Filling-Curve Data Layout](https://arxiv.org/abs/2602.19873)
*Felix Thaler,Sebastian Keller*

**TL;DR**: 提出了一种基于空间填充曲线（SFC）内存布局和粒子簇的压缩近邻列表（compressed neighbor list）方法，在GPU上高效构建并支持变交互半径，适用于高密度对比天体物理系统，且能与八叉树（octree）长程作用计算方法无缝集成。


<details>
  <summary>Details</summary>

Background: 研究聚焦于基于空间填充曲线（SFC）和粒子簇的短程粒子相互作用压缩邻域列表开发，旨在提升高密度对比系统（如天体物理和宇宙学应用）的计算效率。

Data: 研究中使用的数据来源于基于空间填充曲线（SFC）内存布局和粒子簇的短程粒子间相互作用的压缩邻居列表构建，支持NVIDIA和AMD硬件的高效GPU计算。

Method: 基于空间填充曲线（SFC）内存布局和粒子簇的压缩邻居列表方法。

Result: 基于空间填充曲线（SFC）和粒子簇的压缩邻居列表在GPU上高效构建，支持NVIDIA和AMD硬件，每个粒子仅需4字节内存即可存储约200个邻居。
...
### [50] [Development of a Cherenkov-Based Time-of-Flight Detector Using Silicon Photomultipliers](https://arxiv.org/abs/2602.20139)
...
**TL;DR**: 开发了基于高折射率固体切伦科夫辐射体(Cherenkov radiator)和硅光电倍增管(SiPM)的高精度飞行时间(TOF)探测器，通过优化辐射体材料与几何结构及光学耦合效率，实现了优于33.2 ps的系统时间分辨率和100%带电粒子探测效率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发基于高折射率固体切伦科夫辐射体(Cherenkov radiator)和硅光电倍增管(SiPMs)的高精度飞行时间(TOF)探测器，以突破粒子鉴别系统的时间分辨率极限。

Data: 数据来源于CERN-PS T10束流线实验采集的束流测试数据，以及基于Petiroc 2A或Radioroc 2前端电子学与picoTDC读出的电荷和时间测量数据。

Method: 采用高折射率固体切伦科夫辐射体(Cherenkov radiator)与硅光电倍增管(SiPMs)耦合的优化设计，通过蒙特卡洛模拟(Monte Carlo simulation)系统分析限制时间分辨率的关键因素，并结合实验束流测试数据验证。

Result: 基于高折射率固体切伦科夫辐射体（Cherenkov radiator）和硅光电倍增管（SiPMs）开发的高精度飞行时间（TOF）探测器实现了优于33.2 ps的系统级时间分辨率和100%的带电粒子探测效率。
...
### [51] [Variable Red Giants Exploration with MASTER Robotic Net: Main Algorithms and Study of Three Mira-type Targets](https://arxiv.org/abs/2602.18754)
...
**TL;DR**: 利用MASTER望远镜网络发现并分析了三颗米拉变星（Mira-type variables）的光变曲线和周期，估算了其光度、半径和零龄主序质量（ZAMS masses），其中一颗碳星（carbon star）的光度增长但脉动周期恒定，暗示可能存在长次级周期（long secondary period）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用MASTER机器人望远镜网络（MASTER Robotic Net）观测变光红巨星（variable red giant stars）的光变特性及其天体物理参数。

Data: 数据来源于MASTER Robotic Net全天地巡天观测和Lomonosov数据库存储的MASTER历史光变曲线。

Method: 基于MASTER机器人望远镜网络的全天巡天观测和Lomonosov数据库存储的MASTER光变曲线数据，采用宽视场图像分析和光变曲线周期性计算方法。

Result: 发现三个米拉变星（Mira-type variables）的光变曲线并计算其周期，其中碳星MASTER OT J07010810-6818548的平均星等增长但脉动周期恒定，可能暗示存在长次级周期。
...
### [52] [HelioSpectrotron 5000: An Interactive Multi-Resolution Solar Spectral Atlas](https://arxiv.org/abs/2602.20101)
*A. G. M. Pietrow*

**TL;DR**: 开发了HelioSpectrotron~5000 (HS5000)多分辨率太阳光谱图集，支持高分辨率参考光谱与多种地基仪器观测数据的直接比对，提供任意波段和分辨率的绝对强度与归一化连续谱，并包含谱线标识和可选的地球大气污染信息。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发一种交互式多分辨率太阳光谱图集（HelioSpectrotron~5000），旨在支持高分辨率参考光谱与多种地基仪器观测数据的直接比对。

Data: 数据来源为汉堡傅里叶变换光谱仪（Hamburg FTS atlas）提供的高分辨率参考光谱。

Method: 基于汉堡傅里叶变换光谱仪（Hamburg FTS）图谱构建的多分辨率交互式太阳光谱图集（HS5000），提供任意光谱范围和分辨率的绝对强度与连续谱归一化光谱数据。

Result: HS5000（HelioSpectrotron~5000）是一个支持多分辨率交互操作的太阳光谱图集，旨在实现高分辨率参考光谱与多种地基仪器观测数据的直接对比。
...
### [53] [21,864 Unresolved, Low-mass Binaries Identified via their Overluminosity in \textit{Gaia} DR3 and a Catalog of 347,440 Systems within 100 pc of the Sun](https://arxiv.org/abs/2602.20096)
*Zachary Way,Sébastien Lépine,Jonathan Gagné,Ilija Medan*

**TL;DR**: 基于盖亚(Gaia) DR3 XP光谱数据开发了区分单星与等质量双星的方法，发现100秒差距内13%的K型和M型矮星因光度异常被判定为未解析双星，并据此构建了系统目录以提供0.1-0.7太阳质量恒星的多重性下限。


<details>
  <summary>Details</summary>

Background: 研究主题为利用盖亚(Gaia) DR3 XP光谱区分低质量恒星中的单星与等质量双星系统，并构建100秒差距内的系统编目(Catalog of Systems)以限制0.1至0.7太阳质量恒星的多重性比例下限。

Data: 使用\textit{Gaia} DR3 XP光谱数据，并筛选了太阳系100秒差距（parsec）内具有纯净天体测量和测光数据的恒星，排除了可能为不等质量双星的恒星，构建了主要由单星或等质量双星组成的训练集。

Method: 采用随机森林回归（Random Forest Regression, RFR）模型迭代训练预测恒星绝对星等和颜色，并通过移除显著亮于预测值的恒星逐步收敛至仅含单星的模型。

Result: 基于盖亚(Gaia) DR3 XP光谱数据，通过随机森林回归(RFR)模型识别出13%的低质量K和M型矮星为未分辨的双星系统，并据此构建了100秒差距内的系统目录以提供0.1-0.7太阳质量恒星的多重性下限。
...
### [54] [Experimental and numerical modeling of liposome congregation in meteorite craters of Early Earth](https://arxiv.org/abs/2602.18510)
*Vladimir M. Subbotin,Benjamin A. Turner,Brian A. Davies,Alric G. Lopez,Gennady Fiksel*

**TL;DR**: 早期地球陨石坑（meteor crater）底部可形成脂质体（liposome）聚集，促进其通过融合、生长和分裂实现种群维持与膜稳定性进化。


<details>
  <summary>Details</summary>

Background: 该研究探讨了早期地球陨石坑（meteor crater）底部脂质体（liposome）聚集现象对生命起源的潜在影响。

Data: 数据来源于实验观测和数值模拟（simulation）。

Method: 采用实验模拟和数值计算分析早期地球陨石坑内脂质体（liposome）聚集现象，重点考察碗状陨石坑地形及周期性地震扰动对脂质体高密度聚集的促进作用。

Result: 早期地球陨石坑底部可形成脂质体(liposome)聚集，尤其在周期性地震扰动下。
...
### [55] [Tempawral: A Time-Resolved Retrieval Framework for Variable Brown Dwarfs and Exoplanets](https://arxiv.org/abs/2602.18984)
*Fei Wang,Ben Burningham,Stuart Littlefair,Etienne Artigau,Yuka Fujii,Jacqueline K. Faherty,Johanna M. Vos*

**TL;DR**: 开发了首个数据驱动的时序大气反演框架Tempawral，通过特征光谱反演技术定量解析褐矮星（brown dwarf）大气参数变化，成功识别出云层覆盖不均、化学丰度演变和温度结构扰动等关键变率驱动因素，并在JWST观测的T2.5型褐矮星中检测到近1-bar压力层约300 K的温度扰动及H2O、CO、FeH丰度变化与铁云层厚度变化。


<details>
  <summary>Details</summary>

Background: 研究聚焦于褐矮星（brown dwarfs）和系外行星（exoplanets）大气动态过程（如云层、风暴、化学非均匀性）的光谱时变特性及其定量化反演方法。

Data: 数据来源包括模拟的褐矮星（brown dwarf）时变光谱数据和JWST/NIRISS-SOSS对一颗高度变化的T2.5型褐矮星的时变观测数据。

Method: 采用基于特征光谱反演技术（eigen-spectra inversion technique）的数据驱动时间分辨大气反演框架Tempawral，定量提取大气参数变化。

Result: Tempawral首次实现了通过特征光谱反演技术定量估计褐矮星（brown dwarf）大气参数随时间的变化，成功恢复了非均匀云层覆盖、化学丰度演变及温度结构变化等关键变率驱动因素。
...
### [56] [Exo Skryer: A JAX-accelerated sub-stellar atmospheric retrieval framework](https://arxiv.org/abs/2602.19687)
*Elspeth K. H. Lee*

**TL;DR**: 开发了基于JAX库的Exo Skryer亚恒星（sub-stellar）大气反演框架，显著提高了JWST时代传输和发射光谱数据的计算效率，并创新性地实现了气溶胶红外吸收特征光学常数（n, k）的直接反演。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用高效计算框架（如Exo Skryer）应对系外行星和褐矮星大气研究中因JWST（詹姆斯·韦伯太空望远镜）及地基高分辨率光谱数据激增带来的计算挑战。

Data: 研究中使用的数据来源包括JWST（James Webb Space Telescope）时代前后的观测数据（transmission和emission光谱）以及地面高分辨率光谱数据。

Method: 采用JAX库构建的Exo Skryer框架，通过可扩展的高效前向建模和后验采样方法，实现亚恒星大气参数反演，并开发了直接反演气溶胶红外吸收特征复折射率（n, k）的新技术。

Result: 开发了基于JAX库的Exo Skryer框架，显著提升了亚恒星大气反演（retrieval）建模的计算效率，并支持直接反演气溶胶红外吸收特征的光学常数（n, k）。
...
### [57] [Periodic Wobble of the Post-Perihelion Jet Structure Around 3I/ATLAS](https://arxiv.org/abs/2602.18512)
*Toni Scarmato,Abraham Loeb*

**TL;DR**: 通过分析星际天体3I/ATLAS的喷流位置角（PAs）和彗发测光数据，发现其近日点后存在约7.1小时的非主轴旋转（姿态进动/章动）周期，喷流结构以约20度角绕旋转轴进动且旋转轴与太阳方向夹角在20度以内。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际天体3I/ATLAS的近日点后形态学特征，特别是通过喷流方位角（PAs）和彗发主导测光数据揭示其自转轴进动/章动（非主轴旋转）动力学行为。

Data: 数据来源包括哈勃太空望远镜（HST）经Larson Sekanina旋转梯度滤波处理的图像，以及MPC站L92提供的Gr（R波段）时序测光数据集。

Method: 采用拉尔森-塞卡尼纳（Larson Sekanina）旋转梯度滤波处理哈勃太空望远镜（HST）图像，测量喷流位置角（PAs）并拟合加权傅里叶模型进行周期扫描。

Result: 通过喷流位置角（PAs）和彗发主导测光分析，确定星际天体3I/ATLAS在近日点后的特征周期约为7.1小时，该周期被解释为由喷流方向和彗发光通量重分布追踪的姿态进动/章动（非主轴旋转）。
...
### [58] [Inferring the population properties of galactic binaries from LISA's stochastic foreground](https://arxiv.org/abs/2602.18560)
*Federico De Santi,Alessandro Santini,Alexandre Toubiana,Nikolaos Karnesis,Davide Gerosa*

**TL;DR**: 提出基于模拟推理的框架，通过训练神经网络后验估计器从LISA（激光干涉空间天线）重建的引力波前景谱中直接测量银河系双星（Galactic binaries）的群体特性，包括双星总数等参数，并开发了GPU加速的减法算法实现百倍提速。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用LISA（Laser Interferometer Space Antenna）探测到的银河系双星（Galactic binaries）引力波随机前景（stochastic gravitational-wave foreground）光谱特征，通过模拟推断方法揭示其群体属性。

Data: 数据来源为通过全局拟合启发式减法算法生成的合成星表（synthetic catalogs）和前景光谱（foreground spectra），并采用GPU加速版本提升计算效率。

Method: 采用基于模拟的推断框架（simulation-based inference）结合神经网络后验估计器（neural posterior estimator），在可观测参数空间（信号振幅、频率、频率导数）生成合成目录和前景谱，并通过GPU加速的全局拟合减法算法（global-fit-inspired subtraction algorithm）进行验证。

Result: 基于重建的前景谱，利用模拟推断框架可直接测量银河双星（Galactic binaries）的群体特性，包括双星总数等参数，且验证显示参数恢复精度良好。
...
### [59] [Parameter Estimation Limits in Blazars](https://arxiv.org/abs/2602.19301)
*Agniva Roychowdhury*

**TL;DR**: 采用费希尔信息(Fisher Information)方法量化了单区(one-zone)模型中耀变体(SED)参数提取的理论极限，发现逆康普顿(EC)模型的信息量比同步自康普顿(SSC)模型低4个数量级，且多普勒因子(δ)是受约束最强的参数，而EC主导的耀变体(如FSRQ)参数推断限制比BL Lac天体更严格。


<details>
  <summary>Details</summary>

Background: 该研究探讨了耀变体（blazar）光谱能量分布（SED）中参数简并性（parameter degeneracy）的理论信息提取极限问题。

Data: 数据来源包括对耀变体CTA 102和3C 279的实际耀发期光谱能量分布（SED）观测数据，以及基于单区模型（one-zone models）通过变化参数（$δ$、$B$、$p$、$γ_{\rm min}$和$γ_{\rm max}$）模拟得到的同步自康普顿（SSC）和外康普顿（EC）模型数据。

Method: 采用费希尔信息(Fisher Information)方法量化一区模型(one-zone models)中参数简并性并确定信息提取的理论极限。

Result: 在同步自康普顿（SSC）模型中提取的物理信息极限比外部康普顿（EC）模型高至少10^4倍。
...
### [60] [A Bayesian Method for Air-Shower Reconstruction using Information Field Theory](https://arxiv.org/abs/2602.19864)
...
**TL;DR**: 基于贝叶斯推断和信息场理论（IFT）构建了可微分前向模型，结合高斯过程处理系统误差，实现了对高能宇宙线广延大气簇射（EAS）的高效（比传统方法快三个数量级）重建，在LOFAR模拟中获得了25 g/cm²的Xmax（最大深度）分辨率和12%的辐射能量分辨率。


<details>
  <summary>Details</summary>

Background: 研究高能宇宙射线(high-energy cosmic rays)起源的无线电探测方法及其重建技术。

Data: 数据来源于低频阵列（LOFAR）对广延大气簇射（extensive air showers）无线电发射足迹的高精度测量以及基于物理发射参数化和精确波前模型的模拟数据。

Method: 基于贝叶斯推断和信息场理论（Information Field Theory, IFT）构建了包含物理发射参数化和精确波前模型的全可微前向模型，并采用高斯过程处理信号注量和到达时间的系统不确定性。

Result: 基于贝叶斯推断和信息场理论(IFT)的完全可微分正向模型结合高斯过程，实现了高能宇宙线空气簇射参数(如原初能量和最大深度X_max)的高效(比传统方法快三个数量级)稳健推断，在LOFAR模拟中达到X_max分辨率25 g/cm²和辐射能量分辨率12%。
...
### [61] [Winds of Change: XRISM Resolve X-ray spectroscopy of NGC 4051](https://arxiv.org/abs/2602.19897)
...
**TL;DR**: 首次利用XRISM Resolve高分辨率光谱在NGC 4051（窄线塞弗特1型星系）中观测到两个蓝移铁K壳吸收特征（速度0.025c和0.04c），并发现吸收速度在一天内变化，支持磁驱动加速盘风模型，其动能功率低于爱丁顿极限。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于近邻窄线赛弗特1型星系(Narrow Line Seyfert 1 galaxy)NGC 4051的X射线高分辨率光谱分析，特别是其快速X射线变异性与复杂多组分外流风(wind)的动力学特征。

Data: 数据来源于XRISM Resolve对NGC 4051在历史高亮度状态下进行的150 ks曝光观测所获取的高分辨率X射线光谱。

Method: 采用高分辨率X射线光谱分析技术（XRISM Resolve）对NGC 4051进行时间依赖性光谱分析，识别出两个蓝移Fe K壳层吸收槽（H-like iron）并测量其流速变化。

Result: XRISM首次在NGC 4051的高分辨率X射线光谱中解析出两个蓝移的Fe K壳层吸收谷，分别对应0.025c和0.04c速度的两股外流成分，且吸收特征在一天内呈现速度递增的变化模式。
...
### [62] [IceCube's convex all-sky neutrino spectrum consistent with the magnetically powered corona scenario for active galactic nuclei](https://arxiv.org/abs/2602.20145)
*Kohta Murase,Shigeo S. Kimura,Mainak Mukhopadhyay,Mukul Bhattacharya*

**TL;DR**: 研究发现活动星系核（AGN）的冕区（coronal regions）是隐藏中微子源的主要场所，其磁驱动冕区模型（magnetically powered corona scenario）能解释全天空中微子谱在3-30 TeV的拐折特征，并符合NGC 1068的多信使观测数据，且类似源对中微子通量的贡献未超出观测上限。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高能多信使（multimessenger）背景下隐藏中微子源的探测及其与活动星系核（AGNs）磁化冕区（coronal regions）中非热粒子产生的关联。

Data: 数据来源包括IceCube观测的全天中微子能谱（all-sky neutrino spectrum）和NGC 1068的多信使（multimessenger）观测数据。

Method: 采用磁化日冕（magnetically powered corona）模型分析全天空中微子能谱，并结合多信使数据验证NGC 1068的观测结果。

Result: 最新全天中微子能谱在3-30 TeV处的谱弯曲与磁驱动日冕（magnetically powered corona）模型的预测一致，且该模型能同时在观测与建模不确定度范围内解释NGC 1068的多信使数据。
