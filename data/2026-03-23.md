...
**TL;DR**: 开发了模拟自相互作用暗物质(SIDM)子晕演化的高效计算框架，发现散射诱导的子晕-晕相互作用显著影响子晕结构演化，导致其中心密度和密度轮廓斜率比无碰撞暗物质(CDM)模型呈现更大多样性，并在强透镜系统和卫星星系中产生可观测特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于自相互作用暗物质（SIDM）模型如何通过短程粒子相互作用影响星系旋转曲线内部分布及引力透镜特征，并探索卫星星系和暗物质子晕（subhalo）作为其关键测试平台。

Data: 采用N体模拟（N-body simulations）和虚拟宿主粒子（virtual host particles）方法模拟子晕演化，并利用爱丁顿反演法（Eddington inversion method）精确建模晕内局部速度分布。

Method: 采用N体模拟结合虚拟宿主粒子处理散射诱导的子晕-晕相互作用过程，并运用埃丁顿反演法（Eddington inversion method）精确建模晕内局部速度分布。

Result: 开发了一种模拟框架，通过虚拟宿主粒子（virtual host particles）和埃丁顿反演法（Eddington inversion method）高效建模自相互作用暗物质（SIDM）子晕在真实环境中的演化，重点捕捉散射诱导的子晕-宿主晕相互作用（scattering-induced subhalo-halo interaction）对密度轮廓的显著影响。
...
**TL;DR**: 该研究通过分析M巨星（M giants）和Mira变星（Mira variable stars）的分布，确定了银河系长棒（long bar）与太阳-银心连线的平均夹角为27.4°±1.5°，半长轴约4千秒差距（kpc），并发现长棒末端与3千秒差距臂（3 kpc arm）相连，其近侧和远侧分别位于约5千秒差距和9-12千秒差距处。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系高度消光（extinction）的盘面中心区域（银经$10^\circ \lesssim |\ell |\lesssim 30^\circ$、银纬$|b|\lesssim 3^\circ$）的长棒（long-bar）和3千秒差距臂（3 kpc arm）的形态学特征，该区域因恒星年龄分布和结构不清晰而成为探索难点。

Data: 数据来源包括WISE-4.6μm波段明亮M巨星（M giants）的星计数、APOGEE-DR17光谱视差（spectroscopic parallaxes）导出的距离分布、年轻OGLE富氧米拉变星（OGLE-O-rich Mira variable stars）的距离分布以及该区域内红团簇（red clump）距离测定的文献综述。

Method: 采用WISE-4.6μm波段明亮M巨星（M giants）的星数统计和APOGEE-DR17光谱视差（spectroscopic parallax）距离分布分析，结合OGLE富氧米拉变星（O-rich Mira variable stars）距离分布及红团簇（red clump）测距文献综述。

Result: 银河系长棒（long bar）的平均倾角为27.4°±1.5°，与三轴核球（triaxial bulge）对齐，半长轴长度约4千秒差距，其末端与椭圆形的3千秒差距臂（3 kpc arm）接触。
...
**TL;DR**: 通过分析TNG50模拟数据，揭示了暗物质晕中轨道子晕（subhalos）产生的动力学摩擦加热（dynamical friction heating）是低红移大质量晕中星系淬灭（galaxy quenching）的重要机制，并发现该机制在高红移（$z \sim 15$）能显著抑制金属贫乏迷你晕（metal-free minihalos）的气体冷却和碎片化，使第三族恒星（Pop III）形成的临界分子氢丰度提高一个量级，同时与莱曼-维尔纳辐射（Lyman-Werner background）共同促进原子冷却晕（atomic cooling halos）中直接坍缩黑洞（DCBH）种子的形成。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗物质晕（dark matter halos）中轨道子晕（subhalos）通过动力学摩擦（dynamical friction）加热气体的机制及其对星系演化（galaxy quenching）和早期宇宙恒星形成（Pop III star formation）的影响。

Data: 数据来源于TNG50数值模拟（TNG50 simulation），涵盖从红移$z = 15$到0的广泛晕质量和子晕质量函数。

Method: 采用TNG50模拟数据，结合暗晕（dark matter halos）中轨道子晕（subhalos）的动力学摩擦（dynamical friction）模型，计算了从红移$z=15$到0范围内不同晕质量和红移下的子晕质量函数和动力学摩擦加热率。

Result: 在低红移大质量暗晕（dark matter halos）中，动力学摩擦（dynamical friction）通过将引力势能转化为热能成为星系淬灭（galaxy quenching）的重要机制。
...
**TL;DR**: 该论文综述了活动星系核(AGN)考古学方法，通过分析电离锥(ionisation cones)和射电喷流(radio jets)等遗迹现象，揭示了超大质量黑洞(supermassive black holes)活动在辐射模式与动能模式间切换的历史及其对星系际介质(intergalactic medium)和环星系环境(circumgalactic environment)的可观测影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过遗迹观测特征（如电离锥和射电喷流）重建超大质量黑洞（supermassive black hole）在星系核中的瞬态活动历史及其对星系际介质（intergalactic medium）和环星系环境（circumgalactic environment）的影响。

Data: 数据来源包括遗迹射电结构(relic radio structures)、受过去活动"电离回波"(ionising echo)照射的气体云(gas clouds)、费米/eROSITA气泡(Fermi/eROSITA bubbles)以及电离锥(ionisation cones)的光学观测数据。

Method: 基于光学观测的电离锥（ionisation cones）和辐射模式（电离锥）与动力学模式（射电喷流）切换证据的分析方法。

Result: 活动星系核（AGN）的瞬态特性可通过电离锥（ionisation cones）和射电喷流（radio jets）的切换模式重建其历史活动轨迹。
...
**TL;DR**: 利用L-GALAXIES半解析模型研究发现，星系团中心最亮星系（BCGs）在z~1.3才成为结构中质量最大的星系，但其整个演化历史始终处于更高星系和恒星质量密度的环境中，形成了区别于其他大质量星系的独特演化路径。


<details>
  <summary>Details</summary>

Background: 研究聚焦于最亮团星系（Brightest Cluster Galaxies, BCGs）在星系团中心特殊环境下的独特演化路径和物理特性。

Data: 数据来源于L-GALAXIES半解析模型（semi-analytic model）模拟的180个z∼0星系团中恒星质量超过10^9 M⊙的所有成员星系及其并合历史，并与场星系（field galaxies）对照样本进行比较。

Method: 采用L-GALAXIES半解析模型（semi-analytic model）研究星系团最亮成员星系（BCGs）的演化，通过追踪180个模拟星系团中恒星质量超过$10^9 \ {\rm M_\odot}$的成员星系的并合历史。

Result: 最亮团星系（BCGs）的恒星质量形成历史与其他大质量星系不同，且仅在红移约1.3时才成为其结构中质量最大的星系。
...
### [6] [COSMOS-3D: Black Hole Mass Estimators and Luminosity Functions of Paschen-line AGNs](https://arxiv.org/abs/2603.19548)
...
**TL;DR**: 利用JWST COSMOS-3D数据首次构建了红移1<z<3的62个宽帕邢线(Paschen lines)活动星系核(AGNs)样本，建立了基于帕邢线光度与连续谱光度的三种黑洞质量($M_{\mathrm{BH}}$)估算方法，并发现帕邢线选型1类AGNs的完备性是紫外/光学选类的3-5倍。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用近红外帕邢线（Near-IR Paschen lines）作为受尘埃消光影响较小的I型活动星系核（Type 1 AGNs）示踪手段，并探索红移z>1的帕邢线天体。

Data: 数据来自JWST（James Webb Space Telescope）的COSMOS-3D项目，包括NIRCam成像和光栅无缝光谱数据。

Method: 采用近红外相机（NIRCam）成像和光栅无缝光谱数据高效筛选和识别AGN，通过图像分解分离AGN-宿主星系辐射，利用多波段数据量化尘埃衰减，并基于光学光谱构建校准样本，以单历元Mg II为基准推导新的帕邢线（Paschen-line）黑洞质量估算器。

Result: result: 基于帕邢(Paschen)线光度和活动星系核(AGN)连续谱光度构建了三组黑洞质量($M_{\mathrm{BH}}$)估算器，经尘埃校正后相互一致且与先前结果吻合，并首次构建了1型AGN的帕邢α(Pa$α$)和帕邢β(Pa$β$)光度函数(LFs)，其值比紫外/光学选AGN高3-5倍，表明帕邢线选择更完备。
...
### [7] [The radial velocity stability for reference sources in the MWISP CO survey](https://arxiv.org/abs/2603.19748)
*Lixia Yuan,Ji Yang,Min Fang,Shaobo Zhang,Dengrong Lu,Jixian Sun*

**TL;DR**: 通过分析MWISP巡天中六种参考源的CO谱线速度稳定性，发现速度偏移（velocity shifts）与谱线宽度、望远镜方位高度角（Azimuth-Elevation）及源的速度梯度密切相关，主要归因于指向误差（pointing errors）对高速度梯度源的敏感性增强。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于评估银河画卷巡天（MWISP）中一氧化碳（CO）谱线的速度稳定性，探讨望远镜指向误差对分子云速度场测量的系统性影响。

Data: 数据来源于银河画卷巡天（MWISP）中观测的六个参考源的约10,000条CO谱线，覆盖了巡天十年的持续时间。

Method: 采用互相关方法（cross-correlation method）测量了MWISP巡天中六个参考源的约10,000条CO谱线速度偏移。

Result: 速度偏移（velocity shifts）主要源于指向误差（pointing errors），且对具有较高速度梯度（velocity gradients）的参考源更敏感。
...
### [8] [FATHOMER survey: III. Preliminary HI galaxy identification results](https://arxiv.org/abs/2603.19777)
*Shuanghao Shu,Yichao Li,Wenxiu Yang,Jiaxin Wang,Wenkai Hu,Furen Deng,Shifan Zuo,Yougang Wang,Xuelei Chen*

**TL;DR**: FAST望远镜通过FATHOMER巡天在1.05-1.45 GHz频段探测到702个红移z < 0.09、中性氢(HI)质量超过10^6.2倍太阳质量(M⊙)的星系，其中371个为新发现（含77个无光学对应体的潜在暗星系候选体），与ALFALFA巡天相比展现出对高红移和低HI流量星系更强的探测能力，其HI质量函数分析显示更高的特征质量和更陡的低质量斜率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用五百米口径球面射电望远镜（FAST）开展的中性氢（HI）强度映射实验（FATHOMER），旨在探索星系中性氢气体分布及其质量函数特性。

Data: 数据来源于2021年使用五百米口径球面射电望远镜（FAST）进行的28小时漂移扫描观测，覆盖1.05-1.45 GHz频段和60平方度天区。

Method: 采用匹配滤波算法(matched-filtering algorithm)和SoFiA源探测流水线(SoFiA source-finding pipeline)进行中性氢(HI)星系识别。

Result: FAST望远镜通过FATHOMER巡天项目在1.05-1.45 GHz频段探测到702个中性氢（HI）质量超过10^6.2太阳质量、信噪比大于5且红移z < 0.09的星系，其中371个为新发现目标（含77个无光学对应体的潜在暗弱星系候选体）。
...
### [9] [Unveiling large-scale rotational motions in the intragroup medium at z~1 through gravitational-arc tomography](https://arxiv.org/abs/2603.20006)
...
**TL;DR**: 首次在红移z=1.167的星系群中通过引力透镜效应观测到延伸至62千秒差距的富金属冷却星系际介质(IGrM)，其分布和二维运动学特征表明存在潮汐剥离与恒星形成驱动外流共同作用，且数据支持该介质以约130 km/s速度与星系共转形成一个动力学平衡的多相态群级晕，总气体质量估计为1.3-2.5×10^11太阳质量。


<details>
  <summary>Details</summary>

Background: 研究聚焦于红移z=1.167的星系群内冷星系际介质（IGrM）的首例空间分辨特征刻画，探讨其分布、动力学及金属富集气体的来源与演化机制。

Data: 数据来源于对引力透镜星系SGAS J0033+02的30条独立视线观测，结合了扩展引力弧（extended gravitational arc）和场中其他背景光源的光谱，用于绘制星系群内弥散金属富集气体的分布和运动学特征。

Method: 采用背景光源（background light）通过引力透镜效应（gravitational lensing）和场源（field sources）的多视线（multiple sightlines）观测，结合吸收线分析（absorption line analysis）和二维运动学建模（2D kinematic modeling）来研究星系群内冷介质（cool intragroup medium, IGrM）的分布与动力学特征。

Result: 首次在红移z=1.167的星系群中探测到延伸至62千秒差距的金属增丰冷却气体（MgII、FeII、CaII、MgI吸收），其分布和运动学特征表明受潮汐剥离和恒星形成驱动外流共同影响。
...
### [10] [VINTERGATAN-GM: long-lived satellite planes induced by a massive GSE-like merger](https://arxiv.org/abs/2603.20171)
...
**TL;DR**: 研究发现银河系质量量级（Milky Way-mass）星系中，大质量（质量比>1:6）的早期（z∼2）主并合（major merger）事件（类似盖亚-香肠-恩克拉多斯（GSE））通过产生扁平的各向异性宿主晕（host halo），促使卫星星系优先沿赤道面（equatorial plane）吸积并经各向异性动力学摩擦（anisotropic dynamical friction）形成与观测相符的运动学持久平面（KPPs）。


<details>
  <summary>Details</summary>

Background: 研究探讨了本星系群(Local Group)卫星星系薄平面分布与早期大质量并合事件(如银河系的盖亚-香肠-恩克拉多斯(Gaia-Sausage-Enceladus, GSE)事件)之间的潜在联系，旨在解决ΛCDM模型下"卫星星系平面问题"(planes of satellites problem)的理论预测与观测差异。

Data: 使用VINTERGATAN-GM高分辨率放大模拟套件，包含通过针对银河系盖亚-香肠-恩克拉多斯（GSE）前身进行定向遗传修饰生成的五个相同银河系质量晕实现，系统变化GSE类似合并质量比（1:10至1:2.1）但保持最终动力学质量和大尺度环境固定。

Method: 采用高分辨率变焦模拟(VINTERGATAN-GM套件)，通过系统改变GSE类似并合事件的质量比(1:10至1:2.1)，研究其对卫星星系平面性和运动相干性的影响。

Result: 更巨大的GSE类并合事件（质量比>1:6）通过宿主晕（host halo）的扁平化和速度各向异性，促使卫星群形成高度平面化且运动学相干的结构（KPPs），其形成机制包括卫星优先沿赤道面吸积及各向异性动力学摩擦对轨道的重塑。
...
**TL;DR**: 基于暗能量光谱仪(DESI)数据发布1(DR1)的星系聚类数据，首次结合功率谱多极子和双谱单极子(ShapeFit框架)进行全形状分析，获得了物质密度参数(Ω_m=0.310±0.012)和哈勃常数(H_0=68.92±0.97 km s^-1 Mpc^-1)的精确约束，并通过引入双谱信号将涨落振幅(ln(A_s×10^10))的误差范围缩小约20%，同时发现暗能量状态方程(w_0w_a)和中微子质量总和(∑m_ν)与标准ΛCDM模型一致。


<details>
  <summary>Details</summary>

Background: 利用暗能量光谱仪(DESI)数据发布1(DR1)的星系成团数据，基于功率谱多极子和双谱单极子的联合全形状分析(ShapeFit框架)，研究宇宙学参数约束和暗能量状态方程演化。

Data: 数据来源于暗能量光谱仪(DESI)数据发布1(DR1)的星系成团性数据，包括功率谱多极矩和双谱单极矩的联合全形状分析。

Method: 采用ShapeFit框架对DESI DR1星系聚类数据的功率谱多极矩和双谱单极矩进行联合全形状分析。

Result: 首次利用DESI DR1数据中的双谱（bispectrum）信号通过ShapeFit框架获得了物质密度参数Ω_m=0.310±0.012和哈勃常数H_0=68.92±0.97 km s^-1 Mpc^-1的约束值，与未使用双谱信号的DESI DR1完整数据集结果一致。
...
**TL;DR**: 通过Quijote N体模拟(N-body simulations)分析宇宙三维物质密度场的形态学特征，发现非线性引力演化在准线性尺度($R_G \sim 10 h^{-1} \mathrm{Mpc}$)显著扭曲单点分布并平滑场结构，红移空间畸变(redshift-space distortions)产生最强的各向异性形态信号，而大质量中微子(massive neutrinos)导致小尺度结构的近似各向同性抑制。


<details>
  <summary>Details</summary>

Background: 研究宇宙大尺度结构的形态学特性及其物理过程对三维物质密度场远足集形态（excursion-set morphology）的修饰作用。

Data: 数据来源于Quijote N-body模拟套件，用于研究三维物质密度场的形态学特性及其受非线性引力演化、红移空间畸变和大质量中微子自由流动的影响。

Method: 采用Minkowski泛函（Minkowski Functionals）、贝蒂数（Betti numbers）、Minkowski张量（Minkowski Tensors）及连通分量与空洞的局部尺寸形状测量等形态学描述符量化非线性引力演化、红移空间畸变（redshift-space distortions）和大质量中微子自由流动（massive neutrino free-streaming）对三维物质密度场形态的影响。

Result: 引力演化在准线性尺度（$R_G \sim 10 h^{-1} \mathrm{Mpc}$）上显著偏斜单点分布并通过临界点合并轻微平滑密度场，对极小值和壁鞍点的影响比峰值更明显。
...
**TL;DR**: 通过模拟研究了强度映射（IM）在构型空间测量重子声学振荡（BAO）时望远镜波束和前景去除的影响，发现针对BINGO和MeerKAT低红移实验楔形相关函数对横向和径向BAO约束最优，而对SKA-mid中红移实验径向和楔形相关函数分别对径向和横向BAO约束最佳。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用强度映射（IM）技术测量宇宙晚期重子声波振荡（BAO），重点探讨望远镜波束和前景去除效应对BAO测量的影响。

Data: 研究中使用的数据来源于针对21厘米强度映射（IM）实验（BINGO、MeerKAT和SKA-mid）生成的模拟星表（mock catalogs），用于分析望远镜波束和前景去除效应对重子声波振荡（BAO）测量的影响。

Method: 采用三种相关函数（径向相关函数、多极相关函数和楔形相关函数）测量重子声学振荡（BAO），并通过模拟检验管道性能以优化参数选择。

Result: 对于低红移实验BINGO和MeerKAT，楔形相关函数(wedge correlation function)在横向和径向重子声波振荡(BAO)测量中均能获得最严格的约束；对于SKA-mid实验，径向相关函数(radial correlation function)和楔形相关函数分别在径向和横向BAO测量中提供最优约束。
...
**TL;DR**: 通过比较人工神经网络(ANN)、混合密度网络(MDN)和贝叶斯神经网络(BNN)在类星体(quasar)测光红移(photo-z)不确定性估计中的表现，发现三组分MDN重建红移分布效果最优，BNN对超出训练范围的光度数据推断更优但会降低亮源精度，且准确的不确定性建模对消除(1.2,2.3)和(1.6,2.5)红移对的简并解至关重要。


<details>
  <summary>Details</summary>

Background: 研究聚焦于评估不同模型在重建类星体光度红移（photo-z）真实红移分布中的表现，并探索在数据质量变化情况下最有效的不确定性估计方法。

Data: 数据来源包括Kilo-Degree Survey Data Release 5的光度测量数据和Dark Energy Spectroscopic Instrument Data Release 1的光谱确认类星体数据。

Method: 采用人工神经网络（ANNs）、混合密度网络（MDNs）和贝叶斯神经网络（BNNs）结合高斯混合模型（GMM）输出的方法，通过构建四种测试集评估不同数据质量下的鲁棒性。

Result: result：三组分混合密度网络（MDN）在类星体光变红移（photo-$z$）分布重建中表现最优，而贝叶斯神经网络（BNN）对超出训练范围的光度数据推断有改进但会降低明亮源精度，无监督聚类识别出两组主导简并解（1.2, 2.3）和（1.6, 2.5）。
...
**TL;DR**: 基于磁流体动力学模拟（Magneticum simulations）研究弱引力透镜质量偏差（weak-lensing mass bias）对宇宙学参数的依赖性，发现采用宇宙学依赖的晕浓度模型可消除晕轮廓变化导致的偏差，而重子效应强度需在未来模型中显式考虑。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于弱引力透镜效应（weak gravitational lensing）在星系团（galaxy cluster）视线方向引起的剪切测量，旨在校准星系团可观测质量关系并精确推断宇宙学参数，重点关注宇宙学参数对透镜质量偏差（lensing mass bias）的影响。

Data: 数据来源于Magneticum模拟（Magneticum simulations）中115,920个质量$M_{200\mathrm c}>1.56\times10^{14}\,h^{-1}M_\odot$的星系团投影的合成弱透镜剪切图，模拟盒子边长为$896\,h^{-1}$Mpc，设置了15种不同的宇宙学参数组合（$Ω_\mathrm{m}$、$Ω_\mathrm{b}$、$σ_8$和$H_0$），并对比了纯引力模拟和全物理流体动力学模拟以研究重子效应的影响。

Method: 采用数值模拟（Magneticum simulations）生成合成弱引力透镜剪切（weak-lensing shear）地图，并基于纳维-弗伦克-怀特（Navarro-Frenk-White, NFW）轮廓模型提取弱透镜质量测量值，量化其与真实晕质量（halo mass）的偏差（bias）。

Result: 弱引力透镜质量偏差（weak-lensing mass bias）随宇宙学参数变化而变化，采用依赖宇宙学参数的浓度模型可消除晕轮廓变化影响，使质量偏差保持恒定。
...
**TL;DR**: 首次利用BINGO和SKA1-MID Band 2射电望远镜的全天相对论性双谱（bispectrum）进行预测分析，发现二阶速度项贡献了低红移处总信号的24%，且双谱联合角功率谱（angular power spectrum）和普朗克CMB数据可将CPL参数w0和wa的约束提升70%以上、哈勃参数h提升60%，显著改善暗能量探测能力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用BINGO和SKA1-MID射电望远镜的全天角功率谱（angular power spectrum）和双谱（bispectrum）预测宇宙学参数约束，特别关注相对论性双谱在红移空间中对暗能量（dark energy）性质探测的关键作用。

Data: 数据来源于BINGO和SKA1-MID Band 2射电望远镜的全天角功率谱（angular power spectrum）和双谱（bispectrum）观测数据，并结合了普朗克宇宙微波背景（Planck CMB）数据。

Method: 采用全天空角功率谱（angular power spectrum）和双谱（bispectrum）及其费希尔矩阵（Fisher matrices）进行宇宙学参数约束预测，首次在红移空间中应用全天空相对论性双谱（relativistic bispectrum）分析方法。

Result: 二阶速度项贡献在低红移下占总信号的约24%，表明其必须被纳入精确建模。
...
**TL;DR**: 基于南极望远镜第三代相机(SPT-3G)2019-2020年观测数据，首次制作并公开发布了95/150/220 GHz频段宇宙微波背景(CMB)强度(I)和偏振(Q/U)斯托克斯参数的高精度天图，其白噪声水平分别达到5.4/4.4/16.2 μK-arcmin(I)和8.4/6.6/25.8 μK-arcmin(Q/U)，是目前测量中高多极矩温度各向异性和E模偏振最深的观测数据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用毫米波天空测绘（包含宇宙微波背景辐射（CMB）各向异性信息）开展宇宙学分析。

Data: 数据来源于南极望远镜第三代相机（SPT-3G）在2019和2020年前两年主巡天期间采集的95、150和220 GHz频段数据，覆盖了4%的天区。

Method: 采用高频滤波（high-pass filter）处理单探测器时间序列数据，经像素化分箱后通过校准和清洗流程降低系统误差，并通过零检验（null test）识别并修正显著系统误差。

Result: 成功构建了基于SPT-3G相机2019-2020年观测数据的95/150/220 GHz多频段I/Q/U斯托克斯(Stokes)参数天空图，其I参数白噪声水平分别达5.4/4.4/16.2 μK-arcmin，Q/U参数达8.4/6.6/25.8 μK-arcmin，是目前测量中高ℓ值CMB温度/E模偏振各向异性和引力透镜势的最深空图。
...
**TL;DR**: 通过统计暗汽笛法（dark siren）分析17个引力波事件（包括7个来自第四观测轮次O4a）并结合亮汽笛GW170817数据，改进宿主星系加权和选择效应处理方法，测得哈勃常数（Hubble constant）$H_0 = 69.9^{+4.1}_{-4.0}$ km/s/Mpc，将不确定性较单独使用GW170817的结果降低了约11%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用引力波（GW）事件和星系巡天数据改进哈勃常数（Hubble constant, H0）的测量精度。

Data: 数据来源包括第四轮LIGO-Virgo-KAGRA（LVK）观测运行的7个精确定位引力波（GW）事件、前三轮运行的10个额外事件（来自GWTC-4天图）、DESI Legacy Imaging Survey（LS）星系目录的光度红移概率密度函数（通过深度学习模型计算）以及GW170817明亮警报事件（包含喷流视角约束和宿主星系本动速度修正）。

Method: 采用统计暗铃（dark siren）方法结合深度学习模型计算光度学红移概率密度函数，并引入r波段光度权重、扩展的引力波（GW）似然函数及选择效应一致性处理等关键方法改进。

Result: 基于17个暗警报（dark siren）引力波事件（其中7个来自LVK第四观测季O4a）并结合r波段光度加权方法，测得哈勃常数（Hubble constant）$H_0 = 78.2^{+12.0}_{-11.0}$ km/s/Mpc；进一步结合亮警报（bright siren）GW170817数据后，最终约束结果为$H_0 = 69.9^{+4.1}_{-4.0}$ km/s/Mpc，相对仅使用GW170817的结果将不确定性降低了约11%。
...
**TL;DR**: 研究发现星系大小密度涨落（size density fluctuations）显著违背普适性关系（universality relation），其有效$b_φ/b_1$比值在幅度上更大且符号与数量计数相反，从而为同一星系样本中的原初非高斯性（local PNG）探测提供了互补性探针。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用星系尺寸密度涨落（size density fluctuations）突破局域原初非高斯性（local primordial non-Gaussianity）探测中传统数量密度涨落（number density fluctuations）因普适性关系（universality relation）导致的灵敏度限制。

Data: 数据来源于DESI类巡天（Dark Energy Spectroscopic Instrument-like survey）的星系数量和尺寸观测数据。

Method: 采用尺寸密度涨落（size density fluctuations）分析技术，通过测量星系尺寸对长波长原初势（local primordial non-Gaussianity）和晚期密度涨落（late-time density fluctuations）的响应比（$b_φ/b_1$），突破传统数量密度涨落（number density fluctuations）的普适性关系限制。

Result: 星系尺寸密度涨落显著违背普适性关系，其有效$b_φ/b_1$在量级上远大于数量计数且符号相反。
...
**TL;DR**: 维拉·C·鲁宾天文台（Vera C. Rubin Observatory）的快速处理系统（Prompt Processing）实现了每晚自动处理10 TB原始图像并生成多达1000万条暂现源（transient）警报，满足吞吐量、延迟和可靠性要求。


<details>
  <summary>Details</summary>

Background: 研究聚焦于维拉·鲁宾天文台(Vera C. Rubin Observatory)瞬变源(transient)实时处理系统在大规模天文数据流下的性能优化与可靠性验证。

Data: 数据来源为Vera C. Rubin天文台（Vera C. Rubin Observatory）每晚自动处理的10 TB原始图像（raw images）及由此产生的1000万条暂现源警报（transient alerts）。

Method: 采用实时流处理（stream processing）框架结合分布式计算架构，通过并行化图像差分（image differencing）和机器学习分类（machine learning classification）技术实现瞬变源（transient）的高效检测与分类。

Result: 鲁宾天文台（Vera C. Rubin Observatory）的快速处理系统（Prompt Processing）每晚可自动处理10 TB原始图像并生成多达1000万条暂现源（transient）警报。
...
**TL;DR**: 开发了AI驱动的光谱分析平台“SpecZoo”，整合信息技术和机器学习实现交互式可视化、自动分类、参数测量等功能，显著提升大规模光谱巡天项目（如LAMOST、SDSS、DESI）的数据处理效率并支持科教融合。


<details>
  <summary>Details</summary>

Background: 天文光谱（astronomical spectra）作为蕴含丰富天体物理和化学信息的关键载体，是研究天体对象和宇宙规律的基础工具。

Data: 数据来源于LAMOST（郭守敬望远镜）、SDSS（斯隆数字化巡天）和DESI（暗能量光谱仪）等大型光谱巡天项目。

Method: 采用人工智能驱动的光谱分析平台（SpecZoo），集成机器学习与信息技术，实现交互式可视化、自动分类、参数测量、注释及多波段/多模态数据融合。

Result: 开发了一个名为SpecZoo的人工智能平台，集成现代信息技术和机器学习，提供交互式可视化、自动光谱分类、物理参数测量、光谱注释及多波段/多模态数据融合功能，支持LAMOST、SDSS、DESI等重大项目的光谱数据处理和研究。
...
**TL;DR**: 开发了基于JAX的开源框架Furax，为天体物理和宇宙微波背景(CMB)研究提供可组合的线性算子、预处理器及求解器，支持数据采集系统建模和反问题求解。


<details>
  <summary>Details</summary>

Background: 研究主题为开发基于JAX的通用开源框架（Furax），用于天体物理学和宇宙学中的逆问题求解及数据采集系统建模。

Data: 数据来源于开源Python框架Furax提供的通用和领域特定线性算子（linear operators）、预处理器（preconditioners）及求解器（solvers），支持天体物理和宇宙微波背景（CMB）数据分析中的地图制作（map-making）、仪器建模（instrument modeling）和天体成分分离（astrophysical component separation）。

Method: 基于JAX构建的模块化框架，提供通用及领域专用线性算子（linear operators）、预处理器和数值求解器，支持天体物理和宇宙微波背景（CMB）数据建模与反演。

Result: Furax框架基于JAX构建，为天体物理学和宇宙学提供可组合的通用及领域专用线性算子（linear operators）、预处理器和数值求解器，支持数据采集系统建模和反问题求解。
...
**TL;DR**: 该论文通过比较蒙特卡洛保形预测（MCCP）与贝叶斯深度学习模型在射电星系（radio galaxies）分类中的不确定性度量，发现两者仅存在弱相关性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于海量天文数据背景下利用深度学习模型进行射电星系（radio galaxies）分类时预测不确定性（uncertainty）的量化挑战。

Data: 数据来源于预训练的射电星系基础模型（radio galaxy foundation model）和经过微调的小型标记射电星系数据集，其中标记数据包含注释者提供的软标签分布（soft label distributions）。

Method: 采用蒙特卡洛保形预测（MCCP）集大小和置信度作为射电星系分类的模型不确定性度量，并通过标注者提供的软标签分布（soft label distributions）进行校准。

Result: 蒙特卡罗保形预测（MCCP）的集合大小和置信度与贝叶斯深度学习模型的预测熵测度仅呈现弱相关性。
...
**TL;DR**: 这篇论文的核心贡献是提出了一种考虑光束偏振特性和光源偏振状态的光学长基线干涉测量理论，引入了广义穆勒矩阵（generalized Mueller matrix）概念来描述多孔径干涉仪中观测到的斯托克斯可见度（Stokes visibilities）与目标斯托克斯可见度之间的矩阵关系，并证明了即使对于非偏振光源也需要消除偏振串扰对经典复可见度（complex visibilities）的影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于光学长基线干涉仪（optical long-baseline interferometers）中光束偏振特性对天文源空间相干性测量的影响及其理论建模。

Data: 数据来源于非偏振中性干涉仪（non-polarization neutral interferometers）对偏振或非偏振源的观测，结合了射电孔径合成（radio aperture synthesis）概念发展的光学长基线干涉测量理论。

Method: 采用广义穆勒矩阵（generalized Mueller matrix）理论描述多孔径干涉仪中偏振特性对斯托克斯可见度（Stokes visibilities）的扰动，并建立仪器偏振特性与目标斯托克斯可见度间的矩阵关系。

Result: 建立了考虑光束偏振特性和光源偏振特性的长基线光学干涉测量理论，引入了广义穆勒矩阵（generalized Mueller matrix）概念来描述多孔径干涉仪中受仪器偏振特性影响的斯托克斯可见度（Stokes visibilities）与目标斯托克斯可见度之间的矩阵关系。
...
### [25] [A Planetary Illusion's Funeral: Non-detection of a Gaia DR3 Exoplanet Candidate, and the Role of Intermediate-precision Radial Velocities in Gaia Exoplanet Follow-up](https://arxiv.org/abs/2603.19402)
*Alexander Venner,Chelsea X. Huang,David W. Latham,Samuel N. Quinn,Allyson Bieryla,Andrew Vanderburg,Robert A. Wittenmyer*

**TL;DR**: 通过TRES光谱仪对HD 12800进行的径向速度（radial velocity）观测未能证实盖亚（Gaia）DR3发布的系外行星候选体，并指出中等精度（≈10 m s⁻¹）的径向速度观测在盖亚DR4系外行星候选体后续验证中对8<G<12恒星具有重要作用。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是利用天体测量学（astrometry）探测系外行星的挑战与机遇，特别是基于盖亚（Gaia）任务的高精度全天观测数据。

Data: 数据来源于盖亚(Gaia)任务发布的DR3数据以及使用TRES摄谱仪获取的HD 12800径向速度(radial velocity)观测数据。

Method: 采用径向速度（radial velocity, RV）观测技术，利用TRES摄谱仪对HD 12800进行后续观测，并结合中等精度（≈10 m s⁻¹）的RV光谱分析手段验证盖亚（Gaia）天体测量法发现的外行星候选体。

Result: 使用TRES光谱仪进行的径向速度（radial velocity）观测未能证实盖亚（Gaia）任务在HD 12800周围发现的系外行星候选体。
...
### [26] [Cosmology and modified GW propagation from the BNS mass function at third-generation detector networks](https://arxiv.org/abs/2603.19377)
*Dounia Nanadoumgar-Lacroze,Niccolò Muttoni,Michele Maggiore,Michele Mancarella*

**TL;DR**: 通过双中子星(BNS)质量函数预测哈勃参数(H_0)和修正引力波传播参数(Xi_0)，发现爱因斯坦望远镜(ET)单独使用时对H_0和Xi_0的测量精度分别为11-12%和18%，而加入宇宙探索者(CE)后精度提升至9%和6%，且在红移z≈0.2-0.4处对哈勃参数H(z)的测量精度可达3-10%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用双中子星（BNS）质量函数和第三代引力波探测器（如爱因斯坦望远镜ET和宇宙探索者CE）网络，对哈勃参数（H_0）和修正引力波传播参数（Xi_0）进行精确测量，以约束ΛCDM宇宙学模型的膨胀历史。

Data: 数据来源包括爱因斯坦望远镜（Einstein Telescope, ET）的三角形（triangle）和“2L”配置观测数据，以及由ET与40公里宇宙探索者（Cosmic Explorer, CE）组成的探测器网络数据，限制信噪比（SNR）大于50的双中子星（BNS）事件。

Method: 采用贝叶斯(Bayesian)联合宇宙学-种群推断方法对高信噪比(SNR>50)的双中子星(BNS)系统进行全参数分析。

Result: 对于ET单独使用，三角形和2L构型对哈勃参数(H_0)的测量不确定度分别为12%和11%，对修正引力波传播参数(Xi_0)的不确定度均为18%。
...
### [27] [Observational imprints and quasi-Periodic oscillations of magnetically charged anti-de Sitter black holes](https://arxiv.org/abs/2603.19458)
*Faizuddin Ahmed,Mohsen Fathi,Ahmad Al-Badawi*

**TL;DR**: 研究了弦理论启发下欧拉-海森堡理论中磁荷反德西特黑洞（magnetically charged Anti-de Sitter black hole）的可观测特征，发现磁荷参数$Q_m$会减小光子球和阴影半径、改变轨道结构并影响准周期振荡（QPO）数据拟合，但当前观测数据仅给出$Q_m/M$小于0.2的上限约束。


<details>
  <summary>Details</summary>

Background: 研究探索了弦理论启发的欧拉-海森堡理论中磁荷反德西特黑洞（magnetically charged Anti-de Sitter black hole）的可观测特征及其对光子轨迹、粒子运动和准周期振荡（QPO）数据的影响。

Data: 研究使用理论推导方法分析光子轨迹、粒子运动及准周期振荡（QPO）数据，其中QPO数据来源于恒星质量、中等质量及超大质量黑洞候选体的双峰准周期振荡观测。

Method: 采用有效势方法（effective potential method）分析光子轨迹、粒子运动及计算相应环向频率（epicyclic frequencies），并通过二维Delta卡方分析（Delta chi-square analysis）在（r, Q_m）空间拟合准周期振荡（QPO）数据。

Result: 磁荷参数$Q_m$增大导致光子球和阴影半径单调减小，表明磁荷改变了黑洞附近的光传播行为。
...
### [28] [Casimir-Induced Quintessence in Dark Dimension](https://arxiv.org/abs/2603.19819)
*Tomoki Katayama,Hiroki Matsui,Yuri Michinobu,Fumiya Okamatsu,Yutaka Sakamura,Takahiro Terada*

**TL;DR**: 该研究在暗维度（Dark Dimension）场景下提出一个五维模型，通过引入额外自由度（如右手中微子）构建平坦的正径向子（radion）势能，使其作为精质场（quintessence field）在亚电子伏特尺度缓慢演化，从而解释观测到的暗能量并与DESI重子声学振荡测量结果一致。


<details>
  <summary>Details</summary>

Background: 该研究探讨了暗维度（Dark Dimension）场景的具体实现，其中单个大额外维度设定在亚毫米尺度，并利用体场（bulk fields）的卡西米尔能量解释观测到的暗能量。

Data: 研究使用了5维理论框架下的模拟数据（simulation data），包括标准模型（Standard Model）限制在4维膜（brane）上的设定以及额外自由度（bulk degrees of freedom）的扩展。

Method: 采用五维框架，将标准模型限制在四维膜上，推导出径向子（radion）的有效作用量，并通过引入额外体自由度构建平坦正势能，使径向子表现为精质场（quintessence field）。

Result: 在五维框架中，仅包含引力和三个右手中微子（right-handed neutrinos）的最小模型通常产生负的radion势，而引入额外自由度后可生成足够平坦的正势，使radion作为精质场（quintessence field）在亚电子伏特尺度缓慢演化。
...
### [29] [The characteristics of variability of AGNs based on the structure function](https://arxiv.org/abs/2603.19837)
*Xuan Wei,Jie Tang,Yu Tao*

**TL;DR**: 通过改进的结构函数（structure function）方法有效区分了射电、光学和伽马波段的活动星系核（AGN）样本，证实不同波段的变异性分别由吸积盘波动（光学波段）和相对论性喷流辐射（射电和伽马波段）引起，并验证了费米（Fermi）与非费米样本中BL Lac天体与平谱射电类星体（FSRQ）的差异。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核(AGN)的多波段光变特性及其物理机制差异，支持统一标准解释框架。

Data: 数据来源于OVRO（射电波段）、ASAS-SN（光学波段）和Fermi（伽马射线波段）的观测样本。

Method: 采用归一化结构函数（normalized structure function）结合幂律函数模型（power-law function model）对射电、光学和伽马波段样本进行变异性分析，并通过整合两个参数实现有效区分。

Result: 改进的结构函数能有效区分射电、光学和伽马波段样本，并验证了BL Lac天体（BL Lacs）和平谱射电类星体（FSRQs）在费米（Fermi）与非费米样本中的差异。
...
### [30] [VHE gamma-ray intranight variability from BL Lacertae during the extreme flaring state of 2022](https://arxiv.org/abs/2603.19889)
...
**TL;DR**: 研究发现BL Lacertae（BL Lac）在2020年至2022年期间经历了一系列多波段爆发，其中2022年9月至11月的耀变活动观测到创纪录的甚高能（VHE, E>100 GeV）γ射线通量，并首次记录到8分钟内通量倍增的极快内禀变现象，通过双区轻子模型解释了耀变期间多波段光谱能量分布（SED）演化及喷流物理参数变化。


<details>
  <summary>Details</summary>

Background: BL Lacertae（BL Lac）作为其亚类的原型耀变体（blazar）和过去几十年研究最多的耀变体之一，自2020年以来经历了一系列剧烈的多波段爆发活动，并在2022年9月至11月期间达到迄今记录的最高γ射线耀发水平。

Data: 数据来源于2020年至2024年期间对BL Lacertae（BL Lac）的多波段（multi-wavelength）观测，包括2022年9月至11月、2021年8月和2024年10月的γ射线（γ-ray）耀发事件，重点分析了2022年10月20日和11月13日的甚高能（VHE, E>100 GeV）γ射线夜间快速变光数据，并采用贝叶斯块（Bayesian block）分析方法识别稳定发射期。

Method: 采用贝叶斯块分析（Bayesian block analysis）识别稳定发射期，并基于轻子双区模型（leptonic two-zone model）解释耀斑期间相对论性粒子变化和喷流物理参数演化。

Result: BL Lac在2022年9月至11月期间经历了迄今记录的最强γ射线耀发，并在10月20日和11月13日夜间观测到快至约8分钟的甚高能(VHE, E>100 GeV)γ射线通量倍增变化。
...
### [31] [Vector-channel scattering of dark particles in a Sp(4) gauge theory](https://arxiv.org/abs/2603.19557)
...
**TL;DR**: 研究通过格点计算（lattice study）分析了Sp(4)规范理论耦合两个基本狄拉克费米子（fundamental Dirac fermions）的伪标量态（pseudoscalar states）自旋-1通道散射振幅，为强相互作用大质量粒子（SIMPs）范式暗物质模型提供了动力学起源的候选理论。


<details>
  <summary>Details</summary>

Background: 研究聚焦于$Sp(4)$规范场论（gauge theory）耦合两个基本狄拉克费米子（Dirac fermions）的格点模拟，探索强相互作用大质量粒子（SIMPs）范式下暗物质（dark matter）模型的动力学起源。

Data: 数据来源为基于不同（Wilson）费米子质量范围生成的格点（lattice）系综（ensemble）数据。

Method: 采用格点（lattice）QCD技术结合Lüscher有限体积能级分析方法，研究Sp(4)规范理论中两个基本狄拉克费米子的自旋1通道赝标量态散射振幅。

Result: 该研究通过格点计算分析了$Sp(4)$规范理论耦合两个基本狄拉克费米子的情况，为强相互作用大质量粒子范式中的暗物质模型提供了动力学起源的候选方案。
...
**TL;DR**: 该论文以“瑜伽”（yoga）弛豫模型为框架，揭示了真空能弛豫机制通过抑制轴子（axion）真空势能改变其宇宙学行为（使其偏离标准QCD质量-耦合关系），导致传统QCD轴子在现有观测约束下难以存活。


<details>
  <summary>Details</summary>

Background: 研究探讨了真空能（vacuum energy）动态弛豫机制对低能物理（low-energy physics）的影响及其在宇宙学常数问题中的应用。

Data: 数据来源为理论模拟（simulation）框架下的瑜伽松弛模型（'yoga' relaxation models）和QCD轴子（QCD axion）相关参数。

Method: 采用“瑜伽”（yoga）弛豫模型作为框架，分析弛豫子（relaxon）质量设定的特征时间尺度对慢物理过程的差异化抑制效应。

Result: 真空能（vacuum energy）弛豫机制不仅改变宇宙学常数，还会显著影响低能标物理，特别是轻标量场（light scalar fields）在宇宙学时间尺度上的行为。
...
### [33] [High-resolution mid-IR spectroscopy of SVS 13-A with EXES/SOFIA: The surprisingly high CH$_3$OH/H$_2$O ratio in the planet-forming zone of a solar mass protostar](https://arxiv.org/abs/2603.19992)
...
**TL;DR**: 通过高分辨率中红外光谱首次在低质量原恒星SVS13-A的内包层（inner envelope）探测到暖气体（~140-170 K）中甲醇（CH3OH）柱密度显著高于水（H2O）（比例达4倍），揭示了行星形成区冰升华与气相化学的直接联系及冰层分馏（stratification）或结合能分布导致的选择性升华机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低质量原恒星（low-mass protostar）内区水（H$_2$O）和甲醇（CH$_3$OH）的化学组成及其在行星形成环境中的冰-气相转变过程。

Data: 研究使用SOFIA机载EXES仪器在26微米波段对I型原恒星SVS13-A进行了高分辨率（R=70,500）中红外光谱观测，检测到了H2O和CH3OH的吸收线。

Method: 采用高分辨率（R = 70,500）中红外光谱技术，通过SOFIA机载EXES仪器在26 μm波段观测H₂O和CH₃OH吸收线，分析暖气体（rotational temperatures）的旋转温度（约140-170 K）和柱密度比。

Result: 在低质量原恒星SVS13-A的内包层中，甲醇(CH3OH)柱密度比水(H2O)高出约4倍，显著高于典型星际冰比例(<10%)，可能反映了结合能分布或冰层分选导致的选择性升华。
