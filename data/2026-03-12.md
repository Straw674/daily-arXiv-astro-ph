<div id=toc></div>

# Table of Contents

- [astro-ph.GA](#astro-ph.GA) [Total: 11]
- [astro-ph.CO](#astro-ph.CO) [Total: 10]
- [astro-ph.IM](#astro-ph.IM) [Total: 8]
- [gr-qc](#gr-qc) [Total: 1]
- [astro-ph.EP](#astro-ph.EP) [Total: 6]
- [hep-ph](#hep-ph) [Total: 2]
- [astro-ph.HE](#astro-ph.HE) [Total: 1]
- [astro-ph.SR](#astro-ph.SR) [Total: 4]


<div id='astro-ph.GA'></div>

# astro-ph.GA [[Back]](#toc)

### [1] [A Kiloparsec-Scale Stellar Cavity in the Center of Abell402-BCG May be Caused by Dynamic Interactions with an Ultramassive Black Hole](https://arxiv.org/abs/2603.10104)
*Michael McDonald,Gourav Khullar,David Lagattuta,Guillaume Mahler,Shashank Dattathri,Jose M. Diego,Alastair C. Edge,Benjamin Floyd,Michael D. Gladders,Scott A. Hughes,Mathilde Jauzac,Nader Khonji,Gavin Leroy,Richard Massey,Mireia Montes,Priyamvada Natarajan,Michael Reefe,Keren Sharon,Frank van den Bosch,Stepane Werner,Adi Zitrin*

**TL;DR**: JWST和HST观测发现Abell402星系团中心星系存在一个kpc尺度的恒星缺失腔（cavity），对应缺失约2×10^9太阳质量的恒星，并揭示其中心存在一个质量6±4×10^10太阳质量的超大质量黑洞（ultra-massive black hole），以及一个可能由动力学相互作用形成的kpc分离的双活动星系核（dual AGN）系统，总质量达6±2×10^10太阳质量。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST和HST观测数据揭示星系团Abell402中心星系恒星分布中的空洞现象及其与超大质量黑洞（SMBH）动力学相互作用的关联。

Data: 数据来源包括JWST（James Webb Space Telescope）NIRCam（Near-Infrared Camera）的新观测数据、HST（Hubble Space Telescope）的辅助数据以及MUSE（Multi Unit Spectroscopic Explorer）的光谱数据。

Method: 采用JWST（James Webb Space Telescope）NIRCam（近红外相机）和HST（Hubble Space Telescope）观测数据结合MUSE（Multi Unit Spectroscopic Explorer）光谱分析，通过测量恒星密度场局部凹陷和超大质量黑洞（supermassive black hole）动力学特征，推断双活动星系核（dual AGN）系统的存在及其相互作用机制。

Result: 在Abell402星系团中心星系中发现了一个千秒差距（kpc）尺度的恒星分布空洞（cavity），对应约2×10^9太阳质量（Msun）的恒星缺失，且该空洞边缘存在一个中红外（mid-IR）明亮的点源，表明中心超大质量黑洞（M_BH = 6±4×10^10 Msun）正在吸积；同时光谱数据暗示可能存在一个千秒差距分离的双活动星系核（dual AGN），总质量达6±2×10^10 Msun，可能是迄今发现的最巨大双黑洞系统。

Abstract: We present new observations from JWST NIRCam that reveal a striking kpc-wide cavity in the stellar distribution of the central galaxy in the cluster Abell402. Supporting data from HST allow us to rule out extinction due to dust as an explanation and, instead, suggest that this is a localized depression in the stellar density field corresponding to ~2x10^9 Msun in missing stars within a volume of 0.5kpc^3. On larger scales, both the JWST and HST data show evidence for a 2.2kpc flattened core in the stellar distribution (on which the smaller-scale cavity is superimposed), which implies the presence of a central ultra-massive black hole with M_BH = 6 +/- 4 x10^10 Msun. We report evidence for a mid-IR-bright point source at one edge of the cavity, suggesting that this black hole is actively accreting. MUSE spectroscopy reveal that this source is a LINER AGN and that there is a second candidate AGN on the opposite side of the cavity with a relative velocity of 370km/s -- if real, this implies the presence of a kpc-separation dual AGN with a total binary mass of 6 +/- 2 x10^10 Msun, which would make this the most massive binary black hole system discovered to date. We propose that this unique stellar cavity is the result of a short-lived dynamical interaction between at least one supermassive black hole and the background stellar density field, caused either by three-body scattering during binary hardening or the induction of a dipole instability in the stellar density field.

</details>

### [2] [Right round: onset and long-term evolution of rotation in star clusters](https://arxiv.org/abs/2603.10109)
*E. Dalessandro,A. Della Croce,E. Vesperini,M. Cadelano,S. Leanza,G. Ettorre,M. Hughes*

**TL;DR**: 首次发现银河系星团(star clusters)任何年龄阶段都存在旋转现象，约25%-30%的样本系统显示显著旋转证据，且年轻星团(<500 Myr)旋转系统比例(50%-60%)明显高于年老星团(~15%)，表明旋转可能形成于星团早期演化阶段并随动力学演化逐渐衰减，同时三维分析显示演化程度较高的星团倾向于顺向旋转(prograde)，与分子云原始旋转和星系扭矩驱动对齐的预期定性一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系星团（Galactic star clusters）旋转现象的起源与演化机制，填补了星团旋转随年龄变化的观测空白。

Data: 数据来源于对银河系已知星团（Galactic star clusters）样本的详细运动学分析，包括年轻（<500 Myr）和年老星团的旋转速度测量及三维运动学参数提取。

Method: 采用三维运动学(3D kinematics)分析和旋转速度分布统计方法，结合分子云印记和层次性星团组装(hierarchical cluster assembly)的物理模型，量化研究了星团旋转轴与轨道运动轴的夹角演化。

Result: 首次发现银河系星团（Galactic star clusters）中任何年龄阶段均存在旋转现象，约25%-30%的样本系统显示出显著旋转证据。

Abstract: We present the results of a detailed kinematic analysis of a significant fraction of the known population of Galactic star clusters aimed at constraining the physical mechanisms driving the onset and evolution of cluster rotation. Our study reveals for the very first time the presence of rotation in clusters at any age, with about $25\%-30\%$ of systems in the sample showing significant evidence of rotation. This result increases by a factor of $\sim5$ the number of clusters identified as rotators so far and it finally enables an observational reading of cluster rotation as a function of time. Young ($<500$ Myr) clusters show a larger range of rotation velocities than older systems. In addition, at young ages we observe a significantly larger fraction ($50\%-60\%$) of rotating systems than at older ones ($\sim 15\%$). These purely empirical results are compatible with rotation being imprinted during the very early stages of cluster formation and early evolution and then being progressively erased by the long-term effects of dynamical evolution. For the sub-sample of clusters for which we were able to perform a full 3D analysis, we calculated the angle between the internal rotation axis and that of the cluster orbital motion. Interestingly, while for clusters with an age smaller than their orbital period we observe similar fractions of prograde and retrograde systems, more evolved clusters appear to be preferentially prograde. We argue that such a behavior is in qualitative agreement with the expectations for the evolution of systems in which primordial rotation was imprinted by the parent molecular cloud and/or by the following hierarchical cluster assembly processes, and in which internal cluster dynamics and interactions with the Galactic field have induced a torque-driven alignment between cluster rotation and orbital motion.

</details>

### [3] [A Statistical Framework to Identify Kinematically Outlying LMC Globular Clusters and Implications for the LMC's Dark Matter Profile](https://arxiv.org/abs/2603.10118)
*Tamojeet Roychowdhury,Navdha,Himansh Rathore,Knut A. G. Olsen*

**TL;DR**: 基于盖亚卫星(Gaia-DR3)自行运动数据结合视线速度(LoS)测量，开发了新统计框架识别5个自行运动异常和6个视线速度异常的大麦哲伦云(LMC)球状星团(GC)，发现多数异常星团集中在距中心3-4千秒差距处，其存在会导致LMC质量估算偏差达30%，暗示它们可能源自外部星系吸积。


<details>
  <summary>Details</summary>

Background: 研究主题为利用大麦哲伦云（LMC）球状星团（GCs）的运动学异常特征探索其吸积历史和暗物质（DM）性质。

Data: 数据来源为盖亚卫星第三次数据释放（Gaia-DR3）的自行（Proper Motions）测量结果，并结合了先前的视线速度（Line-of-Sight velocity）测量数据。

Method: 采用盖亚卫星DR3自行运动数据(Gaia-DR3 Proper Motions)结合视线速度(Line-of-Sight velocity)测量，通过计算球状星团速度矢量与周围红团簇星平均速度矢量的差异作为运动学异常度量，同时考虑速度测量不确定性和大麦哲伦云(LMC)本征速度弥散，建立新的统计框架识别运动学异常球状星团。

Result: 基于盖亚卫星DR3自行运动(PMs)和视线速度(LoS)测量，开发了新统计框架识别5个自行运动异常和额外6个视线速度异常的LMC球状星团(GCs)，其中多数异常星团距LMC中心3-4千秒差距。

Abstract: The LMC's Globular Clusters (GCs) bring a novel opportunity to understand the LMC's assembly history and dark matter (DM) properties, provided the kinematically outlying GCs can be reliably identified. However, traditional diagnostics like the Energy-Angular Momentum space fail because of large uncertainties on the GC velocities. In this work, we develop a new, robust statistical framework for identifying kinematically outlying LMC GCs, by using their Gaia-DR3 Proper Motions (PMs) combined with previous Line-of-Sight (LoS) velocity measurements. We use the difference between a GC's velocity vector and the average velocity vector of the surrounding red clump stars as a metric for quantifying a GC's kinematic peculiarity. We account for both the velocity measurement uncertainties and the LMC's intrinsic velocity dispersion. We find 5 LMC GCs to be kinematically outlying based on PM differences alone, and additional 6 GCs if LoS velocity information is also used. Majority of the GCs with outlying PMs are clustered at a distance of 3-4 kpc from the LMC center. The inclusion of outlying LMC GCs introduces a bias of upto 30% in the LMC's enclosed mass estimates derived using GCs as dynamical tracers; caution must be exercised in choosing the GC sample for precisely determining the LMC's DM content. We discuss the possibility that the kinematically outlying LMC GCs may have been accreted from external galaxies, and motivate future spectroscopic follow-up of the GC population to better understand the assembly history of massive satellite galaxies of Milky Way like hosts.

</details>

### [4] [Three Hundred Quasars from the Couch: A first look at high-redshift quasar discovery with SPHEREx](https://arxiv.org/abs/2603.10135)
*Frederick B. Davies,Sarah E. I. Bosman,Arpita Ganguly,Eduardo Bañados,Silvia Belladitta,Daniel Stern,Javier A. Acevedo Barroso,Daming Yang,Joseph F. Hennawi,Feige Wang,Jinyi Yang,Xiaohui Fan*

**TL;DR**: 利用SPHEREx分光光度测量数据无需地面后续观测即可高效确认高红移（z≳4）类星体，成功发现87个4.0<z<5.7的明亮类星体（中位M₁₄₅₀=-27.5）并验证其选择方法具有100%确认率，同时揭示了传统类星体巡天可能遗漏的红化及宽吸收线等罕见低红移（0.3<z<4）类星体。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用SPHEREx分光光度测量数据高效确认高红移（$z\gtrsim4$）类星体，解决传统测光选择中因恒星、低红移类星体及致密红星系污染导致的低成功率问题。

Data: 数据来源为SPHEREx分光光度测量（spectrophotometric survey）数据，结合盖亚卫星（Gaia）和广域红外巡天探测器（WISE）的光度测量（photometric）及天体测量（astrometric）数据，并辅以地面4米和8米级望远镜的光谱观测（ground-based spectroscopic follow-up）验证。

Method: 采用SPHEREx分光光度测量数据（SPHEREx spectrophotometric survey data）结合盖亚卫星（Gaia）和广域红外巡天探测器（WISE）的光度与天体测量筛选方法，通过检测强宽Hα发射线（broad Hα emission lines）确认高红移类星体。

Result: 利用SPHEREx光谱测光数据无需地面后续观测即可确认高红移（z≳4）类星体，成功发现87个新的4.0<z<5.7光度类星体（中位M₁₄₅₀=-27.5），包括19个z>5的类星体，并复现了219个已发表的z>4类星体。

Abstract: Photometric selection of luminous high-redshift ($z\gtrsim4$) quasars is plagued by contamination from numerous low-mass Galactic stars, reddened lower-redshift quasars, as well as compact luminous red galaxies. Confirmation of these rare objects thus requires extensive spectroscopic campaigns on 4 and 8-meter-class telescopes with relatively low success rates. Here we demonstrate the utility of SPHEREx spectrophotometric survey data for quasar confirmation with no ground-based follow-up required, "from the couch," applied to candidates from a purposefully simplistic photometric and astrometric Gaia+WISE selection down to low Galactic latitudes ($|b|\geq8^\circ$). Primarily from the detection of their strong broad H$α$ emission lines, we discover 87 new luminous $4.0 < z < 5.7$ quasars with median $M_\text{1450} = -27.5$, including 19 quasars at $z>5$, and recover 219 previously published quasars at $z>4$. We validate our SPHEREx selection with a 100% confirmation rate in ground-based spectroscopic follow-up of 29 of our new $z>4$ quasars, including 11 unpublished archival spectra. We also discover 203 additional lower-redshift quasars at $0.3 < z < 4$, consisting primarily of relatively rare highly-reddened and strong broad-absorption-line objects that are likely missed by traditional quasar surveys. Finally, we show that the Ly$α$ absorption breaks and H$α$ lines of luminous quasars are already detectable at redshifts $5.7\lesssim z\lesssim6.5$ after the completion of only the first of four all-sky surveys to be performed by SPHEREx during its planned two-year mission.

</details>

### [5] [The X-ray weakness of Little Red Dots and JWST-selected AGN: comparison with local AGN in different accretion regimes](https://arxiv.org/abs/2603.10162)
*A. Tortosa,C. Ricci,P. Du,G. Venturi,L. C. Ho,R. Li,J. -M. Wang,M. Berton*

**TL;DR**: 通过对比高红移（high z）LRDs和JWST选AGN与低红移（low z）SEAMBHs等AGN样本的X射线与光学性质，发现多数高红移源在$L_x/L_{Hα}$-$λ_{Edd}$平面上偏离SEAMBHs分布，表明其可能具有更极端吸积条件、冕区辐射抑制或强遮蔽，且高吸积系统普遍存在X射线相对于总辐射的不足，支持高红移LRDs和JWST选AGN的X射线弱化与高吸积超大质量黑洞（SMBHs）物理机制相关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（high z）莱曼断裂星系（LRDs）和JWST选活动星系核（AGN）的X射线辐射减弱（X-ray weakness）现象，探讨其与本地高吸积率超大质量黑洞（SEAMBHs）等低红移AGN的物理联系，旨在揭示极端吸积条件下X射线辐射抑制的普遍机制及其对宇宙不同时期高吸积率系统的适用性。

Data: 数据来源包括高红移LRDs和JWST选定的宽线AGN的X射线与光学性质数据，以及低红移AGN的多样化样本（含SEAMBHs、NLS1s和大巡天中的I型AGN）。

Method: 采用X射线与光学性质对比分析，结合宽Hα线光度（broad Hα line luminosity）、爱丁顿比（Eddington ratio）、总光度（bolometric luminosity）和X射线-总光度修正（X-ray-to-bolometric luminosity correction）的关系研究，并探讨高红移源是否为本地高吸积系统的类似物。

Result: 高红移LRDs和JWST选AGN的X射线弱化现象可能与高度吸积超大质量黑洞(SMBHs)的物理机制相关，包括极端吸积条件、日冕辐射抑制或强遮蔽效应。

Abstract: We investigate the origin of the observed X-ray weakness in high z LRDs and other JWST-selected broad line AGN by comparing their X-ray and optical properties with those of a diverse sample of low z AGN, including super-Eddington accreting massive black holes (SEAMBHs), NLS1s, and type I AGN from large surveys. We examine the relations between X-ray luminosity, broad Hα line luminosity, Eddington ratio, bolometric luminosity and X-ray-to-bolometric luminosity correction, and explore whether high z sources may represent analogues of local highly accreting systems. While a few LRDs and JWST-selected AGN are consistent with the SEAMBHs population in the $L_x/L_{Hα}$ versus $λ_{Edd}$ plane, most lie below it, suggesting either more extreme accretion conditions, suppressed coronal emission or heavy obscuration. We identify an anti-correlation between $L_x/L_{Hα}$ and $λ_{Edd}$ in the low z, high accreting subsample, consistent with theoretical expectations of slim-disc accretion. We further show that, for SEAMBHs, $Hα$-based bolometric luminosities underestimate SED-based values even after dust correction. We find that SEAMBHs, LRDs, and JWST-selected AGN occupy a similar high-$κ_{bol,x}$ regime, indicating that the relative deficit of X-ray emission compared to the bolometric output could potentially support the view that suppression of the hot corona emission is a common feature of highly accreting systems across cosmic time. Our results are consistent with the idea that the observed X-ray weakness of LRDs and JWST-selected AGN may be linked to the physics of highly accreting SMBHs. Moreover, observational limitations at high z, including instrumental sensitivity and the steep X-ray spectra expected for highly accreting systems, likely further suppress the detected X-ray signal.

</details>

### [6] [Radio selection of heavily obscured AGN in the J1030 field: unraveling a missing Compton-thick population](https://arxiv.org/abs/2603.10176)
*Giovanni Mazzolari,Roberto Gilli,Marco Mignoli,Marcella Brusa,Isabella Prandoni,Fabio Vito,Ivan Delvecchio,Giorgio Lanzuisi,Alessandro Peca,Andrea Comastri,Stefano Marchesi,Marco Chiaberge,Marisa Brienza,Cristian Vignali,Matilde Signorini,Quirino D'Amato,Fabrizio Gentile,Kazushi Iwasawa,Colin Norman,Alberto Traina,Federica Loiacono,Pietro Baldini,Marianna Annunziatella,Roberto Decarli*

**TL;DR**: 首次通过射电视角测量高红移(high-z)强遮蔽活动星系核(AGN)丰度，发现红移3附近射电选择的康普顿厚(CTK) AGN数密度比X射线模型预测高2-3倍，证实射电选择能有效探测X射线遗漏的最遮蔽源。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用射电（radio）选择方法探测高红移（high-z）严重遮蔽活动星系核（AGN）的有效性及其丰度测量。

Data: 数据来源于J1030天区的1.4 GHz射电观测和钱德拉(Chandra)X射线观测，其中射电源检测基于该天区最深组合的射电和X射线观测数据。

Method: 采用射电超量参数（REX）筛选射电超量活动星系核（AGN），并通过X射线叠加分析（X-ray stacking analysis）验证其高度遮蔽特性。

Result: 首次从射电视角测量了高红移（high-z）强遮蔽活动星系核（AGN）的丰度，发现射电选择在$z\sim3$时返回的康普顿厚（CTK）AGN数密度比X射线模型预测值高2-3倍。

Abstract: We tested the effectiveness of radio selection to discover heavily obscured AGNs, particularly at high-z, and we measured their abundance for the first time from a radio perspective. We consider the radio sources detected in the J1030 field, which is one of the fields with the deepest combination of 1.4 GHz radio and X-ray observations. We defined a radio excess parameter as the ratio between the star formation rate (SFR) that would correspond to the observed radio luminosity and the one directly derived from the spectral energy distribution (SED) fitting, $\rm REX=SFR_{1.4GHz}/SFR^{corr}_{SED}$. We then select as radio excess AGN those sources with $\rm REX>8.5$, corresponding to a $3σ$ excess above the median value. In this way, we find 145 radio-excess sources falling into the \textit{Chandra} X-ray image footprint but without X-ray detection. From the deep X-ray upper limits, we estimated a lower limit to the obscuration of each radio-excess AGN, finding on average $\log (N_H/\rm{cm^{-2}})>23.7$. A CTK AGN scenario is also supported by the results of the X-ray stacking analysis performed on sources at $z>1.5$, which revealed X-ray luminosities and hardness ratios compatible with very highly obscured AGN. Finally, we computed the number density of these radio-selected CTK AGN. While at $z\sim 2$ the radio number density agrees well with the CTK AGN predictions of different population synthesis models, at $z\sim3$ the radio selection returns a CTK AGN number density $\sim 2-3$ times larger than what is predicted by the X-ray models and observations. This result supports the effectiveness of radio emission in selecting the most obscured sources, unraveling a population of AGN potentially missed by X-rays surveys at $z>3$, paving the way to a synergistic use of the future radio and X-ray facilities such as the \textit{SKAO} and \textit{NewAthena}.

</details>

### [7] [The Rayleigh Taylor instability in partially ionized plasmas: ambipolar diffusion effects in the non linear phase](https://arxiv.org/abs/2603.10566)
*E. Callies,Z. Meliani,A. Marcowith,V. Guillet*

**TL;DR**: 该研究通过高分辨率双流体数值模拟和线性理论分析，揭示了在从非耦合到强耦合的不同离子-中性粒子耦合（ion neutral coupling）状态下，RT不稳定性（RTinstability）在线性和非线性阶段的生长特性，发现非线性阶段的双极扩散（ambipolar diffusion）会改变经典二次增长模式并引入耦合依赖的演化，且多波长扰动下磁化和非磁化配置呈现不同的尺度依赖性界面重组现象。


<details>
  <summary>Details</summary>

Background: 研究聚焦于离子中性耦合（ion neutral coupling）和双极扩散（ambipolar diffusion）对天体物理条件下瑞利-泰勒不稳定性（RT instability）线性和非线性演化的影响机制及其偏离经典单流体模型的耦合区间识别。

Data: 数据来源于高分辨率双流体数值模拟（high resolution two fluid numerical simulations），使用MPI AMRVAC代码覆盖了从非耦合到强耦合（包括中间态或双极扩散主导态）的多种扰动波长、耦合强度及磁场构型。

Method: 采用高分辨率双流体数值模拟（MPI AMRVAC code）结合线性理论重访，通过形态学诊断、谱分析、力及能量收支分析研究离子中性耦合（ion neutral coupling）和双极扩散（ambipolar diffusion）对RT不稳定性（RT instability）的影响。

Result: 在非线性阶段，双极扩散（ambipolar diffusion）改变了经典二次增长模式并引入耦合依赖的演化；多波长扰动下，非线性动力学呈现强烈尺度依赖性：中等耦合增强流体构型中的碎裂，而磁化情形则表现出界面的非单调重组，最平滑形态出现在中等耦合状态。

Abstract: Aims. We aim to determine how ion neutral coupling and ambipolar diffusion affect the linear and the nonlinear growth of the RTinstability under astrophysically relevant conditions, and to identify the coupling regimes in which departures from the classical single fluid picture become significant. Methods. We perform high resolution two fluid numerical simulations using the MPI AMRVAC code, spanning a wide range of perturbation wavelengths, coupling strengths, from uncoupled to strongly coupled passing by intermediate or ambipolar diffusion dominated regimes, and magnetic field configurations. The linear theory is revisited using a physically consistent formulation with different ion neutral coupling strengths across the interface and validated against the simulations. We investigate the physics of the instability using morphology based diagnostics of the mixing layer to compare simulations at equivalent nonlinear stages, complemented by spectral, force, and energy budgets analyses. Results. In the linear regime, theoretical growth rates are recovered over a wide range of wavelengths, from the single fluid limit to intermediate bi fluid coupling. In the nonlinear regime, ambipolar diffusion modifies the classical quadratic growth and introduces a coupling dependent evolution. For multi wavelength perturbations, the nonlinear dynamics becomes strongly scale dependent: intermediate coupling enhances fragmentation in hydrodynamic configurations, while magnetised cases exhibit a non monotonic reorganisation of the interface, with the smoothest morphologies occurring at intermediate coupling. Spectral and energetic diagnostics indicate that these behaviours correlate with changes in the relative contributions of ion neutral drift and magnetic stresses during thenonlinear evolution

</details>

### [8] [Extended Radio Galaxies in EMU: A Comparative Look at Source-Finding Techniques](https://arxiv.org/abs/2603.10579)
*Lachlan J. Barnes,Andrew M. Hopkins,Yjan Gordon,Nikhel Gupta,Gary Segal,Heinz Andernach,Michael J. I. Brown,Duncan Farrah,Stanislav S. Shabala,Sarah V. White,O. Ivy Wong*

**TL;DR**: 该研究通过比较DRAGNHunter、粗粒度复杂度指标和RG-CAT机器学习管道三种方法在EMU-G09射电巡天数据中对扩展射电源（extended radio sources）的检测效果，发现这些方法虽能联合覆盖几乎所有扩展源但识别结果重叠有限（仅375个源被共同检出），表明未来大规模巡天中需结合互补技术才能实现扩展射电源的完整普查。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宽视场射电巡天（wide-field radio surveys）中扩展射电源（extended radio sources）的自动化检测与分类方法面临的挑战及解决方案。

Data: 数据来源为EMU-G09（Evolutionary Map of the Universe观测的Galaxy And Mass Assembly 09天区）的射电观测数据。

Method: 采用三种自动检测方法：DRAGNHunter（基于组分目录检测DRAGNs）、粗粒度复杂度指标（识别复杂发射区域）和RG-CAT（基于EMU先导巡天训练的机器学习流程）。

Result: 在EMU-G09（Galaxy And Mass Assembly 09）观测中，三种自动检测方法（DRAGNHunter、粗粒度复杂度指标和RG-CAT机器学习流程）联合应用可覆盖几乎所有延展射电源，但各自识别出大部分独立且部分重叠的子集，仅375个源被所有方法共同检出，表明未来大尺度巡天需结合互补技术才能实现延展射电源的完整普查。

Abstract: Extended radio sources present unique challenges for automated detection and classification in wide-field radio surveys. With current surveys such as the Evolutionary Map of the Universe (EMU), robust and scalable methods are essential to identify and catalogue these complex sources. We apply three automatic approaches to detect complex radio emission in EMU observations of the Galaxy And Mass Assembly (GAMA) 09 field (EMU-G09) in order to evaluate their relative strengths and limitations in preparation for large-scale application across future EMU data releases. These include DRAGNHunter, designed to detect likely DRAGNs (Double Radio sources associated with Active Galactic Nuclei) from a component catalogue; coarse-grained complexity, a metric designed to highlight regions of complex emission; and RG-CAT, a machine learning pipeline trained on radio sources identified in the EMU pilot survey. We find that together, the three methods recover nearly all extended sources in EMU-G09 but identify largely distinct, partially-overlapping subsets, with only 375 sources identified by all finders. This demonstrates that a combination of complementary techniques will be required to achieve a complete census of extended radio sources in future large-scale surveys.

</details>

### [9] [The in-situ growth of stellar-mass "light" seed black holes in nuclear star clusters](https://arxiv.org/abs/2603.10581)
*Yanlong Shi,Norman Murray*

**TL;DR**: 通过磁流体动力学模拟发现，大质量恒星遗留的黑洞（light seeds）在巨型分子云（GMCs）中形成后虽集中在星团中心，但受限于反馈调节的吸积效率，仅少数种子黑洞可通过高吸积率增长至约$10^6\,M_\odot$，无法有效形成超大质量黑洞（SMBHs）的重种子（heavy seeds）。


<details>
  <summary>Details</summary>

Background: 研究探讨了恒星质量黑洞（light seeds）作为超大质量黑洞（SMBHs）潜在起源的假说，重点关注它们在巨分子云（GMCs）中的形成与增长机制。

Data: 使用磁流体动力学（MHD）模拟研究了质量为$10^5$--$10^9\,M_\odot$的恒星形成巨型分子云（GMC）中轻种子黑洞（BHs）的形成和增长，模拟持续时间为$\sim 10$--$30\,\rm Myr$，并追踪了包含辐射和力学反馈的超大质量恒星（VMSs, 100--$300\,M_\odot$）以及反馈调节的吸积过程。

Method: 采用磁流体动力学(MHD)模拟研究巨型分子云(GMC)中轻种子黑洞的形成与增长，包括对超大质量恒星(VMS)的辐射与力学反馈的精细解析，并建立反馈调节的吸积模型。

Result: 在致密巨分子云（GMC）中，即使存在深势阱，持续恒星反馈仍会驱逐气体并抑制黑洞（BH）的原位吸积，导致大多数轻种子（light seeds）仅增长至400-500倍太阳质量，仅当邦迪吸积（Bondi inflow）效率极高时少数种子可增长至约10^6倍太阳质量。

Abstract: Remnant black holes (BHs) of massive stars (``light seeds'') are a potential origin for supermassive black holes (SMBHs). We use magnetohydrodynamic simulations to study the formation and growth of light seeds in star-forming giant molecular clouds (GMCs) with masses $10^5$--$10^9\,M_\odot$, which evolve for $\sim 10$--$30\,\rm Myr$ and form compact star clusters, akin to high-redshift nuclear star clusters. In particular, the simulations resolve very massive stars (VMSs, 100--$300\,M_\odot$), including their radiative and mechanical feedback, and model feedback-regulated accretion onto remnant BHs. We find that, even in compact GMCs capable of forming deep potential wells, the gas reservoir is expelled by sustained stellar feedback and rapidly dispersed after supernova explosions. Remnant BH populations emerge $\sim 3\,\rm Myr$ after the starburst and concentrate at the cluster center (where $ρ_{\rm BH}\sim 10^4$--$10^6\,M_\odot\,{\rm pc}^{-3}$). With our fiducial sub-grid BH accretion/feedback model, in-situ BH accretion is inefficient for forming heavy seeds: some direct-collapse BHs briefly accrete at $\sim$\,(1--10)$\times$ the Eddington rate, but they reach only $\sim 400$--$500\,M_\odot$. A top-heavy initial mass function or natal kicks do not change this conclusion. Runaway accretion is only possible if the sub-grid BH model allows a high fraction of Bondi inflow to reach the BH, in which case a few seeds can grow to $\sim 10^6\,M_\odot$. We also discuss multiple-generation star formation that may be intrinsically correlated with remnant BH accretion.

</details>

### [10] [Star formation in the circumgalactic high-velocity cloud Complex H](https://arxiv.org/abs/2603.10607)
*Zhihong He,Wenkang Pang,Kun Wang,Yangping Luo,Qian Cui*

**TL;DR**: 在银河系高速云复合体H（HVC Complex H）中发现了一对年龄11.2±0.6 Myr、金属丰度0.05(+0.05-0.02) Zsun的疏散星团，首次为高速云提供了13.8±0.6 kpc的恒星距离锚定，揭示了其沿外盘南向北顺行轨道运动时因气体减速合并形成"慢-快-慢"速度梯度，并证实星团由云内碰撞触发形成，表明高速云可通过短暂（<20 Myr）恒星形成事件为星系吸积贫金属气体提供直接证据。


<details>
  <summary>Details</summary>

Background: 研究探讨了贫金属气体吸积（metal-poor gas accretion）对维持星系恒星形成的作用，重点关注银河系内高速云（HVCs）作为该过程燃料的基本性质及其与星系盘的相互作用。

Data: data部分：研究数据来源于对高速云复合体H（HVC Complex H）中一对疏散双星团的观测，包括其年龄（11.2±0.6 Myr）、金属丰度（0.05(+0.05-0.02) Zsun）、距离（13.8±0.6 kpc）和自行运动信息。

Method: 采用恒星示踪法（stellar tracers）测定高速云（HVCs）的距离和运动学参数，结合轨道积分（orbit integration）分析云团碰撞触发恒星形成的机制。

Result: 在高速云复合体H（HVC Complex H）中发现了一对年龄11.2±0.6 Myr、金属丰度0.05(+0.05-0.02) Zsun的疏散双星团，为云团提供了13.8±0.6 kpc的直接恒星距离锚定。

Abstract: The accretion of metal-poor gas sustains galactic star formation. In the Milky Way, this process is fueled by high-velocity clouds (HVCs), yet their fundamental properties have remained elusive in the absence of stellar tracers. Here we report a binary open cluster within HVC Complex H. With an age of 11.2 +- 0.6 Myr and a subsolar metallicity of 0.05(+0.05-0.02) Zsun, the clusters provide a direct stellar distance anchor to the cloud at 13.8 +- 0.6 kpc. Their proper motions indicate Complex H is on a prograde, south-to-north orbit through the outer Galactic disk. The resulting interaction produces a 'slow-fast-slow' velocity gradient, with the cloud's outer layers decelerating as they merge into the disk. Orbit integration suggests the clusters formed from an internal cloud-cloud collision. This triggering mechanism implies other HVCs could similarly produce high-velocity stars. The scarcity of previous stellar detections in HVCs is explained by the rapid escape of young stars (< 20 Myr), while CO non-detections may stem from weak emission due to low metallicity and gas dispersal. This work reveals that the circumgalactic medium can sustain star formation, offering a tangible laboratory to probe the physical conditions of accreting gas before it merges with the Galactic disk.

</details>

### [11] [Spectroscopic galaxy redshifts in the Peanut cluster - a massive nearly head-on cluster merger shortly after pericenter passage](https://arxiv.org/abs/2603.10849)
*I. Zaznobin,N. Lyskova,I. Bikmaev,R. Burenin,A. Arshinova,E. Churazov,S. Dodonov,M. Gilfanov,I. Khabibullin,I. Khamitov,S. Kotov,A. Moiseev,S. Sazonov,R. Sunyaev,M. Suslikov,R. Uklein*

**TL;DR**: 研究发现花生星系团（Peanut cluster）可能是一个由两个速度差约2000 km/s的子结构组成的罕见大质量并合系统（类似子弹星系团），或单个速度弥散达1455 km/s、总质量2×10¹⁵M⊙的极端大质量星系团。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索花生星系团（Peanut cluster）作为罕见大质量并合体的候选者，其科学背景涉及宇宙中大质量星系团的形成与演化机制，特别是与子弹星系团（Bullet cluster）等极端系统的类比。

Data: 数据来源于2024年10月至2025年1月期间使用俄罗斯科学院特殊天体物理台(SAO RAS)的6米BTA望远镜(Big Telescope Alt-azimuthal)获取的26个新红移测量数据，以及已有的5个红移数据。

Method: 采用光学识别和光谱红移测量技术，结合BTA望远镜观测数据，通过视线速度分布分析和Dressler-Schectman检验评估子结构存在可能性。

Result: 花生星系团（Peanut cluster）可能是由两个视线速度差约2000 km/s的子团组成的罕见大质量并合系统，但单一团的正态速度分布假设也无法排除，其总质量估计为$M_{200} = 2 \times 10^{15} M_\odot$，与子弹星系团（Bullet cluster）或胖子星系团（El Gordo）等极端系统相当。

Abstract: The Peanut cluster (SRGe J023820.8+200556, SRGe CL0238.3+2005, $z_{spec}$ = 0.42) has recently emerged as a candidate for a rare, massive merger, potentially analogous to the Bullet cluster. We present the results of optical identification and spectroscopic redshift measurements for 31 galaxies in the Peanut cluster, including 26 new redshifts obtained with the 6-m telescope BTA (Big Telescope Alt-azimuthal) at SAO RAS between October 2024 and January 2025. The derived distribution of line-of-sight velocities reveals the possible presence of two subclusters with a line-of-sight velocity difference of ~2000 km/s. However, statistical tests and the Dressler-Schectman test show that the hypothesis that the observed velocity distribution can be described by a normal distribution for a single cluster cannot be ruled out, and the evidence for the existence of two gravitationally bound substructures remains ambiguous. Assuming a single cluster with the normal velocity distribution, the estimated galaxy velocity dispersion is $σ_{los} = 1455 \pm 83$ km/s, corresponding to the total cluster mass of $M_{200} = 2 \times 10^{15} M_\odot$ based on the mass-velocity dispersion scaling relation. In either scenario -- a single extremely massive cluster or an ongoing merger -- the Peanut cluster appears to be a very rare and peculiar object, comparable to such extreme systems as the Bullet cluster (1E 0657-56) or El Gordo (ACT-CL J0102-4915).

</details>

<div id='astro-ph.CO'></div>

# astro-ph.CO [[Back]](#toc)

### [12] [Steeling Weak Lensing Source Galaxy Samples against Systematics using Wide Field Spectroscopy](https://arxiv.org/abs/2603.10113)
*Joseph DeRose,Noah Weaverdyck,Martin White,Shi-Fan Chen,David Schlegel,Anže Slosar*

**TL;DR**: 该论文通过结合弱引力透镜（weak galaxy lensing）和星系聚集（galaxy clustering）的3×2点分析，发现在重子物理（baryonic physics）现有不确定性水平下，弱透镜分析信息在准线性尺度达到饱和，允许使用密度低至5 arcmin⁻²的源星系样本而不损失约束力，前提是红移分布能校准至σ(⟨z⟩)=0.005精度，并证明在足够窄的透镜和源红移分布下，内禀排列（intrinsic alignment）信号可被自校准。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙学参数约束中弱引力透镜（weak galaxy lensing）与星系成团性（galaxy clustering）联合探测（$3\times2$-point analyses）的系统误差限制问题，涉及红移不确定性、星系固有排列（intrinsic alignments）、星系偏差（galaxy bias）及重子物理（baryonic physics）的模型灵活性。

Data: 数据来源包括弱引力透镜（weak galaxy lensing）和星系成团性（galaxy clustering）的观测数据，以及基于拉格朗日扰动理论（Lagrangian perturbation theory）和混合有效场论（hybrid effective field theory）模型的模拟数据。

Method: 采用拉格朗日扰动理论(Lagrangian perturbation theory)和混合有效场论(hybrid effective field theory)模型分析星系固有排列(intrinsic alignments)、星系偏差(galaxy bias)和重子物理(baryonic physics)，并结合弱引力透镜(weak lensing)与星系成团性(galaxy clustering)的$3\times2$点相关分析。

Result: 在存在当前重子物理(baryonic physics)不确定性水平的情况下，弱引力透镜(weak lensing)分析的信息含量在准线性尺度上达到饱和，允许使用显著更低密度的源星系样本（如5 arcmin⁻²），前提是红移分布能以σ(⟨z⟩)=0.005的精度校准。

Abstract: We investigate the cosmological constraining power of combined weak galaxy lensing and galaxy clustering probes, i.e. $3\times2$-point analyses, assuming flexible models for redshift uncertainty, and Lagrangian perturbation theory and hybrid effective field theory models for galaxy intrinsic alignments, galaxy bias and baryonic physics. In this context, we provide a detailed accounting of the limiting systematics on $3\times2$-point analyses. Our main finding is that in the presence of current levels of uncertainty on baryonic physics, the information content of weak lensing analyses saturates on quasi-linear scales, allowing the use of source galaxy samples that are significantly less dense, e.g. with number densities of $5\rm \, arcmin^{-2}$, without sacrificing constraining power, provided that redshift distributions can be calibrated at the $σ(\langle z\rangle)=0.005$ level. We show that for sufficiently narrow lens and source redshift distributions, intrinsic alignment contributions can be largely self-calibrated, though sufficient flexibility must be given to the redshift and scale dependence of this signal. The near optimality of such relatively sparse source galaxy samples opens the possibility to directly calibrate the redshift distributions and intrinsic alignment contamination of such a sample using a spectroscopic instrument like DESI, thus mitigating the dominant systematics in weak lensing analyses.

</details>

### [13] [Thermal enhancement of inflationary magnetic fields](https://arxiv.org/abs/2603.10157)
*Arjun Berera,Suddhasattwa Brahma,Zizang Qiu,Rudnei O. Ramos*

**TL;DR**: 研究发现，在暴胀期间将规范场（gauge field）置于热态（thermal state）而非标准邦奇-戴维斯真空（Bunch-Davies vacuum）可通过耗散增强（dissipative boost）将磁能密度标度从a^{-4}改为a^{-3}，使现今星系尺度磁场（B_0）增强至10^8-10^16倍，表明热初始条件能显著缓解共形阻碍（conformal obstruction），为暴胀磁生成（inflationary magnetogenesis）提供无需非最小耦合或非线性电动力学的可行路径。


<details>
  <summary>Details</summary>

Background: 研究探讨了暴胀期间规范场（gauge field）处于热态（thermal state）而非标准邦奇-戴维斯真空（Bunch-Davies vacuum）对原初磁生成（primordial magnetogenesis）的影响，重点分析了热温度（$\mathcal{T}$）破坏共形不变性（conformal invariance）但保留标准麦克斯韦作用量（Maxwell action）的机制。

Data: 研究中假设规范场（gauge field）在暴胀期间处于热态（thermal state）而非标准邦奇-戴维斯真空（Bunch-Davies vacuum），通过引入温度$\mathcal{T}$这一物理尺度在状态层面打破共形不变性（conformal invariance），同时保留标准麦克斯韦作用量（Maxwell action）。

Method: 采用热态（thermal state）初始条件替代邦奇-戴维斯真空（Bunch-Davies vacuum），通过引入温度标度（$\mathcal{T}$）破坏共形不变性，利用标准麦克斯韦作用量实现磁能密度从$a^{-4}$到$a^{-3}$的耗散性增强（dissipative boost）标度变换。

Result: 通过假设规范场（gauge field）在暴胀期间处于热态而非标准邦奇-戴维斯真空态，热温度$\mathcal{T}$引入的物理尺度在保持标准麦克斯韦作用量的同时打破了共形不变性，导致磁能密度标度从$a^{-4}$变为$a^{-3}$，从而显著增强了现今星系尺度上的磁场强度$B_0$（潜在增强范围约$10^{8}$至$10^{16}$）。

Abstract: We investigate primordial magnetogenesis by assuming the gauge field is prepared in a thermal state during inflation rather than the standard Bunch-Davies vacuum. The temperature $\mathcal{T}$ introduces a physical scale that breaks conformal invariance at the level of the state while preserving the standard Maxwell action. This modification results in a {\it dissipative boost} that alters the magnetic energy density scaling from $a^{-4}$ to $a^{-3}$, resulting in a present-day magnetic field $B_0$ enhancement that can potentially range from about $10^{8}$ to $10^{16}$ on galactic scales. While this toy model alone does not satisfy observational lower bounds, it demonstrates that thermal initial conditions can significantly mitigate the conformal obstruction. Our results suggest that embedding this mechanism within a fully dynamical warm inflation framework, where dissipation continuously maintains the thermal bath, provides a highly promising path towards successfully realizing a minimal model of inflationary magnetogenesis without the need to invoke non-minimal couplings, anomalous background dynamics or nonlinear extensions of electrodynamics.

</details>

### [14] [A modern halo streaming model for redshift space distortions](https://arxiv.org/abs/2603.10179)
*Cheng-Zong Ruan,Baojiu Li,Carlton M. Baugh,Sownak Bose,Alexander Eggemeier,David F. Mota*

**TL;DR**: 提出了一种精确且物理可解释的晕族（halo）流动模型，通过结合流动模型和晕模型分解方法，并利用N体模拟训练专用仿真器，实现了对高度非线性尺度上红移空间畸变（RSD）信号的准确建模，支持DESI和Euclid等巡天的全形状RSD分析。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升红移空间畸变（RSD）建模精度以支持宇宙大尺度结构巡天（如DESI和Euclid）的宇宙学参数推断和引力理论检验。

Data: data部分：研究使用了一套N体模拟（N-body simulations）数据训练专用仿真器，包括晕质量函数（halo mass functions）、真实空间晕两点相关函数（real-space halo two-point correlation functions）和成对速度矩（pairwise velocity moments）。

Method: 采用晕族（halo）流模型（streaming model）结合晕族模型（halo model）分解星系成团性（galaxy clustering）为中心/卫星（central/satellite）和一晕/二晕（one-/two-halo）贡献，并基于N体模拟（N-body simulations）训练关键物理成分（质量函数、实空间两点相关函数、成对速度矩）的专用仿真器（emulators）。

Result: 提出了一种精确且物理可解释的晕族（halo）流模型，通过结合流模型与晕族分解方法（central/satellite和one-/two-halo贡献），并基于N体模拟训练专用仿真器，实现了对高度非线性尺度上红移空间各向异性聚类信号的准确建模。

Abstract: Accurate modelling of redshift-space distortions (RSD) in galaxy clustering is essential for extracting cosmological information from current and forthcoming large-scale structure surveys. While perturbation theory is reliable on large scales, much of the constraining power lies at intermediate and small separations, where nonlinear dynamics within and between dark matter haloes dominate. We present a halo streaming model for nonlinear galaxy clustering in redshift space that is accurate and physically interpretable. Our framework combines the streaming model for RSD with a halo-model decomposition of the galaxy clustering into central/satellite and one-/two-halo contributions. We build dedicated emulators for the key physical ingredients, trained on a suite of $N$-body simulations: halo mass functions, real-space halo two-point correlation functions, and pairwise velocity moments. By emulating these modular building blocks rather than the final redshift-space observable, this approach preserves physical transparency, enables targeted optimisation for each ingredient, and remains flexible to changes in tracer populations and galaxy-halo connection models. The resulting halo streaming model reproduces the simulated nonlinear anisotropic clustering signal down to highly nonlinear scales, while achieving the computational efficiency required for cosmological parameter inference. This framework is designed to support full-shape RSD analyses for surveys such as DESI and \textit{Euclid}, facilitating precision measurements of structure growth and tests of gravity. All codes and trained emulators are publicly available in the \href{https://github.com/chzruan/freyja}{\texttt{freyja}} repository.

</details>

### [15] [Cosmological simulation of radio synchrotron bridge between pre-merging galaxy clusters](https://arxiv.org/abs/2603.10187)
*Kosuke Nishiwaki,Gianfranco Brunetti,Franco Vazza,Claudio Gheller*

**TL;DR**: 通过改进的宇宙磁流体动力学(MHD)模拟和福克-普朗克(FP)方程并行计算，证实了早期合并阶段星系团间湍流对宇宙线(CRs)的再加速是产生Mpc尺度射电桥(radio bridges)的可行机制，其模拟结果与Abell 399和Abell 401间观测到的射电桥特性相符。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系团合并过程中宇宙射线（CRs）和磁场通过螺线管湍流再加速机制在宇宙网中产生可观测同步辐射（radio bridges）的物理条件。

Data: 使用Enzo代码进行宇宙学磁流体力学(MHD)模拟，并改进运行时拉格朗日示踪方法，跟踪N=O(10^7)个示踪粒子的重子物质轨迹。

Method: 采用改进的运行时拉格朗日示踪法(Lagrangian tracer method)结合宇宙磁流体力学(MHD)模拟，并行求解福克-普朗克(Fokker-Planck)方程以研究湍流对宇宙线(CRs)的再加速效应。

Result: 在团簇合并早期阶段，湍流对宇宙线（CRs）的再加速机制能够产生符合观测特征的Mpc尺度射电桥（radio bridge），其光谱形态、强度分布及射电-X射线像素相关性均与Abell 399和Abell 401间的实测射电桥一致。

Abstract: Radio bridges are diffuse synchrotron emission observed between merging galaxy clusters. Recent radio observations have reported both detections and non-detections of radio bridges between clusters. The detections imply the presence of cosmic rays (CRs) and magnetic fields permeating the cosmic web that produce synchrotron emission observable with current facilities, whereas the non-detections suggest that specific physical conditions are required for their formation. We study the CR reacceleration by solenoidal turbulence in the filament connecting two massive clusters at an early stage of the merger. Our aim is to test whether this mechanism can generate diffuse emission in the inter-cluster region. We perform a cosmological magneto-hydrodynamical (MHD) simulation using the Enzo code. We improved a run-time Lagrangian tracer method implemented in Enzo, and follow the trajectories of baryonic matter using $N=\mathcal{O}(10^7)$ tracer particles. In post-processing, we conduct a parallel computation of the Fokker-Planck (FP) equation for all tracers, with cooling and reacceleration efficiencies evaluated from the local quantities recorded along each tracer trajectory. Our simulation generate a Mpc-sized radio bridge in the early stage of the cluster merger. Within a reasonable parameter range, the reacceleration model produces a broad variety of spectra. In our fiducial model, the simulated bridge matches several properties of the one found between Abell 399 and Abell 401, such as its spectral shape, intensity profile, and pixel-by-pixel correlation between radio and X-ray intensities. The inter-cluster region is filled with turbulence induced by infalling mass clumps and subsequently amplified by the approaching motion of the clusters. The CR reacceleration by the turbulence is a viable mechanism to power a Mpc-sized synchrotron emission observed as radio bridges.

</details>

### [16] [New classification method for the dynamical state of galaxy clusters with a Gaussian mixture model](https://arxiv.org/abs/2603.10319)
*Hyowon Kim,Marco Canducci,Rory Smith,Peter Tino,Yara Jaffe,Ho Seong Hwang,Jihye Shin,Kyungwon Chun*

**TL;DR**: 开发了一种基于贝叶斯分类器和高斯混合分布模型的星系团（galaxy cluster）动力学状态分类方法，利用更多指标（如幅度差、中心偏移等）和投影分类器提升了分类性能，并提供了结果可靠性评估。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系团（Galaxy clusters）在等级式ΛCDM宇宙中的动力学状态分类方法改进，以克服传统二分法分类、指标数量限制及观测与模拟方法不兼容等局限性。

Data: 数据来源为N-cluster Run模拟数据（N-cluster Run simulation data）。

Method: 采用贝叶斯分类器（Bayesian classifier）结合类条件高斯混合分布模型（class-conditional Gaussian mixture distribution model）分析N-cluster Run模拟数据，设计了两阶段（并合与弛豫）和三阶段（近期并合、古老并合与弛豫）分类方案。

Result: 基于贝叶斯分类器和高斯混合分布模型开发了适用于观测的星系团动力学状态分类新方法，能更详细解析合并过程并提升分类可靠性。

Abstract: Galaxy clusters are the largest gravitationally bound systems, and they continue their growth through mergers in a hierarchical ΛCDM Universe. Therefore, we can describe the merger stage of a cluster as the dynamical state of clusters. Previous studies have investigated this phenomenon, but several limitations remain, including reliance on dichotomous classifications, constraints on the number of indicators used, absence of reliability, and incompatibility of methods between observation and simulation studies. To overcome this, we developed an enhanced and observation-applicable cluster dynamical state classification method using the Bayesian classifier with the class-conditional Gaussian mixture distribution model using the N-cluster Run simulation data. The Bayesian classifier was designed for two merger stages (merger and relaxed) as well as three merger stages (recent merger, ancient merger, and relaxed) to provide a more detailed interpretation of the merger processes. In the results, using a larger number of indicators yields better results, with their order of importance being: magnitude difference, center offset, sparsity, Kuiper V statistic, and mirror asymmetry. Additionally, our analyses show that a projected classifier (built on the 6D space, but evaluated on lower dimensional projections) consistently produces better outcomes than non-projected classifiers (i.e., classifiers built directly on the corresponding low dimensional spaces), which means limited observation data can be used to classify with enhanced performance. Furthermore, the new classification method outperforms our previous research. This new method can suggest a way of overcoming previous limitations and provides new insights by providing the reliability of dynamical state classification results.

</details>

### [17] [Interacting dark sector from intrinsic entropy couplings](https://arxiv.org/abs/2603.10622)
*Erik Jensko,Elsa M. Teixeira,Vivian Poulin*

**TL;DR**: 提出了一类新型相互作用暗区（dark sector）模型，通过将暗物质（dark matter）内禀熵与标量场暗能量（dark energy）耦合，在保持背景宇宙学与ΛCDM或非耦合精质（quintessence）模型一致的同时，在线性扰动层面产生纯动量交换的尺度依赖性结构增长抑制或增强效应。


<details>
  <summary>Details</summary>

Background: 研究提出了一种新型相互作用暗区模型，将暗物质（dark matter）的内禀熵与标量场暗能量（dark energy）耦合，为探索标准宇宙学模型之外的暗区相互作用提供了理论框架。

Data: 数据来源于理论构建的拉格朗日形式（Lagrangian formulation）下的相对论性完美流体（relativistic perfect fluids）模型，通过代数熵耦合（algebraic entropy couplings）和导数熵耦合（derivative entropy couplings）实现暗物质（dark matter）与标量场暗能量（scalar field dark energy）的相互作用。

Method: 采用拉格朗日（Lagrangian）形式描述相对论性完美流体，构建包含代数熵耦合和导数熵耦合的协变作用量（covariant actions）。

Result: 引入了一类新的暗物质（dark matter）与标量场暗能量（dark energy）通过内禀熵（intrinsic entropy）耦合的相互作用模型。

Abstract: We introduce a new class of interacting dark sector models that couple the intrinsic entropy of dark matter to scalar field dark energy. Using the Lagrangian formulation for relativistic perfect fluids, we construct consistent covariant actions that incorporate algebraic and derivative entropy couplings. These interactions leave the expansion history unchanged, rendering the background cosmology indistinguishable from $Λ$CDM or uncoupled quintessence. At the level of cosmological perturbations, the entropy couplings generate scale-dependent modifications to the dark matter Euler equation, while the continuity equation remains unaltered at linear order. The resulting interactions correspond to a pure-momentum exchange within the dark sector. We show that intrinsic entropy perturbations can carry primordial scale dependence, and non-minimal couplings can lead to a scale-dependent suppression or enhancement of structure growth. Finally, we demonstrate that these models are generically compatible with current Cosmic Microwave Background observations, while inducing distinctive signatures in large-scale structure. The framework provides a theoretically well-motivated and observationally viable extension to the standard cosmological model, opening new directions to explore novel interactions in the dark sector.

</details>

### [18] [Multi-scale weak lensing detection of galaxy clusters with source redshift tomography](https://arxiv.org/abs/2603.10636)
*L. Chappuis,S. Pires,G. W. Pratt,G. Leroy,A. Daurelle,C. Giocoli,C. Carbone*

**TL;DR**: 研究通过引入$z_{s,\mathrm{min}}$-cut技术分析多红移分区间弱引力透镜（weak lensing）信号叠加效果，发现单一红移分区间（$z_{s,\mathrm{min}}=0.4$）的检测效率与多区间组合相当，且红移分区间叠加会因伪检测累积降低纯度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用弱引力透镜(weak lensing)信号探测星系团(galaxy clusters)，并探索通过引入源星系(source galaxies)的层析(tomographic)信息提升探测效率的潜力。

Data: 研究使用了模拟数据集（mock data sets），包括从孤立纳维-弗伦克-怀特（Navarro Frenk White）晕到嵌入并形成于N体宇宙学模拟（N-body cosmological simulations）中的晕，同时考虑了真实和光度测量（photometric）源红移数据。

Method: 采用小波多尺度检测方法（wavelet multi-scale detection method）结合源星系层析（tomographic）红移分区的$z_{s,\mathrm{min}}$-cut技术，通过逐步增加最小源红移$z_{s,\mathrm{min}}$构建重叠的层析红移分区，并分析不同红移分区组合对弱引力透镜（weak lensing）信号检测效率的影响。

Result: 利用$z_{s,\mathrm{min}}=0.4$的单一红移箱进行弱透镜峰值检测时，其效果与多红移箱组合相当，且主要限制因素在于跨红移箱的虚假检测累积导致固定检测阈值下纯度下降。

Abstract: Recently, a number of methods have emerged to detect galaxy clusters solely through their weak lensing signal. Using the recently-introduced wavelet multi-scale detection method, we focus here on the potential for the use of tomographic information of the source galaxies to increase the number of weak lensing detections. We apply the $z_{s,\mathrm{min}}$-cut technique, consisting of the combination of weak lensing peak detections emerging from lensing maps obtained using different source redshift bins, to mock data sets of progressively increasing sophistication. The source redshift distribution is chosen to be $Euclid$-like, with a maximum depth of $z_{s,\mathrm{max}}=3$, and overlapping tomographic redshift bins are constructed by progressively increasing the minimum source redshift $z_{s,\mathrm{min}}$. Considering all possible detection combinations from one to four tomographic bins, we find that a single source redshift bin, with $z_{s,\mathrm{min}}=0.4$, performs as well as the combination of multiple redshift bins. By running detections on synthetic clusters of varying complexity -- from isolated Navarro Frenk White haloes to haloes embedded in and formed within N-body cosmological simulations, and considering both true and photometric source redshifts -- we show that while large-scale structure contamination and photometric redshift errors reduce the potential gains of the tomographic approach, the dominant limitation is the accumulation of spurious detections across redshift bins, leading to decreased purity at a fixed detection threshold.

</details>

### [19] [Euclid: The linear-construction covariance and cosmology](https://arxiv.org/abs/2603.10735)
*V. Lindholm,E. Sihvola,J. Valiviita,A. Fumagalli,B. Altieri,S. Andreon,N. Auricchio,C. Baccigalupi,M. Baldi,S. Bardelli,P. Battaglia,A. Biviano,E. Branchini,M. Brescia,S. Camera,V. Capobianco,C. Carbone,V. F. Cardone,J. Carretero,S. Casas,M. Castellano,G. Castignani,S. Cavuoti,K. C. Chambers,A. Cimatti,C. Colodro-Conde,G. Congedo,L. Conversi,Y. Copin,F. Courbin,H. M. Courtois,A. Da Silva,H. Degaudenzi,G. De Lucia,H. Dole,F. Dubath,X. Dupac,S. Dusini,S. Escoffier,M. Farina,R. Farinelli,S. Ferriol,F. Finelli,P. Fosalba,S. Fotopoulou,M. Frailis,E. Franceschi,M. Fumana,S. Galeotta,K. George,B. Gillis,C. Giocoli,J. Gracia-Carpio,A. Grazian,F. Grupp,S. V. H. Haugan,W. Holmes,F. Hormuth,A. Hornstrup,K. Jahnke,M. Jhabvala,S. Kermiche,A. Kiessling,B. Kubik,M. Kunz,H. Kurki-Suonio,A. M. C. Le Brun,S. Ligori,P. B. Lilje,I. Lloro,G. Mainetti,E. Maiorano,O. Mansutti,S. Marcin,O. Marggraf,M. Martinelli,N. Martinet,F. Marulli,R. J. Massey,E. Medinaceli,S. Mei,M. Melchior,M. Meneghetti,E. Merlin,G. Meylan,A. Mora,M. Moresco,L. Moscardini,R. Nakajima,C. Neissner,S. -M. Niemi,C. Padilla,S. Paltani,F. Pasian,K. Pedersen,V. Pettorino,S. Pires,G. Polenta,M. Poncet,L. A. Popa,F. Raison,A. Renzi,J. Rhodes,G. Riccio,E. Romelli,M. Roncarelli,C. Rosset,R. Saglia,Z. Sakr,A. G. Sánchez,D. Sapone,P. Schneider,T. Schrabback,A. Secroun,G. Seidel,P. Simon,C. Sirignano,G. Sirri,L. Stanco,P. Tallada-Crespí,A. N. Taylor,I. Tereno,S. Toft,R. Toledo-Moreo,F. Torradeflot,I. Tutusaus,T. Vassallo,G. Verdoes Kleijn,Y. Wang,J. Weller,G. Zamorani,E. Zucca,T. Castro,J. Martín-Fleitas,P. Monaco,A. Pezzotta,V. Scottez,M. Sereno,M. Viel,D. Sciotti*

**TL;DR**: 通过对比线性构造法（LC）与样本协方差法在星系团两点相关函数协方差矩阵估计中的表现，发现两者在宇宙学参数（物质密度参数Ωm和物质密度涨落幅度σ8）估计中具有高度一致的后验分布和精度，验证了LC方法在计算效率提升20倍的同时保持相同准确性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于评估线性构造法（LC）在星系团两点相关函数协方差矩阵计算中的性能及其对宇宙学参数（物质密度参数Ω_m和物质密度涨落幅度σ_8）估计的影响。

Data: 使用了1000个模拟暗物质晕目录(dark matter halo catalogues)在四个红移壳层(redshift shells)中计算线性构造(LC)协方差和样本协方差估计，这些目录基于空间平坦的ΛCDM宇宙学模型模拟，参数设置为Ωm=0.30711和σ8=0.8288。

Method: 采用线性构造法（LC）和样本协方差法计算星系团两点相关函数协方差矩阵，并比较二者在宇宙学参数估计中的性能。

Result: 线性构造（LC）方法估计的星系团两点相关函数协方差矩阵在宇宙学参数估计中与样本协方差方法表现一致，所得物质密度参数（$Ω_{\rm m}$）和物质密度涨落幅度（$σ_8$）的后验分布与模拟宇宙学吻合，且两者差异小于$0.16\,σ$。

Abstract: We study the properties of galaxy cluster 2-point correlation function covariance matrices estimated using the linear-construction (LC) method, which is computationally up to 20 times faster than the standard sample-covariance method. Our goal is to assess how well the LC method performs in cosmological parameter estimation compared to the sample covariance. We use a set of 1000 mock dark matter halo catalogues to compute both the LC-covariance and the sample-covariance estimates in four redshift shells. These numerical matrices are used to fit a theoretical four-parameter model for the covariance. We then use the two fitted covariance models in a likelihood function to estimate two cosmological parameters - the matter density parameter $Ω_{\rm m}$ and the amplitude of the matter density fluctuations $σ_8$ - from the simulated mock catalogues. The purpose of this is to validate the LC-covariance-based model against the sample-covariance model. The catalogues were simulated assuming the spatially flat $Λ$CDM cosmology, with $Ω_{\rm m} = 0.30711$ and $σ_8=0.8288$. We find that the parameter posteriors obtained using the sample- and LC-covariance models agree well with each other and with the simulation cosmology. The two pairs of marginalized constraints are $Ω_{\rm m} = 0.307 \pm 0.003$ and $σ_8 = 0.826\pm 0.009$ (sample covariance), and $Ω_{\rm m} = 0.308 \pm 0.003$ and $σ_8 = 0.825 \pm 0.009$ (LC covariance). The posterior widths are the same, and the difference in the median values is less than $0.16\,σ$ for both parameters.

</details>

### [20] [Measuring neutrino mass in light of ACT DR6 and DESI DR2](https://arxiv.org/abs/2603.10787)
*Lu Feng,Tian-Nuo Li,Guo-Hong Du,Jing-Fei Zhang,Xin Zhang*

**TL;DR**: 基于ACT DR6、DESI DR2和DESY5最新数据，研究发现中微子总质量(∑mν)上限受暗能量状态方程演化行为显著影响，其中全息暗能量(HDE)模型约束最严格而w0waCDM模型约束最宽松，且在所有模型中倒置层级(inverted hierarchy)约束最弱而简并层级(degenerate hierarchy)约束最强。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用最新宇宙学数据（如ACT的小尺度宇宙微波背景辐射(CMB)和DESI的重子声波振荡(BAO)数据）精确测量中微子质量总和(∑mν)及其质量等级结构，并探讨不同暗能量模型（如ΛCDM、wCDM、全息暗能量(HDE)和w0waCDM）对中微子质量约束的影响。

Data: 数据来源包括ACT DR6（宇宙微波背景辐射）、DESI DR2（重子声波振荡）和DESY5数据集。

Method: 采用马尔可夫链蒙特卡洛（MCMC）方法结合最新ACT DR6、DESI DR2和DESY5数据集，在ΛCDM、wCDM、全息暗能量（HDE）和w0waCDM模型中约束中微子总质量（∑mν）及其质量等级结构。

Result: 中微子质量总和（$\sum m_ν$）的上限受暗能量状态方程演化行为的显著影响，其中早期精质（quintessence）特征的模型（如全息暗能量HDE）约束最严格，而允许早期幻影（phantom）行为的模型（如$w_0w_a$CDM）约束显著放宽。

Abstract: The recent release of high-precision cosmological data, particularly the small-scale cosmic microwave background (CMB) measurements from ACT and baryon acoustic oscillation (BAO) data from DESI, has opened a new landscape for probing the neutrino mass. In this work, we present updated constraints on the total neutrino mass, $\sum m_ν$, and its hierarchy within the $Λ$CDM, $w$CDM, holographic dark energy (HDE), and $w_0w_a$CDM models, using the latest ACT DR6, DESI DR2, and DESY5 datasets. We find that the upper limits on $\sum m_ν$ are critically governed by the evolutionary behavior of the dark energy equation of state. Specifically, models exhibiting early-time quintessence features (e.g., HDE) yield the most stringent constraints, whereas those allowing for early-time phantom behavior (e.g., $w_0w_a$CDM) result in significantly looser bounds. Despite these model-dependent variations, we observe a robust hierarchy dependence across all scenarios, where the inverted hierarchy consistently yields weaker constraints and the degenerate hierarchy consistently yields tightest constraints. Our analysis demonstrates that the improved small-scale CMB information from ACT, combined with high-precision BAO data, systematically tightens the limits on $\sum m_ν$, providing a crucial benchmark for future neutrino mass measurement.

</details>

### [21] [Blind mitigation of foreground-induced biases on primordial $B$ modes for ground-based CMB experiments](https://arxiv.org/abs/2603.11026)
*Aliza Mustafa,Alessandro Carones,Nicoletta Krachmalnicoff,Marina Migliaccio,Carlo Baccigalupi*

**TL;DR**: 论文提出了两种改进的Needlet内部线性组合（NILC）方法，通过成分分离步骤中的前景矩去投影（deprojection）和基于数据驱动模板的似然水平边缘化，有效抑制了宇宙微波背景（CMB）B模偏振测量中银河系前景残留对张标比（tensor-to-scalar ratio）$r$的偏差影响，在SO-SAT模拟中实现了无偏估计和透镜B模振幅的准确重建。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过宇宙微波背景（CMB）B模偏振观测探测暴胀（inflationary）物理，并解决银河系弥散前景（diffuse Galactic foregrounds）对张量标量比（tensor-to-scalar ratio）$r$测量偏差的挑战。

Data: 数据来源于西蒙斯天文台小孔径望远镜（Simons Observatory Small Aperture Telescope, SO-SAT）的模拟数据。

Method: 该研究采用的核心分析方法包括在成分分离步骤中直接去除选定前景矩（deprojection）的方法，以及利用数据驱动模板在似然水平上对残余前景功率进行边缘化（marginalisation）的技术。

Result: 提出了两种扩展的NILC（Needlet Internal Linear Combination）方法，有效降低前景污染对张标比（tensor-to-scalar ratio, r）估计的偏差。

Abstract: Observations of the Cosmic Microwave Background (CMB) B-mode polarisation provide a unique probe of inflationary physics. Extracting a reliable constraint on the tensor-to-scalar ratio $r$ nonetheless demands stringent suppression of diffuse Galactic foregrounds, whose residuals can bias the inferred signal. This work introduces and evaluates two extensions of the Needlet Internal Linear Combination (NILC) framework aimed at reducing foreground-induced biases on $r$. The first extension implements the deprojection of selected foreground moments directly within the component-separation step. The second performs a likelihood-level marginalisation over residual foreground power using a data-driven template. Using Simons Observatory Small Aperture Telescope (SO-SAT) - like simulations, we show that both methods effectively control residual contamination, yielding unbiased estimates of $r$ and a consistent reconstruction of the lensing B-mode amplitude. These results indicate that enhanced foreground-mitigation strategies will be useful for next-generation CMB polarisation analyses seeking a robust detection of primordial B-modes.

</details>

<div id='astro-ph.IM'></div>

# astro-ph.IM [[Back]](#toc)

### [22] [CHRONOS Science Program](https://arxiv.org/abs/2603.10070)
*Yuki Inoue,Mario Juvenal S Onglao,Vivek Kumar,Daiki Tanabe*

**TL;DR**: 提出CHRONOS（低温亚赫兹量子无损速度计扭杆探测器）作为下一代地基引力波观测站，利用低温扭杆干涉仪配置和量子无损速度计读数技术，填补亚赫兹频段探测空白，实现双星并合前长期追踪、随机引力波背景（SGWB）探测及宇宙学频谱重建。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发下一代地基引力波探测器CHRONOS，旨在填补亚赫兹(sub-Hz)频段的观测空白，连接空间任务与现有高频地基探测器，为引力波天文学开辟新窗口。

Data: 数据来源为基于低温扭杆干涉仪配置的量子非破坏速度计（quantum non-demolition speed-meter）观测数据。

Method: 采用低温扭杆干涉仪配置结合量子非破坏速度计读出技术（quantum non-demolition speed-meter readout）。

Result: CHRONOS（Cryogenic sub-Hz cROss torsion-bar detector with quantum NOn-demolition Speed meter）是一种采用低温扭杆干涉仪配置和量子非破坏速度计读出的下一代地基引力波天文台，旨在以前所未有的灵敏度探索亚赫兹频段。

Abstract: Cryogenic sub-Hz cROss torsion-bar detector with quantum NOn-demolition Speed meter(CHRONOS) is a proposed next-generation ground-based gravitational-wave observatory designed to explore the sub-Hz frequency band with unprecedented sensitivity. Utilizing a cryogenic torsion-bar interferometric configuration with quantum non-demolition speed-meter readout, CHRONOS targets a frequency window that bridges space-based missions and current high-frequency ground-based detectors, opening a new frontier in gravitational-wave astronomy. The observatory will enable long-duration tracking of compact binary inspirals well before merger, significantly improving source localization, parameter estimation, and tests of general relativity. In addition to transient signals, CHRONOS is optimized to probe the stochastic gravitational-wave background (SGWB) in the sub-Hz regime, providing powerful constraints on primordial gravitational waves, inflationary tensor spectra with red or blue tilts, first-order phase transitions, cosmic strings, and other relics of high-energy physics. By connecting gravitational-wave measurements across cosmological frequency scales-from cosmic microwave background polarization to pulsar timing arrays and high-frequency interferometers-CHRONOS will contribute to a coherent reconstruction of the gravitational-wave spectrum over more than twenty orders of magnitude. Crossing critical sensitivity thresholds in the sub-Hz band, CHRONOS will establish a new pillar of gravitational-wave astronomy and cosmology, enabling transformative advances in astrophysics and fundamental physics.

</details>

### [23] [The Cosmological Simulation Code OpenGadget3 -- Implementation of Self-Interacting Dark Matter](https://arxiv.org/abs/2603.10107)
*Moritz S. Fischer,Marc Wiertel,Cenanda Arido,Yashraj Patil,Antonio Ragagnin,Klaus Dolag,Marcus Brüggen,Mathias Garny,Andrew Robertson,Kai Schmidt-Hoberg*

**TL;DR**: 这篇论文的核心贡献是开发并公开了宇宙学流体动力学N体代码OpenGadget3的自相互作用暗物质(SIDM)实现模块，能够精确模拟多种微分截面(包括强各向异性截面)的弹性散射以及双物种模型的跨物种相互作用。


<details>
  <summary>Details</summary>

Background: 暗物质（DM）可能存在非引力自相互作用（self-interaction），这对解决冷暗物质（cold DM）在小尺度上的潜在问题具有重要意义。

Data: 研究中使用的数据来源于基于OpenGadget3代码进行的N体模拟（N-body simulation），该代码能够模拟弹性散射（elastic scattering）以及包含强各向异性截面（strongly anisotropic cross-sections）在内的多种微分截面（differential cross-sections），并支持双物种模型（two-species model）的跨物种相互作用（cross-species interactions）模拟。

Method: 采用宇宙学流体动力学N体代码OpenGadget3模拟自相互作用暗物质(SIDM)的弹性散射过程，支持多种微分截面（differential cross-sections）模型及双物种交叉相互作用。

Result: 在OpenGadget3中实现了自相互作用暗物质（SIDM）的弹性散射模拟，支持多种微分截面（differential cross-sections）和双物种模型（two-species model），并通过测试验证了其准确性。

Abstract: Dark matter (DM) could be subject to non-gravitational self-interactions which is relevant to resolve potential problems of cold DM on small scales. Their impact on astrophysical objects such as galaxies and galaxy clusters allows for constraining the strength of this scattering and eventually further properties of the cross-section. To model self-interacting dark matter (SIDM), N-body simulations are a crucial tool widely employed by the SIDM community. In this paper, we describe the SIDM implementation in the cosmological hydrodynamical N-body code OpenGadget3 and release it to the public. It is capable of simulating elastic scattering for various differential cross-sections, including strongly anisotropic cross-sections. Beyond single-species models, the code also allows simulating a two-species model with cross-species interactions. In addition to describing the numerical schemes for modelling various flavours of SIDM, we discuss the technical challenges of implementing them. Moreover, we demonstrate through several test problems that OpenGadget3 can accurately simulate DM self-interactions. Furthermore, we assess the performance of the code and provide scaling tests. Lastly, we highlight remaining challenges in the context of SIDM and describe directions for improving the current state of the art.

</details>

### [24] [Attaining Spectral Energy Distributions With Sub-Percent Uncertainties: All-Sky DA White Dwarf Spectrophotometric Standard Stars For Large Telescopes And Surveys](https://arxiv.org/abs/2603.10185)
*Abhijit Saha,Edward W. Olszewski,Benjamin M. Boyd,Thomas Matheson,Tim Axelrod,Gautham Narayan,Annalisa Calamida,Jay B. Holberg,Ivan Hubeny,Ralph C. Bohlin,Susana Deustua,Armin Rest,Jenna Claver,Sean Points,Christopher W. Stubbs,Elena Sabbi,John W. Mackenty*

**TL;DR**: 研究建立了32颗暗弱（16.5 ≤ V ≤ 19.8）DA型白矮星（DA white dwarfs）作为全天空分布的光谱光度标准，验证了纯氢辐射模型在近紫外至近红外波段预测宽带流量的千分之几精度，并通过同时拟合视线红化与观测数据证明残余均方根误差仅0.4%，补充了CALSPEC的3颗亮星，构成覆盖全天、亮度匹配大型望远镜动态范围的高精度标准星系统。


<details>
  <summary>Details</summary>

Background: 该研究致力于建立覆盖全天的32颗新发现的微弱DA型白矮星（DA white dwarfs）作为分光光度标准星，以补充现有CALSPEC系统的三颗较亮标准星，提升地面和空间观测的绝对定标精度。

Data: data部分：数据来源于对32颗新发现的暗弱DA白矮星（16.5 ≤ V ≤ 19.8）的光谱能量分布（spectral energy distributions）拟合，结合其光谱观测和多波段测光观测（multi-band photometric observations）。

Method: 采用完全辐射纯氢模型通量（fully radiative pure hydrogen model fluxes）拟合视向红化（line of sight reddenings）与光谱能量分布（spectral energy distributions），结合光谱和多波段测光观测数据进行分析。

Result: result：验证了全辐射纯氢模型通量可准确预测热DA白矮星（hot DA white dwarfs）从近紫外到近红外的宽带通量，残差均方根（rms）通常为0.4%。

Abstract: We present a synopsis of the project to establish thirty-two new faint ($ 16.5 \leq V \leq 19.8 $) DA white dwarfs as spectrophotometric standards distributed over the whole sky. Our results validate the use of fully radiative pure hydrogen model fluxes for hot DA white dwarfs to predict the observed broadband fluxes from near ultraviolet through the near infrared to accuracies of a few parts per thousand. After fitting the line of sight reddenings simultaneously with the model spectral energy distributions of these stars against spectroscopic and multi-band photometric observations, we have shown that residuals have an rms of typically 0.4 percent. This indicates that the complications from interstellar dust extinction have been adequately mitigated. Our stars supplement the three brighter DA white dwarfs that define the flux scale of CALSPEC. The consequent photometric accuracy, their all sky coverage, and their brightness range that matches the dynamic range of large telescopes, constitutes an unprecedented ensemble of standard stars for both ground as well as space based use. This paper targets readers who may wish to use these as standard stars, and provides for them the essential content to understand their strengths and limitations, without traversing the technical details of analysis that are already captured in a series of papers since 2016. The narrative here describes the motivation, justification, and evolution of the analysis methods; the input data that constrain the modeling; as well as the stability of our results in the face of future improvements in models.

</details>

### [25] [Evaluating the spatial intra-pixel sensitivity variations and influence based on space observation](https://arxiv.org/abs/2603.10355)
*Peipei Wang,Zihuang Cao,Chao Liu,Peng Wei,Xin Zhang,Jialu Nie*

**TL;DR**: 提出一种通过最小二乘拟合恒星图像直接计算像素内灵敏度变化(IPSV)的方法，能高精度重建IPSV并提升星象定心精度近30倍，有效消除周期性像素相位误差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于电荷耦合器件(CCD)和互补金属氧化物半导体(CMOS)探测器中的像素内灵敏度变化(IPSVs)对欠采样恒星观测造成的天体测量误差问题。

Data: 使用模拟数据（simulated data）验证了通过最小化理论模型与观测恒星图像之间的通量残差来推断像素内灵敏度变化（IPSVs）的方法。

Method: 采用最小二乘拟合方法最小化理论恒星模型与观测数据间的流量残差，直接反演并验证了像素内灵敏度变化(IPSVs)的计算模型，假设各像素的IPSVs近乎相同。

Result: 提出了一种通过最小化理论模型与观测恒星图像的流量残差来直接反演像素内灵敏度变化(IPSVs)的计算方法，该方法能高精度重建IPSVs并显著提升星象定心精度。

Abstract: Intra-pixel sensitivity variations (IPSVs) in charge-coupled devices (CCDs) and complementary metal-oxide-semiconductor (CMOS) detectors constitute a significant source of astrometric error for undersampled stellar observations. Since laboratory-based IPSV measurements suffer from limited applicability, we propose a computational method to directly infer IPSV from stellar images and validate it with simulated data. By minimizing the flux residuals between theoretical and observed stellar models through least-squares fitting, we can successfully recover the IPSV, which is treated as nearly identical across pixels. Simulations demonstrate that the reconstructed IPSV achieves high accuracy, and the instrumental point spread function (IPSF) restored using this IPSV improves stellar centroiding by nearly 30$\times$, effectively eliminating periodic pixel-phase errors. The method remains robust under different morphologies of IPSV and varying sampling conditions. Additionally, the framework can be extended to an iterative IPSF-IPSV closed-loop scheme that updates both components simultaneously, providing a practical pathway for continuous detector calibration in future space-based astronomical surveys.

</details>

### [26] [CSST-PSFNet: A Point Spread Function Reconstruction Model for the CSST Based on Deep Learning](https://arxiv.org/abs/2603.10424)
*Peipei Wang,Peng Wei,Chao Liu,Rui Wang,Feng Wang,Xin Zhang*

**TL;DR**: 提出了一种结合残差神经网络、轻量Transformer架构和变分潜在表示的深度学习模型CSST-PSFNet，在中国空间站巡天望远镜(CSST)点扩散函数(PSF)重建中实现了优于PSFEx的像素级精度和弱透镜相关形状参数恢复，尺寸和椭圆率残差精度分别低于0.005和0.002。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发深度学习模型（CSST-PSFNet）以解决中国空间站巡天望远镜（CSST）成像中因点扩散函数（PSF）欠采样、波段间变异性和焦平面空间平滑变化带来的关键挑战。

Data: 数据来源于中国空间站巡天望远镜(CSST)主巡天模拟器(CSST Main Survey Simulator)生成的高分辨率恒星-点扩散函数(PSF)对。

Method: 采用深度学习方法CSST-PSFNet，结合残差神经网络、轻量级Transformer架构和变分潜在表示，解决中国空间站巡天望远镜(CSST)成像中的点扩散函数(PSF)欠采样、波段间变异性和焦平面平滑空间变化等关键问题。

Result: CSST-PSFNet通过融合残差神经网络、轻量级Transformer架构和变分潜在表示，显著提升了中国空间站巡天望远镜(CSST)点扩散函数(PSF)重建的像素级精度和形状参数恢复能力，在标准测试集和模拟在轨退化场景下分别实现了0.005以下的大小残差精度和0.002以下的椭圆率残差精度。

Abstract: This paper presents CSST-PSFNet, a deep learning method for high-fidelity point spread function (PSF) reconstruction developed for the Chinese Space Station Survey Telescope (CSST). The model integrates a residual neural network, a lightweight Transformer architecture, and a variational latent representation to address key challenges in CSST imaging, including severe PSF undersampling, inter-band variability, and smooth spatial variation across the focal plane. Trained and validated on high-resolution star-PSF pairs generated by the CSST Main Survey Simulator, CSST-PSFNet achieves improved pixel-level accuracy and more precise recovery of shape parameters relevant to weak lensing compared to widely used PSFEx. On both the standard test dataset and a blurred dataset representing the upper bound of expected on-orbit PSF degradation, the model achieves a size residual precision below 0.005 and an ellipticity residual precision below 0.002. A weak-label adaptation experiment further shows that the model can recover PSFEx-level performance when the true PSF is unknown, demonstrating robustness in controlled degradation scenarios and weak-label adaptation experiments. These results indicate that CSST-PSFNet provides a flexible and extensible framework for future on-orbit PSF calibration in large-scale CSST surveys, with potential applications in weak-lensing cosmology and precision astrophysical measurements.

</details>

### [27] [Design and performance of the coded mask for the Lunar Electromagnetic Monitor in X-rays (LEM-X)](https://arxiv.org/abs/2603.10752)
*Yuri Evangelista,Alessio Nuti,Francesco Ceraudo,Edoardo Giancarli,Giuseppe Dilillo,Riccardo Campana,Giovanni Della Casa,Ettore Del Monte,Marco Feroci,Mauro Fiorini,Giovanni Lombardi,Massimo Rapisarda,Francesca Esposito,Immacolata Donnarumma,Alessandro Turchi,Ugo Cortesi,Fabio D'Amico,Marco Gai,Andrea Argan*

**TL;DR**: LEM-X（Lunar Electromagnetic Monitor in X-rays）是一种部署于月球表面的宽视场X射线天文台，通过配备硅漂移探测器（SDDs）的编码孔径相机在2-50 keV能段实现高光谱分辨率（≤350 eV@6 keV）和1角分的点源定位精度，显著提升多信使天体物理（multi-messenger astrophysics）中高能暂现源与变源的探测能力。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过月球表面部署的宽视场X射线观测站（LEM-X）提升多信使（multi-messenger）天体物理学能力，重点探测、定位并监测天空中的高能暂现现象和可变X射线源。

Data: 数据来源于月球表面部署的宽视场X射线观测站LEM-X（Lunar Electromagnetic Monitor in X-rays）配备的编码孔径相机系统，该系统采用大面积线性硅漂移探测器（Silicon Drift Detectors, SDDs）在2-50 keV能段进行观测。

Method: 采用编码孔径相机（coded-aperture cameras）结合大面积硅漂移探测器（Silicon Drift Detectors, SDDs）技术，通过优化掩模编码生成与解码算法实现高精度角分辨率和灵敏度。

Result: LEM-X（Lunar Electromagnetic Monitor in X-rays）采用配备大面积硅漂移探测器（SDDs）的编码孔径相机，在2-50 keV能区实现≤350 eV（6 keV）的优异光谱分辨率，并具备1弧分的点源定位精度（PSLA）及5 mCrab（50 ks）的轴灵敏度。

Abstract: The Lunar Electromagnetic Monitor in X-rays (LEM-X) is a proposed wide-field X-ray observatory designed for deployment on the Moon's surface. Its primary scientific goal is to enhance multi-messenger astrophysics by detecting, localizing, and monitoring high-energy transient phenomena and variable X-ray sources across the sky. Building on the heritage of the eXTP and LOFT mission proposals, LEM-X employs pairs of coded-aperture cameras equipped with large-area linear Silicon Drift Detectors (SDDs), offering excellent spectral resolution ($\leq$350 eV at 6 keV) over the 2-50 keV energy range. Each camera provides a field of view of ~1 steradian at 25% effective area and achieves a Point-Source Location Accuracy (PSLA) of 1 arcminute, with an on-axis sensitivity better than 5 mCrab in 50 ks and 700 mCrab in 1 s. In this paper we describe the experiment and focus on the detailed design and optimization of the LEM-X coded mask, analyzing its scientific performance, imaging capabilities, and thermo-mechanical properties. We describe the mask code generation, decoding algorithms, and the trade-offs involved in achieving the required angular resolution, sensitivity, and structural integrity. Imaging simulations and mechanical analyses confirm the effectiveness of the proposed design, demonstrating its suitability for high-precision, wide-field X-ray imaging devoted to multi-messenger astrophysics and transient events detection.

</details>

### [28] [A Python-Based Peeling Framework for Radio Interferometry: Application to uGMRT 650MHz Imaging](https://arxiv.org/abs/2603.10758)
*Hao Peng,Fangxia An,Yuheng Zhang,Srikrishna Sekhar,Russ Taylor,Xianzhong Zheng,Yongming Liang*

**TL;DR**: 开发了一种基于Python、兼容CASA（Common Astronomy Software Applications）的方向依赖（direction-dependent）校准与剥离框架，通过优化“模型恢复”（model-restoration）策略有效抑制了升级版巨米波射电望远镜（uGMRT）数据中的方向依赖效应（如电离层相位畸变和主波束变化），显著提升了图像背景平整度、保真度及弱源探测能力。


<details>
  <summary>Details</summary>

Background: 现代射电干涉阵列面临方向依赖效应（direction-dependent effects）校准的挑战，需解决电离层相位畸变（ionospheric phase distortions）和主波束变化（primary beam variations）等影响以提升成像动态范围和微弱源探测能力。

Data: 数据来源于升级后的巨型米波射电望远镜（uGMRT）的射电连续谱成像数据。

Method: 基于标准CASA任务开发了Python方向依赖校准(direction-dependent calibration)和剥离(peeling)框架，并采用优化的"模型恢复"(model-restoration)策略。

Result: 提出了一种基于Python、兼容CASA（Common Astronomy Software Applications）的方向依赖（direction-dependent）校准与剥离框架，有效抑制了明亮射电源周围的方向依赖伪影（direction-dependent artifacts），显著提升了图像背景平整度、保真度及暗弱源探测能力。

Abstract: Modern radio interferometric arrays offer high sensitivity, wide fields of view, and broad frequency coverage, but also pose significant data calibration challenges. Standard direction-independent calibration is insufficient to correct direction-dependent effects, such as ionospheric phase distortions and primary beam variations, which produce strong artifacts around bright sources and limit achievable image dynamic range. Built on standard CASA tasks, we present a Python-based direction-dependent calibration and peeling framework, demonstrated using radio continuum imaging data from the upgraded Giant Metrewave Radio Telescope (uGMRT). The framework efficiently subtracts bright-source models and suppresses their associated direction-dependent artifacts, producing significantly flattened backgrounds and improving image fidelity and faint-source detectability. We further introduce an optimized ``model-restoration'' strategy that mitigates direction-dependent artifacts while preserving the flux densities and morphologies of bright sources that are themselves of scientific interest. For fields containing multiple bright sources, sequential application of the framework systematically reduces background noise, thereby increasing sensitivity and faint-source detectability. The framework is Python-based, CASA-compatible, and can be readily applied to other mid- and low-frequency interferometric arrays. The code is publicly released with this paper.

</details>

### [29] [Identifying and Measuring Satellite Streaks in DECam Images](https://arxiv.org/abs/2603.10790)
*Alexandra Serrano Mendoza,Meredith L. Rawls,Andrés Alejandro Plazas Malagón*

**TL;DR**: 开发了基于暗能量相机(DECam)档案图像的卫星轨迹检测方法，发现不同轨道物体亮度差异显著但均能被稳定检测，为统计研究卫星对天文观测的影响提供了基础方案。


<details>
  <summary>Details</summary>

Background: 研究聚焦于快速增长的低轨卫星星座（如星链）对地基天文观测的影响，通过分析暗能量相机（DECam）历史图像中的卫星轨迹亮度，评估其对天文巡天的潜在干扰。

Data: 数据来源于暗能量相机（DECam）的存档图像，通过NOIRLab数据档案库获取，并筛选出带有可见条纹的图像进行后续分析。

Method: 采用基于霍夫变换（Hough Transform）的轨迹检测算法（通过satmetrics实现）结合表面亮度测光（surface brightness photometry）技术，并利用SatChecker进行轨道物体识别。

Result: 在DECam（Dark Energy Camera）存档图像中，卫星和其他轨道物体可被稳定检测，但其亮度差异显著，反映了不同物体类型和型号的设计与运行差异。

Abstract: The rapid growth of satellite constellations, particularly Starlink, is increasingly affecting ground-based astronomy. In this project, we developed a workflow to detect, identify, and measure the brightness of trails from artificial satellites and other orbiting objects in archival images from the Dark Energy Camera (DECam), available through the NOIRLab Data Archive. We filtered images with visible streaks, retrieved detector-level images, applied the Hough Transform (via satmetrics) to detect and align trails, and performed surface brightness photometry. We also used SatChecker to obtain likely identifications for each trail. Our sample of nine measured streaks includes Starlink satellites, a navigation satellite, a decommissioned science satellite, and a rocket body. Our results show that satellites and other orbiting objects are consistently detectable in DECam images, but their brightness varies significantly, reflecting design and operational differences across object types and models. While the methodology proved effective, detecting faint streaks was challenging, and short-lived glints remain an even harder problem for future work. This proof-of-concept establishes a foundation for larger statistical studies of satellite impacts on astronomical surveys. The code is available at https://github.com/iausathub/reca-streaks

</details>

<div id='gr-qc'></div>

# gr-qc [[Back]](#toc)

### [30] [Holographic dark energy from a new two-parameter entropic functional](https://arxiv.org/abs/2603.10964)
*G. G. Luciano,E. N. Saridakis*

**TL;DR**: 基于双参数广义熵泛函构建了扩展全息暗能量模型，通过微观熵泛函和微观态计数推导出包含两个独立面积项的广义全息标度熵，其暗能量密度自然包含两个全息分量并能退化为标准全息暗能量和ΛCDM模型，双熵指数丰富了宇宙动力学行为（允许精质或幻影态）且与标准热历史相容。


<details>
  <summary>Details</summary>

Background: 该研究基于双参数广义熵泛函（generalized entropic functional）构建了扩展的全息暗能量（holographic dark energy）理论框架，旨在从微观熵函数和微观态计数出发统一描述标准全息暗能量与ΛCDM模型。

Data: 数据来源于理论推导的广义全息暗能量模型（generalized holographic dark energy model），基于两参数广义熵泛函（two-parameter generalized entropic functional）的微观熵函数及对应微观态计数，未使用实际观测或模拟数据。

Method: 基于两参数广义熵泛函（generalized entropic functional）的微观熵函数和微观态计数构建了扩展全息暗能量模型。

Result: 基于两参数广义熵泛函的扩展全息暗能量模型成功再现了物质-暗能量转变，其两个熵指数丰富了动力学行为，允许类精质（quintessence-like）或幻影（phantom）状态，同时与宇宙标准热历史兼容。

Abstract: We formulate an extended holographic dark energy scenario based on a recently proposed two-parameter generalized entropic functional. Unlike constructions that phenomenologically impose modified entropy-area relations at the horizon level, the present framework is rooted in a microscopic entropy functional and the corresponding microstate counting. For bounded systems, the entropy acquires a generalized holographic scaling with two independent area contributions, recovering the Bekenstein-Hawking entropy in the appropriate limits. Implementing this entropy within the holographic principle, we derive a generalized dark energy density containing two distinct holographic sectors, naturally embedding standard holographic dark energy and $Λ$CDM as limiting cases. We analyze the cosmological evolution for both Hubble and future event horizon cutoffs and show that the model successfully reproduces the matter-to-dark-energy transition. The two entropic exponents enrich the dynamics, allowing for quintessence-like behavior or phantom regimes, while remaining compatible with the standard thermal history of the Universe.

</details>

<div id='astro-ph.EP'></div>

# astro-ph.EP [[Back]](#toc)

### [31] [A test of the Dedalus software for exoplanet atmospheric dynamics](https://arxiv.org/abs/2603.10076)
*Rick Bonhof,Quentin Changeat,James Y-K. Cho*

**TL;DR**: 采用浅水方程和Dedalus3软件包验证了系外行星（exoplanet）大气动力学模拟的可靠性，发现该工具能有效研究木星纬向急流（zonal jets）和非线性演化过程，但需针对不同问题谨慎测试。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用浅水方程（shallow-water equations）和光谱方法（spectral method）精确求解系外行星（exoplanet）大气动力学问题。

Data: 使用基于谱方法的Dedalus3软件包求解浅水方程（shallow-water equations），并采用木星纬向（zonal）急流不稳定性测试作为验证基准。

Method: 采用基于谱方法（spectral method）的软件包Dedalus3求解浅水方程（shallow-water equations）来研究行星流动动力学。

Result: Dedalus3可作为研究行星流动动力学的有效工具，但需针对不同问题谨慎测试和执行。

Abstract: Studying exoplanet flow and variability requires solving atmospheric dynamics equations accurately. Here we use the shallow-water equations to evaluate and employ Dedalus3, a spectral method-based software package for solving differential equations. A well-known jet instability test is used for the evaluation; then, the package is used to investigate the nonlinear evolution of observed, Jupiter's zonal (east--west) jets; finally, the package is used to compare hot-Jupiter flows with different initial conditions. Our results indicate that Dedalus3 can be a useful tool for investigating planetary flow dynamics, but careful testing and execution are necessary for each problem.

</details>

### [32] [Interstellar Object 3I/ATLAS Observed from Mars by China's Tianwen-1 Spacecraft](https://arxiv.org/abs/2603.10350)
*Xin Ren,Wei Yan,Ruining Zhao,Shu Wang,Xingye Gao,Qiang Fu,Qing Zhang,Bin Yang,Man-To Hui,Zhiyong Xiao,Xiaodong Liu,Cunhui Li,Renhao Tian,Wenguang Liu,Dong Wang,Shaoran Liu,Cong Ren,Jie Dong,Xinbo Zhu,Pan Xie,Jian-Yang Li,Yan Geng,Jianjun Liu*

**TL;DR**: 中国天问一号火星轨道器首次在深空利用高分辨率相机（HiRIC CMOS）成功拍摄第三颗星际天体3I/ATLAS，通过尘埃动力学模型揭示其彗发由数百微米大颗粒主导（辐射压力参数β≈10⁻³-10⁻²），喷射速度3-10 m/s，尘埃质量流失率约10³ kg/s，结合此前发现的2I/Borisov，表明这类富含超挥发物的星际天体可能源自母行星盘外缘。


<details>
  <summary>Details</summary>

Background: 首次从显著偏离轨道平面的视角对星际天体3I/ATLAS进行成像观测，为研究星际彗星尘埃动力学（dust dynamics）和太阳系外行星盘形成机制提供了独特约束。

Data: 数据来源于天问一号(Tianwen-1)火星轨道器搭载的高分辨率相机(HiRIC CMOS)在2025年9月30日至10月3日期间对第三颗星际天体3I/ATLAS进行的三个观测时段的成像数据。

Method: 采用HiRIC CMOS相机对3I/ATLAS进行多时段成像观测，并结合Finson-Probstein尘埃动力学模型分析尘埃参数（β值、粒径、抛射速度）和亮度分布。

Result: 首次从显著偏离轨道平面的视角对3I/ATLAS星际天体成像，揭示了其尘埃动力学特征。

Abstract: China's Tianwen-1 Mars orbiter successfully imaged the third interstellar object, 3I/ATLAS, during its close encounter with Mars using the onboard HiRIC CMOS camera. This is China's first deep-space observation of an astronomical object. These observations constitute the first imaging of this object from a vantage point significantly out of its orbital plane, providing a unique constraint on dust dynamics. Three observing epochs between 2025 September 30 and October 3 reveal clear changes in coma and tail morphology driven by the rapidly evolving viewing geometry. Comparison with Finson-Probstein dust dynamical models indicates that the coma is dominated by large grains with solar radiation pressure parameter $β\approx 10^{-3} $ - $10^{-2}$, corresponding to grain sizes of a few 100s $μ$m. The extent of the sunward coma implies dust ejection velocities of $3$ - $10$ m s$^{-1}$. Despite the morphological evolution, the azimuthally averaged surface brightness profile remains nearly unchanged through the three epochs, transitioning from a radial slope near -1 close to the nucleus to slightly steeper than -1.5 at larger cometocentric distances, consistent with steady-state dust outflow accelerated by solar radiation pressure. Photometry yields an average $Afρ\sim (2.0\pm0.2)\times10^4$ cm and a corresponding dust mass loss rate of $\dot{M} \sim 10^3$ kg s$^{-1}$. The dominance of large grains in both interstellar comets discovered to date, 2I/Borisov and 3I/ATLAS, together with their high supervolatile contents, may indicate that these objects originate from the outer regions of their parent planetary disks.

</details>

### [33] [At the stellar noise frontier: a transit survey of 121 TESS M3--M6 dwarfs](https://arxiv.org/abs/2603.10247)
*Yohann Tschudi*

**TL;DR**: 论文系统性地调查了121颗新获得足够TESS多扇区覆盖的中晚期M型矮星（M3-M6），发现20个凌星信号（16个系统），其中2个为高可靠性候选体（Tier 1），7个中等可靠性（Tier 2），10个易受噪声影响（Tier 3），并建立了17.4%的全局误报率。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用TESS（凌日系外行星巡天卫星）新观测周期数据系统搜寻中晚期M型矮星（M-dwarf stars）周围短周期（0.5-100天）小型凌日行星（transiting planets），填补了此前多扇区覆盖不足导致的探测空白。

Data: 数据来源为TESS（Transiting Exoplanet Survey Satellite）的Cycle 6+观测数据，样本选自498,312颗TIC（TESS Input Catalog）M型矮星，并利用Gaia DR3（Data Release 3）进行验证。

Method: 采用基于TLS（Transit Least Squares）的自动化凌星信号检测流水线，结合TRICERATOPS（Transiting Exoplanet Reliability Investigation via Convolutional Autoencoder and Three-dimensional Orbital Parameter Statistics）验证层、Gaia DR3（Data Release 3）交叉验证及三重经验可靠性测试框架。

Result: 在121颗M3-M6矮星（有效温度2700-3400 K）的系统性凌星巡天中，发现了20个凌星信号（分布在16个系统中），其中2个被归类为Tier 1（高可靠性）、7个为Tier 2（中等可靠性）、10个为Tier 3（易受噪声影响），且这些信号均无先前的TOI（TESS Objects of Interest）标识。

Abstract: M-dwarf stars are the most favorable hosts for detecting small transiting planets, yet mid-to-late M-dwarfs that acquired sufficient TESS multi-sector coverage only through recent Cycle 6+ observations represent a newly accessible discovery space. This paper presents a systematic transit survey of 121 M3-M6 dwarfs (Teff = 2700-3400 K) selected as "newly enabled" targets -- stars with <=2 archival TESS sectors that only recently crossed the multi-sector detection threshold, covering P = 0.5-100 d. The sample was selected from 498,312 TIC M-dwarfs via a 9-step funnel. The pipeline combines TLS with a signal validation cascade, TRICERATOPS vetting, Gaia DR3 verification, and three empirical signal reliability tests. Pipeline validation achieved 100% recovery (16/16 planets) on 10 known systems with zero false positives. The survey identifies 20 transit-like signals across 16 systems, none with prior TOI designations. The reliability framework classifies 2 as Tier 1 (High Robustness), 7 as Tier 2 (Moderate), and 10 as Tier 3 (Noise-Susceptible); one monotransit is excluded. No candidate SDE significantly exceeds its host star's noise floor. The global false alarm rate is 17.4% (21/121; Wilson 95% CI: [11.6%, 25.1%]).The 2 Tier 1 candidates are priorities for RV confirmation. The 10 Tier 3 candidates require additional TESS sectors to establish signal persistence; 9 systems need high-resolution imaging.

</details>

### [34] [Spectral Decomposition Reveals Surface Processes on Europa](https://arxiv.org/abs/2603.10520)
*Gideon Yoffe,Sahar Shahaf*

**TL;DR**: 通过JWST近红外光谱仪（NIRSpec-IFU）观测发现，木卫二（Europa）前导半球上二氧化碳（CO2）富集区域超出Tara Regio范围且呈透镜状分布，与异常冰纹理特征共存，表明挥发性物质富集反映了近表层微物理特性和物质迁移过程，修正了表面-内部交换机制的解释框架。


<details>
  <summary>Details</summary>

Background: 木卫二（Europa）表面化学组成与地质活动及带电粒子轰击的竞争过程及其对宜居性评估的影响。

Data: 数据来源为JWST（James Webb Space Telescope）近红外光谱仪-积分视场单元（NIRSpec-IFU）对木卫二（Europa）前导半球三个观测几何形态下九个敏感水冰、辐射分解产物及挥发物的光谱波段观测数据。

Method: 采用光谱分解（spectral factorization）方法分析JWST近红外光谱仪积分场单元（NIRSpec-IFU）观测数据，分离主导光谱变异性的组分并重建其空间分布。

Result: CO2富集区域延伸至Tara Regio以外，并以透镜状模式覆盖多个混沌单元（chaos units），且与异常冰纹理特征共存。

Abstract: Competing processes shape Europa's surface: geological activity replenishes material through resurfacing, while bombardment by charged particles alters surface chemical composition. Each process leaves distinct spectral signatures. We present a novel data-driven analysis of JWST NIRSpec-IFU observations of Europa's leading hemisphere across three observing geometries, targeting nine spectral bands sensitive to water ice, radiolytic products, and volatiles. Through spectral factorization, we isolate the dominant components of spectral variability and reconstruct their spatial distributions. We find that CO2 enrichment extends beyond Tara Regio, and covers multiple chaos units in a lens-like pattern. These CO2-enriched areas co-occur with anomalous ice-texture signatures. Together, these findings suggest that enrichment in volatiles on Europa may reflect retention-favorable near-surface microphysics as well as emplacement, refining how they are interpreted in the context of surface--interior exchange. This has implications for interpreting the sources and supply rates of extant carbon-bearing species and, ultimately, for assessing Europa's habitability.

</details>

### [35] [The Asteroid Framing Cameras on ESA's Hera mission](https://arxiv.org/abs/2603.10594)
*Jean-Baptiste Vincent,Gábor Kovács,Balázs V. Nagy,Frank Preusker,Naomi Murdoch,Maurizio Pajola,Michael Kueppers,Patrick Michel,Seiji Sugita,Hannah Goldberg*

**TL;DR**: 首次验证了DART动能撞击器成功偏转双星小行星系统65803 Didymos的次级星Dimorphos后，欧空局Hera任务搭载的小行星取景相机(Asteroid Framing Cameras)将通过多阶段高分辨率成像（最高达<10厘米/像素）全面表征该双星系统的物理特性，为行星防御提供关键数据支持。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过赫拉任务(Hera mission)的小行星双星系统(65803 Didymos)动力学特性表征及DART撞击效应精确测量，以评估行星防御(planetary defense)策略的有效性。

Data: 数据来源于欧空局(ESA)赫拉(Hera)任务搭载的两台相同的小行星框架相机(Asteroid Framing Cameras)，这些相机由德国耶拿-奥普特罗尼克(ena-Optronik)公司制造，将在任务期间从不同距离和观测几何角度获取图像数据，包括全球测绘数据（早期特征阶段2-3米/像素、详细特征阶段1-2米/像素、近距离操作阶段0.5-2米/像素）以及针对特定区域（如DART撞击点和JUVENTAS立方星着陆点）的高分辨率飞越数据（分辨率<10厘米/像素）。

Method: 采用两台相同的全色相机（Asteroid Framing Cameras）进行多阶段高分辨率成像，支持导航与科学探测，具备5.5×5.5度视场和93.7微弧度/像素角分辨率，通过不同任务阶段（空间分辨率2-3米/像素至<10厘米/像素）实现全球测绘与局部精细观测。

Result: Asteroid Framing Cameras（小行星框架相机）将通过在任务不同阶段（早期特征描述阶段2-3米/像素、详细特征描述阶段1-2米/像素、近距离操作阶段0.5-2米/像素）对双星系统进行全球测绘，并在特定感兴趣区域（如DART撞击点和JUVENTAS立方星着陆点）通过专用飞越实现<10厘米/像素的分辨率，从而提供满足任务需求的数据。

Abstract: As the first asteroid deflection test, NASA's successfully hit asteroid Dimorphos (secondary of the binary asteroid 65803 Didymos) with the DART kinetic impactor on September 26, 2022. To fully characterise the physical properties of the objects, and measure precisely the effects of this impact in the context of planetary defence, ESA launched the Hera mission on 7 October 2024, with scheduled arrival at Didymos in fall 2026. Among the core payload of the mission, the Asteroid Framing Cameras are two identical imaging systems that will support navigation and scientific activities, by acquiring images from various distances and observing geometries during the course of the mission. Built by ena-Optronik (Germany), the cameras match the requirements designed by the science team and will provide data that supports a wide range of investigations: hazard detection, system dynamics, shape reconstruction, surface morphology and mapping, and surface photometry. Each instrument is a panchromatic camera equipped with a 5.5 x 5.5 degree field of view, and an angular resolution of 93.7 micro-radians per pixel. The cameras shall provide the necessary data to address the mission requirements through a global mapping of the two components of the binary system at spatial scales of 2-3 m/pixel in the Early Characterisation Phase, 1-2 m/pixel in the Detailed Characterisation Phase, and 0.5-2 m/pixel in the Close Operation Phase. Dedicated flybys will bring the resolution down to < 10 cm/pixel on specific areas of interest on Dimorphos, such as the DART impact site and the JUVENTAS cubesat landing site. Here, we present the technical specifications of the camera, as well as the status of the calibration. We then summarise the planned operations in cruise and at the asteroids. Finally, we provide examples of the scientific investigations and products that will make use of the data returned by the cameras

</details>

### [36] [The Desert Fireball Network Clear-Sky Survey](https://arxiv.org/abs/2603.10778)
*Konstantinos S. Servis,Hadrien A. R. Devillepoix,Eleanor K. Sansom,Thomas W. C. Stevenson*

**TL;DR**: 提出了一种基于HEALPix框架的自动化方法，用于校正多相机网络（如沙漠火球网络DFN）的流星观测数据，并通过对2015年南金牛座流星雨（Southern Taurid）的案例分析，验证了该方法在厘米至米级流星体通量密度（meteoroid flux density）测量中的有效性，结果与彗星起源的流星体密度（~300 kg.m⁻³）假设一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于厘米至米级流星体通量密度（meteoroid flux density）的精确估算，填补了威胁航天器的小型流星体与具有行星防御意义的近地天体（Near-Earth Objects）之间的关键数据空白。

Data: 数据来源于沙漠火球网络（Desert Fireball Network, DFN）的多摄像头网络观测数据，包括2015年南金牛座流星雨期间33个摄像头在三个月（2015年10月至12月）内拍摄的数百万张全天相机图像。

Method: 采用基于HEALPix框架的自动化天空分区方法，结合多相机网络数据，通过恒星源检测和通量分布分析确定晴空条件，实现高效流星体通量测量。

Result: 提出了一种基于HEALPix框架的自动化方法，用于校正多相机网络（如沙漠火球网络DFN）的流星观测数据偏差，实现了对70公里高度天空的等面积像素划分和有效覆盖范围的精确测量。

Abstract: Estimating the meteoroid flux density at centimetre to metre sizes is notoriously difficult. Yet it is an important endeavour, as these sizes represent the transition between small meteoroids that pose a risk to spacecraft, and the Near-Earth Objects that are relevant for planetary defense.
  We present a novel automated methodology for debiasing meteor observations from multi-camera networks, applied to data from the Desert Fireball Network (DFN). Our approach utilizes the Hierarchical Equal Area isoLatitude Pixelisation (HEALPix) framework to partition the sky into equal-area pixels at 70 km altitude, enabling precise and convenient measurement of effective survey coverage and fireball counting across the network. We developed a comprehensive data processing pipeline that analyses millions of all-sky camera images to determine clear-sky conditions through automated star source detection and flux distribution analysis.
  As a case study, we apply this methodology to observations of the 2015 Southern Taurid meteor shower, during which there was significant fireball activity. Processing data from 33 cameras over a three-month period (October-December 2015), we calculate an effective observation coverage of $1.58 \times 10^{12}$ km$^2$.h and identified 54 Southern Taurid fireballs from 141 validated detections. Our results are consistent with the extrapolation of previous work done on the same meteor shower at smaller sizes, when we set a $\sim300$ kg.m$^{-3}$ mean meteoroid density, consistent with the cometary origin of the Taurid stream.
  The HEALPix-based approach successfully automates what was previously a labor-intensive manual process, providing a scalable solution for accurate flux measurements from distributed camera networks; it is directly applicable to other meteor surveys.

</details>

<div id='hep-ph'></div>

# hep-ph [[Back]](#toc)

### [37] [Stodolsky effect in the framework of Generalised Neutrino Interactions](https://arxiv.org/abs/2603.10114)
*Siddhartha Bandyopadhyay,Ujjal Kumar Dey*

**TL;DR**: 通过研究最一般形式的中微子（neutrino）与电子相互作用，发现非标准相互作用（non-standard neutrino interactions）和张量相互作用项（tensor interaction terms）对斯托多斯基效应（Stodolsky effect）有非零贡献，并分析了宇宙中微子背景（CνB）能量偏移及其不对称性对探测的影响。


<details>
  <summary>Details</summary>

Background: 研究探讨了在电弱能标下利用最一般形式的中微子-电子相互作用（包括所有满足SU(3)⊗U(1)对称性的洛伦兹不变算子）来探索斯托多斯基效应（Stodolsky effect）的理论框架。

Data: 研究中使用的数据来源于理论计算（simulation），基于考虑所有可能的洛伦兹不变算子并尊重SU(3)⊗U(1)对称性的中微子与电子相互作用的最一般形式。

Method: 采用最一般的洛伦兹不变算子（Lorentz invariant operators）并考虑SU(3)⊗U(1)对称性，分析中微子（neutrino）与电子的相互作用，重点考察非标准中微子相互作用（non-standard neutrino interactions）和张量相互作用项（tensor interaction terms）的贡献。

Result: 在标准模型贡献之外，仅非标准中微子相互作用（non-standard neutrino interactions）和张量相互作用项（tensor interaction terms）对斯托多斯基效应（Stodolsky effect）产生非零贡献。

Abstract: We study the Stodolsky effect utilizing the most general form of neutrino interactions with electrons below the electroweak scale by considering all possible Lorentz invariant operators respecting SU(3)$\otimes$U(1) symmetry. We perform our calculation for both Dirac and Majorana neutrinos and find that in the most general setting, only the non-standard neutrino interactions and the tensor interaction terms provide a non-zero contribution, apart from the Standard Model contribution. We investigate the implications for the possible detection of the cosmic neutrino background (C$ν$B) by analysing the energy shifts that are characteristic of the Stodolsky effect. We also discuss the implication of considerable asymmetry in the C$ν$B on the present scenario.

</details>

### [38] [Searching for axions with time resolved pulsar polarimetry](https://arxiv.org/abs/2603.10189)
*Francesca Chadha-Day,Tanmay Kumar Poddar*

**TL;DR**: 通过蟹状星云脉冲星(Crab pulsar)光学偏振的时间分辨观测，对轴子-光子耦合(axion-photon coupling)给出了限制，展示了时间分辨脉冲星双折射在轴子(axion)搜索中的潜力。


<details>
  <summary>Details</summary>

Background: 该研究探讨了脉冲星（pulsar）强偶极磁场通过轴子-光子（axion-photon）相互作用产生轴子场（axion field）的可能性及其对偏振观测的影响。

Data: 数据来源于蟹状星云脉冲星(Crab pulsar)光学偏振的时间分辨观测(time resolved observations)。

Method: 采用时间分辨（time-resolved）光学偏振观测技术分析蟹状星云脉冲星（Crab pulsar）的双折射效应（birefringence），约束轴子-光子耦合（axion-photon coupling）参数。

Result: 通过蟹状星云脉冲星(Crab pulsar)光学偏振的时间分辨观测，对轴子-光子耦合(axion-photon coupling)给出了限制，证实了时间分辨脉冲星双折射(time resolved pulsar birefringence)在轴子搜索中的潜力。

Abstract: Pulsars possess strong dipole magnetic fields that can source axion fields through the axion-photon interaction. Pulsars may therefore be surrounded by axion field configurations oscillating with the pulsar's rotational period. These axions could be detected by observing their effect on the polarization of the pular's emission. In this paper, we use time resolved observations of the optical polarization of the Crab pulsar to place bounds on the axion-photon coupling, demonstrating the potential of time resolved pulsar birefringence in the search for axions.

</details>

<div id='astro-ph.HE'></div>

# astro-ph.HE [[Back]](#toc)

### [39] [On the angular localization of gravitational-wave signals by pulsar timing arrays](https://arxiv.org/abs/2603.10120)
*Stephen R. Taylor*

**TL;DR**: 论文推导了脉冲星计时阵列（PTA）中引力波信号定位精度的解析表达式，揭示了脉冲星与波源角距（ξ）和脉冲星距离测量精度（σ_L）的影响机制，并量化了不同参数区间下定位精度随脉冲星数量、信噪比（SNR）和距离精度的变化规律。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用脉冲星计时阵列（PTA）定位引力波信号的影响因素，特别是脉冲星与引力波源的角距离（ξ）和脉冲星距离测量精度（σL）对定位精度的作用机制。

Data: 研究中使用的数据来源为脉冲星计时阵列（pulsar timing array, PTA）观测数据，包括脉冲星与引力波源的角距离（ξ）和脉冲星距离测量精度（σ_L）等参数。

Method: 采用解析推导的克拉美-罗下界(Cramér-Rao bound)分析脉冲星计时阵(PTA)引力波信号定位精度，量化脉冲星与波源角距(ξ)和脉冲星距离测量精度(σ_L)的影响，并建立地球项与脉冲星项干涉导致的费希尔矩阵(Fisher matrix)角向振荡模型。

Result: 当脉冲星距离精确已知时（即σ_L≤λ_GW/(1-cosξ)），信号定位精度随σ_L²提升直至达到衍射极限；当距离不精确时，增加靠近波源的脉冲星数量可改善定位精度，其关系在小角度极限下为ΔΩ_sky∝(∑_αSNR_α²/ξ_α²)^(-1)。

Abstract: We provide a complete study of the factors influencing gravitational-wave signal localization using pulsar timing arrays. We derive analytical expressions for the Cramér-Rao sky localization precision that delineate the impact of the angular proximity, $ξ$, between the pulsar and the gravitational wave source, and the precision, $σ_L$, with which pulsar distances are known. Interference between the Earth and pulsar terms creates rapid angular oscillations for sky-coordinate Fisher matrix elements that aids localization, which is complemented by more broadly varying antenna response gradient information. The relative importance of these factors depends on whether pulsar distances are known precisely [i.e., $σ_L\leqλ_\mathrm{GW}/(1-\cosξ)$] or imprecisely, respectively. If the former, tightening pulsar distance precisions improves signal localization according to $ΔΩ_\mathrm{sky}\proptoσ_L^2$ until the Earth-pulsar system reaches its diffraction limit. If the latter, localization precision is degraded, but more pulsars in close proximity to the source is the best means of improving. With $α$ indexing pulsars, this scales as $ΔΩ_\mathrm{sky}~\propto~(\sum_α\mathrm{SNR}_α^2/ξ_α^2)^{-1}$ in the small-angle limit of the unmarginalized Fisher matrix, and we derive the analytic generalization to any angle between a pulsar and the source. Finally, we study a scenario where pulsar-term phases are treated as nuisance variables that are unconnected to binary or PTA properties. This phase-decoupled scenario, which is how all PTA continuous wave searches are currently conducted, delivers localization performance similar to the antenna-response--driven case, and does not exhibit significant improvement as pulsar distance precisions are tightened.

</details>

<div id='astro-ph.SR'></div>

# astro-ph.SR [[Back]](#toc)

### [40] [White Dwarfs with Infrared Excess from LAMOST Data Release 11](https://arxiv.org/abs/2603.10481)
*Keyi Wang,Qiong Liu,Siyi Xu,Alberto Rebassa-Mansergas*

**TL;DR**: 该研究通过交叉比对LAMOST DR11白矮星(WD)目录与多波段巡天数据，利用VOSA工具对1818颗白矮星进行能谱拟合，新发现167个红外(IR)超候选体（经筛选后确认139个），包括30个WD+M矮星双星、19个WD+褐矮星(BD)双星、66个WD+尘埃盘系统及24个WD+BD或WD+尘埃盘候选系统，显著扩展了白矮星伴星与尘埃盘系统的样本。


<details>
  <summary>Details</summary>

Background: 研究聚焦于白矮星(WD)的红外(IR)超现象及其伴星或碎片盘的相互作用，为恒星演化晚期阶段及白矮星与周围物质的关联提供独特视角。

Data: 数据来源包括LAMOST DR11的白矮星（WD）光谱数据，并与SDSS、2MASS、UKIDSS和WISE的光学及红外巡天数据进行交叉匹配。

Method: 采用交叉匹配LAMOST DR11白矮星(WD)目录与SDSS、2MASS、UKIDSS和WISE等多波段巡天数据，并利用VOSA工具进行光谱能量分布(SED)拟合的分析方法。

Result: 通过交叉比对LAMOST DR11白矮星(WD)目录与多波段巡天数据并利用VOSA工具进行光谱能量分布拟合，共发现139个具有候选红外(IR)超量的白矮星系统，包括30个白矮星+M矮星双星候选体（18个新系统）、19个白矮星+褐矮星(BD)双星候选体（8个新系统）、66个白矮星+尘埃盘候选体（38个新系统）以及24个白矮星+褐矮星或尘埃盘的混合候选体（19个新系统）。

Abstract: Infrared (IR) excess observed around white dwarfs (WDs) is typically attributed to companions or debris disks. These systems are interesting because they offer a unique opportunity to study the late stages of stellar evolution and the interactions between WDs and surrounding material. The 11th data release (DR11) of the Large Sky Area Multi-Object Fiber Spectroscopic Telescope (LAMOST) - one of the largest spectroscopic surveys to date - has recently provided spectra for 3092 WDs, many of which have yet to be systematically investigated for IR excess. In this study, we cross-correlated the LAMOST DR11 WD catalog with optical and IR surveys, including the Sloan Digital Sky Survey (SDSS), Two Micron All-Sky Survey (2MASS), UKIRT Infrared Deep Sky Survey (UKIDSS), and Wide-field Infrared Survey Explorer (WISE). We performed spectral energy distribution fitting using the VOSA tool for 1818 WDs and identified 167 IR excess WD candidates. After excluding 23 sources with potential contamination within 6" and five additional sources identified through WISE ccf flag analysis, we identified 139 objects with candidate IR excess. These include 30 candidate WD + M dwarf binaries (18 new systems), 19 candidate WD + brown dwarf (BD) binaries (eight new systems), 66 candidate WD + dust disks (38 new systems), and 24 candidate either WD + BD or WD + dust disks (19 new systems). Given the limited spatial resolution of WISE, all candidate systems require follow-up IR observations for confirmation, such as high spatial resolution imaging or IR spectroscopy. This will help expand the parameter space of dust disks, allowing us to explore a broader range of possibilities.

</details>

### [41] [Newly discovered Luminous blue variable candidates in M31 & M33](https://arxiv.org/abs/2603.10508)
*Sai Li,Cheng Liu,Jincheng Guo*

**TL;DR**: 研究确认M31和M33中8颗候选明亮蓝变星(cLBVs)的光谱特征和物理性质，发现1颗确认为明亮蓝变星(LBV)、4颗高概率cLBVs，并估算其初始质量约32-60倍太阳质量，显著增加了这两个星系中已知LBVs的数量。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近邻星系M31和M33中候选明亮蓝变星（cLBVs）的光谱特征与演化状态，旨在扩充当前稀缺的已知LBV样本并完善其分类标准。

Data: 数据来源于LAMOST（Large Sky Area Multi-Object Fiber Spectroscopic Telescope）的多历元光谱以及先前文献中已确认的候选明亮蓝变星（cLBVs）的光谱数据。

Method: 采用光谱能量分布(SED)拟合和光谱建模确定关键恒星参数，并通过与温度-光度图上的恒星演化轨迹比较估算初始质量。

Result: 在近红外双色图中，除双星系统外，所有样本均位于LBVs区域而非B[e]SGs区域。

Abstract: This study presents an investigation of nearly two dozen candidate Luminous Blue Variables (cLBVs) in the galaxies M31 and M33. Eight stars have been studied in detail, while an additional sixteen objects are briefly mentioned. Multi-epoch spectra of confirmed cLBVs from LAMOST and previous literature show broad hydrogen, He I lines, abundant Fe II and [Fe II] emission lines, and discernible spectral variability, consistent with the characteristics of known LBVs. Low outflow velocities inferred from P Cygni profiles are also incorporated into the classification criteria. Moreover, key stellar properties, including temperature and luminosity, are determined using the Spectral Energy Distribution (SED) fitting and spectral modeling. By comparison with stellar evolutionary tracks on the temperature luminosity diagram, the initial masses are estimated to be in the range of approximately 32 to 60 $M_{\odot}$. Except for J013401 and J013411, other stars locate within the typical LBV region between the S Doradus instability strip and their outburst phase. More importantly, our sample, except for the binary system, are all positioned in the LBVs region rather than that of B[e]SGs in the near-infrared color-color diagram. Based on all available information, one of the eight sources is confirmed as an LBV, four stars are designated as high-probability cLBVs, and the remaining three stars await further photometric observations to secure their classification. Given the current scarcity of known cLBVs, our study has the potential to make a significant increase in the number of LBVs in M31 and M33.

</details>

### [42] [Searching for Magnetic White Dwarfs in LAMOST DR10](https://arxiv.org/abs/2603.11004)
*Si-Cheng Yu,Juan-Juan Ren,Vitaly V. Neustroev,Thomas Hackman,Hao-Tong Zhang,Yi-Qiao Dong,Zhong-Rui Bai,Hai-Long Yuan,Mengxin Wang,Ming Zhou*

**TL;DR**: 利用LAMOST DR10低分辨率光谱结合盖亚（Gaia）数据新发现32颗孤立磁白矮星（MWDs），并测量了其表面磁场强度（几至数十MG），验证了LAMOST在磁白矮星研究中的有效性。


<details>
  <summary>Details</summary>

Background: 磁白矮星（MWDs）是理解致密恒星中磁场起源与演化的关键研究对象。

Data: 数据来源包括LAMOST DR10光谱数据、Gaia EDR3白矮星候选体数据以及基于SDSS的磁白矮星（MWDs）最新星表。

Method: 利用塞曼分裂（Zeeman splitting）分析巴尔末线和氦吸收线作为主要诊断手段来识别磁场并估算其强度。

Result: 在LAMOST DR10中发现了63颗孤立磁白矮星（MWDs），其中32颗为新发现，并通过塞曼分裂测量其表面磁场强度范围从几MG至数十MG。

Abstract: Magnetic white dwarfs (MWDs) are key to understanding the origin and evolution of magnetic fields in compact stars. While large spectroscopic surveys such as SDSS have greatly expanded the known sample, the potential of LAMOST has not yet been fully explored. Our aim is to identify and characterize isolated MWDs in the LAMOST DR10 database. We cross-matched LAMOST DR10 spectra with white dwarf candidates from Gaia EDR3 and with recent SDSS-based catalogs of MWDs. Zeeman splitting in Balmer and helium absorption lines was used as the primary diagnostic to identify magnetic fields and to estimate their strengths. Reference objects from SDSS catalogs were used to test the detectability of MWDs in LAMOST low-resolution spectra. We identified 63 isolated MWDs in LAMOST DR10, of which 32 are new discoveries. Surface magnetic field strengths were measured from Zeeman splitting, covering a range from a few MG up to several tens of MG. For previously known SDSS MWDs, our LAMOST-based field measurements show mostly agreement with published values. This work demonstrates the capability of LAMOST low-resolution spectroscopy to identify and characterize isolated MWDs. The newly discovered objects expand the known population and provide valuable targets for future high-resolution spectroscopic and polarimetric follow-up studies. Our results highlight the potential of combining LAMOST with Gaia and other large surveys to build a more complete census of MWDs.

</details>

### [43] [Gravitational Anomaly Measurement in Wide Binaries is Sensitive to Orbital Modeling](https://arxiv.org/abs/2603.11015)
*Serat M. Saad,Yuan-Sen Ting*

**TL;DR**: 通过分层贝叶斯模型重新分析36个宽双星对(Wide-binary pairs)数据，发现轨道三维建模方法会显著影响引力异常因子$γ$的推算结果，采用独立半长轴参数可使$γ$值降至1.12而与牛顿引力($γ=1$)相符。


<details>
  <summary>Details</summary>

Background: 研究聚焦于验证修正牛顿动力学（MOND）理论对宽双星系统低加速度区域引力异常（$γ\equiv G_{\rm eff}/G_{\rm N}$）的预测，并探讨不同轨道建模方法对结果的影响。

Data: 数据来源于对Chae等人（2026）研究中36个宽双星对的相同数据集（same dataset）重新分析。

Method: 采用分层贝叶斯模型（hierarchical Bayesian model）推断全局重力增强因子（γ）并拟合三维轨道参数。

Result: 在重新分析宽双星系统数据时，采用分层贝叶斯模型（hierarchical Bayesian model）拟合三维轨道参数得到的重力增强因子（γ）为1.12^{+0.27}_{-0.22}，与牛顿引力（γ=1）在0.4σ水平一致。

Abstract: Recent work by Chae et al. (2026) reported a gravitational anomaly in 36 wide-binary pairs, finding a gravity boost factor of $γ\equiv G_{\rm eff}/G_{\rm N} \approx 1.60_{-0.14}^{+0.17}$ at low accelerations, consistent with predictions from Modified Newtonian Dynamics (MOND). We reanalyze the same dataset using a hierarchical Bayesian model that infers a global $γ$ across all systems while fitting three-dimensional orbital elements. Our model yields $γ= 1.12^{+0.27}_{-0.22}$, consistent with Newtonian gravity ($γ= 1$) at the $\sim0.4σ$ level. To identify the source of the discrepancy, we perform a test using an approach similar to Chae et al. (2026), replacing the semi-major axis with a geometric de-projection of the observed projected separation. This test yields $γ= 1.56^{+0.21}_{-0.18}$, closely matching the result of Chae et al. (2026). This suggests that the inferred value of $γ$ is sensitive to how the three-dimensional orbital separation is modeled, and including an independent semi-major axis parameter can account for velocity excesses that would otherwise be attributed to non-Newtonian gravity.

</details>