...
### [1] [Intermediate-mass black hole incubators. Gas accretion onto stellar black hole clusters in galactic central molecular zones](https://arxiv.org/abs/2601.07905)
*Jaroslav Haas,Pavel Kroupa,Sergij Mazurenko*

**TL;DR**: 研究发现，位于银河系中央分子区（central molecular zone）的≥10^4 M⊙恒星质量黑洞（stellar-mass black hole）子星团可通过气体吸积在数百万年内积累等量物质，最终坍缩形成中等质量黑洞（intermediate-mass black hole）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量星团（massive star clusters）在星暴期（starburst periods）形成后，其恒星动力学演化导致中心区域出现$\gtrsim10^4 M_\odot$量级的恒星级黑洞子团（stellar-mass black hole sub-clusters），并探讨其在富气环境（如银河系中央分子带，central molecular zone）中通过吸积气体可能触发向中等质量黑洞（intermediate-mass black hole）的坍缩过程。

Data: 数据来源于银河系中央分子区（central molecular zone）的观测结果，包括其天体物理条件、恒星形成历史以及半径约100 pc的扭曲分子云环（twisted ring of molecular clouds）的结构特征。

Method: 采用解析方法（analytical method）估算黑洞团吸积率及其辐射反馈，并基于银河系中央分子区（central molecular zone）的天体物理条件和恒星形成历史评估黑洞团坍缩成中等质量黑洞（intermediate-mass black hole）的可能性。

Result: 在银河系中央分子区（central molecular zone）观测到的质量$\gtrsim10^4 M_\odot$恒星黑洞（stellar black hole）子星团可通过吸积周围气体在数百万年内获得等量质量，进而坍缩形成中等质量黑洞（intermediate-mass black hole）。
...
### [2] [A survey of molecular clouds in the Galactic center's outflow](https://arxiv.org/abs/2601.07907)
...
**TL;DR**: 首次在银河系外流风中系统探测到200多个分子云团（CO clumps），发现其具有小尺寸（1-3秒差距）、高湍流（速度弥散1-6 km/s）和低引力束缚（维里参数>>10-100）特征，并证实分子气体在距银心1千秒差距外完全解离，支持热风裹挟冷云团逐步解离的演化模型。


<details>
  <summary>Details</summary>

Background: 银河系中心驱动的大尺度多相星系外流（galactic outflow）中分子气体的存在与演化机制。

Data: 数据来源于阿塔卡马探路者实验望远镜（APEX）对19个以已知高速原子氢（HI）云为中心的区域的12CO(2-1)发射线观测。

Method: 采用阿塔卡马探路者实验（APEX）观测12CO(2-1)发射线，结合高速度原子氢（HI）云数据，分析分子团块（CO clumps）的半径、速度弥散、质量和维里参数（virial parameter）等物理特性。

Result: 在银河系外流中首次系统探测到分子气体，发现超过200个CO团块具有典型半径1-3秒差距、速度弥散1-6 km/s和质量数倍至数百倍太阳质量，这些团块位于正常分子云质量-尺寸关系的低质量端但显著偏离质量（或尺寸）-线宽关系，表现出更高湍流和内部压力。
...
### [3] [Relatively Fast and Reasonably Furious: Evidence for Increased Burstiness in Smaller Halos at Cosmic Dawn](https://arxiv.org/abs/2601.07912)
...
**TL;DR**: 通过联合拟合星系紫外光度函数（UVLF）、成团性和Hα/UV比值，首次实证发现早期星系中低质量暗晕（halo）宿主的恒星形成爆发性（burstiness）更强，且该爆发性随暗晕质量减小而增强，最高可达1 dex（对应$M_h \leq 10^9 M_\odot$时$σ_{M_{\rm UV}} \approx 1.5$），这一质量依赖性爆发性可缓解JWST观测前后星系形成模型的矛盾。


<details>
  <summary>Details</summary>

Background: 早期星系恒星形成的突发性（burstiness）与暗物质晕质量相关，是理解高红移星系质量组装的关键因素。

Data: 数据来源于红移$z\sim 4-6$的星系紫外光度函数（UVLFs）、成团性以及H$α$/UV比率的观测结果。

Method: 采用联合拟合星系紫外光度函数（UVLFs）、成团性及Hα/UV比值的框架，结合流体动力学模拟预测的质量依赖爆发性（mass-dependent burstiness）模型。

Result: 早期星系中低质量暗晕（halo）寄主星系具有更强的恒星形成爆发性（burstiness），且爆发性随暗晕质量减小而增强，在$M_h \leq 10^{9}\, M_\odot$时达到$\gtrsim 1$ dex。
...
**TL;DR**: 通过贝叶斯框架分析红移(z)≈3.3的莱曼阿尔法发射体(LAEs)周围75个吸收体，发现高柱密度(\log\,N(HI)/cm^{-2}≳16.5)吸收体呈现双峰金属丰度分布(峰值在[Z/H]=-3.8±0.2和-1.8±0.6)，分别对应原始宇宙网流入气体和未探测暗弱星系外流金属增丰的环星系介质(CGM)，且吸收体间存在显著物化性质不均匀性(金属丰度差异约1 dex，密度差异约0.5 dex)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于红移$z\approx3.3$的莱曼阿尔法发射体（LAEs）周围吸收体（absorbers）的物理性质，特别是其密度和金属丰度（metallicity）的分布特征及其与星系际介质（IGM）和星系周介质（CGM）的关联。

Data: 数据来源于MUSEQuBES巡天中红移（redshift）$z\approx3.3$的莱曼阿尔法发射体（LAEs）的吸收体（absorbers），通过沃伊特轮廓拟合（Voigt profile fitting）获取中性氢（\HI）和离子柱密度（ionic column densities）。

Method: 采用自定义贝叶斯框架（Bayesian framework）对75个吸收体进行密度和金属丰度分析，基于沃伊特轮廓拟合（Voigt profile fitting）获得的氢原子（HI）和离子柱密度数据。

Result: 在$z\approx3.3$的莱曼α发射体(LAEs)相关吸收体中，高柱密度($\log\, N(\rm HI)/{\rm cm^{-2}} \gtrsim 16.5$)吸收体表现出双峰金属丰度分布，峰值分别位于\met$=-3.8\pm 0.2$和$-1.8\pm 0.6$，前者可能来自原始宇宙网流入气体，后者则与未被莱曼α探测到的暗弱星系外流金属富集CGM相关。
...
### [5] [FROST-CLUSTERS -- III. Metallicity-dependent intermediate mass black hole formation by runaway collisions in dense star clusters](https://arxiv.org/abs/2601.07917)
*Antti Rantala,Thorsten Naab,Natalia Lahén,Klaus Reuter,Markus Rampp,Martyna Chruślińska,Bastián Reinoso*

**TL;DR**: 通过大量数值模拟揭示了星团密度($Σ_\mathrm{h}$)和金属丰度(Z)对中等质量黑洞(IMBHs)形成的决定性影响，建立了IMBH质量与宿主星团参数的拟合公式，并提出了宇宙IMBH形成率密度模型，挑战了本地IMBHs均为早期超大质量黑洞(SMBHs)失败种子的观点。


<details>
  <summary>Details</summary>

Background: 研究聚焦于中等质量黑洞（IMBHs）作为超大质量黑洞（SMBHs）潜在种子的形成机制，特别关注不同星团表面密度和金属丰度条件下恒星碰撞的失控过程。

Data: 数据来源包括1400多个孤立星团和30个层次结构星团的数值模拟(N-body simulation)，使用BIFROST代码模拟了包含最多1.8×10^6颗恒星的年轻大质量星团，涉及恒星碰撞动力学、恒星演化和黑洞的后牛顿运动方程。

Method: 采用BIFROST代码进行数值模拟，结合碰撞恒星动力学、恒星演化及后牛顿运动方程，分析不同星团密度（4×10³-4×10⁶ M⊙ pc⁻²）和金属丰度（0.01-1.0 Z⊙）下恒星碰撞形成中等质量黑洞（IMBHs）的过程。

Result: 高金属丰度（Z≳0.2Z⊙）下恒星风损失率抑制中等质量黑洞（IMBHs）形成，而低密度（Σh≲3×10^4M⊙pc^-2）下碰撞率不足也阻碍其形成。
...
### [6] [On the Jeans criterion for hydrostatic and infalling gas](https://arxiv.org/abs/2601.07929)
*Carlo Nipoti*

**TL;DR**: 研究发现非旋转天体物理流体（non-rotating astrophysical fluids）在外部引力势和自引力共同作用下仍遵循经典金斯判据（Jeans criterion），且流体静力学平衡（hydrostatic equilibrium）构型中线性扰动保持稳定，而下落气体（infalling gas）在风力等非引力作用下可满足引力不稳定条件。


<details>
  <summary>Details</summary>

Background: 研究探讨了非旋转天体物理流体在外部引力势和流体自引力共同作用下的局部引力不稳定性问题。

Data: 研究使用了线性扰动分析（linear-perturbation analysis）方法，基于两种稳态构型（steady-state configurations）的非旋转天体物理流体数据：流体静力学平衡（hydrostatic equilibrium）和气体向引力势阱（gravitational potential well）的下落过程。

Method: 采用自洽线性扰动分析（self-consistent linear-perturbation analysis）方法，考虑背景介质中的压力和密度梯度。

Result: 在存在外部引力场的情况下，非旋转天体物理流体的局部引力不稳定性仍遵循经典金斯判据（Jeans criterion），且该判据不受外部引力场影响。
...
### [7] [Galaxy interactions in void substructures: Morphology and stellar populations of two triplets from CAVITY](https://arxiv.org/abs/2601.07952)
*G. M. Azevedo,A. L. Chies-Santos,R. Riffel,I. Perez,F. Ferrari,R. S. de Souza,M. Argudo-Fernandéz,B. Bidaran*

**TL;DR**: 研究发现宇宙空洞(void)中两个三重星系系统CAVITY5273X和VGS31呈现不同演化特征：前者包含一个具有活动星系核(AGN)的静止大质量星系，后者所有成员均以年轻恒星和近期恒星形成为主，且五个星系在过去20亿年经历快速质量增长，形态扰动表明存在持续相互作用，局部相互作用可在极端低密度环境中驱动显著星系演化。


<details>
  <summary>Details</summary>

Background: 研究主题为宇宙空洞（cosmic voids）环境下星系三重态（galaxy triplets）的恒星演化、形态特征及动力学性质。

Data: 数据来源于卡拉斯阿尔托空洞积分视场光谱巡天（CAVITY）中的两个空洞三重星系系统CAVITY5273X和VGS31，采用积分视场单元（IFU）光谱观测并结合FADO代码进行全光谱拟合。

Method: 采用积分视场单元光谱技术（integral-field unit spectroscopy）结合FADO代码进行全光谱拟合，同时建模恒星和电离气体发射，并利用集成嵌套拉普拉斯近似（integrated nested Laplace approximation）重建恒星年龄、金属丰度和恒星形成率的空间分布图。

Result: 两个空洞三重星系系统呈现不同演化特征：CAVITY52731是包含活动星系核（AGN）的大质量静止星系，其伴星系及VGS31所有星系均以年轻恒星群体和近期恒星形成为主导；五个星系在过去20亿年经历快速恒星质量增长，形态扰动表明持续相互作用；CAVITY5273X成员符合预期空洞质量-金属丰度关系，而VGS31成员显著偏离该关系。
...
### [8] [Evidence of Feedback Effects in Low-luminosity Active Galactic Nuclei Revealed by JWST Spectroscopy](https://arxiv.org/abs/2601.07990)
...
**TL;DR**: 通过分析四个低光度活动星系核(AGN)的红外光谱，发现其核区(r < 150 pc)表现出弱高电离势线、大尺寸多环芳烃(PAHs)比例较高以及分子氢(H2)未完全热化的特征，揭示了激波主导的电离机制和AGN反馈强度随光度变化的系统性差异。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低光度活动星系核(AGN)的核区物理特性及其反馈效应，通过中红外光谱分析揭示AGN光度与反馈模式（激波激发、分子破坏等）的系统关联。

Data: 数据来源于JWST（James Webb Space Telescope）的NIRSpec/IFU和MIRI/MRS对四个低光度活动星系核（AGN）核区（r < 150 pc）的红外光谱（~3-28 μm）观测，作为GATOS（Galaxy Activity, Torus, and Outflow Survey）的扩展研究。

Method: 采用JWST（James Webb Space Telescope）的NIRSpec/IFU和MIRI/MRS仪器获取近红外（3-28微米）光谱数据，结合电离气体、多环芳烃（PAHs）和分子氢（H2）的发射特性分析，通过线比（line ratios）和激波速度（vs）评估辐射激波（radiative shocks）和分子激波（molecular shocks）的激发机制。

Result: 低光度活动星系核(AGN)的核区表现出较弱的高电离势线发射(如[Ne V]和[O IV])，线比表明快速辐射激波(速度约每秒数百公里)是其电离气体的主要激发源。
...
### [9] [Investigating quenching in Recently Quenched Elliptical galaxies with HI studies](https://arxiv.org/abs/2601.08027)
*Deepak K. Deo,Daniel H. McIntosh,Sravani Vaddi,Kameswara B. Mantha,Ruta Kale,Alfonso G. Franco,Paul Rulis*

**TL;DR**: 研究发现近期停止恒星形成的椭圆星系（RQEs）保留了显著的中性氢（HI）含量（f_gas≥17%），并确定了log M_halo=12.1 M⊙的临界晕质量阈值，表明其淬灭机制并非由气体耗尽主导，而是与冷热模式气体吸积转换、并合事件及反馈机制相关。


<details>
  <summary>Details</summary>

Background: 该研究探讨了近期淬灭椭圆星系(RQEs)从恒星形成到静止星系转变过程中的关键阶段及其淬灭机制。

Data: 使用了155个近期淬灭椭圆星系（RQEs）及其前体（preRQEs）和后体（postRQEs）的多波段观测数据，重点分析了中性氢（HI）含量和恒星形成特性。

Method: 采用多波段（multi-wavelength）分析方法，结合中性氢（HI）气体含量测量和恒星形成特性研究，识别出临界晕质量阈值（$\log M_{\text{halo}} = 12.1 M_{\odot}$）并区分两种演化路径。

Result: 近期淬灭椭圆星系(RQEs)保留了显著的中性氢(HI)气体储备($f_{\text{gas}} \geq 17\%$)，表明淬灭过程并非主要由气体耗尽驱动。
...
### [10] [Reducing the Dimensions of AGN Lightcurve Manifolds](https://arxiv.org/abs/2601.08037)
...
**TL;DR**: 通过数据驱动的低维流形学习，发现活动星系核(AGN)多波段光变曲线投影结构与光谱特性(如[OIII]光度、D$_n$(4000))及宿主星系性质(如恒星质量)存在关联，无需物理建模即可区分变脸活动星系核(CLAGN)类型并定位潮汐瓦解事件(TDE)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）分类与光变行为多样性在统一框架下的表征难题，特别是变脸活动星系核（CLAGNs）带来的复杂性。

Data: 数据来源包括NASA Fornax科学平台整合的ZTF、Pan-STARRS、Gaia和WISE/NEOWISE光变曲线，涵盖两类样本：一类是约2000个红移$z \lesssim 1$的异质AGN（含SDSS类星体、变选源和CLAGN），另一类是约65000个红移$z \approx 0.1$的窄线AGN（具备完善光学发射线测量）。

Method: 采用数据驱动的低维流形学习方法分析多波段测光光变曲线，无需依赖类别标签或显式物理建模。

Result: 通过数据驱动的低维流形学习，多波段光变曲线能自发区分活动星系核（AGN）类别并关联独立光谱特性（如恒星质量、[OIII]光度），无需显式物理建模。
...
**TL;DR**: 研究发现不同核合成源（Ia型超新星、II型超新星和渐近巨星支恒星）产生的金属在星系风中的载出效率存在显著差异（约0.3 dex），且这种差异随星系环境变化，挑战了基于丰度诊断的星系形成解释方法。


<details>
  <summary>Details</summary>

Background: 研究探讨了Ia型超新星（type Ia supernovae）、II型超新星（type II supernovae）和渐近巨星分支（AGB）恒星作为恒星核合成场所对星系风金属负载（metal loading）差异的影响及其在星系形成诊断中的意义。

Data: 使用高分辨率“tall box”模拟（tall box simulations）结合GPU加速代码QUOKKA（QUOKKA GPU-accelerated code），通过系统改变星系气体面密度、金属丰度、核合成源的标高及相对发生率来生成数据。

Method: 采用高分辨率“高盒”（tall box）模拟和GPU加速代码QUOKKA，系统研究了不同核合成源（Ia型超新星、II型超新星和渐近巨星分支星AGB）的金属加载（metal loading）差异。

Result: 不同核合成来源（Ia型超新星、II型超新星、AGB星）产生的金属在星系风中的载出效率存在显著差异（典型变化幅度约0.3 dex），这种现象称为差异金属载出（differential metal loading）。
...
**TL;DR**: 开发了用于高红移(high-z)星系旋转曲线分析的参数化正向建模工具RotCurves，通过校正"光束涂抹效应(beam smearing)"并在星系平面投影和卷积光束点扩散函数(PSF)，其运行速度比dysmalpy快250倍，在良好分辨系统(PSF FWHM < Reff)中旋转和弥散曲线在3Reff范围内吻合度达95%，适用于大样本积分视场单元(IFU)巡天数据的快速处理。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发高效灵活的旋转曲线（rotation curve）建模工具以支持大样本积分视场单元（IFU）运动学巡天对高红移星系的动力学研究。

Data: 数据来源包括合成观测（synthetic observations）和大型积分视场单元（IFU）运动学巡天数据。

Method: 采用参数化正向建模工具RotCurves进行旋转曲线分析，通过投影和卷积星系平面内的波束点扩散函数（PSF）校正"波束模糊"效应。

Result: RotCurves是一种高效的参数化正向建模工具，专为高红移(high-z)旋转曲线分析设计，通过投影和卷积星系平面上的波束点扩散函数(PSF)来校正“波束涂抹效应(beam smearing)”。
...
### [13] [Long Term (250 years) Hydrodynamical Simulation of the Supermassive Black Hole Binary OJ287](https://arxiv.org/abs/2601.08080)
*Ariel Chitan,Sarah C. Gallagher,Shahram Abbassi*

**TL;DR**: 通过3D流体动力学模拟(3D hydrodynamic simulations)发现超大质量黑洞双星(SMBH binary) OJ287在质量比为1.835×10¹⁰ M⊙和1.4×10⁸ M⊙时，次黑洞撞击主盘会形成螺旋密度波(spiral density waves)并导致主黑洞吸积率增加10-20%，且撞击时间间隔呈现1-10年的准周期变化，验证了OJ287作为进动双星(precessing binary)的现有模型，同时排除了近等质量双星系统作为OJ287候选体的可能性。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用即将投入使用的观测设施探测超大质量黑洞双星(SMBH binary)的光度与引力波信号，并探究其长期吸积驱动的光变特性。

Data: 使用PHANTOM代码对OJ287进行了3D流体动力学模拟（3D hydrodynamic simulation），模拟了两种质量比情形：(i) M$_1$ = 1.835$\times$10$^{10}$ M$_\odot$与M$_2$ = 1.4$\times$10$^{8}$ M$_\odot$，(ii) M$_1\approx$ M$_2$ ($\sim10^{8}$ M$_\odot$)以及(iii)单个超大质量黑洞（SMBH）和吸积盘的对照模拟。

Method: 采用PHANTOM代码进行3D流体动力学模拟，研究超大质量黑洞(SMBH)双星系统OJ287在不同质量比下的吸积盘动力学行为。

Result: 质量比为1.835×10¹⁰ M⊙和1.4×10⁸ M⊙的双黑洞（SMBH binary）模拟结果与当前最被接受的OJ287模型一致，表现为次级黑洞撞击盘面引发螺旋密度波并导致主黑洞吸积率增加10-20%。
...
### [14] [The Secret Lives of Open Clusters: a Multiwavelength Examination of Three Open Clusters](https://arxiv.org/abs/2601.08087)
...
**TL;DR**: 通过多波段观测发现三个邻近银河系疏散星团（IC 2602、NGC 2632和M67）中X射线源数量（77、31、31）和性质随星团年龄（30 Myr、750 Myr、4 Gyr）演化，并首次在M67中探测到可能为休眠黑洞双星或活跃恒星三重系统的多波段变源（WOCS 3012/S1077）。


<details>
  <summary>Details</summary>

Background: 研究主题为利用多波段观测探索银河系近距疏散星团（open clusters）中动力学相互作用（dynamical interactions）导致的独特天体现象及其随星团年龄的演化规律。

Data: 数据来源包括全天空巡天项目eROSITA的X射线观测、钱德拉X射线天文台(Chandra X-ray Observatory)的存档数据、ASKAP宇宙演化巡天(Evolutionary Map of the Universe survey)的射电观测、甚大天线阵(VLA)的存档射电数据以及盖亚(Gaia)的新星团表。

Method: 采用多波段（X射线、射电和光学）观测数据结合盖亚（Gaia）星团新编目进行交叉比对分析。

Result: 通过多波段观测发现三个银河系疏散星团（IC 2602、NGC 2632和M67）的X射线源数量分别为77、31和31个，并基于光学变率和射电辐射对这些源进行了分类。
...
### [15] [The Contribution of Stars, Dust, Neutral Gas and SMBHs in Galaxies to the Cosmic Baryon Inventory](https://arxiv.org/abs/2601.08112)
...
**TL;DR**: 通过光谱能量分布建模分析约80万个星系，发现恒星、中性气体、超大质量黑洞(SMBH)和尘埃仅占重子物质的约5%，其余95%可能以电离态分布于星系内外介质中。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙恒星、尘埃和中性气体质量随红移（redshift）演化的历史，旨在构建星系内重子物质（baryon）的自洽普查。

Data: 数据来源于星系和物质组装巡天（GAMA）和深空河外可见遗产巡天（DEVILS）中约80万个星系的光谱能量分布（SED）建模。

Method: 采用ProSpect光谱能量分布(SED)建模方法分析GAMA和DEVILS巡天中约80万个星系的光谱数据，结合金属度依赖的尘埃-气体比(dust-to-gas ratio)估算中性气体质量。

Result: 宇宙尘埃质量历史（cosmic dust mass history）与宇宙恒星形成历史（cosmic star formation history）形态相似，但下降速度较慢，表明恒星形成在红移$z\approx 2$达到峰值后尘埃增长和破坏速率减缓。
...
### [16] [Diverse Origins of Broad H$α$ Lines in Heavily Obscured AGNs Revealed by Multi-epoch Spectroscopy](https://arxiv.org/abs/2601.08212)
...
**TL;DR**: 研究发现重度遮蔽活动星系核（AGN）中的宽Hα线可能源自宽线区（BLR）、电离外流或极区物质散射，揭示了遮蔽AGN中宽发射线的多起源性。


<details>
  <summary>Details</summary>

Background: 研究探讨了经典活动星系核（AGN）模型中宽发射线（broad emission lines）起源的例外情况，特别是在高消光（$A_V > 50$ mag）AGN中观测到宽H$α$线的现象。

Data: 数据来源于对MCG -3-34-64、UGC 5101和Mrk 268三个活动星系核（AGN）的多历元光学光谱观测，通过测量宽Hα线通量变化及光谱拟合分析（spectral fitting analysis），并利用Hα复合线与[SII]λλ6716, 6731双线的通量比进行验证。

Method: 采用多纪元光学光谱观测（multi-epoch optical spectroscopic observations）和光谱拟合分析（spectral fitting analysis）研究宽线区（BLR）和电离外流（ionized outflows）对宽发射线的贡献。

Result: 在严重遮蔽（$A_V > 50$ mag）的AGN中观测到的宽H$α$线可能具有多种起源，包括电离外流（ionized outflows）、极区物质散射（scattering by polar material）或直接来自宽线区（broad-line region, BLR）。
...
### [17] [Self-consistent dynamical models with a finite extent -- V. Smooth radial truncations and phase-space consistency](https://arxiv.org/abs/2601.08361)
*Maarten Baes*

**TL;DR**: 论文提出了一种无限可微的径向截断方法（infinitely differentiable radial truncation scheme），通过渐近截断（soft truncation）解决了有限空间范围恒星系统（stellar systems）模型因密度突变导致动力学不一致的问题，并发现该方法可支持多种轨道结构（Osipkov-Merritt orbital structures），临界截断锐度（critical truncation sharpness）对应的分布函数在截断能（truncation energy）附近呈现凸凹特征（bump-dip feature）。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于构建具有有限空间范围且动力学自洽的球对称恒星系统模型，探讨了密度分布截断方式对模型自洽性的影响。

Data: 数据来源于理论推导的无限可微径向截断方案（infinitely differentiable radial truncation scheme）及其在赫奎斯特模型（Hernquist model）上的应用验证。

Method: 采用无限可微径向截断方案（infinitely differentiable radial truncation scheme）对任意密度剖面进行平滑截断，并基于赫奎斯特模型（Hernquist model）验证其动态一致性。

Result: 软截断方法通过引入无限可微的径向截断方案，解决了有限空间范围恒星系统模型因密度突变导致分布函数不一致的问题，并确定了动态一致性所需的临界截断锐度。
...
### [18] [Local Group analogues in a cosmological context -- I. Relating velocity structure to the cosmic web](https://arxiv.org/abs/2601.08419)
*Kai Wang,Peder Norberg,Azadeh Fattahi,Louis E. Strigari*

**TL;DR**: 研究发现本地群类似体（LGAs）的耦合能量（E_coupling）与其所处大尺度环境密度相关，高耦合能量系统偏好高密度区域且呈现弱极向各向异性但强方位各向异性，而低耦合能量系统倾向低密度环境且与宇宙纤维结构强烈对齐，本地群属于典型耦合能量系统且处于低密度环境。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用本星系群（Local Group）及其类似系统（LGAs）作为邻近宇宙的独特实验室，探讨其在ΛCDM宇宙学框架下与小尺度结构及大尺度环境（large-scale environment）的关联性。

Data: 数据来源于ABACUSSUMMIT模拟套件（ABACUSSUMMIT simulation suite）。

Method: 采用ABACUSSUMMIT模拟套件分析局部群类似体（LGAs）与周围大尺度环境的关系，重点关注银河系-M31轨道耦合能量（E_coupling）的关键作用。

Result: 高耦合能量（$E_{\rm coupling}$）的本地群类似体（LGAs）倾向于位于高密度区域，而低耦合能量的类似体则分布在低密度环境中。
...
### [19] [Three Models of the Gravitational Potential of the Milky Way](https://arxiv.org/abs/2601.08378)
*V. V. Bobylev,A. T. Bajkova,A. A. Smirnov*

**TL;DR**: 通过整合多种天体速度数据构建了银河系0-190千秒差距范围内的精确旋转曲线（rotation curve），并开发了包含盘（disk）、晕（halo）及少量核球（bulge）的引力势（gravitational potential）模型，适用于研究银河系中心区域（R<4千秒差距）内天体的长期轨道演化。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过改进银河系轴对称引力势模型参数，构建更精确的银河系旋转曲线（rotation curve），以探索暗物质晕（halo）和星系盘（disk）对星系动力学的影响。

Data: 数据来源包括脉泽源(masers)、经典造父变星(classical Cepheids)、红团簇星(Red Clump stars)、蓝水平分支星(Blue Horizontal Branch stars)、晕族恒星(halo stars)、球状星团(globular clusters)以及银河系矮卫星星系(dwarf satellite galaxies)的速度数据，覆盖了$R:0-190$ kpc的距离范围。

Method: 采用轴对称模型（axisymmetric model）对银河系引力势参数进行优化，构建了包含盘、暗物质晕（halo）以及小质量核球（bulge）的多组分模型，并通过与Illustris TNG50软件生成的理论曲线对比验证自洽性。

Result: 构建了两个包含盘和暗晕（halo）贡献的双组分银河系势场模型，以及一个额外加入小质量核球（bulge）的三组分模型。
...
**TL;DR**: 通过MUSE-ALMA Haloes巡天观测60个红移约0.5的强HI吸收星系，发现低质量星系遵循恒星形成主序关系而高质量星系恒星形成效率受抑制，揭示了HI吸收体同时追踪演化成熟和年轻星系的特性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用MUSE-ALMA Haloes巡天数据探索红移(redshift)约0.5处与强中性氢(HI)吸收相关的星系样本，旨在完善该红移处重子物质(baryon)普查。

Data: 数据来源于ALMA（Atacama Large Millimeter/submillimeter Array）Cycle 10观测，新增39个系统至初始21个，总计60个星系，其中9个新星系检测到CO发射，总计12个星系检测到CO发射，探测深度比先前研究提高了1.2 dex分子气体质量。

Method: 采用ALMA（Atacama Large Millimeter/submillimeter Array）观测CO(2-1)和CO(3-2)谱线，结合VLT/MUSE（Very Large Telescope/Multi Unit Spectroscopic Explorer）和HST（Hubble Space Telescope）的恒星形成率与气体金属丰度数据，进行多波段联合分析。

Result: result：在60个星系中检测到12个存在CO发射，将HI选择星系中CO发射数量翻倍，并比先前研究探测到分子气体质量深1.2个数量级，发现低质量系统遵循主序标度关系而高质量系统呈现恒星形成抑制的双重行为。
...
### [21] [On the Interstellar Extinction Curve toward HD 93222, A Sightline with an Exceedingly Narrow 2175 Angstrom Extinction Bump](https://arxiv.org/abs/2601.08563)
*Qian Wang,Aigen Li,Xuejuan Yang*

**TL;DR**: 研究发现船底座星云中年轻恒星HD 93222的2175埃消光凸起（2175 Angstrom extinction bump）异常尖锐，需在传统硅酸盐和石墨尘埃混合物基础上引入纳米级石墨颗粒（nano-sized graphitic grains）才能解释其消光特征。


<details>
  <summary>Details</summary>

Background: 2175埃消光凸起（2175 Angstrom extinction bump）作为星际消光曲线最显著的光谱特征，广泛存在于银河系和近远外星系的星际介质（ISM）中。

Data: 数据来源于对船底座星云中年轻恒星HD 93222视线方向的消光曲线观测。

Method: 采用硅酸盐和石墨尘混合物以及纳米级石墨颗粒（nano-sized graphitic grains）的混合模型来解释HD 93222的消光特征。

Result: 在HD 93222视线方向的星际消光曲线中观测到极窄的2175埃消光峰（2175 Angstrom bump），这是银河系及外部星系中最窄的之一。
...
### [22] [Stellar masses of optically dark galaxies: uncertainty introduced by the attenuation law and star-formation histories](https://arxiv.org/abs/2601.08693)
...
**TL;DR**: 通过贝叶斯光谱能量分布(SED)建模分析，发现高红移(high-redshift)星系S1、S2和S3的恒星质量估算受恒星形成历史(SFH)先验和尘埃衰减曲线等系统因素影响，修正后仍有两个星系在各自红移处保持极高恒星质量和恒星形成效率。


<details>
  <summary>Details</summary>

Background: 研究探讨了高红移(high-redshift)星系可能具有超大质量对早期星系形成和宇宙学标准模型的挑战。

Data: 研究使用了JWST（James Webb Space Telescope）的NIRCam/grism和NIRSpec/IFU观测数据，包括三个星系（S1、S2和S3）的光谱红移和光谱能量分布（SED）数据。

Method: 采用贝叶斯光谱能量分布（SED）建模工具Prospector，分析恒星质量时重点考察了恒星形成历史（SFH）先验（恒定与上升SFH基）、尘埃衰减规律及发射线流量处理等关键模型假设的影响。

Result: result：JWST观测的三颗高红移星系（S1、S2、S3）经贝叶斯SED建模修正后恒星质量分别为log(M⋆/M⊙)≈10.36+0.47−0.32、10.95+0.11−0.10和10.31+0.24−0.19，其中采用上升型SFH先验会降低恒星质量估计值，且尘埃衰减曲线斜率与恒星质量存在显著简并性，但S2和S3仍属于各自红移处最大质量且恒星形成效率极高的系统。
...
### [23] [First direct electron temperature measurement in [O II] zone in I Zw 18](https://arxiv.org/abs/2601.08722)
*I. A. Zinchenko,J. M. Vílchez,C. Kehrig,P. Papaderos,J. E. Méndez-Delgado*

**TL;DR**: 首次在极贫金属蓝致密矮星系I Zw 18东南结中直接测量了低电离区[O II]电子温度，并联合高、中电离区温度差异确定了总氧丰度为7.066±0.046，为极贫金属电离区温度结构提供了关键校准基准。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于极贫金属蓝致密矮星系（blue compact dwarf galaxy）I Zw 18的电子温度和氧丰度精确测量，旨在拓展对极端贫金属H II区（H II region）温度结构的理解。

Data: 数据来源于暗能量光谱仪数据发布1（DESI DR1）的光谱数据。

Method: 利用暗能量光谱仪数据发布1（DESI DR1）的光谱数据，首次在I Zw 18中通过罕见探测到的[O II] λλ7320,7330双线直接测量低电离区电子温度，并探测了分别对应高、中电离区的[O III] λ4363和[S III] λ6312极光线。

Result: 首次在I Zw 18中直接测量了低电离区电子温度（[O II] λλ7320,7330双线），并探测到高电离区（[O III] λ4363）和中电离区（[S III] λ6312）的极光线，测得各电离区电子温度分别为Te([O III])=21200±860 K、Te([O II])=16170±950 K和Te([S III])=17290±1750 K，揭示了显著的电离区温度差异。
...
### [24] [A precessing jet from an active galactic nucleus drives gas outflow from a disk galaxy](https://arxiv.org/abs/2601.08791)
...
**TL;DR**: 研究发现活动星系VV 340a中心超大质量黑洞驱动的低功率喷流存在(8.2±5.5)×10^5年周期的进动(precession)，并以19.4±7.9倍太阳质量/年的速率驱动气体外流，产生的电离等离子体延伸数千秒差距，显著影响星系恒星形成率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）反馈机制对调控大质量星系恒星形成的关键作用及其能量学和时间尺度约束问题。

Data: 数据来源包括光学、红外、亚毫米波和射电波段对活动星系VV 340a的观测。

Method: 结合光学、红外、亚毫米和射电多波段观测数据，分析活动星系VV 340a中心超大质量黑洞驱动的低功率喷流（jet）进动周期及气体外流速率。

Result: 在活动星系VV 340a中观测到低功率喷流（jet）以（8.2±5.5）×10^5年为周期进动，驱动气体外流速率达19.4±7.9太阳质量/年，并通过激波产生高度电离等离子体，影响星系恒星形成率。
...
**TL;DR**: 通过引力透镜（gravitational lensing）结合强弱透镜数据，发现星系团子晕（subhalo）的质量函数和潮汐截断半径符合冷暗物质（CDM）预期，但内部密度轮廓（inner density profile）和径向分布与CDM强烈不符，核心区密度斜率（γ≳2.5）和星系-星系强透镜（GGSL）发生率支持自相互作用暗物质（SIDM）的坍缩核心模型，揭示了纯碰撞less CDM的微观物理矛盾。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过引力透镜（gravitational lensing）精确探测星系团子结构特性，以严格检验暗物质（dark matter）模型尤其是冷暗物质（CDM）在小尺度上的预测。

Data: 数据来源为强引力透镜（strong lensing）和弱引力透镜（weak lensing）观测数据。

Method: 采用最先进的引力透镜模型（state-of-the-art lens models）提取了子晕（subhalo）的四个关键属性：质量函数（mass function）、投影径向分布（projected radial distribution）、内部密度轮廓（internal density profile）和潮汐截断半径（tidal truncation radius）。

Result: 子晕（subhalo）的质量函数和截断半径与冷暗物质（CDM）模型预期一致，但内部密度轮廓和径向分布与CDM模型存在显著差异。
...
### [26] [Sahyadri: A simulation suite for the cosmology dependence of the Cosmic Web](https://arxiv.org/abs/2601.07924)
*Saee Dhawalikar,Shadab Alam,Aseem Paranjape,Arka Banerjee*

**TL;DR**: 该论文介绍了Sahyadri宇宙学N体模拟套件，通过系统变化六个宇宙学参数（基于普朗克2018约束）并采用种子匹配初始条件，实现了对低红移宇宙的高精度研究，其分辨率（暗物质晕最小质量$3.2 \times 10^{9}$ $h^{-1}$ $M_\odot$）比现有模拟套件提升25倍以上，并能捕捉DESI BGS巡天中40%的星系（约160万个），同时验证了物质功率谱、晕质量函数等统计量与理论预测的高度一致性。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于为下一代光谱巡天（spectroscopic surveys）提供支持，通过开发Sahyadri这套宇宙学N体模拟（cosmological N-body simulations）套件，旨在精确研究低红移（low-redshift）宇宙的物理特性。

Data: 数据来源于一套名为Sahyadri的宇宙学N体模拟（N-body simulation）套件，该套件在普朗克2018年约束条件附近系统性地变化了六个宇宙学参数，并采用种子匹配的初始条件以实现宇宙学参数导数计算，每个模拟在边长为200 h⁻¹ Mpc的周期性盒子中演化2048³个粒子，基准普朗克2018宇宙学下的粒子质量为8.1×10⁷ h⁻¹ M⊙。

Method: 采用宇宙学N体模拟（cosmological N-body simulations）结合种子匹配初始条件（seed-matched initial conditions）和自定义压缩方案（custom compression scheme），系统性地变化六个宇宙学参数（cosmological parameters）并测量物质功率谱（matter power spectrum）、晕质量函数（halo mass function）及非两点统计量（beyond 2-point statistics）。

Result: Sahyadri是一套宇宙学N体模拟（N-body simulations）套件，旨在通过系统变化六个宇宙学参数（围绕普朗克2018年约束）和种子匹配初始条件，支持下一代光谱巡天对低红移宇宙的高精度研究。
...
### [27] [Baryon Acoustic Oscillations from the C IV Forest with DESI DR2](https://arxiv.org/abs/2601.08103)
...
**TL;DR**: 通过重子声波振荡（BAO）分析三重电离碳（C IV）吸收与类星体（QSO）及发射线星系（ELG）位置的交叉关联，首次在CIV×QSO（4.2σ）和CIV×ELG（2.5σ）中检测到BAO信号，并精确测量了各向同性距离尺度（D_V）与拖曳时期声波视界（r_d）的比值（zeff=1.92时为30.3±0.9，zeff=1.47时为24.6±1.0）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用重子声波振荡（BAO）信号测量宇宙大尺度结构，通过三电离碳（C IV）吸收与类星体（QSO）及发射线星系（ELG）的互相关分析来约束宇宙学距离尺度。

Data: 数据来源于暗能量光谱仪（DESI）第二次数据发布（DR2）中的250万颗类星体（QSO）和310万颗发射线星系（ELG），以及150万颗红移$z > 1.3$的高红移类星体视线方向的三重电离碳（C IV）吸收测量。

Method: 采用重子声波振荡（BAO）分析方法，通过三重电离碳（C IV）吸收与类星体（QSO）及发射线星系（ELG）位置的交叉关联测量各向同性距离尺度（D_V）与拖拽时期声波视界（r_d）的比值。

Result: 首次在三重电离碳（C IV）吸收与类星体（QSO）互相关中测量到4.2σ显著性的各向同性重子声波振荡（BAO）信号，并首次在C IV与发射线星系（ELG）互相关中以2.5σ显著性探测到BAO特征。
...
### [28] [A new magnitude--redshift relation based on SNe Ia](https://arxiv.org/abs/2601.08505)
*Ósmar Rodríguez,Alejandro Clocchiatti*

**TL;DR**: 发现了Ia型超新星(SNe Ia)标准化星等($m$)与红移($z$)间的负线性关系$m-5\log(z(1+z)) \propto z$，该双参数关系优于多数宇宙学模型且无需低红移样本，通过八个深场区域验证了宇宙加速的各向同性($q_0$介于$-0.6$至$-0.4$)。


<details>
  <summary>Details</summary>

Background: 该研究探讨了Ia型超新星（SNe Ia）的标准化星等（standardized magnitude）与红移（redshift）之间的新经验关系及其在宇宙学模型比较和各向同性检验中的应用。

Data: 数据来源于Pantheon+和DES-SN5YR超新星样本，并在八个深场区域（deep-field regions）以及DES-Dovekie和Amalgame超新星样本中进行了验证。

Method: 采用双参数线性模型（intercept $\mathcal{M}$和slope $b$）对Ia型超新星（SNe Ia）的标准化星等（$m$）与红移（$z$）关系进行经验性参数化。

Result: 发现Ia型超新星(SNe Ia)的标准化星等($m$)与红移($z$)之间存在负线性相关关系$m-5\log(z(1+z)) \propto z$，可用截距$\mathcal{M}$和斜率$b$两参数描述，对应光度距离$d_L(z)=c\,H_0^{-1}z(1+z)10^{bz/5}$，该关系在$z\simeq1.1$范围内优于ΛCDM、平坦$w$CDM模型及(2,1)帕德逼近式。
...
### [29] [Revisiting the Great Attractor: The Local Group's streamline trajectory, cosmic velocity and dynamical fate](https://arxiv.org/abs/2601.08524)
*Richard Stiskalek,Harry Desmond,Stuart McAlpine,Guilhem Lavaux,Jens Jasche,Michael J. Hudson*

**TL;DR**: 该研究通过Manticore-Local数字孪生模拟重新审视巨引源（Great Attractor），发现155 h⁻¹Mpc内的质量仅能解释本地星系群（Local Group）宇宙速度72%的幅度且方向偏移38°，证明巨引源并非主导结构而是瞬时速度场假象，且没有单一吸引体能完全解释本地星系群在宇宙静止系中的运动。


<details>
  <summary>Details</summary>

Background: 研究聚焦于重新评估巨引源（Great Attractor）在解释本星系群（Local Group）宇宙速度、现今速度流线汇聚点及未来运动方向中的作用，揭示其并非动力学主导结构而是瞬时速度场产物。

Data: 数据来源于Manticore-Local数字孪生（digital twins）模拟套件对邻近宇宙的建模，以及新开发的Beyond-Present-Time模拟套件对局域群（Local Group）未来轨迹的演化预测。

Method: 采用Manticore-Local数字孪生（digital twins）套件对邻近宇宙进行建模，结合Beyond-Present-Time模拟套件在尺度因子a=10下演化局部宇宙实现体，通过多尺度平滑分析速度流线收敛特性。

Result: 质量在155 h⁻¹Mpc范围内仅能解释本地星系群宇宙速度的约72%，方向偏移约38°。
...
### [30] [Reconstructing Gamma Ray Burst Energy Relations with Observational H(z) data in Neural Network Framework](https://arxiv.org/abs/2601.08550)
*Nilanjana Bagchi Aurpa,Abha Dev Habib,Nisha Rani*

**TL;DR**: 利用人工神经网络(ANN)和贝叶斯神经网络(BNN)对伽马射线暴(GRB)光度关系进行无模型校准，发现BNN方法能更稳健地处理统计和系统误差，所得阿玛蒂(Amati)关系参数与低红移无模型校准结果一致。


<details>
  <summary>Details</summary>

Background: 伽马射线暴（GRB）为研究超出Ia型超新星红移范围的宇宙膨胀历史提供了强大工具。

Data: data：研究使用了来自A220和J220汇编的观测哈勃参数H(z)数据，并采用人工神经网络(ANN)和贝叶斯神经网络(BNN)进行模型无关的伽马射线暴(GRB)光度关系校准。

Method: 采用人工神经网络（ANN）和贝叶斯神经网络（BNN）框架对伽马射线暴（GRBs）光度关系进行模型无关校准，直接基于观测哈勃参数H(z)数据重建校准关系并处理统计与系统不确定性。

Result: 通过人工神经网络(ANN)和贝叶斯神经网络(BNN)对伽马射线暴(GRB)光度关系进行无模型校准，发现BNN方法提供了更稳健的框架。
...
### [31] [Primordial Gravitational Waves from Scalar Backreaction in Axion-SU(2) Inflation](https://arxiv.org/abs/2601.08622)
*Mattia Cielo,Matteo Fasiello,Alexandros Papageorgiou*

**TL;DR**: 首次数值研究了伴随衰变的旁观者色自然暴胀(SCNI)模型中强标量反作用效应，发现超过$m_Q = \sqrt{2}$阈值时标量涨落的快子不稳定性会放大扰动并加速轴子-规范场系统衰变，产生的引力波信号峰值可能位于LISA探测范围内。


<details>
  <summary>Details</summary>

Background: 研究背景涉及强标量反作用（strong scalar backreaction）在旁观者色自然暴胀（SCNI）模型中的首次数值模拟，特别关注旁观者场在暴胀期间衰变的情形。

Data: 数据来源于数值模拟（numerical simulation）研究强标量反作用在旁观者色自然暴胀（SCNI）中的动力学行为。

Method: 采用数值模拟方法研究强标量反作用（strong scalar backreaction）对背景动力学的影响，并推导引力波谱。

Result: 首次数值研究表明，强标量反作用（strong scalar backreaction）在SCNI模型中通过引入规范场背景势的有效四次项加速了轴子-规范系统的衰变。
...
### [32] [One-Loop Tensor Power Spectrum from a Non-Minimally Coupled Spectator Field during Inflation](https://arxiv.org/abs/2601.08675)
*Zhe Li,Chen Yuan,Qing-Guo Huang*

**TL;DR**: 研究了非最小耦合旁观者场（non-minimally coupled spectator field）暴胀场景中一阶张量功率谱修正，发现高斯凹陷耦合模型（Model G）对有效张标比（$r_{\mathrm{eff}}$）产生$\mathcal{O}(1)$修正，而快速振荡耦合模型（Model O）可将其提升数个量级，且原初引力波能量密度远低于Taiji/TianQin/LISA探测灵敏度。


<details>
  <summary>Details</summary>

Background: 研究探讨了暴胀（inflation）场景中非最小耦合旁观场（non-minimally coupled spectator field）对原初张量功率谱（primordial tensor power spectrum）的单圈（one-loop）修正及其对有效张量标量比（$r_{\mathrm{eff}}$）和原初引力波（primordial gravitational waves）能量密度的影响。

Data: 研究采用理论计算（simulation）方法，基于暴胀场景中非最小耦合旁观者场（non-minimally coupled spectator field）的模型设定，通过in-in形式体系推导半解析结果，未使用实际观测数据。

Method: 采用in-in形式主义计算了非最小耦合旁观者场（non-minimally coupled spectator field）暴胀场景中原始张量功率谱的完整单圈修正，推导了标量源单圈张量谱和有效张量标量比（$r_{\mathrm{eff}}$）的半解析结果。

Result: 在非最小耦合旁观者场（non-minimally coupled spectator field）的暴胀场景中，Model G对有效张标比（$r_{\mathrm{eff}}$）产生$\mathcal{O}(1)$修正，而Model O可将其提升数个量级。
...
### [33] [Evidence of sloshing-driven mini-halo formation in the cool-core cluster RXCJ1558.3-1410](https://arxiv.org/abs/2601.08757)
...
**TL;DR**: 通过对X射线冷核心星系团RXCJ1558.3-1410的多波段研究，发现其射电迷你晕（radio mini-halo）的空间分布与X射线冷锋（cold front）和冷却半径相关，表明团内介质（ICM）的晃动（sloshing）而非活动星系核（AGN）反馈是驱动迷你晕辐射的主要机制。


<details>
  <summary>Details</summary>

Background: 射电迷你晕（radio mini-halos）的起源研究聚焦于揭示X射线冷核星系团中热与非热辐射的关联机制。

Data: 数据来源包括钱德拉X射线天文台（Chandra X-ray）的存档数据和超宽频带uGMRT射电数据（Bands 3、4和5）。

Method: 采用钱德拉X射线和uGMRT宽频带射电数据（波段3、4和5）进行多波段分析，结合X射线空腔（cavity）和冷锋（cold front）特征识别，并通过机械功率计算与辐射冷却平衡评估。

Result: 在RXCJ1558.3-1410星系团中，射电迷你晕（radio mini-halo）的空间分布与X射线特征（如冷锋和冷却半径）的关联表明，星系团内介质（ICM）的晃动（sloshing）而非活动星系核（AGN）反馈是驱动射电迷你晕辐射的主要机制。
...
### [34] [Quantifying Symmetry: Transformation Information for Planetary Nebulae and Supernova Remnants](https://arxiv.org/abs/2601.07913)
*Dmitry Shishkin,Amir Michaelis*

**TL;DR**: 提出基于变换信息（Transformation Information, TI）的天体图像对称性定量识别方法，通过计算图像与几何变换版本间的交叉熵（Kullback-Leibler divergence）定位局部极小值来识别旋转/反射对称轴，验证其在行星状星云（bipolar/multipolar lobes）和超新星遗迹（Type Ia与core-collapse）形态分类中的有效性。


<details>
  <summary>Details</summary>

Background: 开发了一种基于变换信息（Transformation Information）的天体图像对称性定量识别方法，用于分析行星状星云和超新星遗迹的形态学特征。

Data: 研究中使用的数据来源包括行星状星云（planetary nebulae）和超新星遗迹（supernova remnants）的天体物理图像，以及一个用于验证的风玫瑰图案（wind-rose pattern）。

Method: 基于变换信息（Transformation Information）的定量对称性识别流程，通过计算图像及其旋转或反射对应部分在重叠域上的Kullback-Leibler散度（交叉熵）来度量自相似性。

Result: 基于变换信息（Transformation Information, TI）的非参数化对称性识别框架可有效量化天体图像的自相似性，并通过局部极小值检测旋转或反射对称轴。
...
### [35] [The LIRA-Ising Model: Estimating the boundaries of irregularly shaped X-ray sources](https://arxiv.org/abs/2601.08002)
...
**TL;DR**: 提出了一种结合LIRA贝叶斯多尺度重建和伊辛(Ising)模型的三步法(LIRA-Ising)，用于精确识别低表面亮度X射线图像中不规则天体(如高红移类星体PKS J1421-0643和0730+257喷流)的边界和形态结构。


<details>
  <summary>Details</summary>

Background: 研究主题为开发贝叶斯方法（Bayesian method）精确界定低表面亮度（low surface brightness）X射线图像中不规则扩展源（extended source）的形态边界。

Data: 使用钱德拉X射线天文台(Chandra X-ray Observatory)的观测数据对高红移类星体PKS J1421-0643和0730+257的X射线喷流进行成像分析，并通过模拟研究验证方法。

Method: 采用贝叶斯多尺度重建算法LIRA结合伊辛模型(Ising model)的三步法(LIRA-Ising)进行边界识别。

Result: 开发了一种结合LIRA贝叶斯多尺度重建和伊辛(Ising)模型的三步贝叶斯方法(LIRA-Ising)，可有效识别低表面亮度X射线图像中不规则源的边界。
...
### [36] [Evaluating the effectiveness of radio frequency interference removal algorithms for single pulse searches](https://arxiv.org/abs/2601.08351)
...
**TL;DR**: 论文通过模拟多种射电频率干扰(RFI)环境并评估IQRM、SKF和ZDMF等主流RFI消除方法对注入脉冲的恢复效果，为下一代望远镜（如SKAO）实时搜索脉冲星和射电暂现源（如RRATs和FRBs）提供了最优RFI抑制算法选择依据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于射电频率干扰（RFI）对脉冲星和射电暂现源（如旋转射电暂现源RRATs和快速射电暴FRBs）搜索的挑战及其在下一代望远镜（如SKAO）实时数据处理中的关键影响。

Data: 数据来源于模拟多种射电频率干扰（RFI）环境的注入脉冲数据。

Method: 采用脉冲注入模拟不同射电频率干扰（RFI）环境并结合单脉冲搜索流程，重点评估了IQRM、SKF和ZDMF等主流RFI剔除算法的效能与副作用。

Result: 射频干扰（RFI）抑制算法（如IQRM、SKF和ZDMF）在不同脉冲亮度、宽度和色散量（DM）条件下对模拟数据中注入脉冲的恢复效果存在差异。
...
### [37] [Polychromatic pyramid wavefront sensor with MKID technology for high contrast imaging](https://arxiv.org/abs/2601.08398)
*Aurélie Magniez,Charlotte Z. Bond,Peter Wizinowich,TIm Morris,Kieran O'Brien*

**TL;DR**: 提出了一种利用宽波长范围（800-1800 nm）的多色金字塔波前传感器（polychromatic pyramid wavefront sensor），相比单波段传感器将极限星等提高了1-2等、对比度提升了1.5-4倍，并通过优化重构器补偿光学增益以最小化噪声传播。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用多色金字塔波前传感器（polychromatic pyramid wavefront sensor）提升高对比度自适应光学系统的波前传感性能，以应对未来极大望远镜（Extremely Large Telescope）等系统对更高性能的需求。

Data: 数据来源为基于凯克二号（Keck II）自适应光学系统的端到端仿真（end-to-end simulation）结果。

Method: 采用多色金字塔波前传感器（polychromatic pyramid wavefront sensor）技术，通过利用更宽波长范围（800-1800 nm）的额外信息跟踪光学增益，并在优化重构器中补偿光学增益以最小化噪声传播。

Result: 多色金字塔波前传感器（polychromatic pyramid wavefront sensor）通过利用更宽波长范围（800-1800 nm）的额外信息，将极限星等提高了1至2个星等，对比度提升了1.5至4倍，相较于单波段金字塔波前传感器。
...
### [38] [Blind Deconvolution in Astronomy: How Does a Standalone U-Net Perform?](https://arxiv.org/abs/2601.08666)
*Jean-Eric Campagne*

**TL;DR**: 研究发现U-Net架构无需点扩散函数(PSF)或噪声特性先验知识即可实现天文图像的端到端盲解卷积(blind deconvolution)，在低峰值信噪比(PSNR)/中等结构相似性(SSIM)条件下表现优于传统Tikhonov方法，且训练数据超过5,000张时性能趋于稳定，能有效适应未见的视宁度(seeing)和噪声条件。


<details>
  <summary>Details</summary>

Background: 研究探讨了U-Net架构在缺乏点扩散函数(PSF)和噪声特性先验知识的情况下，独立完成天文图像端到端盲解卷积的可行性及其性能表现。

Data: 数据来源为使用GalSim工具包模拟的天文观测数据，结合了随机变换、点扩散函数（PSF）卷积（包含光学和大气效应）及高斯白噪声，并基于COSMOS Real Galaxy Dataset生成最多40,000张48x48尺寸的图像作为训练集。

Method: 采用U-Net架构结合均方误差（MSE）损失函数进行端到端盲反卷积（blind deconvolution），通过模拟真实天文观测（包括随机变换、点扩散函数（PSF）卷积和高斯白噪声）生成训练数据集，并利用峰值信噪比（PSNR）、结构相似性（SSIM）和余弦相似性（cosine similarity）指标评估模型性能。

Result: U-Net架构在无点扩散函数（PSF）或噪声先验知识下能有效执行端到端盲解卷积，性能随训练图像数量增加而提升并在超过5,000张图像后趋于饱和，且在低峰值信噪比（PSNR）/中等结构相似性（SSIM）条件下优于传统Tikhonov方法，同时展现出对未见过的大气视宁度（seeing）和噪声水平的良好泛化能力。
...
### [39] [Constraining the Fraction of LIGO/Virgo/KAGRA Binary Black Hole Merger Events Associated with Active Galactic Nucleus Flares](https://arxiv.org/abs/2601.08286)
*Liang-Gui Zhu,Lei He,Xian Chen,Wen Zhao*

**TL;DR**: 通过分析GWTC-4.0中80个双黑洞（BBH）并合事件与ZTF DR23数据中活动星系核（AGN）耀斑的时空关联，发现耀斑关联比例$f_{\rm flare}$为0.07（90%置信区间），其中GW190412事件与候选耀斑J143041.67+355703.8的时空一致性支持了AGN盘形成通道的理论预期。


<details>
  <summary>Details</summary>

Background: 研究探讨了LIGO/Virgo/KAGRA (LVK)网络探测到的双黑洞(BBH)并合事件的形成机制及其与活动星系核(AGN)盘中气体相互作用产生可观测光学耀斑的潜在关联。

Data: 数据来源于引力波事件（GWTC-4.0）和来自兹威基瞬变设施（ZTF）DR23数据的活动星系核（AGN）耀发观测数据。

Method: 采用三维沃罗诺伊镶嵌（3D Voronoi tessellation）方法估计局部耀斑数密度，并结合事件的三维定位构建耀斑关联比例（$f_{\rm flare}$）的似然函数，同时明确考虑巡天边界和不完整星表覆盖的影响。

Result: 通过分析GWTC-4.0中的双黑洞（BBH）并合事件与ZTF DR23数据中活动星系核（AGN）耀发的时空相关性，推断出耀发关联比例$f_{\rm flare}$为$0.07_{-0.05}^{+0.25}$（90%置信水平），其中GW190412事件存在一个时空一致的耀发候选体（J143041.67+355703.8）。
...
### [40] [Evolution and afterglow emission of gamma-ray burst jets from binary neutron star mergers](https://arxiv.org/abs/2601.08714)
*Emma Dreas,Om Sharan Salafia,Andrea Pavan,Riccardo Ciolfi,Annalisa Celotti*

**TL;DR**: 研究发现双中子星并合(BNS)后产生的相对论性喷流(relativistic jets)在穿越非均匀各向异性环境时会形成复杂的多激波结构，导致喷流角度和速度分布不对称，并在离轴观测的短伽马暴(GRB)余辉光变曲线中产生早期暗弱峰值特征，所有模拟结果与GW170817多信使观测数据相符。


<details>
  <summary>Details</summary>

Background: 研究主题为双中子星(BNS)并合中相对论性喷流(relativistic jets)在非均匀各向异性环境中的结构形成与演化及其对短伽马暴(GRBs)余辉的影响。

Data: 使用三维相对论磁流体力学（3D relativistic magnetohydrodynamic）模拟研究了双中子星并合（BNS merger）后环境中喷流的形成与演化，模拟考虑了不同喷射条件并持续到动力学演化可忽略的弹道阶段（ballistic regime）。

Method: 采用三维相对论磁流体力学（3D relativistic magnetohydrodynamic）模拟结合半解析（semi-analytic）多波段余辉光变曲线计算方法，研究非均匀各向异性（non-homogeneous, anisotropic）双中子星并合（BNS merger）环境中喷流的形成与演化。

Result: 双中子星并合(BNS merger)环境中喷流的非均匀各向异性传播会形成复杂的多激波突破结构，并在离轴余辉光变曲线中留下明显印记。
...
### [41] [Not so Swift: 20 years of multiwavelength observations of Mrk 421 and Mrk 501](https://arxiv.org/abs/2601.08344)
*Gabrielle L. Taylor,Stefan J. Wagner,Alicja Wierzcholska,Michael Zacharias*

**TL;DR**: 基于20年Swift望远镜数据研究发现，耀变体(blazar) Mrk 421和Mrk 501在同步辐射能段(光学/紫外/X射线)呈现能量依赖的变异性，X射线光子指数遵循"越亮越硬"趋势，仅在Mrk 501的X射线波段发现约390天准周期信号(>3σ)，两源的光学/紫外与X射线波段无时延相关性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于耀变体（blazar）Mrk 421和Mrk 501在同步辐射能段（光学、紫外和X射线）的长期变异性特征，以评估当前关于这类天体及其物理过程的模型。

Data: 使用Swift-UVOT和Swift-XRT望远镜2005-2025年长达20年的观测数据。

Method: 采用结构函数（structure functions）、Lomb-Scargle周期图（Lomb-Scargle periodograms）和离散相关函数（discrete correlation functions）进行时间序列分析，并考察了光谱模型、分数变异性（fractional variabilities）、流量分布及X射线光子指数与流量的关系。

Result: Mrk 421和Mrk 501在所有研究波段均表现出能量依赖的显著变异性，且流量分布呈现对数正态性趋势。
...
### [42] [Ghost-free non-local $F(R)$ Gravity Compatible with ACT](https://arxiv.org/abs/2601.07879)
*Shin'ichi Nojiri,S. D. Odintsov,V. K. Oikonomou*

**TL;DR**: 研究证实无鬼非局域F(R)引力理论（ghost-free non-local F(R) gravity）中的幂律模型和R²模型均符合ACT及Planck/BICEP对标量扰动谱指数（spectral index）和张标比（tensor-to-scalar ratio）的最新观测约束，并能导出德西特解（de Sitter solution）且等价于F(R,φ)引力理论。


<details>
  <summary>Details</summary>

Background: 研究探讨无鬼（ghost-free）非局域（non-local）F(R)引力理论与宇宙微波背景辐射（ACT）及引力波（Planck/BICEP）观测数据的兼容性。

Data: 研究中使用的数据来源于阿塔卡马宇宙学望远镜（ACT）对标量扰动谱指数的观测结果，以及普朗克/BICEP实验对张标比（tensor-to-scalar ratio）的最新约束。

Method: 采用幽灵自由(ghost-free)非局域(non-local) $F(R)$ 引力理论框架，结合幂律模型和$R^2$模型进行理论分析。

Result: 在无鬼非局域F(R)引力框架下，幂律模型和R²模型均与ACT及更新后的Planck/BICEP对标量扰动谱指数和张量-标量比的约束兼容。
...
### [43] [The Scalar Mach-Sciama Theory of Gravitation](https://arxiv.org/abs/2601.07904)
*Velásquez-Toribio,A. M*

**TL;DR**: 在伯格曼-瓦格纳(Bergmann-Wagoner)标量-张量引力框架内实现了西亚马(Sciama)马赫原理的标量化表述，通过不变集$\{{\cal I}_1,{\cal I}_2,{\cal I}_3,\hat g_{μν}\}$构建了与共形框架无关的马赫条件，并采用推迟响应机制将背景标量演化与哈勃区域内物质分布因果关联，在保持弱场检验一致性的同时实现了宇宙惯性尺度的马赫式因果决定。


<details>
  <summary>Details</summary>

Background: 该研究在伯格曼-瓦格纳(Bergmann-Wagoner)类标量-张量引力理论框架下，探索了实现夏马(Sciama)马赫原理(Machian programme)的标量化方案。

Data: 数据来源于广义伯格曼-瓦格纳标量-张量引力理论中普遍共形耦合物质场的场方程重写形式，以及空间平坦FLRW宇宙中光速缓变状态下的显式时间核函数。

Method: 采用标量-张量引力（scalar-tensor gravity）的伯格曼-瓦格纳（Bergmann-Wagoner）类模型，通过不变集$\{{\cal I}_1,{\cal I}_2,{\cal I}_3,\hat g_{μν}\}$重写场方程，并基于因果选择规则限定不变标量方程的解空间，仅保留物质分布的推迟响应且剔除无源贡献。

Result: 在广义伯格曼-瓦格纳标量-张量引力框架内，通过不变集$\{{\cal I}_1,{\cal I}_2,{\cal I}_3,\hat g_{μν}\}$实现了与共形框架无关的Sciama马赫原理因果要求，其允许解仅为因果过去物质分布的推迟响应且剔除无源贡献。
...
### [44] [Stellar Rotation in the First Six Million Years: Rotational Velocities and Radii Estimates of T Tauri Stars in IC 5070 and IC 348](https://arxiv.org/abs/2601.08048)
*Laurin M. Gray,Katherine L. Rhode,Luisa M. Rebull*

**TL;DR**: 通过对IC 5070和IC 348中T Tauri星(TTSs)的高分辨率光谱分析，发现原行星盘(circumstellar disk)相互作用会减缓TTSs自转速度，且IC 348中无盘(Class III)慢速自转星比例较高可能与近期盘耗散有关，同时双星系统比单星系统更易成为快速自转体，而引入星斑(starspot)覆盖度的恒星演化模型能更准确解释半径膨胀现象并与年龄估计相符。


<details>
  <summary>Details</summary>

Background: 研究聚焦于金牛T型星（TTSs）在疏散星团中的自转速度及其与星周盘（circumstellar disk）状态、双星性的关联，旨在揭示恒星早期演化阶段的自转动力学机制。

Data: 数据来源包括使用WIYN 3.5米望远镜上的Hydra设备获取的IC 5070和IC 348中T Tauri星（TTSs）的高分辨率光学光谱数据，以及已发表的恒星温度、光度、自转周期、星周盘状态和双星性参数。

Method: 采用高分辨率光学光谱（high-resolution optical spectroscopy）结合最大似然法（maximum likelihood method）分析恒星自转速度（v sin i）和半径，并对比恒星演化模型（stellar evolution models）以评估星斑覆盖（starspot coverage）对半径膨胀的影响。

Result: 发现与星周盘（circumstellar disk）相互作用的T Tauri星（TTSs）比两个星团中的III类星（Class III stars）旋转更慢的证据。
...
### [45] [Substellar population of the young massive cluster RCW 36 in Vela](https://arxiv.org/abs/2601.08555)
...
**TL;DR**: 通过结合新观测与深度学习算法，首次测得年轻大质量星团RCW 36的初始质量函数（IMF）呈现分段幂律分布（0.20-20太阳质量α=1.62±0.03，0.03-0.20太阳质量α=0.46±0.14），发现恒星与褐矮星（BD）比例2-5，并检测到可能的原初质量分层现象。


<details>
  <summary>Details</summary>

Background: 研究主题为探讨年轻大质量星团RCW 36的初始质量函数（IMF）普适性及其质量分层现象。

Data: 数据来源包括HAWK-I/VLT的新GLAO观测、2MASS和SOFI/NTT的存档数据以及Gaia DR3的运动学数据。

Method: 结合GLAO观测(HAWK-I/VLT)、归档数据(2MASS、SOFI/NTT)和盖亚(Gaia)DR3运动学数据，利用深度学习算法\textsc{DeNeb}去除复杂星云发射以提高测光精度和源提取，通过颜色-星等图与控制场比较分配成员概率，并采用模型等龄线估计恒星质量。

Result: 在年轻大质量星团RCW 36中首次测得初始质量函数（IMF）呈现分段幂律分布，高质量段（0.20-20 M⊙）斜率α=1.62±0.03，低质量段（0.03-0.20 M⊙）斜率α=0.46±0.14。
...
### [46] [The drastic impact of Eddington-limit induced mass ejections on massive star populations](https://arxiv.org/abs/2601.08822)
*D. Pauli,N. Langer,A. Schootemeijer,P. Marchant,H. Jin,A. Ercolino,A. Picco,R. Willcox,H. Sana*

**TL;DR**: 提出了一种适用于一维恒星演化代码的物理驱动且经验校准的质量损失（mass-loss）方法，通过模拟大质量恒星在接近爱丁顿极限（Eddington limit）时的质量抛射效应，解决了当前恒星演化模型中缺乏此类机制的问题，并成功复现了大麦哲伦云（Large Magellanic Cloud）和小麦哲伦云（Small Magellanic Cloud）中观测到的恒星种群关键特征，包括汉弗莱-戴维森极限（Humphreys-Davidson limit）外无恒星存在、红超巨星（RSG）光度上限、最暗单颗沃尔夫-拉叶星（WR）、各类恒星绝对数量等观测现象，同时结合双星模型解释了O型星70%和WR星40%的双星比例。


<details>
  <summary>Details</summary>

Background: 大质量恒星（massive stars）的演化及其电离反馈机制是理解宇宙化学演化的核心问题，当前模型在爱丁顿极限（Eddington limit）下的物质抛射（如明亮蓝变星LBV阶段）仍存在显著理论空白。

Data: 数据来源于使用1D恒星演化代码MESA计算的单星模型网格，并与大麦哲伦云（Large Magellanic Cloud）和小麦哲伦云（Small Magellanic Cloud）中观测到的恒星群体进行比较。

Method: 采用一维恒星演化代码MESA并实现新的质量损失处方（mass-loss prescription），以模拟恒星在达到爱丁顿极限（Eddington limit）时因过度膨胀而发生的质量抛射效应。

Result: 提出的新质量损失（mass-loss）方案在单星模型中成功复现了观测到的恒星群体关键特征，包括汉弗莱-戴维森极限（Humphreys-Davidson limit）外无恒星存在、红超巨星（RSG）光度上限、最暗单星沃尔夫-拉叶星（WR）、特定恒星类型数量统计等，缓解了大质量恒星演化模型与观测间的矛盾。
...
### [47] [Charting the Flavour Structure of Dark Matter](https://arxiv.org/abs/2601.07921)
*Simone Biondini,Admir Greljo,Xavier Ponce Díaz,Alessandro Valenti*

**TL;DR**: 论文通过味对称性（flavour symmetry）框架系统研究了t道热暗物质（t-channel thermal dark matter）的味结构限制，发现作为味单态（flavour singlets）的费米型暗物质与标量媒介子在夸克亲和（q_L）或轻子亲和（e_R）基准场景下，其残留丰度与味观测、直接探测及对撞机数据的联合约束仍允许显著偏离味对称极限。


<details>
  <summary>Details</summary>

Background: 研究主题为探索符合当前味物理和直接探测限制的t道热暗物质（t-channel thermal dark matter）的味结构及其与味对称性（flavour symmetries）破缺模式的关系。

Data: 研究中使用的数据来源包括现有味物理实验数据、直接探测实验数据以及对撞机搜索数据。

Method: 采用味对称性（flavour symmetry）及其破缺模式的框架分析暗物质（dark matter）的味结构。

Result: 热暗物质（thermal dark matter）的t道（t-channel）味结构在味对称性（flavour symmetries）及其破缺模式下仍与当前味物理和直接探测限制兼容。
...
### [48] [Democratic heliocentric coordinates underestimate the rate of instabilities in long-term integrations of the Solar System](https://arxiv.org/abs/2601.08019)
*Hanno Rein,Kavi Dey,Daniel Tamayo*

**TL;DR**: 研究发现民主日心坐标系（DHC）在数天步长下能抑制水星轨道不稳定性，但雅可比坐标（Jacobi coordinates）在高偏心轨道下更可靠。


<details>
  <summary>Details</summary>

Background: Wisdom-Holman (WH)积分器是用于行星系统长期N体模拟的辛算子分裂方法，其科学背景涉及通过不同坐标选择（如雅可比坐标或民主日心坐标）优化哈密顿量分裂以提高数值稳定性。

Data: 数据来源于使用民主日心坐标（DHC）和雅可比坐标（Jacobi coordinates）进行的数值实验，时间步长从几天到约0.6天不等，以模拟太阳系长期动力学演化。

Method: 采用民主日心坐标系（DHC）和雅可比坐标系（Jacobi coordinates）进行哈密顿量分裂（Hamiltonian splitting）的辛算子分裂方法（symplectic operator-splitting methods）。

Result: 民主日心坐标系(DHC)分裂在典型时间步长(数天)下会引入偏心相关的人工数值进动(precession)，从而抑制水星轨道不稳定性。
...
### [49] [Anatomy of Empirical Transit Spectra of Mars based on TGO/NOMAD](https://arxiv.org/abs/2601.08397)
...
**TL;DR**: 基于火星季节性实测透射光谱（transit spectra）发现25公里以下大气因微米级尘埃和水冰云（water ice clouds）不透明，2.7-2.8微米处CO2吸收特征和3.1微米处亚微米中层水冰云（sub-micron-sized mesospheric water ice clouds）特征随季节变化，为类火星系外行星（如TRAPPIST-1f）大气探测提供了新途径。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用凌星光谱(transit spectroscopy)技术探测系外行星大气结构，特别关注气溶胶(aerosols)对重建大气特性的影响，并以火星为研究对象探索季节性大气特征变化。

Data: 数据来源于ExoMars/TGO上的NOMAD（Nadir and Occultation for Mars Discovery）太阳掩星通道（Solar Occultation channel），覆盖波长范围为0.2-0.65微米和2-4微米。

Method: 基于ExoMars/TGO的NOMAD太阳掩星通道(0.2-0.65和2-4微米波段)数据，采用经验合成方法构建火星季节性透射光谱，重点分析气溶胶(尘埃和水冰云)对光谱特征的影响。

Result: 火星大气在25公里以下因微米级尘埃和水冰云的存在基本不透明，显著削弱了光谱特征；光谱在2.7-2.8微米处呈现CO2吸收特征，在3.1微米附近显示亚微米级中间层水冰云特征及连续谱斜率，且这些特征振幅随火星季节变化，尘暴会减弱CO2特征并增强水冰特征。
...
### [50] [Modeling Cosmogenic 10Be During the Heliosphere's Encounter with an Interstellar Cold Cloud](https://arxiv.org/abs/2601.07983)
*Anna Nica,Merav Opher,Jesse Miller,Jennifer L. Middleton*

**TL;DR**: 研究发现太阳穿越巨型冷星际云（cold clouds）期间，氦球层（heliosphere）被压缩至地月轨道范围内，导致地球大气中宇宙成因核素10Be产率显著升高，通过分析海洋沉积物和铁锰结壳（iron-manganese crusts）中10Be记录可探测此类事件，其中氦球层压缩持续约100年的事件在海洋沉积物中可识别，而跨越时间达0.1-1 Myr的秒差距（parsecs）级尺度冷云事件可通过铁锰结壳测量检出。


<details>
  <summary>Details</summary>

Background: 研究探讨了太阳穿越冷星际云时对地球大气中宇宙成因核素铍-10（10Be）生成的影响及其在地质记录中的可检测性。

Data: 数据来源于对地球大气中宇宙成因核素10Be的建模分析，并结合海洋沉积物（ocean sediment）和铁锰结壳（iron-manganese crusts）中10Be含量的实测记录。

Method: 采用蒙特卡洛模拟（Monte Carlo simulation）和大气传输模型（atmospheric transport model）计算了不同星际冷云（cold cloud）相遇情景下地球大气中宇宙成因核素10Be的产率。

Result: result：AU尺度（astronomical unit-scale）的冷云相遇事件可通过海洋沉积物中的10Be测量检测到，前提是地球处于压缩日球层内部；典型相对速度下穿越时间约100年，而可检测的冷云延伸尺度需达到秒差距至数十秒差距（穿越时间0.1-1百万年）。
...
### [51] [Dust Properties of the Interstellar Object 3I/ATLAS Revealed by Optical and Near-Infrared Polarimetry](https://arxiv.org/abs/2601.08591)
...
**TL;DR**: 首次在近红外波段对星际天体3I/ATLAS进行偏振观测，发现其线性偏振度随相位角和波长的变化特征显著不同于太阳系彗星，表明其由亚微米级单体组成的尘埃聚集体具有原始星际尘埃的光学特性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际天体3I/ATLAS的偏振特性及其与太阳系典型彗星的差异，旨在揭示其原始尘埃成分和形成于其他行星系统的演化历史。

Data: 数据来源为使用成像偏振仪（imaging polarimeters）在可见光RC波段（0.64 μm）至近红外KS波段（2.25 μm）对星际天体3I/ATLAS进行的独立偏振观测，包括首次近红外偏振测量。

Method: 采用成像偏振仪（imaging polarimeters）测量了从可见光RC波段（0.64 μm）到近红外KS波段（2.25 μm）的线偏振度，并分析了其随太阳相位角（偏振相位曲线；PPC）和波长（偏振颜色曲线；PCC）的变化关系。

Result: 首次在近红外波段测得星际天体3I/ATLAS的偏振特性，其偏振相位曲线(PPC)振幅异常大且不同于太阳系典型彗星，表明其异常偏振行为由耐火尘埃颗粒固有光学性质驱动而非瞬时挥发活动。
...
### [52] [Rare Near-Opposition Alignment of 3I/ATLAS on 22 January, 2026](https://arxiv.org/abs/2601.08624)
*Mauro Barbieri,Abraham Loeb*

**TL;DR**: 2026年1月22日星际天体3I/ATLAS将与日地轴形成0.69度的罕见小角度对齐，为首次精确测量星际彗星尘埃的反冲效应（opposition surge）和偏振特性提供了独特机遇。


<details>
  <summary>Details</summary>

Background: 研究聚焦于2026年1月22日星际天体3I/ATLAS与地日轴罕见对齐（alpha=0.69度）带来的独特科学机遇，为测量星际彗星尘埃的反冲效应（opposition surge）和偏振特性提供了理想条件。

Data: 数据来源于对星际物体3I/ATLAS在2026年1月22日与地球-太阳轴对齐时的观测（observation）数据，包括对齐前后获取的尘埃反冲（opposition surge）和偏振（polarimetric）特性测量数据。

Method: 采用几何构型分析和极坐标测量技术（polarimetric measurement）研究星际尘埃的反向散射（opposition surge）和偏振特性。

Result: 2026年1月22日星际天体3I/ATLAS将与地日轴形成极小的0.69度夹角，为测量星际彗星尘埃的反冲效应（opposition surge）和偏振特性提供了独特机会。
