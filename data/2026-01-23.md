...
**TL;DR**: 提出了一种基于二维投影星流（stellar streams）的层次贝叶斯框架（hierarchical Bayesian framework），通过快速模拟工具StreaMAX结合群体分布统计，首次证明仅凭外部星系的光测星流数据即可在群体层面准确约束暗物质晕（dark matter halo）的形态分布（如扁球、球状或长球）。


<details>
  <summary>Details</summary>

Background: 研究主题为利用恒星流（stellar streams）的二维投影轨迹推断外部星系暗物质晕（dark matter halo）形态的群体分布。

Data: 数据来源于使用StreaMAX（一种基于JAX加速的粒子喷射包）对恒星流（stellar streams）进行正向建模生成的模拟（simulation）数据，并结合外部星系中低表面亮度特征（即二维测光数据）的观测数据。

Method: 采用分层贝叶斯框架（hierarchical Bayesian framework）结合JAX加速的粒子喷射包StreaMAX进行暗物质晕形态的群体分布推断，通过前向建模投影星流轨迹并分层重加权后验分布约束晕扁平度（flattening）的总体分布。

Result: 提出的分层贝叶斯框架仅利用二维投影星流轨迹即可推断暗物质晕（dark matter halo）扁平度的群体分布，并通过层次重加权整合个体后验，实现对暗物质晕形态（如扁球、球状或长球状）群体分布的准确约束。
...
**TL;DR**: 该研究通过决策树回归算法分析LOFAR巡天数据，发现局域($z\,\leq$\,0.3)恒星形成星系中射电连续谱-恒星形成率(RC--SFR)关系主要受恒星形成率(SFR)和恒星质量(stellar mass)影响（显著性>5σ），并提供了新的RC--SFR校准关系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用射电连续谱-恒星形成率(RC--SFR)关系探究星系属性(如恒星质量)如何影响该关系的普适性。

Data: 数据来源于LOFAR两米巡天项目第二版数据（LoTSS-DR2）的射电连续谱观测，结合斯隆数字巡天第八版数据（SDSS-DR8）的光谱数据筛选出18,828个发射线分类的恒星形成星系（SFGs），并补充使用GALEX-SDSS-WISE遗产目录（GSWLC）的光度谱能量分布拟合（SED-fitting）获得的恒星形成率和恒星质量数据。

Method: 采用非参数决策树回归算法（non-parametric decision-tree regression algorithm）分析射电连续谱-恒星形成率（RC--SFR）关系对星系属性的依赖性。

Result: 在本地（$z\,\leq$\,0.3）恒星形成星系（SFGs）中，恒星形成率（SFR）对预测射电光度最为重要，其次是恒星质量，显著性超过$5σ$。
...
### [3] [Optimizing Optical Searches for Supermassive Black Hole Binaries in AGN Light Curves: Fourier versus Bayesian Periodicity Detection](https://arxiv.org/abs/2601.15379)
*Sebastian Banaszak,Caitlin Witt,Adam Miller*

**TL;DR**: 研究发现嵌套贝叶斯采样器(NBS)结合广义Lomb Scargle周期图(GLSP)能有效检测超大质量黑洞双星(SMBHBs)的周期性光度变化，在LSST类模拟中检测率最高(~40%)且假阳性率最低(~0.5%)。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于超大质量黑洞双星（SMBHBs）周期性亮度变化的探测方法优化及其在活动星系核（AGN）随机变率背景下的识别效能。

Data: 数据来源于模拟的光变曲线，其特性基于卡塔利娜实时瞬变巡天（CRTS）、时空遗产巡天（LSST）和兹威基瞬变设施（ZTF）的观测数据。

Method: 采用广义伦巴-斯卡格周期图（GLSP）、嵌套贝叶斯采样器（NBS）和加权小波Z变换（WWZ）三种方法分析叠加了阻尼随机游走（DRW）活动星系核（AGN）变率和轨道运动正弦信号的超大质量黑洞双星（SMBHB）光变曲线周期信号。

Result: 超大质量黑洞双星(SMBHBs)的周期性亮度变化可通过嵌套贝叶斯采样器(NBS)和广义Lomb Scargle周期图(GLSP)联合筛选方法以最低假阳性率和最高检测率识别。
...
### [4] [Blazars define a stable celestial reference frame](https://arxiv.org/abs/2601.15381)
*Nathan Secrest,Sebastien Lambert*

**TL;DR**: 研究发现基于高光度变化活动星系核（AGN）构建的S/X波段参考架稳定性比低变化类高6倍，且通过光度变化优化加权可减少光学-射电参考架间失真。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过活动星系核(AGN)的光度变异性优化光学与射电天球参考架(optical-radio celestial reference frames)的链接稳定性。

Data: 数据来源于ICRF3（国际天球参考架第三版）与Gaia DR3（盖亚数据发布第三版）的匹配数据，并采用自举法（bootstrap method）评估不同光度变异性水平下活动星系核（AGN）构建参考架的多历元稳定性。

Method: 采用自助法（bootstrap method）估计不同光度变异性水平活动星系核（AGN）构建参考框架的多历元稳定性，并拟合矢量球谐函数（vector spherical harmonics）量化统计离散度。

Result: 基于高光度变化活动星系核(AGN)构建的参考框架比未考虑光度变化的框架稳定性更高，其中S/X波段采用耀变体(blazar)构建的框架稳定性是类星体(quasar)框架的6倍且比ICRF3定义源高2倍。
...
### [5] [A Stratification in Magnetic Field Structures: The Radio Outflow in NGC 4151](https://arxiv.org/abs/2601.15382)
*Salmoli Ghosh,P. Kharb,E. Costantini,J. Gallimore,D. Williams-Baldwin,M. Mehdipour*

**TL;DR**: 通过高分辨率偏振观测发现赛弗特星系NGC 4151存在分层射电外流结构（"脊柱-鞘层"构型和双锥形"风"），其磁场呈螺旋形态且由磁力驱动，证实射电宁静活动星系核(AGN)虽喷流较弱但仍可能以磁主导机制为主。


<details>
  <summary>Details</summary>

Background: 射电宁静活动星系核(radio-quiet AGN)中射电外流的物理本质仍是未解难题。

Data: 数据来源于卡尔·G·扬斯基甚大阵(VLA)的B阵列在3 GHz和10 GHz波段对塞弗特星系NGC 4151进行的千秒尺度偏振观测。

Method: 采用卡尔·G·扬斯基甚大阵(VLA)的B阵列在3和10 GHz进行千秒差距尺度偏振观测，结合旋转测量(RM)分析和X射线数据比对，推断射流磁驱动机制。

Result: 在10 GHz高分辨率(0.5角秒)图像中观测到NGC 4151射电喷流存在"脊柱-鞘层"(spine-sheath)结构的磁场分层，其中脊柱区域磁场方向垂直于喷流而鞘层区域平行于喷流。
...
### [6] [An Investigation of 5-year Simultaneous X-ray and Radio Light Curves of the Dwarf Seyfert Galaxy UGC 6728](https://arxiv.org/abs/2601.15383)
*Krista Lynne Smith,Macon Magno,Michael Koss*

**TL;DR**: 对矮赛弗特星系UGC 6728的5年同步射电和X射线观测发现，X射线耀斑期间光谱硬度比不变而射电流量在耀斑结束后下降一半，支持吸积盘磁重联喷射等离子体暂时遮挡射电源的解释。


<details>
  <summary>Details</summary>

Background: 研究聚焦于矮赛弗特星系（dwarf Seyfert galaxy）UGC 6728的射电和X射线辐射特性，探讨其与吸积盘磁重联（magnetic reconnection）活动相关的暂现性等离子体团（blob of plasma）遮挡现象及其与X射线耀发的能量学关联。

Data: 数据来源于对矮赛弗特星系UGC 6728长达5年的偶然同步射电和X射线光变曲线观测。

Method: 采用射电和X射线光度曲线同步分析方法，结合磁重联（magnetic reconnection）驱动的吸积盘等离子体抛射模型解释观测现象。

Result: X射线光变曲线呈现先耀发后逐渐上升再下降的特征，且硬度和光谱未发生显著变化。
...
**TL;DR**: 通过分析斯蒂芬五重星系（Stephan's Quintet）中NGC 7319的双锥形窄线区（narrow-line region）外流模型，首次区分了活动星系核（AGN）驱动的外流（径向速度达550 km/s）、星系旋转和引力驱动的潮汐流（tidal flows），并发现2.4-6.3 kpc处存在AGN外流向潮汐流的转变。


<details>
  <summary>Details</summary>

Background: 致密星系群（compact galaxy groups）是研究活动星系核（AGN）与邻近星系间相互作用效应的理想实验室。

Data: 数据来源为使用基特峰俄亥俄州立大学多目标光谱仪（Kitt Peak Ohio State Multi-Object Spectrograph）在阿帕奇点天文台（Apache Point Observatory）获取的斯蒂芬五重星系（Stephan's Quintet）多位置长缝光谱的空间分辨数据。

Method: 采用多高斯拟合Hα和[N II]发射线分离不同运动学成分，结合星系旋转模型、双锥流出模型和电离气体运动学图，区分了星系群中的流出、旋转和潮汐流。

Result: 在Stephan五重奏中观测到AGN驱动的外流径向速度峰值达550 km/s（距中心2.6 kpc），并在2.4-6.3 kpc投影距离处发现向引力驱动潮汐流的转变。
...
### [8] [The role of gas stripping in the quenching of satellite galaxies using SHARK v2.0](https://arxiv.org/abs/2601.15435)
*Megan K. Oxland,Matías Bravo,Laura C. Parker,Claudia del P. Lagos*

**TL;DR**: 结合SDSS观测和SHARK v2.0半解析模型校准了星系群和星系团中卫星星系（satellite galaxies）的熄灭（quenching）过程，发现降低热晕（hot halo）和冷星际介质（cold ISM）气体剥离效率能更好匹配观测数据，并证实热晕气体移除是卫星星系熄灭的主要机制，其熄灭时标随恒星质量增加而缩短且在星系团中比星系群更快。


<details>
  <summary>Details</summary>

Background: 研究聚焦于卫星星系（satellite galaxies）在星系群和星系团环境中的恒星形成熄灭（quenching）机制及时标问题，旨在通过半解析模型（semi-analytic model）与观测数据结合揭示环境过程（environmental process）对气体剥离（gas stripping）效率的影响及其主导作用。

Data: 数据来源包括斯隆数字巡天(SDSS)的观测数据和SHARK v2.0半解析模型生成的模拟SDSS观测数据。

Method: 采用斯隆数字巡天(SDSS)观测数据与SHARK v2.0半解析模型相结合的方法，通过生成模拟SDSS观测来校准热晕(hot halo)和冷星际介质(ISM)气体剥离方案。

Result: 卫星星系在星系团中的淬灭（quenching）时标比在星系群中更短，且时标通常随恒星质量增加而减小。
...
**TL;DR**: 针对下一代巡天项目（如Euclid、LSST和CSST）预期发现的大量星系-星系尺度强引力透镜（galaxy-galaxy scale strong gravitational lenses），开发了一种基于Lenstronomy的可扩展贝叶斯建模流程，通过PISCO四波段（z、i、r、g）数据同步多波段建模验证了其有效性，成功恢复了15/16透镜候选体并首次报道了双曲脐型（hyperbolic-umbilic）透镜候选体DESJ0533-2536，证明地面数据多波段建模能可靠约束透镜质量和源形态。


<details>
  <summary>Details</summary>

Background: 下一代巡天项目（如Euclid、LSST和CSST）预计将发现约10^5个星系-星系尺度强引力透镜系统，推动可扩展且稳健的透镜建模方法的发展以高效处理宽视场巡天数据。

Data: 数据来源于麦哲伦望远镜上的南天宇宙学观测并行成像仪（PISCO）提供的四波段（z、i、r、g）成像数据，其色彩、深度和视宁度条件与LSST相当。

Method: 采用基于贝叶斯框架的Lenstronomy多波段联合建模方法，在四波段（z、i、r、g）数据中共享透镜质量分布模型但独立拟合各波段光源亮度分布。

Result: 开发了一种基于贝叶斯框架和Lenstronomy的可扩展多波段强引力透镜建模流程，能有效利用地面观测数据同时约束透镜质量和源形态。
...
### [10] [K-DRIFT Science Theme: Galactic Cirrus Clouds and Circumgalactic Medium](https://arxiv.org/abs/2601.15665)
...
**TL;DR**: 该论文通过K-DRIFT望远镜的宽视场和高精度背景扣除能力，系统研究了星系晕(CGM)和星系际空间中的尘埃与气体分布，重点探索了利用H-alpha发射线追踪星系风、环境效应（如冲压剥离）以及多相气体混合阶段的演化特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系外围延展晕物质（extended halo material）和环星系介质（circumgalactic medium, CGM）的多相态（气体与尘埃）特性及其在星系化学演化、恒星形成中的作用。

Data: data部分：研究使用了KASI Deep Rolling Imaging Fast Telescope (K-DRIFT)的观测数据，包括测量背景源（如类星体）的红化效应和H-alpha发射线观测。

Method: 采用K-DRIFT望远镜的宽视场成像和精确背景扣除技术测量背景源（如类星体）红化，并结合H-alpha发射线观测追踪星系风（galactic winds）和冲压剥离（ram pressure stripping）产生的电离气体尾迹（H-alpha tails）。

Result: 银河系（Milky Way）中的卷云（cirrus clouds）散射星光对研究河外天体低表面亮度特征构成主要挑战，需有效区分河外源与卷云特征。
...
**TL;DR**: 通过分析L1544前恒星核中HC3N和CH3CCH的氘代分布，发现HC3N的氘代分数（D-fraction）在核外层均匀分布（0.04-0.07），CH3CCD在核中心高密度区形成效率最高，而CH2DCCH的氘代分数（0.09-0.15）与东北部局部密度增强相关，表明气相反应主导分子形成和氘代，但CH2DCCH可能还存在尘埃表面氘代途径。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用氘代分子（deuterated molecules）作为诊断工具探索恒星形成早期阶段的化学演化和动力学过程。

Data: 数据来源于IRAM 30米单天线射电望远镜观测的CH3CCH、CH2DCCH、CH3CCD、HC3N、HCC13CN和DC3N发射图。

Method: 采用非局部热动平衡(non-LTE)辐射传输计算结合分子丰度的化学建模方法，约束观测物种的物理参数。

Result: 在L1544前恒星核中，HC3N和CH3CCH的氘代分数（D-fraction）分别为0.04-0.07和0.09-0.15，其空间分布差异反映了气体相反应主导的形成和氘代过程受物理结构、密度及外部辐射驱动，且CH2DCCH的氘代机制可能与CH3CCD不同，暗示可能存在尘埃颗粒表面的额外氘代途径。
...
### [12] [JWST Advanced Deep Extragalactic Survey (JADES) Data Release 5: NIRCam Imaging in GOODS-S and GOODS-N](https://arxiv.org/abs/2601.15954)
...
**TL;DR**: 这篇论文的核心贡献是发布了詹姆斯·韦伯太空望远镜(JWST)高级深空河外巡天(JADES)第五次数据释放(DR5)的近红外相机(NIRCam)成像产品，包括覆盖469平方角分的18个波段的深度拼接图像，并对最终数据产品的光度特性、点扩散函数(PSF)和天体测量对齐进行了详细测试。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用詹姆斯·韦伯太空望远镜(JWST)的先进深空河外巡天(JADES)项目获取的近红外相机(NIRCam)数据，探索宇宙最早期的星系演化。

Data: 数据来源于詹姆斯·韦伯太空望远镜(JWST)近红外相机(NIRCam)的第五次数据发布(DR5)中的高级深空河外巡天(JADES)项目，以及在这两个顶级深场(GOODS-S和GOODS-N)中共同处理的19个其他项目的数据。

Method: 采用近红外相机(NIRCam)数据还原程序生成深度、多波段拼接图像，并进行光度特性、点扩散函数(PSF)和天体测量对齐的详细测试。

Result: 詹姆斯·韦伯太空望远镜(JWST)高级河外深场巡天(JADES)第五次数据发布(DR5)的近红外相机(NIRCam)成像产品覆盖469平方角分区域，其中250平方角分具备至少8个滤光片覆盖，包含18个滤光片的深度拼接图像。
...
### [13] [JWST Advanced Deep Extragalactic Survey (JADES) Data Release 5: MIRI Coordinated Parallels in GOODS-S and GOODS-N](https://arxiv.org/abs/2601.15955)
...
**TL;DR**: 利用JWST的MIRI仪器在GOODS-S和GOODS-N天区开展的中至超深中红外成像巡天（7.7-15 μm）优化了数据处理流程并释放了首批科学数据，为研究早期宇宙（包括尘埃星系、静止星系和再电离时代）提供了关键的红外观测约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用詹姆斯·韦伯太空望远镜（JWST）的中红外仪器（MIRI）开展的中等至超深中红外成像巡天，旨在揭示早期宇宙中极红尘埃星系、宁静星系（quiescent galaxies）的涌现、再电离时期（epoch of reionization）及首批星系的形成过程。

Data: 数据来源于詹姆斯·韦伯太空望远镜（JWST）的中红外仪器（MIRI）在GOODS-S和GOODS-N天区进行的超深（155 ks）和中深（5-15 ks）成像观测，覆盖7.7、12.8和15 μm波段，面积分别约为10、36、25和22平方角分。

Method: 采用詹姆斯韦伯太空望远镜(JWST)的中红外仪器(MIRI)进行中深度至超深度的中红外成像观测，结合官方JWST校准管道与自定义步骤优化热像素标记和背景扣除，并引入新方法处理饱和源导致的持久性伪影。

Result: 通过JWST的MIRI仪器在GOODS-S和GOODS-N天区获得了超深（155 ks）7.7 μm和中深（5-15 ks）7.7、12.8、15 μm的成像数据，覆盖面积分别为10、36、25和22平方角分。
...
### [14] [JWST Advanced Deep Extragalactic Survey (JADES) Data Release 5: Photometric Catalog](https://arxiv.org/abs/2601.15956)
...
**TL;DR**: 这篇论文的核心贡献是发布了JADES数据发布5（DR5）的光度测量目录，详细描述了针对JWST和HST多波段深度成像优化的源检测、去混叠、测光、不确定性估计和目录处理方法，并提供了基于新开发的二维高斯回归方法和像素级回归框架的精确测光数据及红移测量。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用JWST（James Webb Space Telescope）和HST（Hubble Space Telescope）的多波段深场观测数据构建高精度测光星表，旨在优化极端深度和复杂点扩散函数条件下的天体检测、测光及红移测量方法。

Data: 数据来源包括JWST/NIRCam、JWST/MIRI、HST/ACS和HST/WFC3获取的35幅空间成像拼接图，结合了约1250小时的JADES观测数据及GOODS天区的大量公开JWST和HST数据。

Method: 采用基于信噪比的自定义检测与去混叠算法、二维高斯回归方法确定源参数，并引入像素级回归框架建模光度不确定性。

Result: JADES DR5发布了基于JWST/NIRCam、JWST/MIRI、HST/ACS和HST/WFC3共35个空间成像拼接图的测光星表，采用优化的信噪比检测算法和创新的二维高斯回归方法确定天体参数。
...
### [15] [JWST Advanced Deep Extragalactic Survey (JADES) Data Release 5: Catalogs of inferred morphological properties of galaxies from JWST/NIRCam imaging in GOODS-N and GOODS-S](https://arxiv.org/abs/2601.15957)
...
**TL;DR**: 基于JWST/NIRCam观测数据，通过贝叶斯推断对>10^5个星系进行单组分Sérsic模型拟合，构建了迄今最大的河外星系形态学数据集，发现高红移(z>1)星系的有效半径(r_eff)随(1+z)^(-0.635±0.013)演化，而1千秒差距内的表面亮度密度(Σ_1kpc)基本不变，并揭示盘组分(r_eff,disk)的尺寸演化速率(-1.091±0.043)显著高于核球组分。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST/NIRCam观测数据探究高红移(z>1)星系的结构参数(如有效半径和表面亮度密度)随宇宙时间的演化规律。

Data: 数据来源于JWST/NIRCam在GOODS-N和GOODS-S的成像观测，以及JWST Advanced Deep Extragalactic Survey (JADES)目录中的JADES Data Release 5提供的成像和测光数据。

Method: 采用贝叶斯推断（Bayesian inference）对JWST/NIRCam宽波段成像中的星系进行单组分塞尔西克轮廓（Sérsic profile）拟合，计算了超过300万个形态参数。

Result: 研究发现有效半径（effective radius）随红移演化遵循$r_{\text{eff}} \propto (1+z)^{-0.635 \pm 0.013}$千秒差距的关系，而1千秒差距内的表面光度密度（$Σ_{\text{1 kpc}}$）随时间保持相对恒定。
...
### [16] [JWST Advanced Deep Extragalactic Survey (JADES) Data Release 5: Photometrically Selected Galaxy Candidates at z > 8](https://arxiv.org/abs/2601.15959)
...
**TL;DR**: 基于JADES DR5数据在GOODS-S和GOODS-N区域发现2081个红移(photometric redshift)大于8的天体样本，揭示了紫外连续谱斜率(UV continuum slope)与紫外光度(UV magnitude)关系随红移增加而陡化的新现象，并发现超过四分之一的源呈现形态延展或多亮结特征，为宇宙黎明(Cosmic Dawn)时期星系生长演化提供了直接证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙黎明（Cosmic Dawn）时期高红移（$z_{\mathrm{phot}} > 8$）星系的光度特性、形态演化及其与紫外（UV）连续谱斜率的关系。

Data: 数据来源于JADES DR5数据发布的GOODS-S和GOODS-N区域469平方角分范围内的测光红移（photometric redshift）$z_{\mathrm{phot}} > 8$的2081个源样本。

Method: 采用测光红移（photometric redshift）拟合和紫外（UV）连续谱斜率分析，结合形态学评估和光谱红移（spectroscopic redshift）验证，优化了高红移星系样本的筛选与表征。

Result: 在JADES DR5数据中发现了2081个光度红移（photometric redshift）大于8的天体样本，其中19个天体红移超过14，紫外连续谱斜率（UV continuum slope）与紫外光度（M_UV）的关系随红移增加而变陡。
...
### [17] [JADES: A Prominent Galaxy Overdensity Candidate within the First 500 Myr](https://arxiv.org/abs/2601.15960)
...
**TL;DR**: 在JWST深度河外巡天（JADES）中发现了一个红移约10.5的星系超密度区（overdensity），包含18个具有一致测光红移的星系，其数量密度是背景场的4倍，占GOODS-S天区同亮度星系的1/3，并显示出频繁相互作用和潜在电离气泡的光谱证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST先进深空河外巡天(JADES)数据探测红移$z\approx 10.5$的星系过密度区（galaxy overdensity），探索宇宙最初5亿年内星系演化环境效应与再电离（cosmic reionization）起始阶段的关联。

Data: 数据来源于JWST先进深空河外巡天（JADES）和GOODS-S天区的观测数据。

Method: 采用光度红移（photometric redshift）分析、F115W波段缺失（dropout）技术及半光半径（half-light radius）测量方法，结合恒星质量（stellar mass）和恒星形成率（star formation rate）标度关系（scaling relations）评估星系特性。

Result: 在JWST先进深空河外巡天(JADES)中发现了一个红移$z\approx 10.5$的星系过密度区(overdensity)，其星系数密度是背景场预期的四倍。
...
### [18] [JADES: Discovery of Large Reservoirs of Small Dust Grains in the Circumgalactic Medium of Massive Galaxies at $z\sim3.5$ through Deep JWST/NIRCam Imaging and Grism Spectroscopy](https://arxiv.org/abs/2601.15961)
...
**TL;DR**: 基于JWST NIRCam观测数据，在红移(z)3.47和3.69处发现两个由尘埃遮蔽的恒星形成星系(DSFGs)主导的原星系团，并首次在7-30千秒差距投影距离内探测到其环星系介质(CGM)中存在高柱密度(≥0.1太阳质量/千秒差距²)的小颗粒尘埃，导致背景星系紫外-光学连续谱异常红化且金属丰度达太阳值。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（James Webb Space Telescope）观测高红移（z=0-9）发射线星系及其原星系团（protoclusters），重点探索宇宙早期星系际介质（CGM）的尘埃消光特性及其对背景星系光谱的影响。

Data: 数据来源于JWST（James Webb Space Telescope）的NIRCam（Near-Infrared Camera）成像和JADES（JWST Advanced Deep Extragalactic Survey）起源场的棱镜光谱观测。

Method: 采用JWST近红外相机（NIRCam）成像和JADES起源场的棱栅光谱技术，结合对发射线星系（emission-line galaxies）的光谱红移测量及尘埃消光（dust extinction）分析，重点研究了前景DSFGs的星系际介质（CGM）尘埃特性。

Result: 在红移(redshift)3.47和3.69处发现两个由大质量尘埃恒星形成星系(DSFGs)主导的原星系团(protocluster)，并探测到其前景星系周介质(CGM)对背景星系产生显著尘埃消光(dust extinction)。
...
### [19] [Undermassive Hosts of $z = 4-6 $ AGN from JWST/NIRCam Image Decomposition with CONGRESS, FRESCO, and JADES](https://arxiv.org/abs/2601.15962)
...
**TL;DR**: 通过高红移（z~4-6）低光度活动星系核（AGN）的宿主星系成像分解，发现其黑洞质量（M_BH）与恒星质量（M_*）比值（0.01-1.48）显著高于本地宇宙的M_BH-M_*关系，表明宿主星系可能质量不足或过于致密。


<details>
  <summary>Details</summary>

Background: 研究探讨了高红移(high-redshift)下超大质量黑洞(SMBH)质量与宿主星系恒星质量($M_{*}$)关系的偏离现象。

Data: 数据来源于JWST的NIRCam在JADES GOODS-N区域拍摄的7个宽波段图像，以及CONGRESS和FRESCO巡天通过宽Hα发射线识别的17个低光度活动星系核(AGN)样本。

Method: 采用AGN-宿主星系图像分解方法（galfit+MCMC）对7个宽波段图像进行空间轮廓拟合，检测并分析延伸发射成分。

Result: 在红移z~4-6的低光度活动星系核(AGN)中，黑洞质量与宿主星系恒星质量之比(MBH/M*)显著高于本地宇宙的MBH-M*关系。
...
### [20] [There Is More to Outshining: 2D Dust Effects on Stellar Mass Estimates at $3 \leq z < 9$ with JWST in the JADES Field](https://arxiv.org/abs/2601.15963)
...
**TL;DR**: 通过对3408个红移3≤z<9星系的空间分辨（spatially resolved）和积分光谱能量分布（SED）拟合对比，发现空间分辨分析得到的恒星质量中位数高0.24 dex，源于其能更准确捕捉致密恒星形成区的高尘埃衰减（ΔA(V)≈+0.08 mag）、低诞生云分数（Δμ≈-0.28）及更平坦的衰减曲线（Δδ_ISM=+0.08），且年龄依赖的光致遮蔽效应和尘埃几何分布差异是导致积分法低估恒星质量（z<5时23%、z≳5时31%）的主要原因。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（high-redshift）星系中尘埃衰减（dust attenuation）对空间分辨光谱能量分布（SED）拟合所得恒星质量估计的系统性影响。

Data: 数据来源为JWST先进深空河外巡天(JADES)的深度近红外相机(NIRCam)数据，以及来自GOODS和CANDELS的哈勃太空望远镜(HST)/先进巡天相机(ACS)成像数据。

Method: 采用双组分尘埃衰减模型（two-component dust attenuation model）结合可变斜率，分析尘埃衰减斜率（A(V)）、恒星族特性对恒星质量估计差异的影响。

Result: 空间分辨（spatially resolved）SED拟合得到的恒星质量系统性地高于积分（integrated）SED拟合结果，中位偏移为+0.24 dex。
...
### [21] [JADES: Evolution of nitrogen abundances in star-forming galaxies from z ~ 1.5-7](https://arxiv.org/abs/2601.15964)
...
**TL;DR**: 基于JWST深场巡天(JADES)对588个1.5<z<7.0星系中[NII]6583线的观测，发现低金属丰度(12+log(O/H)<8.0)星系平均氮氧比(N/O)比近邻宇宙样本高至少0.1 dex，其中5个星系通过电子温度($T_e$)测量显示出显著氮增强(log(N/O)>-1.1)，结合强线法另发现14个候选体，表明约13%的低金属丰度高红移星系存在氮增强现象，且这种增强与恒星形成率(SFR)等参数相关，但未发现log(N/O)>-0.6的极端氮富集星系，证实极端氮增强仅出现在最剧烈星暴环境中。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST先进深空星系巡天(JADES)数据，通过低电离态[NII]6583发射线测量588个1.5<z<7.0星系中的氮丰度，并探讨高红移低金属丰度(12+log(O/H)<8.0)星系中氮氧比(N/O)的演化特征及其与恒星形成活动的关联。

Data: 数据来源为JWST先进深空河外巡天(JADES)中1.5<z<7.0红移范围内的588个星系样本，其中40个星系通过[OIII]4363极光谱线进行了电子温度(Te)测量。

Method: 基于低电离[NII]6583发射线测量氮丰度，并结合[OIII]4363极光线进行电子温度(Te)校准。

Result: 在低金属丰度（12+log(O/H)<8.0）样本中，平均氮氧丰度比（N/O）比z~0样本至少高0.1 dex，且存在显著的高N/O离散性，其中5个星系通过电子温度（$T_e$）测量显示出增强的氮丰度（log(N/O)>-1.1）。
...
### [22] [Clump-like Structures in High-Redshift Galaxies: Mass Scaling and Radial Trends from JADES](https://arxiv.org/abs/2601.15965)
...
**TL;DR**: 基于JWST/NIRCam对3600个红移2-8星系中星团状结构（clump）的统计分析，发现这些低质量（约10^7-8M⊙）、活跃恒星形成的结构具有多样化物理性质，其尺寸-质量关系随宇宙时间增大且呈现内外差异（外围接近维里关系Re∝M*^0.3，内部较平缓Re∝M*^0.2），支持湍流盘碎裂模型并揭示了星团迁移/瓦解对星系形态演化的影响。


<details>
  <summary>Details</summary>

Background: 研究主题为利用JWST/NIRCam观测数据探究高红移星系中大质量恒星形成团块（massive star-forming clumps）的物理特性及其对星系形态演化的影响。

Data: 数据来源于深度JWST/NIRCam成像观测的JADES GOODS-South天区约3600个红移2≲z≲8的星系样本。

Method: 采用Sérsic轮廓平滑扣除后的残差特征识别星团（clump），并基于JWST/NIRCam深场成像数据统计分析了其物理性质、尺寸-质量关系和空间分布。

Result: 样本中的团块（clump）通常质量较低（约10^7-8太阳质量）、恒星形成活跃，且表现出多样化的气体相金属丰度、尘埃消光和恒星族性质。
...
### [23] [HE0144-4657: A Carbon-Enhanced Ultra Metal-Poor Star ([Fe/H] ~ -4.1) from the Helmi Stream Disrupted Dwarf Galaxy](https://arxiv.org/abs/2601.15974)
*Vinicius M. Placco,Guilherme Limberg,Catherine R. Kennedy,Norbert Christlieb*

**TL;DR**: 发现了一颗与赫米流（Helmi Stream）瓦解的矮星系遗迹动态关联的极贫金属（[Fe/H]=-4.11）、CNO元素增强的恒星HE0144-4657，其化学丰度模式表明它可能是一颗由低爆炸能量的50倍太阳质量第三族恒星产生的第二代恒星，支持了银河系中碳增强极贫金属恒星的河外起源假说。


<details>
  <summary>Details</summary>

Background: 该研究探讨了银河系中碳增强极贫金属星（carbon-enhanced ultra metal-poor stars）的起源及其与赫米流（Helmi Stream）瓦解矮星系遗迹的动力学关联。

Data: 数据来源于汉堡/ESO物镜棱镜巡天（Hamburg/ESO objective-prism survey）的中高分辨率光谱（medium- and high-resolution spectroscopy）观测。

Method: 采用中高分辨率光谱分析技术（medium- and high-resolution spectroscopy）测定恒星化学丰度模式。

Result: HE0144-4657是一颗超贫金属（[Fe/H]=-4.11）、CNO元素增强的恒星，动态学上属于Helmi Stream（赫尔米流）瓦解的矮星系遗迹，其化学丰度模式表明它是一颗典型的第二代恒星，可能源自一颗质量约50倍太阳质量、爆炸能量较低的第一代恒星（Population III）。
...
### [24] [Extreme line profile variations in the repeating changing-look active galactic nucleus IRAS23226-3843](https://arxiv.org/abs/2601.15979)
...
**TL;DR**: 通过对IRAS23226-3843（赛弗特星系）的持续观测发现其具有重复性变脸（changing-look）特征，在2019年至2023年间经历了从赛弗特1型到1.9/2型再转回1型的光谱类型转变，并展现出极端高的巴尔末减幅（Balmer decrement）和多种宽线轮廓形态，且这些变化与连续谱强度无明确相关性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（active galactic nucleus, AGN）中变化外观（changing-look）现象的动态特性及其光谱演化。

Data: 数据来源包括SWIFT望远镜的X射线观测数据、2023年和2024年获取的新光学光谱数据以及2014年至2025年的ASAS-SN（全天自动巡天-超新星）V波段连续谱测光数据。

Method: 采用SWIFT X射线观测、2014至2025年ASAS-SN V波段测光数据及2023-2024年光学光谱分析，结合巴尔末线轮廓（Balmer line profiles）和巴尔末减幅（Balmer decrement）测量，追踪连续谱和发射线强度变化。

Result: IRAS23226-3843在2019年爆发后持续保持高X射线和光学连续谱流量水平，但表现出强烈的数月时间尺度短期变化。
...
### [25] [A multi-wavelength approach of AGN feedback in LINERs: The case of NGC 4438](https://arxiv.org/abs/2601.15991)
*M. Puig-Subirà,J. Moldón,I. Márquez,J. Masegosa,O. González-Martín,L. Hermosa Muñoz,S. Cazzoli,D. Williams-Baldwin*

**TL;DR**: 通过多波段高分辨率观测首次在NGC 4438中区分出活动星系核(AGN)致密辐射与扩展射电气泡的物理起源，证实弱准直喷流与星际介质相互作用驱动激波电离气体并产生动能达5×10⁴⁴ erg/s的射电气泡，其功率超出电离外流三个量级。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低电离核发射线区（LINERs）中多相外流的产生机制及其与射电连续谱结构、X射线辐射的关联。

Data: 数据来源包括增强型多单元射电连接干涉仪网络(e-MERLIN)和卡尔·G·扬斯基甚大阵(VLA)提供的L、C和X波段射电图像(1.4至12 GHz)，光学积分场光谱(IFS)数据(GTC/MEGARA)，以及钱德拉(Chandra)X射线数据。

Method: 结合增强型多单元射电连接干涉仪网络(e-MERLIN)和卡尔·G·扬斯基甚大阵(VLA)的高分辨率射电数据，通过生成射电流量、谱指数图及能量学模型来分析NGC4438的多波段(1.4-12 GHz)射电结构。

Result: 在NGC 4438中发现了射电泡（radio bubble）与电离气体泡（ionised gas bubble）的形态对应关系，并通过多波段分析确认其经历了喷流模式反馈（jet-mode feedback），即低光度弱准直喷流与致密星际介质相互作用驱动了激波电离气体、产生中等速度外流并形成与射电和Hα空腔重合的热X射线辐射。
...
### [26] [Spatially resolved stellar-to-total dynamical mass relation: Radial variations, gradients and profiles of galaxy stellar populations](https://arxiv.org/abs/2601.16019)
*L. Scholz-Diaz,A. R. Gallazzi,S. Zibetti,D. Mattolini*

**TL;DR**: 基于265个CALIFA星系的光学积分视场光谱（IFS）和测光数据，研究发现恒星形成历史（年龄）和金属丰度（[M/H]）的径向分布及梯度与恒星质量和总动力学质量（暗物质晕）均相关，其中年龄在星系外围更依赖总质量而金属丰度在内区更显著，且早型星系具有更高恒星-总质量比、更平坦年龄梯度及更陡负金属丰度梯度，表明暗物质晕对星系性质塑造起重要作用。


<details>
  <summary>Details</summary>

Background: 研究星系演化中重子物理与暗物质晕（dark matter halo）组装的相互作用如何影响观测到的星系性质。

Data: 使用了265个CALIFA星系的积分视场光谱仪（IFS）光学数据和测光数据，并结合基于随机恒星形成和金属丰度历史及尘埃衰减的模型光谱库，通过贝叶斯框架推导出空间分辨的年龄和金属丰度（[M/H]）。

Method: 采用贝叶斯框架分析基于随机恒星形成和金属丰度历史及尘埃衰减的模型光谱库，结合积分视场光谱(IFS)和测光数据，推导空间分辨的恒星年龄和金属丰度([M/H])。

Result: 恒星年龄和金属丰度([M/H])的径向分布及梯度变化同时依赖于恒星质量和总动力学质量(总质量)，且后者对星系外围年龄和内部金属丰度的影响更为显著。
...
### [27] [Constraining Nuclear Molecular Gas Content with High-resolution CO Imaging of GOALS Galaxies](https://arxiv.org/abs/2601.16057)
...
**TL;DR**: 通过高分辨率ALMA观测发现III Zw 035和IRAS F01364-1042星系核区存在大量冷分子气体（3×10⁷-6×10⁸ M⊙），修正后的超大质量黑洞（SMBH）质量与星系比例关系相符，但核区气体角动量假设可能面临挑战。


<details>
  <summary>Details</summary>

Background: 研究探讨了超大质量黑洞（SMBH）与宿主星系质量关系在气体丰富合并星系中的偏差问题，重点关注III Zw 035和IRAS F01364-1042中心区域分子气体对黑洞质量测量的影响。

Data: 数据来源于阿塔卡马大型毫米波/亚毫米波阵列（ALMA）Band 6长基线观测的CO(J=2-1)和230 GHz连续谱发射数据，分辨率达到14-20 pc。

Method: 采用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)长基线观测CO(J=2-1)谱线和230 GHz连续谱辐射，分辨率达14-20 pc，通过测量冷分子气体质量独立估算中心区域总质量(M$_\mathrm{enc}$)。

Result: 在III Zw 035和IRAS F01364-1042中心70 pc范围内探测到约3×10⁷至6×10⁸ M⊙的分子气体质量，这可能挑战某些假设核区气体无角动量的黑洞吸积模型。
...
### [28] [Cis--Trans Rotational Isomerism of Seleno-, Thio-, and Formic Acids and Their Dimers: Chemical Kinetics under Interstellar Conditions](https://arxiv.org/abs/2601.16115)
*Judith Wurmel,John M. Simmie*

**TL;DR**: 研究通过多维计算获得了硒代（seleno-）、硫代（thio-）和单体甲酸及其二聚体在10~300 K气相中顺反异构化（cis-trans isomerisation）的化学动力学速率常数，为低温惰性基质（cryogenic noble-gas matrices）和星际冰颗粒（ice-grains）化学反应提供了理论依据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低温惰性气体基质（cryogenic noble-gas matrices）中分子隧穿反应（tunnelling reactions）的动力学特性及其对星际介质（interstellar medium）冰颗粒化学过程的启示。

Data: 研究数据来源于多维计算（multidimensional calculations）在气相（gas-phase）条件下从10 K到300 K的模拟结果。

Method: 采用多维计算（multidimensional calculations）在气相中计算10~K至300~K温度范围内的化学动力学速率常数。

Result: 通过多维计算获得了硒代甲酸、硫代甲酸和单体甲酸及其三种二聚体在10K至300K温度范围内的顺反异构化反应速率常数。
...
### [29] [A general spectral solver for the axisymmetric Jeans equations: fast galaxy modelling with arbitrary anisotropy](https://arxiv.org/abs/2601.16179)
*Michele Cappellari*

**TL;DR**: 提出了一种新型光谱求解器（spectral solver），通过直接求解轴对称让斯方程（axisymmetric Jeans equations）并采用光谱技术、慢变速度离散化处理和罗宾边界条件（Robin boundary condition），在支持任意速度各向异性分布的同时将计算速度提升数个量级，且精度达到亚百分比级别。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用动力学建模（dynamical modelling）精确测量星系质量和密度分布，旨在克服传统半解析让斯方程（Jeans equations）求解方法在速度各向异性（velocity anisotropy）假设上的局限性。

Data: 研究中使用的数据来源为轨道积分（orbit integrations）在真实势场（realistic potentials）中的模拟结果。

Method: 采用谱方法（spectral method）直接求解轴对称Jeans方程（axisymmetric Jeans equations），通过求解缓慢变化的流速散度而非快速变化的压力，并施加Robin边界条件（Robin boundary condition）在有限域上强制渐近衰减。

Result: 提出了一种新的轴对称金斯方程谱解法，支持任意速度各向异性分布β(r,θ)并显著提升计算效率。
...
### [30] [On the Missing Red Giants near the Galactic Center](https://arxiv.org/abs/2601.16191)
*Taeho Kim,Jeremy Goodman*

**TL;DR**: 研究发现，银河系中心黑洞(Sgr A*)附近亮红巨星(red giant)数量不足的原因更可能是恒星碰撞而非潮汐剥离(tidal stripping)，因为潮汐剥离对主序星和红巨星的区分度不足。


<details>
  <summary>Details</summary>

Background: 研究探讨了银河系中心超大质量黑洞（Sgr A*）附近明亮红巨星数量显著少于较暗老年恒星的现象是否由黑洞潮汐剥离作用导致。

Data: 研究中使用的数据来源为观测数据（observational data），主要分析了人马座A*（Sgr A*）附近几角秒范围内明亮红巨星与较暗老年恒星的分布情况。

Method: 采用标量共振弛豫（scalar resonant relaxation）和非共振弛豫（non-resonant relaxation）分析轨道角动量的扩散过程，评估潮汐剥离（tidal stripping）对主序星和红巨星的影响差异。

Result: 潮汐剥离机制无法充分区分主序星和红巨星（red giant），因为尽管潮汐损失锥（tidal loss cone）随恒星半径增大，但扩散进入损失锥的速率仅对数增长，而红巨星分支（red giant branch）的寿命衰减快于$R_*^{-1}$。
...
**TL;DR**: 论文提出了一种基于拉格朗日位移场负散度区域识别宇宙空洞（cosmic voids）的新算法BitVF，相比传统拓扑方法能更稳定地生成物理意义明确的空洞目录，并有效减轻红移空间系统效应和采样噪声影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙空洞（cosmic voids）作为宇宙学探针的应用挑战及新型识别算法的开发。

Data: 数据来源包括高分辨率N体模拟（N-body simulations）和DESI类模拟光锥星系目录（DESI-like mock light-cone galaxy catalogs）。

Method: 采用离散最优输运算法（discrete optimal transport algorithm）重建拉格朗日位移场（Lagrangian displacement field）的负散度区域来识别宇宙空洞（cosmic voids），并优化处理示踪粒子偏置（tracer bias）且不依赖宇宙学假设。

Result: BitVF算法通过拉格朗日位移场负散度区域识别宇宙空洞（cosmic voids），其基于离散最优输运的反演动力学方法无需依赖宇宙学假设即可自然修正示踪物偏差（tracer bias），在模拟测试中展现出比拓扑方法更稳定的空洞丰度（void abundance）和更平滑的密度轮廓（density profile）。
...
**TL;DR**: 论文通过结合早期宇宙（大爆炸核合成）和晚期宇宙（Ia型超新星、重子声波振荡、宇宙计时器）观测数据，对非最小挠率-物质耦合的f(T)引力理论进行了多时期检验，发现大爆炸核合成数据严格限制了弱冻结温度（weak freeze-out temperature）的分数变化|δτ_f/τ_f|，而晚期宇宙观测与近ΛCDM背景兼容，联合分析打破了参数简并并给出了最严格的约束界限。


<details>
  <summary>Details</summary>

Background: 研究主题为通过结合早期和晚期宇宙观测数据，测试具有非最小挠率-物质耦合的f(T)引力理论，并评估其对标准宇宙学模型的偏离。

Data: data部分：研究结合了早期宇宙观测（大爆炸核合成（Big-Bang Nucleosynthesis）约束的轻元素丰度）和晚期宇宙数据（Ia型超新星、重子声学振荡（baryon acoustic oscillations）及宇宙计时器（cosmic-chronometer）数据）。

Method: 采用多时期观测数据联合分析方法，结合早期宇宙（Big-Bang Nucleosynthesis）轻元素丰度约束与晚期宇宙（Ia型超新星、重子声波振荡、宇宙计时器）的距离、标准尺度和哈勃参量测量，通过联合拟合（SNe Ia + BAO + CC）打破参数简并性。

Result: f(T)引力理论在非最小挠率-物质耦合下通过结合早期和晚期宇宙观测，展示了早期宇宙核物理与精密宇宙学观测在评估引力挠率扩展中的协同作用。
...
**TL;DR**: 研究发现，在包含物质和辐射的收缩相反弹宇宙学模型中，辐射能自然诱导由量子真空涨落产生的曲率扰动谱呈现红倾，且熵扰动相对次要，表明仅含普通物质和辐射的最小双组分反弹模型可产生与观测相符的初始条件。


<details>
  <summary>Details</summary>

Background: 反弹宇宙学（bouncing cosmology）通过收缩相和反弹相替代暴胀理论，解决初始奇点问题。

Data: 研究中使用的数据来源于量子真空涨落（quantum vacuum fluctuations）在收缩相早期产生的曲率扰动（curvature perturbations）和熵扰动（entropy perturbations）的理论模拟（simulation）。

Method: 采用量子真空涨落（quantum vacuum fluctuations）和耦合流体（coupled fluids）引力相互作用分析技术，研究收缩相中物质与辐射共同主导的扰动谱演化。

Result: 在反弹宇宙学（bouncing cosmology）框架下，引入物质和辐射双组分的收缩相可自然产生曲率扰动的红倾谱（red tilt），且熵扰动（entropy perturbations）无需精细调节即保持次要地位。
...
**TL;DR**: 提出利用10米级望远镜（如宽视场光谱望远镜）的大规模多目标光谱仪绘制红移（redshift）z~1.5（基线）或z~3.5（近红外扩展）的暗物质（dark matter）和大尺度结构（large-scale structure）分布图。


<details>
  <summary>Details</summary>

Background: 研究主题为利用10米级望远镜（如宽视场光谱望远镜）的大规模多目标光谱仪绘制暗物质（dark matter）和大尺度结构（large-scale structure）分布图至红移z~1.5（基线）或z~3.5（近红外扩展）。

Data: 数据来源为10米级望远镜（如拟议的宽视场光谱望远镜）上配备的大规模多目标光谱仪（massively multiplexed spectrograph）的观测数据，覆盖红移z~1.5（基线）或z~3.5（近红外扩展）。

Method: 采用大规模多目标光谱仪（massively multiplexed spectrograph）在10米级望远镜上进行红移（redshift）测量。

Result: 该研究提出利用10米级望远镜（如拟议的宽视场光谱望远镜）上的大规模多目标光谱仪绘制红移z~1.5（基线）或z~3.5（近红外扩展）的暗物质（dark matter）和大尺度结构（large-scale structure）分布图的潜在巡天计划。
...
### [35] [Fuzzy dark matter soliton core hosting a supermassive black hole as a dense low-mass perturber in strong gravitational lensing](https://arxiv.org/abs/2601.15718)
*Masamune Oguri,Naoi Kubo*

**TL;DR**: 提出模糊暗物质(FDM)模型中由超大质量黑洞(SMBH)增强密度的孤子核(soliton core)可解释强引力透镜系统JVAS B1938+666内约10^6M⊙扰动体的质量分布，其参数为FDM质量4×10^-21eV且中心存在4×10^5M⊙的SMBH。


<details>
  <summary>Details</summary>

Background: 研究探讨了模糊暗物质(FDM)模型中由超大质量黑洞(SMBH)增强密度的孤子核(soliton core)作为强引力透镜系统中有效扰源的可能性。

Data: 研究中使用的数据来源于高分辨率成像观测（high-resolution imaging observations）获得的强引力透镜系统（strong lens systems）数据。

Method: 采用孤子核(soliton core)模型解释强引力透镜系统中的低质量扰动体，其中孤子核中心密度因超大质量黑洞(SMBH)的存在而增强。

Result: 模糊暗物质(FDM)模型中的孤子核(soliton core)被超大质量黑洞(SMBH)增强中心密度后，可作为强引力透镜中的高效扰动源，其质量轮廓能合理解释JVAS B1938+666中约10^6M⊙的未知扰动体。
...
### [36] [Redshift-Binned Constraints on the Hubble Constant under $Λ$CDM, CPL, and Padé Cosmography](https://arxiv.org/abs/2601.15765)
*Zhi-Yuan Mo,Kang Jiao,Tong-Jie Zhang*

**TL;DR**: 通过多宇宙学探针联合分析发现，晚期宇宙哈勃常数($H_0$)表观振荡行为源于已知参数简并性，而非真实红移演化证据。


<details>
  <summary>Details</summary>

Background: 研究旨在检验近期关于哈勃常数（Hubble constant, H0）在晚期宇宙（late-Universe）测定中可能存在红移（redshift）依赖性的主张，并探讨其稳健性和潜在起源。

Data: 数据来源包括Pantheon+超新星（SNe Ia）、DESI重子声波振荡（BAO）、宇宙计时器（cosmic chronometers）和水脉泽（water megamasers）观测数据。

Method: 采用联合红移分箱分析（joint redshift-binned analysis）结合三种宇宙学框架（平坦ΛCDM、CPL和Padé宇宙学）对哈勃常数（H0）进行多探针约束。

Result: 在三种宇宙学框架（平坦ΛCDM、CPL和Padé宇宙学）下，联合分析显示哈勃常数（H_0）存在低振幅变化，但统计显著性仅为1.71-1.94σ，表明其源于已知参数简并性而非真实的红移演化。
...
### [37] [Intrinsic alignments in the FLAMINGO simulations with two-point statistics](https://arxiv.org/abs/2601.15851)
*A. Herle,N. E. Chisari,H. Hoekstra,D. Navarro-Gironés,M. Schaller,J. Schaye*

**TL;DR**: 通过FLAMINGO流体动力学模拟测量了490万个红移0的亮红星系（Luminous Red Galaxy）样本的位置-位置和位置-形状相关性，发现非线性排列（NLA）和潮汐排列潮汐扭矩（TATT）模型均能良好拟合数据，并提出了仅依赖一个参数A₁的质量相关TATT-M模型，该模型显著优于NLA且不受AGN和超新星反馈的显著影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于本征排列(intrinsic alignments)对下一代大天区巡天项目(如Euclid和LSST)的污染效应及其建模优化。

Data: 数据来源于FLAMINGO流体动力学模拟套件中的亮红星系样本，测量了超过490万个红移为0的星系的位置-位置和位置-形状相关性。

Method: 采用非线性对齐(NLA)模型、潮汐对齐潮汐扭矩(TATT)模型及新引入的质量依赖型TATT-M模型，通过贝叶斯方法联合拟合星系位置-位置和位置-形状相关性。

Result: 在红移0下测量了超过490万个星系的位置-位置和位置-形状相关性，发现非线性对准(NLA)模型和更复杂的潮汐对准潮汐扭矩(TATT)模型都能很好地拟合数据，且TATT-M模型（引入质量依赖关系）相比NLA模型被数据强烈偏好。
...
### [38] [Enhancing Thermal Sunyaev-Zel'dovich Analyses with Digital Twins of the Local Universe](https://arxiv.org/abs/2601.15935)
*Richard Stiskalek,Harry Desmond*

**TL;DR**: 通过结合热苏尼亚耶夫-泽尔多维奇效应(tSZ)和贝叶斯起源重建(BORG)约束模拟，开发了CSiBORG-Manticore数字孪生套件，改进了星系团质量观测关系校准，并验证了其与普朗克(Planck)康普顿-y图及eROSITA弱透镜校准X射线质量的一致性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用热苏尼亚耶夫-泽尔多维奇效应(tSZ)与约束模拟的协同作用，探索宇宙网中电离气体热压力及大尺度结构信息的整合。

Data: 研究使用了普朗克卫星(Planck)的康普顿-y(Compton-y)图测量数据和eROSITA的弱引力透镜校准X射线质量数据。

Method: 采用贝叶斯起源重建（BORG）范式下的CSiBORG-Manticore数字孪生（digital twins）技术，通过数据约束的后验模拟和贝叶斯前向建模推断初始条件。

Result: 热苏尼亚耶夫-泽尔多维奇效应(tSZ)与约束模拟相互验证，通过CSiBORG-Manticore数字孪生套件提升了普朗克Compton-y图与eROSITA弱透镜校准X射线质量的匹配精度。
...
### [39] [Reanalyzing DESI DR1: 4. Percent-Level Cosmological Constraints from Combined Probes and Robust Evidence for the Normal Neutrino Mass Hierarchy](https://arxiv.org/abs/2601.16165)
*Mikhail M. Ivanov,James M. Sullivan,Shi-Fan Chen,Anton Chudaykin,Mark Maus,Oliver H. E. Philcox*

**TL;DR**: 通过结合DESI DR1星系成团性数据、重子声波振荡(BAO)、宇宙微波背景(CMB)和超新星(SNe)数据，首次在低红移数据中发现了2.6σ置信度的动态暗能量模型($w_0w_a$)偏好，并将中微子质量总和限制在$Λ$CDM模型下$M_ν<0.057$ eV、$w_0w_a$模型下$M_ν<0.095$ eV，同时以0.6%、1.7%和2%的精度测定了哈勃常数($H_0=69.08\pm 0.37$)、物质密度($Ω_m=0.2973\pm 0.0050$)和物质涨落幅度($σ_8=0.815\pm 0.016$)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用DESI DR1星系巡天数据结合大尺度结构有效场论（large-scale structure effective field theory）精确测量宇宙学参数，特别是对ΛCDM模型和动态暗能量模型（dynamical dark energy model）的约束。

Data: 数据来源包括DESI DR1星系成团数据、光测星系(photometric galaxies)和宇宙微波背景透镜交叉相关(CMB lensing cross-correlations)的额外数据集，结合了最新的DESI重子声波振荡(BAO)数据和宇宙微波背景(CMB)功率谱倾斜度与重子密度的先验信息，并加入了Pantheon+超新星样本(SNe)。

Method: 采用大尺度结构有效场论（large-scale structure effective field theory）结合双谱似然函数扩展到更小尺度的一圈理论计算，并整合星系聚类、光变星系、宇宙微波背景（CMB）透镜交叉关联等多源数据。

Result: 结合DESI DR1星系聚类数据与多种补充数据集（光变星系和CMB透镜交叉相关）并扩展双谱似然至更小尺度，测得哈勃常数$H_0=69.08\pm 0.37~\mathrm{km}\,\mathrm{s}^{-1}\mathrm{Mpc}^{-1}$、物质密度分数$Ω_m=0.2973\pm 0.0050$和质量涨落幅度$σ_8=0.815\pm 0.016$（或透镜参数$S_8=0.811\pm 0.016$），精度分别达0.6%、1.7%和2%。
...
### [40] [Constraining dark energy models using Jackknife and Bootstrap resampling](https://arxiv.org/abs/2601.16197)
*Roshna K,Nikhil Fernandes,P Praveen,V. Sreenath*

**TL;DR**: 该研究利用重采样技术（Jackknife和Bootstrap）结合广义最小二乘法分析PantheonPlus和SH0ES超新星数据，估算了多种宇宙学模型（ΛCDM、wCDM等）参数，发现重采样方法能揭示数据局限性（如h和M间的强正相关性及其更高标准差），对哈勃张力（Hubble tension）有重要启示。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用Ia型超新星（type Ia supernovae）数据分析暗能量（dark energy）的存在与性质，探索宇宙学模型参数估计方法。

Data: 使用PantheonPlus和SH0ES（观测）数据进行分析。

Method: 采用重采样技术（resampling techniques），包括广义最小二乘法（generalised least squares method）、刀切法（Jackknife）和自助法（Bootstrap），结合马尔可夫链蒙特卡洛（MCMC）和嵌套抽样（nested sampling）算法进行贝叶斯分析，估计多种宇宙学模型参数。

Result: 采用重采样技术（resampling techniques）结合广义最小二乘法分析超新星数据，揭示了哈勃常数（h）和绝对星等（M）之间存在强正相关性，且两者标准差较高，这对哈勃张力（Hubble tension）可能有重要影响。
...
### [41] [OmniSpectra: A Unified Foundation Model for Native Resolution Astronomical Spectra](https://arxiv.org/abs/2601.15351)
*Md Khairul Islam,Judy Fox*

**TL;DR**: 提出了首个原生分辨率的天文光谱基础模型OmniSpectra，通过自适应分块、全局波长编码和局部位置嵌入等创新架构设计，实现了跨不同配置的大规模多源光谱数据联合学习，并在零样本泛化能力和多任务性能上达到最优水平。


<details>
  <summary>Details</summary>

Background: 研究主题为开发首个原生分辨率的天文光谱基础模型（foundation model）OmniSpectra，突破传统模型固定输入尺寸的限制，实现跨不同配置的大规模多源光谱数据联合学习。

Data: 数据来源于多个真实世界的大规模光谱巡天(survey)数据集，具有不同配置。

Method: 采用自适应分块处理变长光谱、正弦全局波长编码、深度卷积局部位置嵌入及有效性感知自注意力掩码的新型架构。

Result: OmniSpectra是首个能够处理任意长度原始分辨率光谱的天文学基础模型（foundation model），无需重采样或插值即可学习多尺度空间模式。
...
### [42] [LISA and the LISA Science Team](https://arxiv.org/abs/2601.15365)
*Anna Heffernan*

**TL;DR**: LISA（激光干涉仪空间天线）是欧空局主导、计划于2035年发射的首个空间引力波探测器，通过探测低频引力波拓展新科学领域，其科学团队已开展相关准备工作。


<details>
  <summary>Details</summary>

Background: LISA（激光干涉仪空间天线）是欧洲航天局（ESA）主导、联合NASA和ESA成员国计划于2035年发射的首个空间引力波探测器，旨在通过空间探测拓展低频引力波源的科学窗口。

Data: 数据来源为LISA（Laser Interferometer Space Antenna）空间引力波探测器的仿真模拟（simulation）数据。

Method: 采用激光干涉测量技术（laser interferometry）在太空部署探测器以探测低频引力波信号。

Result: LISA（激光干涉仪空间天线）预计于2035年中期发射，将成为首个空间引力波探测器，通过探测低频引力波源拓展新科学领域。
...
### [43] [Non-uniform Antenna Loading Effect on Embedded Element Patterns and Application to Fault Detection](https://arxiv.org/abs/2601.15367)
*Georgios Kyriakou*

**TL;DR**: 提出了一种迭代算法，用于计算均匀端口负载（标量单位矩阵）与非均匀负载（任意对角矩阵）之间的嵌入式单元方向图（EEP）转换，并通过噪声测试确定了测量装置所需的最小信噪比（SNR）、衰落水平及参考天线的最优选择以最小化估计误差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过迭代算法计算嵌入式单元方向图（Embedded Element Pattern, EEP）的变换，以解决非均匀端口负载条件下天线方向图计算的数值稳定性问题。

Data: 数据来源主要为测量获取的嵌入式单元方向图（Embedded Element Pattern, EEP），并在方法测试中加入了多种噪声成分以评估收敛性。

Method: 采用迭代算法计算从均匀天线端口负载（比例恒等矩阵）到非均匀负载（任意对角矩阵）的嵌入式单元方向图（EEP）变换，并通过噪声测试评估收敛性以确定最小信噪比（SNR）、测量设备衰落水平及参考天线最优选择。

Result: 提出了一种新的迭代算法，用于计算从均匀天线端口负载（标量单位矩阵）到非均匀负载（任意对角矩阵）的嵌入式单元方向图（EEP）变换。
...
### [44] [Distance-Independent Atmospheric Refraction Correction for Accurate Retrieval of Fireball Trajectories](https://arxiv.org/abs/2601.15805)
*Jaakko Visuri,Maria Gritsevich,Janne Sievinen*

**TL;DR**: 提出大气折射delta z校正技术，通过人工抬高观测点海拔高度并结合常用折射模型，显著提高了火球（fireball）位置测定的天体测量精度，消除了对火球距离或高度的显式依赖，并简化了低仰角火球观测数据的处理流程。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升火流星（fireball）方向测量精度对轨道反演和速度计算的关键影响，特别针对传统大气折射（atmospheric refraction）校正方法因假设无限远距离导致的过校正问题。

Data: 采用解析推导的大气折射 delta z 校正公式结合常用折射模型，并通过光线追迹（ray-tracing）数值解验证，该数值解将光线穿过精细分层的大气网格计算精确校正值。

Method: 采用大气折射delta z校正技术（atmospheric refraction delta z correction technique），结合解析推导公式与常用折射模型，并通过精细分层大气中的光线追迹（ray-tracing）数值解验证校正精度。

Result: 提出了一种新型大气折射校正技术（atmospheric refraction delta z correction technique），通过人为提升观测点海拔高度来修正传统方法对大气内物体过度补偿的问题，显著提高了火球位置的天文测量精度。
...
### [45] [Radio-Interferometric Image Reconstruction with Denoising Diffusion Restoration Models](https://arxiv.org/abs/2601.15844)
*Michel Morales,Emma Tolley,Remi Poitevineau*

**TL;DR**: 提出了一种基于去噪扩散概率模型（DDPMs）的数据驱动先验方法，用于射电干涉测量图像重建，在VLA、EHT和ALMA模拟观测中实现了峰值信噪比（PSNR）>60的高保真重建效果，显著优于CLEAN等传统方法。


<details>
  <summary>Details</summary>

Background: 研究主题为利用去噪扩散概率模型（DDPMs）改进射电干涉测量（radio interferometry）中不完整傅里叶（Fourier）数据的天体图像重建方法。

Data: 数据来源包括VLA FIRST巡天的射电星系观测数据，以及模拟的VLA、EHT和ALMA射电星系观测数据。

Method: 采用基于去噪扩散概率模型（DDPMs）的数据驱动先验，结合测量过程的物理特性，通过无监督后验采样方法（DDRM）重建射电干涉图像。

Result: 基于去噪扩散概率模型（DDPMs）的数据驱动先验方法显著提升了射电干涉图像重建质量，峰值信噪比（PSNR）超过60，优于传统CLEAN及条件DDPM方法。
...
### [46] [JWST Advanced Deep Extragalactic Survey (JADES) Data Release 5: Wisp Subtraction with the Non-negative Matrix Factorization Algorithm](https://arxiv.org/abs/2601.15958)
...
**TL;DR**: 提出了一种基于非负矩阵分解（NMF）算法的新方法，通过构建多组分、特定滤光片和探测器的卷须（wisp）模板来建模和去除JWST/NIRCam成像中的卷须散射光伪影，有效降低了残余均方根误差和光度测量偏差。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于詹姆斯·韦伯太空望远镜近红外相机（JWST/NIRCam）成像中由杂散光引起的条纹伪影（wisps）对微弱天体测光观测的干扰问题。

Data: data：研究使用了来自JWST高级深度河外巡天（JADES）及其他项目的深场NIRCam观测数据。

Method: 采用非负矩阵分解（NMF）算法构建多成分、滤光片和探测器专用的光晕（wisp）模板，通过非负线性组合建模并去除光晕结构及其曝光间形态变化。

Result: 提出了一种基于非负矩阵分解（NMF）算法的新方法，通过构建多成分、滤光片和探测器特定的条纹模板来有效建模和去除JWST/NIRCam成像中的散射光伪影（wisps）。
...
### [47] [The FarView Low Frequency Radio Array on the Moon's Far Side: Science and Array Architecture](https://arxiv.org/abs/2601.16170)
...
**TL;DR**: FarView是部署在月球背面的一种低频射电干涉仪，利用月球无线电静默环境在1-50 MHz频段首次测量宇宙黑暗时期（红移z=30-100）的21厘米信号，为宇宙学、太阳物理学和系外行星研究开辟新窗口。


<details>
  <summary>Details</summary>

Background: FarView是一项旨在利用月球背面无线电静默环境部署的低频射电干涉仪（low frequency radio interferometer）研究，其科学主题聚焦于通过观测宇宙黑暗时期（Cosmic Dark Ages）红移21厘米信号（redshifted 21 cm signal）探索宇宙学、太阳物理学、银河系及系外行星天体物理等领域的前沿发现。

Data: data：FarView将使用部署在月球背面的10万个交叉偶极天线（crossed dipole antennas）组成的密集核心-晕结构（dense core-halo configuration），覆盖200平方公里区域，其中4公里紧凑核心区含83000个偶极天线用于探测大尺度宇宙学模式，14公里延伸晕区含20000个偶极天线提供角分辨率和前景校准。

Method: 采用基于快速傅里叶变换的EPIC波束成形器（FFT-based EPIC beamformer）作为高效信号处理架构，结合密集核心-晕结构（core-halo configuration）的10万个交叉偶极天线阵列进行低频射电干涉测量。

Result: FarView将利用月球背面无线电静默环境，在1-50 MHz频段首次实现宇宙黑暗时期（z=30-100）红移21厘米信号的3D断层扫描测量和精确功率谱分析，为检验暴胀初始条件、原初非高斯性、暗物质性质、中微子质量和早期暗能量提供关键数据。
...
### [48] [Gravitational Waves and Primordial Black Holes produced by Dark Meta Stable Vacuum Decay](https://arxiv.org/abs/2601.14366)
*Haipeng An,Tingyu Li,Chen Yang*

**TL;DR**: 研究了暗物质和暗能量可能存在的"暗景观"（dark landscape）中真空亚稳态相变现象，发现引力波谱峰值位于$k_{\mathrm{peak}}=3.1 H_{\mathrm{PT}}$且振幅符合$Ω_{\mathrm{GW}}^{\mathrm{peak}}\simeq1.5 Ω_γ(Δρ/ρ_{\mathrm{tot}})^2$关系，同时原初黑洞（primordial black holes）形成受到$ΔN_{\mathrm{eff}}$限制而受到抑制。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗物质和暗能量构成的暗区（dark sector）可能存在亚稳态真空结构（metastable vacua）及其宇宙学相变现象。

Data: 研究中使用的数据来源为理论模拟（simulation）数据，基于弦理论（string theory）和宇宙学常数问题（cosmological constant problem）构建的暗区（dark sector）相变模型。

Method: 研究了暗真空（dark vacuum）亚稳态相变产生的随机引力波背景（stochastic gravitational-wave background）和原初黑洞（primordial black holes）形成，采用哈勃膨胀率（Hubble expansion rate）单调递减条件下相变成核率恒定假设，计算了引力波谱峰值位置和振幅。

Result: 引力波谱在$k_{\mathrm{peak}}=3.1 H_{\mathrm{PT}}$处达到峰值，振幅为$Ω_{\mathrm{GW}}^{\mathrm{peak}}\simeq1.5 Ω_γ(Δρ/ρ_{\mathrm{tot}})^2$，且原初黑洞形成因$ΔN_{\mathrm{eff}}$限制而受到抑制。
...
### [49] [Single-wave solutions of the neutrino fast flavor system. Part I. Mechanical properties](https://arxiv.org/abs/2601.15372)
*Damiano F. G. Fiorillo,Georg G. Raffelt*

**TL;DR**: 该论文发现密集中微子等离子体（neutrino plasma）中存在一类新的精确非线性非均匀解——单波（single-wave, SW）解，揭示了其与经典自旋系统的动力学相似性，并建立了所有已知非线性集体味（flavor）演化解的分类体系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于稠密中微子等离子体中由中微子-中微子折射（neutrino-neutrino refraction）引发的集体味（flavor）演化现象。

Data: data部分：研究使用了理论模拟（simulation）数据，基于中微子-中微子折射（neutrino-neutrino refraction）的集体味演化（collective flavor evolution）模型。

Method: 采用非线性动力学方法分析密集中微子等离子体的单波（SW）解，类比经典自旋系统建立运动方程。

Result: 密集中微子等离子体中的快味转换系统存在一类新的精确非线性非均匀解——单波（SW）解，其特点是味占据数保持均匀而味相干场呈现单波矢空间变化。
...
### [50] [Development of an early warning method incorporating pre-supernova neutrino light curves](https://arxiv.org/abs/2601.15691)
...
**TL;DR**: 提出了一种基于前超新星（pre-SN）中微子事件率时间演化的警报方法，通过参考多种恒星演化模型并将核心坍缩时间作为干扰参数进行轮廓分析，显著提高了KamLAND和SK-Gd探测器对核心坍缩超新星的预警时间。


<details>
  <summary>Details</summary>

Background: 大质量恒星（massive stars）在演化末期通过热核反应释放前超新星中微子（pre-SN neutrinos）以冷却核心，实时监测其相互作用率为核心坍缩超新星（core-collapse supernova）提供早期预警。

Data: 数据来源于KamLAND、溶解钆的Super-Kamiokande（SK-Gd）以及两者联合分析的模拟数据。

Method: 采用对数似然比检验（log likelihood ratio test）方法，结合多个恒星演化理论模型，并将核心坍缩时间作为待消除的干扰参数进行处理。

Result: 提出了一种基于对数似然比检验的警报方法，通过分析前身超新星（pre-SN）中微子事件率的时间演化并参考多个恒星演化理论模型，显著提升了警报灵敏度且保持相同误报率。
...
### [51] [Unveiling the Spectral Morphological Division of Fast Radio Bursts with CHIME/FRB Catalog 2](https://arxiv.org/abs/2601.16048)
...
**TL;DR**: 通过无监督机器学习分析CHIME/FRB Catalog 2数据，发现快速射电暴(FRB)在光谱形态参数(spectral running)主导下仍分为重复暴和非重复暴两类，其中6%的重复暴具有宽带、短时标、高光度等非重复暴特征，表明两类FRB可能存在物理联系且非重复暴可能源于观测不完备性。


<details>
  <summary>Details</summary>

Background: 快速射电暴（FRB）通常分为重复和非重复源，但二者是否代表不同物理群体仍不确定。

Data: 数据来源于CHIME/FRB Catalog 2（加拿大氢强度测绘实验快速射电暴目录第二版）。

Method: 采用无监督机器学习方法结合均匀流形逼近与投影（UMAP）和基于密度的聚类技术分析多维参数空间数据。

Result: 谱跑（spectral running）是区分重复快速射电暴（FRBs）和非重复快速射电暴的关键参数，表明窄带发射是重复快速射电暴的固有且持续特性。
...
### [52] [Source identification for the Swift-BAT 150-month hard X-ray catalog using soft X-ray observations](https://arxiv.org/abs/2601.15577)
...
**TL;DR**: 通过分析斯威夫特爆发警报望远镜（Swift BAT）150个月硬X射线巡天中250个未关联源的251个潜在对应体，发现其中94个（37%）为活动星系核（AGN）、58个（23%）为星系，其余99个（40%）包含脉冲星、激变变星及未分类软X射线源，并补充了9个星系的光谱观测结果（多为红移略高于BAT AGN样本中值的塞弗特2型星系）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用软X射线观测（低于10 keV）识别Swift-BAT硬X射线巡天（15-150 keV）中未关联源的低能对应体（counterpart），以揭示其天体物理本质。

Data: 数据来源于钱德拉(Chandra)、雨燕-XRT(Swift-XRT)、eROSITA和XMM-牛顿(XMM-Newton)在0.5-10 keV能段获取的软X射线观测数据，这些数据与新的雨燕-BAT(Swift-BAT)硬X射线源在空间上重叠。

Method: 采用软X射线观测数据（0.5-10 keV）与Swift-BAT硬X射线源进行空间匹配分析，结合钱德拉（Chandra）、Swift-XRT、eROSITA和XMM-牛顿（XMM-Newton）的观测数据。

Result: 在251个潜在对应体中，94个（37%）被识别为活动星系核（AGN），58个（23%）为星系，其余99个（40%）包括脉冲星、激变变星和0.5-10 keV波段未分类的软X射线对应体。
...
### [53] [Natural Language-Driven Global Mapping of Martian Landforms](https://arxiv.org/abs/2601.15949)
...
**TL;DR**: 开发了MarScope框架，通过将火星图像与文本对齐到共享语义空间（shared semantic space），实现了无需预定义标签的自然语言驱动火星地貌（Martian landforms）检索和全球形态学制图（geomorphic mapping），准确率（F1）达0.978。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决行星表面分析中自然语言语义概念与像素级图像归档之间的不匹配问题，提出通过视觉-语言（vision-language）框架实现无标签（label-free）的火星地貌自然语言驱动映射。

Data: 数据来源为20万组经过筛选的火星图像-文本对（image-text pairs）。

Method: 采用基于视觉-语言对齐（vision-language alignment）的深度学习框架，将火星图像与自然语言文本映射到共享语义空间，支持无标签（label-free）的灵活语义检索。

Result: MarScope通过将行星图像与文本对齐到共享语义空间（shared semantic space），实现了无需预定义标签的自然语言驱动火星地貌映射，其F1分数高达0.978，支持任意用户查询并在5秒内完成全球范围检索。
...
**TL;DR**: 利用ALMA高分辨率(~230 au)观测49个高质量恒星形成区气体示踪分子，首次在统计显著样本中分离出32个盘候选体（disk candidates），揭示其速度梯度符合-0.7中值幂律分布（介于开普勒旋转与角动量守恒之间），中心质量7-45 M$_\odot$且多数引力不稳定（Toomre-$Q$中值0.5），证实各向异性坍缩（anisotropic collapse）对高质量原恒星的物质供给作用。


<details>
  <summary>Details</summary>

Background: 研究高分辨率下大质量恒星形成区中盘状结构（disk）的运动学特征及其在恒星质量增长过程中的作用。

Data: 数据来源于阿塔卡马大型毫米波/亚毫米波阵列（ALMA）对30个形成大质量恒星的区域进行的高分辨率（平均约0.08角秒，约230天文单位）观测，分析了49个天体中气体示踪分子（CH3OH、CH3CN以及初步探测到的HNCO和cis-HCOOH）的发射谱线。

Method: 采用高分辨率（约0.08角秒）ALMA观测数据，通过分析CH3OH、CH3CN等气体示踪物的发射谱线，检测速度梯度以揭示旋转盘（disk candidates）的存在，并利用幂律拟合位置-速度图以确定旋转模式（Keplerian rotation或特定角动量守恒旋转）。

Result: 在49个天体中发现32个盘候选体（disk candidates），其位置-速度图普遍不对称且符合幂律分布（中值指数-0.7），支持各向异性坍缩（anisotropic collapse）对高质量（原）恒星形成的供能作用。
...
**TL;DR**: 基于盖亚(Gaia) DR3 GSP-Spec数据构建了包含6594颗太阳相似体(Solar twins)的大样本星表，通过贝叶斯等龄线投影方法确定了精确年龄，并利用模拟星表验证了选择函数，证实了年龄与化学丰度[X/Fe]关系的统计显著性。


<details>
  <summary>Details</summary>

Background: 研究旨在利用盖亚(Gaia) DR3 GSP-Spec数据构建一个模型驱动的大样本太阳相似星(Solar twins)目录，以支持高精度银河系考古学研究。

Data: 数据来源为盖亚卫星第三次数据释放(Gaia DR3)的GSP-Spec星表。

Method: 基于盖亚(Gaia) DR3 GSP-Spec星表参数，采用贝叶斯等龄线投影法(Bayesian isochrone-projection method)结合有效温度(Teff)、金属丰度([M/H])及表面重力(log g)或绝对星等(M_G/M_Ks)确定年龄，并通过模拟星表(mock catalog)验证选择函数。

Result: 基于盖亚(Gaia) DR3 GSP-Spec数据构建了包含6,594颗太阳相似星(Solar twins)的模型驱动型大样本目录，并通过贝叶斯等龄线投影法确定了精确年龄，验证了先前在小样本中发现的年龄-化学丰度关系在大样本中依然成立。
...
**TL;DR**: 基于初始质量函数一致性假设，通过比较英仙座恒星形成区六个年轻星团的质量分布，确定了它们的精确年龄序列（NGC1333+Autochthe 1 Myr、IC348 2 Myr、Heleus 2-3 Myr、Mestor 3-4 Myr、Electryon+Cynurus 4-5 Myr、Alcaeus 5-8 Myr），验证了该方法可作为现有年龄测定技术的有效补充和交叉验证。


<details>
  <summary>Details</summary>

Background: 研究主题为利用初始质量函数（IMF）一致性假设开发年轻星团年龄测定新方法，以追踪恒星形成历史。

Data: 数据来源包括盖亚卫星第三次数据释放（Gaia DR3）的成员样本和2MASS测光数据。

Method: 基于初始质量函数（IMF）一致性假设，通过比较不同年龄假设下各星团质量分布差异来确定最佳拟合年龄序列。

Result: NGC1333+Autochthe年龄为1 Myr，IC348为2 Myr，Heleus为2-3 Myr，Mestor为3-4 Myr，Electryon+Cynurus为4-5 Myr，Alcaeus为5-8 Myr。
...
### [57] [Formation and X-ray emission from hot bubbles in planetary nebulae - III. The impact of [Wolf-Rayet]-type winds](https://arxiv.org/abs/2601.16100)
*Rogelio Orozco-Duarte,Jesús A. Toalá,S. Jane Arthur,Janis B. Rodríguez-González,Luke Conmy,Rolf Kuiper*

**TL;DR**: 通过辐射流体动力学模拟发现，行星状星云（PNe）中由贫氢[沃尔夫-拉叶星]（[WR]）型恒星强风驱动的热气泡在合成X射线辐射方面比富氢模型具有更高光度（$L_\mathrm{X}$），但无论[WR]型或富氢后AGB演化路径，发射加权等离子体温度均收敛于$T_\mathrm{X} = [1-3] \times 10^{6}$~K，证实混合机制是产生软X射线辐射的关键。


<details>
  <summary>Details</summary>

Background: 研究聚焦于行星状星云（PNe）中由氢缺乏的沃尔夫-拉叶型（[WR]）恒星强风驱动的热气泡形成及其合成X射线辐射。

Data: 采用基于1-3 M$_{\odot}$前身星的{\sc mesa}恒星演化轨迹的辐射流体动力学模拟（radiation-hydrodynamical simulations），结合[沃尔夫-拉叶星]（[WR]）最新质量损失率公式及其富碳物质增强的辐射冷却效应。

Method: 采用辐射流体动力学模拟（radiation-hydrodynamical simulations）结合MESA恒星演化轨迹和Wolf-Rayet（[WR]）型星质量损失率模型，分析行星状星云（PNe）中热气泡的形成与X射线合成辐射。

Result: [WR]型恒星驱动的行星状星云(PNe)中热气泡的X射线辐射显著高于富氢模型，但发射加权等离子体温度均收敛于1-3×10^6 K。
...
### [59] [Dynamical Origin of (469219) Kamo`oalewa of Tianwen-2 Mission from the Main-Belt: $ν_6$ Secular Resonance, Flora Family or 3:1 Resonance with Jupiter](https://arxiv.org/abs/2601.13585)
*Yandong Wang,Shoucun Hu,Jianghui Ji,Jiajun Ying*

**TL;DR**: 通过长期动力学模拟发现近地天体Kamo`oalewa(469219)可能起源于主小行星带的三个候选区域（$ν_6$共振区、3:1木星共振区和Flora族），其中$ν_6$共振区转移概率最高（3.31%），并确定了这些区域到地球准卫星轨道的代表性动力学路径。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近地天体(469219) Kamo`oalewa的起源问题，旨在揭示地球准卫星（quasi-satellite）群体的形成机制与动力学演化过程。

Data: 数据来源于对42,825个测试粒子在100万年时间尺度上的长期动力学模拟（long-term dynamical simulations），模拟考察了三个候选来源区域：$ν_6$长期共振区（$ν_6$ secular resonance）、木星3:1平运动共振区（3:1J MMR）和Flora家族（Flora family）。

Method: 采用长期动力学模拟（long-term dynamical simulations）分析候选源区（$ν_6$ 长期共振、3:1木星平运动共振和Flora族）的42,825个测试粒子在100 Myr内的轨道演化。

Result: Kamo`oalewa（469219）可能起源于主小行星带，其中来自$ν_6$共振区的转移概率最高（3.31%），其次是Flora家族（2.54%）和3:1J MMR（0.39%）。
...
### [60] [Perihelion Asymmetry in the Water Production Rate of the Interstellar Object 3I/ATLAS](https://arxiv.org/abs/2601.15443)
*Hanjie Tan,Xiaoran Yan,Jian-Yang Li*

**TL;DR**: 基于SOHO/SWAN莱曼α(Lyman-α)观测数据，研究发现星际天体3I/ATLAS的峰值水生成率(Q_H2O)达4×10²⁸分子/秒，其水活动呈现日心距(r_h)不对称性（入轨r_h^(-5.9±0.8)与出轨r_h^(-3.3±0.3)），表明水活动主要由稳定活跃区域的太阳辐射驱动，且可能以冰粒分布式来源为主。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际天体3I/ATLAS的水活动特征及其动力学机制，旨在揭示其组成、起源及挥发性物质释放规律。

Data: 数据来源于SOHO/SWAN的莱曼阿尔法(Lyman-α)观测数据，覆盖2025年11月至12月期间（日心距1.4至2.2天文单位）。

Method: 采用SOHO/SWAN莱曼-α（Lyman-α）观测数据结合三维蒙特卡洛（3D Monte Carlo）建模方法分析水生成率。

Result: 3I/ATLAS的彗发活动主要由稳定的活跃区域受太阳辐射变化驱动，且其水生成可能由冰粒分布源主导。
