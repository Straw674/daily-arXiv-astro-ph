...
**TL;DR**: 基于盖亚(Gaia)数据对6594颗高质量本地太阳相似星(solar twins)的年龄分布分析发现，2 Gyr处的窄峰对应近期恒星形成爆发，4-6 Gyr的宽峰表明大量内盘恒星与太阳经历了长距离(≥3 kpc)迁移，挑战了银河系棒结构形成的共转屏障(corotation barrier)理论预测。


<details>
  <summary>Details</summary>

Background: 太阳相似体（solar twins）因其与太阳相似的金属丰度（[Fe/H] ~ 0）且年龄分布广泛，成为研究银河系盘演化的重要示踪体。

Data: 数据来源于盖亚卫星第三次数据发布（Gaia Data Release 3）的光谱（GSP-Spec）目录中筛选出的6,594颗高质量局部（距离≤300秒差距）太阳相似星样本。

Method: 基于盖亚卫星第三次数据发布（Gaia DR3）光谱（GSP-Spec）目录中的6594颗高质量本地（约300秒差距内）太阳相似星样本，通过仔细解卷积选择函数推导其年龄分布。

Result: 年龄分布呈现两个显著特征：约2 Gyr处的窄峰和4-6 Gyr范围的宽峰，前者对应近期盘星爆发事件，后者暗示太阳与众多内盘恒星经历长距离迁移并与棒结构形成时期相关。
...
**TL;DR**: 研究发现大麦哲伦云（LMC）对高径向速度各向异性（β≥0.5）的银河系恒星晕（包括盖亚香肠-恩克拉多斯遗迹GSE）产生显著扰动，导致其形态从轴对称变为近似三轴且主轴向LMC轨道面倾斜达14°，并在15千秒差距内形成与室女座超密度区（VOD）和武仙-天鹰云（HAC）重合的40%强密度扰动，证实这些结构源于LMC对高偏心率轨道的动力学扰动而非GSE合并遗迹。


<details>
  <summary>Details</summary>

Background: 研究探讨了大麦哲伦云(LMC)对银河系恒星晕和暗物质晕的动力学扰动效应，特别关注了高径向速度各向异性(如盖亚香肠-恩克拉多斯(GSE)遗迹)对扰动响应的放大作用。

Data: data：研究使用了一系列测试粒子模拟（test particle simulations）数据，模拟了速度各向异性（velocity anisotropy）参数β在0.5至0.9范围内的银河系恒星晕（stellar halo）。

Method: 采用测试粒子模拟(test particle simulations)分析不同速度各向异性(velocity anisotropy)参数β∈[0.5,0.9]的恒星晕在LMC扰动下的动力学响应，并通过解析模型(analytical model)验证高偏心率(eccentricity)轨道与LMC潮汐场的方位角对齐效应。

Result: 大麦哲伦云（LMC）对银河系恒星晕和暗物质晕的扰动会导致高径向速度各向异性（β≥0.5）的晕从初始轴对称变为近似三轴形态，其长轴与LMC轨道平面对齐且相对固定银盘倾斜达14°，且β越高效应越显著，在日心距15 kpc处形成40%的强密度增强区，解释了武仙-天鹰云（HAC）和室女超密度区（VOD）的成因并非盖亚香肠-恩克拉多斯（GSE）并合几何遗迹而是LMC对高偏心率轨道的动力学对齐结果。
...
**TL;DR**: 论文通过JWST的F090W波段观测，基于红巨星分支顶端(TRGB)测量方法校准了低金属丰度([M/H] = -0.57)下TRGB的绝对星等(-4.40±0.03 mag)，并应用于16个星系相对于NGC 4258的距离测定，发现修订距离比文献值略近。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用詹姆斯·韦伯太空望远镜(JWST)的F090W波段观测红巨星分支顶端(TRGB)测量星系距离的校准方法及其在低金属丰度与高金属丰度环境下的亮度变化特性。

Data: 数据来源为詹姆斯·韦伯太空望远镜（JWST）在F090W波段的观测数据，并结合NGC 4258的脉泽（maser）距离作为绝对尺度基准。

Method: 基于红巨星分支顶端（TRGB）测量方法，结合JWST的F090W波段观测数据，采用理论RGB等龄线（isochrone）模型进行金属丰度（[M/H]）与颜色关系的校准，并通过NGC 4258的脉泽（maser）距离作为绝对尺度基准。

Result: 在F090W波段中，红巨星分支顶端(TRGB)的绝对星等在低金属丰度下几乎恒定，但在高金属丰度下会变暗，其斜率转折点对精确测距至关重要。
...
**TL;DR**: 研究发现矮星系（dwarf galaxies）与巨星系在等光轮廓（isophotes）参数上相似，仅矮旋涡星系结构更简单，且矮早型星系（dwarf ETGs）可能均为三轴椭球体，而形态分类在参数空间中无法区分，需借助高维统计方法解析其演化。


<details>
  <summary>Details</summary>

Background: 矮星系（dwarf galaxies）作为对重子反馈和潮汐扰动等关键演化驱动因素敏感的星系，对理解整体星系演化至关重要。

Data: 研究使用了宽带成像数据（broadband imaging）和单Sérsic剖面拟合（single-Sérsic profile fits）的残差数据，并基于等光轮廓（isophotes）提取了位置角扭曲（position angle twists）、椭圆率（ellipticity）和偏离纯椭圆的偏差（deviations from pure ellipses）等参数。

Method: 采用等光轮廓（isophote）衍生的参数（位置角扭转、椭率、纯椭圆偏离度）和单Sérsic剖面拟合残差进行形态学分析，并结合主成分分析（PCA）评估形态与物理参数的高维相关性。

Result: 矮星系螺旋（dwarf spirals）在结构上比其大质量对应体更简单，而矮星系早型星系（dwarf ETGs）与大质量早型星系（ETGs）的等光轮廓相似性表明所有矮星系早型星系可能都是三轴椭球体。
...
**TL;DR**: 研究发现X射线活动星系核（AGN）普遍存在于暗物质晕（DMH）质量约为$\log(M_{\mathrm{DMH}}/h^{-1}M_\odot)\simeq13$的环境中，且黑洞质量（$M_{\mathrm{BH}}$）、爱丁顿比（$λ_{\mathrm{Edd}}$）和X射线光度（$L_{\mathrm{X}}$）对其无显著影响，表明AGN性质主要由寄主星系内部过程调控，而非大尺度环境。


<details>
  <summary>Details</summary>

Background: 研究主题为探讨大尺度环境（large-scale environment）与寄主星系（host-galaxy）内部过程对活动星系核（AGN）活动的相对影响，以约束黑洞增长与星系演化模型。

Data: 数据来源于XXL和Stripe 82X巡天（survey）的427个宽线活动星系核（AGN）样本（$0.5<z<1.2$）和超过20,000个星系样本，采用相同的光谱能量分布（SED）拟合方法获取寄主星系性质。

Method: 采用暗物质晕（DMH）质量推断和多元最近邻匹配算法分析活动星系核（AGN）环境依赖性。

Result: X射线活动星系核（AGN）通常位于暗物质晕（DMH）质量约为$\log(M_{\mathrm{DMH}}/h^{-1}M_\odot)\simeq13$的环境中，且黑洞质量（$M_{\mathrm{BH}}$）、爱丁顿比率（$λ_{\mathrm{Edd}}$）或X射线光度（$L_{\mathrm{X}}$）对其无明显影响。
...
**TL;DR**: 利用暗能量光谱仪（DESI）首次联合测量了C-19星流（stellar stream）的径向速度、自行运动和金属丰度，确认其47颗成员星（含41颗新发现），测得速度弥散为7.8 km/s、平均金属丰度[Fe/H]=-3.36，并发现其内部存在新型"支脉"结构，支持该星流为"高温"贫金属星流的结论。


<details>
  <summary>Details</summary>

Background: 恒星流（stellar streams）是宿主星系引力势通过潮汐力瓦解卫星矮星系和球状星团（GCs）形成的结构，其研究有助于理解星系动力学和暗物质分布。

Data: 数据来源于暗能量光谱仪（DESI），提供了超过1000万颗恒星的径向速度和金属丰度，探测极限显著低于同类巡天。

Method: 采用混合模型方法（mixture model approach）联合分析自行运动、视向速度和金属丰度数据，结合暗能量光谱仪（DESI）观测的视向速度和金属丰度信息，对银河系晕背景下的星流群体进行特征描述。

Result: C-19恒星流（stellar stream）的径向速度弥散度为7.8^{+1.5}_{-1.3} km s^{-1}，平均金属丰度[Fe/H]为-3.36^{+0.12}_{-0.10}，并发现了一个新的"支流"（spur）结构特征。
...
**TL;DR**: 首次利用MACER框架研究盘状星系演化，发现活动星系核（AGN）反馈与恒星形成率（SFR）正相关且冷丝（cold filaments）在星系周介质（CGM）中凝结并增强两者活动，最终AGN反馈在约1吉年时标上淬灭星系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核（AGN）反馈和宇宙学流入在盘星系演化中的作用。

Data: 数据来源于基于MACER框架的数值模拟（simulation），该模拟结合了宇宙学流入（cosmological inflows）并强调了活动星系核（AGN）反馈的作用。

Method: 采用MACER框架结合活动星系核(AGN)反馈和宇宙学流入，通过辐射冷却模拟冷丝在星系周介质(CGM)区域的凝结过程，并分析其对恒星形成率(SFR)和AGN活动的增强效应。

Result: AGN（active galactic nucleus）反馈与恒星形成率（SFR）呈正相关，但增强的AGN活动仍能在约1 Gyr时间尺度上抑制星系活动。
...
**TL;DR**: 通过结合X射线和光学光谱分析，发现活动星系核(AGN)中11个宽线(BL)源存在X射线吸收(BLAbs)且气尘比升高，58个窄线(NL)源呈现X射线无遮挡(NLUnabs)且气尘比低，揭示了遮蔽现象不仅源于取向还涉及多尺度气体尘埃分布。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核(AGN)中光学遮蔽与X射线吸收的关联性，旨在揭示气体和尘埃的多尺度分布对AGN遮蔽现象的贡献。

Data: 数据来源为4XMM-DR11的X射线光谱和SDSS DR16Q的光谱数据。

Method: 采用贝叶斯X射线光谱拟合（Bayesian X-ray spectral fits）结合XMM2Athena项目数据，并通过CIGALE光谱能量分布（SED）拟合获取宿主星系性质。

Result: 通过结合X射线和光学数据，发现活动星系核（AGN）的遮蔽现象不仅与取向有关，还受到多尺度气体和尘埃分布的影响，并识别出两类不匹配的群体（BLAbs和NLUnabs）。
...
**TL;DR**: 研究发现一维自引力系统（self-gravitating systems）在简谐势（harmonic potential）中弛豫时间随粒子数$N$呈二次方增长，部分简并系统在低$N$时也遵循此规律但高$N$时转为线性增长，非简并系统则保持线性增长但比例因子更大。


<details>
  <summary>Details</summary>

Background: 研究探讨了一维自引力系统（self-gravitating systems）在简谐势（harmonic potential）和部分简并轨道（degenerate orbits）条件下的热力学平衡与弛豫时间标度（relaxation timescale）问题。

Data: 使用精确的碰撞驱动一维积分器（exact collision-driven 1D integrator）进行数值模拟，研究了自洽的一维谐振自引力系统（1D harmonic self-gravitating systems）的弛豫过程。

Method: 采用精确的碰撞驱动一维积分器（exact collision-driven 1D integrator）对一维谐波自引力系统的自洽弛豫进行数值研究。

Result: 一维自引力系统在简谐势（harmonic potential）中表现出弛豫时间随粒子数$N$呈二次方增长的关系。
...
**TL;DR**: 研究基于JWST/NIRSpec观测数据，揭示了2.7 < z < 7.0恒星形成星系中星云消光（nebular reddening）和恒星消光（stellar reddening）随星系质量、恒星形成率和金属丰度的演化规律，发现质量主导尘埃柱密度且高红移下星云与恒星消光差异减小，表明尘埃质量和几何分布共同影响高红移星系的消光观测。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移（$2.7 < z< 7.0$）恒星形成星系中星云消光（nebular reddening）与恒星消光（stellar reddening）随星系整体性质（恒星质量、恒星形成率、金属丰度）的变化规律及其物理机制。

Data: 数据来源为JWST/NIRSpec对283个星系的个体光谱和327个星系的复合光谱测量。

Method: 采用巴尔末减幅（Balmer decrement，Hα/Hβ）测量星云消光（nebular reddening），通过Prospector SED拟合获取恒星消光（stellar reddening）和恒星形成率（SFR），并基于强发射线比（strong emission-line ratios）估算气体金属丰度（gas-phase metallicity）。

Result: 恒星质量是决定高红移星系整体尘埃柱密度的主要因素，即使在红移$z \sim 7$时也保持稳定。
...
**TL;DR**: 基于盖亚(Gaia) DR3数据首次连续测量了银河系恒星晕的光度函数(LF)，发现其从绝对星等$M_G\sim13$的亚矮星(subdwarfs)到$M_G\sim-3$的亮巨星呈连续分布，并确定了局部恒星晕数密度为$1.7\times10^{-4}$颗$\,$pc$^{-3}$及盘晕比480:1。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用盖亚(Gaia)DR3数据测量银河系恒星晕的光度函数(LF)，揭示其恒星分布特征及与金属丰度较高盘状恒星的相似性。

Data: 数据来源于盖亚卫星(Gaia)第三次数据发布(DR3)中一个星等完备、距离受限的恒星样本。

Method: 采用高纯度本地晕族恒星样本，基于盖亚(Gaia)DR3数据，通过横向速度筛选（>250 km/s）和绝对星等分析，构建了从次矮星（M_G~13）到亮巨星（M_G~-3）连续覆盖的晕族光度函数(LF)。

Result: 银河系恒星晕的光度函数（LF）在绝对星等$M_G=10$处呈现显著峰值，在$M_G\sim7$附近出现平坦化（Wielen dip），并首次连续测量了从亚矮星（$M_G\sim13$）到亮巨星（$M_G\sim-3$）的完整光度分布。
...
**TL;DR**: 在MUSE极深场(MXDF)中首次观测到宇宙网细丝(z=3.066)内蓝移主导的莱曼α(Lyα)发射与两个致密恒星形成区相关联，通过辐射传输模拟证实了100 km/s的气体团块径向流入，为宇宙网气体吸积触发极低质量星系(ULMGs)的星暴活动提供了直接证据。


<details>
  <summary>Details</summary>

Background: 研究主题为宇宙网（cosmic web）气体吸积对极低质量星系（ultra-low-mass galaxies）恒星形成的触发机制。

Data: 数据来源包括MUSE eXtremely Deep Field（MXDF）的观测数据以及JWST/NIRCam的深度成像数据。

Method: 采用团块状多相模型（clumpy multiphase model）对莱曼α（Lyα）谱线进行辐射传输模拟（radiative transfer simulation），推断出气体团块以100 km/s速度径向流入。

Result: 在z=3.066的宇宙网（cosmic web）细丝核心区域探测到由气体团块径向流入（速度100 km/s）驱动的蓝移主导莱曼α（Lyα）发射，直接观测到宇宙网气体吸积诱发极低质量星系（$\log(M_\star/M_\odot) = 6.3\text{-}6.9$）内爆发性恒星形成的证据。
...
**TL;DR**: 通过哈勃太空望远镜（HST/ACS）和詹姆斯·韦伯太空望远镜（JWST/NIRCam、JWST/NIRSpec）观测，发现一个红移（redshift）$z_{\rm spec} = 4.0148$的候选碰撞环星系（collisional ring galaxy），其形态包含半径约1.8千秒差距（kpc）的环状结构和三个亮团，恒星形成率（SFR）为140$^{+20}_{-30}$ ${\rm M}_{\rm \odot}$/年、恒星质量$\log(M_\ast/{\rm M}_\odot) = 10.41^{+0.11}_{-0.13}$，虽不能完全排除前景星系（$z \simeq 1.7$）对背景星系（$z \simeq 4.0$）产生强引力透镜（strong lensing）形成爱因斯坦环（Einstein ring）的可能性，但支持其为高红移碰撞环星系，并指出此类天体可能污染引力透镜巡天样本。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(high-redshift)环状星系(ring galaxies)的形成机制及其在星系演化动力学过程中的作用，特别关注碰撞性环状星系(collisional ring galaxies)与强引力透镜效应(strong lensing)的鉴别问题。

Data: 数据来源于哈勃太空望远镜(HST)的先进巡天相机(ACS)、詹姆斯·韦伯太空望远镜(JWST)的近红外相机(NIRCam)和近红外光谱仪(NIRSpec)对红移z=4.0148的候选环状星系进行的观测。

Method: 采用哈勃太空望远镜（HST/ACS）、詹姆斯·韦伯太空望远镜（JWST/NIRCam和JWST/NIRSpec）的多波段观测数据，结合光谱红移（z_spec）测量和形态学分析，通过对比高红移碰撞环星系（collisional ring galaxies）的恒星形成率（SFR）与恒星质量（M_*）特征，并排除强引力透镜（galaxy-galaxy strong lensing）假说。

Result: 观测到一个红移(redshift)为4.0148的候选环状星系(candidate ring galaxy)，其形态包含半径约1.8千秒差距(kpc)的环状结构上三个明亮团块，恒星形成率(SFR)为140±30太阳质量每年，恒星质量(log(M*/M⊙))为10.41±0.13。
...
### [14] [Merger-driven buildup of the $M_{\rm BH}$ - $M_*$ relation bridging high-$z$ overmassive black holes with the local relation](https://arxiv.org/abs/2603.11747)
*Takumi S. Tanaka,John D. Silverman,Kazuhiro Shimasaku,Knud Jahnke,Junyao Li,Makoto Ando*

**TL;DR**: 研究发现仅通过星系多次并合（merger）的非因果机制即可重现超大质量黑洞（SMBH）与星系（$M_*$）质量关系的局部弥散（$σ$），初始高红移（$z\sim6$）散射（$σ=0.8-1.0\,{\rm dex}$）与JWST观测一致，且弥散演化高度依赖并合质量比和并合率。


<details>
  <summary>Details</summary>

Background: 研究探讨了超大质量黑洞（SMBHs）与宿主星系（$M_*$）之间质量比例关系的起源，特别是通过非因果机制（non-causal mechanism）解释宇宙历史中该关系散射（$σ$）减小的现象。

Data: 数据来源于蒙特卡洛模拟（Monte Carlo simulation），结合了高红移观测对星系并合率（galaxy merger rate）和超大质量黑洞-星系质量关系散射（σ）的最新约束。

Method: 采用蒙特卡洛模拟（Monte Carlo simulation）分析星系及其超大质量黑洞（SMBHs）在纯并合驱动演化下的质量比例关系（$M_{\rm BH}/M_*$）散射（$σ$）变化，并整合高红移观测约束的并合率与散射值参数。

Result: 超大质量黑洞（SMBHs）与宿主星系（$M_*$）的质量比例关系（$M_{\rm BH}/M_*$）的弥散度（$σ$）可通过纯并合驱动的演化机制从高红移（$z\sim6$）的初始高弥散状态（$σ=0.8\,{\rm dex}$或$1.0\,{\rm dex}$）降至局域质量关系的观测值，且该演化对并合质量比和并合率高度敏感，次要并合（minor mergers）对弥散度演化同样具有贡献。
...
**TL;DR**: 基于盖亚(Gaia) DR3疏散星团(open cluster)样本分析最大恒星质量(m_max)与星团质量(M_cluster)关系演化，发现星团合并(subcluster coalescence)模拟比单星团演化更符合观测到的m_max-M_cluster关系随年龄分层现象，支持星团合并是疏散星团形成的主要途径。


<details>
  <summary>Details</summary>

Background: 研究主题为利用盖亚(Gaia) DR3疏散星团(open cluster)目录探究星团最亮恒星质量(m_max)与星团总质量(M_cluster)关系的演化机制及其对星团形成途径的约束。

Data: 数据来源为盖亚(Gaia) DR3疏散星团目录和N体模拟(N-body simulation)数据。

Method: 采用N体模拟(N-body simulations)分析单个星团演化和子星团并合过程，测试了四种气体抛射模式和两种并合情景。

Result: 随着星团年龄增长，m_max$-$M_cluster关系逐渐偏离初始m_max$-$M_ecl关系并呈现明显年龄分层。
...
### [16] [Continuum Reverberation in Bright Quasars Using NASA/ATLAS](https://arxiv.org/abs/2603.11789)
...
**TL;DR**: 通过分析9498个高光度类星体的连续谱反响映射，发现其吸积盘尺寸与标准盘理论预期的普遍差异（约3倍）在高光度样本中依然存在，且延迟幅度与波长呈现复杂非单调关系，表明可变弥散辐射对延迟的广泛影响。


<details>
  <summary>Details</summary>

Background: 活动星系核（AGN）的连续谱辐射变化可用于通过反响映射（reverberation mapping）探测其吸积盘结构。

Data: 数据来源为对9,498颗红移0.3-2.5范围内最亮类星体的高采样率光变曲线进行分析，这是迄今最大规模的连续谱反响映射研究。

Method: 采用连续谱反响映射（reverberation mapping）技术分析多波段光变曲线的时间延迟，通过联合拟合分箱样本的延迟参数提升探测精度。

Result: 低光度活动星系核（AGN）中观测到的连续谱波段间延迟普遍比标准盘理论预期长约3倍，且该尺寸差异可能在高光度AGN中减小。
...
### [17] [Binary disruption during the early phase of open clusters](https://arxiv.org/abs/2603.11803)
*Zepeng Zheng,Long Wang,Holger Baumgardt*

**TL;DR**: 研究发现年轻疏散星团（open cluster）的双星比例高于场星（field star），通过基于盖亚（Gaia）DR3数据的N体模拟揭示了双星瓦解呈现两阶段线性特征，早期瓦解率（k1）与初始密度（ρ0）呈幂律关系（指数约0.56），并开发了预测双星存活率的Python工具，同时指出逃逸至场星的双星比例因质量分层（mass segregation）而系统性偏低。


<details>
  <summary>Details</summary>

Background: 研究聚焦于年轻疏散星团（open clusters）中双星比例高于场星（field stars）的现象，探讨疏散星团内双星动力学演化对理解场双星起源和演化的关键作用。

Data: 数据来源为基于盖亚(Gaia)DR3疏散星团观测数据的N体模拟(N-body simulation)，并假设初始双星比例为100%。

Method: 采用N体模拟(N-body simulation)结合盖亚(Gaia)DR3疏散星团观测数据，并假设100%初始双星比例，研究双星在星团中的早期动力学演化。

Result: 年轻疏散星团中双星破坏呈现两阶段特征：初始快速下降（速率k1与初始密度ρ0呈指数0.56的幂律关系）后转为缓慢下降，转变时间tb与ρ0呈指数-0.46的幂律关系。
...
### [18] [Chemo-dynamical reconstruction of Milky Way globular cluster progenitors: age--metallicity relations and the universality of multiple stellar populations](https://arxiv.org/abs/2603.11814)
*Carmela Lardo,David Valcin,Raul Jimenez*

**TL;DR**: 通过重建69个银河系球状星团（globular clusters）的年龄-金属丰度关系（AMRs）并分析多星族（MP）特性，发现星团质量和金属丰度主导氦（helium）富集幅度，与起源无关，仅人马座（Sagittarius）星团化学演化程度更高，且Sequoia星群在固定质量和金属丰度下第一星族比例（f_P1）更高，表明氦富集过程具有普适性而f_P1保留环境印记。


<details>
  <summary>Details</summary>

Background: 研究主题为通过球状星团（globular clusters）的年龄-金属丰度关系（AMRs）和多恒星族（multiple stellar populations）特性揭示银河系的层级吸积历史和恒星形成物理机制。

Data: 数据来源于对69个银河系球状星团（globular clusters）的同质化恒星参数建模，包括年龄、氦丰度差异（δY）、平均氦丰度（$\bar{Y}$）和第一星族比例（$f_{\rm P1}$），这些参数通过分层贝叶斯CMD（color-magnitude diagram）建模获得，并基于化学动力学聚类识别了前身星系家族。

Method: 采用分层贝叶斯框架下的颜色-星等图（CMD）建模方法提取年龄、氦丰度参数，并通过化学动力学聚类识别前身星系家族。

Result: 大多数系统的化学演化程度达到Δ[Fe/H]~1.1-1.3 dex，而人马座(Sagittarius)达到~1.6 dex且具有更高的终端金属丰度。
...
### [19] [Energy dependence of the X-ray power spectrum in NGC4051 and NGC4395](https://arxiv.org/abs/2603.11825)
*V. A. Diamantopoulos,I. E. Papadakis,A. Akylas,A. Zoghbi,E. Kammoun,B. Rani*

**TL;DR**: 研究发现低质量赛弗特星系（Seyfert galaxies）NGC4051和NGC4395的功率谱密度（PSD）弯曲频率与能量无关，高频斜率随能量增加而变平，振幅随能量增加而减小。


<details>
  <summary>Details</summary>

Background: 活动星系核（AGN）在宽时标范围内表现出强烈的全电磁波段变异性，尤其在X射线波段呈现快速高幅波动，功率谱密度（PSD）分析是表征该变异性特征的主要方法。

Data: 数据来源于XMM-Newton、Suzaku和NuSTAR的存档观测数据。

Method: 采用弯曲幂律模型(bending power-law model)拟合六个能带(0.3-20 keV)的功率谱密度(PSD)，分析PSD参数（弯曲频率、高频斜率和振幅）的能量依赖性。

Result: PSD（功率谱密度）弯曲频率与能量无关，高频斜率随能量增加而变平，且PSD振幅随能量增加而减小。
...
### [20] [\HI 21-cm Line Properties of the Nearby LIRG IRAS 04296+2923](https://arxiv.org/abs/2603.11899)
*Guixiang Feng,Zhongzu Wu,Chuanpeng Zhang,Ming Zhu*

**TL;DR**: 通过分析VLA和FAST的\HI\ 21 cm数据及多波段射电连续谱观测，发现近邻亮红外星系IRAS~04296+2923与其伴星HI~0432+2926构成一个旋转主导、弱潮汐作用的束缚轨道星系对，其亮红外特性更可能由内部机制（如棒诱导气体流入）而非并合驱动。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近邻亮红外星系（LIRG）IRAS~04296+2923及其所在小星系群（small galaxy group）的动力学环境和相互作用机制。

Data: 使用了甚大天线阵（VLA）和五百米口径球面射电望远镜（FAST）的存档 \HI\ 21厘米数据，以及存档的多波段射电连续谱观测数据。

Method: 采用甚大天线阵(VLA)和五百米口径球面射电望远镜(FAST)的\HI\ 21厘米谱线数据及多波段射电连续谱观测数据，结合旋转主导运动学模型和自由-自由辐射(free-free emission)谱分解方法。

Result: IRAS~04296+2923和HI~0432+2926构成一个引力束缚的轨道星系对，嵌入小型星系群中，而非晚期并合系统。
...
### [21] [Chemical radial gradients for the bulge-bar stellar populations from the APOGEE survey](https://arxiv.org/abs/2603.12070)
...
**TL;DR**: 基于APOGEE和BAWLAS数据，在银河系核球-棒（bulge-bar）中识别出六种轨道和化学组成不同的星族，发现低/高[Mg/Fe]低偏心率星族分别呈现轻微负/正金属丰度梯度，高偏心率星族梯度平坦，且高[Mg/Fe]棒族元素丰度梯度显著陡峭，可能反映花生结构（peanut structure）的年龄变化。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系核球-棒结构（Milky Way bulge-bar）的化学和动力学特性，揭示其由多个不同轨道和化学组成的星族构成。

Data: 数据来源于APOGEE DR17（提供Mg、Si、Ca、Al、K、Mn、Co、Ni和Fe的化学丰度）和BAWLAS星表（提供Ce和Nd的化学丰度），样本覆盖$-2.0\lesssim$[Fe/H]$\lesssim+0.5$的金属丰度范围和银河系中心距离$R_{Gal}$ $<6$ kpc的区域。

Method: 基于化学丰度（[Mg/Fe]等）和运动学轨道特征对银河系核球-棒结构样本进行群体划分，结合APOGEE DR17和BAWLAS星表数据构建化学-运动学平面（chemical and kinematical planes）实现六类星群的动力学分类。

Result: 银河系核球-棒结构（Milky Way bulge-bar）中存在六种不同轨道和化学组成的星族，其中低/高[Mg/Fe]低偏心率星族分别呈现轻微负/正金属丰度梯度，而高偏心率星族金属丰度梯度平坦，且高[Mg/Fe]棒星族的中子俘获元素（neutron-capture elements）Ce和Nd的[X/H]梯度显著陡于低[Mg/Fe]棒星族。
...
### [22] [The shocking features in the closest rich galaxy cluster Norma](https://arxiv.org/abs/2603.12082)
*Chong Ge,Ming Sun,Mpati Ramatsoku,Chris Nolting,Bärbel S. Koribalski*

**TL;DR**: 在诺玛（Norma）星系团（A3627）西北侧探测到马赫数约1.3的并合激波（merger shock），该激波增强了后激波区域中星系（如ESO 137-001和ESO 137-007）的冲压剥离（RPS）效应，导致显著X射线尾和射电形态改变，并可能将喷流茧（jet cocoon）卷成涡环结构，同时揭示该星系团东南侧残留的冷核（cool core）可能被并合或活动星系核（AGN）破坏。


<details>
  <summary>Details</summary>

Background: 星系团并合激波通过提升星系团内介质(intracluster medium)压力显著影响嵌入星系演化，是研究星系团动力学与星系环境效应的关键课题。

Data: 数据来源于XMM-Newton和Chandra的X射线观测数据以及ASKAP的射电连续谱观测数据。

Method: 利用XMM-Newton和钱德拉(Chandra)X射线数据探测星系团合并激波(Mach数~1.3)，并分析激波对后震区(head-tail)射电星系(如ESO 137-007)和剥离星系(如ESO 137-001)的形态演化影响。

Result: 在诺玛星系团（A3627）西北侧探测到马赫数约1.3的并合激波（merger shock），该激波增强了冲压剥离效应（RPS）并可能诱导形成已知最亮的X射线尾迹，同时显著改变了星系团内星系（如ESO 137-001和ESO 137-007）的形态与结构。
...
### [23] [Comparison of Bar Formation Mechanisms. IIIA. The role of classical bulges in spontaneous bar formation](https://arxiv.org/abs/2603.12121)
*Yirui Zheng,Juntai Shen,Bin-Hui Chen*

**TL;DR**: 研究发现经典核球（classical bulges）通过增加托尔曼稳定性参数（Toomre Q）和中心区域圆周速度、降低盘质量分数，延缓棒结构（bar）形成并延长其增长时标，且在形成阶段核球质量和致密性导致棒结构初始模式速度（pattern speed）更高、减速更快，但在长期演化阶段盘成分主导模式速度演化。


<details>
  <summary>Details</summary>

Background: 研究探讨了经典核球（classical bulges）的质量和致密性如何通过内部形成机制影响星系棒（bar）的形成动力学和演化特性。

Data: 研究中使用的数据来源于一套孤立环境下运行的N体模拟（N-body simulations），通过调整不同质量和致密度的经典核球（classical bulges）参数生成模拟数据。

Method: 采用N体模拟(N-body simulations)分析不同质量和致密度的经典核球(classical bulges)对棒旋结构形成和演化的影响，重点考察核球对棒旋结构模式速度(pattern speed)和生长时间尺度的影响机制。

Result: 经典核球（classical bulges）的质量和致密性增加会延迟棒旋结构（bar）的形成并延长其生长时标，足够强的核球可完全抑制棒旋结构的形成。
...
### [24] [AstroSat-UVIT observations of a possibly interacting pair of galaxies in HCG 77](https://arxiv.org/abs/2603.12173)
*Anshul Srivastava,Harvinder K. Jassal,Mamta Gulati,Kulinder P. Singh*

**TL;DR**: 利用AstroSat卫星的紫外成像望远镜（UVIT）对希克森致密群77中的PGC 56121和PGC 56125这对可能相互作用的星系进行了高分辨率远紫外（FUV）观测，发现了多个恒星形成区、一个候选潮汐矮星系及其三个主要恒星形成区，并通过光谱能量分布分析揭示了这三个星系的物理特性及相互作用证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于希克森致密星系群77（Hickson Compact Group 77）中可能相互作用的星系PGC 56121和PGC 56125的恒星形成区及光谱能量分布（spectral energy distribution）。

Data: 数据来源于搭载在AstroSat上的紫外成像望远镜(Ultra Violet Imaging Telescope, UVIT)的远紫外(Far-Ultraviolet, FUV)通道观测数据，其分辨率显著高于GALEX的近紫外(Near-UV)和PS-1、DSS的先前星系巡天数据。

Method: 采用远紫外成像望远镜（UVIT）的高分辨率远紫外（FUV）通道观测数据，结合光谱能量分布（SED）分析技术，研究了PGC 56121和PGC 56125的恒星形成区及相互作用特征。

Result: 利用星载紫外成像望远镜（UVIT）的高分辨率远紫外（FUV）图像，在希克森致密群77中观测到PGC 56121和PGC 56125这对可能相互作用的星系内存在多个恒星形成区，并在其东侧潮汐尾末端发现了一个候选潮汐矮星系（tidal dwarf galaxy）。
...
### [25] [GATOS N: The first direct kinematic evidence of dusty outflows from AGN via PAH kinematics of local Seyfert galaxies with JWST](https://arxiv.org/abs/2603.12200)
...
**TL;DR**: 首次在活动星系核(AGN)外流中通过多环芳烃(PAH)特征动力学检测到尘埃，发现11.3和17微米PAH特征速度场与高电离势线[NeVI]匹配，表明外流PAH比恒星形成区更大且更中性。


<details>
  <summary>Details</summary>

Background: 研究首次利用空间分辨的运动学证据探讨了活动星系核（AGN）外流中的尘埃（dust）特性。

Data: 数据来源于JWST（James Webb Space Telescope）的NIRSpec IFU（近红外光谱仪积分场单元）和MIRI MRS（中红外仪器中分辨率光谱仪）对10个本地塞弗特星系（Seyferts）的观测。

Method: 采用主成分分析层析成像技术（PCA tomography）提取多环芳烃（PAH）特征的运动学信息。

Result: 首次在活动星系核(AGN)外流中检测到多环芳烃(PAH)的动力学特征，其速度与分子气体(H$_2$旋转跃迁示踪)相似，且大中性PAH(11.3 $μ$m和17 $μ$m)的运动学与高电离势线([NeVI])匹配，表明外流中PAH比恒星形成区更大更中性。
...
### [26] [Comparing the $M_{gas}-N_{yso}$ Relation inside a Giant Molecular Cloud](https://arxiv.org/abs/2603.12212)
*Carlos G. Román-Zúñiga,Aina Palau,Javier Ballesteros-Paredes,Manuel Zamora-Avilés,Joshua Peltonen,Karla Gutiérrez-Davila*

**TL;DR**: 基于猎户座A分子云（Orion A）的柱密度图和年轻恒星目录分析，验证了在分子云内部到云际尺度上年轻恒星数量（N_yso）与气体质量（M_gas）呈线性关系，并探讨了自由下落时标对致密气体区恒星形成效率的影响。


<details>
  <summary>Details</summary>

Background: 研究围绕分子云内(intra-cloud)尺度上基于施密特猜想(Schmidt conjecture)的恒星形成标度关系展开，重点探讨猎户座A分子云(Orion A)中致密气体结构与恒星形成活动的关联。

Data: 数据来源于赫歇尔(Herschel)远红外地图构建的猎户座A分子云(Orion A)柱密度图，并结合已知年轻恒星列表进行结构分离和当前嵌入恒星群估计。

Method: 采用分层树（dendrograms）方法在恒定阈值（$A_V$ = 7 mag）上分离猎户座A分子云柱密度图中的气体结构，并结合年轻恒星目录构建了$N_{yso}$ vs. $M_{gas}$、$Σ_{SFR}$ vs. $Σ_{gas}$和$R_{eq}$ vs. $M_{gas}$三个标度关系的系列图。

Result: result：在猎户座A分子云（Orion A）内尺度分析中，证实了年轻恒星数量（N_yso）与气体质量（M_gas）在三个数量级上呈线性关系，且该关系可能适用于更大尺度范围，同时发现恒星形成率面密度（Σ_SFR）与气体面密度（Σ_gas）的关系与先前拟合结果不一致。
...
### [27] [An Updated \synthpop Model for Microlensing Simulations I: Model Description, Evaluation, and Microlensing Event Rates Near the Galactic Center](https://arxiv.org/abs/2603.12219)
...
**TL;DR**: 更新了银河系模型（Galactic model）在\synthpop框架中的实现，针对南希·格雷斯·罗曼空间望远镜（Nancy Grace Roman Space Telescope）的银河系核球时域巡天（RGBTDS）进行了优化，发现该模型在大部分核球区域（包括RGBTDS连续低核球区域）与观测数据吻合良好，但在银河平面纬度$b\lesssim0.5^\circ$内存在不一致性，可能影响RGBTDS银河中心区域的预测。


<details>
  <summary>Details</summary>

Background: 研究聚焦于优化银河系模型以支持微引力透镜巡天（microlensing surveys）的准确模拟和解释，特别是针对南希·格雷斯·罗曼空间望远镜（Nancy Grace Roman Space Telescope）的银河系核球时域巡天（RGBTDS）。

Data: 数据来源包括光学和红外巡天（optical and infrared surveys）对银河系核球（Galactic bulge）的恒星目录（stellar catalogs）和运动学（kinematics）数据。

Method: 采用更新后的银河系模型实现（Galactic model implementation）在合成星族框架（synthpop framework）内，针对南希·格雷斯·罗曼空间望远镜的银河核球时域巡天（RGBTDS）进行了优化调参。

Result: 在银河系中心区域（Galactic latitudes $b\lesssim0.5^\circ$）发现了与现有模型不一致的现象，可能影响南希·格雷斯·罗曼空间望远镜（Nancy Grace Roman Space Telescope）银河核球时域巡天（RGBTDS）的银河中心场（Galactic center field）预测结果。
...
### [28] [Kinematic cosmic dipole from a large sample of strong lenses](https://arxiv.org/abs/2603.11152)
*Martin Millon,Charles Dalang,Thomas Collett,Camille Bonvin*

**TL;DR**: 提出利用强引力透镜（strong gravitational lenses）的爱因斯坦半径（Einstein radius）测量结合运动学信息（kinematic information）来独立估计宇宙运动学偶极（kinematic cosmic dipole），并证明该方法能以约4σ显著性区分宇宙微波背景（CMB）与高红移源计数（source number counts）得出的观测者本动速度（peculiar velocity）差异。


<details>
  <summary>Details</summary>

Background: 该研究探讨了宇宙运动学偶极子（kinematic cosmic dipole）的测量不一致问题，特别是通过引力透镜的爱因斯坦半径（Einstein radius）来估计观测者本动速度（peculiar velocity）引起的相对论性畸变。

Data: 数据来源为欧几里得（Euclid）宽视场高分辨率成像巡天、4MOST或DESI的光谱速度弥散测量（spectroscopic velocity-dispersion measurements）。

Method: 利用强引力透镜（strong gravitational lenses）的爱因斯坦半径（Einstein radius）测量和光谱速度弥散（spectroscopic velocity dispersion）数据，结合相对论性畸变效应（relativistic distortion）分析观测者本动速度（peculiar velocity）对宇宙运动学偶极（kinematic cosmic dipole）的影响。

Result: 通过测量强引力透镜的爱因斯坦半径（Einstein radius）并结合透镜星系运动学信息（如基本平面关系或光谱速度弥散），可在欧几里得（Euclid）等巡天数据支持下以约4σ显著性区分宇宙微波背景（CMB）与高红移源计数推断的运动学宇宙偶极（kinematic cosmic dipole）幅度差异。
...
### [29] [FlowSN: Normalising Flows for Simulation-Based Inference under Realistic Selection Effects applied to Supernova Cosmology](https://arxiv.org/abs/2603.11165)
...
**TL;DR**: 提出FlowSN框架，利用基于模拟的推断和归一化流（normalising flows）解决观测天文学中的选择效应（selection effects）问题，显著降低了Ia型超新星（SN Ia）宇宙学参数（如暗能量状态方程$w_0$）的推断偏差并提高了频率校准精度。


<details>
  <summary>Details</summary>

Background: 观测天文学中需解决选择效应（selection effects）导致的全局参数推断偏差问题，特别是在Ia型超新星（SN Ia）宇宙学中，选择效应会系统性地影响宇宙学参数后验分布推断。

Data: 数据来源于首次使用的类LSST（Large Synoptic Survey Telescope）SNANA模拟（simulation）。

Method: 采用基于模拟的推断（simulation-based inference）结合归一化流（normalising flows）学习非解析选择效应下的超新星似然函数，并通过哈密顿蒙特卡洛（Hamiltonian Monte Carlo）进行后验采样。

Result: FlowSN框架通过归一化流(normalising flows)和基于模拟的推断(simulation-based inference)有效校正了观测天文学中的选择效应(selection effects)，显著降低了宇宙学参数（如暗能量状态方程$w_0$）的后验估计偏差，其偏差量级比传统方法低一个数量级且频率校准更优。
...
### [30] [Spontaneous Baryogenesis and Primordial Black Hole Dark Matter from Ultra-Slow-Roll Inflation](https://arxiv.org/abs/2603.11177)
*Shyam Balaji*

**TL;DR**: 该论文提出统一框架，将暗物质（DM）、宇宙重子不对称性和可探测随机引力波（GW）背景的起源归因于超慢滚（USR）暴胀，发现原初黑洞（PBH）丰度与重子产率存在预测性关联，并通过参数扫描揭示不同谱尾斜率对暴胀能标和未来观测（如CMB B模、LISA/DECIGO/ET引力波探测）的约束关系。


<details>
  <summary>Details</summary>

Background: 研究探讨了超慢滚暴胀（USR）如何统一解释暗物质（DM）、宇宙重子不对称性及可探测随机引力波背景（GW）的起源。

Data: 数据来源于对超慢滚暴胀（USR inflation）阶段的参数空间扫描，包括原初功率谱峰后斜率、宇宙微波背景（CMB）尺度上的张量标量比（r_CMB）以及再加热温度（T_reh）的函数关系。

Method: 采用超慢滚（USR）暴胀模型结合导数耦合机制，通过抑制暴胀子速度实现原初黑洞（PBH）暗物质产生并触发自发重子生成，同时关联PBH丰度与重子产额并约束再加热温度（T_reh）。

Result: 超慢滚(USR)暴胀阶段通过初始暴胀子速度的急剧抑制，同时解释了暗物质(DM)的原初黑洞(PBH)产生机制和自发重子生成过程，并建立了PBH丰度与重子产额之间的预测性关联。
...
### [31] [Imprints of Reheating Dynamics on Gravitational Waves from Phase Transitions](https://arxiv.org/abs/2603.11184)
*Basabendu Barman,Maciej Kierkla,Marek Lewicki,Marco Merchand*

**TL;DR**: 研究发现暴胀后扰动再加热（perturbative reheating）通过改变宇宙背景有效状态方程（effective equation of state）抑制了宇宙学相变（cosmological phase transitions）产生的原初引力波（primordial gravitational wave）谱强度，并产生可作为再加热时期膨胀历史特征标识的谱特征。


<details>
  <summary>Details</summary>

Background: 研究探讨了暴胀后扰动再加热（perturbative reheating）如何改变宇宙学相变（cosmological phase transitions）产生的原初引力波（primordial gravitational wave）谱。

Data: 数据来源于特定暴胀（inflation）模型下的扰动再加热（perturbative reheating）过程模拟，通过假设标量场凝聚体（inflaton condensate）以玻色子或费米子对形式耗散来计算相变（phase transition）产生的引力波（GW）信号。

Method: 采用扰动再加热（perturbative reheating）模型分析暴胀后原初引力波（primordial gravitational wave）谱的修正效应，结合暴胀子势能形式确定宇宙背景有效状态方程（effective equation of state），量化相变热力学参数对再加热时期膨胀历史的依赖性。

Result: 膨胀后的微扰再加热（perturbative reheating）会系统性抑制宇宙学相变产生的原初引力波（primordial gravitational wave）信号强度，并产生可区分再加热时期膨胀历史改变的特征谱特征。
...
### [32] [Gravitational Lensing Effects by Galaxy Clusters on Ionised Bubble Size Distribution during the Epoch of Reionisation](https://arxiv.org/abs/2603.11371)
*Di Wu,Nan Li,Huanyuan Shan,Zhenghao Zhu*

**TL;DR**: 通过多重透镜平面模拟研究发现，星系团引力透镜效应（lensing）显著增加了再电离时期（EoR）大电离泡（R > 15 cMpc）的数量（EOS暗弱星系模型在z=14时增加219%，明亮星系模型增加832%），但对小电离泡数量无影响，这为平方公里阵列（SKA）时代的电离泡尺度分布（BSD）研究提供了必须考虑的系统误差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于再电离时期（EoR）中电离结构的统计特性如何受前景大质量结构（如星系团）引力透镜效应的影响。

Data: 数据来源包括基于蒙特卡洛方法生成的偏转体（deflector）群体（依据晕质量函数和经验标度关系）、采用截断纳瓦罗-弗伦克-怀特（TNFW）模型描述的偏转体质量分布，以及通过半数值方法生成或直接来自21厘米结构演化（EOS）项目的源光锥（source light cone）。

Method: 采用多透镜平面透镜模拟（multiple-lens-plane lensing simulations）结合蒙特卡洛方法生成偏转体（deflector）分布，并利用截断纳维-斯托克斯-弗兰克-怀特模型（TNFW）描述偏转体质量剖面，同时基于半数值方法或21厘米结构演化项目（EOS）数据生成源光锥，通过平均自由程方法（Mean Free Path method）测量未透镜与透镜化电离气泡尺寸分布（BSD）。

Result: 引力透镜效应增加了大电离泡（R > 15 cMpc）的数量，但对小电离泡数量无影响，其中EOS暗星系模型在z=14时大泡数量增加219%，EOS亮星系模型在相同条件下增加832%。
...
### [33] [Warm Inflation Beyond the Markovian Limit](https://arxiv.org/abs/2603.11629)
*Mayukh R. Gangopadhyay,Nilanjana Kumar*

**TL;DR**: 研究了暖暴胀（warm inflation）中非马尔可夫性（non-Markovian）噪声对原初标量功率谱的影响，发现记忆效应会抑制标量谱并建立了修正项与背景热力学参数的关系，揭示了热浴温度与哈勃尺度之比直接决定了马尔可夫近似的有效性。


<details>
  <summary>Details</summary>

Background: 研究探讨了暖暴胀（warm inflation）中非马尔可夫性（non-Markovian）噪声对原初标量功率谱的影响及其与背景热动力学的关系。

Data: 数据来源于对暖暴胀（warm inflation）模型中非马尔可夫（non-Markovian）噪声效应的理论研究，通过分析背景热动力学参数（如浴温度与哈勃尺度之比）与功率谱修正的关系，建立了有限关联时间效应的诊断方法。

Method: 采用非马尔可夫（non-Markovian）随机力模型分析暖暴胀（warm inflation）中有限关联时间对原初标量功率谱的影响。

Result: 记忆效应会抑制标量功率谱（scalar power spectrum），其修正幅度与背景热力学参数（thermal ratio）直接相关。
...
### [34] [Constraints on Axion-Photon Mixing from Fast Radio Burst Dispersion Measures](https://arxiv.org/abs/2603.11657)
*Gunalan Muthusami,Gopal Kashyap*

**TL;DR**: 通过贝叶斯分析和局部化快速射电暴(FRB)样本，首次约束了轴子质量(ma=1.16+4.40−1.08μeV)和轴子-光子耦合常数(gaγγ=(1.76+6.69−1.64)×10−16GeV−1)，并验证了高磁场中子星(NS)磁层中轴子-光子混合机制对观测色散(DM)的贡献。


<details>
  <summary>Details</summary>

Background: 快速射电暴(FRB)可作为探针研究电离宇宙的色散量(DM)，但其宿主星系和源周围环境的贡献仍不确定，且物理起源尚待研究。

Data: 使用一组具有测量红移的定位快速射电暴(FRB)样本进行贝叶斯马尔可夫链蒙特卡洛(MCMC)分析。

Method: 采用贝叶斯马尔可夫链蒙特卡洛（MCMC）分析和非参数高斯过程重构（GPR）技术约束轴子质量（ma）和轴子-光子耦合常数（gaγγ）。

Result: 通过贝叶斯马尔可夫链蒙特卡洛(MCMC)分析约束轴子质量$m_a$和轴子-光子耦合常数$g_{aγγ}$，得到$m_a = 1.16^{+4.40}_{-1.08}\,μ{\rm eV}$、$g_{aγγ} = (1.76^{+6.69}_{-1.64})\times10^{-16}\,{\rm GeV}^{-1}$以及星际重子分数$f_{\rm IGM} = 0.837^{+0.053}_{-0.056}$。
...
### [35] [A unified scalar-field resolution of the $H_0$, $S_8$ and evolving Dark Energy tensions](https://arxiv.org/abs/2603.11706)
*Gerasimos Kouniatalis*

**TL;DR**: 该论文提出了一种统一标量场（scalar-field）框架，通过在指数尾势上叠加局域凸起势能，实现了早期能量注入提升哈勃常数（$H_0$）、中期快速红移避免残留效应、晚期类精质（quintessence-like）演化缓解$S_8$张力的三阶段宇宙学异常解决方案。


<details>
  <summary>Details</summary>

Background: 研究主题为通过标量场（scalar-field）统一框架在标准广义相对论中解决哈勃常数（$H_0$）张力、晚期结构形成减弱（$S_8$）及暗能量演化等当前宇宙学异常问题。

Data: 数据来源于标准广义相对论框架下的统一标量场模型（unified scalar-field framework）的数值模拟（simulation），该模型通过叠加局部凸起（localized bump）和指数尾部（exponential tail）的光滑势能（smooth potential）演化单个最小耦合规范标量场（minimally coupled canonical scalar field）。

Method: 采用单一最小耦合规范标量场（canonical scalar field）在由局部隆起叠加指数尾构成的平滑势能中演化的物理模型。

Result: 提出的标量场模型通过单一光滑势能（含局部隆起和指数尾部）实现了早期能量注入、快速稀释和晚期解冻行为，同时缓解了哈勃常数（H_0）张力、降低了晚期物质成团（S_8）并支持暗能量状态方程参数（w_0>-1, w_a<0）的演化。
...
### [36] [The impact of baryons on weak lensing statistics as a function of halo mass and radius](https://arxiv.org/abs/2603.11815)
*Max E. Lee,Zoltan Haiman,Shy Genel*

**TL;DR**: 通过逐步替换暗物质模拟(DMO)中不同质量晕及其周围区域为流体力学模拟结果，发现替换质量≥10¹²h⁻¹M⊙晕至5R₂₀₀范围可恢复90%重子效应导致的功率谱(P(k))和弱引力透镜功率谱(Cℓ)抑制，剩余10%来自更小质量晕或晕外物质，并揭示了不同统计量对重子分布的特异性敏感模式及现有重子修正模型(BCMs)在校准P(k)时对核心质量低估与大半径效应高估的抵消性错误。


<details>
  <summary>Details</summary>

Background: 研究聚焦于弱引力透镜（WL）巡天中重子效应（baryonic effects）的系统误差控制，探索重子修正模型（BCMs）在暗物质模拟（DMO）中替代不同质量与半径区域的有效性及其对多类统计量的影响。

Data: 研究中使用的数据来源包括暗物质数值模拟（DMO simulations）和流体动力学模拟（hydrodynamical simulations），通过逐步替换暗物质模拟中不同质量范围（$M\geq10^{12}\,h^{-1}\,{\rm M}_\odot$）和半径范围（$r\leq5R_{200}$）的晕结构（halos）来生成"Replace"场数据。

Method: 采用半解析重子修正模型（BCMs）对暗物质模拟（DMO）中的晕轮剖面进行修正，并通过逐步替换不同质量与半径范围的流体动力学模拟区域来评估模型精度。

Result: 在暗物质模拟中替换质量$M\geq10^{12}\,h^{-1}\,{\rm M}_\odot$且半径$r\leq5R_{200}$的晕后，可恢复物质功率谱($P(k)$)和弱引力透镜功率谱($C_\ell$)中约90%的重子抑制效应，剩余10%源自更低质量晕或晕外物质。
...
### [37] [Cosmological gravity on all scales V: MCMC forecasts combining large scale structure and CMB lensing for binned phenomenological modified gravity](https://arxiv.org/abs/2603.11895)
*Sankarshana Srinivasan,Shreya Prabhu,Kai Lehman,Ajiv Krishnan V.,Jochen Weller*

**TL;DR**: 该论文通过快速模拟非线性修正引力效应，在红移分箱参数化下实现了对时变有效引力常数（μ）和引力滑移（η）的高精度（<1%）物质功率谱预测，结合LSST和CMB透镜联合数据分析揭示了μ与η的退化关系及其主导透镜势的组合Σ=μ(1+η)/2，同时验证了大尺度结构与CMB透镜分别对低红移和高红移结构增长的敏感性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于在宇宙学进入第四阶段大规模结构巡天数据主导时代背景下，非线性尺度建模面临的挑战，特别是超越$w$CDM模型的修正引力理论（modified gravity）参数化问题。

Data: 数据来源包括模拟的3×2点LSST Y10类数据向量和6×2点LSST Y10与西蒙斯天文台宇宙微波背景（CMB）透镜数据向量。

Method: 采用COLA（COmoving Lagrangian Acceleration）模拟和红移分箱方法对时变有效引力常数μ和引力滑移η进行参数化，构建了非线性修正引力物质功率谱快速仿真器。

Result: 通过快速模拟非线性修正引力效应，实现了对时变有效引力常数（μ）和引力滑移（η）分箱参数化模型的高精度（<1%）功率谱预测，支持基于实际巡天数据的全贝叶斯分析。
...
### [38] [The Targeted Standard Siren Cosmology with Pulsar Timing Arrays](https://arxiv.org/abs/2603.12168)
*Shubhit Sardana,Boris Goncharov,Jacob Cardinal Tremblay*

**TL;DR**: 提出通过靶向搜索（targeted searches）而非全天空盲搜（all-sky blind searches）探测超大质量黑洞双星（SMBHBs），利用中国脉冲星计时阵（CPTA）可将哈勃常数（Hubble constant）的推断精度提升至2 km/s/Mpc，为解决哈勃张力（Hubble tension）提供关键支持。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用脉冲星计时阵列（PTA）探测超大质量黑洞双星（SMBHB）引力波信号，以解决其大尺度天区定位（约10至100平方度）对宇宙学标准汽笛（standard siren）应用的制约问题。

Data: 数据来源为基于当前脉冲星计时阵列（PTA）观测信息生成的模拟数据（simulated data）。

Method: 采用目标搜索（targeted searches）而非全天空盲搜索（all-sky blind searches）方法分析超大质量黑洞双星（SMBHBs）的引力波信号，并结合基于当前脉冲星计时阵列（PTA）观测的模拟数据。

Result: 通过定向搜索（targeted searches）而非全天空盲搜索（all-sky blind searches）方法，利用中国脉冲星计时阵列（CPTA）模拟数据可将哈勃常数（Hubble constant）的测量精度提升至2 km/s/Mpc。
...
### [39] [Scale-Dependent Loop Corrections to the Inflationary Power Spectrum](https://arxiv.org/abs/2603.12216)
*Matteo Braglia,Sebastián Céspedes,Lucas Pinol*

**TL;DR**: 提出了适用于强破坏德西特对称性并产生尺度依赖特征的暴胀背景的一致重整化方法，证明了紫外发散和蝌蚪项可通过有限局域对偶项消除，并首次展示了局部特征产生的重整化单圈功率谱在大、小尺度均消失。


<details>
  <summary>Details</summary>

Background: 研究主题为在破坏德西特对称性（de Sitter symmetry）的暴胀背景下，建立适用于含尺度依赖特征的原初功率谱的紫外发散重整化（renormalisation）框架。

Data: 研究采用有效场论（EFT）框架下的理论模拟数据，通过任意时间依赖的威尔逊系数（Wilson coefficients）进行计算，并具体分析了共振特征（resonant features）和尖锐特征（sharp features）两种实现方式。

Method: 采用有效场论（EFT）框架下的重整化方法处理强破坏德西特对称性的暴胀背景，通过局部抵消项消除紫外发散和tadpole（时间与尺度依赖）。

Result: 建立了适用于强烈破坏德西特对称性并产生尺度依赖特征的暴胀背景的一致重整化程序。
...
### [40] [Searching for Life-As-We-Don't-Know-It: Mission-relevant Application of Assembly Theory for Exoplanet Life Detection](https://arxiv.org/abs/2603.11086)
*Sara Walker,Estelle Janin,Evgenya Shkolnik,Louie Slocombe*

**TL;DR**: 提出将组装理论（Assembly Theory, AT）应用于行星大气分析的新框架，通过量化分子集合的最小组合复杂度来连续评估行星化学空间的选择和进化程度，为可居住世界天文台（Habitable Worlds Observatory, HWO）提供不依赖特定生物化学假设的生命探测方法。


<details>
  <summary>Details</summary>

Background: 研究主题为将组装理论（Assembly Theory, AT）应用于行星大气层分析，作为宜居世界天文台（Habitable Worlds Observatory, HWO）的生物特征检测框架，通过量化分子集合的最小组合复杂度来评估行星大气化学空间中编码的选择与演化程度，无需预设特定生物化学、动力学或代谢机制。

Data: 研究中使用的数据来源包括现有光谱数据（existing spectroscopic data）和未来HWO（Habitable Worlds Observatory）的观测数据。

Method: 采用组装理论（Assembly Theory, AT）量化行星大气分子集合的最小组合复杂度，提供衡量化学空间中选择和演化程度的连续指标。

Result: 基于组装理论（Assembly Theory, AT）的行星大气分析框架可提供行星复杂度的连续测量，为探测未知生命形式开辟新途径。
...
### [41] [SpectralUnmix: A Torch-Based Regularized Non-negative Matrix Factorization](https://arxiv.org/abs/2603.11111)
*Rafael S. de Souza,Paula Coelho,Niranjana P,Ana L. Chies-Santos,Rogério Riffel*

**TL;DR**: 开发了基于torch的R包SpectralUnmix，通过近端梯度更新实现光谱轴平滑正则化的非负矩阵分解(NMF)，并验证了其在恒星光谱分析中的有效性。


<details>
  <summary>Details</summary>

Background: 开发用于光谱数据分解的正则化非负矩阵分解(NMF)算法工具。

Data: 研究中使用的数据来源为恒星光谱（stellar spectra）子集。

Method: 采用基于torch实现的非负矩阵分解(NMF)方法，支持GPU加速，通过近端梯度更新估计低秩非负表示，并允许沿光谱轴进行平滑正则化。

Result: 开发了基于torch框架的R包SpectralUnmix，支持GPU加速的平滑正则化非负矩阵分解(NMF)光谱解混。
...
### [42] [The ocean worlds science case for the Pollux spectropolarimeter](https://arxiv.org/abs/2603.11112)
...
**TL;DR**: Pollux（欧洲候选仪器）作为高分辨率光谱仪（分辨率>40,000）具备紫外至近红外连续覆盖和偏振测量能力，将用于研究太阳系海洋天体（ocean worlds）表面反射特性、大气辉光及气溶胶微观物理性质，并搜寻系外行星生命迹象。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是探索宇宙中宜居系外行星（extrasolar planets）和太阳系海洋天体（ocean worlds）的生命迹象及其形成演化机制。

Data: 数据来源为高分辨率光谱仪（Pollux）的紫外（UV）至近红外（NIR）波段观测数据，包括表面反射率、气辉（airglow）发射和大气气溶胶（aerosols）的微物理特性。

Method: 采用高分辨率（R>40,000）紫外至近红外（100 nm至1.9 µm）光谱偏振测量技术（spectropolarimetric capabilities）分析表面反射率、成分特征和气辉（airglow）发射。

Result: Pollux（英文原词Pollux）作为HWO（Habitable Worlds Observatory）的候选欧洲仪器，旨在通过高分辨率（R>40,000）紫外至近红外（100 nm至1.9 µm）光谱偏振观测，研究宇宙结构形成演化并搜寻系外行星生命迹象。
...
### [43] [Masses of Potentially Habitable Planets Characterized by the Habitable Worlds Observatory](https://arxiv.org/abs/2603.11146)
*Kaz Gary,B. Scott Gaudi,Eduardo Bendek,Tyler Robinson,Renyu Hu,Breann Sitarski,Aki Roberge,Eric Mamajek*

**TL;DR**: 该论文提出利用宜居世界天文台（HWO）的高分辨率仪器进行超精密天体测量，通过模拟参考星分布和光子噪声误差预算，确定在盖亚G波段进行为期200天、包含100个观测历元的巡天方案，能以约10%精度测量约40颗地球质量宜居带行星的质量。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用宜居世界天文台（HWO）通过高精度天体测量（astrometry）约束直接成像系外行星质量，以评估其大气成分和潜在宜居性。

Data: 数据来源为基于盖亚(Gaia)G波段滤波器的观测数据，通过6米口径HWO高分辨率仪器获取，视场为6' x 6'，包含约100个观测历元，分布在5年主要任务期间，覆盖银河系不同经度和纬度的参考星。

Method: 采用超高精度天体测量（ultra-high-precision astrometry）技术，结合盖亚（Gaia）G波段观测和光子噪声误差预算分析，通过模拟不同天区参考星分布评估天体测量不确定性。

Result: 基于盖亚(Gaia)G波段200天巡天观测，配合6米口径HWO和6' x 6'视场，通过100次目标星观测可实现约40颗地球质量宜居带行星质量测量精度达10%。
...
**TL;DR**: 提出预测增强型Landy-Szalay（PP-LS）估计器，通过结合全样本噪声标记和小规模光谱样本精确标记，在无需概率校准或污染建模条件下，有效校正成像巡天两点相关函数（2PCF）测量中因目标选择偏差和空间结构化污染导致的虚假大尺度信号。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决成像巡天中两点相关函数（2PCF）因目标选择与数据处理流程不完善导致的污染与样本不完整问题。

Data: 数据来源包括全样本的噪声包含标签和一个小型高分辨率光谱（spectroscopic）子集的精确标签。

Method: 采用预测赋权兰迪-萨莱（PP--LS）估计器，结合全样本噪声标记与小光谱子集精确标记，通过基于残差的设计加权校正消除偏差，无需概率校准或已知误分类率。

Result: 提出了一种预测驱动的Landy-Szalay（PP-LS）估计器，通过结合全目录的噪声包含标签和小型光谱子集的精确标签，在保持标准随机目录归一化的同时，利用标记子集上的残差加权校正来消除配对计数偏差。
...
### [45] [Proto-NUX: A prototype telescope for ground-based near-ultraviolet observations](https://arxiv.org/abs/2603.11336)
*Rasjied Sloot,Rudy Wijnands,Steven Bloemen,Rik ter Horst,Hans Ellermeijer,Alexander Hoogerbrug*

**TL;DR**: 提出近紫外探测器（NUX）原型机Proto-NUX，通过改装36厘米宽场天体摄影仪在300-350 nm波段验证阵列可行性并量化大气消光（atmospheric extinction）特性，重点区分瑞利散射（Rayleigh scattering）和臭氧吸收的波长依赖性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发近紫外波段（NUV）地面广角望远镜阵列（NUX）及其原型机（Proto-NUX），旨在探测和表征近紫外波段高温快速演化暂现源（transients），并量化大气消光（atmospheric extinction）在该波段的特性。

Data: 使用改装后的36厘米Celestron RASA宽视场天体照相仪（Proto-NUX）进行观测，并计划于2026年在法国Pic du Midi天文台（海拔2877米）进行实地测试。

Method: 采用改进型36厘米Celestron RASA宽场天体照相仪（Proto-NUX）结合三种滤光片配置测量大气消光（atmospheric extinction）的波长依赖性，区分瑞利散射（Rayleigh scattering）和臭氧分子吸收的主导贡献。

Result: Proto-NUX旨在通过三种滤光片配置测量大气消光（atmospheric extinction）的波长依赖性，区分瑞利散射（Rayleigh scattering，主导波长>325 nm）和臭氧分子吸收（主导波长<315 nm）的贡献。
...
### [46] [Exploring the Viability of Fisher Discriminants in Galaxy Morphology Classification](https://arxiv.org/abs/2603.11652)
*Sazatul Nadhilah Zakaria,Santtosh Muniyandy,John Y. H. Soo*

**TL;DR**: 通过比较四种机器学习算法（费希尔判别、人工神经网络、提升决策树和k近邻）对斯隆数字巡天（SDSS）星系中心隆起形态的分类效果，发现采用均匀化预处理的费希尔判别法以93.10%的准确率最优，比人工神经网络、提升决策树和k近邻分别高出1.93%、0.42%和3.08%。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于探索费希尔判别（Fisher discriminants）这一简化算法在星系形态分类中的可行性，以解决现有复杂算法处理时间长且可解释性差的问题。

Data: 数据来源于斯隆数字巡天（Sloan Digital Sky Survey, SDSS）。

Method: 采用费希尔判别（Fisher discriminants）、人工神经网络（ANNs）、提升决策树（BDTs）和k近邻（kNNs）四种机器学习算法，结合归一化、去相关、主成分分析（PCA）、均匀化（uniformisation）和高斯化五种数据预处理变换，基于斯隆数字巡天（SDSS）数据对星系中心隆起形状进行分类。

Result: Fisher判别分析(Fisher discriminants)结合均匀化预处理在星系中心隆起形态分类中准确率达0.9310，优于人工神经网络(ANNs)、提升决策树(BDTs)和k近邻(kNNs)算法。
...
### [47] [AS-Bridge: A Bidirectional Generative Framework Bridging Next-Generation Astronomical Surveys](https://arxiv.org/abs/2603.11928)
*Dichang Zhang,Yixuan Shao,Simon Birrer,Dimitris Samaras*

**TL;DR**: 提出了一种双向生成模型AS-Bridge（Astronomical Survey-Bridge），通过基于扩散模型（diffusion model）的随机布朗桥（Brownian Bridge）过程实现地面LSST和空间Euclid观测数据的相互转换，支持跨巡天联合分析并提升科学产出。


<details>
  <summary>Details</summary>

Background: 未来十年观测宇宙学的发展将由大型巡天项目（如地基的LSST和天基的Euclid任务）主导，它们在不同观测模态、天区覆盖、点扩散函数和扫描频率上的差异使得联合分析既重要又具有挑战性。

Data: 研究中使用的数据来源于地面观测的LSST（Vera C. Rubin Observatory）和空间观测的Euclid任务，两者具有重叠天区。

Method: 采用双向生成模型AS-Bridge，基于扩散模型（diffusion model）和随机布朗桥过程（stochastic Brownian Bridge process）在LSST和Euclid观测数据间建立条件概率分布映射。

Result: AS-Bridge（Astronomical Survey-Bridge）双向生成模型通过随机布朗桥（Brownian Bridge）过程实现了地面LSST和空间Euclid观测数据的相互转换，支持跨巡天联合分析并增强科学产出。
...
### [48] [Phase Retrieval using Nonlinear Curvature Sensing within Convergent Beams](https://arxiv.org/abs/2603.11832)
*Justin R. Crepp,Caleb G. Abbott,James Smous,Matthew Engstrom,Brian Sands*

**TL;DR**: 提出了一种基于会聚光束中四平面强度测量的紧凑型路径长度多样性波前传感方法，通过物理光学传播模型验证了其优于自由空间波前传感器的实用性优势（减小尺寸、重量、复杂性和成本）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用路径长度多样性方法（path-length diversity）在自适应光学（AO）系统中通过多平面强度测量实现波前传感（wavefront sensing）的紧凑化设计优化。

Data: 研究中使用的数据来源为在会聚光束（convergent beam）中同时记录的四个平面强度测量数据，并通过仿真和实验室实验进行波前重建验证。

Method: 基于物理光学传播模型(physical optics propagation model)开发并验证了傅里叶变换强度域重建方法，用于会聚光束中的多平面相位恢复。

Result: 基于收敛光束(convergent beam)的四平面强度测量方法实现了紧凑型波前传感，相比自由空间系统显著减小了尺寸、重量、复杂度和成本。
...
### [49] [Assessing the robustness of amortized simulation-based inference to transient noise in gravitational-wave ringdowns](https://arxiv.org/abs/2603.12032)
*Song-Tao Liu,Tian-Yang Sun,Yu-Xin Wang,Yong-Xin Zhang,Shang-Jie Jin,Jing-Fei Zhang,Xin Zhang*

**TL;DR**: 提出基于摊销神经后验估计的模拟推理方法，实现了黑洞并合后铃荡（ringdown）信号参数的高效精确推断，并验证了其在瞬态噪声污染下的鲁棒性。


<details>
  <summary>Details</summary>

Background: 引力波（GW）天文学中利用双星系统发射的引力波在强场（strong field）区域对广义相对论进行精密检验，并通过铃宕（ringdown）信号高精度测量黑洞质量和自旋参数。

Data: 数据来源于模拟生成的引力波(GW)铃宕(ringdown)信号及添加瞬态噪声(transient noise)的合成数据集。

Method: 采用基于摊销神经后验估计(amortized neural posterior estimation)的模拟推断方法，通过训练覆盖先验范围内所有数据段的神经密度估计器来直接估计引力波铃荡(ringdown)参数。

Result: 提出了一种基于摊销神经后验估计的模拟推理策略，能够快速准确地估计引力波（GW）环荡（ringdown）参数，并在瞬态噪声污染下保持稳健性。
...
### [50] [Analytic Singular Slow-roll Inflation](https://arxiv.org/abs/2603.11794)
*V. K. Oikonomou*

**TL;DR**: 研究发现一类最小耦合标量场（minimally coupled scalar field）理论能解析求解哈勃参数和标量场，其暴胀现象学与ACT数据兼容，且通过共形反常（conformal anomaly）机制避免经典压力奇点，同时产生极端粒子创造实现再加热，无需暴胀子（inflaton）与标准模型粒子的多重耦合。


<details>
  <summary>Details</summary>

Background: 研究探讨了一类最小耦合标量场理论（minimally coupled scalar field theories）的解析解及其在宇宙早期非奇异暴胀（non-singular inflation）和量子效应主导的奇点避免（singularity avoidance）中的应用。

Data: 数据来源于ACT（Atacama Cosmology Telescope）观测数据以及理论模拟的标量场（scalar field）解析解。

Method: 采用Nojiri-Odintsov共形反常（conformal anomaly）机制分析宇宙演化，通过量子效应消除经典压力奇点并产生极端粒子创生以实现再加热。

Result: result：该模型通过共形异常（conformal anomaly）机制避免了经典压力奇点，同时产生极端粒子创造实现宇宙再加热，无需暴胀子（inflaton）与标准模型粒子的多重耦合，并增强了标量扰动从而促进原初黑洞形成和次级引力波产生。
...
### [51] [50-250 MHz Pulsar Census with an SKA-Low prototype station: Spectra and Polarization](https://arxiv.org/abs/2603.11553)
*Pratik Kumar,Marcin Sokolowski,Randall Wayth*

**TL;DR**: 该研究利用SKA-Low原型站EDA2在南半球50-250 MHz频段开展了最大规模低频脉冲星普查，探测到120颗脉冲星（含23颗首次在150 MHz以下和5颗在100 MHz以下发现的），提供了完整偏振脉冲轮廓和频段流量密度测量，改进了110颗脉冲星的色散量（DM）和40颗的旋转量（RM）数据，为完善脉冲星种群模型、优化巡天策略及星际介质（ISM）和电离层研究提供了关键低频观测基准。


<details>
  <summary>Details</summary>

Background: 低频脉冲星观测对研究脉冲星辐射谱、种群物理及探测星际介质（ISM）和地球电离层至关重要。

Data: 数据来源于使用SKA-Low原型站EDA2在50-250 MHz频段进行的南半球低频脉冲星普查观测。

Method: 采用南半球SKA-Low原型站EDA2在50-250 MHz频段开展低频巡天观测，通过多子带积分脉冲轮廓测量、色散量（DM）修正、法拉第旋转（Faraday rotation）分析及全偏振脉冲轮廓构建等核心方法。

Result: 在南半球50-250 MHz频段开展的迄今最大低频脉冲星普查中，使用SKA-Low原型站EDA2探测到120颗脉冲星，包括23颗首次在150 MHz以下和5颗在100 MHz以下探测到的脉冲星。
...
### [52] [Gravitational Wave Measurement of the Mbh-Mbulge Intrinsic Scatter at High Redshift](https://arxiv.org/abs/2603.11167)
...
**TL;DR**: 通过引入随红移增大的超大质量黑洞（SMBH）与星系核球质量关系（Mbh-Mbulge）内禀散射（intrinsic scatter），发现该演化模型能将纳赫兹引力波背景（GWB）振幅提升至与观测相符，并解释高红移（4 < z < 6）电磁观测中质量异常大的SMBH存在，表明Mbh-Mbulge关系存在非普适性演化（epsilon(z) = epsilon_0 + (0.56 +/- 0.4) log10(1 + z)）和归一化演化（alpha(z) = alpha_0 (1 + z)^(0.84 +/- 0.35)），暗示早期SMBH-星系演化存在多样化的种子模型和增长机制。


<details>
  <summary>Details</summary>

Background: 研究探讨了超大质量黑洞（SMBH）与星系核球质量（Mbh-Mbulge）关系的演化对纳赫兹引力波背景（GWB）谱振幅的影响，揭示了该关系在宇宙时间上的非普适性及其对早期SMBH-星系演化中种子模型和增长机制多样性的暗示。

Data: 数据来源于半解析模型（semi-analytic model）对超大质量黑洞（SMBH）与星系核球质量关系（Mbh-Mbulge）的模拟分析，并结合了低频纳赫兹引力波背景（GWB）谱的观测数据与局域电磁观测数据。

Method: 采用半解析模型（semi-analytic model）评估超大质量黑洞（SMBH）与星系核球质量关系（Mbh-Mbulge）的高散射效应对纳赫兹引力波背景（GWB）模型的影响，重点分析随红移演化的本征散射（intrinsic scatter）和归一化（normalization）演化对GWB振幅的修正作用。

Result: 超大质量黑洞（SMBH）与星系核球质量（Mbh-Mbulge）关系的内在散射（intrinsic scatter）随红移演化，且高红移时散射增强，使引力波背景（GWB）模型振幅与低频端观测谱一致。
...
### [53] [Sensitivity to Axion-like Particle dark matter with very-high-energy gamma-ray observations of Active Galactic Nuclei located behind Galaxy Clusters](https://arxiv.org/abs/2603.12009)
*Cervane Grimaud,Denys Malyshev,Emmanuel Moulin*

**TL;DR**: 通过叠加分析活动星系核(AGN)与星系团(GC)对的伽马射线光谱，将轴子类粒子(ALP)与光子耦合探测灵敏度提升至6×10⁻¹³ GeV⁻¹（ALP质量为3×10⁻⁸ eV），开辟了10⁻⁸-10⁻⁷ eV质量区间ALP暗物质参数空间探测新途径。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于轴子类粒子（ALPs）作为轻暗物质候选体的探测，通过其与光子的耦合在伽马射线谱中产生的特征吸收信号来探索暗物质参数空间。

Data: 数据来源于费米大面积望远镜(Fermi-LAT)对活动星系核(AGN)的模拟观测，以及正在运行的成像大气切伦科夫望远镜(Imaging Atmospheric Cherenkov Telescopes)如H.E.S.S.、MAGIC和VERITAS对选定AGN的模拟观测。

Method: 采用堆叠分析（stacking analysis）方法结合模拟观测数据，评估多个活动星系核（AGN）-星系团（GC）对的ALP-光子耦合灵敏度，并考虑建模与仪器不确定性影响。

Result: 通过叠加分析多个活动星系核（AGN）-星系团（GC）对的伽马射线光谱，能够探测轴子类粒子（ALPs）与光子耦合强度低至6×10⁻¹³ GeV⁻¹（ALP质量为3×10⁻⁸ eV），并探索10⁻⁸-10⁻⁷ eV质量范围内未被涉足的ALP暗物质参数空间。
...
**TL;DR**: 通过高红移（$z>1$）射电宁静活动星系核（AGN）的准同步NuSTAR和XMM-Newton观测，首次系统测量了其高能截断（$E_{\rm cut}=80.8\pm8.1$ keV）和冕区温度（$kT_{\rm e}=18.4\pm1.6$ keV），发现显著低于本地Swift-BAT AGN且光学深度更高，支持混合冕模型中高效康普顿冷却或非热电子成分的存在，与纯热电子辐射磁流体模拟预测一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移活动星系核(AGN)的X射线辐射机制，特别是通过宇宙学红移将光谱截止能移至可观测硬X射线波段，以系统约束冕区高温电子逆康普顿散射过程的高能截止特性。

Data: 数据来源于NuSTAR和XMM-Newton对13个红移大于1的射电宁静活动星系核(AGN)的准同步观测。

Method: 采用逆康普顿散射（inverse Compton scattering）模型分析活动星系核（AGN）冕区特性，结合混合冕（hybrid coronal）框架评估热与非热电子组分对观测特性的影响。

Result: 高红移活动星系核(AGN)样本的平均高能截断能量为80.8±8.1 keV，平均日冕温度为18.4±1.6 keV，显著低于本地Swift-BAT AGN的测量值。
...
**TL;DR**: 研究发现，大质量星系团（halo mass ~10^14.5 M⊙）中由弱喷流（low-powered jets）驱动的遗留瓣（remnant lobes）会在几百万年内快速内爆（implosion），导致其观测数量比贫星系团（halo mass ~10^12 M⊙）至少低估五倍。


<details>
  <summary>Details</summary>

Background: 研究探讨了致密星系团中遗留瓣状活动星系核（AGN）稀缺现象的动力学机制，聚焦于喷流活动停止后由动量通量支撑的瓣状结构快速“内爆”（implosion）过程及其对AGN反馈估计的影响。

Data: 数据来源于RAiSE动力学模型的渐近行为分析结果，并与完整模型和流体动力学模拟（hydrodynamic simulations）的预测结果进行对比。

Method: 采用RAiSE动力学模型的渐近行为分析并结合流体力学模拟验证，研究了喷流活动停止后瓣状结构（lobes）快速“内爆”（implosion）的动力学特性。

Result: 在富星系团（massive clusters）中，由弱喷流（weak jets）驱动的残留射电瓣（remnant lobes）会在最多几百万年内因动力学不稳定而发生快速内爆（implosion），导致其观测数量比贫星系群（poorer groups）至少低估五倍。
...
### [56] [The twin-jet system in the FRII radio galaxy 3C 452: A sub-parsec scale VLBI study](https://arxiv.org/abs/2603.12003)
*Eftychia Madika,Bia Boccardi,Luca Ricci,Paola Grandi,Eleonora Torresi,Gabriele Giovannini,Matthias Kadler,J. Anton Zensus*

**TL;DR**: 首次在亚秒差距尺度上解析了高激发射电星系(high-excitation radio galaxy)3C 452的双喷流结构，发现其具有对称抛物线形膨胀（喷流指数k≈0.66，反向喷流k≈0.47）、低多普勒因子(δ∼0.03-0.83)和强反转核心频谱(α>2)，并揭示窄线源喷流在≤10^5史瓦西半径(Schwarzschild radii)完成准直而宽线源在10^6-10^7史瓦西半径发生形态转变。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用多频段甚长基线干涉测量(VLBI)技术首次解析FRII型高激发射电星系3C 452的双喷流亚秒差距(sub-parsec)结构特征。

Data: 数据来源为高灵敏度阵列（High Sensitivity Array, HSA）在4.9、8.4、15.4、23.6和43.2 GHz频段的观测数据。

Method: 采用多频甚长基线干涉测量(VLBI)技术结合可见度平面和图像平面拟合方法，首次在亚秒差距尺度解析并分析了3C 452的双喷流结构。

Result: 首次在VLBI尺度上解析了3C 452的双喷流结构，确认其为罕见的FRII型源，喷流和反向喷流在数千史瓦西半径（Schwarzschild radii）尺度上呈现对称抛物线膨胀（幂律指数k≈0.66和0.47）。
...
### [57] [Cold giant discoveries from a joint radial-velocity and astrometry framework](https://arxiv.org/abs/2603.11202)
...
**TL;DR**: 结合径向速度(RV)和绝对天体测量(astrometry)方法，在金属丰度较高的FGK型恒星周围发现了五个新的长周期冷巨行星（包括一颗暖木星HIP 10090c和四颗类木星HIP 8923b、HIP 10090b、HIP 39330b、HIP 98599b），并证实天体测量数据能显著降低轨道周期和质量不确定性（3-10倍）、提升贝叶斯因子（最高60倍），实现真实质量测量。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用视向速度(RV)和天体测量(astrometry)协同方法探索长周期冷木星(cold Jupiters)的分布特征及其对行星系统形成机制的影响。

Data: 数据来源包括智利-赫特福德系外行星巡天(CHEPS)长达16年的视向速度(RV)观测数据，以及依巴谷(Hipparcos)和盖亚(Gaia)任务提供的绝对天体测量数据。

Method: 采用径向速度（RV）数据与绝对天体测量（absolute astrometry）联合拟合的贝叶斯模型比较方法，升级了EMPEROR框架并引入天体测量差分技术。

Result: 结合径向速度(RV)和绝对天体测量(astrometry)数据，升级EMPEROR框架后检测到五颗新冷巨行星（包括一颗暖木星和四颗木星类似物），并显著降低了轨道周期和质量的不确定性（3-10倍）及提升贝叶斯因子（最高60倍）。
...
### [58] [Self-consistent-field method for triaxial differentiated bodies in hydrostatic equilibrium](https://arxiv.org/abs/2603.11754)
*C. Staelen,J. -M. Huré*

**TL;DR**: 开发了新型数值代码BALEINES研究分层三轴天体（triaxial differentiated bodies）的流体静力学形状，发现Quaoar的观测形状与流体静力平衡（hydrostatic equilibrium）不符。


<details>
  <summary>Details</summary>

Background: 该研究探讨了豪摩亚（Haumea）和创神星（Quaoar）等三轴天体的流体静力学平衡形状问题。

Data: data来源为观测数据和数值模拟（simulation）数据。

Method: 开发了新的数值代码BALEINES，通过将流体质量假设为多个均匀层并重写引力势为表面积分之和，迭代求解各层边界形状而非质量密度，以研究三分层（triaxial）分化天体的流体静力学平衡形状。

Result: 开发了新型数值代码BALEINES，通过重写引力势为表面积分和迭代求解多层边界形状，证明Quaoar的观测形状与流体静力平衡状态不符。
...
### [59] [AREPAS: A Resource for Exploring Protostellar Accretion Systems - Data Release I](https://arxiv.org/abs/2603.11151)
*Marbely Micolta,Thanawuth Thanathibodee,Katya Gozman,Nuria Calvet*

**TL;DR**: 开发了可视化工具AREPAS，用于探索金牛座T型星(T Tauri Stars)磁层吸积模型开放库的首批数据，支持参数探索和用户观测数据对比。


<details>
  <summary>Details</summary>

Background: 研究背景为金牛T型星(T Tauri Stars)磁层吸积(magnetospheric accretion)模型的开放库首期数据发布及可视化探索。

Data: 数据来源于开放库中T Tauri星磁层吸积模型（magnetospheric accretion models）的首个数据发布版本，覆盖了典型原行星盘（protoplanetary disks）的光谱类型、质量吸积率和倾角范围，包含Hα、Hβ、Hγ、Paβ、Paγ、Paδ、Brγ、Ca II K、Ca II 8498 Å和Ca II 8542 Å等发射线。

Method: 开发了可视化工具AREPAS（Accretion Rate Exploration and Parameter Analysis System），用于探索金牛座T型星（T Tauri Stars）磁层吸积（magnetospheric accretion）模型开放库的首批数据，支持参数探索和用户输入观测数据的模型比对。

Result: 开发了可视化工具AREPAS，用于探索T Tauri星（T Tauri Stars）磁层吸积模型开放库的首批数据，支持参数探索和用户输入观测数据的模型对比。
...
### [60] [pyKurucz: A Pure Python Reimplementation of Kurucz SYNTHE for Stellar Spectrum Synthesis](https://arxiv.org/abs/2603.11693)
*Elliot M. Kim,Yuan-Sen Ting*

**TL;DR**: 开发了纯Python实现的pyKurucz代码，完整复现了Kurucz的SYNTHE恒星光谱合成功能，验证显示在300-1800纳米波段与原始Fortran代码的中值差异小于0.01%，并支持与机器学习工作流和大规模巡天管线的直接集成。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于恒星光谱合成计算（stellar spectrum synthesis）领域，针对传统Fortran代码（SYNTHE）在现代计算环境中的维护与扩展难题，开发了纯Python实现版本（pyKurucz）以保障该关键算法的长期可持续性。

Data: 数据来源于对100个大气模型的验证，覆盖2500 K的冷巨星至44000 K的O型星，波长范围300-1800 nm，分辨率（resolving power）R = 300,000。

Method: 采用纯Python逐行重写Kurucz的SYNTHE代码，实现连续和线状不透明度（opacity）、Voigt轮廓（Voigt profiles）、氢斯塔克展宽（hydrogen Stark broadening）、Saha-Boltzmann分布（Saha-Boltzmann populations）、189种分子平衡（molecular equilibrium）及辐射传输（radiative transfer），全程依赖NumPy、SciPy和Numba，无需Fortran。

Result: pyKurucz通过纯Python重实现了Kurucz的SYNTHE光谱合成代码，在300-1800纳米波长范围、分辨本领R=300000条件下验证了与原始Fortran代码在2500K冷巨星至44000K O型星的100个大气模型上具有亚0.01%的中值一致性。
...
### [61] [Identifying highly magnetized white dwarfs: A dimensionality reduction framework for estimating magnetic fields](https://arxiv.org/abs/2603.11945)
*Surajit Kalita,Akhil Uniyal,Tomasz Bulik,Yosuke Mizuno*

**TL;DR**: 该论文通过无监督机器学习（UMAP降维和DBSCAN聚类）系统分析氢大气层白矮星（DA WDs）样本，揭示了具有独特物理特性的磁化白矮星（MWDs）亚群，并估算了未测量磁场MWDs的场强，证明了机器学习在天体物理发现中的有效性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于白矮星（WDs）强磁场（$10^9\rm\,G$量级）的观测稀缺性与物理机制，探索高密度环境下磁场的形成与演化。

Data: 数据来源于对氢大气层白矮星(DA WDs)样本的系统性分析，包括已观测到的磁化白矮星(MWDs)和缺乏磁场测量的其他白矮星。

Method: 采用无监督机器学习（ML）技术，包括均匀流形近似与投影（UMAP）降维和基于密度的噪声应用空间聚类（DBSCAN）进行聚类识别，系统分析氢大气层（DA）白矮星样本。

Result: 应用无监督机器学习（UMAP降维和DBSCAN聚类）成功区分了氢大气层白矮星（DA WDs）中磁化与非磁化或弱磁化子群，并估算了未测量磁场强度的磁化白矮星（MWDs）的场强。
...
### [62] [Thermal Evolution of the Central Star in Pa 30](https://arxiv.org/abs/2603.11190)
*Anthony L. Piro,Yossef Zenati,Tin Long Sunny Wong*

**TL;DR**: 研究发现Pa 30（SN 1181遗迹）中心恒星（WD J005311）的热演化模型支持其由氧/氖（O/Ne）与碳/氧（C/O）白矮星合并形成，核心质量约1.15-1.4倍太阳质量（$M_\odot$），外层质量约0.02-0.04倍太阳质量，且大部分碳/氧白矮星物质在爆炸中被抛射。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于Pa 30（SN 1181的历史遗迹星云）中心恒星（WD J005311）的热演化过程及其物理特性约束，旨在揭示其从超新星爆发到当前观测状态的演化机制。

Data: 数据来源于对Pa 30中心恒星WD J005311的观测，包括其温度（约200,000 K）、风速（约16,000 km/s）和辐射光度（约1.5×10^38 erg/s）等参数。

Method: 采用半解析双组分模型（semi-analytic two-component model）近似描述中心星体为热辐射包层在相对较冷核心上收缩冷却的过程。

Result: Pa 30中心恒星（WD J005311）的热演化模型表明其核心质量（core mass）约为1.15-1.4倍太阳质量，核心半径（core radius）约为(6-8)×10^8厘米，热包层质量（hot envelope mass）约为0.02-0.04倍太阳质量。
...
### [63] [Unveiling Massive Main-Sequence Stars in Sextans A through Panchromatic Photometry](https://arxiv.org/abs/2603.11215)
...
**TL;DR**: 在低金属丰度（~6% Z_sun）矮星系Sextans A中发现了867颗大质量主序星候选体（>8 M_sun），通过BEAST建模确定了292颗OBe星候选体（下限比例15-23%）、57个OB星协及24-28%孤立大质量星，并预测该星系莱曼连续谱（LyC）逃逸分数为0.35-0.71，表明低金属环境下电离光子高效泄漏。


<details>
  <summary>Details</summary>

Background: 研究聚焦于极低金属丰度（~6% Z_sun）环境下大质量（>8 M_sun）主序星的演化与反馈机制，以矮星系Sextans A为样本探索恒星形成与电离光子逃逸（LyC escape）的关联。

Data: 数据来源于近紫外至近红外波段对Sextans A矮星系中单颗恒星的测光观测，并使用贝叶斯消光和恒星工具(BEAST)进行建模。

Method: 采用贝叶斯消光和恒星工具(BEAST)对近紫外至近红外测光数据进行建模，推断恒星有效温度、光度和初始质量等参数。

Result: 在六分仪座A（Sextans A）中发现了867颗大质量主序星候选体（当前质量>8 M_sun，表面重力>3.7 dex [cgs]），其中500颗具有较优的光谱能量分布（SED）拟合结果。
...
### [64] [The Inner and Outer Shock Layers of Bow Shocks in Cataclysmic Variables](https://arxiv.org/abs/2603.11742)
*Krystian Ilkiewicz,Christian Knigge,Simone Scaringi,Noel Castro Segura,Santiago del Palacio,Martina Veresvarska*

**TL;DR**: 研究发现激变变星（CVs）的弓激波（bow shock）实际由内层终端风激波（terminal wind shock）和外层前向激波（forward shock）组成的双层结构构成，而非传统认为的单一光学弧。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是重新审视激变变星(CV)周围弓激波(bow shock)的传统单弧模型，揭示其实际存在由终端风激波(terminal wind shock)和前向激波(forward shock)构成的双层结构。

Data: 研究中使用的数据来源于对BZ Cam、V341 Ara和1RXS J052832.5+283824三个系统的紫外、光学和红外波段存档成像观测。

Method: 采用紫外、光学和红外成像技术分析激波结构，识别出内层终端风激波（terminal wind shock）和外层前向激波（forward shock）的双层构型。

Result: 在激变变星（CVs）周围发现的传统单弧结构实为由内层终端风（reverse）激波和外层前向激波组成的双层激波构型，其中明亮内弧对应终端风激波而非此前认为的前向激波。
...
### [65] [The Cold Debris Disk Surveys I. Host Star Properties](https://arxiv.org/abs/2603.11994)
*Scott J. Kenyon,Benjamin C. Bromley,Joan R. Najita*

**TL;DR**: 这篇论文的核心贡献是整合了3675颗恒星的动力学、测光和光谱数据，建立了冷碎片盘巡天（CDDS）样本，并利用这些数据研究了恒星和行星伴星频率、恒星活动指标相关性以及恒星归属的移动星群或星团。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用斯皮策(Spitzer)和赫歇尔(Herschel)望远镜数据搜寻冷碎屑盘(cold debris disks)的尘埃发射特征，并建立名为冷碎屑盘巡天(CDDS)的多波段恒星数据库。

Data: 数据来源包括Hipparcos和Gaia的视差（parallax）数据、0.4-1250微米的光度测量（photometry）、光谱类型（spectral types）、有效温度（effective temperatures）、表面重力（gravities）、辐射光度（bolometric luminosities）、视觉消光（visual extinctions）、金属丰度（metallicities）、锂丰度（lithium abundances）、自转周期（rotational periods）、投影自转速度（projected rotational velocities）、Ca II HK和红外三重线活动指标（activity indicators）以及X射线光度（X-ray luminosities）。

Method: 采用动力学、测光和光谱数据综合分析，结合希帕科斯(Hipparcos)和盖亚(Gaia)视差、多波段测光及恒星参数（如光谱型、有效温度、重力加速度等），并利用恒星活动指标（如Ca II HK、红外三重线）和X射线光度进行相关性研究。

Result: result：在3675颗恒星样本中分析了恒星和行星伴星（包括潜在新自行伴星）的频率，将CDDS（Cold Debris Disk Surveys）恒星归类到场星或不同移动星群、疏散星团及恒星协会，并研究了恒星活动指标间的相关性。
