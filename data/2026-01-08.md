...
**TL;DR**: 通过结合ALMA高分辨率冷气体观测和JWST近红外成像，发现三个规则旋转盘星系(z>0.5)的动力学模型在外部区域吻合良好，但系统性地低估了内部旋转速度，揭示了中心质量不足可能源于恒星质量-光比低估、超大质量黑洞或非标准暗物质晕分布。


<details>
  <summary>Details</summary>

Background: 研究主题为利用高红移(z>0.5)盘星系(disk galaxies)的旋转曲线(rotation curves)探测引力势能和暗物质晕(dark matter halos)分布。

Data: 数据来源为ALMA（阿塔卡马大型毫米波/亚毫米波阵列）的高分辨率冷气体观测数据和JWST（詹姆斯·韦伯太空望远镜）的静止帧近红外成像数据。

Method: 采用高分辨率ALMA（阿塔卡马大型毫米波/亚毫米波阵列）冷气体观测与JWST（詹姆斯·韦伯太空望远镜）静止帧近红外成像相结合的方法，构建动力学模型分析星系旋转曲线并约束暗物质晕分布。

Result: result：JWST观测显示ALPAKA样本中三个星系的内旋转速度被系统性地低估，表明中心质量存在不足，可能源于恒星质量-光比或尘埃衰减导致的核球质量低估、超大质量黑洞的存在，或标准暗物质晕轮廓的偏离（如中心浓度增强）。
...
**TL;DR**: 提出了一种考虑非完全侧向倾斜的三维建模新方法，测量了红移1<z<3星系（stellar mass≳10⁹M⊙）的盘厚度（scale height z₀=0.25±0.14kpc），发现其与银河系薄盘一致但比局部星系单盘拟合结果薄1.6倍，表明薄盘在z∼3已存在且厚盘（thick disc）亮度低于薄盘10%，支持厚盘通过动力学加热逐步形成的假说。


<details>
  <summary>Details</summary>

Background: 研究恒星盘（stellar discs）垂直结构的形成与演化机制，通过测量不同宇宙时期的盘厚度来区分早期湍流相或后续动力学加热两种可能的形成场景。

Data: 数据来源于四个主要巡天项目中JWST（James Webb Space Telescope）对红移1<z<3、恒星质量≳10⁹M☉的星系成像观测。

Method: 采用三维正向建模方法（forward modelling）拟合边缘星系的三维模型，显式修正小角度偏离完美边缘取向的影响。

Result: result：在1<z<3的星系中测得的中值标高（scale height）为0.25±0.14 kpc，且hr/z0比值为8.4±3.7，表明薄盘（thin disc）在z∼3时已存在，且任何厚盘（thick disc）贡献的光度必须低于薄盘的10%，支持厚盘通过动力学加热逐步形成的演化场景。
...
**TL;DR**: 利用积分视场光谱技术分析了相互作用星系NGC 2207/IC 2163的1000个HII区，发现两星系均呈现-0.015 dex kpc$^{-1}$的氧丰度径向梯度，IC 2163旋臂间HII区光度函数斜率较缓表明相互作用改变了其星暴过程，数值模拟证实近距离交会驱动了局部化学增丰和恒星形成增强。


<details>
  <summary>Details</summary>

Background: 研究聚焦于相互作用星系系统NGC 2207/IC 2163的恒星形成活动、化学丰度分布及动力学特征，探索星系相互作用对星际介质物理条件和化学演化的影响。

Data: 数据来源为使用成像傅里叶变换光谱仪SITELLE获取的相互作用星系系统NGC 2207/IC 2163积分场光谱数据，检测并分析了约1000个HII区复合体的强光学发射线。

Method: 采用积分视场光谱(integral field spectroscopy)结合成像傅里叶变换光谱仪(SITELLE)观测数据，并运用数值模拟算法(GCD+)对星系碰撞过程进行建模分析。

Result: result：NGC 2207/IC 2163系统显示出-0.015 dex kpc$^{-1}$的负向氧丰度（oxygen abundance）梯度，并存在可能的梯度不连续性，同时IC 2163的臂间区域HII区光度函数斜率较浅，表明相互作用改变了其恒星形成过程。
...
### [4] [Time-dependent chemical evolution during cloud formation: H$_2$-regulated chemistry in diffuse molecular cloud](https://arxiv.org/abs/2601.03441)
*Yuto Komichi,Yuri Aikawa,Kazunari Iwasaki,Kenji Furuya*

**TL;DR**: 通过三维磁流体动力学(MHD)模拟和化学网络计算，揭示了星际激波后方分子云形成过程中H2与CO丰度不相关，发现稀薄区域(nH≲10³ cm⁻³)碳氢化合物和含氧分子丰度由局部H2稳态化学决定，而致密区域丰度受水冰形成影响气相C/O比，并建立了与观测一致的CH、CCH、OH柱密度与H2线性关系及HCO⁺非线性依赖关系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际激波后方分子云形成过程中的化学演化（chemical evolution）机制。

Data: 研究使用了三维磁流体力学（magnetohydrodynamics, MHD）模拟数据，包括原子气体汇聚流动的模拟结果、简单化学网络以及沿局部速度场移动的示踪粒子（tracer particles）轨迹数据。

Method: 采用三维磁流体动力学（MHD）模拟结合示踪粒子轨迹的详细化学网络计算，并基于准稳态化学假设推导分子丰度的解析解。

Result: 在分子云形成过程中，稀薄区域（n_H≲10^3 cm^-3）的碳氢化合物和含氧分子丰度由反映局部H_2丰度的稳态化学过程决定，而致密区域则受水冰形成影响导致气相碳氧元素丰度（C/O比）变化。
...
### [5] [Jet-torus interaction revealed by sub-parsec SO absorption in NGC 1052](https://arxiv.org/abs/2601.03530)
...
**TL;DR**: 首次在NGC 1052中利用宽频带毫米波甚长基线干涉测量（VLBI）探测到亚秒差距尺度上宽达700 km/s的多组分SO J_N = 3_3 - 2_2吸收谱，证实SO分子气体局限在超大质量黑洞（SMBH）附近小于0.45 pc的致密区域，支持喷流-环相互作用通过激波加热蒸发SO分子并驱动气体外流或回流的模型，且与水分子（H2O）脉泽分布和速度部分相似。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用甚长基线干涉测量（VLBI）技术探索射电星系NGC 1052中心超大质量黑洞（SMBH）附近亚秒差距尺度上SO分子气体的分布动力学及其与喷流-环面相互作用的关联机制。

Data: 数据来源于韩国甚长基线干涉网（KVN）在λ2毫米波段进行的甚长基线干涉测量（VLBI）观测，采用宽带记录模式覆盖2300 km/s的速度范围。

Method: 采用宽波段记录模式的韩国甚长基线干涉仪网络(KVN)进行λ2毫米波段的甚长基线干涉测量(VLBI)，通过分析二氧化硫(SO)分子J_N = 3_3 - 2_2跃迁的吸收谱线特征。

Result: 首次在NGC 1052中利用KVN宽频带观测检测到多组分SO J_N = 3_3 - 2_2宽吸收线（> 700 km/s），包括相对系统速度新发现的-412 km/s蓝移成分。
...
**TL;DR**: 通过对HH 80和HH 81的偏振观测发现，其未检测到线性偏振信号的原因在于反向激波加速相对论电子效率低于内部喷流节点，且法拉第退偏振效应显著。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用线性偏振辐射（linearly polarized emission）探索原恒星喷流（protostellar jets）中磁场几何结构和粒子加速机制的科学主题。

Data: 使用卡尔·G·扬斯基甚大阵（Karl G. Jansky Very Large Array）在4-6 GHz频段对HH 80和HH 81进行了偏振观测。

Method: 采用卡尔·G·扬斯基甚大阵(VLA)在4-6 GHz频段对HH 80和HH 81进行偏振观测，通过同步辐射偏振条件评估和退偏振机制分析（包括法拉第退偏振和波束退偏振）解释未探测到线偏振信号的原因。

Result: HH 80和HH 81未检测到线偏振发射是由于相对论电子加速效率降低（与内部结相比）和强退偏振效应（包括法拉第退偏振和波束退偏振）共同导致的。
...
### [7] [An Enhanced Sample of Galactic Red Supergiants Reveals Spiral Structures](https://arxiv.org/abs/2601.03642)
*Zehao Zhang,Biwei Jiang,Yi Ren*

**TL;DR**: 基于盖亚(Gaia) BP/RP光谱数据开发前馈神经网络分类器，筛选出2436颗高置信度红超巨星(RSG)，证实其空间分布与OB星相关且能有效示踪银河系旋臂结构。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用红超巨星（RSG）作为示踪天体探索银河系结构，填补了该恒星种群在银河系研究中长期样本不足的空白。

Data: 数据来源为盖亚卫星(Gaia)的BP/RP光谱（XP），覆盖宽波长范围，用于识别红超巨星(RSG)，并筛选了G星等小于12等的恒星。

Method: 采用前馈神经网络分类器（feedforward neural network classifier）对盖亚BP/RP光谱（XP）进行分析，通过十次独立运行随机划分的训练集和验证集计算每颗恒星被分类为红超巨星（RSG）的概率P(RSG)。

Result: 开发了一种前馈神经网络分类器，利用盖亚(Gaia) BP/RP光谱(XP)数据通过十次独立运行筛选出2,436颗高置信度红超巨星(RSG)候选体，其空间分布与OB星高度相关且清晰示踪银河系旋臂结构。
...
**TL;DR**: 利用机器学习技术从猎户座A分子云（Orion A）的CO同位素分子谱线数据构建尘埃温度（dust temperature）分布图，发现仅需5%像素训练即可准确预测温度，且12CO/13CO比值是预测尘埃温度的关键特征，无需气体-尘埃热耦合条件即可建立尘埃温度与光致离解区（PDR）的联系。


<details>
  <summary>Details</summary>

Background: 该研究探讨了利用机器学习技术从猎户座A分子云（Orion A）的CO同位素分子谱线数据构建尘埃温度（dust temperature）分布图的可行性及其局限性，旨在解决缺乏多波段连续谱数据区域中尘埃温度的测定问题。

Data: 数据来源为猎户座A分子云(Orion A molecular cloud)的CO同位素分子谱线(CO isotopologue molecular line)数据。

Method: 采用机器学习技术从猎户座A分子云(Orion A molecular cloud)的CO同位素分子谱线数据构建尘埃温度(dust temperature)分布图，重点分析训练像素数量、温度样本覆盖范围及$^{12}$CO/$^{13}$CO比值特征重要性。

Result: 通过机器学习技术可从CO同位素分子谱线数据构建猎户座A分子云尘埃温度图，无需依赖多波段连续谱数据。
...
**TL;DR**: 利用JWST（James Webb Space Telescope）首次在16颗原恒星中确认了固态一氧化二氮（N2O）的存在，发现其主要形成于富含CO、CO2和N2的冰幔非极性层中，丰度约为固态CO的0.2-2.1%。


<details>
  <summary>Details</summary>

Background: 探索星际介质中固态一氧化二氮（N2O）的首次探测及其在富含CO、CO2和N2的冰层中的形成机制。

Data: 数据来源为詹姆斯·韦伯太空望远镜（JWST）的近红外光谱仪（NIRSpec）观测的50颗原恒星的红外光谱，以及实验室制备的含N2O星际冰类似物的红外透射光谱。

Method: 采用实验室红外透射光谱分析N2O冰类似物混合物（N2O:CO2=1:20等）并结合JWST NIRSpec光谱拟合4.44-4.47 μm（2250-2235 cm-1）NN伸缩吸收带的方法。

Result: 首次在16颗原恒星中明确探测到固态一氧化二氮（N2O），其丰度约为固态一氧化碳（CO）的0.2-2.1%，主要形成于富含CO、CO2和N2的冰幔非极性层。
...
**TL;DR**: 首次通过高分辨率近红外光谱和哈勃深场测光确认球状星团候选体Patchick 126为银河系内原生的低质量球状星团（金属丰度[Fe/H]=-0.30±0.03、年龄11.9+0.3-0.4 Gyr），具有α元素增强（[α/Fe]=+0.19±0.02）和C-N反相关性等典型特征，轨道参数显示其遵循盘状结构运动。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于球状星团（GC）候选体Patchick~126的首次全面光谱和深度测光分析，旨在揭示其化学丰度、年龄、轨道特性及在银河系中的形成位置。

Data: 光谱分析数据来自IGRINS摄谱仪获取的高分辨率近红外数据，测光分析数据来自哈勃太空望远镜（HST）的哈勃缺失球状星团巡天（MGCS）观测。

Method: 基于高分辨率近红外光谱仪(IGRINS)获取的光谱数据和哈勃太空望远镜(HST)测光数据，采用元素丰度分析和色指数法(CMD)确定球状星团(GC)候选体Patchick~126的化学组成、年龄及轨道参数。

Result: result：首次测得球状星团候选体Patchick 126的精确年龄为11.9^{+0.3}_{-0.4} Gyr，其平均金属丰度[Fe/H] = -0.30±0.03且无内禀变化，α元素增强[α/Fe] = +0.19±0.02，轨道参数显示其遵循盘状轨道，证实其为银河系原位的低质量球状星团。
...
**TL;DR**: 研究发现恒星质量黑洞（XRBs）与超大质量黑洞类似，在毫米波段也存在由核1毫米光度、本征2-10keV X射线光度和黑洞质量构成的"毫米波段吸积基本平面"，且该平面仅适用于硬态（hard state）下的X射线双星，其存在可由平流主导吸积流（ADAF）和致密喷流模型预测。


<details>
  <summary>Details</summary>

Background: 研究揭示了黑洞吸积的毫米波基本平面（millimetre fundamental plane of black hole accretion）在超大质量黑洞和恒星质量黑洞（XRBs）中的普适性，并探讨了其与吸积流（advection-dominated accretion flows）和喷流（compact jet）模型的关联。

Data: 数据来源为毫米波（mm）和X射线（2-10 keV）观测数据，包括5个X射线双星（XRBs）的多波段数据，其中部分源具有多时期观测数据。

Method: 采用毫米波(1mm)和X射线(2-10keV)光度测量结合黑洞质量数据，验证了吸积主导流(ADAF)和致密喷流模型在恒星质量黑洞与超大质量黑洞中的普适性。

Result: 发现了毫米波基本平面（millimetre fundamental plane of black hole accretion）在X射线双星（XRBs）中同样适用，且仅适用于硬态（hard state）下的X射线双星。
...
**TL;DR**: 通过流体动力学模拟预测了本地星系群(LG)中暗星系(dark galaxies)的丰度和特性，发现其存在于低密度外围区域且气体保持弥散状态，预计FAST望远镜可探测到最多8个暗星系。


<details>
  <summary>Details</summary>

Background: 暗星系（dark galaxies）是暗物质（DM）主导的小质量晕体，其气体保持流体静力学和热平衡且从未形成恒星，是LCDM模型的重要预测对象。

Data: 使用多个先进流体动力学模拟（hydrodynamical simulations），包括不同代码、重子物理模型、反馈方案及两种恒星形成密度阈值（n_th=0.13和10 cm^-3），分析局域星系群（Local Group）约束初始条件下的暗星系（dark galaxies）样本。

Method: 采用多种先进流体动力学模拟（hydrodynamical simulations），结合不同重子物理模型、反馈机制及恒星形成密度阈值（n_th=0.13和10 cm^-3），分析局域星系群（Local Group）内暗星系（dark galaxies）的丰度与特性。

Result: 暗星系（dark galaxies）主要存在于低密度区域且位于本星系群（Local Group）外围，其暗物质晕（DM halos）的集中度较低、自转较高，周围暗物质和气体密度始终低于明亮星系。
...
**TL;DR**: 研究发现大质量旋涡星系（MaNGA样本）的氧丰度径向分布存在内外断裂半径（Rb,inner和Rb,outer），其中外断裂半径（0.8-1.45R_{25}）是星系与星系周介质（CGM）的分界，CGM氧丰度为12+log(O/H)~8.0-8.5，且星系外区丰度梯度比内区更陡，该现象可通过伴星气体捕获引发的径向气体流化学演化模型解释。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量旋涡星系（massive spiral galaxies）的氧丰度（oxygen abundance）径向分布特征及其与星系化学演化的关联，特别关注星系与伴星系相互作用对气体混合和金属丰度梯度（metallicity gradient）的影响。

Data: 数据来源于MaNGA巡天（Mapping Nearby Galaxies at APO）中选取的大质量旋涡星系样本的氧丰度分布观测数据。

Method: 采用径向气体流动的星系化学演化模型（model for the chemical evolution of galaxies with a radial gas flow）并假设与小伴星相互作用（capture of the gas from a small companion）来解释氧丰度分布特征。

Result: 氧丰度分布在外部分界半径（Rb,outer）之外趋于恒定，该半径可作为星系与星系周介质（CGM）的分界。
...
### [14] [Modeling the Effect of C/O Ratio on Complex Carbon Chemistry in Cold Molecular Clouds](https://arxiv.org/abs/2601.04103)
...
**TL;DR**: 通过NAUTILUS模型和机器学习方法研究发现，星际暗云化学中碳氧比(C/O)显著影响碳链和多环芳烃(PAHs)等富碳分子的丰度，且C3H4异构体在富氧条件下仍作为重要碳库存在，但现有模型无法重现TMC-1 CP观测到的气相碳氢比(C/H)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际化学动力学模型中碳氧比（C/O ratio）对分子丰度的调控作用及其在暗云化学中的关键影响。

Data: 研究使用了NAUTILUS代码（simulation）和机器学习技术进行分子表征分析。

Method: 采用NAUTILUS化学模型和机器学习分子表征技术分析碳氧比（C/O ratio）对暗云化学的影响。

Result: result：碳氧比（C/O）显著影响星际暗云化学模型中碳链和多环芳烃（PAHs）等富碳分子的丰度，且C3H4异构体在富氧条件下仍作为重要碳库出现，但模型无法在C/O比为0.1-3范围内复现TMC-1 CP观测到的气相碳氢比（C/H）。
...
### [15] [A census of star-formation and gas mass tracers in two lensed $z \sim 4$ dusty star-forming galaxies](https://arxiv.org/abs/2601.04133)
...
**TL;DR**: 通过ALMA观测SPT0418-47和SPT2147-50两个强引力透镜尘埃恒星形成星系（DSFGs），发现[C II]发射最延展且不总适合作为高红移星系分子气体质量和恒星形成率（SFR）示踪剂，同时提出[C I](2-1)/CO(7-6)通量密度比可作为气体耗尽时间（τdep）的可观测替代指标。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用ALMA观测高红移(z~3.7-4.2)强引力透镜尘埃恒星形成星系(DSFGs)的气体质量与恒星形成活动关系，重点分析[C II]、CO(7-6)和[C I](2-1)等多波段发射线作为示踪剂的适用性。

Data: 数据来源包括新的和存档的阿塔卡马大型毫米波/亚毫米波阵列（ALMA）观测数据，研究对象为南极望远镜（South Pole Telescope）巡天中选出的两个强引力透镜尘埃恒星形成星系（DSFGs）SPT0418-47（红移z=4.225）和SPT2147-50（红移z=3.760）。

Method: 采用像素级图像平面分析（pixel-by-pixel analysis）和透镜建模方法（lens modeling methods）研究[C II]、CO(7-6)、[C I](2-1)及p-H$_2$O发射线分布特征，并开发[C I](2-1)/CO(7-6)通量密度比作为气体耗尽时间尺度（$τ_{\rm dep}$）的可观测替代指标。

Result: 通过像素级分析发现，不同示踪剂（[C II]、CO(7-6)、[C I](2-1)等）得出的恒星形成率与气体质量面密度关系斜率基本一致。
...
### [16] [Mind the peak: improving cosmological constraints from GWTC-4.0 spectral sirens using semiparametric mass models](https://arxiv.org/abs/2601.03347)
*Matteo Tagliazucchi,Michele Moresco,Nicola Borghi,Chiara Ciapetti*

**TL;DR**: 采用基于\textsc{Bspline}的半参数化方法分析GWTC-4.0中137个双黑洞(BBH)事件，发现质量分布(MD)存在$\sim10$、$18$和$33\,\mathrm{M}_\odot$三个显著峰值，该模型将哈勃常数($H_0$)测量精度提升12%-21%，最优结果为$H_0 = 57.8^{+21.9}_{-20.6}\,\mathrm{km/s/Mpc}$。


<details>
  <summary>Details</summary>

Background: 研究探讨了利用引力波谱警报器（spectral sirens）通过双黑洞（BBH）质量分布（MD）的形状提供宇宙学约束的潜力。

Data: 数据来源于引力波天文台最新发布的GWTC-4.0目录中的137个双黑洞（BBH）事件。

Method: 采用基于\textsc{Bspline}的数据驱动半参数化方法，通过自适应地在质量分布（MD）信息最丰富的结构周围放置节点，同时保持参数空间维度适中。

Result: 基于半参数化方法在GWTC-4.0数据中识别出10、18和33倍太阳质量的三个显著质量峰，并通过贝叶斯因子（最高226）证明其统计显著性优于参数化模型。
...
### [17] [Filtering interlopers with photometry and diagnostic features: A machine learning framework validated with CSST slitless spectroscopy](https://arxiv.org/abs/2601.03883)
...
**TL;DR**: 针对欧几里得(Euclid)和中国空间站巡天望远镜(CSST)的无缝光谱(slitless spectroscopy)观测中因光谱分辨率和信噪比限制导致的发射线误认问题，开发了基于XGBoost分类器的混合筛选方法，在模拟样本中实现了96.6%的高精度红移(redshift)测量比例和0.13%的低异常值(outlier)比例，有效解决了传统严格筛选在红移纯度和样本完整性(completeness)之间的权衡难题。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升无狭缝光谱巡天（如欧几里得任务和中国空间站巡天望远镜CSST）中红移测量的准确性，解决因光谱分辨率和信噪比限制导致的发射线误识别问题。

Data: 数据来源于中国空间站巡天望远镜(CSST)的无缝光谱模拟器生成的模拟样本光谱。

Method: 采用XGBoost分类器结合测光(photometric)特性和光谱诊断(spectroscopic diagnostics)构建高纯度红移(redshift)样本。

Result: 开发了一种XGBoost分类器，通过结合测光（photometric）属性和光谱（spectroscopic）诊断指标，在模拟CSST无缝光谱（slitless spectroscopy）数据中实现了96.6%的高精度红移（redshift）测量准确率（$|Δz| \leq 0.002(1+z)$），同时将异常值比例（outlier fraction）控制在0.13%，显著优于简化配置方案。
...
### [18] [Towards an optimal extraction of cosmological parameters from galaxy cluster surveys using convolutional neural networks](https://arxiv.org/abs/2601.03894)
*Iñigo Sáez-Casares,Matteo Calabrese,Davide Bianchi,Marina S. Cagliari,Marco Chiarenza,Jean-Marc Christille,Luigi Guzzo*

**TL;DR**: 该论文通过采用星系团（galaxy clusters）作为大尺度结构示踪体，并利用3LPT代码（Pinocchio）生成32768个模拟X射线星系团目录作为训练集，结合三维卷积神经网络（3D CNN）对星系团分布导出的密度场进行场级（field-level）分析，发现相比传统统计量（星系团丰度和功率谱）方法，场级分析结合星系团丰度可将Ωm和σ8预测的平均绝对相对误差分别降低约10%和20%，而加入单个星系团光度信息后精度提升超过50%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用场级机器学习方法（field-level machine learning）通过星系巡天数据约束宇宙学参数（cosmological parameters），并探索生成大规模模拟训练集以提升预测精度的重要性。

Data: 数据来源于使用3LPT代码（Pinocchio）生成的32,768个模拟X射线星系团（X-ray cluster）目录，其中X射线光度通过经验性的M-LX标度关系随机分配给暗物质晕（dark matter haloes）。

Method: 采用三维卷积神经网络(3D CNN)分析星系团分布导出的密度场，并结合经验性质量-光度(M-L_X)标度关系随机分配X射线光度至暗物质晕。

Result: 基于星系团分布和3D卷积神经网络(CNN)的场级分析结合星系团丰度信息，对宇宙学参数Ω_m和σ_8的预测精度比传统统计量方法分别提高约10%和20%，加入单个星系团X射线光度信息后精度提升超过50%。
...
### [19] [Cosmological constraints on viable $f(R)$ models using weak lensing](https://arxiv.org/abs/2601.04048)
*Leandro Pardo,Leonardo Castañeda*

**TL;DR**: 通过弱引力透镜观测约束多种可行$f(R)$引力模型，发现标准宇宙学参数与$Λ$CDM一致，但对不同$f(R)$理论特征参数给出了非平凡限制。


<details>
  <summary>Details</summary>

Background: 宇宙加速膨胀是现代宇宙学核心未解问题之一，暗能量（dark energy）的物理本质仍不明确，促使研究广义相对论（General Relativity）的替代理论如$f(R)$引力模型。

Data: 数据来源于弱引力透镜（weak gravitational lensing）和宇宙微波背景透镜（CMB lensing）的观测数据。

Method: 采用贝叶斯框架结合改进重力扩展代码MGCobaya，通过弱引力透镜和CMB透镜数据约束多种可行f(R)引力模型对物质功率谱、收敛功率谱及宇宙剪切功率谱的影响。

Result: 通过弱引力透镜观测对多种可行$f(R)$引力模型的特征参数给出了非平凡且模型依赖的约束。
...
### [20] [Simulation-Based Inference for Probabilistic Galaxy Detection and Deblending](https://arxiv.org/abs/2601.03422)
...
**TL;DR**: 提出了一种名为贝叶斯光源分离器（BLISS）的概率方法，通过卷积神经网络和去噪自编码器处理LSST等宽场巡天中星系重叠（blending）导致的系统误差，显著提高了高度重叠和暗弱天体光通量测量的准确性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决下一代宇宙学巡天（如LSST）中因星系视觉重叠（blending）导致的系统误差问题。

Data: 数据来源于模拟的单波段图像，其特性代表LSST（Vera C. Rubin Observatory Legacy Survey of Space and Time）十年叠加观测数据。

Method: 采用贝叶斯光源分离器(BLISS)方法，结合卷积神经网络(convolutional neural networks)近似后验分布和去噪自编码器(denoising autoencoder)重建未混合星系轮廓。

Result: BLISS（Bayesian Light Source Separator）通过卷积神经网络和去噪自编码器有效降低了星系重叠（blending）导致的系统误差，显著提高了高度重叠和微弱天体通量测量的准确性。
...
### [21] [MARVEL: A Multi Agent-based Research Validator and Enabler using Large Language Models](https://arxiv.org/abs/2601.03436)
*Nikhil Mukund,Yifang Luo,Fan Zhang,Lisa Barsotti,Erik Katsavounidis*

**TL;DR**: MARVEL是一个可本地部署的开源框架，专为引力波（gravitational-wave）研究设计，通过结合检索增强生成（retrieval-augmented generation）和蒙特卡洛树搜索（Monte Carlo Tree Search）技术，在文献查询上匹配GPT-4o mini基线，并在探测器操作（detector-operations）内容上显著超越它。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发面向科学领域的本地可部署、开源问答框架（MARVEL），以应对科研团队对技术数据处理、精准引用及安全网络环境下数字助手日益增长的需求。

Data: 数据来源于精选的语义索引（包括研究文献、博士论文、LIGO文档和长期运行的探测器电子日志），并在适当情况下结合针对性网络搜索。

Method: 结合检索增强生成（retrieval-augmented generation）和蒙特卡洛树搜索（Monte Carlo Tree Search）的DeepSearch模式，通过探索互补子查询、分配计算资源至有潜力分支并维护全局证据账本（global evidence ledger）实现精确引用。

Result: MARVEL框架在引力波研究中结合检索增强生成和蒙特卡洛树搜索，在探测器操作内容上显著优于GPT-4o mini基准，其中领域检索和引导推理起决定性作用。
...
### [22] [Modelling spacecraft-emitted electrons measured by SWA-EAS experiment on board Solar Orbiter mission](https://arxiv.org/abs/2601.03818)
*Š. Štverák,D. Herčík,P. Hellinger,M. Popďakunik,G. R. Lewis,G. Nicolaou,C. J. Owen,Yu. V. Khotyaintsev,M. Maksimovic*

**TL;DR**: 通过数值模拟分析太阳轨道飞行器（Solar Orbiter）搭载的电子分析系统（SWA-EAS）测量数据，发现航天器表面发射的冷电子（cold electrons）污染显著超过航天器电位阈值，且污染源来自远处表面，同时检测器实际电位可能与航天器测量值存在偏差。


<details>
  <summary>Details</summary>

Background: 研究聚焦于太阳风等离子体中热电子（thermal electrons）测量的干扰机制，特别是航天器光电子（photo-electrons）和次级电子（secondary electrons）发射以及航天器充电效应的影响。

Data: 数据来源于太阳轨道器任务（Solar Orbiter mission）搭载的电子分析仪系统（SWA-EAS）原位测量数据，并结合空间等离子体相互作用软件（Spacecraft Plasma Interaction Software）在0.3天文单位（AU）处根据实测等离子体条件进行的数值模拟。

Method: 采用航天器等离子体相互作用软件（Spacecraft Plasma Interaction Software）模拟太阳轨道器（Solar Orbiter）与太阳风等离子体的相互作用，并通过虚拟探测器模拟电子分析仪系统（SWA-EAS）实测的电子能谱。

Result: SWA-EAS（Electron Analyser System）探测器的模拟与实测数据定性吻合，但航天器发射的冷电子（cold electrons）污染出现在高于航天器电势（spacecraft potential）阈值的区域，且该污染源自远处航天器表面的冷电子通量。
...
### [23] [Science with a large field-of-view polarization survey: The Large Array Survey Telescope Polarization Node (LAST-P)](https://arxiv.org/abs/2601.03921)
...
**TL;DR**: 提出建造大视场光学偏振仪阵列LAST-P，用于高频率监测伽马暴、超新星等暂现源偏振特性，并创建X射线双星、白矮星偏振目录及星际介质大视场研究，其88.8平方度视场和0.7%-3.5%的偏振测量精度可探测暗至20.9星等的目标。


<details>
  <summary>Details</summary>

Background: 光学偏振测量（optical polarimetry）为研究天体物理源的发射区几何结构、磁场构型及尘埃特性提供了关键信息。

Data: data：LAST-P将使用自行设计的宽视场（FoV）光学偏振仪阵列获取观测数据，覆盖88.8平方度的视场，通过15次1分钟曝光测量暗至盖亚（Gaia）Bp星等20.9的目标，并在2.7角秒视宁度、约1大气质量条件下实现17/18/19等目标的线偏振度（PD）测量精度分别达0.7%/1.5%/3.5%。

Method: 采用宽视场光学偏振测量阵列（LAST-P）进行高时间分辨率（≤1天）偏振监测，覆盖88.8平方度视场，通过15次1分钟曝光测量暗至盖亚Bp星等20.9的目标，线性偏振度（PD）测量精度在星等17、18、19时分别达0.7%、1.5%、3.5%。

Result: LAST-P（Large Array Survey Telescope Polarization Node）是一种宽视场光学偏振仪阵列，旨在对伽马射线暴早期阶段、超新星、新星等天体物理暂现源进行高时间分辨率（≤1天）偏振监测，并创建X射线双星和白矮星的大规模偏振星表，同时开展星际介质的大视场研究。
...
### [24] [Polarization rotation through differential transmission in refractive CMB telescopes identified using a hybrid physical optics method](https://arxiv.org/abs/2601.03925)
*Xiaodong Ren,Rustam Balafendiev,Jon E. Gudmundsson*

**TL;DR**: 研究发现折射式宇宙微波背景（CMB）望远镜远场光束中存在由抗反射（AR）涂层差异透射引起的偏振旋转系统误差，典型幅度为0.05-0.5度，可能导致温度-偏振泄漏和斯托克斯Q/U混合。


<details>
  <summary>Details</summary>

Background: 研究聚焦于折射式宇宙微波背景（CMB）望远镜远场光束中由抗反射（AR）涂层差分透射引起的偏振旋转系统误差。

Data: 数据来源于结合全波电磁模拟（full-wave electromagnetic simulations）的抗反射涂层（AR coatings）的混合物理光学方法（hybrid physical optics method）对非理想抗反射涂层的两透镜宇宙微波背景（CMB）望远镜的偏振响应建模。

Method: 开发了一种混合物理光学方法，结合全波电磁模拟（full-wave electromagnetic simulations）分析抗反射涂层（AR coatings）以建模折射系统的完整偏振响应。

Result: 在折射式宇宙微波背景（CMB）望远镜远场波束中发现了由光学元件抗反射（AR）涂层的差分透射引起的偏振旋转系统误差，典型幅度为0.05-0.5度。
...
### [25] [Can Dynamic Spectrum Sharing Protect Passive Radio Sciences?](https://arxiv.org/abs/2601.03966)
*Gregory Hellbourg*

**TL;DR**: 论文提出适时静默区（JITQZ）机制保护被动无线电科学（passive radio sciences）观测，通过混合框架在核心频段保持静态保护的同时允许相邻频段受限动态接入，证明动态频谱共享（DSS）需作为高可靠性安全关键系统实施，静态分配仍不可或缺且动态接入需保守保障措施和可执行追责机制。


<details>
  <summary>Details</summary>

Background: 研究探讨了动态频谱共享（DSS）在现代频谱政策中的关键作用及其对被动无线电科学（如射电天文、地球遥感和气象学）的影响，重点关注高可靠性保护机制的必要性。

Data: 研究中使用的数据来源包括被动无线电科学（passive radio sciences）领域的观测数据（如射电天文、地球遥感和气象学）以及动态频谱共享（DSS）相关的仿真数据（simulation data）。

Method: 采用博弈论框架(game theoretic framework)分析动态频谱共享(DSS)中的合作失败原因、持续合作条件及激励相容机制，包括基于伪度量(pseudonymetry-enabled attribution)的合规促进手段。

Result: DSS（动态频谱共享）仅能作为高可靠性、安全关键系统支持被动无线电科学（passive radio sciences），静态分配仍不可或缺，而动态接入需保守保障措施和可执行问责机制才可行。
...
### [26] [Broadband spectroscopy of astrophysical ice analogues: IV. Optical constants of N$_2$ ice in the terahertz and mid-infrared ranges](https://arxiv.org/abs/2601.03951)
...
**TL;DR**: 首次直接测量了分子氮(N2)冰在0.3-16太赫兹(THz)波段的复折射率(complex refractive index)，发现1.47和2.13 THz处的共振吸收峰源于α-N2晶体的光学活性声子(phonons)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原行星盘（protoplanetary disk）中氮分子（N$_2$）冰的太赫兹（THz）—红外（IR）光学性质测量，以解决其在尘埃连续谱辐射和辐射传输（radiative transfer）模型中的不确定性。

Data: 使用太赫兹脉冲光谱（TPS）和傅里叶变换红外光谱（FTIR）直接测量了低温气相沉积在硅窗上的N$_2$冰的光学性质，并通过密度泛函理论（DFT）计算和文献数据对比进行验证。

Method: 采用太赫兹脉冲光谱（TPS）和傅里叶变换红外光谱（FTIR）测量并结合密度泛函理论（DFT）计算，通过洛伦兹介电模型参数化氮冰（N$_2$ ice）的光学响应。

Result: 首次在0.3-16太赫兹（1毫米-18.75微米）波段完整测定了氮冰（N$_2$ ice）的复折射率，并识别出α相氮晶体在1.47和2.13太赫兹处由光学活性声子（optically active phonons）引起的共振吸收峰。
...
### [27] [Disc fragmentation. II. Ejection of low mass Free Floating Planets from growing binary systems](https://arxiv.org/abs/2601.03820)
*Sergei Nayakshin,Luyao Zhang,Aleksandra Ćalović,Hans Lee,Clement Baruteau,Farzana Meru,Lucio Mayer*

**TL;DR**: 研究发现引力不稳定性（GI）导致的盘碎裂不仅形成恒星的大质量伴星（如M矮星的巨行星、FGK恒星的褐矮星（BD）伴星和间距小于100 au的双星），还能通过双星系统动力学相互作用高效抛射大量小质量自由漂浮行星（FFPs），为微透镜观测发现的亚木星质量行星提供了形成机制。


<details>
  <summary>Details</summary>

Background: 研究探讨了引力不稳定性（GI）导致的盘碎裂在恒星伴星（如M型矮星周围的巨行星、FGK型恒星的褐矮星（BD）伴星及间距小于100 au的双星系统）形成中的作用，并扩展至微引力透镜观测发现的更丰富的小质量自由漂浮行星（FFPs）群体。

Data: 研究使用了三种互补的模拟方法（three complementary simulation approaches）的数据来源。

Method: 采用引力不稳定性（GI）模型和三种互补的模拟方法分析行星形成与抛射过程。

Result: 引力不稳定性（GI）导致的盘碎裂可以形成质量远小于木星的自由漂浮行星（FFPs），微透镜观测发现的这类行星可能是盘碎裂形成低质量行星的最有力证据。
...
### [28] [A constrained-transport embedded boundary method for compressible resistive magnetohydrodynamics](https://arxiv.org/abs/2601.04099)
*Samuel W. Jones,Colin P. McNally,Meritt Reynolds*

**TL;DR**: 提出了一种在笛卡尔网格上求解可压缩电阻磁流体动力学(MHD)方程时实现任意形状嵌入边界的方法，通过有限体积法和面中心约束输运格式避免了切割单元的小时间步问题，并扩展至处理移动界面问题，验证了该方法在无间断时二阶收敛、有材料属性间断时一阶收敛的特性。


<details>
  <summary>Details</summary>

Background: 研究背景为脉冲功率磁惯性约束聚变(pulsed-power magneto-inertial fusion)装置中可压缩电阻磁流体动力学(compressible resistive magnetohydrodynamics)方程的数值求解方法开发。

Data: 数据来源于数值模拟（simulation），采用笛卡尔网格上的有限体积法求解可压缩电阻磁流体动力学方程，并利用黎曼求解器计算网格面通量。

Method: 采用有限体积法求解可压缩电阻磁流体动力学方程，结合黎曼求解器计算网格面通量和面心约束输运（constrained transport）的感应方程，并通过鬼流体（ghost-fluid）方法扩展至移动界面建模。

Result: 提出了一种在笛卡尔网格上求解可压缩电阻磁流体动力学方程时实现任意形状嵌入边界的方法，该方法基于有限体积公式和面中心约束输运公式，避免了切割单元导致的小时间步问题。
...
### [29] [The steep redshift evolution of the hierarchical merger rate may cause the $z$-$χ_{\rm eff}$ correlation](https://arxiv.org/abs/2601.03456)
*Amanda M. Farah,Aditya Vijaykumar,Maya Fishbach*

**TL;DR**: 通过引力波数据识别出致密星团中等级合并( hierarchical-merger )形成的黑洞亚群，其主质量分布峰值(16.7太阳质量)约为次质量分布模态(8.0太阳质量)的两倍，且自旋倾角分布各向同性，同时发现等级合并率随红移(redshift)上升而显著增加(红移1时为0.17，红移0.1时为0.05)，解释了有效自旋分布随红移增宽的现象。


<details>
  <summary>Details</summary>

Background: 引力波观测证据表明部分合并黑洞源自先前合并事件，研究聚焦于星团（star cluster）中等级合并（hierarchical merger）黑洞的动力学形成机制及其宇宙学演化特征。

Data: 数据来源于引力波观测（gravitational-wave observations）和致密星团（dense star clusters）中的动力学模拟（dynamical simulations）。

Method: 基于引力波数据识别出符合致密星团中等级合并（hierarchical-merger）起源的子群，并利用其无纲量自旋（dimensionless spin）幅度χ≈0.69的预测特征进行分析。

Result: 在致密星团中，通过引力波数据识别出一个符合等级合并起源（hierarchical-merger origin）的子群，其主质量分布峰值位于$16.7^{+3.1}_{-4.6},\mathrm{M}_{\odot}$，约为次质量分布模态值（$8.0^{+29.7}_{-2.3},\mathrm{M}_{\odot}$）的两倍，且自旋倾角分布与各向同性一致。
...
### [30] [The maximum mass ratio of hierarchical mergers may cause the $q$-$χ_{\rm eff}$ correlation](https://arxiv.org/abs/2601.03457)
*Aditya Vijaykumar,Amanda M. Farah,Maya Fishbach*

**TL;DR**: 研究发现，在LIGO-Virgo-KAGRA探测器观测的双黑洞（BBH）并合事件中，存在一个次群体其主黑洞自旋与等级并合（hierarchical mergers）一致，且质量比低于0.59，其中19%-88%的BBH可能属于等级并合群体，这解释了其他研究中观察到的有效旋进自旋分布随质量比减小的现象。


<details>
  <summary>Details</summary>

Background: 研究聚焦于致密恒星环境中黑洞（BHs）的等级合并（hierarchical mergers）及其对双黑洞（BBH）自旋和质量比分布的影响。

Data: 数据来源于LIGO-Virgo-KAGRA探测器最新发布的双黑洞(BBH)并合事件目录。

Method: 采用贝叶斯分析(Bayesian analysis)方法对LIGO-Virgo-KAGRA探测器观测的双黑洞(BBH)并合事件进行自旋参数统计推断，并结合等级并合(hierarchical merger)理论模型识别次级自旋特征。

Result: 在质量比低于0.59^{+0.18}_{-0.23}的BBH（双黑洞）子群中，19%-88%的成员可能属于层级合并（hierarchical merger）产生的黑洞群体。
...
### [31] [Sterile Neutrino Dark Matter as a Probe of Inflationary Reheating](https://arxiv.org/abs/2601.03346)
*James M. Cline,Yong Xu*

**TL;DR**: 研究发现惰性中微子（sterile neutrino）可通过暴胀子（inflaton）衰变在重加热（reheating）过程中以极小分支比（${\rm BR}\lesssim 10^{-4}$）高效产生，其参数空间可同时满足现有X射线观测限制并解释暗物质（DM）丰度，且能作为探测暴胀重加热过程的探针。


<details>
  <summary>Details</summary>

Background: 该研究探讨了惰性中微子（sterile neutrino）作为暗物质（DM）候选体的可能性及其在宇宙再加热（reheating）过程中的产生机制。

Data: 数据来源于对惰性中微子（sterile neutrino）暗物质（DM）辐射衰变的X射线观测（X-ray observations）以及重加热（reheating）过程中由暴胀子（inflaton）衰变产生惰性中微子的模拟（simulation）分析。

Method: 采用辐射衰变（radiative decay）和重加热（reheating）过程中微分支比（branching ratio）的物理模型，系统映射了惰性中微子（sterile neutrino）质量、混合角（mixing angle）、暴胀子（inflaton）质量、重加热温度（reheating temperature）等参数的可行空间。

Result: 惰性中微子（sterile neutrino）可以通过暴胀子衰变以极小分支比（${\rm BR}\lesssim 10^{-4}$）在再加热过程中被高效产生，其参数空间可同时满足现有X射线观测限制并重现暗物质丰度。
...
### [32] [Cavity Multimodes as an Array for High-Frequency Gravitational Waves](https://arxiv.org/abs/2601.03341)
*Diego Blas,Yifan Chen,Yuxin Liu,Yanfei Shang,Jing Shu*

**TL;DR**: 首次证明单个微波腔中多个电磁模式的独特天线模式可定位高频引力波（HFGWs）并重建其偏振比和频率漂移率等关键特性。


<details>
  <summary>Details</summary>

Background: 该研究探讨了在背景磁场中使用微波腔（microwave cavity）探测高频引力波（HFGWs）的科学潜力。

Data: 数据来源于粒子加速器中常用的9-cell腔体（9-cell cavity）内18个近简并模式（nearly degenerate modes）的时域响应（time-domain response）。

Method: 利用单腔体中多个电磁模式的独特天线模式（antenna pattern）分析高频引力波（HFGWs）的偏振比和频率漂移率。

Result: 单个微波腔中多个电磁模式的独特天线模式能够定位并重建高频引力波（HFGWs）信号的偏振比和频率漂移率等关键特性。
...
### [33] [Real-time Gravitational Wave Response in Thermal Spinning fields](https://arxiv.org/abs/2601.03631)
*Atsuhisa Ota,Hui-Yu Zhu,Yuhang Zhu*

**TL;DR**: 研究发现引力波在辐射主导宇宙中的传播动力学响应（dynamical response）虽具有自旋（spin）依赖性，但其短期效应被局域响应抵消，最终长期响应呈现普适性且与硬热极限（hard thermal limit）下的动力学理论一致，自旋机制未在大尺度引力波有效动力学中留下可观测痕迹。


<details>
  <summary>Details</summary>

Background: 研究探讨了热等离子体自旋含量（spin content）对辐射主导宇宙中引力波传播的影响。

Data: data：研究采用量子场论框架下的实时模拟数据，背景辐射由共形标量场（conformal scalar）、外尔费米子（Weyl fermion）和麦克斯韦场（Maxwell fields）构成，应力张量分解为背景部分、动态响应和局域接触项，并通过弗里德曼方程进行壳上投影固定。

Method: 采用实时量子场论框架分析引力波传播，结合弗里德曼方程（Friedmann equation）确定的壳上投影，将应力张量分解为背景部分、动态（历史依赖）响应和局域接触项。

Result: 引力波（gravitational waves）在大尺度有效动力学中的长期响应具有普适性，与自旋无关。
...
### [34] [A Multiband Photometric Study of RR Lyrae Stars in M53 (NGC 5024)](https://arxiv.org/abs/2601.03758)
*Shantanu A. Gaur,Nitesh Kumar,Anupam Bhardwaj,Aasheesh Raturi*

**TL;DR**: 通过对球状星团Messier 53中RR Lyrae变星(RRL)的多波段(UBVRI)时序测光研究，精确测定了其脉动特性(基模RRab平均周期0.649天、一阶谐波RRc 0.346天)和距离模数(16.242±0.05星等)，确认该星团属于Oosterhoff II型并具有高RRc比例(54.7%)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过多波段（UBVRI）时域测光分析球状星团Messier 53（NGC 5024）中的天琴RR型变星（RRL），以精确定位其脉动特性并测定星团距离。

Data: 数据来源于UBVRI多波段测光时间序列观测，使用了跨度22年3个月的档案测光数据（archival photometric data），这些数据由不同光学望远镜获取。

Method: 采用Lomb-Scargle周期图（Lomb-Scargle periodogram）分析多波段（UBVRI）时序测光数据，并通过模板拟合相位折叠光变曲线获取平均星等和振幅。

Result: 通过多波段（UBVRI）测光分析确认M53为Oosterhoff II型球状星团，其RRab和RRc型天琴座RR变星的平均周期分别为0.649天和0.346天，且RRc占比达54.7%。
...
### [35] [Dissecting the dust distribution and polarization around two B213 young stellar objects with ALMA](https://arxiv.org/abs/2601.04091)
*Asako Sato,Anaëlle Maury,Josep M. Girart,Andrea Bracco,Patrick Hennebelle,Qizhou Zhang,Valeska Valdivia*

**TL;DR**: 利用ALMA多尺度偏振观测揭示了金牛座两个年轻恒星天体（K04166和K04169）在20-50 au过渡区从包层磁排列尘埃到盘内自散射的转变，表明K04166是保留大量包层的年轻嵌入天体，而K04169是更演化的T-Tauri星，两者盘内均已存在大颗粒，证实了0级阶段盘内尘埃早期演化。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原恒星阶段（protostellar phase）盘形成和尘埃演化的早期过程，特别是通过毫米波偏振观测揭示包层-盘（envelope-disk）界面处的物理机制和物质分布特征。

Data: 数据来源于ALMA在1.4毫米和3毫米波段对金牛座两个年轻恒星天体（IRAS 04166+2706和IRAS 04169+2702）的极化观测，探测尺度从25天文单位到3000天文单位。

Method: 采用ALMA（阿塔卡马大型毫米波/亚毫米波阵列）1.4毫米和3毫米双波段偏振观测数据，结合斯托克斯I辐射建模分离盘与包层成分，并通过偏振特性分析识别主导极化机制。

Result: K04166的内包层和盘（<100 au）区域的偏振发射特性变化表明存在环形磁场（toroidal magnetic field）或大颗粒，而K04169的偏振发射符合盘内自散射（self-scattering）机制。
...
### [36] [Characterizing the physical and chemical properties of the Class I protostellar system Oph-IRS 44. Binarity, infalling streamers, and accretion shocks](https://arxiv.org/abs/2601.04108)
*E. Artur de la Villarmois,V. V. Guzmán,M. L. van Gelder,E. F. van Dishoeck,E. A. Bergin,D. Harsono,N. Sakai,J. K. Jørgensen*

**TL;DR**: 通过高分辨率ALMA观测发现IRS 44双星系统中B源周围存在下落物质流与盘-包层界面激波（accretion shocks），导致SO和SO2显著增丰并形成富氧化学环境。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低质量恒星形成过程中盘-包层界面（disk-envelope interface）的激波（shocks）现象及其对含硫分子（S-bearing species）生成路径的影响。

Data: 研究中使用的数据来源于ALMA（Atacama Large Millimeter/submillimeter Array）对IRS 44的高角分辨率（0.1角秒）观测，包括CO、H2CO及简单含硫分子的多条分子谱线以及连续谱发射。

Method: 采用高角分辨率（0.1"）的ALMA观测技术，针对IRS 44的CO、H2CO及简单含硫分子（sulfur-bearing species）的多条谱线进行探测，并结合连续谱分析揭示其双星性质。

Result: 在IRS 44 B周围观测到SO和SO2的高柱密度，表明该区域存在由吸积激波（accretion shocks）导致的尘埃加热和硫化物释放过程。
...
### [37] [Constraining Lorentz and parity violations in gravity with multiband gravitational wave observations](https://arxiv.org/abs/2601.03571)
*Zhi-Xin Jia,Tao Zhu,Zhoujian Cao*

**TL;DR**: 通过联合第三代地基（CE、ET）和天基（LISA、太极、天琴）引力波探测器网络的多波段观测，显著提升对引力领域洛伦兹对称性（Lorentz symmetry）和宇称对称性（parity symmetry）破坏参数（β）的约束精度达数个量级，其中高信噪比事件（如GW250114）对高频修正（β>0）更敏感，而大质量双黑洞系统（如GW231123）对低频修正（β<0）约束更强。


<details>
  <summary>Details</summary>

Background: 研究主题为利用多波段引力波观测（ground-based和space-based探测器网络联合）检验引力领域中的宇称（parity）和洛伦兹对称性（Lorentz symmetry）破坏效应。

Data: 数据来源包括第三代地基引力波探测器（CE、ET）和天基探测器网络（LISA、Taiji、TianQin）联合观测的双黑洞并合引力波多波段数据，以及GW250114和GW231123等高信噪比（SNR）事件。

Method: 采用参数化波形框架（parameterized waveform framework）建模引力波信号，通过引入幂律指数β量化广义相对论偏差，分析高低频修正效应。

Result: 多波段引力波观测可将洛伦兹对称性（Lorentz symmetry）和宇称对称性（parity symmetry）破缺的现有约束提高数个量级，对高频（β>0）和低频（β<0）修正均有效。
...
### [38] [Symmetries of cosmological perturbations: The residual low multipole ambiguity](https://arxiv.org/abs/2601.04144)
*Jibril Ben Achour,Etera Livine,Vincent Vennin*

**TL;DR**: 该研究将宇宙学中的长波模式（long-wavelength modes）与局部无限维对称性联系起来，揭示了其通过任意时间函数生成的低多极软模式（low-multipole soft modes）会改变宇宙补丁的平均曲率能量和角动量，并证明这些由大规范变换（LGT）产生的绝热扰动会保留背景几何的所有显式和隐藏Killing对称性，从而为解析研究其对宇宙学观测值的影响提供了新途径。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙学中长波模（long-wavelength modes）与大规范变换（LGT）的关联及其作为宇宙微扰理论对称性的物理意义与应用。

Data: 数据来源于理论推导（simulation），未涉及实际观测数据。

Method: 采用大规范变换（LGT）分析长波长模对宇宙学扰动理论的影响，扩展了全局对称性为局部无限维对称性，并研究了其对背景几何显式和隐藏Killing对称性的变形保持特性。

Result: 首次将文献已知的全局对称性扩展为依赖任意时间函数的局部无限维对称性，其生成元产生改变平均曲率能量和角动量的低多极软模式（low-multipole soft modes），需在计算宇宙学可观测量前固定这种新的坐标系模糊性。
...
### [39] [A framework for LISA population inference](https://arxiv.org/abs/2601.04168)
*Alexandre Toubiana,Jonathan Gair*

**TL;DR**: 提出了一种分层贝叶斯框架（hierarchical Bayesian framework），用于从LISA（Laser Interferometer Space Antenna）全局拟合（Global Fit）输出中直接推断天体物理种群属性，统一处理可分辨源与未分辨背景的信息。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用LISA（激光干涉空间天线）的多源数据流，通过全局拟合（Global Fit）解码引力波源天体物理族群的统计特性，填补当前地基探测器与脉冲星计时阵列之间的观测空白。

Data: 数据来源于LISA（Laser Interferometer Space Antenna）任务预期获取的包含大量不同类型引力波源（包括可分辨信号和重叠随机背景）的观测数据流。

Method: 采用分层贝叶斯框架（hierarchical Bayesian framework）从LISA全局拟合（Global Fit）输出中直接推断天体物理族群的属性，同时考虑已分辨源和未分辨背景的信息。

Result: LISA（Laser Interferometer Space Antenna）将采用分层贝叶斯框架从全局拟合（Global Fit）输出中直接推断天体物理群体的属性，统一处理可分辨源和未分辨背景的信息。
