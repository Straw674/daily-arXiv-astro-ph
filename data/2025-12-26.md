...
**TL;DR**: 基于453个硬X射线选活动星系核(AGN)的近红外(NIR)光谱分析，发现SiVI λ1.964等近红外日冕线比光学[OIII]能更可靠示踪AGN光度，并提出了有效区分AGN、恒星形成和复合源的新NIR诊断图，揭示了热分子气体质量与X射线光度存在中度相关性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用近红外(NIR)光谱分析硬X射线选活动星系核(AGN)的发射线特性、AGN诊断及核周气体动力学，揭示AGN活动与周围环境的相互作用机制。

Data: 数据来源于BAT AGN Spectroscopic Survey (BASS) NIR Data Release 3 (DR3)中453个硬X射线(14-195 keV)选的活动星系核(AGN)样本，并包含完整DR2样本，观测使用VLT X-shooter多波段(0.3-2.5μm)光谱仪(R=4,000-18,000)以≥2σ检测阈值获取。

Method: 采用近红外（NIR）光谱分析技术（VLT X-shooter多波段光谱仪，分辨率R=4,000–18,000）和σ≥2的检测阈值，重点分析发射线特征（如SiVI λ1.964、Paα、Paβ、FeII λ1.257μm/Paβ、H₂ λ2.122μm/Brγ）并构建改进的NIR诊断图。

Result: 近红外（NIR）禁线（coronal lines）尤其是SiVI λ1.964比光学[OIII]更可靠地示踪活动星系核（AGN）光度，与硬X射线光度相关性更紧密（σ=0.25dex）。
...
**TL;DR**: 基于JWST SMILES巡天数据构建了0.5 < z < 6的中红外光度函数（LFs），首次精确约束了中红外LF的暗端斜率演化，量化了z~5之前的尘埃恒星形成和黑洞吸积历史。
...
Data: 数据来源于JWST系统性中红外仪器遗产河外巡天（SMILES）项目的8个MIRI波段（5-25μm）和存档的23个HST+JWST NIRCam波段测光数据，构建了包含2813个中红外亚微焦耳水平完整星系的广泛目录。

Method: 采用JWST中红外仪器（MIRI）8个波段（5-25 μm）和HST+JWST NIRCam 23个波段的测光数据构建星系光度函数（LFs），并通过单色光度（5.6、7.7、10、12.8、15、18、21和25.5 μm）、红外光度（L_IR）和活动星系核（AGN）光度函数量化尘埃宇宙恒星形成和黑洞吸积历史。

Result: 基于JWST SMILES项目的中红外（mid-IR）观测数据，构建了红移范围0.5至6的星系光度函数（LFs），包括单色（5.6-25.5 μm）、总红外（$L_{\rm IR}$）和活动星系核（AGN）的光度函数，其极限光度在$z=0.5-1.0$为$10^{9.5}$ $L_\odot$，$z=2.0-4.0$为$\sim10^{10.5}$ $L_\odot$，$z=4.0-6.0$为$\sim10^{11}$ $L_\odot$。
...
**TL;DR**: 基于SDSS-IV MaNGA巡天数据，研究发现棒旋星系（barred galaxies）具有更中心集中的恒星质量分布（1-2有效半径内）和更低恒星角动量λRe，且在固定总恒星质量下其内部区域恒星质量更高、角动量更低；棒结构比例随邻近星系潮汐作用强度呈双峰分布（孤立环境下最高，中等相互作用时降低，强相互作用时回升），证实内部引力不稳定性是棒结构形成主因，外部潮汐扰动对已具备内部条件的星系起次要触发或增强作用。


<details>
  <summary>Details</summary>

Background: 研究探讨了盘星系（disc galaxies）的物理属性与棒状结构（bars）存在之间的关系，聚焦于内部动力学特性与环境因素的相互作用对棒形成机制的影响。

Data: 数据来源于SDSS-IV MaNGA巡天（Mapping Nearby Galaxies at Apache Point Observatory）的观测数据。

Method: 结合内部运动学特性（kinematical properties）和环境诊断指标（environmental diagnostics）分析棒旋星系（barred galaxies）的恒星质量分布集中度和角动量（angular momentum）特征。

Result: 棒旋星系（barred galaxies）更频繁地与中心集中的恒星质量分布（1和2个有效半径内）和较低的恒星角动量$λ_{Re}$相关，且在固定总恒星质量下，其内部区域具有更高恒星质量和更低角动量；棒结构比例（bar fraction）对邻近星系潮汐作用呈现双峰依赖，孤立星系中最高（内部长期过程主导），中等相互作用时降低，强相互作用时再次上升（密集环境维持或触发棒结构）；内部引力不稳定性是棒形成的主要驱动力，外部潮汐扰动起次要作用，可触发或增强已具备内部条件的星系中的棒结构。
...
**TL;DR**: 提出了一种基于吸积盘（accretion disc）的子网格模型，结合辐射流体动力学模拟结果，揭示了角动量（angular momentum）错位通过快速移除盘角动量维持超高爱丁顿比率（super-Eddington ratio）吸积的关键机制，为早期宇宙中黑洞（BHs）快速增生长至十亿倍太阳质量提供了可能解释。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超大质量黑洞（BHs）在宇宙早期（高红移星系）通过超爱丁顿吸积（super-Eddington accretion）快速增长的机制及其环境条件。

Data: 数据来源于在流体动力学代码GIZMO中实现的理想化模拟，包括嵌入气体环核盘和球状分布恒星成分的黑洞（BH）系统，以及基于辐射流体动力学模拟结果的子网格吸积盘模型（包含内光子捕获区和外薄α盘）。

Method: 采用基于辐射流体力学模拟的亚网格吸积盘模型（sub-grid accretion disc），包含内光子捕获区和外薄α盘（α-disc），并结合自洽的角动量演化机制（Bardeen-Petterson效应与内厚盘进动之间的转换）。

Result: 超爱丁顿吸积（super-Eddington accretion）可通过气体流入与吸积盘角动量错位触发，从而在高度动态的高红移星系环境中有效促进黑洞（BH）种子质量增长。
...
**TL;DR**: 结合DESI DR2重子声波振荡（BAO）、Planck+ACT宇宙微波背景（CMB）及超新星数据，发现解冻精质（thawing quintessence）比宇宙学常数（cosmological constant）更符合观测，且偏好性不依赖先验选择但需超新星数据支持，同时确定偏离信息准则（DIC）比AIC或BIC更可靠地追踪贝叶斯证据。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙学常数与解冻精质（thawing quintessence）模型的贝叶斯比较，重点分析了解冻动力学参数（Padé-w）的理论先验对暗能量（dark energy）演化机制的影响。

Data: 数据来源包括DESI DR2的重子声波振荡（BAO）数据、Planck+ACT的宇宙微波背景（CMB）数据以及多个主要超新星样本（如更新的DES-Dovekie样本）。

Method: 采用贝叶斯(Bayesian)比较方法结合理论驱动的帕德(Padé)参数先验，对解冻型精质场(thawing quintessence)与宇宙学常数(cosmological constant)进行模型对比分析。

Result: 结合DESI DR2的重子声波振荡(BAO)、Planck+ACT的宇宙微波背景(CMB)及超新星数据时，解冻五元组(quintessence)模型比宇宙学常数(cosmological constant)更受青睐。
...
**TL;DR**: 通过结合宽波段光学至中红外数据和基于Prospector的非参数化恒星形成历史分析，发现尽管宿主星系质量被低估，Ia型超新星(SN Ia)光度与宿主质量的相关性（mass step）依然显著，且低金属丰度环境(Z < -1 Zsol)可能是驱动该相关性主要因素，表明SN Ia标准化亮度受环境化学组成关键影响。


<details>
  <summary>Details</summary>

Background: 研究主题为探究Ia型超新星（SNe Ia）标准化光度与宿主星系质量（mass step）及环境金属丰度的关联机制。

Data: 数据来源为宽带光学（3000埃至1微米）和近红外（最高1.8微米）的完整观测数据，部分样本还包括中红外（最高4.5微米）观测。

Method: 采用宽带光学（3000埃至1微米）和近红外（至1.8微米）数据，部分包含中红外（至4.5微米）观测，结合Prospector工具推导非参数化恒星形成历史（SFH）。

Result: 低金属丰度（Z < -1 Zsol）环境可能主导了观测到的Ia型超新星（SN Ia）光度与宿主星系质量的相关性，表明SN环境的化学组成对标准化亮度起关键作用。
...
**TL;DR**: 基于随机森林模型分类VLASS巡天快视数据中的双射电源(DRAGNs)，有效剔除成像伪影(artifacts)污染，构建了完整度99.3%、纯净度97.7%的改进版星表。


<details>
  <summary>Details</summary>

Background: 研究聚焦于改进甚大天线阵巡天项目(VLASS)快速查看数据中双射电源(DRAGNs)的识别精度，通过机器学习方法消除成像伪影(artifact)对活动星系核(AGN)相关射电源分类的干扰。

Data: 数据来源于甚大天线阵巡天（VLASS）第一纪元快速查看（Quick Look）数据产品中的双射电源（DRAGNs）目录，并使用DRAGNhunter算法识别。

Method: 采用随机森林模型（random forest models）对VLASS快速巡天数据中的双射电源（DRAGNs）进行分类，基于其包含的成像伪影（artifacts）数量（0至3个），并通过优化训练集选择缓解类别不平衡问题。

Result: 基于随机森林模型分类的VLASS双射电源（DRAGNs）目录中可提取出97.7%无伪影（artifact-free）且99.3%完整的源表。
...
**TL;DR**: 该论文提出天文发展办公室(OAD)开发的"影响循环"(Impact Cycle)方法，通过科学评估和证据驱动的项目设计优化天文促进发展活动的有效性，建立最佳实践库以实现持续改进。


<details>
  <summary>Details</summary>

Background: 探讨天文学促进发展（astronomy-for-development）活动的科学评估方法以优化项目设计与实施效果。

Data: 数据来源于天文促进发展办公室（OAD）通过评估现有项目收集的经验证据库（evidence library），用于确定最佳实践和无效方案。

Method: 采用基于证据（evidence-informed）的影响循环（Impact Cycle）方法优化项目设计、选择和实施系统。

Result: 采用科学方法评估天文促进发展（astronomy-for-development）活动效果是提升项目设计、选择和实施系统效能的关键。
...
**TL;DR**: 在Schwinger-Keldysh框架下构建了暴胀有效场论（effective field theory of inflation）的引力开放扩展，识别出与标量（scalar）和张量（tensor）模式传播兼容的最小开放算符（open operators），重现了暴胀的耗散模型（dissipative models）。


<details>
  <summary>Details</summary>

Background: 研究背景是构建引力开放扩展的有效场论（effective field theory of inflation）以探索暴胀（inflation）中的耗散模型（dissipative models）。

Data: 数据来源于理论构建的Schwinger-Keldysh框架下的引力开放扩展有效场论（effective field theory）。

Method: 采用施温格-凯尔迪什（Schwinger-Keldysh）框架构建了暴胀有效场论的引力开放扩展，识别了与传播标量及张量模式兼容的最小开放算符并构建了引力作用量。

Result: 在Schwinger-Keldysh框架下构建了包含最小开放算符的引力膨胀有效场论，恢复了具有传播标量模和引力波模的耗散膨胀模型。
...
### [10] [GW241011 and GW241110: Hints of Hierarchical Mergers from the Merger Entropy Index](https://arxiv.org/abs/2512.20965)
*Guo-Peng Li,Xi-Long Fan*

**TL;DR**: 研究发现GW241011符合致密星团中的等级并合（hierarchical merger）机制，而GW241110的起源因分布不确定性较大仍存争议。


<details>
  <summary>Details</summary>

Background: 研究探讨了引力波事件GW241011和GW241110可能源自等级合并机制（hierarchical merger mechanism）的科学背景，挑战了恒星坍缩形成第一代双黑洞合并的传统观点。

Data: data部分：研究使用了引力波事件GW241011和GW241110的观测数据，并通过广义相对论中的并合熵指数（merger entropy index）分析其层级并合（hierarchical merger）机制。

Method: 采用合并熵指数(merger entropy index)测量广义相对论下双黑洞合并的熵转移效率。

Result: GW241011符合致密星团中等级合并( hierarchical merger)机制的特征。
...
### [11] [IMPACTX: An X-ray Spectral Model for Polar Dust and Clumpy Torus](https://arxiv.org/abs/2512.20993)
*Kanta Fujiwara,Yoshihiro Ueda,Shoji Ogawa,Yuya Nakatani,Ryosuke Uematsu*

**TL;DR**: 开发了包含活动星系核(AGN)中团块环(clumpy torus)和极区尘埃(polar dusty gas)反射成分的通用X射线光谱模型IMPACTX，并通过与红外数据联合建模成功应用于NGC 4388，证明了多波段数据对约束AGN核结构的重要性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于构建活动星系核（AGN）中来自团块状环面（clumpy torus）和极区尘埃气体（polar dusty gas）反射成分的通用X射线光谱模型（IMPACTX），以探讨AGN核区结构的X射线光谱特征。

Data: 研究中使用的数据来源于蒙特卡洛（Monte-Carlo）三维辐射传输代码SKIRT，基于Nenkova等（2008）提出的团块环（clumpy torus）和抛物面壳极区尘埃气体（paraboloid-shell polar dusty gas）的几何结构进行计算。

Method: 采用蒙特卡洛（Monte-Carlo）三维辐射传输代码SKIRT，结合包含团块状环（clumpy torus）和抛物面壳层极区尘埃气体（paraboloid-shell polar dusty gas）的几何结构，构建了X射线反射光谱模型IMPACTX。

Result: 构建了一个包含活动星系核(AGN)中团块状环(clumpy torus)和极区尘埃气体(polar dusty gas)反射成分的通用X射线光谱模型IMPACTX，并验证了其通过联合X射线与红外数据约束AGN核区结构的有效性。
...
**TL;DR**: 基于ZTF DR2数据对124颗亚光度Ia型超新星（subluminous SNe Ia）的分析发现，除02es-like亚型外，其余亚型均呈现本征红色特征，且多分布于质量更大、更红的主星系及局域环境中，同时4300Å吸收特征和s_BV可作为其亚型分类的可靠指标。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用兹威基瞬变设施第二次数据发布（ZTF DR2）中的Ia型超新星（SNe Ia）样本，特别是124颗亚光度（subluminous）SNe Ia的光谱和测光分析，探讨其物理特性、宿主星系环境及分类指标。

Data: 数据来源为兹威基瞬变设施第二版数据发布（ZTF DR2）中的3628颗Ia型超新星（SNe Ia）样本，并重点分析了其中124颗次亮SNe Ia的光变曲线和光谱数据，包括87颗91bg-like、12颗86G-like、18颗04gs-like和7颗02es-like事件。

Method: 采用基于SNooPy的模板拟合方法获取光变曲线参数，并使用Spextractor测量关键光谱特征的膨胀速度（expansion velocities）和伪等效宽度（pseudo-equivalent widths, pEW）。

Result: 所有亚光度Ia型超新星（除02es-like亚型外）本质呈现红色，通过分离外在和内在颜色成分证实了这一点。
...
**TL;DR**: 利用JWST-MIRI在4颗0级原恒星中首次精确测量了二氧化硫(SO$_2$)冰含量（占挥发性硫的0.2-0.9%），为评估其作为星际硫储库的可靠性提供了关键约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原恒星包层中二氧化硫(SO$_2$)冰作为固态硫储库的可靠性及其对"缺失硫"问题的潜在意义。

Data: 数据来源于JWST（James Webb Space Telescope）的中红外（MIRI）中分辨率光谱仪对四颗Class 0原恒星（B335、IRAS 15398-3359、L483、Ser-emb~7）在6.8-8.5 μm波段的观测，以及莱顿天体化学冰数据库（LIDA）的实验室光谱。

Method: 采用随机多项式拟合评估吸收光谱的连续谱基线不确定性，并使用开源Python库Omnifit拟合莱顿天体化学冰数据库(LIDA)中的实验室光谱。

Result: 在原始恒星包层中检测到二氧化硫（SO$_2$）冰，其作为固态硫储库的可靠性仍不明确。
...
### [14] [Quirks Live in Cool Universes](https://arxiv.org/abs/2512.20696)
*Pouya Asadi,Graham D. Kribs,Markus A. Luty*

**TL;DR**: 研究发现宇宙学观测对标准模型（SM）中"奇子"（quirks）模型的重加热温度（reheat temperature, $T_\text{RH}$）设定了严格上限（$\lesssim \mathcal{O}(100)$ GeV），并通过暗胶球（glueballs）残留丰度与宇宙学观测的矛盾揭示了奇子模型与重子生成机制的潜在冲突。


<details>
  <summary>Details</summary>

Background: 研究探讨了标准模型（Standard Model, SM）中"quirks"（奇特粒子）最小模型的再加热温度（reheat temperature, T_RH）受宇宙学观测严格限制的科学背景。

Data: 数据来源于宇宙学观测（cosmological observations）对标准模型（Standard Model, SM）中重加热温度（reheat temperature, T_RH）的限制分析。

Method: 采用紫外冻结机制（UV freeze-in）计算胶球（glueball）残留丰度，并结合其长寿命特性通过BBN破坏、CMB畸变、γ射线过量、自相互作用暗物质（self-interacting dark matter）超丰度及ΔN_eff贡献等多重宇宙学观测约束重加热温度（reheat temperature）T_RH上限。

Result: 宇宙学观测对标准模型（SM）中“奇子”（quirks）最小模型的再加热温度（reheat temperature, $T_\text{RH}$）给出了严格上限，要求$T_\text{RH} \lesssim \mathcal{O}(100)$ GeV，且该限制对强相互作用不确定性不敏感。
...
**TL;DR**: 研究发现，在$R^2$-希格斯暴胀模型中引入六维$R^3$项可缓解高谱指数$n_s$与观测的$\sim2σ$张力，同时诱导快速戈德斯通(Goldstone)和希格斯预加热，从而支持观测到的$n_s$。


<details>
  <summary>Details</summary>

Background: 该研究探讨了在宇宙微波背景（CMB）和重子声波振荡（BAO）观测数据下，$R^2$-希格斯暴胀模型及其单场近似机制与高谱指数$n_s$的$\sim2\sigma$水平不兼容性，并引入六维$R^3$项缓解这一矛盾。

Data: 数据来源为宇宙微波背景辐射（CMB）和重子声学振荡（BAO）的联合观测数据。

Method: 采用六维$R^3$项修正作用量并结合戈德斯通(Goldstone)模和希格斯(Higgs)预热的快速再加热机制分析。

Result: 在$R^2$-希格斯暴胀模型中引入六维$R^3$项可缓解高谱指数$n_s$与观测的$\sim2σ$张力，并诱导快速戈德斯通(Goldstone)和希格斯预加热，从而支持观测到的$n_s$。
...
### [16] [Axion-photon conversion in stochastic magnetic fields](https://arxiv.org/abs/2512.21108)
*Wataru Chiba,Ryusuke Jinno,Kimihiro Nomura*

**TL;DR**: 研究发现随机磁场（stochastic magnetic fields）中轴子（axion）-光子转换会导致光子强度和偏振态的演化，揭示了强度、线偏振和圆偏振的期望值与方差对磁场相关长度、传播距离和轴子质量的非平庸依赖关系，并首次观测到初始非偏振光子可产生显著偏振、特定频段方差抑制及螺旋磁场（helical magnetic fields）下圆偏振期望值的峰值结构，同时建立了与磁场功率谱形式无关的统计量一致性关系。
...
Data: 使用高斯随机磁场（Gaussian magnetic fields）的螺旋/非螺旋分量（helical/non-helical components）的功率谱（power spectra）作为数据来源。

Method: 采用高斯随机磁场（Gaussian magnetic fields）功率谱分析方法，结合螺旋/非螺旋分量特征，推导了轴子-光子转换（axion-photon conversion）过程中光子强度及线/圆偏振（linear/circular polarizations）的期望值与方差表达式。

Result: 在随机磁场中，轴子（axion）-光子转换会导致初始非偏振光子产生非平庸偏振（包括圆偏振在统计螺旋磁场中出现峰值结构），且这些统计量对磁场相关长度、传播距离和轴子质量具有复杂依赖关系。
...
### [17] [Large lepton asymmetry from axion inflation and helium abundance hinted by ACT](https://arxiv.org/abs/2512.21117)
*Di Wu,Yifan Hu,Kohei Kamada*

**TL;DR**: 在赝标量暴胀（pseudoscalar inflation）模型中，通过引入抑制费米子产生的机制，利用U(1)$_{L_μ-L_τ}$对称性下的手征反常（chiral anomaly）和螺旋磁场（helical magnetic fields）衰变，可产生足够大的轻子不对称性（lepton asymmetry），从而显著降低原初氦丰度。


<details>
  <summary>Details</summary>

Background: 研究探讨了赝标量暴胀（pseudoscalar inflation）中通过手征反常（chiral anomaly）产生螺旋磁场（helical magnetic fields）及手征不对称性（chiral asymmetry）的机制，并分析了在规范轻子味对称性（U(1)$_{L_i-L_j}$）下生成可观轻子不对称性（lepton asymmetry）的可能性。

Data: 数据来源于理论模拟（simulation）分析，基于假标量暴胀（pseudoscalar inflation）模型中U(1)规范场与暴胀子的Chern-Simons耦合作用，通过抑制暴胀期间费米子产生机制来增强螺旋磁场（helical magnetic fields）的螺旋度（helicity）衰变产生的轻子不对称性。

Method: 采用伪标量暴胀模型中的手征反常（chiral anomaly）机制，结合U(1)规范场与暴胀子的Chern-Simons耦合，通过抑制暴胀期间费米子产生以增强螺旋磁场（helical magnetic fields）衰变产生的轻子不对称性。

Result: 在赝标量暴胀（pseudoscalar inflation）模型中，通过手征反常（chiral anomaly）产生螺旋磁场（helical magnetic fields）和手征不对称性（chiral asymmetry）是普遍现象。
...
### [18] [Acetaldehyde as a molecule for testing variations of electron-to-proton mass ratio](https://arxiv.org/abs/2512.21052)
*J. S. Vorotyntseva,S. A. Levshakov,M. G. Kozlov*

**TL;DR**: 该论文通过量子力学计算获得了乙醛分子（CH3CHO）在1-50 GHz低频跃迁中对基本物理常数μ（电子-质子质量比）变化的无量纲敏感系数Q（0.62-3.61），并利用乙醛和甲醇（CH3OH）分子谱线对银河系外围（D_GC ~ 8 kpc）三个分子云的观测数据给出了μ变化上限Δμ/μ = (0.1 ± 0.4)*10^(-7)，将银河系盘面大尺度下爱因斯坦局域位置不变性原理的可能违反限制在4*10^(-8)水平。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过乙醛分子（CH3CHO）的低频跃迁（1-50 GHz）探索基本物理常数μ（电子-质子质量比，m_e/m_p）的微小变化及其对爱因斯坦局域位置不变性原理（local position invariance）的潜在违背。

Data: 研究使用了量子力学计算方法和乙醛（CH3CHO）分子在1-50 GHz频段的跃迁数据，并结合三个位于大银心距（D_GC ~ 8 kpc）分子云中乙醛和甲醇（CH3OH）谱线的实际观测数据。

Method: 采用量子力学方法计算了乙醛（CH3CHO）分子低频（1-50 GHz）跃迁对基本物理常数μ（电子-质子质量比）变化的无量纲敏感系数（Q）。

Result: 乙醛（CH3CHO）分子在1-50 GHz低频跃迁中对电子-质子质量比（mu）变化的无量纲敏感系数（Q）计算值为0.62至3.61。
...
### [19] [Effects of External Magnetic Fields on the Multi-mode Rayleigh-Taylor Instability](https://arxiv.org/abs/2512.20863)
*Xin Bian,Riccardo Betti,Dongxiao Zhao,Hussein Aluie*

**TL;DR**: 通过二维和三维电阻磁流体动力学(MHD)模拟揭示了均匀外磁场（平行或垂直于初始界面）对多模磁瑞利-泰勒不稳定性(mRTI)非线性演化的调控机制：弱平行磁场促进混合区增长而强场抑制，垂直磁场初期抑制后期增强，其本质是磁张力改变了流动各向异性、浮力、阻力和涡旋动力学。


<details>
  <summary>Details</summary>

Background: 研究聚焦于磁瑞利-泰勒不稳定性(mRTI)在天体物理惯性约束聚变和星际介质等离子体结构形成中的作用机制。

Data: 使用了电阻磁流体动力学（resistive MHD）模拟数据，研究了二维和三维空间中多模式磁瑞利-泰勒不稳定性（mRTI）的非线性演化过程。

Method: 采用电阻磁流体动力学(MHD)模拟方法研究多模磁瑞利-泰勒不稳定性(mRTI)的非线性演化。

Result: 弱平行磁场增强混合区增长而强磁场抑制增长，垂直磁场初期抑制增长但后期增强。
...
### [20] [Expanding Asteroseismic Studies in Star Clusters Using NASA's TESS and ESA's Gaia Missions](https://arxiv.org/abs/2512.20923)
*Carli Mankowski,Jamie Tayar,Cassidy Martin*

**TL;DR**: 该研究利用TESS和Gaia数据分析了14颗来自3个密集星团的恒星、5颗各自星团中唯一检测到的振荡恒星及3颗未知星团归属的振荡恒星，显著增加了可用于星震学分析的星团巨星数量，从而改进了恒星模型与星震学方法的交叉验证及银河系年龄估算技术。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用星震学（asteroseismology）和星团（star clusters）的同质特性来改进恒星演化模型的校准及银河系年龄估算。

Data: 数据来源为NASA的TESS（Transiting Exoplanet Survey Satellite）任务和ESA的Gaia（Global Astrometric Interferometer for Astrophysics）任务。

Method: 利用NASA的TESS任务和ESA的Gaia任务数据，结合星震学（asteroseismology）分析恒星振荡特性，对星团恒星进行精确质量、半径和年龄的标定。

Result: 利用TESS和Gaia数据识别出14颗属于3个密集星团的恒星、5颗各自星团中唯一探测到的振荡恒星及3颗未知星团归属的振荡恒星，显著增加了可用于星震学分析的星团恒星数量。
...
**TL;DR**: 论文研究了在空间平坦FRW宇宙中黑洞辐射的两种非经典量子态（压缩数态SNS和相干压缩数态CSNS），发现霍金温度随压缩参数ρ和数态参数n单调增长，熵变ΔS_SNS和ΔS_CSNS呈现显著非线性增强，揭示了霍金辐射对非经典量子构型的敏感性。


<details>
  <summary>Details</summary>

Background: 研究探索了在空间平坦的FRW宇宙中黑洞辐射的非经典量子态（挤压数态SNS和相干挤压数态CSNS）对霍金温度、熵变和质量损失的半经典理论影响。

Data: 数据来源于半经典理论框架下对黑洞辐射的解析计算，通过分析挤压数态（SNS）和相干挤压数态（CSNS）两种非经典量子态的参数（挤压参数ρ和数态参数n）对霍金辐射的影响，并辅以十二幅详细图表支持。

Method: 采用半经典引力理论框架下的压缩数态（SNS）和相干压缩数态（CSNS）量子构型，通过解析推导霍金温度、熵变及质量损失的态分辨表达式，结合挤压参数ρ和数态参数n对发射特性的影响分析。

Result: 在空间平坦的FRW宇宙中，对挤压数态（SNS）和相干挤压数态（CSNS）这两种非经典量子态的半经典分析表明，霍金温度随挤压参数ρ和数态参数n单调增长，导致黑洞视界处有效温度升高。
...
### [22] [Acoustic gravitational waves from primordial curvature perturbations](https://arxiv.org/abs/2512.21151)
*Zhuan Ning,Zi-Yan Yuwen,Xiang-Xi Zeng,Rong-Gen Cai,Shao-Jiang Wang*

**TL;DR**: 通过混合数值框架揭示了原始曲率扰动诱导的引力波（SIGWs）在声学通道（流体运动）中的非微扰效应，发现大振幅下声学引力波幅度可增强一个量级且峰值频率向低频偏移，并确定了与峰值平均间距（$R_{*c}^{-7}$）的强依赖关系及$k^{3}$低频尾特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原初曲率扰动诱导的引力波（SIGWs）中非微扰效应（nonperturbative effects）对随机引力波背景的影响，特别关注流体声学通道（acoustic channel）在强振幅区域的贡献。

Data: 数据来源于全广义相对论球对称模拟提取的非微扰声壳剖面（nonperturbative sound-shell profiles）和三维晶格演化中的相对论流体力学耦合横向无迹度规扰动计算。

Method: 采用混合数值框架结合完全广义相对论球对称模拟和三维晶格演化相对论流体动力学耦合横向无迹度规扰动，计算声学引力波谱。

Result: 非微扰效应导致原初曲率扰动诱导的声学引力波（acoustic GWs）振幅可放大一个量级且峰值频率向低频偏移，低频段呈现稳定的$k^{3}$因果尾巴，紫外功率因非线性流体作用增强。
...
### [23] [(Lovelock)$^2$ inflation: explaining the ACT data and equivalence to Higgs-Gauss-Bonnet inflation](https://arxiv.org/abs/2512.21167)
*Andrea Addazi,Yermek Aldabergenov,Daulet Berkimbayev,Yifu Cai*

**TL;DR**: 通过引入含高斯-博内特项（Gauss-Bonnet term）的二次拉格朗日量（quadratic Lagrangian）修正Starobinsky暴胀模型，该研究使标量谱指数（scalar spectral index）和张量标量比（tensor-to-scalar ratio）的预测更符合ACT观测数据，同时揭示了该模型与耦合高斯-博内特项的希格斯暴胀（Higgs inflation）在标量-张量表述下的等价性。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于基于阿塔卡马宇宙学望远镜（ACT）最新数据重新审视斯塔罗宾斯基暴胀模型（Starobinsky model of inflation），探讨其对标量谱指数（scalar spectral index）预测与观测数据间微小张力的修正。

Data: 数据来源于阿塔卡马宇宙学望远镜（ACT）的最新观测数据。

Method: 采用标量-张量（scalar-tensor）理论框架下的洛维洛克不变量（Lovelock invariant）$L=R+\fracα{4}{\cal G}$构建二次模型$\sim L+L^2$，通过引入非零高斯-博内项（Gauss-Bonnet term）耦合参数$α$修正暴胀慢滚动力学（slow-roll dynamics）。

Result: 在洛夫洛克不变量（Lovelock invariant）$L=R+\fracα{4}{\cal G}$中引入二次项$\sim L+L^2$的单参数扩展模型，通过高斯-博内项（Gauss-Bonnet term）的非零耦合$α$调整标量谱指数$n_s$和张量标量比$r$的轨迹，使预测更符合ACT观测数据。
...
### [24] [When Geometry Radiates Review: Gravitational Waves in Theory, Cosmology, and Observation](https://arxiv.org/abs/2512.21328)
*Azadeh Maleknejad*

**TL;DR**: 该论文系统构建了引力波（gravitational waves）的理论与观测框架，从线性化广义相对论推导其基本性质，探讨了宇宙膨胀背景下的演化机制及暴胀（inflation）产生的原初引力波谱，并综述了多波段探测技术及其天体物理与宇宙学起源。
...
Data: 数据来源包括宇宙微波背景辐射偏振(cosmic microwave background polarization)、脉冲星计时阵列(pulsar timing arrays)、地基和天基激光干涉仪(ground- and space-based laser interferometers)以及共振腔探测器(resonant cavity detectors)。

Method: 采用线性化广义相对论推导引力波解，结合外尔张量（Weyl tensor）的几何分析，并扩展到膨胀宇宙中的引力波演化与暴胀谱生成。
