...
**TL;DR**: 首次在亚千秒差距尺度上测量了银河系中心黑洞周围约1 pc半径的环核盘/环（CND/CNR）区域的CO-to-H2转换因子（α_CO≈4.5±2.5 M⊙ (K km s^-1 pc^2)^-1），发现其与银河系盘面一致但高于中心分子区（CMZ）。


<details>
  <summary>Details</summary>

Background: 研究探讨了一氧化碳(CO)作为分子氢(H2)示踪剂在星际介质(ISM)中的应用，重点关注了银河系中心附近极端环境下CO-to-H2转换因子的变化特性。

Data: 数据来源于银河系中心黑洞周围约1 pc半径范围内的环核盘/环（CND/CNR）的观测数据。

Method: 采用CO-to-$\mathrm{H}_2$转换因子（$α_\mathrm{CO}$或X[CO]）量化分子氢分布，通过亚秒差距尺度观测数据计算银河系中心黑洞周围环核盘/环（CND/CNR）区域的转换系数。

Result: 在银河系中心黑洞周围约1 pc半径的环核盘/环(CND/CNR)区域测得CO-to-H$_2$转换因子为$α_\mathrm{CO} \simeq 4.5\pm2.5 \, M_\odot (\mathrm{K \, km \, s^{-1} pc^2})^{-1}$，与银盘一致但高于中央分子区(CMZ)。
...
**TL;DR**: 通过Hestia和Auriga模拟研究发现，HI-rich暗星系（HIDES）具有低光度、高中性氢（HI）含量、古老恒星年龄（约110亿年）等特征，其多样性与质量聚集历史和环境效应（如潮汐剥离、并合事件）密切相关。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于宇宙中富含中性氢（HI-rich）但光学暗弱的"暗星系"（Dark galaxiEs）的成因及演化机制，特别关注其质量累积历史和环境效应对星系多样性的影响。

Data: 数据来源于Hestia和Auriga数值模拟（simulation），筛选出89个暗弱（$M_g > -10$）且富含中性氢（$M_\mathrm{HI} > 10^5\,M_\odot$）的星系样本。

Method: 采用Hestia和Auriga宇宙学模拟（cosmological simulations）筛选HI-rich暗弱星系（$M_g > -10$且$M_\mathrm{HI} > 10^5\,M_\odot$），分析其质量分布、气体金属丰度、恒星形成活动及环境效应（如冲压剥离（ram pressure stripping）、并合历史），并建立经验公式$n = 0.25 \left(d_\mathrm{MW}/{1\,\mathrm{Mpc}}\right)^{-1.4}\, \mathrm{Mpc}^{-3}$描述局域体积数密度。

Result: 在Hestia和Auriga模拟中发现的89个HI-rich暗星系（HIDES）具有典型特征：$M_{200} \sim 10^{9.5}\,M_\odot$、$M_\mathrm{gas} \sim 10^{7.4}\,M_\odot$、$M_\mathrm{HI} \sim 10^{6.5}\,M_\odot$、$M_\mathrm{*} \sim 10^{5.6}\,M_\odot$、低气体金属丰度、极少或零恒星形成、平均恒星年龄约110亿年，部分可在距银河系质量邻居约300千秒差距的致密环境中存活。
...
**TL;DR**: 通过分析63颗具有幂律谱且带指数截断的脉冲星（PSRs）射电连续谱，发现其截断频率与归一化流量密度呈幂律相关，揭示了截断频率集中在558 MHz附近的高发射量（EM ~ 10^5 pc cm^-6）特征，并结合色散测量（DM）解耦了吸收介质的电子密度与路径长度，发现了一类具有小尺寸（L ~ 0.1 pc）、高电子密度（n_e ~ 10^3 cm^-3）且呈现尺寸-密度反相关性的离散近域吸收体群体。


<details>
  <summary>Details</summary>

Background: 研究聚焦于脉冲星（PSRs）射电连续谱的多样性及其与视线方向上自由-自由热吸收（free-free thermal absorption）介质的关联。

Data: 数据来源于四个已发表研究中汇编的63颗脉冲星（PSRs）样本，其射电连续谱呈现具有明确幂律（ν^{-2.1}）和指数截断的谱形特征。

Method: 采用归一化处理（normalisation）和幂律拟合（power law fitting）分析63颗脉冲星（PSRs）的射电连续谱，结合色散测量（DM）和发射测量（EM）解耦电子密度与吸收体路径长度的简并性。

Result: 脉冲星（PSRs）的射电连续谱呈现出多种形态，包括纯幂律、转折幂律、截断幂律以及对数抛物线轮廓，其中显著部分具有明确的ν⁻²·¹指数转折特征，表明视线方向上存在自由-自由热吸收过程。
...
**TL;DR**: 通过ALMA观测典型主序恒星形成星系（MS SFGs）在宇宙正午时期（zmed=1.3）的CO(2-1)数据，发现其分子气体（MH2）空间延伸尺度（~12 kpc）与恒星辐射（~13 kpc）相近，且未检测到高速分子外流（outflows）的显著证据，表明该时期主序星系分子外流较弱，未能将大量分子气体驱逐至星系恒星辐射区以外的环星系介质（CGM）中。


<details>
  <summary>Details</summary>

Background: 星系演化主要受重子物质流入和流出驱动，其中外流(Outflows)不仅消耗星系气体储备，还将处理后气体注入星系周介质(CGM)，影响后续气体吸积过程并导致分子气体存在于星系恒星成分之外的CGM尺度。

Data: 使用阿塔卡马大型毫米波/亚毫米波阵列（ALMA）的Band 3 CO(2-1)数据和UltraVISTA Ks波段数据，通过叠加技术研究了26个典型主序（MS）恒星形成星系（SFGs）样本的分子气体（MH2）和恒星辐射的空间分布。

Method: 采用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)的Band 3 CO(2-1)数据，通过叠加(stacking)技术分析26个典型主序(MS)恒星形成星系(SFGs)样本中冷分子气体(MH2)的空间分布。

Result: 在宇宙正午（cosmic noon）时期的主序星系（MS SFGs）中，分子外流（molecular outflows）较弱，未能将大量分子气体（MH2）驱逐至超出恒星发射区域的星系周介质（CGM）尺度。
...
**TL;DR**: 通过比较不同双星演化模型（如BPASS和新颖剥离恒星模型）与高红移（high-z）星系本地类似体的静止紫外-光学光谱，首次揭示了双星演化对解释高红移星系光谱特征的关键作用，并确定了改进模型的核心方向。


<details>
  <summary>Details</summary>

Background: 研究主题聚焦于大质量恒星演化模型的不完善性及其对高红移(high-z)星系光谱特征（如He II发射线）的解释不足。

Data: 数据来源为高红移（high-z）星系的局域类似体（local analogs）的静止紫外-光学波段（rest-UV-optical）光谱。

Method: 采用不同双星演化模型（如BPASS和新颖剥离恒星模型）与高红移星系本地类似体的静止紫外-光学光谱进行比较分析。

Result: 标准单星模型无法解释高红移(high-z)星系中观测到的强He II发射线，需引入双星演化(binary evolution)和非太阳丰度(non-solar abundance)模式才能重现JWST观测到的光谱特征。
...
**TL;DR**: 提出ViSta（Visibility Stacking）方法，通过在傅里叶域叠加射电至亚毫米波段的干涉观测数据（interferometric observations），显著提升星系（galaxies）信号重建精度，尤其对低于噪声水平的微弱延展源（extended sources）和尘埃恒星形成星系（DSFGs）的CO(3-2)谱线检测效果优于传统图像域叠加方法。


<details>
  <summary>Details</summary>

Background: 研究主题为开发一种在射电至亚毫米波段（radio to sub-millimeter wavelengths）的傅里叶域（Fourier domain）中结合干涉观测数据的可见度叠加方法（Visibility Stacking method），以最大化利用现有存档干涉数据并提升信号重建精度。

Data: 数据来源包括模拟干涉数据集（simulated interferometric datasets）和ALMA观测的尘埃恒星形成星系（DSFGs）样本的存档干涉数据（archival interferometric data）。

Method: 采用基于傅里叶域的可视度叠加（Visibility Stacking）方法，在射电至亚毫米波段对星系干涉观测数据进行直接叠加和静止坐标系转换。

Result: ViSta方法通过傅里叶域叠加干涉观测数据（visibility stacking），有效增强信号、抑制噪声并改善图像重建，尤其适用于微弱和延展源的低信噪比探测。
...
**TL;DR**: 基于92个近邻星系（近邻星系多线成像项目COMING）的CO(1-0)观测数据，研究发现偏离恒星形成主序(MS)的星系中：上主序(UMS)星系（多为低质量或棒旋星系）通过提升恒星形成效率(SFE)实现高ΔMS值，而下主序(LMS)星系（多为含活动星系核AGN的大质量星系）因分子气体比例(fgas)降低导致ΔMS值下降，并呈现中心区域恒星形成由内向外熄灭的趋势。


<details>
  <summary>Details</summary>

Background: 研究探讨了近邻星系中恒星形成率面密度（ΣSFR）、恒星质量面密度（ΣM*）和分子气体质量面密度（ΣMmol）在千秒差距尺度上的关系，重点分析了偏离恒星形成主序（ΔMS）的星系分类及其物理机制。

Data: 数据来源于CO多线成像近邻星系巡天项目（COMING）中92个近邻星系的$^{12}$CO $J=1-0$成图数据，通过同心倾斜环平均得到约1千秒差距尺度的观测数据。

Method: 采用近1千秒差距（kpc）尺度的同心倾斜环平均方法，基于CO多线成像巡天（COMING）项目中92个近邻星系的$^{12}$CO $J=1-0$成图数据，分析恒星形成率面密度（Σ$_{\mathrm{SFR}}$）、恒星质量面密度（Σ$_{M_{\ast}}$）与分子气体质量面密度（Σ$_{M_\mathrm{mol}}$）的关系。

Result: 上主序星系（UMS）的恒星形成效率（SFE）高于主序星系（MS），而分子气体比例（fgas）与MS星系相当。
...
**TL;DR**: 通过高分辨率ALMA 3毫米深场观测结合MUSE数据，首次在红移z>1的星系群环境中揭示了分子气体分布的多样性特征，发现其星际介质气体质量（2×10^9至9×10^10太阳质量）与场星系群体相当。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用阿塔卡马大型毫米波/亚毫米波阵列（ALMA）的长基线观测，结合多单元光谱探测器（MUSE）数据，探索红移（redshift, z）大于1的星系群环境中分子气体的空间分辨性质及其与场星系（field galaxies）的对比。

Data: 数据来源于阿塔卡马大型毫米波/亚毫米波阵列(ALMA)对哈勃深场南(Hubble Deep Field South)1x1平方角分区域的深度3毫米观测，采用包含4个指向的马赛克观测和单波段3(3mm)频谱调谐，总有效观测时间为61小时，最长基线达15公里，同时结合多单元光谱探测器(MUSE)对同一区域的观测数据。

Method: 采用长基线（long-baselines）配置的阿塔卡马大型毫米波/亚毫米波阵列（ALMA）进行深度3毫米观测，结合多单元光谱探测仪（MUSE）数据，通过4点马赛克拼接和单波段（Band3）光谱调谐覆盖不同红移组的CO谱线跃迁。

Result: 在z>1的星系群环境中，通过高分辨率观测发现星际介质气体质量（inter-stellar medium gas mass）介于~2E9至~9E10太阳质量之间，且该星系群成员具有高度多样性，其宇宙分子气体质量密度（cosmic molecular gas mass density）与场星系环境相当。
...
**TL;DR**: 通过对天鹰座分子云复合体（Aquila molecular cloud complex）中W40和Serpens South区域的分子谱线观测，发现速度相干细丝（velocity-coherent filaments）的热超临界状态（thermally supercritical）、引力束缚特性及纵向质量流（longitudinal flows）是细丝内致密核（dense cores）形成的关键因素。


<details>
  <summary>Details</summary>

Background: 研究聚焦于分子云中丝状结构（filamentary structures）在致密核（dense cores）和恒星形成过程中的作用机制。

Data: data部分：研究使用了TRAO-FUNS项目对天鹰座分子云复合体W40和Serpens South区域的分子谱线观测数据，包括C$^{18}$O谱线的高斯分解结果和N$_{2}$H$^{+}$积分强度图，并结合Herschel巡天数据进行了对比分析。

Method: 采用高斯分解（Gaussian decomposition）处理C$^{18}$O谱线数据以分离视线方向的多速度成分，结合“朋友的朋友”（Friends-of-Friends）算法识别速度相干丝状结构，并应用“FellWalker”算法在N$_{2}$H$^{+}$积分强度图中定位致密核。

Result: 在W40和蛇夫座南部区域观测中发现，速度相干细丝(filaments)的热临界状态(thermally supercritical)和引力束缚性质对致密核(core)形成起关键作用。
...
**TL;DR**: MAUVE项目通过结合VLT/MUSE和ALMA对40个晚型（late-type）室女座星系团（Virgo Cluster）星系的多相观测，以分子云复合体（giant molecular cloud complexes）尺度揭示了环境淬灭（environmental quenching）过程中分子气体盘内恒星形成、运动学和化学增丰的演化特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于致密环境（dense environments）如何影响星系演化，特别是追踪星系进入星系团（Virgo Cluster）后内部冷气体（cold gas）的演化过程。

Data: 数据来源为甚大望远镜(VLT)多单元光谱探测仪(MUSE) P110大型观测项目和ALMA(阿塔卡马大型毫米波/亚毫米波阵列)对40个晚型室女座星系团(Virgo Cluster)星系的观测数据。

Method: 采用甚大望远镜(VLT)多单元光谱探测仪(MUSE)和ALMA（阿塔卡马大型毫米波/亚毫米波阵列）观测数据，结合分子气体盘内恒星形成、运动学和化学增丰的多相协同分析，以巨分子云复合体(GMC)物理尺度的高空间分辨率追踪星系环境淬灭过程。

Result: MAUVE项目通过结合VLT/MUSE P110大型观测计划和ALMA对40个晚型室女座星系团星系的观测，以分子气体盘内恒星形成、运动学和化学增丰的高分辨率数据，追踪卫星星系进入星系团后其内部冷气体在不同坠入阶段的演化过程。
...
**TL;DR**: 通过对比TNG100流体动力学模拟和暗物质模拟，发现重子物质(baryons)使星系并合时间缩短、轨道更偏向螺旋式(spiral-in)，且碰撞角减小6°-10°，分辨率降低时效应减弱。


<details>
  <summary>Details</summary>

Background: 研究探讨了重子物质(baryons)在模拟中对星系并合轨道的影响，通过比较TNG100流体动力学模拟和对应暗物质模拟中匹配并合对的并合时标和轨道差异。

Data: 数据来源于TNG100流体动力学模拟（TNG100-1）及其对应的纯暗物质模拟（TNG100-1-Dark），并对比了不同分辨率水平下的匹配合并对。

Method: 采用匹配分析(matched analysis)方法对比TNG100流体动力学模拟(hydrodynamical simulation)与对应暗物质模拟(dark-matter-only simulation)中星系并合对的轨道参数和时间尺度差异。

Result: 重子物质（baryons）的存在略微加速了星系并合过程，并导致不同恒星质量星系中主并合和次并合产生更多螺旋轨道（spiral-in orbits）。
...
**TL;DR**: 开发了基于JAX的可微分端到端管道RUBIX，实现了积分视场单元(IFU)数据与粒子模拟的梯度关联，为机器学习驱动的IFU推断奠定了基础。


<details>
  <summary>Details</summary>

Background: 研究背景是开发可微分（differentiable）的端到端积分视场单元（IFU）数据建模流程，以弥合基于粒子的数值模拟与星系观测之间的鸿沟。

Data: 使用JAX框架开发的RUBIX软件（源代码公开于https://github.com/AstroAI-Lab/rubix）对模拟星系积分视场单元（IFU）数据进行端到端建模，并通过粒子输入计算梯度。

Method: 采用基于JAX的端到端可微分管道（RUBIX）建模积分视场单元（IFU）数据立方体，并通过纯函数式分片实现梯度计算。

Result: RUBIX是一种基于JAX的端到端管道，能够建模积分视场单元(IFU)数据立方体并计算粒子输入的梯度。
...
**TL;DR**: 首次通过大双筒望远镜（Large Binocular Telescope）在亚角秒视宁条件下解析KK 153（气体丰富的超暗弱矮星系）外围区域的单个恒星并测得其红巨星分支尖端，获得更精确距离（3.06 (+0.17/-0.14) Mpc）和恒星质量（2.4±0.2×10^6 M_⊙），证实其位于本星系群外且比Leo T和Leo P质量更大，暗示其恒星形成历史可能受宇宙再电离影响但仍保留足够气体形成新恒星。


<details>
  <summary>Details</summary>

Background: 研究聚焦于KK 153这一新发现的富气超暗矮星系（gas-rich ultra faint dwarfs）的成员身份及其在宇宙再电离（re-ionisation）背景下独特的恒星形成历史。

Data: 数据来源于大双筒望远镜（Large Binocular Telescope）在亚角秒视宁度条件下对KK 153星系外围区域首次分辨出的单颗恒星观测数据，包括达到红巨星分支（red giant branch）的测光数据。

Method: 采用大型双筒望远镜(Large Binocular Telescope)在亚角秒视宁度条件下首次分辨出KK 153外围区域的单个恒星，并测量红巨星分支顶端星等以确定距离。

Result: 首次利用大双筒望远镜（Large Binocular Telescope）在亚角秒视宁条件下解析KK 153外围区域的单个恒星并触及红巨星分支（red giant branch），测得红巨星分支尖端星等对应距离为D=3.06 (+0.17/-0.14) Mpc，显著修正了此前基于重子塔尔-费希尔关系（baryonic Tully-Fisher relation）的估算值D=2.0 (+1.7/-0.8) Mpc。
...
**TL;DR**: Arp@VST项目利用VLT巡天望远镜(VST)对阿普特殊星系表(Atlas of Peculiar Galaxies)中可见的338个形态扭曲或相互作用的星系进行深度多波段(g、r、i、Hα)成像观测，旨在通过公开巡天数据重新研究这些星系。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用VLT巡天望远镜(VST)对阿普特殊星系表(Atlas of Peculiar Galaxies)中338个形态扭曲或相互作用的星系进行深度多波段成像，以建立公开巡天数据库。

Data: 数据来源于VLT巡天望远镜（VST）在ESO帕拉纳尔天文台进行的深度多波段（g、r、i、Hα）观测，覆盖阿普（Arp）星系目录中可见于帕拉纳尔（Declination < +10 deg）的全部目标，处理后数据将通过ESO科学档案公开。

Method: 采用VLT巡天望远镜(VST)进行深空多波段(g、r、i、Hα)成像观测，结合ESO帕拉纳天文台的低表面亮度探测能力，系统性地重测阿普特殊星系表(Arp catalogue)中可观测目标。

Result: Arp@VST项目旨在利用VLT巡天望远镜(VST)对Arp星表中338个形态扭曲或相互作用的星系进行深度多波段(g、r、i、Hα)成像观测，并将处理后的数据通过ESO科学档案公开释放。
...
**TL;DR**: 该研究利用多波段全天巡天数据精确拟合了6个邻近明亮星系（LMC、SMC、M31、M33、NGC 253和NGC 4945）在毫米-厘米波段的综合光谱能量分布（SED），发现其辐射主要来自热尘埃、自由-自由辐射和同步辐射，未明确检测到异常微波辐射（AME），并在5个星系背景中推断出正宇宙微波背景（CMB）温度涨落。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于毫米波（mm）-厘米波（cm）波段星系辐射的谱能量分布（SED）特征，填补了该波段相较于红外和射电领域的研究空白。

Data: 数据来源于COBE-DIRBE、IRAS、Planck和WMAP全天巡天观测，覆盖了从97微米到1.3厘米的18个光度波段，分辨率统一为1度。

Method: 采用多波段（18个）全天空巡天数据（COBE-DIRBE、IRAS、Planck、WMAP），通过热尘埃（thermal dust）、自由-自由辐射（free-free）、同步辐射（synchrotron）、反常微波辐射（AME）和宇宙微波背景（CMB）温度涨落模型拟合毫米-厘米波段的星系积分谱能量分布（SED）。

Result: 在毫米-厘米波段未明确检测到目标星系的异常微波辐射（AME），且其上发射率上限低于银河系标准，表明AME信号可能在星系整体尺度上被稀释。
...
**TL;DR**: 基于SRG/eROSITA对银河系质量星系周围X射线发射星系际介质（CGM）的观测，研究发现气体温度分布和晕质量不确定性导致X射线发射重子质量估算存在四倍差异（0.8-3.5×10¹¹M⊙），金属丰度假设带来额外50%不确定性。
...
Data: 数据来源于SRG/eROSITA观测获取的叠加（stacked）X射线信号，用于分析银河系质量星系（MW-mass galaxies）周围X射线发射的环星系介质（CGM）的平均表面亮度分布。

Method: 基于堆叠eROSITA信号评估气体密度分布、温度、金属丰度及暗晕质量分布假设对X射线发射的星系周介质(CGM)重子质量推断的影响。

Result: 温度剖面变化和晕质量(halo mass)不确定性是X射线辐射晕际介质(CGM)重子质量估算的主要误差来源，导致结果在0.8-3.5×10¹¹M⊙范围内波动。
...
**TL;DR**: 构建了模拟盘状宽线区（BLR）几何结构并响应连续谱光变产生合成发射线轮廓的流程，重现了观测中宽发射线中心速度的随机波动（jitter），为研究BLR模型提供了计算工具。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核(AGN)和类星体宽发射线的变异性特征，特别是探讨宽线区(BLR)在中心源辐射变化下的动力学响应机制及其对BLR结构模型的约束作用。

Data: 数据来源于模拟生成的盘状宽线区（BLR）几何结构响应不同连续谱光变曲线产生的合成发射线轮廓。

Method: 采用盘状宽线区(BLR)几何模型和合成发射线轮廓模拟管道，结合选定连续谱光变曲线进行反响映射分析。

Result: 通过模拟盘状宽线区（BLR）几何结构并生成合成发射线轮廓，成功重现了观测中发现的宽发射线径向速度随机波动（jitter）。
...
**TL;DR**: 基于盖亚(Gaia)-unWISE类星体目录(Quaia)中约130万颗类星体的两点相关函数测量，发现相关长度(r0)随红移(z)单调增加(0≤z<4时从6.8增至13.9 h⁻¹Mpc)，宿主晕最小特征质量(log10(Mmin/M⊙)≈12.8)和活动周期(fduty≈2%-7%)在宇宙时间中保持稳定，表明类星体生长存在与时代无关的自调节过程。


<details>
  <summary>Details</summary>

Background: 研究主题为利用类星体（quasar）两点相关函数追踪其成团强度随宇宙时间（红移）的演化规律，以揭示类星体宿主暗晕（halo）质量及活动寿命的长期稳定性。

Data: 数据来源于盖亚(Gaia)-unWISE类星体目录(Quaia)中G星等≤20.5、红移范围0≤z≤4的全天区均匀选取的约130万颗类星体样本。

Method: 采用两点相关函数(two-point correlation function)分析类星体(quasar)的空间分布，并结合偏差-晕质量关系(bias-halo mass relation)和阶跃函数(step-function)形式的晕占据分布(halo occupation distribution)模型推断宿主晕特征参数。

Result: 发现类星体（quasar）的关联长度（correlation length）随红移（redshift）稳定增加，从红移0-1时的6.8±0.2 h⁻¹Mpc增至3-4时的13.9±1.2 h⁻¹Mpc，斜率γ≈2，表明红移>3时的成团信号略弱于先前研究，但整体呈平滑单调上升趋势。
...
**TL;DR**: 研究发现莱曼阿尔法发射体(LAE)在N501窄波段(z~3.12)的原星系团(protocluster)区域莱曼阿尔法线光度(Lyα line luminosity)增加约15%，表明这些区域恒星形成活动增强，但在N419窄波段(z~2.45)未观察到类似效应，暗示该效应可能存在红移演化，同时莱曼阿尔法线轮廓形状不随环境密度变化。


<details>
  <summary>Details</summary>

Background: 研究聚焦于莱曼阿尔法发射体（LAEs）的莱曼阿尔法（Lyα）谱线轮廓与环境密度的关系，旨在揭示高红移（high redshift）大尺度结构中恒星形成活动的环境依赖性。

Data: 数据来源于暗能量光谱仪（DESI）对通过ODIN（One-hundred-deg$^2$ DECam Imaging in Narrowbands）巡天窄带测光选出的莱曼阿尔法发射体（LAEs）的光谱观测，并利用N419（z $\sim$ 2.45）和N501（z $\sim$ 3.12）窄波段的LAE面密度图分析局部环境密度与莱曼阿尔法（Ly$α$）线轮廓的关系。

Method: 采用核心分析方法为利用ODIN巡天窄带测光数据构建莱曼阿尔法发射体（LAE）表面密度图，结合DESI光谱仪观测数据，通过分环境密度区间和原星团内外区域对LAE光谱进行叠加分析。

Result: 莱曼阿尔法发射体(LAE)在N501窄波段(z~3.12)的原星系团(protocluster)区域显示出约15%更高的莱曼阿尔法(Lyα)线光度，表明这些区域恒星形成活动增强。
...
**TL;DR**: 通过模拟BAO（重子声波振荡）、CMB（宇宙微波背景）和Ia型超新星数据集的联合分析，发现单一或双数据集组合会因似然几何效应在暗能量状态方程（w0,wa）平面产生虚假偏移，而完整三数据集组合能准确恢复标准LambdaCDM宇宙学参数（w0,wa）=(-1,0)及（Omegam0,H0），强调多探测联合分析的必要性。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙学中动态暗能量(dynamical dark energy)状态方程可能存在的迹象，并分析了这些迹象是否源于数据集本身的似然几何结构、残余的探测手段间张力或限制性先验等因素的干扰。

Data: 研究中使用的模拟数据包括基于标准LambdaCDM宇宙学生成的模拟重子声波振荡（BAO）、宇宙微波背景（CMB）和Ia型超新星（SNe）数据集，具体包含DESI DR2 BAO协方差矩阵、Planck 2018距离先验协方差矩阵以及完整的Pantheon+ SH0ES超新星协方差矩阵。

Method: 采用模拟的BAO（重子声学振荡）、CMB（宇宙微波背景）和Ia型超新星数据集，基于共同基准LambdaCDM宇宙学，结合DESI DR2 BAO协方差、Planck 2018距离先验协方差和Pantheon+ SH0ES超新星协方差，通过CPL参数化隔离物理信息与几何统计效应。

Result: BAO（重子声学振荡）、CMB（宇宙微波背景）和SNe（Ia型超新星）单独或两两组合时在(w0,wa)平面出现伪动态暗能量信号，而三者联合分析能准确恢复基准ΛCDM宇宙学参数(w0,wa)=(-1,0)和(Omegam0,H0)。
...
**TL;DR**: 基于演化映射框架提出了一种新的晕质量函数（HMF）参数化方法，通过结构形成历史和线性物质功率谱形状来物理描述HMF的非普适性，并在多种宇宙学条件下实现了优于5%的精度。


<details>
  <summary>Details</summary>

Background: 研究主题为基于演化映射框架（Evolution Mapping framework）构建高精度晕质量函数（halo mass function, HMF）模型，重点解决HMF的非普适性（non-universality）问题及其与结构形成历史和线性物质功率谱（linear matter power spectrum）局部形态的关联。

Data: 数据来源于N-body模拟（N-body simulations）测量结果，使用十个不同过密度阈值（Δ）的晕样本，范围从150到1600倍平均背景物质密度。

Method: 采用基于演化映射框架（Evolution Mapping framework）的新方法，通过结构形成历史和线性物质功率谱（linear matter power spectrum）局部形状参数化晕质量函数（HMF）的非普适性。

Result: 基于演化映射框架构建的晕质量函数（HMF）新模型通过结构形成历史和线性物质功率谱（linear matter power spectrum）局部形状参数化其非普适性，在$Δ=200$质量定义下对多种质量、红移和结构形成历史实现百分之一精度，且在不同宇宙学功率谱形状下保持同等精度。
...
**TL;DR**: 通过引入基于海森矩阵(Hessian matrix)特征值的连续环境分类框架，揭示了星系本征性质与其所处大尺度结构环境之间存在明确的依赖关系，并识别出关键天体物理性质行为转变的临界值。


<details>
  <summary>Details</summary>

Background: 星系形成和演化中环境（environment）对星系本征性质的影响。

Data: 数据来源于半解析星系形成模型与宇宙学数值模拟（cosmological numerical simulations）的结合。

Method: 采用基于物质密度场海森矩阵(Hessian matrix)特征值的连续分类框架，结合半解析星系形成模型和宇宙学数值模拟，分析星系内禀属性与环境的关系。

Result: 提出了一种基于海森矩阵(Hessian matrix)特征值的连续环境分类框架，揭示了星系本征性质与环境之间存在连续依赖关系并识别出关键转变阈值。
...
**TL;DR**: 通过模拟快速射电暴(FRB)群体和构建神经网络模拟器，研究发现当前样本量下观测选择效应对条件分布p(DM_exgal|z)影响可忽略（偏差<0.8σ），但未来大样本（10^4 FRBs）需显式建模选择效应以避免超过3σ的偏差。


<details>
  <summary>Details</summary>

Background: 快速射电暴（FRB）已成为探测宇宙重子物质的有力工具，通过其河外色散测量-红移（DM$_\mathrm{exgal}$-$z$）关系为宇宙学参数和反馈机制提供约束。

Data: 研究中使用的数据来源包括：通过前向建模（forward-modeled）的快速射电暴（FRB）群体模拟数据，以及基于5×10^4个重子化（baryonification）晕模型模拟训练的神经网络模拟器数据。

Method: 采用前向建模（forward-modeling）的快速射电暴（FRB）群体模拟，结合神经网络模拟器对宇宙色散量（DM_cosmic）方差进行快速似然评估。

Result: 快速射电暴(FRB)的观测和源群选择效应对条件分布$p(\mathrm{DM}_{\mathrm{exgal}}|z)$的宇宙学推断影响可忽略（当前样本量下偏差$\lesssim0.8σ$），但对$10^4$个FRB样本可能导致$>3σ$偏差。
...
**TL;DR**: 通过分析宇宙计时器（CC）和重子声学振荡（BAO）数据集，发现G因子与红移z分别呈线性和三次方关系，且高G因子数据集的品质因数（FoM）显著更高，证实G因子可作为评估观测数据质量、减少参数简并的有效工具。


<details>
  <summary>Details</summary>

Background: 在宇宙学模型拟合中，参数简并性(parameter degeneracy)是核心挑战，研究聚焦于利用G因子评估观测数据质量以优化参数约束。
...
Method: 采用马尔可夫链蒙特卡洛（MCMC）模拟结合品质矩阵（FoM）作为拟合性能评估标准，重点利用G因子（G factor）分析观测数据质量以降低参数简并性（parameter degeneracy）。

Result: CC数据集的G因子（G factor）随红移z线性增长，而BAO数据集的G因子呈现三次方关系。
...
**TL;DR**: 通过引力透镜成像数量异常现象（image number anomaly）约束暗物质（DM）子结构丰度，发现原始黑洞（PBHs）丰度上限在95%置信水平下随角分辨率提高而降低（0.1角秒时为0.125%，0.05角秒时为0.08%，0.01角秒时为0.04%），并排除了模糊暗物质（FDM）粒子质量低于0.4×10⁻²²、0.6×10⁻²²和3.5×10⁻²²电子伏特的情况。


<details>
  <summary>Details</summary>

Background: 引力透镜（gravitational lensing）现象，特别是图像数量异常（image number anomaly），被用于探测暗物质（dark matter）子结构（substructure）的分布和性质。

Data: 数据来源于《强引力透镜晕模型模拟星表》(SL-Hammocks)生成的3500个透镜系统样本以及未来时空遗产巡天(LSST)观测数据。

Method: 利用引力透镜成像数量异常现象（image number anomaly）分析暗物质子结构（DM substructure）的分布特征，并基于不同角分辨率（angular resolution）观测数据推导原初黑洞（PBHs）和模糊暗物质（FDM）的丰度上限（abundance upper limits）。

Result: 基于3500个透镜系统样本的零异常图像数量检测，在95%置信水平下对原初黑洞（PBHs）丰度的上限约束为：在角分辨率0.1″、0.05″和0.01″时分别不超过0.125%、0.08%和0.04%，对应质量范围∼10⁷–10⁹ M⊙。
...
**TL;DR**: 通过分析平方四次希尔伯特暴胀势能（squared-Quartic Hilltop inflation potential）在爱因斯坦和乔丹框架下的表现，发现弱耦合（weak coupling）下微扰修正使标量谱指数（scalar spectral index）\(n_s\)略增至0.9743并抑制张量标量比（tensor-to-scalar ratio）\(r\)至\(7.8 \times 10^{-5}\)，强耦合（strong coupling）下共形变换产生指数平坦势能平台使\(n_s \approx 0.9743\)且\(r \lesssim 5 \times 10^{-4}\)，暴胀能量尺度（energy scale of inflation）\(V_0^{1/4} \sim 10^{-3}{-}10^{-2} M_p\)与高能标暴胀情景一致。


<details>
  <summary>Details</summary>

Background: 研究探讨了DESI和ACT最新观测数据与普朗克2018结果在标量谱指数（scalar spectral index）上的差异对标准暴胀模型的挑战，聚焦于平方四次希尔托普暴胀势（squared-Quartic Hilltop inflation potential）在爱因斯坦和乔丹框架下的理论解释。

Data: 数据来源为暗能量光谱仪（DESI）和阿塔卡马宇宙学望远镜（ACT）的联合观测数据。

Method: 采用平方四次希尔托普暴胀势（squared-Quartic Hilltop inflation potential）在爱因斯坦框架和乔丹框架下分析，乔丹框架中引入耦合项ξφ²R，计算弱耦合（ξ≪1）和强耦合（ξ≫1）区域的慢滚参数、标量谱指数和张量标量比解析表达式。

Result: result：在弱耦合极限（ξ≪1）下，平方四次希尔伯特暴胀势（squared-Quartic Hilltop inflation potential）得出标量谱指数（scalar spectral index）n_s≈0.9743和张量标量比（tensor-to-scalar ratio）r∼7.8×10⁻⁵，与P-ACT-LB联合约束一致；强耦合（ξ≫1）时共形变换产生指数平坦势平台，得到n_s≈0.9743且r≲5×10⁻⁴，符合ACT和BK18限制，暴胀能量尺度V₀¹ᐟ⁴∼10⁻³–10⁻²M_p仍兼容高能标暴胀模型。
...
**TL;DR**: 基于Kilo-Degree Survey Data Release 4（KiDS DR4）的光选类星体（quasar）样本，采用深度学习框架Hybrid-z更新了光红移（photo-z）测量，发现类星体偏置（bias）随红移呈二次增长关系（从z≈0.6时的1.6增至z≈2.2时的4.0），并揭示其寄居于质量对数范围log10(Meff/h−1M⊙)~12.7–12.9的暗物质晕中。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用千度巡天(KiDS)第四版数据中光度选类星体的角聚类和有效偏差，探索其宇宙学应用及暗物质晕分布特性。

Data: 数据来源于千度巡天数据发布4（KiDS DR4）的光度选类星体，结合了四波段KiDS图像和九波段KiDS+VIKING星等，并通过混合红移（Hybrid-z）深度学习框架更新了光度红移（photo-$z$s），训练数据来自最新的暗能量光谱仪（DESI）DR1和斯隆数字巡天（SDSS）DR17中与KiDS匹配的类星体光谱样本。

Method: 采用深度学习框架Hybrid-z结合四波段KiDS图像和九波段KiDS+VIKING星等数据更新光度红移(photo-z)，并基于角两点相关函数测量与暗物质成团理论预测比较分析类星体偏置(bias)。

Result: 基于混合深度学习框架Hybrid-z更新了KiDS DR4类星体（quasar）的光度红移（photometric redshifts），在测试样本上实现了平均偏差$\langle δz \rangle < 0.01$和离散度$\sim 0.04(1 + z)$。
...
**TL;DR**: 研究了原初黑洞(PBHs)蒸发速率受记忆负担(memory burden)抑制时对周围等离子体热斑(hot spots)形态的影响，发现标准记忆负担情景下热斑温度显著降低，但其他情景可能产生形态迥异且规模可观的热斑。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原初黑洞（PBHs）蒸发过程中能量沉积对周围等离子体温度梯度（热斑）形态的影响，特别关注记忆负担（memory burden）对蒸发速率的抑制效应及其热力学后果。

Data: 数据来源于对原初黑洞（PBHs）蒸发过程中周围等离子体能量沉积的模拟（simulation）分析。

Method: 采用转移函数(transfer functions)形式纳入蒸发速率抑制效应，推导了热斑核心温度与半径的普适公式。

Result: 在记忆负担（memory burden）抑制蒸发速率的情况下，热斑（hot spot）核心温度显著降低，但不同情景可能形成形态迥异且尺寸可观的热斑。
...
**TL;DR**: 研究了含各向异性的均匀宇宙模型中两个超流体（shift-symmetric scalar fields）的相对运动，发现其主轴会相对于宇宙共动框架旋转，且旋转能量密度不衰减而影响整体状态方程，表明旋转是该模型不可忽略的关键特征。


<details>
  <summary>Details</summary>

Background: 研究探讨了具有平移对称标量场（或超流体）相对运动的均匀宇宙学模型，重点关注各向异性导致宇宙膨胀主轴相对于共动坐标系旋转的现象。

Data: 研究中使用的数据来源于对2+1维空间中两个超流体（superfluids）在相对运动下的均匀宇宙学模型的理论模拟（simulation）。

Method: 采用相对运动的位移对称标量场（shift-symmetric scalar fields）或超流体（superfluids）构建的2+1维双超流体模型，通过动量约束强制空间梯度共线并分析动力学对齐机制。

Result: 在存在各向异性的情况下，宇宙通常表现出旋转特征，即各向异性膨胀的主轴相对于宇宙共动坐标系发生旋转。
...
**TL;DR**: 提出了一类简化宇宙对撞机（cosmological collider）模板，通过正交化处理使其与单场暴胀预测解耦，在普朗克CMB数据中发现了自旋0粒子2.4σ信号，为探测原初非高斯性（PNG）和排除单场暴胀提供了新途径。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索宇宙暴胀时期原初非高斯性（primordial non-Gaussianities, PNG）的理论动机与观测挑战，特别关注宇宙对撞机（cosmological collider）模型中重粒子产生的新型非高斯信号及其对单场暴胀理论的检验潜力。

Data: 数据来源为普朗克卫星(Planck)的宇宙微波背景(CMB)观测数据。

Method: 采用模态管线（Modal pipeline）和正交化处理技术分析宇宙对撞机（cosmological collider）模板，通过系统参数扫描评估非高斯性（PNG）信号。

Result: 通过模态分析（Modal pipeline）和普朗克（Planck）CMB数据对碰撞机模板（collider templates）进行系统参数扫描，最显著的结果在考虑多重检验效应后达到2.4σ（自旋0），自旋1和自旋2的峰值结果接近2σ。
...
**TL;DR**: 该论文核心贡献是提出通过研究行星整体化学组成（bulk planetary chemistry）及其形成条件来理解行星宜居性（habitability），强调需将太阳系和系外行星系统作为整体来探究影响宜居性的关键因素。


<details>
  <summary>Details</summary>

Background: 研究主题为探索行星化学组成（bulk planetary chemistry）与宜居性（habitability）的关系，强调通过研究太阳系和系外行星系统的形成条件来理解生命存在的关键因素。

Data: 研究中使用的数据来源包括火星探测任务（如Viking着陆器）的观测数据、系外行星系统的天文观测数据（如大气成分、轨道参数）以及行星形成过程的理论模拟（simulation）数据。

Method: 采用行星整体化学（bulk planetary chemistry）分析来研究行星宜居性（habitability）的形成条件。

Result: astrobiology（天体生物学）的研究范围已从火星土壤生物标志物检测扩展至星系尺度宜居行星和生命前化学环境的识别。
...
**TL;DR**: 这篇论文通过开发测试零膨胀玻璃干涉仪(ZIFO)光学演示器，验证了LISA(激光干涉空间天线)干涉核心测试装置的关键技术，在1 mHz至1 Hz频段实现了10 pm/√Hz的光路长度稳定性指标，并识别了相位计和光束倾斜相关的主要噪声源。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于验证激光干涉仪空间天线(LISA)任务中关键干涉测量技术的可行性，特别是针对引力波探测所需的皮米级光学路径长度稳定性。

Data: 数据来源于激光干涉仪空间天线(LISA)任务的光学演示器Zerodur干涉仪(ZIFO)的开发和测试结果，包括光路长度稳定性测量及噪声源分析。

Method: 采用零膨胀玻璃（Zerodur）干涉仪（ZIFO）作为光学演示器，通过光程稳定性测量验证了在1 mHz至1 Hz频段内将台面噪声降低至10 pm/√Hz规格的技术可行性，并识别了相位计和光束倾斜相关路径长度噪声等主要噪声源。

Result: Zerodur干涉仪(ZIFO)的光学演示验证了在1 mHz至1 Hz频段内将台架噪声降低至10 pm/√Hz规格要求的能力，并识别了相位计和光束倾斜相关路径长度噪声的主要来源。
...
**TL;DR**: 开发了开源Python框架CLAWDIA，整合稀疏字典学习（SDL）方法实现引力波形态的物理可解释降噪和分类，并在低信噪比下验证了其对双中子星事件GW170817信号降噪和LIGO第三观测期仪器干扰分类的有效性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于引力波数据分析中深度学习方法的局限性及其替代方案稀疏字典学习（SDL）的应用与优化。

Data: 数据来源于真实探测器噪声条件下的实测（LIGO第三观测期仪器干扰信号）和模拟（GWADAMA工具箱生成的干涉仪数据）引力波数据。

Method: 采用稀疏字典学习（SDL）方法，结合LASSO正则化稀疏编码和低秩共享字典学习分类器，实现引力波信号的去噪和分类。

Result: CLAWDIA（Comprehensive Library for the Analysis of Waves via Dictionary-based Algorithms）是一个基于稀疏字典学习（SDL）的开源Python框架，能有效实现引力波形态去噪和分类，并在低信噪比条件下表现出鲁棒性。
...
**TL;DR**: 提出了一种基于多元高斯建模的灵活数据驱动方法GSPICE（GausSian PIxelwise Conditional Estimator），通过条件估计单个像素的期望值和方差来检测并校正LAMOST巡天390万条恒星光谱中的宇宙射线异常值和系统性波长校准误差，同时精确量化了像素级测量不确定性。


<details>
  <summary>Details</summary>

Background: 光谱数据（spectral data）降噪流程面临多种挑战，需在保持模型无关性的同时处理宇宙射线掩蔽、波长校准和背景噪声估计等问题。

Data: 数据来源于LAMOST（Large Sky Area Multi-Object Fiber Spectroscopic Telescope）巡天项目中的390万条恒星光谱。

Method: 采用高斯像素条件估计器(GSPICE)对光谱集合进行多元高斯建模，条件估计各像素的期望值和方差，并通过比较观测值与估计值识别异常值。
...
Background: 研究聚焦于利用主动碎片清除（Active Debris Removal）技术维护近地轨道空间安全，重点解决空间碎片（如Ajisai卫星）旋转运动精确测量的挑战。

Data: 数据来源于2019年8月使用SuperWASP望远镜对Ajisai卫星进行的四次观测，通过从拖尾图像中提取高时间分辨率的光度测量数据。

Method: 采用马尔可夫链蒙特卡洛（MCMC）驱动的方法，通过比较模拟镜面位置图与新型反照率分布图的匹配度来确定Ajisai卫星的自旋状态。

Result: 基于条纹测光技术结合MCMC方法，成功确定了Ajisai卫星的自转轴和自转周期，验证了该方法适用于快速旋转目标的地面观测。
...
**TL;DR**: 提出基于YOLO算法的GSpyNetTreeS方法，可自动检测、分类并定位LIGO-Virgo-KAGRA探测器噪声瞬变（detector noise transients），准确识别Blip等常见故障类型并捕获其时频信息，为引力波观测提供自动化事件验证工具。


<details>
  <summary>Details</summary>

Background: 引力波探测数据中非高斯仪器伪迹（detector noise transients）的识别与表征是确保源属性无偏估计的关键挑战。

Data: 数据来源于LIGO探测器在第三次引力波观测运行（third observing run）期间采集的数据以及GSpyNetTree中包含的常见探测器故障类别（Blip、Low frequency blip、Low frequency line和Scratchy）。

Method: 采用基于You Only Look Once算法的GSpyNetTreeS扩展方法，用于自动检测、分类和定位引力波探测器噪声瞬变（detector noise transients）的时频信息。

Result: GSpyNetTreeS能自动检测、分类并定位引力波探测器噪声瞬变（detector noise transients）的频率和时间信息，准确识别常见故障类型（如Blip、Low frequency blip等），具备作为LIGO-Virgo-KAGRA观测自动事件验证工具的潜力。
...
**TL;DR**: 开发了一种基于衍射的快速低成本波束建模方法，成功应用于BICEP3望远镜远旁瓣（far-sidelobes）建模并与实测数据定性吻合，为优化仪器挡板设计和扫描策略提供了高效预测工具。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发新型衍射光束建模方法以优化下一代厘米至亚毫米级望远镜对宇宙微波背景B模（Cosmic Microwave Background B modes）等微弱信号的精确测量能力。

Data: 研究中使用的数据来源包括BICEP3望远镜的实测波束数据和在阿塔卡马沙漠托科山附近特定观测场景下的扫描数据。

Method: 开发了一种基于衍射的快速低成本波束建模方法（diffraction-based beam modeling method），用于计算远旁瓣响应。
...
**TL;DR**: 提出双边缘前挡板(double-rim forebaffle)设计，通过数值模拟证明该方案能有效抑制厘米至亚毫米波段天文观测中的杂散辐射(stray radiation)，显著提升未来观测质量。


<details>
  <summary>Details</summary>

Background: 研究聚焦于厘米至亚毫米波段天文观测中由杂散辐射（stray radiation）导致的信号退化问题。
...
**TL;DR**: 该论文在ALMA科学档案库（ASA）中首次实现了基于形态学图像相似性（morphological image similarity）的自监督对比学习搜索功能，使天文学家能够通过动态选择图像组合来精确定位形态相似的天体观测数据。


<details>
  <summary>Details</summary>

Background: 天文数据爆炸式增长背景下，亟需突破传统元数据检索局限，实现对观测内容本身的智能搜索。

Data: 数据来源于ALMA科学档案库（ALMA Science Archive, ASA）中的观测图像。

Method: 采用自监督对比学习（self-supervised contrastive learning）和深度神经网络（deep neural network）实现形态学图像相似性搜索。

Result: 在天文科学档案（ALMA Science Archive）中首次实现了基于形态学（morphological）图像相似性搜索功能，通过自监督对比学习（self-supervised contrastive learning）和深度神经网络提取与仿射变换无关（affine-transformation-independent）的源形态特征。
...
**TL;DR**: JEM-EUSO合作项目通过开发搭载超快紫外相机（UV camera）和切伦科夫相机（Cherenkov camera）的多平台探测系统（包括地基、气球载和天基任务），实现了对超高能宇宙射线（UHECRs）及其切伦科夫辐射（Cherenkov light）和荧光辐射（fluorescence light）的高分辨率观测，显著提升了对极端天体物理现象的探测能力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过国际合作探索超高能宇宙射线（UHECRs）及其相关现象的极端天体物理过程。

Data: 数据来源于地基实验（EUSO-TA）、平流层气球任务（EUSO-Balloon和EUSO-SPB1/SPB2）以及空间任务（Mini-EUSO在ISS上），并计划通过POEMMA、PBR和M-EUSO卫星任务扩展观测能力。

Method: 采用超快高灵敏度紫外相机（UV camera）结合切伦科夫相机（Cherenkov camera）的混合探测技术，通过多平台（地面、气球、空间站）策略实现荧光（fluorescence）与切伦科夫光的协同观测。

Result: 开发了超快、高灵敏度紫外相机（UV camera）和专用切伦科夫相机（Cherenkov camera），结合荧光与切伦科夫探测器实现混合探测，显著提升对超高能宇宙线（UHECRs）及中微子相互作用的荧光与切伦科夫光的详细观测能力。
...
**TL;DR**: 提出了一种用于下一代射电干涉仪（如ASKAP和SKA）的uv网格叠加（uv-grid stacking）方法，相比传统全数据处理和默认图像叠加方法，该方法在保持计算效率的同时恢复了99%的HI（中性氢）流量且避免了非物理伪影。
...
Data: 数据来源于澳大利亚平方公里阵列探路者（ASKAP）和平方公里阵列（SKA）等新一代射电干涉仪对中性氢（HI）谱线的深度观测，采用每日观测后形成的残余可见度网格（residual visibility grids）进行堆叠处理。

Method: 采用了一种替代性的uv网格叠加方法（uv-grid stacking method），在形成每日残余可见度网格（residual visibility grids）后暂停标准成像流程，将这些网格叠加并进行联合反卷积以整合多时期数据。

Result: 提出的uv-grid stacking方法在HI（中性氢）流量恢复上达到99%，与传统方法在噪声限制内高度一致，且避免了图像叠加引入的非物理伪影。
...
**TL;DR**: 该论文通过将CLEAN算法与牛顿法类比，在保留其框架的基础上引入Nesterov加速和共轭梯度正交化技术，显著提升了射电干涉测量（radio-interferometry）中图像重建的收敛速度和动态范围。


<details>
  <summary>Details</summary>

Background: 射电干涉测量（radio-interferometry）中从非完整采样的傅里叶数据重建图像的科学主题。

Data: 研究中使用的数据来源为射电干涉测量（radio-interferometry）中不完全采样的傅里叶数据（incompletely sampled Fourier data）。

Method: 采用牛顿法（Newton scheme）将CLEAN框架与凸优化加速技术（如Nesterov加速和共轭梯度正交化）结合，在主要循环（major loop）和次要循环（minor loop）中实现算法加速。

Result: 通过将CLEAN算法与牛顿法(Newton scheme)类比并引入Nesterov加速和共轭梯度正交化(Nesterov acceleration and conjugate gradient orthogonalization)，显著提升了传统框架的收敛速度和残差降低深度，且计算成本低于替换次要循环(minor loop)的改进方案。
...
**TL;DR**: 提出用于凯克I望远镜的可见光多共轭自适应光学系统KOLA，通过多激光导星和变形镜组合实现60角秒视场内34%斯特列尔比和15毫角秒分辨率（652纳米），支持黑洞影响半径解析等科学目标。


<details>
  <summary>Details</summary>

Background: 研究旨在开发用于10米凯克I望远镜的可见光多共轭自适应光学系统（KOLA），以实现光学波段的近衍射极限角分辨率。

Data: 数据来源于开源多线程自适应光学模拟器（MAOS），并利用凯克I望远镜现有自适应光学系统的实测数据进行了验证。

Method: 采用开源多线程自适应光学模拟器（MAOS）进行性能仿真，结合多激光导星（LGS）和变形镜实现60角秒视场的宽视场校正。
...
### [44] [The 59th Karl Jansky Lecture: Discovering the Radio Universe](https://arxiv.org/abs/2511.16779)
*K. I. Kellermann*

**TL;DR**: 该讲座回顾了射电天文学（radio astronomy）发展历程，总结了百年来的重大偶然发现，并展望了未来新发现前景。


<details>
  <summary>Details</summary>

Background: 研究聚焦于射电天文学（radio astronomy）的历史发展、重要发现及未来展望。

Data: 数据来源为NRAO（美国国家射电天文台）在夏洛茨维尔、索科罗和绿岸三地举办的Karl Jansky讲座内容。

Method: 讲座回顾了射电天文学（radio astronomy）百年来的重大偶然发现，并展望了未来新发现的前景。

Result: 讲座回顾了射电天文学（radio astronomy）的起源、从业者构成、过去一个世纪主要由偶然发现推动的重大成果，并展望了未来新发现的可能性。
...
### [45] [WIMP Meets ALP: Coherent Freeze-Out of Dark Matter](https://arxiv.org/abs/2511.16731)
*Steven Ferrante,Maxim Perelstein,Bingrong Yu*

**TL;DR**: 研究了弱相互作用大质量粒子(WIMP)与类轴子粒子(ALP)通过二次耦合的宇宙学演化，发现温度依赖的质量偏移导致新型相干冻结机制，在强耦合下WIMP延迟冻结允许更高湮灭截面，弱耦合下ALP可通过"ALP奇迹"机制贡献暗物质。


<details>
  <summary>Details</summary>

Background: 研究探讨了弱相互作用大质量粒子（WIMP）与轻轴子样粒子（ALP）通过二次耦合的宇宙学演化及其对暗物质丰度的非热产生机制的影响。

Data: 数据来源于理论模拟（simulation）研究，未使用实际观测数据。

Method: 采用核心分析方法为研究弱相互作用大质量粒子(WIMP)与类轴子(ALP)通过二次耦合的宇宙学演化，重点分析温度依赖的质量偏移对WIMP冻结和ALP错位动力学的双重影响。

Result: 弱相互作用大质量粒子(WIMP)与轴子样粒子(ALP)的二次耦合通过相干前向散射引发温度依赖的质量偏移，导致WIMP延迟冻结和ALP错位动力学改变，形成新型相干冻结机制。
...
### [46] [Growth of Cosmic Strings with Field-Dependent Tension](https://arxiv.org/abs/2511.16725)
*Luca Brunelli*

**TL;DR**: 研究发现依赖场张力的弦（包括基本超弦和由$p$-膜包裹$(p-1)$维环产生的有效弦）在完美流体宇宙及动态系统背景下，不仅能在运动学主导（kinating）环境中，还能在标度固定点实现宇宙学增长。


<details>
  <summary>Details</summary>

Background: 该研究探讨了在时变弦张力（time-varying string tension）背景下产生宇宙基本超弦（fundamental superstrings）网络的新机制，并扩展分析了弦在完美流体填充宇宙中的宇宙学增长。

Data: 研究中使用的数据来源于理论模拟（simulation），基于IIB型弦紧致化（Type IIB string compactifications）中体积模量（volume modulus）滚动的动力学系统分析。

Method: 采用动力学系统技术分析场依赖张力弦的宇宙学增长，并应用于基本超弦和从$p$-膜包裹$(p-1)$维环获得的等效弦。

Result: 在动态系统分析中发现，基本超弦（fundamental superstrings）和通过$p$-膜（$p$-branes）缠绕$(p-1)$维环得到的有效弦（effective strings）均能在动力学背景和标度固定点（scaling fixed points）中实现宇宙学增长。
...
### [47] [An Investigation of Systematic Effects from Background Priors on PSR J0740$+$6620 Radius Estimates using Synthetic NICER and XMM-Newton Data](https://arxiv.org/abs/2511.16759)
*Isiah M. Holt,M. Coleman Miller,Alexander J. Dittmann,Frederick K. Lamb*

**TL;DR**: 研究发现，在联合分析NICER和XMM-Newton的合成数据时，即使背景模型低估实际背景超过五倍，中子星（neutron star）半径后验分布的偏移仅约1σ，证实此类数据能准确测量中子星半径。


<details>
  <summary>Details</summary>

Background: 研究主题为通过X射线脉冲波形分析测量中子星（neutron star）半径以约束其核心致密物质状态方程（equation of state）。

Data: 研究中使用的数据来源为合成NICER和XMM-Newton数据，并以PSR J0740+6620（脉冲星）为原型示例。

Method: 采用贝叶斯（Bayesian）统计方法联合分析NICER和XMM-Newton的合成X射线脉冲波形数据，评估背景模型参数化对中子星半径估计的系统误差影响。

Result: 背景未建模的背景成分（unmodulated background components）参数化错误是影响中子星半径测量精度的潜在系统误差源。
...
### [48] [Deep Learning Analysis of Ions Accelerated at Shocks](https://arxiv.org/abs/2511.17363)
*Paxson Swierc,Damiano Caprioli,Luca Orusa,Miha Cernetic*

**TL;DR**: 采用深度学习技术分析混合模拟（kinetic ions--fluid electrons）中无碰撞激波加速离子，通过局部磁场时间序列高精度（>90%）预测离子注入加速过程（thermal/suprathermal/nonthermal分类），并验证自编码器重构参数能力，为构建流体亚网格模型奠定基础。
...
Data: 数据来源于混合模拟（hybrid simulation，即动力学离子-流体电子模拟）中无碰撞激波加速离子的时间序列局部磁场参数。

Method: 采用深度学习技术（deep learning）对混合模拟（hybrid simulations）中无碰撞激波（collisionless shocks）加速的离子进行分类，并测试自编码器架构（autoencoder）从编码表示重构时间序列参数。

Result: 利用深度学习技术成功预测了无碰撞激波（collisionless shocks）中离子注入加速过程的分类，准确率超过90%。
...
**TL;DR**: 该论文通过解析研究证明原始黑洞(PBHs)周围会形成具有3种不同幂律谱指数(3/4、3/2、9/4)和4种渐近态的高密度暗物质(DM)尖峰，并推导出快速计算密度分布的近似解析解，特别指出亚太阳质量原始黑洞的发现将对自湮灭暗物质的s波湮灭截面施加严格限制。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是探讨原初黑洞（PBHs）作为暗物质（DM）候选者的可能性及其与热粒子暗物质的相互作用机制。

Data: 研究中使用的数据来源为理论推导（simulation）的近似解析解，用于快速预测密度分布。

Method: 采用解析方法（analytical method）研究原初黑洞（PBHs）周围热粒子暗物质（DM）的密度分布，推导出幂律密度剖面的标度关系（scaling relations）和近似解析解，并针对自湮灭暗物质（self-annihilating DM）提出新的近似解析公式。

Result: 原初黑洞（PBHs）作为暗物质（DM）候选者时，若与热粒子共同构成DM，会在辐射纪元形成具有3种不同幂律谱指数（3/4、3/2、9/4）和4个渐近区域的致密密度尖峰。
...
### [50] [Discovering periodic and repeating nuclear transients in the XMM-Newton archives](https://arxiv.org/abs/2511.17385)
*Natalie A. Webb,Vincent Foustoul,Robbie Webbe,Matteo Bachetti,Erwan Quintin,Laurent Michel*

**TL;DR**: 该论文通过分析XMM-牛顿（XMM-Newton）X射线目录数据，利用创新方法识别出潮汐瓦解事件（TDEs）、准周期性喷发源和大质量黑洞双星等变源，并开发了STONKS流水线和实时警报系统以追踪新暂现源。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用X射线观测数据探索超大质量黑洞（supermassive black hole）周围区域的时变现象及其潜在物理机制。

Data: 数据来源于XMM-牛顿（XMM-Newton）EPIC存档数据和XMM-牛顿自动处理流程中的STONKS管道（STONKS pipeline），包括实时警报系统监测的新发和衰减暂现源（transients）。

Method: 采用XMM-牛顿卫星EPIC档案数据的周期性变量搜索方法，并整合STONKS自动处理流水线及实时警报系统进行瞬变源追踪。

Result: 发现了新的超大质量黑洞双星（massive black hole binaries）和候选潮汐瓦解事件（TDEs）源，并通过STONKS流水线和实时警报系统实现了对瞬变源的快速跟踪。
...
### [51] [Distributed Acoustic Fiber Sensing for Research Campuses and Large Scientific Infrastructures -- The Hamburg WAVE proto-network](https://arxiv.org/abs/2511.17141)
...
**TL;DR**: 提出利用分布式声学传感（DAS）监测研究园区和大型科学设施的环境振动，通过原型网络实验证实其可有效识别自然、人为和基础设施振动耦合路径，提升高精度实验环境稳定性。


<details>
  <summary>Details</summary>

Background: 研究分布式声学传感（DAS）在科研园区和大型科学设施中监测环境振动并降低其对高精度实验影响的潜力与应用。

Data: 数据来源于2021年5月在汉堡科学城巴伦菲尔德（Science City Hamburg Bahrenfeld）进行的原型网络（proto-network）地震测量活动，该活动覆盖了超过12公里的光纤，涉及DESY、欧洲X射线自由电子激光装置（European XFEL）、PETRA III和汉堡大学等机构。

Method: 采用分布式声学传感（DAS）技术对研究园区和大型科学基础设施中的环境振动进行监测和分析，并通过原型网络（proto-network）对超过12公里的光纤进行地震测量，重点研究了噪声和运动耦合效应在双冗余光纤环路中的表现。

Result: 分布式声学传感（DAS）技术能有效监测研究园区和大科学设施中的环境振动，降低其对高精度实验的影响。
...
**TL;DR**: 提出了一种不依赖慢滚近似(slow-roll approximation)的数值方法，通过分解快慢模式分量并引入林德布拉德方程(Lindblad equation)环境修正，高效计算原初标量扰动(primordial scalar perturbations)在受控态变形下的演化及其对再加热过程的影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过分解模式自由度（mode degrees of freedom）为快慢分量计算两点关联函数（two-point correlators），并推广至任意自由度数量，以探索原初标量扰动（primordial scalar perturbations）在Lindblad方程诱导的最简环境修正下的受控态变形演化。

Data: 数据来源于数值模拟（simulation）方法，通过分解模式自由度（mode degrees of freedom）为快慢分量并基于Lindblad方程的最简环境修正来模拟原初标量扰动（primordial scalar perturbations）的演化。

Method: 采用基于林德布拉德方程(Lindblad equation)的数值方法，通过将模式自由度分解为快慢分量来计算两点关联函数(two-point correlators)，并支持任意自由度数量和可配置的退相干(decoherence)事件序列。

Result: 提出了一种数值方法，可计算任意自由度下原初标量扰动（primordial scalar perturbations）在Lindblad方程诱导环境修正中的演化，无需依赖慢滚近似（slow-roll approximation）。
...
**TL;DR**: 该论文通过将相对分箱法（relative binning）应用于TIGER框架，显著降低了引力波（GWs）广义相对论（GR）参数化测试的计算成本，在保持后验准确性的同时将计算时间缩短了10至100倍，并成功验证了该方法在现有和下一代（XG）引力波探测器中对GR一致性检验和非GR偏差恢复的有效性。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用引力波（GWs）对广义相对论（GR）进行参数化检验时，因波形模型中引入额外偏离参数导致计算成本激增的问题，尤其在下一代（XG）探测器更高信噪比（SNRs）和更长信号的挑战下，亟需高效方法以支持系统性研究。

Data: 研究中使用的数据来源为模拟的双黑洞引力波信号（simulated binary black hole signals）以及实际观测事件GW150914和GW250114的引力波数据。

Method: 采用相对分箱法（relative binning）对参数化广义相对论检验框架TIGER进行优化，通过自适应选择频率分箱替代密集频率计算，在保持后验精度前提下显著降低每次似然调用的计算成本。

Result: 通过将相对分箱法（relative binning）应用于TIGER框架，在保持后验精度的前提下显著降低了引力波（GW）广义相对论（GR）参数化测试的计算成本，使单参数和多参数分析能在一天内完成，且偏差约束与GR在90%置信度下一致。
...
**TL;DR**: 研究发现相对论宇宙学中非线性不均匀性形成的反作用（backreaction）和动态曲率通过引入等效完美流体压力影响平均宇宙学动力学，并推导了该系统中线性扰动的演化方程，强调忽略这些效应会导致大尺度结构增长的预测偏差。
...
Data: 数据来源为文献中已有的四个宇宙学平均模型（averaged cosmological models）的模拟数据（simulation data）。

Method: 采用协变空间平均（covariant spatial averaging）结合协变且规范不变（covariant and gauge-invariant）的微扰理论，并引入由尘埃密度与涌现有效流体共同驱动的有效背景，推导该系统的线性扰动演化方程。

Result: 非线性不均匀性形成引起的反作用（backreaction）和动态曲率效应显著影响晚期宇宙膨胀及大尺度结构的线性增长。
...
**TL;DR**: 研究比较了含L形和三角形爱因斯坦望远镜(ET)的第三代引力波探测器网络对宇称破缺引力波背景(GWB)的探测能力，发现L形ET设计的网络具有更强的宇称破缺约束能力。


<details>
  <summary>Details</summary>

Background: 研究探讨了利用第三代地基探测器网络（包括爱因斯坦望远镜（ET）和两台宇宙探索者（CE））探测宇称破坏引力波背景（GWB）的前景。

Data: 数据来源于对爱因斯坦望远镜（Einstein Telescope, ET）和两台宇宙探索者（Cosmic Explorer, CE）探测器组成的网络进行参数化模拟（simulation），包括ET的不同设计构型、探测器朝向和臂长变化。

Method: 采用爱因斯坦望远镜（ET）和宇宙探索者（CE）探测器网络，通过改变ET设计（L形与三角形）、探测器方位和臂长，评估几何结构与尺度对探测能力的定量影响。

Result: L形爱因斯坦望远镜(ET)设计比三角形ET设计对宇称破坏引力波背景(GWB)的探测约束能力更强。
