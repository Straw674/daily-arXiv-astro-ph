...
**TL;DR**: 通过磁流体动力学模拟结合CREST框架的自洽宇宙线电子演化，发现仅依靠对流输运无法解释观测到的侧向星系垂直射电强度轮廓，且轫致辐射和库仑损失即使损失率不占主导也必须纳入考虑。


<details>
  <summary>Details</summary>

Background: 该研究探讨了利用射电连续谱观测研究恒星形成星系中宇宙线电子和高能粒子输运过程时，常用简化假设的有效性。

Data: 数据来源为基于孤立银河系质量星系的磁流体动力学模拟，并使用CREST框架沿拉格朗日示踪粒子自洽演化包括超新星遗迹注入、随气体平流以及时空变化辐射损失的宇宙线电子。

Method: 核心分析方法为基于磁流体动力学模拟中随时间演化的宇宙线电子谱，采用CREST框架沿拉格朗日示踪粒子自洽演化电子注入、平流输运及辐射损失。

Result: 仅基于平流输运（advection）的模型无法重现观测到的盘外射电强度分布，除非采用不切实际的高速星系风，因此额外的输运或再加速过程是必需的。
...
**TL;DR**: 利用XMM-XXL和Stripe 82X巡天中的宽线X射线活动星系核（AGN）以及VIPERS和SDSS/Stripe 82中的星系，通过构建匹配的星系对照样本和交叉相关函数分析，发现较低黑洞质量（8.0 ≤ log(M_BH/M_⊙) < 8.5）的AGN与对照星系的暗物质晕质量统计上不可区分，而较高黑洞质量（8.5 ≤ log(M_BH/M_⊙) < 9.0）的AGN可能居住在略大（约0.4 dex）的晕中，表明晕尺度过程主要在高黑洞质量端变得重要。


<details>
  <summary>Details</summary>

Background: 探究超大质量黑洞（SMBHs）与其寄主星系及大尺度暗物质晕之间的关联。

Data: 使用了XMM-XXL和Stripe 82X巡天中的宽发射线X射线活动星系核（AGN），以及VIPERS和SDSS/Stripe 82巡天中的星系数据。

Method: 运用多元最近邻匹配法构建对照星系样本，并通过AGN-星系互相关函数推断特征暗物质晕质量。

Result: AGN较低黑洞质量区间的暗物质晕质量与匹配的非AGN星系不可区分，而在较高黑洞质量区间，AGN可能存在于质量略大（约0.4 dex）的晕中，表明晕尺度过程主要在高黑洞质量时变得重要。
...
**TL;DR**: 基于DESI三年观测数据构建的608颗成员星最大同质光谱样本，通过高斯混合模型将GD-1星流分解为冷薄组分和动力学热晕，发现后者约30%成员的11 Gyr冷暗物质次晕加热信号及沿流方向的大自行弥散。


<details>
  <summary>Details</summary>

Background: 研究利用银河系中最长、最冷的恒星流之一GD-1作为探针，通过动力学加热效应来约束暗物质（dark matter）亚结构。

Data: 数据来源为暗能量光谱仪器（Dark Energy Spectroscopic Instrument, DESI）前三年观测中，通过光谱确认的608颗GD-1星流成员星。

Method: 采用高斯混合模型将流分解为动力学冷的薄成分和动力学热的包层成分，并测量其速度弥散、角宽度及自行的致密性结构。

Result: 基于DESI前三年观测的608颗光谱确认成员星，GD-1星流可分解为一个动力学冷薄组分（速度弥散σ_V = 2.49±0.28 km s⁻¹，宽度0.23±0.01°）和一个动力学热茧层（σ_V = 6.13±0.75 km s⁻¹，宽度2.18±0.17°），其中茧层包含约30%的成员星，其速度弥散与暗物质次晕（cold dark matter subhalos）约110亿年的加热预期一致。
...
**TL;DR**: 通过对比半解析模型与解析模型，发现只有原初黑洞（PBHs）以及以超爱丁顿速率吸积的重种子天体模型能同时解释詹姆斯·韦伯空间望远镜在红移5至10观测到的超大质量黑洞质量、宿主星系恒星质量及极低金属丰度，而原初黑洞在黑洞质量-恒星质量比随暗晕质量增加而下降这一特征为区分其起源提供了关键鉴别依据。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于早期宇宙中质量超标的超大质量黑洞（obese black holes）起源之谜，旨在区分天体物理种子黑洞与原始黑洞（PBHs）两种形成路径。

Data: 数据来源于DELPHI半解析模型模拟的天体种子黑洞和PHANES解析模型模拟的宇宙学种子原初黑洞（primordial black holes, PBHs）。

Method: 采用DELPHI半解析模型模拟天体物理种子黑洞，以及PHANES解析模型模拟宇宙学起源的原初黑洞。

Result: 只有PBH和爱丁顿极限吸积下的重种子模型能同时重现观测到的黑洞质量、恒星质量及极低宿主金属丰度。
...
**TL;DR**: 基于对百万倍太阳质量双星系统黄金圆轨极端质量比旋近在湍流吸积盘中迁移的建模，发现当考虑气体湍流导致的混沌迁移时，之前被视为不可观测的气体诱导引力波相位偏移可能变得可被LISA探测器检测到。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是在湍流气体盘环境中，引力波探测器LISA观测到的极端质量比旋近系统的气体诱导引力波相位偏移的可探测性问题。

Data: 数据来源于一项全局流体动力学（HD）研究，并基于该研究对湍流转矩（$T_{\rm turb}$）的建模，以及LISA探测器对特定参数空间下的近并合极端质量比旋进（EMRI）系统的模拟。

Method: 将湍流力矩建模为以线性力矩为中心的随机高斯分布，以计算极端质量比旋近系统的气体诱导引力波相位偏移。

Result: 对于具有高爱丁顿比率、强湍流、厚盘和高粘度的极端质量比旋近系统，仅考虑线性气体力矩时气体导致的引力波相位偏移不可观测，但若计入湍流力矩则可能变得可探测。
...
**TL;DR**: 基于JWST/NIRSpec光谱数据，在红移3.71和4.01处确认了由大质量宁静星系（quiescent galaxies, QGs）构成的过密区域，发现其星系数密度是平均场的三倍，且大多数QGs具有一致的形成和熄灭时标，表明大尺度环境通过增强并合和气体吸积触发活动星系核（AGN）反馈，从而同步驱动星系熄灭。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用JWST/NIRSpec光谱证认早期宇宙中大质量宁静星系（QGs）的过密区域，以探究大规模环境对星系形成与熄灭的作用。

Data: 基于DeepDive NIRSpec项目的观测数据和Dawn JWST Archive的存档数据，以及用于重现超密度的Illustris TNG300模拟数据。

Method: 使用SED拟合分析星系恒星形成历史，结合星系数密度分析、JWST/NIRSpec光谱及Illustris TNG300模拟。

Result: 利用JWST/NIRSpec在红移3.71和4.01处确认了三个和两个大质量宁静星系（QGs）存在于星系数密度为平均场三倍的超密区域，并发现这些QGs具有相似的形成与熄灭时标，宽Hα发射线与弱发射线被识别为活跃星系核（AGN）驱动，而Illustris TNG300模拟支持大尺度结构通过增强并合或气体吸积触发AGN反馈共同驱动熄灭。
...
**TL;DR**: 评估了CSST通过Segue 1中宽距双星的两点相关函数探测暗物质晕尖核结构的能力，发现需恒星样本量Nstar≳6000且双星比例fb≳0.1才能在约40kpc内区分核与尖点轮廓。


<details>
  <summary>Details</summary>

Background: 暗物质晕在亚星系尺度上的内部结构仍是核心-尖点问题（core-cusp problem）中的一个关键未解难题。

Data: 数据来源包括基于已有N体模拟生成的模拟双星种群，以及结合中国空间站望远镜（CSST）预期深场极限星等（g波段约27.5等）和约90%测光完备度的真实观测条件。

Method: 采用两点相关函数（2PCF）作为统计工具，基于现有N体模拟生成双星种群，并融入CSST深场极限星等（g ~ 27.5 mag）及约90%测光完备性的实际观测条件。

Result: CSST在恒星样本量N_star ≳ 2300时能以3σ置信度检测到宽双星（wide binary）比例低至f_b ≳ 0.01，但区分尖点状（cusped）与核心状（cored）暗物质晕轮廓需要N_star ≳ 6000且f_b ≳ 0.1。
...
**TL;DR**: 通过局部动力学摩擦近似，研究低质量高偏心点质量吸积体在含有密度环的等温盘中的轨道演化，发现顺行吸积体轨道会逐渐圆化并使其半长轴收敛于环半径，从而形成叠加在气体环上的种群环，而逆行吸积体则始终向内迁移。


<details>
  <summary>Details</summary>

Background: 行星形成和活动星系核（AGN）吸积盘中的长寿命过密环状结构，例如死区外缘气体堆积或物质内落所引发的致密环与弧，是理解盘内天体轨道演化和物质聚集的关键背景。

Data: 数据来源为基于局域近似的动力摩擦（dynamical friction）模拟，使用了等温盘（isothermal disc）模型，并在盘中预设了一个密度环（density ring）。

Method: 采用局部动力学摩擦近似，研究了嵌入含密度环的等温盘中的低质量高偏心率点质量扰动体（perturber）的轨道演化。

Result: 对于低质量高偏心率的顺行扰动体，其穿越行星盘密度环的轨道会逐渐圆化，半长轴向环半径收敛，使环成为这些偏心轨道的迁移陷阱。
...
**TL;DR**: 利用JWST/NIRSPEC对红移7.2-9.3的三个星系进行紫外光谱观测，发现在宇宙再电离中点之前，星系周围已存在具有中性、低电离和高电离离子相的多相富金属气体，且其速度结构表现为大范围蓝移偏移，表明早期恒星世代已快速产生金属并驱动了气体运动学扰动。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是宇宙再电离时期前后星系周围金属增丰气体的存在与动力学状态，以及早期恒星形成如何快速驱动化学元素增丰和气体再分布。

Data: JWST/NIRSpec rest-frame ultraviolet spectroscopy of three galaxies at redshifts z=7.2–9.3.

Method: 使用JWST/NIRSpec的静止系紫外光谱，通过探测中性、低电离和高电离离子（如O I、Si II、C II、Si IV和C IV）的吸收线特征及速度偏移，分析金属富集气体的多相离子共存与速度结构。

Result: 通过JWST/NIRSpec光谱在红移7.2-9.3的三个星系中探测到中性、低电离和高电离金属吸收线，表明富金属气体在宇宙再电离中点之前就已存在于星系周围并具有动力学扰动。
...
**TL;DR**: 通过对21个X射线选择的射电宁静活动星系核（Radio Quiet AGN）样本的多波段射电观测，发现80%的源存在外流（outflow）信号，且拥有超快外流（Ultra-Fast Outflow, UFO）的源倾向于具有更大射电延展度和陡峭射电谱，其射电发射与风驱动激波模型预测一致，暗示了两者可能存在直接联系。


<details>
  <summary>Details</summary>

Background: 该研究旨在探究射电宁静活动星系核中射电辐射的多重起源机制及其与超快外流的潜在联系，以量化活动星系核反馈在星系演化中的作用。

Data: 数据来源包括使用Jansky Very Large Array（JVLA）在1.5和6 GHz获得的专有观测数据，以及来自LoTSS和其他公开射电巡天在150至1400 MHz波段的图像。

Method: 采用多波段联合分析方法，结合自有JVLA 1.5和6 GHz数据与LoTSS等公开巡天数据构建射电能谱（radio SED），并综合谱指数、光度、形态、恒星形成率、X射线光度、爱丁顿比率及超快外流（Ultra-Fast Outflow, UFO）动能光度等多波段信息，以区分星暴活动、AGN驱动外流、弱喷流和日冕活动等机制。

Result: 在SUBWAYS样本中，检测到超快外流（UFO）的活动星系核（AGN）往往具有更大的射电延展度和与流出一致的陡峭射电频谱，且其射电辐射与风驱动的激波模型预测相符，暗示了这两种外流阶段之间可能存在的直接联系。
...
**TL;DR**: 提出了一种基于CUDA/HIP和自适应网格细化实现的自引力辐射流体力学代码SFUMATO#，通过引入线性化隐式非平衡化学与热求解器及伪尘埃热容加速技术，在保持精度的同时显著提升计算效率。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是恒星形成过程中自引力辐射流体动力学问题的数值模拟。

Data: 数据来源包括：采用自适应网格细化（AMR）与CUDA/HIP编程框架进行的自引力辐射流体动力学数值模拟。

Method: 采用基于线性化隐式方法的非平衡化学与热求解器，并通过牛顿-拉弗森法验证其精度。

Result: 通过将伪粉尘热容量提升至真实值的三次方量级，可在保持精度的同时加速化学求解器。
...
**TL;DR**: 通过JWST/NIRSpec对红移约8.7的两个星系（CEERS-1019和CEERS-1025）的超深紫外光谱观测，发现包含极亮恒星（VMS）的恒星种群模型能更好地拟合光谱中强风线P-Cygni轮廓和宽HeII发射线，表明高红移星系中存在丰度过量的、初始质量函数延伸至超过100太阳质量的极亮恒星，并揭示了其极年轻年龄（约150-200万年）和高电离光子产生效率（log ξ_ion > 25.8 Hz erg^-1）。


<details>
  <summary>Details</summary>

Background: 高红移星系中极端年轻恒星种群的性质及其在宇宙再电离中的贡献是本研究聚焦的核心科学主题。

Data: 使用NIRSpec/JWST对两个z~8.7的UV亮星系CEERS-1019和CEERS-1025进行了约20-30小时的极深紫外余辉光谱观测。

Method: 使用包含极亮恒星（VMS）的恒星种群合成模型与非包含VMS的模型进行比较。

Result: 对红移约8.7的两个明亮星系的高红移紫外光谱观测与包含极 massive 恒星（VMS）的恒星种群模型最为吻合，表明早期宇宙中存在丰度过高的、初始质量函数延伸至远超100太阳质量的极 massive 恒星。
...
**TL;DR**: 利用JWST的SPURS项目对红移9.3的超亮弱发射线星系进行超深紫外光谱观测，发现其具有高金属丰度（0.4-0.7倍太阳丰度）和星暴后近期下降的恒星形成历史，且其光滑的Lyα吸收断表明它位于一个约0.29 pMpc的小电离泡中，这解释了此类星系在高中性氢分数环境下弱发射线的成因。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用詹姆斯·韦伯空间望远镜（JWST）发现的红移z~9以上的超亮早型星系，探讨其极端物理性质（如高恒星质量、弱发射线、金属丰度及星暴历史）与再电离时期中性氢环境（电离泡尺度）之间的关联。

Data: 数据来源为JWST Cycle 4大型项目SPURS获得的超深（29小时）静止系紫外光谱观测。

Method: 通过分析Balmer跳变、弱发射线和恒星风特征，并结合星际吸收线揭示的中性气体覆盖分数与流出速度，利用随机星暴模型约束星族合成（stellar population synthesis）及星族年龄。

（注：为满足“只返回答案内容”的要求，且省略主语，此处直接输出分析方法描述。但需确认：是否要求严格以“研究采用的核心分析方法”为回答内容？已按此执行。）

Result: 一颗红移9.3的超亮星系（M_UV=-21.66）展现出微弱发射线、高金属丰度（0.4-0.7 Z_⊙）和近期恒星形成活动衰减（最后10 Myr内下降），其弱发射线主要归因于恒星形成历史的随机性，而非大逃逸分数，并且该星系位于一个半径约0.29 pMpc的小电离泡中，表明其周围中性氢分数较高（x_HI=0.81）。
...
**TL;DR**: 利用DESI DR1光谱和Gaia DR3天体测量数据发现了一颗名为DESI-HVS1的旧、低质量、贫金属F型超高速星（hypervelocity star, HVS）候选体，其银心总速度（523 km/s）略超本地逃逸速度，轨道反向积分显示其最近接近银心距离为0.40 kpc且仅有一次银道面穿越（概率>0.95），最可能由Hills机制解释，首次为源自银心的旧、低质量HVS候选体提供了强有力证据，表明年轻大质量银心起源HVS的明显主导地位很可能是观测选择效应所致。


<details>
  <summary>Details</summary>

Background: 该研究的科学主题是通过发现一颗老年、低质量、金属贫乏的银河系中心起源的超高速星候选体，检验Hills机制并揭示观测选择效应对超高速星样本构成的影响。

Data: DESI-HVS1的观测数据来自暗能量光谱巡天（DESI）第一轮释放（DR1）的光谱和盖亚（Gaia）第三期释放（DR3）的天体测量数据。

Method: 结合光谱数据（DESI DR1）和天体测量数据（Gaia DR3），通过向后轨道积分重建恒星轨迹，验证了其近银心轨道特征符合希尔机制（Hills mechanism）。

Result: 利用DESI和Gaia数据发现了一颗质量为0.8太阳质量、年龄约14.1 Gyr、金属丰度[Fe/H]为-1.6的老年低质量贫金属F型超高速星DESI-HVS1，其最自然的解释是来自银河系中心的Hills机制，表明观测中年轻大质量超高速星占主导很可能是选择效应所致。
...
**TL;DR**: 在人马座E恒星形成复合体子区域（中心坐标l=358.69°，b=0.03°）中识别出一个U型结构，并通过多波段数据分析发现该区域的恒星形成活动主要由棒驱动的云-云碰撞在远尘埃带与中央分子带（CMZ）界面处触发。


<details>
  <summary>Details</summary>

Background: 该研究探讨了银河系棒驱动的气体动力学过程如何通过云-云碰撞在银心区域触发大质量恒星形成活动。

Data: 使用了GLOSTAR巡天的射电连续谱数据（4-8 GHz），该数据结合了Karl G. Jansky甚大阵和埃费尔斯贝格100米望远镜的观测。

Method: 采用BLOBCAT源提取软件识别致密射电源，结合多波段关联、谱指数估计以及中红外流量比图和SiO发射谱线宽成分分析，并利用CO数据构建速度桥结构。

Result: 在棒旋星系驱动的气体-气体碰撞作用下，人马座E星云复合体中远尘埃带与中央分子区交界处的气体被压缩，触发了观测到的大质量恒星形成活动。
...
**TL;DR**: 通过跨学科研讨，系统分析了以超大质量黑洞能量驱动的大型后生物智能体（Dyson Minds）的构建、行为及观测特征，并推荐采用异常检测方法分析WISE、JWST和事件视界望远镜等存档数据以搜寻其技术特征。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是探索以超大质量黑洞能量为支撑的大型后生物智能（Dyson Minds）作为搜寻地外智慧（SETI）的新前沿，融合天体物理、工程、人工智能和哲学等多学科视角。

Data: 数据来源包括WISE、JWST和事件视界望远镜（Event Horizon Telescope）的档案数据集。

Method: 采用了面向超大质量黑洞（SMBHs）的戴森群（Dyson swarms）的热力学、力学和稳定性极限分析，并计划对WISE、JWST和事件视界望远镜（Event Horizon Telescope）的归档数据集应用异常检测方法。

Result: Dyson Minds的架构和行为细节强烈影响其可观测特征，工作坊建议对WISE、JWST和事件视界望远镜（Event Horizon Telescope）等存档数据应用异常检测方法，以发现可能被标准处理流程忽略的特殊信号源。
...
**TL;DR**: 通过筛选化学网络并计算速率系数，发现氢原子隧穿效应能显著加速星际介质中C2H、OH和CN与H2的氢提取反应，从而影响芳香族分子（如c-C6H5+）的丰度模型。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是氢原子隧穿效应对星际介质中芳香分子化学模型和气相化学反应动力学的影响。

Data: 数据来源包括kida.uva.2024化学网络以及研究团队自行扩展的化学网络，并基于新计算的势能面和RRKM分析确定速率系数。

Method: 利用新计算的势能面（potential energy surfaces）和RRKM分析计算10K温度与2×10⁴ cm⁻³密度下的速率系数，并筛选了kida.uva.2024网络及扩展网络以识别星际介质中氢原子隧穿效应显著加速的反应。

Result: 氢原子隧穿效应可显著加速星际介质中64个反应的速率，其中C₂H、OH和CN与H₂的氢抽取反应，尽管速率系数较低，但因分子氢的高丰度而具有竞争性，而NH₂的反应在星际条件下效率低下。
...
**TL;DR**: 利用符号回归从GWTC-4引力波双黑洞（BBH）种群后验推理产物中提取出四个关键种群关系的闭式解析表达式，揭示了质量比-有效自旋与红移-有效自旋关联主要由后验宽度展宽而非均值偏移驱动，并识别出10倍与35倍太阳质量主质量峰对应的条件质量比分布具有定性不同的函数形式。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是利用引力波观测揭示双黑洞（binary black hole）群体的复杂结构，并通过符号回归方法从中提取出稳健、可解析的物理规律。

Data: 使用LIGO-Virgo-KAGRA（LVK）合作的GWTC-4目录中的后验推断产品作为数据来源。

Method: 采用符号回归（symbolic regression）对GWTC-4目录的后验推断产品进行分析，以发现描述双黑洞（BBH）群体关键关系的紧凑闭式解析表达式。

Result: 通过符号回归对GWTC-4目录的后验推断产物进行分析，发现了四个关键双黑洞（BBH）群体关系的紧凑解析表达式，其中质量比-有效自旋和红移-有效自旋相关性是由后验宽度的展宽而非均值偏移驱动的，且对应于10太阳质量和35太阳质量主质量峰的条件质量比分布具有截然不同的函数形式。
...
**TL;DR**: 通过分析冷暗物质和k-本质宇宙中线性与角星系功率谱的三个核心模型（dilaton、tachyon和DBI标量场），发现相对论修正主要在角功率谱中通过视线积分显著增强，从而成为探测k-本质偏离宇宙常数且克服傅里叶空间简并性的更一致和稳健的超大尺度物理探针。


<details>
  <summary>Details</summary>

Background: 探索包含冷暗物质和k-essence动力学暗能量的晚期宇宙中，相对论效应对星系功率谱的贡献及其对暗能量模型区分的敏感性。

Data: 研究使用了基于冷暗物质和k-essence（包括dilaton、tachyon和DBI标量场三种核心模型）的晚期宇宙线性及角度星系功率谱的模拟数据，并通过强制设定与协调模型（concordance model）相同的当前宇宙学参数来隔离k-essence动力学和扰动的影响。

Method: 采用线性与角向星系功率谱分析，基于冷暗物质和k-essence标量场（凝子、快子、DBI三种模型）的微扰动力学，并通过固定相同宇宙学参数隔离其印记。

Result: 在角功率谱中，相对论效应被显著放大，从而对团块型k-essence（clustering k-essence）具有更好的敏感性，其中tachyon模型在多个多极和红移处表现出清晰的偏离。
...
**TL;DR**: 通过计算超轻矢量（自旋-1）暗物质场演化过程中产生的宇宙引力波丰度，发现在Bianchi I几何背景下标量扰动与张量扰动的混合会作为张量模式的源产生随机引力波背景，并利用修改版CLASS获得了其现今频谱。


<details>
  <summary>Details</summary>

Background: 超轻矢量（spin-1）暗物质场演化过程中产生的宇宙引力波背景。

Data: 数据来源包括修改后的CLASS代码生成的模拟结果，以及基于超轻矢量暗物质场演化的理论模型导出的扰动方程解。

Method: 采用修改后的CLASS代码求解考虑超轻矢量暗物质背景的Bianchi I几何中的摄动方程，以计算标量扰动激发张量模式产生的随机引力波背景。

Result: 超轻矢量（自旋为1）暗物质场演化产生的宇宙学引力波谱主要源于标量扰动与张量模式之间的混合，后者由均匀背景矢量场破坏空间各向同性并导致Bianchi I几何所引起。
...
**TL;DR**: 通过比较粒子视界和未来事件视界两种红外截断对线性物质扰动演化的影响，发现基于未来事件视界的Tsallis全息暗能量模型能较好拟合红移空间畸变观测数据且可媲美ΛCDM模型，而基于粒子视界的模型则无法再现观测到的结构增长。


<details>
  <summary>Details</summary>

Background: 该研究探讨了Tsallis全息暗能量模型在宇宙结构增长中的可行性，重点分析了红外截止（IR cutoff）选择对线性物质扰动演化和可观测量的影响。

Data: 观测数据来源于红移空间畸变（redshift-space distortion）的测量。

Method: 利用线性物质扰动演化分析，结合红移空间畸变观测数据比较增长因子、增长率和fσ₈(z)，检验Tsallis全息暗能量（THDE）模型中粒子视界和未来事件视界两种红外（IR）截断选择。

Result: 基于未来事件视界的模型在合适的Tsallis参数δ下与观测数据一致，且拟合效果与ΛCDM模型相当或略优，而使用粒子视界的模型通常无法重现观测到的结构增长。
...
**TL;DR**: 通过在弗里德曼-勒梅特-罗伯逊-沃尔克宇宙中对无质量标量场坍缩的全相对论数值模拟，发现原本在渐近平直系统中发现的离散自相似性在原始黑洞形成中幸存，并解析出原始黑洞质量标度关系中清晰的周期性对数振荡，其谷峰不对称性比渐近平直情况更显著，且不同初始数据族给出的临界指数和离散自相似周期略有差异但总体一致。


<details>
  <summary>Details</summary>

Background: 在宇宙学膨胀背景下，原初黑洞形成过程中发现质量标度关系（mass scaling relation）存在离散自相似性（discrete self-similarity）。

Data: 数据来源于在弗里德曼-勒梅特-罗伯逊-沃尔克宇宙背景下对无质量标量场坍缩的全相对论数值模拟，使用了高斯和分段有理曲率轮廓两类初始数据族。

Method: 采用全相对论性数值模拟，在Friedmann-Lemaître-Robertson-Walker宇宙背景下研究无质量标量场坍缩，并通过初始数据参数与阈值之差的绝对值低至约10^{-8}来解析临界区间，分析两种不同初始数据族（高斯型和分段有理曲率型）以确定临界指数和离散自相似性周期。

Result: 离散自相似性（DSS）在膨胀宇宙背景下原初黑洞形成过程中得以保持，并表现为质量标度关系中的对数周期振荡，其峰谷不对称性比渐进平坦情形更显著。
...
**TL;DR**: 利用N体模拟研究非均匀宇宙中的暗物质晕质量函数（HMF）偏离理论预言5-20%的现象，发现该偏离依赖于输入功率谱斜率，并揭示银河系尺度晕中哈勃常数估计存在高达5%的误差，部分解释了哈勃张力问题。


<details>
  <summary>Details</summary>

Background: 这项研究的宏观科学背景是非线性引力相互作用对星系分布、晕族丰度和本征速度的影响，以及这些效应对精确宇宙学预测、哈勃常数测量和标准宇宙学模型检验的关键作用。

Data: 数据来源为爱因斯坦-德西特宇宙学（Einstein-de Sitter cosmology）下的无标度功率谱（scale-free power spectra）的N体模拟（N-body simulations）。

Method: 采用N体模拟（N-body simulations）分析非线性引力演化，并结合无标度功率谱（scale-free power spectra）与爱因斯坦-德西特宇宙学（Einstein-de Sitter cosmology）检验暗物质晕质量函数（halo mass function）对宇宙学模型的偏离。

Result: N体模拟得到的晕质量函数与理论预测存在5-20%的系统性偏差，且该质量函数显式依赖于输入功率谱的斜率。结构形成导致的非均匀性和各向异性可在至少100 Mpc/h尺度上引起高达5%的哈勃常数估算偏差，这可能是哈勃 tension的部分成因。小尺度缺失功率会在模拟的均方根涨落和质量函数中引入误差，而准确估计模式耦合需要模拟中包含足够多的暗晕。
...
**TL;DR**: 整合了原初黑洞（PBH）播种的活动星系核（AGN）对再电离与黎明时期21厘米信号的X射线加热效应，发现其会显著削弱全球信号的吸收深度并抑制功率谱幅度，且原初黑洞质量函数的选择对信号预测至关重要。


<details>
  <summary>Details</summary>

Background: 21-cm信号作为探测高红移宇宙的重要探针，需纳入早期活动星系核（AGN）及原初黑洞（PBH）的影响，以修正传统模型中忽略该类成分的局限性。

Data: 数据来源包括基于SCRIPT（一种显式光子守恒再电离框架）的半数值模型模拟，其中扩展了宇宙黎明物理和原初黑洞（PBH）贡献，并采用了符合当前观测约束的不同PBH质量函数。

Method: 采用扩展了明确光子守恒再电离框架SCRIPT并纳入原初黑洞贡献的半数值模型，对21厘米信号的全局信号和功率谱进行计算。

Result: 通过纳入由原初黑洞（primordial black holes, PBHs）产生的早期活动星系核（active galactic nuclei, AGN）的X射线加热效应，会使21厘米信号的整体吸收深度变浅并抑制宇宙黎明时期的功率谱振幅，且原初黑洞的质量函数选择对信号形状有关键影响。
...
**TL;DR**: 引入受生态学饱和机制启发的稀疏度参数的暗能量-暗物质相互作用模型族，通过超新星、宇宙计时器、DESI DR2重子声学振荡和红移空间畸变数据联合贝叶斯分析发现，三个模型中有两个以超过95%的置信水平拒绝了稀疏度参数为零的假设，支持暗能量-暗物质相互作用中存在非零稀疏度尺度。


<details>
  <summary>Details</summary>

Background: 探索一个由稀疏度标量参数调节的暗物质与暗能量之间相互作用的唯象宇宙学模型家族，以解释宇宙晚期加速膨胀。

Data: 使用了Supernovae目录、宇宙计时器、DESI DR2的重子声学振荡测量，以及通过$f$和$f\sigma_8$观测量的红移空间畸变测量的大尺度结构增长数据。

Method: 采用哈勃归一化方法对宇宙学场方程进行相空间分析，并利用贝叶斯方法结合超新星、宇宙计时器、DESI DR2重子声学振荡及红移空间畸变数据检验非线性暗能量-暗物质相互作用模型。

Result: 引入稀疏度参数（sparseness scale）的唯象相互作用暗能量-暗物质模型能够有效避免幻象穿越（phantom crossing），且贝叶斯分析表明，对于三个模型中的两个，稀疏度参数为零在超过95%置信区间内被排除，为暗 sector 相互作用中的非零稀疏度尺度提供了观测支持。
...
**TL;DR**: 通过构建基于观测条件化的原初非高斯性偏置参数 $b_\phi$ 先验分布，并利用 CAMELS-SAM 模拟套件对暗物质晕质量函数（SMF）和恒星-暗物质晕质量关系（SHMR）进行约束，将 $b_\phi$ 的不确定性降低了 88%至 97%，从而显著提升了下一代巡天对局域型原初非高斯性参数 $f^{\rm{loc}}_{\rm{NL}}$ 的推断精度。


<details>
  <summary>Details</summary>

Background: 通过限制原初非高斯性（primordial non-Gaussianity）在星系成团性上的尺度依赖印记，并依赖于普遍质量函数假设下的偏置参数关系来打破参数简并，为下一代巡天中约束局域型原初非高斯性参数提供宏观科学背景。

Data: 研究使用CAMELS-SAM模拟套件，并辅以独立宇宙（separate Universe）模拟，测量星系形成可观测量的数据来源。

Method: 核心分析方法为利用CAMELS-SAM模拟套件结合独立宇宙模拟测量星系形成观测量和偏置参数b_φ，并通过对观测条件（如恒星质量函数和恒星-暗物质晕质量关系）进行边缘化来构建b_φ的物理驱动、观测条件化的先验分布。

Result: 构建了基于物理动机且由观测约束的b_φ先验，可将b_φ的不确定性σ_{b_φ}从0.69分别降低至0.08和0.02（通过恒星质量函数和恒星-晕质量关系分别实现88%和97%的缩减），且观测数据直接约束结果与此一致。
...
**TL;DR**: 提出对外太阳系未探索世界采用“行星探索3.0”（PE 3.0）范式，通过单一或少数具有软件定义空间系统（SDSS）的极强自适应任务，结合初步探索与基于自身数据的假设驱动科学，实现对外太阳系星体的有效勘测。


<details>
  <summary>Details</summary>

Background: 面对火星以外天体表面与地下世界探索的巨大挑战，现有通过多次渐进式任务（如火星探测的PE 2.0范式）已不可行。

Data: 该研究基于凯克空间研究所（Keck Institute for Space Studies, KISS）研讨会的成果，未使用新的观测或仿真数据，而是系统整合了现有外太阳系探测经验与软件定义空间系统（Software-Defined Space Systems, SDSSs）的技术概念。

Method: 核心分析方法是基于软件定义空间系统的行星探索3.0范式，并通过来自基克空间研究所研讨会的系统工程技术、软件定义技术、机载智能及三项任务概念进行阐述。

Result: 提出并定义了“行星探索3.0”范式，即通过单个或少数任务，利用可软件重定义的空间系统（software-defined space systems）在未探索的外太阳系世界自主执行从初步探测到假设驱动的后续科学。
...
**TL;DR**: 通过模拟月球次表层介电性质的变化，发现其显著影响LuSEE-Night天线共振特性（振幅、位置和宽度），并利用贝叶斯推断管道实现了星系前景模型与次表层介电参数的同时估算。


<details>
  <summary>Details</summary>

Background: 该研究探讨月球次表层未知介电特性对LuSEE-Night低频射电望远镜主波束和校准的影响，并提出利用贝叶斯推断联合估计银河前景模型与次表层介电参数的可行性。

Data: 模拟数据来源于对LuSEE-Night着陆点月球次表层不同介电剖面（dielectric profiles）的远场波束特性仿真。

Method: 采用贝叶斯推断（Bayesian inference）管道联合估计银河前景模型参数和着陆点月表下介电特性。

Result: 月球次表层介电性质的变化在天线共振附近对LuSEE-Night波束的幅度、位置和宽度影响最大，而前景性质的变化则影响整个观测波段。
...
**TL;DR**: 在圆形限制性三体问题中，导出了以主星为中心球坐标下的希尔曲面（Hill's surfaces）闭合形式表达式φ(r,θ)，该表达式基于一个在每个分界线两侧最多给出两个根的三次方程，并精确再现了蝌蚪形、马蹄形、花生形、洛希瓣（Roche lobes）及希尔准球体（Hill's quasi-spheres）等著名图案。


<details>
  <summary>Details</summary>

Background: 基于圆形限制性三体问题中的希尔曲面，研究推导出其封闭形式解析解以精确描绘星体引力影响区域的拓扑结构。

Data: 数据来源为圆形限制性三体问题的理论解析推导，未使用观测或模拟数据。

Method: 基于主天体为中心的球坐标系中推导出的三次方程解析解，给出希尔曲面（Hill's surfaces）的闭式表达式。

Result: 提供了一类闭环表达式，可精确生成圆形限制性三体问题中希尔曲面的所有经典形态，包括蝌蚪形、马蹄形、花生形、洛希瓣及希尔准球体。
...
**TL;DR**: 利用Proba-3卫星编队飞行的ASPIICS日冕仪的外掩体衍射模型，通过早期观测数据对模型进行微调，在视场内大部分区域将衍射光校正至低于日冕信号两个数量级的水平，并实现30%至50%的定量一致性。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是通过高精度编队飞行技术实现的大基线可见光日冕仪观测，旨在减少衍射杂散光以提升太阳日冕探测能力。

Data: ASPIICS观测数据来自2024年12月5日发射的Proba-3任务中的ASPIICS日冕仪，模拟衍射图像基于先前报道的分析-数值衍射模型生成。

Method: 比较ASPIICS观测数据与解析-数值衍射模型的模拟图像，特别是其几何特性和辐射度信号。

Result: 衍射光在大部分视场中低于日冕信号两个数量级。
...
**TL;DR**: 在SPring-8同步辐射光源上成功运行了一个240像素的过渡边缘传感器（TES）光谱仪，实现了约220像素同时工作、在6 keV下能量分辨率约4 eV（每像素每秒约1计数），并通过对标准样品的多元素同时分析和X射线吸收近边结构（XANES）测量，证实了其在高计数率下的实用性能及对弱信号和重叠谱线（如As Kα与Pb Lα2）的分辨能力，为硬X射线同步辐射设施中基于TES的高分辨率光谱学应用迈出了第一步。


<details>
  <summary>Details</summary>

Background: 该研究基于超导转变边缘传感器（TES）在同步辐射硬X射线光源上实现高能量分辨光谱学的应用。

Data: 实验数据来源于SPring-8同步辐射X射线光源，使用240像素的美国国家标准与技术研究院（NIST）过渡边缘传感器（TES）系统进行观测，其中220个像素以约1计数每秒每像素的计数率和6 keV下4 eV的能量分辨率同时运行。

Method: 采用240像素NIST过渡边缘传感器（TES）系统，同步运行220像素，在6 keV处实现4 eV能量分辨率，并基于能量分辨率与活时间分数评估高计数率容忍度，最终以约2 x 10^3 c/s全像素和5 eV能量分辨率进行X射线吸收近边结构（XANES）分析。

Result: 通过在SPring-8同步辐射光源成功运行过渡边缘传感器（TES）光谱仪并评估其性能，实现了在6 keV处4 eV的能量分辨率和约1计数每秒每像素的计数率，证明了其在多元素同时分析和荧光模式X射线吸收近边结构（XANES）分析中的卓越能力，能够清晰分辨之前难以区分的邻近发射线（如As Kα和Pb Lα2）并检测到稀释样品和痕量元素中的微弱荧光谱线。
...
**TL;DR**: 利用在FIREFLY加速算法基础上构建的环减（ringdown）分析流程，首次证明该算法与时间延迟干涉测量（TDI）可观测量的兼容性和有效性，针对包含六个模态的模拟信号实现了约200倍加速，为空间背景下多模态环减分析提供了可扩展方案。


<details>
  <summary>Details</summary>

Background: 微扰黑洞的铃宕信号为研究黑洞时空、强场引力及基础物理提供了清晰窗口。

Data: 模拟数据来源于空间引力波探测器的时延干涉（time-delay interferometry, TDI）可观测量，其中包含一个具有六个准正则模式的致密双黑洞并合环降（ringdown）信号。

Method: 采用FIREFLY加速算法，结合时延干涉（TDI）可观测量，实现了包含六个准正规模（quasi-normal modes）的模拟铃宕信号分析。

Result: FIREFLY算法首次被证明与天基引力波探测器的延时干涉测量法（TDI）兼容且有效，可将包含六个模式的模拟振铃信号分析速度提升约200倍，为天基多模式振铃分析提供了可行且可扩展的途径。
...
**TL;DR**: 基于超辐射不稳定性产生的超轻玻色子云，利用相对论性微扰理论首次统一计算了自相互作用和自引力对多激发模引力波频率偏移的贡献，为下一代引力波观测提供了精确预测。


<details>
  <summary>Details</summary>

Background: 超轻玻色子（ultralight bosons）的超辐射不稳定性能够在旋转黑洞周围产生云团，其连续引力波（GW）发射是重要的观测目标。

Data: 使用Kerr黑洞时空中的相对论性微扰理论，并结合轴子场自相互作用的非线性耦合模型，同时考虑了自引力贡献，基于文献中推导的双线性形式框架进行计算。

Method: 采用基于双线性形式的相对论扰动理论计算引力波频移，并首次将其应用于轴子自相互作用效应分析，同时重新审视自引力贡献的处理。

Result: 质轻玻色子的超辐射不稳定性可在旋转黑洞周围产生云团，其连续引力波发射是前景可观的观测目标，而精确预测信号频率及其演化对探测此类连续引力波至关重要。
...
**TL;DR**: 通过分析NANOGrav 15年数据集，针对标量诱导引力波（scalar-induced gravitational waves, SIGWs）能谱中的偶极型各向异性，推导出频率依赖的脉冲星计时阵列重叠约化函数（overlap reduction functions, ORFs），并发现当前观测尚未探测到首选方向的显著证据，仅给出各向异性振幅的弱上限（g≲0.5）。


<details>
  <summary>Details</summary>

Background: 该研究探讨脉冲星计时阵列（pulsar timing array, PTA）探测到的随机引力波背景各向异性，特别是原初功率谱中偶极型统计各向异性对标量诱导引力波（scalar-induced gravitational waves, SIGWs）能谱的影响及其在观测数据中的约束。

Data: NANOGrav 15-year dataset。

Method: 采用贝叶斯分析（Bayesian analysis）对NANOGrav 15年数据集进行分析，并基于标量诱导引力波（scalar-induced gravitational waves）的能量密度谱推导出偶极和四极各向异性，同时计算了相应的脉冲星计时阵列重叠约化函数（overlap reduction functions）。

Result: 标量诱导引力波能谱中的原初偶极子会产生偶极和四极各向异性，但不会激发额外极化模式，且基于NANOGrav 15年数据的贝叶斯分析未发现显著优势方向，对各向异性振幅给出了弱上限（g≲0.5），该限制源于观测频段低于谱峰导致各向异性贡献被抑制。
...
**TL;DR**: 系统研究了具有非平凡声速的宇宙学微扰的量子信息诊断，利用归一化开放双模压缩态框架，发现非平凡声速显著抑制了约化态的纯度，增强了熵与纠缠诊断，并推迟了经典性的出现。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于早期宇宙原初扰动在非平凡声速下的量子信息表征，探讨其如何通过修改挤压参数动力学影响宇宙学扰动的纯度和纠缠结构。

Data: 研究使用了基于归一化开放双模压缩态框架的理论推导，并利用边界变量 $x = \tanh r_k$ 进行部分正则化后，仅在暴胀时期（inflationary regime）内进行的数值模拟。

Method: 采用归一化开放双模压缩态框架，并引入有界变量 x = tanh r_k 作为部分正则化手段以克服数值模拟中的多尺度刚度。

Result: 非平凡声速会显著降低约化态的纯度（增强有效混合性），同时强烈放大并调制熵和纠缠诊断信号，从而通过调节退相干过程推迟经典性的出现，并在早期宇宙的纠缠结构中留下独特的量子信息特征。
...
**TL;DR**: 通过分析无压暗物质与重子流体的六种相互作用模型，发现仅当相互作用由暗物质密度主导且能量从暗物质流向第二流体时，才会出现无需暗能量的加速缩放吸引子。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是在无需引入暗能量（dark energy）的情况下，通过物质创生机制解释宇宙晚期加速膨胀的动力学理论。

Data: 数据来源为非观测性，基于理论构建的两流体系统，其中无压暗物质发生绝热粒子创生并与一流体交换能量，采用六种广为使用的相互作用形式进行数值模拟。

Method: 核心分析方法为紧致相空间中的统一动力学分析，物理模型是包含绝热粒子创生的无压暗物质与一种正压流体的双流体系统。

Result: 在无暗能量组分的情况下，仅当相互作用由暗物质（DM）密度控制且能量从暗物质流向第二种流体时，加速标度吸引子（accelerating scaling attractors）才会出现。
...
### [37] [Turbulent infall onto class 0 disks as cause of CAI brief condensation episode in the solar system](https://arxiv.org/abs/2604.21322)
*Jiachen Zheng,Xing Wei,Hongping Deng,Wenrui Xu,Douglas N. C. Lin*

**TL;DR**: 通过数值模拟发现，在类0盘（class-0 disk）形成早期的吸积过程中，富钙铝包体（CAI）颗粒会在延伸区域（2-3天文单位）内经历升华与再凝聚，这可能是现今陨石中CAI的最后一次主要吸积事件遗迹。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是探索太阳系最古老固体物质（富钙铝包体，CAIs）在早期恒星吸积盘形成过程中的热历史与来源。

Data: 本研究的模拟初始条件取自先前对巨分子云（GMC）坍缩及年轻星团形成过程的流体动力学模拟。

Method: 通过一系列数值模拟分析原恒星盘与下落气体相互作用导致的热演化和动力学结构变迁，采用高分辨率模拟追踪外部流丝下落的动态过程。

Result: 在早期（约10万年）的星云塌缩和0级原恒星盘形成过程中，落入云团中的“前太阳系”富钙铝包体（CAI）颗粒几乎全部经历了升华和再凝结过程。
...
### [38] [MINDS: Intertwined evolution of dust and gas in large planet-forming disks. A diversity driven by halted pebble drift?](https://arxiv.org/abs/2604.21803)
...
**TL;DR**: 分析MIRI光谱揭示T Tauri星周围大质量盘（如V1094 Sco和DL Tau）内区具有高C₂H₂/H₂O通量比和二氧化硅尘埃成分，提出该特征源于外部盘冰粒漂移受阻及化学演化导致内盘挥发物C/O比接近1且丰度降低，而非富氧冰粒快速漂移留下的富碳内盘。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是探索T Tauri星周围大质量大尺度盘的内区化学与尘埃组成，以理解宽轨道行星形成和卵石漂移受阻过程对盘内挥发性元素比（如C/O比）的影响。

Data: 数据来源于MINDS项目的MIRI光谱观测，包括V1094 Sco、DL Tau和IM Lup三个T Tauri星盘的谱线数据。

Method: 基于MIRI光谱分析发现高C₂H₂/H₂O通量比与显著硅质尘埃成分关联，提出停滞卵石漂移模型作为解释，该模型需满足约95% icy颗粒在外盘被阻挡、外盘化学演化充分、气体有足够时间抵达内盘，并假设硅酸盐升华前沿在超太阳但低于1的C/O比气体中重新形成尘埃导致二氧化硅化学计量比。

Result: 盘状星周盘（protoplanetary disk）内部的高乙炔/水分子（C₂H₂/H₂O）通量比与显著的硅石（silica）尘埃成分相关，且该现象的成因不同于极低质量恒星盘中富氧砾石快速漂移导致的富碳内盘模型。
...
### [39] [Chaos Gated Tunneling Drives Molecular Reactivity in Astrophysical Environments](https://arxiv.org/abs/2604.21005)
*Saptarshi G. Dastider,K. Prashant,P. Shruti,C. Sudheesh,Jobin Cyriac*

**TL;DR**: 基于混沌诊断框架与绝热规范势（AGP）和随机矩阵理论（RMT），以木星大气中的H₃⁺和H₅⁺为模型系统，揭示了过渡态作为动力学瓶颈可抑制量子混沌并增强隧穿概率，同时引入AGP斜率定义的脆弱性指数量化特定振动模式如何重新引入混沌并抑制反应性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于巨行星电离层（ionosphere）中离子-分子反应网络的化学演化，特别是通过质子转移链驱动大气组成的超冷环境（ultracold environments）中反应速率的理论建模。

Data: 本研究中使用的数据来源于基于多参考电子结构理论、绝热规范势（Adiabatic Gauge Potentials, AGP）和随机矩阵理论（Random Matrix Theory, RMT）的数值模拟，并以H₃⁺及质子束缚团簇H₅⁺的形成为代表模型系统。

Method: 核心分析方法包括基于多参考电子结构理论、绝热规范势（Adiabatic Gauge Potentials, AGP）和随机矩阵理论（Random Matrix Theory, RMT）的混沌诊断框架，并引入基于AGP斜率的脆弱性指数。

Result: 采用绝热量规势（AGP）和随机矩阵理论（RMT）的混沌诊断框架，揭示了过渡态作为动力学瓶颈时量子混沌显著抑制，从而增强隧穿概率，并提出基于AGP斜率的脆弱性指数来量化特定振动模式对混沌和反应性的影响。
...
### [40] [Filter Design for Estimating the Stellar Metallicity of Metal-poor Stars from Gaia XP Spectra](https://arxiv.org/abs/2604.21385)
*Ruifeng Shi,Yang Huang,Kai Xiao,Chuanjie Zheng,Bowen Zhang,Hongrui Gu,Xinyi Li,Huiling Chen*

**TL;DR**: 通过优化中心波长和带宽分别为3960/80埃（巨星星）和3920/80埃（矮星星）的测光滤光片设计，从盖亚XP光谱合成测光中实现了对金属丰度[Fe/H]低至-4（巨星）和-3.3（矮星）的约1450万颗贫金属星的高精度金属丰度测量，并识别出超过一万颗[Fe/H]<-4的超贫金属红巨星候选体。


<details>
  <summary>Details</summary>

Background: 该研究的科学主题是利用优化的测光滤光片设计，从大样本恒星中精确测量贫金属星（metal-poor stars）的金属丰度，以服务于银河考古学。

Data: 研究使用盖亚（Gaia）探测器的XP光谱生成合成测光（synthetic photometry）作为数据来源。

Method: 采用优化的窄带测光滤光片设计（中心波长λ_c=3960埃/带宽Δλ=80埃用于巨星，λ_c=3920埃/带宽Δλ=80埃用于矮星），并应用于来自Gaia XP光谱的合成测光数据，通过光度测光方法推断恒星金属丰度（[Fe/H]）。

Result: 优化的测光滤光片配置对[Fe/H]<-1的极贫金属星实现了高精度金属丰度测量，巨星的精度可达[Fe/H]约-4，矮星约-3.3，并发布了约1450万颗贫金属星的星表及超万颗超贫金属红巨星候选体。
...
### [41] [Mitigating Systematic Errors in Parameter Estimation of Binary Black Hole Mergers in O1-O3 LIGO-Virgo Data](https://arxiv.org/abs/2604.21859)
*Sumit Kumar,Max Melching,Frank Ohme,Harsh Narola,Tom Dooney,Chris Van Den Broeck*

**TL;DR**: 通过数据驱动的参数化波形模型引入相位和振幅不确定性先验，有效减少了引力波事件参数估计中的系统误差，并使得不同波形模型对GW191109_010717的反向自旋和GW200129_065458的非零进动参数χp推断结果趋于一致。


<details>
  <summary>Details</summary>

Background: 引力波（gravitational wave）并合事件的参数估计（parameter estimation）中存在的系统误差会导致不同波形模型间推算结果不一致，并可能源于波形系统学、噪声误表征及数据伪影等多种因素。

Data: 数据来源于LIGO-Virgo-KAGRA（LVK）合作组织前三次观测运行中选取的、已在多项研究中被标记为可能受系统误差影响的事件。

Method: 采用基于数据驱动（data-driven）的贝叶斯参数估计方法，在引力波波形中引入描述相位和振幅不确定性的参数化模型，并对其施加宽先验（broad priors）以吸收系统误差。

Result: 提出的数据驱动方法有效降低了引力波参数估计中的系统误差，并使不同波形模型之间原本不一致的推断结果（如GW191109_010717和GW200129_065458的进动参数）变得一致。
...
### [42] [Geometry, Not Calorimetry, Drives the Radio/Infrared/Gamma-Ray Correlation](https://arxiv.org/abs/2604.21224)
*Troy A. Porter,Igor V. Moskalenko,Gudlaugur Johannesson*

**TL;DR**: 通过几何投影效应而非局域宇宙线量热学，解释了恒星形成星系中观测到的射电-红外-伽马射线关联，并指出该关联的离散度可诊断星系结构和视角。


<details>
  <summary>Details</summary>

Background: 该研究的宏观科学背景是天体物理学中星系尺度上射电、红外与伽马射线波段之间观测到的关联性是否反映了局域宇宙线能量损失过程，还是主要由观测几何效应主导。

Data: GALPROP框架生成的银河系三维模型网格，其参数包括宇宙射线源、气体、星际辐射场和磁场分布，并均以重现局部宇宙射线数据为归一化标准。

Method: 采用GALPROP框架生成基于三维银河系模型网格的合成观测，通过改变宇宙线源、气体、星际辐射和磁场分布（均以本地宇宙线数据归一化），并利用视线积分方法分析几何投影效应。

Result: 射电-红外-伽马射线相关性主要源于视线积分穿过延展的、具有径向结构的盘面所产生的几何投影效应，而非局部宇宙线量热计（calorimetry）的物理特征。
...
### [43] [Gauging the Impact of Cosmic Ray Feedback on the Stellar Initial Mass Function](https://arxiv.org/abs/2604.21754)
*Margot Fitz Axen,Stella S. R. Offner,Philip F. Hopkins,Michael Y. Grudić*

**TL;DR**: 通过模拟包含宇宙线输运的20000倍太阳质量分子云塌缩和恒星形成，发现宇宙线输运显著增强了恒星形成效率（最高增加43%）并导致初始质量函数（IMF）在高质量端变平（斜率变缓约20%），尤其在包含恒星风加速宇宙线的情况下。


<details>
  <summary>Details</summary>

Background: 该研究探讨宇宙线（cosmic rays, CRs）驱动分子云电离并影响气体动力学，进而对恒星形成过程及初始质量函数（initial mass function, IMF）产生的潜在作用。

Data: 数据来源为对20000 M⊙分子云坍缩和后续恒星形成过程的数值模拟，包含有和无年轻大质量恒星风加速宇宙线的宇宙线输运（CRT）模拟，并与无CRT模拟进行对比。

Method: 采用磁流体动力学（MHD）数值模拟，结合宇宙线输运（CRT）模型，并包含由大质量恒星风加速的宇宙线源。

Result: 宇宙线传输模拟显著提升了分子云中的恒星形成效率，使包含恒星风宇宙线的模型相比无宇宙线传输模型高出43%，并导致初始质量函数（IMF）在高质量端变平约20%。
...
### [44] [Neutron Portal and Dark Matter-Baryon Coincidence: from UV Completion to Phenomenology](https://arxiv.org/abs/2604.21168)
*Sudhakantha Girmohanta,Yuichiro Nakai,Yoshihiro Shigekami,Zhihao Zhang*

**TL;DR**: 提出基于中子门户算子（neutron portal operator）的动力学方案，通过一个强过冷暗禁闭相变（dark confinement phase transition）同时解释暗物质-重子巧合问题（dark matter-baryon coincidence problem）和脉冲星计时阵列探测到的纳赫兹随机引力波信号，并自然地将GeV量级的不对称暗物质质量与多TeV截断尺度关联起来。


<details>
  <summary>Details</summary>

Background: 基于中子门户算子（neutron portal operator）连接可见和暗物质扇区不对称性，提出了一种动力学解决暗物质-重子巧合问题（dark matter-baryon coincidence problem）的方案，该框架由强过冷暗色禁闭相变（dark confinement phase transition）产生脉冲星计时阵列观测到的纳赫兹随机引力波信号所驱动。

Data: 数据来源为理论计算与解析推导，未涉及直接观测或数值模拟数据。

Method: 采用中子入口算符（neutron portal operator）连接可见扇区与暗扇区的不对称性，并基于强过冷暗色闭禁闭相变（strongly supercooled dark confinement phase transition）进行动力学分析。

Result: 基于中子门户算子的动力学机制解释了暗物质-重子巧合问题，并将GeV量级的非对称暗物质质量与多TeV截断尺度通过紫外完备自然关联起来。
...
### [45] [Constraining dark matter self-interaction from kinetic heating in neutron stars](https://arxiv.org/abs/2604.21652)
*Sambo Sarkar*

**TL;DR**: 核心贡献是研究了光学薄极限下中子星对暗物质自相互作用的捕获与热化效应，发现观测表面温度约为1000-1200 K的冷中子星可将非对称暗物质自相互作用截面的约束提升至比子弹星系团严格两个数量级。


<details>
  <summary>Details</summary>

Background: 暗物质搜索策略正朝着中微子雾区推进，并利用中子星等致密天体探测暗物质-核子低散射截面，其中暗物质自相互作用在光学薄极限下影响俘获与热化过程，可被詹姆斯·韦布空间望远镜及未来大型望远镜观测。

Data: 使用了中子星作为暗物质探测器，以及詹姆斯·韦伯空间望远镜、三十米望远镜和欧洲极大望远镜作为潜在观测数据来源。

Method: 在光学薄极限下，采用暗物质自相互作用的捕获与热化效应分析其对中子星温度的影响，并结合詹姆斯·韦布空间望远镜及未来三十米望远镜和欧洲极大望远镜的观测进行约束。

Result: 在光学薄极限下，暗物质自相互作用可促进中子星内部暗物质的俘获与热化，导致表面温度升至约1000-1200 K，从而将非对称暗物质自相互作用截面的约束推至比子弹星系团严格约两个数量级。
