...
### [1] [Resolving dust and Lyα emission in a lensed galaxy at the epoch of reionization with JWST/CANUCS](https://arxiv.org/abs/2512.13778)
...
**TL;DR**: 研究发现引力透镜（gravitational lensing）放大下的莱曼α发射体（LAE）HCM 6A（红移z=6.5676）通过反馈驱动的多相星际介质（ISM）结构和尘埃清除中心区域促进了莱曼α（Lyα）逃逸。


<details>
  <summary>Details</summary>

Background: 研究主题聚焦于再电离时期(EoR)尘埃或气体丰富星系中莱曼α(Lyα)辐射逃逸的物理机制，特别关注星际介质(ISM)多相结构与反馈驱动外流对Lyα传输的影响。

Data: 数据来源包括JWST/NIRISS无缝光谱、HST+JWST/NIRCam成像以及来自CANUCS项目的JWST/NIRSpec狭缝光谱。

Method: 采用定制化BAGPIPES光谱能量分布拟合框架结合灵活消光律，在强透镜放大（μ≈8.3-9.1）支持下实现了星系整体（∼kpc）、狭缝尺度（∼0.1 kpc）和像素级（∼25 pc）的恒星-电离气体-尘埃性质多尺度分析。

Result: Ly$α$辐射主要来自最年轻区域（S3），该区域存在一个尘埃清除的中心团块，促进了Ly$α$的高效逃逸。
...
### [2] [Confirming membership in Local Group galaxies with the Dark Energy Spectroscopic Instrument Data Release 1](https://arxiv.org/abs/2512.13783)
*Federico Sestito,Chiaki Kobayashi*

**TL;DR**: 利用DESI DR1数据结合盖亚(Gaia)天体测量确认了16个本星系群矮星系的成员恒星，首次通过DESI追踪六分仪座(Sextans)大半径（约10个半光半径）的恒星运动学特征，发现贫金属星族速度弥散更高且分布更广，富金属星族运动学更冷且中心聚集，并揭示了六分仪座内区存在陡峭金属丰度梯度（约-0.29 dex/kpc）而外区几乎无梯度，同时指出DESI对极贫金属星（[Fe/H]≲-2.0）的[α/Fe]测量存在大散射和强反相关性。


<details>
  <summary>Details</summary>

Background: 研究利用暗能量光谱仪首批数据（DESI DR1）识别本星系群矮星系的恒星成员，通过结合盖亚卫星（Gaia）自行、位置和测光数据验证成员性，并拓展至16个星系系统（涵盖超暗弱、经典和不规则矮星系）。

Data: 数据来源为暗能量光谱仪第一版数据（DESI DR1），并结合了盖亚（Gaia）自行、位置和测光数据筛选的候选成员星进行交叉匹配。

Method: 采用暗能量光谱仪数据发布1（DESI DR1）结合盖亚（Gaia）自行运动、位置和测光数据交叉匹配候选成员星，并利用DESI视向速度进行成员星可靠判定。

Result: result：DESI DR1数据确认Sextans（六分仪座矮星系）贫金属（metal-poor）星族具有更高速度弥散度且延伸至更大半径，而富金属（metal-rich）星族运动学更冷且中心聚集，其内区金属丰度梯度（metallicity gradient）较陡（约-0.29 dex/kpc），外围几乎无梯度，极贫金属星（[Fe/H]≲-2.0）的[α/Fe]与金属丰度呈现强反相关。
...
### [3] [Connection between galaxy morphology and dark-matter halo structure II: predicting disk structure from dark-matter halo properties](https://arxiv.org/abs/2512.13822)
*Jinning Liang,Fangzhou Jiang,Houjun Mo,Andrew Benson,Philip F. Hopkins,Avishal Dekel,Luis C. Ho*

**TL;DR**: 基于TNG50模拟研究发现，星系盘结构（disk）的尺寸和厚度可通过暗物质晕（dark-matter halo）的密度结构、角动量及形成历史等参数高精度预测，且低质量晕的预测准确性更高，同时揭示盘结构对晕内区（inner halo）的重构效应导致其与密度轮廓形状的关联性。


<details>
  <summary>Details</summary>

Background: 研究主题为利用TNG50模拟探究星系盘结构（galactic disk structures）与宿主暗物质晕（dark-matter halos）性质之间的关联机制。

Data: 数据来源于TNG50流体动力学模拟（hydrodynamic simulation）及其匹配的纯暗物质模拟（dark-matter-only runs），测量了描述密度结构、角动量、形状、组装历史和环境的综合晕族（halo）属性列表。

Method: 采用随机森林回归（Random Forest regression）和符号回归（Symbolic Regression）方法分析暗物质晕（dark-matter halo）属性与星系盘（galactic disk）结构的关系，并通过SHAP分析（SHAP analysis）识别关键晕参数。

Result: 基于TNG50模拟发现，暗物质晕（dark-matter halo）的浓度（concentration）、形状（Einasto shape）、整体与内部角动量（global and inner spin）及近期质量吸积（recent mass accretion）等参数能高精度预测星系盘（disk）的尺寸和厚度，且盘高度预测精度始终高于盘尺寸。
...
### [4] [Searching for Intermediate-Mass Black Holes in Milky Way satellites](https://arxiv.org/abs/2512.13823)
*R. Pascale,G. Battaglia*

**TL;DR**: 通过探测银河系矮卫星星系中中等质量黑洞（IMBHs）的占据比例（occupation fraction），为区分早期宇宙黑洞种子形成模型提供了关键观测依据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于中等质量黑洞（IMBHs）作为黑洞种群中尚未充分探索的组成部分，其形成机制与早期宇宙中多种理论路径（如第三星族恒星遗迹、致密星团坍缩或贫金属气体直接坍缩）密切相关，对理解黑洞种子形成模型具有关键意义。

Data: 研究使用了下一代宽视场光谱设施（next-generation wide-field spectroscopic facility）获取的多重深度恒星光谱数据和高分辨率积分场星系中心观测数据。

Method: 采用下一代宽视场光谱设施（next-generation wide-field spectroscopic facility）结合深度复用恒星光谱与高分辨率积分场星系中心观测技术。

Result: 中等质量黑洞（IMBHs）的存在及其在矮星系中的占据比例（occupation fraction）是区分早期宇宙黑洞种子形成模型的关键观测依据。
...
### [5] [Local Group dwarf galaxies as dark matter probes](https://arxiv.org/abs/2512.13827)
*G. Battaglia,J. M. Arroyo Polonio,R. Pascale,M. Benito,R. Leaman,G. Thomas*

**TL;DR**: 研究发现本地群(LG)矮星系暗物质晕(DM halo)属性与恒星质量及恒星形成历史(SFH)的系统关联可为区分标准冷暗物质(CDM)模型与重子过程影响提供关键观测约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于揭示非重子暗物质（DM）的本质及其对宇宙学和粒子物理标准模型的挑战，特别关注冷暗物质（CDM）范式在小尺度星系中的问题以及重子过程对暗物质晕（DM halo）内区性质的影响。

Data: 研究数据来源于12米级望远镜（视场约几平方度、复用能力数千）和30-40米级望远镜（视场约几平方角分）的多目标光谱仪观测，获取了本地群（LG）矮星系中数千颗恒星的视线速度（l.o.s. velocities）和金属丰度（metallicities）数据。

Method: 采用多目标光谱仪（multi-object spectrographs）在12米级望远镜和30-40米级望远镜上获取高精度视线速度（l.o.s. velocities）和金属丰度（metallicities）数据，系统测定矮星系的暗物质晕（DM halo）属性随恒星质量（stellar mass）和恒星形成历史（SFH）的变化关系。

Result: 本地群（Local Group）矮星系暗物质晕（DM halo）特性与恒星质量及恒星形成历史（SFH）的系统关联研究可为揭示暗物质本质及重子过程（baryonic processes）对暗物质晕内部区域的影响提供关键观测基准。
...
### [6] [The molecular diversity of the ISM in galaxies across cosmic time](https://arxiv.org/abs/2512.13881)
...
**TL;DR**: 论文核心贡献是提出利用亚毫米分子谱线（如CO、HCN、SiO）的多物种多跃迁观测建立星系际介质（ISM）过程的化学诊断方法，揭示气体对局部环境的响应及星系物质演化机制。


<details>
  <summary>Details</summary>

Background: 亚毫米分子谱线（如CO、HCN、SiO）为研究恒星形成（SF）和星系演化的物理化学过程提供了独特视角，但当前对银河系外分子宇宙的认识仍存在显著空白。

Data: 数据来源为亚毫米波分子谱线（如CO、HCN、SiO）的多物种多跃迁观测（multi-species, multi-transitions observations），覆盖多样化星系环境。

Method: 采用亚毫米分子谱线（如CO、HCN、SiO）的多物种多跃迁观测技术，结合银河系外天体化学（extragalactic astrochemistry）分析方法。

Result: 亚毫米分子谱线（如CO、HCN、SiO）为研究恒星形成（SF）和星系演化的物理化学过程提供了独特而强大的观测手段。
...
**TL;DR**: 提出利用AtLAST望远镜开展大面积均匀亚毫米(submm)波段的巡天观测，可大幅增加强引力透镜星系(SLGs)样本数量，突破当前样本不足对宇宙学、暗物质(DM)和高红移星系演化研究的限制。


<details>
  <summary>Details</summary>

Background: 强引力透镜（strong gravitational lensing）是研究宇宙学、暗物质（DM）和高红移星系演化的有力工具，但现有强透镜星系（SLGs）样本规模远不足以发挥其全部潜力。

Data: 数据来源于赫歇尔（Herschel）亚毫米（submm）巡天观测和拟议的阿塔卡马大毫米波望远镜（AtLAST）未来宽视场均匀亚毫米巡天。

Method: 采用亚毫米(submm)波段巡天技术，结合类似赫歇尔(Herschel)望远镜的观测方法，通过宽视场均匀巡天实现强引力透镜星系(SLGs)的大规模筛选。

Result: 亚毫米波(submm)选择是识别高红移强引力透镜星系(SLGs)最有效且偏差最小的方法，但现有样本数量不足。
...
### [8] [A wide-field, multi-line survey of CO in the Magellanic Clouds at parsec-scale resolution: characterising the molecular gas content with a 50-m single-dish submillimeter telescope](https://arxiv.org/abs/2512.13911)
...
**TL;DR**: 这篇论文提出通过大视场高分辨率亚毫米波观测（如CO及其同位素分子谱线）结合现有HI和HII巡天数据，构建麦哲伦云系统（LMC和SMC）的全分子图谱，以校准低金属丰度条件下的恒星形成规律和气体相分配机制，为高红移星系和宇宙学模拟提供基准。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用麦哲伦云（LMC和SMC）的低金属丰度特性模拟宇宙恒星形成高峰期的物理条件，以高分辨率观测研究星际介质（ISM）相态、分子云生命周期及反馈过程对星系演化的调控机制。

Data: 数据来源包括大麦哲伦云(LMC)和小麦哲伦云(SMC)的全天分子云尺度分辨率观测、CO及其同位素分子不同跃迁的宽视场光谱测绘、[CI]观测数据，以及已有的HI和HII巡天数据。

Method: 采用宽视场光谱测绘技术（wide-field spectroscopic mapping）对麦哲伦云系统进行CO及其同位素分子谱线多跃迁观测，结合[CI]谱线数据并与现有HI和HII巡天数据协同分析。

Result: 大麦哲伦云（LMC）和小麦哲伦云（SMC）的低金属丰度环境为研究宇宙恒星形成高峰期的星际介质（ISM）相态、分子云生命周期及反馈过程提供了独特条件。
...
### [9] [Cold Gas Infall onto A Brightest Group Galaxy via A Gas-Rich Minor Merger](https://arxiv.org/abs/2512.13932)
*Ming-Yang Zhuang,Jinyi Shangguan,Yuan Bian,Yue Shen,Luis C. Ho,Min Du,Junyao Li,Zhao-Yu Li,Jing Wang*

**TL;DR**: 研究发现近邻早型星系（ETGs）中的尘埃和冷气体（cold gas）可通过质量比为1:56的气体丰富小合并事件直接传输至最亮群星系（BGG），并形成10千秒差距的尘埃带，揭示了气体丰富小合并对星系群中心ETGs冷气体补充和演化的重要作用。


<details>
  <summary>Details</summary>

Background: 近邻早型星系(ETGs)中尘埃和冷气体的起源问题长期存在争议，涉及内部恒星演化物质抛射与外部机制(如小质量并合或冷却流)的相互作用。

Data: 数据来源于高分辨率JWST（James Webb Space Telescope）成像和宇宙学模拟（cosmological simulations）。

Method: 采用高分辨率JWST成像和宇宙学模拟（cosmological simulations）分析气体富集的小质量并合（gas-rich minor mergers）对最亮群星系（BGG）冷气体输送的直接证据。

Result: 首次在红移0.25的最亮群星系(BGG)中观测到由质量比约1:56的气体丰富小质量并合直接输送冷气体的明确证据。
...
### [10] [Investigating the Reionization Epoch through 21\,cm and Line Intensity Mapping Experiments](https://arxiv.org/abs/2512.13943)
*Anirban Roy,Anthony Pullen,Patrick C. Breysse,Rachel S. Somerville*

**TL;DR**: 通过21厘米信号与多谱线强度映射（LIM）的互相关分析，结合SKA-low和FYST类实验模拟观测，发现[CII]和CO(1-0)与21厘米信号的互相关能显著约束再电离化（reionization）历史参数（如电离分数xe）及恒星形成率与谱线光度关系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙再电离时期(EoR)的物理过程及其与第一代恒星和星系形成的关系，通过多波段谱线强度映射(LIM)与21厘米信号的交叉关联来探索再电离的时序、形态及星系际介质的物理条件。

Data: 数据来源于平方公里阵列（SKA）-低频段21厘米模拟观测和弗雷德·杨亚毫米波望远镜（FYST）类多线强度映射（LIM）实验的模拟数据。

Method: 采用21厘米信号与多线强度映射（LIM）的互相关分析，结合SKA-low和FYST模拟观测数据，重点约束电离源最小质量和电离分数（xe）等参数。

Result: 通过[CII]和CO(1-0)等多线强度映射(LIM)与21厘米信号的互相关分析，可显著约束再电离化历史参数（如电离分数xe）及恒星形成率与多线光度关系等物理特性。
...
### [11] [From ASTRID to BRAHMA -- The role of overmassive black holes in little red dots in cosmological simulations](https://arxiv.org/abs/2512.13957)
...
**TL;DR**: 研究发现超大质量黑洞（overmassive black holes）被稠密气体云遮蔽时能产生类似JWST观测到的"小红点"（LRD）特征，其数量密度（红移5-8时为2.04±0.32×10⁻⁴ Mpc⁻³）与观测一致，且AGN主导的强巴尔末跳变和低尘埃衰减（A_V=0.21±0.12）是解释LRD红颜色和ALMA不可测的关键。


<details>
  <summary>Details</summary>

Background: 研究探讨了超大质量黑洞（overmassive black holes）在詹姆斯·韦伯太空望远镜（JWST）发现的独特“小红点”（LRD）天体发射中的作用及其与宿主星系质量比的异常关系。

Data: 数据来源于BRAHMA宇宙流体动力学模拟套件（cosmological hydrodynamic simulation suite）中对超大质量黑洞（overmassive black holes）的模拟实现，以及ASTRID模拟（ASTRID simulation）中系统产生的较小黑洞数据。

Method: 采用BRAHMA宇宙学流体动力学模拟套件中的超大质量黑洞（overmassive black holes）模型，结合活动星系核（AGN）被致密气体云遮蔽的发射模型进行分析。

Result: 超大质量黑洞（overmassive black holes）被稠密气体云遮蔽时能产生类似“小红点”（LRD）的观测特征，其活动星系核（AGN）主导的发射通过极强的巴尔默间断（Balmer break）呈现红色特征。
...
### [12] [Primordial Black Holes as Seeds for Extremely Overmassive AGN Observed by JWST](https://arxiv.org/abs/2512.14066)
*Saiyang Zhang,Boyuan Liu,Volker Bromm,Florian Kühnel*

**TL;DR**: 研究发现原始大质量黑洞（PBH）通过反馈调节机制（包括吸积、外流和恒星形成）能自然形成与JWST观测到的Abell 2744-QSO1类似的低金属丰度（Z/Z⊙≲10⁻²）、极端黑洞-恒星质量比（M_BH/M⋆）的高红移（z≃7）致密星系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高分辨率宇宙学模拟探讨大质量原初黑洞（PBH）作为种子如何通过吸积、反馈与恒星形成相互作用形成高红移（$z\simeq 7$）极端贫金属且黑洞主导的星系（如Abell 2744-QSO1），以解释詹姆斯·韦伯太空望远镜（JWST）观测到的异常黑洞-恒星质量比和低金属丰度现象。

Data: 研究使用了高分辨率宇宙学模拟（cosmological simulation）数据，首次整合了原初黑洞（PBH）吸积、黑洞反馈以及第三族/第二族恒星形成和恒星反馈的完全耦合处理。

Method: 采用高分辨率宇宙学模拟（high-resolution cosmological simulations）结合原初黑洞（PBH）种子模型，首次完整耦合了原初黑洞吸积、黑洞反馈、第三族/第二族恒星形成及恒星反馈过程。

Result: 大质量原初黑洞（PBH）通过反馈调节机制（包括吸积延迟恒星形成、金属丰度降低和极端质量比形成）为高红移极端系统（如Abell 2744-QSO1）的形成提供了物理解释。
...
**TL;DR**: 研究发现费米/eROSITA气泡（Fermi/eROSITA bubbles）中的多相气体（multiphase gas）源自核区外流（nuclear outflows）与棒驱动（bar-driven）倾斜尘埃带（tilted dust lanes）和扭曲超射流（distorted overshooting streams）的相互作用，剥离并裹挟了外银河区（R_GC~0.5--1.7 kpc）而非仅中央分子区（CMZ）的冷气体，解释了气泡的高湍流、复杂光谱特征及空间运动学不对称性，揭示了棒驱动气体动力学与核反馈在银河系长期演化中的关键作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中心（GC）反馈机制中多相气体（multiphase gas）的起源及其与银河系棒（Galactic bar）驱动气体动力学和核反馈（nuclear feedback）的相互作用。

Data: 数据来源于HI4PI巡天数据（HI4PI data）用于研究银心（GC）方向高速云（HVCs）的运动学和物理性质。

Method: 采用HI4PI数据结合核外流（nuclear outflows）与棒驱动（bar-driven）气体动力学模型分析高速云（HVCs）的运动学和物理性质。

Result: 高速云（HVCs）的不对称分布与棒驱动（bar-driven）的倾斜尘埃带和扭曲超调流（overshooting streams）密切相关，表明核区外流（nuclear outflows）与这些富含气体的离平面结构相互作用，从外银河区（R_GC~0.5--1.7 kpc）剥离并夹带冷气体而非仅来自中央分子区（CMZ; R_GC<0.3 kpc）。
...
**TL;DR**: 论文核心贡献是探讨超新星遗迹（SNR）冲击分子云（molecular clouds）的化学复杂性对揭示太阳系形成和地球生命起源的作用，并提出未来望远镜的技术需求。


<details>
  <summary>Details</summary>

Background: 研究探讨了太阳可能形成于受超新星遗迹（SNR）影响的星云中，并探索了超新星遗迹的喷出物和冲击波如何为地球生命的元素组成提供基础，从而揭示恒星形成过程及地球生命起源的化学复杂性。

Data: 研究使用了模拟数据(simulation data)来探讨超新星遗迹(SNR)对分子云化学复杂性的影响。

Method: 采用核心分析方法探讨超新星遗迹（SNR）冲击分子云的化学复杂性及其对恒星形成和地球生命起源的影响。

Result: 未来望远镜的技术规格将聚焦于探测超新星遗迹(SNR)冲击分子云的化学复杂性，以揭示恒星形成过程及地球生命起源。
...
### [15] [Can dwarf spheroidal galaxies host a central black hole ?](https://arxiv.org/abs/2512.14146)
*K. Aditya,A. Mangalam*

**TL;DR**: 通过构建银河系矮椭球星系(dSph)质量模型并分析动力学数据，得出其中心黑洞(BH)质量的95%置信上限为log(M•/M⊙)<6，并结合文献数据建立了覆盖速度弥散(σ)范围10-300 km/s的统一M•-σ关系：log(M•)=8.32+4.08log(σ/200 km/s)，其内禀散射σint=0.55，该关系与动量驱动吸积和恒星捕获模型预测的103-104 M⊙黑洞质量范围一致。


<details>
  <summary>Details</summary>

Background: 研究旨在通过构建银河系矮椭球星系(dwarf spheroidal galaxies)的质量模型，约束其中心可能存在的黑洞(BH)质量上限。

Data: 研究使用了银河系矮椭球星系(dwarf spheroidal galaxies)的恒星运动学(kinematic)观测数据。

Method: 采用Osipkov-Merritt-Cuddeford各向异性分布函数(anisotropic distribution function)对银河系矮椭球星系(dwarf spheroidal galaxies)进行三成分(恒星、暗物质晕和中心黑洞)质量建模。

Result: 银河系矮椭球星系中心黑洞(BH)质量95%置信上限为log(M•/M⊙) < 6，与动量驱动吸积和恒星捕获模型的预测值103-104 M⊙相符。
...
### [16] [NIKA2 Cosmological Legacy Survey. First measurement of the confusion noise at the IRAM 30 m telescope](https://arxiv.org/abs/2512.14176)
...
**TL;DR**: NIKA2宇宙学遗留巡天（N2CLS）首次在GOODS-North天区利用30米望远镜测得1.2毫米和2毫米波段的混淆噪声（confusion noise）分别为139.1±11.9 μJy/beam和38.6±3.7 μJy/beam，并开发了交叉方差（cross variance）方法估计两波段间的相关混淆噪声为49.6±6.4 μJy/beam，结果与SIDES模型一致。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用NIKA2双波段相机提升对遥远尘埃恒星形成星系（DSFGs）物理特性的理解。

Data: 数据来源于NIKA2双波段相机在IRAM 30米望远镜上对GOODS-North区域进行的78.2小时深度观测，同时获取1.2毫米和2毫米波段数据，覆盖视场约240平方角分，并利用最佳采样的约62平方角分区域测量混淆噪声。

Method: 采用双波段（1.2和2毫米）同步观测并结合新设计的交叉方差（cross variance）估计器，首次测量了30米望远镜的混淆噪声极限并估算了双波段间的相关混淆噪声。

Result: 首次在30米望远镜上测得1.2毫米和2毫米波段的混淆噪声(confusion noise)分别为139.1^{+15.9}_{-19.2}±11.9 μJy/beam和38.6^{+9.6}_{-13.1}±3.7 μJy/beam，并通过新设计的交叉方差(cross variance)估计器获得两波段间相关混淆噪声为49.6^{+15.9}_{-24.8}±6.4 μJy/beam。
...
### [17] [Mass Assembly and Chemical Complexity in the Milky Way](https://arxiv.org/abs/2512.14203)
...
**TL;DR**: 亚毫米波/毫米波（sub-mm/mm）谱线揭示了银河系气体成分的物理条件、动力学特性、电离状态及磁场强度，是研究质量聚集和结构形成的关键示踪剂。


<details>
  <summary>Details</summary>

Background: 亚毫米波/毫米波（sub-mm/mm）谱线研究旨在揭示银河系气体成分的化学复杂性、富集历史及物理条件（如动力学、电离状态和磁场强度），并作为跨尺度质量聚集和结构形成的关键示踪剂。

Data: 数据来源于亚毫米波/毫米波(Sub-millimeter/millimeter)光谱线观测数据。

Method: 采用(亚)毫米波谱线(Sub-millimeter spectral lines)进行光谱分辨分析，揭示银河系气体组分的物理条件、动力学特性、电离状态及磁场强度。

Result: (Sub-)毫米波谱线可用于揭示银河系气体组分的物理条件、动力学特征、电离状态及磁场强度。
...
### [18] [Exploring Cosmic Dawn with PANORAMIC II: Cosmic Variance and Galaxy Clustering at $z\sim10$](https://arxiv.org/abs/2512.14212)
...
**TL;DR**: 通过测量红移(z)~10处莱曼断裂星系(Lyman break galaxies)的宇宙方差(σ_CV)并与UniverseMachine模拟对比，发现现有模型低估了紫外亮星系(UV-bright galaxies)数密度至少5倍，且支持通过降低质量-光比或假设恒星形成效率(SFE)与晕质量(M_halo)呈幂律关系的物理机制来解释观测数据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST观测数据揭示高红移(z≳10)紫外亮星系(UV-bright galaxies)异常丰度现象，通过星系成团性(clustering)分析为不同理论模型提供互补性约束。

Data: 数据来源于34条独立视线上的PANORAMIC纯并行和遗留成像（PANORAMIC pure parallel and legacy imaging），以及UniverseMachine模拟数据（UniverseMachine simulation data）。

Method: 采用PANORAMIC纯并行和传统成像技术，结合34条独立视线测量宇宙方差（cosmic variance, σ_CV）以量化z∼10莱曼断裂星系（Lyman break galaxies）的成团强度。

Result: 在红移（redshift）z∼10处，莱曼断裂星系（Lyman break galaxies）的宇宙方差（cosmic variance）测量值与UniverseMachine模拟结果一致，但数密度低至少5倍。
...
### [19] [Connecting galaxies with their haloes -- from parsec to Mpc scales](https://arxiv.org/abs/2512.14281)
...
**TL;DR**: 论文核心贡献是提出需要结合大视场光谱观测（wide-field spectroscopic observations）和统计显著样本，以解决星系（galaxies）与暗物质晕（dark matter haloes）关联、低表面亮度特征形成及长期演化等跨尺度问题。


<details>
  <summary>Details</summary>

Background: 星系演化研究涉及从巨分子云（GMC）内恒星形成（秒差距尺度）到星系外流和长期演化（千秒差距尺度），以及星系、暗物质晕（dark matter haloes）和大尺度结构（Mpc尺度）相互作用的跨尺度物理过程。

Data: 研究中使用的数据来源包括高度空间分辨（parsec-scale）的积分场光谱观测（如VLT/MUSE）以及探测星系外围极低表面亮度区域的大范围天区测光巡天数据。

Method: 采用高空间分辨率积分场光谱观测（如VLT/MUSE）结合大范围低表面亮度星系外围探测的宽视场光谱观测技术。

Result: 星系演化研究的关键挑战在于通过宽视场光谱观测（wide-field spectroscopic observations）获取邻近星系的统计显著样本，以解决星系与暗物质晕（dark matter haloes）的关联、低表面亮度特征（low surface brightness features）形成及长期演化驱动机制等核心问题。
...
### [20] [Surveying Galaxy Clusters (in formation) in the Distant Universe](https://arxiv.org/abs/2512.14372)
...
**TL;DR**: 该论文提出未来大型单天线(submillimeter)观测台站通过高灵敏度、大视场和多波段覆盖能力，结合光学/近红外巡天数据，将首次实现对原星系团(protocluster)中恒星、气体和暗物质多尺度组装的完整观测，填补早期结构形成研究的空白。


<details>
  <summary>Details</summary>

Background: 研究聚焦于理解高红移(z>2)原星系团(protocluster)的早期组装过程及其冷气体和尘埃在星系快速增长中的作用。

Data: 研究中使用的数据来源为未来具备大视场（degree-scale coverage）、宽波段（broad spectral access）和高多路复用能力（high-multiplex capability）的单天线（single-dish）亚毫米波观测设备，并与欧几里得（Euclid）、LSST和罗曼（Roman）等宽视场光学/近红外巡天协同观测。

Method: 采用未来大型单天线观测站（large single-dish observatory）进行度级覆盖、宽谱段访问和高复用能力的综合均匀测绘，结合光学/近红外（optical/NIR）广域巡天数据，揭示原星系团（protocluster）中恒星、气体和暗物质组分的多尺度多相态亚毫米波观测。

Result: 未来大型单天线观测站将能全面绘制原星系团（protoclusters）的冷气体和尘埃分布，揭示恒星形成触发或抑制的区域并量化冷气体总量，从而提供原星系团环境中气体冷却的信息。
...
### [21] [Bowshocks driven by the pole-on molecular jet of outbursting protostar SVS 13](https://arxiv.org/abs/2512.14458)
...
**TL;DR**: 论文通过ALMA观测揭示了极高速度（EHV）分子子弹（molecular bullets）的精细形态和运动学结构，证实其符合动量守恒弓形激波（bowshock）模型，并发现喷流变率驱动的夹带（entrainment）与吸积爆发及盘雪线（snowlines）十年尺度变化相关，直接影响颗粒生长和行星形成。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高速分子外流（outflows）在恒星和行星形成过程中的关键作用，特别是极端高速（EHV）分子团块（molecular bullets）如何通过激波驱动机制向周围介质传递质量和动量。

Data: 数据来源于ALMA（阿塔卡马大型毫米波/亚毫米波阵列）对CO(J=3-2)谱线的观测，分辨率达30天文单位（au），覆盖了中间速度范围（约30-100 km s$^{-1}$）的完整运动学结构。

Method: 采用阿塔卡马大型毫米波/亚毫米波阵列(ALMA)的CO(J=3-2)谱线观测数据，结合薄层弓形激波(bowshock)动量守恒模型分析分子子弹(molecular bullets)的形态和运动学结构。

Result: 分子子弹（molecular bullets）的形态和运动学结构支持其由时间变化的极高速度（EHV）喷流产生的动量守恒弓形激波（bowshock）驱动，且喷流变率与吸积爆发及盘雪线（disk snowlines）变化在十年时间尺度上相关。
...
**TL;DR**: 通过ALMA对4个红移(z)8.5-10.3的强引力透镜星系进行[OIII]88μm观测，首次在z>8星系UNCOVER-10646中探测到高信噪比的宽成分（FWHM=1366 km/s），表明存在质量流失率高达128 M⊙/yr的离子化外流，并发现z≈9星系遵循局域[OIII]-恒星形成率关系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用ALMA Band 7观测高红移（$z = 8.5 - 10.3$）透镜星系（lensed galaxies）的[OIII] $88μ$m谱线和尘埃连续谱，以探索早期宇宙星系（early Universe galaxies）的物理性质与电离气体外流（ionized outflows）。

Data: 数据来源于ALMA Band 7对四颗引力透镜放大（magnification μ=1.4-3.8）的JWST选定星系（z=8.5-10.3）进行的[OIII] 88μm谱线和尘埃连续谱深度观测。

Method: 采用ALMA Band 7对[OIII] 88μm谱线和尘埃连续谱进行深度观测，结合引力透镜放大效应（magnification μ=1.4-3.8）提升探测灵敏度。

Result: 在UNCOVER-10646和DHZ1中分别以15σ和6σ的置信度检测到[OIII] 88μm发射线，表明它们是遵循本地[OIII]-SFR关系的高光度系统[L_[OIII] = (1.1 - 1.6) × 10^9 L_⊙]。
...
### [23] [The role of mergers and rejuvenation in the buildup of the quiescent population at cosmic noon](https://arxiv.org/abs/2512.14599)
...
**TL;DR**: 基于半解析模型(SAM)L-Galaxies模拟观测光锥研究发现，低质量($M_\ast < 10^{10}$M$_\odot$)被动星系中60-80%经历后星暴(PSB)阶段，而高质量($M_\ast>10^{10}$M$_\odot$)星系在$1<z<2$时PSB贡献占被动质量函数增长的18-28%，表明高低质量星系遵循不同淬灭路径。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系淬灭（quenching）过程及其对被动星系质量函数（passive galaxy mass function）累积的影响，特别关注不同质量星系（如恒星形成星系、静止星系和后发星暴星系（PSB））的并合、复苏（rejuvenation）和可见时间（visibility times）等关键机制。

Data: 数据来源于半解析模型（Semi-Analytic Model, SAM）L-Galaxies生成的模拟观测光锥（mock observational lightcone），并与UKIDSS超深巡天（UKIDSS Ultra Deep Survey, UDS）的观测数据紧密匹配。

Method: 采用半解析模型（Semi-Analytic Model, SAM）L-Galaxies生成模拟观测光锥（mock observational lightcone），并与UKIDSS超深巡天（UKIDSS Ultra Deep Survey, UDS）数据匹配，研究星系淬灭（quenching）过程中的并合（merging）、复苏（rejuvenation）及后星暴（post-starburst, PSB）星系的可见时间（visibility times）。

Result: 在恒星质量较低（$M_\ast < 10^{10}$\,M$_\odot$）的星系中，约60-80%的被动星系经历过后星暴（PSB）阶段，表明低质量星系与高质量星系遵循不同的淬灭路径。
...
### [24] [Cluster properties as a function of dynamical state in the DESI Legacy x UNIONS surveys](https://arxiv.org/abs/2512.14636)
...
**TL;DR**: 该研究通过观测和模拟分析发现，演化完成（evolved）和演化中（evolving）的星系团（galaxy clusters）在弱引力透镜（weak-lensing）轮廓、恒星质量函数（stellar mass functions）及卫星星系分布上存在显著差异，揭示了星系团星系种群与其暗物质晕（dark matter halo）动力学状态的紧密关联。


<details>
  <summary>Details</summary>

Background: 研究主题为星系团动力学状态（dynamical state）对其星系群质量分布的影响及其与暗物质晕（dark matter halo）演化的关联。

Data: 数据来源于DESI Legacy Imaging Survey的光度选星系团与UNIONS星系剪切目录的交叉匹配，以及IllustrisTNG模拟的星系团数据。

Method: 基于静止r波段星等差和BCG（brightest cluster galaxies）与亮卫星星系间的恒星质量比，对星系团进行演化状态分类，并测量其恒星质量函数、弱引力透镜（weak-lensing）剖面、径向数密度及红星系比例剖面。

Result: 演化星系团（evolved clusters）表现出更集中的弱引力透镜（weak-lensing）轮廓、由大质量最亮团星系（BCGs）主导的双峰恒星质量函数以及中等质量卫星星系的缺失。
...
### [25] [Radio emission in star-forming galaxies: connection to restarted or relic AGN activity](https://arxiv.org/abs/2512.14646)
*Marco Albán,Dominika Wylezalek,Pranav Kukreti,Rogemar A. Riffel,Rogerio Riffel*

**TL;DR**: 研究发现，在无明确活动星系核（AGN）特征的恒星形成（SF）星系中，GHz波段探测到的样本比未探测到的样本表现出更宽的发射线宽度、更高中心外流比例、更红颜色、更强中心遮蔽以及更接近AGN区域的发射线比率偏移，且具有更紧凑的射电形态，这些特征与射电探测到的AGN相似，暗示GHz探测的SF星系可能处于AGN演化前兆阶段或低功率AGN状态。


<details>
  <summary>Details</summary>

Background: 研究探讨了射电活动星系核（AGN）与星系电离气体动力学扰动及外流现象之间的关联，并聚焦于缺乏明显AGN特征的恒星形成（SF）星系中射电辐射的潜在物理机制。

Data: 数据来源于低频射电巡天（LOFAR，144MHz）和高频射电巡天（FIRST，1.4GHz）的观测数据，并结合了MaNGA巡天提供的空间分辨光谱数据。

Method: 结合低频（LOFAR，144MHz）和高频（FIRST，1.4GHz）射电巡天数据，并整合MaNGA（Mapping Nearby Galaxies at APO）巡天的空间分辨光谱观测，对比分析具有144MHz探测但有无GHz探测的恒星形成（SF）星系样本。

Result: GHz探测到的恒星形成星系表现出增强的电离气体发射线宽度、更高的中心外流比例、更红的颜色、更强的中心遮蔽以及向AGN区域偏移的发射线比例（[NII] BPT图），其物理机制与射电探测到的活动星系核（AGN）相似。
...
### [26] [NGC 3521 as the Milky Way near twin: spectral energy distribution from UV to radio decameter ranges](https://arxiv.org/abs/2512.14664)
...
**TL;DR**: 该研究通过构建银河系近邻星系NGC~3521从紫外到分米波段的均匀光谱能量分布（SED），首次在24--32 MHz波段设定了上限流量，并利用多波段数据建模获得了恒星质量、恒星形成率、尘埃质量及有效尘埃温度等参数，为基于SED相似性识别银河系类似天体（MWAs）提供了新方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过构建银河系类似体（MWAs）的多波段光谱能量分布（SED）来完善其识别标准，特别关注低频射电波段（<100 MHz）的均匀观测数据对验证银河系类似体的互补作用。

Data: 数据来源包括GALEX、SDSS、WISE、Spitzer/MIPS、Herschel/PACS+SPIRE、VLA观测数据，以及乌克兰T形射电望远镜（Ukrainian T-shape radio telescope）在2022年1-2月获取的新观测数据，补充了米波/十米波段的限制条件，并整合了ZTF和NEOWISE（2014-2025年）的变异性监测数据。

Method: 采用\textsc{CIGALE}模型（包含专用低频射电处方\texttt{radio_extra}）对紫外至十米波段的27个点光谱能量分布（SED）进行建模，同时整合发射与吸收效应。

Result: 首次在24-32 MHz波段获得银河系近双胞胎NGC 3521的上限流量（S_{28,MHz}<11.22 Jy），并通过整合紫外至分米波段的均匀光谱能量分布（SED）为识别和验证银河系类似体（MWAs）提供了互补诊断依据。
...
### [27] [1D stellar mergers: entropy sorting and PyMMAMS](https://arxiv.org/abs/2512.14669)
*Max Heller,Fabian R. N. Schneider,Jan Henneco,Vincent A. Bronner,Mike Y. M. Lau*

**TL;DR**: 通过比较基于熵排序（entropy sorting）的两种一维方法与三维模拟，发现考虑激波加热（shock-heating）的PM方法能更准确预测恒星合并（stellar mergers）后的热力学和成分结构，且一维方法可较好近似三维模拟结果。


<details>
  <summary>Details</summary>

Background: 恒星多星系统（stellar multiple systems）是普遍现象而非特例，许多系统在其生命周期中会经历相互作用阶段，其中部分相互作用会导致恒星并合（stellar mergers）。

Data: 研究中使用的数据来源包括用于校准Python版'Make Me A Massive Star'（PM）代码的SPH（光滑粒子流体动力学）模拟恒星正面碰撞数据，以及一组39个不同于PM校准所用的近期SPH正面碰撞模拟数据，还涉及一个3D磁流体动力学（MHD）模拟的9 + 8太阳质量主序星合并案例。

Method: 采用基于熵排序（entropy sorting）概念的两种一维方法（基础熵排序法'ES'和Python版'Make Me A Massive Star'代码'PM'），并与三维模拟结果进行对比，其中PM额外应用了基于SPH（smoothed particle hydrodynamics）模拟校准的激波加热（shock-heating）方案。

Result: 基于熵排序（entropy sorting）和冲击加热（shock-heating）校准的1D方法能较好地近似3D恒星并合模拟，且冲击加热对准确预测并合后结构至关重要。
...
### [28] [JWST Observations of the Double Nucleus in NGC 4486B: Possible Evidence for a Recent Binary SMBH Merger and Recoil](https://arxiv.org/abs/2512.14695)
...
**TL;DR**: 通过JWST-NIRSpec IFU运动学观测确认NGC 4486B星系中心存在质量为$3.6\pm0.7\times10^8$太阳质量的超大质量黑洞(SMBH)，其双核结构和核区不对称运动学特征（6 pc位移的速度弥散峰和16 km/s的视线速度偏移）支持该黑洞由引力波(GW)反冲形成的偏心核盘(END)模型，推断前身黑洞质量比q>0.15且经历了~340 km/s的反冲速度，表明该星系近期发生过黑洞合并事件。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过JWST-NIRSpec IFU运动学确认NGC 4486B星系中心超大质量黑洞（SMBH）及其伴生偏心核盘（END）的形成机制，探讨双黑洞合并后引力波（GW）反冲对星系核动力学的影响。

Data: 数据来源于JWST-NIRSpec积分视场单元（IFU）的运动学观测和直接N体模拟（direct N-body simulations）。

Method: 采用轨道叠加模型（orbit-superposition models）和直接N体模拟（direct N-body simulations）分析核区动力学不对称性，并测试偏心核盘（END）形成机制与引力波反冲（GW recoil）假说。

Result: 观测结果支持NGC 4486B中心存在一个被偏心核盘(END)环绕的超大质量黑洞(SMBH)，其形成源于双黑洞合并后的引力波(GW)反冲。
...
### [29] [Halo Lightcones with Optimised Orientation and Interpolation in Cosmological Simulations -- an application to mock H$α$ selected galaxies](https://arxiv.org/abs/2512.13801)
...
**TL;DR**: 开发了HOLCon框架高效构建暗物质晕光锥（lightcone）并生成模拟星系目录，通过优化光锥方向、线性插值晕属性及结合SciPIC算法，在Uchuu模拟中创建了覆盖50平方度至红移（redshift）z=10的代表性光锥，验证了其宇宙学适用性并预测了Hα发射体（Hα emitters）的成团性。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发高效构建暗物质晕光锥（lightcone）的框架HOLCon，以支持大规模测光和光谱巡天（sky surveys）所需的精确宇宙学模拟，并考虑红移依赖的星系形成与演化过程。

Data: 数据来源于公开可用的Uchuu宇宙学模拟（Uchuu simulation），通过HOLCon框架构建了50平方度、红移延伸至z=10的暗物质晕光锥（lightcone）。

Method: 采用HOLCon框架（Halo Optimised Lightcone Constructor）优化暗物质晕光锥（lightcone）构建，结合线性插值方案追踪晕属性演化，并利用Dask并行计算技术实现高效生成。

Result: 开发了高效框架HOLCon（Halo Optimised Lightcone Constructor），通过优化光锥（lightcone）方向并采用线性插值方案追踪暗物质晕（dark matter halo）演化，基于Uchuu模拟构建了覆盖50平方度且延伸至红移z=10的光锥，验证了其适用于宇宙学分析，并通过扩展版SciPIC算法生成包含Hα发射体（Hα emitters）聚类预测的综合星系模拟目录。
...
### [30] [Buried but not destroyed: the evolution from prompt cusps to NFW haloes](https://arxiv.org/abs/2512.13804)
*Yuchan Wang,Sownak Bose,Carlos Frenk,Adrian Jenkins*

**TL;DR**: 通过64次高分辨率N体模拟，揭示了暗物质晕(Dark Matter Halo)初始形成的陡峭幂律密度分布("prompt cusps")通过大质量并合、人工碎片吸积和大尺度纤维结构相互作用三种途径向NFW(Navarro-Frenk-White)轮廓转变的演化机制，并发现原始峰值相关粒子仍保留初始幂律特征。


<details>
  <summary>Details</summary>

Background: 暗物质晕（dark matter haloes）的内部结构反映了其形成历史，并为暗物质本质和结构形成机制提供了关键线索。

Data: data部分：研究采用64次放大N体模拟（zoom-in N-body simulations）数据，针对八个暗物质晕（dark matter haloes）分别用八种不同自由流波数（free-streaming wavenumbers）重新模拟，以控制小尺度结构丰度同时保持大尺度环境固定。

Method: 采用64次变分辨率N体模拟（zoom-in N-body simulations）结合自由流波数（free-streaming wavenumbers）调控小尺度结构丰度，并通过物理驱动的子晕-原初峰匹配方法区分人工碎片与真实子晕。

Result: 暗物质晕（dark matter haloes）最初形成陡峭的幂律密度分布（prompt cusps），随后通过大质量并合、人工碎片吸积和大尺度纤维结构相互作用三种途径向NFW（Navarro-Frenk-White）轮廓转变。
...
### [31] [A high-dynamic-range view of the growth of structure and the warm/hot Universe](https://arxiv.org/abs/2512.13818)
...
**TL;DR**: 论文核心贡献是提出需要建造大口径单天线（亚）毫米波望远镜以高分辨率和高测绘速度绘制电离气体（ionized gas）在宇宙大尺度结构（large-scale structures）形成过程中的温/热分布。


<details>
  <summary>Details</summary>

Background: 研究聚焦于揭示大质量结构（星系、星系群、星系团和纤维结构）形成过程中电离气体（warm/hot gas）的热演化与空间分布规律。

Data: 数据来源包括ALMA的深毫米波观测（探测了约4个红移z~2-4的大质量系统）以及低分辨率毫米波巡天（在角分分辨率下探测了数千个红移z≈2的天体）。

Method: 采用深毫米波观测（如ALMA）和低分辨率毫米波巡天技术，结合大孔径单天线（亚）毫米波望远镜实现高分辨率成像与污染物信号剔除。

Result: 需要一台大口径单天线（亚）毫米波望远镜以提供比现有设备高几个数量级的测绘速度，同时保持几角秒分辨率，用于成像电离气体并去除全（亚）毫米波段中污染的射电和尘埃热信号。
...
### [32] [AtLAST -- Cosmology with submillimetre galaxies magnification bias](https://arxiv.org/abs/2512.13894)
...
**TL;DR**: 该论文提出放大偏差（magnification bias）作为一种独立且互补的宇宙学探测手段，通过引力透镜效应测量星系-物质关联性，无需依赖星系形状或点扩散函数（PSF）建模，其敏感度覆盖物质密度、结构振幅及红移z≤1区间暗能量（DE）演化，且参数简并与剪切（shear）、重子声波振荡（BAO）和宇宙微波背景（CMB）数据不同，但当前应用受限于天区覆盖和红移测量精度，而下一代亚毫米波设施（如AtLAST）可突破限制使其成为高精度宇宙学工具。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用放大偏差（magnification bias）作为独立且强大的宇宙学探针，探索暗能量（DE）演化、物质密度及结构增长等核心宇宙学问题。

Data: 研究中使用的数据来源于下一代亚毫米波广域巡天设施AtLAST（Atacama Large Aperture Submillimeter Telescope）的均匀深度巡天和光谱测绘观测。

Method: 采用放大偏差（magnification bias）作为引力透镜观测手段，通过星系-物质相关性分析来探测物质密度、结构振幅及暗能量（DE）红移演化，无需依赖星系形态、点扩散函数（PSF）建模或本征排列校正。

Result: 放大偏差（magnification bias）作为一种引力透镜观测手段，能够独立探测星系-物质关联性，且不依赖星系形态、点扩散函数建模或内禀排列校正，其灵敏度同时涵盖几何结构和暗能量（DE）在红移$z \leq 1$以下的演化特性。
...
### [33] [AtLAST -- Determination of Halo Mass Density Profiles at kpc Scales through Magnification Bias](https://arxiv.org/abs/2512.13895)
...
**TL;DR**: 该论文提出利用放大偏置效应（magnification bias）探测星系和星系团质量密度分布的新方法，指出下一代宽视场亚毫米波（submm）望远镜AtLAST可克服现有观测限制，显著提升该技术的诊断能力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用放大偏置效应（magnification bias）探测星系和星系团的质量密度分布，克服传统弱引力透镜剪切测量的局限性。

Data: 数据来源于拟议的50米望远镜AtLAST（Atacama Large Aperture Submillimeter Telescope）进行的下一代宽视场、高通量亚毫米波（submm）巡天观测。

Method: 采用放大偏置（magnification bias）技术分析背景源数量变化，结合下一代宽视场高吞吐量亚毫米波（submm）设施（如AtLAST）提升诊断能力。

Result: 放大偏置（magnification bias）作为引力透镜效应导致的背景源数量密度变化，能够独特且高效地探测星系和星系团在千秒差距（kpc）尺度上的质量密度分布。
...
### [34] [Catalogs of optically-selected clusters and photometric luminous red galaxies from the Hyper Suprime-Cam Subaru Strategic Program final year dataset](https://arxiv.org/abs/2512.13954)
...
**TL;DR**: 基于Hyper Suprime-Cam Subaru Strategic Program数据构建了红移达1.38的10,000余个光学星系团（cluster）样本和600万红移达1.25的光度红星系（LRG）样本，更新了质量-富度关系（mass-richness relation）约束，并验证了星系团红移精度优于0.01、红星系红移精度在0.4-1.0范围内优于0.02。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用Hyper Suprime-Cam Subaru Strategic Program（HSC-SSP）最终年数据集构建光学选星系团和光度学红序星系（LRGs）样本，以探索高红移（$z \sim 1.38$）星系团及其成员星系的统计性质和弱引力透镜效应。

Data: 数据来源于Hyper Suprime-Cam Subaru Strategic Program（HSC-SSP）最终年度数据集，覆盖约1200平方度天区，包含光学选择的星系团和光度学选择的红序星系（LRGs）样本。

Method: 采用光学选择星系团和光度学红星系（LRGs）样本，基于Hyper Suprime-Cam Subaru Strategic Program数据，通过成员概率加权富度（richness）和光度学红移（photometric redshift）分析，并利用弱引力透镜（weak lensing）信号约束质量-富度关系。

Result: result：构建了红移达1.38、包含超10000个富集度（richness）大于15的星系团样本，并测得红移散射（scatter）优于0.01，同时利用弱引力透镜（weak lensing）信号更新了质量-富集度关系（mass-richness relation）的约束。
...
### [35] [Revealing the baryon cycle in Galaxy Clusters: connecting galaxy dynamics and gas thermodynamics using (sub-)mm-wave and optical IFU surveys](https://arxiv.org/abs/2512.14238)
...
**TL;DR**: 结合宽视场光学积分视场单元(IFU)巡天和大口径亚毫米(sub-mm)望远镜观测，首次实现了星系动力学与气体热力学(thermodynamics)的联合研究，填补了星系演化环境过程研究中高分辨率大尺度观测的空白。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于星系演化过程中大尺度环境（large scale environment）相互作用的影响，旨在填补当前对暖/热星系团内介质（warm/hot intracluster medium）、星系周介质（circumgalactic medium）和星系际介质（intergalactic medium）缺乏大范围高分辨率观测的空白。

Data: 数据来源包括可见光和近红外波段观测数据，以及未来可能部署的大视场亚毫米望远镜（sub-mm telescope）与宽场光学积分视场光谱仪（IFU）巡天（如CATARSIS）的协同观测。

Method: 采用大直径亚毫米(sub-mm)望远镜结合宽视场光学积分视场单元(IFU)巡天技术，实现星系动力学与气体热力学联合观测。

Result: 大直径亚毫米(sub-mm)望远镜与宽视场光学积分视场单元(IFU)巡天相结合，可实现对星系动力学和气体热动力学的联合观测，从而革新对环境过程的理解。
...
### [36] [Invisible gravitons and large-scale magnetism](https://arxiv.org/abs/2512.14307)
*Massimo Giovannini*

**TL;DR**: 该研究揭示了引力波遗迹信号在aHz（亚赫兹）区域被显著抑制，而在kHz和GHz频段相对较强，并通过修改暴胀后时间线、超电荷场演化和大尺度磁性的约束，进一步限制了核合成前的膨胀率，同时发现MHz至THz频段的引力波遗迹信号可能与磁生成要求兼容，而音频域引力波谱能量密度的峰值会增强原星系引力坍缩尺度与哈勃半径相当时的磁场强度。


<details>
  <summary>Details</summary>

Background: 研究探讨了引力波遗迹信号在不同频段的限制条件，重点关注了aHz（attohertz）区域的谱能量密度抑制及其与暴胀后时间线、核合成前膨胀率的关系。

Data: 数据来源于对引力波遗迹信号（relic gravitons）的频谱能量密度在不同频段（包括音频带、nHz、kHz、GHz、MHz、THz）的理论分析和数值模拟，结合了膨胀后时间线修正、超电荷场演化以及大规模磁性的约束条件。

Method: 采用修改后的暴胀后时间线（modified postinflationary timeline）分析引力波遗迹信号，结合超电荷场（hypercharge field）演化和大尺度磁场的约束来限制核合成前的膨胀率。

Result: 引力波遗迹信号的谱能量密度在aHz（亚赫兹）区域被显著抑制，而在kHz和GHz频段可能相对较大。
...
### [37] [A sub-ppm upper limit on the cosmological variations of the fine structure constant alpha](https://arxiv.org/abs/2512.14441)
*S. Muller,A. Beelen,M. Guelin,J. H. Black,F. Combes,H. L. Bethlem,M. Gerin,C. Henkel,K. M. Menten,M. T. Murphy,W. Ubachs,N. Wozny*

**TL;DR**: 通过ALMA同步观测甲基炔（CH）和水（H2O）的吸收谱线，在红移0.88582的PKS1830-211和0.68466的B0218+357方向上测得精细结构常数（alpha）变化上限分别为0.55 ppm和1.5 ppm（3σ置信水平），将宇宙半年龄时期的alpha变化约束精度提高了2-4倍。


<details>
  <summary>Details</summary>

Background: 利用高红移类星体的吸收光谱研究精细结构常数(alpha)和质子电子质量比(mu)在宇宙时间尺度上的潜在变化。

Data: 数据来源于ALMA（阿塔卡马大型毫米波/亚毫米波阵列）对PKS1830-211（z_abs=0.88582）和B0218+357（z_abs=0.68466）两个引力透镜类星体的同时观测，获取了甲基（CH）和水（H2O）的亚毫米波基态跃迁吸收谱线。

Method: 利用ALMA（阿塔卡马大型毫米波/亚毫米波阵列）同步观测甲基（CH）和水（H2O）的亚毫米基态跃迁谱线，通过分析吸收轮廓间的速度偏移来约束精细结构常数（alpha）和质子-电子质量比（mu）的宇宙学变化。

Result: 利用ALMA同步观测甲基(CH)和水(H2O)的吸收谱线，在红移0.88582的PKS1830-211(NE)和0.68466的B0218+357(SW)方向分别测得体速度偏移δv=-0.048±0.028 km/s和-0.13±0.14 km/s，对应精细结构常数(alpha)变化率的3σ上限|Δα/α|<0.55 ppm和1.5 ppm，将宇宙半年龄时标的约束精度提高了2-4倍。
...
### [38] [Primordial features and low-$\ell$ suppression from isocurvature modes in multi-field Higgs-$R^2$ inflation](https://arxiv.org/abs/2512.14455)
*Flavio Pineda,Luis O. Pimentel*

**TL;DR**: 研究了具有希格斯场（Higgs）和标量场（scalaron）非最小动能混合的希格斯-$R^2$暴胀模型中的原初扰动，发现非最小耦合参数$ξ_h$控制不同动力学机制：$ξ_h \sim \mathcal{O}(0.1)$时暴胀轨迹转向率引发瞬态多场效应并在标量功率谱留下局部特征，而$ξ_h\ll 1$时绝热谱无特征但等曲率模不完全衰减，最终在宇宙微波背景（CMB）功率谱中产生可观测信号。


<details>
  <summary>Details</summary>

Background: 研究探讨了存在希格斯场（Higgs）与标量子（scalaron）非最小动能混合情况下，希格斯-$R^2$暴胀模型中的原初扰动特性及其对宇宙微波背景（CMB）观测信号的影响。

Data: 数据来源于对希格斯场（Higgs）和标量场（scalaron）非最小动能混合下原初扰动的多场背景和线性扰动的全数值积分。

Method: 采用多场背景和线性扰动的全数值积分方法，重点分析非最小耦合参数$ξ_h$调控的动力学机制。

Result: 在希格斯-$R^2$暴胀模型中，非最小动能混合项（non-minimal kinetic mixing）的存在导致原初扰动呈现两种动力学机制：当非最小耦合参数$ξ_h \sim \mathcal{O}(0.1)$时，暴胀轨迹的转动率引发瞬态多场效应，在标量功率谱中留下局部特征；而当$ξ_h\ll 1$时，绝热谱趋于无特征，但等曲率模（isocurvature modes）未完全衰减并在暴胀末期残留可观测贡献。
...
### [39] [Kinematic Reconstruction of $Λ(t)$CDM Models](https://arxiv.org/abs/2512.14578)
*Pedro Augusto Silva Guillen,José Fernando de Jesus,Rodolfo Valentim da Costa Lima*

**TL;DR**: 研究通过宇宙计时器(CC)、Ia型超新星(Pantheon+\&SH0ES)和重子声学振荡(BAO)数据，对含时宇宙学常数Λ(z)和相互作用项Q(z)的Λ(t)CDM模型进行运动学参数化约束，发现ΩΛ1=0.02±0.14、gQ0=0.1±5.8和gQ1=0.06±0.67等参数与标准ΛCDM模型兼容但存在较大误差范围。


<details>
  <summary>Details</summary>

Background: 研究探讨了Λ(t)CDM（冷暗物质）模型中Λ(z)和Q(z)两种动力学参数化形式对宇宙学常数（cosmological constant）和相互作用项（interaction term）的约束。

Data: 使用了宇宙计时器（Cosmic Chronometers, CC）、Ia型超新星数据（Pantheon+&SH0ES）和重子声学振荡（BAO）数据。

Method: 采用线性展开参数化方法对Λ(t)CDM模型进行运动学分析，包括Λ(z)=Λ_0+Λ_1z和Q(z)=Q_0+Q_1z两种形式，并测试了恒定相互作用项Q(z)=Q_0的特殊情况。

Result: 对$Λ(z)$参数化模型，发现宇宙学常数（cosmological constant）的变率参数$Ω_{\Lambda1}=0.02\pm0.14$（68%置信水平）。
...
### [40] [kSZ for everyone: the pseudo-Cl approach to stacking](https://arxiv.org/abs/2512.14625)
*Lea Harscouet,Kevin Wolz,Amy Wayland,David Alonso,Boryana Hadzhiyska*

**TL;DR**: 提出了一种谐波空间(kSZ效应与星系动量场互相关)估计器，相比传统叠加方法具有计算效率高、统计误差易量化且能精确重建小尺度信息的优势。


<details>
  <summary>Details</summary>

Background: 研究主题为开发一种谐波空间（harmonic-space）估计器，用于测量运动学苏尼亚耶夫-泽尔多维奇效应（kSZ效应）与重建星系动量场之间的互相关。

Data: 数据来源于星系动量场重建和运动学Sunyaev-Zel'dovich效应(kSZ)的交叉相关分析。

Method: 采用谐空间（harmonic-space）估计器分析运动学苏尼亚耶夫-泽尔多维奇效应（kSZ）与重建星系动量场的互相关。

Result: 提出了一种谐波空间估计器（harmonic-space estimator），用于测量运动学苏尼亚耶夫-泽尔多维奇效应（kSZ）与重建星系动量场的互相关，相比传统叠加方法具有计算效率高、信息提取完整且统计误差可精确估计的优势。
...
### [41] [A universal scaling law for gravitational waves induced during inflation](https://arxiv.org/abs/2512.14670)
*Martin Teuscher,Ruth Durrer,Killian Martineau,Aurélien Barrau*

**TL;DR**: 研究了宇宙暴胀期间任意源场诱导的随机引力波背景（stochastic gravitational wave background），发现其张量谱指数（tensor spectral index）在最小假设下可由简单公式描述，且无论源场原始谱如何均保持标度不变性（scale invariance）。


<details>
  <summary>Details</summary>

Background: 研究探讨了宇宙暴胀（cosmological inflation）期间任意源场放大诱导的随机引力波背景（stochastic gravitational wave background）及其张量谱指数（tensor spectral index）的普适性规律。

Data: 研究中使用的数据来源于理论模拟（simulation）推导出的张量谱指数公式，基于宇宙暴胀（cosmological inflation）期间任意源场放大的随机引力波背景（stochastic gravitational wave background）假设。

Method: 采用随机引力波背景（stochastic gravitational wave background）分析方法，基于宇宙暴胀（cosmological inflation）期间任意源场放大的物理模型，推导出张量谱指数（tensor spectral index）的通用公式。

Result: 在慢滚暴胀（slow-roll inflation）模型中，引力波背景的诱导谱近乎尺度不变，其张量谱指数（tensor spectral index）与真空产生引力波的标准结果略有偏离。
...
### [42] [Clustering with Light (but Massive) Relics](https://arxiv.org/abs/2512.14672)
*Jason Kumar,Pearl Sandick,Shuting Xu*

**TL;DR**: 研究发现轻质量但大质量遗迹粒子（LiMRs）在≥eV质量范围内可构成不可忽略的暗物质组分，其早期大尺度聚集特性影响宇宙微波背景（CMB）弱透镜效应，同时规避ΔN_{eff}限制。


<details>
  <summary>Details</summary>

Background: 研究探讨了轻质量但大质量遗迹粒子（LiMRs）对早期宇宙物质成团性的影响及其作为暗物质组分的可能性。

Data: 数据来源于宇宙微波背景(CMB)弱引力透镜效应的观测数据以及针对轻质量但大质量遗迹粒子(LiMRs)的理论模拟(simulation)数据。

Method: 采用微扰理论（perturbation theory）和数值模拟（N-body simulation）分析轻质量遗迹粒子（LiMRs）对早期宇宙物质成团性的影响。

Result: 轻但大质量遗迹粒子（LiMRs）在≥eV质量范围（甚至>10 eV）可构成暗物质不可忽略的组分。
...
### [43] [Cosmic Strings Gravitational Wave Probe of Leptogenesis: Thermal, Non-thermal, Near-resonant and Flavourful](https://arxiv.org/abs/2512.14684)
*Anish Goshal,Angus Spalding,Graham White*

**TL;DR**: 在高能标下破缺全局或局域$U(1)_{\rm B-L}$对称性会同时产生重右手中微子(heavy right-handed neutrinos)的马约拉纳质量(Majorana masses)和宇宙弦网络(cosmic strings)，并通过宇宙弦演化与衰变产生可被引力波实验探测的随机背景信号，同时右手中微子衰变可通过轻子生成(leptogenesis)机制产生宇宙重子不对称性(baryon asymmetry)，研究确定了热与非热初始条件下轻子生成成功所需最轻右手中微子质量下限分别为$M_1 > 1.74 \times 10^{8}\,\mathrm{GeV}$和$M_1 > \mathcal{O}(10^{6})\,\mathrm{GeV}$，并发现近共振轻子生成可将质量下限放宽至TeV尺度，且味效应(flavour effects)挽救了被现有CMB或引力波测量排除的参数空间区域。


<details>
  <summary>Details</summary>

Background: 研究探讨了高能标下破缺全局或局域$U(1)_{\rm B-L}$对称性对重右手中微子马约拉纳质量生成、宇宙弦网络演化及其引力波背景信号与重子不对称性通过轻子生成机制（leptogenesis）产生的关联。

Data: 研究中使用的数据来源为理论模拟（simulation）数据，基于全局和局域$U(1)_{\rm B-L}$对称性破缺的模型演化及宇宙弦网络产生的随机引力波背景。

Method: 采用解析推导方法结合全味效应（full flavour effects）分析重马约拉纳中微子（Majorana neutrinos）质量下限及近共振轻子生成（near-resonant leptogenesis）参数空间。

Result: 在全局或局域$U(1)_{\rm B-L}$对称性破缺下，成功轻子生成（leptogenesis）要求最轻右手中微子质量$M_1$存在绝对下限：热初始条件为$1.74 \times 10^{8}\,\mathrm{GeV}$，非热初始条件为$\mathcal{O}(10^{6})\,\mathrm{GeV}$。
...
### [44] [Evolving the Loeb Scale](https://arxiv.org/abs/2512.13743)
*Oem Trivedi,Abraham Loeb*

**TL;DR**: 该研究通过将洛布标度（Loeb Scale）扩展为径向演化的实时分类方案，实现了基于日心距离（heliocentric distance）的异常指标动态计算和有效分数松弛方程，从而能够利用远距离稀疏数据对星际天体最终洛布等级进行早期稳定预测。


<details>
  <summary>Details</summary>

Background: 研究主题是发展星际物体（interstellar objects）的动态分类框架，将静态的Loeb尺度（Loeb Scale）扩展为随日心距离（heliocentric distance）演化的实时评估体系。

Data: 数据来源于对星际天体(interstellar objects)的实时径向演化观测数据，通过引入日心距(heliocentric distance)函数和有效分数松弛方程处理稀疏观测数据。

Method: 采用径向演化的实时分类方法，将异常指标推广为日心距离（heliocentric distance）函数并引入有效分数的弛豫方程（relaxation equation），从而整合记忆效应、滞后性和预测能力。

Result: 提出了一种将洛布标度（Loeb Scale）静态框架扩展为径向演化实时分类方案的微分公式化方法。
...
### [45] [The next frontier in exoplanet science: Imaging our neighbouring planetary systems](https://arxiv.org/abs/2512.13756)
*Ignas Snellen,Sebastiaan Haffert,Matthew Kenworthy,Tomas Stolker*

**TL;DR**: 论文提出利用下一代极大望远镜（ELTs）的高对比度成像技术（如PCS和GmagAO-X）在2035-2040年间对邻近红矮星周围数十颗小型温带岩石行星（rocky planets）进行大气表征，弥补现有透射光谱和掩星光谱技术受恒星表面不均匀性和变异性噪声限制的不足，并探讨了其在未来空间任务（Habitable Worlds Observatory和LIFE）时代的互补作用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用高对比度成像技术突破恒星噪声限制，以表征邻近恒星周围小型温带岩石行星（rocky planets）的大气特征，推动系外行星科学的前沿发展。

Data: 研究中使用的数据来源为下一代地面极端大望远镜（ELTs）的观测数据，包括欧洲南方天文台（ESO）的ELT上搭载的行星相机和光谱仪（PCS）以及GMT望远镜的GmagAO-X系统，计划在2035-2040年间投入运行。

Method: 采用高对比度成像（high-contrast imaging）技术结合下一代地面极端大望远镜（ELTs）的仪器（如PCS和GmagAO-X）进行系外行星大气表征。

Result: 下一代系外行星表征需依赖高对比度成像技术，聚焦邻近恒星周围小型温带岩石行星（rocky planets），极大望远镜（ELTs）将在此领域发挥核心作用。
...
### [46] [Direct imaging characterization of cool gaseous planets](https://arxiv.org/abs/2512.13766)
...
**TL;DR**: 该论文提出利用宜居世界观测站（HWO）的直接成像和光谱技术首次系统表征有效温度低于400 K的冷气体巨行星（cool gas giants），通过反射光光谱和偏振测量揭示其大气分子组成、云雾特征和温度结构，填补热系外行星与太阳系巨行星之间的观测空白，为行星形成、云微物理和光化学作用提供关键约束。


<details>
  <summary>Details</summary>

Background: 当前对类似木星和土星的冷气体巨行星（cool gas giant exoplanets）的观测仍受技术限制，亟需通过直接成像和光谱分析揭示其大气特性以填补热系外行星与太阳系巨行星间的观测空白。

Data: 研究利用哈勃太空望远镜（HST）、斯皮策太空望远镜（Spitzer）和未来哈勃世界观测站（HWO）的反射光光谱与偏振测量数据，结合地面大型望远镜（如VLT和Keck）的辅助观测。

Method: 利用反射光光谱学(reflected light spectroscopy)和偏振测量(polarimetry)技术直接成像并光谱分析有效温度低于400 K的冷气态巨行星。

Result: HWO（Habitable Worlds Observatory）将首次实现对有效温度低于400 K的系外气态巨行星（exo-Jupiters）的直接成像和反射光光谱表征，填补热系外行星与太阳系巨行星之间的观测空白。
...
### [47] [How Mass Flows Through Accretion Discs: A Spectral-Timing Vision for the 2040s](https://arxiv.org/abs/2512.13802)
...
**TL;DR**: 论文核心贡献是提出通过高采样率光学光谱时域分析（spectral-timing）同时解析连续谱和发射线，直接绘制吸积盘（accretion disc）中白矮星（white dwarfs）到超大质量黑洞（supermassive black holes）的尺度不变性（scale-invariant）物理过程。


<details>
  <summary>Details</summary>

Background: 研究聚焦于吸积盘（accretion disc）中质量与角动量传输机制这一天体物理学核心未解难题。

Data: 数据来源包括鲁宾-LSST（Rubin-LSST）发现的吸积系统、天基引力波观测站以及第三代地基和天基探测器。

Method: 采用高时间分辨率的光谱时序分析（spectral-timing）技术，同时解析连续谱和独立发射线成分。

Result: 吸积盘（accretion disc）物理过程具有从白矮星到超大质量黑洞的尺度不变性（scale-invariance）。
...
### [48] [Millimeter-Wavelength Observations of the Active Sun: Unveiling the Origins of Space Weather](https://arxiv.org/abs/2512.13813)
...
**TL;DR**: 毫米波(AtLAST)全盘高频率多频成像设施通过实时监测太阳大气层物理条件，为空间天气预测提供了革命性观测手段，首次实现可靠业务化预报。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升对太阳爆发事件影响的预测能力，以应对社会对空间服务的依赖需求。

Data: 数据来源为毫米波（millimeter-wavelength）观测，通过全盘面（full-disk）、高时间分辨率（high-cadence）、多频（multi-frequency）成像获取太阳大气层动态物理条件。

Method: 采用毫米波长（millimeter-wavelength）观测技术进行全太阳盘（full-disk）、高时间分辨率（high-cadence）及多频率（multi-frequency）成像。

Result: 毫米波长观测能直接获取太阳大气层中爆发性太阳活动起源区域的实时物理条件。
...
### [49] [The search for dark matter axions from neutron stars in the inner parsecs of the Milky Way with AtLAST](https://arxiv.org/abs/2512.13861)
*J. De Miguel,E. Hatziminaoglou,J. Prieto-Polo,J. D. Marrero-Falcón,Abaz Kryemadhi*

**TL;DR**: 研究发现银河系中心中子星（neutron star）磁层在厘米至亚毫米波段为轴子（axion）光子共振转换提供了理想条件，填补了轴子参数空间的研究空白。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用银河系中心中子星磁层（magnetosphere）中轴子（axion）与光子的共振转换探测暗物质轴子。

Data: data：研究利用银河系中心中子星磁层(magnetosphere)的极端磁场和高暗物质密度环境，在厘米至亚毫米波段进行轴子(axion)-光子转换观测。

Method: 基于中子星磁层(magnetosphere)的轴子-光子共振转换(axion-photon resonant conversion)机制探测暗物质轴子(dark matter axions)。

Result: 银河系中心的中子星磁层（magnetosphere）为通过轴子（axion）共振转化为光子探测暗物质提供了独特环境，其极端磁场和高暗物质密度使厘米至亚毫米波段的轴子-光子转化成为可能。
...
### [50] [Atangana-Baleanu Regularized Wavelet Compression For Astronomical Time-Series](https://arxiv.org/abs/2512.13916)
*Taylan Demir,Atakan Koçyiğit*

**TL;DR**: 提出了一种基于阿塔甘纳-巴莱努-卡普托(Atangana Baleanu Caputo)分数阶导数和米塔格-莱夫勒(Mittag Leffler)核的小波压缩方法，在抑制天文光变曲线噪声的同时更好地保留了微弱瞬变信号(transients)、耀斑(flares)和振荡(oscillations)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于天文光变曲线（light curves）的噪声抑制与微弱瞬变信号保留的压缩方法。

Data: data：研究使用天文光变曲线（astronomical light curves）作为观测数据来源。

Method: 采用基于阿塔甘纳-巴莱亚努-卡普托导数(Atangana Baleanu Caputo derivative)和非奇异米塔格-莱夫勒核(nonsingular Mittag Leffler kernel)的分数阶小波压缩方法，通过长记忆平滑抑制噪声并保留微弱瞬变信号。

Result: 提出了一种基于阿塔甘纳-巴莱亚努-卡普托导数（Atangana Baleanu Caputo derivative）和非奇异米塔格-莱弗勒核（nonsingular Mittag Leffler kernel）的分形小波压缩方法，在抑制噪声的同时有效保留了微弱瞬变信号。
...
### [51] [Detection and characterisation of submm transient sources with a large single-dish telescope](https://arxiv.org/abs/2512.13924)
*Mike Peel,Dave Clements,Tony Mroczkowski,Allen Foster*

**TL;DR**: 论文核心贡献是提出利用大型单天线射电望远镜（如AtLAST）在亚毫米波（submm）波段开展高灵敏度、大视场观测，以表征宇宙微波背景（CMB）望远镜发现的暂现源（transients）并探测更暗弱天体群。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用亚毫米波（submm）波段的大规模巡天观测探索时变天文现象，特别是借助宇宙微波背景（Cosmic Microwave Background）望远镜的高灵敏度探测器阵列提升探测能力。

Data: 数据来源包括阿塔卡马宇宙学望远镜（Atacama Cosmology Telescope）、南极望远镜（South Pole Telescope）的观测数据，以及未来西蒙斯天文台（Simons Observatory）和CCAT的预期观测数据。

Method: 采用亚毫米波段大视场单天线望远镜（如AtLAST）进行高灵敏度观测，结合宇宙微波背景（CMB）巡天望远镜的多探测器阵列技术。

Result: 大型单镜面望远镜（如AtLAST）可为亚毫米波（submm）波段瞬变源（transients）的后续表征及暗弱天体普查提供高灵敏度与大视场支持。
...
### [52] [The multiple facets of millisecond pulsar binaries](https://arxiv.org/abs/2512.13960)
...
**TL;DR**: 该论文核心贡献是发现毫秒脉冲星(MSP)双星系统在光学波段展现出多样且高效的辐射机制，为研究极端条件下的粒子加速、盘-脉冲星相互作用和双星内激波提供了新窗口。


<details>
  <summary>Details</summary>

Background: 毫秒脉冲星(MSP)双星为研究地球上无法实现的极端条件下的物质和辐射提供了独特实验室。

Data: 数据来源于高时间分辨率光学观测（high-time-resolution optical observations）、快速响应观测能力（rapid-response observing capabilities）以及中等光谱分辨率的时间分辨光谱（time-resolved spectroscopy at moderate spectral resolution）。

Method: 采用高时间分辨率光学观测、快速响应观测能力及中等光谱分辨率的时间分辨光谱(time-resolved spectroscopy)技术，结合旋转供能模型(rotation-powered models)和双星风相互作用(intrabinary wind interactions)物理机制分析。

Result: 光学毫秒脉冲星(MSP)双星系统展现出跨越不同吸积状态的多样化辐射机制，部分系统的光学效率远超旋转供能模型的预测。
...
### [53] [The ORCA-TWIN qCMOS Project I. Commissioning at Calar Alto Observatory](https://arxiv.org/abs/2512.14279)
...
**TL;DR**: 研究通过远程望远镜同步观测验证了新型qCMOS（quantum CMOS）图像传感器在时域天文（time domain astronomy）中高时间分辨率成像的性能优势，其综合表现优于传统CCD但略逊于EMCCD（electron-multiplying CCD）的极短曝光和暗源探测场景。


<details>
  <summary>Details</summary>

Background: 探索新型快速低噪声CMOS（qCMOS）图像传感器在时域天文学中的应用潜力。

Data: 使用两台相距1800公里的远程望远镜进行同步观测，数据来源包括新型qCMOS图像传感器（ORCA-Quest 2相机）在卡拉阿托天文台（CAHA）1.23米望远镜上的实测数据。

Method: 采用新型qCMOS（quantum CMOS）图像传感器技术进行直接成像，结合快速读出与低读出噪声特性。

Result: qCMOS（quantum CMOS）技术在1米级望远镜的高频成像中通常优于传统CCD（电荷耦合器件），但在极短曝光和暗弱目标情况下EMCCD（电子倍增CCD）仍具竞争力甚至略优。
...
### [54] [Development of a Central Trigger Processor board for the Advanced SiPM based camera of the CTA Large-Sized Telescopes](https://arxiv.org/abs/2512.14343)
...
**TL;DR**: 该论文介绍了用于切伦科夫望远镜阵列观测站（CTAO）大型望远镜（LST）相机的中央触发处理器板（CTPb）设计，通过全数字触发提高低能γ射线事件与夜天光背景噪声的区分能力，从而增强望远镜灵敏度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于切伦科夫望远镜阵列观测站（CTAO）下一代大型望远镜（LST）相机的硅光电倍增管（SiPM）触发系统开发，旨在提升低能伽马射线事件与夜天光背景噪声的鉴别能力。

Data: 数据来源于下一代触发架构中正在评估的硬件原型（hardware prototypes）。

Method: 采用全数字触发（fully digital trigger）技术，利用高分辨率提升低能γ射线事件与夜天光背景噪声（Night Sky Background noise）的区分能力。

Result: 中央触发处理器板（CTPb）采用全数字触发设计，利用更高分辨率提升低能γ射线事件与夜天背景噪声的区分能力，从而提高望远镜灵敏度并满足严格的时序和数据速率限制。
...
### [55] [QUIJOTE-TFGI polarization calibration -- Ground characterization and on-sky validation with Tau A and the Moon](https://arxiv.org/abs/2512.14407)
...
**TL;DR**: 该研究通过地面参考信号（二极管）和天体源（Tau A与月球）校准QUIJOTE三十和四十千兆赫兹仪器（TFGI），验证了仪器相位切换误差角度在2σ精度内无需修正，测得31 GHz下月球折射率为1.209±0.007（统计）±0.005（系统），并发现通道间相对响应稳定性良好但整体响应存在时间变化。


<details>
  <summary>Details</summary>

Background: 研究聚焦于QUIJOTE三十和四十千兆赫兹仪器(TFGI)的校准与性能表征，涉及地面参考信号与天体源(如Tau A和月球)的交叉验证。

Data: 数据来源包括地面参考校准信号（二极管）和天基观测（Tau A和月球）。

Method: 采用参考校准信号（二极管）拟合数据以解析仪器角度间的简并性，并利用Tau A和月球的天文观测验证系统响应及时稳定性。

Result: 通过地面参考校准信号（二极管）和天体源（Tau A与月球）观测验证，QUIJOTE三十和四十千兆赫仪器（TFGI）的偏振角度与响应度校准结果一致，且相位切换误差角度在2σ精度内与0度吻合无需额外修正。
...
### [56] [Why the Northern Hemisphere Needs a 30-40 m Telescope and the Science at Stake: Time-domain astronomy](https://arxiv.org/abs/2512.14470)
...
**TL;DR**: 论文提出在拉帕尔马（La Palma）建设30-40米北半球光学/红外极大望远镜（ELT）的科学必要性，其核心能力（快速响应、宽波段覆盖、高时间分辨率等）将有效捕捉快速演化的暂现源（transients）早期关键阶段，填补多信使观测空白，并推动吸积现象研究。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于为2040年代变革性时域天文学（time-domain astronomy）优化设计的30-40米北半球光学/红外望远镜（optical/infrared telescope）的科学需求与战略定位。

Data: 数据来源于北半球拉帕尔马(La Palma)站点的30-40米光学/红外望远镜(optical/infrared telescope)观测。

Method: 采用快速响应、宽波段覆盖、高时间分辨率、偏振测量能力和衍射极限成像的北方极大望远镜（Northern ELT）技术手段。

Result: 在2040年代，北半球建造一台30-40米光学/红外望远镜（Northern ELT）对捕捉快速演化的暂现源（transients）早期关键阶段、获取深度光谱及光变数据至关重要。
...
### [57] [Interfacing adaptive optics simulations with the optical model: a powerful tool for MORFEO](https://arxiv.org/abs/2512.14523)
*Giorgio Pariani,Guido Agapito,Demetrio Magrin,Matteo Munari,Lorenzo Busoni,Marco Riva,Andrea Di Rocco,Paolo Ciliegi*

**TL;DR**: 开发了名为ASSO（Adaptive opticS Simulation interfaced with Optical model）的集成建模工具，通过结合PASSATA模拟器、Zemax OpticStudio光学建模和Matlab接口软件，预测欧洲极大望远镜（ELT）多共轭自适应光学（MCAO）模块的波前误差校正性能。


<details>
  <summary>Details</summary>

Background: 研究背景为开发用于欧洲极大望远镜（ELT）多共轭自适应光学（MCAO）模块的集成建模工具ASSO（Adaptive opticS Simulation interfaced with Optical model），以预测自适应光学中继性能并估计校正后的波前误差。

Data: 数据来源基于IDL模拟器PASSATA、Zemax OpticStudio光学建模软件以及Matlab接口软件。

Method: 开发了基于PASSATA（PyrAmid Simulator Software for Adaptive opTics Arcetri）、Zemax OpticStudio光学建模和Matlab接口软件的集成建模工具ASSO（Adaptive opticS Simulation interfaced with Optical model）。

Result: 开发了名为ASSO（Adaptive opticS Simulation interfaced with Optical model）的集成建模工具，用于将光学模型与自适应光学（Adaptive Optics）仿真相连接，以预测自适应光学中继的性能并估计校正后的波前误差（wavefront error）。
...
### [58] [Multi-messenger and time-domain astronomy in the 2040s](https://arxiv.org/abs/2512.14546)
...
**TL;DR**: 这篇论文的核心贡献是论证2040年代多信使（multi-messenger）天文学的发展需要配备大口径、低延迟、高吞吐量的光学至近红外光谱观测设施，以充分挖掘引力波、高能中微子、宇宙射线等跨信使数据在核合成、致密物质物理、宇宙学等领域的科学潜力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于2040年代多信使（multi-messenger）天文学在引力波、高能中微子、宇宙线和时域观测等领域的突破性发展及其对科学问题的广泛影响。

Data: 研究使用的数据来源包括下一代引力波（gravitational wave）、高能及超高能中微子（ultra-high energy neutrino）、宇宙射线（cosmic ray）和时域观测台（time domain observatories）的探测数据。

Method: 采用光学至近红外光谱技术（optical to near infrared spectroscopic capability），包括低延迟响应、高通量和灵活的时域操作。

Result: 2040年代多信使（multi-messenger）天文学将因引力波、高能及超高能中微子、宇宙线和时域观测设备的海量探测数据而发生革命性变革。
...
### [59] [The Dynamics of the Milky Way: Unveiling the 6D Skeleton of Star Formation in the 2040s](https://arxiv.org/abs/2512.14606)
...
**TL;DR**: 该论文以银河系(Milky Way)为研究对象，通过分析年轻恒星族群的分布探讨了大规模动力学不稳定性（如翘曲和垂直波）对薄盘恒星形成的驱动机制、恒星形成区的触发机制（随机自传播反馈或银河尺度动力学过程）以及恒星形成环境（星团或弥散结构）三大核心问题。


<details>
  <summary>Details</summary>

Background: 银河系（Milky Way）作为研究恒星形成理论的独特实验室（Rosetta Stone），旨在通过年轻星族揭示其结构与演化机制，并探讨大尺度动力学不稳定性（如翘曲和垂直波）对恒星形成的影响。

Data: 数据来源包括银河系(Milky Way)年轻恒星族群的观测数据以及相关动力学过程的大尺度数值模拟(simulation)结果。

Method: 采用多尺度动力学模拟（multi-scale dynamical simulations）结合年轻恒星族群的观测数据，重点分析大尺度动力学不稳定性（如翘曲和垂直波）对恒星形成的影响。

Result: 银河系(MW)是研究单个恒星层面恒星形成理论的独特实验室，可作为解读河外星系观测的罗塞塔石碑(Rosetta Stone)。
...
### [60] [Attention-Based Preprocessing Framework for Improving Rare Transient Classification](https://arxiv.org/abs/2512.14644)
*Xinyue Sheng,Tuan Dung Pham,Zichi Zhang,Matt Nicholl,Thai Son Mai*

**TL;DR**: 提出了一种结合图像和光变曲线数据增强的流程，通过相似性指数去除图像伪影、掩膜处理保留宿主星系（host galaxy）和暂现源（transient），并利用二维高斯过程（Gaussian Process）生成合成样本，显著提高了I型超亮超新星（SLSNe-I）和潮汐瓦解事件（TDEs）的分类纯度（75%和43%）与完备性（75%和66%）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用机器学习增强技术（如数据增强和特征提取）解决瞬变天体（transient）分类中因训练集类别极度不平衡和宿主星系图像干扰导致的稀有类型（如I型超亮超新星和潮汐瓦解事件）识别难题。

Data: 数据来源包括观测到的多波段光变曲线（light curves）和宿主星系图像（host galaxy images），通过二维高斯过程（two-dimensional Gaussian Process）拟合光变曲线并生成数据驱动的合成样本，同时与同类星系图像交叉匹配以产生新的训练样本。

Method: 采用相似性指数（Similarity Index）去除图像伪影并结合掩膜程序剔除无关光源，同时保留瞬变源及其宿主星系（host galaxy），并利用二维高斯过程（Gaussian Process）拟合多波段光变曲线以生成数据驱动的合成样本。

Result: 提出了一种针对图像和光变曲线的数据增强流程，通过相似性指数去除图像伪影并结合掩膜处理保留瞬变源及其宿主星系，同时采用二维高斯过程拟合多波段光变曲线并通过重采样与红移生成数据驱动的合成样本，显著提升了I型超亮超新星（SLSNe-I）和潮汐瓦解事件（TDEs）的分类纯度（置信度≥0.8时分别达到75%和43%）与完备性（75%和66%）。
...
### [61] [XID+PRIMA, II: Stepping Through Hyperspectral Imaging to Deblend PRIMAger Beyond the Extragalactic Confusion Limit](https://arxiv.org/abs/2512.13682)
*J. M. S. Donnellan,B. Pautasso,S. J. Oliver,M. Béthermin,L. Bing,A. Bolatto,L. Ciesla,D. Koopmans,A. Pope,S. Serjeant,L. Wang*

**TL;DR**: 提出了一种新的贝叶斯建模方法XID+stepwise，利用PRIMAger（PRobe far-Infrared Mission for Astrophysics）的高光谱成像能力，通过从短波到长波顺序传播约束，在45-235微米波段实现了超出混淆极限（confusion limit）1.3-7倍的灵敏度提升，并成功探测到红移z~2处红外明亮星系（IR-luminous galaxies）在多数通道的辐射。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用PRIMAger远红外（FIR）任务的高光谱成像能力，突破传统远红外空间望远镜在25-235微米波段的光谱覆盖与灵敏度限制，以解决星系演化中远红外光谱能量分布（SED）的高精度测量难题。

Data: 数据来源于合成的深场成像巡天数据，包含真实仪器噪声和混淆噪声（confusion noise），并基于最新PRIMAger仪器参数生成。

Method: 采用贝叶斯建模方法XID+stepwise，利用PRIMAger的高光谱成像技术，通过从短波长到长波长顺序传播约束，推导自洽且信息丰富的流量先验。

Result: PRIMAger任务采用新型贝叶斯建模方法XID+stepwise，通过从短波到长波顺序传播约束，在45-84微米波段将流量测量精度提升至0.2-0.7毫央（mJy），达到比混淆极限（confusion limit）暗1.3-3.4倍的灵敏度。
...
### [62] [Stochastic Inflation in Numerical Relativity](https://arxiv.org/abs/2512.14649)
*Yoann L. Launay,Gerasimos I. Rigopoulos,E. Paul S. Shellard*

**TL;DR**: 这篇论文的核心贡献是采用规范不变方式重新推导了包含所有度规和标量物质自由度的3+1维随机暴胀方程，并通过BSSN数值相对论框架验证了其在慢滚和超慢滚情景下的数值稳健性，为非微扰现象预测和后续宇宙演化提供了精确初始条件。


<details>
  <summary>Details</summary>

Background: 研究主题为在随机暴胀（stochastic inflation）框架下发展并验证包含所有度规（metric）和标量物质（scalar matter）自由度的3+1方程组的规范不变性推导及数值实现。

Data: 数据来源于数值模拟（simulation），采用BSSN（Baumgarte-Shapiro-Shibata-Nakamura）数值相对论框架实现随机方程在慢滚（slow-roll）和超慢滚（ultra slow-roll）情景下的非线性随机动力学演化，并保留梯度信息。

Method: 采用BSSN（Baumgarte-Shapiro-Shibata-Nakamura）数值相对论框架对包含所有度规和标量物质自由度的3+1维随机暴胀方程进行规范不变性重构与数值实现。

Result: 在BSSN（Baumgarte-Shapiro-Shibata-Nakamura）数值相对论框架下实现的随机方程数值模拟，成功复现了所有动力学变量的演化过程并良好满足能量和动量约束条件，证实了该随机方程在理论和数值上的鲁棒性。
...
### [63] [Search for Gravitational Wave Memory in PPTA and EPTA Data: A Complete Signal Model](https://arxiv.org/abs/2512.14650)
...
**TL;DR**: 首次基于全数值相对论波形（含非线性引力波记忆效应）在欧洲和澳大利亚脉冲星计时阵（PTA）数据中搜寻超大质量黑洞双星（SMBHBs）晚期旋近与合并信号，并以95%置信度排除了在700兆秒差距内啁啾质量（chirp mass）达10^10太阳质量的SMBHBs合并事件及短时全天空或长期特定天区应变幅度>10^-14的通用位移记忆暴发事件。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用欧洲和澳大利亚的脉冲星计时阵（PTA）数据探测超大质量黑洞双星（SMBHBs）产生的引力波记忆效应（gravitational wave memory）。

Data: 数据来源于欧洲和澳大利亚的两个主要脉冲星计时阵列（PTA）实验的观测数据。

Method: 采用基于完整数值相对论波形（包含非线性引力波记忆效应）的分析方法，结合核密度估计和归一化流近似技术降低计算成本。

Result: 首次基于完整数值相对论波形（含非线性引力波记忆效应）对超大质量黑洞双星（SMBHBs）晚期旋近与合并进行了搜索，并探索了通过核密度与归一化流近似降低通用零记忆引力波暴搜索计算成本的可能性。
...
### [64] [Any Light Particle Searches with ALPS II: first science results](https://arxiv.org/abs/2512.14110)
...
### [65] [Spirals and Vertical Motions in the Planet-Forming Disk around HD 100546. A multi-line study of its gas kinematics](https://arxiv.org/abs/2512.13814)
...
**TL;DR**: 论文通过ALMA观测HD100546（原恒星）五条CO分子谱线，揭示了其原行星盘中存在动力学螺旋结构、伴星存在证据（50au内及90-150au区域）以及首次探测到与尘埃间隙重合的垂直下流气体运动，表明该系统中存在多尺度行星形成活动。


<details>
  <summary>Details</summary>

Background: 研究HD100546系统中行星-盘动力学相互作用（planet-disk interaction）及行星形成过程（planet formation）的直接和间接观测证据。

Data: 数据来源于ALMA（Atacama Large Millimeter/submillimeter Array）观测的HD 100546系统中五种分子CO发射谱线（包括12CO 2-1、13CO和C18O）的谱线强度通道数据。

Method: 采用Discminer包对分子强度通道进行线轮廓建模，并分别对蓝移和红移侧进行独立拟合以检测不对称性。

Result: 在HD100546的原行星盘(protoplanetary disk)中发现了显著的运动学螺旋结构(kinematical spiral features)，其螺距角(pitch angles)与50天文单位(au)内嵌入伴星驱动的动力学机制一致，而非引力不稳定性所致。
...
### [66] [Vertically resolved minimal-set k-distribution for thermal infrared absorption in the Venus atmosphere](https://arxiv.org/abs/2512.14120)
*Boris Fomin,Mikhail Razumovskiy*

**TL;DR**: 开发了一种基于k分布技术(k-distribution technique)的垂直分辨率参数化方法，用于金星中低层大气热辐射吸收的高效计算，在保持精度（90公里以下冷却率误差约1 K/天、通量误差低于1%）的同时将k项数量减少至典型相关k方法的1/2.5-1/3。


<details>
  <summary>Details</summary>

Background: 研究主题为金星（Venus）中低层大气热辐射吸收的参数化方法开发，旨在为气候建模等需要高效简化参数化的场景提供解决方案。

Data: 数据来源为基于蒙特卡洛辐射传输方法（Monte Carlo radiative transfer method）的逐线参考模型（line-by-line reference modeling），用于构建k项（k-terms），并在10–6000 cm^{-1}范围内使用16个光谱波段生成32个k项和16个每波段光谱点（per-band spectral points）来计算金星云光学特性。

Method: 采用改进的k分布(k-distribution)技术参数化金星中低层大气热辐射吸收，基于蒙特卡洛辐射传输(Monte Carlo radiative transfer)的逐线(line-by-line)参考建模构建高度依赖的k(z)函数，在10-6000 cm^{-1}范围内划分16个光谱带并生成32个k项。

Result: 该方法通过16个光谱波段生成32个k项和16个每波段光谱点，用于计算金星云光学特性，在90公里以下精度合理（冷却率误差约1 K/天，通量误差低于1%），且k项数量比典型相关k方法减少约2.5-3倍。
...
### [67] [Hot Jupiters in Old Wide-Binary Systems](https://arxiv.org/abs/2512.13773)
*Evgeni Grishin,Jet Winter,Jaime A. Alvarado-Montes*

**TL;DR**: 研究发现宽双星系统（wide binaries）受银河潮汐场扰动可将冷木星（Cold Jupiters）转化为热木星（Hot Jupiters），解释了26%-40%的观测热木星数量，并自然产生自由漂浮巨行星（free-floating giant planets），同时解决了冷/热木星比例过高的问题。


<details>
  <summary>Details</summary>

Background: 研究聚焦于热木星（Hot Jupiters, HJs）的起源机制，探讨了高偏心率迁移（high-eccentricity migration）模型在解释其形成过程中的不足，并提出了银河潮汐场扰动下的宽双星系统（wide binaries）作为关键驱动因素。

Data: 研究中使用的数据来源包括对宽双星系统（wide binaries）在银河潮汐场（Galactic tidal field）扰动下的动力学模拟（simulation），以及观测到的热木星（Hot Jupiters）和冷木星（Cold Jupiters）的轨道参数、恒星自转轨道不对齐（stellar spin-orbit misalignments）分布和宿主星年龄等观测数据（observation）。

Method: 采用动力学框架分析宽双星系统（wide binaries）在银河系潮汐场扰动下对热木星（Hot Jupiters）形成的影响。

Result: 宽双星（wide binaries）在银河系潮汐场扰动下产生的热木星（Hot Jupiters）数量比孤立双星系统多1.8±0.14倍，解释了26%-40%的观测热木星数量。
...
### [68] [ExoMol line lists -- LXIII: ExoMol line lists for 12 isotopologues of CO$_2$](https://arxiv.org/abs/2512.13889)
...
**TL;DR**: 构建了12种二氧化碳同位素分子（包括$^{12}$C$^{16}$O$_2$等）的精细转动振动谱线表，结合变分程序TROVE、精确动力学能量算子、经验势能面（Ames-2）和偶极矩面（Ames-2021-40K），并通过机器学习估算AFGL量子数补充数据，最终生成适用于多种辐射传输代码的透明度数据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于构建12种二氧化碳同位素分子（isotopologues）的高精度振转光谱线列表（rovibrational line lists），以支持行星大气光谱建模和同位素丰度分析。

Data: 数据来源于变分程序TROVE结合精确动能算符、经验势能面（Ames-2）和从头算偶极矩面（Ames-2021-40K），并整合了MARVEL分析、HITRAN和CDSD数据库中的经验能级数据，同时通过机器学习估计器补充了AFGL量子数。

Method: 采用变分程序TROVE结合精确动能算符、经验势能面(Ames-2)和从头算偶极矩面(Ames-2021-40K)构建二氧化碳同位素分子的转振谱线列表。

Result: 构建了12种二氧化碳同位素体（isotopologues）的广泛振转谱线列表，包括$^{12}$C$^{16}$O$_2$、$^{13}$C$^{16}$O$_2$等，并采用变分程序TROVE结合精确动能算符、经验势能面（Ames-2）和从头算偶极矩面（Ames-2021-40K）进行计算。
...
### [69] [Demonstrating sub-picometer non-reciprocity levels in the Three-Backlink Experiment for LISA](https://arxiv.org/abs/2512.14484)
...
**TL;DR**: 通过三反向链路实验（3BL）验证了三种反向链路（Backlink）配置在激光干涉仪空间天线（LISA）频段内均实现亚皮米级非互易性（non-reciprocity）性能，并揭示了光纤方案中背向散射噪声（backscatter noise）的关键影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于空间引力波探测器中双向光学连接（Backlink）的非互易性（non-reciprocity）对干涉测量精度的影响及其优化方案。

Data: 数据来源于激光干涉仪空间天线（LISA）框架下设计的三反向链路实验（3BL），该实验使用包含两个可旋转光学平台的光学测试设施，研究了两种基于光纤和一种自由光束的反向链路配置的性能。

Method: 采用双向光学连接（Backlink）性能测试实验（3BL）评估三种配置（两种光纤方案和一种自由光束方案）的非互易性（non-reciprocity）噪声水平，重点分析光纤背向散射噪声（backscatter noise）对本地干涉测量的影响。

Result: 双向光链路（Backlink）的非对称性会导致反向传播光束的有效光程差引入差分相位噪声（非互易性），从而限制时间延迟干涉测量（TDI）后处理的精度。
...
### [70] [Construction techniques and commissioning of the Three-Backlink Experiment for the LISA mission](https://arxiv.org/abs/2512.14485)
...
**TL;DR**: 构建了用于测试LISA（激光干涉空间天线）任务中相位参考回传链路（Backlink）的超稳定准单片光学实验平台，验证了三种回传链路方案在0.1mHz至1Hz频段内性能上限均达到15pm/√Hz水平。


<details>
  <summary>Details</summary>

Background: 研究聚焦于为未来空间引力波探测任务（如LISA）开发超高稳定度光学测试平台，以验证不同相位参考传递方案（Backlink）在微重力环境下的性能极限。

Data: 研究中使用的数据来源于Three-Backlink Experiment（三反向链路实验）中三种反向链路（Backlink）方案的实验测量结果，包括直接光纤、频率分离光纤和自由光束链路的性能比较。

Method: 采用超稳定准单片光学测试台（ultra-stable quasi-monolithic optical testbed）比较三种背向链路（Backlink）方案，并开发专用对准技术和高精度光束对准测量工具（Calibrated Quadrant Photodiode Singleton）。

Result: 通过构建和调试超稳定准单片光学测试平台（Three-Backlink Experiment），验证了三种Backlink方案（直接光纤、分频光纤和自由光束链路）在LISA频段（0.1mHz至1Hz）的性能上限均能达到15pm/√Hz等效水平。
...
### [71] [The Alignment of High-resolution Solar Prominence Images Observed by the New Vacuum Solar Telescope](https://arxiv.org/abs/2512.14201)
*Yunfang Cai,Yongyuan Xiang,Kaifan Ji*

**TL;DR**: 提出了一种结合日面光流场、太阳边缘梯度和日珥(protuberance)区域互相关的综合对齐方法，有效解决了新真空太阳望远镜(NVST)拍摄的高分辨率日珥图像在不同时间和波段下的高精度对齐难题。


<details>
  <summary>Details</summary>

Background: 太阳日珥（solar prominence）高空间分辨率观测是新真空太阳望远镜（NVST）的重要观测目标。

Data: 数据来源于新真空太阳望远镜（NVST）获取的高空间分辨率日珥（solar prominence）观测图像。

Method: 提出了一种结合太阳表面光流场、太阳边缘梯度和日珥区域互相关的综合配准方法。

Result: 提出了一种结合太阳表面光流场、太阳边缘梯度和日珥区域互相关的综合配准方法，能够高精度确定日珥图像间的运动位移。
...
### [72] [New Bulgarian-Austrian project 'Joint observations and investigations of solar chromospheric and coronal activity'](https://arxiv.org/abs/2512.14296)
...
**TL;DR**: 该论文介绍了保加利亚与奥地利在色球层（chromosphere）和日冕活动领域的双边合作项目，核心贡献是通过建立罗詹色球层望远镜（Rozhen chromospheric telescope）的技术配置与校准方案、联合多国地基与空间观测数据开展太阳现象分析，提升太阳监测能力并促进科研与公众教育。


<details>
  <summary>Details</summary>

Background: 研究聚焦于保加利亚与奥地利在太阳色球层(chromosphere)和日冕(coronal)活动领域的双边合作，旨在提升太阳和空间天气(solar and space weather)监测能力。

Data: 数据来源包括保加利亚国家天文台（NAO）Rozhen色球望远镜（chromospheric telescope）的地面观测数据、两国其他地基仪器（ground-based instruments）的观测数据以及补充的航天器（spacecraft）数据。

Method: 采用机器/深度学习模型进行特征识别，并利用地面仪器和航天器数据进行联合科学分析。

Result: 保加利亚和奥地利研究人员合作开展色球层（chromosphere）和日冕活动研究，重点包括优化Rozhen色球层望远镜观测方案、自动化数据处理及机器学习特征识别。
...
### [73] [Hydrodynamical mass-loss rates for Very Massive Stars. I. Investigating the wind kink](https://arxiv.org/abs/2512.13782)
*Gautham N. Sabhahit,Jorick S. Vink,Andreas A. C. Sander*

**TL;DR**: 研究发现极高质量恒星(VMS)在电子散射爱丁顿比(Γₑ)≈0.43（对应质量≈60M⊙）处存在质量损失率急剧上升的"拐点"，证实了单次散射极限被突破时恒星风从光学薄到光学厚的转变，并识别出由铁电离态变化驱动的两个双稳态跳变。


<details>
  <summary>Details</summary>

Background: 研究聚焦于极高质量恒星（VMSs）辐射驱动星风的质量损失率及其在突破单散射极限时出现的"拐点"现象。

Data: 数据来源为使用PoWR$^\mathrm{HD}$代码计算的水动力学一致的非局域热动平衡(NLTE)大气模型，网格参数覆盖40-135倍太阳质量($M_\odot$)、12-50千开尔文(kK)，固定$\log(L_\star/L_\odot) = 6.0$及太阳金属丰度($Z=0.02$)。

Method: 采用流体动力学一致的非局域热动平衡（non-LTE）大气模型（PoWR$^\mathrm{HD}$代码）分析辐射驱动星风（radiation-driven winds）的光学深度转变和质量损失率突变（kink）。

Result: 在极高质量恒星(VMS)中，当电子散射爱丁顿比(Γₑ)达到约0.43时，质量损失率会出现急剧上升的"拐点"(kink)，该现象与蒙特卡洛模拟预测一致且不依赖于恒星质量的不确定性。
...
### [74] [The chemical DNA of the Magellanic Clouds V. R-process dominates neutron capture elements production in the oldest SMC stars](https://arxiv.org/abs/2512.13789)
...
**TL;DR**: 通过分析12颗金属贫乏（[Fe/H]介于-2.3至-1.4）的小麦哲伦云（SMC）巨星中α元素和中子俘获元素的丰度，发现这些最古老的SMC恒星表现出α元素增强但低于银河系贫金属星的水平，且中子俘获元素（尤其是r-过程元素Eu和Sm）存在显著星间离散性，证实早期SMC的r-过程效率极高但受限于随机生产源和低效气体混合，同时s-过程元素（Y、Ba、La、Ce、Nd）丰度低于太阳且与r-过程元素正相关。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过分析12颗金属贫乏（metal-poor）的小麦哲伦云（SMC）巨星中Fe、α元素及中子俘获元素（neutron-capture elements）的化学丰度，揭示该星系早期恒星形成和核合成过程（nucleosynthesis）的独特特征。

Data: 数据来源于使用高分辨率光谱仪UVES/VLT和MIKE/Magellan观测的12颗金属贫乏的小麦哲伦云(SMC)巨星的高分辨率光谱。

Method: 采用高分辨率光谱仪UVES/VLT和MIKE/Magellan观测12颗金属贫乏的小麦哲伦云(SMC)巨星中Fe、α元素和慢中子俘获过程(s-process)元素的化学丰度，并结合定制的随机化学演化模型进行分析。

Result: result：SMC（Small Magellanic Cloud）金属贫乏巨星中快中子俘获过程（r-process）元素（Eu和Sm）丰度比从太阳值到+1 dex不等，其中三颗星[Eu/Fe]>+0.7 dex且被归类为r-II星，表明SMC中r-process效率极高但受限于随机性生产位点和早期气体混合效率低下。

Abstract: We present the chemical abundances of Fe, alpha- and neutron-capture elements in 12 metal-poor Small Magellanic Cloud (SMC) giant stars, observed with the high-resolution spectrographs UVES/VLT and MIKE/Magellan. These stars have [Fe/H] between -2.3 and -1.4 dex, 10 of them with [Fe/H]<-1.8 dex. According to theoretical age-metallicity relations for this galaxy, these stars formed in the first Gyr of life of the SMC and represent the oldest SMC stars known so far. [alpha/Fe] abundance ratios are enhanced but at a lower level than MW metal-poor stars, as expected according to the slow star formation rate of the SMC. The sample exhibits a large star-to-star scatter in all the neutron-capture elements. The two r-process elements measured in this work (Eu and Sm) have abundance ratios from solar up to +1 dex, three of them with [Eu/Fe]>+0.7 dex and labeled as r-II stars. This [r/Fe] distribution indicates that the r-process in the SMC can be extremely efficient but is still largely affected by the stochastic nature of the main sites of production and the inefficient gas mixing in the early SMC evolution. A similar scatter is observable also for the s-process elements (Y, Ba, La, Ce, Nd), with the stars richest in Eu also being rich in these s-elements. Also, all the stars exhibit subsolar [s/Eu] abundance ratios. At the metallicities of these stars, the production of neutron-capture elements is driven by r-process, because the low-mass AGB stars have not yet evolved and left their s-process signature in the interstellar medium. We also present stochastic chemical evolution models tailored for the SMC that confirm this scenario.

</details>

### [75] [A Bridge between Young Stars Formed by Gravitational Interaction](https://arxiv.org/abs/2512.14299)
*Youngwoo Choi,Woojin Kwon,Leslie W. Looney,John J. Tobin*

**TL;DR**: 研究发现L1448 IRS3A和L1448 IRS3B年轻恒星系统之间存在潮汐剥离桥（tidal bridge）和盘结构扰动，证实两者约1.5万年前发生过动力学飞掠（dynamical flyby），为恒星相互作用重塑原行星盘（protoplanetary disk）结构提供了直接证据。


<details>
  <summary>Details</summary>

Background: 研究主题为恒星形成过程中密集星团环境下年轻恒星系统（young stellar systems）及其行星形成盘（planet-forming disks）之间的动力学相互作用（dynamical interactions）及其对盘结构的重塑影响。

Data: 数据来源于毫米波（millimeter wavelength）观测和流体动力学模拟（hydrodynamical simulation）。

Method: 采用流体动力学模拟（hydrodynamical simulations）重现观测特征并推断动力学飞掠事件的时间尺度。

Result: 在L1448 IRS3A和L1448 IRS3B年轻恒星系统之间观测到潮汐剥离桥（tidal bridge）及盘结构扰动，结合流体动力学模拟证实两者约1.5万年前经历动力学飞掠（dynamical flyby），为恒星相互作用重塑盘结构及影响多星系统形成提供了直接证据。
...
### [76] [Quantum Walks and Exact RG in de Sitter Space](https://arxiv.org/abs/2512.13842)
*Daniel Green,Kshitij Gupta*

**TL;DR**: 该论文通过精确重正化群（exact RG）框架建立了德西特（de Sitter）空间中长波长场约化密度矩阵的演化方程，将轻标量场（light scalar fields）的量子动力学描述为量子随机行走（quantum walk），弥补了经典随机行走（classical random walks）与量子描述间的鸿沟，并揭示了高阶修正项的起源及德西特空间中发散与长期增长问题的动态重正化（dynamical renormalization）吸收机制。


<details>
  <summary>Details</summary>

Background: 研究主题为德西特（de Sitter）空间中轻标量场（light scalar fields）的量子动力学及其与经典随机行走（classical random walks）描述的关联。

Data: 使用精确重整化群（exact RG）框架计算长波长场（long wavelength fields）约化密度矩阵的演化方程。

Method: 采用精确重正化群(exact RG)框架计算长波长场约化密度矩阵的演化方程，通过主方程(master equation)建立轻场与量子行走(quantum walk)模型的直接映射。

Result: 利用精确重正化群（exact RG）框架计算长波长场约化密度矩阵的演化方程，将德西特空间（de Sitter）中的轻场映射到量子行走模型，并揭示了高阶修正项的起源。
...
### [77] [The Role of Microstate Degeneracy in Phase Transitions: Gravitational Waves from Bubble Entanglement](https://arxiv.org/abs/2512.13947)
*Gia Dvali,Lucy Komisel*

**TL;DR**: 研究发现，在自发对称性破缺的一阶相变中，具有高微观态简并度（high microstate degeneracy）的真空泡（vacuum bubbles）会因内部量子纠缠态显著改变其合并的经典动力学行为，从而在产生的引力波中留下宏观特征，成为引力波的定性新来源。


<details>
  <summary>Details</summary>

Background: 该研究探讨了一阶相变中形成的真空泡（vacuum bubbles）及其高微观态简并度（high microstate degeneracy）对宇宙学中引力波产生的宏观量子动力学影响。

Data: 数据来源于对真空泡（vacuum bubbles）在量子态下的演化及其合并过程的数值模拟（simulation）分析。

Method: 采用量子动力学模型分析高微观态简并度（high microstate degeneracy）真空泡（vacuum bubbles）的合并过程，重点研究内部量子态纠缠对经典动力学的影响。

Result: 高微观态简并度（high microstate degeneracy）的真空泡（vacuum bubbles）在合并过程中展现出与经典动力学显著不同的量子动力学行为，其内部量子态纠缠（entanglement）导致引力波产生机制存在本质差异。
...
### [78] [NASA/NOAA MOU Annex Final Report: Evaluating Model Advancements for Predicting CME Arrival Time](https://arxiv.org/abs/2512.14462)
...
**TL;DR**: 研究发现采用GONG台站时序磁图驱动ADAPT-WSA-ENLIL模型可将日冕物质抛射(CME)到达时间误差降低0.2-0.9小时，且零位校正磁图数据使2017-2019年事件的中位数误差显著减小。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用空军数据同化光球通量传输模型（ADAPT）结合全球振荡网络组（GONG）地面观测数据，评估日冕物质抛射（CME）到达地球时间预报精度的改进。

Data: 数据来源为全球振荡网络组（Global Oscillation Network Group, GONG）地面观测站提供的磁图（magnetogram），包括单张磁图和时间依赖的磁图序列，并分为零点校正（zero-point corrected）和未校正（uncorrected）两类。

Method: 采用空军数据同化光球通量传输模型（ADAPT）结合全球振荡网络组（GONG）地面观测数据驱动耦合Wang-Sheeley-Arge（WSA）-ENLIL模型，对比无ADAPT的WSA-ENLIL操作版本，通过1292次模拟验证研究评估不同磁图输入对CME到达时间预测的影响。

Result: 使用时间依赖的零点校正磁图（zero-point corrected magnetograms）序列相比单张磁图输入，可将所有38个事件的CME到达时间误差减少0.2至0.9小时。
...
### [79] [Neutrino Constraints on Scalar-Tensor Gravity](https://arxiv.org/abs/2512.13798)
*Arturo de Giorgi,Ivan Martinez Soler,Sergio Sevillano Muñoz*

**TL;DR**: 该论文通过中微子（neutrino）物理对一类标量-张量（scalar-tensor）理论提出了新约束，发现背景标量场（background scalar field）的空间变化会改变中微子振荡和超新星延迟，并将约束映射至对称子（Symmetron）和变色龙（Chameleon）等屏蔽机制模型。


<details>
  <summary>Details</summary>

Background: 研究探讨了标量-张量理论(scalar-tensor theories)与中微子物理学的交叉领域，重点关注背景标量场(background scalar field)对中微子传播特性的影响及其作为探测标量场的新型工具潜力。

Data: 数据来源于标准模型（Standard Model）中微子物理观测和标量张量理论（scalar-tensor theories）的模拟计算。

Method: 采用标量-张量理论（scalar-tensor theory）分析中微子（neutrino）振荡和超新星时间延迟效应，推导观测约束表达式并映射到对称子（Symmetron）和变色龙（Chameleon）等屏蔽机制模型。

Result: 中微子物理为标量-张量理论(scalar-tensor theories)提供了新约束，通过标量场背景的空间变化影响中微子振荡和超新星时延。
...
### [80] [A Cosmic Higgs Collider](https://arxiv.org/abs/2512.13815)
*Bibhushan Shakya*

**TL;DR**: 研究了早期宇宙中超相对论性希格斯真空泡碰撞（ultrarelativistic Higgs vacuum bubble collisions）作为宇宙尺度希格斯对撞机的现象学框架，发现其能探测普朗克能标的新物理并实现非热产生超重希格斯门户暗物质（up to $10^{16}$ GeV）及大统一理论尺度右手中微子的轻子生成。


<details>
  <summary>Details</summary>

Background: 研究聚焦于早期宇宙中与标准模型希格斯场（Standard Model Higgs field）相关的一级相变过程中超相对论性希格斯真空泡碰撞的框架和现象学，为探索远超宇宙历史温度（可能接近普朗克尺度）的新物理提供了独特窗口。

Data: 数据来源于对早期宇宙中超相对论性希格斯真空泡碰撞（ultrarelativistic Higgs vacuum bubble collisions）的理论模拟（simulation）。

Method: 采用相对论性希格斯真空泡碰撞（ultrarelativistic Higgs vacuum bubble collisions）框架和现象学分析，结合标准模型希格斯场（Standard Model Higgs field）的一阶相变（first-order phase transition）理论模型。

Result: 超相对论性希格斯真空泡碰撞可作为宇宙尺度希格斯对撞机，探测远超宇宙历史温度（直至普朗克能标）的新物理现象。
...
### [81] [Cosmological Bounds on Scotogenic Model with Asymmetric Mediator](https://arxiv.org/abs/2512.14271)
*Kento Asai,Seishi Enomoto,Takuya Hirose,Masato Yamanaka*

**TL;DR**: 该论文通过求解包含η粒子衰变和散射过程的玻尔兹曼方程，结合暗物质（DM）残留丰度和大爆炸核合成（BBN）限制，确定了不对称媒介体（asymmetric mediator）模型中η和σ粒子的优选参数空间（如η质量上限约10 TeV），同时解释了中微子质量、暗物质和重子不对称性的起源。


<details>
  <summary>Details</summary>

Background: 该研究探讨了不对称媒介体(scotogenic模型变体)如何同时解释中微子质量、暗物质(DM)和重子不对称性的宇宙学起源问题。

Data: 数据来源于玻尔兹曼方程数值模拟（Boltzmann equations simulation）和宇宙大爆炸核合成（Big Bang nucleosynthesis）观测约束。

Method: 采用玻尔兹曼方程求解包含η粒子衰变和散射过程的暗物质（DM）残留丰度，并结合大爆炸核合成（BBN）对η粒子晚期衰变的限制来分析模型参数空间。

Result: 该模型（asymmetric mediator scenario）通过结合暗物质（DM）丰度和大爆炸核合成（BBN）限制，确定了有利的参数空间，例如媒介粒子（mediator）质量mη≲O(10) TeV。
...
### [82] [Gravitational Waves from Confinement in $SU(N)$ Yang-Mills Theory](https://arxiv.org/abs/2512.14530)
*Stephan Huber,Rory Phipps,Manuel Reichert*

**TL;DR**: 这篇论文通过有效Polyakov圈模型结合最新格点数据，分析了$SU(N)$纯Yang-Mills理论中伴随引力波谱的禁闭相变，发现薄壁近似在小$N$时适用而在大$N$时失效，并预测$N=20$时信号最强但整体较弱。


<details>
  <summary>Details</summary>

Background: 研究聚焦于SU(N)纯杨-米尔斯理论(Yang-Mills theory)的引力波谱及其在禁闭相变(confinement phase transition)中的动力学行为。

Data: 使用最新的格点数据(lattice data)作为输入，通过有效Polyakov圈模型描述禁闭相变。

Method: 采用有效Polyakov圈模型（effective Polyakov loop model）结合最新格点数据，通过修正动力学项引入大N极限下的表面张力标度关系，并基于高焓变框架估算气泡壁速度。

Result: 在SU(N)纯杨-米尔斯理论中，最强的引力波信号出现在N=20时，但总体预测信号较弱。
