...
**TL;DR**: 在红移z~10.6的星系GN-z11附近3 pkpc处检测到无金属污染的电离氢(Hγ)和氦(HeII)发射线，为宇宙大爆炸后440 Myr存在第三星族(PopIII)恒星提供了直接证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过探测高红移(z~10.6)星系GN-z11附近区域的电离氢(Hγ)和双电离氦(HeII)发射线，验证宇宙早期第一代恒星(第三族恒星PopIII)存在的理论预测。

Data: 研究使用了JWST（詹姆斯·韦伯空间望远镜）的NIRSpec-IFS G395H数据，并参考了NIRCam成像数据。

Method: 采用JWST/NIRSpec-IFS G395H光谱仪进行近红外积分场光谱观测，结合Hγ和HeII谱线分析及气体相金属丰度上限测定。

Result: 在GN-z11附近检测到与第三族恒星(PopIII)相符的电离氢(H$γ_{4342}$)和双电离氦(HeII$_{1640}$)发射线，支持大爆炸后440 Myr存在第三族恒星活动。
...
**TL;DR**: 在红移z=10.6的GN-z11星系附近3 pkpc处确认了一个具有极高等效宽度（>20 Å）且光谱分裂为120 km/s双成分的HeII$λ$1640发射体，最可能由第三族恒星（Population III stars）产生。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用JWST（James Webb Space Telescope）NIRSpec-IFU高分辨率光谱在红移z=10.6处探测并确认GN-z11星系附近3 pkpc处的HeII$λ$1640发射体及其起源机制。

Data: 数据来源为JWST（詹姆斯·韦伯太空望远镜）NIRSpec-IFU（近红外光谱仪积分场单元）高分辨率光谱观测数据以及Übler等人（2026年）在同一位置独立探测到的H$γ$数据。

Method: 基于JWST NIRSpec-IFU高分辨率光谱分析确认了HeII$λ$1640发射线并解析出其光谱双峰结构。

Result: 在z=10.6的GN-z11星系附近3 pkpc处确认存在一个HeII$λ$1640发射体，其等效宽度极高（$>$20 A），且未检测到金属线，最可能由第三族恒星（Population III stars）产生。
...
**TL;DR**: 通过观测GN-z11附近HeII发射体Hebe的C1和C2组分并分析其HeII/Hγ比，首次直接证实了二者为无金属(原初恒星PopIII)系统，排除了陡峭初始质量函数(IMF)的可能性，支持顶部较重(top-heavy)分布，并约束其总恒星质量范围为2×10⁴至6×10⁵太阳质量，为原初恒星形成和性质研究提供了独立观测依据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过观测高红移(z)第三星族恒星(PopIII)系统直接约束第一代无金属恒星的初始质量函数(IMF)及其形成特性。

Data: 数据来源于对GN-z11星系附近HeII发射体Hebe的C1和C2组分的新观测（near-field cosmology），并结合局部校准模型进行分析。

Method: 采用局部校准模型（locally calibrated model）和简单参数化方法（parametric approach）分析HeII发射体Hebe的C1和C2组分，通过测量HeII/Hγ比值和HeII光度约束第一代恒星（PopIII）的初始质量函数（IMF）及恒星质量。

Result: 观测到的$\rm HeII/H_γ$比值排除了陡峭的初始质量函数（IMF），支持顶部较重（top-heavy）的分布，尤其是对于年轻恒星年龄（$\leq 1$ Myr）。
...
**TL;DR**: 首次揭示了耗散暗物质（ADM）对恒星流（stellar streams）的影响，发现ADM通过增强卫星星系中心密度使其更抗潮汐瓦解，导致恒星质量≥10^5.5 M⊙的恒星流形成更晚且持续恒星形成，并改变了恒星化学丰度轨迹（[Fe/H]和[Mg/Fe]），同时在低近心距处识别出高ADM质量分数的小质量卫星群。


<details>
  <summary>Details</summary>

Background: 研究探讨了耗散暗物质（ADM）对恒星流（stellar streams）动力学和化学演化的影响，重点比较了冷暗物质（CDM）与含ADM子成分的暗物质模型下恒星流形成和演化的差异。

Data: 采用宇宙学流体动力学放大模拟（cosmological hydrodynamic zoom-in simulation）数据，假设暗物质由冷暗物质（CDM）和原子暗物质（ADM，占比约6%）组成，其中ADM表现为碰撞性高效耗散气体。

Method: 采用宇宙学流体动力学放大模拟（cosmological hydrodynamic zoom-in simulation），假设暗物质由冷暗物质（CDM）和原子暗物质（ADM）子组分构成，研究ADM作为碰撞性耗散气体对恒星流的影响。

Result: 在ADM（Atomic Dark Matter）模型中，恒星质量$M_{\rm{tot}, \star} \gtrsim 10^{5.5} \ \text{M}_\odot$的恒星流形成较晚且演化过程中持续经历更长时间的恒星形成活动，其最年轻成员表现出[Fe/H]和[Mg/Fe]的化学丰度增强。
...
**TL;DR**: 通过半解析群体合成代码B-POP模拟孤立双星演化和动力学相互作用形成的双黑洞（BBH）合并事件，发现孤立双星主导主质量小于20 M$_\odot$的合并，而动力学过程主导更大质量合并，且主质量大于45 M$_\odot$的子群体呈现平坦的质量比分布和独特自旋特性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过半解析群体合成方法（semi-analytic population synthesis）模拟双黑洞（BBH）合并群体的形成机制与宇宙学演化，揭示孤立双星演化与星团动力学相互作用对双黑洞可观测特性的影响。

Data: 数据来源于半解析群体合成代码B-POP（B-POP）模拟的孤立双星演化和年轻、球状及核星团中动力学相互作用形成的双黑洞（BBH）群体。

Method: 采用半解析群体合成代码B-POP（B-POP）模拟孤立双星演化和动力学相互作用（dynamical interactions）在年轻星团、球状星团及核星团中形成的双黑洞（BBH）群体，并结合恒星形成历史、金属丰度演化及单星与双星演化过程量化其对双黑洞群体可观测特性及不同形成通道相对贡献的影响。

Result: 通过半解析群体合成代码B-POP模拟发现，孤立双星演化主导了主质量小于20 M$_\odot$的双黑洞并合事件，而动力学组装的第一代及更高代并合在更大质量区间占据主导地位。
...
**TL;DR**: 研究发现超致密大质量星系(UCMGs)的恒星与动力学质量比主要受速度弥散($σ_*$)表征的引力势深度调控，而非单纯由致密性(C)决定，揭示了非均匀性源于动力学与演化过程的耦合机制。


<details>
  <summary>Details</summary>

Background: 研究探讨了超致密大质量星系(UCMGs)中恒星与动力学质量比与致密性(C)、速度弥散($σ_*$)、恒星族性质及形成历史的关联机制。

Data: 数据来源于INSPIRE和E-INSPIRE巡天项目中482个超致密大质量星系(UCMGs)的同质样本。

Method: 采用经验性处方（empirical prescription）修正恒星初始质量函数（IMF）变化并重新计算恒星与动力学质量比，同时基于半径和恒星质量变化的位力系数（virial coefficient）进行动力学分析。

Result: 超致密大质量星系（UCMGs）的恒星与动力学质量比主要由引力势深度（由速度弥散$σ_*$表征）主导，而非单纯由致密性（C）决定。
...
**TL;DR**: 通过观测猎户座A和B分子云中北侧$\int$形纤维结构和20个无星核，发现DNC和DCO$^+$分布差异源于氮碳分子特性差异，且DNC/HN$^{13}$C柱密度比在OMC-2和OMC-3较高表明这些区域仍处于恒星形成起始阶段。


<details>
  <summary>Details</summary>

Background: 研究主题为利用氘代分子（DNC和DCO$^+$）的分布和丰度比（DNC/HN$^{13}$C和DCO$^+$/H$^{13}$CO$^+$）探索猎户座A和B分子云中单个无星核（starless cores）和$\int$形丝状结构的化学演化及恒星形成初始条件。

Data: 数据来源于使用野边山（Nobeyama）45米射电望远镜上的接收器7BEE对猎户座A和B云中的北部$\int$形细丝和20个独立无星核进行的DNC和DCO$^+$分子观测。

Method: 采用Nobeyama 45米射电望远镜的7BEE接收器对DNC和DCO$^+$分子进行成图观测。

Result: 在猎户座A云（Orion A）和B云（Orion B）中，DNC（氘代氰化氢）和DCO$^+$（氘代甲酰离子）的分布差异揭示了氮基（N-bearing）和碳基（C-bearing）分子的化学演化路径不同。
...
**TL;DR**: 基于JWST NIRCam多波段观测构建了M16创生之柱(Pillars of Creation)高分辨率(2角秒)消光图，发现柱状结构内气体柱密度概率分布(N-PDF)从中等消光(A_V≈10-30星等)的对数正态分布过渡到高消光(A_V≳30星等)的幂律分布，且幂律斜率α从面向NGC 6611星团的柱尖(α≈2.0)向远离星团区域(α≈4.0)逐渐变陡，证实恒星反馈通过辐射压缩局部增强致密自引力结构的形成。


<details>
  <summary>Details</summary>

Background: 研究聚焦于恒星反馈（stellar feedback）对M16鹰状星云中创生之柱（Pillars of Creation）分子云演化的影响机制。

Data: 数据来源于JWST近红外相机（NIRCam）的多波段测光观测，包括F090W、F200W、F335M和F444W滤光片，构建了分辨率为2角秒、深度达$A_V\sim 100$星等的消光图。

Method: 采用多波段测光观测（JWST NIRCam）构建高分辨率（2角秒）消光图，并通过柱密度概率分布函数（N-PDF）分析揭示分子云密度结构的对数正态分布与幂律尾特征。

Result: 高分辨率消光图揭示了M16创生之柱区域分子云内致密气体的精细结构，柱密度概率分布函数在中等消光（$A_V\approx10-30$\,mag）呈对数正态分布，在高消光（$A_V\gtrsim$ 30\,mag）转为幂律尾。
...
**TL;DR**: 提出了一种结合CO谱线观测与三维消光(3D extinction)地图的新方法，精确测定了四个超新星遗迹(G93.7$-$0.2、G109.1$-$1.0、G156.2+5.7和G166.0+4.3)的距离，结果分别为1.82$\pm$0.13、3.05$\pm$0.15、0.60$\pm$0.15和3.44$\pm$0.23千秒差距(kpc)，比先前估计更准确可靠。


<details>
  <summary>Details</summary>

Background: 超新星遗迹（SNR）的精确距离测量对于理解其物理特性和演化至关重要。

Data: 数据来源包括CO谱线观测和三维(3D)消光(extinction)地图，用于通过超新星遗迹(SNRs)相关的分子云确定距离。

Method: 结合CO谱线观测与三维(3D)消光(extinction)地图，通过分析超新星遗迹(SNR)与分子云的相互作用确定距离。

Result: 通过结合CO谱线观测与三维消光(3D extinction)地图的新方法，测定了四个超新星遗迹(SNR)的距离：G93.7$-$0.2为1.82$\pm$0.13 kpc，G109.1$-$1.0为3.05$\pm$0.15 kpc，G156.2+5.7为0.60$\pm$0.15 kpc，G166.0+4.3为3.44$\pm$0.23 kpc。
...
**TL;DR**: 提出时域射电光度（TDRL）框架，通过分解射电核（compact-core）和延展瓣（extended-lobe）成分并引入喷流占空比（jet duty cycle）的贝塔分布模型，证明经典GHz波段射电宁静/噪分界低谷可由单一活动星系核（AGN）种群的短时标射电记忆效应产生，无需假设双引擎机制，同时提出核-瓣失配指数和双势垒相图用于区分喷流触发、持续和残余阶段。


<details>
  <summary>Details</summary>

Background: 研究聚焦于改进经典射电噪度参数（radio-loudness parameter）的时间尺度不匹配问题，提出时域射电噪度（TDRL）框架以区分致密核（compact-core）和延展瓣（extended-lobe）射电贡献，并建立与喷流活动周期（jet duty cycle）相关的解析参考模型。

Data: 数据来源包括射电波段5 GHz的致密核区（compact-core）辐射、延展瓣（extended-lobe）辐射以及宿主星系同步辐射的观测数据，通过不同角分辨率和表面亮度灵敏度进行分解加权。

Method: method：采用时间域射电响度（TDRL）框架分解射电成分，建立包含紧凑核（compact-core）和延展瓣（extended-lobe）权重的双态喷流（two-state jet）周期模型，并推导出归一化延展射电响应的精确稳态贝塔（Beta）分布。

Result: 提出了时域射电响度（TDRL）框架，通过分解射电成分（紧凑核与延展瓣）并引入恢复分数，建立了考虑观测参数影响的射电活动性量化模型。
...
### [11] [No evidence of polarization in the $11.3\,μ$m PAH emission line by independent analyses](https://arxiv.org/abs/2603.21205)
*Enrique Lopez-Rodriguez*

**TL;DR**: 通过重新分析赫比格Be星MWC 1080西北星云中11.3微米多环芳烃(PAH)发射线的偏振数据，发现其偏振度仅为0.5±0.6%，与未偏振源一致，否定了此前1.9±0.2%的偏振测量结果，表明该数据不支持PAH发射线偏振的存在。


<details>
  <summary>Details</summary>

Background: 多环芳烃（PAHs）作为恒星形成、分子气体含量及星际介质（ISM）其他性质的示踪剂被广泛应用于银河系及其他星系的研究。

Data: 数据来源于使用加那利大型望远镜(Gran Telescopio CANARIAS, GTC)上的CanariCam仪器获取的8-13μm光谱偏振观测数据。

Method: 采用CanariCam/GTC获取的8-13μm光谱偏振观测数据，重新分析了11.3±0.2μm波段内偏振度。

Result: 重新分析显示11.3微米多环芳烃(PAH)发射线的偏振度为0.5±0.6%，与未偏振源一致，不支持此前报告的1.9±0.2%偏振结果。
...
**TL;DR**: 首次在猎户座分子云23个无星核（starless cores）中通过Q/W波段宽频扫描检测约40种分子并测定其柱密度，发现复杂有机分子（COMs）CH3OH和CH3CHO普遍存在且其柱密度比在无星核与热核（hot corinos）阶段存在约25倍差异。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用分子清单探测恒星形成最早阶段（starless cores）的物理与化学结构。

Data: 数据来源于使用Yebes 40米望远镜对猎户座云中23个无星核（starless cores）在Q波段（31.0--50.5 GHz）和W波段（71.1--91.4 GHz）进行的总带宽40 GHz的宽频带光谱扫描观测。

Method: 采用Yebes 40米望远镜在Q波段(31.0--50.5 GHz)和W波段(71.1--91.4 GHz)对猎户座23个无星核进行宽频带(总带宽40 GHz)光谱扫描观测，检测约40种分子并计算其柱密度。

Result: 在猎户座云团23个无星核（starless cores）中检测到约40种分子并测得其柱密度，各物种柱密度呈现约一个数量级的特征性分布。
...
### [13] [Revisiting the Excess of Bar-like Structures in TNG50 Early-type Galaxies: Consistency and Tension with Observations](https://arxiv.org/abs/2603.21279)
*Hangci Du,Yougang Wang,Junqiang Ge*

**TL;DR**: TNG50模拟发现早期型星系(ETGs)中普遍存在(占比75-80%)物理长度较大(≥3千秒差距)、旋转速度较慢(Ω_p≲20 km/s/kpc)的棒状结构，这些结构起源于高红移(z≳0.2)气体丰富盘中的快速旋转棒，经历星系淬熄后通过动力学摩擦减速并伸长，最终成为红序星系中缓慢旋转的"化石"棒。


<details>
  <summary>Details</summary>

Background: 研究聚焦于早型星系（ETGs）中普遍存在的长条形棒状结构（bar-like structures）的动力学特性及其演化起源，探讨其与星系形态转变（quenching）和动力学摩擦（dynamical friction）过程的关联。

Data: 研究中使用的数据来源于IllustrisTNG模拟套件（特别是TNG50-1）生成的星系演化数据。

Method: 采用形态不可知（morphology-agnostic）的普查方法，结合演化历史追踪技术，重点分析了TNG50-1模拟中弥散主导星系（dispersion-dominated galaxies）的非轴对称不稳定性特征及其模式速度（pattern speed）。

Result: 在TNG50-1模拟中，75-80%的弥散主导星系（D/T < 0.2）普遍存在物理长度更长（≳3千秒差距）、旋转速度显著更慢（Ω_p ≲20 km s⁻¹ kpc⁻¹）的非轴对称不稳定棒状结构，这些结构起源于高红移（z≳0.2）气体更丰富盘中的典型快速棒，经历星系淬熄相后通过动力学摩擦发生长期减速和伸长，最终成为红序中缓慢的化石旋转体。
...
### [14] [ASKAP EMU detection of an Odd Radio Circle (ORC) candidate: J094412-751016 (Anglerfish)](https://arxiv.org/abs/2603.21467)
...
**TL;DR**: 发现与红外源WISEA J094409.17-751012.8空间重合的弥散射电连续辐射（EMU J094412-751016，昵称"Anglerfish"），其观测特征（如尺寸、亮度、频谱指数）最可能属于新发现的奇特射电圈（ORCs）类别，而该红外源可能是其宿主椭圆星系。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索与红外源WISEA J094409.17-751012.8和半变星V687 Carinae空间重合的弥散射电连续谱辐射（EMU J094412-751016，昵称"Anglerfish"）的物理本质，重点讨论其是否属于新发现的奇特射电圈（ORCs）类别或银河系高纬度超新星遗迹（SNR）。

Data: 研究使用了来自大尺度宇宙演化图（Evolutionary Map of the Universe, EMU）巡天的944 MHz射电数据来分析被昵称为“Anglerfish”的弥散射电连续谱辐射（EMU J094412-751016）。

Method: 采用944 MHz射电数据（来自EMU巡天）分析弥散射电连续谱辐射（EMU J094412-751016），结合红外颜色判定宿主星系类型，并通过非热辐射特性、恒星风强度与壳层理论膨胀速度对比排除恒星关联性，最终基于尺寸、亮度、频谱指数等观测特征提出其可能属于新发现的奇特射电圈（ORCs）候选体。

Result: 在射电波段发现的弥散扩展辐射源“Anglerfish”最可能属于新发现的奇特射电圈（ORCs）类别，其宿主星系可能是红外源WISEA J094409.17-751012.8。
...
### [15] [The SPHEREx Ices Investigation: An Overview](https://arxiv.org/abs/2603.22135)
...
**TL;DR**: SPHEREx任务通过全天空光谱巡天测量银河系及麦哲伦云中约1000万个源的冰吸收特征，重点研究星际尘埃表面分子冰（molecular ices）丰度与环境参数的统计关联，为行星系统和生命前分子演化提供新认知。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用全天空光谱巡天（SPHEREx）探索星际尘埃颗粒表面分子冰（molecular ices）的丰度及其与行星形成区域预生命化学（prebiotic chemistry）的关联。

Data: 数据来源于SPHEREx任务对银河系和麦哲伦云中近千万个预选源进行的光谱巡天观测，结合辅助数据集用于连续谱估计。

Method: 采用基于红外颜色、空间隔离度和亮度的目标选择方法，结合机器学习进行连续谱估计的数据处理流程，优先测量关键分子冰吸收特征的积分光学深度（integrated optical depths）。

Result: SPHEREx任务通过全天空光谱巡天测量星际尘埃颗粒表面分子冰（molecular ices）的吸收特征光学深度（optical depths），建立冰丰度与环境参数的统计关联，揭示银河系冰的形成、演化及其向行星系统和生命前物质的传递机制。
...
### [16] [Tracing Star Formation in Quasar Hosts via [O II] $λ$3727: A Kinematically Consistent Approach](https://arxiv.org/abs/2603.21509)
*Liang Wu,Jun-Xian Wang,Luis C. Ho,Junfeng Wang,Zhicheng He*

**TL;DR**: 提出了一种基于[O II]和[Ne V]谱线运动学分解的新方法，发现类星体中[O II]发射主要来自恒星形成而非活动星系核(AGN)，并揭示恒星形成率(SFR)与光度存在紧密相关性，支持恒星形成与黑洞吸积共同依赖于冷气体供应的理论框架。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超大质量黑洞（SMBHs）与星系共演化过程中类星体宿主星系恒星形成的观测挑战及解析方法。

Data: 数据来源于斯隆数字巡天（Sloan Digital Sky Survey）的大样本类星体观测数据。

Method: 基于运动学一致分解方法（kinematically consistent decomposition）分离[O II]λ3727和[Ne V]λ3426发射线的AGN（活动星系核）贡献。

Result: 类星体(quasar)宿主星系中[O II]发射线主要来源于恒星形成活动，仅存在微弱的AGN贡献。
...
### [17] [Chemical Abundance Ratios of Nitrogen Rich Galaxies Identified at $z\sim 6-12$: Observational Demographics and Models](https://arxiv.org/abs/2603.21570)
...
**TL;DR**: 这篇论文通过分析JWST观测到的8个高氮氧比([N/O]$>0.3$)的$z\sim 6-12$星系化学丰度，发现Wolf-Rayet星模型比超巨星和潮汐瓦解事件(TDE)模型更能解释观测到的[Ne/C]与[N/O]关系，但所有模型的高[N/O]持续时间都过短，暗示频繁的失败超新星(failed supernovae)抑制了氧相对于氮的增长。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST观测数据探究高红移（$z\sim 6-12$）富氮星系（[N/O]$>0.3$）的化学丰度比及其演化机制。

Data: 数据来源于JWST（James Webb Space Telescope）前4年观测的NIRSpec（Near-Infrared Spectrograph）数据，重新分析了8个富氮星系（[N/O]$>0.3$）在$z\sim 6-12$的红移范围内的线流量和上限值。

Method: 采用自洽方法重新分析JWST/NIRSpec数据，通过测量[OIII]λ4363和[OII]λλ3727,3729线确定电子温度和密度，推导[N/O]、[C/O]、[Ne/O]、[Ne/C]、[Ar/O]、[S/O]和[Fe/O]等丰度比，并整合沃尔夫-拉叶星(WR stars)、超巨星(supermassive stars)和潮汐瓦解事件(TDEs)三种主要情景的化学演化模型，结合渐近巨星分支(AGB stars)贡献及数值模拟计算的核坍缩超新星(CCSNe)抛射物。

Result: 在[N/O] vs. [Ne/C]平面上，观测到的丰度比值与沃尔夫-拉叶星（Wolf-Rayet）模型吻合度高于超巨星（supermassive-star）和潮汐瓦解事件（TDE）模型。
...
### [18] [Research on the central region of quasars based on variability and structure function](https://arxiv.org/abs/2603.21575)
*Xuan Wei,J. Tang,Yu Tao,XiaoHan Zhang*

**TL;DR**: 论文通过分析类星体(quasars)光变结构函数特征参数，发现其与光度、黑洞质量及爱丁顿比(Eddington ratio)显著相关，而与红移或静止波长无关，表明类星体光变特性主要由其自身物理属性决定。


<details>
  <summary>Details</summary>

Background: 研究聚焦于类星体(quasars)中心区域物理本质的间接揭示，通过其光变特性探索超大质量黑洞(supermassive black holes)与吸积盘(accretion disks)构成的复杂系统。

Data: 数据来源于斯隆数字巡天（Sloan Digital Sky Survey, SDSS）的大样本观测数据。

Method: 采用时间域观测策略并结合光变曲线分析和结构函数（structure function）构建，间接揭示类星体中心区域的物理本质。

Result: 类星体(quasar)的结构函数特征参数与光度、黑洞质量及爱丁顿比(Eddington ratio)显著相关，高光度、大黑洞质量和大爱丁顿比的类星体具有更大的结构函数值。
...
### [19] [The ALMA-QUARKS survey: Investigating Thermal Feedback of Massive Protostars in Hot Molecular Cores](https://arxiv.org/abs/2603.21670)
...
**TL;DR**: 通过分析83个热分子核（HMCs）的温度、密度和CH3CN丰度分布，发现大质量原恒星光度与核质量、内半径及中心密度存在显著相关性，并证实其热反馈通过提高局部金斯质量（Jeans mass）抑制HMCs进一步碎裂，从而促进大质量恒星形成。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量恒星形成过程中热反馈（thermal feedback）对热分子核（HMCs）的物理影响及其与恒星形成效率的关系。

Data: 数据来源于QUARKS巡天观测的83个空间分辨热分子核（HMCs）的CH3CN(12-11)谱线发射和1.3毫米连续谱数据。

Method: 采用CH$_3$CN(12--11)谱线发射和1.3毫米连续谱数据推导83个热分子核(HMCs)的径向温度、体积密度和CH$_3$CN丰度分布，并基于包层温度与密度分布利用RADMC辐射传输模型计算大质量原恒星的辐射光度。

Result: 基于83个热分子核（HMCs）的观测数据，发现大质量原恒星光度与局部热金斯质量（Jeans mass）呈强正相关，且平均金斯质量是HMC质量的两倍，表明大质量原恒星的辐射反馈能有效抑制HMCs的进一步碎裂，从而促进大质量恒星形成。
...
### [20] [Are "Changing-Look" Active Galactic Nuclei Special in the Coevolution of Supermassive Black Holes and their Hosts? II. The Case of Changing-Look Narrow-Line Seyfert 1 Galaxies](https://arxiv.org/abs/2603.21767)
*J. Wang,S. Jin,D. W. Xu,WeiKang Zheng,Thomas G. Brink,S. Komossa,1 Alexei V. Filippenko,J. Y. Wei*

**TL;DR**: 研究发现"变脸"(CL)活动星系核(AGN)的演化特征在窄线赛弗特1型星系(NLS1)中同样适用，尽管这类天体倾向于处于中等-年老恒星群体的年轻端。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探究"变脸"(changing-look)活动星系核(AGN)的演化角色及其与不同年龄恒星种群的关联。

Data: 数据来源包括Paliya等人（2024）提供的大规模窄线赛弗特1星系（NLS1）目录和斯隆数字巡天第五阶段（SDSS-V）数据发布19（DR19）的光谱巡天数据。

Method: 采用光谱分析（spectral analysis）方法验证CL-AGNs（changing-look active galactic nucleus）的演化角色。

Result: CL窄线塞弗特1星系(NLS1)的观测分析支持其处于中等-年老恒星群体的年轻端演化阶段。
...
### [21] [APOSTLE vs. AURIGA Simulations: How Subgrid Models Shape Milky Way Analogs](https://arxiv.org/abs/2603.21837)
*Jianhong Hu,Hang Yang,Liang Gao*

**TL;DR**: 通过对比AURIGA和APOSTLE两组初始条件相同但重子物理(baryonic physics)实现方式不同的星系形成模拟，发现AURIGA模拟产生的星系具有更高的恒星质量、表面密度、盘质量占比以及更早的盘形成时间，这些差异主要源于星系周介质(CGM)气体冷却效率的差异。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系形成模拟中亚网格物理过程(subgrid physics)对星系详细特性的影响，旨在通过对比不同重子物理(baryonic physics)实现方案揭示其作用机制。

Data: data部分：研究使用了AURIGA和APOSTLE两组具有相同初始条件但采用不同重子物理(baryonic physics)实现的zoom-in（放大）模拟数据。

Method: 采用对比分析两组共享初始条件但采用不同重子物理（baryonic physics）实现方案的星系形成放大模拟（zoom-in simulations），重点考察气体冷却效率差异对星系特性的影响。

Result: AURIGA模拟的星系比APOSTLE具有更高的恒星质量和表面密度，主要源于环星系介质（CGM）气体冷却效率的差异。
...
### [22] [The MUSE Ultra Deep Field (MUDF) VIII. The cool gas distribution surrounding galaxies at redshifts z ~ 0.5-2](https://arxiv.org/abs/2603.21855)
...
**TL;DR**: 基于MUSE深场数据研究发现，0.5 < z < 2星系周围冷气体（cool gas）的流出（outflow）特征与恒星质量正相关，横向吸收（MgII）随撞击参数（impact parameter）递减，且现有辐射转移模型（radiative transfer models）因忽略几何结构和星际介质（ISM）会高估等效宽度（equivalent widths）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于红移0.5至2区间星系周围冷气体（cool gas）的分布特征及其与星系质量、外流（outflows）几何形态的关联机制。

Data: 数据来源于MUDF巡天的深度MUSE（Multi Unit Spectroscopic Explorer）观测数据，用于研究红移0.5 < z < 2范围内星系周围的冷气体。

Method: 采用多单元光谱探测仪（MUSE）深度观测数据，结合沿视线方向（down-the-barrel）和横向投影星系对样本的镁离子（MgII）吸收线分析，并对比辐射传输模型（radiative transfer models）进行几何与气体保留效应约束。

Result: 在横向投影样本中，镁离子（MgII）吸收强度随碰撞参数（impact parameter）增大呈平缓递减趋势。
...
### [23] [Observations of Early Black Holes Before and After JWST](https://arxiv.org/abs/2603.21976)
*Eduardo Banados*

**TL;DR**: 该论文总结了利用JWST（詹姆斯·韦伯太空望远镜）空间红外观测对宇宙最初十亿年内超大质量黑洞（supermassive black holes）的演化认知的重大进展，包括黑洞质量测量、宿主星系表征及大尺度环境研究等新兴方向。


<details>
  <summary>Details</summary>

Background: 研究聚焦于超大质量黑洞（supermassive black holes）在宇宙早期（前10亿年）的演化认知进展，涵盖从地基观测到詹姆斯·韦伯太空望远镜（JWST）红外空间观测的突破性发现。

Data: 数据来源包括地基望远镜观测和JWST(James Webb Space Telescope)空间红外观测。

Method: method：采用地面观测与空间红外观测（JWST）相结合的方法，重点分析高红移（high-redshift）活动星系核（AGN）和类星体（quasar）的黑洞质量测量、宿主星系表征及大尺度环境研究。

Result: JWST的红外观测显著推进了对宇宙前10亿年内超大质量黑洞（supermassive black holes）演化、质量测量及宿主星系特性的理解。
...
### [24] [JOYS: Linking the molecular ice and gas-phase composition towards the high-mass hot core IRAS 18089-1732](https://arxiv.org/abs/2603.22007)
...
**TL;DR**: 论文通过JWST和ALMA观测，首次在年轻大质量热核（hot core）IRAS 18089-1732的5000 au区域内同步测定了冰相（H2O、SO2等12种）与气相（38种）分子柱密度，发现C2H5OH、CH3OH和CH3OCH3在两相中丰度相近，SO2和CH3COCH3在气相更富集，CH3CHO在冰相丰度高一个量级，且冰相丰度（相对H2O）与银河系其他低/高质量原恒星相似，OCN-、CH3OH和CH3CHO冰丰度可能随银心距递减。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星际介质中分子（包括冰相和气体相）的形成与破坏机制，特别是借助JWST和ALMA等先进设备揭示年轻大质量热核（hot core）IRAS 18089-1732的分子组成及其空间分布规律。

Data: 冰相（ice）数据来自詹姆斯·韦伯太空望远镜（JWST）在5-28微米波段的光谱观测，气相（gas-phase）数据来自阿塔卡马大型毫米波/亚毫米波阵列（ALMA）在3毫米波段的分子谱线观测。

Method: 利用詹姆斯·韦伯太空望远镜(JWST)的5-28微米光谱数据推导冰相分子柱密度，并结合阿塔卡马大型毫米波/亚毫米波阵列(ALMA)的3毫米波段分子谱线观测获取气相分子柱密度。

Result: 在IRAS 18089-1732热核区域探测到冰相（H2O、SO2、OCN-等）和气相（38种分子）柱密度，发现C2H5OH、CH3OH和CH3OCH3在两相中丰度相近，SO2和CH3COCH3在气相更丰富，CH3CHO在冰相较气相高一个量级，且冰相丰度（相对于H2O冰）与其他银河系原恒星相似，OCN-、CH3OH和CH3CHO冰丰度可能随银心距减小。
...
### [25] [J-PAS: unprecedented precision in stellar populations of diffuse tidal features](https://arxiv.org/abs/2603.22166)
...
**TL;DR**: 利用J-PAS窄带滤光片系统首次精确解析了PGC 3087775（红移z=0.046179）并合星系潮汐结构的恒星种群特征，发现其具有中等消光、较低金属丰度及快速淬灭的恒星形成历史（SFH），相较SDSS数据将恒星质量和Dn(4000)指数测量精度提升四倍，并验证了SDSS经验质量-光比方法会高估恒星质量0.4-0.5 dex。


<details>
  <summary>Details</summary>

Background: 星系频繁与邻近系统相互作用，这一过程会显著改变其形态和恒星形成活动，但对其微弱弥散遗迹的光谱研究面临长曝光时间限制和积分视场单元(IFU)视场狭窄的挑战。

Data: 研究中使用的数据来源包括J-PAS早期数据发布的光学窄带滤波观测数据、MaNGA积分视场单元(IFU)数据用于中心部分验证以及SDSS测光数据。

Method: 采用54个光学窄带滤光片和宽视场覆盖（计划8000平方度）的J-PAS系统填补了光谱分辨率与视场限制的缺口，并结合SDSS测光数据推导恒星族性质。

Result: J-PAS（Javalambre Physics of the Astronomical Structure）对PGC 3087775的分析表明其恒星种群金属丰度较低、消光适度且恒星形成历史（SFH）更快速，符合淬灭恒星种群特征。
...
### [26] [Axionlike dark-matter winds driven by galactic baryon redistribution](https://arxiv.org/abs/2603.22234)
*A. Nazarenko*

**TL;DR**: 论文研究了矮星系中玻色-爱因斯坦凝聚态(BEC)轴子型暗物质(DM)的流体动力学方程解，发现亚星系重子过程引发的微扰会导致中心DM密度增加0.01%和数米/秒的DM风速度，并揭示了32 Myr时间尺度的量子激发衰减现象。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗物质(DM)作为具有类轴子(axionlike)相互作用的玻色-爱因斯坦凝聚态(BEC)在矮星系中形成球对称晕时，其流体动力学方程的解及其受亚星系重子过程引力背景变化引发的微扰和退相干效应。

Data: 数据来源于对矮星系NGC 2366的观测数据（observational data for NGC 2366）以及基于玻色-爱因斯坦凝聚体（Bose-Einstein condensate, BEC）暗物质（dark matter, DM）模型的数值模拟（numerical simulation）。

Method: 采用玻色-爱因斯坦凝聚态(BEC)暗物质(DM)的流体动力学方程和量子激发模型(quantum excitation model)分析矮星系中球对称暗物质晕的微扰与退相干过程。

Result: 在矮星系中，玻色-爱因斯坦凝聚态(BEC)暗物质(DM)的微扰和退相干源于亚星系重子过程引起的引力背景变化，导致中心DM密度增加0.01%和仅几米每秒的DM风速度。
...
### [27] [A bending in the size-mass relation of star-forming galaxies across $0.5 < z < 6.0$ at a critical stellar mass of $10^{10}M_\odot$ revealed by JWST](https://arxiv.org/abs/2603.22239)
...
**TL;DR**: 利用JWST/NIRCam和MIRI成像数据发现，恒星形成星系(SFGs)在$0.5 < z < 6.0$红移范围内存在拐点质量($M_{\rm p}$)恒定为$\sim 10^{10} M_\odot$的断幂律尺寸-质量关系，而静止星系(QGs)的拐点质量随红移增加，表明致密恒星形成星系是$z \gtrsim 2$大质量静止星系的主要前身星体，支持致密化路径主导其形成。


<details>
  <summary>Details</summary>

Background: 研究星系大小（galaxy size）与恒星质量（stellar mass）的关系，以揭示星系形成和演化过程中的物理机制，特别是星暴星系（SFGs）和静止星系（QGs）在不同红移下的尺寸增长模式及其与暗晕（halo）增长的关联。

Data: 使用PRIMER巡天的JWST/NIRCam和MIRI深度成像数据获取了0.5 < z < 6.0红移范围内星系的静止光学波段观测数据。

Method: 采用JWST/NIRCam和MIRI成像数据，通过分析静止波段光学尺寸-恒星质量关系（size-stellar mass relation）并拟合分段幂律（broken power-law）和混合幂律模型（mixture power-law model），研究了星系尺寸演化与质量的关系。

Result: 在$0.5 < z < 6.0$红移范围内，恒星形成星系(SFGs)呈现断裂幂律尺寸-质量关系，转折质量($M_{\rm p}$)稳定在$\sim 10^{10} M_\odot$，且高于$M_{\rm p}$时斜率变平，表明存在先前研究中低估的致密大质量恒星形成星系群体。
...
### [28] [Mainly on the Plane: Observing the Extended, Ionized Disks of Milky Way Analogs in IllustrisTNG](https://arxiv.org/abs/2603.22257)
*Michael Messere,Kirill Tchernyshyov,Mary E. Putman,Greg L. Bryan,Jessica K. Werk,Yong Zheng,David Schiminovich*

**TL;DR**: 研究发现银河系类似星系（Milky Way-like galaxies）的环星系介质（CGM）中低电离态物质（如MgII）主要分布在延展共转盘状结构中，且63%的MgII质量沿主轴运动学与星系角动量轴对齐。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系类似星系周介质（circumgalactic medium, CGM）是否呈现延展电离盘状结构的科学问题。

Data: 数据来源于IllustrisTNG模拟中对银河系类似星系样本中不同离子（HI、MgII、SiIV、CIV和OVI）的空间和运动学分布分析。

Method: 采用角动量错位角（θ）和共转吸收比例（f_EWcorot）量化不同离子（HI、MgII、SiIV、CIV、OVI）在星系盘共转结构中的分布特征。

Result: 63%的MgII、45%的SiIV、38%的CIV和35%的OVI质量沿主轴的动力学与星系角动量轴对齐，表明典型银河系类似星系（MW-like）的环星系介质（CGM）尤其是低电离态物质主要分布在盘面上。
...
### [29] [Little Red and Blue Dots: simply stratified Broad Line Regions](https://arxiv.org/abs/2603.22277)
...
**TL;DR**: 研究发现Hα宽线轮廓（broad line profiles）在活动星系核（AGN）中普遍不符合单一高斯或指数模型，洛伦兹（Lorentzian）或多高斯拟合更优，且指数轮廓可能源于宽线区（BLR）分层而非电子散射主导。


<details>
  <summary>Details</summary>

Background: 研究探讨了活动星系核(AGN)中宽发射线(如Hα)的轮廓特征及其物理起源，特别是针对小红色点源(LRDs)和小蓝色点源(LBDs)的指数型轮廓是否普遍存在及其形成机制。

Data: 使用高信噪比（high SNR）和高分辨率光谱观测数据分析了32个活动星系核（AGN），包括小红点（LRDs）、小蓝点（LBDs）和X射线探测源。

Method: 采用高信噪比（high SNR）和高分辨率光谱技术分析Hα宽线轮廓，并比较单高斯（single Gaussian）、指数（exponential）、洛伦兹（Lorentzian）及多高斯（multi-Gaussian）模型拟合效果。

Result: 部分小蓝点(LBDs)更倾向于指数型轮廓，而小红点(LRDs)更多符合洛伦兹型轮廓。
...
### [30] [Improved dark matter measurements with flexible modeling of resolved strongly-lensed quasar narrow-line emission](https://arxiv.org/abs/2603.20563)
*Maria F. Perez Mendoza,Anna M. Nierenberg,Vardha N. Bennert*

**TL;DR**: 通过模拟观测和开发新数据处理流程，验证了在强引力透镜类星体窄线区(NLR)流量比(flux ratio)测量中，即使存在千秒差距尺度延展辐射，核区点源模型仍能确保暗物质子结构探测精度优于5%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用强引力透镜类星体窄线区（NLR）的流量比（flux ratios）探测低质量暗物质晕（dark matter halos）的扰动效应，以检验暗物质模型。

Data: 研究中使用的数据来源是模拟的凯克（Keck）OSIRIS自适应光学（AO）观测数据，模拟对象为七个红移小于1、光度约10^46 erg/s的类星体。

Method: 采用核心分析方法为模拟凯克(Keck) OSIRIS自适应光学(AO)观测并结合点源(核区)与Sérsic椭圆轮廓(扩展[O III]发射)同步拟合的新流程。

Result: result：新开发的管线（pipeline）和拟合程序能确保在窄线区（NLR）通量比测量中实现<5%的精度，适用于欧几里得（Euclid）、鲁宾（Rubin）和罗曼（Roman Space Telescopes）望远镜将发现的数千个透镜系统。
...
### [31] [Probing the stochastic signal from primordial gravitational waves with pulsar timing arrays](https://arxiv.org/abs/2603.20742)
*Jun Li,Guanghai Guo,Pengfei Yan*

**TL;DR**: 研究通过结合脉冲星计时阵（PTAs）、宇宙微波背景（CMB）B模偏振和重子声学振荡（BAO）数据，更新了对原初引力波功率谱张量谱指数（tensor spectral index）的约束：未探测到随机引力波背景（SGWB）时排除大部分正值区域（$n_t= -0.165^{+1.20}_{-1.56}$），而将PTA信号解释为SGWB时支持蓝倾谱（$n_t= 2.39^{+1.46}_{-1.35}$）并排除负值区域。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原初引力波（primordial gravitational waves）产生的随机信号及其对宇宙微波背景（CMB）B模极化和脉冲星计时阵（PTAs）观测的随机引力波背景（SGWB）的影响。

Data: 使用了来自BICEP/Keck (BK18)、Planck (Planck18)的宇宙微波背景(CMB)B模偏振数据、重子声学振荡(BAO)测量数据以及脉冲星计时阵(PTAs)的随机引力波背景(SGWB)限制数据。

Method: 采用宇宙微波背景（CMB）B模偏振数据（BK18、Planck18）和重子声学振荡（BAO）测量结合脉冲星计时阵（PTAs）的随机引力波背景（SGWB）限制，推导原始功率谱张量谱指数（tensor spectral index）的更新约束。

Result: result：在PTA信号未被检测为随机引力波背景（SGWB）时，张量谱指数（n_t）约束为-0.165^{+1.20}_{-1.56}（95%置信水平），排除了大部分正值区域；若PTA信号被视为SGWB，则张量谱指数倾向于正值（n_t=2.39^{+1.46}_{-1.35}），支持蓝倾原初引力波功率谱并排除负值区域。
...
### [32] [Field-Level Inference of Primordial Non-Gaussianity with the Quijote Simulation Suite](https://arxiv.org/abs/2603.20855)
...
**TL;DR**: 首次在真实暗物质晕目录中通过场级贝叶斯前向建模方法联合推断局域原初非高斯性参数（$f_{\rm NL}^{\rm local}$）、初始条件及系统参数，相比传统功率谱和双谱估计器将约束精度提升约1.3倍，并证实分辨率提高可进一步优化参数约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用下一代星系红移巡天数据，通过场级贝叶斯前向建模方法优化对局域原初非高斯性参数（local primordial non-Gaussianity, $f_{\rm NL}^{\rm local}$）的约束，以探索宇宙暴胀物理和多场驱动机制。

Data: 数据来源于暗物质专用N体模拟（dark-matter-only N-body simulations）生成的现实晕族目录（halo catalogues），并基于Quijote模拟套件（Quijote simulation suite）的子集进行推断，推断尺度达到$k_{\rm max} \approx 0.1 h \rm{Mpc}^{-1}$。

Method: 采用贝叶斯场级前向建模方法（Bayesian forward-modelling approach at the field level）联合推断局部原初非高斯性参数（$f_{\rm NL}^{\rm local}$）、初始条件及 nuisance 参数，并基于暗物质N体模拟（dark-matter-only $N$-body simulations）生成的晕族目录验证模型分辨率与结构形成假设。

Result: 首次在真实暗物质晕目录中联合推断局部原初非高斯性参数(f_NL^local)、干扰参数和初始条件，验证了场级贝叶斯前向建模方法优于传统功率谱和双谱估计器，约束精度提升约1.3倍。
...
### [33] [Redshift Dipoles from Non-Geodesic Observer Congruences in Covariant Cosmology](https://arxiv.org/abs/2603.20963)
*Erick Pastén*

**TL;DR**: 研究发现非测地线观测者运动在宇宙膨胀的红移关系中引入了与观测者四维加速度视线投影成正比的额外贡献，导致红移空间出现由观测者沿过去光锥运动学特性驱动的非平凡红移依赖性偶极调制。


<details>
  <summary>Details</summary>

Background: 研究探讨了非均匀宇宙中哈勃参考系(Hubble frame)与测地物质流(geodesic matter flow)偏离时，观测者4维加速度对红移-仿射参数关系的额外贡献及其导致的偶极调制效应。

Data: 研究使用了基于完全协变框架（fully covariant framework）的理论模拟数据，分析了非测地线观测者汇（non-geodesic observer congruence）对红移关系的影响。

Method: 采用完全协变框架分析非测地观测者汇（observer congruence）对仿射参数与观测红移关系的额外贡献，重点考察观测者4-加速度（4-acceleration）的视线投影引起的红移空间偶极调制效应。

Result: 在非均匀宇宙中，非测地线观测者共动系会引入与观测者四维加速度视线投影成正比的附加项，导致红移空间出现偶极调制效应。
...
### [34] [Model-Independent Reconstruction of Quintessence Potential and Kinetic Energy from DESI DR2 and Pantheon+ Supernovae](https://arxiv.org/abs/2603.21125)
*Shengjia Wang,Tian-Nuo Li,Tonghua Liu,Guo-Hong Du*

**TL;DR**: 基于DESI DR2重子声学振荡和Pantheon+超新星数据，采用高斯过程无模型重建了精质场(quintessence)动力学演化，发现势能随红移单调递减符合解冻型精质场特征，动能于红移$z\sim 1$附近过零点对应暗能量-物质相等时期，中间红移(0.5<z<1.0)表观负动能源于导数重建误差而非新物理。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过非参数方法重建精质场（quintessence field）动力学特性，以探索动态暗能量（dynamical dark energy）的本质及其在宇宙演化中的作用。

Data: 数据来源于DESI DR2重子声波振荡（baryon acoustic oscillation）测量和Pantheon plus Ia型超新星（Type Ia supernova）汇编。

Method: 采用高斯过程（Gaussian process）结合四种不同协方差核函数对精质场（quintessence scalar field）动力学进行无模型依赖的非参数重建。

Result: 动力学重构揭示五元标量场（quintessence scalar field）势能随红移单调递减符合解冻型五元场特征，动能则在红移z∼1处穿越零点对应暗能量-物质平等时期。
...
### [35] [Testing General Relativity on Galactic Scales via DESI-BAO and Strong Lensing: Circumventing Assumptions on the Hubble Constant, Sound Horizon, and Dark Energy](https://arxiv.org/abs/2603.21127)
*Hengyu Wu,Tonghua Liu,Chenggang Shao*

**TL;DR**: 结合重子声波振荡(BAO)角度尺度和120个星系尺度强引力透镜系统，通过人工神经网络(ANN)和三次样条重建方法，在两种透镜质量模型下约束参数化后牛顿(PPN)参数γ_PPN，发现当前观测结果与广义相对论(GR)基本一致，但约束结果显著依赖于透镜质量模型选择。


<details>
  <summary>Details</summary>

Background: 该研究旨在通过结合重子声学振荡（BAO）角度尺度测量和120个星系尺度强引力透镜系统，建立一个不依赖宇宙学模型的框架，以在星系尺度上检验广义相对论（GR）的有效性。

Data: 数据来源于SDSS、BOSS、eBOSS和DESI Data Release 2（DR2）的重子声波振荡（BAO）角尺度测量以及120个星系尺度强引力透镜系统。

Method: 采用人工神经网络（ANNs）和三次样条重构技术重构重子声波振荡（BAO）角尺度，并结合强引力透镜系统推断透镜和源的角直径距离。

Result: 在恒定密度斜率模型（P1）下，人工神经网络重建得到的参数化后牛顿参数（γPPN）为1.102+0.148−0.125，与广义相对论在1σ置信水平一致；三次样条重建结果为1.150+0.139−0.118，在2σ水平一致。在红移演化模型（P2）下，人工神经网络重建结果为1.315+0.181−0.155（2σ一致），而样条重建结果为1.485+0.193−0.168（存在约2.5σ的轻微张力）。
...
### [36] [Closed-form approximations of fundamental quantities of Lemaitre-Tolman-Bondi cosmologies from Symbolic Regression: I. Results on the Garcia-Bellido-Haugbølle parameterization](https://arxiv.org/abs/2603.21277)
*A. Carvalho,A. Krone-Martins,A. Da Silva,J. P. Mimoso,C. Bœhm*

**TL;DR**: 通过符号回归（SR）为勒梅特-托尔曼-邦迪（LTB）宇宙学中的五个关键函数建立了高精度解析近似，实现了径向/横向尺度因子、哈勃函数及角直径距离的快速计算（平均误差<0.3%）。


<details>
  <summary>Details</summary>

Background: 研究主题为利用符号回归（Symbolic Regression, SR）方法在非均匀勒梅特-托尔曼-邦迪（Lemaitre-Tolman-Bondi, LTB）宇宙模型中构建径向与横向尺度因子、哈勃函数及角直径距离的解析近似表达式。

Data: 数据来源于使用bubble LTB数值代码对四维LTB参数空间进行采样生成的数值模拟数据。

Method: 采用符号回归（Symbolic Regression, SR）方法重建了勒梅特-托尔曼-邦迪（LTB）宇宙模型中五个基本函数的解析近似表达式。

Result: 通过符号回归（SR）方法，在受限的Garcia-Bellido-Haugboelle（GBH）参数化框架下，为径向和横向尺度因子A_parallel(r,t)、A_perp(r,t)、哈勃函数H_parallel(r,t)、H_perp(r,t)及角直径距离D_A(z)构建了高精度解析近似表达式，平均相对误差普遍低于0.3%（径向哈勃函数除外，达1.4%）。
...
### [37] [Unimodular Diffusion and Interacting Vacuum Cosmology](https://arxiv.org/abs/2603.21675)
*Gopal Kashyap,Naveen K. Singh*

**TL;DR**: 研究揭示了单模扩散宇宙学(unimodular diffusion cosmology)与暗能量-暗物质相互作用模型在背景和线性扰动层面的等效性，发现扩散框架下时间依赖的宇宙常数Λ(t)在背景水平可映射到w=-1的相互作用暗能量模型，且线性扰动仅与能量转移均匀(Q∝ρ_de, δQ=0)的真空相互作用模型等效，观测约束显示ξ=-0.0147±0.0075与ΛCDM在2σ内一致，结构增长振幅S_8=0.782±0.026略高于ΛCDM的0.77±0.025。


<details>
  <summary>Details</summary>

Background: 研究探讨了单模扩散宇宙学(unimodular diffusion cosmology)与相互作用暗区模型(interacting dark sector models)在背景和线性扰动层面的对应关系。

Data: 数据来源包括超新星（supernovae）、DESI重子声波振荡（DESI BAO）、宇宙计时器（cosmic chronometers）和CMB距离先验（CMB distance priors）。

Method: 采用扩散框架（diffusion framework）将有效宇宙学常数（cosmological constant）建模为时间依赖的Λ(t)，并通过扩散电流（diffusion current）驱动其演化，同时在背景和线性扰动层面与暗能量（dark energy）和暗物质（dark matter）的相互作用模型建立映射关系。

Result: 在背景（background）层面，单模扩散宇宙学框架可映射到具有能量转移Q且状态方程参数w=-1的相互作用暗能量模型。
...
### [38] [Inflationary Phase Transitions in the Early Universe: A Bayesian Study with Space-Based Gravitational Waves Detectors](https://arxiv.org/abs/2603.21762)
*Qingyuan Liang,Chen Yang,Haipeng An,Huai-Ke Guo*

**TL;DR**: 研究了利用类似太极(Taiji)的空间引力波探测器探测宇宙暴胀时期相变产生的随机引力波背景，建立了包含仪器噪声、天体物理前景和数据通道的完整分析框架，发现中等信噪比可实现探测但强信号更利于参数重建。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用空间引力波探测器探测宇宙暴胀（inflation）期间相变（phase transitions）产生的随机引力波背景（stochastic gravitational-wave background），以探索早期宇宙物理。

Data: 数据来源于模拟的太极（Taiji）类空间引力波探测器任务，包括仪器噪声、天体物理前景和背景以及A、E、T时间延迟干涉测量通道。

Method: 采用费希尔矩阵(Fisher matrix)预测和嵌套采样贝叶斯推断(Bayesian inference with nested sampling)分析目标信号，构建了包含仪器噪声、天体物理前景和背景以及TDI（time-delay interferometry）通道的Taiji类任务数据分析框架。

Result: result：研究表明，基于太极（Taiji）类任务的引力波探测器能够探测到暴胀期间相变（phase transitions）产生的随机引力波背景（stochastic gravitational-wave background），并在中等信噪比下实现探测，但更强信号能提供更可靠的参数重建。
...
### [39] [A plug-and-play approach with fast uncertainty quantification for weak lensing mass mapping](https://arxiv.org/abs/2603.22006)
*Hubert Leterme,Andreas Tersenov,Jalal Fadili,Jean-Luc Starck*

**TL;DR**: 提出PnPMass方法，通过交替梯度下降和去噪步骤实现弱引力透镜质量分布（weak lensing mass mapping）的高效重建，兼具单次训练、快速推理和校准误差估计优势，适用于未来大规模巡天数据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用下一代宇宙巡天（stage-IV surveys）的高精度弱引力透镜（weak lensing）数据，通过暗物质分布重建提升宇宙学模型约束精度。

Data: 数据来源为模拟数据（simulated data）并添加了高斯白噪声（Gaussian white noise）进行训练。

Method: 采用交替梯度下降与深度学习去噪结合的插拔式方法（PnPMass），通过模拟数据训练单一模型实现快速推断，并结合矩网络和保形预测进行不确定性量化。

Result: PnPMass方法在保持接近最先进深度学习模型精度的同时，实现了快速推理（仅需数次迭代收敛）且仅需单次训练，不受观测噪声协方差影响。
...
### [40] [Probing the Cosmic Web with Fast Radio Bursts. I. Scattering](https://arxiv.org/abs/2603.22108)
*Sharon Lapiner,Nir Mandelker,Paz Beniamini,S. Peng Oh*

**TL;DR**: 研究发现宇宙网结构（cosmic web objects）中后激波区域的多相气体（multiphase gas）通过局部不稳定性形成小尺度致密电离团块（dense, ionised inhomogeneities），这些团块能通过快速射电暴（FRBs）散射特征探测，其中高红移（high-$z$）丝状结构（filaments）和片状结构（sheets）对散射贡献可忽略，而大质量晕（massive haloes）的环星系介质（CGM）中若存在湍流云团（turbulent cloudlets）且体积填充率（volume-filling fraction）$f_{\rm v}\gtrsim 10^{-3}$，则会导致显著的累积散射（cumulative scattering），但当前观测未发现散射时间与红移的相关性，暗示CGM冷气体（cool gas）可能存在非热压力（non-thermal pressure）或小尺度密度涨落（density fluctuations）阻尼。


<details>
  <summary>Details</summary>

Background: 研究宇宙网状结构（cosmic web objects, CWOs）中多相气体（multiphase gas）的形成机制及其对快速射电暴（FRBs）传播的影响。

Data: 数据来源于对宇宙网结构（cosmic web objects）中热介质局部不稳定性的模拟分析，重点关注了后激波区域多相气体形成过程及其对快速射电暴（FRBs）射电波传播的影响。

Method: 采用流体动力学不稳定性分析和多相介质冷却模型，结合快速射电暴（FRB）散射特征观测数据反演宇宙网状结构（CWOs）中的小尺度密度扰动。

Result: 高温丝状结构（filaments）和片状结构（sheets）对快速射电暴（FRBs）观测散射的贡献可忽略不计。
...
### [41] [Using Global Gravitational Potential Weighted Correlation Function to Constrain Modified Gravity Models](https://arxiv.org/abs/2603.22138)
*Yizhao Yang,Yu Yu,Pengjie Zhang*

**TL;DR**: 提出了一种基于全局引力势（global gravitational potential）加权的标记两点相关函数新方法，能够有效区分f(R)引力和nDGP修正引力模型与广义相对论（GR），信号在特定引力势区间显著增强。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过引入基于全局引力势（global gravitational potential）的新型标记两点相关函数来测试修正引力（modified gravity）模型，旨在突破传统基于局部属性标记的局限性。

Data: 数据来源于基于星系分布通过泊松方程重构的全局引力势（global gravitational potential）标记，并对比了两种修正引力（modified gravity）模型（f(R)和nDGP）与广义相对论（GR）下的LCDM模型。

Method: 采用基于全局引力势（global gravitational potential）重建的标记两点相关函数（marked two-point correlation function）作为核心分析方法，通过两种加权方案量化环境依赖性聚类。

Result: 提出的基于全局引力势（global gravitational potential）加权的两点相关函数能有效区分修正引力（MG）模型与广义相对论（GR），信号在特定引力势范围内增强。
...
### [42] [Advanced Virgo Plus for O5 -- Design Report Overview](https://arxiv.org/abs/2603.20342)
...
**TL;DR**: Advanced Virgo Plus（AdV+）升级通过稳定回收腔（stable recycling cavities）、核心干涉仪布局改造及关键子系统更新，显著提升了O5观测期干涉仪性能（operational stability）、噪声抑制（noise reduction）和灵敏度（detector sensitivity），从而扩大天体物理探测范围（astrophysical reach）和科学回报（scientific return）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升高级室女座干涉仪（Advanced Virgo Plus）性能的升级策略，以应对O5观测期需求。

Data: 数据来源包括升级后的稳定回收腔（stable recycling cavities）、改进的中央干涉仪布局、关键子系统更新、新型真空基础设施、悬挂系统、镜面、光学配置、量子降噪系统以及高功率激光技术。

Method: 采用稳定回收腔（stable recycling cavities）、改进中心干涉仪布局及更新关键子系统的综合升级方案，结合量子降噪系统和高功率激光技术提升探测器性能。

Result: 升级后的高级室女座探测器（Advanced Virgo Plus）通过稳定回收腔（stable recycling cavities）、核心干涉仪布局改造及关键子系统更新，显著提升了运行稳定性、噪声抑制能力和探测灵敏度。
...
### [43] [DETECT: A Pipeline to Quantify Detection Thresholds in Rubin for Nearby Targets Embedded in Bright Host Galaxies](https://arxiv.org/abs/2603.20374)
*Tobias Géron,Maria R. Drout,W. V. Jacobson-Galán,C. D. Kilpatrick*

**TL;DR**: 开发了DETECT方法（Detection Efficiency and Threshold Estimation for Characterization of Transients），通过源注入、图像差分和强制测光技术，在明亮宿主星系中可靠检测超新星前身（pre-SN）变源并计算上限阈值，有效抑制了信噪比（SNR）5-10区间的假阳性。


<details>
  <summary>Details</summary>

Background: 研究主题聚焦于利用超新星前身(pre-SN)变率观测约束恒星演化的最终阶段。

Data: 数据来源包括鲁宾天文台（Rubin Observatory）的遗产时空巡天（Legacy Survey of Space and Time, LSST）模拟数据（DP0）和实测数据（DP1）。

Method: 开发了检测效率和阈值估计工具（DETECT），通过源注入（source injection）、图像差分（image subtraction）和强制测光（forced photometry）序列来获取特定宿主星系位置的可靠检测阈值。

Result: 开发的方法DETECT能可靠识别前身星(pre-SN)变率，计算严格上限并抑制亮宿主星系内目标的假阳性。
...
### [44] [Searching for Fast Astronomical Transients in Archival Photographic Plates](https://arxiv.org/abs/2603.20407)
*Ivo Busko*

**TL;DR**: 基于汉堡天文台历史底片独立发现与VASCO项目类似的亚秒级光学瞬变（transients），其半高全宽（FWHM）系统性窄于恒星点扩散函数，支持其为近地轨道扁平旋转物体反射的解释。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用20世纪50年代历史天文底片（photographic plates）搜寻快速天文暂现源（fast astronomical transients），特别是通过分析汉堡天文台（Hamburg Observatory）施密特相机（Schmidt camera）拍摄的连续底片对，独立验证VASCO项目此前在帕洛玛天文台（Palomar Observatory）底片中发现的亚秒级光学闪变现象（sub-second optical flashes）及其可能的地球轨道反射物（reflections from flat, rotating objects in orbit around Earth）解释。

Data: 研究中使用的数据来源于汉堡天文台（Hamburg Observatory）1.2米施密特相机（Großer Schmidtspiegel）在20世纪50年代中期拍摄的存档底片，这些底片由APPLAUSE档案库（APPLAUSE Archive）进行数字化处理并提供图像和检测到的天体表格。

Method: 采用快速序列拍摄的数字化底片对（间隔约30分钟）分析，通过比较瞬变源与恒星点扩散函数（PSF）的半高全宽（FWHM）差异，独立验证了其系统性更窄的特征。

Result: 在汉堡天文台（Hamburg Observatory）的1.2米施密特相机（Großer Schmidtspiegel）拍摄的数字化底片中发现了与VASCO项目（Villarroel et al 2020）类似的快速天文暂现源（fast astronomical transients），其半高全宽（FWHM）系统性窄于恒星点扩散函数（stellar point spread functions），进一步支持了这些暂现源是亚秒级光学闪光的解释，与地球轨道上平坦旋转物体的反射一致。
...
### [45] [Astrophysics Research Organizations in the 21st Century: Database and Comparative Dashboards](https://arxiv.org/abs/2603.20454)
*Michael J. Kurtz,Carlolyn S. Grant,Matthew R. Templeton,The ADS/SciX Team*

**TL;DR**: 基于ADS/SciX数据分析了1949个天文研究机构（institution）和65个国家（country）的科研产出变化，开发了可定制排名和关键绩效指标（KPI）仪表盘。


<details>
  <summary>Details</summary>

Background: 21世纪以来天文学文献数量呈指数增长，同时天体物理学研究的组织结构、执行机构及地域分布发生显著变化。

Data: 数据来源于史密森尼/美国宇航局天体物理学数据系统/科学探索者（ADS/SciX），涵盖了自1997年以来几乎所有发表过天文学研究论文的机构。

Method: 采用基于文章数量和引用的指标集（metrics）分析机构和国家层面的天文学研究表现，并开发关键绩效指标（KPI）仪表盘进行可视化评估。

Result: 21世纪以来天文学论文数量与之前历史总和相当，且天体物理学研究结构、执行机构和地域分布发生显著变化。
...
### [46] [Parallel Adaptive Reweighting Importance Sampling (PARIS)](https://arxiv.org/abs/2603.21249)
*Miaoxin Liu,Alvin J. K. Chua*

**TL;DR**: 提出并行自适应重加权重要性采样（PARIS）方法，通过高斯混合模型（Gaussian mixture）动态调整采样权重，显著降低了引力波（gravitational-wave）参数估计等天体物理高维多模态后验分布的计算成本。


<details>
  <summary>Details</summary>

Background: 研究聚焦于解决天体物理学（尤其是引力波天文学）中高维多模态后验分布（high-dimensional, multi-modal posteriors）的高效采样难题。

Data: 研究中使用的数据来源包括引力波参数估计任务中的观测数据（observational data）和用于验证的示例玩具问题（toy problems）模拟数据（simulation data）。

Method: 采用并行自适应重加权重要性采样（PARIS）方法，通过高斯混合模型（Gaussian mixture）构建动态更新的提议分布，利用现有样本作为分量中心、重要性权重作为分量权重，并引入高效重加权更新机制。

Result: PARIS（Parallel Adaptive Reweighting Importance Sampling）通过高斯混合模型（Gaussian mixture model）构建自适应提案分布，显著减少了高维多模态后验采样所需的目标函数计算次数，在引力波参数估计等任务中实现了精确的后验重构和证据估计。
...
### [47] [Diffuse and specular brightness models applied to LEO satellites. Case study: The ONEWEB constellation](https://arxiv.org/abs/2603.21859)
...
**TL;DR**: 通过分析300多颗ONEWEB卫星的反射特性，发现现有亮度模型（依赖太阳-卫星-观测者相对位置但忽略卫星表面具体朝向）无法完全解释其观测亮度，需结合卫星真实形状、姿态变化及地球反照率效应以改进模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于低地球轨道(LEO)卫星反射特性建模，特别是分析ONEWEB卫星群的亮度观测数据与多种反射模型(漫反射球体、混合反射模型等)的匹配程度。

Data: 使用了来自天体测量和测光观测的数据，以及双行轨道根数（TLE）信息来计算视星等、距离星等以及与太阳、卫星和观测者相关的角度。

Method: 采用漫反射朗伯球体（diffuse Lambertian sphere）、漫反射和镜面反射朗伯球体（diffuse and specular Lambertian sphere）以及相对反射亮度模型分析卫星反射特性，结合天体测量和光度测量数据及双行轨道根数（TLE），利用差分进化蒙特卡洛算法拟合模型参数。

Result: 所有研究的亮度模型（仅依赖太阳-物体-观测者相对位置而忽略反射体表面相对于观测者的具体朝向）均无法完全解释ONEWEB星座卫星的观测亮度。
...
### [48] [Breaking the degeneracy among regular black holes with gravitational lensing](https://arxiv.org/abs/2603.20596)
*Hong Liu,Xiaolong Liao,Yi Zhang*

**TL;DR**: 通过分析Culetu、Bardeen和Hayward正则黑洞（regular black holes）在引力透镜、阴影和准正规模（quasinormal mode）等观测中的参数简并性，发现事件视界望远镜（Event Horizon Telescope）观测能给出正则化参数q的严格限制（Culetu: 0≤q<0.0466，Bardeen: 0≤q<0.5115，Hayward: 0≤q<1.0258），而高阶几何观测量（如李雅普诺夫指数Lyapunov exponent和次级时间延迟）及吸积流亮度分布反转现象是区分不同正则黑洞几何结构的关键特征。


<details>
  <summary>Details</summary>

Background: 研究探讨了Culetu、Bardeen和Hayward正则黑洞（regular black holes）在引力透镜效应、阴影和准正规模（quasinormal mode）等不同观测条件下的参数简并性（parameter degeneracies）问题。

Data: 数据来源包括爱因斯坦环（Einstein ring）观测数据和事件视界望远镜（Event Horizon Telescope）的观测数据。

Method: 采用强引力透镜观测、事件视界望远镜（Event Horizon Telescope）数据以及高阶特征（如李雅普诺夫指数（Lyapunov exponent）和次级时间延迟）分析来研究正则黑洞（regular black holes）的参数简并性。

Result: Event Horizon Telescope观测对Culetu、Bardeen和Hayward正则黑洞的规范化参数q给出了严格限制（Culetu: 0 ≤ q < 0.0466 <0.0847；Bardeen: 0 ≤ q < 0.5115 <0.6682；Hayward: 0 ≤ q < 1.0258 <1.1881），表明强场区域主导了可用参数空间。
...
### [49] [Non-local gravity effects in cosmological dynamics probed by IceCube/KM3NeT signals and dark matter relic abundance](https://arxiv.org/abs/2603.20771)
*Salvatore Capozziello,Gaetano Lambiase,Giuseppe Meluccio*

**TL;DR**: 该论文通过将曲率（curvature）和挠率（torsion）宇宙学模型中的非局域（non-local）引力项标量化（scalarised），研究了其对IceCube/KM3NeT探测的高能中微子通量和暗物质丰度的解释能力，并发现宇宙演化动力学系统对非局域引力参数高度敏感。


<details>
  <summary>Details</summary>

Background: 研究探讨了非局部引力项（non-local gravity terms）在决定宇宙学动力学中的关键作用及其与高能天体物理中微子通量的关联。

Data: 研究中使用的数据来源为IceCube/KM3NeT实验报告的高能天体物理中微子通量（neutrino fluxes）数据，能量范围达220 PeV。

Method: 采用曲率（curvature）和挠率（torsion）的非局部引力模型，通过诺特对称性（Noether symmetries）导出幂律解（power law solutions）分析宇宙动力学演化。

Result: 非局域引力（non-local gravity）参数对宇宙学动力学具有重要影响，特别是在IceCube/KM3NeT探测的高能中微子能区（220 PeV）表现出显著敏感性。
...
### [50] [Dark energy and accelerating cosmological evolution in a Universe with a Weylian boundary](https://arxiv.org/abs/2603.21766)
*Tiberiu Harko,Shahab Shahidi*

**TL;DR**: 通过将引力理论中的边界项扩展为韦尔(Weyl)型非度量几何描述，研究发现包含韦尔矢量及其协变导数的新几何项能有效修正爱因斯坦场方程和弗里德曼方程，其预测与ΛCDM模型高度吻合，为几何起源的暗能量提供了可行替代方案。


<details>
  <summary>Details</summary>

Background: 研究探讨了引力场理论中边界项（boundary terms）的影响，通过将爱因斯坦-希尔伯特作用量（Einstein-Hilbert action）的边界描述为非度规（non-metric）的韦尔型几何（Weyl-type geometry），从而将引力作用和场方程推广至包含来自边界非度规性质的新几何项。

Data: 研究中使用的数据来源为晚期观测数据（late-time observational data）和ΛCDM范式（ΛCDM paradigm）的预测结果。

Method: 采用非度量(non-metric)外尔(Weyl)型几何描述边界，通过引入外尔矢量及其协变导数推广爱因斯坦-希尔伯特作用量和场方程。

Result: 在非度量外尔型(Weyl-type)边界几何框架下，广义引力场方程通过引入外尔矢量及其协变导数，成功将暗能量解释为边界诱导的几何效应，并数值求解出与ΛCDM模型高度吻合的宇宙学演化行为。
...
### [51] [Observational Constraints on Noncoincident $f(Q)$-Gravity with Matter-Gravity Coupling](https://arxiv.org/abs/2603.21881)
*Andronikos Paliathanasis*

**TL;DR**: 研究通过观测数据约束了非对称联络的$f(Q)$引力（$f(Q)=f_0Q^n$）模型，发现其幂律指数最佳拟合值$n\simeq2$，在描述宇宙晚期加速膨胀时与ΛCDM模型具有统计等效性但似然值更高。


<details>
  <summary>Details</summary>

Background: 研究探索了物质-引力耦合的$f(Q)$引力理论作为几何暗能量候选者解释晚期宇宙加速膨胀的现象。

Data: 数据来源于超新星（Supernovae）、重子声波振荡（Baryon Acoustic Oscillations）和宇宙计时器（Cosmic Chronometers）。

Method: 采用非重合连接(noncoincident connection)的$f(Q)$引力理论框架，结合幂律模型$f(Q)=f_0Q^n$进行参数拟合。

Result: 在空间平坦的弗里德曼-勒梅特-罗伯逊-沃克几何框架下，$f(Q)$引力（f(Q)-gravity）作为几何暗能量候选者能够描述晚期宇宙加速膨胀，其幂律模型$f(Q)=f_0Q^n$的最佳拟合指数为$n\simeq2$，与ΛCDM模型相比具有更高的似然值但统计等价。
...
### [52] [Generalized JMN Naked Singularity Models](https://arxiv.org/abs/2603.20676)
*Jay Verma Trivedi,Pankaj S. Joshi*

**TL;DR**: 构建了一类广义JMN裸奇点时空（generalized JMN naked singularity spacetimes），通过引入径向依赖质量函数$F(r)=(M_0+M_n r^n)r^3$形成双参数解族，发现其光子球在外部史瓦西区域时阴影与史瓦西黑洞相同，但吸积盘光谱高频辐射增强且非均匀性参数（inhomogeneity parameter）受限使偏离原始JMN模型较小，证实该广义模型可作为JMN时空的微扰并展现其几何鲁棒性。


<details>
  <summary>Details</summary>

Background: 研究探讨了具有非零切向压力的引力坍缩平衡终态下广义Joshi-Malafarina-Narayan（JMN）裸奇点时空的构建及其观测特性。

Data: 数据来源于对广义JMN裸奇点时空（JMN naked singularity spacetimes）的理论模拟（simulation），通过引入径向依赖的质量函数$F(r)=(M_0+M_n r^n)r^3$构建了两参数解族，并平滑匹配外部史瓦西时空（Schwarzschild spacetime）进行观测特性分析。

Method: 采用广义化的JMN裸奇点时空模型，引入径向依赖的质量函数$F(r)=(M_0+M_n r^n)r^3$构建具有非零切向压力的两参数解族，并平滑匹配外部史瓦西时空。

Result: 构建了一类广义化的JMN裸奇点时空（JMN naked singularity spacetimes），通过引入径向依赖的质量函数$F(r)=(M_0+M_n r^n)r^3$形成双参数解族，平滑匹配外部史瓦西时空（Schwarzschild spacetime），证实其作为JMN时空的小扰动保持几何鲁棒性。
...
### [53] [Polytropes, logotropes, the universal value of the surface density of dark matter halos, and the value of the cosmological constant](https://arxiv.org/abs/2603.21302)
*Pierre-Henri Chavanis*

**TL;DR**: 该论文揭示了天体物理学和宇宙学中对数温态（logotropes）与多方温态（polytropes）的联系，证明对数温态分布函数对应于指数γ=0的多方温态分布函数，其密度有限而压力对数发散，并通过对Lynden-Bell暴力弛豫理论在非简并极限下的分析，为Tsallis熵提供了基于Lynden-Bell熵的理论依据，同时表明对数温态模型能解释暗物质晕的普适表面密度、有效NFW密度尖峰、Tully-Fisher关系以及宇宙常数Λ的精确表达式，为修正牛顿动力学（MOND）理论提供了替代方案。


<details>
  <summary>Details</summary>

Background: 研究探讨了天体物理学和宇宙学中对数物态方程(logotropic equation of state)与多方物态方程(polytropic equation of state)的关联性及其在暗物质晕结构、宇宙学常数推导等领域的理论意义。

Data: 数据来源于理论推导（simulation），基于林登-贝尔（Lynden-Bell）暴力弛豫理论中的非简并极限，并采用卡方分布（χ-squared distribution）描述相空间能级分布。

Method: 采用对数物态方程(logotropic EOS)和多方物态方程(polytropic EOS)的关联分析，结合林登-贝尔(Lynden-Bell)暴力弛豫理论中的非简并极限推导分布函数。

Result: 对数状态方程(logotropic equation of state)是多方状态方程(polytropic equation of state)在极限条件下的退化形式，其中γ趋近于0且K趋近于无穷大时保持A=Kγ为固定值。
...
### [54] [Radial Velocity Orbital Solutions for Candidate Black Hole and Neutron Star Binary Systems in the Gaia Data Release 3 Catalog](https://arxiv.org/abs/2603.20371)
...
**TL;DR**: 该研究通过光谱跟踪观测盖亚(Gaia) DR3双星目录中可能包含黑洞或中子星的大质量函数双星系统，发现除已知的Gaia BH1、Gaia BH2和Gaia NS1外，所有候选暗伴星质量超过2倍太阳质量的双星轨道解均存在错误，并提出更严格的筛选标准，同时在一个加速系统中发现可能为中子星或超质量白矮星的1.16倍太阳质量伴星。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用盖亚(Gaia)卫星DR3双星目录的光谱观测数据，搜寻具有大质量函数(暗示可能存在黑洞或中子星伴星)的双星系统。

Data: 数据来源为盖亚卫星第三次数据发布（Gaia Data Release 3, DR3）的双星系统目录，并针对其中可能包含黑洞或中子星伴星的大质量函数系统进行了光谱后续观测。

Method: 研究采用的核心分析方法包括对盖亚(Gaia) DR3双星目录中选定系统的光谱跟踪观测，结合天体测量和/或光谱双星数据，以及径向速度轨道解算方法。

Result: 在盖亚(Gaia) DR3双星目录中，所有候选暗伴星质量超过2倍太阳质量(Msun)的轨道解均存在错误，仅Gaia BH1、Gaia BH2和Gaia NS1三个已发表双星系统例外。
...
### [55] [Hide and Seek with Gaia. Detectability of Predicted Thin-Disc Metal-Rich RR Lyrae Binaries in Gaia DR3 and DR4](https://arxiv.org/abs/2603.20429)
...
**TL;DR**: 研究发现金属丰度较高([Fe/H]>-0.5)的中龄(2-7 Gyr)天琴RR型变星(RRLs)在银河系薄盘中的双星比例与现有双星演化模型预测存在矛盾，提示需修正观测系统误差或模型假设。


<details>
  <summary>Details</summary>

Background: RR Lyrae变星（RRLs）作为古老恒星族群的经典示踪体，近期证据表明银河系盘存在金属丰度较高（[Fe/H]>-0.5）、中等年龄（2-7 Gyr）的子群体。

Data: 数据来源于盖亚卫星第三次数据释放（Gaia DR3）的天体测量双星目录，并针对100颗具有薄盘运动学特征的金属丰度较高RR Lyrae变星（RRLs）样本进行了分析，同时生成了包含变异性引起天体测量偏差的盖亚观测模拟数据。

Method: 采用基于盖亚(Gaia) DR3观测模拟的贝叶斯推断方法，结合变星引起的天体测量偏差和轨道周期探测效率分析，评估了金属丰度较高([Fe/H]>-0.5)的室女座RR型变星(RRL)双星系统的可探测性。

Result: 金属富集（[Fe/H]>-0.5）的中龄（2-7 Gyr）天琴RR型变星（RRLs）在银河系薄盘中的存在与现有双星演化模型预测存在矛盾，表明可能忽略了盖亚（Gaia）观测中的系统误差或需修正双星演化模型假设。
...
### [56] [A quasi-star is born: formation and evolution of accreting quasi-stars as a metallicity-independent pathway to Little Red Dots](https://arxiv.org/abs/2603.21714)
*J. Roman-Garza,D. Schaerer,C. Charbonnel,T. Fragos,E. Cenci,R. Marques-Chaves,P. Oesch,M. Xiao*

**TL;DR**: 通过模拟准恒星（quasi-stars）的形成演化过程，发现其光度特征与红移z<4.5的"小红点"（Little Red Dots）观测数据匹配，支持准恒星作为其光学辐射源的解释框架，并揭示其质量范围为10^4.5-10^6.5 M⊙、寿命达10^7-10^8年且基本不受金属丰度影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过准恒星（quasi-stars）模型解释高红移（z<4.5）"小红点"（Little Red Dots）的静止光学辐射，探索超大质量恒星（>10^4 M⊙）在快速吸积（accretion）环境下演化至广义相对论不稳定性（general relativistic instability）并形成中心黑洞的物理过程。

Data: 研究使用恒星演化模型（stellar evolution models）模拟了净质量增加率（net mass gain rates）为0.01、0.1和1 M$_{\odot}$/yr且金属丰度（metallicity）Z=0-0.01的准恒星（quasi-stars）形成与演化过程。

Method: 采用恒星演化模型（stellar evolution models）计算净质量增加率（net mass gain rates）为0.01、0.1和1 M$_{\odot}$/yr及金属丰度（metallicity）Z=0-0.01条件下准恒星（quasi-stars）的形成与演化。

Result: 准星（quasi-stars）在质量吸积率≥0.1 M$_{\odot}$/yr时保持近全对流状态，有效温度（$T_\mathrm{eff}$）为4000-9000 K，广义相对论不稳定性导致中心黑洞形成时的恒星质量（$M_\star$）为3.5×10$^4$ M$_{\odot}$（吸积率0.1 M$_{\odot}$/yr）或6.6×10$^4$ M$_{\odot}$（吸积率1 M$_{\odot}$/yr），对应光度（$L$）约10$^9$ L$_{\odot}$。
...
### [57] [Accurate and efficient simulation-based inference for massive black-hole binaries with LISA](https://arxiv.org/abs/2603.20431)
...
**TL;DR**: 开发了基于模拟的精确推理框架（DINGO），用于LISA可观测的高质量（≳10⁷ M☉）黑洞双星（black-hole binaries）参数估计，验证了在信噪比≤500时后验分布稳健一致，信噪比∼1000时仍能生成无偏且紧致的后验样本，支持快速全参数估计并兼容非平稳噪声等复杂场景。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发针对LISA可观测的高质量（$\gtrsim\!10^7 \rm{M_\odot}$）双黑洞（black-hole binaries）的精确模拟推断框架。

Data: 数据来源为使用对齐自旋高阶模波形模型（aligned-spin higher-mode waveform models）和探测器响应的低频近似（low-frequency approximation of the detector response）生成的模拟信号（simulated signals）。

Method: 采用基于模拟的推断框架（simulation-based inference framework），结合归一化流模型（normalizing-flow model）和对齐自旋高阶模波形模型（aligned-spin higher-mode waveform models）以及低频近似探测器响应（low-frequency approximation of the detector response），通过重要性采样（importance-sample）获得真实后验分布。

Result: 开发了一种基于模拟的精确推断框架，适用于LISA可观测的高质量（≳10⁷ M☉）双黑洞系统，扩展了DINGO引力波参数估计代码的应用范围至LISA频段。
...
### [58] [X-ray transients in the Chandra archive: Introducing the cumulative distribution discriminator (CuDiDi)](https://arxiv.org/abs/2603.21791)
*I. Saathoff,J. Larsson*

**TL;DR**: 开发了基于累积分布判别器(CuDiDi)的新检测流程，在钱德拉(Chandra)X射线天文台数据中系统发现了765个高置信度亚观测时标瞬变源(transient)，扩展了短时标高能暂现源样本并建立了挖掘档案数据的通用方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于亚观测时标（sub-observation timescales）X射线暂现源的系统性探测与分类，填补了从恒星耀斑、磁星爆发到河外快速暂现源和超新星激波 breakout 等多样化高能暂现现象的研究空白。

Data: 数据来源于钱德拉X射线天文台（Chandra）的ACIS（Advanced CCD Imaging Spectrometer）仪器获取的20,212次观测数据。

Method: 采用新检测流程结合源识别、光变曲线分析、星表交叉匹配及新型统计分类器累积分布判别器（CuDiDi）。

Result: 通过新的检测流程（CuDiDi）在钱德拉（Chandra）数据中系统发现了765个高置信度瞬变源，涵盖广泛时标、流量和能谱形态，其中短于30秒的软能谱事件可能源于矮星耀发，并扩展了已知瞬变源样本至更短时标。
...
### [59] [Analytical Framework for Expanding Bubbles in a Hot Circumgalactic Medium](https://arxiv.org/abs/2603.21306)
*Jiang-Tao Li*

**TL;DR**: 该研究建立了一个分析框架，描述反馈驱动气泡在热环绕星系介质（CGM）中的四阶段演化（自由膨胀、Sedov-Taylor膨胀、压力修正/跨音速过渡、跨音速后弛豫），发现热晕中强激波演化终止主要由压力限制和跨音速性决定而非剧烈冷却，并量化了气泡尺寸、寿命、X射线辐射和高电离离子柱密度等可观测特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于反馈驱动气泡在高温、充满星系周介质（circumgalactic medium, CGM）环境中的演化动力学，特别关注压力限制和跨声速转变对强激波演化的关键作用。

Data: 数据来源于理论模型推导和数值模拟轨迹（stitched numerical trajectories），对比了适用于热环星系介质（CGM）条件的压力修正模型与经典Sedov-Taylor基准。

Method: 采用自洽解析框架分析反馈驱动气泡在热环绕星系介质(CGM)中的四阶段演化模型，包括自由膨胀、Sedov-Taylor膨胀、压力修正/跨声速过渡及跨声速后弛豫阶段，并推导了界定各阶段的特征半径和时间尺度的标度关系。

Result: 在热晕（hot halos）中，反馈驱动气泡（feedback-driven bubbles）的强激波演化终止通常由压力约束和跨声速（transonicity）决定，而非灾难性冷却（catastrophic cooling）的触发，导致仅出现轻微的超压平衡/跨声速点（pressure-balance/transonic point）后期超调。
...
### [60] [Synchrotron Self-Absorption Spectral Modeling Reveals a Magnetically Driven Shock-in-Jet Scenario in Blazar 1156+295](https://arxiv.org/abs/2603.20701)
*Wancheng Xu,Lang Cui,Tao An,Sándor Frey,Xin Wang,Yuanqi Liu,Ning Chang,Liang Chen,Yingkang Zhang*

**TL;DR**: 通过多波段观测和同步自吸收谱分析，发现耀变体1156+295的射电耀变符合激波驱动框架，其磁通量达到或超过磁滞吸积盘（MAD）阈值，支持磁驱动喷流机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于揭示活动星系核（AGN）中喷流的产生和驱动机制，以理解超大质量黑洞（SMBHs）与其宿主星系的共同演化。

Data: 使用2007至2012年间的Effelsberg单天线多频段射电流量密度数据（radio monitoring）和准同时甚长基线干涉测量（VLBI）图像中的核区尺寸与亮度温度数据。

Method: 采用同步自吸收谱(SSA)建模结合甚长基线干涉(VLBI)测量的核区尺寸和亮度温度，估算喷流磁场强度和磁通量。

Result: 1156+295的射电光变、谱形演化与喷流结构变化符合激波驱动框架，磁通量达到或超过磁滞吸积盘（MAD）阈值且磁能释放先于射电耀斑，支持磁驱动喷流机制。
...
### [61] [Southern eROSITA bubble as a forward shock and the low-metallicity CGM. South-east side story](https://arxiv.org/abs/2603.20740)
*E. Churazov,I. I. Khabibullin,A. M. Bykov,N. N. Chugai,R. A. Sunyaev,V. P. Utrobin,I. I. Zinchenko*

**TL;DR**: 研究发现eROSITA气泡（eROSITA bubbles）东南部可描述为由银河系中心（Galactic Center）瞬态能量释放产生的传播前向激波（forward shock），其物理半径约7-8千秒差距（kpc）、年龄约5-8百万年（Myr），激波前沿正以约700公里/秒（km/s）速度穿过电子密度约3×10⁻⁴厘米⁻³（cm⁻³）、金属丰度≤0.1倍太阳值（Z⊙）的低金属丰度气体，为银晕介质（CGM）提供了原位测量数据，并可能是PeV能量级质子型宇宙线（cosmic rays）的加速场所。


<details>
  <summary>Details</summary>

Background: 研究探讨了银河系中心瞬时能量释放产生的正向激波（forward shock）在低金属丰度（Z≲0.1×Z⊙）的环银河介质（CGM）中传播的物理特性及其对极高能宇宙线加速的潜在贡献。

Data: 数据来源于对eROSITA气泡（eROSITA bubbles）东南部分的观测，通过测量激波前缘（shock front）的物理参数（如半径、膨胀速度、电子密度和重元素丰度）实现对星系周介质（CGM）的原位测量（in situ measurements）。

Method: 采用前向激波传播模型（forward shock）描述eROSITA气泡的东南部分，假设由银河系中心瞬时能量释放驱动。

Result: 南eROSITA气泡的东南部分可合理描述为由银河系中心瞬态能量释放产生的传播前向激波（forward shock），其物理半径约7-8千秒差距（kpc），爆发年龄约5-8百万年（Myr）。
...
### [62] [Detection of a Molecular Cloud toward the Heartbeating Gamma-ray Source near the Microquasar SS 433](https://arxiv.org/abs/2603.21550)
*Tomoharu Oka,Ryo Ariyama,Tatsuya Kotani*

**TL;DR**: 在SS 433/W50系统北部边界发现分子云CO+40.05-2.40与GeV源Fermi J1913+0515位置重合，提出其伽马射线可能源自云内致密天体通过邦迪吸积（Bondi-type accretion）机制产生，且心跳式变光可能由SS 433进动喷流（precessing equatorial outflow）激发的密度波对吸积流的周期性调制导致。


<details>
  <summary>Details</summary>

Background: 研究探讨了费米伽马射线源Fermi J1913+0515与分子云CO+40.05-2.40的关联性及其高能辐射机制。

Data: 数据来源于野边山（Nobeyama）45米望远镜和詹姆斯·克拉克·麦克斯韦望远镜（James Clerk Maxwell Telescope）的毫米波和亚毫米波光谱观测。

Method: 采用毫米波和亚毫米波光谱学（millimeter and submillimeter spectroscopy）结合野边山45米望远镜（Nobeyama 45 m telescope）和詹姆斯·克拉克·麦克斯韦望远镜（James Clerk Maxwell Telescope）观测数据，分析分子云物理特性并排除激波加热或增强激发可能性。

Result: result:在分子云CO+40.05-2.40中观测到的GeV辐射可能源于云内嵌藏的致密天体通过邦迪(Bondi)型吸积过程产生，其心跳式变光特性反映了SS 433进动喷流引发的密度波对吸积流的周期性调制。
...
### [63] [Inverse Electroweak Baryogenesis](https://arxiv.org/abs/2603.20414)
*Jacopo Azzola,Oleksii Matsedonskyi,Andreas Weiler*

**TL;DR**: 提出了一种基于弱电规范场拓扑跃变（electroweak sphaleron transitions）的平衡重子生成机制，通过在存在近似守恒全局电荷X（global charge X）的非对称等离子体中诱导补偿规范电荷密度，使弱电规范场拓扑跃变更倾向于产生非零重子数，该机制可在与标准电弱重子生成不同的参数依赖性和相变动力学下实现更高温度的重子生成。


<details>
  <summary>Details</summary>

Background: 该研究探讨了在电弱对称性破缺（electroweak symmetry breaking）区域通过弱拓扑孤子（weak sphalerons）产生重子不对称性（baryon asymmetry）的新机制，提出了一种与标准电弱重子生成（electroweak baryogenesis）在参数依赖性和相变动力学上存在本质差异的替代路径。

Data: 数据来源于理论模拟（simulation）分析弱电（electroweak）鞘层（sphaleron）在特定温度条件（$h/T\lesssim 1$）下的平衡响应机制。

Method: 采用弱规范场拓扑跃迁（weak sphaleron transitions）在电弱对称性破缺（electroweak symmetry breaking）区域保持非抑制状态（$h/T\lesssim 1$）时产生的平衡响应机制，结合近似守恒全局电荷$X$的非对称性及等离子体屏蔽（plasma screening）效应导致的规范电荷中性约束。

Result: 提出了一种重子生成机制，其中重子不对称性作为弱瞬子(sphaleron)在电弱瞬子跃迁未被抑制区域($h/T\lesssim 1$)的平衡响应产生，通过近似守恒的全局电荷$X$和等离子体屏蔽效应实现非零平衡重子密度。
...
### [64] [Kinky vortons in the 2HDM](https://arxiv.org/abs/2603.22040)
*Richard A. Battye,Steven J. Cotterill,Adam K. Thomasson*

**TL;DR**: 在$\mathbb{Z}_2$对称全局双希格斯二重态模型(2HDM)中发现了多种动态稳定的环状电流解(kinky vortons)，并通过薄弦近似和弹性弦形式精确描述了其平衡半径和振荡频率，同时提出了一种由次级畴壁支撑局域凝聚的复合畴壁构型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于在具有$\mathbb{Z}_2$对称性的全局双希格斯二重态模型(2HDM)中构建和分析携带电流的二维环形解（kinky vortons），探索其在标准模型现象学扩展中的稳定性与动力学特性。

Data: 数据来源于理论模拟（simulation）构建的$\mathbb{Z}_2$对称全局双希格斯二重态模型（2HDM）中的二维载流环状解（kinky vortons）及其复合畴壁构型。

Method: 采用薄弦近似（thin string approximation）和弹性弦形式（elastic string formalism）精确描述$\mathbb{Z}_2$对称全局双希格斯二重态模型（2HDM）中动态稳定的环形电流解（kinky vortons）的平衡半径和振荡频率。

Result: 在$\mathbb{Z}_2$对称的全局双希格斯二重态模型(2HDM)中构建并分析了携带电流的二维环形解(kinky vortons)，证明了存在多种在非轴对称扰动下保持动态稳定的构型。
...
### [65] [Modelling solar radial velocities and photometric variability with SOAPv4](https://arxiv.org/abs/2603.21413)
...
**TL;DR**: 通过数学形态学（MM）变换精确识别太阳活动区（active regions），并基于SOAPv4模型模拟太阳径向速度（RV）和测光变化，将观测值与模拟值的残差标准差降至约0.91米/秒，验证了该模型在量化黑子（spots）和光斑（faculae）对太阳RV及测光影响方面的可靠性。


<details>
  <summary>Details</summary>

Background: 恒星活动性是限制利用视向速度（RV）测量探测类地行星的主要因素之一，而太阳作为唯一可解析表面特征的恒星为研究活动区对RV和光度变化的影响提供了独特测试平台。

Data: 数据来源于SDO（Solar Dynamics Observatory）的HMI（Helioseismic and Magnetic Imager）和AIA（Atmospheric Imaging Assembly）图像，以及HARPS-N（High Accuracy Radial velocity Planet Searcher-North）的径向速度（RV）观测数据和VIRGO/SPM（Variability of Solar Irradiance and Gravity Oscillations/Sun Photometers）的光度测量数据。

Method: 采用数学形态学变换（MM transforms）处理SDO/HMI和AIA图像识别活动区，并利用SOAPv4模型模拟太阳径向速度（RV）和测光变率。

Result: 数学形态学（MM）变换准确识别太阳活动区，基于此输入的SOAPv4模型重现了观测到的太阳径向速度（RV）变异性，残差标准差约为0.91 m/s。
...
### [66] [QLP Data Release Notes 004: TESS-Gaia Light Curve Photometry Implementation](https://arxiv.org/abs/2603.22236)
...
**TL;DR**: 通过采用TESS-Gaia光变曲线包的光度测量方法，改进后的QLP（Quick-Look Pipeline）在10.5 < T < 13.5星等范围内提供了噪声更低、精度更高的光变曲线，增强了微弱信号探测能力。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于提升TESS（Transiting Exoplanet Survey Satellite）快速查看管线(Quick-Look Pipeline, QLP)对暗弱恒星的光变曲线测量精度，以支持基于机器学习系外行星搜索的科学目标。

Data: 数据来源于TESS（Transiting Exoplanet Survey Satellite）观测的每27.4天生成多达200万颗恒星的光变曲线，并通过TESS-Gaia光变曲线包（TESS-Gaia Light Curve package）改进测光方法，适用于星等范围10.5 < T < 13.5的恒星。

Method: 采用TESS-Gaia光变曲线包（TESS-Gaia Light Curve package）的光度测量方法改进原始QLP测光，在10.5 < T < 13.5星等范围内提供更低噪声特性，同时保留孔径测光（aperture photometry）并输出3种孔径和3种去趋势处理。

Result: 采用TESS-Gaia光变曲线包的光度测量方法改善了QLP在10.5 < T < 13.5星等范围内的噪声特性，从第94扇区开始为用户提供更高精度的光变曲线并提升了暗弱信号的探测能力。
...
### [67] [Isotopic Signature of Organic Molecules from Beyond the Solar System: An Enriched Methane D/H Ratio in the Interstellar Object 3I/ATLAS](https://arxiv.org/abs/2603.20445)
...
**TL;DR**: 通过詹姆斯·韦伯太空望远镜观测发现星际天体3I/ATLAS彗发中甲烷的氘氢比（D/H）异常高（3.31±0.34%），表明其形成于原行星盘局部极冷环境且氘元素丰度远高于太阳系。


<details>
  <summary>Details</summary>

Background: 星际天体（interstellar objects）的挥发性成分可揭示其宿主恒星周围行星形成过程，是研究系外行星系统的重要窗口。

Data: 数据来源于詹姆斯·韦伯太空望远镜（James Webb Space Telescope）对星际天体3I/ATLAS彗发进行的近红外光谱观测。

Method: 采用詹姆斯·韦伯空间望远镜(JWST)获取的近红外光谱数据，结合甲烷(CH$_4$)及其氘代产物(CH$_3$D)的丰度比(D/H)分析技术。

Result: 在星际天体3I/ATLAS的彗发中检测到异常高的甲烷氘氢比（D/H = 3.31±0.34%），表明其形成于原行星盘中局部极冷环境导致的高氘氢元素比例区域。
...
### [68] [Coma Physics of an Interstellar Object: JWST Spatial-Spectral Mapping of 3I/ATLAS](https://arxiv.org/abs/2603.20460)
...
**TL;DR**: 利用詹姆斯·韦伯太空望远镜（JWST）首次在星际天体3I/ATLAS远离太阳至日心距2.37-2.41天文单位（au）时探测到其挥发物中CO主导的分子丰度分布（CO占比最高，H₂O和CO₂相对丰度分别为44.4%和42.4%），并发现非极性分子在彗发中的空间分布呈现显著各向异性，而极性分子分布更对称，揭示了该天体冰核中挥发物的分馏机制及彗发加热冷却过程。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用詹姆斯·韦伯太空望远镜（JWST）观测星际天体3I/ATLAS在日心距2.37-2.41天文单位（au）处挥发物的分子发射，探索其穿越水冰线（H$_2$O ice line）时分子化学组成的变化规律。

Data: 数据来源于詹姆斯·韦伯太空望远镜（JWST）在2025年12月22日至23日对星际天体3I/ATLAS进行的分子发射观测，当时该天体位于日心距（heliocentric distance）2.37-2.41天文单位（au）范围内。

Method: 利用詹姆斯·韦伯太空望远镜（JWST）对星际天体3I/ATLAS的分子发射进行光谱测量，获取CO、CO₂、H₂O、CH₃OH和CH₄的柱密度和转动温度的空间-光谱分布图。

Result: 在星际天体3I/ATLAS的彗发中，非极性分子（如CO、CO$_2$）的柱密度和转动温度空间分布呈现高度各向异性，而极性分子（如H$_2$O、CH$_3$OH）则接近对称分布。
...
### [69] [A New Method of Measuring Magnetic Field Strength in Highly Structured Protostellar Envelopes](https://arxiv.org/abs/2603.20515)
*Yisheng Tu,Xiaoyuan Yang,Zhi-Yun Li*

**TL;DR**: 提出了一种基于磁流体动力学动量方程的新方法，通过观测可获取的投影引力加速度（g_R）和假想盘（pseudodisk）面密度（Σ）结合两个标定参数（a_{b,R}和γ_{zR}），直接估算原恒星包层（protostellar envelopes）中垂直和总磁场强度（B_z），并验证了该方法在湍流或非湍流环境中的普适性及对L1157的适用性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原恒星(protostellar)坍缩和盘形成过程中磁场(magnetic field)强度的直接测量难题。

Data: 研究使用非理想磁流体动力学（non-ideal magnetohydrodynamic）模拟数据验证方法假设、校准关键参数并评估其时空变化特性。

Method: 基于磁流体动力学动量方程，通过观测可获取的投影重力加速度和假想盘（pseudodisk）面柱密度，结合表征磁力贡献的无量纲参数$a_{b, R}$和磁场几何参数$γ_{zR}$，建立垂直和总磁场强度的估算方法。

Result: 提出了一种基于磁流体动力学动量方程的新方法，通过观测可获取的伪盘（pseudodisk）面柱密度和坍缩中心投影重力加速度，结合两个表征磁力贡献与几何结构的无量纲参数（$a_{b,R}$和$γ_{zR}$），直接估算坍缩原恒星包层中的垂直与总磁场强度。
