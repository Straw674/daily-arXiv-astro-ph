...
**TL;DR**: 首次在礁湖星云（M 8）中获得了空间分辨的氧复合线（RL）丰度图和丰度差异因子（ADF）图，发现全局平均ADF为0.47±0.02 dex且在中心区域呈现0.35-0.50 dex的径向变化。


<details>
  <summary>Details</summary>

Background: 研究聚焦于电离星云中重离子碰撞激发线（CELs）与复合线（RLs）丰度差异（ADF）这一未解难题。

Data: 数据来源于SDSS-V Local Volume Mapper项目获取的覆盖整个Lagoon Nebula（M 8）的深度积分场光谱数据集，空间分辨率为每个spaxel 0.21 pc。

Method: 采用积分视场光谱技术（integral field spectroscopy）获取高空间分辨率数据，结合碰撞激发线（CELs）和复合线（RLs）的电子温度与密度诊断，首次构建了空间分辨的氧复合线丰度图及丰度差异因子（ADF）分布图。

Result: 首次在H II区（H II region）中构建了空间分辨的ADF(O2+)图，并得出全球平均ADF约为0.47±0.02 dex，中心区域ADF径向变化范围为0.35-0.50 dex。
...
**TL;DR**: 利用詹姆斯·韦伯太空望远镜（JWST）的中红外（mid-IR）数据发现了一个红移达z~6的此前未被识别的遮蔽活动星系核（AGN）群体，揭示了早期宇宙中超大质量黑洞增长主要发生在其他波段难以探测的强遮蔽阶段，遮蔽比例在4.5 < z < 6时高达98-99%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用中红外（mid-IR）波段探测高红移（z ~ 6）被尘埃遮蔽的活动星系核（obscured AGN），以完善AGN活动与黑洞增长的完整普查，并揭示早期宇宙中超大质量黑洞增长可能主要发生在其他波段难以观测的强遮蔽阶段。

Data: 数据来源于詹姆斯·韦伯太空望远镜（JWST）的中红外仪器（MIRI）成像数据（来自MEOW巡天）、近红外相机（NIRCam）数据、光谱数据以及哈勃太空望远镜（HST）成像数据。

Method: 采用光谱能量分布（SED）建模分析詹姆斯·韦伯太空望远镜（JWST）中红外仪器（MIRI）探测到的源，结合近红外相机（NIRCam）、光谱数据和哈勃太空望远镜成像数据识别高红移（z ~ 6）的遮蔽活动星系核（AGN）。

Result: 通过中红外（mid-IR）选择揭示了一个此前未被识别的高红移（z ~ 6）遮蔽活动星系核（AGN）群体，其遮蔽比例随红移增加而显著上升（4.5 < z < 6时达98-99%），表明早期超大质量黑洞增长主要发生在其他波段难以探测的强遮蔽阶段。
...
### [3] [Reinterpreting the puzzling properties of z>6 galaxies within a variable IMF framework](https://arxiv.org/abs/2603.22405)
...
**TL;DR**: 这篇论文通过引入受宇宙射线(CR)调控的变星初始质量函数(IMF)的GAEA模型，成功解释了JWST观测到的z>6星系群（包括紫外光度函数和恒星质量函数）的演化特征，并指出变IMF对恒星质量估算误差和早期星系形成模型判别的重要性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用可变恒星初始质量函数（IMF）改进星系形成理论模型，以解释詹姆斯·韦伯太空望远镜（JWST）观测到的高红移（z>7）明亮大质量星系数量远超理论预期的现象。

Data: data来源为詹姆斯·韦伯太空望远镜（JWST）的观测数据以及基于GAlaxy Evolution and Assembly（GAEA）模型的模拟数据。

Method: 采用可变恒星初始质量函数（IMF）的GAEA星系演化与组装模型，结合宇宙射线（CR）调控恒星形成率（SFR）的数值模拟启发机制，将SFR密度作为CR密度代理以关联IMF形态与星际介质物理条件。

Result: 采用可变初始质量函数（IMF）的GAEA模型能重现z>6星系群的多个观测特性，包括高达z~13的紫外光度函数演化，而无需额外调整反馈模型。
...
### [4] [Detailed Analysis of the NGC 2168 Cluster, Leveraging Gaia DR3](https://arxiv.org/abs/2603.22410)
*Nasser M. Ahmed,Remziye Canbay,Deniz Cennet Çınar*

**TL;DR**: 基于盖亚(Gaia) DR3和2MASS数据，研究发现NGC 2168（M35）是一个年龄190±12 Myr、距离840±54 pc的年轻星团，具有延伸恒星晕、垂直潮汐形变特征及多模态高斯质量函数，揭示了其复杂的动力学形成历史。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用NGC 2168（M35）作为关键基准，探索恒星演化及年轻与中等年龄星群过渡阶段的动力学环境。

Data: 数据来源包括盖亚卫星(Gaia)第三次数据释放(DR3)的高精度天体测量和测光数据，并辅以2MASS数据。

Method: 采用高精度天体测量（Gaia DR3）和测光数据（2MASS）进行运动学、结构及天体物理特性分析，结合等龄线拟合（isochrone fitting）和广义金模型（generalized King model）确定基本参数。

Result: NGC 2168（M35）的径向密度分布符合广义金模型（generalized King model），显示出存在一个松散束缚的延伸恒星晕（stellar halo）。
...
### [5] [A systematic study of AGN feedback in a disk galaxy II: MACER prediction of X-ray surface brightness profile and comparison with eROSITA observations](https://arxiv.org/abs/2603.22412)
*Yuxuan Zou,Feng Yuan,Suoqing Ji,Lin He,Zhiyuan Li,Yi Zhang,Johan Comparat,Zhijie Qu,Taotao Fang*

**TL;DR**: 基于MACER框架模拟的盘状星系活动星系核(AGN)反馈模型预测的X射线表面亮度轮廓与eROSITA观测的星系周介质(CGM)辐射数据在10-100千秒差距范围内高度吻合，证实了X射线辐射主要为热起源。


<details>
  <summary>Details</summary>

Background: 研究聚焦于活动星系核(AGN)反馈对盘状星系中星系周介质(CGM)的X射线辐射特性的影响及其与观测数据的对比验证。

Data: 数据来源于MACER框架下的模拟数据（simulation data）和两组堆叠的eROSITA径向X射线表面亮度剖面观测数据（Y. Zhang et al. 2024和L. He & Z. Li 2026）。

Method: 采用MACER框架进行数值模拟，直接利用Paper I的模拟数据计算X射线表面亮度轮廓，并与eROSITA观测的星系周介质(CGM)辐射叠加轮廓进行对比。

Result: 模拟的X射线表面亮度轮廓与eROSITA观测的累积轮廓在较大径向范围内（约100千秒差距）吻合良好，表明eROSITA探测到的X射线辐射主要为热起源。
...
### [6] [CHANG-ES. XXXVIII. A Thin Radio Halo Shaped by Slow Cosmic-Ray Transport in the Quiescent Galaxy NGC 4565](https://arxiv.org/abs/2603.22417)
...
**TL;DR**: 对近邻侧向旋涡星系NGC 4565的射电连续谱观测揭示了其垂直紧凑的辐射结构（平均高斯晕尺度高度约3.0千秒差距）、弱且主要平行于盘面的磁场（等分强度≲5微高斯），以及局域性垂直磁场成分暗示的X形结构，结合宇宙射线输运模型支持低速通量管平流情景，表明低恒星形成率、弱磁场和低效宇宙射线输运共同抑制了延展射电晕的形成。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近邻侧向旋涡星系NGC 4565的射电连续谱辐射特性及其磁场结构，旨在揭示低恒星形成率环境下宇宙线（CR）传输机制与星系晕形成条件的物理关联。

Data: 数据来源于甚大天线阵(VLA)C阵列S波段(2-4 GHz)射电连续谱观测，属于邻近星系连续谱晕巡天(CHANG-ES)项目对NGC 4565的观测数据。

Method: 采用旋转测量合成（rotation measure synthesis）分析磁场结构，并利用一维宇宙射线输运模型（1-D cosmic ray transportation models）拟合垂直射电连续谱强度分布。

Result: NGC 4565的射电连续谱辐射在垂直方向上较为紧凑，垂直与径向延伸比约为1/6，其垂直轮廓最优拟合为双高斯分量分布，平均高斯晕尺度高度约3.0千秒差距。
...
### [7] [Carbon Nitride Monolayer Nanosheets: Astrochemical Insights into the Fate of Interstellar Hydrogen](https://arxiv.org/abs/2603.22443)
*Dubois David,Guichard Pierre,Pasquier Remi*

**TL;DR**: 研究发现二维碳氮单层结构（2D-CN）可作为星际介质中氢原子吸附和氢分子形成的新型潜在载体，其表面多种位点对氢原子具有能量有利的结合特性。


<details>
  <summary>Details</summary>

Background: 原子氢（atomic hydrogen）作为宇宙中广泛存在的中性气体成分（占银河系的70%），可通过物理吸附（physisorb）或化学吸附（chemisorb）作用在星际尘埃颗粒和冰幔表面，促进H2及更复杂氢化分子的形成，而具有高比表面积的硅酸盐、无定形碳、石墨烯片或水冰等结构可能通过异相化学（heterogeneous chemistry）促进天体物理环境中复杂有机物质的形成。

Data: data：研究采用密度泛函理论（DFT）计算分析了C2N1、C3N1、C3N2、C3N4、C4N3、C6N6、C6N8、C9N4和C9N7单层纳米片上的氢吸附特性。

Method: 采用密度泛函理论（DFT）计算研究了氢原子在多种二维碳氮单层结构（2D-CN）上的吸附特性。

Result: 提出了二维石墨状碳氮单层结构（2D-CN）作为可能影响星际化学的分子家族，并通过密度泛函理论（DFT）计算发现其表面存在多个氢原子吸附位点，可能促进星际介质（ISM）中氢的物理化学处理和演化。
...
### [8] [The Two Component Circumgalactic Medium Emission around z~2 Radio-loud Quasars](https://arxiv.org/abs/2603.22657)
...
**TL;DR**: 通过KCWI观测7个红移~2的射电噪类星体（radio-loud quasars）及其周围80-120千秒差距的Lyα和10-40千秒差距的He II与C IV辐射，发现内圈（<30千秒差距）Lyα亮度比文献样本高2-10倍，且He II与C IV辐射由类星体光致电离驱动并沿射电喷流轴逃逸，支持星系周介质（CGM）内外圈的两成分模型。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用凯克宇宙网成像仪（KCWI）观测红移~2的射电噪类星体（radio-loud quasars）周围80-120千秒差距的莱曼α（Lyα）及10-40千秒差距的氦II（He II）与碳IV（C IV）发射现象，并探讨其与射电喷流（radio jet）辐射的关联性。

Data: 数据来源包括凯克宇宙网成像仪（KCWI）对7个红移约2的射电噪类星体的Lyα、He II和C IV观测，以及VLA和ALMA档案射电观测数据。

Method: 采用凯克宇宙网成像仪(KCWI)观测莱曼α(Lyα)、氦II(He II)和碳IV(C IV)发射线，并结合甚大阵(VLA)和阿塔卡马大型毫米波阵列(ALMA)的射电数据对比分析。

Result: 在红移~2的射电噪类星体(radio-loud quasars)周围观测到80-120千秒差距直径的Lyα发射和10-40千秒差距的He II与C IV发射。
...
**TL;DR**: 通过分析440对星系对（包括364对旋涡-旋涡星系和76对旋涡-椭圆星系），发现小投影间距（<50 h⁻¹kpc）的成对星系尤其是旋涡-旋涡对中氢原子气体分数降低8.8%、恒星形成效率提高15%，表明并合过程会高效消耗气体并增强恒星形成，而26对红旋涡星系则表现出类似椭圆星系的氢原子气体匮乏和恒星形成抑制特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原子氢（HI）在星系相互作用系统（特别是星系对）中的行为及其对恒星形成的影响。

Data: 数据来源于FAST全天空中性氢巡天（FASHI）的河外星系中性氢（HI）目录与已建立的孤立星系对样本的交叉匹配。

Method: 采用交叉匹配(cross-matching) FAST全天空中性氢巡天(FASHI)的河外星系中性氢(HI)目录与孤立星系对样本的方法，构建了包含440个星系对的最大并合系统HI探测样本。

Result: 配对星系（特别是投影距离小于50 h⁻¹kpc的螺旋-螺旋对）的原子氢（HI）气体分数比对照样本低8.8%，且螺旋-螺旋对的恒星形成效率（SFE_HI）比对照样本高约15%，表明并合过程能有效消耗HI气体并增强恒星形成。
...
### [10] [The Fate of the Milky Way--Andromeda System: To Merge or Not?](https://arxiv.org/abs/2603.22863)
*Hao Wu,Yang Huang,Huawei Zhang,Guangze Sun,Shi Shao*

**TL;DR**: 基于最新盖亚(Gaia)天体测量数据并考虑大麦哲伦云(LMC)和M33动力学影响，研究发现银河系(MW)与仙女座星系(M31)在65亿年后合并概率提升至90%，但当前测量仍存在不确定性，需更高精度自行运动(PM)数据才能最终确认。


<details>
  <summary>Details</summary>

Background: 研究探讨了银河系(MW)与仙女座星系(M31)在考虑大麦哲伦云(LMC)和M33动力学影响下的并合概率及其对本地星系群(LG)演化的意义。

Data: 研究使用了基于最新和最精确的盖亚卫星(Gaia)自行运动数据(PMs)，并校正了盖亚天体测量中的系统偏移。

Method: 采用半解析框架（semi-analytic framework）结合最新盖亚（Gaia）天体测量数据修正后的M31和M33自行运动（proper motions），通过径向-切向轨道平衡及卫星介导效应（satellite-mediated effect）的双通道敏感性分析评估合并概率。

Result: 银河系(MW)与仙女座星系(M31)的合并概率提升至90%，中位合并时间为6.5_{-1.5}^{+1.3} Gyr。
...
**TL;DR**: 这篇论文重新定义了星际消光曲线（extinction curve）为红化恒星与无介质时恒星光谱的近红外至紫外的比值F*/F0，发现银河系具有2200 Å突起的紫外消光曲线可分为三类，并提出这些曲线并非传统认为的不同粒子消光规律的混合，而是由相干前向散射星光导致偏离普适指数消光规律的结果。


<details>
  <summary>Details</summary>

Background: 研究探讨了银河系紫外星际消光曲线（ultraviolet Galactic interstellar extinction curves）的解释问题，特别是关于2200 Å吸收峰（2200 A bump）与金属丰度（metallicity）之间未被观测充分支持的假设关联。

Data: 数据来源为银河系恒星的红外至紫外光谱（F*/F0），通过比较有红化与无红化恒星的光谱比定义消光曲线（extinction curve）。

Method: 采用紫外外推法（ultraviolet extrapolation）将银河系消光曲线（extinction curves）与近红外至光学段的指数消光律（exponential extinction law）关联，并基于此将所有消光曲线划分为三种类型。

Result: 紫外银河系星际消光曲线可分为三种类型，其解释需考虑相干前向散射星光对普遍指数消光律的偏离贡献。
...
**TL;DR**: 通过拟合EAGLE宇宙学模拟中约31,000个恒星形成星系的合成光谱能量分布(SED)，发现MAGPHYS模型在理想条件下能较准确恢复恒星质量、恒星形成率等参数（偏差<0.14 dex），但恒星年龄和尘埃特性受恒星形成史先验假设和波长覆盖范围限制影响较大。


<details>
  <summary>Details</summary>

Background: 研究探讨了利用光谱能量分布(SED)模型从多波段测光数据推断星系物理性质的准确性及其系统偏差来源。

Data: 数据来源于EAGLE宇宙学模拟（EAGLE cosmological simulations）中约31,000个恒星形成星系的合成光谱能量分布（SED），并使用SKIRT辐射传输代码（SKIRT radiative transfer code）进行后处理，同时构建了类似WAVES巡天的模拟样本，包含光学和近红外测光数据及实际观测误差。

Method: 采用MAGPHYS光谱能量分布(SED)建模框架对EAGLE宇宙学模拟中约31,000个恒星形成星系进行合成SED拟合，并结合SKIRT辐射传输代码进行后处理。

Result: 在理想条件下，MAGPHYS光谱能量分布(SED)建模框架能准确恢复恒星质量、恒星形成率、比恒星形成率、尘埃质量和尘埃光度（偏差<~0.14 dex），但质量加权恒星年龄无法可靠约束。
...
### [13] [The time-delay model and its applications to galactic archaeology](https://arxiv.org/abs/2603.23066)
*Francesca Matteucci*

**TL;DR**: 该论文通过时延模型（time-delay model）解释恒星和星际气体中[X/Fe]与[Fe/H]的丰度模式，揭示了α元素（alpha-elements）与铁（Fe）的产量时间差源于大质量恒星与Ia型超新星（Type Ia supernovae）的不同演化时标，并应用于银河系和不同形态星系化学演化及高红移（high redshift）天体丰度分析。


<details>
  <summary>Details</summary>

Background: 该研究探讨了基于不同质量恒星寿命产生不同元素的时延模型（time-delay model），用于解释[X/Fe]与[Fe/H]图（X为碳至铀的任意元素丰度）中观测到的化学丰度模式，从而约束恒星核合成过程及星系的恒星形成历史。

Data: 数据来源包括恒星和星际气体中观测到的元素丰度模式（[X/Fe] vs. [Fe/H]），以及银河系和不同形态类型星系的化学演化观测数据。

Method: 采用时间延迟模型(time-delay model)解释恒星和星际气体中元素丰度模式，通过不同质量恒星的元素产生时间差异约束恒星核合成(nucleosynthesis)和星系恒星形成历史。

Result: 时间延迟模型（time-delay model）通过元素丰度模式（[X/Fe] vs. [Fe/H]）有效约束恒星核合成过程及星系恒星形成历史。
...
### [14] [Intracluster light is a close tracer of the dark matter halo shape](https://arxiv.org/abs/2603.23158)
*Adela Fernandez,Yannick Bahé,Nina Hatch,Joseph Butler,Tutku Kolcu,Garreth Martin,Mireia Montes*

**TL;DR**: 研究发现星系团内光（ICL）与暗物质（DM）晕的形状高度一致，其主轴偏差小于10度、轴比差异约0.07，且ICL比卫星星系更能准确追踪DM晕的椭率和取向。


<details>
  <summary>Details</summary>

Background: 研究探讨了星系团内光（intracluster light, ICL）能否作为暗物质晕（dark matter haloes）形状的可靠示踪剂。

Data: 数据来源于宇宙学Hydrangea星系团模拟（cosmological Hydrangea cluster simulations）。

Method: 采用形状张量（shape tensor）在同心椭球壳层中测量星系团内光（ICL）和暗物质（DM）晕的三维及投影形状。

Result: 星系团内光（ICL）能可靠地追踪暗物质（DM）晕的形状，两者在三维和投影方向上主轴夹角小于10度且轴比差异仅约0.07，DM稍圆于ICL。
...
### [15] [First Detection of the Glycine Isomer Glycolamide in Hot Molecular Core](https://arxiv.org/abs/2603.23170)
*Chunguo Duan,Fengwei Xu,Qian Gou,Xuefang Xu,Donghui Quan,Laurent Pagani,Xi Chen,Jun Kang,Jiaxin Du*

**TL;DR**: 首次在热分子核(hot molecular core)G358.93-0.03 MM1中检测到乙醇酰胺(glycolamide)，证实了不同环境中酰胺类化合物(amides)的化学连续性及其在大质量恒星形成过程中的持续存在与演化。


<details>
  <summary>Details</summary>

Background: 探索星际化学（interstellar chemistry）向生命演化的连续性需要验证前生命分子（prebiotic molecules）能否在恒星形成的能量阶段存续与重组。

Data: 数据来源于ALMA（阿塔卡马大型毫米波/亚毫米波阵列）1毫米波段对热分子核G358.93-0.03 MM1的观测。

Method: 采用ALMA（阿塔卡马大型毫米波/亚毫米波阵列）1毫米波段观测数据，通过谱线分析识别了7条未混叠或轻度混叠的发射线，结合丰度计算和化学网络比较方法。

Result: 在G358.93-0.03 MM1热分子核中首次探测到乙醇酰胺（glycolamide），丰度为(1.7±0.2)×10^{-10}（相对于H_{2}），表明酰胺类分子在恒星形成过程中持续存在并化学演化。
...
### [16] [How Massive Can a Population III Starburst Be? Simulating the First Galaxies with High Lyman-Werner Background](https://arxiv.org/abs/2603.23209)
*Tae Bong Jeong,Alessandra Venditti,Volker Bromm,Myoungwon Jeon,Tiger Yu-Yang Hsiao,Steven L. Finkelstein,John Chisholm*

**TL;DR**: 通过宇宙学辐射流体动力学模拟发现，在再电离时期（EoR）高莱曼-维尔纳（LW）背景辐射（$\gtrsim 10^3J_{21}$）条件下，质量为$M_{\rm vir} \approx 10^{8} \msun$的原子冷却晕（atomic-cooling halos）内可发生延迟但剧烈的第三星族（Pop~III）恒星爆发，但其质量上限为$M_{\star, \rm Pop~III} <10^6\msun$。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索再电离时期(EoR)内大质量第三星族(Pop~III)恒星暴发形成的物理条件及其观测可能性。

Data: 使用宇宙学辐射流体动力学放大模拟（cosmological radiation-hydrodynamic zoom-in simulations）数据，重点关注维里质量（virial mass）约$10^{8} \msun$、红移$7 \lesssim z \lesssim 8$的原子冷却晕（atomic-cooling halos），并调节莱曼-维尔纳（Lyman-Werner, LW）背景辐射强度至$J_{\rm LW} \leq 10^4J_{21}$，同时设定高达$ε_{\rm ff} = 1.0$的恒星形成效率（star formation efficiency）。

Method: 采用宇宙学辐射流体动力学放大模拟（cosmological radiation-hydrodynamic zoom-in simulations），通过调节莱曼-维尔纳背景辐射（Lyman-Werner background radiation）强度至$J_{\rm LW} \leq 10^4J_{21}$并设定高恒星形成效率（$ε_{\rm ff} = 1.0$），探究再电离时期（EoR）内产生大质量第三星族（Pop~III）星暴所需的物理条件。

Result: 在足够高的莱曼-维尔纳(Lyman-Werner, LW)背景辐射通量(≥10³J₂₁)条件下，再电离时期(EoR)仍可发生延迟但剧烈的第三星族(Pop~III)恒星爆发，但其质量受限于首次超新星爆发后的内部金属增丰，恒星形成总质量M⋆,Pop~III<10⁶M☉。
...
**TL;DR**: 通过贝叶斯框架整合多波段数据构建银河系中央分子区(CMZ)三维模型，发现其气体分布符合嵌套x2轨道族结构，主要轴范围72-146 pc，且大多数分子云位于银河中心近侧。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中央分子区(CMZ)的三维结构建模，旨在揭示其与恒星形成周期、黑洞吸积及星系核演化的关联。

Data: 数据来源包括银河系中心分子区（CMZ）分子云目录中的尘埃消光（dust extinction）、吸收（absorption）、恒星密度（stellar densities）、X射线回波（X-ray echoes）和自行运动（proper motions）等多波段电磁谱数据集。

Method: 采用贝叶斯框架（Bayesian framework）结合多波段电磁谱数据，并开发近/远距离指标（near/far metrics）计算分子云的后验位置概率密度函数（PPDF）。

Result: 银河系中央分子区(CMZ)的三维结构可通过嵌套x$_2$轨道族(轨道长轴72-146 pc)较好描述，其气体分布比单一闭合或开放椭圆轨道更为复杂。
...
**TL;DR**: 研究发现，在宇宙再电离（cosmic reionization）辐射传递模拟中，任何仅依赖辐射强度（radiation intensity）和辐射通量（radiation flux）而不显式依赖辐射压力（radiation pressure）的局部二阶闭合关系（second-order closure）均存在物理不稳定性，从而限制了二阶闭合关系的选择范围。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙再电离（cosmic reionization）模拟中辐射传递的矩方法（moment methods）闭合关系（closure relation）的物理稳定性问题。

Data: 研究中使用的数据来源于宇宙再电离（cosmic reionization）模拟，采用矩方法（moment methods）进行辐射传输建模，并对比了M1和OTVET两种一阶闭合关系及探索了二阶闭合关系的物理稳定性。

Method: 采用二阶闭合关系（second-order closure）分析宇宙再电离（cosmic reionization）辐射传输的物理稳定性。

Result: 高阶局部闭包关系（local second-order closure）仅依赖辐射强度（radiation intensity）和辐射通量（radiation flux）而不显式依赖辐射压力（radiation pressure）时必然存在物理不稳定性。
...
**TL;DR**: 结合宇宙微波背景(CMB)、重子声学振荡(BAO)和Ia型超新星(SN)数据发现暗能量状态方程$w$存在幻影穿越(phantom crossing)倾向，这是由于$Λ$CDM框架下各数据集单独拟合时$Ω_m$值存在$Ω_m^\mathrm{BAO} < Ω_m^\mathrm{CMB} < Ω_m^\mathrm{SN}$的张力序列，而幻影穿越模型能协调这些$Ω_m$值差异，且BAO和CMB测量（而非SN数据）是联合分析中偏好幻影穿越而非精质(quintessence)理论的主要原因。


<details>
  <summary>Details</summary>

Background: 研究探讨了暗能量状态方程（phantom crossing）的观测偏好与ΛCDM模型中Ω_m参数张力（Ω_m tensions）之间的关联机制。

Data: 数据来源包括宇宙微波背景辐射（cosmic microwave background, CMB）、重子声波振荡（baryon acoustic oscillations, BAO）和Ia型超新星（Type Ia supernovae, SN）的观测数据。

Method: 采用理论分析和实证检验相结合的方法，研究了暗能量状态方程(w)的幻影穿越(phantom crossing)现象对Ωm（物质密度参数）观测值的影响。

Result: 暗能量状态方程（dark energy equation of state）$w$的幻影穿越（phantom crossing）偏好源于$Ω_m^\mathrm{BAO} < Ω_m^\mathrm{CMB} < Ω_m^\mathrm{SN}$的排序导致的$Ω_m$张力（$Ω_m$ tensions）。
...
**TL;DR**: 该论文通过场级克拉美-罗界(Cramer-Rao bound)分析揭示了原初非高斯性(primordial non-Gaussianity)信息在星系分布图中的编码机制，发现多示踪剂尺度依赖偏置(multi-tracer scale-dependent bias)对局域型非高斯性的探测灵敏度优于传统高阶统计，但未能达到包含暗物质晕尺度全部模式时的最优约束，并预测了未来巡天对局域型和等边型(equilateral)非高斯性的探测潜力。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于原初非高斯性（primordial non-Gaussianity）作为暴胀时期的关键探针，探讨其通过绝热模（adiabatic modes）统计关联编码的粒子谱（包括质量和自旋）信息如何在星系分布图中有效提取。

Data: 研究中使用的数据来源为现实宇宙星系分布图（realistic maps of the Universe）和暗物质晕尺度（scale of the dark matter halos）的模态信息。

Method: 采用场级克拉美-罗界（field-level Cramer-Rao bound）分析技术，结合多示踪尺度依赖偏差（multi-tracer scale-dependent bias）方法，探究了原初非高斯性（primordial non-Gaussianity）在星系分布图中的最优信息提取极限。

Result: 非线性结构形成过程中，多示踪剂（multi-tracer）尺度依赖偏置对局域型原初非高斯性（local non-Gaussianity）的探测灵敏度优于传统高阶统计量分析。
...
**TL;DR**: 通过分析普朗克(Planck)卫星SMICA温度图的局部形态和拓扑特性，发现宇宙微波背景(CMB)的半球不对称性不仅存在于温度场方差，还延伸至梯度方差和拟合优度，且三个偶极子指向天球同一区域，为更全面理解该异常的物理本质提供了新证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙微波背景辐射（CMB）的半球功率不对称性（Hemispherical Asymmetry）对宇宙学原理（Cosmological Principle）统计各向同性假设的挑战，旨在通过形态和拓扑性质检验该异常是否超出温度场方差范畴。

Data: 数据来源于普朗克卫星（Planck）SMICA温度图（SMICA temperature map）的局部区域和基于ΛCDM模型的真实模拟（realistic simulations）。

Method: 采用闵可夫斯基泛函（Minkowski Functionals）分析普朗克（Planck）SMICA温度图的局部区域形态和拓扑特性，并与高斯各向同性场理论预期进行拟合比较。

Result: 宇宙微波背景(CMB)的半球不对称性不仅体现在温度场方差上，还存在于局部形态学特征中，并可能暗示非高斯性。
...
**TL;DR**: 通过整合普朗克(Planck)和WMAP卫星的大尺度偏振数据构建了多频谐波空间似然函数(elica)，精确测定了宇宙再电离光学深度(Thomson scattering optical depth)τ=0.0581^{+0.0048}_{-0.0059}，并结合其他观测数据将中微子总质量上限限定为∑m_ν<0.069 eV，排除了高τ值缓解DESI-BAO与CMB观测矛盾的可能性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用宇宙微波背景（CMB）大尺度E模偏振数据精确测定再电离汤姆逊散射光学深度（Thomson scattering optical depth to reionisation, τ），以检验ΛCDM模型参数并探讨其与DESI-BAO观测张力的潜在关联。

Data: 数据来源包括普朗克卫星(Planck)LFI 70 GHz通道、采用SRoll2制图算法处理的普朗克HFI 100和143 GHz通道，以及WMAP Ka、Q和V波段数据。

Method: 采用扩展的Hamimeche-Lewis形式（Hamimeche-Lewis formalism）构建多频段、谐波空间似然函数（harmonic-space likelihood），结合多场部分天区观测数据。

Result: 结合所有可用的大尺度卫星偏振数据，通过elica似然分析得到汤姆逊散射光学深度（Thomson scattering optical depth）τ=0.0581_{-0.0059}^{+0.0048}，并确定中微子总质量上限∑m_ν<0.069 eV（95%置信水平）。
...
**TL;DR**: 开发了全Python模块化代码CLOE（Cosmology Likelihood for Observables in Euclid），整合了弱引力透镜（weak gravitational lensing）、星系聚类（galaxy clustering）等欧几里得（Euclid）任务核心观测量的理论预测与似然计算，支持光度和光谱数据统一分析框架。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发统一框架CLOE（Cosmology Likelihood for Observables in Euclid），用于计算宇宙学可观测量的理论预测并基于前沿星系巡天数据（如欧几里得任务）进行似然评估。

Data: 数据来源包括欧几里得(Euclid)等星系巡天项目的最先进观测数据，涵盖弱引力透镜(weak gravitational lensing)、光度星系成团性(photometric galaxy clustering)、星系-星系透镜效应(galaxy-galaxy lensing)、光谱星系成团性(spectroscopic galaxy clustering)等核心观测量，以及星系团(clusters of galaxies)和星系位置/形状与宇宙微波背景(cosmic microwave background)的交叉关联等扩展探针。

Method: 采用模块化Python代码CLOE（Cosmology Likelihood for Observables in Euclid）计算宇宙学可观测量的理论预测值，并通过统一似然评估与前沿星系巡天数据的匹配性。

Result: CLOE是一个模块化Python代码，用于计算宇宙学可观测量（cosmological observables）的理论预测，并通过统一似然框架评估其与前沿星系巡天数据（如Euclid）的拟合度。
...
**TL;DR**: 该研究通过数值模拟校准了暗物质晕（dark matter halos）在尺度r≥5h⁻¹Mpc上的速度分布P(vr,vtan|r,M)，并基于视线速度分布P(vLOS|R,M)预测DESI光谱数据能以亚百分之一精度约束星系团质量，精度媲美第四阶段弱引力透镜巡天。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用星系团外围（infall regions）的动力学特征约束团块质量，以规避重子物理过程对质量校准的影响。

Data: 数据来源包括数值模拟（numerical simulations）和光谱巡天数据（spectroscopic survey data），后者来自暗能量光谱仪（DESI）。

Method: 采用参数化速度分布模型（$P(v_{\rm r},v_{\rm tan}|r,M)$）并基于数值模拟校准径向和质量依赖性，通过视线投影获得线速度分布（$P(v_{\rm LOS}|R,M)$）的精确预测。

Result: 星系外围（infall regions）的速度分布模型结合暗能量光谱仪（DESI）数据可实现亚百分比精度的星系团质量约束。
...
**TL;DR**: 提出一种基于埃尔米特-高斯卷积滤波器和双曲正切变换的两步框架，通过稳定提取多尺度任意阶导数场增强宇宙网结构可见性，在Quijote模拟中将七个宇宙学参数约束精度提升1.2-5.3倍。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过提取宇宙大尺度结构（large-scale structure）的多尺度信息来增强宇宙学参数（cosmological parameters）约束，特别关注高阶导数场（higher-order derivative fields）对宇宙网（cosmic web）几何拓扑信息的捕获能力。

Data: 数据来源于Quijote数值模拟（Quijote simulations）。

Method: 采用Hermite-Gaussian卷积滤波器获取稳定的多尺度任意阶导数场，并通过tanh非线性变换压缩极端密度对比度以增强宇宙网状结构（cosmic web）的可见性。

Result: 提出了一种基于埃尔米特-高斯卷积滤波器和双曲正切非线性变换的两步框架，能稳定提取宇宙网的多尺度任意阶导数场，显著提升宇宙学参数约束精度。
...
**TL;DR**: 该研究构建了利用各向同性两点相关函数推断未精确定位快速射电暴（FRB）线性大尺度偏置（bias）的端到端框架，证实了即使存在严重定位误差，通过正确建模弥散效应仍可从FRB中提取有效的大尺度结构（LSS）聚类信息。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用快速射电暴(FRB)作为宇宙大尺度结构(LSS)的新型示踪剂，克服其定位和红移测量不精确的挑战。

Data: 数据来源于合成快速射电暴（FRB）样本，其红移分别为z=0.3、0.5和0.7，并注入了偏差值b=1.2、1.5和2.0。

Method: 采用各向同性两点相关函数（isotropic two-point correlation function）结合Landy-Szalay估计量、噪声匹配随机目录、蒙特卡洛前向模型（Monte Carlo forward model）处理定位模糊效应，并基于对数正态模拟样本（lognormal mock samples）的协方差矩阵进行似然推断。

Result: 通过各向同性两点相关函数和正向建模方法，成功从未精确定位的快速射电暴(FRB)样本中提取出线性大尺度偏置(bias)信息，证实位置不确定性主导了聚类信号抑制但可通过正确建模进行校正。
...
**TL;DR**: 研究发现原初黑洞（PBHs）的质量与原始曲率扰动峰值尺度存在近似一一对应关系，通过最新丰度限制推导出窄峰和宽峰原始曲率功率谱振幅上限，并系统比较了Press-Schechter（PS）形式与峰值理论的差异，表明非球对称塌缩会显著增加所需原始功率谱振幅。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过原初黑洞(PBHs)的形成机制间接探测宇宙暴胀末期小尺度原初曲率扰动(primordial curvature perturbations)的功率谱振幅。

Data: data来源包括基于原初黑洞（PBHs）丰度的最新观测限制数据，以及通过Press-Schechter（PS）形式主义和峰值理论（peak theory）对原初曲率功率谱（primordial curvature power spectrum）振幅的系统分析。

Method: 采用普雷斯-谢希特（Press-Schechter, PS）形式体系和峰值理论（peak theory）系统比较了原初曲率扰动功率谱振幅的约束，并量化了球对称与非球对称坍缩准则的影响。

Result: 原初黑洞（PBHs）的质量与原始曲率扰动峰值尺度之间存在近似一一对应关系，从而将PBHs丰度限制转化为原始曲率功率谱振幅的上限。
...
**TL;DR**: 基于盖亚(Gaia)DR3类星体自行数据，采用矢量球谐函数(VSH)和Hellings-Downs曲线(HDC)两种方法分析低频引力波(GW)信号，发现VSH统计稳健且计算高效，而HDC灵敏度更高但计算复杂，当前应变探测下限为10^{-11}，未来盖亚DR4有望提升至3×10^{-12}。


<details>
  <summary>Details</summary>

Background: 研究探讨利用盖亚任务(Gaia)观测的遥远类星体自行运动中的低频引力波(GW)印记来探测引力波的潜力。

Data: 数据来源于盖亚任务(Gaia mission)第三次数据发布(Gaia DR3)的天体测量数据(astrometric data)，并基于类星体的实际天空位置和测量误差进行模拟。

Method: 采用矢量球谐函数（VSH）和角相关函数（即Hellings-Downs曲线，HDC）两种数据分析技术从类星体自行运动中提取和表征引力波信号。

Result: 基于盖亚(Gaia)DR3数据，通过矢量球谐函数(VSH)和赫林斯-当斯曲线(HDC)方法分析类星体自行运动，得出可探测引力波(GW)应变下限为10^{-11}，未来盖亚DR4数据可能提升至3×10^{-12}。
...
**TL;DR**: 通过分析低红移(0.07<z<0.2)星系群和星系团的X射线光度($L_X$)与温度($T$)关系，发现其斜率($B=3.2 \pm 0.1$)比自相似模型预测更陡，表明在质量较低的星系群中气体因反馈机制更易从较浅引力势阱中排出导致光度随质量下降更快。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系群和星系团内热气体（hot intracluster and intragroup medium）的非引力过程（non-gravitational processes）对X射线光度（$L_X$）与温度（$T$）标度关系的调控作用。

Data: 数据来源于X射线选星系团大样本目录（X-CLASS），结合了Legacy Surveys的深度测光数据和针对52个低红移系统的定制光谱后续观测。

Method: 采用贝叶斯框架推断X射线光度（$L_X$）与温度（$T$）的关系。

Result: result：低质量星系群和星系团的$L_X-T$关系斜率（$B=3.2 \pm 0.1$）比自相似模型预测的更陡，表明在星系群尺度上光度的质量依赖性比大质量星系团更强。
...
**TL;DR**: 提出了一种结合帕德(Padé)有理函数参数化光度距离和两体衰变暗物质(DDM)现象学的半宇宙图像框架，通过DESI DR2 BAO数据和SKA1-Mid-like仪器在z≈1.75处的21厘米强度映射功率谱，约束了帕德参数和DDM参数，并重建了大质量衰变产物和暗能量的状态方程。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过半宇宙学重构（semi-cosmographic）框架探索宇宙演化，特别关注无模型假设下利用帕德近似（Padé）参数化光度距离并结合唯象驱动的二体衰变暗物质（DDM）组分来约束暗能量状态方程。

Data: 数据来源包括DESI DR2重子声波振荡（BAO）数据和模拟的21厘米强度映射功率谱（SKA1-Mid-like仪器在红移z≈1.75处）。

Method: 采用半宇宙学框架结合帕德有理分数参数化光度距离和现象学启发的二体衰变暗物质(DDM)模型，通过马尔可夫链蒙特卡洛(MCMC)分析约束帕德参数和DDM参数。

Result: 开发了一种半宇宙学框架，结合帕德(Padé)有理分式参数化光度距离和现象学启发的二体衰变暗物质(DDM)模型，无需假设暗能量模型即可重建暗能量状态方程。
...
**TL;DR**: 利用模拟的强透镜引力波（SLGW）信号结合太极和LISA联合观测，将宇宙距离对偶关系（CDDR）的偏离参数η₀约束精度提升至10⁻⁴量级，未发现显著偏离证据。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于现代宇宙学中的基本关系——宇宙距离对偶关系（CDDR），旨在通过强引力透镜引力波（SLGW）信号验证其有效性。

Data: 基于Population III恒星形成模型生成了90天的模拟强透镜引力波(SLGW)数据，包含10个事件，源红移范围$z_s \in [2,6]$、透镜红移范围$z_L \in [0.2,1]$，并整合了Taiji和LISA的联合仿真观测数据。

Method: 采用点质量透镜模型(point-mass lens model)对强透镜引力波(SLGW)信号进行贝叶斯统计框架下的参数估计，结合Taiji和LISA的联合观测数据。

Result: 基于模拟的强透镜引力波（SLGW）信号，联合太极（Taiji）和LISA观测可将宇宙距离对偶关系（CDDR）偏离参数η₀的测量精度提高约一倍，10次事件联合约束下η₀的95%置信区间半宽在η₁(z)参数化下达到2.61×10⁻⁴（仅太极）和1.72×10⁻⁴（太极+LISA），且结果与η₀=0一致，未发现显著偏离CDDR的证据。
...
**TL;DR**: 研究发现原初黑洞（PBHs）成团分布时，邻近黑洞和团簇引力势会显著增加微引力透镜光变曲线的复杂性，导致现有观测实验（如MACHO等）漏检高达93%的PBH暗物质，但孤立透镜行为仍使微引力透镜约束对PBH暗物质占比保持部分有效性。


<details>
  <summary>Details</summary>

Background: 该研究探讨了原始黑洞（PBHs）以团簇形式存在时对微引力透镜观测的影响及其对暗物质（dark matter）探测约束的修正。

Data: 数据来源于MACHO、EROS、OGLE、POINT-AGAPE和HSC等观测实验的微引力透镜（microlensing）数据。

Method: 采用微引力透镜(gravitational microlensing)技术分析原始黑洞(PBHs)团簇的光变曲线(light curve)，考虑了邻近PBHs和团簇整体引力势的联合影响。

Result: 在考虑原始黑洞(PBHs)成团分布的情况下，现有微引力透镜观测数据中仍有显著比例的PBH暗物质（研究中最高达93%）未被探测到，但成团性不会完全消除对PBH暗物质占比的观测限制。
...
**TL;DR**: 提出局部泵浦暗能量（LPDE）机制，发现非线性暗物质结构形成会通过重标量场χ的局部位移产生真空能，其体积平均效应在红移z∼1时主导宇宙加速膨胀，自然关联了结构形成与加速膨胀现象。


<details>
  <summary>Details</summary>

Background: 该研究探讨了宇宙加速膨胀与暗物质（dark matter）非线性结构形成之间的关联，提出局部泵浦暗能量（LPDE）机制作为解释这一现象的新理论框架。

Data: 数据来源于基准ΛCDM宇宙学中的晕质量函数（halo mass function）和暗能量光谱仪（Dark Energy Spectroscopic Instrument, DESI）的最新宇宙学观测数据。

Method: 采用有效场论（effective-field-theory）框架，通过粗粒化（coarse-graining）密度对比剖面并诱导重标量场$χ$的局域平衡点偏移，构建了局域泵浦暗能量（LPDE）机制。

Result: LPDE机制通过暗物质晕形成时密度扰动短波模增长诱导重标量场χ局域平衡点偏移，在平均场框架下产生由晕体积填充因子控制的真空能，最终表现为大尺度均匀暗能量（dark energy）组分。
...
### [34] [LISA science ground segment conventions](https://arxiv.org/abs/2603.22377)
...
**TL;DR**: 该文献制定了LISA（Laser Interferometer Space Antenna）任务中数据模拟、波形生成和分析流程的标准规范，并提供了相关出版物的最佳实践指南。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于制定激光干涉空间天线（LISA）任务中数据模拟、波形生成与分析流程（analysis pipelines）的标准化规范。

Data: 数据来源于激光干涉仪空间天线（LISA）分布式数据处理中心（DDPC）进行的模拟（simulation）和波形生成。

Method: 采用的核心分析方法包括时频变换（time-to-frequency transformations）、引力波源参数化（gravitational-wave source parametrization）、仪器响应建模（instrumental response to gravitational waves）、时延干涉测量（time-delay interferometry）以及参考系定义（reference frame definitions）。

Result: 该文档制定了激光干涉空间天线（LISA）分布式数据处理中心（DDPC）内数据模拟、波形和分析流程的规范，并可作为LISA任务相关出版物的最佳实践指南。
...
### [35] [Model validation and tolerancing of scalar vortex masks in the High Contrast Imaging Testbed (HCIT) facility](https://arxiv.org/abs/2603.22537)
*Niyati Desai,Garreth Ruane,Daniel Shanks,Lorenzo König,Susan Redmond,Bertrand Mennesson*

**TL;DR**: 该研究验证了标量涡旋日冕仪(scalar vortex coronagraph)的消色差性能，通过实验测量和模型优化证实其在HWO任务中实现10^-10对比度以直接成像系外行星的潜力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发用于哈勃世界天文台（HWO）任务的日冕仪技术，特别是针对直接成像系外行星所需的10^-10对比度水平的消色差日冕仪掩模（coronagraphic mask）性能优化。

Data: 研究中使用的数据来源包括实验室演示中通过相位计量学评估的标量涡旋（scalar vortex）掩模缺陷和制造偏差数据，以及来自喷气推进实验室（JPL）高对比度成像测试平台（HCIT）的实验结果。

Method: 采用标量涡旋日冕仪（scalar vortex coronagraph）设计，通过相位计量学评估制造缺陷并优化电场共轭（EFC）算法模型，结合实验室演示和JPL高对比度成像测试台（HCIT）的实验验证。

Result: 标量涡旋日冕仪（scalar vortex coronagraph）设计通过单光路实现双偏振观测，简化了仪器设计并提高了透射率，同时与实验数据吻合良好，为未来HWO任务中的日冕仪性能预测提供了基准。
...
### [36] [Dimpled scalar vortex coronagraph laboratory demonstration](https://arxiv.org/abs/2603.22551)
*Niyati Desai,Garreth Ruane,Susan Redmond,Dimitri Mawet,Eugene Serabyn,Bertrand Mennesson*

**TL;DR**: 通过引入径向相位凹槽（radial phase dimples）的第二代标量涡旋日冕仪（SVC）原型在窄带（2%）和宽带（10%）光谱范围内实现了接近测试台极限的对比度，验证了地形消色差标量涡旋掩模作为未来空间高对比度成像任务的候选技术潜力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发宽带高对比度日冕仪（coronagraph）技术以实现未来空间望远镜（如宜居世界天文台HWO）对系外行星的直接成像需求。

Data: 数据来源于在In-Air Coronagraph Testbed（空中日冕仪测试台）上进行的高对比度成像实验。

Method: 采用电共轭场（electric field conjugation）技术结合径向相位凹陷（radial phase dimples）改进的第二代标量涡旋日冕仪（scalar vortex coronagraphs）原型进行高对比度成像实验。

Result: 第二代标量涡旋日冕仪（SVC）通过径向相位凹坑设计在窄带（2%）和宽带（10%）光谱范围内实现了接近测试台极限的对比度，验证了其在18%带宽下与实测对比度误差在两倍以内的理论预测，为未来偏振无关日冕仪满足HWO性能要求提供了可行方案。
...
### [37] [The Radio-wave Observations at the Lunar Surface of the photoElectron Sheath (ROLSES) instrument onboard the Intuitive Machines-1 Mission to the Moon](https://arxiv.org/abs/2603.22600)
...
**TL;DR**: ROLSES（Radio wave Observations at the Lunar Surface of the photoElectron Sheath）仪器通过部署在月球近地面的四根2.5米单极天线，首次在2 kHz至30 MHz频段内观测到受地球电离层截止频率限制而无法在地面探测的太阳、银河系、木星、地球极光区及地面无线电发射源的信号，并利用快速傅里叶变换（FFT）进行星上频谱分析。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用月球表面射电望远镜系统（ROLSES）探测近月面2 kHz至30 MHz频段的射电和等离子体波环境，填补地球电离层截止频率（15 MHz以下）导致的观测空白。

Data: 数据来源于搭载在Intuitive Machines（IM 1）着陆器Odysseus上的四根2.5米无线电单极天线（ROLSES传感器），通过隔离前置放大器和信号调理模拟电子设备处理后以14位分辨率、120兆样本每秒的速率数字化，并由现场可编程门阵列（FPGA）进行快速傅里叶变换（FFT）频谱分析，最终存储并下传时间平均频谱值和未处理的原始波形数据。

Method: 采用四根2.5米单极天线(monopole antennas)接收2 kHz至30 MHz频段信号，通过隔离前置放大器(isolating pre-amp)和模拟信号调理电路处理后，以14位精度、120兆次采样每秒进行数字化，并利用现场可编程门阵列(FPGA)执行快速傅里叶变换(FFT)实现星上频谱分析。

Result: ROLSES（Radio wave Observations at the Lunar Surface of the photoElectron Sheath）仪器成功观测到月球近地面2 kHz至30 MHz频段的无线电波和等离子体波环境，填补了地球电离层截止频率（15 MHz以下）无法探测的空白。
...
### [38] [A Slit Mask Integral Field Unit for the Robert Stobie Spectrograph on the Southern African Large Telescope: I. Instrument Development](https://arxiv.org/abs/2603.22698)
*Sabyasachi Chattopadhyay,Matthew A. Bershady,Michael P. Smith,Antoine Mahoro,Marsha J. Wolf*

**TL;DR**: 为南非大望远镜（Southern African Large Telescope）的Robert Stobie光谱仪（RSS）开发了紧凑型积分视场单元（IFU）SMI-200，在22.5$\arcsec$ x 17.6$\arcsec$视场（FOV）内提供0.8$\arcsec$空间分辨率，实测总吞吐率为77%，但因焦比退化（FRD）导致有效吞吐率降至55-60%。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于为南非大望远镜（Southern African Large Telescope）的多模式主力仪器罗伯特·斯蒂比光谱仪（Robert Stobie Spectrograph）开发一种新型紧凑型积分视场单元（integral field unit），以在严格受限的机械空间内实现积分视场光谱（integral field spectroscopy）观测功能。

Data: 使用南非大望远镜（Southern African Large Telescope）上的罗伯特·斯蒂比光谱仪（Robert Stobie Spectrograph，RSS）进行积分视场光谱（integral field spectroscopy，IFS）观测，通过光纤阵列和棱镜折叠镜构建的紧凑型积分视场单元（integral field units，IFUs）采集数据，其中首个完成的SMI-200单元包含303根目标光纤和24根天空光纤，覆盖22.5$\arcsec$ x 17.6$\arcsec$的视场（field of view，FOV）。

Method: 采用光纤光学和棱镜折叠镜技术构建紧凑型积分视场单元(IFUs)，将其集成在长狭缝盒式结构(134 mm x 130 mm x 8 mm)内，并通过二维天空朝向光纤阵列将望远镜光束引导至8角分一维伪狭缝。

Result: 成功研制了可集成在狭缝盒式结构中的紧凑型积分视场单元（IFU）SMI-200，其空间分辨率为0.8角秒，视场覆盖22.5角秒×17.6角秒，实测总透射率达77%，但受限于紧凑空间导致的焦比退化（FRD），在f/4.2准直光束下的有效透射率降至55-60%。
...
### [39] [A High-frequency Geodetic VLBI Experiment for Optical Clock Comparison](https://arxiv.org/abs/2603.23078)
...
**TL;DR**: 通过K波段甚长基线干涉测量(VLBI)对比意大利和韩国氢脉泽(H-maser)时钟，发现与卫星链路和光学时钟结果在10^-15 s/s精度一致，验证了该技术可作为洲际时钟比对新方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用大地测量K波段甚长基线干涉测量(VLBI)技术实现意大利与韩国间跨大陆氢脉泽钟(H-masers)频率比对，探索替代卫星链路的精密时频传递新方法。

Data: 研究使用了意大利Medicina和韩国Sejong射电望远镜的氢脉泽（H-masers）数据，通过K波段大地测量甚长基线干涉测量（VLBI）观测获取，并同步结合卫星链路以及韩国KRISS和意大利INRIM国家计量院维护的高精度光学钟数据，通过光纤传输至VLBI天线。

Method: 采用大地测量K波段甚长基线干涉测量（VLBI）技术进行时钟比对，结合卫星链路和本地光学钟（optical clocks）数据，通过两种大地测量VLBI软件外推氢脉泽（H-masers）频率差异。

Result: 通过K波段大地测量甚长基线干涉测量(VLBI)观测实现了意大利与韩国间氢脉泽(H-maser)频率差异的洲际比对，不同方法获得的结果在$10^{-15}$ s/s量级一致。
...
### [40] [Data-calibrated point spread function prediction: General description of the method and demonstration on MUSE-NFM](https://arxiv.org/abs/2603.23339)
*Arseniy Kuznetsov,Benoit Neichel,Sylvain Oberti,Thierry Fusco*

**TL;DR**: 提出了一种不依赖完整自适应光学（AO）遥测数据、基于物理参数和轻量神经网络的快速精确点扩散函数（PSF）建模框架，在MUSE-NFM数据中实现了斯特列尔比（Strehl ratio）误差13.5%和半高全宽（FWHM）误差10.9%的精度，并能预测拥挤场中波长依赖的离轴PSF（斯特列尔比误差<5%，FWHM误差4.6%），适用于多种AO仪器。


<details>
  <summary>Details</summary>

Background: 天文观测中精确的点扩散函数（PSF）建模是光度测量、天体测量、源分离和解卷积等数据分析的基础，但自适应光学（AO）系统的PSF会随波长、视场位置和观测条件高度变化，传统PSF重建（PSF-R）依赖完整AO遥测数据且流程复杂，限制了其广泛应用。

Data: 数据来源为欧洲南方天文台（ESO）档案中可检索的一组紧凑且具有物理意义的参数集。

Method: 采用基于傅里叶的点扩散函数（PSF）模型（astro-TIPTOP启发）结合轻量级神经网络校正输入参数，实现自适应光学（AO）观测中空间与光谱可变PSF的高精度建模。

Result: 提出了一种不依赖完整自适应光学（AO）遥测的紧凑型物理信息数据校准模型，能够准确预测多色且随视场变化的点扩散函数（PSF），在MUSE-NFM标准星测试中斯特列尔比（Strehl ratio）和半高全宽（FWHM）中位误差分别为13.5%和10.9%，在ω Centauri拥挤场观测中斯特列尔比误差<5%、半高全宽误差4.6%。
...
### [41] [SGR 1935+2154's Quiet Local Environment: Clues for Its Progenitor](https://arxiv.org/abs/2603.22720)
*Wenlang He,Ping Zhou,Bingqiu Chen*

**TL;DR**: 该研究基于盖亚(Gaia)DR3天体测量和多波段测光数据，发现磁星(magnetar)SGR 1935+2154周围大质量恒星表面密度上限仅为太阳邻域的四分之一，表明其可能由非极大质量恒星(8<M_*<25 M⊙)或双星并合产物的核心坍缩(CC)形成，而非极大质量恒星(≥25 M⊙)的坍缩。


<details>
  <summary>Details</summary>

Background: 磁星（magnetar）的起源是当前天体物理学中的开放性问题，涉及大质量恒星（$\ge 25~M_\odot$）或非极大质量恒星（$8<M_*<25~M_\odot$）的核心坍缩（core-collapse, CC）、恒星系统并合以及白矮（white dwarf, WD）吸积诱导坍缩（accretion-induced collapse, AIC）等多种前身星演化路径。

Data: 数据来源为盖亚卫星(Gaia)第三次数据释放(DR3)的天体测量数据以及从光学到红外(IR)的多波段测光数据。

Method: 基于盖亚卫星(Gaia DR3)天体测量数据和多波段(光学至红外)测光数据，采用恒星轨迹回溯分析技术研究磁星(SGR 1935+2154)周围恒星环境。

Result: SGR 1935+2154周围大质量恒星表面密度上限仅为太阳邻域的四分之一，暗示其可能由非极大质量恒星（8<M_*<25 M_⊙）或双星并合产物的核心坍缩（CC）形成，而非极大质量恒星（≥25 M_⊙）的CC。
...
### [42] [3D NLTE Sodium abundances in late-type stars. Abundance corrections and synthetic spectra](https://arxiv.org/abs/2603.22975)
*G. Canocchi,E. X. Wang,A. M. Amarsi,K. Lind,M. Racca*

**TL;DR**: 首次采用3D非局部热动平衡(NLTE)辐射传输后处理方法研究了FGK型矮星和巨星中9条Na I谱线的形成，发现相对于一维(1D)局部热动平衡(LTE)模型，3D NLTE丰度修正通常为负值且比1D NLTE修正更显著，尤其在低重力巨星(log g < 2.0)中饱和线的修正可达-0.7 dex，而极低金属丰度([Fe/H]=-4)下D共振线出现约+0.05 dex的正修正。


<details>
  <summary>Details</summary>

Background: 中性钠（neutral sodium）是研究银河系化学演化的重要示踪剂、不同恒星族群的诊断工具，以及通过透射光谱分析系外行星大气成分的关键研究对象。

Data: 数据来源于Stagger网格中的三维辐射流体力学恒星大气模型（3D radiation hydrodynamic stellar atmospheres），覆盖有效温度（Teff）4000至6500 K、表面重力（log g）1.5至5.0、金属丰度（[Fe/H]）-4至+0.5的范围，并通过Balder代码进行三维非局部热动平衡（3D NLTE）辐射转移后处理。

Method: 采用三维非局部热动平衡（3D NLTE）辐射传输后处理方法，结合Stagger网格的三维辐射流体力学恒星大气模型，利用Balder代码对钠线形成进行定量分析。

Result: 3D NLTE（非局部热动平衡）相对于1D LTE（局部热动平衡）的钠丰度修正通常为负值，且比1D NLTE修正更显著，反映了3D模型中更陡的温度梯度导致更高效的过度电离效应。
...
### [43] [Unlocking accretion rate diagnostics for high-mass protostars using JWST/MIRI HI lines](https://arxiv.org/abs/2603.23180)
...
**TL;DR**: 该论文利用JWST/MIRI首次探测并解析了不同演化阶段大质量恒星形成区中原子氢（HI）发射线，评估了现有低质量恒星吸积率（Macc）校准方法对大质量原恒星的适用性，发现部分校准结果显著高估吸积光度（Lacc），为后续研究大质量原恒星吸积过程奠定了方法论基础。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST/MIRI在中红外波段（5至27微米）探索高质量恒星形成区中原子氢（HI）发射线作为吸积示踪物的潜力，以首次估算不同演化阶段高质量原恒星的吸积光度（Lacc）和吸积率（Macc）。

Data: data：研究使用JWST/MIRI的中红外成像仪（MRS/IFU）在5至27微米波段观测高质量恒星形成区样本，检测并解析原子氢（HI）发射线，包括HI 7-6、HI 8-6和HI 6-5等谱线，同时结合去红化的HI线光度作为吸积过程的示踪剂。

Method: 采用中红外仪器（MIRI）的中分辨率光谱/积分场单元（MRS/IFU）技术探测原子氢（HI）发射线，并基于现有线光度-吸积光度（Lacc-calibrations）关系估算吸积参数。

Result: JWST/MIRI首次在高质量恒星形成区探测到氢原子（HI）发射线，但现有吸积光度（Lacc）校准关系对高光度源存在显著高估问题。
...
### [44] [Formation of spirals in early stage protoplanetary discs](https://arxiv.org/abs/2603.23317)
*Marc Van den Bossche,Oliver Gressel*

**TL;DR**: 研究发现，在托尔曼稳定（Toomre-stable）的原行星盘（$Q > 1$）中，气体自引力通过非线性放大机制触发大尺度螺旋结构（spiral structures）的快速生长，其增长率约为10个轨道周期，且低密度盘最终形成弱引力湍流态（$α\approx 10^{-3}$），而高密度盘则导致螺旋臂的失控坍缩。


<details>
  <summary>Details</summary>

Background: 研究聚焦于原行星盘（protoplanetary discs）中非轴对称子结构（如旋臂）的形成机制，尤其关注引力不稳定性（gravitational instability）在托尔曼参数（Toomre parameter）$Q > 1$的稳定盘中触发旋臂生长的作用。

Data: 使用GPU加速代码Idefix对自引力盘进行高分辨率二维剪切薄片模拟，探测了不同初始密度和热力学模型的托尔米稳定盘（Toomre-stable discs）。

Method: 采用高分辨率二维剪切薄片模拟（high-resolution 2D shearing sheet simulations）结合GPU加速代码Idefix，对托尔敏稳定（Toomre-stable）盘进行线性稳定性分析和非线性放大研究。

Result: 在托尔米稳定（Toomre-stable）的吸积盘中，自引力触发的大尺度螺旋结构在非线性阶段被快速放大，其增长率约为10个轨道周期，最终放大程度可达线性理论预测的1000倍。
...
### [45] [What does it take to have $N_{\rm eff} < 3$ at CMB times?](https://arxiv.org/abs/2603.22391)
*Miguel Escudero,Maksym Ovchynnikov,Neal Weiner*

**TL;DR**: 该论文首次系统研究了可能导致有效相对论性中微子种类数（$N_{\rm eff}$）小于3的新物理场景，发现热亲电遗迹粒子（thermal electrophilic relics）和特定衰变特性的非平衡态粒子能自洽解释$N_{\rm eff}=2.81\pm0.12$的观测结果。


<details>
  <summary>Details</summary>

Background: 研究探讨了标准模型扩展中可能导致有效相对论性中微子种类数（$N_{\rm eff}$）小于3的新物理场景，并分析了其与宇宙学、天体物理及实验室数据的兼容性。

Data: 数据来源包括ACT（阿塔卡马宇宙学望远镜）、SPT（南极望远镜）和Planck（普朗克卫星）的联合测量结果，以及新的原初氦丰度测量数据，同时未来Simons Observatory（西蒙斯天文台）的观测数据也将被用于精确测试宇宙辐射含量。

Method: 采用热平衡电亲性遗迹粒子（thermal electrophilic relics）和非平衡态粒子衰变模型分析可能导致有效相对论性中微子种类数（$N_{\rm eff}$）低于标准模型值的物理机制。

Result: 发现存在两种简单场景可解释$N_{\rm eff}=2.81\pm0.12$：一类是质量为8-13 MeV的热电性遗迹粒子（thermal electrophilic relics），另一类是寿命0.05秒至3分钟、质量250-600 MeV、丰度$ρ/ρ_γ\sim 0.1$的非平衡态粒子衰变为$e^+e^-$或$γγ$。
...
### [46] [Kinetic Isocurvature Perturbation](https://arxiv.org/abs/2603.22394)
*Kyu Jung Bae,Dhong Yeon Cheong,Jinn-Ouk Gong,Keisuke Harigaya,Chang Sub Shin*

**TL;DR**: 提出了一类称为"动力学等曲率扰动(kinetic isocurvature perturbations)"的新型原初扰动，发现暗物质质量密度相对于光子数密度保持恒定而其动能存在空间涨落，这种扰动在暗物质冷却后不影响大尺度密度扰动但会导致物质功率谱的局域变化。


<details>
  <summary>Details</summary>

Background: 研究提出了一类新型原初扰动（primordial perturbations）——动力学等曲率扰动（kinetic isocurvature perturbations），其核心科学背景涉及非相对论性重场衰变产生具有空间调制速率的相对论性暗物质粒子（dark matter particles）的宇宙学场景。

Data: 数据来源于理论模拟（simulation）构建的原始扰动模型，通过非相对论性重场衰变为相对论性暗物质粒子的情景生成具有空间调制速率的动力学等曲率扰动（kinetic isocurvature perturbations）。

Method: 采用了一种新的原始扰动类别——动力学等曲率扰动（kinetic isocurvature perturbations）模型，其中暗物质质量密度相对于光子数密度保持恒定而其动能存在空间波动。

Result: 提出了一类新型原初扰动（kinetic isocurvature perturbations），其表现为暗物质质量密度相对于光子数密度保持恒定，但暗物质动能存在空间涨落。
...
### [47] [Slow-down of expanding bubbles in the early Universe](https://arxiv.org/abs/2603.22397)
*Nabeen Bhusal,Simone Blasi,Thomas Konstandin,Enrico Perboni,Jorinde van de Vis*

**TL;DR**: 研究了宇宙学一级相变（PT）中气泡减速效应，发现最快气泡壁因冲击波加热和流体运动显著减速，而假真空加热液滴收缩速度与初始爆燃/混合性质相关，两者均影响引力波（GW）辐射抑制，其中自由度变化和几何特性（如冲击宽度）是关键参数。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙学一级相变（first-order phase transition）中气泡减速效应及其对引力波（gravitational wave）产生的抑制机制，特别关注伴随激波（shockwave）的爆燃（deflagration）和混合（hybrid）模式。

Data: 研究中使用的数据来源包括多气泡模拟（multi-bubble simulations）中观测到的减速效应和引力波（GW）发射抑制现象，以及通过稳态单气泡解（steady-state single-bubble solutions）结合两种不同加热机制分析冲击波对泡壁速度的影响。

Method: 采用稳态单气泡解分析冲击波对泡壁速度的影响，并纳入两种加热机制（膨胀进入高温冲击波区域和假真空热液滴收缩）的动力学效应。

Result: 研究发现在宇宙学一级相变（PT）中，气泡减速效应对于高速壁最为显著，但无法解释模拟中观测到的引力波（GW）抑制现象；而虚假真空加热液滴的缓慢收缩与GW抑制自然相关，且几何特性（如激波宽度）也起重要作用。
...
### [48] [Inside the Black Box of Big Bang Nucleosynthesis: Parameter Sensitivity Studies in Light of new LBT Data](https://arxiv.org/abs/2603.22414)
*Anne-Katherine Burns*

**TL;DR**: 构建了大爆炸核合成(BBN)的灵敏度图谱，量化了氦-4、氘、锂-7原始丰度及有效中微子数(N_eff)对14个基本粒子物理和宇宙学参数与63个热核反应率的依赖关系，为超出标准模型(BSM)研究提供了模型无关的参考。


<details>
  <summary>Details</summary>

Background: 研究主题为通过构建大爆炸核合成（BBN）的灵敏度图谱，量化原初元素丰度（氦-4、氘、锂-7）及有效中微子数（$N_{\rm{eff}}$）对基本粒子物理参数、宇宙学参数及热核反应速率的依赖性，并探讨其在标准模型外（BSM）的适用性。

Data: 数据来源于公开可用的BBN代码PRyMordial（https://github.com/vallima/PRyMordial），结合两种核反应速率汇编和两种弱速率归一化方案进行计算，并参考了LBT（Large Binocular Telescope）对氦-4丰度的最新观测数据（Aver:2026dxv）以及CMB+BAO+BBN 2026联合分析结果（Goldstein:2026iuu）。

Method: 采用公开的BBN代码PRyMordial（GitHub）计算14个基本粒子物理和宇宙学参数及63个热核反应率对原初丰度的敏感性，结合两种核反应率汇编和两种弱速率归一化方案。

Result: 构建了一个独立于模型的大爆炸核合成（BBN）灵敏度图谱，量化了14个基本粒子物理与宇宙学参数及63个热核反应率对氦-4、氘、锂-7原初丰度及有效中微子数（$N_{\rm{eff}}$）的影响。
...
**TL;DR**: 该论文研究了磁悬浮超导体对自旋2型暗物质场（暗引力子）在dHz至kHz频段的响应，发现其对暗引力子与电磁耦合的探测灵敏度优于现有实验，尤其在低频段具有潜力。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用磁悬浮超导体探测自旋为2的暗物质场（dark graviton）与电磁耦合的相互作用，以拓展对轴子类粒子（axion-like particles）和暗光子（dark photons）等超轻暗物质的探测手段。

Data: 数据来源于磁悬浮超导体对自旋-2暗物质场（dark graviton）在dHz至kHz频率范围内的响应观测。

Method: 采用磁悬浮超导体（magnetically levitated superconductor）技术，通过计算暗引力子（dark graviton）对超导体产生的物质耦合（matter coupling）和光耦合（light coupling）作用力，分别分析其引起的应变式潮汐加速度（strain-like tidal acceleration）和超抗磁响应（superdiamagnetic response）的电磁耦合效应。

Result: 磁悬浮超导体可通过超抗磁响应探测暗引力子（dark graviton）与电磁场的耦合，尤其在低频段具有潜在的高灵敏度优势。
...
### [50] [Cosmic Ray Boosted Dark Matter in COSINUS: Modeling and Constraints](https://arxiv.org/abs/2603.22971)
...
**TL;DR**: 这篇论文的核心贡献是通过提出高能宇宙射线或中微子散射增强暗物质（boosted dark matter）动能的框架，建立了自旋0、1/2和1的暗物质与标量或矢量介子（mediator）相互作用的核子散射截面目录，并基于此预测了COSINUS实验对亚GeV能区增强暗物质的探测灵敏度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于通过提升暗物质（boosted dark matter）框架探测亚GeV质量暗物质与核子散射截面的理论建模与实验约束。

Data: 数据来源于高能宇宙射线或中微子与暗物质粒子散射产生的动能增强暗物质粒子（boosted dark matter particles），通过COSINUS实验（假设曝光量为100 kg d）进行探测。

Method: 采用重媒介极限下的暗物质-核子散射截面（dark matter-nucleon scattering cross section）计算方法，针对自旋0、1/2和1的暗物质及标量与矢量媒介子（scalar and vector mediators）的偶奇宇称（even or odd parity）情况建立截面目录。

Result: 通过高能宇宙射线或中微子散射使暗物质粒子获得额外动能（boosted dark matter framework），可在较高能量阈值的实验中探测亚MeV质量暗物质。
...
### [51] [Dark Matter Detection through Rydberg Atom Transducer](https://arxiv.org/abs/2603.23337)
*J. F. Chen,Haokun Fu,Christina Gao,Jing Shu,Geng-Bo Wu,Peiran Yin,Yi-Ming Zhong,Ying Zuo*

**TL;DR**: 提出了一种集成介电光栅望远镜（dielectric haloscope）、里德伯原子（Rydberg-atom）转换器和超导纳米线单光子探测器的低温探测架构，通过相位匹配共振增强将超轻玻色暗物质（meV质量）转换为太赫兹光子并上转换至光学波段，在0.4 meV质量处对轴子-光子耦合（gaγγ）的探测灵敏度可达10⁻¹³ GeV⁻¹，首次实现了太赫兹窗口对轴子和暗光子暗物质的高效探测。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索质量在meV范围内的超轻玻色子暗物质（ultralight bosonic dark matter），特别是针对太赫兹（THz）频率下信号转换与单光子探测的技术挑战。

Data: 数据来源包括介电光栅（dielectric haloscope）转换的THz光子、冷铷（87Rb）原子系综的六波混频（six-wave mixing）信号以及超导纳米线单光子探测器（superconducting nanowire single-photon detection）的低温测量数据。

Method: 采用了一种混合探测架构，结合介电光栅（dielectric haloscope）、里德伯原子（Rydberg-atom）换能器和超导纳米线单光子探测器，在低于1 K的低温平台上实现暗物质到太赫兹（THz）光子的相位匹配共振转换和六波混频（six-wave mixing）上变频。

Result: 提出了一种集成介质光镜(haloscope)、里德伯原子换能器和超导纳米线单光子探测器的混合探测架构，可在≤1K的低温平台下将暗物质转化为太赫兹光子并上转换至光学波段，预计在0.4meV质量处对轴子-光子耦合常数gaγγ的探测灵敏度达10⁻¹³GeV⁻¹。
...
### [52] [Ca2+ transient detection and segmentation with the Astronomically motivated algorithm for Background Estimation And Transient Segmentation (Astro-BEATS)](https://arxiv.org/abs/2603.22311)
*Bolin Fan,Anthony Bilodeau,Frederic Beaupre,Theresa Wiesner,Christian Gagne,Flavie Lavoie-Cardinal,Renee Hlozek*

**TL;DR**: 将天文学瞬变检测技术应用于荧光显微成像，开发出Astro-BEATS算法，显著提升了微型突触钙瞬变（miniature Synaptic Calcium Transients）检测和分割性能。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用天文学瞬变检测技术改进荧光显微成像中微型突触钙瞬变（miniature Synaptic Calcium Transients）的自动检测与分割。

Data: 研究使用了基于荧光显微成像（fluorescence microscopy）的Ca$^{2+}$信号视频数据。

Method: 采用天文瞬变检测技术开发了自动分割算法Astro-BEATS，结合图像估计和源查找(source-finding)方法用于突触钙瞬变检测。

Result: Astro-BEATS算法在荧光显微成像中检测微型突触钙瞬变（miniature Synaptic Calcium Transients）时，其性能优于当前基于阈值的方法。
...
### [53] [Automatic search for transiting planets in TESS-SPOC FFIs with RAVEN: over 100 newly validated planets and over 2000 vetted candidates](https://arxiv.org/abs/2603.22597)
*M. Lafarga,D. J. Armstrong,K. Cui,A. Hadjigeorghiou,V. Kunovac,L. Doyle,E. M. Bryant,R. F. Díaz,L. A. Nieto,A. Osborn*

**TL;DR**: 利用RAVEN管道对TESS前4年观测的220万颗主序星进行均匀搜索和验证，新确认118颗行星（含31颗新发现）并识别2000余颗高概率候选体，显著改善了短周期（0.5-16天）凌星行星样本的完整性和可靠性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用TESS（凌日系外行星巡天卫星）数据开展短周期（≲30天）凌日行星的搜寻与验证，以解决当前样本不完整和存在偏差的问题。

Data: 数据来源为TESS（Transiting Exoplanet Survey Satellite）前四年运行期间（第1至55扇区）获取的全帧图像（Full Frame Images）中观测到的220多万颗主序星，这些恒星的光度数据由盖亚（Gaia）任务精确表征。

Method: 采用RAVEN流水线结合盒最小二乘算法（box least squares algorithm）检测候选体，并利用基于真实模拟训练的机器学习模型进行分类与统计验证。

Result: 使用RAVEN管道在TESS数据中统一搜索并验证了118颗新行星（包括31颗首次发现），同时识别出2000余颗高概率候选体（含约1000颗新候选体），显著改善了TESS短周期（0.5-16天）凌星行星样本的完整性和可靠性。
...
### [54] [Perihelion observations of interstellar comet 3I/ATLAS with the IRAM 30-m telescope](https://arxiv.org/abs/2603.23240)
*N. Biver,D. Bockelée-Morvan,R. Moreno,J. Crovisier,G. Paubert,V. Zakharov,J. Boissier,M. A. Cordiner,N. X. Roth*

**TL;DR**: 通过毫米波光谱观测星际彗星3I/ATLAS（3I/阿特拉斯）发现其彗发中HCN、CH3OH、CO和H2CO丰度高于太阳系彗星上限，硫碳比处于彗星观测下限，异常低的气体膨胀速度表明彗核附近存在由CO2等重分子驱动的气流或大量冰粒升华贡献。


<details>
  <summary>Details</summary>

Background: 研究聚焦于第三颗星际彗星3I/ATLAS（3I/阿特拉斯）在2025年10月29日近日点附近的高排气活动，利用毫米波光谱分析其彗发气体组成以揭示星际物质的化学特征。

Data: 数据来源于2025年11月1日至3日使用IRAM 30米望远镜对3I/ATLAS彗星在日心距1.36-1.37天文单位（au）处进行的毫米波谱观测。

Method: 采用毫米波光谱技术（millimeter spectroscopy）结合IRAM 30米望远镜观测数据，对彗发气体成分（HCN、CH3OH、CO、H2CO等）进行定量分析，并基于探测限评估未检出物种（如H2S）的丰度上限。

Result: 在3I/ATLAS彗星（3I/ATLAS）的彗发中检测到了HCN、CH3OH、CO和H2CO的明显谱线，并获得了CS和CH3CN的约4σ探测信号，但未探测到H2S。
...
### [55] [Shaping the diffuse X-ray sky: Structure, Variability and Visibility](https://arxiv.org/abs/2603.22392)
*Philipp Girichidis,Erika Rea,Ralf S. Klessen,Michael C. H. Yeung,Efrem Maconi,Manami Sasaki,Michael Freyberg,Juan D. Soler*

**TL;DR**: 通过磁流体动力学(MHD)模拟分析本地泡(LB)类似结构，发现其X射线辐射特性主要受超新星(SN)活动阶段、视线柱密度(NH)吸收效应和百万年尺度时间变异性的三重调控。


<details>
  <summary>Details</summary>

Background: 研究聚焦于太阳系所在局部泡（Local Bubble）的X射线辐射特性及其形成演化机制，旨在揭示热、低密度星际空腔中X射线辐射与气体密度、温度和视线柱密度（line-of-sight column density）的物理关联。

Data: data部分：研究使用磁流体动力学模拟（magnetohydrodynamical simulation）生成的局部泡（Local Bubble）类比数据，通过后处理分析X射线发射的空间分布、变异性及观测约束条件。

Method: 采用磁流体力学(MHD)模拟和后期处理技术分析局部泡(LB)类似物的X射线特性，重点考察空间分布、时间变率及观测限制。

Result: 超新星(SN)爆发后短时间内X射线辐射主要集中于气泡体积的极小部分(约1%)，其中95%的亮度来自最近超新星周围的高温区域，而在无新近超新星的静止期，X射线亮区分布更广。
...
### [56] [Where within the 3C 84 jet are $γ$-rays produced?](https://arxiv.org/abs/2603.22403)
...
**TL;DR**: 通过分析3C 84星系喷流在伽马射线耀发期间的厘米和毫米波VLBI（甚长基线干涉测量）偏振信号，发现距中央引擎秒差距（parsec）尺度区域的线偏振度增强，结合X射线偏振数据支持该区域通过同步自康普顿（synchrotron self-Compton）过程产生伽马射线。


<details>
  <summary>Details</summary>

Background: 研究聚焦于河外喷流（extra-galactic jets）中伽马射线（γ-ray）产生和发射位置的确定问题，以邻近明亮射电星系3C 84为研究对象探讨其喷流内高能辐射机制。

Data: 数据来源为与γ射线耀发峰值时间相近的厘米波和毫米波甚长基线干涉测量（VLBI）观测的线性偏振信号。

Method: 采用厘米波和毫米波甚长基线干涉测量（VLBI）技术分析线性偏振信号，并结合同步自康普顿（synchrotron self-Compton）物理模型推断γ射线产生区域。

Result: 在3C 84射电星系中，γ射线耀斑期间观测到的近中心引擎（central engine）秒差距尺度区域的线偏振（linear polarisation）增强支持γ射线产生于该区域，其过程符合同步自康普顿（synchrotron self-Compton）机制。
...
### [57] [MAGIC observations of NGC 4278. The first low-luminosity radio galaxy with compact jets detected at TeV energies](https://arxiv.org/abs/2603.22955)
...
**TL;DR**: 首次利用MAGIC望远镜对低光度射电星系（low-luminosity radio galaxy）NGC 4278在耀发和准静态期间的高能特性进行研究，未探测到显著γ射线辐射但获得与LHAASO一致的上限，发现喷流（jet）以粒子主导且需高效加速机制才能达到TeV能量，喷流功率（jet power）约10^42 erg/s且动能主导，支持高能耀发可能复发的观点但喷流强度不足以突破宿主星系束缚。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用MAGIC望远镜探究低光度射电星系（low-luminosity radio galaxy）NGC 4278在TeV能段的高能特性及其耀发（flaring）与准静态（quasi-quiescent）状态间的物理机制。

Data: 数据来源于2010年至2024年间使用MAGIC望远镜对1ES 1215+303和1ES 1218+304两个耀变体（blazar）的观测数据重新分析。

Method: 采用佛罗里安·戈贝尔大气切伦科夫成像望远镜(MAGIC)对NGC 4278在耀发态和准静态的高能特性进行观测，并重新分析了2010至2024年间对邻近耀变体(1ES 1215+303和1ES 1218+304)的观测数据，同时建模了TeV能段耀发态及准静态的宽波段能谱分布(SED)。

Result: 利用MAGIC望远镜观测未在NGC 4278中检测到显著的γ射线辐射，其上限通量（$F_{{\rm UL, }\, >150\, \mathrm{GeV}}=1.5 \times 10^{-12}\, \mathrm{ph \, s^{-1}\, cm^{-2}}$）与LHAASO结果一致。
