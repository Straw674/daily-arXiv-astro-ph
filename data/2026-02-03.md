...
**TL;DR**: 研究发现盖亚-恩克拉多斯/香肠（GES）星系被银河系吸积时存在两个动力学和化学性质显著不同的恒星群体，分别对应其前身星系的内外区域，支持GES经历多次穿越银河系盘的多阶段吸积事件。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系（Milky Way）与盖亚-恩克拉多斯/香肠（Gaia-Enceladus/Sausage）并合事件对银河系晕（halo）质量、形成历史及形态演化的显著影响。

Data: 数据来源为盖亚卫星(Gaia satellite)与大型光谱巡天(large spectroscopic surveys)的观测数据。

Method: 采用统计检验评估GES中两个星群分离的显著性，并运用银河化学演化模型（galactic chemical evolution models）分析化学差异的起源。

Result: 确认盖亚-土卫二/香肠（GES）中存在两个动力学和化学性质不同的星群，分别与母体最内层（低能量）和最外层（高能量）区域相关。
...
**TL;DR**: 利用JWST的近红外相机（NIRCam）和中红外仪器（MIRI）观测W51A区域（含大质量原星团W51-E和W51-IRS2），发现W51-IRS2的反馈抑制气体吸积而W51-E仍持续吸积，并首次解析出[Fe II]和H$_2$发射结为大质量恒星驱动原恒星喷流撞击稠密介质的典型案例。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用JWST（詹姆斯·韦伯空间望远镜）的近红外相机（NIRCam）和中红外仪器（MIRI）观测银河系内高活跃恒星形成区W51A（包括大质量原星团W51-E和W51-IRS2），揭示其尘埃、电离气体和多环芳烃（PAHs）的空间分布差异及反馈机制对气体吸积的影响。

Data: 数据来源于JWST（James Webb Space Telescope）的10个NIRCam（近红外相机）和5个MIRI（中红外仪器）滤光片对W51A区域的观测，并与ALMA（阿塔卡马大型毫米波/亚毫米波阵列）数据进行对比。

Method: 采用詹姆斯·韦伯太空望远镜(JWST)的近红外相机(NIRCam)和中红外仪器(MIRI)在10个和5个滤光片下对W51A区域进行多波段观测，并结合ALMA数据开展交叉比对分析。

Result: JWST观测揭示了W51A区域中暖尘埃和电离气体主导的区域与多环芳烃(PAH)主导的区域存在明显区分。
...
**TL;DR**: 通过分析22个分子云及其原子氢（HI）包层的运动学特征，发现HI包层比分子云具有更小的速度梯度（平均0.03 km s⁻¹ pc⁻¹）和更大的比角动量（中值5.7×10²⁴ cm² s⁻¹），两者角动量比值平均为4且方向常不重合，表明分子云形成过程中角动量不守恒，并构建了角动量传输模型得出典型重分布时间尺度为13 Myr。


<details>
  <summary>Details</summary>

Background: 研究分子云（molecular clouds）及其原子氢（HI）包层（envelopes）的动力学演化，重点关注角动量（angular momentum）在分子云形成过程中的传输机制。

Data: 数据来源为使用$^{12}\mathrm{CO}$和21厘米谱线分别示踪分子气体和原子氢（HI）的观测数据，分析了22个分子云及其HI包层的运动学特征。

Method: 采用一氧化碳($^{12}\mathrm{CO}$)和21厘米谱线分别示踪分子气体和原子氢(HI)，通过测量22个分子云及其HI包层速度场的大尺度梯度($Ω$)并计算比角动量($j\propto ΩR^2$)，结合超音速湍流模型分析角动量传输过程。

Result: 分子云（H_2）和其原子氢（HI）包层的速度梯度方向通常不重合，表明分子云形成过程中角动量不守恒。
...
**TL;DR**: 该研究通过多波段观测分析18个超爱丁顿候选体（super-Eddington candidates）的射电辐射起源，发现其中7个主要来自恒星形成（SF），6个来自恒星形成与活动星系核（AGN）机制共同作用，仅3个显示核心或喷流主导的AGN起源，支持高爱丁顿比率（Eddington ratio）下射电辐射主要与恒星形成活动而非相对论性喷流相关，但首次在3个源中同时发现超爱丁顿吸积与相对论性喷流共存的证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于极高吸积率类星体（quasars）的射电辐射起源及其对星系演化的反馈作用。

Data: 数据来源包括新获取的Karl G. Jansky甚大阵(VLA)在L、C和X波段的观测数据，并结合了低频阵列(LOFAR)两米巡天(LoTSS)和甚大阵巡天(VLASS)的观测数据，同时使用了光学和红外数据来估算吸积、风参数及恒星形成率。

Method: 采用多波段射电观测（VLA的L、C和X波段）结合低频巡天数据（LOFAR的LoTSS和VLASS的观测），并辅以光学和红外数据估算吸积、外流参数及恒星形成率（SFR），通过分析射电变率、光度、形态、频谱特性、射电与红外SFR估算值及射电-中红外流量比来区分射电辐射起源。

Result: 在18个候选体中，7个的射电辐射主要来自恒星形成(SF)，6个来自SF和活动星系核(AGN)机制的组合，仅3个显示核心或喷流AGN起源。
...
**TL;DR**: 基于LEGUS巡天的5105个星团回归分析发现，星团半径主要取决于质量（对数质量系数0.131），而年龄和对数恒星形成率（sSFR）贡献微弱（系数分别为0.038和0.0002），支持质量在星团半径-质量关系中起主导作用的维里平衡理论预测。


<details>
  <summary>Details</summary>

Background: 星团（star clusters）作为星系的关键组成部分，其半径与质量的关系蕴含了星团形成和演化的重要信息。

Data: 数据来源于LEGUS巡天项目中的5,105个星团样本。

Method: 采用回归分析（regression analysis）比较多元模型（包括质量、年龄和特定恒星形成率）与仅含质量的基线模型对星团半径的预测能力。

Result: 质量主导了星团半径-质量关系，而特定恒星形成率(sSFR)和年龄的贡献可忽略不计。
...
**TL;DR**: 基于IllustrisTNG模拟发现星系盘存在三种表面密度分布类型（I型单指数型、II型下弯型、III型上弯型），其中II型盘（质量低于10^10.6太阳质量）占主导且形态延展，其下弯断裂由z=1以来冷气体吸积和局域恒星形成峰驱动的内向外生长机制自然形成，挑战了传统单指数形态假说。


<details>
  <summary>Details</summary>

Background: 该研究探讨星系盘（galactic disk）形态多样性（包括I型、II型和III型）的起源及其与恒星质量、演化历史和气体吸积过程的关联，挑战了传统单一指数形态（single-exponential paradigm）的假设。

Data: 数据来源于IllustrisTNG模拟（simulation）生成的星系盘表面密度分布数据，包括I型（单指数）、II型（下弯）和III型（上弯）剖面，覆盖恒星质量范围$M_\star < 10^{10.6} M_\odot$，并验证了与观测质量-尺寸关系和运动学的一致性。

Method: 采用IllustrisTNG数值模拟分析星系盘形态演化，重点研究冷气体吸积（cold-gas accretion）与局域恒星形成率峰值（specific star formation rate peaks）驱动的由内向外生长（inside-out growth）机制。

Result: Type II星系盘（Type II disks）通过$z=1$以来由同步冷气体吸积（synchronized cold-gas accretion）和局域恒星形成率峰值（localized peaks in specific star formation rate）驱动的内向外生长（inside-out growth）自然形成断裂结构，并产生其特征性的U形年龄分布（U-shaped age profiles）。
...
**TL;DR**: 通过比较IllustrisTNG模拟与积分场光谱（IFS）观测数据，研究发现早期型星系（ETGs）的标准运动学分类标准（$λ_{R}(R_e)=0.31\sqrt{\varepsilon}$和$\overline{k_5}$系数）与观测的双峰性不符，提出了修正阈值（$λ_{R,\mathrm{intr}} \sim 0.4$、$κ_{\mathrm{rot}} \sim 0.5$和$f_{\mathrm{spheroid}} \sim 0.6$）并揭示了模拟中快速旋转体不足的双峰性缺失现象，同时开发了从IFS运动学估算恒星晕质量分数（$f_{\mathrm{halo}}$）的新方法。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过数值模拟与积分视场光谱（IFS）观测的对比分析，探讨星系运动学（galaxy kinematics）分类标准的优化及其与结构参数（如球体质量分数和恒星晕质量分数）的关联。

Data: 数据来源包括IllustrisTNG模拟（TNG100和TNG50）的2,342个和236个早型星系（ETGs），以及MaNGA和ATLAS$^{3D}$的积分场光谱（IFS）观测数据。

Method: 采用积分视场光谱（IFS）观测与IllustrisTNG数值模拟对比分析星系运动学参数，包括内禀自转参数（λ_{R,intr}）、柱面转动能占比（κ_{rot}）及结构质量比（f_{spheroid}和f_{halo}），并基于新阈值（λ_{R,intr}∼0.4、κ_{rot}∼0.5、f_{spheroid}∼0.6）提出改进分类标准。

Result: 标准分类器（$λ_{R}(R_e)=0.31\sqrt{\varepsilon}$关系和$\overline{k_5}$系数）与观测的动力学双峰性不符，需采用修正阈值（$λ_{R,\mathrm{intr}} \sim 0.4$、$κ_{\mathrm{rot}} \sim 0.5$和$f_{\mathrm{spheroid}} \sim 0.6$）区分旋转主导（快转体）和随机运动主导（慢转体）星系。
...
**TL;DR**: 该研究通过气体运动学分析发现，大质量原星团G318.049+00.086的丝状结构（filamentary structures）向中心枢纽（hub）输送物质（质量流入率>10³ M⊙/Myr），支持竞争吸积（competitive accretion）机制下大质量恒星的形成过程，其中枢纽区域原恒星核（protostellar cores）的塌缩速率显著高于丝状结构上的核。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量原星团（protocluster）G318.049+00.086的气体动力学特征及其在巨恒星形成中的作用。

Data: 研究使用了1.3毫米尘埃连续谱图（1.3 mm dust continuum map）和H$^{13}$CO$^{+}$、CCH分子谱线观测数据。

Method: 采用H$^{13}$CO$^{+}$和CCH分子谱线分析、PV（位置-速度）图分析以及1.3毫米尘埃连续谱成像技术，结合速度梯度计算和质量流入率估算，研究了原恒星核心的引力坍缩特征和丝状结构动力学。

Result: 在G318.049+00.086原星团中，H$^{13}$CO$^{+}$和CCH谱线分析显示多数核心呈现蓝色不对称线型特征，表明存在引力坍缩，而少数核心呈现红色不对称线型特征，表明气体膨胀。
...
**TL;DR**: 研究发现，通过分析哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）拍摄的1000多个高红移（$z \lesssim 2.5$）侧向星系样本，星系盘翘曲（warp）现象在早期宇宙中更为普遍，且该趋势与观测效应无关，支持星系合并或相互作用是翘曲形成的主要机制。


<details>
  <summary>Details</summary>

Background: 星系盘面翘曲（warp）现象的普遍存在及其形成机制尚未完全明确，但可能与星系合并和相互作用的历史演化相关。

Data: 数据来源于哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）对超过1000个红移值（redshift）$z \lesssim 2.5$的侧向（edge-on）星系成像观测。

Method: 采用哈勃太空望远镜（HST）和詹姆斯·韦伯太空望远镜（JWST）成像数据，测量边缘朝向星系（edge-on galaxies）的翘曲特征参数并分析其随时间的变化规律。

Result: 星系盘翘曲（warp）现象在过去（红移z≲2.5）比现代更为普遍，且该趋势与观测效应无关，可能反映了星系合并（mergers）和相互作用（interactions）作为翘曲形成主要机制的真实演化。
...
**TL;DR**: 研究发现，在分子云和致密核（dense cores）尺度上，尘埃通过吸积和凝聚（coagulation）生长以及200微高斯磁场共同作用可解释观测到的约100米/秒的离子-中性粒子漂移速度（ion-neutral drift velocity），该速度可作为约束尘埃尺寸分布和磁场强度的新诊断手段。


<details>
  <summary>Details</summary>

Background: 研究聚焦于分子云和致密核中离子-中性粒子漂移速度（ion-neutral drift velocity）的观测与理论模拟，探讨尘埃生长（dust growth）和磁场强度对漂移速度的影响机制。

Data: 使用数值模拟（simulation）方法研究了尘埃通过吸积（accretion）和凝聚（coagulation）生长对分子云和核中离子-中性粒子漂移速度的影响。

Method: 采用数值模拟方法研究尘埃通过吸积和凝聚生长对分子云和核中离子-中性粒子漂移速度的影响。

Result: 在分子云和致密核（dense cores）尺度上，尘埃生长（dust growth）和200微高斯（microgauss）的磁场强度共同作用才能重现观测到的100米每秒的离子-中性粒子漂移速度（ion-neutral drift velocity）。
...
### [11] [What Drives the Bimodal Distribution of Eddington-Scaled Radio Luminosity in Nearby Early-Type Galaxies?](https://arxiv.org/abs/2602.01495)
*A. Wójtowicz,N. Werner,Ł. Stawarz,C. C. Cheung*

**TL;DR**: 通过分析117个近邻早型星系(Early-type galaxies)的埃丁顿标度射电光度分布，发现其存在双峰性（分界点≈-8.6），且射电亮星系与射电暗星系在喷流结构、宿主星系运动学（快/慢转体）和中心恒星结构上存在显著差异，表明持续喷流产生主要取决于星系组装历史和吸积模式而非仅由黑洞质量或吸积率决定。


<details>
  <summary>Details</summary>

Background: 研究聚焦于近邻早型星系（early-type galaxies）中低光度活动星系核（AGN）的射电辐射特性及其与寄主星系动力学和中心结构的关联。

Data: 数据来源于117个邻近早型星系（early-type galaxies）的直接测量黑洞质量及1.4 GHz和3 GHz射电流量密度，并结合VLASS成像、寄主星系运动学和中心恒星结构特征。

Method: 采用统计检验评估双峰性，并结合甚大阵列巡天（VLASS）成像、寄主星系运动学及中心恒星结构特征分析射电暗源与亮源。

Result: 在117个近邻早型星系样本中，证实了1.4 GHz爱丁顿标度射电光度（$L_{\rm 1.4,GHz}/L_{\mathrm{Edd}}$）存在双峰分布，反模位于≈-8.6处，且该双峰性在使用$M_{\rm BH}$-$σ_\star$关系推算黑洞质量时消失。
...
### [12] [Photometric Redshift PDFs via Neural Network Classification for DESI Legacy Imaging Surveys and Pan-STARRS](https://arxiv.org/abs/2602.01548)
*Da-Chuan Tian,Zhong-Lue Wen,Jun-Qing Xia*

**TL;DR**: 提出了一种基于神经网络分类（NNC）的光度红移（photo-z）估计方法，通过离散化红移空间并优化连续排序概率评分（CRPS），在DESI Legacy和Pan-STARRS数据中实现了高精度（σ_NMAD=0.0153/0.0222）且能捕捉多峰后验分布的红移概率密度函数（PDF），显著优于随机森林等传统方法。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用神经网络分类（NNC）方法改进测光红移（photometric redshift）估计，旨在生成校准良好的红移概率密度函数（PDFs）以解决传统回归方法在捕捉多模态后验分布时的局限性。

Data: 数据来源于DESI Legacy Imaging Surveys Data Release 10 (LSDR10)、Pan-STARRS Data Release 2 (PS1DR2)以及DESI DR1和SDSS DR19的光谱训练样本，并结合了unWISE红外测光数据。

Method: 采用神经网络分类（NNC）方法，通过将红移空间离散化为有序区间并优化连续排序概率评分（CRPS）来生成校准良好的红移概率密度函数（PDF）。

Result: 提出的神经网络分类（NNC）方法通过离散化红移空间并优化连续排序概率得分（CRPS），生成了校准良好的红移概率密度函数（PDF），优于随机森林、XGBoost和标准神经网络回归方法。
...
### [13] [Stellar Mass Growth in the First Galaxies: Theory and Observation](https://arxiv.org/abs/2602.01549)
*Alan Dressler,Andrew Benson*

**TL;DR**: 通过对比JWST观测数据与Galacticus半解析模型，发现$6<z<12$时期星系恒星质量（stellar mass）增长与模型预测高度一致，尤其在反馈（feedback）强度降低至原值1/3时，模型无需调参即可匹配积分恒星质量随红移的分布形态和幅度，并证实Dressler等提出的$8<z<6$期间恒星质量$M_*>2\times10^8 \mathrm{M}_\odot$的星系中半数在$z=8$前存在$M_*<10^8 \mathrm{M}_\odot$的星暴（starburst）现象，模型显示这些爆发源于$z\le8$的原位增长或富含气体/恒星的并合事件。


<details>
  <summary>Details</summary>

Background: 研究聚焦于红移（redshift）$6<z<12$时期星系恒星质量（stellar mass）增长的观测与半解析星系形成模型Galacticus预测的一致性验证。

Data: 数据来源为JWST（James Webb Space Telescope）观测数据与Galacticus半解析星系形成模型的模拟数据。

Method: 采用半解析星系形成模型（semi-analytic galaxy formation model）Galacticus进行星系恒星质量增长的模拟分析，并与JWST观测数据直接比较。

Result: 在$6<z<12$时期，星系恒星质量增长与Galacticus半解析模型的预测高度一致，特别是在反馈(feedback)降低至匹配后期宇宙模型所需值的1/3时，恒星质量积分形状和幅度(amplitude)均吻合。
...
### [14] [MAUVE: Cold neutral gas in the outflow of NGC 4383 and evidence for a fountain flow](https://arxiv.org/abs/2602.01612)
*L. Cortese,A. B. Watts,J. Sun,S. Sankar,B. Catinella,T. Brown,A. Boselli,P. Jáchym,T. Kolcu,S. Thater,J. van de Sande,V. Villanueva*

**TL;DR**: 通过多波段观测发现NGC 4383星系中原子相(HI)主导了1千秒差距以上的冷气体外流，分子相(CO)仅在内区有贡献，且整体表现为物质在晕内再分配的星系喷泉而非大规模逃逸风。


<details>
  <summary>Details</summary>

Background: 研究聚焦于室女座星系NGC 4383中恒星形成驱动的多相外流（outflow），重点探索外流对冷分子相（CO）和原子相（HI）的影响及其在星系反馈中的作用。

Data: 数据来源包括ALMA CO(2-1)观测数据、MeerKAT HI深度成像以及MUSE光谱数据，均来自MAUVE（Multiphase Astrophysics to Unveil the Virgo Environment）项目。

Method: 结合ALMA CO(2-1)数据、MeerKAT HI深度成像及MUSE光谱仪观测，采用多波段联合分析方法解析分子气体(CO)、原子气体(HI)与电离气体的空间分布与运动学特征。

Result: 在NGC 4383星系中，原子相（HI）主导了1千秒差距以上的冷气体外流，而分子相（CO）仅在盘面内1千秒差距范围内表现出扰动运动学特征且对流出气体质量贡献较小。
...
### [15] [Cloud-Cloud Collisions Induce Filament-Mediated Super Star Cluster Formation in the Antennae Overlap Region: Evidence from ALMA and JWST](https://arxiv.org/abs/2602.01616)
...
**TL;DR**: 通过高分辨率ALMA和JWST观测发现，触须星系（Antennae galaxies）中超巨分子云（SGMC）内两个速度差50 km/s的组分间的云-云碰撞（CCC）触发了超星团（SSC）的形成，并提供了多波段证据支持该机制。


<details>
  <summary>Details</summary>

Background: 研究聚焦于星系中超星团(SSCs)形成的未解之谜，特别关注云-云碰撞(CCCs)作为触发机制在并合星系中的作用。

Data: 使用高分辨率（0.12角秒，约14秒差距）的ALMA（阿塔卡马大型毫米波/亚毫米波阵列）观测数据，针对天线星系重叠区域的一个超巨分子云（SGMC）的CO（J=1-0）发射线，并辅以JWST（詹姆斯·韦伯空间望远镜）的红外成像数据。

Method: 采用高分辨率（0.12角秒）ALMA观测CO（J=1-0）谱线并结合JWST红外成像数据，通过分析分子云速度分量分离（~50 km/s）和形态学特征（U形结构及枢纽-纤维形态）验证了云-云碰撞（CCC）机制。

Result: 在触须星系重叠区的超巨分子云（SGMC）中，通过多波段观测首次获得了云-云碰撞（CCC）触发超星团（SSC）形成的直接证据。
...
### [16] [Physics-Informed Neural Networks for Modeling Galactic Gravitational Potentials](https://arxiv.org/abs/2602.01806)
*Charlotte Myers,Nathaniel Starkman,Lina Necib*

**TL;DR**: 提出了一种结合物理约束和神经网络的混合方法（physics-informed neural framework），用于高精度（平均加速度误差0.14%）建模静态和时变星系引力势（galactic gravitational potentials），在保持物理一致性的同时捕捉小尺度特征，优于传统解析方法。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发一种结合物理约束与数据驱动的神经网络框架，用于精确建模静态和时变（time-dependent）星系引力势（galactic gravitational potentials）。

Data: 数据来源于模拟（simulation）生成的具有不同复杂度的虚拟星系系统（mock systems）。

Method: 采用物理信息神经网络框架（physics-informed neural framework）结合贝叶斯不确定性量化（Bayesian framework）和神经常微分方程（neural ODE）建模静态与时变星系引力势（galactic gravitational potentials）。

Result: 提出了一种结合物理约束与数据驱动的神经网络框架，用于精确建模静态和时变（time-dependent）星系引力势场。
...
### [17] [Two faces of Gaia-Sausage-Enceladus: Mining the chemical abundance space with graph attention networks](https://arxiv.org/abs/2602.02226)
*Milan Quandt-Rodriguez,Sara Lucatello,Lorenzo Spina,Mario Pasquato,Marco Canducci*

**TL;DR**: 通过结合GALAH DR4高维化学丰度与运动积分（IoM），利用图注意力自编码器重建动力学信息化学空间并应用集成聚类，发现银河系晕中存在两个化学性质迥异的Gaia-Sausage-Enceladus（GSE）子结构，分别对应前身星系贫金属外围和富金属核心区域，并估算出41%的原位恒星比例。


<details>
  <summary>Details</summary>

Background: 研究主题是利用高维化学丰度（chemical abundances）结合运动积分（IoM）探索银河系恒星晕（stellar halo）的亚结构及其起源。

Data: 数据来源于GALAH DR4（Galactic Archaeology with HERMES Data Release 4）的高维化学丰度数据。

Method: 采用图注意力自编码器（graph attention autoencoder）重构动力学信息去噪化学空间，并结合整体聚类（ensemble clustering）识别恒星子结构。

Result: 通过高维化学丰度与运动积分（IoM）协同分析，发现盖亚香肠-恩克拉多斯（GSE）动态关联恒星分裂为两个化学性质不同的星群，分别对应前身星系贫金属外围和富金属核心区域。
...
### [18] [PRISMS. U37126, a very blue, ISM-naked starburst at z=10.255 with nearly 100% Lyman continuum escape fraction](https://arxiv.org/abs/2602.02322)
...
**TL;DR**: 这篇论文发现高红移(z=10.255)星系U37126具有极年轻的大质量恒星主导的星族、极高的电离光子产生效率(log(Xi_ion)~25.75)和接近完全的莱曼连续谱光子逃逸率(fesc>86%)，表明其处于"星际介质缺失"(ISM-naked)的星暴阶段，这类星系可能对宇宙再电离时期的电离光子预算做出不成比例的重大贡献。


<details>
  <summary>Details</summary>

Background: 研究聚焦于高红移(z=10.255)星系U37126的极端恒星形成活动及其对宇宙再电离(cosmic reionization)的潜在贡献。

Data: 数据来源为JWST（詹姆斯·韦伯空间望远镜）的MIRI（中红外仪器）低分辨率光谱观测和NIRSpec（近红外光谱仪）光谱观测。

Method: 采用合成恒星种群模型（synthetic stellar population models）和光谱能量分布拟合（SED fitting）分析恒星年龄、金属丰度及电离光子逃逸分数（fesc）。

Result: U37126展现出极年轻大质量恒星主导的星族特征，具有极高电离光子产生效率（log(Xi_ion / Hz erg^-1) ~ 25.75）和接近完全的莱曼连续谱光子逃逸率（fesc>86%），表明其处于“星际介质裸露”的星暴阶段。
...
### [19] [PRISMS. UNCOVER-26185, a metal-poor SFG at z=10.05 with no evidence for a X-ray-luminous AGN](https://arxiv.org/abs/2602.02323)
...
**TL;DR**: 首次在红移(redshift) z=10.054的透镜星系UNCOVER-26185中探测到显著Hβ+[OIII]复合发射线和Hα发射线，证实其为目前已知红移>9星系中金属丰度最低(Z=0.04Z⊙)、电离度最低(logU=-2.5)的主序恒星形成星系，具有年轻(7Myr)和中等年龄(65Myr)的恒星种群，总恒星质量为1.7×10⁸M⊙。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用JWST（詹姆斯·韦伯太空望远镜）的PRISMS（PRImordial galaxy Survey with MIRI Spectroscopy）项目，首次系统性地探索大爆炸后约5亿年（红移z≈10）的中等紫外光度星系（UV absolute magnitudes范围-17.9至-20.5）的物理特性，填补了此前对暗弱星系研究的空白。

Data: 数据来源于JWST（James Webb Space Telescope）第四周期项目PRISMS（PID 8051）使用MIRI LRS（Mid-Infrared Instrument Low Resolution Spectrometer）对透镜星系UNCOVER-26185（红移z=10.054）进行的13.9小时深度光谱观测。

Method: 采用中红外仪器低分辨率光谱仪（MIRI LRS）对透镜星系UNCOVER-26185进行深度光谱观测，检测H$β$+[OIII]复合发射线、H$α$发射线及光学连续谱，并结合紫外-光学光谱能量分布（SED）分析恒星形成历史（SFH）、消光及金属丰度（metallicity）。

Result: 首次在红移z=10.054的透镜星系UNCOVER-26185中探测到显著Hβ+[OIII]复合线(10σ)和Hα发射线(8σ)，其低金属丰度(Z=0.04Z⊙)和低电离度(logU=-2.5)使其成为迄今发现的最极端贫金属星系。
...
### [20] [DeepDive: Tracing the early quenching pathways of massive quiescent galaxies at $z>3$ from their star-formation histories and chemical abundances](https://arxiv.org/abs/2602.02485)
...
**TL;DR**: 研究发现红移(redshift)3-4的大质量(log10(M⋆/M⊙)>10.5)静止星系(quiescent galaxies)具有早期形成(50%恒星质量在z∼5前形成)和快速淬灭(quenching)(z∼4前完成)特征，其α元素丰度([α/Fe])平均值为0.22且与低红移样本一致，表明z∼4到z∼1期间[α/Fe]演化较弱。


<details>
  <summary>Details</summary>

Background: 研究聚焦于红移（redshift）3至4区间内大质量静止星系（quiescent galaxies）的化学丰度（chemical abundances）与恒星形成历史（star-formation histories），旨在揭示其早期形成与淬灭（quenching）机制。

Data: 数据来源于JWST（James Webb Space Telescope）DeepDive Cycle 2 GO项目获取的深度中分辨率光谱数据。

Method: 采用光谱-光度拟合（spectro-photometric fitting）方法分析化学丰度和恒星形成历史（SFH），并探索多种SFH参数化方案。

Result: result：高红移（$3<z<4$）大质量静止星系（quiescent galaxies）普遍呈现α元素增强（$\langle [α/\mathrm{Fe}]\rangle$= $0.22^{+0.22}_{-0.17}$）且形成历史多样，部分经历快速（$\lesssim$ 100 Myr）而另一些经历较长时间（$\gtrsim$ 200 Myr）的恒星形成过程，其化学丰度与低红移研究一致表明[$α$/Fe]从$z \sim 4$到$z\sim 1$演化较弱。
...
### [21] [Progress in Particle Physics and Modern Cosmology](https://arxiv.org/abs/2602.00106)
*A. D. Dolgov*

**TL;DR**: 首次提出了凤凰宇宙（phoenix universe）的动力学机制，解决了宇宙周期性重生（rebirth）的关键问题，并通过动态抵消真空能（vacuum energy）实现宇宙向高温态的跃迁。


<details>
  <summary>Details</summary>

Background: 该研究探讨了凤凰宇宙（phoenix universe）模型的动力学机制，延续了勒梅特（Lemaitre）1933年提出的宇宙周期性膨胀与收缩的假设，并解决了宇宙重生机制这一核心问题。

Data: 数据来源于理论推导和数值模拟（simulation）结合凤凰宇宙（phoenix universe）模型的动力学机制分析。

Method: 采用动力学机制（dynamical mechanism）和真空能量动态抵消机制（dynamical cancellation of vacuum energy）解释宇宙重生过程。

Result: 首次提出了凤凰宇宙（phoenix universe）的动力学机制，解决了宇宙重生机制这一根本问题。
...
### [22] [Studying dark gaps in Ly-$α$ forest transmission with large reionization simulation](https://arxiv.org/abs/2602.00301)
*Barun Maity,Frederick B. Davies,Benedetta Spina,Sarah E. I. Bosman*

**TL;DR**: 通过莱曼阿尔法森林暗隙分布分析，发现宇宙再电离（reionization）可能延续至红移5.4左右，且短平均自由程（mean free path）模型在低红移与观测不符，但无法排除存在增强紫外背景波动的早期再电离情景。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙再电离末期（红移z∼5.0-6.0）星系际介质（IGM）物理条件的不确定性及其对再电离结束阶段的影响。

Data: 使用扩展的XQR-30类星体光谱（extended XQR-30 quasar spectra）作为观测数据，并基于大体积（$\sim 1 ~\mathrm{Gpc}$）模拟框架生成合成视线（synthetic sightlines）进行对比分析。

Method: 采用高效的大体积（约1 Gpc）模拟框架，通过构建物理驱动的模型变体（如改变再电离红移、星系际介质温度及电离光子平均自由程）生成合成视线，并与观测光谱的暗隙累积分布进行对比分析。

Result: 再电离末期（redshift 5.0-6.0）的星系际介质（IGM）物理条件可通过暗隙分布（dark-gap distribution）有效约束，其中稍晚结束再电离（redshift 5.4）的模型与观测数据最吻合。
...
### [23] [Cosmological Dynamics of Multi-Axion Quintessence](https://arxiv.org/abs/2602.00820)
*Supakorn Katewongveerachart,David J. E. Marsh*

**TL;DR**: 研究发现双轴子（axion）模型可产生偏离单场"解冻精质"（thawing quintessence）行为的暗能量状态方程参数$(w_0,w_a)$，尤其存在交叉相互作用时支持更大有效衰变常数（decay constant）并降低最大衰变常数需求。


<details>
  <summary>Details</summary>

Background: 该研究探讨了暗能量光谱仪（DESI）观测结果的一种基础物理解释，即宇宙加速膨胀可能由缓慢滚动的标量场（如接近普朗克尺度的轴子）驱动，而非宇宙学常数，并研究了多轴子模型（如“弦轴子宇宙”）中暗能量动力学及其对状态方程参数（w0,wa）先验概率的影响。

Data: 研究使用了基于双轴子（two axions）玩具模型的模拟数据（simulation data），其中考虑了轴子间相互作用（cross-interactions）及不同衰变常数（decay constants）对暗能量状态方程参数（$(w_0,w_a)$）先验概率的影响。

Method: 采用双轴子（two axions）玩具模型研究暗能量动力学，计算暗能量状态方程参数$(w_0,w_a)$的先验概率分布，并分析轴子交叉相互作用对有效衰变常数（decay constant）的影响。

Result: 双轴子（axion）模型可产生偏离DESI偏好的"解冻精质"（thawing quintessence）状态方程参数$(w_0,w_a)$，且交叉相互作用会强化这种偏离效应。
...
### [24] [Skewness in the Hellings-Downs curve](https://arxiv.org/abs/2602.01108)
*Ryosuke Fujimoto,Keitaro Takahashi*

**TL;DR**: 研究发现，在纳赫兹引力波背景（nano-Hertz gravitational-wave background）中，Hellings-Downs（HD）相关性的三阶中心矩（偏度）在源数量增多时仍保持非零，并由新的几何三点函数控制，表明高阶矩分析可提供超越高斯统计的离散源信息。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用脉冲星计时阵（Pulsar Timing Array）数据探测纳赫兹引力波背景（nano-Hertz gravitational-wave background）时，通过分析Hellings-Downs（HD）相关性的高阶统计矩（如偏度）来揭示离散源（discrete sources）的非高斯性特征。

Data: 数据来源于纳赫兹引力波背景的脉冲星计时阵列（Pulsar Timing Array）观测数据集，并基于艾伦（Allen）2023年的方差计算进行了扩展分析。

Method: 采用三阶脉动星平均形式体系（pulsar-averaging formalism）推导了单点源和混淆噪声（confusion-noise）状态下多源干扰的赫林斯-唐斯（Hellings-Downs）相关性的第三中心矩（偏度），并引入三点平均相关函数以定义宇宙偏度（cosmic skewness）。

Result: 在大量源极限下，宇宙偏度（cosmic skewness）仍保持非零并由新的几何三点函数控制，表明高阶矩分析能提供超越标准高斯方法的离散源信息。
...
### [25] [Wigner Function Shapelets I : formalism](https://arxiv.org/abs/2602.01141)
*Shun Arai*

**TL;DR**: 提出了用于星系图像分析的维格纳函数形状基（Wigner Function Shapelets，WFS），通过构建基于拉盖尔-高斯模式交叉维格纳函数的正交完备基，在四维相空间中以辛群$\mathrm{Sp}(4,\mathbb{R})$表示图像，并引入运动常数$(Q_0,Q_2)$空间的标量函数$\mathcal{W}_{k\ell}$揭示相空间能带结构，同时保留了维格纳函数对星系形态和宇宙学印记的敏感振荡模式特征。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于通过引入维格纳函数形状基（Wigner Function Shapelets, WFS）将星系图像分析扩展至相位空间（phase space），利用辛群$\mathrm{Sp}(4,\mathbb{R})$和$\mathrm{SU}(2)$不可约表示构建对称性保持的完整正交基，以统一处理天体物理与宇宙学中的图像信息及量子信息理论框架。

Data: 数据来源为基于相位空间（phase space）的维格纳函数形状基（Wigner Function Shapelets, WFS），通过拉盖尔-高斯模式（Laguerre-Gaussian modes）的交叉维格纳函数（cross-Wigner function）构建正交完备基，并利用相位空间单元（phase-space cell）$2π\lambdabar$量化望远镜分辨率限制。

Method: 采用维格纳函数形状基（Wigner Function Shapelets, WFS）作为核心方法，通过基于辛群$\mathrm{Sp}(4,\mathbb{R})$的四维相空间表示，利用拉盖尔-高斯模的交叉维格纳函数双线性形式构建正交完备基，并结合$\mathrm{SU}(2)$不可约表示与霍普夫环面（Hopf tori）实现相空间量子化。

Result: 引入的维格纳函数形状基（WFS）通过辛群$\mathrm{Sp}(4,\mathbb{R})$在四维相空间中直接表示图像，并利用相位空间单元$2π\lambdabar$量化望远镜分辨率极限。
...
### [26] [Detectability and Model Discriminability of the Dark Ages 21 cm Global Signal](https://arxiv.org/abs/2602.01263)
*Shintaro Yoshiura,Fumiya Okamatsu,Tomo Takahashi*

**TL;DR**: 研究发现，在1-50 MHz宽频观测中，除低频平滑谱模型外，其他宇宙学模型的中性氢21厘米信号（21 cm signal）均可被识别，且15 MHz以下观测对避免前景（foreground）简并至关重要，稀疏频点（5 MHz间隔）观测在误差足够小时也能捕捉信号特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用中性氢原子21厘米信号（21 cm signal）探测宇宙黑暗时期（Dark Ages），该信号是研究标准宇宙学模型预测的21厘米光谱形态的关键探针。

Data: 数据来源于对1-50 MHz频段内21厘米信号的宽频带观测（wide-band observations），包括低于15 MHz的关键频段和以5 MHz为间隔的稀疏采样（sparse sampling）频点。

Method: 采用贝叶斯证据比较方法评估不同宇宙学模型下21厘米信号的可探测性和模型选择能力。

Result: 宽频段观测（1-50 MHz）可识别除低频平滑谱模型外所有考虑模型中非零21厘米信号的证据。
...
### [27] [A Variable-Slope Smooth-$k$ Filter for Modeling Halo Abundances with Damped and Oscillatory Power Spectra](https://arxiv.org/abs/2602.01320)
*Andreu Rocamora Martorell*

**TL;DR**: 提出可变斜率平滑k（VSMK）滤波器，在Press-Schechter形式下统一描述具有阻尼和振荡功率谱的非冷暗物质（non-cold dark matter）模型中晕质量函数，通过解耦小尺度抑制效应与中尺度暗声振荡（dark acoustic oscillations）特征，用单一参数组同时匹配温暗物质（warm dark matter）和ETHOS模型的N体模拟结果。


<details>
  <summary>Details</summary>

Background: 研究聚焦于在Press-Schechter形式下开发可变斜率平滑k（VSMK）滤波器，以解决非冷暗物质（如温暗物质和ETHOS模型）中因阻尼和振荡功率谱导致的晕质量函数建模问题。

Data: 数据来源为数值模拟（N-body simulations）中的温暗物质（warm dark matter）和基于ETHOS的模型（ETHOS-based models）。

Method: 采用可变斜率平滑k（VSMK）滤波器在Press-Schechter形式体系中建模阻尼和振荡物质功率谱导出的晕质量函数。

Result: 可变斜率平滑k（VSMK）滤波器通过使k空间窗函数的有效对数斜率在两个渐近区域间平滑变化，解耦了小尺度晕丰度抑制与中尺度暗声学振荡特征。
...
### [28] [A single field inflationary potential consistent with recent observations](https://arxiv.org/abs/2602.01327)
*Md. Wali Hossain*

**TL;DR**: 该论文提出并验证了一种反指数形式（inverse exponential form）的单场暴胀势能模型，该模型不仅与观测数据高度吻合，还通过引入陡峭指数项实现了暴胀后宇宙的再加热（reheating），最高再加热温度可达$10^{13}\,\mathrm{GeV}$。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于单场暴胀（single-field inflation）的理论模型构建与观测验证，探讨逆指数形式暴胀势（inverse exponential inflaton potential）在描述早期宇宙暴胀及后续再加热（reheating）过程中的理论自洽性与观测兼容性。

Data: 研究中使用的数据来源为SPA+BK+DESI数据集。

Method: 采用逆指数形式暴胀势（inverse exponential potential）作为单场暴胀模型，并通过添加陡峭指数项描述暴胀后演化和再加热过程。

Result: 逆指数暴胀势（inverse exponential potential）在单场暴胀模型中表现出优异的观测一致性，且在$n_{\rm s}$-$r$平面内与SPA$+$BK$+$DESI数据集在$1σ$范围内高度吻合。
...
### [29] [Resonant production of heavy particles during inflation and its gravitational wave signature](https://arxiv.org/abs/2602.01343)
*Qi Chen,Yuan Yin*

**TL;DR**: 研究发现，在暴胀期间，二次U(1)对称性破缺项与暴胀子和U(1)电流的五维导数耦合诱导的有效化学势共同作用，即使U(1)场质量超过哈勃尺度仍能驱动高效粒子产生，并产生可观测的原初引力波信号和宇宙对撞机信号。


<details>
  <summary>Details</summary>

Background: 研究探讨了暴胀（inflation）期间通过二次U(1)对称性破缺项和有效化学势（chemical potential）驱动粒子高效产生的机制及其对引力波信号和暗物质（dark matter）的宇宙学影响。

Data: 数据来源为理论模拟（simulation）研究，未使用实际观测数据。

Method: 采用二次U(1)对称性破缺项与暴胀子(Inflaton)和U(1)流之间的五维导数耦合诱导的有效化学势(effective chemical potential)作为核心物理模型。

Result: 二次U(1)对称性破缺项与暴胀子同U(1)流之间的五维导数耦合诱导的有效化学势(effective chemical potential)共同作用，可在U(1)场质量超过哈勃尺度时仍驱动暴胀期间的高效粒子产生。
...
### [30] [Partial Relief of the Hubble Tension and a Natural Self-Interacting Dark Matter Candidate From Staged Symmetry Breaking](https://arxiv.org/abs/2602.01543)
*Zachary J. Hoelscher,Thomas W. Kephart,Robert J. Scherrer,Kelly-Holley Bockelmann*

**TL;DR**: 提出暗物质（dark matter）亚组分通过暗区自发对称性破缺（spontaneous symmetry breaking）触发衰变，使其状态方程参数（equation of state parameter）从早期$w \approx 0$转变为晚期$w \approx -1/3$，部分缓解哈勃常数（Hubble constant）测量间的$\sim5σ$张力，同时自然产生速度依赖截面的自相互作用暗物质候选者，有助于解决$Λ$CDM模型与小尺度结构观测间的矛盾。


<details>
  <summary>Details</summary>

Background: 研究聚焦于哈勃常数（Hubble constant, H₀）在宇宙微波背景（CMB）和本地距离阶梯测量间的显著张力（~5σ），提出暗物质亚组分通过暗区自发对称性破缺触发衰变可能部分缓解该张力。

Data: 数据来源包括宇宙微波背景辐射（CMB）的观测数据和通过距离阶梯（distance ladder）方法获得的局部测量数据。

Method: 采用有效场论（effective field theory）方法构建暗物质（dark matter）子成分衰变模型，其动力学由暗区自发对称性破缺触发，并自然产生速度依赖的自相互作用截面。

Result: 暗物质（dark matter）的一个子成分通过自发对称性破缺（spontaneous symmetry breaking）触发衰变，使其状态方程参数（equation of state parameter）从早期的$w \approx 0$转变为晚期的$w \approx -1/3$，从而可能部分缓解哈勃常数（Hubble constant）的张力。
...
### [31] [Bounds on the Tsallis Parameter from a deformed Neutrino Sector in the Early Universe](https://arxiv.org/abs/2602.01596)
*Matias P. Gonzalez*

**TL;DR**: 研究通过推广Tsallis非广延统计中的中微子能量密度（neutrino energy density），利用Curado-Tsallis约束得到广义分布函数$f_q(E)$，并基于重标度比$R^{(ξ= +1)}_ρ(q)$将变形能量密度映射到有效中微子数$N_{\rm eff}$，结合CMB+BAO和BBN数据联合拟合得出Tsallis参数$q$的最佳约束（95\% CL下$|q-1|\le 1.09\times 10^{-2}$，99\% CL下$|q-1|\le 1.32\times 10^{-2}$）。


<details>
  <summary>Details</summary>

Background: 研究探讨了早期宇宙大爆炸核合成（BBN）时期中微子能量密度在Tsallis非广延统计框架下的广义化描述及其对有效中微子数（Neff）的形变影响。

Data: 研究中使用的数据来源于宇宙微波背景辐射（CMB）、重子声学振荡（BAO）以及大爆炸核合成（BBN）的观测数据。

Method: 采用Tsallis非广延统计（Tsallis nonextensive statistics）结合Curado-Tsallis约束推导广义分布函数$f_q(E)$，并通过重标度$R^{(ξ= +1)}_ρ(q)$映射变形能量密度与标准情形的比值，进而利用联合卡方拟合（$χ^2$ type-fit）约束Tsallis参数$q$。

Result: 在早期宇宙大爆炸核合成(BBN)时期(T≃1 MeV)内，基于Tsallis非广延统计推广了中微子能量密度内容，并通过Curado-Tsallis约束获得广义分布函数f_q(E)，进而计算广义热力学积分ρ_q，定义重标度因子R_ρ^(ξ=+1)(q)=ρ_q/ρ^std以直接映射和变形中微子含量。
...
### [32] [Simulation-based cosmological inference from optically-selected galaxy clusters with $\texttt{Capish}$](https://arxiv.org/abs/2602.01911)
*Constantin Payerne,Calum Murray,Hugo Simon*

**TL;DR**: 该论文开发了基于模拟推理（SBI）的前向建模方法及配套Python工具$\texttt{Capish}$，通过联合建模星系团丰度和弱透镜质量观测数据，在考虑观测噪声、红移误差、选择函数等系统效应下，实现了比传统解析似然方法更精确的宇宙学参数约束。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用星系团（galaxy clusters）作为宇宙结构增长的探针，通过其丰度与质量和红移的函数关系来提取精确的宇宙学约束。

Data: data：研究使用基于模拟的推断（SBI）方法，结合神经密度估计和归一化流（normalizing flows）训练，数据来源包括观测的红移-富度（redshift-richness）分档中的星系团丰度和平均透镜质量测量值，以及从大型宇宙学模拟构建的星系团目录。

Method: 采用基于模拟的推断（Simulation-Based Inference, SBI）结合神经密度估计和归一化流（normalizing flows）进行正向建模，通过Python代码Capish生成包含观测效应的星系团目录。

Result: 提出了一种基于模拟的推断（SBI）前向建模方法，通过神经网络密度估计和归一化流联合建模星系团丰度和透镜质量观测数据，有效捕获系统性不确定性且无需显式似然函数。
...
### [33] [Clustering of emission line galaxies with IllustrisTNG -- II. cosmology challenge with anisotropic correlation functions and ELG-halo connections](https://arxiv.org/abs/2602.01925)
*Ken Osato,Teppei Okumura*

**TL;DR**: 利用IllustrisTNG流体动力学模拟和恒星种群合成框架构建了发射线星系(ELG)模拟样本，发现ELG各向异性相关函数的四极矩因向宿主晕中心下落运动而受到抑制，导致线性增长率低估并揭示ELG与暗物质间存在速度偏差，同时通过相空间分布证实了卫星ELG下落运动挑战了卫星径向分布遵循暗物质的假设。


<details>
  <summary>Details</summary>

Background: 发射线星系（ELG）是当前第四阶段宇宙学光谱巡天中宇宙大尺度结构的主要示踪体，旨在以前所未有的精度测量更高红移（z≈1.5–2）处的成团统计特性。

Data: 数据来源于IllustrisTNG流体动力学模拟（hydrodynamical simulations）和恒星种群合成框架（stellar population synthesis framework）构建的模拟发射线星系（ELGs）样本以及作为对照样本的质量限定子晕（mass-limited subhalo）样本。

Method: 采用IllustrisTNG流体动力学模拟和恒星种群合成框架构建模拟ELG样本，并通过测量各向异性相关函数推断线性增长率（linear growth rate），同时利用相空间分布和恒星形成活动的星系一致性（galactic conformity）研究ELG-晕（halo）关联。

Result: 发射线星系(ELG)的各向异性关联函数四极矩受到抑制，表明ELG存在向宿主晕中心坠落的运动，导致线性增长率被低估并暗示ELG与暗物质间存在速度偏差。
...
### [34] [Negentropy as Diagnostic of Cosmic Density Fields and Dynamical Dark Energy Models](https://arxiv.org/abs/2602.02339)
*Suman Sarkar*

**TL;DR**: 采用信息熵差（negentropy）作为宇宙密度场非高斯性的信息论探针，量化其对动力学暗能量（dynamical dark energy）的敏感性，发现最大非高斯结构形成特征红移（$z_{NG}$）为0.81、信息产生转变红移（$z_{TA}$）为0.18（$Λ$CDM模型），并区分了低红移下解冻（thawing）与冻结（freezing）精质模型及幻影暗能量（phantom dark energy）的演化差异。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用负熵（negentropy）作为信息论探针，在宇宙密度场中探测非高斯性（non-Gaussianity），并量化其对动力学暗能量（dynamical dark energy）演化的敏感性。

Data: 使用三种参数化方案（CPL、JBP和BA）下的宇宙密度场模拟数据，通过负熵（negentropy）分析非高斯性演化特征。

Method: 采用负熵（negentropy）作为信息论探针，结合三种参数化方案（CPL、JBP和BA）分析宇宙密度场的非高斯性，并通过演化函数J(a)及其导数Γ₁(a)和Γ₂(a)量化其对动力学暗能量的敏感性。

Result: 负熵（negentropy）作为宇宙密度场非高斯性的信息论探针，能有效区分不同动力学暗能量模型（如解冻型、冻结型和幻影型），并确定了非高斯结构形成最大时期对应的特征红移（z_{NG}∼0.81）和信息产生转变对应的转折红移（z_{TA}∼0.18）。
...
### [35] [Reconstructing the largest scales of the Universe with field-level inference applied to the Quaia Quasar Catalogue](https://arxiv.org/abs/2602.02363)
*Adam Andrews,Arthur Loureiro,Jens Jasche,Stuart McAlpine,Guilhem Lavaux,Florent Leclercq*

**TL;DR**: 利用Quaia类星体（quasar）目录和BORG算法重建了宇宙初始条件及现今物质分布，实现了迄今最大共动体积（(10h^{-1} Gpc)^3）的场级宇宙学重建，并通过与普朗克CMB透镜（Planck CMB lensing）的4σ相关信号验证了结果可靠性。


<details>
  <summary>Details</summary>

Background: 利用Quaia类星体（quasar）目录的宽红移范围和全天覆盖特性，开展宇宙物质三维分布重建以探索宇宙大尺度结构演化。

Data: 使用Quaia类星体（quasar）目录的G < 20.0（Quaia Clean）和G < 20.5（Quaia Deep）样本数据，其中G表示盖亚（Gaia）宽光学波段星等，并采用保守天区切割以确保对前景污染的鲁棒性。

Method: 采用场级推断算法BORG（Bayesian Origin Reconstruction from Galaxies）结合拉格朗日扰动理论（Lagrangian perturbation theory）构建物理驱动的前向模型，整合光锥效应（light-cone effects）、红移空间畸变（redshift-space distortions）、类星体偏置（quasar bias）及巡天选择效应（survey selection effects），实现对宇宙三维密度场及初始条件的高精度推断。

Result: 基于Quaia类星体（quasar）目录，应用BORG算法重建了宇宙初始条件及现今物质分布，验证了场级宇宙学框架在类星体巡天中的有效性。
...
### [36] [Mitigating half-wave plate systematics at the map-making level: calibration requirements for LiteBIRD](https://arxiv.org/abs/2602.02410)
...
**TL;DR**: 论文提出了一种评估半波片（HWP）非理想性对LiteBIRD卫星测量张量标量比（r）偏差影响的端到端分析框架，并推导了确保科学目标所需的HWP特性测量要求。


<details>
  <summary>Details</summary>

Background: 研究聚焦于宇宙微波背景（CMB）实验中半波片（HWP）非理想性引入的系统误差及其对张量标量比（$r$）测量偏差的影响。

Data: 数据来源于LiteBIRD卫星任务中半波片（HWP）非理想性的真实频率剖面与地图制作器假设剖面之间的差异分析。

Method: 采用端到端框架（end-to-end framework）结合盲成分分离（blind component-separation）技术分析半波片（HWP）非理想性对张标比（tensor-to-scalar ratio）测量偏差的影响。

Result: 半波片（HWP）非理想特性的测量误差会导致张量标量比（tensor-to-scalar ratio）$r$的估计偏差，需通过精确表征其频率响应曲线来满足LiteBIRD任务的科学目标要求。
...
### [37] [The FLAMINGO Project: Exploring the X-ray--cosmic-shear cross-correlation as a probe of large-scale structure](https://arxiv.org/abs/2602.02484)
*William McDonald,Joop Schaye,Konrad Kuijken,John Helly,Joey Braspenning,Matthieu Schaller*

**TL;DR**: 通过FLAMINGO流体动力学模拟结合暗能量巡天（Dark Energy Survey）和ROSAT全天X射线数据，发现星系团（$10^{14}\leq M_{200\mathrm{c}}/\mathrm{M}_{\odot}\leq10^{15}$）中气体分布对活动星系核（AGN）反馈强度敏感，但需解决较暗AGN的观测限制并消除重子反馈与宇宙学参数的简并性才能充分发挥X射线-弱引力透镜互相关测量的潜力。


<details>
  <summary>Details</summary>

Background: 研究聚焦于重子反馈过程（baryonic feedback）对星系团和星系群中气体分布的影响及其与宇宙剪切（cosmic shear）的交叉关联。

Data: 研究中使用的数据来源于FLAMINGO宇宙流体动力学模拟（FLAMINGO cosmological, hydrodynamical simulations）和ROSAT全天X射线巡天图（ROSAT all-sky X-ray maps）与暗能量巡天（Dark Energy Survey）的宇宙剪切（cosmic shear）数据的交叉关联。

Method: 采用FLAMINGO宇宙学流体动力学模拟（FLAMINGO cosmological, hydrodynamical simulations）分析X射线-宇宙剪切（X-ray--cosmic-shear）互相关信号，重点考察质量范围$10^{14}\leq M_{200\mathrm{c}}/\mathrm{M}_{\odot}\leq10^{15}$的晕中气体分布对反馈强度的敏感性。

Result: 通过宇宙剪切（cosmic shear）与X射线辐射的互相关分析发现，反馈强度与气体分布存在显著关联，但重子反馈效应与宇宙学参数之间存在明显简并性。
...
### [38] [aurel: A Python package for automatic relativistic calculations](https://arxiv.org/abs/2602.00155)
*Robyn L. Munoz,Christian T. Byrnes,Will J. Roper*

**TL;DR**: 开发了开源Python工具包aurel（automatically calculate relativistic quantities），通过高效缓存和依赖跟踪系统支持符号与数值计算，扩展了SymPy的张量运算能力，并能直接从时空与物质数据数组计算曲率等张量量，为数值相对论（Numerical Relativity）模拟提供后处理支持。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于开发一个开源Python软件包（aurel），用于自动化计算相对论（relativity）中的各类物理量，以支持广义相对论（general relativity）中高度非线性问题的数值和符号计算需求。

Data: 数据来源包括解析表达式生成的输入和数值相对论（Numerical Relativity, NR）模拟导入的数据，并提供了辅助函数用于读取标准NR代码的输出数据。

Method: 采用高效灵活的缓存和依赖跟踪系统，结合符号计算（扩展SymPy）和有限差分法数值计算，支持从解析表达式或数值相对论(NR)模拟数据中自动计算相对论性物理量。

Result: 开发了一个名为aurel的开源Python包，用于自动计算相对论性(relativistic)物理量，支持符号和数值计算，并提供了高效的缓存和依赖跟踪系统。
...
### [39] [Model-Agnostic Population Inference for Gravitational-Wave Astronomy: From LIGO to LISA](https://arxiv.org/abs/2602.00195)
*Yi-kun Li,Cheng Cheng,Lang Cui,Yun Fang*

**TL;DR**: 提出了一种基于深度生成建模的推断框架（Correlated Compound-Mixture Density Network），通过模拟LISA超大质量黑洞双星合并目录和真实LIGO-Virgo-KAGRA GWTC-3观测数据验证，该方法能准确恢复复杂三维分布、绝对合并率并有效校正选择效应和测量误差，适用于不同探测器灵敏度与源群体。


<details>
  <summary>Details</summary>

Background: 研究主题为利用深度生成建模克服传统参数化方法在推断致密双星并合（compact binary mergers）本征群体分布时的模型依赖性偏差。

Data: 数据来源于模拟的LISA超大质量黑洞双星并合目录和LIGO-Virgo-KAGRA GWTC-3引力波事件目录的实际观测数据。

Method: 采用深度生成建模框架，结合相关复合混合密度网络（Correlated Compound-Mixture Density Network）整合混合模型处理多模态、高斯Copula处理参数依赖关系以及灵活边缘分布库，通过摊销变分推断（amortized variational inference）和标准化流（Normalizing Flows）训练网络近似群体超参数后验分布。

Result: 通过深度生成建模构建的推断框架能准确恢复复杂三维分布和绝对合并率，有效校正选择效应和测量不确定性。
...
### [40] [An autoencoder-based surrogate waveform model for quasi-circular binary-black-hole mergers](https://arxiv.org/abs/2602.00203)
...
**TL;DR**: 提出基于自编码器(Autoencoder)的AESur3dq8替代波形模型，能快速生成与数值相对论波形失配度达10^-4量级的双黑洞(BBH)并合引力波信号，且参数估计结果与LIGO-Virgo-KAGRA合作组观测数据一致。


<details>
  <summary>Details</summary>

Background: 引力波天文学中双黑洞(BBH)并合精确波形生成是核心挑战。

Data: 数据来源包括数值相对论(Numerical Relativity)启发的替代波形模型NRHybSur3dq8和SXS目录中的双黑洞(BBH)模拟数据。

Method: 采用基于自编码器(autoencoder)的机器学习方法构建了AESur3dq8替代波形模型，通过数值相对论(Numerical Relativity)启发的NRHybSur3dq8模型训练，并利用SXS目录的BBH模拟数据进行微调。

Result: 基于自编码器（autoencoder）的AESur3dq8模型能快速生成与数值相对论波形失配度约10^-4的引力波信号模板。
...
### [41] [A Roman Coronagraph Spectroscopic Mode Demonstration](https://arxiv.org/abs/2602.00225)
*Thayne Currie,Brianna Lacy,Yiting Li,Mona El Morsy,Danielle Bovie,Kellen Lawson,Masayuki Kuzuhara,Naoshi Murakami*

**TL;DR**: 该论文提出利用730纳米高对比度光谱观测研究自发光直接成像系外行星（HIP 99770 b、HIP 54515 b、β Pic b和HR 8799 e），以评估罗曼日冕仪（Roman Coronagraph）的行星表征能力，并首次分析散斑噪声波长依赖性及碎片盘信号干扰下的光学行星光谱提取方法。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用730纳米高对比度光谱观测评估罗曼日冕仪（Roman Coronagraph）对自发光直接成像系外行星（self-luminous directly-imaged planets）的表征能力。

Data: 数据来源为地基红外（ground-based IR）发现的行星样本，包括NASA总部支持的Subaru/OASIS巡天项目中的HIP 99770 b和HIP 54515 b，以及标志性行星β Pic b和HR 8799 e。

Method: 采用730纳米高对比度光谱观测技术评估罗曼日冕仪（Roman Coronagraph）的行星表征能力。

Result: 730 nm高对比度光谱观测可测试罗曼日冕仪（Roman Coronagraph）的行星表征能力，并首次评估散斑噪声（speckle noise）的波长依赖性及在此噪声下提取精确大气信息的能力。
...
### [42] [Multigroup Thermal Radiation Transport with Tensor Trains](https://arxiv.org/abs/2602.00290)
*Aditya S. Deshpande,Patrick D. Mullen,Alex A. Gorodetsky,Joshua C. Dolence,Chad D. Meyer,Jonah M. Miller,Luke F. Roberts*

**TL;DR**: 提出张量链（TT）算法用于多群热辐射传输（photon radiation transport）模拟，利用解的低秩特性实现单节点万亿参数规模计算，压缩率超100倍且加速超2倍。


<details>
  <summary>Details</summary>

Background: 研究聚焦于张量链（TT）算法在多群热辐射传输（即光子辐射传输）模拟中的应用，旨在解决传统硬件难以处理的高维离散化计算问题。

Data: 数据来源于基于张量列（tensor train）算法的多群热辐射传输（multigroup thermal radiation transport）数值模拟（simulation）。

Method: 采用张量链（tensor train, TT）算法对多群热辐射传输（photon radiation transport）问题进行低秩结构建模与高效压缩求解。

Result: 张量列（TT）算法能有效利用多群热辐射传输（photon radiation transport）问题固有的低秩结构，实现单节点上万亿级离散解参数的高效求解。
...
### [43] [3D Printed Alumina as a Millimeter-Wave Optical Element](https://arxiv.org/abs/2602.00304)
...
**TL;DR**: 在158-700 GHz频段测得3D打印氧化铝(Alumina)的折射率(n=3.107±0.007)和损耗角正切(tanδ=1×10⁻³~2.49×10⁻³)，并证实单面亚波长结构抗反射涂层(SWS-ARC)可将158-200 GHz频段反射率从64%降至25%，表明该材料及制备技术适用于天体物理应用。


<details>
  <summary>Details</summary>

Background: 研究聚焦于三维打印氧化铝（alumina）材料在毫米波和亚毫米波频段（158-700 GHz）的光学特性及其在天文应用中的潜力。

Data: 数据来源于毫米波和亚毫米波室温环境下对3D打印氧化铝（alumina）圆盘及单面镀有亚波长结构抗反射涂层（SWS-ARC）圆盘的透射与损耗测量。

Method: 采用毫米波和亚毫米波室温透射与损耗测量技术，结合有限元分析（finite element analysis）对3D打印氧化铝（alumina）圆盘及单面亚波长抗反射结构（sub-wavelength structures anti-reflection coatings, SWS-ARC）进行性能表征。

Result: result：在158-700 GHz频段测得氧化铝折射率（refractive index）为3.107±0.007，损耗角正切（tan δ）介于1×10⁻³至2.49×10⁻³，且单面亚波长结构抗反射镀层（SWS-ARC）使158-200 GHz频段反射率从64%降至25%，与有限元分析预测一致。
...
### [44] [Simulating Roman+Gaia Combined Astrometry, Parallaxes, and Proper Motions](https://arxiv.org/abs/2602.00310)
*Kevin A. McKinnon,Roeland P. van der Marel*

**TL;DR**: 开发了结合盖亚(Gaia)和罗曼(Roman)数据的新型天体测量精度模拟工具，预测高纬度巡天(HLWAS)和银河核球时域巡天(GBTD)分别能在G=26.5和G=29.0星等达到盖亚-DR3级别的自行运动(PM)精度。


<details>
  <summary>Details</summary>

Background: 下一代高精度天体测量学（astrometry）即将通过欧几里得（Euclid）、LSST和RST等任务实现突破。

Data: 研究中使用的数据来源包括盖亚(Gaia)任务的高精度天体测量数据以及罗曼(Roman)空间望远镜在不同滤镜、星等和曝光时间下的模拟位置不确定性数据。

Method: 采用的核心分析方法是通过构建罗马望远镜（Roman）位置不确定性与滤镜、星等和曝光时间的函数关系，结合盖亚（Gaia）精度和用户定义的观测策略，预测位置、视差和自行运动（PM）的预期不确定性。

Result: 开发了一个新工具（https://github.com/KevinMcK95/gaia_roman_astrometry）用于模拟盖亚（Gaia）与罗曼（Roman）图像数据结合时将达到的天体测量精度。
...
### [45] [GBD-DART-I : Pulsars and transient source observation between 130 MHz and 350 MHz at Gauribidanur](https://arxiv.org/abs/2602.00677)
...
**TL;DR**: 研制了由64个短偶极子组成的菱形布局低功耗偶极天线阵列（GBD-DART），用于在130-350 MHz频段探测脉冲星（Pulsars）和太阳瞬变现象（Solar transients），并通过东西-南北方向的高旁瓣抑制设计及双极化运行实现了对强脉冲星和太阳耀斑的成功观测。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用新型低成本射电望远镜阵列（GBD-DART）在低频段（130-350 MHz）对明亮脉冲星（Pulsars）和太阳瞬变现象（Solar transients）的观测，同时为射电天文研究提供简易训练平台。

Data: 数据来源于Gauribidanur Diamond Array Radio Telescope (GBD-DART)定制开发的64个短偶极子天线及其接收器阵列，部署在Gauribidanur天文台（13.604 N, 77.427 E），用于观测130-350 MHz频段的明亮脉冲星（Pulsars）和太阳瞬变现象（Solar transients）。

Method: 采用64个短偶极子组成的菱形布局LPDA（对数周期偶极子天线）阵列，通过东西和南北方向的对角线排列实现高旁瓣抑制，并利用相反极化对组合实现双极化操作。

Result: GBD-DART（Gauribidanur Diamond Array Radio Telescope）是由64个短偶极子天线和相关接收器组成的新型小型LPDA（对数周期偶极子天线）阵列，部署于Gauribidanur天文台（13.604 N, 77.427 E），用于研究130-350 MHz频段的明亮脉冲星和太阳瞬变现象。
...
### [46] [Prospects for the detection of gamma rays using Cherenkov telescopes enhanced by a ground array observatory](https://arxiv.org/abs/2602.00691)
...
**TL;DR**: 研究发现结合水切伦科夫探测器阵列(WCDs)的单镜小型成像大气切伦科夫望远镜(SST-1M)在10 TeV以上能区使单镜和立体观测的流量灵敏度分别提升60%和30%，主要归因于伽马/强子分辨能力的改善。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用混合观测站（结合单镜小型成像大气切伦科夫望远镜(SST-1M)与水切伦科夫探测器阵列(WCDs)）提升甚高能伽马射线探测灵敏度。

Data: 数据来源于蒙特卡洛模拟（Monte Carlo simulations）和位于高海拔水切伦科夫探测器阵列（Water-Cherenkov Detectors, WCDs）内的单镜小型成像大气切伦科夫望远镜（SST-1M）。

Method: 采用详细的蒙特卡洛模拟（Monte Carlo simulations）分析单镜小型成像大气切伦科夫望远镜（SST-1M）与高海拔水切伦科夫探测器阵列（WCD array）的混合观测数据，通过结合WCD阵列参数优化伽马/强子（gamma/hadron）区分能力。

Result: 在单镜小型成像大气切伦科夫望远镜（SST-1M）与广域水切伦科夫探测器（WCD）阵列的混合观测模式下，利用WCD阵列提供的额外参数可将10 TeV以上能区的单镜和立体观测流量灵敏度分别提升约60%和30%。
...
### [47] [GBD-DART-II: 175 MHz Polarimetric Observation of Pulsars from Gauribidanur and a New Pulsar Signal Processing Pipeline](https://arxiv.org/abs/2602.00736)
*Arul Pandian B,Joydeep Bagchi,Prabu Thiagaraj,K. B. Raghavendra Rao,Vinutha Chandrashekar*

**TL;DR**: 为高里比达努尔(Gauribidanur)射电天文台的钻石阵列射电望远镜(Diamond Array Radio Telescope)开发了新的脉冲星信号处理流程，实现了130-350MHz频段内脉冲星观测、实时数据处理及偏振测量，并通过对五颗明亮脉冲星在175MHz的观测验证了其性能。


<details>
  <summary>Details</summary>

Background: 开发用于高灵敏度低频射电脉冲星观测的新型信号处理系统（pipeline），提升Gauribidanur射电天文台Diamond阵列望远镜在130-350 MHz波段的脉冲星探测与研究能力。

Data: 数据来源包括使用钻石阵列射电望远镜（Diamond Array Radio Telescope）在130-350 MHz频段采集的32个离轴双极化对数周期天线（LPDAs）原始电压信号、高低分辨率全极化频谱数据、已知脉冲星的全极化折叠轮廓档案（full-polar folded profile archives），以及人工生成的脉冲星信号和强天体源验证数据。

Method: 采用定制开发的Python程序、FFT库、DSPSR、PSRCHIVE和Presto模块构建了实时数据捕获与分析流水线，支持相干与非相干消色散(coherent and incoherent dedispersion)处理。

Result: 为高里比达努尔(Gauribidanur)射电天文台的钻石阵列射电望远镜(Diamond Array Radio Telescope)开发了一套新的脉冲星信号处理流程(pipeline)，用于观测脉冲星。
...
### [48] [Method on Using Shadow Altitude to Remove Geocoronal H$α$](https://arxiv.org/abs/2602.00779)
*Wai-Kiu Ricky Wong,Renbin Yan,Zesen Lin*

**TL;DR**: 基于SDSS-IV/MaStar巡天的空白天区光纤观测数据，建立了地冕Hα发射与阴影高度（shadow altitude）之间的经验关系（均方根分数散度为23.52%），可用于预测并扣除光谱观测中的地冕污染，从而支持银河系Hα的可靠研究。


<details>
  <summary>Details</summary>

Background: 研究背景聚焦于利用光谱巡天（spectroscopic surveys）开展星系星际介质（ISM）研究时，地球冕Hα发射（geocoronal Hα emission）对银河系Hα观测的污染问题。

Data: 数据来源于SDSS-IV/MaStar巡天中空白天区的光纤观测数据。

Method: 基于SDSS-IV/MaStar巡天的空白天区光纤观测数据，建立了地球气辉Hα发射与阴影高度（shadow altitude）之间的经验关系模型。

Result: 基于SDSS-IV/MaStar巡天的空白天区光纤观测数据，建立了地冕Hα发射与阴影高度（shadow altitude）之间的经验关系，均方根分数散度为23.52%，可用于预测并扣除观测光谱中的地冕Hα污染。
...
### [49] [Factorized neural posterior estimation for rapid and reliable inference of parameterized post-Einsteinian deviation parameters in gravitational waves](https://arxiv.org/abs/2602.01319)
*Yong-Xin Zhang,Tian-Yang Sun,Chun-Yu Xiong,Song-Tao Liu,Yu-Xin Wang,Shang-Jie Jin,Jing-Fei Zhang,Xin Zhang*

**TL;DR**: 提出一种基于因子化神经后验估计的快速波形参数推断方法，通过独立建模九个后爱因斯坦参数(ppE)并整合先验信息，在保持统计可靠性的同时将引力波(GW)参数估计速度提升9万倍。


<details>
  <summary>Details</summary>

Background: 引力波（GW）直接探测为广义相对论（GR）提供了验证，但通过引力波测试广义相对论需要估计波形模型中的参数化后爱因斯坦（ppE）偏差参数。

Data: data部分：研究使用来自LIGO探测器的双黑洞引力波信号（binary black hole signals）作为观测数据源。

Method: 采用因子化神经后验估计框架，结合条件嵌入网络整合先验信息，并利用卷积神经网络和残差神经网络的混合模型进行特征提取。

Result: 提出了一种基于因子化神经后验估计的深度学习框架，通过独立归一化流模型和条件嵌入网络高效估计引力波波形中的后爱因斯坦参数（ppE deviation parameters），相比传统MCMC方法实现了9×10^4倍的加速且保持统计可靠性。
...
### [50] [Astro-COLIBRI: An Innovative Platform for Real-Time Multi-Messenger Astrophysics](https://arxiv.org/abs/2602.02058)
*Bernardo Cornejo Avila,Sofia Bisero,Mickael Costa,Antoine Ciric,Ilja Jaroschewski,Weizmann Kindrébéogo,Fabian Schussler*

**TL;DR**: 开发了Astro-COLIBRI平台，通过整合实时多信使（multi-messenger）与多波段（multi-wavelength）警报数据、提供可视化及观测条件评估，支持天体瞬变现象（如超新星、快速射电暴）的高效追踪与研究。


<details>
  <summary>Details</summary>

Background: 近年来瞬变现象（如超新星、新星、快速射电暴、伽马射线暴和恒星耀斑）的发现以及高能中微子和引力波等新宇宙信使的出现彻底改变了天体物理学研究。

Data: 数据来源包括实时接收的多信使（multi-messenger）和多波段（multi-wavelength）警报，通过公共RESTful API、实时数据库和云端警报系统整合。

Method: 采用实时数据处理管道（data pipeline）和RESTful API架构，结合多信使（multi-messenger）与多波段（multi-wavelength）上下文分析技术。

Result: Astro-COLIBRI是一个集成了公共RESTful API、实时数据库、云端警报系统和用户友好界面的综合平台，用于实时处理多信使和多波段天文事件警报并为其提供上下文信息。
...
### [51] [JWST NIRSpec's Cosmic Ray Experience at L2](https://arxiv.org/abs/2602.02134)
*Bernard J. Rauscher,D. J. Fixsen*

**TL;DR**: 研究分析了詹姆斯·韦伯太空望远镜（JWST）近红外光谱仪（NIRSpec）在L2轨道上受宇宙射线（cosmic ray）影响的特征，发现屏蔽后的宇宙射线撞击率从4.3降至2.3离子·cm⁻²·s⁻¹（前三年），典型撞击影响7.1像素并沉积6 keV能量，预计2027年将回升至4.3离子·cm⁻²·s⁻¹，2030年代初或达6离子·cm⁻²·s⁻¹，同时探讨了罕见“雪球”撞击和次级簇射事件的来源及对校准的挑战。


<details>
  <summary>Details</summary>

Background: 研究聚焦于分析詹姆斯·韦伯太空望远镜（JWST）近红外光谱仪（NIRSpec）在日地L2晕轨道运行时遭遇的高能宇宙射线（cosmic ray）相互作用及其对探测器性能的影响。

Data: 数据来源于JWST（James Webb Space Telescope）NIRSpec（Near-Infrared Spectrograph）在日地L2晕轨道（Sun/Earth-Moon L2 halo orbit）上获取的屏蔽宇宙射线（cosmic ray）暗场曝光观测数据。

Method: 采用屏蔽暗曝光技术分析宇宙射线(cosmic ray)在JWST近红外光谱仪(NIRSpec)中的相互作用特性。

Result: 在JWST（James Webb Space Telescope）NIRSpec（Near-Infrared Spectrograph）屏蔽暗曝光中，宇宙射线（cosmic ray）的典型撞击影响约7.1个像素并在HgCdTe探测器材料中沉积约6 keV能量（相当于约5200个电荷）。
...
### [52] [LISA Non-Linear Dynamics and Tilt-To-Length Coupling](https://arxiv.org/abs/2602.02203)
*Lavinia Heisenberg,Henri Inchauspé,Sarah Paczkowski,Ricardo Waibel*

**TL;DR**: 首次采用闭环、非线性和时变动力学模拟航天器与光学平台角抖动（angular jitter），证实当前倾斜-长度（TTL）耦合系数下噪声影响有限，并通过最小二乘估计器验证正弦机动可将TTL系数推断精度提升至0.1%。


<details>
  <summary>Details</summary>

Background: 研究聚焦于LISA（激光干涉空间天线）任务中倾斜-长度（TTL）耦合噪声的建模与抑制，这是激光频率噪声被抑制后最主要的仪器噪声来源之一。

Data: data：研究首次采用闭环、非线性且时变的动力学模型模拟了航天器和光学平台（optical benches）的详细角度抖动（angular jitters）数据。

Method: 采用闭环非线性时变动力学模型模拟航天器和光学平台的角抖动（angular jitter），并结合时域最小二乘估计器（Least Squares estimator）推断倾斜-长度耦合（TTL）参数。

Result: TTL（Tilt-To-Length）耦合的影响在当前耦合系数估计下是有限的。
...
### [53] [Simple Improved Reference Subtraction for H4RG, H2RG, and H1RG Near-infrared Array Detectors](https://arxiv.org/abs/2602.02253)
*Bernard J. Rauscher,Dale J. Fixsen,Gregory Mosby*

**TL;DR**: 这篇论文提出了一种基于线性代数和训练数据的简单改进参考像素减法（SIRS）方法，利用Teledyne H4RG、H2RG和H1RG近红外阵列探测器中的参考像素（reference pixels）来最优降低相关读取噪声（correlated read noise），并通过地面测试数据验证了其有效性。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用近红外阵列探测器（H4RG、H2RG和H1RG）中的参考像素（reference pixels）通过线性代数和训练数据优化降低相关读取噪声（correlated read noise）的方法。

Data: 数据来源为南希·格蕾丝·罗曼空间望远镜项目（Nancy Grace Roman Space Telescope Project）的H4RG地面测试数据。

Method: 采用线性代数和训练数据结合参考像素（reference pixels）进行最小二乘优化的简单改进参考减法（Simple Improved Reference Subtraction, SIRS）。

Result: 简单改进参考减法（SIRS）利用线性代数和训练数据，通过处理嵌入的参考像素（reference pixels）数据流，以最小二乘法最优地降低相关读取噪声（correlated read noise）。
...
### [54] [The Nulling Interferometry Cryogenic Experiment (NICE): Architecture, requirements, and preliminary warm precursor results](https://arxiv.org/abs/2602.02279)
...
**TL;DR**: NICE（Nulling Interferometry Cryogenic Experiment）测试台通过验证LIFE（Large Interferometer For Exoplanets）任务所需的高对比度（消光深度<10^-5）和高通量（>17%）中红外（4.7 μm）零干涉技术，提升了其技术成熟度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于提升大型外行星干涉仪（LIFE）空间任务的中红外零干涉（nulling interferometry）技术成熟度，以抑制宿主恒星眩光并精确测量系外行星微弱光谱。

Data: 数据来源为实验室搭建的中红外零干涉测试平台NICE（Nulling Interferometry Cryogenic Experiment）及其前驱常温测试平台\enquote{Warm Bench}，使用偏振窄带4.7微米光源进行验证。

Method: 采用中红外零干涉（nulling interferometry）技术结合低温（cryogenic）操作的高通量（high throughput）光束合成器（beam combiner）验证架构。

Result: NICE（Nulling Interferometry Cryogenic Experiment）测试台成功在4.7微米偏振窄带光源下实现了所需消光深度（<10⁻⁵）并在其中一个消光通道中实现了所需吞吐量（>17%）。
...
### [55] [Surface Interactions in Photon Monte Carlo Simulations](https://arxiv.org/abs/2602.02321)
*J. R. Peterson,D. Valls-Gabaud,A. Dutta,C. Kim,G. Sembroski*

**TL;DR**: 采用蒙特卡洛方法实现了光子与望远镜镜面（包括裸界面、单层/多层镀膜）及污染物相互作用的综合模拟，验证了微粗糙度散射理论对WIYN ODI观测点扩散函数(PSF)翼部和尘埃环的预测能力，并区分了镜面微粗糙度与湍流PSF模式的影响。


<details>
  <summary>Details</summary>

Background: 研究聚焦于光子与望远镜镜面及透镜相互作用的蒙特卡洛模拟，涵盖多层涂层、微粗糙度及污染物影响的电磁特性建模。

Data: 使用蒙特卡洛方法模拟光子与表面相互作用，包括裸界面、单层界面、保护层涂层和多层涂层，并采用全电磁解模拟波长和角度依赖性；通过蒙特卡洛实现的哈维-沙克散射理论处理微粗糙度功率谱对应的角分布；利用米氏散射高效模拟单个污染物导致的表面污染相互作用。

Method: 采用蒙特卡洛方法模拟光子表面相互作用，结合全电磁解处理波长与角度依赖性，应用哈维-沙克散射理论（Harvey-Shack scatter theory）处理微粗糙度，并利用米氏散射（Mie scattering）模拟污染物影响。

Result: 采用蒙特卡洛方法实现了光子与表面相互作用的综合模拟，可有效模拟光与望远镜镜片和镜面的相互作用。
...
### [56] [Depth Calibration of Double-sided Strip Germanium Detectors for the Compton Spectrometer and Imager Satellite](https://arxiv.org/abs/2602.02442)
...
**TL;DR**: 首次实现了采用COSI卫星几何结构的探测器深度校准，通过基于Julia的SolidStateDetectors.jl模拟将电荷收集时间差映射为深度，并在59.5 keV和122.1 keV能量下分别实现<0.9 mm和<0.6 mm（FWHM）的像素级深度分辨率。


<details>
  <summary>Details</summary>

Background: 研究聚焦于开发具有三维位置重建能力的双面条带高纯锗探测器（Double-sided strip high-purity germanium detectors），用于高能天体物理学和太阳物理学（heliophysics）的空间应用。

Data: 数据来源包括基于Julia的模拟软件SolidStateDetectors.jl生成的仿真数据以及COSI卫星任务中采用专用集成电路(ASIC)读取的实际探测器观测数据。

Method: 采用基于Julia的模拟软件SolidStateDetectors.jl进行电荷收集时间差与作用深度映射的深度校准（depth calibration），并通过与实测时序分布对比验证该模型。

Result: 首次实现了基于COSI卫星几何结构的探测器深度校准，利用Julia模拟工具SolidStateDetectors.jl将电荷收集时间差映射为深度，并通过与实测时序分布对比验证了该方法的有效性。
...
### [57] [X-ray characterization of fully-depleted p-channel Skipper-CCDs for the DarkNESS mission](https://arxiv.org/abs/2602.02461)
...
**TL;DR**: DarkNESS任务通过测量Skipper-CCD（Skipper电荷耦合器件）在217 MeV质子辐照前后的X射线响应，量化了低地球轨道辐射损伤导致的电荷转移效率下降和能量分辨率退化，为评估探测器在轨寿命性能提供了依据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用配备Skipper传感器（Skipper-CCDs）的暗物质纳米卫星（DarkNESS）在低地球轨道（low-Earth orbit）探测暗物质（dark matter）衰变产生的X射线谱线。

Data: 数据来源于217 MeV质子辐照实验（fluence为8.4 x 10^10 protons cm^-2）和55Fe源X射线测量，对比了辐照区域与未辐照区域及未辐照参考传感器（Skipper-CCDs）的能量分辨率。

Method: 采用217 MeV质子束辐照实验和55Fe源X射线能谱测量，量化评估了Skipper-CCD（Skipper电荷耦合器件）在空间质子辐射环境下的电荷转移效率下降及能谱分辨率退化效应。

Result: Skipper-CCD（Skipper Charge-Coupled Device）在217 MeV质子辐照后能量分辨率显著下降，辐照区域的55Fe特征X射线峰展宽程度明显高于未辐照区域。
...
### [58] [Nested Fermi and eROSITA bubbles require very similar $\sim10^{55}$ erg collimated Galactic-center outbursts; their asymmetry indicates an eastern density gradient](https://arxiv.org/abs/2602.00226)
*Arka Ghosh,Uri Keshet,Santanu Mondal*

**TL;DR**: 研究发现银河系中心存在两对嵌套的双极气泡（eROSITA气泡和费米气泡），其边缘为强前向激波，通过模拟证实它们由不同准直爆发驱动，能量约10^55尔格，速度约2000 km/s，可能源于相隔约1000万年的相同爆发事件。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系中心喷发形成的嵌套双极气泡结构（eROSITA气泡和费米气泡）的动力学特征与形成机制。

Data: 数据来源包括银河系中心区域观测到的eROSITA气泡（RBs）和费米气泡（FBs）的X射线观测数据，以及验证性二维和三维流体动力学模拟（hydrodynamic simulations）数据。

Method: 采用分层一维模型（stratified 1D model）并通过二维和三维流体动力学模拟（2D and 3D hydrodynamic simulations）验证，约束气泡起源并重现X射线观测结果。

Result: 银河系中心的两对嵌套双极气泡（eROSITA气泡（RBs）和费米气泡（FBs））由不同准直爆发驱动，RB处于减速初期而FB仍在RB激波介质中弹道膨胀，两次爆发具有相似参数（半开角约4度、速度约2000 km/s、能量约10^55 erg）且可能间隔约10 Myr，纵向不对称性支持东向环境密度梯度而非西向风解释。
...
### [59] [A Serendipitous NuSTAR Detection of a Giant Radio Source Harboring an Obscured Active Galactic Nucleus](https://arxiv.org/abs/2602.01647)
*Vaidehi S. Paliya,S. Marchesi,X. Zhao,D. J. Saikia,Moumita Pal,Somak Raychaudhury*

**TL;DR**: 首次通过多波段观测确认巨型射电源（GRS）J1128+5831为具有高柱密度中性氢遮蔽（>10²³ cm⁻²）且辐射效率高的II型活动星系核（AGN），其陡峭射电谱（α=-0.86）和低核主导性（C_D=-2.4）表明该源以大倾角被观测到。


<details>
  <summary>Details</summary>

Background: 研究聚焦于探索巨型射电源（GRSs）这一宇宙中由单个星系产生的最大结构在高能波段（X射线等）的物理特性，特别是针对目前唯一被核光谱望远镜阵列（NuSTAR）偶然探测到的NuSTAR J112829+5831.8（J1128+5831）开展多波段联合研究。

Data: 数据来源于核光谱望远镜阵列(NuSTAR)、钱德拉X射线天文台(Chandra X-ray Observatory)、哈勃空间望远镜(Hubble Space Telescope)、XMM-牛顿卫星(XMM-Newton)、低频阵列(Low Frequency Array)、NRAO甚大天线阵巡天(NRAO VLA Sky Survey)、甚大天线阵巡天(Very Large Array Sky Survey)和暗能量光谱仪(Dark Energy Spectroscopic Instrument)。

Method: 采用多波段（multi-wavelength）观测数据与X射线光谱分析（X-ray spectral analysis）技术，结合射电（radio）频谱斜率测定与光学（optical）发射线特征识别。

Result: J1128+5831是一个被中性氢柱密度超过$10^{23}$ cm$^{-2}$遮蔽的2型活动星系核（AGN），由辐射效率较高的吸积系统驱动。
...
### [60] [Evaluating the Contribution of Active Galactic Nuclei to the Diffuse High-Energy Neutrino Flux](https://arxiv.org/abs/2602.02390)
*Samyak Jain,Dan Hooper,Francis Halzen*

**TL;DR**: 通过分析10年IceCube数据发现，X射线明亮（X-ray-bright）的塞弗特型活动星系核（Seyfert-type AGN）可能贡献了11.2%至全部的弥散中微子通量（diffuse neutrino flux），而伽马射线明亮（gamma-ray-bright）的耀变体（blazars）贡献不超过16%，表明中微子产生于光学厚环境（optically thick environments）如超大质量黑洞（supermassive black holes）周围的冕区（coronae）。


<details>
  <summary>Details</summary>

Background: 高能中微子（high-energy neutrinos）与活动星系核（AGN）的关联研究揭示了这类天体对冰立方（IceCube）观测到的弥散中微子通量的潜在重要贡献。

Data: 研究使用了公开可用的10年IceCube数据，并参考了Swift-BAT目录中的X射线明亮活动星系核(AGN)数据。

Method: 采用10年公开冰立方(IceCube)数据对X射线和伽马射线明亮活动星系核(AGN)进行系统性群体分析，评估其对弥漫中微子通量的贡献。

Result: 伽马射线明亮耀变体（gamma-ray-bright blazars）最多贡献16%的IceCube总弥散中微子通量。
...
### [61] [Multi-Messenger Modeling of Low-Luminosity Gamma-Ray Bursts](https://arxiv.org/abs/2602.02367)
*Shiqi Yu,Bing Theodore Zhang*

**TL;DR**: 研究发现低光度伽马暴（LL GRBs）具有多样化的轻强子（leptohadronic）辐射机制，其中GRB 060218和GRB 100316D因最高宇宙射线负载因子（ξ_p∼0.2-1.6）成为高效中微子源，支持其作为多信使天体物理候选目标。


<details>
  <summary>Details</summary>

Background: 低光度伽马射线暴（LL GRBs）作为宇宙中最强大的暂现源亚类，在高能天体物理中微子研究领域具有重要潜力。

Data: 研究使用了七例低光度伽马射线暴（LL GRBs）的多波段观测数据（multi-wavelength observations），包括GRB 060218和GRB 100316D等符合激波突破（SBO）情景的极低光度暴（$L_{γ, \rm iso} \sim 10^{46}$-$10^{47}\rm~erg~s^{-1}$）。

Method: 采用多波段观测数据约束轻子强子(leptohadronic)过程中的磁场、非热电子和质子能量密度，并通过机器学习分析拟合参数来研究低光度伽马暴(LL GRBs)的宇宙射线(CR)负载因子(ξ_p)。

Result: 低光度伽马射线暴(LL GRBs)的宇宙射线(CR)负载因子(ξ_p)后验中值范围为0.2-1.6，其中GRB 060218和GRB 100316D具有最高负载因子，是最有效的中微子产生源。
...
### [62] [Carbon measurements in two ultra-faint dwarf galaxies: Grus II and Tucana IV](https://arxiv.org/abs/2602.00306)
...
**TL;DR**: 通过光谱分析发现超暗弱矮星系（UFDs）Grus II和Tucana IV中存在碳增强贫金属星（CEMP-no），为研究宇宙早期第一代恒星（仅含H和He）的化学增丰过程提供了关键证据。


<details>
  <summary>Details</summary>

Background: 研究聚焦于利用超暗弱矮星系（UFDs）中的碳增强贫金属星（CEMP-no）探索宇宙早期第一代恒星（仅含氢和氦）的化学产物及其对原始星系首次化学增丰的影响。

Data: 数据来源于使用甚大望远镜(VLT)的多目标光谱仪FLAMES/Giraffe获取的光谱，包括CaII三重态(~8498 Å、8542 Å、8662 Å)附近的红色光谱和覆盖~4300 Å CH带的蓝色光谱，共分析了Grus II的21颗成员候选体和Tucana IV的17颗成员候选体（含红巨星分支(RGB)和水平分支(HB)恒星）。

Method: 采用多目标光谱仪FLAMES/Giraffe在甚大望远镜(VLT)上获取光谱数据，包括CaII三重态(8498 Å、8542 Å、8662 Å)附近红光谱和~4300 Å CH带蓝光谱，用于测量径向速度、[Fe/H]及碳丰度。

Result: 在Grus II和Tucana IV两个超暗弱矮星系（UFDs）中发现了碳增强极贫金属星（CEMP-no），其中Grus II有5颗（3颗[Fe/H]~-3且[C/Fe]>+1，2颗[Fe/H]略高且[C/Fe]>+0.7），Tucana IV有1颗（[Fe/H]=-2.75且[C/Fe]=+0.83）。
...
### [63] [Radial and Rotational Velocities of a Volume-Complete Sample of M Dwarfs with Masses 0.1-0.3 Msun within 15 parsecs](https://arxiv.org/abs/2602.00382)
...
**TL;DR**: 通过对15秒差距内413颗质量为太阳10-30%的M矮星（M dwarfs）进行五年高分辨率光谱观测，首次系统测量了其径向速度（RV）和自转展宽（$v \sin i$），发现71%单星自转速度低于3.4 km/s检测限，81%和8%分别属于薄盘和厚盘成员，且两者伴星率（22% vs 21%）无显著差异，同时新发现7个多星系统和6个长周期双星候选体。


<details>
  <summary>Details</summary>

Background: 研究聚焦于15秒差距内质量约为太阳10-30%的M矮星（M dwarfs）的径向速度（RV）、自转展宽（$v \sin i$）及银河系薄盘（thin disk）与厚盘（thick disk）成员归属的统计分析。

Data: 数据来源于对413颗质量相当于太阳10-30%的M矮星（M dwarfs）进行的为期五年多历元高分辨率光谱观测，这些目标构成15秒差距（parsecs）内体积完备样本。

Method: 采用多历元高分辨率光谱（multi-epoch, high-resolution spectra）测量径向速度（RV）和旋转展宽（$v \sin i$），并结合天体测量数据计算银河系空间运动以划分薄盘（thin disk）和厚盘（thick disk）成员。

Result: 81%和8%的样本分别高度可能属于薄盘（thin disk）和厚盘（thick disk）。
...
### [64] [Two late-T dwarfs at kiloparsec distances revealed by JWST UNCOVER survey](https://arxiv.org/abs/2602.01894)
*D. H. Li,Z. H. Zhang,H. H. Peng,M. C. Galvez-Ortiz,S. Y. Zhou,H. R. A. Jones*

**TL;DR**: 利用詹姆斯·韦伯太空望远镜（JWST）对A2744深场观测发现两颗T型矮星（T dwarf）候选体和一颗可能的L型亚矮星（L subdwarf）候选体，首次测量了0.094/平方角分的T型矮星面密度并揭示银河系薄盘（thin disc）与厚盘（thick disc）外缘的T5-T7矮星密度显著低于太阳邻域，而厚盘内T8矮星密度与本地值相当。


<details>
  <summary>Details</summary>

Background: 研究主题为利用詹姆斯·韦伯太空望远镜(JWST)深场观测数据探究褐矮星(brown dwarf)在千秒差距(kiloparsec)距离上的空间分布密度及其银河系结构关联。

Data: 数据来源于詹姆斯·韦伯太空望远镜（James Webb Space Telescope）对A2744深场（deep field）的宽带近红外至中红外测光观测。

Method: 采用基于测光颜色的初始筛选结合光谱能量分布拟合（SED fitting）方法，同时匹配恒星大气模型和高红移星系模板。

Result: 在A2744附近的詹姆斯·韦伯太空望远镜深场中发现了两个可靠的T型矮星候选体和一个可能的L型亚矮星候选体，其T型矮星表面数密度为每平方角分0.094个，低于先前预测但与数量级一致。
...
### [65] [Gravitational trapping and ram pressure trapping of ultracompact and hypercompact H II regions](https://arxiv.org/abs/2602.01900)
*Lauren Martini,André Oliva,Rolf Kuiper,Pamela Klaassen*

**TL;DR**: 该研究通过二维轴对称辐射流体动力学模拟发现，早期H II区（H II region）在无辐射压力时会被引力（gravity）永久束缚在超致密（ultracompact）相，而加入辐射压力后虽能逃逸引力束缚但仍受冲压（ram pressure）作用在大尺度上受限，且初始质量库（mass reservoir）的中心密度和梯度决定了束缚相的存在与否，其中超致密束缚相表现出与观测一致的半径闪烁变化（flickering variation）。


<details>
  <summary>Details</summary>

Background: 研究聚焦于早期电离氢区（H II regions）的演化动力学，探讨重力、辐射压（radiation pressure）和冲压（ram pressure）等物理过程如何导致电离氢区在超致密（ultracompact）和超超致密（hypercompact）相中被捕获或逃逸，以解决其寿命与观测数量不符的理论矛盾。

Data: 研究中使用的数据来源为二维轴对称辐射流体动力学模拟（two-dimensional, axially symmetric radiation hydrodynamic simulations），模拟内容包括年轻膨胀H II区（H II regions）的演化过程以及早期恒星和吸积盘（disk）形成阶段。

Method: 采用二维轴对称辐射流体动力学模拟（radiation hydrodynamic simulations）分析早期H II区的动力学演化，重点考察重力（gravity）与冲压压力（ram pressure）对热压膨胀（thermal pressure expansion）、辐射压（radiation pressure）及离心力（centrifugal force）的平衡作用。

Result: 通过二维轴对称辐射流体动力学模拟发现，辐射压力（radiation pressure）的存在使H II区摆脱引力束缚但遭遇冲压束缚（ram pressure trapping），而初始质量库（mass reservoir）的中心密度梯度决定了是否出现束缚阶段。
...
### [66] [X-ray counterparts to stellar MeerKAT Galactic-plane compact radio sources](https://arxiv.org/abs/2602.02054)
*O. D. Egbo,P. J. Groot,D. A. H. Buckley,J. Robrade,A. D. Schwope,S. Freund,P. C. Schneider,B. Stelzer*

**TL;DR**: 通过结合MeerKAT射电数据与X射线观测，在银河系平面识别出137颗同时具有射电和X射线辐射的活跃恒星，其亮度温度（$T_B$）介于$10^7$至$10^{12}$ K之间（除两个异常值外），样本整体偏离经典Güdel-Benz关系（热日冕X射线与非热回旋同步加速射电辐射的关联），表明该关系可能仅代表上限而非严格相关性，且早型恒星在eROSITA数据中显示出低于典型$\log (L_{\rm X}/L_{\rm bol}) \sim -3$关系的特征。


<details>
  <summary>Details</summary>

Background: 研究聚焦于银河系平面内磁活动恒星（magnetically active stars）的非热射电辐射与热X射线辐射的关联性，特别是验证经典Güdel-Benz关系的普适性。

Data: 数据来源包括SARAO MeerKAT Galactic Plane Survey的射电数据、ROSAT All-Sky Survey的软X射线观测数据以及SRG/eROSITA首次发布数据（eRASS1）。

Method: 采用交叉匹配(cross-matching)方法结合MeerKAT射电数据与ROSAT、eRASS1软X射线观测，通过计算射电亮度温度(radio-brightness temperature)和射电-X射线光度比验证Güdel-Benz关系。

Result: result：在137颗同时具有射电和X射线探测的恒星中，亮度温度（$T_B$）介于$10^7$至$10^{12}$ K之间（AXJ1600.9-5142和HD~124831除外），样本整体位于经典Güdel-Benz关系下方，表明该关系代表上限而非紧密相关性，且eROSITA探测显示早型恒星偏离典型的$\log (L_{\rm X}/L_{\rm bol}) \sim -3$关系。
...
### [67] [Noble gases Neon and argon: a role for the chemical patterns of multiple populations in globular clusters?](https://arxiv.org/abs/2602.02085)
*P. Ventura,F. D'Antona,M. Tailo,A. P. Milone,F. Dell'Agli,A. F. Marino*

**TL;DR**: 研究发现通过调整初始氖(Neon)丰度和降低质量损失率(mass-loss rate)能更好解释球状星团NGC 2808中极端星群(E group)的钠(Sodium)富集和氧(Oxygen)、镁(Magnesium)贫化现象，并提出该星群可能具有略低金属丰度(metallicity)。


<details>
  <summary>Details</summary>

Background: 研究聚焦于AGB（渐近巨星支）模型中钠元素破坏机制及其对球状星团NGC 2808内极端恒星群（group E）元素丰度模式的解释，探索初始氖(neon)丰度提升与质量损失率降低对氧、氮、镁等轻元素循环的影响。

Data: 数据来源为AGB（渐近巨星支）模型模拟数据，包括初始氖(Ne)丰度调整（提高2-4倍）和质量损失率降低的数值实验。

Method: 采用恒星演化模型（stellar evolution model）结合改进的质量损失率（mass-loss rate）和初始氖丰度（initial neon abundance）调整，通过热底部燃烧（hot bottom burning）模拟AGB星（asymptotic giant branch）中钠破坏过程。

Result: result：提高初始氖(neon)丰度并降低质量损失率(mass-loss rate)能更好地解释NGC 2808中极端星群(group E)的钠(sodium)富集和氧(oxygen)、镁(magnesium)贫化现象，但需略微降低铁(iron)含量以匹配观测数据。
...
### [68] [Massive stellar cannibals: How stellar mergers drive mass-loss in extremely massive stars](https://arxiv.org/abs/2602.02141)
*J. Roman-Garza,T. Fragos,C. Charbonnel,L. Ramírez-Galeano,M. Kruckow,E. Farag*

**TL;DR**: 研究发现低金属丰度（low metallicity）高密度（dense）环境中大质量比（mass ratio <0.1）恒星并合会导致极端大质量恒星（>1000 M⊙）损失10-30%系统质量，证明并合诱导的质量损失对极端大质量恒星形成和演化具有重要影响。


<details>
  <summary>Details</summary>

Background: 研究探讨了极低金属丰度（Z<10^-1 Z⊙）和高密度（≳10^3 M⊙ pc^-3）恒星环境中通过恒星合并形成极高质量（>10^3 M⊙）和超大质量恒星的可行性及其对宇宙化学演化的潜在影响。

Data: 研究使用了一维流体动力学、恒星结构与演化代码MESA（Modules for Experiments in Stellar Astrophysics）模拟恒星螺旋过程，考虑了质量超过1000太阳质量（M⊙）的主星与质量小于100太阳质量（M⊙）的伴星（质量比<0.1）的相互作用。

Method: 采用一维流体动力学、恒星结构与演化代码MESA（Modules for Experiments in Stellar Astrophysics）模拟恒星螺旋运动，并分析轨道能量通过流体动力学和引力拖曳力损失导致的包层热沉积过程。

Result: 在低金属丰度（Z<10^-1 Z☉）和高密度（≳10^3 M☉ pc^-3）环境中，恒星合并可能导致质量损失达系统总质量的10-30%，且大部分轨道能量用于抛射物质。
...
### [69] [Painting a Family Portrait of the Yellow Super- and Hypergiants in the Milky Way I. Constraining the Distances and Luminosities](https://arxiv.org/abs/2602.02449)
*A. Kasikov,A. Mehner,I. Kolka,A. Aret*

**TL;DR**: 通过结合共动恒星群成员关系和H I运动学地图，改进了28颗黄特超巨星(YHGs)和黄超巨星(YSGs)的距离估计，并更新了20颗恒星的亮度，发现YHGs普遍具有更高亮度(log L/L > 5.4)且起源环境更复杂。


<details>
  <summary>Details</summary>

Background: 银河系演化大质量恒星（如黄特超巨星YHGs和黄超巨星YSGs）的距离测量因盖亚（Gaia）视差限制而存在不确定性，制约了其基本参数（如半径、光度）测定和演化状态推断。

Data: 数据来源包括盖亚卫星第三次数据释放（Gaia DR3）的天体测量数据、文献中收集的35颗高光度黄色大质量恒星样本、以及银河系中性氢（H I）运动学图。

Method: 采用盖亚(Gaia) DR3天体测量数据识别恒星在星团和OB星协中的可能成员关系，并结合共动恒星的视差推导距离，同时利用银河系H I运动学图验证径向速度一致性。

Result: 通过识别共动恒星群成员关系，改进了28颗黄超巨星（YHGs）和黄特超巨星（YSGs）的距离估计，并基于H I运动学图为另外6颗恒星提供了距离估计。
...
### [70] [Detecting and Characterizing Companions with a Calibrated Gaia DR2, DR3, and Hipparcos Catalog (G23H)](https://arxiv.org/abs/2602.00235)
...
**TL;DR**: 通过整合Hipparcos和Gaia数据并开发Octofitter联合似然模型，实现了对大量伴星（包括恒星双星和类木系外行星系统）完整轨道后验概率的高精度计算，验证了94个类木系外行星系统的独立证据，且无假阳性发现。


<details>
  <summary>Details</summary>

Background: 该研究聚焦于利用盖亚(Gaia)和依巴谷(Hipparcos)天体测量数据联合建模，提升系外行星及伴星的轨道参数反演精度。

Data: 数据来源包括盖亚(Gaia) DR2自行运动数据、DR3-DR2比例位置差、盖亚DR3参考框架、依巴谷-盖亚加速度星表(Hipparcos-Gaia Catalog of Accelerations)、依巴谷中间天体测量数据(Hipparcos intermediate astrometric data)、盖亚天体测量过量噪声(Gaia astrometric excess noise)以及盖亚径向速度变异性约束(Gaia radial velocity variability constraints)。

Method: 采用联合似然模型（joint likelihood model）在Octofitter中实现，通过边缘化处理盖亚（Gaia）未公开观测历元，整合了盖亚DR2自行、DR3-DR2缩放位置差、依巴谷-盖亚加速度星表（Hipparcos-Gaia Catalog of Accelerations）、依巴谷中间天体测量数据、盖亚天体测量超噪声及径向速度变异性约束。

Result: 基于盖亚(Gaia)DR2自行运动和DR3-DR2缩放位置差的校准数据，结合依巴谷(Hipparcos)-盖亚加速度目录等多源观测约束，开发了联合似然模型(Octofitter)，可统一计算大量伴星天体的完整轨道后验分布。
...
### [71] [A parametric model for externally irradiated protoplanetary disks with photoevaporative winds](https://arxiv.org/abs/2602.02011)
*Luke Keyte,Thomas J. Haworth*

**TL;DR**: 开发了名为"PUFFIN"的参数化框架，可高效生成受外部远紫外(FUV)辐射的原行星盘(protoplanetary disk)光致蒸发风(photoevaporative wind)密度结构，并证实外部FUV辐射通过间接加热机制显著增强了外盘中CO气相丰度。


<details>
  <summary>Details</summary>

Background: 研究聚焦于大质量恒星形成区中原行星盘(protoplanetary disks)在极端紫外辐射下的光致蒸发(photoevaporation)过程及其对盘演化和化学组成的影响。

Data: 数据来源于FRIED网格的流体动力学模拟（hydrodynamical simulations），覆盖恒星质量0.3至3.0 M_sun、盘半径20至150 au以及外部远紫外辐射场（FUV fields）100至100,000 G0的参数范围。

Method: 采用参数化框架（parametric framework）结合球面发散风配置（spherically diverging wind configuration）和FUV加热表面层（FUV-heated surface layer）平滑过渡模型，开发了快速生成原行星盘（protoplanetary disks）密度结构的Python工具PUFFIN。

Result: 开发了名为"PUFFIN"的参数化框架，可高效生成具有光致蒸发风（photoevaporative winds）的外部辐照原行星盘（protoplanetary disks）密度结构，其计算速度比流体动力学模拟快数个数量级。
...
### [72] [Todos estos edificios se hacen de diverso modo que en Europa: Estudio arqueoastronómico de las iglesias jesuíticas de Chiquitos](https://arxiv.org/abs/2602.01302)
*Alejandro Gangui*

**TL;DR**: 研究发现位于玻利维亚奇基托斯（Chiquitania）的耶稣会教堂半数建筑朝向太阳活动范围（solar range），其中三座精确朝向春秋分方向（equinoctial orientation），揭示了建筑师可能考虑光照效应的设计意图。


<details>
  <summary>Details</summary>

Background: 研究聚焦于基督教教堂空间布局（spatial layout）的历史演变及其与天文方位（astronomical orientation）的关联，尤其关注早期教堂主轴朝向东方（east-facing）的宗教象征意义与太阳升起的宇宙观联系。

Data: 数据来源包括对玻利维亚奇基托斯(Chiquitania)地区现存8座教堂和1座废墟的实地测量，以及基于平面图和卫星地图对第10座教堂的方位测量，同时结合了对周边地形的详细勘测。

Method: 采用实地测量和景观调查方法，结合卫星地图分析教堂主轴线方位，并探讨可能的天文关联模式。

Result: 在奇基托斯（Chiquitania）耶稣会城镇的历史教堂中，半数测量建筑的主轴方向落在太阳范围内，其中三座教堂以高精度朝向春分/秋分方向。
...
### [73] [Reheating in geometric Weyl-invariant Einstein-Cartan gravity](https://arxiv.org/abs/2602.00317)
*Ioannis D. Gialamas*

**TL;DR**: 研究在爱因斯坦-嘉当(Einstein-Cartan)框架下构建的Weyl不变纯引力理论，发现其动力学等效于广义相对论耦合类轴子赝标量场，能自然驱动宇宙暴胀，并证明再加热过程对暴胀观测量的预测具有关键影响。


<details>
  <summary>Details</summary>

Background: 研究探讨了爱因斯坦-嘉当（Einstein-Cartan）框架下的外尔不变（Weyl-invariant）纯引力理论及其与暴胀宇宙学的关联。

Data: 研究中使用的数据来源为理论模拟（simulation）数据。

Method: 采用爱因斯坦-嘉当(Einstein-Cartan)框架构建的Weyl不变纯引力理论，在爱因斯坦标架下动态等价于标准广义相对论与类轴子赝标量自由度的耦合。

Result: 在爱因斯坦-嘉当（Einstein-Cartan）框架下构建的Weyl不变纯引力理论动态等价于标准广义相对论与类轴子赝标量自由度的耦合，可自然驱动宇宙暴胀。
...
**TL;DR**: 通过引入暗能量与远平行引力（teleparallel gravity）中Nieh-Yan密度的耦合，解决了传统模型在状态方程参数$w=-1$边界处的不稳定性问题，实现了稳定的五元暗能量（quintom dark energy）演化场景。


<details>
  <summary>Details</summary>

Background: 该研究探讨了在远平行引力（teleparallel gravity）框架下，通过引入暗能量与聂-燕密度（Nieh-Yan density）的耦合来规避传统模型在宇宙学常数边界（$w=-1$）处产生微扰不稳定性（perturbative instabilities）的问题。

Data: 数据来源于DESI（Dark Energy Spectroscopic Instrument）合作组的最新观测结果。

Method: 采用引入暗能量与远平行引力（teleparallel gravity）中Nieh-Yan密度的耦合方法，消除动力学自由度中的暗能量扰动。

Result: 通过引入暗能量与尼尔-杨密度（Nieh-Yan density）的耦合，在远平行引力（teleparallel gravity）中消除了扰动不稳定性问题，同时不影响背景演化。
...
### [75] [Running Love Numbers of Charged Black Holes](https://arxiv.org/abs/2602.00349)
*Sergio Barbosa,Sylvain Fichet,Lucas de Souza*

**TL;DR**: 研究发现量子场论（QFT）真空中的虚粒子环使黑洞产生潮汐形变，通过微扰理论计算无自旋带电黑洞的静态潮汐响应（Love矩阵），揭示了电磁对偶性破缺下电-磁背景的$Z_2$对称关系，并证明强场区磁黑洞的QFT潮汐响应趋于饱和，暗示近极端暗区磁黑洞可通过引力波观测探测。


<details>
  <summary>Details</summary>

Background: 研究探讨了量子场论（QFT）真空中的虚粒子环如何使黑洞产生潮汐形变，并扩展了洛夫数（Love numbers）的概念至洛夫矩阵（Love matrices），以描述引力与电磁潮汐响应的耦合效应。

Data: 使用量子场论（QFT）真空中的虚粒子环扰动计算无自旋带电黑洞的静态潮汐响应，并推广了Love数的概念至Love矩阵（Love matrices）。

Method: 采用微扰理论方法（perturbative formalism）计算无自旋带电黑洞的静态潮汐响应，并推广了洛夫数（Love numbers）概念至洛夫矩阵（Love matrices）。

Result: 静态潮汐响应（static tidal response）通过引入爱矩阵（Love matrices）量化了无自旋带电黑洞在量子场论真空虚粒子环影响下的形变特性。
...
### [76] [Energy Absorption Interferometry](https://arxiv.org/abs/2602.00745)
*Stafford Withington,Willem Jellema*

**TL;DR**: 该论文系统阐述了能量吸收干涉测量法（EAI）的理论基础，首次提出了涉及采样、相位参考、模式重建和噪声的新技术，并建立了噪声传播模型以量化其对光谱和模态图误差的影响。


<details>
  <summary>Details</summary>

Background: 能量吸收干涉测量法（Energy Absorption Interferometry, EAI）是一种用于测量多体系统吸收能量的各自由度响应度及复数值空间极化形式的技术，其理论基础源于量子关联函数，适用于电磁、弹性和声场等多种激发类型。

Data: 数据来源包括电磁场（electromagnetic）、弹性场（elastic）和声场（acoustic）的量子关联函数（quantum correlation functions）理论计算，以及远红外（far-infrared）和光学器件的多模行为实验观测。

Method: 采用量子关联函数（quantum correlation functions）语言构建的能量吸收干涉测量法（Energy Absorption Interferometry, EAI），用于分析多体系统吸收能量的自由度响应特性与复数值空间极化形式。

Result: 能量吸收干涉测量法（EAI）是一种通过多体系统吸收能量的各个自由度来测量其响应度及复数值空间极化形式的技术。
